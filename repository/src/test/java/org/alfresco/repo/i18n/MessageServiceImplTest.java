...
public class MessageServiceImplTest extends TestCase implements MessageDeployer {
    ...
    private void addMessageResource(NodeRef rootNodeRef, String name, InputStream resourceStream)
            throws Exception {
    ...
    public void test8Reset() throws Exception {
        ...
        messageService.register(
                this); // register with message service to allow reset (via initMessages callback)
    ...
    public void test9LocaleMatching() {
        ...
        assertTrue(
                chineseMatches.contains(
                        messageService.getNearestLocale(
                                zh_CN_3,
                                options))); // must match the last variant - but set can have any
        // order an IBM JDK differs!
    ...
    public void testLocaleParsing() {
        ...
        assertEquals(
                new Locale("en", "GB", "cockney"), messageService.parseLocale("en_GB_cockney"));
    ...
    private void getMessagesWithParams() {
        ...
        assertEquals(
                VALUE_PARAMS, messageService.getMessage(MSG_PARAMS, new Object[] {PARAM_VALUE}));
        ...
        // Check the default value
        assertEquals(
                VALUE_FR_PARAMS, messageService.getMessage(MSG_PARAMS, new Object[] {PARAM_VALUE}));

        // Check values when overriding the locale
        assertEquals(
                VALUE_PARAMS,
                messageService.getMessage(
                        MSG_PARAMS, Locale.getDefault(), new Object[] {PARAM_VALUE}));
    }
    ...
    public void testDictionaryDAOLock() {
        ...
        try {
            if (ddt.isAlive() && mst.isAlive()) {
                fail(
                        "Deadlock: DictionaryDAOThread and MessageServiceThread are both still"
                                + " alive.");
            } else if (ddt.isAlive()) {
                fail(
                        "Possible deadlock with a background process: DictionaryDAOThread is still"
                                + " alive.");
            } else if (mst.isAlive()) {
                fail(
                        "Possible deadlock with a background process: MessageServiceThread is still"
                                + " alive.");
            } else if (!ddt.success) {

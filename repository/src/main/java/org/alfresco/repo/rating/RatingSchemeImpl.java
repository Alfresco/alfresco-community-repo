...
public class RatingSchemeImpl implements RatingScheme, BeanNameAware, InitializingBean {
    ...
    /**
     * Is the cm:creator of the content node allowed to apply ratings to it? <code>true</code> if
     * yes, else <code>false</code>.
     */
    private boolean selfRatingAllowed;

    /**
     * This property is used to determine where in the Alfresco content model the ratings rollup
     * aspect should go. If it is not injected, a default value of "cm" for the Alfresco content
     * model is used. Individual rating schemes can provide their own namespace prefixes.
     *
     * @since 4.1.5
    ...
    public void afterPropertiesSet() throws Exception {
        if (this.minRating > this.maxRating) {
            ...
            msg.append("Illegal rating limits for ")
                    .append(name)
                    .append(". Min > Max. ")
                    .append(minRating)
                    .append(" > ")
    ...
    public String toString() {
        ...
        msg.append(this.getClass().getSimpleName())
                ...
                .append(this.name)
                .append(" [")
                .append(this.minRating)
                .append("..")
                .append(this.maxRating)
                .append("]");

        // Injected rollups.
        msg.append(" <");
        for (Iterator<AbstractRatingRollupAlgorithm> iter = propertyRollups.iterator();
                iter.hasNext(); ) {

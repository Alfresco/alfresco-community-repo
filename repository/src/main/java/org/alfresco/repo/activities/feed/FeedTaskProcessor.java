...
public abstract class FeedTaskProcessor {
  ...
  private static final String URL_SERVICE_TEMPLATES =
  ...
  public void process(int jobTaskNode, long minSeq, long maxSeq, RepoCtx ctx)
    ...
    try {
      ...
      List<String> fmTemplates = Arrays.asList(
        new String[] { "activities/org/alfresco/generic.json.ftl" }
      );
      ...
      for (ActivityPostEntity activityPost : activityPosts) {
        ...
        if (fmTemplates.size() == 0) {
          ...
          updatePostStatus(
            activityPost.getId(),
            ActivityPostEntity.STATUS.ERROR
          );
          continue;
        }
        ...
        try {
          model =
            JSONtoFmModel.convertJSONObjectToMap(
        ...
        model.put(
          ActivityFeedEntity.KEY_ACTIVITY_FEED_TYPE,
  ...
  private Set<String> getRecipients(
    ...
    // MT Share
    Pair<String, String> userTenantKey = new Pair<String, String>(
      postUserId,
      tenantDomain
    );
    ...
    if ((null != siteId) && (siteId.length() > 0)) {
      for (String followerUser : followerUsers) {
        ...
        Pair<String, String> userSiteKey = new Pair<String, String>(
          followerUser,
          siteId
        );
  ...
  public abstract List<ActivityPostEntity> selectPosts(
  ...
  public abstract int updatePostStatus(
    long id,
  ...
  protected String callWebScript(String urlString, String ticket)
    ...
    if (ticket != null) {
      ...
      conn.addRequestProperty(
        "Authorization",
        "Basic " + Base64.encodeBytes(ticket.getBytes())
      );
    }
  ...
  protected Set<String> getSiteMembers(
    RepoCtx ctx,
    String siteId,
    String tenantDomain
  ) throws Exception {
  ...
  protected abstract Set<String> getFollowers(
    String userId,
    String tenantDomain
  ) throws Exception;
  ...
  protected Map<String, List<String>> getActivityTemplates(
    List<String> allTemplateNames
  ) {
    Map<String, List<String>> activityTemplates = new HashMap<String, List<String>>(
      10
    );
      ...
      if (!template.contains(" (Working Copy).")) {
        ...
        if (idx2 != -1) {
          ...
          if (idx3 != -1) {
            ...
            List<String> activityTypeTemplateList = activityTemplates.get(
              baseActivityType
            );
  ...
  protected Configuration getFreemarkerConfiguration(RepoCtx ctx) {
    ...
    cfg.setTemplateLoader(
      new TemplateWebScriptLoader(ctx.getRepoEndPoint(), ctx.getTicket())
    );
  ...
  protected List<FeedControlEntity> getFeedControls(String connectedUser)
    throws SQLException {
    //MNT-9104 If username contains uppercase letters the action of joining a site will not be displayed in "My activities"
    if (!userNamesAreCaseSensitive) {
  ...
  protected boolean acceptActivity(
    ...
    for (FeedControlEntity feedControl : feedControls) {
      if (
        ActivitiesDAO.KEY_ACTIVITY_NULL_VALUE.equals(
          feedControl.getSiteNetwork()
        ) &&
        ...
        if (
          feedControl.getSiteNetwork().equals(activityPost.getSiteNetwork())
        ) {

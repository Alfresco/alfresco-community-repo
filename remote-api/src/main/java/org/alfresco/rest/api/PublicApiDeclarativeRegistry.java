...
public class PublicApiDeclarativeRegistry extends DeclarativeRegistry {
        ...
        if (HttpMethod.GET.equals(httpMethod)) {
            if (uri.equals(PublicApiTenantWebScriptServletRequest.NETWORKS_PATH)) {
                ...
                if (rwm != null) {
                    ...
                    final Map<String, String> resourceVars =
                            locator.parseTemplateVars(templateVars);
                    ...
                    switch (rwm.getMetaData().getType()) {
                        ...
                        case PROPERTY:
                            if (StringUtils.isNotBlank(entityId)) {
                                ...
                                } else if (RelationshipResourceBinaryAction.Read.class
                                        .isAssignableFrom(rwm.getResource().getClass())) {
                    ...
                    final boolean noAuth =
                            (resAction != null && rwm.getMetaData().isNoAuth(resAction));
                    if (noAuth) {
            ...
            if (match != null) {
                ResourceWithMetadata rwm =
                        getResourceWithMetadataOrNull(match.getTemplateVars(), httpMethod);
                if (rwm != null) {
                    Class<? extends ResourceAction> resAction = null;
                    Boolean noAuth = null;
                    switch (rwm.getMetaData().getType()) {
                        case ENTITY:
                            ...
                            } else if (EntityResourceAction.CreateWithResponse.class
                                    .isAssignableFrom(rwm.getResource().getClass())) {
                        ...
                        case RELATIONSHIP:
                            ...
                            } else if (RelationshipResourceAction.CreateWithResponse.class
                                    .isAssignableFrom(rwm.getResource().getClass())) {
    ...
    private ResourceWithMetadata getResourceWithMetadataOrNull(
            Map<String, String> templateVars, HttpMethod method) {
        if (templateVars.get("apiName") != null) {
            ...
            // TODO can we avoid locating resource more than once (or at least provide a common code
            // to determine the GET resourceAction) ?
    ...
    private Match overrideMatch(final Match match) {
        // TODO is there a better way (to dynamically override "requiredAuthentication") or handle
        // noAuth check earlier ?
        WebScript noAuthWebScriptWrapper =
                new WebScript() {
                    @Override
                    ...
                    public Description getDescription() {
                        ...
                        return new Description() {
                            ...
                            @Override
                            public RequiredTransactionParameters
                    ...
                    @Override
                    public void execute(
                            WebScriptRequest webScriptRequest, WebScriptResponse webScriptResponse)
                            throws IOException {
        ...
        return new Match(
                match.getTemplate(),
                match.getTemplateVars(),
                match.getPath(),
                noAuthWebScriptWrapper);
    }

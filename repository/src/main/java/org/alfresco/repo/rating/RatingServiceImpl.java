...
public class RatingServiceImpl implements RatingService, Extensible {
  ...
  public void setRollupNamingConventions(
    RatingNamingConventionsUtil namingConventions
  ) {
    this.ratingNamingConventions = namingConventions;
  }

  @Extend(
    traitAPI = RatingServiceTrait.class,
    extensionAPI = RatingServiceExtension.class
  )
  ...
  private boolean isCurrentUserNodeCreator(NodeRef targetNode) {
    final String currentUser = AuthenticationUtil.getFullyAuthenticatedUser();
    Serializable creator = nodeService.getProperty(
      targetNode,
  ...
  private void applyRating(
    ...
    if (ratingScheme == null) {
      throw new RatingServiceException(
        "Unrecognised rating scheme: " + ratingSchemeName
      );
    }
    if (
      rating < ratingScheme.getMinRating() ||
      rating > ratingScheme.getMaxRating()
    ) {
      throw new RatingServiceException(
    ...
    final boolean rollupAspectIsDefined =
      dictionaryService.getAspect(rollupAspectName) != null;

    // Ensure that the application of a rating does not cause updates
    // to the modified, modifier properties on the rated node.
    if (
      nodeService.hasAspect(targetNode, ContentModel.ASPECT_RATEABLE) == false
    ) {
    ...
    final QName assocQName = ratingNamingConventions.getRatingAssocNameFor(
      userName,
      ratingScheme.getName()
    );
    List<ChildAssociationRef> myRatingChildren = nodeService.getChildAssocs(
      targetNode,
  ...
  private void recalculateRatingRollups(
    NodeRef targetNode,
    ...
    AspectDefinition rollupAspect = dictionaryService.getAspect(
      rollupAspectName
    );
    ...
    if ((rollupAspect != null)) {
      ...
      try {
        for (AbstractRatingRollupAlgorithm rollupAlgorithm : ratingScheme.getPropertyRollups()) {
          ...
          QName rollupPropertyName = ratingNamingConventions.getRollupPropertyNameFor(
            ratingScheme,
            rollupAlgorithm.getRollupName()
          );
          nodeService.setProperty(targetNode, rollupPropertyName, s);

          if (
            !rollupAspect.getProperties().containsKey(rollupPropertyName) &&
            log.isDebugEnabled()
          ) {
            StringBuilder msg = new StringBuilder();
            msg
              .append("Rating property rollup property ")
      ...
      if (log.isDebugEnabled()) {
        StringBuilder msg = new StringBuilder();
        msg
          .append("Rating property rollup aspect ")
          .append(rollupAspectName)
          .append(
            " is not defined in the content model & therefore the rollup was not persisted."
          );
        log.debug(msg.toString());
      }
    }
  ...
  public Rating getRatingByCurrentUser(
    NodeRef targetNode,
    String ratingSchemeName
  ) {
  ...
  public List<Rating> getRatingsByCurrentUser(NodeRef targetNode) {
    ...
    List<ChildAssociationRef> children = getRatingNodeChildren(
      targetNode,
      null,
    ...
    for (ChildAssociationRef child : children) {
      result.add(
        convertNodeRefToRating(fullyAuthenticatedUser, child.getChildRef())
      );
    }
    return result;
  }
  ...
  private Rating getRating(
    ...
    List<ChildAssociationRef> ratingChildren = getRatingNodeChildren(
      targetNode,
      ratingSchemeName,
      user
    );
  ...
  private Rating convertNodeRefToRating(String user, NodeRef ratingNode) {
    ...
    String existingRatingScheme = (String) properties.get(
      ContentModel.PROP_RATING_SCHEME
    );

    Float existingRatingScore = (Float) properties.get(
  ...
  public Rating removeRatingByCurrentUser(
    NodeRef targetNode,
    String ratingScheme
  ) {
    String currentUser = AuthenticationUtil.getFullyAuthenticatedUser();
  ...
  private Rating removeRating(
    ...
    if (
      ratingSchemeName.equals(properties.get(ContentModel.PROP_RATING_SCHEME))
    ) {
  ...
  public float getTotalRating(NodeRef targetNode, String ratingSchemeName) {
    Serializable result =
      this.getRatingRollup(
          targetNode,
          ratingSchemeName,
          RatingTotalRollupAlgorithm.ROLLUP_NAME
        );
    if (result == null) {
      result = new Float(0f);
    }
  ...
  public float getAverageRating(NodeRef targetNode, String ratingSchemeName) {
    ...
    return ratingCount == 0
      ? -1
      : Float.valueOf(totalRating / (float) ratingCount);
  }
  ...
  public Serializable getRatingRollup(
    ...
    RatingScheme scheme = schemeRegistry
      .getRatingSchemes()
      .get(ratingSchemeName);
    if (scheme == null) {
      throw new RatingServiceException(
  ...
  List<ChildAssociationRef> getRatingNodeChildren(
    ...
    QNamePattern qnamePattern = ratingNamingConventions.getRatingAssocPatternForUser(
      user,
      ratingSchemeName
    );
  ...
  Rating getRatingFrom(NodeRef ratingNode) {
    ...
    final String schemeName = (String) properties.get(
      ContentModel.PROP_RATING_SCHEME
    );
  ...
  @Override
  public <M extends Trait> ExtendedTrait<M> getTrait(
    Class<? extends M> traitAPI
  ) {

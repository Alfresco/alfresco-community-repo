...
public class FilePlanPermissionServiceImplTest extends BaseRMTestCase {
    ...
    private void setPermission(
            final NodeRef nodeRef, final String userName, final String permission) {
        doTestInTransaction(
                new Test<Void>() {
                    @Override
                    public Void run() {
    ...
    public void testSetDeletePermissionFilePlan() throws Exception {
        ...
        // nothing .. user still has read on file plan .. only removing the user from all roles will
        // remove read on file plan
        assertPermissions(
                userName,
                AccessStatus.ALLOWED, // fileplan read
                AccessStatus.DENIED, // fileplan file
    ...
    public void testMoveRecord() throws Exception {
        ...
        final NodeRef otherFolder =
                doTestInTransaction(
                        new Test<NodeRef>() {
                            @Override
                            public NodeRef run() {
                                return recordFolderService.createRecordFolder(
                                        rmContainer, "otherFolder");
                            }
                        });
                    ...
                    public Void run() {
                        assertEquals(
                                ...
                                permissionService.hasPermission(
                                        otherFolder, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(
                                        otherFolder, RMPermissionModel.FILING));
                        return null;
                    }
                },
                userOne);
    ...
    private void assertPermissions(final String userName, final AccessStatus... accessStatus) {
        ...
        doTestInTransaction(
                new Test<Void>() {
                    ...
                    public Void run() {
                        assertEquals(
                                "Everyone who has a role has read permissions on the file plan",
                                accessStatus[0],
                                permissionService.hasPermission(
                                        filePlan, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                accessStatus[1],
                                permissionService.hasPermission(
                                        filePlan, RMPermissionModel.FILING));

                        assertEquals(
                                accessStatus[2],
                                permissionService.hasPermission(
                                        rmContainer, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                accessStatus[3],
                                permissionService.hasPermission(
                                        rmContainer, RMPermissionModel.FILING));

                        assertEquals(
                                accessStatus[4],
                                permissionService.hasPermission(
                                        rmFolder, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                accessStatus[5],
                                permissionService.hasPermission(
                                        rmFolder, RMPermissionModel.FILING));

                        assertEquals(
                                accessStatus[6],
                                permissionService.hasPermission(
                                        recordOne, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                accessStatus[7],
                                permissionService.hasPermission(
                                        recordOne, RMPermissionModel.FILING));
                ...
                new Test<Void>() {
                    ...
                    public Void run() {
                        ...
                        assertEquals(
                                accessStatus[2],
                                permissionService.hasPermission(
                                        transfersContainer, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                accessStatus[3],
                                permissionService.hasPermission(
                                        transfersContainer, RMPermissionModel.FILING));

                        assertEquals(
                                accessStatus[4],
                                permissionService.hasPermission(
                                        holdsContainer, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                accessStatus[5],
                                permissionService.hasPermission(
                                        holdsContainer, RMPermissionModel.FILING));

                        assertEquals(
                                accessStatus[6],
                                permissionService.hasPermission(
                                        unfiledContainer, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                accessStatus[7],
                                permissionService.hasPermission(
                                        unfiledContainer, RMPermissionModel.FILING));
                        ...
                        assertEquals(
                                accessStatus[10],
                                permissionService.hasPermission(
                                        subCategory, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                accessStatus[11],
                                permissionService.hasPermission(
                                        subCategory, RMPermissionModel.FILING));

                        assertEquals(
                                accessStatus[12],
                                permissionService.hasPermission(
                                        folder, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                accessStatus[13],
                                permissionService.hasPermission(folder, RMPermissionModel.FILING));

                        assertEquals(
                                accessStatus[14],
                                permissionService.hasPermission(
                                        record, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                accessStatus[15],
                                permissionService.hasPermission(record, RMPermissionModel.FILING));
    ...
    public void testFilePlanComponentInheritance() {
        doTestInTransaction(
                new Test<Void>() {
                    @Override
                    public Void run() {
                        // Inheritance is turned off for file plan, transfer, holds, unfiled records
                        // and root categories
                        ...
                        assertFalse(
                                permissionService.getInheritParentPermissions(unfiledContainer));
                        assertFalse(permissionService.getInheritParentPermissions(rmContainer));
                        assertTrue(
                                permissionService.getInheritParentPermissions(
                                        recordFolderService.createRecordFolder(
                                                rmContainer, "subCategory")));
    ...
    public void testRolesSetByDefault() {
        ...
        // Admin user has read/filing permissions on file plan, transfer, hold, unfiled records,
        // root categories, sub categories, folders and records
        ...
        // Test user has read permissions on file plan, transfer, hold and unfiled records as the
        // user will be added in the all records management roles
    ...
    public void testAddUserToContainers() {
        ...
        // and read permissions on transfer, hold and unfiled records as the user will be in the all
        // records management users role
        ...
        // and read permissions on file plan, hold and unfiled records as the user will be in the
        // all records management users role
        ...
        // as the inheritance is turned on for the sub category the user will have also read
        // permissions on sub category, folder and record
        ...
        // as the inheritance is turned on the user will have also read and filing permissions on
        // folder and record
        // and also read permissions on file plan, transfer, hold and unfiled records
    ...
    public void testAccessPermissionOnSingleRecordWithSeveralUsers() {
        final NodeRef subCategory =
                filePlanService.createRecordCategory(rmContainer, "subCategory3");
        ...
        // user1 will have access to file plan, root category and because of inheritance sub
        // category, folder and record
        doTestInTransaction(
                new Test<Void>() {
                    @Override
                    public Void run() {
    ...
    public void testDenyPermissionsOnRecordsWithSeveralUsers() {
        final NodeRef subCategory =
                filePlanService.createRecordCategory(rmContainer, "subCategory4");
        ...
        // user1 will have access to file plan, root category and because of inheritance sub
        // category, folder, record4 and record5
        doTestInTransaction(
                new Test<Void>() {
                    @Override
                    public Void run() {
                        ...
                        assertEquals(
                                AccessStatus.ALLOWED,
                                permissionService.hasPermission(
                                        record4, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                AccessStatus.ALLOWED,
                                permissionService.hasPermission(
                                        record5, RMPermissionModel.READ_RECORDS));
        ...
        // user2 will have access to file plan, root category and because of inheritance sub
        // category and folder
    ...
    public void testMoveRootCategoryIntoAnotherRootCategory() {
                ...
                new Test<Void>() {
                    ...
                    public Void run() {
                        assertEquals(
                                AccessStatus.ALLOWED,
                                permissionService.hasPermission(
                                        category5, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(
                                        category5, RMPermissionModel.FILING));
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(
                                        category6, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(
                                        category6, RMPermissionModel.FILING));
        ...
        final NodeRef movedCategory5 =
                doTestInTransaction(
                        new Test<NodeRef>() {
                            @Override
                            public NodeRef run() throws Exception {
                                return fileFolderService
                                        .move(category5, category6, null)
                                        .getNodeRef();
                            }
                        });
                ...
                new Test<Void>() {
                    ...
                    public Void run() {
                        assertEquals(
                                AccessStatus.ALLOWED,
                                permissionService.hasPermission(
                                        movedCategory5, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(
                                        movedCategory5, RMPermissionModel.FILING));
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(
                                        category6, RMPermissionModel.READ_RECORDS));
                ...
                new Test<Void>() {
                    ...
                    public Void run() {
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(
                                        category7, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(
                                        category7, RMPermissionModel.FILING));
                        assertEquals(
                                AccessStatus.ALLOWED,
                                permissionService.hasPermission(
                                        folder7, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                AccessStatus.ALLOWED,
                                permissionService.hasPermission(folder7, RMPermissionModel.FILING));
                        assertEquals(
                                AccessStatus.ALLOWED,
                                permissionService.hasPermission(
                                        record7, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                AccessStatus.ALLOWED,
                                permissionService.hasPermission(record7, RMPermissionModel.FILING));

                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(
                                        category8, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(
                                        category8, RMPermissionModel.FILING));
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(
                                        folder8, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(folder8, RMPermissionModel.FILING));
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(
                                        record8, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                AccessStatus.DENIED,
                                permissionService.hasPermission(record8, RMPermissionModel.FILING));
                ...
                new Test<Void>() {
                    ...
                    public Void run() {
                        ...
                        assertEquals(
                                AccessStatus.ALLOWED,
                                permissionService.hasPermission(
                                        movedRecord8, RMPermissionModel.READ_RECORDS));
                        assertEquals(
                                AccessStatus.ALLOWED,
                                permissionService.hasPermission(
                                        movedRecord8, RMPermissionModel.FILING));
    ...
    public void testSpecialRoles() {
        ...
        final NodeRef subCategory9 =
                filePlanService.createRecordCategory(category9, "subCategory9");
        ...
        // After setting the permissions off the special roles should be still available as they
        // will be added to the node automatically
    ...
    private void assertExistenceOfSpecialRolesAndPermissions(NodeRef node) {
        ...
        String adminRole =
                authorityService.getName(
                        AuthorityType.GROUP, FilePlanRoleService.ROLE_ADMIN + filePlan.getId());
    ...
    public void testMoveSubCategoryIntoFilePlan() {
        final NodeRef rootCategory =
                filePlanService.createRecordCategory(filePlan, GUID.generate());
        final NodeRef subCategory =
                filePlanService.createRecordCategory(rootCategory, GUID.generate());
        ...
        final NodeRef movedSubCategory =
                doTestInTransaction(
                        new Test<NodeRef>() {
                            @Override
                            public NodeRef run() throws Exception {
                                return fileFolderService
                                        .move(subCategory, filePlan, null)
                                        .getNodeRef();
                            }
                        });

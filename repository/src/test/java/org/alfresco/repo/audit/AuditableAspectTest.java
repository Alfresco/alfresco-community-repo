...
public class AuditableAspectTest extends TestCase {
  ...
  public synchronized void testAddAndRemoveAspect() throws Exception {
    ...
    Date aspectCreatedDate1 = (Date) nodeService.getProperty(
      nodeRef,
      ContentModel.PROP_CREATED
    );
    Date aspectModifiedDate1 = (Date) nodeService.getProperty(
      nodeRef,
      ContentModel.PROP_MODIFIED
    );
    assertTrue(
      "Created date should be later than t1",
      t1 < aspectCreatedDate1.getTime()
    );
    assertTrue(
    ...
    // Check that the dates were set correctly
    Date aspectCreatedDate2 = (Date) nodeService.getProperty(
      nodeRef,
      ContentModel.PROP_CREATED
    );
    Date aspectModifiedDate2 = (Date) nodeService.getProperty(
      nodeRef,
      ContentModel.PROP_MODIFIED
    );
    assertEquals(
      "The created date must not change",
      aspectCreatedDate1,
      aspectCreatedDate2
    );
    assertTrue(
      "New modified date should be later than t3",
      t3 < aspectModifiedDate2.getTime()
    );
  ...
  public void testCreateAndUpdateAuditableProperties() {
    ...
    String modifiedBy = (String) nodeService.getProperty(
      nodeRef,
    ...
    transactionService
      ...
      .doInTransaction(setAuditableCallback1);
    // Check
    assertAuditableProperties(nodeRef, auditableProps);
    ...
    transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(setAuditableCallback2);
    // Check
    assertAuditableProperties(nodeRef, auditableProps);
  }
  ...
  public void testPutContent() throws Exception {
    ...
    final Date modified = (Date) nodeService.getProperty(
      nodeRef,
      ContentModel.PROP_MODIFIED
    );
    ...
    final Date txnModified = transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(dateCallback);
    assertTrue(
    ...
    RetryingTransactionCallback<Integer> countCallback = new RetryingTransactionCallback<Integer>() {
      ...
      public Integer execute() throws Throwable {
        ...
        writer.putContent(text);
        Date newModified = (Date) nodeService.getProperty(
          nodeRef,
          ContentModel.PROP_MODIFIED
        );
    ...
    Integer count = transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(countCallback);

    assertTrue(
      "Repeated content uploads in the same transaction should only modify the cm:modified once.",
      count == 1
    );
  }
  ...
  private void assertAuditableProperties(
    NodeRef nodeRef,
    Map<QName, Serializable> checkProps
  ) {
    assertTrue(

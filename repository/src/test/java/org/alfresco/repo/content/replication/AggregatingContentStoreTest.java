...
public class AggregatingContentStoreTest
  ...
  public void before() throws Exception {
    ...
    String storeDir =
      tempDir.getAbsolutePath() + File.separatorChar + GUID.generate();
  ...
  private void checkForUrl(String contentUrl, boolean mustExist) {
    ...
    assertEquals(
      "Reader state differs from expected: " + reader,
      mustExist,
      reader.exists()
    );
  }
  ...
  public void testReadFromSecondaryStore() {
    ...
    ContentWriter writer = secondaryStore.getWriter(
  ...
  public void testRequestContentDirectUrl() {
    ...
    // By default it is unsupported
    DirectAccessUrl directAccessUrl = aggStore.requestContentDirectUrl(
      "url",
      true,
      "anyfilename",
      "anyMimetype",
    ...
    try {
      when(
        primaryStoreMock.requestContentDirectUrl(
          any(),
          any(),
          any(),
          any(),
      ...
      when(
        secondaryStoreMock.requestContentDirectUrl(
          eq("urlDANotSupported"),
          any(),
          any(),
          any(),
          any()

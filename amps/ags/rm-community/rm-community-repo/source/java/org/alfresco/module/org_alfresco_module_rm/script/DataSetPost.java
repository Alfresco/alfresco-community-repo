        ...
        try {
            ...
            if (StringUtils.isBlank(dataSetId)) {
                throw new WebScriptException(
                        Status.STATUS_BAD_REQUEST, "A data set id was not provided.");
            }
            if (!dataSetService.existsDataSet(dataSetId)) {
                throw new WebScriptException(
                        Status.STATUS_NOT_FOUND,
                        "A data set with the id '" + dataSetId + "'" + " does not exist.");
            }
            ...
            if (siteService.getSite(siteName) == null) {
                throw new WebScriptException(
                        ...
                        "A Records Management site with the name '"
                                + siteName
                                + "' does not exist.");
            }
            ...
            NodeRef filePlan =
                    siteService.getContainer(siteName, RmSiteType.COMPONENT_DOCUMENT_LIBRARY);
            if (filePlan == null) {
                filePlan =
                        siteService.createContainer(
                                siteName,
                                RmSiteType.COMPONENT_DOCUMENT_LIBRARY,
                                TYPE_FILE_PLAN,
                                null);
            }

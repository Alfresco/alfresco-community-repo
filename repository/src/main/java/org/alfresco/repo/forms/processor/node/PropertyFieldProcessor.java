...
public class PropertyFieldProcessor
  ...
  @Override
  protected PropertyDefinition getTypeDefinition(
    QName fullName,
  ...
  public Field makeField(
    ...
    PropertyFieldDefinition fieldDef = makePropertyFieldDefinition(
      propDef,
      group
    );
  ...
  private Object getDefaultValue(QName name, ContentModelItemData<?> data) {
    ...
    if (propDef != null) {
      ...
      if (NodePropertyValue.isDataTypeSupported(typeQName)) {
        ...
        NodePropertyValue pv = new NodePropertyValue(
          typeQName,
          strDefaultValue
        );
  ...
  private PropertyFieldDefinition makePropertyFieldDefinition(
    ...
    if (
      NamespaceService.SYSTEM_MODEL_1_0_URI.equals(
  ...
  private List<FieldConstraint> makeFieldConstraints(
    PropertyDefinition propDef
  ) {
    ...
    if (constraints != null && constraints.size() > 0) {
      ...
      for (ConstraintDefinition constraintDef : constraints) {
        ...
        if (
          RegisteredConstraint.class.isAssignableFrom(constraint.getClass())
        ) {
          constraint =
        ...
        if (
          ListOfValuesConstraint.class.isAssignableFrom(constraint.getClass())
        ) {
          ...
          List<String> localisedValues = new ArrayList<String>(
            allowedValues.size()
          );

          // Look up each localised display-label in turn.
          for (String value : allowedValues) {
            String displayLabel = lovConstraint.getDisplayLabel(
              value,
              dictionaryService
            );
          ...
          params.put(
            ListOfValuesConstraint.ALLOWED_VALUES_PARAM,
            localisedValues
          );
        }

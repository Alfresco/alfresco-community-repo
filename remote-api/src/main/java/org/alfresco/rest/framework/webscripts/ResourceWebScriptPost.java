...
public class ResourceWebScriptPost
  ...
  @Override
  public Params extractParams(
    ResourceMetadata resourceMeta,
    WebScriptRequest req
  ) {
    final Map<String, String> resourceVars = locator.parseTemplateVars(
      req.getServiceMatch().getTemplateVars()
    );
    final String entityId = resourceVars.get(ResourceLocator.ENTITY_ID);
    final String relationshipId = resourceVars.get(
      ResourceLocator.RELATIONSHIP_ID
    );

    final String operationName = resourceVars.get(
      ResourceLocator.RELATIONSHIP_RESOURCE
    );
    final String propertyName = resourceVars.get(ResourceLocator.PROPERTY);

    final RecognizedParams params = getRecognizedParams(req);
    final ResourceOperation operation = resourceMeta.getOperation(
      HttpMethod.POST
    );
    ...
    switch (resourceMeta.getType()) {
      ...
      case OPERATION:
        if (
          StringUtils.isNotBlank(entityId) &&
          StringUtils.isNotBlank(operationName)
        ) {
          Object postedObj = processRequest(resourceMeta, operation, req);
      ...
      default:
        throw new UnsupportedResourceOperationException(
          "POST not supported for Actions"
        );
    }
  }
  ...
  private Object extractObjFromJson(
    ...
    boolean isTypeOperation = resourceMeta
      .getType()
      .equals(ResourceMetadata.RESOURCE_TYPE.OPERATION);
    ...
    if (!params.isEmpty()) {
      for (ResourceParameter resourceParameter : params) {
        ...
        boolean notMultiple =
          ((!resourceParameter.isAllowMultiple()) || isTypeOperation);

        if (
          ResourceParameter.KIND.HTTP_BODY_OBJECT.equals(
          ...
          try {
            ...
            if (
              iae.getMessage().contains("START_ARRAY") &&
  ...
  private Object executeOperation(
    ...
    switch (
      operationResourceMetaData.getOperationMethod().getParameterTypes().length
    ) {
      case 4:
  ...
  public Object executeAction(
    ...
    switch (resource.getMetaData().getType()) {
      case ENTITY:
        if (
          resObj instanceof MultiPartResourceAction.Create<?> &&
          ...
          if (
            EntityResourceAction.Create.class.isAssignableFrom(
            ...
            if (
              resource
                .getMetaData()
                .isDeleted(EntityResourceAction.Create.class)
            ) {
            ...
            List<Object> created = creator.create(
          ...
          if (
            EntityResourceAction.CreateWithResponse.class.isAssignableFrom(
                resource.getResource().getClass()
              )
          ) {
            if (
      ...
      case RELATIONSHIP:
        if (
          resObj instanceof MultiPartRelationshipResourceAction.Create<?> &&
          params.getPassedIn() instanceof FormData
        ) {
          ...
          if (
            RelationshipResourceAction.Create.class.isAssignableFrom(
                resource.getResource().getClass()
              )
          ) {
            if (
            ...
            List<Object> createdRel = createRelation.create(
              params.getEntityId(),
              (List<Object>) params.getPassedIn(),
              params
            );
          ...
          if (
            RelationshipResourceAction.CreateWithResponse.class.isAssignableFrom(
                resource.getResource().getClass()
              )
          ) {
            if (

...
public class RMCaveatConfigComponentImpl
     ...
     *     boolean) RM-2770 - this method has to be fired on transaction commit to be able to
     *     validate the content when the content store is encrypted
     */
    @Override
    @Behaviour(
            ...
            if (caveatConfigData != null) {
                ...
                if ((existing != null && (!existing.equals(nodeRef)))) {
                    throw new AlfrescoRuntimeException(
                            "Cannot create more than one caveat config (existing="
                                    + existing
                                    + ", new="
                                    + nodeRef
                                    + ")");
                ...
                try {
                    ...
                    for (QName model : models) {
                        ...
                        expectedPrefixes.addAll(
                                namespaceService.getPrefixes(model.getNamespaceURI()));
                    }
                    ...
                    Map<String, Object> caveatConfigMap =
                            JSONtoFmModel.convertJSONObjectToMap(caveatConfigData);
                    ...
                    for (Map.Entry<String, Object> conEntry : caveatConfigMap.entrySet()) {
                        ...
                        Map<String, List<String>> caveatMap =
                                (Map<String, List<String>>) conEntry.getValue();
                        ...
                        for (QName propertyName : props) {
                            PropertyDefinition propDef =
                                    dictionaryService.getProperty(propertyName);
                            ...
                            for (ConstraintDefinition conDef : conDefs) {
                                ...
                                if (con instanceof RMListOfValuesConstraint) {
                                    String conName =
                                            ((RMListOfValuesConstraint) con).getShortName();
                                    if (conName.equals(conStr)) {
                                        // note: assumes only one caveat/LOV against a given
                                        // property
                                        allowedValues =
                                                AuthenticationUtil.runAs(
                                                        new RunAsWork<List<String>>() {
                                                            public List<String> doWork() {
                                                                return ((RMListOfValuesConstraint)
                                                                                con)
                                                                        .getAllowedValues();
                                                            }
                                                        },
                        ...
                        if (allowedValues != null) {
                            ...
                            for (Map.Entry<String, List<String>> caveatEntry :
                                    caveatMap.entrySet()) {
                                String authorityName = caveatEntry.getKey();
                                List<String> caveatList = caveatEntry.getValue();

                                // validate authority (user or group) - note: groups are configured
                                // with fullname (ie. GROUP_xxx)
                    ...
                    try {
                        writeLock.lock();
                        // we can't just clear the cache, as all puts to the cache afterwards in
    ...
    private NodeRef getCaveatConfigNode() {
        ...
        return nodeService.getChildByName(
                rootNode, RecordsManagementModel.ASSOC_CAVEAT_CONFIG, CAVEAT_CONFIG_NAME);
    }
    ...
    public NodeRef updateOrCreateCaveatConfig(InputStream is) {
        ...
        ContentWriter writer =
                this.contentService.getWriter(caveatConfig, ContentModel.PROP_CONTENT, true);
            ...
            caveatConfig =
                    nodeService
                            .createNode(
                                    ...
                                    QName.createQName(
                                            RecordsManagementModel.RM_URI, CAVEAT_CONFIG_NAME),
    ...
    private List<String> getRMAllowedValues(
            String userName, Set<String> userGroupFullNames, String constraintName) {
            ...
            if (!found) {
                ...
                if (userName != null) {
                    ...
                    for (Map.Entry<QName, Serializable> entry : props.entrySet()) {
                        ...
                        if ((propDef != null)
                                && (propDef.getDataType().getName().equals(DATATYPE_TEXT))) {
                            List<ConstraintDefinition> conDefs = propDef.getConstraints();
                            for (ConstraintDefinition conDef : conDefs) {
                                Constraint con = conDef.getConstraint();
                                if (con instanceof RMListOfValuesConstraint) {
                                    RMListOfValuesConstraint rmCon =
                                            ((RMListOfValuesConstraint) con);
                                    String conName = rmCon.getShortName();
                                    MatchLogic matchLogic = rmCon.getMatchLogicEnum();
                                    Map<String, List<String>> caveatConstraintDef =
                                            caveatConfig.get(conName);
                                    if (caveatConstraintDef == null) {
                                        continue;
                                    } else {
                                        ...
                                        List<String> allowedValues =
                                                getRMAllowedValues(
                                                        userName, userGroupNames, conName);
                                        ...
                                        if (propValues != null
                                                && !isAllowed(
                                                        propValues, allowedValues, matchLogic)) {
    ...
    private boolean isAllowed(
            List<String> propValues, List<String> userGroupValues, MatchLogic matchLogic) {
        if (matchLogic.equals(MatchLogic.AND)) {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace(
                        "Not allowed: " + propValues + ", " + userGroupValues + ", " + matchLogic);
            }
    ...
    public void addRMConstraintListValue(String listName, String authorityName, String value) {
        ...
        try {
            ...
            if (members == null) {
                throw new AlfrescoRuntimeException(
                        "unable to add to list, list not defined:" + listName);
            }
            ...
            try {
                ...
                if (values == null) {
                    throw new AlfrescoRuntimeException(
                            "Unable to add to authority in list.   Authority not member listName: "
                                    + listName
                                    + " authorityName:"
                                    + authorityName);
                }

...
public class SiteLoadPatch extends AbstractPatch {
  ...
  static {
    ...
    DEFAULT_PATHS.put(
      PROPERTIES_CONTENTS,
      "/${spaces.company_home.childname}/${spaces.sites.childname}"
    );
  }

  private static final String MSG_SITE_ALREADY_EXISTS =
    "patch.siteLoadPatch.exists";
  ...
  private static final String MSG_SITE_NOT_CREATED =
    "patch.siteLoadPatch.siteNotCreated";
  private static final String MSG_SITE_LOAD_DISABLED =
  ...
  private String applyInternalImpl() throws Exception {
    ...
    if (bootstrapViews == null || bootstrapViews.size() == 0) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "No Bootstraps given to import from - bootstrap import ignored"
        );
      }
    ...
    if (siteService.getSite(siteName) != null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Site " + siteName + " already exists - bootstrap import ignored"
        );
      }
    ...
    for (ImporterBootstrap bootstrap : new ImporterBootstrap[] {
      spacesBootstrap,
      usersBootstrap,
    }) {
  ...
  private void doGroupImport(String location) throws Throwable {
    ...
    while ((line = reader.readLine()) != null) {
      ...
      StringTokenizer groups = new StringTokenizer(

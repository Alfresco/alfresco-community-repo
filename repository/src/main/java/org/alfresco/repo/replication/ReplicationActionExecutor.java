...
public class ReplicationActionExecutor extends ActionExecuterAbstractBase {
    ...
    protected Set<NodeRef> expandPayload(ReplicationDefinition replicationDef) {
        ...
        crawler.setNodeFinders(
                new ChildAssociatedNodeFinder(
                        ContentModel.ASSOC_CONTAINS, RenditionModel.ASSOC_RENDITION));
    ...
    protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
        ...
        } else if (action.getActionDefinitionName()
        ...
        try {
            ...
            lock.close();
            throw new ReplicationServiceException(
                    I18NUtil.getMessage(MSG_ERR_PROCESSING_PAYLOAD, e.getMessage()), e);
        }
        ...
        try {
            ...
            endEvent =
                    transferService.transfer(
                            replicationDef.getTargetName(), transferDefinition, lock);
            ...
            if (e instanceof TransferFailureException) {
                ...
                throw new ReplicationServiceException(
                        I18NUtil.getMessage(MSG_ERR_EXECUTING_TRANSFER, cause.getMessage()), cause);
            }
            writeDefinitionReports(replicationDef, null, null);
            throw new ReplicationServiceException(
                    I18NUtil.getMessage(MSG_ERR_EXECUTING_TRANSFER, e.getMessage()), e);
        } finally {
            lock.close();
        }
    }
    ...
    protected class ReplicationDefinitionLockExtender
        ...
        /**
         * Get a lock on the job. Tries every 5 seconds for 30 seconds, then every 30 seconds for
         * half an hour.
         *
        ...
        private void releaseLock() {
            if (active) {
                ...
                jobLockService.releaseLock(lockToken, replicationDef.getReplicationQName());
                active = false;
            }
        }
        ...
        private void checkCancel() {
            ...
            if (actionTrackingService.isCancellationRequested(replicationDef)) {
                ...
                if (transferId != null) {
                    ...
                    logger.warn(
                            "Unable to cancel replication as requested, as transfer has yet to"
                                    + " reach a cancellable state");
                }
            }
        }

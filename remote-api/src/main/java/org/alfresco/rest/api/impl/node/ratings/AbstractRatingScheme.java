...
public abstract class AbstractRatingScheme implements RatingScheme, InitializingBean {
    ...
    public NodeRating getNodeRating(NodeRef nodeRef) {
        Rating ratingByCurrentUser =
                ratingService.getRatingByCurrentUser(nodeRef, ratingSchemeName);
        Float rating = null;
        Date appliedAt = null;
        ...
        NodeRating nodeRating =
                new NodeRating(ratingSchemeId, myRating, appliedAt, documentRatingSummary);
        return nodeRating;
    }
    ...
    private JSONObject getActivityData(final NodeRef nodeRef, String siteId) {
        ...
        if (siteId != null) {
            // may not be able to read these nodes, but we need to for the activity processing so
            // run as system
            activityData =
                    AuthenticationUtil.runAsSystem(

...
public class HashStoreConfiguration {
        ...
        public void setClasspathsHashes(String classpathsHashes) {
            ...
            for (int i = 0; i < hashes.length; i++) {
                ...
                if (pathHash.length != 2) {
                    ...
                    HashStoreConfiguration.getInstance()
                            .getClasspathHashStore()
                            .put(pathHash[0], pathHash[1]);
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "Configured classpath hash " + pathHash[0] + " -> " + pathHash[1]);
                    }
                }
            }
        }
    ...
    private HashStoreConfiguration() {
        ...
        storeProtocolStore.put(StoreRef.PROTOCOL_WORKSPACE, "1");
        storeProtocolStore.put(StoreRef.PROTOCOL_ARCHIVE, "2");
        storeProtocolStore.put(StoreRef.PROTOCOL_AVM, "3");
        storeProtocolStore.put(StoreRef.PROTOCOL_DELETED, "4");
        storeProtocolStore.put(StoreRef.PROTOCOL_TEST, "5");
        storeProtocolStore.put(VersionService.VERSION_STORE_PROTOCOL, "6");
        storeIdStore.put("SpacesStore", "1");
        storeIdStore.put(VersionModel.STORE_ID, "2");
        storeIdStore.put(Version2Model.STORE_ID, "3");
    }

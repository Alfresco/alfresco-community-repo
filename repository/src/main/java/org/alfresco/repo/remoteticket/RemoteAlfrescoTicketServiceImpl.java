...
public class RemoteAlfrescoTicketServiceImpl
  ...
  private static Log logger = LogFactory.getLog(
    RemoteAlfrescoTicketServiceImpl.class
  );
   ...
   */
  public void setRemoteCredentialsService(
    RemoteCredentialsService remoteCredentialsService
  ) {
   ...
   */
  public void setRemoteConnectorService(
  ...
  public synchronized void registerRemoteSystem(
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Registered System " + remoteSystemId + " as " + baseUrl
    );
  }

  protected void ensureRemoteSystemKnown(String remoteSystemId)
  ...
  protected PasswordCredentialsInfo ensureCredentialsFound(
    ...
    if (!(credentails instanceof PasswordCredentialsInfo)) {
      throw new AlfrescoRuntimeException(
        "Credentials found, but of the wrong type, needed PasswordCredentialsInfo but got " +
        credentails
      );
    }
  ...
  public BaseCredentialsInfo storeRemoteCredentials(
    ...
    if (existing != null) {
      ...
      if (existing instanceof PasswordCredentialsInfoImpl) {
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "Updating existing credentials from " + credentials.getNodeRef()
        );
      } else {
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Credentials correct for " + username + " on " + remoteSystemId
    );
    ...
    if (credentials.getNodeRef() != null) {
      ...
      return remoteCredentialsService.createPersonCredentials(
        remoteSystemId,
        credentials
      );
    }
   ...
   */
  public boolean deleteRemoteCredentials(String remoteSystemId)
    throws NoSuchSystemException {
    // Try to retrieve
    ...
    if (credentials == null) {
      if (logger.isDebugEnabled()) logger.debug(
        "No credentials found to delete on " + remoteSystemId
      );
      return false;
    }
  ...
  public RemoteAlfrescoTicketInfo getAlfrescoTicket(String remoteSystemId)
    ...
    PasswordCredentialsInfo credentials = ensureCredentialsFound(
      remoteSystemId,
      creds
    );
  ...
  protected RemoteAlfrescoTicketInfo refreshTicket(
    ...
    final PasswordCredentialsInfo credentials
  )
    ...
    RemoteConnectorRequest request = remoteConnectorService.buildRequest(
      url,
      "POST"
    );
    request.setRequestBody(json.toJSONString());

    Map<String, String> reqHeaders = remoteSystemsReqHeaders.get(
      remoteSystemId
    );
    if (reqHeaders != null) {
    ...
    try {
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "JSON Ticket Response Received: " + response
      );
      ...
      throw new RemoteSystemUnavailableException(
        "Error talking to remote system",
        ioEx
      );
    } catch (ParseException jsonEx) {
      if (logger.isDebugEnabled()) logger.debug(
        "Invalid JSON from remote Alfresco instance " + remoteSystemId,
        jsonEx
      );

      throw new RemoteSystemUnavailableException(
        "Invalid JSON response from remote system",
        jsonEx
      );
      ...
      if (
        credentials.getNodeRef() != null &&
        credentials.getLastAuthenticationSucceeded()
      ) {

...
public class DictionaryRestApiTest extends BaseWebScriptTest {
    ...
    private void validateChildAssociation(JSONObject result) throws Exception {
        ...
        assertEquals(
                "/api/defclasses/wca/form/association/wca/formworkflowdefaults", result.get("url"));
    }
    ...
    private void validateAssociation(JSONObject result) throws Exception {
        ...
        assertEquals(
                "/api/defclasses/wca/form/association/wca/renderingenginetemplates",
                result.get("url"));
    }
    ...
    private void validateTypeClass(JSONObject result) throws Exception {
        ...
        assertEquals(
                "sys:referenceable",
                result.getJSONObject("defaultAspects")
                        .getJSONObject("sys:referenceable")
                        .get("name"));
        assertEquals(
                "Referenceable",
                result.getJSONObject("defaultAspects")
                        ...
                        .get("title"));
        assertEquals(
                "/api/defclasses/cm/cmobject/property/sys/referenceable",
                result.getJSONObject("defaultAspects")
                        .getJSONObject("sys:referenceable")
                        .get("url"));
        ...
        assertEquals(
                "cm:auditable",
                result.getJSONObject("defaultAspects").getJSONObject("cm:auditable").get("name"));
        assertEquals(
                "Auditable",
                result.getJSONObject("defaultAspects").getJSONObject("cm:auditable").get("title"));
        assertEquals(
                "/api/defclasses/cm/cmobject/property/cm/auditable",
                result.getJSONObject("defaultAspects").getJSONObject("cm:auditable").get("url"));

        // assertEquals("cm:name",
        // result.getJSONObject("properties").getJSONObject("cm:name").get("name"));
        // assertEquals("Name",
        // result.getJSONObject("properties").getJSONObject("cm:name").get("title"));
        // assertEquals("/api/defclasses/cm/cmobject/property/cm/name",
        // result.getJSONObject("properties").getJSONObject("cm:name").get("url"));
    ...
    private void validateAspectClass(JSONObject result) throws Exception {
        ...
        if (result.getJSONObject("properties").has("cm:automaticUpdate") == true) {
            assertEquals(
                    "cm:automaticUpdate",
                    result.getJSONObject("properties")
                            .getJSONObject("cm:automaticUpdate")
                            .get("name"));
            assertEquals(
                    "Automatic Update",
                    result.getJSONObject("properties")
                            ...
                            .get("title"));
            assertEquals(
                    "/api/defclasses/cm/thumbnailed/property/cm/automaticUpdate",
                    result.getJSONObject("properties")
                            .getJSONObject("cm:automaticUpdate")
                            .get("url"));
        }
    ...
    private void validatePropertiesConformity(JSONArray classDefs) throws Exception {
        ...
        for (int i = 0; (i < itemsToTest) && (i < classDefs.length()); ++i) {
            ...
            Response responseFromGetPropertiesDef =
                    sendRequest(new GetRequest(classUrl + "/properties"), 200);
            JSONArray propertiesDefs =
    ...
    private void assertEquivalenceProperties(
            List<String> propertyValues1, List<String> propertyValues2) {
        if ((propertyValues1.size() != propertyValues2.size())
                || !propertyValues1.containsAll(propertyValues2)) {
    ...
    public void testGetPropertyDef() throws Exception {
        Response response =
                sendRequest(
                        new GetRequest("/api/defclasses/cm/auditable/property/cm/created"), 200);
        ...
        // invalid property name , returns a null JsonObject as such a property doesn't exist under
        // cm_auditable
        response =
                sendRequest(
                        new GetRequest("/api/defclasses/cm/auditable/property/cm/welcome"), 200);
    ...
    public void testGetClassDetails() throws Exception {
         ...
         * Classfilter namespaceprefix name Returns 1 yes yes yes single class 2 yes yes no Array of
         * classes [returns array of classes of the particular namespaceprefix] 3 yes no no Array of
         * classes [depends on classfilter, either type or aspect or all classes in the repo] 4 no
         * no no Array of classes [returns all classes of both type and aspects in the entire
         * repository] 5 no yes yes single class [returns a single class of a valid
         * namespaceprefix:name combination] 6 no yes no Array of classes [returns an array of all
         * aspects and types under particular namespaceprefix] 7 no no yes 404 error [since name
         * alone doesn't makes any meaning] 8 yes no yes 404 error [since name alone doesn't makes
         * any meaning] Test cases are provided for all the above scenarios
         */
        ...
        result = new JSONArray(response.getContentAsString());
        assertEquals(result.length() > 0, true);
        // the above result has all the types under cm, so now check for the presence type
        // cm:cmobject in the array of classes of all types
        for (int i = 0; i < result.length(); i++) {
            if (result.getJSONObject(i).get("name").equals("cm:cmobject")) {
                validateTypeClass(result.getJSONObject(i));
            }
        ...
        result = new JSONArray(response.getContentAsString());
        assertEquals(result.length() > 0, true);
        // the above result has all the aspects under cm, so now check for the presence aspect
        // cm:thumnailed in the array of classes of all aspects
        for (int i = 0; i < result.length(); i++) {
            if (result.getJSONObject(i).get("name").equals("cm:thumbnailed")) {
                validateAspectClass(result.getJSONObject(i));
            }
        ...
        // check for a classname [namespaceprefix:name => cm:cmobject] without classfilter option
        // [case-type:5]
        arguments.clear();
        arguments.put("nsp", "cm");
        arguments.put("n", "cmobject");
        ...
        // check for a classname [namespaceprefix:name => cm:thumbnailed] without classfilter option
        // [case-type:5]
        arguments.clear();
        arguments.put("nsp", "cm");
        arguments.put("n", "thumbnailed");
        ...
        // check for a namespaceprefix [namespaceprefix => cm] without classfilter and name option
        // [case-type:6]
        arguments.clear();
        arguments.put("nsp", "cm");
        req.setArgs(arguments);
        ...
        // check for all types dictionary data  without option=>name and option=>namespaceprefix and
        // option=>classfilter
        arguments.clear();
        req.setArgs(arguments);
        response = sendRequest(req, 200);
    ...
    public void testSubClassDetails() throws Exception {
        ...
        arguments.put(
                "n",
                "dublincore"); // name and namespaceprefix are valid one , but its not present in
        // sys_base as a sub-class
        arguments.put("nsp", "cm");
        req.setArgs(arguments);
        response = sendRequest(req, 404);
        assertEquals(404, response.getStatus());
    ...
    public void testGetAssociationDef() throws Exception {
        ...
        response =
                sendRequest(
                        new GetRequest(URL_SITES + "/cm/personalbe/association/cms/avatarsara"),
                        404);
        assertEquals(404, response.getStatus());

        // ask for an invalid association under wca:form , which returns 404
        response =
                sendRequest(new GetRequest(URL_SITES + "/wca/form/association/cmsavatarsara"), 404);
        assertEquals(404, response.getStatus());
    }
    ...
    public void testGetAssociationDefs() throws Exception {
        ...
        // validate with associationfilter=>general(that means an association and not child) and
        // classname=>wca_form
        arguments.clear();
        ...
        // look for association wca_renderingenginetemplates in the class wca_form => returns a
        // single valid class
        arguments.clear();
        arguments.put("af", "general");
        ...
        // look for childassociation wca_formworkflowdefaults in the class wca_form =>returns a
        // single valid class
        arguments.clear();
        arguments.put("af", "child");
        arguments.put("nsp", "wca");
        ...
        // wca_formworkflowdefaults has a child_assoc relation with wca_form , but ask for general
        // association, this then returns a null array
        arguments.clear();
        arguments.put("af", "general");
        arguments.put("nsp", "wca");
        arguments.put("n", "formworkflowdefaults");
        ...
        // wca_renderingenginetemplates has a general association relation with wca_form , but ask
        // for child association, this then returns a null array
        arguments.clear();
        arguments.put("af", "child");
        arguments.put("nsp", "wca");
        arguments.put("n", "renderingenginetemplates");
        ...
        // look for childassociation  in the class wca_form , with no name parameter =>both name and
        // namespaceprefix are needed
        arguments.clear();
        arguments.put("af", "child");
        arguments.put("nsp", "wca");
        req.setArgs(arguments);
        ...
        // look for associations (excluding child assocs)  in the class wca_form , with no name
        // parameter
        arguments.clear();
        arguments.put("af", "general");
        arguments.put("nsp", "wca");

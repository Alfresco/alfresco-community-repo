...
public class EmailUserNotifier extends AbstractUserNotifier implements InitializingBean {
    ...
    protected boolean skipUser(NodeRef personNodeRef) {
        ...
        if (authenticationContext.isSystemUserName(feedUserId)
                || authenticationContext.isGuestUserName(feedUserId)) {
            ...
            if (lowerEmailAddress.endsWith(excludedEmailSuffix.toLowerCase())) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Skip for '"
                                    + feedUserId
                                    + "' since email address is excluded ("
                                    + emailAddress
                                    + ")");
                }
                return true;
    ...
    protected void notifyUser(
        ...
        // mail.setParameterValue(MailActionExecuter.PARAM_TEXT, buildMailText(emailTemplateRef,
        // model));

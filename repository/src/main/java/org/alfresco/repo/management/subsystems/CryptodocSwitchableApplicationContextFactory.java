...
/**
 * {@link SwitchableApplicationContextFactory} that only allows the subsystem to be switched from
 * unencrypted to encrypted, or if the two subsystems have the same ecrypted state. Switching back
 * to unencrypted from encrypted content store is not allowed.
 *
 * @author Matt Ward
 */
public class CryptodocSwitchableApplicationContextFactory
    ...
    private static final Log logger =
            LogFactory.getLog(CryptodocSwitchableApplicationContextFactory.class);
    ...
    protected class CryptoSwitchableState extends SwitchableState {
        ...
        public void setProperty(String name, String value) {
            if (name.equals(SOURCE_BEAN_PROPERTY)) {
                ...
                try {
                    newSourceBean =
                            getParent().getBean(value, ChildApplicationContextFactory.class);
                } catch (BeansException e) {
                    throw new IllegalStateException(
                            "Switching to the unknown content store \""
                                    + value
                                    + "\" is not possible.");
                }
                ...
                if (canSwitchSubsystemTo(newSourceBean, value)) {
                    ...
                    if (isNewEncrypted && !isEncryptionSupported()) {
                        throw new IllegalStateException(
                                "Switching to the encrypted content store \""
                                        + value
                                        + "\" is not licensed.");
                    }
                } else {
                    throw new IllegalStateException(
                            "Switching to the unencrypted content store \""
                                    + value
                                    + "\" is not possible.");
                }
            }
    ...
    private boolean isEncryptedContentStoreSubsystem(Object sourceBean, String beanName) {
        ...
        if (sourceBean instanceof EncryptedContentStoreChildApplicationContextFactory) {
            isEncrypted =
                    ((EncryptedContentStoreChildApplicationContextFactory) sourceBean)
                            .isEncryptedContent();
        }
        // If not explicitly set as encrypted, check if the source bean is the cryptodoc subsystem
        // bean
        if (!isEncrypted) {

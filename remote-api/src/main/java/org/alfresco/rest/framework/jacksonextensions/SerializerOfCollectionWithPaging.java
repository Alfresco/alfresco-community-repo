...
/**
 * Serializes SerializablePagedCollection into the correct response format, with Paging information
 * and entries
 *
 * @author Gethin James
 */
...
public class SerializerOfCollectionWithPaging extends StdSerializer<SerializablePagedCollection> {
    ...
    public void serialize(
            SerializablePagedCollection pagedCol, JsonGenerator jgen, SerializerProvider provider)
    ...
    private void serializeIncludedSource(SerializablePagedCollection pagedCol, JsonGenerator jgen)
    ...
    private void serializeContext(SerializablePagedCollection pagedCol, JsonGenerator jgen)
            throws IOException, JsonProcessingException {
    ...
    private void serializePagination(SerializablePagedCollection pagedCol, JsonGenerator jgen)
            throws IOException, JsonProcessingException {
        ...
        if (pagedCol.getPaging() != null) {
            jgen.writeNumberField(
                    RecognizedParamsExtractor.PARAM_PAGING_SKIP,
                    pagedCol.getPaging().getSkipCount());
            jgen.writeNumberField(
                    RecognizedParamsExtractor.PARAM_PAGING_MAX, pagedCol.getPaging().getMaxItems());
        }
        jgen.writeEndObject();
    }
}

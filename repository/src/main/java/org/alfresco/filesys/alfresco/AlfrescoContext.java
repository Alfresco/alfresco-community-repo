...
public abstract class AlfrescoContext extends DiskDeviceContext {
  ...
  public static final int DBG_FILE = 0x00000001; // file/folder create/delete
  public static final int DBG_FILEIO = 0x00000002; // file read/write/truncate
  public static final int DBG_SEARCH = 0x00000004; // folder search
  public static final int DBG_INFO = 0x00000008; // file/folder information
  public static final int DBG_LOCK = 0x00000010; // file byte range locking
  public static final int DBG_PSEUDO = 0x00000020; // pseudo files/folders
  public static final int DBG_RENAME = 0x00000040; // rename file/folder

  // Filesystem debug flag strings

  private static final String m_filesysDebugStr[] = {
    "FILE",
    "FILEIO",
    "SEARCH",
    "INFO",
  ...
  public final void setURLFileName(String urlFileName) {
    ...
    if (urlFileName != null) {
      if (!urlFileName.endsWith(".url")) throw new AlfrescoRuntimeException(
        "URL link file must end with .url, " + urlFileName
      );
    }
  }
  ...
  public final void setShareURLFileName(String urlFileName) {
    ...
    if (urlFileName != null) {
      if (!urlFileName.endsWith(".url")) throw new AlfrescoRuntimeException(
        "URL Share link file must end with .url, " + urlFileName
      );
    }
  }
    ...
    if (flagsStr != null) {
      ...
      while (token.hasMoreTokens()) {
        ...
        while (idx < m_filesysDebugStr.length && match == false) {
          if (m_filesysDebugStr[idx].equalsIgnoreCase(dbg) == true) match =
            true; else idx++;
        }

        if (match == false) throw new AlfrescoRuntimeException(
          "Invalid filesystem debug flag, " + dbg
        );

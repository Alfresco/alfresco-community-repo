...
public abstract class AbstractMimeMessage extends MimeMessage {
    ...
    private void setPersistedHeaders() throws MessagingException {
        ...
        if (nodeService.hasAspect(
                messageFileInfo.getNodeRef(), ImapModel.ASPECT_IMAP_MESSAGE_HEADERS)) {
            @SuppressWarnings("unchecked")
            List<String> messageHeaders =
                    (List<String>)
                            nodeService.getProperty(
                                    messageFileInfo.getNodeRef(), ImapModel.PROP_MESSAGE_HEADERS);
            ...
            for (String header : messageHeaders) {
                ...
                String headerName =
                        header.substring(
                                0, header.indexOf(ImapModel.MESSAGE_HEADER_TO_PERSIST_SPLITTER));

...
public class EmailHelper {
    ...
    public void init() {
        ...
        setTemplateLoader(
                new ClassPathRepoTemplateLoader(
                        nodeService, serviceRegistry.getContentService(), "utf-8"));
    }
     ...
     * @return
     *     <ul>
     *       <li>If {@code emailTemplatePath} is empty the fallback template is returned.
     *       <li>if the given {@code emailTemplatePath} is an xpath (i.e. starts with
     *           app:company_home), then an xpath search will be performed to find the {@code
     *           NodeRef}. If no nodeRef is found, the fallback template is returned.
     *       <li>If {@code emailTemplatePath} is a nodeRef and the node does not exist, the fallback
     *           template is returned, otherwise a string representation of the NodeRef is returned.
     *       <li>if {@code emailTemplatePath} is a class path which results in a template being
     *           found, then the {@code emailTemplatePath} is returned; otherwise, the fallback
     *           template is returned.
     *     </ul>
     */
    public String getEmailTemplate(
            String clientName, String emailTemplatePath, String fallbackTemplatePath) {
        if (StringUtils.isEmpty(emailTemplatePath)) {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(
                        "No email template path is set for client ["
                                + clientName
                                + "]. The fallback template will be used: "
        ...
        if (emailTemplatePath.startsWith(companyHomeChildName)) {
            ...
            if (nodeRef == null) {
                LOGGER.warn(
                        "Couldn't find email template with the XPath ["
                                + emailTemplatePath
                                + "] for client ["
                                + clientName
                                + "]. The fallback template will be used: "
            ...
            if (!nodeService.exists(ref)) {
                LOGGER.warn(
                        "Couldn't find email template with the NodeRef ["
                                + ref
                                + "] for client ["
                                + clientName
                                + "]. The fallback template will be used: "
            ...
            // No need to return the nodeRef, as this will be handled later by the
            // 'ClassPathRepoTemplateLoader' class
            ...
            try {
                ...
                if (template != null) {
                    if (LOGGER.isDebugEnabled()) {
                        LOGGER.debug(
                                "Using email template with class path ["
                                        + emailTemplatePath
                                        + "] for client: "
                                        + clientName);
                    }
                    ...
                    LOGGER.warn(
                            "Couldn't find email template with class path ["
                                    + emailTemplatePath
                                    + "] for client ["
                                    + clientName
                                    + "]. The fallback template will be used: "
                ...
                LOGGER.error(
                        "Error occurred while finding the email template with the class path ["
                                + emailTemplatePath
                                + "] for client ["
                                + clientName
                                + "]. The fallback template will be used: "
    ...
    public NodeRef getLocalizedEmailTemplateNodeRef(String emailTemplateXPath) {
        ...
        if (nodeRefs.size() > 1) {
            LOGGER.error(
                    "Found too many email templates using XPath ["
                            + emailTemplateXPath
        ...
        if (userId != null && personService.personExists(userId)) {
            String localeString =
                    AuthenticationUtil.runAsSystem(
                            () -> (String) preferenceService.getPreference(userId, "locale"));

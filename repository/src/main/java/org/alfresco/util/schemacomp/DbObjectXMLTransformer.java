...
public class DbObjectXMLTransformer {
  ...
  private void attemptOutput(DbObject dbObject) throws SAXException {
    ...
    if (dbObject instanceof Schema) {
      ...
      attribs.addAttribute(
        ...
        "xmlns",
        "CDATA",
        "http://www.alfresco.org/repo/db-schema"
      );
      attribs.addAttribute(
        "",
        "",
   ...
   * @throws SAXException
   */
  private void transformValidators(List<DbValidator> validators)
    throws SAXException {
  ...
  private void addAttributes(DbObject dbObject, AttributesImpl attribs) {
    if (dbObject instanceof Schema) {
      ...
      attribs.addAttribute(
        ...
        XML.ATTR_VERSION,
        "CDATA",
        Integer.toString(schema.getVersion())
      );
      attribs.addAttribute(
        "",
        "",
  ...
  private void transformColumn(Column column) throws SAXException {
    ...
    simpleElement(
      XML.EL_AUTOINCREMENT,
      Boolean.toString(column.isAutoIncrement())
    );
  }

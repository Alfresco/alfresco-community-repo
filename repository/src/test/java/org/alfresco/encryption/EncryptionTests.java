...
public class EncryptionTests extends TestCase {
  ...
  private static final String TEST_MODEL =
    "org/alfresco/encryption/reencryption_model.xml";

  private static int NUM_PROPERTIES = 500;
  private static ApplicationContext ctx = ApplicationContextHelper.getApplicationContext();

  private static QName NODE_TYPE = QName.createQName(
    "http://www.alfresco.org/test/reencryption_test/1.0",
    "base"
  );
  private static QName PROP = QName.createQName(
    "http://www.alfresco.org/test/reencryption_test/1.0",
    "prop1"
  );
  ...
  public void setUp() throws Exception {
    ...
    backupKeyStoreParameters =
      (KeyStoreParameters) ctx.getBean("backupKeyStoreParameters");
    keyStoreChecker = (KeyStoreChecker) ctx.getBean("keyStoreChecker");
    encryptionKeysRegistry =
      (EncryptionKeysRegistryImpl) ctx.getBean("encryptionKeysRegistry");
  ...
  protected KeyProvider getKeyProvider(KeyStoreParameters keyStoreParameters) {
    KeyProvider backupKeyProvider = new KeystoreKeyProvider(
      keyStoreParameters,
      keyResourceLoader
    );
  ...
  protected void testChangeKeysImpl(boolean cacheCiphers) throws Throwable {
    ...
    assertEquals(
      "Expected encrypt,decrypt to end up with the original value",
      test,
      test1
    );

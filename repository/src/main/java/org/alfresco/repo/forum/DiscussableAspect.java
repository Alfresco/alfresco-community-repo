...
public class DiscussableAspect
     ...
     *
     * <p>Only the aspect is copied (to get the default behaviour). All topics are copied later.
     *
     * @author Derek Hulley
     * @since 3.2
     */
    private static class DiscussableAspectCopyBehaviourCallback
        ...
        private static final CopyBehaviourCallback INSTANCE =
                new DiscussableAspectCopyBehaviourCallback();
    ...
    public void onAddAspect(NodeRef discussableNodeRef, QName aspectTypeQName) {
        String name =
                (String) this.nodeService.getProperty(discussableNodeRef, ContentModel.PROP_NAME);
    ...
    //
    //    /**
    //     * Copies the discussions to the new node, whilst being sensitive to any existing
    // discussions.
    //     */
    ...
    //                {
    //                    throw new CheckOutCheckInServiceException(
    //                          "Failed to copy topic from working copy to checked out content",
    // fnfe);
    //                }
    //                catch (FileExistsException fee)
    //                {
    //                    throw new CheckOutCheckInServiceException("Failed to checkin content as a
    // topic called " +
    ...
    //
    //        if (logger.isDebugEnabled())
    //            logger.debug("Copied " + copied + " topics from the working copy to the checked
    // out content");
    //    }
    //
        ...
        for (ChildAssociationRef childAssocRef : childAssocRefs) {
            ...
            if (referencedNode != null && this.nodeService.exists(referencedNode) == false) {
                ...
                NodeRef archivedNodeRef =
                        new NodeRef(archiveRootNodeRef.getStoreRef(), referencedNode.getId());
                ...
                NodeRef existingChild =
                        this.nodeService.getChildByName(
                                ...
                                this.nodeService
                                        .getProperty(archivedNodeRef, ContentModel.PROP_NAME)
                                        .toString());

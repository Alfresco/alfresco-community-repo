...
public class ActionFormProcessor
   ...
   */
  @Override
  protected ActionFormResult internalPersist(
    ...
    // extract the result
    Object result = actionToExecute.getParameterValue(
      ActionExecuter.PARAM_RESULT
    );
  ...
  private Action createActionAndParams(ActionDefinition item, FormData data) {
    ...
    try {
      ...
      throw new FormException(
        "Unrecognised action name " + item.getName(),
        nsbdx
      );
    }
    ...
    for (ParameterDefinition actionParamDef : actionParamDefs) {
      ...
      FieldData fieldData = data.getFieldData(
        FormFieldConstants.PROP_DATA_PREFIX + paramDefinitionName
      );
      if (fieldData == null) {
        fieldData =
          data.getFieldData(
            FormFieldConstants.ASSOC_DATA_PREFIX +
            paramDefinitionName +
      ...
      if (fieldData != null) {
        ...
        DataTypeDefinition typeDef = dictionaryService.getDataType(
          expectedParamType
        );
        ...
        if (actionParamDef.isMultiValued()) {
          if (fieldValueObj instanceof String) {
            ...
            try {
              ...
              throw new FormException(
                "Failed to convert JSONArray to List",
                je
              );
            }
          ...
          if (fieldValueObj instanceof Collection<?>) {
            convertedObj =
              DefaultTypeConverter.INSTANCE.convert(
                typeDef,
                (Collection<?>) fieldValueObj
              );
          } else if (fieldValueObj != null) {
            throw new FormException(
              "Not a recognized multi-value parameter value instance: " +
              convertedObj
            );
          }
        } else {
          if (fieldValueObj instanceof String) {
            if (((String) fieldValueObj).length() == 0) {
              ...
              if (((String) fieldValueObj).charAt(0) == QName.NAMESPACE_BEGIN) {
                ...
                fieldValueObj =
                  QName.createQName((String) fieldValueObj, namespaceService);
              }
            }
          }
          convertedObj =
            DefaultTypeConverter.INSTANCE.convert(typeDef, fieldValueObj);
        }
  ...
  private NodeRef getActionedUponNodeRef(ActionDefinition item, FormData data) {
    ...
    if (actionedUponNodeRef != null) {
      ...
      if (NodeRef.isNodeRef(nodeRefString)) {
        ...
        throw new FormException(
          "Illegal actionedUponNodeRef: " + nodeRefString
        );
      }
    }
   ...
   */
  private boolean isAsynchronousActionRequest(
    ActionDefinition item,
    FormData data
  ) {
  ...
  protected List<Field> generateDefaultFields(
    ...
    for (ParameterDefinition paramDef : itemData
      .getActionDefinition()
      .getParameterDefinitions()) {
      if (!fieldsToIgnore.contains(paramDef.getName())) {
        ActionParameterField actionParameterField = new ActionParameterField(
          paramDef,
          actionService
        );
  ...
  @Override
  protected List<Field> generateSelectedFields(
  ...
  protected Object makeItemData(ActionDefinition item) {
    ...
    try {
      ...
      throw new FormException(
        "Failed to create action '" + item.getName() + "'",
        nsbdx
      );
    }

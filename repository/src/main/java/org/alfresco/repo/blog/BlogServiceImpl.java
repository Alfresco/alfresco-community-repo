...
public class BlogServiceImpl implements BlogService {
    ...
    private DraftsAndPublishedBlogPostsCannedQueryFactory
    ...
    public void setCannedQueryRegistry(
            NamedObjectRegistry<CannedQueryFactory<BlogPostInfo>> cannedQueryRegistry) {
        this.cannedQueryRegistry = cannedQueryRegistry;
    }

    public void setDraftBlogPostsCannedQueryFactory(
            GetBlogPostsCannedQueryFactory cannedQueryFactory) {
        this.draftPostsCannedQueryFactory = cannedQueryFactory;
    }

    public void setPublishedBlogPostsCannedQueryFactory(
            GetBlogPostsCannedQueryFactory cannedQueryFactory) {
        this.publishedPostsCannedQueryFactory = cannedQueryFactory;
    }

    public void setPublishedExternallyBlogPostsCannedQueryFactory(
            GetBlogPostsCannedQueryFactory cannedQueryFactory) {
        this.publishedExternallyPostsCannedQueryFactory = cannedQueryFactory;
    }

    public void setDraftsAndPublishedBlogPostsCannedQueryFactory(
            DraftsAndPublishedBlogPostsCannedQueryFactory cannedQueryFactory) {
    ...
    @Override
    public BlogPostInfo createBlogPost(
            String siteShortName, String blogTitle, String blogContent, boolean isDraft) {
    ...
    @Override
    public BlogPostInfo createBlogPost(
            NodeRef blogContainerNode, String blogTitle, String blogContent, boolean isDraft) {
        ...
        ContentWriter writer =
                contentService.getWriter(postNode.getChildRef(), ContentModel.PROP_CONTENT, true);
        ...
        if (isDraft) {
            ...
            // disable permission inheritance. The result is that only the creator will have access
            // to the draft
            NodeRef draft = postNode.getChildRef();
            permissionService.setInheritParentPermissions(draft, false);

            // MNT-12082: give permissions to the post creator. He should be able to comment in his
            // post's
            ...
            permissionService.setPermission(
                    draft, creator, permissionService.getAllPermission(), true);
        } else {
            setOrUpdateReleasedAndUpdatedDates(postNode.getChildRef());
        }

        BlogPostInfo post =
                new BlogPostInfoImpl(postNode.getChildRef(), blogContainerNode, nodeName);
    ...
    public BlogPostInfo updateBlogPost(BlogPostInfo post) {
        if (post.getNodeRef() == null) {
            throw new IllegalArgumentException(
                    "Can't update a post that was never persisted, call create instead");
        }
        ...
        try {
            postNode =
                    nodeService.getChildByName(
                            parentNodeRef, ContentModel.ASSOC_CONTAINS, postName);
    ...
    @Override
    public PagingResults<BlogPostInfo> getDrafts(
            String siteShortName, String username, PagingRequest pagingReq) {
        NodeRef container = getSiteBlogContainer(siteShortName, false);
        if (container == null) {
            // No blog posts yet
    ...
    @Override
    public PagingResults<BlogPostInfo> getDrafts(
            NodeRef blogContainerNode, String username, PagingRequest pagingReq) {
        ParameterCheck.mandatory("blogContainerNode", blogContainerNode);
        ParameterCheck.mandatory("pagingReq", pagingReq);

        // get canned query
        GetBlogPostsCannedQuery cq =
                (GetBlogPostsCannedQuery)
                        draftPostsCannedQueryFactory.getGetDraftsCannedQuery(
                                blogContainerNode, username, pagingReq);
    ...
    @Override
    public PagingResults<BlogPostInfo> getPublished(
    ...
    public PagingResults<BlogPostInfo> getPublished(
        ...
        GetBlogPostsCannedQuery cq =
                (GetBlogPostsCannedQuery)
                        publishedPostsCannedQueryFactory.getGetPublishedCannedQuery(
                                blogContainerNode, fromDate, toDate, byUser, pagingReq);
    ...
    /** @deprecated */
    @Override
    public PagingResults<BlogPostInfo> getMyDraftsAndAllPublished(
            NodeRef blogContainerNode, Date createdFrom, Date createdTo, PagingRequest pagingReq) {
        ParameterCheck.mandatory("blogContainerNode", blogContainerNode);
        ParameterCheck.mandatory("pagingReq", pagingReq);

        // get canned query
        String currentUser = AuthenticationUtil.getFullyAuthenticatedUser();
        DraftsAndPublishedBlogPostsCannedQuery cq =
                (DraftsAndPublishedBlogPostsCannedQuery)
                        draftsAndPublishedBlogPostsCannedQueryFactory.getCannedQuery(
                                blogContainerNode, createdFrom, createdTo, currentUser, pagingReq);
    ...
    private void setOrUpdateReleasedAndUpdatedDates(NodeRef blogPostNode) {
        ...
        if (!permissionService.getInheritParentPermissions(blogPostNode)) {
            // MNT-12082
            String creator =
                    (String) nodeService.getProperty(blogPostNode, ContentModel.PROP_CREATOR);
            permissionService.deletePermission(
                    blogPostNode, creator, permissionService.getAllPermission());
    ...
    @Override
    public PagingResults<BlogPostInfo> findBlogPosts(
            final NodeRef blogContainerNode,
            final RangedDateProperty dateRange,
            final String tag,
            final PagingRequest pagingReq) {
        ...
        luceneQuery
                ...
                .append("\" ")
                .append("+PARENT:\"")
                .append(blogContainerNode.toString())
                .append("\" ");
        if (tag != null && !tag.trim().isEmpty()) {
            luceneQuery
                    .append("+PATH:\"/cm:taggable/cm:")
                    .append(ISO9075.encode(tag))
                    .append("/member\"");
        }
        if (dateRange != null) {
            luceneQuery.append(
        ...
        try {
            ...
            final List<NodeRef> nodeRefs =
                    finalLuceneResults
                            .getNodeRefs()
                            .subList(0, min(pagingReq.getMaxItems(), finalLuceneResults.length()));

            results =
                    new PagingResults<BlogPostInfo>() {

                        @Override
                        public List<BlogPostInfo> getPage() {
                            List<BlogPostInfo> blogPostInfos =
                                    new ArrayList<BlogPostInfo>(nodeRefs.size());
                            for (NodeRef nodeRef : nodeRefs) {
                                ...
                                blogPostInfos.add(
                                        new BlogPostInfoImpl(nodeRef, blogContainerNode, postName));
                            }
                        ...
                        public Pair<Integer, Integer> getTotalResultCount() {
                            ...
                            return new Pair<Integer, Integer>(
                                    (int) totalResultCount, (int) totalResultCount);
                        }
    ...
    private PagingResults<BlogPostInfo> wrap(
            final CannedQueryResults<BlogEntity> results, final NodeRef containerNodeRef) {
    ...
    /**
     * This method creates a Lucene query fragment which constrains the specified dateProperty to a
     * range given by the fromDate and toDate parameters.
     *
     ...
     * @param dateProperty the Alfresco property value to check against the range (must be a valid
     *     Date or DateTime property).
     * @return the Lucene query fragment.
     * @throws NullPointerException if dateProperty is null or if the dateProperty is not recognised
     *     by the system.
     * @throws IllegalArgumentException if dateProperty refers to a property that is not of type
     *     {@link DataTypeDefinition#DATE} or {@link DataTypeDefinition#DATETIME}.
     */
    private String createDateRangeQuery(Date fromDate, Date toDate, QName dateProperty) {
        return LuceneUtils.createDateRangeQuery(
                fromDate, toDate, dateProperty, dictionaryService, namespaceService);
    }
}

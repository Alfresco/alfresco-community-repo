...
/**
 * Executes JavaScript virtual folders templates ensuring their required context and the {@link
...
public class TemplateResourceProcessor implements ResourceProcessor<VirtualFolderDefinition> {
    ...
    /**
     * Processes the JavaScript template given by the {@link ClasspathResource} and creates a {@link
     * VirtualFolderDefinition} (composite) structure that represents the virtual folder.
     *
     * @param A {@link ClasspathResource} reference.
     * @return A {@link VirtualFolderDefinition} reference that represents the structure (tree of
     *     nodes) of the virtual folder.
     */
    @SuppressWarnings("unchecked")
    @Override
    public VirtualFolderDefinition process(ClasspathResource classpath)
            throws ResourceProcessingError {
        Object result;
        try {
            result =
                    context.getActualEnviroment()
                            .executeScript(
                                    classpath.getClasspath(), createScriptParameters(context));
    ...
    /**
     * Processes the JavaScript template given by the {@link RepositoryResource} and creates a
     * {@link VirtualFolderDefinition} (composite) structure that represents the virtual folder.
     *
    ...
    @SuppressWarnings("unchecked")
    @Override
    public VirtualFolderDefinition process(RepositoryResource repositoryResource)
            throws ResourceProcessingError {
        Object result;
        try {
            NodeRef templateNodeRef =
                    repositoryResource.getLocation().asNodeRef(context.getActualEnviroment());
            result =
                    context.getActualEnviroment()
                            .executeScript(templateNodeRef, createScriptParameters(context));
            return asVirtualStructure(context, (Map<String, Object>) result);
        } catch (ActualEnvironmentException e) {
            throw new ResourceProcessingError(e);
        }
     ...
     */
    protected Map<String, Object> createScriptParameters(VirtualContext context)
            throws ActualEnvironmentException {
        Map<String, Object> parameters = context.getParameters();
        Object scriptContext = context.getActualEnviroment().createScriptVirtualContext(context);
        parameters.put(VirtualContext.CONTEXT_PARAM, scriptContext);
        return parameters;
    }

    /**
     * Creates a filing rule based on the criteria (path, classification: type, aspects) given in
     * the template.
     *
     * @param context The context in which the virtualization process takes place.
     * @param filing A map containing the filing criteria used to create the rule.
     * @return A {@link FilingRule} reference.
     * @throws ResourceProcessingError
     */
    @SuppressWarnings("unchecked")
    protected FilingRule asFilingRule(VirtualContext context, Object filing)
            throws ResourceProcessingError {
        if (filing == null) {
            return null;
        }
        ...
        Map<String, Object> classificationMap =
                (Map<String, Object>) filingMap.get(CLASSIFICATION_KEY);
     ...
     *
     * @param context The context in which the virtualization process takes place.
     * @param search A map containing the details that define the query: language, store, query
     *     statement.
    ...
    /**
     * Creates a {@link VirtualFolderDefinition} that represents the structure of the virtual
     * folder.
     *
     * @param context The context in which the virtualization process takes place.
     * @param result A map containing the details that define the virtual entries.
     * @return a {@link VirtualFolderDefinition} reference.
     * @throws ResourceProcessingError
     */
    protected VirtualFolderDefinition asVirtualStructure(
            VirtualContext context, Map<String, Object> result) throws ResourceProcessingError {
        try {
            ...
            VirtualQuery virtualQuery =
                    asVirtualQuery(context, (Map<String, Object>) mapResult.get(SEARCH_KEY));

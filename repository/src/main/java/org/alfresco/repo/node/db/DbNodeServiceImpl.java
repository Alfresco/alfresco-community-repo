...
public class DbNodeServiceImpl extends AbstractNodeServiceImpl implements Extensible, NodeService {
    ...
    public DbNodeServiceImpl() {
        nodeServiceTrait =
                new ExtendedTrait<NodeServiceTrait>(
                        AJProxyTrait.create(this, NodeServiceTrait.class));
    ...
    private Pair<Long, NodeRef> getNodePairNotNull(NodeRef nodeRef) throws InvalidNodeRefException {
        ...
        if (unchecked == null) {
            ...
            throw new InvalidNodeRefException(
                    "Node does not exist: " + nodeRef + " (status:" + nodeStatus + ")", nodeRef);
        }
        return unchecked;
    }
    ...
    public ChildAssociationRef createNode(
        ...
        if (assocQName.getLocalName().length() > QName.MAX_LENGTH) {
            throw new IllegalArgumentException(
                    "Localname is too long. Length of "
                            + assocQName.getLocalName().length()
        ...
        Map<QName, Serializable> propertiesAfter =
                nodeDAO.getNodeProperties(childNodePair.getFirst());
    ...
    private boolean addAspectsAndProperties(
        ...
        return addAspectsAndProperties(
                nodePair,
                classQName,
                null,
                existingAspects,
    ...
    private boolean addAspectsAndProperties(
        ...
        if (assocTypeQName != null) {
            missingAspects.addAll(
                    getMissingAspectsAssoc(existingAspects, allProperties, assocTypeQName));
        }
        ...
        Map<QName, Serializable> missingProperties =
                getMissingProperties(existingProperties, allClassQNames);
    ...
    private Set<QName> getMissingAspects(
        ...
        for (QName missingAspect : missingAspectsCopy) {
            Set<QName> furtherMissingAspects =
                    getMissingAspects(
                            allTypesAndAspects,
                            Collections.<QName, Serializable>emptyMap(),
                            missingAspect);
     ...
     */
    private Map<QName, Serializable> getMissingProperties(
            Map<QName, Serializable> existingProperties, Set<QName> classQNames) {
        ...
        Map<QName, Serializable> missingProperties =
                new HashMap<QName, Serializable>(allDefaultProperties);
    ...
    public void addAspect(
        ...
        if (aspectDef == null) {
            throw new InvalidAspectException(
                    "The aspect is invalid: " + aspectTypeQName, aspectTypeQName);
        }

        // Don't allow spoofed aspect(s) to be added
        if (aspectTypeQName.equals(ContentModel.ASPECT_PENDING_DELETE)) {
            throw new IllegalArgumentException(
                    "The aspect is reserved for system use: " + aspectTypeQName);
        }
    ...
    public void removeAspect(NodeRef nodeRef, QName aspectTypeQName)
        ...
        if (aspectDef != null) {
            ...
            // child
            final List<Pair<Long, ChildAssociationRef>> assocsToDelete =
                    new ArrayList<Pair<Long, ChildAssociationRef>>(5);
            ...
            NodeDAO.ChildAssocRefQueryCallback callback =
                    new NodeDAO.ChildAssocRefQueryCallback() {
                        ...
                        public boolean handle(
                            ...
                            if (isPendingDelete(parentNodePair.getSecond())
                                    || isPendingDelete(childNodePair.getSecond())) {
                                if (logger.isTraceEnabled()) {
                                    logger.trace(
                                            "Aspect-triggered association removal: Ignoring child"
                                                + " associations where one of the nodes is pending"
                                                + " delete: "
                                                    + childAssocPair);
                                }
                                return true;
                            ...
                            // Double check that it's not a primary association.  If so, we can't
                            // delete it and
            ...
            Set<QName> assocTypeQNamesToRemove =
                    new HashSet<QName>(aspectDef.getChildAssociations().keySet());
            ...
            for (Map.Entry<QName, AssociationDefinition> entry : nodeAssocDefs.entrySet()) {
                if (isPendingDelete(nodeRef)) {
                    if (logger.isTraceEnabled()) {
                        logger.trace(
                                "Aspect-triggered association removal: Ignoring peer associations"
                                        + " where one of the nodes is pending delete: "
                                        + nodeRef);
                    }
                    continue;
                }
                ...
                Collection<Pair<Long, AssociationRef>> targetAssocRefs =
                        nodeDAO.getTargetNodeAssocs(nodeId, assocTypeQName);
                for (Pair<Long, AssociationRef> assocPair : targetAssocRefs) {
                    if (isPendingDelete(assocPair.getSecond().getTargetRef())) {
                        if (logger.isTraceEnabled()) {
                            logger.trace(
                                    "Aspect-triggered association removal: Ignoring peer"
                                        + " associations where one of the nodes is pending delete: "
                                            + assocPair);
                        }
                        continue;
                    }
                ...
                // MNT-9580: Daisy chained cm:original associations are cascade-deleted when the
                // first original is deleted
                //           As a side-effect of the investigation of MNT-9446, it was dicovered
                // that inbound associations (ones pointing *to* this aspect)
                //           were also being removed.  This is incorrect because the aspect being
                // removed here has no say over who points at it.
                //           Therefore, do not remove inbound associations because we only define
                // outbound associations on types and aspects.
                //           Integrity checking will ensure that the correct behaviours are in place
    ...
    private boolean isPendingDelete(NodeRef nodeRef) {
        ...
        Set<NodeRef> nodesPendingDelete =
                TransactionalResourceHelper.getSet(KEY_PENDING_DELETE_NODES);
    ...
    private void deleteNode(NodeRef nodeRef, boolean allowArchival) {
        ...
        Set<NodeRef> nodesPendingDeleteTxn =
                TransactionalResourceHelper.getSet(KEY_PENDING_DELETE_NODES);
        ...
        for (VisitedNode nodeToDelete : nodesToDelete) {
            ...
            for (Pair<Long, ChildAssociationRef> secondaryChildAssocPair :
                    nodeToDelete.secondaryChildAssocs) {
            ...
            for (Pair<Long, ChildAssociationRef> secondaryParentAssocPair :
                    nodeToDelete.secondaryParentAssocs) {
    ...
    public ChildAssociationRef addChild(
            NodeRef parentRef, NodeRef childRef, QName assocTypeQName, QName assocQName) {
        return addChild(Collections.singletonList(parentRef), childRef, assocTypeQName, assocQName)
                .get(0);
    }
    ...
    public List<ChildAssociationRef> addChild(
        ...
        Long childNodeId = childNodePair.getFirst();
        Map<QName, Serializable> childNodeProperties =
                nodeDAO.getNodeProperties(childNodePair.getFirst());
        ...
        List<ChildAssociationRef> childAssociationRefs =
                new ArrayList<ChildAssociationRef>(parentRefs.size());
        List<Pair<Long, NodeRef>> parentNodePairs =
                new ArrayList<Pair<Long, NodeRef>>(parentRefs.size());
        for (NodeRef parentRef : parentRefs) {
            ...
            Pair<Long, ChildAssociationRef> childAssocPair =
                    nodeDAO.newChildAssoc(
                            parentNodeId, childNodeId, assocTypeQName, assocQName, childNodeName);
        ...
        for (Pair<Long, NodeRef> parentNodePair : parentNodePairs) {
            addAspectsAndPropertiesAssoc(
                    parentNodePair, assocTypeQName, null, null, null, null, false);
        }
    ...
    public void removeChild(NodeRef parentRef, NodeRef childRef) throws InvalidNodeRefException {
        ...
        Pair<Long, ChildAssociationRef> primaryChildAssocPair =
    ...
    public boolean removeChildAssociation(ChildAssociationRef childAssocRef) {
        ...
        Pair<Long, ChildAssociationRef> assocPair =
                nodeDAO.getChildAssoc(parentNodeId, childNodeId, assocTypeQName, assocQName);
        if (assocPair == null) {
     ...
     */
    private boolean setPropertiesCommonWork(
            Pair<Long, NodeRef> nodePair, Map<QName, Serializable> properties) {
        Long nodeId = nodePair.getFirst();
        ...
        if (properties.containsKey(ContentModel.PROP_NAME)) {
            ...
            Pair<Long, ChildAssociationRef> primaryParentAssocPair =
                    nodeDAO.getPrimaryParentAssoc(nodeId);
     ...
     *
     * <p>To remove a property, <b>remove it from the map</b> before calling this method.
     * Null-valued properties are allowed.
     *
     * <p>If any of the values are null, a marker object is put in to mimic nulls. They will be
     * turned back into a real nulls when the properties are requested again.
     */
    @Extend(traitAPI = NodeServiceTrait.class, extensionAPI = NodeServiceExtension.class)
    public void setProperties(NodeRef nodeRef, Map<QName, Serializable> properties)
        ...
        boolean changed =
                addAspectsAndProperties(nodePair, null, null, null, null, properties, true);
    ...
    public void addProperties(NodeRef nodeRef, Map<QName, Serializable> properties)
        ...
        boolean changed =
                addAspectsAndProperties(nodePair, null, null, null, null, properties, false);
    ...
    public Collection<NodeRef> getParents(NodeRef nodeRef) throws InvalidNodeRefException {
        List<ChildAssociationRef> parentAssocs =
                getParentAssocs(nodeRef, RegexQNamePattern.MATCH_ALL, RegexQNamePattern.MATCH_ALL);
    ...
    public List<ChildAssociationRef> getChildAssocs(
            NodeRef nodeRef, final QNamePattern typeQNamePattern, final QNamePattern qnamePattern) {
                ...
                new ChildAssocRefQueryCallback() {
                    ...
                    public boolean handle(
                        ...
                        if (typeQNamePattern != null
                                && !typeQNamePattern.isMatch(
                                        childAssocPair.getSecond().getTypeQName())) {
                            return true;
                        }
                        if (qnamePattern != null
                                && !qnamePattern.isMatch(childAssocPair.getSecond().getQName())) {
                            return true;
                        }
                        results.add(childAssocPair.getSecond());
                        return true;
    ...
    public NodeRef getChildByName(NodeRef nodeRef, QName assocTypeQName, String childName) {
        ...
        Pair<Long, ChildAssociationRef> childAssocPair =
                nodeDAO.getChildAssoc(nodeId, assocTypeQName, childName);
    ...
    public void setAssociations(NodeRef sourceRef, QName assocTypeQName, List<NodeRef> targetRefs) {
        ...
        Collection<Pair<Long, AssociationRef>> assocsBefore =
                nodeDAO.getTargetNodeAssocs(sourceNodeId, assocTypeQName);
    ...
    /**
     * Specific properties <b>not</b> supported by {@link #getChildAssocsByPropertyValue(NodeRef,
     * QName, Serializable)}
     */
    ...
    public List<ChildAssociationRef> getChildAssocsByPropertyValue(
        ...
        if (getChildAssocsByPropertyValueBannedProps.contains(propertyQName)) {
            throw new IllegalArgumentException(
                    "getChildAssocsByPropertyValue does not allow search of system maintained"
                            + " properties: "
                            + propertyQName);
        }
    ...
    public List<AssociationRef> getTargetAssocs(NodeRef sourceRef, QNamePattern qnamePattern) {
        ...
        Collection<Pair<Long, AssociationRef>> assocPairs =
                nodeDAO.getTargetNodeAssocs(sourceNodeId, qnameFilter);
    ...
    public List<AssociationRef> getTargetAssocsByPropertyValue(
        ...
        if (getChildAssocsByPropertyValueBannedProps.contains(propertyQName)) {
            throw new IllegalArgumentException(
                    "getTargetAssocsByPropertyValue does not allow search of system maintained"
                            + " properties: "
                            + propertyQName);
        }

        Collection<Pair<Long, AssociationRef>> assocPairs =
                nodeDAO.getTargetAssocsByPropertyValue(
                        sourceNodeId, qnameFilter, propertyQName, propertyValue);
        List<AssociationRef> nodeAssocRefs = new ArrayList<AssociationRef>(assocPairs.size());
        for (Pair<Long, AssociationRef> assocPair : assocPairs) {
            AssociationRef assocRef = assocPair.getSecond();
            // check qname pattern, if not already filtered
    ...
    public List<AssociationRef> getSourceAssocs(NodeRef targetRef, QNamePattern qnamePattern) {
        ...
        Collection<Pair<Long, AssociationRef>> assocPairs =
                nodeDAO.getSourceNodeAssocs(targetNodeId, qnameFilter);
        List<AssociationRef> nodeAssocRefs = new ArrayList<AssociationRef>(assocPairs.size());
        for (Pair<Long, AssociationRef> assocPair : assocPairs) {
            AssociationRef assocRef = assocPair.getSecond();
            // check qname pattern, if not already filtered
    ...
    private void archiveHierarchyImpl(NodeHierarchyWalker walker, StoreRef archiveStoreRef) {
        ...
        Map<Long, Pair<Long, NodeRef>> archiveRecord =
                new HashMap<Long, Pair<Long, NodeRef>>(walker.getNodes(false).size() * 2);
        for (VisitedNode node : walker.getNodes(false)) {
            ...
            if (firstNode) {
                ...
                archiveProperties.put(
                        ContentModel.PROP_ARCHIVED_BY,
                        AuthenticationUtil.getFullyAuthenticatedUser());
                archiveProperties.put(ContentModel.PROP_ARCHIVED_DATE, new Date());
                archiveProperties.put(
                        ContentModel.PROP_ARCHIVED_ORIGINAL_PARENT_ASSOC, primaryParentAssocRef);
                ...
                archiveProperties.put(
                        ContentModel.PROP_OWNER, AuthenticationUtil.getFullyAuthenticatedUser());
                // Create new primary association
                archivePrimaryParentAssocRef =
                ...
                if (parentNode == null) {
                    throw new IllegalStateException(
                            "Expected that a child has a visited primary parent: "
                                    + primaryParentAssocRef);
                }
            ...
            while (true) {
                try {
                    ChildAssocEntity archiveChildAssocEntity =
                            nodeDAO.newNode(
            ...
            Set<AccessPermission> originalNodePermissions =
                    permissionService.getAllSetPermissions(node.nodeRef);
            ...
            if (firstNode) {
                ...
                if (!nodeDAO.hasNodeAspect(
                        archiveStoreRootNodeId, ContentModel.ASPECT_ARCHIVE_ROOT)) {
                ...
                Pair<Long, ChildAssociationRef> userArchiveAssocPair =
                        nodeDAO.getChildAssoc(
                ...
                if (userArchiveAssocPair == null) {
                    ...
                    QName archiveUserAssocQName =
                            QName.createQName(
                                    NamespaceService.CONTENT_MODEL_1_0_URI,
                                    QName.createValidLocalName(username));
                    Map<QName, Serializable> userArchiveNodeProps =
     ...
     * {@inheritDoc}
     *
     * <p>Archives the node without the <b>cm:auditable</b> aspect behaviour
     */
    @Extend(traitAPI = NodeServiceTrait.class, extensionAPI = NodeServiceExtension.class)
    public NodeRef restoreNode(
        ...
        policyBehaviourFilter.disableBehaviour(ContentModel.ASPECT_AUDITABLE);
        try {
            return restoreNodeImpl(
                    archivedNodeRef, destinationParentNodeRef, assocTypeQName, assocQName);
        } finally {
            policyBehaviourFilter.enableBehaviour(ContentModel.ASPECT_AUDITABLE);
        }
    }

    private NodeRef restoreNodeImpl(
            NodeRef archivedNodeRef,
            NodeRef destinationParentNodeRef,
            QName assocTypeQName,
            QName assocQName) {
        ...
        List<ChildAssociationRef> parentAssocsToRemove =
                getParentAssocs(
                        archivedNodeRef,
                        ContentModel.ASSOC_ARCHIVED_LINK,
        ...
        ChildAssociationRef originalPrimaryParentAssocRef =
                (ChildAssociationRef)
                        existingProperties.get(ContentModel.PROP_ARCHIVED_ORIGINAL_PARENT_ASSOC);
        Serializable originalOwner =
                existingProperties.get(ContentModel.PROP_ARCHIVED_ORIGINAL_OWNER);
        ...
        ChildAssociationRef newChildAssocRef =
                moveNode(archivedNodeRef, destinationParentNodeRef, assocTypeQName, assocQName);
     ...
     * Move Node
     *
     * <p>Drops the old primary association and creates a new one
     */
    @Extend(traitAPI = NodeServiceTrait.class, extensionAPI = NodeServiceExtension.class)
    public ChildAssociationRef moveNode(
        ...
        List<ChildAssociationRef> nodesToRestoreAssociationsFor =
                new ArrayList<ChildAssociationRef>();

        // Get the primary parent association
        Pair<Long, ChildAssociationRef> oldParentAssocPair =
                nodeDAO.getPrimaryParentAssoc(nodeToMoveId);
        if (oldParentAssocPair == null) {
            // The node doesn't have parent.  Moving it is not possible.
            throw new IllegalArgumentException(
                    "Node "
                            + nodeToMoveId
                            + " doesn't have a parent.  Use 'addChild' instead of move.");
        }
        ...
        if (movingStore) {
            ...
            Map<NodeRef, Pair<Long, NodeRef>> movedNodePairs =
                    new HashMap<NodeRef, Pair<Long, NodeRef>>(childAssocs.size() * 2 + 2);
            ...
            Pair<Pair<Long, ChildAssociationRef>, Pair<Long, NodeRef>> moveNodeResult =
                    nodeDAO.moveNode(nodeToMoveId, parentNodeId, assocTypeQName, assocQName);
    ...
    private void findNodeChildrenToMove(
            Long nodeId,
            final StoreRef storeRef,
            final List<ChildAssociationRef> childAssocsToMove,
            final Map<NodeRef, Long> nodeIds) {
     ...
     */
    private boolean setChildNameUnique(
            Pair<Long, NodeRef> childNodePair, String newName, String oldName) {
        if (newName == null) {
     ...
     * association, along with the <b>cm:modifier</b> of who changed it. The parent node has to be
     * <b>cm:auditable</b> and the association has to be marked for propagation as well.
     *
    ...
    private void propagateTimeStamps(ChildAssociationRef assocRef) {
        ...
        if (assocDef == null || !assocDef.isChild()) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Not propagating cm:auditable for unknown association type "
        ...
        if (!childAssocDef.getPropagateTimestamps()) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Not propagating cm:auditable for association type "
                                + childAssocDef.getName());
            }
            return;
        }
        ...
        if (!policyBehaviourFilter.isEnabled(parentNodeRef, ContentModel.ASPECT_AUDITABLE)) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Not propagating cm:auditable for non-auditable parent on " + assocRef);
            }
            return;
        }
        ...
        if (TransactionalResourceHelper.getMap(KEY_AUDITABLE_PROPAGATION_PRE)
                .containsKey(parentNodeId)) {
        ...
        if (nodeDAO.isInCurrentTxn(parentNodeId)) {
            ...
            TransactionalResourceHelper.getMap(KEY_AUDITABLE_PROPAGATION_PRE)
                    .put(parentNodeId, childNodeId);
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Performing in-transaction cm:auditable update for "
                                + parentNodePair
                                + " from "
                                + childNodePair);
            }
        } else {
            TransactionalResourceHelper.getMap(KEY_AUDITABLE_PROPAGATION_POST)
                    .put(parentNodeId, childNodeId);

            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Requesting later cm:auditable update for "
                                + parentNodePair
                                + " from "
                                + childNodePair);
            }
    ...
    private AuditableTransactionListener auditableTransactionListener =
            new AuditableTransactionListener();
    /**
    ...
    private class AuditableTransactionListener extends TransactionListenerAdapter {
        ...
        public void beforeCommit(boolean readOnly) {
            ...
            if (readOnly) {
                throw new IllegalStateException(
                        "Attempting to modify parent cm:modified in read-only txn.");
            }

            Map<Long, Long> parentNodeIds =
                    TransactionalResourceHelper.getMap(KEY_AUDITABLE_PROPAGATION_PRE);
         ...
         *     and the updating child node from which to get the <b>cm:modifier</b> from
         * @param modifiedDate the date to set
         * @param useCurrentTxn <tt>true</tt> to use the current transaction
         */
        private void process(
                final Map<Long, Long> parentNodeIds, Date modifiedDate, boolean useCurrentTxn) {
            // Walk through the IDs
            for (Long parentNodeId : parentNodeIds.keySet()) {
                processSingle(
                        parentNodeId, parentNodeIds.get(parentNodeId), modifiedDate, useCurrentTxn);
            }
        }
        ...
        private void processSingle(
            ...
            RetryingTransactionCallback<Void> callback =
                    new RetryingTransactionCallback<Void>() {
                        ...
                        public Void execute() throws Throwable {
                            ...
                            } else if (!nodeDAO.hasNodeAspect(
                                    parentNodeId, ContentModel.ASPECT_AUDITABLE)) {
                            ...
                            if (childNodePair == null) {
                                // Child has gone away, can't fetch details from children's
                                // properties
                                modifiedByToPropagate =
                                        AuthenticationUtil.getFullyAuthenticatedUser();
                            } else if (!nodeDAO.hasNodeAspect(
                                    childNodeId, ContentModel.ASPECT_AUDITABLE)) {
                                ...
                                modifiedByToPropagate =
                                        (String)
                                                nodeDAO.getNodeProperty(
                                                        childNodeId, ContentModel.PROP_MODIFIER);
                                modifiedDateToPropagate =
                                        (Date)
                                                nodeDAO.getNodeProperty(
                                                        childNodeId, ContentModel.PROP_MODIFIED);
                            }

                            // Did another child get there first?
                            Date parentModifiedAt =
                                    (Date)
                                            nodeDAO.getNodeProperty(
                                                    parentNodeId, ContentModel.PROP_MODIFIED);
                            if (parentModifiedAt != null
                                    && modifiedDateToPropagate != null
                                    && parentModifiedAt.getTime()
                                            > modifiedDateToPropagate.getTime()) {
                                // Parent was modified more recently, don't update
                                if (logger.isDebugEnabled()) {
                                    logger.debug(
                                            "Parent "
                                                    + parentNodeRef
                                                    + " was modified more recently than child "
                                                    + childNodePair
                                                    + " so not propogating auditable details");
                                }
                                return null;
                            }
                            ...
                            boolean changed =
                                    nodeDAO.setModifiedProperties(
                                            parentNodeId, modifiedDate, modifiedByToPropagate);
            ...
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Touched cm:modified date for node "
                                    + parentNodeId
                                    + " ("
                                    + modifiedDate
                                    + ")"

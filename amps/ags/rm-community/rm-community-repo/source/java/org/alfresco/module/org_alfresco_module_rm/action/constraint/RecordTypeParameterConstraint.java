...
public class RecordTypeParameterConstraint extends BaseParameterConstraint {
    ...
    protected Map<String, String> getAllowableValuesImpl() {
        return AuthenticationUtil.runAsSystem(
                new RunAsWork<Map<String, String>>() {
                    @SuppressWarnings("unchecked")
                    public Map<String, String> doWork() {
                        Map<String, String> result = Collections.EMPTY_MAP;

                        // get the file plan
                        // TODO we will likely have to re-implement as a custom control so that
                        // context of the file
                        ...
                        if (filePlan != null) {
                            Set<QName> recordTypes =
                                    recordService.getRecordMetadataAspects(filePlan);

                            result = new HashMap<>(recordTypes.size());
                            for (QName recordType : recordTypes) {
                                AspectDefinition aspectDefinition =
                                        dictionaryService.getAspect(recordType);
                                if (aspectDefinition != null) {
                                    result.put(
                                            aspectDefinition.getName().getLocalName(),
                                            aspectDefinition.getTitle(new StaticMessageLookup()));
                                }
                            }
                        }

 ...
 *
 * <ul>
 *   <li><tt>-font Helvetica -pointsize 50</tt>
 *   <li><tt>-font Helvetica -pointsize 50 -draw "circle 100,100 150,150"</tt>
 *   <li><tt>-font Helvetica -pointsize 50 -draw "gravity south fill black text 0,12
 *       'CopyRight'"</tt>
 * </ul>
 *
 * The first is the simple case which would be parsed into Strings as follows: <tt>"-font",
 * "Helvetica", "-pointsize", "50"</tt>
 *
 * <p>The second is more complex in that it includes a quoted parameter, which would be parsed as a
 * single String: <tt>"-font", "Helvetica", "-pointsize", "50", "circle 100,100 150,150"</tt> Note
 * however that the quotation characters will be stripped from the token.
 *
 * <p>The third shows an example with embedded quotation marks, which would parse to: <tt>"-font",
 * "Helvetica", "-pointsize", "50", "gravity south fill black text 0,12 'CopyRight'"</tt> In this
 * case, the embedded quotation marks (which must be different from those surrounding the parameter)
 * are preserved in the extracted token.
 *
 * <p>The class does not understand escaped quotes such as <tt>p1 p2 "a b c \"hello\" d" p4</tt>
 *
...
public class ExecParameterTokenizer {
     ...
     * or double quotes will be tokenized in the normal way i.e. by using whitespace separators and
     * the standard StringTokenizer algorithm. Any tokens which are contained within single or
     * double quotes will be returned as single String instances and will have their quote marks
     * removed.
     *
    ...
    public List<String> getAllTokens() {
        ...
        if (tokens == null) {
            ...
            if (str.indexOf('\'') == -1 && str.indexOf('"') == -1) {
                ...
                List<Pair<Integer, Integer>> quotedRegions =
                        new ArrayList<Pair<Integer, Integer>>();

                for (Pair<Integer, Integer> next = identifyNextQuotedRegion(str, 0);
                        next != null; ) {
                ...
                // We need to get substrings of quoted and unquoted blocks, whilst maintaining
                // order.
    ...
    private List<Substring> getSubstrings(
        ...
        for (Pair<Integer, Integer> nextQuotedRegionIndices : quotedRegionIndices) {
            ...
            result.add(
                    new QuotedSubstring(
                            str.substring(
                                    nextQuotedRegionIndices.getFirst(),
    ...
    private Pair<Integer, Integer> findIndexOfClosingQuote(
        ...
        int indexAfterClosingQuote =
                str.indexOf(quoteChar, indexOfStartingQuote + 1)
                        + 1; // + 1 to search after opening quote. + 1 to give result including
        // closing quote.

 ...
 *
 * <p>This class is used by the Spring framework to inject system properties into the runtime
 * environment (e.g.: alfresco.jmx.dir). The motivation for this is that certain values must be set
 * within spring must be computed in advance for
 * org.springframework.beans.factory.config.PropertyPlaceholderConfigurer to work properly.
 *
 * @author Jon Cox
 * @see #setJvmProperties(Map)
 */
public class RuntimeSystemPropertiesSetter
        implements BeanFactoryPostProcessor, ApplicationContextAware, PriorityOrdered {
    ...
    /**
     * Set the properties that will get pushed into the JVM system properties. This will be akin to
     * running the JVM with the <b>-Dprop=value</b>. Existing system JVM properties <i>will not be
     * overwritten</i>.
     *
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory)
        ...
        try {
            ...
            path =
                    this.resolver
                            .getResource("classpath:alfresco/alfresco-jmxrmi.password")
                            .getFile()
                            .getCanonicalFile();
        } catch (Exception e) {

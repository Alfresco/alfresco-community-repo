...
public class DeclareDocumentAsRecordTests extends BaseRMRestTest {
    ...
    @Autowired RecordsAPI recordsAPI;
    ...
    public void userWithPrivilegesCanDeclareDocumentAsRecord() throws Exception {
        ...
        List<UnfiledContainerChildEntry> matchingRecords =
                unfiledContainersAPI
                        ...
                        .filter(
                                e ->
                                        e.getEntry()
                                                .getId()
                                                .equals(document.getNodeRefWithoutVersion()))
        ...
        // verify the original file in collaboration site has been renamed to reflect the record
        // identifier
        List<RestNodeModel> filesAfterRename =
                getRestAPIFactory().getNodeAPI(testFolder).listChildren().getEntries().stream()
                        .filter(
                                f ->
                                        f.onModel()
                                                .getId()
        ...
        assertEquals(
                ...
                document.getName()
                        .replace(
                                ".",
                                String.format(" (%s).", record.getProperties().getIdentifier())));

        // verify the document in collaboration site is now a record, note the file is now renamed
        // hence folder + doc. name concatenation
        // this also verifies the document is still in the initial folder
        Document documentPostFiling =
                dataContent
                        .usingSite(testSite)
                        .usingUser(testUser)
        ...
        List<SecondaryType> documentSecondary =
                documentPostFiling.getSecondaryTypes().stream()
        ...
        try (InputStream recordInputStream =
                        getRestAPIFactory()
                                .getRecordsAPI()
                                .getRecordContent(record.getId())
                                .asInputStream();
                InputStream documentInputStream =
                        documentPostFiling.getContentStream().getStream()) {
            assertEquals(
                    DigestUtils.sha1(recordInputStream), DigestUtils.sha1(documentInputStream));
        }
    }
    ...
    public void recordCantBeDeclaredARecord() {
        ...
        Record record =
                getRestAPIFactory()
                        .getRecordFolderAPI()
                        .createRecord(
                                nonelectronicRecord, createCategoryFolderInFilePlan().getId());
        assertStatusCode(CREATED);
    ...
    /**
     * Given a file that has version declared as record When the file is declared as record Then the
     * action is successful
     */
    @Test(
            description =
                    "Declaring as record a file that already has its version declared as record is"
                            + " successful")
    ...
    // unfiledContainersAPI.getUnfiledContainer(UNFILED_RECORDS_CONTAINER_ALIAS).getId();
    ...
    //            {
    //                Record record =
    // getRestAPIFactory().getFilesAPI(testUser).declareAsRecord(document.getNodeRefWithoutVersion());
    //                assertStatusCode(CREATED);
    //

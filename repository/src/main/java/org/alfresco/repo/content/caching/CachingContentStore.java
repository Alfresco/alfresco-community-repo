 ...
 *
 * <p>CachingContentStore should only be used to wrap content stores that are significantly slower
 * that FileContentStore - otherwise performance may actually degrade from its use.
 *
 * <p>It is important that cacheOnInbound is set to true for exceptionally slow backing stores.
 *
 * <p>This store handles the {@link FileContentStore#SPOOF_PROTOCOL} and can be used to wrap stores
 * that do not handle the protocol out of the box e.g. the S3 connector's store.
 *
 * @author Matt Ward
 */
public class CachingContentStore
        implements ContentStore, ApplicationEventPublisherAware, BeanNameAware {
     ...
     * {@inheritDoc}
     *
     * <p>For {@link FileContentStore#SPOOF_PROTOCOL spoofed} URLs, the URL always exists.
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * <p>This store handles the {@link FileContentStore#SPOOF_PROTOCOL} so that underlying stores
     * do not need to implement anything <a
     * href="https://issues.alfresco.com/jira/browse/ACE-4516">related to spoofing</a>.
     */
    @Override
    ...
    private ContentReader cacheAndRead(String url) {
        ...
        try {
            ...
            // but there is a recurring problem - give up and return the item from the backing
            // store.
     ...
     * already in the cache (possibly due to a race condition between the read/write locks) then a
     * reader onto that content is returned.
     *
     * <p>If it is not possible to cache the content and/or get a reader onto the cached content
     * then <code>null</code> is returned and the method ensure that the URL is not stored in the
     * cache.
     *
    ...
    public ContentWriter getWriter(final ContentContext context) {
        if (cacheOnInbound) {
            ...
            final BackingStoreAwareCacheWriter cacheWriter =
                    new BackingStoreAwareCacheWriter(cache.getWriter(url), bsWriter);
            ...
            cacheWriter.addListener(
                    new ContentStreamListener() {
                        ...
                        public void contentStreamClosed() throws ContentIOException {
                            ...
                            // ensuring that the encoding attributes are set to the same as for the
                            // cache writer.
                            ...
                            // MNT-11758 fix, re-cache files for which content url has changed after
                            // write to backing store (e.g. XAM, Centera)
                            if (!quota.afterWritingCacheFile(cacheWriter.getSize())
                                    || contentUrlChanged) {
                                if (contentUrlChanged) {
                                    // MNT-11758 fix, cache file with new and correct contentUrl
                                    // after write operation to backing store completed
    ...
    public boolean delete(String contentUrl) {
        ...
        try {
            ...
            // Whether the item was in the cache or not, it must still be deleted from the backing
            // store.
            return backingStore.delete(contentUrl);
        } finally {
            writeLock.unlock();
    ...
    /**
     * Get a ReentrantReadWriteLock for a given URL. The lock is from a pool rather than per URL, so
     * some contention is expected.
     *
     * @param url String
    ...
    public DirectAccessUrl requestContentDirectUrl(
            ...
            String mimeType,
            Long validFor) {
        return backingStore.requestContentDirectUrl(

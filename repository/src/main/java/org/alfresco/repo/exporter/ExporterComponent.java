...
public class ExporterComponent implements ExporterService {
    ...
    /**
     * @param exportSecondaryNodes whether children that do dot have a primary association with
     *     their parent are exported as nodes If false, these nodes will be exported as secondary
     *     links.
     */
     ...
     */
    public void exportView(
            ExportPackageHandler exportHandler,
    ...
    private class DefaultCrawler implements ExporterCrawler {
        ...
        public void export(ExporterCrawlerParameters parameters, Exporter exporter) {
            ...
            while (context.canRetrieve()) {
                ...
                if (parameters.isCrawlSelf()) {
                    ...
                    boolean rootNode =
                            nodeService.getRootNode(nodeRef.getStoreRef()).equals(nodeRef);
        ...
        private void walkNode(
            ...
            for (QName property : properties.keySet()) {
                ...
                if (isExcludedURI(
                        parameters.getExcludeNamespaceURIs(), property.getNamespaceURI())) {
                    continue;
                }
                ...
                if (value instanceof Collection) {
                    ...
                    for (Object valueInCollection : (Collection) value) {
                        walkProperty(
                                nodeRef, property, valueInCollection, index, parameters, exporter);
                        index++;
                    }
            ...
            if (parameters.isCrawlChildNodes()) {
                ...
                Map<QName, List<ChildAssociationRef>> assocTypes =
                        new HashMap<QName, List<ChildAssociationRef>>();
                List<ChildAssociationRef> childAssocs = nodeService.getChildAssocs(nodeRef);
                for (ChildAssociationRef childAssoc : childAssocs) {
                    QName childAssocType = childAssoc.getTypeQName();
                    if (isExcludedURI(
                            parameters.getExcludeNamespaceURIs(),
                            childAssocType.getNamespaceURI())) {
                        continue;
                    }
                    if (isExcludedChildAssoc(parameters.getExcludeChildAssocs(), childAssocType)) {
                        continue;
                    }
                    if (isExcludedAspectAssociation(
                            parameters.getExcludeAspects(), childAssocType)) {
                        continue;
                    }
                ...
                if (assocTypes.size() > 0) {
                    ...
                    for (Map.Entry<QName, List<ChildAssociationRef>> assocType :
                            assocTypes.entrySet()) {
            ...
            if (valueDataType == null || !valueDataType.equals(DataTypeDefinition.CONTENT)) {
                ...
                try {
                    ...
                    exporter.warning(
                            "Value of property "
                                    + property
                                    + " could not be converted to xml string");
                    exporter.value(
                            nodeRef, property, (value == null ? null : value.toString()), index);
                }
            } else {
                ...
                if (!parameters.isCrawlContent() || reader == null || reader.exists() == false) {
                    ...
                    exporter.warning(
                            "Skipped content for property " + property + " on node " + nodeRef);
                } else {
                    InputStream inputStream = reader.getContentInputStream();
                    try {
                        exporter.content(
                                nodeRef, property, inputStream, reader.getContentData(), index);
                    } finally {
                        try {
                            inputStream.close();
                        } catch (IOException e) {
                            throw new ExporterException(
                                    "Failed to export node content for node " + nodeRef, e);
                        }
                    }
                }
            }
        ...
        private void walkNodeAssociations(
                NodeRef nodeRef, ExporterCrawlerParameters parameters, Exporter exporter) {
            // sort associations into assoc type buckets filtering out unneccessary associations
            Map<QName, List<AssociationRef>> assocTypes =
                    new HashMap<QName, List<AssociationRef>>();
            List<AssociationRef> assocs =
                    nodeService.getTargetAssocs(nodeRef, RegexQNamePattern.MATCH_ALL);
            for (AssociationRef assoc : assocs) {
                QName assocType = assoc.getTypeQName();
                if (isExcludedURI(
                        parameters.getExcludeNamespaceURIs(), assocType.getNamespaceURI())) {
                    continue;
                }
        ...
        private boolean isExcludedAspectAssociation(
                QName[] excludeAspects, QName associationQName) {
                ...
                for (NodeRef exportRoot : context.getExportList()) {
                    if (nodeRef.equals(exportRoot) && parameters.isCrawlSelf() == true) {
                        ...
                        for (int i = nodePath.size() - 1; i >= 0; i--) {
                            Path.ChildAssocElement pathElement =
                                    (Path.ChildAssocElement) nodePath.get(i);
    ...
    private class ExporterContextImpl implements ExporterContext {
        ...
        private Map<Integer, Set<NodeRef>> nodesWithSecondaryLinks =
                new HashMap<Integer, Set<NodeRef>>();
        private Map<Integer, Set<NodeRef>> nodesWithAssociations =
                new HashMap<Integer, Set<NodeRef>>();
        ...
        public ExporterContextImpl(ExporterCrawlerParameters parameters) {
            ...
            exportList =
                    (parameters.getExportFrom() == null)
                            ? null
                            : parameters.getExportFrom().getNodeRefs();
            if (exportList == null) {
                // multi-node export
                ...
                if (nodeRefs.size() == 0) {
                    throw new ImporterException(
                            "Path "
                                    ...
                                    + " within node "
                                    + nodeRef
                                    + " does not exist - the path must resolve to a valid"
                                    + " location");
                }
                if (nodeRefs.size() > 1) {
                    throw new ImporterException(

...
public class SiteExportGet extends AbstractWebScript {
  ...
  public void execute(WebScriptRequest req, WebScriptResponse res)
    ...
    CloseIgnoringOutputStream outputForNesting = new CloseIgnoringOutputStream(
      mainZip
    );
    ...
    for (String contextName : authenticationContextManager.getInstanceIds()) {
      ApplicationContext ctx = authenticationContextManager.getApplicationContext(
        contextName
      );
      try {
        authenticationDao =
          (RepositoryAuthenticationDao) ctx.getBean(
    ...
    if (authenticationDao == null) {
      mainZip.putNextEntry(
        new ZipEntry("Users_Skipped_As_Wrong_Authentication.txt")
      );
      String text =
  ...
  protected void doSiteACPExport(
    SiteInfo site,
    CloseIgnoringOutputStream writeTo
  ) throws IOException {
  ...
  protected void doPeopleACPExport(
    final List<NodeRef> peopleNodes,
    SiteInfo site,
    CloseIgnoringOutputStream writeTo
  ) throws IOException {
    if (!peopleNodes.isEmpty()) {
  ...
  protected void doUserACPExport(
    ...
    ExporterCrawlerParameters parameters = new ExporterCrawlerParameters();
    parameters.setExportFrom(
      new Location(userNodes.toArray(new NodeRef[userNodes.size()]))
    );
    parameters.setCrawlChildNodes(true);
    parameters.setCrawlSelf(true);
    parameters.setCrawlContent(true);

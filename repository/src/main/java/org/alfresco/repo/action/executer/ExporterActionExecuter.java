...
public class ExporterActionExecuter extends ActionExecuterAbstractBase {
  ...
  public void executeImpl(Action ruleAction, NodeRef actionedUponNodeRef) {
    File zipFile = null;
    try {
      String packageName = (String) ruleAction.getParameterValue(
        PARAM_PACKAGE_NAME
      );
      ...
      ACPExportPackageHandler zipHandler = new ACPExportPackageHandler(
      ...
      Boolean withKids = (Boolean) ruleAction.getParameterValue(
      ...
      Boolean andMe = (Boolean) ruleAction.getParameterValue(
        PARAM_INCLUDE_SELF
      );
      if (andMe != null) {
      ...
      ContentWriter writer =
        this.contentService.getWriter(zip, ContentModel.PROP_CONTENT, true);
   ...
   */
  private NodeRef createExportZip(
    Action ruleAction,
    NodeRef actionedUponNodeRef
  ) {
    // create a node in the repository to represent the export package
    NodeRef exportDest = (NodeRef) ruleAction.getParameterValue(
      PARAM_DESTINATION_FOLDER
    );
    String packageName = (String) ruleAction.getParameterValue(
      PARAM_PACKAGE_NAME
    ...
    if (!packageName.endsWith("." + ACPExportPackageHandler.ACP_EXTENSION)) {
      packageName +=
        (packageName.charAt(packageName.length() - 1) == '.')
          ? ACPExportPackageHandler.ACP_EXTENSION
          : "." + ACPExportPackageHandler.ACP_EXTENSION;
    }
    ...
    if (rootNode.equals(actionedUponNodeRef)) {
      ...
      String spaceName = (String) this.nodeService.getProperty(
          actionedUponNodeRef,

...
public class RepositoryAuthenticationDao
    ...
    @Override
    public UserDetails loadUserByUsername(String incomingUserName)
            throws UsernameNotFoundException, DataAccessException {
        CacheEntry userEntry = getUserEntryOrNull(incomingUserName);
        if (userEntry == null) {
            throw new UsernameNotFoundException(
                    "Could not find user by userName: "
                            + AuthenticationUtil.maskUsername(incomingUserName));
        }
        UserDetails userDetails = userEntry.userDetails;
        if (userEntry.credentialExpiryDate == null
                || userEntry.credentialExpiryDate.getTime() >= System.currentTimeMillis()) {
            return userDetails;
        }
        ...
        if (userDetails instanceof RepositoryAuthenticatedUser) {
            ...
            return new RepositoryAuthenticatedUser(
                    ...
                    repoUser.getHashIndicator(),
                    repoUser.getSalt());
        }

        throw new AlfrescoRuntimeException(
                "Unable to retrieve a compatible UserDetails object (requires"
    ...
    private CacheEntry getUserEntryOrNullImpl(final String caseSensitiveSearchUserName) {
        ...
        class SearchUserNameCallback implements RetryingTransactionCallback<CacheEntry> {
            ...
            public CacheEntry execute() throws Throwable {
                ...
                if (cacheEntry != null && !nodeService.exists(cacheEntry.nodeRef)) {
                    logger.warn(
                            "Detected state cache entry for '"
                ...
                List<ChildAssociationRef> results =
                        nodeService.getChildAssocs(
                                ...
                                QName.createQName(
                                        ContentModel.USER_MODEL_URI, caseSensitiveSearchUserName));
                if (!results.isEmpty()) {
                    ...
                    String userName =
                            DefaultTypeConverter.INSTANCE.convert(
                                    String.class, properties.get(ContentModel.PROP_USER_USERNAME));
                    ...
                    Date credentialsExpiryDate =
                            getCredentialsExpiryDate(userName, properties, isAdminAuthority);
                    boolean credentialsHaveNotExpired =
                            (credentialsExpiryDate == null
                                    || credentialsExpiryDate.getTime()
                                            >= System.currentTimeMillis());

                    UserDetails ud =
                            new RepositoryAuthenticatedUser(
                                    userName,
        ...
        return transactionService
                ...
                .doInTransaction(new SearchUserNameCallback(), true);
    }
     ...
     */
    public static Pair<List<String>, String> determinePasswordHash(
            Map<QName, Serializable> properties) {
        @SuppressWarnings("unchecked")
        List<String> hashIndicator =
                (List<String>) properties.get(ContentModel.PROP_HASH_INDICATOR);
        if (hashIndicator != null && hashIndicator.size() > 0) {
            // We have hashed the value so get it.
            return new Pair<>(
                    hashIndicator,
                    DefaultTypeConverter.INSTANCE.convert(
                            String.class, properties.get(ContentModel.PROP_PASSWORD_HASH)));
        } else {
            String passHash =
                    DefaultTypeConverter.INSTANCE.convert(
                            String.class, properties.get(ContentModel.PROP_PASSWORD_SHA256));
            if (passHash != null) {
                ...
                passHash =
                        DefaultTypeConverter.INSTANCE.convert(
                                String.class, properties.get(ContentModel.PROP_PASSWORD));
                if (passHash != null) {
                    // Use MD4
        ...
        throw new AlfrescoRuntimeException(
                "Unable to find a password for user '"
                        + AuthenticationUtil.maskUsername(
                                (String) properties.get(ContentModel.PROP_USER_USERNAME))
                        + "', please check your repository authentication settings.");
    }

    @Override
    public void createUser(String caseSensitiveUserName, char[] rawPassword)
            throws AuthenticationException {
        createUser(caseSensitiveUserName, null, rawPassword);
    }

    @Override
    public void createUser(String caseSensitiveUserName, String hashedPassword, char[] rawPassword)
            throws AuthenticationException {
        ...
        if (userRef != null) {
            throw new AuthenticationException(
                    "User already exists: "
                            + AuthenticationUtil.maskUsername(caseSensitiveUserName));
        }
        ...
        if (hashedPassword == null) {
            if (logger.isTraceEnabled()) {
                logger.trace(
                        "Hashing raw password to "
                                + compositePasswordEncoder.getPreferredEncoding()
                                + " for "
                                + AuthenticationUtil.maskUsername(caseSensitiveUserName));
            }
            hashedPassword =
                    compositePasswordEncoder.encodePreferred(new String(rawPassword), salt);
        } else {
            if (logger.isTraceEnabled()) {
                logger.trace(
                        "Using hashed password for  "
                                + AuthenticationUtil.maskUsername(caseSensitiveUserName));
            }
        }
        ...
        properties.put(
                ...
                (Serializable) Arrays.asList(compositePasswordEncoder.getPreferredEncoding()));
    ...
    private NodeRef getUserFolderLocation(String caseSensitiveUserName) {
        ...
        if (userNodeRef == null) {
            ...
            if (TenantUtil.isCurrentDomainDefault()) {
                userStoreRef =
                        tenantService.getName(
                                ...
                                new StoreRef(
                                        STOREREF_USERS.getProtocol(),
                                        STOREREF_USERS.getIdentifier()));
            } else {
                userStoreRef =
                        new StoreRef(STOREREF_USERS.getProtocol(), STOREREF_USERS.getIdentifier());
            }
            ...
            List<ChildAssociationRef> results =
                    nodeService.getChildAssocs(
                            rootNode, RegexQNamePattern.MATCH_ALL, qnameAssocSystem);
            NodeRef sysNodeRef = null;
            if (results.size() == 0) {
                throw new AlfrescoRuntimeException(
                        "Required authority system folder path not found: " + qnameAssocSystem);
            } else {
                sysNodeRef = results.get(0).getChildRef();
            }
            results =
                    nodeService.getChildAssocs(
                            sysNodeRef, RegexQNamePattern.MATCH_ALL, qnameAssocUsers);
            if (results.size() == 0) {
                throw new AlfrescoRuntimeException(
                        "Required user folder path not found: " + qnameAssocUsers);
            } else {
                userNodeRef = tenantService.getName(results.get(0).getChildRef());
            }
            singletonCache.put(
                    (tenantService.getUserDomain(caseSensitiveUserName) + KEY_USERFOLDER_NODEREF),
                    userNodeRef);
        }
        return userNodeRef;
    }
    ...
    public void updateUser(String userName, char[] rawPassword) throws AuthenticationException {
        ...
        if (userRef == null) {
            throw new AuthenticationException(
                    "User name does not exist: " + AuthenticationUtil.maskUsername(userName));
        }
        Map<QName, Serializable> properties = nodeService.getProperties(userRef);
        String salt = GUID.generate();
        ...
        properties.put(
                ...
                compositePasswordEncoder.encodePreferred(new String(rawPassword), salt));
        properties.put(
                ContentModel.PROP_HASH_INDICATOR, compositePasswordEncoder.getPreferredEncoding());
    ...
    public Date getAccountExpiryDate(String userName) {
        ...
        if (DefaultTypeConverter.INSTANCE.booleanValue(
                nodeService.getProperty(userNode, ContentModel.PROP_ACCOUNT_EXPIRES))) {
            return DefaultTypeConverter.INSTANCE.convert(
                    Date.class,
                    nodeService.getProperty(userNode, ContentModel.PROP_ACCOUNT_EXPIRY_DATE));
        } else {
            return null;
        }
    }
     ...
     */
    protected boolean getHasExpired(
            String userName, Map<QName, Serializable> properties, Boolean isAdminAuthority) {
        ...
        if (DefaultTypeConverter.INSTANCE.booleanValue(
                properties.get(ContentModel.PROP_ACCOUNT_EXPIRES))) {
            Date date =
                    DefaultTypeConverter.INSTANCE.convert(
                            Date.class, properties.get(ContentModel.PROP_ACCOUNT_EXPIRY_DATE));
            if (date == null) {
                return false;
            } else {
    ...
    public Date getCredentialsExpiryDate(String userName) {
        ...
        if (DefaultTypeConverter.INSTANCE.booleanValue(
                nodeService.getProperty(userNode, ContentModel.PROP_CREDENTIALS_EXPIRE))) {
            return DefaultTypeConverter.INSTANCE.convert(
                    Date.class,
                    nodeService.getProperty(userNode, ContentModel.PROP_CREDENTIALS_EXPIRY_DATE));
        } else {
            return null;
        }
    }
     ...
     */
    protected boolean getCredentialsHaveExpired(
            String userName, Map<QName, Serializable> properties, Boolean isAdminAuthority) {
        Date credentialsExpiryDate =
                getCredentialsExpiryDate(userName, properties, isAdminAuthority);
        boolean credentialsHaveNotExpired =
                (credentialsExpiryDate == null
                        || credentialsExpiryDate.getTime() >= System.currentTimeMillis());
     ...
     */
    private Date getCredentialsExpiryDate(
            String userName, Map<QName, Serializable> properties, Boolean isAdminAuthority) {
        if (isAdminAuthority == null) {
            isAdminAuthority = authorityService.isAdminAuthority(userName);
        }
        ...
        if (DefaultTypeConverter.INSTANCE.booleanValue(
                properties.get(ContentModel.PROP_CREDENTIALS_EXPIRE))) {
            return DefaultTypeConverter.INSTANCE.convert(
                    Date.class, properties.get(ContentModel.PROP_CREDENTIALS_EXPIRY_DATE));
        }
        return null;
    }
    ...
    public void setAccountExpires(String userName, boolean expires) {
        ...
        nodeService.setProperty(
                userNode, ContentModel.PROP_ACCOUNT_EXPIRES, Boolean.valueOf(expires));
    }
    ...
    public void setCredentialsExpire(String userName, boolean expires) {
        ...
        validateUserNode(userName, userNode);
        nodeService.setProperty(
                userNode, ContentModel.PROP_CREDENTIALS_EXPIRE, Boolean.valueOf(expires));
    }
    ...
    public void setLocked(String userName, boolean locked) {
        ...
        validateUserNode(userName, userNode);
        nodeService.setProperty(
                userNode, ContentModel.PROP_ACCOUNT_LOCKED, Boolean.valueOf(locked));
    }

    private void validateUserNode(String userName, NodeRef userNode) {
        if (userNode == null) {
            throw new AuthenticationException(
                    "User not found: " + AuthenticationUtil.maskUsername(userName));
        }
    }
    ...
    public String getMD4HashedPassword(String userName) {
        ...
        if (userNode == null) {
            ...
            if (hashIndicator != null
                    && hashIndicator.size() == 1
                    && CompositePasswordEncoder.MD4.equals(hashIndicator)) {
                // We have hashed the value so get it.
                return DefaultTypeConverter.INSTANCE.convert(
                        String.class, properties.get(ContentModel.PROP_PASSWORD_HASH));
            } else {
    ...
    public void onUpdateProperties(
        ...
        String uidBefore =
                DefaultTypeConverter.INSTANCE.convert(
                        String.class, before.get(ContentModel.PROP_USERNAME));
        String uidAfter =
                DefaultTypeConverter.INSTANCE.convert(
                        String.class, after.get(ContentModel.PROP_USERNAME));
        if (uidBefore != null && !EqualsHelper.nullSafeEquals(uidBefore, uidAfter)) {
            ...
            if (userNode != null) {
                ...
                nodeService.moveNode(
                        userNode,
                        nodeService.getPrimaryParent(userNode).getParentRef(),
                        ContentModel.ASSOC_CHILDREN,
                        QName.createQName(ContentModel.USER_MODEL_URI, uidAfter));
    ...
    public void onUpdateUserProperties(
            NodeRef nodeRef, Map<QName, Serializable> before, Map<QName, Serializable> after) {
        String uidBefore =
                DefaultTypeConverter.INSTANCE.convert(
                        String.class, before.get(ContentModel.PROP_USER_USERNAME));
        if (uidBefore != null) {
            removeAuthenticationFromCache(uidBefore);
        }
    }
    ...
    public void beforeDeleteNode(NodeRef nodeRef) {
        String userName =
                (String) nodeService.getProperty(nodeRef, ContentModel.PROP_USER_USERNAME);
        if (userName != null) {
    ...
    private void removeAuthenticationFromCache(String key) {
        ...
        if (authenticationCache instanceof TransactionalCache) {
            TransactionalCache<String, CacheEntry> authenticationCacheTxn =
                    (TransactionalCache<String, CacheEntry>) authenticationCache;

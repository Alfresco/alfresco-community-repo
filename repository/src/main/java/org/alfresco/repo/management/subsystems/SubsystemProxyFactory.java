...
/**
 * A factory bean, normally used in conjunction with {@link ChildApplicationContextFactory} allowing
 * selected interfaces in a child application context to be proxied by a bean in the parent
 * application context. This allows 'hot-swapping' and reconfiguration of entire subsystems.
 */
public class SubsystemProxyFactory extends ProxyFactoryBean implements ApplicationContextAware {
     ...
     *
     * @param sourceApplicationContextFactoryName the name of the sourceApplicationContextFactory to
     *     set
     */
     ...
     *
     * @param sourceApplicationContextFactory the sourceApplicationContextFactory to set
     */
    public void setSourceApplicationContextFactory(
            ApplicationContextFactory sourceApplicationContextFactory) {
        ...
        if (sourceApplicationContextFactory != null) {
            ...
            try {
                return applicationContext.getBean(
                        sourceApplicationContextFactoryName, ApplicationContextFactory.class);
    ...
    /**
     * Sets an optional bean name to target all calls to in the source application context. If not
     * set, an appropriate bean is looked up based on method class.
     *
     * @param sourceBeanName the sourceBeanName to set
     */
    ...
    /**
     * Sets an optional default bean to be used if the target bean is not found. Generally used when
     * a subsystem does not exist.
     *
    ...
    // Bring our cached copies of the source beans in line with the application context factory,
    // using a RW lock to
    // ensure consistency
    protected Object locateBean(MethodInvocation mi) {
        ...
        try {
            ApplicationContextFactory sourceApplicationContextFactory =
                    getSourceApplicationContextFactory();
            if (sourceApplicationContextFactory != null) {
                ApplicationContext newContext =
                        sourceApplicationContextFactory.getApplicationContext();
                            ...
                            if (beans.size() == 0 && defaultBean != null) {
                                ...
                                if (beans.size() != 1) {
                                    throw new RuntimeException(
                                            "Don't know where to route call to method " + method);
                                }

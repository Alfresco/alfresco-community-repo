...
public class ActionParameterField implements Field {
  ...
  public ActionParameterField(
    ...
    final List<FieldConstraint> fieldConstraints = processActionConstraints(
      parameterDef,
      actionService
    );

    if (
      DataTypeDefinition.NODE_REF.equals(type) && fieldConstraints.isEmpty()
    ) {
      ...
      this.fieldDef =
        new AssociationFieldDefinition(
          this.name,
          "cm:cmobject",
          AssociationFieldDefinition.Direction.TARGET
        );
      ...
      if (DataTypeDefinition.NODE_REF.equals(type)) {
        ...
        this.fieldDef =
          new PropertyFieldDefinition(this.name, type.getLocalName());
      }
  ...
  private List<FieldConstraint> processActionConstraints(
    ...
    if (paramConstraintName != null) {
      ...
      if (paramConstraint == null) {
        ...
        List<String> pipeSeparatedAllowedValues = new ArrayList<String>(
          allowableValuesMap.size()
        );
        for (Map.Entry<String, String> entry : allowableValuesMap.entrySet()) {
          pipeSeparatedAllowedValues.add(
        ...
        fieldConstraints =
          new ArrayList<FieldConstraint>(allowableValuesMap.size());

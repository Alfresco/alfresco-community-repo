...
public abstract class BaseKerberosAuthenticationFilter
   ...
   */
  public void setStripKerberosUsernameSuffix(
    boolean stripKerberosUsernameSuffix
  ) {
  ...
  public boolean authenticateRequest(
    ...
    if (authHdr != null) {
      ...
      if (authHdr.startsWith("Negotiate")) reqAuth = true; else if (
        authHdr.startsWith("NTLM")
      ) {
        if (getLogger().isTraceEnabled()) {
    ...
    if (authHdr == null) {
      ...
      final byte[] spnegoByts = Base64.decodeBase64(
        authHdr.substring(10).getBytes()
      );
      ...
      if (tokType == SPNEGO.NegTokenInit) {
        ...
        try {
          ...
          String oidStr = null;
          if (negToken.numberOfOids() > 0) oidStr =
            negToken.getOidAt(0).toString();
          ...
          if (
            ...
            try {
              NegTokenTarg negTokenTarg = doKerberosLogon(
                negToken,
                req,
                resp,
                httpSess
              ...
              WebCredentials webCredentials = user == null
                ? null
                : new TicketCredentials(user.getTicket());
  ...
  protected boolean checkLoginPage(
    HttpServletRequest req,
    HttpServletResponse resp
  ) {
  ...
  public void handle(Callback[] callbacks)
    ...
    if (getLogger().isTraceEnabled()) {
      getLogger()
        .trace(
          "Processing the JAAS callback list of " + callbacks.length + " items."
        );
    }
  ...
  private final NegTokenTarg doKerberosLogon(
    ...
    try {
      ...
      SessionSetupPrivilegedAction sessSetupAction = new SessionSetupPrivilegedAction(
        m_accountName,
        negToken.getMechtoken()
      );
      Object result = Subject.doAs(
        m_loginContext.getSubject(),
        sessSetupAction
      );

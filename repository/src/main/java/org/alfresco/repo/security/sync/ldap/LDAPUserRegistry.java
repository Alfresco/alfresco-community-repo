...
public class LDAPUserRegistry
  implements
    UserRegistry, LDAPNameResolver, InitializingBean, ActivateableBean {
  ...
  private static final Pattern PATTERN_RANGE_END = Pattern.compile(
    ";range=[0-9]+-\\*"
  );

  public static final String NAMING_TIMEOUT_EXCEPTION_MESSAGE =
   ...
   *            the modification timestamp attribute name
   */
  public void setModifyTimestampAttributeName(
   ...
   *            the person attribute defaults
   */
  public void setPersonAttributeDefaults(
    Map<String, String> personAttributeDefaults
  ) {
   ...
   *            the person attribute mapping
   */
  public void setPersonAttributeMapping(
   ...
   *            the group attribute defaults
   */
  public void setGroupAttributeDefaults(
    Map<String, String> groupAttributeDefaults
  ) {
   ...
   *            the group attribute mapping
   */
  public void setGroupAttributeMapping(
    ...
    processQuery(
      new AbstractSearchCallback() {
        protected void doProcess(SearchResult result)
          ...
          Attribute nameAttribute = result
            .getAttributes()
          ...
          if (nameAttribute == null) {
            if (LDAPUserRegistry.this.errorOnMissingUID) {
              ...
              throw new AlfrescoRuntimeException(
                "synchronization.err.ldap.get.user.id.missing",
                params
              );
            } else {
  ...
  public Collection<NodeDescription> getGroups(Date modifiedSince) {
    ...
    try {
      groupDistinguishedNamePrefix =
        fixedLdapName(this.groupSearchBase.toLowerCase());
    } catch (InvalidNameException e) {
      Object[] params = {
        this.groupSearchBase.toLowerCase(),
        e.getLocalizedMessage(),
      };
      throw new AlfrescoRuntimeException(
        "synchronization.err.ldap.search.base.invalid",
        params,
        e
      );
    }
    ...
    try {
      userDistinguishedNamePrefix =
        fixedLdapName(this.userSearchBase.toLowerCase());
    } catch (InvalidNameException e) {
      Object[] params = {
        this.userSearchBase.toLowerCase(),
        e.getLocalizedMessage(),
      };
      throw new AlfrescoRuntimeException(
        "synchronization.err.ldap.search.base.invalid",
    ...
    processQuery(
      new AbstractSearchCallback() {
        ...
        private DirContext ctx =
          LDAPUserRegistry.this.ldapInitialContextFactory.getDefaultIntialDirContext();

        protected void doProcess(SearchResult result)
          throws NamingException, ParseException {
          Attributes attributes = result.getAttributes();
          Attribute gidAttribute = attributes.get(
            LDAPUserRegistry.this.groupIdAttributeName
          );
          ...
          Attribute memAttribute = getRangeRestrictedAttribute(
            attributes,
            ...
            for (int i = 0; i < memAttribute.size(); i++) {
              ...
              if (attribute != null && attribute.length() > 0) {
                try {
                  ...
                  // in the catch block
                  LdapName distinguishedNameForComparison = fixedLdapName(
                  ...
                  if (disjoint) {
                    ...
                    if (
                      ...
                      if (LDAPUserRegistry.logger.isDebugEnabled()) {
                        LDAPUserRegistry.logger.debug(
                          "User DN recognized: " + nameAttribute.get()
                        );
                      }
                  ...
                  if (
                    ...
                    try {
                      ...
                      Attribute objectClass = childAttributes.get(
                        "objectclass"
                      );
                      if (
                        hasAttributeValue(
                        ...
                        if (LDAPUserRegistry.logger.isDebugEnabled()) {
                          LDAPUserRegistry.logger.debug(
                            "User DN recognized by directory lookup: " +
                            nameAttribute.get()
                          );
                        }
                        childAssocs.add((String) nameAttribute.get());
                        ...
                        if (nameAttribute == null) {
                          if (LDAPUserRegistry.this.errorOnMissingGID) {
                            ...
                            LDAPUserRegistry.logger.warn(
                              "Missing GID on " + childAttributes
                            );
                            continue;
                          }
                        }
                        if (LDAPUserRegistry.logger.isDebugEnabled()) {
                          LDAPUserRegistry.logger.debug(
                            "Group DN recognized by directory lookup: " +
                            "GROUP_" +
                            nameAttribute.get()
                          );
                        }
                      ...
                      ServiceUnavailableException | CommunicationException e
                    ) {
                      ...
                      if (
                        ...
                        e
                          .getMessage()
                          .startsWith(NAMING_TIMEOUT_EXCEPTION_MESSAGE)
                      ) {
                        Object[] params = { e.getLocalizedMessage() };
                        throw new AlfrescoRuntimeException(
                          "synchronization.err.ldap.search",
                  ...
                  if (LDAPUserRegistry.logger.isDebugEnabled()) {
                    LDAPUserRegistry.logger.debug(
                      "Member DN recognized as posixGroup: " + attribute
                    );
                  }
   ...
   */
  public String resolveDistinguishedName(
    String userId,
    ...
    userSearchCtls.setReturningAttributes(
      new String[] { this.userIdAttributeName }
    );
    ...
    try {
      ctx =
        this.ldapInitialContextFactory.getDefaultIntialDirContext(diagnostic);
      ...
      diagnostic.addStep(
        AuthenticationDiagnostic.STEP_KEY_LDAP_LOOKUP_USER,
        false,
        args
      );

      throw new AuthenticationException(
        "authentication.err.connection.ldap.user.notfound",
        args,
        diagnostic
      );
    } catch (NamingException e) {
      ...
      diagnostic.addStep(
        AuthenticationDiagnostic.STEP_KEY_LDAP_SEARCH,
        false,
        args1
      );
      ...
      throw new AuthenticationException(
        "authentication.err.connection.ldap.search",
        diagnostic,
        args,
        e
      );
  ...
  private NodeDescription mapToNode(
    ...
    // Parse the timestamp
    Attribute modifyTimestamp = ldapAttributes.get(
  ...
  private void processQuery(
    ...
    if (LDAPUserRegistry.logger.isDebugEnabled()) {
      ...
      LDAPUserRegistry.logger.debug(
        "    DerefLink: " + searchControls.getDerefLinkFlag()
      );
      LDAPUserRegistry.logger.debug(
    ...
    try {
      ctx =
        this.ldapInitialContextFactory.getDefaultIntialDirContext(
            this.queryBatchSize
          );
      do {
        ...
        this.ldapInitialContextFactory.hasNextPage(ctx, this.queryBatchSize)
      );
    } catch (NamingException e) {
      Object[] params = { e.getLocalizedMessage() };
      throw new AlfrescoRuntimeException(
   ...
   */
  private boolean hasAttributeValue(Attribute attribute, String value)
    throws NamingException {
    if (attribute != null) {
   ...
   */
  private Attribute getRangeRestrictedAttribute(
        ...
        try {
          ...
          try {
            ...
            if (LDAPUserRegistry.this.queryBatchSize > 0) {
              ((LdapContext) this.ctx).setRequestControls(
                  new Control[] {
      ...
      private NodeDescription fetchNext() throws NamingException {
        ...
        do {
          ...
          while (!readyForNextPage && this.searchResults.hasMore()) {
            ...
            Attributes attributes = result.getAttributes();
            Attribute uidAttribute = attributes.get(
              LDAPUserRegistry.this.userIdAttributeName
            );
            if (uidAttribute == null) {
              if (LDAPUserRegistry.this.errorOnMissingUID) {
            ...
            // Apply the mapped properties to the node description
            NodeDescription nodeDescription = mapToNode(
  ...
  protected static interface SearchCallback {
     ...
     *             on parse exceptions
     */
    public void process(SearchResult result)
      throws NamingException, ParseException;
   ...
   */
  protected abstract static class AbstractSearchCallback

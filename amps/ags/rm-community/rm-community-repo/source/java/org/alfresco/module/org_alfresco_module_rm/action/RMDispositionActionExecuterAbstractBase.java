...
public abstract class RMDispositionActionExecuterAbstractBase
  ...
  /** I18N */
  private static final String MSG_RECORD_NOT_DECLARED =
    "rm.action.record-not-declared";
  private static final String MSG_EXPECTED_RECORD_LEVEL =
    "rm.action.expected-record-level";
  private static final String MSG_NOT_ALL_RECORDS_DECLARED =
    "rm.action.not-all-records-declared";
  private static final String MSG_NOT_ELIGIBLE = "rm.action.not-eligible";
  private static final String MSG_NO_DISPOITION_INSTRUCTIONS =
    "rm.action.no-disposition-instructions";
  private static final String MSG_NO_DIS_LIFECYCLE_SET =
    "rm.action.no-disposition-lisfecycle-set";
  private static final String MSG_NEXT_DISP_NOT_SET =
    "rm.action.next-disp-not-set";
  private static final String MSG_NOT_NEXT_DISP = "rm.action.not-next-disp";
  private static final String MSG_NOT_RECORD_FOLDER =
    "rm.action.not-record-folder";
  ...
  protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
    NodeRef nextDispositionActionNodeRef = getNextDispostionAction(
      actionedUponNodeRef
    );

    // determine whether we should be raising errors during state checking or not
    boolean checkError = true;
    Boolean checkErrorValue = (Boolean) action.getParameterValue(
    ...
    DispositionSchedule di = checkDispositionActionExecutionValidity(
      actionedUponNodeRef,
    ...
    if (di != null) {
      ...
      if (
        !checkEligibility(actionedUponNodeRef) ||
        getDispositionService()
          .isNextDispositionActionEligible(actionedUponNodeRef)
      ) {
        if (di.isRecordLevelDisposition()) {
          ...
          if (getRecordService().isRecord(actionedUponNodeRef)) {
            ...
            if (getRecordService().isDeclared(actionedUponNodeRef)) {
              ...
              if (
                getNodeService().exists(nextDispositionActionNodeRef) &&
          ...
          if (getRecordFolderService().isRecordFolder(actionedUponNodeRef)) {
            if (
              getRecordFolderService()
                .isRecordFolderDeclared(actionedUponNodeRef)
            ) {
              // Indicate that the disposition action is underway
              getNodeService()
                .setProperty(
   ...
   * @param record
   */
  protected abstract void executeRecordLevelDisposition(
    Action action,
    NodeRef record
  );
   ...
   * @param recordFolder
   */
  protected abstract void executeRecordFolderLevelDisposition(
    Action action,
   ...
   * @return
   */
  protected DispositionSchedule checkDispositionActionExecutionValidity(
    NodeRef nodeRef,
    NodeRef nextDispositionActionNodeRef,
    boolean throwError
  ) {
    // Check the node has associated disposition instructions
    DispositionSchedule di = getDispositionService()
      .getDispositionSchedule(nodeRef);
    if (di == null) {
      if (throwError) {
        throw new AlfrescoRuntimeException(
          I18NUtil.getMessage(
            MSG_NO_DISPOITION_INSTRUCTIONS,
            getName(),
            nodeRef.toString()
          )
        );
    ...
    if (checkNextDispositionAction(nodeRef)) {
      ...
      String actionName = (String) getNodeService()
        .getProperty(nextDispositionAction, PROP_DISPOSITION_ACTION);

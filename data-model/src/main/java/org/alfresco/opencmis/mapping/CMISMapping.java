...
/**
 * CMIS <-> Alfresco mappings. It additionally excludes a list of QNames based on a user defined
 * list
 *
 * @author andyh
 */
public class CMISMapping implements InitializingBean {
    ...
    public static QName SECONDARY_TYPES_QNAME =
            QName.createQName(CMIS_MODEL_URI, "secondary"); // cmis 1.1
    ...
    public boolean isValidCmisSecondaryType(QName typeQName) {
        ...
        // Anything derived from the aspects here would at some point have to linked up with an
        // invalid parent so exclude these aspects
        ...
        if (dictionaryService.isSubClass(aspectDef.getName(), ContentModel.ASPECT_VERSIONABLE)
                ...
                || dictionaryService.isSubClass(
                        aspectDef.getName(), ContentModel.ASPECT_REFERENCEABLE)) {
            return false;
        }
        return true;
    }
    ...
    public boolean isValidCmisPolicy(QName typeQName) {
        ...
        if (cmisVersion.equals(CmisVersion.CMIS_1_0)) {
            ...
            if (dictionaryService.isSubClass(aspectDef.getName(), ContentModel.ASPECT_VERSIONABLE)
                    || dictionaryService.isSubClass(
                            aspectDef.getName(), ContentModel.ASPECT_AUDITABLE)
                    || dictionaryService.isSubClass(
                            aspectDef.getName(), ContentModel.ASPECT_REFERENCEABLE)) {
                return false;
            }
    ...
    /**
     * Is an association valid in CMIS? It must be a non-child relationship and the source and
     * target must both be valid CMIS types.
     *
    ...
    public boolean isValidCmisRelationship(QName associationQName) {
        ...
        AssociationDefinition associationDefinition =
                dictionaryService.getAssociation(associationQName);
    ...
    /**
     * Given an Alfresco model type map it to the appropriate type. Maps cm:folder and cm:content to

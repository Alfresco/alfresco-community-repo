...
public class AuthenticationUtil implements InitializingBean {
  ...
  private static String defaultGuestUserName =
  ...
  public static String maskUsername(String userName) {
    if (userName != null) {
      try {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Failed to mask the username because: " + e.getMessage(),
            e
          );
        }
      }
  ...
  private static Authentication setFullyAuthenticatedUser(
    ...
    try {
      ...
      UsernamePasswordAuthenticationToken auth = getAuthenticationToken(
        userName,
        providedDetails
      );
   ...
   */
  public static Authentication setFullAuthentication(
  ...
  /*package*/static Authentication setRunAsUser(
    String userName,
    UserDetails providedDetails
  ) throws AuthenticationException {
    if (userName == null) {
  ...
  /*package*/static Authentication setRunAsAuthentication(
    Authentication authentication
  ) {
    if (authentication == null) {
      clearCurrentSecurityContext();
      ...
      if (logger.isTraceEnabled()) {
        logger.trace(
          "Setting RunAs principal: " + getMaskedUsername(authentication)
        );
      }
      Context context = ContextHolder.getContext();
      AlfrescoSecureContext sc = null;
      ...
      if (sc.getRealAuthentication() == null) {
        if (logger.isTraceEnabled()) {
          logger.trace(
            "There is no fully authenticated principal. Setting fully authenticated principal: " +
            getMaskedUsername(authentication)
          );
        }
        sc.setRealAuthentication(authentication);
   ...
   * @throws AuthenticationException
   */
  public static Authentication getRunAsAuthentication()
    throws AuthenticationException {
    Context context = ContextHolder.getContext();
    if ((context == null) || !(context instanceof AlfrescoSecureContext)) {
      return null;
   ...
   * @throws AuthenticationException
   */
  public static Authentication getFullAuthentication()
    throws AuthenticationException {
    Context context = ContextHolder.getContext();
    if ((context == null) || !(context instanceof AlfrescoSecureContext)) {
      return null;
  ...
  public static String getAdminUserName() {
    ...
    if (isMtEnabled()) {
      ...
      if (runAsUser != null) {
        String tenantDomain = AuthenticationUtil
          .getUserTenant(runAsUser)
          .getSecond();
  ...
  public static void pushAuthentication() {
    ...
    threadLocalTenantDomainStack
      .get()
      .push(TenantContextHolder.getTenantDomain());
    if (logger.isTraceEnabled()) {
      logger.trace(
        "Pushed authentication in thread: " + Thread.currentThread().getName()
      );
    }
  }
  ...
  public static void popAuthentication() {
    Authentication originalFullAuthentication = threadLocalFullAuthenticationStack
      .get()
      .pop();
    Authentication originalRunAsAuthentication = threadLocalRunAsAuthenticationStack
      .get()
      .pop();
    ...
    if (logger.isTraceEnabled()) {
      logger.trace(
        "Popped authentication in thread: " + Thread.currentThread().getName()
      );
    }
  }
  ...
  public static void logNDC(String userNameIn) {
    ...
    if (userNameIn != null) {
      if (isMtEnabled()) {
        ...
        if (!TenantService.DEFAULT_DOMAIN.equals(tenantDomain)) {
          NDC.push(
            "Tenant:" + tenantDomain + " User:" + maskUsername(userName)
          );
        } else {

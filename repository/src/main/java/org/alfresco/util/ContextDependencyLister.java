...
/**
 * Helper class to list the dependencies between different spring context files that make up a full
 * or partial Application Context. Useful when trying to produce paired-down application contexts
 * for unit testing or embedding.
 *
 * @author Nick Burch
 */
public class ContextDependencyLister {
    ...
    private void calculateDependencies() {
        ...
        for (String parentBean : instance.btl.beanAllDependencies.keySet()) {
            ...
            if (parentContext == null) {
                System.err.println(
                        "Warning - I don't know where defined bean " + parentBean + " came from!");
                continue;
            }
            ...
            for (String depBean : instance.btl.beanAllDependencies.get(parentBean)) {
                ...
                if (depContext == null) {
                    System.err.println(
                            "Warning - I don't know where depdendency " + depBean + " came from!");
    ...
    public void graphVizDependencies(String outFile) throws FileNotFoundException {
        ...
        for (String context : instance.btl.usedContextFiles) {
            if (contextBeans.containsKey(context)) {
                for (String ourBean : contextBeans.get(context)) {
                    if (beanDependencies.containsKey(ourBean)
                            || beanDependedOnBy.containsKey(ourBean)) {
        ...
        for (String context : instance.btl.usedContextFiles) {
            ...
            // Other context files
            if (contextDependsOn.containsKey(context)) {
                String ourBean1Id = makeGraphVizNodeId(linkBean.get(context));

                for (String ctx : contextDependsOn.get(context)) {
                    // We have to draw bean->bean links for this, as
                    // graphviz won't do cluster->cluster links
                    String theirBean1Id = makeGraphVizNodeId(linkBean.get(ctx));
    ...
    /** Need to break over lines or the boxes end up being much too big to look nice */
    ...
    public static class BeanTrackingListener implements ReaderEventListener {
        ...
        private Hashtable<String, List<String>> beanAllDependencies =
        ...
        private void processBeanDefinition(String beanName, BeanDefinition bd) {
            ...
            if (!bd.getConstructorArgumentValues().isEmpty()) {
                ...
                for (ValueHolder vh :
                        bd.getConstructorArgumentValues().getIndexedArgumentValues().values()) {
                    Object v = vh.getValue();
                    addRefIfNeeded(v, refs);
                }
            }

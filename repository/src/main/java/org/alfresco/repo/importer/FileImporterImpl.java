...
public class FileImporterImpl implements FileImporter {
    ...
    public int loadFile(NodeRef container, File file, boolean recurse)
    ...
    public int loadNamedFile(NodeRef container, File file, boolean recurse, String name)
            throws FileImporterException {
        try {
            Counter counter = new Counter();
    ...
    public int loadFile(NodeRef container, File file, FileFilter filter, boolean recurse)
            throws FileImporterException {
        try {
            Counter counter = new Counter();
    ...
    private NodeRef create(
        ...
        if (containerName != null) {
            RetryingTransactionCallback<NodeRef> createDirectoryWork =
        ...
        if (file.isDirectory()) {
            ...
            counter.increment();
            RetryingTransactionCallback<NodeRef> createFileWork =
                    new RetryingTransactionCallback<NodeRef>() {
                        public NodeRef execute() throws Exception {
    ...
    private NodeRef createFile(NodeRef parentNodeRef, File file) {
        ...
        if (assocTypeQName == null) {
            throw new IllegalArgumentException(
                    "Unable to create file.  "
                            + "Parent type is inappropriate: "
                            + nodeService.getType(parentNodeRef));
        }
        ...
        contentProps.put(ContentModel.PROP_CONTENT, new ContentData(null, mimetype, 0L, "UTF-8"));
        ...
        if (logger.isDebugEnabled()) logger.debug("Created file node for file: " + file.getName());
        ...
        ContentWriter writer =
                contentService.getWriter(fileNodeRef, ContentModel.PROP_CONTENT, true);
        try {
            ...
            throw new FileImporterException(
                    "Failed to load content (file not found) " + file.getPath(), e);
        }
    ...
    private NodeRef createDirectory(NodeRef parentNodeRef, String name, String path) {
        ...
        if (logger.isDebugEnabled()) logger.debug("Created folder node with name: " + name);
    ...
    /** @param txnPerFile true to force each file or directory creation to be in its own file */

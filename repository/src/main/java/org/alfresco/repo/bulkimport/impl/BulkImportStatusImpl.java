...
public class BulkImportStatusImpl implements BulkImportStatus {
    ...
    public void startImport(final String sourceDirectory, final String targetSpace) {
        ...
        // Read-side information
        this.numberOfFoldersScanned.set(
                1); // We set this to one to count the initial starting directory (which doesn't
        // otherwise get counted)
    ...
    public void incrementImportableItemsRead(
            final ImportableItem importableItem, final boolean isDirectory) {
        if (importableItem.getHeadRevision().contentFileExists()) {
            if (!isDirectory) {
                numberOfContentFilesRead.incrementAndGet();
                numberOfContentBytesRead.addAndGet(
                        importableItem.getHeadRevision().getContentFileSize());
            }
        }

        if (importableItem.getHeadRevision().metadataFileExists()) {
            numberOfMetadataFilesRead.incrementAndGet();
            numberOfMetadataBytesRead.addAndGet(
                    importableItem.getHeadRevision().getMetadataFileSize());
        }

        if (!isDirectory && importableItem.hasVersionEntries()) {
            for (final ImportableItem.ContentAndMetadata versionEntry :
    ...
    public void incrementImportableItemsSkipped(
            final ImportableItem importableItem, final boolean isDirectory) {
        if (importableItem.getHeadRevision().contentFileExists()) {
            long ignored =
                    isDirectory
                            ? numberOfSpaceNodesSkipped.incrementAndGet()
                            : numberOfContentNodesSkipped.incrementAndGet();
        }
    ...
    public void incrementContentBytesWritten(
            final ImportableItem importableItem, final boolean isSpace, final NodeState nodeState) {
        if (importableItem.getHeadRevision().contentFileExists()) {
            switch (nodeState) {
                case CREATED:
                    numberOfContentBytesWritten.addAndGet(
                            importableItem.getHeadRevision().getContentFileSize());
                    break;
    ...
    public void incrementNodesWritten(
        ...
        if (importableItem.getHeadRevision().contentFileExists()) {
            switch (nodeState) {
                ...
                case CREATED:
                    ignored =
                            isSpace
                                    ? numberOfSpaceNodesCreated.incrementAndGet()
                                    : numberOfContentNodesCreated.incrementAndGet();
                    break;

                case REPLACED:
                    ignored =
                            isSpace
                                    ? numberOfSpaceNodesReplaced.incrementAndGet()
                                    : numberOfContentNodesReplaced.incrementAndGet();
                    break;
            }
        }
        ...
        switch (nodeState) {
            ...
            case REPLACED:
                ignored =
                        isSpace
                                ? numberOfSpacePropertiesWritten.addAndGet(numProperties)
                                : numberOfContentPropertiesWritten.addAndGet(numProperties);
                break;
        }
                ...
                if (versionEntry.contentFileExists()) {
                    switch (nodeState) {
                        ...
                        case REPLACED:
                            ...
                            numberOfContentVersionBytesWritten.addAndGet(
                                    versionEntry.getContentFileSize());
                            break;
                    }
                }
            }
    ...
    public Long getFilesReadPerSecond() {
        ...
        if (duration != null) {
            long totalFilesRead =
                    numberOfContentFilesRead.longValue()
                            + numberOfMetadataFilesRead.longValue()
                            + numberOfContentVersionFilesRead.longValue()
                            + numberOfMetadataVersionFilesRead.longValue();
    ...
    public Long getBytesReadPerSecond() {
        ...
        if (duration != null) {
            long totalDataRead =
                    numberOfContentBytesRead.longValue()
                            + numberOfMetadataBytesRead.longValue()
                            + numberOfContentVersionBytesRead.longValue()
                            + numberOfMetadataVersionBytesRead.longValue();
    ...
    public Long getEntriesScannedPerSecond() {
        Long duration = getDuration();
        if (duration != null) {
            long entriesScannedPerSecond =
                    (numberOfFilesScanned.longValue() + numberOfFoldersScanned.longValue())
                            / duration;
    ...
    public Long getBytesWrittenPerSecond() {
        ...
        if (duration != null) {
            long totalDataWritten =
                    numberOfContentBytesWritten.longValue()
                            + numberOfContentVersionBytesWritten.longValue();
    ...
    public Long getNodesCreatedPerSecond() {
        ...
        if (duration != null) {
            ...
            long totalNodesWritten =
                    numberOfSpaceNodesCreated.longValue()
                            + numberOfSpaceNodesReplaced.longValue()
                            + numberOfContentNodesCreated.longValue()
                            + numberOfContentNodesReplaced.longValue()
                            + numberOfContentVersionsCreated.longValue();

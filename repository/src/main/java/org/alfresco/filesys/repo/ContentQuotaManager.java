...
public class ContentQuotaManager implements QuotaManager, Runnable {
  ...
  public long allocateSpace(
    ...
    if (userQuota != null) {
      ...
      "Failed to allocate " + alloc + " bytes for sess " + sess.getUniqueId()
    );
    ...
    if (allowedAlloc < alloc) {
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "Allocation failed userQuota=" + userQuota
      );

      throw new DiskFullException();
    } else if (logger.isDebugEnabled()) logger.debug(
      "Allocated " + alloc + " bytes, userQuota=" + userQuota
    );
  ...
  public void releaseSpace(
    ...
    if (userQuota != null) {
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "Released " + alloc + " bytes, userQuota=" + userQuota
      );
    } else if (logger.isDebugEnabled()) logger.debug(
      "Failed to release " + alloc + " bytes for sess " + sess.getUniqueId()
    );
  }
   ...
   */
  private UserQuotaDetails getQuotaDetails(
    SrvSession sess,
    boolean loadDetails
  ) {
    synchronized (m_liveUsageLock) {
  ...
  private UserQuotaDetails loadUsageDetails(String userName)
    ...
    try {
      if (userName == null || userName.length() == 0) {
        logger.debug(
          "user name is null or empty - throw QuotaManagerException"
        );
      ...
      throw new QuotaManagerException(
        "Failed to load usage for " + userName + ", " + ex
      );
    }
  ...
  public void run() {
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Content quota manager checker thread starting"
    );
    ...
    while (m_shutdown == false) {
      ...
      if (m_shutdown == true) {
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "Content quota manager checker thread closing"
        );
        ...
        if (m_liveUsage != null && m_liveUsage.size() > 0) {
          try {
            ...
            while (removeNameList.numberOfStrings() > 0) {
              ...
              if (logger.isDebugEnabled()) logger.debug(
                "Removed inactive usage tracking, " + quotaDetails
              );
            }
          } catch (Exception ex) {

...
/**
 * Integration tests for Alfresco Rules. This class does not test the internals of the {@link
 * RuleService} but rather sets up and runs common or previously problematic rules use cases and
...
public class MiscellaneousRulesTest {
    ...
    public static RuleChain STATIC_RULE_CHAIN =
            RuleChain.outerRule(APP_CTXT_INIT).around(TEST_USER);
    ...
    public static void initSpringBeans() throws Exception {
        ...
        TRANSACTION_HELPER =
                appCtxt.getBean("retryingTransactionHelper", RetryingTransactionHelper.class);
    ...
    public void alf14568() throws Exception {
        final NodeRef testSiteDocLib =
                TRANSACTION_HELPER.doInTransaction(
                        new RetryingTransactionCallback<NodeRef>() {
                            public NodeRef execute() throws Throwable {
                                return SITE_SERVICE.getContainer(
        ...
        for (int i : new int[] {0, 1, 2, 3}) {
            folders[i] =
                    testNodes.createFolder(testSiteDocLib, "folder" + i, TEST_USER.getUsername());
        }
                ...
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        org.alfresco.service.cmr.rule.Rule rule =
                                new org.alfresco.service.cmr.rule.Rule();
        ...
        testNodes.createQuickFile(
                MimetypeMap.MIMETYPE_TEXT_PLAIN, folders[1], "quick.txt", TEST_USER.getUsername());
                ...
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        for (NodeRef folder : folders) {
                            NodeRef child =
                                    NODE_SERVICE.getChildByName(
                                            folder, ContentModel.ASSOC_CONTAINS, "quick.txt");
                            if (child != null) {
        ...
        // Now disable all the rules - I don't think this should be necessary, but if we don't do
        // this, the teardown
    ...
    /**
     * ALF-14568 doesn't explicitly say so, but there is a related problem on top of the 'creating
     * cm:original assoc triggers rule' bug. It is the related 'deleting cm:original assoc triggers
     * rule' bug. This test case validates the fix for that issue.
     */
    @Test
    public void alf14568_supplementary() throws Exception {
        ...
        final NodeRef ruleFolder =
                testNodes.createFolder(testSiteDocLib, "ruleFolder", TEST_USER.getUsername());

        // Create a piece of content outside our Rules folder.
        final NodeRef originalContent =
                testNodes.createQuickFile(
        ...
        final QName exifAspectQName =
                QName.createQName("{http://www.alfresco.org/model/exif/1.0}exif");
                ...
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        Action addAspectAction =
                                ACTION_SERVICE.createAction("add-features", params);
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        assertFalse(
                                "Rule executed when it shouldn't have.",
                                NODE_SERVICE.hasAspect(copyNode, exifAspectQName));
    ...
    public void alf13192_rulesFromFirstFolderMoveToSecondWhenDeleteFirstFolder() throws Exception {
        ...
        final NodeRef folder1 =
                testNodes.createFolder(testSiteDocLib, "folder 1", TEST_USER.getUsername());
        final NodeRef folder2 =
                testNodes.createFolder(testSiteDocLib, "folder 2", TEST_USER.getUsername());
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        List<ChildAssociationRef> ruleFolders =
                        ...
                        log.debug(
                                "Rule SystemFolder noderef is " + ruleFolders.get(0).getChildRef());

...
public class UpdateTagScopesActionExecuterTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        actionTrackingService =
                (ActionTrackingService)
                        applicationContext.getBean(ACTION_TRACKING_SERVICE_BEAN_NAME);
     ...
     * @param createdTagScopes - {@link List}&lt;{@link NodeRef}&gt; instance which contains all tag
     *     scope folders
     */
        ...
        for (int i = 0; i < TAGSCOPE_LAYERS; i++) {
            FileInfo newFolder =
                    fileFolderService.create(
                            currentParent,
                            String.format(TEST_FOLDER_NAME_PATTERN, i),
            ...
            for (int j = 0; j < TEST_DOCUMENTS_AMOUNT; j++) {
                FileInfo newDocument =
                        fileFolderService.create(
                                currentParent,
                                String.format(TEST_DOCUMENT_NAME_PATTERN, i, j),
    ...
    /**
     * Tests that tag scopes are properly updated. Cache on the first layer MUST NOT be empty. All
     * other tag scopes MUST BE null
     *
     * @throws Exception
     */
    @Test
    public void testSimpleTagScopesUpsdate() throws Exception {
        Action tagScopeUpdateAction =
                actionService.createAction(UpdateTagScopesActionExecuter.NAME);
        tagScopeUpdateAction.setParameterValue(
                UpdateTagScopesActionExecuter.PARAM_TAG_SCOPES, (Serializable) expectedTagScopes);
        ...
        for (NodeRef tagScopeFolder = iterator.next();
                iterator.hasNext();
    ...
    /**
     * <a href="https://issues.alfresco.com/jira/browse/ACE-1979">ACE-1979</a>: tag scope cache must
     * be emptied when tag scope doesn't contain tags anymore. The fix nullifies content data
     * property for the tag scope cache. This approach allows avoiding immediate update in content
     * store and postponing it untill content store cleaner job is executed
     *
     * @throws Exception
     */
    @Test
     ...
     * @return {@link ContentModel#PROP_TAGSCOPE_CACHE} {@link ContentData} property instance for
     *     the given <code>nodeRef</code> or
     */
                                ...
                                new RetryingTransactionCallback<Serializable>() {
                                    ...
                                    public Serializable execute() throws Throwable {
                                        return nodeService.getProperty(
                                                nodeRef, ContentModel.PROP_TAGSCOPE_CACHE);
                                    }
                                },
                                false,
                                true);

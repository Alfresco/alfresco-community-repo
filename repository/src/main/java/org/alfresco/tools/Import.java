...
public class Import extends Tool {
   ...
   */
  protected @Override /*package*/ToolContext processArgs(String[] args)
    ...
    while (i < args.length) {
      if (args[i].equals("-h") || args[i].equals("-help")) {
        ...
        try {
          context.uuidBinding =
            UUID_BINDING.valueOf(UUID_BINDING.class, args[i]);
  ...
  protected @Override /*package*/void displayHelp() {
    ...
    logError(
      " -encoding package file encoding (default: " +
      ...
      try {
        ...
        try {
          ...
          throw new ToolException(
            "Failed to import package due to " + e.getMessage(),
            e
          );
        }
      } catch (Throwable t) {
  ...
  private class ImportContext extends ToolContext {
    ...
    /*package*/void validate() {
      ...
      if (storeRef == null) {
        throw new ToolArgumentException(
          "Store to import into has not been specified."
        );
      }
      ...
      for (int i = 0; i < packageNames.length; i++) {
        if (
          packageNames[i].endsWith(".acp") || packageNames[i].endsWith(".zip")
        ) {
    ...
    private File getDataFile(int i) {
      String dataFile = (packageNames[i].indexOf('.') != -1)
        ? packageNames[i]
        : packageNames[i] + ".xml";

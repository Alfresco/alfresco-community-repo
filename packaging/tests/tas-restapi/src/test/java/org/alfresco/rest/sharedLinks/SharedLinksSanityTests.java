...
public class SharedLinksSanityTests extends RestTest {
  ...
  public void dataPreparation() throws Exception {
    ...
    folder1 =
      dataContent.usingUser(adminUser).usingSite(siteModel1).createFolder();
    ...
    FileModel newFile = FileModel.getFileModelBasedOnTestDataFile(
      "sampleContent.txt"
    );
    ...
    file6 =
      dataContent
        .usingUser(adminUser)
        .usingResource(folder1)
        .createContent(newFile);
  }

  @TestRail(
    section = { TestGroup.REST_API, TestGroup.SHAREDLINKS },
    executionType = ExecutionType.SANITY,
    description = "Verify create sharedLinks without Path"
  )
  @Test(
    groups = { TestGroup.REST_API, TestGroup.SHAREDLINKS, TestGroup.SANITY }
  )
  public void testCreateAndGetSharedLinks() throws Exception {
    ...
    restClient
      ...
      .body(
        "entry.nodeId",
        org.hamcrest.Matchers.equalTo(file1.getNodeRefWithoutVersion())
      );
    restClient
      .onResponse()
      .assertThat()
      .body("entry.name", org.hamcrest.Matchers.equalTo(file1.getName()));
    restClient
      .onResponse()
      .assertThat()
      .body("entry.id", org.hamcrest.Matchers.equalTo(sharedLink1.getId()));
    restClient
      .onResponse()
      .assertThat()
      .body("entry.path", org.hamcrest.Matchers.nullValue());

    // Same Checks above using sharedLink methods: GET sharedLink: without includePath
    Assert.assertEquals(
      sharedLink1.getNodeId(),
      file1.getNodeRefWithoutVersion()
    );
    Assert.assertEquals(sharedLink1.getName(), file1.getName());
    Assert.assertNull(
      sharedLink1.getPath(),
      "Path is expected to be null for noauth api: Response shows: " +
      sharedLink1.toJson()
    );
    ...
    Utility.sleep(
      ...
      () -> {
        sharedLinksCollection =
          restClient.withCoreAPI().usingSharedLinks().getSharedLinks();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        sharedLinksCollection
          .assertThat()
          .entriesListContains("id", sharedLink1.getId())
          .and()
  ...
  @TestRail(
    ...
    description = "Verify create sharedLinks with Path"
  )
  @Test(
    groups = { TestGroup.REST_API, TestGroup.SHAREDLINKS, TestGroup.REGRESSION }
  )
  public void testCreateAndGetSharedLinksWithInclude() throws Exception {
    ...
    restClient
      ...
      .body(
        "entry.nodeId",
        org.hamcrest.Matchers.equalTo(file2.getNodeRefWithoutVersion())
      );
    restClient
      .onResponse()
      .assertThat()
      .body("entry.name", org.hamcrest.Matchers.equalTo(file2.getName()));
    restClient
      .onResponse()
      .assertThat()
      .body("entry.id", org.hamcrest.Matchers.equalTo(sharedLink2.getId()));
    restClient
      .onResponse()
      .assertThat()
      .body("entry.path", org.hamcrest.Matchers.notNullValue());

    // Same Checks above using sharedLink methods: POST sharedLink: includePath
    Assert.assertEquals(
      sharedLink2.getNodeId(),
      file2.getNodeRefWithoutVersion()
    );
    Assert.assertEquals(sharedLink2.getName(), file2.getName());
    Assert.assertNotNull(
      sharedLink2.getPath(),
      "Path not expected to be null for noauth api: Response shows: " +
      sharedLink1.toJson()
    );
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.SHAREDLINKS },
    executionType = ExecutionType.SANITY,
    description = "Verify delete sharedLinks with and without Path"
  )
  @Test(
    groups = { TestGroup.REST_API, TestGroup.SHAREDLINKS, TestGroup.SANITY }
  )
  public void testDeleteSharedLinks() throws Exception {
    sharedLink3 =
      restClient
        ...
        .withCoreAPI()
        .usingSharedLinks()
        .createSharedLink(file3);
    restClient
      .authenticateUser(testUser1)
      .withCoreAPI()
      .usingSharedLinks()
      .deleteSharedLink(sharedLink3);
    ...
    sharedLink4 =
      restClient
        ...
        .withCoreAPI()
        .usingSharedLinks()
        .createSharedLink(file4);
    restClient
      .authenticateUser(testUser1)
      .withCoreAPI()
      .includePath()
      .usingSharedLinks()
      .deleteSharedLink(sharedLink4);
    restClient.assertStatusCodeIs(HttpStatus.NO_CONTENT);
  }
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.SHAREDLINKS },
    executionType = ExecutionType.SANITY,
    description = "Sanity tests for GET {sharedId}/content and POST {sharedId}/email endpoints"
  )
  @Test(
    groups = { TestGroup.REST_API, TestGroup.SHAREDLINKS, TestGroup.SANITY }
  )
  public void testGetSharedLinkContentAndPostEmail() throws Exception {
    ...
    restClient
      .withCoreAPI()
      .usingSharedLinks()
      .getSharedLinkContent(sharedLink6);
    restClient.assertStatusCodeIs(HttpStatus.OK);
    ...
    restClient
      .withCoreAPI()
      .usingSharedLinks()
      .sendSharedLinkEmail(sharedLink6, postBody);
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.SHAREDLINKS },
    executionType = ExecutionType.REGRESSION,
    description = "Verify get sharedLink/content and get/renditions"
  )
  @Test(
    groups = {
      TestGroup.REST_API,
      TestGroup.SHAREDLINKS,
      TestGroup.REGRESSION,
      TestGroup.RENDITIONS,
    }
  )
  public void testCreateWithExpiryDateAndGetSharedLinkRendition()
    throws Exception {
    sharedLink5 =
      restClient
        .authenticateUser(testUser1)
        ...
        .includePath()
        .usingSharedLinks()
        .createSharedLinkWithExpiryDate(file5, expiryDate);
    restClient.assertStatusCodeIs(HttpStatus.CREATED);

    Assert.assertEquals(sharedLink5.getExpiresAt(), expiryDate);
    Assert.assertNotNull(
      sharedLink5.getPath(),
      "Path not expected to be null: Response shows: " + sharedLink5.toJson()
    );
    ...
    restClient
      ...
      .withCoreAPI()
      .usingSharedLinks()
      .getSharedLinkRenditions(sharedLink5);
    restClient.assertStatusCodeIs(HttpStatus.OK);
  }
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.SHAREDLINKS },
    executionType = ExecutionType.SANITY,
    description = "Sanity tests for GET /renditions, GET /renditions/{renditionId} and GET /renditions/{renditionId}/content endpoints"
  )
  @Test(
    groups = {
      TestGroup.REST_API,
      ...
      TestGroup.SANITY,
      TestGroup.RENDITIONS,
      TestGroup.NOT_SUPPORTED_BY_ATS,
    }
  )
  public void testGetSharedLinkRendition() throws Exception {
    sharedLink7 =
      restClient
        ...
        .withCoreAPI()
        .usingSharedLinks()
        .createSharedLink(file7);
    restClient.assertStatusCodeIs(HttpStatus.CREATED);
    restClient
      .withCoreAPI()
      .usingNode(file7)
      .createNodeRenditionIfNotExists("doclib");
    ...
    Utility.sleep(
      ...
      50000,
      () -> {
        nodeRenditionInfoCollection =
          restClient
            .authenticateUser(testUser1)
            .withCoreAPI()
            .usingSharedLinks()
            .getSharedLinkRenditions(sharedLink7);
        restClient.assertStatusCodeIs(HttpStatus.OK);
    ...
    nodeRenditionInfo =
      restClient
        ...
        .withCoreAPI()
        .usingSharedLinks()
        .getSharedLinkRendition(sharedLink7, "pdf");
    restClient.assertStatusCodeIs(HttpStatus.OK);
    nodeRenditionInfo
      .assertThat()
      .field("id")
    ...
    restClient
      ...
      .withCoreAPI()
      .usingSharedLinks()
      .getSharedLinkRenditionContent(sharedLink7, "pdf");
    restClient.assertStatusCodeIs(HttpStatus.OK);
    restClient.assertHeaderValueContains(
      "Content-Type",
    ...
    Assert.assertTrue(
      restClient.onResponse().getResponse().body().asInputStream().available() >
      0
    );
  }
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.SHAREDLINKS },
    executionType = ExecutionType.SANITY,
    description = "Sanity tests for Range reuest header on   GET shared-links/{sharedId}/renditions/{renditionId}/content endpoints"
  )
  @Test(
    groups = {
      TestGroup.REST_API,
    ...
    Utility.sleep(
      ...
      () -> {
        restClient
          ...
          .addHeader("content-range", "bytes=1-10");
        restClient
          .authenticateUser(testUser1)
          .withCoreAPI()
          .usingSharedLinks()
          .getSharedLinkRenditionContent(sharedLink8, "pdf");

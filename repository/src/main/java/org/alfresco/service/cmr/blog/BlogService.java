...
/**
 * The Blog Service handles the management (CRUD) of Alfresco blog data, namely the blog posts which
 * are exposed in the Share UI under the "Blog" heading.
 *
 * <p>Please note that this service is a work in progress and currently exists primarily to support
 * the blogs REST API.
 *
...
public interface BlogService {
    ...
    /**
     * For a given NodeRef corresponding to a {@link BlogPostInfo}, returns the object wrapping the
     * Node.
     *
     * <p>For anything else, the response is null.
     */
    @NotAuditable
    ...
    /**
     * Retrieves an existing {@link BlogPostInfo} from the repository, which is attached to the
     * specified Node. The parent Node should normally be a Site Container
     */
     ...
     *
     * @param siteShortName the name of the site to add the post to.
     * @param username to limit results to blogs with this cm:creator. <tt>null</tt> means all
     *     users.
     * @param pagingReq an object defining the paging parameters for the result set.
     * @return a {@link PagingResults} object containing some or all of the results (subject to
     *     paging).
     * @see SiteService#getContainer(String, String) to retrieve the blogContainerNode
     */
    PagingResults<BlogPostInfo> getDrafts(
     ...
     * @param byUser if not <tt>null</tt> limits results to posts by the specified user. if
     *     <tt>null</tt> results will be by all users.
     * @param pagingReq an object defining the paging parameters for the result set.
     * @return a {@link PagingResults} object containing some or all of the results (subject to
     *     paging).
     * @see SiteService#getContainer(String, String) to retrieve the blogContainerNode
     */
    PagingResults<BlogPostInfo> getPublished(
     ...
     *     paging).
     * @see SiteService#getContainer(String, String) to retrieve the blogContainerNode
     * @deprecated This method is a domain-specific query used by the Blog REST API and is not
     *     considered suitable for general use.
     */
    PagingResults<BlogPostInfo> getMyDraftsAndAllPublished(
            NodeRef blogContainerNode, Date fromDate, Date toDate, PagingRequest pagingReq);

    /**
     * Finds blog posts by the specified user tagged with the given tag string. This method allows
     * date ranges to be applied to any valid {@link DataTypeDefinition#DATE} or {@link
     * DataTypeDefinition#DATETIME} property. Examples include {@link ContentModel#PROP_CREATED} or
     ...
     * @see SiteService#getContainer(String, String) to retrieve the blogContainerNode
     */
    PagingResults<BlogPostInfo> findBlogPosts(
            String siteShortName,
    ...
    public class RangedDateProperty {
         ...
         * @param dateProperty the Alfresco node property which is to be checked against the range.
         *     (must be a valid date or datetime property)
         */

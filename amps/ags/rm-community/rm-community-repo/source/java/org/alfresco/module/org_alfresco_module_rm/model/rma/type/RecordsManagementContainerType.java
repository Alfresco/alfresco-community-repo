...
public class RecordsManagementContainerType
  ...
  /** I18N */
  private static final String MSG_CANNOT_CAST_TO_RM_TYPE =
    "rm.action.cast-to-rm-type";
    ...
    AuthenticationUtil.runAsSystem(
      new RunAsWork<Void>() {
        ...
        public Void doWork() {
          ...
          boolean isContentSubType = dictionaryService.isSubClass(
            nodeService.getType(childAssocRef.getChildRef()),
            ContentModel.TYPE_CONTENT
          );
          boolean parentIsUnfiledRecordContainer = parentType.equals(
            RecordsManagementModel.TYPE_UNFILED_RECORD_CONTAINER
          );
          boolean parentIsUnfiledRecordFolder = parentType.equals(
            RecordsManagementModel.TYPE_UNFILED_RECORD_FOLDER
          );

          NodeRef child = childAssocRef.getChildRef();
          if (
            (parentIsUnfiledRecordContainer || parentIsUnfiledRecordFolder) &&
            isContentSubType &&
   ...
   */
  protected QName convertNodeToFileplanComponent(
    final ChildAssociationRef childAssocRef
  ) {
    NodeRef child = childAssocRef.getChildRef();
    ...
    if (childType.equals(ContentModel.TYPE_FOLDER)) {
      ...
      if (
        parentType.equals(TYPE_UNFILED_RECORD_CONTAINER) ||

...
public class EmptyTranslationAspectTest extends AbstractMultilingualTestCases {
    ...
    public void testCopy() throws Exception {
        ...
        empty =
                multilingualContentService.addEmptyTranslation(
                        pivot, "empty_" + System.currentTimeMillis(), Locale.CHINESE);
            ...
            copy =
                    fileFolderService
                            .copy(
                                    ...
                                    "copyOfEmpty" + System.currentTimeMillis())
                            .getNodeRef();
    ...
    public void testDeleteNode() throws Exception {
        ...
        assertFalse(
                "The empty translation must be removed from the wokspace",
    ...
    public void testGetContent() throws Exception {
        ...
        // Ensure that the content retourned by a get reader of the empty document is the same as
        // the pivot
        assertEquals(
                "The content retourned of the empty translation must be the same that the content"
                        + " of the pivot",
                fileFolderService.getReader(pivot).getContentString(),
                fileFolderService.getReader(empty).getContentString());
        // Ensure that the content retourned by a get reader of the empty document is different to
        // the non-pivot translation
        assertNotSame(
                "The content retourned of the empty translation must be different that the content"
                        + " of a non-pivot translation",
        ...
        assertEquals(
                "The modification of the pivot is not take in account",
                fileFolderService.getReader(otherTranslation).getContentString(),
                fileFolderService.getReader(empty).getContentString());
    }
    ...
    public void testUpdateContent() throws Exception {
        ...
        assertNotNull(
                "The url of an updated (ex)empty transation can't be null",
                ((ContentData) nodeService.getProperty(empty, ContentModel.PROP_CONTENT))
                        .getContentUrl());
        // Ensure that the mlEmptyTranslation aspect is removed
        assertFalse(
                "The "
                        + ContentModel.ASPECT_MULTILINGUAL_EMPTY_TRANSLATION
                        + " aspect of an updated (ex)empty translation must be removed",
        ...
        assertTrue(
                "The "
                        ...
                        + " aspect of an updated (ex)empty translation must be keeped",
                nodeService.hasAspect(empty, ContentModel.ASPECT_MULTILINGUAL_DOCUMENT));
        // Ensure that the content is written
        assertEquals(
                "The content of the (ex)empty translation is not correct",
                fileFolderService.getReader(empty).getContentString(),
                "ANY_CONTENT");
        // Ensure that the content is different that the content of the pivot
        assertNotSame(
                "The content of the (ex)empty translation is not updated and is the same as the"
                        + " content of the pivot",
                fileFolderService.getReader(empty).getContentString(),
                fileFolderService.getReader(pivot).getContentString());
    }
}

 ...
 * <p/>
 * The RatingService can be used to manage ratings on any content node in the repository. These ratings are defined by {@link RatingScheme rating schemes} which are injected via spring (see <code>rating-services-context.xml</code>). The rating schemes define a minimum and a maximum score value for that scheme.
 * <p/>
 * Ratings can be {@link RatingService#applyRating(NodeRef, float, String) applied}, {@link RatingService#applyRating(NodeRef, float, String) updated} and {@link RatingService#removeRatingByCurrentUser(NodeRef, String) removed}.
 * 
...
public interface RatingService
{
     ...
     * @return The {@link RatingScheme rating schemes} if one of that name is registered, else <code>null</code>.
     */
    @NotAuditable
     ...
     * 
     * @param targetNode
     *            the node to which the rating is to be applied.
     * @param rating
     *            the rating which is to be applied.
     * @param ratingSchemeName
     *            the name of the rating scheme to use.
     * 
     * @throws RatingServiceException
     *             if the rating is not within the range defined by the named scheme or if the named scheme is not registered.
    ...
    /**
     * This method gets the number of individual ratings which have been applied to the specified node in the specified {@link RatingScheme}.
     * 
     * @param targetNode
     *            the node on which the rating is sought.
     * @param ratingSchemeName
     *            the rating scheme name to use.
     * 
    ...
    /**
     * This method gets the total accumulated rating score for the specified node in the specified {@link RatingScheme}. That is, the rating scores for all users for the specified node are summed to give the result.
     * 
     * @param targetNode
    ...
    /**
     * This method gets the {@link Rating} applied by the current user to the specified node in the specified {@link RatingScheme} - if there is one.
     * 
     * @param targetNode
     *            the node on which the rating is sought.
     * @param ratingSchemeName
     ...
     * 
     * @param targetNode
     *            the node on which the ratings are sought.
     * 
    ...
    /**
     * This method removes any {@link Rating} applied by the current user to the specified node in the specified {@link RatingScheme}.
     * 
     * @param targetNode
     ...
     * <p/>
     * Rolled up properties in the RatingService are stored as properties on the rated node and have their values calculated by running a fixed algorithm on properties stored across the <code>cm:rating</code> child nodes. An example of such a roll up would be 'ratingTotal' which would be the sum of all ratings applied to the targetNode in the specified rating scheme.
     * <p/>
     * By rolling up property values from the various <code>cm:rating</code> child nodes and persisting them as individual properties on the <code>cm:rateable</code> node itself, we are able to support indexing, searching and sorting of such properties.
     * 
     * @param targetNode
     *            the rated node whose rolled up property we wish to read.
     * @param ratingSchemeName
     *            the rating scheme name in which the property is relevant.
     * @param ratingRollupName
     *            the name of the rating rollup property, as given in {@link AbstractRatingRollupAlgorithm#getRollupName()}.

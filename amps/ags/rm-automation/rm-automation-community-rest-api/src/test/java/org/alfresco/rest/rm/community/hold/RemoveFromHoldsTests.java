...
public class RemoveFromHoldsTests extends BaseRMRestTest {
    ...
    public void removeContentFromHold(String nodeId) throws Exception {
        ...
        assertTrue(
            holdEntries.isEmpty(),
            "Content held is still added to a hold."
        );
    }
    ...
    public void removeContentAddedToManyHolds() throws Exception {
        ...
        assertTrue(
            holdEntries
                .stream()
                .anyMatch(holdEntry -> holdEntry.getName().contains(HOLD_TWO)),
            "Content held is " + "not held after removing from one hold."
        );
    }
    ...
    public Object[][] getUserWithoutPermissionForAddToHold() {
        ...
        RecordCategoryChild recordFolder = createCategoryFolderInFilePlan();
        nodesToBeClean.add(recordFolder.getParentId());
        UserModel user = roleService.createUserWithRMRole(
        ...
        FileModel contentNoHoldPerm = dataContent
            .usingAdmin()
            .usingSite(testSite)
            .createContent(CMISUtil.DocumentType.TEXT_PLAIN);
        FileModel contentNoHoldCap = dataContent
            .usingAdmin()
            .usingSite(testSite)
            .createContent(CMISUtil.DocumentType.TEXT_PLAIN);
        FileModel privateFile = dataContent
            .usingAdmin()
    ...
    public void removeFromHoldWithUserWithoutPermission(
        UserModel userModel,
        String nodeIdToBeRemoved
    ) throws Exception {
        STEP("Update the list of users to be deleted after running the tests");
        usersToBeClean.add(userModel);

        STEP(
            "Remove node from hold with user without right permission or capability"
        );
        String responseNoHoldPermission = holdsAPI.removeFromHoldAndGetMessage(
        ...
        assertTrue(
            responseNoHoldPermission.contains(ACCESS_DENIED_ERROR_MESSAGE)
        );
    ...
    public Object[][] getUserWithPermissionForAddToHold() {
        ...
        FileModel contentPermission = dataContent
            .usingAdmin()
            .usingSite(testSite)
            .createContent(CMISUtil.DocumentType.TEXT_PLAIN);
    ...
    public void removeFromHoldWithUserWithPermission(
        UserModel userModel,
        String nodeIdToBeRemoved
    ) throws Exception {
        STEP("Update the list of users to be deleted after running the tests");
        ...
        STEP(
            "Remove node from hold with user with right permission and capability"
        );
        holdsAPI.removeItemFromHold(
            userModel.getUsername(),
            userModel.getPassword(),
    ...
    public void cleanUpRemoveContentFromHold() {
        ...
        usersToBeClean.forEach(user ->
            getDataUser().usingAdmin().deleteUser(user)
        );

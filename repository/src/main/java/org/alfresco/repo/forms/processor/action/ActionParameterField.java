...
/**
 * This {@link Field} implementation is a form field which represents a defined {@link
 * ActionDefinition#getParameterDefinitions() action parameter}.
 *
...
public class ActionParameterField implements Field {
    ...
    public ActionParameterField(ParameterDefinition parameterDef, ActionService actionService) {
        ...
        final List<FieldConstraint> fieldConstraints =
                processActionConstraints(parameterDef, actionService);

        if (DataTypeDefinition.NODE_REF.equals(type) && fieldConstraints.isEmpty()) {
            // Parameters of type NodeRef need to be AssociationPickers so that a NodeRef can be
            ...
            // be handled below.
            this.fieldDef =
                    new AssociationFieldDefinition(
                            this.name, "cm:cmobject", AssociationFieldDefinition.Direction.TARGET);
            ...
            if (DataTypeDefinition.NODE_REF.equals(type)) {
                ...
                this.fieldDef =
                        new PropertyFieldDefinition(
                                this.name, DataTypeDefinition.TEXT.getLocalName());
            } else {
     ...
     */
    private List<FieldConstraint> processActionConstraints(
            ParameterDefinition parameterDef, ActionService actionService) {
        ...
        if (paramConstraintName != null) {
            ParameterConstraint paramConstraint =
                    actionService.getParameterConstraint(paramConstraintName);
            if (paramConstraint == null) {
                throw new FormException(
                        "ParameterConstraint name '" + paramConstraintName + "' not recognised.");
            } else {
                ...
                // We need to concatenate each key-value entry into a String like
                // "value|displaylabel"
                // Then the FormService can display the labels but deal with the values as the
                // underlying data.
                List<String> pipeSeparatedAllowedValues =
                        new ArrayList<String>(allowableValuesMap.size());
    ...
    public Object getValue() {
        // Action forms always have every value set to null as there are no default values for
        // action parameters.
        return null;
    }
}

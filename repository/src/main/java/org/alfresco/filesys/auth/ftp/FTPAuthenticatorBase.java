...
public abstract class FTPAuthenticatorBase
  ...
  protected static final Log logger = LogFactory.getLog(
    "org.alfresco.ftp.protocol.auth"
  );
  ...
  public void initialize(ServerConfiguration config, ConfigElement params)
    ...
    AlfrescoConfigSection alfrescoConfig = (AlfrescoConfigSection) config.getConfigSection(
  ...
  protected final void checkForAdminUserName(ClientInfo cInfo) {
    ...
    try {
      ...
      if (
        cInfo.getLogonType() == ClientInfo.LogonNormal &&
      ...
      if (ex instanceof RuntimeException) {
        ...
        throw new RuntimeException(
          "Error during execution of transaction.",
          ex
        );
      }
    }
  ...
  protected final UserTransaction createTransaction() {
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Using " +
      (txService.isReadOnly() ? "ReadOnly" : "Write") +
      " transaction"
    );

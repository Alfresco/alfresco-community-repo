...
public class GetCommentsTests extends RestTest {
    ...
    public void managerIsNotAbleToRetrieveCommentIfAuthenticationFails() throws Exception {
        ...
        restClient
                .authenticateUser(nonexistentModel)
                .withCoreAPI()
                .usingResource(document1)
                .getNodeComments();
    ...
    @TestRail(
            ...
            description =
                    "Verify Manager user gets comments created by admin user with Rest API and"
                            + " status code is 200")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Contributor user gets comments created by admin user with Rest API and"
                            + " status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Collaborator user gets comments created by admin user with Rest API and"
                            + " status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Consumer user gets comments created by admin user with Rest API and"
                            + " status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Manager user gets comments created by another user and status code is"
                            + " 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    public void managerIsAbleToRetrieveCommentsCreatedByAnotherUser() throws Exception {
        ...
        restClient.assertStatusCodeIs(HttpStatus.OK);
        comments.assertThat()
                .entriesListContains("content", contentManager)
                .and()
                .entriesListContains("content", comment);
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify admin user gets comments created by another user and status code is"
                            + " 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    public void adminIsAbleToRetrieveCommentsCreatedByAnotherUser() throws Exception {
        ...
        restClient
                ...
                .withCoreAPI()
                .usingResource(document1)
                .addComment(contentCollaborator);
        restClient.assertStatusCodeIs(HttpStatus.CREATED);
        comments =
                restClient
                        .authenticateUser(adminUserModel)
        ...
        restClient.assertStatusCodeIs(HttpStatus.OK);
        comments.assertThat()
                .entriesListContains("content", contentCollaborator)
                .and()
                .entriesListContains("content", comment);
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify request returns status 403 if the user does not have permission read"
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify invalid request returns status code 400 for invalid maxItems or"
                            + " skipCount")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify User can't get comments for node with ID that does not exist and status"
                            + " code is 404")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    public void userCanNotGetCommentsOnNonExistentFile() throws Exception {
        ...
        restClient
                ...
                .containsSummary(
                        String.format(
                                RestErrorModel.ENTITY_NOT_FOUND, nonexistentFile.getNodeRef()));
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify User can't get comments for node that exists but is not a document or a"
                            + " folder and status code is 400")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    public void userCanNotGetCommentsOnLink() throws Exception {
        ...
        fileWithNodeRefFromLink.setNodeRef(
                link.getNodeRef()
                        .replace("workspace://SpacesStore/", "workspace%3A%2F%2FSpacesStore%2F"));
        restClient
                .authenticateUser(adminUserModel)
                .withCoreAPI()
                .usingResource(fileWithNodeRefFromLink)
                .getNodeComments();
        restClient
                .assertStatusCodeIs(HttpStatus.NOT_FOUND)
                .assertLastError()
                .containsSummary(
                        String.format(
                                ...
                                fileWithNodeRefFromLink.getNodeRef()))
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify get comments from node with invalid network id returns status code 401")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify get comments from node with empty network id returns status code 401")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that if manager adds one comment, it will be returned in getComments"
                            + " response")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that if collaborator adds one comment, it will be returned in"
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that if contributor adds one comment, it will be returned in"
                            + " getComments response")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    @Bug(id = "ACE-4614")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that consumer cannot add a comment and no comments will be returned in"
                            + " getComments response")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Add one comment with Manager and check that returned person is the right one")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    public void addCommentWithManagerCheckReturnedPersonIsTheRightOne() throws Exception {
        ...
        comments =
                restClient
                        ...
                        .usingResource(file)
                        .getNodeComments();
        comments.getOneRandomEntry()
                .onModel()
                .getCreatedBy()
                .assertThat()
                .field("firstName")
                .is("FN-" + user1.getUsername())
                .assertThat()
                .field("lastName")
                .is("LN-" + user1.getUsername());
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Add one comment with Collaborator and check that returned company details are"
                            + " correct")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Add 2 comments with Manager and Collaborator users and verify valid request"
                            + " using skipCount. Check that param is applied")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    public void addTwoCommentsWithManagerCollaboratorVerifySkipCountParamIsApplied()
            throws Exception {
        file =
                dataContent
                        .usingSite(siteModel)
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Add 2 comments with Admin and Collaborator users and verify valid request"
                            + " using maxItems. Check that param is applied")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    public void addTwoCommentsWithAdminCollaboratorVerifyMaxItemsParamIsApplied() throws Exception {
        ...
        comments =
                restClient
                        .authenticateUser(adminUserModel)
                        .withParams("maxItems=1")
                        .withCoreAPI()
                        .usingResource(file)
                        .getNodeComments();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        comments.assertThat()
                .entriesListContains("content", comment2)
                .and()
                .paginationField("count")
                .is("1");
        comments.assertThat().paginationField("totalItems").is("2");
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Add 2 comments with Manager and Admin users and verify valid request using"
                            + " properties. Check that param is applied")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    public void addTwoCommentsWithAdminManagerVerifyPropertiesParamIsApplied() throws Exception {
        ...
        comments =
                restClient
                        .authenticateUser(adminUserModel)
                        .withParams("properties=createdBy,modifiedBy")
                        .withCoreAPI()
                        .usingResource(file)
                        .getNodeComments();
        comments.assertThat().entriesListIsNotEmpty().and().paginationField("count").is("2");
        comments.assertThat().paginationField("totalItems").is("2");
        ...
        comments.getEntries()
                ...
                .assertThat()
                .field("firstName")
                .is("FN-" + usersWithRoles.getOneUserWithRole(UserRole.SiteManager).getUsername())
                .assertThat()
                .field("lastName")
                .is("LN-" + usersWithRoles.getOneUserWithRole(UserRole.SiteManager).getUsername());

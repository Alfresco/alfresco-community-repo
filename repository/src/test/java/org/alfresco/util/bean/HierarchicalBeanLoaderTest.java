...
public class HierarchicalBeanLoaderTest extends TestCase {
  ...
  private String getBean(Class<?> clazz, boolean setBeforeInit)
    throws Exception {
    if (setBeforeInit) {
    ...
    ctx =
      new ClassPathXmlApplicationContext(
        "bean-loader/hierarchical-bean-loader-test-context.xml"
      );
    if (!setBeforeInit) {
      System.setProperty("hierarchy-test.dialect", clazz.getName());
    }
  ...
  public void testFailure1() throws Throwable {
    try {
      ...
      fail(
        "Should not be able to retrieve bean using class " + Collection.class
      );
    } catch (Throwable e) {
      e.printStackTrace();
    }

...
public class SearchQuerySerializerTests {
  ...
  public void testDeserializeQuery() throws IOException {
    ...
    assertEquals(
      "cm:created:bob",
      searchQuery.getFacetQueries().get(0).getQuery()
    );
    ...
    assertEquals(
      2000,
      searchQuery.getLimits().getPermissionEvaluationCount().intValue()
    );
    assertEquals(
      5000,
      searchQuery.getLimits().getPermissionEvaluationTime().intValue()
    );
    ...
    FieldHighlightParameters high2 = searchQuery
      .getHighlight()
      .getFields()
      .get(1);
  ...
  public void testSerializeContext() throws IOException {
    ...
    FacetFieldContext ffc = new FacetFieldContext(
      "theLabel",
      Arrays.asList(
    ...
    SearchContext searchContext = new SearchContext(
      ...
      null,
      Arrays.asList(
        new FacetQueryContext("f1", "creator:bob", 15),
        new FacetQueryContext("f2", null, 20)
      ),
      Arrays.asList(ffc),
    ...
    assertTrue(
      "There must 'context' json output",
      out.contains("\"context\":{\"consistency\":{\"lastTxId\":23}")
    );
    assertTrue(
      "There must 'facetQueries' json output",
      out.contains("\"facetQueries\":")
    );
    assertTrue(
      "There must 'facetQueries f1' json output",
      out.contains(
        "{\"label\":\"f1\",\"filterQuery\":\"creator:bob\",\"count\":15}"
      )
    );
    assertTrue(
      "There must 'facetQueries f2' json output",
      out.contains("{\"label\":\"f2\",\"count\":20}")
    );
    assertTrue(
      "There must 'spellCheck' json output",
      out.contains(
        "\"spellCheck\":{\"type\":\"aFlag\",\"suggestions\":[\"bish\",\"bash\"]}"
      )
    );
    assertTrue(
      "There must 'facetsFields' json output",
      out.contains("\"facetsFields\":[{\"label\":\"theLabel\",\"buckets\"")
    );
    assertTrue(
      "There must 'bucket1' json output",
      out.contains(
        "{\"label\":\"b1\",\"filterQuery\":\"name:b1\",\"count\":23,\"display\":\"displayText1\"}"
      )
    );
    assertTrue(
      "There must 'bucket2' json output",
      out.contains(
        "{\"label\":\"b2\",\"count\":34,\"display\":\"displayText2\"}"
      )
    );
    assertTrue(
      "There must 'bucket3' json output",
      out.contains("{\"label\":\"\",\"count\":42,\"display\":\"displayText3\"}")
    );
    ...
    assertTrue(
      "There must NOT BE a 'context' json output",
      out.contains("\"context\":{}")
    );
    assertFalse(
      "There must NOT BE a 'facetsFields' json output",
      out.contains("\"facetsFields\":{}")
    );
  }
}

...
public class EMailNotificationProvider implements NotificationProvider {
    ...
    if (body != null && body.length() != 0) {
      ...
      if (template == null) {
        ...
        if (template.indexOf(StoreRef.URI_FILLER) != -1) {
          template =
            fileFolderService
              .getLocalizedSibling(new NodeRef(template))
              .toString();
        }
        mail.setParameterValue(MailActionExecuter.PARAM_TEMPLATE, template);
        mail.setParameterValue(
          MailActionExecuter.PARAM_TEMPLATE_MODEL,
          (Serializable) buildTemplateModel(
            notificationContext.getTemplateArgs()
          )
        );
      }
    }
   ...
   */
  private Map<String, Serializable> buildTemplateModel(

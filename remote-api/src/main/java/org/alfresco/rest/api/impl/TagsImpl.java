...
public class TagsImpl implements Tags {
  ...
  public List<Tag> addTags(String nodeId, final List<Tag> tags) {
    ...
    try {
      List<Pair<String, NodeRef>> tagNodeRefs = taggingService.addTags(
        nodeRef,
        tagValues
      );
  ...
  public CollectionWithPagingInfo<Tag> getTags(
    StoreRef storeRef,
    Parameters params
  ) {
    Paging paging = params.getPaging();
    PagingResults<Pair<NodeRef, String>> results = taggingService.getTags(
      storeRef,
    ...
    if (params.getInclude().contains(PARAM_INCLUDE_COUNT)) {
      ...
      if (tagsByCount != null) {
        for (Pair<String, Integer> tagByCountElem : tagsByCount) {
          tagsByCountMap.put(
            tagByCountElem.getFirst(),
            tagByCountElem.getSecond()
          );
        }
      }
    }
  ...
  public Tag changeTag(StoreRef storeRef, String tagId, Tag tag) {
    try {
      ...
      NodeRef newTagNodeRef = taggingService.changeTag(
        storeRef,
        existingTagName,
        newTagName
      );
  ...
  public CollectionWithPagingInfo<Tag> getTags(
    ...
    PagingResults<Pair<NodeRef, String>> results = taggingService.getTags(
      nodeRef,
      Util.getPagingRequest(params.getPaging())
    );
    Integer totalItems = results.getTotalResultCount().getFirst();
    List<Pair<NodeRef, String>> page = results.getPage();
    List<Tag> tags = new ArrayList<Tag>(page.size());

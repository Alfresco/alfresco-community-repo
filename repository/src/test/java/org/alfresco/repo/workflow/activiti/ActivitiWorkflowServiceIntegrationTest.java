...
public class ActivitiWorkflowServiceIntegrationTest extends AbstractWorkflowServiceIntegrationTest {
    ...
    public void testOutcome() throws Exception {
        ...
        QName outcomePropName =
                QName.createQName(NamespaceService.WORKFLOW_MODEL_1_0_URI, "reviewOutcome");
    ...
    /**
     * Actually tests if the priority is the default value. This is based on the assumption that
     * custom tasks are defaulted to a priority of 50 (which is invalid). I'm testing that the code
     * I wrote decides this is an invalid number and sets it to the default value (2).
     */
    @Test
    ...
    public void testReviewAndPooledNotModifiedDate() {
        ...
        final ChildAssociationRef childAssoc =
                nodeService.createNode(
                        companyHome,
                        ContentModel.ASSOC_CONTAINS,
                        QName.createQName(NamespaceService.CONTENT_MODEL_PREFIX, "MNT-11522-test"),
                        ContentModel.TYPE_CONTENT,
                        props);
        NodeRef addedNodeRef = childAssoc.getChildRef();
        Date lastModifiedDate =
                (Date) nodeService.getProperty(addedNodeRef, ContentModel.PROP_MODIFIED);
    ...
    public void testGetWorkflowTaskDefinitionsWithMultiInstanceTask() {
        ...
        // The first task is the start-task, the second one is a multi-instance UserTask. This
        // should have the right form-key
        WorkflowTaskDefinition taskDef = taskDefs.get(1);
        assertEquals("wf:activitiReviewTask", taskDef.getId());
    }

    // Added after MNT-17601. Failed to find any completed tasks when the workflow had a sub
    // process.
    @Test
    public void testCompletedTaskInWorkflowWithSubProcess() {
        ...
        String workflowDefId = definition.getId();
        List<WorkflowTaskDefinition> taskDefs = workflowService.getTaskDefinitions(workflowDefId);
        assertEquals(2, taskDefs.size()); // Prior to the fix for MNT-17601 this list only contained
        // "Alfresco start".
    ...
    public void testAccessStartTaskAsAssigneeFromTaskPartOfProcess() {
        // Test added to validate fix for CLOUD-1929 - start-task can be accesses by assignee of a
        // task
        ...
        workflowService.updateTask(
                ...
                Collections.singletonMap(
                        WorkflowModel.ASSOC_ASSIGNEE, (Serializable) personManager.get(USER3)),
                null,
                null);
    ...
    public void testLongTextValues() throws Exception {
        ...
        // No exception should be thrown when using *very* long String variables (in this case,
        // 10000)
        WorkflowPath path = workflowService.startWorkflow(workflowDef.getId(), params);
        assertNotNull(path);
    ...
    public void testWorkflowRecreatedUser() {
        ...
        // user has access to wokflow because owner value is extracted after fix from initiatorhome
        // noderef
        assertNull(owner);
        workflowService.endTask(startTask.getId(), null);
    }
    ...
    protected void checkTaskQueryStartTaskCompleted(
            String workflowInstanceId, WorkflowTask startTask) {
        ...
        QName startTaskName =
                QName.createQName(NamespaceService.WORKFLOW_MODEL_1_0_URI, "submitAdhocTask");
        checkTaskNameQuery(
                startTaskName, expectedTasks, WorkflowTaskState.COMPLETED, workflowInstanceId);
    ...
    protected void checkTaskQueryTaskCompleted(
            String workflowInstanceId, WorkflowTask theTask, WorkflowTask startTask) {
    ...
    public void testStartWorkflowFromTaskListener() throws Exception {
        WorkflowDefinition testDefinition =
                deployDefinition("activiti/testStartWfFromListener.bpmn20.xml");
        ...
        final ChildAssociationRef childAssoc =
                nodeService.createNode(
                        companyHome,
                        ContentModel.ASSOC_CONTAINS,
                        QName.createQName(NamespaceService.CONTENT_MODEL_PREFIX, "MNT-11926-test"),
                        ContentModel.TYPE_CONTENT,
                        props);
    ...
    public void testWorkflowWithNodes() throws Exception {
        ...
        nodeService.setProperty(
                workflowNode, WorkflowModel.PROP_WORKFLOW_DEF_ENGINE_ID, getEngine());
        ...
        List<WorkflowDefinition> defs =
                workflowService.getAllDefinitionsByName(
    ...
    public void testWorkflowVarious() throws Exception {
        ...
        List<WorkflowTask> workflowTasks =
                workflowService.getStartTasks(Arrays.asList(instanceId), true);
    ...
    public void testBuildWorkflowWithNoUserTasks() throws Exception {
        ...
        WorkflowDefinition testDefinition =
                deployDefinition("activiti/testWorkflowNoUserTasks.bpmn20.xml");
        WorkflowBuilder builder =
                new WorkflowBuilder(testDefinition, workflowService, nodeService, null);
        // Build a workflow
        WorkflowInstance builtInstance = builder.build();
        assertNotNull(builtInstance);

        // Check that there is no active workflow for the deployed definition(it should have
        // finished already due to absence of user tasks)
        List<WorkflowInstance> activeInstances =
                workflowService.getActiveWorkflows(testDefinition.getId());
    ...
    public void testNonAdminCannotDeployWorkflowBySwitchingNodeType() {
        ...
        WorkflowDefinition workflowDef =
                createContentAndSwitchToWorkflow(
                        "activiti$testProcess", "alfresco/workflow/test-security.bpmn20.xml", home);
     ...
     *
     * <p>This should not be allowed to happen if you are non-admin.
     */
    private WorkflowDefinition createContentAndSwitchToWorkflow(
            String processName, String resource, NodeRef parent) {

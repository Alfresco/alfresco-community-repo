...
public class XSLTRenderingEngineTest extends BaseAlfrescoSpringTest {
        ...
        try {
            ...
            RenditionDefinition def =
                    renditionService.createRenditionDefinition(
                            QName.createQName("Test"), XSLTRenderingEngine.NAME);
            ...
            ContentReader reader =
                    contentService.getReader(rendition.getChildRef(), ContentModel.PROP_CONTENT);
    ...
    public void testSimplestTemplateWithTargetPath() throws Exception {
        // We need to run this test as Administrator. The ScriptAction has to run as a full user
        // (instead of as System)
        ...
        try {
            ...
            def.setParameterValue(
                    ...
                    "output/path/for/rendition/output.txt");
        ...
        try {
            ...
            FileInfo folder =
                    FileFolderServiceImpl.makeFolders(
                            fileFolderService, companyHome, pathElements, ContentModel.TYPE_FOLDER);
            ...
            assertEquals(
                    "Avocado DipBagels, New York StyleBeef Frankfurter, Quarter PoundChicken Pot"
                        + " PieCole SlawEggsHazelnut SpreadPotato ChipsSoy Patties,"
                        + " GrilledTruffles, Dark ChocolateAvocado DipBagels, New York StyleBeef"
                        + " Frankfurter, Quarter PoundChicken Pot PieCole SlawEggsHazelnut"
                        + " SpreadPotato ChipsSoy Patties, GrilledTruffles, Dark ChocolateAvocado"
                        + " DipBagels, New York StyleBeef Frankfurter, Quarter PoundChicken Pot"
                        + " PieCole SlawEggsHazelnut SpreadPotato ChipsSoy Patties,"
                        + " GrilledTruffles, Dark ChocolateAvocado DipBagels, New York StyleBeef"
                        + " Frankfurter, Quarter PoundChicken Pot PieCole SlawEggsHazelnut"
                        + " SpreadPotato ChipsSoy Patties, GrilledTruffles, Dark ChocolateAvocado"
    ...
    public void testImportXMLDocument() throws Exception {
        try {
            ...
            FileInfo testImportTable =
                    createXmlFile(companyHome, "TestImportTableXML.xml", testImportTableXml);

            RenditionDefinition def =
                    renditionService.createRenditionDefinition(
                            QName.createQName("Test"), XSLTRenderingEngine.NAME);
            def.setParameterValue(
                    XSLTRenderingEngine.PARAM_TEMPLATE_NODE, testImportTable.getNodeRef());
    ...
    private FileInfo createXmlFile(NodeRef folder, String filename, String content) {
        FileInfo testXmlFile =
                fileFolderService.create(folder, filename, ContentModel.TYPE_CONTENT);
    ...
    private String callParseXmlDocuments =
            "<?xml version=\"1.0\" encoding=\"UTF-8\"?><xsl:stylesheet version=\"1.0\" "
                    + " xmlns:xsl=\"http://www.w3.org/1999/XSL/Transform\""
                    + " xmlns:fn=\"http://www.w3.org/2005/02/xpath-functions\"> <xsl:output"
                    + " method=\"text\" /><xsl:preserve-space elements=\"*\"/><xsl:variable"
                    + " name=\"all_docs\" select=\"alf:parseXMLDocuments('cm:content',"

...
public class ResourceLookupDictionary implements ResourceLocator {
  ...
  @Override
  public Map<String, String> parseTemplateVars(
    Map<String, String> templateVars
  ) {
    if (
      "preferences".equals(
          templateVars.get(ResourceLocator.RELATIONSHIP_RESOURCE)
        )
    ) {
  ...
  public ResourceWithMetadata locateRelationPropertyResource(
    ...
    String resourceKey = ResourceDictionary.resourceKey(
      entityResource,
      relationResource
    );
    String propertyResourceKey = ResourceDictionary.propertyResourceKey(
      resourceKey,
      property
    );
    Map<String, ResourceWithMetadata> apiResources = dictionary
      .getAllResources()
      .get(api);
    ...
    if (resource == null) {
      if (relationResource != null) {
        ...
        resourceKey =
          ResourceDictionary.resourceKey(entityResource, relationResource);
        String relationResourceKey = ResourceDictionary.resourceKey(
          resourceKey,
          property
        );
        resource = apiResources.get(relationResourceKey);
        if (resource != null) {
          ResourceOperation op = resource
            .getMetaData()
            .getOperation(httpMethod);
          if (op == null) {
  ...
  public ResourceWithMetadata locateResource(
    ...
    if (StringUtils.isNotBlank(property2)) {
      String resourceName2 = ResourceDictionary.resourceKey(
        resourceName,
        property
      );
  ...
  public Map<String, ResourceWithMetadata> locateEmbeddedResources(
    ...
    if (embeddedKeys != null && !embeddedKeys.isEmpty()) {
      ...
      for (Entry<String, String> embeddedEntry : embeddedKeys.entrySet()) {
        ResourceWithMetadata res = locateEntityResource(
          api,
          embeddedEntry.getValue(),
          HttpMethod.GET
        );
  ...
  public Map<String, ResourceWithMetadata> locateRelationResource(
    ...
    if (relationshipKeys != null && !relationshipKeys.isEmpty()) {
      ...
      for (String key : relationshipKeys) {
        embeds.put(
          key,
          locateRelationResource(api, entityKey, key, httpMethod)
        );
      }

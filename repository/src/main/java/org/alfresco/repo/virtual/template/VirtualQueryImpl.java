...
public class VirtualQueryImpl implements VirtualQuery {
     ...
     */
    @Override
    public PagingResults<Reference> perform(
        ...
        if (!files && !folders) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Deprecated query  will be skipped due do incompatible types request.");
            }

            return asPagingResults(
                    environment, pagingRequest, new EmptyResultSet(), parentReference);
    ...
    /**
     * @deprecated will be replaced by {@link VirtualQueryConstraint}s once complex constrains are
     *     implemented
     */
    ...
    private PagingResults<Reference> asPagingResults(
            ActualEnvironment environment,
            PagingRequest pagingRequest,
            ResultSet result,
            Reference parentReference)
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Unsupported ResultSet.getStart() when trying to create query paging"
                                + " result");
            }
        ...
        final int totlaSecond =
                !hasMore
                        ? (int) result.getNumberFound()
                        : (int) (start + result.getNumberFound() + 1);
    ...
    private SearchParameters createSearchParameters(
            boolean files, boolean folders, PagingRequest pagingRequest)
            throws VirtualizationException {

...
public class ISO9075 {
     ...
     *
     * <p>Allowed at the start: 'a'..'z' | 'A'..'Z' | '_' Allowed after: 'a'..'z' | 'A'..'Z' |
     * '0'..'9' | '_' | ':' | '$'| '#'
     *
    ...
    public static String encode(String toEncode) {
        if ((toEncode == null) || (toEncode.length() == 0)) {
            return toEncode;
        } else if (XMLChar.isValidName(toEncode)
    ...
    public static String getXPathName(QName qName, NamespacePrefixResolver nspr) {
        ...
        if (prefixes.size() == 0) {
            throw new NamespaceException(
                    "A namespace prefix is not registered for uri " + qName.getNamespaceURI());
        }

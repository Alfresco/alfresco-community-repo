 ...
 *
 * <ul>
 *   <li>{@literal <host>:<port>/alfresco/api/<networkId>/public/alfresco/versions/1/queries/sites}
 * </ul>
 *
 * @author janv
 */
public class QueriesSitesApiTest extends AbstractSingleNetworkSiteTest {
    ...
    private void checkApiCall(
        ...
        if (expectedStatus == 200) {
            ...
            String expectedQuery =
                    "TYPE:\"{http://www.alfresco.org/model/site/1.0}site\" AND (\"*"
            ...
            List<Site> sites =
                    RestApiUtil.parseRestApiEntries(response.getJsonResponse(), Site.class);
     ...
     *
     * <p>GET: {@literal
     * <host>:<port>/alfresco/api/<networkId>/public/alfresco/versions/1/queries/sites}
     */
    @Test
    public void testLiveSearchSites() throws Exception {
        ...
        try {
            ...
            // We can no longer check the assumption that there is a clean repo (ie. no sites start
            // with ab)
            ...
            // create some some sites with site id: ab00001, abc00002, abcd00003, abcde00004,
            // abcdef00005 (and some specific titles and descriptions)
            for (int i = 1; i <= sCount; i++) {
                ...
                String createdSiteId =
                        createSite(siteId, siteTitle, siteDescrip, SiteVisibility.PRIVATE, 201)
                                .getId();
            ...
            // basic search tests
            {
                ...
                checkApiCall(
                        siteT + String.format("%05d", 2) + siteT, null, paging, 200, null, abc);
                ...
                checkApiCall(
                        siteD + String.format("%05d", 3) + siteD, null, paging, 200, null, abcd);
            }
    ...
    private NodeRef getNodeRef(String createdSiteId) {
        ...
        // The following call to siteService.getSite(createdSiteId).getNodeRef() returns a NodeRef
        // like:
        ...
        try {
            ...
            String s1 =
                    createSite(
                            ...
                            .getId();
            String s2 =
                    createSite("siABCD" + RUNID, "ABCD DEF", "sdABCD", SiteVisibility.PRIVATE, 201)
                            .getId();
            String s3 =
                    createSite(
                                    "siABCDE" + RUNID,
                            ...
                            .getId();
            String s4 =
                    createSite("siAB" + RUNID, "AB DEF", "sdAB", SiteVisibility.PRIVATE, 201)
                            .getId();
            String s5 =
                    createSite("siABC" + RUNID, "ABC DEF", "sdABC", SiteVisibility.PRIVATE, 201)
                            .getId();

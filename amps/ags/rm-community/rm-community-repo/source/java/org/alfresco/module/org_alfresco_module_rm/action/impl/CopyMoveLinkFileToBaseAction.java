...
public abstract class CopyMoveLinkFileToBaseAction
  ...
  private static Log logger = LogFactory.getLog(
    CopyMoveLinkFileToBaseAction.class
  );

  /** action parameters */
  public static final String PARAM_DESTINATION_RECORD_FOLDER =
  ...
  protected synchronized void executeImpl(
    ...
    if (isOkToProceedWithAction(actionedUponNodeRef, actionName)) {
      ...
      NodeRef recordFolder = (NodeRef) action.getParameterValue(
          ...
          public Void doWork() {
            try {
              ...
              throw new AlfrescoRuntimeException(
                ...
                (mode == CopyMoveLinkFileToActionMode.MOVE ? "move" : "copy") +
                " operation failed.",
                fileNotFound
              );
            }
   ...
   * @return
   */
  private boolean isOkToProceedWithAction(
      ...
      if (ACTION_FILETO.equals(actionName)) {
        ...
        if (!okToProceed && logger.isDebugEnabled()) {
          logger.debug(
            "Unable to run " +
            actionName +
            " action on a node that isn't unfiled and a sub-class of content type"
          );
        }
        ...
        if (!okToProceed && logger.isDebugEnabled()) {
          logger.debug(
            "Unable to run " +
            actionName +
            " action on a node that isn't filed and a sub-class of content type"
          );
        }
      } else {
        okToProceed = true;
  ...
  private void validateActionPostPathResolution(
    ...
    if (target == null) {
      throw new AlfrescoRuntimeException(
        "Unable to run " +
        actionName +
        " action, because the destination record folder could not be determined."
      );
    }
    if (targetIsUnfiledRecords) {
      QName targetFolderType = getNodeService().getType(target);
      if (
        !TYPE_UNFILED_RECORD_CONTAINER.equals(targetFolderType) &&
        !TYPE_UNFILED_RECORD_FOLDER.equals(targetFolderType)
      ) {
        throw new AlfrescoRuntimeException(
          "Unable to run " +
          actionName +
          " action, because the destination record folder is an inappropriate type."
        );
      }
    } else {
      if (
        getRecordFolderService().isRecordFolder(target) &&
        !getDictionaryService()
          .isSubClass(actionedUponType, ContentModel.TYPE_CONTENT) &&
        (
        ...
        throw new AlfrescoRuntimeException(
          "Unable to run " +
          actionName +
          " action, because the destination record folder is an inappropriate type. A record folder cannot contain another folder or a category"
        );
      } else if (
        filePlanService.isRecordCategory(target) &&
        getDictionaryService()
          .isSubClass(actionedUponType, ContentModel.TYPE_CONTENT)
      ) {
        throw new AlfrescoRuntimeException(
          "Unable to run " +
          actionName +
          " action, because the destination record folder is an inappropriate type. A record category cannot contain a record"
        );
      }
    }
  }
  ...
  private NodeRef createOrResolvePath(
    ...
    final NodeRef context = getContext(
      action,
      actionedUponNodeRef,
      targetisUnfiledRecords
    );
    ...
    final String[] pathElementsArray = StringUtils.tokenizeToStringArray(
      pathParameter,
      "/",
      false,
      true
    ...
    if ((pathElementsArray != null) && (pathElementsArray.length > 0)) {
      ...
      final boolean create = createValue == null
        ? false
        : createValue.booleanValue();
      QName type = getNodeService().getType(actionedUponNodeRef);
      final boolean isRecord = getDictionaryService()
  ...
  private NodeRef createOrResolvePath(
    ...
    if (nodeRef == null) {
      ...
      if (
        nodeType.equals(RecordsManagementModel.TYPE_HOLD_CONTAINER) ||
  ...
  private NodeRef getChild(NodeRef parent, String childName) {
    return getNodeService()
      .getChildByName(parent, ContentModel.ASSOC_CONTAINS, childName);
  }
  ...
  private NodeRef createChild(
    ...
    if (child == null) {
      if (targetisUnfiledRecords) {
        ...
        if (
          RecordsManagementModel.TYPE_RECORD_FOLDER.equals(
          ...
          throw new AlfrescoRuntimeException(
            "Unable to execute " +
            action.getActionDefinitionName() +
            " action, because the destination path has a record category within a record folder."
          );
        }
  ...
  private NodeRef getContext(
    ...
    if ((context == null) || (!getNodeService().exists(context))) {
      throw new AlfrescoRuntimeException(
        "Unable to execute " +
        action.getActionDefinitionName() +
        " action, because the path resolution context could not be determined."
      );
    }
    return context;
  }

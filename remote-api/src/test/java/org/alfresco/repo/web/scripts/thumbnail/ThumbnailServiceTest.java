...
public class ThumbnailServiceTest extends BaseWebScriptTest {
  ...
  protected void setUp() throws Exception {
    ...
    try {
      this.transactionService =
        (TransactionServiceImpl) getServer()
          .getApplicationContext()
          .getBean("transactionComponent");
    } catch (ClassCastException e) {
      throw new AlfrescoRuntimeException(
        "The ThumbnailServiceTest needs direct access to the TransactionServiceImpl"
      );
    }
    ...
    InputStream pdfStream =
      ThumbnailServiceTest.class.getClassLoader()
        .getResourceAsStream(
          "org/alfresco/repo/web/scripts/thumbnail/test_doc.pdf"
        );
    assertNotNull(pdfStream);
    InputStream jpgStream =
      ThumbnailServiceTest.class.getClassLoader()
        .getResourceAsStream(
          "org/alfresco/repo/web/scripts/thumbnail/test_image.jpg"
        );
  ...
  public void testCreateThumbnail() throws Exception {
    ...
    if (
      ...
      String url =
        "/api/node/" +
        pdfNode.getStoreRef().getProtocol() +
        "/" +
        pdfNode.getStoreRef().getIdentifier() +
        "/" +
        pdfNode.getId() +
        "/content/thumbnails";
      ...
      Response response = sendRequest(
        new PostRequest(url, tn.toString(), "application/json"),
        200
      );
    ...
    Response getAllResp = sendRequest(
      new GetRequest(getThumbnailsURL(jpgNode)),
      200
    );
    ...
    String url =
      "/api/node/" +
      jpgNode.getStoreRef().getProtocol() +
      "/" +
      jpgNode.getStoreRef().getIdentifier() +
      "/" +
      jpgNode.getId() +
      "/content/thumbnails";
    JSONObject tn = new JSONObject();
  ...
  public void testCreateThumbnailInReadonlyMode() throws Exception {
    ...
    if (
      ...
      String url =
        ...
        "/" +
        pdfNode.getId() +
        "/content/thumbnails/webpreview?c=force";
  ...
  public void testUpdateThumbnail() throws Exception {
    ...
    sendRequest(
      new GetRequest(getThumbnailsURL(jpgNode) + "/incorrectname"),
      404
    );
    //Request for update of thumbnail, that is absent
    sendRequest(
      new PutRequest(
        getThumbnailsURL(jpgNode) + "/incorrectname",
        "",
        "application/json"
      ),
      404
    ...
    sendRequest(
      new PutRequest(
        getThumbnailsURL(jpgNode) + "/doclib",
        "",
        "application/json"
      ),
      200
  ...
  private void getWait(NodeRef node, String thumbnailName) throws Exception {
    ...
    while (true) {
      ...
      if (response.getStatus() == 200) {
        ...
        System.out.println(
          "Error during getWait: " + response.getContentAsString()
        );
        fail(
          "A 500 status was found whilst waiting for the thumbnail to be processed"
        );
      } else {
        Thread.sleep(100);
      }
  ...
  public void testPlaceHolder() throws Exception {
    if (
      ...
      sendRequest(
        new GetRequest(getThumbnailsURL(pdfNode) + "/webpreview"),
        404
      );
      sendRequest(
        new GetRequest(getThumbnailsURL(pdfNode) + "/webpreview?ph=true"),
        404
      );
    }
    ...
    sendRequest(
      new GetRequest(getThumbnailsURL(jpgNode) + "/medium?ph=true"),
      200
    );

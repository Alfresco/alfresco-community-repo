...
public class ExplicitSolrStoreMappingWrapper
  ...
  public ExplicitSolrStoreMappingWrapper(
    SolrStoreMapping wrapped,
  ...
  public void init() {
    httpClientFactory =
      (HttpClientFactory) beanFactory.getBean(wrapped.getHttpClientFactory());
    ...
    if ((wrapped.getNodes() == null) || (wrapped.getNodes().length == 0)) {
      ...
      httpClientsAndBaseURLs.add(
        new HttpClientAndBaseUrl(httpClient, wrapped.getBaseUrl())
      );
    } else {
      for (String node : wrapped.getNodes()) {
        ...
        try {
          ...
          httpClientsAndBaseURLs.add(
            new HttpClientAndBaseUrl(
              httpClientFactory.getHttpClient(nodeHost, realPort),
              nodeBaseUrl
            )
          );
        } catch (NumberFormatException nfe) {
    ...
    policy =
      new ExplicitShardingPolicy(
        wrapped.getNumShards(),
        wrapped.getReplicationFactor(),
        httpClientsAndBaseURLs.size()
      );
  }
  ...
  public String getShards() {
    if (!policy.configurationIsValid()) {
      throw new AlfrescoRuntimeException(
        "Invalid shard configuration: shard = " +
        wrapped.getNumShards() +
        "   reoplicationFactor = " +
        wrapped.getReplicationFactor() +
        " with node count = " +
  ...
  private String getShards2() {
    try {
      ...
      for (int shard = 0; shard < wrapped.getNumShards(); shard++) {
        ...
        HttpClientAndBaseUrl httpClientAndBaseUrl = httpClientsAndBaseURLs.toArray(
          new HttpClientAndBaseUrl[0]
        )[nodeId - 1];
        builder.append(
          encoder.encode(httpClientAndBaseUrl.getProtocol() + "://", "UTF-8")
        );
        builder.append(encoder.encode(httpClientAndBaseUrl.getHost(), "UTF-8"));
        builder.append(':');
        builder.append(
          encoder.encode("" + httpClientAndBaseUrl.getPort(), "UTF-8")
        );
        if (httpClientAndBaseUrl.getBaseUrl().startsWith("/")) {
          builder.append(
            encoder.encode(httpClientAndBaseUrl.getBaseUrl(), "UTF-8")
          );
        } else {
          builder.append(
            encoder.encode("/" + httpClientAndBaseUrl.getBaseUrl(), "UTF-8")
  ...
  private static class HttpClientAndBaseUrl {
    ...
    public int hashCode() {
      ...
      result =
        prime * result + ((getHost() == null) ? 0 : getHost().hashCode());
    ...
    public boolean equals(Object obj) {
      ...
      } else if (!getHost().equals(other.getHost())) return false; else if (
    ...
    public String toString() {
      return (
        "HttpClientAndBaseUrl [getBaseUrl()=" +
        getBaseUrl() +
        ", getHost()=" +
        getHost() +
        ", getPort()=" +
  ...
  public Pair<HttpClient, String> getHttpClientAndBaseUrl() {
    ...
    return new Pair<>(
      httpClientAndBaseUrl.httpClient,
      isSharded()
        ? httpClientAndBaseUrl.baseUrl + "-" + shard
        : httpClientAndBaseUrl.baseUrl
    );
  }
}

...
public class WorkflowObjectFactory {
    ...
    public WorkflowInstance createInstance(
        ...
        String description =
                (String) getVariable(variables, WorkflowModel.PROP_WORKFLOW_DESCRIPTION);

        NodeRef initiator = null;
        ScriptNode initiatorSN =
                (ScriptNode) getVariable(variables, WorkflowConstants.PROP_INITIATOR);
        if (initiatorSN != null) {
        ...
        workflowInstance.priority =
                (Integer) getVariable(variables, WorkflowModel.PROP_WORKFLOW_PRIORITY);
    ...
    public WorkflowPath createPath(
            String id, WorkflowInstance wfInstance, WorkflowNode node, boolean isActive) {
        String actualId = buildGlobalId(id);
    ...
    public WorkflowTaskDefinition createTaskDefinition(
            String id, WorkflowNode node, String typeName, boolean isStart) {
    ...
    public WorkflowTask createTask(
        ...
        return createTask(
                ...
                state,
                path,
                path.getInstance().getDefinition().getName(),
                properties);
    }
    ...
    public WorkflowTask createTask(
        ...
        String title =
                getLabel(
                        processKey,
                        TITLE_LABEL,
                        metadata.getTitle(dictionaryService),
                        defaultTitle,
                        name);
        defaultDescription =
                (defaultDescription != null && defaultDescription.trim().length() == 0)
                        ? null
                        : defaultDescription;
        String description =
                getLabel(
                        ...
                        DESC_LABEL,
                        defaultDescription,
                        metadata.getDescription(dictionaryService),
                        title);
        return new WorkflowTask(
                actualId, taskDef, name, title, description, state, path, properties);
    }

    public WorkflowTimer createWorkflowTimer(
            String id,
            String name,
            String error,
            Date dueDate,
    ...
    public String getTaskTitle(
            TypeDefinition typeDefinition, String defName, String defaultTitle, String name) {
    ...
    private NodeRef getNodeVariable(Map<String, Object> variables, QName qName) {
        ...
        String message =
                "Variable " + qName + " should be of type ScriptNode but was " + obj.getClass();
        throw new WorkflowException(message);
    }

    private Object getVariable(Map<String, Object> variables, QName qName) {
        if (variables == null || qName == null) return null;
    ...
    public <T extends Object> List<T> filterByDomain(
            Collection<T> values, final Function<T, String> processKeyGetter) {
                ...
                new Filter<T>() {
                    public Boolean apply(T value) {
                        ...
                        return currentDomain.equals(domain)
                                || domain.equals(TenantService.DEFAULT_DOMAIN);
                    }
                });
    }

    /** For "default" domain return ALL workflows For tenant-specific workflows exclude "default" */
    public <T extends Object> List<T> doSpecialTenantFilter(
            Collection<T> values, final Function<T, String> processKeyGetter) {
        final String currentDomain = tenantService.getCurrentUserDomain();
        return CollectionUtils.filter(
                values,
                new Filter<T>() {
                    public Boolean apply(T value) {
                        ...
                        String domain = TenantUtil.getTenantDomain(key);
                        return currentDomain.equals(domain)
                                || currentDomain.equals(TenantService.DEFAULT_DOMAIN);
                    }
                });
    }
    ...
    public TypeDefinition getTaskTypeDefinition(String name, boolean isStart) {
        ...
        if (typeDef == null) {
            QName defaultTypeName =
                    isStart ? defaultStartTaskType : WorkflowModel.TYPE_WORKFLOW_TASK;

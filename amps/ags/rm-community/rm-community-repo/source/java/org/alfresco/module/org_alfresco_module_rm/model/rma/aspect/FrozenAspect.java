...
public class FrozenAspect
   ...
   */
  private static final String ON_UPDATE_PROP_FROZEN_BEHAVIOUR_NAME =
   ...
   */
  public void setPropertyModificationAllowedCheck(
    PropertyModificationAllowedCheck propertyModificationAllowedCheck
  ) {
  ...
  public void disableOnPropUpdateFrozenAspect() {
    org.alfresco.repo.policy.Behaviour behaviour = getBehaviour(
      ...
      (RunAsWork<Void>) () -> {
        if (nodeService.exists(nodeRef) && freezeService.isFrozen(nodeRef)) {
          ...
          throw new PermissionDeniedException(
            I18NUtil.getMessage("rm.hold.delete-frozen-node")
          );
        }
    ...
    for (ChildAssociationRef assoc : assocs) {
      ...
      if (assoc.isPrimary()) {
        ...
        if (freezeService.isFrozen(nodeRef)) {
          ...
          throw new PermissionDeniedException(
            I18NUtil.getMessage("rm.hold.delete-node-frozen-children")
          );
        }
      ...
      (RunAsWork<Void>) () -> {
        if (nodeService.exists(nodeRef) && instanceOf(nodeRef, TYPE_CONTENT)) {
          ...
          if (nodeService.hasAspect(parentRef, ASPECT_HELD_CHILDREN)) {
            ...
            if (
              instanceOf(parentRef, TYPE_FOLDER) &&
              !nodeService.hasAspect(parentRef, ASPECT_SITE_CONTAINER)
            ) {
              ...
              getInternalNodeService()
                .addAspect(parentRef, ASPECT_HELD_CHILDREN, props);
            }
          }
        }
        return null;
  ...
  public void onRemoveAspect(final NodeRef nodeRef, QName aspectTypeQName) {
    AuthenticationUtil.runAsSystem(
      (RunAsWork<Void>) () -> {
        if (nodeService.exists(nodeRef) && instanceOf(nodeRef, TYPE_CONTENT)) {
          // get the owning folder
          final NodeRef owningFolder = nodeService
            .getPrimaryParent(nodeRef)
            .getParentRef();
          ...
          if (nodeService.hasAspect(owningFolder, ASPECT_HELD_CHILDREN)) {
            ...
            final int currentCount = (Integer) nodeService.getProperty(
              owningFolder,
              PROP_HELD_CHILDREN_COUNT
            );
            if (currentCount > 0) {
  ...
  public void beforeMoveNode(
    ...
    AuthenticationUtil.runAsSystem(
      (RunAsWork<Void>) () -> {
        if (
          nodeService.exists(oldChildAssocRef.getChildRef()) &&
          freezeService.isFrozen(oldChildAssocRef.getChildRef())
        ) {
          throw new PermissionDeniedException(
            I18NUtil.getMessage("rm.hold.move-frozen-node")
          );
        }
        return null;
      }
    ...
    AuthenticationUtil.runAsSystem(
      (RunAsWork<Void>) () -> {
        ...
        if (
          ...
          throw new PermissionDeniedException(
            I18NUtil.getMessage("rm.hold.update-frozen-node")
          );
        }
        return null;
      }

...
public class RM1008Test extends BaseRMTestCase {
    ...
    doTestInTransaction(
      new Test<Void>() {
        ...
        public Void run() {
          ...
          Capability viewRecords = capabilityService.getCapability(
            "ViewRecords"
          );
          ...
          assertEquals(
            ...
            viewRecords.hasPermission(holdContainer)
          );
          assertEquals(
            AccessStatus.ALLOWED,
            viewRecords.hasPermission(transferContainer)
          );
  ...
  public void testTransfer() {
    final NodeRef transferFolder = doTestInTransaction(
      new Test<NodeRef>() {
        @Override
        public NodeRef run() {
          NodeRef transferCat = filePlanService.createRecordCategory(
            filePlan,
            "transferCat"
          );
    ...
    final NodeRef transfer = doTestInTransaction(
      new Test<NodeRef>() {
        @Override
        public NodeRef run() {
    ...
    doTestInTransaction(
      new Test<Void>() {
        ...
        public Void run() {
          ...
          assertEquals(
            AccessStatus.ALLOWED,
            viewRecords.hasPermission(transfer)
          );
          assertEquals(
            AccessStatus.ALLOWED,
            permissionService.hasPermission(transfer, RMPermissionModel.FILING)
          );

 ...
 * but in this test class is mocked using a separate Thread that returns well known values. What
 * makes the AsynchronousExtractor different from other {@link AbstractMappingMetadataExtracter} sub
 * classes is that the calling Thread does not do the work of updating properties or the content, as
 * the T-Engine will reply at some later point.
 *
 * @author adavis
 */
...
public class AsynchronousExtractorTest extends BaseSpringTest {
    ...
    private class TestAsynchronousExtractor extends AsynchronousExtractor {
        ...
        TransformClient mockTransformClient =
                new TransformClient() {
                    @Override
         ...
         *
         * @param mockResult if specified indicates a value was returned. The result is read as a
         *     resource from the classpath.
         * @param changedHashcode if specified indicates that the source node content changed or was
         *     deleted between the request to extract or embed and the response.
         */
        TestAsynchronousExtractor(
                String mockResult, Integer changedHashcode, OverwritePolicy policy) {
        ...
        @Override
        protected Map<String, Serializable> mapSystemToRaw(
                Map<QName, Serializable> systemMetadata) {
        ...
        private void mockTransform(
            ...
            int transformContentHashCode =
                    changedHashcode == null ? sourceContentHashCode : changedHashcode;
            if (mockResult != null) {
                try (InputStream transformInputStream =
                        getClass().getClassLoader().getResourceAsStream(mockResult)) {
                    renditionService2.consume(
                            sourceNodeRef,
                            transformInputStream,
                            renditionDefinition,
                            transformContentHashCode);
                } catch (IOException e) {
                    throw new RuntimeException(
                            "Could not read '" + mockResult + "' from the classpath.", e);
                }
            } else {
                renditionService2.failure(
                        sourceNodeRef, renditionDefinition, transformContentHashCode);
            }
    ...
    /**
     * Mock CategoryService that only knows about taggable. Unlike the real implementation it does
     * not call Solr to find tag nodes.
     */
            ...
            NodeRef catRoot =
                    nodeService
                            .createNode(
                                    ...
                                    ContentModel.ASSOC_CHILDREN,
                                    QName.createQName(
                                            NamespaceService.CONTENT_MODEL_1_0_URI, "categoryRoot"),
                                    ContentModel.TYPE_CATEGORYROOT)
                            .getChildRef();
            taggableCat =
                    nodeService
                            .createNode(
    ...
    public void before() throws Exception {
        ...
        namespacePrefixResolver =
                (NamespacePrefixResolver) applicationContext.getBean("namespaceService");
        ...
        transformServiceRegistry =
                (TransformServiceRegistry) applicationContext.getBean("transformServiceRegistry");
        renditionDefinitionRegistry2 =
                (RenditionDefinitionRegistry2Impl)
                        applicationContext.getBean("renditionDefinitionRegistry2");
        ...
        asynchronousExtractor =
                (AsynchronousExtractor) applicationContext.getBean("extractor.Asynchronous");
        ...
        contentMetadataExtracter.setApplicableTypes(
                new String[] {ContentModel.TYPE_CONTENT.toString()});
        ...
        contentMetadataEmbedder.setApplicableTypes(
                new String[] {ContentModel.TYPE_CONTENT.toString()});
                ...
                .doInTransaction(
                        new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                            ...
                            public Void execute() throws Throwable {
                                ...
                                cw.putContent(
                                        AbstractContentTransformerTest.loadQuickTestFile("pdf"));

                                origProperties = nodeService.getProperties(nodeRef);
                                nodeService.setProperties(nodeRef, origProperties);
                                origProperties =
                                        new HashMap<>(origProperties); // just in case the contents
                                // changed.
                                expectedProperties =
                                        new HashMap<>(origProperties); // ready to be modified.

                                origSize = getSize(nodeRef);

                                mockCategoryService =
                                        new MockCategoryService(nodeService, publicNodeService);
    ...
    private void assertAsyncMetadataExecute(
            ActionExecuterAbstractBase executor,
            String mockResult,
            Integer changedHashcode,
            long expectedSize,
            Map<QName, Serializable> expectedProperties,
            OverwritePolicy policy,
            QName... ignoreProperties)
            throws Exception {
        TestAsynchronousExtractor extractor =
                new TestAsynchronousExtractor(mockResult, changedHashcode, policy);
    ...
    private void executeAction(
            ActionExecuterAbstractBase extractor, TestAsynchronousExtractor asynchronousExtractor)
    ...
    private void assertProperties(
            NodeRef nodeRef,
            Map<QName, Serializable> expectProperties,
            String state,
            QName[] ignoreProperties) {
        properties = nodeService.getProperties(nodeRef);

        // Work out the difference in a human readable form and ignore the 5 system set properties
        // (as they always
        // change) plus any the caller has requested.
        StringJoiner sj = new StringJoiner("\n");
        List<QName> ignoreKeys =
                new ArrayList<>(
                        asList(
                                PROP_MODIFIED,
                                PROP_MODIFIER,
    ...
    public void testExtractHtml() throws Exception {
        expectedProperties.put(
                QName.createQName("cm:author", namespacePrefixResolver), "Nevin Nollop");
        expectedProperties.put(
                QName.createQName("cm:description", namespacePrefixResolver),
                "Gym class featuring a brown fox and lazy dog");
        expectedProperties.put(
                QName.createQName("cm:title", namespacePrefixResolver),
                "The quick brown fox jumps over the lazy dog");

        assertAsyncMetadataExecute(
                contentMetadataExtracter,
                "quick/quick.html_metadata.json",
                UNCHANGED_HASHCODE,
                origSize,
                expectedProperties,
    ...
    public void testUnknownNamespaceInResponse() throws Exception {
        ...
        assertAsyncMetadataExecute(
                contentMetadataExtracter,
                "quick/unknown_namespace_metadata.json",
                UNCHANGED_HASHCODE,
                origSize,
                expectedProperties,
                OverwritePolicy.PRAGMATIC);
    ...
    public void testExtractMsg() throws Exception // has dates as RFC822
            {
        expectedProperties.put(
                QName.createQName("cm:addressee", namespacePrefixResolver),
                "mark.rogers@alfresco.com");
        expectedProperties.put(
                QName.createQName("cm:description", namespacePrefixResolver),
                "This is a quick test");
        expectedProperties.put(
                QName.createQName("cm:addressees", namespacePrefixResolver),
                new ArrayList<>(
                        asList(
                                "mark.rogers@alfresco.com",
                                "speedy@quick.com",
        ...
        expectedProperties.put(
                QName.createQName("cm:sentdate", namespacePrefixResolver),
                SIMPLE_DATE_FORMAT.parse("Fri Jan 18 13:44:20 GMT 2013")); // 2013-01-18T13:44:20Z
        expectedProperties.put(
                QName.createQName("cm:subjectline", namespacePrefixResolver),
                "This is a quick test");
        expectedProperties.put(
                QName.createQName("cm:author", namespacePrefixResolver), "Mark Rogers");
        expectedProperties.put(
                QName.createQName("cm:originator", namespacePrefixResolver), "Mark Rogers");

        assertAsyncMetadataExecute(
                contentMetadataExtracter,
                "quick/quick.msg_metadata.json",
                UNCHANGED_HASHCODE,
                origSize,
                expectedProperties,
                OverwritePolicy.PRAGMATIC);

        Serializable sentDate =
                properties.get(QName.createQName("cm:sentdate", namespacePrefixResolver));
    }
    ...
    public void testExtractEml() throws Exception // has dates as longs since 1970
        ...
        expectedProperties.put(
                QName.createQName("cm:addressee", namespacePrefixResolver),
                "Nevin Nollop <nevin.nollop@gmail.com>");
        expectedProperties.put(
                QName.createQName("cm:description", namespacePrefixResolver),
                "The quick brown fox jumps over the lazy dog");
        expectedProperties.put(
        ...
        expectedProperties.put(
                QName.createQName("imap:dateSent", namespacePrefixResolver),
                SIMPLE_DATE_FORMAT.parse("Fri Jun 04 13:23:22 BST 2004"));
        expectedProperties.put(
                QName.createQName("imap:messageTo", namespacePrefixResolver),
                "Nevin Nollop <nevin.nollop@gmail.com>");
        expectedProperties.put(
                QName.createQName("imap:messageId", namespacePrefixResolver),
                "<20040604122322.GV1905@phoenix.home>");
        expectedProperties.put(
                QName.createQName("cm:title", namespacePrefixResolver),
                "The quick brown fox jumps over the lazy dog");
        expectedProperties.put(
                QName.createQName("imap:messageSubject", namespacePrefixResolver),
                "The quick brown fox jumps over the lazy dog");
        expectedProperties.put(
                QName.createQName("imap:messageCc", namespacePrefixResolver),
        ...
        expectedProperties.put(
                ...
                "The quick brown fox jumps over the lazy dog");
        expectedProperties.put(
                QName.createQName("imap:messageFrom", namespacePrefixResolver),
                "Nevin Nollop <nevin.nollop@alfresco.com>");
        expectedProperties.put(
                QName.createQName("cm:originator", namespacePrefixResolver),
                "Nevin Nollop <nevin.nollop@alfresco.com>");

        // Note: As the metadata is for eml, an aspect gets added resulting in a second extract
        // because of
        // ImapContentPolicy.onAddAspect. I cannot see a good way to avoid this.
        assertAsyncMetadataExecute(
                contentMetadataExtracter,
                "quick/quick.eml_metadata.json",
                UNCHANGED_HASHCODE,
                origSize,
                expectedProperties,
                OverwritePolicy.PRAGMATIC,
                // cm:author is not in the quick.eml_metadata.json but is being added by the second
                // extract which thinks
                // the source mimetype is MimetypeMap.MIMETYPE_PDF, because that is what the
                // before() method sets the
                // content to. As a result the PdfBox metadata extractor is called, which extracts
                // cm:author. Given that
                // we don't know when this will take place, we simply ignore this property. We could
                // fix this up, but it
    ...
    public void testEmbed() throws Exception {
        ...
        assertTrue(
                "System properties were not set: simple value",
                metadata.contains(
                        "\"{http://www.alfresco.org/model/content/1.0}creator\":\"System\""));

        // Check the metadata sent to the T-Engine contains the collection value added by the
        // mockTransform.
        // The order of elements in the collection may change, so we cannot use a simple string
        // compare.
        ...
        assertTrue(
                "There should have 3 elements: " + collection, collection.split(",").length == 3);
    ...
    public void testOverwritePolicyEager() throws Exception {
        ...
        assertAsyncMetadataExecute(
                contentMetadataExtracter,
                "quick/quick.eager_policy_metadata.json",
                UNCHANGED_HASHCODE,
                origSize,
                expectedProperties,
                OverwritePolicy.EAGER);
    }
    ...
    public void testOverwritePolicyCautious() throws Exception {
        ...
        assertAsyncMetadataExecute(
                contentMetadataExtracter,
                "quick/quick.cautious_policy_metadata.json",
                UNCHANGED_HASHCODE,
                origSize,
                expectedProperties,
                OverwritePolicy.CAUTIOUS);
    }
    ...
    public void testOverwritePolicyPrudent() throws Exception {
        ...
        transactionService
                ...
                .doInTransaction(
                        () -> {
                            ...
                            properties.put(
                                    audio,
                                    "Default audio"); // will not be changed because it's a media
                            // property
                            nodeService.addProperties(nodeRef, properties);
        ...
        assertAsyncMetadataExecute(
                contentMetadataExtracter,
                "quick/quick.prudent_policy_metadata.json",
                UNCHANGED_HASHCODE,
                origSize,
                expectedProperties,
                OverwritePolicy.PRUDENT);
    }
    ...
    public void testOverwritePolicyPragmatic() throws Exception {
        ...
        transactionService
                ...
                .doInTransaction(
                        () -> {
                            ...
                            properties.put(
                                    audio,
                                    "Default audio"); // will be changed using PRAGMATIC policy
                            nodeService.addProperties(nodeRef, properties);
        ...
        assertAsyncMetadataExecute(
                contentMetadataExtracter,
                "quick/quick.pragmatic_policy_metadata.json",
                UNCHANGED_HASHCODE,
                origSize,
                expectedProperties,
                OverwritePolicy.PRAGMATIC);
    ...
    public void testCarryAspectFalse() throws Exception {
        ...
        assertAsyncMetadataExecute(
                contentMetadataExtracter,
                "quick/quick.carryAspectFalse_metadata.json",
                UNCHANGED_HASHCODE,
                origSize,
                expectedProperties,
                OverwritePolicy.PRAGMATIC);
                ...
                .doInTransaction(
                        () -> {
                            Map<QName, Serializable> properties =
                                    nodeService.getProperties(nodeRef);
                            assertFalse(
                                    "Title should be removed from properties because the extracted"
                                            + " value is null",
    ...
    public void testCarryAspectTrue() throws Exception {
        ...
        assertAsyncMetadataExecute(
                contentMetadataExtracter,
                "quick/quick.carryAspectTrue_metadata.json",
                UNCHANGED_HASHCODE,
                origSize,
                expectedProperties,
                OverwritePolicy.PRAGMATIC);
                ...
                .doInTransaction(
                        () -> {
                            Map<QName, Serializable> properties =
                                    nodeService.getProperties(nodeRef);
                            assertTrue(
                                    "Title should NOT be removed from properties because"
                                            + " carryAspectProperties is true",
                                    properties.containsKey(title));
                            return null;
                        });
    }
    ...
    public void testExtractTagging() throws Exception {
        ...
        assertAsyncMetadataExecute(
                contentMetadataExtracter,
                "quick/quick.tagging_metadata.json",
                UNCHANGED_HASHCODE,
                origSize,
                expectedProperties,
                OverwritePolicy.PRAGMATIC,
        ...
        for (String expectedTag : expectedTags) {
            assertTrue(
                    "Expected tag " + expectedTag + " not in " + actualTags,
    ...
    public void testExtractTaggingWhenDisabled() throws Exception {
        ...
        assertAsyncMetadataExecute(
                contentMetadataExtracter,
                "quick/quick.tagging_metadata_enable_false.json",
                UNCHANGED_HASHCODE,
                origSize,
                expectedProperties,
                OverwritePolicy.PRAGMATIC);
    ...
    public void testStaticMethods() {
        assertTrue(
                ...
                AsynchronousExtractor.isMetadataExtractMimetype("alfresco-metadata-extract"));
        assertTrue(
                "isMetadataEmbedMimetype",
                AsynchronousExtractor.isMetadataEmbedMimetype("alfresco-metadata-embed"));
        assertFalse(
                "isMetadataExtractMimetype",
                AsynchronousExtractor.isMetadataExtractMimetype("alfresco-metadata-embed"));
        assertFalse(
                "isMetadataEmbedMimetype",
                AsynchronousExtractor.isMetadataEmbedMimetype("alfresco-metadata-extract"));

        assertEquals(
                "getTargetMimetypeFromTransformName",
                "alfresco-metadata-extract",
                AsynchronousExtractor.getTargetMimetypeFromTransformName(
                        "alfresco-metadata-extract/text/plain"));
        assertEquals(
                "getTargetMimetypeFromTransformName",
                "alfresco-metadata-embed",
                AsynchronousExtractor.getTargetMimetypeFromTransformName(
                        "alfresco-metadata-embed/text/plain"));
        ...
        assertEquals(
                "getTargetMimetypeFromTransformName",
                "text/plain",
                AsynchronousExtractor.getSourceMimetypeFromTransformName(
                        "alfresco-metadata-extract/text/plain"));
        assertEquals(
                "getTargetMimetypeFromTransformName",
                "text/plain",

...
public abstract class AbstractRenderingEngine
   ...
   */
  private static final QName DEFAULT_DEFAULT_RENDITION_NODE_TYPE =
  ...
  private static final String DEFAULT_MIMETYPE =
    RenditionService2Impl.DEFAULT_MIMETYPE;
  private static final String DEFAULT_ENCODING =
    RenditionService2Impl.DEFAULT_ENCODING;
  ...
  private QName defaultRenditionContentProp =
    DEFAULT_DEFAULT_RENDITION_CONTENT_PROP;

  /**
   * This is the default content property.
   */
  private static final QName DEFAULT_CONTENT_PROPERTY =
    ContentModel.TYPE_CONTENT;
  ...
  public void setRenditionLocationResolver(
    RenditionLocationResolver renditionLocationResolver
  ) {
  ...
  public AbstractRenderingEngine(
    NodeLocator temporaryParentNodeLocator,
  ...
  public void setDefaultRenditionContentProp(String prop) {
    ...
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error(
          "Error when setting default rendition content property: ",
          nx
        );
      }
      throw nx;
  ...
  protected void executeImpl(
    final RenditionDefinition renditionDef,
    ...
    if (!nodeService.exists(sourceNode)) {
      if (logger.isDebugEnabled() == true) {
        logger.debug(
          "Rendition has not been created, because the node no longer exists.  (sourceNode=" +
          sourceNode +
          ")"
        );
      }
      ...
      nodeService.getProperty(sourceNode, ContentModel.PROP_CONTENT) == null
    ) {
      if (logger.isDebugEnabled() == true) {
        logger.debug(
          "Rendition has not been created, because the node has no content to render.  (sourceNode=" +
          sourceNode +
          ")"
        );
      }
    ...
    Serializable runAsParam = renditionDef.getParameterValue(
      ...
      new AuthenticationUtil.RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          ...
          try {
            ...
            result =
              (ChildAssociationRef) renditionDef.getParameterValue(
                PARAM_RESULT
              );
          } catch (Throwable t) {
  ...
  protected void executeRenditionImpl(Action action, NodeRef sourceNode) {
    if (logger.isDebugEnabled()) {
      StringBuilder msg = new StringBuilder();
      msg
        .append("Executing rendering engine; name:")
        .append(this.name)
        .append(", class:")
    ...
    RenditionDefinition renditionDefinition = checkActionIsRenditionDefinition(
      action
    );
    ...
    action
      .getParameterValues()
      .put(PARAM_RESULT, context.getChildAssociationRef());
  }
   ...
   * @return QName
   */
  protected QName getRenditionContentProperty(
    RenditionDefinition renditionDefinition
  ) {
    return getParamWithDefault(
      PARAM_TARGET_CONTENT_PROPERTY,
      defaultRenditionContentProp,
      renditionDefinition
    );
  }
  ...
  protected void checkSourceNodeExists(NodeRef actionedUponNodeRef) {
    if (nodeService.exists(actionedUponNodeRef) == false) {
      String msg =
        "Cannot execute action as node does not exist: " + actionedUponNodeRef;
      logger.warn(msg);
   ...
   * @param action Action
   */
  protected RenditionDefinition checkActionIsRenditionDefinition(
    Action action
  ) {
  ...
  private QName getRenditionNodeType(RenditionDefinition renditionDefinition) {
    return getParamWithDefault(
      ...
      defaultRenditionNodeType,
      renditionDefinition
    );
  }

  @Override
  protected final void addParameterDefinitions(
  ...
  private Serializable getRenditionContentProp(
    RenditionDefinition renditionDefinition
  ) {
    return getParamWithDefault(
      PARAM_TARGET_CONTENT_PROPERTY,
  ...
  public static <T> T getCheckedParam(
    ...
    if (value == null) return null; else {
      if (clazz == null) throw new RenditionServiceException(
  ...
  public static <T> T getParamWithDefault(
    ...
    if (defaultValue == null) throw new RenditionServiceException(
  ...
  protected class RenderingContext
    ...
    public <T> T getCheckedParam(String paramName, Class<T> clazz) {
      return AbstractRenderingEngine.getCheckedParam(
        paramName,
        clazz,
        definition
      );
    ...
    public <T> T getParamWithDefault(String paramName, T defaultValue) {
      return AbstractRenderingEngine.getParamWithDefault(
        paramName,
        defaultValue,
        definition
      );
    ...
    public ContentReader makeContentReader() {
      QName srcContentProp = getParamWithDefault(
        PARAM_SOURCE_CONTENT_PROPERTY,
      ...
      if (contentReader == null || !contentReader.exists()) {
        throw new UnimportantTransformException(
    ...
    public int getIntegerParam(String key, int defaultValue) {
      ...
      if (serializable == null) return defaultValue; else {
  ...
  protected void switchToFinalRenditionNode(
    ...
    ChildAssociationRef tempRendAssoc = (ChildAssociationRef) renditionDef.getParameterValue(
      PARAM_RESULT
    );
    if (logger.isDebugEnabled()) {
      logger.debug("Switching temporary rendition: " + tempRendAssoc);
    }
    ChildAssociationRef result = createOrUpdateRendition(
      actionedUponNodeRef,
      tempRendAssoc,
      renditionDef
    );
  ...
  protected void notifyCallbackOfException(
    RenditionDefinition renditionDefinition,
    Throwable t
  ) {
  ...
  private void setTemporaryRenditionProperties(
    ...
    // created.
    NodeRef parent = temporaryParentNodeLocator.getNode(
      sourceNode,
      definition.getParameterValues()
    );
    ...
    if (logger.isDebugEnabled()) {
      StringBuilder msg = new StringBuilder();
      msg
        .append("Temporary rendition will have parent=")
        .append(parent)
  ...
  private ChildAssociationRef createOrUpdateRendition(
    ...
    RenditionLocation renditionLocation = resolveRenditionLocation(
      sourceNode,
      renditionDefinition,
      tempRenditionNode
    );
  ...
  private void manageRenditionAspects(
    NodeRef sourceNode,
    ChildAssociationRef renditionParentAssoc
  ) {
  ...
  private String getRenditionName(
    ...
    Serializable tempName = nodeService.getProperty(
      tempRenditionNode,
      ContentModel.PROP_NAME
    );
    if (tempName != null) {
  ...
  protected ExecutionSummary getExecutionSummary(
    ...
    List<ExecutionSummary> executionSummaries = actionTrackingService.getExecutingActions(

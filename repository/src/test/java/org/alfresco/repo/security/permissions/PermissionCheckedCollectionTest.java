...
public class PermissionCheckedCollectionTest extends TestCase {
  ...
  public void testBasicWrapping() throws Exception {
    ...
    // Check
    assertTrue(
      "Proxied object must still be a List",
      proxiedList instanceof List
    );
    ...
    assertEquals(
      "cutOff value incorrect",
      isCutOff,
      proxiedPermissionCheckedCollection.isCutOff()
    );
    assertEquals(
  ...
  public void testVolumeWrapping() throws Exception {
    ...
    double average =
      ((double) (after - before) / (double) count) / (double) 1.0E6;

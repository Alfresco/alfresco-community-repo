...
public class ModuleDetailsImplTest extends TestCase {
  ...
  static {
    ...
    DEFAULT_PROPS.setProperty(
      ModuleDetails.PROP_ID,
      "org.alfresco.module.Test"
    );
    ...
    DEFAULT_PROPS.setProperty(
      ModuleDetails.PROP_REPO_VERSION_MIN,
      new VersionNumber("1.2").toString()
    );
    DEFAULT_PROPS.setProperty(
      ModuleDetails.PROP_REPO_VERSION_MAX,
      new VersionNumber("1.4.3").toString()
    );
    ...
    DEFAULT_PROPS.setProperty(
      ModuleDetails.PROP_DEPENDS_PREFIX + "e",
      "* - 1.2"
    );
    DEFAULT_PROPS.setProperty(
      ModuleDetails.PROP_DEPENDS_PREFIX + "f",
      "1.2 - *"
    );
    DEFAULT_PROPS.setProperty(
      ModuleDetails.PROP_DEPENDS_PREFIX + "g",
      "0.5, 0.6"
    );
    DEFAULT_PROPS.setProperty(
      ModuleDetails.PROP_DEPENDS_PREFIX + "h",
      "0.5 - 0.6, 0.9 - *"
    );
    DEFAULT_PROPS.setProperty(
      ModuleDetails.PROP_INSTALL_STATE,
      ModuleInstallState.INSTALLED.toString()
    );
  }
  ...
  public void testWriteAndReadProperties() {
    ...
    assertEquals(
      "The number of properties changed",
      props.size(),
  ...
  public void testDependencyChecks() {
    ...
    for (ModuleDependency dependency : dependencies) {
      if (dependency.getDependencyId().equals(tempDetails.getId())) {
        ...
        assertFalse(
          "No match expected",
          dependency.isValidDependency(tempDetails)
        );
      }
    }
  }
  ...
  public void testTrimming() throws Exception {
    ...
    ModuleDetails details = new ModuleDetailsImpl(props);
    assertEquals(
      "Expected the install state to be UNKNOWN",
      ModuleInstallState.UNKNOWN,
      details.getInstallState()
    );
  }

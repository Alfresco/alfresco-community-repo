...
public class InMemoryTicketComponentImpl implements TicketComponent {
    ...
    public void setOneOff(boolean oneOff) {
        ...
        if (this.oneOff) {
            logger.warn(
                    "The 'oneOff' feature has been deprecated and will be removed in a future"
                            + " version. This feature may not work as intended even in the current"
                            + " version");
        }
    }
    ...
    /**
     * All put operations into ticketsCache and usernameToTicketIdCache should go through this
     * method, so we can debug/trace ticket problems easier from the logs
     */
    private void putIntoCache(Ticket ticket) {
        if (logger.isTraceEnabled()) {
            logger.trace(
                    "Putting into ticketsCache " + ticketsCache.toString() + " ticket: " + ticket);
        }
        ticketsCache.put(ticket.getTicketId(), ticket);

        if (logger.isTraceEnabled()) {
            logger.trace(
                    "Putting into usernameToTicketIdCache "
                            + usernameToTicketIdCache.toString()
                            + " username and ticketId of: "
                            + ticket);
        }
    ...
    /**
     * All remove operations from ticketsCache and usernameToTicketIdCache should go through this
     * method, so we can debug/trace ticket problems easier from the logs
     */
    ...
    public String getNewTicket(String userName) throws AuthenticationException {
        ...
        if (ticket == null) {
            ...
            ticket =
                    new Ticket(
                            ticketsExpire ? expiryMode : ExpiryMode.DO_NOT_EXPIRE,
                            expiryDate,
                            userName,
                            validDuration);
            putIntoCache(ticket);
        ...
        if (logger.isTraceEnabled()) {
            logger.trace(
                    "Setting the current ticket for this thread: "
    ...
    public int invalidateTickets(boolean expiredOnly) {
        ...
        if (!expiredOnly) {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace(
                        "Clearing all tickets from the ticketsCache, that used to have size: "
                                + count);
            }
            ticketsCache.clear();
    ...
    public void invalidateTicketByUser(String userName) {
        ...
        for (String key : ticketsCache.getKeys()) {
            Ticket ticket = ticketsCache.get(key);
            // Hack: The getKeys() call might return keys for null marker objects, yielding null
            // values
            if (ticket == null) {
                continue;
            }
    ...
    public static class Ticket implements Serializable {
        ...
        private static String computeTicketId(
                ExpiryMode expires, Date expiryDate, String userName, String guid) {
            String encode =
                    expires.toString() + getExpireDateAsString(expiryDate) + userName + guid;
        ...
        private Ticket(
            ...
            if (logger.isTraceEnabled()) {
                logger.trace(
                        "Creating (cloning) new ticket for user: "
                                + AuthenticationUtil.maskUsername(userName)
                                + " with ticketId: "
                                + this.ticketId);
            }
        ...
        private static void checkValidTicketParameters(
                ExpiryMode expires, Date expiryDate, String userName, Duration validDuration) {
                ...
                case AFTER_INACTIVITY:
                    ...
                    if (hasExpired(now)) {
                        ...
                        if (remaining.compareTo(testDuration) < 0) {
                            if (logger.isTraceEnabled()) {
                                logger.trace(
                                        "AFTER_INACTIVITY case, Creating new ticket based on the"
                                                + " current one that expires at: "
                                                + expiryDate);
                            }
                            return new Ticket(
                                    expires,
                                    Duration.add(now, validDuration),
                                    userName,
                                    validDuration,
                                    ticketId);
                        } else {
        ...
        public boolean equals(Object o) {
            ...
            return this.ticketId.equals(t.ticketId)
                    ...
                    && this.expires.equals(t.expires)
                    && areTheExpiryDatesEqual(t);
        }
        ...
        private boolean areTheExpiryDatesEqual(Ticket t) {
            ...
            // in this case this.expiryDate should not ever be null; There are checks in the
            // constructors that validate this;
    ...
    public static void clearCurrentSecurityContext() {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace(
                    "Clearing the current ticket for this thread: "
                            + Thread.currentThread().getName()
                            + " . Previous ticket was: "
                            + prevTicket);
        }

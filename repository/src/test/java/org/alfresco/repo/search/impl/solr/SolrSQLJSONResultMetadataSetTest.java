...
public class SolrSQLJSONResultMetadataSetTest {
  ...
  public void parseSQLErrorResponse() throws JSONException {
    String response =
      "{\"result-set\":{\"docs\":[{\"EXCEPTION\":\"Column 'SIT1E' not found in any table\",\"EOF\":true,\"RESPONSE_TIME\":18943}]}}";
    try {
      JSONObject json = new JSONObject(response);
      SolrSQLJSONResultSet ssjr = new SolrSQLJSONResultSet(json, null);
      ...
      Assert.assertEquals(
        "Unable to execute the query, error caused by: Column 'SIT1E' not found in any table",
        e.getMessage()
      );
    }
  }
  ...
  public void parseInvalidInsightEngineResponse() throws JSONException {
    String response =
      "{\"result-set\":{\"docs\":[{\"EXCEPTION\":\"/sql handler only works in Solr Cloud mode\",\"EOF\":true}]}}";
    try {
      JSONObject json = new JSONObject(response);
      SolrSQLJSONResultSet ssjr = new SolrSQLJSONResultSet(json, null);
      Assert.assertNull(ssjr);

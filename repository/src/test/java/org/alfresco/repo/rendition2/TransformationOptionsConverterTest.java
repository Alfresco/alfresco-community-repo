...
public class TransformationOptionsConverterTest {
  ...
  private static final String TO_STRING_OF_UNSET_TRANSFORMATION_OPTIONS =
    "{maxSourceSizeKBytes=-1, pageLimit=-1, use=null, timeoutMs=-1, " +
  ...
  private void assertConverterToMapAndBack(
    ...
    String sortedOldOptions = getSortedOptions(
      oldOptions,
      sourceMimetype,
      targetMimetype
    );
    ...
    Map<String, String> newOptions = converter.getOptions(
      oldOptions,
      sourceMimetype,
      targetMimetype
    );
    Map<String, String> newOptionsWithoutDiscards = discardNoopOptions(
      newOptions
    );
    String sortedNewOptions = getSortedOptions(newOptionsWithoutDiscards);
    assertEquals(
      "Conversion to a map appears to be wrong",
      expectedArgs,
      sortedNewOptions
    );

    TransformationOptions backToOldOptions = converter.getTransformationOptions(
      "null",
      newOptions
    );
    String sortedBackToOldOptions = getSortedOptions(
      backToOldOptions,
      sourceMimetype,
      targetMimetype
    );
  ...
  private String getSortedOptions(
    ...
    Map<String, String> map = converter.getOptions(
      options,
      sourceMimetype,
      targetMimetype
    );
  ...
  public static Map<String, String> discardNoopOptions(
  ...
  public void testRenditionImagepreview() {
    ...
    TransformationOptions oldOptions = converter.getTransformationOptions(
      "null",
      newOptions
    );
    String sortedOldOptions = getSortedOptions(
    ...
    if (sortedOldOptions.endsWith("timeout=-1 ")) {
      sortedOldOptions =
        sortedOldOptions.substring(
          0,
          sortedOldOptions.length() - "timeout=-1 ".length()
        );
    }
  ...
  public void testCommandOptionsFromOldOptions() {
    ImageTransformationOptions oldOptions = new ImageTransformationOptions();
    oldOptions.setCommandOptions(
      "-resize 350x50> -background none -gravity center"
    );

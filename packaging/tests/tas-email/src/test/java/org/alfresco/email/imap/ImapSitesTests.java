...
public class ImapSitesTests extends EmailTest {
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify site cannot be accessed in IMAP client if it is no longer an IMAP site"
                            + " or marked as IMAP favorite")
    @Test(
            groups = {TestGroup.PROTOCOLS, TestGroup.IMAP, TestGroup.FULL},
            expectedExceptions = FolderNotFoundException.class)
    public void userCannotAccessSiteThatIsNoLongerAnImapSiteOrMarkedAsImapFavorite()
            throws Exception {
        testFolder = dataContent.usingUser(testUser).usingSite(testSite).createFolder();
        imapProtocol
                .authenticateUser(testUser)

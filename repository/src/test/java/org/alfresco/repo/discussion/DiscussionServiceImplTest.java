...
public class DiscussionServiceImplTest {
  ...
  public static void initTestsContext() throws Exception {
    ...
    DISCUSSION_SERVICE =
      (DiscussionService) testContext.getBean("DiscussionService");
    ...
    TENANT_ADMIN_SERVICE =
      testContext.getBean("tenantAdminService", TenantAdminService.class);
  ...
  public void createNewTopic() throws Exception {
    ...
    PagingResults<TopicInfo> results = DISCUSSION_SERVICE.listTopics(
    ...
    results =
      DISCUSSION_SERVICE.listTopics(FORUM_NODE, true, new PagingRequest(10));
    assertEquals(0, results.getPage().size());

    // Get with an arbitrary name gives nothing
    siteTopic =
      DISCUSSION_SERVICE.getTopic(DISCUSSION_SITE.getShortName(), "madeUp");
    assertEquals(null, siteTopic);

    siteTopic =
      DISCUSSION_SERVICE.getTopic(DISCUSSION_SITE.getShortName(), "madeUp2");
    assertEquals(null, siteTopic);
    ...
    NodeRef container = PUBLIC_NODE_SERVICE
      .getPrimaryParent(siteTopic.getNodeRef())
      .getParentRef();
    ...
    assertEquals(
      FORUM_NODE,
      PUBLIC_NODE_SERVICE
        .getPrimaryParent(nodeTopic.getNodeRef())
        .getParentRef()
    );
    ...
    // Fetch and re-check
    nodeTopic =
      DISCUSSION_SERVICE.getTopic(FORUM_NODE, nodeTopic.getSystemName());
    assertEquals("Node Title", nodeTopic.getTitle());
    assertEquals(ADMIN_USER, nodeTopic.getCreator());
    assertEquals(0, siteTopic.getTags().size());
  ...
  public void createNewTopicAndPostAndReply() throws Exception {
    ...
    for (TopicInfo topic : new TopicInfo[] { siteTopic, nodeTopic }) {
      ...
      PagingResults<PostInfo> posts = DISCUSSION_SERVICE.listPosts(
        topic,
        new PagingRequest(10)
      );
      ...
      assertEquals(
        post.getNodeRef(),
        DISCUSSION_SERVICE.getPrimaryPost(topic).getNodeRef()
      );
      ...
      assertEquals(
        post.getNodeRef(),
        DISCUSSION_SERVICE.getMostRecentPost(topic).getNodeRef()
      );
  ...
  public void createUpdateDeleteEntries() throws Exception {
    ...
    siteTopic2 =
      DISCUSSION_SERVICE.getForNodeRef(siteTopic.getNodeRef()).getFirst();
  ...
  public void basicListings() throws Exception {
    ...
    TopicInfo siteT1 = DISCUSSION_SERVICE.createTopic(
      DISCUSSION_SITE.getShortName(),
      "ST1"
    );
    AuthenticationUtil.setFullyAuthenticatedUser(TEST_USER);
    TenantContextHolder.setTenantDomain(TENANT_DOMAIN);
    TopicInfo siteT2 = DISCUSSION_SERVICE.createTopic(
      DISCUSSION_SITE.getShortName(),
      "ST2"
    );
    ...
    topics =
      DISCUSSION_SERVICE.listTopics(FORUM_NODE, false, new PagingRequest(10));
    assertEquals(3, topics.getPage().size());
  ...
  public void replyListing() throws Exception {
    ...
    for (TopicInfo topic : new TopicInfo[] { siteT1, nodeT1 }) {
      ...
      PostInfo reply1_3_1_1 = DISCUSSION_SERVICE.createReply(
        reply1_3_1,
        "R1_3_1_1"
      );
      PostInfo reply1_3_1_1_1 = DISCUSSION_SERVICE.createReply(
        reply1_3_1_1,
        "R1_3_1_1_1"
      );
      ...
      assertEquals(
        reply1_2.getNodeRef(),
        pr.getReplies().get(0).getReplies().get(1).getNodeRef()
      );
      assertEquals(
        reply1_3.getNodeRef(),
        pr.getReplies().get(0).getReplies().get(2).getNodeRef()
      );
      assertEquals(
        0,
        pr.getReplies().get(0).getReplies().get(0).getReplies().size()
      );
      assertEquals(
        0,
        pr.getReplies().get(0).getReplies().get(1).getReplies().size()
      );
      assertEquals(
        0,
        pr.getReplies().get(0).getReplies().get(2).getReplies().size()
      );
      ...
      assertEquals(
        0,
        pr1.getReplies().get(1).getReplies().get(0).getReplies().size()
      );
      assertEquals(
        0,
        pr1.getReplies().get(2).getReplies().get(0).getReplies().size()
      );
      ...
      assertEquals(
        reply1_3_1_1.getNodeRef(),
        rp1_3_1.getReplies().get(0).getNodeRef()
      );
      ...
      assertEquals(
        0,
        rp1_3_1.getReplies().get(0).getReplies().get(0).getReplies().size()
      );
      ...
      assertEquals(
        reply1_3_1_1.getNodeRef(),
        pr.getReplies().get(0).getNodeRef()
      );
  ...
  public void tagging() throws Exception {
    ...
    topic =
      DISCUSSION_SERVICE.createTopic(DISCUSSION_SITE.getShortName(), "Title");
  ...
  public void discussionsHotTopicsListing() throws Exception {
    ...
    assertEquals(
      topicSB.getTitle(),
      topics.getPage().get(0).getFirst().getTitle()
    );
    assertEquals(3, topics.getPage().get(0).getSecond().intValue());
    assertEquals(
      topicSA.getTitle(),
      topics.getPage().get(1).getFirst().getTitle()
    );
    ...
    PostInfo replySA1_1 = DISCUSSION_SERVICE.createReply(
      replySA1,
      "Reply to a reply"
    );
    PostInfo replyNA1_1 = DISCUSSION_SERVICE.createReply(
      replyNA1,
      "Reply to a reply"
    );
  ...
  public void discussionsListingPermissionsChecking() throws Exception {
    ...
    assertEquals(
      true,
      SITE_SERVICE.isMember(DISCUSSION_SITE.getShortName(), TEST_USER)
    );
    assertEquals(
      false,
      SITE_SERVICE.isMember(ALTERNATE_DISCUSSION_SITE.getShortName(), TEST_USER)
    );
  ...
  public void alf_9460() {
    ...
    for (TopicInfo topic : new TopicInfo[] { siteTopic, nodeTopic }) {
      ...
      List<AssociationRef> assocs = PUBLIC_NODE_SERVICE.getTargetAssocs(
        reply.getNodeRef(),
  ...
  private void pushAuditableDatesBack(
    ...
    final Date created = (Date) PUBLIC_NODE_SERVICE.getProperty(
      node,
      ContentModel.PROP_CREATED
    );
    final Date newCreated = new Date(
      created.getTime() - createdDaysAgo * ONE_DAY_MS
    );
    final Date modified = (Date) PUBLIC_NODE_SERVICE.getProperty(
      node,
      ContentModel.PROP_MODIFIED
    );
    final Date newModified = new Date(
      modified.getTime() - modifiedDaysAgo * ONE_DAY_MS
    );
  ...
  private static void createTestSites() throws Exception {
    final DiscussionServiceImpl privateDiscussionService = (DiscussionServiceImpl) testContext.getBean(
      "discussionService"
    );
    ...
    TRANSACTION_HELPER.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
        ...
        public Void execute() throws Throwable {
          ...
          {
            BehaviourFilter behaviourFilter = (BehaviourFilter) testContext.getBean(

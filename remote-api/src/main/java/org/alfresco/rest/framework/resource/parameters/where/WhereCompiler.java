...
public class WhereCompiler {
  ...
  public static CommonTree compileWhereClause(String expression)
  ...
  public static CommonTree compileSelectClause(String selectParam)
    throws RecognitionException {
    //lexer splits input into tokens
  ...
  private static void print(CommonTree tree, int level) {
    //indent level
    for (int i = 0; i < level; i++) logger.debug("--");

    //print node description: type code followed by token text
    logger.debug(" " + tree.getType() + " " + tree.getText());

    //print all children
    if (tree.getChildren() != null) for (Object ie : tree.getChildren()) {

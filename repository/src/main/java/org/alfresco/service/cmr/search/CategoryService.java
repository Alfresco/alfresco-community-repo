 ...
 * Category Service
 *
 * <p>The service for querying and creating categories. All other management can be carried out
 * using the node service.
 *
 * <p>Classification - the groupings of categories. There is a one-to-one mapping with aspects. For
 * example, Region. Root Category - the top level categories in a classification. For example,
 * Northern Europe Category - any other category below a root category
 *
 * @author Andy Hind
 */
@AlfrescoPublicApi
public interface CategoryService {
     ...
     *
     * <p>MEMBERS - get only category members (the things that have been classified in a category,
     * not the sub categories) SUB_CATEGORIES - get sub categories only, not the things that hyave
     * been classified. ALL - get both of the above
     */
    public enum Mode {
        MEMBERS,
        SUB_CATEGORIES,
     ...
     *
     * <p>IMMEDIATE - only immediate sub categories or members ANY - find subcategories or members
     * at any level
     */
    public enum Depth {
        IMMEDIATE,
    ...
    /**
     * Get a list of all the categories appropriate for a given property. The full list of
     * categories that may be assigned for this aspect.
     *
    ...
    PagingResults<ChildAssociationRef> getRootCategories(
    ...
    /**
     * Looks up a category by name under its immediate parent. Index-independent so can be used for
     ...
     * Index-independent so can be used for cluster-safe existence checks.
     *
     * @param storeRef the store ref
     * @param aspectName the aspect name
     * @param name the aspect name
     * @param create should a category node be created if one does not exist?
     ...
     *
     * <p>This will extend the category types in the data dictionary All it needs is the type name
     * and the attribute in which to store noderefs to categories.
     *
     * @param storeRef StoreRef
     * @param aspectName QName

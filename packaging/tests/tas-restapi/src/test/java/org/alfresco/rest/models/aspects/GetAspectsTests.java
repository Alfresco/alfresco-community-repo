...
public class GetAspectsTests extends RestTest {
    ...
    public void getAspects() throws Exception {
        RestAspectsCollection aspects =
                restClient.authenticateUser(regularUser).withModelAPI().getAspects();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        aspects.assertThat()
                .entriesListCountIs(100)
                .and()
                .entriesListContains("id", "cm:classifiable")
                .and()
                .entriesListContains("id", "cm:author")
                .and()
                .entriesListContains("id", "cm:checkedOut");
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Should filter aspects using modelId with subaspects and gets status code OK"
                            + " (200)")
    public void getAspectByModelsIdsWithIncludeSubAspects() throws Exception {
        RestAspectsCollection aspects =
                restClient
                        ...
                        .withModelAPI()
                        .usingParams(
                                "where=(modelId in ('cm:contentmodel INCLUDESUBASPECTS',"
                                        + " 'smf:smartFolder INCLUDESUBASPECTS'))")
                        .getAspects();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        aspects.getPagination()
                .assertThat()
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Should Aspects association, properties and mandatory aspects and gets status"
                            + " code OK (200)")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify if any user gets aspects with high skipCount and maxItems parameter"
                            + " applied")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify if any user gets aspects with hasMoreItems applied bases on skip count"
                            + " and maxItems")

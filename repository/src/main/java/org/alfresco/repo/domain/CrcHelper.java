...
public class CrcHelper {
    ...
    /**
     * Calculate a persistable, unique pair of values that can be persisted in a database unique key
     * and guarantee correct case-sensitivity.
     *
     * <p>While the short-string version of the value is always lowercase, the CRC is calculated
     * from the virgin string if case-sensitivity is enforced; in the case-insensitive case, the CRC
     * is calculated from a lowercase version of the string.
     *
     * <p>If the value is an empty string, then {@link #EMPTY_STRING} is used instead. This ensures
     ...
     * @param useCharsFromStart <tt>true</tt> if the shortened string value must be made from the
     *     first characters of the string or <tt>false</tt> to use characters from the end of the
     *     string.
     ...
     * @return Return the persistable pair. The result will never be <tt>null</tt>, but the
     *     individual pair values will be <tt>null</tt> if the value given is <tt>null</tt>
     */
    public static Pair<String, Long> getStringCrcPair(
            String value, int dataLength, boolean useCharsFromStart, boolean caseSensitive) {

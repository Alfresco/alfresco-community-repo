 ...
 *
 * <p>N.B. {@link RenditionDefinition Rendition definitions} are stored within the Data Dictionary
 * in the Alfresco Repository &amp; therefore calls to load or save definitions will be subject to
 * the normal authorisation checks for those nodes. In particular this means that if the Data
 * Dictionary has been given restricted access control (it is Consumer for Group ALL by default), it
...
public class RenditionDefinitionPersisterImpl implements RenditionDefinitionPersister {
    private static final Log log = LogFactory.getLog(RenditionDefinitionPersisterImpl.class);

    /** Reference to the rendering action space node */
    private static final StoreRef SPACES_STORE =
            new StoreRef(StoreRef.PROTOCOL_WORKSPACE, "SpacesStore");

    protected static final NodeRef RENDERING_ACTION_ROOT_NODE_REF =
            new NodeRef(SPACES_STORE, "rendering_actions_space");
    ...
    public List<RenditionDefinition> loadRenditionDefinitions() {
        ...
        List<ChildAssociationRef> childAssocs =
                nodeService.getChildAssocs(RENDERING_ACTION_ROOT_NODE_REF, actionTypes);

        List<RenditionDefinition> renderingActions =
                new ArrayList<RenditionDefinition>(childAssocs.size());
    ...
    public List<RenditionDefinition> loadRenditionDefinitions(String renditionEngineName) {
        ...
        List<RenditionDefinition> filteredRenditionDefinitions =
                new ArrayList<RenditionDefinition>();
    ...
    public void saveRenditionDefinition(RenditionDefinition renderingAction) {
        ...
        // ALF-9166 describes a problem whereby versionable saved rendition definition nodes cause
        // problems on upgrade.
        // This appears to be due to a rule defined on Company Home. The behaviour suppression below
    ...
    private NodeRef findActionNode(QName renditionDefinitionName) {
        ...
        if (childAssocs.isEmpty()) {
            ...
            if (childAssocs.size() > 1) {
                log.warn(
                        "Multiple rendition definitions with the name: "
                                + renditionDefinitionName
                                + " exist. Taking the first ones.");
            }

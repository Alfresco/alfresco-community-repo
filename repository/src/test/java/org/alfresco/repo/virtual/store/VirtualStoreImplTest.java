...
public class VirtualStoreImplTest extends VirtualizationIntegrationTest {
  ...
  public void testResolveVirtualFolderDefinition_inactiveSynchronization()
    ...
    try {
      ...
      if (jsonTemplateContent == null) {
        ChildAssociationRef templateChild = createContent(
          companyHomeNodeRef,
          templateName,
      ...
      if (ntVirtualizedFolder == null) {
        ChildAssociationRef folderChild = createFolder(
          companyHomeNodeRef,
          folderName
        );
      ...
      Reference aVanillaRef =
        ((VanillaProtocol) Protocols.VANILLA.protocol).newReference(
  ...
  public void testNonVirtualizable() throws Exception {
    NodeRef aNodeRef = createVirtualizedFolder(
  ...
  public void testCanVirtualize() throws Exception {
    NodeRef solrFacetsNodeRef = new NodeRef(
      "workspace://SpacesStore/solr_facets_root_space"
    );
  ...
  public void testCanCreateFolderNamedV() throws Exception {
    ...
    Reference reference = Reference.fromNodeRef(
      new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, v)
    );
  ...
  private void assertHasQueryNodePermission(
  ...
  private void assertHasVirtualNodePermission(
    ...
    assertEquals(
      AccessStatus.DENIED,
      virtualUserPermissions.hasVirtualNodePermission(
        asTypedPermission(perm),
        readonly
      )
    );
  }
  ...
  public void testCanExistNodeIDwithV() {
    ...
    createAndCheckNodeId(
      "v" + Encodings.ZERO.encoding.token + "file",
      "specialFile3.txt"
    );

    // vH...
    createAndCheckNodeId(
      "v" + Encodings.HASH.encoding.token + "file",
      "specialFile4.txt"
    );

    // vp...
    createAndCheckNodeId(
      "v" + Encodings.PLAIN.encoding.token + "file",
      "specialFile5.txt"
    );

    // MNT-21968
    createAndCheckNodeId(
      "v" + Encodings.ZERO.encoding.token + "0Draft.pdf",
      "specialFile6.txt"
    );
  ...
  private void createAndCheckNodeId(String nodeId, String nodeName) {
    ...
    NodeRef nfileNodeRef = nodeService
      .createNode(
        companyHomeNodeRef,
        ContentModel.ASSOC_CONTAINS,
        assocQName,

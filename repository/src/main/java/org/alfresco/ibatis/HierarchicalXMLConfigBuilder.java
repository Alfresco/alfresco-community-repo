...
public class HierarchicalXMLConfigBuilder extends BaseBuilder {
  ...
  private void parseConfiguration(XNode root) {
    try {
      ...
      throw new BuilderException(
        "Error parsing SQL Mapper Configuration. Cause: " + e,
        e
      );
    }
  }
      ...
      for (XNode child : parent.getChildren()) {
        if ("package".equals(child.getName())) {
          ...
          try {
            ...
            throw new BuilderException(
              "Error registering typeAlias for '" + alias + "'. Cause: " + e,
              e
            );
          }
        }
  ...
  private void objectWrapperFactoryElement(XNode context) throws Exception {
    if (context != null) {
      String type = context.getStringAttribute("type");
      ObjectWrapperFactory factory = (ObjectWrapperFactory) resolveClass(type)
        .newInstance();
  ...
  private void reflectionFactoryElement(XNode context) throws Exception {
    if (context != null) {
      String type = context.getStringAttribute("type");
      ReflectorFactory factory = (ReflectorFactory) resolveClass(type)
        .newInstance();
  ...
  private void databaseIdProviderElement(XNode context) throws Exception {
    ...
    if (context != null) {
      ...
      Properties properties = context.getChildrenAsProperties();
      databaseIdProvider =
        (DatabaseIdProvider) resolveClass(type).newInstance();
      databaseIdProvider.setProperties(properties);
    }
    Environment environment = configuration.getEnvironment();
    if (environment != null && databaseIdProvider != null) {
      String databaseId = databaseIdProvider.getDatabaseId(

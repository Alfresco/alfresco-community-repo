...
/**
 * An {@link AspectVirtualizationMethod} that uses an aspect defined String property that holds the
 * system path of a template resource.<br>
 * System paths are custom string reference of a resource that can be located either in the
 * repository or in the java classpath - <i>system paths are deprecated and they will be replaced by
 * {@link Encodings#PLAIN} encoded {@link Reference} strings</i>.<br>
 *
 * @author Bogdan Horje
 */
public class SystemVirtualizationMethod extends AspectVirtualizationMethod {
    ...
    /**
     * String name of template system path holding property. Will be converted into a {@link QName}
     * during {@link #init()}
     */
    ...
    public void init() {
        ...
        if (systemPathPropertyName != null) {
            systemPathPropertyQName =
                    QName.createQName(systemPathPropertyName, namespacePrefixResolver);
        }
    }
    ...
    public Reference virtualize(ActualEnvironment env, NodeRef nodeRef)
        ...
        String templateSystemPath = (String) env.getProperty(nodeRef, systemPathPropertyQName);
        if (templateSystemPath != null) {
            return newVirtualReference(env, nodeRef, templateSystemPath);
        } else {
            // default branch - invalid virtual node
            throw new VirtualizationException(
                    "Invalid virtualization : missing template system-path.");
        }
    }

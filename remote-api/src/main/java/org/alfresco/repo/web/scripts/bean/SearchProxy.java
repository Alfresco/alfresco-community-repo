...
public class SearchProxy extends AbstractWebScript implements InitializingBean {
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    searchConfig =
      (OpenSearchConfigElement) config.getConfigElement(
        OpenSearchConfigElement.CONFIG_ELEMENT_ID
      );
  ...
  public void execute(WebScriptRequest req, WebScriptResponse res)
    ...
    if (extensionPaths.length != 2) {
      throw new WebScriptException(
        "OpenSearch engine has not been specified as /{engine}/{format}"
      );
    }
    ...
    if (engineConfig == null) {
      throw new WebScriptException(
        "OpenSearch engine '" + engine + "' does not exist"
      );
    }
    ...
    if (mimetype == null) {
      throw new WebScriptException(
        "Format '" + format + "' does not map to a registered mimetype"
      );
    }
    ...
    if (!(res.getRuntime() instanceof WebScriptServletRuntime)) {
      throw new WebScriptException(
        "Search Proxy must be executed in HTTP Servlet environment"
      );
    }

    HttpServletResponse servletRes = WebScriptServletRuntime.getHttpServletResponse(
      res
    );
    SearchEngineHttpProxy proxy = new SearchEngineHttpProxy(
      req.getServerPath() + req.getServiceContextPath(),
      engine,
      engineUrl,
      servletRes,
      Collections.singletonMap("User-Agent", req.getHeader("User-Agent"))
    );
    proxy.service();
  }
  ...
  private class SearchEngineHttpProxy extends HTTPProxy {
    ...
    private static final String ATOM_LINK_XPATH =
      "atom:link[@rel=\"first\" or @rel=\"last\" or @rel=\"next\" or @rel=\"previous\" or @rel=\"self\" or @rel=\"alternate\"]";
    ...
    public SearchEngineHttpProxy(
      ...
      super(
        engineUrl.startsWith("/") ? rootPath + engineUrl : engineUrl,
        response
      );
      this.engine = engine;
    ...
    protected void writeResponse(InputStream input, OutputStream output)
      throws IOException {
      if (
        response.getContentType().startsWith(MimetypeMap.MIMETYPE_ATOM) ||
  ...
  public String createUrl(
    OpenSearchConfigElement.EngineConfig engine,
    String mimetype
  ) {
    ...
    if (url != null) {
      ...
      if (
        proxy != null &&
        !mimetype.equals(MimetypeMap.MIMETYPE_OPENSEARCH_DESCRIPTION)
      ) {
  ...
  public String createUrl(String engine, String url, String mimetype) {
    ...
    if (format == null) {
      throw new WebScriptException(
        "Mimetype '" + mimetype + "' is not registered."
      );
    }
    ...
    if (argIdx == -1) {
      ...
      proxyUrl =
        proxyPath + "/" + engine + "/" + format + url.substring(argIdx);
    }
    return proxyUrl;
  }
}

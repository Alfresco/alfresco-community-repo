...
/**
 * Tests for REST API call DELETE "/deployments/{deploymentId}" Class has priority 100 in order to be executed last in the list of tests classes
 * 
...
public class DeleteDeploymentTests extends RestTest
{
    ...
    public void adminDeletesDeploymentWithSuccess() throws Exception
    {
        ...
        RestProcessDefinitionModel processDefinitionAssociated = restClient.withWorkflowAPI().getAllProcessDefinitions().getProcessDefinitionById(deployment.getId());

        RestProcessModel addedProcess = restClient.withWorkflowAPI().addProcess(processDefinitionAssociated.getName(), adminUser, false, CMISUtil.Priority.Normal);

        List<RestTaskModel> processTasks = restClient.withWorkflowAPI().usingProcess(addedProcess).getProcessTasks().getEntries();

 ...
 *
 * <p>If {@link #setEagerOrphanCleanup(boolean) eager cleanup} is activated, then this component
 * listens to all content property change events and recorded for post-transaction processing. All
 * orphaned content is deleted from the registered store(s). Note that any {@link
 * #setListeners(List) listeners} are called as normal; backup or scrubbing procedures should be
 * plugged in as listeners if this is required.
 *
 * <p><u><b>Lazy cleanup:</b></u>
 *
 * <p>This is triggered by means of a {@link ContentStoreCleanupJob Quartz job}. This is a
 * heavy-weight process that effectively compares the database metadata with the content URLs
 * controlled by the various stores. Once again, the listeners are called appropriately.
 *
...
public class EagerContentStoreCleaner extends TransactionListenerAdapter {
     ...
     * @see #afterCommit()
     */
    private static final String KEY_POST_COMMIT_DELETION_URLS =
            "ContentStoreCleaner.PostCommitDeletionUrls";
    /**
     * Content URLs to delete if the transaction rolls back.
     *
     * @see #afterRollback()
     */
    private static final String KEY_POST_ROLLBACK_DELETION_URLS =
            "ContentStoreCleaner.PostRollbackDeletionUrls";
    ...
    /**
     * Initializes the cleaner based on the {@link #setEagerOrphanCleanup(boolean) eagerCleanup}
     * flag.
     */
    public void init() {
        checkProperties();
    ...
    public void registerNewContentUrl(String contentUrl) {
        Set<String> urlsToDelete =
                TransactionalResourceHelper.getSet(KEY_POST_ROLLBACK_DELETION_URLS);
     ...
     * Queues orphaned content for post-transaction removal
     *
     * <p><b>NB: </b>Any content registered <u>will</u> be deleted if the current transaction
     * commits and if 'eager' cleanup is turned on.
     *
     * <p>Note that listeners are not called for this process.
     *
     * @return Returns <tt>true</tt> if the content was scheduled for post-transaction deletion. If
     *     the return value is <tt>true</tt> then the calling code <b>must</b> delete the row entry
     *     for the content URL provided <b>BEFORE THE TRANSACTION COMMITS!</b>
     */
     ...
     * <p>Note that listeners are not called for this process.
     *
     * @param force <tt>true</tt> for force the post-commit URL deletion regardless of the setting
     *     {@link #setEagerOrphanCleanup(boolean)}.
     * @return Returns <tt>true</tt> if the content was scheduled for post-transaction deletion. If
     *     the return value is <tt>true</tt> then the calling code <b>must</b> delete the row entry
     *     for the content URL provided <b>BEFORE THE TRANSACTION COMMITS!</b>
     */
    public boolean registerOrphanedContentUrl(String contentUrl, boolean force) {
        ...
        Set<String> urlsToDelete =
                TransactionalResourceHelper.getSet(KEY_POST_COMMIT_DELETION_URLS);
        urlsToDelete.add(contentUrl);
     ...
     *
     * <p>Note that listeners <b>are</b> called for this process.
     *

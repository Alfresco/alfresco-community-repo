...
public class FileFolderLoader {
  ...
  public int createFiles(
    ...
    if (
      descriptionCount < 0 || descriptionCount > Locale.getISOLanguages().length
    ) {
      throw new IllegalArgumentException(
    ...
    RetryingTransactionCallback<NodeRef> findFolderWork = new RetryingTransactionCallback<NodeRef>() {
      ...
      public NodeRef execute() throws Throwable {
        ...
        List<String> folderPathElements = Arrays.asList(
          folderPathFixed.substring(1).split("/")
        );
        FileInfo folderInfo = fileFolderService.resolveNamePath(
          companyHomeNodeRef,
          folderPathElements,
          true
        );
    ...
    NodeRef folderNodeRef = txnHelper.doInTransaction(
      findFolderWork,
      false,
      true
    );
          ...
          if (descriptionCount > 0) {
            ...
            try {
              ...
              for (
                ...
                String descriptionUrl = SpoofedTextContentReader.createContentUrl(
                  locale,
                  seed + descriptionNum,
                  descriptionSize
                );

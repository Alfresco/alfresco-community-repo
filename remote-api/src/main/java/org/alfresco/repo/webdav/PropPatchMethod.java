...
public class PropPatchMethod extends PropFindMethod {
      ...
      for (int i = 0; i < childList.getLength(); i++) {
        ...
        switch (currentNode.getNodeType()) {
          ...
          case Node.ELEMENT_NODE:
            if (
              currentNode.getNodeName().endsWith(WebDAV.XML_SET) ||
              ...
              for (int j = 0; j < propertiesList.getLength(); j++) {
                ...
                switch (propertiesNode.getNodeType()) {
                  ...
                  case Node.ELEMENT_NODE:
                    if (
                      propertiesNode.getNodeName().endsWith(WebDAV.XML_PROP)
                    ) {
                      ...
                      for (int k = 0; k < propList.getLength(); k++) {
                        ...
                        switch (propNode.getNodeType()) {
                          case Node.TEXT_NODE:
                            break;
                          case Node.ELEMENT_NODE:
                            int action = currentNode
                                .getNodeName()
                                .endsWith(WebDAV.XML_SET)
                              ? PropertyAction.SET
                              : PropertyAction.REMOVE;
                            m_propertyActions.add(
                              new PropertyAction(
                                action,
                                createProperty(propNode)
                              )
                            );
                            break;
                        }
  ...
  protected void patchProperties(FileInfo nodeInfo, String path)
    ...
    for (PropertyAction propertyAction : m_propertyActions) {
      ...
      if (failedProperty == null) {
        PropertyDefinition propDef = getDAVHelper()
          .getDictionaryService()
          .getProperty(property.createQName());

        boolean deadProperty =
          propDef == null ||
          (
            !propDef.getContainerClass().isAspect() &&
            !getDAVHelper()
              .getDictionaryService()
              .isSubClass(
                getNodeService().getType(nodeInfo.getNodeRef()),
        ...
        if (PropertyAction.SET == propertyAction.getAction()) {
          ...
          if (deadProperty) {
            ...
            getNodeService()
              .removeProperty(nodeInfo.getNodeRef(), property.createQName());
          }
        } else {
   ...
   */
  protected void generatePropertyResponse(
    XMLWriter xml,
  ...
  protected void generateError(XMLWriter xml) {
    try {
      ...
      xml.write(
        DocumentHelper.createElement(
          WebDAV.XML_NS_CANNOT_MODIFY_PROTECTED_PROPERTY
        )
      );

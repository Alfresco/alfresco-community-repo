...
public class DefaultEncryptionUtils implements EncryptionUtils {
    ...
    /** {@inheritDoc} */
    @Override
    public void setRequestAlgorithmParameters(HttpMethod method, AlgorithmParameters params)
            throws IOException {
        if (params != null) {
            method.setRequestHeader(
                    HEADER_ALGORITHM_PARAMETERS, Base64.encodeBytes(params.getEncoded()));
        }
    }
    ...
    protected AlgorithmParameters decodeAlgorithmParameters(HttpMethod method) throws IOException {
        ...
        if (header != null) {
            ...
            AlgorithmParameters algorithmParameters =
                    encryptor.decodeAlgorithmParameters(algorithmParams);
     ...
     * @throws IOException
     */
    protected AlgorithmParameters decodeAlgorithmParameters(HttpServletRequest req)
            throws IOException {
    ...
    public byte[] decryptResponseBody(HttpMethod method) throws IOException {
        ...
        if (body != null) {
            ...
            if (params != null) {
                byte[] decrypted =
                        encryptor.decrypt(KeyProvider.ALIAS_SOLR, params, out.toByteArray());
                return decrypted;
            } else {
    ...
    public boolean authenticateResponse(HttpMethod method, String remoteIP, byte[] decryptedBody) {
        try {
            ...
            return authenticate(
                    expectedMAC, new MACInput(decryptedBody, timestamp.longValue(), remoteIP));
        } catch (Exception e) {
    ...
    public boolean authenticate(HttpServletRequest req, byte[] decryptedBody) {
        try {
            ...
            return authenticate(
                    expectedMAC, new MACInput(decryptedBody, timestamp.longValue(), ipAddress));
        } catch (Exception e) {
    ...
    public void setRequestAuthentication(HttpMethod method, byte[] message) throws IOException {
        ...
        // add MAC header
        byte[] mac =
                macUtils.generateMAC(
                        KeyProvider.ALIAS_SOLR,
                        new MACInput(message, requestTimestamp, getLocalIPAddress()));

        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Setting MAC " + Arrays.toString(mac) + " on HTTP request " + method.getPath());
            logger.debug(
                    "Setting timestamp "
                            + requestTimestamp
                            + " on HTTP request "
    ...
    protected boolean authenticate(byte[] expectedMAC, MACInput macInput) {
        // check the MAC and, if valid, check that the timestamp is under the threshold and that the
        // remote IP is
        // the expected IP
        boolean authorized =

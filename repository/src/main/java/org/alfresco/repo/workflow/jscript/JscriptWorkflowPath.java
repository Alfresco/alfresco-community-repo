 ...
 *
 * <p>A simple workflow consists of only one root path of execution. That path may branch at some
 * subsequent transition, so that execution follows multiple paths through the workflow.
 *
 * @author glenj
 */
public class JscriptWorkflowPath implements Serializable {
    ...
    public JscriptWorkflowPath(
            ...
            final JscriptWorkflowInstance instance,
    ...
    /**
     * Creates a new instance of WorkflowPath from an instance of the WorkflowPath class provided by
     * the CMR workflow model
     *
     * @param cmrWorkflowPath an instance of WorkflowPath from the CMR workflow object model
     * @param serviceRegistry Service Registry object
     * @param scope the root scripting scope for this object
     */
    public JscriptWorkflowPath(
            final WorkflowPath cmrWorkflowPath,
            final ServiceRegistry serviceRegistry,
            Scriptable scope) {
        this.id = cmrWorkflowPath.id;
        this.node = cmrWorkflowPath.node;
        this.instance =
                new JscriptWorkflowInstance(cmrWorkflowPath.instance, serviceRegistry, scope);
    ...
    public Scriptable getTasks() {
        ...
        Scriptable tasksScriptable =
                (Scriptable)
                        new ValueConverter()
                                .convertValueForScript(this.serviceRegistry, scope, null, tasks);

...
public interface TaskComponent
{
     ...
     *            hint in order to return partially-initialized entities
    ...
    /**
     * @deprecated Use overloaded method with the {@code sameSession} parameter (this method defaults the parameter to {@code false}).
     */
     ...
     * Query for tasks
     * 
     * Hint: use {@link WorkflowTaskQuery} setLimit() method to limit the number of processed items if you don't really need to go through all of them
     * 
     * @param query
     *            the filter by which tasks are queried
     * @param sameSession
     *            indicates that the returned {@link WorkflowTask} elements will be used in the same session. If {@code true}, the returned List will be a lazy loaded list providing greater performance.
     ...
     * @param add
     *            the map of items to associate with the task (or null, if none to add)
     * @param remove
     ...
     * 
     * Note: this is an optional task operation. It may be used to track when work started on a task as well as resume a suspended task.
     * 
     * @param taskId
     *            the task to start
     ...
     *            the task id to end
     * @param transitionId
     *            the task transition id to take on completion (or null, for the default transition)
     * @return the updated task
     */

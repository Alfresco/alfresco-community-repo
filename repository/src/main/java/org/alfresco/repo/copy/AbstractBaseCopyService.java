...
public class AbstractBaseCopyService {
  ...
  protected AssociationCopyInfo getAssociationCopyInfo(
    ...
    ChildAssociationRef primaryAssocRef = nodeService.getPrimaryParent(
      sourceNodeRef
    );
    ...
    if (sourceParentRef != null) {
      ...
      boolean copyingFromPrimaryParent = sourceParentRef.equals(
        primaryAssocRef.getParentRef()
      );
      if (!copyingFromPrimaryParent) {
        ...
        List<ChildAssociationRef> assocList = nodeService.getParentAssocs(
          sourceNodeRef
        );
    ...
    if (
      nameChanged && !systemNamespaces.contains(existingQName.getNamespaceURI())
    ) {
  ...
  public static class AssociationCopyInfo {
    ...
    public AssociationCopyInfo(
      QName targetAssocQName,

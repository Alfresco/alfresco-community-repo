...
public class VirtualStoreImplTest extends VirtualizationIntegrationTest {
    ...
    public void testResolveVirtualFolderDefinition_inactiveSynchronization() throws Exception {
        txnTamperHint =
                "VirtualStoreImplTest::testResolveVirtualFolderDefinition_inactiveSynchronization";
        txn.rollback();
        ...
        try {
            ...
            jsonTemplateContent =
                    nodeService.getChildByName(
                            companyHomeNodeRef, ContentModel.ASSOC_CONTAINS, templateName);
            if (jsonTemplateContent == null) {
                ChildAssociationRef templateChild =
                        createContent(
                                companyHomeNodeRef,
                                templateName,
                                ApplyTemplateMethodTest.class.getResourceAsStream(
            ...
            ntVirtualizedFolder =
                    nodeService.getChildByName(
                            companyHomeNodeRef, ContentModel.ASSOC_CONTAINS, folderName);
            ...
            Reference aVanillaRef =
                    ((VanillaProtocol) Protocols.VANILLA.protocol)
                            .newReference(
    ...
    public void testCanVirtualize() throws Exception {
        ...
        assertEquals(false, canVirtualize);
    }
    ...
    public void testCanCreateFolderNamedV() throws Exception {
        ...
        Reference reference =
                Reference.fromNodeRef(new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, v));
    ...
    private void assertHasVirtualNodePermission(
            AccessStatus accessStatus, String perm, boolean readonly) {
        VirtualUserPermissions virtualUserPermissions = smartStore.getUserPermissions();

        assertEquals(
                AccessStatus.DENIED,
                virtualUserPermissions.hasVirtualNodePermission(perm, readonly));
        assertEquals(
                AccessStatus.DENIED,
                virtualUserPermissions.hasVirtualNodePermission(asTypedPermission(perm), readonly));
    }
    ...
    public void testConfiguredUserPermissions() throws Exception {
        ...
        assertHasVirtualNodePermission(
                AccessStatus.DENIED, PermissionService.CREATE_ASSOCIATIONS, true);
        assertHasVirtualNodePermission(AccessStatus.DENIED, PermissionService.UNLOCK, true);
        assertHasVirtualNodePermission(
                AccessStatus.DENIED, PermissionService.CANCEL_CHECK_OUT, true);
        ...
        assertHasVirtualNodePermission(
                AccessStatus.DENIED, PermissionService.CHANGE_PERMISSIONS, true);
        ...
        assertHasVirtualNodePermission(
                AccessStatus.DENIED, PermissionService.WRITE_PROPERTIES, true);
        assertHasVirtualNodePermission(AccessStatus.DENIED, PermissionService.WRITE, false);
        assertHasVirtualNodePermission(
                AccessStatus.DENIED, PermissionService.WRITE_PROPERTIES, false);
    }
    ...
    public void testCanExistNodeIDwithV() {
        ...
        // MNT-21968
        createAndCheckNodeId(
                "v" + Encodings.ZERO.encoding.token + "0Draft.pdf", "specialFile6.txt");

        NodeRef virtualFolder =
                createVirtualizedFolder(
                        testRootFolder.getNodeRef(),
                        VIRTUAL_FOLDER_3_NAME,

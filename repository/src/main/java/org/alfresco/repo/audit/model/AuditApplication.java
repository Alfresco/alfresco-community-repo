...
/**
 * Helper class that wraps the {@link Application audit application}. Once wrapped, client code
 * doesn't need access to any of the generated model-driven classes.
 *
 * @author Derek Hulley
 * @since 3.2
 */
public class AuditApplication {
    ...
    /** Derived expaned map for fast lookup */
    private Map<String, Map<String, DataGenerator>> dataGenerators =
            new HashMap<String, Map<String, DataGenerator>>(11);
    ...
    public void checkPath(String path) {
        ...
        if (path == null || path.length() == 0) {
            ...
            generateException(
                    path, "An audit must match regular expression: " + AUDIT_PATH_PATTERN);
        } else if (path.indexOf(applicationKey, 0) != 1) {
            generateException(
                    path,
                    "An audit path's first element must be the application's key i.e. '"
                            + applicationKey
                            + "'.");
        }
    }
    ...
    /**
     * Compile a path or part of a path into a single string which always starts with the {@link
     * #AUDIT_PATH_SEPARATOR}. This can be a relative path so need not always start with the
     * application root key.
     *
     * <p>If the path separator is present at the beginning of a path component, then it is not
     * added, so <code>"/a", "b", "/c"</code> becomes <code>"/a/b/c"</code> allowing path to be
     * appended to other paths.
     *
     * <p>The final result is checked against a {@link #AUDIT_PATH_PATTERN regular expression} to
     * ensure it is valid.
     *
    ...
    public static String buildPath(String... pathComponents) {
        ...
        if (!AUDIT_PATH_PATTERN.matcher(path).matches()) {
            ...
            msg.append("The audit path is invalid and must be matched by regular expression: ")
                    .append(AUDIT_PATH_PATTERN)
                    .append("\n")
    ...
    public static class DataExtractorDefinition {
         ...
         */
        public DataExtractorDefinition(
                String dataTrigger,
                String dataSource,
                String dataTarget,
    ...
    private void buildAuditPaths(AuditPath auditPath) {
        buildAuditPaths(
                auditPath, null, new HashSet<String>(37), new HashMap<String, DataGenerator>(13));
    }
    ...
    private void buildAuditPaths(
        ...
        for (RecordValue element : auditPath.getRecordValue()) {
            ...
            if (extractor == null) {
                generateException(
                        extractorPath, "No data extractor exists for name: " + extractorName);
            }
            ...
            DataExtractorDefinition extractorDef =
                    new DataExtractorDefinition(dataTrigger, sourcePath, extractorPath, extractor);
        ...
        for (GenerateValue element : auditPath.getGenerateValue()) {
            ...
            if (generator == null) {
                generateException(
                        generatorPath, "No data generator exists for name: " + generatorName);
            }
    ...
    /**
     * Returns {@code true} if the application name has a prefix of {@code "PreCallData"} that
     * indicates that the only purpose of the Application is to generate data to be passed to a post
     * call audit application. In this situation the application's audit data is not audited. This
     * allows the post audit application to have access to 'before' values including those created
     * by extractors and generators. Some of which will not be available (for example the node has
     * been deleted) or will have changed as a result of the call.
     */
    public boolean isApplicationJustGeneratingPreCallData() {
        return applicationName != null
                && applicationName.startsWith(AUDIT_APPLICATION_PREFIX_FOR_PRE_DATA);
    }
}

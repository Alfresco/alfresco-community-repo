...
public class RenditionsImpl implements Renditions, ResourceLoaderAware {
  ...
  private static final Set<String> RENDITION_STATUS_COLLECTION_EQUALS_QUERY_PROPERTIES = Collections.singleton(
    PARAM_STATUS
  );
  ...
  public void setScriptThumbnailService(
    ScriptThumbnailService scriptThumbnailService
  ) {
  ...
  public void setRenditionsDataCollector(
  ...
  public CollectionWithPagingInfo<Rendition> getRenditions(
    ...
    final NodeRef validatedNodeRef = validateNode(
      nodeRef.getStoreRef(),
      nodeRef.getId(),
      versionLabelId,
      parameters
    ...
    if (status != null) {
      includeCreated =
        RenditionStatus.CREATED.equals(RenditionStatus.valueOf(status));
    ...
    List<ChildAssociationRef> nodeRefRenditions = renditionService2.getRenditions(
      validatedNodeRef
    );
    ...
    PagingResults<Rendition> results = Util.wrapPagingResults(
      paging,
      apiRenditions.values()
    );
  ...
  public Rendition getRendition(
    ...
    NodeRef renditionNodeRef = getRenditionByName(
      validatedNodeRef,
      renditionId,
      parameters
    );
  ...
  public void createRendition(
    ...
    if (!renditionService2.isEnabled()) {
      throw new DisabledServiceException(
        "Rendition generation has been disabled."
      );
    }

    final NodeRef sourceNodeRef = validateNode(
      nodeRef.getStoreRef(),
      nodeRef.getId(),
      versionLabelId,
      parameters
    );
    final NodeRef renditionNodeRef = getRenditionByName(
      sourceNodeRef,
      rendition.getId(),
      parameters
    );
    if (renditionNodeRef != null) {
      throw new ConstraintViolatedException(
        rendition.getId() + " rendition already exists."
      ); // 409
    }
  ...
  public void createRenditions(
    ...
    if (renditionNamesNotRegistered.length() != 0) {
      throw new NotFoundException(
        "Renditions not registered: " + renditionNamesNotRegistered
      ); // 404
    }

    if (renditionNamesToCreate.size() == 0) {
      throw new ConstraintViolatedException(
        "All renditions requested already exist: " + renditionNamesAlreadyExist
      ); // 409
    }
  ...
  private BinaryResource getContentImpl(
    ...
    if (renditionNodeRef == null) {
      boolean isPlaceholder = Boolean.valueOf(
        parameters.getParameter(PARAM_PLACEHOLDER)
      );
      if (!isPlaceholder) {
        throw new NotFoundException(
      ...
      String phPath = scriptThumbnailService.getMimeAwarePlaceHolderResourcePath(
        renditionId,
        sourceNodeMimeType
      );
      if (phPath == null) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Retrieving content from resource path [" + phPath + ']'
          );
        }
    ...
    ContentData contentData = (ContentData) nodeProps.get(
    ...
    if (nodeRef != null) {
      if (StringUtils.isEmpty(renditionId)) {
        throw new InvalidArgumentException(
          "renditionId can't be null or empty."
        );
      }

      ChildAssociationRef nodeRefRendition = renditionService2.getRenditionByName(
        nodeRef,
        renditionId
      );
      if (nodeRefRendition != null) {
        ContentData contentData = getContentData(
          nodeRefRendition.getChildRef(),
          false
        );
        if (contentData != null) {
  ...
  protected Rendition toApiRendition(NodeRef renditionNodeRef) {
    ...
    String renditionName = (String) nodeService.getProperty(
      renditionNodeRef,
  ...
  private NodeRef findVersionIfApplicable(
    NodeRef nodeRef,
    String versionLabelId
  ) {
  ...
  private ContentData getContentData(NodeRef nodeRef, boolean validate) {
    ...
    if (validate && !ContentData.hasContent(contentData)) {
      throw new InvalidArgumentException(
        "Node id '" + nodeRef.getId() + "' has no content."
      );
    }
    return contentData;
  }
  ...
  private String getStatus(Parameters parameters) {
    ...
    if (query != null) {
      ...
      MapBasedQueryWalker propertyWalker = new MapBasedQueryWalker(
        RENDITION_STATUS_COLLECTION_EQUALS_QUERY_PROPERTIES,
        null
      );
      QueryHelper.walk(query, propertyWalker);

      status =
        propertyWalker.getProperty(PARAM_STATUS, WhereClauseParser.EQUALS);
    }
    return status;
  }
}

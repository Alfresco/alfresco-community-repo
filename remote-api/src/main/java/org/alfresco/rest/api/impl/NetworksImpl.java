...
public class NetworksImpl implements Networks {
  ...
  public Network validateNetwork(String networkId) {
    org.alfresco.repo.tenant.Network network = networksService.getNetwork(
      networkId
    );
    if (network == null) {
  ...
  private PersonNetwork getPersonNetwork(
    org.alfresco.repo.tenant.Network network
  ) {
    Network restNetwork = new NetworkImpl(network);
  ...
  public PersonNetwork getNetwork(String personId, String networkId) {
    ...
    org.alfresco.repo.tenant.Network tenantNetwork = networksService.getNetwork(
      network.getId()
    );
  ...
  public CollectionWithPagingInfo<PersonNetwork> getNetworks(
    ...
    PagingResults<org.alfresco.repo.tenant.Network> networks = networksService.getNetworks(
      Util.getPagingRequest(paging)
    );
    List<PersonNetwork> ret = new ArrayList<PersonNetwork>(
      networks.getPage().size()
    );
    ...
    return CollectionWithPagingInfo.asPaged(
      paging,
      ret,
      networks.hasMoreItems(),
      networks.getTotalResultCount().getFirst()
    );
  }
}

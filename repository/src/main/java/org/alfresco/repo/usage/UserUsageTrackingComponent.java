...
public class UserUsageTrackingComponent
  ...
  private static Log logger = LogFactory.getLog(
    UserUsageTrackingComponent.class
  );
  ...
  public void onCreateNode(ChildAssociationRef childAssocRef) {
    if (enabled == true) {
      ...
      final String userName = (String) this.nodeService.getProperty(
          personRef,
        ...
        RetryingTransactionCallback<Long> updateUserWithUsage = new RetryingTransactionCallback<Long>() {
          public Long execute() throws Throwable {
            ...
            for (String store : stores) {
              final StoreRef storeRef = tenantService.getName(
                new StoreRef(store)
              );

              Long contentSize = usageDAO.getContentSizeForStoreForUser(
                storeRef,
                userName
              );
              ...
              if (contentSize != null) {
                currentUsage =
                  (currentUsage == null ? 0L : currentUsage) + contentSize;
              }
            }

            if (currentUsage != null && currentUsage > 0L) {
              List<Pair<NodeRef, Long>> batchUserUsages = new ArrayList<Pair<NodeRef, Long>>(
                1
              );
              batchUserUsages.add(
  ...
  private void clearAllUsages() {
      ...
      public Object execute() throws Throwable {
        ...
        MapHandler userHandler = new MapHandler() {
          public void handle(Map<String, Object> result) {
            ...
            if (
              !username.equalsIgnoreCase(AuthenticationUtil.getSystemUserName())
            ) {
        ...
        usageDAO.getUsersWithUsage(
          tenantService.getName(personStoreRef),
          userHandler
        );
    ...
    transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(getUsersWithUsage, true);
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
        "... cleared non-missing usages for " + clearCount + " users"
      );
    }
  }
  ...
  private void calculateMissingUsages() {
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
        "... calculated missing usages for " + updateCount + " users"
      );
    }
  }
  ...
  private int recalculateUsages(final Map<String, NodeRef> users) {
    final Map<String, Long> currentUserUsages = new HashMap<String, Long>(
      users.size()
    );
  ...
  private void collapseUsages() {
    ...
    // execute in READ-ONLY txn
    Set<NodeRef> usageNodeRefs = transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(getUsageNodeRefs, true);

    int collapseCount = 0;
    for (final NodeRef usageNodeRef : usageNodeRefs) {
      Boolean collapsed = TenantUtil.runAsSystemTenant(
    ...
    RetryingTransactionCallback<Boolean> collapseUsages = new RetryingTransactionCallback<Boolean>() {
      public Boolean execute() throws Throwable {
        ...
        if (nodeType.equals(ContentModel.TYPE_PERSON)) {
          ...
          long currentUsage = contentUsageImpl.getUserStoredUsage(
            personNodeRef
          );
          if (currentUsage != -1) {

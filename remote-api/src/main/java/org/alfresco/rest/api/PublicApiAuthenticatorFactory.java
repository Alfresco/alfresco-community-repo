...
public class PublicApiAuthenticatorFactory
  ...
  private static Log logger = LogFactory.getLog(
    PublicApiAuthenticatorFactory.class
  );

  public static final String DEFAULT_AUTHENTICATOR_KEY_HEADER =
    "X-Alfresco-Authenticator-Key";
  ...
  private Map<String, String[]> getOutboundHeaders(
    TenantWebScriptServletRequest req
  ) {
            ...
            if (!authorized) {
              listener.authenticationFailed(
                new TenantCredentials(
                  tenant,
                  email,
                  proxyListener.getOrignalCredentials()
                )
              );

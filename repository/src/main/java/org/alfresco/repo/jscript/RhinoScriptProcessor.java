...
public class RhinoScriptProcessor
  ...
  implements ScriptProcessor, ScriptResourceLoader, InitializingBean {

  private static final Log logger = LogFactory.getLog(
    RhinoScriptProcessor.class
  );
  private static final Log callLogger = LogFactory.getLog(
    RhinoScriptProcessor.class.getName() + ".calls"
  );
  ...
  private final Map<String, Script> scriptCache = new ConcurrentHashMap<String, Script>(
    256
  );
    ...
    try {
      ...
      if (script == null) {
        if (logger.isDebugEnabled()) logger.debug(
          "Resolving and compiling script path: " + path
        );
    ...
    try {
      if (this.services.getNodeService().exists(nodeRef) == false) {
        throw new AlfrescoRuntimeException(
          "Script Node does not exist: " + nodeRef
        );
      }
      ...
      ContentReader cr =
        this.services.getContentService().getReader(nodeRef, contentProp);
      if (cr == null || cr.exists() == false) {
        throw new AlfrescoRuntimeException(
          "Script Node content not found: " + nodeRef
        );
      }
      ...
      try {
        script =
          cx.compileString(
            resolveScriptImports(cr.getContentString()),
            nodeRef.toString(),
            1,
            null
          );
  ...
  public Object executeString(String source, Map<String, Object> model) {
    try {
      ...
      try {
        script =
          cx.compileString(resolveScriptImports(source), "AlfrescoJS", 1, null);
      } finally {
        Context.exit();
      }
      ...
      throw new ScriptException(
        "Failed to execute supplied script: " + err.getMessage(),
        err
      );
    }
  }
  ...
  public String loadScriptResource(String resource) {
    ...
    if (resource.startsWith(PATH_CLASSPATH)) {
      try {
        ...
        URL scriptResource = getClass()
          .getClassLoader()
          .getResource(scriptClasspath);
        if (scriptResource == null && scriptClasspath.startsWith("/")) {
          // The Eclipse classloader prefers alfresco/foo to /alfresco/foo, try that
          scriptResource =
            getClass()
              .getClassLoader()
              .getResource(scriptClasspath.substring(1));
        }
        if (scriptResource == null) {
          throw new AlfrescoRuntimeException(
            "Unable to locate included script classpath resource: " + resource
          );
        }
      ...
      if (resource.startsWith("/")) {
        ...
        NodeRef rootNodeRef =
          this.services.getNodeService().getRootNode(this.storeRef);
        List<NodeRef> nodes =
        ...
        if (nodes.size() == 0) {
          throw new AlfrescoRuntimeException(
            "Unable to find store path: " + this.storePath
          );
        }
        ...
        try {
          FileInfo fileInfo =
            this.services.getFileFolderService()
              .resolveNamePath(nodes.get(0), elements);
          scriptRef = fileInfo.getNodeRef();
        } catch (FileNotFoundException err) {
          throw new AlfrescoRuntimeException(
            "Unable to load included script repository resource: " + resource
          );
        }
      } else {
      ...
      try {
        ContentReader cr =
          this.services.getContentService()
            .getReader(scriptRef, ContentModel.PROP_CONTENT);
        if (cr == null || cr.exists() == false) {
          throw new AlfrescoRuntimeException(
            "Included Script Node content not found: " + resource
          );
        }
   ...
   */
  private Object executeScriptImpl(
    Script script,
    Map<String, Object> model,
    boolean secure,
    String debugScriptName
  ) throws AlfrescoRuntimeException {
    long startTime = 0;
    if (callLogger.isDebugEnabled()) {
    ...
    try {
      ...
      if (callLogger.isDebugEnabled()) {
        ...
        callLogger.debug(
          debugScriptName + " End " + (endTime - startTime) / 1000000 + " ms"
        );
      }
    }
  }
    ...
    if (model != null) {
      ...
      for (Map.Entry<String, Object> entry : model.entrySet()) {
        if (entry.getValue() instanceof NodeRef) {
          newModel.put(
            entry.getKey(),
            new ScriptNode((NodeRef) entry.getValue(), this.services)
          );
        } else {
  ...
  private static class RhinoWrapFactory extends WrapFactory {
    ...
    protected Scriptable wrapBasicJavaObject(
      Context cx,
      Scriptable scope,
      Object javaObject,
      Class<?> staticType
    ...
    public Scriptable wrapAsJavaObject(
      ...
      if (
        javaObject instanceof Map && !(javaObject instanceof ScriptableHashMap)
      ) {

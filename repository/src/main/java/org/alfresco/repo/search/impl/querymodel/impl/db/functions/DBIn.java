    ...
    public void prepare(
        ...
        Boolean not =
                DefaultTypeConverter.INSTANCE.convert(
                        Boolean.class, inverseArgument.getValue(functionContext));

        ListArgument listArgument = (ListArgument) functionArgs.get(ARG_LIST);
        Collection<Serializable> collection =
                (Collection<Serializable>) listArgument.getValue(functionContext);
        ...
        if (propertyArgument.getPropertyName().equals(PropertyIds.PARENT_ID)) {
            ...
            parentSupport.setDbids(
                    DBQuery.getDbids(DBQuery.toStringValues(collection), nodeDAO, tenantService));
            ...
            for (String typeName : DBQuery.toStringValues(collection)) {
                ...
                typeIds.addAll(
                        DBQuery.findTypeIds(
                                alfTypeName, namespaceService, dictionaryService, qnameDAO, true));
            }
            ...
            for (String typeName : DBQuery.toStringValues(collection)) {
                ...
                typeIds.addAll(
                        DBQuery.findTypeIds(
                                alfTypeName, namespaceService, dictionaryService, qnameDAO, false));
            }
            typeSupport.setQnameIds(typeIds);
            if ((not != null) && (not.equals(Boolean.TRUE))) {
                typeSupport.setCommandType(DBQueryBuilderPredicatePartCommandType.NOTIN);
            ...
            propertySupport.setFieldName(
                    DBQuery.getFieldName(
                            dictionaryService, propertyQName, supportBooleanFloatAndDouble));
            if ((not != null) && (not.equals(Boolean.TRUE))) {
                propertySupport.setCommandType(DBQueryBuilderPredicatePartCommandType.NOTIN);
            } else {
                propertySupport.setCommandType(DBQueryBuilderPredicatePartCommandType.IN);

...
public class DeclarativeCapabilityTest extends BaseRMTestCase {
    ...
    protected void setupTestDataImpl() {
        ...
        recordFolderContainsFrozen =
                recordFolderService.createRecordFolder(rmContainer, "containsFrozen");
        ...
        frozenRecordFolder =
                recordFolderService.createRecordFolder(rmContainer, "frozenRecordFolder");
                    ...
                    public Object execute() throws Throwable {
                        ...
                        hold =
                                holdService.createHold(
                                        filePlan, GUID.generate(), "reason", "description");
    ...
    protected void setupTestUsersImpl(NodeRef filePlan) {
        ...
        for (String user : testUsers) {
            ...
            filePlanPermissionService.setPermission(
                    rmContainer, user, RMPermissionModel.READ_RECORDS);
            filePlanPermissionService.setPermission(
                    moveToFolder, user, RMPermissionModel.READ_RECORDS);
            filePlanPermissionService.setPermission(
                    moveToCategory, user, RMPermissionModel.READ_RECORDS);
        }
    }
    ...
    private void testDeclarativeCapability(
            final DeclarativeCapability capability,
            final String userName,
            final NodeRef filePlanComponent) {
        doTestInTransaction(
                new Test<Void>() {
                    @Override
                    public Void run() {
                        ...
                        if (roles.isEmpty()) {
                            assertEquals(
                                    "User "
                                            + userName
                                            + " has no RM role so we expect access to be denied for"
                                            + " capability "
                                            + capability.getName(),
                                    AccessStatus.DENIED,
                                    accessStatus);
                            ...
                            FilePlanComponentKind actualKind =
                                    filePlanService.getFilePlanComponentKind(filePlanComponent);
                            ...
                            if (kinds == null || kinds.contains(actualKind.toString())) {
                                ...
                                boolean conditionResult =
                                        getConditionResult(filePlanComponent, conditions);

                                assertEquals(
                                        "User is expected to only have one role.", 1, roles.size());
                                ...
                                if (roleCapabilities.contains(capability) && conditionResult) {
                                    assertEquals(
                                            "User "
                                                    ...
                                                    + " so we expect access to be allowed for"
                                                    + " capability "
                                                    + capability.getName()
                                                    + " on the object "
                                                    + (String)
                                    ...
                                    assertEquals(
                                            "User "
                                                    ...
                                                    + " has the role "
                                                    + role.getDisplayLabel()
                                                    + " so we expect access to be denied for"
                                                    + " capability "
                                                    + capability.getName(),
                                            AccessStatus.DENIED,
                                            accessStatus);
                                ...
                                assertEquals(
                                        "NodeRef is of kind"
                                                + actualKind
                                                + " so we expect access to be denied for capability"
                                                + " "
                                                + capability.getName(),
                                        AccessStatus.DENIED,
                                        accessStatus);
        ...
        if (conditions != null && conditions.size() != 0) {
            for (Map.Entry<String, Boolean> entry : conditions.entrySet()) {
                ...
                CapabilityCondition condition =
                        (CapabilityCondition) applicationContext.getBean(entry.getKey());
        ...
        doTestInTransaction(
                new Test<Void>() {
                    ...
                    public Void run() {
                        ...
                        assertEquals(
                                AccessStatus.DENIED, capability.hasPermission(frozenRecordFolder));
                        assertEquals(
                                AccessStatus.DENIED,
                                capability.hasPermission(recordFolderContainsFrozen));
        ...
        doTestInTransaction(
                new Test<Void>() {
                    ...
                    public Void run() {
                        ...
                        assertEquals(
                                AccessStatus.UNDETERMINED,
                                capability.hasPermission(undeclaredRecord));

                        // now lets take a look when we know what the destination is
                        // NOTE:  should be denied since we do not have file permission on the
                        // destination folder
                        //        despite having the capability!
                        assertEquals(
                                AccessDecisionVoter.ACCESS_DENIED,
                                capability.evaluate(record, moveToFolder));
                        assertEquals(
                                AccessDecisionVoter.ACCESS_DENIED,
                                capability.evaluate(declaredRecord, moveToFolder));
                        assertEquals(
                                AccessDecisionVoter.ACCESS_DENIED,
                                capability.evaluate(undeclaredRecord, moveToFolder));
                        assertEquals(
                                AccessDecisionVoter.ACCESS_DENIED,
        ...
        doTestInTransaction(
                new Test<Void>() {
                    ...
                    public Void run() {
                        for (String user : testUsers) {
                            filePlanPermissionService.setPermission(
                                    moveToFolder, user, RMPermissionModel.FILING);
                        }
                        return null;
                    }
                },
        ...
        doTestInTransaction(
                new Test<Void>() {
                    ...
                    public Void run() {
                        ...
                        // now lets take a look when we know what the destination is
                        // NOTE:  should be allowed now since we have filling permission on the
                        // destination folder
                        assertEquals(
        ...
        doTestInTransaction(
                new Test<Void>() {
                    ...
                    public Void run() {
                        ...
                        assertEquals(AccessStatus.DENIED, capability.hasPermission(closedFolder));
                        assertEquals(
                                AccessStatus.DENIED, capability.hasPermission(undeclaredRecord));
        ...
        doTestInTransaction(
                new Test<Void>() {
                    ...
                    public Void run() {
                        ...
                        assertEquals(
                                AccessDecisionVoter.ACCESS_DENIED,
                                capability.evaluate(rmFolder, moveToCategory));
        ...
        doTestInTransaction(
                new Test<Void>() {
                    ...
                    public Void run() {
                        for (String user : testUsers) {
                            filePlanPermissionService.setPermission(
                                    moveToCategory, user, RMPermissionModel.FILING);
                        }
                        return null;
                    }
                },

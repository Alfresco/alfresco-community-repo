...
public class TemplateResourceProcessorTest
  ...
  public void setUp() throws Exception {
    ...
    VirtualContext context = new VirtualContext(
      environment,
    ...
    InputStream vanillaIS = getClass()
      .getResourceAsStream(TEST_TEMPLATE_2_JSON_CLASSPATH);
    String vanillaJSON = IOUtils.toString(vanillaIS, StandardCharsets.UTF_8);
    context.setParameter(
      ApplyTemplateMethod.VANILLA_JSON_PARAM_NAME,
      vanillaJSON
    );
    TemplateResourceProcessor processor = new TemplateResourceProcessor(
      context
    );

    testTemplate2Definition =
      processor.process(new ClasspathResource(VANILLA_PROCESSOR_JS_CLASSPATH));
  }
  ...
  public void testBasicNodeInfo() throws Exception {
    ...
    assertEquals(4, testTemplate2Definition.getChildren().size());

    assertEquals("Test", testTemplate2Definition.getName());

    VirtualFolderDefinition node2 = testTemplate2Definition.findChildByName(
      "Node2"
    );

    assertEquals("Node2", node2.getName());
    assertEquals("The2ndNode", node2.getDescription());
    ...
    assertTrue(
      node2Properties
        .keySet()
        .containsAll(
          Arrays.asList("cm:modifier", "cm:description", "sys:node-dbid")
        )
    );

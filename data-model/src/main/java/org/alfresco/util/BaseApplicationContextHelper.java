...
/**
 * Helper class to provide static and common access to the Spring {@link
 * org.springframework.context.ApplicationContext application context}.
 *
 * @author Derek Hulley
 */
public abstract class BaseApplicationContextHelper {
    ...
    /**
     * Build a classloader for the given classLocations, using the thread's context class loader as
     * its parent.
     *
     ...
     * repeatedly.
     *
     * <p>If the configuration requested differs from one used previously, then the
     * previously-created context is shut down.
     *
    ...
    /**
     * Should the Spring beans be initilised in a lazy manner, or all in one go? Normally lazy
     * loading/intialising shouldn't be used when running with the full context, but it may be
     * appropriate to reduce startup times when using a small, cut down context.
     */
    ...
    /**
     * Will the Spring beans be initilised in a lazy manner, or all in one go? The default it to
     * load everything in one go, as spring normally does.
     */
    ...
    /**
     * Should the autoStart=true property on subsystems be honoured, or should this property be
     * ignored and the auto start prevented? Normally we will use the spring configuration to decide
     * what to start, but when running tests, you can use this to prevent the auto start.
     */
    ...
    /**
     * Will Subsystems with the autoStart=true property set on them be allowed to auto start? The
     * default is to honour the spring configuration and allow them to, but they can be prevented if
     * required.
     */
    ...
    /**
     * A wrapper around {@link ClassPathXmlApplicationContext} which allows us to enable lazy
     * loading or prevent Subsystem autostart as requested.
     */
    protected static class VariableFeatureClassPathXmlApplicationContext
            extends ClassPathXmlApplicationContext {
        protected VariableFeatureClassPathXmlApplicationContext(String[] configLocations)
    ...
    /**
     * Can be used in Spring configuration to search for all resources matching an array of
     * patterns.
     *
     * @author dward
     */
    public static class ResourceFinder extends ServletContextResourcePatternResolver {
         ...
         * @return the matching resources, ordered by locationPattern index and location in the
         *     classpath

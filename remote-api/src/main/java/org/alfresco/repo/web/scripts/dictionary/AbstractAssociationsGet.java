...
public abstract class AbstractAssociationsGet extends DictionaryWebServiceBase {
  ...
  private static final String MODEL_PROP_KEY_INDIVIDUAL_PROPERTY_DEFS =
    "individualproperty";
  ...
  protected Map<String, Object> executeImpl(
    ...
    String associationFilter = req.getParameter(
      REQ_URL_TEMPL_VAR_ASSOCIATION_FILTER
    );
    String namespacePrefix = req.getParameter(
      REQ_URL_TEMPL_VAR_NAMESPACE_PREFIX
    );
    ...
    if (
      ...
      throw new WebScriptException(
        Status.STATUS_NOT_FOUND,
        "Missing either name or namespaceprefix parameter in the URL - both combination of name and namespaceprefix is needed"
      );
    }
    ...
    if (associationFilter.equals("child")) {
      ...
      for (AssociationDefinition assocname : this.dictionaryservice.getClass(
          classQname
        )
        .getAssociations()
        .values()) {
    ...
    if (name != null && namespacePrefix != null) {
      ...
      if (
        ...
        throw new WebScriptException(
          Status.STATUS_NOT_FOUND,
          "not a Valid - namespaceprefix_name combination"
        );
      }

      model.put(
        MODEL_PROP_KEY_INDIVIDUAL_PROPERTY_DEFS,
        this.dictionaryservice.getClass(classQname)
          .getAssociations()
          .get(associationQname)
      );
   ...
   */
  protected abstract QName getAssociationQname(

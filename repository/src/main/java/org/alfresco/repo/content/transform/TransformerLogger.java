...
/**
 * Implementation of {@link Log} that logs messages to an internal structure which only keeps a
 * predefined number of entries.
 *
 * <p>Only supports debug level logging.
 *
 * @author Alan Davis
 */
@AlfrescoPublicApi
abstract class TransformerLogger<T> extends LogAdapter implements LogEntries {
    ...
    /**
     * Enabled if number of transformer debug entries is greater than 0 AND there is not a test
        ...
        if (getMaxEntries() > 0) {
            ...
            return new String[] {
                "No entries are available. "
                        + getPropertyName()
                        + " must be set to a number between 1 and "
                        + getUpperMaxEntries()
            };
        }
    }
    ...
    private static void appendProperty(
            StringBuilder sb, String propertyName, String value, String defaultValue) {
    ...
    /**
     * Overridden to specify the maximum value the maxEntries property may set. Generally quite a
     * small number as values are stored in memory.
     */

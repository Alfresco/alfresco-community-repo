...
public class RepositoryContainer extends AbstractRuntimeContainer {
    ...
    /**
     * @param fallbackTransactionHelper an unlimited transaction helper used to generate error
     *     responses
     */
     ...
     *
     * @param notPublicExceptions - {@link Class}&lt;?&gt;[] instance which contains list of not
     *     public exceptions
     */
     ...
     *
     * @param publicExceptions - {@link Class}&lt;?&gt;[] instance which contains list of public
     *     exceptions
     */
    ...
    public ServerModel getDescription() {
        return new RepositoryServerModel(
                descriptorService.getCurrentRepositoryDescriptor(),
    ...
    public Map<String, Object> getTemplateParameters() {
        // Ensure we have a transaction - we might be generating the status template after the main
        // transaction failed
        return fallbackTransactionHelper.doInTransaction(
                () -> {
                    Map<String, Object> params =
                            new HashMap<>(RepositoryContainer.super.getTemplateParameters());
                    params.put(
                            TemplateService.KEY_IMAGE_RESOLVER, imageResolver.getImageResolver());
                    addRepoParameters(params);
                    return params;
                },
                true);
    ...
    public void executeScript(
            WebScriptRequest scriptReq, WebScriptResponse scriptRes, final Authenticator auth)
            throws IOException {
        try {
            ...
            if (displayCause == null && hideCause != null) {
                ...
                throw new RuntimeException(
                        "Server error (" + num + ").  Details can be found in the server logs.");
            } else {
                throw e;
            }
        }
    ...
    protected void executeScriptInternal(
        ...
        // Escalate the webscript declared level of authentication to the container required
        // authentication
        ...
        final RequiredAuthentication required =
                (desc.getRequiredAuthentication().compareTo(containerRequiredAuthentication) < 0
        ...
        // if the required authentication is not equal to guest, then it should be one of the
        // following:
        ...
        if (required != RequiredAuthentication.guest && isGuest) {
            throw new WebScriptException(
                    ...
                    "Web Script "
                            + desc.getId()
                            + " requires user authentication; however, a guest has attempted"
                            + " access.");
        }
        ...
        try {
            ...
            final RetryingTransactionCallback<Boolean> authWork =
                    () -> {
                        ...
                        // The user will now have been authenticated, based on HTTP Auth, Ticket,
                        // etc.
                        // Check that the user they authenticated as has appropriate access to the
                        // script
    ...
    private void checkScriptAccess(RequiredAuthentication required, String scriptDescriptorId) {
        ...
        if (required == RequiredAuthentication.sysadmin
                && !(isSysAdminUser() || isAdminOrSystemUser())) {
            throw new WebScriptException(
                    HttpServletResponse.SC_UNAUTHORIZED,
                    "Web Script "
                            + scriptDescriptorId
                            + " requires system-admin authentication; however, a non-system-admin"
            ...
            throw new WebScriptException(
                    ...
                    "Web Script "
                            + scriptDescriptorId
                            + " requires admin authentication; however, a non-admin has attempted"
                            + " access.");
        }
    }
     ...
     * @throws IOException
     */
    protected void transactionedExecute(
        ...
        final RequiredTransactionParameters trxParams =
                description.getRequiredTransactionParameters();

        try (final BufferedRequest bufferedReq =
                        newBufferedRequest(trxParams, scriptReq, streamFactory);
                final BufferedResponse bufferedRes =
                        newBufferedResponse(trxParams, scriptRes, streamFactory)) {
            boolean readonly =
                    description.getRequiredTransactionParameters().getCapability()
                            == TransactionCapability.readonly;
            boolean requiresNew =
                    description.getRequiredTransaction() == RequiredTransaction.requiresnew;

            // log a warning if we detect a GET webscript being run in a readwrite transaction, GET
            // calls should
            // NOT have any side effects so this scenario as a warning sign something maybe amiss,
            // see ALF-10179.
            if (logger.isDebugEnabled()
                    && !readonly
                    && "GET".equalsIgnoreCase(description.getMethod())) {
                logger.debug(
                        "Webscript with URL '"
                                + scriptReq.getURL()
                                + "' is a GET request but it's descriptor has declared a readwrite"
            ...
            try {
                ...
                if (script instanceof LoginPost) {
                    // login script requires read-write transaction because of authorization
                    // interceptor
                ...
                transactionHelper.doInTransaction(
                        () -> {
                            try {
                                ...
                                if (logger.isDebugEnabled()) {
                                    logger.debug(
                                            "Transaction exception: "
                                                    ...
                                                    + ": "
                                                    + e.getMessage());
                                    // Note: user transaction shouldn't be null, but just in case
                                ...
                                final UserTransaction userTrx =
                                        RetryingTransactionHelper.getActiveUserTransaction();
                                if (userTrx != null) {
                                    if (userTrx.getStatus() != Status.STATUS_MARKED_ROLLBACK) {
                                        if (logger.isDebugEnabled())
                                            logger.debug(
                                                    "Marking web script transaction for rollback");
                                        try {
                                            userTrx.setRollbackOnly();
                                        } catch (Throwable re) {
                                            if (logger.isDebugEnabled())
                                                logger.debug(
                                                        "Caught and ignoring exception during"
                ...
                throw new WebScriptException(
                        HttpServletResponse.SC_SERVICE_UNAVAILABLE, e.getMessage(), e);
            }
    ...
    private static void handleIOException(final IOException ioe) throws IOException {
        ...
        // Note: if you need to look for more exceptions in the stack, then create a static array
        // and pass it in
     ...
     * @throws IOException
     */
    private void transactionedExecuteAs(
            final WebScript script,
            final WebScriptRequest scriptReq,
            final WebScriptResponse scriptRes)
            throws IOException {
    ...
    private void transactionedExecuteAs(
            ...
            throws IOException {
        // Execute as System if and only if, the current user is a member of System-Admin group, and
        // he is not a super admin.
        // E.g. if 'jdoe' is a member of ALFRESCO_SYSTEM_ADMINISTRATORS group, then the work should
        // be executed as System to satisfy the ACL checks.
        // But, if the current user is Admin (i.e. super admin, which by default he is a member fo
        // the ALFRESCO_SYSTEM_ADMINISTRATORS group)
        // then don't wrap the work as RunAs, since he can do anything!
        if (requiredAuthentication == RequiredAuthentication.sysadmin
                && isSysAdminUser()
                && !isAdmin()) {
            AuthenticationUtil.runAs(
                    () -> {
    ...
    private static BufferedResponse newBufferedResponse(
        ...
        if (logger.isDebugEnabled())
            logger.debug(
                    "Creating Transactional Response for ReadWrite transaction; buffersize="

...
public class ParamsExtractorTests {
  ...
  public void testGetExtractor() {
    ...
    when(request.getServiceMatch())
      .thenReturn(new Match(null, templateVars, null));

    Params params = extractor.extractParams(mockEntity(), request);
    assertNull(
      "For getting a Collection there should be no entity params.",
      params.getEntityId()
    );
    assertNull(
      "For getting a Collection there should be no passed params.",
      params.getPassedIn()
    );
    assertNull(
    ...
    assertTrue(
      ...
      BeanPropertiesFilter.AllProperties.class.equals(
          params.getFilter().getClass()
        )
    );
  ...
  public void testPostExtractor() throws IOException {
    ...
    when(content.getReader())
      .thenReturn(new StringReader(JsonJacksonTests.FARMER_JSON));
    WebScriptRequest request = mock(WebScriptRequest.class);
    when(request.getServiceMatch())
      .thenReturn(new Match(null, templateVars, null));
    ...
    assertTrue(
      "A Farmer was passed in.",
      Farmer.class.equals(passedObjs.get(0).getClass())
    );
    ...
    when(content.getReader())
      .thenReturn(new StringReader(JsonJacksonTests.FARMER_JSON)); //reset the reader
    params = extractor.extractParams(mockRelationship(), request);
    assertNotNull(params);
    assertEquals("1234", params.getEntityId());
    ...
    try {
      ...
      assertNotNull(
        "POSTING to a relationship collection by id is not correct.",
        iae
    ...
    templateVars.clear();
    Method aMethod = ResourceInspector.findMethod(
      EntityResourceAction.Create.class,
      GrassEntityResource.class
    );
    ResourceOperation op = ResourceInspector.inspectOperation(
      GrassEntityResource.class,
      aMethod,
      HttpMethod.POST
    );
    List<ResourceMetadata> metainfo = ResourceInspector.inspect(
      GrassEntityResource.class
    );
    assertNotNull(op);
    assertTrue(
      "Create method should have two params",
      op.getParameters().size() == 2
    );
    ResourceParameter singleParam = op.getParameters().get(0);
    assertTrue(
      ResourceParameter.KIND.HTTP_BODY_OBJECT.equals(singleParam.getParamType())
    );
    assertFalse(
      "Create grass does not support multiple grass creations",
      singleParam.isAllowMultiple()
    );
    ...
    when(content.getReader())
      .thenReturn(new StringReader(JsonJacksonTests.GRASS_JSON));
    params = extractor.extractParams(metainfo.get(0), request);
    assertNotNull(params);
  }
  ...
  private Params testExtractOperationParams(
    ...
    assertNotNull(params);
    assertNull(
      "For a Collection there should be no relationshipId params.",
      params.getRelationshipId()
    );
  ...
  public void testPutExtractor() throws IOException {
    ...
    when(request.getContentType())
      .thenReturn("application/pdf; charset=UTF-16BE");
    ...
    assertTrue(
      "A Farmer was passed in.",
      Farmer.class.equals(passed.getClass())
    );
    ...
    assertEquals(
      "Relationship id should be automatically set on the object passed in.",
      aFarmer.getId(),
      "67890"
    );
  ...
  public void testSpecialChars() throws IOException {
    ...
    String mockMe =
      "{\"name\":\"" +
      specialChars +
      "\",\"created\":\"2012-03-23T15:56:18.552+0000\",\"age\":54,\"id\":\"1234A3\",\"farm\":\"LARGE\"}";
    Content content = mock(Content.class);
    ...
    when(getRequest.getServiceMatch())
      .thenReturn(new Match(null, getTemplateVars, null));
    when(getRequest.getParameterNames()).thenReturn(new String[] { "aParam" });
    when(getRequest.getParameterValues("aParam"))
      .thenReturn(new String[] { specialChars });
  ...
  private static ResourceMetadata mockEntity() {
    ...
    when(resourceMock.getType())
      .thenReturn(ResourceMetadata.RESOURCE_TYPE.ENTITY);
    when(resourceMock.getOperation(notNull(HttpMethod.class)))
      .thenReturn(resourceOperation);
    when(resourceMock.getObjectType(notNull(ResourceOperation.class)))
      .thenReturn(Farmer.class);
    return resourceMock;
  }
  ...
  private static ResourceMetadata mockProperty() {
    ResourceMetadata resourceMock = mock(ResourceMetadata.class);
    when(resourceMock.getType())
      .thenReturn(ResourceMetadata.RESOURCE_TYPE.PROPERTY);
  ...
  private static ResourceMetadata mockOperation() {
    ResourceMetadata resourceMock = mock(ResourceMetadata.class);
    when(resourceMock.getType())
      .thenReturn(ResourceMetadata.RESOURCE_TYPE.OPERATION);
    return resourceMock;
  }
  ...
  private static ResourceMetadata mockRelationship() {
    ...
    when(resourceMock.getOperation(notNull(HttpMethod.class)))
      .thenReturn(resourceOperation);
    when(resourceMock.getType())
      .thenReturn(ResourceMetadata.RESOURCE_TYPE.RELATIONSHIP);
    when(resourceMock.getObjectType(notNull(ResourceOperation.class)))
      .thenReturn(Farmer.class);
    return resourceMock;
  }

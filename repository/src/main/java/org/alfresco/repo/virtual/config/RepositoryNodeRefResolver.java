...
public class RepositoryNodeRefResolver implements NodeRefResolver {
    ...
    public NodeRef resolveNodeReference(String[] reference) {
        return repository.findNodeRef(NODE_REF_EXPRESSION, reference);
    }

    @Override
    public NodeRef resolvePathReference(String[] reference) {
        return repository.findNodeRef(PATH_REF_EXPRESSION, reference);
    }
    ...
    public NodeRef resolveQNameReference(String[] reference) {
        ...
        if (reference.length > 0) {
            ...
            List<ChildAssociationRef> rootChildren =
                    nodeService.getChildAssocs(
                            theNodeRef,
                            ContentModel.ASSOC_CHILDREN,
                            QName.createQName(reference[0], namespacePrefixResolver),
                            false);
            ...
            for (int i = 1; i < reference.length; i++) {
                ...
                List<ChildAssociationRef> children =
                        nodeService.getChildAssocs(
                                theNodeRef,
                                ContentModel.ASSOC_CONTAINS,
                                QName.createQName(reference[i], namespacePrefixResolver),
                                false);
    ...
    public NodeRef createNamePath(String[] reference) {
        ...
        if (reference == null || reference.length == 0) {
            ...
            while (!notFoundStack.isEmpty()) {
                ...
                final HashMap<QName, Serializable> newProperties =
                        new HashMap<QName, Serializable>();
                newProperties.put(ContentModel.PROP_NAME, toCreate);
                ChildAssociationRef newAssoc =
    ...
    public NodeRef createQNamePath(String[] reference, String[] names) {
        ...
        if (reference == null || reference.length == 0) {
            ...
            while (!notFoundStack.isEmpty()) {
                ...
                QName qNameToCreate =
                        QName.createQName(stringQNameToCreate, namespacePrefixResolver);
                String nameToCreate;

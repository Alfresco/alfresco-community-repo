...
public class CheckOutCheckInServiceImplTest extends BaseSpringTest {
  ...
  private static final QName PROP_NAME_QNAME = QName.createQName(
  ...
  public void before() {
    ...
    this.versionService =
      (VersionService) this.applicationContext.getBean("versionService");
    ...
    ServiceRegistry serviceRegistry = (ServiceRegistry) this.applicationContext.getBean(
        "ServiceRegistry"
      );
  ...
  private NodeRef checkout() {
    ...
    String expectedWorkingCopyLabel = I18NUtil.getMessage(
      "coci_service.working_copy_label"
    );
    String expectedWorkingCopyName = CheckOutCheckInServiceImpl.createWorkingCopyName(
      name,
      expectedWorkingCopyLabel
    );
    String workingCopyName = (String) this.nodeService.getProperty(
        workingCopy,
        PROP_NAME_QNAME
      );
  ...
  public void testCheckIn() {
    ...
    NodeRef origNodeRef = cociService.checkin(
      workingCopy3,
      versionProperties3,
      contentUrl,
      true
    ...
    assertEquals(
      TEST_VALUE_2,
      nodeService.getProperty(versionNodeRef, PROP_NAME_QNAME)
    );
    assertEquals(
      TEST_VALUE_2,
      nodeService.getProperty(origNodeRef, PROP_NAME_QNAME)
    );

    // Check that the other properties are updated during the check-in
    assertEquals(
      TEST_VALUE_3,
      nodeService.getProperty(versionNodeRef, PROP2_QNAME)
    );
    assertEquals(
      TEST_VALUE_3,
      nodeService.getProperty(origNodeRef, PROP2_QNAME)
    );
  ...
  public void testCheckInVersionedNode_MNT_8789() {
    ...
    Serializable modifiedDate = nodeService.getProperty(
      nodeA,
  ...
  public void testDeleteSourceOfLockedCopy() {
    // Create a FolderA
    final NodeRef folderA = transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(
        new RetryingTransactionCallback<NodeRef>() {
          public NodeRef execute() throws Exception {
    ...
    // Create a FolderB
    NodeRef folderB = transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(
        new RetryingTransactionCallback<NodeRef>() {
          public NodeRef execute() throws Exception {
            NodeRef b = nodeService
              .createNode(
                rootNodeRef,
                ContentModel.ASSOC_CONTAINS,
    ...
    NodeRef file = transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(
        new RetryingTransactionCallback<NodeRef>() {
          public NodeRef execute() throws Exception {
            NodeRef file = nodeService
              .createNode(
                folderA,
                ContentModel.ASSOC_CONTAINS,
    ...
    assertNotNull(workingCopy);
    Date wcModBefore = (Date) nodeService.getProperty(
      workingCopy,
      ContentModel.PROP_MODIFIED
    );
    ...
    Date wcModAfter = (Date) nodeService.getProperty(
      workingCopy,
      ContentModel.PROP_MODIFIED
    );
    assertEquals(
      "cm:modified should not change on the copied node when deleting the original",
      wcModBefore,
      wcModAfter
    );
  }
  ...
  public void testCheckOutCheckInWithDifferentLocales() {
    ...
    // Check that the working copy name has been set correctly
    String workingCopyName = (String) nodeService.getProperty(
      workingCopy,
      PROP_NAME_QNAME
    );
    assertEquals(
      "Working copy name not correct",
      "myDocument (Copie de Travail).doc",
      workingCopyName
    );
  ...
  public void testCheckInWithNameChange() {
    ...
    NodeRef fileWorkingCopyNodeRefCheck = cociService.getWorkingCopy(
      fileNodeRef
    );
    assertEquals(
      "Working copy not found ",
      fileWorkingCopyNodeRef,
      fileWorkingCopyNodeRefCheck
    );
  ...
  public void testCheckOutCheckInWithTranslatableAspect() {
    ...
    NodeRef translationNodeRef = nodeService
      .createNode(
        rootNodeRef,
        ContentModel.ASSOC_CHILDREN,
  ...
  public void testAutoCancelCheckOut() {
    Date modifiedDateBeforeCheckOut = (Date) this.nodeService.getProperty(
        this.nodeRef,
        ContentModel.PROP_MODIFIED
      );
    NodeRef workingCopy = checkout();
    ...
    Date modifiedDateAfterCheckOut = (Date) this.nodeService.getProperty(
        this.nodeRef,
        ContentModel.PROP_MODIFIED
      );
  ...
  public void testBidirectionalReferences() {
    final NodeRef origNodeRef = nodeService
      .createNode(
        this.rootNodeRef,
        ContentModel.ASSOC_CHILDREN,
  ...
  public void testETWOTWO_733() {
    NodeRef origNodeRef = nodeService
      .createNode(
        this.rootNodeRef,
        ContentModel.ASSOC_CHILDREN,
        QName.createQName("test2"),
  ...
  public void testMultipleCheckoutsCheckInsWithPropChange() {
    ...
    // Checkout
    final NodeRef workingCopy1 = transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(
        new RetryingTransactionCallback<NodeRef>() {
          public NodeRef execute() throws Exception {
    ...
    // Checkout
    final NodeRef workingCopy2 = transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(
        new RetryingTransactionCallback<NodeRef>() {
          public NodeRef execute() throws Exception {
    ...
    // Checkout
    final NodeRef workingCopy3 = transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(
        new RetryingTransactionCallback<NodeRef>() {
          public NodeRef execute() throws Exception {
  ...
  public void testAlfrescoCheckoutDoesNotModifyNode() {
    ...
    Serializable initModifier = nodeService.getProperty(
      nodeRef,
      ContentModel.PROP_MODIFIER
    );
    Serializable initModified = nodeService.getProperty(
      nodeRef,
      ContentModel.PROP_MODIFIED
    );
    assertFalse(
      "The initial modifier should not be Admin!",
      adminUser.equals(initModifier)
    );
    ...
    Serializable modifier = nodeService.getProperty(
      nodeRef,
      ContentModel.PROP_MODIFIER
    );
    assertEquals(
      "Checkout should not cause the modifier to change!",
      initModifier,
      modifier
    );
    ...
    assertEquals(
      "Checkout should not cause the modified date to change!",
      initModified,
      modified
    );
    ...
    assertEquals(
      "Cancel checkout should not cause the modifier to change!",
      initModifier,
      modifier
    );
    modified = nodeService.getProperty(nodeRef, ContentModel.PROP_MODIFIED);
    assertEquals(
      "Cancel checkout should not cause the modified date to change!",
      initModified,
      modified
    );
    ...
    assertEquals(
      "The modifier should NOT change to Admin after checkin!",
      initModifier,
      modifier
    );
  }
  ...
  public void testCheckOutPermissions_ALF7680_ALF535() {
    ...
    NodeRef folder1 = createFolderWithPermission(
      rootNodeRef,
      userName,
    ...
    NodeRef folder2 = createFolderWithPermission(
      rootNodeRef,
      userName,
      PermissionService.ALL_PERMISSIONS
    );
    ...
    NodeRef folder3 = createFolderWithPermission(
      rootNodeRef,
      userName,
      PermissionService.CONSUMER
    );
    try {
      ...
      fail(
        "Node can't be checked out to folder where user has no permissions to create children"
      );
    } catch (Exception e) {
    ...
    NodeRef node2 = createNodeWithPermission(
      folder3,
      userName,
      PermissionService.CONSUMER
    );
    try {
      ...
      // Ensure that the working copy was not created and exception occurs
      fail(
        "Node can't be checked out if user has no permissions to edit document"
      );
    } catch (Exception e) {
      // Exception is expected
    }
  ...
  public void testCheckInLockableAspectDoesntCopies_ALF16194() {
    ...
    if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_LOCKABLE)) {
      fail(
        "Lockable aspect should not be copied from the working copy to the original document"
      );
    }
  }
  ...
  public void testCanCheckInWhenOriginalHasUndeletableAspect() {
    ...
    assertFalse(
      nodeService.hasAspect(workingCopy, ContentModel.ASPECT_UNDELETABLE)
    );
  ...
  private NodeRef createFolderWithPermission(
    NodeRef parent,
    String username,
    String permission
  ) {
    // Authenticate as system user because the current user should not be node owner
    AuthenticationComponent authenticationComponent = (AuthenticationComponent) this.applicationContext.getBean(
        "authenticationComponent"
      );
    authenticationComponent.setSystemUserAsCurrentUser();

    // Create the folder
    NodeRef folder = nodeService
      .createNode(
        parent,
        ContentModel.ASSOC_CHILDREN,
  ...
  private NodeRef createNodeWithPermission(
    NodeRef parent,
    String username,
    String permission
  ) {
  ...
  public void testMoveOriginalWithWorkingCopy() {
    // Create a FolderA
    final NodeRef folderA = createFolder(
      "MoveOriginalWithWorkingCopy_" + GUID.generate()
    );

    // Create a FolderB
    final NodeRef folderB = createFolder(
      "MoveOriginalWithWorkingCopy_" + GUID.generate()
    );
    ...
    final NodeRef origDenied = createContent(
      "original_" + GUID.generate(),
      folderA
    );
    ...
    try {
      movedWorkingCopyAllowed =
        fileFolderService
          .moveFrom(workingCopyAllowed, folderA, folderB, null)
          .getNodeRef();
    } catch (Exception e) {
    ...
    assertEquals(
      folderB,
      nodeService.getPrimaryParent(movedWorkingCopyAllowed).getParentRef()
    );
    ...
    assertEquals(
      folderA,
      nodeService.getPrimaryParent(origAllowed).getParentRef()
    );

    // try to move original - action must be denied
    try {
      fileFolderService
        .moveFrom(origAllowed, folderA, folderB, null)
        .getNodeRef();
        ...
        new RunAsWork<NodeRef>() {
          ...
          public NodeRef doWork() throws Exception {
            return fileFolderService
              .moveFrom(workingCopyDenied, folderA, folderB, null)
              .getNodeRef();
          }
        },
        denyUser
    ...
    try {
      AuthenticationUtil.runAs(
        new RunAsWork<NodeRef>() {
          ...
          public NodeRef doWork() throws Exception {
            return fileFolderService
              .moveFrom(origDenied, folderA, folderB, null)
              .getNodeRef();
          }
        },
        denyUser
  ...
  public void testDeleteUpdateOriginalOfCheckedOutDocument() {
    // Create a FolderA
    final NodeRef folderA = createFolder(
      "DeleteUpdateOriginalOfCheckedOutDocument_" + GUID.generate()
    );
  ...
  public void testDeleteOfWorkingCopy() {
    // Create a FolderA
    final NodeRef folderA = createFolder(
      "DeleteOfWorkingCopy_" + GUID.generate()
    );
  ...
  public void testDeleteWorkingCopyAspect() {
    // Create a FolderA
    final NodeRef folderA = createFolder(
      "DeleteCopiedFromAspectFromWorkingCopy_" + GUID.generate()
    );
    ...
    assertTrue(
      "cm:workingCopy aspect not found on working copy.",
      nodeService.hasAspect(workingCopy, ContentModel.ASPECT_WORKING_COPY)
    );
    assertTrue(
      "cm:copiedFrom aspect not found on working copy.",
      nodeService.hasAspect(workingCopy, ContentModel.ASPECT_COPIEDFROM)
    );
  ...
  public void testDeleteWorkingCopyLinkAssociation() {
    // Create a FolderA
    final NodeRef folderA = createFolder(
      "DeleteOriginalAssociationFromCopy_" + GUID.generate()
    );
    ...
    assertEquals(
      "Did not find cm:workingcopylink",
      1,
      nodeService
        .getSourceAssocs(workingCopy, ContentModel.ASSOC_WORKING_COPY_LINK)
        .size()
    );
    ...
    try {
      ...
      fail(
        "Should not have been allowed to remove the association from working copy to original"
      );
  ...
  public void testCancelCheckoutUnlockedWCopy() {
    ...
    NodeRef folderA = createFolder(
      rootNodeRef,
      "testMnt9502_" + System.currentTimeMillis()
    );
  ...
  public void testDeleteAndRestore() {
    ...
    NodeRef archiveRootNodeRef = nodeService.getStoreArchiveNode(
      spacesStoreRef
    );
    ...
    NodeRef folderRef = createFolder(
      spacesRootNodeRef,
      "testDeleteAndRestore-folder-" + System.currentTimeMillis()
    );
    NodeRef contentRef = createContent(
      "testDeleteAndRestore-content",
      folderRef
    );
    ...
    assertFalse(
      nodeService.hasAspect(contentRef, ContentModel.ASPECT_CHECKED_OUT)
    );

    // checkout content node
    NodeRef workingCopyRef = cociService.checkout(
      contentRef,
      folderRef,
      ContentModel.ASSOC_CHILDREN,
      QName.createQName("workingCopy")
    ...
    assertTrue(
      nodeService.hasAspect(workingCopyRef, ContentModel.ASPECT_COPIEDFROM)
    );
    assertTrue(
      nodeService.hasAspect(contentRef, ContentModel.ASPECT_CHECKED_OUT)
    );
    ...
    contentWriter =
      contentService.getWriter(workingCopyRef, ContentModel.PROP_CONTENT, true);
    contentWriter.setMimetype("text/plain");
    contentWriter.setEncoding("UTF-8");
    ...
    // restore folder
    NodeRef archiveNodeRef = new NodeRef(
    ...
    assertEquals(
      initialText,
      contentService
        .getReader(contentRef, ContentModel.PROP_CONTENT)
        .getContentString()
    );
    assertEquals(
      updatedText,
      contentService
        .getReader(workingCopyRef, ContentModel.PROP_CONTENT)
        .getContentString()
    );

    // belts-and-braces - also show that we can move a folder with checked-out item
    NodeRef folderRef2 = createFolder(
      spacesRootNodeRef,
      "testDeleteAndRestore-folder2-" + System.currentTimeMillis()
    );

...
public class CMISNodeInfoImpl implements CMISNodeInfo {
  ...
  private static final GregorianCalendar DUMMY_DATE = new GregorianCalendar(
    2010,
    4,
    1
  );
    ...
    if (versionHistory == null) {
      ...
      objectId =
        connector.constructObjectId(
          nodeRef,
          CMISConnector.UNVERSIONED_VERSION_LABEL
        );
      ...
      versionLabel =
        (String) getNodeProps().get(ContentModel.PROP_VERSION_LABEL);

      Version headVersion = versionHistory.getHeadVersion();

      objectId =
        connector.constructObjectId(
          headVersion.getVersionedNodeRef(),
          versionLabel
        );
      currentObjectId =
        connector.constructObjectId(
    ...
    try {
      ...
      if (objectId.startsWith(CMISConnector.ASSOC_ID_PREFIX)) {
        ...
        try {
          assocId =
            new Long(
              objectId.substring(CMISConnector.ASSOC_ID_PREFIX.length())
            );
        ...
        if (versionLabel == null) {
          if (isDocument()) {
            ...
            Version currentVersion = connector
              .getVersionService()
        ...
        if (!isNodeVersioned(nodeRef)) {
          ...
          if (connector.filter(nodeRef)) {
            ...
            versionLabel.equals(CMISConnector.UNVERSIONED_VERSION_LABEL)
          ) {
            objecVariant = CMISObjectVariant.CURRENT_VERSION;
          } else {
            objecVariant = CMISObjectVariant.NOT_EXISTING;
          ...
          String headVersionLabel = (String) connector
            .getNodeService()
            .getProperty(nodeRef, ContentModel.PROP_VERSION_LABEL);
          currentObjectId =
            connector.constructObjectId(currentNodeId, headVersionLabel);
        ...
        hasPWC =
          connector
            .getCheckOutCheckInService()
            .isCheckedOut(getCurrentNodeNodeRef());
      }
    ...
    if (nodeRef != null) {
      QName typeQName =
        (
          nodeType != null
            ? nodeType
            : connector.getNodeService().getType(nodeRef)
        );
      ...
      if (path.size() - i == 1) {
        ...
        while (i < path.size() - 1) {
          ...
          if (element instanceof ChildAssocElement) {
            ...
            displayPath.append("/");
            try {
              String propertyName = (String) connector
                .getNodeService()
              ...
              return null;
            } catch (net.sf.acegisecurity.AccessDeniedException e) { // Somewhere this has not been wrapped correctly
              // if the user does not have enough permissions to construct the entire path then the object
              // should have a null path
              return null;
            }
  ...
  public Serializable getModificationDate() {
    if (isDocument()) {
      if (isCurrentVersion() || isPWC()) {
        ...
        return getVersion()
          .getVersionProperty(ContentModel.PROP_MODIFIED.getLocalName());
      }
  ...
  public List<CMISNodeInfo> getParents() {
    ...
    if (parents == null) {
      ...
      NodeRef nodeRefForParent =
        (isCurrentVersion() ? getCurrentNodeNodeRef() : nodeRef);

      List<ChildAssociationRef> nodeParents = connector
        .getNodeService()
        .getParentAssocs(
          nodeRefForParent,

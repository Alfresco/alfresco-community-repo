...
public class FeedCleaner implements NodeServicePolicies.BeforeDeleteNodePolicy {
    ...
    public int execute() throws JobExecutionException {
        ...
        try {
            ...
            JobLockRefreshCallback callback =
        ...
        try {
            ...
            if (maxAgeMins > 0 && keepGoing.get()) {
                ...
                long keepTimeOffset =
                        nowTimeOffset
                                - ((long) maxAgeMins
                                        * 60000L); // millsecs = mins * 60 secs * 1000 msecs
            ...
            if (maxFeedSize > 0 && keepGoing.get()) {
                ...
                List<ActivityFeedEntity> userFeedsTooMany =
                        feedDAO.selectUserFeedsToClean(maxFeedSize);
                for (ActivityFeedEntity userFeedTooMany : userFeedsTooMany) {
                    ...
                    if (logger.isTraceEnabled()) {
                        logger.trace(
                                "Get the feeds to keep for user for all sites, not exluding"
                                        + " users.");
                    }
                    List<ActivityFeedEntity> feedsToKeep =
                            feedDAO.selectUserFeedEntries(
                                    feedUserId, null, false, false, -1L, maxFeedSize);
                    if (logger.isTraceEnabled()) {
                ...
                List<ActivityFeedEntity> siteFeedsTooMany =
                        feedDAO.selectSiteFeedsToClean(maxFeedSize);
                for (ActivityFeedEntity siteFeedTooMany : siteFeedsTooMany) {
                    ...
                    List<ActivityFeedEntity> feedsToKeep =
                            feedDAO.selectSiteFeedEntries(siteId, maxFeedSize);
                    if (logger.isTraceEnabled()) {
                        for (ActivityFeedEntity feedToKeep : feedsToKeep) {
                            logger.trace(
    ...
    public void beforeDeleteNodePerson(NodeRef personNodeRef) {
        ...
        Set<String> deletedUserIds =
                (Set<String>) AlfrescoTransactionSupport.getResource(KEY_DELETED_USER_IDS);
        ...
        public void afterCommit() {
            ...
            if (deletedUserIds != null) {
                for (String user : deletedUserIds) {
                    // MNT-9104 If username contains uppercase letters the action of joining a site
                    // will not be displayed in "My activities"
                    final String userId;
                    if (!userNamesAreCaseSensitive) {

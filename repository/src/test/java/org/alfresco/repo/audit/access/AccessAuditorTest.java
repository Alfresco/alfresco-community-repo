...
public class AccessAuditorTest {
  ...
  private static ServiceRegistry serviceRegistry = (ServiceRegistry) ctx.getBean(
  ...
  private static PolicyComponent policyComponent = (PolicyComponent) ctx.getBean(
    "policyComponent"
  );
  private static AuthenticationComponent authenticationComponent = (AuthenticationComponent) ctx.getBean(
  ...
  public static void setUpBeforeClass() throws Exception {
    ...
    when(auditComponent.recordAuditValues(anyString(), anyMap()))
      .thenAnswer(
        new Answer<Map<String, Serializable>>() {
          public Map<String, Serializable> answer(InvocationOnMock invocation)
            throws Throwable {
    ...
    auditor.setNodeInfoFactory(
      new NodeInfoFactory(nodeService, namespaceService)
    );
  ...
  private static NodeRef newFolder(NodeRef parent, String name) {
    return serviceRegistry
      ...
      .create(parent, name, ContentModel.TYPE_FOLDER)
      .getNodeRef();
  }
  ...
  private static NodeRef newContent(NodeRef parent, String name) {
    ...
    propertyMap0.put(
      ...
      new ContentData(null, "text/plain", 0L, "UTF-16", Locale.ENGLISH)
    );
    NodeRef content = nodeService
      .createNode(
        parent,
    ...
    ContentWriter writer = serviceRegistry
      .getContentService()
      .getWriter(content, ContentModel.TYPE_CONTENT, true);
  ...
  private void assertContains(String expected, Serializable actual) {
    ...
    if (actual == null || !actualString.contains(expected)) {
      throw new ComparisonFailure(
        "Expected not contained in actual.",
        expected,
        actualString
      );
    }
  ...
  public final void test04OnMoveNode() throws Exception {
    serviceRegistry
      .getNodeService()
      .moveNode(content3, folder1, ContentModel.ASSOC_CONTAINS, null); // keep leaf name
    ...
    assertEquals(
      "/cm:homeFolder/cm:folder3/cm:content3",
  ...
  public final void test05OnMoveNodeAndNewName() throws Exception {
    serviceRegistry
      ...
      .moveNode(
        ...
        folder1,
        ContentModel.ASSOC_CONTAINS,
        QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "newName2")
      );
    ...
    assertEquals(
      "/cm:homeFolder/cm:folder1/cm:content3",
      auditMap.get("move/from/path")
    );
    assertEquals("cm:content", auditMap.get("type"));
  }
  ...
  public final void test07OnAddAspect() throws Exception {
    serviceRegistry
      .getNodeService()
      .addAspect(content1, ContentModel.ASPECT_AUTHOR, null);
    serviceRegistry
      .getNodeService()
      .addAspect(content1, ContentModel.ASPECT_OWNABLE, null);
    ...
    assertTrue(
      "Individual author aspect missing",
      auditMap.containsKey("aspects/add/cm:author")
    );
    assertTrue(
      "Individual ownable aspect missing",
      auditMap.containsKey("aspects/add/cm:ownable")
    );
    assertEquals("cm:content", auditMap.get("type"));
  }
  ...
  public final void test08OnRemoveAspect() throws Exception {
    serviceRegistry
      .getNodeService()
      .removeAspect(content1, ContentModel.ASPECT_AUTHOR);
    ...
    assertTrue(
      "Individual author aspect missing",
      auditMap.containsKey("aspects/delete/cm:author")
    );
    assertEquals("cm:content", auditMap.get("type"));
  }
  ...
  public final void test09OnContentUpdate() throws Exception {
    ContentWriter writer = serviceRegistry
      .getContentService()
      .getWriter(content1, ContentModel.TYPE_CONTENT, true);
  ...
  public final void test10OnContentRead() throws Exception {
    serviceRegistry
      .getContentService()
      .getReader(content1, ContentModel.TYPE_CONTENT);
  ...
  public final void test11OnCreateVersion() throws Exception {
    ...
    serviceRegistry
      .getVersionService()
      .createVersion(content1, versionProperties);
    ...
    assertEquals("/cm:homeFolder/cm:folder1/cm:content1", auditMap.get("path"));
    assertTrue(
      "cm:versionable should be a value with in the set",
      ((Set<?>) auditMap.get("aspects/add")).contains(
          ContentModel.ASPECT_VERSIONABLE
        )
    );
    assertTrue(
      "Individual versionable aspect should exist",
      auditMap.containsKey("aspects/add/cm:versionable")
    );
    assertEquals("cm:content", auditMap.get("type"));
  }
  ...
  public final void test12OnCheckOut() throws Exception {
    workingCopyNodeRef =
      serviceRegistry.getCheckOutCheckInService().checkout(content1);
    ...
    boolean origIn0 =
      ((String) auditMapList.get(0).get("path")).endsWith("cm:content1");
    ...
    assertTrue(
      ...
      ((String) workMap.get("path")).endsWith("(Working Copy)") &&
  ...
  public final void test13OnCheckIn() throws Exception {
    ...
    serviceRegistry
      ...
      .checkin(workingCopyNodeRef, checkinProperties);

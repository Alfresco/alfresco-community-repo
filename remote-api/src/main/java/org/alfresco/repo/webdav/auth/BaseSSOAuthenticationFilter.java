...
public abstract class BaseSSOAuthenticationFilter
  ...
  public void setServerConfiguration(
    ExtendedServerConfigurationAccessor serverConfiguration
  ) {
  ...
  public void doFilter(
    ...
    RuntimeContainer container = (RuntimeContainer) appContext.getBean(
      "publicapi.container"
    );
    ...
    try {
      match =
        container
          .getRegistry()
          .findWebScript(req.getMethod(), getScriptUrl(req));
    ...
    // check the authentication required - if none then we don't want any of the filters down the chain to require any authentication checks
      ...
      if (getLogger().isDebugEnabled()) {
        getLogger()
          .debug(
            "Found webscript with no authentication - set NO_AUTH_REQUIRED flag."
          );
      }
    ...
    if (ticket != null && ticket.length() != 0) {
      ...
      try {
        ...
        if (user != null && !ticket.equals(user.getTicket())) {
          if (getLogger().isDebugEnabled()) {
            getLogger()
              .debug("The ticket doesn't match, invalidate the session.");
          }
        ...
        if (user == null) {
          if (getLogger().isDebugEnabled()) {
            getLogger()
              .debug(
                "There is no valid cached user, validate the ticket and create one."
              );
          }
  ...
  protected void redirectToLoginPage(
    ...
    if (hasLoginPage()) res.sendRedirect(
  ...
  protected final boolean isNTLMSSPBlob(byte[] byts, int offset) {
    ...
    if ((byts.length - offset) >= NTLM.Signature.length) {
      ...
      int idx = 0;
      while (
        idx < NTLM.Signature.length && byts[offset + idx] == NTLM.Signature[idx]
      ) idx++;
  ...
  protected synchronized String getServerName() {
    ...
    if (
      ...
      (
        m_lastConfiguredServerName == null &&
        srvName == null ||
    ...
    if (getLogger().isTraceEnabled()) {
      getLogger()
        .trace(
          "Found server name in the file server configuration: " + srvName
        );
    }
    ...
    if (serverConfiguration != null) {
      if (m_lastConfiguredServerName != null) {
        try {
          InetAddress resolved = InetAddress.getByName(
          ...
          if (resolved == null) {
            ...
            m_lastResolvedServerName =
              serverConfiguration.getLocalServerName(true);
          } else {
      ...
      if (m_lastResolvedServerName == null) {
        ...
        if (getLogger().isInfoEnabled()) {
          getLogger()
            .info("NTLM filter using server name " + m_lastResolvedServerName);
        }
      }
    } else {
      // Get the host name
    ...
    if (
      m_lastResolvedServerName == null || m_lastResolvedServerName.length() == 0
    ) {
  ...
  protected SecurityConfigSection getSecurityConfigSection() {
    return serverConfiguration == null
      ? null
      : (SecurityConfigSection) serverConfiguration.getConfigSection(
  ...
  protected boolean performFallbackAuthentication(
    ...
    boolean fallbackSuccess = fallbackDelegate.authenticateRequest(
      context,
      req,
      resp
    );
    ...
    if (!fallbackSuccess) {
      ...
      if (getLogger().isDebugEnabled()) {
        getLogger()
          .debug("Fallback authentication failed. Restarting login...");
      }
    }

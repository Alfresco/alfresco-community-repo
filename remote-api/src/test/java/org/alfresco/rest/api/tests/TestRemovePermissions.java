...
public class TestRemovePermissions extends EnterpriseTestApi {
  ...
  public static final String ATOMPUB_URL_OC =
    "http://{0}:{1}/alfresco/cmisatom";
  ...
  public static final String ACLSERVICE_URL =
    "http://{0}:{1}/alfresco/cmis/ACLService?wsdl";
  ...
  public void testRemoveAllPermissions_WEBSERVICE_10() {
    ...
    try {
      ...
      testFolder =
        createFolder(session, "testRemoveAllPermissions_WEBSERVICE_10");
      List<Ace> acl = create2TestACLs(session);

      // adding new ACE
      testFolder.addAcl(acl, AclPropagation.PROPAGATE);

      Acl allacl = session.getAcl(
        session.createObjectId(testFolder.getId()),
        false
      );
      int oldSize = allacl.getAces().size();

      // Removing ALL ACEs
      Acl newAcl = testFolder.removeAcl(
        allacl.getAces(),

...
public class Deleter {
  ...
  public static void deleteEmptyParents(File file, String rootDir) {
    ...
    do {
      try {
        if (
          parent.isDirectory() && !parent.getCanonicalPath().equals(rootDir)
        ) {
        ...
        log.error(
          "Unable to construct canonical path for " + parent.getAbsolutePath()
        );
        break;
      }
  ...
  public static void deleteEmptyParents(File file, File rootDir) {
    try {
      ...
      String msg =
        "Unable to convert rootDir to canonical form [rootDir=" + rootDir + "]";

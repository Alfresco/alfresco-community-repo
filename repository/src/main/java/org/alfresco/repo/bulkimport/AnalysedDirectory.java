...
public class AnalysedDirectory {
    ...
    public AnalysedDirectory(List<Path> paths) {
        originalPaths = paths;
        // Sort the files/directories so that the *.metadata.properties.xml found later, see
        // ALF-17965 for details.
    ...
    public void addImportableItem(ImportableItem importableItem) {
        if (importableItem.getHeadRevision().contentFileExists()
            ...
            importableDirectories.put(
                    importableItem.getHeadRevision().getContentFile(), importableItem);
        } else {

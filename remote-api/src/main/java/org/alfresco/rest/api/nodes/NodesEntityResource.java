...
public class NodesEntityResource
     ...
     *
     * @param nodeId String id of node (folder or document) - will also accept well-known aliases,
     *     eg. "-root-", "-my-", "-shared-"
     *     <p>Optional parameters: - path
     */
    @WebApiDescription(
    ...
    public BinaryResource readProperty(String fileNodeId, Parameters parameters)
            throws EntityNotFoundException {
     ...
     *
     * <p>This allow binary content update of an existing file/content node.
     *
     * <p>Note: alternatively, can upload via POST (multipart/form-data) with existing file name and
     * form "overwrite=true".
     *
     * @param fileNodeId
     ...
     *
     * <p>Can update name (which is a "rename" and hence must be unique within the current parent
     * folder) or update other properties.
     *
    ...
    @WebApiDescription(
            title = "Delete Node",
            description = "Delete the file or folder with id 'nodeId'. Folder will cascade delete")
    ...
    @WebApiDescription(
            title = "Copy Node",
            description =
                    "Copy one or more nodes (files or folders) to a new target folder, with option"
                            + " to rename.")
    public Node copyById(
            String nodeId, NodeTarget target, Parameters parameters, WithResponse withResponse) {
        return nodes.moveOrCopyNode(
                nodeId, target.getTargetParentId(), target.getName(), parameters, true);
    }
    ...
    @WebApiDescription(
            title = "Move Node",
            description =
                    "Moves one or more nodes (files or folders) to a new target folder, with option"
                            + " to rename.",
            successStatus = HttpServletResponse.SC_OK)
    public Node moveById(
            String nodeId, NodeTarget target, Parameters parameters, WithResponse withResponse) {
        return nodes.moveOrCopyNode(
                nodeId, target.getTargetParentId(), target.getName(), parameters, false);
    }
    ...
    @WebApiDescription(
            ...
            successStatus = HttpServletResponse.SC_OK)
    public Node lock(
            String nodeId, LockInfo lockInfo, Parameters parameters, WithResponse withResponse) {

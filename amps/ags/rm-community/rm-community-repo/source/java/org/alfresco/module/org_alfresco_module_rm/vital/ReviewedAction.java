...
public class ReviewedAction extends RMActionExecuterAbstractBase {
  ...
  protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
    VitalRecordDefinition vrDef = getVitalRecordService()
      .getVitalRecordDefinition(actionedUponNodeRef);
    if (vrDef != null && vrDef.isEnabled()) {
      if (getRecordService().isRecord(actionedUponNodeRef)) {
        ...
        for (NodeRef record : getRecordService()
          .getRecords(actionedUponNodeRef)) {
  ...
  private void reviewRecord(NodeRef nodeRef, VitalRecordDefinition vrDef) {
    ...
    if (
      vrDef.getReviewPeriod().getPeriodType().equals(Immediately.PERIOD_TYPE)
    ) {
      // Log
      ...
      if (reviewAsOf != null) {
        ...
        if (logger.isDebugEnabled()) {
          StringBuilder msg = new StringBuilder();
          msg
            .append("Setting new reviewAsOf property [")
            .append(reviewAsOf)
            .append("] on ")
            .append(nodeRef);
          logger.debug(msg.toString());
        ...
        this.getNodeService()
          .setProperty(nodeRef, PROP_REVIEW_AS_OF, reviewAsOf);

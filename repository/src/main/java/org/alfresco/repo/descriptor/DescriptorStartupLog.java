...
public class DescriptorStartupLog extends AbstractLifecycleBean {
    ...
    protected void onBootstrap(ApplicationEvent event) {
        ...
        String version =
                (properties.get("java.runtime.version") == null)
                        ? "unknown"
                        : (String) properties.get("java.runtime.version");
        ...
        if (logger.isInfoEnabled()) {
            logger.info(
                    String.format(
                            "Alfresco JVM - v%s; maximum heap size %.3fMB", version, maxHeapMB));
        }
        if (logger.isWarnEnabled()) {
            if (version.startsWith("1.2")
                ...
                logger.warn(
                        String.format(
                                "Alfresco JVM - WARNING - v1.5 is required; currently using v%s",
                                version));
            }
            if (maxHeapMB < 500) {
                logger.warn(
                        String.format(
                                "Alfresco JVM - WARNING - maximum heap size %.3fMB is less than"
                                        + " recommended 512MB",
                                maxHeapMB));
            }
        }
        ...
        if (logger.isInfoEnabled()) {
            ...
            Descriptor installedRepoDescriptor =
                    descriptorService.getInstalledRepositoryDescriptor();
            ...
            Object[] params =
                    new Object[] {
                        serverEdition,
                        currentMode != LicenseMode.TEAM
                                ? ""
                                : (" " + currentMode), // only append TEAM

...
public class IncompleteNodeTagger
  ...
  private Map<NodeRef, Set<QName>> getNodes() {
    return (Map<NodeRef, Set<QName>>) AlfrescoTransactionSupport.getResource(
      KEY_NODES
    );
  }
  ...
  public void onCreateNode(ChildAssociationRef childAssocRef) {
    if (
      !storesToIgnore.contains(
        childAssocRef.getChildRef().getStoreRef().toString()
      )
    ) {
  ...
  public void onAddAspect(NodeRef nodeRef, QName aspectTypeQName) {
    if (!storesToIgnore.contains(nodeRef.getStoreRef().toString())) {
      if (aspectTypeQName.equals(ContentModel.ASPECT_INCOMPLETE)) {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Ignoring aspect addition: " + ContentModel.ASPECT_INCOMPLETE
          );
        }
      }
      save(nodeRef);
  ...
  public void onRemoveAspect(NodeRef nodeRef, QName aspectTypeQName) {
    if (!storesToIgnore.contains(nodeRef.getStoreRef().toString())) {
      if (aspectTypeQName.equals(ContentModel.ASPECT_INCOMPLETE)) {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Ignoring aspect removal: " + ContentModel.ASPECT_INCOMPLETE
          );
        }
      }
      save(nodeRef);
  ...
  public void onCreateAssociation(AssociationRef nodeAssocRef) {
    if (
      !storesToIgnore.contains(
        nodeAssocRef.getSourceRef().getStoreRef().toString()
      )
    ) {
  ...
  private void processNode(NodeRef nodeRef, Set<QName> assocTypes) {
    ...
    boolean isTagged = aspectTypeQNames.contains(
      ContentModel.ASPECT_INCOMPLETE
    );
    ...
    if (typeDef == null) {
      throw new AlfrescoRuntimeException(
        "Node type is not recognised: " + nodeTypeQName
      );
    }
    Collection<PropertyDefinition> propertyDefs = typeDef
      .getProperties()
      .values();
    // check them
    ...
    for (QName aspectTypeQName : aspectTypeQNames) {
      ...
      // check them
      boolean aspectPropertiesOK = checkProperties(
        propertyDefs,
        nodeProperties
      );
      // were there outstanding properties to check?
  ...
  private boolean checkAssociation(
    ...
    if (assocDef.isTargetMandatory() && !assocDef.isTargetMandatoryEnforced()) {
      ...
      if (assocDef.isChild()) {
        ...
        List<AssociationRef> targetAssocRefs = nodeService.getTargetAssocs(
          nodeRef,
          assocDef.getName()
        );

...
public class RepositoryDescriptorDAOImpl implements DescriptorDAO {
  ...
  private static Log logger = LogFactory.getLog(
    RepositoryDescriptorDAOImpl.class
  );
  private String name;
  ...
  @Override
  public Descriptor updateDescriptor(
    ...
    try {
      ...
      if (!this.transactionService.isReadOnly()) {
        ...
        props.put(
          ContentModel.PROP_SYS_VERSION_MINOR,
        ...
        props.put(
          ContentModel.PROP_SYS_VERSION_LABEL,
          serverDescriptor.getVersionLabel()
        );
        props.put(
          ContentModel.PROP_SYS_VERSION_BUILD,
          serverDescriptor.getVersionBuild()
        );
        props.put(
          ContentModel.PROP_SYS_VERSION_SCHEMA,
  ...
  public byte[] getLicenseKey() {
        ...
        try {
          ...
          if (os != null) {
            try {
              ...
              logger.debug(
                "getLicenseKey: Error closing ByteArrayOutputStream",
                ignore
              );
            }
          }
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "getLicenseKey: key " +
        (key == null ? "is null" : "length=" + key.length)
      );
    }
    return key;
  }
  ...
  private NodeRef getDescriptorNodeRef(final boolean create) {
    ...
    if (this.nodeService.exists(storeRef)) {
      ...
      } else if (nodeRefs.size() == 0) {} else if (nodeRefs.size() > 1) {
        if (RepositoryDescriptorDAOImpl.logger.isDebugEnabled()) {
          RepositoryDescriptorDAOImpl.logger.debug(
  ...
  private class RepositoryDescriptor extends BaseDescriptor {
    ...
    public String getDescriptor(final String key) {
      ...
      final QName qname = QName.createQName(
        key,
        RepositoryDescriptorDAOImpl.this.namespaceService
      );

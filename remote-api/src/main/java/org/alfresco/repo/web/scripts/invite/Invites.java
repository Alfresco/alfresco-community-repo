...
public class Invites extends DeclarativeWebScript {
  ...
  protected Map<String, Object> executeImpl(
    ...
    } else // parameters // the invite URL request // 'inviteId' has not been provided, so create the query properties from
    ...
    // at this point
    {
      ...
      criteria.setInvitationType(
        InvitationSearchCriteria.InvitationType.NOMINATED
      );
      ...
      if (
        siteShortNameProvided == true &&
        (SiteModel.SITE_MANAGER).equals(
            siteService.getMembersRole(siteShortName, currentUser)
          ) &&
      ...
      for (Invitation invitation : invitations) {
        inviteInfoList.add(
          toInviteInfo(siteInfoCache, (NominatedInvitation) invitation)
        );
      }
    }
  ...
  private InviteInfo toInviteInfo(
    ...
    TemplateNode inviterPerson = getPersonIfAllowed(
      invitation.getInviterUserName()
    );

    // fetch the person node for the invitee
    TemplateNode inviteePerson = getPersonIfAllowed(
      invitation.getInviteeUserName()
    );

    InviteInfo ret = new InviteInfo(
      invitationStatus,
      invitation.getInviterUserName(),
      inviterPerson,
      invitation.getInviteeUserName(),
  ...
  private TemplateNode getPersonIfAllowed(final String userName) {
    ...
    NodeRef inviterRef = AuthenticationUtil.runAs(

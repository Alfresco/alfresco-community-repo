...
public abstract class BaseRemoteStore extends AbstractWebScript {
  ...
  public void execute(WebScriptRequest req, WebScriptResponse res)
    ...
    try {
      ...
      switch (method) {
        ...
        case CREATE:
          ...
          if (logger.isDebugEnabled()) logger.debug(
            "CREATE: content length=" + httpReq.getContentLength()
          );
          createDocument(res, store, path, httpReq.getInputStream());
          break;
        case CREATEMULTI:
          if (logger.isDebugEnabled()) logger.debug(
            "CREATEMULTI: content length=" + httpReq.getContentLength()
          );
        ...
        case UPDATE:
          validatePath(path);
          if (logger.isDebugEnabled()) logger.debug(
            "UPDATE: content length=" + httpReq.getContentLength()
          );
      ...
      throw new WebScriptException(
        "Error during remote store API: " + ioErr.getMessage()
      );
    }
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(
        "Time to execute method: " + (end - start) / 1000000f + "ms"
      );
    }
  }
  ...
  private static void validatePath(String path) {
    if (path == null) {
      throw new WebScriptException(
        "Remote Store expecting document path elements."
      );
    }
  }
   ...
   */
  protected abstract void lastModified(
   ...
   */
  protected abstract void hasDocument(
    WebScriptResponse res,
    String store,
    String path
  ) throws IOException;
   ...
   *
   */
  protected abstract void deleteDocument(
    WebScriptResponse res,
    String store,
    String path
  );

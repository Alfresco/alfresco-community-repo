 ...
 *
 * <p>AuthSSLProtocolSocketFactory can be used to validate the identity of the HTTPS server against
 * a list of trusted certificates and to authenticate to the HTTPS server using a private key.
 *
 * <p>Adapted from code here:
 * http://svn.apache.org/viewvc/httpcomponents/oac.hc3x/trunk/src/contrib/org/apache/commons/httpclient/contrib/ssl/AuthSSLX509TrustManager.java?revision=608014&view=co
 *
 * <p>AuthSSLProtocolSocketFactory will enable server authentication when supplied with a {@link
 * KeyStore truststore} file containing one or several trusted certificates. The client secure
...
public class AuthSSLProtocolSocketFactory implements SecureProtocolSocketFactory {
    ...
    public Socket createSocket(String host, int port) throws IOException, UnknownHostException {
        SSLSocket sslSocket =
                (SSLSocket) getSSLContext().getSocketFactory().createSocket(host, port);
        return sslSocket;
    }

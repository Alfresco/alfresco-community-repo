...
public class ActivityServiceImpl implements ActivityService, InitializingBean {
  ...
  public List<String> getUserFeedEntries(
    ...
    try {
      List<ActivityFeedEntity> activityFeeds = getUserFeedEntries(
        feedUserId,
        siteId,
        excludeThisUser,
        excludeOtherUsers,
      ...
      AlfrescoRuntimeException are = new AlfrescoRuntimeException(
  ...
  public PagingResults<ActivityFeedEntity> getPagedUserFeedEntries(
    ...
    try {
      ...
      PagingResults<ActivityFeedEntity> activityFeedEntries = feedDAO.selectPagedUserFeedEntries(
        feedUserId,
        networkId,
        siteId,
        excludeThisUser,
  ...
  protected NodeRef getUserAvatarNodeRef(
    ActivityFeedEntity activityFeed,
    ...
    if (
      activityFeed
        .getActivityType()
        .equals("org.alfresco.site.user-role-changed")
    ) {
      try {
  ...
  public List<ActivityFeedEntity> getUserFeedEntries(
    ...
    try {
      ...
      List<ActivityFeedEntity> activityFeeds = feedDAO.selectUserFeedEntries(
        feedUserId,
        siteId,
        excludeThisUser,
        excludeOtherUsers,
      ...
      for (ActivityFeedEntity activityFeed : activityFeeds) {
        ...
        if (
          userFilter != null &&
          !userFilter.contains(activityFeed.getPostUserId())
        ) {
          if (logger.isTraceEnabled()) {
            logger.trace(
  ...
  public List<String> getSiteFeedEntries(String siteId) {
    ...
    try {
      ...
      List<ActivityFeedEntity> activityFeeds = feedDAO.selectSiteFeedEntries(
        siteId,
        maxFeedItems
      );
  ...
  public void setFeedControl(FeedControl feedControl) {
    ...
    try {
      if (!existsFeedControl(feedControl)) {
        ...
        feedControlDAO.insertFeedControl(
          new FeedControlEntity(userId, feedControl)
        );
      }
    } catch (SQLException e) {
      AlfrescoRuntimeException are = new AlfrescoRuntimeException(
        "Failed to set feed control: " + e,
        e
      );
      logger.error(are);
      throw are;
  ...
  private List<FeedControl> getFeedControlsImpl(String userId) {
    ...
    try {
      List<FeedControlEntity> feedControlDaos = feedControlDAO.selectFeedControls(
        userId
      );
      if (logger.isDebugEnabled()) {
        logger.debug("Selecting feed controls for userId: " + userId);
      }
      List<FeedControl> feedControls = new ArrayList<FeedControl>(
        feedControlDaos.size()
      );
  ...
  public void unsetFeedControl(FeedControl feedControl) {
    ...
    try {
      ...
      feedControlDAO.deleteFeedControl(
        new FeedControlEntity(userId, feedControl)
      );
    } catch (SQLException e) {
      AlfrescoRuntimeException are = new AlfrescoRuntimeException(
        "Failed to unset feed control: " + e,
        e
      );
      logger.error(are);
      throw are;
  ...
  public boolean existsFeedControl(FeedControl feedControl) {
    ...
    try {
      ...
      long id = feedControlDAO.selectFeedControl(
        new FeedControlEntity(userId, feedControl)
      );
      boolean exists = (id != -1);
      if (logger.isDebugEnabled()) {
      ...
      AlfrescoRuntimeException are = new AlfrescoRuntimeException(
        "Failed to query feed control: " + e,
        e
      );
      logger.error(are);
      throw are;

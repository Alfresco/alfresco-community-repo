...
public class SiteServiceTestHuge {
    private enum Allocation {
        /**
         * Allocate source (groups) to target (sites) on a round robin basis until all targets
         * (sites) have been allocated a single source (group).
         *
         * <p>Some source (groups) might have been allocated to more than one target (site) or none
         * at all.
         */
        ROUND_ROBIN_TO_TARGET,

        /**
         * Allocate source (users) to target (groups) on a round robin basis until all source
         * (users) have been allocated a target (group).
         *
         * <p>Some target (groups) might have been allocated to more than one source (user) or none
         * at all.
         */
        ...
        /**
         * Allocate source (users) to target (groups) on a round robin basis until all source
         * (users) AND all target (groups) have been allocated to at least one a target (group) or
         * one source (user).
         *
         * <p>If there are more target (groups) than source (users) then some source (users) will be
         * in more than one target (group).
         *
         * <p>If there are more source (users) than target (groups) then some target (groups) will
         * contain more than one source (user).
         */
        ROUND_ROBIN_BOTH,

        /**
         * Allocate all source (users) to each target (group). OR Allocate all source (users) to
         * each target (site). OR Allocate all source (groups) to each target (site).
         */
        ALL_TO_EACH,
    ...
    // Used to save having to check if users, groups and sites exist if already created by this
    // test.
    ...
    private static AuthorityService authorityService =
            (AuthorityService) ctx.getBean("AuthorityService");
    ...
    private void createAndAllocate(
            int userCount,
            int groupCount,
            int siteCount,
            Allocation usersToGroups,
            Allocation usersToSites,
            Allocation groupsToSites)
            throws Exception {
        createUsersGroupsAndSites(userCount, groupCount, siteCount);
        allocateUsersToGroupsAndSites(
                userCount, groupCount, siteCount, usersToGroups, usersToSites, groupsToSites);
    }
    ...
    private void allocateUsersToGroupsAndSites(
            int userCount,
            int groupCount,
            int siteCount,
            Allocation usersToGroups,
        ...
        if ((usersToGroups == Allocation.ALL_TO_EACH
            ...
            allocateGroupsToSites(
                    groupCount, siteCount, groupsToSites, userCount, 0, OnFailure.GIVE_UP);
        } else {
    ...
    private void addMoreSites(int sitesToAdd, OnFailure onFailureAction) throws Exception {
        ...
        allocateGroupsToSites(
                ...
                NUM_USERS,
                sitesAlreadyCreated,
                OnFailure.KEEP_GOING);
        assertTime(
                "Add more sites",
                sitesAlreadyCreated + 1,
                sitesToAdd,
                ms,
                MAX_CREATE_SITE_MS + MAX_GROUP_TO_SITE_MS,
                onFailureAction,
                sitesToAdd);
    }
    ...
    private void assertTime(
        ...
        String msg =
                what
                        ...
                        + ms
                        + ",ms"
                        + ((blockSize == 1) ? "" : " average for," + blockSize);
        log(msg);
    ...
    private void createGroups(int groupCount) throws Exception {
        for (int groupId = 1; ; groupId++) {
            ...
            boolean exists =
                    (groupId <= groupsCreated)
                            || authorityService.authorityExists(groupAuthorityName);
    ...
    private void createSites(
            int siteCount, int userCount, int sitesAlreadyCreated, OnFailure onFailureAction)
            throws Exception {
    ...
    private void deleteSites(int fromSiteId, int toSiteId, OnFailure onFailureAction)
            throws Exception {
    ...
    private void creatSite(int siteId, String doAsUser, OnFailure onFailureAction)
            throws Exception {
        String siteName = getSiteName(siteId);
        long ms = System.currentTimeMillis();
    ...
    private void creatSite(String siteName, String doAsUser) throws Exception {
        ...
        UserTransaction txn = transactionService.getUserTransaction();
        try {
            if (doAsUser != null) authenticationComponent.setCurrentUser(doAsUser);
            txn.begin();
            ...
            if (siteService.getSite(siteName) == null) {
                ...
                siteService.createContainer(
                        siteName, "documentLibrary", ContentModel.TYPE_FOLDER, null);
    ...
    private void deleteSite(int siteId, String doAsUser, OnFailure onFailureAction)
            throws Exception {
        String siteName = getSiteName(siteId);
        long ms = System.currentTimeMillis();
    ...
    private void allocateUsersToGroups(int userCount, int groupCount, Allocation allocation)
        ...
        if (allocation == Allocation.ALL_TO_EACH) {
            for (int userId = 1; userId <= userCount; userId++) {
                ...
                try {
                    txn.begin();
                    Set<String> existingAuthorities =
                            authorityService.getContainingAuthoritiesInZone(
                                    AuthorityType.GROUP,
                                    getUserName(userId),
                    ...
                    for (int groupId = 1; groupId <= groupCount; groupId++) {
                        if (!existingAuthorities.contains(
                                authorityService.getName(
                                        AuthorityType.GROUP, getGroupName(groupId)))) {
    ...
    private void allocateGroupsToSites(
            int groupCount,
            int siteCount,
            Allocation allocation,
            int userCount,
            int sitesAlreadyCreated,
            OnFailure onFailureAction)
            throws Exception {
        if (allocation == Allocation.ALL_TO_EACH) {
            for (int groupId = 1; groupId <= groupCount; groupId++) {
            ...
            for (int siteId = sitesAlreadyCreated + 1, groupId = startGroupId;
                    ;
                    siteId++, groupId++) {
                if (groupId > groupCount) {
                    if (targetEnd) break;
    ...
    private void allocateGroupToSite(
            int firstSiteId,
            int userCount,
            String doAsUser,
            OnFailure onFailureAction,
        ...
        try {
            ...
            for (int siteId = firstSiteId; siteId < firstSiteId + blockSize; siteId++) {
                try {
                    ...
                    String groupAuthorityName =
                            authorityService.getName(AuthorityType.GROUP, groupName);
                    siteService.setMembership(
                            siteName, groupAuthorityName, SiteModel.SITE_COLLABORATOR);
                    assertTime(
                            "Adding a group to a site",
                            groupId,
                            siteId,
                            ms2,
                            MAX_GROUP_TO_SITE_MS,
                            onFailureAction,
                            1);
                } catch (DuplicateChildNodeNameException e) {
                    // Already allocated.
    ...
    private void allocateGroupToSite(
            int siteId, int userCount, String doAsUser, OnFailure onFailureAction)
            throws Exception {
        int groupId = (siteId - 1) % NUM_GROUPS + 1;
        allocateGroupToSite(groupId, siteId, userCount, doAsUser, onFailureAction);
    }
    ...
    private void allocateGroupToSite(
            int groupId, int siteId, int userCount, String doAsUser, OnFailure onFailureAction)
            throws Exception {
        try {
            String groupName = getGroupName(groupId);
            String siteName = getSiteName(siteId);
    ...
    //        Allocation groupsToSites = Allocation.ROUND_ROBIN_TO_TARGET;
    //
    //        createAndAllocate(NUM_USERS, NUM_GROUPS, NUM_SITES, usersToGroups, usersToSites,
    // groupsToSites);
    //    }
    ...
    public void commandLine() throws Exception {
        ...
        if ((usersOnly && (from != null || to != null || restart != null))
                || (!usersOnly
                        && (from == null
                                || to == null
                                || (action != null && !sites && !groups && !test)))) {
            System.err.println(
                    "Usage: -Dfrom=<fromSiteId> -Dto=<toSiteId>                     ["
                            + " -Dlog=<logFilename> ]\n"
                            + "       -Daction=usersOnly                                      ["
                            + " -Dlog=<logFilename> ]\n"
                            + "       -Daction=sites      -Dfrom=<fromSiteId> -Dto=<toSiteId> ["
                            + " -Dlog=<logFilename> ] [ -Drestart=<restartAtSiteId> ]\n"
                            + "       -Daction=groups     -Dfrom=<fromSiteId> -Dto=<toSiteId> ["
                            + " -Dlog=<logFilename> ] [ -Drestart=<restartAtSiteId> ]      "
                            + " -Daction=test       -Dfrom=<fromSiteId> -Dto=<toSiteId> ["
                            + " -Dlog=<logFilename> ] ");
        } else {
            try {
                ...
                if (test) {
                    ...
                    if (groups) {
                        int blockSize = 10;
                        for (int siteId = getNextSiteToAddGroupTo(restartFromId);
                                siteId <= NUM_SITES;
                                siteId += blockSize) {
                            int size = Math.min(blockSize, NUM_SITES - siteId + 1);
                            allocateGroupToSite(
                                    siteId, NUM_USERS, ADMIN_USER, OnFailure.KEEP_GOING, size);
                        }
                    }
                }
            } catch (Exception e) {
     ...
     *
     * <p>set
     * SITE_CPATH=%TOMCAT_HOME%/lib/*;%TOMCAT_HOME%/endorsed/*;%TOMCAT_HOME%/webapps/alfresco/WEB-INF/lib/*;%TOMCAT_HOME%/webapps/alfresco/WEB-INF/classes;%TOMCAT_HOME%/shared/classes;
     * java -Xmx2048m -XX:MaxPermSize=512M -classpath %SITE_CPATH%

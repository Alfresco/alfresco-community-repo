...
public class PreventActionsOnFrozenContentTests extends BaseRMRestTest {
    private static final String HOLD_ONE =
            "HOLD" + generateTestPrefix(PreventActionsOnFrozenContentTests.class);
    ...
    @Autowired private RMRolesAndActionsAPI rmRolesAndActionsAPI;
    ...
    public void preconditionForPreventActionsOnFrozenContent() {
        ...
        folderModel = dataContent.usingAdmin().usingSite(testSite).createFolder();

        STEP("Create a record folder with some records");
        recordFolder = createCategoryFolderInFilePlan();
        recordFrozen =
                createElectronicRecord(recordFolder.getId(), getRandomName("elRecordFrozen"));
        recordNotHeld =
                createElectronicRecord(recordFolder.getId(), getRandomName("elRecordNotHeld"));
    ...
    /**
     * Given active content on hold When I try to edit the properties Or perform an action that
     * edits the properties Then I am not successful
     */
    @Test
    ...
    public void copyFrozenFile() {
        ...
        String postBody =
                JsonBodyGenerator.keyValueJson("targetParentId", folderModel.getNodeRef());
        ...
        getRestAPIFactory()
                .getRmRestWrapper()
                .assertLastError()
                .containsSummary("Permission was denied");
    }
    ...
    public void moveFrozenFile() throws Exception {
        ...
        getRestAPIFactory()
                .getNodeAPI(contentHeld)
                .move(createBodyForMoveCopy(folderModel.getNodeRef()));
        ...
        getRestAPIFactory()
                .getRmRestWrapper()
                .assertLastError()
                .containsSummary("Frozen content can't be moved.");
    }

    /**
     * Given a record folder with a frozen record and another record not held When I update the
     * record folder and make the records as vital Then I am successful and the records not held are
     * marked as vital And the frozen nodes have the vital record search properties updated
     */
    @Test
    ...
    public void updateRecordFolderVitalProperties() {
        ...
        RecordFolder recordFolderToUpdate =
                RecordFolder.builder()
                        .properties(
                                RecordFolderProperties.builder()
        ...
        RecordFolder updatedRecordFolder =
                getRestAPIFactory()
                        .getRecordFolderAPI()
                        .updateRecordFolder(recordFolderToUpdate, recordFolder.getId());
        assertStatusCode(OK);
        ...
        assertTrue(
                recordFrozen
                        .getProperties()
                        .getRecordSearchVitalRecordReviewPeriod()
                        .contains("month"));
        assertTrue(
                recordFrozen
                        .getProperties()
                        .getRecordSearchVitalRecordReviewPeriodExpression()
                        .contains("1"));
    ...
    /**
     * Given a record folder with a frozen record and another record not held When I add a
     * disposition schedule Then I am successful And the record search disposition schedule
    ...
    public void createDispositionScheduleOnCategoryWithHeldChildren() {
        ...
        dispositionScheduleService.addDestroyWithGhostingImmediatelyAfterCutOff(
                categoryWithRS.getName());

        STEP("Check the record folder has a disposition schedule");
        RecordFolder folderWithRS =
                getRestAPIFactory().getRecordFolderAPI().getRecordFolder(recordFolder.getId());
    ...
    /**
     * Given a record category with a disposition schedule applied to records And the disposition
     * schedule has a retain step immediately and destroy step immediately And a complete record
     * added to one hold When I execute the retain action Then the action is executed And the record
     * search disposition schedule properties are updated
     */
    @Test
    ...
    public void retainActionOnFrozenHeldRecords() {
        ...
        dispositionScheduleService.addRetainAfterPeriodStep(
                categoryWithRS.getName(), "immediately");
        dispositionScheduleService.addDestroyWithGhostingImmediatelyAfterCutOff(
                categoryWithRS.getName());

        STEP("Create record folder with a record.");
        RecordCategoryChild folder =
                createFolder(categoryWithRS.getId(), getRandomName("RecFolder"));
        ...
        rmRolesAndActionsAPI.executeAction(
                getAdminUser().getUsername(),
                getAdminUser().getPassword(),
                record.getName(),

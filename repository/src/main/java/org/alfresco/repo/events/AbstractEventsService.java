...
public abstract class AbstractEventsService extends TransactionListenerAdapter {
  ...
  protected static final String TRANSACTION_HAS_EVENTS =
  ...
  public void beforeCommit(boolean readOnly) {
    if (sendEventsBeforeCommit) {
      // send all events
      final TxnEvents transactionEvents = (TxnEvents) AlfrescoTransactionSupport.getResource(
        EVENTS_KEY
      );
      if (transactionEvents != null) {
        List<Event> filteredEvents = filterEventsBeforeSend(
  ...
  protected Client getAlfrescoClient(Client knownClientType) {
    ...
    Object noAlfrescoClientIdHint = AlfrescoTransactionSupport.getResource(
    ...
    if (context != null) {
      ...
      if (request != null) {
        String alfrescoClientId = (String) request.getHeader(
          "alfrescoClientId"
        );
  ...
  public void afterCommit() {
    if (sendEventsBeforeCommit) {
      ...
      final Event event = new TransactionCommittedEvent(
        nextSequenceNumber(),
        txnId,
        networkId,
        timestamp,
  ...
  public void afterRollback() {
    ...
    Event event = new TransactionRolledBackEvent(
      nextSequenceNumber(),
      txnId,
      networkId,
      timestamp,
    ...
    try {
      ...
      TxnEvents events = (TxnEvents) AlfrescoTransactionSupport.getResource(
        EVENTS_KEY
      );
      if (events != null) {
        events.clear();
  ...
  protected class NodeInfo {
    ...
    public List<String> getPaths() {
      if (stringPaths == null && nodePaths != null) {
        stringPaths =
          AbstractEventsService.this.getPaths(nodePaths, getToAppend());
      }
    ...
    public List<List<String>> getParentNodeIds() {
      if (parentNodeIds == null && nodePaths != null) {
        parentNodeIds =
          AbstractEventsService.this.getNodeIdsFromParent(nodePaths);
      }
   ...
   */
  protected void retrieveAdditionalProps(
      ...
      String displayPath = AuthenticationUtil.runAsSystem(
        new RunAsWork<String>() {
          ...
          public String doWork() throws Exception {
            String displayPath = path.toDisplayPath(
              nodeService,
              permissionService
            );
            return displayPath;
  ...
  protected List<List<String>> getNodeIdsFromParent(List<Path> nodePaths) {
    // TODO use fileFolderService.getNamePath instead?
    List<List<String>> pathNodeIds = new ArrayList<List<String>>(
      nodePaths.size()
    );
  ...
  protected NodeInfo getNodeInfo(
    ...
    NodeInfo nodeInfo = AuthenticationUtil.runAsSystem(
      new RunAsWork<NodeInfo>() {
        public NodeInfo doWork() throws Exception {
          ...
          if (!includeEventType(eventType)) {
            ...
            Visibility visibility = hiddenAspect.getVisibility(
              filterclient,
              nodeRef
            );
            ...
            if (!typeMatches(type)) {
              ...
              Date modifiedTime = (Date) nodeService.getProperty(
                nodeRef,
                ContentModel.PROP_MODIFIED
              );
              Long modificationTimestamp =
                (modifiedTime != null ? modifiedTime.getTime() : null);

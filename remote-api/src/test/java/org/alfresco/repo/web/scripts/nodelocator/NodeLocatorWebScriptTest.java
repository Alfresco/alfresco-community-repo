...
public class NodeLocatorWebScriptTest extends BaseWebScriptTest {
  ...
  public void testAncestorOfTypeNodeLocator() throws Exception {
    ...
    try {
      ...
      String encodedContentType = URLEncoder.encode(
        ContentModel.TYPE_CONTENT.toPrefixString(namespaceService)
      );
      ...
      String encodedSysFolderType = URLEncoder.encode(
        ContentModel.TYPE_SYSTEM_FOLDER.toPrefixString(namespaceService)
      );
      String sysFolderAncestorUrl =
        ancestorUrl + "?type=" + encodedSysFolderType;
      checkNodeLocator(sysFolderAncestorUrl, sysFolder);

      // Set aspect to cm:ownable. Should not match any node.
      String encodedOwnableAspect = URLEncoder.encode(
        ContentModel.ASPECT_OWNABLE.toPrefixString(namespaceService)
      );
      String ownableAncestorUrl =
        ancestorUrl + "?aspect=" + encodedOwnableAspect;
      ...
      String ownableSysFolderAncestorUrl =
        sysFolderAncestorUrl + "&aspect=" + encodedOwnableAspect;
      checkNodeLocator(ownableSysFolderAncestorUrl, null);

      // Set aspect to cm:ownable and type to cm:folder. Should find folder node.
      String encodedFOlderType = URLEncoder.encode(
        ContentModel.TYPE_FOLDER.toPrefixString(namespaceService)
      );
      String ownableFolderAncestorUrl =
        ownableAncestorUrl + "&type=" + encodedFOlderType;
  ...
  public void testXPathNodeLocator() throws Exception {
    ...
    try {
      ...
      String defaultStoreUrl =
        baseURL + XPathNodeLocator.NAME + "?query=" + encodedPath;
      checkNodeLocator(defaultStoreUrl, content);

      // Check specified store ref works.
      String storeIdUrl =
        defaultStoreUrl + "&store_type=workspace&store_id=SpacesStore";
      checkNodeLocator(storeIdUrl, content);

      // Check node store ref works.
      String nodePathUrl =
        makeUrl(companyHome, XPathNodeLocator.NAME) + "?query=" + encodedPath;
  ...
  private String makeUrl(NodeRef node, String locatorName) {
    ...
    url
      .append(storeRef.getProtocol())
      .append("/")
      .append(storeRef.getIdentifier())
      .append("/")
      .append(node.getId())
      .append("/")
      .append("nodelocator")
  ...
  protected void setUp() throws Exception {
    ...
    this.nodeArchiveService =
      (NodeArchiveService) appContext.getBean("nodeArchiveService");
  }
}

...
public class RecordsManagementQueryDAOImpl
  ...
  private static final Log logger = LogFactory.getLog(
    RecordsManagementQueryDAOImpl.class
  );

  /**
   * query names
   */
  private static final String COUNT_IDENTIFIER =
    "alfresco.query.rm.select_CountRMIndentifier";
  ...
  @Override
  public Set<String> getChildrenStringPropertyValues(
    NodeRef parent,
    QName property
  ) {
    ...
    Pair<Long, NodeRef> nodePair = nodeDAO.getNodePair(
      tenantService.getName(parent)
    );
    if (nodePair == null) {
    ...
    // Perform the query
    return new HashSet<>(
      template.selectList(GET_CHILDREN_PROPERTY_VALUES, queryParams)
    );
  }
  ...
  public Set<NodeRef> getNodeRefsWhichReferenceContentUrl(String contentUrl) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Executing query " + SELECT_NODE_IDS_WHICH_REFERENCE_CONTENT_URL
      );
    }

    // Get all the node ids which reference the given content url
    List<Long> nodeIds = template.selectList(
    ...
    Set<NodeRef> nodesReferencingContentUrl = new HashSet<NodeRef>(
      nodeIds.size()
    );
    for (Long nodeId : nodeIds) {
      ...
      if (nodeId != null && nodeDAO.exists(nodeId)) {
        ...
        NodeRef version2FrozenNodeRef = (NodeRef) nodeDAO.getNodeProperty(
          nodeId,
          Version2Model.PROP_QNAME_FROZEN_NODE_REF
        );
        if (
          version2FrozenNodeRef != null && nodeDAO.exists(version2FrozenNodeRef)
        ) {
  ...
  public List<NodeRef> getRecordFoldersWithSchedules(Long start, Long end) {
    ...
    params.put(
      "processed",
      qnameDAO.getQName(ASPECT_DISPOSITION_PROCESSED).getFirst()
    );
    params.put(
      "folderQnameId",
      qnameDAO.getQName(TYPE_RECORD_FOLDER).getFirst()
    );
    params.put("start", start);
    params.put("end", end);

    List<NodeRefEntity> entities = template.selectList(
      SCHEDULED_FOLDERS,
      params
    );
   ...
   */
  public PropertyStringValueEntity getPropertyStringValueEntity(
    String stringValue
  ) {

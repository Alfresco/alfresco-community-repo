...
public class UpdatePolicy extends AbstractBasePolicy {
  ...
  public int evaluate(
    ...
    if (cad.getParameters().size() > 2 && cad.getParameters().get(2) > -1) {
      properties =
        getProperties(invocation, params, cad.getParameters().get(2));
    }

    UpdateCapability updateCapability = (UpdateCapability) getCapabilityService()

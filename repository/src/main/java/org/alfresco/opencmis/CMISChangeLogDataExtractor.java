...
/**
 * An extractor that allows to filter data using the following rule: Audit records should only be
 * created for items in the CMIS domain model.
 *
    ...
    public boolean isSupported(Serializable data) {
        if (data != null) {
            ...
            if (nodeRef != null) {
                ...
                return (type != null)
                        && (type.getBaseTypeId() == BaseTypeId.CMIS_DOCUMENT
                                || type.getBaseTypeId() == BaseTypeId.CMIS_FOLDER);
            }
        }
        return false;
    }

...
/**
 * Authenticator to provide Remote User based Header authentication dropping back to Basic Auth
 * otherwise. Statelessly authenticating via a secure header now does not require a Session so can
 * be used with request-level load balancers which was not previously possible.
 *
 * <p>
 *
 * @see web-scripts-application-context.xml and web.xml - bean id
 *     'webscripts.authenticator.remoteuser'
 *     <p>This authenticator can be bound to /service and does not require /wcservice (Session)
 *     mapping.
 * @since 5.1
 * @author Kevin Roast
 */
public class RemoteUserAuthenticatorFactory extends BasicHttpAuthenticatorFactory {
    ...
    public void setAlwaysAllowBasicAuthForAdminConsole(
            boolean alwaysAllowBasicAuthForAdminConsole) {
    ...
    /**
     * Remote User authenticator - adds header authentication onto Basic Auth. Stateless does not
     * require Session.
     *
     * @author Kevin Roast
     */
    public class RemoteUserAuthenticator extends BasicHttpAuthenticator {
        ...
        public boolean authenticate(RequiredAuthentication required, boolean isGuest) {
            ...
            if (isRemoteUserMapperActive()) {
                if (isAlwaysAllowBasicAuthForAdminConsole()) {
                    final boolean useTimeoutForAdminAccessingAdminConsole =
                            shouldUseTimeoutForAdminAccessingAdminConsole(required, isGuest);

                    if (useTimeoutForAdminAccessingAdminConsole
                    ...
                    // retrieve the remote user if configured and available - authenticate that user
                    // directly
            ...
            if (userId != null) {
                try {
                    ...
                    listener.userAuthenticated(
                            new TicketCredentials(authenticationService.getCurrentTicket()));
                    authenticated = true;
                } catch (AuthenticationException authErr) {
                    // don't propagate if the user is disabled
                    Throwable disabledCause =
                            ExceptionStackUtil.getCause(authErr, DisabledException.class);
                ...
                if (session != null) {
                    try {
                        SessionUser user =
                                (SessionUser)
                                        session.getAttribute(
                                                AuthenticationDriver.AUTHENTICATION_USER);
                        if (user != null) {
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "An Authentication error occur. Removing User session.",
                                    authErr);
                        }
        ...
        private boolean callBasicAuthForAdminConsoleAccess(
                RequiredAuthentication required, boolean isGuest) {
            // return REST call, after a timeout/basic auth challenge
            if (logger.isTraceEnabled()) {
                logger.trace(
                        "An Admin Console request has come in with Basic Auth headers present for"
                                + " an admin user.");
            }
        ...
        private boolean shouldUseTimeoutForAdminAccessingAdminConsole(
                RequiredAuthentication required, boolean isGuest) {
            boolean useTimeoutForAdminAccessingAdminConsole =
                    RequiredAuthentication.admin.equals(required)
                            && !isGuest
                            && servletReq.getServiceMatch() != null
                            && isAdminConsoleWebScript(servletReq.getServiceMatch().getWebScript());

            if (logger.isTraceEnabled()) {
                logger.trace(
                        "Should ensure that the admins can login with basic auth: "
        ...
        private boolean isRemoteUserMapperActive() {
            return remoteUserMapper != null
                    && (!(remoteUserMapper instanceof ActivateableBean)
                            || ((ActivateableBean) remoteUserMapper).isActive());
        }

        protected boolean isAdminConsoleWebScript(WebScript webScript) {
            if (webScript == null
                    || adminConsoleScriptFamilies == null
                    || webScript.getDescription() == null
            ...
            if (logger.isTraceEnabled()) {
                logger.trace(
                        "WebScript: "
                                + webScript
                                + " has these families: "
                                + webScript.getDescription().getFamilys());
            }
        ...
        protected String getRemoteUserWithTimeout(boolean useTimeout)
            ...
            if (workerGettingTheRemoteUser.isAlive()) {
                ...
                final String message =
                        "Could not get the remote user in a reasonable time: "
                                + getRemoteUserTimeoutMilliseconds
                                + " milliseconds. Adjust the timeout property"
                                + " 'authentication.getRemoteUserTimeoutMilliseconds' if required.";

                if (logger.isWarnEnabled()) {
                    logger.warn(
                            "Returning basic auth challenge for Admin Console. Cause: " + message);
                }
        ...
        /**
         * Retrieve the remote user from servlet request header when using a secure connection. The
         * RemoteUserMapper bean must be active and configured.
         *
        ...
        protected String getRemoteUser() {
            ...
            // If the remote user mapper is configured, we may be able to map in an externally
            // authenticated user
            if (isRemoteUserMapperActive()) {
            ...
            if (logger.isDebugEnabled()) {
                String message =
                        (userId == null)
                                ...
                                : "Extracted external user ID from request: "

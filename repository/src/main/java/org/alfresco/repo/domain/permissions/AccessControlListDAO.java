...
public interface AccessControlListDAO {
     ...
     *
     * @param nodeRef The reference to the node.
     * @param aclId The ID of the ACL entity.
    ...
    /**
     * Set the inheritance on a given node and it's children. If the operation takes too long and
     * asyncCall parameter set accordingly, fixed ACLs method will be synchronously called.
     */
    public List<AclChange> setInheritanceForChildren(
            NodeRef parent, Long inheritFrom, Long sharedAclToReplace, boolean asyncCall);

    /**
     * Set the inheritance on a given node and it's children. If an unexpected ACL occurs in a
     * child, it can be overriden by setting forceSharedACL
     */
    public List<AclChange> setInheritanceForChildren(
            NodeRef parent,
            Long inheritFrom,

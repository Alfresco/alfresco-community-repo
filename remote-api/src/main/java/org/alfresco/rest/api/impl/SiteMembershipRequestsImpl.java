...
public class SiteMembershipRequestsImpl implements SiteMembershipRequests {
  ...
  private static final Log logger = LogFactory.getLog(
    SiteMembershipRequestsImpl.class
  );
  ...
  private static final Set<String> LIST_SITE_MEMBERSHIPS_EQUALS_QUERY_PROPERTIES = new HashSet<>(
    Arrays.asList(new String[] { PARAM_SITE_ID, PARAM_PERSON_ID })
  );
  ...
  private SiteMembershipRequest inviteToPublicSite(
    ...
    String siteCreator = (String) nodeService.getProperty(
      siteNodeRef,
      ContentModel.PROP_CREATOR
    );

    final String siteNetwork = networksService.getUserDefaultNetwork(
      siteCreator
    );
  ...
  public SiteMembershipRequest createSiteMembershipRequest(
    ...
    if (invitation != null) {
      // yes
      throw new InvalidArgumentException(
        inviteeId + " is already invited to site " + siteId
      );
    }
    ...
    if (siteVisibility.equals(SiteVisibility.MODERATED)) {
      request =
        inviteToModeratedSite(message, inviteeId, siteId, inviteeRole, null);
  ...
  public SiteMembershipRequest createSiteMembershipRequest(
    ...
    if (siteVisibility.equals(SiteVisibility.MODERATED)) {
      request =
        inviteToModeratedSite(message, inviteeId, siteId, inviteeRole, client);
    } else if (siteVisibility.equals(SiteVisibility.PUBLIC)) {
      request = inviteToPublicSite(siteInfo, message, inviteeId, inviteeRole);
    } else {
      // note: security, no indication that this is a private site
  ...
  public SiteMembershipRequest updateSiteMembershipRequest(
    ...
    try {
      ModeratedInvitation updatedInvitation = invitationService.updateModeratedInvitation(
        inviteeId,
        siteId,
        message
      );
  ...
  public SiteMembershipRequest getSiteMembershipRequest(
    ...
    SiteInfo siteInfo = AuthenticationUtil.runAsSystem(
    ...
    if (siteInfo.getVisibility().equals(SiteVisibility.MODERATED)) {
      ...
      if (invitation instanceof ModeratedInvitation) {
        ...
        SiteMembershipRequest siteInvite = getSiteMembershipRequest(
          moderatedInvitation
        );
        return siteInvite;
      } else {
    ...
    if (resourceType.equals(ResourceType.WEB_SITE)) {
      ...
      if (siteInfo.getVisibility().equals(SiteVisibility.MODERATED)) {
        ...
        siteMembershipRequest.setMessage(
          moderatedInvitation.getInviteeComments()
        );
        siteMembershipRequest.setCreatedAt(moderatedInvitation.getCreatedAt());
        siteMembershipRequest.setModifiedAt(
          moderatedInvitation.getModifiedAt()
        );

        if (includePersonDetails) {
          Person person = people.getPerson(
  ...
  private List<SiteMembershipRequest> toSiteMembershipRequests(
    ...
    for (Invitation invitation : invitations) {
      if (invitation instanceof ModeratedInvitation) {
        ModeratedInvitation moderatedInvitation = (ModeratedInvitation) invitation;
        SiteMembershipRequest siteMembershipRequest = getSiteMembershipRequest(
          moderatedInvitation,
  ...
  public CollectionWithPagingInfo<SiteMembershipRequest> getPagedSiteMembershipRequests(
    ...
    return createPagedSiteMembershipRequests(
      toSiteMembershipRequests(invitations),
      paging
    );
  }
  ...
  public CollectionWithPagingInfo<SiteMembershipRequest> getPagedSiteMembershipRequests(
    ...
    if (q != null) {
      ...
      String siteId = propertyWalker.getProperty(
        PARAM_SITE_ID,
      ...
      String personId = propertyWalker.getProperty(
        PARAM_PERSON_ID,
        WhereClauseParser.EQUALS,
        String.class
      );
    ...
    List<Invitation> invitations = invitationService.searchInvitation(criteria);
    return createPagedSiteMembershipRequests(
      toSiteMembershipRequests(invitations, true),
  ...
  public void approveSiteMembershipRequest(
    ...
    if (
      !resourceType.equals(ResourceType.WEB_SITE) ||
  ...
  private MapBasedQueryWalker createSiteMembershipRequestQueryWalker() {
    return new MapBasedQueryWalker(
      LIST_SITE_MEMBERSHIPS_EQUALS_QUERY_PROPERTIES,
      null
    );
  }

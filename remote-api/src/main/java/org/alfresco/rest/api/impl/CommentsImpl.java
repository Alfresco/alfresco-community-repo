...
public class CommentsImpl implements Comments {
  ...
  private Comment toComment(
    ...
    Map<String, Boolean> map = commentService.getCommentPermissions(
      nodeRef,
      commentNodeRef
    );
  ...
  public Comment createComment(String nodeId, Comment comment) {
    ...
    if (!typeConstraint.matches(nodeRef)) {
      throw new UnsupportedResourceOperationException(
        "Cannot comment on this node"
      );
    }

    try {
      NodeRef commentNode = commentService.createComment(
        nodeRef,
        comment.getTitle(),
        comment.getContent(),
        false
  ...
  public CollectionWithPagingInfo<Comment> getComments(
    String nodeId,
    Paging paging,
    List<String> include
  ) {
    ...
    /* MNT-10536 : fix */
    final Set<QName> contentAndFolders = new HashSet<>(
      Arrays.asList(ContentModel.TYPE_FOLDER, ContentModel.TYPE_CONTENT)
    );
    if (!nodes.nodeMatches(nodeRef, contentAndFolders, null)) {
      throw new InvalidArgumentException(
        "NodeId of folder or content is expected"
      );
    }

    PagingRequest pagingRequest = Util.getPagingRequest(paging);
    final PagingResults<NodeRef> pagingResults = commentService.listComments(
      nodeRef,
      pagingRequest
    );
  ...
  public void deleteComment(String nodeId, String commentNodeId) {
    try {
      ...
      if (
        !nodeRef.equals(commentService.getDiscussableAncestor(commentNodeRef))
      ) {
        throw new InvalidArgumentException(
          "Unexpected " + nodeId + "," + commentNodeId
        );
      }

...
public class CachedContentCleanupJobTest {
  ...
  public void filesNotInCacheAreDeleted() throws InterruptedException {
    ...
    assertEquals(
      "Incorrect number of deleted files",
      numFiles,
      cleaner.getNumFilesDeleted()
    );
    assertEquals(
  ...
  public void standardCleanAfterAggressiveFinished()
    ...
    for (int i = 0; i < numFiles; i++) {
      ...
      if (i >= 21 && i <= 24) {
        ...
        files[i] =
          createCacheFile(calendar, UrlSource.REVERSE_CACHE_LOOKUP, true);
      }
    }
    ...
    for (int i = 0; i < numFiles; i++) {
      if (i < 7) {
        assertFalse(
          "First 7 files should have been aggressively cleaned",
          files[i].exists()
        );
      }
      ...
      if (i >= 21 && i <= 24) {
        assertFalse(
          "Files with indexes 21-24 should have been deleted",
          files[i].exists()
        );
      }
    }
  ...
  public void emptyParentDirectoriesAreDeleted() throws FileNotFoundException {
    ...
    assertTrue(
      ...
      new File(cacheRoot, "243235984/a/b/c").exists()
    );
    ...
    assertFalse(
      ...
      new File(cacheRoot, "243235984").exists()
    );
  }
  ...
  private void checkFilesDeleted(File file) {
    ...
    assertFalse(
      "Properties file should have been deleted, cache file: " + file,
      props.exists()
    );
  }

  private void checkWatchCountForCacheFile(
    File file,
    Integer expectedWatchCount
  ) {

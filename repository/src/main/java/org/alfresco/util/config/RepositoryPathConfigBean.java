...
public class RepositoryPathConfigBean {
     ...
     *
     * @param storeRef the Alfresco store name
     */
    ...
    public void setRootPath(String path) {
        ...
        if (!path.startsWith("/")) {
            throw new IllegalArgumentException(
                    "Propety 'path' must start with '/' - it is a path relative to the store"
                            + " root.");
        }
        this.rootPath = path;
    }
     ...
     */
    public NodeRef resolveNodePath(
            NamespaceService namespaceService,
            NodeService nodeService,
            SearchService searchService) {
        NodeRef rootNodeRef = nodeService.getRootNode(store);
        List<NodeRef> nodeRefs =
                searchService.selectNodes(rootNodeRef, rootPath, null, namespaceService, true);
        if (nodeRefs.size() == 0) {
            return null;
        } else {

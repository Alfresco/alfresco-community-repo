...
/**
 * A base class for chaining authentication services. Where appropriate, methods will 'chain' across
 * multiple {@link AuthenticationService} instances, as returned by {@link
...
public abstract class AbstractChainingAuthenticationService extends AbstractAuthenticationService
    ...
    public void createAuthentication(String userName, char[] password)
        ...
        if (getMutableAuthenticationService() == null) {
            throw new AuthenticationException(
                    "Unable to create authentication as there is no suitable authentication"
                            + " service.");
        }
    ...
    public void updateAuthentication(String userName, char[] oldPassword, char[] newPassword)
        ...
        if (getMutableAuthenticationService() == null) {
            throw new AuthenticationException(
                    "Unable to update authentication as there is no suitable authentication"
                            + " service.");
        }
    ...
    public void deleteAuthentication(String userName) throws AuthenticationException {
        if (getMutableAuthenticationService() == null) {
            throw new AuthenticationException(
                    "Unable to delete authentication as there is no suitable authentication"
                            + " service.");
        }
    ...
    public void setAuthenticationEnabled(String userName, boolean enabled)
        ...
        if (getMutableAuthenticationService() == null) {
            throw new AuthenticationException(
                    "Unable to set authentication enabled as there is no suitable authentication"
                            + " service.");
        }
    ...
    public boolean isAuthenticationMutable(String userName) {
        ...
        return mutableAuthenticationService == null
                ? false
                : mutableAuthenticationService.isAuthenticationMutable(userName);
    }
    ...
    public boolean isAuthenticationCreationAllowed() {
        ...
        return mutableAuthenticationService == null
                ? false
                : mutableAuthenticationService.isAuthenticationCreationAllowed();
    }
        ...
        for (AuthenticationService authService : getUsableAuthenticationServices()) {
            try {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Not that important: Exception in chaining the verification of"
    ...
    public void authenticate(String userName, char[] password) throws AuthenticationException {
        ...
        List<AuthenticationService> usableAuthenticationServices =
                getUsableAuthenticationServices();
     ...
     * @return the ID of the authService. This implementation has no way to work this out so returns
    ...
    public void authenticateAsGuest() throws AuthenticationException {
        ...
        for (AuthenticationService authService : getUsableAuthenticationServices()) {
            try {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Not that important: Exception in chaining the authenticate as guest."
                                    + " Message: "
                                    + e.getMessage(),
                            e);
                }
    ...
    public String getCurrentUserName() throws AuthenticationException {
        for (AuthenticationService authService : getUsableAuthenticationServices()) {
            try {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Not that important: Exception getting the current username. Message: "
                                    + e.getMessage(),
                            e);
                }
            }
    ...
    public void invalidateUserSession(String userName) throws AuthenticationException {
        for (AuthenticationService authService : getUsableAuthenticationServices()) {
            try {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Not that important: Exception invalidating the user session. Message: "
                                    + e.getMessage(),
                            e);
                }
            }
    ...
    public void invalidateTicket(String ticket) throws AuthenticationException {
        for (AuthenticationService authService : getUsableAuthenticationServices()) {
            try {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Not that important: Exception invalidating the ticket. Message: "
                                    + e.getMessage(),
                            e);
                }
            }
    ...
    public void validate(String ticket) throws AuthenticationException {
        for (AuthenticationService authService : getUsableAuthenticationServices()) {
            try {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Not that important: Exception validating the ticket. Message: "
                                    + e.getMessage(),
                            e);
                }
            }
    ...
    public String getCurrentTicket() {
        for (AuthenticationService authService : getUsableAuthenticationServices()) {
            try {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Not that important: Exception getting the current ticket. Message: "
                                    + e.getMessage(),
                            e);
                }
            }
    ...
    public String getNewTicket() {
        for (AuthenticationService authService : getUsableAuthenticationServices()) {
            try {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Not that important: Exception getting a new ticket. Message: "
                                    + e.getMessage(),
                            e);
                }
            }
    ...
    public void clearCurrentSecurityContext() {
        for (AuthenticationService authService : getUsableAuthenticationServices()) {
            try {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Not that important: Exception clearing the current security context."
                                    + " Message: "
                                    + e.getMessage(),
                            e);
                }
    ...
    public Set<String> getDomiansThatAllowUserPasswordChanges() {
        ...
        if (getMutableAuthenticationService() != null) {
            domains.addAll(
                    getMutableAuthenticationService().getDomiansThatAllowUserPasswordChanges());
        }
        return domains;
    }
    ...
    public Set<String> getUsersWithTickets(boolean nonExpiredOnly) {
        ...
        for (AuthenticationService authService : getUsableAuthenticationServices()) {
            if (authService instanceof AbstractAuthenticationService) {
                users.addAll(
                        ((AbstractAuthenticationService) authService)
                                .getUsersWithTickets(nonExpiredOnly));
            }
        }
        return users;
    }
    ...
    public int invalidateTickets(boolean nonExpiredOnly) {
        ...
        for (AuthenticationService authService : getUsableAuthenticationServices()) {
            if (authService instanceof AbstractAuthenticationService) {
                count +=
                        ((AbstractAuthenticationService) authService)
                                .invalidateTickets(nonExpiredOnly);
            }
        }
        return count;
    }

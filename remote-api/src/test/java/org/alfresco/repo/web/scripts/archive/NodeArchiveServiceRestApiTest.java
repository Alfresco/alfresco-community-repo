...
public class NodeArchiveServiceRestApiTest extends BaseWebScriptTest {
    ...
    protected void setUp() throws Exception {
        ...
        nodeArchiveService =
                (NodeArchiveService)
                        getServer()
                                ...
                                .getBean("nodeArchiveService"); // Intentionally small 'n'.
        transactionHelper =
                (RetryingTransactionHelper)
                        getServer().getApplicationContext().getBean("retryingTransactionHelper");
        personService =
        ...
        archiveMap =
                (StoreArchiveMap) getServer().getApplicationContext().getBean("storeArchiveMap");
        ...
        adminUndeletedTestNode =
                createTestNode(AuthenticationUtil.getAdminUserName(), createNodeName(), false);
        createdNodes.add(adminUndeletedTestNode);
        // We need to remember the StoreRef where this node originally lived. i.e.
        // workspace://SpacesStore
        nodesOriginalStoreRef = adminUndeletedTestNode.getStoreRef();
        // This will ensure that there is always at least some NodeRefs in the 'trash' i.e. the
        // archive store.
        adminDeletedTestNode =
                createTestNode(AuthenticationUtil.getAdminUserName(), createNodeName(), true);
    ...
    /**
     * This test calls the GET REST API to read some deleted items from the archive store and checks
     * the various JSON data fields.
     */
    public void testGetDeletedItems() throws Exception {
        ...
        JSONArray deletedNodesArray =
                (JSONArray) dataObj.get(AbstractArchivedNodeWebScript.DELETED_NODES);
        ...
        for (int i = 0; i < deletedNodesArray.length(); i++) {
            ...
            String nextArchivedDateString =
                    nextJSONObj.getString(AbstractArchivedNodeWebScript.ARCHIVED_DATE);
            ...
            if (previousDate != null) {
                assertTrue(
                        "Archived Dates were not reverse-sorted.",
                        nextArchivedDate.before(previousDate));
            }
    ...
    private void doTest(JSONObject deletedNodeToTest, String archivedBy) throws Exception {
        assertNotNull(
                "Failed to find an expected NodeRef within the archive store.", deletedNodeToTest);

        assertEquals(
                archivedBy, deletedNodeToTest.getString(AbstractArchivedNodeWebScript.ARCHIVED_BY));
        assertEquals(TEST_TITLE, deletedNodeToTest.getString(AbstractArchivedNodeWebScript.TITLE));
        assertEquals(
                TEST_DESCRIPTION,
                deletedNodeToTest.getString(AbstractArchivedNodeWebScript.DESCRIPTION));
    ...
    /**
     * This test calls the GET REST API to read some deleted items for the current user from the
     * archive store
     */
    public void testGetDeletedItemsAsNonAdminUser() throws Exception {
        ...
        JSONArray deletedNodesArray =
                (JSONArray) dataObj.get(AbstractArchivedNodeWebScript.DELETED_NODES);
        assertNotNull("JSON 'deletedNodesArray' object was null", deletedNodesArray);
        // User_One deleted only 1 node and he doesn't have permission to see other users' archived
        // data.
        assertTrue(
                "Unexpectedly found more than 1 item in the archive store.",
                1 == deletedNodesArray.length());
        ...
        assertNotNull("JSON 'deletedNodesArray' object was null", deletedNodesArray);
        // User_Two deleted only 1 node and he doesn't have permission to see other users' archived
        // data.
        assertTrue(
                "Unexpectedly found more than 1 item in the archive store.",
                1 == deletedNodesArray.length());
    ...
    private JSONObject getArchivedNodes()
    ...
    public void testPurgeDeletedItems() throws Exception {
        ...
        JSONArray archivedNodesArray =
                dataJsonObj.getJSONArray(AbstractArchivedNodeWebScript.DELETED_NODES);
        ...
        for (int i = 0; i < archivedNodesLength; i++) {
            ...
            if (archivedNode
                    .getString(AbstractArchivedNodeWebScript.NODEREF)
                    .equals(adminDeletedTestNode.toString())) {
                requiredNodeInArchive = archivedNode;
                break;
            } else if (archivedNode
                    .getString(AbstractArchivedNodeWebScript.NODEREF)
                    .equals(user1_DeletedTestNode.toString())) {
                requiredNodeInArchive = archivedNode;
                break;
            } else if (archivedNode
                    .getString(AbstractArchivedNodeWebScript.NODEREF)
                    .equals(user2_DeletedTestNode.toString())) {
                requiredNodeInArchive = archivedNode;
                break;
            }
        }
        ...
        // So we have identified a specific Node in the archive that we want to delete permanently
        // (purge).
        String nodeRefString =
                requiredNodeInArchive.getString(AbstractArchivedNodeWebScript.NODEREF);
        ...
        assertEquals(
                "Only expected one NodeRef to have been purged.", 1, purgedNodesArray.length());
    ...
    public void testPurgeDeletedItemsAsNonAdminUser() throws Exception {
        ...
        String deleteUrl =
                getArchiveUrl(user2_DeletedTestNode.getStoreRef())
                        + "/"
                        + user2_DeletedTestNode.getId();
        ...
        assertNotNull("JSON 'deletedNodesArray' object was null", deletedNodesArray);
        assertEquals(
                "There is 1 item in the archive store which was deleted by the Admin.",
                1,
                deletedNodesArray.length());
    }

    /**
     * This method makes a REST call to retrieve the contents of the archive store, returning the
     * number of individual nodes contained in it.
     */
    private int getArchivedNodesCount()
            throws IOException, JSONException, UnsupportedEncodingException {
        JSONObject archiveContents = getArchivedNodes();
        JSONObject datatObject = archiveContents.getJSONObject(DATA);
        JSONArray deletedNodesArray =
                datatObject.getJSONArray(AbstractArchivedNodeWebScript.DELETED_NODES);
    ...
    public void testRestoreDeletedItems() throws Exception {
        ...
        String jsonString =
                new JSONStringer().object().key("restoreLocation").value("").endObject().toString();
        Response rsp = sendRequest(new PutRequest(restoreUrl, jsonString, "application/json"), 200);

        assertEquals(
                "Expected archive to shrink by one",
                archivedNodesCountBeforeRestore - 1,
                getArchivedNodesCount());
    }
    ...
    private String getArchiveUrl(StoreRef storeRef) {
        String result =
                MessageFormat.format(
                        ARCHIVE_URL_FORMAT, storeRef.getProtocol(), storeRef.getIdentifier());
        return result;
    }
    ...
    private NodeRef createTestNode(
            final String authenticatedUser, final String cmName, final boolean deleteNode) {
                                ...
                                new RunAsWork<NodeRef>() {
                                    public NodeRef doWork() throws Exception {
                                        ...
                                        if (deleteNode) {
                                            ...
                                            // At his point the chAssRef.getChildRef  NodeRef will
                                            // point to the
                                            // location of the node before it got deleted. We need
                                            // to store
                                            // it's NodeRef *after* deletion, which will point to
                                            // the archive store.

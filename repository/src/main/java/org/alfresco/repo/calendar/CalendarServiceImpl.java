...
public class CalendarServiceImpl implements CalendarService {
   ...
   */
  protected NodeRef getSiteCalendarContainer(
  ...
  public CalendarEntry getCalendarEntry(
    ...
    NodeRef event = nodeService.getChildByName(
      container,
  ...
  private CalendarEntry getCalendarEntry(
    ...
    if (event != null) {
      CalendarEntryImpl entry = new CalendarEntryImpl(
        event,
        container,
        entryName
      );
  ...
  public CalendarEntry createCalendarEntry(
    ...
    Map<QName, Serializable> properties = CalendarEntryImpl.toNodeProperties(
      entry
    );
    ...
    String name =
      (new Date()).getTime() + "-" + Math.round(Math.random() * 10000) + ".ics";
    ...
    Date createdAt = (Date) nodeService.getProperty(
      nodeRef,
  ...
  public CalendarEntry updateCalendarEntry(CalendarEntry entry) {
    ...
    for (Map.Entry<QName, Serializable> prop : nodeService
      .getProperties(entry.getNodeRef())
      .entrySet()) {
      if (
        !prop
          .getKey()
          .getNamespaceURI()
          .equals(CalendarModel.CALENDAR_MODEL_URL)
      ) {
  ...
  public PagingResults<CalendarEntry> listCalendarEntries(
    ...
    // Run the canned query
    GetCalendarEntriesCannedQueryFactory cqFactory = (GetCalendarEntriesCannedQueryFactory) cannedQueryRegistry.getNamedObject(
    ...
    if (outlookUID != null) {
      ...
      filters.add(
        new FilterPropString(
          CalendarModel.PROP_OUTLOOK_UID,
          outlookUID,
      ...
      filters.add(
        new FilterPropBoolean(CalendarModel.PROP_IS_OUTLOOK, Boolean.TRUE)
      );
    }
  ...
  private PagingResults<CalendarEntry> wrap(
    final PagingResults<NodeRef> results,
    ...
    return new PagingResults<CalendarEntry>() {
      ...
      public List<CalendarEntry> getPage() {
        ...
        for (NodeRef nodeRef : results.getPage()) {
          String entryName = (String) nodeService.getProperty(
            nodeRef,

...
/*package*/class M2PropertyDefinition implements PropertyDefinition {
  ...
  /*package*/M2PropertyDefinition(
    ...
    this.propertyTypeName =
      QName.createQName(m2Property.getType(), prefixResolver);
  }
  ...
  /*package*/void resolveDependencies(
    ...
    if (
      propertyTypeName.equals(DataTypeDefinition.CONTENT) && isMultiValued()
    ) {
      throw new DictionaryException(
        "d_dictionary.property.err.single_valued_content"
      );
    }
  ...
  private static List<ConstraintDefinition> buildConstraints(
    ...
    Map<QName, ConstraintDefinition> constraintsByQName = new HashMap<QName, ConstraintDefinition>(
      7
    );
    for (M2Constraint constraint : m2constraints) {
      ...
      if (constraint.getName() == null) {
        ...
        builder
          .append(m2PropertyDef.classDef.getModel().getName().getLocalName())
          .append("_");
        builder
          .append(m2PropertyDef.classDef.getName().getLocalName())
          .append("_");
        builder.append(m2PropertyDef.getName().getLocalName()).append("_");
        builder.append("anon_");
        builder.append(constraints.size());
        QName newName = QName.createQName(
          m2PropertyDef.classDef.getModel().getName().getNamespaceURI(),
          builder.toString()
        );
        constraint.setName(
          newName.getPrefixedQName(prefixResolver).toPrefixString()
        );
      }

      ConstraintDefinition def = new M2ConstraintDefinition(
        m2PropertyDef,
        constraint,
        prefixResolver
      );

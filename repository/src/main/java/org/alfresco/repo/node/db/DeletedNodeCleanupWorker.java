...
public class DeletedNodeCleanupWorker extends AbstractNodeCleanupWorker {
  ...
  protected List<String> doCleanInternal() throws Throwable {
    if (minPurgeAgeMs < 0) {
      return Collections.singletonList(
        "Minimum purge age is negative; purge disabled"
      );
    }
  ...
  private List<String> purgeOldDeletedNodes(long minAge) {
    ...
    while (true) {
      ...
      try {
        DeleteNodesByTransactionsCallback purgeNodesCallback = new DeleteNodesByTransactionsCallback(
          nodeDAO,
          fromCommitTime,
          toCommitTime
        );
  ...
  private List<String> purgeOldEmptyTransactions(long minAge) {
    ...
    while (true) {
      ...
      // Purge transactions
      try {
        DeleteTransactionsCallback purgeTxnsCallback = new DeleteTransactionsCallback(
          nodeDAO,
          fromCommitTime,
          toCommitTime
        );
  ...
  private static class DeleteNodesByTransactionsCallback
    ...
    DeleteNodesByTransactionsCallback(
      NodeDAO nodeDAO,
      long fromCommitTime,
      long toCommitTime
    ) {

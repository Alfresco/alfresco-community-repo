...
public class StandardRenditionLocationResolverTest extends BaseAlfrescoSpringTest {
    ...
    public void testChildAssociationFinder() {
        ...
        assertEquals(targetFolder, location.getParentRef());
        assertEquals(
                "test-" + ContentModel.PROP_CONTENT.getLocalName() + ".xml",
                location.getChildName());
        assertNull(location.getChildRef());
        ...
        String destinationName =
                (String) nodeService.getProperty(destinationNode, ContentModel.PROP_NAME);
    ...
    public void testCreatesFoldersForTemplatedLocation() throws Exception {
        ...
        List<ChildAssociationRef> childAssocs =
                nodeService.getChildAssocs(companyHome, ContentModel.ASSOC_CONTAINS, fooName);
        ...
        RenditionDefinition renditionDef =
                renditionService.createRenditionDefinition(renditionKind, "nicks_test_engine");

        String pathTemplate = barPath + "${cwd}nick.xml";
        renditionDef.setParameterValue(
                RenditionService.PARAM_DESTINATION_PATH_TEMPLATE, pathTemplate);
        ...
        NodeRef finalFolderNode =
                checkFolder(
                        nodeService.getPrimaryParent(sourceFolder).getQName(),
                        barNode,
                        "Final Folder");

        assertEquals(
                "Final folder is not the rendition parent!",
                finalFolderNode,
    ...
    //        NodeRef tempRenditionNode = makeNode(sourceNode, ContentModel.TYPE_CONTENT);
    //        ChildAssociationRef
    // tempRenditionLocation=nodeService.getPrimaryParent(tempRenditionNode);
    //
    //        RenditionDefinition definition =
    // renditionService.createRenditionDefinition(renditionKind,
    //                    "nicks_test_engine");
    //        ChildAssociationRef oldRendition =
    //            nodeService.createNode(sourceNode, RenditionModel.ASSOC_RENDITION,
    // definition.getRenditionName(), ContentModel.TYPE_CONTENT);
    ...
    //        assertFalse("The old rendition association should ahve been removed!",
    // sourceChildren.contains(oldRendition));
    //        assertFalse("The old rendition should have been deleted!",
    // nodeService.exists(oldRendition.getChildRef()) );
    //    }
    ...
    private NodeRef checkFolder(QName folderName, NodeRef parentName, String folderMessageName) {
        List<ChildAssociationRef> folderAssocs =
                nodeService.getChildAssocs(parentName, ContentModel.ASSOC_CONTAINS, folderName);
        assertEquals("Folder " + folderMessageName + " should exist!", 1, folderAssocs.size());
        NodeRef folderNode = folderAssocs.get(0).getChildRef();
        assertEquals(
                folderMessageName + " node is wrong type!",
                ContentModel.TYPE_FOLDER,
                nodeService.getType(folderNode));
        assertEquals(
                folderMessageName + " node has wrong name!",
                folderName.getLocalName(),
    ...
    private NodeRef makeNode(NodeRef parent, QName nodeType) {
        ...
        ChildAssociationRef assoc =
                nodeService.createNode(
                        ...
                        QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, uuid),
                        nodeType,
                        props);

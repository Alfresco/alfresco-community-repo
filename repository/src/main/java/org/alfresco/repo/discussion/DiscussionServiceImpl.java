...
public class DiscussionServiceImpl implements DiscussionService {
    ...
    protected static final String CANNED_QUERY_GET_CHILDREN =
            "discussionGetChildrenCannedQueryFactory";
    protected static final String CANNED_QUERY_GET_CHILDREN_TARGETS =
            "discussionGetChildrenWithTargetAssocsAuditableCannedQueryFactory";
    protected static final String CANNED_QUERY_GET_TOPICS_WITH_POSTS =
            "discussionGetDiscussionTopcisWithPostsCannedQueryFactory";
    ...
    private String generateName() {
        ...
        String name = "post-" + (new Date()).getTime() + "_" + Math.round(Math.random() * 10000);
        return name;
    }
    ...
    private PostInfo buildPost(
            NodeRef nodeRef, TopicInfo topic, String name, String preLoadedContents) {
    ...
    public TopicInfo getTopic(NodeRef parentNodeRef, String topicName) {
        NodeRef topic =
                nodeService.getChildByName(parentNodeRef, ContentModel.ASSOC_CONTAINS, topicName);
        if (topic != null) {
    ...
    public PostInfo getPost(TopicInfo topic, String postName) {
        ...
        if (topic.getNodeRef() == null) {
            throw new IllegalArgumentException(
                    "Can't get posts for a topic that was never persisted!");
        }

        // Fetch
        NodeRef post =
                nodeService.getChildByName(
                        topic.getNodeRef(), ContentModel.ASSOC_CONTAINS, postName);
        if (post != null) {
    ...
    public PostInfo createPost(TopicInfo topic, String contents) {
        ...
        if (topic.getNodeRef() == null) {
            throw new IllegalArgumentException(
                    "Can't create posts for a topic that was never persisted!");
        }
    ...
    public TopicInfo updateTopic(TopicInfo topic) {
        ...
        if (topic.getNodeRef() == null) {
            throw new IllegalArgumentException(
                    "Can't update a topic that was never persisted, call create instead");
        }
    ...
    public PostInfo getMostRecentPost(TopicInfo topic) {
        ...
        CannedQueryResults<NodeBackedEntity> results =
                listEntries(
                        topic.getNodeRef(), ForumModel.TYPE_POST, null, null, null, false, paging);
    ...
    @Override
    public PagingResults<TopicInfo> listTopics(
            String siteShortName, boolean sortAscending, PagingRequest paging) {
        NodeRef container = getSiteDiscussionsContainer(siteShortName, false);
        if (container == null) {
            // No topics
    ...
    @Override
    public PagingResults<TopicInfo> listTopics(
            NodeRef nodeRef, boolean sortAscending, PagingRequest paging) {
        // Do the listing, oldest first
        CannedQueryResults<NodeBackedEntity> nodes =
                listEntries(
                        nodeRef, ForumModel.TYPE_TOPIC, null, null, null, sortAscending, paging);
    ...
    @Override
    public PagingResults<TopicInfo> listTopics(
            String siteShortName, String username, boolean sortAscending, PagingRequest paging) {
        NodeRef container = getSiteDiscussionsContainer(siteShortName, false);
        if (container == null) {
            // No topics
            return new EmptyPagingResults<TopicInfo>();
    ...
    @Override
    public PagingResults<TopicInfo> listTopics(
            NodeRef nodeRef, String username, boolean sortAscending, PagingRequest paging) {
        // Do the listing, oldest first
        CannedQueryResults<NodeBackedEntity> nodes =
                listEntries(
                        nodeRef,
    ...
    @Override
    public PagingResults<TopicInfo> listTopics(
            String siteShortName, Date from, Date to, boolean sortAscending, PagingRequest paging) {
        NodeRef container = getSiteDiscussionsContainer(siteShortName, false);
        if (container == null) {
            // No topics
            return new EmptyPagingResults<TopicInfo>();
    ...
    @Override
    public PagingResults<TopicInfo> listTopics(
            NodeRef nodeRef, Date from, Date to, boolean sortAscending, PagingRequest paging) {
    ...
    public PagingResults<Pair<TopicInfo, Integer>> listHotTopics(
        ...
        GetDiscussionTopcisWithPostsCannedQueryFactory getCQFactory =
                (GetDiscussionTopcisWithPostsCannedQueryFactory)
                        cannedQueryRegistry.getNamedObject(CANNED_QUERY_GET_TOPICS_WITH_POSTS);
        GetDiscussionTopcisWithPostsCannedQuery cq =
                (GetDiscussionTopcisWithPostsCannedQuery)
                        getCQFactory.getCannedQuery(nodeRef, null, since, true, null, paging);
    ...
    @Override
    public PagingResults<TopicInfo> findTopics(
    ...
    public PagingResults<PostInfo> listPosts(TopicInfo topic, PagingRequest paging) {
        ...
        CannedQueryResults<NodeBackedEntity> nodes =
                listEntries(
                        topic.getNodeRef(), ForumModel.TYPE_POST, null, null, null, true, paging);
    ...
    public PostWithReplies listPostReplies(PostInfo primaryPost, int levels) {
        ...
        GetChildrenWithTargetAssocsAuditableCannedQueryFactory cqFactory =
                (GetChildrenWithTargetAssocsAuditableCannedQueryFactory)
                        cannedQueryRegistry.getNamedObject(CANNED_QUERY_GET_CHILDREN_TARGETS);

        // Sort by date
        CannedQuerySortDetails sorting = cqFactory.createDateAscendingCQSortDetails();

        // Run the canned query
        GetChildrenWithTargetAssocsAuditableCannedQuery cq =
                (GetChildrenWithTargetAssocsAuditableCannedQuery)
                        cqFactory.getCannedQuery(
                                primaryPost.getTopic().getNodeRef(),
                                ForumModel.TYPE_POST,
                                ContentModel.ASSOC_REFERENCES,
                                sorting,
                                new PagingRequest(MAX_REPLIES_FETCH_SIZE));
        ...
        Map<NodeRef, List<NodeWithTargetsEntity>> idToReplies =
                new HashMap<NodeRef, List<NodeWithTargetsEntity>>();
        for (NodeWithTargetsEntity e : results.getPage()) {
    ...
    private void calculateRepliesPreLoad(
            NodeRef nodeRef,
            List<NodeRef> preLoad,
            Map<NodeRef, List<NodeWithTargetsEntity>> idToReplies,
            int levels) {
        preLoad.add(nodeRef);
        if (levels > 0) {
            ...
            if (replies != null && replies.size() > 0) {
                for (NodeWithTargetsEntity entity : replies) {
                    calculateRepliesPreLoad(
                            tenantService.getBaseName(entity.getNodeRef()),
                            preLoad,
                            idToReplies,
                            levels - 1);
                }
    ...
    private PostWithReplies wrap(
            PostInfo post, Map<NodeRef, List<NodeWithTargetsEntity>> idToReplies, int levels) {
    ...
    /**
     * Finds nodes in the specified parent container, with the given type, optionally filtered by
     * creator
     */
    private CannedQueryResults<NodeBackedEntity> listEntries(
            NodeRef parent,
            QName nodeType,
        ...
        GetChildrenAuditableCannedQueryFactory getChildrenCannedQueryFactory =
                (GetChildrenAuditableCannedQueryFactory)
        ...
        // Run the canned query
        GetChildrenAuditableCannedQuery cq =
    ...
    private PagingResults<TopicInfo> wrap(
            final ResultSet finalLuceneResults,
            final NodeRef container,
            final PagingRequest paging) {
        final List<TopicInfo> topics = new ArrayList<TopicInfo>();
        int cnt = 1;
        for (ResultSetRow row : finalLuceneResults) {
            TopicInfo topic =
                    buildTopic(row.getNodeRef(), container, row.getQName().getLocalName());
            topics.add(topic);
            cnt++;
        ...
        return new PagingResults<TopicInfo>() {
            ...
            public Pair<Integer, Integer> getTotalResultCount() {
                ...
                return new Pair<Integer, Integer>(
                        totalItemsInUnpagedResultSet, totalItemsInUnpagedResultSet);
            }
    ...
    private PagingResults<TopicInfo> wrap(
            final PagingResults<NodeBackedEntity> results, final NodeRef container) {
    ...
    private PagingResults<PostInfo> wrap(
            final PagingResults<NodeBackedEntity> results, final TopicInfo topic) {
        // Pre-load the nodes before we create them
        List<Long> ids = new ArrayList<Long>();
        for (NodeBackedEntity node : results.getPage()) {
            ids.add(node.getId());
     ...
     */
    private PagingResults<Pair<TopicInfo, Integer>> wrapWithCount(
            final PagingResults<NodeWithChildrenEntity> results, final NodeRef container) {
        // Pre-load the nodes before we create them
        List<Long> ids = new ArrayList<Long>();
        for (NodeBackedEntity node : results.getPage()) {
            ids.add(node.getId());

...
public class NodesUnlockTests extends RestTest {
  ...
  public void lockEphemeralAndUnlock() throws Exception {
    STEP(
      "1. Add user(s) as collaborators to the site created by administrator and add a file in this site."
    );
    file1 =
      dataContent
        .usingUser(adminUser)
    ...
    RestNodeModel file1Model1 = restClient
    ...
    STEP(
      "3. Lock the file using mode EPHEMERAL with user1 (POST nodes/{nodeId}/lock)."
    );
    ...
    RestNodeModel file1Model2 = restClient
      .authenticateUser(user1)
      .withCoreAPI()
      .usingNode(file1)
      .usingParams("include=isLocked")
  ...
  public void lockPersistentAndUnlock() throws Exception {
    ...
    STEP(
      "3. Lock the file using mode PERSISTENT with user1 (POST nodes/{nodeId}/lock)."
    );
    RestNodeLockBodyModel lockBodyModel = new RestNodeLockBodyModel();

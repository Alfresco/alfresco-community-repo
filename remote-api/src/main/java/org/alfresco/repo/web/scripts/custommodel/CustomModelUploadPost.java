...
public class CustomModelUploadPost extends DeclarativeWebScript {
  ...
  protected Map<String, Object> executeImpl(
    ...
    if (
      !customModelService.isModelAdmin(
        AuthenticationUtil.getFullyAuthenticatedUser()
      )
    ) {
      throw new WebScriptException(
        Status.STATUS_FORBIDDEN,
        PermissionDeniedException.DEFAULT_MESSAGE_ID
      );
    }
    ...
    if (formData == null || !formData.getIsMultiPart()) {
      throw new WebScriptException(
        ...
        "cmm.rest_api.model.import_not_multi_part_req"
      );
    }
    ...
    if (!processed) {
      throw new WebScriptException(
        Status.STATUS_BAD_REQUEST,
        "cmm.rest_api.model.import_no_zip_file_uploaded"
      );
    }
  ...
  protected ImportResult processUpload(ZipFile zipFile, String filename)
    throws IOException {
    if (zipFile.size() > 2) {
      throw new WebScriptException(
        Status.STATUS_BAD_REQUEST,
        "cmm.rest_api.model.import_invalid_zip_package"
      );
    }
    ...
    while (entries.hasMoreElements()) {
      ...
      if (!entry.isDirectory()) {
        ...
        try (
          ...
          if (
            !(
              entryName.endsWith(CustomModelServiceImpl.SHARE_EXT_MODULE_SUFFIX)
            ) &&
            customModel == null
          ) {
            try {
              ...
              if (shareExtModule == null) {
                ...
                try (
                  InputStream moduleInputStream = new BufferedInputStream(
                    zipFile.getInputStream(entry),
                    BUFFER_SIZE
                  )
                ) {
  ...
  protected String getExtensionModule(

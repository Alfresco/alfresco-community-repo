...
public class WorkflowManager extends BaseScopableProcessorExtension {
  ...
  public JscriptWorkflowDefinition getDefinition(String id) {
    WorkflowDefinition definition = services
      .getWorkflowService()
  ...
  public JscriptWorkflowDefinition getDefinitionByName(String name) {
    WorkflowDefinition cmrWorkflowDefinition = services
      .getWorkflowService()
      .getDefinitionByName(name);
    return cmrWorkflowDefinition == null
      ? null
      : new JscriptWorkflowDefinition(
  ...
  public JscriptWorkflowInstance getInstance(String workflowInstanceID) {
    WorkflowInstance instance = services
      .getWorkflowService()
  ...
  public Scriptable getPooledTasks(final String authority) {
    List<WorkflowTask> cmrPooledTasks = services
      .getWorkflowService()
    ...
    for (WorkflowTask cmrPooledTask : cmrPooledTasks) {
      pooledTasks.add(
        new JscriptWorkflowTask(cmrPooledTask, services, getScope())
      );
    }
  ...
  public JscriptWorkflowTask getTask(String id) {
    ...
    return task == null
      ? null
      : new JscriptWorkflowTask(task, services, this.getScope());
  }
  ...
  public Scriptable getLatestDefinitions() {
    List<WorkflowDefinition> cmrDefinitions = services
      .getWorkflowService()
      .getDefinitions();
    ArrayList<Serializable> workflowDefs = new ArrayList<Serializable>();
    for (WorkflowDefinition cmrDefinition : cmrDefinitions) {
      workflowDefs.add(
        new JscriptWorkflowDefinition(cmrDefinition, services, getScope())
      );
    }

    return (Scriptable) new ValueConverter()
  ...
  private Scriptable getAssignedTasksByState(WorkflowTaskState state) {
    ...
    String currentUser = services
      .getAuthenticationService()
      .getCurrentUserName();
    List<WorkflowTask> cmrAssignedTasks = workflowService.getAssignedTasks(
      currentUser,
      state
    );

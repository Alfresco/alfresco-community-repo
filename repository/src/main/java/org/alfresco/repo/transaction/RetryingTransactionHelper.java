...
public class RetryingTransactionHelper {
  ...
  private static final String KEY_ACTIVE_TRANSACTION =
    "RetryingTransactionHelper.ActiveTxn";
  ...
  private static List<Class<?>> enterpriseRetryExceptions() {
    ...
    try {
      Class<?> c = Class.forName(
        "org.alfresco.enterprise.repo.transaction.RetryExceptions"
      );
      ...
      throw new AlfrescoRuntimeException(
        "Unable to instantiate enterprise RetryExceptions."
      );
    ...
    try {
      ...
      for (int count = 0; count == 0 || count < maxRetries; count++) {
        ...
        try {
          if (requiresNew) {
            txn =
              txnService.getNonPropagatingUserTransaction(
                readOnly,
                forceWritable
              );
          ...
          if (txn != null) {
            try {
              ...
              logger.error(
                "Rollback failure.  Normal retry behaviour will resume.",
                e1
              );
            }
          }
          ...
          if (
            ...
            retryCause =
              ExceptionStackUtil.getCause(
                e,
                extraExceptions.toArray(new Class[] {})
              );
          }
  ...
  public static Throwable extractRetryCause(Throwable cause) {
    ...
    if (retryCause == null) {
      ...
      ExceptionStackUtil.getCause(cause, DoNotRetryException.class) != null
    ) {
      ...
      if (
        retryCause.getCause() != null && retryCause.getCause() != retryCause
      ) {
        ...
        if (retryCause.getMessage().toLowerCase().contains("deadlock")) {
          ...
          retryCause.getMessage().toLowerCase().contains("constraint")
        ) {
  ...
  public static UserTransaction getActiveUserTransaction() {
    ...
    UserTransaction txn = (UserTransaction) AlfrescoTransactionSupport.getResource(
      KEY_ACTIVE_TRANSACTION
    );
    if (txn == null) {
      return null;

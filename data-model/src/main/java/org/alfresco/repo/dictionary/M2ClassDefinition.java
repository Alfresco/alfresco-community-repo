...
/*package*/class M2ClassDefinition implements ClassDefinition {
  ...
  /*package*/M2ClassDefinition(
    ...
    if (
      m2Class.getParentName() != null && m2Class.getParentName().length() > 0
    ) {
    ...
    for (M2Property property : m2Class.getProperties()) {
      PropertyDefinition def = new M2PropertyDefinition(
        this,
        property,
        resolver
      );
      if (
        !model.isNamespaceDefined(def.getName().getNamespaceURI()) &&
    ...
    for (M2ClassAssociation assoc : m2Class.getAssociations()) {
      ...
      if (assoc instanceof M2ChildAssociation) {
        def =
          new M2ChildAssociationDefinition(
            this,
            (M2ChildAssociation) assoc,
            resolver
          );
      } else {
  ...
  /*package*/void resolveDependencies(
    ...
    for (PropertyDefinition def : properties.values()) {
      ((M2PropertyDefinition) def).resolveDependencies(
          query,
          prefixResolver,
          modelConstraints
        );
  ...
  /*package*/void resolveInheritance(
    ...
    ClassDefinition parentClass = (parentName == null)
      ? null
      : query.getClass(parentName);
    ...
    for (AssociationDefinition def : inheritedAssociations.values()) {
      if (def instanceof ChildAssociationDefinition) {
        inheritedChildAssociations.put(
          def.getName(),
          (ChildAssociationDefinition) def
        );
      }
    }
  ...
  /* package */List<M2ModelDiff> diffClass(ClassDefinition classDef) {
    ...
    Collection<M2ModelDiff> propertyDiffs = M2PropertyDefinition.diffPropertyLists(
      getProperties().values(),
      classDef.getProperties().values()
    );
    ...
    Collection<M2ModelDiff> assocDiffs = M2AssociationDefinition.diffAssocLists(
      getAssociations().values(),
      classDef.getAssociations().values()
    );
    ...
    if (getIncludedInSuperTypeQuery() == null) {
      ...
      if (
        classIncludedInSuperTypeQuery == null ||
    ...
    if (isUpdated) {
      modelDiffs.add(
        new M2ModelDiff(name, modelDiffType, M2ModelDiff.DIFF_UPDATED)
      );
    } else if (isUpdatedIncrementally) {
      modelDiffs.add(
        new M2ModelDiff(name, modelDiffType, M2ModelDiff.DIFF_UPDATED_INC)
      );
    } else {
      modelDiffs.add(
        new M2ModelDiff(name, modelDiffType, M2ModelDiff.DIFF_UNCHANGED)
      );
    }
    ...
    for (ClassDefinition previousClass : previousClasses) {
      ...
      for (ClassDefinition newClass : newClasses) {
        if (newClass.getName().equals(previousClass.getName())) {
          modelDiffs.addAll(
            ((M2ClassDefinition) previousClass).diffClass(newClass)
          );
          found = true;
          break;
        }

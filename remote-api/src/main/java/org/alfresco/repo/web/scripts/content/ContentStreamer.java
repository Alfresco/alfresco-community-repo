...
/**
 * Can be used when the binary data of a content property needs to be streamed back to the client as
 * the result of executing a web script.
 *
 * <p>These methods are taken from the StreamContent class so they can be reused by other
 * webscripts.
 */
public class ContentStreamer implements ResourceLoaderAware {
     ...
     * @param modifiedTime The modified datetime to use for the streamed content. If <tt>null</tt>
     *     the file's timestamp will be used.
    ...
    public void streamContent(
        ...
        // TODO
        // This was commented out to accomadate records management permissions.  We need to review
            ...
            if (modifiedSinceStr != null) {
                try {
                    ...
                    if (logger.isInfoEnabled())
                        logger.info(
                                "Browser sent badly-formatted If-Modified-Since header: "
                                        + modifiedSinceStr);
                }

                if (modifiedSince > 0L) {
                    // round the date to the ignore millisecond value which is not supplied by
                    // header
        ...
        streamContentImpl(
                ...
                attach,
                modified,
                modified == null ? null : Long.toString(modified.getTime()),
                attachFileName,
                model);
    }
    ...
    protected void streamContent(
        ...
        if (logger.isDebugEnabled())
            logger.debug(
                    "Retrieving content from resource path "
                            + resourcePath
                            + " (attach: "
                            + attach
                            + ")");
    ...
    public void streamContentImpl(
        ...
        try {
            ...
            //                    @Override
            //                    public Event prepareEvent(String user, String networkId, String
            // transactionId)
            //                    {
            //                        String siteId = siteService.getSiteShortName(nodeRef);
            //
            //                        return new ContentEventImpl(ContentEvent.DOWNLOAD, user,
            // networkId, transactionId,
            //                                    nodeRef.getId(), siteId, propertyQName.toString(),
            // Client.asType(ClientType.webclient), attachFileName, finalMimetype, size, encoding);
            //                    }
            //                });
            //            }
            ...
            if (processedRange == false) {
                ...
                // assuming the repository is capable of streaming in chunks, this should allow
                // large files
    ...
    public void setAttachment(
            WebScriptRequest req, WebScriptResponse res, boolean attach, String attachFileName) {
        if (attach == true) {
                ...
                if (req == null) {
                    headerValue +=
                            "; filename*=UTF-8''"
                                    ...
                                    + "; filename=\""
                                    + filterNameForQuotedString(attachFileName)
                                    + "\"";
                } else {
            ...
            // set header based on filename - will force a Save As from the browse if it doesn't
            // recognize it
     ...
     * @param eTag String
     */
    protected void setResponseCache(
            WebScriptResponse res, Date modified, String eTag, Map<String, Object> model) {

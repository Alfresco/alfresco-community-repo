...
public class OpenCmisLocalTest extends TestCase {
  ...
  private static final String BEAN_NAME_AUTHENTICATION_COMPONENT =
  ...
  public void setUp() throws Exception {
    ...
    this.eventPublisher =
      (EventPublisherForTestingOnly) ctx.getBean("eventPublisher");
  }
  ...
  public void DISABLED_testBasicFileOps() {
    ...
    Folder folder = rootFolder.createFolder(
      folderProps,
      null,
      null,
      null,
    ...
    ContentStreamImpl fileContent = new ContentStreamImpl();
    {
      ContentWriter writer = new FileContentWriter(
        TempFileProvider.createTempFile(getName(), ".txt")
      );
  ...
  public void testDownloadEvent() throws InterruptedException {
    ...
    ContentStream contentStream = new ContentStreamImpl(
      docname,
      BigInteger.valueOf(byteContent.length),
      "text/plain",
      stream
    );

    Document doc1 = rootFolder.createDocument(
      props,
      contentStream,
    ...
    //range request
    content =
      doc1.getContentStream(BigInteger.valueOf(2), BigInteger.valueOf(4));
    assertNotNull(content);
  }
  ...
  public void testALF10085() throws InterruptedException {
    ...
    doc1.refresh();
    Calendar doc1LastModifiedBefore = (Calendar) doc1
      .getProperty(PropertyIds.LAST_MODIFICATION_DATE)
      .getFirstValue();
    assertNotNull(doc1LastModifiedBefore);

    doc2.refresh();
    Calendar doc2LastModifiedBefore = (Calendar) doc2
      .getProperty(PropertyIds.LAST_MODIFICATION_DATE)
      .getFirstValue();
    ...
    doc1.refresh();
    Calendar doc1LastModifiedAfter = (Calendar) doc1
      .getProperty(PropertyIds.LAST_MODIFICATION_DATE)
      .getFirstValue();
    assertNotNull(doc1LastModifiedAfter);

    doc2.refresh();
    Calendar doc2LastModifiedAfter = (Calendar) doc2
      .getProperty(PropertyIds.LAST_MODIFICATION_DATE)
      .getFirstValue();
  ...
  private ContentStreamImpl makeContentStream(
    String filename,
    String mimetype,
    String content
  ) throws IOException {
   ...
   */
  private void simulateCallWithAdvice(boolean includeStreamInterceptor)
    throws Exception {
    final int loops = 3;
    ...
    ContentStreamImpl csa = makeContentStream(
      "file1",
      MimetypeMap.MIMETYPE_TEXT_PLAIN,
      "The cat sat on the mat"
    );
    ContentStreamImpl csb = makeContentStream(
      "file2",
      MimetypeMap.MIMETYPE_TEXT_PLAIN,
  ...
  public void testEncodingForCreateContentStream() {
    ...
    try {
      ...
      content =
        document.getContentStream(BigInteger.valueOf(2), BigInteger.valueOf(4));
      assertNotNull(content);
      ...
      ContentDataWithId contentData = (ContentDataWithId) properties.get(
  ...
  private static Document createDocument(
    ...
    String content =
      "aegif Mind Share Leader Generating New Paradigms by aegif corporation.";
    byte[] buf = null;
    try {
    ...
    ContentStream contentStream = session
      ...
      .createContentStream(
        ...
        input
      ); // additionally set the charset here

...
public class CalendarEntryGet extends AbstractCalendarWebScript {
                ...
                Date updatedDate =
                        (Date)
                                nodeService.getProperty(
                                        nodeRef, CalendarModel.PROP_UPDATED_EVENT_DATE);
                if (fdt.format(updatedDate).equals(fdt.format(date))) {
                    entry.setStart(
                            (Date)
                                    nodeService.getProperty(
                                            nodeRef, CalendarModel.PROP_UPDATED_START));
                    entry.setEnd(
                            (Date)
                                    nodeService.getProperty(
                                            nodeRef, CalendarModel.PROP_UPDATED_END));
                    entry.setTitle(
                            (String)
                                    nodeService.getProperty(
                                            nodeRef, CalendarModel.PROP_UPDATED_WHAT));
                    entry.setLocation(
                            (String)
                                    nodeService.getProperty(
                                            nodeRef, CalendarModel.PROP_UPDATED_WHERE));
                    break;
                }
            }
        }
    ...
    /**
     * This method replicates the pre-existing behaviour for recurring events. Rather than try to
     * render the text for them on the client, we instead statically render the description text
     * here on the server. When we properly support recurring events in the client (and not just for
     * SharePoint ones), this can be replaced.
     */
    protected String buildRecurrenceString(CalendarEntry event) {
        ...
        // And the rest
        DateFormat dFormat =
                SimpleDateFormat.getDateInstance(SimpleDateFormat.MEDIUM, I18NUtil.getLocale());

        DateFormat tFormat =
                SimpleDateFormat.getTimeInstance(SimpleDateFormat.SHORT, I18NUtil.getLocale());
    ...
    private static StringBuffer buildParams(
            Map<String, String> params, StringBuffer text, Map<String, String> days) {

...
public class FrozenAspect extends BaseBehaviourBean
    ...
    public void disableOnPropUpdateFrozenAspect() {
        org.alfresco.repo.policy.Behaviour behaviour =
                getBehaviour(ON_UPDATE_PROP_FROZEN_BEHAVIOUR_NAME);
        if (behaviour != null) {
            behaviour.disable();
        }
    }
                                    ...
                                    int currentCount =
                                            (Integer)
                                                    nodeService.getProperty(
                                                            parentRef, PROP_HELD_CHILDREN_COUNT);
                                    currentCount = currentCount + 1;
                                    nodeService.setProperty(
                                            parentRef, PROP_HELD_CHILDREN_COUNT, currentCount);
                                } else {
                                    if (instanceOf(parentRef, TYPE_FOLDER)
                                            && !nodeService.hasAspect(
                                                    parentRef, ASPECT_SITE_CONTAINER)) {
                            ...
                            if (nodeService.exists(nodeRef) && instanceOf(nodeRef, TYPE_CONTENT)) {
                                ...
                                if (nodeService.hasAspect(owningFolder, ASPECT_HELD_CHILDREN)) {
                                    ...
                                    final int currentCount =
                                            (Integer)
                                                    nodeService.getProperty(
                                                            owningFolder, PROP_HELD_CHILDREN_COUNT);
                                    if (currentCount > 0) {
                                        nodeService.setProperty(
                                                owningFolder,
     ...
     *
     * <p>Prevent frozen items being moved
     */
    @Override
    @Behaviour(
            kind = BehaviourKind.CLASS,
     ...
     *
     * <p>Prevents frozen items being updated
     */
    @Override
    @Behaviour(
            kind = BehaviourKind.CLASS,

...
/* package */ class M2ConstraintDefinition implements ConstraintDefinition {
    ...
    /* package */ synchronized void resolveDependencies(
            ModelQuery query, boolean enableConstraintClassLoading) {
        if (resolving) {
    ...
    private synchronized void resolveInternal(
        ...
        if (ref != null && type != null) {
            ...
            M2ConstraintDefinition constraintDef =
                ...
                if (constraintNamedValues != null) {
                    for (M2NamedValue namedValue : constraintNamedValues) {
                        ...
                        if (namedValueName.equals(PROP_SHORT_NAME)) {
                            throw new DictionaryException(
                                    ERR_RESERVED_PROPERTY, PROP_SHORT_NAME, namedValueName);
                        }
                        ...
                        if (namedValue.getSimpleValue() != null
                            ...
                            throw new DictionaryException(
                                    ERR_SIMPLE_AND_LIST, shortName, namedValue.getName());
                        ...
                        try {
                            ...
                            throw new DictionaryException(
                                    ERR_PROPERTY_MISMATCH, e, namedValueName, shortName);
                    ...
                    if (beanWrapper.isWritableProperty(PROP_SHORT_NAME)) {
                        try {
                            ...
                            throw new DictionaryException(
                                    ERR_PROPERTY_MISMATCH, e, shortName, shortName);
                        } catch (InvalidPropertyException e) {
                            throw new DictionaryException(
                                    ERR_PROPERTY_MISMATCH, e, shortName, shortName);
                        }
    ...
    /* package */ M2ModelDiff diffConstraint(ConstraintDefinition conDef) {
        ...
        if (this == conDef) {
            modelDiff =
                    new M2ModelDiff(name, M2ModelDiff.TYPE_CONSTRAINT, M2ModelDiff.DIFF_UNCHANGED);
            return modelDiff;
        }
        ...
        if (!EqualsHelper.nullSafeEquals(
                getDescription(null), conDef.getDescription(null), false)) {
            isUpdatedIncrementally = true;
        }

        // check type string
        if (!EqualsHelper.nullSafeEquals(
                getConstraint().getType(), conDef.getConstraint().getType())) {
            isUpdated = true;
        }

        if (isUpdated) {
            modelDiff =
                    new M2ModelDiff(name, M2ModelDiff.TYPE_CONSTRAINT, M2ModelDiff.DIFF_UPDATED);
        } else if (isUpdatedIncrementally) {
            modelDiff =
                    new M2ModelDiff(
                            name, M2ModelDiff.TYPE_CONSTRAINT, M2ModelDiff.DIFF_UPDATED_INC);
        } else {
            modelDiff =
                    new M2ModelDiff(name, M2ModelDiff.TYPE_CONSTRAINT, M2ModelDiff.DIFF_UNCHANGED);
        }
        ...
        for (ConstraintDefinition previousConstraint : previousConstraints) {
            ...
            for (ConstraintDefinition newConstraint : newConstraints) {
                if (newConstraint.getName().equals(previousConstraint.getName())) {
                    modelDiffs.add(
                            ((M2ConstraintDefinition) previousConstraint)
                                    .diffConstraint(previousConstraint));
                    found = true;
                    break;
                }
            }

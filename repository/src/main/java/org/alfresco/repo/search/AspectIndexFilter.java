...
public class AspectIndexFilter extends AbstractIndexFilter {
  ...
  public void setIgnorePathsForSpecificAspects(
    boolean ignorePathsForSpecificAspects
  ) {
  ...
  public void init() {
    ...
    initIgnoringPathsByCriterion(
      aspectsForIgnoringPathsString,
      aspectsForIgnoringPaths,
    ...
    if ((null != aspects) && !aspects.isEmpty()) {
      for (QName aspectForIgnoringPaths : aspectsForIgnoringPaths) {
        ...
        for (QName nodeAspect : aspects) {
          if (
            dictionaryService.isSubClass(nodeAspect, aspectForIgnoringPaths)
          ) {
            return true;
          }
        }

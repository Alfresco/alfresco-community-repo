 ...
 *
 * <p>Checks are ignored is the target node doesn't exist.
 *
 * @author Derek Hulley
 */
    ...
    protected void checkSourceMultiplicity(
        ...
        if (assocDef.isChild()) {
            ...
            List<ChildAssociationRef> parentAssocRefs =
                    nodeService.getParentAssocs(
                            targetNodeRef, assocTypeQName, RegexQNamePattern.MATCH_ALL);
            actualSize = parentAssocRefs.size();
        } else {
            // check the source assocs present
            List<AssociationRef> sourceAssocRefs =
                    nodeService.getSourceAssocs(targetNodeRef, assocTypeQName);
        ...
        if ((mandatory && actualSize == 0) || (!allowMany && actualSize > 1)) {
            if (actualSize == 0) {
                ...
                //           At this point, there is no point worrying.  There are no more
                // associations
            ...
            IntegrityRecord result =
                    new IntegrityRecord(
                            "The association "
                                    + parentOrSourceStr
                                    + " multiplicity has been violated: \n"
                                    + "   Target Node: "
                                    + targetNodeRef
                                    + "\n"
                                    + "   Association: "
                                    ...
                                    + parentOrSourceStr
                                    + " Multiplicity: "
                                    + getMultiplicityString(mandatory, allowMany)
                                    + "\n"
                                    + "   Actual "
                                    + parentOrSourceStr
                                    + " Multiplicity: "

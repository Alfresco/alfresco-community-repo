...
public abstract class AbstractTestApi {
  ...
  protected Paging getPaging(
    ...
    ExpectedPaging expectedPaging = ExpectedPaging.getExpectedPaging(
      skipCount,
      maxItems,
      total,
      expectedTotal
  ...
  protected <T extends ExpectedComparison> void check(
  ...
  protected <T1 extends ExpectedComparison, T extends Object> void checkList(
    List<T1> expected,
    ExpectedPaging expectedPaging,
    ListResponse<T> actual
  ) {
  ...
  protected <T extends Object> List<T> sublist(
    List<T> list,
    int skipCount,
    int maxItems
  ) {
   ...
   */
  protected abstract TestFixture getTestFixture(boolean createTestData)
    throws Exception;
}

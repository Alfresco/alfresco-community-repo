...
public class VirtualCheckOutCheckInServiceExtension
  ...
  public NodeRef checkout(
    ...
    NodeRef materialDestination = smartStore.materializeIfPossible(
      destinationParentNodeRef
    );
    NodeRef workingCopy = theTrait.checkout(
      materialNodeRef,
      materialDestination,
    ...
    if (parentReference != null) {
      Reference workingCopyReference = NodeProtocol.newReference(
        workingCopy,
        parentReference
      );
  ...
  public NodeRef checkin(
    ...
    CheckOutCheckInServiceTrait theTrait = getTrait();
    NodeRef materialWorkingCopy = smartStore.materializeIfPossible(
      workingCopyNodeRef
    );
    NodeRef materialOriginalNode = theTrait.checkin(
      materialWorkingCopy,
      versionProperties,
      contentUrl,
      keepCheckedOut
  ...
  private NodeRef virtualizeOriginalIfNeeded(
    ...
    if ((materialOriginalNode != null) && (workingCopyReference != null)) {
      Reference parentReference = workingCopyReference.execute(
        new GetParentReferenceMethod()
      );
      Reference originalReference = NodeProtocol.newReference(
  ...
  public NodeRef cancelCheckout(NodeRef workingCopyNodeRef) {
    NodeRef materialOriginalNode = getTrait()
      .cancelCheckout(smartStore.materializeIfPossible(workingCopyNodeRef));
  ...
  public NodeRef getWorkingCopy(NodeRef nodeRef) {
    CheckOutCheckInServiceTrait theTrait = getTrait();
    NodeRef materialWorkingCopy = theTrait.getWorkingCopy(
      smartStore.materializeIfPossible(nodeRef)
    );

    return virtualizeVersionIfNeeded(nodeRef, materialWorkingCopy);
  }

  private NodeRef virtualizeVersionIfNeeded(
    NodeRef originalNodeRef,
    NodeRef materialVersion
  ) {
    ...
    if (
      ...
      Reference parentReference = reference.execute(
        new GetParentReferenceMethod()
      );
      Reference workingCopyReference = NodeProtocol.newReference(
        materialVersion,
  ...
  public NodeRef getCheckedOut(NodeRef nodeRef) {
    CheckOutCheckInServiceTrait theTrait = getTrait();
    NodeRef materialChekedOut = theTrait.getCheckedOut(
      smartStore.materializeIfPossible(nodeRef)
    );

...
public class RecordUtils {
    ...
    /**
     * Utility method that generates a record identifier and adds the new identifier to the record's
     * name
     *
     * @param record the record to generate the identifier for
     */
    public static void generateRecordIdentifier(
            NodeService nodeService, IdentifierService identifierService, NodeRef record) {
        if (nodeService.getProperty(record, PROP_IDENTIFIER) == null) {
            // get the record id
            String recordId =
                    identifierService.generateIdentifier(
                            ASPECT_RECORD, nodeService.getPrimaryParent(record).getParentRef());
        ...
        // append the identifier to the name even if it's been randomly generated or it was already
        // set

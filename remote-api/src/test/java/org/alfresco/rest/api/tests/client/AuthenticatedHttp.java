...
public class AuthenticatedHttp extends AbstractHttp {
  ...
  private <T extends Object> T executeWithBasicAuthentication(
    ...
    try {
      ...
      if (!handled) {
        throw new RuntimeException(
          "Error while executing HTTP-call (" + method.getPath() + ")",
          t
        );
      }
  ...
  private <T extends Object> T executeWithTicketAuthentication(
    ...
    try {
      ...
      // Try executing the method
      int result = httpProvider
        .getHttpClient()
        .executeMethod(null, method, state);
   ...
   */
  private HttpState applyTicketToMethod(HttpMethod method, String ticket)
    throws URIException {

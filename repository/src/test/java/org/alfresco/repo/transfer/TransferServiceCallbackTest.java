...
public class TransferServiceCallbackTest extends TestCase {
  ...
  private static final String TRANSFER_TARGET_NAME =
    "TransferServiceImplUnitTest";
  ...
  protected void setUp() throws Exception {
    ...
    folder1 =
      fileFolderService
        .create(companyHome, "folder1", ContentModel.TYPE_FOLDER)
        .getNodeRef();

    file1 =
      fileFolderService
        .create(folder1, "file1", ContentModel.TYPE_CONTENT)
        .getNodeRef();
    fileFolderService
      .getWriter(file1)
      .putContent("This is purely test content.");
    file1ContentUrl =
      fileFolderService.getFileInfo(file1).getContentData().getContentUrl();

    file2 =
      fileFolderService
        .create(folder1, "file2", ContentModel.TYPE_CONTENT)
        .getNodeRef();
    fileFolderService
      .getWriter(file2)
      .putContent("This is purely test content.");
    file2ContentUrl =
      fileFolderService.getFileInfo(file2).getContentData().getContentUrl();

    file3 =
      fileFolderService
        .create(folder1, "file3", ContentModel.TYPE_CONTENT)
        .getNodeRef();
    fileFolderService
      .getWriter(file3)
      .putContent("This is purely test content.");
    file3ContentUrl =
      fileFolderService.getFileInfo(file3).getContentData().getContentUrl();
  }
  ...
  public void testCompleteSuccess() {
    ...
    TransferProgress[] statuses = new TransferProgress[] {
      status0,
      status1,
      status2,
      status3,
    ...
    when(mockedTransferTransmitter.begin(target, localRepositoryId, version))
      .thenReturn(transfer);
  ...
  public void testTargetAlreadyLocked() {
    ...
    when(mockedTransferTransmitter.begin(target, "localRepositoryId", version))
      .thenThrow(new TransferException("Simulate lock unavailable"));
  ...
  public void testSendContentFailed() {
    ...
    when(mockedTransferTransmitter.begin(target, localRepositoryId, version))
      .thenReturn(transfer);
    doThrow(new TransferException("Simulate failure to write content"))
      .when(mockedTransferTransmitter)
      .sendManifest(
        any(Transfer.class),
        any(File.class),
  ...
  private void verifyCallback(List<TransferEvent> expectedEvents) {
    ArgumentCaptor<TransferEvent> eventCaptor = ArgumentCaptor.forClass(
      TransferEvent.class
    );
    ...
    for (int index = 0; index < count; ++index) {
      ...
      assertEquals(
        ...
        capturedEvent.getClass()
      );
      assertEquals(
        "Event " + index,
        expectedEvent.getTransferState(),
        capturedEvent.getTransferState()
      );
      if (TransferEventError.class.isAssignableFrom(expectedEvent.getClass())) {
        assertEquals(
          ((TransferEventError) expectedEvent).getException(),
  ...
  private void configureBasicMockTransmitter(TransferProgress[] statuses) {
    doAnswer(
      new Answer<Object>() {
        ...
        public Object answer(InvocationOnMock invocation) throws Throwable {
          ...
          XMLTransferRequsiteWriter requisiteWriter = new XMLTransferRequsiteWriter(
            writer
          );
          requisiteWriter.startTransferRequsite();
          requisiteWriter.missingContent(
            file1,
    ...
    doAnswer(
      ...
      .when(mockedTransferTransmitter)
      .getTransferReport(any(Transfer.class), any(OutputStream.class));
    ...
    if (statuses != null) {
      if (statuses.length > 1) {
        when(mockedTransferTransmitter.getStatus(transfer))
          .thenReturn(
            statuses[0],
            Arrays.copyOfRange(statuses, 1, statuses.length)
          );

...
public class NodeDAOTest extends TestCase {
  ...
  public void setUp() {
    ...
    rootNodesCache =
      (SimpleCache<Serializable, ValueHolder<Node>>) ctx.getBean(
  ...
  public void testTransaction() throws Throwable {
    ...
    assertNull(
      "No Txn Commit time should be present in read-only txn",
      commitTime0
    );
    // First success
    ...
    assertNull(
      "No Txn Commit time should be present in untouched txn",
      commitTime1
    );
    // Second success
    newTxn[0] = true;
    ...
    assertNotNull(
      "Txn commit time should be present by forcing it",
      commitTime2
    );
  }
  ...
  public void testSelectNodePropertiesByDataType() throws Exception {
    ...
    RetryingTransactionCallback<List<NodePropertyEntity>> callback = new RetryingTransactionCallback<List<NodePropertyEntity>>() {
      public List<NodePropertyEntity> execute() throws Throwable {
        ...
        return nodeDAO.selectNodePropertiesByDataType(
    ...
    while (min.longValue() <= maxNodeId.longValue()) {
      ...
      // Get the properties
      List<NodePropertyEntity> props = txnHelper.doInTransaction(
        callback,
        true
      );
      for (NodePropertyEntity prop : props) {
        ...
        assertTrue(
          "the min should be inclusive.",
          min.longValue() <= nodeId.longValue()
        );
        assertTrue(
          "the max should be exclusive.",
          max.longValue() > nodeId.longValue()
        );
  ...
  public void testGetNodeIdsIntervalForType() throws Exception {
    ...
    Pair<Long, Long> interval1 = getNodeIdsInterval(
      0L,
      System.currentTimeMillis()
    );
    Pair<Long, Long> interval2 = getNodeIdsInterval(
      null,
      System.currentTimeMillis()
    );
  ...
  private Pair<Long, Long> getNodeIdsInterval(
    final Long minTxnTime,
    final Long maxTxnTime
  ) {
    RetryingTransactionCallback<Pair<Long, Long>> callback = new RetryingTransactionCallback<Pair<Long, Long>>() {
      public Pair<Long, Long> execute() throws Throwable {
        return nodeDAO.getNodeIdsIntervalForType(
  ...
  public void testSelectChildAssocsWithoutNodeAssocsOfTypes() {
    final StoreRef storeRef = new StoreRef(
    ...
    final AtomicLong min = new AtomicLong(0L);
    final AtomicLong max = new AtomicLong(0L);
    final Set<QName> typeAssocsToExclude = Collections.singleton(
      QName.createQName("noType")
    );
    ...
    RetryingTransactionCallback<List<Node>> callback = new RetryingTransactionCallback<List<Node>>() {
      public List<Node> execute() throws Throwable {
        long minNodeId = min.get();
        long maxNodeId = max.get();
        return nodeDAO.selectChildAssocsWithoutNodeAssocsOfTypes(
          parentId,
          minNodeId,
          maxNodeId,
          typeAssocsToExclude
    ...
    Pair<Long, Long> nodeRange = getNodeIdsInterval(
      0L,
      System.currentTimeMillis()
    );
  ...
  public void testRootNodeCacheEntries() throws Throwable {
    ...
    for (Serializable key : keys) {
      NodeEntity node = (NodeEntity) TransactionalCache.getSharedCacheValue(
        rootNodesCache,
        key
      );
  ...
  public void testGetMinTxInNodeIdRange() {
    ...
    Long minTxCommitTimeInRange = nodeDAO.getMinTxInNodeIdRange(
      fromNodeId,
      toNodeId
    );
  ...
  public void testGetMaxTxInNodeIdRange() {
    ...
    Long maxTxCommitTimeInRange = nodeDAO.getMaxTxInNodeIdRange(
      fromNodeId,
      toNodeId
    );

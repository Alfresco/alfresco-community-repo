...
/**
 * Unit tests for the SITE facet handling introduced in {@link ElasticsearchAggregationBuilder}.
 * <p>
...
public class ElasticsearchAggregationBuilderTest
{
    ...
    public void setUp()
    {
        builder = new ElasticsearchAggregationBuilder(namespaceDAO, dictionaryService, siteService, nodeService);
    ...
    @Test
    public void termsAggregations_withSiteFacet_detected_producesComplementaryAggregation()
    {
        setupSharedHomeResolution();
        when(siteService.listSites(null, null)).thenReturn(List.of(site1));

        SearchParameters params = new SearchParameters();
        params.addFieldFacet(new SearchParameters.FieldFacet(FIELD_SITE));

        List<TermsAggregationWrapper> wrappers = builder.termsAggregations(params, null).toList();

        assertEquals("SITE facet should produce exactly one wrapper", 1, wrappers.size());
        assertTrue("SITE wrapper must have a complementary aggregation",
                wrappers.get(0).complementaryAggregation().isPresent());
    }

    @Test
    public void termsAggregations_withSiteFacet_usesPrimaryHierarchyField()
    {
        setupSharedHomeResolution();
        when(siteService.listSites(null, null)).thenReturn(List.of(site1));

        List<TermsAggregationWrapper> wrappers = builder.termsAggregations(siteSearchParams(FIELD_SITE), null).toList();

        assertEquals(1, wrappers.size());
        assertEquals("SITE facet must target the primaryHierarchy field",
                PRIMARY_HIERARCHY_FIELD, wrappers.get(0).termsAggregation().field());
    }

    @Test
    public void termsAggregations_withLuceneSyntaxSiteFacet_usesPrimaryHierarchyField()
    {
        ...
        assertEquals(1, wrappers.size());
        assertEquals("@SITE (Lucene syntax) must also use primaryHierarchy",
                PRIMARY_HIERARCHY_FIELD, wrappers.get(0).termsAggregation().field());
    }

    @Test
    public void termsAggregations_withSiteFacet_includeListContainsAllSiteUUIDs()
    {
        setupSharedHomeResolution();
        ...
        List<String> includeTerms = wrappers.get(0).termsAggregation().include().terms();
    ...
    @Test
    public void termsAggregations_withSiteFacet_includeListContainsSharedHomeUUID()
    {
        setupSharedHomeResolution();
        when(siteService.listSites(null, null)).thenReturn(List.of(site1));
    ...
    @Test
    public void termsAggregations_withSiteFacet_includeListSizeMatchesTotalEntries()
    {
        setupSharedHomeResolution();
        when(siteService.listSites(null, null)).thenReturn(List.of(site1, site2));
        ...
        Integer aggSize = wrappers.get(0).termsAggregation().size();
        assertNotNull(aggSize);
        assertEquals("Aggregation size should match include list size", includeTerms.size(), aggSize.intValue());
    }

    @Test
    public void termsAggregations_withSiteFacet_postProcessingDataMapsSiteUUIDsToShortNames()
    {
        setupSharedHomeResolution();
        when(siteService.listSites(null, null)).thenReturn(List.of(site1, site2));
    ...
    @Test
    public void termsAggregations_withSiteFacet_postProcessingDataContainsSharedFilesLabel()
    {
        setupSharedHomeResolution();
    ...
    @Test
    public void termsAggregations_withSiteFacet_hasComplementaryAggregation()
    {
        setupSharedHomeResolution();
        when(siteService.listSites(null, null)).thenReturn(List.of(site1));
    ...
    @Test
    public void termsAggregations_withSiteFacet_complementaryAggregationNameHasRepositorySuffix()
    {
        setupSharedHomeResolution();
        when(siteService.listSites(null, null)).thenReturn(List.of(site1));
        ...
        TermsAggregationWrapper wrapper = wrappers.get(0);
    ...
    @Test
    public void termsAggregations_withSiteFacet_complementaryAggregationDisplayLabelIsRepository()
    {
        setupSharedHomeResolution();
        when(siteService.listSites(null, null)).thenReturn(List.of(site1));
        ...
        ComplementaryAggregation complementary = wrappers.get(0).complementaryAggregation().get();
        assertEquals("Complementary aggregation display label must be _REPOSITORY_",
                REPOSITORY_LABEL, complementary.displayLabel());
    }

    @Test
    public void termsAggregations_withSiteFacet_complementaryFilterQueryExcludesAllSiteAndSharedUUIDs()
    {
        setupSharedHomeResolution();
        when(siteService.listSites(null, null)).thenReturn(List.of(site1));
    ...
    @Test
    public void termsAggregations_withSiteFacet_usesFieldNameWhenNoLabelSet()
    {
        setupSharedHomeResolution();
        when(siteService.listSites(null, null)).thenReturn(List.of(site1));
        ...
        assertEquals("Aggregation name should default to the field name when no label is set",
                FIELD_SITE, wrappers.get(0).name());
    }

    @Test
    public void termsAggregations_withSiteFacet_usesCustomLabelWhenSet()
    {
        setupSharedHomeResolution();
        when(siteService.listSites(null, null)).thenReturn(List.of(site1));
        ...
        assertEquals("Aggregation name should use the custom label when set",
                customLabel, wrappers.get(0).name());
    }

    @Test
    public void termsAggregations_withSiteFacet_noSitesButSharedHomeExists_producesWrapper()
    {
        setupSharedHomeResolution();
        when(siteService.listSites(null, null)).thenReturn(List.of());
        ...
        assertEquals("Should produce one wrapper when only shared home is resolvable", 1, wrappers.size());
        List<String> includeTerms = wrappers.get(0).termsAggregation().include().terms();
    ...
    @Test
    public void termsAggregations_withSiteFacet_noSitesAndSharedHomeNotResolvable_returnsEmptyStream()
    {
        ...
        assertTrue("Should return empty stream when no sites and shared home cannot be resolved",
                wrappers.isEmpty());
    }

    @Test
    public void termsAggregations_withSiteFacet_siteWithNullNodeRef_isIgnored()
    {
        setupSharedHomeResolution();
    ...
    private SearchParameters siteSearchParams(String fieldName)
    {
        SearchParameters params = new SearchParameters();
        params.addFieldFacet(new SearchParameters.FieldFacet(fieldName));
        return params;
    }

...
public class DefaultComparisonUtils implements ComparisonUtils {
    ...
    @Override
    public void compareSimpleOrderedLists(
            DbProperty refProp, DbProperty targetProp, DiffContext ctx) {
        ...
        ArrayList<Object> refList =
                new ArrayList<Object>((List<Object>) refProp.getPropertyValue());
        @SuppressWarnings("unchecked")
        ArrayList<Object> targetList =
                new ArrayList<Object>((List<Object>) targetProp.getPropertyValue());
        ...
        for (int i = 0; i < maxSize; i++) {
            if (i < refList.size() && i < targetList.size()) {
                DbProperty refIndexedProp =
                        new DbProperty(refProp.getDbObject(), refProp.getPropertyName(), i);
                DbProperty targetIndexedProp =
                        new DbProperty(targetProp.getDbObject(), targetProp.getPropertyName(), i);

                if (refList.get(i).equals(targetList.get(i))) {
                    differences.add(Where.IN_BOTH_NO_DIFFERENCE, refIndexedProp, targetIndexedProp);
                } else {
                    differences.add(
                            Where.IN_BOTH_BUT_DIFFERENCE, refIndexedProp, targetIndexedProp);
                }
            } else if (i < refList.size()) {
                DbProperty indexedProp =
                        new DbProperty(refProp.getDbObject(), refProp.getPropertyName(), i);
    ...
    /**
     * Ensure the property is carrying a list as its payload. A List is required rather than a
     * Collection as the latter may not be ordered.
     *
    ...
    private void checkPropertyContainsList(DbProperty prop) {
        if (!List.class.isAssignableFrom(prop.getPropertyValue().getClass())) {
            throw new IllegalArgumentException(
                    "List required, but was " + prop.getPropertyValue().getClass());
        }
    }

    @Override
    public void compareSimpleCollections(
            DbProperty leftProp, DbProperty rightProp, DiffContext ctx) {
        // Check whether the leftProperty should be compared to the rightProperty
        ...
        @SuppressWarnings("unchecked")
        Collection<? extends Object> leftCollection =
                (Collection<? extends Object>) leftProp.getPropertyValue();
        @SuppressWarnings("unchecked")
        Collection<? extends Object> rightCollection =
                (Collection<? extends Object>) rightProp.getPropertyValue();
        ...
        for (int leftIndex = 0; leftIndex < leftList.size(); leftIndex++) {
            ...
            DbProperty leftIndexedProp =
                    new DbProperty(leftProp.getDbObject(), leftProp.getPropertyName(), leftIndex);
            ...
            if ((rightIndex = rightList.indexOf(leftObj)) != -1) {
                ...
                // with a 'simple' value — as there is no way of knowing if the term represents the
                // same value
                // (e.g. two strings {red_value, green_value}, are these meant to be the same or
                // different?)
                DbProperty rightIndexedProp =
                        new DbProperty(
                                rightProp.getDbObject(), rightProp.getPropertyName(), rightIndex);
    ...
    public void compareCollections(
            Collection<? extends DbObject> leftCollection,
    ...
    private void checkNotDbObject(Object obj) {
        if (obj != null && DbObject.class.isAssignableFrom(obj.getClass())) {
            throw new IllegalArgumentException(
                    "Property value is a DbObject - this method shouldn't be used to compare this"
                            + " type: "
                            + obj);
        }
    }

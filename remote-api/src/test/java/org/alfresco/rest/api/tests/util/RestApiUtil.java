...
public class RestApiUtil {
    ...
    /**
     * Parses the alfresco REST API response for a collection of entries. Basically, it looks for
     * the {@code list} JSON object, then it uses {@literal Jackson} to convert the list's entries
     * to their corresponding POJOs based on the given {@code clazz}.
     *
     ...
     * @throws Exception
     */
    public static <T> List<T> parseRestApiEntries(JSONObject jsonObject, Class<T> clazz)
            throws Exception {
    ...
    /**
     * Parses the alfresco REST API response for a single entry. Basically, it looks for the {@code
     * entry} JSON object, then it uses {@literal Jackson} to convert it to its corresponding POJO
     ...
     * @throws Exception
     */
    public static PublicApiClient.ExpectedPaging parsePaging(JSONObject jsonObject)
            throws Exception {
        assertNotNull(jsonObject);
        JSONObject jsonList = (JSONObject) jsonObject.get("list");
        assertNotNull(jsonList);
    ...
    /**
     * Parses the alfresco REST API response, uses {@literal Jackson} to convert it to its
     * corresponding POJO based on the given {@code clazz}.
     *
     * @param jsonObject the {@code JSONObject} derived from the response
     * @param clazz the class which represents the JSON payload
     * @return the POJO of the given {@code clazz} type
     * @throws Exception
     */
    public static <T> T parsePojo(String key, JSONObject jsonObject, Class<T> clazz)
            throws Exception {
        assertNotNull(jsonObject);
        assertNotNull(clazz);
     ...
     * @throws Exception
     */
    public static PublicApiClient.ExpectedErrorResponse parseErrorResponse(JSONObject jsonObject)
            throws Exception {
    ...
    /**
     * Converts the POJO which represents the JSON payload into a JSON string. null values will be
     * ignored.
     */
    public static String toJsonAsStringNonNull(Object object) throws IOException {
        ...
        // keep null values in maps (like 'properties' for example) since these are part of the
        // partial update/delete payload

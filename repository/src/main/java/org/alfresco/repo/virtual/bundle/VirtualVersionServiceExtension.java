...
public class VirtualVersionServiceExtension
  ...
  public class VirtualVersionHistory implements VersionHistory {
    ...
    public Version getPredecessor(Version version) {
      Version actualVersion =
        VirtualVersionServiceExtension.this.materializeVersionIfReference(
            version
          );
    ...
    public Collection<Version> getSuccessors(Version version) {
      ...
      Collection<Version> actualSuccessors = actualHistory.getSuccessors(
        actualVersion
      );
  ...
  private Version materializeVersionIfReference(Version virtualVersion) {
    ...
    Map<String, Serializable> actualProperties = new HashMap<>(
      virtualProperties
    );

    if (frozenStoreRef.getIdentifier().equals(Version2Model.STORE_ID)) {
      // V2 version store (eg. workspace://version2Store)
      NodeRef propFrozenNode = (NodeRef) virtualProperties.get(
      ...
      String frozenNodeStoreProtocol = (String) virtualProperties.get(
        VersionModel.PROP_FROZEN_NODE_STORE_PROTOCOL
      );
      String frozenNodeStoreId = (String) virtualProperties.get(
        VersionModel.PROP_FROZEN_NODE_STORE_ID
      );
      String frozenNodeId = (String) virtualProperties.get(
        VersionModel.PROP_FROZEN_NODE_ID
      );

      NodeRef propFrozenNode = new NodeRef(
    ...
    Version actualVersion = new VersionImpl(
      actualProperties,
  ...
  private Version virtualizeVersion(
    Reference versionedReference,
    Version actualVersion
  ) {
    ...
    Reference parentReference = versionedReference.execute(
      new GetParentReferenceMethod()
    );
    Reference virtualFrozenReference = NodeProtocol.newReference(
      frozenStateNodeRef,
      parentReference
    );

    Map<String, Serializable> properties = actualVersion.getVersionProperties();
    Map<String, Serializable> virtualProperties = new HashMap<String, Serializable>(
      properties
    );
    ...
    if (frozenStoreRef.getIdentifier().equals(Version2Model.STORE_ID)) {
      // V2 version store (eg. workspace://version2Store)
      NodeRef propFrozenNodeRef = (NodeRef) virtualProperties.get(
        Version2Model.PROP_FROZEN_NODE_REF
      );
      ...
      StoreRef propFrozenStoreRef = new StoreRef(
        frozenNodeStoreProtocol,
        frozenNodeStoreId
      );
      ...
      NodeRef virtualPropFrozenNodeRef = virtualPropFrozenReference.toNodeRef(
        propFrozenStoreRef
      );
  ...
  private Collection<Version> virtualizeVersions(
    ...
    for (Version actualVersion : actualVersions) {
      Version virtualizedVersion = virtualizeVersion(
        versionedReference,
        actualVersion
      );
  ...
  public Version createVersion(
    ...
    if (reference == null) {
      ...
      Version actualVersion = theTrait.createVersion(
        materialNode,
        versionProperties
      );
  ...
  public Collection<Version> createVersion(
    ...
    if (reference == null) {
      ...
      NodeRef materialNode = smartStore.materializeIfPossible(nodeRef);
      Collection<Version> actualVersions = theTrait.createVersion(
        materialNode,
        versionProperties,
        versionChildren
      );
  ...
  public Collection<Version> createVersion(
    ...
    Collection<Version> versions = theTrait.createVersion(
      materialNodeRefs,
      versionProperties
    );
    Collection<Version> virtualizedVersions = new LinkedList<>();
  ...
  public VersionHistory getVersionHistory(NodeRef nodeRef)
    ...
    if (reference == null) {
      ...
      NodeRef materialNode = smartStore.materialize(reference);
      VersionHistory actualVersionHistory = theTrait.getVersionHistory(
        materialNode
      );
      if (actualVersionHistory == null) {
        ...
        return new VirtualVersionHistory(
          versionedReference,

...
public class RepositoryScriptProcessor implements ScriptProcessor {
  ...
  public Object executeScript(
    ...
    return scriptService.executeScript(
      "javascript",
      new RepositoryScriptLocation(content),
      model
    );
  }
  ...
  private void init() {
    ...
    for (Store apiStore : searchPath.getStores()) {
      ...
      if (loader == null) {
        throw new WebScriptException(
          "Unable to retrieve script loader for Web Script store " +
    ...
    scriptLoader =
      new MultiScriptLoader(loaders.toArray(new ScriptLoader[loaders.size()]));
  }

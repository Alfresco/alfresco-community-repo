...
public class ScriptRenditionService extends BaseScopableProcessorExtension {
    ...
    private RenditionDefinition loadRenditionDefinitionImpl(String shortOrLongFormQName) {
        ...
        // has Consumer access by default. However, we cannot assume that that access level applies
        // for all deployments. See ALF-7334.
    ...
    /**
     * Creates a new {@link ScriptRenditionDefinition} and sets the rendition name and the rendering
     ...
     * @see org.alfresco.service.cmr.rendition.RenditionService#createRenditionDefinition(QName,
     *     String)
     */
    public ScriptRenditionDefinition createRenditionDefinition(
        ...
        RenditionDefinition rendDef =
                renditionService.createRenditionDefinition(renditionQName, renderingEngineName);

        return new ScriptRenditionDefinition(
                serviceRegistry, this.getScope(), engineDefinition, rendDef);
    }
     ...
     * @see
     *     org.alfresco.service.cmr.rendition.RenditionService#render(org.alfresco.service.cmr.repository.NodeRef,
     *     RenditionDefinition)
     */
    public ScriptNode render(ScriptNode sourceNode, String renditionDefQName) {
        if (logger.isDebugEnabled()) {
            ...
            msg.append("Rendering source node '")
                    .append(sourceNode)
                    .append("' with renditionDef '")
                    .append(renditionDefQName)
                    .append("'");
            logger.debug(msg.toString());
        }
    ...
    public ScriptNode render(ScriptNode sourceNode, ScriptRenditionDefinition scriptRenditionDef) {
        if (logger.isDebugEnabled()) {
            ...
            msg.append("Rendering source node '")
                    .append(sourceNode)
                    .append("' with renditionDefQName '")
                    .append(scriptRenditionDef)
                    .append("'");
            logger.debug(msg.toString());
        }

        ChildAssociationRef chAssRef =
                this.renditionService.render(
                        sourceNode.getNodeRef(), scriptRenditionDef.getRenditionDefinition());
     ...
     * @see
     *     org.alfresco.service.cmr.rendition.RenditionService#getRenditions(org.alfresco.service.cmr.repository.NodeRef)
     */
    public ScriptNode[] getRenditions(ScriptNode node) {
        List<ChildAssociationRef> renditions =
                this.renditionService.getRenditions(node.getNodeRef());
    ...
    public ScriptNode[] getRenditions(ScriptNode node, String mimeTypePrefix) {
        List<ChildAssociationRef> renditions =
                this.renditionService.getRenditions(node.getNodeRef(), mimeTypePrefix);
     ...
     *     "{http://www.alfresco.org/model/content/1.0}imgpreview"
     * @return the parent association for the rendition or <code>null</code> if there is no such
     *     rendition.
     * @see
     *     org.alfresco.service.cmr.rendition.RenditionService#getRenditionByName(org.alfresco.service.cmr.repository.NodeRef,
     *     QName)
     */
    public ScriptNode getRenditionByName(ScriptNode node, String renditionName) {
        ...
        ChildAssociationRef result =
                this.renditionService.getRenditionByName(node.getNodeRef(), qname);

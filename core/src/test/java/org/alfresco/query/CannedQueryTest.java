...
public class CannedQueryTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        // ONE
        TestCannedQueryFactory<String> namedQueryFactoryOne =
                new TestCannedQueryFactory(RESULTS_ONE);
    ...
    public void testRegistry() throws Exception {
        ...
        try {
            TestCannedQueryFactory<Long> namedQueryFactoryBogus =
                    new TestCannedQueryFactory(RESULTS_TWO);
    ...
    public void testQueryPermissionCheckedResults() throws Exception {
        ...
        assertEquals(
                "Incorrect result order", "ONE_4", qrOne.getPages().get(0).get(4)); // << missing 5!
    ...
    public void testQueryPermissionCheckedPagedTotalCount() throws Exception {
        ...
        CannedQueryParameters params =
                new CannedQueryParameters(null, qPageDetails, qSortDetails, 1000, null);
        CannedQuery<String> qOne = qfOne.getCannedQuery(params);
        CannedQueryResults<String> qrOne = qOne.execute();
        // Check pages
        assertEquals(
                ...
                qrOne.getTotalResultCount()); // Pre-paging
        assertEquals(
                "Incorrect number of paged results", 1, qrOne.getPagedResultCount()); // Skipped 5
        assertEquals(
                "Incorrect result order",
                "ONE_3",
                qrOne.getPages().get(0).get(0)); // Order reversed
        assertTrue("Should have more pages/items", qrOne.hasMoreItems());
    }
    ...
    private static class TestCannedQuery<T> extends AbstractCannedQuery<T> {
        ...
        protected List<T> applyPostQuerySorting(
                List<T> results, CannedQuerySortDetails sortDetails) {

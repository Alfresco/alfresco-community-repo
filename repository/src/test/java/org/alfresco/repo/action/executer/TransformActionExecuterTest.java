    ...
    public void transformName() throws Exception {
        ...
        String newName =
                TransformActionExecuter.transformName(
                        dummyMimetypeService, original, newMimetype, true);
        ...
        newName =
                TransformActionExecuter.transformName(
                        dummyMimetypeService, original, newMimetype, false);
        ...
        newName =
                TransformActionExecuter.transformName(
                        dummyMimetypeService, original, unknownMimetype, true);
        assertEquals("Letter to Bank Manager.bin", newName);
    }
}

/**
 * This is a very simple stub for the MimetypeService. For this test, we're only interested in the
 * values that come back from the getExtension call.
 */
class DummyMimetypeService implements MimetypeService {
    private final String result;

    public DummyMimetypeService(String result) {
        this.result = result;
    }

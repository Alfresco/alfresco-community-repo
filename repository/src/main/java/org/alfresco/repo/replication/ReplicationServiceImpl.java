...
public class ReplicationServiceImpl
  ...
  public ReplicationDefinition createReplicationDefinition(
    ...
    if (log.isDebugEnabled()) {
      ...
      msg
        .append("Creating replication definition ")
        .append(replicationDefinitionName);
      log.debug(msg.toString());
    }
    return new ReplicationDefinitionImpl(
      GUID.generate(),
  ...
  public ReplicationDefinition loadReplicationDefinition(
    ...
    ReplicationDefinitionImpl rd = (ReplicationDefinitionImpl) replicationDefinitionPersister.loadReplicationDefinition(
      replicationDefinitionName
    );
    ...
    if (rd != null) {
      ...
      rd.setSchedule(scheduledPersistedActionService.getSchedule(rd));
    }
    return rd;
  }

  private List<ReplicationDefinition> attachSchedules(
    ...
    for (ReplicationDefinition rd : definitions) {
      if (rd != null) {
        ...
        rdi.setSchedule(scheduledPersistedActionService.getSchedule(rdi));
      }
    }
    return definitions;
  }
  ...
  public List<ReplicationDefinition> loadReplicationDefinitions(String target) {
    return attachSchedules(
      replicationDefinitionPersister.loadReplicationDefinitions(target)
    );
  }
  ...
  public void renameReplicationDefinition(
    ...
    replicationDefinitionPersister.renameReplicationDefinition(
      oldReplicationName,
      newReplicationName
    );
  }
  ...
  public void deleteReplicationDefinition(
    ...
    replicationDefinitionPersister.deleteReplicationDefinition(
      replicationDefinition
    );
  }
  ...
  public void replicate(ReplicationDefinition replicationDefinition) {
    if (isEnabled()) {
      ...
      throw new ReplicationServiceException(
        "Unable to replicate. The replication service is not enabled"
      );
    }
  }
  ...
  public void disableScheduling(ReplicationDefinition replicationDefinition) {
    ...
    if (replicationDefinition.isSchedulingEnabled()) {
      scheduledPersistedActionService.deleteSchedule(definition.getSchedule());
    }
  ...
  public void enableScheduling(ReplicationDefinition replicationDefinition) {
    if (!replicationDefinition.isSchedulingEnabled()) {
      ScheduledPersistedAction schedule = scheduledPersistedActionService.createSchedule(
        replicationDefinition
      );

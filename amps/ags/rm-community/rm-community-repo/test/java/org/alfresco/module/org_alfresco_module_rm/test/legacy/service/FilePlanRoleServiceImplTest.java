...
public class FilePlanRoleServiceImplTest extends BaseRMTestCase {
    ...
    doTestInTransaction(
      new Test<Void>() {
        public Void run() {
          ...
          Set<Role> rolesIncludingSystemRoles = filePlanRoleService.getRoles(
            filePlan,
            true
          );
          ...
          Set<Role> rolesWithoutSystemRoles = filePlanRoleService.getRoles(
            filePlan,
            false
          );
          ...
          assertTrue(
            rolesIncludingSystemRoles.size() > rolesWithoutSystemRoles.size()
          );
          assertTrue(
            rolesIncludingSystemRoles.size() ==
            rolesWithoutSystemRoles.size() +
            FilePlanRoleService.SYSTEM_ROLES.size()
          );
    ...
    doTestInTransaction(
      new Test<Void>() {
        public Void run() {
          ...
          Set<Role> rolesIncludingSystemRoles = filePlanRoleService.getRolesByUser(
            filePlan,
            rmUserName,
            true
          );
  ...
  public void testGetRole() throws Exception {
    doTestInTransaction(
      new Test<Void>() {
        public Void run() {
          Role role = filePlanRoleService.getRole(
            filePlan,
  ...
  public void testExistsRole() throws Exception {
    doTestInTransaction(
      new Test<Void>() {
        public Void run() {
          assertTrue(
            filePlanRoleService.existsRole(
              filePlan,
              FilePlanRoleService.ROLE_POWER_USER
            )
          );
  ...
  public void testCreateUpdateDeleteRole() throws Exception {
    doTestInTransaction(
      new Test<Void>() {
        public Void run() {
          assertFalse(
            filePlanRoleService.existsRole(filePlan, "Michelle Holt")
          );

          Set<Capability> caps = new HashSet<>(2);
          caps.add(
            capabilityService.getCapability(RMPermissionModel.ACCESS_AUDIT)
          );
          caps.add(
          ...
          caps.add(
            capabilityService.getCapability(
              RMPermissionModel.AUTHORIZE_ALL_TRANSFERS
            )
          );
    ...
    doTestInTransaction(
      new Test<Void>() {
        public Void run() {
          ...
          Set<String> authorities = filePlanRoleService.getUsersAssignedToRole(
            filePlan,
          ...
          authorities =
            filePlanRoleService.getAllAssignedToRole(
              filePlan,
              FilePlanRoleService.ROLE_RECORDS_MANAGER
            );
          assertNotNull(authorities);

 ...
 *
 * <p>Note: based on earlier AclDaoComponentImpl
 *
...
public class AclDAOImpl implements AclDAO {
    ...
    public Acl createAccessControlList(
            AccessControlListProperties properties, List<AccessControlEntry> aces, Long inherited) {
        if (properties == null) {
            throw new IllegalArgumentException("Properties cannot be null");
        }
        ...
        if ((aces != null) && aces.size() > 0) {
            for (AccessControlEntry ace : aces) {
                ...
                // Find ACE
                Ace entry =
                        aclCrudDAO.getOrCreateAce(
                                permission, authority, ace.getAceType(), ace.getAccessStatus());
    ...
    private void getWritable(
        ...
        if ((mode == WriteMode.ADD_INHERITED)
    ...
    /**
     * Make a whole tree of ACLs copy on write if required Includes adding and removing ACEs which
     * can be optimised slightly for copy on write (no need to add and then remove)
     */
    private void getWritable(
            final Long id,
            final Long parent,
                ...
                if (visitedAcls.contains(nextId)) {
                    if (logger.isWarnEnabled()) {
                        StringBuilder message =
                                new StringBuilder("ACL cycle detected! Repeated ALC id = '")
                                        .append(nextId)
    ...
    private AclChange getWritable(
        ...
        if (!acl.isVersioned()) {
            ...
            return new AclChangeImpl(id, id, acl.getAclType(), acl.getAclType());
        } else if ((acl.getAclChangeSetId() == getCurrentChangeSetId())
                && (!requiresVersion)
            ...
            if (members.size() > 0) {
                List<Pair<Long, Integer>> aceIdsWithDepths =
                        new ArrayList<Pair<Long, Integer>>(members.size());
    ...
    private void removeAcesFromAcl(
            final Long id, final List<? extends AccessControlEntry> exclude, final int depth) {
        if (exclude == null) {
    ...
    private void replaceInherited(
            Long id, Acl acl, List<Ace> inherited, List<Integer> positions, int depth) {
        truncateInherited(id, depth);
        addInherited(acl, inherited, positions, depth);
    }
    ...
    private void addInherited(Acl acl, List<Ace> inherited, List<Integer> positions, int depth) {
        if ((inherited != null) && (inherited.size() > 0)) {
            List<Pair<Long, Integer>> aceIdsWithDepths =
                    new ArrayList<Pair<Long, Integer>>(inherited.size());
    ...
    private void insertInherited(
            final Long id, AclEntity acl, List<Ace> inherited, List<Integer> positions, int depth) {
        // get aces for acl (via acl member)
        List<AclMemberEntity> members = aclCrudDAO.getAclMembersByAclForUpdate(id);
        ...
        if (members.size() > 0) {
            ...
            for (AclMember member : members) {
                ...
                if (!hasAnotherTenantNodes) {
                    ...
                    aclChanges.add(
                            new AclChangeImpl(aclId, aclId, list.getAclType(), list.getAclType()));
                    acls.add(list);
    ...
    public List<AclChange> deleteAccessControlList(final Long id) {
        ...
        if (acl.getAclType() == ACLType.SHARED) {
            throw new UnsupportedOperationException(
                    "Delete is not supported for shared acls - they are deleted with the defining"
                            + " acl");
        }
        ...
        if ((acl.getAclType() == ACLType.DEFINING) || (acl.getAclType() == ACLType.LAYERED)) {
            if ((acl.getInheritedAcl() != null) && (acl.getInheritedAcl() != -1)) {
                ...
                if (inherited.isVersioned()) {
                    AclUpdateEntity inheritedForUpdate =
                            aclCrudDAO.getAclForUpdate(inherited.getId());
    ...
    public AccessControlList getAccessControlList(Long id) {
        ...
        // for (AclMemberEntity member : members)
        {
            ...
            QName permTypeQName =
                    qnameDAO.getQName(perm.getTypeQNameId()).getSecond(); // Has an ID so must exist
            SimplePermissionReference permissionRefernce =
                    SimplePermissionReference.getPermissionReference(permTypeQName, perm.getName());
    ...
    public List<AclChange> mergeInheritedAccessControlList(Long inherited, Long target) {
        ...
        if ((targetAcl.getAclType() != ACLType.DEFINING)
        ...
        if ((inheritedAcl.getAclType() == ACLType.DEFINING)
    ...
    public List<AclChange> enableInheritance(Long id, Long parent) {
        ...
        switch (acl.getAclType()) {
            ...
            case SHARED:
                ...
                throw new IllegalArgumentException(
                        "Shared acls should be replace by creating a definig ACL, wiring it up for"
                            + " inhertitance, and then applying inheritance to any children. It can"
            ...
            default:
                ...
                List<AclChange> merged =
                        mergeInheritedAccessControlList(parent, changes.get(0).getAfter());
        ...
        switch (mode) {
            ...
            case REDIRECT:
                ...
                switch (aclToCopy.getAclType()) {
                    case DEFINING:
                        // This is not called on the redirecting node as only LAYERED change
                        // permissions when redirected
                    ...
                    case LAYERED:
                        ...
                        List<AclChange> changes =
                                mergeInheritedAccessControlList(toInheritFrom, toCopy);
    ...
    private List<AclChange> disableInheritanceImpl(
            Long id, boolean setInheritedOnAcl, AclEntity aclIn) {
        List<AclChange> changes = new ArrayList<AclChange>();
        ...
        // Manages caching
        getWritable(
                acl.getId(), null, null, null, null, true, changes, WriteMode.TRUNCATE_INHERITED);
    ...
    private class UpdateChangeSetListener extends TransactionListenerAdapter {
        ...
        public void beforeCommit(boolean readOnly) {
            ...
            Long changeSetId =
                    (Long) AlfrescoTransactionSupport.getResource(RESOURCE_KEY_ACL_CHANGE_SET_ID);
            if (changeSetId == null) {
    ...
    /**
     * Support to get the current ACL change set and bind this to the transaction. So we only make
     * one new version of an ACL per change set. If something is in the current change set we can
     * update it.
     */
    ...
    public void fixSharedAcl(Long shared, Long defining) {
        ...
        List<AclChange> changes = new ArrayList<AclChange>();
        getWritable(
                shared, defining, null, null, defining, true, changes, WriteMode.CHANGE_INHERITED);
    }

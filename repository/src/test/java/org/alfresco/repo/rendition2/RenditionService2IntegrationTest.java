...
public class RenditionService2IntegrationTest
  ...
  public void changedSourceToNullContent() {
    ...
    ChildAssociationRef assoc = AuthenticationUtil.runAs(
      () -> renditionService2.getRenditionByName(sourceNodeRef, DOC_LIB),
      ADMIN
    );
  ...
  public void changedSourceToNonNull() {
    ...
    ContentData contentData1 = DefaultTypeConverter.INSTANCE.convert(
      ContentData.class,
      nodeService.getProperty(rendition1, PROP_CONTENT)
    );

    updateContent(ADMIN, sourceNodeRef, "quick.png");
    render(ADMIN, sourceNodeRef, DOC_LIB);
    NodeRef rendition2 = waitForRendition(ADMIN, sourceNodeRef, DOC_LIB, true);
    ContentData contentData2 = DefaultTypeConverter.INSTANCE.convert(
      ContentData.class,
      nodeService.getProperty(rendition2, PROP_CONTENT)
    );

    assertEquals(
      "The rendition node should not change",
      rendition1,
      rendition2
    );
    assertNotEquals(
      "The content should have change",
      contentData1.toString(),
  ...
  public void testCreateRenditionByUser() {
    ...
    NodeRef renditionNodeRef = waitForRendition(
      userName,
      sourceNodeRef,
      DOC_LIB,
      true
    ...
    assertNotNull(
      "The rendition was not generated for non-admin user",
      renditionNodeRef
    );
  }
  ...
  public void testReadRenditionByOtherUser() {
    ...
    assertNotNull(
      "The rendition is not visible for owner of source node",
      renditionNodeRef
    );
    renditionNodeRef =
      waitForRendition(otherUserName, sourceNodeRef, DOC_LIB, true);
    assertNotNull(
      "The rendition is not visible for non-owner user with read permissions",
      renditionNodeRef
    );
    assertEquals(
      "The creator of the rendition is not correct",
      ownerUserName,
      nodeService.getProperty(sourceNodeRef, ContentModel.PROP_CREATOR)
    );
  }
  ...
  public void testRenderByReader() {
    ...
    renditionNodeRef =
      waitForRendition(otherUserName, sourceNodeRef, DOC_LIB, true);
    assertNotNull(
      "The rendition is not visible for owner of rendition node",
      renditionNodeRef
    );
    assertEquals(
      "The creator of the rendition is not correct",
  ...
  public void testUpgradeRenditionService() throws InterruptedException {
    String ownerUserName = createRandomUser();
    NodeRef sourceNodeRef = createSource(ownerUserName, "quick.jpg");
    final QName doclibRendDefQName = QName.createQName(
    ...
    transactionService
      ...
      .doInTransaction(() ->
        AuthenticationUtil.runAs(
          () -> renditionService.render(sourceNodeRef, doclibRendDefQName),
          ownerUserName
        )
      );
    ...
    NodeRef oldRendition = AuthenticationUtil.runAs(
      () ->
        renditionService
          .getRenditionByName(sourceNodeRef, doclibRendDefQName)
          .getChildRef(),
      ownerUserName
    );
    assertFalse(
      "The rendition should be generated by old Rendition Service",
      AuthenticationUtil.runAs(
        () ->
          nodeService.hasAspect(oldRendition, RenditionModel.ASPECT_RENDITION2),
        ownerUserName
      )
    );

    updateContent(ownerUserName, sourceNodeRef, "quick.png");
    NodeRef newRendition = waitForRendition(
      ownerUserName,
      sourceNodeRef,
      DOC_LIB,
      true
    ...
    assertNotNull(
      "The rendition should be reported via RenditionService2",
      newRendition
    );
    Thread.sleep(200);
    boolean hasRenditionedAspect = false;
    for (int i = 0; i < 5; i++) {
      hasRenditionedAspect =
        AuthenticationUtil.runAs(
          () ->
    ...
    assertTrue(
      "The rendition should be generated by new Rendition Service",
      hasRenditionedAspect
    );
  }
  ...
  public void testUseOldService() throws InterruptedException {
    ...
    try {
      ...
      assertNotNull(
        "The old renditions service did not render",
        waitForRendition(ADMIN, sourceNodeRef, DOC_LIB, true)
      );
      List<String> lastThumbnailModification = transactionService
        .getRetryingTransactionHelper()
        .doInTransaction(() ->
          AuthenticationUtil.runAs(
            () ->
      ...
      assertFalse(
        "The old rendition service did not update the rendition.",
        newThumbnailModification.equals(lastThumbnailModification)
      );
      NodeRef renditionNodeRef = AuthenticationUtil.runAs(
        () ->
          renditionService
            .getRenditionByName(sourceNodeRef, doclibRendDefQName)
            .getChildRef(),
      ...
      assertFalse(
        "The rendition should be rendered by the old rendition service",
        AuthenticationUtil.runAs(
          () ->
            nodeService.hasAspect(
              renditionNodeRef,
  ...
  public void testSwitchBackToOldService() throws InterruptedException {
    ...
    try {
      ...
      assertFalse(
        "Should have switched to the old rendition service",
        hasRendition2Aspect
      );
    } finally {
      renditionService2.setEnabled(true);
  ...
  public void testDowngradeRenditionService() throws InterruptedException {
    ...
    boolean hasRendition2Aspect = AuthenticationUtil.runAs(
      () ->
        nodeService.hasAspect(newRendition, RenditionModel.ASPECT_RENDITION2),
      ownerUserName
    );
    assertTrue(
      "The source should have the old renditioned aspect",
      AuthenticationUtil.runAs(
        () ->
          nodeService.hasAspect(
            sourceNodeRef,
    ...
    try {
      renditionService2.setEnabled(false);
      updateContent(ownerUserName, sourceNodeRef, "quick.png");
      NodeRef oldRendition = waitForRendition(
        ownerUserName,
        sourceNodeRef,
        DOC_LIB,
        true
  ...
  public void testUpgradeRenditionViaRender() throws InterruptedException {
    ...
    try {
      ...
      assertTrue(
        "Should have switched to the new rendition service",
        hasRendition2Aspect
      );
    } finally {
      renditionService2.setEnabled(true);

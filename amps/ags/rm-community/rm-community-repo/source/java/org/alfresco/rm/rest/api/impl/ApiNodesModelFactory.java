...
public class ApiNodesModelFactory {
  ...
  public static final List<String> EXCLUDED_NS = Arrays.asList(
  ...
  private void mapBasicInfo(
    ...
    if (propertyFilter.isAllowed(RMNode.PARAM_MODIFIED_BY_USER)) {
      ...
      UserInfo modifer = Node.lookupUserInfo(
    ...
    if (propertyFilter.isAllowed(RMNode.PARAM_CREATED_BY_USER)) {
      ...
      UserInfo creator = Node.lookupUserInfo(
  ...
  private void mapUnfiledChildInfo(
    UnfiledChild unfiledChild,
    FileInfo info,
    BeanPropertiesFilter propertyFilter
  ) {
    if (
      RecordsManagementModel.TYPE_UNFILED_RECORD_FOLDER.equals(info.getType())
    ) {
      if (
        propertyFilter.isAllowed(UnfiledChild.PARAM_IS_UNFILED_RECORD_FOLDER)
      ) {
  ...
  private void mapTransferContainerInfo(
    ...
    if (propertyFilter.isAllowed(RMNode.PARAM_NODE_TYPE)) {
      transferContainer.setNodeType(
        info.getType().toPrefixString(namespaceService)
      );
    }
    if (propertyFilter.isAllowed(RMNode.PARAM_MODIFIED_AT)) {
    ...
    if (!isMinimalInfo && propertyFilter.isAllowed(RMNode.PARAM_PROPERTIES)) {
      transferContainer.setProperties(
        mapFromNodeProperties(info.getProperties())
      );
    }
  ...
  private void mapTransferInfo(
    ...
    if (
      (
        !isMinimalInfo &&
        propertyFilter.isAllowed(Transfer.PARAM_TRANSFER_LOCATION)
      ) ||
    ...
    if (
      (
        !isMinimalInfo &&
        propertyFilter.isAllowed(Transfer.PARAM_TRANSFER_PDF_INDICATOR)
      ) ||
      (
        isMinimalInfo &&
  ...
  private void mapTransferChildInfo(
    ...
    if (RecordsManagementModel.TYPE_RECORD_FOLDER.equals(info.getType())) {
      if (
        isMinimalInfo &&
        includeParam.contains(TransferChild.PARAM_IS_RECORD_FOLDER)
      ) {
        transferChild.setIsRecordFolder(true);
      }
      if (
        isMinimalInfo && includeParam.contains(TransferChild.PARAM_IS_RECORD)
      ) {
      ...
      if (isMinimalInfo && includeParam.contains(RMNode.PARAM_IS_CLOSED)) {
        transferChild.setIsClosed(
          (Boolean) nodeService.getProperty(
            info.getNodeRef(),
            RecordsManagementModel.PROP_IS_CLOSED
          )
        );
      }
    } else {
  ...
  private void mapRecordCategoryChildInfo(
    ...
    if (RecordsManagementModel.TYPE_RECORD_FOLDER.equals(info.getType())) {
      ...
      if (
        (
          !isMinimalInfo &&
          propertyFilter.isAllowed(RecordCategoryChild.PARAM_IS_RECORD_CATEGORY)
        ) ||
        (
          isMinimalInfo &&
          includeParam.contains(RecordCategoryChild.PARAM_IS_RECORD_CATEGORY)
        )
      ) {
        recordCategoryChild.setIsRecordCategory(false);
      }
      if (
        (!isMinimalInfo && propertyFilter.isAllowed(RMNode.PARAM_IS_CLOSED)) ||
      ...
      if (includeParam.contains(RMNode.PARAM_HAS_RETENTION_SCHEDULE)) {
        DispositionSchedule ds = dispositionService.getDispositionSchedule(
          info.getNodeRef()
        );
  ...
  private void mapRecordInfo(
    ...
    if (includeParam.contains(Record.PARAM_CONTENT)) {
      ...
      if ((val != null) && (val instanceof ContentData)) {
        ...
        String mimeTypeName = serviceRegistry
          .getMimetypeService()
   ...
   */
  private Map<String, Object> mapFromNodeProperties(
  ...
  private void mapAssociations(
    ...
    if (includeParam.contains(RMNode.PARAM_INCLUDE_ASSOCIATION)) {
      ...
      ChildAssociationRef parentAssocRef = nodeService.getPrimaryParent(
        nodeRef
      );

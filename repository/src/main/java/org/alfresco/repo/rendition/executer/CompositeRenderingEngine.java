...
/**
 * This RenderingEngine is used for rendering {@link CompositeRenditionDefinition}s, which specify a
 * list of {@link RenditionDefinition}s. The {@link CompositeRenderingEngine} iterates over the
 * {@link RenditionDefinition}s sequentially and feeds the output of one definition in as the input
 * of the next definition. The output of the last definition executed is the output of this
 * rendering engine.
 *
 * @author Nick Smith
...
public class CompositeRenderingEngine extends AbstractRenderingEngine {
    ...
    protected void executeRenditionImpl(Action action, NodeRef sourceNode) {
        ...
        if (action instanceof CompositeRenditionDefinition) {
            CompositeRenditionDefinition compositeDefinition =
                    (CompositeRenditionDefinition) action;
            ChildAssociationRef renditionAssoc =
                    executeCompositeRendition(compositeDefinition, sourceNode);

            // Setting result.
            compositeDefinition.setParameterValue(PARAM_RESULT, renditionAssoc);
        } else {
            String msg =
                    "This method requires that the RenditionDefinition be of type"
    ...
    private ChildAssociationRef executeCompositeRendition(
            CompositeRenditionDefinition definition, NodeRef sourceNode) {
        ...
        for (RenditionDefinition subDefinition : definition.getActions()) {
            ChildAssociationRef nextResult =
                    executeSubDefinition(source, subDefinition, parent, assocType);
            if (result != null) {
     ...
     * will be executed with the {@link RenditionService#PARAM_IS_COMPONENT_RENDITION
     * is-component-rendition} flag set to true. This is so that the common pre- and post-rendition
     * code in {@link AbstractRenderingEngine#executeImpl(Action, NodeRef)} will only be executed at
     * the start and at the end of the chain of components.
     */
    private ChildAssociationRef executeSubDefinition(
            NodeRef source, //
        ...
        ChildAssociationRef newResult =
                (ChildAssociationRef) subDefinition.getParameterValue(PARAM_RESULT);
        return newResult;
    }

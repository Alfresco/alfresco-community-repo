...
public class ResourceDictionaryBuilder {
  ...
  private static Log logger = LogFactory.getLog(
    ResourceDictionaryBuilder.class
  );
  ...
  private static void processResources(
    ...
    for (Entry<ApiScopeKey, Map<Integer, List<ResourceWithMetadata>>> apiGroupedByNameAndScopeEntry : entityApiMap.entrySet()) {
      ...
      for (int i = 0; i < versions.size(); i++) {
        ...
        Map<String, ResourceWithMetadata> resourcesByApiAndVersion = findResources(
          rd.getAllResources(),
          apiVersion
        );
        ...
        if (i > 0) {
          logger.debug(
            "Has previous so adding all entities from previous version "
          );
          Api previousVersion = Api.valueOf(
            apiVersion.getName(),
            apiVersion.getScope().toString(),
            Integer.toString(apiVersion.getVersion() - 1)
          );
          logger.debug("Previous version is " + previousVersion);
          Map<String, ResourceWithMetadata> resourcesForPreviousVersion = findResources(
            rd.getAllResources(),
            previousVersion
          );
        ...
        for (ResourceWithMetadata resourceWithMetadata : apiGroupedByNameAndScopeEntry
          .getValue()
          .get(apiVersion.getVersion())) {
          resourcesByApiAndVersion.put(
            resourceWithMetadata.getMetaData().getUniqueId(),
  ...
  private static Map<String, ResourceWithMetadata> findResources(
    Map<Api, Map<String, ResourceWithMetadata>> allResources,
    Api api
  ) {
    Map<String, ResourceWithMetadata> resourcesByApiAndVersion = allResources.get(
      api
    );
  ...
  private static <T> Map<ApiScopeKey, Map<Integer, List<ResourceWithMetadata>>> parseResources(
    ...
    for (Object bean : resources) {
      List<ResourceMetadata> metaData = ResourceInspector.inspect(
        bean.getClass()
      );
      ...
      List<ResourceWithMetadata> resourcesWithMeta = apiVersions.get(
        api.getVersion()
      );
  ...
  public static ResourceDictionary build(
    Collection<Object> entityResources,
    ...
    Map<ApiScopeKey, Map<Integer, List<ResourceWithMetadata>>> apiMap = parseResources(
  ...
  public static class ApiScopeKey {
    ...
    public int hashCode() {
      ...
      result =
        prime * result + ((this.scope == null) ? 0 : this.scope.hashCode());
      return result;
    }

...
public class CompositeCapabilityTest extends BaseRMTestCase {
      ...
      new Test<Void>() {
        ...
        public Void run() {
          assertEquals(
            ...
            capability.hasPermission(rmContainer)
          );
          assertEquals(
            AccessStatus.ALLOWED,
            capability.hasPermission(rmFolder)
          );
          assertEquals(AccessStatus.ALLOWED, capability.hasPermission(record));
          assertEquals(
            AccessStatus.ALLOWED,
            capability.hasPermission(declaredRecord)
          );
  ...
  public void testUpdateProperties() {
    final Capability capability = capabilityService.getCapability(
      "UpdateProperties"
    );
    assertNotNull(capability);

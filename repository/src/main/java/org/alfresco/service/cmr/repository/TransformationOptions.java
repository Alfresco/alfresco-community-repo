...
/**
 * Class containing values of options that are passed to content transformers. These options are
 * used to determine the applicability of a content transformer and also during the transformation
 * process to provide context or parameter values.
 *
 * <p>This base class provides some common, optional contextual information about the source and
...
public class TransformationOptions implements Cloneable {
    ...
    private Map<Class<? extends TransformationSourceOptions>, TransformationSourceOptions>
            sourceOptionsMap;
    ...
    /**
     * Constructor. Creates a transformation options object from a map. Provided for back ward
     * compatibility.
     *
    ...
    /**
     * If the source content includes embedded resources, should the transformer attempt to
     * transform these as well? Not many transformers do support embedded resources, so this option
     * will only affect those that can.
     *
    ...
    /**
     * The use to which the transform will be put. Initially used to select different transformation
     * limits depending on the use: "Index", "Preview"...
     *
    ...
    /**
     * Gets the timeout (ms) on the InputStream after which an IOExecption is thrown to terminate
     * very slow transformations or a subprocess is terminated (killed).
     *
     * @return timeoutMs in milliseconds. If less than or equal to zero (the default) there is no
     *     timeout.
     */
    ...
    /**
     * Sets a timeout (ms) on the InputStream after which an IOExecption is thrown to terminate very
     * slow transformations or to terminate (kill) a subprocess.
     *
     * @param timeoutMs in milliseconds. If less than or equal to zero (the default) there is no
     *     timeout. If greater than zero the {@code readLimitTimeMs} must not be set.
     */
     ...
     * end of file.
     *
     * @param readLimitTimeMs if less than or equal to zero (the default) there is no limit. If
     *     greater than zero the {@code timeoutMs} must not be set.
     */
    ...
    /**
     * Gets the maximum source content size, to skip transformations where the source is just too
     * large to expect it to perform. If the source is larger the transformer indicates it is not
     * available.
     *
    ...
    /**
     * Sets a maximum source content size, to skip transformations where the source is just too
     * large to expect it to perform. If the source is larger the transformer indicates it is not
     * available.
     *
     * @param maxSourceSizeKBytes if less than or equal to zero (the default) there is no limit. If
     *     greater than zero the {@code readLimitKBytes} must not be set.
     */
    ...
    /**
     * Gets the limit in terms of the about of data read to limit transformations where only the
     * start of the content is needed. After this limit is reached the InputStream reports end of
     * file.
     *
     * @return readLimitKBytes if less than or equal to zero (the default) no limit should be
     *     applied.
     */
    ...
    /**
     * Sets a limit in terms of the about of data read to limit transformations where only the start
     * of the content is needed. After this limit is reached the InputStream reports end of file.
     *
     * @param readLimitKBytes if less than or equal to zero (the default) there is no limit. If
     *     greater than zero the {@code maxSourceSizeKBytes} must not be set.
     */
     ...
     *
     * @param pageLimit the number of pages to be read from the source. If less than or equal to
     *     zero (the default) no limit is applied.
     */
    ...
    /**
     * Gets the map of source options further describing how the source should be transformed based
     * on its mimetype
     *
    ...
    /**
     * Gets the immutable list of source options further describing how the source should be
     * transformed based on its mimetype. Use {@link
     * TransformationOptions#addSourceOptions(TransformationSourceOptions)} to add source options.
     *
     * @return the source options list
     */
    public Collection<TransformationSourceOptions> getSourceOptionsList() {
        if (sourceOptionsMap == null) return null;
        return sourceOptionsMap.values();
    }

    /**
     * Sets the list of source options further describing how the source should be transformed based
     * on its mimetype.
     *
     ...
     *
     * <p>Note that if source options of the same class already exists a new merged source options
     * object is added.
     *
     * @param sourceOptions TransformationSourceOptions
     */
    public void addSourceOptions(TransformationSourceOptions sourceOptions) {
        if (sourceOptionsMap == null) {
            sourceOptionsMap =
                    new HashMap<
                            Class<? extends TransformationSourceOptions>,
                            TransformationSourceOptions>(1);
        }
        TransformationSourceOptions newOptions = sourceOptions;
        TransformationSourceOptions existingOptions =
                sourceOptionsMap.get(sourceOptions.getClass());
     ...
     *
     * <p>Basic options (optional) are:
     *
     * <ul>
     *   <li>{@link #OPT_SOURCE_NODEREF}
     *   <li>{@link #OPT_SOURCE_CONTENT_PROPERTY}
     *   <li>{@link #OPT_TARGET_NODEREF}
     *   <li>{@link #OPT_TARGET_CONTENT_PROPERTY}
     *   <li>{@link #OPT_INCLUDE_EMBEDDED}
     *   <li>{@link #OPT_USE}
     *   <li>{@link TransformationOptionLimits#OPT_TIMEOUT_MS}
     *   <li>{@link TransformationOptionLimits#OPT_READ_LIMIT_TIME_MS}
     *   <li>{@link TransformationOptionLimits#OPT_MAX_SOURCE_SIZE_K_BYTES}
     *   <li>{@link TransformationOptionLimits#OPT_READ_LIMIT_K_BYTES}
     *   <li>{@link TransformationOptionLimits#OPT_MAX_PAGES}
     *   <li>{@link TransformationOptionLimits#OPT_PAGE_LIMIT}
     * </ul>
     *
     * <p>Override this method to append option values to the map. Derived classes should call the
     * base class before appending further values and returning the result.
     */
    ...
    public static TypeConverter.Converter<String, Boolean> relaxedBooleanTypeConverter =
            new TypeConverter.Converter<String, Boolean>() {
    ...
    public int hashCode() {
        ...
        result =
                prime * result
                        + ((this.includeEmbedded == null) ? 0 : this.includeEmbedded.hashCode());
        result = prime * result + ((this.limits == null) ? 0 : this.limits.hashCode());
        result =
                prime * result
                        + ((this.sourceContentProperty == null)
                                ? 0
                                : this.sourceContentProperty.hashCode());
        result =
                prime * result + ((this.sourceNodeRef == null) ? 0 : this.sourceNodeRef.hashCode());
        result =
                prime * result
                        + ((this.targetContentProperty == null)
                                ? 0
                                : this.targetContentProperty.hashCode());
        result =
                prime * result + ((this.targetNodeRef == null) ? 0 : this.targetNodeRef.hashCode());
        return result;
    }

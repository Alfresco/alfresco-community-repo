...
public class TransformerLogTest {
  ...
  static String[] stripDateStamp(String[] actual) {
    int beginIndex =
      (TransformerLogger.DATE_FORMAT.format(new Date()) + ' ').length();
  ...
  public void noEntriesDisabledTest() {
    assertArrayEquals(
      new String[] {
        "No entries are available. transformer.log.entries must be set to a number between 1 and 1000",
      },
      log.getEntries(10)
    );
  }

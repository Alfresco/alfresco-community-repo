...
public class AuthenticationServiceImpl
  ...
  private static final String AUTHENTICATION_UNSUCCESSFUL =
  ...
  public void setProtectedUsersCache(
    SimpleCache<String, ProtectedUser> protectedUsersCache
  ) {
  ...
  public boolean isActive() {
    return (
      !(this.authenticationComponent instanceof ActivateableBean) ||
  ...
  public void authenticate(String userName, char[] password)
    ...
    if (logger.isTraceEnabled()) {
      logger.trace(
        "Authenticated user: " + AuthenticationUtil.maskUsername(userName)
      );
    }
  }
    ...
    if (protectionEnabled) {
      ...
      if (protectedUser != null) {
        ...
        isProtected =
          ...
          currentTimeStamp -
          protectedUser.getTimeStamp() <
          protectionPeriodSeconds *
          1000;
      }
    }
    return isProtected;
  ...
  public void recordFailedAuthentication(String userName) {
    if (protectionEnabled) {
      ...
      if (protectedUser == null) {
        ...
        protectedUser =
          new ProtectedUser(userName, protectedUser.getNumLogins() + 1);
        if (
          protectedUser.getNumLogins() == protectionLimit &&
  ...
  public String getCurrentTicket() throws AuthenticationException {
    ...
    if (ticket == null) {
      ...
      if (logger.isTraceEnabled()) {
        logger.trace(
          "Ticket was null, but, if we got through the authentication check, then it's safe to issue a new ticket for user: " +
          AuthenticationUtil.maskUsername(userName)
        );
      }
      return getNewTicket();
  ...
  public String getNewTicket() {
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Exception in pre authentication check: " + e.getMessage(),
          e
        );
      }
      throw e;

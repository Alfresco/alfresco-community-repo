...
public class BlogPostsPost extends AbstractBlogWebScript {
  ...
  protected Map<String, Object> executeImpl(
    ...
    Map<String, Object> blogPostData = BlogPostLibJs.getBlogPostData(
      post.getNodeRef(),
      services
    );
    ...
    model.put(
      ...
      BlogPostLibJs.hasExternalBlogConfiguration(nodeRef, services)
    );

    boolean isDraft =
      blogPostData.get("isDraft") != null &&
      ((Boolean) blogPostData.get("isDraft")).booleanValue();
    if (
      jsonPostParams.getSite() != null &&
   ...
   * Creates a blog post
   */
  private BlogPostInfo createBlogPost(
    JsonParams jsonParams,
    SiteInfo site,
    NodeRef blogNode
  ) {
    String titleParam = jsonParams.getTitle() == null
      ? ""
      : jsonParams.getTitle();
    String contentParam = jsonParams.getContent() == null
      ? ""
      : jsonParams.getContent();

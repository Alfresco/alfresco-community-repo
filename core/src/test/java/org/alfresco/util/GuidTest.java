...
public class GuidTest extends TestCase {
    ...
    public void testGuid() {
            ...
            for (Thread current : threads) {
                ...
                if (state == State.BLOCKED) {
                    if (!blocked.contains(name)) {
                        blocked.add(name);
                        maxItemsBlocked =
                                blocked.size() > maxItemsBlocked ? blocked.size() : maxItemsBlocked;
                    }
        ...
        // worst case scenario : max number of threads blocked at a moment = number of threads - 2 (
        // usually ~5 for 30 threads)
        // the implementation without RandomSecure pool reaches constantly (number of threads - 1)
        // max blocked threads
        Assert.assertTrue(
                "Exceeded number of blocked threads : " + maxItemsBlocked, maxItemsBlocked < n - 2);
    }
}

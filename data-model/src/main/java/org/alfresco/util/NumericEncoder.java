 ...
 *
 * <p>To support range queries in the lucene index numeric types need to be indexed specially. This
 * has been addressed for int and long types for lucene and limited support (via scaling) for float
 * and double.
 *
 * <p>The implementation splits an int, long, float or double into the sign bit, optional exponent
 * and mantissa either from the int or long format or its IEEE 754 byte representation.
 *
 * <p>To index content so small negative numbers are indexed correctly and are after big negative
 * numbers in range queries.
 *
 * <p>The algorithm finds the sign, if the number is negative, then the mantissa and exponent are
 * XORed against the appropriate masks. This reverses the order. As negative numbers appear first in
 * the list their sign bit is 0 and positive numbers are 1.
 *
 * @author Andy Hind
 */
public class NumericEncoder {
    ...
    /**
     * Encode an integer into a string that orders correctly using string comparison
     * Integer.MIN_VALUE encodes as 00000000 and MAX_VALUE as ffffffff.
     *
    ...
    /**
     * Encode a long into a string that orders correctly using string comparison Long.MIN_VALUE
     * encodes as 0000000000000000 and MAX_VALUE as ffffffffffffffff.
     *
    ...
    /**
     * Encode a float into a string that orders correctly according to string comparison. Note that
     * there is no negative NaN but there are codings that imply this. So NaN and -Infinity may not
     * compare as expected.
     *
    ...
    /**
     * Encode a double into a string that orders correctly according to string comparison. Note that
     * there is no negative NaN but there are codings that imply this. So NaN and -Infinity may not
     * compare as expected.
     *

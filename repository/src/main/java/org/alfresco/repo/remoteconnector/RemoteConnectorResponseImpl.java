...
public class RemoteConnectorResponseImpl implements RemoteConnectorResponse {
    ...
    /**
     * Creates a new Response object with the data coming from a stream. Because of the HttpClient
     * lifecycle, a HttpClient response InputStream shouldn't be used as cleanup is needed
     */
    public RemoteConnectorResponseImpl(
    ...
    public RemoteConnectorResponseImpl(
        ...
        this(
                ...
                status,
                headers,
                new ByteArrayInputStream(ensureBytes(response)));
        this.bodyBytes = ensureBytes(response);
    }
    /**
     * HttpClient will return a null response body for things like 204 (No Content). We want to
     * treat that as an empty byte array, to meet our contracts
     */

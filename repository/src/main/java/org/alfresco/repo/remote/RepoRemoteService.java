...
public class RepoRemoteService implements RepoRemote {
    ...
    public NodeRef createDirectory(NodeRef base, String path) {
        ...
        FileInfo created =
                fFileFolderService.create(
                        parentChild.getFirst(), parentChild.getSecond(), ContentModel.TYPE_FOLDER);
    ...
    public OutputStream createFile(NodeRef base, String path) {
        ...
        FileInfo info =
                fFileFolderService.create(
                        parentChild.getFirst(), parentChild.getSecond(), ContentModel.TYPE_CONTENT);
        // TODO is update supposed to be true.
        ContentWriter writer =
                fContentService.getWriter(info.getNodeRef(), ContentModel.PROP_CONTENT, true);
    ...
    public Map<String, Pair<NodeRef, Boolean>> getListing(NodeRef dir) {
        ...
        for (FileInfo info : listing) {
            result.put(
                    info.getName(), new Pair<NodeRef, Boolean>(info.getNodeRef(), info.isFolder()));
        }
        return result;
    }
    ...
    public NodeRef getRoot() {
        NodeRef storeRoot =
                fNodeService.getRootNode(new StoreRef(StoreRef.PROTOCOL_WORKSPACE, "SpacesStore"));
    ...
    public InputStream readFile(NodeRef fileRef) {
        return fContentService
                .getReader(fileRef, ContentModel.PROP_CONTENT)
    ...
    public OutputStream writeFile(NodeRef base, String path) {
        ...
        return fContentService
                .getWriter(target, ContentModel.PROP_CONTENT, true)

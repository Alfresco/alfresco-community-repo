...
public class WebDAV {
  ...
  public static final String XML_NS_GET_CONTENT_LANGUAGE =
    DAV_NS_PREFIX + "getcontentlanguage";
   ...
   * @return Object
   */
  public static Object getDAVPropertyValue(
    ...
    if (propName == null) throw new AlfrescoRuntimeException(
      "No mapping for WebDAV property " + davPropName
    );
  ...
  public static String decodeURL(String strPath) {
    ...
    if (strNormalized.indexOf('\\') >= 0) strNormalized =
      strNormalized.replace('\\', '/');
    ...
    while (true) {
      ...
      if (index < 0) break;
      strNormalized =
        strNormalized.substring(0, index) + strNormalized.substring(index + 1);
    }
    ...
    while (true) {
      ...
      if (index < 0) break;
      strNormalized =
        strNormalized.substring(0, index) + strNormalized.substring(index + 2);
    }
    ...
    while (true) {
      ...
      strNormalized =
        strNormalized.substring(0, index2) + strNormalized.substring(index + 3);
    }
  ...
  public static final String[] parseLockToken(String lockToken) {
    ...
    if (lockToken.startsWith(WebDAV.OPAQUE_LOCK_TOKEN)) lockToken =
      lockToken.substring(WebDAV.OPAQUE_LOCK_TOKEN.length());

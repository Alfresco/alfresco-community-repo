 ...
 * content worker to provide. The XPath rules are simple, i.e. if an XML node is found by the XPath
 * statement, then it is considered to be a hit and the corresponding worker is returned.
 *
 * <p>Currently, the only namespaces supported are those contained in the XML documents being
 * tested.
 *
 * @since 2.1
 * @author Derek Hulley
...
public class XPathContentWorkerSelector<W extends ContentWorker> implements ContentWorkerSelector {
    ...
    public XPathContentWorkerSelector() {
        try {
            ...
            throw new AlfrescoRuntimeException(
                    "Failed to initialize XPathContentWorkerSelector", e);
        }
    ...
    /**
     * Set the workers to use. All the XPath statements provided must be compatible with a return
     * value of type {@linkplain XPathConstants#NODE NODE}.
     *
     * @param workers a map of {@linkplain ContentWorker} instances keyed by XPath statements
     */
    ...
    /**
     * Execute the XPath statements, in order, against the document. Any statements that fail to run
     * will be ignored.
     */

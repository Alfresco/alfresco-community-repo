...
public class TemporarySites extends AbstractPersonRule {
    ...
    protected void after() {
        ...
        final SiteService siteService =
                appContextRule.getApplicationContext().getBean("siteService", SiteService.class);
        final ActivityPostDAO postDAO =
                appContextRule.getApplicationContext().getBean("postDAO", ActivityPostDAO.class);
        final NodeArchiveService nodeArchiveService =
                (NodeArchiveService)
                        appContextRule.getApplicationContext().getBean("nodeArchiveService");
                        ...
                        transactionHelper.doInTransaction(
                                new RetryingTransactionCallback<Void>() {
                                    ...
                                    public Void execute() throws Throwable {
                                        ...
                                        for (ActivityPostEntity.STATUS status :
                        ...
                        transactionHelper.doInTransaction(
                                new RetryingTransactionCallback<Void>() {
                                    ...
                                    public Void execute() throws Throwable {
                                        for (SiteInfo site : temporarySites) {
                                            log.debug(
                                                    "Purging temporary site from trashcan: "
                                                            + site.getShortName());
                                            nodeArchiveService.purgeArchivedNode(
                                                    nodeArchiveService.getArchivedNode(
                                                            site.getNodeRef()));
                                        }
    ...
    /**
     * This method creates a Share Site and adds it to the internal list of NodeRefs to be tidied up
     * by the rule. This method will be run in its own transaction and will be run with the
     * specified user as the fully authenticated user, thus ensuring the named user is the creator
     * of the new site.
     *
    ...
    /**
     * This method creates a Share Site (<b>or subtype</b>) and adds it to the internal list of
     * NodeRefs to be tidied up by the rule. This method will be run in its own transaction and will
     * be run with the specified user as the fully authenticated user, thus ensuring the named user
     * is the creator of the new site.
     *
     * @param sitePreset the site preset
     * @param siteShortName the short name of the new site
     * @param siteTitle the title of the new site
    ...
    public SiteInfo createSite(
        ...
        SiteInfo newSite =
                transactionHelper.doInTransaction(
                        new RetryingTransactionCallback<SiteInfo>() {
                            public SiteInfo execute() throws Throwable {
                                final SiteService siteService =
                                        appContextRule
                                                .getApplicationContext()
                                                .getBean("siteService", SiteService.class);
                                ...
                                // ensure that the Document Library folder is pre-created so that
                                // test code can start creating content straight away.
                                // At the time of writing HEAD does not create this folder
                                // automatically, but Thor does.
                                // So to be safe, I'll pre-check if the node is there.
                                NodeRef docLibFolder =
                                        siteService.getContainer(
                                                siteShortName, SiteService.DOCUMENT_LIBRARY);
    ...
    /**
     * This method creates a test site (of Alfresco type <code>st:site</code>) and one user for each
     * of the Share Site Roles. This method will be run in its own transaction and will be run with
     * the specified user as the fully authenticated user, thus ensuring the named user is the
     * creator of the new site. The site and its users will be deleted automatically by the rule.
     *
     * @param sitePreset the site preset.
     * @param visibility the Site visibility.
     * @param siteCreator the username of a user who will be used to create the site (user must
     *     exist of course).
     * @return the {@link SiteInfo} object for the newly created site.
     */
    public TestSiteAndMemberInfo createTestSiteWithUserPerRole(
            final String siteShortName,
            String sitePreset,
                            ...
                            public List<String> execute() throws Throwable {
                                ...
                                for (String shareRole : SiteModel.STANDARD_PERMISSIONS) {
                                    final String userName =
                                            siteShortName + "_" + shareRole + "_" + GUID.generate();
        ...
        return new TestSiteAndMemberInfo(
                result,
                doclibFolder,
                userNames.get(0),
                userNames.get(1),
     ...
     */
    public List<String> createTestSiteWithGroups(
            final String siteShortName,
            String sitePreset,
            SiteVisibility visibility,
            String siteCreator,
                        ...
                        new RetryingTransactionCallback<List<String>>() {
                            public List<String> execute() throws Throwable {
                                ...
                                for (int i = 0; i < noOfGroups; i++) {
                                    ...
                                    properties.put(
                                            ContentModel.PROP_EMAIL, GUID.generate() + "@test.com");
    ...
    /**
     * A simple POJO class to store the {@link SiteInfo} for this site and its initial,
     * automatically created members' usernames.
     *
     * @author Neil Mc Erlean
     */
    public static class TestSiteAndMemberInfo {
        ...
        public TestSiteAndMemberInfo(
                SiteInfo siteInfo,
                NodeRef siteDocLib,
                String siteManager,
                String siteCollaborator,
                String siteContributor,
                String siteConsumer) {

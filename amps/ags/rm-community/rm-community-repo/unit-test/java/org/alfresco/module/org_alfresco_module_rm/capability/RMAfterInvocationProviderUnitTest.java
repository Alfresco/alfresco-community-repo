...
public class RMAfterInvocationProviderUnitTest {
  ...
  public void testDecide_resultSet_unlimited() {
    ...
    FilteringResultSet filteringResultSet = (FilteringResultSet) rmAfterInvocationProvider.decide(
      authentication,
      object,
      config,
      returnedObject
    ...
    assertEquals(
      "Expected total of one result.",
      1,
      filteringResultSet.getNumberFound()
    );
    assertEquals(
      "Expected one result returned.",
      1,
      filteringResultSet.length()
    );
    assertEquals(

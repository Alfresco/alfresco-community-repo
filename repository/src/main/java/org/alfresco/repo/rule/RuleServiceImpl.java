...
public class RuleServiceImpl
  ...
  private static final String KEY_DISABLED_RULE_TYPES =
    "RuleServiceImpl.disabledRuleTypes";

  /** key against which to store disabled rule nodes in the current txn */
  private static final String KEY_DISABLED_RULE_NODES =
    "RuleServiceImpl.disabledRuleNodes";

  /** key against which to store rules pending on the current transaction */
  private static final String KEY_RULES_PENDING =
    "RuleServiceImpl.PendingRules";

  /** key against which to store executed rules on the current transaction */
  private static final String KEY_RULES_EXECUTED =
    "RuleServiceImpl.ExecutedRules";

  /** qname of assoc to rules */
  private String ASSOC_NAME_RULES_PREFIX = "rules";
  private RegexQNamePattern ASSOC_NAME_RULES_REGEX = new RegexQNamePattern(
    RuleModel.RULE_MODEL_URI,
    "^" + ASSOC_NAME_RULES_PREFIX + ".*"
  );

  private static final Set<QName> IGNORE_PARENT_ASSOC_TYPES = new HashSet<QName>(
    7
  );
   ...
   */
  private TransactionListener ruleTransactionListener = new RuleTransactionListener(
    this
  );
   ...
   * @see NullCache
   */
  public void setNodeRulesCache(
    SimpleCache<NodeRef, List<Rule>> nodeRulesCache
  ) {
  ...
  public ChildAssociationRef getSavedRuleFolderAssoc(NodeRef nodeRef) {
    ...
    List<ChildAssociationRef> assocs =
      this.runtimeNodeService.getChildAssocs(
          nodeRef,
          RuleModel.ASSOC_RULE_FOLDER,
          RuleModel.ASSOC_RULE_FOLDER
        );
    if (assocs.size() > 1) {
      throw new ActionServiceException(
        "There is more than one rule folder, which is invalid."
      );
  ...
  public boolean isEnabled() {
    return (
      this.globalRulesDisabled == false && this.rulesDisabled.get() == null
    );
  }

  @Override
  public boolean rulesEnabled(NodeRef nodeRef) {
    Set<NodeRef> disabledRuleNodes = TransactionalResourceHelper.getSet(
      KEY_DISABLED_RULE_NODES
    );
  ...
  public void disableRuleType(String ruleType) {
    Set<String> disabledRuleTypes = TransactionalResourceHelper.getSet(
      KEY_DISABLED_RULE_TYPES
    );
  ...
  public List<Rule> getRules(
    ...
    return AuthenticationUtil.runAs(
      new AuthenticationUtil.RunAsWork<List<Rule>>() {
        public List<Rule> doWork() throws Exception {
          ...
          for (Rule rule : nodeRules) {
            if (
              (rules.contains(rule) == false) &&
              (
                ruleTypeName == null ||
                rule.getRuleTypes().contains(ruleTypeName) == true
              )
            ) {
              rules.add(rule);
            }
  ...
  private List<Rule> getRulesForNode(NodeRef nodeRef) {
    ...
    if (
      !runtimeNodeService.hasAspect(nodeRef, RuleModel.ASPECT_RULES) ||
  ...
  public int countRules(NodeRef nodeRef) {
    ...
    if (
      this.runtimeNodeService.exists(nodeRef) == true &&
      checkNodeType(nodeRef) == true
    ) {
      if (
        this.runtimeNodeService.hasAspect(nodeRef, RuleModel.ASPECT_RULES) ==
        true
      ) {
        NodeRef ruleFolder = getSavedRuleFolderRef(nodeRef);
        if (ruleFolder != null) {
    ...
    if (
      ...
      if (logger.isDebugEnabled() == true) {
        logger.debug(
          "A node of type " +
          nodeType.toString() +
          " was checked and can not have rules."
        );
      }
    }
   ...
   */
  private List<Rule> getInheritedRules(
    NodeRef nodeRef,
    String ruleTypeName,
    ...
    if (
      this.runtimeNodeService.hasAspect(
          nodeRef,
          RuleModel.ASPECT_IGNORE_INHERITED_RULES
        ) ==
      false
      ...
      if (visitedNodeRefs.contains(nodeRef) == false) {
        ...
        List<ChildAssociationRef> parents =
          this.runtimeNodeService.getParentAssocs(nodeRef);
        for (ChildAssociationRef parent : parents) {
          ...
          for (Rule rule : getInheritedRules(
            parent.getParentRef(),
            ruleTypeName,
            visitedNodeRefs
          )) {
          ...
          for (Rule rule : rules) {
            ...
            if (
              rule.isAppliedToChildren() == true &&
              allInheritedRules.contains(rule) == false
            ) {
              allInheritedRules.add(rule);
            }
          }
  ...
  public Rule getRule(NodeRef ruleNodeRef) {
    ...
    Map<QName, Serializable> props =
      this.runtimeNodeService.getProperties(ruleNodeRef);
    ...
    String description = DefaultTypeConverter.INSTANCE.convert(
      String.class,
      props.get(ContentModel.PROP_DESCRIPTION)
    );
    rule.setTitle(title);
    ...
    if (actions.size() == 0) {
      ...
      throw new RuleServiceException(
        "Rule exists with more than one specified action"
      );
    }
  ...
  public void saveRule(NodeRef nodeRef, Rule rule) {
    ...
    if (
      ...
      try {
        ...
        if (ruleNodeRef == null) {
          if (
            this.nodeService.hasAspect(nodeRef, RuleModel.ASPECT_RULES) == false
          ) {
          ...
          ruleNodeRef =
            this.nodeService.createNode(
                ...
                QName.createQName(
                  RuleModel.RULE_MODEL_URI,
                  ASSOC_NAME_RULES_PREFIX + GUID.generate()
                ),
                RuleModel.TYPE_RULE
              )
              .getChildRef();
        ...
        this.nodeService.setProperty(
            ruleNodeRef,
            RuleModel.PROP_RULE_TYPE,
            (Serializable) rule.getRuleTypes()
          );
        this.nodeService.setProperty(
            ruleNodeRef,
            RuleModel.PROP_APPLY_TO_CHILDREN,
            rule.isAppliedToChildren()
          );
        this.nodeService.setProperty(
            ruleNodeRef,
            RuleModel.PROP_EXECUTE_ASYNC,
            rule.getExecuteAsynchronously()
          );
        this.nodeService.setProperty(
            ruleNodeRef,
            RuleModel.PROP_DISABLED,
            rule.getRuleDisabled()
          );
      ...
      throw new RuleServiceException(
        "Insufficient permissions to save a rule."
      );
    }
  }
  ...
  public void setRulePosition(NodeRef nodeRef, NodeRef ruleNodeRef, int index) {
    ...
    if (ruleFolder != null) {
      ...
      List<ChildAssociationRef> orderedAssocs = new ArrayList<ChildAssociationRef>(
        assocs.size()
      );
  ...
  private void saveAction(NodeRef ruleNodeRef, Rule rule) {
    ...
    if (action == null) {
      throw new RuleServiceException(
        "An action must be specified when defining a rule."
      );
    }
    ...
    if (actions.size() == 1) {
      ...
      throw new RuleServiceException(
        "The rule has become corrupt.  More than one action is associated with the rule."
      );
    }
    ...
    if (actionNodeRef == null) {
      actionNodeRef =
        this.runtimeActionService.createActionNodeRef(
            action,
            ruleNodeRef,
            RuleModel.ASSOC_ACTION,
            RuleModel.ASSOC_ACTION
  ...
  public void removeRule(NodeRef nodeRef, Rule rule) {
    ...
    if (
        ...
        if (countRules(nodeRef) == 0) {
          ...
          if (linkedFrom.isEmpty() == false) {
            for (NodeRef linkedFromNodeRef : linkedFrom) {
              NodeRef ruleFolder = getSavedRuleFolderAssoc(nodeRef)
                .getChildRef();
      ...
      throw new RuleServiceException(
        "Insufficient permissions to remove a rule."
      );
    }
  }
  ...
  private void checkForLinkedRules(NodeRef nodeRef) {
    if (isLinkedToRuleNode(nodeRef) == true) {
      throw new RuleServiceException(
        "Can not edit rules as they are linked to another rule set."
      );
    }
  }
  ...
  public void removeAllRules(NodeRef nodeRef) {
    ...
    if (
      ...
      if (
        ...
        if (folder != null) {
          List<ChildAssociationRef> ruleChildAssocs =
            this.nodeService.getChildAssocs(
                folder,
                RegexQNamePattern.MATCH_ALL,
                ASSOC_NAME_RULES_REGEX
  ...
  public void removeRulePendingExecution(NodeRef actionedUponNodeRef) {
    ...
    List<PendingRuleData> pendingRules = (List<PendingRuleData>) AlfrescoTransactionSupport.getResource(
      KEY_RULES_PENDING
    );
  ...
  public void addRulePendingExecution(
    ...
    if (
      ...
      PendingRuleData pendingRuleData = new PendingRuleData(
        actionableNodeRef,
        actionedUponNodeRef,
        rule,
        executeAtEnd
      ...
      if (pendingRules == null) {
        ...
        if (logger.isDebugEnabled() == true) {
          logger.debug(
            "Rule '" +
            rule.getTitle() +
            "' has been added pending execution to action upon node '" +
      ...
      if (pendingRules.contains(pendingRuleData) == false) {
        if (
          ...
          (rule.getAction() instanceof ActionImpl)
        ) {
          ((ActionImpl) rule.getAction()).setRunAsUser(
      ...
      if (logger.isDebugEnabled() == true) {
        logger.debug(
          ...
          rule.getTitle() +
          "' or the node '" +
          this.getOwningNodeRef(rule).getId() +
          "' has been disabled."
        );
      }
    }
  ...
  private void executePendingRulesImpl(
    ...
    if (pendingRules != null && !pendingRules.isEmpty()) {
      PendingRuleData[] pendingRulesArr = pendingRules.toArray(
        new PendingRuleData[0]
      );
  ...
  private void executePendingRule(final PendingRuleData pendingRule) {
    if (
      AuthenticationUtil.getRunAsAuthentication() == null &&
      pendingRule.getRunAsUser() != null
    ) {
  ...
  private void executePendingRuleImpl(PendingRuleData pendingRule) {
    Set<ExecutedRuleData> executedRules = (Set<ExecutedRuleData>) AlfrescoTransactionSupport.getResource(
      KEY_RULES_EXECUTED
    );
    ...
    if (
      ...
      isSystemUser =
        AuthenticationUtil.SYSTEM_USER_NAME.equals(
          ((ActionImpl) rule.getAction()).getRunAsUser()
        );
    }

    NodeRef ruleNodeRef = rule.getNodeRef();
    if (
      !ruleNodeRef.getStoreRef().equals(actionedUponNodeRef.getStoreRef()) &&
      !nodeService.exists(ruleNodeRef)
    ) {
      NodeRef newRuleNodeRef = new NodeRef(
    ...
    rule =
      AuthenticationUtil.runAs(
        new AuthenticationUtil.RunAsWork<Rule>() {
    ...
    if (
      executedRules == null ||
      canExecuteRule(executedRules, actionedUponNodeRef, rule) == true
    ) {
      if (isSystemUser) {
  ...
  public void executeRule(
    ...
    if (action == null) {
      throw new RuleServiceException(
        "Attempting to execute a rule that does not have a rule specified."
      );
    }

    // Evaluate the condition
    if (
      this.actionService.evaluateAction(action, actionedUponNodeRef) == true
    ) {
      ...
      if (
        action.getActionDefinitionName().equals(CompositeActionExecuter.NAME)
      ) {
        for (Action subAction : ((CompositeAction) action).getActions()) {
          if (
            subAction.getActionDefinitionName().equals(MailActionExecuter.NAME)
          ) {
            subAction.setParameterValue(
        ...
        action.getActionDefinitionName().equals(MailActionExecuter.NAME)
      ) {
   ...
   */
  private boolean canExecuteRule(
    Set<ExecutedRuleData> executedRules,
    NodeRef actionedUponNodeRef,
    Rule rule
  ) {
    ...
    if (logger.isDebugEnabled() == true) {
      logger.debug(
        " >> Current executed items count = " + executedRules.size()
      );
    }

    if (executedRules != null) {
      if (
        executedRules.contains(
          new ExecutedRuleData(actionedUponNodeRef, rule)
        ) ==
        true
      ) {
        if (logger.isDebugEnabled() == true) {
          logger.debug(
            " >> Already executed this rule (" +
            rule.getTitle() +
            ") on this nodeRef (" +
            actionedUponNodeRef.getId() +
            ")"
  ...
  private boolean checkForCopy(
    ...
    if (
      ...
      if (copiedFrom != null) {
        if (
          executedRules.contains(new ExecutedRuleData(copiedFrom, rule)) == true
        ) {
          if (logger.isDebugEnabled() == true) {
            logger.debug(
              " >> Already executed this rule (" +
              rule.getTitle() +
              ") on this the copied from nodeRef (" +
              copiedFrom.getId() +
              ")"
            );
          }
  ...
  public class ExecutedRuleData {
    ...
    public boolean equals(Object obj) {
      ...
      if (obj instanceof ExecutedRuleData) {
        ...
        return (
          this.actionableNodeRef.equals(that.actionableNodeRef) &&
  ...
  private NodeRef getOwningNodeRefActionImpl(NodeRef actionNodeRef) {
    ...
    NodeRef parentNodeRef =
      this.nodeService.getPrimaryParent(actionNodeRef).getParentRef();
  ...
  public NodeRef getLinkedToRuleNode(NodeRef nodeRef) {
    ...
    if (nodeService.hasAspect(nodeRef, RuleModel.ASPECT_RULES) == true) {
      ...
      if (assoc.isPrimary() == false) {
        result =
          nodeService.getPrimaryParent(assoc.getChildRef()).getParentRef();
      }
    }

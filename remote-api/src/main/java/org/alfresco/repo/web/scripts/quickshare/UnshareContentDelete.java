 ...
 *
 * <p>DELETE web script to "Unshare" access to some content (ie. disable unauthenticated access to
 * this node)
 *
 * <p>Note: authenticated web script
 *
 * @author janv
 * @since Cloud/4.2
 */
    ...
    protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
        if (!isEnabled()) {
            throw new WebScriptException(
                    HttpServletResponse.SC_FORBIDDEN, "QuickShare is disabled system-wide");
        }
        ...
        if (sharedId == null) {
            throw new WebScriptException(
                    HttpServletResponse.SC_BAD_REQUEST, "A valid sharedId must be specified !");
        }
        ...
        try {
            ...
            String sharedBy =
                    (String) nodeService.getProperty(nodeRef, QuickShareModel.PROP_QSHARE_SHAREDBY);
            if (!quickShareService.canDeleteSharedLink(nodeRef, sharedBy)) {
                throw new WebScriptException(
                        HttpServletResponse.SC_FORBIDDEN,
                        "Can't perform unshare action: " + sharedId);
            }
            ...
            throw new WebScriptException(
                    HttpServletResponse.SC_NOT_FOUND, "Unable to find: " + sharedId);

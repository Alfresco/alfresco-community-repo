...
public class ActivitiPooledActorsPropertyHandler
  ...
  private void updateTaskCandidates(
    String taskId,
    UserAndGroupUpdates updates
  ) {
  ...
  private UserAndGroupUpdates getUserAndGroupUpdates(
    Serializable value,
    Collection<? extends IdentityLink> links
  ) {
    ...
    for (NodeRef actor : actors) {
      ...
      List<String> pooledAuthorities = authorityManager.isUser(authorityName)
        ? users
        : groups;
   ...
   */
  public void setWorkflowAuthorityManager(
  ...
  private static class UserAndGroupUpdates {
    ...
    public UserAndGroupUpdates(
      List<String> users,
      List<String> groups,
      List<IdentityLink> linksToRemove
    ) {
      this.users = users;
      this.groups = groups;

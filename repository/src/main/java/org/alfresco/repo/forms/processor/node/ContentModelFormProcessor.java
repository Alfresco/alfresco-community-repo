...
/**
 * Abstract FormProcessor implementation that provides common functionality for form processors that
 * deal with Alfresco content models i.e. types and nodes.
 *
...
public abstract class ContentModelFormProcessor<ItemType, PersistType>
    ...
    /**
     * A regular expression which can be used to match property names. These names will look like
     * <code>"prop_cm_name"</code>. The pattern can also be used to extract the "cm" and the "name"
     * parts.
     */
    protected Pattern propertyNamePattern =
            Pattern.compile(PROP_DATA_PREFIX + "([a-zA-Z0-9-]+)_(.*)");

    /**
     * A regular expression which can be used to match tranisent property names. These names will
     * look like <code>"prop_name"</code>. The pattern can also be used to extract the "name" part.
     */
    protected Pattern transientPropertyPattern = Pattern.compile(PROP_DATA_PREFIX + "(.*){1}?");

    /**
     * A regular expression which can be used to match association names. These names will look like
     * <code>"assoc_cm_references_added"</code>. The pattern can also be used to extract the "cm",
     * the "name" and the suffix parts.
     */
    protected Pattern associationNamePattern =
            Pattern.compile(ASSOC_DATA_PREFIX + "([a-zA-Z0-9-]+)_(.*)(_[a-zA-Z]+)");
    ...
    protected void addPropertyDataIfRequired(
            QName propName, Form form, ContentModelItemData<?> itemData) {
    ...
    protected List<Field> generateDefaultFields(
        ...
        DefaultFieldBuilder defaultFieldBuilder =
                new DefaultFieldBuilder(
                        data, fieldProcessorRegistry, namespaceService, fieldsToIgnore);
    ...
    protected ContentModelItemData<ItemType> makeItemData(ItemType item) {
        ...
        return new ContentModelItemData<ItemType>(
                item, propDefs, assocDefs, propValues, assocValues, transientValues);
    }
    ...
    protected void persistNode(NodeRef nodeRef, FormData data) {
        ...
        TypeDefinition typeDef =
                this.dictionaryService.getAnonymousType(type, this.nodeService.getAspects(nodeRef));
        ...
        Map<QName, Serializable> propsToPersist =
                new HashMap<QName, Serializable>(data.getNumberOfFields());
            ...
            if (fieldData.isFile() == false) {
                ...
                if (fieldName.startsWith(PROP_DATA_PREFIX)) {
                    ...
                    processAssociationPersist(
                            nodeRef, assocDefs, childAssocDefs, fieldData, assocsToPersist);
     ...
     * @param data The FormData to persist
     */
    protected void processPropertyPersist(
            NodeRef nodeRef,
        ...
        Matcher m =
                this.propertyNamePattern.matcher(
                        fieldData.getName().replaceAll(DOT_CHARACTER_REPLACEMENT, DOT_CHARACTER));
        if (m.matches()) {
    ...
    protected void processAssociationPersist(
        ...
        if (m.matches()) {
            ...
            // If the association is not defined on this node i.e. it is not defined for this node's
            // content
            // type and is not defined for any of the aspects that have already been applied to this
            // node,
            // then assocDef here will be null. This is because assocDef is passed in to this
            // method, its value
            // having been obtained from dictionaryService.getAnonymousType()
            //
            // However if the association type is defined on an aspect which has not yet been
            // applied to this node
            // then setting a value for this association should lead to the automatic application of
            // the aspect defining it.
            if (assocDef == null) {
                // Is it defined on any other type?
                AssociationDefinition assocDefFromDictionary =
                        this.dictionaryService.getAssociation(fullQName);

                // If the association is defined on any *aspect* type...
                if (assocDefFromDictionary != null
                        && assocDefFromDictionary.getSourceClass().isAspect()) {
                    // ... then it should be safe to proceed with applying the association value.
                    assocDef = assocDefFromDictionary;
                } else {
                    // ... else it is either undefined in the dictionary service or is defined on a
                    // concrete
                    // content type which is inconsistent with the node's type. We'll ignore it,
                    // which is what
                    ...
                    if (getLogger().isWarnEnabled()) {
                        getLogger()
                                .warn(
                                        "Ignoring field '"
                                                + fieldName
                                                + "' as a valid association definition can not be"
                                                + " found");
                    }
                ...
                if (nextTargetNode.length() > 0) {
                    if (NodeRef.isNodeRef(nextTargetNode)) {
                        if (assocSuffix.equals(ASSOC_DATA_ADDED_SUFFIX)) {
                            if (assocDef.isChild()) {
                                assocCommands.add(
                                        new AddChildAssocCommand(
                                                nodeRef, new NodeRef(nextTargetNode), fullQName));
                            } else {
                                assocCommands.add(
                                        new AddAssocCommand(
                                                nodeRef, new NodeRef(nextTargetNode), fullQName));
                            ...
                            if (assocDef.isChild()) {
                                assocCommands.add(
                                        new RemoveChildAssocCommand(
                                                nodeRef, new NodeRef(nextTargetNode), fullQName));
                            } else {
                                assocCommands.add(
                                        new RemoveAssocCommand(
                                                nodeRef, new NodeRef(nextTargetNode), fullQName));
                            }
                        } else {
                            if (getLogger().isWarnEnabled()) {
                                ...
                                msg.append("Ignoring 'fieldName ")
                                        .append(fieldName)
                                        .append(
                                                "' as it does not have one of the expected suffixes"
                                                        + " (")
    ...
    protected void processNamePropertyPersist(
            NodeRef nodeRef, FieldData fieldData, Map<QName, Serializable> propsToPersist) {
        ...
        if (fileInfo != null) {
            try {
                ...
                // ALF-6739: Notification should be more user friendly on editing with duplicated
                // name.
                ...
                // throw new FormException("Failed to persist field '" + fieldData.getName() + "'",
                // fee);
                throw fee;
            } catch (FileNotFoundException fnne) {
                throw new FormException(
                        "Failed to persist field '" + fieldData.getName() + "'", fnne);
            }
        } else {
    ...
    protected void processMimetypePropertyPersist(
        ...
        if (contentData == null) {
            ...
            contentData =
                    (ContentData) this.nodeService.getProperty(nodeRef, ContentModel.PROP_CONTENT);
        }
     ...
     */
    protected void processContentPropertyPersist(
            NodeRef nodeRef,
            FieldData fieldData,
            Map<QName, Serializable> propsToPersist,
            FormData data) {
        ...
        if (writer != null) {
            ...
            boolean defaultMimetypeRequired =
            ...
            if (defaultMimetypeRequired) {
                    ...
                    if (mimetype == null) {
                        contentData =
                                ContentData.setMimetype(
                                        contentData, determineDefaultMimetype(data));
                    }
                } else {
                    // content data has not been persisted yet so get it from the node
                    contentData =
                ...
                if (contentData != null) {
                    // if the ContentData object already exists in propsToPersist extract the
                    // mimetype
                    // and encoding and set on the ContentData object just retrieved
                    if (propsToPersist.containsKey(ContentModel.PROP_CONTENT)) {
                        ContentData mimetypeEncoding =
                                (ContentData) propsToPersist.get(ContentModel.PROP_CONTENT);
                        contentData =
                                ContentData.setMimetype(
                                        contentData, mimetypeEncoding.getMimetype());
                        contentData =
                                ContentData.setEncoding(
                                        contentData, mimetypeEncoding.getEncoding());
                    }
                }
            }
    ...
    /**
     * Looks through the form data for the 'mimetype' transient field and returns it's value if
     * found, otherwise the default 'text/plain' is returned
     *
    ...
    protected String determineDefaultMimetype(FormData data) {
        ...
        if (data != null) {
            FieldData mimetypeField =
                    data.getFieldData(PROP_DATA_PREFIX + MimetypeFieldProcessor.KEY);
...
abstract class AbstractAssocCommand {
    ...
    /**
     * This method should use the specified nodeService reference to effect the update to the
     * supplied associations.
     *
...
class AddAssocCommand extends AbstractAssocCommand {
    ...
    protected void updateAssociations(NodeService nodeService) {
        List<AssociationRef> existingAssocs =
                nodeService.getTargetAssocs(sourceNodeRef, assocQName);

...
/**
 * This class provides helper functions for when working with Timezones for Calendar events. It
 * provides support for generating iCal timezone information blocks, and building Java TimeZones
 * based on iCal timezone information.
 *
 * @author Nick Burch
 * @since 4.0
 */
public class CalendarTimezoneHelper {
    ...
    /**
     * Internal version that takes the parameters from {@link #getICalParams(String)} and builds a
     * TimeZone from it. This is not public as it will be refactored when {@link
     * #getICalParams(String)} is replaced. Note - because it uses the icalParams, we can't handle
     * cases where we're given historic TZ info (eg until 2004 it was that, now it's this)
     */
    ...
    /**
     * Turn an iCal repeating rule like "FREQ=YEARLY;BYDAY=-1SU;BYMONTH=10" into a SimpleTimeZone
     * rule like Month=March, StartDay=0, StartDayOfWeek=-Sunday See the JavaDocs of {@link
     * SimpleTimeZone} for how to express the different requirements in the required int formats
     */
        ...
        if (quote == -1) {
            ...
            if (colon < quote) {
                ...
                if (nextQuote == -1) {
                    ...
                    return nextQuote
                            + indexOfFirstUnquotedColon(
                                    icalLine.substring(nextQuote + 1, icalLine.length()))
                            + 1;
                }
            }
        }
    ...
    /**
     * Turns an iCal event into event + timezone parameters. This is very closely tied to the SPP /
     * VTI implementation, and should be replaced with something more general. Until then, it is
    ...
    protected static Map<String, String> getICalParams(String icalText) {
        ...
        for (String line : segregatedLines) {
            ...
            if (keyValue.length >= 2) {
                    ...
                    if (keyValue[0].contains(";")) {
                        ...
                        if (splitAt < keyValue[0].length() - 2) {
                            ...
                            for (String extra : extras) {
                                ...
                                if (splitAt > -1
                                        && !result.containsKey(
                                                mainKey + "-" + extra.substring(0, splitAt - 1))) {
                                    result.put(
                                            mainKey + "-" + extra.substring(0, splitAt - 1),
                ...
                if (!stack.isEmpty()
                        && stack.peek().equals(ICAL_SECTION_TIMEZONE)
                ...
                if (stack.size() >= 2
                        && stack.get(stack.size() - 2).equals(ICAL_SECTION_TIMEZONE)

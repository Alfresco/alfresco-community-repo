...
public abstract class AbstractAuthenticationService
  ...
  public static final String GUEST_AUTHENTICATION_NOT_SUPPORTED =
    "Guest authentication not supported";
    ...
    if (sysAdminParams != null) {
      ...
      if ((allowedUsers != null) && (!allowedUsers.contains(userName))) {
        throw new AuthenticationDisallowedException(
          "Username not allowed: " + AuthenticationUtil.maskUsername(userName)
        );
      }
      ...
      if (
        (maxUsers != null) &&
        (maxUsers > -1) &&
        (getUsersWithTickets(true).size() >= maxUsers)
      ) {
        throw new AuthenticationMaxUsersException(

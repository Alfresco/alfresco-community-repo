...
public class ActivitiPooledActorsPropertyHandler extends ActivitiTaskPropertyHandler {
    ...
    /** {@inheritDoc} */
    @Override
    protected Object handleTaskProperty(
            Task task, TypeDefinition type, QName key, Serializable value) {
        ...
        for (IdentityLink link : updates.getLinksToRemove()) {
            if (link.getUserId() != null) {
                taskService.deleteUserIdentityLink(
                        link.getTaskId(), link.getUserId(), link.getType());
            } else {
                taskService.deleteGroupIdentityLink(
                        link.getTaskId(), link.getGroupId(), link.getType());
            }
        }
    }
     ...
     */
    private UserAndGroupUpdates getUserAndGroupUpdates(
            Serializable value, Collection<? extends IdentityLink> links) {
        ...
        for (NodeRef actor : actors) {
            ...
            List<String> pooledAuthorities =
                    authorityManager.isUser(authorityName) ? users : groups;
    ...
    private static class UserAndGroupUpdates {
        ...
        public UserAndGroupUpdates(
                List<String> users, List<String> groups, List<IdentityLink> linksToRemove) {
            this.users = users;
            this.groups = groups;

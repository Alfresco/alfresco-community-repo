 ...
 * that the request came in through a "secure" tomcat connector. (but it will not validate the
 * certificate on the request; this done in a different filter).
 *
 * @since 4.0
 */
    ...
    public void afterPropertiesSet() throws Exception {
        if (secureComms == SecureCommsType.SECRET) {
            if (sharedSecret == null || sharedSecret.length() == 0) {
                logger.fatal(
                        "Missing value for solr.sharedSecret configuration property. If"
                            + " solr.secureComms is set to \"secret\", a value for"
                            + " solr.sharedSecret is required. See"

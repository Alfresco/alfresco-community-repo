...
public class DestroyContentTest extends BaseRMTestCase {
    ...
    protected void initServices() {
        ...
        contentCleanser =
                (TestContentCleanser) applicationContext.getBean(BEAN_NAME_CONTENT_CLEANSER);
        eagerContentStoreCleaner =
                (EagerContentStoreCleaner) applicationContext.getBean("eagerContentStoreCleaner");
        contentDestructionComponent =
                (ContentDestructionComponent)
                        applicationContext.getBean("contentDestructionComponent");
    ...
    /**
     * Given that a record folder is eligible for destruction And record ghosting is applied When
     * the record folder is destroyed Then the record folder and records are ghosted And the content
     * is destroyed
     */
        ...
        doBehaviourDrivenTest(
                new BehaviourDrivenTest() {
                    ...
                    public void given() throws Exception {
                        ...
                        InputStream is =
                                DestroyContentTest.class.getResourceAsStream(
                                        "/alfresco/test/content/Image.jpg");
                        ...
                        // Commented out, because Bamboo doesn't currently support rendition
                        // creation
                        ...
                        */
                        ...
                        utils.completeEvent(
                                destroyableFolder, CommonRMTestUtils.DEFAULT_EVENT_NAME);
                        rmActionService.executeRecordsManagementAction(
                                destroyableFolder, CutOffAction.NAME);

                        // assert things are as we expect
                        assertEquals(
                                DestroyAction.NAME,
                                dispositionService
                                        .getNextDispositionAction(destroyableFolder)
                                        .getName());
                        assertTrue(
                                dispositionService.isNextDispositionActionEligible(
                                        destroyableFolder));
                    ...
                    public void when() throws Exception {
                        // destroy the folder
                        rmActionService.executeRecordsManagementAction(
                                destroyableFolder, DestroyAction.NAME);
                    }
                    ...
                    public void then() throws Exception {
                        ...
                        NodeRef archiveNodeRef =
                                new NodeRef(
                                        StoreRef.STORE_REF_ARCHIVE_SPACESSTORE, subRecord.getId());
    ...
    /**
     * Given that a record is eligible for destruction And record ghosting is applied When the
     * record is destroyed Then the record is ghosted And the content is destroyed
     */
        ...
        doBehaviourDrivenTest(
                new BehaviourDrivenTest() {
                    ...
                    public void given() throws Exception {
                        ...
                        destroyableRecord =
                                utils.createRecord(recordFolder, GUID.generate(), GUID.generate());
                        utils.completeRecord(destroyableRecord);
                        utils.completeEvent(
                                destroyableRecord, CommonRMTestUtils.DEFAULT_EVENT_NAME);
                        rmActionService.executeRecordsManagementAction(
                                destroyableRecord, CutOffAction.NAME);

                        // assert things are as we expect
                        assertEquals(
                                DestroyAction.NAME,
                                dispositionService
                                        .getNextDispositionAction(destroyableRecord)
                                        .getName());
                        assertTrue(
                                dispositionService.isNextDispositionActionEligible(
                                        destroyableRecord));
                    ...
                    public void when() throws Exception {
                        // destroy the folder
                        rmActionService.executeRecordsManagementAction(
                                destroyableRecord, DestroyAction.NAME);
                    }
                    ...
                    public void then() throws Exception {
                        ...
                        ContentReader reader =
                                contentService.getReader(destroyableRecord, PROP_CONTENT);
                        assertNull(reader);
    ...
    /**
     * Given that a record is eligible for destruction And record ghosting is applied And cleansing
     * is configured on When the record is destroyed Then the record is ghosted And the content is
     * cleansed And then content is destroyed
     */
    ...
    /**
     * When a unclassified document (non-record) is deleted Then it is deleted but the the content
        ...
        doBehaviourDrivenTest(
                new BehaviourDrivenTest() {
                    ...
                    public void given() throws Exception {
                        ...
                        deleteableContent =
                                fileFolderService
                                        .create(folder, "myDocument.txt", TYPE_CONTENT)
                                        .getNodeRef();
                        ...
                        contentData =
                                (ContentData)
                                        nodeService.getProperty(deleteableContent, PROP_CONTENT);

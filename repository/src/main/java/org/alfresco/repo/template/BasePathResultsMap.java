...
/**
 * A special Map that executes an XPath against the parent Node as part of the get() Map interface
 * implementation.
 *
 * @author Kevin Roast
 */
public abstract class BasePathResultsMap extends BaseTemplateMap {
     ...
     */
    protected List<TemplateNode> getChildrenByXPath(
        ...
        if (xpath.length() != 0) {
            ...
            if (firstOnly == true) {
                if (nodes.size() != 0) {
                    ...
                    result.add(
                            new TemplateNode(
                                    nodes.get(0), this.services, this.parent.getImageResolver()));
                }
            }
            // or all the results
            else {
                ...
                for (NodeRef ref : nodes) {
                    result.add(
                            new TemplateNode(ref, this.services, this.parent.getImageResolver()));
                }
            }
        }

...
public class InvitationWebScriptTest extends BaseWebScriptTest {
  ...
  private static final QName avatarQName = QName.createQName(
  ...
  private final Map<String, Map<String, String>> userProperties = new HashMap<String, Map<String, String>>(
    3
  );
   ...
   * @throws Exception
   */
  protected void testInvitationsGet(boolean requireAcceptance)
    throws Exception {
    // Create two sites.
    ...
    String moderatedIdAUSER_TWO = createModeratedInvitation(
      shortNameSiteA,
      inviteeComments,
      userName,
      roleName
    ...
    String moderatedIdBUSER_TWO = createModeratedInvitation(
      shortNameSiteB,
      inviteeComments,
      userName,
      roleName
    ...
    String moderatedIdBUSER_THREE = createModeratedInvitation(
      shortNameSiteB,
      inviteeCommentsB,
      userNameB,
      roleNameB
    ...
    {
      Response response = sendRequest(
        new GetRequest(URL_INVITATIONS + "?inviteeUserName=" + userTwo),
        200
      );
      ...
      assertNotNull(
        "Moderated invitation to Site A not present!",
        moderatedAInv
      );
      JSONObject moderatedBInv = getInvitation(moderatedIdBUSER_TWO, data);
      assertNotNull(
        "Moderated invitation to Site B not present!",
        moderatedBInv
      );
      JSONObject nominatedInv = getInvitation(nominatedId, data);
      if (requireAcceptance) {
        assertNotNull(
          "Nominated invitation to Site A not present!",
          nominatedInv
        );
      }
    ...
    {
      Response response = sendRequest(
        new GetRequest(URL_INVITATIONS + "?invitationType=MODERATED"),
        200
      );
      JSONObject top = new JSONObject(response.getContentAsString());
      // System.out.println(response.getContentAsString());
      ...
      JSONObject moderatedBThreeInv = getInvitation(
        moderatedIdBUSER_THREE,
        data
      );
   ...
   * @throws Exception
   */
  protected void testSiteInvitationsGet(boolean requireAcceptance)
    throws Exception {
    // Create two sites.
    String shortNameSiteA = GUID.generate();
    createSite(
    ...
    // search for all invitations to site A: One Moderated Usr2, One Nominated User2
    {
      ...
      JSONArray data = top.getJSONArray("data");
      assertEquals(
        "Wrong number of invitations!",
        requireAcceptance ? 2 : 1,
        data.length()
      );
    ...
    // search for all invitations to site B: One Moderated User2, One Moderated User3.
    {
      ...
      assertNotNull(
        "Moderated invitation User 2 to Site B not present!",
        moderatedB2Inv
      );
      JSONObject moderatedB3Inv = getInvitation(moderatedIdBUSER_THREE, data);
      assertNotNull(
        "Moderated invitation User 3 to Site B not present!",
        moderatedB3Inv
      );
      checkJSONInvitations(data);
    }
    ...
    // search SiteA by type Moderated: One Moderated User2
    {
      ...
      assertEquals(
        "Wrong invitation type",
        "MODERATED",
        invitation.getString("invitationType")
      );
      JSONObject moderatedATwoInv = getInvitation(moderatedIdAUSER_TWO, data);
      assertNotNull("first is null", moderatedATwoInv);
    }
    ...
    if (requireAcceptance) {
      String siteANominatedUrl =
        ...
        "/" +
        shortNameSiteA +
        "/invitations?invitationType=NOMINATED";
    ...
    // search SiteB by userId 2: One Moderated User2
    {
      ...
      assertEquals(
        ...
        userTwo,
        invitation.getString("inviteeUserName")
      );
      JSONObject moderatedBTwoInv = getInvitation(moderatedIdBUSER_TWO, data);
   ...
   * @throws Exception
   */
  public void testSiteInvitationsGetRequireNominatedAcceptance()
    throws Exception {
    this.invitationServiceImpl.setNominatedInvitationWorkflowId(
        WorkflowModelNominatedInvitation.WORKFLOW_DEFINITION_NAME_ACTIVITI_INVITE
      );
  ...
  private void checkJSONInvitations(JSONArray data) throws JSONException {
    for (int i = 0; i < data.length(); i++) {
      ...
      assertEquals(
        "User name is wrong for user: " + i,
        userId,
        invitee.getString("userName")
      );
      assertEquals(
  ...
  public static String makeAvatar(
  ...
  private JSONObject getInvitation(String inviteId, JSONArray data)
    throws Exception {
    for (int i = 0; i < data.length(); i++) {
      JSONObject obj = data.getJSONObject(i);

...
public class GetAuthoritiesCannedQueryFactory
  ...
  public CannedQuery<AuthorityInfo> getCannedQuery(
    ...
    Pair<Long, NodeRef> nodePair = nodeDAO.getNodePair(
      tenantService.getName(containerRef)
    );
    if (nodePair == null) {
      throw new InvalidNodeRefException(
        "Container ref does not exist: " + containerRef,
        containerRef
      );
    }
    ...
    Pair<Long, QName> qnameAuthDisplayNamePair = qnameDAO.getQName(
    ...
    GetAuthoritiesCannedQueryParams paramBean = new GetAuthoritiesCannedQueryParams(
      type,
      typeQNameId,
      containerNodeId,

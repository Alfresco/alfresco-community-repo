...
public class SiteServiceImpl extends AbstractLifecycleBean implements SiteServiceInternal, SiteModel, NodeArchiveServicePolicies.BeforePurgeNodePolicy, NodeServicePolicies.OnRestoreNodePolicy, NodeArchiveServicePolicies.BeforeRestoreArchivedNodePolicy, NodeArchiveServicePolicies.OnRestoreArchivedNodePolicy
{
     ...
     * <p>
     * Note - Changes here likely need to be replicated to the {@link #updateSite(SiteInfo)} method too, as that also has to deal with Site Permissions.
     * 
     * @param siteNodeRef
     *            NodeRef
     * @param shortName
            ...
            final NodeRef siteNodeRef, final String shortName, final SiteVisibility visibility, final Map<String, Set<String>> memberships, final boolean ignoreExistingGroups)
    {
        ...
        AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<Object>() {
            public String doWork() throws Exception
            {
                ...
                Set<String> currentManagers = authorityService.getContainedAuthorities(AuthorityType.USER, getSiteRoleGroup(shortName, SiteModel.SITE_MANAGER, true), false);
    ...
    /**
     * @see org.alfresco.service.cmr.site.SiteService#getSiteRoleGroup(java.lang.String, java.lang.String)
     */
    ...
    /**
     * This method uses {@link CannedQuery canned queries} to retrieve {@link SiteModel#TYPE_SITE st:site} NodeRefs with support for {@link PagingRequest result paging}.
     */
    @Override
    ...
    /**
     * This method returns the {@link SiteInfo siteInfos} for sites to which the specified user has access. Note that if the user has access to more than 1000 sites, the list will be truncated to 1000 entries.
     * 
     * @param userName
     *            the username
    ...
    /**
     * Helper method to get the visibility of the site. If no value is present in the repository then it is calculated from the set permissions. This will maintain backwards compatibility with earlier versions of the service implementation.
     * 
     * @param siteNodeRef
     *            site node reference
    ...
    /**
     * This method gets the <code>st:site</code> NodeRef for the Share Site which contains the given NodeRef. If the given NodeRef is not contained within a Share Site, then <code>null</code> is returned.
     * 
     * @param nodeRef
     *            the node whose containing site is to be found.
     ...
     * @param shortName
     *            short name
     * @param enforcePermissions
     *            should we ensure that we have access to this node?
     * 
     * @return NodeRef node reference
     */
    ...
    /* Build an array of name filter tokens pre lowercased to test against person properties We require that matching people have at least one match against one of these on either their firstname or last name */
    ...
    /**
     * @see org.alfresco.service.cmr.site.SiteService#listMembersInfo(String, String, String, int, boolean)
     */
     ...
     * 
     * One of the user's firstname or lastname must match at least one of the filters given.
     * 
     * @param nameFilters
     *            String[]
     * @param username
    ...
    /**
     * @see org.alfresco.service.cmr.site.SiteService#getMembersRole(java.lang.String, java.lang.String)
     */
    ...
    /**
     * Helper method to get the permission groups for a given authority on a site. Returns empty List if the user does not have a explicit membership to the site.
     * 
    ...
    /**
     * @see org.alfresco.service.cmr.site.SiteService#canAddMember(java.lang.String, java.lang.String, java.lang.String)
     */
    ...
    /**
     * @see org.alfresco.service.cmr.site.SiteService#setMembership(java.lang.String, java.lang.String, java.lang.String)
     */
    ...
    /**
     * @see org.alfresco.service.cmr.site.SiteService#createContainer(java.lang.String, java.lang.String, org.alfresco.service.namespace.QName, java.util.Map)
     */
     ...
     *            the site which the site is a member of. If <code>null</code>, it will be calculated.
     */
    @Override
    ...
    /**
     * Moderated sites have separate ACLs on each component and don't inherit from the site which has consumer role for everyone.
     */
    ...
    public List<SiteMembership> listSiteMemberships(String userName, int size)
    {
        ...
        /* Get the site names and the map between the site name and the role MNT-13198 - use the bridge table */
        ...
        for (String authority : containingAuthorities)
        {
            ...
            if (size > 0)
            {
                ...
                /* MNT-10836 fix, after MNT-10109 we should also check site existence. A simple exists check would be better than getting the site properties etc - profiling suggests x2 faster. If size = -1 the sites that don't exist will be removed when getting the child associations. */
                if (!hasSite(siteName))
                {
        ...
        for (NodeRef site : siteNodes)
        {
            ...
            if (this.dictionaryService.isSubClass(siteClassName, SiteModel.TYPE_SITE))
            {
                ...
                /* Fix for ALF-21924. Role will be null in case the site id doesn't match the site added in roleSitePairs. This will fix cases where there is a site in trashcan with different case than an existing site. */
                if (role != null)
                {
     ...
     * 
     * @param siteNames
     *            names of the sites to be retrieved
    ...
    private List<ChildAssociationRef> getSitesAssocsByName(final List<String> siteNames)
    {
        ...
        while (sitesIterator.hasNext())
        {
            ...
            {
                /* get the child associations for the current page having siteRoot as parent use directNodeService to avoid permission check */
    ...
    /**
     * Private sites have separate ACLs on each component and don't inherit from the site which has consumer role for everyone.
     */

...
public class DeletedNodeCleanupWorker extends AbstractNodeCleanupWorker {
    ...
    // Unused transactions will be purged in chunks determined by commit time boundaries.
    // 'index.tracking.purgeSize' specifies the size
    ...
    /**
     * Set the minimum age (days) that nodes and transactions must be before they get purged. The
     * default is 7 days.
     *
    ...
    /**
     * Set a custom "from commit time" that will consider only the transactions after this specified
     * time Setting a negative value or 0 will trigger the default behaviour to get the oldest "from
     * time" for any deleted node
     *
    ...
    /**
     * Set the purge transaction block size. This determines how many unused transactions are purged
     * in one go.
     *
     * @param purgeSize int
     */
    ...
    private List<String> purgeOldDeletedNodes(long minAge) {
        ...
        while (true) {
            ...
            txnHelper.setRetryWaitIncrementMs(
                    1000); // 1 second to allow other cleanups time to get through
            ...
            try {
                DeleteNodesByTransactionsCallback purgeNodesCallback =
                        new DeleteNodesByTransactionsCallback(
                                nodeDAO, fromCommitTime, toCommitTime);
                ...
                loopPurgeSize /= 2L;
                // If the purge size drops below 10% of the original size, the entire process must
                // stop
                if (loopPurgeSize < 0.1 * purgeSize) {
                    msg =
                            "Failed to purge nodes. \n"
                                ...
                                + " ("
                                    + purgeSize
                                    + "), so the purging process was stopped.";
                    if (logger.isWarnEnabled()) {
                        logger.warn(msg, e);
                    } else {
                        logger.error(msg);
    ...
    private List<String> purgeOldEmptyTransactions(long minAge) {
        ...
        while (true) {
            ...
            // Purge transactions
            try {
                DeleteTransactionsCallback purgeTxnsCallback =
                        new DeleteTransactionsCallback(nodeDAO, fromCommitTime, toCommitTime);
    ...
    private abstract static class DeleteByTransactionsCallback
            implements RetryingTransactionCallback<Long> {

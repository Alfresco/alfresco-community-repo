...
public class GetTaskItemsRegressionTests extends RestTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify if get task items call return status code 404 when invalid taskId is"
                            + " provided")
    ...
    public void getTaskItemsUsingInvalidTaskId() throws Exception {
        ...
        restClient.authenticateUser(assignee).withWorkflowAPI().usingTask(taskModel).getTaskItems();
        restClient
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify if get task items request returns status code 200 after the task is"
                            + " finished.")
    @Test(groups = {TestGroup.REST_API, TestGroup.WORKFLOW, TestGroup.TASKS, TestGroup.REGRESSION})
    public void getTaskItemsAfterFinishingTask() throws Exception {
        ...
        itemModels
                ...
                .and()
                .entriesListContains("name", fileModel.getName());
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify if get task items request returns status code 200 after the process is"
                            + " deleted (Task state is now completed.)")
    @Test(groups = {TestGroup.REST_API, TestGroup.WORKFLOW, TestGroup.TASKS, TestGroup.REGRESSION})
    ...
    public void getTaskItemsByAdminUser() throws Exception {
        ...
        itemModels
                ...
                .entriesListIsNotEmpty()
                .and()
                .entriesListContains("id", taskItem.onModel().getId())
                .and()
                ...
                .assertThat()
                .field("createdBy")
                .contains(dataUser.getAdminUser().getUsername())
                .assertThat()
                .field("modifiedAt")
                .isNotNull()
                .assertThat()
                .field("name")
                .contains(fileModel.getName())
                .assertThat()
                .field("modifiedBy")
                .contains(userWhoStartsTask.getUsername())
                .assertThat()
                .field("id")
                .contains(fileModel.getNodeRef().split(";")[0])
                .assertThat()
                .field("mimeType")
                .contains(fileModel.getFileType().mimeType);
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that user who started the process can get task items with skip count"
                            + " parameter")
    public void getTaskItemsWithSkipCount() throws Exception {
        ...
        itemModels =
                restClient
                        ...
                        .getTaskItems();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        itemModels.assertThat().entriesListIsNotEmpty().and().paginationField("count").is("1");
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that user who started the process can get task items with max items"
                            + " parameter")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that user who started the process can get task items with valid"
                            + " properties")
    public void getTaskItemsWithValidProperties() throws Exception {
        ...
        itemModels =
                restClient
                        .authenticateUser(userWhoStartsTask)
                        .withWorkflowAPI()
                        .usingParams("properties=createdAt,createdBy,id,size")
                        .usingTask(taskModel)
                        .getTaskItems();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        itemModels
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that user who started the process can get task items with an invalid"
                            + " property")
    public void getTaskItemsWithInvalidProperties() throws Exception {
        ...
        itemModels =
                restClient
                        .authenticateUser(userWhoStartsTask)
                        .withWorkflowAPI()
                        .usingParams("properties=size,fake-prop")
                        .usingTask(taskModel)
                        .getTaskItems();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        itemModels
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that involved user in process that was deleted cannot get task items")
    public void getTaskItemsByDeletedUserInvolvedInProcess() throws Exception {
        ...
        taskModel =
                dataWorkflow
                        ...
                        .usingSite(siteModel)
                        .usingResource(fileModel)
                        .createNewTaskAndAssignTo(assigneeDeleted);

        dataUser.usingAdmin().deleteUser(assigneeDeleted);

        itemModels =
                restClient
                        .authenticateUser(assigneeDeleted)
                        .withWorkflowAPI()
                        .usingTask(taskModel)
                        .getTaskItems();
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that involved user in process that was disabled cannot get task items")
    public void getTaskItemsByDisabledUserInvolvedInProcess() throws Exception {
        ...
        taskModel =
                dataWorkflow
                        ...
                        .usingSite(siteModel)
                        .usingResource(fileModel)
                        .createNewTaskAndAssignTo(assigneeDisabled);

        dataUser.usingAdmin().disableUser(assigneeDisabled);

        itemModels =
                restClient
                        .authenticateUser(assigneeDisabled)
                        .withWorkflowAPI()
                        .usingTask(taskModel)
                        .getTaskItems();
        restClient.assertStatusCodeIs(HttpStatus.UNAUTHORIZED);

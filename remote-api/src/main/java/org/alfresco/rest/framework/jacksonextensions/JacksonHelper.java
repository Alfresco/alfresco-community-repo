...
/**
 * Helper Class for outputting Jackson content, makes use of the RestJsonModule (main Jackson
 * config). Default settings : Date format is ISO8601, only serializes non-empty / non-null values.
 *
 * @author Gethin James
 */
public class JacksonHelper implements InitializingBean {
    ...
    public void withWriter(OutputStream outStream, Writer writer) throws IOException {
        try {
            JsonGenerator generator =
                    objectMapper.getFactory().createGenerator(outStream, encoding);
    ...
    /**
     * Constructs the object based on the content as a List, the JSON can be an array or just a
     * single value without the [] symbols
     *
     * @param content Reader
        ...
        ObjectReader reader =
                objectMapper.readerFor(
                        TypeFactory.defaultInstance()
                                .constructParametricType(List.class, requiredType));
        try {

...
public class ModelValidatorImpl implements ModelValidator {
    ...
    private boolean canDeleteModel(
            Collection<NamespaceDefinition> namespaceDefs,
            Collection<TypeDefinition> typeDefs,
            Collection<AspectDefinition> aspectDefs,
            Tenant tenant) {
        ...
        if (workflowDefs.size() > 0) {
            if (namespaceDefs.size() > 0) {
                ...
                for (WorkflowDefinition workflowDef : workflowDefs) {
                    ...
                    try {
                        workflowNamespaceURI =
                                QName.createQName(
                                                BPMEngineRegistry.getLocalId(workflowDefName),
                                                namespaceService)
                                        .getNamespaceURI();
                    } catch (NamespaceException ne) {
                        logger.warn(
                                "Skipped workflow when validating model delete - unknown namespace:"
                                        + " "
                                        + ne);
                        continue;
                    }
                    ...
                    for (NamespaceDefinition namespaceDef : namespaceDefs) {
                        if (workflowNamespaceURI.equals(namespaceDef.getUri())) {
                            logger.warn(
                                    "Failed to validate model delete"
                                            + tenantDomain
                                            + " - found workflow process definition "
                                            + workflowDefName
    ...
    private void validateDeleteClass(final Tenant tenant, final ClassDefinition classDef) {
                ...
                .doInTransaction(
                        new RetryingTransactionCallback<Void>() {
                            ...
                            public Void execute() throws Throwable {
                                try {
                                    // The class QName may not have been created in the database if
                                    // no
                                    // properties have been created that use it, so check first and
                                    // then
                                    // try to delete it.
                                    if (qnameDAO.getQName(className) != null) {
                                        qnameDAO.deleteQName(className);
                                    }
                                    throw new ModelNotInUseException(
                                            "Class " + className + " not in use");
                                } catch (DataIntegrityViolationException e) {
                                    // catch data integrity violation e.g. foreign key constraint
                                    // exception
                                    logger.debug(e);
                                    throw new ModelInUseException(
                                            "Cannot delete model, class "
                                                    + className
                                                    + " is in use");
                                }
                            }
        ...
        for (WorkflowDefinition workflowDef : workflowService.getDefinitions()) {
            for (WorkflowTaskDefinition workflowTaskDef :
                    workflowService.getTaskDefinitions(workflowDef.getId())) {
                TypeDefinition workflowTypeDef = workflowTaskDef.metadata;
                if (workflowTypeDef.getName().equals(className)) {
                    throw new AlfrescoRuntimeException(
                            "Failed to validate model delete"
                                    + tenantDomain
                                    + " - found task definition in workflow "
                                    + workflowDef.getName()
                                    + " with "
                                    + classType
                                    + " '"
    ...
    private void validateDeleteProperty(QName modelName, QName propertyQName, boolean sharedModel) {
        ...
        if (prop != null
                && prop.getName().equals(propertyQName)
                && prop.getModel().getName().equals(modelName)) {
    ...
    private void validateDeleteProperty(
            final String tenantDomain, final PropertyDefinition propDef) {
                                ...
                                return TenantUtil.runAsTenant(
                                        new TenantRunAsWork<Void>() {
                                            ...
                                            public Void doWork() throws Exception {
                                                try {
                                                    // The property QName may not have been created
                                                    // in the database if no
                                                    // properties have been created that use it, so
                                                    ...
                                                    // catch data integrity violation e.g. foreign
                                                    // key constraint exception
                                                    logger.debug(e);
                                                    throw new ModelInUseException(
                                                            "Failed to validate property delete,"
                                                                    + " property "
                                                                    + propName
                                                                    + " is in use");
                                                }
    ...
    private void validateDeleteConstraint(
            CompiledModel compiledModel, QName constraintName, boolean sharedModel) {
        String tenantDomain = TenantService.DEFAULT_DOMAIN;
        if (sharedModel) {
            tenantDomain = " for tenant [" + tenantService.getCurrentUserDomain() + "]";
        }
        ...
        if (referencedBy.size() == 1) {
            throw new AlfrescoRuntimeException(
                    "Failed to validate constraint delete"
                            + tenantDomain
                            + " - constraint definition '"
                            + constraintName
                            + "' is being referenced by '"
                            + referencedBy.toArray()[0]
                            + "' property constraint");
    ...
    public boolean canDeleteModel(final QName modelName) {
        ...
        // TODO add model locking during delete (would need to be tenant-aware & cluster-aware) to
        // avoid potential
        ...
        try {
            ...
            // TODO - in case of MT we do not currently allow deletion of an overridden model (with
            // usages) ... but could allow if (re-)inherited model is equivalent to an incremental
            // update only ?
            ...
            if (canDeleteModel) {
                if (tenantService.isEnabled() && tenantService.isTenantUser() == false) {
                    ...
                    // shared model - need to check all tenants (whether enabled or disabled) unless
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Dictionary model '"
                                + modelName
                                + "' does not exist ... skip delete validation : "
                                + e);
            }
    ...
    public void validateModel(CompiledModel compiledModel) {
        ...
        for (M2ModelDiff modelDiff : modelDiffs) {
            ...
            if (modelDiff.getDiffType().equals(M2ModelDiff.DIFF_UPDATED)) {
                throw new AlfrescoRuntimeException(
                        "Failed to validate model update - found non-incrementally updated "
                                + modelDiff.getElementType()
                                + " '"
                                + modelDiff.getElementName()
                                + "'");
            ...
            if (modelDiff.getDiffType().equals(M2ModelDiff.DIFF_CREATED)) {
                if (modelDiff.getElementType().equals(M2ModelDiff.TYPE_NAMESPACE)) {
                    ModelDefinition importedModel =
                            dictionaryService.getModelByNamespaceUri(
                                    modelDiff.getNamespaceDefinition().getUri());
                    if (importedModel != null && !model.getNamespaces().isEmpty()) {
                        checkCircularDependency(
                                importedModel, model, importedModel.getName().getLocalName());
                    }
                }
            }
        }

        // TODO validate that any deleted constraints are not being referenced - else currently will
        // become anon - or push down into model compilation (check backwards compatibility ...)
    }

    private void checkCircularDependency(
            ModelDefinition model, M2Model existingModel, String parentPrefixedName)
            throws AlfrescoRuntimeException {
        for (NamespaceDefinition importedNamespace : model.getImportedNamespaces()) {
            ModelDefinition md = null;
            if ((md = dictionaryService.getModelByNamespaceUri(importedNamespace.getUri()))
                    != null) {
                if (existingModel.getNamespace(importedNamespace.getUri()) != null) {
                    throw new AlfrescoRuntimeException(
                            "Failed to validate model update - found circular dependency. You can't"
                                    + " set parent "
                                    + parentPrefixedName
                                    + " as it's model already depends on "

...
public class CompanyHomeContext implements NodeRefContext {
  ...
  private String[] createRelativeNamePath(String[] namePath) {
    ...
    companyHomeRealtiveRef[0] =
      StoreRef.STORE_REF_WORKSPACE_SPACESSTORE.getProtocol();
    companyHomeRealtiveRef[1] =
      StoreRef.STORE_REF_WORKSPACE_SPACESSTORE.getIdentifier();
    companyHomeRealtiveRef[2] = companyHomeQName;

    if (namePath.length > 0) {
      System.arraycopy(namePath, 0, companyHomeRealtiveRef, 3, namePath.length);
    }
    return companyHomeRealtiveRef;
  }

  @Override
  public NodeRef resolveQNamePath(
    String[] qNamePath,
  ...
  @Override
  public NodeRef createQNamePath(
    String[] qNamePath,
    String[] names,
    NodeRefResolver resolver
  ) {
    String[] relativeQNamePath = createRelativeQNamePath(qNamePath);
    return resolver.createQNamePath(relativeQNamePath, names);
  }

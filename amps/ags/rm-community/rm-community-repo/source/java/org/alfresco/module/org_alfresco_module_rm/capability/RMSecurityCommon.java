...
public class RMSecurityCommon implements ApplicationContextAware {
   ...
   */
  public void setCaveatConfigComponent(
    RMCaveatConfigComponent caveatConfigComponent
  ) {
  ...
  protected FilePlanService getFilePlanService() {
    if (filePlanService == null) {
      filePlanService =
        (FilePlanService) applicationContext.getBean("filePlanService");
    }
  ...
  public int checkRmRead(NodeRef nodeRef) {
    ...
    Map<Pair<String, NodeRef>, Integer> transactionCache = TransactionalResourceHelper.getMap(
      "rm.security.checkRMRead"
    );
    Pair<String, NodeRef> key = new Pair<>(
    ...
    if (transactionCache.containsKey(key)) {
      ...
      if (
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "\t\tUser does not have read record permission on node, access denied.  (nodeRef=" +
            nodeRef.toString() +
            ", user=" +
            AuthenticationUtil.getRunAsUser() +
            ")"
        ...
        if (
          filePlan != null && hasViewCapability(filePlan) == AccessStatus.DENIED
        ) {
          if (logger.isDebugEnabled()) {
            logger.debug(
  ...
  private AccessStatus hasViewCapability(NodeRef filePlan) {
    Map<Pair<String, NodeRef>, AccessStatus> transactionCache = TransactionalResourceHelper.getMap(
      "rm.security.hasViewCapability"
    );
    Pair<String, NodeRef> key = new Pair<>(
      AuthenticationUtil.getRunAsUser(),
      filePlan
    ...
    if (transactionCache.containsKey(key)) {
      ...
      AccessStatus result = permissionService.hasPermission(
        filePlan,
  ...
  protected NodeRef getTestNode(
    ...
    if (position < 0) {
      ...
      if (invocation.getArguments()[position] != null) {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "\tPermission test against the store - using permissions on the root node"
          );
        }
      ...
      AssociationRef.class.isAssignableFrom(params[position]) &&
      invocation.getArguments()[position] != null
    ) {
      if (parent) {
        testNodeRef =
          ((AssociationRef) invocation.getArguments()[position]).getSourceRef();
      } else {
        testNodeRef =
          ((AssociationRef) invocation.getArguments()[position]).getTargetRef();
      }
      if (logger.isDebugEnabled()) {
        if (nodeService.exists(testNodeRef)) {
          logger.debug(

...
/**
 * A default {@link ChildApplicationContextManager} implementation that manages a 'chain' of {@link
 * ChildApplicationContextFactory} objects, perhaps corresponding to the components of a chained
 * subsystem such as authentication. As with other {@link PropertyBackedBean}s, can be stopped,
 * reconfigured, started and tested. Its one special <code>chain</code> property allows an ordered
 * list of {@link ChildApplicationContextFactory} objects to be managed. This property is a comma
 * separated list with the format:
 *
 * <ul>
 *   <li>&lt;id1>:&lt;typeName1>,&lt;id2>:&lt;typeName2>,...,&lt;id<i>n</i>>:&lt;typeName<i>n</i>>
 * </ul>
 *
 * See {@link ChildApplicationContextManager} for the meanings of &lt;id> and &lt;typeName>. In the
 * enterprise edition, this property is editable at runtime via JMX. If a new &lt;id> is included in
 * the list then a new {@link ChildApplicationContextFactory} will be brought into existence.
 * Similarly, if one is removed from the list, then the corresponding instance will be destroyed.
 * For Alfresco community edition, the chain is best configured through the method via Spring
 * configuration.
 *
 * @author dward
 */
public class DefaultChildApplicationContextManager extends AbstractPropertyBackedBean
    ...
    /**
     * Sets the default type name. This is used when a type name is not included after an instance
     * ID in a chain string.
     *
     * @param defaultTypeName the new default type name
     */
    ...
    /**
     * Configures the default chain of {@link ChildApplicationContextFactory} instances. May be set
     * on initialization by the Spring container.
     *
     * @param defaultChain a comma separated list in the following format:
     *     <ul>
     *       <li>&lt;id1>:&lt;typeName1>,&lt;id2>:&lt;typeName2>,...,&lt;id<i>n</i>>:&lt;typeName<i>n</i>>
     *     </ul>
     */
    ...
    public void destroy(boolean permanent) {
        if (getState(false) != null) {
            ...
            try {
                ApplicationContextManagerState state =
                        (ApplicationContextManagerState) getState(false);

                if (state != null) {
                    // Cascade the destroy / shutdown
                    for (String id : state.getInstanceIds()) {
                        ChildApplicationContextFactory factory =
                                state.getApplicationContextFactory(id);
    ...
    protected class ApplicationContextManagerState implements PropertyBackedBeanState {
        ...
        private Map<String, ChildApplicationContextFactory> childApplicationContexts =
                new TreeMap<String, ChildApplicationContextFactory>();
        ...
        protected ApplicationContextManagerState(String defaultChain, String defaultTypeName) {
            // Work out what the default type name should be; either specified explicitly or implied
            // by the first member
            ...
            if (defaultChain != null && defaultChain.length() > 0) {
                ...
                if (defaultTypeName == null) {
                    ...
                    this.defaultTypeName =
                            this.childApplicationContexts
                                    .get(this.instanceIds.get(0))
                                    .getTypeName();
                } else {
                    this.defaultTypeName = defaultTypeName;
        ...
        public void setProperty(String name, String value) {
            if (!name.equals(DefaultChildApplicationContextManager.ORDER_PROPERTY)) {
                throw new IllegalStateException(
                        "Illegal attempt to write to property \"" + name + "\"");
            }
        ...
        public void start() {
            ...
            for (String instance : getInstanceIds()) {
                try {
                    ...
                    // One of the subsystems failed to initialize. The cause would have been logged.
                    // Treat this as
                    // non-fatal
                    lastError = e;
                }
        ...
        private String getOrderString() {
            ...
            for (String id : this.instanceIds) {
                ...
                orderString
                        .append(id)
                        .append(':')
                        .append(this.childApplicationContexts.get(id).getTypeName());
            }
         ...
         *
         * @param orderString the order as a comma or whitespace separated string
         * @param defaultTypeName the default type name
         */
        private void updateOrder(String orderString, String defaultTypeName) {
            try {
                ...
                while (tkn.hasMoreTokens()) {
                    ...
                    String typeName =
                            sepIndex == -1 || sepIndex + 1 >= instance.length()
                                    ? defaultTypeName
                    ...
                    // If we have the same instance ID but a different type, treat that as a destroy
                    // and remove
                    ...
                    if (factory == null) {
                        ...
                        childId.add("managed");
                        childId.add(id);
                        this.childApplicationContexts.put(
                                id,
                                new ChildApplicationContextFactory(
                                        getParent(),
                                        getRegistry(),
                                        getPropertyDefaults(),
                ...
                Set<String> idsToRemove =
                        new TreeSet<String>(this.childApplicationContexts.keySet());
                idsToRemove.removeAll(newInstanceIds);
                for (String id : idsToRemove) {
                    ChildApplicationContextFactory factory =
                            this.childApplicationContexts.remove(id);
                    factory.lock.writeLock().lock();
                    try {
                        factory.destroy(true);
                    } finally {

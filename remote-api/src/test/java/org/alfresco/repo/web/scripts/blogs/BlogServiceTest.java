...
public class BlogServiceTest extends BaseWebScriptTest {
  ...
  private JSONObject getPost(String name, int expectedStatus) throws Exception {
    Response response = sendRequest(
      new GetRequest(URL_BLOG_POST + name),
      expectedStatus
    );
    if (expectedStatus == 200) {
      JSONObject result = new JSONObject(response.getContentAsString());
  ...
  public void testCreateDraftPost() throws Exception {
    ...
    Response response = sendRequest(
      new GetRequest(URL_MY_DRAFT_BLOG_POSTS),
      200
    );
    JSONObject result = new JSONObject(response.getContentAsString());
  ...
  public void testCreateDraftPostWithTagsAndComment() throws Exception {
    ...
    List<String> recoveredTagsList = Arrays.asList(
      new String[] { reportedTags.getString(0), reportedTags.getString(1) }
    );
    ...
    String jsonToPost = new JSONStringer()
      .object()
      .key("title")
    ...
    Response response = sendRequest(
      new PostRequest(commentsPostUrl, jsonToPost, "application/json"),
      200
    );
    ...
    assertEquals(
      "Wrong comment count",
      1,
      items.getJSONObject(0).getInt("commentCount")
    );

    // and that there is content at the commentsURL.
    String commentsUrl =
      "/api" + items.getJSONObject(0).getString("commentsUrl");
    response = sendRequest(new GetRequest(commentsUrl), 200);

    // Now get blog-post by tag.
    // 1. No such tag
    response =
      sendRequest(new GetRequest(URL_BLOG_POSTS + "?tag=NOSUCHTAG"), 200);
  ...
  public void testCreatePublishedPost() throws Exception {
    ...
    Response response = sendRequest(
      new GetRequest(URL_MY_PUBLISHED_BLOG_POSTS),
      200
    );
    JSONObject result = new JSONObject(response.getContentAsString());
    ...
    assertEquals(
      this.posts.size() + this.drafts.size(),
      result.getInt("total")
    );
  ...
  public void testGetAll() throws Exception {
    ...
    response =
      sendRequest(new GetRequest(url + "?pageSize=2&startIndex=0"), 200);
    result = new JSONObject(response.getContentAsString());
    assertEquals(3, result.getInt("total"));
    assertEquals(0, result.getInt("startIndex"));
    assertEquals(2, result.getInt("itemCount"));
    ...
    assertEquals(
      TITLE_3,
      result.getJSONArray("items").getJSONObject(0).get("title")
    );
    assertEquals(
      TITLE_1,
      result.getJSONArray("items").getJSONObject(1).get("title")
    );

    response =
      sendRequest(new GetRequest(url + "?pageSize=2&startIndex=1"), 200);
    result = new JSONObject(response.getContentAsString());
    assertEquals(3, result.getInt("total"));
    ...
    response =
      sendRequest(new GetRequest(url + "?pageSize=2&startIndex=2"), 200);
    result = new JSONObject(response.getContentAsString());
    assertEquals(3, result.getInt("total"));
  ...
  public void testComments() throws Exception {
    ...
    // fetch the comments
    Response response = sendRequest(
      new GetRequest(getCommentsUrl(nodeRef)),
      200
    );
    JSONObject result = new JSONObject(response.getContentAsString());
    assertEquals(2, result.getInt("total"));
    ...
    response =
      sendRequest(
        new DeleteRequest(getCommentUrl(commentThree.getString("nodeRef"))),
        200
      );
    ...
    JSONObject commentTwoUpdated = updateComment(
      commentTwo.getString("nodeRef"),
      "new title",
      "new content",
      200
    );
  ...
  public void testDeleteCommentPostActivity() throws Exception {
    ...
    int activityNumStart = activityService
      .getSiteFeedEntries(SITE_SHORT_NAME_BLOG)
      .size();
    String nodeRef = item.getString("nodeRef");
    JSONObject commentOne = createComment(nodeRef, "comment", "content", 200);
    assertNotNull(item);
    postLookup.execute();
    feedGenerator.execute();
    int activityNumNext = activityService
      .getSiteFeedEntries(SITE_SHORT_NAME_BLOG)
      .size();
    assertEquals(
    ...
    Response resp = sendRequest(
      new DeleteRequest(getDeleteCommentUrl(commentNodeRef)),
      200
    );
    ...
    feedGenerator.execute();
    activityNumNext =
      activityService.getSiteFeedEntries(SITE_SHORT_NAME_BLOG).size();
    assertEquals(
  ...
  public void off_testClearTags() throws Exception {
    ...
    item =
      updatePost(item.getString("name"), null, null, new String[0], false, 200);

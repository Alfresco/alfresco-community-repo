...
public class AuthenticationFilter extends BaseAuthenticationFilter
    ...
    static {
        String[] encodings =
                new String[] {"UTF-8", System.getProperty("file.encoding"), "ISO-8859-1"};
    ...
    public void doFilter(
            ServletContext context, ServletRequest req, ServletResponse resp, FilterChain chain)
    ...
    protected void doFilterInternal(
        ...
        if (user == null) {
            ...
            if (authHdr != null
                ...
                // ALF-13621: Due to browser inconsistencies we have to try a fallback path of
                // encodings
                ...
                for (String encoding : ENCODINGS) {
                    ...
                    try {
                        ...
                        String basicAuth =
                                decoder.decode(ByteBuffer.wrap(encodedString)).toString();
                        ...
                        if (logger.isTraceEnabled()) {
                            logger.trace(
                                    "Successfully created user environment, login using basic auth"
                                            + " or ROLE_TICKET for user: "
                ...
                if (ticket != null && ticket.length() > 0) {
                    ...
                    user =
                            createUserEnvironment(
                                    httpReq.getSession(), currentUsername, ticket, false);
                    if (logger.isTraceEnabled()) {
                        logger.trace(
                                "Successfully created user environment, login using TICKET for"
                                        + " user: "
                                        + AuthenticationUtil.maskUsername(user.getUserName()));
                    }
                }

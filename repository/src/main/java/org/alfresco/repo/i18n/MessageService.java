...
public interface MessageService extends TenantDeployer, MessageLookup
{
    ...
    /**
     * Searches for the nearest locale from the available options. To match any locale, pass in <tt>null</tt>.
     *
     * @param templateLocale
     *            the template to search for or <tt>null</tt> to match any locale
     * @param options
     *            the available locales to search from
     * @return Returns the best match from the available options, or the <tt>null</tt> if all matches fail
     */
     ...
     * @param localeStr
     *            e.g. fr_FR
     * @return Returns the locale instance, or the {@link Locale#getDefault() default} if the string is invalid
     */

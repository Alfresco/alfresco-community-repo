...
public class HoldServiceImpl
  ...
  /** I18N */
  private static final String MSG_ERR_ACCESS_DENIED =
  ...
  public void init() {
    ...
    beforeRemoveFromHoldPolicyDelegate =
      getPolicyComponent()
        .registerClassPolicy(BeforeRemoveFromHoldPolicy.class);
  ...
  public List<NodeRef> getHolds(NodeRef filePlan) {
    ...
    if (holdContainer != null) {
      ...
      List<ChildAssociationRef> holdsAssocs = nodeService.getChildAssocs(
        holdContainer,
        ASSOC_CONTAINS,
  ...
  public List<NodeRef> heldBy(NodeRef nodeRef, boolean includedInHold) {
    ...
    if (isRecord(nodeRef)) {
      final List<NodeRef> recordFolders = recordFolderService.getRecordFolders(
        nodeRef
      );
      ...
      if (!CollectionUtils.isEmpty(filePlans)) {
        ...
        filePlans.forEach(filePlan -> {
          ...
          holdsNotIncludingNodeRef.addAll(
            ListUtils.subtract(allHolds, new ArrayList<>(holdsIncludingNodeRef))
          );
        });
  ...
  private Set<NodeRef> getParentHolds(NodeRef nodeRef) {
    List<ChildAssociationRef> holdsAssocs = nodeService.getParentAssocs(
      nodeRef,
  ...
  public NodeRef getHold(NodeRef filePlan, String name) {
    ...
    NodeRef hold = nodeService.getChildByName(
      holdContainer,
      ASSOC_CONTAINS,
      name
    );
  ...
  private void checkPermissionsForDeleteHold(NodeRef hold) {
    ...
    for (NodeRef nodeRef : held) {
      try {
        ...
        if (
          permissionService.hasPermission(nodeRef, permission) ==
          AccessStatus.DENIED
        ) {
          heldNames.add(
        ...
        throw new AccessDeniedException(
          I18NUtil.getMessage(MSG_ERR_HOLD_PERMISSION_GENERIC_ERROR),
          ade
        );
      }
    }
    ...
    if (heldNames.size() > 0) {
      ...
      stream1
        .limit(MAX_HELD_ITEMS_LIST_SIZE)
        .forEach(name -> {
          sb.append("\n ");
          sb.append("'");
          sb.append(name);
      ...
      throw new AccessDeniedException(
        I18NUtil.getMessage(MSG_ERR_HOLD_PERMISSION_DETAILED_ERROR) +
        sb.toString()
      );
    }
  }
  ...
  public void addToHolds(final List<NodeRef> holds, final NodeRef nodeRef) {
    ...
    for (final NodeRef hold : holds) {
      if (!isHold(hold)) {
        final String holdName = (String) nodeService.getProperty(
          hold,
          ContentModel.PROP_NAME
        );
        throw new IntegrityException(
          I18NUtil.getMessage("rm.hold.not-hold", holdName),
          null
        );
      }
      ...
      if (
        ...
        throw new AccessDeniedException(
          I18NUtil.getMessage(MSG_ERR_ACCESS_DENIED)
        );
      }
  ...
  private void checkNodeCanBeAddedToHold(NodeRef nodeRef) {
    if (
      ...
      final String nodeName = (String) nodeService.getProperty(
        nodeRef,
        ContentModel.PROP_NAME
      );
      throw new IntegrityException(
        I18NUtil.getMessage("rm.hold.add-to-hold-invalid-type", nodeName),
        null
      );
    }
    ...
    if (
      (
        (isRecord(nodeRef) || isRecordFolder(nodeRef)) &&
    ...
    if (nodeService.hasAspect(nodeRef, ASPECT_ARCHIVED)) {
      throw new IntegrityException(
        I18NUtil.getMessage("rm.hold.add-to-hold-archived-node"),
        null
      );
    }
    ...
    if (nodeService.hasAspect(nodeRef, ASPECT_LOCKABLE)) {
      throw new IntegrityException(
        I18NUtil.getMessage("rm.hold.add-to-hold-locked-node"),
        null
      );
    }
  }
   ...
   */
  protected void invokeBeforeCreateHold(
    NodeRef nodeRef,
    String name,
    String reason
  ) {
    // execute policy for node type and aspects
    BeforeCreateHoldPolicy policy = beforeCreateHoldPolicyDelegate.get(
      getTypeAndApsects(nodeRef)
    );
  ...
  protected void invokeOnCreateHold(NodeRef nodeRef) {
    OnCreateHoldPolicy policy = onCreateHoldPolicyDelegate.get(
      getTypeAndApsects(nodeRef)
    );
  ...
  protected void invokeBeforeDeleteHold(NodeRef nodeRef) {
    BeforeDeleteHoldPolicy policy = beforeDeleteHoldPolicyDelegate.get(
      getTypeAndApsects(nodeRef)
    );
  ...
  protected void invokeBeforeAddToHold(NodeRef hold, NodeRef contentNodeRef) {
    BeforeAddToHoldPolicy policy = beforeAddToHoldPolicyDelegate.get(
      getTypeAndApsects(hold)
    );
  ...
  protected void invokeOnAddToHold(NodeRef hold, NodeRef contentNodeRef) {
    OnAddToHoldPolicy policy = onAddToHoldPolicyDelegate.get(
      getTypeAndApsects(hold)
    );
   ...
   * @param contentNodeRef content node reference
   */
  protected void invokeBeforeRemoveFromHold(
    NodeRef hold,
    NodeRef contentNodeRef
  ) {
    BeforeRemoveFromHoldPolicy policy = beforeRemoveFromHoldPolicyDelegate.get(
      getTypeAndApsects(hold)
    );
  ...
  protected void invokeOnRemoveFromHold(NodeRef hold, NodeRef contentNodeRef) {
    OnRemoveFromHoldPolicy policy = onRemoveFromHoldPolicyDelegate.get(
      getTypeAndApsects(hold)
    );

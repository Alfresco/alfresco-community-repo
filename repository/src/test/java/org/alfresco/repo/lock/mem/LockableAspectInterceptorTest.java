...
public class LockableAspectInterceptorTest {
  ...
  public void setUp() {
    ...
    transactionService =
      (TransactionService) appCtx.getBean("TransactionService");
    ...
    interceptor =
      (LockableAspectInterceptor) appCtx.getBean("lockableAspectInterceptor");
  ...
  public void testHasAspectEphemeralLock() {
    QName nodeName = QName.createQName(
      "http://www.alfresco.org/test/" + getClass().getSimpleName(),
      "testNode"
    );
   ...
   */
  private void ensurePropertyNotPresent(
    final NodeRef nodeRef,
    final QName propQName
  ) {
  ...
  public void testGetPropertyEphemeralLock() {
    ...
    assertEquals(
      null,
      nodeService.getProperty(nodeRef, ContentModel.PROP_LOCK_OWNER)
    );
    assertEquals(
      null,
    ...
    assertEquals(
      null,
      nodeService.getProperty(nodeRef, ContentModel.PROP_EXPIRY_DATE)
    );
    assertEquals(
      null,
  ...
  @Test
  public void testEnableDisableForThread()
    ...
    assertFalse(
      "Node should be reported as lockable (new thread)",
      future.get()
    );
  ...
  private void writeData(String fileName, final NodeRef fileNodeRef) {
    ...
    InputStream is = getClass()
      .getClassLoader()
      .getResourceAsStream("farmers_markets_list_2003.doc");

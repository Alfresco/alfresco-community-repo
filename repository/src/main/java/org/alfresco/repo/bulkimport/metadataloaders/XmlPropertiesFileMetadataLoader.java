 ...
 * This shadow properties file must have <strong>exactly</strong> the same name and extension as the
 * file for whom it is storing metadata, but with the suffix ".metadata.properties.xml". So for
 * example, if there is a file called "IMG_1967.jpg", the "shadow" metadata file for it would be
 * called "IMG_1967.jpg.metadata.properties.xml".
 *
 * <p>The metadata file itself follows the usual rules for Java properties XML files, with a
 * property with the key "type" containing the qualified name of the content type to use for the
 * file, a property with the key "aspects" containing a comma-delimited list of qualified names of
 * the aspects to attach to the file, and then one Java property per metadata property, with the key
 * being the Alfresco property QName and the value being the value of that property.
 *
 * <p>For example (note escaping rules for namespace separator!): <code>
 ...
 *
 * <ul>
 *   <li>Java XML properties files fully support Unicode characters (unlike the original properties
 *       file format), so use of this class is strongly recommended over and <code>
 *       PropertiesFileMetadataLoader</code>.
 *   <li>the metadata must conform to the type and aspect definitions configured in Alfresco
 *       (including mandatory fields, constraints and data types). Any violations will terminate the
 *       bulk import process.
 *   <li>associations are not yet supported
 *   <li>dates, times and date times <u>must</u> be stored in ISO8601 format (although note that
 *       Alfresco ignores timezone modifiers)
 * </ul>
 *
 * @since 4.0
 * @see MetadataLoader
...
public final class XmlPropertiesFileMetadataLoader extends AbstractMapBasedMetadataLoader {
    ...
    public XmlPropertiesFileMetadataLoader(
            final ServiceRegistry serviceRegistry, final String multiValuedSeparator) {
    ...
    protected Map<String, Serializable> loadMetadataFromFile(Path metadataFile) {
        ...
        try {
            ...
            if (log.isWarnEnabled())
                log.warn(
                        "Metadata file '"
                                + FileUtils.getFileName(metadataFile)
                                + "' could not be read.",
                        ioe);
        }

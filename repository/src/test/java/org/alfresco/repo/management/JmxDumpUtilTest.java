...
public class JmxDumpUtilTest extends TestCase {
    ...
    public void testCleanPasswordsFromInputArgument() throws Exception {
        Pattern pattern =
                Pattern.compile("(?i)(.*(password=|pwd=|token=))((?<=password=|pwd=|token=).*+)");
        ...
        assertEquals(
                "Expectected output: " + expected + " Actual output: " + actual, expected, actual);
    ...
    public void testCleanPasswordsFromInputArguments() throws Exception {
        ...
        expectedArray =
                new String[] {"-Ddb.port=1234", "-Ddb.user=alfresco", "-DtestArg=Test1234password"};
    ...
    public void testCreatePasswordFindRegexString() throws Exception {
        ...
        String expected =
                "(?i)(.*(password=|any old ending :D=|token=))((?<=password=|any old ending"
                        + " :D=|token=).*+)";

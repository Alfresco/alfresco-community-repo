...
public class DictionaryDAOTest {
  ...
  private static final String TEST2_URL =
    "http://www.alfresco.org/test/dictionarydaotest2/1.0";
  private static final String TEST_MODEL =
    "org/alfresco/repo/dictionary/dictionarydaotest_model.xml";
  private static final String TEST_MODEL_NEW_FORMAT =
    "org/alfresco/repo/dictionary/dictionarydaotest_model_new_format.xml";
  private static final String TEST_NS_CLASH_MODEL =
    "org/alfresco/repo/dictionary/nstest_model.xml";
  private static final String TEST_BUNDLE =
    "org/alfresco/repo/dictionary/dictionarydaotest_model";
  private static final String TEST_COMMON_NS_PARENT_MODEL =
    "org/alfresco/repo/dictionary/commonpropertynsparent_model.xml";
  private static final String TEST_COMMON_NS_CHILD_MODEL =
    "org/alfresco/repo/dictionary/commonpropertynschild_model.xml";
  ...
  public void testNamespaceClashResultsInSensibleError() {
    ...
    try {
      ...
      fail(
        "Expected " +
        NamespaceException.class.getName() +
        " to be thrown, but it was not."
      );
  ...
  public void testLabels() {
    ...
    assertEquals(
      "Wrong localised lov value.",
      "ABC display",
      lovConstraint.getDisplayLabel("ABC", service)
    );
    assertEquals(
      "Wrong localised lov value.",
      "DEF display",
      lovConstraint.getDisplayLabel("DEF", service)
    );
    assertEquals(
      "Wrong localised lov value.",
      "VALUE WITH SPACES display",
      lovConstraint.getDisplayLabel("VALUE WITH SPACES", service)
    ); // Keys with spaces.
    assertEquals(
      "Wrong localised lov value.",
      "VALUE WITH TRAILING SPACE display",
      lovConstraint.getDisplayLabel("VALUE WITH TRAILING SPACE ", service)
    ); // Keys with trailing space.
    ...
    PropertyDefinition constrainedPropDef = service.getProperty(
      constrainedPropName
    );
    ...
    lovConstraint =
      (ListOfValuesConstraint) inlineConstraintDef.getConstraint();
    assertEquals(
      "Wrong localised lov value.",
      "ALPHA display",
      lovConstraint.getDisplayLabel("ALPHA", service)
    );
    assertEquals(
      "Wrong localised lov value.",
      "BETA display",
      lovConstraint.getDisplayLabel("BETA", service)
    );
    assertEquals(
      "Wrong localised lov value.",
      "GAMMA, DELTA display",
      lovConstraint.getDisplayLabel("GAMMA, DELTA", service)
    ); // Keys with commas
    assertEquals(
      "Wrong localised lov value.",
      "OMEGA",
      lovConstraint.getDisplayLabel("OMEGA", service)
    );
    assertNull(lovConstraint.getDisplayLabel("nosuchLOV", service));
  ...
  public void testConstraintsOverrideInheritanceOnAspects() {
    ...
    // get the base property
    PropertyDefinition propDef = service.getProperty(
      aspectBaseQName,
      propQName
    );
    ...
    assertTrue(
      "Constraint instance incorrect",
      propConstraints.get(0).getConstraint() instanceof ListOfValuesConstraint
    );
    ListOfValuesConstraint constraint = (ListOfValuesConstraint) propConstraints
      .get(0)
      .getConstraint();
    ...
    constraint =
      (ListOfValuesConstraint) propConstraints.get(0).getConstraint();
    ...
    assertTrue(
      "Constraint instance incorrect",
      propConstraints.get(1).getConstraint() instanceof ListOfValuesConstraint
    );
    constraint =
      (ListOfValuesConstraint) propConstraints.get(0).getConstraint();
    allowedValues = constraint.getAllowedValues();
    ...
    assertEquals("VALUE WITH TRAILING SPACE ", allowedValues.get(3));
    constraint =
      (ListOfValuesConstraint) propConstraints.get(1).getConstraint();
    allowedValues = constraint.getAllowedValues();
  ...
  public void testCreateModelWithCircularTypeDependency() {
    ...
    try {
      ...
      fail(
        "Model should not be saved successfully because it contains a cyclic reference"
      );
    } catch (DictionaryException e) {
      assertEquals(
        e.getMsgId(),

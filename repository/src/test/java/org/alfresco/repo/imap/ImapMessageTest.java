...
public class ImapMessageTest extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    authenticationService.authenticate(
      ADMIN_USER_NAME,
      ADMIN_USER_PASSWORD.toCharArray()
    );
    ...
    ImapServiceImpl imapServiceImpl = (ImapServiceImpl) imapCtx.getBean(
      "imapService"
    );
    ...
    imapHome.setFolderPath(
      NamespaceService.CONTENT_MODEL_PREFIX + ":" + IMAP_FOLDER_NAME
    );
  ...
  public void testMessageModifiedBetweenReads() throws Exception {
    ...
    assertFalse(
      Arrays.equals(
        bodyNew.getByteArray().getBytes(),
        body.getByteArray().getBytes()
      )
    );
  ...
  @Test
  public void testSearchTermWithNonEnglishLocaleFalsePositive()
    throws Exception {
    Locale defaultLocale = Locale.getDefault();
    try {
      folder = (IMAPFolder) store.getFolder(TEST_FOLDER);
  ...
  public void testEncodedFromToAddresses() throws Exception {
    ...
    String decodedAddress = MimeUtility.decodeText(
      MimeUtility.fold(6, address.toString())
    );
    ...
    FileInfo messageFile = fileFolderService.create(
    ...
    while (
      ...
      origModified.equals(getModified(nodeRef))
    ) {
    ...
    List<String> cmAddressees = (List<String>) properties.get(
      ContentModel.PROP_ADDRESSEES
    );
    String imapMessageFrom = (String) properties.get(
      ImapModel.PROP_MESSAGE_FROM
    );
  ...
  private Serializable getModified(NodeRef nodeRef) {
    Map<QName, Serializable> origProperties = nodeService.getProperties(
      nodeRef
    );
  ...
  private static RFC822DATA getRFC822Message(
    final IMAPFolder folder,
    final long uid
  ) throws MessagingException {
    return (RFC822DATA) folder.doCommand(
  ...
  private static BODY getMessageBodyPart(
    ...
    return (BODY) folder.doCommand(
      new IMAPFolder.ProtocolCommand() {
        public Object doCommand(IMAPProtocol p) throws ProtocolException {
          Response[] r = p.command(
            "UID FETCH " +
            uid +
            " (FLAGS BODY.PEEK[]<" +
            from +
          ...
          if (!response.isOK()) {
            throw new ProtocolException(
              "Unable to retrieve message part <" + from + "." + count + ">"
            );
          }

          FetchResponse fetchResponse = (FetchResponse) r[0];
          BODY body = (BODY) fetchResponse.getItem(
   ...
   * @throws MessagingException
   */
  private static Long getMessageUid(IMAPFolder folder, final int msn)
    throws MessagingException {
    return (Long) folder.doCommand(
      new IMAPFolder.ProtocolCommand() {
        public Object doCommand(IMAPProtocol p) throws ProtocolException {

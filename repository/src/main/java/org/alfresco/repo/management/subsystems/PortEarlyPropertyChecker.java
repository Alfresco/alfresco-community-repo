...
/**
 * "Early" checker for Port property value (checks for "null/empty", "out-of-bounds",
 * "unable-to-parse", "already-in-use" problems). Also see the implemented interface {@link
...
public class PortEarlyPropertyChecker implements SubsystemEarlyPropertyChecker {
     ...
     * @param hasMultiplePorts Specify if the property value that will be checked is a list of ports
     *     (they must be separated by ",").
     * @param shouldCheckForBlockedPort Enable/disable checking for port-already-in-use (i.e.:
     *     disable this for remote ports).
     */
    public PortEarlyPropertyChecker(
            String subsystemName, boolean hasMultiplePorts, boolean shouldCheckForBlockedPort) {
     ...
     *
     * @param subsystemName Name of the subsystem; used for custom error messages.
     * @param requiredPairedPropertyName Name of the required paired property (see {@link
    ...
    public void checkPropertyValue(
        ...
        if (propertyValue == null || propertyValue.isEmpty()) {
            createLogAndThrowAnInvalidPropertyValueException(
                    PORT_CANT_BE_EMPTY_MESSAGE, new String[] {subsystemName});
        }
        ...
        try {
            if (!hasMultiplePorts) {
                ...
                if (!portsInUse.equals("")) {
                    String portsInUseDisplayMessage =
                            resolveMessage(
                                    PORTS_IN_USE_MESSAGE, new String[] {subsystemName, portsInUse});
                ...
                if (!portsWithNetworkingErrors.equals("")) {
                    String portsWithNetworkingErrorsDisplayMessage =
                            resolveMessage(
            ...
            createLogAndThrowAnInvalidPropertyValueException(
                    UNABLE_TO_PARSE_PORT_MESSAGE, new String[] {subsystemName, propertyValue});
        }
    }
    ...
    private String appendToErrorString(
            String stringToAppendTo, String separator, String valueToAppend) {
    ...
    private void createLogAndThrowAnInvalidPropertyValueException(
            String message, String[] messageParams) {
        InvalidPropertyValueException invalidPropertyValueException =
                new InvalidPropertyValueException(message, messageParams);

...
/**
 * System test for RM-2190: Concurrency exception when upload document to several folders with rules
 * configured to file records
 *
 * @author Tuna Aksoy
 * @since 2.2.1.1
 */
        ...
        doTestInTransaction(
                new Test<Void>() {
                    ...
                    public Void run() {
                        ...
                        folder1 =
                                fileFolderService
                                        .create(rootFolder, generate(), TYPE_FOLDER)
                                        .getNodeRef();
                        folder2 =
                                fileFolderService
                                        .create(rootFolder, generate(), TYPE_FOLDER)
                    ...
                    public void test(Void result) throws Exception {
                        FileInfo category =
                                fileFolderService.resolveNamePath(
                                        filePlan,
                                        asList(tokenizeToStringArray(PATH, "/", false, true)));
                        assertEquals(
                                NUMBER_IN_BATCH * 2,
                                nodeService.getChildAssocs(category.getNodeRef()).size());
                    }
                });
    }
                            ...
                            new Test<List<NodeRef>>() {
                                ...
                                public List<NodeRef> run() throws Exception {
                                    ...
                                    for (int j = 0; j < NUMBER_IN_BATCH; j++) {
                                        ...
                                        String name =
                                                folder.getId() + " - content" + count + ".txt";
                                        System.out.println(name + " - creating");

                                        NodeRef file =
                                                fileFolderService
                                                        .create(folder, name, TYPE_CONTENT)
                                                        .getNodeRef();
                                        files.add(file);
                                    }
                                    return files;
                        ...
                        public Void run() throws Exception {
                            while (temp.hasNext()) {
                                ...
                                if (nodeService.hasAspect(record, ASPECT_RECORD)
                                    ...
                                    String name =
                                            (String) nodeService.getProperty(record, PROP_NAME);

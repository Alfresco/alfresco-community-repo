...
public class BulkUpdatePropertiesCustomTest extends AbstractSessionTest {
  ...
  public void init(Map<String, String> parameters) {
    ...
    setDescription(
      "Creates a few folders and documents,bulk update the description and check that it has been updated, and deletes all created objects."
    );
  }
  ...
  public void run(Session session) {
    ...
    try {
      ...
      List<BulkUpdateObjectIdAndChangeToken> updatedIds = session.bulkUpdateProperties(
        objects,
        properties,
        null,
        null
      ...
      if (getBinding() == BindingType.WEBSERVICES) {
        addResult(
          createResult(
            INFO,
            "The Web Services binding does not return the updated ids." +
        ...
        if (updatedIds == null || updatedIds.isEmpty()) {
          ...
          addResult(
            assertEquals(documents.size(), updatedIds.size(), null, failure)
          );
        }
      }

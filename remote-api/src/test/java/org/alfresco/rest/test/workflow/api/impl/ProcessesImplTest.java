...
public class ProcessesImplTest
  ...
  private static final String[] CONFIG_LOCATIONS = new String[ApplicationContextHelper.CONFIG_LOCATIONS.length +
  2];

  static {
    System.arraycopy(
      ApplicationContextHelper.CONFIG_LOCATIONS,
      0,
      CONFIG_LOCATIONS,
      0,
      ApplicationContextHelper.CONFIG_LOCATIONS.length
    );
  ...
  private static final String DESIRED_WORKFLOW_ID_PREFIX =
    "activiti$activitiReview";

  private static final String QUERY_STATUS_ACTIVE = "(status=active)";

  private static final String QUERY_WORKFLOWDESCRIPTION_MATCHES =
    "(variables/bpm_workflowDescription MATCHES ('%s'))";
  ...
  public void setUp() throws Exception {
    ...
    NodeRef adminUserNodeRef = personService.getPerson(
      AuthenticationUtil.getAdminUserName()
    );
  ...
  private void startWorkflow(
    WorkflowDefinition neededDefinition,
    NodeRef assignee
  ) {
    Map<QName, Serializable> parameters = new HashMap<QName, Serializable>();
    parameters.put(
      WorkflowModel.ASSOC_ASSIGNEE,
      (Serializable) Collections.singletonList(assignee)
    );
    parameters.put(
      WorkflowModel.PROP_WORKFLOW_DESCRIPTION,
      "Test workflow api calls review and approve"
    ); // MNT-14631
  ...
  public void testHasMoreItemsTrue() throws Exception {
    CollectionWithPagingInfo<ProcessInfo> actualProcesses = queryActiveProcessesAndAssertResult(
      0,
      (ACTIVE_WORKFLOWS_INITIAL_AMOUNT - 5)
    );
    assertTrue(actualProcesses.hasMoreItems());

    actualProcesses =
      queryActiveProcessesAndAssertResult(
        10,
        (ACTIVE_WORKFLOWS_INITIAL_AMOUNT - 15)
      );
    assertTrue(actualProcesses.hasMoreItems());
  }
  ...
  public void testHasMoreFalseAsPerMnt13567() throws Exception {
    ...
    actualProcesses =
      queryActiveProcessesAndAssertResult(
        ACTIVE_WORKFLOWS_INITIAL_AMOUNT + 1,
        ACTIVE_WORKFLOWS_INITIAL_AMOUNT
      );
    assertFalse(actualProcesses.hasMoreItems());
  }
  ...
  public void testGetProcessesMatchesIgnoreCase() {
    CollectionWithPagingInfo<ProcessInfo> result = queryMatchesProcesses(
      "(?i)test workflow api calls review and approve"
    );
  ...
  private CollectionWithPagingInfo<ProcessInfo> queryMatchesProcesses(
    String matchesString
  ) {
    Query query = getWhereClause(
      String.format(QUERY_WORKFLOWDESCRIPTION_MATCHES, matchesString)
    );
    Parameters parameters = Params.valueOf(
      new RecognizedParams(
        null,
        Paging.valueOf(0, ACTIVE_WORKFLOWS_INITIAL_AMOUNT),
        null,
        null,
        null,
        null,
  ...
  private CollectionWithPagingInfo<ProcessInfo> queryActiveProcessesAndAssertResult(
    int skipCount,
    int maxItems
  ) {
    ...
    Parameters parameters = Params.valueOf(
      new RecognizedParams(
        null,
        Paging.valueOf(skipCount, maxItems),
        null,
        null,
        null,
        null,
    ...
    CollectionWithPagingInfo<ProcessInfo> result = processes.getProcesses(
      parameters
    );
    ...
    int remainingProcessesAmount = skipCount > ACTIVE_WORKFLOWS_INITIAL_AMOUNT
      ? 0
      : ACTIVE_WORKFLOWS_INITIAL_AMOUNT - skipCount;

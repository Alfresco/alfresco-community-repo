...
/**
 * A {@link CannedQueryFactory} for various queries relating to {@link CalendarEntry calendar
 * entries}.
 *
 * @author Nick Burch
 * @since 4.0
...
public class GetCalendarEntriesCannedQueryFactory
    ...
    public CannedQuery<CalendarEntry> getCannedQuery(CannedQueryParameters parameters) {
        final GetCalendarEntriesCannedQuery cq =
                new GetCalendarEntriesCannedQuery(
                        cannedQueryDAO, nodeService, taggingService, methodSecurity, parameters);

        return (CannedQuery<CalendarEntry>) cq;
    }

    public CannedQuery<CalendarEntry> getCannedQuery(
            NodeRef[] containerNodes, Date fromDate, Date toDate, PagingRequest pagingReq) {
        ...
        GetCalendarEntriesCannedQueryParams paramBean =
                new GetCalendarEntriesCannedQueryParams(
                        containerIds,
    ...
    protected CannedQuerySortDetails createCQSortDetails() {
        ...
        List<Pair<? extends Object, SortOrder>> sort =
                new ArrayList<Pair<? extends Object, SortOrder>>();

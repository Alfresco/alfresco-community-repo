...
public class DescriptorStartupLog extends AbstractLifecycleBean {
  ...
  protected void onBootstrap(ApplicationEvent event) {
    ...
    String version = (properties.get("java.runtime.version") == null)
      ? "unknown"
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
        String.format(
          "Alfresco JVM - v%s; maximum heap size %.3fMB",
          version,
          maxHeapMB
        )
      );
    ...
    if (logger.isWarnEnabled()) {
      ...
      if (maxHeapMB < 500) {
        logger.warn(
          String.format(
            "Alfresco JVM - WARNING - maximum heap size %.3fMB is less than recommended 512MB",
            maxHeapMB
          )
        );
      }

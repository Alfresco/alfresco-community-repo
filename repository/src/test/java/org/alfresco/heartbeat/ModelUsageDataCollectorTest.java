...
public class ModelUsageDataCollectorTest {
  ...
  public void setUp() {
    ...
    when(customModelService.getCustomModelsInfo())
      .thenReturn(mockCustomModelsInfo);

    TransactionService mockTransactionService = mock(TransactionService.class);
    RetryingTransactionHelper mockRetryingTransactionHelper = mock(
      RetryingTransactionHelper.class
    );
    when(
      mockRetryingTransactionHelper.doInTransaction(
  ...
  public void testModelUsageDataIsCollected() {
    HBData modelUsage = grabDataByCollectorId(
      usageModelCollector.getCollectorId()
    );
  ...
  private HBData grabDataByCollectorId(String collectorId) {
    for (HBData d : this.collectedData) {
      if (
        d.getCollectorId() != null && d.getCollectorId().equals(collectorId)
      ) {
        return d;
      }
    }

...
public class LinksServiceImpl implements LinksService {
    ...
    public LinkInfo createLink(
            String siteShortName, String title, String description, String url, boolean internal) {
        ...
        String name = "link-" + (new Date()).getTime() + "-" + Math.round(Math.random() * 10000);
    ...
    public LinkInfo updateLink(LinkInfo link) {
        ...
        if (link.getNodeRef() == null) {
            throw new IllegalArgumentException(
                    "Can't update a link that was never persisted, call create instead");
        }
    ...
    public void deleteLink(LinkInfo link) {
        if (link.getNodeRef() == null) {
            throw new IllegalArgumentException(
                    "Can't delete a link entry that was never persisted");
        }
    ...
    @Override
    public PagingResults<LinkInfo> listLinks(
            String siteShortName, String user, PagingRequest paging) {
        return listLinks(siteShortName, user, null, null, paging);
    }

    @Override
    public PagingResults<LinkInfo> listLinks(
            String siteShortName, Date from, Date to, PagingRequest paging) {
        return listLinks(siteShortName, null, from, to, paging);
    }

    private PagingResults<LinkInfo> listLinks(
            String siteShortName, String user, Date from, Date to, PagingRequest paging) {
        NodeRef container = getSiteLinksContainer(siteShortName, false);
        if (container == null) {
            // No events
    ...
    @Override
    public PagingResults<LinkInfo> findLinks(
        ...
        luceneQuery.append(
                " +PATH:\""
                        + nodeService.getPath(container).toPrefixString(namespaceService)
                        + "/*\"");
    ...
    private PagingResults<LinkInfo> wrap(
        ...
        for (ResultSetRow row : finalLuceneResults) {
            LinkInfo link = buildLink(row.getNodeRef(), container, row.getQName().getLocalName());
            links.add(link);
            cnt++;
    ...
    /** Our class to wrap up paged results of NodeBackedEntities as LinkInfo instances */
    private PagingResults<LinkInfo> wrap(

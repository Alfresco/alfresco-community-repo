    ...
    private void executeAction(final List<NodeRef> actionNodes)
    {
        RetryingTransactionCallback<Boolean> processTranCB = () -> {
            for (NodeRef actionNode : actionNodes)
            {
                ...
                {
                    // TODO maybe just `continue` to the next *actionNode* instead of throwing an Exception

                    throw new AlfrescoRuntimeException("unable to perform action " + dispAction +

...
public class SolrFacetConfigAdminPut
  ...
  private static final Log logger = LogFactory.getLog(
    SolrFacetConfigAdminPost.class
  );
  ...
  protected Map<String, Object> unprotectedExecuteImpl(
    ...
    try {
      if (relativePosString != null) {
        ...
        try {
          ...
          throw new WebScriptException(
            ...
            "Cannot move facet as could not parse relative position: '" +
            relativePosString +
            "'"
          );
        }
        ...
        try {
          ...
          List<String> facetIDs = CollectionUtils.transform(
            facets,
          ...
          List<String> reorderedIDs = CollectionUtils.moveRight(
            relativePos,
            filterId,
            facetIDs
          );
          ...
          throw new WebScriptException(
            ...
            "Unrecognised filter ID: " + ufi.getFacetId()
          );
        }
      }
      ...
      throw new WebScriptException(
        Status.STATUS_BAD_REQUEST,
        "Could not update the facet configuration.",
        t
      );
    }
  ...
  private SolrFacetProperties parseRequestForFacetProperties(
    WebScriptRequest req
  ) {
    ...
    try {
      ...
      final String facetQNameStr = getValue(
        String.class,
        json.opt(PARAM_FACET_QNAME),
        null
      );
      ...
      final QName facetQName = (facetQNameStr == null)
        ? null
        : FacetQNameUtils.createQName(facetQNameStr, namespaceService);
      final String displayName = getValue(
        String.class,
        json.opt(PARAM_DISPLAY_NAME),
        null
      );
      final String displayControl = getValue(
        String.class,
        json.opt(PARAM_DISPLAY_CONTROL),
        null
      );
      final int maxFilters = getValue(
        Integer.class,
        json.opt(PARAM_MAX_FILTERS),
        -1
      );
      final int hitThreshold = getValue(
        Integer.class,
        json.opt(PARAM_HIT_THRESHOLD),
        -1
      );
      final int minFilterValueLength = getValue(
        Integer.class,
        json.opt(PARAM_MIN_FILTER_VALUE_LENGTH),
        -1
      );
      final String sortBy = getValue(
        String.class,
        json.opt(PARAM_SORT_BY),
        null
      );
      final String scope = getValue(String.class, json.opt(PARAM_SCOPE), null);
      final Boolean isEnabled = getValue(
        Boolean.class,
        json.opt(PARAM_IS_ENABLED),
        null
      );
      JSONArray scopedSitesJsonArray = getValue(
        JSONArray.class,
        json.opt(PARAM_SCOPED_SITES),
        null
      );
      final Set<String> scopedSites = getScopedSites(scopedSitesJsonArray);
      final JSONObject customPropJsonObj = getValue(
        JSONObject.class,
        json.opt(PARAM_CUSTOM_PROPERTIES),
        null
      );
      final Set<CustomProperties> customProps = getCustomProperties(
        customPropJsonObj
      );

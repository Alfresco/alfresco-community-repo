...
public class CopyTests extends CmisTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify site manager is able to copy file to an existent location in"
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Verify site manager is able to copy folder to an existent location in"
                            + " DocumentLibrary with CMIS")
    ...
    @TestRail(
            ...
            description =
                    "Verify site manager is not able to copy file to a nonexistent location in"
                            + " DocumentLibrary with CMIS")
    @Test(
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify site manager is not able to copy a nonexistent file in DocumentLibrary"
                            + " with CMIS")
    @Test(
            groups = {TestGroup.REGRESSION, TestGroup.CMIS},
            expectedExceptions = CmisObjectNotFoundException.class)
    ...
    public void contributorCanCopyDocument() throws Exception {
        sourceFile =
                dataContent
                        ...
                        .createContent(FileModel.getRandomFileModel(FileType.TEXT_PLAIN));
        targetFolder = dataContent.usingUser(testUser).usingSite(testSite).createFolder();
        cmisApi.authenticateUser(contributorUser)
                .usingResource(sourceFile)
                .copyTo(targetFolder)
                .usingResource(targetFolder)
                .assertThat()
    ...
    public void collaboratorCanCopyDocument() throws Exception {
        sourceFile =
                dataContent
                        ...
                        .createContent(FileModel.getRandomFileModel(FileType.TEXT_PLAIN));
        targetFolder = dataContent.usingUser(testUser).usingSite(testSite).createFolder();
        cmisApi.authenticateUser(collaboratorUser)
                .usingResource(sourceFile)
                .copyTo(targetFolder)
                .usingResource(targetFolder)
                .assertThat()
    ...
    public void consumerCannotCopyDocument() throws Exception {
        sourceFile =
                dataContent
                        ...
                        .createContent(FileModel.getRandomFileModel(FileType.TEXT_PLAIN));
        targetFolder = dataContent.usingUser(testUser).usingSite(testSite).createFolder();
        cmisApi.authenticateUser(consumerUser)
                .usingResource(sourceFile)
                .copyTo(targetFolder)
                .usingResource(targetFolder)
                .assertThat()

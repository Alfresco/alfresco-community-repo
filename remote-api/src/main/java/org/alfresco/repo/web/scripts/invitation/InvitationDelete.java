...
public class InvitationDelete extends DeclarativeWebScript {
  ...
  private void validateParameters(String siteShortName, String invitationId) {
    if ((invitationId == null) || (invitationId.length() == 0)) {
      throw new WebScriptException(
        ...
        "Invalid invitation id provided"
      );
    }
  ...
  protected void checkAndCancelTheInvitation(
    final String invId,
    ...
    try {
      ...
      org.alfresco.service.cmr.invitation.InvitationExceptionNotFound ienf
    ) {
    ...
    if (
      invitation != null &&
      invitation.getResourceName() != null &&
      !siteShortName.equals(invitation.getResourceName())
    ) {
      throw new WebScriptException(
        Status.STATUS_FORBIDDEN,
        "Unable to cancel workflow"
  ...
  protected void throwInvitationNotFoundException(
    final String invId,
    String siteShortName
  ) {
    throw new WebScriptException(

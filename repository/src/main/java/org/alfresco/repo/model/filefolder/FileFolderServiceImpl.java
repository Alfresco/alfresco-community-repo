 ...
 * @author Derek Hulley
 */
public class FileFolderServiceImpl extends AbstractBaseCopyService
    ...
    // TODO: Replace this with a more formal means of identifying "system" folders (i.e. aspect or
    // UUID)
    ...
    public FileFolderServiceImpl() {
        ...
        fileFolderTrait =
                new ExtendedTrait<FileFolderServiceTrait>(
                        AJProxyTrait.create(createFileFolderTrait(), FileFolderServiceTrait.class));
    }
    ...
    public void setCannedQueryRegistry(
            NamedObjectRegistry<CannedQueryFactory<NodeRef>> cannedQueryRegistry) {
    ...
    @Override
    @Extend(
            traitAPI = FileFolderServiceTrait.class,
            extensionAPI = FileFolderServiceExtension.class)
    ...
    private PagingResults<FileInfo> getPagingResults(
            PagingRequest pagingRequest, final CannedQueryResults<NodeRef> results) {
    ...
    public PagingResults<FileInfo> list(
        ...
        // execute query
        final CannedQueryResults<NodeRef> results =
                listImpl(
                        contextNodeRef,
                        null,
                        searchTypeQNames,
    ...
    private CannedQueryResults<NodeRef> listImpl(
            NodeRef contextNodeRef, boolean files, boolean folders) {
        Set<QName> searchTypeQNames =
                buildSearchTypesAndIgnoreAspects(files, folders, null).getFirst();
        return listImpl(contextNodeRef, searchTypeQNames);
    }

    private CannedQueryResults<NodeRef> listImpl(
            NodeRef contextNodeRef, Set<QName> searchTypeQNames) {
        return listImpl(
                contextNodeRef,
                null,
                searchTypeQNames,
                null,
                null,
                new PagingRequest(defaultListMaxResults, null));
    }

    // note: similar to getChildAssocs(contextNodeRef, searchTypeQNames) but enables paging
    // features, including max items, sorting etc (with permissions per-applied)
    ...
    private CannedQueryResults<NodeRef> listImpl(
        ...
        GetChildrenCannedQueryFactory getChildrenCannedQueryFactory =
                (GetChildrenCannedQueryFactory)
                        cannedQueryRegistry.getNamedObject(CANNED_QUERY_FILEFOLDER_LIST);
        ...
        if (start != null) {
            ...
            Pair<Integer, Integer> totalCount =
                    (pagingRequest.getRequestTotalCountMax() > 0
    ...
    public NodeRef searchSimple(NodeRef contextNodeRef, String name) {
        ...
        NodeRef childNodeRef =
                nodeService.getChildByName(contextNodeRef, ContentModel.ASSOC_CONTAINS, name);
        if (logger.isTraceEnabled()) {
            logger.trace(
    ...
    public List<FileInfo> search(
        ...
        List<NodeRef> nodeRefs =
                searchInternal(
                        contextNodeRef, namePattern, fileSearch, folderSearch, includeSubFolders);
    ...
    /**
     * Performs a full search, but doesn't translate the node references into file info objects.
     * This allows {@link #checkExists(NodeRef, String)} to bypass the retrieval of node properties.
     */
        ...
        if (anyName) {
            ...
            nodeRefs =
                    searchService.selectNodes(
                            contextNodeRef, query, params, namespaceService, false);
        }
        // done
        return nodeRefs;
    }

    private List<FileInfo> listSimple(NodeRef contextNodeRef, boolean files, boolean folders)
            throws InvalidTypeException {
    ...
    private Pair<Set<QName>, Set<QName>> buildSearchTypesAndIgnoreAspects(
        ...
        if (ignoreQNameTypes != null) {
            ...
            ignoreQNamesNotSearchTypes.remove(
                    ContentModel.TYPE_SYSTEM_FOLDER); // note: not included in buildFolderTypes()
    ...
    /**
     * A deep version of listSimple. Which recursively walks down the tree from a given starting
     * point, returning the node refs of files or folders found along the way.
     *
     * <p>The folder filter is called for each sub-folder to determine whether to search in that
     * sub-folder, should a subfolder be excluded then all its chidren are excluded as well.
     *
     * @param contextNodeRef the starting point.
     * @param files return nodes of type files.
     * @param folders return nodes of type folders.
     * @param folderFilter filter controls which folders to search. If null then all subfolders are
     *     searched.
     ...
     */
    private List<NodeRef> listSimpleDeep(
            NodeRef contextNodeRef, boolean files, boolean folders, SubFolderFilter folderFilter) {
        ...
        while (!toSearch.empty()) {
            ...
            List<ChildAssociationRef> folderAssocRefs =
                    nodeService.getChildAssocs(currentDir, folderTypeQNames);
            ...
            if (files) {
                ...
                List<ChildAssociationRef> fileAssocRefs =
                        nodeService.getChildAssocs(currentDir, fileTypeQNames);
    ...
    private FileInfo moveOrCopy(
        ...
        AssociationCopyInfo targetInfo =
                getAssociationCopyInfo(
                        nodeService, sourceNodeRef, sourceParentRef, newName, nameChanged);
        ...
        if (!nameChanged && !changedParent) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Doing nothing - neither filename or parent has changed: \n"
                                + "   parent: "
                                + targetParentRef
                                + "\n"
                                + "   before: "
        ...
        if (nameChanged && move) {
            ...
            if (dictionaryService.isSubClass(targetParentType, ContentModel.TYPE_FOLDER)) {
                assocTypeQname = ContentModel.ASSOC_CONTAINS; // cm:folder -> cm:contains
                    ...
                    targetParentType, ContentModel.TYPE_CONTAINER)) {
        ...
        if (move) {
            // TODO: Replace this with a more formal means of identifying "system" folders (i.e.
            // aspect or UUID)
            if (!isSystemPath(sourceNodeRef)) {
                // The cm:name might clash with another node in the target location.
                if (nameChanged) {
                    // The name will be changing, so we really need to set the node's name to the
                    // new
                    // name.  This can't be done at the same time as the move - to avoid incorrect
                    // violations
                    // of the name constraints, the cm:name is set to something random and will be
                    // reset
                        ...
                        try {
                            newAssocRef =
                                    nodeService.moveNode(
                                            sourceNodeRef, targetParentRef, assocTypeQname, qname);
                        } finally {
                            if (auditableBehaviorWasDisabled) {
            ...
            try {
                ...
                targetNodeRef =
                        copyService.copy(
                                sourceNodeRef, targetParentRef, assocTypeQname, qname, true);
            } catch (DuplicateChildNodeNameException e) {
                throw new FileExistsException(targetParentRef, newName);
            }
        }
        ...
        String currentName =
                (String) nodeService.getProperty(targetNodeRef, ContentModel.PROP_NAME);

        // ALF-13949: WorkingCopyAspect intentionally generates new names for all copies of working
        // copies (which no
        // longer have the working copy aspect) so leave these alone after copy
        if (!currentName.equals(newName)
                && (move
                        || !nodeService.hasAspect(
                                sourceNodeRef, ContentModel.ASPECT_WORKING_COPY))) {
            try {
                ...
                ContentData contentData =
                        (ContentData)
                                nodeService.getProperty(targetNodeRef, ContentModel.PROP_CONTENT);
                ...
                if (contentData != null
                    ...
                    final ContentReader reader =
                            contentService.getReader(targetNodeRef, ContentModel.PROP_CONTENT);
                    ...
                    if (!targetMimetype.equalsIgnoreCase(newMimetype)) {
                        ...
                        nodeService.setProperty(
                                targetNodeRef, ContentModel.PROP_CONTENT, contentData);
                    }
                }
            } catch (DuplicateChildNodeNameException e) {
                throw new FileExistsException(targetParentRef, newName);
     ...
     *
     * <p>TODO: Replace this with a more formal means of identifying "system" folders (i.e. aspect
     * or UUID)
     *
    ...
    private FileInfo createImpl(
            NodeRef parentNodeRef, String name, QName typeQName, QName assocQName)
            throws FileExistsException {
        ...
        switch (type) {
            case SYSTEM_FOLDER:
                throw new InvalidTypeException(
                        "System Folders are not handled by this service :" + typeQName);
            case INVALID:
    ...
    /**
     * Checks for the presence of, and creates as necessary, the folder structure in the provided
     * path.
     *
     * <p>An empty path list is not allowed as it would be impossible to necessarily return file
     * info for the parent node - it might not be a folder node.
     *
     ...
     * @param folderTypeQName the types of nodes to create. This must be a valid subtype of {@link
     *     org.alfresco.model.ContentModel#TYPE_FOLDER they folder type}.
     * @return Returns the info of the last folder in the path.
     * @deprecated Use FileFolderUtil.makeFolders rather than directly accessing this implementation
     *     class.
     */
    public FileInfo makeFolders(
            NodeRef parentNodeRef, List<String> pathElements, QName folderTypeQName) {
     ...
     *     class.
     */
    public static FileInfo makeFolders(
    ...
    public List<FileInfo> getNamePath(NodeRef rootNodeRef, NodeRef nodeRef)
        ...
        try {
            ...
            for (Path.Element element : path) {
                ...
                // Run as system as the user could not have access to all folders in the path, see
                // ALF-13816
                FileInfo pathInfo =
                        AuthenticationUtil.runAs(
                                new AuthenticationUtil.RunAsWork<FileInfo>() {
                ...
                // we can't append a path element to the results if there is already a (non-folder)
                // file at the tail
                ...
                if (!results.isEmpty() && !results.get(results.size() - 1).isFolder()) {
                    throw new InvalidTypeException(
                            "File is not the last element in path: files cannot contain other"
                                    + " files.");
                }
     ...
     */
    @Override
    public List<String> getNameOnlyPath(NodeRef rootNodeRef, final NodeRef nodeRef)
            throws FileNotFoundException {
        // check the root
        if (rootNodeRef == null) {
            rootNodeRef = nodeService.getRootNode(nodeRef.getStoreRef());
        ...
        try {
            ...
            AuthenticationUtil.runAs(
                    new AuthenticationUtil.RunAsWork<Void>() {
                        public Void doWork() throws Exception {
                            ...
                            for (Path.Element element : path) {
                                // ignore everything down to the root
                                Path.ChildAssocElement assocElement =
                                        (Path.ChildAssocElement) element;
                                final NodeRef childNodeRef = assocElement.getRef().getChildRef();
                                ...
                                results.add(
                                        nodeService
                                                .getProperty(childNodeRef, ContentModel.PROP_NAME)
                                                .toString());
                            }
                            // check that we found the root
                            if (!foundRoot) {
            ...
            // the runAs() is too keen on wrapping everything in an outer RuntimeException - which
            // we don't want.
    ...
    public ContentReader getReader(NodeRef nodeRef) {
        ...
        if (fileInfo.isFolder()) {
            throw new InvalidTypeException(
                    "Unable to get a content reader for a folder: " + fileInfo);
        }
    ...
    public ContentWriter getWriter(NodeRef nodeRef) {
        ...
        if (fileInfo.isFolder()) {
            throw new InvalidTypeException(
                    "Unable to get a content writer for a folder: " + fileInfo);
        }
        ...
        for (Client client : hiddenAspect.getClients()) {
            Visibility clientVisibility =
                    client == FileFilterMode.getClient()
                            ? webDavVisibility
                            : hiddenAspect.getVisibility(client, nodeRef);
    ...
    public boolean isHidden(NodeRef nodeRef) {
        return hiddenAspect.getVisibility(FileFilterMode.getClient(), nodeRef)
                != Visibility.Visible;
    }
    ...
    public static class FileFolderServiceTraitImpl implements FileFolderServiceTrait {
        ...
        @Override
        public Pair<Set<QName>, Set<QName>> buildSearchTypesAndIgnoreAspects(
                boolean files, boolean folders, Set<QName> ignoreQNameTypes) {
            return thisService.buildSearchTypesAndIgnoreAspects(files, folders, ignoreQNameTypes);
        }
        ...
        public PagingResults<FileInfo> list(
                ...
                final PagingRequest pagingRequest) {
            return thisService.list(
                    rootNodeRef, searchTypeQNames, ignoreAspectQNames, sortProps, pagingRequest);
        }
        ...
        public List<FileInfo> search(
            ...
            return thisService.search(
                    contextNodeRef, namePattern, fileSearch, folderSearch, includeSubFolders);
        }

        @Override
        public FileInfo rename(final NodeRef sourceNodeRef, final String newName)
                throws FileExistsException, FileNotFoundException {
        ...
        public PagingResults<FileInfo> list(
                ...
                PagingRequest pagingRequest) {
            return thisService.list(
                    contextNodeRef, files, folders, ignoreQNames, sortProps, pagingRequest);
        }
    }
    ;
}

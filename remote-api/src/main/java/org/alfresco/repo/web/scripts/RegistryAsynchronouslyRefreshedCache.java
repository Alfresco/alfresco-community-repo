 ...
 *
 * <p>This does not stop gratuitous calls to <i>refresh</i> but will ensure that, once an instance
 * has been created, a version of the registry is returned even if it is slighly out of date. This
 * can be changed so that it waits for reset but is probably not required.
 *
 * @author Derek Hulley
 * @since 4.2.0
 */
public class RegistryAsynchronouslyRefreshedCache
        extends AbstractMTAsynchronouslyRefreshedCache<Registry> implements InitializingBean {
    ...
    protected Registry buildCache(final String tenantId) {
        return retryingTransactionHelper.doInTransaction(
                new RetryingTransactionCallback<Registry>() {
                    @Override

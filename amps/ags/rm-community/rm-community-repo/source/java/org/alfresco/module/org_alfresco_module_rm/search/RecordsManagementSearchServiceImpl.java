...
public class RecordsManagementSearchServiceImpl
  ...
  private static final String SITES_SPACE_QNAME_PATH =
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.length(); i++) {
        ...
        if (!report.has(SavedSearchDetails.SEARCH)) {
          throw new AlfrescoRuntimeException(
            "Unable to load report details because search has not been specified for report " +
            name +
            ". \n" +
            reportsJSON
          );
        ...
        if (report.has("searchparams")) {
          recordsManagementSearchParameters =
            RecordsManagementSearchParameters.createFromJSON(
              report.getJSONObject("searchparams"),
              namespaceService
            );
        }
  ...
  public List<Pair<NodeRef, NodeRef>> search(
    String siteId,
    String query,
    RecordsManagementSearchParameters rmSearchParameters
  ) {
    ...
    searchParameters.setMaxItems(
      recordsManagementSearchParameters.getMaxItems()
    );
    ...
    for (SortItem entry : rmSearchParameters.getSortOrder()) {
      searchParameters.addSort(
        entry.property.toPrefixString(namespaceService),
        entry.assc
      );
    }

    // set templates
    for (Entry<String, String> entry : rmSearchParameters
      .getTemplates()
      .entrySet()) {
    ...
    for (ChildAssociationRef childAssoc : resultSet.getChildAssocRefs()) {
      result.add(
        new Pair<>(childAssoc.getParentRef(), childAssoc.getChildRef())
      );
    }
  ...
  /*package*/String buildQueryString(
    ...
    if (includedContainerTypes != null && includedContainerTypes.size() != 0) {
      for (QName includedContainerType : includedContainerTypes) {
        appendType(
          typeQuery,
          includedContainerType.toPrefixString(namespaceService)
        );
      }
    }
  ...
  public List<SavedSearchDetails> getSavedSearches(String siteId) {
    ...
    if (container != null) {
      ...
      NodeRef userContainer = fileFolderService.searchSimple(
        container,
        userName
      );
      if (userContainer != null) {
        List<FileInfo> userSearches = fileFolderService.listFiles(
          userContainer
        );
   ...
   */
  private void addSearchDetailsToList(
    ...
    SavedSearchDetails savedSearchDetails = SavedSearchDetails.createFromJSON(
      jsonString,
      namespaceService,
      this,
      searchNode
  ...
  @Override
  public SavedSearchDetails saveSearch(
    final SavedSearchDetails savedSearchDetails
  ) {
    // Check for mandatory parameters
            ...
            new RunAsWork<NodeRef>() {
              ...
              public NodeRef doWork() {
                return fileFolderService
                  .create(parentContainer, userName, ContentModel.TYPE_FOLDER)
                  .getNodeRef();
              }
            },
            AuthenticationUtil.getSystemUserName()
    ...
    // Get the saved search node
    NodeRef searchNode = fileFolderService.searchSimple(
      container,

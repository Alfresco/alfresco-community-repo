...
public class CryptodocSwitchableApplicationContextFactoryTest {
    ...
    public void setUp() throws Exception {
        ...
        when(parentContext.getBean(
                        UNENCRYPTED_STORE_SUBSYSTEM, ChildApplicationContextFactory.class))
                .thenReturn(unencrytedContentStore);

        when(parentContext.containsBean(NEW_UNENCRYPTED_STORE_SUBSYSTEM)).thenReturn(true);
        when(parentContext.getBean(
                        NEW_UNENCRYPTED_STORE_SUBSYSTEM, ChildApplicationContextFactory.class))
        ...
        when(parentContext.getBean(ENCRYPTED_STORE_SUBSYSTEM, ChildApplicationContextFactory.class))
        ...
        when(parentContext.getBean(NEW_STORE_SUBSYSTEM, ChildApplicationContextFactory.class))
    ...
    public void canSwitchFromUnencryptedToEncrypted_Supported() {
        ...
        DescriptorServiceAvailableEvent event =
                new DescriptorServiceAvailableEvent(descriptorService);
    ...
    public void cannotSwitchFromUnencryptedToEncrypted_NotSupported() {
        ...
        try {
            switchableContext.setProperty(SOURCE_BEAN_NAME_PROPERTY, ENCRYPTED_STORE_SUBSYSTEM);
            fail(
                    "It shouldn't be possible to switch to an encrypted content store when the"
                            + " license doesn't support it.");
    ...
    public void cannotSwitchFromEncryptedToUnencrypted() {
        ...
        try {
            ...
            fail(
                    "It shouldn't be possible to switch to an unencrypted content store from an"
                            + " encrypted one.");
        } catch (IllegalStateException e) {
            // expected
        }

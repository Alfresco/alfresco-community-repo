...
public class EncodingDAOTest extends TestCase {
    ...
    try {
      ...
      if (expectSuccess) {
        ...
        throw new RuntimeException(
          "Expected to get encoding '" + encoding + "'.",
          e
        );
      } else {
        return null;
  ...
  public void testCreateWithCommit() throws Exception {
    ...
    Pair<Long, String> encodingPairCheck = get(
      encodingPair.getSecond(),
      false,
      true
    );
    assertEquals(
      "Encoding ID changed",
      encodingPair.getFirst(),
      encodingPairCheck.getFirst()
    );
  }

...
/**
 * This class is responsible for the persistence of objects using lower-level repo services such as
 * the {@link NodeService}. The higher-level business logic around these CRUD calls is contained
 * within the {@link DownloadServiceImpl}.
 *
 * @author Alex Miller
 */
public class DownloadStorage {
    ...
    /**
     * This method finds the SyncSet Definition Container NodeRef, creating one if it does not
     * exist.
     *
    ...
    public NodeRef getOrCreateDowloadContainer() {
        ...
        if (downloadsContainer == null) {
            if (log.isInfoEnabled()) log.info("Lazy creating the Downloads System Container ");
    ...
    private QName getContainerQName() {
        String name =
                bootstrap.getConfiguration().getProperty("system.downloads_container.childname");
    ...
    public NodeRef createDownloadNode(boolean recursive) {
        ...
        ChildAssociationRef newChildAssoc =
                noPermissionCheckNodeService.createNode(
                        downloadsContainer,
        ...
        // MNT-11911 fix, add ASPECT_INDEX_CONTROL and property that not create indexes for search
        // and not visible files/folders at 'My Documents' dashlet
    ...
    public DownloadStatus getDownloadStatus(NodeRef downloadNodeRef) {
        ...
        return new DownloadStatus(
                DownloadStatus.Status.valueOf((String) properties.get(DownloadModel.PROP_STATUS)),
    ...
    public int getSequenceNumber(NodeRef nodeRef) {
        ...
        Serializable sequenceNumber =
                nodeService.getProperty(nodeRef, DownloadModel.PROP_SEQUENCE_NUMBER);
    ...
    public List<List<DownloadEntity>> getDownloadsCreatedBefore(
            Date before, int batchSize, boolean cleanAllSysDownloadFolders) {
        ...
        for (NodeRef folderToBeCleaned : allFoldersToBeCleaned) {
            gatherDownloadFilesToBeCleanedFromFolder(
                    childDownloads, before, batchSize, folderToBeCleaned);
        }
    ...
    private void gatherDownloadFilesToBeCleanedFromFolder(
        ...
        GetDownloadsCannedQueryFactory getDownloadCannedQueryFactory =
                (GetDownloadsCannedQueryFactory)
                        queryRegistry.getNamedObject("downloadGetDownloadsCannedQueryFactory");

        // Run the canned query
        GetDownloadsCannedQuery cq =
                (GetDownloadsCannedQuery)
                        getDownloadCannedQueryFactory.getDownloadsCannedQuery(
                                folderToBeCleaned, before);
    ...
    private List<NodeRef> getAllFoldersToBeCleaned(boolean cleanAllSysDownloadFolders) {
        ...
        if (cleanAllSysDownloadFolders) {
            for (NodeRef nodeRef :
                    SystemNodeUtils.getSystemChildContainers(
                            getContainerQName(), nodeService, repositoryHelper)) {
                if (nodeRef != null) {

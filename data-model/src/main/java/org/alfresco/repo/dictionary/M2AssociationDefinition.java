...
/*package*/ class M2AssociationDefinition implements AssociationDefinition {
     ...
     */
    /*package*/ M2AssociationDefinition(
            ClassDefinition classDef, M2ClassAssociation assoc, NamespacePrefixResolver resolver) {
    ...
    /*package*/ void resolveDependencies(ModelQuery query) {
        if (targetClassName == null) {
            throw new DictionaryException(
                    "d_dictionary.association.target_class_not_specified", name.toPrefixString());
        }
        targetClass = query.getClass(targetClassName);
        if (targetClass == null) {
            throw new DictionaryException(
                    "d_dictionary.association.target_class_not_found",
                    targetClassName.toPrefixString(),
                    name.toPrefixString());
        }
    }
    ...
    public String getTitle(MessageLookup messageLookup) {
        String value =
                M2Label.getLabel(classDef.getModel(), messageLookup, "association", name, "title");
        if (value == null) {
    ...
    public String getDescription(MessageLookup messageLookup) {
        String value =
                M2Label.getLabel(
                        classDef.getModel(), messageLookup, "association", name, "description");
        if (value == null) {
    ...
    /* package */ M2ModelDiff diffAssoc(AssociationDefinition assocDef) {
        ...
        if (this == assocDef) {
            modelDiff =
                    new M2ModelDiff(name, M2ModelDiff.TYPE_ASSOCIATION, M2ModelDiff.DIFF_UNCHANGED);
            return modelDiff;
        }
        ...
        if (!EqualsHelper.nullSafeEquals(
                getDescription(null), assocDef.getDescription(null), false)) {
            isUpdatedIncrementally = true;
        }

        // check source class qname
        if (!EqualsHelper.nullSafeEquals(
                getSourceClass().getName(), assocDef.getSourceClass().getName())) {
            isUpdated = true;
        }
        ...
        if (!EqualsHelper.nullSafeEquals(
                getTargetClass().getName(), assocDef.getTargetClass().getName())) {
            isUpdated = true;
        }
        ...
        if (isUpdated) {
            modelDiff =
                    new M2ModelDiff(name, M2ModelDiff.TYPE_ASSOCIATION, M2ModelDiff.DIFF_UPDATED);
        } else if (isUpdatedIncrementally) {
            modelDiff =
                    new M2ModelDiff(
                            name, M2ModelDiff.TYPE_ASSOCIATION, M2ModelDiff.DIFF_UPDATED_INC);
        } else {
            modelDiff =
                    new M2ModelDiff(name, M2ModelDiff.TYPE_ASSOCIATION, M2ModelDiff.DIFF_UNCHANGED);
        }
    ...
    /*package*/ static Collection<M2ModelDiff> diffAssocLists(
            Collection<AssociationDefinition> previousAssocs,
            Collection<AssociationDefinition> newAssocs) {
        ...
        for (AssociationDefinition previousAssoc : previousAssocs) {
            ...
            if (!found) {
                modelDiffs.add(
                        new M2ModelDiff(
                                previousAssoc.getName(),
                                M2ModelDiff.TYPE_ASSOCIATION,

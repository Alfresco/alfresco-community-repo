...
public class ActivitiUtil {
    ...
    public ProcessDefinition getProcessDefinitionForDeployment(String deploymentId) {
        return repoService.createProcessDefinitionQuery().deploymentId(deploymentId).singleResult();
    }
    ...
    public Task getTaskInstance(String taskId) {
        ...
        if (!deployWorkflowsInTenant) {
            taskQuery.processVariableValueEquals(
                    ActivitiConstants.VAR_TENANT_DOMAIN, TenantUtil.getCurrentDomain());
        }
    ...
    public ReadOnlyProcessDefinition getDeployedProcessDefinition(String processDefinitionId) {
        ...
        return ((RepositoryServiceImpl) repoService)
                .getDeployedProcessDefinition(processDefinitionId);
    }

    public String getStartFormKey(String processDefinitionId) {
        ProcessDefinitionEntity procDef =
                (ProcessDefinitionEntity) getDeployedProcessDefinition(processDefinitionId);
    ...
    public HistoricTaskInstance getHistoricTaskInstance(String localId) {
        HistoricTaskInstanceQuery taskQuery =
                historyService.createHistoricTaskInstanceQuery().taskId(localId);
        if (!deployWorkflowsInTenant) {
            taskQuery.processVariableValueEquals(
                    ActivitiConstants.VAR_TENANT_DOMAIN, TenantUtil.getCurrentDomain());
        }

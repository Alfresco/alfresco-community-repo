...
public class RuleServiceTest extends BaseWebScriptTest {
  ...
  private static final String URL_ACTIONCONDITIONDEFINITIONS =
    "/api/actionconditiondefinitions";

  private static final String URL_ACTIONCONSTRAINTS = "/api/actionConstraints";
  private static final String URL_ACTIONCONSTRAINT =
    "/api/actionConstraints/{0}";
  ...
  private static final String URL_RULE =
    "/api/node/{0}/{1}/{2}/ruleset/rules/{3}";

  private static final String TEST_STORE_IDENTIFIER =
    "test_store-" + System.currentTimeMillis();
  private static final String TEST_FOLDER =
    "test_folder-" + System.currentTimeMillis();
  private static final String TEST_FOLDER_2 =
    "test_folder-2-" + System.currentTimeMillis();
  ...
  protected void setUp() throws Exception {
    ...
    this.ruleService =
      (RuleService) getServer().getApplicationContext().getBean("RuleService");
    this.actionService =
      (ActionService) getServer()
        .getApplicationContext()
  ...
  private void createTestFolders() {
    StoreRef testStore = new StoreRef(
      StoreRef.PROTOCOL_WORKSPACE,
      TEST_STORE_IDENTIFIER
    );
  ...
  private JSONObject createRule(NodeRef ruleOwnerNodeRef, String title)
    throws Exception {
  ...
  private JSONArray getNodeRules(NodeRef nodeRef, boolean inherited)
    throws Exception {
    Response response = sendRequest(
      new GetRequest(formatRulesUrl(nodeRef, inherited)),
      200
    );
    JSONObject result = new JSONObject(response.getContentAsString());
  ...
  private void checkRuleComplete(JSONObject result) throws Exception {
    ...
    assertEquals(
      ...
      "this is description for test_rule"
    );
    ...
    assertEquals(
      jsonAction.getString("description"),
      "this is description for composite-action"
    );
  ...
  private void checkUpdatedRule(JSONObject before, JSONObject after)
    throws JSONException {
    // check saving of basic feilds
    assertEquals(
      "It seams that 'id' is not correct",
      before.getString("id"),
      after.getString("id")
    );
    ...
    assertEquals(
      before.getBoolean("applyToChildren"),
      after.getBoolean("applyToChildren")
    );
    assertEquals(
  ...
  public void testGetActionConditionDefinitions() throws Exception {
    Response response = sendRequest(
      new GetRequest(URL_ACTIONCONDITIONDEFINITIONS),
      200
    );
    JSONObject result = new JSONObject(response.getContentAsString());
  ...
  public void testGetActionConstraint() throws Exception {
    ...
    Response response = sendRequest(
      new GetRequest(formateActionConstraintUrl(name)),
      200
    );
    JSONObject result = new JSONObject(response.getContentAsString());
  ...
  public void testQueueAction() throws Exception {
    ...
    Response successResponse = sendRequest(
      new PostRequest(url, copyAction.toString(), "application/json"),
      200
    );

    JSONObject successResult = new JSONObject(
      successResponse.getContentAsString()
    );
    ...
    Response response = sendRequest(
      new PostRequest(asyncUrl, copyAction.toString(), "application/json"),
      200
    );
  ...
  public void testGetRuleset() throws Exception {
    ...
    String[] parentRuleIds = new String[] {
      parentRule.getJSONObject("data").getString("id"),
    };

    JSONObject jsonRule = createRule(testNodeRef);
    String[] ruleIds = new String[] {
      jsonRule.getJSONObject("data").getString("id"),
    };

    Action linkRulesAction = actionService.createAction(LinkRules.NAME);
    linkRulesAction.setParameterValue(
      LinkRules.PARAM_LINK_FROM_NODE,
      testNodeRef
    );
    actionService.executeAction(linkRulesAction, testNodeRef2);

    Response linkedFromResponse = sendRequest(
      new GetRequest(formatRulesetUrl(testNodeRef)),
      200
    );
    JSONObject linkedFromResult = new JSONObject(
      linkedFromResponse.getContentAsString()
    );

    checkRuleset(linkedFromResult, 1, ruleIds, 1, parentRuleIds, true, false);

    Response linkedToResponse = sendRequest(
      new GetRequest(formatRulesetUrl(testNodeRef2)),
      200
    );
    JSONObject linkedToResult = new JSONObject(
      linkedToResponse.getContentAsString()
    );
  ...
  public void testGetRuleDetails() throws Exception {
    ...
    Response response = sendRequest(
      new GetRequest(formateRuleUrl(testNodeRef, ruleId)),
      200
    );
    JSONObject result = new JSONObject(response.getContentAsString());
  ...
  public void testDeleteRule() throws Exception {
    ...
    Response response = sendRequest(
      new DeleteRequest(formateRuleUrl(testNodeRef, ruleId)),
      200
    );
    JSONObject result = new JSONObject(response.getContentAsString());
  ...
  public void testRuleReorder() throws Exception {
    ...
    // execute before response (should be successful)
    Response successResponse = sendRequest(
      new PostRequest(url, action.toString(), "application/json"),
      200
    );
    JSONObject successResult = new JSONObject(
      successResponse.getContentAsString()
  ...
  private NodeRef createRuleNodeRef(NodeRef folder, String title)
    throws Exception {
  ...
  private JSONObject buildTestCondition(String conditionName)
    throws JSONException {
    JSONObject result = new JSONObject();

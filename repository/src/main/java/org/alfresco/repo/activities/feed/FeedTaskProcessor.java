...
public abstract class FeedTaskProcessor {
    ...
    public void process(int jobTaskNode, long minSeq, long maxSeq, RepoCtx ctx) throws Exception {
        ...
        try {
            ...
            Map<String, Set<String>> siteConnectedUsers =
                    new HashMap<String, Set<String>>(); // site -> site members
            Map<Pair<String, String>, Set<String>> followerConnectedUsers =
                    new HashMap<Pair<String, String>, Set<String>>(); // user -> followers
            Map<Pair<String, String>, Boolean> canUserReadSite =
                    new HashMap<
                            Pair<String, String>,
                            Boolean>(); // <user, site> -> true/false (note: used when following,
            // implied as true for site members)
            Map<String, List<FeedControlEntity>> userFeedControls =
                    new HashMap<String, List<FeedControlEntity>>();

            List<String> fmTemplates =
                    Arrays.asList(new String[] {"activities/org/alfresco/generic.json.ftl"});
            ...
            for (ActivityPostEntity activityPost : activityPosts) {
                ...
                if (fmTemplates.size() == 0) {
                    logger.error(
                            "Skipping activity post "
                                    + activityPost.getId()
                                    + " since no specific/generic templates for activityType: "
                                    + activityType);
                ...
                try {
                    ...
                    logger.error(
                            "Skipping activity post "
                                    + activityPost.getId()
                                    + " due to invalid nodeRef: "
                                    + nodeRefStr);
                    updatePostStatus(activityPost.getId(), ActivityPostEntity.STATUS.ERROR);
                    continue;
                }

                // note: for MT share, site id should already be mangled - in addition to extra
                // tenant domain info
                ...
                model.put(
                        ActivityFeedEntity.KEY_ACTIVITY_FEED_TYPE, activityPost.getActivityType());
                ...
                model.put(
                        "date",
                        activityPost.getPostDate()); // post date rather than time that feed is
                // generated
                ...
                try {
                    recipients =
                            getRecipients(
                                    ...
                                    tenantDomain,
                                    siteConnectedUsers,
                                    followerConnectedUsers,
                                    canUserReadSite);
                } catch (Exception e) {
                    logger.trace(
                            "Skipping activity post "
                                    + activityPost.getId()
                                    + " since failed to get recipients: ",
                            e);
                    updatePostStatus(activityPost.getId(), ActivityPostEntity.STATUS.ERROR);
                    continue;
                }
                ...
                try {
                    ...
                    if (logger.isTraceEnabled()) {
                        logger.trace(
                                "Process: "
                                        + recipients.size()
                                        + " candidate connections for activity post "
                                        + activityPost.getId());
                    }
                    ...
                    for (String recipient : recipients) {
                        ...
                        if (!acceptActivity(activityPost, feedControls)) {
                            ...
                            for (String fmTemplate : fmTemplates) {
                                ...
                                // MNT-9104 If username contains uppercase letters the action of
                                // joining a site will not be displayed in "My activities"
                                ...
                                if (!activitySummary.equals("")) {
                                    if (activitySummary.length()
                                            > ActivityFeedDAO.MAX_LEN_ACTIVITY_SUMMARY) {
                                        logger.warn(
                                                "Skip feed entry (activity post "
                                                        + activityPost.getId()
                                    ...
                                    if (logger.isDebugEnabled()) {
                                        logger.debug(
                                                "Empty template result for activityType '"
                                                        + activityType
                                                        + "' using format '"
                                                        + formatFound
                                                        + "' hence skip feed entry (activity post "
                                                        + activityPost.getId()
                                                        + ")");
                                    }
                                }
    ...
    private Set<String> getRecipients(
        ...
        if ((null != siteId) && (siteId.length() > 0)) {
            // Get the members of this site - save hammering the repository by reusing cached site
            // members
        ...
        if ((null != siteId) && (siteId.length() > 0)) {
            for (String followerUser : followerUsers) {
                ...
                if ((caseSensitive && recipients.contains(followerUser))
                        || (!caseSensitive
                                && (recipients.contains(followerUser)
                ...
                if (canRead == null) {
                    // site read permission check (note: only check followers since implied for site
                    // members)
    ...
    public abstract List<ActivityPostEntity> selectPosts(ActivityPostEntity selector)
            throws SQLException;

    public abstract List<FeedControlEntity> selectUserFeedControls(String userId)
            throws SQLException;

    public abstract long insertFeedEntry(ActivityFeedEntity feed) throws SQLException;

    public abstract int updatePostStatus(long id, ActivityPostEntity.STATUS status)
            throws SQLException;
    ...
    protected String callWebScript(String urlString, String ticket)
        ...
        if (ticket != null) {
            ...
            // refer to:
            // http://wiki.alfresco.com/wiki/Web_Scripts_Framework#HTTP_Basic_Authentication
            conn.addRequestProperty(
                    "Authorization", "Basic " + Base64.encodeBytes(ticket.getBytes()));
        }
    ...
    protected Set<String> getSiteMembers(RepoCtx ctx, String siteId, String tenantDomain)
            throws Exception {
    ...
    protected abstract Set<String> getFollowers(String userId, String tenantDomain)
            throws Exception;

    protected abstract boolean canReadSite(
            final RepoCtx ctx, String siteIdIn, String connectedUser, final String tenantDomain)
            throws Exception;

    protected abstract boolean canRead(
            RepoCtx ctx, final String connectedUser, Map<String, Object> model) throws Exception;

    protected Map<String, List<String>> getActivityTypeTemplates(
            String repoEndPoint, String ticket, String subPath) throws Exception {
        StringBuffer sbUrl = new StringBuffer();
        sbUrl.append(repoEndPoint)
                .append(URL_SERVICE_TEMPLATES)
                .append(subPath)
                .append("*")
                .append("?format=json");
            ...
            if (!template.contains(" (Working Copy).")) {
                ...
                if (idx2 != -1) {
                    ...
                    if (idx3 != -1) {
                        ...
                        List<String> activityTypeTemplateList =
                                activityTemplates.get(baseActivityType);
    ...
    protected List<FeedControlEntity> getFeedControls(String connectedUser) throws SQLException {
        // MNT-9104 If username contains uppercase letters the action of joining a site will not be
        // displayed in "My activities"
        if (!userNamesAreCaseSensitive) {
    ...
    protected boolean acceptActivity(
            ActivityPostEntity activityPost, List<FeedControlEntity> feedControls) {
        if (feedControls == null) {
            return true;
        }
        ...
        for (FeedControlEntity feedControl : feedControls) {
            ...
            } else if (((feedControl.getAppTool() == null)
                            || (feedControl.getAppTool().length() == 0))
            ...
            } else if (((feedControl.getSiteNetwork() != null)
                            && (feedControl.getSiteNetwork().length() > 0))
                    && ((feedControl.getAppTool() != null)
                            && (feedControl.getAppTool().length() > 0))) {
                if ((feedControl.getSiteNetwork().equals(activityPost.getSiteNetwork()))
    ...
    protected void addMissingFormats(
            String activityType, List<String> fmTemplates, List<String> templatesToAdd) {

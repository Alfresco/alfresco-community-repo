...
public abstract class AbstractEmailMessageHandler implements EmailMessageHandler {
    ...
    //     * @throws InvalidArgumentException The exception is thrown if input string has incorrect
    // format or empty.
    //     */
     ...
     * @param mimetype MIME content type. For exaple you can set this parameter to "text/html" or
     *     "text/xml", etc.
     */
    ...
    protected void writeContent(
            NodeRef nodeRef, InputStream content, String mimetype, String encoding) {
        ...
        if (log.isDebugEnabled()) {
            log.debug(
                    "Write content (MimeType=\"" + mimetype + "\", Encoding=\"" + encoding + "\"");
        }
     ...
     * @param overwrite if true then overwrite an existing node with the same name. if false the
     *     name is changed to make it unique.
     * @param assocType Association type that should be set between parent node and the new one.
     * @return Reference to created node
     */
    protected NodeRef addContentNode(
        ...
        for (int counter = 1; counter < 10000; counter++) {
            QName safeQName =
                    QName.createQNameWithValidLocalName(
                            NamespaceService.CONTENT_MODEL_1_0_URI, workingName);

            NodeRef childNodeRef =
                    nodeService.getChildByName(parent, ContentModel.ASSOC_CONTAINS, workingName);
            ...
            if (childNodeRef != null) {
                ...
                ChildAssociationRef associationRef =
     ...
     * Add new node into Alfresco repository with specified parameters. Node content isn't added.
     *
     * <p>New node will be created with ContentModel.ASSOC_CONTAINS association with parent.
     *
     * @param nodeService Alfresco Node Service
     * @param parent Parent node
     * @param name Name of the new node
     * @param overwrite whether a new node should overwrite an existing node with the same name or
     *     have its name mangled to be alongside the existing node.
     * @return Reference to created node
     */
    protected NodeRef addContentNode(
            NodeService nodeService, NodeRef parent, String name, boolean overwrite) {
     ...
     * @param mainContentNode Main content node. Any mail is added into Alfresco as one main content
     *     node and several its attachments. Each attachment related with its main node.
     * @param fileName File name for the attachment.
     * @return Reference to created node.
     */
    protected NodeRef addAttachment(
            NodeService nodeService, NodeRef folder, NodeRef mainContentNode, String fileName) {
        ...
        nodeService.createAssociation(
                mainContentNode, attachmentNode, ContentModel.ASSOC_ATTACHMENTS);
    ...
    protected void addAttachments(NodeRef spaceNodeRef, NodeRef nodeRef, EmailMessage message) {
        ...
        for (EmailMessagePart attachment : attachments) {
            ...
            NodeRef attachmentNode =
                    addAttachment(getNodeService(), spaceNodeRef, nodeRef, fileName);
            writeContent(attachmentNode, contentIs, mimetype, encoding);
        }
    }

    // Lookup Table for dubious characters.
    static final String[][] dubiousChars =
            new String[][] {
                {"\\", "%5c"},
                {"/", "%2f"},
                {"*", "%2a"},
    ...
    /**
     * Subject field is used as name of the content, so we need to replace characters that are
     * forbidden in file names.
     *
     * <p>Trims whitespace
     *
     * <p>Replaces characters \/*|:"<>? with their hex values.
     *

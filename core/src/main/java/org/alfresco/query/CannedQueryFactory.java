...
/**
 * Interface for factory implementations for producing instances of {@link CannedQuery} based on all
...
public interface CannedQueryFactory<R> {
    /**
     * Retrieve an instance of a {@link CannedQuery} based on the full range of available
     * parameters.
     *
     ...
     *
     * @param parameterBean the values that the query will be based on or <tt>null</tt> if not
     *     relevant to the query
     * @param skipResults results to skip before page
     * @param pageSize the size of page - ie. max items (if skipResults = 0)
     * @param queryExecutionId ID of a previously-executed query to be used during follow-up page
     *     requests - <tt>null</tt> if not available
     * @return an implementation that will execute the query
     */
    CannedQuery<R> getCannedQuery(

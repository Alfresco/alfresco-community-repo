...
/**
 * This class provides the implementation for directory analysis, the process by which a directory
 * listing of files is broken up into ImportableItems.
 *
 * @since 4.0
 */
public class DirectoryAnalyserImpl implements DirectoryAnalyser {
    ...
    private static final Pattern VERSION_SUFFIX_PATTERN =
            Pattern.compile(".+" + VERSION_SUFFIX_REGEX);
    ...
    /**
     * @see
     *     org.alfresco.repo.bulkimport.DirectoryAnalyser#analyseDirectory(org.alfresco.repo.bulkimport.ImportableItem,
     *     java.nio.file.DirectoryStream.Filter)
     */
    public AnalysedDirectory analyseDirectory(
            ImportableItem directory, DirectoryStream.Filter<Path> filter) {
    ...
    private List<Path> listFiles(Path sourceDirectory, DirectoryStream.Filter<Path> filter) {
        ...
        try (DirectoryStream<Path> paths =
                (filter != null)
                        ? Files.newDirectoryStream(sourceDirectory, filter)
    ...
    private boolean isMetadataFile(Path file) {
        ...
        if (metadataLoader != null) {
            ...
            result =
                    name.endsWith(
                            MetadataLoader.METADATA_SUFFIX
    ...
    private void addVersionFile(
            ImportableItem parent, AnalysedDirectory analysedDirectory, Path versionFile) {
        ...
        ImportableItem importableItem =
                findOrCreateImportableItem(parent, analysedDirectory, parentContentFile);
        int version = getVersionNumber(versionFile);
        ImportableItem.VersionedContentAndMetadata versionEntry =
                findOrCreateVersionEntry(importableItem, version);
    ...
    private void addMetadataFile(
            ImportableItem parent, AnalysedDirectory analysedDirectory, Path metadataFile) {
    ...
    private boolean addParentFile(
            ImportableItem parent, AnalysedDirectory analysedDirectory, Path contentFile) {
        ImportableItem importableItem =
                findOrCreateImportableItem(parent, analysedDirectory, contentFile);
    ...
    private ImportableItem findOrCreateImportableItem(
            ImportableItem parent, AnalysedDirectory analysedDirectory, Path contentFile) {
    ...
    private ImportableItem findImportableItem(
            AnalysedDirectory analysedDirectory, Path contentFile) {
    ...
    private ImportableItem.VersionedContentAndMetadata findOrCreateVersionEntry(
            ImportableItem importableItem, int version) {
        ImportableItem.VersionedContentAndMetadata result =
                findVersionEntry(importableItem, version);
    ...
    private ImportableItem.VersionedContentAndMetadata findVersionEntry(
            ImportableItem importableItem, int version) {
        ImportableItem.VersionedContentAndMetadata result = null;

        if (importableItem.hasVersionEntries()) {
            for (ImportableItem.VersionedContentAndMetadata versionEntry :
    ...
    private int getVersionNumber(Path versionFile) {
        ...
        if (!isVersionFile(versionFile)) {
            throw new IllegalStateException(
                    FileUtils.getFileName(versionFile) + " is not a version file.");
        }
    ...
    private Path getParentOfVersionFile(Path versionFile) {
        ...
        String parentFilename =
                versionFile.getFileName().toString().replaceFirst(VERSION_SUFFIX_REGEX, "");
    ...
    private Path getParentOfMetadatafile(Path metadataFile) {
        ...
        if (!isMetadataFile(metadataFile)) {
            throw new IllegalStateException(
                    FileUtils.getFileName(metadataFile) + " is not a metadata file.");
        }

 ...
 *
 * <p>Each action applies to a set of nodes.
 *
 * <p>These actions may be executed in one overall transaction or one individual transaction. If
 * actions are in individual transactions an error may halt subsequent execution or processing can
 * try and invoke the action for each node.
 *
 * @author Andy Hind
 */
public abstract class AbstractScheduledAction implements ScheduledActionDefinition {
    ...
    public enum CompensatingActionMode {
         ...
         */
        public static CompensatingActionMode getCompensatingActionMode(
                String compensatingActionModeString) {
    ...
    public void setCompensatingActionMode(String compensatingActionModeString) {
        this.compensatingActionMode =
                CompensatingActionMode.getCompensatingActionMode(compensatingActionModeString);
    }
    ...
    public static class JobDefinition implements Job {
        ...
        public void execute(JobExecutionContext ctx) throws JobExecutionException {
            final AbstractScheduledAction abstractScheduledAction =
                    (AbstractScheduledAction)
                            ctx.getJobDetail().getJobDataMap().get(ACTION_JOB_DATA_MAP_KEY);
            ...
            AuthenticationUtil.runAs(
                    new AuthenticationUtil.RunAsWork<Object>() {
                        public Object doWork() {
                            ...
                            if (abstractScheduledAction.getTransactionModeEnum()
                                    == TransactionMode.ONE_TRANSACTION) {
                                if (s_logger.isDebugEnabled()) {
                                ...
                                for (NodeRef nodeRef : nodes) {
                                    ...
                                    try {
                                        ...
                                        if (abstractScheduledAction.getTransactionModeEnum()
                                            ...
                                            s_logger.error(
                                                    "Error in scheduled action executed in isolated"
                                                            + " transactions (other actions will"
                                                            + " continue",
                                                    t);
                                        } else {
                        ...
                        public void runTransactionalActions(final List<NodeRef> nodes) {
                            boolean runCompensatingActions =
                                        ...
                                        .doInTransaction(
                                                new RetryingTransactionCallback<Object>() {
                                                    public Object execute() throws Exception {
                                                        // Build the full list of compensating
                                                        // actions
                                                        // If anything goes wrong we need to do all
                                                        // these instead
                                                        List<Pair<Action, NodeRef>>
                                                                compensatingActions =
                                                                        new ArrayList<
                                                                                Pair<
                                                                                        Action,
                         ...
                         *
                         * <p>These are always in their own transaction. We try to run all
                         * compensating actions.
                         *
                        ...
                        private void doCompensation(
                                boolean runCompensatingActions, boolean rethrow, Throwable t) {
                            // If the error triggers compensation, and they should be processed.
                            if (runCompensatingActions
                                    && (t instanceof CompensatingActionException)) {
                                ...
                                for (Pair<Action, NodeRef> pair : cae.getCompensatingActions())
                                    if ((pair != null)
                                        ...
                                        try {
                                            ...
                                            s_logger.error(
                                                    "Error executing compensating action", t);
                                        }
                                    }
                            }
                            ...
                            try {
                                abstractScheduledAction
                                        ...
                                        .doInTransaction(
                                                new RetryingTransactionCallback<Object>() {
                                                    public Object execute() throws Exception {
                                                        // try action - failure triggers
                                                        // compensation
                                                        Action action =
                                                                abstractScheduledAction.getAction(
                                                                        nodeRef);
                                                        Action compensatingAction =
                                                                action.getCompensatingAction();
                                                        try {
                        ...
                        /**
                         * Manage running a compensating action and chaining all its compensating
                         * actions until done
                         *
                         * @param pair Pair<Action, NodeRef>
                         */
                        public void runTransactionalCompensatingAction(
                                final Pair<Action, NodeRef> pair) {
                            boolean runCompensatingActions =
                                    abstractScheduledAction.getCompensatingActionModeEnum()
                                            == CompensatingActionMode
                                                    .RUN_COMPENSATING_ACTIONS_ON_FAILURE;

...
public class LockBehaviourImplTest extends BaseSpringTest {
  ...
  public void testVersionServiceLockBehaviour01() {
    ...
    try {
      ...
      fail(
        "Should have passed, as we should be able to create a version. See ALF-16540"
      );
    }
    this.lockService.unlock(this.nodeRef);
  }
  ...
  public void testVersionServiceLockBehaviour02() {
    ...
    try {
      ...
      fail(
        "Should have failed since this node has been locked by another user with a write lock."
      );
  ...
  public void testCannotMoveNodeWhenLocked() {
    ...
    NodeRef newParentRef = nodeService
      .createNode(
        parentNode,
        ContentModel.ASSOC_CONTAINS,
        QName.createQName("{test}nodeServiceLockTest"),
  ...
  public void testCanMoveCopyDeleteWithLockOwner() {
    ...
    NodeRef archivingBehaviorNodeRef = nodeService
      .createNode(
        this.inSpaceStoreNode,
        ContentModel.ASSOC_CONTAINS,
        QName.createQName("{test}nodeServiceLockTest"),
    ...
    // Create the new container that we'll move the node to.
    NodeRef newParentRefToMove = nodeService
      .createNode(
        parentNode,
        ContentModel.ASSOC_CONTAINS,
        QName.createQName("{test}nodeServiceLockTest"),
    ...
    NodeRef newParentRefToCopy = nodeService
      .createNode(
        parentNode,
        ContentModel.ASSOC_CONTAINS,
        QName.createQName("{test}nodeServiceLockTest"),
  ...
  private void checkForLockForBadAndGoodUsers(NodeRef nodeToCheck) {
    String currentUserName = TestWithUserUtils.getCurrentUser(
      this.authenticationService
    );

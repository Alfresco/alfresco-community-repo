...
public abstract class AbstractGetBlogWebScript extends AbstractBlogWebScript {
  ...
  private static final Log log = LogFactory.getLog(
    AbstractGetBlogWebScript.class
  );

  @Override
  protected Map<String, Object> executeImpl(
    SiteInfo site,
    NodeRef nonSiteContainer,
    BlogPostInfo blog,
    ...
    pagingReq.setRequestTotalCountMax(
      pagingReq.getSkipCount() + pagingReq.getRequestTotalCountMax()
    );
    ...
    if (this.getClass().equals(BlogPostsNewGet.class)) {
      ...
      if (numDays != null) {
        ...
        fromDate =
          new Date(
            System.currentTimeMillis() - (numDaysInt * oneDayInMilliseconds)
          );
      }
    }

    // Fetch and assign the data
    PagingResults<BlogPostInfo> blogPostList = getBlogPostList(
      site,
      nonSiteContainer,
      fromDate,
      toDate,
    ...
    if (log.isDebugEnabled()) {
      ...
      msg
        .append("Retrieved ")
        .append(blogPostList.getPage().size())
        .append(" blog posts in page.");
  ...
  protected void createFtlModel(
    ...
    List<Map<String, Object>> blogPostDataSets = new ArrayList<Map<String, Object>>(
      blogPostList.getPage().size()
    );
    for (BlogPostInfo postInfo : blogPostList.getPage()) {
      Map<String, Object> data = BlogPostLibJs.getBlogPostData(
        postInfo.getNodeRef(),
        services
      );
    ...
    int contentLength = contentLengthStr == null
      ? -1
   ...
   */
  private PagingResults<BlogPostInfo> getBlogPostList(
    SiteInfo site,
    NodeRef nonSiteContainer,
    Date fromDate,
    Date toDate,
    ...
    if (tag == null || tag.trim().isEmpty()) {
      ...
      RangedDateProperty dateRange = new RangedDateProperty(
        fromDate,
        toDate,

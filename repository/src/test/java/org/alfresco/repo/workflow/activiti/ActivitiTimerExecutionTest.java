...
public class ActivitiTimerExecutionTest extends BaseSpringTest {
    ...
    public void testTimerExecutionAuthentication() throws Exception {
        try {
            WorkflowInstance taskAssigneeWorkflowInstance =
                    transactionHelper.doInTransaction(
                            new RetryingTransactionHelper.RetryingTransactionCallback<
                                ...
                                public WorkflowInstance execute() throws Throwable {
                                    ...
                                    // Start the test timer transaction process, with 'error' =
                                    // false, expecting a timer job
                                    // to be executed without an error, with task timer is assigned
                                    // to assigned to USER1
                                    ...
                                    WorkflowPath path =
                                            workflowService.startWorkflow(
                                                    definition.getId(), params);
                                    // End start-task
                                    workflowService.endTask(
                                            workflowService
                                                    .getStartTask(path.getInstance().getId())
                                                    .getId(),
                                            null);
            ...
            // No timers should be available after a while they should have been executed, otherwise
            // test fails
            waitForTimersToBeExecuted(taskAssigneeWorkflowInstance.getId());

            // Test assigned task
            WorkflowPath path =
                    workflowService.getWorkflowPaths(taskAssigneeWorkflowInstance.getId()).get(0);
    ...
    public void testTimerExecutionTransactionRollback() throws Exception {
        try {
                    ...
                    transactionHelper.doInTransaction(
                            new RetryingTransactionHelper.RetryingTransactionCallback<
                                ...
                                public WorkflowInstance execute() throws Throwable {
                                    ...
                                    // Start the test timer transaction process, with 'error' =
                                    // true, expecting a timer job
                                    // to be executed with an error, with task timer is assigned to
                                    // assigned to USER1
                                    Map<QName, Serializable> params =
                                            new HashMap<QName, Serializable>();
            ...
            String fullExceptionStacktrace =
                    activitiProcessEngine
                            .getManagementService()
                ...
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        try {
                            ...
                            // Make sure process-definition is still deleted, even when clearing
                            // people fails.
    ...
    protected WorkflowDefinition deployDefinition(String resource) {
        ...
        WorkflowDeployment deployment =
                workflowService.deployDefinition(
                        ActivitiConstants.ENGINE_ID, input, MimetypeMap.MIMETYPE_XML);

...
public class VirtualDefaultCopyBehaviourCallbackExtension
  extends SpringBeanExtension<DefaultCopyBehaviourCallbackExtension, DefaultCopyBehaviourCallbackTrait>
  implements DefaultCopyBehaviourCallbackExtension {
    ...
    if (environment.isSubClass(classQName, ContentModel.TYPE_FOLDER)) {
      if (
        copyDetails
          ...
          .contains(VirtualContentModel.ASPECT_VIRTUAL)
      ) {
        return false;
      }
    }
    return getTrait().getMustCopy(classQName, copyDetails);
  }
}

...
/**
 * {@link CacheFactory} implementation that creates {@link DefaultSimpleCache} instances. The caches
 * are created with a capacity specified by the property {name}.maxItems. For example, a cache named
 * <tt>cache.ticketsCache</tt> would have a capacity specified by the property
 * <tt>cache.ticketsCache.maxItems</tt>
 *
 * @author Matt Ward
 */
public class DefaultCacheFactory<K extends Serializable, V> extends AbstractCacheFactory<K, V> {
    ...
    private SimpleCache<K, V> createLocalCache(String cacheName) {
        ...
        DefaultSimpleCache<K, V> cache =
                new DefaultSimpleCache<K, V>(
                        maxItems, useMaxItems, ttlSecs, maxIdleSeconds, cacheName);

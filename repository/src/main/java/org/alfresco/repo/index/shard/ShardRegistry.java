...
public interface ShardRegistry {
     ...
     *
     * @param shardState the shard state, which contains the information about the shard that wants
    ...
    /**
     * Returns the shard instance (i.e. shard number) which owns (or should own) the transaction
     * associated with the given timestamp.
     *
     * @param coreId an identifier (e.g. core name, base url) of the core / collection whose
     *     requested shard belongs to.
     * @param txnTimestamp the transaction timestamp used as search criteria.
     * @return the shard instance (i.e. shard number) which owns (or should own) the transaction
     *     associated with the given timestamp.
     */

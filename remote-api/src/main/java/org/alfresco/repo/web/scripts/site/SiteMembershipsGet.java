...
/**
 * Lists the members of a site, optionally filtering by name, role or authority type.
 *
 * <p>Based on the old memberships.get.js webscript controller
 *
 * @author Nick Burch
 * @since 4.0
 */
public class SiteMembershipsGet extends AbstractSiteWebScript {
    ...
    protected Map<String, Object> executeImpl(
            SiteInfo site, WebScriptRequest req, Status status, Cache cache) {
        // Grab out filters
        ...
        if (authorityType != null) {
            if ("USER".equals(authorityType)) {
                ...
                throw new WebScriptException(
                        Status.STATUS_BAD_REQUEST, "The Authority must be one of USER or GROUP");
            }
        }
        ...
        List<SiteMemberInfo> members =
                this.siteService.listMembersInfo(
        ...
        for (SiteMemberInfo authorityObj : members) {
            ...
            if (authorityObj.getMemberName().startsWith("GROUP_")) {
                if (authorityType == null || authorityType.equals("GROUP")) {
                    ...
                    authorities.put(
                            ftlSafeName,
                            scriptAuthorityService.getGroupForFullAuthorityName(
                ...
                if (authorityType == null || authorityType.equals("USER")) {
                    // Record the details
                    authorities.put(
                            ftlSafeName, personService.getPerson(authorityObj.getMemberName()));
                    memberInfo.put(ftlSafeName, authorityObj);
                }
            }
        }

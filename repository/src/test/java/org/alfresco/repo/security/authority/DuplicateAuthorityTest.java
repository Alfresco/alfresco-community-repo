...
public class DuplicateAuthorityTest extends TestCase {
  ...
  public void testDuplicateGroupDetection() throws Exception {
    ...
    RetryingTransactionCallback<String> callback = new CreateAuthorityCallback(
      0
    );
    String result = retryingTransactionHelper.doInTransaction(
      callback,
      false,
      true
    );
  ...
  private class CreateAuthorityCallback
    implements RetryingTransactionCallback<String> {
  ...
  private class AuthThread extends Thread {
    ...
    public void run() {
      ...
      while (true) {
        ...
        try {
          System.out.println(
            "Thread " + getName() + " attempting file: " + number
          );
          System.out.flush();
      ...
      if (error != null) {
        System.err.println(
          "Thread " + getName() + " failed to create file " + number + ":"
        );
        System.err.flush();
        error.printStackTrace();
      } else {
        System.out.println(
          "\t\t\tThread " + getName() + " created auth: " + success
        );
        System.out.flush();
      }
      // Tick the latch
  ...
  private class PersonThread extends Thread {
    ...
    public void run() {
      ...
      int number = 0;
      while (true) {
        RetryingTransactionCallback<String> callback = new CreatePersonCallback(
          number
        );
        try {
          System.out.println(

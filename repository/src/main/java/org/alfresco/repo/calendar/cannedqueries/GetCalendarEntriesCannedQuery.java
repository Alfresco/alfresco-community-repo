...
public class GetCalendarEntriesCannedQuery
  ...
  private static final String QUERY_SELECT_GET_BLOGS =
    "select_GetCalendarEntriesCannedQuery";
  ...
  @Override
  protected List<CalendarEntry> queryAndFilter(
    ...
    if (paramBeanObj == null) throw new NullPointerException(
      "Null GetCalendarEntries query params"
    );
    ...
    List<CalendarEntity> results = cannedQueryDAO.executeQuery(
      QUERY_NAMESPACE,
      QUERY_SELECT_GET_BLOGS,
      paramBean,
      0,
      Integer.MAX_VALUE
    );

    List<CalendarEntity> filtered = new ArrayList<CalendarEntity>(
      results.size()
    );
    for (CalendarEntity result : results) {
      ...
      if (
        strFromDate != null &&
        strFromDate.equals(strToDate) &&
        isMidnightUTC(strFromDate)
      ) {
        // it is all day event and should conform with current server's timezone
        fromDate =
          ISO8601DateFormat.parseDayOnly(strFromDate, TimeZone.getDefault());
        toDate = fromDate;
      } else {
        fromDate =
          DefaultTypeConverter.INSTANCE.convert(Date.class, strFromDate);
      ...
      Date recurringLastDate = DefaultTypeConverter.INSTANCE.convert(
        Date.class,
        result.getRecurrenceLastMeeting()
      );
          ...
          if (dates != null && dates.size() > 0) {
            ...
            for (Date date : dates) {
              if (entriesFromDate != null && entriesToDate != null) {
                ...
                if (
                  entriesFromDate.getTime() <= date.getTime() &&
    ...
    if (sortPairs != null && !sortPairs.isEmpty()) {
      ...
      for (Pair<? extends Object, SortOrder> sortPair : sortPairs) {
        ...
        final CalendarEntityComparator comparator = new CalendarEntityComparator(
          sortProperty
        );
        comparators.add(
          new Pair<Comparator<CalendarEntity>, SortOrder>(
            comparator,
            sortPair.getSecond()
          )
        );
      }
      NestedComparator<CalendarEntity> comparator = new NestedComparator<CalendarEntity>(
        comparators
      );
  ...
  protected static class CalendarEntityComparator
    extends PropertyBasedComparator<CalendarEntity> {
    ...
    protected Comparable getProperty(CalendarEntity entity) {
      if (comparableProperty.equals(CalendarModel.PROP_FROM_DATE)) {
        ...
        throw new IllegalArgumentException(
          "Unsupported calendar sort property: " + comparableProperty
        );
      }
    }
  }

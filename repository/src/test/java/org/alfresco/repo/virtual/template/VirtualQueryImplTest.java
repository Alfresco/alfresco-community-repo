...
public class VirtualQueryImplTest extends TestCase {
  ...
  private static final String QUERY_TEST_STRING_QUERY =
  ...
  protected void setUp() throws Exception {
    ...
    Mockito
      .when(mockitoActualEnvironment.query(Mockito.any(SearchParameters.class)))
      .thenReturn(new EmptyResultSet());

    mockitoPrefixResolver = Mockito.mock(NamespacePrefixResolver.class);
    Mockito
      .when(mockitoPrefixResolver.getNamespaceURI(TST_PREFIX))
      .thenReturn(TEST_URI);
    Mockito
      .when(mockitoPrefixResolver.getPrefixes(TEST_URI))
      .thenReturn(Arrays.asList(TST_PREFIX));

    Mockito
      .when(mockitoActualEnvironment.getNamespacePrefixResolver())
      .thenReturn(mockitoPrefixResolver);

    testQName1 =
      QName.createQName(TST_PREFIX, TEST_LOCAL_NAME_1, mockitoPrefixResolver);

    testQName2 =
      QName.createQName(TST_PREFIX, TEST_LOCAL_NAME_2, mockitoPrefixResolver);

    NodeRef n1 = new NodeRef(
      "workspace://SpacesStore/17c8f11d-0936-4295-88a0-12b85764c76f"
    );
    NodeRef n2 = new NodeRef(
      "workspace://SpacesStore/27c8f11d-0936-4295-88a0-12b85764c76f"
    );
    nodeOneReference =
      ((VirtualProtocol) Protocols.VIRTUAL.protocol).newReference(n1, "/1", n2);
  }

  @Test
  public void testPerform_1() throws Exception {
    Pair<QName, Boolean> withSortDefinitions = new Pair<QName, Boolean>(
      testQName2,
      true
    );

    VirtualQueryConstraint constraint = BasicConstraint.INSTANCE;
    constraint = new FilesFoldersConstraint(constraint, true, true);
    constraint =
      new IgnoreConstraint(
        constraint,
        Collections.singleton(testQName2),
        Collections.singleton(testQName1)
      );
    constraint =
      new SortConstraint(constraint, Arrays.asList(withSortDefinitions));

    query.perform(mockitoActualEnvironment, constraint, null, nodeOneReference);
  ...
  public void testPerform_2() throws Exception {
    VirtualQueryConstraint constraint = BasicConstraint.INSTANCE;
    constraint = new FilesFoldersConstraint(constraint, false, true);
    constraint =
      new IgnoreConstraint(
        constraint,
        Collections.singleton(testQName2),
  ...
  private void assertPerform1Results(Pair<QName, Boolean> withSortDefinitions) {
    ArgumentCaptor<SearchParameters> queryCaptor = ArgumentCaptor.forClass(
      SearchParameters.class
    );
    Mockito.verify(mockitoActualEnvironment).query(queryCaptor.capture());

    assertEquals(
      "(QUERY_TEST_STRING_QUERY) and !ASPECT:'tst:testQName1' and !TYPE:'tst:testQName2'",
      queryCaptor.getValue().getQuery()
    );

    ArrayList<SortDefinition> sortDefinitions = queryCaptor
      .getValue()
      .getSortDefinitions();
    ...
    assertEquals(
      withSortDefinitions.getFirst().getPrefixString(),
      sortDefinitions.get(0).getField()
    );

    assertEquals(
      withSortDefinitions.getSecond(),
  ...
  private void assertPerform2Results() {
    ...
    assertEquals(
      "((QUERY_TEST_STRING_QUERY) and TYPE:\"cm:folder\") and !ASPECT:'tst:testQName1' and !TYPE:'tst:testQName2'",
      queryCaptor.getValue().getQuery()
    );

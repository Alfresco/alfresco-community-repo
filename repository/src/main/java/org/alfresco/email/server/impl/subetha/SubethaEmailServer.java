...
public class SubethaEmailServer extends EmailServer {
  ...
  public void startup() {
    ...
    if (isAuthenticate()) {
      AuthenticationHandlerFactory authenticationHandler = new EasyAuthenticationHandlerFactory(
        new AlfrescoLoginUsernamePasswordValidator()
      );
    ...
    serverImpl.start();
    log.info(
      "Inbound SMTP Email Server has started successfully, on hostName:" +
      getDomain() +
      "port:" +
      getPort()
    );
  ...
  class Handler implements MessageHandler {
    ...
    public void from(String fromString) throws RejectException {
      ...
      try {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "check whether user is allowed to send email from" + from
          );
        }
        filterSender(from);
    ...
    public void recipient(String recipient) throws RejectException {
      ...
      deliveries.add(
        new EmailDelivery(
          recipient,
          from,
          auth != null ? (String) auth.getIdentity() : null
        )
      );
    }

    public void data(InputStream data)
      throws TooMuchDataException, IOException, RejectException {
    ...
    private void processDelivery(EmailDelivery delivery, InputStream data)
      ...
      try {
        ...
        throw new RejectException(
          554,
          "An internal error prevented mail delivery."
        );
      }
    }

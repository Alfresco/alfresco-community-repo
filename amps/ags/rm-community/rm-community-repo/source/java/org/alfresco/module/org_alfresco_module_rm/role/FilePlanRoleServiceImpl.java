...
public class FilePlanRoleServiceImpl
  ...
  public void setupFilePlanRoles(final NodeRef filePlan) {
    ...
    if (nodeService.exists(filePlan)) {
      List<NodeRef> systemContainers = AuthenticationUtil.runAsSystem(
    ...
    AuthenticationUtil.runAsSystem(
      new AuthenticationUtil.RunAsWork<Object>() {
        public Object doWork() {
          ...
          String allRolesGroup = authorityService.getName(
            AuthorityType.GROUP,
            getAllRolesGroupShortName(filePlan)
          );
          Set<String> groups = authorityService.getContainedAuthorities(
            AuthorityType.GROUP,
            allRolesGroup,
            true
          );
  ...
  private void bootstrapDefaultRoles(
    ...
    final List<NodeRef> systemContainers
  ) {
        ...
        public Object doWork() {
          try {
            ...
            try {
              ...
              throw new AlfrescoRuntimeException(
                "Unable to load rm-default-roles-bootstrap.json configuration file.",
                ioe
              );
            }
            ...
            for (int i = 0; i < array.length(); i++) {
              ...
              if (object.has(JSON_NAME)) {
                ...
                throw new AlfrescoRuntimeException(
                  "No name given to default bootstrap role.  Check json configuration file."
                );
              }
                ...
                for (int index = 0; index < arrCaps.length(); index++) {
                  ...
                  if (capability == null) {
                    throw new AlfrescoRuntimeException(
                      "The capability '" +
                      capName +
                      "' configured for the deafult boostrap role '" +
                      name +
                      "' is invalid."
                    );
                  }
            ...
            throw new AlfrescoRuntimeException(
              "Error loading json configuration file rm-default-roles-bootstrap.json",
              exception
            );
          }
  ...
  public Set<Role> getRoles(
    ...
    final boolean includeSystemRoles
  ) {
    return AuthenticationUtil.runAs(
      new AuthenticationUtil.RunAsWork<Set<Role>>() {
        public Set<Role> doWork() {
          ...
          Set<String> roleAuthorities = authorityService.getAllAuthoritiesInZone(
            getZoneName(rmRootNode),
            AuthorityType.GROUP
          );
          for (String roleAuthority : roleAuthorities) {
            if (includeSystemRoles || !isSystemRole(roleAuthority)) {
              String groupShortName = authorityService.getShortName(
                roleAuthority
              );
              String name = getShortRoleName(groupShortName, rmRootNode);
              String displayLabel = authorityService.getAuthorityDisplayName(
                roleAuthority
              );
              ...
              Set<Capability> capabilities = getCapabilitiesImpl(
                rmRootNode,
                roleAuthority
              );
   ...
   * @return
   */
  private Set<Capability> getCapabilitiesImpl(
    NodeRef rmRootNode,
      ...
      .runAs(
        new AuthenticationUtil.RunAsWork<Boolean>() {
          public Boolean doWork() {
            String fullRoleName = authorityService.getName(
              AuthorityType.GROUP,
              getFullRoleName(role, rmRootNode)
            );

            String zone = getZoneName(rmRootNode);
            Set<String> roles = authorityService.getAllAuthoritiesInZone(
              zone,
              AuthorityType.GROUP
            );
    ...
    return AuthenticationUtil.runAs(
      new AuthenticationUtil.RunAsWork<Role>() {
        public Role doWork() {
          ...
          String roleGroup = authorityService.createAuthority(
            AuthorityType.GROUP,
            fullRoleName,
            groupDisplayLabel,
            zones
          ...
          if (!isSystemRole(role)) {
            ...
            String allRoleGroup = authorityService.getName(
              AuthorityType.GROUP,
              getAllRolesGroupShortName(filePlan)
            );
      ...
      new AuthenticationUtil.RunAsWork<Role>() {
        public Role doWork() {
          if (!existsRole(rmRootNode, role)) {
            throw new AlfrescoRuntimeException(
              "Unable to update role " + role + ", because it does not exist."
            );
          }
   ...
   */
  @Override
  public Set<String> getUsersAssignedToRole(
    final NodeRef filePlan,
   ...
   */
  @Override
  public Set<String> getGroupsAssignedToRole(
    final NodeRef filePlan,
    final String roleName
  ) {
    ParameterCheck.mandatory("filePlan", filePlan);
    ...
    return getAuthoritiesAssignedToRole(
      filePlan,
      roleName,
      AuthorityType.GROUP
    );
  ...
  public String getAllRolesContainerGroup(NodeRef filePlan) {
    return authorityService.getName(
      AuthorityType.GROUP,
      getAllRolesGroupShortName(filePlan)
    );
  }

...
/**
 * Checks that the behaviour of the {@link ContentModel#ASPECT_AUDITABLE auditable aspect} is
 * correct.
 *
 * @author Derek Hulley
 */
...
public class AuditableAspectTest extends TestCase {
    ...
    public void testNoAudit() {
        ...
        assertFalse(
                "cm:auditable must not be present.",
                aspects.contains(ContentModel.ASPECT_AUDITABLE));
        Map<QName, Serializable> properties = nodeService.getProperties(nodeRef);
        assertFalse(
                "cm:creator must not be present",
                properties.containsKey(ContentModel.PROP_CREATOR));
        assertFalse(
                "cm:created must not be present",
    ...
    public synchronized void testAddAndRemoveAspect() throws Exception {
        ...
        Date aspectCreatedDate1 =
                (Date) nodeService.getProperty(nodeRef, ContentModel.PROP_CREATED);
        Date aspectModifiedDate1 =
                (Date) nodeService.getProperty(nodeRef, ContentModel.PROP_MODIFIED);
        ...
        // Pause to allow for node modifiedDate tolerance (of 1000ms - refer to
        // AbstractNodeDAOImpl.updateNodeImpl)
        this.wait(1500); //
        ...
        // Check that the dates were set correctly
        Date aspectCreatedDate2 =
                (Date) nodeService.getProperty(nodeRef, ContentModel.PROP_CREATED);
        Date aspectModifiedDate2 =
                (Date) nodeService.getProperty(nodeRef, ContentModel.PROP_MODIFIED);
     ...
     * ALF-4117: NodeDAO: Allow cm:auditable to be set ALF-3569: Alfresco repository CIFS driver not
     * setting timestamps
     */
    public void testCreateAndUpdateAuditableProperties() {
        ...
        nodeService.setProperty(
                ...
                "TEST-FIRST-NAME-" + System.currentTimeMillis());
        ...
        RetryingTransactionCallback<Void> setAuditableCallback1 =
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        behaviourFilter.disableBehaviour(
                                ContentModel.ASPECT_AUDITABLE); // Lasts for txn
                        // Set the auditable properties explicitly
                        Long currentTime = System.currentTimeMillis();
                        auditableProps.put(ContentModel.PROP_CREATOR, "Creator-" + currentTime);
                        auditableProps.put(
                                ContentModel.PROP_CREATED, new Date(currentTime - 1000L));
                        auditableProps.put(ContentModel.PROP_MODIFIER, "Modifier-" + currentTime);
                        auditableProps.put(
                                ContentModel.PROP_MODIFIED, new Date(currentTime - 1000L));
        ...
        RetryingTransactionCallback<Void> setAuditableCallback2 =
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        behaviourFilter.disableBehaviour(
                                ContentModel.ASPECT_AUDITABLE); // Lasts for txn
    ...
    public void testPutContent() throws Exception {
        ...
        ChildAssociationRef childAssocRef =
                nodeService.createNode(
                        ...
                        QName.createQNameWithValidLocalName(
                                NamespaceService.CONTENT_MODEL_1_0_URI, fileName),
        ...
        RetryingTransactionCallback<Date> dateCallback =
        ...
        final Date txnModified =
                transactionService.getRetryingTransactionHelper().doInTransaction(dateCallback);
        assertTrue(
                "Last modified date should be changed in individual transaction",
                modified.getTime() < txnModified.getTime());

        RetryingTransactionCallback<Integer> countCallback =
        ...
        Integer count =
                transactionService.getRetryingTransactionHelper().doInTransaction(countCallback);

        assertTrue(
                "Repeated content uploads in the same transaction should only modify the"
                        + " cm:modified once.",
                count == 1);
    }
    ...
    private void assertAuditableProperties(NodeRef nodeRef, Map<QName, Serializable> checkProps) {
        ...
        if (checkProps != null) {
            assertEquals(
                    "PROP_CREATED not correct",
                    checkProps.get(ContentModel.PROP_CREATED),
                    props.get(ContentModel.PROP_CREATED));
            assertEquals(
                    "PROP_MODIFIED not correct",
                    checkProps.get(ContentModel.PROP_MODIFIED),
                    props.get(ContentModel.PROP_MODIFIED));
            assertEquals(
                    "PROP_CREATOR not correct",
                    checkProps.get(ContentModel.PROP_CREATOR),
                    props.get(ContentModel.PROP_CREATOR));
            assertEquals(
                    "PROP_MODIFIER not correct",
                    checkProps.get(ContentModel.PROP_MODIFIER),

...
public class GetBlogPostsCannedQuery
  ...
  protected List<BlogEntity> queryAndFilter(CannedQueryParameters parameters) {
    ...
    if (paramBeanObj == null) throw new NullPointerException(
      "Null GetBlogPosts query params"
    );
    ...
    List<BlogEntity> results = cannedQueryDAO.executeQuery(
      QUERY_NAMESPACE,
    ...
    for (BlogEntity result : results) {
      ...
      Date actualPublishedDate = DefaultTypeConverter.INSTANCE.convert(
        Date.class,
      ...
      if (requestedCreator != null) {
        AuditablePropertiesEntity auditProps = result
          .getNode()
      ...
      if ((publishedFromDate != null) || (publishedToDate != null)) {
        if (actualPublishedDate != null) {
          ...
          if (
            ...
            actualPublishedDate.after(publishedToDate)
          ) {
            nextNodeIsAcceptable = false;
          }
        } else {
    ...
    if (sortPairs != null && !sortPairs.isEmpty()) {
      ...
      Comparator<BlogEntity> comparator = new BlogEntityComparator(
        sortProperty
      );

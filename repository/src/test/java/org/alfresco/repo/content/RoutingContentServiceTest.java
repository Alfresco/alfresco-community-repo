...
public class RoutingContentServiceTest extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    contentService =
      (ContentService) ctx.getBean(
        ServiceRegistry.CONTENT_SERVICE.getLocalName()
      );
  ...
  public void testSimpleNonTempWriter() throws Exception {
    ...
    assertEquals(
      "Content Encoding was not set",
      "UTF-16",
      reader.getEncoding()
    );
    assertEquals(
      "Content Locale was not set",
      Locale.CHINESE,
      reader.getLocale()
    );
  }
  ...
  public void testGetRawReader() throws Exception {
    ...
    assertNotNull(
      "A reader is expected with content URL referencing no content",
      reader
    );
  ...
  public void testMissingContent() throws Exception {
    ...
    ContentPropertyRestrictionInterceptor contentPropertyRestrictionInterceptor = (ContentPropertyRestrictionInterceptor) ctx.getBean(
    ...
    try {
      ...
      assertFalse(
        "Reader should indicate that content is missing",
        reader.exists()
      );
  ...
  public void onContentUpdateBehaviourTest(
    NodeRef nodeRef,
    boolean newContent
  ) {
  ...
  public void testTempWrite() throws Exception {
    ...
    // check
    assertNotSame(
      "Temp URLs must be different",
      writer1.getContentUrl(),
      writer2.getContentUrl()
    );
  }
  ...
  public void testUpdatingWrite() throws Exception {
    ...
    String contentCheck = reader.getContentString();
    assertEquals(
      "Content fetched doesn't match that written",
      SOME_CONTENT,
      contentCheck
    );
    ...
    assertEquals(
      "Mimetype not set on content data",
      "text/plain",
  ...
  public void testConcurrentWritesWithSingleTxn() throws Exception {
    ...
    assertNull(
      "Transaction was rolled back - no content should be visible",
      reader
    );
   ...
   */
  public synchronized void testConcurrentWritesWithMultipleTxns()
    throws Exception {
    NodeRef nodeRef = nodeService
      .createNode(
        rootNodeRef,
    ...
    for (int i = 0; i < writeThreads.length; i++) {
      ContentWriter threadWriter = contentService.getWriter(
        nodeRef,
        ContentModel.PROP_CONTENT,
        true
      );
  ...
  private class WriteThread extends Thread {
    ...
    public void run() {
      ...
      synchronized (this) {
        ...
        try {
          this.wait();
        } catch (InterruptedException e) {} // wait until notified
      }
  ...
  public void testPostCopyContentRetrieval() throws Exception {
    ...
    ContentData nodeContentData = (ContentData) nodeService.getProperty(
      contentNodeRef,
      ContentModel.PROP_CONTENT
    );
    ...
    Map<QName, Serializable> copyProperties = nodeService.getProperties(
      contentNodeRef
    );
    copyProperties.remove(ContentModel.PROP_NODE_UUID);
    // Copy the node
    NodeRef contentCopyNodeRef = copyService.copy(
      contentNodeRef,
      rootNodeRef,
      ContentModel.ASSOC_CHILDREN,
      QName.createQName(TEST_NAMESPACE, GUID.generate())
    ...
    ContentData copyNodeContentData = (ContentData) nodeService.getProperty(
      contentCopyNodeRef,
      ContentModel.PROP_CONTENT
    );
    ...
    assertNotNull(copyNodeContentData);
    assertEquals(
      "Post-delete value didn't remain the same",
      writerContentData,
      copyNodeContentData
    );
    ContentReader copyNodeContentReader = contentService.getReader(
      contentCopyNodeRef,
      ContentModel.PROP_CONTENT
    );
  ...
  public void testRollbackCleanup_ALF2890() throws Exception {
    ...
    try {
      ContentWriter updatingWriter = contentService.getWriter(
        contentNodeRef,
        ContentModel.PROP_CONTENT,
        true
      );
      ...
      ContentReader readerStep1 = contentService.getReader(
        contentNodeRef,
        ContentModel.PROP_CONTENT
      );
      assertEquals(
        "Incorrect content",
        "STEP 1",
        readerStep1.getContentString()
      );

      ContentWriter simpleWriter = contentService.getWriter(
        contentNodeRef,
        ContentModel.PROP_CONTENT,
        false
      );
      ...
      nodeService.setProperty(
        contentNodeRef,
        ContentModel.PROP_CONTENT,
        simpleWriter.getContentData()
      );
      ContentReader readerStep2 = contentService.getReader(
        contentNodeRef,
        ContentModel.PROP_CONTENT
      );
      assertEquals(
      ...
      try {
        transactionService
          .getRetryingTransactionHelper()
          .doInTransaction(rollbackCallback);
      } catch (RuntimeException e) {
        if (!e.getMessage().equals("aaa")) {
          throw e;
        }

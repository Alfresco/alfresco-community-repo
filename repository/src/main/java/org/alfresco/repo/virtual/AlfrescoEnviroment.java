...
public class AlfrescoEnviroment implements ActualEnvironment {
    ...
    public Object executeScript(String classpath, Map<String, Object> model) {
        ...
        return apiFacet.getScriptService()
                .executeScript(new ClasspathScriptLocation(classpath.substring(1)), model);
    }

    @Override
    public Object executeScript(NodeRef templateNodeRef, Map<String, Object> model) {

        return apiFacet.getScriptService().executeScript(templateNodeRef, null, model);
    }

    @Override
    public boolean hasAspect(final NodeRef nodeRef, final QName aspectTypeQName) {
        return apiFacet.getNodeService().hasAspect(nodeRef, aspectTypeQName);
    }
    ...
    public NodeRef getTargetAssocs(NodeRef nodeRef, QName associationQName) {
        List<AssociationRef> assocs =
                apiFacet.getNodeService().getTargetAssocs(nodeRef, associationQName);
    ...
    public Serializable getProperty(NodeRef nodeRef, QName qname) {
        return apiFacet.getNodeService().getProperty(nodeRef, qname);
    }
    ...
    public InputStream openContentStream(NodeRef nodeRef) throws ActualEnvironmentException {
        ContentReader contentReader =
                apiFacet.getContentService().getReader(nodeRef, ContentModel.PROP_CONTENT);
    ...
    @Override
    public Object createScriptVirtualContext(VirtualContext context)
            throws ActualEnvironmentException {
        return new AlfrescoScriptVirtualContext(context, serviceRegistry);
    }
    ...
    public boolean isSubClass(QName className, QName ofClassName) {
        return apiFacet.getDictionaryService().isSubClass(className, ofClassName);
    }
    ...
    public List<ChildAssociationRef> getChildAssocs(
        ...
        return nodeService.getChildAssocs(
                nodeRef, typeQNamePattern, qnamePattern, maxResults, preload);
    }

    @Override
    public NodeRef findNodeRef(String referenceType, String[] reference) {
        return repositoryHelper.findNodeRef(referenceType, reference);
    }
    ...
    public NodeRef getChildByName(NodeRef nodeRef, QName assocTypeQName, String childName) {
        return apiFacet.getNodeService().getChildByName(nodeRef, assocTypeQName, childName);
    }
    ...
    public FileInfo create(NodeRef parentNodeRef, String name, QName typeQName)
        ...
        return apiFacet.getFileFolderService().create(parentNodeRef, name, typeQName);
    }

    @Override
    public ContentWriter getWriter(NodeRef nodeRef, QName propertyQName, boolean update)
            throws InvalidNodeRefException, InvalidTypeException {
        return apiFacet.getContentService().getWriter(nodeRef, propertyQName, update);
    }
    ...
    public void addAspect(
        ...
        apiFacet.getNodeService().addAspect(nodeRef, aspectTypeQName, aspectProperties);
    }
    ...
    public boolean hasPermission(NodeRef nodeRef, String perm) {
        return apiFacet.getPermissionService()
                .hasPermission(nodeRef, perm)

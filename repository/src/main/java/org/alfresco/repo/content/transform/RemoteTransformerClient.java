...
/**
 * Client class that transfers content (from a ContentReader) to a remote transformation agent
 * together with request parameters that will be used to transform the content. The transformed
 * content is then returned and saved in a ContentWriter. In the event of an error an Exception is
 * thrown.
 *
 * @since 6.0
 */
public class RemoteTransformerClient {
    ...
    HttpEntity getRequestEntity(
        ...
        builder.addBinaryBody(
                "file", reader.getContentInputStream(), contentType, "tmp." + sourceExtension);
    ...
    void request(
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    name + ' ' + sourceExtension + ' ' + targetExtension + ' ' + url + ' ' + args);
        }
                ...
                try (CloseableHttpResponse response = execute(httpclient, httppost)) {
                    ...
                    if (statusLine == null) {
                        throw new AlfrescoRuntimeException(
                                name + " returned no status " + url + ' ' + args);
                    }
                    ...
                    if (resEntity != null) {
                        ...
                        if (statusCode == 200) {
                            try {
                                ...
                                throw new AlfrescoRuntimeException(
                                        name + " failed to read the returned content", e);
                            }
                        } else {
                        ...
                        throw new AlfrescoRuntimeException(
                                name + " did not return an entity " + url);
                    }
                } catch (IOException e) {
                    // In the case of transform requests, unlike version checks, it is only the
                    // failure to connect that
    ...
    /**
     * Indicates if a remote transform: a) ready probe has ever indicated success {@code new
     * Pair<>(true, <version string>)}, b) a ready probe has just failed {@code new Pair<>(false,
     * <error string>)}, or c) we are not performing a ready check as we have just done one {@code
     * new Pair<>(null, null)}.
     */
        ...
        try {
            try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
                try (CloseableHttpResponse response = execute(httpclient, httpGet)) {
                    ...
                    if (statusLine == null) {
                        throw new AlfrescoRuntimeException(
                                name + " check returned no status " + url);
                    }
                    HttpEntity resEntity = response.getEntity();
                    if (resEntity != null) {
                        int statusCode = statusLine.getStatusCode();
                        if (statusCode == 200) {
                            try {
                                ...
                                throw new AlfrescoRuntimeException(
                                        name + " check failed to read the returned content", e);
                            }
                        } else {
                            String message = getErrorMessage(resEntity);
                            throw new AlfrescoRuntimeException(
                        ...
                        throw new AlfrescoRuntimeException(
                                name + " check did not return an entity " + url);
                    }
                } catch (IOException e) {
                    throw new AlfrescoRuntimeException(
                ...
                throw new AlfrescoRuntimeException(
                        name + " check failed to create an HttpClient", e);
            }
        } catch (AlfrescoRuntimeException e) {
            // In the case of version checks, unlike transform requests, any failure forces a wait
            // before trying again.
            connectionFailed();
            if (logger.isDebugEnabled()) {
                logger.debug(e.getMessage());
    ...
    CloseableHttpResponse execute(CloseableHttpClient httpclient, HttpPost httppost)
            throws IOException {
        return httpclient.execute(httppost);
    }

    // Tests mock the return values
    CloseableHttpResponse execute(CloseableHttpClient httpclient, HttpGet httpGet)
            throws IOException {
    ...
    /**
     * Helper method that returns the same result type as {@link #check(Log)} given a local
     * checkCommand.
     */

...
public class ViewRecordTest extends BaseRMTestCase {
    /**
     * Given a user with read permission on a record and without read permission on the parent
     * folder check if the user can check if the record is filed
     *
                    ...
                    public void given() {
                        ...
                        record =
                                recordService.createRecordFromContent(
                                        recordFolder, GUID.generate(), TYPE_CONTENT, null, null);
                    }

                    public void when() {
                        // give read and file permissions on folder and remove permission from
                        // parent
                        filePlanPermissionService.setPermission(
                                rc, user, RMPermissionModel.READ_RECORDS);
                        permissionService.setInheritParentPermissions(recordFolder, false);
                        filePlanPermissionService.setPermission(
                                record, user, RMPermissionModel.READ_RECORDS);

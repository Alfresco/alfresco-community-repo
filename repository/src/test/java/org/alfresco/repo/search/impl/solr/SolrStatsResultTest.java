...
public class SolrStatsResultTest
{
    ...
    public static String TEST_MIMETYPE_EMPTY_BUCKET = "{\"responseHeader\":{\"status\":0,\"QTime\":12},\"response\":{\"numFound\":188,\"start\":0,\"docs\":[]},\"stats\":{\"stats_fields\":{\"contentsize\":{\"min\":25.0,\"max\":3737049.0,\"sum\":1.4742886E7,\"count\":179,\"missing\":9,\"sumOfSquares\":2.9213896971782E13,\"mean\":82362.49162011173,\"stddev\":396612.3128060779,\"facets\":{\"@{http://www.alfresco.org/model/content/1.0}content.mimetype\":{\"\":{\"min\":null,\"max\":null,\"mean\":\"NaN\",\"count\":0,\"missing\":9,\"sum\":0,\"sumOfSquares\":0,\"stddev\":0},\"text/plain\":{\"min\":55.0,\"max\":10734.0,\"sum\":377009.0,\"count\":106,\"missing\":0,\"sumOfSquares\":2.300122507E9,\"mean\":3556.688679245283,\"stddev\":3022.485361304367}}}}}},\"lastIndexedTx\":17}";
    public static String TEST_MIMETYPE_NULL_SUMMARY = "{\"responseHeader\":{\"status\":0,\"QTime\":5},\"response\":{\"numFound\":0,\"start\":0,\"docs\":[]},\"stats\":{\"stats_fields\":{\"contentsize\":{\"min\":null,\"max\":null,\"sum\":0,\"count\":0,\"missing\":9,\"mean\":\"NaN\",\"stddev\":0,\"facets\":{\"@{http://www.alfresco.org/model/content/1.0}content.mimetype\":{\"\":{\"min\":null,\"max\":null,\"mean\":\"NaN\",\"count\":0,\"missing\":9,\"sum\":0}}}}}},\"lastIndexedTx\":17}";
    ...
    @Test
    public void testSolrStatsResultSkipsEmptyMimetypeBucket() throws JSONException
    {
        JSONObject json = new JSONObject(new JSONTokener(TEST_MIMETYPE_EMPTY_BUCKET));
        SolrStatsResult result = new SolrStatsResult(json, false);

        assertNotNull(result);
        assertEquals(188, result.getNumberFound());
        assertEquals(1, result.getStats().size());
        assertEquals("text/plain", result.getStats().get(0).getName());
        assertEquals(Long.valueOf(106), result.getStats().get(0).getCount());
    }

    @Test
    public void testSolrStatsResultNullSummaryStats() throws JSONException
    {
        JSONObject json = new JSONObject(new JSONTokener(TEST_MIMETYPE_NULL_SUMMARY));
        SolrStatsResult result = new SolrStatsResult(json, false);

        assertNotNull(result);
        assertEquals(0, result.getNumberFound());
        assertEquals(0, result.getStats().size());
        assertEquals(Long.valueOf(0), result.getSum());
        assertEquals(Long.valueOf(0), result.getMax());
        assertEquals(Long.valueOf(0), result.getMean());
    }

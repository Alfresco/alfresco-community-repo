...
/**
 * Patch to add <i>cm:indexControl</i> aspect to sites' surf-config folders and their children as
 * well as to the shared surf-config folder(s) and its/their children.
 *
...
public class SurfConfigFolderPatch extends AsynchronousPatch {
    ...
    protected String applyInternal() throws Exception {
        ...
        BatchProcessWorker<Long> siteWorker =
                new BatchProcessWorkerAdaptor<Long>() {
                    @Override
        ...
        BatchProcessor<Long> siteBatchProcessor =
        ...
        // Instance to provide raw data to process
        BatchProcessWorkProvider<NodeRef> surfConfigWorkProvider =
                new SharedSurfConfigWorkProvider();

        // Instance to handle each item of work
        BatchProcessWorker<NodeRef> surfConfigWorker =
                new BatchProcessWorkerAdaptor<NodeRef>() {
                    @Override
                    public void beforeProcess() throws Throwable {
                        // Run as the systemuser
                        AuthenticationUtil.setRunAsUser(systemUser);
        ...
        BatchProcessor<NodeRef> surfConfigBatchProcessor =
                new BatchProcessor<NodeRef>(
                        "SurfConfigFolderPatch",
                        transactionService.getRetryingTransactionHelper(),
                        surfConfigWorkProvider,
                        SHARED_SURF_CONFIG_BATCH_THREADS,
                        BATCH_SIZE,
                        null,
                        progress_logger,
                        1000);
    ...
    private void process(long siteId) {
        ...
        Pair<Long, ChildAssociationRef> surfConfigPair =
                nodeDAO.getChildAssoc(siteId, ContentModel.ASSOC_CONTAINS, SURF_CONFIG);
        ...
        if (componentsNodeRef != null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Unable to find surf-config/components folder for site: ["
                                + siteName
                                + ']');
            }
        }
        ...
        NodeRef pagesNodeRef =
                nodeService.getChildByName(surfConfigNodeRef, ContentModel.ASSOC_CONTAINS, PAGES);
        if (pagesNodeRef == null) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Unable to find surf-config/pages folder for site: [" + siteName + ']');
            }
            return;
        }
        ...
        NodeRef siteNodeRef =
                nodeService.getChildByName(pagesNodeRef, ContentModel.ASSOC_CONTAINS, SITE);
        if (siteNodeRef == null) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Unable to find surf-config/pages/site folder for site: ["
                                + siteName
                                + ']');
            }
            return;
        ...
        NodeRef siteChildNodeRef =
                nodeService.getChildByName(siteNodeRef, ContentModel.ASSOC_CONTAINS, siteName);

        if (siteChildNodeRef == null) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Unable to find surf-config/pages/site/"
                                + siteName
                                + " folder for site: ["
                                + siteName
                                + ']');
        ...
        List<ChildAssociationRef> listOfComponents =
                nodeService.getChildAssocs(
                        siteChildNodeRef, ContentModel.ASSOC_CONTAINS, RegexQNamePattern.MATCH_ALL);
    ...
    /**
     * Work provider which performs incremental queries to find shared Surf-Config folders and their
     * children.
     *
     * @author Jamal Kaabi-Mofrad
     */
    private class SharedSurfConfigWorkProvider implements BatchProcessWorkProvider<NodeRef> {
        ...
        public synchronized Collection<NodeRef> getNextWork() {
            ...
            final List<NodeRef> folderNodes =
                    new ArrayList<NodeRef>(SHARED_SURF_CONFIG_BATCH_MAX_QUERY_RANGE);

            // Keep querying until we have enough results to give back
            int minResults = SHARED_SURF_CONFIG_BATCH_MAX_QUERY_RANGE / 2;
            while (currentId <= maxId && folderNodes.size() < minResults) {

                List<NodeRef> nodeIds =
                        patchDAO.getChildrenOfTheSharedSurfConfigFolder(

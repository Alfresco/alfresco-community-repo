...
public class RemoveSiteGroupTests extends RestTest {
  ...
  public void dataPreparation() throws DataPreparationException {
    ...
    privateSiteModel =
      dataSite.usingUser(siteCreator).createPrivateRandomSite();
  ...
  @TestRail(
    ...
    description = "Verify manager can delete site group and gets status code 204, 'No Content'"
  )
  public void siteManagerIsAbleToDeleteSiteGroup() throws Exception {
    GroupModel group = addGroupToSite(publicSiteModel, UserRole.SiteConsumer)
      .get(0);
    ...
    Utility.sleep(
      ...
      () ->
        restClient
          ...
          .getSiteGroups()
          .assertThat()
          .entriesListDoesNotContain("id", getId(group))
    );
  ...
  @TestRail(
    ...
    description = "Verify collaborator cannot delete site group and gets status code 403, 'Forbidden'"
  )
    ...
    Utility.sleep(
      ...
      () ->
        restClient
          ...
          .getSiteGroups()
          .assertThat()
          .entriesListContains("id", getId(group))
    );
    restClient.assertStatusCodeIs(HttpStatus.OK);
  }
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.SITES },
    executionType = ExecutionType.REGRESSION,
    description = "Verify contributor cannot delete site group and gets status code 403, 'Forbidden'"
  )
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.SITES },
    executionType = ExecutionType.REGRESSION,
    description = "Verify consumer cannot delete site member and gets status code 403, 'Forbidden'"
  )
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.SITES },
    executionType = ExecutionType.SANITY,
    description = "Verify that unauthenticated user is not able to delete site group"
  )
  public void unauthenticatedUserIsNotAuthorizedToDeleteSiteGroup()
    throws Exception {
    GroupModel group = addGroupToSite(publicSiteModel, UserRole.SiteConsumer)
      .get(0);

    UserModel inexistentUser = new UserModel(
      "inexistent user",
      "inexistent password"
    );
    restClient
  ...
  public void managerIsNotAbleToDeleteInexistentSiteGroup() throws Exception {
    ...
    restClient
      .withCoreAPI()
      .usingSite(publicSiteModel)
      .deleteSiteGroup(getId(inexistentUser));
    restClient
      .assertStatusCodeIs(HttpStatus.NOT_FOUND)
      .assertLastError()
      .containsSummary(
        String.format(
          "An authority was not found for %s",
          getId(inexistentUser)
        )
      );
  }
  ...
  public void managerIsNotAbleToDeleteNotSiteGroup() throws Exception {
    ...
    restClient
      .withCoreAPI()
      .usingSite(publicSiteModel)
      .deleteSiteGroup(getId(nonMember));
    restClient
      .assertStatusCodeIs(HttpStatus.BAD_REQUEST)
      .assertLastError()
      .containsSummary(
        String.format("Given authority is not a member of the site")
      );
  }
  ...
  @Test(groups = { TestGroup.REST_API, TestGroup.SITES, TestGroup.REGRESSION })
  public void managerIsNotAbleToDeleteSiteMemberOfInvalidSite()
    throws Exception {
    ...
    restClient
      .withCoreAPI()
      .usingSite(invalidSite)
      .deleteSiteGroup(getId(testGroupModel));
    restClient
      .assertStatusCodeIs(HttpStatus.NOT_FOUND)
      .assertLastError()
      .containsSummary(
        String.format(RestErrorModel.ENTITY_WAS_NOT_FOUND, "invalidSite")
      );
  }
  ...
  public void adminIsAbleToDeletePrivateSiteGroup() throws Exception {
    GroupModel group = addGroupToSite(privateSiteModel, UserRole.SiteConsumer)
      .get(0);
  ...
  public void adminIsNotAbleToRemoveSiteGroupTwice() throws Exception {
    GroupModel group = addGroupToSite(publicSiteModel, UserRole.SiteContributor)
      .get(0);

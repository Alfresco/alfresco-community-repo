...
public class SOLRTrackingComponentTest extends BaseSpringTest {
  ...
  private static final Log logger = LogFactory.getLog(
    SOLRTrackingComponentTest.class
  );
  ...
  public void before() throws Exception {
    ...
    solrTrackingComponent =
      (SearchTrackingComponent) applicationContext.getBean(
    ...
    dbNodeService =
      (DbNodeServiceImpl) applicationContext.getBean("dbNodeService");
  ...
  public void testAclChangeSetLimits() {
    ...
    List<AclChangeSet> testSets = getAclChangeSets(
      first,
      null,
      first,
      null,
  ...
  public void testGetNodeMetaData() {
    ...
    List<Transaction> checkedTransactions = checkTransactions(
      txns,
      createdTransactions,
      updates,
      deletes
  ...
  private List<Transaction> getTransactions(
    ...
    RetryingTransactionCallback<List<Transaction>> callback = new RetryingTransactionCallback<List<Transaction>>() {
      ...
      public List<Transaction> execute() throws Throwable {
        return solrTrackingComponent.getTransactions(
          minTxnId,
          fromCommitTime,
          maxTxnId,
          toCommitTimeint,
  ...
  private void getNodes(
    final NodeParameters nodeParameters,
    final SOLRTest bt
  ) {
    long startTime = System.currentTimeMillis();
    transactionService
  ...
  private List<Acl> getAcls(
    ...
    RetryingTransactionCallback<List<Acl>> callback = new RetryingTransactionCallback<List<Acl>>() {
      ...
      public List<Acl> execute() throws Throwable {
        return solrTrackingComponent.getAcls(
          aclChangeSetIds,
          minAclId,
          maxResults
        );
  ...
  private List<AclChangeSet> getAclChangeSets(
    ...
    RetryingTransactionCallback<List<AclChangeSet>> callback = new RetryingTransactionCallback<List<AclChangeSet>>() {
      ...
      public List<AclChangeSet> execute() throws Throwable {
        return solrTrackingComponent.getAclChangeSets(
          minAclChangeSetId,
          fromCommitTime,
          maxAclChangeSetId,
          toCommitTime,
  ...
  public void testGetNodeMetaDataExludesResidualProperties() {
    ...
    SOLRTest st = new SOLRTestResidualProperties(
      txnHelper,
      fileFolderService,
      nodeDAO,
      qnameDAO,
      ...
      dictionaryService,
      rootNodeRef,
      "testNodeMetaDataNullPropertyValue",
      true,
      true
    );
    List<Long> createdTransactions = st.buildTransactions();
  ...
  public void testGetNodeMetaData100Nodes() {
    ...
    SOLRTest st = new SOLRTest100Nodes(
      txnHelper,
      fileFolderService,
      nodeDAO,
      qnameDAO,
  ...
  public void testModelDiffs() {
    ...
    assertEquals(
      "Unexpected number of new models",
      allModels.size(),
      diffResults.getNewModels().size()
    );
    assertEquals(
      "Expected no removed models",
      0,
      diffResults.getRemovedModels().size()
    );
    assertEquals(
      "Expected no changed models",
      0,
      diffResults.getChangedModels().size()
    );

    // create a new model
    InputStream modelStream = getClass()
      .getClassLoader()
      .getResourceAsStream("org/alfresco/repo/solr/testModel.xml");
    ...
    assertEquals(
      "Unexpected number of changed models",
      0,
      diffResults1.getChangedModels().size()
    );
    assertEquals(
      "Unexpected number of removed models",
      0,
      diffResults1.getRemovedModels().size()
    );
    AlfrescoModelDiff diff = diffResults1.getNewModels().get(0);
    assertEquals(
      "Unexpected model name change",
      QName.createQName(testModel.getName(), namespaceService).toString(),
      diff.getModelName()
    );

    // get current checksum for the test model
    Long testModelChecksum = tracker.getChecksum(
      QName.createQName(testModel.getName(), namespaceService)
    );
    ...
    assertEquals(
      "Expected no new models",
      0,
      diffResults2.getNewModels().size()
    );
    assertEquals(
      "Expected no removed models",
      0,
      diffResults2.getRemovedModels().size()
    );
    assertEquals(
      "Expected detection of changed testmodel",
      1,
      changedModels.size()
    );
    ...
    assertEquals(
      "Unexpected changed model name",
      QName.createQName(testModel.getName(), namespaceService).toString(),
      changedModel.getModelName()
    );
    assertNotNull("", changedModel.getOldChecksum().longValue());
    assertEquals(
      "Old checksum value is incorrect",
      testModelChecksum.longValue(),
      changedModel.getOldChecksum().longValue()
    );
    assertNotSame(
      "Expected checksums to be different",
      changedModel.getOldChecksum(),
      changedModel.getNewChecksum()
    );

    // remove the model
    dictionaryDAO.removeModel(
      QName.createQName(testModel.getName(), namespaceService)
    );
    ...
    QName removedModelName = QName.createQName(
      removedModels.get(0).getModelName()
    );
    String removedModelNamespace = removedModelName.getNamespaceURI();
    String removedModelLocalName = removedModelName.getLocalName();
    assertEquals(
      "Removed model namespace is incorrect",
      "http://www.alfresco.org/model/solrtest/1.0",
      removedModelNamespace
    );
    assertEquals(
      "Removed model name is incorrect",
      "contentmodel",
      removedModelLocalName
    );
    assertEquals(
      "Expected no new models",
      0,
      diffResults3.getNewModels().size()
    );
    assertEquals(
      "Expected no changed modeks",
      0,
      diffResults3.getChangedModels().size()
    );
  }
  ...
  private static class ModelDiffResults {
    ...
    public ModelDiffResults(
      List<AlfrescoModelDiff> newModels,
      List<AlfrescoModelDiff> changedModels,
      List<AlfrescoModelDiff> removedModels
    ) {
      super();
  ...
  private class ModelDiffsTracker {
    ...
    public ModelDiffResults diff() {
      List<AlfrescoModelDiff> modelDiffs = solrTrackingComponent.getModelDiffs(
        trackedModels
      );
      ...
      for (AlfrescoModelDiff diff : modelDiffs) {
        if (diff.getType().equals(AlfrescoModelDiff.TYPE.NEW)) {
          newModels.add(diff);
          trackedModels.put(
            QName.createQName(diff.getModelName()),
            diff.getNewChecksum()
          );
  ...
  private List<Transaction> checkTransactions(
    List<Transaction> txns,
    List<Long> createdTransaction,
    int[] updates,
    int[] deletes
  ) {
    ...
    assertEquals(
      "Number of transactions is incorrect",
      createdTransaction.size(),
    ...
    for (Transaction txn : matchedTransactions) {
      assertEquals(
        "Number of deletes is incorrect",
        deletes[i],
        txn.getDeletes()
      );
      assertEquals(
        "Number of updates is incorrect",
        updates[i],
        txn.getUpdates()
      );
      i++;
  ...
  private void getNodeMetaData(
    final NodeMetaDataParameters params,
    final MetaDataResultsFilter filter,
    final SOLRTest bt
  ) {
  ...
  private abstract static class SOLRTest
    implements NodeQueryCallback, NodeMetaDataQueryCallback {
    ...
    SOLRTest(
      ...
      boolean doNodeChecks,
      boolean doMetaDataChecks
    ) {
    ...
    void runNodeChecks(int maxResults) {
      if (doNodeChecks) {
        if (maxResults != 0 && maxResults != Integer.MAX_VALUE) {
          assertEquals(
            "Number of returned nodes is incorrect",
            maxResults,
            getActualNodeCount()
          );
        } else {
    ...
    void runNodeMetaDataChecks(int maxResults) {
      if (maxResults != 0 && maxResults != Integer.MAX_VALUE) {
        assertEquals(
          "Number of returned nodes is incorrect",
          maxResults,
          getActualNodeMetaDataCount()
        );
      } else {
        assertEquals(
          "Number of returned nodes is incorrect",
          getExpectedNumMetaDataNodes(),
          getActualNodeMetaDataCount()
        );
      }
    }
    ...
    protected void setExpectedNodeStatus(
      NodeRef nodeRef,
      ...
      if (doNodeChecks) {
        ...
        if (expectedStatus == null) {
          throw new RuntimeException(
            "Unexpected missing assertion for NodeRef " + nodeRef
          );
        }

        if (
          (expectedStatus.getNodeStatus() == NodeStatus.DELETED && isDeleted) ||
    ...
    private Map<QName, Serializable> filterResudualProperties(
      Map<QName, Serializable> sourceProps
    ) {
      Map<QName, Serializable> props = new HashMap<QName, Serializable>(
        (int) (sourceProps.size() * 1.3)
      );
    ...
    public boolean handleNodeMetaData(NodeMetaData nodeMetaData) {
      ...
      if (doMetaDataChecks) {
        ...
        if (nodeService.exists(nodeRef)) {
          ...
          Map<QName, Serializable> actualProperties = filterResudualProperties(
            nodeDAO.getNodeProperties(nodeId)
          );
          ...
          assertTrue(
            "Expecting paths but didn't get any.",
            expectPaths == (nodeMetaData.getPaths() != null)
          );
          assertTrue(
            "Expecting name path but didn't get it.",
            expectPaths == (nodeMetaData.getNamePaths() != null)
          );
          if (expectPaths) {
            // Check QName paths
            // TODO: Check paths
            ...
            if (
              nodeService.getProperty(nodeRef, ContentModel.PROP_NAME) == null
            ) {
              assertEquals(
                "Expect an empty list where there is no cm:name on the node. ",
                0,
                namePaths.size()
              );
            } else {
              assertTrue(
                "Expect some name paths for the node. ",
                namePaths.size() > 0
              );
              ...
              assertTrue(
                "Did not find node name at end.  Have " +
                namePathStr +
                " and wanted to see " +
                name,
          ...
          if (expectProperties && nodeMetaData.getProperties() == null) {
            ...
            !expectProperties && nodeMetaData.getProperties() != null
          ) {
          ...
          if (expectAssociations && nodeMetaData.getChildAssocs() == null) {
            ...
            !expectAssociations && nodeMetaData.getChildAssocs() != null
          ) {
        ...
        txnHelper.doInTransaction(
          new RetryingTransactionCallback<Long>() {
            public Long execute() throws Throwable {
              ...
              FileInfo contentInfo = fileFolderService.create(
                container,
                "Content1",
  ...
  public void testGetNodeMetaDataWithNoType() {
    ...
    SOLRTest st = new SOLRTestWithNoType(
      txnHelper,
      fileFolderService,
      nodeDAO,
      qnameDAO,

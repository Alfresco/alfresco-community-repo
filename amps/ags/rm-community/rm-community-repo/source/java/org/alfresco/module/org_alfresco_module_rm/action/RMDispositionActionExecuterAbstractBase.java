...
public abstract class RMDispositionActionExecuterAbstractBase extends RMActionExecuterAbstractBase {
     ...
     *
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.action.RMActionExecuterAbstractBase#isDispositionAction()
     */
    @Override
    ...
    /**
     * Indicates whether we should validate the next disposition action is the action we are trying
     * to execute.
     *
     * @return
     */
    ...
    protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
        ...
        // Check the validity of the action (is it the next action, are we dealing with the correct
        // type of object for
        // the disposition level?
        DispositionSchedule di =
                checkDispositionActionExecutionValidity(
                        actionedUponNodeRef, nextDispositionActionNodeRef, checkError);
        if (di != null) {
            ...
            if (!checkEligibility(actionedUponNodeRef)
                ...
                throw new AlfrescoRuntimeException(
                        I18NUtil.getMessage(
                                MSG_NOT_ELIGIBLE, getName(), actionedUponNodeRef.toString()));
            }
        }
    }
    ...
    protected abstract void executeRecordFolderLevelDisposition(
            Action action, NodeRef recordFolder);
    ...
    protected DispositionSchedule checkDispositionActionExecutionValidity(
            NodeRef nodeRef, NodeRef nextDispositionActionNodeRef, boolean throwError) {
        ...
        if (di == null) {
            if (throwError) {
                throw new AlfrescoRuntimeException(
                        I18NUtil.getMessage(
                                MSG_NO_DISPOITION_INSTRUCTIONS, getName(), nodeRef.toString()));
            } else {
                return null;
            }
        }
        ...
        if (!getNodeService().hasAspect(nodeRef, ASPECT_DISPOSITION_LIFECYCLE)) {
            if (throwError) {
                throw new AlfrescoRuntimeException(
                        I18NUtil.getMessage(
                                MSG_NO_DIS_LIFECYCLE_SET, getName(), nodeRef.toString()));
            } else {
                return null;
            }
        }
        ...
        if (checkNextDispositionAction(nodeRef)) {
            ...
            if (nextDispositionAction == null) {
                if (throwError) {
                    throw new AlfrescoRuntimeException(
                            I18NUtil.getMessage(
                                    MSG_NEXT_DISP_NOT_SET, getName(), nodeRef.toString()));
                } else {
                    return null;
                }
            }
            ...
            if (actionName == null || !actionName.equals(getName())) {
                if (throwError) {
                    throw new AlfrescoRuntimeException(
                            I18NUtil.getMessage(MSG_NOT_NEXT_DISP, getName(), nodeRef.toString()));
                } else {
                    return null;
                }
            }

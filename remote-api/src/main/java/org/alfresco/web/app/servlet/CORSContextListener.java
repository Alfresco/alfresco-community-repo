...
/**
 * The CORSContextListener adds the Tomcat CORS filter org.apache.catalina.filters.CorsFilter which
 * allows to enable/disable and configure Cross-Origin Resource Sharing (CORS) with alfresco
 * properties.
 */
public class CORSContextListener implements ServletContextListener {
    ...
    private final EnumSet<DispatcherType> DISPATCHER_TYPE =
    ...
    private void initCORS(ServletContext servletContext) {
        ...
        if (corsEnabled) {
            ...
            corsFilter.setInitParameter(
                    CORS_SUPPORT_CREDENTIALS, gP.getProperty(CORS_SUPPORT_CREDENTIALS));
            corsFilter.setInitParameter(
                    CORS_PREFLIGHT_CREDENTIALS, gP.getProperty(CORS_PREFLIGHT_CREDENTIALS));

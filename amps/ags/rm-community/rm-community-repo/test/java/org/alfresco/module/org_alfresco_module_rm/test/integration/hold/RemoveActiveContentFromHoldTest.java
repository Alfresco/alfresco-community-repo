...
public class RemoveActiveContentFromHoldTest extends BaseRMTestCase {
    ...
    /**
     * Given a piece of active content on hold When I try to remove the active content from the hold
     * Then the active content is unfrozen And the active content is not contained within the hold
     */
        ...
        doBehaviourDrivenTest(
                new BehaviourDrivenTest() {
                    ...
                    public void when() {
                        before =
                                (Integer)
                                        nodeService.getProperty(dmFolder, PROP_HELD_CHILDREN_COUNT);
                                ...
                                before
                                        > (Integer)
                                                nodeService.getProperty(
                                                        dmFolder, PROP_HELD_CHILDREN_COUNT));
                    }
                });
    }

    /**
     * Given a piece of active content in multiple holds When I try to remove the active content
     * from a single hold Then the active content is still frozen And the active content is not
     * contained within the specified hold And is still added to any other holds
     */
    ...
    /**
     * Given a piece of active content on hold When I try to remove the active content from the hold
     * without permission Then an access denied exception is thrown
     */
    ...
    /**
     * Given a piece of active content on hold When I try to remove the active content from the hold
     * without the remove hold capability Then an access denied exception is thrown
     */

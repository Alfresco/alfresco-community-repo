...
public class NodeEntity implements Node, PermissionCheckValue, Serializable, Cloneable {
    ...
    public String toString() {
        ...
        sb.append("NodeEntity").append("[ ID=").append(id).append(", version=").append(version);
        if (store != null) {
            sb.append(", store=")
                    .append(store.getProtocol())
                    .append("://")
                    .append(store.getIdentifier());
        } else {
    ...
    public synchronized void lock() {
        ...
        // Help to avoid NPEs e.g. MNT-10486: NPE in NodeEntity during post-commit write through to
        // shared cache
        if (id == null || version == null) {
            throw new IllegalStateException("The NodeEntity has not be filled: " + this);
        }
    ...
    public NodeRef.Status getNodeStatus(QNameDAO qnameDAO) {
        ...
        return new NodeRef.Status(
                id, nodeRef, transaction.getChangeTxnId(), transaction.getId(), deleted);
    }
    ...
    public boolean getDeleted(QNameDAO qnameDAO) {
        ...
        return deletedTypeQNamePair != null && deletedTypeQNamePair.getFirst().equals(typeQNameId);
    }

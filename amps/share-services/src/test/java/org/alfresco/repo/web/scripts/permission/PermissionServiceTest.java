...
public class PermissionServiceTest extends BaseWebScriptTest {
  ...
  private static final String URL_DOCLIB_PERMISSIONS =
    "/slingshot/doclib/permissions";
  ...
  public void testDowngradePermissions() throws Exception {
    NodeRef rootNodeRef =
      this.nodeService.getRootNode(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE);
    NodeRef folderRef = nodeService
      .createNode(
        rootNodeRef,
    ...
    AccessStatus accessStatus = permissionService.hasPermission(
      folderRef,
      PermissionService.CONSUMER
    );
    assertTrue(
      "The permission was not set correctly",
  ...
  public void testMultipleInheritedPermissions() throws Exception {
    ...
    NodeRef siteContainer = siteService.createContainer(
      siteName,
    ...
    NodeRef folder1 = fileFolderService
      .create(siteContainer, "Folder1", ContentModel.TYPE_FOLDER)
      .getNodeRef();
    permissionService.setPermission(
      folder1,
      USER_ONE,
    ...
    NodeRef folder2 = fileFolderService
      .create(folder1, "Folder2", ContentModel.TYPE_FOLDER)
      .getNodeRef();
    permissionService.setPermission(
      folder2,
      USER_ONE,
    ...
    NodeRef folder3 = fileFolderService
      .create(folder2, "Folder3", ContentModel.TYPE_FOLDER)
      .getNodeRef();
    permissionService.setPermission(
      folder3,
      USER_ONE,
    ...
    for (int i = 0; i < directPermissionsArray.length(); i++) {
      AccessPermission permission = new AccessPermission(
        directPermissionsArray.getJSONObject(i)
      );
    ...
    JSONArray inheritedPermissionsArray = jsonResponse.getJSONArray(
      "inherited"
    );
    for (int i = 0; i < inheritedPermissionsArray.length(); i++) {
      AccessPermission permission = new AccessPermission(
        inheritedPermissionsArray.getJSONObject(i)
      );
      if (
        USER_TWO.equals(permission.getAuthority().getName()) &&
        PermissionService.CONTRIBUTOR.equals(permission.getRole())
      ) {
        denyPermissionInheritedTest = permission;
      }
      if (
        USER_THREE.equals(permission.getAuthority().getName()) &&
        PermissionService.CONSUMER.equals(permission.getRole())
      ) {
...
class AccessPermission {
  ...
  public AccessPermission(JSONObject jsonPermission) throws JSONException {
    authority =
      new PermissionAuthority(jsonPermission.getJSONObject("authority"));
  ...
  public boolean equals(Object obj) {
    ...
    return (
      role.equals(((AccessPermission) obj).getRole()) &&
      authority.equals(((AccessPermission) obj).getAuthority())
    );
  }
...
class PermissionAuthority {
  ...
  public boolean equals(Object obj) {
    ...
    return (
      name.equals(((PermissionAuthority) obj).getName()) &&
      displayName.equals(((PermissionAuthority) obj).getDisplayName())
    );
  }

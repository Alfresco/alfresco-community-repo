...
public class RenditionsDataCollectorTest {
  ...
  private ThumbnailDefinition doclib = new ThumbnailDefinition(
    "png",
    options,
    "doclib"
  );
  private ThumbnailDefinition preview = new ThumbnailDefinition(
    "pdf",
    options,
    "preview"
  );
  ...
  public void testCollectedDataInDetail() {
    ...
    assertEquals(
      "There should have been 3 data elements",
      3,
      collectedData.size()
    );
  ...
  public void testMultipleCollections() throws InterruptedException {
    ...
    collectedData = renditionsDataCollector.collectData();
    assertEquals(
      "There should have been 0 data elements",
      0,
      collectedData.size()
    );
    ...
    collectedData = renditionsDataCollector.collectData();
    assertEquals(
      "There should have been 1 data element",
      1,
      collectedData.size()
    );
  ...
  private boolean assertHBDataContains(
    ...
    for (HBData data : collectedData) {
      ...
      if (
        rendition.equals(values.get("rendition")) &&

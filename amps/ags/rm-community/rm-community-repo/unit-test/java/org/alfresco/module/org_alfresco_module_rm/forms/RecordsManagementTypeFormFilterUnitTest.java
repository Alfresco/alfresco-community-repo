...
public class RecordsManagementTypeFormFilterUnitTest extends BaseUnitTest {
  ...
  public void testAfterGenerateNotCustomisable() {
    ...
    when(mockRecordsManagementAdminService.isCustomisable(MY_CUSTOM_TYPE))
      .thenReturn(false);

    typeFormFilter.afterGenerate(
      mockTypeDefinition,
      null,
      null,
      mockForm,
    ...
    verify(typeFormFilter, never())
      .addCustomRMProperties(any(QName.class), any(Form.class));
  }
  ...
  public void testDefaultFormValues() {
    ...
    verify(mockedIdentifierService)
      .generateIdentifier(any(QName.class), any(NodeRef.class));
  ...
  public void testAddCustomRMProperties() {
    ...
    when(
      mockRecordsManagementAdminService.getCustomPropertyDefinitions(
        MY_CUSTOM_TYPE
      )
    )
  ...
  private Map<QName, PropertyDefinition> mockPropertyDefintionMap(int size) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      DataTypeDefinition mockDataTypeDefinition = mock(
        DataTypeDefinition.class
      );
      when(mockDataTypeDefinition.getName())
        .thenReturn(DataTypeDefinition.TEXT);

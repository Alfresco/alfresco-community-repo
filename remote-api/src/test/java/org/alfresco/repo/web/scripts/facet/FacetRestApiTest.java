...
public class FacetRestApiTest extends BaseWebScriptTest {
  ...
  private static final String GET_ALL_FACETABLE_PROPERTIES_URL =
    "/api/facet/facetable-properties";
  private static final String GET_SPECIFIC_FACETABLE_PROPERTIES_URL =
    "/api/facet/classes/{classname}/facetable-properties";
  private static final String GET_FACETS_URL = "/api/facet/facet-config";
  private static final String PUT_FACET_URL_FORMAT =
    "/api/facet/facet-config/{0}?relativePos={1}";
  private static final String POST_FACETS_URL = GET_FACETS_URL;
  private static final String PUT_FACETS_URL = GET_FACETS_URL;
        ...
        public Void doWork() throws Exception {
          ...
          if (
            !authorityService
              ...
              .contains(
                SolrFacetServiceImpl.GROUP_ALFRESCO_SEARCH_ADMINISTRATORS_AUTHORITY
              )
          ) {
  ...
  public void testNonSearchAdminUserCannotCreateUpdateSolrFacets()
    throws Exception {
    // Create a filter
      ...
      new RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          Response response = sendRequest(
            new GetRequest(GET_FACETS_URL + "/" + filterName),
            200
          );
          JSONObject jsonRsp = new JSONObject(
            new JSONTokener(response.getContentAsString())
          );
    ...
    AuthenticationUtil.runAs(
      new RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          ...
          List<String> filters = getListFromJsonArray(
            jsonRsp.getJSONArray(FACETS)
          );
    ...
    AuthenticationUtil.runAs(
      new RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          ...
          assertTrue(
            "There should be more than 1 built-in facet",
            facetsArray.length() > 1
          );

          final String lastIndexId = idsIndexes.get(idsIndexes.size() - 1);
          final String url = PUT_FACET_URL_FORMAT
            .replace("{0}", lastIndexId)
            .replace("{1}", "-1");
          ...
          final List<String> newIdsIndexes = getListFromJsonArray(
            newfacetsArray
          );
          ...
          assertEquals(
            CollectionUtils.moveLeft(1, lastIndexId, idsIndexes),
            newIdsIndexes
          );
          return null;
        }
    ...
    AuthenticationUtil.runAs(
      new RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          ...
          filter_one.put(
            "displayControl",
            "alfresco/search/FacetFilters/test1"
          );
          ...
          response =
            sendRequest(
              new GetRequest(GET_FACETS_URL + "/" + filterNameOne),
              200
            );
          JSONObject jsonRsp = new JSONObject(
            new JSONTokener(response.getContentAsString())
          ...
          assertEquals(
            "{http://www.alfresco.org/model/content/1.0}test1",
            jsonRsp.getString("facetQName")
          );
          assertEquals(
            "facet-menu.facet.test1",
            jsonRsp.getString("displayName")
          );
          assertEquals(
            "alfresco/search/FacetFilters/test1",
            jsonRsp.getString("displayControl")
          );
          assertEquals(5, jsonRsp.getInt("maxFilters"));
          ...
          filter_two.put(
            "displayControl",
            "alfresco/search/FacetFilters/test2"
          );
          ...
          response =
            sendRequest(
              new GetRequest(GET_FACETS_URL + "/" + filterNameTwo),
              200
            );
          jsonRsp =
            new JSONObject(new JSONTokener(response.getContentAsString()));
    ...
    AuthenticationUtil.runAs(
      new RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          ...
          // Retrieve the created filter
          response =
            sendRequest(new GetRequest(GET_FACETS_URL + "/" + filterName), 200);
          JSONObject jsonRsp = new JSONObject(
            new JSONTokener(response.getContentAsString())
          );
          ...
          blockIncludeRequest =
            customProp.getJSONObject("blockIncludeFacetRequest");
          assertEquals(
            "{http://www.alfresco.org/model/solrfacetcustomproperty/1.0}blockIncludeFacetRequest",
            blockIncludeRequest.get("name")
          );
          assertEquals("true", blockIncludeRequest.get("value"));

          multipleValue = customProp.getJSONObject("multipleValueTest");
          assertEquals(
            "{http://www.alfresco.org/model/solrfacetcustomproperty/1.0}multipleValueTest",
            multipleValue.get("name")
          );
    ...
    AuthenticationUtil.runAs(
      new RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          ...
          filter.put(
            "filterID",
            "<script>alert('Maliciouse-FilterID')</script>"
          );
          // Post the filter
          sendRequest(
            new PostRequest(
  ...
  public void testCreateFacetWithLongFormQnameFilterId() throws Exception {
    ...
    filter.put(
      "facetQName",
      "{http://www.alfresco.org/model/content/1.0}testLongQname"
    );
    ...
    AuthenticationUtil.runAs(
      new RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          ...
          assertEquals(
            "facet-menu.facet.modifiedValue",
            jsonRsp.getString("displayName")
          );
          assertFalse(jsonRsp.getBoolean("isEnabled"));
          ...
          assertEquals(
            "alfresco/search/FacetFilters/test",
            jsonRsp.getString("displayControl")
          );
          assertEquals(5, jsonRsp.getInt("maxFilters"));
          assertEquals(1, jsonRsp.getInt("hitThreshold"));
          ...
          JSONObject retrievedCustomProp = jsonRsp.getJSONObject(
            "customProperties"
          );
          JSONObject retrievedBlockIncludeRequest = retrievedCustomProp.getJSONObject(
            "blockIncludeFacetRequest"
          );
          assertEquals(
            "{http://www.alfresco.org/model/solrfacetcustomproperty/1.0}blockIncludeFacetRequest",
            retrievedBlockIncludeRequest.get("name")
          );
          ...
          assertEquals(
            "{http://www.alfresco.org/model/content/1.0}testModifiedValue",
            jsonRsp.getString("facetQName")
          );
          assertNull(
            "Custom properties should have been deleted.",
            jsonRsp.opt("customProperties")
          );
          // Make sure the rest of values haven't been changed
          assertEquals(filterName, jsonRsp.getString("filterID"));
          assertEquals(
  ...
  public void testGetAllFacetableProperties() throws Exception {
    AuthenticationUtil.runAs(
      new RunAsWork<Void>() {
        @Override
        public Void doWork() throws Exception {
          final Response rsp = sendRequest(
            new GetRequest(GET_ALL_FACETABLE_PROPERTIES_URL),
            200
          );
          ...
          JSONArray properties = data.getJSONArray(
            FacetablePropertiesGet.PROPERTIES_KEY
          );

          final int arbitraryLimit = 25;
          assertTrue(
            "Expected 'many' properties, but found 'not very many'",
            properties.length() > arbitraryLimit
          );
  ...
  public void testGetFacetablePropertiesForSpecificContentClasses()
    throws Exception {
    AuthenticationUtil.runAs(
      new RunAsWork<Void>() {
        @Override
        public Void doWork() throws Exception {
          final Response rsp = sendRequest(
            new GetRequest(
              GET_SPECIFIC_FACETABLE_PROPERTIES_URL.replace(
                "{classname}",
                "cm:content"
              )
            ),
          ...
          assertTrue(
            "Expected 'not very many' properties, but found 'many'",
            properties.length() < arbitraryLimit
          );
  ...
  private List<String> getListFromJsonArray(JSONArray facetsArray)

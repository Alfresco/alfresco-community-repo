...
public class SortConstraint extends VirtualQueryConstraintDecorator {
    private static final Set<QName> IGNORED_SORT_PROPERTIES =
            new HashSet<>(Arrays.asList(GetChildrenCannedQuery.SORT_QNAME_NODE_IS_FOLDER));

    private List<Pair<QName, Boolean>> sortProps;

    public SortConstraint(
            VirtualQueryConstraint decoratedConstraint, List<Pair<QName, Boolean>> sortProps) {

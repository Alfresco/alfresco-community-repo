 ...
 *
 * <p>Checks are ignored if the target node has been deleted.
 *
 * @author Derek Hulley
 */
        ...
        if (targetDef instanceof TypeDefinition) {
            ...
            if (!found) {
                ...
                if (assocDef.isChild()) {
                    ...
                    if (nodeService.getSourceAssocs(targetNodeRef, assocDef.getName()).size()
                            == 0) {
                        // The association does not exist any more
                        return;
                    }
                ...
                if (nodeService.getSourceAssocs(targetNodeRef, assocDef.getName()).size() > 0) {
                    IntegrityRecord result =
                            new IntegrityRecord(
                                    "The association target is missing the aspect required for this"
                                            + " association: \n"
                                            + "   Target Node: "
                                            + targetNodeRef
                                            + "\n"
                                            ...
                                            + assocDef
                                            + "\n"
                                            + "   Required Target Aspect: "
                                            + targetDef.getName()
                                            + "\n"
                                            + "   Actual Target Aspects: "
                                            + targetAspects);
                    eventResults.add(result);
                }
            }

...
/**
 * Value class describing the lock state of a node. Lock specific properties may be added using the
 * {@link #additionalInfo} field - <strong>objects</strong> assigned to this field
 * <strong>MUST</strong> implement hashCode and equals methods properly.
 *
 * @author Matt Ward
 */
public final class LockState implements Serializable {
    ...
    public static LockState createLock(
            NodeRef nodeRef,
            LockType lockType,
            String owner,
            Date expires,
    ...
    /**
     * Returns whether this {@link LockState} is for a lock or whether there is no lock defined for
     * the node. If a lock is defined for a node, that does not mean that the node <em>is</em>
     * locked - the {@link org.alfresco.service.cmr.lock.LockService} must be used to determine
     * that.
     *

...
public class RetryingTransactionHelperTest extends BaseSpringTest {
  ...
  private static final QName PROP_CHECK_VALUE = QName.createQName(
  ...
  private Long getCheckValue() {
    Long checkValue = (Long) nodeService.getProperty(
      workingNodeRef,
      PROP_CHECK_VALUE
    );
    if (checkValue == null) {
  ...
  public void testSuccessNoRetry() {
    ...
    assertEquals(
      "The txn value must be the same as the value after",
      afterValue,
      txnValue
    );
  }
        ...
        RetryingTransactionCallback<Long> callbackInner = new RetryingTransactionCallback<Long>() {
          public Long execute() throws Throwable {
            ...
            try {
              txnInner.commit();
              fail(
                "Should not be able to commit the UserTransaction.  It is for info only."
              );
            } catch (Throwable e) {
              // Expected
            }
  ...
  public void testNestedWithoutPropagationConcurrentUntilFailureMySQL()
    ...
    if (!(dialect instanceof MySQLInnoDBDialect)) {
      ...
      logger.warn(
        "NOTE: Skipping testNestedWithoutPropogationConcurrentUntilFailureMySQLOnly for dialect: " +
        dialect
      );
    } else {
      RetryingTransactionCallback<Long> callback = new RetryingTransactionCallback<Long>() {
      ...
      try {
        ...
        Throwable validCause = ExceptionStackUtil.getCause(
          e,
          RetryingTransactionHelper.RETRY_EXCEPTIONS
        );
  ...
  private class ConcurrentTransaction implements Runnable {
    ...
    public void run() {
      ...
      try {
        System.out.println(
          "Txn start: " + Thread.currentThread() + " (" + wait + ")"
        );
        txnHelperForTest.doInTransaction(callback);
        System.out.println(
          "Txn finish: " + Thread.currentThread() + " (" + wait + ")"
        );
      } catch (Throwable e) {
        Throwable validCause = ExceptionStackUtil.getCause(
          e,
  ...
  public void testTimeLimit() {
    ...
    final List<Throwable> caughtExceptions = Collections.synchronizedList(
    ...
    assertTrue(
      "Excpected TooBusyException",
      caughtExceptions.get(0) instanceof TooBusyException
    );
  ...
  public void testForceWritable() throws Exception {
    ...
    txnService.setAllowWrite(
      false,
      QName.createQName("{test}testForceWritable")
    );
    try {
      ...
      try {
        ...
        fail(
          "Inner, non-propagating transactions should still fall foul of the write veto."
        );
  ...
  public void testStartNewTransaction() throws Exception {
    ...
    assertFalse(
      "New transaction has not started",
      txnId.equals(listener.newTxnId)
    );
  }
  ...
  private void runThreads(
    ...
    for (Pair<Integer, Integer> pair : startDurationPairs) {
      ...
      Runnable work = new Work(
        startLatch,
        startTime + currentStart + pair.getSecond()
      );

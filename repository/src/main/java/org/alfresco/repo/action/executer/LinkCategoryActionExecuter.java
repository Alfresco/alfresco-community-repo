        ...
        if (this.nodeService.exists(actionedUponNodeRef) == true) {
            ...
            if (this.dictionaryService.isSubClass(categoryAspect, ContentModel.ASPECT_CLASSIFIABLE)
                ...
                for (Map.Entry<QName, PropertyDefinition> entry : propertyDefs.entrySet()) {
                    if (DataTypeDefinition.CATEGORY.equals(entry.getValue().getDataType().getName())
                            == true) {
                ...
                if (categoryAspect != null) {
                    if (this.nodeService.hasAspect(actionedUponNodeRef, categoryAspect) == false) {
                        ...
                        Serializable value =
                                this.nodeService.getProperty(actionedUponNodeRef, categoryProperty);
                        Collection<NodeRef> categories = null;
                        if (value == null) {
                            categories =
                                    DefaultTypeConverter.INSTANCE.getCollection(
                                            NodeRef.class, categoryValue);
                        } else {
                            categories =
                                    DefaultTypeConverter.INSTANCE.getCollection(
                                            NodeRef.class, value);
                        ...
                        this.nodeService.setProperty(
                                actionedUponNodeRef, categoryProperty, (Serializable) categories);
                    }
                }
            }
        }

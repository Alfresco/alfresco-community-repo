...
public class AclDAOImpl implements AclDAO {
   ...
   *
   */
  public void setAclCache(
    SimpleCache<Serializable, AccessControlList> aclCache
  ) {
    ...
    switch (properties.getAclType()) {
      ...
      case SHARED:
        throw new IllegalArgumentException(
          "Can not create shared acls direct - use get inherited"
        );
      case DEFINING:
      case LAYERED:
        break;
   ...
   */
  @Override
  public Acl createAccessControlList(
    ...
    if ((aces != null) && aces.size() > 0) {
      for (AccessControlEntry ace : aces) {
        ...
        // Find authority
        Authority authority = aclCrudDAO.getOrCreateAuthority(
          ace.getAuthority()
        );
        ...
        // Find ACE
        Ace entry = aclCrudDAO.getOrCreateAce(
          permission,
          authority,
          ace.getAceType(),
          ace.getAccessStatus()
  ...
  private void getWritable(
    final Long id,
    final Long parent,
    List<? extends AccessControlEntry> exclude,
    List<Ace> toAdd,
    Long inheritsFrom,
    boolean cascade,
    ...
    if (
      (mode == WriteMode.ADD_INHERITED) ||
      (mode == WriteMode.INSERT_INHERITED) ||
      (mode == WriteMode.CHANGE_INHERITED) ||
      (mode == WriteMode.CREATE_AND_INHERIT)
    ) {
        ...
        if (visitedAcls.contains(nextId)) {
          if (logger.isWarnEnabled()) {
            StringBuilder message = new StringBuilder(
              "ACL cycle detected! Repeated ALC id = '"
            )
              .append(nextId)
              .append("', inherited ACL id = '")
              .append(id)
              .append("', already visited ACLs: '")
              .append(visitedAcls)
  ...
  private AclChange getWritable(
    ...
    if (!acl.isVersioned()) {
      ...
      (acl.getAclChangeSetId() == getCurrentChangeSetId()) &&
      (!requiresVersion) &&
      ...
      newAcl.setInheritsFrom(
        (inheritsFrom != null) ? inheritsFrom : acl.getInheritsFrom()
      );
      ...
      if (members.size() > 0) {
        List<Pair<Long, Integer>> aceIdsWithDepths = new ArrayList<Pair<Long, Integer>>(
          members.size()
        );

        for (AclMember member : members) {
          aceIdsWithDepths.add(
            new Pair<Long, Integer>(member.getAceId(), member.getPos())
          );
        }
      ...
      if (newAcl.getAclType() == ACLType.SHARED) {
        if (parent != null) {
          Long writableParentAcl = getWritable(
            parent,
            null,
            null,
            null,
            ...
            .getAfter();
          AclUpdateEntity parentAcl = aclCrudDAO.getAclForUpdate(
            writableParentAcl
          );
   ...
   */
  private void removeAcesFromAcl(
    final Long id,
    final List<? extends AccessControlEntry> exclude,
    final int depth
  ) {
    if (exclude == null) {
      ...
      List<Map<String, Object>> results = aclCrudDAO.getAcesAndAuthoritiesByAcl(
        id
      );
  ...
  private void addInherited(
    ...
    if ((inherited != null) && (inherited.size() > 0)) {
      ...
      for (int i = 0; i < inherited.size(); i++) {
        ...
        aceIdsWithDepths.add(
          new Pair<Long, Integer>(add.getId(), position.intValue() + depth + 1)
        );
      }
  ...
  public List<AclChange> deleteAccessControlEntries(final String authority) {
    ...
    if (members.size() > 0) {
      Set<AclUpdateEntity> acls = new HashSet<AclUpdateEntity>(
        members.size() * 2
      );
      List<Long> membersToDelete = new ArrayList<Long>(members.size());
    ...
    if (!leaveAuthority) {
      ...
      List<Ace> unreferenced = aclCrudDAO.getAcesByAuthority(
        authEntity.getId()
      );

      if (unreferenced.size() > 0) {
        List<Long> unrefencedAcesToDelete = new ArrayList<Long>(
          unreferenced.size()
        );
  ...
  public List<AclChange> deleteAccessControlList(final Long id) {
    if (logger.isDebugEnabled()) {
      ...
      for (Long nodeId : nodeIds) {
        logger.debug(
          "deleteAccessControlList: Found nodeId=" + nodeId + ", aclId=" + id
        );
      }
    }
    ...
    if (!acl.isLatest()) {
      throw new UnsupportedOperationException(
        "Old ACL versions can not be updated"
      );
    }
    ...
    if (
      (acl.getAclType() == ACLType.DEFINING) ||
      (acl.getAclType() == ACLType.LAYERED)
    ) {
      if ((acl.getInheritedAcl() != null) && (acl.getInheritedAcl() != -1)) {
        ...
        for (AclChange change : acls) {
          if (
            ...
            change.getBefore().equals(inherited.getId())
          ) {
        ...
        if (inherited.isVersioned()) {
          AclUpdateEntity inheritedForUpdate = aclCrudDAO.getAclForUpdate(
            inherited.getId()
          );
   ...
   */
  @Override
  public List<AclChange> deleteAccessControlEntries(
    Long id,
  ...
  public AccessControlList getAccessControlList(Long id) {
    ...
    for (Map<String, Object> result : results) { // for (AclMemberEntity member : members)
      ...
      sacEntry.setAccessStatus(
        aceIsAllowed ? AccessStatus.ALLOWED : AccessStatus.DENIED
      );
      ...
      QName permTypeQName = qnameDAO
        .getQName(perm.getTypeQNameId())
        .getSecond(); // Has an ID so must exist
      SimplePermissionReference permissionRefernce = SimplePermissionReference.getPermissionReference(
        permTypeQName,
        perm.getName()
      );
   ...
   */
  @Override
  public List<AclChange> mergeInheritedAccessControlList(
    Long inherited,
    Long target
  ) {
      ...
      if (targetAcl.getInheritsFrom() != null) {
        ...
        if (inheritedAcl == null) {
          ...
          throw new IllegalStateException(
            "No old inheritance definition to use"
          );
        } else {
          ...
          if (!inheritedAcl.isLatest()) {
            ...
            Long actualInheritor = (Long) aclCrudDAO.getLatestAclByGuid(
              searchAclId
            );
    ...
    while (test != null) {
      ...
      if (visitedAcls.contains(testId)) {
        throw new IllegalStateException(
          "Cyclical InheritsFrom detected. AclId: " + testId
        );
      } else {
    ...
    if (
      (targetAcl.getAclType() != ACLType.DEFINING) &&
      (targetAcl.getAclType() != ACLType.LAYERED)
    ) {
      throw new IllegalArgumentException(
        "Only defining ACLs can have their inheritance set"
      );
    }
    ...
    if (
      (inheritedAcl.getAclType() == ACLType.DEFINING) ||
      (inheritedAcl.getAclType() == ACLType.LAYERED)
    ) {
   ...
   */
  @Override
  public List<AclChange> setAccessControlEntry(
    final Long id,
  ...
  public List<AclChange> enableInheritance(Long id, Long parent) {
    ...
    switch (acl.getAclType()) {
      ...
      case GLOBAL:
        throw new IllegalArgumentException(
          "Fixed and global permissions can not inherit"
        );
      case OLD:
        acl.setInherits(Boolean.TRUE);
        acl.setAclChangeSetId(getCurrentChangeSetId());
        aclCrudDAO.updateAcl(acl);
        changes.add(
          new AclChangeImpl(id, id, acl.getAclType(), acl.getAclType())
        );
        return changes;
      case SHARED:
      ...
      default:
        ...
        List<AclChange> merged = mergeInheritedAccessControlList(
          parent,
   ...
   */
  @Override
  public List<AclChange> disableInheritance(
    Long id,
    ...
    switch (acl.getAclType()) {
      case FIXED:
      case GLOBAL:
        return Collections.<AclChange>singletonList(
          new AclChangeImpl(id, id, acl.getAclType(), acl.getAclType())
        );
      case OLD:
        acl.setInherits(Boolean.FALSE);
    ...
    switch (mode) {
      ...
      case COW:
        ...
        if ((inheritedId != null) && (!inheritedId.equals(toCopy))) {
          AclUpdateEntity inheritedAcl = aclCrudDAO.getAclForUpdate(
            inheritedId
          );
      ...
      case REDIRECT:
        ...
        switch (aclToCopy.getAclType()) {
          ...
          case LAYERED:
            ...
            List<AclChange> changes = mergeInheritedAccessControlList(
              toInheritFrom,
              toCopy
            );
  ...
  private List<AclChange> disableInheritanceImpl(
    Long id,
    boolean setInheritedOnAcl,
    AclEntity aclIn
  ) {
  ...
  private static final String RESOURCE_KEY_ACL_CHANGE_SET_ID =
    "acl.change.set.id";
  private static final String RESOURCE_KEY_ACL_CHANGE_SET_COMMIT_TIME_MS =
    "acl.change.commit.set.time.ms";
  ...
  private class UpdateChangeSetListener extends TransactionListenerAdapter {
    ...
    public void beforeCommit(boolean readOnly) {
      ...
      Long changeSetId = (Long) AlfrescoTransactionSupport.getResource(
  ...
  public Long getCurrentChangeSetCommitTime() {
    return AlfrescoTransactionSupport.getResource(
      RESOURCE_KEY_ACL_CHANGE_SET_COMMIT_TIME_MS
    );
  }
  ...
  private static class AcePatternMatcher {
    ...
    private boolean checkPattern(
      ...
      if (pattern.getAccessStatus() != null) {
        if (
          pattern.getAccessStatus() !=
          (result_aceIsAllowed ? AccessStatus.ALLOWED : AccessStatus.DENIED)
        ) {
          return false;
        }
      }
      ...
      if (pattern.getAuthority() != null) {
        if (
          (pattern.getAuthorityType() != AuthorityType.WILDCARD) &&

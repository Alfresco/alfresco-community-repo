...
public class ProcessesImplTest extends TestCase implements RecognizedParamsExtractor {
    private static final String[] CONFIG_LOCATIONS =
            new String[ApplicationContextHelper.CONFIG_LOCATIONS.length + 2];
    ...
    private void startWorkflow(WorkflowDefinition neededDefinition, NodeRef assignee) {
        ...
        parameters.put(
                WorkflowModel.ASSOC_ASSIGNEE, (Serializable) Collections.singletonList(assignee));
        parameters.put(
                WorkflowModel.PROP_WORKFLOW_DESCRIPTION,
                "Test workflow api calls review and approve"); // MNT-14631
    ...
    public void testHasMoreItemsTrue() throws Exception {
        CollectionWithPagingInfo<ProcessInfo> actualProcesses =
                queryActiveProcessesAndAssertResult(0, (ACTIVE_WORKFLOWS_INITIAL_AMOUNT - 5));
        assertTrue(actualProcesses.hasMoreItems());

        actualProcesses =
                queryActiveProcessesAndAssertResult(10, (ACTIVE_WORKFLOWS_INITIAL_AMOUNT - 15));
        assertTrue(actualProcesses.hasMoreItems());
    }
    ...
    public void testHasMoreFalseAsPerMnt10977() throws Exception {
        CollectionWithPagingInfo<ProcessInfo> actualProcesses =
                queryActiveProcessesAndAssertResult(5, ACTIVE_WORKFLOWS_INITIAL_AMOUNT);
    ...
    public void testHasMoreFalseAsPerMnt13567() throws Exception {
        CollectionWithPagingInfo<ProcessInfo> actualProcesses =
                queryActiveProcessesAndAssertResult(
                        ACTIVE_WORKFLOWS_INITIAL_AMOUNT, ACTIVE_WORKFLOWS_INITIAL_AMOUNT);
        assertFalse(actualProcesses.hasMoreItems());

        actualProcesses =
                queryActiveProcessesAndAssertResult(
                        ACTIVE_WORKFLOWS_INITIAL_AMOUNT + 1, ACTIVE_WORKFLOWS_INITIAL_AMOUNT);
        assertFalse(actualProcesses.hasMoreItems());
    }

    @Test
    public void testGetProcessesMatchesIgnoreCase() {
        CollectionWithPagingInfo<ProcessInfo> result =
                queryMatchesProcesses("(?i)test workflow api calls review and approve");
    ...
    public void testGetProcessesMatchesIgnoreCaseNoResults() {
        ...
        //        }
        CollectionWithPagingInfo<ProcessInfo> result =
                queryMatchesProcesses("test workflow api calls review and approve");
    ...
    private CollectionWithPagingInfo<ProcessInfo> queryMatchesProcesses(String matchesString) {
        Query query =
                getWhereClause(String.format(QUERY_WORKFLOWDESCRIPTION_MATCHES, matchesString));
    ...
    private CollectionWithPagingInfo<ProcessInfo> queryActiveProcessesAndAssertResult(
        ...
        int remainingProcessesAmount =
                skipCount > ACTIVE_WORKFLOWS_INITIAL_AMOUNT
                        ? 0

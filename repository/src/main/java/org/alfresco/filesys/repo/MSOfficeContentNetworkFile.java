...
public class MSOfficeContentNetworkFile extends ContentNetworkFile {
  ...
  private static final Log logger = LogFactory.getLog(
    MSOfficeContentNetworkFile.class
  );
    ...
    if (m_writeList != null) {
      ...
      if (getReadCount() > 0 && m_writeList.size() < 2) {
        ...
        if (length != 0) {
          ...
          "MSOfficeFile: Ignored zero length write"
        );
      ...
      if (m_writeList.size() > 0) {
        ...
        while (m_writeList.size() > 0) {
          ...
          try {
            ...
            if (logger.isDebugEnabled()) logger.debug(
              "MSOfficeFile: Buffered write error, " + ex.getMessage()
            );
          }
        }
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "MSOfficeFile: Buffered writes flushed"
        );
  ...
  public void closeFile() throws IOException {
    ...
    if (logger.isDebugEnabled() && m_writeList != null) logger.debug(
      "MSOfficeFile: Discarded buffered writes - " + m_writeList.size()
    );

...
public class RenditionsImpl implements Renditions, ResourceLoaderAware {
    ...
    private static final Set<String> RENDITION_STATUS_COLLECTION_EQUALS_QUERY_PROPERTIES =
            Collections.singleton(PARAM_STATUS);
    ...
    public CollectionWithPagingInfo<Rendition> getRenditions(
            NodeRef nodeRef, Parameters parameters) {
        return getRenditions(nodeRef, null, parameters);
    }

    @Override
    public CollectionWithPagingInfo<Rendition> getRenditions(
            NodeRef nodeRef, String versionLabelId, Parameters parameters) {
        final NodeRef validatedNodeRef =
                validateNode(nodeRef.getStoreRef(), nodeRef.getId(), versionLabelId, parameters);
        ...
        Set<String> renditionNames =
                renditionDefinitionRegistry2.getRenditionNamesFrom(sourceMimetype, size);
        ...
        List<ChildAssociationRef> nodeRefRenditions =
                renditionService2.getRenditions(validatedNodeRef);
    ...
    public Rendition getRendition(
            NodeRef nodeRef, String versionLabelId, String renditionId, Parameters parameters) {
        final NodeRef validatedNodeRef =
                validateNode(nodeRef.getStoreRef(), nodeRef.getId(), versionLabelId, parameters);
        ...
        // if there is no rendition, then try to find the available/registered rendition (yet to be
        // created).
        if (renditionNodeRef == null && includeNotCreated) {
            ...
            RenditionDefinition2 renditionDefinition =
                    renditionDefinitionRegistry2.getRenditionDefinition(renditionId);
            if (renditionDefinition == null) {
                ...
                if (!found) {
                    throw new NotFoundException(
                            renditionId
                                    + " is not applicable for the node's mimeType "
                                    + sourceMimetype);
                }
            }
        ...
        if (renditionNodeRef == null) {
            throw new NotFoundException(
                    "The rendition with id: " + renditionId + " was not found.");
        }
    ...
    public void createRendition(
        ...
        final NodeRef renditionNodeRef =
                getRenditionByName(sourceNodeRef, rendition.getId(), parameters);
        if (renditionNodeRef != null) {
            throw new ConstraintViolatedException(
                    rendition.getId() + " rendition already exists."); // 409
        }
    ...
    public void createRenditions(
        ...
        // So that POST /nodes/{nodeId}/renditions can specify rendition names as a comma separated
        // list just like
        // POST /nodes/{nodId}/children can specify a comma separated list, the following code
        // checks to see if the
        // supplied Rendition names are actually comma separated lists. The following example shows
        // it is possible to
        // use both approaches.
        // [
        //  { "id": "doclib" },
        ...
        for (String renditionName : renditionNames) {
            ...
            final NodeRef renditionNodeRef =
                    getRenditionByName(sourceNodeRef, renditionName, parameters);
            if (renditionNodeRef == null) {
        ...
        if (renditionNamesNotRegistered.length() != 0) {
            throw new NotFoundException(
                    "Renditions not registered: " + renditionNamesNotRegistered); // 404
        }

        if (renditionNamesToCreate.size() == 0) {
            throw new ConstraintViolatedException(
                    "All renditions requested already exist: " + renditionNamesAlreadyExist); // 409
        }
    ...
    @Override
    public BinaryResource getContentNoValidation(
    ...
    public DirectAccessUrl requestContentDirectUrl(
            ...
            Long validFor) {
        final NodeRef validatedNodeRef =
                validateNode(nodeRef.getStoreRef(), nodeRef.getId(), versionId, null);
        ...
        if (renditionNodeRef == null) {
            ...
            String phPath =
                    scriptThumbnailService.getMimeAwarePlaceHolderResourcePath(
                            renditionId, sourceNodeMimeType);
            if (phPath == null) {
                // 404 since no thumbnail was found
                throw new NotFoundException(
                        "Thumbnail was not found and no placeholder resource available for ["
                                + renditionId
                                + ']');
            } else {
    ...
    protected NodeRef getRenditionByName(
            NodeRef nodeRef, String renditionId, Parameters parameters) {
        if (nodeRef != null) {
            ...
            ChildAssociationRef nodeRefRendition =
                    renditionService2.getRenditionByName(nodeRef, renditionId);
    ...
    protected Rendition toApiRendition(NodeRef renditionNodeRef) {
        ...
        String renditionName =
                (String) nodeService.getProperty(renditionNodeRef, ContentModel.PROP_NAME);
    ...
    protected Rendition toApiRendition(String renditionName) {
        ...
        ContentInfo contentInfo =
                new ContentInfo(
                        renditionDefinition.getTargetMimetype(),
                        getMimeTypeDisplayName(renditionDefinition.getTargetMimetype()),
                        null,
                        null);
    ...
    private NodeRef validateNode(
            StoreRef storeRef, final String nodeId, String versionLabelId, Parameters parameters) {
        if (nodeId == null) {
    ...
    private void isContentFile(NodeRef nodeRef) {
        if (!nodes.isSubClass(nodeRef, ContentModel.PROP_CONTENT, false)) {
            throw new InvalidArgumentException(
                    "Node id '" + nodeRef.getId() + "' does not represent a file.");
        }
    }

...
public abstract class AbstractNodeImporter implements NodeImporter {
  ...
  protected final int importContentVersions(
    ...
    for (final ImportableItem.VersionedContentAndMetadata versionEntry : importableItem.getVersionEntries()) {
      ...
      if (logger.isDebugEnabled()) logger.debug(
        ...
        " of node '" +
        nodeRef.toString() +
        "' (note: version label in Alfresco will not be the same - it is not currently possible to explicitly force a particular version label)."
      );
    ...
    if (
      headRevContentAndMetadata != null &&
      (
      ...
      MetadataLoader.Metadata metadata = loadMetadata(
        headRevContentAndMetadata
      );
  ...
  protected final Triple<NodeRef, Boolean, NodeState> createOrFindNode(
    ...
    NodeState nodeState = replaceExisting
      ? NodeState.REPLACED
      : NodeState.SKIPPED;
      ...
      if (importableItem.getHeadRevision().contentFileExists()) {
        ...
        try {
          ...
          if (logger.isWarnEnabled()) logger.warn(
            ...
            "' within node '" +
            target.toString() +
            "' was created concurrently to the bulk import.  Skipping importing it.",
            fee
          );
          nodeRef = null;
    ...
    // We found the node in the repository.  Make sure we return the NodeRef, so that recursive loading works (we need the NodeRef of all sub-spaces, even if we didn't create them).
      ...
      if (replaceExisting) {
        boolean targetNodeIsSpace = fileFolderService
          .getFileInfo(nodeRef)
          .isFolder();
    ...
    result =
      new Triple<NodeRef, Boolean, NodeState>(nodeRef, isDirectory, nodeState);
      ...
      try {
        ...
        if (!nodeRef.equals(inre.getNodeRef())) {
          ...
          throw new IllegalStateException(
            ...
            "NodeRef for the target of that association ('" +
            inre.getNodeRef() +
            "') is invalid.  " +
  ...
  protected final void importImportableItemDirectory(
    ...
    importImportableItemMetadata(
      nodeRef,
      importableItem.getHeadRevision().getContentFile(),
      metadata
    );
  }
  ...
  protected final MetadataLoader.Metadata loadMetadata(
    ...
    if (contentAndMetadata != null && contentAndMetadata.contentFileExists()) {
      final String filename = contentAndMetadata
        .getContentFile()
        .getFileName()
        .toString()
        .trim()
      ...
      result.setType(
        ImportableItem.FileType.FILE.equals(
            contentAndMetadata.getContentFileType()
          )
  ...
  public NodeRef importImportableItem(
    ...
    NodeRef nodeRef = importImportableItemImpl(
      importableItem,
      existingFileMode
    );

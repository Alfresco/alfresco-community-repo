...
public class SimpleTemplateActionDefinition
  ...
  public Action getAction(NodeRef nodeRef) {
    ...
    ActionExecuter actionExecutor = (ActionExecuter) applicationContext.getBean(
      getActionName()
    );
    ...
    for (String paramName : parameterTemplates.keySet()) {
      ...
      // There can be no definition for such an ad hoc property.
        ...
        dataTypeDef =
          dictionaryService
            .getProperty(QName.createQName(paramName))
            .getDataType();
      }

      // Convert the template result into the correct type and set the parameter
      Object value = DefaultTypeConverter.INSTANCE.convert(
        dataTypeDef,
        stringValue
      );

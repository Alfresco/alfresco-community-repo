...
public class ContentStoreCleanerTest extends TestCase {
  ...
  public void testEagerCleanupOnCommit() throws Exception {
    ...
    RetryingTransactionCallback<NodeRef> makeContentCallback = new RetryingTransactionCallback<NodeRef>() {
      public NodeRef execute() throws Throwable {
        ...
        StoreRef storeRef = nodeService.createStore(
          "test",
          "testEagerCleanupOnCommit-" + GUID.generate()
        );
    ...
    final NodeRef contentNodeRef = transactionService
    ...
    try {
      transactionService
        .getRetryingTransactionHelper()
        .doInTransaction(failUpdateCallback);
    ...
    transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(successUpdateCallback);
    ...
    transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(pointlessUpdateCallback);
  ...
  public void testEagerCleanupDereferencing() throws Exception {
    ...
    try {
      ...
      ContentData contentData = transactionService
        .getRetryingTransactionHelper()
  ...
  public void testImmediateRemoval() throws Exception {
    ...
    assertNotNull(reader);
    assertTrue(
      "Content should not have been eagerly deleted.",
      reader.exists()
    );
    ...
    assertTrue(
      "Content listener was not called",
      deletedUrls.contains(reader.getContentUrl())
    );
  }
  ...
  public void testMNT_12150() {
    ...
    eagerCleaner.setListeners(
      Collections.<ContentStoreCleanerListener>emptyList()
    );
    cleaner.setProtectDays(0);
    ...
    for (String url : keptUrlsById.values()) {
      if (url.equalsIgnoreCase(contentData.getContentUrl())) {
        fail(
          "Failed to cleanup orphaned content: " + contentData.getContentUrl()
        );
      }
    }
  }

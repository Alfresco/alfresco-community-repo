...
public class BlogServiceTest extends BaseWebScriptTest {
    ...
    private static final String URL_BLOG_POST =
            "/api/blog/post/site/" + SITE_SHORT_NAME_BLOG + "/" + COMPONENT_BLOG + "/";
    private static final String URL_BLOG_CORE =
            "/api/blog/site/" + SITE_SHORT_NAME_BLOG + "/" + COMPONENT_BLOG;
    private static final String URL_BLOG_POSTS = URL_BLOG_CORE + "/posts";
    private static final String URL_MY_DRAFT_BLOG_POSTS =
            "/api/blog/site/" + SITE_SHORT_NAME_BLOG + "/" + COMPONENT_BLOG + "/posts/mydrafts";
    private static final String URL_MY_PUBLISHED_BLOG_POSTS =
            "/api/blog/site/" + SITE_SHORT_NAME_BLOG + "/" + COMPONENT_BLOG + "/posts/mypublished";

    private static final String URL_DELETE_COMMENT =
            "api/comment/node/{0}/{1}/{2}?site={3}&itemtitle={4}&page={5}";
    ...
    protected void setUp() throws Exception {
        ...
        this.activityService =
                (ActivityService) getServer().getApplicationContext().getBean("activityService");
        ChildApplicationContextFactory activitiesFeed =
                (ChildApplicationContextFactory)
                        getServer().getApplicationContext().getBean("ActivitiesFeed");
    ...
    private JSONObject getRequestObject(
            String title, String content, String[] tags, boolean isDraft) throws Exception {
    ...
    private JSONObject createPost(
            String title, String content, String[] tags, boolean isDraft, int expectedStatus)
            throws Exception {
        JSONObject post = getRequestObject(title, content, tags, isDraft);
        Response response =
                sendRequest(
                        new PostRequest(URL_BLOG_POSTS, post.toString(), "application/json"),
                        expectedStatus);
    ...
    private JSONObject updatePost(
        ...
        Response response =
                sendRequest(
                        new PutRequest(URL_BLOG_POST + name, post.toString(), "application/json"),
                        expectedStatus);
    ...
    private String getDeleteCommentUrl(NodeRef commentNodeRef) {
        ...
        String URL =
                MessageFormat.format(
                        URL_DELETE_COMMENT,
                        new Object[] {
                            commentNodeRef.getStoreRef().getProtocol(),
                            commentNodeRef.getStoreRef().getIdentifier(),
                            commentNodeRef.getId(),
                            SITE_SHORT_NAME_BLOG,
                            itemTitle,
                            page
                        });
    ...
    private JSONObject createComment(
            String nodeRef, String title, String content, int expectedStatus) throws Exception {
        ...
        Response response =
                sendRequest(
                        new PostRequest(
                                getCommentsUrl(nodeRef), comment.toString(), "application/json"),
                        expectedStatus);
    ...
    private JSONObject updateComment(
            String nodeRef, String title, String content, int expectedStatus) throws Exception {
        JSONObject comment = new JSONObject();
        comment.put("title", title);
        comment.put("content", content);
        Response response =
                sendRequest(
                        new PutRequest(
                                getCommentUrl(nodeRef), comment.toString(), "application/json"),
                        expectedStatus);
    ...
    public void testCreateDraftPostWithTagsAndComment() throws Exception {
        ...
        List<String> recoveredTagsList =
                Arrays.asList(new String[] {reportedTags.getString(0), reportedTags.getString(1)});
        ...
        // Currently (mid-Swift dev) there is no Java CommentService, so we have to post a comment
        // via the REST API.
        String commentsPostUrl =
                "/api/node/"
                        + blogPostNode.getStoreRef().getProtocol()
                        + "/"
        ...
        Response response =
                sendRequest(new PostRequest(commentsPostUrl, jsonToPost, "application/json"), 200);
    ...
    public void testComments() throws Exception {
        ...
        response =
                sendRequest(
                        new DeleteRequest(getCommentUrl(commentThree.getString("nodeRef"))), 200);

        response = sendRequest(new GetRequest(getCommentsUrl(nodeRef)), 200);
        result = new JSONObject(response.getContentAsString());
        assertEquals(2, result.getInt("total"));

        JSONObject commentTwoUpdated =
                updateComment(commentTwo.getString("nodeRef"), "new title", "new content", 200);
    ...
    public void testDeleteCommentPostActivity() throws Exception {
        ...
        assertEquals(
                "The activity feeds were not generated after adding a comment",
                activityNumStart + 1,
                activityNumNext);
        ...
        assertEquals(
                "The activity feeds were not generated after deleting a comment",
                activityNumStart + 1,
                activityNumNext);
    }

    /**
     * You can attach information to the blog container relating to integration with external blogs.
    ...
    public void testBlogIntegration() throws Exception {
        ...
        sendRequest(
                new PutRequest(URL_BLOG_CORE, blog.toString(), "application/json"),
                Status.STATUS_OK);
     ...
     *
     * <p>Currently observed errors: 1. [repo.action.AsynchronousActionExecutionQueueImpl] Failed to
     * execute asynchronous action: Action[ id=485211db-f117-4976-9530-ab861a19f563, node=null ]
     * org.alfresco.repo.security.permissions.AccessDeniedException: Access Denied. You do not have
     * the appropriate permissions to perform this operation.
     *
     * <p>2. JSONException, but with root cause being get(assocs) failed on instance of
     * org.alfresco.repo.template.TemplateNode The problematic instruction: ---------- ==> if
     * person.assocs["cm:avatar"]?? [on line 4, column 7 in
     * org/alfresco/repository/blogs/blogpost.lib.ftl]
     *
     * @throws Exception
     */
    ...
    private void checkBlogPermissions(String siteName) throws Exception {
        ...
        assertTrue(
                "The user sould have permission to create a new blog.",
                result.getJSONObject("item").getJSONObject("permissions").getBoolean("create"));
    }
}

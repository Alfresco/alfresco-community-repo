...
public class EmailMapScriptTest extends BaseRMWebScriptTestCase {
  ...
  public static final String URL_RM_EMAILMAP_DELETE =
    "/api/rma/admin/emailmap/%s/%s";
  ...
  public void testEmailMap() throws IOException, JSONException {
    ...
    Response getResponse = sendRequest(
      new GetRequest(URL_RM_EMAILMAP),
      Status.STATUS_OK
    );

    JSONObject getResponseContent = new JSONObject(
    ...
    Response postResponse = sendRequest(
      new PostRequest(URL_RM_EMAILMAP, newMapping.toString(), APPLICATION_JSON),
      Status.STATUS_OK
    );
    JSONObject postResponseContent = new JSONObject(
    ...
    /** Test DELETE */
    Response deleteResponse = sendRequest(
      new DeleteRequest(
        String.format(URL_RM_EMAILMAP_DELETE, "messageTo", "rmc:Wibble")
      ),
      Status.STATUS_OK
    );
    JSONObject deleteResponseContent = new JSONObject(
  ...
  private boolean existsMapping(JSONArray mappings) throws JSONException {
    ...
    for (int i = 0; i < mappings.length(); i++) {
      ...
      if (
        from.equalsIgnoreCase("messageTo") && to.equalsIgnoreCase("rmc:Wibble")
      ) {
        result = true;
        break;
      }

...
/**
 * An interceptor that replaces files nodes with their equivalent translations according to the
 * locale. It is to be used with the {@link FileFolderService}.
 *
 * @since 2.1
 * @author Derek Hulley
 */
public class MLTranslationInterceptor implements MethodInterceptor {
    ...
    /**
     * Names of methods that don't need interception. This is used to catch any new methods added to
     * the interface.
     */
    ...
    private NodeRef getTranslatedNodeRef(NodeRef nodeRef) {
        ...
        NodeRef translatedNodeRef =
                multilingualContentService.getTranslationForLocale(nodeRef, filterLocale);
        // Done
    ...
    private PagingResults<FileInfo> processPagingResults(final PagingResults<FileInfo> fileInfos) {
        ...
        PagingResults<FileInfo> orderedPagingResults =
                new PagingResults<FileInfo>() {
                    @Override
        ...
        if (I18NUtil.getContentLocaleOrNull() == null) {
            ...
            if (result instanceof List) {
                ...
                throw new ClassCastException(
                        "Unexpected return type from method " + methodName + " in " + this);
            }

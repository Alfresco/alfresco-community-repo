 ...
 *
 * <p>Since 4.0, the behaviour enabling/disabling is recorded using reference counting, meaning that
 * the outermost disable call in a stack has an effective veto. Use proper try-finally patterns to
 * ensure behaviour is released after it is no longer needed.
 *
 * <pre><code>
...
public interface BehaviourFilter {
     ...
     *
     * <p>The change applies <b>ONLY</b> to the current transaction.
     */
    public void disableBehaviour();

    /**
     * Disable behaviour for a type or aspect for all nodes. <br>
     * Given a direct instance of className (ie. not a subclass) all behaviour is disabled
     * (including superclass behaviour). </br> <br>
    ...
    /**
     * Disable behaviour for a type or aspect for all nodes. <br>
     * Given an instance of className (including instances that are subclasses of className, if
     * includeSubClasses is true) all behaviour is disabled (including superclass behaviour). </br>
     * <br>
     * Successive calls (within the current transaction) will overwrite the filter for this
     * class.</br> The change applies <b>ONLY</b> to the current transaction.
     *
     * @param className the type/aspect behaviour to disable
     ...
     * This is also applied to the to the disabled behaviours with {@link #disableBehaviour(QName,
     * boolean)}</br>
     *
     * <p>The change applies <b>ONLY</b> to the current transaction.
     *
     ...
     *
     * <p>Note: A node behaviour is enabled only when: a) the behaviour is not disabled across all
     * nodes b) the behaviour is not disabled specifically for the provided node
     *
     * <p>The change applies <b>ONLY</b> to the current transaction.
     *

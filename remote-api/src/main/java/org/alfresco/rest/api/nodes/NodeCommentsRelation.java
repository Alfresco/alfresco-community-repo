...
public class NodeCommentsRelation
     ...
     *
     * <p>THOR-1153: "F314: Add a comment to a folder or document"
     */
    @Override
    ...
    /**
     * Returns a paged list of comments for the document/folder identified by nodeId, sorted
     * chronologically with the newest first.
     *
     * <p>THOR-1152: “F313: For a folder or document, get the list of associated comments”
     *
     * <p>If nodeId does not exist, EntityNotFoundException (status 404). If nodeId does not
     * represent a document or folder, InvalidArgumentException (status 400).
     */
    @Override
    @WebApiDescription(
            title =
                    "Returns a paged list of comments for the document/folder identified by nodeId,"
                            + " sorted chronologically with the newest first.")
    ...
    @WebApiParameters({
        @WebApiParam(
                name = "nodeId",
                title = "The unique id of the parent Node being addressed",
                description = "A single node id"),
        @WebApiParam(
                name = "commentNodeId",
                title = "The unique id of the comment Node being addressed",
                description = "A single node id")
    })

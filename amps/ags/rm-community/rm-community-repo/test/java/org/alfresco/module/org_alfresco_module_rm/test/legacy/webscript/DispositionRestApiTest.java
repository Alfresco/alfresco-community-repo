...
public class DispositionRestApiTest
  ...
  protected static StoreRef SPACES_STORE = new StoreRef(
    StoreRef.PROTOCOL_WORKSPACE,
    "SpacesStore"
  );
  protected static final String GET_SCHEDULE_URL_FORMAT =
    "/api/node/{0}/dispositionschedule";
  protected static final String GET_LIFECYCLE_URL_FORMAT =
    "/api/node/{0}/nextdispositionaction";
  protected static final String POST_ACTIONDEF_URL_FORMAT =
    "/api/node/{0}/dispositionschedule/dispositionactiondefinitions";
  protected static final String DELETE_ACTIONDEF_URL_FORMAT =
    "/api/node/{0}/dispositionschedule/dispositionactiondefinitions/{1}";
  protected static final String PUT_ACTIONDEF_URL_FORMAT =
    "/api/node/{0}/dispositionschedule/dispositionactiondefinitions/{1}";
  ...
  public void testGetDispositionSchedule() throws Exception {
    ...
    String nonExistentNode =
      "workspace/SpacesStore/09ca1e02-1c87-4a53-97e7-xxxxxxxxxxxx";
    String nonExistentUrl = MessageFormat.format(
      GET_SCHEDULE_URL_FORMAT,
      nonExistentNode
    );
    ...
    String wrongNodeUrl = MessageFormat.format(
      GET_SCHEDULE_URL_FORMAT,
      seriesNodeUrl
    );
    ...
    NodeRef newRecordCategory = filePlanService.createRecordCategory(
      recordSeries,
      GUID.generate()
    );
    ...
    // get response as JSON
    jsonParsedObject =
      new JSONObject(new JSONTokener(rsp.getContentAsString()));
  ...
  public void testPostDispositionAction() throws Exception {
    ...
    Response rsp = sendRequest(
      new PostRequest(requestUrl, jsonPostString, APPLICATION_JSON),
      200
    );
  ...
  public void testPutDispositionAction() throws Exception {
    ...
    String categoryNodeUrl = newRecordCategory.toString().replace("://", "/");
    String postRequestUrl = MessageFormat.format(
      POST_ACTIONDEF_URL_FORMAT,
      categoryNodeUrl
    );
    JSONObject jsonPostData = new JSONObject();
    ...
    String jsonPostString = jsonPostData.toString();
    sendRequest(
      new PostRequest(postRequestUrl, jsonPostString, APPLICATION_JSON),
      200
    );

    // verify the action definition is present and retrieve it's id
    String getRequestUrl = MessageFormat.format(
      GET_SCHEDULE_URL_FORMAT,
      categoryNodeUrl
    );
    ...
    String putRequestUrl = MessageFormat.format(
      PUT_ACTIONDEF_URL_FORMAT,
      categoryNodeUrl,
      "xyz"
    );
    rsp =
      sendRequest(
        new PutRequest(putRequestUrl, jsonPostString, APPLICATION_JSON),
        404
      );
    ...
    assertEquals(
      "no_longer_needed",
      actionDef.getJSONArray("events").getString(1)
    );
  }
  ...
  public void testDeleteDispositionAction() throws Exception {
    ...
    String deleteRequestUrl = MessageFormat.format(
    ...
    getRequestUrl =
      MessageFormat.format(GET_SCHEDULE_URL_FORMAT, categoryNodeUrl);
  ...
  public void testGetDispositionLifecycle() throws Exception {
    ...
    NodeRef newRecordFolder = recordFolderService.createRecordFolder(
      recordCategory,
      "recordFolder"
    );
    ...
    requestUrl =
      MessageFormat.format(
        GET_LIFECYCLE_URL_FORMAT,
        newRecordFolder.toString().replace("://", "/")
      );
  ...
  public void testGetListOfValues() throws Exception {
    ...
    assertEquals(
      SERVICE_URL_PREFIX + GET_LIST_URL + "/dispositionactions",
      actions.getString("url")
    );
    ...
    assertEquals(
      SERVICE_URL_PREFIX + GET_LIST_URL + "/events",
      events.getString("url")
    );
    ...
    assertEquals(
      SERVICE_URL_PREFIX + GET_LIST_URL + "/periodtypes",
      periodTypes.getString("url")
    );
    ...
    assertEquals(
      SERVICE_URL_PREFIX + GET_LIST_URL + "/periodproperties",
      periodProperties.getString("url")
    );

...
/*package*/class PolicyFactory<B extends BehaviourBinding, P extends Policy> {
  ...
  private static TransactionInvocationHandlerFactory transactionHandlerFactory =
    null;
   ...
   */
  protected static void setTransactionInvocationHandlerFactory(
      ...
      if (
        !(
          behaviour
            ...
            .equals(NotificationFrequency.EVERY_EVENT)
        )
      ) {
        ...
        if (transactionHandlerFactory == null) {
          throw new PolicyException(
            "Transaction-level policies not supported as transaction support for the Policy Component has not been initialised."
          );
        }
        InvocationHandler trxHandler = transactionHandlerFactory.createHandler(
          behaviour,
          behaviourDef.getPolicyDefinition(),
          policyIF
        );
        policyIF =
          (P) Proxy.newProxyInstance(
            policyClass.getClassLoader(),
            new Class[] { policyClass },
            trxHandler
          );
      }
  ...
  public P toPolicy(Collection<P> policyList) {
    if (policyList.size() == 1) {
      ...
      return (P) Proxy.newProxyInstance(
        policyClass.getClassLoader(),
        new Class[] { policyClass, PolicyList.class },
        new MultiHandler<P>(policyList)
      );
    }
  }
   ...
   * @param <P>
   */
  private static class SingleHandler<P extends Policy>
    ...
    public Object invoke(Object proxy, Method method, Object[] args)
      throws Throwable {
      if (
        (tenantService != null) && (tenantService.isEnabled()) && (args != null)
      ) {
    ...
    public boolean equals(Object obj) {
      if (obj != null && Proxy.isProxyClass(obj.getClass())) {
        ...
        if (handler instanceof SingleHandler) {
          return ((SingleHandler) handler).policyInterface.equals(
              policyInterface
            );
        }
      }
  ...
  private static class MultiHandler<P extends Policy>
    implements InvocationHandler, PolicyList {
    ...
    public MultiHandler(Collection<P> policyInterfaces) {
      this.policyInterfaces =
        Collections.unmodifiableCollection(policyInterfaces);
    }
    ...
    public Object invoke(Object proxy, Method method, Object[] args)
      ...
      if (method.getName().equals("toString")) {
        return (
          toString() + ": wrapped " + policyInterfaces.size() + " policies"
        );
      } else if (method.getName().equals("hashCode")) {
        return hashCode();
      } else if (method.getName().equals("equals")) {

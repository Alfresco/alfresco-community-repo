...
public class CalendarServiceImplTest {
  ...
  private static final String TEST_USER =
    CalendarServiceImplTest.class.getSimpleName() + "_testuser";
  ...
  public static void initTestsContext() throws Exception {
    AUTHENTICATION_SERVICE =
      (MutableAuthenticationService) testContext.getBean(
        "authenticationService"
      );
    BEHAVIOUR_FILTER =
      (BehaviourFilter) testContext.getBean("policyBehaviourFilter");
    CALENDAR_SERVICE = (CalendarService) testContext.getBean("CalendarService");
    DICTIONARY_SERVICE =
      (DictionaryService) testContext.getBean("dictionaryService");
    NODE_SERVICE = (NodeService) testContext.getBean("nodeService");
    PUBLIC_NODE_SERVICE = (NodeService) testContext.getBean("NodeService");
    PERSON_SERVICE = (PersonService) testContext.getBean("personService");
    TRANSACTION_HELPER =
      (RetryingTransactionHelper) testContext.getBean(
        "retryingTransactionHelper"
      );
    PERMISSION_SERVICE =
      (PermissionService) testContext.getBean("permissionService");
    SITE_SERVICE = (SiteService) testContext.getBean("SiteService");
    // Get the canned query registry, and from that the factory
    @SuppressWarnings("unchecked")
    NamedObjectRegistry<CannedQueryFactory<? extends Object>> calendarCannedQueryRegistry = (NamedObjectRegistry<CannedQueryFactory<? extends Object>>) testContext.getBean(
      "calendarCannedQueryRegistry"
    );
    CALENDAR_CQ_FACTORY =
      (GetCalendarEntriesCannedQueryFactory) calendarCannedQueryRegistry.getNamedObject(
        CalendarServiceImpl.CANNED_QUERY_GET_ENTRIES
      );
  ...
  public void before() {
    ...
    (
      (CalendarServiceImpl) testContext.getBean("calendarService")
    ).setTaggingService(TAGGING_SERVICE);
    when(TAGGING_SERVICE.isTagScope(Matchers.any(NodeRef.class)))
      .thenReturn(true);
  }
  ...
  public void createNewEntry() throws Exception {
    ...
    PagingResults<CalendarEntry> results = CALENDAR_SERVICE.listCalendarEntries(
    ...
    entry =
      CALENDAR_SERVICE.getCalendarEntry(CALENDAR_SITE.getShortName(), "madeUp");
    ...
    // Have it saved
    entry =
      CALENDAR_SERVICE.createCalendarEntry(CALENDAR_SITE.getShortName(), entry);

    // Ensure it got a noderef, and the correct site
    assertNotNull(entry.getNodeRef());
    assertNotNull(entry.getSystemName());

    NodeRef container = PUBLIC_NODE_SERVICE
      .getPrimaryParent(entry.getNodeRef())
      .getParentRef();
    NodeRef site = PUBLIC_NODE_SERVICE
      .getPrimaryParent(container)
      .getParentRef();
    ...
    TRANSACTION_HELPER.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
        ...
        public Void execute() throws Throwable {
          ...
          NodeRef container = SITE_SERVICE.getContainer(
            TEST_SITE_NAME,
            CalendarServiceImpl.CALENDAR_COMPONENT
          );
  ...
  public void tagging() throws Exception {
    ...
    // Check
    when(TAGGING_SERVICE.getTags(entry.getNodeRef()))
      .thenReturn(Arrays.asList(TAG_1, TAG_2));
    entry =
      CALENDAR_SERVICE.getCalendarEntry(
        CALENDAR_SITE.getShortName(),
        entry.getSystemName()
    ...
    when(TAGGING_SERVICE.getTags(entry.getNodeRef()))
      .thenReturn(Arrays.asList(TAG_1, TAG_3));
    entry =
      CALENDAR_SERVICE.getCalendarEntry(
        CALENDAR_SITE.getShortName(),
        entry.getSystemName()
    ...
    // Check
    when(TAGGING_SERVICE.getTags(entry.getNodeRef()))
      .thenReturn(Arrays.asList(TAG_1, TAG_2, TAG_3));
    entry =
      CALENDAR_SERVICE.getCalendarEntry(
        CALENDAR_SITE.getShortName(),
        entry.getSystemName()
  ...
  public void calendarSingleSiteListing() throws Exception {
    ...
    // Add a few
    CalendarEntry entryA = CALENDAR_SERVICE.createCalendarEntry(
      CALENDAR_SITE.getShortName(),
      new CalendarEntryDTO(
        "TitleA",
        "Description",
    ...
    CalendarEntry entryB = CALENDAR_SERVICE.createCalendarEntry(
      CALENDAR_SITE.getShortName(),
      new CalendarEntryDTO(
        "TitleB",
        "Description",
    ...
    CalendarEntry entryC = CALENDAR_SERVICE.createCalendarEntry(
      CALENDAR_SITE.getShortName(),
      new CalendarEntryDTO(
        "TitleC",
        "Description",
    ...
    // Check now
    results =
      CALENDAR_SERVICE.listCalendarEntries(
        CALENDAR_SITE.getShortName(),
        paging
      );
    ...
    CalendarEntry entryD = CALENDAR_SERVICE.createCalendarEntry(
      CALENDAR_SITE.getShortName(),
      new CalendarEntryDTO(
        "TitleD",
        "Description",
    ...
    paging = new PagingRequest(3);
    results =
      CALENDAR_SERVICE.listOutlookCalendarEntries(
        CALENDAR_SITE.getShortName(),
        null,
        paging
      );
  ...
  public void calendarListingPermissionsChecking() throws Exception {
    ...
    assertEquals(
      true,
      SITE_SERVICE.isMember(CALENDAR_SITE.getShortName(), TEST_USER)
    );
    assertEquals(
      false,
      SITE_SERVICE.isMember(ALTERNATE_CALENDAR_SITE.getShortName(), TEST_USER)
    );
    ...
    CALENDAR_SERVICE.createCalendarEntry(
      CALENDAR_SITE.getShortName(),
      new CalendarEntryDTO(
        "TitleA",
        "Description",
    ...
    NodeRef priv3 = CALENDAR_SERVICE
      .createCalendarEntry(
        ALTERNATE_CALENDAR_SITE.getShortName(),
        new CalendarEntryDTO(
          "PrivateTitleC",
  ...
  public void calendarMultiSiteListing() throws Exception {
    ...
    results =
      CALENDAR_SERVICE.listCalendarEntries(
        new String[] { CALENDAR_SITE.getShortName() },
        new Date(1302131400),
        new Date(1302432000),
        paging
      );
    ...
    results =
      CALENDAR_SERVICE.listCalendarEntries(
        new String[] { ALTERNATE_CALENDAR_SITE.getShortName() },
        new Date(1302131400),
        new Date(1302432000),
        paging
      );
  ...
  public void testCannedQueryEntityResults() throws Exception {
    ...
    CalendarEntry c1 = CALENDAR_SERVICE.createCalendarEntry(
      CALENDAR_SITE.getShortName(),
      new CalendarEntryDTO(
        "SiteNormal",
        "Description",
    ...
    CalendarEntry c2 = CALENDAR_SERVICE.createCalendarEntry(
      CALENDAR_SITE.getShortName(),
      new CalendarEntryDTO(
        "SiteRepeating",
        "Description",
    ...
    CalendarEntry c3 = CALENDAR_SERVICE.createCalendarEntry(
      ALTERNATE_CALENDAR_SITE.getShortName(),
      new CalendarEntryDTO(
        "AltSiteNormal",
        "Description",
    ...
    assertEquals(
      c3.getStart(),
      ISO8601DateFormat.parse(filtered.get(0).getFromDate(), utc)
    );
    assertEquals(
      c3.getEnd(),
      ISO8601DateFormat.parse(filtered.get(0).getToDate(), utc)
    );
    ...
    assertEquals(
      c1.getStart(),
      ISO8601DateFormat.parse(filtered.get(1).getFromDate(), utc)
    );
    assertEquals(
      c1.getEnd(),
      ISO8601DateFormat.parse(filtered.get(1).getToDate(), utc)
    );
    ...
    assertEquals(
      c2.getStart(),
      ISO8601DateFormat.parse(filtered.get(2).getFromDate(), utc)
    );
    assertEquals(
      c2.getEnd(),
      ISO8601DateFormat.parse(filtered.get(2).getToDate(), utc)
    );
    ...
    assertEquals(
      c3.getLastRecurrence(),
      ISO8601DateFormat.parse(filtered.get(0).getRecurrenceLastMeeting(), utc)
    );
    assertEquals(c3.getRecurrenceRule(), filtered.get(0).getRecurrenceRule());
    ...
    TRANSACTION_HELPER.doInTransaction(
      new RetryingTransactionCallback<Void>() {
        ...
        public Void execute() throws Throwable {
          GetCalendarEntriesCannedQuery cq = (GetCalendarEntriesCannedQuery) CALENDAR_CQ_FACTORY.getCannedQuery(
            containers,
            c20110718mon.getTime(),
            c20110719tue.getTime(),
            paging
          );
          cq.setTestHook(hook);
          cq.execute();
  ...
  private static void createTestSites() throws Exception {
    final CalendarServiceImpl privateCalendarService = (CalendarServiceImpl) testContext.getBean(
      "calendarService"
    );
        ...
        new RetryingTransactionHelper.RetryingTransactionCallback<SiteInfo>() {
          ...
          public SiteInfo execute() throws Throwable {
            SiteInfo site = SITE_SERVICE.createSite(
              TEST_SITE_PREFIX,
              CalendarServiceImplTest.class.getSimpleName() +
              "_testSite" +
            ...
            privateCalendarService.getSiteCalendarContainer(
              site.getShortName(),
              true
            );
   ...
   */
  private static void performDeletionOfNodes(
    final List<NodeRef> nodesToDelete
  ) {
    TRANSACTION_HELPER.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
        @Override

...
public class GetChildrenCannedQueryTest extends TestCase {
    ...
    private static QName TEST_CONTENT_SUBTYPE =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "savedquery");
    ...
    private static final String FOLDER_3 =
            "emptySystemFolder"; // note: using FileFolderService directly will auto-hide this
    // folder
    ...
    public void setUp() throws Exception {
        ...
        GetChildrenCannedQueryFactory getChildrenCannedQueryFactory =
                new GetChildrenCannedQueryFactory();
        ...
        getChildrenCannedQueryFactory.setCannedQueryDAO(
                (CannedQueryDAO) ctx.getBean("cannedQueryDAO"));
        getChildrenCannedQueryFactory.setContentDataDAO(
                (ContentDataDAO) ctx.getBean("contentDataDAO"));
        getChildrenCannedQueryFactory.setDictionaryService(
                (DictionaryService) ctx.getBean("dictionaryService"));
        getChildrenCannedQueryFactory.setTenantService(
                (TenantService) ctx.getBean("tenantService"));
        ...
        getChildrenCannedQueryFactory.setMethodSecurity(
                (MethodSecurityBean<NodeRef>) ctx.getBean("FileFolderService_security_list"));
        ...
        if (!setupTestData) {
            ...
            createUser(
                    TEST_USER_PREFIX + "aaaa",
                    TEST_USER_PREFIX + "bbbb",
                    TEST_USER_PREFIX + "cccc");
            createUser(
                    TEST_USER_PREFIX + "cccc",
                    TEST_USER_PREFIX + "dddd",
                    TEST_USER_PREFIX + "eeee");
            createUser(
                    TEST_USER_PREFIX + "dddd",
                    TEST_USER_PREFIX + "ffff",
                    TEST_USER_PREFIX + "gggg");
            createUser(
                    TEST_USER_PREFIX + "hhhh",
                    TEST_USER_PREFIX + "cccc",
                    TEST_USER_PREFIX + "jjjj");

            NodeRef testParentFolder =
                    getOrCreateParentTestFolder("GetChildrenCannedQueryTest-" + TEST_RUN_ID);

            // create folder subtype (note: system folder here)
            createFolder(testParentFolder, FOLDER_3, TEST_FOLDER_SUBTYPE);

            // create content subtype (note: no pun intended ... "cm:savedquery" already exists in
            // content model ... but is NOT related to canned queries !)
        ...
        NodeRef testFolder =
                createFolder(
                        ...
                        "GetChildrenCannedQueryTest-testFolder-" + TEST_RUN_ID,
                        QName.createQName(
                                "http://www.alfresco.org/test/getchildrentest/1.0", "folder"));
        createContent(testFolder, "textContent1", ContentModel.TYPE_CONTENT);
        createContent(
                testFolder,
                QName.createQName("http://www.alfresco.org/test/getchildrentest/1.0", "contains1"),
                "textContent2",
    ...
    private NodeRef getOrCreateParentTestFolder(String name) throws Exception {
        NodeRef testFolder =
                nodeService.getChildByName(
                        repositoryHelper.getCompanyHome(), ContentModel.ASSOC_CONTAINS, name);
        if (testFolder == null) {
            testFolder =
                    createFolder(repositoryHelper.getCompanyHome(), name, ContentModel.TYPE_FOLDER);
        }
        return testFolder;
    }
    ...
    public void testTypeFiltering() throws Exception {
        ...
        // Specific super-type (that likely does not exist in DB, at least yet - see ACE-5114 -
        // alternatively could create custom type to ensure this)
        // note: results should return 0

        childTypeQNames.clear();
        childTypeQNames.add(ContentModel.TYPE_LINK);

        PagingResults<NodeRef> results =
                list(parentNodeRef, -1, -1, 0, childTypeQNames, null, null);
    ...
    public void testPrimaryVsSecondary() throws Exception {
        ...
        NodeRef parentNodeRef1 =
                createFolder(
                        userHomeRef,
                        "GetChildrenCannedQueryTest-PrimaryVsSecondary-" + TEST_RUN_ID,
        ...
        filterPropsPrimary.add(
                new FilterPropBoolean(GetChildrenCannedQuery.FILTER_QNAME_NODE_IS_PRIMARY, true));

        List<FilterProp> filterPropsSecondary = new ArrayList<>(1);
        filterPropsSecondary.add(
                new FilterPropBoolean(GetChildrenCannedQuery.FILTER_QNAME_NODE_IS_PRIMARY, false));
        ...
        NodeRef parentNodeRef2 =
                getOrCreateParentTestFolder("GetChildrenCannedQueryTest-2-" + TEST_RUN_ID);
    ...
    public void testPropertyStringFiltering() throws Exception {
        ...
        filterByPropAndCheck(
                parentNodeRef,
                ContentModel.PROP_NAME,
                "GC-CQ-File-" + TEST_RUN + "-",
                FilterTypeString.STARTSWITH,
                5);
        filterByPropAndCheck(
                parentNodeRef,
                ContentModel.PROP_NAME,
        ...
        filterByPropAndCheck(
                ...
                ContentModel.PROP_TITLE,
                "zZ tItLe " + TEST_RUN,
                FilterTypeString.EQUALS_IGNORECASE,
                2);
        filterByPropAndCheck(
                parentNodeRef,
                ContentModel.PROP_TITLE,
        ...
        PagingResults<NodeRef> results =
                list(peopleContainerRef, -1, -1, 0, null, filterProps, null);
        assertEquals(2, results.getPage().size());
        ...
        filterProps.add(
                new FilterPropString(
                        ContentModel.PROP_NAME, "a", FilterTypeString.STARTSWITH_IGNORECASE));
        filterProps.add(
                new FilterPropString(
                        ContentModel.PROP_TITLE, "a", FilterTypeString.STARTSWITH_IGNORECASE));
        filterProps.add(
                new FilterPropString(
        ...
        filterProps.add(
                new FilterPropString(
                        ContentModel.PROP_CREATOR, "a", FilterTypeString.STARTSWITH_IGNORECASE));
        try {
            // -ve test
    ...
    private float getFiveStarRating() {
        float rating =
                random.nextFloat()
                                * (fiveStarRatingScheme.getMaxRating()
                                        - fiveStarRatingScheme.getMinRating())
                        + fiveStarRatingScheme.getMinRating();
        assertTrue(
                "Five star rating is out of range: " + rating,
                (rating >= fiveStarRatingScheme.getMinRating()
                        && rating <= fiveStarRatingScheme.getMaxRating()));
        return rating;
    }
    ...
    public void testPropertySorting() throws Exception {
        ...
        // Create some nodes with integer properties on which to sort, in this case
        // cm:fiveStarRatingScheme and cm:likesRatingScheme
        ...
        // note: initial test list derived from default Share DocLib
        ...
        sortQNames.add(
                QName.createQName(
                        NamespaceService.CONTENT_MODEL_1_0_URI, "fiveStarRatingSchemeCount"));
        sortQNames.add(
                QName.createQName(
                        NamespaceService.CONTENT_MODEL_1_0_URI, "likesRatingSchemeCount"));
        ...
        assertEquals(
                TEST_FILE_PREFIX + "quick.pdf",
                nodeService.getProperty(
                        results.getPage().get(0),
                        ContentModel.PROP_NAME)); // ZZ title + YY description
        assertEquals(
                TEST_FILE_PREFIX + "quick.txt",
                nodeService.getProperty(
                        results.getPage().get(1),
                        ContentModel.PROP_NAME)); // ZZ title + XX description

        // sort with two props - folders first (ALF-13968) then by name
        sortPairs = new ArrayList<Pair<QName, Boolean>>(3);
        sortPairs.add(
                new Pair<QName, Boolean>(GetChildrenCannedQuery.SORT_QNAME_NODE_IS_FOLDER, false));
        ...
        assertEquals(
                FOLDER_1,
                nodeService.getProperty(results.getPage().get(0), ContentModel.PROP_NAME));
        assertEquals(
                FOLDER_2,
                nodeService.getProperty(results.getPage().get(1), ContentModel.PROP_NAME));
        assertEquals(
                FOLDER_3,
                nodeService.getProperty(results.getPage().get(2), ContentModel.PROP_NAME));
        assertEquals(
                FOLDER_4,
                nodeService.getProperty(results.getPage().get(3), ContentModel.PROP_NAME));
        assertEquals(
                FOLDER_5,
                nodeService.getProperty(results.getPage().get(4), ContentModel.PROP_NAME));

        assertEquals(
                TEST_FILE_PREFIX + "quick.bmp",
                nodeService.getProperty(results.getPage().get(5), ContentModel.PROP_NAME));
        assertEquals(
                TEST_FILE_PREFIX + "quick.doc",
                nodeService.getProperty(results.getPage().get(6), ContentModel.PROP_NAME));

        sortPairs = new ArrayList<Pair<QName, Boolean>>(3);
        sortPairs.add(
                new Pair<QName, Boolean>(GetChildrenCannedQuery.SORT_QNAME_NODE_IS_FOLDER, true));
        ...
        assertEquals(
                FOLDER_5,
                nodeService.getProperty(results.getPage().get(len - 5), ContentModel.PROP_NAME));
        assertEquals(
                FOLDER_4,
                nodeService.getProperty(results.getPage().get(len - 4), ContentModel.PROP_NAME));
        assertEquals(
                FOLDER_3,
                nodeService.getProperty(results.getPage().get(len - 3), ContentModel.PROP_NAME));
        assertEquals(
                FOLDER_2,
                nodeService.getProperty(results.getPage().get(len - 2), ContentModel.PROP_NAME));
        assertEquals(
                FOLDER_1,
                nodeService.getProperty(results.getPage().get(len - 1), ContentModel.PROP_NAME));
    ...
    public void testPatterns() throws Exception {
        ...
        NodeRef nodeRef1 =
                createContent(
                        parentNodeRef,
                        "page.component-1-2.user~admin~dashboard.xml",
                        ContentModel.TYPE_CONTENT);
        NodeRef nodeRef2 =
                createContent(
                        parentNodeRef,
                        "page.component-1-4.user~admin~dashboard.xml",
                        ContentModel.TYPE_CONTENT);
        NodeRef nodeRef3 = createContent(parentNodeRef, "page.xml", ContentModel.TYPE_CONTENT);
        NodeRef nodeRef4 =
                createContent(
                        parentNodeRef,
                        "page.component-1-4.user~admin~panel.xml",
                        ContentModel.TYPE_CONTENT);
        NodeRef nodeRef5 =
                createContent(
                        parentNodeRef,
    ...
    public void testRestrictByAssocType() throws Exception {
        NodeRef parentNodeRef =
                getOrCreateParentTestFolder("GetChildrenCannedQueryTest-testFolder-" + TEST_RUN_ID);
        ...
        List<NodeRef> children =
                filterByAssocTypeAndCheck(parentNodeRef, assocTypeQNames, childTypeQNames);
    ...
    public void testAspectFiltering() throws Exception {
        ...
        Set<QName> includedAspects =
                new HashSet<QName>(Arrays.asList(new QName[] {ContentModel.ASPECT_CLASSIFIABLE}));
        results = list(parentNodeRef, includedAspects, null, -1, -1, 0);
        assertEquals(3, results.getPage().size());

        Set<QName> excludedAspects =
                new HashSet<QName>(Arrays.asList(new QName[] {RenditionModel.ASPECT_RENDITION}));
    ...
    private PagingResults<NodeRef> list(
            NodeRef parentNodeRef,
            final int skipCount,
            final int maxItems,
            final int requestTotalCountMax,
            String pattern,
        ...
        // get canned query
        GetChildrenCannedQueryFactory getChildrenCannedQueryFactory =
                (GetChildrenCannedQueryFactory) cannedQueryRegistry.getNamedObject(CQ_FACTORY_NAME);
        final GetChildrenCannedQuery cq =
        ...
        RetryingTransactionCallback<CannedQueryResults<NodeRef>> callback =
                new RetryingTransactionCallback<CannedQueryResults<NodeRef>>() {
                    @Override
                    public CannedQueryResults<NodeRef> execute() throws Throwable {
                        return cq.execute();
                    }
                };
        CannedQueryResults<NodeRef> results =
    ...
    private void filterByTypeAndCheck(
            NodeRef parentNodeRef, Set<QName> childTypeQNames, Set<QName> antiChildTypeQNames) {
        // belts-and-braces
        ...
        PagingResults<NodeRef> antiResults =
                list(parentNodeRef, -1, -1, 0, antiChildTypeQNames, null, null);
    ...
    private List<NodeRef> filterByAssocTypeAndCheck(
            NodeRef parentNodeRef, Set<QName> assocTypeQNames, Set<QName> childTypeQNames) {
        PagingResults<NodeRef> results =
                list(
                        parentNodeRef,
                        -1,
    ...
    private void filterByPropAndCheck(
            NodeRef parentNodeRef,
            QName filterPropQName,
            String filterVal,
    ...
    private void sortAndCheck(NodeRef parentNodeRef, QName sortPropQName, boolean sortAscending) {
        ...
        for (NodeRef nodeRef : results.getPage()) {
            ...
            if (sortPropQName.equals(GetChildrenCannedQuery.SORT_QNAME_CONTENT_SIZE)
                ...
                val =
                        dictionaryService.isSubClass(
                                nodeService.getType(nodeRef), ContentModel.TYPE_FOLDER);
            } else {
            ...
            if (prevVal == null) {
                ...
                String prevName =
                        (String) nodeService.getProperty(prevNodeRef, ContentModel.PROP_NAME);
    ...
    private PagingResults<NodeRef> list(
        ...
        // get canned query (note: test the fileFolder extension - including support for sorting
        // folders first)
        GetChildrenCannedQueryFactory getChildrenCannedQueryFactory =
                (GetChildrenCannedQueryFactory) cannedQueryRegistry.getNamedObject(CQ_FACTORY_NAME);
        final GetChildrenCannedQuery cq =
    ...
    private class PagingNodeRefResultsImpl implements PagingResults<NodeRef> {
        ...
        public PagingNodeRefResultsImpl(
                List<NodeRef> nodeRefs,
                boolean hasMorePages,
                Integer totalResultCount,
                Boolean isTotalResultCountCutoff) {
        ...
        public Pair<Integer, Integer> getTotalResultCount() {
            return new Pair<Integer, Integer>(
                    totalResultCount, (isTotalResultCountCutoff ? null : totalResultCount));
        }
    ...
    private NodeRef createContent(NodeRef parentNodeRef, String fileName, QName contentType)
            throws IOException {
        Map<QName, Serializable> properties = new HashMap<QName, Serializable>();
        ...
        NodeRef nodeRef =
                nodeService.getChildByName(parentNodeRef, ContentModel.ASSOC_CONTAINS, fileName);
        if (nodeRef != null) {
            nodeService.deleteNode(nodeRef);
        }
    ...
    private NodeRef createContent(
            NodeRef parentNodeRef, QName childAssocType, String fileName, QName contentType)
            throws IOException {
        Map<QName, Serializable> properties = new HashMap<QName, Serializable>();
        properties.put(ContentModel.PROP_NAME, fileName);
        properties.put(ContentModel.PROP_TITLE, fileName + " my title");

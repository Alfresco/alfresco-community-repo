...
public class SolrSQLHttpClient extends AbstractSolrQueryHTTPClient implements SolrQueryClient {
    ...
    private HashMap<StoreRef, SolrStoreMappingWrapper> mappingLookup =
            new HashMap<StoreRef, SolrStoreMappingWrapper>();
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        for (SolrStoreMapping mapping : storeMappings) {
            mappingLookup.put(
                    mapping.getStoreRef(),
                    new ExplicitSolrStoreMappingWrapper(mapping, beanFactory));
        }
    }
    ...
    public ResultSet executeQuery(SearchParameters searchParameters, String language) {
        ...
        try {
            ...
            if (searchParameters.getTimezone() != null
                    && !searchParameters.getTimezone().isEmpty()) {
            ...
            if (mapping.isSharded()) {
                ...
                String solrurl =
                        httpClient.getHostConfiguration().getHostURL()
                                + httpClientAndBaseUrl.getSecond();
                url.append(solrurl);
            }
            JSONObject body = new JSONObject();

            // Authorities go over as is - and tenant mangling and query building takes place on the
            // SOLR side
    ...
    protected ResultSet postSolrQuery(
            HttpClient httpClient, String url, JSONObject body, SolrJsonProcessor<?> jsonProcessor)
    ...
    /**
     * When set, a single DENIED ACL entry for any authority will result in access being denied as a
     * whole. See system property {@code security.anyDenyDenies}
     *

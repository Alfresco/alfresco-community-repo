...
public class FeedNotifierTest {
  ...
  public void before() throws Exception {
    ChildApplicationContextFactory activitiesFeed = (ChildApplicationContextFactory) ctx.getBean(
      "ActivitiesFeed"
    );
    ApplicationContext activitiesFeedCtx = activitiesFeed.getApplicationContext();
    this.feedNotifier =
      (FeedNotifierImpl) activitiesFeedCtx.getBean("feedNotifier");
    this.activityService =
      (ActivityService) activitiesFeedCtx.getBean("activityService");
    this.postLookup = (PostLookup) activitiesFeedCtx.getBean("postLookup");
    this.feedGenerator =
      (FeedGenerator) activitiesFeedCtx.getBean("feedGenerator");
    ObjectFactory<ActivitiesFeedModelBuilder> feedModelBuilderFactory = (ObjectFactory<ActivitiesFeedModelBuilder>) activitiesFeedCtx.getBean(
      "feedModelBuilderFactory"
    );

    Scheduler scheduler = (Scheduler) ctx.getBean("schedulerFactory");

    JobDetail feedGeneratorJobDetail = (JobDetail) activitiesFeedCtx.getBean(
      "feedGeneratorJobDetail"
    );
    JobDetail postLookupJobDetail = (JobDetail) activitiesFeedCtx.getBean(
      "postLookupJobDetail"
    );
    JobDetail feedCleanerJobDetail = (JobDetail) activitiesFeedCtx.getBean(
      "feedCleanerJobDetail"
    );
    JobDetail postCleanerJobDetail = (JobDetail) activitiesFeedCtx.getBean(
      "postCleanerJobDetail"
    );
    JobDetail feedNotifierJobDetail = (JobDetail) activitiesFeedCtx.getBean(
      "feedNotifierJobDetail"
    );
    ...
    this.subscriptionService =
      (SubscriptionService) ctx.getBean("SubscriptionService");
    this.errorProneActionExecutor =
      (ErrorProneActionExecutor) ctx.getBean("errorProneActionExecutor");
        ...
        new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
          ...
          public Void execute() throws Throwable {
            ...
            NodeRef workingRootNodeRef = nodeService
              ...
              .getChildRef();
            FileInfo file1 = fileFolderService.create(
              workingRootNodeRef,
              GUID.generate(),
  ...
  public void testFailedNotifications() {
    ...
    assertEquals(
      numSuccessfulNotifications,
      errorProneActionExecutor.getNumSuccess()
    );
  }
  ...
  public void testNullSiteNetworkStatus() throws Exception {
    ...
    for (ActivityFeedEntity feed : feeds) {
      if (
        feed.getActivityType().equals(activityType) &&
        ActivitiesDAO.KEY_ACTIVITY_NULL_VALUE.equals(feed.getSiteNetwork())
      ) {
        found = true;
      }
    }

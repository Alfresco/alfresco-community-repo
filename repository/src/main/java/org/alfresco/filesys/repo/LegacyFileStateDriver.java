...
public class LegacyFileStateDriver implements ExtendedDiskInterface {
  ...
  public NetworkFile openFile(
    ...
    if (tctx.hasStateCache()) {
      ...
      if (!params.isDirectory()) {
        try {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug(
              "UNABLE to grant file access for path:" +
              path +
              ", params" +
              params,
              e
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "open file created lock token:" + token + ", for path:" + path
          );
        }
      }
    }
    ...
    try {
      ...
      if (openFile instanceof NetworkFileLegacyReferenceCount) {
        ...
        logger.debug(
          "openFile does not implement NetworkFileLegacyReferenceCount"
        );
      }
      ...
      if (openFile.hasAccessToken()) {
        ...
        if (cache != null && fstate != null && token != null) {
          if (logger.isDebugEnabled()) {
            logger.debug(
              "already has access token, release lock token:" + token
            );
          }
          cache.releaseFileAccess(fstate, token);
        }
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "store access token on open network file object token:" + token
          );
        }
  ...
  public void closeFile(SrvSession sess, TreeConnection tree, NetworkFile file)
    ...
    if (file instanceof NetworkFileLegacyReferenceCount) {
      ...
      logger.debug(
        "file to close does not implement NetworkFileLegacyReferenceCount"
      );
    }
    ...
    try {
      if (file.hasOpLock()) {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "File Has OpLock - release oplock for closed file, file=" +
            file.getFullName()
          );
        }
        // Release the oplock
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Released oplock for closed file, file=" + file.getFullName()
          );
        }
      }
      ...
      if (file.hasLocks()) {
        ...
        if (lockMgr != null) {
          if (logger.isDebugEnabled()) {
            logger.debug(
              "Releasing locks for closed file, file=" +
              file.getFullName() +
              ", locks=" +
              file.numberOfLocks()
            );
      ...
      if (tctx.hasStateCache()) {
        ...
        if (legacyOpenCount == 0 || file.isForce()) {
          if (
            cache != null && fstate != null && file.getAccessToken() != null
          ) {
            FileAccessToken token = file.getAccessToken();
            if (logger.isDebugEnabled() && token != null) {
              logger.debug(
                "close file, legacy count == 0 release access token:" + token
              );
            }
            cache.releaseFileAccess(fstate, token);
  ...
  public void setFileInformation(
    ...
    if (tctx.hasStateCache()) {
      ...
      if (info.hasSetFlag(FileInfo.SetModifyDate)) {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Set modification date in file state cache" +
            name +
            ", " +
            info.getModifyDateTime()
          );
        }
  ...
  public SearchContext startSearch(
    ...
    SearchContext ctx = diskInterface.startSearch(
      sess,
      tree,
      searchPath,
      attrib
  ...
  public void setFileLockingInterface(
    FileLockingInterface fileLockingInterface
  ) {

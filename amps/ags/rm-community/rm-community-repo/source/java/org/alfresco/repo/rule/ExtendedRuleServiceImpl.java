...
public class ExtendedRuleServiceImpl extends RuleServiceImpl {
            ...
            for (Action action : compositeAction.getActions()) {
                if (WORM_LOCK_ACTION.equals(action.getActionDefinitionName())) {
                    String retentionPeriodParamValue =
                            (String) action.getParameterValue(PARAM_RETENTION_PERIOD);
                    if (retentionPeriodParamValue != null) {
                        Matcher matcher = pattern.matcher(retentionPeriodParamValue);
                        if (!matcher.matches()) {
                            throw new IntegrityException(
                                    I18NUtil.getMessage(MSG_RETENTION_PERIOD_NOT_VALID), null);
                        }
                    }
                }
            }
    ...
    /**
     * @see
     *     org.alfresco.repo.rule.RuleServiceImpl#saveRule(org.alfresco.service.cmr.repository.NodeRef,
    ...
    /**
     * @see
     *     org.alfresco.repo.rule.RuleServiceImpl#removeRule(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.cmr.rule.Rule)
     */
    @Override
    ...
    /**
     * @see org.alfresco.repo.rule.RuleServiceImpl#executeRule(org.alfresco.service.cmr.rule.Rule,
     *     org.alfresco.service.cmr.repository.NodeRef, java.util.Set)
     */
    @Override
    public void executeRule(
            final Rule rule, final NodeRef nodeRef, final Set<ExecutedRuleData> executedRules) {
        if (nodeService.exists(nodeRef)) {
            QName typeQName = nodeService.getType(nodeRef);

            // check if this is a rm rule on a rm artifact
            if (filePlanService.isFilePlanComponent(nodeRef) && isFilePlanComponentRule(rule)) {
                // ignore and

...
public class TestNetworks extends EnterpriseTestApi {
  ...
  public void setup() {
    ...
    for (int i = 0; i < 2; i++) {
      final TestNetwork network = repoService.createNetworkWithAlias(
        "network" + i,
        true
      );
      network.create();
  ...
  public void testPersonNetworks() throws Exception {
...

    {
      ...
      HttpResponse response = publicApiClient.delete(
        null,
        null,
        null,
        null,
      ...
      PublicApiClient.ExpectedErrorResponse errorResponse = RestApiUtil.parseErrorResponse(
...

      {
        ...
        publicApiClient.setRequestContext(
          new RequestContext("-default-", person.getId())
        );
        response = publicApiClient.index(createParams(paging, null));
        ListResponse<PersonNetwork> resp = PersonNetwork.parseNetworkMembers(
          response.getJsonResponse()
        );
        assertEquals(200, response.getStatusCode());

        checkList(
          new ArrayList<PersonNetwork>(expectedNetworkMembers),
    ...
    {
      publicApiClient.setRequestContext(
        new RequestContext("-default-", person21.getId())
      );
      ...
      try {
        ...
        peopleProxy.getNetworkMemberships(
          person11.getId(),
    ...
    try {
      ...
      publicApiClient.setRequestContext(
        new RequestContext("-default-", person12.getId())
      );
    ...
    {
      publicApiClient.setRequestContext(
        new RequestContext("-default-", person11.getId())
      );
      ...
      ListResponse<PersonNetwork> resp = peopleProxy.getNetworkMemberships(
        person11.getId(),
        createParams(paging, null)
      );
      checkList(
        networksMemberships.subList(
          skipCount,
    ...
    try {
      ...
      int skipCount = 0;
      int maxItems = 2;
      Paging expectedPaging = getPaging(
        skipCount,
        maxItems,
    ...
    try {
      ...
      publicApiClient.setRequestContext(
        new RequestContext("-default-", person11.getId(), GUID.generate())
      );
    ...
    try {
      ...
      peopleProxy.remove(
        ...
        "networks",
        null,
        "Unable to DELETE person networks"
      );
      fail("");
    } catch (PublicApiException e) {
      assertEquals(
  ...
  public void testCLOUD1856() throws Exception {
    ...
    publicApiClient.setRequestContext(
      new RequestContext(network3.getId(), person31.getId())
    );

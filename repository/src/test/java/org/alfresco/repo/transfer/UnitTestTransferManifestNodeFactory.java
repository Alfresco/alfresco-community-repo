...
/**
 * This is a test class to enable unit testing on a single machine. Since the single machine will
 * already have the target node.
 *
 * @author Mark Rogers
 */
public class UnitTestTransferManifestNodeFactory implements TransferManifestNodeFactory {
    ...
    public TransferManifestNode createTransferManifestNode(
            NodeRef nodeRef, TransferDefinition definition, TransferContext transferContext) {
    ...
    public TransferManifestNode createTransferManifestNode(
            ...
            boolean forceDelete) {
        TransferManifestNode newNode =
                realFactory.createTransferManifestNode(nodeRef, definition, transferContext);
        ...
        newNode.setPrimaryParentAssoc(
                new ChildAssociationRef(
                        primaryParentAssoc.getTypeQName(),
                        mappedParentNodeRef,
                        primaryParentAssoc.getQName(),
                        mappedNodeRef,
                        primaryParentAssoc.isPrimary(),
        ...
        if (newNode instanceof TransferManifestNormalNode) {
            ...
            for (AssociationRef ref : source) {
                mappedSourceAssocs.add(
                        new AssociationRef(
                                6L,
                                getMappedNodeRef(ref.getSourceRef()),
                                ref.getTypeQName(),
                                getMappedNodeRef(ref.getTargetRef())));
            }

            for (AssociationRef ref : target) {
                mappedTargetAssocs.add(
                        new AssociationRef(
                                6L,
                                getMappedNodeRef(ref.getSourceRef()),
                                ref.getTypeQName(),
    ...
    public Path getMappedPath(Path from) {
        ...
        for (int i = 0; i < from.size(); i++) {
            ...
            for (Pair<Path, Path> xx : getPathMap()) {
                // Can't use direct equals because of mismatched node refs (which we don't care
                // about)
    ...
    private NodeRef mapNodeRef(NodeRef in) {
        ...
        if (mappedNodeRef == null) {
            /**
             * Map the node ref by replacing the 36th digit with a Z. The existing UUID could have
             * 0-9 1-F in the 36th digit
             */

...
public class ScriptAction implements Serializable, Scopeable {
  ...
  public Map<String, Serializable> getParameters() {
    if (this.parameters == null) {
      ...
      for (Map.Entry<String, Serializable> entry : actionParams.entrySet()) {
        ...
        this.parameters.put(
            name,
            converter.convertActionParamForScript(name, entry.getValue())
          );
      }
  ...
  public void execute(final ScriptNode node, boolean readOnly, boolean newTxn) {
    ...
    transactionService
      ...
      .doInTransaction(executionActionCallback, readOnly, newTxn);
  ...
  protected void performParamConversionForRepo() {
    if (this.parameters != null && this.parameters.isModified()) {
      ...
      for (Map.Entry<String, Serializable> entry : this.parameters.entrySet()) {
        ...
        String name = entry.getKey();
        Serializable value = converter.convertActionParamForRepo(
          name,
          entry.getValue()
        );
  ...
  private class ActionValueConverter extends ValueConverter {
     ...
     */
    @SuppressWarnings("synthetic-access")
    public Serializable convertActionParamForScript(
      String paramName,
      Serializable value
    ) {
      ParameterDefinition paramDef = actionDef.getParameterDefintion(paramName);
      if (
        paramDef != null && paramDef.getType().equals(DataTypeDefinition.QNAME)
      ) {
     ...
     */
    @SuppressWarnings("synthetic-access")
    public Serializable convertActionParamForRepo(
      String paramName,
      Serializable value
    ) {
      ParameterDefinition paramDef = actionDef.getParameterDefintion(paramName);
      ...
      if (
        ...
        if (value instanceof Wrapper) {
          ...
          return convertActionParamForRepo(
            paramName,
            (Serializable) ((Wrapper) value).unwrap()
          );
        } else {

...
public class BaseUnitTest implements RecordsManagementModel, ContentModel {
  ...
  protected NodeRef generateHoldNodeRef(String name) {
    ...
    doReturn(name)
      .when(mockedNodeService)
      .getProperty(hold, ContentModel.PROP_NAME);
  ...
  protected void setupAsFilePlanComponent(NodeRef nodeRef) {
    doReturn(true)
      .when(mockedNodeService)
      .hasAspect(nodeRef, ASPECT_FILE_PLAN_COMPONENT);
   ...
   */
  protected ChildAssociationRef generateChildAssociationRef(
    NodeRef parent,
    NodeRef child
  ) {
    ChildAssociationRef mockedChildAssociationRef = mock(

...
public class LDAPAuthenticationComponentImpl
  ...
  implements InitializingBean, ActivateableBean {
  ...
  public void setUserNameFormat(String userNameFormat) {
    this.userNameFormat =
      userNameFormat == null || userNameFormat.length() == 0
        ? null
        : userNameFormat;
  }
  ...
  public void afterPropertiesSet() throws Exception {
    if (this.ldapNameResolver == null && this.userNameFormat == null) {
      throw new IllegalStateException(
        "At least one of ldapNameResolver and userNameFormat must be set"
      );
    }
  }
  ...
  protected void authenticateImpl(String userName, char[] password)
    ...
    if (logger.isTraceEnabled()) {
      logger.trace(
        "Authentication for user: " + AuthenticationUtil.maskUsername(userName)
      );
    }
    ...
    if (userNameFormat == null) {
      ...
      diagnostic.addStep(
        AuthenticationDiagnostic.STEP_KEY_LDAP_LOOKEDUP_USER,
        true,
        params
      );
    } else { // ghost users (MNT-2597) // Otherwise, use the format, but disallow leading or trailing whitespace in the user ID as this can result in
      if (!userName.equals(userName.trim())) {
        throw new AuthenticationException(
          "Invalid user ID with leading or trailing whitespace"
        );
      }
      ...
      userDN =
        String.format(
          userNameFormat,
          new Object[] { escapeUserName(userName, escapeCommasInBind) }
        );
      Object[] params = { userName, userDN, userNameFormat };
      diagnostic.addStep(
        AuthenticationDiagnostic.STEP_KEY_LDAP_FORMAT_USER,
        true,
        params
      );
    }
    ...
    try {
      ctx =
        ldapInitialContextFactory.getInitialDirContext(
          userDN,
          new String(password),
          diagnostic
        );

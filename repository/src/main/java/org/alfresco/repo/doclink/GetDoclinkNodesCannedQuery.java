...
public class GetDoclinkNodesCannedQuery extends AbstractCannedQuery<Long> {
    ...
    private static final String QUERY_SELECT_GET_DOCLINK_NODES =
            "select_GetDoclinkNodesCannedQuery";
    ...
    protected List<Long> queryAndFilter(CannedQueryParameters parameters) {
        ...
        // Get parameters
        GetDoclinkNodesCannedQueryParams paramBean =
                (GetDoclinkNodesCannedQueryParams) paramBeanObj;
        ...
        List<Long> nodeIds =
                cannedQueryDAO.executeQuery(
                        QUERY_NAMESPACE,
                        QUERY_SELECT_GET_DOCLINK_NODES,
                        paramBean,
                        0,
                        paramBean.getLimit());
        ...
        if (logger.isDebugEnabled()) {
            ...
            if (paramBean.getLimit() == nodeIds.size()) {
                logger.debug(
                        "Node "
                                + paramBean.getParentNodeStringValue()
                                + " has at least 100,000 link nodes attached. Results have been"
                                + " truncated.");
            }
        }

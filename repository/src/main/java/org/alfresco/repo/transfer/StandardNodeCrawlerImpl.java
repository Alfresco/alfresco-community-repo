...
/**
 * This class can be used to build a set of node references from a given starting point. The caller
 * can provide a list of {@link NodeFinder} objects and a list of {@link NodeFilter} objects.
 * Starting with the nodes supplied by the caller, the crawler uses the NodeFinder objects to find
 * other nodes. Each node that is found is then passed to the NodeFilter objects to determine
 * whether it should be included or ignored. Any included nodes are then fed back into the
 * NodeFinder objects to continue the crawl. This class was originally written to assist users of
 * the {@link TransferService} in combination with the {@link ChildAssociatedNodeFinder} and the
 * {@link ContentClassFilter}.
 *
 * @author brian
 */
public class StandardNodeCrawlerImpl implements NodeCrawler {
    ...
    public synchronized Set<NodeRef> crawl(Set<NodeRef> startingNodes) {
        ...
        while (nodesToProcess.peek() != null) {
            ...
            // Check that we haven't already processed it. Skip it if we have, process it if we
            // haven't

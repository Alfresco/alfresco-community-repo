...
public class BroadcastVitalRecordDefinitionAction
  ...
  private void propagateChangeToChildrenOf(NodeRef actionedUponNodeRef) {
    ...
    for (ChildAssociationRef nextAssoc : assocs) {
      ...
      if (
        filePlanService.isFilePlanComponent(nextChild) &&
        ...
        if (getRecordService().isRecord(nextChild)) {
          if (parentVri) {
            VitalRecordDefinition vrDefn = getVitalRecordService()
              .getVitalRecordDefinition(nextChild);
          ...
          Map<QName, Serializable> childProps = getNodeService()
            .getProperties(nextChild);

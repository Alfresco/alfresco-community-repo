...
public abstract class QueryHelper {
    /**
     * An interface used when walking a query tree. Calls are made to methods when the particular
    ...
    /**
     * Default implementation. Override the methods you are interested in. If you don't override the
     * methods then an InvalidQueryException will be thrown.
     */
    public static class WalkerCallbackAdapter implements WalkerCallback {
        ...
        protected static final InvalidQueryException UNSUPPORTED =
                new InvalidQueryException(UNSUPPORTED_TEXT);
        ...
        public void comparison(
                int type, String propertyName, String propertyValue, boolean negated) {
            throw UNSUPPORTED;
        }
                ...
                case WhereClauseParser.BETWEEN:
                    if (WhereClauseParser.PROPERTYNAME == tree.getChild(0).getType()) {
                        callback.between(
                                tree.getChild(0).getText(),
                                stripQuotes(tree.getChild(1).getText()),
                                stripQuotes(tree.getChild(2).getText()),
                                negated);
                        return;
                    }
                    break;
    ...
    /**
     * Finds the siblings of the current tree item (does not include the current item) that are
     * after it in the tree (but at the same level).
     *
    ...
    public static String stripQuotes(String toBeStripped) {
        if (StringUtils.isNotEmpty(toBeStripped)
                && toBeStripped.startsWith(SINGLE_QUOTE)

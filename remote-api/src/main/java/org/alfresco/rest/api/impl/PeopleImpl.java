...
public class PeopleImpl implements People {
    ...
    private static final List<String> EXCLUDED_NS =
            Arrays.asList(
    ...
    private static final String[] RESERVED_AUTHORITY_PREFIXES = {
        PermissionService.GROUP_PREFIX, PermissionService.ROLE_PREFIX
    };
    ...
    protected void processPersonProperties(
            String userName, final Map<QName, Serializable> nodeProps) {
        ...
        // The person description is located in a separate content file located at
        // cm:persondescription
        // "Inline" this data, by removing the cm:persondescription property and adding a temporary
        // property
        // (Person.PROP_PERSON_DESCRIPTION) containing the actual content as a string.
        final ContentData personDescription =
                (ContentData) nodeProps.get(ContentModel.PROP_PERSONDESC);
                    ...
                    new RunAsWork<Void>() {
                        ...
                        public Void doWork() throws Exception {
                            ContentReader reader =
                                    contentService.getRawReader(personDescription.getContentUrl());
        ...
        if (personNode != null) {
            ...
            avatar =
                    thumbnailService.getThumbnailByName(
                            avatarOrig, ContentModel.PROP_CONTENT, "avatar");
        }
    ...
    private NodeRef getAvatarOriginal(NodeRef personNode) {
        ...
        List<ChildAssociationRef> avatarChildAssocs =
                nodeService.getChildAssocs(
                        personNode, Collections.singleton(ContentModel.ASSOC_PREFERENCE_IMAGE));
        if (avatarChildAssocs.size() > 0) {
            ...
            List<AssociationRef> avatorAssocs =
                    nodeService.getTargetAssocs(personNode, ContentModel.ASSOC_AVATAR);
    ...
    @Override
    public Person uploadAvatarContent(
            String personId,
        ...
        if (mimeType.indexOf("image/") != 0) {
            throw new InvalidArgumentException(
                    "Uploaded content must be an image (content type determined to be '"
                            + mimeType
                            + "')");
        }
    ...
    public CollectionWithPagingInfo<Person> getPeople(final Parameters parameters) {
        ...
        // please see REPO-555
        final PagingResults<PersonService.PersonInfo> pagingResult =
                personService.getPeople(null, null, sortProps, pagingRequest);
        ...
        List<Person> people =
                new AbstractList<Person>() {
                    @Override
                    public Person get(int index) {
                        PersonService.PersonInfo personInfo = page.get(index);
                        Person person =
                                getPersonWithProperties(
                                        personInfo.getUserName(), parameters.getInclude());
                        return person;
                    }
        ...
        return CollectionWithPagingInfo.asPaged(
                paging, people, pagingResult.hasMoreItems(), totalItems);
    }
    ...
    private List<Pair<QName, Boolean>> getSortProps(Parameters parameters) {
        ...
        if ((sortCols != null) && (sortCols.size() > 0)) {
            for (SortColumn sortCol : sortCols) {
                ...
                sortProps.add(
                        new Pair<>(sortPropQName, (sortCol.asc ? Boolean.TRUE : Boolean.FALSE)));
            }
        } else {
            // default sort order
    ...
    private Person getPersonWithProperties(String personId, List<String> include) {
        ...
        if (personNode != null) {
            ...
            // Remove the temporary property used to help inline the person description content
            // property.
            ...
            if (include.contains(PARAM_INCLUDE_PROPERTIES)) {
                ...
                Map<String, Object> custProps =
                        nodes.mapFromNodeProperties(
                                nodeProps,
                                new ArrayList<>(),
                                new HashMap<>(),
                                EXCLUDED_NS,
            ...
            if (include.contains(PARAM_INCLUDE_ASPECTNAMES)) {
                ...
                person.setAspectNames(
                        nodes.mapFromNodeAspects(aspects, EXCLUDED_NS, EXCLUDED_ASPECTS));
            }
    ...
    public Person create(Person person) {
        ...
        if (!isAdminAuthority()) {
            // note: do an explict check for admin here (since personExists does not throw 403
            // unlike createPerson,
        ...
        // no sensible way to know that it was thrown due to the user already existing - hence this
        // check here.
        if (personService.personExists(person.getUserName())) {
            throw new ConstraintViolatedException(
                    "Person '" + person.getUserName() + "' already exists.");
        }
                        ...
                        if (description != null) {
                            ContentWriter writer =
                                    contentService.getWriter(
                                            nodeRef, ContentModel.PROP_PERSONDESC, true);
    ...
    private void validateUsername(String username) {
        if (username.length() > 100) {
            throw new InvalidArgumentException(
                    "Username exceeds max length of " + USERNAME_MAXLENGTH + " characters.");
        }

        for (String prefix : RESERVED_AUTHORITY_PREFIXES) {
            if (username.toUpperCase().startsWith(prefix)) {
                throw new IllegalArgumentException(
                        "Username cannot start with the reserved prefix '" + prefix + "'.");
            }
        }
    }
    ...
    private void checkRequiredField(String fieldName, Object fieldValue) {
        if (fieldValue == null) {
            throw new InvalidArgumentException(
                    "Field '" + fieldName + "' is null, but is required.");
        }

        // belts-and-braces - note: should not see empty string (since converted to null via custom
        // json deserializer)
        if ((fieldValue instanceof String) && ((String) fieldValue).isEmpty()) {
            throw new InvalidArgumentException(
                    "Field '" + fieldName + "' is empty, but is required.");
        }
    }
    ...
    public Person update(String personId, final Person person) {
        ...
        if (person.isEnabled() != null) {
            ...
            mutableAuthenticationService.setAuthenticationEnabled(
                    personIdToUpdate, person.isEnabled());
        }
        ...
        Set<QName> immutableProperties =
                userRegistrySynchronizer.getPersonMappedProperties(personIdToUpdate);
    ...
    private void validateUpdatePersonData(Person person) {
        ...
        if (person.wasSet(ContentModel.PROP_EMAIL_FEED_DISABLED)
                && (person.isEmailNotificationsEnabled() == null)) {
            throw new IllegalArgumentException(
                    "'emailNotificationsEnabled' field cannot be empty.");
        }
    }
    ...
    private void updatePassword(boolean isAdmin, String personIdToUpdate, Person person) {
        ...
        if (isPassword || isOldPassword) {
            if (isOldPassword
                    && ((person.getOldPassword() == null) || (person.getOldPassword().isEmpty()))) {
            ...
            if (!isAdmin) {
                // Non-admin users can update their own password, but must provide their current
                // password.
                if (!isOldPassword) {
                    throw new IllegalArgumentException(
                            "To change password, both 'oldPassword' and 'password' fields are"
                                    + " required.");
                }

                char[] oldPassword = person.getOldPassword().toCharArray();
                try {
                    mutableAuthenticationService.updateAuthentication(
                            personIdToUpdate, oldPassword, newPassword);
                } catch (AuthenticationException e) {
                    throw new PermissionDeniedException("Incorrect password.");
                }
            } else {
                // An admin user can update without knowing the original pass - but must know their
                // own!
        ...
        AuthenticationUtil.runAsSystem(
                () -> {
                    try {
                        ...
                        // For security reason (prevent the attackers to determine that userId
    ...
    public void resetPassword(String personId, final PasswordReset passwordReset) {
        ...
        ResetPasswordDetails resetDetails =
                new ResetPasswordDetails()
                        .setUserId(personId)

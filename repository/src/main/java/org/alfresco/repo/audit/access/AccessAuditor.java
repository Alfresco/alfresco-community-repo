...
/**
 * Creates high level audit records on the creation, deletion, modification and access of content
 * and folders. Lower level events are grouped together by transaction and node.
 *
 * <p>To turn on auditing of these events and sub events add the following property to
 ...
 * </pre>
 *
 * The following properties are set by default to discard events where the user is 'null' or
 * 'System', the node path is '/sys:archivedItem' or under '/ver:' or the node type is not
 * 'cm:folder', 'cm:content' or 'st:site'. These values result in events only being recorded for
 * common actions initiated by users of the system. These vales may be overridden if required.
 *
 * <pre>
 ...
 * </pre>
 *
 * Node and Content changes generate the following audit structure. Elements are omitted if not
 * changed by the transaction. The {@code /sub-action/<sequence>} structure holds cut down details
 * of each sub-action, but are only included if the global property {@code
 * audit.alfresco-access.sub-actions.enabled=true}.
 *
 * <pre>
 *    /alfresco-access
 ...
 * </pre>
 *
 * The trace output from this class may be useful to developers as it logs method calls grouped by
 * transaction. The debug output is of the audit records written and full inbound audit data.
 * However for developers trace will provide a more readable form. Setting the following
...
public class AccessAuditor
    ...
    public void setProperties(Properties properties) {
        ...
        auditSubActions =
                properties.getProperty(AUDIT_SUB_ACTIONS, "false").equalsIgnoreCase("true");
    }
    ...
    public void afterPropertiesSet() {
        ...
        policyComponent.bindClassBehaviour(
                OnCreateNodePolicy.QNAME, this, new JavaBehaviour(this, "onCreateNode"));
        policyComponent.bindClassBehaviour(
        ...
        policyComponent.bindClassBehaviour(
                OnAddAspectPolicy.QNAME, this, new JavaBehaviour(this, "onAddAspect"));
        policyComponent.bindClassBehaviour(
                OnRemoveAspectPolicy.QNAME, this, new JavaBehaviour(this, "onRemoveAspect"));

        policyComponent.bindClassBehaviour(
                OnContentUpdatePolicy.QNAME, this, new JavaBehaviour(this, "onContentUpdate"));
        policyComponent.bindClassBehaviour(
                OnContentReadPolicy.QNAME, this, new JavaBehaviour(this, "onContentRead"));
        ...
        policyComponent.bindClassBehaviour(
                OnCheckOut.QNAME, ContentModel.TYPE_CONTENT, new JavaBehaviour(this, "onCheckOut"));
        policyComponent.bindClassBehaviour(
                OnCheckIn.QNAME, ContentModel.TYPE_CONTENT, new JavaBehaviour(this, "onCheckIn"));
        policyComponent.bindClassBehaviour(
                OnCancelCheckOut.QNAME,
                ContentModel.TYPE_CONTENT,
    ...
    public void onMoveNode(
        ...
        if (auditEnabled()) {
            getNodeChange(toChildAssocRef.getChildRef())
                    .onMoveNode(fromChildAssocRef, toChildAssocRef);
        }
    }
    ...
    public void onCreateVersion(
        ...
        if (auditEnabled()) {
            getNodeChange(nodeRef)
                    .onCreateVersion(classRef, nodeRef, versionProperties, nodeDetails);
        }
    }
    ...
    public void onCopyComplete(
        ...
        if (auditEnabled()) {
            getNodeChange(targetNodeRef)
                    .onCopyComplete(classRef, sourceNodeRef, targetNodeRef, copyToNewNode, copyMap);
        }
    }
    ...
    /**
     * @return the {@link NodeChange} for the supplied {@code nodeRef} from the current transaction
     *     context or create one if required.
     */
     ...
     *
     * @param action String giving the action performed. Becomes the second component of the audit
     *     path after the root path.
    ...
    private boolean recordAuditValues(String action, Map<String, Serializable> auditMap) {
        ...
        Map<String, Serializable> recordedAuditMap =
                auditComponent.recordAuditValues(rootPath, auditMap);
                ...
                for (String key : new TreeSet<String>(auditMap.keySet())) {
                    if (!devOutput || !NodeChange.SUMMARY_KEYS.contains(key)) {
                        StringBuilder output =
                                (key.startsWith(NodeChange.SUB_ACTION_PREFIX)) ? subActions : sb;
    ...
    /**
     * Appends a more readable version of an audit map value. The prefix is used for {@link QName}
     * values, including when used in Maps, Sets and Lists.
     */

...
public class CommentsPost extends AbstractCommentsWebScript {
    ...
    private Map<String, Object> generateItemValue(NodeRef commentNodeRef) {
        ...
        String creator =
                (String) this.nodeService.getProperty(commentNodeRef, ContentModel.PROP_CREATOR);

        Serializable created =
                this.nodeService.getProperty(commentNodeRef, ContentModel.PROP_CREATED);
        Serializable modified =
                this.nodeService.getProperty(commentNodeRef, ContentModel.PROP_MODIFIED);
        ...
        boolean isSiteManager =
                this.permissionService.hasPermission(commentNodeRef, SiteModel.SITE_MANAGER)
                        == (AccessStatus.ALLOWED);
        boolean isCoordinator =
                this.permissionService.hasPermission(commentNodeRef, PermissionService.COORDINATOR)
                        == (AccessStatus.ALLOWED);
        ...
        if (nodeService.hasAspect(nodeRef, ForumModel.ASPECT_DISCUSSABLE)) {
            List<ChildAssociationRef> assocs =
                    nodeService.getChildAssocs(
                            nodeRef, ForumModel.ASSOC_DISCUSSION, RegexQNamePattern.MATCH_ALL);
            ChildAssociationRef firstAssoc = assocs.get(0);

            return nodeService.getChildByName(
                    firstAssoc.getChildRef(), ContentModel.ASSOC_CONTAINS, COMMENTS_TOPIC_NAME);
        } else {
            return null;
        }
    }
                ...
                AuthenticationUtil.runAs(
                        new AuthenticationUtil.RunAsWork<NodeRef>() {
                            public NodeRef doWork() throws Exception {
                                ...
                                try {
                                    // MNT-12082: set System user for creating forumFolder and
                                    // commentsFolder nodes

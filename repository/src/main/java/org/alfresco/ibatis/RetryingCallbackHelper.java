...
public class RetryingCallbackHelper {
    ...
    /**
     * Execute a callback until it succeeds, fails or until a maximum number of retries have been
     * attempted.
     *
        ...
        for (int count = 0; count == 0 || count < maxRetries; count++) {
            try {
                ...
                if (logger.isDebugEnabled()) {
                    ...
                    String msg =
                            String.format(
                                    "Retrying %s: count %2d; wait: %3dms; msg: \"%s\"; exception:"
                                            + " (%s)",
                                    Thread.currentThread().getName(),
                                    count,
                                    retryWaitMs,

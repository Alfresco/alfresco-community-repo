...
public interface PreferenceService {
     ...
     *
     * <p>If no filter if provided all preferences are returned.
     *
     * <p>If a filter is provided it's used to filter the results. For example the filter
    ...
    @Auditable(parameters = {"userName", "preferenceFilter"})
    PagingResults<Pair<String, Serializable>> getPagedPreferences(
            String userName, String preferenceFilter, PagingRequest pagingRequest);

    /**
     * Sets the preference values for a user.
     *
     * <p>Values provided overlay those already present.
     *
     * <p>Preference value names can be "namespaced" by using package notation. For example
     ...
     *
     * <p>If no filter if present then all preferences are cleared.
     *
     * @param userName the user name
     * @param preferenceFilter the preference filter
     */

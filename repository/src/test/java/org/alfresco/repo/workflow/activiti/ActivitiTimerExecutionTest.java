...
public class ActivitiTimerExecutionTest extends BaseSpringTest {
  ...
  public void testTimerExecutionAuthentication() throws Exception {
    try {
      WorkflowInstance taskAssigneeWorkflowInstance = transactionHelper.doInTransaction(
        new RetryingTransactionHelper.RetryingTransactionCallback<WorkflowInstance>() {
          public WorkflowInstance execute() throws Throwable {
            // Create test person
            personManager.createPerson(USER1);

            WorkflowDefinition definition = deployDefinition(
              "activiti/testTimerTransaction.bpmn20.xml"
            );
            ...
            workflowService.endTask(
              workflowService.getStartTask(path.getInstance().getId()).getId(),
              null
            );
      ...
      // Test assigned task
      WorkflowPath path = workflowService
        .getWorkflowPaths(taskAssigneeWorkflowInstance.getId())
        .get(0);
      ...
      assertEquals(
        "waitTask",
        tasks.get(0).getDefinition().getNode().getName()
      );
  ...
  public void testTimerExecutionTransactionRollback() throws Exception {
    try {
      WorkflowInstance workflowInstance = transactionHelper.doInTransaction(
        new RetryingTransactionHelper.RetryingTransactionCallback<WorkflowInstance>() {
          public WorkflowInstance execute() throws Throwable {
            // Create test person
      ...
      String processInstanceId = BPMEngineRegistry.getLocalId(
        workflowInstance.getId()
      );

      // Check the timer, should have "error" set in it
      TimerEntity timer = (TimerEntity) activitiProcessEngine
      ...
      for (int i = 0; i < numberOfRetries; i++) {
        ...
        Thread.sleep(1000);
        timer =
          (TimerEntity) activitiProcessEngine
            .getManagementService()
            .createJobQuery()
            .timers()
            .processInstanceId(processInstanceId)
      ...
      assertNotNull(
        "Job should have exception message set",
        timer.getExceptionMessage()
      );

      // Check if exception is the one we deliberately caused
      String fullExceptionStacktrace = activitiProcessEngine
        .getManagementService()
        .getJobExceptionStacktrace(timer.getId());
      ...
      transactionHelper.doInTransaction(
        (RetryingTransactionHelper.RetryingTransactionCallback<Void>) () -> {
          AuthenticationUtil.runAsSystem(() -> {
            ...
            String homeFolderName = (String) nodeService.getProperty(
              userHomeNode,
      ...
      new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
        public Void execute() throws Throwable {
          try {
            ...
            ProcessDefinition procDef = activitiProcessEngine
            ...
            if (procDef != null) {
              activitiProcessEngine
                .getRepositoryService()
                .deleteDeployment(procDef.getDeploymentId(), true);
            }
          }
          return null;
        }
  ...
  public void before() throws Exception {
    ...
    this.activitiProcessEngine =
      (ProcessEngine) this.applicationContext.getBean("activitiProcessEngine");
  ...
  private void waitForTimersToBeExecuted(String workflowInstanceId)
    throws Exception {
    ...
    if (timers.size() > 0) {
      fail(
        "There are still timers available for the process: " + processInstanceId
      );
    }
  }

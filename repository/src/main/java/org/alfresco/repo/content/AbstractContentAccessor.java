...
public abstract class AbstractContentAccessor implements ContentAccessor {
    ...
    private static final Log loggerTrace =
            LogFactory.getLog(AbstractContentAccessor.class.getName() + ".trace");
    ...
    /**
     * Derived classes can call this method to ensure that necessary trace logging is performed when
     * the IO Channel is opened.
     */
    ...
    /**
     * Allow derived implementations to set the Content URL. This allows for implementations where
     * the URL is not known when the accessor is first constructed.
     *
     ...
     * @return Returns a new channel that functions just like the original, except that it issues
     *     callbacks to the listeners
     * @throws ContentIOException
     */
    protected FileChannel getCallbackFileChannel(
            FileChannel directChannel, List<ContentStreamListener> listeners)
            throws ContentIOException {
    ...
    /**
     * Advise that listens for the completion of specific methods on the {@link
     * java.nio.channels.ByteChannel} interface. This advise reacts only in the {@link
     * #afterReturning(Object, Method, Object[], Object) afterReturning} phase so that the
     * underlying stream methods have been successfully completed.
     *
     * @author Derek Hulley
     */
    ...
    /**
     * Wraps a <code>FileChannel</code> to provide callbacks to listeners when the channel is {@link
     * java.nio.channels.Channel#close() closed}.
     *
     * <p>This class is unfortunately necessary as the {@link FileChannel} doesn't have an single
     * interface defining its methods, making it difficult to put an advice around the methods that
     * require overriding.
     *
     * @author Derek Hulley
     */
        ...
        public CallbackFileChannel(FileChannel delegate, List<ContentStreamListener> listeners) {
            ...
            if (delegate instanceof CallbackFileChannel) {
                throw new IllegalArgumentException(
                        "FileChannel delegate may not be a CallbackFileChannel");
            }

...
public class RecordServiceImpl
  ...
  /** I18N */
  private static final String MSG_NODE_HAS_ASPECT =
    "rm.service.node-has-aspect";
  private static final String FINAL_VERSION = "rm.service.final-version";
  private static final String FINAL_DESCRIPTION =
    "rm.service.final-version-description";
  private static final String MSG_UNDECLARED_ONLY_RECORDS =
    "rm.action.undeclared-only-records";
  private static final String MSG_NO_DECLARE_MAND_PROP =
    "rm.action.no-declare-mand-prop";
   ...
   */
  public void setExtendedPermissionService(
    ExtendedPermissionService extendedPermissionService
  ) {
  ...
  public void setIncompleteNodeTagger(
    IncompleteNodeTagger incompleteNodeTagger
  ) {
  ...
  public void init() {
    // bind policies
    beforeFileRecord =
      policyComponent.registerClassPolicy(BeforeFileRecord.class);
    ...
    onRecordRejectionDelegate =
      policyComponent.registerClassPolicy(OnRecordRejection.class);
  }
        ...
        public Void doWork() {
          try {
            ...
            if (
              ...
              if (bNew) {
                Set<NodeRef> newRecords = transactionalResourceHelper.getSet(
                  KEY_NEW_RECORDS
                );
  ...
  public void disablePropertyEditableCheck(NodeRef nodeRef) {
    Set<NodeRef> ignoreOnUpdate = transactionalResourceHelper.getSet(
      KEY_IGNORE_ON_UPDATE
    );
    ...
    if (
      ...
      for (Map.Entry<QName, Serializable> entry : after.entrySet()) {
        ...
        if (beforeValue instanceof Date && afterValue instanceof Date) {
          ...
          // otherwise
          propertyUnchanged =
            EqualsHelper.nullSafeEquals(beforeValue, afterValue);
        }
    ...
    for (QName aspect : aspects) {
      ...
      if (def != null) {
        ...
        if (parent != null && ASPECT_RECORD_META_DATA.equals(parent)) {
          recordMetaDataAspects.put(
            aspect,
            Collections.singleton(TYPE_FILE_PLAN)
          );
        }
      }
    }
   ...
   */
  @Override
  public void registerRecordMetadataAspect(
    QName recordMetadataAspect,
    QName filePlanType
  ) {
  ...
  public Set<QName> getRecordMetadataAspects(QName filePlanType) {
    ...
    for (Entry<QName, Set<QName>> entry : getRecordMetadataAspectsMap()
      .entrySet()) {
  ...
  public void createRecord(
    ...
    final NodeRef newRecordContainer = recordCreationSanityCheckOnDestinationNode(
      destinationNodeRef,
      filePlan
    );
        ...
        public Void doWork() {
          if (!nodeService.hasAspect(nodeRef, ASPECT_RECORD)) {
            ...
            try {
              ...
              Pair<Set<String>, Set<String>> readersAndWriters = extendedPermissionService.getReadersAndWriters(
                nodeRef
              );
  ...
  private NodeRef recordCreationSanityCheckOnFilePlan(NodeRef filePlan) {
    ...
    if (filePlan == null) {
      ...
      if (result == null) {
        String msg =
          "Cannot create record, because the default file plan cannot be determined. Make sure at least one file plan has been created.";
        LOGGER.debug(msg);
      ...
      if (!filePlanService.isFilePlan(filePlan)) {
        String msg =
          "Cannot create record, because the provided file plan node reference is not a file plan.";
        LOGGER.debug(msg);
        throw new RecordCreationException(msg);
      }
  ...
  private NodeRef recordCreationSanityCheckOnDestinationNode(
    ...
    final NodeRef checkedFilePlan = recordCreationSanityCheckOnFilePlan(
      filePlan
    );
    ...
    // if optional location supplied, check that it is a valid record folder, unfiled record container or folder
      ...
      if (
        !(
          nodeType.equals(RecordsManagementModel.TYPE_RECORD_FOLDER) ||
        ...
        throw new AlfrescoRuntimeException(
          "Unable to create record, because container is not a valid type for new record."
        );
      }
      ...
      if (
        extendedPermissionService.hasPermission(
          newRecordContainer,
      ...
      if (freezeService.isFrozen(newRecordContainer)) {
        throw new IntegrityException(
          I18NUtil.getMessage(
            "rm.service.add-children-to-frozen-record-folder"
          ),
          null
        );
      }
  ...
  private void recordCreationSanityCheckOnNode(NodeRef nodeRef) {
    ...
    if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_WORKING_COPY)) {
      String msg =
        "Can node create record, because " +
        nodeRef.toString() +
        " is a working copy.";
      LOGGER.debug(msg);
      throw new RecordCreationException(msg);
   ...
   */
  @Override
  public NodeRef createRecordFromCopy(
    final NodeRef filePlan,
    final NodeRef nodeRef
  ) {
    return authenticationUtil.runAsSystem(
      new RunAsWork<NodeRef>() {
        public NodeRef doWork() throws Exception {
          ...
          final NodeRef unfiledRecordFolder = filePlanService.getUnfiledContainer(
            filePlan
          );
          ...
          try {
            ...
            try {
              ...
              FileInfo recordInfo = fileFolderService.copy(
                nodeRef,
                unfiledRecordFolder,
                null
              );
            ...
            List<AssociationRef> recordAssocs = nodeService.getTargetAssocs(
              record,
              ContentModel.ASSOC_ORIGINAL
            );
            ...
            throw new AlfrescoRuntimeException(
              "Can't create recorded version, because copy fails.",
              e
            );
          }
  ...
  private NodeRef getLatestVersionRecord(NodeRef nodeRef) {
    ...
    if (versionHistory != null) {
      ...
      for (Version previousVersion : previousVersions) {
        ...
        final NodeRef previousRecord = recordableVersionService.getVersionRecord(
          previousVersion
        );
  ...
  public NodeRef createRecordFromContent(
    ...
    try {
      ...
      final NodeRef record = fileFolderService
        .create(destination, name, type)
        .getNodeRef();
        ...
        AuthenticationUtil.runAsSystem(
          new RunAsWork<Boolean>() {
            public Boolean doWork() throws Exception {
              return (
                null != nodeService.getProperty(nodeRef, PROP_DATE_FILED)
              );
            }
          }
        );
      ...
      new RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          ...
          try {
            ...
            final String documentOwner = (String) properties.get(
              PROP_RECORD_ORIGINATING_USER_ID
            );
            final String originalName = (String) properties.get(
              PROP_ORIGIONAL_NAME
            );
            final NodeRef originatingLocation = (NodeRef) properties.get(
              PROP_RECORD_ORIGINATING_LOCATION
            );
            ...
            if (originatingLocation != null) {
              ...
              final List<ChildAssociationRef> parentAssocs = nodeService.getParentAssocs(
                nodeRef
              );
              for (ChildAssociationRef childAssociationRef : parentAssocs) {
                if (
                  ...
                  (
                    ...
                    nodeService
                      .getType(childAssociationRef.getParentRef())
              ...
              final Map<QName, Serializable> aspectProperties = new HashMap<>(
                3
              );
        ...
        private void removeRmAspectsFrom(NodeRef nodeRef) {
          ...
          final List<String> rmURIs = asList(
            RM_URI,
            DOD_URI,
            RM_CUSTOM_URI,
            RMV_URI
          ...
          for (ChildAssociationRef renditionAssoc : renditionService.getRenditions(
            nodeRef
          )) {
  ...
  public boolean isPropertyEditable(NodeRef record, QName property) {
    ...
    if (debugEnabled) {
      ...
      for (Role role : roles) {
        LOGGER.debug(
          "     ... user has role " + role.getName() + " with capabilities "
        );
      ...
      LOGGER.debug(
        " ... user has the following set permissions on the file plan"
      );
      ...
      for (AccessPermission perm : perms) {
        if (
          (
            ...
            perm
              .getPermission()
              .contains(RMPermissionModel.EDIT_RECORD_METADATA)
          )
        ) {
          LOGGER.debug(
            "     ... " +
    ...
    if (result) {
      ...
      AccessStatus accessNonRecord = capabilityService.getCapabilityAccessState(
        record,
        RMPermissionModel.EDIT_NON_RECORD_METADATA
      );
      AccessStatus accessDeclaredRecord = capabilityService.getCapabilityAccessState(
        record,
        RMPermissionModel.EDIT_DECLARED_RECORD_METADATA
      );
      AccessStatus accessRecord = capabilityService.getCapabilityAccessState(
        record,
      ...
      if (
        AccessStatus.ALLOWED.equals(accessRecord) ||
        AccessStatus.ALLOWED.equals(accessDeclaredRecord)
      ) {
        LOGGER.debug(
          " ... user has edit record or declared metadata capability"
        );
      ...
      if (allowNonRecordEdit && allowRecordEdit) {
        ...
        if (!isRecordMetadata(filePlan, property)) {
          LOGGER.debug(
            " ... property is not considered record metadata so editable."
          );
          result = true;
        } else {
          LOGGER.debug(
            " ... property is considered record metadata so not editable."
          );
        }
      } else if (!allowNonRecordEdit && allowRecordEdit) {
        ...
        if (isRecordMetadata(filePlan, property)) {
          LOGGER.debug(
            " ... property is considered record metadata so editable."
          );
          result = true;
        } else {
          LOGGER.debug(
  ...
  private boolean isRecordMetadata(NodeRef filePlan, QName property) {
    ...
    if (
      parent != null && TYPE_NON_ELECTRONIC_DOCUMENT.equals(parent.getName())
    ) {
      result = false;
    } else {
      // check the URI's
      ...
      if (
        ...
        if (parent != null && parent.isAspect()) {
          result =
            getRecordMetadataAspects(filePlan).contains(parent.getName());
        }
      }
    }
  ...
  private boolean alwaysEditProperty(QName property) {
    return (
      getAlwaysEditURIs().contains(property.getNamespaceURI()) ||
  ...
  public void link(NodeRef record, NodeRef recordFolder) {
    ...
    if (isRecord(record) && isRecordFolder(recordFolder)) {
      ...
      String name = nodeService
        .getProperty(record, ContentModel.PROP_NAME)
        .toString();
  ...
  private void validateLinkConditions(NodeRef record, NodeRef recordFolder) {
    ...
    DispositionSchedule recordDispositionSchedule = dispositionService.getOriginDispositionSchedule(
      record
    );

    if (recordDispositionSchedule != null) {
      DispositionSchedule recordFolderDispositionSchedule = dispositionService.getDispositionSchedule(
        recordFolder
      );
      if (recordFolderDispositionSchedule != null) {
        if (
          recordDispositionSchedule.isRecordLevelDisposition() !=
          recordFolderDispositionSchedule.isRecordLevelDisposition()
        ) {
          // we can't link a record to an incompatible disposition schedule
          throw new RecordLinkRuntimeException(
            "Cannot link a record to a record folder with an incompatible retention schedule.  " +
  ...
  protected void invokeBeforeRecordDeclaration(NodeRef nodeRef) {
    ...
    BeforeRecordDeclaration policy = beforeRecordDeclarationDelegate.get(
      qnames
    );
  ...
  private void validateForCompletion(NodeRef nodeRef) {
    if (!nodeService.exists(nodeRef)) {
      LOGGER.warn(
        I18NUtil.getMessage(MSG_UNDECLARED_ONLY_RECORDS, nodeRef.toString())
      );
    ...
    if (checkMandatoryPropertiesEnabled) {
      Map<QName, Serializable> nodeRefProps = nodeService.getProperties(
        nodeRef
      );
      ...
      checkDefinitionMandatoryPropsSet(
        typeDef,
        nodeRefProps,
        missingProperties
      );
   ...
   */
  private String buildMissingPropertiesErrorString(
  ...
  private void checkDefinitionMandatoryPropsSet(
    ...
    for (PropertyDefinition propDef : classDef.getProperties().values()) {
      if (
        propDef.isMandatory() && nodeRefProps.get(propDef.getName()) == null
      ) {

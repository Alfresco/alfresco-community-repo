...
/**
 * A {@link VirtualProtocol} extension that uses a scripted processor virtual template in order to
 * process a so-called vanilla JSON static template definition on template execution.<br>
 * Vanilla references store have an extra {@link ResourceParameter} for the vanilla-JSON template.
 *
 * @author Bogdan Horje
 */
public class VanillaProtocol extends VirtualProtocol {
    ...
    public Reference newReference(
        ...
        return this.newReference(
                new ClasspathResource(vanillaProcessorClasspath),
                templatePath,
                actualNodeRef,
    ...
    public Reference newReference(
        ...
        return this.newReference(
                encoding, virtualTemplateResource, templatePath, actualResource, parameters);
    }

    public Resource getVanillaTemplateResource(Reference reference) {
        ResourceParameter vanillaTemplateParamter =
                (ResourceParameter) reference.getParameters().get(VANILLA_TEMPLATE_PARAM_INDEX);

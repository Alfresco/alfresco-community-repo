...
public class WorkflowDeployer extends AbstractLifecycleBean {
  ...
  public static final String CATEGORY_ALFRESCO_INTERNAL =
    "http://alfresco.org/workflows/internal";
  ...
  public static final String defaultSubtypeOfWorkflowDefinitionType =
    "subtypeOf('bpm:workflowDefinition')";
  ...
  public void setRepositoryWorkflowDefsLocations(
    RepositoryLocation repoWorkflowDefsLocation
  ) {
  ...
  public void init() {
    ...
    if (
      (!transactionService.getAllowWrite()) &&
      (
      ...
      if (logger.isWarnEnabled()) logger.warn(
        "Repository is in read-only mode; not deploying workflows."
      );
    ...
    try {
      ...
      if (
        ...
        AlfrescoTransactionSupport.bindListener(
          this.workflowDeployerTransactionListener
        );
      }
        ...
        for (Properties workflowDefinition : workflowDefinitions) {
          ...
          if (location == null || location.length() == 0) {
            throw new WorkflowException(
              "Workflow definition location must be provided"
            );
          }

          if (workflowAdminService.isEngineEnabled(engineId)) {
            Boolean redeploy = Boolean.valueOf(
              workflowDefinition.getProperty(REDEPLOY)
            );
            String mimetype = workflowDefinition.getProperty(MIMETYPE);

            // retrieve input stream on workflow definition
            ClassPathResource workflowResource = new ClassPathResource(
              location
            );
            ...
            if (
              !redeploy &&
              workflowService.isDefinitionDeployed(
                engineId,
                workflowResource.getInputStream(),
                mimetype
              )
            ) {
              if (logger.isDebugEnabled()) logger.debug(
                "Workflow deployer: Definition '" +
                location +
                "' already deployed"
              );
            } else {
  ...
  public void deploy(NodeRef nodeRef, boolean redeploy) {
    ...
    if (
      ...
      if (!isValidLocation(nodeRef)) {
        if (logger.isDebugEnabled()) {
          ...
          logger.debug(
            ...
            "' (" +
            nodePath +
            ") not deployed as it is not in the correct location."
          );
        }
        return;
      }
      ...
      if ((value != null) && (value.booleanValue() == true)) {
        String engineId = (String) nodeService.getProperty(
          nodeRef,
        ...
        if (workflowAdminService.isEngineEnabled(engineId)) {
          if (!redeploy && workflowService.isDefinitionDeployed(nodeRef)) {
            if (logger.isDebugEnabled()) logger.debug(
              "Workflow deployer: Definition '" + nodeRef + "' already deployed"
            );
          } else {
            // deploy / re-deploy
            WorkflowDeployment deployment = workflowService.deployDefinition(
            ...
            if (deployment != null) {
              ...
              if (deployment.getProblems().length > 0) {
                for (String problem : deployment.getProblems()) {
                  if (logger.isWarnEnabled()) logger.warn(problem);
                }
              }
  ...
  private boolean isValidLocation(NodeRef definitionNode) {
    ...
    for (ChildAssociationRef assoc : nodeService.getParentAssocs(
      definitionNode
    )) {
  ...
  private void logDeployment(Object location, WorkflowDeployment deployment) {
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(
        "Workflow deployer: Deployed process definition '" +
        title +
        "' (version " +
        version +
        ") from '" +
  ...
  public void undeploy(NodeRef nodeRef) {
    ...
    if (
      ...
      String defName = (String) nodeService.getProperty(
        nodeRef,
      ...
      if (defName != null) {
        ...
        for (WorkflowDefinition def : defs) {
          if (logger.isDebugEnabled()) logger.debug(
            "Undeploying workflow '" + defName + "' ..."
          );

          workflowService.undeployDefinition(def.getId());

          if (logger.isDebugEnabled()) logger.debug(
            "... undeployed '" + def.getId() + "' v" + def.getVersion()
          );
        }
      }
    } else {
            ...
            new RunAsWork<Object>() {
              public Object doWork() {
                ...
                if (logger.isTraceEnabled()) {
                  logger.trace(
                    "Workflow deployer afterCommit: Dictionary destroyed [" +

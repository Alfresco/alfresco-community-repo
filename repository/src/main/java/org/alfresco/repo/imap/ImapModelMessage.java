...
public class ImapModelMessage extends AbstractMimeMessage {
    ...
    private void buildRFC822Message() throws MessagingException {
        ...
        ContentReader reader =
                contentService.getReader(messageFileInfo.getNodeRef(), ContentModel.PROP_CONTENT);
        InputStream is = null;
        try {
            ...
            throw new MessagingException(
                    "The error occured during message creation from content stream.", e);
        } finally {
            if (is != null) {
                try {
                    is.close();

...
public abstract class FormProcessorTest extends TestCase {
  ...
  protected static final QName PROP_WITH_ = QName.createQName(
  ...
  protected static final QName ASSOC_WITH_ = QName.createQName(
    NamespaceService.BPM_MODEL_1_0_URI,
    "some_assoc"
  );
  protected static final NodeRef FAKE_NODE = new NodeRef(
    NamespaceService.BPM_MODEL_1_0_URI + "/FakeNode"
  );
  protected static final NodeRef FAKE_NODE2 = new NodeRef(
    NamespaceService.BPM_MODEL_1_0_URI + "/FakeNode2"
  );
  protected static final NodeRef FAKE_NODE3 = new NodeRef(
    NamespaceService.BPM_MODEL_1_0_URI + "/FakeNode3"
  );
  protected static final NodeRef PCKG_NODE = new NodeRef(
    NamespaceService.BPM_MODEL_1_0_URI + "/FakePackage"
  );
  ...
  protected DefaultFieldProcessor makeDefaultFieldProcessor(
  ...
  public void testPersistPropertyComment(String taskId) throws Exception {
    ...
    wikiHamlet.append(
      "Hamlet is the son of the King of Denmark. When Hamlet`s father dies, his uncle Claudius becomes king and marries Hamlet`s mother (Gertrude)."
    );
    wikiHamlet.append(
      "Hamlet`s father appears as a ghost and tells Hamlet that he was murdered by Claudius. Hamlet is not sure that the ghost is really his father."
    );
    wikiHamlet.append(
      "He gets some travelling actors to perform a play which shows the murder of a king in the same way Hamlet`s father said he was killed."
    );
    wikiHamlet.append(
      "When Claudius reacts badly to seeing this, Hamlet believes he is guilty."
    );
    wikiHamlet.append(
      "Hamlet tells his mother that he knows about the murder. While there he kills Polonius, who is the king`s advisor, because he thinks he is Claudius."
    );
    wikiHamlet.append(
      "Rosencrantz and Guildenstern were Hamlet`s childhood friends."
    );
    wikiHamlet.append(
      "Claudius sends Rosencrantz and Guildenstern with Hamlet to England to have Hamlet killed, but their ship is attacked by pirates who take Hamlet prisoner but then return him to Denmark."
    );
    wikiHamlet.append(
      "Rosencrantz and Guildenstern are taken to England where they die. Ophelia is Polonius` daughter . After her father, Polonius, is killed by Hamlet she goes mad."
    );
    wikiHamlet.append(
      "Then she falls into a river and drowns. Hamlet returns just as her funeral is happening. Laertes, her brother, decides to kill Hamlet in revenge."
    );
    wikiHamlet.append(
      "He challenges Hamlet to a sword fight, and puts poison on his own sword. Claudius makes some poisoned wine for Hamlet to drink in case that does not work."
    );
    wikiHamlet.append(
      "At first Hamlet wins the sword fight, and in the meantime his mother drinks the poisoned wine without knowing, and dies."
    );
    wikiHamlet.append(
      "On the other hand Laertes falsely pierces Hamlet with a poisoned blade, but then Hamlet stabs Laertes with the same sword. Laertes tells Hamlet about the plot and then dies."
    );
    wikiHamlet.append(
      "Hamlet kills Claudius with the poisoned sword. Horatio, Hamlet`s friend, tells everyone about the murder of the old king."
    );
    wikiHamlet.append(
    ...
    wikiHamlet.append(
      "Many people disagree about what Hamlet is really thinking. For many actors, playing Hamlet is one of the most important parts of their career."
    );
    ...
    if (propFile.exists()) {
      ...
      try {
        ...
        maxLength =
          properties.getProperty("system.workflow.comment.property.max.length");
      } finally {
        if (propStream != null) {
          propStream.close();
        }
    ...
    while (repeatTest) {
      ...
      try {
        ...
        assertTrue(
          "Caught Exception, value is " + value,
          value > maxLengthPropery
        );
      }
  ...
  protected void mockPackageItems(NodeRef... children) {
    ArrayList<ChildAssociationRef> results = new ArrayList<ChildAssociationRef>(
      children.length
    );
    for (NodeRef nodeRef : children) {
      ChildAssociationRef child = new ChildAssociationRef(
  ...
  protected Map<QName, AssociationDefinition> makeTaskAssociationDefs() {
    ...
    QName actorName = QName.createQName(
      NamespaceService.BPM_MODEL_1_0_URI,
      "Actor"
    );
    ...
    // Add Assigneee association
    MockClassAttributeDefinition actorsDef = MockClassAttributeDefinition.mockAssociationDefinition(
      ACTORS_NAME,
      actorName
    );
    ...
    MockClassAttributeDefinition with_ = MockClassAttributeDefinition.mockAssociationDefinition(
      ASSOC_WITH_,
      actorName
    );

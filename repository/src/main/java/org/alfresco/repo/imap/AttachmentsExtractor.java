...
public class AttachmentsExtractor {
  ...
  public void setAttachmentsFolder(
    RepositoryFolderConfigBean attachmentsFolder
  ) {
  ...
  public void setAttachmentsExtractorMode(String attachmentsExtractorMode) {
    this.attachmentsExtractorMode =
      AttachmentsExtractorMode.valueOf(attachmentsExtractorMode);
  }
      ...
      AuthenticationUtil.runAs(
        new AuthenticationUtil.RunAsWork<NodeRef>() {
          public NodeRef doWork() throws Exception {
            RetryingTransactionHelper helper = serviceRegistry
            ...
            RetryingTransactionCallback<NodeRef> getDescriptorCallback = new RetryingTransactionCallback<NodeRef>() {
              public NodeRef execute() {
                NodeRef attFolderRef = attachmentsFolder.getOrCreateFolderPath(
                ...
                if (
                  attachmentsExtractorMode != null &&
                  attachmentsExtractorMode == AttachmentsExtractorMode.COMMON
                ) {
                  serviceRegistry
  ...
  public void extractAttachments(
    ...
    switch (attachmentsExtractorMode) {
      case SAME:
        attachmentsFolderRef =
          nodeService.getPrimaryParent(messageRef).getParentRef();
        break;
      case COMMON:
      ...
      case SEPARATE:
      default:
        String messageName = (String) nodeService.getProperty(
          messageRef,
  ...
  private NodeRef createAttachmentFolder(
    NodeRef messageRef,
    String attachmentsFolderName
  ) {
    NodeRef attachmentsFolderRef = null;
    NodeRef parentFolder = nodeService
      .getPrimaryParent(messageRef)
      .getParentRef();
    attachmentsFolderRef =
      fileFolderService
  ...
  private void createAttachment(
    ...
    if (contentType.getBaseType().equalsIgnoreCase("application/ms-tnef")) {
      ...
      for (org.apache.poi.hmef.Attachment attachment : attachments) {
        ...
        NodeRef attachmentNode = fileFolderService.searchSimple(
          attachmentsFolderRef,
          subName
        );
      ...
      // not TNEF
      NodeRef attachmentFile = fileFolderService.searchSimple(
        attachmentsFolderRef,
        fileName
      );

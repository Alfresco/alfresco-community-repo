...
/**
 * Concrete email service implementation. This is responsible for routing the emails into the
 * server.
 *
 * @since 2.2
 */
public class EmailServiceImpl implements EmailService {
    private static final String ERR_INBOUND_EMAIL_DISABLED =
    ...
    private static final String ERR_UNKNOWN_SOURCE_ADDRESS =
            "email.server.err.unknown_source_address";
    private static final String ERR_USER_NOT_EMAIL_CONTRIBUTOR =
            "email.server.err.user_not_email_contributor";
    ...
    /**
     * The authority that needs to contain the users and groups who are allowed to contribute email.
     */
     ...
     *
     * @param delivery - who gets the message and who is it from (may be different from the contents
     *     of the message)
     * @param nodeRef Addressed node (target node).
     * @param message Email message
     * @throws EmailMessageException Any exception occured inside the method will be converted and
     *     thrown as <code>EmailMessageException</code>
     */
    private void processMessage(
            final EmailDelivery delivery, final NodeRef nodeRef, final EmailMessage message) {
        ...
        try {
            ...
            final RetryingTransactionCallback<String> getUsernameCallback =
                    new RetryingTransactionCallback<String>() {
                        ...
                        public String execute() throws Throwable {
                            ...
                            if (userName == null) {
                                if (unknownUser.isEmpty()) {
                                    ...
                                    throw new EmailMessageException(
                                            ERR_UNKNOWN_SOURCE_ADDRESS, message.getFrom());
                                } else {
                                    if (logger.isDebugEnabled()) {
                                        logger.debug(
                                                "unable to find user for from - return anonymous:"
                                                        + " ");
                                    }
                            ...
                            if (userName == null || !isEmailContributeUser(userName)) {
                                throw new EmailMessageException(
                                        ERR_USER_NOT_EMAIL_CONTRIBUTOR, userName);
                            }

                            return userName;
                        }
                    };
            RunAsWork<String> getUsernameRunAsWork =
                    new RunAsWork<String>() {
                        public String doWork() throws Exception {
                            return retryingTransactionHelper.doInTransaction(
                                    getUsernameCallback, false);
                        }
                    };
            ...
            if (delivery.getAuth() != null) {
                ...
                username =
                        AuthenticationUtil.runAs(
                                getUsernameRunAsWork, AuthenticationUtil.SYSTEM_USER_NAME);
            }

            // Process the message using the username's account
            final RetryingTransactionCallback<Object> processMessageCallback =
            ...
            RunAsWork<Object> processMessageRunAsWork =
                    new RunAsWork<Object>() {
                        public Object doWork() throws Exception {
                            return retryingTransactionHelper.doInTransaction(
                                    processMessageCallback, false);
                        }
                    };
            ...
            throw new EmailMessageException(
                    ERR_ACCESS_DENIED, delivery.getFrom(), delivery.getRecipient());
     ...
     * @throws EmailMessageException is thrown if the target node couldn't be determined by some
     *     reasons.
     */
    private NodeRef getTargetNode(String recipient) {
        ...
        NodeRef ref =
                (NodeRef)
                        getAttributeService()
                                .getAttribute(
                                        AliasableAspect.ALIASABLE_ATTRIBUTE_KEY_1,
    ...
    private String getUsername(String from) {
        ...
        ResultSet resultSet =
                searchService.query(storeRef, SearchService.LANGUAGE_FTS_ALFRESCO, query);
        try {
            ...
            if (resultSet.length() > 1) {
                if (logger.isWarnEnabled()) {
                    logger.warn(
                            "found more as one result for email '"
                                    + from
     ...
     * @exception EmailMessageException Exception will be thrown if the <b>EMAIL_CONTRIBUTORS</b>
     *     group isn't found
     */
        ...
        return this.authorityService
                ...
                .contains(
                        authorityService.getName(
                                AuthorityType.GROUP, getEmailContributorsAuthority()));
    }

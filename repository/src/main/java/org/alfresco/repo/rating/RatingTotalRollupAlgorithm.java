...
/**
 * An implementation of {@link AbstractRatingRollupAlgorithm} which calculates the total (sum) of
 * all ratings in a given scheme.
 *
...
public class RatingTotalRollupAlgorithm extends AbstractRatingRollupAlgorithm {
    ...
    public Float recalculate(NodeRef ratedNode) {
        ...
        if (nodeService.hasAspect(ratedNode, ContentModel.ASPECT_RATEABLE)) {
            List<ChildAssociationRef> ratingsNodes =
                    ratingServiceImpl.getRatingNodeChildren(ratedNode, ratingSchemeName, null);
            // Filter by scheme

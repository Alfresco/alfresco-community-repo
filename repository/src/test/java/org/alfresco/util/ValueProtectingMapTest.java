...
public class ValueProtectingMapTest extends TestCase {
    ...
    private void checkMaps(boolean expectMapClone) {
        ...
        if (expectMapClone) {
            ...
            assertEquals(
                    "Modified the backing directly but value is not visible: ",
                    map.get("ONE"),
                    "ONE");
            map.put("TWO", "TWO");
    ...
    /**
     * No matter how many times we wrap instances in instances, the backing map must remain the
     * same.
     */
    public void testMapWrapping() {
        ValueProtectingMap<String, Serializable> mapTwo =
                new ValueProtectingMap<String, Serializable>(map);
        assertTrue(
                "Backing map must be shared: ", mapTwo.getProtectedMap() == map.getProtectedMap());
        ValueProtectingMap<String, Serializable> mapThree =
                new ValueProtectingMap<String, Serializable>(map);
        assertTrue(
                "Backing map must be shared: ",
                mapThree.getProtectedMap() == map.getProtectedMap());
    }
    ...
    /**
     * Ensures that single, immutable values are given out as-is and without affecting the backing
     * storage
     */
    ...
    /**
     * Ensures that single, immutable values are cloned before being given out without affecting the
     * backing storage
     */
    ...
    public void testSerializability() throws Exception {
        ...
        ValueProtectingMap<String, Serializable> reloadedMap =
                (ValueProtectingMap<String, Serializable>) ois.readObject();
        ois.close();

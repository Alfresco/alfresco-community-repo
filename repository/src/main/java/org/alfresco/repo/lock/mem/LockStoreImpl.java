...
public class LockStoreImpl
  ...
  public LockStoreImpl() {
    super(
      createMap(LockServiceImpl.MAX_EPHEMERAL_LOCK_SECONDS, TimeUnit.SECONDS)
    );
  }
  ...
  private static ConcurrentMap<NodeRef, LockState> createMap(
    long expiry,
    TimeUnit timeUnit
  ) {
    Cache<NodeRef, LockState> cache = CacheBuilder
      .newBuilder()
      .concurrencyLevel(32)

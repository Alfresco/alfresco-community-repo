...
public class PageCollatorTest extends TestCase {
  ...
  private int[] createMergedPage(
    ...
    int[] s1Primitive = (s1 == null || s1.isEmpty())
      ? new int[] {}
  ...
  class ArrayPageSource<R> implements PagingResultsSource<R> {
    ...
    @Override
    public PagingResults<R> retrieve(PagingRequest pr)
      ...
      if (boundsError && (skip >= array.length)) {
        throw new InvalidPageBounds(
          "Out of bounds " + skip + ">=" + array.length
        );
      }
  ...
  public void assertCollate(
    ...
    PagingResults<Integer> actualResults = new PageCollator<Integer>()
      .collate(
        results,

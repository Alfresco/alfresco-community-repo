...
public class RuleServiceImplTest extends BaseRuleTest {
  ...
  public void testGetRules() {
    ...
    assertNotNull(
      this.nodeService.getProperty(rule.getNodeRef(), ContentModel.PROP_CREATED)
    );
    assertNotNull(
      this.nodeService.getProperty(rule.getNodeRef(), ContentModel.PROP_CREATOR)
    );
  ...
  public void testGetRulesOrder() {
    ...
    NodeRef ruleFolder =
      ((RuntimeRuleService) ruleService).getSavedRuleFolderAssoc(nodeRef)
        .getChildRef();
    if (ruleFolder != null) {
      // Get the rules for this node
      List<ChildAssociationRef> ruleChildAssocRefs = nodeService.getChildAssocs(
        ruleFolder,
      ...
      for (ChildAssociationRef ruleChildAssocRef : ruleChildAssocRefs) {
        System.out.println(
          " - Assoc index = " +
          ruleChildAssocRef.getNthSibling() +
          ", name = " +
          nodeService.getProperty(
            ruleChildAssocRef.getChildRef(),
    ...
    action.setParameterValue(
      ReorderRules.PARAM_RULES,
      (Serializable) ruleNodeRefs
    );
  ...
  public void testRuleInheritance() {
    ...
    List<? extends Rule> myRules =
      this.ruleService.getRules(childWithRules, false);
    assertNotNull(myRules);
    assertEquals(2, myRules.size());
    assertTrue(myRules.contains(rule3));
    assertTrue(myRules.contains(rule4));

    List<? extends Rule> allRules2 =
      this.ruleService.getRules(nonActionableChild, true);
    assertNotNull(allRules2);
    assertEquals(1, allRules2.size());
    assertTrue(allRules2.contains(rule2));

    List<? extends Rule> myRules2 =
      this.ruleService.getRules(nonActionableChild, false);
    assertNotNull(myRules2);
    assertEquals(0, myRules2.size());

    List<? extends Rule> allRules3 =
      this.ruleService.getRules(rootWithRules, true);
    ...
    List<? extends Rule> myRules3 =
      this.ruleService.getRules(rootWithRules, false);
    assertNotNull(myRules3);
    assertEquals(2, myRules3.size());
    assertTrue(myRules3.contains(rule1));
    assertTrue(myRules3.contains(rule2));

    List<? extends Rule> allRules4 =
      this.ruleService.getRules(rootWithRules2, true);
    ...
    List<? extends Rule> myRules4 =
      this.ruleService.getRules(rootWithRules2, false);
    ...
    List<? extends Rule> allRules5 =
      this.ruleService.getRules(childWithRules, true);
  ...
  public void testCyclicAsyncRules() throws Exception {
    ...
    actionProps.put(
      ImageTransformActionExecuter.PARAM_MIME_TYPE,
    ...
    Action action =
      this.actionService.createAction(ImageTransformActionExecuter.NAME);
      ...
      .doInTransaction(
        new RetryingTransactionCallback<Object>() {
          public Object execute() {
            ...
            NodeRef companyHomeNodeRef = searchService
              .selectNodes(
                storeRootNodeRef,
                "/app:company_home",
                null,
            ...
            QName testFolderName = QName.createQName(
              NamespaceService.CONTENT_MODEL_1_0_URI,
              "ISSUE_ETWOTWO_738_" + System.currentTimeMillis()
            );
            ...
            // get script nodeRef
            NodeRef scriptRef = searchService
              .selectNodes(
                storeRootNodeRef,
                "/app:company_home/app:dictionary/app:scripts/cm:backup.js.sample",
                null,
                namespaceService,
                false
              )
  ...
  public void testPermissionsForPropagatedRules_ALF_8408() throws Exception {
    ...
    NodeRef parentNodeRef =
      this.nodeService.createNode(
          rootNodeRef,
          ContentModel.ASSOC_CHILDREN,
          QName.createQName("parentnode" + GUID.generate()),
          ContentModel.TYPE_FOLDER
        )
        .getChildRef();
    ...
    NodeRef childNodeRef =
      this.nodeService.createNode(
          parentNodeRef,
          ContentModel.ASSOC_CHILDREN,
          QName.createQName("childnode" + GUID.generate()),
          ContentModel.TYPE_FOLDER
        )
        .getChildRef();
    ...
    List<Rule> rules =
      this.ruleService.getRules(
          childNodeRef,
          true,
          testRule.getRuleTypes().get(0)
        );
    assertNotNull("No rules found", rules);
    assertTrue(
      "Created rule is not found",
      new HashSet<Rule>(rules).contains(testRule)
    );
    ...
    ((RuntimeRuleService) ruleService).executeRule(
        testRule,
        actionedUponNodeRef,
        null
      );
    assertTrue(
      "Rule was not executed",
      this.nodeService.hasAspect(
          actionedUponNodeRef,
          ContentModel.ASPECT_VERSIONABLE
        )
    ...
    ((RuntimeRuleService) ruleService).addRulePendingExecution(
        parentNodeRef,
        actionedUponNodeRef,
        testRule
      );
  ...
  public void testOutboundRuleTriggeredAfterRename1() throws Exception {
    ...
    action.setParameterValue(
      CopyActionExecuter.PARAM_DESTINATION_FOLDER,
      folder2NodeRef
    );
    testRule.setAction(action);
    ...
    QName actionedQName = QName.createQName(
      "actioneduponnode" + GUID.generate()
    );
    // New node
    NodeRef actionedUponNodeRef =
      this.nodeService.createNode(
    ...
    assertEquals(
      "The rule should not be triggered and no document should be present.",
      0,
      childAssoc.size()
    );
    ...
    assertEquals(
      "The rule should not be triggered and the document should be in folder1.",
      1,
      childAssoc.size()
    );
  ...
  public void testOutboundRuleTriggeredAfterRename2() throws Exception {
    ...
    assertEquals(
      ...
      newName,
      nodeService.getProperty(actionedUponNodeRef, ContentModel.PROP_NAME)
    );
  ...
  public void testOutBoundRuleTriggerForPendingDelete() throws Exception {
    ...
    try {
      ...
      fail(
        "The nodes should be deleted without errors, but exception was thrown: " +
        e
      );
    }
    txn.commit();
    ...
    assertFalse(
      ...
      nodeService.exists(parentFolderNodeRef)
    );
    txn.commit();
  ...
  public void testRuleTriggerWithTemporaryFiles() throws Exception {
    ...
    assertTrue(
      "Created rule is not found",
      new HashSet<>(rules).contains(testRule)
    );
    ...
    assertFalse(
      "Node with Temporary aspect has versionable aspect: ",
      this.nodeService.hasAspect(
          actionedUponNodeRef,
          ContentModel.ASPECT_VERSIONABLE
        )
  ...
  public void testRemoveAllRulesForInboundAndOutbound() {
    ...
    NodeRef scriptRef = searchService
      .selectNodes(
        storeRootNodeRef,
        "/app:company_home/app:dictionary/app:scripts/cm:" + scriptName,
        null,
        namespaceService,
        false
      )
  ...
  private void createNothingToDoScript(String scriptName) {
    ...
    NodeRef scriptFolderRef = searchService
      .selectNodes(
        storeRootNodeRef,
  ...
  private Action createScriptAction() {
    ...
    NodeRef scriptRef = searchService
      .selectNodes(
        storeRootNodeRef,
        "/app:company_home/app:dictionary/app:scripts/cm:nothingToDo.js",
        null,
        namespaceService,
        false
      )

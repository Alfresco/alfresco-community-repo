...
public class ImapServiceImplTest extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    authenticationService.createAuthentication(
      anotherUserName,
      anotherUserName.toCharArray()
    );

    user =
      new AlfrescoImapUser(
        anotherUserName + "@alfresco.com",
        anotherUserName,
        anotherUserName
      );
    ...
    ChildApplicationContextFactory imap = (ChildApplicationContextFactory) ctx.getBean(
      "imap"
    );
    ...
    imapHome.setFolderPath(
      NamespaceService.CONTENT_MODEL_PREFIX + ":" + TEST_IMAP_FOLDER_NAME
    );
    ...
    NodeRef storeRootNodeRef = nodeService.getRootNode(
  ...
  private boolean checkSubscribedMailbox(
    AlfrescoImapUser user,
    String mailboxName
  ) {
    List<AlfrescoImapFolder> aifs = imapService.listMailboxes(
      user,
      mailboxName,
      true
    );
  ...
  public void testListMailbox() throws Exception {
    ...
    List<AlfrescoImapFolder> mf = imapService.listMailboxes(
      user,
      MAILBOX_PATTERN,
      false
    );
     ...
     */
    List<AlfrescoImapFolder> aif = imapService.listMailboxes(
      user,
      MAILBOX_PATTERN,
      true
    );
    ...
    List<AlfrescoImapFolder> aif2 = imapService.listMailboxes(
      user,
      MAILBOX_PATTERN,
      true
    );
  ...
  public void testExcludeFoldersByComponentIt() {
    NodeRef imapFolderA = imapService
      .getOrCreateMailbox(user, MAILBOX_NAME_A, false, true)
      .getFolderInfo()
      .getNodeRef();
    NodeRef imapFolderB = imapService
      .getOrCreateMailbox(user, MAILBOX_NAME_B, false, true)
      .getFolderInfo()
      .getNodeRef();
    NodeRef imapFolderC = imapService
      .getOrCreateMailbox(user, "mailboxCalendarFolder", false, true)
      .getFolderInfo()
      .getNodeRef();
    NodeRef imapFolderD = imapService
      .getOrCreateMailbox(user, "mailboxDataListsFolder", false, true)
      .getFolderInfo()
      .getNodeRef();
    NodeRef imapFolderE = imapService
      .getOrCreateMailbox(user, "mailboxDocumentLibraryFolder", false, true)
      .getFolderInfo()
      .getNodeRef();
    ...
    mf = imapService.listMailboxes(user, MAILBOX_PATTERN, false);
    assertEquals(
      "Imap folders with component IDs 'calendar' or 'dataLists' were not excluded.",
      3,
      mf.size()
    );
  }
  ...
  public void testListSubscribedMailbox() throws Exception {
    ...
    assertTrue(
      "Can't subscribe mailbox A",
      checkSubscribedMailbox(user, MAILBOX_NAME_A)
    );
    assertTrue(
      "Can't subscribe mailbox B",
      checkSubscribedMailbox(user, MAILBOX_NAME_B)
    );
  }
  ...
  private void setFlags(FileInfo messageFileInfo) throws Exception {
    ...
    assertTrue(
      "Can't set SEEN flag",
      props.containsKey(ImapModel.PROP_FLAG_SEEN)
    );
    assertTrue(
      "Can't set FLAGGED flag",
      props.containsKey(ImapModel.PROP_FLAG_FLAGGED)
    );
    assertTrue(
      "Can't set ANSWERED flag",
      props.containsKey(ImapModel.PROP_FLAG_ANSWERED)
    );
    assertTrue(
      "Can't set DELETED flag",
      props.containsKey(ImapModel.PROP_FLAG_DELETED)
    );
  }

  public void testSetFlags() throws Exception {
    NavigableMap<Long, FileInfo> fis = imapService.getFolderStatus(
  ...
  public void testSetFlag() throws Exception {
    ...
    if (fis != null && fis.size() > 0) {
      ...
      Serializable prop = nodeService.getProperty(
        messageFileInfo.getNodeRef(),
        ImapModel.PROP_FLAG_RECENT
      );
  ...
  public void testRenameAccentedMailbox() throws Exception {
    ...
    imapService.getOrCreateMailbox(user, MAILBOX_ACCENTED_NAME_A, false, true);
    imapService.renameMailbox(
      user,
      MAILBOX_ACCENTED_NAME_A,
      MAILBOX_ACCENTED_NAME_B
    );
    assertFalse(
      "Can't rename mailbox",
      checkMailbox(user, MAILBOX_ACCENTED_NAME_A)
    );
    assertTrue(
      "Can't rename mailbox",
      checkMailbox(user, MAILBOX_ACCENTED_NAME_B)
    );
  ...
  public void testContentRecovery() throws Exception {
    ...
    if (nodeService.exists(archivedNodeRef)) {
      NodeRef restoredNode = nodeService.restoreNode(
        archivedNodeRef,
        testImapFolderNodeRef,
        null,
        null
      ...
      if (
        ...
        value =
          !(Boolean) props.get(ImapModel.PROP_FLAG_DELETED) &&
          !(Boolean) props.get(ImapModel.PROP_FLAG_SEEN);
      }
    }
  ...
  public void testAttachmentExtraction() throws Exception {
    ...
    ClassPathResource fileResource = new ClassPathResource(
      "imap/test-tnef-message.eml"
    );
    assertNotNull(
      "unable to find test resource test-tnef-message.eml",
      fileResource
    );
    ...
    FileInfo f1 = fileFolderService.create(
      companyHomeNodeRef,
      "ImapServiceImplTest",
      ContentModel.TYPE_FOLDER
    );
    FileInfo f2 = fileFolderService.create(
      f1.getNodeRef(),
      "test-tnef-message.eml",
      ContentModel.TYPE_CONTENT
    );
  ...
  public void testMailboxRenamingInTheRootForMNT9055() throws Exception {
    reauthenticate(USER_NAME, USER_PASSWORD);
    AlfrescoImapUser poweredUser = new AlfrescoImapUser(
      (USER_NAME + "@alfresco.com"),
      USER_NAME,
      USER_PASSWORD
    );
    ...
    FileInfo targetNode = fileFolderService.create(
      root,
      targetNodeName,
      ContentModel.TYPE_FOLDER
    );
    ...
    String renamedNodePath =
      IMAP_ROOT + AlfrescoImapConst.HIERARCHY_DELIMITER + renamedNodeName;

    assertMailboxRenaming(
      poweredUser,
      root,
      (IMAP_ROOT + AlfrescoImapConst.HIERARCHY_DELIMITER + targetNodeName),
      java.util.Collections.singletonList(renamedNodeName),
      renamedNodePath,
      targetNode
    );

    AlfrescoImapFolder actualMailbox = imapService.getOrCreateMailbox(
      poweredUser,
      renamedNodePath,
      true,
      false
    ...
    List<ChildAssociationRef> parentAssocs = nodeService.getParentAssocs(
      actualMailbox.getFolderInfo().getNodeRef()
    );
  ...
  public void testMailboxRenamingInTheUserImapHomeForMNT9055()
    throws Exception {
    reauthenticate(USER_NAME, USER_PASSWORD);
    AlfrescoImapUser poweredUser = new AlfrescoImapUser(
      (USER_NAME + "@alfresco.com"),
    ...
    AlfrescoImapFolder mailbox = imapService.getOrCreateMailbox(
      poweredUser,
      targetNodeName,
      false,
      true
    ...
    List<String> pathBeforeRenaming = fileFolderService.getNameOnlyPath(
      root,
      mailbox.getFolderInfo().getNodeRef()
    );
    ...
    assertMailboxRenaming(
      ...
      root,
      targetNodeName,
      Collections.singletonList(renamedNodeName),
      renamedNodeName,
      null
    );

    mailbox =
      imapService.getOrCreateMailbox(poweredUser, renamedNodeName, true, false);
    assertMailboxNotNull(mailbox);

    List<String> pathAfterRenaming = fileFolderService.getNameOnlyPath(
      root,
      mailbox.getFolderInfo().getNodeRef()
    );
  ...
  public void testMailboxRenamingInDeepHierarchyForMNT9055() throws Exception {
    ...
    FileInfo renamedNode = fileFolderService.resolveNamePath(
      root,
      pathAfterRenaming
    );
  ...
  public void testMiddleMailboxRenamingInDeepHierarchyForMNT9055()
    throws Exception {
    reauthenticate(USER_NAME, USER_PASSWORD);
    AlfrescoImapUser poweredUser = new AlfrescoImapUser(
      (USER_NAME + "@alfresco.com"),
    ...
    List<String> renamedFullLeftPathList = new LinkedList<String>(
      fullLeftPathList
    );
    ...
    String fullRenamedPath = new StringBuilder(path)
      .append(renamedNodeName)
      .append(AlfrescoImapConst.HIERARCHY_DELIMITER)
      .append(fullPath)
      .append(targetNodeName)
      .toString();
    mailbox =
      imapService.getOrCreateMailbox(poweredUser, fullRenamedPath, true, false);
    assertMailboxNotNull(mailbox);

    assertEquals(targetNode.getNodeRef(), mailbox.getFolderInfo().getNodeRef());

    pathAfterRenaming =
      fileFolderService.getNameOnlyPath(
        middleNode.getNodeRef(),
  ...
  public void testMailboxMoving() throws Exception {
    ...
    StringBuilder fullPath = new StringBuilder();
    List<String> fullPathList = new LinkedList<String>(
      Arrays.asList(new String[] { TEST_IMAP_FOLDER_NAME, USER_NAME })
    );
    String targetNodeName = new StringBuilder(IMAP_ROOT)
      .append(AlfrescoImapConst.HIERARCHY_DELIMITER)
      .append(TEST_IMAP_FOLDER_NAME)
      .append(AlfrescoImapConst.HIERARCHY_DELIMITER)
      .append(USER_NAME)
      .toString();
    ...
    for (int i = 0; i < 4; i++) {
      ...
      AlfrescoImapFolder mailbox = imapService.getOrCreateMailbox(
        poweredUser,
        (imapUserHomePath + targetNodeName),
        false,
        true
      );
    ...
    String renamedNodeName =
      IMAP_ROOT + AlfrescoImapConst.HIERARCHY_DELIMITER + newName;
    ...
    assertMailboxInUserImapHomeDirectory(
      root,
      mailbox.getFolderInfo().getNodeRef()
    );
  ...
  public void testMoveViaDeleteAndAppend() throws Exception {
    ...
    String destinationPath =
      IMAP_ROOT + AlfrescoImapConst.HIERARCHY_DELIMITER + destinationName;
    String nodeContent = "test content";
    NodeRef root = findCompanyHomeNodeRef();
    AuthenticationUtil.setAdminUserAsFullyAuthenticatedUser();

    // Create node and destination folder
    FileInfo origFile = fileFolderService.create(
      root,
      fileName,
      ContentModel.TYPE_CONTENT
    );
    ...
    FileInfo destinationNode = fileFolderService.create(
      root,
      destinationName,
      ContentModel.TYPE_FOLDER
    );
    ...
    AlfrescoImapFolder destinationMailbox = imapService.getOrCreateMailbox(
      poweredUser,
      destinationPath,
      true,
      false
    ...
    assertTrue(
      !nodeService.exists(origFile.getNodeRef()) ||
      imapService.getFlags(origFile).contains(Flags.Flag.DELETED)
    );

    // new file should be in destination
    assertEquals(
      "There should be only one node in the destination folder",
      1,
      nodeService.getChildAssocs(destinationNode.getNodeRef()).size()
    );
    NodeRef newNodeRef = nodeService
      .getChildAssocs(destinationNode.getNodeRef())
      .get(0)
      .getChildRef();
  ...
  public void testMoveViaAppendAndDelete() throws Exception {
    ...
    origMessage
      .getMimeMessage()
      .addHeader(
        AlfrescoImapConst.X_ALF_NODEREF_ID,
        origFile.getNodeRef().getId()
      );
    origMessage.getMimeMessage().saveChanges();
    ...
    long uuid = destinationMailbox.appendMessage(
      origMessage.getMimeMessage(),
      flags,
      null
    );
    ...
    FileInfo copiedNode = fileFolderService.getFileInfo(
      nodeService.getNodeRef(uuid)
    );
    ...
    NodeRef copiedParentNodeRef = nodeService
      .getPrimaryParent(copiedNode.getNodeRef())
      .getParentRef();
    assertEquals(
      "The parent should change to destination.",
      destinationNode.getNodeRef(),
      copiedParentNodeRef
    );
    assertEquals(
      "There should be only one node in the destination folder",
    ...
    assertTrue(
      "New node should have original node aspects",
      nodeService.hasAspect(
        copiedNode.getNodeRef(),
        ContentModel.ASPECT_TAGGABLE
      )
    );
  }
   ...
   */
  private void assertMailboxRenaming(
    AlfrescoImapUser user,
    NodeRef root,
    String targetNodePath,
    List<String> renamedNodeName,
    String renamedNodePath,
    FileInfo targetNode
  ) throws FileNotFoundException {
    AlfrescoImapFolder mailbox = imapService.getOrCreateMailbox(
    ...
    assertNotNull(
      (
        "Just created mailbox can't be received by full path via the ImapService. Path: '" +
        targetNodePath +
        "'"
      ),
      mailbox
    ...
    if (null != targetNode) {
      FileInfo actualFileInfo = fileFolderService.resolveNamePath(
        root,
        renamedNodeName
      );
      ...
      mailbox =
        imapService.getOrCreateMailbox(user, renamedNodePath, true, false);
      assertNotNull(mailbox);
      actualNode = mailbox.getFolderInfo().getNodeRef();
    }

    assertNotNull(
      ("Can't receive renamed node by full path: '" + renamedNodePath + "'"),
      actualNode
    );
   ...
   * @throws FileNotFoundException
   */
  private void assertMailboxInUserImapHomeDirectory(
    NodeRef root,
    NodeRef actualNode
  ) throws FileNotFoundException {
   ...
   */
  private void assertPathHierarchy(
    List<String> pathBeforeRenaming,
    List<String> pathAfterRenaming
  ) {
  ...
  public void testSearchTerms() throws AddressException {
    List<AlfrescoImapFolder> mf = imapService.listMailboxes(
      user,
      IMAP_ROOT + "/" + TEST_IMAP_FOLDER_NAME + "/_*",
      false
    );
    ...
    assertEquals(
      "Size of mails isn't correct. Search by sent date",
      3,
      res.size()
    );
    ...
    extractSearchTermResultsToList(mf, st, res);
    assertEquals(
      "Size of mails isn't correct. Search by subject",
      1,
      res.size()
    );
    ...
    extractSearchTermResultsToList(mf, st, res);
    assertEquals(
      "Size of mails isn't correct. Search by \"From\" term",
      1,
      res.size()
    );
    ...
    extractSearchTermResultsToList(mf, st, res);
    assertEquals(
      "Size of mails isn't correct. Search by \"From\" and \"Subject\" terms",
      1,
      res.size()
    );
  }

  private void extractSearchTermResultsToList(
    List<AlfrescoImapFolder> folders,
    SearchTerm searchTerm,
    ArrayList<Long> resList
  ) {

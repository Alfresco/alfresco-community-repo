...
public class FolderTypeDefintionWrapper extends ShadowTypeDefinitionWrapper {
  ...
  public FolderTypeDefintionWrapper(
    ...
    if (BaseTypeId.CMIS_FOLDER.value().equals(typeId)) {
      ...
      if (cmisMapping.isValidCmisFolder(parentQName)) {
        typeDef.setParentTypeId(
          cmisMapping.getCmisTypeId(BaseTypeId.CMIS_FOLDER, parentQName)
        );
      }

      if (
        alfrescoName.equals(ContentModel.TYPE_SYSTEM_FOLDER) ||
        cmisMapping
          .getDictionaryService()
          .isSubClass(alfrescoName, ContentModel.TYPE_SYSTEM_FOLDER)
      ) {
        isSystemFolder = true;
      }
    }

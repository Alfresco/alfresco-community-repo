...
public class DispositionScheduleInheritanceTest extends BaseRMTestCase {
    /**
     * Given a root record category with a retention schedule and another root record category When
     * moving the second record category into the first one (with the disposition) Then records
     * under the second record category inherit the retention schedule of the parent record category
     *
     * <p>relates to https://issues.alfresco.com/jira/browse/RM-7065
     */
        ...
        doBehaviourDrivenTest(
                new BehaviourDrivenTest() {
                    ...
                    public void given() {
                        ...
                        NodeRef subcategory1 =
                                filePlanService.createRecordCategory(category1, generate());
                    ...
                    public void then() throws Exception {
                        ...
                        assertTrue(
                                "Record "
                                        + record
                                        + " doesn't have the disposition lifecycle aspect.",
                                nodeService.hasAspect(record, ASPECT_DISPOSITION_LIFECYCLE));
                    }
                });
    }

    /**
     * Given a root record category A with a retention schedule set to cut off after 10 days and
     * another root record category B with a retention schedule set to cut off after 5 days
     * containing a subcategory When moving the subcategory into the first root category Then
     * records under the subcategory inherit the retention schedule of the parent record category
     * The cut off date is updated to the new one, since the initial date was before the new one
     *
     * <p>Please see https://alfresco.atlassian.net/browse/RM-7103
     */
        ...
        doBehaviourDrivenTest(
                new BehaviourDrivenTest() {
                    ...
                    public void given() {
                        ...
                        // create record level disposition schedule for category1
                        createDispositionScheduleCutOff(
                                category1, CutOffAction.NAME, CommonRMTestUtils.PERIOD_TEN_DAYS);
                        ...
                        createDispositionScheduleCutOff(
                                category2, CutOffAction.NAME, CommonRMTestUtils.PERIOD_FIVE_DAYS);
                    ...
                    public void then() throws Exception {
                        ...
                        DispositionAction dispositionActionAfterMove =
                                dispositionService.getNextDispositionAction(record);
                        ...
                        assertTrue(
                                dispositionActionAfterMove.getAsOfDate().after(asOfDateBeforeMove));
                        ...
                        assertNotNull(
                                nodeService.getProperty(record, PROP_RS_DISPOSITION_ACTION_AS_OF));
                        assertNull(
                                (List<String>)
                                        nodeService.getProperty(
                                                record, PROP_RS_DISPOSITION_EVENTS));
                        assertNotNull(
                                nodeService.getProperty(record, PROP_RS_DISPOITION_INSTRUCTIONS));
                        assertNotNull(
                                nodeService.getProperty(record, PROP_RS_DISPOITION_AUTHORITY));
                        assertTrue(
                                (Boolean)
                                        nodeService.getProperty(
                                                record, PROP_RS_HAS_DISPOITION_SCHEDULE));
                    }
                });
    }

    /**
     * Given a root record category A with a retention schedule set to retain and destroy after 1
     * day and another root record category B with a retention schedule set to cut off and destroy
     * after 1 day containing a subcategory When moving the subcategory into the first root category
     * Then records under the subcategory inherit the retention schedule of the parent record
     * category The events list contain the retain event step inherited from the new parent category
     *
     * <p>Please see https://alfresco.atlassian.net/browse/APPS-1004
     */
    ...
    /**
     * Given a root record category A with a retention schedule set to cut off on event 'case
     * closed' and another root record category B with a retention schedule set to cut off on event
     * 'separation' When moving the subcategory into the first root category Then records under the
     * subcategory inherit the retention schedule of the parent record category The events list
     * contain the case closed event step inherited from the new parent category
     *
     * <p>Please see https://alfresco.atlassian.net/browse/APPS-1005
     */
    ...
    private void createDispositionScheduleStep(
            DispositionSchedule ds, String action, String period) {

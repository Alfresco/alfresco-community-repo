    ...
    public void testFavouriteSites() throws Exception {
        ...
        try {
            ...
            sitesProxy.createFavouriteSite(
                    person1.getId(), new FavouriteSite(testSite.getSiteId()));
            fail();
        } catch (PublicApiException e) {
        ...
        // "-me" user
        {
            ...
            FavouriteSite fs = new FavouriteSite(testSite.getSiteId());
            FavouriteSite resp =
                    sitesProxy.createFavouriteSite(org.alfresco.rest.api.People.DEFAULT_USER, fs);
            fs.expected(resp);

            final List<FavouriteSite> expectedFavouriteSites =
        ...
        // GET favourite sites
        {
            ...
            try {
                ...
                publicApiClient.setRequestContext(
                        new RequestContext(network1.getId(), person2.getId()));
                ListResponse<FavouriteSite> response =
                        sitesProxy.getFavouriteSites(person1.getId(), createParams(paging, null));
        ...
        // user is a member of the site which he has favourited
        {
            ...
            Paging paging =
                    getPaging(
                            ...
                            expectedFavouriteSites.size());
            ListResponse<FavouriteSite> response =
                    sitesProxy.getFavouriteSites(person5.getId(), createParams(paging, null));
            checkList(expectedFavouriteSites, paging.getExpectedPaging(), response);
        }
        ...
        // remove
        {
            ...
            // known user
            {
                ...
                List<FavouriteSite> expectedFavouriteSites =
                        TenantUtil.runAsUserTenant(
                                new TenantRunAsWork<List<FavouriteSite>>() {
                                    @Override
                                    public List<FavouriteSite> doWork() throws Exception {
            ...
            // "-me-" user
            {
                ...
                publicApiClient.setRequestContext(
                        new RequestContext(network1.getId(), person3.getId()));
                ListResponse<FavouriteSite> response =
                        sitesProxy.getFavouriteSites(person3.getId(), createParams(paging, null));
                assertFalse(response.getList().contains(fs));
            }
        }
    }

 ...
 *
 * <p>A content store implementation that aggregates a set of stores. Content is not persisted by
 * this store, but rather it relies on any number of child {@link
 * org.alfresco.repo.content.ContentStore stores} to provide access to content readers and writers.
 *
 * <p>The order in which the stores appear in the list of stores participating is important. The
 * first store in the list is known as the <i>primary store</i>.
 *
 * <p>Content is written to the primary store only. The other stores are only used to retrieve
 * content and the primary store is not updated with the content.
 *
 * @author Derek Hulley
 * @author Mark Rogers
...
public class AggregatingContentStore extends AbstractContentStore {
    ...
    /**
     * Performs a delete on the local store and if outbound replication is on, propogates the delete
    ...
    public DirectAccessUrl requestContentDirectUrl(
        ...
        try {
            ...
            try {
                directAccessUrl =
                        primaryStore.requestContentDirectUrl(
                                contentUrl, attachment, fileName, mimetype, validFor);
            ...
            if (directAccessUrl == null) {
                if (!directAccessUrlSupported) {
                    ...
                    throw new UnsupportedOperationException(
                            "Retrieving direct access URLs is not supported by this content"
                                    + " store.");

...
public class ResultMapperTests {
    ...
    public static final String JSON_REPONSE =
            "{\"responseHeader\":{\"status\":0,\"QTime\":9},\"_original_parameters_\":\"org.apache.solr.common.params.DefaultSolrParams:{params(df=TEXT&alternativeDic=DEFAULT_DICTIONARY&fl=DBID,score&start=0&fq={!afts}AUTHORITY_FILTER_FROM_JSON&fq={!afts}TENANT_FILTER_FROM_JSON&rows=1000&locale=en_US&wt=json),defaults(carrot.url=id&spellcheck.collateExtendedResults=true&carrot.produceSummary=true&spellcheck.maxCollations=3&spellcheck.maxCollationTries=5&spellcheck.alternativeTermCount=2&spellcheck.extendedResults=false&defType=afts&spellcheck.maxResultsForSuggest=5&spellcheck=false&carrot.outputSubClusters=false&spellcheck.count=5&carrot.title=mltext@m___t@{http://www.alfresco.org/model/content/1.0}title&carrot.snippet=content@s___t@{http://www.alfresco.org/model/content/1.0}content&spellcheck.collate=true)}\",\"_field_mappings_\":{},\"_date_mappings_\":{},\"_range_mappings_\":{},\"_pivot_mappings_\":{},\"_interval_mappings_\":{},\"_stats_field_mappings_\":{},\"_stats_facet_mappings_\":{},\"_facet_function_mappings_\":{},\"response\":{\"numFound\":6,\"start\":0,\"maxScore\":0.7849362,\"docs\":[{\"DBID\":565,\"score\":0.7849362},{\"DBID\":566,\"score\":0.7849362},{\"DBID\":521,\"score\":0.3540957},{\"DBID\":514,\"score\":0.33025497},{\"DBID\":420,\"score\":0.32440513},{\"DBID\":415,\"score\":0.2780319}]},\"facet_counts\":{\"facet_queries\":{\"small\":0,\"large\":0,\"xtra"
                ...
                + "\"facet_intervals\":{\"creator\":{\"last\":4,\"first\":0},\"TheCreated\":{\"earlier\":5,\"lastYear\":0,\"currentYear\":854}}},\"spellcheck\":{\"searchInsteadFor\":\"alfresco\"},\"highlighting\":"
                + " {  \"_DEFAULT_!800001579e3d1964!800001579e3d1969\": {\"name\": [\"some very"
                + " <al>long<fresco> name\"],\"title\": [\"title1 is very <al>long<fresco>\"],"
                + " \"DBID\": \"521\"}, \"_DEFAULT_!800001579e3d1964!800001579e3d196a\": {\"name\":"
                + " [\"this is some <al>long<fresco> text.  It\", \" has the word <al>long<fresco>"
                + " in many places\", \".  In fact, it has <al>long<fresco> on some\", \" happens"
                + " to <al>long<fresco> in this case.\"], \"DBID\": \"1475846153692\"}"
                + "},\"stats\":{\"stats_fields\":{\"numericLabel\":{\"sumOfSquares\":0,\"min\":null,\"max\":null,\"mean\":\"NaN\",\"percentiles\":[\"0.0\",12,\"0.99\",20.0685],"
                + " \"count\":0,\"missing\":0,\"sum\":0,\"distinctValues\":[12,13,14,15,16,17,1],\"stddev\":0},"
                + " \"creator\":{\"min\":\"System\",\"max\":\"mjackson\",\"count\":\"990\",\"missing\":\"290\"},"
                + " \"created\":{\"sumOfSquares\":2.1513045770343806E27,\"min\":\"2011-02-15T20:16:27.080Z\",\"max\":\"2017-04-10T15:06:30.143Z\",\"mean\":\"2016-09-05T04:20:12.898Z\",\"count\":990,\"missing\":290,\"sum\":1.458318720769983E15,\"stddev\":5.6250677994522545E10}}},\"processedDenies\":true,"
                + " \"lastIndexedTx\":34}";
    ...
    public static void setupTests() {
        ...
        mapUserInfo.put(
                ...
                new UserInfo(AuthenticationUtil.getSystemUserName(), "sys", "sys"));
        ...
        when(versionHistory.getVersion(anyString()))
                .thenAnswer(
                        invocation ->
                                new VersionImpl(
                                        versionProperties,
                                        new NodeRef(
                                                StoreMapper.STORE_REF_VERSION2_SPACESSTORE,
                                                GUID.generate())));
        NodeService nodeService = mock(NodeService.class);

        when(versionService.getVersionHistory(any(NodeRef.class))).thenReturn(versionHistory);

        when(nodeService.getProperties(any(NodeRef.class)))
                .thenAnswer(
                        invocation -> {
                            Object[] args = invocation.getArguments();
                            NodeRef aNode = (NodeRef) args[0];
                            if (StoreMapper.STORE_REF_VERSION2_SPACESSTORE.equals(
                                    aNode.getStoreRef())) {
                                nodeProps.put(
                                        Version2Model.PROP_QNAME_FROZEN_NODE_REF,
                                        new NodeRef(
                                                StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
                                                FROZEN_ID + aNode.getId()));
        ...
        when(nodes.validateOrLookupNode(nullable(String.class), nullable(String.class)))
                .thenAnswer(
                        invocation -> {
                            Object[] args = invocation.getArguments();
        ...
        when(nodes.getFolderOrDocument(
                        any(NodeRef.class),
                ...
                .thenAnswer(
                        (Answer<Node>)
                                invocation -> {
                                    Object[] args = invocation.getArguments();
                                    NodeRef aNode = (NodeRef) args[0];
                                    if (StoreRef.STORE_REF_ARCHIVE_SPACESSTORE.equals(
                                            aNode.getStoreRef())) {
                                        // Return NULL if its from the archive store.
                                        return null;
                                    }
                                    return new Node(
                                            aNode, (NodeRef) args[1], nodeProps, mapUserInfo, sr);
                                });

        when(deletedNodes.getDeletedNode(
                        nullable(String.class),
                        nullable(Parameters.class),
                        anyBoolean(),
                        nullable(Map.class)))
                .thenAnswer(
                        (Answer<Node>)
                                invocation -> {
                                    ...
                                    NodeRef aNode =
                                            new NodeRef(
                                                    StoreRef.STORE_REF_ARCHIVE_SPACESSTORE, nodeId);
                                    return new Node(
                                            aNode,
                                            new NodeRef(
        ...
        when(propertyLookups.supports())
                .thenReturn(Stream.of("creator", "modifier").collect(Collectors.toSet()));
        when(propertyLookups.lookup(notNull(String.class)))
                .thenAnswer(
                        invocation -> {
                            Object[] args = invocation.getArguments();
    ...
    public void testNoResults() {
        ...
        CollectionWithPagingInfo<Node> collection =
                mapper.toCollectionWithPagingInfo(
                        EMPTY_PARAMS, searchRequest, null, new EmptyResultSet());
    ...
    public void testToCollectionWithPagingInfo() {
        ...
        SearchRequestContext searchRequest = SearchRequestContext.from(SearchQuery.EMPTY);
        CollectionWithPagingInfo<Node> collectionWithPage =
                mapper.toCollectionWithPagingInfo(
                        EMPTY_PARAMS, searchRequest, SearchQuery.EMPTY, results);
        ...
        collectionWithPage.getCollection().stream()
                .forEach(
                        aNode -> {
        ...
        assertEquals(
                1l,
                collectionWithPage.getCollection().stream()
                        .filter(node -> StoreMapper.DELETED.equals(node.getLocation()))
                        .count());
        ...
        assertEquals(
                1l,
                collectionWithPage.getCollection().stream()
                        .filter(node -> StoreMapper.VERSIONS.equals(node.getLocation()))
                        .count());
    }
    ...
    public void testToSearchContext() {
        ...
        SearchContext searchContext =
                mapper.toSearchContext((SearchEngineResultSet) results, searchRequest, searchQuery);
        ...
        // Facet intervals
        List<GenericFacetResponse> intervalFacets =
                searchContext.getFacets().stream()
                        .filter(f -> f.getType().equals(FACET_TYPE.interval))
        ...
        assertEquals(
                "cm:creator:<\"a\" TO \"b\"]",
                intervalFacets.get(0).getBuckets().get(0).getFilterQuery());
        Metric[] metrics =
                intervalFacets
                        .get(0)
                        .getBuckets()
        ...
        assertTrue(
                statsMetrics.contains(
                        new SimpleMetric(
                                METRIC_TYPE.sumOfSquares,
                                BigDecimal.valueOf(2.1513045770343806E27))));
        assertTrue(
                statsMetrics.contains(
                        new SimpleMetric(METRIC_TYPE.min, "2011-02-15T20:16:27.080Z")));
        assertTrue(
                statsMetrics.contains(
                        new SimpleMetric(METRIC_TYPE.max, "2017-04-10T15:06:30.143Z")));
        assertTrue(
                statsMetrics.contains(
                        new SimpleMetric(METRIC_TYPE.mean, "2016-09-05T04:20:12.898Z")));
        ...
        assertTrue(
                statsMetrics.contains(
                        new SimpleMetric(
                                METRIC_TYPE.sum, BigDecimal.valueOf(1.458318720769983E15))));
        assertTrue(
                statsMetrics.contains(
                        new SimpleMetric(
                                METRIC_TYPE.stddev, BigDecimal.valueOf(5.6250677994522545E10))));
        ...
        assertEquals(
                "min must be excluded because its null",
                0,
                statsMetrics.stream()
                        .filter(metric -> METRIC_TYPE.min.equals(metric.getType()))
                        .count());
        assertEquals(
                "max must be excluded because its null",
                0,
                statsMetrics.stream()
                        .filter(metric -> METRIC_TYPE.max.equals(metric.getType()))
                        .count());
        assertEquals(
                "mean must be excluded because its NaN",
                0,
                statsMetrics.stream()
                        .filter(metric -> METRIC_TYPE.mean.equals(metric.getType()))
                        .count());
    }
    ...
    public void testIsNullContext() {
        ...
        assertFalse(
                mapper.isNullContext(
                        new SearchContext(
                                0l, null, null, null, new SpellCheckContext(null, null), null)));
        assertFalse(
                mapper.isNullContext(
                        new SearchContext(
                                0l,
                                null,
                                asList(new FacetQueryContext(null, null, 0)),
                                null,
                                null,
                                null)));
        assertFalse(
                mapper.isNullContext(
                        new SearchContext(
                                ...
                                null,
                                null,
                                asList(new FacetFieldContext(null, null)),
                                null,
                                null)));
        assertFalse(
                mapper.isNullContext(
                        new SearchContext(
                                0l,
                                asList(new GenericFacetResponse(null, null, null)),
                                null,
                                null,
                                null,
                                null)));
    ...
    public void testHighlight() {
        ...
        highlightParameters =
                new GeneralHighlightParameters(1, 2, null, null, null, 50, true, fields);
        sp.setHighlight(highlightParameters);
    ...
    public void testInterval() {
        ...
        assertEquals(
                "cm:created:[\"*\" TO \"2016\">",
                intervalFacets.get(1).getBuckets().get(0).getFilterQuery());
        assertEquals(METRIC_TYPE.count, ((SimpleMetric) metrics[0]).getType());
        ...
        assertEquals(
                "cm:created:[\"2016\" TO \"2017\">",
                intervalFacets.get(1).getBuckets().get(1).getFilterQuery());
        assertEquals(METRIC_TYPE.count, ((SimpleMetric) metrics[0]).getType());
        ...
        assertEquals(
                "cm:created:[\"NOW/YEAR\" TO \"NOW/YEAR+1YEAR\"]",
                intervalFacets.get(1).getBuckets().get(2).getFilterQuery());
        assertEquals(METRIC_TYPE.count, ((SimpleMetric) metrics[0]).getType());
    ...
    public void testRange() {
        ...
        // Numeric facet range
        List<GenericFacetResponse> rangeFacets =
                searchContext.getFacets().stream()
                        .filter(f -> f.getType().equals(FACET_TYPE.range))
                        .collect(Collectors.toList());
        ...
        assertEquals(
                "[2015-09-29T10:45:15.729Z - 2016-01-07T10:45:15.729Z)",
                rangeFacets.get(0).getBuckets().get(0).getLabel());
        ...
        assertEquals(
                "created:[\"2015-09-29T10:45:15.729Z\" TO \"2016-01-07T10:45:15.729Z\">",
                rangeFacets.get(0).getBuckets().get(0).getFilterQuery());
        assertEquals(null, rangeFacets.get(0).getBuckets().get(0).getBucketInfo().get("count"));
        assertEquals(
                "false",
                rangeFacets.get(0).getBuckets().get(0).getBucketInfo().get("endInclusive"));
        assertEquals(
                "true",
                rangeFacets.get(0).getBuckets().get(0).getBucketInfo().get("startInclusive"));
        ...
        assertEquals("4", ((SimpleMetric) metrics[0]).getValue().get("count"));
        assertEquals(
                "content.size:[\"0\" TO \"100\">",
                rangeFacets.get(1).getBuckets().get(0).getFilterQuery());
        ...
        assertEquals(
                "content.size:[\"100\" TO \"200\">",
                rangeFacets.get(1).getBuckets().get(1).getFilterQuery());
        assertEquals(null, rangeFacets.get(1).getBuckets().get(1).getBucketInfo().get("count"));
        assertEquals(
                "false",
                rangeFacets.get(1).getBuckets().get(0).getBucketInfo().get("endInclusive"));
        assertEquals(
                "true",
                rangeFacets.get(1).getBuckets().get(0).getBucketInfo().get("startInclusive"));
        ...
        assertEquals(
                "content.size:[\"200\" TO \"300\">",
                rangeFacets.get(1).getBuckets().get(2).getFilterQuery());
    }
    ...
    public void testFacetingGroupResponse() {
        String jsonQuery =
                "{\"query\": {\"query\": \"alfresco\"},\"facetQueries\": [{\"query\":"
                        + " \"content.size:[o TO 102400]\", \"label\":"
        ...
        String expectedResponse =
                "{\"responseHeader\":{\"status\":0,\"QTime\":9},\"_original_parameters_\":\"org.apache.solr.common.params.DefaultSolrParams:{params(df=TEXT&alternativeDic=DEFAULT_DICTIONARY&fl=DBID,score&start=0&fq={!afts}AUTHORITY_FILTER_FROM_JSON&fq={!afts}TENANT_FILTER_FROM_JSON&rows=1000&locale=en_US&wt=json),defaults(carrot.url=id&spellcheck.collateExtendedResults=true&carrot.produceSummary=true&spellcheck.maxCollations=3&spellcheck.maxCollationTries=5&spellcheck.alternativeTermCount=2&spellcheck.extendedResults=false&defType=afts&spellcheck.maxResultsForSuggest=5&spellcheck=false&carrot.outputSubClusters=false&spellcheck.count=5&carrot.title=mltext@m___t@{http://www.alfresco.org/model/content/1.0}title&carrot.snippet=content@s___t@{http://www.alfresco.org/model/content/1.0}content&spellcheck.collate=true)}\",\"_field_mappings_\":{},\"_date_mappings_\":{},\"_range_mappings_\":{},\"_pivot_mappings_\":{},\"_interval_mappings_\":{},\"_stats_field_mappings_\":{},\"_stats_facet_mappings_\":{},\"_facet_function_mappings_\":{},\"response\":{\"numFound\":6,\"start\":0,\"maxScore\":0.7849362,\"docs\":[{\"DBID\":565,\"score\":0.7849362},{\"DBID\":566,\"score\":0.7849362},{\"DBID\":521,\"score\":0.3540957},{\"DBID\":514,\"score\":0.33025497},{\"DBID\":420,\"score\":0.32440513},{\"DBID\":415,\"score\":0.2780319}]},"
                    + "\"spellcheck\":{\"searchInsteadFor\":\"alfresco\"},\"facet_counts\":{\"facet_queries\":"
                    + " {\"small\": 52,\"large\": 0,\"medium\": 0}},\"processedDenies\":true,"
                    + " \"lastIndexedTx\":34}";
        ...
        assertEquals(
                "content.size:[o TO 102400]",
                searchContext.getFacets().get(0).getBuckets().get(0).getFilterQuery());
    ...
    public void testFacetingGroupResponseV1() {
        String jsonQuery =
                "{\"query\": {\"query\": \"alfresco\"}, \"facetFormat\":\"V1\",\"facetQueries\":"
                        + " [{\"query\": \"content.size:[o TO 102400]\", \"label\":"
                        + " \"small\"},{\"query\": \"content.size:[102400 TO 1048576]\", \"label\":"
                        + " \"medium\",\"group\":\"foo\"},{\"query\": \"content.size:[1048576 TO"
                        + " 16777216]\", \"label\": \"large\"}]}";
    ...
    /**
     * Test facet fields with out group label in the query. This is to support original api methods
     * query for facet query.
     */
    @Test
    public void testFacetQueryWithoutGroupResponse() {
        String jsonQuery =
                "{\"query\": {\"query\": \"alfresco\"},\"facetQueries\": [{\"query\":"
                    + " \"content.size:[0 TO 102400]\", \"label\": \"small\"},{\"query\":"
                    + " \"content.size:[102400 TO 1048576]\", \"label\": \"medium\"},{\"query\":"
                    + " \"content.size:[1048576 TO 16777216]\", \"label\": \"large\"}]}";
        ...
        assertEquals("small", searchContext.getFacetQueries().get(0).getLabel());
        assertEquals(
                "content.size:[0 TO 102400]",
                searchContext.getFacetQueries().get(0).getFilterQuery());
        ...
        assertEquals(
                "content.size:[1048576 TO 16777216]",
                searchContext.getFacetQueries().get(1).getFilterQuery());
        ...
        assertEquals(
                ...
                searchContext.getFacetQueries().get(2).getFilterQuery());
    ...
    private ResultSet mockResultSet(String json) {
        ...
        sp.setBulkFetchEnabled(false);
        ResultSet results =
                new SolrJSONResultSet(jsonObj, sp, nodeService, null, LimitBy.FINAL_SIZE, 10);
        return results;
    }
    ...
    private ResultSet mockResultSet(List<Long> archivedNodes, List<Long> versionNodes) {
        ...
        NodeService nodeService = mock(NodeService.class);
        when(nodeService.getNodeRef(any()))
                .thenAnswer(
                        (Answer<NodeRef>)
                                invocation -> {
                                    Object[] args = invocation.getArguments();
                                    // If the DBID is in the list archivedNodes, instead of
                                    // returning a noderef return achivestore noderef
        ...
        ResultSet results =
                new SolrJSONResultSet(json, sp, nodeService, null, LimitBy.FINAL_SIZE, 10);
        return results;
    }

    /**
     * Validates that when facetFormat is specified then all facets are returned in the generic
     * facet response format AKA V2.
     */
    @Test
    public void facetFormatTest() {
        ...
        expectedResponse =
                "{\"responseHeader\":{\"status\":0,\"QTime\":9},\"_original_parameters_\":\"org.apache.solr.common.params.DefaultSolrParams:{params(df=TEXT&alternativeDic=DEFAULT_DICTIONARY&fl=DBID,score&start=0&fq={!afts}AUTHORITY_FILTER_FROM_JSON&fq={!afts}TENANT_FILTER_FROM_JSON&rows=1000&locale=en_US&wt=json),defaults(carrot.url=id&spellcheck.collateExtendedResults=true&carrot.produceSummary=true&spellcheck.maxCollations=3&spellcheck.maxCollationTries=5&spellcheck.alternativeTermCount=2&spellcheck.extendedResults=false&defType=afts&spellcheck.maxResultsForSuggest=5&spellcheck=false&carrot.outputSubClusters=false&spellcheck.count=5&carrot.title=mltext@m___t@{http://www.alfresco.org/model/content/1.0}title&carrot.snippet=content@s___t@{http://www.alfresco.org/model/content/1.0}content&spellcheck.collate=true)}\",\"_field_mappings_\":{},\"_date_mappings_\":{},\"_range_mappings_\":{},\"_pivot_mappings_\":{},\"_interval_mappings_\":{},\"_stats_field_mappings_\":{},\"_stats_facet_mappings_\":{},\"_facet_function_mappings_\":{},\"response\":{\"numFound\":6,\"start\":0,\"maxScore\":0.7849362,\"docs\":[{\"DBID\":565,\"score\":0.7849362},{\"DBID\":566,\"score\":0.7849362},{\"DBID\":521,\"score\":0.3540957},{\"DBID\":514,\"score\":0.33025497},{\"DBID\":420,\"score\":0.32440513},{\"DBID\":415,\"score\":0.2780319}]},"
                    ...
                    + "\"facet_counts\":{\"facet_fields\":{\"creator\":[\"System\",124,\"mjackson\",11,\"abeecher\",4],\"modifier\":[\"System\",124,\"mjackson\",8,\"admin\",7]}},\"processedDenies\":true,"
                    + " \"lastIndexedTx\":34}";
        results = mockResultSet(expectedResponse);
        searchQuery = helper.extractFromJson(jsonQuery);
        searchContext =
        ...
        assertEquals(
                "creator:\"System\"",
                searchContext.getFacetsFields().get(0).getBuckets().get(0).getFilterQuery());
        assertEquals(
                "System", searchContext.getFacetsFields().get(0).getBuckets().get(0).getLabel());
    ...
    public void hasGroupTest() {
        ...
        String noFacetQueries =
                "{\"query\": {\"query\": \"alfresco\"}," + "\"facetQueries\": []" + "}";
        SearchQuery searchQuery3 = helper.extractFromJson(noFacetQueries);
        assertFalse(ResultMapper.hasGroup(searchQuery3));
    }

    /**
     * When the following is passed "facetQueries": [ {"query": "content.size:[0 TO 102400]",
     * "label": "small", "group": "one"}, {"query": "content.size:[102400 TO 1048576]", "label":
     * "medium", "group": "two"}, {"query": "content.size:[1048576 TO 16777216]", "label": "large"}
    ...
    public void testFacetingWithPartialGroup() {
        String jsonQuery =
                "{\"query\": {\"query\": \"alfresco\"},\"facetQueries\": [{\"query\":"
                        + " \"content.size:[o TO 102400]\", \"label\":"
                        + " \"small\",\"group\":\"1\"},{\"query\": \"content.size:[102400 TO"
                        + " 1048576]\", \"label\": \"medium\",\"group\":\"2\"},{\"query\":"
                        + " \"content.size:[1048576 TO 16777216]\", \"label\": \"large\"}]}";
        ...
        assertEquals(
                "content.size:[o TO 102400]",
                searchContext.getFacets().get(1).getBuckets().get(0).getFilterQuery());
        assertFalse(searchContext.getFacets().get(0).getBuckets().get(0).getMetrics().isEmpty());
        Metric[] metrics =
                searchContext
                        .getFacets()
    ...
    public void testSqlResponse() {
        JSONObject response =
                new JSONObject(
                        "{\"docs\":[{\"SITE\":\"_REPOSITORY_\"},{\"SITE\":\"surf-config\"},{\"SITE\":\"swsdp\"},{\"EOF\":true,\"RESPONSE_TIME\":96}]}");
        JSONArray docs = response.getJSONArray("docs");
        SearchSQLQuery query =
                new SearchSQLQuery(
                        "select SITE from alfresco group by SITE",
                        null,
                        null,
                        100,
                        false,
        ...
        try {
            ...
            assertNotNull(e);
            assertEquals(
                    "Solr response is required instead of JSONArray docs was null", e.getMessage());
        }
        try {
    ...
    /**
     * Check that when a node is returned from the index the permissions are still checked in the
     * DB.
     */
    @Test
    public void testGetNode() {
        ...
        when(nodes.getFolderOrDocument(eq(nodeRef), any(), any(), any(), any()))

...
public class EnterprisePublicApiTestFixture extends EnterpriseTestFixture {
     ...
     */
    public static synchronized EnterprisePublicApiTestFixture getInstance(boolean createTestData)
            throws Exception {
        if (instance == null) {
    ...
    private EnterprisePublicApiTestFixture() {
        super(
                ...
                PORT,
                CONTEXT_PATH,
                PUBLIC_API_SERVLET_NAME,
                DEFAULT_NUM_MEMBERS_PER_SITE,
                false);
    }

    @Override
    protected JettyComponent makeJettyComponent() {
        JettyComponent jettyComponent =
                new EnterpriseJettyComponent(
                        getPort(), getContextPath(), getConfigLocations(), getClassLocations());

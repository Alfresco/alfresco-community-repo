...
public class SqlSessionMetricsWrapper implements SqlSession {
  ...
  public SqlSessionMetricsWrapper(
  ...
  private void reportQueryExecuted(
    ...
    final String queryTypeTag,
    final String statementID
  ) {
    try {
      if (
        dbMetricsReporter != null && dbMetricsReporter.isQueryMetricsEnabled()
      ) {
        final long delta = System.currentTimeMillis() - startTime;
        dbMetricsReporter.reportQueryExecutionTime(
          delta,
          queryTypeTag,
          statementID
        );
  ...
  public void select(
    ...
    PassThroughMetricsResultsHandler passThroughHandler = new PassThroughMetricsResultsHandler(
      handler,
      startTime,
      statement
    );
  ...
  private void logCouldNotReportDBQueryExecution(Exception e) {
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Could not report DB query execution. Message:" + e.getMessage(),
        e
      );
    }
  }

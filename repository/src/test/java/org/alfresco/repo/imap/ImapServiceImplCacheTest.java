...
/**
 * Unit test for cache implementation in the ImapServiceImpl. Based on ImapServiceImplTest, but we
 * need this separate test because we need to get transactions to commit to trigger behaviours in
 * ImapServiceImpl.
 *
 * @author ArsenyKo
 */
...
public class ImapServiceImplCacheTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        oldFile =
                fileFolderService.create(folder.getNodeRef(), "oldFile", ContentModel.TYPE_CONTENT);
    ...
    public void testRepoBehaviourWithFoldersCache() throws Exception {
        ...
        // Mailbox path with default mount point should be like 'Alfresco
        // IMAP/aaa/ALF9361/ALF9361_0/sub_0
        FileInfo localRootFolder =
                fileFolderService.create(
                        testImapFolderNodeRef, folderName, ContentModel.TYPE_FOLDER);
        List<FileInfo> subFolders = new ArrayList<FileInfo>(10);
        for (int i = 0; i < 3; i++) {
            String childMailbox = folderName + "_" + i;
            FileInfo subFolder =
                    fileFolderService.create(
                            localRootFolder.getNodeRef(), childMailbox, ContentModel.TYPE_FOLDER);
            for (int j = 0; j < 3; j++) {
                String subChildMailbox = "sub_" + j;
                fileFolderService.create(
                        subFolder.getNodeRef(), subChildMailbox, ContentModel.TYPE_FOLDER);
            }
        ...
        fileFolderService.move(
                oldFile.getNodeRef(), folder.getFolderInfo().getNodeRef(), folder.getName());
        ...
        for (int i = 0; i < count; i++) {
            ...
            ContentWriter contentWriter =
                    contentService.getWriter(
                            contentItem.getNodeRef(), ContentModel.PROP_CONTENT, false);

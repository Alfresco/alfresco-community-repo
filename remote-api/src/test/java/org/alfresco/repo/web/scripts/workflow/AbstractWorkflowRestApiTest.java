...
public abstract class AbstractWorkflowRestApiTest extends BaseWebScriptTest {
    ...
    public void testTaskInstancesGet() throws Exception {
        ...
        Response response =
                sendRequest(new GetRequest(MessageFormat.format(URL_USER_TASKS, USER2)), 200);
        ...
        WorkflowDefinition adhocDef =
                workflowService.getDefinitionByName(getAdhocWorkflowDefinitionName());
        ...
        List<WorkflowTask> tasks =
                workflowService.getAssignedTasks(USER2, WorkflowTaskState.IN_PROGRESS);
        ...
        checkTasksPresent(
                MessageFormat.format(
                        URL_TASKS_DUE_AFTER, ISO8601DateFormat.format(dueDateCal.getTime())),
                true,
                task.getId());
        ...
        checkTasksPresent(
                MessageFormat.format(
                        URL_TASKS_DUE_BEFORE, ISO8601DateFormat.format(dueDateCal.getTime())),
                false,
                task.getId());
        ...
        checkTaskPropertyFiltering(
                customProperties, Arrays.asList("bpm_description", "bpm_priority"));
    ...
    public void testTaskInstancesGetWithFiltering() throws Exception {
        ...
        // Check USER2's tasks without filtering. It should return two tasks as USER2 is a member of
        // the GROUP
        personManager.setUser(USER2);
        response = sendRequest(new GetRequest(MessageFormat.format(URL_USER_TASKS, USER2)), 200);
        getJsonArray(response, 2);

        // Check USER2's tasks With filtering where property bpm:description should match
        // "descTest1"
        response =
                sendRequest(
                        new GetRequest(
        ...
        // Check USER3's tasks without filtering. It should return 0 task as USER3 is not a member
        // of the GROUP
    ...
    public void testWorkflowPermissions() throws Exception {
        ...
        personManager.setUser(USER2);
        Response response =
                sendRequest(new GetRequest(MessageFormat.format(URL_USER_TASKS, USER1)), 200);
        assertEquals(Status.STATUS_OK, response.getStatus());
        String jsonStr = response.getContentAsString();
        JSONObject json = new JSONObject(jsonStr);
        JSONArray results = json.getJSONArray("data");
    ...
    public void testTaskInstanceGet() throws Exception {
        ...
        // Get the start-task
        Response response =
                sendRequest(new GetRequest(URL_TASKS + "/" + startTask.getId()), Status.STATUS_OK);
        String jsonStr = response.getContentAsString();
        JSONObject json = new JSONObject(jsonStr);
        ...
        checkWorkflowInstance(
                startTask.getPath().getInstance(), result.getJSONObject("workflowInstance"));
        ...
        checkWorkflowInstance(
                firstTask.getPath().getInstance(), result.getJSONObject("workflowInstance"));
    ...
    private void checkWorkflowInstance(WorkflowInstance wfInstance, JSONObject instance)
            throws Exception {
        assertNotNull(instance);
        ...
        assertEquals(
                wfInstance.getDefinition().getDescription(), instance.getString("description"));
    ...
    private void checkWorkflowTaskDefinition(
            WorkflowTaskDefinition wfDefinition, JSONObject definition) throws Exception {
        ...
        assertEquals(
                startType.getDescription(this.dictionaryService), type.getString("description"));
    ...
    public void testTaskInstancePutCompletedTask() throws Exception {
        ...
        JSONObject jsonProperties =
                new JSONObject(getResponse.getContentAsString())
                        .getJSONObject("data")
                        .getJSONObject("properties");
    ...
    public void testWorkflowDefinitionsGet() throws Exception {
        ...
        response =
                sendRequest(new GetRequest(URL_WORKFLOW_DEFINITIONS + "?exclude=" + exclude), 200);
        assertEquals(Status.STATUS_OK, response.getStatus());
        ...
        assertFalse(
                "Found adhoc workflow when it was supposed to be excluded", adhocWorkflowPresent);
    ...
    public void testWorkflowDefinitionGet() throws Exception {
        ...
        assertEquals(
                startTaskDefUrl, "api/classes/" + getSafeDefinitionName(ADHOC_START_TASK_TYPE));

        assertTrue(workflowDefinitionJSON.has("startTaskDefinitionType"));
        assertEquals(
                ADHOC_START_TASK_TYPE, workflowDefinitionJSON.getString("startTaskDefinitionType"));
    ...
    private void checkTaskDefinitionTypeAndUrl(String expectedTaskType, JSONObject taskDefinition)
            throws Exception {
        // Check type
    ...
    public void testWorkflowInstanceGet() throws Exception {
        ...
        assertEquals(
                adhocInstance.getDefinition().getDescription(), result.getString("description"));
    ...
    public void testWorkflowInstancesGet() throws Exception {
        ...
        Response forDefinitionResponse =
                sendRequest(
                        new GetRequest(
                                MessageFormat.format(
                                        URL_WORKFLOW_INSTANCES_FOR_DEFINITION, adhocDef.getId())),
                        200);
        ...
        checkFiltering(
                URL_WORKFLOW_INSTANCES + "?startedAfter=" + ISO8601DateFormat.format(anHourAgo));

        // filter by startedBefore
        checkFiltering(
                URL_WORKFLOW_INSTANCES + "?startedBefore=" + ISO8601DateFormat.format(anHourLater));
        ...
        // filter by dueBefore
        checkFiltering(
                URL_WORKFLOW_INSTANCES + "?dueBefore=" + ISO8601DateFormat.format(anHourLater));
        ...
        checkFiltering(
                URL_WORKFLOW_INSTANCES + "?definitionName=" + getAdhocWorkflowDefinitionName());

        // paging
        int maxItems = 3;
        for (int skipCount = 0; skipCount < totalItems; skipCount += maxItems) {
            checkPaging(
                    URL_WORKFLOW_INSTANCES + "?maxItems=" + maxItems + "&skipCount=" + skipCount,
                    totalItems,
                    maxItems,
                    skipCount);
        }
    ...
    public void testWorkflowInstanceDeleteAsAdministrator() throws Exception {
        ...
        List<WorkflowInstance> instances =
                workflowService.getActiveWorkflows(adhocInstance.getDefinition().getId());
    ...
    public void testWorkflowInstanceDeleteAsRecreatedUser() throws Exception {
        ...
        personManager.setUser(USER1);
        sendRequest(
                new DeleteRequest(URL_WORKFLOW_INSTANCES + "/" + adhocPath.getInstance().getId()),
                Status.STATUS_OK);
    ...
    protected void runReviewFlow(boolean approve) throws Exception {
        // Start workflow as USER1
        personManager.setUser(USER1);
        WorkflowDefinition reviewDef =
                workflowService.getDefinitionByName(getReviewWorkflowDefinitionName());
        Map<QName, Serializable> params = new HashMap<QName, Serializable>();
        // Reviewer is USER2
        params.put(WorkflowModel.ASSOC_ASSIGNEE, personManager.get(USER2));
        Date dueDate = new Date();
    ...
    protected void runReviewPooledFlow(boolean approve) throws Exception {
        ...
        personManager.setUser(USER3);
        Response response =
                sendRequest(new GetRequest(MessageFormat.format(URL_USER_TASKS, USER3)), 200);
        assertEquals(Status.STATUS_OK, response.getStatus());
        String jsonStr = response.getContentAsString();
        JSONObject json = new JSONObject(jsonStr);
        JSONArray results = json.getJSONArray("data");
        ...
        sendRequest(
                new PutRequest(URL_TASKS + "/" + taskId, properties.toString(), "application/json"),
                200);
    ...
    protected void setUp() throws Exception {
        ...
        MutableAuthenticationService authenticationService =
                (MutableAuthenticationService) appContext.getBean("AuthenticationService");
        PersonService personService = (PersonService) appContext.getBean("PersonService");
        SearchService searchService = (SearchService) appContext.getBean("SearchService");
        FileFolderService fileFolderService =
                (FileFolderService) appContext.getBean("FileFolderService");
        nodeService = (NodeService) appContext.getBean("NodeService");

        // for the purposes of the tests make sure workflow engine is enabled/visible.
        WorkflowAdminServiceImpl workflowAdminService =
                (WorkflowAdminServiceImpl) appContext.getBean("workflowAdminService");
        this.wfTestHelper = new WorkflowTestHelper(workflowAdminService, getEngine(), true);

        AuthorityService authorityService =
                (AuthorityService) appContext.getBean("AuthorityService");
            ...
            if (before.has(name)) {
                if (before.get(name) instanceof JSONArray) {
                    for (int i = 0; i < before.getJSONArray(name).length(); i++) {
                        assertEquals(
                                before.getJSONArray(name).get(i), after.getJSONArray(name).get(i));
                    }
                } else {
    ...
    private void checkTasksPresent(String url, boolean mustBePresent, String... ids)
            throws Exception {
    ...
    private void checkTaskPropertyFiltering(
            String propertiesParamValue, List<String> expectedProperties) throws Exception {
        JSONObject data =
                getDataFromRequest(
                        MessageFormat.format(
                                URL_USER_TASKS_PROPERTIES, USER2, propertiesParamValue));
    ...
    private void checkPaging(String url, int totalItems, int maxItems, int skipCount)
            throws Exception {
        Response response = sendRequest(new GetRequest(url), 200);

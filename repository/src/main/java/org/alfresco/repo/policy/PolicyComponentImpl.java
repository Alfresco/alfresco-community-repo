...
public class PolicyComponentImpl implements PolicyComponent {
    ...
    private Map<QName, ClassBehaviourIndex<ClassBehaviourBinding>> classBehaviours =
            new HashMap<QName, ClassBehaviourIndex<ClassBehaviourBinding>>();

    // Map of Property Behaviours (by policy name)
    private Map<QName, ClassBehaviourIndex<ClassFeatureBehaviourBinding>> propertyBehaviours =
            new HashMap<QName, ClassBehaviourIndex<ClassFeatureBehaviourBinding>>();

    // Map of Association Behaviours (by policy name)
    private Map<QName, ClassBehaviourIndex<ClassFeatureBehaviourBinding>> associationBehaviours =
            new HashMap<QName, ClassBehaviourIndex<ClassFeatureBehaviourBinding>>();

    // Wild Card Feature
    private static final QName FEATURE_WILDCARD =
            QName.createQName(NamespaceService.DEFAULT_URI, "*");
     ...
     */
    public void setTransactionInvocationHandlerFactory(
            TransactionInvocationHandlerFactory factory) {
    ...
    public <P extends ClassPolicy> ClassPolicyDelegate<P> registerClassPolicy(Class<P> policy) {
        ...
        registeredPolicies.put(
                new PolicyKey(definition.getType(), definition.getName()), definition);
        ClassPolicyDelegate<P> delegate =
                new ClassPolicyDelegate<P>(
                        dictionary,
                        policy,
                        getClassBehaviourIndex(definition.getName()),
                        tryLockTimeout);
    ...
    public <P extends PropertyPolicy> PropertyPolicyDelegate<P> registerPropertyPolicy(
        ...
        registeredPolicies.put(
                new PolicyKey(definition.getType(), definition.getName()), definition);
        PropertyPolicyDelegate<P> delegate =
                new PropertyPolicyDelegate<P>(
                        dictionary,
                        policy,
                        getPropertyBehaviourIndex(definition.getName()),
                        tryLockTimeout);

        if (logger.isInfoEnabled())
            logger.info(
                    "Registered property policy "
                            + definition.getName()
                            + " ("
                            + definition.getPolicyInterface()
                            + ")");
    ...
    public <P extends AssociationPolicy> AssociationPolicyDelegate<P> registerAssociationPolicy(
        ...
        registeredPolicies.put(
                new PolicyKey(definition.getType(), definition.getName()), definition);
        AssociationPolicyDelegate<P> delegate =
                new AssociationPolicyDelegate<P>(
                        dictionary,
                        policy,
                        getAssociationBehaviourIndex(definition.getName()),
                        tryLockTimeout);

        if (logger.isInfoEnabled())
            logger.info(
                    "Registered association policy "
                            + definition.getName()
                            + " ("
                            + definition.getPolicyInterface()
                            + ")");
    ...
    public BehaviourDefinition<ClassBehaviourBinding> bindClassBehaviour(
            QName policy, QName classRef, Behaviour behaviour) {
        // Validate arguments
        ...
        if (classDefinition == null) {
            throw new IllegalArgumentException(
                    "Class " + classRef + " has not been defined in the data dictionary");
        }
        ...
        BehaviourDefinition<ClassBehaviourBinding> definition =
                createBehaviourDefinition(PolicyType.Class, policy, binding, behaviour);
    ...
    public BehaviourDefinition<ServiceBehaviourBinding> bindClassBehaviour(
        ...
        BehaviourDefinition<ServiceBehaviourBinding> definition =
                createBehaviourDefinition(PolicyType.Class, policy, binding, behaviour);
    ...
    public BehaviourDefinition<ClassFeatureBehaviourBinding> bindPropertyBehaviour(
        ...
        // Create behaviour definition and bind to policy
        ClassFeatureBehaviourBinding binding =
                new ClassFeatureBehaviourBinding(dictionary, className, propertyName);
        BehaviourDefinition<ClassFeatureBehaviourBinding> definition =
                createBehaviourDefinition(PolicyType.Property, policy, binding, behaviour);
    ...
    public BehaviourDefinition<ClassFeatureBehaviourBinding> bindPropertyBehaviour(
        ...
        if (classDefinition == null) {
            throw new IllegalArgumentException(
                    "Class " + className + " has not been defined in the data dictionary");
        }

        // Create behaviour definition and bind to policy
        ClassFeatureBehaviourBinding binding =
                new ClassFeatureBehaviourBinding(dictionary, className, FEATURE_WILDCARD);
        BehaviourDefinition<ClassFeatureBehaviourBinding> definition =
                createBehaviourDefinition(PolicyType.Property, policy, binding, behaviour);
        getPropertyBehaviourIndex(policy).putClassBehaviour(definition);
    ...
    public BehaviourDefinition<ClassFeatureBehaviourBinding> bindAssociationBehaviour(
        ...
        // Create behaviour definition and bind to policy
        ClassFeatureBehaviourBinding binding =
                new ClassFeatureBehaviourBinding(dictionary, className, assocName);
        BehaviourDefinition<ClassFeatureBehaviourBinding> definition =
                createBehaviourDefinition(PolicyType.Association, policy, binding, behaviour);
     ...
     */
    private synchronized ClassBehaviourIndex<ClassBehaviourBinding> getClassBehaviourIndex(
            QName policy) {
     ...
     */
    private synchronized ClassBehaviourIndex<ClassFeatureBehaviourBinding>
            getPropertyBehaviourIndex(QName policy) {
     ...
     */
    private synchronized ClassBehaviourIndex<ClassFeatureBehaviourBinding>
            getAssociationBehaviourIndex(QName policy) {
     ...
     */
    @SuppressWarnings("unchecked")
    private <B extends BehaviourBinding> BehaviourDefinition<B> createBehaviourDefinition(
            PolicyType type, QName policy, B binding, Behaviour behaviour) {
    ...
    private PolicyDefinition createPolicyDefinition(Class policyIF) {
        ...
        try {
            ...
            if (!String.class.isAssignableFrom(metadata.getType())) {
                throw new PolicyException(
                        "NAMESPACE metadata incorrectly specified in policy "
        ...
        if (methods.length != 1) {
            throw new PolicyException(
                    "Policy " + policyIF.getCanonicalName() + " must declare only one method");
        }
            ...
            try {
                ...
                if (!Arg.class.isAssignableFrom(argMetadata.getType())) {
                    throw new PolicyException(
                            "ARG_"
                                    + i
                                    + " metadata incorrectly specified in policy "
                                    + policyIF.getCanonicalName());
                }
     ...
     * @param <B> the type of binding
     */
    /*package*/ class BehaviourDefinitionImpl<B extends BehaviourBinding>
        ...
        /*package*/ BehaviourDefinitionImpl(
                PolicyType type, QName policy, B binding, Behaviour behaviour) {
            this.type = type;
            this.policy = policy;

...
public class GetFavoritesTests extends RestTest {
    ...
    public void dataPreparation() throws Exception {
        ...
        firstFolderModel =
                dataContent.usingUser(adminUserModel).usingSite(firstSiteModel).createFolder();
        secondFolderModel =
                dataContent.usingUser(adminUserModel).usingSite(firstSiteModel).createFolder();
        firstFileModel =
                dataContent
                        .usingUser(adminUserModel)
                        .usingResource(firstFolderModel)
    ...
    @TestRail(
            section = {TestGroup.REST_API, TestGroup.FAVORITES},
    ...
    public void managerIsAbleToRetrieveFavorites() throws Exception {
        ...
        userFavorites
                .assertThat()
                .entriesListContains("targetGuid", firstSiteModel.getGuid())
                .assertThat()
    ...
    @TestRail(
            ...
            description =
                    "Verify Admin user gets favorites sites with Rest API and status code is 200")
    ...
    public void adminIsAbleToRetrieveFavoritesSites() throws Exception {
        restClient
                ...
                .withCoreAPI()
                .usingUser(adminUserModel)
                .addSiteToFavorites(firstSiteModel);
        restClient.withCoreAPI().usingUser(adminUserModel).addSiteToFavorites(secondSiteModel);

        userFavorites =
                restClient.withCoreAPI().usingAuthUser().where().targetSiteExist().getFavorites();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        userFavorites
                .assertThat()
                .entriesListContains("targetGuid", firstSiteModel.getGuid())
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Admin user gets favorites folders with Rest API and status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void adminIsAbleToRetrieveFavoritesFolders() throws Exception {
        restClient
                ...
                .withCoreAPI()
                .usingUser(adminUserModel)
                .addFolderToFavorites(firstFolderModel);
        restClient.withCoreAPI().usingUser(adminUserModel).addFolderToFavorites(secondFolderModel);

        userFavorites =
                restClient.withCoreAPI().usingAuthUser().where().targetFolderExist().getFavorites();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        userFavorites
                .assertThat()
                .entriesListContains("targetGuid", firstFolderModel.getNodeRef())
                .and()
                .entriesListContains("targetGuid", secondFolderModel.getNodeRef());
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Admin user gets favorites files with Rest API and status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void adminIsAbleToRetrieveFavoritesFiles() throws Exception {
        restClient
                ...
                .withCoreAPI()
                .usingUser(adminUserModel)
                .addFileToFavorites(firstFileModel);
        restClient.withCoreAPI().usingUser(adminUserModel).addFileToFavorites(secondFileModel);

        userFavorites =
                restClient.withCoreAPI().usingAuthUser().where().targetFileExist().getFavorites();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        userFavorites
                .assertThat()
                .entriesListContains("targetGuid", firstFileModel.getNodeRefWithoutVersion())
                .and()
                .entriesListContains("targetGuid", secondFileModel.getNodeRefWithoutVersion());
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Collaborator user gets favorites with Rest API and status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void collaboratorIsAbleToRetrieveFavorites() throws Exception {
        restClient
                .authenticateUser(firstSiteUsers.getOneUserWithRole(UserRole.SiteCollaborator))
                .withCoreAPI()
                .usingAuthUser()
                .addSiteToFavorites(firstSiteModel);
        restClient.withCoreAPI().usingAuthUser().addSiteToFavorites(secondSiteModel);
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Contributor user gets favorites with Rest API and status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void contributorIsAbleToRetrieveFavorites() throws Exception {
        restClient
                .authenticateUser(firstSiteUsers.getOneUserWithRole(UserRole.SiteContributor))
                .withCoreAPI()
                .usingAuthUser()
                .addSiteToFavorites(firstSiteModel);
        restClient.withCoreAPI().usingAuthUser().addSiteToFavorites(secondSiteModel);
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Consumer user gets favorites with Rest API and status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void consumerIsAbleToRetrieveFavorites() throws Exception {
        restClient
                .authenticateUser(firstSiteUsers.getOneUserWithRole(UserRole.SiteConsumer))
                .withCoreAPI()
                .usingAuthUser()
                .addSiteToFavorites(firstSiteModel);
        restClient.withCoreAPI().usingAuthUser().addSiteToFavorites(secondSiteModel);
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user doesn't have permission to get favorites of another user with Rest"
                            + " API and status code is 404")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void userIsNotAbleToRetrieveFavoritesOfAnotherUser() throws Exception {
        restClient
                .authenticateUser(firstSiteUsers.getOneUserWithRole(UserRole.SiteConsumer))
                .withCoreAPI()
                .usingUser(firstSiteUsers.getOneUserWithRole(UserRole.SiteCollaborator))
                .getFavorites();
        restClient
                ...
                .containsSummary(
                        String.format(
                                ...
                                firstSiteUsers
                                        .getOneUserWithRole(UserRole.SiteCollaborator)
                                        .getUsername()));
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user doesn't have permission to get favorites of admin user with Rest"
                            + " API and status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void userIsNotAbleToRetrieveFavoritesOfAdminUser() throws Exception {
        ...
        restClient
                ...
                .containsSummary(
                        String.format(
                                RestErrorModel.ENTITY_NOT_FOUND, adminUserModel.getUsername()));
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify admin user doesn't have permission to get favorites of another user"
                            + " with Rest API and status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify User gets only favorites sites with Rest API and status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void userIsAbleToRetrieveOnlyFavoritesSites() throws Exception {
        ...
        userFavorites
                ...
                .entriesListContains("targetGuid", firstSiteModel.getGuid())
                .and()
                .entriesListDoesNotContain("targetGuid", secondSiteModel.getGuid())
                .and()
                .entriesListDoesNotContain("targetGuid", firstFileModel.getNodeRefWithoutVersion())
                .and()
                .entriesListDoesNotContain("targetGuid", firstFolderModel.getNodeRef());
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify User gets only favorites files with Rest API and status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void userIsAbleToRetrieveOnlyFavoritesFiles() throws Exception {
        ...
        userFavorites
                ...
                .entriesListContains("targetGuid", firstFileModel.getNodeRefWithoutVersion())
                .and()
                .entriesListDoesNotContain("targetGuid", secondFileModel.getNodeRefWithoutVersion())
                .and()
                .entriesListDoesNotContain("targetGuid", firstSiteModel.getGuid())
                .and()
                .entriesListDoesNotContain("targetGuid", firstFolderModel.getNodeRef());
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify User gets only favorites folders with Rest API and status code is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void userIsAbleToRetrieveOnlyFavoritesFolders() throws Exception {
        ...
        userFavorites
                ...
                .entriesListContains("targetGuid", firstFolderModel.getNodeRef())
                .and()
                .entriesListDoesNotContain("targetGuid", secondFolderModel.getNodeRef())
                .and()
                .entriesListDoesNotContain("targetGuid", firstSiteModel.getGuid())
                .and()
                .entriesListDoesNotContain("targetGuid", firstFileModel.getNodeRefWithoutVersion());
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify User gets only favorites files or folders with Rest API and status code"
                            + " is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify User gets only favorites files or sites with Rest API and status code"
                            + " is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify User gets only favorites folders or sites with Rest API and status code"
                            + " is 200")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    ...
    public void userIsAbleToRetrieveAllFavorites() throws Exception {
        ...
        userFavorites
                ...
                .entriesListDoesNotContain(
                        "targetGuid", secondFileModel.getNodeRefWithoutVersion());
    ...
    public void getFavoritesUsingInvalidWhereParameter() throws Exception {
        ...
        userFavorites = restClient.withCoreAPI().usingAuthUser().where().or().getFavorites();
        restClient
                .assertStatusCodeIs(HttpStatus.BAD_REQUEST)
                .assertLastError()
                .containsSummary(String.format(RestErrorModel.INVALID_ARGUMENT, "WHERE query"));
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify get favorites specifying -me- string in place of <personid> for"
                            + " request")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void userIsAbleToGetFavoritesWhenUsingMeAsUsername() throws Exception {
        userFavorites =
                restClient.authenticateUser(userModel).withCoreAPI().usingMe().getFavorites();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        userFavorites
                .assertThat()
                .entriesListContains("targetGuid", firstSiteModel.getGuid())
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that for invalid skipCount parameter status code returned is 400.")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void checkInvalidSkipCountStatusCode() throws Exception {
        restClient
                .authenticateUser(adminUserModel)
                .withParams("skipCount=AB")
                .withCoreAPI()
                .usingUser(adminUserModel)
                .getFavorites();
        restClient
    ...
    public void getFavoritesWhenNetworkIdIsInvalid() throws Exception {
        ...
        restClient
                ...
                .containsSummary(
                        String.format(
                                RestErrorModel.ENTITY_NOT_FOUND, networkUserModel.getUsername()));
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify get favorites using AND instead of OR in where parameter for request")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify get favorites using wrong name instead of EXISTS in where parameter for"
                            + " request")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void userIsNotAbleToGetFavoritesWhenUsingWrongWhereParameter() throws Exception {
        userFavorites =
                restClient
                        ...
                        .usingAuthUser()
                        .where()
                        .invalidWhereParameter("EXIST((target/site))")
                        .targetFileExist()
                        .getFavorites();
        restClient
                .assertStatusCodeIs(HttpStatus.BAD_REQUEST)
    ...
    public void userIsAbleToGetFavoritesExceptTheFirstOne() throws Exception {
        ...
        userFavorites
                ...
                .entriesListContains("targetGuid", secondSiteModel.getGuid())
                .and()
                .entriesListDoesNotContain("targetGuid", thirdSiteModel.getGuid());
    }
    ...
    public void userIsAbleToGetFirstTwoFavorites() throws Exception {
        ...
        userFavorites
                .assertThat()
                .entriesListContains("targetGuid", thirdSiteModel.getGuid())
                .and()
                .entriesListContains("targetGuid", secondSiteModel.getGuid())
                .and()
                .entriesListDoesNotContain("targetGuid", firstSiteModel.getGuid())
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify get favorites sites when using empty values for skipCount and maxItems")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void userIsAbleToGetFavoritesWhenSkipCountAndMaxItemsAreEmpty() throws Exception {
        restClient
                .authenticateUser(userModel)
                .withParams("skipCount= ")
                .withCoreAPI()
                .usingUser(userModel)
                .getFavorites();
        restClient
                .assertStatusCodeIs(HttpStatus.BAD_REQUEST)
                .assertLastError()
                .containsSummary(String.format(RestErrorModel.INVALID_SKIPCOUNT, " "));
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify the get favorites request for a high value for skipCount parameter")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void userIsAbleToGetFavoritesWithHighSkipCount() throws Exception {
        userFavorites =
                restClient
                        .authenticateUser(userModel)
                        .withParams("skipCount=999999999")
                        .withCoreAPI()
                        .usingUser(userModel)
                        .getFavorites();
        restClient.assertStatusCodeIs(HttpStatus.OK);
    ...
    public void userIsAbleToGetFavoritesWithPropertiesParamApplied() throws Exception {
        userFavorites =
                restClient
                        .authenticateUser(userModel)
                        .withParams("properties=targetGuid")
                        .withCoreAPI()
                        .usingUser(userModel)
                        .getFavorites();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        ...
        RestPersonFavoritesModel restPersonFavoritesModel =
                userFavorites.getEntries().get(0).onModel();
        restPersonFavoritesModel
                .assertThat()
                .field("targetGuid")
                .is(thirdSiteModel.getGuid())
                .and()
                .field("createdAt")
                .isNull();
    }
    ...
    @TestRail(
            section = {TestGroup.REST_API, TestGroup.FAVORITES},
            executionType = ExecutionType.REGRESSION,
            description = "Verify entry details for get favorites response with Rest API")
    public void checkResponseSchemaForGetFavorites() throws Exception {
        userFavorites =
                restClient.authenticateUser(userModel).withCoreAPI().usingAuthUser().getFavorites();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        ...
        restSiteModel
                .assertThat()
                .field("visibility")
                .is(thirdSiteModel.getVisibility())
                .and()
                .field("guid")
                .is(thirdSiteModel.getGuid())
                .and()
                .field("description")
                .is(thirdSiteModel.getDescription())
                .and()
                .field("id")
                .is(thirdSiteModel.getId())
                .and()
                .field("title")
                .is(thirdSiteModel.getTitle());
    }
}

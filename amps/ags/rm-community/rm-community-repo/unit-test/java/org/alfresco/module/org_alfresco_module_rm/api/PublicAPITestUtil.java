...
public class PublicAPITestUtil {
    ...
    /**
     * Check the consistency of the public API exposed from the given package. For each class in the
     * package that is annotated {@link AlfrescoPublicApi}, check that no exposed methods (or
     * fields, constructors, etc.) use non-public-API classes from Alfresco.
     *
     * @param basePackageName The package to check classes within.
     * @param knownBadReferences Any references that would cause this test to fail, but which we
     *     don't want to change. The keys should be public API classes within our code and the
     *     values should be the non-public-API class that is being referenced.
     */
    public static void testPublicAPIConsistency(
            String basePackageName, SetMultimap<Class<?>, Class<?>> knownBadReferences) {
        Reflections reflections = new Reflections(basePackageName);
        Set<Class<?>> publicAPIClasses =
                reflections.getTypesAnnotatedWith(AlfrescoPublicApi.class, true);
        ...
        for (Class<?> publicAPIClass : publicAPIClasses) {
            Set<Class<?>> referencedClassesFromClass =
                    getReferencedClassesFromClass(publicAPIClass, new HashSet<>());
            referencedClassesFromClass.forEach(clazz -> referencedFrom.put(clazz, publicAPIClass));

            // Remove any references in knownBadReferences and error if an expected reference wasn't
            // found.
            if (knownBadReferences.containsKey(publicAPIClass)) {
                for (Class<?> clazz : knownBadReferences.get(publicAPIClass)) {
                    assertTrue(
                            "Supplied knownBadReferences expects "
                                    + clazz
                                    + " to be referenced by "
                                    + publicAPIClass
        ...
        for (Class<?> referencedClass : referencedClasses) {
            if (isInAlfresco(referencedClass) && !isPartOfPublicApi(referencedClass)) {
                Set<String> referencerNames =
                        referencedFrom.get(referencedClass).stream()
                                .map(c -> c.getName())
                                .collect(Collectors.toSet());
                errorMessages.add(
                        referencedClass.getName()
                                + " <- "
                                + StringUtils.join(referencerNames, ", "));
            }
        }
        ...
        assertEquals(
                "Found references to non-public API classes from public API classes.",
    ...
    /**
     * Get all the classes referenced by the given class, which might be used by an extension. We
     * consider visible methods, constructors, fields and inner classes, as well as superclasses and
     * interfaces extended by the class.
     *
     * @param initialClass The class to analyse.
     * @param consideredClasses Classes that have already been considered, and which should not be
     *     considered again. If the given class has already been considered then an empty set will
     *     be returned. This set will be updated with the given class.
     * @return The set of classes that might be accessible by an extension of this class.
     */
    private static Set<Class<?>> getReferencedClassesFromClass(
            Class<?> initialClass, Set<Class<?>> consideredClasses) {
        Set<Class<?>> referencedClasses = new HashSet<>();
            ...
            for (Class<?> clazz : initialClass.getDeclaredClasses()) {
                if (isVisibleToExtender(clazz.getModifiers())) {
                    referencedClasses.addAll(
                            getReferencedClassesFromClass(clazz, consideredClasses));
                }
            }
            if (initialClass.getSuperclass() != null) {
                referencedClasses.addAll(
                        getReferencedClassesFromClass(
                                initialClass.getSuperclass(), consideredClasses));
            }
     ...
     * can access the element. Here we assume that an extension can see public and protected items,
     * but not package protected (or private).
     *
    ...
    /**
     * Get all classes involved in the type of the supplied field. For example {@code
     * Pair<Set<String>, Integer> foo} involves four classes.
     *
    ...
    /**
     * Get all types references by the supplied method signature (i.e. the parameters, return type
     * and exceptions).
     *
     * @param method The method to analyse.
    ...
    /**
     * Find all classes that are within the supplied types. For example a {@code Pair<Set<String>,
     * Integer>} contains references to four classes.
     *
    ...
    /**
     * Find all classes that are within the supplied type. For example a {@code Pair<Set<String>,
     * Integer>} contains references to four classes.
     *
     * @param type The type to examine.
     * @param processedTypes The set of types which have already been processed. If {@code type} is
     *     within this set then the method returns an empty set, to prevent analysis of the same
     *     type multiple times, and to guard against circular references. The underlying set is
     *     updated with the given type.
    ...
    private static Set<Class<?>> getClassesFromType(Type type, Set<Type> processedTypes) {
        ...
        if (processedTypes.add(type)) {
            if (type instanceof ParameterizedType) {
                ...
                returnClasses.addAll(
                        getClassesFromType(
                                genericArrayType.getGenericComponentType(), processedTypes));
            } else {
                throw new IllegalStateException(
                        "This test was not written to work with type " + type);
            }
        }
    ...
    /**
     * Check if a class is within org.alfresco, and so whether it could potentially be part of the
     * public API.
     *

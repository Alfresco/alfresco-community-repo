...
public class NodeVersionsApiTest extends AbstractSingleNetworkSiteTest {
  ...
  public void testDeleteVersion() throws Exception {
    ...
    try {
      ...
      int cnt = 4;
      Pair<String, String> pair = uploadTextFileVersions(
        user1,
        f1Id,
        contentName,
        cnt,
      ...
      response =
        put(URL_NODES, docId, toJsonAsStringNonNull(nUpdate), null, 200);
      nodeResp =
        RestApiUtil.parseRestApiEntry(response.getJsonResponse(), Node.class);
  ...
  private Pair<String, String> uploadTextFileVersions(
    ...
    for (int i = 1; i <= cnt; i++) {
      boolean expectedMajorVersion =
        (majorVersionIn != null ? majorVersionIn : false);
  ...
  public void testUploadFileVersionUpdate() throws Exception {
    ...
    try {
      ...
      assertEquals(
        versionId,
        nodes.get(0).getProperties().get("cm:versionLabel")
      );
  ...
  public void testRevert() throws Exception {
    ...
    try {
      f1Id =
        createFolder(
          sharedFolderNodeId,
          "testRevert-f1-" + System.currentTimeMillis()
        )
          .getId();
      ...
      for (int i = 1; i <= updateCnt; i++) {
        ...
        params = new HashMap<>();
        params.put(
          Nodes.PARAM_VERSION_COMMENT,
          updateVerCommentSuffix + verCnt
        );
  ...
  private void checkVersionHistoryAndContent(
    ...
    for (Node versionNode : nodesWithProps) {
      ...
      assertEquals(
        versionId,
        versionNode.getProperties().get("cm:versionLabel")
      );

      if (versionId.endsWith(".0")) {
        assertEquals(
          "MAJOR",
          versionNode.getProperties().get("cm:versionType")
        );
      } else {
        assertEquals(
          "MINOR",
  ...
  public void testCreateEmptyFileVersionUpdate() throws Exception {
    ...
    try {
...

      {
        ...
        n.setNodeType(TYPE_CM_CONTENT);
        response =
          post(getNodeChildrenUrl(f1Id), toJsonAsStringNonNull(n), 201);
        String z1Id = RestApiUtil
          .parseRestApiEntry(response.getJsonResponse(), Node.class)
          .getId();

...
/**
 * This factory provides component redirection based on Store or Node References passed into the
 * component.
 *
 * <p>Redirection is driven by StoreRef and NodeRef parameters. If none are given in the method
 * call, the default component is called. Otherwise, the store type is extracted from these
 * parameters and the appropriate component called for the store type.
 *
 * <p>An error is thrown if multiple store types are found.
 *
...
public class StoreRedirectorProxyFactory<I> implements FactoryBean, InitializingBean {
     ...
     *
     * @param defaultBinding the component to call by default
     */
     ...
     *
     * @param protocolBindings the bindings
     */
     ...
     * Sets the binding of store type (protocol string) to component
     *
     * @param storeBindings the bindings
     */
    public void setRedirectedStoreBindings(Map<String, I> storeBindings) {

        Map<StoreRef, I> redirectedStoreBindingsMap =
                new HashMap<StoreRef, I>(storeBindings.size());
    ...
    public void afterPropertiesSet() throws ServiceException {
        ...
        this.redirectorProxy =
                (I)
                        Proxy.newProxyInstance(
                                proxyInterface.getClassLoader(),
                                new Class[] {proxyInterface, StoreRedirector.class},
                                new RedirectorInvocationHandler());
    }
    ...
    /* package */ class RedirectorInvocationHandler implements InvocationHandler, StoreRedirector {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            ...
            if (storeRef == null) {
                ...
                if (StoreRedirectorProxyFactory.this.redirectedStoreBindings != null) {
                    binding =
                            StoreRedirectorProxyFactory.this.redirectedStoreBindings.get(storeRef);
                }
                if ((binding == null)
                        && (StoreRedirectorProxyFactory.this.redirectedProtocolBindings != null)) {
                    binding =
                            StoreRedirectorProxyFactory.this.redirectedProtocolBindings.get(
        ...
        private StoreRef getStoreRef(Class[] argTypes, Object[] args) {
            ...
            for (int i = 0; i < argTypes.length; i++) {
                ...
                if (argStoreRef != null) {
                    ...
                    //                    {
                    //                        throw new ServiceException("Multiple store types are
                    // not supported - types " + storeRef + " and " + argStoreRef + " passed");
                    //                    }
        ...
        public Collection<String> getSupportedStoreProtocols() {
            return Collections.unmodifiableCollection(
                    StoreRedirectorProxyFactory.this.redirectedProtocolBindings.keySet());
        }
        ...
        public Collection<StoreRef> getSupportedStores() {
            return Collections.unmodifiableCollection(
                    StoreRedirectorProxyFactory.this.redirectedStoreBindings.keySet());
        }
    }
}

...
public class RecordFolderType
  ...
  /** I18N */
  private static final String MSG_CANNOT_CREATE_RECORD_FOLDER_CHILD =
    "rm.action.create.record.folder.child-error-message";

  private static final String MSG_CANNOT_CREATE_CHILDREN_IN_CLOSED_RECORD_FOLDER =
    "rm.service.add-children-to-closed-record-folder";
  ...
  public void onMoveNode(
    ...
    if (
      !nodeService
        .getType(newChildAssocRef.getParentRef())
        .equals(TYPE_RECORD_FOLDER)
    ) {
      if (
        !oldChildAssocRef.getParentRef().equals(newChildAssocRef.getParentRef())
      ) {
        reinitializeRecordFolder(newChildAssocRef);
      }
    } else {
      throw new UnsupportedOperationException(
        "Cannot move record folder into another record folder."
      );
    }
  }
  ...
  public CopyBehaviourCallback getCopyCallback(
    ...
    return new DefaultCopyBehaviourCallback() {
      ...
      public Map<QName, Serializable> getCopyProperties(
        ...
        Map<QName, Serializable> sourceProperties = super.getCopyProperties(
          classRef,
          copyDetails,
          properties
        );
      ...
      public boolean getMustCopy(QName classQName, CopyDetails copyDetails) {
        boolean targetParentIsRecordFolder = nodeService
          .getType(copyDetails.getTargetParentNodeRef())
          .equals(TYPE_RECORD_FOLDER);
        boolean containsUnwantedAspect = ArrayUtils.contains(
          unwantedAspects,
          classQName
        );
  ...
  public void onCreateChildAssociation(
    ...
    if (nodeService.exists(nodeRef)) {
      boolean notFolderOrRmFolderSubType =
        !instanceOf(nodeRef, ContentModel.TYPE_FOLDER) ||
        instanceOf(
          nodeRef,
          RecordsManagementModel.TYPE_RECORDS_MANAGEMENT_CONTAINER
        ) ||
      ...
      Boolean isClosed = (Boolean) nodeService.getProperty(
        recordFolder,
        PROP_IS_CLOSED
      );
  ...
  public void onCreateChildAssociationOnCommit(
    ...
    if (
      instanceOf(child, ContentModel.TYPE_FOLDER) &&

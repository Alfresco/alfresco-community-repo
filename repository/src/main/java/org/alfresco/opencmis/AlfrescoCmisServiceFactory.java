...
public class AlfrescoCmisServiceFactory extends AbstractServiceFactory {
  ...
  private static final Log logger = LogFactory.getLog(
    AlfrescoCmisServiceFactory.class
  );
   ...
   */
  public void setCmisTransactions(
    RetryingTransactionInterceptor cmisTransactions
  ) {
   ...
   */
  public void setCmisExceptions(
    AlfrescoCmisExceptionInterceptor cmisExceptions
  ) {
  ...
  public void setCmisCreateDocRequestRenditionsSet(
    String cmisCreateDocRequestRenditionsSet
  ) {
  ...
  public void init() {
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
        "init: cmis.create.doc.request.renditions.set=" +
        cmisCreateDocRequestRenditionsSet
      );
    }
  }
  ...
  public CmisService getService(final CallContext context) {
    if (logger.isDebugEnabled()) {
      ...
      sb
        ...
        .append(",ctxRepoId=")
        .append(context.getRepositoryId())
        .append("]");
    ...
    if (
      ...
      authorityService.isGuestAuthority(
    ...
    if (service instanceof AlfrescoCmisServiceImpl) {
      Set<String> stringSet = parseCommaSeparatedSet(
        getCmisCreateDocRequestRenditionsSet()
      );
      ((AlfrescoCmisServiceImpl) service).setCmisRequestRenditionsOnCreateDoc(
          stringSet
        );

      if (logger.isTraceEnabled()) {
        logger.trace(
          "getService: cmis.create.doc.request.renditions.set=" + stringSet
        );
      }
    }
    ...
    ConformanceCmisServiceWrapper wrapperService = new ConformanceCmisServiceWrapper(
      cmisService,
      connector.getTypesDefaultMaxItems(),
      connector.getTypesDefaultDepth(),
      connector.getObjectsDefaultMaxItems(),
      connector.getObjectsDefaultDepth()
    );

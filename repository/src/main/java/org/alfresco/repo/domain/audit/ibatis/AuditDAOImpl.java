...
public class AuditDAOImpl extends AbstractAuditDAOImpl {
  ...
  private static final String SELECT_MODEL_BY_CRC =
    "alfresco.audit.select_AuditModelByCrc";
  private static final String INSERT_MODEL =
    "alfresco.audit.insert.insert_AuditModel";

  private static final String SELECT_APPLICATION_BY_ID =
    "alfresco.audit.select_AuditApplicationById";
  private static final String SELECT_APPLICATION_BY_NAME_ID =
    "alfresco.audit.select_AuditApplicationByNameId";
  private static final String INSERT_APPLICATION =
    "alfresco.audit.insert.insert_AuditApplication";
  private static final String UPDATE_APPLICATION =
    "alfresco.audit.update_AuditApplication";

  private static final String DELETE_ENTRIES =
    "alfresco.audit.delete_AuditEntries";
  private static final String DELETE_ENTRIES_BY_ID =
    "alfresco.audit.delete_AuditEntriesById";
  private static final String INSERT_ENTRY =
    "alfresco.audit.insert.insert_AuditEntry";
  private static final String SELECT_MINMAX_ENTRY_FOR_APP =
    "alfresco.audit.select_MinMaxAuditEntryId";
  private static final String SELECT_COUNT_ENTRIES_FOR_APP =
    "alfresco.audit.select_CountAuditEntryId";

  @SuppressWarnings("unused")
  private static final String SELECT_ENTRIES_SIMPLE =
    "alfresco.audit.select_AuditEntriesSimple";

  private static final String SELECT_ENTRIES_WITH_VALUES =
    "alfresco.audit.select_AuditEntriesWithValues";
  private static final String SELECT_ENTRIES_WITHOUT_VALUES =
    "alfresco.audit.select_AuditEntriesWithoutValues";
  ...
  protected AuditModelEntity getAuditModelByCrc(long crc) {
    ...
    entity = template.selectOne(SELECT_MODEL_BY_CRC, entity);
    // Done
    return entity;
  }
  ...
  protected AuditApplicationEntity getAuditApplicationById(Long id) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Searched for audit application ID " + id + " and found: " + entity
      );
    }
    return entity;
  }
  ...
  protected AuditApplicationEntity getAuditApplicationByName(String appName) {
    ...
    Pair<Long, Serializable> appNamePair = propertyValueDAO.getPropertyValue(
      appName
    );
    if (appNamePair == null) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Searched for audit application '" + appName + "' and found: " + entity
      );
    }
    return entity;
  }
  ...
  @Override
  protected AuditApplicationEntity updateAuditApplication(
    AuditApplicationEntity entity
  ) {
    ...
    if (updated != 1) {
      ...
      throw new ConcurrencyFailureException(
        "Incorrect number of rows affected for updateAuditApplication: " +
        updateEntity +
        ": expected 1, actual " +
        updated
      );
  ...
  public int deleteAuditEntriesByIdRange(
    Long applicationId,
    Long fromId,
    Long toId
  ) {
  ...
  @Override
  protected AuditEntryEntity createAuditEntry(
    Long applicationId,
    long time,
    Long usernameId,
    Long valuesId
  ...
  public HashMap<String, Long> getAuditMinMaxByApp(
    long appId,
    List<String> extremes
  ) {
    // Build parameters to be used in the query. Filter the duplicates when inserting into map
    Map<String, Object> params = extremes
      .stream()
      .collect(Collectors.toMap(s -> s, s -> Boolean.TRUE, (s1, s2) -> s1));
    params.put("auditAppId", appId);

    HashMap<String, Long> result = template.selectOne(
      SELECT_MINMAX_ENTRY_FOR_APP,
      params
    );
  ...
  protected void findAuditEntries(
    ...
    if (user != null) {
      // Look up the application's ID (this is unique)
      Pair<Long, Serializable> userPair = propertyValueDAO.getPropertyValue(
        user
      );
      if (userPair == null) {
        // No such value
    ...
    if (searchKey != null) {
      ...
      Pair<Long, Serializable> searchKeyPair = propertyValueDAO.getPropertyValue(
        searchKey
      );
    ...
    if (searchValue != null) {
      // Look up the ID of the search key
      Pair<Long, Serializable> searchValuePair = propertyValueDAO.getPropertyValue(
        searchValue
      );
    ...
    if (maxResults > 0) {
      ...
      // values afterwards.
      final Map<Long, AuditQueryResult> resultsByValueId = new HashMap<Long, AuditQueryResult>(
        173
      );
      ...
      List<AuditQueryResult> rows = template.selectList(
        SELECT_ENTRIES_WITHOUT_VALUES,
        params,

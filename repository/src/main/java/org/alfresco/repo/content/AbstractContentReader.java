...
/**
 * Implements all the convenience methods of the interface. The only methods that need to be
 * implemented, i.e. provide low-level content access are:
 *
 * <ul>
 *   <li>{@link #createReader()} to read content from the repository
 *   <li>{@link #getDirectReadableChannel()} to provide direct storage access
 * </ul>
 *
 * @author Derek Hulley
 */
@AlfrescoPublicApi
public abstract class AbstractContentReader extends AbstractContentAccessor
    ...
    @Deprecated private TransformationOptionLimits limits;

    // Only needed if limits are set
    @Deprecated private TransformerDebug transformerDebug;

    // For testing: Allows buffering to be turned off
    private boolean useBufferedInputStream = true;

    /**
     * @param contentUrl the content URL - this should be relative to the root of the store and not
     *     absolute: to enable moving of the stores
     */
    ...
    /**
     * A factory method for subclasses to implement that will ensure the proper implementation of
     * the {@link ContentReader#getReader()} method.
     *
     * <p>Only the instance need be constructed. The required mimetype, encoding, etc will be copied
     * across by this class.
     *
     * @return Returns a reader onto the location referenced by this instance. The instance must
     *     <b>always</b> be a new instance.
    ...
    public final ContentReader getReader() throws ContentIOException {
        ...
        } else if (reader.getContentUrl() == null
                || !reader.getContentUrl().equals(getContentUrl())) {
            throw new AlfrescoRuntimeException(
                    "ContentReader has different URL: \n"
                            + "   reader: "
                            + this
                            + "\n"
                            + "   new reader: "
     ...
     *
     * <p>This is the only of the content <i>reading</i> methods that needs to be implemented by
     * derived classes. All other content access methods make use of this in their underlying
     * implementations.
     *
     * @return Returns a channel from which content can be read
     * @throws ContentIOException if the channel could not be opened or the underlying content has
     *     disappeared
     */
    ...
    private ReadableByteChannel getCallbackReadableChannel(
            ReadableByteChannel directChannel, List<ContentStreamListener> listeners)
            throws ContentIOException {
    ...
    public FileChannel getFileChannel() throws ContentIOException {
        ...
        if (channel instanceof FileChannel) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Content reader provided direct support for FileChannel: \n"
                                + "   reader: "
                                + this);
            }
        } else {
            ...
            ContentStreamListener spoofListener =
                    new ContentStreamListener() {
                        public void contentStreamClosed() throws ContentIOException {
                            try {
                                ...
                                throw new ContentIOException(
                                        "Failed to close underlying channel", e);
                            }
                        }
                    };
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Content writer provided indirect support for FileChannel: \n"
                                + "   writer: "
                                + this
                                + "\n"
                                + "   temp writer: "
        ...
        try {
            ...
            if (limits != null) {
                ...
                if (timeoutMs > 0 || readLimitBytes > 0) {
                    ...
                    is =
                            new TimeSizeRestrictedInputStream(
                                    is,
                                    timeoutMs,
                                    timeoutAction,
                                    readLimitBytes,
    ...
    /**
     * Copies the {@link #getContentInputStream() input stream} to the given <code>OutputStream
     * </code>
     */
     ...
     *
     * <p>All the content is streamed into memory. So, like the interface said, be careful with this
     * method.
     *
    ...
    /**
     * Does a comparison of the binaries associated with two readers. Several shortcuts are assumed
     * to be valid:<br>
     ...
     * Otherwise the binaries are {@link EqualsHelper#binaryStreamEquals(InputStream, InputStream)
     * compared}.
     *
     * @return Returns <tt>true</tt> if the underlying binaries are the same
     * @throws ContentIOException
     */
    public static boolean compareContentReaders(ContentReader left, ContentReader right)
            throws ContentIOException {
        if (left == right) {
            return true;
    ...
    /**
     * InputStream that wraps another InputStream to terminate early after a timeout or after
     * reading a number of bytes. It terminates by either returning end of file (-1) or throwing an
     * IOException.
     *
     * @author Alan Davis
     */
        ...
        private boolean hitTimeout() throws IOException {
            if (timeoutMs > 0 && timeoutFlag.get()) {
                timeoutAction.throwIOExceptionIfRequired(
                        "Transformation has taken too long (" + (timeoutMs / 1000) + " seconds)",
                        transformerDebug);
                return true;
            }
            return false;
        ...
        private boolean hitReadLimit() throws IOException {
            if (readLimitBytes > 0 && readCount >= readLimitBytes) {
                readLimitAction.throwIOExceptionIfRequired(
                        "Transformation has read too many bytes (" + (readLimitBytes / 1024) + "K)",
                        transformerDebug);
                return true;
            }
            return false;

...
public class IdentityServiceRemoteUserMapperTest
  ...
  private static final String DEPLOYMENT_BEAN_NAME =
  ...
  private static final String CONFIG_SILENT_ERRORS =
  ...
  protected void setUp() throws Exception {
    ...
    childApplicationContextManager =
      (DefaultChildApplicationContextManager) ctx.getBean("Authentication");
    childApplicationContextManager.stop();
    childApplicationContextManager.setProperty(
      "chain",
      "identity-service1:identity-service"
    );
    ...
    this.identityServiceConfig =
      (IdentityServiceConfig) childApplicationContextFactory
  ...
  public void testKeycloakConfig() throws Exception {
    ...
    assertEquals(
      "identity-service.cors-allowed-methods",
      "POST, PUT, DELETE, GET",
      this.identityServiceConfig.getCorsAllowedMethods()
    );

    assertEquals(
      "identity-service.cors-exposed-headers",
      "WWW-Authenticate, My-custom-exposed-Header",
    ...
    assertEquals(
      "identity-service.confidential-port",
      100,
      this.identityServiceConfig.getConfidentialPort()
    );

    assertEquals(
      "identity-service.cors-max-age",
      1000,
    ...
    assertEquals(
      "identity-service.register-node-period",
      50,
      this.identityServiceConfig.getRegisterNodePeriod()
    );

    assertEquals(
      "identity-service.token-minimum-time-to-live",
      10,
      this.identityServiceConfig.getTokenMinimumTimeToLive()
    );

    assertEquals(
      "identity-service.min-time-between-jwks-requests",
      60,
    ...
    assertTrue(
      "identity-service.use-resource-role-mappings",
      this.identityServiceConfig.isUseResourceRoleMappings()
    );

    assertTrue(
      "identity-service.enable-cors",
      this.identityServiceConfig.isCors()
    );

    assertTrue(
      "identity-service.expose-token",
      this.identityServiceConfig.isExposeToken()
    );

    assertTrue(
      "identity-service.bearer-only",
      this.identityServiceConfig.isBearerOnly()
    );

    assertTrue(
      "identity-service.autodetect-bearer-only",
    ...
    assertTrue(
      "identity-service.always-refresh-token",
      this.identityServiceConfig.isAlwaysRefreshToken()
    );

    assertTrue(
      "identity-service.register-node-at-startup",
  ...
  public void testValidToken() throws Exception {
    ...
    assertEquals(
      TEST_USER_USERNAME,
      (
        (RemoteUserMapper) childApplicationContextFactory
          .getApplicationContext()
          .getBean(REMOTE_USER_MAPPER_BEAN_NAME)
  ...
  public void testWrongPublicKeyWithError() throws Exception {
    ...
    assertEquals(
      "Returned user should be null when wrong public key is used.",
      null,
      user
    );
  }
  ...
  public void testExpiredTokenWithError() throws Exception {
    ...
    assertEquals(
      "Returned user should be null when the token is expired.",
      null,
      user
    );
  }
  ...
  private HttpServletRequest createMockBasicRequest() {
    ...
    when(mockRequest.getHeaders(AUTHORIZATION_HEADER))
      .thenReturn(authHeaderValues.elements(), authHeaderValues.elements());
  ...
  private HttpClient createMockHttpClient() throws Exception {
    ...
    ByteArrayInputStream jsonResponseStream = new ByteArrayInputStream(
  ...
  private void applyHardcodedPublicKey(PublicKey publicKey) {
    KeycloakDeployment deployment = (KeycloakDeployment) childApplicationContextFactory
      .getApplicationContext()
      .getBean(DEPLOYMENT_BEAN_NAME);
    HardcodedPublicKeyLocator publicKeyLocator = new HardcodedPublicKeyLocator(
      publicKey
    );

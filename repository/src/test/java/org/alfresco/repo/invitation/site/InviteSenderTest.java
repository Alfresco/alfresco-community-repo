...
public class InviteSenderTest extends TestCase {
    ...
    private static final Person whitespaceInvitee =
            new Person("First Second\tthird\nFourth\r\nFifth");
    ...
    protected void testSendMailWorkingPath(
            String emailTemplateXpath, String emailSubjectKey, boolean requireAcceptance)
            throws Exception {
        ...
        verify(mailAction)
                .setParameterValue(eq(MailActionExecuter.PARAM_SUBJECT), eq(subjectPropertyName));
        verify(mailAction)
                .setParameterValue(
                        eq(MailActionExecuter.PARAM_SUBJECT_PARAMS),
                        argThat(
        ...
        verify(mailAction)
                .setParameterValue(eq(MailActionExecuter.PARAM_TEMPLATE), (Serializable) any());

        ArgumentCaptor<Map> modelC = ArgumentCaptor.forClass(Map.class);
        verify(mailAction)
                .setParameterValue(
                        eq(MailActionExecuter.PARAM_TEMPLATE_MODEL),
        ...
        if (requireAcceptance) {
            ...
            assertEquals(
                    "test://test/path/accpet?inviteId=InstanceId&inviteeUserName=invitee&siteShortName=Full"
                        + " Site Name&inviteTicket=Ticket",
                    argsMap.get("acceptLink"));
            assertEquals(
                    "test://test/path/reject?inviteId=InstanceId&inviteeUserName=invitee&siteShortName=Full"
                        + " Site Name&inviteTicket=Ticket",
    ...
    protected void testSendMailWithWhitespaceUserName(
        ...
        if (requireAcceptance) {
            ...
            assertEquals(
                    "test://test/path/accpet?inviteId=InstanceId&inviteeUserName=First%20Second%09third%0aFourth%0d%0aFifth&siteShortName=Full"
                        + " Site Name&inviteTicket=Ticket",
                    acceptLink);
            String rejectLink = argsMap.get("rejectLink");
            assertEquals(
                    "test://test/path/reject?inviteId=InstanceId&inviteeUserName=First%20Second%09third%0aFourth%0d%0aFifth&siteShortName=Full"
                        + " Site Name&inviteTicket=Ticket",
                    rejectLink);
        }
    }
    ...
    protected void testSendMailWithSpecialCharUserName(
        ...
        if (requireAcceptance) {
            ...
            String acceptLink = argsMap.get("acceptLink");
            assertEquals(
                    "test://test/path/accpet?inviteId=InstanceId&inviteeUserName=%c3%a0%c3%a2%c3%a6%c3%a7%c3%a9%c3%a8%c3%aa%c3%ab%c3%ae%c3%af%c3%b4%c5%93%c3%b9%c3%bb%c3%bc%c3%bf%c3%b1&siteShortName=Full"
                        + " Site Name&inviteTicket=Ticket",
                    acceptLink);
            String rejectLink = argsMap.get("rejectLink");
            assertEquals(
                    "test://test/path/reject?inviteId=InstanceId&inviteeUserName=%c3%a0%c3%a2%c3%a6%c3%a7%c3%a9%c3%a8%c3%aa%c3%ab%c3%ae%c3%af%c3%b4%c5%93%c3%b9%c3%bb%c3%bc%c3%bf%c3%b1&siteShortName=Full"
                        + " Site Name&inviteTicket=Ticket",
                    rejectLink);
        }
    }
    ...
    private FileFolderService mockFileFolderService() {
        ...
        when(fileFolderService.getLocalizedSibling((NodeRef) null))
                .thenAnswer(
                        new Answer<NodeRef>() {
    ...
    private SearchService mockSearchService() {
        ...
        when(searchService.selectNodes(any(), any(String.class), any(), any(), eq(false)))
                .thenReturn(nodeRefs);
        return searchService;
    }
    ...
    private NodeService mockNodeService() {
        ...
        when(nodeService.getProperty(inviter.node, ContentModel.PROP_EMAIL))
                .thenReturn(inviter.email);
        when(nodeService.getProperty(invitee.node, ContentModel.PROP_EMAIL))
                .thenReturn(invitee.email);
        when(nodeService.getProperty(whitespaceInvitee.node, ContentModel.PROP_EMAIL))
                .thenReturn(whitespaceInvitee.email);
        when(nodeService.getProperty(specialCharInvitee.node, ContentModel.PROP_EMAIL))
                .thenReturn(specialCharInvitee.email);
        return nodeService;
    }

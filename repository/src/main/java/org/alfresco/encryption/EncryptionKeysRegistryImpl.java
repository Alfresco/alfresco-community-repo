...
public class EncryptionKeysRegistryImpl implements EncryptionKeysRegistry {
  ...
  private static final Log logger = LogFactory.getLog(
    EncryptionKeysRegistryImpl.class
  );
  ...
  public void registerKey(String keyAlias, Key key) {
    if (isKeyRegistered(keyAlias)) {
      throw new IllegalArgumentException(
        "Key " + keyAlias + " is already registered"
      );
    }
    ...
    Pair<String, Serializable> keyCheck = new Pair<String, Serializable>(
      guid,
      encrypted
    );
  ...
  public KEY_STATUS checkKey(String keyAlias, Key key) {
    ...
    if (attributeService.exists(TOP_LEVEL_KEY, keyAlias)) {
      try {
        ...
        try {
          keyCheck =
            (Pair<String, Serializable>) attributeService.getAttribute(
              TOP_LEVEL_KEY,
              keyAlias
            );
        } catch (Throwable e) {
        ...
        keys.setKey(keyAlias, key);
        Encryptor encryptor = getEncryptor(keys);
        Serializable storedGUID = encryptor.unsealObject(
          keyAlias,
          keyCheck.getSecond()
        );

...
/**
 * A factory to create HttpClients and AlfrescoHttpClients based on the setting of the
 * 'secureCommsType' property.
 *
 * @since 4.0
 */
public class HttpClientFactory {
    /**
     * Communication type for HttpClient: - NONE is plain http - SECRET is plain http with a shared
     * secret via request header - HTTPS is mTLS with client authentication (certificates are
     * required)
     */
    ...
    public HttpClientFactory(
            SecureCommsType secureCommsType,
    ...
    /**
     * Recommended constructor for subsystems supporting Shared Secret communication. This
     * constructor supports Shared Secret ("secret") communication method additionally to the legacy
     * ones: "none" and "https".
     */
    public HttpClientFactory(
            SecureCommsType secureCommsType,
    ...
    public void init() {
        this.sslKeyStore =
                new AlfrescoKeyStoreImpl(
                        sslEncryptionParameters.getKeyStoreParameters(), keyResourceLoader);
        this.sslTrustStore =
                new AlfrescoKeyStoreImpl(
                        sslEncryptionParameters.getTrustStoreParameters(), keyResourceLoader);
        this.sslSocketFactory =
                new AuthSSLProtocolSocketFactory(sslKeyStore, sslTrustStore, keyResourceLoader);
    ...
    /**
     * Sets the default socket timeout (<tt>SO_TIMEOUT</tt>) in milliseconds which is the timeout
     * for waiting for data. A timeout value of zero is interpreted as an infinite timeout.
     *
    ...
    /**
     * Attempts to connect to a server will timeout after this period (millis). Default is zero (the
     * timeout is not used).
     *
    ...
    protected RequestHeadersHttpClient getHttpsClient(String httpsHost, int httpsPort) {
        ...
        // Default port is 443 for the HostFactory, when including customised port (like 8983) the
        // port name is skipped from "getHostURL" string
        HttpHostFactory hostFactory =
                new HttpHostFactory(new Protocol("https", sslSocketFactory, HttpsURL.DEFAULT_PORT));
    ...
    public AlfrescoHttpClient getRepoClient(String host, int port) {
        switch (secureCommsType) {
            case HTTPS:
                return getAlfrescoHttpsClient();
            case NONE:
                return getAlfrescoHttpClient();
            case SECRET:
                return getAlfrescoSharedSecretClient();
            default:
                throw new AlfrescoRuntimeException(
                        "Invalid Solr secure communications type configured in"
                            + " [solr|alfresco].secureComms, should be 'ssl', 'none' or 'secret'");
        }
    }
    ...
    public RequestHeadersHttpClient getHttpClient() {
        switch (secureCommsType) {
            ...
            case SECRET:
                return getSharedSecretHttpClient();
            default:
                throw new AlfrescoRuntimeException(
                        "Invalid Solr secure communications type configured in"
                            + " [solr|alfresco].secureComms, should be 'ssl', 'none' or 'secret'");
    ...
    static class SecureHttpMethodResponse extends HttpMethodResponse {
        ...
        // Need to get as a byte array because we need to read the request twice, once for
        // authentication
        // and again by the web service.
        protected byte[] decryptedBody;

        public SecureHttpMethodResponse(
                HttpMethod method, HostConfiguration hostConfig, EncryptionUtils encryptionUtils)
        ...
        protected boolean authenticate() throws IOException {
            return encryptionUtils.authenticateResponse(
                    method, hostConfig.getHost(), decryptedBody);
        }
    ...
    /**
     * An extension of the DefaultHttpParamsFactory that uses a RRW lock pattern rather than full
     * synchronization around the parameter CRUD - to avoid locking on many reads.
     *
     * @author Kevin Roast
     */
    public static class NonBlockingHttpParamsFactory extends DefaultHttpParamsFactory {
        ...
        /**
         * NOTE: This is a copy of the code in {@link DefaultHttpParamsFactory} Unfortunately this
         * is required because although the factory pattern allows the override of the default param
         * creation, it does not allow the class of the actual HttpParam implementation to be
         * changed.
         */
        @Override
        protected HttpParams createParams() {
            ...
            params.setParameter(
                    HttpMethodParams.USER_AGENT, "Spring Surf via Apache HttpClient/3.1");

...
public class RMRolesTests extends BaseRMRestTest {
    ...
    private static final java.util.HashSet<String> CAPABILITIES = newHashSet(
        UserCapabilities.VIEW_RECORDS_CAP,
        UserCapabilities.DECLARE_RECORDS_CAP
    );
    ...
    public void checkRMRolesExist() {
        Set<String> configuredRoles = rmRolesAndActionsAPI.getConfiguredRoles(
            getAdminUser().getUsername(),
            getAdminUser().getPassword()
        );
        RM_ROLES.forEach(role ->
            assertTrue(
                "Could not found role " + role,
                configuredRoles.contains(role)
            )
        );
    }
    ...
    public void checkCapabilitiesForUser() {
        Set<String> capabilities = rmRolesAndActionsAPI.getCapabilitiesForRole(
            getAdminUser().getUsername(),
            getAdminUser().getPassword(),
            ROLE_RM_USER.roleId
        );
        assertEquals(
            "Unexpected capabilities found for RM User.",
            capabilities,
            CAPABILITIES
        );
    }
    ...
    public void createNewRole() {
        ...
        Set<String> actualCapabilities = rmRolesAndActionsAPI.getCapabilitiesForRole(
            getAdminUser().getUsername(),
            getAdminUser().getPassword(),
            roleName
        );

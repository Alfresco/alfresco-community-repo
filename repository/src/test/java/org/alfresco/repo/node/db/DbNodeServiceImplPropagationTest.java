...
public class DbNodeServiceImplPropagationTest extends BaseSpringTest {
  ...
  public void testAuditablePropagation() throws Exception {
    ...
    assertFalse(
      "Shouldn't be auditable in " + nodeService.getAspects(n2Ref),
    ...
    NodeRef naC = nodeService
      .createNode(
        n2Ref,
        ASSOC_NOT_AUDITABLE,
        QName.createQName("not-auditable"),
        TYPE_NOT_AUDITABLE
      )
      .getChildRef();
    ...
    NodeRef adC = nodeService
      .createNode(
        n2Ref,
        ASSOC_NOT_AUDITABLE,
        QName.createQName("is-auditable"),
        TYPE_AUDITABLE
      )
      .getChildRef();
    nodeService.addAspect(adC, ContentModel.ASPECT_AUDITABLE, null);
    logger.debug(
      "Created auditable child " + naC + " of non-auditable parent " + n2Ref
    );
    restartAuditableTxn();
    ...
    assertEquals(
      modified.getTime(),
      (
        (Date) nodeDAO.getNodeProperty(n2Id, ContentModel.PROP_MODIFIED)
      ).getTime()
    );
    assertEquals(
      "TestModifier",
      nodeDAO.getNodeProperty(n2Id, ContentModel.PROP_MODIFIER)
    );

    // Delete the non-auditable child
    // No change to the parent as non-auditable child
    logger.debug(
      "Deleting non-auditable child " + naC + " of auditable parent " + n2Ref
    );
    ...
    restartAuditableTxn();
    modifiedAt =
      (Date) nodeService.getProperty(n2Ref, ContentModel.PROP_MODIFIED);
    assertNotNull(modifiedAt);
    assertEquals(
      (double) new Date().getTime(),
      (double) modifiedAt.getTime(),
      10000d
    );
    assertEquals(
      fullyAuthenticatedUser,
      nodeService.getProperty(n2Ref, ContentModel.PROP_MODIFIER)
    );

    // Set well-known modified details on both nodes
    nodeDAO.setModifiedProperties(
      n2Id,
    ...
    logger.debug(
      "Deleting auditable child " + adC + " of auditable parent " + n2Ref
    );
    ...
    NodeRef ac1 = nodeService
      .createNode(
        n2Ref,
        ASSOC_AUDITABLE,
        QName.createQName("is-auditable-1"),
        TYPE_AUDITABLE
      )
      .getChildRef();
    NodeRef ac2 = nodeService
      .createNode(
        n2Ref,
        ASSOC_AUDITABLE,
        QName.createQName("is-auditable-2"),
        TYPE_AUDITABLE
      )
      .getChildRef();
    ...
    assertEquals(
      fullyAuthenticatedUser,
      nodeService.getProperty(ac1, ContentModel.PROP_MODIFIER)
    );
    assertEquals(
      fullyAuthenticatedUser,
      nodeService.getProperty(ac2, ContentModel.PROP_MODIFIER)
    );
    ...
    NodeRef ac3 = nodeService
      .createNode(
        n2Ref,
        ASSOC_AUDITABLE,
        QName.createQName("is-auditable-3"),
        TYPE_AUDITABLE
      )
      .getChildRef();
    ...
    assertEquals(
      fullyAuthenticatedUser,
      nodeService.getProperty(ac3, ContentModel.PROP_MODIFIER)
    );

    modifiedAt =
      (Date) nodeService.getProperty(n2Ref, ContentModel.PROP_MODIFIED);
    assertEquals(
      (double) future.getTime(),
      (double) modifiedAt.getTime(),
      1000d
    );
    modifiedAt =
      (Date) nodeService.getProperty(ac3, ContentModel.PROP_MODIFIED);
    ...
    NodeRef ac4 = nodeService
      .createNode(
        n2Ref,
        ASSOC_NOT_AUDITABLE,
        QName.createQName("is-auditable-4"),
        TYPE_AUDITABLE
      )
      .getChildRef();
    ...
    assertEquals(
      fullyAuthenticatedUser,
      nodeService.getProperty(ac4, ContentModel.PROP_MODIFIER)
    );
    ...
    modifiedAt =
      (Date) nodeService.getProperty(ac4, ContentModel.PROP_MODIFIED);
    assertEquals(
      (double) new Date().getTime(),
      (double) modifiedAt.getTime(),
      3000d

...
public class CompositePasswordEncoder {
    ...
    public boolean lastEncodingIsPreferred(List<String> hashIndicator) {
        if (hashIndicator != null
                && hashIndicator.size() > 0
                && preferredEncoding.equals(hashIndicator.get(hashIndicator.size() - 1))) {
            return true;
        }
        return false;
    }
    ...
    /**
     * Determines if its safe to encode the encoding chain. This applies particularly to
     * double-hashing. BCRYPT uses its own internal salt so its NOT safe to use it more than once in
     * an encoding chain (because the result will be different each time.) BCRYPT CAN BE USED
     * successfully as the last element in an encoding chain.
     *
     * <p>Anything that implements springframework PasswordEncoder is considered "unsafe" (because
     * the method takes no salt param).
     *
    ...
    public boolean isSafeToEncodeChain(List<String> encodingChain) {
        if (encodingChain != null && encodingChain.size() > 0) {
            ...
            for (String encoderKey : encodingChain) {
                ...
                if (encoder
                        instanceof org.springframework.security.crypto.password.PasswordEncoder) {
                    // BCRYPT uses its own internal salt so its NOT safe to use it more than once in
                    // an encoding chain.
                    // the Spring PasswordEncoder class doesn't require a salt and BCRYPTEncoder
                    // implements this, so
            ...
            if (unsafeEncoders.size() == 1
                    && unsafeEncoders.get(0).equals(encodingChain.get(encodingChain.size() - 1))) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Non-upgradable encoders in the encoding chain: "
                                + Arrays.toString(unsafeEncoders.toArray())
                                + ". Only 1 non-upgradable encoder is allowed at the end of the"
                                + " chain: "
                                + Arrays.toString(encodingChain.toArray()));
            }
        }
        return false;
    }
    ...
    public void init() {
        ...
        if (!encoders.containsKey(preferredEncoding))
            throw new AlfrescoRuntimeException(
                    "Invalid preferredEncoding specified: "
                            + preferredEncoding
                            + ". Permissible encoders are "
                            + encoders.keySet());
    }
    ...
    public String encodePassword(String rawPassword, Object salt, List<String> encodingChain) {
        ...
        if (encoded == rawPassword)
            throw new AlfrescoRuntimeException("No password encoding specified. " + encodingChain);
        return encoded;
    }
    ...
    protected String encode(String encoderKey, String rawPassword, Object salt) {
        ...
        if (encoder instanceof net.sf.acegisecurity.providers.encoding.PasswordEncoder) {
            net.sf.acegisecurity.providers.encoding.PasswordEncoder pEncoder =
                    (net.sf.acegisecurity.providers.encoding.PasswordEncoder) encoder;
        ...
        if (encoder instanceof org.springframework.security.crypto.password.PasswordEncoder) {
            org.springframework.security.crypto.password.PasswordEncoder passEncoder =
    ...
    public boolean matchesPassword(
            String rawPassword, String encodedPassword, Object salt, List<String> encodingChain) {
        ParameterCheck.mandatoryString("rawPassword", rawPassword);
        ...
        if (encodingChain.size() > 1) {
            ...
            String encoded =
                    encodePassword(
                            rawPassword, salt, encodingChain.subList(0, encodingChain.size() - 1));
    ...
    protected boolean matches(
            String encoderKey, String rawPassword, String encodedPassword, Object salt) {
        ParameterCheck.mandatoryString("rawPassword", rawPassword);
        ParameterCheck.mandatoryString("encodedPassword", encodedPassword);
        ParameterCheck.mandatoryString("encoderKey", encoderKey);
        Object encoder = encoders.get(encoderKey);

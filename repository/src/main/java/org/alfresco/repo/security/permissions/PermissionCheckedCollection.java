...
/**
 * Interface for collection-based results that carry extra information about the state of permission
 * cut-offs.
 *
 * @author Derek Hulley
 * @since 4.0
...
public interface PermissionCheckedCollection<T> {
     ...
     *
     * @return <tt>true</tt> - if the results (usually a collection) have been cut off by permission
     *     check limits
     */
    boolean isCutOff();

    /**
     * Get the number of objects in the original (unfiltered) collection that did <b>not</b> have
    ...
    /**
     * Helper 'introduction' to allow simple addition of the {@link PermissionCheckedCollection}
     * interface to existing collections.
     *
    ...
    public static class PermissionCheckedCollectionMixin<T>
            extends DelegatingIntroductionInterceptor implements PermissionCheckedCollection<T> {
        private final boolean isCutOff;
        private final int sizeUnchecked;
        private final int sizeOriginal;

        private PermissionCheckedCollectionMixin(
        ...
        /**
         * Helper method to create a {@link PermissionCheckedCollection} from an existing <code>
         * Collection</code> by applying the same values as present on a potentially
         * permission-checked source. If the existing checked source is <b>NOT</b>
         * permission-checked, then the collection will not be decorated.
         *
         * @param <TT> the type of the <code>Collection</code>
         * @param collection the <code>Collection</code> to proxy
         * @param checkedSource a collection that might implement {@link
         *     PermissionCheckedCollection}
         * @return a <code>Collection</code> of the same type but including the {@link
         *     PermissionCheckedCollection} interface
         */
        public static final <TT> Collection<TT> create(
            ...
            if (checkedSource instanceof PermissionCheckedCollection) {
                PermissionCheckedCollection<?> source =
                        (PermissionCheckedCollection<?>) checkedSource;
                return create(
                        collection,
                        source.isCutOff(),
                        source.sizeUnchecked(),
                        source.sizeOriginal());
            } else {
                return collection;
            }
        ...
        public static final <TT> Collection<TT> create(
                Collection<TT> collection, boolean isCutOff, int sizeUnchecked, int sizeOriginal) {
            // Create the mixin
            DelegatingIntroductionInterceptor mixin =
                    new PermissionCheckedCollectionMixin<Integer>(
                            isCutOff, sizeUnchecked, sizeOriginal);
            // Create the advisor
            IntroductionAdvisor advisor =
                    new DefaultIntroductionAdvisor(mixin, PermissionCheckedCollection.class);
            // Proxy

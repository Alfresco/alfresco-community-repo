...
public class SiteContainersRelation
  implements
    RelationshipResourceAction.Read<SiteContainer>,
    RelationshipResourceAction.ReadById<SiteContainer>,
    InitializingBean {

  private static final Log logger = LogFactory.getLog(
    SiteContainersRelation.class
  );
  ...
  @Override
  @WebApiDescription(
    title = "Site container information for container 'containerId' in site 'siteId'."
  )
  public SiteContainer readById(
    String siteId,
    String containerId,
  ...
  public CollectionWithPagingInfo<SiteContainer> readAll(
    String siteId,
    Parameters parameters
  ) {
    PagingResults<SiteContainer> siteContainers = sites.getSiteContainers(
      siteId,
      parameters.getPaging()
    );
    return CollectionWithPagingInfo.asPaged(
      parameters.getPaging(),
      siteContainers.getPage(),
      siteContainers.hasMoreItems(),
      siteContainers.getTotalResultCount().getFirst()
    );
  }
}

...
public class GetChildrenCannedQuery
  ...
  protected UnsortedChildQueryCallback getUnsortedChildQueryCallback(
    ...
    UnsortedChildQueryCallback callback = new FileFolderUnsortedChildQueryCallback(
      rawResult,
      requestedCount,
      inclusiveAspects,
      exclusiveAspects
  ...
  protected FilterSortChildQueryCallback getFilterSortChildQuery(
    ...
    FilterSortChildQueryCallback callback = new FileFolderFilterSortChildQueryCallback(
      children,
      filterProps
    );
    return callback;
  ...
  private class FileFolderFilterSortChildQueryCallback
    ...
    private Map<QName, Boolean> isTypeFolderMap = new HashMap<QName, Boolean>(
      10
    );

    public FileFolderFilterSortChildQueryCallback(
      List<FilterSortNode> children,
    ...
    public boolean handle(FilterSortNode node) {
      ...
      QName nodeTypeQName = (QName) propVals.get(
  ...
  private class FileFolderUnsortedChildQueryCallback
    extends DefaultUnsortedChildQueryCallback {
    ...
    try {
      ...
      if (ignoreAspectQNames != null) {
        if (ignoreAspectQNames.size() > 1) {
          ...
          if (
            nodeService.hasAspect(nodeRef, ignoreAspectQNames.iterator().next())
          ) {
            notIgnore = false;
          }
        }

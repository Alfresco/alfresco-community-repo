...
public class NetworkImpl implements Network, Serializable, ExpectedComparison {
    ...
    public NetworkImpl(org.alfresco.repo.tenant.Network network) {
        ...
        this.createdAt =
                (network.getCreatedAt() != null
                        ? PublicApiDateFormat.getDateFormat().format(network.getCreatedAt())
                        : null);
    ...
    public static NetworkImpl parseNetwork(JSONObject jsonObject) {
        ...
        List<org.alfresco.rest.api.tests.client.data.Quota> quotas =
                new ArrayList<org.alfresco.rest.api.tests.client.data.Quota>(quotasJSON.size());
        Iterator it = quotasJSON.iterator();
        while (it.hasNext()) {
            JSONObject quotaJSON = (JSONObject) it.next();
            org.alfresco.rest.api.tests.client.data.Quota quota =
                    org.alfresco.rest.api.tests.client.data.Quota.parseQuota(quotaJSON);
            quotas.add(quota);
        }
        NetworkImpl network =
                new NetworkImpl(id, isEnabled, createdAt, quotas, subscriptionLevel, paidNetwork);
        return network;
    }
    ...
    public void expected(Object o) {
        ...
        if (createdAt != null) {
            try {
                ...
                Date otherCreatedAt =
                        PublicApiDateFormat.getDateFormat().parse(other.getCreatedAt());

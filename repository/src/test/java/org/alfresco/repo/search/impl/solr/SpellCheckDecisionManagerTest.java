...
public class SpellCheckDecisionManagerTest {
    ...
    private static final String SEARCH_REQUEST_URL =
            "/solr4/alfresco/afts?wt=json&fl=DBID%2Cscore&rows=251&df=keywords&start=0&locale=en&alternativeDic=DEFAULT_DICTIONARY&fq=%7B%21afts%7DAUTHORITY_FILTER_FROM_JSON&fq=%7B%21afts%7DTENANT_FILTER_FROM_JSON";

    /**
     * Test Collation Solr4. The {@code isCollate} method returns true, when the original query
     * contains one or more misspelled terms leading to 0 hit and if spell-suggestion is available
     * and collation's hits are greater than 0, the class should modify the original search query to
     * replace the misspelled term with the suggested term.
     *
     * @throws Exception
     */
    @Test
    public void testCollation() throws Exception {
        ...
        SpellCheckDecisionManager manager =
                new SpellCheckDecisionManager(
                        resultJson,
                        SEARCH_REQUEST_URL + spellCheckParam,
                        requestBody,
                        spellCheckParam);
        ...
        assertEquals(
                createJsonSearchRequest("alfresco").getString("query"),
                requestBody.getString("query"));

        // Check that the number of found docs (original query) is 0 and the collation hits are
        // greater than 0
    ...
    /**
     * Test Collation Solr6. The {@code isCollate} method returns true, when the original query
     * contains one or more misspelled terms leading to 0 hit and if spell-suggestion is available
     * and collation's hits are greater than 0, the class should modify the original search query to
     * replace the misspelled term with the suggested term.
     *
    ...
    /**
     * Test didYouMean Solr4. If the original query resulting in a few hits; suggestions are
     * available and have more hits, the class should only return the suggested terms, without
     * modifying the query or setting the collation flag to true.
     *
     * @throws Exception
     */
    @Test
    public void testDidYouMean() throws Exception {
        ...
        assertEquals(
                createJsonSearchRequest(searchTerm).getString("query"),
                requestBody.getString("query"));
    ...
    /**
     * Test didYouMean Solr6. If the original query resulting in a few hits; suggestions are
     * available and have more hits, the class should only return the suggested terms, without
     * modifying the query or setting the collation flag to true.
     *
     * @throws Exception
    ...
    /**
     * Test no suggestions. Query contains a term that does not exist in the index and also no
     * suggestions are available. In this case, the class can't do anything.
     *
     * @throws Exception
     */
    @Test
    ...
    /**
     * Solr4 Query contains a correctly spelled term; suggestions are available, but the collation's
     * hits are less than or equal to the original query term hits. In this case, the class should
     * not do anything
     *
     * @throws Exception
     */
    ...
    public void testCorrectlySpelledTerm() throws Exception {
        ...
        // Check that the number of found docs (original query) is greater than or equal to the
        // collation hits
        long numberFound = getOrigQueryHit(resultJson);
        long collationHit = getCollationHit(resultJson);
    ...
    /**
     * Solr6 Query contains a correctly spelled term; suggestions are available, but the collation's
     * hits are less than or equal to the original query term hits. In this case, the class should
     * not do anything
     *
     * @throws Exception
    ...
    public JSONObject getSearchResponseAsJson(String jsonResponse)
            throws FileNotFoundException, JSONException {
        URL url =
                SpellCheckDecisionManagerTest.class
                        .getClassLoader()
                        .getResource(RESOURCE_PREFIX + jsonResponse);
        if (url == null) {
    ...
    public JSONObject createJsonSearchRequest(String searchTerm) throws JSONException {
        String requestStr =
                "{\"queryConsistency\" : \"DEFAULT\",\"textAttributes\" : [],\"allAttributes\" :"
                    + " [],\"templates\" : [{\"template\" : \"%(cm:name cm:title cm:description"
                    + " ia:whatEvent ia:descriptionEvent lnk:title lnk:description TEXT"
                    + " TAG)\",\"name\" : \"keywords\"}],\"authorities\" : [\"GROUP_EVERYONE\","
                    + " \"ROLE_ADMINISTRATOR\", \"ROLE_AUTHENTICATED\", \"admin\"],\"tenants\" :"
                    + " [\"\"],\"query\" : \"("
                        + searchTerm
                        + "  AND (+TYPE:\\\"cm:content\\\" OR +TYPE:\\\"cm:folder\\\")) AND"
                        + " -TYPE:\\\"cm:thumbnail\\\" AND -TYPE:\\\"cm:failedThumbnail\\\" AND"
                        + " -TYPE:\\\"cm:rating\\\" AND -TYPE:\\\"st:site\\\" AND"
                        + " -ASPECT:\\\"st:siteContainer\\\" AND -ASPECT:\\\"sys:hidden\\\" AND"
                        + " -cm:creator:system AND -QNAME:comment\\\\-*\",\"locales\" :"
                        + " [\"en\"],\"defaultNamespace\" :"
                        + " \"http://www.alfresco.org/model/content/1.0\",\"defaultFTSFieldOperator\""
                        + " : \"AND\",\"defaultFTSOperator\" : \"AND\"}";
    ...
    private long getCollationHit(JSONObject resultJson) throws JSONException {
        ...
        for (int key = 0, value = 1, length = suggestions.length();
                value < length;
                key += 2, value += 2) {
    ...
    private long getCollationHit2(JSONObject resultJson) throws JSONException {
        ...
        for (int key = 0, value = 1, length = collations.length();
                value < length;
                key += 2, value += 2) {

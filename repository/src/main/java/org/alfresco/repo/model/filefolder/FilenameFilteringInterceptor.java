...
public class FilenameFilteringInterceptor implements MethodInterceptor {
  ...
  private static Log logger = LogFactory.getLog(
    FilenameFilteringInterceptor.class
  );
  ...
  private static final Pattern XSL_MACOS_TEMPORARY_FILENAME_FITLER = Pattern.compile(
    "^(\\" + MACOS_TEMPORARY_FILE_NAME_PREFIX + ")?[0-9,a-f]{8}$",
  ...
  private int getSystemFileVisibilityMask() {
    int mask = 0;
    mask |=
      hiddenAspect.getClientVisibilityMask(
        Client.cifs,
        Visibility.HiddenAttribute
      );
    mask |=
      hiddenAspect.getClientVisibilityMask(Client.webdav, Visibility.Visible);
    mask |=
      hiddenAspect.getClientVisibilityMask(Client.nfs, Visibility.Visible);
    mask |=
      hiddenAspect.getClientVisibilityMask(Client.ftp, Visibility.Visible);
    return mask;
  }
    ...
    if (methodName.startsWith("create")) {
      ...
      if (getMode() == Mode.ENHANCED) {
        if (systemPaths.isFiltered(filename)) {
          ...
          hiddenAspect.hideNode(
            fileInfo,
            getSystemFileVisibilityMask(),
            false,
            false,
            false
          );
          ...
          if (isSystemPath(nodeRef, filename)) {
            ...
            boolean isTmp = isTemporaryObject(
              filename,
              sourceInfo.getNodeRef()
            );
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Checking filename returned by " + methodName + ": " + filename
        );
      }
      ...
      if (args != null && args.length == 2) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Checking filename returned by " + methodName + ": " + newName
          );
        }
        ...
        if (getMode() == Mode.ENHANCED) {
          boolean isHidden = hiddenAspect.checkHidden(
            sourceNodeRef,
            true,
            true
          );
        ...
        throw new AlfrescoRuntimeException(
          "FilenameFilteringInterceptor: unknown rename method"
        );
      }
    } else {
      ret = invocation.proceed();
  ...
  private boolean isTemporaryObject(String name, NodeRef nodeRef) {
    ...
    boolean result = XSL_MACOS_TEMPORARY_FILENAME_FITLER
      .matcher(name)
      .matches();

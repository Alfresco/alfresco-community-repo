...
public class GetMethodRegressionTest extends TestCase {
  ...
  private static final String PROP_AUDIT_ALFRESCO_ACCESS_ENABLED =
    "audit.alfresco-access.enabled";

  private static final String ROOT_TEST_FOLDER_NAME =
    "TestFolder-" + System.currentTimeMillis();

  private static final String TEST_DOCUMENT_NAME_PATTERN =
    "TestDocument-%d-%d.pdf";

  private static final String TEXT_DOCUMENT_CONTENT_PATTERN =
    "Text content for '%s' document";
  ...
  public void setUp() throws Exception {
    ...
    ServiceRegistry registry = (ServiceRegistry) applicationContext.getBean(
      ServiceRegistry.SERVICE_REGISTRY
    );
    davHelper =
      (WebDAVHelper) applicationContext.getBean(WebDAVHelper.BEAN_NAME);
    auditRegistry =
      (AuditModelRegistryImpl) applicationContext.getBean(
        AUDIT_REGISTRY_BEAN_NAME
      );
  ...
  public void tearDown() throws Exception {
    if (
      (null != transaction) &&
      (Status.STATUS_ROLLEDBACK != transaction.getStatus()) &&
      (Status.STATUS_COMMITTED != transaction.getStatus())
    ) {
  ...
  public void testAuditRecordsAdditionAsbsence() throws Exception {
    String url = new StringBuilder(TEST_WEBDAV_URL_PREFIX)
      .append(ROOT_TEST_FOLDER_NAME)
      .toString();
    MockHttpServletRequest mockRequest = new MockHttpServletRequest(
      HTTP_METHOD_GET,
      url
    );
    testingMethod.setDetails(
      mockRequest,
      mockResponse,
      davHelper,
      companyHomeNodeRef
    ...
    boolean alfrescoAccessEnabled = Boolean.valueOf(
      auditRegistry.getProperty(PROP_AUDIT_ALFRESCO_ACCESS_ENABLED)
    );

    if (!alfrescoAccessEnabled) {
      setAuditRegistryProperty(
        PROP_AUDIT_ALFRESCO_ACCESS_ENABLED,
    ...
    try {
      ...
      if (null == expectedLastTime) {
        assertNull(
          "Audit entry table is not empty after 'GetMethod.executeImpl()' invocation. But it is expected to be empty!",
          actualLastTime
        );
      } else {
  ...
  private void setAuditRegistryProperty(String propertyName, String value) {
    assertTrue(
      ("'" + propertyName + "' is not updatable!"),
      auditRegistry.isUpdateable(propertyName)
    );
  ...
  private void createTestContent(NodeRef parentNode, int documentsAmount) {
    for (int i = 0; i < documentsAmount; i++) {
      String testDocumentName = String.format(
        TEST_DOCUMENT_NAME_PATTERN,
        i,
        System.currentTimeMillis()
      );
      FileInfo testDocument = fileFolderService.create(
        parentNode,
        testDocumentName,
        ContentModel.TYPE_CONTENT
      );
      ContentWriter writer = fileFolderService.getWriter(
        testDocument.getNodeRef()
      );
      writer.putContent(
        String.format(TEXT_DOCUMENT_CONTENT_PATTERN, testDocumentName)
      );
   ...
   * @throws Exception
   */
  private void restartTransaction(TransactionActionEnum transactionAction)
    throws Exception {
    if (
      (null != transaction) &&
      (Status.STATUS_ROLLEDBACK != transaction.getStatus()) &&

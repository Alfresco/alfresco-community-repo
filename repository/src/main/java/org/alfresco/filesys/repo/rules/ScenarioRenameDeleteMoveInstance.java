...
/**
 * This is an instance of a rename, delete, move scenario triggered by a rename of a file matching a
 * specified pattern.
 *
 * <p>a) Original file is renamed. Typically with an obscure name. b) Renamed file is deleted via
 * delete command or via deleteOnClose flag and close operation. c) Temp file is moved into original
 * file location.
 *
 * <p>If this filter is active then this is what happens. a) Original file is renamed: - File is
 * renamed. b) Renamed file is deleted via delete command or via deleteOnClose flag and close
 * operation: - File is deleted. c) Temp file is moved into original file location - Scenario fires
 * - Deleted file is restored. - Restored file is renamed to it's original name. - Content from file
 * that must be moved is copied to restored file. - File that must be moved is deleted.
 */
            ...
            case DELETE:
                if (operation instanceof DeleteFileOperation) {
                    ...
                    if (d.getName().equalsIgnoreCase(fileEnd)) {
                        ...
                        // Rename node to remove "hidden". In this case node will be moved to the
                        // archive store and can be restored later.
                        // This can be replaced with command that removes hidden aspect in
                        // future(when ContentDiskDriver2.setFileInformation() method will support
                        // hidden attribute)
                        ...
                        commands.add(
                                new DeleteFileCommand(
                                        fileEnd, d.getRootNodeRef(), folderEnd + "\\" + fileEnd));
                ...
                if (operation instanceof CloseFileOperation) {
                    ...
                    if (c.getNetworkFile().hasDeleteOnClose()
                            && c.getName().equalsIgnoreCase(fileEnd)) {
                        internalState = InternalState.MOVE;
                        ArrayList<Command> commands = new ArrayList<Command>();
                        ArrayList<Command> postCommitCommands = new ArrayList<Command>();
                        ArrayList<Command> postErrorCommands = new ArrayList<Command>();
                    ...
                    if (fileMiddle.equalsIgnoreCase(m.getTo())) {
                        ...
                        RestoreFileCommand rest1 =
                                new RestoreFileCommand(
                                        fileEnd, m.getRootNodeRef(), folderEnd, 0, originalNodeRef);
                        RenameFileCommand r1 =
                                new RenameFileCommand(
                                        fileEnd,
                                        fileMiddle,
                        ...
                        DeleteFileCommand d1 =
                                new DeleteFileCommand(
                                        oldFolder, m.getRootNodeRef(), oldFolder + "\\" + fileFrom);
                        commands.add(d1);

...
public class CalendarServiceImplTest {
    ...
    public static void initTestsContext() throws Exception {
        ...
        NamedObjectRegistry<CannedQueryFactory<? extends Object>> calendarCannedQueryRegistry =
                (NamedObjectRegistry<CannedQueryFactory<? extends Object>>)
                        testContext.getBean("calendarCannedQueryRegistry");
        CALENDAR_CQ_FACTORY =
                (GetCalendarEntriesCannedQueryFactory)
                        calendarCannedQueryRegistry.getNamedObject(
    ...
    public void createNewEntry() throws Exception {
        ...
        PagingResults<CalendarEntry> results =
                CALENDAR_SERVICE.listCalendarEntries(
                        CALENDAR_SITE.getShortName(), new PagingRequest(10));
                    ...
                    public Void execute() throws Throwable {
                        ...
                        assertFalse(
                                SITE_SERVICE.hasContainer(
                                        TEST_SITE_NAME, CalendarServiceImpl.CALENDAR_COMPONENT));
    ...
    public void calendarSingleSiteListing() throws Exception {
        ...
        results =
                CALENDAR_SERVICE.listOutlookCalendarEntries(
                        CALENDAR_SITE.getShortName(), null, paging);
        assertEquals(0, results.getPage().size());
        ...
        results =
                CALENDAR_SERVICE.listOutlookCalendarEntries(
                        CALENDAR_SITE.getShortName(), uidB, paging);
        assertEquals(1, results.getPage().size());
        assertEquals("TitleB", results.getPage().get(0).getTitle());
        ...
        results =
                CALENDAR_SERVICE.listOutlookCalendarEntries(
                        CALENDAR_SITE.getShortName(), uidC, paging);
        assertEquals(1, results.getPage().size());
        assertEquals("TitleC", results.getPage().get(0).getTitle());
    }

    /**
     * Checks that the correct permission checking occurs on fetching calendar listings (which go
    ...
    public void calendarListingPermissionsChecking() throws Exception {
        ...
        results =
                CALENDAR_SERVICE.listCalendarEntries(
                        ALTERNATE_CALENDAR_SITE.getShortName(), paging);
        assertEquals(0, results.getPage().size());
        ...
        assertEquals(
                false, SITE_SERVICE.isMember(ALTERNATE_CALENDAR_SITE.getShortName(), TEST_USER));
                ...
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        SITE_SERVICE.removeMembership(
                                ALTERNATE_CALENDAR_SITE.getShortName(), TEST_USER);
                        AuthenticationUtil.setFullyAuthenticatedUser(TEST_USER);
                        return null;
                    }
                });
    ...
    /**
     * Ensure that the canned query returns the right entity objects for the underlying calendar
     * entries. Checks both the low level filtering, and the DB fetching of the properties used in
     * the filter
     */
    @Test
    public void testCannedQueryEntityResults() throws Exception {
        final PagingRequest paging = new PagingRequest(10);
        final NodeRef[] containers =
                new NodeRef[] {
                    SITE_SERVICE.getContainer(
                            CALENDAR_SITE.getShortName(), CalendarServiceImpl.CALENDAR_COMPONENT),
                    SITE_SERVICE.getContainer(
                            ALTERNATE_CALENDAR_SITE.getShortName(),
        ...
        final GetCalendarEntriesCannedQueryTestHook hook =
                new GetCalendarEntriesCannedQueryTestHook() {
                    @Override
                    public void notifyComplete(
                            List<CalendarEntity> fullList, List<CalendarEntity> filteredList) {
                        full.clear();
                        filtered.clear();
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    @Override
                    public Void execute() throws Throwable {
                        GetCalendarEntriesCannedQuery cq =
                                (GetCalendarEntriesCannedQuery)
                                        CALENDAR_CQ_FACTORY.getCannedQuery(
    ...
    private static void createTestSites() throws Exception {
        final CalendarServiceImpl privateCalendarService =
                (CalendarServiceImpl) testContext.getBean("calendarService");

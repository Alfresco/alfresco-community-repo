...
public class EmailServiceImpl implements EmailService {
  ...
  private static final String ERR_INVALID_SUBJECT =
    "email.server.err.invalid_subject";
  private static final String ERR_ACCESS_DENIED =
    "email.server.err.access_denied";
  ...
  private static final String ERR_INVALID_NODE_ADDRESS =
    "email.server.err.invalid_node_address";
  private static final String ERR_HANDLER_NOT_FOUND =
    "email.server.err.handler_not_found";
   ...
   */
  public void setEmailMessageHandlerMap(
    Map<String, EmailMessageHandler> emailMessageHandlerMap
  ) {
        ...
        public String execute() throws Throwable {
          ...
          if (userName == null) {
            if (unknownUser.isEmpty()) {
              if (logger.isDebugEnabled()) {
                logger.debug(
                  "unable to find user for from: " + message.getFrom()
                );
              }
              throw new EmailMessageException(
                ERR_UNKNOWN_SOURCE_ADDRESS,
                message.getFrom()
              );
            } else {
              if (logger.isDebugEnabled()) {
  ...
  private EmailMessageHandler getMessageHandler(NodeRef nodeRef) {
    ...
    EmailMessageHandler handler = emailMessageHandlerMap.get(
      prefixedNodeTypeStr
    );
    ...
    if (handler == null) {
      ...
      while (typeDef != null) {
        ...
        if (parentName != null) {
          String prefixedSubTypeStr = parentName.toPrefixString(
            namespaceService
          );
          ...
          if (handler != null) {
            if (logger.isDebugEnabled()) {
              logger.debug(
                "found a handler for a subtype:" + prefixedSubTypeStr
              );
            }
            return handler;
          }
  ...
  private NodeRef getTargetNode(String recipient) {
    ...
    NodeRef ref = (NodeRef) getAttributeService()
      .getAttribute(
    ...
    try {
      ...
      NodeRef byNodeId = AuthenticationUtil.runAsSystem(() ->
  ...
  private boolean isEmailContributeUser(String userName) {
    return this.authorityService.getAuthoritiesForUser(userName)
      .contains(

...
/**
 * Implementation for Java backed webscript to return full details about a disposition lifecycle
...
public class DispositionLifecycleGet extends DispositionAbstractBase {
    ...
    protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
        ...
        if (nextAction == null) {
            ...
            nextActionModel.put(
                    "eventsEligible",
                    getDispositionService().isNextDispositionActionEligible(nodeRef));
            ...
            if (nextAction.getStartedAt() != null) {
                nextActionModel.put(
                        "startedAt", ISO8601DateFormat.format(nextAction.getStartedAt()));
            }
            ...
            if (nextAction.getCompletedAt() != null) {
                nextActionModel.put(
                        "completedAt", ISO8601DateFormat.format(nextAction.getCompletedAt()));
            }
     ...
     */
    protected void addUsersRealName(
            Map<String, Object> model, String userName, String propertyPrefix) {
        NodeRef user = this.personService.getPerson(userName);
        if (user != null) {
            String firstName =
                    (String) getNodeService().getProperty(user, ContentModel.PROP_FIRSTNAME);
            if (firstName != null) {
                model.put(propertyPrefix + "FirstName", firstName);
            }

            String lastName =
                    (String) getNodeService().getProperty(user, ContentModel.PROP_LASTNAME);
            if (lastName != null) {

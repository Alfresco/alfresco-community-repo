...
public class SubsystemsTest extends BaseSpringTest {
  ...
  public void testSubsystems() throws Exception {
    ...
    assertEquals(
      ...
      testBeans[2].getAnotherStringProperty()
    );
  }
  ...
  public void testAbstractPropertyBackedBean_performEarlyPropertyChecks_PortEarlyPropertyChecker() {
    ...
    ChildApplicationContextFactory testBean = (ChildApplicationContextFactory) applicationContext.getBean(
      "testsubsystem"
    );
    ...
    assertTrue(
      errorMessage.contains(
        "The port chosen for TestSubsystem is outside the required range (1, 65535): 0."
      )
    );
    assertTrue(
      errorMessage.contains(
        "The port chosen for TestSubsystem is outside the required range (1, 65535): 65536."
      )
    );

    assertTrue(
      errorMessage.contains(
        "The port chosen for TestSubsystem is already in use or you don't have permission to use it: " +
        testPortNumber +
        "."
      )
    );
  ...
  public void testAbstractPropertyBackedBean_performEarlyPropertyChecks_CustomEarlyPropertyChecker() {
    ...
    SubsystemEarlyPropertyChecker testEarlyPropertyChecker = new SubsystemEarlyPropertyChecker() {
      ...
      public void checkPropertyValue(
        ...
        if (pairedPropertyValue == null) {
          ...
          (propertyValue + pairedPropertyValue).contains("bad value")
        ) {
    ...
    assertTrue(
      errorMessage.contains("Property value cannot be a 'Bad value'.")
    );
    ...
    testProperties.clear();
    testProperties.put(
      "testPairedPropertyName",
      "Test paired property bad value"
    );

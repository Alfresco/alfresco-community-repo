...
public abstract class FormProcessorTest extends TestCase {
    ...
    protected static final QName PROP_WITH_ =
            QName.createQName(NamespaceService.BPM_MODEL_1_0_URI, "some_prop");
    protected static final QName ACTORS_NAME = ASSOC_POOLED_ACTORS;
    protected static final QName ASSIGNEE_NAME = ASSOC_ASSIGNEE;
    protected static final QName ASSOC_WITH_ =
            QName.createQName(NamespaceService.BPM_MODEL_1_0_URI, "some_assoc");
    protected static final NodeRef FAKE_NODE =
            new NodeRef(NamespaceService.BPM_MODEL_1_0_URI + "/FakeNode");
    protected static final NodeRef FAKE_NODE2 =
            new NodeRef(NamespaceService.BPM_MODEL_1_0_URI + "/FakeNode2");
    protected static final NodeRef FAKE_NODE3 =
            new NodeRef(NamespaceService.BPM_MODEL_1_0_URI + "/FakeNode3");
    protected static final NodeRef PCKG_NODE =
            new NodeRef(NamespaceService.BPM_MODEL_1_0_URI + "/FakePackage");
    ...
    protected DefaultFieldProcessor makeDefaultFieldProcessor(DictionaryService dictionaryService)
            throws Exception {
    ...
    public void testPersistPropertyComment(String taskId) throws Exception {
        ...
        wikiHamlet.append(
                "Hamlet is the son of the King of Denmark. When Hamlet`s father dies, his uncle"
                        + " Claudius becomes king and marries Hamlet`s mother (Gertrude).");
        wikiHamlet.append(
                "Hamlet`s father appears as a ghost and tells Hamlet that he was murdered by"
                        + " Claudius. Hamlet is not sure that the ghost is really his father.");
        wikiHamlet.append(
                "He gets some travelling actors to perform a play which shows the murder of a king"
                        + " in the same way Hamlet`s father said he was killed.");
        wikiHamlet.append(
        ...
        wikiHamlet.append(
                "Hamlet tells his mother that he knows about the murder. While there he kills"
                    + " Polonius, who is the king`s advisor, because he thinks he is Claudius.");
        wikiHamlet.append("Rosencrantz and Guildenstern were Hamlet`s childhood friends.");
        wikiHamlet.append(
                "Claudius sends Rosencrantz and Guildenstern with Hamlet to England to have Hamlet"
                    + " killed, but their ship is attacked by pirates who take Hamlet prisoner but"
                    + " then return him to Denmark.");
        wikiHamlet.append(
                "Rosencrantz and Guildenstern are taken to England where they die. Ophelia is"
                    + " Polonius` daughter . After her father, Polonius, is killed by Hamlet she"
                    + " goes mad.");
        wikiHamlet.append(
                "Then she falls into a river and drowns. Hamlet returns just as her funeral is"
                        + " happening. Laertes, her brother, decides to kill Hamlet in revenge.");
        wikiHamlet.append(
                "He challenges Hamlet to a sword fight, and puts poison on his own sword. Claudius"
                    + " makes some poisoned wine for Hamlet to drink in case that does not work.");
        wikiHamlet.append(
                "At first Hamlet wins the sword fight, and in the meantime his mother drinks the"
                        + " poisoned wine without knowing, and dies.");
        wikiHamlet.append(
                "On the other hand Laertes falsely pierces Hamlet with a poisoned blade, but then"
                    + " Hamlet stabs Laertes with the same sword. Laertes tells Hamlet about the"
                    + " plot and then dies.");
        wikiHamlet.append(
                "Hamlet kills Claudius with the poisoned sword. Horatio, Hamlet`s friend, tells"
                        + " everyone about the murder of the old king.");
        wikiHamlet.append(
                "Hamlet tells everyone that the Norwegian prince, Fortinbras, should be king, and"
                        + " then dies from the poison.");
        wikiHamlet.append(
                "When Fortinbras arrives, Horatio recounts the tale and Fortinbras orders Hamlet`s"
                        + " body borne off in honour.");
        wikiHamlet.append(
                "Hamlet is one of the hardest parts for an actor to perform. It is one of the"
                        + " largest roles written by Shakespeare.");
        wikiHamlet.append(
                "Many people disagree about what Hamlet is really thinking. For many actors,"
                        + " playing Hamlet is one of the most important parts of their career.");
    ...
    protected AbstractWorkflowFormProcessor makeTaskFormProcessor(
            AbstractWorkflowFormProcessor processor1,
            DictionaryService dictionaryService,
            MockFieldProcessorRegistry fieldProcessorRegistry,
            DefaultFieldProcessor defaultProcessor) {
    ...
    protected void mockPackageItems(NodeRef... children) {
        ArrayList<ChildAssociationRef> results =
                new ArrayList<ChildAssociationRef>(children.length);
        for (NodeRef nodeRef : children) {
            ChildAssociationRef child =
                    new ChildAssociationRef(ASSOC_PACKAGE_CONTAINS, PCKG_NODE, null, nodeRef);
            results.add(child);
        }
    ...
    protected void checkAddPackageItem(NodeRef child, boolean wasCalled) {
        ...
        verify(nodeService, times(times))
                .addChild(eq(PCKG_NODE), eq(child), eq(ASSOC_PACKAGE_CONTAINS), (QName) any());
    }
    ...
    protected void checkSingleField(
            Form form, String fieldName, Serializable fieldData, String expDataKey) {
    ...
    protected Map<QName, AssociationDefinition> makeTaskAssociationDefs() {
        Map<QName, AssociationDefinition> associations =
        ...
        MockClassAttributeDefinition assigneeDef =
                MockClassAttributeDefinition.mockAssociationDefinition(ASSIGNEE_NAME, actorName);
        associations.put(ASSIGNEE_NAME, assigneeDef);

        // Add Assigneee association
        MockClassAttributeDefinition actorsDef =
                MockClassAttributeDefinition.mockAssociationDefinition(ACTORS_NAME, actorName);
        associations.put(ACTORS_NAME, actorsDef);

        // Add association with _
        MockClassAttributeDefinition with_ =
                MockClassAttributeDefinition.mockAssociationDefinition(ASSOC_WITH_, actorName);

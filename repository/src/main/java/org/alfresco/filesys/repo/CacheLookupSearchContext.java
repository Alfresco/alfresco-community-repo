 ...
 *
 * <p>Use the file state cache to check for current timestamp values for file information being
 * returned in the current search.
 *
 * @author gkspencer
 */
public class CacheLookupSearchContext extends DotDotContentSearchContext {
    ...
    public boolean nextFileInfo(FileInfo info) {
        ...
        if (super.nextFileInfo(info) == false) return false;
        ...
        if (fstate != null) {
            // Check if there are current timestamps that can be used to override the database
            // values

            if (fstate.hasAccessDateTime()) info.setAccessDateTime(fstate.getAccessDateTime());

            if (fstate.hasModifyDateTime()) info.setModifyDateTime(fstate.getModifyDateTime());

            // File used/allocation size

            if (fstate.hasFileSize()) info.setFileSize(fstate.getFileSize());

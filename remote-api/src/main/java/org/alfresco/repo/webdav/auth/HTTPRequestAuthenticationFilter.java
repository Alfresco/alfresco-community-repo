...
public class HTTPRequestAuthenticationFilter extends BaseAuthenticationFilter implements Filter {
    ...
    public void init(FilterConfig config) throws ServletException {
        ...
        WebApplicationContext ctx =
                WebApplicationContextUtils.getRequiredWebApplicationContext(m_context);
        ...
        setPersonService(
                (PersonService)
                        ctx.getBean("PersonService")); // transactional and permission-checked
                    ...
                    if (matcher.matches()) {
                        ...
                        if ((userName == null) || (userName.length() < 1)) {
                            if (logger.isDebugEnabled()) {
                                logger.debug(
                                        "Extracted null or empty user name from pattern "
                                                + m_authPatternString
                                                + " against "
                                                + authHdr);
                            }
    ...
    private void reject(HttpServletRequest httpReq, HttpServletResponse httpResp)
            throws IOException {

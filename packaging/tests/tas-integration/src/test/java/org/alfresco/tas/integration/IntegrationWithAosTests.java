...
public class IntegrationWithAosTests extends IntegrationTest {
  ...
  @Test(
    groups = {
      TestGroup.INTEGRATION, TestGroup.AOS, TestGroup.FULL, TestGroup.SSO,
    }
  )
  @TestRail(
    section = { TestGroup.INTEGRATION, TestGroup.AOS },
    executionType = ExecutionType.REGRESSION,
    description = "Security check for AOS Protocol with External Authentification - MNT-17474"
  )
  public void aosCSRFVulnerabilityInSSOEnvironment() throws Exception {
    ...
    String authCookie = String.format(
      "%s:%s",
      adminUser.getUsername(),
      adminUser.getPassword()
    );
    String authCookieEncoded = new String(
      Base64.encodeBase64(authCookie.getBytes())
    );
    headers.put("Authorization", String.format("Basic %s", authCookieEncoded));

    restAPI.configureRequestSpec().addHeaders(headers);
    JsonObject postBody = JsonBodyGenerator
      .defineJSON()
      .add("method", "create url-directories:6.0.2.8164")
      .add("urldirs", "[[url=ExploitedMNT17474;meta_info=[]]]")
      .build();
    restAPI
      .authenticateUser(adminUser)
      .usingContentType(ContentType.URLENC)
      .withAosAPI();
  ...
  @TestRail(
    section = { TestGroup.INTEGRATION, TestGroup.AOS },
    executionType = ExecutionType.REGRESSION,
    description = "Non domain user can login to aos through Kerberos successfully."
  )
  public void nonDomainKerberosUserCanLoginSuccessfulInAos() throws Exception {
    ...
    RestHtmlResponse response = restAPI
      .authenticateUser(dataUser.getAdminUser())
      .processHtmlResponse(request);
    restAPI.assertStatusCodeIs(HttpStatus.OK);
    response.assertPathInHtmlBodyEquals(
      "html.body.table.tr[1].td[0].text()",
      "Shared"
    );
    response.assertPathInHtmlBodyEquals(
      "html.body.table.tr[2].td[0].text()",
      "Imap Attachments"
    );
  }
  ...
  @Test(
    groups = {
      TestGroup.INTEGRATION, TestGroup.WEBDAV, TestGroup.FULL, TestGroup.SSO,
    }
  )
  @TestRail(
    section = { TestGroup.INTEGRATION, TestGroup.WEBDAV },
    executionType = ExecutionType.REGRESSION,
    description = "Non domain user can login to webdav through Kerberos successfully."
  )
  public void nonDomainKerberosUserCanLoginSuccessfulInWebDav()
    throws Exception {
    ...
    restAPI.assertStatusCodeIs(HttpStatus.OK);
    response.assertPathInHtmlBodyEquals(
      "html.body.table.tr.td[0].text()",
      "Directory listing for /"
    );
    response.assertPathInHtmlBodyEquals(
      "html.body.table.tr.td.find{it.@class=='textData'}[0].text()",
      "Shared"
    );
  }
}

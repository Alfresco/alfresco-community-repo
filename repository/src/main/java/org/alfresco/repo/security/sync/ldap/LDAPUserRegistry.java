...
/**
 * A {@link UserRegistry} implementation with the ability to query Alfresco-like descriptions of
 * users and groups from an LDAP directory, optionally restricted to those modified since a certain
 * time.
 *
 * @author dward
 */
public class LDAPUserRegistry
        implements UserRegistry, LDAPNameResolver, InitializingBean, ActivateableBean {
    ...
    /**
     * Enable progress estimation? When enabled, the user query has to be run twice in order to
     * count entries.
     */
    ...
    private String groupDifferentialQuery =
    ...
    private String personDifferentialQuery =
    ...
    /**
     * The query batch size. If positive, indicates that RFC 2696 paged results should be used to
     * split query results into batches of the specified size. Overcomes any size limits imposed by
     * the LDAP server.
     */
    private int queryBatchSize;

    /**
     * The attribute retrieval batch size. If positive, indicates that range retrieval should be
     * used to fetch multi-valued attributes (such as member) in batches of the specified size.
    ...
    public LDAPUserRegistry() {
        // Default to official LDAP generalized time format (unfortunately not used by Active
        // Directory)
    ...
    /**
     * Controls whether progress estimation is enabled. When enabled, the user query has to be run
     * twice in order to count entries.
     *
     * @param enableProgressEstimation <code>true</code> if progress estimation is enabled
     */
     ...
     *
     * @param groupIdAttributeName the group id attribute name
     */
     ...
     *
     * @param groupDifferentialQuery the group differential query
     */
     ...
     *
     * @param personDifferentialQuery the person differential query
     */
     ...
     *
     * @param memberAttribute the member attribute name
     */
     ...
     *
     * @param modifyTimestampAttributeName the modification timestamp attribute name
     */
     ...
     *
     * @param errorOnMissingMembers <code>true</code> if we should error on missing group members
     */
     ...
     *
     * @param errorOnMissingGID <code>true</code> if we should error on missing group IDs
     */
     ...
     *
     * @param errorOnMissingUID <code>true</code> if we should error on missing user IDs
     */
     ...
     *
     * @param errorOnDuplicateGID <code>true</code> if we should error on duplicate group IDs
     */
     ...
     *
     * @param personAttributeDefaults the person attribute defaults
     */
     ...
     *
     * @param personAttributeMapping the person attribute mapping
     */
     ...
     *
     * @param groupAttributeDefaults the group attribute defaults
     */
     ...
     *
     * @param groupAttributeMapping the group attribute mapping
     */
     ...
     *
     * @param queryBatchSize If positive, indicates that RFC 2696 paged results should be used to
     *     split query results into batches of the specified size. Overcomes any size limits imposed
     *     by the LDAP server.
     */
     ...
     *
     * @param attributeBatchSize If positive, indicates that range retrieval should be used to fetch
     *     multi-valued attributes (such as member) in batches of the specified size. Overcomes any
     *     size limits imposed by the LDAP server.
     */
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        this.personAttributeMapping.put(
                ContentModel.PROP_USERNAME.toPrefixString(this.namespaceService),
        ...
        this.groupAttributeMapping.put(
                ContentModel.PROP_AUTHORITY_NAME.toPrefixString(this.namespaceService),
                this.groupIdAttributeName);
        this.groupKeys =
                initKeys(
                        this.groupAttributeMapping,
                        this.attributeBatchSize > 0
                                ? this.memberAttributeName
                                        + ";range=0-"
                                        + (this.attributeBatchSize - 1)
                                : this.memberAttributeName);
    }
    ...
    public Collection<String> getPersonNames() {
        ...
        processQuery(
                new AbstractSearchCallback() {
                    protected void doProcess(SearchResult result)
                        ...
                        if (nameAttribute == null) {
                            if (LDAPUserRegistry.this.errorOnMissingUID) {
                                ...
                                throw new AlfrescoRuntimeException(
                                        "synchronization.err.ldap.get.user.id.missing", params);
                            } else {
                            ...
                            if (LDAPUserRegistry.logger.isDebugEnabled()) {
                                LDAPUserRegistry.logger.debug(
                                        "Person DN recognized: " + nameAttribute.get());
                            }
                ...
                this.userSearchBase,
                this.personQuery,
                new String[] {this.userIdAttributeName});
        return personNames;
    }
    ...
    public Collection<String> getGroupNames() {
        ...
        processQuery(
                    ...
                    protected void doProcess(SearchResult result)
                        ...
                        if (nameAttribute == null) {
                            if (LDAPUserRegistry.this.errorOnMissingGID) {
                                ...
                                LDAPUserRegistry.logger.warn(
                                        "Missing GID on " + result.getNameInNamespace());
                            }
                        } else {
                ...
                this.groupQuery,
                new String[] {this.groupIdAttributeName});
        return groupNames;
    }
    ...
    public Collection<NodeDescription> getGroups(Date modifiedSince) {
        // Work out whether the user and group trees are disjoint. This may allow us to optimize
        // reverse DN
        // resolution.
        ...
        try {
            ...
            throw new AlfrescoRuntimeException(
                    "synchronization.err.ldap.search.base.invalid", params, e);
        }
        ...
        final boolean disjoint =
                !groupDistinguishedNamePrefix.startsWith(userDistinguishedNamePrefix)
        ...
        processQuery(
                new AbstractSearchCallback() {
                    // We get a whole new context to avoid interference with cookies from paged
                    // results
                    private DirContext ctx =
                            LDAPUserRegistry.this.ldapInitialContextFactory
                    ...
                    protected void doProcess(SearchResult result)
                        ...
                        Attribute gidAttribute =
                                attributes.get(LDAPUserRegistry.this.groupIdAttributeName);
                        ...
                        if (group == null) {
                            ...
                            group =
                                    mapToNode(
                                            LDAPUserRegistry.this.groupAttributeMapping,
                                            LDAPUserRegistry.this.groupAttributeDefaults,
                                            result);
                            ...
                            throw new AlfrescoRuntimeException(
                                    "Duplicate group id found for " + gid);
                        } else {
                            LDAPUserRegistry.logger.warn(
                                    "Duplicate gid found for " + gid + " -> merging definitions");
                        }
                        ...
                        Attribute memAttribute =
                                getRangeRestrictedAttribute(
                                        attributes, LDAPUserRegistry.this.memberAttributeName);
                        ...
                        while (memAttribute != null) {
                            for (int i = 0; i < memAttribute.size(); i++) {
                                ...
                                if (attribute != null && attribute.length() > 0) {
                                    try {
                                        // Attempt to parse the member attribute as a DN. If this
                                        // fails we have a fallback
                                        // in the catch block
                                        LdapName distinguishedNameForComparison =
                                                fixedLdapName(attribute.toLowerCase());
                                        Attribute nameAttribute;

                                        // If the user and group search bases are different we may
                                        // be able to recognize user
                                        // and group DNs without a secondary lookup
                                        if (disjoint) {
                                            LdapName distinguishedName = fixedLdapName(attribute);
                                            Attributes nameAttributes =
                                                    distinguishedName
                                                            .getRdn(distinguishedName.size() - 1)
                                                            .toAttributes();

                                            // Recognize user DNs
                                            if (distinguishedNameForComparison.startsWith(
                                                            userDistinguishedNamePrefix)
                                                    && (nameAttribute =
                                                                    nameAttributes.get(
                                                                            LDAPUserRegistry.this
                                                                                    .userIdAttributeName))
                                                            != null) {
                                                if (LDAPUserRegistry.logger.isDebugEnabled()) {
                                                    LDAPUserRegistry.logger.debug(
                                        ...
                                        // If we can't determine the name and type from the DN
                                        // alone, try a directory lookup
                                        if (distinguishedNameForComparison.startsWith(
                                                        userDistinguishedNamePrefix)
                                                || distinguishedNameForComparison.startsWith(
                                                        groupDistinguishedNamePrefix)) {
                                            try {
                                                ...
                                                Attribute objectClass =
                                                        childAttributes.get("objectclass");
                                                if (hasAttributeValue(
                                                        objectClass,
                                                        LDAPUserRegistry.this.personType)) {
                                                    nameAttribute =
                                                            childAttributes.get(
                                                                    LDAPUserRegistry.this
                                                                            .userIdAttributeName);
                                                    if (nameAttribute == null) {
                                                        if (LDAPUserRegistry.this
                                                                .errorOnMissingUID) {
                                                            throw new AlfrescoRuntimeException(
                                                                    "User missing user id attribute"
                                                                            + " DN ="
                                                                            + attribute
                                                                            + "  att = "
                                                                            + LDAPUserRegistry.this
                                                    ...
                                                    if (LDAPUserRegistry.logger.isDebugEnabled()) {
                                                        LDAPUserRegistry.logger.debug(
                                                                "User DN recognized by directory"
                                                                        + " lookup: "
                                                                        + nameAttribute.get());
                                                    }
                                                    childAssocs.add((String) nameAttribute.get());
                                                    ...
                                                    if (LDAPUserRegistry.logger.isDebugEnabled()) {
                                                        LDAPUserRegistry.logger.debug(
                                                                "Group DN recognized by directory"
                                                                        + " lookup: GROUP_"
                                                                        + nameAttribute.get());
                                                    }
                                                    childAssocs.add("GROUP_" + nameAttribute.get());
                                                ...
                                                // MNT-21614: Check & fail if communication breaks
                                                // due to ServiceUnavailableException or
                                                ...
                                                LDAPUserRegistry.logger.warn(
                                                        "Failed to resolve member of group '"
                                                                + groupShortName
                                        ...
                                        if (LDAPUserRegistry.this.errorOnMissingMembers) {
                                            ...
                                            throw new AlfrescoRuntimeException(
                                                    "synchronization.err.ldap.group.member.missing",
                                                    params);
                                        }
                                        LDAPUserRegistry.logger.warn(
                                                "Failed to resolve member of group '"
                                        ...
                                        // The member attribute didn't parse as a DN. So assume we
                                        // have a group class like
                                        // posixGroup (FDS) that directly lists user names
                                        if (LDAPUserRegistry.logger.isDebugEnabled()) {
                                            LDAPUserRegistry.logger.debug(
                                                    "Member DN recognized as posixGroup: "
                                                            + attribute);
                                        }
                            ...
                            // If we are using attribute matching and we haven't got to the end
                            // (indicated by an asterisk),
                            // fetch the next batch
                            if (nextStart > 0
                                    && !LDAPUserRegistry.PATTERN_RANGE_END
                                            .matcher(memAttribute.getID().toLowerCase())
                                            .find()) {
                                Attributes childAttributes =
                                        this.ctx.getAttributes(
                                                jndiName(result.getNameInNamespace()),
                                                new String[] {
    ...
    public String resolveDistinguishedName(String userId, AuthenticationDiagnostic diagnostic)
        ...
        // Although we don't actually need any attributes, we ask for the UID for compatibility with
        // Sun Directory Server. See ALF-3868
        ...
        try {
            ...
            // Execute the user query with an additional condition that ensures only the user with
            // the required ID is
            // returned. Force RFC 2254 escaping of the user ID in the filter to avoid any
            // manipulation
            ...
            if (searchResults.hasMore()) {
                ...
                if (uidAttribute == null) {
                    if (this.errorOnMissingUID) {
                        throw new AlfrescoRuntimeException(
                                "User returned by user search does not have mandatory user id"
                                        + " attribute "
                                        + attributes);
                    } else {
                        LDAPUserRegistry.logger.warn(
                ...
                // MNT:2597 We don't trust the LDAP server's treatment of whitespace, accented
                // characters etc. We will
            ...
            throw new AuthenticationException(
                    "authentication.err.connection.ldap.user.notfound", args, diagnostic);
        } catch (NamingException e) {
            // Connection is good here - AuthenticationException would be thrown by
            ...
            throw new AuthenticationException(
                    "authentication.err.connection.ldap.search", diagnostic, args, e);
        } finally {
            if (result != null) {
                try {
                    Context context = (Context) result.getObject();
    ...
    private Pair<String[], Set<QName>> initKeys(
            Map<String, String> attributeMapping, String... extraAttibutes) {
    ...
    private NodeDescription mapToNode(
        ...
        if (modifyTimestamp != null) {
            try {
                nodeDescription.setLastModified(
    ...
    /**
     * Converts a given DN into one suitable for use through JNDI. In particular, escapes special
     * characters such as '/' which have special meaning to JNDI.
     *
     * @param dn the dn
     * @return the name
    ...
    /**
     * Works around a bug in the JDK DN parsing. If an RDN has trailing escaped whitespace in the
     * format "\\20" then LdapName would normally strip this. This method works around this by
     * replacing "\\20" with "\\ " and "\\0D" with "\\\r".
     *
     * @param dn the DN
    ...
    private static LdapName fixedLdapName(String dn) throws InvalidNameException {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (c == '\\') {
                if (i + 2 < length
                        && Character.isLetterOrDigit(c1 = dn.charAt(i + 1))
     ...
     * @param returningAttributes the attributes to include in search results
    ...
    private void processQuery(
        ...
        if (LDAPUserRegistry.logger.isDebugEnabled()) {
            ...
            LDAPUserRegistry.logger.debug(
                    "    Return result limit: " + searchControls.getCountLimit());
            LDAPUserRegistry.logger.debug("    DerefLink: " + searchControls.getDerefLinkFlag());
            LDAPUserRegistry.logger.debug(
                    "    Return named object: " + searchControls.getReturningObjFlag());
            LDAPUserRegistry.logger.debug(
                    "    Time limit for search: " + searchControls.getTimeLimit());
            LDAPUserRegistry.logger.debug(
                    "    Attributes to return: " + returningAttributes.length + " items.");
     ...
     * @throws NamingException if there is a problem accessing the attribute values
     */
    ...
    /**
     * Gets the values of a repeating attribute that may have range restriction options. If an
     * attribute is range restricted, it will appear in the attribute set with a ";range=i-j"
     * option, where i and j indicate the start and end index, and j is '*' if it is at the end.
     *
    ...
    public class PersonCollection extends AbstractCollection<NodeDescription> {
         ...
         *
         * @param modifiedSince if non-null, then only descriptions of users modified since this
         *     date should be returned; if <code>null</code> then descriptions of all users should
         *     be returned.
         */
        public PersonCollection(Date modifiedSince) {
            ...
            if (modifiedSince == null) {
                ...
                this.query =
                        MessageFormat.format(
                                LDAPUserRegistry.this.personDifferentialQuery,
            ...
            if (LDAPUserRegistry.this.enableProgressEstimation) {
                ...
                processQuery(
                        countingCallback,
                        LDAPUserRegistry.this.userSearchBase,
                        this.query,
                        new String[] {});
        ...
        private class PersonIterator implements Iterator<NodeDescription> {
            ...
            /**
             * Indicates that sync process has been retried and we still don't reach last processed
             * person
             */
            ...
            public PersonIterator() {
                try {
                    ...
                    this.userSearchCtls.setReturningAttributes(
                            LDAPUserRegistry.this.userKeys.getFirst());
                    // MNT-14001 fix, set search limit to ensure that server will not return more
                    // search results then provided by paged result control
                    this.userSearchCtls.setCountLimit(
                            LDAPUserRegistry.this.queryBatchSize > 0
                                    ? LDAPUserRegistry.this.queryBatchSize
                                    : 0);
            ...
            public NodeDescription next() {
                ...
                try {
                    ...
                    try {
                        if (LDAPUserRegistry.logger.isDebugEnabled()) {
                            LDAPUserRegistry.logger.debug(
                                    "CommunicationException was thrown with message: "
                                            + e.getMessage()
                                            + ". Performing another attempt to sync with ldap. Last"
                                            + " processed person was: '"
                                            + (this.lastProcessedPerson == null
                                                    ? "null"
                                                    : this.lastProcessedPerson)
                                            + "'");
                        }
                        ...
                        throw new AlfrescoRuntimeException(
                                "Failed to import people. Also failed to restart sync process.",
                                ex);
                    }
                } catch (NamingException e) {
                    throw new AlfrescoRuntimeException("Failed to import people.", e);
             ...
             * @throws NamingException on a naming exception
             */
                ...
                do {
                    ...
                    while (!readyForNextPage && this.searchResults.hasMore()) {
                        ...
                        Attribute uidAttribute =
                                attributes.get(LDAPUserRegistry.this.userIdAttributeName);
                        if (uidAttribute == null) {
                            if (LDAPUserRegistry.this.errorOnMissingUID) {
                                Object[] params = {
                                    result.getNameInNamespace(),
                                ...
                                LDAPUserRegistry.logger.warn(
                                        "User returned by user search does not have mandatory user"
                                                + " id attribute "
                                                + attributes);
                                continue;
                            }
                        ...
                        if (!this.skipToLastProcessedPerson) {
                            ...
                            // this will serve as indicator where we should restart sync in case if
                            // sync retry occurs
                        ...
                        if (this.uids.contains(uid)) {
                            if (this.skipToLastProcessedPerson) {
                                LDAPUserRegistry.logger.info(
                                        "Skipping already synchronized person during sync retry - "
                                                + uid);

                                if (uid.equals(this.lastProcessedPerson)) {
                                    // MNT-14001 fix, it looks like we already reached last
                                    // processed person
                                ...
                                LDAPUserRegistry.logger.warn(
                                        "Duplicate uid found - there will be more than one person"
                        ...
                        NodeDescription nodeDescription =
                                mapToNode(
                                        LDAPUserRegistry.this.personAttributeMapping,
                    ...
                    // Examine the paged results control response for an indication that another
                    // page is available
                    if (!readyForNextPage) {
                        readyForNextPage =
                                LDAPUserRegistry.this.ldapInitialContextFactory.hasNextPage(
                                        this.ctx, LDAPUserRegistry.this.queryBatchSize);

                        if (readyForNextPage) {
                            // MNT-14001 fix, next page available - remember last paged results
                            // control
                            // using cookie from this control we can restart search from current
                            // position if needed
                            ...
                            if (controls != null) {
                                for (Control control : controls) {
                                    if (control instanceof PagedResultsResponseControl) {
                                        this.pagedResultsResponseControl =
    ...
    /**
     * An interface for callbacks passed to the {@link LDAPUserRegistry#processQuery(SearchCallback,
    ...
    protected static interface SearchCallback {
         ...
         * @throws NamingException on naming exceptions
         * @throws ParseException on parse exceptions
         */
    ...
    /**
     * An abstract implementation of SearchCallback interface. Responsible for correct release of
    ...
    protected abstract static class AbstractSearchCallback implements SearchCallback {
        ...
        protected abstract void doProcess(SearchResult result)
                throws NamingException, ParseException;
    }
}

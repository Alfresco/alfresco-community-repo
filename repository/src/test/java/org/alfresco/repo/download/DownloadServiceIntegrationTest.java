...
public class DownloadServiceIntegrationTest {
    ...
    public static RuleChain ruleChain =
            RuleChain.outerRule(APP_CONTEXT_INIT).around(TEST_USER).around(STATIC_TEST_NODES);
    ...
    private static final String CUSTOM_FOLDER_MODEL_XML =
            "<model name=\"custom:customModel\""
                + " xmlns=\"http://www.alfresco.org/model/dictionary/1.0\">   <description>Custom"
                + " Model</description>   <author></author>   <version>1.0</version>   <imports>   "
                + "   <import uri=\"http://www.alfresco.org/model/dictionary/1.0\" prefix=\"d\"/>  "
                + "    <import uri=\"http://www.alfresco.org/model/content/1.0\" prefix=\"cm\"/>  "
                + " </imports>   <namespaces>      <namespace uri=\"custom.model\""
                + " prefix=\"custom\"/>   </namespaces>   <types>      <type"
                + " name=\"custom:folderx\">         <title>Custom FolderX</title>        "
                + " <parent>cm:folder</parent>         <properties>            <property"
                + " name=\"custom:invoice\">               <type>d:text</type>           "
                + " </property>         </properties>         <associations>        "
                + " </associations>      </type>   </types>   <aspects>   </aspects></model>";

    @BeforeClass
    public static void init() {
        // Resolve required services
        CHECK_OUT_CHECK_IN_SERVICE =
                APP_CONTEXT_INIT
        ...
        DOWNLOAD_STORAGE =
                APP_CONTEXT_INIT
                        .getApplicationContext()
                        .getBean("downloadStorage", DownloadStorage.class);
        NODE_SERVICE =
        ...
        INTEGRITY_CHECKER =
                APP_CONTEXT_INIT
                        .getApplicationContext()
                        .getBean("integrityChecker", IntegrityChecker.class);
        ...
        REPO_ADMIN_SERVICE =
                APP_CONTEXT_INIT
                        .getApplicationContext()
                        .getBean("RepoAdminService", RepoAdminService.class);
    }
    ...
    public void createContent() {
        ...
        secondaryNode =
                testNodes.createNodeWithTextContent(
                        COMPANY_HOME,
                        "secondaryNodeFile.txt",
                        ContentModel.TYPE_CONTENT,
                        AuthenticationUtil.getAdminUserName(),
                        "Secondary node");
        ChildAssociationRef assoc =
                NODE_SERVICE.addChild(
                        rootFolder,
                        secondaryNode,
        ...
        PERMISSION_SERVICE.setPermission(
                level1Folder2, TEST_USER.getUsername(), PermissionService.ALL_PERMISSIONS, true);
        PERMISSION_SERVICE.setPermission(
                fileToCheckout, TEST_USER.getUsername(), PermissionService.ALL_PERMISSIONS, true);
    ...
    public void createDownload() throws IOException, InterruptedException {
        ...
        final NodeRef downloadNode =
                DOWNLOAD_SERVICE.createDownload(new NodeRef[] {rootFile, rootFolder}, true);
                ...
                new RetryingTransactionCallback<Object>() {
                    ...
                    public Object execute() throws Throwable {
                        Map<QName, Serializable> properties =
                                NODE_SERVICE.getProperties(downloadNode);
                        Assert.assertEquals(
                                Boolean.TRUE, properties.get(DownloadModel.PROP_RECURSIVE));

                        List<AssociationRef> associations =
                                NODE_SERVICE.getTargetAssocs(
                                        downloadNode, DownloadModel.ASSOC_REQUESTED_NODES);
                        for (AssociationRef association : associations) {
                            Assert.assertTrue(
                                    association.getTargetRef().equals(rootFile)
                                            || association.getTargetRef().equals(rootFolder));
                        }

                        Assert.assertTrue(
                                NODE_SERVICE.hasAspect(
                                        downloadNode, ContentModel.ASPECT_INDEX_CONTROL));
                        Assert.assertEquals(
                                Boolean.FALSE, properties.get(ContentModel.PROP_IS_INDEXED));
                        Assert.assertEquals(
                                Boolean.FALSE,
                                properties.get(ContentModel.PROP_IS_CONTENT_INDEXED));
    ...
    private void validateEntries(
            final Set<String> entryNames, final Set<String> expectedEntries, boolean onlyExpected) {
                ...
                new RetryingTransactionCallback<Set<String>>() {
                    ...
                    public Set<String> execute() throws Throwable {
                        ...
                        ContentReader reader =
                                CONTENT_SERVICE.getReader(downloadNode, ContentModel.PROP_CONTENT);
    ...
    // see MNT-20212
    @Test
    public void deleteBeforeDateAsNormalUserFromAllSysDownloadFolders()
        ...
        final NodeRef problematicDuplicateSystemDownloadNode =
                createProblematicDuplicateSystemNode(container, NODE_SERVICE, repositoryHelper);
        ...
        try {
            ...
            try {
                beforeNodeRef =
                        DOWNLOAD_SERVICE.createDownload(new NodeRef[] {level1Folder1}, true);
                testNodes.addNodeRef(beforeNodeRef);
                waitForDownload(beforeNodeRef);
            ...
            moveDownloadedFileToProblematicFolder(
                    fileFolderService, problematicDuplicateSystemDownloadNode, beforeNodeRef);
    ...
    private void deleteDownloadsAndCheckParameters(
            NodeRef beforeNodeRef, NodeRef afterNodeRef, Date beforeTime) {
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() {
                        ...
                        // because when we moved the file (with fileFolderService) the modified date
                        // is updated,
    ...
    private void moveDownloadedFileToProblematicFolder(
            FileFolderService fileFolderService,
            NodeRef problematicDuplicateSystemDownloadNode,
            NodeRef beforeNodeRef) {
        FileInfo fileInfo =
                TRANSACTION_HELPER.doInTransaction(
                        new RetryingTransactionCallback<FileInfo>() {
                            ...
                            public FileInfo execute() {
                                try {
                                    // now move the beforeNodeRef to the
                                    // problematicDuplicateSystemDownloadNode
                                    return fileFolderService.move(
                                            beforeNodeRef,
                                            problematicDuplicateSystemDownloadNode,
                                            null);
                                } catch (Exception e) {
    ...
    private NodeRef createProblematicDuplicateSystemNode(
                        ...
                        return AuthenticationUtil.runAsSystem(
                                new AuthenticationUtil.RunAsWork<NodeRef>() {
                                    ...
                                    public NodeRef doWork() throws Exception {
                                        NodeRef system =
                                                SystemNodeUtils.getSystemContainer(
    ...
    /**
     * This test verifies that a user is given the correct file, when it is checked. The user who
     * checked out the file should get the working copy, while any other user should get the default
     * version.
     *
     * @throws InterruptedException
     */
    ...
    public void workingCopies() throws InterruptedException {
        ...
        validateWorkingCopyFolder(
                preCheckoutExpectedEntries, level1Folder2, TEST_USER.getUsername());
        validateWorkingCopyFolder(
                preCheckoutExpectedEntries, level1Folder2, TEST_USER2.getUsername());
        ...
        try {
            ...
            validateWorkingCopyFolder(
                    postCheckoutExpectedEntries, level1Folder2, TEST_USER.getUsername());
        } finally {
    ...
    private void validateWorkingCopyFolder(
            final Set<String> expectedEntries, final NodeRef folder, final String userID)
            throws InterruptedException {
        ...
        try {
            final NodeRef downloadNode =
                    DOWNLOAD_SERVICE.createDownload(new NodeRef[] {folder}, true);
            testNodes.addNodeRef(downloadNode);
                ...
                new RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        try {
                            ...
                            final List<AssociationRef> assocsList =
                                    NODE_SERVICE.getTargetAssocs(
                                            nodeRef, RegexQNamePattern.MATCH_ALL);
                            Assert.assertEquals(1, assocsList.size());

                            NODE_SERVICE.removeAssociation(
                                    assocsList.get(0).getSourceRef(),
                                    assocsList.get(0).getTargetRef(),
                                    DownloadModel.ASSOC_REQUESTED_NODES);

                            INTEGRITY_CHECKER.checkIntegrity();
                        } catch (Exception ex) {
                            fail(
                                    "The association should have been removed successfully from the"
                                            + " target node.");
                        }
                        return null;
                    }
    ...
    public void checkNormalUsersCanNotAccessSysDownloadFolder() throws Exception {
        ...
        try {
            try {
                ...
                // normal users should not be able to read the properties of this sys:download
                // container folder
            }
            try {
                ...
                // we expect this to happen.
                // normal users should not be able to list the children of this sys:download
                // container folder
            }
        } finally {
            // assuming previous authentication is the system user

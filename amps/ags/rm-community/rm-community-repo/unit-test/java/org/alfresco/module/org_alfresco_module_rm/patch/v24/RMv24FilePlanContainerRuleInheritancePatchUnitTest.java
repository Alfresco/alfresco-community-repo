...
public class RMv24FilePlanContainerRuleInheritancePatchUnitTest {
    ...
    /** Given there are not file plans, When the patch is executed, Then nothing happens */
    @SuppressWarnings("unchecked")
    @Test
    public void noFilePlans() {
        // given
        when(mockedFilePlanService.getFilePlans()).thenReturn(Collections.EMPTY_SET);
    ...
    /**
     * Given there is a file plan, When the patch is executed, Then the file plan containers are
     * updated
     */
    @Test
    public void atLeastOneFilePlan() {
        NodeRef filePlan =
                generateNodeRef(mockedNodeService, RecordsManagementModel.TYPE_FILE_PLAN);
        ...
        // given
        when(mockedFilePlanService.getFilePlans()).thenReturn(Collections.singleton(filePlan));
        when(mockedFilePlanService.getHoldContainer(filePlan)).thenReturn(holdsContainer);
        when(mockedFilePlanService.getTransferContainer(filePlan)).thenReturn(transferContainer);
        ...
        verify(mockedNodeService)
                .addAspect(transferContainer, RuleModel.ASPECT_IGNORE_INHERITED_RULES, null);
        verify(mockedNodeService)
                .addAspect(unfiledRecordsContainer, RuleModel.ASPECT_IGNORE_INHERITED_RULES, null);
    }
}

...
public class DownloadStorage {
  ...
  public void setQueryRegistry(
    NamedObjectRegistry<CannedQueryFactory<? extends Object>> queryRegistry
  ) {
  ...
  public void setNoPermissionCheckNodeService(
    NodeService noPermissionCheckNodeService
  ) {
  ...
  public NodeRef getOrCreateDowloadContainer() {
    ...
    if (downloadsContainer == null) {
      if (log.isInfoEnabled()) log.info(
        "Lazy creating the Downloads System Container "
      );
  ...
  private QName getContainerQName() {
    String name = bootstrap
      .getConfiguration()
      .getProperty("system.downloads_container.childname");
  ...
  public NodeRef createDownloadNode(boolean recursive) {
    ...
    ChildAssociationRef newChildAssoc = noPermissionCheckNodeService.createNode(
      downloadsContainer,
  ...
  public boolean isCancelled(NodeRef downloadNodeRef) {
    ...
    return (Boolean) nodeService.getProperty(
      downloadNodeRef,
      DownloadModel.PROP_CANCELLED
    );
  }
  ...
  public void addNodeToDownload(NodeRef downloadNode, NodeRef nodeToAdd) {
    ...
    if (log.isDebugEnabled()) {
      StringBuilder msg = new StringBuilder();
      msg
        .append("Node added to Download-NodeRef '")
        .append(downloadNode)
  ...
  public DownloadRequest getDownloadRequest(NodeRef downloadNodeRef) {
    ...
    List<AssociationRef> requestedNodes = nodeService.getTargetAssocs(
      downloadNodeRef,
      DownloadModel.ASSOC_REQUESTED_NODES
    );
  ...
  private void validateNode(NodeRef downloadNodeRef) {
    if (
      !nodeService.getType(downloadNodeRef).equals(DownloadModel.TYPE_DOWNLOAD)
    ) {
      throw new IllegalArgumentException(
        "Invalid node type for nodeRef: " + downloadNodeRef
      );
    }
  }
  ...
  public DownloadStatus getDownloadStatus(NodeRef downloadNodeRef) {
    ...
    return new DownloadStatus(
      DownloadStatus.Status.valueOf(
        (String) properties.get(DownloadModel.PROP_STATUS)
      ),
  ...
  public int getSequenceNumber(NodeRef nodeRef) {
    ...
    Serializable sequenceNumber = nodeService.getProperty(
      nodeRef,
      DownloadModel.PROP_SEQUENCE_NUMBER
    );
  ...
  public void updateStatus(NodeRef nodeRef, DownloadStatus status) {
    ...
    nodeService.setProperty(
      nodeRef,
      DownloadModel.PROP_STATUS,
      status.getStatus().toString()
    );
    nodeService.setProperty(
      nodeRef,
      DownloadModel.PROP_DONE,
      new Long(status.getDone())
    );
    nodeService.setProperty(
      nodeRef,
      DownloadModel.PROP_TOTAL,
      new Long(status.getTotal())
    );
    nodeService.setProperty(
      nodeRef,
      DownloadModel.PROP_FILES_ADDED,
      status.getFilesAdded()
    );
    nodeService.setProperty(
      nodeRef,
      DownloadModel.PROP_TOTAL_FILES,
   ...
   */
  public List<List<DownloadEntity>> getDownloadsCreatedBefore(
    Date before,
    int batchSize,
    boolean cleanAllSysDownloadFolders
  ) {
    List<NodeRef> allFoldersToBeCleaned = getAllFoldersToBeCleaned(
    ...
    for (NodeRef folderToBeCleaned : allFoldersToBeCleaned) {
      gatherDownloadFilesToBeCleanedFromFolder(
        childDownloads,
        before,
        batchSize,
        folderToBeCleaned
  ...
  private void gatherDownloadFilesToBeCleanedFromFolder(
    final List<List<DownloadEntity>> childDownloads,
    final Date before,
    final int batchSize,
    final NodeRef folderToBeCleaned
  ) {
    // Grab the factory
    GetDownloadsCannedQueryFactory getDownloadCannedQueryFactory = (GetDownloadsCannedQueryFactory) queryRegistry.getNamedObject(
      "downloadGetDownloadsCannedQueryFactory"
    );

    // Run the canned query
    GetDownloadsCannedQuery cq = (GetDownloadsCannedQuery) getDownloadCannedQueryFactory.getDownloadsCannedQuery(
      folderToBeCleaned,
      before
    );
    ...
    if (batchSize > 0) {
      int i = 0;
      while (
        childDownloads.size() < batchSize && downloadsInThisFolder.size() > i
      ) {
  ...
  private List<NodeRef> getAllFoldersToBeCleaned(
    boolean cleanAllSysDownloadFolders
  ) {
    List<NodeRef> allFoldersToBeCleaned = new ArrayList<NodeRef>();

    if (cleanAllSysDownloadFolders) {
      for (NodeRef nodeRef : SystemNodeUtils.getSystemChildContainers(
        getContainerQName(),
        nodeService,
        repositoryHelper
      )) {

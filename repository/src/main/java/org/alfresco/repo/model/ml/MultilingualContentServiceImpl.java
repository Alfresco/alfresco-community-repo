 ...
 *
 * <p>The basic structure supported is that of a hidden container of type <b>cm:mlContainer</b>
 * containing one or more secondary children of type <b>cm:mlDocument</b>. One of these will have a
 * matching locale and is referred to as the <i>pivot translation</i>. It is also possible to have
 * several transient <b>cm:emptyTranslation</b> instances that live and die with the container until
 * they get their own content.
 *
 * <p>It is not possible to guarantee that there is always a pivot translation available in the set
 * of sibling translations. The strategy is to hide all translations when there isn't a pivot
 * translation available. A background task should be cleaning up the empty or invalid
 * <b>cm:mlContainer</b> instances.
 *
 * @author Derek Hulley
...
public class MultilingualContentServiceImpl implements MultilingualContentService {
    private static final QName QNAME_ASSOC_ML_ROOT =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "multilingualRoot");
    private static final String KEY_ML_CONTAINERS_TO_DELETE =
            "MultilingualContentServiceImpl.mlContainersToDelete";
    ...
    private NodeRef getMLContainerRoot() {
        ...
        List<ChildAssociationRef> assocRefs =
                nodeService.getChildAssocs(
                        rootNodeRef, ContentModel.ASSOC_CHILDREN, QNAME_ASSOC_ML_ROOT);
        if (assocRefs.size() != 1) {
    ...
    private static final QName QNAME_ML_CONTAINER =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "mlContainer");
    private static final QName QNAME_ML_TRANSLATION =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "mlTranslation");
    ...
    private NodeRef makeMLContainer() {
        ...
        nodeService.addAspect(
                mlContainerNodeRef, ContentModel.ASPECT_VERSIONABLE, versionProperties);
    ...
    private NodeRef getMLContainer(NodeRef mlDocumentNodeRef, boolean allowNull) {
        ...
        List<ChildAssociationRef> parentAssocRefs =
     ...
     * @param allowCreate <tt>true</tt> if a <b>cm:mlContainer</b> must be created if on doesn't
     *     exist, otherwise <tt>false</tt> if a parent <b>cm:mlContainer</b> is expected to exist.
    ...
    private NodeRef getOrCreateMLContainer(NodeRef mlDocumentNodeRef, boolean allowCreate) {
        ...
        if (parentAssocRefs.size() == 0) {
            ...
            logger.warn(
                    "Cleaning up multiple multilingual containers on node: " + mlDocumentNodeRef);
            ChildAssociationRef toKeepAssocRef = parentAssocRefs.get(0);
            mlContainerNodeRef = toKeepAssocRef.getParentRef();
    ...
    private NodeRef makeTranslationImpl(
            NodeRef mlContainerNodeRef, NodeRef contentNodeRef, Locale locale) {
        ...
        boolean forceNewVersion =
                nodeService.hasAspect(contentNodeRef, ContentModel.ASPECT_VERSIONABLE);
        ...
        if (mlContainerNodeRef == null) {
            ...
            Serializable containerFunctionalName =
            ...
            nodeService.setProperty(
                    mlContainerNodeRef, ContentModel.PROP_NAME, containerFunctionalName);
        } else {
        ...
        // Make sure that we don't delete the container if it was previously scheduled for
        // pre-commit deletion.
    ...
    private boolean isPivotTranslation(NodeRef contentNodeRef) {
        ...
        Locale containerLocale =
                (Locale) nodeService.getProperty(containerNodeRef, ContentModel.PROP_LOCALE);
    ...
    public void deleteTranslationContainer(NodeRef mlContainerNodeRef) {
        if (!ContentModel.TYPE_MULTILINGUAL_CONTAINER.equals(
                nodeService.getType(mlContainerNodeRef))) {
            throw new IllegalArgumentException(
    ...
    private void unmakeTranslationSimple(NodeRef translationNodeRef) {
        try {
            ...
            if (nodeService.hasAspect(
                    translationNodeRef, ContentModel.ASPECT_MULTILINGUAL_EMPTY_TRANSLATION)) {
    ...
    public void unmakeTranslation(NodeRef translationNodeRef) {
        ...
        // Turns the document from a translation into a normal document by removing MLDocument
        // aspect
    ...
    public void addTranslation(
            NodeRef newTranslationNodeRef, NodeRef translationOfNodeRef, Locale locale) {
        // Get the container
        NodeRef mlContainerNodeRef = null;

        if (ContentModel.TYPE_MULTILINGUAL_CONTAINER.equals(
                nodeService.getType(translationOfNodeRef))) {
    ...
    /** {@inheritDoc} */
    public List<Locale> getMissingTranslations(
            NodeRef localizedNodeRef, boolean addThisNodeLocale) {
        List<Locale> foundLocales =
                new ArrayList<Locale>(getTranslations(localizedNodeRef).keySet());
        ...
        if (addThisNodeLocale) {
            Locale localeNode =
                    (Locale) nodeService.getProperty(localizedNodeRef, ContentModel.PROP_LOCALE);
    ...
    public NodeRef addEmptyTranslation(NodeRef translationOfNodeRef, String name, Locale locale) {
        ...
        NodeRef newTranslationNodeRef =
                fileFolderService.create(parentNodeRef, name, newTranslationType).getNodeRef();

        // add the translation to the container
        addTranslation(newTranslationNodeRef, translationOfNodeRef, locale);

        // Although the content is spoofed from the pivot translation, it isn't done for all
        // services
        // TODO: Fix http://issues.alfresco.com/browse/AR-1487
        ContentData translationOfContentData =
                (ContentData)
                        nodeService.getProperty(translationOfNodeRef, ContentModel.PROP_CONTENT);
        if (translationOfContentData != null) {
            ContentData newTranslationContentData =
                    new ContentData(
                            null,
            ...
            nodeService.setProperty(
                    newTranslationNodeRef, ContentModel.PROP_CONTENT, newTranslationContentData);
        }

        // set it empty
        nodeService.addAspect(
                newTranslationNodeRef, ContentModel.ASPECT_MULTILINGUAL_EMPTY_TRANSLATION, null);
        // Initially, the file should be temporary.  This will be changed as soon as some content is
        // added.
        nodeService.addAspect(newTranslationNodeRef, ContentModel.ASPECT_TEMPORARY, null);

        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Added an empty translation: \n"
                            + "   Translation of:  "
                            + translationOfNodeRef
                            + "\n"
                            + "   New translation: "
    ...
    /** @inheritDoc */
    public NodeRef copyTranslationContainer(
            NodeRef mlContainerNodeRef, NodeRef newParentRef, String prefixName) throws Exception {
        ...
        NodeRef spaceBefore =
                nodeService
                        .getPrimaryParent(getPivotTranslation(mlContainerNodeRef))
                        .getParentRef();

        if (spaceBefore.equals(newParentRef)) {
            throw new AlfrescoRuntimeException(
                    "Impossible to copy the mlContainer, source folder is the same as the"
        ...
        Locale pivotLocale =
                (Locale) nodeService.getProperty(pivotNodeRef, ContentModel.PROP_LOCALE);
        String pivotName =
                prefixName + (String) nodeService.getProperty(pivotNodeRef, ContentModel.PROP_NAME);
        ...
        pivotCopyNodeRef =
                fileFolderService.copy(pivotNodeRef, newParentRef, pivotName).getNodeRef();
                    ...
                    prefixName
                            + (String)
                                    nodeService.getProperty(
                                            translationNodeRef, ContentModel.PROP_NAME);
            ...
            if (!translationNodeRef.equals(pivotNodeRef)) {
                if (nodeService.hasAspect(
                    ...
                    try {
                        ...
                        NodeRef copyNodeRef =
                                fileFolderService
                                        .copy(translationNodeRef, newParentRef, name)
                                        .getNodeRef();
                    ...
                    nodeService.setProperty(
                            copyNodeRef, ContentModel.PROP_LOCALE, translationLocale);
                }
            } else {
    ...
    private class MLContainerCleaner extends TransactionListenerAdapter {
        ...
        public void beforeCommit(boolean readOnly) {
            ...
            Set<NodeRef> mlContainerNodeRefs =
                    TransactionalResourceHelper.getSet(KEY_ML_CONTAINERS_TO_DELETE);

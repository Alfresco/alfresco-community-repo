...
public class ProcessesImpl extends WorkflowRestImpl implements Processes {
  ...
  protected static final Set<String> PROCESS_STATUS_LIST = new HashSet<String>(
    Arrays.asList(
      PROCESS_STATUS_ANY,
  ...
  protected static final Set<String> PROCESS_COLLECTION_EQUALS_QUERY_PROPERTIES = new HashSet<String>(
    Arrays.asList(
      "processDefinitionId",
      "businessKey",
  ...
  protected static final Set<String> PROCESS_COLLECTION_GREATERTHAN_QUERY_PROPERTIES = new HashSet<String>(
    Arrays.asList("startedAt", "endedAt")
  );

  protected static final Set<String> PROCESS_COLLECTION_LESSTHAN_QUERY_PROPERTIES = new HashSet<String>(
    Arrays.asList("startedAt", "endedAt")
  );

  protected static final Set<String> PROCESS_COLLECTION_SORT_PROPERTIES = new HashSet<String>(
    Arrays.asList(
      "processDefinitionId",
      "businessKey",
      "id",
  ...
  public void setWorkflowPackageComponent(
    WorkflowPackageImpl workflowPackageComponent
  ) {
  ...
  protected ActivitiUtil getActivitiUtil() {
    if (activitiUtil == null) {
      activitiUtil =
        new ActivitiUtil(activitiProcessEngine, deployWorkflowsInTenant);
    }
    return activitiUtil;
  }
  ...
  protected WorkflowPropertyHandlerRegistry getHandlerRegistry() {
    if (handlerRegistry == null) {
      handlerRegistry =
        new WorkflowPropertyHandlerRegistry(
  ...
  @Override
  public CollectionWithPagingInfo<ProcessInfo> getProcesses(
    ...
    String businessKey = propertyWalker.getProperty(
      "businessKey",
      WhereClauseParser.EQUALS
    );
    ...
    String startUserId = propertyWalker.getProperty(
      "startUserId",
      WhereClauseParser.EQUALS
    );
    ...
    Boolean includeVariables = propertyWalker.getProperty(
      "includeVariables",
      WhereClauseParser.EQUALS,
      Boolean.class
    );
    ...
    if (processDefinitionKey != null) {
      if (tenantService.isEnabled() && deployWorkflowsInTenant) {
        if (
          processDefinitionKey.startsWith(
          ...
          query.processDefinitionKey(
            "@" + TenantUtil.getCurrentDomain() + "@" + processDefinitionKey
          );
        }
      } else {
        query.processDefinitionKey(processDefinitionKey);
    ...
    if (variableProperties != null) {
      for (QueryVariableHolder queryVariableHolder : variableProperties) {
        if (queryVariableHolder.getOperator() == WhereClauseParser.EQUALS) {
          ...
          query.variableValueGreaterThan(
            queryVariableHolder.getPropertyName(),
            queryVariableHolder.getPropertyValue()
          );
        } else if (
          ...
          if (
            ((String) queryVariableHolder.getPropertyValue()).startsWith("(?i)")
          ) {
            query.variableValueLikeIgnoreCase(
              queryVariableHolder.getPropertyName(),
              ((String) queryVariableHolder.getPropertyValue()).substring(
                  "(?i)".length()
                )
                .toLowerCase()
            );
          } else {
            query.variableValueLike(
              queryVariableHolder.getPropertyName(),
    ...
    if (sortColumn != null) {
      if (PROCESS_COLLECTION_SORT_PROPERTIES.contains(sortColumn.column)) {
        ...
        throw new InvalidArgumentException(
          ...
          Arrays.toString(PROCESS_COLLECTION_SORT_PROPERTIES.toArray())
        );
      }
    ...
    List<HistoricProcessInstance> processInstances = query.listPage(
      paging.getSkipCount(),
      paging.getMaxItems()
    );
    int totalCount = (int) query.count();

    List<ProcessInfo> page = new ArrayList<ProcessInfo>(
    ...
    for (HistoricProcessInstance processInstance : processInstances) {
      ...
      if (includeVariables != null && includeVariables) {
        if (
          definitionTypeMap.containsKey(processInfo.getProcessDefinitionId()) ==
          false
        ) {
          StartFormData startFormData = activitiProcessEngine
            .getFormService()
            .getStartFormData(processInfo.getProcessDefinitionId());
        ...
        if (
          ...
          List<Variable> resultingVariables = restVariableHelper.getVariables(
            ...
            definitionTypeMap.get(processInfo.getProcessDefinitionId())
          );
  ...
  public ProcessInfo getProcess(String processId) {
    if (processId == null) {
      throw new InvalidArgumentException(
        "processId is required to get the process info"
      );
    }
  ...
  public ProcessInfo create(ProcessInfo process) {
    if (process == null) {
      throw new InvalidArgumentException(
        "post body expected when starting a new process instance"
      );
    }
    ...
    if (process.getProcessDefinitionId() != null) {
      ...
      ProcessDefinition definition = activitiProcessEngine
        ...
        .processDefinitionKey(
          getProcessDefinitionKey(process.getProcessDefinitionKey())
        )
        .latestVersion()
        .singleResult();
      ...
      if (definition == null) {
        throw new InvalidArgumentException(
          "No workflow definition could be found with key '" +
      ...
      throw new InvalidArgumentException(
        "Either processDefinitionId or processDefinitionKey is required"
      );
    }
    ...
    if (definitionExistingChecked == false) {
      ...
      if (query.count() == 0) {
        throw new InvalidArgumentException(
          "No workflow definition could be found with id '" +
          processDefinitionId +
          "'."
        );
      }
    ...
    if (startFormData != null) {
      if (CollectionUtils.isEmpty(process.getVariables()) == false) {
        TypeDefinition startTaskType = getWorkflowFactory()
        ...
        for (String variableName : process.getVariables().keySet()) {
          if (propNameMap.containsKey(variableName)) {
            ...
            if (taskAssociations.containsKey(propNameMap.get(variableName))) {
              AssociationDefinition associationDef = taskAssociations.get(
                propNameMap.get(variableName)
              );
              variableValue =
              ...
              taskProperties.containsKey(propNameMap.get(variableName))
            ) {
              PropertyDefinition propDef = taskProperties.get(
                propNameMap.get(variableName)
              );
              DataTypeDefinition propDataType = propDef.getDataType();
              if (
                "java.util.Date".equalsIgnoreCase(
                    propDataType.getJavaClassName()
                  )
              ) {
    ...
    try {
      ...
      throw new ApiException(
        "couldn't create workflow package: " + e.getMessage(),
        e
      );
    }
      ...
      try {
        for (String item : process.getItems()) {
          ...
          QName workflowPackageItemId = QName.createQName(
            "wpi",
            itemNodeRef.toString()
          );
          nodeService.addChild(
            workflowPackageNodeRef,
            itemNodeRef,
        ...
        throw new ApiException(
          "Error while adding items to package: " + e.getMessage(),
          e
        );
      }
    }
    ...
    Map<String, Object> variables = getPropertyConverter()
      .getStartVariables(processDefinitionId, startParams);
    variables.put(WorkflowConstants.PROP_CANCELLED, Boolean.FALSE);

    // Add company home
    Object companyHome = getNodeConverter()
      .convertNode(repositoryHelper.getCompanyHome());
  ...
  public void deleteProcess(String id) {
    ...
    ProcessInstance processInstance = activitiProcessEngine
      .getRuntimeService()
    ...
    activitiProcessEngine
      .getRuntimeService()
      .deleteProcessInstance(id, "deleted through REST API call");
  }

  @Override
  public CollectionWithPagingInfo<Item> getItems(
    String processId,
    Paging paging
  ) {
    if (processId == null) {
      throw new InvalidArgumentException(
        "processId is required to get the attached items of a process instance"
      );
    }
  ...
  public Item getItem(String processId, String itemId) {
    if (processId == null) {
      throw new InvalidArgumentException(
        "processId is required to get the attached item of a process instance"
      );
    }

    if (itemId == null) {
      throw new InvalidArgumentException(
        "itemId is required to get an attached item"
      );
    }
  ...
  public Item createItem(String processId, Item item) {
    if (processId == null) {
      throw new InvalidArgumentException(
        "processId is required to add an attached item to a process instance"
      );
    }

    if (item == null || item.getId() == null) {
      throw new InvalidArgumentException(
        "itemId is required to add an attached item"
      );
    }
  ...
  public void deleteItem(String processId, String itemId) {
    if (processId == null) {
      throw new InvalidArgumentException(
        "processId is required to delete an attached item to a process instance"
      );
    }

    if (itemId == null) {
      throw new InvalidArgumentException(
        "itemId is required to delete an attached item"
      );
    }
  ...
  @Override
  public CollectionWithPagingInfo<Variable> getVariables(
    String processId,
    Paging paging
  ) {
    ...
    List<HistoricVariableInstance> variableInstances = validateIfUserAllowedToWorkWithProcess(
      processId
    );
    ...
    if (processInstance != null) {
      ...
      HistoricProcessInstance historicInstance = activitiProcessEngine
        .getHistoryService()
        .createHistoricProcessInstanceQuery()
        .processInstanceId(processId)
        .singleResult();
  ...
  public Variable updateVariable(String processId, Variable variable) {
    ...
    return updateVariableInProcess(
      processId,
  ...
  @Override
  public List<Variable> updateVariables(
    String processId,
  ...
  protected Variable updateVariableInProcess(
    String processId,
    ...
    if (context.getPropertyDefinition(variable.getName()) != null) {
      dataTypeDefinition =
        context.getPropertyDefinition(variable.getName()).getDataType();
      if (
    ...
    if (dataTypeDefinition == null && variable.getType() != null) {
      try {
        ...
        throw new InvalidArgumentException(
          "Unsupported type of variable: '" + variable.getType() + "'."
        );
      }
      ...
      dataTypeDefinition =
        dictionaryService.getDataType(
          restVariableHelper.extractTypeFromValue(variable.getValue())
        );
    }
    ...
    if (
      ...
      if (nodeService.exists(initiator)) {
        ...
        throw new InvalidArgumentException(
          "Variable value should be a valid person NodeRef."
        );
      }
    } else {
      if (context.getAssociationDefinition(variable.getName()) != null) {
        actualValue =
          convertAssociationDefinitionValue(
            context.getAssociationDefinition(variable.getName()),
            variable.getName(),
            variable.getValue()
          );
      } else {
    ...
    activitiProcessEngine
      .getRuntimeService()
      .setVariable(processId, variable.getName(), actualValue);
  ...
  public void deleteVariable(String processId, String variableName) {
    ...
    try {
      if (
        activitiProcessEngine
          .getRuntimeService()
          .hasVariable(processId, variableName) ==
        false
      ) {
      ...
      activitiProcessEngine
        .getRuntimeService()
        .removeVariable(processId, variableName);
  ...
  public BinaryResource getProcessImage(String processId) {
    ...
    try {
      BpmnModel model = activitiProcessEngine
        .getRepositoryService()
        .getBpmnModel(processInstance.getProcessDefinitionId());

      if (model != null && model.getLocationMap().size() > 0) {
        List<String> activeActivities = activitiProcessEngine
          .getRuntimeService()
          .getActiveActivityIds(processId);
        ProcessDiagramGenerator generator = new DefaultProcessDiagramGenerator();
        InputStream generateDiagram = generator.generateDiagram(
          model,
          "png",
          activeActivities
        );
  ...
  protected String getProcessDefinitionKey(String paramProcessDefinitionKey) {
    ...
    if (tenantService.isEnabled() && deployWorkflowsInTenant) {
      processDefinitionKey =
        "@" + TenantUtil.getCurrentDomain() + "@" + paramProcessDefinitionKey;
    } else {
  ...
  protected ProcessInfo createProcessInfo(
    HistoricProcessInstance processInstance
  ) {
    ProcessInfo processInfo = new ProcessInfo(processInstance);
    ProcessDefinition definitionEntity = activitiProcessEngine
      .getRepositoryService()
      .getProcessDefinition(processInstance.getProcessDefinitionId());
    processInfo.setProcessDefinitionKey(
      getLocalProcessDefinitionKey(definitionEntity.getKey())
    );
    return processInfo;
  }
}

...
public class RuntimeExecTest extends TestCase {
  ...
  public void testStreams() throws Exception {
    ...
    commandMap.put(
      "*",
      new String[] { "find", "/", "-maxdepth", "1", "-name", "var" }
    );
  ...
  public void testWildcard() throws Exception {
    ...
    assertTrue(
      "Expected default match to work",
      Arrays.deepEquals(new String[] { "TEST" }, commandStr)
    );
  }
  ...
  public void testWithProperties() throws Exception {
    ...
    String dynamicCommand[] = exec.getCommand(
    ...
    assertTrue(
      "Default command for OS " + os + " is incorrect",
      Arrays.deepEquals(defaultCommandCheck, defaultCommand)
    );
    assertTrue(
      "Dynamic command for OS " + os + " is incorrect",
      Arrays.deepEquals(dynamicCommandCheck, dynamicCommand)
    );
  }
  ...
  private void longishRunningProcess(int runFor, long timeout) {
    ...
    assertTrue(
      "The timeout when set must be more than " + marginOfError + "ms",
      shouldComplete || timeout >= marginOfError
    );
    assertTrue(
      "The timeout when set plus " +
      marginOfError +
      "ms must less than the runFor value",
      shouldComplete || timeout + marginOfError <= runFor
    );
    ...
    commandMap.put(
      "Windows.*",
      new String[] { "ping", "-n", "" + (runFor / 1000 + 1), "127.0.0.1" }
    ...
    if (shouldComplete) assertEquals(
      "Didn't expect error code",
      0,
      ret.getExitValue()
    ); else assertFalse("Didn't expect success code", 0 == ret.getExitValue());
  }
}

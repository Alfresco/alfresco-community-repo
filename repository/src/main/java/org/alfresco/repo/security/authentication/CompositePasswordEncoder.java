...
public class CompositePasswordEncoder {
  ...
  public boolean isSafeToEncodeChain(List<String> encodingChain) {
    if (encodingChain != null && encodingChain.size() > 0) {
      ...
      for (String encoderKey : encodingChain) {
        ...
        if (encoder == null) throw new AlfrescoRuntimeException(
          "Invalid encoder specified: " + encoderKey
        );
        if (
          encoder instanceof org.springframework.security.crypto.password.PasswordEncoder
        ) {
          //BCRYPT uses its own internal salt so its NOT safe to use it more than once in an encoding chain.
      ...
      if (
        unsafeEncoders.size() == 1 &&
        unsafeEncoders
          .get(0)
          .equals(encodingChain.get(encodingChain.size() - 1))
      ) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          ...
          ". Only 1 non-upgradable encoder is allowed at the end of the chain: " +
  ...
  public void init() {
    ...
    if (
      !encoders.containsKey(preferredEncoding)
  ...
  public String encodePassword(
    ...
    if (encoded == rawPassword) throw new AlfrescoRuntimeException(
      "No password encoding specified. " + encodingChain
    );
    return encoded;
  }
  ...
  protected String encode(String encoderKey, String rawPassword, Object salt) {
    ...
    if (
      encoder instanceof net.sf.acegisecurity.providers.encoding.PasswordEncoder
    ) {
    ...
    throw new AlfrescoRuntimeException(
      "Unsupported encoder specified: " + encoderKey
    );
  }
  ...
  public boolean matchesPassword(
    ...
    if (encodingChain.size() > 1) {
      ...
      String encoded = encodePassword(
        rawPassword,
        salt,
        encodingChain.subList(0, encodingChain.size() - 1)
      );
  ...
  protected boolean matches(
    ...
    Object encoder = encoders.get(encoderKey);
    if (encoder == null) throw new AlfrescoRuntimeException(
      "Invalid matches encoder specified: " + encoderKey
    );
    if (
      encoder instanceof net.sf.acegisecurity.providers.encoding.PasswordEncoder
    ) {
    ...
    throw new AlfrescoRuntimeException(
      "Unsupported encoder for matching: " + encoderKey
    );
  }
}

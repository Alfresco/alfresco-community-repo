...
public class LDAPUserRegistryTest {
  ...
  private static final String GROUP_DIFFERENTIAL_QUERY =
    "(objectclass=groupOfNames)";
  private static final String PERSON_DIFFERENTIAL_QUERY =
  ...
  private static final String MEMBER_ATTRIBUTE_VALUE =
    "cn=foouser10,cn=Users,dc=example,dc=foo";
  ...
  private LDAPUserRegistry createRegistry() throws Exception {
    ...
    when(namespaceService.getPrefixes(NamespaceService.CONTENT_MODEL_1_0_URI))
      .thenReturn(prefixes);
    when(
      namespaceService.getNamespaceURI(NamespaceService.CONTENT_MODEL_PREFIX)
    )
      .thenReturn(NamespaceService.CONTENT_MODEL_1_0_URI);
    when(contextFactory.getDefaultIntialDirContext())
      .thenReturn(initialDirContext);
    when(contextFactory.getDefaultIntialDirContext(0))
      .thenReturn(initialDirContext);
    when(
      initialDirContext.search(
        eq(GROUP_SEARCH_BASE),
        eq(GROUP_DIFFERENTIAL_QUERY),
        any()
      )
    )
    ...
    when(attributes.get(MEMBER_ATTRIBUTE_NAME))
      .thenReturn(rangeRestrictedAttribute);
  ...
  public void testTimeoutDuringSync() throws Exception {
    ...
    when(
      initialDirContext.getAttributes(
        eq(LDAPUserRegistry.jndiName(MEMBER_ATTRIBUTE_VALUE)),
        any()
      )
    )
      .thenThrow(
   ...
   */
  @Test
  public void testTimeoutDuringSyncForServiceUnavailableException()
    throws Exception {
    LDAPUserRegistry userRegistry = createRegistry();
   ...
   */
  @Test
  public void testTimeoutDuringSyncForCommunicationException()
    throws Exception {
    LDAPUserRegistry userRegistry = createRegistry();

...
public class GetPeopleCannedQuery extends AbstractCannedQuery<NodeRef> {
  ...
  private static final String QUERY_SELECT_GET_PEOPLE =
    "select_GetPeopleCannedQuery";
  ...
  protected List<NodeRef> queryAndFilter(CannedQueryParameters parameters) {
    ...
    if (paramBean.getExclusiveAspects() != null) {
      Set<Long> qnamesIds = qnameDAO.convertQNamesToIds(
        paramBean.getExclusiveAspects(),
        false
      );
    ...
    Map<QName, Boolean> sortAsc = new HashMap<QName, Boolean>(
    ...
    final PersonQueryCallback c = new DefaultPersonQueryCallback(
      result,
      paramBean.getIncludeAdministrators()
    );
    ...
    int origLimit = totalResultCountMax > 0
      ? totalResultCountMax

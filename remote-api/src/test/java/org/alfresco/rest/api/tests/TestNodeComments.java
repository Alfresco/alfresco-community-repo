...
public class TestNodeComments extends EnterpriseTestApi {
  ...
  public void testNodeComments() throws Exception {
    ...
    // multi-byte characters, create and update comments
    {
      ...
      for (Comment comment : multiByteComments) {
        ...
        Comment ret = commentsProxy.createNodeComment(
          nodeRef2.getId(),
          comment
        );
      ...
      int maxItems = 2;
      Paging paging = getPaging(skipCount, maxItems);
      ListResponse<Comment> resp = commentsProxy.getNodeComments(
        nodeRef2.getId(),
        createParams(paging, null)
      );
...

    {
      ...
      for (Comment comment : comments) {
        ...
        Comment resp = commentsProxy.createNodeComment(
          nodeRef1.getId(),
          comment
        );
        // check response
        ...
        assertEquals(
          resp.getCreatedBy().getDisplayName(),
          person11.getDisplayName()
        );
...

      {
        ...
        ListResponse<Activity> activities = peopleProxy.getActivities(
          person11.getId(),
          createParams(paging, null)
        );
    ...
    try {
      ...
      Comment newComment = commentsProxy.createNodeComment(
        nodeRef1.getId(),
        comment
      );
    ...
    // update comments
    {
      ...
      for (Comment comment : createdComments.values()) {
        ...
        Comment ret = commentsProxy.updateNodeComment(
          nodeRef4.getId(),
          comment.getId(),
          updateComment
        );
    ...
    // successful update
    {
      ...
      Comment resp = commentsProxy.updateNodeComment(
        nodeRef1.getId(),
        comment.getId(),
        update
      );
    ...
    try {
      ...
      Comment createdComment = commentsProxy.createNodeComment(
        nodeRef1.getId(),
        comment
      );
  ...
  public void testNodeCommentsAndLocking() throws Exception {
    ...
    try {
      ...
      try {
        commentsProxy.removeNodeComment(
          nodeRef1.getId(),
          createdComment.getId()
        );
  ...
  public void testSkipCountHighValue(

...
public class GetSiteTests extends RestTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify user with Manager role gets site information and gets status code OK"
                            + " (200)")
    ...
    @TestRail(
            ...
            description =
                    "Verify user with Collaborator role gets site information and gets status code"
                            + " OK (200)")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user with Contributor role gets site information and gets status code"
                            + " OK (200)")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user with Consumer role gets site information and gets status code OK"
                            + " (200)")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user with admin role gets site information and gets status code OK"
                            + " (200)")
    ...
    public void unauthenticatedUserIsNotAuthorizedToRetrieveSite() throws Exception {
        ...
        restClient
                .authenticateUser(unauthenticatedManager)
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user gets all public and moderated sites if an empty siteId is"
                            + " provided")
    public void checkStatusCodeForEmptySiteId() throws Exception {
        ...
        RestSiteModelsCollection sites =
                restClient.processModels(RestSiteModelsCollection.class, request);
        restClient.assertStatusCodeIs(HttpStatus.OK);
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify if member of a private site gets that site details and status code is"
                            + " 200")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify if user that is not member of a private site does not get that site"
                            + " details and status code is 200")
    ...
    @Test(
            groups = {TestGroup.REST_API, TestGroup.SITES, TestGroup.REGRESSION},
            expectedExceptions = java.lang.AssertionError.class)
    @TestRail(
            section = {TestGroup.REST_API, TestGroup.SITES},
            executionType = ExecutionType.REGRESSION,
            description =
                    "Check that properties parameter is applied (guid field is mandatory, thus"
                            + " assertion error is expected)")
    public void checkThatPropertiesParameterIsApplied() throws Exception {
        restSiteModel =
                restClient
                        .authenticateUser(adminUserModel)
                        .withParams("properties=id, visibility")
                        .withCoreAPI()
                        .usingSite(publicSite)
                        .getSite();
        restSiteModel
    ...
    public void checkThatPropertiesParameterIsAppliedPositiveTest() throws Exception {
        restSiteModel =
                restClient
                        .authenticateUser(adminUserModel)
                        .withParams("properties=id,guid,title,visibility")
                        .withCoreAPI()
                        .usingSite(publicSite)
                        .getSite();
        restSiteModel
    ...
    public void deleteSiteThenGetSiteDetails() throws Exception {
        ...
        restClient
                .assertStatusCodeIs(HttpStatus.NOT_FOUND)
                .assertLastError()
                .containsSummary(String.format(RestErrorModel.ENTITY_NOT_FOUND, newSite.getId()))
                .containsErrorKey(RestErrorModel.ENTITY_NOT_FOUND_ERRORKEY)
                .descriptionURLIs(RestErrorModel.RESTAPIEXPLORER)
                .stackTraceIs(RestErrorModel.STACKTRACE);
    }
    ...
    public void updateSiteVisibilityToPrivateThenGetSite() throws Exception {
        ...
        restSiteModel
                ...
                .and()
                .field("visibility")
                .is(SiteService.Visibility.PRIVATE.toString());
    }
    ...
    public void checkThatRelationsParameterIsAppliedForContainers() throws Exception {
        ...
        RestSiteContainerModelsCollection containers =
                (RestSiteContainerModelsCollection) jsonObjects.get(1);
    ...
    public void checkThatRelationsParameterIsAppliedForMembers() throws Exception {
        ...
        List<Object> jsonObjects =
                restClient
                        ...
                        .withCoreAPI()
                        .usingSite(publicRandomSite)
                        .getSiteWithRelations();

        RestSiteModel siteModel = (RestSiteModel) jsonObjects.get(0);
        RestSiteMemberModelsCollection siteMembers =
                (RestSiteMemberModelsCollection) jsonObjects.get(1);

        siteModel
                .assertThat()
                .field("visibility")
                .is(publicRandomSite.getVisibility())
                .and()
                .field("id")
                .is(publicRandomSite.getId())
                .and()
                .field("description")
                .is(publicRandomSite.getDescription())
                .and()
                .field("title")
                .is(publicRandomSite.getTitle())
                .and()
                .field("preset")
                .is("site-dashboard")
                .and()
    ...
    public void checkThatRelationsParameterIsAppliedForMembersCustom() {
        ...
        response.assertThat()
                .body("relations.members.list.entries.entry[0].role", equalTo("SiteManager"));
    }
    ...
    public void checkThatRelationsParameterIsAppliedForContainersAndMembers() throws Exception {
        ...
        RestSiteContainerModelsCollection containers =
                (RestSiteContainerModelsCollection) jsonObjects.get(1);
        RestSiteMemberModelsCollection siteMembers =
                (RestSiteMemberModelsCollection) jsonObjects.get(2);

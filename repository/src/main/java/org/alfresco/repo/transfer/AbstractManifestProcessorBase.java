...
/**
 * This abstract class handles the progress monitoring functionality as well as providing some
...
public abstract class AbstractManifestProcessorBase
    ...
    /**
     * Given the node ref, this method constructs the appropriate ChildAssociationRef that would
     * place this node in the transfer's temporary folder. Useful when handling orphans.
     *
    ...
    protected ChildAssociationRef getTemporaryLocation(NodeRef nodeRef) {
        ...
        QName parentAssocName =
                QName.createQName(NamespaceService.APP_MODEL_1_0_URI, nodeRef.getId());
        return new ChildAssociationRef(
                parentAssocType, parentNodeRef, parentAssocName, nodeRef, true, -1);
    }
    ...
    private void handleException(TransferManifestNode node, Throwable ex) {
        ...
        try {
            ...
            String message =
                    (node != null)
                            ? "Error while processing incoming node " + node.getNodeRef()
    ...
    /**
     * This method is invoked if an exception or error occurs while processing the manifest. By
     * default it does nothing, but classes that extend this class can override this to provide
     * custom clean-up.
     *
    ...
    protected void logCreated(
            NodeRef sourceNode,
            NodeRef destNode,
            NodeRef newParentNode,
            String parentPath,
            boolean orphan) {
        receiver.getProgressMonitor()
                .logCreated(transferId, sourceNode, destNode, newParentNode, parentPath, orphan);
    }
    ...
    protected void logMoved(
            ...
            String newPath) {
        receiver.getProgressMonitor()
                .logMoved(transferId, sourceNode, destNode, oldPath, newParent, newPath);
    }
     ...
     */
    protected void logInvasionHierarchy(
            NodeRef parentRef, NodeRef childRef, NodeService nodeService, Log log) {
    ...
    protected void logSummaryCreated(
            NodeRef sourceNode,
            NodeRef destNode,
            NodeRef newParentNode,
            String parentPath,
        ...
        TransferSummaryReport transferSummaryReport = getTransferSummaryReport();
        if (transferSummaryReport != null) {
            transferSummaryReport.logSummaryCreated(
                    sourceNode, destNode, newParentNode, parentPath, orphan);
        }
    }
    ...
    protected void logSummaryMoved(
            NodeRef sourceNode,
            NodeRef destNode,
            String oldPath,
            NodeRef newParent,
        ...
        TransferSummaryReport transferSummaryReport = getTransferSummaryReport();
        if (transferSummaryReport != null) {
            transferSummaryReport.logSummaryMoved(
                    sourceNode, destNode, oldPath, newParent, newPath);
        }
    }
}

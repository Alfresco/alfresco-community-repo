 ...
 *
 * <ul>
 *   <li>a {@link Protocol} - defines what kind of virtualzied artefact is referred by this
 *       reference as well as what virtualization process was applied
 *   <li>a {@link Resource} - identifies the main resource used in the virtualization (egg. a
 *       classpath location or repository node reference)
 *   <li>a list of {@link ResourceParameter}s - used in customizing the virtualization process (egg.
 *       a resource pointing to the actual node of a semi-virtual folder reference)
 * </ul>
 *
 * @author Bogdan Horje
 */
public class Reference {
     ...
     * NodeRef Reference representations validated by this method should produce valid Reference
     * objects based on the given {@link NodeRef} when passed to the {@link #fromNodeRef(NodeRef)}
     * method.
     *
     * @deprecated
     * @param nodeRef
     * @return <code>true</code> if the given {@link NodeRef} is a valid Reference representation
     *     <br>
     ...
     * It expects a {@link #VIRTUAL_TOKEN} prefixed encoded string. The encoded string must start
     * with a valid {@link Encoding} token. The Reference representation structure is (no delimiters
     * between the 3 elements): VIRTUAL_TOKEN ENCODING_TOKEN referenceString Given that a valid
     * encoding was detected {@link Encoding#urlNative} information is used to obtain a reference
     * string. The reference string is parsed using the encoding configured parser.
     *
     * @param nodeRef
    ...
    private static synchronized void _trace_avg_ref_length(long refLength) {
        ...
        if (_trace_refBatchSize > 0 && _trace_refCount % _trace_refBatchSize == 0) {
            logger.trace(
                    "Average reference encoding size : " + (_trace_refLength / _trace_refCount));
            _trace_refCount = 0;
            _trace_refLength = 0;
        }
    }
     ...
     * Constructor
     *
     * @param encoding the default {@link Encoding} of the new resource - to be used where an
     *     encoding is required and none is specified
     * @param protocol
     * @param resource
     * @param parameters resource parameters - a copy of the provided list will be stored by this
     *     reference
     */
    public Reference(
            Encoding encoding,
    ...
    /**
     * @return a {@link NodeRef} representation of this resource using the {@link
     *     StoreRef#STORE_REF_WORKSPACE_SPACESSTORE} and the default encoding of this resource
     * @throws ReferenceEncodingException
     */
    ...
    /**
     * @param storeRef
     * @return a {@link NodeRef} representation of this resource using the given {@link StoreRef}
     *     and the default encoding of this resource
     * @throws ReferenceEncodingException
     */
     ...
     * @throws ReferenceEncodingException
     */
    public NodeRef toNodeRef(StoreRef storeRef, Encoding encoding)
            throws ReferenceEncodingException {
     ...
     * Uses {@link Protocol#dispatch(ProtocolMethod, Reference)} to trigger concrete protocol based
     * double dispatch <code>ProtocolMethod::execute</code> invocation on the given method object.
     *
     * @param method
    ...
    /**
     * Despite claimed {@link NodeRef} opacity Alfresco sometimes alters NodeRefs representation to
     * achieve functionality. For example see {@link VersionUtil#convertNodeRef(NodeRef)}.<br>
     * We say that altered {@link NodeRef}s have suffered mutations and we try to detect those
     * mutations and create a correspondent reference.
     *
     * @param mutatedNodeRef
     * @return a mutated version of this {@link Reference} corresponding to the given mutated node
     *     or <code>this</code> Reference if no mutations are detected
     */

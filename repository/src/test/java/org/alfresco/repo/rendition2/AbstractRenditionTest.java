...
public abstract class AbstractRenditionTest
  ...
  public static final List<String> TAS_REST_API_EXCLUDE_LIST =
  ...
  private Set<String> getThumbnailNames(
  ...
  private void assertRenditionsOkayFromSourceExtension(
    ...
    for (String sourceExtension : sourceExtensions) {
      ...
      if (testFileName != null) {
        Set<String> renditionNames = renditionDefinitionRegistry2.getRenditionNamesFrom(
          sourceMimetype,
          -1
        );
        List<ThumbnailDefinition> thumbnailDefinitions = thumbnailRegistry.getThumbnailDefinitions(
          sourceMimetype,
          -1
        );
    ...
    int expectedSuccessCount =
      expectedRenditionCount - excludedCount - expectedFailedCount;
    ...
    System.out.println(
      "   failedCount: " + failedCount + " expected " + expectedFailedCount
    );
    System.out.println(
      "  successCount: " + successCount + " expected " + expectedSuccessCount
    );

    assertEquals(
      "Rendition count has changed",
      expectedRenditionCount,
      renditionCount
    );
    assertEquals(
      "Failed rendition count has changed",
      expectedFailedCount,
      failedCount
    );
    assertEquals(
      "Successful rendition count has changed",
  ...
  private void assertMetadataExtractsOkayFromSourceExtension(
      ...
      if (testFileName != null) {
        ...
        if (excludeList.contains(sourceExtension)) {
          ...
          try {
            checkExtract(
              testFileName,
              !expectedToFail.contains(sourceExtension)
            );
    ...
    int expectedSuccessCount =
      expectedExtractCount - excludedCount - expectedFailedCount;
    System.out.println("FAILURES:\n" + failures + "\n");
    System.out.println("SUCCESSES:\n" + successes + "\n");
    System.out.println(
      "extractCount: " + extractCount + " expected " + expectedExtractCount
    );
    System.out.println(
      " failedCount: " + failedCount + " expected " + expectedFailedCount
    );
    ...
    assertEquals(
      "Extract count has changed",
      expectedExtractCount,
      extractCount
    );
    assertEquals(
      "Failed extract count has changed",
      expectedFailedCount,
      failedCount
    );
    assertEquals(
      "Successful extract count has changed",
      expectedSuccessCount,
      successCount
    );
  }
  ...
  public void testAllSourceExtensions() throws Exception {
    internalTestAllSourceExtensions(
      196,
      0,
      ALL_SOURCE_EXTENSIONS_EXCLUDE_LIST_LEGACY
    );
  }
  ...
  protected void internalTestGifRenditions(
    int expectedRenditionCount,
    int expectedFailedCount
  ) throws Exception {
    assertRenditionsOkayFromSourceExtension(
  ...
  public void testCountTotalTransforms() {
    ...
    System.out.println(
      "Number of source to target mimetype transforms: " + count
    );
    System.out.println(
      "Number of source to plain text transforms: " + textTargetCount
    );
  ...
  public void testCountTotalRenditionTransforms() {
    ...
    for (String sourceMimetype : sourceMimetypes) {
      ...
      for (String renditionName : renditionDefinitionRegistry.getRenditionNamesFrom(
        sourceMimetype,
        1
      )) {
        RenditionDefinition2 renditionDefinition = renditionDefinitionRegistry.getRenditionDefinition(
          renditionName
        );
        String targetMimetype = renditionDefinition.getTargetMimetype();
    ...
    System.out.println(
      "Number of source to target mimetype transforms via renditions: " +
      count.get()
    );
    System.out.println(sourceTargetList);
  }
  ...
  private void logSourceTarget(
    ...
    if (sourceTargetPriorityList != null) {
      ...
      if (registry != null) {
        Map<String, List<SupportedTransform>> supportedTransformsByTargetMimetype = registry
          .getData()
          .retrieveTransforms(sourceMimetype);
        List<SupportedTransform> supportedTransforms = new ArrayList<>(
          supportedTransformsByTargetMimetype.get(targetMimetype)
        );

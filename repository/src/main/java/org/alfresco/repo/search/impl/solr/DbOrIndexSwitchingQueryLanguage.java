...
public class DbOrIndexSwitchingQueryLanguage
  ...
  protected static final Log logger = LogFactory.getLog(
    DbOrIndexSwitchingQueryLanguage.class
  );
    ...
    switch (consistency) {
      ...
      case TRANSACTIONAL:
        if (dbQueryLanguage != null) {
          ...
          stopWatch.start();
          ResultSet results = dbQueryLanguage.executeQuery(
            flattenDBQuery(searchParameters)
          );
          stopWatch.stop();
          if (logger.isDebugEnabled()) {
            logger.debug(
        ...
        if (dbQueryLanguage != null && !searchParameters.hasFaceting()) {
          try {
            ...
            if (indexQueryLanguage != null) {
              ...
              ResultSet results = indexQueryLanguage.executeQuery(
                searchParameters
              );
          ...
          if (indexQueryLanguage != null) {
            if (logger.isDebugEnabled()) {
              logger.debug(
                "(No DB QL) Using SOLR query: " +
                "dbQueryLanguage==null" +
                " for " +
                searchParameters
              );
            }
  ...
  private ResultSet executeHybridQuery(SearchParameters searchParameters) {
    if (indexQueryLanguage == null || dbQueryLanguage == null) {
      throw new QueryModelException(
        "Both index and DB query language required for hybrid search [index=" +
        indexQueryLanguage +
        ", DB=" +
        dbQueryLanguage +
        "]"
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Hybrid search, using SOLR query: " +
        dbQueryLanguage.getName() +
        " for " +
        searchParameters
      );
    ...
    if (!(indexResults instanceof SolrJSONResultSet)) {
      if (logger.isWarnEnabled()) {
        logger.warn(
          "Hybrid search can only use database when SOLR is also in use. " +
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Nodes changed since last indexed transaction (ID " +
        lastTxId +
        ") = " +
        changedNodeList.size() +
        " (took " +
        stopWatch.getLastTaskTimeMillis() +
        "ms)"
      );
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(
        "Hybrid search returning combined results with counts: " + stats
      );

...
public class LinksServiceImplTest {
  ...
  public void before() {
    ...
    ((LinksServiceImpl) testContext.getBean("linksService")).setTaggingService(
        TAGGING_SERVICE
      );
  ...
  public void createNewEntry() throws Exception {
    ...
    PagingResults<LinkInfo> results = LINKS_SERVICE.listLinks(
    ...
    NodeRef container = PUBLIC_NODE_SERVICE
      .getPrimaryParent(link.getNodeRef())
      .getParentRef();
    ...
    link =
      LINKS_SERVICE.getLink(LINKS_SITE.getShortName(), link.getSystemName());
    assertEquals("Title", link.getTitle());
    assertEquals("Description", link.getDescription());
    assertEquals("http://www.alfresco.com/", link.getURL());
    assertEquals(false, link.isInternal());
  ...
  public void createUpdateDeleteEntry() throws Exception {
    ...
    assertEquals(
      "Description",
      PUBLIC_NODE_SERVICE.getProperty(
        link.getNodeRef(),
        LinksModel.PROP_DESCRIPTION
      )
    );
    assertEquals(
      "http://www.alfresco.com/",
    ...
    assertEquals(
      false,
      PUBLIC_NODE_SERVICE.hasAspect(
        link.getNodeRef(),
        LinksModel.ASPECT_INTERNAL_LINK
      )
    );
        ...
        public Void execute() throws Throwable {
          ...
          assertFalse(
            SITE_SERVICE.hasContainer(
              TEST_SITE_NAME,
              LinksServiceImpl.LINKS_COMPONENT
            )
          );
  ...
  public void linksListing() throws Exception {
    ...
    results =
      LINKS_SERVICE.listLinks(LINKS_SITE.getShortName(), TEST_USER, paging);
    assertEquals(3, results.getPage().size());
    assertEquals("TitleC", results.getPage().get(0).getTitle());
    assertEquals("TitleB", results.getPage().get(1).getTitle());
    assertEquals("TitleA", results.getPage().get(2).getTitle());
    ...
    results =
      LINKS_SERVICE.listLinks(LINKS_SITE.getShortName(), ADMIN_USER, paging);
    assertEquals(1, results.getPage().size());
    assertEquals("TitleD", results.getPage().get(0).getTitle());
    ...
    results =
      LINKS_SERVICE.listLinks(ALTERNATE_LINKS_SITE.getShortName(), paging);
    for (LinkInfo link : results.getPage()) {
      PUBLIC_NODE_SERVICE.deleteNode(link.getNodeRef());
    }
  }
    ...
    TRANSACTION_HELPER.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
        ...
        public Void execute() throws Throwable {
          assertEquals(
            true,
            SITE_SERVICE.isMember(LINKS_SITE.getShortName(), TEST_USER)
          );
          assertEquals(
            false,
   ...
   */
  private void pushCreatedDateBack(LinkInfo link, int daysAgo)
    throws Exception {
  ...
  private static void createTestSites() throws Exception {
    final LinksServiceImpl privateCalendarService = (LinksServiceImpl) testContext.getBean(
      "linksService"
    );

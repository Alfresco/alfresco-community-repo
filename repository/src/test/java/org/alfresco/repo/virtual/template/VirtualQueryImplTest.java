...
public class VirtualQueryImplTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        Mockito.when(mockitoPrefixResolver.getPrefixes(TEST_URI))
                .thenReturn(Arrays.asList(TST_PREFIX));

        Mockito.when(mockitoActualEnvironment.getNamespacePrefixResolver())
    ...
    public void testPerform_1() throws Exception {
        Pair<QName, Boolean> withSortDefinitions = new Pair<QName, Boolean>(testQName2, true);
        ...
        constraint = new SortConstraint(constraint, Arrays.asList(withSortDefinitions));
    ...
    private void assertPerform1Results(Pair<QName, Boolean> withSortDefinitions) {
        ArgumentCaptor<SearchParameters> queryCaptor =
    ...
    private void assertPerform2Results() {
        ...
        assertEquals(
                "((QUERY_TEST_STRING_QUERY) and TYPE:\"cm:folder\") and !ASPECT:'tst:testQName1'"
                        + " and !TYPE:'tst:testQName2'",
                queryCaptor.getValue().getQuery());

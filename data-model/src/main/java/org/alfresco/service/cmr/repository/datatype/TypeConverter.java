...
public class TypeConverter {
  ...
  public final <T> T convert(Class<T> c, Object value) {
    ...
    if (c.isPrimitive()) {
      ...
      throw new TypeConversionException(
        "Can not convert direct to primitive type " + c.getName()
      );
    }
   ...
   * @throws TypeConversionException if the conversion cannot be performed
   */
  public final Collection<?> convert(
    DataTypeDefinition propertyType,
    Object[] values
  ) {
    if (values == null) {
   ...
   */
  public final <F, T> void addConverter(
    Class<F> source,
    Class<T> destination,
   ...
   */
  public final <F, I, T> Converter<F, T> addDynamicTwoStageConverter(
    Class<F> source,
    Class<I> intermediate,
    Class<T> destination
  ) {
    Converter<F, T> converter = new TypeConverter.DynamicTwoStageConverter<F, I, T>(
      source,
      intermediate,
      destination
    );
  ...
  public final <T> Converter<Object, T> getConverter(
    Object value,
    Class<T> dest
  ) {
  ...
  public <F, T> Converter<F, T> getConverter(Class<F> source, Class<T> dest) {
    ...
    do {
      ...
      if (converter == null) {
        ...
        Converter<F, Number> first = (Converter<F, Number>) map.get(
          Number.class
        );
  ...
  protected class DynamicTwoStageConverter<F, I, T> implements Converter<F, T> {
    ...
    public T convert(F source) {
      Converter<F, I> iConverter =
        TypeConverter.this.getConverter(from, intermediate);
      Converter<I, T> tConverter =
        TypeConverter.this.getConverter(intermediate, to);
      if (iConverter == null || tConverter == null) {
        throw new TypeConversionException(
          "Cannot convert from " + from.getName() + " to " + to.getName()
        );
      }

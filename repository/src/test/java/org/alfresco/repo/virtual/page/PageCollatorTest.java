...
public class PageCollatorTest extends TestCase
{
    ...
    private int[] createMergedPage(int skip, int count, List<Integer> s1, int[] s2)
    {
        int[] s1Primitive = (s1 == null || s1.isEmpty()) ? new int[]{}
                : ArrayUtils.toPrimitive((Integer[]) s1
    ...
    /**
     * Creates an page-array by concatenating the given arrays than skipping and trimming the resulted array.
     * 
     * @param skip
     * @param count
     * @param s1
     * @param s2
     * @return a page-array obtained by merging <code>s1</code> and <code>s2</code>, sorting the result than copying <code>count</code> elements from the concatenation starting with the element at index <code>skip</code> into the returned array of size <code>count</code>. If <code>skip</code> value is out of bounds an empty array is returned.
     */

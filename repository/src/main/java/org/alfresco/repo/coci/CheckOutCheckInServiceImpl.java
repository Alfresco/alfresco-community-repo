...
public class CheckOutCheckInServiceImpl implements CheckOutCheckInService, Extensible {
    ...
    private static final String MSG_ERR_ALREADY_WORKING_COPY =
            "coci_service.err_workingcopy_checkout";
    private static final String MSG_ERR_NOT_AUTHENTICATED = "coci_service.err_not_authenticated";
    private static final String MSG_ERR_WORKINGCOPY_HAS_NO_CONTENT =
            "coci_service.err_workingcopy_has_no_content";
    ...
    public CheckOutCheckInServiceImpl() {
        this.checkOutCheckInServiceTrait =
                new ExtendedTrait<CheckOutCheckInServiceTrait>(
                        AJProxyTrait.create(this, CheckOutCheckInServiceTrait.class));
    }
    ...
    public void init() {
        ...
        beforeCheckOut =
                policyComponent.registerClassPolicy(
                        CheckOutCheckInServicePolicies.BeforeCheckOut.class);
        onCheckOut =
                policyComponent.registerClassPolicy(
                        CheckOutCheckInServicePolicies.OnCheckOut.class);
        beforeCheckIn =
                policyComponent.registerClassPolicy(
                        CheckOutCheckInServicePolicies.BeforeCheckIn.class);
        onCheckIn =
                policyComponent.registerClassPolicy(CheckOutCheckInServicePolicies.OnCheckIn.class);
        beforeCancelCheckOut =
                policyComponent.registerClassPolicy(
                        CheckOutCheckInServicePolicies.BeforeCancelCheckOut.class);
        onCancelCheckOut =
                policyComponent.registerClassPolicy(
                        CheckOutCheckInServicePolicies.OnCancelCheckOut.class);
    }
    ...
    private void invokeBeforeCheckIn(
        ...
        for (QName invokeClass : classes) {
            ...
            for (BeforeCheckIn policy : policies) {
                policy.beforeCheckIn(
                        workingCopyNodeRef, versionProperties, contentUrl, keepCheckedOut);
            }
        }
    }
    ...
    @Override
    @Extend(
            traitAPI = CheckOutCheckInServiceTrait.class,
            extensionAPI = CheckOutCheckInServiceExtension.class)
    ...
    public NodeRef checkout(
        ...
        // It is not enough to check LockUtils.isLockedOrReadOnly in case when the same user does
        // offline and online edit (for instance in two open browsers). In this case we get
        // set ContentModel.ASPECT_LOCKABLE and LockType.WRITE_LOCK. So, here we have to check
        // following
        ...
        try {
            ...
            behaviourFilter.enableBehaviour(
                    destinationParentNodeRef, ContentModel.ASPECT_AUDITABLE);
        }
    }
    ...
    private NodeRef doCheckout(
        ...
        try {
            ...
            final QName copyQName =
                    QName.createQName(
                            destinationAssocQName.getNamespaceURI(),
            ...
            // then working copy should be created even if the user has no permissions to create
            // children in
            ...
            nodeService.addAspect(
                    workingCopy, ContentModel.ASPECT_WORKING_COPY, workingCopyProperties);
            ...
            nodeService.createAssociation(
                    nodeRef, workingCopy, ContentModel.ASSOC_WORKING_COPY_LINK);
    ...
    public NodeRef checkin(
            NodeRef workingCopyNodeRef, Map<String, Serializable> versionProperties) {
    ...
    public NodeRef checkin(
        ...
        if (contentUrl != null) {
            ContentData contentData =
                    (ContentData)
                            nodeService.getProperty(workingCopyNodeRef, ContentModel.PROP_CONTENT);
            if (contentData == null) {
                throw new AlfrescoRuntimeException(
                        MSG_ERR_WORKINGCOPY_HAS_NO_CONTENT, new Object[] {workingCopyNodeRef});
            } else {
                contentData =
                        new ContentData(
                                contentUrl,
        ...
        if (fileFolderService.getFileInfo(workingCopyNodeRef) != null) {
            ...
            String name =
                    (String) nodeService.getProperty(workingCopyNodeRef, ContentModel.PROP_NAME);
            String wcLabel =
                    (String)
                            this.nodeService.getProperty(
                                    workingCopyNodeRef, ContentModel.PROP_WORKING_COPY_LABEL);
            if (wcLabel == null) {
            ...
            if (hasWorkingCopyNameChanged(name, origName, wcLabel)) {
                ...
                if (!name.contains(" " + wcLabel)) {
                    try {
                        fileFolderService.rename(
                                workingCopyNodeRef, createWorkingCopyName(name, wcLabel));
                    } catch (FileExistsException e) {
                        throw new CheckOutCheckInServiceException(
                                e, MSG_ERR_CANNOT_RENAME, name, wcLabel);
                ...
                try {
                    ...
                    throw new CheckOutCheckInServiceException(
                            e, MSG_ERR_CANNOT_RENAME, origName, newName);
                } catch (FileNotFoundException e) {
                    throw new CheckOutCheckInServiceException(
                            e, MSG_ERR_CANNOT_RENAME, name, newName);
                }
            }
        }
        ...
        if (keepCheckedOut == false) {
            ...
            try {
                ...
                behaviourFilter.enableBehaviour(
                        workingCopyNodeRef, ContentModel.ASPECT_WORKING_COPY);
                behaviourFilter.enableBehaviour(nodeRef, ContentModel.ASPECT_AUDITABLE);
            }
        } else {
    ...
    public NodeRef getWorkingCopy(NodeRef nodeRef) {
        ...
        if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_CHECKED_OUT)) {
            List<AssociationRef> assocs =
                    nodeService.getTargetAssocs(nodeRef, ContentModel.ASSOC_WORKING_COPY_LINK);
            // It is a 1:1 relationship
            if (assocs.size() == 0) {
                logger.warn(
                        "Found node with cm:checkedOut aspect but no association.  Current node"
                                + " state: "
                                + nodeService.getNodeStatus(nodeRef));
            } else if (assocs.size() > 1) {
                logger.warn(
                        "Found multiple "
                                + ContentModel.ASSOC_WORKING_COPY_LINK
                                + " association from node: "
                                + nodeRef);
            } else {
    ...
    public NodeRef getCheckedOut(NodeRef nodeRef) {
        ...
        if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_WORKING_COPY)) {
            List<AssociationRef> assocs =
                    nodeService.getSourceAssocs(nodeRef, ContentModel.ASSOC_WORKING_COPY_LINK);
            // It is a 1:1 relationship
            if (assocs.size() == 0) {
                logger.warn(
                        "Found node with cm:workingcopy aspect but no association.  Current node"
                                + " state: "
                                + nodeService.getNodeStatus(nodeRef));
            } else if (assocs.size() > 1) {
                logger.warn(
    ...
    /**
     * Create a working copy name using the given fileName and workingCopyLabel. The label will be
     * inserted before the file extension (if present), or else appended to the name (in either case
     * a space is prepended to the workingCopyLabel).
     *
     * <p>Examples, where workingCopyLabel is "wc":
     *
     * <p>"Myfile.txt" becomes "Myfile wc.txt", "Myfile" becomes "Myfile wc".
     *
     * <p>In the event that fileName is empty or null, the workingCopyLabel is used for the new
     * working copy name
     *
     * <p>Example: "" becomes "wc".
     *
     * @param name String
    ...
    /**
     * Get original name from the working copy name and the cm:workingCopyLabel that was used to
     * create it.
     *
     ...
     */
    private boolean hasWorkingCopyNameChanged(
            String workingCopyName, String origName, String wcLabel) {

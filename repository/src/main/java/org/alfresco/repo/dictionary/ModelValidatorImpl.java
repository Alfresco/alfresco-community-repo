...
public class ModelValidatorImpl implements ModelValidator {
  ...
  private void validateDeleteClass(
    final Tenant tenant,
          ...
          public Void execute() throws Throwable {
            try {
              ...
              throw new ModelInUseException(
                "Cannot delete model, class " + className + " is in use"
              );
            }
          }
        },
    ...
    for (WorkflowDefinition workflowDef : workflowService.getDefinitions()) {
      for (WorkflowTaskDefinition workflowTaskDef : workflowService.getTaskDefinitions(
        workflowDef.getId()
      )) {
  ...
  private void validateDeleteProperty(
    ...
    if (sharedModel) {
      tenantDomain =
        " for tenant [" + tenantService.getCurrentUserDomain() + "]";
    }
              ...
              new TenantRunAsWork<Void>() {
                ...
                public Void doWork() throws Exception {
                  try {
                    ...
                    logger.debug(e);
                    throw new ModelInUseException(
                      "Failed to validate property delete, property " +
                      propName +
                      " is in use"
                    );
                  }
  ...
  public boolean canDeleteModel(final QName modelName) {
    ...
    try {
      ...
      canDeleteModel &=
        canDeleteModel(namespaceDefs, typeDefs, aspectDefs, null);
      if (canDeleteModel) {
        if (
          tenantService.isEnabled() && tenantService.isTenantUser() == false
        ) {
              ...
              AuthenticationUtil.runAs(
                new RunAsWork<Boolean>() {
                  public Boolean doWork() {
                    boolean canDelete = canDeleteModel(
                      namespaceDefs,
                      typeDefs,
                      aspectDefs,
                      tenant
    ...
    for (M2ModelDiff modelDiff : modelDiffs) {
      if (modelDiff.getDiffType().equals(M2ModelDiff.DIFF_DELETED)) {
        ...
        if (modelDiff.getElementType().equals(M2ModelDiff.TYPE_PROPERTY)) {
          ...
          modelDiff.getElementType().equals(M2ModelDiff.TYPE_CONSTRAINT)
        ) {
      ...
      if (modelDiff.getDiffType().equals(M2ModelDiff.DIFF_CREATED)) {
        if (modelDiff.getElementType().equals(M2ModelDiff.TYPE_NAMESPACE)) {
          ModelDefinition importedModel = dictionaryService.getModelByNamespaceUri(
          ...
          if (importedModel != null && !model.getNamespaces().isEmpty()) {
            checkCircularDependency(
              importedModel,
              model,
              importedModel.getName().getLocalName()
            );
          }
        }
      }
  ...
  private void checkCircularDependency(
    ...
    for (NamespaceDefinition importedNamespace : model.getImportedNamespaces()) {
      ...
      if (
        (
          md =
            dictionaryService.getModelByNamespaceUri(importedNamespace.getUri())
        ) !=
        null
      ) {
        if (existingModel.getNamespace(importedNamespace.getUri()) != null) {
          throw new AlfrescoRuntimeException(
            "Failed to validate model update - found circular dependency. You can't set parent " +
            parentPrefixedName +

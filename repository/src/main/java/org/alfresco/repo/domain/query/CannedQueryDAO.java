...
{
    /* This interface looks very much like SqlSessionTemplate; this is not accidental. However, the use of generics and the ability to trace all general SQL queries through an Alfresco API make it useful. Additionally, there are specific checks done in the implementation to insulate the client from some common problems. */

    /**
     * Execute a <b>count(*)</b>-style query returning a count value. The implementation will ensure that <tt>null</tt> is substituted with <tt>0</tt>, if required.
     * <p>
     * All exceptions can be safely caught and handled as required.
     * 
     * @param sqlNamespace
     *            the query namespace (defined by config file) e.g. <b>alfresco.query.usage</b>
     * @param queryName
     *            the name of the query e.g. <b>select_userCount</b>
     * @param parameterObj
     *            the values to drive the selection (may be <tt>null</tt> if not required)
     * @return a non-null count
     * 
     * @throws QueryException
     *             if the query returned multiple results
     */
     ...
     * @param limit
     *            the maximum number of results to retrieve or <code>Integer.MAX_VALUE</code> for no limit
    ...
    {
        /**
         * Allow implementations to process a result. Note that the interface contract will be met, but internally the querying mechanism <i>might not</i> be able to optimise out all result fetching.
         * 
         * @return <tt>true</tt> if more results are required or <tt>false</tt> to terminate result fetching.
         */

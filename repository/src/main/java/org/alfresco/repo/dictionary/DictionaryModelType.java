...
public class DictionaryModelType
    ...
    private static final String KEY_PENDING_DELETE_MODELS =
            "dictionaryModelType.pendingDeleteModels";

    /** Key to the removed "workingcopy" aspect */
    private static final String KEY_WORKING_COPY = "dictionaryModelType.workingCopy";

    /**
     * The name of the lock used to ensure that DictionaryModelType updates do not run on more than
     * one thread/node at the same time.
     */
    private static final QName LOCK_QNAME =
            QName.createQName(NamespaceService.SYSTEM_MODEL_1_0_URI, "DictionaryModelType");
    ...
    public void init() {
        ...
        this.transactionListener =
                new DictionaryModelTypeTransactionListener(this.nodeService, this.contentService);
    }
    ...
    private void queueModel(NodeRef nodeRef) {
        Set<NodeRef> pendingModels =
                (Set<NodeRef>) AlfrescoTransactionSupport.getResource(KEY_PENDING_MODELS);
    ...
    public void onRemoveAspect(NodeRef nodeRef, QName aspect) {
        ...
        // undo/cancel checkout removes the "workingcopy" aspect prior to deleting the node - hence
        // need to track here
    ...
    public void beforeDeleteNode(NodeRef nodeRef) {
        ...
        if (!(workingCopy || isVersionNode)) {
            QName modelName =
                    (QName) this.nodeService.getProperty(nodeRef, ContentModel.PROP_MODEL_NAME);

            if (logger.isTraceEnabled()) {
                logger.trace(
                        "beforeDeleteNode: nodeRef="
                                + nodeRef
                                + " validate model delete (modelName="
                                + modelName
                                + ")");
            }
            ...
            if (modelName != null) {
                ...
                Set<NodeRef> pendingModelDeletes =
                        (Set<NodeRef>)
                                AlfrescoTransactionSupport.getResource(KEY_PENDING_DELETE_MODELS);
                if (pendingModelDeletes == null) {
                    // pendingModelDeletes = Collections.newSetFromMap(new ConcurrentHashMap()); //
                    // Java 6
                    pendingModelDeletes = new CopyOnWriteArraySet<NodeRef>();
                    AlfrescoTransactionSupport.bindResource(
                            KEY_PENDING_DELETE_MODELS, pendingModelDeletes);
                }
    ...
    public void onCreateNode(ChildAssociationRef childAssocRef) {
        ...
        if (nodeService.getType(nodeRef).equals(ContentModel.TYPE_DICTIONARY_MODEL)) {
            Boolean value =
                    (Boolean) nodeService.getProperty(nodeRef, ContentModel.PROP_MODEL_ACTIVE);
            if ((value != null) && (value == true)) {
                queueModel(nodeRef);
            }
        }
    ...
    public class DictionaryModelTypeTransactionListener extends TransactionListenerAdapter {
        ...
        public DictionaryModelTypeTransactionListener(
        ...
        public void afterCommit() {
            Set<NodeRef> pendingModels =
                    (Set<NodeRef>) AlfrescoTransactionSupport.getResource(KEY_PENDING_MODELS);
            Set<NodeRef> pendingDeleteModels =
                    (Set<NodeRef>)
                            AlfrescoTransactionSupport.getResource(KEY_PENDING_DELETE_MODELS);
            ...
            if (logger.isTraceEnabled()) {
                logger.trace(
                        "afterCommit: "
                                ...
                                + " pendingModelsCnt="
                                + (pendingModels != null ? pendingModels.size() : "0")
                                + ", pendingDeleteModelsCnt="
                                + (pendingDeleteModels != null ? pendingDeleteModels.size() : "0"));
            }
            ...
            if (pendingModels != null) {
                ...
                for (NodeRef nodeRef : pendingModels) {
                    String tenantDomain =
                            tenantService.getDomain(nodeRef.getStoreRef().getIdentifier());
            ...
            if (systemTenants.size() > 0) {
                for (final String tenantName : systemTenants) {
                                    ...
                                    return TenantUtil.runAsSystemTenant(
                                            new TenantRunAsWork<Void>() {
                                                public Void doWork() {
                                                    ...
                                                    if (logger.isTraceEnabled()) {
                                                        logger.trace(
                                                                "afterCommit: Dictionary destroyed"
                                                                        + " ["
                    ...
                    transactionService
                            ...
                            .doInTransaction(work, true, true);
                }
            }
        }
        ...
        public void beforeCommit(boolean readOnly) {
            if (jobLockService != null) {
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "beforeCommit: "
                                    + Thread.currentThread().getName()
                                    + " attempt to get transactional lock ["
                                    + AlfrescoTransactionSupport.getTransactionId()
                                    + "]");
                }
            ...
            if (pendingModels != null) {
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "beforeCommit: pendinpolicy-context.xmlgModelsCnt="
                                    + pendingModels.size()
                                    + " ["
                                    + AlfrescoTransactionSupport.getTransactionId()
                                    + "]");
                ...
                for (NodeRef pendingNodeRef : pendingModels) {
                    String tenantDomain =
                            tenantService.getDomain(pendingNodeRef.getStoreRef().getIdentifier());
                            ...
                            new RunAsWork<Object>() {
                                public Object doWork() {
                                    ...
                                    if (!(nodeService.hasAspect(
                                            nodeRef, ContentModel.ASPECT_WORKING_COPY))) {
                                        if (isActive == true) {
                                            // 1. Compile the model and update the details on the
                                            // node
                                            // 2. Re-put the model
                                            ...
                                            if (contentReader != null) {
                                                ...
                                                CompiledModel compiledModel =
                                                        m2Model.compile(
                                                                dictionaryDAO, namespaceDAO, true);
                                                ...
                                                props.put(
                                                        ...
                                                        modelDefinition.getDescription(null));
                                                props.put(
                                                        ContentModel.PROP_MODEL_AUTHOR,
                                                        modelDefinition.getAuthor());
                                                props.put(
                                                        ContentModel.PROP_MODEL_PUBLISHED_DATE,
                                                        modelDefinition.getPublishedDate());
                                                props.put(
                                                ...
                                                // Validate model against dictionary - could be new,
                                                ...
                                                if (logger.isTraceEnabled()) {
                                                    logger.trace(
                                                            "beforeCommit: activating nodeRef="
                                                                    + nodeRef
                                                                    + " ("
                                                                    + modelDefinition.getName()
                                                                    + ") ["
                                            ...
                                            QName modelName =
                                                    (QName)
                                                            nodeService.getProperty(
                                                                    ...
                                                                    ContentModel.PROP_MODEL_NAME);
                                            if (modelName != null) {
                                                // Validate model delete against usages - content
                                                // and/or workflows
                                                if (!modelValidator.canDeleteModel(modelName)) {
                                                    throw AlfrescoRuntimeException.create(
                                                            MODEL_IN_USE, modelName);
                                                }
                                                ...
                                                if (logger.isTraceEnabled()) {
                                                    logger.trace(
                                                            "beforeCommit: deactivating nodeRef="
                                                                    + nodeRef
                                                                    + " ("
                                                                    + modelName
                                                                    + ") ["
        ...
        public boolean equals(Object obj) {
            ...
            if (obj instanceof DictionaryModelTypeTransactionListener) {
                DictionaryModelTypeTransactionListener that =
                        (DictionaryModelTypeTransactionListener) obj;

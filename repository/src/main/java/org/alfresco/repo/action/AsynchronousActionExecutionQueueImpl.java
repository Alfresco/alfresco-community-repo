...
public class AsynchronousActionExecutionQueueImpl implements AsynchronousActionExecutionQueue {
    ...
    private Map<String, AbstractAsynchronousActionFilter> actionFilters =
            new ConcurrentHashMap<String, AbstractAsynchronousActionFilter>();
    private String id;

    /**
     * We keep a record of ongoing asynchronous actions (this includes those being executed and
     * those that are in the queue). This needs to be thread-safe - hence the Vector.
     */
    ...
    public void init() {
        ...
        onAsyncActionExecuteDelegate =
                policyComponent.registerClassPolicy(OnAsyncActionExecute.class);
    }
    ...
    /**
     * Set the thread pool, which may be shared with other components, that will be used to run the
     * actions.
     *
    ...
    public void executeAction(
        ...
        executeAction(
                actionService, action, actionedUponNodeRef, checkConditions, actionChain, null);
    }
    ...
    public void executeAction(
        ...
        if (logger.isDebugEnabled()) {
            StringBuilder msg = new StringBuilder();
            msg.append("Received request to execute async action ")
                    .append(action.getActionDefinitionName())
                    .append(" on ")
                    .append(actionedUponNodeRef);
            logger.debug(msg.toString());

            msg = new StringBuilder();
            msg.append("ThreadPool's active count = ")
                    .append(this.threadPoolExecutor.getActiveCount());
            logger.debug(msg.toString());

            msg = new StringBuilder();
            msg.append("ThreadPool's queue size = ")
                    .append(this.threadPoolExecutor.getQueue().size());
            logger.debug(msg.toString());
        }

        Set<RuleServiceImpl.ExecutedRuleData> executedRules =
                (Set<RuleServiceImpl.ExecutedRuleData>)
        ...
        OngoingAsyncAction nodeBeingNewlyActioned =
                new OngoingAsyncAction(actionedUponNodeRef, action);
        ...
        for (Entry<String, AbstractAsynchronousActionFilter> entry : actionFilters.entrySet()) {
            ...
            if (actionDefinitionName.equals(action.getActionDefinitionName()) == false) {
                ...
                // So we'll go through the actions that are ongoing and consider them for matches
                // with this one.
                // Need to synchronize to prevent changes to ongoingActions whilst iterating. Assume
                // that ongoingActions
                // is not going to be too big and the loop will execute quite quickly, so that the
                // synchronization
    ...
    private class ActionExecutionWrapper implements Runnable {
         ...
         * @param executedRules list of executions done to helps to prevent loop scenarios with
         *     async rules
         */
        ...
        public void run() {
            try {
                ...
                final String userName =
                        ((ActionImpl) ActionExecutionWrapper.this.action).getRunAsUser();
                if (userName == null) {
                    throw new ActionServiceException(
                            "Cannot execute action asynchronously since run as user is 'null'");
                }
                // Get the tenant the action was submitted from
                final String tenantId =
                        ((ActionImpl) ActionExecutionWrapper.this.action).getTenantId();
                        ...
                        new TenantRunAsWork<Object>() {
                            public Object doWork() throws Exception {
                                RetryingTransactionCallback<Object> actionCallback =
                                        new RetryingTransactionCallback<Object>() {
                                            public Object execute() {
                                                ...
                                                if (ActionExecutionWrapper.this.executedRules
                                                        != null) {
                                                    AlfrescoTransactionSupport.bindResource(
                                                            "RuleServiceImpl.ExecutedRules",
                                                            ActionExecutionWrapper.this
                                                                    .executedRules);
                                                }

                                                // Allow other classes to know when this action
                                                // completes
                                                AlfrescoTransactionSupport.bindListener(
                                                        new CallbackTransactionListener(
                                                                ActionExecutionWrapper.this.action,
                                                                ActionExecutionWrapper.this
                                                                        .actionedUponNodeRef));

                                                // Have the action run
                                                ActionExecutionWrapper.this.actionService
                                                        .executeActionImpl(
                                                                ActionExecutionWrapper.this.action,
                                                                ActionExecutionWrapper.this
                                                                        .actionedUponNodeRef,
                                ...
                                return transactionService
                                        ...
                                        .doInTransaction(actionCallback);
                            }
                        };
                ...
                Throwable rootCause =
                        (e instanceof AlfrescoRuntimeException)
                                ? ((AlfrescoRuntimeException) e).getRootCause()
                                : null;
                ...
                if (!ActionExecutionWrapper.this.actionService.onLogException(
                        action, logger, rootCause, message)) {

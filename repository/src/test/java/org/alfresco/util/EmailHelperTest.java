...
public class EmailHelperTest {
  ...
  public void setup() throws Exception {
    ...
    when(serviceRegistryMock.getNamespaceService())
      .thenReturn(namespaceServiceMock);
    when(serviceRegistryMock.getFileFolderService())
      .thenReturn(fileFolderServiceMock);
    when(serviceRegistryMock.getPersonService()).thenReturn(personServiceMock);
    when(serviceRegistryMock.getContentService())
      .thenReturn(mock(ContentService.class));
  ...
  public void testGetEmailTemplate() throws Exception {
    String result = emailHelper.getEmailTemplate(
      CLIENT_NAME,
      null,
    ...
    // XPath
    {
      ...
      assertEquals(
        "Couldn't find email template with the given XPath, so the fallback template should be returned.",
        result,
        FALLBACK_TEMPLATE_PATH
      );
    ...
    // NodeRef
    {
      ...
      result =
        emailHelper.getEmailTemplate(
          CLIENT_NAME,
          dummyTemplateNodeRef.toString(),
          FALLBACK_TEMPLATE_PATH
        );
      assertEquals(
        "Couldn't find email template with the given NodeRef, so the fallback template should be returned.",
        result,
        FALLBACK_TEMPLATE_PATH
      );
    ...
    // Class path
    {
      String classPathTemplate =
        "alfresco/templates/email-templates/new-template.html";
      ...
      assertEquals(
        "Couldn't find email template with the given class path, so the fallback template should be returned.",
        result,
        FALLBACK_TEMPLATE_PATH
      );
      ...
      assertEquals(
        "Error occurred while finding the email template with the class path, so the fallback template should be returned.",
        result,
        FALLBACK_TEMPLATE_PATH
      );
    }
  ...
  public void testGetUserLocaleOrDefault() throws Exception {
    ...
    when(preferenceServiceMock.getPreference(userId, "locale"))
      .thenReturn(null);
    locale = emailHelper.getUserLocaleOrDefault(userId);
    assertEquals(I18NUtil.getLocale(), locale);

...
/**
 * JSON returned from SOLR API Parser This class defines common properties and performs response
 * header parsing. An abstract method is provided for implementers to parse Core Information.
 *
 * @author aborroy
 * @since 6.2
 */
public abstract class AbstractJSONAPIResult implements JSONAPIResult {
    ...
    /**
     * Creates a property-value Map from a JSON Object containing properties and values This method
     * provides the right input for MBeans to expose the SOLR response values from the Response
     *
    ...
    protected Map<String, Object> getPropertyValueMap(JSONObject json) throws JSONException {
        ...
        for (int j = 0; j < nodesPropertyNameList.length(); j++) {
            ...
            if (propertyValue != JSONObject.NULL) {
                // MBeans Objects are defined as Long types, so we need casting to provide the
                // expected type

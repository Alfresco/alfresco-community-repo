...
public class ScriptActionExecuter extends ActionExecuterAbstractBase {
    ...
    if (nodeService.exists(actionedUponNodeRef)) {
      ...
      if (!isValidScriptRef(action)) {
        throw new IllegalStateException(
          "Invalid script ref path: " + scriptRef
        );
      }
      NodeRef spaceRef =
        this.serviceRegistry.getRuleService().getOwningNodeRef(action);
      if (spaceRef == null) {
        // the actionedUponNodeRef may actually be a space
        if (
          this.serviceRegistry.getDictionaryService()
            .isSubClass(
      ...
      if (
        ...
        (scriptRef != null && nodeService.exists(scriptRef) == true)
      ) {
        // get the references we need to build the default scripting data-model
        String userName =
          this.serviceRegistry.getAuthenticationService().getCurrentUserName();
        ...
        Map<String, Object> model =
          this.serviceRegistry.getScriptService()
            .buildDefaultModel(
              personRef,
              getCompanyHome(),
              homeSpaceRef,
        ...
        model.put(
          "webApplicationContextUrl",
          UrlUtil.getAlfrescoUrl(sysAdminParams)
        );
        ...
        if (this.scriptLocation == null) {
          ...
          result =
            this.serviceRegistry.getScriptService()
              .executeScript(scriptRef, ContentModel.PROP_CONTENT, model);
        } else {
          // execute the script at the specified script location
          result =
            this.serviceRegistry.getScriptService()
              .executeScript(this.scriptLocation, model);
        }
  ...
  private NodeRef getCompanyHome() {
    ...
    List<NodeRef> refs =
      this.serviceRegistry.getSearchService()
        .selectNodes(
  ...
  private boolean isValidScriptRef(Action action) {
    ...
    ActionDefinition actDef = actionService.getActionDefinition(
      action.getActionDefinitionName()
    );
    ParameterDefinition parameterDef = actDef.getParameterDefintion(
      PARAM_SCRIPTREF
    );

...
/**
 * A {@link BeanFactoryPostProcessor} that upgrades old-style Spring overrides that add location
 * paths to the <code>repository-properties</code> or <code>hibernateConfigProperties</code> beans
 * to instead add these paths to the <code>global-properties</code> bean. To avoid the warning
 * messages output by this class, new property overrides should be added to
 * alfresco-global.properties without overriding any bean definitions.
 *
 * @author dward
 */
public class LegacyConfigPostProcessor implements BeanFactoryPostProcessor, PriorityOrdered {
    ...
    /**
     * The name of the bean that expands repository properties. These should now be defaulted from
     * global-properties.
     */
    private static final String BEAN_NAME_REPOSITORY_PROPERTIES = "repository-properties";

    /**
     * The name of the bean that holds hibernate properties. These should now be overriden by
     * global-properties.
     */
    private static final String BEAN_NAME_HIBERNATE_PROPERTIES = "hibernateConfigProperties";

    /**
     * The name of the property on a Spring property loader that holds a list of property file
     * location paths.
     */
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory)
        ...
        try {
            ...
            MutablePropertyValues globalProperties =
                    beanFactory
                            .getBeanDefinition(
                                    LegacyConfigPostProcessor.BEAN_NAME_GLOBAL_PROPERTIES)
                            .getPropertyValues();
            PropertyValue pv =
                    globalProperties.getPropertyValue(LegacyConfigPostProcessor.PROPERTY_LOCATIONS);
            ...
            if (pv != null && (value = pv.getValue()) != null && value instanceof Collection) {
                ...
                globalProperties.addPropertyValue(
                        LegacyConfigPostProcessor.PROPERTY_LOCATIONS, globalPropertyLocations);
            }

            // Move location paths added to repository-properties
            MutablePropertyValues repositoryProperties =
                    processLocations(
                            beanFactory,
            ...
            repositoryProperties.addPropertyValue(
                    "systemPropertiesModeName", "SYSTEM_PROPERTIES_MODE_NEVER");

            // Move location paths added to hibernateConfigProperties
            MutablePropertyValues hibernateProperties =
                    processLocations(
                            beanFactory,
                            globalPropertyLocations,
            ...
            // Because Spring gets all post processors in one shot, the bean may already have been
            // created. Let's try to
            // fix it up!
            PropertyPlaceholderConfigurer repositoryConfigurer =
     ...
     */
    @SuppressWarnings("unchecked")
    private MutablePropertyValues processLocations(
            ConfigurableListableBeanFactory beanFactory,
            Collection<Object> globalPropertyLocations,
            String beanName,
            String[] newLocations) {
        // Get the bean an check its existing properties value
        MutablePropertyValues beanProperties =
                beanFactory.getBeanDefinition(beanName).getPropertyValues();
        PropertyValue pv =
                beanProperties.getPropertyValue(LegacyConfigPostProcessor.PROPERTY_PROPERTIES);
        Object value;

        // If the properties value already references the global-properties bean, we have nothing
        // else to do. Otherwise,
        // we have to 'upgrade' the bean definition.
        if (pv == null
                || (value = pv.getValue()) == null
                || !(value instanceof BeanReference)
                || ((BeanReference) value)
                        .getBeanName()
                        .equals(LegacyConfigPostProcessor.BEAN_NAME_GLOBAL_PROPERTIES)) {
            // Convert the array of new locations to a managed list of type string values, so that
            // it is
            ...
            if (pv != null && (value = pv.getValue()) != null && value instanceof Collection) {
                ...
                // Compute the set of locations that need to be added to globalPropertyLocations
                ...
                for (Object location : addedLocations) {
                    LegacyConfigPostProcessor.logger.warn(
                            "Legacy configuration detected: adding "
                                    + (location instanceof TypedStringValue
                                            ? ((TypedStringValue) location).getValue()
                                            : location.toString())
            ...
            beanProperties.addPropertyValue(
                    LegacyConfigPostProcessor.PROPERTY_PROPERTIES,
            ...
            if (newLocationList.size() > 0) {
                beanProperties.addPropertyValue(
                        LegacyConfigPostProcessor.PROPERTY_LOCATIONS, newLocationList);
            } else {

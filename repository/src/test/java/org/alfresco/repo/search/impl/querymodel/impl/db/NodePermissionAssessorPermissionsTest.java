...
public class NodePermissionAssessorPermissionsTest {
  ...
  public void shouldGrantPermissionWhenSystemIsReading() {
    ...
    when(assessor.isOwnerReading(any(Node.class), any(Authority.class)))
      .thenReturn(false);
  ...
  private NodePermissionAssessor createAssessor() {
    ...
    EntityLookupCache<Long, Node, NodeRef> nodeCache = mock(
      EntityLookupCache.class
    );
    return spy(
      new NodePermissionAssessor(
        nodeService,
        permissionService,
        authority,
        nodeCache

...
public class SiteServiceTestHuge {
  ...
  private static TransactionService transactionService = (TransactionService) ctx.getBean(
    "TransactionService"
  );
  private static AuthenticationComponent authenticationComponent = (AuthenticationComponent) ctx.getBean(
    "authenticationComponent"
  );
  private static MutableAuthenticationService authenticationService = (MutableAuthenticationService) ctx.getBean(
    "authenticationService"
  );
  private static PersonService personService = (PersonService) ctx.getBean(
    "PersonService"
  );
  private static AuthorityService authorityService = (AuthorityService) ctx.getBean(
    "AuthorityService"
  );
  private static SiteService siteService = (SiteService) ctx.getBean(
    "SiteService"
  ); // Big 'S'
  ...
  private void createUsersGroupsAndSites(
    int userCount,
    int groupCount,
    int siteCount
  ) throws Exception {
  ...
  private void allocateUsersToGroupsAndSites(
    ...
    if (
      (
        usersToGroups == Allocation.ALL_TO_EACH &&
      ...
      fail(
        "Users are not able to see all sites or can see a site directly and via a group"
      );
    }
  }

  private void addMoreSites(int sitesToAdd, OnFailure onFailureAction)
    throws Exception {
  ...
  private void assertTime(String what, int id1, int id2, long start, long max)
    throws Exception {
  ...
  private void createUsers(int userCount) throws Exception {
    for (int userId = 1;; userId++) {
      ...
      boolean exists =
        (userId <= usersCreated) || personService.personExists(userName);
  ...
  private void createGroups(int groupCount) throws Exception {
    for (int groupId = 1;; groupId++) {
      ...
      String groupAuthorityName = authorityService.getName(
        AuthorityType.GROUP,
        groupName
      );
      boolean exists =
  ...
  private void createSites(
    ...
    for (int siteId = sitesAlreadyCreated + 1;; siteId++) {
      ...
      boolean exists =
        (siteId <= sitesCreated) || siteService.getSite(siteName) != null;
  ...
  private void allocateUsersToGroups(
    ...
    if (allocation == Allocation.ALL_TO_EACH) {
      for (int userId = 1; userId <= userCount; userId++) {
        ...
        try {
          txn.begin();
          Set<String> existingAuthorities = authorityService.getContainingAuthoritiesInZone(
            AuthorityType.GROUP,
            getUserName(userId),
  ...
  private void allocateUsersToSites(
    int userCount,
    int siteCount,
    Allocation allocation
  ) throws Exception {
  ...
  private void allocateGroupsToSites(
    ...
    if (allocation == Allocation.ALL_TO_EACH) {
      ...
      for (
        int siteId = sitesAlreadyCreated + 1, groupId = startGroupId;;
        siteId++, groupId++
      ) {
        if (groupId > groupCount) {
          if (targetEnd) break;
  ...
  private void allocateUserToGroup(int userId, int groupId) throws Exception {
    ...
    assertTime(
      ...
      groupId,
      ms,
      MAX_USER_TO_GROUP_MS
    );
  }

  private void allocateUserToSite(int userId, int siteId, int userCount)
    throws Exception {
    try {
      String userName = getUserName(userId);
      String siteName = getSiteName(siteId);
  ...
  private int getNextSiteToAddGroupTo(int firstSiteIdToCheck) throws Exception {
    ...
    Set<String> existingAuthorities = authorityService.getAuthoritiesForUser(
      userName
    );
  ...
  private void allocateUserToGroup(String userName, String groupName)
    throws Exception {
    String groupAuthorityName = authorityService.getName(
      AuthorityType.GROUP,
      groupName
  ...
  private void allocateUserToSite(
    String userName,
    String siteName,
    String doAsUser
  ) throws Exception {
  ...
  private void setSiteMembership(
    String authority,
    String siteName,
    String doAsUser
  ...
  public void testInit() throws Exception {
    ...
    int blockSize = 10;
    for (
      int siteId = getNextSiteToAddGroupTo(1);
      siteId <= NUM_SITES;
      siteId += blockSize
    ) {
      allocateGroupToSite(
  ...
  public void commandLine() throws Exception {
    ...
    if (
      (usersOnly && (from != null || to != null || restart != null)) ||
      (
        !usersOnly &&
        (
          from == null ||
      ...
      try {
        ...
        int restartFromId = (restart == null)
          ? fromId
        ...
        if (test) {
          ...
          if (groups) {
            int blockSize = 10;
            for (
              int siteId = getNextSiteToAddGroupTo(restartFromId);
              siteId <= NUM_SITES;
              siteId += blockSize
            ) {
  ...
  public void testAddingSitesAndDelete(int fromSiteId, int toSiteId)
    throws Exception {
    usersCreated = NUM_USERS;
    groupsCreated = NUM_GROUPS;
    sitesCreated = fromSiteId - 1;

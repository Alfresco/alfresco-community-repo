...
public class BlogServiceImplTest extends BaseSpringTest {
    ...
    private static final String TEST_USER =
            BlogServiceImplTest.class.getSimpleName() + GUID.generate();
    ...
    public void before() throws Exception {
        ...
        // We need to create the test site as the test user so that they can contribute content to
        // it in tests below.
                            ...
                            public SiteInfo execute() throws Throwable {
                                SiteInfo site =
                                        SITE_SERVICE.createSite(
                                                "BlogSitePreset",
                                                BlogServiceImplTest.class.getSimpleName()
                                                        + "_testSite"
                                                        + GUID.generate(),
                                                "test site title",
                            ...
                            public NodeRef execute() throws Throwable {
                                ...
                                NodeRef result =
                                        SITE_SERVICE.getContainer(
                                                site.getShortName(),
                                                BlogServiceImpl.BLOG_COMPONENT);
    ...
    public void createDraftBlogPostsAndGetPagedResults() throws Exception {
        ...
        final List<NodeRef> submittedBlogPosts =
                TRANSACTION_HELPER.doInTransaction(
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        List<BlogPostInfo> recoveredBlogPosts =
                                new ArrayList<BlogPostInfo>(arbitraryNumberGreaterThanPageSize);

                        final int pageSize = 10;
                        PagingRequest pagingReq = new PagingRequest(0, pageSize, null);
                        pagingReq.setRequestTotalCountMax(
                                arbitraryNumberGreaterThanPageSize); // must be set if calling
                        // getTotalResultCount() later

                        PagingResults<BlogPostInfo> pagedResults =
                                BLOG_SERVICE.getDrafts(BLOG_CONTAINER_NODE, ADMIN_USER, pagingReq);
                        assertEquals(
                                "Wrong total result count.",
                                arbitraryNumberGreaterThanPageSize,
                                (int) pagedResults.getTotalResultCount().getFirst());

                        while (pagedResults.hasMoreItems()) {
                            recoveredBlogPosts.addAll(pagedResults.getPage());
                            pagingReq =
                                    new PagingRequest(
                                            pagingReq.getSkipCount() + pageSize, pageSize, null);
                            pagedResults =
                                    BLOG_SERVICE.getDrafts(
                                            BLOG_CONTAINER_NODE, ADMIN_USER, pagingReq);
                        }
                        // and the last page, which only has 2 items in it.
                        recoveredBlogPosts.addAll(pagedResults.getPage());

                        assertEquals(
                                "Wrong number of blog posts.",
                                submittedBlogPosts.size(),
                                recoveredBlogPosts.size());

                        // Check the list is sorted by cm:created, descending order.
                        assertNodeRefsAreSortedBy(
                                recoveredBlogPosts, ContentModel.PROP_CREATED, false);
    ...
    /**
     * This method asserts that the given List<BlogPostInfo> has NodeRefs in order of the specified
     * date property.
     *
     ...
     */
    private void assertNodeRefsAreSortedBy(
            List<BlogPostInfo> blogPosts, QName property, boolean ascendingOrder) {
        ...
        if (blogPosts.size() > 1) {
            for (int i = 0; i < blogPosts.size() - 1; i++) {
                ...
                if (ascendingOrder) {
                    assertTrue(
                            "BlogPosts not asc-sorted by " + property + ". Error at index " + i,
                            date1.before(date2));
                } else {
                    assertTrue(
                            "BlogPosts not desc-sorted by " + property + ". Error at index " + i,
                            date1.after(date2));
                }
            }
        }
    ...
    public void createTaggedDraftBlogPost() throws Exception {
                ...
                new RetryingTransactionHelper.RetryingTransactionCallback<List<NodeRef>>() {
                    ...
                    public List<NodeRef> execute() throws Throwable {
                        ...
                        for (String tag : tags) {
                            ...
                            BlogPostInfo newBlogPost =
                                    BLOG_SERVICE.createBlogPost(
                                            BLOG_CONTAINER_NODE, blogTitle, "Hello world", true);
                            TAGGING_SERVICE.addTags(
                                    newBlogPost.getNodeRef(), Arrays.asList(new String[] {tag}));
                            testNodesToTidy.add(newBlogPost.getNodeRef());
                            results.add(newBlogPost.getNodeRef());
                        }
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    @Override
                    public Void execute() throws Throwable {
                        // Now we'll recover these blogposts & we should expect to find the same
                        // tags.
                        ...
                        for (BlogPostInfo bpi : pagedResults.getPage()) {
                            ...
                            assertTrue(
                                    "Tag found on node but not expected: " + tag,
                                    expectedTags.remove(tag));
                        }
                        assertTrue(
                                "Not all tags were recovered from a blogpost",
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        PagingRequest pagingReq = new PagingRequest(0, 10, null);
                        RangedDateProperty dates =
                                new RangedDateProperty(null, null, ContentModel.PROP_CREATED);
                        for (String tag : tags) {
                            PagingResults<BlogPostInfo> pagedResults =
                                    BLOG_SERVICE.findBlogPosts(
                                            BLOG_CONTAINER_NODE, dates, tag, pagingReq);

                            // Check we found our post
                            assertEquals(
                                    "Wrong number of blog posts for " + tag,
                                    1,
                                    pagedResults.getPage().size());
                        }
                        return null;
    ...
    /**
     * This test method uses the eventually consistent find*() method and so may fail if Lucene is
     * disabled.
     */
    @Category(RedundantTests.class)
    @Test
    public void findBlogPostsByPublishedDate() throws Exception {
        ...
        final Map<Integer, NodeRef> blogPosts =
                TRANSACTION_HELPER.doInTransaction(
                        new RetryingTransactionHelper.RetryingTransactionCallback<
                            ...
                            public Map<Integer, NodeRef> execute() throws Throwable {
                                ...
                                // Create some blog posts. They'll all be published 'now' of
                                // course...
                                final BlogPostInfo blogPost1971 =
                                        BLOG_SERVICE.createBlogPost(
                                                BLOG_CONTAINER_NODE,
                                                "publishedPostWithTags1971",
                                                "Hello world",
                                                true);
                                final BlogPostInfo blogPost1981 =
                                        BLOG_SERVICE.createBlogPost(
                                                BLOG_CONTAINER_NODE,
                                                "publishedPostWithTags1981",
                                                "Hello world",
                                                true);
                                final BlogPostInfo blogPost1991 =
                                        BLOG_SERVICE.createBlogPost(
                                                BLOG_CONTAINER_NODE,
                                                "publishedPostWithTags1991",
                                                "Hello world",
                                                true);
                                ...
                                // We need to 'cheat' and set the nodes' cm:published dates to
                                // specific values.
                                NODE_SERVICE.setProperty(
                                        blogPost1971.getNodeRef(),
                                        ContentModel.PROP_PUBLISHED,
                                        _1971);
                                NODE_SERVICE.setProperty(
                                        blogPost1981.getNodeRef(),
                                        ContentModel.PROP_PUBLISHED,
                                        _1981);
                                NODE_SERVICE.setProperty(
                                        blogPost1991.getNodeRef(),
                                        ContentModel.PROP_PUBLISHED,
                                        _1991);
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        final RangedDateProperty publishedBefore1980 =
                                new RangedDateProperty(null, _1980, ContentModel.PROP_PUBLISHED);
                        final RangedDateProperty publishedAfter1980 =
                                new RangedDateProperty(_1980, null, ContentModel.PROP_PUBLISHED);
                        final RangedDateProperty publishedBetween1975And1985 =
                                new RangedDateProperty(_1975, _1985, ContentModel.PROP_PUBLISHED);

                        // Find all
                        PagingResults<BlogPostInfo> pagedResults =
                                BLOG_SERVICE.findBlogPosts(
                                        BLOG_CONTAINER_NODE, null, null, pagingReq);
                        assertEquals(
                                "Wrong number of blog posts", 3, pagedResults.getPage().size());
                        ...
                        assertTrue(
                                "Missing expected BlogPost NodeRef 71",
                                recoveredBlogNodes.contains(blogPosts.get(1971)));
                        assertTrue(
                                "Missing expected BlogPost NodeRef 81",
                                recoveredBlogNodes.contains(blogPosts.get(1981)));
                        assertTrue(
                                "Missing expected BlogPost NodeRef 91",
                                recoveredBlogNodes.contains(blogPosts.get(1991)));

                        // Find posts before date
                        pagedResults =
                                BLOG_SERVICE.findBlogPosts(
                                        BLOG_CONTAINER_NODE, publishedBefore1980, null, pagingReq);
    ...
    public void ensureBlogPostsAreCorrectlySorted() throws Exception {
        ...
        // Set up some test data to check sorting. We don't need to retain references to these
        // posts.
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    @Override
                    public Void execute() throws Throwable {
                        // Create some blog posts. They'll all be published 'now' but the slight
                        // delay between each should ensure they
                        // are given distinct creation dates
                        final long slightDelay = 50;

                        for (int i = 0; i < testBlogCount; i++) {
                            BlogPostInfo newDraft =
                                    BLOG_SERVICE.createBlogPost(
                                            BLOG_CONTAINER_NODE,
                                            "draftPost_ensureBlogPostsAreCorrectlySorted" + i,
                                            "x",
                                            true);
                            ...
                            BlogPostInfo newPublished =
                                    BLOG_SERVICE.createBlogPost(
                                            BLOG_CONTAINER_NODE,
                                            "publishedPost_ensureBlogPostsAreCorrectlySorted" + i,
                                            "x",
                                            false);
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        // get DRAFTS
                        PagingResults<BlogPostInfo> resultsPage =
                                BLOG_SERVICE.getDrafts(BLOG_CONTAINER_NODE, currentUser, pagingReq);
                        List<BlogPostInfo> blogPosts = resultsPage.getPage();
                        assertTrue(
                                "Expected more draft blog posts than " + blogPosts.size(),
                        ...
                        resultsPage =
                                BLOG_SERVICE.getPublished(
                                        BLOG_CONTAINER_NODE,
                                        null,
                                        null,
                                        currentUser,
                                        pagingReq); // Date filtering tested elsewhere.
                        blogPosts = resultsPage.getPage();
                        assertTrue(
                                "Expected more published blog posts than " + blogPosts.size(),
                                blogPosts.size() >= testBlogCount);
                        ...
                        resultsPage =
                                BLOG_SERVICE.getMyDraftsAndAllPublished(
                                        BLOG_CONTAINER_NODE, null, null, pagingReq);
                        blogPosts = resultsPage.getPage();
    ...
    private void assertSortingIsCorrect(List<BlogPostInfo> blogPosts) {
        ...
        for (BlogPostInfo bpi : blogPosts) {
            System.out.println("  -----");
            Date published =
                    (Date) NODE_SERVICE.getProperty(bpi.getNodeRef(), ContentModel.PROP_PUBLISHED);
            Date created =
                    (Date) NODE_SERVICE.getProperty(bpi.getNodeRef(), ContentModel.PROP_CREATED);
            System.out.print(
                    "    published: "
                            + (published == null ? "             " : published.getTime()));
            System.out.println("    created  : " + created.getTime());
        }

        for (int i = 0;
                i < blogPosts.size() - 1;
                i++) // We only want to iterate to the second-last item
        {
            BlogPostInfo nextBPI = blogPosts.get(i);
            BlogPostInfo followingBPI = blogPosts.get(i + 1);

            Date nextPublishedDate =
                    (Date)
                            NODE_SERVICE.getProperty(
                                    nextBPI.getNodeRef(), ContentModel.PROP_PUBLISHED);
            Date followingPublishedDate =
                    (Date)
                            NODE_SERVICE.getProperty(
                                    followingBPI.getNodeRef(), ContentModel.PROP_PUBLISHED);
            Date nextCreatedDate =
                    (Date)
                            NODE_SERVICE.getProperty(
                                    nextBPI.getNodeRef(), ContentModel.PROP_CREATED);
            Date followingCreatedDate =
                    (Date)
                            NODE_SERVICE.getProperty(
                                    followingBPI.getNodeRef(), ContentModel.PROP_CREATED);
            ...
            if (nextPublishedDate == null && followingPublishedDate != null) {
                ...
                assertTrue(
                        "Error in BlogPostInfo sorting. Published dates in wrong order.",
                        !nextPublishedDate.before(followingPublishedDate));
            } else if (nextPublishedDate == null && followingPublishedDate == null) {
                assertTrue(
                        "Error in BlogPostInfo sorting. Created dates in wrong order.",
                        !nextCreatedDate.before(followingCreatedDate));
            }
        }
    }

    /**
     * This test uses two different users to create draft and internally published blog posts. Then
     * it ensures that each user sees the correct posts when they retrieve them from the service.
     */
    @Test
    public void multipleUsersCreateDraftsAndPublishedPostsAndBrowse() throws Exception {
        ...
        final Pair<NodeRef, NodeRef> adminPosts =
                TRANSACTION_HELPER.doInTransaction(
                        new RetryingTransactionHelper.RetryingTransactionCallback<
                            ...
                            public Pair<NodeRef, NodeRef> execute() throws Throwable {
                                BlogPostInfo newDraftBlogPost =
                                        BLOG_SERVICE.createBlogPost(
                                                BLOG_CONTAINER_NODE, "adminDraft", "", true);
                                testNodesToTidy.add(newDraftBlogPost.getNodeRef());

                                BlogPostInfo newPublishedBlogPost =
                                        BLOG_SERVICE.createBlogPost(
                                                BLOG_CONTAINER_NODE, "adminPublished", "", false);
                                testNodesToTidy.add(newPublishedBlogPost.getNodeRef());

                                return new Pair<NodeRef, NodeRef>(
                                        newDraftBlogPost.getNodeRef(),
                                        newPublishedBlogPost.getNodeRef());
                            }
                        });

        // Then another user does the same.
        AuthenticationUtil.setFullyAuthenticatedUser(TEST_USER);
        final Pair<NodeRef, NodeRef> userPosts =
                TRANSACTION_HELPER.doInTransaction(
                        new RetryingTransactionHelper.RetryingTransactionCallback<
                                Pair<NodeRef, NodeRef>>() {
                            ...
                            public Pair<NodeRef, NodeRef> execute() throws Throwable {
                                BlogPostInfo newDraftBlogPost =
                                        BLOG_SERVICE.createBlogPost(
                                                BLOG_CONTAINER_NODE, "userDraft", "", true);
                                testNodesToTidy.add(newDraftBlogPost.getNodeRef());

                                BlogPostInfo newPublishedBlogPost =
                                        BLOG_SERVICE.createBlogPost(
                                                BLOG_CONTAINER_NODE, "userPublished", "", false);
                                testNodesToTidy.add(newPublishedBlogPost.getNodeRef());
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        PagingResults<BlogPostInfo> pagedDraftResults =
                                BLOG_SERVICE.getDrafts(BLOG_CONTAINER_NODE, ADMIN_USER, pagingReq);
                        assertEquals(
                                "Wrong number of admin draft blog posts",
                                1,
                                pagedDraftResults.getPage().size());
                        NodeRef blogNode = pagedDraftResults.getPage().get(0).getNodeRef();
                        assertEquals(
                                "Incorrect admin draft NodeRef.", blogNode, adminPosts.getFirst());

                        PagingResults<BlogPostInfo> pagedPublishedResults =
                                BLOG_SERVICE.getPublished(
                                        BLOG_CONTAINER_NODE, null, null, ADMIN_USER, pagingReq);
                        assertEquals(
                                "Wrong number of admin published blog posts",
                                1,
                                pagedPublishedResults.getPage().size());
                        blogNode = pagedPublishedResults.getPage().get(0).getNodeRef();
                        assertEquals(
                                "Incorrect admin published NodeRef.",
                                blogNode,
                                adminPosts.getSecond());
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        PagingResults<BlogPostInfo> pagedDraftResults =
                                BLOG_SERVICE.getDrafts(BLOG_CONTAINER_NODE, TEST_USER, pagingReq);
                        assertEquals(
                                "Wrong number of user draft blog posts",
                                1,
                                pagedDraftResults.getPage().size());
                        NodeRef blogNode = pagedDraftResults.getPage().get(0).getNodeRef();
                        assertEquals(
                                "Incorrect user draft NodeRef.", blogNode, userPosts.getFirst());

                        PagingResults<BlogPostInfo> pagedPublishedResults =
                                BLOG_SERVICE.getPublished(
                                        BLOG_CONTAINER_NODE, null, null, TEST_USER, pagingReq);
                        assertEquals(
                                "Wrong number of user published blog posts",
                                1,
                                pagedPublishedResults.getPage().size());
                        blogNode = pagedPublishedResults.getPage().get(0).getNodeRef();
                        assertEquals(
                                "Incorrect user published NodeRef.",
                                blogNode,
                                userPosts.getSecond());
                ...
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        for (NodeRef nr : posts) {
                            ...
                            // This should allow us to set the cm:created date without auditing
                            // overwriting our value.
                            // These nodes get deleted after the test anyway.
                            BEHAVIOUR_FILTER.disableBehaviour(nr, ContentModel.ASPECT_AUDITABLE);

                            // Yes, cm:published will be before cm:created. But I don't think that
                            // matter.
                    ...
                    public Void execute() throws Throwable {
                        ...
                        PagingResults<BlogPostInfo> pagedResults =
                                BLOG_SERVICE.getPublished(
                                        ...
                                        pagingReq);
                        assertEquals(
                                "Wrong number of user blog posts",
                                31,
                                pagedResults.getPage().size());
                ...
                new RetryingTransactionHelper.RetryingTransactionCallback<List<NodeRef>>() {
                    ...
                    public List<NodeRef> execute() throws Throwable {
                        ...
                        do {
                            ...
                            BlogPostInfo newBlogPost =
                                    BLOG_SERVICE.createBlogPost(
                                            BLOG_CONTAINER_NODE, blogTitle, "Hello world", false);
                            TAGGING_SERVICE.addTags(
                                    newBlogPost.getNodeRef(),
                                    Arrays.asList(new String[] {tagToSearchBy}));
                            testNodesToTidy.add(newBlogPost.getNodeRef());
                            results.add(newBlogPost.getNodeRef());
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        PagingResults<BlogPostInfo> pagedResults =
                                BLOG_SERVICE.findBlogPosts(
                                        BLOG_CONTAINER_NODE, dates, tagToSearchBy, pagingReq);

                        assertEquals(
                                "Wrong number of blog posts on page 1 for " + tagToSearchBy,
                                1,
                                pagedResults.getPage().size());

                        assertEquals(
                                "Wrong total number of blog posts for " + tagToSearchBy,
                                new Pair<Integer, Integer>(2, 2),
                                pagedResults.getTotalResultCount());

                        assertEquals(
                                "There should still be blog posts available to be retrieved for "
                                        + tagToSearchBy,
                                true,
                                pagedResults.hasMoreItems());
                        ...
                        assertEquals(
                                "Wrong number of blog posts on page 2 for " + tagToSearchBy,
                                1,
                                pagedResults.getPage().size());
                        ...
                        assertEquals(
                                "All blog posts should have been retrieved by now for "
                                        + tagToSearchBy,
                                false,
                                pagedResults.hasMoreItems());
                        return null;
                ...
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        if (!AUTHENTICATION_SERVICE.authenticationExists(userName)) {
                            AUTHENTICATION_SERVICE.createAuthentication(

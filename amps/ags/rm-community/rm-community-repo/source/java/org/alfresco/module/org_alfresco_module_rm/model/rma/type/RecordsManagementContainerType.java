...
public class RecordsManagementContainerType extends BaseBehaviourBean
    ...
    /**
     * @see
     *     org.alfresco.repo.node.NodeServicePolicies.OnCreateChildAssociationPolicy#onCreateChildAssociation
    ...
    public void onCreateChildAssoiationFirstEvent(
            final ChildAssociationRef childAssocRef, final boolean bNew) {
        AuthenticationUtil.runAsSystem(
                new RunAsWork<Void>() {
                    @Override
                    public Void doWork() {
                        QName parentType = nodeService.getType(childAssocRef.getParentRef());
                        boolean isContentSubType =
                                dictionaryService.isSubClass(
                                        nodeService.getType(childAssocRef.getChildRef()),
                                        ContentModel.TYPE_CONTENT);
                        boolean parentIsUnfiledRecordContainer =
                                parentType.equals(
                                        RecordsManagementModel.TYPE_UNFILED_RECORD_CONTAINER);
                        boolean parentIsUnfiledRecordFolder =
                                parentType.equals(
                                        RecordsManagementModel.TYPE_UNFILED_RECORD_FOLDER);

                        NodeRef child = childAssocRef.getChildRef();
                        if ((parentIsUnfiledRecordContainer || parentIsUnfiledRecordFolder)
                                && isContentSubType
    ...
    /**
     * @see
     *     org.alfresco.repo.node.NodeServicePolicies.OnCreateChildAssociationPolicy#onCreateChildAssociation(ChildAssociationRef,
     *     boolean)
     */
    @Behaviour(
            kind = BehaviourKind.ASSOCIATION,
    ...
    public void onCreateChildAssociation(
            final ChildAssociationRef childAssocRef, boolean isNewNode) {
        AuthenticationUtil.runAsSystem(
                new RunAsWork<Void>() {
                    @Override
                    public Void doWork() {
                        ...
                        if (nodeService.exists(child)) {
                            ...
                            if (dictionaryService.isSubClass(childType, ContentModel.TYPE_FOLDER)
                                ...
                                if (dictionaryService.isSubClass(
                                        childType, ContentModel.TYPE_SYSTEM_FOLDER)) {
                                    // this is a rule container, make sure it is an file plan
                                    // component
                                    nodeService.addAspect(child, ASPECT_FILE_PLAN_COMPONENT, null);
                                } else {
                                    // Catch all to generate the rm id (assuming it doesn't already
                                    // have one!)
     ...
     *
     * @see
     *     org.alfresco.repo.node.NodeServicePolicies.OnDeleteAssociationPolicy#onDeleteAssociation(org.alfresco.service.cmr.repository.AssociationRef)
     */
    @Override
    @Behaviour(
            kind = BehaviourKind.ASSOCIATION,
                    ...
                    public Object doWork() {
                        if (nodeService.hasAspect(nodeRef, ASPECT_FILE_PLAN_COMPONENT)
                            ...
                            nodeService.setProperty(
                                    nodeRef, RecordsManagementModel.PROP_IDENTIFIER, id);
                        }
                        return null;
                    }
                });
    ...
    /**
     * Converted the child node to a fileplan component The conversion is needed here to be able to
     * generate the identifier If there is no conversion rule for the created type nothing happens
     * and the current type is returned
     *
    ...
    protected QName convertNodeToFileplanComponent(final ChildAssociationRef childAssocRef) {
        ...
        if (childType.equals(ContentModel.TYPE_FOLDER)) {
            ...
            if (parentType.equals(TYPE_UNFILED_RECORD_CONTAINER)
                    || parentType.equals(TYPE_UNFILED_RECORD_FOLDER)) {

...
public class DBQueryEngineTest {
    ...
    @Test
    public void
            shouldResultSetHaveCorrectAmountOfRequiredNodesWhenSomeAreExcludedDueToDeclinedPermission() {
        withMaxItems(5);
    ...
    @Test
    public void
            shouldResultSetLengthMatchTheAmountOfAllAccessibleNodesWhenMaxPermissionCheckEnabled() {
        withMaxItems(5);
    ...
    private void prepareTemplate(DBQuery dbQuery, List<Node> nodes) {
        doAnswer(
                        invocation -> {
                            ResultHandler<Node> handler =
                                    (ResultHandler<Node>) invocation.getArgument(2);
                            for (Node node : nodes) {

...
public class XSLTProcessorTest extends BaseAlfrescoSpringTest {
  ...
  public void before() throws Exception {
    ...
    this.xsltProcessor =
      (TemplateProcessor) this.applicationContext.getBean("xsltProcessor");
    this.templateService =
      (TemplateService) this.applicationContext.getBean("TemplateService");
  ...
  public void testSimplestStringTemplate() throws Exception {
    try {
      ...
      model.put(
        ...
        XMLUtil.parse(file.getNodeRef(), contentService)
      );
  ...
  public void testLocalisedClasspathTemplate() throws Exception {
    try {
      ...
      log.debug("XSLT Processor output: " + output);
      assertEquals(
        "G'day, Avocado DipBagels, New York StyleBeef Frankfurter, Quarter PoundChicken Pot PieCole SlawEggsHazelnut SpreadPotato ChipsSoy Patties, GrilledTruffles, Dark Chocolate",
        output
      );
      ...
      log.debug("XSLT Processor output: " + output);
      assertEquals(
        "Bonjour, Avocado DipBagels, New York StyleBeef Frankfurter, Quarter PoundChicken Pot PieCole SlawEggsHazelnut SpreadPotato ChipsSoy Patties, GrilledTruffles, Dark Chocolate",
        output
      );
    } catch (Exception ex) {
      log.error("Error!", ex);
  ...
  private FileInfo createXmlFile(NodeRef folder, String content) {
    ...
    FileInfo testXmlFile = fileFolderService.create(
      folder,
      name + ".xml",
      ContentModel.TYPE_CONTENT
    );
    ContentWriter writer = contentService.getWriter(
      testXmlFile.getNodeRef(),

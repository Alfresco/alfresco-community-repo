...
public class LargeArchiveAndRestoreTest extends TestCase {
  ...
  private static Log logger = LogFactory.getLog(
    LargeArchiveAndRestoreTest.class
  );
  ...
  private class CreateDataCallback
    implements RetryingTransactionCallback<NodeRef> {
    ...
    public NodeRef execute() throws Throwable {
      ...
      System.out.println(
        "Creating folder " + foldername + " in parent " + parentNodeRef
      );
    ...
    private NodeRef addFolder(
      ...
      FileInfo info = fileFolderService.create(
        parentNodeRef,
        foldername,
        ContentModel.TYPE_FOLDER
      );
      ...
      if (!name.equals(foldername)) {
        String msg =
          "A foldername '" + foldername + "' was not persisted: " + info;
        logger.error(msg);
  ...
  public void testCreateAndRestore() throws Exception {
    ...
    final NodeRef parentNodeRef = AuthenticationUtil.runAs(
      createHierarchyWork,
    ...
    final RetryingTransactionCallback<NodeRef> deleteHierarchyCallback = new RetryingTransactionCallback<NodeRef>() {
      public NodeRef execute() throws Throwable {
        ...
        NodeRef archivedParentNodeRef = new NodeRef(
          "archive",
        ...
        if (!nodeService.exists(archivedParentNodeRef)) {
          throw new InvalidNodeRefException(
            "Archived node not found after delete: " + archivedParentNodeRef,
    ...
    final NodeRef archivedParentNodeRef = AuthenticationUtil.runAs(
      deleteHierarchyWork,
      AuthenticationUtil.SYSTEM_USER_NAME
    );
    // Restore it
    final RetryingTransactionCallback<NodeRef> restoreHierarchyCallback = new RetryingTransactionCallback<NodeRef>() {
      public NodeRef execute() throws Throwable {
        // Delete it
        List<RestoreNodeReport> report = nodeArchiveService.restoreArchivedNodes(
        ...
        if (nodeService.exists(archivedParentNodeRef)) {
          throw new InvalidNodeRefException(
            "Archived node was found after restore: " + archivedParentNodeRef,
            archivedParentNodeRef
          );
        }
        if (!nodeService.exists(parentNodeRef)) {
    ...
    final NodeRef checkParentNodeRef = AuthenticationUtil.runAs(
      restoreHierarchyWork,
      AuthenticationUtil.SYSTEM_USER_NAME
    );
    assertEquals(

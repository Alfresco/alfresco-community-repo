...
public class ImapMessageTest extends TestCase {
    ...
    private static final String TEST_FOLDER =
            "Alfresco IMAP/" + IMAP_FOLDER_NAME + "/___-___folder_a/" + "___-___folder_a_a";
    ...
    public void setUp() throws Exception {
        ...
        this.store.connect(
                imapServer.getHost(), imapServer.getPort(), anotherUserName, anotherUserName);
    ...
    public void testMessageModifiedBetweenReads() throws Exception {
        ...
        assertFalse(
                Arrays.equals(bodyNew.getByteArray().getBytes(), body.getByteArray().getBytes()));
        ...
        for (int i = 0; i < 51; i++) {
            FileInfo fi =
                    fileFolderService.create(
                            nodeService.getParentAssocs(contentNode).get(0).getParentRef(),
                            "test" + i,
    ...
    public void testSearchTermWithNonEnglishLocaleFalsePositive() throws Exception {
        ...
        try {
            ...
            Response[] ret = (Response[]) folder.doCommand(uid_search_since);
            assertEquals(
                    "java.net.SocketException: Connection reset", ret[0].getException().toString());
        } catch (MessagingException e) {
            // expected
        } finally {
            Locale.setDefault(defaultLocale);
    ...
    public void testEncodedFromToAddresses() throws Exception {
        ...
        // <ars.kov@gmail.com>>
        // NOTE! This should be coincided with RFC822MetadataExtracter. Would 'addresses' be quoted
        // or not?
        // String decodedAddress = address.toUnicodeString();
        // Starting from javax.mail 1.5.6 the address is folded at linear whitespace so that each
        ...
        // InternetAddress.toString(new Address[] {address}) - is used in the
        // RFC822MetadataExtracter
        // So, compare with that
        assertFalse(
                "Non ASCII characters in the address should be encoded",
        ...
        FileInfo messageFile =
                fileFolderService.create(testImapFolderNodeRef, name, ContentModel.TYPE_CONTENT);
        // Writing a content.
        ...
        // Calls to new IncomingImapMessage(...) only takes place when a new nodeRef is being
        // created.
        // No other code will be changing the nodeRef. An ImapModel.ASPECT_IMAP_CONTENT is added,
        // which
        // triggers a metadata extract to take place in a post commit method. Previously this would
        // have been a
        // synchronous process. This is no longer true as it may now take place in a T-Engine. So,
        // we need to wait
        ...
        // cm:originator, cm:addressee, cm:addressees, imap:messageFrom, imap:messageTo,
        // imap:messageCc
    ...
    public void testEightBitMessage() throws Exception {
        ...
        lstore.connect(
                imapServer.getHost(), imapServer.getPort(), ADMIN_USER_NAME, ADMIN_USER_PASSWORD);
        ...
        try {
            ...
            Message message =
                    new MimeMessage(
                            Session.getDefaultInstance(new Properties()), messageFileInputStream1);
            ...
            List<NodeRef> nodeRefs =
                    searchService.selectNodes(
                            storeRootNodeRef, messageXPath, params, namespaceService, true);
            ...
            assertTrue(
                    "Original message doesn't coincide to the message processed by the repository",
    ...
    private static RFC822DATA getRFC822Message(final IMAPFolder folder, final long uid)
                            ...
                            public Object doCommand(IMAPProtocol p) throws ProtocolException {
                                ...
                                if (!response.isOK()) {
                                    throw new ProtocolException(
                                            "Unable to retrieve message in RFC822 format");
                                }
     ...
     * @throws MessagingException
     */
    private static BODY getMessageBodyPart(
            IMAPFolder folder, final Long uid, final Integer from, final Integer count)
            throws MessagingException {
        return (BODY)
                folder.doCommand(
                        new IMAPFolder.ProtocolCommand() {
                            public Object doCommand(IMAPProtocol p) throws ProtocolException {
                                ...
                                if (!response.isOK()) {
                                    throw new ProtocolException(
                                            "Unable to retrieve message part <"
                                                    + from
                                                    + "."
                                                    + count
                                                    + ">");
    ...
    private NodeRef findNode(String path) {
        List<NodeRef> nodeRefs =
                searchService.selectNodes(storeRootNodeRef, path, null, namespaceService, false);
                ...
                folder.doCommand(
                        new IMAPFolder.ProtocolCommand() {
                            public Object doCommand(IMAPProtocol p) throws ProtocolException {
                                ...
                                /**
                                 * Uh-oh - this is where we would intermittently fall over with a
                                 * class cast exception. The following code probes why we don't have
                                 * a FetchResponse
                                 */
                                StringBuffer sb = new StringBuffer();
     ...
     * @throws MessagingException
     */
    private static Integer getMessageSize(IMAPFolder folder, final Long uid)
            throws MessagingException {
     ...
     * @throws MessagingException
     */
    private static BODY getMessageBody(IMAPFolder folder, final Long uid)
            throws MessagingException {
        return (BODY)
                folder.doCommand(
                        new IMAPFolder.ProtocolCommand() {
                            public Object doCommand(IMAPProtocol p) throws ProtocolException {
                                Response[] r =
                                        p.command(
                                                "UID FETCH " + uid + " (FLAGS BODY.PEEK[])", null);
                                logResponse(r);
                                Response response = r[r.length - 1];

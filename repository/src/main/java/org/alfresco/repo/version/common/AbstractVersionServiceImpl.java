...
public abstract class AbstractVersionServiceImpl {
  ...
  public void initialise() {
    ...
    this.beforeCreateVersionDelegate =
      this.policyComponent.registerClassPolicy(
          VersionServicePolicies.BeforeCreateVersionPolicy.class
        );
    this.afterCreateVersionDelegate =
      this.policyComponent.registerClassPolicy(
          VersionServicePolicies.AfterCreateVersionPolicy.class
        );
    this.onCreateVersionDelegate =
      this.policyComponent.registerClassPolicy(
          VersionServicePolicies.OnCreateVersionPolicy.class
        );
    this.calculateVersionLabelDelegate =
      this.policyComponent.registerClassPolicy(
          VersionServicePolicies.CalculateVersionLabelPolicy.class
        );
    this.onRevertVersionDelegate =
      this.policyComponent.registerClassPolicy(
          VersionServicePolicies.OnRevertVersionPolicy.class
        );
    this.afterVersionRevertDelegate =
      this.policyComponent.registerClassPolicy(
          VersionServicePolicies.AfterVersionRevertPolicy.class
        );
  }
  ...
  protected void invokeAfterVersionRevert(NodeRef nodeRef, Version version) {
    ...
    this.afterVersionRevertDelegate.get(nodeRef, nodeTypeQName)
      .afterVersionRevert(nodeRef, version);
    // invoke for node aspects
    Set<QName> nodeAspectQNames = nodeService.getAspects(nodeRef);
    this.afterVersionRevertDelegate.get(nodeRef, nodeAspectQNames)
      .afterVersionRevert(nodeRef, version);
  }
  ...
  protected void invokeBeforeCreateVersion(NodeRef nodeRef) {
    // invoke for node type
    QName nodeTypeQName = nodeService.getType(nodeRef);
    this.beforeCreateVersionDelegate.get(nodeRef, nodeTypeQName)
      .beforeCreateVersion(nodeRef);
    // invoke for node aspects
    Set<QName> nodeAspectQNames = nodeService.getAspects(nodeRef);
    this.beforeCreateVersionDelegate.get(nodeRef, nodeAspectQNames)
      .beforeCreateVersion(nodeRef);
  }
  ...
  protected void invokeAfterCreateVersion(NodeRef nodeRef, Version version) {
    // invoke for node type
    QName nodeTypeQName = nodeService.getType(nodeRef);
    this.afterCreateVersionDelegate.get(nodeRef, nodeTypeQName)
      .afterCreateVersion(nodeRef, version);
    // invoke for node aspects
    Set<QName> nodeAspectQNames = nodeService.getAspects(nodeRef);
    this.afterCreateVersionDelegate.get(nodeRef, nodeAspectQNames)
      .afterCreateVersion(nodeRef, version);
  }
   ...
   */
  protected RevertAspectAction getRevertAspectAction(
    QName aspectName,
    VersionRevertDetails revertDetails
  ) {
    // first check for a policy for the node type
    Collection<OnRevertVersionPolicy> policies = onRevertVersionDelegate.getList(
      revertDetails.getNodeRef(),
      aspectName
    );
    for (OnRevertVersionPolicy policy : policies) {
      VersionRevertCallback cb = policy.getRevertVersionCallback(
        aspectName,
        revertDetails
      );
      if (cb != null) {
        RevertAspectAction action = cb.getRevertAspectAction(
          aspectName,
          revertDetails
        );
   ...
   * @return the action to be taken
   */
  protected RevertAssocAction getRevertAssocAction(
    QName className,
    QName assocName,
    VersionRevertDetails revertDetails
  ) {
    ...
    for (OnRevertVersionPolicy policy : policies) {
      ...
      if (cb != null) {
        RevertAssocAction action = cb.getRevertAssocAction(
          assocName,
          revertDetails
        );
  ...
  private void invokeOnCreateVersion(
    ...
    defaultOnCreateVersion(classRef, nodeRef, versionProperties, nodeDetails);

    // Call the policy definitions
    Collection<OnCreateVersionPolicy> policies =
      this.onCreateVersionDelegate.getList(nodeRef, classRef);
   ...
   */
  protected abstract void defaultOnCreateVersion(
    QName classRef,
    NodeRef nodeRef,
    Map<String, Serializable> versionProperties,
    PolicyScope nodeDetails
  ...
  protected String invokeCalculateVersionLabel(
    ...
    Collection<CalculateVersionLabelPolicy> behaviours =
      this.calculateVersionLabelDelegate.getList(classRef);
    if (behaviours.size() == 0) {
      ...
      versionLabel =
        defaultVersionLabelPolicy.calculateVersionLabel(
          classRef,
          preceedingVersion,
          versionNumber,
          versionProperties
      ...
      CalculateVersionLabelPolicy[] arr = behaviours.toArray(
        new CalculateVersionLabelPolicy[] {}
      );
      versionLabel =
        arr[0].calculateVersionLabel(
            classRef,
            preceedingVersion,
            versionNumber,
            versionProperties
      ...
      throw new VersionServiceException(
        "More than one CalculateVersionLabelPolicy behaviour has been registered for the type " +
        classRef.toString()
      );
    }

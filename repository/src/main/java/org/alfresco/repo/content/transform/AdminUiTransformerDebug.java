...
public class AdminUiTransformerDebug
  ...
  public void setSynchronousTransformClient(
    SynchronousTransformClient transformClient
  ) {
    ...
    try {
      ...
      for (String sourceMimetype : sourceMimetypes) {
        for (String targetMimetype : targetMimetypes) {
          ...
          boolean supportedByTransformService = remoteTransformServiceRegistry ==
            null ||
          ...
          List<SupportedTransform> localTransformers = localTransformServiceRegistry ==
            null
          ...
          if (!localTransformers.isEmpty() || supportedByTransformService) {
            try {
              ...
              if (supportedByTransformService) {
                long maxSourceSizeKBytes = remoteTransformServiceRegistry.findMaxSize(
                  sourceMimetype,
                  targetMimetype,
                  Collections.emptyMap(),
                  null
  ...
  public Collection<String> getSourceMimetypes(String sourceExtension) {
    Collection<String> sourceMimetypes = mimetypeService.getMimetypes(
      sourceExtension
    );
    if (sourceMimetypes.isEmpty()) {
      throw new IllegalArgumentException(
        "Unknown source extension " + sourceExtension
      );
    }
  ...
  public Collection<String> getTargetMimetypes(
    ...
    if (targetMimetypes.isEmpty()) {
      throw new IllegalArgumentException(
        "Unknown target extension " + targetExtension
      );
    }
  ...
  private URL loadQuickTestFile(String extension) {
    ...
    URL url =
      this.getClass().getClassLoader().getResource("quick/quick." + extension);
  ...
  private class TestTransform {
    ...
    private String runWithinTransaction(
      String sourceExtension,
      String targetExtension
    ) {
      ...
      File tempFile = TempFileProvider.createTempFile(
        "TestTransform_" + sourceExtension + "_",
        "." + targetExtension
      );
    ...
    private String getMimetype(String extension, boolean isSource) {
      ...
      if (extension != null) {
        Iterator<String> iterator = mimetypeService
    ...
    public NodeRef createSourceNode(String extension, String sourceMimetype) {
      ...
      NodeRef contentNodeRef = getTransactionService()
        .getRetryingTransactionHelper()

...
public class NodeRefRadixHasher implements NodeRefHasher {
  ...
  public static final NodeRefRadixHasher RADIX_36_HASHER = new NodeRefRadixHasher(
    36
  );
  ...
  public NodeRefRadixHasher(int radix) {
    super();
    this.radix = radix;
    this.storeProtocolStore =
      HashStoreConfiguration.getInstance().getStoreProtocolStore();
  ...
  public Pair<String, String> hash(NodeRef nodeRef) {
    ...
    return new Pair<String, String>(storeHash, bigIntId.toString(radix));
  }
  ...
  public NodeRef lookup(Pair<String, String> hash) {
    ...
    if (storeProtocol == null || storeId == null) {
      throw new RuntimeException(
        "Lookup found no protocol or id for " + storeHash
      );
    }

...
public class CustomModelServiceImplTest {
  ...
  @Rule
  public RunAsFullyAuthenticatedRule runAsRule = new RunAsFullyAuthenticatedRule(
  ...
  public static void initStaticData() throws Exception {
    customModelService =
      APP_CONTEXT_INIT
        ...
        .getBean("customModelService", CustomModelService.class);
    transactionHelper =
      APP_CONTEXT_INIT
        .getApplicationContext()
        .getBean("retryingTransactionHelper", RetryingTransactionHelper.class);
    personService =
      APP_CONTEXT_INIT
        .getApplicationContext()
        .getBean("personService", PersonService.class);
    authorityService =
      APP_CONTEXT_INIT
        .getApplicationContext()
        .getBean("authorityService", AuthorityService.class);
    cmmDownloadTestUtil =
      new CMMDownloadTestUtil(APP_CONTEXT_INIT.getApplicationContext());
  }
  ...
  public void testCreateBasicInactiveModel() throws Exception {
    ...
    M2Model model = M2Model.createModel(
      namespacePair.getSecond() + QName.NAMESPACE_PREFIX + modelName1
    );
    ...
    assertTrue(
      "There is no imported namespace.",
      modelDefinition.getImportedNamespaces().isEmpty()
    );

    NamespaceDefinition namespaceDefinition = modelDefinition
      .getNamespaces()
      .iterator()
      .next();
    ...
    model =
      M2Model.createModel(
        namespacePair.getSecond() + QName.NAMESPACE_PREFIX + modelName2
      );
    ...
    try {
      ...
      fail(
        "Shouldn't be able to create a model with an already in-use namespace uri."
      );
    ...
    model =
      M2Model.createModel(
        namespacePair.getSecond() + QName.NAMESPACE_PREFIX + modelName3
      );
    model.createNamespace(
      getTestNamespacePrefixPair().getFirst(),
      namespacePair.getSecond()
    );
    ...
    try {
      createModel(model, false);
      fail(
        "Shouldn't be able to create a model with an already in-use namespace prefix."
      );
    } catch (NamespaceConstraintException ex) {
      // Expected
    }
    ...
    try {
      ...
      createModel(model, false);
      fail(
        "Shouldn't be able to create a model with the same name as the bootstrapped model."
      );
    } catch (Exception e) {
      // Expected
    }
    ...
    try {
      ...
      fail(
        "Should have thrown IllegalArgumentException as PagingRequest was null."
      );
    } catch (IllegalArgumentException e) {
      // Expected
    }
    final PagingRequest pagingRequest = new PagingRequest(0, Integer.MAX_VALUE);
    PagingResults<CustomModelDefinition> result = transactionHelper.doInTransaction(
      new RetryingTransactionCallback<PagingResults<CustomModelDefinition>>() {
  ...
  public void testListTypesAspects_Empty() throws Exception {
    ...
    final M2Model model = M2Model.createModel(
      namespacePair.getSecond() + QName.NAMESPACE_PREFIX + modelName
    );
    model.createNamespace(namespacePair.getFirst(), namespacePair.getSecond());
  ...
  public void testModelAdmin() throws Exception {
    ...
    final NodeRef personNodeRef = transactionHelper.doInTransaction(
  ...
  public void testisNamespaceUriExists() {
    ...
    assertNull(
      customModelService.getCustomModelByUri(namespacePair.getFirst())
    );
    // Create the model
    CustomModelDefinition modelDefinition = createModel(model, false);
    assertNotNull(modelDefinition);
    ...
    ModelDefinition modelDefinitionByUri = transactionHelper.doInTransaction(
      new RetryingTransactionCallback<ModelDefinition>() {
        @Override
        public ModelDefinition execute() throws Throwable {
          assertTrue(
            customModelService.isNamespaceUriExists(namespacePair.getFirst())
          );
          return customModelService.getCustomModelByUri(
  ...
  public void testCreateModelWithTypesAndAspects() throws Exception {
    ...
    M2Type m2Type = model.createType(
      namespacePair.getSecond() + QName.NAMESPACE_PREFIX + typeName
    );
    ...
    model.createAspect(
      namespacePair.getSecond() + QName.NAMESPACE_PREFIX + aspectName
    );

    try {
      createModel(model, false);
      fail(
        "Shouldn't be able to create a model without namespace imports, when type and/or aspect have parent name."
      );
    } catch (Exception ex) {
      // Expected
    }
    ...
    // list all custom models's types and aspects
    {
      ...
      PagingResults<TypeDefinition> allTypes = transactionHelper.doInTransaction(
        new RetryingTransactionCallback<PagingResults<TypeDefinition>>() {
      ...
      PagingResults<AspectDefinition> allAspects = transactionHelper.doInTransaction(
        new RetryingTransactionCallback<PagingResults<AspectDefinition>>() {
    ...
    QName aspectQName = QName.createQName(
      "{" + namespacePair.getFirst() + "}" + aspectName
    );
    ...
    QName typeQName = QName.createQName(
      "{" + namespacePair.getFirst() + "}" + typeName
    );
    ...
    model.createAspect(
      namespacePair.getSecond() + QName.NAMESPACE_PREFIX + aspectName2
    );
    updateModel(modelName, model, false);

    // Retrieve the created aspect
    aspectQName =
      QName.createQName("{" + namespacePair.getFirst() + "}" + aspectName2);
    ...
    model.createType(
      namespacePair.getSecond() + QName.NAMESPACE_PREFIX + typeName2
    );
    updateModel(modelName, model, false);

    // Retrieve the created type
    typeQName =
      QName.createQName("{" + namespacePair.getFirst() + "}" + typeName2);
...

    {
      ...
      try {
        updateModel(modelName, model, false);
        fail(
          "Shouldn't be able to create a type and an aspect with the same name in a model."
        );
      } catch (Exception ex) {
        // Expected
      }
  ...
  public void testUpdateModel() throws Exception {
    ...
    try {
      updateModel(modelName, model, false);
      fail(
        "Shouldn't be able to update a custom model with an empty namespace."
      );
    ...
    try {
      updateModel(modelName, model, false);
      fail(
        "Model validation should have failed, as the namespace prefix is null."
      );
    ...
    model.createNamespace(
      "http://www.alfresco.org/model/contenttest/1.0",
      namespacePair.getSecond()
    );
    try {
      updateModel(modelName, model, false);
      fail(
        "Shouldn't be able to add more than one namespace URI into a custom model."
      );
    } catch (InvalidNamespaceException ex) {
      // Expected
    }
    ...
    try {
      updateModel(modelName, model, false);
      fail(
        "Shouldn't be able to update a model with an already in-use namespace URI."
      );
    } catch (NamespaceConstraintException ex) {
      // Expected
    }
    ...
    try {
      updateModel(modelName, model, false);
      fail(
        "Shouldn't be able to update a model with an already in-use namespace Prefix."
      );
    } catch (NamespaceConstraintException ex) {
      // Expected
    }
    ...
    model =
      M2Model.createModel(
        newNamespacePair.getSecond() + QName.NAMESPACE_PREFIX + modelName
      );
    model.createNamespace(
      newNamespacePair.getFirst(),
      newNamespacePair.getSecond()
    );
    model.setDescription(desc);
    ...
    model.createNamespace(
      activeModelNamespacePair.getFirst(),
      newNamespacePair.getSecond()
    );
    try {
    ...
    model =
      M2Model.createModel(
        activeModelNamespacePair.getSecond() +
    ...
    try {
      updateModel(modelName, model, true); // true => as we activated the model
      fail(
        "Shouldn't be able to update the namespace prefix of an active model."
      );
    } catch (ActiveModelConstraintException ax) {
      // Expected
    }
    ...
    try {
      updateModel(modelName, model, true); // true => as we activated the model
      fail(
        "Shouldn't be able to update the namespace URI and namespace prefix of an active model."
      );
    } catch (ActiveModelConstraintException ax) {
      // Expected
    }
    ...
    model =
      M2Model.createModel(
        namespaceDefinition.getPrefix() + QName.NAMESPACE_PREFIX + modelName
      );
    model.createNamespace(
  ...
  public void testCreateDownload() throws Exception {
    ...
    String modelEntry = cmmDownloadTestUtil.getDownloadEntry(
      entries,
      modelExportFileName
    );
    ...
    modelEntry =
      cmmDownloadTestUtil.getDownloadEntry(entries, modelExportFileName);
    assertNotNull(modelEntry);
    assertEquals(modelEntry, modelExportFileName);

    String shareExtEntry = cmmDownloadTestUtil.getDownloadEntry(
      entries,
      shareExtExportFileName
    );
    ...
    cmmDownloadTestUtil.createShareExtModule(
  ...
  public void testModelsInfo() throws Exception {
    CustomModelsInfo info = transactionHelper.doInTransaction(
      new RetryingTransactionCallback<CustomModelsInfo>() {
    ...
    CustomModelsInfo newInfo = transactionHelper.doInTransaction(
      new RetryingTransactionCallback<CustomModelsInfo>() {
        public CustomModelsInfo execute() throws Exception {
          return customModelService.getCustomModelsInfo();
        }
    ...
    assertEquals(
      info.getNumberOfActiveModels() + 1,
      newInfo.getNumberOfActiveModels()
    );
    assertEquals(
      info.getNumberOfActiveTypes() + 1,
      newInfo.getNumberOfActiveTypes()
    );
    assertEquals(
      info.getNumberOfActiveAspects() + 1,
      newInfo.getNumberOfActiveAspects()
    );

    // Add another aspect
    String aspectNameTwo = "testMarkerAspectTwo";
    model.createAspect(
      namespacePair.getSecond() + QName.NAMESPACE_PREFIX + aspectNameTwo
    );
    // Update the model
    ...
    assertEquals(
      info.getNumberOfActiveAspects() + 2,
      newInfo.getNumberOfActiveAspects()
    );
  }
  ...
  private Pair<String, String> getTestNamespacePrefixPair() {
    ...
    String uri =
      "http://www.alfresco.org/model/testcmmservicenamespace" +
      timeMillis +
      "/1.0";
  ...
  private CustomModelDefinition createModel(
  ...
  private CustomModelDefinition updateModel(
    ...
    return transactionHelper.doInTransaction(
      new RetryingTransactionCallback<CustomModelDefinition>() {
        public CustomModelDefinition execute() throws Exception {
          return customModelService.updateCustomModel(
            modelName,
            m2Model,
            activate
          );
  ...
  private NodeRef createDownload(
    final String modelName,
    final boolean withShareExtModule
  ) {
    return transactionHelper.doInTransaction(
      new RetryingTransactionCallback<NodeRef>() {
        public NodeRef execute() throws Exception {
  ...
  private AspectDefinition getAspect(final QName aspectQName) {
    return transactionHelper.doInTransaction(
      new RetryingTransactionCallback<AspectDefinition>() {
        @Override
  ...
  private TypeDefinition getType(final QName typeQName) {
    return transactionHelper.doInTransaction(
      new RetryingTransactionCallback<TypeDefinition>() {
        @Override

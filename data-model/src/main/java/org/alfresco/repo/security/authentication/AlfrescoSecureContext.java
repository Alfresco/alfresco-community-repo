 ...
 *
 * <p>This is based on the Linux model and supports real, effective and stored authorities
 *
 * <p>The real authority is used for auditing and reporting who the user is etc. The effective
 * authority is used for permission checks.
 *
 * <p>RunAs support leaves the real authority and changes only the effective authority That means
 * "special" code can run code as system but still be audited as Joe
 *
 * <p>In the future scrips etc can support a setUId flag and run as the owner of the script. If the
 * script chooses to do this .... A method invocation could do the same (after entry security
 * checks)
 *
 * <p>TODO: extent runAs to take a nodeRef context - it can then set the stored atc and set this as

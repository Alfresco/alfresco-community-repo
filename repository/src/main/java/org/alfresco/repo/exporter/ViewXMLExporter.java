...
/*package*/ class ViewXMLExporter implements Exporter {
    ...
    ViewXMLExporter(
        ...
        VIEW_QNAME =
                QName.createQName(
                        NamespaceService.REPOSITORY_VIEW_PREFIX, VIEW_LOCALNAME, namespaceService);
        VALUE_QNAME =
                QName.createQName(
                        NamespaceService.REPOSITORY_VIEW_PREFIX, VALUE_LOCALNAME, namespaceService);
        VALUES_QNAME =
                QName.createQName(
        ...
        ACL_QNAME =
                QName.createQName(
                        NamespaceService.REPOSITORY_VIEW_PREFIX, ACL_LOCALNAME, namespaceService);
        ACE_QNAME =
                QName.createQName(
                        NamespaceService.REPOSITORY_VIEW_PREFIX, ACE_LOCALNAME, namespaceService);
        ACCESS_QNAME =
                QName.createQName(
                        NamespaceService.REPOSITORY_VIEW_PREFIX,
                        ACCESS_LOCALNAME,
    ...
    public void start(ExporterContext context) {
        try {
            ...
            contentHandler.startElement(
                    ...
                    EMPTY_ATTRIBUTES);
            String date =
                    DefaultTypeConverter.INSTANCE.convert(String.class, context.getExportedDate());
    ...
    public void startNamespace(String prefix, String uri) {
        try {
            ...
            throw new ExporterException(
                    "Failed to process start namespace event - prefix " + prefix + " uri " + uri,
                    e);
        }
    }
    ...
    public void endNamespace(String prefix) {
        try {
            ...
            throw new ExporterException(
                    "Failed to process end namespace event - prefix " + prefix, e);
        }
    }
    ...
    public void endNode(NodeRef nodeRef) {
        try {
            QName type = nodeService.getType(nodeRef);
            contentHandler.endElement(
                    type.getNamespaceURI(), type.getLocalName(), toPrefixString(type));
        } catch (SAXException e) {
            throw new ExporterException(
    ...
    public void startProperty(NodeRef nodeRef, QName property) {
        try {
            QName encodedProperty =
                    QName.createQName(
                            property.getNamespaceURI(), ISO9075.encode(property.getLocalName()));
            contentHandler.startElement(
    ...
    public void value(NodeRef nodeRef, QName property, Object value, int index) {
        try {
            ...
            if (dataTypeDef == null || dataTypeDef.getName().equals(DataTypeDefinition.ANY)) {
                dataTypeDef =
                        (value == null) ? null : dictionaryService.getDataType(value.getClass());
                if (dataTypeDef != null) {
            ...
            // output value wrapper if value is null or property data type is ANY or value is part
            // of collection
    ...
    public void startReference(NodeRef nodeRef, QName childName) {
        try {
            ...
            if (referenceFormat.equals(ReferenceType.PATHREF)) {
                Path path =
                        createPath(context.getExportParent(), context.getExportParent(), nodeRef);
                attrs.addAttribute(
                        NamespaceService.REPOSITORY_VIEW_1_0_URI,
                        PATHREF_LOCALNAME,
                    ...
                    relativePath.append(
                            new Path.ChildAssocElement(
                                    new ChildAssociationRef(
                                            null, null, null, childPath.getRef().getParentRef())));
            ...
            if (i > 0 && pathElement instanceof Path.ChildAssocElement) {
                ...
                List<NodeRef> siblings =
                        searchService.selectNodes(
                                nodeRef, searchPath, null, namespaceService, false);
                if (siblings.size() > 1) {

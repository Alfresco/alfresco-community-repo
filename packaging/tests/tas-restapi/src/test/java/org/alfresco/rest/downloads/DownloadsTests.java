...
public class DownloadsTests extends RestTest {
  ...
  public void dataPreparation() throws Exception {
    ...
    folder =
      dataContent.usingUser(adminModel).usingSite(siteModel).createFolder();
    document =
      dataContent
        .usingSite(siteModel)
        .usingUser(adminModel)
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.DOWNLOADS },
  ...
  public void createDownloadNodeAndGetInfo() throws Exception {
    ...
    JsonObject postBody = JsonBodyGenerator
      ...
      .add(
        ...
        JsonBodyGenerator
          .defineJSONArray()
          .add(document.getNodeRefWithoutVersion())
      )
      .build();
    RestDownloadsModel downloadModel = restClient
      .authenticateUser(adminModel)
      .withCoreAPI()
      .usingDownloads()
      .createDownload(postBody.toString());
    ...
    Utility.sleep(
      ...
      15000,
      () -> {
        RestDownloadsModel downloadModel1 = restClient
          .authenticateUser(adminModel)
          .withCoreAPI()
          .usingDownloads(downloadModel)
          .getDownload();
    ...
    restClient
      ...
      .withCoreAPI()
      .usingNode()
      .getNodeContent(downloadModel.getId());
  ...
  public void cancelDownloadNode() throws Exception {
    // POST /downloads
    String postBody = JsonBodyGenerator
      .defineJSON()
      .add(
        "nodeIds",
        JsonBodyGenerator
          .defineJSONArray()
          .add(document1.getNodeRefWithoutVersion())
      )
      .build()
      .toString();
    RestDownloadsModel downloadModel = restClient

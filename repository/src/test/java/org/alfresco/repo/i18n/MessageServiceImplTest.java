...
public class MessageServiceImplTest
  ...
  private static final String BASE_RESOURCE_CLASSPATH =
  ...
  private void setupRepo() throws Exception {
    ...
    String pattern =
      "classpath*:" + BASE_RESOURCE_CLASSPATH + BASE_BUNDLE_NAME + "*";
  ...
  private void addMessageResource(
    NodeRef rootNodeRef,
    String name,
  ...
  public void test2GetMessagesLoadedFromRepo() throws Exception {
    ...
    messageService.registerResourceBundle(
      testStoreRef + "/cm:" + BASE_BUNDLE_NAME
    );

    getMessages();

    messageService.unregisterResourceBundle(
      testStoreRef + "/cm:" + BASE_BUNDLE_NAME
    );
  }
  ...
  public void test3GetMessagesWithParamsLoadedFromRepo() throws Exception {
    ...
    // Check with no bundles loaded
    assertNull(
      messageService.getMessage(MSG_PARAMS, new Object[] { PARAM_VALUE })
    );
  ...
  public void test4GetMessagesLoadedFromClasspath() throws Exception {
    ...
    // Register the bundle
    messageService.registerResourceBundle(
      BASE_RESOURCE_CLASSPATH + BASE_BUNDLE_NAME
    );
  ...
  public void test9LocaleMatching() {
    ...
    assertEquals(
      Locale.CHINA,
      messageService.getNearestLocale(Locale.CHINA, options)
    );
    assertEquals(
      Locale.CHINESE,
      messageService.getNearestLocale(Locale.CHINESE, options)
    );
    ...
    assertTrue(
      chineseMatches.contains(messageService.getNearestLocale(zh_CN_3, options))
    ); // must match the last variant - but set can have any order an IBM JDK differs!
    assertEquals(
      Locale.FRANCE,
      messageService.getNearestLocale(fr_FR_1, options)
    ); // same here
    ...
    assertEquals(
      Locale.GERMAN,
      messageService.getNearestLocale(Locale.GERMANY, options)
    );
  ...
  public void testLocaleParsing() {
    ...
    assertEquals(
      new Locale("en", "GB", "cockney"),
      messageService.parseLocale("en_GB_cockney")
    );
    assertEquals(
      new Locale("en", "GB", ""),
      messageService.parseLocale("en_GB")
    );
  ...
  private void getMessages() {
    ...
    assertEquals(
      VALUE_YES,
      messageService.getMessage(MSG_YES, Locale.getDefault())
    );
    assertEquals(
      VALUE_NO,
      messageService.getMessage(MSG_NO, Locale.getDefault())
    );
  }
  ...
  public void testDictionaryDAOLock() {
    ...
    try {
      if (ddt.isAlive() && mst.isAlive()) {
        fail(
          "Deadlock: DictionaryDAOThread and MessageServiceThread are both still alive."
        );
      } else if (ddt.isAlive()) {
        fail(
          "Possible deadlock with a background process: DictionaryDAOThread is still alive."
        );
      } else if (mst.isAlive()) {
        fail(
          "Possible deadlock with a background process: MessageServiceThread is still alive."
        );
      } else if (!ddt.success) {

...
public class WarHelperImplTest extends WarHelperImpl {
    ...
    public WarHelperImplTest() {
        ...
        config.setArchiveDetector(
                new TArchiveDetector("war|amp", new ZipDriver(IOPoolLocator.SINGLETON)));
    }
    ...
    public void testCheckCompatibleVersion() {
        ...
        try {
            ...
            assertTrue(
                    exception
                            .getMessage()
                            .contains(
                                    "must be installed on a war version equal to or greater than"
                                            + " 10.1"));
        }
        ...
        try {
            ...
            assertTrue(
                    exception
                            .getMessage()
                            .contains("cannot be installed on a war version greater than 3.0"));
        }

        installingModuleDetails.setRepoVersionMax(new VersionNumber("99"));
        this.checkCompatibleVersion(theWar, installingModuleDetails); // does not throw exception

        installingModuleDetails.setRepoVersionMin(
                new VersionNumber("4.1.0")); // current war version
        installingModuleDetails.setRepoVersionMax(
                new VersionNumber("4.1.0")); // current war version
        this.checkCompatibleVersion(theWar, installingModuleDetails); // does not throw exception

        installingModuleDetails.setRepoVersionMin(
                new VersionNumber("3.4.0")); // current war version
        installingModuleDetails.setRepoVersionMax(
                new VersionNumber("4.1.0")); // current war version
        this.checkCompatibleVersion(theWar, installingModuleDetails); // does not throw exception
        ...
        try {
            ...
            fail(
                    "Should not pass as current version is 4.1.0 and the"
                            + " max value is"
                            + " 4.0.999"); // should never get here
        } catch (ModuleManagementToolException exception) {
            assertTrue(
                    exception
                            .getMessage()
    ...
    public void testCheckCompatibleVersionUsingManifest() throws IOException {
        ...
        installingModuleDetails.setRepoVersionMin(
                new VersionNumber("3.4.11")); // current war version
        installingModuleDetails.setRepoVersionMax(
                new VersionNumber("3.4.11")); // current war version
        this.checkCompatibleVersionUsingManifest(
                theWar, installingModuleDetails); // does not throw exception
        ...
        installingModuleDetails.setRepoVersionMin(
                new VersionNumber("3.4.7")); // current war version
        installingModuleDetails.setRepoVersionMax(
                new VersionNumber("3.4.11")); // current war version
        this.checkCompatibleVersionUsingManifest(
                theWar, installingModuleDetails); // does not throw exception
        ...
        try {
            installingModuleDetails.setRepoVersionMin(
                    new VersionNumber("3.4.0")); // current war version
            installingModuleDetails.setRepoVersionMax(
                    new VersionNumber("3.4.10")); // current war version
            this.checkCompatibleVersionUsingManifest(
                    theWar, installingModuleDetails); // does not throw exception
            fail(
                    "Should not pass as current version is 3.4.11 and"
                            + " the max value is"
                            + " 3.4.10"); // should never get here
        } catch (ModuleManagementToolException exception) {
            assertTrue(
                    exception
                            .getMessage()
        ...
        // this should fail BUT we are using a non-numeric version number so instead it passes
        // without validation
        this.checkCompatibleVersionUsingManifest(theWar, installingModuleDetails);
    ...
    public void testCheckCompatibleEdition() {
        ...
        try {
            ...
            assertTrue(
                    exception
                            .getMessage()
                            .endsWith(
                                    "can only be installed in one of the following"
                                            + " editions[CommuniT]"));
        }
        ...
        props.setProperty(
                ModuleDetails.PROP_EDITIONS, ("enterprise,Community")); // should ignore case
        installingModuleDetails = new ModuleDetailsImpl(props);
        this.checkCompatibleVersion(theWar, installingModuleDetails); // does not throw exception
    }
    ...
    public void testfindManifest() throws Exception {
        ...
        assertEquals(
                ...
                manifest.getMainAttributes().getValue(MANIFEST_IMPLEMENTATION_TITLE));
        assertEquals(
                "3.4.11", manifest.getMainAttributes().getValue(MANIFEST_SPECIFICATION_VERSION));
        ...
        assertEquals(
                ...
                manifest.getMainAttributes().getValue(MANIFEST_IMPLEMENTATION_TITLE));
        assertEquals(
                "4.2.a", manifest.getMainAttributes().getValue(MANIFEST_SPECIFICATION_VERSION));
    }

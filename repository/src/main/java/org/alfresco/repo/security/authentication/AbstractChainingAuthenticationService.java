...
public abstract class AbstractChainingAuthenticationService
  ...
  public void updateAuthentication(
    ...
    getMutableAuthenticationService()
      .updateAuthentication(userName, oldPassword, newPassword);
  }
  ...
  public void setAuthenticationEnabled(String userName, boolean enabled)
    ...
    getMutableAuthenticationService()
      .setAuthenticationEnabled(userName, enabled);
  }
    ...
    for (AuthenticationService authService : getUsableAuthenticationServices()) {
      try {
        ...
        if (logger.isTraceEnabled()) {
          logger.trace(
            "Not that important: Exception in chaining the verification of authentication enabled for user: " +
  ...
  public void authenticateAsGuest() throws AuthenticationException {
    ...
    for (AuthenticationService authService : getUsableAuthenticationServices()) {
      try {
        ...
        if (logger.isTraceEnabled()) {
          logger.trace(
            "Not that important: Exception in chaining the authenticate as guest. Message: " +
            e.getMessage(),
            e
          );
        }
  ...
  public void clearCurrentSecurityContext() {
    for (AuthenticationService authService : getUsableAuthenticationServices()) {
      try {
        ...
        if (logger.isTraceEnabled()) {
          logger.trace(
            "Not that important: Exception clearing the current security context. Message: " +
            e.getMessage(),
            e
          );
        }
  ...
  public Set<String> getDomainsThatAllowUserCreation() {
    ...
    if (getMutableAuthenticationService() != null) {
      domains.addAll(
        getMutableAuthenticationService().getDomainsThatAllowUserCreation()
      );
    }
    return domains;
  }
  ...
  public Set<String> getDomainsThatAllowUserDeletion() {
    ...
    if (getMutableAuthenticationService() != null) {
      domains.addAll(
        getMutableAuthenticationService().getDomainsThatAllowUserDeletion()
      );
    }
    return domains;
  }
  ...
  public Set<String> getUsersWithTickets(boolean nonExpiredOnly) {
    ...
    for (AuthenticationService authService : getUsableAuthenticationServices()) {
      if (authService instanceof AbstractAuthenticationService) {
        users.addAll(
          ((AbstractAuthenticationService) authService).getUsersWithTickets(
              nonExpiredOnly
            )
        );
      }
  ...
  public int invalidateTickets(boolean nonExpiredOnly) {
    ...
    for (AuthenticationService authService : getUsableAuthenticationServices()) {
      if (authService instanceof AbstractAuthenticationService) {
        count +=
          ((AbstractAuthenticationService) authService).invalidateTickets(
              nonExpiredOnly
            );
      }
    }
  ...
  public Set<TicketComponent> getTicketComponents() {
    ...
    for (AuthenticationService authService : getUsableAuthenticationServices()) {
      if (authService instanceof AbstractAuthenticationService) {
        tcs.addAll(
          ((AbstractAuthenticationService) authService).getTicketComponents()
        );
      }
    }
    return tcs;
  ...
  public Set<String> getDefaultAdministratorUserNames() {
    ...
    for (AuthenticationService authService : getUsableAuthenticationServices()) {
      defaultAdministratorUserNames.addAll(
        authService.getDefaultAdministratorUserNames()
      );
    }

...
/**
 * This class provides services for translating exploded properties (as persisted in
 * <b>alf_node_properties</b>) in the public form, which is a <tt>Map</tt> of values keyed by their
 * <tt>QName</tt>.
 *
 * @author Derek Hulley
 * @since 3.4
...
public class NodePropertyHelper {
    ...
    public Map<NodePropertyKey, NodePropertyValue> convertToPersistentProperties(
        ...
        Map<NodePropertyKey, NodePropertyValue> propertyMap =
                new HashMap<NodePropertyKey, NodePropertyValue>(in.size() + 5);
    ...
    /**
     * The collection index used to indicate that the value is not part of a collection. All values
     * from zero up are used for real collection indexes.
     */
     ...
     * @param collectionIndex the index of the property in the collection or <tt>-1</tt> if we are
     *     not yet processing a collection
     */
    private void addValueToPersistedProperties(
        ...
        if (propertyTypeQName.equals(DataTypeDefinition.ANY)) {
            // It is multi-valued if required (we are not in a collection and the property is a new
            // collection)
            isMultiValued =
                    (value != null)
        ...
        if (collectionIndex == IDX_NO_COLLECTION
                && isMultiValued
                && !(value instanceof Collection<?>)) {
            // We are not (yet) processing a collection but the property should be part of a
            // collection
            addValueToPersistedProperties(
                    propertyMap, propertyDef, 0, propertyQNameId, propertyLocaleId, value);
        } else if (collectionIndex == IDX_NO_COLLECTION && value instanceof Collection<?>) {
            // We are not (yet) processing a collection and the property is a collection i.e. needs
            // exploding
            ...
            if (value instanceof Collection<?>
                    && !propertyTypeQName.equals(DataTypeDefinition.ANY)) {
                throw new DictionaryException(
                        "Collections of collections (Serializable) are only supported by type"
                                + " 'd:any': \n"
                                + "   Property: "
                                + propertyDef
                                + "\n"
            ...
            if (value instanceof MLText) {
                ...
                for (Map.Entry<Locale, String> mlTextEntry : mlTextValue.entrySet()) {
                    ...
                    // This is persisted against the current locale, but as a d:text instance
                    NodePropertyValue npValue =
                            new NodePropertyValue(DataTypeDefinition.TEXT, mlTextStr);
                    NodePropertyKey npKey = new NodePropertyKey();
                    npKey.setListIndex(collectionIndex);
                    npKey.setQnameId(propertyQNameId);
    ...
    /**
     * Helper method to convert the <code>Serializable</code> value into a full, persistable {@link
     * NodePropertyValue}.
     *
     * <p>Where the property definition is null, the value will take on the {@link
     * DataTypeDefinition#ANY generic ANY} value.
     *
     * <p>Collections are NOT supported. These must be split up by the calling code before calling
     * this method. Map instances are supported as plain serializable instances.
     *
     ...
     */
    public NodePropertyValue makeNodePropertyValue(
            PropertyDefinition propertyDef, Serializable value) {
    ...
    public Serializable getPublicProperty(
            Map<NodePropertyKey, NodePropertyValue> propertyValues, QName propertyQName) {
        // Get the qname ID
        ...
        SortedMap<NodePropertyKey, NodePropertyValue> scratch =
                new TreeMap<NodePropertyKey, NodePropertyValue>();
    ...
    public Map<QName, Serializable> convertToPublicProperties(
        ...
        Map<QName, Serializable> propertyMap =
                new HashMap<QName, Serializable>(propertyValues.size(), 1.0F);
        // Shortcut
        ...
        SortedMap<NodePropertyKey, NodePropertyValue> sortedPropertyValues =
                new TreeMap<NodePropertyKey, NodePropertyValue>(propertyValues);
        ...
        Iterator<Map.Entry<NodePropertyKey, NodePropertyValue>> iterator =
                sortedPropertyValues.entrySet().iterator();
        while (true) {
            ...
            // If the QName is going to change, and we have some entries to process, then process
            // them.
            if (scratch.size() > 0
                ...
                if (scratch.size() == 1) {
                    ...
                    collapsedValue =
                            collapsePropertiesWithSameQNameAndListIndex(
                ...
                if (currentPropertyDef != null && currentPropertyDef.isMultiValued()) {
                    forceCollection = true;
                        ...
                        && scratch.firstKey().getListIndex().intValue() > -1) {
    ...
    private Serializable collapsePropertiesWithSameQName(
        ...
        while (true) {
            ...
            // If the list index is going to change, and we have some entries to process, then
            // process them.
            if (scratch.size() > 0
                ...
                Serializable collapsedValue =
                        collapsePropertiesWithSameQNameAndListIndex(propertyDef, scratch);
     ...
     *
     * <p>If there are multiple values then they can only be separated by locale. If they are
     * separated by locale, then they have to be text-based. This means that the only way to store
     * them is via MLText. Any other multi-locale properties cannot be deserialized.
     */
        ...
        for (Map.Entry<NodePropertyKey, NodePropertyValue> entry : propertyValues.entrySet()) {
            ...
            if (isDefaultLocale && !isMLText) {
                ...
                if (propertyValuesSize > 1) {
                    logger.warn(
                            "Found localized properties along with a 'null' value in the default"
                                    + " locale. \n"
                                    + "   The localized values will be ignored; 'null' will be"
                                    + " returned: \n"
                                    + "   Default locale ID: "
                                    + defaultLocaleId
                                    + "\n"
     ...
     */
    public Serializable makeSerializableValue(
            PropertyDefinition propertyDef, NodePropertyValue propertyValue) {
        ...
        try {
            ...
            if (value instanceof ContentDataId) {
                ...
                value = new ContentDataWithId(contentData, contentDataId);
                    ...
                    && propertyTypeQName.equals(DataTypeDefinition.CONTENT)) {

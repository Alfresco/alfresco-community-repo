...
/**
 * An {@link ExecutionListener} that set all additional variables that are needed when process
 * starts.
 *
...
public class ProcessStartExecutionListener implements ExecutionListener {
    ...
    public void notify(DelegateExecution execution) throws Exception {
        // Add the workflow ID
        String instanceId =
                BPMEngineRegistry.createGlobalId(ActivitiConstants.ENGINE_ID, execution.getId());
        execution.setVariable(WorkflowConstants.PROP_WORKFLOW_INSTANCE_ID, instanceId);

        if (tenantService.isEnabled() || !deployWorkflowsInTenant) {
            // Add tenant as variable to the process. This will allow task-queries to filter out
            // tasks that
        ...
        if (execution instanceof ExecutionEntity) {
            ...
            if (exc.getSuperExecutionId() != null
                    && exc.getVariable(ActivitiConstants.PROP_START_TASK_END_DATE) == null) {

...
public class PublishUpdatesJobExecuter extends RecordsManagementJobExecuter {
  ...
  private static Log logger = LogFactory.getLog(
    PublishUpdatesJobExecuter.class
  );
   ...
   */
  public void setPublishExecutorRegistry(
    PublishExecutorRegistry publishExecutorRegistry
  ) {
    ...
    RetryingTransactionHelper.RetryingTransactionCallback<Void> execution = new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
      ...
      public Void execute() {
        ...
        try {
          ...
          String updateTo = (String) nodeService.getProperty(
            nodeRef,
            PROP_UPDATE_TO
          );
          ...
          if (updateTo != null) {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "Unable to publish, because publish executor is not set."
              );
            }
          }
          ...
          if (logger.isDebugEnabled()) {
            logger.debug(
              "Publish updates complete. (nodeRef=" + nodeRef.toString() + ")"
            );
          }
        } finally {
          behaviourFilter.enableBehaviour(

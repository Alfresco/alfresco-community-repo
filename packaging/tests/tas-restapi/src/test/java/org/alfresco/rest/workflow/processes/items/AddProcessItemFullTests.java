...
public class AddProcessItemFullTests extends RestTest {
    ...
    @TestRail(
            ...
            description =
                    "Adding multiple process items is falling in case of empty process id is"
                            + " provided")
    @Test(
            groups = {
                TestGroup.REST_API,
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Adding process item is falling in case of empty body item value is provided")
    @Test(
            groups = {
                TestGroup.REST_API,
                TestGroup.WORKFLOW,
    ...
    public void createDeleteCreateMultipleProcessItems() throws Exception {
        ...
        restClient
                ...
                .entriesListDoesNotContain(
                        "name", processItems.getEntries().get(0).onModel().getName());

        processItems =
                restClient.withWorkflowAPI().usingProcess(processModel).addProcessItems(document);
        restClient.assertStatusCodeIs(HttpStatus.CREATED);
    }

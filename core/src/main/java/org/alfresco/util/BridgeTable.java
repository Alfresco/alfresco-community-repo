...
/**
 * Generic bridge table support with optional reference counting to allow multiple membership for an
 * object via several relationships.
 *
 * @author Andy
 */
public class BridgeTable<T> {
    HashMap<T, HashMap<Integer, HashMap<T, Counter>>> descendants =
            new HashMap<T, HashMap<Integer, HashMap<T, Counter>>>();

    HashMap<T, HashMap<Integer, HashMap<T, Counter>>> ancestors =
            new HashMap<T, HashMap<Integer, HashMap<T, Counter>>>();
    ...
    private void addDescendants(T parent, T child) {
        ...
        if (ancestorsToFixUp != null) {
            for (Integer ancestorPosition : ancestorsToFixUp.keySet()) {
                HashMap<T, Counter> ancestorsToFixUpAtPosition =
                        ancestorsToFixUp.get(ancestorPosition);
                for (T ancestorToFixUpAtPosition : ancestorsToFixUpAtPosition.keySet()) {
                    HashMap<Integer, HashMap<T, Counter>> ancestorDescendants =
                            descendants.get(ancestorToFixUpAtPosition);
    ...
    private void removeAncestors(T parent, T child) {
        ...
        if (decendantsToFixUp != null) {
            for (Integer descendantPosition : decendantsToFixUp.keySet()) {
                HashMap<T, Counter> decendantsToFixUpAtPosition =
                        decendantsToFixUp.get(descendantPosition);
                for (T descendantToFixUpAtPosition : decendantsToFixUpAtPosition.keySet()) {
                    HashMap<Integer, HashMap<T, Counter>> descendantAncestors =
                            ancestors.get(descendantToFixUpAtPosition);
                    remove(
    ...
    private void addAncestors(T parent, T child) {
        ...
        if (descenantsToFixUp != null) {
            for (Integer descendantPosition : descenantsToFixUp.keySet()) {
                HashMap<T, Counter> descenantsToFixUpAtPosition =
                        descenantsToFixUp.get(descendantPosition);
                for (T descenantToFixUpAtPosition : descenantsToFixUpAtPosition.keySet()) {
                    HashMap<Integer, HashMap<T, Counter>> descendatAncestors =
                            ancestors.get(descenantToFixUpAtPosition);

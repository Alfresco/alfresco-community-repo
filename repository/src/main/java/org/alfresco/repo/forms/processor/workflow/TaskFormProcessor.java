...
public class TaskFormProcessor
  ...
  private String getMessageValue(WorkflowTask task) {
    ...
    String description = (String) task
      .getProperties()
      .get(WorkflowModel.PROP_DESCRIPTION);
    if (description != null) {
  ...
  private String buildTaskOwnerString(String ownerUsername) {
    ...
    if (ownerNodeRef != null) {
      Map<QName, Serializable> personProps =
        this.nodeService.getProperties(ownerNodeRef);

      builder.append("|");
      builder.append(
        personProps.containsKey(ContentModel.PROP_FIRSTNAME)
          ? personProps.get(ContentModel.PROP_FIRSTNAME)
          : ""
      );
      builder.append("|");
      builder.append(
        personProps.containsKey(ContentModel.PROP_LASTNAME)
  ...
  private String getTransitionValues(WorkflowTask item) {
    WorkflowTransition[] transitions = item
      .getDefinition()
      .getNode()
      .getTransitions();
  ...
  private List<String> getHiddenTransitions(WorkflowTask task) {
    Serializable hiddenValues = task
      .getProperties()
      .get(WorkflowModel.PROP_HIDDEN_TRANSITIONS);
    if (hiddenValues != null) {
      if (hiddenValues instanceof List<?>) {
        return (List<String>) hiddenValues;
      } else if (
        hiddenValues instanceof String && ((String) hiddenValues).length() > 0
      ) {
   ...
   */
  @Override
  protected ContentModelFormPersister<WorkflowTask> makeFormPersister(
    WorkflowTask item
  ) {
    ContentModelItemData<WorkflowTask> itemData = makeItemData(item);
    return new TaskFormPersister(
      itemData,
      namespaceService,
      dictionaryService,
      workflowService,

...
public class IdentityServiceAuthenticationComponent
  ...
  private final Log logger = LogFactory.getLog(
    IdentityServiceAuthenticationComponent.class
  );
  ...
  public void setAuthenticatorAuthzClient(
    AuthzClient authenticatorAuthzClient
  ) {
  ...
  public void authenticateImpl(String userName, char[] password)
    ...
    if (authzClient == null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "AuthzClient was not set, possibly due to the 'identity-service.authentication.enable-username-password-authentication=false' property. "
        );
      }
    ...
    try {
      ...
      throw new AuthenticationException(
        "Failed to authenticate user against Keycloak.",
        e
      );
      ...
      if (cause != null) {
        ...
        throw new AuthenticationException(
          "Couldn't connect to Keycloak server to authenticate user.",
          cause
        );
      }
      if (logger.isDebugEnabled()) {
      ...
      throw new AuthenticationException(
        "Error occurred while authenticating user against Keycloak.",
        e
      );
    }
  }

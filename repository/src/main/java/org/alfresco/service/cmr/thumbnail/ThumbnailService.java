...
public interface ThumbnailService {
     ...
     *
     * <p>The mimetype and transformation options are used to determine the content transformer that
     * will be best suited to create the thumbnail.
     *
     * <p>The thumbnail name is optional, but is usually set to provide an easy way to identify a
     * particular 'type' of thumbnail.
     *
     * <p>Once created the source node will have the 'rn:renditioned' aspect applied and an
     * association to the thumbnail node (of type 'rn:rendition') will be created.
     *
     * <p>The returned node reference is to the 'rn:rendition' content node that contains the
     * thumbnail content in the standard 'cm:content' property.
     *
     ...
     */
    @Auditable(
            parameters = {"node", "contentProperty", "mimetype", "transformationOptions", "name"})
    NodeRef createThumbnail(
            NodeRef node,
     ...
     *     <p>If parent association details are specified then the thumbnail is created as a child
     *     of the specified parent and linked via a non-primary association to the original content
     *     node.
     * @param node the source content node
     * @param contentProperty the content property
     * @param mimetype the thumbnail mimetype
     ...
     *
     * <p>The original thumbnail content is updated from the source content using the transformation
     * options provided. The mimetype and name of the thumbnail remain unchanged.
     *
     * <p>To change the name or mimetype of an updated thumbnail it should be deleted and recreated.
     *
     * <p>An error is raised if the original content no longer exists.
     *
     ...
     *
     * <p>Returns null if no thumbnail with that name for that content property is found.
     *
    ...
    /**
     * Gets a list of thumbnail nodes for a given content property that match the provided mimetype
     * and transformation options.
     *
     * <p>Both mimetype and transformation options are optional parameters. If only one or other is
     * specified the only the other is considered during. If neither are provided all thumbnails for
     * that content property are returned.
     *
     * <p>If no matches are found then an empty list is returned.
     *
     * @param node node reference
     * @param contentProperty content property name
    ...
    List<NodeRef> getThumbnails(
            NodeRef node, QName contentProperty, String mimetype, TransformationOptions options);

    /**
     * This method returns a {@link Map} of {@link FailedThumbnailInfo failed thumbnails} for the
     * specified source node. The map is keyed by {@link ThumbnailDefinition#getName() thumbnail
     * definition name} and the values are the {@link FailedThumbnailInfo failed thumbnails}.
     *
     * @param sourceNode the node whose thumbnails are to be checked.
     * @return a Map of failed thumbnails, if any. If there are no such failures, an empty Map will
     *     be returned.
     * @since 3.5.0
     */
     ...
     *
     * @param thumbnailsEnabled <code>true</code> to enable all thumbnail creation (the default
     *     setting), or <code>false</code> to disable.
     * @since 4.1.7
     */
    void setThumbnailsEnabled(boolean thumbnailsEnabled);

    /**
     * This method indicates whether thumbnail creation via this service has been globally enabled
     * or disabled.
     *
     * @since 4.1.7
     */

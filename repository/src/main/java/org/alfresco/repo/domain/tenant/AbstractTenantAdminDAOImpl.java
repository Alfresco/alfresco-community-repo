...
public abstract class AbstractTenantAdminDAOImpl implements TenantAdminDAO {
   ...
   */
  public void setTenantEntityCache(
    SimpleCache<Serializable, Object> tenantEntityCache
  ) {
    this.tenantEntityCache =
      new EntityLookupCache<String, TenantEntity, Serializable>(
        tenantEntityCache,
  ...
  public TenantEntity createTenant(TenantEntity entity) {
    ...
    Pair<String, TenantEntity> entityPair = tenantEntityCache.getOrCreateByValue(
      entity
    );
  ...
  private TenantEntity getTenantImpl(String tenantDomain) {
    ...
    Pair<String, TenantEntity> entityPair = tenantEntityCache.getByKey(
      tenantDomain
    );
    if (entityPair == null) {
  ...
  public TenantUpdateEntity getTenantForUpdate(String tenantDomain) {
    ...
    // copy for update
    TenantUpdateEntity updateEntity = new TenantUpdateEntity(
      entity.getTenantDomain()
    );
  ...
  public void updateTenant(TenantUpdateEntity entity) {
    ...
    int updated = tenantEntityCache.updateValue(
      entity.getTenantDomain(),
      entity
    );
    if (updated < 1) {
      throw new ConcurrencyFailureException(
        "TenantEntity " +
        entity.getTenantDomain() +
        " no longer exists or has been updated concurrently"
      );
    }
  }
  ...
  public void deleteTenant(String tenantDomain) {
    ...
    if (deleted < 1) {
      throw new ConcurrencyFailureException(
        "TenantEntity " + tenantDomain + " no longer exists"
      );
    }
  }
  ...
  private class TenantEntityCallbackDAO
    implements EntityLookupCallbackDAO<String, TenantEntity, Serializable> {

    private final Pair<String, TenantEntity> convertEntityToPair(
      TenantEntity entity
    ) {
      if (entity == null) {
        return null;
    ...
    public Pair<String, TenantEntity> findByValue(TenantEntity value) {
      if ((value == null) || (value.getTenantDomain() == null)) {
        throw new AlfrescoRuntimeException(
          "Unexpected: TenantEntity / tenantDomain must not be null"
        );
      }

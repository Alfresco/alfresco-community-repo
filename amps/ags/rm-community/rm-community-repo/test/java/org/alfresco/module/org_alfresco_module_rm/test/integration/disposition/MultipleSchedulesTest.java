...
public class MultipleSchedulesTest extends BaseRMTestCase {
    ...
    protected static final String TEST_PREFIX =
            MultipleSchedulesTest.class.getName() + GUID.generate() + "_";
    ...
    /**
     * The internal disposition service is used to avoid permissions issues when updating the
     * record.
     */
    ...
    /**
     * Create two categories each containing a folder. Set up a schedule on category A that applies
     * to records (cutoff immediately, destroy immediately). Set up a schedule on category B that is
     * the same, but with a week delay before destroy becomes eligible.
     */
    private void setUpFilePlan() {
        ...
        // Create a disposition schedule for category A (Cut off immediately, then Destroy
        // immediately).
        DispositionSchedule dispSchedA =
                utils.createBasicDispositionSchedule(
                        categoryA, "instructions", "authority", true, false);
        Map<QName, Serializable> cutOffParamsA =
                ImmutableMap.of(
        ...
        Map<QName, Serializable> destroyParamsA =
                ImmutableMap.of(
                        PROP_DISPOSITION_ACTION_NAME,
                        DestroyAction.NAME,
                        PROP_DISPOSITION_DESCRIPTION,
        ...
        // Create a disposition schedule for category B (Cut off immediately, then Destroy one week
        // after cutoff).
        DispositionSchedule dispSchedB =
                utils.createBasicDispositionSchedule(
                        categoryB, "instructions", "authority", true, false);
        Map<QName, Serializable> cutOffParamsB =
                ImmutableMap.of(
                        PROP_DISPOSITION_ACTION_NAME,
                        CutOffAction.NAME,
                        PROP_DISPOSITION_DESCRIPTION,
        ...
        Map<QName, Serializable> destroyParamsB =
                ImmutableMap.of(
                        ...
                        "description",
                        PROP_DISPOSITION_PERIOD,
                        CommonRMTestUtils.PERIOD_ONE_WEEK,
                        PROP_DISPOSITION_PERIOD_PROPERTY,
                        PROP_CUT_OFF_DATE);
        dispositionService.addDispositionActionDefinition(dispSchedB, destroyParamsB);
        // Create a disposition schedule for category C (Cut off immediately, then Destroy one year
        // after cutoff).
        DispositionSchedule dispSchedC =
                utils.createBasicDispositionSchedule(
                        categoryC, "instructions", "authority", true, false);
        Map<QName, Serializable> cutOffParamsC =
                ImmutableMap.of(
                        PROP_DISPOSITION_ACTION_NAME,
                        CutOffAction.NAME,
                        PROP_DISPOSITION_DESCRIPTION,
        ...
        Map<QName, Serializable> destroyParamsC =
                ImmutableMap.of(
                        PROP_DISPOSITION_ACTION_NAME,
                        DestroyAction.NAME,
                        PROP_DISPOSITION_DESCRIPTION,
    ...
    public void testLinkedToLongerSchedule() {
        ...
        test().given(
                        () -> {
                            ...
                            record =
                                    fileFolderService
                                            .create(folderA, RECORD_NAME, ContentModel.TYPE_CONTENT)
                                            .getNodeRef();
                ...
                .when(
                        () -> {
                            ...
                            calendar.setTime(
                                    (Date) nodeService.getProperty(record, PROP_CUT_OFF_DATE));
                ...
                .because(
                        "Record should follow largest rentention schedule period, which is one"
                                + " week.");
    }
    ...
    public void testLinkedToShorterSchedule() {
        ...
        test().given(
                        () -> {
                            ...
                            record =
                                    fileFolderService
                                            .create(folderB, RECORD_NAME, ContentModel.TYPE_CONTENT)
                                            .getNodeRef();
    ...
    public void testRecalculateDispositionWhenUnlinking() {
        test().given(
                        () -> {
                            setUpFilePlan();
                            // Create a record filed under category A and linked to category B and
                            // C.
                            record =
                                    fileFolderService
                                            .create(folderA, RECORD_NAME, ContentModel.TYPE_CONTENT)
                ...
                .because(
                        "Destroy action shouldn't be available, as the record should follow"
                                + " disposition schedule from category C.");
    }
}

...
/**
 * This class registers behaviours for the {@link ForumModel#TYPE_POST fm:post} content type. These
 * behaviours maintain the correct value for the {@link ForumModel#PROP_COMMENT_COUNT comment count
 * rollup property}.
 *
...
public class ForumPostBehaviours
    ...
    public void onUpdateProperties(
            NodeRef commentsRollupNode,
            Map<QName, Serializable> before,
            Map<QName, Serializable> after) {
        // This method is only concerned with the value of fm:commentCount.
        // If it has been set to a trigger value, then we initiate a full recalculation of the
        // comment count.
            ...
            if (newCommentCountInt == COUNT_TRIGGER_VALUE) {
                if (log.isDebugEnabled()) {
                    ...
                    msg.append(commentsRollupNode)
                            .append(" had its ")
                            .append(ForumModel.PROP_COMMENT_COUNT.getLocalName())
                            .append(" property set to ")
                            .append(newCommentCountInt);
                    log.debug(msg.toString());
                    log.debug("Triggering a comment recount...");
                }

                final Integer realCommentTotal =
                        calculateCommentTotalByNodeCounting(commentsRollupNode);
                if (realCommentTotal != null && realCommentTotal != -1) {
                    nodeService.setProperty(
                            commentsRollupNode, ForumModel.PROP_COMMENT_COUNT, realCommentTotal);
                }
            }
        }
    }
    ...
    private Integer calculateCommentTotalByNodeCounting(NodeRef discussableNode) {
        // This method only counts "Share comments" and not Explorer comments or other fm:post
        // nodes.
        Integer result = null;
        ...
        if (nodeService.hasAspect(discussableNode, ForumModel.ASPECT_DISCUSSABLE)) {
            ...
            // location - as used by "Share comments" - as opposed to e.g. Explorer client
            // discussion fm:posts.
            if (topicNode != null) {
                ...
                // We'll use the raw, small 'n' nodeService as the big 'N' NodeService's
                // interceptors would limit results.
                List<ChildAssociationRef> fmPostChildren =
                        rawNodeService.getChildAssocs(topicNode, childNodeTypeQNames);
    ...
    /**
     * This method adjusts the {@link ForumModel#PROP_COMMENT_COUNT} based on the supplied
    ...
    private void adjustCommentCount(NodeRef fmPostNode, boolean incrementing) {
        ...
        // We need to find the fm:commentsCount ancestor to this comment node (if there is one) and
        // adjust its commentCount
            ...
            if (discussableNodeRequiresFullRecount(discussableAncestor)) {
                ...
                if (recount != null) {
                    nodeService.addAspect(
                            discussableAncestor, ForumModel.ASPECT_COMMENTS_ROLLUP, null);

                    int newCountValue = recount;
                    // If the node is being deleted then the above node-count will include the
                    // to-be-deleted node.
                    ...
                    nodeService.setProperty(
                            discussableAncestor, ForumModel.PROP_COMMENT_COUNT, newCountValue);
                }

            } else {
                Integer existingCommentCount =
                        (Integer)
                                nodeService.getProperty(
                                        discussableAncestor, ForumModel.PROP_COMMENT_COUNT);
                int existingCommentCountInt =
                        existingCommentCount == null ? 0 : existingCommentCount.intValue();
                ...
                nodeService.setProperty(
                        ...
                        existingCommentCountInt + delta);
            }
        }
    }

    /**
     * This method checks if a {@link ForumModel#ASPECT_DISCUSSABLE} node requires a full recount of
     * its comments. This will occur if any of the following are true:
     *
     * <ul>
     *   <li>the node has no {@link ForumModel#ASPECT_COMMENTS_ROLLUP} aspect
     *   <li>the {@link ForumModel#PROP_COMMENT_COUNT} is a negative number
     * </ul>
     */

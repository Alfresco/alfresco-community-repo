...
public class SubscriptionServiceImpl implements SubscriptionService {
  ...
  public void subscribe(String userId, NodeRef node) {
    ...
    if (userId.equalsIgnoreCase(AuthenticationUtil.getRunAsUser())) {
      ...
      activityService.postActivity(
        ActivityType.SUBSCRIPTIONS_SUBSCRIBE,
        null,
        ACTIVITY_TOOL,
        activityDataJSON
      );
  ...
  @Override
  public PagingFollowingResults getFollowing(
    String userId,
    PagingRequest pagingRequest
  ) {
    checkEnabled();
  ...
  @Override
  public PagingFollowingResults getFollowers(
    String userId,
    PagingRequest pagingRequest
  ) {
    checkEnabled();
  ...
  public void follow(String userId, String userToFollow) {
    ...
    if (userId.equalsIgnoreCase(AuthenticationUtil.getRunAsUser())) {
      try {
        ...
        activityService.postActivity(
          ActivityType.SUBSCRIPTIONS_FOLLOW,
          null,
          ACTIVITY_TOOL,
          activityDataJSON
        );
  ...
  public boolean isSubscriptionListPrivate(String userId) {
    ...
    Serializable privateList = nodeService.getProperty(
      getUserNodeRef(userId),
      ContentModel.PROP_SUBSCRIPTIONS_PRIVATE
    );
    if (privateList == null) {
    ...
    if (
      privateList instanceof Boolean && !((Boolean) privateList).booleanValue()
    ) {
      return false;
    }
  ...
  protected void checkEnabled() {
    if (!isActive()) {
      throw new SubscriptionsDisabledException(
        "subscription_service.err.disabled"
      );
    }
  }
  ...
  protected void checkRead(String userId, boolean checkPrivate) {
    ...
    if (
      currentUser.equalsIgnoreCase(userId) ||
    ...
    throw new PrivateSubscriptionListException(
      "subscription_service.err.private-list"
    );
  }
  ...
  protected void sendFollowingMail(String userId, String userToFollow) {
    ...
    Serializable emailFeedDisabled = nodeService.getProperty(
      userNode,
      ContentModel.PROP_EMAIL_FEED_DISABLED
    );
    if (
      emailFeedDisabled instanceof Boolean &&
      ((Boolean) emailFeedDisabled).booleanValue()
    ) {
      // this user doesn't want to be notified
      return;
    }

    Serializable emailAddress = nodeService.getProperty(
      userNode,
    ...
    mail.setParameterValue(
      ...
      "subscription.notification.email.subject"
    );
    mail.setParameterValue(
      MailActionExecuter.PARAM_SUBJECT_PARAMS,
      new Object[] { followerFullName }
    );

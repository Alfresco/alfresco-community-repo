...
public class ContentSearchContext
  ...
  public boolean nextFileInfo(FileInfo info) {
    ...
    try {
      ...
      while (nextInfo == null && index < results.size()) {
        ...
        try {
          ...
          if (logger.isWarnEnabled()) {
            logger.warn(
              "Noderef " + nextNodeRef + " no longer valid, ignoring"
            );
          }
      ...
      if (nextInfo.isLinkNode()) {
        ...
        if (info.isReadOnly() == false) info.setFileAttributes(
          info.getFileAttributes() + FileAttribute.ReadOnly
        );
  ...
  public String nextFileName() {
    ...
    try {
      ...
      FileInfo nextInfo = cifsHelper.getFileInformation(
        nextNodeRef,
        "",
        false,
        false
  ...
  public boolean restartAt(FileInfo info) {
    ...
    if (
      m_lastFileName != null &&
      info.getFileName().equalsIgnoreCase(m_lastFileName)
    ) {
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "Fast search restart - " + m_lastFileName
      );
      return true;
    }

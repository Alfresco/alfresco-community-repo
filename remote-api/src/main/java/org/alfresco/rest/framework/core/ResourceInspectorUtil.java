...
public class ResourceInspectorUtil {
    ...
    /**
     * Determine the expected type as the returned type of the method. If the return type is a List
     * it will return the generic element type instead of a List.
     *
    ...
    protected static Class determineOperationType(Class resource, Method method) {
        ...
        if (paramTypes != null) {
            switch (paramTypes.length) {
                case 4:
                    // EntityResource operation by id, same logic as RelationshipEntityResource
                    // operation by id
                case 5:
        ...
        throw new IllegalArgumentException(
                "Your method signature should have 4 parameters (uniqueId, typePassedin,"
                    + " Parameters, WithResponse), use Void if you are not interested in the second"
                    + " argument. "
                        + resource.getName()
                        + " "
                        + method.getName());
     ...
     *
     * <p>It first finds all public member methods of the class or interface represented by
     * objClass, including those inherited from superclasses and superinterfaces.
     *
     * <p>It then loops through these methods searching for a single Annotation of annotationType,
     ...
     */
    @SuppressWarnings("rawtypes")
    public static List<Method> findMethodsByAnnotation(
            Class objClass, Class<? extends Annotation> annotationType) {

...
public class ACPImportPackageHandler implements ImportPackageHandler {
  ...
  public void startImport() {
    ...
    try {
      ...
      throw new ImporterException(
        "Failed to read zip file due to " + e.getMessage(),
        e
      );
    }
  }
  ...
  public Reader getDataStream() {
    try {
      ...
      if (xmlMetaDataEntry == null) {
        throw new ImporterException(
          "Failed to find xml meta-data file within .acp package"
        );
      }

      // open the meta-data xml file
      InputStream dataStream = zipFile.getInputStream(xmlMetaDataEntry);
      Reader inputReader = (dataFileEncoding == null)
        ? new InputStreamReader(dataStream, DEFAULT_ENCODING)
        : new InputStreamReader(dataStream, dataFileEncoding);
      return new BufferedReader(inputReader);
    } catch (UnsupportedEncodingException e) {
      throw new ImporterException(
        "Encoding " + dataFileEncoding + " is not supported"
      );
    } catch (IOException e) {
      throw new ImporterException(
        "Failed to open xml meta-data file within .acp package due to " +
        e.getMessage()
      );
    }
  }
  ...
  public InputStream importStream(String content) {
    ...
    if (zipEntry == null) {
      ...
      if (zipEntry == null) {
        throw new ImporterException(
          "Failed to find content " + content + " within zip package"
        );
      }
    }
  ...
  public void endImport() {
    if (zipFile != null) {
      try {
        ...
        throw new ImporterException(
          "Failed to close zip package " + file.getAbsolutePath(),
          e
        );
      }
    }

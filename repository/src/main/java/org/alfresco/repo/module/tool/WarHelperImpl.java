...
public class WarHelperImpl implements WarHelper {
  ...
  public static final String VERSION_PROPERTIES =
    "/WEB-INF/classes/alfresco/version.properties";
  ...
  public void checkCompatibleVersion(
    ...
    if (propsFile != null && propsFile.exists()) {
      ...
      VersionNumber warVersion = new VersionNumber(
  ...
  protected void checkCompatibleVersionUsingManifest(
    ...
    if (version != null && version.length() > 0) {
      if (version.matches(REGEX_NUMBER_OR_DOT)) {
        ...
        String edition = findManifestArtibute(
          war,
        ...
        if (edition.endsWith(MANIFEST_COMMUNITY)) {
          ...
          log.info(
            "WARNING: Community edition war detected, the version number is non-numeric so we will not validate it."
          );
        } else {
      ...
      log.info(
        "WARNING: No version information detected in war, therefore version validation is disabled, continuing anyway.  Is this war prior to 3.4.11, 4.1.1 and Community 4.2 ?"
      );
    }
  }
  ...
  private void checkVersions(
    ...
    if (
      warVersion.compareTo(installingModuleDetails.getRepoVersionMin()) == -1
    ) {
      throw new ModuleManagementToolException(
        "The module (" +
    ...
    if (
      warVersion.compareTo(installingModuleDetails.getRepoVersionMax()) == 1
    ) {
      throw new ModuleManagementToolException(
        "The module (" +
        installingModuleDetails.getTitle() +
  ...
  public void checkCompatibleEditionUsingManifest(
    ...
    if (installableEditions != null && installableEditions.size() > 0) {
      String warEdition = findManifestArtibute(
        war,
        MANIFEST_IMPLEMENTATION_TITLE
      );
      if (warEdition != null && warEdition.length() > 0) {
        ...
        log.info(
          "WARNING: No edition information detected in war, edition validation is disabled, continuing anyway. Is this war prior to 3.4.11, 4.1.1 and Community 4.2 ?"
        );
      }
    }
  }
  ...
  public void checkModuleDependencies(
    ...
    List<ModuleDependency> missingDependencies = new ArrayList<ModuleDependency>(
      0
    );
    for (ModuleDependency dependency : installingModuleDependencies) {
      String dependencyId = dependency.getDependencyId();
      ModuleDetails dependencyModuleDetails = getModuleDetails(
        war,
        dependencyId
      );
  ...
  public ModuleDetails getModuleDetailsOrAlias(
    ...
    ModuleDetails installedModuleDetails = getModuleDetails(
      war,
      installingModuleDetails.getId()
    );
    if (installedModuleDetails == null) {
      ...
      for (String installingAlias : installingAliases) {
        ModuleDetails installedAliasModuleDetails = getModuleDetails(
          war,
          installingAlias
        );
    ...
    if (dirs != null && dirs.length != 0) {
      for (java.io.File dir : dirs) {
        if (dir.isDirectory() == true) {
          TFile moduleProperties = new TFile(
            dir.getPath() + WarHelper.MODULE_CONFIG_IN_WAR
          );
          if (moduleProperties.exists() == true) {
            ...
            try {
              ...
              moduleDetails.add(
                ModuleDetailsHelper.createModuleDetailsFromPropertiesStream(is)
              );
  ...
  public String backup(TFile file) throws IOException {
    String backupLocation =
      file.getAbsolutePath() + "-" + System.currentTimeMillis() + ".bak";
  ...
  private Properties loadProperties(TFile propertiesFile) {
    ...
    try {
      ...
      throw new ModuleManagementToolException(
        "Unable to load properties from the war file; " +

...
public final class Export extends Tool {
    ...
    /*package*/ ToolContext processArgs(String[] args) {
        ...
        while (i < args.length) {
            if (args[i].equals("-h") || args[i].equals("-help")) {
                ...
                if (i == args.length || args[i].length() == 0) {
                    throw new ToolArgumentException(
                            "The value <packagedir> for the parameter -packagedir must be"
                                    + " specified");
                }
    ...
    /*package*/ void displayHelp() {
        ...
        logError(
                " -packagedir the directory to place extracted content (default:"
                        + " dir/<packagename>)");
    ...
    private class ExportContext extends ToolContext {
        ...
        private File getPackageFile() {
            String packageFile =
                    (packageName.indexOf('.') != -1) ? packageName : packageName + ".xml";

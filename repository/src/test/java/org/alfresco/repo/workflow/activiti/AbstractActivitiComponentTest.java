...
public class AbstractActivitiComponentTest {
    ...
    @Resource protected MessageService messageService;
    ...
    @Resource protected PermissionService permissionService;

    @Resource(name = "PersonService")
    protected PersonService personService;

    @Resource protected AuthorityDAO authorityDAO;

    @Resource protected ServiceRegistry serviceRegistry;
    ...
    private void mockSearchService() {
        ...
        when(unprotectedSearchService.selectNodes(
                        rootNode, "spaces.company_home.childname", null, namespaceService, false))
    ...
    private void mockDictionaryService() {
                ...
                .thenAnswer(
                        new Answer<TypeDefinition>() {
                            ...
                            public TypeDefinition answer(InvocationOnMock invocation)
                                ...
                                PropertyDefinition outcomePropertyName =
        ...
        when(dictionaryService.isSubClass(ContentModel.TYPE_PERSON, ContentModel.TYPE_PERSON))
                .thenReturn(true);
    }

    private void mockNamespaceService() {
        namespaceService.registerNamespace(
                NamespaceService.BPM_MODEL_PREFIX, NamespaceService.BPM_MODEL_1_0_URI);
        namespaceService.registerNamespace(
                NamespaceService.DEFAULT_PREFIX, NamespaceService.DEFAULT_URI);
        namespaceService.registerNamespace(
                NamespaceService.WORKFLOW_MODEL_PREFIX, NamespaceService.WORKFLOW_MODEL_1_0_URI);
    ...
    public void tearDown() {
        List<ProcessDefinition> defs =
                repo.createProcessDefinitionQuery().processDefinitionKey(TEST_TASK_KEY).list();
        ...
        for (String deployment : deployments) {
            List<ProcessDefinition> definitions =
                    repo.createProcessDefinitionQuery().deploymentId(deployment).list();
    ...
    private void killInstances(ProcessDefinition def) {
        List<ProcessInstance> instances =
                runtime.createProcessInstanceQuery().processDefinitionId(def.getId()).list();

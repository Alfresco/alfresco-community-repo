...
/**
 * An {@link ExecutionListener} that converts a variable of type date to a ISO-8601 date. The
 * variable that should be converted can be set using field "source", the output string will be
 * written in field configured in "target".
 *
        ...
        if (dateVar != null) {
            if (dateVar instanceof Date) {
                ...
                throw new IllegalArgumentException(
                        "Variable with name: " + sourceVarName + " must be a Date or a Calendar");
            }
        }

...
public class CMMDownloadTestUtil {
  ...
  private static final Log logger = LogFactory.getLog(
    CMMDownloadTestUtil.class
  );

  private static final String SURF_CONFIG_PATH =
  ...
  public CMMDownloadTestUtil(ApplicationContext ctx) {
    ...
    this.namespaceService =
      ctx.getBean("namespaceService", NamespaceService.class);
    this.downloadService =
      ctx.getBean("downloadService", DownloadService.class);
  }
  ...
  public synchronized void createShareExtModule(final String moduleId) {
    ...
    if (this.extensionsNodeRef == null) {
      ...
      logger.info(
        "Created 'cm:extensions' folder within the 'app:company_home/st:sites/cm:surf-config'"
      );
    }
    ...
    if (this.sharePersistedExtNodeRef == null) {
      ...
      logger.info(
        "Created 'cm:default-persisted-extension.xml' file within the 'app:company_home/st:sites/cm:surf-config/cm:extensions'"
      );
          ...
          public Void execute() throws Throwable {
            ...
            originalShareExtFile =
              TempFileProvider.createTempFile(
                CustomModelServiceImplTest.class.getName(),
                ".xml"
              );
  ...
  public synchronized void cleanup() {
    if (isExtFolderCreated) {
      ...
      logger.info(
        "Deleted 'cm:extensions' folder within the 'app:company_home/st:sites/cm:surf-config"
      );
    } else if (originalShareExtFile != null) {
      transactionHelper.doInTransaction(
        new RetryingTransactionCallback<Void>() {
  ...
  public String getDownloadEntry(

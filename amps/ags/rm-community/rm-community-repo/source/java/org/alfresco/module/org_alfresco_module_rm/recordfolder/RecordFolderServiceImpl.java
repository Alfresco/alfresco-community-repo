...
public class RecordFolderServiceImpl
  ...
  /** I18N */
  private static final String MSG_RECORD_FOLDER_EXPECTED =
    "rm.service.record-folder-expected";
  ...
  private static final String MSG_RECORD_FOLDER_TYPE =
    "rm.service.record-folder-type";
  ...
  public void setupRecordFolder(NodeRef nodeRef) {
    ...
    if (!nodeService.hasAspect(nodeRef, ASPECT_DISPOSITION_LIFECYCLE)) {
      DispositionSchedule di = dispositionService.getDispositionSchedule(
        nodeRef
      );
  ...
  public NodeRef createRecordFolder(
    ...
    if (filePlanService.isFilePlan(rmContainer)) {
      throw new AlfrescoRuntimeException(
        I18NUtil.getMessage(MSG_PARENT_RECORD_FOLDER_ROOT)
      );
    }
    ...
    if (
      !TYPE_RECORD_CATEGORY.equals(parentType) &&
      !dictionaryService.isSubClass(parentType, TYPE_RECORD_CATEGORY)
    ) {
      throw new AlfrescoRuntimeException(
        I18NUtil.getMessage(
          MSG_PARENT_RECORD_FOLDER_TYPE,
          parentType.toString()
        )
      );
    }
    ...
    if (
      !TYPE_RECORD_FOLDER.equals(type) &&
  ...
  public void closeRecordFolder(NodeRef nodeRef) {
    ...
    if (isRecordFolder(nodeRef)) {
      ...
      if (logger.isWarnEnabled()) {
        logger.warn(
          I18NUtil.getMessage(
            MSG_CLOSE_RECORD_FOLDER_NOT_FOLDER,
            nodeRef.toString()
          )
        );
      }

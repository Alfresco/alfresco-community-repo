...
public class EmailServiceImplTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        ChildApplicationContextFactory emailSubsystem =
                (ChildApplicationContextFactory) ctx.getBean("InboundSMTP");
        ...
        folderEmailMessageHandler =
                (FolderEmailMessageHandler) emailCtx.getBean("folderEmailMessageHandler");
        assertNotNull("folderEmailMessageHandler", folderEmailMessageHandler);
        transactionHelper =
                (RetryingTransactionHelper) emailCtx.getBean("retryingTransactionHelper");
     ...
     * Test the from name.
     *
     * <p>Step 1: User admin will map to the "unknownUser" which out of the box is "anonymous"
     * Sending email From "admin" will fail.
     *
     * <p>Step 2: Send from the test user to the test' user's home folder.
     */
    public void testFromName() throws Exception {
        ...
        Set<String> auths =
                authorityService.getContainedAuthorities(null, "GROUP_EMAIL_CONTRIBUTORS", true);
        ...
        String companyHomeDBID =
                ((Long) nodeService.getProperty(companyHomeNodeRef, ContentModel.PROP_NODE_DBID))
                                .toString()
                        + "@Alfresco.com";
        String testUserDBID =
                ((Long) nodeService.getProperty(person, ContentModel.PROP_NODE_DBID)).toString()
                        + "@Alfresco.com";
        NodeRef testUserHomeFolder =
        ...
        String testUserHomeDBID =
                ((Long) nodeService.getProperty(testUserHomeFolder, ContentModel.PROP_NODE_DBID))
                                .toString()
                        + "@Alfresco.com";

        /**
         * Step 1 Negative test - send from "Bert" who does not exist. User will be mapped to
         * anonymous who is not an email contributor.
         */
        try {
            String from = "admin";
            String to = "bertie";
         ...
         * Step 2
         *
         * <p>Send From the test user TEST_EMAIL to the test user's home
         */
        {
            logger.debug("Step 2");
         ...
         * Step 3
         *
         * <p>message.from From with "name" < name@ domain > format SMTP.FROM="dummy"
         *
         * <p>Send From the test user <TEST_EMAIL> to the test user's home
         */
        {
         ...
         * Step 4
         *
         * <p>From with "name" < name@ domain > format
         *
         * <p>Send From the test user <TEST_EMAIL> to the test user's home
         */
        {
     ...
     * ALF-9544 ALF-751
     *
     * <p>Inbound email to a folder restricts file name to 86 characters or less.
     *
     * <p>Also has tests for other variations of subject
     */
    public void testFolderSubject() throws Exception {
...

        {
            ...
            msg.setRecipients(Message.RecipientType.TO, toa);
            msg.setSubject(
                    "This is a very very long name in particular it is greater than eitghty six"
                            + " characters which was a problem explored in ALF-9544");
            msg.setContent(content, "text/plain");
     ...
     * ALF-1878
     *
     * <p>Duplicate incoming email Subjects over-write each other
     */
    public void testMultipleMessagesToFolder() throws Exception {
        ...
        assertEquals(
                "name of link not as expected",
                assocs.get(0).getQName(),
        ...
        assertTrue(
                EXT_NAME + "not found",
                assocNames.contains(
                        QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "Blob.xls")));
        assertTrue(
                "Blob(1).xls not found",
                assocNames.contains(
                        QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "Blob(1).xls")));
        assertTrue(
                "Blob(2).xls not found",
                assocNames.contains(
                        QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "Blob(2).xls")));
        assertTrue(
                TEST_SUBJECT + "not found",
                assocNames.contains(
                        QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, TEST_SUBJECT)));
        assertTrue(
                TEST_SUBJECT + "(1) not found",
                assocNames.contains(
                        QName.createQName(
                                NamespaceService.CONTENT_MODEL_1_0_URI,
                                "Practical Bee Keeping(1)")));

        /** Check concurrent deliver of the same message. Reuse message from the previous test. */
        logger.debug(
                "Step 5: turn off Overwite Duplicates and check concurrent deliver of the same"
                        + " message");
        folderEmailMessageHandler.setOverwriteDuplicates(false);
        assocs =
                nodeService.getChildAssocs(
        ...
        assertEquals(
                "Two messages must be added",
                numBeforeConcurrentDeliver + 2,
                numAfterConcurrentDeliver);
    }

    private void deliverConcurrently(final EmailDelivery delivery, final SubethaEmailMessage m)
            throws Exception {
     ...
     * MNT-9289
     *
     * <p>Change in case in email Subject causes DuplicateChildNodeNameException
     */
    public void testCaseSensitivity() throws Exception {
        ...
        QName safeQName =
                QName.createQNameWithValidLocalName(
                        NamespaceService.CONTENT_MODEL_1_0_URI, TEST_CASE_SENSITIVITY_SUBJECT);
        List<ChildAssociationRef> assocs =
                nodeService.getChildAssocs(
                        testUserHomeFolder, ContentModel.ASSOC_CONTAINS, safeQName);
     ...
     * ALF-12297
     *
     * <p>Test messages being sent to a cm:content node
     */
    public void testMessagesToDocument() throws Exception {
        ...
        String companyHomeDBID =
                ((Long) nodeService.getProperty(companyHomeNodeRef, ContentModel.PROP_NODE_DBID))
                                .toString()
                        + "@Alfresco.com";
        //     String testUserDBID = ((Long)nodeService.getProperty(person,
        // ContentModel.PROP_NODE_DBID)).toString() + "@Alfresco.com";
        NodeRef testUserHomeFolder =
                (NodeRef) nodeService.getProperty(person, ContentModel.PROP_HOMEFOLDER);
        assertNotNull("testUserHomeFolder is null", testUserHomeFolder);
        //      String testUserHomeDBID = ((Long)nodeService.getProperty(testUserHomeFolder,
        // ContentModel.PROP_NODE_DBID)).toString() + "@Alfresco.com";
        ...
        String testDocDBID =
                ((Long) nodeService.getProperty(testDocNodeRef, ContentModel.PROP_NODE_DBID))
                        .toString();
     ...
     *
     * <p>This test switches between the EMAIL_CONTRIBUTORS group and EVERYONE
     */
    public void testEmailContributorsAuthority() throws Exception {
         ...
         *
         * <p>Test that TEST_USER is allowed to send email - so even though TEST_USER is not a
         * contributor
         */

...
public class ImapServiceImpl implements ImapService
{
    ...
    public void setIgnoreExtractionFolders(final ImapConfigBean[] ignoreExtractionFolders)
    {
        this.ignoreExtractionFolders = AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<Set<NodeRef>>()
        {
            ...
            {
                Set<NodeRef> result = new HashSet<NodeRef>(ignoreExtractionFolders.length * 2);

                for (ImapConfigBean bean : ignoreExtractionFolders)
                {
    ...
    public AlfrescoImapFolder createMailbox(AlfrescoImapUser user, String mailboxName)
    {
        ...
        for (String folderName : getMailPathInRepo(mailboxName).split(String.valueOf(AlfrescoImapConst.HIERARCHY_DELIMITER)))
        {
            ...
            if (folders.size() == 0)
            {
                ...
                return new AlfrescoImapFolder(user.getQualifiedMailboxName(),
                                              mailFolder,
                                              folderName,
    ...
    public void renameMailbox(AlfrescoImapUser user, String oldMailboxName, String newMailboxName)
    {
        ...
        try
        {
            for (int i=0; i < folderNames.length; i++)
            {
                ...
                else // not last element than checks if it exists and creates if doesn't
                {
    ...
    private List<AlfrescoImapFolder> listMailboxes(AlfrescoImapUser user, String mailboxPattern, boolean listSubscribed) 
    {
        ...
        for (String mountPointName : mountPoints.keySet())
        {
            ...
            if (mailboxPattern.equals("*"))
            {
                ...
                {
                    result.add(new AlfrescoImapFolder(user.getQualifiedMailboxName(), mountPointFileInfo, mountPointName, viewMode, mountParent, mountPointName, isExtractionEnabled(mountPointFileInfo.getNodeRef()), serviceRegistry));
                }
                // \NoSelect
                else if (listSubscribed && hasSubscribedChild(mountPointFileInfo, user.getLogin(), viewMode))
                {
                    result.add(new AlfrescoImapFolder(user.getQualifiedMailboxName(), mountPointFileInfo, mountPointName, viewMode, mountParent, mountPointName, serviceRegistry, false, isExtractionEnabled(mountPointFileInfo.getNodeRef())));
                }
            }
                ...
                if (listSubscribed)
                {
                    for (FileInfo fileInfo : list)
                    {
                        if (isSubscribed(fileInfo, user.getLogin()))
                        {
                            ...
                            subscribedList.add(new AlfrescoImapFolder(user.getQualifiedMailboxName(), fileInfo, null, null, mailboxRoot, null, isExtractionEnabled(fileInfo.getNodeRef()), serviceRegistry));
                        }
                        // \NoSelect
                        else if (hasSubscribedChild(fileInfo, user.getLogin(), viewMode))
                        {
                            // folderName, viewMode, mountPointName will be setted in listMailboxes() method
                            subscribedList.add(new AlfrescoImapFolder(user.getQualifiedMailboxName(), fileInfo, null, null, mailboxRoot, null, serviceRegistry, false, isExtractionEnabled(fileInfo.getNodeRef())));
                        }
                    }
                }
                else
     ...
     * @return
     */
    private NodeRef getMailboxRootRef(String mailboxName, String userName)
    {
        String rootFolder;
        int index = mailboxName.indexOf(AlfrescoImapConst.HIERARCHY_DELIMITER);
        if (index > 0)
    ...
    private List<AlfrescoImapFolder> createMailFolderList(AlfrescoImapUser user, Collection<FileInfo> list, NodeRef imapUserHomeRef)
    {
        ...
        for (FileInfo folderInfo : list)
        {
            ...
            result.add(new AlfrescoImapFolder(user.getQualifiedMailboxName(), folderInfo, null, null, imapUserHomeRef, null, isExtractionEnabled(folderInfo.getNodeRef()), serviceRegistry));
        }
    ...
    private List<SiteInfo> getNonFavouriteSites(String userName)
    {
        ...
        PreferenceService preferenceService = (PreferenceService) serviceRegistry.getService(QName.createQName(NamespaceService.ALFRESCO_URI, "PreferenceService"));

...
public class ContentDataTest extends TestCase {
  ...
  public void testToAndFromString() throws Exception {
    ...
    String localeStr = DefaultTypeConverter.INSTANCE.convert(
      String.class,
      locale
    );
    ...
    property =
      new ContentData("test://uuu", "mmm", 123L, "eee", I18NUtil.getLocale());

    // convert to a string
    propertyStr = property.toString();
    assertEquals(
      "Incorrect property string representation",
      "contentUrl=test://uuu|mimetype=mmm|size=123|encoding=eee|locale=" +
      localeStr,
      propertyStr
    );

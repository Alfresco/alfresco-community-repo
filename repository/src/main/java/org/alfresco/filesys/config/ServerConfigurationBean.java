...
public class ServerConfigurationBean
  ...
  public void setCoreServerConfigBean(
    CoreServerConfigBean coreServerConfigBean
  ) {
  ...
  protected void processFTPServerConfig() {
    ...
    try {
      ...
      if (
        bindText != null &&
        bindText.length() > 0 &&
        !bindText.equals(BIND_TO_IGNORE)
      ) {
        ...
        try {
          ...
          throw new AlfrescoRuntimeException(
            "Unable to find FTP bindto address, " + bindText,
            ex
          );
        }
      }
      ...
      if (port != null) {
        ...
        if (
          ftpConfig.getFTPPort() <= 0 || ftpConfig.getFTPPort() >= 65535
        ) throw new AlfrescoRuntimeException(
      ...
      if (sessionTimeout != null) {
        ...
        if (
          ftpConfig.getFTPSrvSessionTimeout() < 0
        ) throw new AlfrescoRuntimeException(
      ...
      if (ftpConfigBean.getAllowAnonymous()) {
        ...
        if (anonAcc != null && anonAcc.length() > 0) {
          ...
          if (
            ftpConfig.getAnonymousFTPAccount() == null ||
      ...
      if (flags != null) {
        ...
        while (token.hasMoreTokens()) {
          ...
          while (
            idx < m_ftpDebugStr.length &&
            m_ftpDebugStr[idx].equalsIgnoreCase(dbg) == false
          ) idx++;

          if (idx >= m_ftpDebugStr.length) throw new AlfrescoRuntimeException(
      ...
      if (charSet != null && charSet.length() > 0) {
        try {
          ...
          throw new AlfrescoRuntimeException(
            "FTP Illegal character set name, " + charSet
          );
        } catch (UnsupportedCharsetException ex) {
          throw new AlfrescoRuntimeException(
            "FTP Unsupported character set name, " + charSet
          );
        }
      }
      ...
      if (auth != null) {
        ...
        "FTP authenticator not specified"
      );

      // Check if a data port range has been specified

      if (
        ftpConfigBean.getDataPortFrom() != 0 &&
        ftpConfigBean.getDataPortTo() != 0
      ) {
        ...
        if (rangeFrom != 0 && rangeTo != 0) {
          ...
          if (
            rangeFrom < 1024 || rangeFrom > 65535
          ) throw new InvalidConfigurationException(
            "Invalid FTP data port range from value, " + rangeFrom
          );

          if (
            rangeTo < 1024 || rangeTo > 65535
          ) throw new InvalidConfigurationException(
            "Invalid FTP data port range to value, " + rangeTo
          );

          if (rangeFrom >= rangeTo) throw new InvalidConfigurationException(
          ...
          logger.info(
            "FTP server data ports restricted to range " +
            rangeFrom +
            ":" +
            rangeTo
          );
      ...
      if (
        ftpConfigBean.getKeyStorePath() != null &&
        ftpConfigBean.getKeyStorePath().length() > 0
      ) {
        ...
        if (
          keyStoreFile.exists() == false
        ) throw new InvalidConfigurationException(
          "FTPS key store file does not exist, " + keyStorePath
        ); else if (
          keyStoreFile.isDirectory()
        ) throw new InvalidConfigurationException(
          "FTPS key store path is a directory, " + keyStorePath
        );
        ...
        if (
          ftpConfigBean.getKeyStoreType() != null &&
          ftpConfigBean.getKeyStoreType().length() > 0
        ) {
          ...
          if (
            keyStoreType == null || keyStoreType.length() == 0
          ) throw new InvalidConfigurationException(
            "FTPS key store type is invalid"
          );
          ...
          try {
            ...
            throw new InvalidConfigurationException(
              "FTPS key store type is invalid, " + keyStoreType,
              ex
            );
          }
        ...
        if (
          ftpConfigBean.getKeyStorePassphrase() != null &&
          ftpConfigBean.getKeyStorePassphrase().length() > 0
        ) {
          // Set the key store passphrase

          ftpConfig.setKeyStorePassphrase(
            ftpConfigBean.getKeyStorePassphrase()
          );
        }
      }
      ...
      if (
        ftpConfigBean.getTrustStorePath() != null &&
        ftpConfigBean.getTrustStorePath().length() > 0
      ) {
        ...
        if (
          trustStoreFile.exists() == false
        ) throw new InvalidConfigurationException(
          "FTPS trust store file does not exist, " + trustStorePath
        ); else if (
          trustStoreFile.isDirectory()
        ) throw new InvalidConfigurationException(
        ...
        if (
          ftpConfigBean.getTrustStoreType() != null &&
          ftpConfigBean.getTrustStoreType().length() > 0
        ) {
          ...
          if (
            trustStoreType == null || trustStoreType.length() == 0
          ) throw new InvalidConfigurationException(
            "FTPS trust store type is invalid"
          );
          ...
          try {
            ...
            throw new InvalidConfigurationException(
              "FTPS trust store type is invalid, " + trustStoreType,
              ex
            );
          }
        ...
        if (
          ftpConfigBean.getTrustStorePassphrase() != null &&
          ftpConfigBean.getTrustStorePassphrase().length() > 0
        ) {
          // Set the trust store passphrase

          ftpConfig.setTrustStorePassphrase(
            ftpConfigBean.getTrustStorePassphrase()
          );
        }
      }
      ...
      if (ftpConfig.getKeyStorePath() != null) {
        ...
        if (
          ftpConfig.getKeyStorePath() == null
        ) throw new InvalidConfigurationException(
          "FTPS configuration requires keyStore to be set"
        );
      }
      ...
      for (DeviceContext filesystem : this.filesystemContexts) {
        ...
        try {
          ...
          if (filesysContext.hasStateCache()) {
            ...
            fsysConfig.addFileStateCache(
              filesystem.getDeviceName(),
              filesysContext.getStateCache()
            );

            // Create the lock manager for the context.
            FileStateLockManager lockMgr = new FileStateLockManager(
              filesysContext.getStateCache()
            );
          ...
          if (
            !ftpConfigBean.getServerEnabled() &&
          ...
          if (
            filesysContext.getDisableChangeNotifications() == false
          ...
          throw new AlfrescoRuntimeException(
            "Error creating filesystem " + filesystem.getDeviceName(),
            ex
          );
        } catch (InvalidConfigurationException ex) {
  ...
  protected void processCoreServerConfig()
    ...
    if (
      initSize < ThreadRequestPool.MinimumWorkerThreads
    ) throw new InvalidConfigurationException(
      "Thread pool size below minimum allowed size"
    );

    if (
      initSize > ThreadRequestPool.MaximumWorkerThreads
    ) throw new InvalidConfigurationException(
    ...
    if (
      maxSize < ThreadRequestPool.MinimumWorkerThreads
    ) throw new InvalidConfigurationException(
      "Thread pool maximum size below minimum allowed size"
    );

    if (
      maxSize > ThreadRequestPool.MaximumWorkerThreads
    ) throw new InvalidConfigurationException(
      "Thread pool maximum size above maximum allowed size"
    );

    if (maxSize < initSize) throw new InvalidConfigurationException(
    ...
    if (coreServerConfigBean.getThreadPoolDebug()) coreConfig
      .getThreadPool()
      .setDebug(true);
    ...
    if (packetSizes != null) {
      ...
      for (MemoryPacketConfigBean curChild : packetSizes) {
        ...
        if (pktSizeLong == null) throw new InvalidConfigurationException(
        ...
        if (
          elemIdx > 0 && pktSizes[elemIdx - 1] >= pktSize
        ) throw new InvalidConfigurationException(
        ...
        if (initAlloc == null) throw new InvalidConfigurationException(
          "Memory pool initial allocation not specified"
        );

        // Range check the initial allocation

        if (
          initAlloc < MemoryPoolMinimumAllocation
        ) throw new InvalidConfigurationException(
          "Initial memory pool allocation below minimum of " +
          MemoryPoolMinimumAllocation
        );

        if (
          initAlloc > MemoryPoolMaximumAllocation
        ) throw new InvalidConfigurationException(
          "Initial memory pool allocation above maximum of " +
        ...
        if (maxAlloc == null) throw new InvalidConfigurationException(
          "Memory pool maximum allocation not specified"
        );

        // Range check the maximum allocation

        if (
          maxAlloc < MemoryPoolMinimumAllocation
        ) throw new InvalidConfigurationException(
          "Maximum memory pool allocation below minimum of " +
          MemoryPoolMinimumAllocation
        );
        ...
        if (
          ...
          "Maximum memory pool allocation above maximum of " +
          MemoryPoolMaximumAllocation
        );
   ...
   */
  public void initialiseRuntimeContext(
    String uniqueName,
    ...
    if (diskCtx.getStateCache() == null) {
      ...
      FilesystemsConfigSection filesysConfig = (FilesystemsConfigSection) this.getConfigSection(
      ...
      if (filesysConfig != null) {
        try {
          ...
          StandaloneFileStateCache standaloneCache = new StandaloneFileStateCache();
          standaloneCache.initializeCache(new GenericConfigElement(""), this);
          filesysConfig.addFileStateCache(
            diskCtx.getDeviceName(),
            standaloneCache
          );
          ...
          // has many side effects including initialisation of the cache
          filesysConfig.addFileStateCache(
            diskCtx.getShareName(),
            diskCtx.getStateCache()
          );
          ...
          throw new AlfrescoRuntimeException(
            "Failed to initialize standalone state cache for " +
            diskCtx.getDeviceName()
          );
        }
      }

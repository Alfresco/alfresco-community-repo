...
public class ProcessDefinitionWorkflowApiTest
  ...
  public void testGetProcessDefinitions() throws Exception {
    ...
    ListResponse<ProcessDefinition> processDefinitionsResponse = processDefinitionsClient.getProcessDefinitions(
      null
    );
    Map<String, ProcessDefinition> processDefinitionMap = getProcessDefinitionMapByKey(
      processDefinitionsResponse.getList()
    );
    ...
    String adhocKey = createProcessDefinitionKey(
      "activitiAdhoc",
      requestContext
    );
    org.activiti.engine.repository.ProcessDefinition activitiDefinition = activitiProcessEngine
    ...
    ProcessDefinition adhocDefinitionRest = processDefinitionMap.get(
      "activitiAdhoc"
    );
    ...
    assertEquals(
      activitiDefinition.getDeploymentId(),
      adhocDefinitionRest.getDeploymentId()
    );
    assertEquals(
      activitiDefinition.getCategory(),
      adhocDefinitionRest.getCategory()
    );
    assertEquals(activitiDefinition.getName(), adhocDefinitionRest.getName());
    assertEquals(
      activitiDefinition.getVersion(),
      adhocDefinitionRest.getVersion()
    );
    assertEquals(
      (
        (ProcessDefinitionEntity) activitiDefinition
      ).isGraphicalNotationDefined(),
      adhocDefinitionRest.isGraphicNotationDefined()
    );
    assertEquals(
      "wf:submitAdhocTask",
      adhocDefinitionRest.getStartFormResourceKey()
    );
    assertEquals("New Task", adhocDefinitionRest.getTitle());
    assertEquals(
      "Assign a new task to yourself or a colleague",
      adhocDefinitionRest.getDescription()
    );

    Map<String, String> params = new HashMap<String, String>();
    params.put("maxItems", "2");
    JSONObject definitionListObject = processDefinitionsClient.getProcessDefinitionsWithRawResponse(
      params
    );
    assertNotNull(definitionListObject);
    JSONObject paginationJSON = (JSONObject) definitionListObject.get(
      "pagination"
    );
    ...
    definitionListObject =
      processDefinitionsClient.getProcessDefinitionsWithRawResponse(params);
    assertNotNull(definitionListObject);
  ...
  public void testGetProcessDefinitionsSorting() throws Exception {
    ...
    assertEquals(5, processDefinitions.size());
    List<String> deploymentIds = collect(
      processDefinitions,
      new Collector() {
        @Override
        public String collect(ProcessDefinition definition) {
    ...
    try {
      ...
      assertTrue(
        e
          .getMessage()
          .contains("OrderBy sausage is not supported, supported items are")
      );
    }
    ...
    try {
      ...
      assertTrue(
        e
          .getHttpResponse()
          .getResponse()
          .contains("Only one orderBy parameter is supported")
      );
    }
  }
  ...
  public void testGetProcessDefinitionsWhereClause() throws Exception {
    ...
    Map<String, ProcessDefinition> processDefinitionMap = getProcessDefinitions(
      processDefinitionsClient,
      "(category = 'http://alfresco.org')"
    );
    ...
    processDefinitionMap =
      getProcessDefinitions(processDefinitionsClient, "(name = 'unexisting')");
    assertEquals(0, processDefinitionMap.size());

    // Filter on key equals
    processDefinitionMap =
      getProcessDefinitions(processDefinitionsClient, "(key='activitiAdhoc')");
    assertTrue(processDefinitionMap.containsKey("activitiAdhoc"));
    assertEquals(1, processDefinitionMap.size());

    processDefinitionMap =
      getProcessDefinitions(processDefinitionsClient, "(key='unexisting')");
    assertEquals(0, processDefinitionMap.size());

    // Filter on version equals
    processDefinitionMap =
      getProcessDefinitions(processDefinitionsClient, "(version='1')");
    assertEquals(5, processDefinitionMap.size());

    processDefinitionMap =
      getProcessDefinitions(processDefinitionsClient, "(version='2')");
    assertEquals(0, processDefinitionMap.size());
    ...
    processDefinitionMap =
      getProcessDefinitions(
        processDefinitionsClient,
        "(deploymentId='" + activitiDefinition.getDeploymentId() + "')"
      );
    assertTrue(processDefinitionMap.containsKey("activitiAdhoc"));
    assertEquals(1, processDefinitionMap.size());
    ...
    processDefinitionMap =
      getProcessDefinitions(
        processDefinitionsClient,
        "(category = 'http://alfresco.org' AND name = 'Adhoc Activiti Process')"
      );
    assertTrue(processDefinitionMap.containsKey("activitiAdhoc"));
    assertEquals(1, processDefinitionMap.size());
    ...
    try {
      processDefinitionMap =
        getProcessDefinitions(
          processDefinitionsClient,
          "(category = 'http://alfresco.org' OR name = 'Adhoc Activiti Process')"
        );
      fail("Expected exception");
    } catch (PublicApiException e) {
      assertEquals(400, e.getHttpResponse().getStatusCode());
  ...
  public void testGetProcessDefinitionById() throws Exception {
    ...
    ProcessDefinitionsClient processDefinitionsClient = publicApiClient.processDefinitionsClient();
    ProcessDefinition adhocDefinition = processDefinitionsClient.findProcessDefinitionById(
      activitiDefinition.getId()
    );
    ...
    assertEquals(
      activitiDefinition.getDeploymentId(),
      adhocDefinition.getDeploymentId()
    );
    assertEquals(
      activitiDefinition.getCategory(),
      adhocDefinition.getCategory()
    );
    ...
    assertEquals(
      ...
      adhocDefinition.isGraphicNotationDefined()
    );
    assertEquals(
      "wf:submitAdhocTask",
      adhocDefinition.getStartFormResourceKey()
    );

    // get process definition with admin
    publicApiClient.setRequestContext(adminContext);
    adhocDefinition =
      processDefinitionsClient.findProcessDefinitionById(
        activitiDefinition.getId()
      );
    assertNotNull(adhocDefinition);
  ...
  public void testGetProcessDefinitionsImage() throws Exception {
    ...
    assertNotNull(activitiDefinition);
    try {
      HttpResponse response = processDefinitionsClient.findImageById(
        activitiDefinition.getId()
      );
  ...
  public void testGetProcessDefinitionStartModel() throws Exception {
    ...
    JSONObject model = processDefinitionsClient.findStartFormModel(
      activitiDefinition.getId()
    );
    assertNotNull(model);
    ...
    assertEquals(
      "{http://www.alfresco.org/model/bpm/1.0}sendEMailNotifications",
      modelEntry.get("qualifiedName")
    );
    ...
    assertEquals(
      "{http://www.alfresco.org/model/bpm/1.0}status",
      modelEntry.get("qualifiedName")
    );
    assertEquals("d:text", modelEntry.get("dataType"));
    ...
    publicApiClient.setRequestContext(adminContext);
    model =
      processDefinitionsClient.findStartFormModel(activitiDefinition.getId());
    assertNotNull(model);
  ...
  public void testMethodNotAllowedURIs() throws Exception {
    ...
    response =
      publicApiClient.delete("public", "process-definitions", null, null, null);
    assertEquals(405, response.getStatusCode());
    response =
      publicApiClient.put(
        "public",
  ...
  public void testAuthenticationAndAuthorization() throws Exception {
    ...
    processDefinitionsResponse =
      processDefinitionsClient.getProcessDefinitions(null);
    processDefinitionMap =
      getProcessDefinitionMapByKey(processDefinitionsResponse.getList());
  ...
  protected Map<String, ProcessDefinition> getProcessDefinitionMapByKey(
    List<ProcessDefinition> processDefinitions
  ) {
  ...
  protected Map<String, ProcessDefinition> getProcessDefinitions(
    ProcessDefinitionsClient processDefinitionsClient,
    String whereClause

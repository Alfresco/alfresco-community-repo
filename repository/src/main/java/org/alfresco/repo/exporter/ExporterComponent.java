...
public class ExporterComponent implements ExporterService {
  ...
  public void exportView(
    ...
    Exporter xmlExporter = createXMLExporter(
      viewWriter,
      parameters.getReferenceType()
    );
  ...
  public void exportView(
    ...
    ChainedExporter chainedExporter = new ChainedExporter(
      new Exporter[] { exporter, progress }
    );
   ...
   */
  private Exporter createXMLExporter(
    OutputStream viewWriter,
    ...
    try {
      ...
      ViewXMLExporter exporter = new ViewXMLExporter(
        namespaceService,
        nodeService,
        searchService,
        dictionaryService,
  ...
  private class DefaultCrawler implements ExporterCrawler {
    ...
    public void export(
      ...
      while (context.canRetrieve()) {
        ...
        if (parameters.isCrawlSelf()) {
          ...
          boolean rootNode = nodeService
            .getRootNode(nodeRef.getStoreRef())
            .equals(nodeRef);
    ...
    private void walkNode(
      ...
      if (parameters.getIncludedPaths() != null) {
        String nodePathPrefixString = nodeService
          .getPath(nodeRef)
          .toPrefixString(namespaceService);
        if (
          !(isIncludedPath(parameters.getIncludedPaths(), nodePathPrefixString))
        ) {
          return;
        }
      }
      ...
      AccessStatus readPermission = permissionService.hasPermission(
        nodeRef,
      ...
      if (
        ...
        boolean inheritPermissions = permissionService.getInheritParentPermissions(
          nodeRef
        );
      ...
      for (QName property : properties.keySet()) {
        ...
        if (
          isExcludedAspectProperty(parameters.getExcludeAspects(), property)
        ) {
          continue;
        }
        ...
        for (ChildAssociationRef childAssoc : childAssocs) {
          ...
          if (
            isExcludedChildAssoc(
              parameters.getExcludeChildAssocs(),
              childAssocType
            )
          ) {
            continue;
    ...
    private void walkProperty(
      ...
      if (
        valueDataType == null ||
        !valueDataType.equals(DataTypeDefinition.CONTENT)
      ) {
          ...
          try {
            ...
            try {
              ...
              throw new ExporterException(
                "Failed to export node content for node " + nodeRef,
                e
              );
            }
          }
     ...
     */
    private boolean isExcludedAspect(
      QName[] excludeAspects,
      QName aspectQName
    ) {
      if (
        aspectQName.equals(ContentModel.ASPECT_MULTILINGUAL_DOCUMENT) ||
     ...
     * @return                      <tt>true</tt> if the aspect can't be exported
     */
    private boolean isExcludedChildAssoc(
      QName[] excludeChildAssocs,
     ...
     */
    private boolean isExcludedAspectProperty(
      QName[] excludeAspects,
      QName propertyQName
    ) {
     ...
     */
    private boolean isWithinExport(
      NodeRef nodeRef,
      ExporterCrawlerParameters parameters
    ) {
        ...
        for (NodeRef exportRoot : context.getExportList()) {
          if (nodeRef.equals(exportRoot) && parameters.isCrawlSelf() == true) {
            ...
            for (int i = nodePath.size() - 1; i >= 0; i--) {
              Path.ChildAssocElement pathElement = (Path.ChildAssocElement) nodePath.get(
                i
              );

...
public class DynamicSolrStoreMappingWrapperFactory {
   ...
   * @return
   */
  public static SolrStoreMappingWrapper wrap(
    List<ShardInstance> slice,
    BeanFactory beanFactory
  ) {
    HttpClientFactory httpClientFactory = (HttpClientFactory) beanFactory.getBean(
    ...
    for (ShardInstance instance : slice) {
      ...
      if (!clients.contains(key)) {
        clients.put(
          key,
          httpClientFactory.getHttpClient(key.getFirst(), key.getSecond())
        );
      }
    }

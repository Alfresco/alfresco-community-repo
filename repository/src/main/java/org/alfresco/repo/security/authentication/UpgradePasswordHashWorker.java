 ...
 * <h2>What it is</h2>
 *
 * A worker for a scheduled job that checks and upgrades users passwords to the system's preferred
 * encoding.
 *
 * <h2>Settings that control the behaviour</h2>
 *
 * <ul>
 *   <li><b>${system.upgradePasswordHash.jobBatchSize}</b> - the number of users to process at one
 *       time.
 *   <li><b>${system.upgradePasswordHash.jobQueryRange}</b> - the node ID range to query for. The
 ...
 *   <li><b>${system.upgradePasswordHash.jobThreadCount}</b> - the number of threads that will
 *       handle user checks and changes. Increase or decrease this to allow for free CPU capacity on
...
public class UpgradePasswordHashWorker implements ApplicationContextAware, InitializingBean {
    private static final QName LOCK =
            QName.createQName(NamespaceService.SYSTEM_MODEL_1_0_URI, "UpgradePasswordHashWorker");
    ...
    public boolean processPasswordHash(Map<QName, Serializable> properties) {
        ...
        Pair<List<String>, String> passwordHash =
        ...
        if (!passwordEncoder.lastEncodingIsPreferred(passwordHash.getFirst())) {
            ...
            if (passwordEncoder.isSafeToEncodeChain(nowHashed)) {
                ...
                properties.put(
                        ...
                        passwordEncoder.encodePreferred(
                                new String(passwordHash.getSecond()), salt));
                ...
                logger.warn(
                        "Unable to upgrade password hash for user '"
                                + username
        ...
        if (hashIndicator == null) {
            ...
            if (logger.isTraceEnabled()) {
                ...
                logger.trace(
                        "User '"
                                + username
                                + "' has preferred encoding, just updating properties to match.");
            }
            properties.put(
    ...
    private synchronized void doWork(UpgradePasswordHashWorkResult progress) throws Exception {
        ...
        BatchProcessor<Long> batchProcessor =
                new BatchProcessor<Long>(
    ...
    private class UpgradePasswordHashWorkProvider implements BatchProcessWorkProvider<Long> {
        ...
        public Collection<Long> getNextWork() {
            ...
            while (ret.isEmpty() && progress.currentMinNodeId.get() < maxNodeId) {
                ...
                ret =
                        patchDAO.getNodesByTypeQNameId(
                                this.userTypeId.getFirst(), minNodeId, maxNodeId);
            }
    ...
    public static class UpgradePasswordHashWorkResult {
        ...
        public String toString() {
            ...
            String part2 =
                    String.format(
                            " %4d out of a potential %4d users. ",
                            usersChanged.get(), usersProcessed.get());
     ...
     *
     * <ul>
     *   <li><b>upgradePasswordHashWorker</b> - The worker that performs the actual processing.
     * </ul>
     *
    ...
    public static class UpgradePasswordHashJob implements Job {
        ...
        public void execute(JobExecutionContext context) throws JobExecutionException {
            ...
            if (upgradePasswordHashWorkerObj == null
                    || !(upgradePasswordHashWorkerObj instanceof UpgradePasswordHashWorker)) {
                throw new AlfrescoRuntimeException(
                        "UpgradePasswordHashJob data '"
                                + JOB_DATA_WORKER
                                + "' must reference a "
                                + UpgradePasswordHashWorker.class.getSimpleName());
            }

            UpgradePasswordHashWorker worker =
                    (UpgradePasswordHashWorker) upgradePasswordHashWorkerObj;
            worker.execute();
        }
    }
}

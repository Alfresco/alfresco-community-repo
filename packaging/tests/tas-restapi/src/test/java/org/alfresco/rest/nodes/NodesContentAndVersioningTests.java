...
public class NodesContentAndVersioningTests extends RestTest {
  ...
  public void checkFileNameWithRegularCharsInHeader() throws Exception {
    restClient
      ...
      .withCoreAPI()
      .usingNode(file1)
      .usingParams("attachment=false")
      .getNodeContent();
    restClient.assertStatusCodeIs(HttpStatus.OK);
    restClient.assertHeaderValueContains(
      "Content-Disposition",
      String.format("filename=\"%s\"", file1.getName())
    );
  }

  @Bug(
    id = "MNT-17545",
    description = "HTTP Header Injection in ContentStreamer",
    status = Bug.Status.FIXED
  )
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.NODES },
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.NODES },
    executionType = ExecutionType.SANITY,
    description = "Verify that alfresco returns the correct encoding for files created via REST."
  )
  public void verifyFileEncodingUsingRestAPI() throws Exception {
    STEP(
      "1. Create a folder, two text file templates and define the expected encoding."
    );
    ...
    STEP(
      "2. Using multipart data upload (POST nodes/{nodeId}/children) the UTF-8 encoded file."
    );
    restClient
      .authenticateUser(user1)
      ...
      .addMultiPart(
        "filedata",
        Utility.getResourceTestDataFile("UTF-8File.txt")
      );
    RestNodeModel fileNode = restClient
      .withCoreAPI()
      .usingNode(folder)
      .createNode();
    ...
    STEP(
      "3. Using multipart data upload (POST nodes/{nodeId}/children) the ISO-8859-1 file."
    );
    restClient
      .configureRequestSpec()
      .addMultiPart(
        "filedata",
        Utility.getResourceTestDataFile("iso8859File.txt")
      );
    ...
    STEP(
      "4. Retrieve the nodes and verify that the content type is the expected one (GET nodes/{nodeId})."
    );
    restClient
      .withCoreAPI()
      .usingNode(utf8File)
    ...
    restClient
      .withCoreAPI()
      .usingNode(iso8859File)
      .getNodeContent()
      .assertThat()
      .contentType(iso8859Type);
  }

  // This test takes around 30 minutes to complete so it will be ignored by default
  @Test(
    enabled = false,
    groups = { TestGroup.REST_API, TestGroup.NODES, TestGroup.REGRESSION }
  )
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.NODES },
    executionType = ExecutionType.REGRESSION,
    description = "Verify that the node content is streamed directly to the client and not buffered in memory."
  )
  public void verifyUploadDownloadLargeFileUsingRestAPI() throws Exception {
    ...
    STEP(
      "2. Using multipart data upload for the large file (POST nodes/{nodeId}/children)."
    );
    ...
    restClient
      .authenticateUser(user1)
      .configureRequestSpec()
      .addMultiPart("filedata", largeFile);
    RestNodeModel fileNode = restClient
      .withCoreAPI()
      .usingNode(folder)
      .createNode();
    ...
    STEP(
      "3. Retrieve the content of the node without running out of memory (GET nodes/{nodeId}/content)."
    );
    RestResponse nodeContent = restClient
      .withCoreAPI()
      .usingNode(largeFileModel)
      .getNodeContent();
    restClient.assertStatusCodeIs(HttpStatus.OK);
    assertEquals(
      Integer.valueOf(nodeContent.getResponse().getHeader("Content-Length")),
      largeFileSizeBytes
    );
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.NODES },
    executionType = ExecutionType.SANITY,
    description = "Verify updating a node content."
  )
  public void testUpdateNodeContent() throws Exception {
    STEP(
      "1. Retrieve the node in order to get data to compare after update GET /nodes/{nodeId}?include=path."
    );
    RestNodeModel initialNode = restClient
      .authenticateUser(user1)
      .withCoreAPI()
      .usingNode(file1)
      .usingParams("include=path")
      .getNode();
    restClient.assertStatusCodeIs(HttpStatus.OK);

    STEP(
      "2. Update the node content (different from the initial one) PUT /nodes/{nodeId}/content?majorVersion=true&name=newfile.txt."
    );
    File updatedConentFile = Utility.getResourceTestDataFile(
      "sampleContent.txt"
    );
    RestNodeModel updatedBodyNode = restClient
      .withCoreAPI()
      .usingNode(file1)
      .usingParams("majorVersion=true&name=newfile.txt")
      .updateNodeContent(updatedConentFile);
    restClient.assertStatusCodeIs(HttpStatus.OK);

    STEP(
      "3. Compare contentSize, modifiedAt, name and version, they should be different."
    );
    assertNotSame(
      initialNode.getContent().getSizeInBytes(),
      updatedBodyNode.getContent().getSizeInBytes()
    );
    ...
    String initialNodeVersion = new JSONObject(initialNode.toJson())
      .getJSONObject("properties")
      .getString("cm:versionLabel");
    String updatedBodyNodeVersion = new JSONObject(updatedBodyNode.toJson())
      .getJSONObject("properties")
      .getString("cm:versionLabel");
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.NODES },
    executionType = ExecutionType.SANITY,
    description = "Test copy a node."
  )
  public void testCopyNode() throws Exception {
    STEP(
      "1. Create a lock and lock the node POST /nodes/{nodeId}/lock?include=path,isLocked."
    );
    ...
    RestNodeModel initialNode = restClient
      ...
      .withCoreAPI()
      .usingNode(file1)
      .usingParams("include=path,isLocked")
      .lockNode(lockBodyModel);
    restClient.assertStatusCodeIs(HttpStatus.OK);

    STEP(
      "2. With another user(that has access to the file), copy the node to another path POST /nodes/{nodeId}/copy?include=path,isLocked."
    );
    String postBody = JsonBodyGenerator.keyValueJson(
      "targetParentId",
      site2.getGuid()
    );
    RestNodeModel copiedNode = restClient
      .authenticateUser(user2)
      .withCoreAPI()
      .usingNode(file1)
      .usingParams("include=path,isLocked")
    ...
    STEP(
      "3. ParentId, createdAt, path and lock are different, but the nodes have the same contentSize."
    );
    ...
    assertSame(
      copiedNode.getContent().getSizeInBytes(),
      initialNode.getContent().getSizeInBytes()
    );
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.NODES },
    executionType = ExecutionType.SANITY,
    description = "Test retrieving node versions, a specific version and version content."
  )
  public void testGetVersionContent() throws Exception {
    ...
    STEP(
      "1. Update the node content in order to increase version(one minor and one major) PUT /nodes/{nodeId}/content."
    );
    ...
    STEP(
      "2. List node version history GET /nodes/{nodeId}/versions. And verify that first (in the list) version is 2.0 and last is 1.0."
    );
    RestVersionModelsCollection versionListing = restClient
      .withCoreAPI()
      .usingNode(file2)
      .listVersionHistory();
    restClient.assertStatusCodeIs(HttpStatus.OK);
    ...
    assertEquals(
      versionListing
        .getEntries()
        .get(versionListing.getPagination().getCount() - 1)
        .onModel()
        .getId(),
      "1.0"
    );
    ...
    STEP(
      "3. List node version using skipCount(1),maxItems(1) and include(properties) GET /nodes/{nodeId}/versions?include=properties&skipCount=1&maxItems=1"
    );
    versionListing =
      restClient
        .withCoreAPI()
        .usingParams("include=properties&skipCount=1&maxItems=1")
        .usingNode(file2)
        .listVersionHistory();
    restClient.assertStatusCodeIs(HttpStatus.OK);
    ...
    STEP(
      "4. Get version information for version 1.1 GET /nodes/{nodeId}/versions/{versionId} ."
    );
    RestVersionModel version11 = restClient
      .withCoreAPI()
      .usingNode(file2)
      .getVersionInformation("1.1");
    restClient.assertStatusCodeIs(HttpStatus.OK);
    ...
    assertEquals(
      version.getContent().getSizeInBytes(),
      version11.getContent().getSizeInBytes()
    );
    assertEquals(version.getName(), version11.getName());

    STEP(
      "5. Retrieve version 2.0 content GET /nodes/{nodeId}/versions/{versionId}/content"
    );
    ...
    Utility.sleep(
      ...
      60000,
      () -> {
        RestResponse versionContent = restClient
          .withCoreAPI()
          .usingNode(file2)
          .getVersionContent("2.0");
        restClient.assertStatusCodeIs(HttpStatus.OK);
        ...
        restClient.assertHeaderValueContains(
          "Content-Disposition",
          String.format("filename=\"%s\"", file2.getName())
        );
      }
    );
  }
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.NODES },
    executionType = ExecutionType.SANITY,
    description = "Test revert and delete a node version."
  )
  public void testRevertDeleteVersion() throws Exception {
    STEP(
      "1. Revert to version 1.0 POST /nodes/{nodeId}/versions/{versionId}/revert"
    );
    RestVersionModel version = restClient
      .authenticateUser(user2)
      .withCoreAPI()
      .usingNode(file2)
      .revertVersion("1.0", new String("{}"));
    restClient.assertStatusCodeIs(HttpStatus.OK);
    String nodeVersionType = new JSONObject(version.toJson())
      .getJSONObject("properties")
      .getString("cm:versionType");
    assertEquals(nodeVersionType, "MINOR");
    String nodeVersion = version.getId();

    STEP(
      "2. Revert to last minor version /nodes/{nodeId}/versions/{versionId}/revert"
    );
    version =
      restClient
        .withCoreAPI()
        .usingNode(file2)
        .revertVersion(nodeVersion, new String("{\"majorVersion\": true}"));
    restClient.assertStatusCodeIs(HttpStatus.OK);
    nodeVersionType =
      new JSONObject(version.toJson())
        .getJSONObject("properties")
        .getString("cm:versionType");
    ...
    STEP(
      "3. Delete last MINOR version DELETE /nodes/{nodeId}/versions/{versionId}"
    );
    ...
    restClient
      .withCoreAPI()
      .usingNode(file2)
      .getVersionInformation(nodeVersion);
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.NODES },
    executionType = ExecutionType.SANITY,
    description = "Verify file name in Content Range header"
  )
  @Test(groups = { TestGroup.REST_API, TestGroup.NODES, TestGroup.SANITY })

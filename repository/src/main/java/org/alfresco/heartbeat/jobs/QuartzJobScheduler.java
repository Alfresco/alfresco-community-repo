...
public abstract class QuartzJobScheduler implements HeartBeatJobScheduler {
  ...
  public void scheduleJob(HBBaseDataCollector collector) {
    ...
    final String cronExpression = testMode
      ? testCronExpression
      : collector.getCronExpression();
    // Schedule job
    ...
    try {
      ...
      throw new RuntimeException(
        "Heartbeat failed to schedule job for collector: " +
  ...
  public void unscheduleJob(final HBBaseDataCollector collector) {
    try {
      scheduler.unscheduleJob(
        new TriggerKey(getTriggerName(collector.getCollectorId()))
      );

      if (logger.isDebugEnabled()) {
        logger.debug(
          "HeartBeat unscheduled job for collector: " +
          collector.getCollectorId()
        );
      }
    } catch (SchedulerException e) {

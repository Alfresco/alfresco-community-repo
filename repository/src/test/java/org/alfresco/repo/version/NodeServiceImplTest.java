...
public class NodeServiceImplTest extends BaseVersionStoreTest {
  ...
  public void before() throws Exception {
    ...
    this.dummyNodeRef =
      new NodeRef(this.versionService.getVersionStoreReference(), "dummy");
  ...
  public void testGetType() {
    ...
    QName versionedType =
      this.versionStoreNodeService.getType(version.getFrozenStateNodeRef());
  ...
  public void testGetProperties() {
    ...
    Map<QName, Serializable> origProps =
      this.dbNodeService.getProperties(versionableNode);
    ...
    Map<QName, Serializable> versionedProperties =
      this.versionStoreNodeService.getProperties(
          version.getFrozenStateNodeRef()
        );

    if (logger.isDebugEnabled()) {
      logger.debug(
        "original (" + origProps.size() + "):  " + origProps.keySet()
      );
      logger.debug(
        "versioned (" +
  ...
  public void testGetProperty() {
    ...
    Collection<String> multiValue = (Collection<String>) this.versionStoreNodeService.getProperty(
        version.getFrozenStateNodeRef(),
        MULTI_PROP
      );
  ...
  public void testGetChildAssocs() {
    ...
    Collection<ChildAssociationRef> versionedChildren =
      this.versionStoreNodeService.getChildAssocs(
          version.getFrozenStateNodeRef()
        );
    ...
    for (ChildAssociationRef versionedChildRef : versionedChildren) {
      ChildAssociationRef origChildAssocRef = originalChildAssocRefs.get(
        versionedChildRef.getChildRef().getId()
      );
  ...
  public void testGetPrimaryParent() {
    ...
    ChildAssociationRef childAssoc =
      this.versionStoreNodeService.getPrimaryParent(nodeRef);
  ...
  public void testUnicodeNamesAndProperties() {
    ...
    cachesToClear.add(
      (TransactionalCache) this.applicationContext.getBean("node.nodesCache")
    );
    cachesToClear.add(
      (TransactionalCache) this.applicationContext.getBean(
    ...
    assertEquals(
      engProp,
      nodeService.getProperty(engNode, ContentModel.PROP_NAME)
    );
    assertEquals(
      1,
      nodeService
    ...
    assertEquals(
      frProp,
      nodeService.getProperty(frNode, ContentModel.PROP_NAME)
    );
    assertEquals(
      1,
      nodeService
    ...
    assertEquals(
      esProp,
      nodeService.getProperty(esNode, ContentModel.PROP_NAME)
    );
    assertEquals(
      1,
      nodeService
    ...
    // Finally Japanese
    String jpProp =
      "\u3092\u30af\u30ea\u30c3\u30af\u3057\u3066\u304f\u3060\u3055\u3044\u3002"; //  ã‚’ã‚¯ãƒªãƒƒã‚¯ã�—ã�¦ã��ã� ã�•ã�„ã€‚
    QName jpQName = QName.createQName(
      "NameSpace",
      "\u3092\u30af\u30ea\u30c3\u30af\u3057\u3066\u304f"
    ); //  ã‚’ã‚¯ãƒªãƒƒã‚¯ã�—ã�¦ã��
    NodeRef jpNode = nodeService
      .createNode(
        this.rootNodeRef,
        ContentModel.ASSOC_CONTAINS,
        jpQName,
    ...
    assertEquals(
      jpProp,
      nodeService.getProperty(jpNode, ContentModel.PROP_NAME)
    );
    assertEquals(
      1,
      nodeService
  ...
  public void testALF1793AddVersionableAspect() {
    ...
    Map<QName, Serializable> oldProperties =
      this.dbNodeService.getProperties(versionableNode);
    Map<QName, Serializable> newProperties = new HashMap<QName, Serializable>();
    newProperties.put(
      ContentModel.PROP_NODE_UUID,
      oldProperties.get(ContentModel.PROP_NODE_UUID)
    );
  ...
  public void testTakeOwnershipPermission() {
    NodeService proxyNodeService = (NodeService) applicationContext.getBean(
      "NodeService"
    );
    ...
    boolean isAble =
      ...
      permissionService.hasPermission(docRef, PermissionService.TAKE_OWNERSHIP);
    ...
    String updatedOwner = (String) this.nodeService.getProperty(
        docRef,
        ContentModel.PROP_OWNER
      );

    boolean isUserOwner = updatedOwner == null || !updatedOwner.equals(userName)
      ? false
      : true;
    assertEquals(
      "Ownership's rights to the document have been taken by the user that has Editor permissions (addAspect).",
      false,
      isUserOwner
    );
    ...
    isUserOwner =
      updatedOwner == null || !updatedOwner.equals(userName) ? false : true;
    assertEquals(
      "Ownership's rights to the document have been taken by the user that has Editor permissions (addProperties).",
      false,
      isUserOwner
    );
    ...
    isUserOwner =
      updatedOwner == null || !updatedOwner.equals(userName) ? false : true;
    assertEquals(
      "Ownership's rights to the document have been taken by the user that has Editor permissions (setProperties).",
      false,
      isUserOwner
    );
    ...
    isUserOwner =
      updatedOwner == null || !updatedOwner.equals(userName) ? false : true;
    assertEquals(
      "Ownership's rights to the document have been taken by the user that has Editor permissions (setProperty).",
      false,
      isUserOwner
    );
  }

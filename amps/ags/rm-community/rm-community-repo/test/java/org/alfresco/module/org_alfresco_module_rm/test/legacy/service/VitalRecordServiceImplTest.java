...
public class VitalRecordServiceImplTest extends BaseRMTestCase {
     ...
     *
     * <p>|--rmRootContainer (no vr def) | |--mhContainer (no vr def) | |--mhContainer-1-1 (has
     * schedule - folder level) (no vr def) | | | |--mhContainer-2-1 (vr def) | | |
     * |--mhContainer-3-1 (no vr def) | |--mhContainer-1-2 (has schedule - folder level) (no vr def)
     * | |--mhContainer-2-2 (no vr def) | | | |--mhContainer-3-2 (vr def disabled) | | |
     * |--mhContainer-3-3 (has schedule - record level) (vr def) | |--mhContainer-2-3 (has schedule
     * - folder level) (vr def) | |--mhContainer-3-4 (no vr def) | |--mhContainer-3-5 (has schedule-
     * record level) (vr def)
     */
    @Override
     ...
     * @param period
     */
    private void setupVitalRecordDefinition(
            final NodeRef nodeRef, final boolean enabled, final Period period) {
        retryingTransactionHelper.doInTransaction(
                new RetryingTransactionCallback<Object>() {
                    @Override
                    public Object execute() throws Throwable {
    ...
    /**
     * Based on the initial data: - check category, folder and record raw values. - check search
     * aspect values.
     */
        ...
        doTestInTransaction(
                new Test<NodeRef>() {
                    ...
                    public NodeRef run() {
                        NodeRef record =
                                fileFolderService
                                        .create(mhRecordFolder41, "test101.txt", TYPE_CONTENT)
                                        .getNodeRef();
    ...
    /**
     * Test to ensure that changes made to vital record definitions are reflected down the
     * hierarchy.
     */
    ...
    private void assertHasVitalRecordDefinition(NodeRef nodeRef, boolean enabled, Period period) {
        ...
        Boolean vitalRecordIndicator =
                (Boolean) nodeService.getProperty(nodeRef, PROP_VITAL_RECORD_INDICATOR);
        ...
        if (enabled) {
            ...
            assertEquals(
                    period.getNextDate(new Date()).getDate(), def.getNextReviewDate().getDate());
        }
    }
        ...
        if (enabled) {
            ...
            assertEquals(
                    ...
                    nodeService.getProperty(
                            nodeRef,
                            RecordsManagementSearchBehaviour.PROP_RS_VITAL_RECORD_REVIEW_PERIOD));
            assertEquals(

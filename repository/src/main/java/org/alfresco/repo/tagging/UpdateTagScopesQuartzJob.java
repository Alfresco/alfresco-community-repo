 ...
 * {@link UpdateTagScopesActionExecuter}, and is typically run after server restart. Also refer to
 * scheduled-jobs-context.xml for more information
 */
public class UpdateTagScopesQuartzJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (transactionServiceO == null || !(transactionServiceO instanceof TransactionService)) {
            throw new AlfrescoRuntimeException(
                    "UpdateTagScopesQuartzJob data must contain a valid 'transactionService'"
                            + " reference");
        }

        Object updateTagsActionO = jobData.get("updateTagsAction");
        if (updateTagsActionO == null
                || !(updateTagsActionO instanceof UpdateTagScopesActionExecuter)) {
            throw new AlfrescoRuntimeException(
                    "UpdateTagScopesQuartzJob data must contain a valid 'updateTagsAction'"
                            + " reference");
        }
        ...
        UpdateTagScopesActionExecuter updateTagsAction =
                (UpdateTagScopesActionExecuter) updateTagsActionO;
    ...
    protected void execute(
        ...
        while (true) {
                ...
                if (handledTagNodes.contains(nodeRef)) {
                    ...
                    if (logger.isDebugEnabled())
                        logger.debug(
                                "Tag scope "
                                        + nodeRef
                                        + " must be being processed by another Thread, not updating"
                                        + " it");
                }
            }
            ...
            action.setParameterValue(
                    UpdateTagScopesActionExecuter.PARAM_TAG_SCOPES, (Serializable) tagNodes);

...
public class VirtualHashReferenceParser extends ProtocolHashParser {
    ...
    public VirtualHashReferenceParser(HashStore classpathHashStore) {
        this(classpathHashStore, VIRTUAL_PROTOCOL_CODE);
    }
    ...
    public Reference parse(Cursor cursor) throws ReferenceParseException {
        ...
        if (HASHED_NUMERIC_PATH_CODE.equals(pathCode)) {
            ...
            templatePath = numericPathHasher.lookup(new Pair<String, String>(pathHash, null));
            ...
            templatePath = numericPathHasher.lookup(new Pair<String, String>(null, pathNonHashed));
        } else if (MIXED_NUMERIC_PATH_CODE.equals(pathCode)) {
            String pathHash = pathToken.substring(1);
            String pathNonHashed = cursor.nextToken();
            templatePath =
                    numericPathHasher.lookup(new Pair<String, String>(pathHash, pathNonHashed));
        } else {
            throw new ReferenceParseException("Pnvalid path token code " + pathCode);
        }

        return parseVirtualExtension(cursor, templateResource, templatePath, actualNodeResource);
    }
    ...
    protected Reference parseVirtualExtension(
        ...
        return ((VirtualProtocol) Protocols.VIRTUAL.protocol)
                .newReference(

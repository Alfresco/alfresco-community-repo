...
public class VersionableAspect
    ...
    /**
     * Optional list of excluded props - only applies if cm:autoVersionOnUpdateProps=true (and
     * cm:autoVersion=true) - if any one these props changes then "auto version on prop update" does
     * not occur (even if there are other property changes)
     */
    ...
    public void init() {
        this.policyComponent.bindClassBehaviour(
                ...
                new JavaBehaviour(
                        this, "beforeAddAspect", Behaviour.NotificationFrequency.EVERY_EVENT));

        this.policyComponent.bindClassBehaviour(
                QName.createQName(NamespaceService.ALFRESCO_URI, "onAddAspect"),
                ContentModel.ASPECT_VERSIONABLE,
                new JavaBehaviour(
                        this, "onAddAspect", Behaviour.NotificationFrequency.TRANSACTION_COMMIT));
        ...
        this.policyComponent.bindClassBehaviour(
                ...
                ContentModel.ASPECT_VERSIONABLE,
                new JavaBehaviour(
                        this, "onDeleteNode", Behaviour.NotificationFrequency.TRANSACTION_COMMIT));

        this.policyComponent.bindClassBehaviour(
                QName.createQName(NamespaceService.ALFRESCO_URI, "afterCreateVersion"),
                ContentModel.ASPECT_VERSIONABLE,
                new JavaBehaviour(
                        this, "afterCreateVersion", Behaviour.NotificationFrequency.EVERY_EVENT));
    ...
    public void onDeleteNode(ChildAssociationRef childAssocRef, boolean isNodeArchived) {
        if (isNodeArchived == false) {
            // If we are perminantly deleting the node then we need to remove the associated version
            // history
            this.versionService.deleteVersionHistory(childAssocRef.getChildRef());
        }
        // otherwise we do nothing since we need to hold onto the version history in case the node
        // is restored later
    }
    ...
    private static class VersionableAspectCopyBehaviourCallback
        ...
        private static final CopyBehaviourCallback INSTANCE =
                new VersionableAspectCopyBehaviourCallback();

        /**
         * Copy the aspect, but only the {@link ContentModel#PROP_AUTO_VERSION} and {@link
         * ContentModel#PROP_AUTO_VERSION_PROPS} properties
         */
        @Override
            ...
            Boolean value =
                    (Boolean)
                            this.nodeService.getProperty(
                                    nodeRef, ContentModel.PROP_INITIAL_VERSION);
            if (value != null) {
                initialVersion = value.booleanValue();
            }
            // else this means that the default value has not been set the versionable aspect we
            // applied pre-1.2

            if (initialVersion == true) {
                @SuppressWarnings("unchecked")
                Map<NodeRef, NodeRef> versionedNodeRefs =
                        (Map<NodeRef, NodeRef>)
                                AlfrescoTransactionSupport.getResource(KEY_VERSIONED_NODEREFS);
                    ...
                    String versionType =
                            (String)
                                    nodeService.getProperty(
                                            nodeRef, ContentModel.PROP_VERSION_TYPE);
                    if (versionType == null || !versionType.equals(VersionType.MINOR.toString())) {
                        versionProperties.put(VersionModel.PROP_VERSION_TYPE, VersionType.MAJOR);
                    }

                    versionProperties.put(
                            Version.PROP_DESCRIPTION, I18NUtil.getMessage(MSG_INITIAL_VERSION));
    ...
    public void onRemoveAspect(NodeRef nodeRef, QName aspectTypeQName) {
        // When the versionable aspect is removed from a node, then delete the associated version
        // history
     ...
     *
     * <p>If applicable and "cm:autoVersion" is TRUE then version the node on content update (even
     * if no property updates)
     */
    ...
    public void onContentUpdate(NodeRef nodeRef, boolean newContent) {
        if (this.nodeService.exists(nodeRef) == true
            ...
            Map<NodeRef, NodeRef> versionedNodeRefs =
                    (Map) AlfrescoTransactionSupport.getResource(KEY_VERSIONED_NODEREFS);
            if (versionedNodeRefs == null || versionedNodeRefs.containsKey(nodeRef) == false) {
                ...
                // else this means that the default value has not been set and the versionable
                // aspect was applied pre-1.1
                ...
                if (autoVersion == true) {
                    ...
                    Map<String, Serializable> versionProperties =
                            new HashMap<String, Serializable>(1);
                    versionProperties.put(
                            Version.PROP_DESCRIPTION, I18NUtil.getMessage(MSG_AUTO_VERSION));
     ...
     *
     * <p>If applicable and "cm:autoVersionOnUpdateProps" is TRUE then version the node on
     * properties update (even if no content updates)
     *
     * @since 3.2
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
            ...
            try {
                ...
                if (versionedNodeRefs == null || versionedNodeRefs.containsKey(nodeRef) == false) {
                    ...
                    Boolean value =
                            (Boolean)
                                    this.nodeService.getProperty(
                                            nodeRef, ContentModel.PROP_AUTO_VERSION_PROPS);
                    if (value != null) {
                        // If the value is not null then
                    ...
                    if (autoVersionProps == true) {
                        // Check for explicitly excluded props - if one or more excluded props
                        // changes then do not auto-version on this event (even if other props
                        // changed)
                        ...
                        // Create the auto-version
                        Map<String, Serializable> versionProperties =
                                new HashMap<String, Serializable>(4);
                        versionProperties.put(
    ...
    /**
     * @see
     *     org.alfresco.repo.version.VersionServicePolicies.OnCreateVersionPolicy#onCreateVersion(org.alfresco.service.namespace.QName,
    ...
    public void afterDictionaryInit() {
        ...
        for (String prefixString : this.excludedOnUpdateProps) {
            try {
                this.excludedOnUpdatePropQNames.add(
                        QName.createQName(prefixString, this.namespacePrefixResolver));
            } catch (Exception e) {

...
public class RepoTertiaryManifestProcessorImpl
  ...
  private static final Log log = LogFactory.getLog(
    RepoTertiaryManifestProcessorImpl.class
  );
   ...
   */
  public RepoTertiaryManifestProcessorImpl(
  ...
  protected void processNode(TransferManifestNormalNode node) {
    ...
    if (isSync) {
      ...
      if (nodeService.exists(nodeRef)) {
        ...
        if (
          (null != resolvedNodes.resolvedParent) &&
        ...
        for (ChildAssociationRef ref : expectedChildren) {
          ...
          if (nodeService.exists(ref.getChildRef())) {
            ...
            ResolvedParentChildPair resolvedChild = nodeResolver.resolveCorrespondingNode(
              ref.getChildRef(),
              ref,
              parentPath
            );
            ...
            if (null != resolvedChild.resolvedChild) {
              ...
              if (log.isDebugEnabled()) {
                log.debug(
                  "The node DOES NOT exist in current repository! Processing will be made by its PATH!"
                );
              }
        ...
        List<ChildAssociationRef> actualChildren = nodeService.getChildAssocs(
          nodeRef
        );
        ...
        for (ChildAssociationRef child : actualChildren) {
          ...
          if (child.isPrimary()) {
            ...
            if (
              !expectedChildNodeRefs.contains(childNodeRef) &&
              ...
              if (
                ...
                logComment(
                  "Transfer sync mode - checking unexpected child node:" + child
                );
                String fromRepositoryId = (String) nodeService.getProperty(
                  childNodeRef,
                ...
                if (fromRepositoryId != null && manifestRepositoryId != null) {
                  if (
                     ...
                     */
                    log.debug(
                      "node to be deleted contains alien content so needs to be pruned." +
                      childNodeRef
                    );
                    logComment(
                      "Transfer sync mode - node contains alien content so can't be deleted. " +
                      childNodeRef
                    );
                    ...
                    String initialRepositoryId = (String) nodeService.getProperty(
                      childNodeRef,
                      TransferModel.PROP_FROM_REPOSITORY_ID
                    );

                    if (
                      manifestRepositoryId.equalsIgnoreCase(initialRepositoryId)
                    ) {
                      if (log.isDebugEnabled()) {
                        log.debug(
                          "Replication is initiated from the same repository from which this node was transferred! Deleting"
                        );
                      }
                      ...
                      if (log.isDebugEnabled()) {
                        log.debug(
                          "It is not an alien, but 'fromRepositoryId' is not equal to the 'manifestRepositoryId'! Cannot delete the foreign node..."
                        );
                      }
                    }
                  }

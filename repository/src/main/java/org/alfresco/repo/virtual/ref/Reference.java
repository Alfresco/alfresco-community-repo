...
public class Reference {
  ...
  public static final boolean isReference(NodeRef nodeRef) {
    if (nodeRef != null) {
      ...
      if (
        (id != null) && (id.length() > 1) && (id.startsWith("" + VIRTUAL_TOKEN))
      ) {
  ...
  private static synchronized void _trace_avg_ref_length(long refLength) {
    ...
    if (_trace_refBatchSize > 0 && _trace_refCount % _trace_refBatchSize == 0) {
      logger.trace(
        "Average reference encoding size : " +
        (_trace_refLength / _trace_refCount)
      );
      _trace_refCount = 0;
      _trace_refLength = 0;
    }
   ...
   * @throws ReferenceEncodingException
   */
  public NodeRef toNodeRef(StoreRef storeRef)
    throws ReferenceEncodingException {
   ...
   */
  public <R> R execute(ProtocolMethod<R> method)
    throws ProtocolMethodException {

...
public abstract class AbstractFeedCleanerTest {
  ...
  public void setUp() throws Exception {
    ...
    JobLockService jobLockService = (JobLockService) ctx.getBean(
      "JobLockService"
    );
  ...
  public void tearDown() throws Exception {
    ...
    feedDAO.deleteFeedEntries(
      new Date(System.currentTimeMillis() + (120 * 1000L))
    );
  ...
  public void testMaxAge() throws Exception {
    ...
    assertEquals(
      2,
      feedDAO
        .selectUserFeedEntries(TEST_USER_B, null, false, false, -1L, -1)
        .size()
    );
  ...
  public void testMaxSize() throws Exception {
    ...
    assertEquals(
      10,
      feedDAO
        .selectUserFeedEntries(TEST_USER_D, null, false, false, -1L, -1)
        .size()
    );
    ...
    assertEquals(
      10,
      feedDAO
        .selectUserFeedEntries(TEST_USER_F, null, false, false, -1L, -1)
        .size()
    );
  ...
  public void testSiteDelete() throws Exception {
    ...
    assertEquals(
      site4FeedCnt,
      feedDAO.selectSiteFeedEntries(TEST_SITE_4, -1).size()
    );
    assertEquals(
  ...
  public void testPersonDelete() throws Exception {
    boolean caseSensitive = Boolean.getBoolean(
      System.getProperty("user.name.caseSensitive")
    );
    if (caseSensitive) {
      ...
      new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
        public Void execute() throws Throwable {
          ...
          assertEquals(
            0,
            feedDAO
              .selectUserFeedEntries(userId, null, false, false, -1L, -1)
              .size()
          );
    ...
    public void run() {
      try {
        ...
        if (type == 0) {
          ...
          System.out.println(
            "[" + i + "] Inserted " + insertCount + " entries"
          );
        }
        ...
        if (type == 1) {
          ...
          int selectCount = transactionHelper.doInTransaction(
            new RetryingTransactionHelper.RetryingTransactionCallback<Integer>() {
              public Integer execute() throws Throwable {
                // query some entries
                int selectCount = feedDAO
                  .selectSiteFeedEntries(TEST_SITE_4, -1)
                  .size();
                return selectCount;
              }
            }
          ...
          System.out.println(
            "[" + i + "] Selected " + selectCount + " entries"
          );
        }
        ...
        if (type == 2) {
          ...
          int deleteCount = transactionHelper.doInTransaction(
            new RetryingTransactionHelper.RetryingTransactionCallback<Integer>() {
              public Integer execute() throws Throwable {
                // clean some entries

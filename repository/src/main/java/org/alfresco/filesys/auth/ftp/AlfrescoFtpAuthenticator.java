...
public class AlfrescoFtpAuthenticator extends FTPAuthenticatorBase {
    ...
    public boolean authenticateUser(ClientInfo client, FTPSrvSession sess) {
        ...
        if (client instanceof AlfrescoClientInfo == false) return false;
        ...
        try {
            ...
            getAuthenticationComponent()
                    .authenticate(client.getUserName(), client.getPasswordAsString().toCharArray());
            authSts = true;

            // Check if the user has been logged on successfully

            if (authSts == true) client.setLogonType(ClientInfo.LogonNormal);

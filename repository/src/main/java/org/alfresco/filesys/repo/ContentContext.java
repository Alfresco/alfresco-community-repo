...
public class ContentContext extends AlfrescoContext {
    ...
    private Pattern renameShufflePattern =
            Pattern.compile("(.*[a-f0-9]{8}+$)|(.*\\.tmp$)|(.*\\.wbk$)|(.*\\.bak$)|(.*\\~$)");
     ...
     */
    public ContentContext(
            String deviceName, String storeName, String rootPath, NodeRef rootNodeRef) {
    ...
    /**
     * Get the regular expression pattern that will be applied to detected potential rename
     * shuffles.
     *
    ...
    public void CloseContext() {
        ...
        if (m_nodeMonitor != null) m_nodeMonitor.shutdownRequest();
    ...
    public void startFilesystem(DiskSharedDevice share) throws DeviceContextException {
        ...
        if (getStateCache() != null) getStateCache().setCaseSensitive(false);

        // Find the thread pool via the configuration

        CoreServerConfigSection coreConfig =
                (CoreServerConfigSection)
                        share.getConfiguration()
                                .getConfigSection(CoreServerConfigSection.SectionName);
        if (coreConfig != null) m_threadPool = coreConfig.getThreadPool();
        ...
        if (m_nodeMonitor != null) m_nodeMonitor.startMonitor();
    }
}

...
/**
 * The SharedFolderPatch is a Generic Bootstrap Patch with the extra ability to rename an existing
 * folder that is in the way (in a different namespace).
 *
 * <p>The first use-case is when there is a child called cm:shared and we want to patch a folder
 * with app:shared
 *
 * @author mrogers
 */
public class SharedFolderPatch extends GenericBootstrapPatch {
    ...
    public void executeAsync() {
        // Lock the push
        QName lockQName =
                QName.createQName(NamespaceService.SYSTEM_MODEL_1_0_URI, "patch.sharedFolder");
        ...
        if (getRenamePath() != null) {
            List<NodeRef> results =
                    searchService.selectNodes(
                            rootNodeRef, getRenamePath(), null, namespaceService, false);
            ...
            if (results.size() > 1) {
                ...
                LinkedList<String> folderElements =
                        new LinkedList<String>(Arrays.asList(getRenamePath().split("/")));
                ...
                List<NodeRef> parentResults =
                        searchService.selectNodes(
                                rootNodeRef, parentPath.toString(), null, namespaceService, false);
                ...
                if (parentResults.size() == 1) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "Found the parent node - doing a move parentNodeRef:"
                                        + parentNodeRef);
                    }
                    ...
                    // Something has gone horribly wrong if we get here - we have multiple parents,
                    // or none despite finding the node earlier
    ...
    public static class SharedFolderPatchJob implements Job {
        ...
        public void execute(JobExecutionContext context) throws JobExecutionException {
            ...
            if (sharedFolderPatchObj == null
                    || !(sharedFolderPatchObj instanceof SharedFolderPatch)) {
                throw new AlfrescoRuntimeException(
                        "'sharedFolderPatch' data must contain valid 'SharedFolderPatch'"
                                + " reference");
            }

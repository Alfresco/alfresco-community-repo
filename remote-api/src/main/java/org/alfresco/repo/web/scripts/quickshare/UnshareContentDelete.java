  ...
  protected Map<String, Object> executeImpl(
    ...
    if (!isEnabled()) {
      throw new WebScriptException(
        ...
        "QuickShare is disabled system-wide"
      );
    }
    ...
    if (sharedId == null) {
      throw new WebScriptException(
        ...
        "A valid sharedId must be specified !"
      );
    }

    try {
      NodeRef nodeRef = quickShareService
        .getTenantNodeRefFromSharedId(sharedId)
        .getSecond();

      String sharedBy = (String) nodeService.getProperty(
        nodeRef,
      ...
      logger.error(
        "Unable to find: " + sharedId + " [" + inre.getNodeRef() + "]"
      );
      throw new WebScriptException(
        HttpServletResponse.SC_NOT_FOUND,
        "Unable to find: " + sharedId

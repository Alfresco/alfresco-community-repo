...
/**
 * WebDAVLockService is used to manage file locks for WebDAV and Sharepoint protocol. It ensures a
 * lock never persists for more than 24 hours, and also ensures locks are timed out on session
 * timeout.
 *
...
public class WebDAVLockServiceImpl implements WebDAVLockService {
    ...
    public void sessionDestroyed() {
        ...
        final List<Pair<String, NodeRef>> lockedResources =
                (List<Pair<String, NodeRef>>) session.getAttribute(LOCKED_RESOURCES);

        if (lockedResources != null && lockedResources.size() > 0) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Found "
                                + lockedResources.size()
                                + " locked resources for session: "
                                + session.getId());
            }
                                        ...
                                        .doInTransaction(
                                                new RetryingTransactionCallback<Void>() {
                                                    ...
                                                    public Void execute() throws Throwable {
                                                        // check whether this document still exists
                                                        // in repo
                                                        if (nodeService.exists(nodeRef)) {
                                                            ...
                                                            // forcibly unlock document if it is
                                                            // still locked and not checked out
    ...
    public void lock(NodeRef nodeRef, LockInfo lockInfo) {
        ...
        lockService.lock(
                nodeRef, LockType.WRITE_LOCK, (int) timeout, Lifetime.EPHEMERAL, additionalInfo);
    ...
    /**
     * Shared method for webdav/vti protocols to lock node. If node is locked for more than 24 hours
     * it is automatically added to the current session locked resources list.
     *
    ...
    /**
     * Add the given <code>object</code> to the session list that is stored in session under <code>
     * listName</code> attribute
     *
     ...
     */
    @SuppressWarnings("unchecked")
    private static final void storeObjectInSessionList(
            HttpSession session, String listName, Object object) {
    ...
    /**
     * Removes the given <code>object</code> from the session list that is stored in session under
     * <code>listName</code> attribute
     *
     * @param session the session
     * @param listName the list name (session attribute name)
     * @param object the object to store in session list
     * @return <tt>true</tt> if session list contained the specified element, otherwise
     *     <tt>false</tt>
     */
    @SuppressWarnings("unchecked")
    private static final boolean removeObjectFromSessionList(
            HttpSession session, String listName, Object object) {
        List<Object> list = null;
     ...
     */
    private LockInfo createLock(
            NodeRef nodeRef, String userName, boolean createExclusive, int timeoutSecs) {
        // Create Lock token
        ...
        if (!currentUser.equals(userName)) {
            throw new IllegalStateException(
                    "Node is being locked for user "
                            + userName
                            + " by (different/current) user "
                            + currentUser);
        }

...
public class AuditLogPost extends BaseAuditRetrievalWebScript {
    ...
    /**
     * @see
     *     org.alfresco.repo.web.scripts.content.StreamContent#execute(org.springframework.extensions.webscripts.WebScriptRequest,
    ...
    public void execute(WebScriptRequest req, WebScriptResponse res) throws IOException {
        try {
            ...
            // extract the destination parameter, ensure it's present and it is a record folder and
            // not closed or cut off
    ...
    /**
     * Helper method to put a key and a value to a json object. It handles the {@link JSONException}
     * so that a try/catch block is not need through out the code
     *
    ...
    private NodeRef fileAuditTrail(NodeRef destination, WebScriptRequest req) {
        ...
        NodeRef record =
                rmAuditService.fileAuditTrailAsRecord(
                        parseQueryParameters(req), destination, parseReportFormat(req));
    ...
    private JSONObject getJSONObjectFromRequest(WebScriptRequest req) throws IOException {
        ...
        try {
            ...
            throw new AlfrescoRuntimeException(
                    "Error creating json object from request content.", error);
        }
    ...
    private NodeRef getDestination(JSONObject json) {
        ...
        try {
            ...
            throw new AlfrescoRuntimeException(
                    "Error extracting 'destination' from parameter.", error);
        }

        if (StringUtils.isBlank(destinationParam)) {
            throw new WebScriptException(
                    Status.STATUS_BAD_REQUEST, "Please select a record folder.");
        }

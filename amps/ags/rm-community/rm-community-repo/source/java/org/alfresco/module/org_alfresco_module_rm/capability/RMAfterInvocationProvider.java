...
public class RMAfterInvocationProvider extends RMSecurityCommon
    ...
    private ResultSet decide(
        ...
        RMFilteringResultSet filteringResultSet =
                new RMFilteringResultSet(returnedObject, inclusionMask);
        ...
        if ((maxSize == null)
                && (returnedObject.getResultSetMetaData().getSearchParameters().getLimitBy()
                        == LimitBy.FINAL_SIZE)) {
            maxSize =
                    Integer.valueOf(
        ...
        // Allow for skip
        if ((maxSize != null)
                && (returnedObject.getResultSetMetaData().getSearchParameters().getSkipCount()
                        >= 0)) {
            maxSize =
                    Integer.valueOf(
                            maxSize
    ...
    private Collection decide(
        ...
        for (Object nextObject : returnedObject) {
            ...
            // NOTE: for reference - the "maxPermissionChecks" has never been honoured by this loop
            // (since previously the count was not being incremented)
            ...
            for (ConfigAttributeDefintion cad : supportedDefinitions) {
                if (cad.mode.equalsIgnoreCase("FilterNode")) {
                    ...
                    if (cad.parent) {
                        if (StoreRef.class.isAssignableFrom(nextObject.getClass())) {
                            ...
                            testNodeRef =
                                    nodeService
                                            ...
                                            .getParentRef();
                        } else if (ChildAssociationRef.class.isAssignableFrom(
                                nextObject.getClass())) {
                        ...
                        } else if (PermissionCheckValue.class.isAssignableFrom(
                                nextObject.getClass())) {

...
public class RepoStore extends AbstractStore implements TenantDeployer {
                                    ...
                                    return retryingTransactionHelper.doInTransaction(
                                            new RetryingTransactionCallback<NodeRef>() {
                                                public NodeRef execute() throws Exception {
                                                    NodeRef repoStoreRootNodeRef =
                                                            nodeService.getRootNode(repoStore);
                                ...
                                new RetryingTransactionCallback<String[]>() {
                                    public String[] execute() throws Exception {
                                        ...
                                        if (scriptNodeRef != null) {
                                            ...
                                            for (NodeRef nodeRef : nodeRefs) {
                                                if (isContentPresent(nodeRef)) {
                                                    ...
                                                    if (name.startsWith(id)) {
                                                        ...
                                                        String documentPath =
                                                                nodeDir.substring(baseDirLength);
                                            ...
                                            //                            String query = "+PATH:\""
                                            // + repoScriptPath.toPrefixString(namespaceService) +
                                            //                                           "//*\"
                                            // +QNAME:" + lucenifyNamePattern(id) + "*";
                                            //                            ResultSet resultSet =
                                            // searchService.query(repoStore,
                                            // SearchService.LANGUAGE_LUCENE, query);
                                            //                            try
                                            //                            {
                                            //                                documentPaths = new
                                            // ArrayList<String>(resultSet.length());
                                            //                                List<NodeRef> nodes =
                                            // resultSet.getNodeRefs();
                                            //                                for (NodeRef nodeRef :
                                            // nodes)
                                            //                                {
                                            //                                    String name =
                                            ...
                                            // = getPath(nodeRef);
                                            //                                        String
                                            // documentPath = nodeDir.substring(baseDirLength);
                                            //
                                            // documentPaths.add(documentPath);
                                            //                                    }
                                            //                                }
                                        ...
                                        return documentPaths != null
                                                ? documentPaths.toArray(
                                                        new String[documentPaths.size()])
                                                : new String[0];
                                    }
                                },
                                true,
    ...
    /**
     * Helper to encode the elements of a path to be used as a Lucene PATH statement using the
     * ISO9075 encoding. Note that leading and trailing '/' elements will NOT be preserved.
     *
    ...
    /**
     * ALF-7059: Because we can't quote QNAME patterns, and because characters like minus have
     * special meaning, we have to pass the document 'pattern' though the Lucene escaper, preserving
     * the wildcard parts. Also, because you can't search for whitespace in a QNAME, we have to
        ...
        while (tkn.hasMoreTokens()) {
            ...
            if (token.length() == 1) {
                ...
                if (Character.isWhitespace(c)) {
                    // We can't include whitespace in a QNAME expression so we will have to use a
                    ...
                    result.append(SearchLanguageConversion.escapeLuceneQuery(token));
                }
            } else {
                result.append(SearchLanguageConversion.escapeLuceneQuery(token));
            }
    ...
    public long lastModified(final String documentPath) throws IOException {
        return AuthenticationUtil.runAs(
                new AuthenticationUtil.RunAsWork<Long>() {
    ...
    public InputStream getDocument(final String documentPath) throws IOException {
        return AuthenticationUtil.runAs(
                new AuthenticationUtil.RunAsWork<InputStream>() {
                        ...
                        return retryingTransactionHelper.doInTransaction(
                                new RetryingTransactionCallback<InputStream>() {
                                    public InputStream execute() throws Exception {
                                        ...
                                        if (reader == null || !reader.exists()) {
                                            throw new IOException(
                                                    "Failed to read content at "
                                                            + documentPath
                                                            + " (content reader does not exist)");
                                        }
    ...
    public void createDocument(String documentPath, String content) throws IOException {
        ...
        FileInfo fileInfo =
                fileService.create(pathInfo.getNodeRef(), fileName, ContentModel.TYPE_CONTENT);
    ...
    private class RepoTemplateSource {
        ...
        public Reader getReader() throws IOException {
            return AuthenticationUtil.runAs(
                    new AuthenticationUtil.RunAsWork<Reader>() {
                        public Reader doWork() throws Exception {
                            return retryingTransactionHelper.doInTransaction(
                                    new RetryingTransactionCallback<Reader>() {
    ...
    private class RepoScriptLoader implements ScriptLoader {
        ...
        public ScriptContent getScript(final String path) {
            return AuthenticationUtil.runAs(
                    new AuthenticationUtil.RunAsWork<ScriptContent>() {
                        public ScriptContent doWork() throws Exception {
                            return retryingTransactionHelper.doInTransaction(
                                    new RetryingTransactionCallback<ScriptContent>() {

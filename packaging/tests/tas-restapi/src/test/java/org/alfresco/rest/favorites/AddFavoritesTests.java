...
public class AddFavoritesTests extends RestTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify Collaborator user add site to favorites with Rest API and status code"
                            + " is 201")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Contributor user add site to favorites with Rest API and status code is"
                            + " 201")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void contributorIsAbleToAddToFavorites()
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Consumer user add site to favorites with Rest API and status code is"
                            + " 201")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Admin user add site to favorites with Rest API and status code is 201")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Check that if a favorite already exists with the specified id status code is"
                            + " 404")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void addFavoriteTwice() throws Exception {
        SiteModel site =
                dataSite.usingUser(usersWithRoles.getOneUserWithRole(UserRole.SiteManager))
                        .createPublicRandomSite();
        restClient
                .authenticateUser(adminUserModel)
                .withCoreAPI()
                .usingAuthUser()
                .addSiteToFavorites(site);
        restClient.assertStatusCodeIs(HttpStatus.CREATED);
    ...
    public void managerIsAbleToAddFileToFavorites() throws Exception {
        restPersonFavoritesModel =
                restClient
                        ...
                        .withCoreAPI()
                        .usingAuthUser()
                        .addFileToFavorites(document);
        restClient.assertStatusCodeIs(HttpStatus.CREATED);
        restPersonFavoritesModel
                .assertThat()
                .field("targetGuid")
                .is(document.getNodeRefWithoutVersion());
    ...
    public void managerIsAbleToAddFolderToFavorites() throws Exception {
        ...
        restPersonFavoritesModel
                .assertThat()
                .field("targetGuid")
                .is(folder.getNodeRefWithoutVersion());
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify add favorite specifying -me- string in place of <personid> for request")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void userIsAbleToAddFavoriteWhenUsingMeAsUsername() throws Exception {
        ...
        restClient
                ...
                .getFavoriteSites()
                .assertThat()
                .entriesListContains("guid", siteModel.getGuid());
    }
    ...
    public void consumerIsAbleToAddFolderToFavorites() throws Exception {
        restPersonFavoritesModel =
                restClient
                        .authenticateUser(usersWithRoles.getOneUserWithRole(UserRole.SiteConsumer))
                        .withCoreAPI()
                        .usingUser(usersWithRoles.getOneUserWithRole(UserRole.SiteConsumer))
                        .addFolderToFavorites(folder);
        restClient.assertStatusCodeIs(HttpStatus.CREATED);
        restPersonFavoritesModel
                .assertThat()
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Check that if target guid does not describe a site, file, or folder status"
                            + " code is 400")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void addFavoriteUsingInvalidGuid() throws Exception {
        ...
        restClient
                ...
                .containsSummary(
                        String.format(
                                RestErrorModel.ENTITY_NOT_FOUND, site.getGuid().split("/")[3]));
    }
    ...
    public void addFavoriteUsingInexistentUser() throws Exception {
        restClient
                .authenticateUser(adminUserModel)
                .withCoreAPI()
                .usingUser(new UserModel("random_user", "random_password"))
                .addSiteToFavorites(siteModel);
        restClient
                .assertStatusCodeIs(HttpStatus.NOT_FOUND)
                .assertLastError()
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Check that if user provides file in target but guid is of a folder status code"
                            + " is 404")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
        ...
        restClient
                ...
                .containsSummary(
                        String.format(
                                ...
                                folder.getNodeRefWithoutVersion()));
    ...
    //    @Bug(id = "MNT-17158", description="Not a bug, If we need to stop comments from being
    // favorited then an improvement/enhancement request should be raised against the platform.")
    @TestRail(
            section = {TestGroup.REST_API, TestGroup.FAVORITES},
            executionType = ExecutionType.REGRESSION,
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Check that if user provides site in target but id is of a file status code is"
                            + " 404")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
        ...
        restClient
                ...
                .containsSummary(
                        String.format(
                                ...
                                document.getNodeRefWithoutVersion()));
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Check that if user provides folder in target but guid is of a file status code"
                            + " is 404")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void addFolderToFavoritesUsingFileGuid() throws Exception {
        ...
        restClient
                ...
                .withCoreAPI()
                .usingAuthUser()
                .addFolderToFavorites(folder1);
        restClient
                .assertStatusCodeIs(HttpStatus.NOT_FOUND)
                .assertLastError()
                .containsSummary(document1.getNodeRef());
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Check that if user provides file in target but guid is of a site status code"
                            + " is 404")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void addSiteToFavoritesUsingFolderId() throws Exception {
        ...
        restClient
                ...
                .withCoreAPI()
                .usingAuthUser()
                .addSiteToFavorites(newSiteModel);
        restClient
                .assertStatusCodeIs(HttpStatus.NOT_FOUND)
                .assertLastError()
                .containsSummary(
                        String.format(RestErrorModel.ENTITY_NOT_FOUND, folder.getNodeRef()));
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Check that if user provides folder in target but guid is of a site status code"
                            + " is 404")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void addFolderToFavoritesUsingSiteId() throws Exception {
        FolderModel newFolder =
                dataContent.usingSite(siteModel).usingUser(adminUserModel).createFolder();
        ...
        restClient
                ...
                .withCoreAPI()
                .usingAuthUser()
                .addFolderToFavorites(newFolder);
        restClient
                .assertStatusCodeIs(HttpStatus.NOT_FOUND)
                .assertLastError()
                .containsSummary(
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Check that if user does not have permission to favorite a site status code is"
                            + " 404")
    @Test(groups = {TestGroup.REST_API, TestGroup.FAVORITES, TestGroup.REGRESSION})
    public void verifyFavoriteASiteIfTheUserDoesNotHavePermission() throws Exception {
        ...
        restClient
                ...
                .withCoreAPI()
                .usingAuthUser()
                .addSiteToFavorites(privateSite);
        restClient
                .assertStatusCodeIs(HttpStatus.NOT_FOUND)
                .assertLastError()
                .containsSummary(
                        String.format(RestErrorModel.ENTITY_NOT_FOUND, privateSite.getGuid()))

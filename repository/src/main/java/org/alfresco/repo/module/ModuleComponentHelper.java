...
public class ModuleComponentHelper {
  ...
  public static final String URI_MODULES_1_0 =
    "http://www.alfresco.org/system/modules/1.0";
  private static final String REGISTRY_PATH_MODULES = "modules";
  private static final String REGISTRY_PROPERTY_INSTALLED_VERSION =
    "installedVersion";
  private static final String REGISTRY_PROPERTY_CURRENT_VERSION =
    "currentVersion";
  private static final String REGISTRY_PATH_COMPONENTS = "components";
  private static final String REGISTRY_PROPERTY_EXECUTION_DATE =
    "executionDate";
  ...
  private static final String WARN_NO_INSTALL_VERSION =
    "module.warn.no_install_version";
  private static final String ERR_MISSING_DEPENDENCY =
    "module.err.missing_dependency";
  private static final String ERR_UNSUPPORTED_REPO_VERSION =
    "module.err.unsupported_repo_version";
  private static final String ERR_NO_DOWNGRADE =
  ...
  private static final String ERR_ORPHANED_COMPONENTS =
    "module.err.orphaned_components";
  ...
  public ModuleComponentHelper() {
    componentsByNameByModule =
      new HashMap<String, Map<String, ModuleComponent>>(7);
  }
  ...
  public synchronized void registerComponent(ModuleComponent component) {
    ...
    Map<String, ModuleComponent> componentsByName = componentsByNameByModule.get(
      moduleId
    );
   ...
   */
  private synchronized Map<String, ModuleComponent> getComponents(
    String moduleId
  ) {
    Map<String, ModuleComponent> componentsByName = componentsByNameByModule.get(
      moduleId
      ...
      new RunAsWork<Object>() {
        public Object doWork() throws Exception {
          try {
            ...
            final Map<String, Set<ModuleComponent>> mapExecutedComponents = new HashMap<String, Set<ModuleComponent>>(
              1
            );
            final Map<String, Set<String>> mapStartedModules = new HashMap<String, Set<String>>(
              1
            );
                      ...
                      TenantUtil.runAsSystemTenant(
                        new TenantRunAsWork<Object>() {
                          public Object doWork() throws Exception {
                            startModule(
                              module,
                              mapStartedModules.get(tenantDomain),
                              mapExecutedComponents.get(tenantDomain)
                            );
                            return null;
                          }
                        },
      ...
      new RunAsWork<Object>() {
        public Object doWork() throws Exception {
          ...
          for (ModuleDetails module : modules) {
            Map<String, ModuleComponent> components = getComponents(
              module.getId()
            );
   ...
   */
  private void checkForOrphanComponents(
    Set<ModuleComponent> executedComponents
  ) {
    Set<ModuleComponent> missedComponents = new HashSet<ModuleComponent>(
      executedComponents
    );
    ...
    if (missedComponents.size() > 0) {
      throw AlfrescoRuntimeException.create(
        ERR_ORPHANED_COMPONENTS,
        missedComponents.size()
      );
    }
  }
  ...
  ModuleVersionNumber getVersion(String moduleId) {
    ...
    Serializable versionCurrent = registryService.getProperty(
      moduleKeyCurrentVersion
    );
  ...
  private void checkForMissingModules() {
    ...
    for (String moduleId : moduleIds) {
      ...
      if (moduleDetails != null) {
        ...
        loggerService.warn(
          I18NUtil.getMessage(MSG_MISSING, moduleId, versionCurrent)
        );
      }
    }
  }
  ...
  private void renameModule(ModuleDetails module) {
    ...
    Collection<String> registeredModuleIds = registryService.getChildElements(
      moduleKeyAllIds
    );
  ...
  private void startModule(
    ...
    for (ModuleDependency moduleDependency : moduleDependencies) {
      ...
      ModuleDetails moduleDependencyDetails = moduleService.getModule(
        moduleDependencyId
      );
    ...
    VersionNumber repoVersionNumber = descriptorService
      .getServerDescriptor()
      .getVersionNumber();
    ...
    if (
      (
        minRepoVersionNumber != null &&
        repoVersionNumber.compareTo(minRepoVersionNumber) < 0
      ) ||
      (
    ...
    Serializable moduleInstallVersion = registryService.getProperty(
      moduleKeyInstalledVersion
    );
    Serializable moduleCurrentVersion = registryService.getProperty(
      moduleKeyCurrentVersion
    );
    String msg = null;
    if (moduleCurrentVersion == null) { // No previous record of it
      ...
      ModuleVersionNumber currentModuleVersion = getModuleVersionNumber(
  ...
  protected ModuleVersionNumber getModuleVersionNumber(
    Serializable moduleVersion
  ) {
    if (
      moduleVersion instanceof ModuleVersionNumber
    ) return (ModuleVersionNumber) moduleVersion;
    if (moduleVersion instanceof VersionNumber) return new ModuleVersionNumber(
      (VersionNumber) moduleVersion
    );
    if (moduleVersion instanceof String) return new ModuleVersionNumber(
  ...
  private void executeComponent(
    ...
    if (
      ...
      currentVersion.compareTo(maxVersion) > 0
    ) {

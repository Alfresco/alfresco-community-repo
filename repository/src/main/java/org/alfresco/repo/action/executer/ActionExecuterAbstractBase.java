 ...
 */
@AlfrescoPublicApi
public abstract class ActionExecuterAbstractBase extends ParameterizedItemAbstractBase
    ...
    /**
     * Set whether the basic action definition requires status tracking. This can be overridden on
     * each action instance but if not, it falls back to this definition.
     *
     * <p>Setting this to <tt>true</tt> introduces performance problems for concurrently-executing
     * rules on V3.4: <a href="https://issues.alfresco.com/jira/browse/ALF-7341">ALF-7341</a>. It
     * should only be used for long, seldom-run actions.
     *
     ...
     * @return Returns <tt>true</tt> if the node is in the list of {@link
     *     #setApplicableTypes(String[]) applicable types} or one of the subtypes
     */
    ...
    public ActionDefinition getActionDefinition() {
        if (this.actionDefinition == null) {
            ...
            ((ActionDefinitionImpl) this.actionDefinition)
    ...
    /**
     * This method returns an instance of an ActionDefinition implementation class. By default this
     * will be an {@link ActionDefinitionImpl}, but this could be overridden.
     */
    ...
    public void execute(Action action, NodeRef actionedUponNodeRef) {
        if (AlfrescoTransactionSupport.getTransactionReadState() == TxnReadState.TXN_NONE) {
            throw new IllegalStateException(
                    "Actions invariably access the repository.  Doing so without a transaction is"
                            + " not recommended.");
        }
        ...
        // Only execute the action if this action is read only or the actioned upon node reference
        // doesn't
            ...
            if (logger.isWarnEnabled() == true) {
                logger.warn(
                        "Action ("
                                ...
                                + ") ignored because actioned upon node ("
                                + actionedUponNodeRef
                                + ") is locked.");
            }
        }
    ...
    /**
     * Set the queueName which will execute this action if blank or null then the action will be
     * executed on the "default" queue
     *

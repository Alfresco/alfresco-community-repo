...
public class LockMethod extends WebDAVMethod {
    ...
    protected void parseRequestHeaders() throws WebDAVServerException {
        ...
        // According to the specification: "Values other than 0 or infinity MUST NOT be used with
        // the Depth header on a LOCK method.".
        // The specification does not specify the error code for this case - so we use
    ...
    protected void parseRequestBody() throws WebDAVServerException {
        ...
        if (body != null) {
            OUTER:
            for (Node currentNode = body.getDocumentElement().getFirstChild();
                    currentNode != null;
                    currentNode = currentNode.getNextSibling()) {
                if (currentNode instanceof Element
                        && WebDAV.DEFAULT_NAMESPACE_URI.equals(
                                ((Element) currentNode).getNamespaceURI())
                        && WebDAV.XML_LOCK_SCOPE.equals(((Element) currentNode).getLocalName())) {
                    for (Node propertiesNode = currentNode.getFirstChild();
                            propertiesNode != null;
                            propertiesNode = propertiesNode.getNextSibling()) {
                        if (propertiesNode instanceof Element
                                && WebDAV.DEFAULT_NAMESPACE_URI.equals(
                                        ((Element) propertiesNode).getNamespaceURI())) {
                            this.createExclusive =
                                    WebDAV.XML_EXCLUSIVE.equals(propertiesNode.getLocalName());
                            break OUTER;
                        }
                    }
                    break OUTER;
    ...
    protected void attemptLock() throws WebDAVServerException, Exception {
        ...
        try {
            ...
            if (m_conditions != null) {
                ...
                for (Condition condition : m_conditions) {
                    ...
                    if (m_request.getContentLength() == -1
                        ...
                        // LOCK method with If header and without body was sent, according to RFC
                        // 2518 section 7.8
                        // this form of LOCK MUST only be used to "refresh" a lock. However resource
                        // doesn't exist ->
                        // so there is nothing to refresh. Return 403 Forbidden as original
                        // SharePoint Server.
            ...
            // create the file
            lockNodeInfo =
                    createNode(dirInfo.getNodeRef(), splitPath[1], ContentModel.TYPE_CONTENT);

            // ALF-10309 fix, mark created node with webdavNoContent aspect, we assume that save
            // operation
            // is performed by client, webdavNoContent aspect normally removed in put method unless
            // there
            ...
            if (lockTimeout > 0
                ...
                // Remove node after the timeout (MS Office 2003 requests 3 minutes) if the PUT or
                // UNLOCK has not taken place
                timer.schedule(
                        new TimerTask() {
                            @Override
                            public void run() {
                                        ...
                                        new RunAsWork<Void>() {
                                            ...
                                            public Void doWork() throws Exception {
                                                try {
                                                    ...
                                                    // Might get this if the node is deleted. If so
                                                    // just ignore.
                                                    if (logger.isDebugEnabled()) {
                                                        logger.debug(
                                                                "Timer DOES NOT EXIST " + path);
                                                    }
                                                }
                                                return null;
                                            }
        ...
        if (hasLockToken()) {
            ...
            if (!lockInfo.isLocked() && m_request.getContentLength() == -1) {
                // MNT-11990 fix, LOCK method with If header and without body was sent, according to
                // RFC 2518 section 7.8
                // this form of LOCK MUST only be used to "refresh" a lock. But node is not actually
                // locked. Fail this request.
                // see http://www.ics.uci.edu/~ejw/authoring/protocol/rfc2518.html#rfc.section.7.8
                throw new WebDAVServerException(HttpServletResponse.SC_BAD_REQUEST);
            }

            // If a request body is not defined and "If" header is sent we have createExclusive as
            // false,
            // but we need to check a previous LOCK was an exclusive. I.e. get the property for
            // node. It
        ...
        m_response.setHeader(
                ...
                "<" + WebDAV.makeLockToken(lockNodeInfo.getNodeRef(), userName) + ">");
     ...
     * @exception WebDAVServerException
     */
    protected final void createLock(FileInfo lockNode, String userName)
            throws WebDAVServerException {
        // Create Lock token
        ...
        if (createExclusive) {
            ...
            // Shared lock creation should already have been prohibited when parsing the request
            // body
            throw new WebDAVServerException(HttpServletResponse.SC_PRECONDITION_FAILED);
        }
     ...
     * @exception WebDAVServerException
     */
    protected final void refreshLock(FileInfo lockNode, String userName)
            throws WebDAVServerException {
    ...
    protected void generateResponse(FileInfo lockNodeInfo, String userName) throws Exception {
        ...
        xml.startElement(
                ...
                WebDAV.XML_NS_PROP + nsdec,
                getDAVHelper().getNullAttributes());

        // Output the lock details
        generateLockDiscoveryXML(
                xml, lockNodeInfo, false, scope, WebDAV.getDepthName(m_depth), lt, owner, expiry);

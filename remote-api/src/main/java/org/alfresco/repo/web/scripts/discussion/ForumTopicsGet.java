...
public class ForumTopicsGet extends AbstractDiscussionWebScript {
    ...
    protected Map<String, Object> executeImpl(
        ...
        if (tagSearch) {
            ...
            if (site != null) {
                topics =
                        discussionService.findTopics(
                                site.getShortName(), null, tag, oldestTopicsFirst, paging);
            } else {
                topics =
                        discussionService.findTopics(nodeRef, null, tag, oldestTopicsFirst, paging);
            }
        } else {
            if (site != null) {
                topics =
                        discussionService.listTopics(
                                site.getShortName(), oldestTopicsFirst, paging);
            } else {
                topics =
                        discussionService.listTopics(
                                nodeRef, oldestTopicsFirst, buildPagingRequest(req));
            }
        }

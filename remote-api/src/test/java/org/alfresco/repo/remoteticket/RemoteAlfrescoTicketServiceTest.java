...
/**
 * Tests for {@link RemoteAlfrescoTicketServiceImpl}, which work by looping back to the local repo.
 * Because this tests talks to local webscripts, it needs to be run in the Remote API package
 *
 * <p>TODO Test OAuth support, once added
 *
 * @author Nick Burch
 * @since 4.0.2
 */
...
public class RemoteAlfrescoTicketServiceTest extends BaseWebScriptTest {
    ...
    protected void setUp() throws Exception {
        ...
        this.remoteAlfrescoTicketService =
                (RemoteAlfrescoTicketService)
                        getServer().getApplicationContext().getBean("remoteAlfrescoTicketService");
        this.remoteCredentialsService =
                (RemoteCredentialsService)
                        getServer().getApplicationContext().getBean("RemoteCredentialsService");
        this.ticketsCache =
        ...
        remoteAlfrescoTicketService.registerRemoteSystem(
                TEST_REMOTE_SYSTEM_ID, "http://localhost:8080/alfresco/service/", null);
    ...
    public void testGetStoreGetCredentials() throws Exception {
        ...
        try {
            remoteAlfrescoTicketService.storeRemoteCredentials(
                    INVALID_REMOTE_SYSTEM_ID, null, null);
            fail("Shouldn't work for an invalid system");
        } catch (NoSuchSystemException e) {
        }
        ...
        try {
            remoteAlfrescoTicketService.storeRemoteCredentials(
                    TEST_REMOTE_SYSTEM_ID, USER_ONE, "invalid");
        ...
        try {
            remoteAlfrescoTicketService.storeRemoteCredentials(
                    TEST_REMOTE_SYSTEM_ID, "thisUSERdoesNOTexist", "invalid");
            fail("Credentials invalid, shouldn't be allowed");
        } catch (AuthenticationException e) {
        }
        ...
        credentials =
                remoteAlfrescoTicketService.storeRemoteCredentials(
                        TEST_REMOTE_SYSTEM_ID, USER_ONE, PASSWORD);
        ...
        credentials =
                remoteAlfrescoTicketService.storeRemoteCredentials(
                        TEST_REMOTE_SYSTEM_ID, USER_TWO, PASSWORD);
        assertNotNull(credentials);
        assertEquals(TEST_REMOTE_SYSTEM_ID, credentials.getRemoteSystemName());
        ...
        boolean deleted =
                remoteAlfrescoTicketService.deleteRemoteCredentials(TEST_REMOTE_SYSTEM_ID);
    ...
    public void testGetTicket() throws Exception {
        ...
        RemoteAlfrescoTicketInfo ticket5 =
                remoteAlfrescoTicketService.getAlfrescoTicket(TEST_REMOTE_SYSTEM_ID);
        ...
        PasswordCredentialsInfoImpl creds =
                (PasswordCredentialsInfoImpl)
                        remoteCredentialsService.getPersonCredentials(TEST_REMOTE_SYSTEM_ID);
        assertNotNull(creds);
        ...
        RemoteAlfrescoTicketInfo ticket6 =
                remoteAlfrescoTicketService.getAlfrescoTicket(TEST_REMOTE_SYSTEM_ID);
        ...
        RemoteAlfrescoTicketInfo ticket7 =
                remoteAlfrescoTicketService.getAlfrescoTicket(TEST_REMOTE_SYSTEM_ID);
        ...
        retryingTransactionHelper.doInTransaction(
                ...
                false,
                true); // after MNT-13871, POST api/login webscript now requires read-write
        // transaction

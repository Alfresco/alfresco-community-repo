...
/**
 * Centralises access to shared link (public "quick share") services and maps between
 * representations.
 *
 * @author janv
...
public class QuickShareLinksImpl
        implements QuickShareLinks, RecognizedParamsExtractor, InitializingBean {
    ...
    private static final List<QName> EXCLUDED_PROPS =
            Arrays.asList(
                    QuickShareModel.PROP_QSHARE_SHAREDBY, QuickShareModel.PROP_QSHARE_SHAREDID);
    ...
    public QuickShareLink readById(final String sharedId, final Parameters parameters) {
        ...
        try {
            ...
            return TenantUtil.runAsSystemTenant(
                    new TenantUtil.TenantRunAsWork<QuickShareLink>() {
                        public QuickShareLink doWork() throws Exception {
                            // note: assume noAuth here (rather than rely on getRunAsUser which will
                            // be null in non-MT)
    ...
    public BinaryResource readProperty(
            String sharedId, final String renditionId, final Parameters parameters)
            throws EntityNotFoundException {
        checkEnabled();
    ...
    public List<QuickShareLink> create(List<QuickShareLink> nodeIds, Parameters parameters) {
        ...
        List<String> includeParam =
                parameters != null ? parameters.getInclude() : Collections.<String>emptyList();
            ...
            try {
                ...
                if (sharedId != null) {
                    throw new ConstraintViolatedException(
                            "sharedId already exists: " + nodeId + " [" + sharedId + "]");
                }

                // Note: will throw AccessDeniedException (=> 403) via QuickShareService (when
                // NodeService tries to getAspects)
                // Note: since we already check node exists above, we can assume that
                // InvalidNodeRefException (=> 404) here means not content (see type check)
                try {
                    QuickShareDTO qsDto =
                            quickShareService.shareContent(nodeRef, qs.getExpiresAt());
                    result.add(getQuickShareInfo(qsDto.getId(), false, includeParam));
                } catch (InvalidNodeRefException inre) {
                    throw new InvalidArgumentException(
                            "Unable to create shared link to non-file content: " + nodeId);
                } catch (QuickShareLinkExpiryActionException ex) {
                    throw new InvalidArgumentException(ex.getMessage());
                }
            } catch (AccessDeniedException ade) {
                throw new PermissionDeniedException(
                        "Unable to create shared link to node that does not exist: " + nodeId);
            } catch (InvalidNodeRefException inre) {
    ...
    private Parameters getParamsWithCreatedStatus() {
        String filterStatusCreated =
                "(" + Renditions.PARAM_STATUS + "='" + Rendition.RenditionStatus.CREATED + "')";
        Query whereQuery = getWhereClause(filterStatusCreated);
        Params.RecognizedParams recParams =
                new Params.RecognizedParams(
                        null, null, null, null, null, null, whereQuery, null, false);
    ...
    private static final Set<String> FIND_SHARED_LINKS_QUERY_PROPERTIES =
    ...
    public CollectionWithPagingInfo<QuickShareLink> findLinks(Parameters parameters) {
        ...
        // note: REST API query parameter (ie. where clause filter) - not to be confused with search
        // query ;-)
        Query q = parameters.getQuery();
        if (q != null) {
            // filtering via "where" clause
            MapBasedQueryWalker propertyWalker =
                    new MapBasedQueryWalker(FIND_SHARED_LINKS_QUERY_PROPERTIES, null);
            QueryHelper.walk(q, propertyWalker);

            String sharedByUserId =
                    propertyWalker.getProperty(
                            PARAM_SHAREDBY, WhereClauseParser.EQUALS, String.class);
            ...
            if (sharedByUserId != null) {
                ...
                String qsharedBy =
                        QuickShareModel.PROP_QSHARE_SHAREDBY.toPrefixString(namespaceService);
                queryString +=
                        " +@"
                                + SearchLanguageConversion.escapeLuceneQuery(qsharedBy)
                                + ":\"${"
                                + qsharedBy
                                + "}\"";
            }
        ...
        return CollectionWithPagingInfo.asPaged(
                ...
                qsLinks,
                results.hasMore(),
                Long.valueOf(results.getNumberFound()).intValue());
    }

    private QuickShareLink getQuickShareInfo(
            String sharedId, boolean noAuth, List<String> includeParam) {
        checkValidShareId(sharedId);

        Map<String, Object> map =
                (Map<String, Object>) quickShareService.getMetaData(sharedId).get("item");
    ...
    private QuickShareLink getQuickShareInfo(
            NodeRef nodeRef, boolean noAuth, List<String> includeParam) {
        Map<String, Object> map =
                (Map<String, Object>) quickShareService.getMetaData(nodeRef).get("item");
        return getQuickShareInfo(nodeRef, map, noAuth, includeParam);
    }

    private QuickShareLink getQuickShareInfo(
            NodeRef nodeRef, Map<String, Object> map, boolean noAuth, List<String> includeParam) {
        ...
        try {
            ...
            // note: if noAuth mode then don't return userids (to limit disclosure and be consistent
            // with v0 internal)
            ...
            // TODO review - should we return sharedByUser for authenticated users only ?? (not
            // exposed by V0 but needed for "find")
            String sharedByUserId = (String) nodeProps.get(QuickShareModel.PROP_QSHARE_SHAREDBY);
            UserInfo sharedByUser =
                    Node.lookupUserInfo(
                            sharedByUserId, mapUserInfo, personService, displayNameOnly);
            ...
            // note: if noAuth mode then do not return allowable operations (eg. but can be
            // optionally returned when finding shared links)
            if (!noAuth) {
                // Cached document
                Node doc = null;
                ...
                if (includeParam.contains(PARAM_INCLUDE_PROPERTIES)) {
                    ...
                    // Create a map from node properties excluding properties already in this
                    // QuickShareLink
                    Map<String, Object> filteredNodeProperties =
                            filterProps(doc.getProperties(), EXCLUDED_PROPS);
    ...
    private Node getNode(
            NodeRef nodeRef, List<String> includeParam, Map<String, UserInfo> mapUserInfo) {
    ...
    private void validateEmailRequest(QuickShareLinkEmailRequest emailRequest) {
        ...
        if (emailRequest.getRecipientEmails() == null
                || emailRequest.getRecipientEmails().isEmpty()) {

...
public class UsageDAOImpl extends AbstractUsageDAOImpl {
  ...
  private static final String INSERT_USAGE_DELTA =
    "alfresco.usage.insert.insert_UsageDelta";
  private static final String SELECT_USAGE_DELTA_TOTAL_SIZE_BY_NODE =
    "alfresco.usage.select_GetTotalDeltaSizeByNodeId";
  private static final String SELECT_USAGE_DELTA_NODES =
    "alfresco.usage.select_GetUsageDeltaNodes";
  private static final String SELECT_USERS_WITH_USAGE =
    "alfresco.usage.select_GetUsersWithUsage";
  private static final String SELECT_USERS_WITHOUT_USAGE =
    "alfresco.usage.select_GetUsersWithoutUsage";
  private static final String SELECT_CONTENT_SIZES_NEW =
    "alfresco.usage.select_GetContentSizesForStoreNew";
  private static final String SELECT_CONTENT_SIZE_NEW_USER =
    "alfresco.usage.select_GetContentSizeForStoreForUser";
  private static final String DELETE_USAGE_DELTAS_BY_NODE =
  ...
  @Override
  protected void selectUsersWithoutUsage(
    StoreRef storeRef,
    MapHandler resultsCallback
  ) {
    Pair<Long, ? extends Object> personTypeQNamePair = qnameDAO.getQName(
      ContentModel.TYPE_PERSON
    );
    Pair<Long, ? extends Object> usernamePropQNamePair = qnameDAO.getQName(
      ContentModel.PROP_USERNAME
    );
    Pair<Long, ? extends Object> sizeCurrentPropQNamePair = qnameDAO.getQName(
      ContentModel.PROP_SIZE_CURRENT
    );

    if (
      personTypeQNamePair == null ||
      usernamePropQNamePair == null ||
      sizeCurrentPropQNamePair == null
    ) {
  ...
  @Override
  protected void selectUsersWithUsage(
    StoreRef storeRef,
    MapHandler resultsCallback
  ) {
    Pair<Long, ? extends Object> personTypeQNamePair = qnameDAO.getQName(
  ...
  @Override
  protected void selectUserContentSizesForStore(
    StoreRef storeRef,
    MapHandler resultsCallback
  ) {
    Pair<Long, ? extends Object> contentTypeQNamePair = qnameDAO.getQName(
      ContentModel.TYPE_CONTENT
    );
    Pair<Long, ? extends Object> ownerPropQNamePair = qnameDAO.getQName(
      ContentModel.PROP_OWNER
    );
    Pair<Long, ? extends Object> contentPropQNamePair = qnameDAO.getQName(
      ContentModel.PROP_CONTENT
    );

    if (
      contentTypeQNamePair == null ||
      ownerPropQNamePair == null ||
      contentPropQNamePair == null
    ) {
      return; // The statics have not been used, so there can be no results
    }
  ...
  @Override
  protected Long selectContentSizeForStoreForUser(
    StoreRef storeRef,
    String userName
  ) {
    Pair<Long, ? extends Object> contentTypeQNamePair = qnameDAO.getQName(

...
/**
 * An authentication component that chains across beans in multiple child application contexts
 * corresponding to different 'subsystems' in a chain determined by a {@link
...
public class SubsystemChainingAuthenticationComponent
    ...
    public void setApplicationContextManager(
            ChildApplicationContextManager applicationContextManager) {
        ...
        for (String instance : this.applicationContextManager.getInstanceIds()) {
            try {
                ApplicationContext context =
                        this.applicationContextManager.getApplicationContext(instance);
                AuthenticationComponent authenticationComponent =
                        (AuthenticationComponent) context.getBean(sourceBeanName);
                // Only add active authentication components. E.g. we might have an ldap context
                ...
                // The bean doesn't exist or this subsystem won't start. The reason would have been
                // logged. Ignore and continue.
            }
        }
        return result;
    }
    ...
    protected AuthenticationComponent getAuthenticationComponent(String instanceId) {
        ApplicationContext context =
                this.applicationContextManager.getApplicationContext(instanceId);
        if (context != null) {
            try {
                AuthenticationComponent authenticationComponent =
                        (AuthenticationComponent) context.getBean(sourceBeanName);

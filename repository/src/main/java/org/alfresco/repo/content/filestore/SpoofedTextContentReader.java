 ...
 *
 * <p>The URL has the format: spoofed://{locale=en_GB,seed=12345,length=1024,strings=["Alfresco",
 * "Cloud"]}
 *
 * <p>The lexicon for the given locale is found by taking the language part of the locale (<b>en</b>
 * in <b>en_GB</b>) and finding the resource <b>alfresco/textgen/lexicon-stem-en.txt</b>.
 *
 * @see TextGenerator
 * @author Derek Hulley
 * @since 5.1
...
public class SpoofedTextContentReader extends AbstractContentReader {
    ...
    private static Map<Locale, TextGenerator> textGeneratorsByLocale =
            new HashMap<Locale, TextGenerator>();
    ...
    public static String createContentUrl(Locale locale, long seed, long size, String... words) {
        if (locale == null || size < 0L) {
            throw new IllegalArgumentException(
                    "Locale must be supplied and size must be zero or greater.");
        }
        ...
        try {
            ...
            for (String word : words) {
                if (word == null) {
                    throw new IllegalArgumentException(
                            "Words to inject into the document may not be null.");
                }
                jsonWords.add(word);
            }
            ...
            if (url.length() > 255) {
                throw new IllegalArgumentException(
                        "Content URLs can be up to 255 characters.  Have "
                                + url.length()
                                + " characters: "
                                + url);
            }
            ...
            throw new RuntimeException(
                    "Unable to create content URL using "
                            + locale
                            + ", "
                            + seed
                            + ", "
    ...
    public SpoofedTextContentReader(String url) {
        ...
        if (url.length() > 255) {
            throw new IllegalArgumentException(
                    "A content URL is limited to 255 characters: " + url);
        }
        // Split out data part
        ...
        try {
            ...
            String jsonLocale =
                    mappedData.containsKey(KEY_LOCALE)
                            ? (String) mappedData.get(KEY_LOCALE)
                            : Locale.ENGLISH.toString();
            String jsonSeed =
                    mappedData.containsKey(KEY_SEED) ? (String) mappedData.get(KEY_SEED) : "0";
            String jsonSize =
                    mappedData.containsKey(KEY_SIZE) ? (String) mappedData.get(KEY_SIZE) : "1024";
            JSONArray jsonWords =
                    mappedData.containsKey(KEY_WORDS)
                            ? (JSONArray) mappedData.get(KEY_WORDS)
                            : new JSONArray();

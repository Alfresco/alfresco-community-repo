...
public class AuditWebScriptTest extends BaseWebScriptTest {
    ...
    protected void setUp() throws Exception {
        ...
        // MNT-10807 : Auditing does not take into account
        ...
        AuditModelRegistryImpl auditModelRegistry =
                (AuditModelRegistryImpl) ctx.getBean("auditModel.modelRegistry");
        URL testModelUrl =
    ...
    protected void tearDown() throws Exception {
        ...
        try {
            ...
            throw new RuntimeException(
                    "Failed to set audit back to globally enabled/disabled state", e);
        }
        try {
            if (wasRepoEnabled) {
                auditService.enableAudit(APP_REPOTEST_NAME, APP_REPOTEST_PATH);
            ...
            throw new RuntimeException(
                    "Failed to set repo audit back to enabled/disabled state", e);
        }
        try {
            if (wasSearchEnabled) {
                auditService.enableAudit(APP_SEARCHTEST_NAME, APP_SEARCHTEST_PATH);
            ...
            throw new RuntimeException(
                    "Failed to set search audit back to enabled/disabled state", e);
        }
    }
    ...
    public void testSetAuditEnabledGlobally() throws Exception {
        ...
        if (wasEnabled) {
            ...
            TestWebScriptServer.PostRequest req =
    ...
    public void testGetAuditSearchService() throws Exception {
        ...
        TestWebScriptServer.PostRequest postReq =
                new TestWebScriptServer.PostRequest(url, "", MimetypeMap.MIMETYPE_JSON);
        ...
        NodeRef testFile =
                this.fileFolderService
                        .create(this.testRoot, filename, ContentModel.TYPE_CONTENT)
                        .getNodeRef();
        ...
        JSONObject values =
                (JSONObject)
                        ((JSONObject) jsonEntries.get(0))
                                .get(AbstractAuditWebScript.JSON_KEY_ENTRY_VALUES);
        assertTrue(
    ...
    public void testSetAuditEnabledRepo() throws Exception {
        ...
        if (wasEnabled) {
            String url =
                    "/api/audit/control/" + APP_REPOTEST_NAME + APP_REPOTEST_PATH + "?enable=false";
            TestWebScriptServer.PostRequest req =
                    new TestWebScriptServer.PostRequest(url, "", MimetypeMap.MIMETYPE_JSON);
            sendRequest(req, Status.STATUS_OK, admin);
        } else {
            String url =
                    "/api/audit/control/" + APP_REPOTEST_NAME + APP_REPOTEST_PATH + "?enable=true";
            TestWebScriptServer.PostRequest req =
                    new TestWebScriptServer.PostRequest(url, "", MimetypeMap.MIMETYPE_JSON);
            sendRequest(req, Status.STATUS_OK, admin);
        }

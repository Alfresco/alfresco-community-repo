...
/**
 * Composite property bean to identify a folder in the repository. This uses the {@link
 * #getFolderPath() path} to identify a root and then a folder-name path to identify a folder.
 *
 * @author Derek Hulley
 * @since 3.2
 */
public class RepositoryFolderConfigBean extends RepositoryPathConfigBean {
    ...
    public String toString() {
        ...
        sb.append("Folder Path: ")
                .append(super.getStoreRef())
                .append(super.getRootPath())
                .append("/")
                .append(folderPath);
        return sb.toString();
    }
     ...
     *
     * @param folderPath a folder-name path using the '<b>/</b>' path separator e.g. '<b>IMAP
     *     HOME/MAIL-1</b>'
     */
    public void setFolderPath(String folderPath) {
        if (!PropertyCheck.isValidPropertyString(folderPath)) {
            throw new IllegalArgumentException(
                    "Invalid folder name path for property 'folderPath': " + folderPath);
        }
    ...
    /**
     * Helper method to find the folder path referenced by this bean. The {@link #getFolderPath()
     * path} to the start of the folder path must exist.
     *
     * <p>Authentication and transactions are the client's responsibility.
     *
    ...
    public NodeRef getFolderPath(
        ...
        return getFolderPathImpl(
                namespaceService, nodeService, searchService, fileFolderService, true);
    }
    ...
    public NodeRef getFolderPathOrNull(
            ...
            FileFolderService fileFolderService) {
        return getFolderPathImpl(
                namespaceService, nodeService, searchService, fileFolderService, false);
    }
    ...
    private NodeRef getFolderPathImpl(
        ...
        NodeRef pathStartNodeRef =
                super.resolveNodePath(namespaceService, nodeService, searchService);
        ...
        if (folderPath.length() == 0) {
            ...
            List<NodeRef> nodeRefs =
                    searchService.selectNodes(
                            pathStartNodeRef, folderPath, null, namespaceService, true);
            if (nodeRefs.size() == 0) {
                return getNullOrThrowAlfrescoRuntimeExcpetion(
                        "Folder not found: " + this, throwException);
            } else {
                NodeRef nodeRef = nodeRefs.get(0);
                FileInfo folderInfo = fileFolderService.getFileInfo(nodeRef);
                if (!folderInfo.isFolder()) {
                    return getNullOrThrowAlfrescoRuntimeExcpetion(
                            "Not a folder: " + this, throwException);
                }
                return nodeRef;
            }
        }
    ...
    private NodeRef getNullOrThrowAlfrescoRuntimeExcpetion(
            String exceptionMessage, boolean throwException) {
        if (throwException) {
    ...
    /**
     * Helper method to find or create the folder path referenced by this bean. The {@link
     * #getFolderPath() path} to the start of the folder path must exist. The folder path will be
     * created, if required.
     *
     * <p>Authentication and transactions are the client's responsibility.
     *
            ...
            while (folders.hasMoreTokens()) {
                ...
                List<ChildAssociationRef> children =
                        nodeService.getChildAssocs(
                                nodeRef, RegexQNamePattern.MATCH_ALL, folderQName);
                if (children.isEmpty()) {
                    nodeRef =
                            fileFolderService
                                    .create(
                                            nodeRef,
                                            folderQName.getLocalName(),

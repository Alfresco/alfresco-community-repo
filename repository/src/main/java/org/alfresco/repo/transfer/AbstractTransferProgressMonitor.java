...
public abstract class AbstractTransferProgressMonitor implements TransferProgressMonitor {
    ...
    protected static final String MSG_TRANSFER_NOT_FOUND =
            "transfer_service.receiver.transfer_not_found";
    protected static final String MSG_TRANSFER_CANCELLED =
            "transfer_service.receiver.transfer_cancelled";

    private Map<String, TransferDestinationReportWriter> transferLogWriters =
            new TreeMap<String, TransferDestinationReportWriter>();
    ...
    @Override
    public final void updateStatus(final String transferId, final Status status)
            throws TransferException {
        Status currentStatus = getProgress(transferId).getStatus();

        // If the transfer has already reached a terminal state then we don't allow any further
        // change
        if (!TransferProgress.getTerminalStatuses().contains(currentStatus)) {
            transactionService
                    ...
                    .doInTransaction(
                            new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                                public Void execute() throws Throwable {
                                    TransferDestinationReportWriter writer =
                                            getLogWriter(transferId);
                                    writer.writeChangeState(status.toString());
                                    updateStatusInternal(transferId, status);
                                    // If the transfer has now reached a terminal state then the
                                    // make sure that the log channel is
    ...
    public void logUpdated(
            String transferId, NodeRef sourceNodeRef, NodeRef destNodeRef, String path) {
        TransferDestinationReportWriter writer = getLogWriter(transferId);
    ...
    public void logDeleted(
            String transferId, NodeRef sourceNodeRef, NodeRef destNodeRef, String oldPath) {
        TransferDestinationReportWriter writer = getLogWriter(transferId);

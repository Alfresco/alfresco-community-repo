...
public class AddToHoldsTests extends BaseRMRestTest {
    ...
    private static final String HOLD =
        "HOLD" + generateTestPrefix(AddToHoldsTests.class);
    ...
    public void retrieveTheContentOfTheHoldUsingV1API() throws Exception {
        STEP(
            "Retrieve the list of children from the hold and collect the entries that have the name of the active " +
            "content held"
        );
        List<RestNodeModel> documentsHeld = restClient
            ...
            .stream()
            .filter(child ->
                child.onModel().getName().contains(documentHeld.getName())
            )
        ...
        assertEquals(
            documentsHeld.get(0).onModel().getName(),
            documentHeld.getName()
        );
    }
    ...
    public void retrieveTheHoldWhereTheContentIsAdded() {
        List<HoldEntry> holdEntries = holdsAPI.getHolds(
            getAdminUser().getUsername(),
            getAdminUser().getPassword(),
            documentHeld.getNodeRefWithoutVersion(),
            true,
        ...
        assertTrue(
            holdEntries
                .stream()
                .anyMatch(holdEntry -> holdEntry.getName().contains(HOLD)),
    ...
    public void addValidNodesToHoldWithAllowedUser(String nodeId)
        throws Exception {
    ...
    public Object[][] getUserWithoutPermissionForAddToHold() {
        ...
        UserModel user = roleService.createUserWithRMRoleAndRMNodePermission(
        ...
        getRestAPIFactory()
            .getRMUserAPI()
            .addUserPermission(holdNodeRef, user, PERMISSION_FILING);
        nodesToBeClean.add(recordFolder.getParentId());
    ...
    public void addContentToHoldWithUserWithoutHoldPermission(
        ...
        String response = holdsAPI.addToHoldAndGetMessage(
    ...
    public Object[][] getInvalidNodesForAddToHold() {
        //create locked file
        FileModel contentLocked = dataContent
            .usingAdmin()
            .usingSite(testSite)
            .createContent(CMISUtil.DocumentType.TEXT_PLAIN);
    ...
    public void addInvalidNodesToHold(
        String itemNodeRef,
        int responseCode,
        String errorMessage
    ) throws Exception {
        STEP("Add the node to the hold ");
        String responseErrorMessage = holdsAPI.addToHoldAndGetMessage(
            getAdminUser().getUsername(),
            getAdminUser().getPassword(),
            responseCode,
            itemNodeRef,

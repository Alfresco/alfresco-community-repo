...
public class EmailUserNotifier
  ...
  protected boolean skipUser(NodeRef personNodeRef) {
    ...
    Boolean emailFeedDisabled = (Boolean) personProps.get(
    ...
    if (
      authenticationContext.isSystemUserName(feedUserId) ||
    ...
    if ((emailAddress == null) || (emailAddress.length() <= 0)) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Skip for '" + feedUserId + "' since they have no email address set"
        );
      }
      return true;
    }

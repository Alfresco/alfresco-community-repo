...
public class ActivitiWorkflowServiceIntegrationTest
  ...
  private static final String USER_RECREATED =
    "WFUserRecreated" + GUID.generate();
  public static final String ACTIVITI_TEST_TRANSACTION_BPMN20_XML =
    "activiti/testTransaction.bpmn20.xml";
  public static final String ALFRESCO_WORKFLOW_ADHOC_BPMN20_XML =
    "alfresco/workflow/adhoc.bpmn20.xml";
  public static final String ALFRESCO_WORKFLOW_REVIEW_POOLED_BPMN20_XML =
    "alfresco/workflow/review-pooled.bpmn20.xml";
  public static final String ALFRESCO_WORKFLOW_PARALLEL_REVIEW_BPMN20_XML =
    "alfresco/workflow/parallel-review.bpmn20.xml";
  public static final String ACTIVITI_TEST_TIMER_BPMN20_XML =
    "activiti/testTimer.bpmn20.xml";
  public static final String ACTIVITI_TEST_WITH_SUB_PROCESS_XML =
    "activiti/testWorkflowWithSubprocess.xml";

  @Test
  public void testOutcome() throws Exception {
    WorkflowDefinition definition = deployDefinition(
    ...
    QName outcomePropName = QName.createQName(
    ...
    Serializable outcome = result
      .getProperties()
      .get(WorkflowModel.PROP_OUTCOME);
  ...
  public void testPriorityIsValid() {
    WorkflowDefinition definition = deployDefinition(
      "activiti/testCustomActiviti.bpmn20.xml"
    );
    ...
    for (WorkflowTask workflowTask : tasks) {
      ...
      TypeDefinition typeDefinition = workflowTask
        .getDefinition()
        .getMetadata();
      Map<QName, PropertyDefinition> propertyDefs = typeDefinition.getProperties();
      PropertyDefinition priorDef = propertyDefs.get(
        WorkflowModel.PROP_PRIORITY
      );
      assertEquals(
        props.get(WorkflowModel.PROP_PRIORITY),
        Integer.valueOf(priorDef.getDefaultValue())
      );
    }
  }
  ...
  public void testReviewAndPooledNotModifiedDate() {
    ...
    final ChildAssociationRef childAssoc = nodeService.createNode(
      companyHome,
    ...
    Date lastModifiedDate = (Date) nodeService.getProperty(
      addedNodeRef,
    ...
    nodeService.addChild(
      ...
      QName.createQName(
        ...
        QName.createValidLocalName(
          (String) nodeService.getProperty(addedNodeRef, ContentModel.PROP_NAME)
        )
      )
    );

    WorkflowPath workflowPath = workflowService.startWorkflow(
      definition.getId(),
      params
    );
    ...
    assertEquals(
      lastModifiedDate,
      nodeService.getProperty(addedNodeRef, ContentModel.PROP_MODIFIED)
    );
  }
  ...
  public void testCompletedTaskInWorkflowWithSubProcess() {
    WorkflowDefinition definition = deployDefinition(
      ACTIVITI_TEST_WITH_SUB_PROCESS_XML
    );
    String workflowDefId = definition.getId();
    List<WorkflowTaskDefinition> taskDefs = workflowService.getTaskDefinitions(
      workflowDefId
  ...
  public void testAccessStartTaskAsAssigneeFromTaskPartOfProcess() {
    ...
    List<NodeRef> taskPackage = workflowService.getPackageContents(
      startTask.getId()
    );
    assertTrue(taskPackage.isEmpty());
    List<NodeRef> packageContents = workflowService.getPackageContents(
      wfPackage
    );
    ...
    workflowService.updateTask(
      ...
      Collections.singletonMap(
        ...
        (Serializable) personManager.get(USER3)
      ),
      null,
      null
    );
  ...
  public void testMultiInstanceListenersCalled() throws Exception {
    ...
    assertEquals(
      "This is the description",
      tasks.get(0).getProperties().get(WorkflowModel.PROP_DESCRIPTION)
    );
  }
  ...
  public void testLongTextValues() throws Exception {
    ...
    WorkflowTask startTask = workflowService.getStartTask(
      path.getInstance().getId()
    );
    assertNotNull(startTask);

    assertEquals(
      veryLongTextValue,
      startTask.getProperties().get(WorkflowModel.PROP_COMMENT)
    );
  }
  ...
  public void testAssignmentListener() {
    WorkflowDefinition definition = deployDefinition(
      getAssignmentListenerDefinitionPath()
    );
    Map<QName, Serializable> params = new HashMap<QName, Serializable>();
    ...
    Double create1 = (Double) props.get(
      QName.createQName("http://www.alfresco.org/model/bpm/1.0", "create1")
    );
    assertEquals("Create listener was not triggered", new Double(1), create1);
    Double complete1 = (Double) props.get(
      QName.createQName("http://www.alfresco.org/model/bpm/1.0", "complete1")
    );
    assertEquals(
      "Complete listener was not triggered",
      new Double(1),
      complete1
    );
    Double assignment1 = (Double) props.get(
      QName.createQName("http://www.alfresco.org/model/bpm/1.0", "assignment1")
    );
    assertEquals(
      "Assign listener was not triggered",
      new Double(1),
      assignment1
    );
  }
  ...
  public void testStartWorkflowFromTaskListener() throws Exception {
    WorkflowDefinition testDefinition = deployDefinition(
      "activiti/testStartWfFromListener.bpmn20.xml"
    );
  ...
  public void testWorkflowWithNodes() throws Exception {
    ...
    FileInfo incorrectNode = serviceRegistry
      .getFileFolderService()
      .create(
        companyHome,
        "NO_WORKFLOW" + GUID.generate() + ".xml",
        ContentModel.TYPE_CONTENT
      );
    FileInfo fileInfo = serviceRegistry
      .getFileFolderService()
      .create(
        companyHome,
        "workflow" + GUID.generate() + ".xml",
    ...
    ContentWriter contentWriter = serviceRegistry
      .getContentService()
      .getWriter(fileInfo.getNodeRef(), ContentModel.TYPE_CONTENT, true);
    ...
    try {
      ...
      assertTrue(
        we.getMessage().contains(" is not of type 'bpm:workflowDefinition'")
      );
    }
    ...
    WorkflowDeployment workflowDeployment = workflowService.deployDefinition(
      workflowNode
    );
    assertNotNull(workflowDeployment);
    assertTrue(workflowService.isDefinitionDeployed(workflowNode));

    List<WorkflowDefinition> defs = workflowService.getAllDefinitionsByName(
      workflowDeployment.getDefinition().getName()
    );
    assertNotNull(defs);
    assertEquals(1, defs.size());

    workflowService.undeployDefinition(
      workflowDeployment.getDefinition().getId()
    );
    assertFalse(workflowService.isDefinitionDeployed(workflowNode));
  ...
  public void testWorkflowVarious() throws Exception {
    ...
    List<WorkflowTask> workflowTasks = workflowService.getStartTasks(
    ...
    try {
      ...
      assertTrue(
        we.getMessage().contains("Failed to retrieve workflow definition")
      );
    }
  }
  ...
  public void testWorkflowQueries() throws Exception {
    ...
    WorkflowTaskQuery workflowTaskQuery = createWorkflowTaskQuery(
      WorkflowTaskState.COMPLETED
    );
  ...
  public void testBuildWorkflowWithNoUserTasks() throws Exception {
    ...
    WorkflowDefinition testDefinition = deployDefinition(
      "activiti/testWorkflowNoUserTasks.bpmn20.xml"
    );
    ...
    List<WorkflowInstance> activeInstances = workflowService.getActiveWorkflows(
      testDefinition.getId()
    );
  ...
  public void testNonAdminCannotDeployWorkflowBySwitchingNodeType() {
    ...
    NodeRef home = (NodeRef) nodeService.getProperty(
      person,
  ...
  public void testNonAdminCannotDeployWorkflowBySwitchingNodeTypeEvenInCorrectLocation() {
    ...
    try {
      ...
      fail(
        "User should not be able to create a node in the 'correct location'."
      );
  ...
  private NodeRef findWorkflowParent() {
    RepositoryLocation workflowLocation = (RepositoryLocation) applicationContext.getBean(
      "customWorkflowDefsRepositoryLocation"
    );
    NodeRef rootNode = nodeService.getRootNode(workflowLocation.getStoreRef());
    List<NodeRef> workflowParents = serviceRegistry
      .getSearchService()
      .selectNodes(
        rootNode,
    ...
    if (workflowParents.size() == 0) {
      throw new IllegalStateException(
        "Unable to find workflow location: " + workflowLocation.getPath()
      );
    }
    if (workflowParents.size() > 1) {
      throw new IllegalStateException(
        "More than one workflow location? [" + workflowLocation.getPath() + "]"
      );
    }
   ...
   */
  private WorkflowDefinition createContentAndSwitchToWorkflow(
    String processName,
    String resource,
    NodeRef parent
  ) {
    ...
    ContentWriter writer = serviceRegistry
      .getContentService()
      .getWriter(workflowNode, ContentModel.PROP_CONTENT, true);

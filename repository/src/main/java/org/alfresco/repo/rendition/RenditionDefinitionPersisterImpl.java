...
public class RenditionDefinitionPersisterImpl
  ...
  private static final Log log = LogFactory.getLog(
    RenditionDefinitionPersisterImpl.class
  );
  ...
  protected static final NodeRef RENDERING_ACTION_ROOT_NODE_REF = new NodeRef(
    SPACES_STORE,
    "rendering_actions_space"
  );
  ...
  public List<RenditionDefinition> loadRenditionDefinitions() {
    ...
    List<RenditionDefinition> renderingActions = new ArrayList<RenditionDefinition>(
      childAssocs.size()
    );
  ...
  public List<RenditionDefinition> loadRenditionDefinitions(
    String renditionEngineName
  ) {
    if (renditionEngineName == null) {
      throw new NullPointerException("Unexpected null renditionEngineName");
    }

    List<RenditionDefinition> allRenditionDefinitions =
      this.loadRenditionDefinitions();
  ...
  public RenditionDefinition loadRenditionDefinition(
  ...
  private void checkRenderingActionRootNodeExists() {
    if (nodeService.exists(RENDERING_ACTION_ROOT_NODE_REF) == false) {
      throw new RenditionServiceException(
        "Unable to find rendering action root node."
      );
    }
  }
}

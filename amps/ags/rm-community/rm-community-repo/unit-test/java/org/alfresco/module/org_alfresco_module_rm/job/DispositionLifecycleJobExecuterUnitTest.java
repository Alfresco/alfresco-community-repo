...
public class DispositionLifecycleJobExecuterUnitTest extends BaseUnitTest {
  ...
  public void before() throws Exception {
    ...
    // setup interactions
    doReturn(mockedResultSet)
      .when(mockedSearchService)
      .query(any(SearchParameters.class));
  ...
  private void verifyQueryTimes(int numberOfInvocation) {
    ArgumentCaptor<SearchParameters> paramsCaptor = ArgumentCaptor.forClass(
      SearchParameters.class
    );
    verify(mockedSearchService, times(numberOfInvocation))
      .query(paramsCaptor.capture());
  ...
  public void noResultsInQuery() {
    ...
    verifyZeroInteractions(
      mockedNodeService,
      mockedRecordFolderService,
      mockedRetryingTransactionHelper
    );
  }
  ...
  public void dispositionActionProcessed() {
    ...
    doReturn(RETAIN)
      .when(mockedNodeService)
      .getProperty(node2, RecordsManagementModel.PROP_DISPOSITION_ACTION);
    doReturn(parentAssoc)
      .when(mockedNodeService)
      .getPrimaryParent(any(NodeRef.class));
    doReturn(false)
      .when(mockedRecordFolderService)
      .isRecordFolder(parentAssoc.getParentRef());
    doReturn(true)
      .when(mockedRecordService)
      .isRecord(parentAssoc.getParentRef());
    doReturn(false)
      .when(mockedFreezeService)
      .isFrozen(parentAssoc.getParentRef());
  ...
  public void testGetQuery() {
    ...
    String expected =
      "TYPE:\"rma:dispositionAction\" AND " +
  ...
  public void testPagination() {
    ...
    when(mockedSearchService.query(any(SearchParameters.class)))
      .thenAnswer(
        (Answer<ResultSet>) invocation -> {
          SearchParameters params = invocation.getArgumentAt(
            0,
            SearchParameters.class
          );
  ...
  public void testInvalidBatchSize() {
    ...
    assertEquals(
      executer.DEFAULT_BATCH_SIZE,
      paramsCaptor.getValue().getMaxItems()
    );

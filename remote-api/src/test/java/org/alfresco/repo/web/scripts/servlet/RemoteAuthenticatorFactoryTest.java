...
public class RemoteAuthenticatorFactoryTest {
  ...
  public static void beforeClass() throws Exception {
    ...
    DefaultChildApplicationContextManager childApplicationContextManager = (DefaultChildApplicationContextManager) ctx.getBean(
      "Authentication"
    );
    remoteUserAuthenticatorFactory =
      (RemoteUserAuthenticatorFactory) ctx.getBean(
  ...
  public void testDisabledUser() throws Exception {
    final String username = transactionService
            ...
            return AuthenticationUtil.runAs(
              new RunAsWork<Void>() {
                ...
                public Void doWork() throws Exception {
                  ...
                  when(mockHttpRequest.getHeader("X-Alfresco-Remote-User"))
                    .thenReturn(username);
                  when(mockHttpRequest.getScheme()).thenReturn("http");
                  WebScriptServletRequest mockRequest = mock(
                    WebScriptServletRequest.class
                  );
                  when(mockRequest.getHttpServletRequest())
                    .thenReturn(mockHttpRequest);

                  HttpServletResponse mockHttpResponse = mock(
                    HttpServletResponse.class
                  );
                  WebScriptServletResponse mockResponse = mock(
                    WebScriptServletResponse.class
                  );
                  when(mockResponse.getHttpServletResponse())
  ...
  public void testLogInWithNonExistingPerson() {
    ...
    assertTrue(
      "The non existing user should be authenticated.",
      authenticator.authenticate(RequiredAuthentication.user, false)
    );
    assertTrue(
      "The user should be auto created.",
      personService.personExists(username)
    );
  }
}

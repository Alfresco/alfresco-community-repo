...
public class LargeArchiveAndRestoreTest extends TestCase {
    ...
    private class CreateDataCallback implements RetryingTransactionCallback<NodeRef> {
        ...
        private NodeRef addFile(NodeRef parentNodeRef, String filename, int depth) {
            System.out.println(
                    makeTabs(depth) + "Creating file " + filename + " in parent " + parentNodeRef);
            FileInfo info =
    ...
    public void testCreateAndRestore() throws Exception {
        RunAsWork<NodeRef> createHierarchyWork =
        ...
        final NodeRef parentNodeRef =
                AuthenticationUtil.runAs(createHierarchyWork, AuthenticationUtil.SYSTEM_USER_NAME);
        // Delete it
        final RetryingTransactionCallback<NodeRef> deleteHierarchyCallback =
                new RetryingTransactionCallback<NodeRef>() {
                    public NodeRef execute() throws Throwable {
                        // Delete it
                        fileFolderService.delete(parentNodeRef);
                        // Now try to find the archived node
                        NodeRef archivedParentNodeRef =
                                new NodeRef(
                                        "archive",
                                        parentNodeRef.getStoreRef().getIdentifier(),
                                        parentNodeRef.getId());
                        // Check it
                        if (!nodeService.exists(archivedParentNodeRef)) {
                            throw new InvalidNodeRefException(
                                    "Archived node not found after delete: "
                                            + archivedParentNodeRef,
                                    archivedParentNodeRef);
                        }
                        if (nodeService.exists(parentNodeRef)) {
                            throw new InvalidNodeRefException(
                                    "Original node was found after delete: " + parentNodeRef,
                                    parentNodeRef);
                        }
                        return archivedParentNodeRef;
                    }
                };
        RunAsWork<NodeRef> deleteHierarchyWork =
                new RunAsWork<NodeRef>() {
                    public NodeRef doWork() throws Exception {
                        return transactionService
                                .getRetryingTransactionHelper()
                                .doInTransaction(deleteHierarchyCallback);
                    }
                };
        final NodeRef archivedParentNodeRef =
                AuthenticationUtil.runAs(deleteHierarchyWork, AuthenticationUtil.SYSTEM_USER_NAME);
        // Restore it
        final RetryingTransactionCallback<NodeRef> restoreHierarchyCallback =
                new RetryingTransactionCallback<NodeRef>() {
                    public NodeRef execute() throws Throwable {
                        // Delete it
                        List<RestoreNodeReport> report =
                                nodeArchiveService.restoreArchivedNodes(
                                        Collections.singletonList(archivedParentNodeRef));
                        // Dump the report
                        System.out.println("Restore report: \n" + report);
                        // Check it
                        if (nodeService.exists(archivedParentNodeRef)) {
                            throw new InvalidNodeRefException(
                                    "Archived node was found after restore: "
                                            + archivedParentNodeRef,
                                    archivedParentNodeRef);
                        }
                        if (!nodeService.exists(parentNodeRef)) {
                            throw new InvalidNodeRefException(
                                    "Original node was not found after restore: " + parentNodeRef,
                                    parentNodeRef);
                        }
                        return parentNodeRef;
                    }
                };
        RunAsWork<NodeRef> restoreHierarchyWork =
                new RunAsWork<NodeRef>() {
                    public NodeRef doWork() throws Exception {
                        return transactionService
                                .getRetryingTransactionHelper()
                                .doInTransaction(restoreHierarchyCallback);
                    }
                };
        final NodeRef checkParentNodeRef =
                AuthenticationUtil.runAs(restoreHierarchyWork, AuthenticationUtil.SYSTEM_USER_NAME);
        assertEquals(
                "Restored node reference doesn't match original",
                parentNodeRef,
                checkParentNodeRef);

        // Purge it
        final RetryingTransactionCallback<Object> purgeHierarchyCallback =
                new RetryingTransactionCallback<Object>() {
                    public Object execute() throws Throwable {
                        ...
                        if (nodeService.exists(archivedParentNodeRef)) {
                            throw new InvalidNodeRefException(
                                    "Node not purged by delete: " + archivedParentNodeRef,
                                    archivedParentNodeRef);
                        }
                        if (nodeService.exists(parentNodeRef)) {
                            throw new InvalidNodeRefException(
                                    "Original node was found after purge: " + parentNodeRef,
                                    parentNodeRef);
                        }
                        return null;
                    }
                };
        RunAsWork<Object> purgeHierarchyWork =
                new RunAsWork<Object>() {
                    public Object doWork() throws Exception {
                        transactionService
                                .getRetryingTransactionHelper()
                                .doInTransaction(purgeHierarchyCallback);
                        return null;
                    }
                };

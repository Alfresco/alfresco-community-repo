...
public class GetSharedLinksFullTests extends RestTest {
  ...
  @TestRail(
    ...
    description = "Verify that a user with permission can get allowableOperations on sharedLinks"
  )
  @Test(
  ...
  public void getSharedLinksWithAllowableOperations() throws Exception {
      ...
      .add(
        ...
        Json
          ...
          .add(
            ...
            Json
              .createObjectBuilder()
              .add("authorityId", userModel.getUsername())
              .add("name", "SiteConsumer")
    ...
    restClient
      ...
      .withCoreAPI()
      .usingSharedLinks()
      .createSharedLink(file);
    restClient.assertStatusCodeIs(HttpStatus.CREATED);

    // verify that the permision exists
    RestNodeModel permissionsRequest = restClient
      .authenticateUser(userModel)
      .withCoreAPI()
      .usingNode(file)
      .usingParams("include=permissions")
      .getNode();
    permissionsRequest
      .assertThat()
      .field("permissions.locallySet.authorityId")
    ...
    do {
      ...
      operationsRequest =
        restClient
          ...
          .withCoreAPI()
          .usingSharedLinks()
          .usingParams("include=allowableOperations")
          .getSharedLinks();
      restClient.assertStatusCodeIs(HttpStatus.OK);
    } while (
      operationsRequest.getPagination().getCount() == 0 && numberOfTries < 15
    );

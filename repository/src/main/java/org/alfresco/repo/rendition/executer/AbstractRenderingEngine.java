...
/**
 * This class adds some new behaviour to the standard ActionExecuterAbstractBase in order to support
 * the RenditionService.
 *
...
public abstract class AbstractRenderingEngine extends ActionExecuterAbstractBase {
    ...
    protected static final String CONTENT_READER_NOT_FOUND_MESSAGE =
            "Cannot find Content Reader for document. Operation can't be performed";
    ...
    /**
     * This is the default default node type for renditions - used if no value is injected from
     * spring.
     */
    private static final QName DEFAULT_DEFAULT_RENDITION_NODE_TYPE = ContentModel.TYPE_CONTENT;

    /**
     * This is the default default property used to specify where rendition content is stored - used
     * if no value is injected from spring.
     */
    private static final QName DEFAULT_DEFAULT_RENDITION_CONTENT_PROP =
            RenditionService2Impl.DEFAULT_RENDITION_CONTENT_PROP;
    ...
    /**
     * This optional {@link String} parameter specifies the location of a classpath resource which
     * can be used as a placeholder while a rendition is being generated. For example, this might be
     * a simple icon to indicate a rendition is not yet available. This is intended to be used in
    ...
    /**
     * This optional {@link QName} parameter specifies which property the Rendering Engine uses to
     * read content from the source node in order to create a rendition. By default this property
     * will be cm:content.
     */
    ...
    /**
     * This optional {@link QName} parameter specifies which property the Rendering Engine uses to
     * write content to the rendition node. By default the property used is cm:content.
     */
    public static final String PARAM_TARGET_CONTENT_PROPERTY = "targetContentProperty";

    /**
     * This optional {@link Boolean} flag property specifies whether a rendition should be updated
     * automatically if the source node changes. If set to <code>true</code> then the rendition will
     * be re-rendered any time any property changes occur on the source node. This parameter
     * defaults to <code>false</code>.
     */
    public static final String PARAM_UPDATE_RENDITIONS_ON_ANY_PROPERTY_CHANGE =
            "update-renditions-on-any-property-change";

    /**
     * This optional {@link String} parameter specifies what user permissions are used when creating
     * a rendition. By default the system user is used.
     */
    ...
    /**
     * This optional {@link String} parameter specifies the mime type of the rendition content. This
     * defaults to the mime type of the source node content.
     */
    public static final String PARAM_MIME_TYPE = "mime-type";

    /**
     * This optional {@link String} paramter specifies the encoding used to create the rendition
     * content. The derfault encoding is UTF-8.
     */
    ...
    public AbstractRenderingEngine(
            NodeLocator temporaryParentNodeLocator, QName temporaryRenditionLinkType) {
        this.publicAction = false;
        this.temporaryParentNodeLocator =
                temporaryParentNodeLocator != null
                        ? temporaryParentNodeLocator
                        : defaultNodeLocator;
        this.temporaryRenditionLinkType =
                temporaryRenditionLinkType != null
                        ? temporaryRenditionLinkType
    ...
    /**
     * This method returns the QName of the property that defines the location of the rendition
     * content. An example would be cm:content.
     *
    ...
    protected void executeImpl(final RenditionDefinition renditionDef, final NodeRef sourceNode) {
        ...
        if (!nodeService.exists(sourceNode)) {
            if (logger.isDebugEnabled() == true) {
                logger.debug(
                        "Rendition has not been created, because the node no longer exists. "
                                + " (sourceNode="
                                + sourceNode
                                + ")");
            }
            notifyCallbackOfException(
                    renditionDef,
                    new UnimportantTransformException(
                            "Rendition was cancelled, because the node no longer exists."));
            return;
        } else if (nodeService.getProperty(sourceNode, ContentModel.PROP_CONTENT) == null) {
            if (logger.isDebugEnabled() == true) {
                logger.debug(
                        "Rendition has not been created, because the node has no content to render."
                                + "  (sourceNode="
                                + sourceNode
                                + ")");
            }
            notifyCallbackOfException(
                    renditionDef,
                    new UnimportantTransformException(
                            "Rendition was cancelled, because the node has no content to render."));
            return;
        }

        if (logger.isDebugEnabled()) {
            StringBuilder msg = new StringBuilder();
            msg.append("Rendering node ")
                    .append(sourceNode)
                    .append(" with rendition definition ")
                    .append(renditionDef.getRenditionName());
            ...
            if (renditionDef.getParameterValues().isEmpty() == false) {
                for (String paramKey : renditionDef.getParameterValues().keySet()) {
                    msg.append("    ")
                            .append(paramKey)
                            .append("=")
                            .append(renditionDef.getParameterValue(paramKey))
                            .append("\n");
                }
            } else {
                msg.append("    [None]");
        ...
        Serializable runAsParam =
                renditionDef.getParameterValue(AbstractRenderingEngine.PARAM_RUN_AS);
        ...
        AuthenticationUtil.runAs(
                new AuthenticationUtil.RunAsWork<Void>() {
                    ...
                    public Void doWork() throws Exception {
                        ...
                        try {
                            // Check whether this rendition is a component of a larger
                            // CompositeRendition
                            ...
                            if (isComponentRendition == false) {
                                ...
                                //  in the wrong place
                                // So, have the correct node created, and switch everything to use
                                // it
                                switchToFinalRenditionNode(renditionDef, sourceNode);
                            }

                            // Grab a link to the rendition node - it's been saved as a parameter
                            // for us
                            // (Wait until now to fetch in case it was moved)
                            result =
                                    (ChildAssociationRef)
                                            renditionDef.getParameterValue(PARAM_RESULT);
                        } catch (Throwable t) {
    ...
    /**
     * Is this a standalone rendition, or is it a sub-component of a composite rendition? This is
     * false for standalone renditions, AND ALSO false for the main part of a composite rendition.
     * This only returns true if we're currently processing a component of a composite rendition.
     *
     * @param action Action
     * @return boolean
     */
    ...
    protected void executeRenditionImpl(Action action, NodeRef sourceNode) {
        ...
        RenderingContext context =
                new RenderingContext(sourceNode, renditionDefinition, targetContentProp);
        render(context);
    ...
    /**
     * This method can be overridden by subclasses to provide checking of parameter values. If a
     * parameter value is illegal or inappropriate, an exception should be thrown.
     */
    ...
    protected QName getRenditionContentProperty(RenditionDefinition renditionDefinition) {
        return getParamWithDefault(
                PARAM_TARGET_CONTENT_PROPERTY, defaultRenditionContentProp, renditionDefinition);
    }
    ...
    private QName getRenditionNodeType(RenditionDefinition renditionDefinition) {
        return getParamWithDefault(
                PARAM_RENDITION_NODETYPE, defaultRenditionNodeType, renditionDefinition);
    }
    ...
    private ChildAssociationRef createRenditionNodeAssoc(
            NodeRef sourceNode, RenditionDefinition renditionDefinition) {
        ...
        nodeProps.put(
                ...
                getRenditionContentProp(renditionDefinition));
        QName assocName =
                QName.createQName(NamespaceService.RENDITION_MODEL_1_0_URI, GUID.generate());
        ...
        try {
            childAssoc =
                    nodeService.createNode(parentNode, assocType, assocName, nodeType, nodeProps);
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Created node "
                                + childAssoc
    ...
    /**
     * Gets the value for the named parameter. Checks the type of the parameter is correct and
     * throws a {@link RenditionServiceException} if it isn't. Returns <code>null</code> if the
     * parameter value is <code>null</code>
     *
     * @param paramName the name of the parameter being checked.
     * @param clazz the expected {@link Class} of the parameter value.
     * @param definition the {@link RenditionDefinition} containing the parameters.
     * @return the parameter value or <code>null</code>.
     */
    @SuppressWarnings("unchecked")
    public static <T> T getCheckedParam(
            String paramName, Class<T> clazz, RenditionDefinition definition) {
        Serializable value = definition.getParameterValue(paramName);
        if (value == null) return null;
        else {
            if (clazz == null)
                throw new RenditionServiceException(
                        "The class must not be null!", new NullPointerException());
    ...
    /**
     * Gets the value for the named parameter. Checks the type of the parameter is the same as the
     * type of <code>defaultValue</code> and throws a {@link RenditionServiceException} if it isn't.
     * Returns <code>defaultValue</code> if the parameter value is <code>null</code>
     *
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T> T getParamWithDefault(
            String paramName, T defaultValue, RenditionDefinition definition) {
        if (defaultValue == null)
            throw new RenditionServiceException(
                    "The defaultValue cannot be null!", new NullPointerException());
    ...
    protected class RenderingContext implements SerializedTransformationOptionsAccessor {
        ...
        public ContentReader makeContentReader() {
            QName srcContentProp =
                    getParamWithDefault(PARAM_SOURCE_CONTENT_PROPERTY, DEFAULT_CONTENT_PROPERTY);
        ...
        public ContentWriter makeContentWriter() {
            ContentWriter contentWriter =
                    contentService.getWriter(getDestinationNode(), renditionContentProperty, true);
    ...
    protected void tagSourceNodeAsRenditioned(
            final RenditionDefinition renditionDef, final NodeRef actionedUponNodeRef) {
    ...
    protected void switchToFinalRenditionNode(
            final RenditionDefinition renditionDef, final NodeRef actionedUponNodeRef) {
        ChildAssociationRef tempRendAssoc =
                (ChildAssociationRef) renditionDef.getParameterValue(PARAM_RESULT);
        if (logger.isDebugEnabled()) {
            logger.debug("Switching temporary rendition: " + tempRendAssoc);
        }
        ChildAssociationRef result =
                createOrUpdateRendition(actionedUponNodeRef, tempRendAssoc, renditionDef);
    ...
    protected void notifyCallbackOfResult(
            RenditionDefinition renditionDefinition, ChildAssociationRef result) {
     ...
     */
    private void setTemporaryRenditionProperties(
            NodeRef sourceNode, RenditionDefinition definition) {
        // Set the parent and assoc type for the temporary rendition to be
        // created.
        NodeRef parent =
                temporaryParentNodeLocator.getNode(sourceNode, definition.getParameterValues());
        definition.setRenditionParent(parent);
        definition.setRenditionAssociationType(temporaryRenditionLinkType);

        if (logger.isDebugEnabled()) {
            StringBuilder msg = new StringBuilder();
            msg.append("Temporary rendition will have parent=")
                    .append(parent)
                    .append(" and assoc-type=")
                    .append(temporaryRenditionLinkType);
            logger.debug(msg.toString());
        }
    }
     ...
     * @param renditionDefinition The definition of the rendition that has just been performed. In
     *     the case of a composite rendition, this parameter refers to that CompositeRendition and
     *     not to any of its component renditions.
     * @return ChildAssociationRef
     */
    private ChildAssociationRef createOrUpdateRendition(
            NodeRef sourceNode,
            ChildAssociationRef tempRendition,
            RenditionDefinition renditionDefinition) {
        NodeRef tempRenditionNode = tempRendition.getChildRef();
        RenditionLocation renditionLocation =
                resolveRenditionLocation(sourceNode, renditionDefinition, tempRenditionNode);

        QName renditionQName = renditionDefinition.getRenditionName();

        RenditionNodeManager renditionNodeManager =
        ...
        String renditionName =
                getRenditionName(tempRenditionNode, renditionLocation, renditionDefinition);
        nodeService.setProperty(
                renditionNode.getChildRef(), ContentModel.PROP_NAME, renditionName); // to manager
        ...
        ChildAssociationRef renditionAssoc =
                renditionService.getRenditionByName(sourceNode, renditionQName);
        if (renditionAssoc == null) {
            String msg =
                    "A rendition of name: "
                            + renditionQName
                            + " should have been created for source node: "
                            + sourceNode;
            throw new RenditionServiceException(msg);
        }
    ...
    /**
     * This method manages the <code>rn:rendition</code> aspects on the rendition node. It applies
     * the correct rendition aspect based on the rendition node's location and removes any
     * out-of-date rendition aspect.
     */
    private void manageRenditionAspects(
            NodeRef sourceNode, ChildAssociationRef renditionParentAssoc) {
    ...
    /**
     * This method calculates the name for a rendition node. The following approaches are attempted
     * in the order given below.
     *
     * <ol>
     *   <li>If a name is defined in the {@link RenditionLocation} then that is used.
     *   <li>If the temporary rendition has a <code>cm:name</code> value, then that is used.
     *   <li>Otherwise use the rendition definition's rendition name.
     * </ol>
     *
    ...
    /**
     * Given a rendition definition, a source node and a temporary rendition node, this method uses
     * a {@link RenditionLocationResolver} to calculate the {@link RenditionLocation} of the
     * rendition.
     */
    protected RenditionLocation resolveRenditionLocation(
            NodeRef sourceNode, RenditionDefinition definition, NodeRef tempRendition) {
        return renditionLocationResolver.getRenditionLocation(
                sourceNode, definition, tempRendition);
    }

    /**
     * Gets the <code>ExecutionSummary</code> for the given <code>renderingContext</code> from the
     * {@link ActionTrackingService}.
     *
     * <p>Note that multiple summaries of the same action instance are not currently supported.
     *
     * @param renderingContext the rendering context
     * @return the found summary or null
     */
    protected ExecutionSummary getExecutionSummary(RenderingContext renderingContext) {
        List<ExecutionSummary> executionSummaries =
                actionTrackingService.getExecutingActions(renderingContext.getDefinition());
        if (executionSummaries == null || executionSummaries.size() == 0) {
            return null;
        }
        if (executionSummaries.size() > 1) {
            throw new ActionServiceException(
                    "getExecutionSummary not supported for "

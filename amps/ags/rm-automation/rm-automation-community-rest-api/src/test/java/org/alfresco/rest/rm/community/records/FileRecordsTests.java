...
public class FileRecordsTests extends BaseRMRestTest {
    private UnfiledContainerChild electronicRecord =
    ...
    private UnfiledContainerChild nonelectronicRecord =
            UnfiledContainerChild.builder()
                    .properties(
    ...
    /**
     * Returns the ids of unfiled electronic and non-electronic records from Unfiled Records
     * container
     */
    ...
    /**
     * Returns the ids of unfiled electronic and non-electronic records from an Unfiled Record
     * Folder
     */
    ...
    public Object[][] getRecordsFromUnfiledRecordFolder() {
        ...
        return new String[][] {
            ...
            {
                unfiledRecordFoldersAPI
                        .createUnfiledRecordFolderChild(nonelectronicRecord, unfiledRecordFolderId)
                        .getId()
            }
        };
    }
    ...
    /**
     * Given an unfiled record in the root unfiled record container And an open record folder When I
     * file the unfiled record into the record folder Then the record is filed
     */
    @Test(dataProvider = "unfiledRecordsFromUnfiledRecordsContainer")
    @AlfrescoTest(jira = "RM-7060")
    public void fileRecordFromUnfiledContainerToOpenFolder(String unfiledRecordId)
            throws Exception {
        ...
        assertTrue(
                isRecordChildOfRecordFolder(unfiledRecordId, targetFolderId),
                unfiledRecordId + " is not filed to " + targetFolderId);

        // check the record doesn't exist into unfiled record container
        assertFalse(
                isRecordChildOfUnfiledContainer(unfiledRecordId),
                unfiledRecordId + " exists in Unfiled Records");
        assertTrue(
                hasAspect(unfiledRecordId, RECORD_SEARCH_ASPECT),
                "recordSearch aspect is lost after filing!");
    }

    /**
     * Given an unfiled record in a unfiled record folder And an open record folder When I file the
     * unfiled record into the record folder Then the record is filed
     */
    @Test(dataProvider = "unfiledRecordsFromUnfiledRecordFolder")
    @AlfrescoTest(jira = "RM-7060")
    public void fileRecordFromUnfiledRecordFolderToOpenFolder(String unfiledRecordId)
            throws Exception {
        // file the record to the folder created
        Record recordFiled = fileRecordToFolder(unfiledRecordId, targetFolderId);
        // check the response status
        ...
        assertFalse(
                isRecordChildOfUnfiledRecordFolder(unfiledRecordId),
    ...
    /**
     * Given an unfiled record in the root unfiled record container And a closed record folder When
     * I file the unfiled record into the record folder Then I get an unsupported operation
     * exception
     */
    @Test(dataProvider = "unfiledRecordsFromUnfiledRecordsContainer")
    public void fileRecordFromUnfiledContainerToClosedFolder(String unfiledRecordId) {
        ...
        assertFalse(
                isRecordChildOfRecordFolder(unfiledRecordId, closedFolderId),
                unfiledRecordId + " is filed to " + closedFolderId);

        // check the record exist into unfiled record container
        assertTrue(
                isRecordChildOfUnfiledContainer(unfiledRecordId),
                unfiledRecordId + " doesn't exist in Unfiled Records");
    }

    /**
     * Given an unfiled record in a unfiled record folder And a closed record folder When I file the
     * unfiled record into the record folder Then I get an unsupported operation exception
     */
    @Test(dataProvider = "unfiledRecordsFromUnfiledRecordFolder")
    ...
    /**
     * Given a filed record in a record folder And an open record folder When I file the filed
     * record into the record folder Then the record is filed in both locations
     */
    @Test(dataProvider = "unfiledRecordsFromUnfiledRecordsContainer")
    @Bug(id = "RM-4578")
    public void linkRecordInto(String unfiledRecordId) {
        ...
        assertTrue(
                recordFolderAPI.getRecordFolderChildren(targetFolderId).getEntries().stream()
                        .anyMatch(
                                c ->
                                        c.getEntry().getId().equals(recordFiled.getId())
                                                && c.getEntry()
                                                        .getParentId()
        ...
        assertTrue(
                recordFolderAPI.getRecordFolderChildren(folderToLink).getEntries().stream()
                        .anyMatch(
                                c ->
                                        c.getEntry().getId().equals(recordFiled.getId())
    ...
    /**
     * Given an unfiled container or filed record And a container that is NOT a record folder When I
     * file the unfiled container or filed record to the container Then I get an unsupported
     * operation exception
     */
    @Test(
    ...
    public void invalidContainerToFile(String containerId) {
        ...
        UnfiledContainerChild recordElectronic =
                unfiledContainersAPI.uploadRecord(
                        electronicRecord,
                        UNFILED_RECORDS_CONTAINER_ALIAS,
                        createTempFile(ELECTRONIC_RECORD_NAME, ELECTRONIC_RECORD_NAME));
        UnfiledContainerChild recordNonElect =
    ...
    private Record fileRecordToFolder(String recordId, String targetFolderId) {
        RecordBodyFile recordBodyFile =
                RecordBodyFile.builder().targetParentId(targetFolderId).build();
     ...
     *
     * @param recordId the record id
     * @return true if any child of the unfiled record folder match the provided record, false
     *     otherwise
     */

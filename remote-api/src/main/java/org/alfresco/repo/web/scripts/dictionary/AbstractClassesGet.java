...
public abstract class AbstractClassesGet extends DictionaryWebServiceBase {
  ...
  protected Map<String, Object> executeImpl(
    ...
    String classFilter = getValidInput(
      req.getParameter(REQ_URL_TEMPL_VAR_CLASS_FILTER)
    );
    String namespacePrefix = getValidInput(
      req.getParameter(REQ_URL_TEMPL_VAR_NAMESPACE_PREFIX)
    );
    ...
    if (namespacePrefix == null && name != null) {
      throw new WebScriptException(
        Status.STATUS_NOT_FOUND,
        "Missing namespaceprefix parameter in the URL - both combination of name and namespaceprefix is needed"
      );
    }
        ...
        for (QName qnameObj : this.dictionaryservice.getAllModels()) {
          if (qnameObj.getNamespaceURI().equals(namespaceUri)) {
            ...
            if (classFilter.equalsIgnoreCase(CLASS_FILTER_OPTION_TYPE1)) {
              ...
              classFilter.equalsIgnoreCase(CLASS_FILTER_OPTION_TYPE3)
            ) {
              qnames.addAll(this.dictionaryservice.getTypes(myModel));
            } else if (
              classFilter.equalsIgnoreCase(CLASS_FILTER_OPTION_TYPE2)
            ) {
              qnames.addAll(this.dictionaryservice.getAspects(myModel));
            }
          }
    ...
    model.put(
      MODEL_PROP_KEY_PROPERTY_DETAILS,
      reorderedValues(classDefinitions, propdef)
    );
    model.put(
      MODEL_PROP_KEY_ASSOCIATION_DETAILS,
      reorderedValues(classDefinitions, assocdef)
    );
   ...
   */
  protected abstract QName getQNameForModel(

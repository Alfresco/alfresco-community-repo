...
/**
 * This Host Manager is assumed to be session local, i.e. there is one instance per IMAP TCP
 * connection. This means that it can locally cache items being interacted with during the session
 * and its knowledge of which folders / messages do or do not exist will match that of the client.
 *
 * @author Mike Shavnev
 * @author David Ward
 */
public class AlfrescoImapHostManager implements ImapHostManager {
    ...
    /**
     * Returns an collection of mailboxes. Method searches mailboxes under mount points defined for
     * a specific user. Mount points include user's IMAP Virtualised Views and Email Archive Views.
     * This method serves LIST command of the IMAP protocol.
     *
     ...
     */
    public Collection<MailFolder> listMailboxes(GreenMailUser user, String mailboxPattern)
            throws FolderException {
        try {
            AlfrescoImapUser alfrescoUser =
                    new AlfrescoImapUser(user.getEmail(), user.getLogin(), user.getPassword());
            return registerMailboxes(
                    imapService.listMailboxes(
                            alfrescoUser,
                            getUnqualifiedMailboxPattern(alfrescoUser, mailboxPattern),
                            false));
        } catch (Throwable e) {
    ...
    /**
     * Returns an collection of subscribed mailboxes. To appear in search result mailboxes should
     * have {http://www.alfresco.org/model/imap/1.0}subscribed property specified for user. Method
     * searches subscribed mailboxes under mount points defined for a specific user. Mount points
     * include user's IMAP Virtualised Views and Email Archive Views. This method serves LSUB
     * command of the IMAP protocol.
     *
     * @param user User making the request
     * @param mailboxPattern String name of a mailbox possible including a wildcard.
     * @return Collection of mailboxes matching the pattern.
     ...
     *
     * <p>Method searches mailbox under mount points defined for a specific user. Mount points
     ...
     */
    public void renameMailbox(GreenMailUser user, String oldMailboxName, String newMailboxName)
            throws FolderException, AuthorizationException {
        try {
            AlfrescoImapUser alfrescoUser =
                    new AlfrescoImapUser(user.getEmail(), user.getLogin(), user.getPassword());
            String oldFolderPath = getUnqualifiedMailboxPattern(alfrescoUser, oldMailboxName);
    ...
    /**
     * Returns a reference to a newly created mailbox. The request should specify a mailbox that
     * does not already exist on this server, that could exist on this server and that the user has
     * rights to create. This method serves CREATE command of the IMAP protocol.
     *
     * @param user User making the request.
        ...
        try {
            ...
            return registerMailBox(
                    imapService.getOrCreateMailbox(
                            alfrescoUser,
                            getUnqualifiedMailboxPattern(alfrescoUser, mailboxName),
                            false,
                            true));
        } catch (Throwable e) {
            logger.debug(e.getMessage(), e);
     ...
     * user must have rights to delete it.
     *
     * <p>This method serves DELETE command of the IMAP protocol.
     *
     * @param user User making the request.
     * @param mailboxName String name of the target
     * @throws com.icegreen.greenmail.store.FolderException if mailbox has a non-selectable store
     *     with children
     */
    ...
    /**
     * Returns a reference to an existing Mailbox. The requested mailbox must already exists on this
     * server and the requesting user must have at least lookup rights.
     *
     * <p>It is also can be used by to obtain hierarchy delimiter by the LIST command:
     *
     * <p>C: 2 list "" ""
     *
     * <p>S: * LIST () "." ""
     *
     * <p>S: 2 OK LIST completed.
     *
     * <p>Method searches mailbox under mount points defined for a specific user. Mount points
     * include user's IMAP Virtualised Views and Email Archive Views.
     *
    ...
    public MailFolder getFolder(GreenMailUser user, String mailboxName) {
        ...
        return result != null
                ? result
                : registerMailBox(
                        imapService.getOrCreateMailbox(alfrescoUser, mailboxName, true, false));
    }

    /**
     * Simply calls {@link #getFolder(GreenMailUser, String)}.
     *
     * <p>Added to implement {@link ImapHostManager}.
     */
    public MailFolder getFolder(
            final GreenMailUser user, final String mailboxName, boolean mustExist)
            throws FolderException {
        try {
    ...
    public void subscribe(GreenMailUser user, String mailbox) throws FolderException {
        try {
            AlfrescoImapUser alfrescoUser =
                    new AlfrescoImapUser(user.getEmail(), user.getLogin(), user.getPassword());
            imapService.subscribe(
                    alfrescoUser, getUnqualifiedMailboxPattern(alfrescoUser, mailbox));
        } catch (Throwable e) {
            logger.debug(e.getMessage(), e);
            throw new FolderException(e.getMessage());
        }
    ...
    private String getUnqualifiedMailboxPattern(AlfrescoImapUser user, String mailboxPattern) {
        ...
        if (mailboxPattern.startsWith(AlfrescoImapConst.NAMESPACE_PREFIX)) {
            ...
            return sepIndex == -1
                    ? mailboxPattern.endsWith("*") ? "*" : ""
                    : mailboxPattern.substring(sepIndex + 1);
        }
     ...
     *
     * @param mailboxes a Collection of Alfresco mailboxes
     ...
     *
     * @param mailbox an Alfresco mailbox
        ...
        if ((mailbox.isSelectable() || folderPath.isEmpty())) {
            ...
            if (oldFolder != null
                    && oldFolder
                            .getFolderInfo()
                            .getNodeRef()
                            .equals(mailbox.getFolderInfo().getNodeRef())
                    && oldFolder.reset()) {
                mailbox = oldFolder;
            } else {

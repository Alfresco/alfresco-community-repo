...
public class SolrSQLJSONResultMetadataSetTest {
    ...
    public void parseSQLResponse() throws JSONException {
        String response =
                "{\"result-set\":{\"docs\":[{\"SITE\":\"_REPOSITORY_\"},{\"SITE\":\"surf-config\"},{\"SITE\":\"swsdp\"},{\"RESPONSE_TIME\":96,\"EOF\":true}]}}";
    ...
    public void parseSQLErrorResponse() throws JSONException {
        String response =
                "{\"result-set\":{\"docs\":[{\"EXCEPTION\":\"Column 'SIT1E' not found in any"
                        + " table\",\"EOF\":true,\"RESPONSE_TIME\":18943}]}}";
        try {
            JSONObject json = new JSONObject(response);
            SolrSQLJSONResultSet ssjr = new SolrSQLJSONResultSet(json, null);
            ...
            Assert.assertEquals(
                    "Unable to execute the query, error caused by: Column 'SIT1E' not found in any"
                            + " table",
                    e.getMessage());
        }
    }
    ...
    /**
     * Validates that when a query is done against SearchService then it should state that it works
     * only with Insight Engine.
     *
    ...
    public void parseInvalidInsightEngineResponse() throws JSONException {
        String response =
                "{\"result-set\":{\"docs\":[{\"EXCEPTION\":\"/sql handler only works in Solr Cloud"
                        + " mode\",\"EOF\":true}]}}";
        try {
            JSONObject json = new JSONObject(response);
            SolrSQLJSONResultSet ssjr = new SolrSQLJSONResultSet(json, null);
            ...
            Assert.assertNotNull(e);
            Assert.assertEquals(
                    "Unable to execute the query, this API requires InsightEngine.",
                    e.getMessage());
        }
    }
}

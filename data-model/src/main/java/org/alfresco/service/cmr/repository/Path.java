 ...
 * </pre>
 *
 * </b> In the above example, there will be <b>4</b> elements, the first being a reference to the
 * root node, followed by qname elements for <b>x</b>, <b>y</b> and <b>z</b>.
 *
 * <p>Methods and constructors are available to construct a <code>Path</code> instance from a path
 * string or by building the path incrementally, including the ability to append and prepend path
 * elements.
 *
 * <p>Path elements supported:
 *
 * <ul>
 *   <li><b>/{namespace}name</b> fully qualified element
 *   <li><b>/name</b> element using default namespace
 *   <li><b>/{namespace}name[n]</b> nth sibling
 *   <li><b>/name[n]</b> nth sibling using default namespace
 *   <li><b>/descendant-or-self::node()</b> descendent or self
 *   <li><b>/.</b> self
 *   <li><b>/..</b> parent
 * </ul>
 *
...
public final class Path implements Iterable<Path.Element>, Serializable {
    ...
    /**
     * Add a path element to the beginning of the path. This operation is useful in cases where a
     * path is built by traversing up a hierarchy.
     *
    ...
    /**
     * @return Returns a string path made up of the component elements of this instance (prefixed
     *     where appropriate)
     */
    ...
    /**
     * Return the human readable form of the specified node Path. Slow version of the method that
     * extracts the name of each node in the Path from the supplied NodeService.
     *
            ...
            if (element instanceof ChildAssocElement) {
                ...
                if (elementRef.getParentRef() != null) {
                    ...
                    if (permissionService.hasPermission(
                                    elementRef.getChildRef(), PermissionService.READ)
                            == AccessStatus.ALLOWED) {
                        nameProp =
                                nodeService.getProperty(
                                        elementRef.getChildRef(), ContentModel.PROP_NAME);
     ...
     * Return a new Path representing this path to the specified depth
     *
     * <p>For example, subPath(2, 4) would return the third and forth elements in the Path.
     *
    ...
    public Path subPath(int start, int end) {
        if (start < 0 || start > (elements.size() - 1)) {
            throw new IndexOutOfBoundsException(
                    "Start index " + start + " must be between 0 and " + (elements.size() - 1));
        }
        if (end < 0 || end > (elements.size() - 1)) {
            throw new IndexOutOfBoundsException(
                    "End index " + end + " must be between 0 and " + (elements.size() - 1));
        }
        if (end < start) {
            throw new IndexOutOfBoundsException(
                    "End index " + end + " cannot be before start index " + start);
        }
     ...
     *
     * <p>In <b>/x/y/z</b>, elements are <b>x</b>, <b>y</b> and <b>z</b>.
     */
    @AlfrescoPublicApi
    ...
    /**
     * Represents a qualified path between a parent and a child node, including the sibling to
     * retrieve e.g. <b>/{namespace}name[5]</b>
     */
    @AlfrescoPublicApi
    public static class ChildAssocElement extends Element {
        ...
        private String createElementString(NamespacePrefixResolver resolver) {
            ...
            if (ref.getParentRef() == null) {
                ...
                sb.append(
                        resolver == null
                                ? ISO9075.getXPathName(ref.getQName())
                                : ISO9075.getXPathName(ref.getQName(), resolver));
            }
        ...
        public Element getBaseNameElement(TenantService tenantService) {
            return new ChildAssocElement(
                    new ChildAssociationRef(
                            ref.getTypeQName(),
                            tenantService.getBaseName(ref.getParentRef(), true),
                            ref.getQName(),
                            tenantService.getBaseName(ref.getChildRef(), true),
                            ref.isPrimary(),
                            ref.getNthSibling()));
        }
    }
    ...
    /**
     * Represents a qualified path to an attribute, including the sibling for repeated
     * properties/attributes to retrieve e.g. <b>/@{namespace}name[5]</b>
     */
    @AlfrescoPublicApi
    public static class AttributeElement extends Element {
        ...
        private String createElementString(NamespacePrefixResolver resolver) {
            ...
            sb.append("@")
                    .append(
                            resolver == null
                                    ? ISO9075.getXPathName(attribute)
                                    : ISO9075.getXPathName(attribute, resolver));

...
public class CategoryPropertyDecorator extends BasePropertyDecorator {
  ...
  private static Log logger = LogFactory.getLog(
    CategoryPropertyDecorator.class
  );
  ...
  public JSONAware decorate(
    ...
    for (NodeRef obj : collection) {
      try {
        ...
        jsonObj.put(
          "name",
          this.nodeService.getProperty(obj, ContentModel.PROP_NAME)
        );
        ...
        logger.warn(
          "Category with nodeRef " + obj.toString() + " does not exist."
        );
      }
    }
  ...
  public String getPath(NodeRef nodeRef) {
    String displayPath =
      this.nodeService.getPath(nodeRef)

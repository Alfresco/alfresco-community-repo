...
public class DiscoveryApiTest extends AbstractSingleNetworkSiteTest {
  ...
  public void setup() throws Exception {
    ...
    when(descriptorServiceMock.getServerDescriptor())
      .thenReturn(serverDescriptor);
    ...
    when(descriptorServiceMock.getLicenseDescriptor())
      .thenReturn(licenseDescriptorMock);
    when(licenseDescriptorMock.getIssued()).thenReturn(this.licenseIssuedAt);
    when(licenseDescriptorMock.getValidUntil())
      .thenReturn(this.licenseExpiresAt);
    when(licenseDescriptorMock.getRemainingDays()).thenReturn(5);
    when(licenseDescriptorMock.getLicenseMode())
      .thenReturn(LicenseMode.ENTERPRISE);
    when(licenseDescriptorMock.getHolderOrganisation())
      .thenReturn("Alfresco Dev Test");
  ...
  public void testGetDiscovery() throws Exception {
    ...
    DiscoveryDetails discoveryDetails = RestApiUtil.parseRestApiEntry(
    ...
    assertEquals(
      licenseIssuedAt.toString(),
      licenseInfo.getIssuedAt().toString()
    );
    assertEquals(
      licenseExpiresAt.toString(),
      licenseInfo.getExpiresAt().toString()
    );

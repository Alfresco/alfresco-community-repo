...
public abstract class QuartzJobScheduler implements HeartBeatJobScheduler {
    ...
    /**
     * This method is called when a job is being scheduled by this scheduler for the given
     * collector. <br>
     * The job is scheduled using the {@link Job} returned from {@link #getHeartBeatJobClass()} and
     * the job map returned from this method, therefor the job map should provide what the job needs
     * to execute.
     *
     * @param collector The collector whose job is being scheduled.
     * @return The job map returned from this method will be used to build up {@link JobDetail} for
     ...
     * The jobs are scheduled together with a {@link JobDataMap} returned from {@link
     * #getJobDetailMap(HBBaseDataCollector)} <br>
     * which will be accessible during job execution.
     *
     * @return {@link Job} implementation which this scheduler will use to schedule jobs for
    ...
    public void scheduleJob(HBBaseDataCollector collector) {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "HeartBeat job scheduled for collector: " + collector.getCollectorId());
            }
        } catch (SchedulerException e) {
            throw new RuntimeException(
                    "Heartbeat failed to schedule job for collector: " + collector.getCollectorId(),
                    e);
        }
    }
    ...
    public void unscheduleJob(final HBBaseDataCollector collector) {
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "HeartBeat unscheduled job for collector: " + collector.getCollectorId());
            }
        } catch (SchedulerException e) {
            throw new RuntimeException(
                    "Heartbeat failed to unschedule job for collector: "

 ...
 *
 * <p>This class decorates an ExtendedDiskInterface with odds and ends to keep JLan happy.
 *
 * <p>In particular this implementation cannot contain any code that requires access to the alfresco
 * repository.
 */
public class LegacyFileStateDriver implements ExtendedDiskInterface {
    ...
    public NetworkFile createFile(SrvSession sess, TreeConnection tree, FileOpenParams params)
        ...
        try {
            ...
            // we could be out of memory or a NPE or some other unforseen situation.  JLAN will
            // complain loudly ... as it should.
            if (logger.isDebugEnabled()) {
    ...
    public NetworkFile openFile(SrvSession sess, TreeConnection tree, FileOpenParams params)
        ...
        try {
            ...
            if (openFile instanceof NetworkFileLegacyReferenceCount) {
                NetworkFileLegacyReferenceCount counter =
                        (NetworkFileLegacyReferenceCount) openFile;
    ...
    public void closeFile(SrvSession sess, TreeConnection tree, NetworkFile file)
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "closeFile:" + file.getFullName() + ", accessToken:" + file.getAccessToken());
        }
    ...
    public void setFileInformation(SrvSession sess, TreeConnection tree, String name, FileInfo info)
        ...
        if (tctx.hasStateCache()) {
            ...
            //               if ( logger.isDebugEnabled())
            //               {
            //                   logger.debug("Set creation date in file state cache" + name + ", "

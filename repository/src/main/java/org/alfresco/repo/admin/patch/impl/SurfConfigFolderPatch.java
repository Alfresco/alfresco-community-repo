...
public class SurfConfigFolderPatch extends AsynchronousPatch {
  ...
  private static final Log logger = LogFactory.getLog(
    SurfConfigFolderPatch.class
  );
  private static final Log progress_logger = LogFactory.getLog(
    PatchExecuter.class
  );

  private static final String MSG_SUCCESS =
    "patch.surfConfigFolderPatch.result";
  ...
  protected String applyInternal() throws Exception {
    ...
    BatchProcessor<Long> siteBatchProcessor = new BatchProcessor<Long>(
      ...
      public void process(NodeRef entry) throws Throwable {
        ...
        try {
          if (logger.isDebugEnabled()) {
            logger.debug(
              "\tP: Processing [company_home/sites/surf-config/pages/user] children"
            );
          }
          // add aspect to
    ...
    BatchProcessor<NodeRef> surfConfigBatchProcessor = new BatchProcessor<NodeRef>(
      "SurfConfigFolderPatch",
      transactionService.getRetryingTransactionHelper(),
  ...
  private void process(long siteId) {
    String siteName = (String) nodeDAO.getNodeProperty(
      siteId,
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "\tP: Processing surf-config folder for the site: [" + siteName + ']'
      );
    }

    // {siteName}/cm:surf-config/
    Pair<Long, ChildAssociationRef> surfConfigPair = nodeDAO.getChildAssoc(
      siteId,
    ...
    if (surfConfigPair == null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Unable to find surf-config folder for site: [" + siteName + ']'
        );
      }
      return;
    }
    ...
    NodeRef componentsNodeRef = nodeService.getChildByName(
      surfConfigNodeRef,
      ContentModel.ASSOC_CONTAINS,
      COMPONENTS
    );
    if (componentsNodeRef != null) {
      ...
      List<ChildAssociationRef> listOfComponents = nodeService.getChildAssocs(
        componentsNodeRef,
        ContentModel.ASSOC_CONTAINS,
    ...
    NodeRef pagesNodeRef = nodeService.getChildByName(
      surfConfigNodeRef,
      ContentModel.ASSOC_CONTAINS,
      PAGES
    );
    ...
    NodeRef siteNodeRef = nodeService.getChildByName(
      pagesNodeRef,
      ContentModel.ASSOC_CONTAINS,
      SITE
    );
    ...
    NodeRef siteChildNodeRef = nodeService.getChildByName(
      siteNodeRef,
      ContentModel.ASSOC_CONTAINS,
      siteName
    );
  ...
  private void addIndexControlAspectIfNotExist(NodeRef nodeRef) {
    ...
    Serializable indexProp = nodeService.getProperty(
      nodeRef,
    ...
    if (indexProp == null || ((Boolean) indexProp)) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "\tP: Adding cm:indexControl aspect to node: [" + nodeRef + ']'
        );
      }
    }
  }
  ...
  private class SiteWorkProvider implements BatchProcessWorkProvider<Long> {
    ...
    public synchronized Collection<Long> getNextWork() {
      ...
      final List<Long> siteNodeIDs = new ArrayList<Long>(
  ...
  private class SharedSurfConfigWorkProvider
    ...
    public synchronized Collection<NodeRef> getNextWork() {
      ...
      final List<NodeRef> folderNodes = new ArrayList<NodeRef>(
      ...
      while (currentId <= maxId && folderNodes.size() < minResults) {
        List<NodeRef> nodeIds = patchDAO.getChildrenOfTheSharedSurfConfigFolder(
          currentId,

...
public abstract class AbstractFeedGenerator implements FeedGenerator {
  ...
  private static VmShutdownListener vmShutdownListener = new VmShutdownListener(
    AbstractFeedGenerator.class.getName()
  );
  ...
  public void setActivityPostServiceImpl(
  ...
  public void execute() throws JobExecutionException {
    ...
    if (!transactionService.getAllowWrite()) {
      if (logger.isTraceEnabled()) {
        logger.trace(
          "Activities feed generator not running due to read-only server"
        );
      }
      return;
    }
        ...
        public Void execute() throws Throwable {
          ...
          try {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "Activities feed generator already underway: " + LOCK_QNAME
              );
            }
          } catch (Throwable e) {

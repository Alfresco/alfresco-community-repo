...
public class AuthenticationTest extends TestCase {
  ...
  private static char[] DONT_CARE_PASSWORD =
    "1 really don't care".toCharArray();
  ...
  private static final String TENANT_ADMIN_PW =
    DEFAULT_ADMIN_PW + TEST_TENANT_DOMAIN;
  ...
  public void setUp() throws Exception {
    ...
    compositePasswordEncoder =
      (CompositePasswordEncoder) ctx.getBean("compositePasswordEncoder");
    ...
    authenticationCache =
      (SimpleCache<String, CacheEntry>) ctx.getBean("authenticationCache");
    immutableSingletonCache =
    ...
    usernameToTicketIdCache =
      (SimpleCache<String, String>) ctx.getBean("usernameToTicketIdCache");

    ChildApplicationContextFactory sysAdminSubsystem = (ChildApplicationContextFactory) ctx.getBean(
      "sysAdmin"
    );
    ...
    ChildApplicationContextManager authenticationChain = (ChildApplicationContextManager) ctx.getBean(
      "Authentication"
    );
    ApplicationContext subsystem = authenticationChain.getApplicationContext(
      authenticationChain.getInstanceIds().iterator().next()
    );
    authenticationManager =
      (AuthenticationManager) subsystem.getBean("authenticationManager");

    transactionService =
      (TransactionService) ctx.getBean(
        ServiceRegistry.TRANSACTION_SERVICE.getLocalName()
      );
    ...
    AuthenticationUtil.runAs(
      new RunAsWork<Void>() {
        public Void doWork() throws Exception {
          ...
          assertEquals(
            "andy",
            ticketComponent.getAuthorityForTicket(
              pubAuthenticationService.getCurrentTicket()
            )
          );
          return null;
        }
  ...
  public void testNewTicketOnLogin() {
    authenticationComponent.setSystemUserAsCurrentUser();
    pubAuthenticationService.createAuthentication(
      "Andy",
      "auth1".toCharArray()
    );
    authenticationComponent.clearCurrentSecurityContext();
  ...
  public void testCreateUsers() {
    ...
    if (!tenantService.isEnabled()) {
      authenticationService.createAuthentication(
        "Mr.Woof.Banana@chocolate.chip.cookie.com",
        DONT_CARE_PASSWORD
      );
      authenticationService.authenticate(
        "Mr.Woof.Banana@chocolate.chip.cookie.com",
  ...
  private void authenticateMultiTenantWork(String userName, String password) {
    ...
    assertEquals(
      compositePasswordEncoder.encode("md4", password, null),
      hashedPassword
    );
  }
  ...
  public void testCreateAndyUserAndOtherCRUD()
    ...
    assertTrue(
      compositePasswordEncoder.matches(
        compositePasswordEncoder.getPreferredEncoding(),
        "cabbage",
        AndyDetails.getPassword(),
        null
      )
    );
    ...
    assertTrue(
      compositePasswordEncoder.matchesPassword(
        "carrot",
        newDetails.getPassword(),
        null,
        rau.getHashIndicator()
      )
    );
    ...
    dao.deleteUser("Andy");
    assertFalse(
      "Should not be a cache entry for 'Andy'.",
      authenticationCache.contains("Andy")
    );
    assertNull(
      "DAO should report that 'Andy' does not exist.",
      dao.getUserOrNull("Andy")
    );
  }
  ...
  public void testStaleAuthenticationCacheRecovery() {
    ...
    assertFalse(
      "Must start with no cache entry for 'Andy'.",
      authenticationCache.contains("Andy")
    );
    ...
    assertTrue(
      "Should still have an entry for 'Andy'.",
      authenticationCache.contains("Andy")
    );

    assertNull(
      "Invalid node should be detected for 'Andy'.",
      dao.getUserOrNull("Andy")
    );
    assertFalse(
      "Cache entry should have been removed for 'Andy'.",
      authenticationCache.contains("Andy")
    );
  }
  ...
  public void testRepositoryAuthenticationDaoWithoutTxn() throws Exception {
    ...
    dao.userExists("Andy");
    assertTrue(
      "Should now have an entry for 'Andy'.",
      authenticationCache.contains("Andy")
    );
  }
        ...
        RunAsWork<Object> authWorkAsSystem = new RunAsWork<Object>() {
          public Object doWork() throws Exception {
            ...
            return transactionService
              .getRetryingTransactionHelper()
              .doInTransaction(txnWork, false, true);
          }
        };
  ...
  public void testAlwaysGetNewTicket() {
    ...
    try {
      ...
      fail(
        " Previous call to validate should have cleared the context, so no new tickets should be issued"
      );
    } catch (Exception e) {
      // we expect this here
    }
  ...
  public void testPubAuthenticationService0() {
    ...
    pubAuthenticationService.authenticate("Andy", "auth3".toCharArray());
    pubAuthenticationService.updateAuthentication(
      "Andy",
      "auth3".toCharArray(),
      "auth4".toCharArray()
    );
  ...
  public void testAbstractAuthenticationComponentGuestUserSupport() {
    ...
    assertEquals(
      ...
      authenticationComponent.getGuestUserName()
    );
  }
  ...
  public void testAuthenticationServiceImpl() {
    ...
    List<AuthenticationService> services =
      (
        (AbstractChainingAuthenticationService) authenticationService
    ...
    for (AuthenticationService service : services) {
      if (service instanceof AuthenticationServiceImpl) {
        ...
        assertFalse(
          "Hardcoded to true",
          impl.getAuthenticationEnabled("anyone")
        );
  ...
  public void testLoginNotExistingTenant() {
    ...
    try {
      ...
      try {
        pubAuthenticationService.authenticate(
          userName,
          GUID.generate().toCharArray()
        );
        fail();
      } catch (AuthenticationException e) {
  ...
  public void testRehashedPasswordOnAuthentication() throws Exception {
    ...
    NodeRef andyUserNodeRef = getRepositoryAuthenticationDao()
      .getUserOrNull("Andy");
    assertNotNull(andyUserNodeRef);

    // ensure the properties are in the state we're expecting
    Map<QName, Serializable> userProps = nodeService.getProperties(
      andyUserNodeRef
    );
    ...
    String password2Prop = (String) userProps.get(
      ContentModel.PROP_PASSWORD_SHA256
    );
    assertNull("Expected the password2 property to be null", password2Prop);
    String passwordHashProp = (String) userProps.get(
      ContentModel.PROP_PASSWORD_HASH
    );
    assertNotNull(
      "Expected the passwordHash property to be populated",
      passwordHashProp
    );
    List<String> hashIndicatorProp = (List<String>) userProps.get(
      ContentModel.PROP_HASH_INDICATOR
    );
    assertNotNull(
      "Expected the hashIndicator property to be populated",
      hashIndicatorProp
    );
    ...
    ShaPasswordEncoderImpl sha256PasswordEncoder = new ShaPasswordEncoderImpl(
      256
    );
    ...
    Map<QName, Serializable> updatedProps = nodeService.getProperties(
      andyUserNodeRef
    );
    String usernameProp = (String) updatedProps.get(
      ContentModel.PROP_USER_USERNAME
    );
    assertEquals(
      "Expected the username property to be 'Andy'",
      "Andy",
      usernameProp
    );
    passwordProp = (String) updatedProps.get(ContentModel.PROP_PASSWORD);
    assertNotNull(
      "Expected the password property to be populated",
      passwordProp
    );
    password2Prop =
      (String) updatedProps.get(ContentModel.PROP_PASSWORD_SHA256);
    assertNotNull(
      "Expected the password2 property to be populated",
      password2Prop
    );
    passwordHashProp =
      (String) updatedProps.get(ContentModel.PROP_PASSWORD_HASH);
    assertNull(
      "Expected the passwordHash property to be null",
      passwordHashProp
    );
    hashIndicatorProp =
      (List<String>) updatedProps.get(ContentModel.PROP_HASH_INDICATOR);
    assertNull(
      "Expected the hashIndicator property to be null",
      hashIndicatorProp
    );
    ...
    Map<QName, Serializable> upgradedProps = nodeService.getProperties(
      andyUserNodeRef
    );
    passwordProp = (String) upgradedProps.get(ContentModel.PROP_PASSWORD);
    assertNull("Expected the password property to be null", passwordProp);
    password2Prop =
      (String) upgradedProps.get(ContentModel.PROP_PASSWORD_SHA256);
    assertNull("Expected the password2 property to be null", password2Prop);
    passwordHashProp =
      (String) upgradedProps.get(ContentModel.PROP_PASSWORD_HASH);
    assertNotNull(
      "Expected the passwordHash property to be populated",
      passwordHashProp
    );
    hashIndicatorProp =
      (List<String>) upgradedProps.get(ContentModel.PROP_HASH_INDICATOR);
    assertNotNull(
      "Expected the hashIndicator property to be populated",
      hashIndicatorProp
    );
    assertTrue(
      "Expected there to be a single hash indicator entry",
      (hashIndicatorProp.size() == 1)
    );
    String preferredEncoding = compositePasswordEncoder.getPreferredEncoding();
    String hashEncoding = (String) hashIndicatorProp.get(0);
    assertEquals(
      "Expected hash indicator to be '" +
      preferredEncoding +
      "' but it was: " +
      hashEncoding,
      preferredEncoding,
  ...
  public void testGetsMD4Password() throws Exception {
    ...
    String encoded = compositePasswordEncoder.encode(
      "md4",
      new String(rawPass),
      null
    );
  ...
  public void testCreateUserWithHashedPassword() throws Exception {
    ...
    for (String enc : encs) {
      ...
      String hash = compositePasswordEncoder.encodePreferred(
        SOME_PASSWORD,
        null
      );
      assertCreateHashed(
        ...
        hash,
        null,
        user1 + TenantService.SEPARATOR + myTestDomain
      );
      assertCreateHashed(
        SOME_PASSWORD,
        null,
        SOME_PASSWORD.toCharArray(),
        user2 + TenantService.SEPARATOR + myTestDomain
      );
    }
    compositePasswordEncoder.setPreferredEncoding(defaultencoding);
  }

  private void assertCreateHashed(
    String rawString,
    String hash,
    char[] rawPassword,
    String user
  ...
  private NamespacePrefixResolver getNamespacePrefixReolsver(
    String defaultURI
  ) {
    ...
    nspr.registerNamespace(
      ContentModel.USER_MODEL_PREFIX,
  ...
  public void testCreatingUserWithEmptyPassword() throws Exception {
    ...
    try {
      ...
      NodeRef userNodeRed = getRepositoryAuthenticationDao()

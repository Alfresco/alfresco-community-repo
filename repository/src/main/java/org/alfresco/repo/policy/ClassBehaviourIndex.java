 ...
 *
 * <p>Note: Uses Class hierarchy to derive bindings.
 *
 * @author David Caruana
 */
/*package*/ class ClassBehaviourIndex<B extends ClassBehaviourBinding>
    ...
    private BehaviourMap<ServiceBehaviourBinding> serviceMap =
            new BehaviourMap<ServiceBehaviourBinding>();
    ...
    /*package*/ ClassBehaviourIndex(BehaviourFilter filter) {
        ...
        this.classMap.addChangeObserver(
                new BehaviourChangeObserver<B>() {
        ...
        this.serviceMap.addChangeObserver(
                new BehaviourChangeObserver<ServiceBehaviourBinding>() {
                    public void addition(ServiceBehaviourBinding binding, Behaviour behaviour) {
                        for (BehaviourChangeObserver<B> listener : observers) {
                            // Note: Don't specify class ref as service-level bindings affect all
                            // classes
    ...
    public Collection<BehaviourDefinition> getAll() {
        LockHelper.tryLock(
                lock.readLock(),
                tryLockTimeout,
                "getting all behavior definitions in 'ClassBehaviourIndex.getAll()'");

        try {
            List<BehaviourDefinition> all =
                    new ArrayList<BehaviourDefinition>(classMap.size() + serviceMap.size());
    ...
    public Collection<BehaviourDefinition> find(B binding) {
        LockHelper.tryLock(
                lock.readLock(),
                tryLockTimeout,
                "searching behavior definitions list in 'ClassBehaviourIndex.find()'");
            ...
            if (isEnabled(binding)) {
                while (binding != null) {
                    ...
                    if (behaviour != null) {
                        behaviours.addAll(
                                0,
                                behaviour); // note: list base/generalised before extended/specific
                    }
    ...
    public void putClassBehaviour(BehaviourDefinition<B> behaviour) {
        LockHelper.tryLock(
                lock.writeLock(),
                tryLockTimeout,
                "putting behavior definition in 'ClassBehaviourIndex.putClassBehavior()'");
        try {
    ...
    public void putServiceBehaviour(BehaviourDefinition<ServiceBehaviourBinding> behaviour) {
        LockHelper.tryLock(
                lock.writeLock(),
                tryLockTimeout,
                "putting behavior definition in 'ClassBehaviourIndex.putServiceBehavior()'");
        try {
    ...
    public void removeClassBehaviour(BehaviourDefinition<B> behaviour) {
        LockHelper.tryLock(
                lock.writeLock(),
                tryLockTimeout,
                "removing behavior definition in 'ClassBehaviourIndex.removeClassBehavior()'");
        try {
    ...
    private boolean isEnabled(B binding) {
        ...
        if (filter != null) {
            ...
            isEnabled =
                    (nodeRef == null)
                            ? filter.isEnabled(className)
                            : filter.isEnabled(nodeRef, className);
        }
        return isEnabled;
    }
}

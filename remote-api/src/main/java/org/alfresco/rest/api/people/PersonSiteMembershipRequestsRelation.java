...
public class PersonSiteMembershipRequestsRelation
        implements RelationshipResourceAction.Read<SiteMembershipRequest>,
    ...
    public SiteMembershipRequest readById(String personId, String siteId, Parameters parameters)
            throws RelationshipResourceNotFoundException {
        SiteMembershipRequest siteInvite =
                siteMembershipRequests.getSiteMembershipRequest(personId, siteId);
        return siteInvite;
    }
    ...
    @WebApiDescription(
            title =
                    "Create a site membership request for personId and siteIds. The personId will"
                            + " be invited to the site as a SiteConsumer.")
    public List<SiteMembershipRequest> create(
            String personId, List<SiteMembershipRequest> invites, Parameters parameters) {
            ...
            if (invite.getClient() != null) {
                siteInvite =
                        siteMembershipRequests.createSiteMembershipRequest(
                                personId, invite, invite.getClient());
            } else {
    ...
    @WebApiDescription(
            title =
                    "Remove an existing site membership request for personId and siteId, if it"
                            + " exists.")
    ...
    public CollectionWithPagingInfo<SiteMembershipRequest> readAll(
            String personId, Parameters parameters) {
        return siteMembershipRequests.getPagedSiteMembershipRequests(
                personId, parameters.getPaging());
    }
    ...
    public SiteMembershipRequest update(
            String personId, SiteMembershipRequest siteInvite, Parameters parameters) {

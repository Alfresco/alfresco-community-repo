...
public class GetSiteMembershipFullTests extends RestTest {
  ...
  @TestRail(
    ...
    description = "Verify that properties parameter is applied."
  )
  public void checkThatPropertiesParameterIsApplied() throws Exception {
    restSiteEntry =
      restClient
        ...
        .withCoreAPI()
        .usingParams("properties=site,role,id")
        .usingUser(publicSiteUsers.getOneUserWithRole(UserRole.SiteManager))
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.PEOPLE },
    executionType = ExecutionType.REGRESSION,
    description = "Verify that regular user " +
  ...
  public void getSiteMembershipDetailsForACollaboratorRole() throws Exception {
    ...
    restSiteEntry
      .assertThat()
      .field("role")
      .is(UserRole.SiteCollaborator)
      .and()
      .field("guid")
      .is(publicSite.getGuid())
      .and()
      .field("id")
      .is(publicSite.getId())
      .and()
      .field("site.visibility")
      .is(publicSite.getVisibility().toString())
      .and()
      .field("site.guid")
      .is(publicSite.getGuid())
      .and()
      .field("site.description")
      .is(publicSite.getDescription())
      .and()
      .field("site.id")
      .is(publicSite.getId())
      .and()
  ...
  public void getSiteMembershipDetailsForAContributorRole() throws Exception {
    restSiteEntry =
      restClient
        .authenticateUser(regularUser)
        .withCoreAPI()
        .usingUser(publicSiteUsers.getOneUserWithRole(UserRole.SiteContributor))
        .getSiteMembership(publicSite);
    restClient.assertStatusCodeIs(HttpStatus.OK);
    restSiteEntry
      .assertThat()
      .field("role")
      .is(UserRole.SiteContributor)
      .and()
      .field("id")
      .is(publicSite.getId())
      .and()
  ...
  public void getSiteMembershipDetailsForAConsumerRole() throws Exception {
    restSiteEntry =
      restClient
        .authenticateUser(regularUser)
        .withCoreAPI()
        .usingUser(publicSiteUsers.getOneUserWithRole(UserRole.SiteConsumer))
        .getSiteMembership(publicSite);
    restClient.assertStatusCodeIs(HttpStatus.OK);
    restSiteEntry
      .assertThat()
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.PEOPLE },
    executionType = ExecutionType.REGRESSION,
    description = "As regular user make a request to join a moderated site." +
  ...
  public void pendingRequestToASiteCheckMembershipDetails() throws Exception {
    ...
    restSiteEntry =
      restClient
        ...
        .getSiteMembership(moderatedSite);
    restClient
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.PEOPLE },
    executionType = ExecutionType.REGRESSION,
    description = "As Collaborator user leave site and perform get call." +
    "Check default error model schema."
  )
  public void leaveSiteAndPerformGetCallCheckDefaultErrorModelSchema()
    throws Exception {
    ...
    restClient
      .authenticateUser(leaveSiteUser)
      .withCoreAPI()
      .usingAuthUser()
      .deleteSiteMember(publicSite);

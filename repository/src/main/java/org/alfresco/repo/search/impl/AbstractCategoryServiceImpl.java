...
public abstract class AbstractCategoryServiceImpl implements CategoryService {
  ...
  public Collection<ChildAssociationRef> getChildren(
    NodeRef categoryRef,
    Mode mode,
    Depth depth
  ) {
  ...
  private Collection<ChildAssociationRef> getChildren(
    NodeRef categoryRef,
    Mode mode,
    Depth depth,
    boolean sortByName,
    ...
    try {
      ...
      switch (mode) {
        case ALL:
          ...
          luceneQuery
            .append(buildXPath(nodeService.getPath(categoryRef)))
            .append("/");
        ...
        case SUB_CATEGORIES:
          ...
          luceneQuery.append("*").append("\" ");
          luceneQuery.append(
            "+TYPE:\"" + ContentModel.TYPE_CATEGORY.toString() + "\""
          );
          break;
      }
      if (filter != null) {
      ...
      SearchService searcher = indexerAndSearcher.getSearcher(
        categoryRef.getStoreRef(),
        true
      );
  ...
  private String buildXPath(Path path) {
    ...
    for (Iterator<Path.Element> elit = path.iterator(); elit.hasNext();/**/) {
      ...
      if (cae.getRef().getParentRef() != null) {
        ...
        pathBuffer.append(
          ISO9075.encode(cae.getRef().getQName().getLocalName())
        );
      }
    }
  ...
  private Collection<ChildAssociationRef> resultSetToChildAssocCollection(
    ResultSet resultSet
  ) {
    ...
    if (resultSet != null) {
      for (ResultSetRow row : resultSet) {
        try {
          ChildAssociationRef car = nodeService.getPrimaryParent(
            row.getNodeRef()
          );
          collection.add(car);
  ...
  protected Set<NodeRef> getClassificationNodes(
    ...
    try {
      resultSet =
        indexerAndSearcher
          .getSearcher(storeRef, false)
          .query(
            storeRef,
            "lucene",
            "PATH:\"/" +
            getPrefix(qname.getNamespaceURI()) +
            ISO9075.encode(qname.getLocalName()) +
            "\"",
            null
          );
  ...
  public Collection<ChildAssociationRef> getClassifications(StoreRef storeRef) {
    ...
    try {
      resultSet =
        indexerAndSearcher
          .getSearcher(storeRef, false)
          .query(storeRef, "lucene", "PATH:\"//cm:categoryRoot/*\"", null);
      return resultSetToChildAssocCollection(resultSet);
    } finally {
      if (resultSet != null) {
        resultSet.close();
  ...
  public Collection<QName> getClassificationAspects() {
    return dictionaryService.getSubAspects(
      ContentModel.ASPECT_CLASSIFIABLE,
      true
    );
  }

  public NodeRef createClassification(
    StoreRef storeRef,
    QName typeName,
  ...
  public PagingResults<ChildAssociationRef> getRootCategories(
    ...
    OUTER:for (NodeRef nodeRef : nodeRefs) {
      Collection<ChildAssociationRef> children = getChildren(
        nodeRef,
        Mode.SUB_CATEGORIES,
        Depth.IMMEDIATE,
        sortByName,
        filter,
        skipCount + maxItems + 1
      );
  ...
  public ChildAssociationRef getCategory(
    NodeRef parent,
    QName aspectName,
    String name
  ) {
    String uri = nodeService
      .getPrimaryParent(parent)
      .getQName()
      .getNamespaceURI();
    String validLocalName = QName.createValidLocalName(name);
    Collection<ChildAssociationRef> assocs = nodeService.getChildAssocs(
      parent,
  ...
  private ChildAssociationRef createCategoryInternal(
    NodeRef parent,
    String name
  ) {
    ...
    String uri = nodeService
      ...
      .getNamespaceURI();
    String validLocalName = QName.createValidLocalName(name);
    ChildAssociationRef newCategory = publicNodeService.createNode(
      parent,
      ContentModel.ASSOC_SUBCATEGORIES,
      QName.createQName(uri, validLocalName),
  ...
  public NodeRef createRootCategory(
    StoreRef storeRef,
    QName aspectName,
    String name
  ) {
    ...
    if (nodeRefs.size() == 0) {
      throw new AlfrescoRuntimeException(
        "Missing classification: " + aspectName
      );
    }
  ...
  public abstract List<Pair<NodeRef, Integer>> getTopCategories(
    StoreRef storeRef,
    QName aspectName,
    int count
  );

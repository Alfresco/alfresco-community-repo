    ...
    public void setup(boolean createTestData) throws Exception {
        ...
        this.transactionHelper =
                (RetryingTransactionHelper)
                        repoService.getApplicationContext().getBean("retryingTransactionHelper");
        ...
        for (final TestNetwork testAccount : getNetworks().values()) {
            ...
            transactionHelper.doInTransaction(
                    new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                        @SuppressWarnings("synthetic-access")
                        public Void execute() throws Throwable {
                            // clear fully authenticated user ("admin") - affects site creation
                            // (which uses this to setup perms)
                            AuthenticationUtil.clearCurrentSecurityContext();

                            if (testAccount.getPersonIds().size() > 0) {
                                // use a fixed sample size of account members (so we have some left
                                // over for tests)
                                List<String> people =
                                        testAccount.peopleSample(testAccount.getPersonIds().size());

                                String tenantDomain = testAccount.getId();

                                int i = 0;
                                for (final SiteInformation site : getSites().values()) {
                                    final Iterator<String> peopleIterator =
                                            RepoService.getWrappingIterator(i++, people);
                                    ...
                                    TenantUtil.runAsUserTenant(
                                            new TenantRunAsWork<Void>() {
                                                ...
                                                public Void doWork() throws Exception {
                                                    final TestSite testSite =
                                                            testAccount.createSite(site);

                                                    // invite some members to the site, leave at
                                                    // least one non-site member
                                                    for (int j = 0; j < numMembersPerSite; j++) {
                                                        String siteMember = peopleIterator.next();
                                                        assertNotNull(siteMember);
                                                        testSite.inviteToSite(
                                                                siteMember,
                                                                SiteRole.SiteContributor);
                                                    }

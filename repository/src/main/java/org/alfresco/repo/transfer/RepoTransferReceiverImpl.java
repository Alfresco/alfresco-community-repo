...
public class RepoTransferReceiverImpl
  ...
  private static final String MSG_LOCK_TIMED_OUT =
    "transfer_service.receiver.lock_timed_out";
  private static final String MSG_LOCK_NOT_FOUND =
    "transfer_service.receiver.lock_not_found";
  ...
  private final String KEY_INBOUND_TRANSFER_RECORDS_NODEREF =
    "key.inboundTransferRecordsFolder.noderef"; // where the destination side transfer report is generated
  ...
  public File getStagingFolder(String transferId) {
    ...
    String tempFolderPath =
      rootStagingDirectory + "/" + transferNodeRef.getId();
  ...
  public NodeRef getTempFolder(String transferId) {
    ...
    QName folderName = QName.createQName(
      NamespaceService.APP_MODEL_1_0_URI,
      tempTransferFolderName
    );

    // Do we already have a temp folder for this transfer?
    List<ChildAssociationRef> tempChildren = nodeService.getChildAssocs(
      transferTempFolder,
  ...
  public String start(
    ...
    if (
      !getTransferVersionChecker().checkTransferVersions(fromVersion, toVersion)
    ) {
      throw new TransferException(
        MSG_INCOMPATIBLE_VERSIONS,
        new Object[] { "None", fromVersion, toVersion }
      );
    }
     ...
     */
    String tenantDomain = tenantService.getUserDomain(
      AuthenticationUtil.getRunAsUser()
    );
    String lockStr = tenantDomain.isEmpty()
      ? "transfer.server.default"
      : "transfer.server.tenant." + tenantDomain;
    QName lockQName = QName.createQName(
    ...
    try {
      TransferServicePolicies.BeforeStartInboundTransferPolicy beforeStartPolicy = beforeStartInboundTransferDelegate.get(
          ...
          txHelper.doInTransaction(
            new RetryingTransactionHelper.RetryingTransactionCallback<String>() {
              public String execute() throws Throwable {
                ...
                TransferServicePolicies.OnStartInboundTransferPolicy onStartPolicy = onStartInboundTransferDelegate.get(
                  TransferModel.TYPE_TRANSFER_RECORD
                );
  ...
  private NodeRef createTransferRecord() {
    ...
    NodeRef inboundTransferRecordsFolder = singletonCache.get(
      KEY_INBOUND_TRANSFER_RECORDS_NODEREF
    );

    if (inboundTransferRecordsFolder == null) {
      log.debug(
        "Trying to find transfer records folder: " + inboundTransferRecordsPath
      );
      List<NodeRef> refs = searchService.selectNodes(
        nodeService.getRootNode(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE),
        inboundTransferRecordsPath,
      ...
      if (refs.size() > 0) {
        ...
        log.debug(
          "Found inbound transfer records folder: " +
          inboundTransferRecordsFolder
        );
      } else {
        throw new TransferException(
    ...
    QName recordName = QName.createQName(
      NamespaceService.APP_MODEL_1_0_URI,
      name
    );
    ...
    props.put(
      ...
      TransferProgress.Status.PRE_COMMIT.toString()
    );
  ...
  private void timeout(final String transferId) {
    ...
    final RetryingTransactionCallback<Void> timeoutCB = new RetryingTransactionCallback<Void>() {
      public Void execute() throws Throwable {
        TransferProgress progress = getProgressMonitor()
        ...
        if (progress.getStatus().equals(TransferProgress.Status.PRE_COMMIT)) {
          ...
          log.warn(
            "Inbound Transfer Lock Timeout - already past PRE-COMMIT - do no cleanup transferId:" +
            transferId
          );
        }
        return null;
  ...
  public void end(final String transferId) {
    ...
    try {
      ...
      TransferChangesRecord changesRecord = progressMonitor.removeChangeRecord(
        transferId
      );
      ...
      TransferServicePolicies.OnEndInboundTransferPolicy onEndPolicy = onEndInboundTransferDelegate.get(
        TransferModel.TYPE_TRANSFER_RECORD
      );
      onEndPolicy.onEndInboundTransfer(
        transferId,
        createdNodes,
        updatedNodes,
        deletedNodes
      ...
      throw new TransferException(
        MSG_ERROR_WHILE_ENDING_TRANSFER,
        new Object[] { transferId },
        ex
      );
    }
  ...
  public void cancel(String transferId) throws TransferException {
    ...
    getProgressMonitor()
      .updateStatus(transferId, TransferProgress.Status.CANCELLED);
    if (progress.getStatus().equals(TransferProgress.Status.PRE_COMMIT)) {
      end(transferId);
    }
  }
  ...
  public void saveSnapshot(String transferId, InputStream openStream)
    ...
    try {
      ...
      try {
        ...
        throw new TransferException(
          MSG_ERROR_WHILE_STAGING_SNAPSHOT,
          new Object[] { transferId },
          ex
        );
      }
  ...
  public void saveContent(
    ...
    try {
      ...
      if (stagedFile.createNewFile()) {
        FileCopyUtils.copy(
          contentStream,
          new BufferedOutputStream(new FileOutputStream(stagedFile))
        );
      }
    } catch (Exception ex) {
      throw new TransferException(
        MSG_ERROR_WHILE_STAGING_CONTENT,
        new Object[] { transferId, contentFileId },
        ex
      );
    } finally {
      lock.enableLockTimeout();
  ...
  public void commitAsync(String transferId) {
    ...
    try {
      ...
      Action commitAction = actionService.createAction(
        TransferCommitActionExecuter.NAME
      );
      commitAction.setParameterValue(
        TransferCommitActionExecuter.PARAM_TRANSFER_ID,
        transferId
      );
      ...
      if (log.isDebugEnabled()) {
        log.debug(
          "Registered transfer commit for asynchronous execution: " + transferId
        );
      }
  ...
  public void commit(final String transferId) throws TransferException {
    ...
    try {
      ...
      RetryingTransactionHelper.RetryingTransactionCallback<Object> commitWork = new RetryingTransactionCallback<Object>() {
        public Object execute() throws Throwable {
          ...
          if (snapshotFile.exists()) {
            if (log.isDebugEnabled()) {
              log.debug(
                "Processing manifest file:" + snapshotFile.getAbsolutePath()
              );
            }
            ...
            for (TransferManifestProcessor processor : commitProcessors) {
              ...
              if (processor instanceof TransferSummaryAware) {
                summaryReports.add(
                  ((TransferSummaryAware) processor).getTransferSummaryReport()
                );
              }
              try {
      ...
      try {
        ...
        log.error(
          "Failed to clean up transfer. Lock may still be in place: " +
          transferId,
          ex
        );
      }
  ...
  public TransferProgress getStatus(String transferId)
    throws TransferException {
   ...
   */
  public void setManifestProcessorFactory(
    ManifestProcessorFactory manifestProcessorFactory
  ) {
   ...
   */
  public void generateRequsite(String transferId, OutputStream out)
    throws TransferException {
  ...
  public void onMoveNode(
    ...
    if (newParentRef.equals(oldParentRef)) {
      log.debug(
        "old parent and new parent are the same - this is a rename, do nothing"
      );
    } else {
      if (log.isDebugEnabled()) {
        log.debug(
          "moving node from oldParentRef:" +
          oldParentRef +
          " to:" +
          newParentRef
        );
      }
      alienProcessor.beforeDeleteAlien(
   ...
   */
  public CopyBehaviourCallback onCopyTransferred(
    QName classRef,
   ...
   */
  public CopyBehaviourCallback onCopyAlien(
    QName classRef,
    CopyDetails copyDetails
  ) {
  ...
  private class Lock implements JobLockService.JobLockRefreshCallback {
    ...
    public synchronized void makeLock() {
      ...
      getJobLockService()
        .refreshLock(lockToken, lockQName, getLockRefreshTime(), this);
      if (log.isDebugEnabled()) {

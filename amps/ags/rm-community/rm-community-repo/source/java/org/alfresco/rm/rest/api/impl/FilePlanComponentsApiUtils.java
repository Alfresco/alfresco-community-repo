...
public class FilePlanComponentsApiUtils {
  ...
  public NodeRef lookupAndValidateNodeType(
    ...
    return lookupAndValidateNodeType(
      nodeId,
      expectedNodeType,
      relativePath,
      false
   ...
   * @return
   */
  public NodeRef validateAndLookUpContainerNode(
    String nodeId,
   ...
   */
  public NodeRef lookupByAllowedPlaceholders(
    String nodeId,
    List<String> allowedPlaceholders
  ) {
    NodeRef nodeRef;
  ...
  public List<Pair<QName, Boolean>> getSortProperties(Parameters parameters) {
    ...
    sortProps.add(
      new Pair<>(GetChildrenCannedQuery.SORT_QNAME_NODE_TYPE, true)
    );
  ...
  public NodeRef lookupAndValidateRelativePath(
    ...
    return lookupAndValidateRelativePath(
      parentNodeRef,
      relativePath,
      false,
      nodesType
  ...
  public NodeRef lookupAndValidateRelativePath(
    ...
    for (; i < pathElements.size(); i++) {
      ...
      NodeRef child = authenticationUtil.runAsSystem(
    ...
    if (nodesType.equals(RecordsManagementModel.TYPE_UNFILED_RECORD_FOLDER)) {
      ...
      for (String pathElement : pathElements) {
        lastNodeRef =
          filePlanService.createRecordCategory(lastNodeRef, pathElement);
      }
    } else {
  ...
  public NodeRef createRMNode(
    ...
    boolean autoRename = Boolean.valueOf(
      parameters.getParameter(RMNode.PARAM_AUTO_RENAME)
    );
    ...
    try {
      ...
      newNodeRef =
        fileFolderService
          .create(parentNodeRef, nodeName, typeQName)
          .getNodeRef();

      // Set the provided properties if any
      Map<QName, Serializable> qnameProperties = mapToNodeProperties(
      ...
      throw new InvalidArgumentException(
        "The given type:'" + nodeType + "' is invalid '"
      );
  ...
  public List<FilterProp> getListChildrenFilterProps(
    ...
    if (q != null) {
      ...
      Boolean isClosed = propertyWalker.getProperty(
        RMNode.PARAM_IS_CLOSED,
        WhereClauseParser.EQUALS,
        Boolean.class
      );
      if (isClosed != null) {
   ...
   * @return
   */
  public NodeRef validateRecord(String recordId)
   ...
   */
  protected List<String> getAllowableOperations(
    NodeRef nodeRef,
    QName typeQName
  ) {
    List<String> allowableOperations = new ArrayList<>();

    boolean isFilePlan = typeQName.equals(
      RecordsManagementModel.TYPE_FILE_PLAN
    );
    boolean isTransferContainer = typeQName.equals(
      RecordsManagementModel.TYPE_TRANSFER_CONTAINER
    );
    boolean isUnfiledContainer = typeQName.equals(
      RecordsManagementModel.TYPE_UNFILED_RECORD_CONTAINER
    );
    boolean isHoldsContainer = typeQName.equals(
      RecordsManagementModel.TYPE_HOLD_CONTAINER
    );
    ...
    // CREATE
    if (
      TYPES_CAN_CREATE.contains(typeQName) &&
      capabilityService
        .getCapability("FillingPermissionOnly")
        .evaluate(nodeRef) ==
      AccessDecisionVoter.ACCESS_GRANTED
    ) {
    ...
    // UPDATE
    if (
      capabilityService.getCapability("Update").evaluate(nodeRef) ==
      AccessDecisionVoter.ACCESS_GRANTED
    ) {
  ...
  public QName getFilePlanType() {
    NodeRef filePlanNodeRef = filePlanService.getFilePlanBySiteId(
      FilePlanService.DEFAULT_RM_SITE_ID
    );
  ...
  public void postActivity(
    ...
    if (rmSite != null && !rmSite.getId().equals("")) {
      if (fileInfo != null) {
        boolean isContent = dictionaryService.isSubClass(
          fileInfo.getType(),
          ContentModel.TYPE_CONTENT
        );
        ...
        if (isContent) {
          activityInfo =
            new ActivityInfo(null, parentNodeRef, RM_SITE_ID, fileInfo);
        }
      }
    } else {

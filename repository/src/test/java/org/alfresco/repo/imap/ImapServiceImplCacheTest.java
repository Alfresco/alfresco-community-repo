...
public class ImapServiceImplCacheTest extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    final ImapServiceImpl imapServiceImpl = (ImapServiceImpl) imapCtx.getBean(
      "imapService"
    );
    ...
    FileInfo folder = FileFolderUtil.makeFolders(
      fileFolderService,
      companyHomeNodeRef,
      folders,
  ...
  public void testRepoBehaviourWithFoldersCache() throws Exception {
    AlfrescoImapUser localUser = new AlfrescoImapUser(
    ...
    FileInfo localRootFolder = fileFolderService.create(
    ...
    for (int i = 0; i < 3; i++) {
      ...
      FileInfo subFolder = fileFolderService.create(
        localRootFolder.getNodeRef(),
        childMailbox,
        ContentModel.TYPE_FOLDER
      );
      for (int j = 0; j < 3; j++) {
    ...
    AlfrescoImapFolder folder = imapService.getOrCreateMailbox(
      localUser,
      mailbox,
      true,
      false
    ...
    fileFolderService.move(
      oldFile.getNodeRef(),
      folder.getFolderInfo().getNodeRef(),
      folder.getName()
    );
    ...
    assertTrue(
      "UIDVALIDITY wasn't incremented",
      (uidValidity - uidValidityBefore) > 0
    );
    ...
    try {
      String subFolderName =
        mailbox + AlfrescoImapConst.HIERARCHY_DELIMITER + folderName + "_0";
      folder =
        imapService.getOrCreateMailbox(localUser, subFolderName, true, false);
    ...
    assertTrue(
      "Messages cache is stale",
      contentItemsCount > folder.getMessageCount()
    );
    long[] uids = folder.getMessageUids();
    Arrays.sort(uids);
    assertFalse(
      "Messages msn cache is stale",
      Arrays.binarySearch(uids, uid) > 0
    );
    assertNull(
      "Message is still in the messages cache",
  ...
  private List<FileInfo> createTestContent(FileInfo parent, int count) {
    ...
    for (int i = 0; i < count; i++) {
      FileInfo contentItem = fileFolderService.create(
        parent.getNodeRef(),
        "content_" + i,
        ContentModel.TYPE_CONTENT,

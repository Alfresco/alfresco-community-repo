...
public class PublicAPITestUtil {
  ...
  public static void testPublicAPIConsistency(
    ...
    Set<Class<?>> publicAPIClasses = reflections.getTypesAnnotatedWith(
      AlfrescoPublicApi.class,
      true
    );
    ...
    for (Class<?> publicAPIClass : publicAPIClasses) {
      Set<Class<?>> referencedClassesFromClass = getReferencedClassesFromClass(
        publicAPIClass,
        new HashSet<>()
      );
      referencedClassesFromClass.forEach(clazz ->
        referencedFrom.put(clazz, publicAPIClass)
      );
    ...
    for (Class<?> referencedClass : referencedClasses) {
      if (
        isInAlfresco(referencedClass) && !isPartOfPublicApi(referencedClass)
      ) {
        Set<String> referencerNames = referencedFrom
          .get(referencedClass)
          .stream()
          .map(c -> c.getName())
   ...
   * @return The set of classes.
   */
  private static Set<Class<?>> getClassesFromConstructor(
   ...
   */
  private static Set<Class<?>> getClassesFromType(
    Type type,

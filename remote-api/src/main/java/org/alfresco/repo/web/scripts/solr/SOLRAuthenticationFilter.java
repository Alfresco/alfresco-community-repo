...
public class SOLRAuthenticationFilter
  implements DependencyInjectedFilter, InitializingBean {
  ...
  private String sharedSecretHeader =
    HttpClientFactory.DEFAULT_SHAREDSECRET_HEADER;
  ...
  public void afterPropertiesSet() throws Exception {
    if (secureComms == SecureCommsType.SECRET) {
      if (sharedSecret == null || sharedSecret.length() == 0) {
        logger.fatal(
          "Missing value for solr.sharedSecret configuration property. If solr.secureComms is set to \"secret\", a value for solr.sharedSecret is required. See https://docs.alfresco.com/search-services/latest/install/options/"
        );
        throw new AlfrescoRuntimeException(
          "Missing value for solr.sharedSecret configuration property"
        );
      }

...
public abstract class AbstractPatch
  implements Patch, ApplicationEventPublisherAware {
  ...
  public static final String ERR_PROPERTY_NOT_SET =
    "patch.general.property_not_set";
  private static final String MSG_PROGRESS = "patch.progress";
  private static final String MSG_DEFERRED =
    "patch.genericBootstrap.result.deferred";
  ...
  public void init() {
    if (patchService == null) {
      throw new AlfrescoRuntimeException(
        "Mandatory property not set: patchService"
      );
    }
    if (false == isIgnored()) {
  ...
  private String applyWithTxns() throws Exception {
    ...
    if (requiresTransaction()) {
      ...
      String temp =
        this.transactionHelper.doInTransaction(patchWork, false, true);
      sb.append(temp);
    } else {
    ...
    if (
      ...
      BatchProcessWorker<Tenant> worker = new BatchProcessWorker<Tenant>() {
        ...
        public void process(Tenant entry) throws Throwable {
          ...
          String tenantReport = AuthenticationUtil.runAs(
  ...
  private String apply(boolean async) {
    ...
    try {
      ...
      String report = AuthenticationUtil.runAs(
        applyPatchWork,
        AuthenticationUtil.getSystemUserName()
      );
  ...
  protected void reportProgress(long estimatedTotal, long currentInteration) {
    ...
    if (currentInteration == 0) {
      ...
      if (percentComplete < 100) {
        ...
        long timeRemaining =
          timeSoFar * (100 - percentComplete) / percentComplete;

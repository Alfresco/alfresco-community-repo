...
public class RepositoryContainer extends AbstractRuntimeContainer {
   ...
   */
  public void setRepositoryImageResolver(
   ...
   */
  public void setFallbackTransactionHelper(
    RetryingTransactionHelper fallbackTransactionHelper
  ) {
  ...
  public void setNotPublicExceptions(List<Class<?>> notPublicExceptions) {
    ...
    if ((null != notPublicExceptions) && !notPublicExceptions.isEmpty()) {
      this.notPublicExceptions =
        notPublicExceptions.toArray(this.notPublicExceptions);
    }
  }
    ...
    return fallbackTransactionHelper.doInTransaction(
      () -> {
        Map<String, Object> params = new HashMap<>(
          RepositoryContainer.super.getTemplateParameters()
        );
        params.put(
  ...
  private void addRepoParameters(Map<String, Object> params) {
    if (
      AlfrescoTransactionSupport.getTransactionId() != null &&
    ...
    final RequiredAuthentication required =
      (
        desc
            ...
            .compareTo(containerRequiredAuthentication) <
          0 &&
  ...
  private void checkGuestAccess(
    ...
    String scriptDescriptorId
  ) {
    if (
      required == RequiredAuthentication.user ||
  ...
  private void checkScriptAccess(
    ...
    if (
      ...
      required == RequiredAuthentication.admin && !isAdminOrSystemUser()
    ) {
      throw new WebScriptException(
        HttpServletResponse.SC_UNAUTHORIZED,
        "Web Script " +
  ...
  protected void transactionedExecute(
    ...
    try (
      final BufferedRequest bufferedReq = newBufferedRequest(
        trxParams,
        scriptReq,
        streamFactory
      );
      final BufferedResponse bufferedRes = newBufferedResponse(
        trxParams,
        scriptRes,
        streamFactory
      )
  ...
  private static void handleIOException(final IOException ioe)
    throws IOException {
    Throwable socketException = ExceptionStackUtil.getCause(
      ioe,
  ...
  private void transactionedExecuteAs(
    ...
    if (
      requiredAuthentication == RequiredAuthentication.sysadmin &&
      isSysAdminUser() &&
      !isAdmin()
    ) {
      AuthenticationUtil.runAs(
  ...
  private static BufferedResponse newBufferedResponse(
    ...
    if (trxParams.getBufferSize() <= 0) {
      if (logger.isDebugEnabled()) logger.debug(
        "Transactional Response bypassed for ReadWrite - buffersize=0"
      );
      return null;
    }
    if (logger.isDebugEnabled()) logger.debug(

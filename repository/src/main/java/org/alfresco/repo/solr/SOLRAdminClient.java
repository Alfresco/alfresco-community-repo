...
/**
 * Provides an interface to the Solr admin APIs, used by the Alfresco Enterprise JMX layer. Also
 * tracks whether Solr is available, sending Spring events when its availability changes.
 *
 * @since 4.0
 */
public class SOLRAdminClient extends AbstractSolrAdminHTTPClient
    ...
    public JSONAPIResult executeAction(
        ...
        if (mapping.isSharded()) {
            ...
            String solrurl =
                    httpClient.getHostConfiguration().getHostURL()
                            + mapping.getHttpClientAndBaseUrl().getSecond();
            url.append(solrurl);
        }
        if (core != null) {
        ...
        try {
            ...
            return JSONAPIResultFactory.buildActionResult(
                    action, getOperation(httpClient, url.toString()));
    ...
    public JSONAPIResult executeCommand(
        ...
        try {
            ...
            JSONAPIResult response =
                    new SolrCommandBackupResult(getOperation(httpClient, url.toString()));
    ...
    class SolrTracker {
        ...
        protected void pingSolr() {
            ...
            SolrActionStatusResult result =
                    (SolrActionStatusResult)
                            executeAction(null, JSONAPIResultFactory.ACTION.STATUS, JSON_PARAM);
        ...
        protected void setupTimer() {
            try {
                ...
                // If a Quartz job already exists with this name and group then we want to replace
                // it.
                // It is not expected that this will occur during production, but it is possible
                // during automated testing
                // where application contexts could be rebuilt between test cases, leading to
                // multiple creations of
                // equivalent Quartz jobs. Quartz disallows the scheduling of multiple jobs with the
                // same name and group.
                ...
                trigger =
                        TriggerBuilder.newTrigger()
                                .withIdentity("rmt")
                                .withSchedule(
                                        CronScheduleBuilder.cronSchedule(solrPingCronExpression))
                                .build();

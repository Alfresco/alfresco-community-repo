 ...
 *
 * <p>In general, we don't want developers having to think about http status codes or rendering
 * responses to the user. This exception hierarchy is designed to abstract the http logic away so
 * the developer can concentrate on implementing services logic. The framework will attempt to
 * render errors in a consistent way.
 *
 * <p>Default status is STATUS_INTERNAL_SERVER_ERROR = 500.
 *
 * @author Gethin James
 */
public class ApiException extends PlatformRuntimeException {
    ...
    /**
     * A free-form object that contains any additional state that the developer thinks might be
     * relevant for troubleshooting. This object will be rendered as JSON.
     */

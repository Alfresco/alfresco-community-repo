...
/**
 * The public API for a permission service The implementation may be changed in the application
 * configuration
 *
 * @author Andy Hind
 */
...
public interface PermissionService {
    /**
     * Prefixes used for authorities of type role. This is intended for external roles, e.g. those
     * set by ACEGI implementations It is only used for admin at the moment - which is done outside
     * the usual permission assignments at the moment. It could be a dynamic authority.
     */
    ...
    /**
     * Get all the AccessPermissions that are granted/denied to the current authentication for the
     * given node
     *
    ...
    /**
     * Check that the given authentication has a particular permission for the given node. (The
     * default behaviour is to inherit permissions)
     *
     ...
     *
     * <p>caveats: doesn't take into account dynamic authorities/groups doesn't take into account
     * node types/aspects for permissions
     *
     * @param nodeRef - the reference to the node
     * @return access status
     */
    ...
    /**
     * Find and delete a access control entry by node, authentication and permission. It is possible
     * to delete
     *
     * <ol>
     *   <li>a specific permission;
     *   <li>all permissions for an authority (if the permission is null);
     *   <li>entries for all authorities that have a specific permission (if the authority is null);
     *       and
    ...
    /**
     * Set the global inheritance behavior for permissions on a node. If the operation takes too
     * long and asyncCall parameter set accordingly, fixed ACLs method will be asynchronously
     * called.
     *
     * @param nodeRef node for which inheritance will be set.
     * @param inheritParentPermissions <tt>true</tt> to inherit parent permissions, <tt>false</tt>
     *     otherwise.
     * @param asyncCall <tt>true</tt> if fixed ACLs should be asynchronously set when operation
     *     execution takes too long, <tt>false</tt> to execute synchronously regardless of execution
     *     time.
     */
    ...
    public void setPermission(
            StoreRef storeRef, String authority, String permission, boolean allow);
     ...
     * Get all the AccessPermissions that are set for anyone for the given node
     *
     * @param storeRef - the reference to the store
     * @return the set of allowed permissions
     */
    @Auditable(parameters = {"storeRef"})

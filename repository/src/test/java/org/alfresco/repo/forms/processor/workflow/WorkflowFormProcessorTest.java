...
public class WorkflowFormProcessorTest extends FormProcessorTest {
  ...
  public void testGetTypedItem() throws Exception {
    ...
    WorkflowDefinition result =
      ((WorkflowFormProcessor) processor).getTypedItem(item);
  ...
  public void testGenerateSetsItemAndUrl() throws Exception {
    Form form =
      ((WorkflowFormProcessor) processor).generate(item, null, null, null);
    ...
    assertEquals(
      "api/workflow-definitions/" + definition.getId(),
      formItem.getUrl()
    );
  }
  ...
  public void testIgnoresUnknownFields() throws Exception {
    String fakeFieldName =
      NamespaceService.BPM_MODEL_PREFIX + ":" + "Fake Field";
  ...
  public void testGenerateDefaultForm() throws Exception {
    ...
    assertTrue(
      fieldDefs.contains(ASSIGNEE_NAME.toPrefixString(namespaceService))
    );
    assertTrue(
      fieldDefs.contains(PRIORITY_NAME.toPrefixString(namespaceService))
    );
    ...
    assertFalse(
      fieldDefs.contains(
        PROP_PACKAGE_ACTION_GROUP.toPrefixString(namespaceService)
      )
    );
    assertFalse(
      fieldDefs.contains(
        PROP_PACKAGE_ITEM_ACTION_GROUP.toPrefixString(namespaceService)
      )
    );
    assertFalse(
      fieldDefs.contains(
        WorkflowModel.PROP_DESCRIPTION.toPrefixString(namespaceService)
      )
    );
    assertFalse(
      fieldDefs.contains(
        WorkflowModel.PROP_DUE_DATE.toPrefixString(namespaceService)
      )
    );
    assertFalse(
      fieldDefs.contains(
        WorkflowModel.PROP_PRIORITY.toPrefixString(namespaceService)
      )
    );
    assertFalse(
      fieldDefs.contains(
    ...
    assertEquals(
      fieldData,
      formData.getFieldData("assoc_bpm_assignee").getValue()
    );
    checkPackageActionGroups(formData);
    assertEquals(
      2,
      formData.getFieldData("prop_bpm_workflowPriority").getValue()
    );
  }
  ...
  private void processPersist(String dataKey, String value) {
    ...
    WorkflowInstance persistedItem = (WorkflowInstance) processor.persist(
      item,
      data
    );
  ...
  private Form processForm(List<String> fields) {
    return ((WorkflowFormProcessor) processor).generate(
        item,
        fields,
        null,
        null
  ...
  private void checkPackageActionGroups(FormData formData) {
    FieldData pckgActionData = formData.getFieldData(
      "prop_bpm_packageActionGroup"
    );
    assertNotNull(pckgActionData);
    assertEquals("add_package_item_actions", pckgActionData.getValue());
    FieldData pckgItemActionData = formData.getFieldData(
      "prop_bpm_packageItemActionGroup"
    );
  ...
  protected void setUp() throws Exception {
    ...
    MockFieldProcessorRegistry fieldProcessorRegistry = new MockFieldProcessorRegistry(
      namespaceService,
      dictionaryService
    );
    DefaultFieldProcessor defaultProcessor = super.makeDefaultFieldProcessor(
      dictionaryService
    );
    super.processor =
      makeWorkflowFormProcessor(
        dictionaryService,
  ...
  private WorkflowFormProcessor makeWorkflowFormProcessor(
    ...
    processor1 =
      (WorkflowFormProcessor) super.makeTaskFormProcessor(
        processor1,
        dictionaryService,
        fieldProcessorRegistry,
        defaultProcessor
  ...
  private Map<QName, PropertyDefinition> makeTaskPropertyDefs() {
    ...
    MockClassAttributeDefinition priorityDef = MockClassAttributeDefinition.mockPropertyDefinition(
      PRIORITY_NAME,
      intType,
      "2"
    );
    ...
    PropertyDefinition descValue = MockClassAttributeDefinition.mockPropertyDefinition(
      DESC_NAME,
      textType
    );
    ...
    PropertyDefinition titleValue = MockClassAttributeDefinition.mockPropertyDefinition(
      STATUS_NAME,
      textType
    );
    ...
    // Add a Status property
    PropertyDefinition with_ = MockClassAttributeDefinition.mockPropertyDefinition(
      PROP_WITH_,
      textType
    );
    ...
    PropertyDefinition pckgAction = MockClassAttributeDefinition.mockPropertyDefinition(
      pckgActionGroup,
      textType,
    ...
    PropertyDefinition pckgItemAction = MockClassAttributeDefinition.mockPropertyDefinition(
      pckgItemActionGroup,
      textType,
  ...
  private NamespaceService makeNamespaceService() {
    ...
    nsService.registerNamespace(
      NamespaceService.SYSTEM_MODEL_PREFIX,
      NamespaceService.SYSTEM_MODEL_1_0_URI
    );
    nsService.registerNamespace(
      NamespaceService.RENDITION_MODEL_PREFIX,
  ...
  private DictionaryService makeDictionaryService() {
    ...
    TypeDefinition taskTypeDef = definition
      .getStartTaskDefinition()
      .getMetadata();
    when(mock.getAnonymousType((QName) any(), (Collection<QName>) any()))
      .thenReturn(taskTypeDef);
    return mock;
  }
  ...
  private WorkflowService makeWorkflowService() {
    ...
    when(service.startWorkflow(eq(definition.getId()), anyMap()))
      .thenAnswer(
        new Answer<WorkflowPath>() {
          public WorkflowPath answer(InvocationOnMock invocation)
            throws Throwable {

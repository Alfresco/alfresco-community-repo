...
/**
 * An assignment handler for the request info workflow. An RM manager/admin can select one or more
 * user(s) and/or one or more group(s) when starting the request info workflow. This assignment
 * handler assigns for everyone a task (it is a pooled task).
 *
 * @author Tuna Aksoy
 * @since 2.1
 */
public class RequestInfoAssignmentHandler implements TaskListener {
    ...
    public void notify(DelegateTask delegateTask) {
        ...
        ActivitiScriptNodeList usersAndGroups =
                (ActivitiScriptNodeList) delegateTask.getVariable("rmwf_mixedAssignees");

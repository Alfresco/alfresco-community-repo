...
public class CommentsPost extends AbstractCommentsWebScript {
  ...
  private NodeRef addComment(NodeRef nodeRef, JSONObject json) {
    ...
    // create the comment
    NodeRef commentNodeRef = nodeService
      .createNode(
        commentsFolder,
  ...
  private Map<String, Object> generateItemValue(NodeRef commentNodeRef) {
    ...
    String creator = (String) this.nodeService.getProperty(
        commentNodeRef,
    ...
    if (created instanceof Date && modified instanceof Date) {
      isUpdated =
        ((Date) modified).getTime() - ((Date) created).getTime() > 5000;
    }

    // TODO refactor v0 Comments API to use CommentService (see ACE-5437)
    Serializable owner =
      this.nodeService.getProperty(commentNodeRef, ContentModel.PROP_OWNER);
    String currentUser =
   ...
   * @return
   */
  private Map<String, Object> generateModel(
    NodeRef nodeRef,

    ...
    public void handle() throws IOException {
        ...
        if (e instanceof WebDAVServerException) {
            ...
            if (logger.isDebugEnabled()) {
                ...
                logger.debug(
                        request.getMethod()
                                + " is returning status code: "
                                + error.getHttpStatusCode());
            }

            if (response.isCommitted()) {
                logger.warn(
                        "Could not return the status code to the client as the response has already"
                                + " been committed!");
            } else {
            ...
            if (response.isCommitted()) {
                logger.warn(
                        "Could not return the internal server error code to the client as the"

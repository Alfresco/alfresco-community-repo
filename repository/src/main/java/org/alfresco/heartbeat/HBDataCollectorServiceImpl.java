...
public class HBDataCollectorServiceImpl
  implements HBDataCollectorService, LicenseChangeHandler {

  /** The logger. */
  private static final Log logger = LogFactory.getLog(
    HBDataCollectorServiceImpl.class
  );
   ...
   */
  @Override
  public synchronized void registerCollector(
    final HBBaseDataCollector collector
  ) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "HeartBeat registered collector: " + collector.getCollectorId()
      );
    }
  }
   ...
   */
  @Override
  public synchronized void deregisterCollector(
    final HBBaseDataCollector collector
  ) {
  ...
  private void restartAllCollectorSchedules() {
    for (HBBaseDataCollector collector : collectors.values()) {
      try {
        ...
        // Log and ignore
        logger.error(
          "HeartBeat failed to restart collector: " +
          collector.getCollectorId(),
          e
        );
      }

...
public class ExecuteAllRulesActionExecuter extends ActionExecuterAbstractBase {
    ...
    private void executeImpl(
            final Action ruleAction, NodeRef actionedUponNodeRef, List<Rule> parentRules) {
        ...
        Boolean includeInheritedValue =
                (Boolean) ruleAction.getParameterValue(PARAM_EXECUTE_INHERITED_RULES);
        if (includeInheritedValue != null) {
            includeInherited = includeInheritedValue.booleanValue();
        }

        boolean runAllChildren = false;
        Boolean runAllChildrenValue =
                (Boolean) ruleAction.getParameterValue(PARAM_RUN_ALL_RULES_ON_CHILDREN);
        ...
        // This is a recursive method, collect the rules specified for this particular invocation's
        // node.
                ...
                if (dictionaryService.isSubClass(childType, ContentModel.TYPE_SYSTEM_FOLDER)
                    ...
                    for (Rule rule : rules) {
                        ...
                        if (rule.getRuleDisabled() == false) {
                            ...
                            if (action != null) {
                                runtimeRuleService.addRulePendingExecution(
                                        actionedUponNodeRef, child, rule);
                            }
                        }
                    }
    ...
    protected void addParameterDefinitions(List<ParameterDefinition> paramList) {
        paramList.add(
                new ParameterDefinitionImpl(
                        ...
                        false,
                        getParamDisplayLabel(PARAM_EXECUTE_INHERITED_RULES)));
        paramList.add(
                new ParameterDefinitionImpl(
                        PARAM_RUN_ALL_RULES_ON_CHILDREN,
                        DataTypeDefinition.BOOLEAN,
                        false,
                        getParamDisplayLabel(PARAM_RUN_ALL_RULES_ON_CHILDREN)));
    }
}

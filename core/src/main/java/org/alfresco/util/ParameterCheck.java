...
public final class ParameterCheck {
    ...
    /**
     * Checks that the string parameter with the given name has content i.e. it is not null and not
     * zero length
     *
     * @param strParamName Name of parameter to check
     * @param strParamValue Value of the parameter to check
     */
    public static final void mandatoryString(
            final String strParamName, final String strParamValue) {
    ...
    public static final void mandatoryCollection(final String strParamName, final Collection coll) {
        if (coll == null || coll.size() == 0) {
            throw new IllegalArgumentException(
                    strParamName + " collection must contain at least one item");
        }
    }
}

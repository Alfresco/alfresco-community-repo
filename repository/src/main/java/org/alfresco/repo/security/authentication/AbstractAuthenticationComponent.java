...
public abstract class AbstractAuthenticationComponent
  ...
  public Authentication setCurrentUser(final String userName)
    throws AuthenticationException {
    return setCurrentUser(userName, UserNameValidationMode.CHECK_AND_FIX);
  }

  public Authentication setCurrentUser(
    String userName,
    UserNameValidationMode validationMode
  ) {
    if (logger.isTraceEnabled()) {
      logger.trace(
    ...
    if (
      validationMode == UserNameValidationMode.NONE ||
      isSystemUserName(userName)
    ) {
      return setCurrentUserImpl(userName);
    } else {
      CurrentUserCallback callback = validationMode ==
        UserNameValidationMode.CHECK_AND_FIX
   ...
   * @return Authentication
   */
  private Authentication setCurrentUserImpl(String userName)
    throws AuthenticationException {
    if (userName == null) {
      throw new AuthenticationException("Null user name");
    }
  ...
  public Authentication setCurrentAuthentication(
    Authentication authentication
  ) {
    ...
    if (allowGuestLogin == null) {
      if (implementationAllowsGuestLogin()) {
        ...
        throw new AuthenticationException(
          "Guest authentication is not allowed"
        );
      }
    } else {
  ...
  abstract class CurrentUserCallback
    implements
      RetryingTransactionHelper.RetryingTransactionCallback<Authentication> {
  ...
  class CheckCurrentUserCallback extends CurrentUserCallback {
    ...
    public Authentication execute() throws Throwable {
      if (logger.isTraceEnabled()) {
        logger.trace(
          "Check current user: " + AuthenticationUtil.maskUsername(userNameIn)
        );
      }
      try {
        // We must set full authentication before calling runAs in order to retain tickets
        Pair<String, String> userTenant = AuthenticationUtil.getUserTenant(
          userNameIn
        );
        ...
        TenantUtil.runAsSystemTenant(
          new TenantRunAsWork<Object>() {
            public Object doWork() throws Exception {
              String identifier;
              if (
                (identifier = personService.getUserIdentifier(userName)) ==
                null ||
  ...
  class FixCurrentUserCallback extends CurrentUserCallback {
    ...
    public Authentication execute() throws Throwable {
      if (logger.isTraceEnabled()) {
        logger.trace(
          "Fix current user: " + AuthenticationUtil.maskUsername(userNameIn)
        );
      }
      try {
        Pair<String, String> userTenant = AuthenticationUtil.getUserTenant(
        ...
        Authentication authentication = setCurrentUserImpl(
          TenantUtil.runAsSystemTenant(
   ...
   *            a set of user names
   */
  public void setDefaultAdministratorUserNames(
    Set<String> defaultAdministratorUserNames
  ) {
   ...
   */
  public void setDefaultAdministratorUserNameList(
    String defaultAdministratorUserNames
  ) {

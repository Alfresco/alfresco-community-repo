...
public class VirtualPermissionServiceExtensionTest
  ...
  private NodeRef testSiteFolder = null, smartFolder =
    null, contributionDocsFolder = null;
  ...
  public void setUp() throws Exception {
    ...
    permissionService =
      ctx.getBean("permissionServiceImpl", PermissionServiceSPI.class);
  ...
  protected void setUpTestPermissions() {
    ...
    VirtualUserPermissions testPermissions = new VirtualUserPermissions(
      savedUserPermissions
    );
    Set<String> allowSmartNodes = new HashSet<>(
      savedUserPermissions.getAllowSmartNodes()
    );
  ...
  public void testHasPermissionAdherence_missingFolderPath() throws Exception {
    NodeRef virtualFolderT5 = createVirtualizedFolder(
      testRootFolder.getNodeRef(),
      "VirtualFolderT5",
      TEST_TEMPLATE_5_JSON_SYS_PATH
    );

    NodeRef filingFolderVirtualNodeRef = nodeService.getChildByName(
      virtualFolderT5,
      ContentModel.ASSOC_CONTAINS,
  ...
  public void testHasPermissionAdherence_folderPath() throws Exception {
    ...
    ChildAssociationRef filingFolderChildAssoc = createFolder(
      rootNodeRef,
      "FilingFolder"
    );
  ...
  public void testHasPermission() throws Exception {
    ...
    assertEquals(
      AccessStatus.DENIED,
      hasPermissionAs(vf1Node2, PermissionService.DELETE, user1)
    );
  ...
  public void testReadonlyNodeHasPermission() throws Exception {
    // virtual permission should override actual permissions
    NodeRef aVFTestTemplate2 = createVirtualizedFolder(
      testRootFolder.getNodeRef(),
      "aVFTestTemplate2",
      TEST_TEMPLATE_2_JSON_SYS_PATH
    );
    NodeRef vf2Node2 = nodeService.getChildByName(
      aVFTestTemplate2,
      ContentModel.ASSOC_CONTAINS,
      "Node2"
    );

    final String[] deniedReadOnly = new String[] {
    ...
    for (int i = 0; i < deniedReadOnly.length; i++) {
      AccessStatus accessStatus = hasPermissionAs(
        vf2Node2,
        deniedReadOnly[i],
        user1
      );
  ...
  private Map<String, List<? extends PermissionEntry>> mapPermissionsByName(
    ...
    for (PermissionEntry permissionEntry : entries) {
      ...
      List<PermissionEntry> permissions = (List<PermissionEntry>) nameMap.get(
        name
      );
      if (permissions == null) {
  ...
  private Map<String, List<AccessPermission>> mapAccessPermissionsByName(
    ...
    for (AccessPermission accessPermission : accessPermissions) {
      ...
      List<AccessPermission> permissions = (List<AccessPermission>) nameMap.get(
        name
      );
      if (permissions == null) {
        permissions = new ArrayList<>();
  ...
  protected void assertUniquePermission(
    ...
    Map<String, List<? extends PermissionEntry>> entriesByName = mapPermissionsByName(
      (List<? extends PermissionEntry>) permissionEntries
    );
    assertNotNull(
  ...
  protected void assertUniqueAccessPermission(
    ...
    Map<String, List<AccessPermission>> apByName = mapAccessPermissionsByName(
      accessPermissions
    );
    assertNotNull(
      "Not null permission " + permissionName + " expected.",
  ...
  public void testGetAllSetPermissions() throws Exception {
    ...
    Set<AccessPermission> allVf1SetPermissions = permissionService.getAllSetPermissions(
      this.virtualFolder1NodeRef
    );

    AccessPermission vf1ReadProperties = mapAccessPermissionsByName(
    ...
    Set<AccessPermission> allNode2SetPermissions = permissionService.getAllSetPermissions(
      vf1Node2
    );
  ...
  public void testGetSetPermissions() throws Exception {
    ...
    NodePermissionEntry vf1SetPermissions = permissionService.getSetPermissions(
      this.virtualFolder1NodeRef
    );
    ...
    NodePermissionEntry node2SetPermissions = permissionService.getSetPermissions(
      vf1Node2
    );
  ...
  public void testPerm_ace_5162() throws Exception {
    ...
    try {
      ...
      FileInfo t1File = fileAndFolderService.create(
        contributionsSMF,
        "T1",
      ...
      prepareMocks(
        "cm:Contribution",
        smartStore.materializeIfPossible(t1File.getNodeRef())
      );

      NodeRef childContet = nodeService.getChildByName(
        contributionsSMF,
        ContentModel.ASSOC_CONTAINS,
        "T1"
      );
  ...
  private String asTypedPermission(String perm) {
    return (
      smartStore.getUserPermissions().getPermissionTypeQName() + "." + perm
    );
  }
}

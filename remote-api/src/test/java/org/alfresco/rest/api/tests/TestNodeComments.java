...
public class TestNodeComments extends EnterpriseTestApi {
                    ...
                    public Void doWork() throws Exception {
                        NodeRef nodeRef =
                                repoService.createDocument(
                                        site1.getContainerNodeRef("documentLibrary"),
                                        "Test Doc",
                                        "Test Content");
                        nodes.add(nodeRef);
                        ...
                        nodes.add(nodeRef);
                        nodeRef =
                                repoService.createCmObject(
                                        site1.getContainerNodeRef("documentLibrary"), "CM Object");
                        nodes.add(nodeRef);
                        nodeRef =
                                repoService.createObjectOfCustomType(
                                        site1.getContainerNodeRef("documentLibrary"),
                                        "Custom type object",
                                        "{custom.model}sop");
                        nodes.add(nodeRef);
    ...
    @Test
    // TODO test embedded entity createdBy full visibility e.g. view comment by another user who's
    // full details the caller can't see
    ...
    public void testNodeComments() throws Exception {
        ...
        // multi-byte characters, create and update comments
        {
            ...
            int maxItems = 2;
            Paging paging = getPaging(skipCount, maxItems);
            ListResponse<Comment> resp =
                    commentsProxy.getNodeComments(nodeRef2.getId(), createParams(paging, null));
            ...
            Comment[] multiByteCommentUpdates =
                    new Comment[] {
...

        {
            ...
            Paging paging =
                    getPaging(
                            skipCount, maxItems, expectedComments.size(), expectedComments.size());
            ListResponse<Comment> resp =
                    commentsProxy.getNodeComments(nodeRef3.getId(), createParams(paging, null));
            checkList(
                    expectedComments.subList(
...

        {
...

            {
                ...
                ListResponse<Activity> activities =
                        peopleProxy.getActivities(person11.getId(), createParams(paging, null));
        ...
        // suffix the node id with a version number
        {
            ...
            Paging paging =
                    getPaging(
                            skipCount, maxItems, expectedComments.size(), expectedComments.size());
            ListResponse<Comment> resp =
                    commentsProxy.getNodeComments(
                            nodeRef1.getId() + ";3.0", createParams(paging, null));
            checkList(
                    expectedComments.subList(
                            skipCount, skipCount + paging.getExpectedPaging().getCount()),
                    paging.getExpectedPaging(),
        ...
        // view comments of a document created by another person in the same network, who is not a
        // member of the site
        ...
        // document owned by another person in another network, the user is not a member of that
        // network
        try {
            int skipCount = 0;
            int maxItems = 2;
        ...
        // invalid methods
        {
            ...
            try {
                commentsProxy.remove(
                        "nodes",
                        nodeRef1.getId(),
                        "comments",
                        null,
                        "Unable to DELETE node comments");
                fail();
            } catch (PublicApiException e) {
                assertEquals(HttpStatus.SC_METHOD_NOT_ALLOWED, e.getHttpResponse().getStatusCode());
            }
        ...
        // update comments
        {
            ...
            for (Comment comment : createdComments.values()) {
                ...
                Comment ret =
                        commentsProxy.updateNodeComment(
                                nodeRef4.getId(), comment.getId(), updateComment);
            ...
            ListResponse<Comment> resp =
                    commentsProxy.getNodeComments(nodeRef4.getId(), createParams(paging, null));
        ...
        try {
            publicApiClient.setRequestContext(
                    new RequestContext(network1.getId(), person22.getId()));
            Comment comment = expectedComments.get(1);
            Comment update = new Comment("Test Comment 4", "Test Comment 4");
            commentsProxy.updateNodeComment(nodeRef1.getId(), comment.getId(), update);
            fail();
        ...
        try {
            ...
            commentsProxy.updateNodeComment(
                    nodeRef1.getId(), createdComment.getId(), updatedComment);
    ...
    public void testNodeCommentsAndLocking() throws Exception {
        ...
        try {
            ...
            publicApiClient.setRequestContext(
                    new RequestContext(network1.getId(), person14.getId()));
    ...
    public void testNodeCommentsAndLockingIncludingChildren() throws Exception {
        ...
        // TODO push-down to future CommentServiceImplTest (see ACE-5437) - since includeChildren is
        // via LockService api only
    ...
    public void testSkipCountHighValue(List<Comment> expectedComments, Comments commentsProxy)

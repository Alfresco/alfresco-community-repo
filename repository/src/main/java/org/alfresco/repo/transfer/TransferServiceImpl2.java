...
public class TransferServiceImpl2 implements TransferService2 {
  ...
  private static final String MSG_NO_HOME =
    "transfer_service.unable_to_find_transfer_home";
  private static final String MSG_NO_GROUP =
    "transfer_service.unable_to_find_transfer_group";
  private static final String MSG_NO_TARGET =
    "transfer_service.unable_to_find_transfer_target";
  private static final String MSG_ERR_TRANSFER_ASYNC =
    "transfer_service.unable_to_transfer_async";
  private static final String MSG_TARGET_EXISTS =
    "transfer_service.target_exists";
  private static final String MSG_NO_NODES = "transfer_service.no_nodes";
  private static final String MSG_MISSING_ENDPOINT_PATH =
    "transfer_service.missing_endpoint_path";
  private static final String MSG_MISSING_ENDPOINT_PROTOCOL =
    "transfer_service.missing_endpoint_protocol";
  private static final String MSG_MISSING_ENDPOINT_HOST =
    "transfer_service.missing_endpoint_host";
  private static final String MSG_MISSING_ENDPOINT_PORT =
    "transfer_service.missing_endpoint_port";
  private static final String MSG_MISSING_ENDPOINT_USERNAME =
    "transfer_service.missing_endpoint_username";
  private static final String MSG_MISSING_ENDPOINT_PASSWORD =
    "transfer_service.missing_endpoint_password";
  private static final String MSG_FAILED_TO_GET_TRANSFER_STATUS =
    "transfer_service.failed_to_get_transfer_status";
  private static final String MSG_TARGET_ERROR =
    "transfer_service.target_error";
  private static final String MSG_UNKNOWN_TARGET_ERROR =
    "transfer_service.unknown_target_error";
  private static final String MSG_TARGET_NOT_ENABLED =
    "transfer_service.target_not_enabled";
   ...
   */
  private Map<String, TransferStatus> transferMonitoring = Collections.synchronizedMap(
    new TreeMap<String, TransferStatus>()
  );
  ...
  private TransferTarget createTransferTarget(TransferTarget newTarget) {
    ...
    // type properties
    properties.put(
      TransferModel.PROP_ENDPOINT_HOST,
      newTarget.getEndpointHost()
    );
    properties.put(
      TransferModel.PROP_ENDPOINT_PORT,
      newTarget.getEndpointPort()
    );
    properties.put(
      TransferModel.PROP_ENDPOINT_PROTOCOL,
      newTarget.getEndpointProtocol()
    );
    properties.put(
      TransferModel.PROP_ENDPOINT_PATH,
      newTarget.getEndpointPath()
    );
    properties.put(TransferModel.PROP_USERNAME, newTarget.getUsername());
    properties.put(
      TransferModel.PROP_PASSWORD,
      new String(encrypt(newTarget.getPassword()))
    );
    ...
    ChildAssociationRef ref = nodeService.createNode(
      ...
      TransferModel.TYPE_TRANSFER_TARGET,
      properties
    );
  ...
  protected NodeRef getDefaultGroup() {
    ...
    if (refs.isEmpty()) {
      ...
      throw new TransferException(
        MSG_NO_GROUP,
        new Object[] { defaultTransferGroup }
      );
    }
  ...
  public Set<TransferTarget> getTransferTargets(String groupName) {
    ...
    NodeRef groupNode = nodeService.getChildByName(
      home,
      ContentModel.ASSOC_CONTAINS,
      groupName
    );
  ...
  private Set<TransferTarget> getTransferTargets(NodeRef groupNode) {
    ...
    for (ChildAssociationRef child : children) {
      if (
        nodeService
          .getType(child.getChildRef())
          .equals(TransferModel.TYPE_TRANSFER_TARGET)
      ) {
        TransferTargetImpl newTarget = new TransferTargetImpl();
  ...
  public TransferTarget saveTransferTarget(TransferTarget update) {
    ...
    properties.put(
      TransferModel.PROP_PASSWORD,
      new String(encrypt(update.getPassword()))
    );
  ...
  public void transferAsync(
    ...
    Action transferAction = actionService.createAction(
      "transfer-async",
      params
    );
  ...
  private TransferEndEvent transferImpl(
    String targetName,
    final TransferDefinition definition,
    final TransferEventProcessor eventProcessor
  ) throws TransferFailureException {
      ...
      try {
        switch (clientState) {
          case Begin:
            {
              ...
              transfer =
                transmitter.begin(target, localRepositoryId, fromVersion);
          ...
          case Prepare:
            {
              ...
              if (
                !this.transferVersionChecker.checkTransferVersions(
                    fromVersion,
                    toVersion
                  )
              ) {
                throw new TransferException(
                  MSG_INCOMPATIBLE_VERSIONS,
                  new Object[] {
          ...
          case Commit:
            {
              logger.debug(
                "about to start committing transferId:" +
                transfer.getTransferId()
              );
          ...
          case Poll:
            {
              ...
              try {
                ...
                if (pollRetries == 3) {
                  throw new TransferException(
                    MSG_FAILED_TO_GET_TRANSFER_STATUS,
              ...
              if (progress.getStatus() == TransferProgress.Status.ERROR) {
                ...
                if (Exception.class.isAssignableFrom(targetError.getClass())) {
                  ...
                  failureException =
                    new TransferException(
                      MSG_TARGET_ERROR,
                      new Object[] { targetError.getMessage() },
                      targetError
                    );
                }
                ...
                progress.getStatus() == TransferProgress.Status.CANCELLED
              ) {
                cancelled = true;
                clientState = ClientTransferState.Finished;
                break;
              ...
              try {
                ...
                try {
                  if (failureException != null) {
                    logger.debug(
                      "TransferException - unable to transfer",
                      failureException
                    );
                    TransferEventError errorEvent = new TransferEventError();
                    errorEvent.setTransferState(
                      TransferEvent.TransferState.ERROR
                    );
                    ...
                    endEventImpl.setTransferState(
                      TransferEvent.TransferState.CANCELLED
                    );
                    endEventImpl.setMessage("cancelled");
                    reportName = "cancelled";
                  } else {
                    logger.debug(
                      "committed transferId:" + transfer.getTransferId()
                    );
                    endEventImpl = new TransferEventSuccess();
                    endEventImpl.setTransferState(
                      TransferEvent.TransferState.SUCCESS
                    );
                  ...
                  reportName = "error";
                  logger.warn(
                    "Exception - unable to notify end transfer state",
                    e
                  );
                }
                ...
                try {
                  if (transfer != null) {
                    logger.debug(
                      "now pull back the destination transfer report"
                    );
                    destinationReport =
                      persistDestinationTransferReport(
                        reportName,
                        transfer,
                        target
                      );
                    if (destinationReport != null) {
                      eventProcessor.writeReport(
                        destinationReport,
                  ...
                  sourceReport =
                    persistTransferReport(
                      reportName,
                      transfer,
                      target,
                      definition,
                      transferReportEvents,
                      manifest,
                      failureException
                    );
                  if (sourceReport != null) {
                    eventProcessor.writeReport(
                      sourceReport,
                      TransferEventReport.ReportType.SOURCE,
                      endEventImpl.getTransferState()
                    );
                  }
                } catch (Exception e) {
                  logger.warn(
                    "Exception - unable to write transfer reports",
                    e
                  );
                }
    ...
    try {
      if (endEvent == null) {
        ...
        TransferFailureException endException = new TransferFailureException(
          error
        );
  ...
  private File createManifest(
    ...
    if (
      ...
      (nodesToRemove == null || nodesToRemove.size() == 0)
    ) {
    ...
    int nodeCount =
      ...
      ((nodesToRemove == null) ? 0 : nodesToRemove.size());
    ...
    File snapshotFile = TempFileProvider.createTempFile(
      "TRX-SNAP",
      FILE_SUFFIX,
      tempDir
    );
    ...
    if (nodes != null) {
      for (NodeRef nodeRef : nodes) {
        TransferManifestNode node = transferManifestNodeFactory.createTransferManifestNode(
          nodeRef,
          definition,
          transferContext
        );
  ...
  private File createRequisiteFile() {
    File tempDir = TempFileProvider.getLongLifeTempDir(FILE_DIRECTORY);
    File reqFile = TempFileProvider.createTempFile(
      "TRX-REQ",
      FILE_SUFFIX,
      tempDir
    );
  ...
  private void sendContent(
    ...
    XMLTransferRequsiteReader reqReader = new XMLTransferRequsiteReader(
      reqProcessor
    );
    ...
    final Long removeNodesRange = Long.valueOf(
      definition.getNodesToRemove() != null
        ? definition.getNodesToRemove().size()
        : 0
    );
    final Long nodesRange = Long.valueOf(
  ...
  private void checkTargetEnabled(TransferTarget target)
    throws TransferException {
    if (!target.isEnabled()) {
      logger.debug("target is not enabled");
      throw new TransferException(
        MSG_TARGET_NOT_ENABLED,
        new Object[] { target.getName() }
      );
    }
  }
  ...
  private final String KEY_TRANSFER_HOME_NODEREF =
    "key.transferServiceImpl2Home.noderef";
  ...
  private void mapTransferTarget(NodeRef nodeRef, TransferTargetImpl def) {
    ...
    String endpointPath = (String) properties.get(
      TransferModel.PROP_ENDPOINT_PATH
    );
    if (endpointPath == null) throw new TransferException(
      MSG_MISSING_ENDPOINT_PATH,
      new Object[] { name }
    );
    def.setEndpointPath(endpointPath);

    String endpointProtocol = (String) properties.get(
      TransferModel.PROP_ENDPOINT_PROTOCOL
    );
    if (endpointProtocol == null) throw new TransferException(
      MSG_MISSING_ENDPOINT_PROTOCOL,
      new Object[] { name }
    );
    def.setEndpointProtocol(endpointProtocol);

    String endpointHost = (String) properties.get(
      TransferModel.PROP_ENDPOINT_HOST
    );
    if (endpointHost == null) throw new TransferException(
      MSG_MISSING_ENDPOINT_HOST,
      new Object[] { name }
    );
    def.setEndpointHost(endpointHost);

    Integer endpointPort = (Integer) properties.get(
      TransferModel.PROP_ENDPOINT_PORT
    );
    if (endpointPort == null) throw new TransferException(
      MSG_MISSING_ENDPOINT_PORT,
      new Object[] { name }
    );
    def.setEndpointPort(endpointPort);

    String username = (String) properties.get(TransferModel.PROP_USERNAME);
    if (username == null) throw new TransferException(
      MSG_MISSING_ENDPOINT_USERNAME,
      new Object[] { name }
    );
    def.setUsername(username);

    Serializable passwordVal = properties.get(TransferModel.PROP_PASSWORD);
    if (passwordVal == null) throw new TransferException(
      MSG_MISSING_ENDPOINT_PASSWORD,
      new Object[] { name }
    );
   ...
   */
  private NodeRef persistTransferReport(
    ...
    RetryingTransactionCallback<NodeRef> writeReportCallback = new RetryingTransactionCallback<NodeRef>() {
      ...
      public NodeRef execute() throws Throwable {
        ...
        if (exception != null) {
          reportNode =
            transferReporter.createTransferReport(
              transferName,
              exception,
              target,
              definition,
    ...
    NodeRef reportNode = transactionService
   ...
   */
  private NodeRef persistDestinationTransferReport(
    final String transferName,
    final Transfer transfer,
    final TransferTarget target
  ) {
    ...
    RetryingTransactionCallback<NodeRef> writeReportCallback = new RetryingTransactionCallback<NodeRef>() {
      ...
      public NodeRef execute() throws Throwable {
        File tempDir = TempFileProvider.getLongLifeTempDir(FILE_DIRECTORY);
        File destReportFile = TempFileProvider.createTempFile(
          "TRX-DREP",
          FILE_SUFFIX,
          tempDir
        );
        FileOutputStream destReportOutput = new FileOutputStream(
          destReportFile
        );
        transmitter.getTransferReport(transfer, destReportOutput);
        logger.debug("transfer report (destination) starting");

        NodeRef reportNode = transferReporter.writeDestinationReport(
          transferName,
          target,
          destReportFile
        );
    ...
    try {
      ...
      logger.debug(
        "unexpected error while obtaining destination transfer report",
        e
      );
      return null;
    }
  ...
  public void setTransferManifestNodeFactory(
    TransferManifestNodeFactory transferManifestNodeFactory
  ) {
  ...
  public void setTransferVersionChecker(
    TransferVersionChecker transferVersionChecker
  ) {

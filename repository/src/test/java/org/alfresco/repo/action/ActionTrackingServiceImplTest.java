...
public class ActionTrackingServiceImplTest extends TestCase {
  ...
  protected void setUp() throws Exception {
    ...
    this.runtimeActionService =
      (RuntimeActionService) ctx.getBean("actionService");
    this.actionTrackingService =
      (ActionTrackingService) ctx.getBean("actionTrackingService");
    this.transactionService =
      (TransactionService) ctx.getBean("transactionService");
    this.executingActionsCache =
      (SimpleCache<String, ExecutionDetails>) ctx.getBean(
  ...
  public void testExecutionDetailsSummary() throws Exception {
    ...
    ExecutionSummary s = ActionTrackingServiceImpl.buildExecutionSummary(
      action
    );
    ...
    ExecutionDetails d = ActionTrackingServiceImpl.buildExecutionDetails(
      action
    );
  ...
  public void testWorkingActions() throws Exception {
    final SleepActionExecuter sleepActionExec = (SleepActionExecuter) ctx.getBean(
    ...
    asyncOccurs.awaitExecution(
      null,
      sleepActionExec.getExecutingThread(),
  ...
  private Action performFailingActionImpl(
    ...
    assertNotNull(
      "Started at time is null, the action has not yet started after " +
      (end - start) +
      "ms",
      d.getStartedAt()
    );
  ...
  public void testListings() throws Exception {
    ...
    assertEquals(
      0,
      actionTrackingService.getExecutingActions(SleepActionExecuter.NAME).size()
    );
    assertEquals(
      0,
      actionTrackingService
        .getExecutingActions(createWorkingSleepAction(null))
        .size()
    );
    ...
    assertEquals(
      1,
      actionTrackingService.getExecutingActions(sleepAction1).size()
    );
    assertEquals(
      0,
      actionTrackingService.getExecutingActions(sleepAction2).size()
    );
    assertEquals(
      0,
      actionTrackingService.getExecutingActions(moveAction).size()
    );
    ...
    assertEquals(
      2,
      actionTrackingService.getExecutingActions(sa13).size() // Others still going
    );
  }
  ...
  public void testCancellation() throws Exception {
    ...
    CancellableSleepAction sleepAction1 = (CancellableSleepAction) createWorkingSleepAction(
      null
    );
    CancellableSleepAction sleepAction2 = (CancellableSleepAction) createWorkingSleepAction(
      null
    );
    actionTrackingService.recordActionExecuting(sleepAction1);
    actionTrackingService.recordActionExecuting(sleepAction2);
    assertEquals(
      false,
      actionTrackingService.isCancellationRequested(sleepAction1)
    );
    assertEquals(
      false,
      actionTrackingService.isCancellationRequested(sleepAction2)
    );
    ...
    ExecutionSummary s2 = ActionTrackingServiceImpl.buildExecutionSummary(
      sleepAction2
    );
    ...
    CancellableSleepAction sleepAction3 = (CancellableSleepAction) createWorkingSleepAction(
      null
    );
    ...
    assertEquals(
      false,
      actionTrackingService.isCancellationRequested(sleepAction3)
    );
    ...
    assertEquals(
      false,
      actionTrackingService.getExecutionDetails(s3).isCancelRequested()
    );
    assertNotNull(executingActionsCache.get(key3));
  ...
  public class AsyncOccurs implements OnAsyncActionExecute {
    ...
    @Override
    public void onAsyncActionExecute(
      Action action,
      NodeRef actionedUponNodeRef
    ) {
      if (
        wantedType == null ||
        action.getActionDefinitionName().equals(wantedType)
      ) {
    ...
    public void awaitExecution(UserTransaction tx, Thread toWake, String type)
      throws Exception {
  ...
  private Action createFailingMoveAction() {
    Action failingAction =
      this.actionService.createAction(MoveActionExecuter.NAME);
   ...
   */
  private Action createFailingSleepAction(String id, boolean isFatal)
    throws Exception {
    return ActionServiceImplTest.createFailingSleepAction(
      id,
      isFatal,
      actionService
    );

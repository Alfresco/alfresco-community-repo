...
public class EnterpriseWorkflowTestApi extends EnterpriseTestApi {
  ...
  public void before() throws Exception {
    ...
    HttpClientProvider httpClientProvider = (HttpClientProvider) applicationContext.getBean(
      "httpClientProvider"
    );
    ...
    UserDataService userDataService = new UserDataService() {
      ...
      public UserData findUserByUserName(String userName) {
        ...
        if (userName.startsWith("admin")) {
          ...
          TestPerson person = getRepoService()
            .getPerson(userName.toLowerCase());
    ...
    AuthenticationDetailsProvider authenticationDetailsProvider = new UserAuthenticationDetailsProviderImpl(
      userDataService,
      "admin",
      "admin"
    );
    AuthenticatedHttp authenticatedHttp = new AuthenticatedHttp(
      httpClientProvider,
      authenticationDetailsProvider
    );
    this.httpClient =
      new WorkflowApiHttpClient(
        "localhost",
        TestFixture.PORT,
        TestFixture.CONTEXT_PATH,
        TestFixture.PUBLIC_API_SERVLET_NAME,
        authenticatedHttp
      );
    this.publicApiClient = new WorkflowApiClient(httpClient, userDataService);
    activitiProcessEngine =
      (ProcessEngine) applicationContext.getBean("activitiProcessEngine");
  }

  protected String deployProcessDefinition(String... artifacts) {
    DeploymentBuilder deploymentBuilder = activitiProcessEngine
      .getRepositoryService()
      .createDeployment();
    boolean firstArtifact = true;
    for (String artifact : artifacts) {
      InputStream bpmnInputStream = getClass()
        .getClassLoader()
        .getResourceAsStream(artifact);
  ...
  protected RequestContext initApiClientWithTestUser() throws Exception {
    ...
    final String personId = currentNetwork
      .getPeople()
      .iterator()
      .next()
      .getId();
  ...
  protected TestPerson getOtherPersonInNetwork(
    String usedPerson,
    String networkId
  ) throws Exception {
    ...
    if (dateText != null) {
      try {
        ...
        throw new RuntimeException(
          "couldn't parse date " + dateText + ": " + e.getMessage(),
          e
        );
      }
    }
  ...
  protected void assertErrorSummary(
    String expectedBriefSummary,
    ...
    assertTrue(
      "Wrong summary of error: " + actualBriefSummary,
      actualBriefSummary.endsWith(expectedBriefSummary)
    );
  }

  protected NodeRef[] createTestDocuments(final RequestContext requestContext) {
    NodeRef[] docNodeRefs = TenantUtil.runAsUserTenant(
      new TenantRunAsWork<NodeRef[]>() {
        ...
        public NodeRef[] doWork() throws Exception {
          ...
          NodeRef nodeRefDoc1 = getTestFixture()
            .getRepoService()
            .createDocument(
          ...
          NodeRef nodeRefDoc2 = getTestFixture()
            .getRepoService()
            .createDocument(
              site.getContainerNodeRef("documentLibrary"),
              "Test Doc2",
   ...
   */
  protected ProcessInfo startAdhocProcess(
    final RequestContext requestContext,
    NodeRef[] documentRefs
   ...
   */
  @SuppressWarnings("unchecked")
  protected ProcessInfo startReviewPooledProcess(
    final RequestContext requestContext
  ) throws PublicApiException {
    org.activiti.engine.repository.ProcessDefinition processDefinition = activitiProcessEngine
      .getRepositoryService()
      .createProcessDefinitionQuery()
      .processDefinitionKey(
        "@" + requestContext.getNetworkId() + "@activitiReviewPooled"
      )
      .singleResult();
   ...
   */
  @SuppressWarnings("unchecked")
  protected ProcessInfo startParallelReviewProcess(
    final RequestContext requestContext
  ) throws PublicApiException {
    org.activiti.engine.repository.ProcessDefinition processDefinition = activitiProcessEngine
      .getRepositoryService()
      .createProcessDefinitionQuery()
      .processDefinitionKey(
        "@" + requestContext.getNetworkId() + "@activitiParallelReview"
      )
      .singleResult();
    ...
    TenantUtil.runAsUserTenant(
      new TenantRunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          ...
          TestPerson otherPerson = getOtherPersonInNetwork(
            requestContext.getRunAsUser(),
            requestContext.getNetworkId()
          );

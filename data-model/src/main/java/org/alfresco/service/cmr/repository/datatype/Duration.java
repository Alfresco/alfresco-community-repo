 ...
 *
 * <p>The lexical representation of duration is PnYnMnDTnHnMnS.
 *
 * <p>P is a literal value that starts the expression nY is an integer number of years followed by
 * the literal Y nM is an integer number of months followed by the literal M nD is an integer number
 * of days followed by the literal D T is the literal that separates the date and time nH is an
 * integer number of hours followed by a literal H nM is an integer number of minutes followed by a
 * literal M nS is a decimal number of seconds followed by a literal S
 *
 * <p>Any numbers and designator may be absent if the value is zero. A minus sign may appear before
 * the literal P to indicate a negative duration. If no time items are present the literal T must
 * not appear.
 *
 * <p>This implementation is immutable and thread safe.
 *
 * <p>There are two forms of duration common on database types. The code contains warnings wheer
 * these are relevant.
 *
 * @author andyh
 */
...
public class Duration implements Comparable, Serializable {
                        ...
                        if (tok.ttype == '-') {
                            ...
                            if (nval != null) {
                                ...
                                throw new RuntimeException(
                                        "IO Error parsing Duration: " + duration);
                            }
                            nval = null;
                        } else if (tok.ttype == 'M') {
                            if (dateMode) {
                            ...
                            if (nval != null) {
                                ...
                                m_nanos =
                                        (int)
                                                ((long) (nval.doubleValue() * 1000000000)
                                                        % 1000000000);
                            } else {
                                throw new RuntimeException(
                                        "IO Error parsing Duration: " + duration);
     ...
     *
     * <p>Sadly, it works out the duration by incrementing the lower calendar until it matches the
     * higher.
     */
    ...
    public Duration(boolean positive_in, long months_in, long seconds_in, long nanos_in) {
        ...
        if ((months < 0) && (seconds + nanos < 0)) {
            ...
            throw new RuntimeException(
                    "Can not convert to period - incompatible signs for year_to_momth and"
                            + " day_to_second elements");
        } else if ((months < 0) && (seconds + nanos > 0)) {
            throw new RuntimeException(
                    "Can not convert to period - incompatible signs for year_to_momth and"
                            + " day_to_second elements");
    ...
    public Duration add(Duration add) {
        ...
        long months =
                (this.m_positive ? 1 : -1) * this.getTotalMonths()
                        + (add.m_positive ? 1 : -1) * add.getTotalMonths();
        long seconds =
                (this.m_positive ? 1 : -1) * this.getTotalSeconds()
                        + (add.m_positive ? 1 : -1) * add.getTotalSeconds();
        long nanos =
                (this.m_positive ? 1 : -1) * this.getTotalNanos()
                        + (add.m_positive ? 1 : -1) * add.getTotalNanos();
    ...
    public Duration subtract(Duration sub) {
        long months =
                (this.m_positive ? 1 : -1) * this.getTotalMonths()
                        - (sub.m_positive ? 1 : -1) * sub.getTotalMonths();
        long seconds =
                (this.m_positive ? 1 : -1) * this.getTotalSeconds()
                        - (sub.m_positive ? 1 : -1) * sub.getTotalSeconds();
        long nanos =
                (this.m_positive ? 1 : -1) * this.getTotalNanos()
                        - (sub.m_positive ? 1 : -1) * sub.getTotalNanos();
        Duration result = new Duration(true, months, seconds, nanos);
        return result;
    }
    ...
    public boolean equals(Object o) {
        ...
        Duration d = (Duration) o;
        return (this.m_positive == d.m_positive)
                && (this.getTotalMonths() == d.getTotalMonths())
                && (this.getTotalSeconds() == d.getTotalSeconds())
                && (this.getTotalNanos() == d.getTotalNanos());
    }

...
public class ResourceInspector {
    ...
    private static final Set<Class<? extends ResourceAction>> ALL_ENTITY_RESOURCE_INTERFACES =
            new HashSet<Class<? extends ResourceAction>>();
    private static final Set<Class<? extends ResourceAction>> ALL_RELATIONSHIP_RESOURCE_INTERFACES =
            new HashSet<Class<? extends ResourceAction>>();
    private static final Set<Class<? extends ResourceAction>> ALL_PROPERTY_RESOURCE_INTERFACES =
            new HashSet<Class<? extends ResourceAction>>();
    ...
    private static List<ResourceMetadata> inspectEntity(EntityResource annot, Class<?> resource) {
        ...
        if (noAuth) {
            throw new IllegalArgumentException(
                    "@WebApiNoAuth should not be on all (entity resource class) - only on"
                            + " individual methods: "
                            + urlPath);
        }
    ...
    public static void inspectAddressedProperties(
            Api api, Class<?> resource, final String entityPath, List<ResourceMetadata> metainfo) {
        final Map<String, List<ResourceOperation>> operationGroupedByProperty =
                new HashMap<String, List<ResourceOperation>>();

        MetaHelperAddressable helperForAddressProps =
                new MetaHelperAddressable(resource, entityPath, operationGroupedByProperty);
        ...
        findOperation(
                BinaryResourceAction.ReadWithResponse.class, HttpMethod.GET, helperForAddressProps);
        findOperation(
        ...
        findOperation(
                RelationshipResourceBinaryAction.Read.class, HttpMethod.GET, helperForAddressProps);
        findOperation(
        ...
        if (noAuth) {
            throw new IllegalArgumentException(
                    "@WebApiNoAuth should not be on all (address properties) - only on individual"
                            + " methods: "
                            + entityPath);
        }
        ...
        if (resource.isAnnotationPresent(WebApiDeleted.class)) {
            ...
            for (Entry<String, List<ResourceOperation>> groupedOps :
                    operationGroupedByProperty.entrySet()) {
                metainfo.add(
                        new ResourceMetadata(
                                groupedOps.getKey(),
                                RESOURCE_TYPE.PROPERTY,
    ...
    private static List<ResourceMetadata> inspectRelationship(
            RelationshipResource annot, Class<?> resource) {
        ...
        RelationshipResource relAnnot =
                AnnotationUtils.findAnnotation(entityResource, RelationshipResource.class);
        if (relAnnot != null) {
        ...
        findOperation(
                RelationshipResourceAction.ReadByIdWithResponse.class, HttpMethod.GET, helper);
        findOperation(RelationshipResourceAction.UpdateWithResponse.class, HttpMethod.PUT, helper);
        findOperation(
                RelationshipResourceAction.DeleteWithResponse.class, HttpMethod.DELETE, helper);
        findOperation(
                RelationshipResourceAction.DeleteSetWithResponse.class, HttpMethod.DELETE, helper);
        ...
        if (noAuth) {
            throw new IllegalArgumentException(
                    "@WebApiNoAuth should not be on all (relationship resource class) - only on"
                            + " methods: "
                            + urlPath);
        }
    ...
    /**
     * Determines if the resources supports the resource action specified by
    ...
    public static ResourceOperation inspectOperation(
        ...
        if (annot != null) {
            ...
            return new ResourceOperation(
                    httpMethod, title, desc, parameters, validSuccessCode(httpMethod, success));
        } else {
            return new ResourceOperation(
                    httpMethod,
    ...
    private static void inspectBodyParamAndReturnType(
            Class<?> resource, Method aMethod, List<ResourceParameter> params) {
    ...
    protected static String findEntityNameByAnnotationAttributes(Map<String, Object> annotAttribs) {
        ...
        EntityResource entityAnnot =
                AnnotationUtils.findAnnotation(entityResourceRef, EntityResource.class);
    ...
    /**
     * For a given class, looks for @EmbeddedEntityResource annotations, using the annotation
     * produce a Map of the property name key and the entity key
     *
    ...
    public static Map<String, Pair<String, Method>> findEmbeddedResources(Class<?> anyClass) {
        ...
        List<Method> annotatedMethods =
                ResourceInspectorUtil.findMethodsByAnnotation(
                        anyClass, EmbeddedEntityResource.class);
        if (annotatedMethods != null && !annotatedMethods.isEmpty()) {
            for (Method annotatedMethod : annotatedMethods) {
                Annotation annot =
                        AnnotationUtils.findAnnotation(
                                annotatedMethod, EmbeddedEntityResource.class);
                if (annot != null) {
    ...
    public static void inspectOperations(
            Api api, Class<?> resource, final String entityPath, List<ResourceMetadata> metainfo) {
        Map<String, Pair<ResourceOperation, Method>> operations =
                findOperations(entityPath, resource);
    ...
    private static Map<String, Pair<ResourceOperation, Method>> findOperations(
            String entityPath, Class<?> anyClass) {
        Map<String, Pair<ResourceOperation, Method>> embeds =
                new HashMap<String, Pair<ResourceOperation, Method>>();
        List<Method> annotatedMethods =
                ResourceInspectorUtil.findMethodsByAnnotation(anyClass, Operation.class);
        if (annotatedMethods != null && !annotatedMethods.isEmpty())
            for (Method annotatedMethod : annotatedMethods) {
                ...
                if (annot != null) {
                    ...
                    String actionPath =
                            ResourceDictionary.propertyResourceKey(entityPath, actionName);
                    ResourceOperation ro =
                            inspectOperation(anyClass, annotatedMethod, HttpMethod.POST);
                    embeds.put(
                            actionPath, new Pair<ResourceOperation, Method>(ro, annotatedMethod));
                }
            }
        return embeds;
    }
    ...
    public static List<ResourceMetadata> inspect(Class resource) {
        ...
        RelationshipResource relAnnot =
                AnnotationUtils.findAnnotation(resource, RelationshipResource.class);
        if (relAnnot != null) {
    ...
    public static Method findUniqueIdMethod(Class<?> objClass) throws IllegalArgumentException {
        List<Method> annotatedMethods =
                ResourceInspectorUtil.findMethodsByAnnotation(objClass, UniqueId.class);
        if (annotatedMethods != null && !annotatedMethods.isEmpty()) {
    ...
    private static class MetaHelperAddressable extends MetaHelperCallback {
        ...
        public void whenNewOperation(ResourceOperation operation, Method aMethod) {
            Annotation addressableProps =
                    AnnotationUtils.findAnnotation(aMethod, BinaryProperties.class);
            if (addressableProps != null) {
                Map<String, Object> annotAttribs =
                        AnnotationUtils.getAnnotationAttributes(addressableProps);
    ...
    private abstract static class MetaHelperCallback {
        ...
        public abstract void whenOperationDeleted(
                Class<? extends ResourceAction> deleted, Method aMethod);

        public abstract void whenOperationNoAuth(
                Class<? extends ResourceAction> noAuth, Method aMethod);
    }
}

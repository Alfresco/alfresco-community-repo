 ...
 *
 * <p>When creating regular folders through clients that are not RM aware the folders must be
 * converted to the appropriate RM container
 *
 * @author Ana Bozianu
 * @since 2.6
 */
public class RM4619Test extends BaseRMTestCase {
    ...
    /**
     * Given the RM site is created When we create a regular folder in the fileplan Then the folder
     * is immediately converted to a record category
     */
    ...
    /**
     * Given an existing category When we create a regular folder in the category Then the folder is
     * immediately converted to a record folder
     */
                ...
                doTestInTransaction(
                        new Test<NodeRef>() {
                            ...
                            public NodeRef run() throws Exception {
                                FileInfo info =
                                        fileFolderService.create(
                                                rmContainer, GUID.generate(), TYPE_FOLDER);
                                return info.getNodeRef();
                            }
                        },
                        ADMIN_USER);
    ...
    /**
     * Given the RM site is created When we create a regular folder in the unfiled record container
     * Then the folder is converted to a unfiled record folder
     *
     * <p>And when we create another regular folder in that unfiled record folder Then the folder is
     * also converted to a unfiled record folder
     */
                ...
                doTestInTransaction(
                        new Test<NodeRef>() {
                            ...
                            public NodeRef run() throws Exception {
                                FileInfo folder =
                                        fileFolderService.create(
                                                unfiledContainer, GUID.generate(), TYPE_FOLDER);
                ...
                doTestInTransaction(
                        new Test<NodeRef>() {
                            ...
                            public NodeRef run() throws Exception {
                                FileInfo folder =
                                        fileFolderService.create(
                                                folder1, GUID.generate(), TYPE_FOLDER);
                                return folder.getNodeRef();
                            }
                        },
                        ADMIN_USER);

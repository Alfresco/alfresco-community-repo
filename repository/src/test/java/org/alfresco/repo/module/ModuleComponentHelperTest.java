 ...
 *     <p>This test creates a bunch of dummy components and then simulates startups with different
 *     module current versions.
 *     <p>There are 3 modules. There are 3 components.
 * @author Derek Hulley
 */
public class ModuleComponentHelperTest extends BaseAlfrescoTestCase {
    ...
    private final String[] MODULE_IDS = {
        "M0 @ " + SOME_GUID, "M1 @ " + SOME_GUID, "M2 @ " + SOME_GUID
    };
    private final String[] COMPONENT_NAMES = {
        "C0 @ " + SOME_GUID, "C1 @ " + SOME_GUID, "C2 @ " + SOME_GUID
    };
    ...
    public void setUp() throws Exception {
        ...
        for (int i = 0; i < 3; i++) // i = module number
        {
            ...
            {
                DummyModuleComponent component =
                        new DummyModuleComponent(
                                MODULE_IDS[i], COMPONENT_NAMES[j], VERSIONS[j], VERSIONS[j + 1]);
                ...
                // Don't initialize the component as that will do the registration.  We do it
                // manually.
    ...
    private class DummyModuleService implements ModuleService {
            ...
            for (int i = 0; i < 3; i++) {
                ModuleDetails moduleDetails =
                        new ModuleDetailsImpl(
                                MODULE_IDS[i], currentVersion, "Module-" + i, "Description-" + i);
        ...
        public List<ModuleDetails> getMissingModules() {
            ...
            for (int i = 0; i < 3; i++) {
                ModuleDetails moduleDetails =
                        new ModuleDetailsImpl(MODULE_IDS[i], currentVersion, "", "");
                details.add(moduleDetails);
            }
            // Done
            return details;
    ...
    private class DummyModuleComponent extends AbstractModuleComponent {
        private DummyModuleComponent(
                String moduleId, String name, VersionNumber from, VersionNumber to) {

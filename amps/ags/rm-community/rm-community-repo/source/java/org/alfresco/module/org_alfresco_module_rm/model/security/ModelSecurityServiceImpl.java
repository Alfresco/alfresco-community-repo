...
public class ModelSecurityServiceImpl
  ...
  public void register(ProtectedModelArtifact artifact) {
    ...
    if (artifact instanceof ProtectedProperty) {
      protectedProperties.put(
        artifact.getQName(),
        (ProtectedProperty) artifact
      );
    ...
    if (
      ...
      for (Map.Entry<QName, Serializable> entry : after.entrySet()) {
        ...
        if (isProtectedProperty(property)) {
          ...
          if (
            before == null || before.isEmpty() || before.get(property) == null
          ) {
            return;
          }
          ...
          if (
            ...
            throw new ModelAccessDeniedException(
              "The user " +
              AuthenticationUtil.getFullyAuthenticatedUser() +
              " does not have the permission to edit the protected property " +

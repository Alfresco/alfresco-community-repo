...
public class ThumbnailRegistry
    ...
    private static final QName LOCK_QNAME =
            QName.createQName(
    ...
    private Map<String, ThumbnailDefinition> thumbnailDefinitions =
            new HashMap<String, ThumbnailDefinition>();

    /** Cache to store mimetype to thumbnailDefinition mapping with max size limit */
    private Map<String, List<ThumbnailDefinitionLimits>> mimetypeMap =
            new HashMap<String, List<ThumbnailDefinitionLimits>>(17);
        ...
        for (ThumbnailDefinition td : thumbnailDefinitions) {
            ...
            if (thumbnailName == null) {
                throw new ThumbnailException(
                        "When adding a thumbnail details object make sure the name is set.");
            }
    ...
    // This is done during system startup. It needs to be done as the system user (see callers) to
    // ensure the thumbnail definitions get saved
    // and also needs to be done within a transaction in order to support concurrent startup. See c
    // for details.
    ...
    public void initThumbnailDefinitions() {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Skipping initialisation from thumbnail definitions (could not get lock): "
                                + e.getMessage());
            }
        } finally {
            if (lockToken != null) {
                ...
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        for (String thumbnailDefName : thumbnailDefinitions.keySet()) {
                            final ThumbnailDefinition thumbnailDefinition =
                                    thumbnailDefinitions.get(thumbnailDefName);
                            ...
                            RenditionDefinition renditionDef =
                                    thumbnailRenditionConvertor.convert(thumbnailDefinition, null);
    ...
    public List<ThumbnailDefinition> getThumbnailDefinitions(String mimetype, long sourceSize) {
        List<ThumbnailDefinitionLimits> thumbnailDefinitionsLimitsForMimetype =
                this.mimetypeMap.get(mimetype);
        ...
        if (thumbnailDefinitionsLimitsForMimetype == null) {
            ...
            for (ThumbnailDefinition thumbnailDefinition : this.thumbnailDefinitions.values()) {
                ...
                if (maxSourceSizeBytes != 0) {
                    thumbnailDefinitionsLimitsForMimetype.add(
                            new ThumbnailDefinitionLimits(thumbnailDefinition, maxSourceSizeBytes));
        ...
        List<ThumbnailDefinition> result =
                new ArrayList<ThumbnailDefinition>(thumbnailDefinitionsLimitsForMimetype.size());
        for (ThumbnailDefinitionLimits thumbnailDefinitionLimits :
                thumbnailDefinitionsLimitsForMimetype) {
    ...
    /**
     * Checks to see if at this moment in time, the specified {@link ThumbnailDefinition} is able to
     * thumbnail the source mimetype. Typically used with Thumbnail Definitions retrieved by name,
     * and/or when dealing with transient {@link ContentTransformer}s.
     *
     * @param sourceUrl The URL of the source (optional)
     * @param sourceMimetype The source mimetype
     * @param sourceSize the size (in bytes) of the source. Use -1 if unknown.
     * @param sourceNodeRef which is set in a copy of the thumbnailDefinition transformation
     *     options, so that it may be used by transformers and debug.
     * @param thumbnailDefinition The {@link ThumbnailDefinition} to check for
     */
    public boolean isThumbnailDefinitionAvailable(
            String sourceUrl,
            String sourceMimetype,
            long sourceSize,
            NodeRef sourceNodeRef,
            ThumbnailDefinition thumbnailDefinition) {
        // Use RenditionService2 if it knows about the definition, otherwise use
        // localTransformServiceRegistry.
        // Needed as disabling local transforms should not disable thumbnails if they can be done
        // remotely.
        boolean supported = false;
        String targetMimetype = thumbnailDefinition.getMimetype();
        RenditionDefinition2 renditionDefinition =
                getEquivalentRenditionDefinition2(thumbnailDefinition);
        if (renditionDefinition != null) {
            Map<String, String> options = renditionDefinition.getTransformOptions();
            String renditionName = renditionDefinition.getRenditionName();
            supported =
                    transformServiceRegistry.isSupported(
                            sourceMimetype, sourceSize, targetMimetype, options, renditionName);
        } else {
            ...
            try {
                ...
                Map<String, String> options =
                        converter.getOptions(transformationOptions, sourceMimetype, targetMimetype);
                supported =
                        localTransformServiceRegistry.isSupported(
                                sourceMimetype, sourceSize, targetMimetype, options, renditionName);
            } finally {
    ...
    public boolean isThumbnailDefinitionAvailable(
            ...
            long sourceSize,
            ThumbnailDefinition thumbnailDefinition) {
        return isThumbnailDefinitionAvailable(
                sourceUrl, sourceMimeType, sourceSize, null, thumbnailDefinition);
    }
     ...
     * @return 0 if there are no transformers, -1 if there is no limit or if positive the size in
     *     bytes.
     */
    public long getMaxSourceSizeBytes(
            String sourceMimetype, ThumbnailDefinition thumbnailDefinition) {
        // Use RenditionService2 if it knows about the definition, otherwise use
        // localTransformServiceRegistry.
        // Needed as disabling local transforms should not disable thumbnails if they can be done
        // remotely.
        ...
        if (renditionDefinition != null) {
            ...
            String renditionName = renditionDefinition.getRenditionName();
            maxSize =
                    transformServiceRegistry.findMaxSize(
                            sourceMimetype, targetMimetype, options, renditionName);
        } else {
            boolean orig = TransformerDebug.setDebugOutput(false);
            try {
                TransformationOptions transformationOptions =
                ...
                Map<String, String> options =
                        converter.getOptions(transformationOptions, sourceMimetype, targetMimetype);
                maxSize =
                        localTransformServiceRegistry.findMaxSize(
                                sourceMimetype, targetMimetype, options, renditionName);
            } finally {
                TransformerDebug.setDebugOutput(orig);
            }
    ...
    private RenditionDefinition2 getEquivalentRenditionDefinition2(
            ThumbnailDefinition thumbnailDefinition) {
        String renditionName = thumbnailDefinition.getName();
        RenditionDefinition2 renditionDefinition =
        ...
        return AuthenticationUtil.runAs(
                new RunAsWork<Boolean>() {
                    public Boolean doWork() throws Exception {
                        return ((getThumbnailDefinitions().size() > 0)
                                && (redeployStaticDefsOnStartup
                                        || renditionService.loadRenditionDefinitions().size()
                                                == 0));
                    }
                },
     ...
     * ThumbnailDefinitions injected by spring are converted to RenditionDefinitions and saved.
     */
    protected class RegistryLifecycle extends AbstractLifecycleBean {
        ...
        protected void onBootstrap(ApplicationEvent event) {
            if (redeploy()) {
                ...
                // If the database is in read-only mode, then do not persist the thumbnail
                // definitions.
                if (transactionService.isReadOnly()) {
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "TransactionService is in read-only mode. Therefore no thumbnail"
                                        + " definitions have been initialised.");
                    }
                    return;
                }
    ...
    private class ThumbnailDefinitionLimits {
        ...
        public ThumbnailDefinitionLimits(
                ThumbnailDefinition thumbnailDefinition, long maxSourceSizeBytes) {

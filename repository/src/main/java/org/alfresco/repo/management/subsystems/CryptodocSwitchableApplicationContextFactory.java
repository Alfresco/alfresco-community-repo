...
public class CryptodocSwitchableApplicationContextFactory
  ...
  private static final Log logger = LogFactory.getLog(
    CryptodocSwitchableApplicationContextFactory.class
  );
    ...
    public void setProperty(String name, String value) {
      if (name.equals(SOURCE_BEAN_PROPERTY)) {
        ...
        if (canSwitchSubsystemTo(newSourceBean, value)) {
          boolean isNewEncrypted = isEncryptedContentStoreSubsystem(
            newSourceBean,
            value
          );
  ...
  private boolean canSwitchSubsystemTo(Object newSourceBean, String beanName) {
    ...
    boolean isCurrentEncrypted = isEncryptedContentStoreSubsystem(
      currentSourceBean,
  ...
  private boolean isEncryptedContentStoreSubsystem(
    Object sourceBean,
    String beanName
  ) {
    boolean isEncrypted = false;
    if (
      sourceBean instanceof EncryptedContentStoreChildApplicationContextFactory
    ) {
      isEncrypted =
        (
  ...
  public void onApplicationEvent(ApplicationEvent event) {
    ...
    if (event instanceof DescriptorServiceAvailableEvent) {
      descriptorService =
        ((DescriptorServiceAvailableEvent) event).getDescriptorService();
    }

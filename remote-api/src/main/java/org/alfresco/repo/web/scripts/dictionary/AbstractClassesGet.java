...
public abstract class AbstractClassesGet extends DictionaryWebServiceBase {
    ...
    protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
        ...
        String namespacePrefix =
                getValidInput(req.getParameter(REQ_URL_TEMPL_VAR_NAMESPACE_PREFIX));
        String name = getValidInput(req.getParameter(REQ_URL_TEMPL_VAR_NAME));

        Map<QName, ClassDefinition> classdef = new HashMap<QName, ClassDefinition>();
        Map<QName, Collection<PropertyDefinition>> propdef =
                new HashMap<QName, Collection<PropertyDefinition>>();
        Map<QName, Collection<AssociationDefinition>> assocdef =
                new HashMap<QName, Collection<AssociationDefinition>>();
        ...
        if (isValidClassFilter(classFilter) == false) {
            throw new WebScriptException(
                    ...
                    "Check the classfilter - " + classFilter + " provided in the URL");
        }
        ...
        if (namespacePrefix == null && name != null) {
            throw new WebScriptException(
                    Status.STATUS_NOT_FOUND,
                    "Missing namespaceprefix parameter in the URL - both combination of name and"
                            + " namespaceprefix is needed");
        }

        // validate the namespaceprefix and name parameters => if namespaceprefix is given, then
        // name has to be validated along with it
        if (namespacePrefix != null) {
            ...
            if (name != null) {
                ...
                propdef.put(
                        classQname,
                        this.dictionaryservice.getClass(classQname).getProperties().values());
                assocdef.put(
                        classQname,
                        this.dictionaryservice.getClass(classQname).getAssociations().values());
            } else {
                // if name is not given then the model is extracted from the namespaceprefix, there
                // can be more than one model associated with one namespaceprefix
        ...
        // if namespacePrefix is null, then check the classfilter to pull out either all or type or
        // aspects
        if (myModel == null) {
            if (classFilter.equalsIgnoreCase(CLASS_FILTER_OPTION_TYPE1)) {
        ...
        if (classdef.isEmpty() == true) {
            for (QName qnameObj : qnames) {
                ...
                propdef.put(
                        qnameObj,
                        this.dictionaryservice.getClass(qnameObj).getProperties().values());
                assocdef.put(
                        qnameObj,
                        this.dictionaryservice.getClass(qnameObj).getAssociations().values());
            }
        }
        ...
        Collections.sort(
                classDefinitions,
                new DictionaryComparators.ClassDefinitionComparator(dictionaryservice));

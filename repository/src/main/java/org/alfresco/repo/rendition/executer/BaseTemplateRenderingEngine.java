...
public abstract class BaseTemplateRenderingEngine
  ...
  private static final Log log = LogFactory.getLog(
    BaseTemplateRenderingEngine.class
  );
  ...
  protected void render(RenderingContext context) {
    ...
    try {
      ...
      log.warn(
        "Unexpected error while rendering through XSLT rendering engine.",
        ex
      );
    } finally {
      if (writer != null) {
  ...
  private void throwTemplateParamsNotFoundException() {
    StringBuilder msg = new StringBuilder(
      "This action requires that either the "
    );
  ...
  protected String getParamDisplayLabel(String paramName) {
    ...
    String message = I18NUtil.getMessage(
      commonPropertiesPrefix + "." + paramName + "." + DISPLAY_LABEL
    );

 ...
 */
public class Version2ServiceImpl extends VersionServiceImpl
        implements VersionService, Version2Model, Extensible {
    ...
    public Version2ServiceImpl() {
        versionServiceTrait =
                new ExtendedTrait<VersionServiceTrait>(
                        AJProxyTrait.create(this, VersionServiceTrait.class));
    }
    ...
    protected Version createVersion(
        ...
        // version "description" property is added as a standard version property (if not null)
        // rather than a metadata version property
        ...
        if (versionHistoryRef == null) {
            ...
            if (!headVersion) {
                ...
                if (versionHistory.getSuccessors(currentVersion).size() == 0) {
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "Belt-and-braces: current version does seem to be head version ["
                                        + versionHistoryRef
                                        + ", "
                                        + nodeRef
                                        + "]");
        ...
        String versionLabel =
                invokeCalculateVersionLabel(
                        classRef, currentVersion, versionNumber, versionProperties);
        ...
        if (currentVersionRef == null) {
            ...
            this.dbNodeService.createAssociation(
                    versionHistoryRef, newVersionRef, Version2Model.ASSOC_ROOT_VERSION);
        }
        ...
        try {
            ...
            this.nodeService.setProperty(
                    nodeRef, ContentModel.PROP_VERSION_TYPE, version.getVersionType());
        } finally {
    ...
    protected NodeRef createVersionHistory(NodeRef nodeRef) {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace(
                    "created version history nodeRef: "
     ...
     */
    protected Map<QName, Serializable> getStandardVersionProperties(
            NodeRef nodeRef,
            long nodeDbId,
    ...
    protected NodeRef createNewVersion(
        ...
        try {
            ...
            childAssocRef =
                    this.dbNodeService.createNode(
                            ...
                            QName.createQName(
                                    Version2Model.NAMESPACE_URI,
                                    Version2Model.CHILD_VERSIONS
                                            + "-"
                                            + versionNumber), // TODO - testing - note: all children
                            // (of a versioned node) will have the
                            // same version number, maybe replace
                            // with a version sequence of some
                            // sort 001-...00n
                            sourceTypeRef,
            ...
            nodeService.addAspect(
                    versionNodeRef, Version2Model.ASPECT_VERSION, standardVersionProperties);
        ...
        // If the auditable aspect is not there then add it to the 'version' node (after original
        // aspects have been frozen)
    ...
    private void storeVersionMetaData(
        ...
        // TODO - these are stored as "residual" properties (ie. without property type) - see also
        // NodeBrowser
        //      - need to review, eg. how can we store arbitrary map of metadata properties, that
        // could be indexed/searched (if configured against versionStore)
        for (Map.Entry<String, Serializable> entry : versionProperties.entrySet()) {
            dbNodeService.setProperty(
                    versionNodeRef,
                    QName.createQName(
                            Version2Model.NAMESPACE_URI,
                            Version2Model.PROP_METADATA_PREFIX + entry.getKey()),
                    entry.getValue());
        }
    }
    ...
    private void freezeAspects(
        ...
        for (QName aspect : aspects) {
            ...
            if (aspect.equals(ContentModel.ASPECT_AUDITABLE)) {
                // freeze auditable aspect properties (eg. created, creator, modifed, modifier,
                // accessed)
                for (Map.Entry<QName, Serializable> entry :
                        nodeDetails.getProperties(aspect).entrySet()) {
                    if (entry.getKey().equals(ContentModel.PROP_CREATOR)) {
                        dbNodeService.setProperty(
                                versionNodeRef,
                                Version2Model.PROP_QNAME_FROZEN_CREATOR,
                                entry.getValue());
                    } else if (entry.getKey().equals(ContentModel.PROP_CREATED)) {
                        dbNodeService.setProperty(
                                versionNodeRef,
                                Version2Model.PROP_QNAME_FROZEN_CREATED,
                                entry.getValue());
                    } else if (entry.getKey().equals(ContentModel.PROP_MODIFIER)) {
                        dbNodeService.setProperty(
                                versionNodeRef,
                                Version2Model.PROP_QNAME_FROZEN_MODIFIER,
                                entry.getValue());
                        ...
                        dbNodeService.setProperty(
                                versionNodeRef,
                                Version2Model.PROP_QNAME_FROZEN_ACCESSED,
                                entry.getValue());
                    } else {
                        throw new AlfrescoRuntimeException(
                                "Unexpected auditable property: " + entry.getKey());
                    }
                }
            } else {
    ...
    private List<ChildAssociationRef> getVersionAssocs(NodeRef versionHistoryRef, boolean preLoad) {
        ...
        return dbNodeService.getChildAssocs(
                versionHistoryRef,
                Version2Model.CHILD_QNAME_VERSIONS,
    ...
    protected Version getVersion(NodeRef versionRef) {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace(
                    "getVersion: " + versionRef + " nodeProperties=\n" + nodeProperties.keySet());
        }
        ...
        for (QName key : nodeProperties.keySet()) {
            ...
            if (idx == 0) {
                // versioned metadata property - additional (optional) metadata, set during
                // versioning
                versionProperties.put(
                        keyName.substring(Version2Model.PROP_METADATA_PREFIX.length()), value);
            } else {
        ...
        NodeRef newNodeRef =
                new NodeRef(
                        new StoreRef(Version2Model.STORE_PROTOCOL, Version2Model.STORE_ID),
                        versionRef.getId());
    ...
    protected NodeRef getVersionHistoryNodeRef(NodeRef nodeRef) {
        ...
        NodeRef vhNodeRef =
                this.dbNodeService.getChildByName(
                        getRootNode(),
                        Version2Model.CHILD_QNAME_VERSION_HISTORIES,
                        nodeRef.getId());

        // DEPRECATED: for backwards compatibility, in case of a version node (eg. via
        // getCurrentVersion) can lookup 'live' node via UUID
        if (vhNodeRef == null) {
                        ...
                        this.dbNodeService.getChildByName(
                                ...
                                (String)
                                        nodeService.getProperty(
                                                nodeRef, ContentModel.PROP_NODE_UUID));
            }
        }
    ...
    /**
     * Gets current version of the passed node reference. The node reference is expected to be the
     * 'live' node.
     *
     * <p>This uses the version label as a mechanism for looking up the version node.
     */
    private Pair<Boolean, Version> getCurrentVersionImpl(
            NodeRef versionHistoryRef, NodeRef nodeRef) {
        // The noderef should not be a version type node.
        if (nodeRef.getStoreRef().getIdentifier().contains(Version2Model.STORE_ID)) {
            throw new IllegalArgumentException(
                    "The node reference "
                            + nodeRef
                            + " is pointing to a version node, when a reference to a live node is"
                            + " expected.");
        }
            ...
            if (tempLabel != null && tempLabel.equals(versionLabel) == true) {
                ...
                if (!headVersion) {
                    throw new ConcurrencyFailureException(
                            "Unexpected: current version does not appear to be 1st version in the"
                                    + " list ["
                                    + versionHistoryRef
                                    + ", "
                                    + nodeRef
                ...
                result =
                        new Pair<Boolean, Version>(
                                headVersion, getVersion(versionAssoc.getChildRef()));
                break;
            }
        }
    ...
    /**
     * Check if versions are marked with invalid version label, if true > apply default serial
    ...
    private void checkForCorruptedVersions(NodeRef versionHistory, NodeRef nodeRef) {
        ...
        if (versionLabel != null && versionLabel.equals("0")) {
            ...
            Collections.sort(
                    ...
                    new Comparator<Version>() {
                        ...
                        public int compare(Version v1, Version v2) {
                            int result =
                                    v1.getFrozenModifiedDate()
                                            .compareTo(v2.getFrozenModifiedDate());
                            if (result == 0) {
                                Long dbid1 =
                                        (Long)
                                                nodeService.getProperty(
                                                        v1.getFrozenStateNodeRef(),
                                                        ContentModel.PROP_NODE_DBID);
                                Long dbid2 =
                                        (Long)
                                                nodeService.getProperty(
                                                        v2.getFrozenStateNodeRef(),
                                                        ContentModel.PROP_NODE_DBID);
                                ...
                                if (dbid1 != null && dbid2 != null) {
                                    ...
                                    if (logger.isWarnEnabled()) {
                                        logger.warn(
                                                "node-dbid property is missing for versions: "
                                                        + v1.toString()
                                                        + " or "
                                                        + v2.toString());
                                    }
            ...
            for (Version version : versions) {
                ...
                versionLabel =
                        serialVersionLabelPolicy.calculateVersionLabel(
                                classRef, preceedingVersion, 0, version.getVersionProperties());
                ...
                this.dbNodeService.setProperty(
                        versionNodeRef, Version2Model.PROP_QNAME_VERSION_LABEL, versionLabel);

                version.getVersionProperties()
                        .put(VersionBaseModel.PROP_VERSION_LABEL, versionLabel);
    ...
    public void revert(NodeRef nodeRef, Version version, boolean deep) {
        ...
        try {
            ...
            boolean needToRestoreDiscussion =
                    !this.nodeService.hasAspect(versionNodeRef, ForumModel.ASPECT_DISCUSSABLE)
            ...
            if (newNodeTypeQName.equals(oldNodeTypeQName)) {
                ...
                if (typeDef != null) {
                    for (QName assocName : typeDef.getAssociations().keySet()) {
                        if (getRevertAssocAction(oldNodeTypeQName, assocName, revertDetails)
                                == RevertAssocAction.IGNORE) {
            ...
            for (QName aspect : oldAspectQNames) {
                ...
                if (aspectDef != null) {
                    ...
                    for (QName assocName : aspectDef.getAssociations().keySet()) {
                        if (getRevertAssocAction(aspect, assocName, revertDetails)
                                == RevertAssocAction.IGNORE) {
            ...
            if (needToRestoreDiscussion) {
                ...
                for (QName aspectForCheck : aspectsToRemove) {
                    if (aspectForCheck
                            .getNamespaceURI()
                            .equals(NamespaceService.FORUMS_MODEL_1_0_URI)) {
            ...
            List<ChildAssociationRef> versionedChildren =
                    ...
                    if (this.nodeService.exists(versionedChild.getChildRef()) == true) {
                        ...
                        childRef =
                                this.nodeService.getChildByName(
                                        nodeRef, versionedChild.getTypeQName(), childRefName);
                        // we can faced with association that allow duplicate names
                        if (childRef == null) {
                            List<ChildAssociationRef> allAssocs =
                ...
                if (!assocsToLeaveAlone.contains(versionedAssoc.getTypeQName())) {
                    ...
                    if (this.nodeService.exists(versionedAssoc.getTargetRef()) == true) {
                        this.nodeService.createAssociation(
                                nodeRef,
                                versionedAssoc.getTargetRef(),
                                versionedAssoc.getTypeQName());
                    }
                }
    ...
    public NodeRef restore(
        ...
        props.put(
                ...
                ((NodeRef) version.getVersionProperty(Version2Model.PROP_FROZEN_NODE_REF)).getId());
        props.put(ContentModel.PROP_VERSION_LABEL, version.getVersionLabel());

        // Get the type of the frozen node
        QName type =
                (QName)
                        dbNodeService.getType(
                                VersionUtil.convertNodeRef(version.getFrozenStateNodeRef()));

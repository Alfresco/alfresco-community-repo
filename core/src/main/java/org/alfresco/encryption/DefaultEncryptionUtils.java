...
public class DefaultEncryptionUtils implements EncryptionUtils {
  ...
  public DefaultEncryptionUtils() {
    try {
      ...
      throw new AlfrescoRuntimeException(
        "Unable to initialise EncryptionUtils",
        e
      );
    }
  }
   ...
   */
  @Override
  public void setRequestAlgorithmParameters(
    HttpMethod method,
    ...
    if (params != null) {
      method.setRequestHeader(
        ...
        Base64.encodeBytes(params.getEncoded())
      );
    }
  }
   ...
   * @throws IOException
   */
  protected void setAlgorithmParameters(
   ...
   * @throws IOException
   */
  protected AlgorithmParameters decodeAlgorithmParameters(HttpMethod method)
    throws IOException {
    Header header = method.getResponseHeader(HEADER_ALGORITHM_PARAMETERS);
    if (header != null) {
      byte[] algorithmParams = Base64.decode(header.getValue());
      AlgorithmParameters algorithmParameters = encryptor.decodeAlgorithmParameters(
        algorithmParams
      );
   ...
   * @throws IOException
   */
  protected AlgorithmParameters decodeAlgorithmParameters(
  ...
  public byte[] decryptBody(HttpServletRequest req) throws IOException {
    if (req.getMethod().equals("POST")) {
      ...
      if (bodyStream != null) {
        ...
        // decrypt the body
        InputStream in = encryptor.decrypt(
          KeyProvider.ALIAS_SOLR,
          p,
          bodyStream
        );
  ...
  public boolean authenticateResponse(
    ...
    try {
      ...
      return authenticate(
        expectedMAC,
        new MACInput(decryptedBody, timestamp.longValue(), remoteIP)
      );
    } catch (Exception e) {
  ...
  public boolean authenticate(HttpServletRequest req, byte[] decryptedBody) {
    try {
      ...
      return authenticate(
        expectedMAC,
        new MACInput(decryptedBody, timestamp.longValue(), ipAddress)
      );
    } catch (Exception e) {
      throw new AlfrescoRuntimeException(
        "Unable to authenticate HTTP request",
        e
      );
    }
  }
   ...
   */
  @Override
  public void setRequestAuthentication(HttpMethod method, byte[] message)
    throws IOException {
    long requestTimestamp = System.currentTimeMillis();

    // add MAC header
    byte[] mac = macUtils.generateMAC(
      KeyProvider.ALIAS_SOLR,
   ...
   */
  @Override
  public void setResponseAuthentication(
  ...
  protected boolean authenticate(byte[] expectedMAC, MACInput macInput) {
    ...
    // the expected IP
    boolean authorized =
      macUtils.validateMAC(KeyProvider.ALIAS_SOLR, expectedMAC, macInput) &&

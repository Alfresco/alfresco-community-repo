 ...
 *
 * <p>Because SPEL will eat ${foo} or #{foo} in the template when specified in the XML, this will
 * also accept \$\{foo\} or \#\{foo\} as well.
 *
 * @author Andy Hind
 */
public class SimpleTemplateActionDefinition extends AbstractTemplateActionDefinition
     ...
     */
    public void setTemplateActionModelFactory(
    ...
    /**
     * Set the map of parameters used by the template. These are processed via the template service
     * to produce the actual poarameters.
     */
    ...
    public Action getAction(NodeRef nodeRef) {
        ...
        ActionExecuter actionExecutor =
                (ActionExecuter) applicationContext.getBean(getActionName());
        ...
        for (String paramName : parameterTemplates.keySet()) {
            ...
            String stringValue =
                    templateService.processTemplateString(
                            ...
                            template,
                            getTemplateActionModelFactory().getModel(nodeRef));
            ...
            // There can be no definition for such an ad hoc property.
                ...
                dataTypeDef =
                        dictionaryService.getProperty(QName.createQName(paramName)).getDataType();
            }

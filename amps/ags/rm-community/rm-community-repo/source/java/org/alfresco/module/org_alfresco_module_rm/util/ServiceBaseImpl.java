...
public class ServiceBaseImpl implements RecordsManagementModel, ApplicationContextAware {
     ...
     *
     * <p>Used for performance reasons.
     */
    ...
    public FilePlanComponentKind getFilePlanComponentKind(NodeRef nodeRef) {
        ...
        Map<NodeRef, FilePlanComponentKind> map =
                transactionalResourceHelper.getMap("rm.transaction.filePlanComponentByNodeRef");
        if (map.containsKey(nodeRef)) {
            ...
            if (isFilePlanComponent(nodeRef)) {
                ...
                } else if (instanceOf(nodeRef, TYPE_DISPOSITION_SCHEDULE)
                        || instanceOf(nodeRef, TYPE_DISPOSITION_ACTION_DEFINITION)) {
     ...
     *
     * <p>Exposed in the FilePlan service.
     *
     ...
     *
     * <p>Exposed in the RecordFolder service.
     *
     ...
     *
     * <p>Exposed publicly in the {@link HoldService}
     *
     * @param nodeRef node reference
     ...
     *
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.record.RecordService#isDeclared(org.alfresco.service.cmr.repository.NodeRef)
     */
    ...
    public NodeRef getFilePlan(final NodeRef nodeRef) {
        NodeRef result = null;
        if (nodeRef != null) {
            Map<NodeRef, NodeRef> transactionCache =
                    transactionalResourceHelper.getMap("rm.servicebase.getFilePlan");
                ...
                if (result == null || !instanceOf(result, TYPE_FILE_PLAN)) {
                    if (instanceOf(nodeRef, TYPE_FILE_PLAN)) {
                        ...
                        ChildAssociationRef parentAssocRef =
                                getInternalNodeService().getPrimaryParent(nodeRef);
    ...
    /**
     * Utility method to safely and quickly determine if a node is a type (or sub-type) of the one
     * specified.
     *
     * @param nodeRef node reference
     ...
     *
     * <p>If the node is not already countable, then rma:countable is added and 0 returned.
     *
     * @param nodeRef node reference

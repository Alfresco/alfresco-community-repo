...
public class AnnotatedBehaviourPostProcessor implements BeanPostProcessor {
  ...
  private static Log logger = LogFactory.getLog(
    AnnotatedBehaviourPostProcessor.class
  );
  ...
  private void registerBehaviours(Object bean, String beanName) {
    if (bean.getClass().isAnnotationPresent(BehaviourBean.class)) {
      BehaviourBean behaviourBean = bean
        .getClass()
        .getAnnotation(BehaviourBean.class);
  ...
  private void registerBehaviour(
    ...
    JavaBehaviour javaBehaviour = new JavaBehaviour(
      bean,
      method.getName(),
    ...
    if (bean instanceof BehaviourRegistry && !behaviour.name().isEmpty()) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "   ... adding behaviour to registry with name " + behaviour.name()
        );
      }

      ((BehaviourRegistry) bean).registerBehaviour(
          behaviour.name(),
          javaBehaviour
        );
    }

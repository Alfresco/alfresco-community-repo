...
public class FavouritesServiceImpl
  implements FavouritesService, InitializingBean {

  private static final Log logger = LogFactory.getLog(
    FavouritesServiceImpl.class
  );
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    this.prefKeys.put(
        ...
        new PrefKeys(
          ...
          "org.alfresco.ext.sites.favourites."
        )
      );
    this.prefKeys.put(
        Type.FILE,
  ...
  private boolean removeFavouriteSite(String userName, NodeRef nodeRef) {
    ...
    if (siteInfo != null) {
      StringBuilder sitePrefKeyBuilder = new StringBuilder(
        prefKeys.getSharePrefKey()
      );
      ...
      exists =
        preferenceService.getPreference(userName, siteFavouritedKey) != null;
      preferenceService.clearPreferences(userName, sitePrefKey);
    } else {
      throw new IllegalArgumentException(
        "NodeRef " + nodeRef + " is not a site"
      );
    }
  ...
  private Comparator<PersonFavouriteKey> getComparator(
    ...
    Comparator<PersonFavouriteKey> comparator = new Comparator<PersonFavouriteKey>() {
      ...
      public int compare(PersonFavouriteKey o1, PersonFavouriteKey o2) {
        ...
        if (ret == 0) {
          ...
          ret =
            o1.getNodeRef().toString().compareTo(o2.getNodeRef().toString());
        }
  ...
  private PersonFavourite addFavouriteSite(String userName, NodeRef nodeRef) {
    ...
    if (siteInfo != null) {
      ...
      if (favourite == null) {
        ...
        OnAddFavouritePolicy policy = onAddFavouriteDelegate.get(
          nodeRef,
          nodeClass
        );
  ...
  private PersonFavourite getFavouriteSite(String userName, SiteInfo siteInfo) {
    ...
    Serializable s = preferenceService.getPreference(
      userName,
      siteFavouritedKey
    );
    if (s != null) {
    ...
    if (isFavourited) {
      String createdAtStr = (String) preferenceService.getPreference(
        userName,
        siteCreatedAtKey
      );
      Date createdAt =
        (createdAtStr == null ? null : ISO8601DateFormat.parse(createdAtStr));
  ...
  private boolean isFavouriteSite(String userName, NodeRef nodeRef) {
    ...
    if (siteInfo != null) {
      ...
      Serializable value = preferenceService.getPreference(
        userName,
        favouritedPrefKey
      );
  ...
  private Map<PersonFavouriteKey, PersonFavourite> getFavouriteNodes(
    String userName,
    Type type
  ) {
    PrefKeys prefKeys = getPrefKeys(type);
  ...
  private Map<PersonFavouriteKey, PersonFavourite> extractFavouriteNodes(
    ...
    while (st.hasMoreTokens()) {
      ...
      Date createdAt =
        (createdAtStr != null ? ISO8601DateFormat.parse(createdAtStr) : null);
      ...
      PersonFavourite personFavourite = new PersonFavourite(
        userName,
        nodeRef,
        type,
        name,
      ...
      try {
        ...
        if (siteInfo != null) {
          StringBuilder builder = new StringBuilder(
            sitePrefKeys.getAlfrescoPrefKey()
          );
          ...
          if (createdAtStr != null) {
            createdAt =
              (
                createdAtStr != null
                  ? ISO8601DateFormat.parse(createdAtStr)
                  : null
              );
          }
          PersonFavourite personFavourite = new PersonFavourite(
  ...
  private PersonFavourite getFavouriteDocumentOrFolder(
    ...
    if (nodes != null) {
      Map<PersonFavouriteKey, PersonFavourite> favouriteNodes = extractFavouriteNodes(
        userName,
        type,
        nodes
      );
  ...
  private PersonFavourite getPersonFavourite(
    String userName,
    Type type,
    NodeRef nodeRef
  ) {
  ...
  private PersonFavourite addFavouriteDocumentOrFolder(
    ...
    Map<PersonFavouriteKey, PersonFavourite> personFavourites = getFavouriteNodes(
      userName,
      type
    );
    PersonFavourite personFavourite = getPersonFavourite(
      userName,
      type,
      nodeRef
    );
  ...
  private boolean isFavouriteNode(String userName, Type type, NodeRef nodeRef) {
    ...
    PersonFavouriteKey personFavouriteKey = new PersonFavouriteKey(
      userName,
      null,
      type,
      nodeRef
  ...
  public Type getType(NodeRef nodeRef) {
    ...
    if (isSite) {
      ...
      if (isContainer) {
        ...
        boolean isFile = dictionaryService.isSubClass(
          type,
  ...
  private boolean removeFavouriteNode(
    String userName,
    Type type,
    NodeRef nodeRef
  ) {
    ...
    PersonFavourite personFavourite = personFavourites.remove(
      personFavouriteKey
    );
    ...
    OnRemoveFavouritePolicy policy = onRemoveFavouriteDelegate.get(
      nodeRef,
      nodeClass
    );
  ...
  public PersonFavourite addFavourite(String userName, NodeRef nodeRef) {
    ...
    if (type == null) {
      ...
      personFavourite =
        addFavouriteDocumentOrFolder(userName, Type.FILE, nodeRef);
    } else if (type.equals(Type.FOLDER)) {
      personFavourite =
        addFavouriteDocumentOrFolder(userName, Type.FOLDER, nodeRef);
  ...
  public PagingResults<PersonFavourite> getPagedFavourites(
    ...
    if (
      authenticationContext.isSystemUserName(currentUserName) == true ||
      ...
      final Map<PersonFavouriteKey, PersonFavourite> sortedFavouriteNodes = new TreeMap<PersonFavouriteKey, PersonFavourite>(
        getComparator(sortProps)
      );
      ...
      for (String key : preferences.keySet()) {
        if (includeFiles && key.startsWith(documentsPrefKeys.sharePrefKey)) {
          ...
          if (nodes != null) {
            sortedFavouriteNodes.putAll(
              extractFavouriteNodes(userName, Type.FILE, nodes)
            );
          }
        } else if (
          includeFolders && key.startsWith(foldersPrefKeys.sharePrefKey)
        ) {
          String nodes = (String) preferences.get(key);
          if (nodes != null) {
            sortedFavouriteNodes.putAll(
              extractFavouriteNodes(userName, Type.FOLDER, nodes)
            );
          }
        } else if (
          includeSites &&
          ...
          extractFavouriteSite(
            userName,
            Type.SITE,
            sortedFavouriteNodes,
            preferences,
            key
          );
        }
      ...
      final List<PersonFavourite> page = new ArrayList<PersonFavourite>(

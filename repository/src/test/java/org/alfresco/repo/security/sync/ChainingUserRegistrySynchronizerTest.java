...
public class ChainingUserRegistrySynchronizerTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        this.synchronizer =
                (UserRegistrySynchronizer)
                        ChainingUserRegistrySynchronizerTest.context.getBean(
                                "testUserRegistrySynchronizer");
        this.applicationContextManager =
                (MockApplicationContextManager)
                        ChainingUserRegistrySynchronizerTest.context.getBean(
                                "testApplicationContextManager");
        this.personService =
                (PersonService)
                        ChainingUserRegistrySynchronizerTest.context.getBean("personService");
        this.authorityService =
                (AuthorityService)
                        ChainingUserRegistrySynchronizerTest.context.getBean("authorityService");
        this.nodeService =
                (NodeService) ChainingUserRegistrySynchronizerTest.context.getBean("nodeService");
        ...
        this.namespaceService =
                (NamespaceService)
                        ChainingUserRegistrySynchronizerTest.context.getBean("namespaceService");
    }
    ...
    /**
     * Sets up the test users and groups in three zones, "Z0", "Z1" and "Z2", by doing a forced
     * synchronize with a Mock user registry. Note that the zones have some overlapping entries.
     * "Z0" is not used in subsequent synchronizations and is used to test that users and groups in
     * zones that aren't in the authentication chain get 're-zoned' appropriately. The layout is as
     * follows
     *
     * <pre>
     * Z0
    ...
    private void setUpTestUsersAndGroups() throws Exception {
        this.applicationContextManager.setUserRegistries(
                new MockUserRegistry(
                        "Z0",
                        new NodeDescription[] {newPerson("U6")},
                        new NodeDescription[] {newGroup("G1")}),
                new MockUserRegistry(
                        "Z1",
                        new NodeDescription[] {newPerson("U1"), newPerson("U2"), newPerson("U7")},
                        new NodeDescription[] {
                            newGroup("G2", "U1", "G3"),
                            newGroup("G3", "U2", "G4", "G5"),
                            newGroup("G4"),
                            newGroup("G5")
                        }),
                new MockUserRegistry(
                        ...
                        new NodeDescription[] {
                            newGroup("G2", "U1", "U3", "U4"),
                            newGroup("G6", "U3", "U4", "G7"),
                            newGroup("G7", "U5")
                        }));
    ...
    public void tearDownTestUsersAndGroups() throws Exception {
        ...
        this.applicationContextManager.setUserRegistries(
                ...
                new MockUserRegistry("Z1", new NodeDescription[] {}, new NodeDescription[] {}),
                new MockUserRegistry("Z2", new NodeDescription[] {}, new NodeDescription[] {}));
        this.synchronizer.synchronize(true, true);
        // Wipe out everything that was in Z0 - Z2
        this.applicationContextManager.setUserRegistries(
                new MockUserRegistry("Z0", new NodeDescription[] {}, new NodeDescription[] {}),
                new MockUserRegistry("Z1", new NodeDescription[] {}, new NodeDescription[] {}),
                new MockUserRegistry("Z2", new NodeDescription[] {}, new NodeDescription[] {}));
        this.synchronizer.synchronize(true, true);
        this.retryingTransactionHelper.doInTransaction(
    ...
    public void testDifferentialUpdate() throws Exception {
        ...
        this.applicationContextManager.updateZone(
                "Z1",
                new NodeDescription[] {
                    newPerson("U1", "changeofemail@alfresco.com"), newPerson("U6"), newPerson("U7")
                },
                new NodeDescription[] {
                    newGroup("G1", "U1", "U6", "UDangling", "G2"),
                    newGroup("G2", "U1", "GDangling", "G1"), // test cyclic G2
                    // <-> G1
        ...
        this.applicationContextManager.updateZone(
                "Z2",
                new NodeDescription[] {
                    newPerson("U1", "shouldbeignored@alfresco.com"),
                    newPerson("U5", "u5email@alfresco.com"),
                    newPerson("U6"),
                    newPerson("U8")
                },
                new NodeDescription[] {
                    newGroup("G2", "U1", "U3", "U4", "U6"),
                    newGroup("G7"),
                    newGroup("G8", "U4", "U8")
                });
        this.retryingTransactionHelper.doInTransaction(
                new RetryingTransactionCallback<Object>() {
                    ...
                    public Object execute() throws Throwable {
                        // re: THOR-293 - note: use runAs else security context is cleared (=>
                        // current system user becomes null and personExists fails)
                        AuthenticationUtil.runAsSystem(
                                new RunAsWork<Void>() {
                                    public Void doWork() throws Exception {
                                        // Split transactions to avoid MNT-9768
                                        ChainingUserRegistrySynchronizerTest.this.synchronizer
    ...
    /**
     * Tests a forced update of the test users and groups. Also tests that groups and users that
     * previously existed in Z2 get moved when they appear in Z1. Also tests that 'dangling
     * references' to removed users (U4, U5) do not cause any problems. Also tests that
     * case-sensitivity is not a problem when an occluded user is recreated with different case. The
    ...
    public void testForcedUpdate() throws Exception {
        ...
        this.applicationContextManager.setUserRegistries(
                new MockUserRegistry(
                        "Z1",
                        new NodeDescription[] {newPerson("U2"), newPerson("u3"), newPerson("U6")},
                        new NodeDescription[] {
                            newGroup("G1", "U6", "G5"),
                            newGroup("G2", "G1"),
                            newGroup("G3", "U2", "G5"),
                            newGroup("G5", "U6", "G2"), // cycle g1 -> g5 -> g2 -> g1
                            newGroup("G6", "u3")
                        }),
                new MockUserRegistry(
                        "Z2",
                        new NodeDescription[] {
                            newPerson("U1", "somenewemail@alfresco.com"),
                            newPerson("U3"),
                            newPerson("U6"),
                            newPerson("U8"),
                        },
    ...
    private class MockLDAPUserRegistry extends LDAPUserRegistry implements IMockUserRegistry {
        ...
        public void updateState(
                Collection<NodeDescription> persons, Collection<NodeDescription> groups) {
            mockUserRegistry.updateState(persons, groups);
        }
    }

    private void testLDAPDisableUserAccount(
            AbstractDirectoryServiceUserAccountStatusInterpreter userAccountStatusInterpreter,
            String enabledAccountPropertyValue,
            String disabledAccountPropertyValue)
            throws Exception {
        MockUserRegistry mockUserRegistry =
                new MockUserRegistry(
                        "ldap1",
                        new NodeDescription[] {
                            newPersonWithUserAccountStatusProperty(
                                    "EnabledUser", enabledAccountPropertyValue),
                            newPersonWithUserAccountStatusProperty(
                                    "DisabledUser", disabledAccountPropertyValue)
                        },
                        new NodeDescription[] {});
        ...
        ChainingUserRegistrySynchronizer chainingSynchronizer =
                (ChainingUserRegistrySynchronizer) this.synchronizer;
        ...
        this.retryingTransactionHelper.doInTransaction(
                new RetryingTransactionCallback<Object>() {
                    public Object execute() throws Throwable {
                        NodeRef enabledUserRef =
                                ChainingUserRegistrySynchronizerTest.this.personService.getPerson(
                                        "EnabledUser", false);
                        assertFalse(
                                ChainingUserRegistrySynchronizerTest.this.nodeService.hasAspect(
                                        enabledUserRef, ContentModel.ASPECT_PERSON_DISABLED));

                        NodeRef disabledUserRef =
                                ChainingUserRegistrySynchronizerTest.this.personService.getPerson(
                                        "DisabledUser", false);
                        assertTrue(
                                ChainingUserRegistrySynchronizerTest.this.nodeService.hasAspect(
                                        disabledUserRef, ContentModel.ASPECT_PERSON_DISABLED));
    ...
    public void testLDAPDisableUserAccountWithActiveDirectoryProperty() throws Exception {
        LDAPADUserAccountStatusInterpreter ldapadUserAccountStatusInterpreter =
                new LDAPADUserAccountStatusInterpreter();
    ...
    public void testLDAPDisableUserAccountWithNetscapDSProperty() throws Exception {
        LDAPUserAccountStatusInterpreter ldapUserAccountStatusInterpreter =
                new LDAPUserAccountStatusInterpreter();
    ...
    /**
     * Tests a forced update of the test users and groups with deletions disabled. No users or
     * groups should be deleted, whether or not they move registry. Groups that would have been
     * deleted should have no members and should only be in the default zone.
     *
     * @throws Exception the exception
     */
    public void testForcedUpdateWithoutDeletions() throws Exception {
        UserRegistrySynchronizer synchronizer =
                (UserRegistrySynchronizer)
                        ChainingUserRegistrySynchronizerTest.context.getBean(
                                "testUserRegistrySynchronizerPreventDeletions");
        setUpTestUsersAndGroups();
        this.applicationContextManager.setUserRegistries(
                new MockUserRegistry(
                        "Z0",
        ...
        this.retryingTransactionHelper.doInTransaction(
                new RetryingTransactionCallback<Object>() {
                    ...
                    public Object execute() throws Throwable {
                        // MNT-13867 fix. User U6 already exists in zone "Z0". According
                        // ChainingUserRegistrySynchronizercurrent
                        // implementation when syncDelete==false person that exists in a different
                        // zone with higher
    ...
    /**
     * Tests a forced update of the test users and groups where some of the users change their case
     * and some groups appear with different case.
     */
    ...
    public void testVolume() throws Exception {
        List<NodeDescription> persons =
                new ArrayList<NodeDescription>(new RandomPersonCollection(100));
        List<NodeDescription> groups =
                new ArrayList<NodeDescription>(new RandomGroupCollection(50, persons));
        this.applicationContextManager.setUserRegistries(
                new MockUserRegistry("Z0", persons, groups));
        this.synchronizer.synchronize(true, true);
        tearDownTestUsersAndGroups();
    }
    ...
    public void testTestSynchronize() throws Exception {
        ...
        try {
            ...
            assertTrue(
                    "pre-condition test registry in application context",
                    instances.contains(TEST_REGISTRY_NAME));
            assertFalse(
                    "pre-condition test registry isActive",
                    instances.contains(TEST_REGISTRY_NAME_BAD));

            if (synchronizer instanceof ChainingUserRegistrySynchronizer) {
                /** positive test using mocked user registry */
                ChainingUserRegistrySynchronizer chainingSynchronizer =
                        (ChainingUserRegistrySynchronizer) synchronizer;
                SynchronizeDiagnostic diagnostic =
                        chainingSynchronizer.testSynchronize(TEST_REGISTRY_NAME);
    ...
    public void testSyncStatus() throws Exception {
        ...
        try {
            List<NodeDescription> persons =
                    new ArrayList<NodeDescription>(new RandomPersonCollection(3));
            List<NodeDescription> groups =
                    new ArrayList<NodeDescription>(new RandomGroupCollection(4, persons));
            ...
            if (this.synchronizer instanceof ChainingUserRegistrySynchronizerStatus) {
                ChainingUserRegistrySynchronizerStatus status =
                        (ChainingUserRegistrySynchronizerStatus) this.synchronizer;
                // Header Status
                ...
                assertEquals(
                        "sync status is not complete",
                        "COMPLETE",
                        status.getSynchronizationStatus());
                ...
                assertEquals(
                        "sync status is not complete",
                        "COMPLETE",
                        status.getSynchronizationStatus("Z0"));
    ...
    /**
     * Test that upon a first sync, the missing properties at the AD level are set as 'null' on the
     * Alfresco Person object.
     *
     * <p>MNT-14026: LDAP sync fails to update attribute's value deletion.
     */
    public void testSyncInexistentProperty() throws Exception {
        try {
            // Execute an LDAP sync where the AD server returns the attributes of a person without a
            // certain property, in this case the 'mail'.
            executeMockedLDAPSyncWithoutActiveDirectoryEmailProp();

            Map<QName, Serializable> userProperties =
                    this.nodeService.getProperties(this.personService.getPerson("U1"));
            assertTrue(
                    "User must have the email property even though it's null",
                    userProperties.containsKey(ContentModel.PROP_EMAIL));
            assertTrue(
                    "User's email must be null on first sync.",
                    userProperties.get(ContentModel.PROP_EMAIL) == null);
        } finally {
            tearDownTestUsersAndGroups();
        }
    }
    ...
    /**
     * Test that an attribute is also removed on the Alfresco side, when it's removed at the AD
     * level.
     *
     * <p>MNT-14026: LDAP sync fails to update attribute's value deletion.
     */
    public void testSyncDeletedProperty() throws Exception {
        try {
            ...
            // Execute an LDAP sync where the AD server returns the attributes of a person,
            // including the 'mail' property.
            executeMockedLDAPSyncWithActiveDirectoryEmailProp();

            Map<QName, Serializable> userProperties =
                    this.nodeService.getProperties(this.personService.getPerson("U1"));
            assertTrue(
                    "User's email must be not null.",
                    userProperties.get(ContentModel.PROP_EMAIL).equals("U1@alfresco.com"));
            ...
            assertTrue(
                    ...
                    userProperties.containsKey(ContentModel.PROP_EMAIL));
            assertTrue(
                    "User's email must be null on a 2rd sync, since the email property was removed"
                            + " at the AD level.",
                    userProperties.get(ContentModel.PROP_EMAIL) == null);
        } finally {
    ...
    private void executeMockedLDAPSync(boolean withEmail) throws NamingException, Exception {
        ...
        LDAPInitialDirContextFactoryImpl mockedLdapInitialDirContextFactory =
                getMockedLDAPSearchResult(withEmail);
    ...
    private LDAPInitialDirContextFactoryImpl getMockedLDAPSearchResult(boolean withEmail)
        ...
        when(mockedInitialDirContext.search((String) any(), anyString(), any(SearchControls.class)))
                .thenReturn(mockedNamingEnumeration);

        LDAPInitialDirContextFactoryImpl mockedLdapInitialDirContextFactory =
                mock(LDAPInitialDirContextFactoryImpl.class);
        when(mockedLdapInitialDirContextFactory.getDefaultIntialDirContext(0))
                .thenReturn(mockedInitialDirContext);
    ...
    public void dontTestAssocs() throws Exception {
        List<NodeDescription> groups =
                this.retryingTransactionHelper.doInTransaction(
                        new RetryingTransactionCallback<List<NodeDescription>>() {
                            ...
                            public List<NodeDescription> execute() throws Throwable {
                                return new ArrayList<NodeDescription>(
                                        new RandomGroupCollection(
                                                1000,
                                                ChainingUserRegistrySynchronizerTest.this
                                                        .authorityService.getAllAuthoritiesInZone(
                                                        AuthorityService.ZONE_AUTH_EXT_PREFIX
                                                                + "Z0",
                                                        null)));
                            }
                        },
                        true,
                        true);
        ChainingUserRegistrySynchronizerTest.this.applicationContextManager.setUserRegistries(
                new MockUserRegistry("Z0", Collections.<NodeDescription>emptyList(), groups));
     ...
     * @return the node description
     */
    private NodeDescription newGroupWithDisplayName(
            String name, String displayName, String... members) {
    ...
    private NodeDescription newPersonWithUserAccountStatusProperty(
            String userName, String userAccountPropertyValue) {
    ...
    /**
     * Perform all the necessary assertions to ensure that an authority and its members exist in the
     * correct zone.
     *
     * @param zone the zone
     * @param name the name
    ...
    private void assertExists(String zone, String name, String... members) {
        ...
        if (zone == null) {
            ...
            assertTrue(
                    this.authorityService
                            ...
                            .contains(AuthorityService.ZONE_AUTH_EXT_PREFIX + zone));
        }
        ...
        if (AuthorityType.getAuthorityType(longName).equals(AuthorityType.GROUP)) {
            ...
            assertEquals(
                    memberSet, this.authorityService.getContainedAuthorities(null, longName, true));
        } else {
    ...
    /**
     * Converts the given short name to a full authority name, assuming that those short names
     * beginning with 'G' correspond to groups and all others correspond to users.
     *
     * @param shortName the short name
     * @return the full authority name
     */
    private String longName(String shortName) {
        return this.authorityService.getName(
                shortName.toLowerCase().startsWith("g") ? AuthorityType.GROUP : AuthorityType.USER,
                shortName);
    }

    /**
     * A Test {@link ApplicationListener} that checks SyncEndTime before the SynchronizeEndEvent and
     * SynchronizeDirectoryEndEvent events.
     */
    ...
    public static interface IMockUserRegistry extends UserRegistry, ActivateableBean {
        ...
        /**
         * Modifies the state to match the arguments. Compares new with old and records new
         * modification dates only for changes.
         *
    ...
    public static class MockUserRegistry implements IMockUserRegistry {
        ...
        public MockUserRegistry(
                String zoneId,
                Collection<NodeDescription> persons,
                Collection<NodeDescription> groups) {
            this.zoneId = zoneId;
        ...
        /**
         * Merges together an old and new list of node descriptions. Retains the old node with its
         * old modification date if it is the same in the new list, otherwises uses the node from
         * the new list.
         *
         * @param oldNodes the old node list
         * @param newNodes the new node list
         * @param idProp the name of the ID property
         * @param caseSensitive are IDs case sensitive?
         */
        private void mergeNodeDescriptions(
                List<NodeDescription> oldNodes,
                Collection<NodeDescription> newNodes,
                QName idProp,
                boolean caseSensitive) {
            Map<String, NodeDescription> nodeMap =
                    new LinkedHashMap<String, NodeDescription>(newNodes.size() * 2);
         ...
         */
        public MockUserRegistry(
                String zoneId, NodeDescription[] persons, NodeDescription[] groups) {
        ...
        public Collection<String> getGroupNames() {
            ...
            for (NodeDescription group : this.groups) {
                groupNames.add(
                        (String) group.getProperties().get(ContentModel.PROP_AUTHORITY_NAME));
            }
            return groupNames;
        }
        ...
        /**
         * Filters the given list of node descriptions, retaining only those with a modification
         * date greater than the given date.
         *
         * @param nodes the list of nodes
         * @param modifiedSince the modified date
         * @return the filter list of nodes
         */
        private Collection<NodeDescription> filterNodeDescriptions(
                Collection<NodeDescription> nodes, Date modifiedSince) {
            ...
            return new HashSet<QName>(
                    Arrays.asList(
                            new QName[] {
                                ...
                                ContentModel.PROP_EMAIL, ContentModel.PROP_ORGID,
    ...
    public static class MockApplicationContextManager implements ChildApplicationContextManager {
         ...
         *
         * @param registries the new user registries
         */
        ...
        /**
         * Removes the application context for the given zone ID (simulating a change in the
        ...
        /**
         * Updates the state of the given zone ID, oopying in new modification dates only where
    ...
    /**
     * A collection whose iterator returns randomly generated groups with random associations to a
            ...
            return new Iterator<NodeDescription>() {
                ...
                public NodeDescription next() {
                    ...
                    String[] authorityNames =
                            new String
                                    [this.pos == RandomGroupCollection.this.size
                                            ? RandomGroupCollection.this.size
                                            : 17];
                    for (int i = 0; i < authorityNames.length; i++) {
                        ...
                        int index =
                                this.pos == RandomGroupCollection.this.size
                                        ? i
                                        : RandomGroupCollection.this.generator.nextInt(
                                                RandomGroupCollection.this.authorities.size());
                        authorityNames[i] =
                                ChainingUserRegistrySynchronizerTest.this.authorityService
                                        .getShortName(
                                                (String)
                                                        RandomGroupCollection.this.authorities.get(
                                                                index));
                    }
                    NodeDescription group = newGroup("G" + GUID.generate(), authorityNames);
                    // Make this group a candidate for adding to other groups
                    RandomGroupCollection.this.authorities.add(
                            (String) group.getProperties().get(ContentModel.PROP_AUTHORITY_NAME));
                    return group;
                }

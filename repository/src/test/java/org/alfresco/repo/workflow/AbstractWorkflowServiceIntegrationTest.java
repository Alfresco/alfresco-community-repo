...
public abstract class AbstractWorkflowServiceIntegrationTest
  ...
  protected static final QName customStringProp = QName.createQName(
  ...
  public void testDeployWorkflowDefinition() {
    ...
    checkDefinitions(
      definition,
      false,
      defs.toArray(new WorkflowDefinition[0])
    );
    ...
    checkDefinitions(
      definition,
      true,
      newDefs.toArray(new WorkflowDefinition[0])
    );
    ...
    WorkflowDefinition defByName = workflowService.getDefinitionByName(
      definition.getName()
    );
  ...
  public void testStartTask() {
    ...
    WorkflowPath path = workflowService.startWorkflow(
      workflowDef.getId(),
      params
    );
    assertNotNull(path);
    ...
    List<WorkflowTask> tasks = workflowService.getTasksForWorkflowPath(
      path.getId()
    );
  ...
  public void testGetPathProperties() throws Exception {
    ...
    Map<QName, Serializable> properties = workflowService.getPathProperties(
      path.getId()
    );
    ...
    assertEquals(
      false,
      properties.get(QName.createQName("{}" + WorkflowConstants.PROP_CANCELLED))
    );
    assertEquals(
      instanceId,
      properties.get(
        QName.createQName("{}" + WorkflowConstants.PROP_WORKFLOW_INSTANCE_ID)
      )
    );

    NodeRef initiator = (NodeRef) properties.get(
      QName.createQName("{}" + WorkflowConstants.PROP_INITIATOR)
    );
    assertEquals(personManager.get(USER1), initiator);

    NodeRef expInitiatorHome = (NodeRef) nodeService.getProperty(
      initiator,
      ContentModel.PROP_HOMEFOLDER
    );
    NodeRef initiatorHome = (NodeRef) properties.get(
      QName.createQName("{}" + WorkflowConstants.PROP_INITIATOR_HOME)
    );
    assertEquals(expInitiatorHome, initiatorHome);

    NodeRef actualCompanyHome = (NodeRef) properties.get(
      QName.createQName("{}" + WorkflowConstants.PROP_COMPANY_HOME)
    );
    ...
    assertEquals(
      description,
      properties.get(WorkflowModel.PROP_WORKFLOW_DESCRIPTION)
    );
  }
  ...
  public void testAssociateWorkflowPackage() {
    ...
    assertTrue(
      nodeService.hasAspect(pckgNode, WorkflowModel.ASPECT_WORKFLOW_PACKAGE)
    );

    List<WorkflowInstance> existingInstances = workflowService.getWorkflowsForContent(
      contentNode,
      true
    );
    ...
    existingInstances =
      workflowService.getWorkflowsForContent(contentNode, false);
    assertNotNull(existingInstances);
    assertEquals(0, existingInstances.size());
    ...
    nodeService.addChild(
      ...
      contentNode,
      ContentModel.ASSOC_CONTAINS,
      QName.createQName(NamespaceService.CONTENT_MODEL_PREFIX, "test123")
    );
    ...
    assertEquals(
      workflowDef.getId(),
      path.getInstance().getDefinition().getId()
    );
    String workflowDefId = (String) nodeService.getProperty(
      pckgNode,
      WorkflowModel.PROP_WORKFLOW_DEFINITION_ID
    );
    assertEquals(workflowDefId, workflowDef.getId());
    String workflowDefName = (String) nodeService.getProperty(
      pckgNode,
      WorkflowModel.PROP_WORKFLOW_DEFINITION_NAME
    );
    assertEquals(workflowDefName, workflowDef.getName());
    String workflowInstanceId = (String) nodeService.getProperty(
      pckgNode,
    ...
    List<WorkflowInstance> instances = workflowService.getWorkflowsForContent(
      contentNode,
      true
    );
    ...
    List<WorkflowInstance> completedInstances = workflowService.getWorkflowsForContent(
      contentNode,
      false
    );
  ...
  public void testTaskCapabilities() {
    ...
    List<WorkflowPath> paths = workflowService.getWorkflowPaths(
      workflowInstanceId
    );
    assertNotNull(paths);
    ...
    assertEquals(
      currentTask.getProperties().get(ContentModel.PROP_OWNER),
      USER2
    );
  ...
  public void testPooledTaskCapabilities() {
    ...
    WorkflowDefinition workflowDef = deployDefinition(
    ...
    Serializable pooledActors = taskProperties.get(
    ...
    Collection<?> newActors = (Collection<?>) taskProperties.get(
      WorkflowModel.ASSOC_POOLED_ACTORS
    );
  ...
  public void testGetWorkflowTaskDefinitions() {
    ...
    List<WorkflowTaskDefinition> taskDefs = workflowService.getTaskDefinitions(
      workflowDefId
    );
  ...
  protected void checkQueryTasksInactiveWorkflow(String workflowInstanceId) {
    WorkflowTaskQuery taskQuery = createWorkflowTaskQuery(
  ...
  protected void checkTaskQueryTaskCompleted(
    ...
    QName taskName = QName.createQName(
      NamespaceService.WORKFLOW_MODEL_1_0_URI,
      "adhocTask"
    );
    checkTaskNameQuery(
      taskName,
      Arrays.asList(theTask.getId()),
      WorkflowTaskState.COMPLETED,
      workflowInstanceId
    );
  ...
  protected void checkTaskQueryStartTaskCompleted(
    ...
    QName startTaskName = QName.createQName(
      NamespaceService.WORKFLOW_MODEL_1_0_URI,
      "submitAdhocTask"
    );
    checkTaskNameQuery(
  ...
  public void testGetWorkflows() throws Exception {
    ...
    List<WorkflowInstance> workflows = workflowService.getActiveWorkflows(
      fakeDefId
    );
    ...
    checkWorkflowsDontContain(
      workflowService.getCompletedWorkflows(),
      instance2
    );
    checkWorkflowsContains(
  ...
  public void testParallelReview() throws Exception {
    ...
    List<NodeRef> assignees = Arrays.asList(
      personManager.get(USER2),
      personManager.get(USER3)
    );
    ...
    tasks =
      workflowService.getAssignedTasks(USER3, WorkflowTaskState.IN_PROGRESS);
    assertEquals(1, tasks.size());
  }
  ...
  public void checkCompletedWorkflows(String defId, String... expectedIds) {
    List<WorkflowInstance> workflows = workflowService.getCompletedWorkflows(
      defId
    );
    checkWorkflows(workflows, expectedIds);
  }
  ...
  private void checkWorkflows(
    ...
    for (WorkflowInstance workflow : workflows) {
      ...
      assertTrue(
        "The id: " + workflowId + " was not expected! Expected Ids: " + expIds,
        expIds.contains(workflowId)
      );
    }
  }

  private void checkWorkflowsContains(
    List<WorkflowInstance> workflows,
    String... expectedIds
  ) {
    List<String> expIds = Arrays.asList(expectedIds);
    List<String> workflowIds = CollectionUtils.transform(
      workflows,
      new Function<WorkflowInstance, String>() {
  ...
  private void checkWorkflowsDontContain(
    List<WorkflowInstance> workflows,
    String... expectedIds
  ) {
    List<String> expIds = Arrays.asList(expectedIds);
  ...
  protected void checkTaskNameQuery(
    QName taskName,
    ...
    QName unexistingTaskName = QName.createQName(
      NamespaceService.WORKFLOW_MODEL_1_0_URI,
      "unexistingTask"
    );
  ...
  protected void checkProcessIdQuery(
    String workflowInstanceId,
    List<String> expectedTaskIds,
    WorkflowTaskState state
  ) {
    ...
    taskQuery = createWorkflowTaskQuery(state);
    taskQuery.setProcessId(
      BPMEngineRegistry.createGlobalId(getEngine(), "99999999999")
    );
    checkNoTasksFoundUsingQuery(taskQuery);
  }

  protected void checkTaskIdQuery(
  ...
  protected void checkIsActiveQuery(
    List<String> expectedTaskIds,
    WorkflowTaskState state,
    String optionalProcessId
  ) {
  ...
  protected void checkActorIdQuery(
    String actorId,
    List<String> expectedTaskIds,
    WorkflowTaskState state,
    String optionalProcessId
  ...
  protected void checkTaskPropsQuery(
    List<String> expectedTaskIds,
    WorkflowTaskState state,
    String optionalProcessId
  ) {
  ...
  protected void checkProcessNameQuery(
    List<String> expectedTaskIds,
    WorkflowTaskState state
  ) {
    WorkflowTaskQuery taskQuery = createWorkflowTaskQuery(state);
  ...
  protected void checkProcessPropsQuery(
    List<String> expectedTaskIds,
    WorkflowTaskState state
  ) {
    WorkflowTaskQuery taskQuery = createWorkflowTaskQuery(state);
  ...
  protected void checkTasksFoundUsingQuery(
    List<String> taskIds,
  ...
  protected void checkNoTasksFoundUsingQuery(
    WorkflowTaskQuery workflowTaskQuery
  ) {
    List<WorkflowTask> tasks = workflowService.queryTasks(workflowTaskQuery);
    assertNotNull(tasks);
  ...
  private void checkDefinitions(
    WorkflowDefinition expDefinition,
    boolean expContainsDef,
    ...
    if (expContainsDef) fail(
      "The definitions did not contain expected id: " + id
    );
  }

  private void checkDefinition(
    WorkflowDefinition expDef,
    WorkflowDefinition actualDef
  ) {
  ...
  public void before() throws Exception {
    ...
    this.historyService =
      (HistoryService) applicationContext.getBean("activitiHistoryService");
    Repository repositoryHelper = (Repository) applicationContext.getBean(
      "repositoryHelper"
    );
    this.companyHome = repositoryHelper.getCompanyHome();
    try {
      this.transactionService =
        (TransactionServiceImpl) serviceRegistry.getTransactionService();
    } catch (ClassCastException e) {
      throw new AlfrescoRuntimeException(
        "The AbstractWorkflowServiceIntegrationTest needs direct access to the TransactionServiceImpl"
      );
    }
    ...
    WorkflowAdminServiceImpl workflowAdminService = (WorkflowAdminServiceImpl) applicationContext.getBean(

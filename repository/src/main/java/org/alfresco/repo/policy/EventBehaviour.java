 ...
 *
 * <p>A client uses an <code>EventBehaviour</code> to bind a send event behaviour to a Class-level
 * Policy. <br>
 *
 * <p>The event behavior delegates the generation of the event to a method pointer. The pointer is
...
public class EventBehaviour extends BaseBehaviour {
    ...
    public EventBehaviour(
            AbstractEventProducer eventProducer,
            String endpointUri,
            Object instance,
            String method) {
        this(
    ...
    public synchronized <T> T getInterface(Class<T> policy) {
        ...
        if (proxy == null) {
            InvocationHandler handler =
                    getInvocationHandler(eventProducer, endpointUri, instance, method, policy);
    ...
    <T> InvocationHandler getInvocationHandler(
        ...
        if (!policyIFMethod.getReturnType().equals(Void.TYPE)) {
            throw new PolicyException(
                    "EventBehaviour can only handle VOID policies, but "
                            + policyLocalName
                            + " expects "
                            + policyIFMethod.getReturnType());
        }

        try {
            Class instanceClass = instance.getClass();
            Method delegateMethod =
                    instanceClass.getMethod(method, (Class[]) policyIFMethod.getParameterTypes());
            return new JavaMethodInvocationHandler(
                    eventProducer, endpointUri, this, delegateMethod);
    ...
    private static class JavaMethodInvocationHandler implements InvocationHandler {
        ...
        @Override
        public String toString() {
            return "EventBehaviour [instance="

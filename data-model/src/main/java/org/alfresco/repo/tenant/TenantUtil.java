...
public abstract class TenantUtil {
    ...
    /**
     * Execute a unit of work in a given tenant context. The thread's tenant context will be
     ...
     */
    public static <R> R runAsUserTenant(
            final TenantRunAsWork<R> runAsWork, final String uid, final String tenantDomain) {
        return AuthenticationUtil.runAs(
                new AuthenticationUtil.RunAsWork<R>() {
                    public R doWork() {
    ...
    public static <R> R runAsDefaultTenant(final TenantRunAsWork<R> runAsWork) {
        // Note: with MT Enterprise, if you're current user is not already part of the default
        // domain then this will switch to System
        if (getCurrentDomain().equals(TenantService.DEFAULT_DOMAIN)) {
            return runAsWork(runAsWork);
        } else {
            return runAsSystemTenant(
                    runAsWork, TenantService.DEFAULT_DOMAIN); // force System in default domain
        }
    }

    // switch tenant and run as System within that tenant
    public static <R> R runAsSystemTenant(
            final TenantRunAsWork<R> runAsWork, final String tenantDomain) {
    ...
    private static <R> R runAsWork(final TenantRunAsWork<R> runAsWork) {
        try {
            ...
            throw new RuntimeException(
                    "Error encountered while performing TenantUtil.runAsWork: " + e.getMessage(),
                    e);
        }
    }

    // note: this does not check if tenant is enabled (unlike non-static
    // MultiTServiceImpl.getCurrentUserDomain)

...
public abstract class AbstractMimeMessage extends MimeMessage {
  ...
  protected void buildMessage(
    ...
    this.isMessageInSitesLibrary =
      imapService.getNodeSiteContainer(messageFileInfo.getNodeRef()) != null
        ? true
        : false;
    RetryingTransactionHelper txHelper = serviceRegistry
  ...
  private void setPersistedHeaders() throws MessagingException {
    ...
    if (
      nodeService.hasAspect(
        ...
        ImapModel.ASPECT_IMAP_MESSAGE_HEADERS
      )
    ) {
      @SuppressWarnings("unchecked")
      List<String> messageHeaders = (List<String>) nodeService.getProperty(
        messageFileInfo.getNodeRef(),
      ...
      for (String header : messageHeaders) {
        String headerValue = header.substring(
          header.indexOf(ImapModel.MESSAGE_HEADER_TO_PERSIST_SPLITTER) + 1
        );
        String headerName = header.substring(
          0,
          header.indexOf(ImapModel.MESSAGE_HEADER_TO_PERSIST_SPLITTER)
        );
  ...
  private Map<String, Object> createEmailTemplateModel(NodeRef ref) {
    ...
    model.put(
      "contextUrl",
      new String(imapService.getWebApplicationContextUrl())
    );
    model.put(
      "alfTicket",
      new String(serviceRegistry.getAuthenticationService().getCurrentTicket())
    );
    String contentFolderUrl = imapService.getContentFolderUrl(ref);
    model.put(
      "shareContextUrl",
      new String(imapService.getShareApplicationContextUrl())
    );
  ...
  protected void updateMessageID() throws MessagingException {
    setHeader(
      "Message-ID",
      "<" + this.messageFileInfo.getNodeRef().getId() + DEFAULT_SUFFIX + ">"
    );
  }
}

 ...
 *
 * <p>This class makes it possible to get only RM related class definitions
 *
...
public class RmClassesGet extends DictionaryWebServiceBase implements RecordsManagementModel {
    ...
    private Map<String, Object> executeImpl(WebScriptRequest req, boolean isRM) {
        ...
        if (namespacePrefix != null) {
            ...
            if (name != null) {
                ...
                if (!isValidClassname(className)) {
                    throw new WebScriptException(
                            Status.STATUS_NOT_FOUND,
                            "Check the name - " + name + "parameter in the URL");
                }
                ...
                for (QName qnameObj : this.dictionaryservice.getAllModels()) {
                    if (qnameObj.getNamespaceURI().equals(namespaceUri)) {
                        ...
                        myModel =
                                QName.createQName(
                                        getFullNamespaceURI(namespacePrefix + "_" + name));
        ...
        // if namespacePrefix is null, then check the class filter to pull out either all, type or
        // aspects
        if (myModel == null) {
            if (classFilter.equalsIgnoreCase(CLASS_FILTER_OPTION_TYPE1)) {
     ...
     * @return The names of the types defined within the specified model or all of them depending on
     *     {@link isRM} parameter
     */
     ...
     * @return The names of the aspects defined within the specified model or all of them depending
     *     on {@link isRM} parameter
     */

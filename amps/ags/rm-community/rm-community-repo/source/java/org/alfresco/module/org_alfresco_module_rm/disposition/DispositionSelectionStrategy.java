...
/**
 * This class offers the default implementation of a strategy for selection of disposition schedule
 * for a record when there is more than one which is applicable. An example of where this strategy
 * might be used would be in the case of a record which was multiply filed.
 *
 * @author neilm
 */
public class DispositionSelectionStrategy implements RecordsManagementModel {
    ...
    public NodeRef selectDispositionScheduleFrom(List<NodeRef> recordFolders) {
        if (recordFolders == null || recordFolders.isEmpty()) {
            ...
            //      46 CHAPTER 2
            //      Records assigned more than 1 disposition must be retained and linked to the
            // record folder (category) with the longest
            //      retention period.
            ...
            if (recordFolders.size() == 1) {
                ...
                SortedSet<NodeRef> sortedFolders =
                        new TreeSet<>(new DispositionableNodeRefComparator());
                sortedFolders.addAll(recordFolders);
                recordFolder = sortedFolders.first();
            }

            DispositionSchedule dispSchedule =
    ...
    /**
     * This class defines a natural comparison order between NodeRefs that have the
     * dispositionLifecycle aspect applied. This order has the following meaning: NodeRefs with a
     * 'lesser' value are considered to have a shorter retention period, although the actual
     * retention period may not be straightforwardly determined in all cases.
     */
    class DispositionableNodeRefComparator implements Comparator<NodeRef> {
        ...
        private int compareImpl(NodeRef f1, NodeRef f2) {
            ...
            // TODO this won't work correctly if we are trying to compare schedules that are record
            // based!!
            ...
            if (da1 != null && da2 != null) {
                ...
                // (Defined date is 'shorter' than undefined date as an undefined date means it may
                // be retained forever - theoretically)
                else if (asOfDate1 != null || asOfDate2 != null) {
                    return asOfDate1 == null ? +1 : -1;
                } else {
                    // Neither has an asOfDate. (Somewhat arbitrarily) we'll use the number of
                    // events to compare now.

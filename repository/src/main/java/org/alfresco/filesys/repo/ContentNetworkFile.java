 ...
 *
 * <p>This provides the interaction with the Alfresco Content Model file/folder structure.
 *
 * @author Derek Hulley
 */
public class ContentNetworkFile extends NodeRefNetworkFile {
    ...
    public static ContentNetworkFile createFile(
        ...
        if (isMSOfficeSpecialFile(path, sess, nodeService, nodeRef)) {
            ...
            netFile =
                    new MSOfficeContentNetworkFile(
                            nodeService, contentService, mimetypeService, nodeRef, path);
        } else if (isOpenOfficeSpecialFile(path, sess, nodeService, nodeRef)) {

            // Create a file for special processing

            netFile =
                    new OpenOfficeContentNetworkFile(
                            nodeService, contentService, mimetypeService, nodeRef, path);
        } else {
        ...
        try {
            ...
            throw new AlfrescoRuntimeException(
                    "File not found when creating network file: " + nodeRef, e);
        }
    ...
    public String toString() {
        ...
        str.append(",channel=");
        str.append(channel);
        if (channel != null) str.append(writableChannel ? "(Write)" : "(Read)");
        ...
        if (modified) str.append(",Modified");
        if (isClosed()) str.append(",Closed");
        str.append(",open=");
     ...
     *
     * <p>Side effect: sets fileSize
     *
     * <p>If the channel is already open, it is left.
     *
    ...
    public void openContent(boolean write, boolean trunc)
        ...
        if (isDirectory()) {
            throw new AlfrescoRuntimeException(
                    "Unable to open content for a directory network file: " + this);
        }
        ...
        synchronized (this) {
            ...
            if (write && !isWritable()) {
                throw new AccessDeniedException(
                        "The network file was created for read-only: " + this);
            }
            ...
            if (write) {
                ...
                // Get the writable channel, do not copy existing content data if the file is to be
                // truncated
    ...
    public void closeFile() throws IOException {
        ...
        synchronized (this) {
            if (modified) {
                ...
                // We may be in a retry block, in which case this section will already have executed
                // and channel will be null
                if (channel != null) {
                    // Close the channel
                    channel.close();
                    channel = null;
                ...
                // Retrieve the content data and stop the content URL from being 'eagerly deleted',
                // in case we need to
                ...
                // Tidy up after ourselves after a successful commit. Otherwise leave things to
                // allow a retry.
    ...
    public void truncateFile(long size) throws IOException {
        ...
        if (logger.isDebugEnabled()) logger.debug("Truncate file=" + this + ", size=" + size);
    }
    ...
    public void writeFile(byte[] buffer, int length, int position, long fileOffset)
        ...
        if (logger.isDebugEnabled()) logger.debug("Write file=" + this + ", size=" + count);
    }
    ...
    public int readFile(byte[] buffer, int length, int position, long fileOffset)
        ...
        if (getFileState() != null) getFileState().updateAccessDateTime();

        // DEBUG

        if (logger.isDebugEnabled()) logger.debug("Read file=" + this + " read=" + count);
    ...
    public long seekFile(long pos, int typ) throws IOException {
        ...
        switch (typ) {
            ...
            case SeekType.StartOfFile:
                if (curPos != pos) channel.position(pos);
                break;
    ...
    public void flushFile() throws IOException {
        ...
        if (logger.isDebugEnabled()) logger.debug("Flush file=" + this);
    }
     ...
     * @return boolean
     */
    private static final boolean isMSOfficeSpecialFile(
            String path, SrvSession sess, NodeService nodeService, NodeRef nodeRef) {
        ...
        if (path.endsWith(".xls") && sess instanceof SMBSrvSession) {
            ...
            if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_VERSIONABLE)) return true;
        }
        return false;
    }
     ...
     * @return boolean
     */
    private static final boolean isOpenOfficeSpecialFile(
            String path, SrvSession sess, NodeService nodeService, NodeRef nodeRef) {

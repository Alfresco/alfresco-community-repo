        ...
        switch (consistency) {
            ...
            case TRANSACTIONAL:
                if (dbQueryLanguage != null) {
                    ...
                    stopWatch.start();
                    ResultSet results =
                            dbQueryLanguage.executeQuery(flattenDBQuery(searchParameters));
                    stopWatch.stop();
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "DB returned "
            ...
            default:
                ...
                if (dbQueryLanguage != null && !searchParameters.hasFaceting()) {
                    try {
                        ...
                        // MNT-10323: Logging configuration on JBoss leads to clogging of the log
                        // with a lot of these errors because of INFO level when WQS module is
                        // installed
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "DB query failed for "

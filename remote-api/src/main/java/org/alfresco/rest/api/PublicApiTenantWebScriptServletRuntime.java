...
public class PublicApiTenantWebScriptServletRuntime
  ...
  private static final Pattern CMIS_URI_PATTERN = Pattern.compile(
  ...
  protected String getScriptUrl() {
    ...
    // MNT-13057 fix, do not decode CMIS uris.
      ...
      pathInfo =
        URLDecoder.decode(requestURI.substring(serviceContextPath.length()));
    }
    ...
    if (pathInfo.length() < 2 || pathInfo.equals("/")) {
      ...
      if (
        !pathInfo.substring(0, 6).toLowerCase().equals("/cmis/") &&
  ...
  protected WebScriptRequest createRequest(Match match) {
    ...
    servletReq =
      new PublicApiTenantWebScriptServletRequest(
        this,
        req,
        match,
        serverProperties
  ...
  protected void renderErrorResponse(
    ...
    if (
      CMIS_URI_PATTERN.matcher(req.getRequestURI()).matches() ||

...
public class SiteMembershipRequestsImpl implements SiteMembershipRequests {
    ...
    public static final String DEFAULT_ROLE = SiteModel.SITE_CONSUMER;

    // List site memberships filtering (via where clause)
    private static final Set<String> LIST_SITE_MEMBERSHIPS_EQUALS_QUERY_PROPERTIES =
            new HashSet<>(Arrays.asList(new String[] {PARAM_SITE_ID, PARAM_PERSON_ID}));
    ...
    private Invitation getSiteInvitation(String inviteeId, String siteId) {
        ...
        if (invitations.size() > 1) {
            ...
            throw new AlfrescoRuntimeException(
                    "There should be only one outstanding site invitation");
        }
    ...
    private SiteMembershipRequest inviteToModeratedSite(
        ...
        ModeratedInvitation invitation =
                invitationService.inviteModerated(
                        message, inviteeId, ResourceType.WEB_SITE, siteId, inviteeRole, clientName);
    ...
    private SiteMembershipRequest inviteToSite(
            String siteId, String inviteeId, String inviteeRole, String message) {
    ...
    private SiteMembershipRequest inviteToPublicSite(
        ...
        String siteCreator =
                (String) nodeService.getProperty(siteNodeRef, ContentModel.PROP_CREATOR);
        ...
        if (StringUtils.isNotEmpty(siteNetwork)) {
            ...
            siteMembershipRequest =
                    TenantUtil.runAsUserTenant(
                            new TenantRunAsWork<SiteMembershipRequest>() {
                                @Override
            ...
            siteMembershipRequest =
                    AuthenticationUtil.runAs(
                            new RunAsWork<SiteMembershipRequest>() {
                                @Override
                                public SiteMembershipRequest doWork() throws Exception {
                                    return inviteToSite(siteId, inviteeId, inviteeRole, message);
                                }
    ...
    public SiteMembershipRequest createSiteMembershipRequest(
        ...
        // Note that the order of error checking is important. The server first needs to check for
        // the status 404
        // conditions before checking for status 400 conditions. Otherwise the server is open to a
        // probing attack.
        ...
        if (isMember) {
            ...
            throw new InvalidArgumentException(
                    inviteeId + " is already a member of site " + siteId);
        }
    ...
    @Override
    public SiteMembershipRequest createSiteMembershipRequest(
            String inviteeId, SiteMembershipRequest siteInvite, String client) {
        SiteMembershipRequest request = null;
    ...
    public SiteMembershipRequest updateSiteMembershipRequest(
        ...
        try {
            ModeratedInvitation updatedInvitation =
                    invitationService.updateModeratedInvitation(inviteeId, siteId, message);
    ...
    public SiteMembershipRequest getSiteMembershipRequest(String inviteeId, final String siteId) {
        ...
        if (siteInfo.getVisibility().equals(SiteVisibility.MODERATED)) {
            // set the site id to the short name (to deal with case sensitivity issues with using
            // the siteId from the url)
    ...
    private SiteMembershipRequest getSiteMembershipRequest(
            ModeratedInvitation moderatedInvitation, boolean includePersonDetails) {
        SiteMembershipRequest siteMembershipRequest = null;
        ...
        if (resourceType.equals(ResourceType.WEB_SITE)) {
            ...
            logger.warn(
                    "Unexpected resource type " + resourceType + " for site membership request");
        }
    ...
    private List<SiteMembershipRequest> toSiteMembershipRequests(
            List<Invitation> invitations, boolean includePersonDetails) {
        List<SiteMembershipRequest> siteMembershipRequests =
                new ArrayList<SiteMembershipRequest>(invitations.size());
        for (Invitation invitation : invitations) {
            if (invitation instanceof ModeratedInvitation) {
                ModeratedInvitation moderatedInvitation = (ModeratedInvitation) invitation;
                SiteMembershipRequest siteMembershipRequest =
                        getSiteMembershipRequest(moderatedInvitation, includePersonDetails);
                if (siteMembershipRequest != null) {
                    // note: siteMembershipRequest may be null if the site is now no longer a
                    // moderated site
                    // or if the invitation is malformed and does not refer to a site.
                    siteMembershipRequests.add(siteMembershipRequest);
                }
            } else {
                // just ignore, shouldn't happen because getSiteInvitations filters by
                // ModeratedInvitation
            }
        }
    ...
    private CollectionWithPagingInfo<SiteMembershipRequest> createPagedSiteMembershipRequests(
            List<SiteMembershipRequest> siteMembershipRequests, Paging paging) {
        ...
        if (skipCount >= totalItems) {
            ...
            return CollectionWithPagingInfo.asPaged(
                    paging, siteMembershipRequests, hasMoreItems, totalItems);
        }
    }

    @Override
    public CollectionWithPagingInfo<SiteMembershipRequest> getPagedSiteMembershipRequests(
            String personId, Paging paging) {
    ...
    public CollectionWithPagingInfo<SiteMembershipRequest> getPagedSiteMembershipRequests(
        ...
        if (q != null) {
            ...
            String siteId =
                    propertyWalker.getProperty(
                            PARAM_SITE_ID, WhereClauseParser.EQUALS, String.class);
            ...
            String personId =
                    propertyWalker.getProperty(
                            PARAM_PERSON_ID, WhereClauseParser.EQUALS, String.class);
        ...
        List<Invitation> invitations = invitationService.searchInvitation(criteria);
        return createPagedSiteMembershipRequests(
                toSiteMembershipRequests(invitations, true), parameters.getPaging());
    }
    ...
    public void approveSiteMembershipRequest(
        ...
        if (!resourceType.equals(ResourceType.WEB_SITE)
                || !SiteVisibility.MODERATED.equals(siteInfo.getVisibility())) {
        ...
        if (siteMembershipApproval != null
                && !(siteMembershipApproval.getRole() == null
                        || siteMembershipApproval.getRole().isEmpty())) {
    ...
    public void rejectSiteMembershipRequest(
        ...
        String reason = null;
        if (siteMembershipRejection != null
                && !(siteMembershipRejection.getComment() == null
                        || siteMembershipRejection.getComment().isEmpty())) {
    ...
    private void addSiteMembership(

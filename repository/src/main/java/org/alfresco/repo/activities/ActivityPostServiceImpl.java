...
public class ActivityPostServiceImpl implements ActivityPostService {
    ...
    public void postActivity(
        ...
        postActivity(
                ...
                appTool,
                activityData,
                ActivityPostEntity.STATUS.PENDING,
                getCurrentUser(),
                null,
                null);
    }
    ...
    public void postActivity(String activityType, String siteId, String appTool, NodeRef nodeRef) {
        ...
        sb.append("{")
                .append("\"" + PostLookup.JSON_NODEREF_LOOKUP + "\":\"")
    ...
    public void postActivity(
        ...
        sb.append("{")
                ...
                .append("\"")
                .append(",")
                .append("\"" + PostLookup.JSON_NAME + "\":\"")
                .append(name)
                .append("\"")
                .append(",")
                .append("\"" + PostLookup.JSON_TYPEQNAME + "\":\"")
                .append(typeQName.toPrefixString())
                .append("\"")
                .append(",") // TODO toPrefixString does not return prefix ??!!
                .append("\"" + PostLookup.JSON_NODEREF_PARENT + "\":\"")
                .append(parentNodeRef.toString())
                .append("\"")
                .append("}");
    ...
    private void postActivity(
            ...
            String userId,
            final Client client,
            final FileInfo contentNodeInfo) {
        ...
        try {
            ...
            if (siteId == null) {
                ...
                throw new IllegalArgumentException(
                        "Invalid siteId - exceeds "
                                + ActivityPostDAO.MAX_LEN_SITE_ID
                                + " chars: "
                                + siteId);
            }
            ...
            if (appTool == null) {
                ...
                throw new IllegalArgumentException(
                        "Invalid app tool - exceeds "
                                + ActivityPostDAO.MAX_LEN_APP_TOOL_ID
                                + " chars: "
                                + appTool);
            }
            ...
            if (activityType.length() > ActivityPostDAO.MAX_LEN_ACTIVITY_TYPE) {
                throw new IllegalArgumentException(
                        "Invalid activity type - exceeds "
                                + ActivityPostDAO.MAX_LEN_ACTIVITY_TYPE
                                + " chars: "
                                + activityType);
            }
            ...
            try {
                ...
                // throw new IllegalArgumentException("Invalid activity data - not valid JSON: " +
                // e);
                // According to test data in
                // org/alfresco/repo/activities/script/test_activityService.js
            ...
            if (activityData.length() > ActivityPostDAO.MAX_LEN_ACTIVITY_DATA) {
                throw new IllegalArgumentException(
                        "Invalid activity data - exceeds "
                                + ActivityPostDAO.MAX_LEN_ACTIVITY_DATA
                                + " chars: "
                                + activityData);
            }
            ...
            if (userId.length() > ActivityPostDAO.MAX_LEN_USER_ID) {
                throw new IllegalArgumentException(
                        "Invalid user - exceeds "
                                + ActivityPostDAO.MAX_LEN_USER_ID
                                + " chars: "
                                + userId);
            }
            ...
            eventPublisher.publishEvent(
                    new EventPreparator() {
                        ...
                        public Event prepareEvent(
                                String user, String networkId, String transactionId) {
                            ...
                            if (fileInfo != null) {
                                if (logger.isDebugEnabled()) {
                                    logger.debug(
                                            "Enhancing the Activity Event with fileInfo provided"
                                                    + " for node "
                                                    + nodeId);
                                }
    ...
    private String getCurrentUser() {
        ...
        if ((userId != null)
                && (!userId.equals(AuthenticationUtil.SYSTEM_USER_NAME))
    ...
    /**
     * Validate that the nodeRef property - if present in the activity data - is valid on a basic
     * level (it can be used to construct a NodeRef object).
     *
     * @param jo

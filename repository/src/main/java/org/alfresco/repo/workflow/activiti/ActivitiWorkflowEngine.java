...
public class ActivitiWorkflowEngine
  ...
  private static final String ERR_DEPLOY_WORKFLOW =
    "activiti.engine.deploy.workflow.error";
  private static final String ERR_IS_WORKFLOW_DEPLOYED =
    "activiti.engine.is.workflow.deployed.error";
  private static final String ERR_UNDEPLOY_WORKFLOW =
    "activiti.engine.undeploy.workflow.error";
  private static final String ERR_UNDEPLOY_WORKFLOW_UNEXISTING =
    "activiti.engine.undeploy.workflow.unexisting.error";
  private static final String ERR_GET_WORKFLOW_DEF =
    "activiti.engine.get.workflow.definition.error";
  private static final String ERR_GET_WORKFLOW_DEF_BY_ID =
    "activiti.engine.get.workflow.definition.by.id.error";
  private static final String ERR_GET_WORKFLOW_DEF_BY_NAME =
    "activiti.engine.get.workflow.definition.by.name.error";
  private static final String ERR_GET_ALL_DEFS_BY_NAME =
    "activiti.engine.get.all.workflow.definitions.by.name.error";
  private static final String ERR_GET_DEF_IMAGE =
    "activiti.engine.get.workflow.definition.image.error";
  private static final String ERR_GET_DEF_UNEXISTING_IMAGE =
    "activiti.engine.get.workflow.definition.unexisting.image.error";
  private static final String ERR_START_WORKFLOW =
    "activiti.engine.start.workflow.error";
  private static final String ERR_GET_WORKFLOW_INSTS =
    "activiti.engine.get.workflows.error";
  private static final String ERR_GET_ACTIVE_WORKFLOW_INSTS =
    "activiti.engine.get.active.workflows.error";
  private static final String ERR_GET_COMPLETED_WORKFLOW_INSTS =
    "activiti.engine.get.completed.workflows.error";
  private static final String ERR_GET_WORKFLOW_PATHS =
    "activiti.engine.get.workflow.paths.error";
  private static final String ERR_CANCEL_WORKFLOW =
    "activiti.engine.cancel.workflow.error";
  private static final String ERR_CANCEL_UNEXISTING_WORKFLOW =
    "activiti.engine.cancel.unexisting.workflow.error";
  private static final String ERR_DELETE_WORKFLOW =
    "activiti.engine.delete.workflow.error";
  private static final String ERR_DELETE_UNEXISTING_WORKFLOW =
    "activiti.engine.delete.unexisting.workflow.error";
  protected static final String ERR_FIRE_EVENT_NOT_SUPPORTED =
    "activiti.engine.event.unsupported";
  private static final String ERR_GET_TASKS_FOR_PATH =
    "activiti.engine.get.tasks.for.path.error";
  private static final String ERR_GET_TIMERS =
    "activiti.engine.get.timers.error";
  protected static final String ERR_FIND_COMPLETED_TASK_INSTS =
    "activiti.engine.find.completed.task.instances.error";
  private static final String ERR_GET_WORKFLOW_TOKEN_INVALID =
    "activiti.engine.get.workflow.token.invalid";
  private static final String ERR_GET_WORKFLOW_TOKEN_NULL =
    "activiti.engine.get.workflow.token.is.null";

  // Task Component Messages
  private static final String ERR_GET_ASSIGNED_TASKS =
    "activiti.engine.get.assigned.tasks.error";
  private static final String ERR_GET_POOLED_TASKS =
    "activiti.engine.get.pooled.tasks.error";
  private static final String ERR_UPDATE_TASK =
    "activiti.engine.update.task.error";
  private static final String ERR_UPDATE_TASK_UNEXISTING =
    "activiti.engine.update.task.unexisting.error";
  private static final String ERR_UPDATE_START_TASK =
    "activiti.engine.update.starttask.illegal.error";
  private static final String ERR_END_UNEXISTING_TASK =
    "activiti.engine.end.task.unexisting.error";
  private static final String ERR_GET_TASK_BY_ID =
    "activiti.engine.get.task.by.id.error";
  private static final String ERR_END_TASK_INVALID_TRANSITION =
    "activiti.engine.end.task.invalid.transition";

  public static final QName QNAME_INITIATOR = QName.createQName(
  ...
  public WorkflowInstance deleteWorkflow(String workflowId) {
    ...
    try {
      ...
      if (deletedInstance == null) {
        throw new WorkflowException(
          messageService.getMessage(ERR_DELETE_UNEXISTING_WORKFLOW, localId)
        );
      }
  ...
  public WorkflowDeployment deployDefinition(
    ...
    try {
      String resourceName =
        GUID.generate() + BpmnDeployer.BPMN_RESOURCE_SUFFIXES[0];
      ...
      List<ProcessDefinition> definitionList = repoService
      ...
      if (definitionList != null && definitionList.size() > 0) {
        ...
        for (ProcessDefinition processDefinition : definitionList) {
          if (
            WorkflowDeployer.CATEGORY_ALFRESCO_INTERNAL.equals(
        ...
        if (internalCategory) {
          repoService.setDeploymentCategory(
            deployment.getId(),
  ...
  public List<WorkflowInstance> getCompletedWorkflows() {
    try {
      ...
      String message = messageService.getMessage(
        ERR_GET_COMPLETED_WORKFLOW_INSTS,
        ""
      );
      if (logger.isDebugEnabled()) {
        logger.debug(message, ae);
  ...
  public List<WorkflowInstance> getActiveWorkflows(
    ...
    try {
      return getWorkflows(
        new WorkflowInstanceQuery(workflowDefinitionId, true)
      );
    } catch (ActivitiException ae) {
      String message = messageService.getMessage(
        ERR_GET_ACTIVE_WORKFLOW_INSTS,
  ...
  public List<WorkflowDefinition> getAllDefinitions() {
    try {
      ...
      if (
        activitiUtil.isMultiTenantWorkflowDeploymentEnabled() &&
  ...
  public List<WorkflowDefinition> getAllDefinitionsByName(String workflowName) {
    try {
      ...
      List<ProcessDefinition> definitions = repoService
        .createProcessDefinitionQuery()
  ...
  public List<WorkflowInstance> getCompletedWorkflows(
    ...
    try {
      return getWorkflows(
        new WorkflowInstanceQuery(workflowDefinitionId, false)
      );
    } catch (ActivitiException ae) {
      String message = messageService.getMessage(
        ERR_GET_COMPLETED_WORKFLOW_INSTS,
  ...
  public WorkflowDefinition getDefinitionById(String workflowDefinitionId) {
    try {
      ...
      ProcessDefinition procDef = repoService
        .createProcessDefinitionQuery()
        .processDefinitionId(localId)
        .singleResult();

      if (
        activitiUtil.isMultiTenantWorkflowDeploymentEnabled() && procDef != null
      ) {
  ...
  public WorkflowDefinition getDefinitionByName(String workflowName) {
    try {
      ...
      ProcessDefinition definition = activitiUtil.getProcessDefinitionByKey(
        key
      );
      return typeConverter.convert(definition);
    } catch (ActivitiException ae) {
      String msg = messageService.getMessage(
        ERR_GET_WORKFLOW_DEF_BY_NAME,
        workflowName
      );
      if (logger.isDebugEnabled()) {
        logger.debug(msg, ae);
  ...
  public byte[] getDefinitionImage(String workflowDefinitionId) {
    try {
      ...
      ProcessDefinition processDefinition = repoService
        .createProcessDefinitionQuery()
  ...
  public List<WorkflowDefinition> getDefinitions() {
    try {
      ProcessDefinitionQuery query = repoService
        .createProcessDefinitionQuery()
        .latestVersion();
      if (
        activitiUtil.isMultiTenantWorkflowDeploymentEnabled() &&
  ...
  public List<WorkflowTaskDefinition> getTaskDefinitions(
    ...
    ReadOnlyProcessDefinition processDefinition =
      ((RepositoryServiceImpl) repoService).getDeployedProcessDefinition(
          processDefinitionId
        );
    ...
    defs.add(
      typeConverter.getTaskDefinition(
        startEvent,
        startTaskName,
        processDefinition.getId(),
        true
    ...
    Collection<PvmActivity> taskActivities = typeConverter.findUserTasks(
      startEvent
    );
  ...
  private boolean isReceiveTask(PvmActivity act) {
    if (act instanceof ActivityImpl) {
      ActivityImpl actImpl = (ActivityImpl) act;
      return (
        actImpl.getActivityBehavior() instanceof ReceiveTaskActivityBehavior
      );
    }
    return false;
  }
  ...
  private boolean isFirstActivity(
    ...
    ReadOnlyProcessDefinition procDef
  ) {
  ...
  public List<WorkflowTask> getTasksForWorkflowPath(String pathId) {
    try {
      ...
      if (executionId == null) {
        throw new WorkflowException(
          messageService.getMessage(ERR_GET_WORKFLOW_TOKEN_INVALID, pathId)
        );
      }
      ...
      if (execution == null) {
        throw new WorkflowException(
          messageService.getMessage(ERR_GET_WORKFLOW_TOKEN_NULL, pathId)
        );
      }
      ...
      if (typeConverter.isStartTaskActive(processInstanceId)) {
        resultList.add(
          typeConverter.getVirtualStartTask(processInstanceId, true)
        );
      } else {
  ...
  public List<WorkflowTimer> getTimers(String workflowId) {
    try {
      ...
      for (Job job : timerJobs) {
        Execution jobExecution = runtimeService
          .createExecutionQuery()
          .executionId(job.getExecutionId())
          .singleResult();

        WorkflowPath path = typeConverter.convert(
          jobExecution,
          jobsProcessInstance
        );
        WorkflowTask workflowTask = getTaskForTimer(
          job,
          jobsProcessInstance,
          jobExecution
        );

        WorkflowTimer workflowTimer = factory.createWorkflowTimer(
          job.getId(),
          job.getId(),
  ...
  private WorkflowTask getTaskForTimer(
    Job job,
  ...
  public WorkflowInstance getWorkflowById(String workflowId) {
    try {
      ...
      String processInstanceId = createLocalId(workflowId);
      ProcessInstance processIntance = runtimeService
        .createProcessInstanceQuery()
        .processInstanceId(processInstanceId)
        .singleResult();
  ...
  private String getProcessKey(InputStream workflowDefinition)
    throws Exception {
    try {
      ...
      if (idAttrib == null) {
        throw new IllegalAccessError(
          "The process definition does not have an id!"
        );
      }
  ...
  public WorkflowPath signal(String pathId, String transitionId) {
    ...
    return typeConverter.buildCompletedPath(
      execId,
      oldExecution.getProcessInstanceId()
    );
  }
  ...
  public WorkflowPath startWorkflow(
    ...
    try {
      ...
      Map<String, Object> variables = propertyConverter.getStartVariables(
        processDefId,
        parameters
      );
      ...
      if (initiator != null) {
        ...
        NodeRef initiatorHome = (NodeRef) nodeService.getProperty(
          initiator,
          ContentModel.PROP_HOMEFOLDER
        );
        if (initiatorHome != null) {
          variables.put(
            WorkflowConstants.PROP_INITIATOR_HOME,
   ...
   */
  private void endStartTaskAutomatically(
    WorkflowPath path,
  ...
  public void undeployDefinition(String workflowDefinitionId) {
    try {
      ...
      if (procDef == null) {
        String msg = messageService.getMessage(
          ERR_UNDEPLOY_WORKFLOW_UNEXISTING,
          workflowDefinitionId
        );
   ...
   */
  private List<WorkflowDefinition> getValidWorkflowDefinitions(
    List<ProcessDefinition> definitions
  ) {
    return typeConverter.filterByDomainAndConvert(
      definitions,
    ...
    return typeConverter.filterByDomainAndConvert(
      ...
      new Function<Task, String>() {
        public String apply(Task task) {
          ...
          ProcessDefinition definition = repoService
            .createProcessDefinitionQuery()
   ...
   */
  private List<WorkflowTask> getValidHistoricTasks(
    ...
    return typeConverter.filterByDomainAndConvert(
      ...
      new Function<HistoricTaskInstance, String>() {
        public String apply(HistoricTaskInstance task) {
          String defId = task.getProcessDefinitionId();
          ProcessDefinition definition = (ProcessDefinition) activitiUtil.getDeployedProcessDefinition(
            defId
          );
          return definition.getKey();
        }
  ...
  private WorkflowTask endNormalTask(
    String taskId,
    String localTaskId,
    String transition
  ) {
    ...
    String currentUserName = AuthenticationUtil.getFullyAuthenticatedUser();
    if (
      task.getAssignee() == null || !task.getAssignee().equals(currentUserName)
    ) {
  ...
  private void setOutcome(Task task, String transition) {
    ...
    if (outcomePropName != null) {
      if (isDefaultTransition == false) {
        ...
        Serializable transitionValue = propertyConverter.convertValueToPropertyType(
          task,
          transition,
          outcomePropName
        );
  ...
  private WorkflowTask endStartTask(String localTaskId) {
    ...
    String processInstanceId = localTaskId.replace(
      ActivitiConstants.START_TASK_PREFIX,
      ""
    );
    ...
    ProcessInstance processInstance = activitiUtil.getProcessInstance(
      processInstanceId
    );
    String currentActivity =
      ((ExecutionEntity) processInstance).getActivityId();
      ...
      if (state == WorkflowTaskState.IN_PROGRESS) {
        ...
        for (Task task : tasks) {
          if (lazyInitialization) {
            resultingTasks.add(
              new LazyActivitiWorkflowTask(
                task,
                typeConverter,
                tenantService,
                typeConverter.getWorkflowDefinitionName(
      ...
      if (authorities != null && authorities.size() > 0) {
        ...
        if (authorityManager.isUser(firstAuthority)) {
          ...
          if (authorities.size() > 1) {
            List<String> remainingAuthorities = authorities.subList(
              1,
              authorities.size()
            );
        ...
        for (Task task : resultingTasks.values()) {
          if (task.getAssignee() == null) {
            ...
            if (lazyInitialization) {
              String workflowDefinitionName = typeConverter.getWorkflowDefinitionName(
                task.getProcessDefinitionId()
              );
              try {
                workflowDefinitionName =
                  tenantService.getBaseName(workflowDefinitionName);
                currentTask =
                  new LazyActivitiWorkflowTask(
                    task,
                    typeConverter,
  ...
  private void addTasksForCandidateGroups(
  ...
  private void addTasksForCandidateUser(
    String userName,
    Map<String, Task> resultingTasks
  ) {
  ...
  public WorkflowTask getTaskById(String taskId) {
    try {
      ...
      if (localId.startsWith(ActivitiConstants.START_TASK_PREFIX)) {
        String processInstanceId = localId.replace(
          ActivitiConstants.START_TASK_PREFIX,
          ""
        );
        ...
        HistoricTaskInstance historicTask = activitiUtil.getHistoricTaskInstance(
          localId
        );
        return typeConverter.convert(historicTask);
      }
  ...
  private void addProcessPropertiesToQuery(
    ...
    for (Entry<QName, Object> customProperty : processCustomProps.entrySet()) {
      ...
      if (name != ActivitiConstants.VAR_TENANT_DOMAIN) {
        ...
        Object converted = propertyConverter.convertPropertyToValue(
  ...
  private void addTaskPropertiesToQuery(
  ...
  private TaskQuery createRuntimeTaskQuery(WorkflowTaskQuery query) {
    ...
    if (!activitiUtil.isMultiTenantWorkflowDeploymentEnabled()) {
      ...
      taskQuery.processDefinitionKeyLike(
        "@" + TenantUtil.getCurrentDomain() + "@%"
      );
    }
    ...
    if (query.getTaskName() != null) {
      ...
      taskQuery.taskVariableValueEquals(
    ...
    if (query.getWorkflowDefinitionName() != null) {
      String processName = factory.getProcessKey(
        query.getWorkflowDefinitionName()
      );
      taskQuery.processDefinitionKey(processName);
    }
  ...
  private HistoricTaskInstanceQuery createHistoricTaskQuery(
    WorkflowTaskQuery query
  ) {
    ...
    if (!activitiUtil.isMultiTenantWorkflowDeploymentEnabled()) {
      // Filter by tenant domain
      historicQuery.processVariableValueEquals(
        ActivitiConstants.VAR_TENANT_DOMAIN,
        TenantUtil.getCurrentDomain()
      );
    } else if (
      ...
      // Process definition keys are prefixed with the tenant ID, in case MT is enabled and
      // deployments are done in tenant-context
      historicQuery.processDefinitionKeyLike(
        "@" + TenantUtil.getCurrentDomain() + "@%"
      );
    }
  ...
  private List<WorkflowTask> queryStartTasks(WorkflowTaskQuery query) {
    ...
    if (taskId != null) {
      String localTaskId = createLocalId(taskId);
      if (
        localTaskId.startsWith(ActivitiConstants.START_TASK_PREFIX)
      ) processInstanceId =
        localTaskId.substring(ActivitiConstants.START_TASK_PREFIX.length());
    } else {
    ...
    if (processInstanceId != null) {
      WorkflowTask workflowTask = typeConverter.getVirtualStartTask(
        processInstanceId,
        null
      );
  ...
  private boolean isStartTaskMatching(
    ...
    if (
      query.getActorId() != null &&
      !query
        .getActorId()
        .equals(workflowTask.getProperties().get(ContentModel.PROP_OWNER))
    ) {
      return false;
    }
    ...
    // See MNT-10931
    if (
      query.getProcessCustomProps() != null &&
      ...
      Map<QName, Serializable> props = getPathProperties(
        workflowTask.getPath().getId()
      );
    ...
    if (query.getProcessId() != null) {
      if (
        !query
          .getProcessId()
          .equals(workflowTask.getPath().getInstance().getId())
      ) {
        return false;
      }
    }
    ...
    if (query.getTaskName() != null) {
      if (
        !query
          .getTaskName()
          .equals(workflowTask.getDefinition().getMetadata().getName())
      ) {
        return false;
      }
    }
  ...
  private boolean checkPropertiesPresent(
    Map<QName, Object> expectedProperties,
  ...
  public WorkflowTask updateTask(
    ...
    try {
      ...
      Task task = taskService
        .createTaskQuery()
        .taskId(createLocalId(taskId))
        .singleResult();
      if (task != null) {
        Task updatedTask = propertyConverter.updateTask(
          task,
          properties,
          add,
          remove
  ...
  public List<WorkflowInstance> getWorkflows(
    ...
    if (Boolean.FALSE.equals(workflowInstanceQuery.getActive()) == false) {
      //Add active.
      results.addAll(
        getWorkflowsInternal(workflowInstanceQuery, true, maxItems, skipCount)
      );
    }
    if (Boolean.TRUE.equals(workflowInstanceQuery.getActive()) == false) {
      //Add complete
      results.addAll(
        getWorkflowsInternal(workflowInstanceQuery, false, maxItems, skipCount)
      );
    }
  ...
  private List<WorkflowInstance> getWorkflowsInternal(
    ...
    List<WorkflowInstance> completedResults = typeConverter.doSpecialTenantFilterAndSafeConvert(
      completedInstances,
      new Function<HistoricProcessInstance, String>() {
        public String apply(HistoricProcessInstance historicProcessInstance) {
          ProcessDefinition procDef = activitiUtil.getProcessDefinition(
  ...
  private HistoricProcessInstanceQuery createQuery(
    ...
    // MNT-9074 My Tasks fails to render if tasks quantity is excessive
    String processDefId = workflowInstanceQuery.getWorkflowDefinitionId() ==
      null
      ? null
    ...
    if (workflowInstanceQuery.getCustomProps() != null) {
      ...
      if (initiatorObject != null && initiatorObject instanceof NodeRef) {
        ...
        if (this.nodeService.exists(initiator)) {
          String initiatorUserName = (String) nodeService.getProperty(
            initiator,
      ...
      for (Map.Entry<QName, Object> prop : customProps.entrySet()) {
        ...
        if (prop.getValue() == null) {
          ...
          if (propertyDefinition == null) {
            ...
            String propertyType = propertyDefinition
              .getDataType()
              .getJavaClassName();
              ...
              for (Map.Entry<DatePosition, Date> dateProp : dateProps.entrySet()) {
                if (dateProp.getValue() != null) {
                  if (dateProp.getKey() == DatePosition.BEFORE) {
                    query.variableValueLessThanOrEqual(
                      propertyName,
                      dateProp.getValue()
                    );
                  }
                  if (dateProp.getKey() == DatePosition.AFTER) {
                    query.variableValueGreaterThanOrEqual(
                      propertyName,
                      dateProp.getValue()
                    );
                  }
  ...
  public void dispatchPackageUpdatedEvent(

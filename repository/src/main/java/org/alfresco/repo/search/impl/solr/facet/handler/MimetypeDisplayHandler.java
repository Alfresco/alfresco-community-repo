...
public class MimetypeDisplayHandler extends AbstractFacetLabelDisplayHandler {
  ...
  public MimetypeDisplayHandler(Set<String> supportedFieldFacets) {
    ...
    this.supportedFieldFacets =
      Collections.unmodifiableSet(new HashSet<>(supportedFieldFacets));
  }

  @Override
  public FacetLabel getDisplayLabel(String value) {
    Map<String, String> mimetypes = serviceRegistry
      .getMimetypeService()
      .getDisplaysByMimetype();
    String displayName = mimetypes.get(value);
    return new FacetLabel(
      value,
      displayName == null ? value : displayName.trim(),
      -1
    );
  }
}

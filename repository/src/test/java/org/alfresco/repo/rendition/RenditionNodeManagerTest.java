...
public class RenditionNodeManagerTest extends TestCase {
    ...
    private final QName renditionName =
            QName.createQName(NamespaceService.RENDITION_MODEL_1_0_URI, "renditionName");
    private final RenditionDefinition definition =
            new RenditionDefinitionImpl("definitionId", renditionName, "engineName");
    ...
    public void testNoOldRenditionAndDestinationSpecified() {
        ...
        RenditionLocation location =
                new RenditionLocationImpl(source, destination, "destinationName");
        RenditionNodeManager manager =
    ...
    public void testNoOldRenditionAndNoDestinationSpecifiedAndParentIsSource() {
        Map<QName, Serializable> indexProps =
                Collections.singletonMap(
                        ContentModel.PROP_IS_INDEXED, (Serializable) Boolean.FALSE);
        ChildAssociationRef parentAssoc = makeAssoc(source, destination, true);
        ...
        RenditionLocation location =
                new RenditionLocationImpl(source, null, renditionName.getLocalName());
        RenditionNodeManager manager =
                new RenditionNodeManager(
                        source,
                        null,
    ...
    public void testNoOldRenditionAndNoDestinationSpecifiedAndParentIsNotSource() {
        ...
        verify(nodeService)
                .addChild(source, destination, RenditionModel.ASSOC_RENDITION, renditionName);
    }
    ...
    public void testHasOldRenditionMatchesSpecifiedDestinationNode() {
        ...
        RenditionLocation location =
                new RenditionLocationImpl(source, oldRendition, renditionName.getLocalName());
    ...
    // Check findOrCreateRenditionNode() works when there is
    // an old rendition which has the specified parent folder.
    // If the correct name is specified and the parent folder is correct then the location should
    // match.
    ...
    public void testHasOldRenditionWrongParentSpecified() {
        ...
        assertEquals(parentAssoc, result);
        verify(nodeService)
                .moveNode(oldRendition, parent, ContentModel.ASSOC_CONTAINS, renditionName);

        // The old rendition is under the parent node but should be under the source node.
        when(nodeService.getPrimaryParent(oldRendition)).thenReturn(parentAssoc);
        when(nodeService.moveNode(
                        oldRendition, source, RenditionModel.ASSOC_RENDITION, renditionName))
        ...
        verify(nodeService)
                .moveNode(oldRendition, source, RenditionModel.ASSOC_RENDITION, renditionName);
        ...
        when(nodeService.getPrimaryParent(oldRendition)).thenReturn(parentAssoc);
        when(nodeService.moveNode(
                        oldRendition, newParent, ContentModel.ASSOC_CONTAINS, renditionName))
        ...
        verify(nodeService)
                .moveNode(oldRendition, newParent, ContentModel.ASSOC_CONTAINS, renditionName);
    }
    ...
    private ChildAssociationRef makeAssoc(
            NodeRef parent, NodeRef child, QName assocName, boolean isRenditionAssoc) {
        QName assocType =
                isRenditionAssoc ? RenditionModel.ASSOC_RENDITION : ContentModel.ASSOC_CONTAINS;

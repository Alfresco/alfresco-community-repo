...
public class GenericMimetypeRenamePatch extends AbstractPatch {
  ...
  protected String applyInternal() throws Exception {
    ...
    for (Map.Entry<String, String> element : mimetypeMappings.entrySet()) {
      ...
      if (newMimetypePair == null) {
        ...
        Long newMimetypeId = mimetypeDAO
          .getOrCreateMimetype(newMimetype)
          .getFirst();
        updateCount =
          patchDAO.updateContentMimetypeIds(oldMimetypeId, newMimetypeId);
      }
      result.append(
        I18NUtil.getMessage(MSG_UPDATED, updateCount, oldMimetype, newMimetype)
      );
    }
    // Done
    if (reindex) {
  ...
  private class Work
    ...
    public Integer execute() throws Throwable {
      List<Long> nodeIds = patchDAO.getNodesByContentPropertyMimetypeId(
        mimetypeId,
        lower,
        lower + BATCH_SIZE
      );

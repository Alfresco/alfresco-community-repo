...
public class ChainingUserRegistrySynchronizerTest extends TestCase {
  ...
  private static final String[] CONFIG_LOCATIONS = {
    ...
    "classpath:sync-test-context.xml",
  };
  ...
  private static final Log logger = LogFactory.getLog(
    ChainingUserRegistrySynchronizerTest.class
  );
  ...
  protected void setUp() throws Exception {
    this.synchronizer =
      (UserRegistrySynchronizer) ChainingUserRegistrySynchronizerTest.context.getBean(
        "testUserRegistrySynchronizer"
      );
    this.applicationContextManager =
      (MockApplicationContextManager) ChainingUserRegistrySynchronizerTest.context.getBean(
        "testApplicationContextManager"
      );
    this.personService =
      (PersonService) ChainingUserRegistrySynchronizerTest.context.getBean(
        "personService"
      );
    this.authorityService =
      (AuthorityService) ChainingUserRegistrySynchronizerTest.context.getBean(
        "authorityService"
      );
    this.nodeService =
      (NodeService) ChainingUserRegistrySynchronizerTest.context.getBean(
        "nodeService"
      );

    this.authenticationContext =
      (AuthenticationContext) ChainingUserRegistrySynchronizerTest.context.getBean(
    ...
    this.retryingTransactionHelper =
      (RetryingTransactionHelper) ChainingUserRegistrySynchronizerTest.context.getBean(
        "retryingTransactionHelper"
      );
    setHomeFolderCreationEager(false); // the normal default if using LDAP

    this.namespaceService =
      (NamespaceService) ChainingUserRegistrySynchronizerTest.context.getBean(
        "namespaceService"
      );
  }
  ...
  public void setHomeFolderCreationEager(boolean homeFolderCreationEager) {
    ...
    ((PersonServiceImpl) personService).setHomeFolderCreationEager(
        homeFolderCreationEager
      );
  }
  ...
  public void testDifferentialUpdate() throws Exception {
    ...
    this.applicationContextManager.updateZone(
        "Z1",
        new NodeDescription[] {
          newPerson("U1", "changeofemail@alfresco.com"),
          newPerson("U6"),
          newPerson("U7"),
        },
        new NodeDescription[] {
            ...
            AuthenticationUtil.runAsSystem(
              new RunAsWork<Void>() {
                public Void doWork() throws Exception {
                  ...
                  ChainingUserRegistrySynchronizerTest.this.synchronizer.synchronize(
                      false,
                      false
                    );
                  return null;
  ...
  private class MockLDAPUserRegistry
    ...
    public Collection<NodeDescription> getPersons(Date modifiedSince) {
      Collection<NodeDescription> persons = mockUserRegistry.getPersons(
        modifiedSince
      );
  ...
  private void testLDAPDisableUserAccount(
    ...
    MockUserRegistry mockUserRegistry = new MockUserRegistry(
      "ldap1",
      new NodeDescription[] {
        newPersonWithUserAccountStatusProperty(
          "EnabledUser",
          enabledAccountPropertyValue
        ),
        newPersonWithUserAccountStatusProperty(
          "DisabledUser",
          disabledAccountPropertyValue
        ),
      },
      new NodeDescription[] {}
    );

    MockLDAPUserRegistry mockLDAPUserRegistry = new MockLDAPUserRegistry(
      mockUserRegistry
    );
    mockLDAPUserRegistry.setUserAccountStatusInterpreter(
      userAccountStatusInterpreter
    );
  ...
  public void testLDAPDisableUserAccountWithActiveDirectoryProperty()
    throws Exception {
  ...
  public void testLDAPDisableUserAccountWithNetscapDSProperty()
    throws Exception {
  ...
  public void testLDAPDisableUserAccountWithOpenLDAPProperty()
    throws Exception {
    LDAPUserAccountStatusInterpreter ldapUserAccountStatusInterpreter = new LDAPUserAccountStatusInterpreter();
    ldapUserAccountStatusInterpreter.setAcceptNullArgument(true);

    // OpenLDAP disabled account: pwdAccountLockedTime=000001010000Z (part of PPolicy module)
    ldapUserAccountStatusInterpreter.setDisabledAccountPropertyValue(
      "000001010000Z"
    );
  ...
  public void testForcedUpdateWithoutDeletions() throws Exception {
    UserRegistrySynchronizer synchronizer = (UserRegistrySynchronizer) ChainingUserRegistrySynchronizerTest.context.getBean(
    ...
    this.applicationContextManager.setUserRegistries(
        new MockUserRegistry(
          ...
          new NodeDescription[] { newGroup("G1"), newGroup("G2") }
        ),
        new MockUserRegistry(
          "Z1",
          new NodeDescription[] { newPerson("U5"), newPerson("u6") },
          new NodeDescription[] {}
        ),
        new MockUserRegistry(
          "Z2",
  ...
  public void testVolume() throws Exception {
    List<NodeDescription> persons = new ArrayList<NodeDescription>(
      new RandomPersonCollection(100)
    );
    List<NodeDescription> groups = new ArrayList<NodeDescription>(
      new RandomGroupCollection(50, persons)
    );
    this.applicationContextManager.setUserRegistries(
  ...
  public void testTestSynchronize() throws Exception {
    ...
    MockUserRegistry testRegistry = new MockUserRegistry(
      TEST_REGISTRY_NAME,
      persons,
      groups
    );
    ...
    Collection<String> instances =
      this.applicationContextManager.getInstanceIds();
    try {
      // pre-conditions
      assertTrue(
        "pre-condition test registry isActive",
        testRegistry.isActive()
      );
      assertTrue(
      ...
      if (synchronizer instanceof ChainingUserRegistrySynchronizer) {
        ...
        SynchronizeDiagnostic diagnostic = chainingSynchronizer.testSynchronize(
          TEST_REGISTRY_NAME
        );
        ...
        fail(
          "test not run - synchroniser is not a ChainingUserRegistrySynchronizer"
        );
      }
    } finally {
      tearDownTestUsersAndGroups();
  ...
  public void testSyncStatus() throws Exception {
    ...
    try {
      List<NodeDescription> persons = new ArrayList<NodeDescription>(
        new RandomPersonCollection(3)
      );
      List<NodeDescription> groups = new ArrayList<NodeDescription>(
        new RandomGroupCollection(4, persons)
      );
      MockUserRegistry testRegistry = new MockUserRegistry(
        "Z0",
        persons,
      ...
      if (this.synchronizer instanceof ChainingUserRegistrySynchronizerStatus) {
        ...
        // Header Status
        assertTrue(
          "end time not updated",
          status.getSyncEndTime().after(testStart)
        );
        assertTrue(
          "start time not updated",
          status.getSyncStartTime().after(testStart)
        );
        assertEquals(
        ...
        assertNotNull(
          "last run on server is null",
          status.getLastRunOnServer()
        );
  ...
  public void testSyncDeletedProperty() throws Exception {
    try {
      ...
      assertTrue(
        "User's email must be null on a 2rd sync, since the email property was removed at the AD level.",
        userProperties.get(ContentModel.PROP_EMAIL) == null
      );
    } finally {
  ...
  private void executeMockedLDAPSyncWithActiveDirectoryEmailProp()
    throws Exception {
    executeMockedLDAPSync(true);
  }

  private void executeMockedLDAPSyncWithoutActiveDirectoryEmailProp()
    throws Exception {
    executeMockedLDAPSync(false);
  }

  private void executeMockedLDAPSync(boolean withEmail)
    ...
    LDAPInitialDirContextFactoryImpl mockedLdapInitialDirContextFactory = getMockedLDAPSearchResult(
      withEmail
    );

    mockLDAPUserRegistry.setLDAPInitialDirContextFactory(
  ...
  private LDAPInitialDirContextFactoryImpl getMockedLDAPSearchResult(
    boolean withEmail
  ) throws NamingException {
    @SuppressWarnings("unchecked")
    NamingEnumeration<SearchResult> mockedNamingEnumeration = mock(
    ...
    SearchResult mockedSearchResult = new SearchResult(
      "CN:U1",
      null,
      attributes
    );
    ...
    when(
      mockedInitialDirContext.search(
        (String) any(),
        anyString(),
    ...
    LDAPInitialDirContextFactoryImpl mockedLdapInitialDirContextFactory = mock(
      LDAPInitialDirContextFactoryImpl.class
    );
  ...
  public void dontTestAssocs() throws Exception {
              ...
              return new ArrayList<NodeDescription>(
                new RandomGroupCollection(
                  ...
                  ChainingUserRegistrySynchronizerTest.this.authorityService.getAllAuthoritiesInZone(
                      AuthorityService.ZONE_AUTH_EXT_PREFIX + "Z0",
                      null
                    )
                )
              );
    ...
    ChainingUserRegistrySynchronizerTest.this.applicationContextManager.setUserRegistries(
        new MockUserRegistry(
          "Z0",
          Collections.<NodeDescription>emptyList(),
          groups
        )
      );
    ChainingUserRegistrySynchronizerTest.this.synchronizer.synchronize(
  ...
  private NodeDescription newPersonWithUserAccountStatusProperty(
    String userName,
    String userAccountPropertyValue
  ) {
  ...
  private void assertExists(String zone, String name, String... members) {
    ...
    if (zone == null) {
      ...
      assertTrue(
        this.authorityService.getAuthorityZones(longName)
    ...
    if (AuthorityType.getAuthorityType(longName).equals(AuthorityType.GROUP)) {
      ...
      assertEquals(
        memberSet,
        this.authorityService.getContainedAuthorities(null, longName, true)
      );
    } else {
  ...
  private void assertEmailEquals(String personName, String email) {
    ...
    assertEquals(
      email,
      this.nodeService.getProperty(personRef, ContentModel.PROP_EMAIL)
    );
  }
  ...
  private void assertGroupDisplayNameEquals(String name, String displayName) {
    assertEquals(
      displayName,
      this.authorityService.getAuthorityDisplayName(longName(name))
    );
  }
  ...
  private String longName(String shortName) {
    return this.authorityService.getName(
        shortName.toLowerCase().startsWith("g")
          ? AuthorityType.GROUP
          : AuthorityType.USER,
        shortName
      );
  ...
  public static class TestSynchronizeEventListener
    ...
    public void onApplicationEvent(ApplicationEvent event) {
      if (
        SynchronizeEndEvent.class.isAssignableFrom(event.getClass()) ||
  ...
  public static interface IMockUserRegistry
    extends UserRegistry, ActivateableBean {
    /**
     * Gets the zone id.
     *
     * @return the zoneId
  ...
  public static class MockUserRegistry implements IMockUserRegistry {
    ...
    public void updateState(
      ...
      List<NodeDescription> newPersons = new ArrayList<NodeDescription>(
        this.persons
      );
      mergeNodeDescriptions(
        newPersons,
        persons,
      ...
      List<NodeDescription> newGroups = new ArrayList<NodeDescription>(
        this.groups
      );
      mergeNodeDescriptions(
        newGroups,
    ...
    private void mergeNodeDescriptions(
      ...
      Map<String, NodeDescription> nodeMap = new LinkedHashMap<String, NodeDescription>(
        newNodes.size() * 2
      );
      ...
      for (int i = 0; i < oldNodes.size(); i++) {
        ...
        if (newNode == null) {
          ...
          !oldNode.getProperties().equals(newNode.getProperties()) ||
    ...
    private Collection<NodeDescription> filterNodeDescriptions(
      Collection<NodeDescription> nodes,
      Date modifiedSince
    ) {
  ...
  public static class MockApplicationContextManager
    ...
    public void setUserRegistries(IMockUserRegistry... registries) {
      this.contexts =
        new LinkedHashMap<String, ApplicationContext>(registries.length * 2);
      for (IMockUserRegistry registry : registries) {
        StaticApplicationContext context = new StaticApplicationContext();
        context
          .getDefaultListableBeanFactory()
          .registerSingleton("userRegistry", registry);
    ...
    public void updateZone(
      ...
      IMockUserRegistry registry = (IMockUserRegistry) context.getBean(
        "userRegistry"
      );
    ...
    public Iterator<NodeDescription> iterator() {
      return new Iterator<NodeDescription>() {
        ...
        public NodeDescription next() {
          ...
          String[] authorityNames = new String[this.pos ==
          ...
          for (int i = 0; i < authorityNames.length; i++) {
            ...
            int index = this.pos == RandomGroupCollection.this.size
              ? i
              : RandomGroupCollection.this.generator.nextInt(
                  RandomGroupCollection.this.authorities.size()
                );
            authorityNames[i] =
              ChainingUserRegistrySynchronizerTest.this.authorityService.getShortName(
                  (String) RandomGroupCollection.this.authorities.get(index)
                );
          }
          NodeDescription group = newGroup(
            "G" + GUID.generate(),
            authorityNames
          );

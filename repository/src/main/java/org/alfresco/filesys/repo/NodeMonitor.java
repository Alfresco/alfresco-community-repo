 ...
 * Node Monitor Class
 *
 * <p>Monitor node events from the node service to update the file state cache and feed notification
 * events into the file server change notification handler.
 *
 * @author gkspencer
 */
public class NodeMonitor extends TransactionListenerAdapter
    ...
    public final void init() {
        ...
        String rootPath =
                (String)
                        m_nodeService.getProperty(
                                m_filesysCtx.getRootNode(), ContentModel.PROP_NAME);
    ...
    public void onCreateNode(ChildAssociationRef childAssocRef) {
        ...
        if (fType != FileFolderServiceType.INVALID) {
            ...
            // Store the event in the transaction until committed, and register the transaction
            // listener
        ...
        return AuthenticationUtil.runAs(
                new RunAsWork<StringBuilder>() {
                    ...
                    public StringBuilder doWork() throws Exception {
                        ...
                        Path nodePath = m_nodeService.getPath(nodeRef);
                        String fName =
                                (String) m_nodeService.getProperty(nodeRef, ContentModel.PROP_NAME);
     ...
     *
     * <p>ALF-2309: construct the path from the old parent of the moved node (parentNodeRef) - this
    ...
    private String buildRelativePathString(NodeRef parentNodeRef, String nodeName) {
        ...
        if (pathStr.length() == 0
                || pathStr.charAt(pathStr.length() - 1) != '/'
                        && pathStr.charAt(pathStr.length() - 1) != '\\') pathStr.append("/");
    ...
    public void run() {
        ...
        while (m_shutdown == false) {
            try {
                ...
                RetryingTransactionCallback<Object> processEventCallback =
    ...
    private final void processCreateNode(CreateNodeEvent createEvent) {
        ...
        if (relPath.startsWith(m_rootPath)) {
            ...
            // Update an existing file state to indicate that the file exists, may have been marked
            // as deleted
            ...
            if (m_filesysCtx.hasChangeHandler()) {
                ...
                if (m_filesysCtx.getChangeHandler().getGlobalNotifyMask() != 0) {
                    ...
                    if (createEvent.getFileType() == FileFolderServiceType.FILE)
                        m_filesysCtx
                                .getChangeHandler()
                                .notifyFileChanged(NotifyChange.ActionAdded, relPath);
                    else
                        m_filesysCtx
                                .getChangeHandler()
    ...
    private final void processDeleteNode(DeleteNodeEvent deleteEvent) {
        ...
        //
        //			if ( logger.isDebugEnabled())
        //				logger.debug("DeleteNode not confirmed, nodeRef=" + deleteEvent.getNodeRef() + ",
        // path=" + deleteEvent.getPath());
        //
        //			return;
        //		}
        ...
        if (m_filesysCtx.hasChangeHandler()) {
            ...
            if (m_filesysCtx.getChangeHandler().getGlobalNotifyMask() != 0) {
                ...
                if (deleteEvent.getFileType() == FileFolderServiceType.FILE)
                    m_filesysCtx
                            .getChangeHandler()
                            .notifyFileChanged(NotifyChange.ActionRemoved, relPath);
                else
                    m_filesysCtx
                            .getChangeHandler()
                ...
                if (logger.isDebugEnabled()) logger.debug("DeleteNode queued change notification");
            }
        }
    }
    ...
    private final void processMoveNode(MoveNodeEvent moveEvent) {
        ...
        if (m_filesysCtx.hasChangeHandler()) {
            ...
            if (m_filesysCtx.getChangeHandler().getGlobalNotifyMask() != 0) {
                ...
                if (logger.isDebugEnabled()) logger.debug("MoveNode queued change notification");
            }
        }
    }
    ...
    private final void processLockNode(LockNodeEvent lockEvent) {
        ...
        if (relPath.startsWith(m_rootPath)) {
            ...
            // Node has been locked or unlocked, send a change notification to indicate the file
            ...
            if (m_filesysCtx.hasChangeHandler()) {
                ...
                if (logger.isDebugEnabled()) logger.debug("LockNode queued change notification");
            }
        }
    }
}

...
public class QuartzJobSchedulerTest {
  ...
  public void testJobsScheduledWithDifferentCronExpressions() throws Exception {
    ...
    String testCron1 =
      (
        (CronTrigger) scheduler.getTrigger(
  ...
  private class SimpleHBDataCollector extends HBBaseDataCollector {
    ...
    public List<HBData> collectData() {
      ...
      result.add(
        new HBData("systemId2", this.getCollectorId(), "1", new Date())
      );
      return result;
    }
  }

  private boolean isJobScheduledForCollector(
    String collectorId,
    Scheduler scheduler
  ) throws Exception {
    ...
    return (
      scheduler.checkExists(new JobKey(jobName, Scheduler.DEFAULT_GROUP)) &&
      scheduler.checkExists(
        new TriggerKey(triggerName, Scheduler.DEFAULT_GROUP)
      )
    );
  }

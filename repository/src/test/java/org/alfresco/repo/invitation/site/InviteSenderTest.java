...
public class InviteSenderTest extends TestCase {
  ...
  private static final StoreRef testStore = new StoreRef(
  ...
  private static final Person whitespaceInvitee = new Person(
    "First Second\tthird\nFourth\r\nFifth"
  );
  private static final Person specialCharInvitee = new Person(
    "àâæçéèêëîïôœùûüÿñ"
  );
  ...
  protected void testSendMailWorkingPath(
    ...
    String subjectPropertyName = requireAcceptance
    ...
    verify(mailAction)
      .setParameterValue(eq(MailActionExecuter.PARAM_FROM), eq(inviter.email));
    verify(mailAction)
      .setParameterValue(eq(MailActionExecuter.PARAM_TO), eq(invitee.email));
    verify(mailAction)
      .setParameterValue(
        eq(MailActionExecuter.PARAM_SUBJECT),
        eq(subjectPropertyName)
      );
    verify(mailAction)
      .setParameterValue(
    ...
    verify(mailAction)
      .setParameterValue(
        eq(MailActionExecuter.PARAM_TEMPLATE),
        (Serializable) any()
      );
  ...
  public void testSendMailWithWhitespaceUserNameRequireAcceptance()
    throws Exception {
    testSendMailWithWhitespaceUserName(
      SendNominatedInviteDelegate.EMAIL_TEMPLATE_XPATH,
      SendNominatedInviteDelegate.EMAIL_SUBJECT_KEY,
  ...
  public void testSendMailWithSpecialCharUserNameRequireAcceptance()
    throws Exception {
    testSendMailWithSpecialCharUserName(
      SendNominatedInviteDelegate.EMAIL_TEMPLATE_XPATH,
      SendNominatedInviteDelegate.EMAIL_SUBJECT_KEY,
  ...
  private PersonService mockPersonService() {
    ...
    when(personService.getPerson(whitespaceInvitee.name))
      .thenReturn(whitespaceInvitee.node);
    when(personService.getPerson(specialCharInvitee.name))
      .thenReturn(specialCharInvitee.node);
    return personService;
  }
  ...
  private NodeService mockNodeService() {
    ...
    when(
      nodeService.getProperty(whitespaceInvitee.node, ContentModel.PROP_EMAIL)
    )
      .thenReturn(whitespaceInvitee.email);
    when(
      nodeService.getProperty(specialCharInvitee.node, ContentModel.PROP_EMAIL)
    )
  ...
  private ActionService mockActionService() {
    ...
    when(actionService.createAction(MailActionExecuter.NAME))
      .thenReturn(mailAction);
    return actionService;
  }

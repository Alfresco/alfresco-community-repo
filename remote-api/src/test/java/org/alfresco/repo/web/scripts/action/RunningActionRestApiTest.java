...
public class RunningActionRestApiTest extends BaseWebScriptTest {
    ...
    /**
     * When we're run from a suite, check the previous test's work has finished before we run. If
     * other things end up on the work queue with us, we'll get confused...
     */
    ...
    public void testRunningActionsGet() throws Exception {
        ...
        assertEquals(
                "/" + URL_RUNNING_ACTION + "replicationActionExecutor=" + id + "=" + instance,
                jsonRD.get("details"));
        ...
        assertEquals(false, jsonRD.getBoolean("cancelRequested"));
        assertEquals(
                "/" + URL_RUNNING_ACTION + "replicationActionExecutor=" + id2 + "=" + instance2,
                jsonRD.get("details"));
        ...
        assertEquals(false, jsonRD.getBoolean("cancelRequested"));
        assertEquals(
                "/" + URL_RUNNING_ACTION + "MadeUp2=54321=" + instanceAlt2, jsonRD.get("details"));
    }
    ...
    public void testRunningReplicationActionsGet() throws Exception {
        ...
        response =
                sendRequest(new GetRequest(URL_RUNNING_REPLICATION_ACTIONS + "?name=Test1"), 200);
        assertEquals(Status.STATUS_OK, response.getStatus());
    ...
    public void testRunningActionGet() throws Exception {
        ...
        response =
                sendRequest(
                        new GetRequest(URL_RUNNING_ACTION + "MadeUp"), Status.STATUS_UNAUTHORIZED);
        assertEquals(Status.STATUS_UNAUTHORIZED, response.getStatus());
        ...
        AuthenticationUtil.setFullyAuthenticatedUser(AuthenticationUtil.getAdminUserName());
        response =
                sendRequest(new GetRequest(URL_RUNNING_ACTION + "MadeUp"), Status.STATUS_NOT_FOUND);
    ...
    public void testRunningActionCancel() throws Exception {
        ...
        response =
                sendRequest(
                        new DeleteRequest(URL_RUNNING_ACTION + "MadeUp"), Status.STATUS_NOT_FOUND);
        assertEquals(Status.STATUS_NOT_FOUND, response.getStatus());
        ...
        response =
                sendRequest(new DeleteRequest(URL_RUNNING_ACTION + key), Status.STATUS_NO_CONTENT);
        ...
        response =
                sendRequest(new DeleteRequest(URL_RUNNING_ACTION + key2), Status.STATUS_NO_CONTENT);
        assertEquals(Status.STATUS_NO_CONTENT, response.getStatus());
    ...
    protected void setUp() throws Exception {
        ...
        executingActionsCache =
                (SimpleCache<String, ExecutionDetails>) appContext.getBean("executingActionsCache");

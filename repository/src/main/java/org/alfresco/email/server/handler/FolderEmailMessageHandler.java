...
public class FolderEmailMessageHandler extends AbstractEmailMessageHandler {
    ...
    /**
     * overwriteDuplicates. if true then overwrite an existing node with the same name. if false the
    ...
    public void addAlfrescoContent(NodeRef spaceNodeRef, EmailMessage message) throws IOException {
        ...
        contentNodeRef =
                addContentNode(getNodeService(), spaceNodeRef, messageSubject, overwriteDuplicates);
        // Add titled aspect
    ...
    /**
     * This method writes space as a content. We need this space because rules doesn't proceed
     * documents with empty content. We need rule processing for command email messages with empty
     * body.
     *
    ...
    private void addTitledAspect(NodeRef nodeRef, String title, String from) {
        ...
        titledProps.put(
                ContentModel.PROP_DESCRIPTION, I18NUtil.getMessage(MSG_RECEIVED_BY_SMTP, from));

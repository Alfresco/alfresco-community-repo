...
public class FixBpmPackagesPatch extends AbstractPatch {
  ...
  private static final String ERR_MSG_EMPTY_CONTAINER =
    "patch.fixBpmPackages.emptyContainer";

  private static final Log logger = LogFactory.getLog(
    FixBpmPackagesPatch.class
  );
  ...
  protected String applyInternal() throws Exception {
    ...
    try {
      ...
      if (refs == null || refs.size() == 0) throw new PatchException(
        ERR_MSG_EMPTY_CONTAINER,
        sysContainer
      );
      ...
      String sysWorkflowContainer = importerBootstrap
        .getConfiguration()
        .getProperty("system.workflow_container.childname");
      QName sysWorkflowQName = QName.createQName(
        sysWorkflowContainer,
        namespaceService
      );
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "Found " + refs.size() + " packages to convert"
      );
  ...
  private class FixBpmPackagesPatchHelper {
    ...
    public String fixBpmPackages(List<ChildAssociationRef> references)
      throws Exception {
      ...
      BatchProcessor.BatchProcessWorker<ChildAssociationRef> worker = new BatchProcessor.BatchProcessWorker<ChildAssociationRef>() {
        ...
        public void process(ChildAssociationRef assocRef) throws Throwable {
          ...
          if (logger.isDebugEnabled()) logger.debug(
            "Package " + name + " type " + typeQname
          );

          if (
            !nodeService.getType(packageRef).equals(WorkflowModel.TYPE_PACKAGE)
          ) {
            // New type of the package is bpm:package
            nodeService.setType(packageRef, WorkflowModel.TYPE_PACKAGE);
          }
          // Get all package items
          List<ChildAssociationRef> packageItemsAssocs = nodeService.getChildAssocs(
            packageRef,
            ContentModel.ASSOC_CONTAINS,
            RegexQNamePattern.MATCH_ALL
          );
          ...
          for (ChildAssociationRef itemAssoc : packageItemsAssocs) {
            ...
            String itemName = (String) nodeService.getProperty(
              childRef,
              ContentModel.PROP_NAME
            );
            ...
            if (itemAssoc.isPrimary()) {
              logger.error(
                ...
                " and item: " +
                itemName +
                " is primary association, so removing this assiciation will result in child node deletion"
              );
              continue;
            }
            ...
            boolean assocRemoved = nodeService.removeChildAssociation(
              itemAssoc
            );
            if (assocRemoved) {
              if (logger.isDebugEnabled()) logger.debug(

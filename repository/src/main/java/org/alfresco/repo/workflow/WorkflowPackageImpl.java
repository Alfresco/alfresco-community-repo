...
public class WorkflowPackageImpl
  implements WorkflowPackageComponent, Extensible {
   ...
   */
  @Extend(
    traitAPI = WorkflowPackageTrait.class,
    extensionAPI = WorkflowPackageExtension.class
  )
  ...
  private NodeRef makePackageContainer() {
    ...
    QName packageName = QName.createQName(
    ...
    try {
      ...
      ChildAssociationRef packageAssoc = nodeService.createNode(
        packages,
  ...
  private NodeRef findOrCreatePackagesFolder() {
    ...
    // an issue
    List<NodeRef> packageFolders = searchService.selectNodes(
      system,
    ...
    if (packageFolders.size() > 0) {
      ...
      QName packageFolderName = QName.createQName(
        NamespaceService.CONTENT_MODEL_1_0_URI,
        PACKAGE_FOLDER
      );
      ChildAssociationRef packageFolderAssoc = nodeService.createNode(
        system,
  ...
  public void deletePackage(NodeRef container) {
    if (
      ...
      Boolean isSystemPackage = (Boolean) nodeService.getProperty(
        container,
        WorkflowModel.PROP_IS_SYSTEM_PACKAGE
      );
    ...
    if (nodeService.exists(packageItem)) {
      ...
      for (ChildAssociationRef parentAssoc : parentAssocs) {
        ...
        if (
          ...
          String workflowInstance = (String) nodeService.getProperty(
            parentRef,
  ...
  private NodeRef getSystemWorkflowContainer() {
    if (tenantService.isEnabled()) {
      ...
      NodeRef tenantSystemWorkflowContainer = findSystemWorkflowContainer(
        systemContainer
      );
  ...
  private NodeRef findSystemContainer() {
    ...
    if (path == null) {
      throw new WorkflowException(
        "Unable to locate system container - path not specified"
      );
    }
  ...
  public boolean setWorkflowForPackage(WorkflowInstance instance) {
    ...
    Serializable pckgInstanceId = nodeService.getProperty(
      packageNode,
      WorkflowModel.PROP_WORKFLOW_INSTANCE_ID
    );

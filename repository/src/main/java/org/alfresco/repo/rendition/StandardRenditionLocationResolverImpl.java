...
public class StandardRenditionLocationResolverImpl
  ...
  private static final Log log = LogFactory.getLog(
    StandardRenditionLocationResolverImpl.class
  );
  ...
  public RenditionLocation getRenditionLocation(
    ...
    NodeRef destination = AbstractRenderingEngine.getCheckedParam(
  ...
  private RenditionLocationImpl createNodeLocation(NodeRef destination) {
    ...
    if (
      nodeService.exists(destination) == false
    ...
    NodeRef parentRef = nodeService
      .getPrimaryParent(destination)
      .getParentRef();
    String destinationCmName = (String) nodeService.getProperty(
      destination,
      ContentModel.PROP_NAME
    );
    RenditionLocationImpl location = new RenditionLocationImpl(
      parentRef,
      destination,
      destinationCmName
    );
  ...
  private RenditionLocationImpl findOrCreateTemplatedPath(
    ...
    if (log.isDebugEnabled()) {
      ...
      msg
        .append("FindOrCreateTemplatedPath for ")
        .append(sourceNode)
        .append(", ")
        .append(path);
    ...
    Serializable companyHomeName = nodeService.getProperty(
      companyHome,
      ContentModel.PROP_NAME
    );
    ...
    if (!folderElements.isEmpty()) {
      FileInfo parentInfo = FileFolderUtil.makeFolders(
        fileFolderService,
        companyHome,
        folderElements,
    ...
    if (log.isDebugEnabled()) {
      ...
      log.debug(
        "   " + nodeService.getType(parent) + " " + nodeService.getPath(parent)
      );
    ...
    if (log.isDebugEnabled()) {
      StringBuilder msg = new StringBuilder();
      msg
        .append("RenditionLocation parent=")
        .append(parent)
        .append(", child=")
        .append(child)
        .append(", fileName=")
  ...
  private String renderPathTemplate(
    ...
    try {
      ...
      FileInfo sourceFileInfo = sourcePathInfo.remove(
        sourcePathInfo.size() - 1
      );
      ...
      fullSourceName =
        nodeService.getPrimaryParent(sourceNode).getQName().getLocalName();
      cwd = "/";
    }
    ...
    if (extensionIndex != -1) {
      trimmedSourceName =
        (extensionIndex == 0)
          ? ""
          : fullSourceName.substring(0, extensionIndex);
      sourceExtension =
    ...
    root.put("cwd", cwd);
    TemplateNode companyHomeNode = new TemplateNode(
      companyHome,
      serviceRegistry,
      null
    );
    ...
    root.put(
      "sourceContentType",
      nodeService.getType(sourceNode).getLocalName()
    );
    root.put(
      "renditionContentType",
      nodeService.getType(tempRenditionLocation).getLocalName()
    );
    ...
    try {
      ...
      log.error(
        "Error while trying to process rendition path template: " + pathTemplate
      );
    ...
    for (int i = 0; i < attributes.getLength(); i++) {
      ...
      if (a.getNodeName().startsWith(XMLConstants.XMLNS_ATTRIBUTE)) {
        final String prefix = a
          .getNodeName()
          .substring((XMLConstants.XMLNS_ATTRIBUTE + ":").length());

...
public class DBQueryEngine implements QueryEngine {
  ...
  public QueryEngineResults executeQuery(
    ...
    if (query.getSource() != null) {
      List<Set<String>> selectorGroups = query
        .getSource()
        .getSelectorGroups(functionContext);
    ...
    Pair<Long, QName> sysDeletedType = qnameDAO.getQName(
  ...
  private ResultSet selectNodesWithPermissions(
    ...
    Authority authority = aclCrudDAO.getAuthority(
      AuthenticationUtil.getRunAsUser()
    );

    NodePermissionAssessor permissionAssessor = createAssessor(authority);
    int maxPermsChecks = options.getMaxPermissionChecks() < 0
      ? maxPermissionChecks
    ...
    FilteringResultSet resultSet = acceleratedNodeSelection(
      options,
      dbQuery,
      permissionAssessor
    );
  ...
  protected NodePermissionAssessor createAssessor(Authority authority) {
    return new NodePermissionAssessor(
      nodeService,
      permissionService,
      authority,
      nodesCache
  ...
  FilteringResultSet acceleratedNodeSelection(
    QueryOptions options,
    DBQuery dbQuery,
    NodePermissionAssessor permissionAssessor
  ) {
    ...
    FilteringResultSet frs = new FilteringResultSet(
      rs,
      formInclusionMask(nodes)
    );
  ...
  private void performTmdqSelectPaging(
    ...
    batchSize =
      Math.min(Math.max(batchSize, minPagingBatchSize), maxPagingBatchSize);
  ...
  private DBResultSet createResultSet(
    QueryOptions options,
    List<Node> nodes,
    int numberFound
  ) {

...
public class ImporterComponent implements ImporterService {
  ...
  private String bindPlaceHolder(String value, ImporterBinding binding) {
    if (binding != null) {
      ...
      while (iStartBinding != -1) {
        int iEndBinding = value.indexOf(
          END_BINDING_MARKER,
          iStartBinding + START_BINDING_MARKER.length()
        );
        if (iEndBinding == -1) {
          throw new ImporterException(
    ...
    for (int i = 0; i < segments.length; i++) {
      if (segments[i] != null && segments[i].length() > 0) {
        ...
        if (colonIndex == -1) {
          ...
          QName segmentQName = QName.createQName(
            qnameComponents[0],
            localName,
            namespaceService
          );
  ...
  public void parserImport(
    ...
    Importer nodeImporter = new NodeImporter(
      nodeRef,
      location,
      binding,
      streamHandler,
  ...
  public ContentHandler handlerImport(
    ...
    ImportPackageHandler streamHandler = new ContentHandlerStreamHandler(
      defaultHandler
    );
    Importer nodeImporter = new NodeImporter(
      nodeRef,
  ...
  private class NodeImporter implements Importer {
    ...
    private NodeImporter(
      ...
      if (binding == null || binding.getExcludedClasses() == null) {
        this.excludedClasses =
          new QName[] { ContentModel.ASPECT_REFERENCEABLE };
      } else {
    ...
    public void importMetaData(Map<QName, String> properties) {
      ...
      String complexPath = properties.get(
        QName.createQName(NamespaceService.REPOSITORY_VIEW_1_0_URI, "exportOf")
      );
    ...
    public NodeRef importNode(ImportNode context) {
      ...
      for (Map.Entry<QName, Serializable> property : context
        .getProperties()
        .entrySet()) {
        // filter out content properties (they're imported later)
        DataTypeDefinition valueDataType = context.getPropertyDataType(
          property.getKey()
        );
        if (
          valueDataType != null &&
    ...
    private void generateVersioningForVersionableNode(final NodeRef nodeRef) {
      ...
      final String label = (String) nodeService.getProperty(
        nodeRef,
      ...
      final NodeRef versionNodeRef = VersionUtil.convertNodeRef(
      ...
      if (!parentRef.equals(getRootRef())) {
        ...
        if (assocDef.isChild()) {
          ...
          if (childQName == null) {
            ...
            childQName =
              QName.createQName(assocType.getNamespaceURI(), localName);
          }
    ...
    private void importContent(
      NodeRef nodeRef,
      QName propertyName,
      String importContentData
    ) {
      ImporterContentCache contentCache = (binding == null)
        ? null
        : binding.getImportConentCache();
      ...
      if (importContentData != null && importContentData.length() > 0) {
        DataTypeDefinition dataTypeDef = dictionaryService.getDataType(
          DataTypeDefinition.CONTENT
        );
        ContentData contentData = (ContentData) DefaultTypeConverter.INSTANCE.convert(
          dataTypeDef,
          importContentData
        );
        ...
        if (contentUrl != null && contentUrl.length() > 0) {
          ...
          if (contentCache != null) {
            ...
            ContentData cachedContentData = contentCache.getContent(
              streamHandler,
              contentData
            );
          ...
          if (contentUsageImpl != null && contentUsageImpl.getEnabled()) {
            ...
            Map<QName, Serializable> propsAfter = nodeService.getProperties(
              nodeRef
            );
            contentUsageImpl.onUpdateProperties(
              nodeRef,
              propsBefore,
              propsAfter
            );
      ...
      for (ImportedNodeRef importedRef : nodeRefs) {
        ...
        if (importedRef.value != null) {
          if (importedRef.value instanceof Collection) {
            ...
            List<NodeRef> resolvedRefs = new ArrayList<NodeRef>(
              unresolvedRefs.size()
            );
            for (String unresolvedRef : unresolvedRefs) {
              if (unresolvedRef != null) {
                NodeRef nodeRef = resolveImportedNodeRef(
        ...
        Set<QName> nodeTypeAndAspects = getNodeTypeAndAspects(
          importedRef.context
        );
        try {
      ...
      if (childName != null) {
        ...
        if (
          ContentModel.TYPE_PERSON.equals(
            context.getTypeDefinition().getName()
          ) &&
        ...
        if (
          nameValue != null &&
          !String.class.isAssignableFrom(nameValue.getClass())
        ) {
          throw new ImporterException(
    ...
    private QName getAssocType(ImportNode context) {
      ...
      QName parentType = nodeService.getType(
      ...
      for (ChildAssociationDefinition childAssocDef : childAssocDefs.values()) {
        targetTypes.put(
          childAssocDef.getTargetClass().getName(),
          childAssocDef.getName()
        );
      }
      Set<QName> parentAspects = nodeService.getAspects(
        context.getParentContext().getParentRef()
      );
    ...
    private Map<QName, Serializable> bindProperties(ImportNode context) {
      ...
      Map<QName, Serializable> boundProperties = new HashMap<QName, Serializable>(
        properties.size()
      );
      for (QName property : properties.keySet()) {
        ...
        if (value instanceof Collection) {
          ...
          for (Serializable collectionValue : (Collection<Serializable>) value) {
            Serializable objValue = bindValue(
              context,
              property,
              valueDataType,
              collectionValue
        ...
        if (
          ...
          ImportedNodeRef importedRef = new ImportedNodeRef(
            context,
            property,
            value
          );
     ...
     */
    private List<AccessPermission> bindPermissions(
      List<AccessPermission> permissions
    ) {
      List<AccessPermission> boundPermissions = new ArrayList<AccessPermission>(
        permissions.size()
      );

      for (AccessPermission permission : permissions) {
        AccessPermission ace = new NodeContext.ACE(
    ...
    private Serializable bindValue(
      ...
      if (value != null && valueType != null) {
        ...
        if (
          (
            valueType.getName().equals(DataTypeDefinition.NODE_REF) ||
     ...
     * @return NodeRef
     */
    private NodeRef resolveImportedNodeRef(
     ...
     */
    private void reportPropertySet(
      NodeRef nodeRef,
     ...
     * @param permissions List<AccessPermission>
     */
    private void reportPermissionSet(
      NodeRef nodeRef,
      List<AccessPermission> permissions
    ) {
    ...
    private class CreateNewNodeImporterStrategy
      ...
      public NodeRef importNode(ImportNode node) {
        ...
        AccessStatus writePermission = permissionService.hasPermission(
          nodeRef,
    ...
    private class RemoveExistingNodeImporterStrategy
      ...
      private NodeImporterStrategy createNewStrategy = new CreateNewNodeImporterStrategy(
        false
      );
        ...
        if (uuid != null && uuid.length() > 0) {
          ...
          if (nodeService.exists(existingNodeRef)) {
            throw new InvalidNodeRefException(
              "Node " + existingNodeRef + " already exists",
              existingNodeRef
            );
          }
        }
      ...
      public NodeRef importNode(ImportNode node) {
        ...
        if (uuid == null && location.getPath() != null) { //need a valid location where to search
          ...
          String path =
            ...
            QName
              .createQName(
                node.getTypeDefinition().getName().getNamespaceURI(),
                node.getChildName()
              )
              .toPrefixString();
        ...
        if (uuid != null && uuid.length() > 0 || existingNodeRef != null) {
          ...
          if (nodeService.exists(existingNodeRef)) {
            // do the update
            Map<QName, Serializable> existingProperties = nodeService.getProperties(
              existingNodeRef
            );
  ...
  private static class DefaultStreamHandler implements ImportPackageHandler {
    ...
    public InputStream importStream(String content) {
      ...
      if (resource.exists() == false) {
        throw new ImporterException(
          "Content URL " + content + " does not exist."
        );
      }

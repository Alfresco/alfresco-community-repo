...
public class ResetPasswordServiceImpl implements ResetPasswordService {
  ...
  private static final Log LOGGER = LogFactory.getLog(
    ResetPasswordServiceImpl.class
  );

  private static final String TIMER_END = "PT1H";
  private static final String WORKFLOW_DESCRIPTION_KEY =
    "resetpasswordwf_resetpassword.resetpassword.workflow.description";
  ...
  public void requestReset(String userId, String clientName) {
    ...
    WorkflowDefinition wfDefinition = workflowService.getDefinitionByName(
      WorkflowModelResetPassword.WORKFLOW_DEFINITION_NAME
    );
    ...
    props.put(
      ...
      I18NUtil.getMessage(WORKFLOW_DESCRIPTION_KEY)
    );
  ...
  protected String validateUserAndGetEmail(String userId) {
    if (!personService.personExists(userId)) {
      throw new ResetPasswordWorkflowInvalidUserException(
  ...
  public void initiateResetPassword(ResetPasswordDetails resetDetails) {
    ...
    validateIdAndKey(
      resetDetails.getWorkflowId(),
      resetDetails.getWorkflowKey(),
      resetDetails.getUserId()
    );
    if (StringUtils.isBlank(resetDetails.getPassword())) {
      throw new IllegalArgumentException(
        "Invalid password value [" + resetDetails.getPassword() + ']'
      );
    }
    ...
    List<WorkflowTask> tasks = workflowService.queryTasks(
      processTaskQuery,
      false
    );
    ...
    Map<QName, Serializable> props = Collections.singletonMap(
      WorkflowModelResetPassword.WF_PROP_PASSWORD,
      resetDetails.getPassword()
    );
    ...
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug(
        "Deleting historical task for security reasons " + activitiTaskId
      );
    }
  }
  ...
  private void validateIdAndKey(String id, String key, String userId) {
    ...
    if (workflowInstance == null) {
      throw new ResetPasswordWorkflowNotFoundException(
        "The reset password workflow instance with the id [" +
        id +
        "] is not found."
      );
    }
    ...
    if (workflowInstance.isActive()) {
      ...
      Map<QName, Serializable> pathProps = workflowService.getPathProperties(
        id
      );

      username =
        (String) pathProps.get(WorkflowModelResetPassword.WF_PROP_USERNAME);
      recoveredKey =
        (String) pathProps.get(WorkflowModelResetPassword.WF_PROP_KEY);
    } else {
      throw new InvalidResetPasswordWorkflowException(
        "The reset password workflow instance with the id [" +
        id +
        "] is not active (it might be expired or has already been used)."
      );
    }
    if (username == null || recoveredKey == null || !recoveredKey.equals(key)) {
      ...
      if (username == null) {
        msg =
          "The recovered user name is null for the reset password workflow instance with the id [" +
          id +
          "]";
      } else if (recoveredKey == null) {
        msg =
          "The recovered key is null for the reset password workflow instance with the id [" +
          id +
          "]";
      } else {
        msg =
          "The recovered key [" +
          recoveredKey +
          "] does not match the given workflow key [" +
          key +
          "] for the reset password workflow instance with the id [" +
          id +
          "]";
      }
      ...
      throw new InvalidResetPasswordWorkflowException(
        "The given user id [" +
        userId +
        "] does not match the person's user id [" +
        username +
  ...
  public ClientApp getClientAppConfig(String clientName) {
    ...
    if (clientApp == null) {
      throw new ClientAppNotFoundException(
        "Client was not found [" + clientName + "]"
      );
    }
    return clientApp;
  }
  ...
  public void sendResetPasswordEmail(
    ...
    final String userName = (String) variables.get(
      WorkflowModelResetPassword.WF_PROP_USERNAME_ACTIVITI
    );
    final String toEmail = (String) variables.get(
      WorkflowModelResetPassword.WF_PROP_USER_EMAIL_ACTIVITI
    );
    final String clientName = (String) variables.get(
      WorkflowModelResetPassword.WF_PROP_CLIENT_NAME_ACTIVITI
    );
    final String key = (String) variables.get(
      WorkflowModelResetPassword.WF_PROP_KEY_ACTIVITI
    );
    final String id = execution.getProcessInstanceId();

    final ClientApp clientApp = getClientAppConfig(clientName);
    Map<String, Serializable> emailTemplateModel = Collections.singletonMap(
      FTL_RESET_PASSWORD_URL,
      createResetPasswordUrl(clientApp, id, key)
    );

    final String templatePath = emailHelper.getEmailTemplate(
      clientName,
  ...
  public void performResetPassword(DelegateExecution execution) {
    ...
    final String userName = (String) execution.getVariable(
      WorkflowModelResetPassword.WF_PROP_USERNAME_ACTIVITI
    );
    ...
    List<Task> activitiTasks = activitiTaskService
      .createTaskQuery()
      .taskDefinitionKey(WorkflowModelResetPassword.TASK_RESET_PASSWORD)
      .processInstanceId(execution.getProcessInstanceId())
      .list();
    if (activitiTasks.size() != 1) {
      throw new ResetPasswordWorkflowException(
        "Unexpected count of task instances: " + activitiTasks.size()
      );
    }
    Task activitiTask = activitiTasks.get(0);
    String activitiTaskId = activitiTask.getId();
    final String password = (String) activitiTaskService.getVariable(
      activitiTaskId,
  ...
  public void sendResetPasswordConfirmationEmail(
    ...
    final String userEmail = (String) variables.get(
      WorkflowModelResetPassword.WF_PROP_USER_EMAIL_ACTIVITI
    );
    final String clientName = (String) variables.get(
      WorkflowModelResetPassword.WF_PROP_CLIENT_NAME_ACTIVITI
  ...
  protected void sendEmail(ResetPasswordEmailDetails emailRequest) {
    ...
    final String templateAssetsUrl = getUrl(
      emailRequest.getTemplateAssetsUrl(),
      ClientAppConfig.PROP_TEMPLATE_ASSETS_URL
    );
    ...
    // Pick the template
    actionParams.put(
      MailActionExecuter.PARAM_TEMPLATE,
    ...
    final Locale locale = emailHelper.getUserLocaleOrDefault(
      emailRequest.getUserName()
    );
    actionParams.put(MailActionExecuter.PARAM_LOCALE, locale);

    actionParams.put(
      MailActionExecuter.PARAM_IGNORE_SEND_FAILURE,
      emailRequest.ignoreSendFailure
    );
    // Now send the email
    Action mailAction = actionService.createAction(
  ...
  private String getUrl(String url, String propName) {
    if (url == null) {
      LOGGER.warn(
        "The url for the property [" + propName + "] is not configured."
      );
      return "";
    }
   ...
   */
  protected String createResetPasswordUrl(
    ClientApp clientApp,
    final String id,
    final String key
  ) {
    ...
    if (StringUtils.isEmpty(pageUrl)) {
      ...
      LOGGER.warn(
        "'resetPasswordPageUrl' property is not set for the client [" +
        clientApp.getName() +
        "]. The default base url of Share will be used [" +
        sb.toString() +
        "]"
      );
    } else {
    ...
    sb
      ...
      .append("&id=")
      .append(
        BPMEngineRegistry.createGlobalId(ActivitiConstants.ENGINE_ID, id)
      );
  ...
  public static class ResetPasswordDetails {
    ...
    public String toString() {
      ...
      sb
        .append("ResetPasswordDetails [userId=")
        .append(userId)
        .append(", workflowId=")
        .append(workflowId)
        .append(", workflowKey=")
  ...
  public static class ResetPasswordEmailDetails {
    ...
    public ResetPasswordEmailDetails setTemplateModel(
    ...
    public ResetPasswordEmailDetails setIgnoreSendFailure(
      boolean ignoreSendFailure
    ) {
    ...
    public String toString() {
      ...
      sb
        .append("ResetPasswordEmailDetails [userName=")
        .append(userName)
        .append(", userEmail=")
        .append(userEmail)
        .append(", fromEmail=")

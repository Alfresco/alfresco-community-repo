...
public class ExecParameterTokenizer {
  ...
  public List<String> getAllTokens() {
    if (this.str == null) {
      throw new NullPointerException(
        "Illegal null string cannot be tokenized."
      );
    }
    ...
    if (tokens == null) {
      ...
      if (str.indexOf('\'') == -1 && str.indexOf('"') == -1) {
        // Contains no quotes.
        for (
          StringTokenizer standardTokenizer = new StringTokenizer(str);
        ...
        for (
          Pair<Integer, Integer> next = identifyNextQuotedRegion(str, 0);
          next != null;
        ) {
  ...
  private List<Substring> getSubstrings(
    ...
    for (Pair<Integer, Integer> nextQuotedRegionIndices : quotedRegionIndices) {
      if (cursorPosition < nextQuotedRegionIndices.getFirst()) {
        int startIndexOfNextQuotedRegion =
          nextQuotedRegionIndices.getFirst() - 1;
        result.add(
    ...
    if (cursorPosition < str.length() - 1) {
      result.add(
        new UnquotedSubstring(str.substring(cursorPosition, str.length() - 1))
      );
    }
  ...
  private Pair<Integer, Integer> identifyNextQuotedRegion(
    String str,
    int startingIndex
  ) {
    ...
    if (indexOfNextSingleQuote == -1 && indexOfNextDoubleQuote == -1) {
      ...
      int indexOfNextQuote = Math.min(
        indexOfNextSingleQuote,
        indexOfNextDoubleQuote
      );
  ...
  private Pair<Integer, Integer> findIndexOfClosingQuote(
    ...
    int indexAfterClosingQuote =
      str.indexOf(quoteChar, indexOfStartingQuote + 1) + 1; // + 1 to search after opening quote. + 1 to give result including closing quote.

    if (indexAfterClosingQuote == 0) { // -1 + 1
      // If no closing quote.
      throw new IllegalArgumentException(
        "No closing " + quoteChar + "quote in" + str
      );
    }

    return new Pair<Integer, Integer>(
      indexOfStartingQuote,
  ...
  public class UnquotedSubstring implements Substring {
    ...
    public String toString() {
      return (
        UnquotedSubstring.class.getSimpleName() + ": '" + regionString + '\''
      );
    }
  }
  ...
  public class QuotedSubstring implements Substring {
    ...
    public List<String> getTokens() {
      String stringWithoutQuotes = regionString.substring(
        1,
        regionString.length() - 1
      );
    ...
    public String toString() {
      return (
        QuotedSubstring.class.getSimpleName() + ": '" + regionString + '\''
      );
    }
  }
}

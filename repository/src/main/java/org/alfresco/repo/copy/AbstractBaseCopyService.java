...
/**
 * This is a base class for all the copy services. Introduces general methods for collecting all the
 * properties, which are required to perform a copying
 *
...
public class AbstractBaseCopyService {
     ...
     *
     * <p>When files or folders are renamed, the association path (QName) is normally modified to
     * follow the name of the node. If, however, the namespace of the patch QName is in this list,
     * the association path is left alone. This allows parts of the application to use well-known
     * paths even if the end-user is able to modify the objects <b>cm:name</b> value.
     *
     ...
     * @return Returns the path part for a new association and the effective primary parent
     *     association that was used
     */
    protected AssociationCopyInfo getAssociationCopyInfo(
        ...
        if (sourceParentRef != null) {
            ...
            boolean copyingFromPrimaryParent =
                    sourceParentRef.equals(primaryAssocRef.getParentRef());
        ...
        if (nameChanged && !systemNamespaces.contains(existingQName.getNamespaceURI())) {
            ...
            targetAssocQName =
                    QName.createQName(
                            sourceParentAssocRef.getQName().getNamespaceURI(),
    ...
    /**
     * Simple container for storing data required to copy a node, including the parent association
     * that will be copied along with the new path part of the association that will be created by
     * the copy.
     *
     * <p>This container is immutable.
     *
     * @author Dmitry Velichkevich
     * @since 4.1.5
     * @see AbstractBaseCopyService#getAssociationCopyInfo(NodeService, NodeRef, NodeRef, String,
     *     boolean)
     */

...
public abstract class CopyMoveLinkFileToBaseAction extends RMActionExecuterAbstractBase {
    ...
    protected void addParameterDefinitions(List<ParameterDefinition> paramList) {
        ...
        paramList.add(
                new ParameterDefinitionImpl(
                        ...
                        false,
                        getParamDisplayLabel(PARAM_CREATE_RECORD_PATH)));
    }
     ...
     */
    @Override
    protected synchronized void executeImpl(
            final Action action, final NodeRef actionedUponNodeRef) {
        ...
        if (isOkToProceedWithAction(actionedUponNodeRef, actionName)) {
            ...
            if (ACTION_FILETO.equals(action.getActionDefinitionName())) {
                ...
                targetIsUnfiledRecords =
                        (getDictionaryService()
                                                .isSubClass(
                                                        actionedUponType, ContentModel.TYPE_CONTENT)
            ...
            NodeRef recordFolder =
                    (NodeRef) action.getParameterValue(PARAM_DESTINATION_RECORD_FOLDER);
            if (recordFolder == null) {
                recordFolder =
                        createOrResolvePath(action, actionedUponNodeRef, targetIsUnfiledRecords);
            }

            // now we have the reference to the target folder we can do some final checks to see if
            // the action is valid
            validateActionPostPathResolution(
                    actionedUponNodeRef, recordFolder, actionName, targetIsUnfiledRecords);
                            ...
                            try {
                                if (getMode() == CopyMoveLinkFileToActionMode.MOVE) {
                                    fileFolderService.move(
                                            actionedUponNodeRef, finalRecordFolder, null);
                                ...
                                throw new AlfrescoRuntimeException(
                                        "Unable to execute file to action, because the "
                                                + (mode == CopyMoveLinkFileToActionMode.MOVE
                                                        ? "move"
                                                        : "copy")
                                                + " operation failed.",
                                        fileNotFound);
        ...
        if (getNodeService().exists(actionedUponNodeRef)
            ...
            if (ACTION_FILETO.equals(actionName)) {
                ...
                okToProceed =
                        !getRecordService().isFiled(actionedUponNodeRef)
                                && getDictionaryService()
                                        .isSubClass(actionedUponType, ContentModel.TYPE_CONTENT);
                if (!okToProceed && logger.isDebugEnabled()) {
                    logger.debug(
                            "Unable to run "
                                    + actionName
                                    + " action on a node that isn't unfiled and a sub-class of"
                                    + " content type");
                }
                ...
                if (!okToProceed && logger.isDebugEnabled()) {
                    logger.debug(
                            "Unable to run "
                                    + actionName
                                    + " action on a node that isn't filed and a sub-class of"
                                    + " content type");
                }
            } else {
                okToProceed = true;
     ...
     */
    private void validateActionPostPathResolution(
            NodeRef actionedUponNodeRef,
            NodeRef target,
            String actionName,
            boolean targetIsUnfiledRecords) {
        QName actionedUponType = getNodeService().getType(actionedUponNodeRef);
        // now we have the reference to the target folder we can do some final checks to see if the
        // action is valid
        if (target == null) {
            throw new AlfrescoRuntimeException(
                    "Unable to run "
                            + actionName
                            + " action, because the destination record folder could not be"
                            + " determined.");
        }
        if (targetIsUnfiledRecords) {
            QName targetFolderType = getNodeService().getType(target);
            if (!TYPE_UNFILED_RECORD_CONTAINER.equals(targetFolderType)
                    && !TYPE_UNFILED_RECORD_FOLDER.equals(targetFolderType)) {
                throw new AlfrescoRuntimeException(
                        "Unable to run "
                                + actionName
                                + " action, because the destination record folder is an"
                                + " inappropriate type.");
            }
        } else {
            if (getRecordFolderService().isRecordFolder(target)
                    && !getDictionaryService()
                            .isSubClass(actionedUponType, ContentModel.TYPE_CONTENT)
                ...
                throw new AlfrescoRuntimeException(
                        "Unable to run "
                                + actionName
                                + " action, because the destination record folder is an"
                                + " inappropriate type. A record folder cannot contain another"
                                + " folder or a category");
            } else if (filePlanService.isRecordCategory(target)
                    && getDictionaryService()
                            .isSubClass(actionedUponType, ContentModel.TYPE_CONTENT)) {
                throw new AlfrescoRuntimeException(
                        "Unable to run "
                                + actionName
                                + " action, because the destination record folder is an"
                                + " inappropriate type. A record category cannot contain a record");
            }
        }
    }
     ...
     * @return
     */
    private NodeRef createOrResolvePath(
            final Action action,
            final NodeRef actionedUponNodeRef,
            final boolean targetisUnfiledRecords) {
        ...
        final String[] pathElementsArray =
                StringUtils.tokenizeToStringArray(pathParameter, "/", false, true);
     ...
     * @param creating true if we have already created the parent and therefore can skip the check
     *     to see if the next path element already exists
     * @return
     */
    private NodeRef createOrResolvePath(
            Action action,
        ...
        if (nodeRef == null) {
            if (create) {
                ...
                nodeRef =
                        createChild(
                                action, parent, childName, targetisUnfiledRecords, lastAsFolder);
            } else {
                throw new AlfrescoRuntimeException(
                        "Unable to execute "
                                + action.getActionDefinitionName()
                                + " action, because the destination path could not be determined.");
            }
        } else {
            ...
            if (nodeType.equals(RecordsManagementModel.TYPE_HOLD_CONTAINER)
                ...
                throw new AlfrescoRuntimeException(
                        "Unable to execute "
                                + action.getActionDefinitionName()
                                + " action, because the destination path in invalid.");
            }
        }
     ...
     * @param targetisUnfiledRecords true if the child is being created in the unfiled directory
     *     (determines type as unfiled container child)
     * @param lastAsFolder true if this is the last element of the pathe being created and it should
     *     be created as a folder. ignored if targetIsUnfiledRecords is true
     * @return
     */
    private NodeRef createChild(
            final Action action,
            final NodeRef parent,
            final String childName,
            final boolean targetisUnfiledRecords,
            final boolean lastAsFolder) {
        ...
        if (child == null) {
            if (targetisUnfiledRecords) {
                ...
                if (RecordsManagementModel.TYPE_RECORD_FOLDER.equals(
                        getNodeService().getType(parent))) {
                    throw new AlfrescoRuntimeException(
                            "Unable to execute "
                                    + action.getActionDefinitionName()
                                    + " action, because the destination path has a record category"
                                    + " within a record folder.");
                }
    ...
    /**
     * Return the context. This will be the unfiled records container of the context if
     * targetisUnfiledRecords is true
     *
     * @param action
     * @param actionedUponNodeRef
     * @param targetisUnfiledRecords
     * @return
     */
    private NodeRef getContext(
            Action action, NodeRef actionedUponNodeRef, boolean targetisUnfiledRecords) {
        ...
        if ((context == null) || (!getNodeService().exists(context))) {
            throw new AlfrescoRuntimeException(
                    "Unable to execute "
                            + action.getActionDefinitionName()
                            + " action, because the path resolution context could not be"
                            + " determined.");
        }
        return context;
    }

...
public class EncryptionTests extends TestCase {
    ...
    private static QName NODE_TYPE =
            QName.createQName("http://www.alfresco.org/test/reencryption_test/1.0", "base");
    private static QName PROP =
            QName.createQName("http://www.alfresco.org/test/reencryption_test/1.0", "prop1");
    ...
    public void setUp() throws Exception {
        ...
        // reencrypt in one txn (since we don't commit the model, the qnames won't be available
        // across transactions)
        ...
        StoreRef storeRef =
                nodeService.createStore(
                        StoreRef.PROTOCOL_WORKSPACE, "ReEncryptor_" + System.currentTimeMillis());
    ...
    protected KeyProvider getKeyProvider(KeyStoreParameters keyStoreParameters) {
        KeyProvider backupKeyProvider =
                new KeystoreKeyProvider(keyStoreParameters, keyResourceLoader);
    ...
    protected void testChangeKeysImpl(boolean cacheCiphers) throws Throwable {
        // on a single thread
        // create an encryptor, encrypt a string, change encryptor keys, decrypt -> should result in
        // Invalid Key

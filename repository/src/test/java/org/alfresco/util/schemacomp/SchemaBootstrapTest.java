...
public class SchemaBootstrapTest {
  ...
  private static final String BOOTSTRAP_TEST_CONTEXT =
    "classpath*:alfresco/dbscripts/bootstrap-context-test.xml";
  private static final String MAIN_SCHEMA_REFERENCE_FILE =
    "classpath:alfresco/dbscripts/create/org.alfresco.repo.domain.dialect.PostgreSQLDialect/Schema-Reference-ALF.xml";
  private static final String TEST_SCHEMA_REFERENCE_FILE =
    "classpath:alfresco/dbscripts/create/org.alfresco.repo.domain.dialect.PostgreSQLDialect/Test-Schema-Reference-ALF.xml";
  ...
  private static ApplicationContextInit APP_CONTEXT_INIT = ApplicationContextInit.createStandardContextWithOverrides(
  ...
  public static void beforeClass() throws Exception {
    ResourcePatternResolver rpr = new PathMatchingResourcePatternResolver(
      SchemaBootstrapTest.class.getClassLoader()
    );
    Document schemaRefXML = loadXML(
      rpr.getResource(MAIN_SCHEMA_REFERENCE_FILE)
    );
  ...
  public void setUp() throws Exception {
    schemaBootstrap =
      (SchemaBootstrap) APP_CONTEXT_INIT
        .getApplicationContext()
        .getBean("schemaBootstrap");
    schemaBootstrap.setSchemaReferenceUrls(TEST_SCHEMA_REFERENCE_URLS);
    optionalPatch =
      (SchemaUpgradeScriptPatch) APP_CONTEXT_INIT
        .getApplicationContext()
        .getBean("patchDbVOAddIndexTest");
  }
  ...
  public void shouldSchemaValidationReportProblemsCausedByUnappliedOptionalPatch() {
    ...
    assertEquals(
      "Expected 1 problem; but these problems were found instead: \n" +
      problems +
      "\n",
      1,
      numProblems
    ...
    assertTrue(
      "Missing optional patch-specific problems report: \n" + problems,
      problems.contains(
        "The following problems will be resolved once the long running patch " +
        optionalPatch.getId() +
        " has been run"
      )
    );
  ...
  private static void updateTestSchemaReferenceFile(
    Resource testSchemaRef,
    Document newDoc
  ) throws Exception {
  ...
  private static Node getIndexesNode(Document document)
    throws XPathExpressionException {
    XPathFactory xPathfactory = XPathFactory.newInstance();
    XPath xpath = xPathfactory.newXPath();
    XPathExpression expr = xpath.compile(
      "/schema/objects/table[@name='alf_node']/indexes"
    );

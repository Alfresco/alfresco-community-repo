...
public class LinksServiceImplTest {
    ...
    private static final ApplicationContext testContext =
    ...
    @Mock private static TaggingService TAGGING_SERVICE;

    private static final String TEST_USER =
            LinksServiceImplTest.class.getSimpleName() + "_testuser";
    ...
    public void createUpdateDeleteEntry() throws Exception {
        ...
        assertEquals(
                "Title", PUBLIC_NODE_SERVICE.getProperty(link.getNodeRef(), LinksModel.PROP_TITLE));
        assertEquals(
                "Description",
                PUBLIC_NODE_SERVICE.getProperty(link.getNodeRef(), LinksModel.PROP_DESCRIPTION));
        assertEquals(
                "http://www.alfresco.com/",
                PUBLIC_NODE_SERVICE.getProperty(link.getNodeRef(), LinksModel.PROP_URL));
        assertEquals(
                "http://www.alfresco.com/",
                CONTENT_SERVICE
                        .getReader(link.getNodeRef(), ContentModel.PROP_CONTENT)
                        .getContentString());
        assertEquals(
                false,
                PUBLIC_NODE_SERVICE.hasAspect(link.getNodeRef(), LinksModel.ASPECT_INTERNAL_LINK));
        ...
        assertEquals(
                "New Title",
                PUBLIC_NODE_SERVICE.getProperty(link.getNodeRef(), LinksModel.PROP_TITLE));
        assertEquals(
                "Description",
                PUBLIC_NODE_SERVICE.getProperty(link.getNodeRef(), LinksModel.PROP_DESCRIPTION));
        assertEquals(
    ...
    /**
     * Ensures that when we try to write an entry to the container of a new site, it is correctly
     * setup for us. This test does it's own transactions
     */
    @Test
                    ...
                    public Void execute() throws Throwable {
                        ...
                        assertFalse(
                                SITE_SERVICE.hasContainer(
                                        TEST_SITE_NAME, LinksServiceImpl.LINKS_COMPONENT));
    ...
    public void tagging() throws Exception {
        ...
        // Check
        when(TAGGING_SERVICE.getTags(link.getNodeRef()))
                .thenReturn(new ArrayList<>(Arrays.asList(TAG_1, TAG_2)));
        link = LINKS_SERVICE.getLink(LINKS_SITE.getShortName(), link.getSystemName());
        ...
        // Check it as-is
        when(TAGGING_SERVICE.getTags(link.getNodeRef()))
                .thenReturn(new ArrayList<>(Arrays.asList(TAG_1, TAG_3)));
        ...
        // Check
        when(TAGGING_SERVICE.getTags(link.getNodeRef()))
                .thenReturn(new ArrayList<>(Arrays.asList(TAG_1, TAG_2, TAG_3)));
        link = LINKS_SERVICE.getLink(LINKS_SITE.getShortName(), link.getSystemName());
        assertEquals(3, link.getTags().size());
        assertEquals(true, link.getTags().contains(TAG_1));
        assertEquals(true, link.getTags().contains(TAG_2));
    ...
    public void linksListing() throws Exception {
        ...
        // Fairly old ones
        results =
                LINKS_SERVICE.listLinks(LINKS_SITE.getShortName(), twoWeeksAgo, yesterday, paging);
        assertEquals(1, results.getPage().size());
    ...
    /**
     * Checks that the correct permission checking occurs on fetching links listings (which go
    ...
    public void linksListingPermissionsChecking() throws Exception {
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        assertEquals(
                                true, SITE_SERVICE.isMember(LINKS_SITE.getShortName(), TEST_USER));
                        assertEquals(
                                false,
                                SITE_SERVICE.isMember(
                                        ALTERNATE_LINKS_SITE.getShortName(), TEST_USER));
                        return null;
                    }
                });
        ...
        LinkInfo linkPrivA =
                LINKS_SERVICE.createLink(
                        ALTERNATE_LINKS_SITE.getShortName(),
                        "PrivTitleA",
                        "Description",
    ...
    public void linksSearching() throws Exception {
        ...
        // Nothing to start with in either site
        results =
                LINKS_SERVICE.findLinks(LINKS_SITE.getShortName(), null, null, null, null, paging);
        assertEquals(0, results.getPage().size());
        results =
                LINKS_SERVICE.findLinks(
                        ALTERNATE_LINKS_SITE.getShortName(), null, null, null, null, paging);
        assertEquals(0, results.getPage().size());
        ...
        results =
                LINKS_SERVICE.findLinks(
                        LINKS_SITE.getShortName(), TEST_USER, null, null, null, paging);
        assertEquals(2, results.getPage().size());
        results =
                LINKS_SERVICE.findLinks(
                        ALTERNATE_LINKS_SITE.getShortName(), TEST_USER, null, null, null, paging);
        assertEquals(1, results.getPage().size());

        results =
                LINKS_SERVICE.findLinks(
                        LINKS_SITE.getShortName(), ADMIN_USER, null, null, null, paging);
        assertEquals(0, results.getPage().size());
        results =
                LINKS_SERVICE.findLinks(
                        ALTERNATE_LINKS_SITE.getShortName(), ADMIN_USER, null, null, null, paging);
        assertEquals(3, results.getPage().size());
        ...
        results =
                LINKS_SERVICE.findLinks(
                        LINKS_SITE.getShortName(), null, yesterday, tomorrow, null, paging);
        assertEquals(1, results.getPage().size());
        results =
                LINKS_SERVICE.findLinks(
                        ALTERNATE_LINKS_SITE.getShortName(),
        ...
        results =
                LINKS_SERVICE.findLinks(
                        LINKS_SITE.getShortName(), null, twoWeeksAgo, yesterday, null, paging);
        assertEquals(1, results.getPage().size());
        results =
                LINKS_SERVICE.findLinks(
                        ALTERNATE_LINKS_SITE.getShortName(),
        ...
        results =
                LINKS_SERVICE.findLinks(
                        LINKS_SITE.getShortName(), null, twoWeeksAgo, tomorrow, null, paging);
        assertEquals(2, results.getPage().size());
        results =
                LINKS_SERVICE.findLinks(
                        ALTERNATE_LINKS_SITE.getShortName(),
        ...
        results =
                LINKS_SERVICE.findLinks(LINKS_SITE.getShortName(), null, null, null, TAG_1, paging);
        assertEquals(1, results.getPage().size());
        results =
                LINKS_SERVICE.findLinks(
                        ALTERNATE_LINKS_SITE.getShortName(), null, null, null, TAG_1, paging);
        assertEquals(2, results.getPage().size());

        // Tag2 is on A
        results =
                LINKS_SERVICE.findLinks(LINKS_SITE.getShortName(), null, null, null, TAG_2, paging);
        assertEquals(1, results.getPage().size());
        results =
                LINKS_SERVICE.findLinks(
                        ALTERNATE_LINKS_SITE.getShortName(), null, null, null, TAG_2, paging);
        assertEquals(0, results.getPage().size());

        // Tag3 is on PrivA
        results =
                LINKS_SERVICE.findLinks(LINKS_SITE.getShortName(), null, null, null, TAG_3, paging);
        assertEquals(0, results.getPage().size());
        results =
                LINKS_SERVICE.findLinks(
                        ALTERNATE_LINKS_SITE.getShortName(), null, null, null, TAG_3, paging);
        assertEquals(1, results.getPage().size());

        // Tag4 isn't on anything
        results =
                LINKS_SERVICE.findLinks(LINKS_SITE.getShortName(), null, null, null, TAG_4, paging);
        assertEquals(0, results.getPage().size());
        results =
                LINKS_SERVICE.findLinks(
                        ALTERNATE_LINKS_SITE.getShortName(), null, null, null, TAG_4, paging);
        assertEquals(0, results.getPage().size());

        // Filter by username and date and tag
        results =
                LINKS_SERVICE.findLinks(
                        LINKS_SITE.getShortName(), ADMIN_USER, yesterday, tomorrow, TAG_1, paging);
                ...
                new RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        PUBLIC_NODE_SERVICE.setProperty(
                                node, ContentModel.PROP_CREATED, newCreated);
                        PUBLIC_NODE_SERVICE.setProperty(
                                node, ContentModel.PROP_DESCRIPTION, "Forced Change");
                        return null;
                    }
                },
                false,
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        assertEquals(
                                newCreated,
                                NODE_SERVICE.getProperty(node, ContentModel.PROP_CREATED));
                        assertEquals(
                                newCreated,
                                PUBLIC_NODE_SERVICE.getProperty(node, ContentModel.PROP_CREATED));
                        return null;
                    }
                },
                false,
    ...
    private static void createTestSites() throws Exception {
        final LinksServiceImpl privateCalendarService =
                (LinksServiceImpl) testContext.getBean("linksService");
                        ...
                        new RetryingTransactionHelper.RetryingTransactionCallback<SiteInfo>() {
                            ...
                            public SiteInfo execute() throws Throwable {
                                SiteInfo site =
                                        SITE_SERVICE.createSite(
                                                TEST_SITE_PREFIX,
                                                LinksServiceImplTest.class.getSimpleName()
                                                        + "_testSite"
                                                        + System.currentTimeMillis(),
                                                "test site title",
                                                "test site description",
                                                SiteVisibility.PUBLIC);
                                privateCalendarService.getSiteLinksContainer(
                                        site.getShortName(), true);

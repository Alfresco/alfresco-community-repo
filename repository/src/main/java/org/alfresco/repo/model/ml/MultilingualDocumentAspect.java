 ...
 *
 * <p>{@link ContentModel#ASPECT_MULTILINGUAL_DOCUMENT ml document aspect}
 *
 * @author yanipig
 */
public class MultilingualDocumentAspect
     ...
     *
     * <p>Ensures that the {@link ContentModel#ASPECT_MULTILINGUAL_DOCUMENT ml document aspect}
     */
    public void init() {
    ...
    /**
     * Make sure that the child is no longer a translation and that the associated container is
     * cleaned up, if required.
     */
    @Override
     ...
     *
     * <p>If the locale of a pivot translation is modified, the pivot locale reference of the
     * mlContainer must be modified too.
     */
    public void onUpdateProperties(
        ...
        if (localeBefore == null || !localeBefore.equals(localeAfter)) {
            ...
            // Since the map returned by the getTranslations doesn't duplicate keys, the size of
            // this map will be
            // different of the size of the number of children of the mlContainer if a duplicate
            // locale is found.
            ...
            if (transSize != childSize) {
                // throw an exception and the current transaction will be rolled back. The
                // properties will not be
                // longer in an illegal state.
                throw new IllegalArgumentException(
                        "The locale "
                                + localeAfter
                                + " can't be changed for the node "
                                + nodeRef
                                + " because this locale is already in use in an other translation"
                                + " of the same "
                                + ContentModel.TYPE_MULTILINGUAL_CONTAINER
                                + ".");
            }
            ...
            Locale localMlContainer =
                    (Locale) nodeService.getProperty(mlContainer, ContentModel.PROP_LOCALE);
            ...
            if (localeBefore != null && localeBefore.equals(localMlContainer)) {
                nodeService.setProperty(mlContainer, ContentModel.PROP_LOCALE, localeAfter);
            }
        }

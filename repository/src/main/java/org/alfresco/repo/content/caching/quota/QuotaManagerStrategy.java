...
public interface QuotaManagerStrategy {
    /**
     * Called immediately before writing a cache file or (when cacheOnInBound is set to true for the
     * CachingContentStore) before handing a ContentWriter to a content producer.
     *
     * <p>In the latter case, the contentSize will be unknown (0), since the content length hasn't
    ...
    /**
     * Called immediately after writing a cache file - specifying the size of the file that was
     * written. The return value allows implementations control over whether the new cache file is
     * kept (true) or immediately removed (false).
     *

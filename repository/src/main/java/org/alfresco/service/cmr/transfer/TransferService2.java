...
/**
 * The transfer service is responsible for transferring nodes between one instance of Alfresco and
 * another remote instance. as well as the transfer method, this interface also provides methods for
...
public interface TransferService2 {
    ...
    /**
     * Transfer nodes sync, with callback. This synchronous version of the transfer method waits for
     * the transfer to complete before returning to the caller. Callbacks are called in the current
     * thread context, so will be associated with the current transaction and user.
     *
     * @param targetName the name of the target to transfer to
     * @param definition - the definition of the transfer. Specifies which nodes to transfer. The
     *     following properties must be set, nodes
     * @param callback - a set of callback handlers that will be called as transfer proceeds. May be
     *     null.
     ...
     * @param definition - the definition of the transfer. Specifies which nodes to transfer. The
     *     following properties must be set, nodes
     * @param callbacks - a list of callback handlers that will be called as transfer proceeds. May
     *     be null.
     * @throws TransferException
     * @return transfer end event (in case of success or cancellation)
     */
    ...
    /**
     * Transfer nodes async with callback. The asynchronous version of the transfer method starts a
     * transfer and returns as soon as possible.
     *
     * <p>The transfer callbacks will be called by a different thread to that used to call the
     * transferAsync method so transaction context will be different to the calling context. The
     * asychronous transfer does not have access to uncommitted data in the calling transaction.
     *
     * @param targetName the name of the target to transfer to
     * @param definition - the definition of the transfer. Specifies which nodes to transfer. The
     *     following properties must be set, nodes
     * @param callback - a collection of callback handlers that will be called as transfer proceeds.
     *     May be null.
     * @throws TransferException
     */
    @Auditable(parameters = {"targetName"})
     ...
     * @param definition - the definition of the transfer. Specifies which nodes to transfer. The
     *     following properties must be set, nodes
     * @param callbacks - a collection of callback handlers that will be called as transfer
    ...
    /**
     * Create and save a new transfer target. Creates and saves a new transfer target with a single,
     * but long, method call.
     *
    ...
    /**
     * Creates an in memory transfer target. Before it is used it must be populated with the
     * following values and saved with the saveTransferTarget method. The name of the transfer
    ...
    /**
     * Save TransferTarget, will create a transfer target if it does not already exist or update an
     * existing transfer target.
     *
     * <p>The following properties may be updated: endpointHost, endpointPort, endpointProtocol,
     * endpointPath, username, password, title, description
     *
     * <p>The following properties may not be updated: name, must be specified. nodeRef, if
     ...
     *
     * <p>This method tells an in-process transfer to give up, rollback and stop as soon as
     * possible.
     *
     * <p>Depending upon the state of the in-progress transfer, the transfer may still complete,
     * despite calling this method, however in most cases the transfer will not complete.
     *
     * <p>Calling this method for a transfer that does not exist, possibly because it has already
     * finished, has no effect and will not throw an exception.
     *
     * <p>The transfer identifier can be obtained from the TransferEventBegin event that is passed
     * to registered callbacks when transfer starts.
     *

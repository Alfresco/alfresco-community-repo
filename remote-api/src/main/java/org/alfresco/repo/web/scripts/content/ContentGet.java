 ...
 *
 * <p>Stream content from the Repository.
 *
 * @author davidc
 */
public class ContentGet extends StreamContent implements ServletContextAware {
    ...
    public void execute(WebScriptRequest req, WebScriptResponse res) throws IOException {
        ...
        if (nodeRef == null) {
            throw new WebScriptException(
                    HttpServletResponse.SC_NOT_FOUND, "Unable to find " + reference.toString());
        }
        ...
        if (contentPart.length() > 0 && contentPart.charAt(0) == ';') {
            if (contentPart.length() < 2) {
                throw new WebScriptException(
                        HttpServletResponse.SC_BAD_REQUEST, "Content property malformed");
            }
    ...
    protected void streamContentLocal(
        ...
        if (attach && rfc5987Supported) {
            ...
            // maintain the original name of the node during the download - do not modify it - see
            // MNT-16510

...
/**
 * Cleans up redundant cache files from the cached content file store. Once references to cache
 * files are no longer in the in-memory cache, the binary content files can be removed.
 *
 * @author Matt Ward
 */
public class CachedContentCleaner extends Thread
        implements FileHandler, ApplicationEventPublisherAware {
    ...
    /**
     * This method MUST be called after the cleaner has been fully constructed to notify interested
     * parties that the cleaner exists and to start the actual cleaner thread.
     */
    public void init() {
        ...
        if (log.isInfoEnabled()) {
            log.info(
                    "Finished, duration: "
                            ...
                            + numFilesDeleted
                            + " ("
                            + String.format("%.2f", getSizeFilesDeletedMB())
                            + "MB, "
                            + sizeFilesDeleted
                            + " bytes)"
                            + ", target: "
                            + targetReductionBytes
                            + " bytes");
        }
        ...
        if (targetReductionBytes > 0 && sizeFilesDeleted < targetReductionBytes) {
            if (log.isDebugEnabled()) {
                log.debug(
                        "Target reduction "
                                + targetReductionBytes
                                + " bytes not yet reached. Deleted so far: "
                                + sizeFilesDeleted);
            }
            ...
            if (oldEnoughForCleanup(cachedContentFile)) {
                ...
                if (url == null || !cache.contains(url)) {
                    // If the url is null, it might still be in the cache, but we were unable to
                    // determine it
                    // from the reverse lookup or the properties file. Delete the file as it is most
                    // likely orphaned.
                    // If for some reason it is still in the cache, cache.getReader(url) must
                    // re-cache it.
    ...
    /**
     * Is the file old enough to be considered for cleanup/deletion? The file must be older than
     * minFileAgeMillis to be considered for deletion - the state of the cache and the file's
     * associated properties file will not be examined unless the file is old enough.
     *
    ...
    /**
     * Marks a file for deletion by a future run of the CachedContentCleaner. Each time a file is
     * observed by the cleaner as being ready for deletion, the deleteWatchCount is incremented
     * until it reaches maxDeleteWatchCount - in which case the next run of cleaner will really
     * delete it.
     *
     * <p>For maxDeleteWatchCount of 1 for example, the first cleaner run will mark the file for
     * deletion and the second run will really delete it.
     *
     * <p>This offers a degree of protection over the fairly unlikely event that a reader will be
     * obtained for a file that is in the cache but gets removed from the cache and is then deleted
     * by the cleaner before the reader was consumed. A maxDeleteWatchCount of 1 should normally be
     * fine (recommended), whilst 0 would result in immediate deletion the first time the cleaner
     * sees it as a candidate for deletion (not recommended).
     *
     * @param file File
    ...
    private boolean markOrDelete(File file, CacheFileProps props) {
        ...
        if (deleteWatchCount < 0) deleteWatchCount = 0;
        ...
        if (deleteWatchCount < maxDeleteWatchCount) {
            ...
            if (log.isDebugEnabled()) {
                log.debug(
                        "Marking file for deletion, deleteWatchCount="
                                + deleteWatchCount
                                + ", file: "
                                + file);
            }
    ...
    /**
     * Deletes both the cached content file and its peer properties file that contains the original
     * content URL and deletion marker information.
     *

...
public class PasswordCredentialsFactory implements RemoteCredentialsInfoFactory {
     ...
     */
    public PasswordCredentialsInfo createCredentials(
            QName type,
            NodeRef nodeRef,
        ...
        String password =
                (String)
                        metadataEncryptor.decrypt(
                                RemoteCredentialsModel.PROP_REMOTE_PASSWORD,
                                properties.get(RemoteCredentialsModel.PROP_REMOTE_PASSWORD));

        // Build the object
        PasswordCredentialsInfoImpl credentials =
                new PasswordCredentialsInfoImpl(
    ...
    public Map<QName, Serializable> serializeCredentials(BaseCredentialsInfo info) {
        if (!(info instanceof PasswordCredentialsInfo)) {
            throw new IllegalStateException(
                    "Incorrect registration, info must be a PasswordCredentialsInfo");
        }

        // Encrypt the password
        PasswordCredentialsInfo credentials = (PasswordCredentialsInfo) info;
        Serializable passwordEncrypted =
                metadataEncryptor.encrypt(
                        RemoteCredentialsModel.PROP_REMOTE_PASSWORD,
                        credentials.getRemotePassword());

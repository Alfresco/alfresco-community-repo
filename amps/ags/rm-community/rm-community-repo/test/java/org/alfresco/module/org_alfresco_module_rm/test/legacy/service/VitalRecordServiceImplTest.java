...
public class VitalRecordServiceImplTest extends BaseRMTestCase {
  ...
  private void assertHasVitalRecordDefinition(
    NodeRef nodeRef,
    boolean enabled,
    Period period
  ) {
    ...
    VitalRecordDefinition def = vitalRecordService.getVitalRecordDefinition(
      nodeRef
    );
    assertNotNull(def);

    Boolean vitalRecordIndicator = (Boolean) nodeService.getProperty(
      nodeRef,
    ...
    if (enabled) {
      Period reviewPeriod = (Period) nodeService.getProperty(
        nodeRef,
        PROP_REVIEW_PERIOD
      );
      ...
      assertEquals(
        period.getNextDate(new Date()).getDate(),
        def.getNextReviewDate().getDate()
      );
    }
  }

  @SuppressWarnings("deprecation")
  private void assertVitalRecord(
    NodeRef nodeRef,
    boolean enabled,
    Period period
  ) {
    ...
    if (enabled) {
      Date reviewAsOf = (Date) nodeService.getProperty(
        nodeRef,
        PROP_REVIEW_AS_OF
      );
      ...
      assertEquals(
        ...
        nodeService.getProperty(
          nodeRef,
          RecordsManagementSearchBehaviour.PROP_RS_VITAL_RECORD_REVIEW_PERIOD_EXPRESSION
        )
      );
    } else {
      assertNull(

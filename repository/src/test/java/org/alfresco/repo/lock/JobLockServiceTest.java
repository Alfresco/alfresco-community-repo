...
/**
 * Tests the high-level capabilities provided by the service implementation. The DAO tests stress
 * the underlying database work, so we only need to deal with deadlock resolution, etc.
 *
...
public class JobLockServiceTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        lockAAA =
                QName.createQName(NAMESPACE, "a-" + testName + ".a-" + testName + ".a-" + testName);
        lockAAB =
                QName.createQName(NAMESPACE, "a-" + testName + ".a-" + testName + ".b-" + testName);
        lockAAC =
                QName.createQName(NAMESPACE, "a-" + testName + ".a-" + testName + ".c-" + testName);
        lockAB = QName.createQName(NAMESPACE, "a-" + testName + ".b-" + testName);
        lockABA =
                QName.createQName(NAMESPACE, "a-" + testName + ".b-" + testName + ".a-" + testName);
        lockABB =
                QName.createQName(NAMESPACE, "a-" + testName + ".b-" + testName + ".b-" + testName);
        lockABC =
                QName.createQName(NAMESPACE, "a-" + testName + ".b-" + testName + ".c-" + testName);
    }
    ...
    public void testLockInReadOnly() throws Exception {
        RetryingTransactionCallback<Object> lockCallback =
    ...
    public void testLockReleaseOnCommit() throws Exception {
        ...
        RetryingTransactionCallback<Object> lockCheckCallback =
                new RetryingTransactionCallback<Object>() {
                    public Object execute() throws Throwable {
    ...
    /**
     * Sets up two threads in a deadlock scenario. Each of the threads has a long wait timeout for
     * the required locks. If there were a deadlock, the shorter of the the wait times would be how
     * long it would take before one of them is thrown out. Firstly, we check that one of the
     * threads <i>is</i> thrown out. Then we check that the thread is thrown out quickly.
     */
    public synchronized void testDeadlockPrevention() throws Throwable {
        ...
        assertNull(
                "T1 should have succeeded as the ordered locker: " + t1.lockFailure,
                t1.lockFailure);
    ...
    private class DeadlockingThread extends Thread {
        ...
        @Override
        public void run() {
            RetryingTransactionCallback<Object> runCallback =
                    new RetryingTransactionCallback<Object>() {
                        public synchronized Object execute() throws Throwable {
                            int currentLock = -1;
                            // Take the locks in turn, quitting when told to take a lock that's not
                            // there
    ...
    public synchronized void testLockCallbackReleaseSelf() throws Exception {
        // ACE-4347 extra debug logging just for this test so we can see what's going on when it
        // next fails

...
public class CustomModelServiceImpl implements CustomModelService {
    ...
    public NodeRef getModelNodeRef(String modelName) {
        ...
        builder.append(repoModelsLocation.getPath())
                .append("//.[@cm:name='")
                .append(modelName)
                .append("' and ")
        ...
        List<NodeRef> nodeRefs =
                searchService.selectNodes(
                        getRootNode(), builder.toString(), null, namespaceDAO, false);
    ...
    public CustomModelDefinition getCustomModel(String modelName) {
        ...
        CustomModelDefinition result =
                (compiledModelPair == null)
                        ? null
    ...
    protected Pair<CompiledModel, Boolean> getCustomCompiledModel(String modelName) {
        ...
        Pair<CompiledModel, Boolean> result =
                (model == null) ? null : new Pair<>(compileModel(model), isActive);
    ...
    public CustomModelDefinition createCustomModel(M2Model m2Model, boolean activate) {
        ...
        final String modelFileName =
                (colonIndex == -1) ? modelName : modelName.substring(colonIndex + 1);

        if (isModelExists(modelFileName)) {
            throw new CustomModelException.ModelExistsException(
                    MSG_NAME_ALREADY_IN_USE, new Object[] {modelFileName});
        }
        ...
        CompiledModel compiledModel =
                createUpdateModel(modelFileName, m2Model, activate, MSG_CREATE_MODEL_ERR, false);
        CustomModelDefinition modelDef =
                new CustomModelDefinitionImpl(compiledModel, activate, dictionaryService);
    ...
    public CustomModelDefinition updateCustomModel(
        ...
        if (existingModelNodeRef == null) {
            throw new CustomModelException.ModelDoesNotExistException(
                    MSG_MODEL_NOT_EXISTS, new Object[] {modelFileName});
        }
        // Existing model property and namespace uri-prefix pair
        final boolean isActive =
                Boolean.TRUE.equals(
                        nodeService.getProperty(
                                existingModelNodeRef, ContentModel.PROP_MODEL_ACTIVE));
        final M2Model existingModel = getM2Model(existingModelNodeRef);
        final Pair<String, String> existingNamespacePair =
                getModelNamespaceUriPrefix(existingModel);
         ...
         */
        CompiledModel compiledModel =
                createUpdateModel(modelFileName, m2Model, activate, MSG_UPDATE_MODEL_ERR, true);
        CustomModelDefinition modelDef =
                new CustomModelDefinitionImpl(compiledModel, activate, dictionaryService);
                        ...
                        new RetryingTransactionCallback<NodeRef>() {
                            public NodeRef execute() throws Exception {
                                return repoAdminService.deployModel(
                                        modelStream, modelFileName, activate);
                            }
                        });
            ...
            if (propertyDef.getDefaultValue() != null && propertyDef.getConstraints().size() > 0) {
                for (ConstraintDefinition constraintDef : propertyDef.getConstraints()) {
                    ...
                    try {
                        ...
                        String message =
                                getRootCauseMsg(
                                        ex, false, "cmm.service.constraint.default_prop_value_err");
        ...
        try {
            ...
            if (cause instanceof DuplicateDefinitionException) {
                ...
                throw new CustomModelException.InvalidCustomModelException(
                        MSG_INVALID_MODEL, new Object[] {message}, ex);
            }
        }
    }
    ...
    public void deactivateCustomModel(final String modelName) {
        ...
        if (customModelDefinition == null) {
            throw new CustomModelException.ModelDoesNotExistException(
                    MSG_MODEL_NOT_EXISTS, new Object[] {modelName});
        }
        ...
        for (CompiledModel cm : getAllCustomM2Models(false)) {
            ...
            if (!customModelDefinition.getName().equals(cm.getModelDefinition().getName())) {
                // Check if the type of the model being deactivated is the parent of another model's
                // type
                validateTypeAspectDependency(modelTypes, cm.getTypes());

                // Check if the aspect of the model being deactivated is the parent of another
                // model's aspect
    ...
    public boolean isNamespacePrefixExists(String modelNamespacePrefix) {
        ...
        for (CompiledModel model : getAllCustomM2Models(false)) {
            if (modelNamespacePrefix.equals(
    ...
    public boolean isNamespacePrefixExists(NodeRef modelNodeRef) {
        ...
        for (CompiledModel model : getAllCustomM2Models(false)) {
            ...
            if (modelNamespacePrefix.equals(existingPrefix)) {
                // Get the nodeRef for the model which owns this prefix to ensure it is not the same
                // model being validated
                NodeRef existingModelNodeRef =
                        getModelNodeRef(model.getModelDefinition().getName().getLocalName());
    ...
    public Pair<String, String> getModelNamespaceUriPrefix(M2Model model) {
        ...
        if (namespaces.isEmpty()) {
            throw new CustomModelException.InvalidNamespaceException(
                    MSG_NAMESPACE_NOT_EXISTS, new Object[] {model.getName()});
        }
        if (namespaces.size() > 1) {
            throw new CustomModelException.InvalidNamespaceException(
                    MSG_NAMESPACE_MANY_EXIST, new Object[] {model.getName()});
        }
    ...
    private void validateModelNamespaceUri(String uri) {
        if (isNamespaceUriExists(uri)) {
            throw new CustomModelException.NamespaceConstraintException(
                    MSG_NAMESPACE_URI_ALREADY_IN_USE, new Object[] {uri});
        }
    }
    ...
    public void validateModelNamespacePrefix(String prefix) {
        if (isNamespacePrefixExists(prefix)) {
            throw new CustomModelException.NamespaceConstraintException(
                    MSG_NAMESPACE_PREFIX_ALREADY_IN_USE, new Object[] {prefix});
        }
    }
     ...
     *
     * @param errMsgId message id for the new wrapper exception ({@link CustomModelException}) when
     *     an exception occurs
     * @param requiresNewTx <tt>true</tt> to force a new transaction or <tt>false</tt> to partake in
    ...
    private <R> R doInTransaction(
            String errMsgId, boolean requiresNewTx, RetryingTransactionCallback<R> cb) {
        try {
    ...
    private static String getRootCauseMsg(
            Throwable cause, boolean withAlfLogNum, String defaultMsg) {
        if (defaultMsg == null) {
            defaultMsg = "";
        }
    ...
    public NodeRef createDownloadNode(final String modelFileName, boolean withAssociatedForm) {
        ...
        if (withAssociatedForm) {
            ...
            try {
                ...
                // draft model and might have never been activated or never had any forms created
                // for it.
    ...
    /**
     * Finds the {@code module} element within the Share persisted-extension XML file and then
     * writes the XML fragment as the content of a newly created node.
     *
    ...
    protected NodeRef createCustomModelShareExtModuleRef(final String modelName) {
        ...
        if (moduleIdXmlNode == null) {
            throw new CustomModelException(
                    "cmm.service.download.share_ext_module_not_found", new Object[] {moduleId});
        }
        ...
        try {
            ...
            throw new CustomModelException(
                    "cmm.service.download.share_ext_write_err", new Object[] {moduleId}, error);
        }
                ...
                new RetryingTransactionCallback<NodeRef>() {
                    ...
                    public NodeRef execute() throws Exception {
                        final NodeRef nodeRef =
                                createDownloadTypeNode(moduleId + SHARE_EXT_MODULE_SUFFIX);
    ...
    /**
     * Creates a copy of the custom model where the created node will be a child of download
     * container.
     *
                ...
                new RetryingTransactionCallback<NodeRef>() {
                    ...
                    public NodeRef execute() throws Exception {
                        ...
                        Serializable content =
                                nodeService.getProperty(modelNodeRef, ContentModel.PROP_CONTENT);
    ...
    /**
     * Creates node with a type {@link DownloadModel#TYPE_DOWNLOAD} within the download container
     * (see {@link DownloadStorage#getOrCreateDowloadContainer()} )
     *
     * <p>Also, the {@code IndexControlAspect} is applied to the created node.
     *
    ...
    public CustomModelsInfo getCustomModelsInfo() {
        List<CustomModelDefinition> page =
                getCustomModels(new PagingRequest(0, Integer.MAX_VALUE)).getPage();

...
public class InviteModeratedSenderTest extends TestCase {
  ...
  private static final NodeRef requesterNodeRef = new NodeRef(
    testStore,
    requesterUserName
  );
  ...
  private static final NodeRef emailTemplateNodeRef = new NodeRef(
    testStore,
    "emailTemplate"
  );
  ...
  public void testSendModeratedEmail() throws Exception {
    ...
    inviteModeratedSender.sendMail(
      ENTERPRISE_EMAIL_TEMPLATE_PATH,
      SendModeratedInviteDelegate.EMAIL_SUBJECT_KEY,
      properties
    );

    verify(mailAction)
      .setParameterValue(eq(MailActionExecuter.PARAM_FROM), eq(requesterMail));
    verify(mailAction)
      .setParameterValue(
        eq(MailActionExecuter.PARAM_TO_MANY),
        eq(SiteManagerGroup)
      );
    verify(mailAction)
      .setParameterValue(
    ...
    String pendingInvitesLink = StringUtils.stripStart(
      MessageFormat.format(
        InviteModeratedSender.SHARE_PENDING_INVITES_LINK,
        StringUtils.EMPTY,
        shortSiteName
      ),
      "/"
  ...
  protected void setUp() throws Exception {
    ...
    inviteModeratedSender =
      new InviteModeratedSender(services, repository, messageService);
  }
  ...
  private FileFolderService mockFileFolderService() {
    ...
    when(fileFolderService.getLocalizedSibling(emailTemplateNodeRef))
      .thenReturn(emailTemplateNodeRef);
  ...
  private PersonService mockPersonService() {
    ...
    when(personService.getPerson(requesterUserName))
      .thenReturn(requesterNodeRef);
    when(personService.getPerson(requesterNodeRef))
      .thenReturn(
        new PersonInfo(
          requesterNodeRef,
          requesterUserName,
  ...
  private NodeService mockNodeService() {
    ...
    when(nodeService.getProperty(requesterNodeRef, ContentModel.PROP_EMAIL))
      .thenReturn(requesterMail);
    return nodeService;
  }

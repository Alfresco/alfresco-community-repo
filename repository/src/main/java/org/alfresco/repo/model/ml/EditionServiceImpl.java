...
public class EditionServiceImpl implements EditionService {
  ...
  public NodeRef createEdition(
    NodeRef startingTranslationNodeRef,
  ...
  private NodeRef createEditionImpl(
    ...
    NodeRef mlContainerToVersion = multilingualContentService.getTranslationContainer(
      startingTranslationNodeRef
    );
    ...
    Version currentEdition = versionService.getCurrentVersion(
      mlContainerToVersion
    );
    ...
    NodeRef space = nodeService
      .getPrimaryParent(startingTranslationNodeRef)
      .getParentRef();
    ...
    String author = (String) nodeService.getProperty(
      startingTranslationNodeRef,
      ContentModel.PROP_AUTHOR
    );
    ...
    for (int count = 0;; count++) {
      try {
        ...
        String tempName =
          "TEMP_NAME" + System.currentTimeMillis() + "_" + count;
    ...
    if (logger.isDebugEnabled()) {
      ...
      Version mlContainerVersion = versionService.getCurrentVersion(
        mlContainerToVersion
      );
  ...
  public Map<QName, Serializable> getVersionedMetadatas(Version version) {
    ...
    if (
      frozenNodeRef
        .getStoreRef()
        .getIdentifier()
        .equals("lightWeightVersionStore")
    ) {
    ...
    if (
      ContentModel.TYPE_MULTILINGUAL_CONTAINER.equals(
        nodeService.getType(frozenNodeRef)
      )
    ) {
  ...
  /** {@inheritDoc} */
  public List<VersionHistory> getVersionedTranslations(
    Version mlContainerEdition
  ) {
    ...
    Map<QName, Serializable> properties = versionNodeService.getProperties(
      mlContainerEdition.getFrozenStateNodeRef()
    );
    ...
    if (
      ...
      Object histories = properties.get(
      ...
      // get the serialisation of the version histories in the version properties
      Object histories = properties.get(
        VersionModel.PROP_QNAME_TRANSLATION_VERSIONS
      );
      if (histories instanceof List) {
        versionHistories = (List<VersionHistory>) histories;
      } else if (histories instanceof VersionHistory) {
  ...
  private void addVersionHistoryProperty(
    ...
    List<VersionHistory> translationVersionHistories = new ArrayList<VersionHistory>(
      childAssocRefs.size()
    );
    ...
    for (ChildAssociationRef ref : childAssocRefs) {
      ...
      translationVersionHistories.add(
        versionService.getVersionHistory(translation)
      );
    }
  ...
  private void addPropertiesToVersion(
    ...
    for (QName prop : ML_CONTAINER_PROPERTIES_TO_VERSION) {
      versionProperties.put(
        prop.toString(),
        nodeService.getProperty(mlContainerToVersion, prop)
      );
    }
  }
  ...
  private void removeTranslations(List<ChildAssociationRef> childAssocRefs) {
    ...
    try {
      for (ChildAssociationRef childAssoc : childAssocRefs) {
        ...
        if (
          nodeService.exists(
            nodeArchiveService.getArchivedNode(documentNodeRef)
          )
        ) {

...
public abstract class BaseKerberosAuthenticationFilter extends BaseSSOAuthenticationFilter
    ...
    /**
     * Sets the HTTP service account password. (the Principal should be configured in
     * java.login.config)
     *
    ...
    /**
     * Sets the HTTP service login configuration entry name. The default is <code>"AlfrescoHTTP"
     * </code>.
     *
     * @param jaasConfigEntryName the jaasConfigEntryName to set
     */
     ...
     *
     * @param stripKerberosUsernameSuffix <code>true</code> if the @domain suffix should be removed
     *     from Kerberos user IDs
     */
    ...
    protected void init() throws ServletException {
        ...
        if (getLogger().isTraceEnabled()) {
            getLogger()
                    .trace(
                            "Logged on using principal "
                                    + AuthenticationUtil.maskUsername(m_accountName));
        }
        ...
        try {
            ...
            // Build the SPNEGO NegTokenInit that contains the authentication types that the HTTP
            // server accepts
    ...
    public boolean authenticateRequest(
        ...
        if (authHdr != null) {
            ...
            if (authHdr.startsWith("Negotiate")) reqAuth = true;
        ...
        if (user == null) {
            ...
            // MNT-13191 Opening /alfresco/webdav from a Kerberos-authenticated IE11 browser causes
            // HTTP error 500
            if (user != null) {
        ...
        if (authHdr == null) {
            ...
            if (getLogger().isTraceEnabled()) {
                getLogger()
                        .trace(
                                "New Kerberos auth request from "
                                        + req.getRemoteHost()
                                        + " ("
                                        + req.getRemoteAddr()
                                        + ":"
            ...
            if (tokType == SPNEGO.NegTokenInit) {
                ...
                try {
                    ...
                    String oidStr = null;
                    if (negToken.numberOfOids() > 0) oidStr = negToken.getOidAt(0).toString();
                    ...
                    if (oidStr != null
                        ...
                        try {
                            NegTokenTarg negTokenTarg =
                                    doKerberosLogon(negToken, req, resp, httpSess);
                            if (negTokenTarg != null) {
                                ...
                                onValidate(
                                        ...
                                        req,
                                        resp,
                                        new KerberosCredentials(negToken, negTokenTarg));
                                if (getLogger().isTraceEnabled()) {
                            ...
                            // Even though the user successfully authenticated, the ticket may not
                            // be granted, e.g. to
                            // max user limit
                            if (getLogger().isDebugEnabled()) {
                            ...
                            WebCredentials webCredentials =
                                    user == null ? null : new TicketCredentials(user.getTicket());
    ...
    public void handle(Callback[] callbacks) throws IOException, UnsupportedCallbackException {
        ...
        if (getLogger().isTraceEnabled()) {
            getLogger()
                    .trace("Processing the JAAS callback list of " + callbacks.length + " items.");
        }
    ...
    private final NegTokenTarg doKerberosLogon(
        ...
        try {
            ...
            SessionSetupPrivilegedAction sessSetupAction =
                    new SessionSetupPrivilegedAction(m_accountName, negToken.getMechtoken());
            ...
            if (result != null) {
                ...
                userName =
                        m_stripKerberosUsernameSuffix
                                ? krbDetails.getUserName()

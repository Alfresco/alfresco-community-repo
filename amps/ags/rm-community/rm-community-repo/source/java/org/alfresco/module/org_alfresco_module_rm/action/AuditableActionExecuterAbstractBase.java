 ...
 * @since 2.1
 */
public abstract class AuditableActionExecuterAbstractBase extends ActionExecuterAbstractBase
    ...
    /**
     * @param auditedImmediately true if to be audited immediately, false to be audited after
     *     transaction commits
     */
        ...
        if (auditService == null) {
            auditService =
                    (RecordsManagementAuditService)
                            getApplicationContext().getBean("recordsManagementAuditService");
        }
        return auditService;
    }
    ...
    /**
     * @see
     *     org.alfresco.repo.action.executer.ActionExecuterAbstractBase#execute(org.alfresco.service.cmr.action.Action,
    ...
    public void execute(Action action, NodeRef actionedUponNodeRef) {
        ...
        if (isAuditable()) {
            if (isAuditedImmediately()) {
                // To be audited immediately before the action is executed, eg. to audit before
                // actionedUponNodeRef gets deleted during the execution.
                getAuditService()
                        .auditEvent(
                                actionedUponNodeRef,
                ...
                // To be stacked up with other audit entries and audited after the transaction
                // commits.
                getAuditService()
                        .auditEvent(actionedUponNodeRef, this.getActionDefinition().getName());
            }
        }

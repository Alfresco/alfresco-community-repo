...
/**
 * DAO services for database control statements. It is sometimes necessary to issue control
 * statements on a database connection; these are not usually supported in the ANSI SQL standard.
 *
 * @author Derek Hulley
 * @since 3.2SP1
 */
public interface ControlDAO {
    ...
    /**
     * Create a "Save Point" in the current transaction, for later selective rollback. Creation
     * <b>must</b> be accompanied by a matching {@link #rollbackToSavepoint(Savepoint)} or {@link
     * #releaseSavepoint(Savepoint)}. <code><pre>
     ...
     * @return Returns the handle to the savepoint or <tt>null</tt> if the implementation does not
     *     support it
     */
    public Savepoint createSavepoint(String savepoint);

    /**
     * Roll back to a previously-created "Save Point", discarding any intervening changes to the
     * current transaction.
     *
    ...
    /**
     * Remove a previously-created "Save Point", writing any intervening updates into the current
     * transaction.
     *
     * @param savepoint the name of the save point
     * @see #createSavepoint(String)
     ...
     *
     * <p><b>Note:</b> The isolation level should not - and for some DBs, cannot - be changed except
     * at the very start of the transaction
     *

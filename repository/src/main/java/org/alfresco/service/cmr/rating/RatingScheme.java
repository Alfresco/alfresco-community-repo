...
/**
 * This interface defines a Rating Scheme, which is a named scheme for user-supplied ratings with a
 * defined minimum value and a defined maximum value. The minimum must not be greater than the
 * maximum but the two values can be equal. These schemes are defined within spring context files
 * and injected into the {@link RatingSchemeRegistry} at startup.
 *
...
public interface RatingScheme extends Comparable<RatingScheme> {
    ...
    /**
     * This method returns the namespace (prefix e.g. "cm") of the Alfresco content model containing
     * the definitions of the rollup aspect and properties.
     *
     * @since 4.1.5
    ...
    /**
     * This method returns <code>true</code> if the cm:creator of the node is allowed to apply a
     * rating to it, else <code>false</code>.
     *
    ...
    /**
     * This method returns a List of {@link AbstractRatingRollupAlgorithm property rollup
     * algorithms} which are used in order to calculate rating totals, counts etc for a rated node.
     *

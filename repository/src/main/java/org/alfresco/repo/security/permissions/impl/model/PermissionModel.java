...
public class PermissionModel implements ModelDAO {
   ...
   * PermissionReference)
   */
  private static PermissionReference ALL = SimplePermissionReference.getPermissionReference(
    QName.createQName(
  ...
  private static class MutableState {
    ...
    private Map<PermissionReference, Set<PermissionReference>> grantingPermissions = new HashMap<PermissionReference, Set<PermissionReference>>(
      256
    );

    // Cache grantees
    private Map<PermissionReference, Set<PermissionReference>> granteePermissions = new HashMap<PermissionReference, Set<PermissionReference>>(
      256
    );

    // Cache the mapping of extended groups to the base
    private Map<PermissionGroup, PermissionGroup> groupsToBaseGroup = new HashMap<PermissionGroup, PermissionGroup>(
      256
    );

    private Map<RequiredKey, Set<PermissionReference>> requiredPermissionsCache = new HashMap<RequiredKey, Set<PermissionReference>>(
      1024
    );

    private Map<Pair<PermissionReference, RequiredPermission.On>, Set<PermissionReference>> unconditionalRequiredPermissionsCache = new HashMap<Pair<PermissionReference, RequiredPermission.On>, Set<PermissionReference>>(
      1024
    );
      ...
      if (pg != null) {
        if (pg.isExtends()) {
          if (pg.getTypeQName() != null) {
            ...
            ClassDefinition classDefinition = dictionaryService.getClass(
              pg.getQName()
            );
            QName parent;
            while ((parent = classDefinition.getParentName()) != null) {
              ...
              PermissionGroup attempt = getPermissionGroupOrNull(
                SimplePermissionReference.getPermissionReference(
                  parent,
                  pg.getName()
                )
    ...
    private Set<PermissionReference> getAllPermissionsImpl(
      QName type,
      boolean exposedOnly
    ) {
     ...
     */
    private void addTypePermissions(
      QName type,
      ...
      if (typeDef.getParentName() != null) {
        ...
        if (
          !exposedOnly || (permissionSet == null) || permissionSet.exposeAll()
        ) {
     ...
     * @param exposedOnly boolean
     */
    private void addAspectPermissions(
      QName type,
      Set<PermissionReference> permissions,
      boolean exposedOnly
    ) {
      ...
      if (aspectDef.getParentName() != null) {
        ...
        if (
          ...
          addAspectPermissions(
            aspectDef.getParentName(),
            permissions,
            exposedOnly
          );
        }
    ...
    private void mergePermissions(
      Set<PermissionReference> target,
      QName type,
      boolean exposedOnly,
    ...
    private void mergeGeneralAspectPermissions(
      Set<PermissionReference> target,
      boolean exposedOnly
    ) {
     ...
     */
    private PermissionGroup getPermissionGroupOrNull(
      PermissionReference target
    ) {
    ...
    private PermissionGroup getPermissionGroup(PermissionReference target) {
      ...
      if (pg == null) {
        throw new PermissionModelException(
          "There is no permission group :" +
          target.getQName() +
          " " +
          target.getName()
        );
     ...
     * @return the permission group
     */
    private PermissionGroup getBasePermissionGroupOrNullImpl(
      PermissionGroup pg
    ) {
      if (pg == null) {
        return null;
    ...
    private PermissionGroup getBasePermissionGroup(PermissionGroup target) {
      ...
      if (pg == null) {
        throw new PermissionModelException(
          "There is no parent for permission group :" +
          target.getQName() +
          " " +
          target.getName()
        );
    ...
    private Set<PermissionReference> getGrantingPermissionsImpl(
    ...
    private Set<PermissionReference> getGranteePermissionsImpl(
      PermissionReference permissionReference
    ) {
      ...
      for (PermissionSet ps : permissionSets.values()) {
        ...
        PermissionGroup baseGroup = getBasePermissionGroupOrNull(
          getPermissionGroupOrNull(permissionReference)
        );
        if (baseGroup != null) {
          for (Permission p : ps.getPermissions()) {
            for (PermissionReference grantedTo : p.getGrantedToGroups()) {
              PermissionGroup base = getBasePermissionGroupOrNull(
                getPermissionGroupOrNull(grantedTo)
              );
    ...
    private Set<PermissionReference> getImmediateGranteePermissionsImpl(
      PermissionReference permissionReference
    ) {
      // Query the model
      HashSet<PermissionReference> permissions = new HashSet<PermissionReference>(
    ...
    private Set<PermissionReference> getGranteePermissions(
      PermissionReference permissionReference
    ) {
      ...
      // Cache the results
      Set<PermissionReference> grantees = granteePermissions.get(
        permissionReference
      );
      if (grantees == null) {
        boolean hadWriteLock = lock.isWriteLockedByCurrentThread();
        ...
        try {
          ...
          if (grantees == null) {
            Set<PermissionReference> internal = getGranteePermissionsImpl(
              permissionReference
            );
    ...
    private Set<PermissionReference> getImmediateGranteePermissions(
      PermissionReference permissionReference
    ) {
      // Cache the results

      Set<PermissionReference> internal = getImmediateGranteePermissionsImpl(
        permissionReference
      );
    ...
    private Set<PermissionReference> getRequiredPermissions(
      ...
      if (answer == null) {
        ...
        try {
          ...
          if (answer == null) {
            PermissionGroup pg = getBasePermissionGroupOrNull(
              getPermissionGroupOrNull(required)
            );
            if (pg == null) {
              answer = getRequirementsForPermission(required, on);
            } else {
              answer =
                getRequirementsForPermissionGroup(pg, on, qName, aspectQNames);
            }
    ...
    private Set<PermissionReference> getUnconditionalRequiredPermissions(
      PermissionReference required,
      RequiredPermission.On on
    ) {
      // Cache lookup as this is static
      ...
      Pair<PermissionReference, RequiredPermission.On> key = new Pair<PermissionReference, RequiredPermission.On>(
        required,
        on
      );

      Set<PermissionReference> answer = unconditionalRequiredPermissionsCache.get(
        key
      );
      if (answer == null) {
        boolean hadWriteLock = lock.isWriteLockedByCurrentThread();
     ...
     */
    private Set<PermissionReference> getRequirementsForPermission(
      PermissionReference required,
      RequiredPermission.On on
    ) {
     ...
     * @return the set of permission references
     */
    private Set<PermissionReference> getRequirementsForPermissionGroup(
      ...
      for (PermissionSet ps : permissionSets.values()) {
        for (PermissionGroup pg : ps.getPermissionGroups()) {
          ...
          if (
            (target.equals(base) || target.isAllowFullControl()) &&
            (
              !base.isTypeRequired() ||
            ...
            for (PermissionReference pr : pg.getIncludedPermissionGroups()) {
              requiredPermissions.addAll(
                getRequirementsForPermissionGroup(
                  getBasePermissionGroupOrNull(getPermissionGroupOrNull(pr)),
                  on,
                  qName,
                  aspectQNames
                )
        ...
        for (Permission p : ps.getPermissions()) {
          for (PermissionReference grantedTo : p.getGrantedToGroups()) {
            ...
            if (
              ...
              (
                !base.isTypeRequired() ||
                isPartOfDynamicPermissionGroup(grantedTo, qName, aspectQNames)
              )
            ) {
    ...
    private Set<PermissionReference> getUnconditionalRequirementsForPermissionGroup(
      PermissionGroup target,
      RequiredPermission.On on
    ) {
      HashSet<PermissionReference> requiredPermissions = new HashSet<PermissionReference>(
      ...
      for (PermissionSet ps : permissionSets.values()) {
        for (PermissionGroup pg : ps.getPermissionGroups()) {
          ...
          if (
            ...
            for (PermissionReference pr : pg.getIncludedPermissionGroups()) {
              requiredPermissions.addAll(
                getUnconditionalRequirementsForPermissionGroup(
                  getBasePermissionGroupOrNull(getPermissionGroupOrNull(pr)),
                  on
                )
              );
     ...
     */
    private boolean isPartOfDynamicPermissionGroup(
      PermissionReference pr,
      QName typeQname,
      Set<QName> aspects
    ) {
    ...
    private void buildUniquePermissionMap() {
      ...
      permissionGroupMap =
        new HashMap<PermissionReference, PermissionGroup>(128);
      permissionMap = new HashMap<PermissionReference, Permission>(64);
      for (PermissionSet ps : permissionSets.values()) {
        for (PermissionGroup pg : ps.getPermissionGroups()) {
          permissionGroupMap.put(
            SimplePermissionReference.getPermissionReference(
              pg.getQName(),
              pg.getName()
            ),
      ...
      for (PermissionSet ps : permissionSets.values()) {
        for (PermissionGroup pg : ps.getPermissionGroups()) {
          if (
            uniqueMap.containsKey(pg.getName()) &&
            !excluded.contains(pg.getName())
          ) {
        ...
        for (Permission p : ps.getPermissions()) {
          if (
            uniqueMap.containsKey(p.getName()) &&
            !excluded.contains(p.getName())
          ) {
  ...
  public void addPermissionModel(String model) {
    ...
    try {
      ...
      if (defaultPermissionAttribute != null) {
        if (
          defaultPermissionAttribute.getStringValue().equalsIgnoreCase(ALLOW)
        ) {
          mutableState.defaultPermission = AccessStatus.ALLOWED;
        } else if (
          defaultPermissionAttribute.getStringValue().equalsIgnoreCase(DENY)
        ) {
          mutableState.defaultPermission = AccessStatus.DENIED;
        } else {
          throw new PermissionModelException(
            "The default permission must be deny or allow"
          );
        }
      } else {
        mutableState.defaultPermission = AccessStatus.DENIED;
      ...
      for (
        Iterator<Element> nsit = root.elementIterator(NAMESPACES);
        nsit.hasNext();
        /**/
      ) {
        Element namespacesElement = (Element) nsit.next();
        for (
          Iterator<Element> it = namespacesElement.elementIterator(NAMESPACE);
          it.hasNext();
          /**/
        ) {
          Element nameSpaceElement = (Element) it.next();
          nspr.registerNamespace(
            nameSpaceElement.attributeValue(NAMESPACE_PREFIX),
            nameSpaceElement.attributeValue(NAMESPACE_URI)
          );
        }
      }
      ...
      for (
        Iterator<Element> psit = root.elementIterator(PERMISSION_SET);
        psit.hasNext();
        /**/
      ) {
        ...
        mutableState.permissionSets.put(
          permissionSet.getQName(),
          permissionSet
        );
      }
      ...
      for (
        Iterator<Element> npit = root.elementIterator(GLOBAL_PERMISSION);
        npit.hasNext();
        /**/
      ) {
  ...
  private Document createDocument(String model) {
    InputStream is =
      this.getClass().getClassLoader().getResourceAsStream(model);
    ...
    try {
      if (validate) {
        if (dtdSchemaUrl != null) {
          ...
          throw new PermissionModelException(
            "Couldn't obtain DTD schema to validate permission model."
          );
        }
      }
      ...
      throw new PermissionModelException(
        "Failed to create permission model document: " + model,
        e
      );
    } catch (IOException e) {
      throw new PermissionModelException(
        "Failed to close permission model document: " + model,
        e
      );
    }
  ...
  private InputStream processModelDocType(InputStream is, String dtdSchemaUrl)
    ...
    if (docType != null) {
      ...
      // add the DOCTYPE
      docType =
        new DefaultDocumentType(doc.getRootElement().getName(), dtdSchemaUrl);
  ...
  public Set<? extends PermissionEntry> getGlobalPermissionEntries() {
    ...
    try {
      return Collections.unmodifiableSet(
        new HashSet<GlobalPermissionEntry>(mutableState.globalPermissions)
      );
    } finally {
      mutableState.lock.readLock().unlock();
    }
  ...
  public Map<QName, PermissionSet> getPermissionSets() {
    ...
    try {
      return Collections.unmodifiableMap(
        new HashMap<QName, PermissionSet>(mutableState.permissionSets)
      );
    } finally {
      mutableState.lock.readLock().unlock();
    }
  ...
  private Set<PermissionReference> getAllPermissionsImpl(
    ...
    try {
      permissions.addAll(
        mutableState.getAllPermissionsImpl(typeName, exposedOnly)
      );
  ...
  public Set<PermissionReference> getGrantingPermissions(
    PermissionReference permissionReference
  ) {
    if (permissionReference == null) {
      return Collections.<PermissionReference>emptySet();
    ...
    mutableState.lock.readLock().lock();
    // Cache the results
    Set<PermissionReference> granters = mutableState.grantingPermissions.get(
      permissionReference
    );
    if (granters == null) {
      mutableState.lock.readLock().unlock();
      ...
      try {
        ...
        if (granters == null) {
          Set<PermissionReference> internal = mutableState.getGrantingPermissionsImpl(
            permissionReference
          );
  ...
  static RequiredKey generateKey(
    PermissionReference required,
    QName qName,
    Set<QName> aspectQNames,
    RequiredPermission.On on
  ...
  public static class RequiredKey {
     ...
     * @return the key
     */
    public static RequiredKey getRequiredKey(
      PermissionReference required,
      QName qName,
      Set<QName> aspectQNames,
      RequiredPermission.On on
      ...
      lock.readLock().lock();
      try {
        Map<QName, Map<Set<QName>, EnumMap<RequiredPermission.On, RequiredKey>>> byPermRef = instances.get(
          required
        );
        if (byPermRef != null) {
          Map<Set<QName>, EnumMap<RequiredPermission.On, RequiredKey>> byType = byPermRef.get(
            qName
          );
          if (byType != null) {
            EnumMap<RequiredPermission.On, RequiredKey> byAspects = byType.get(
              aspectQNames
            );
            if (byAspects != null) {
      ...
      try {
        ...
        if (byPermRef == null) {
          byPermRef =
            new HashMap<QName, Map<Set<QName>, EnumMap<RequiredPermission.On, RequiredKey>>>();
        ...
        if (byType == null) {
          byType =
            new HashMap<Set<QName>, EnumMap<RequiredPermission.On, RequiredKey>>();
        ...
        if (byAspects == null) {
          byAspects =
            new EnumMap<RequiredPermission.On, RequiredKey>(
              RequiredPermission.On.class
            );
    ...
    public int hashCode() {
      if (hashCode == 0) {
        ...
        result =
          PRIME *
          result +
          ((aspectQNames == null) ? 0 : aspectQNames.hashCode());
        result = PRIME * result + ((on == null) ? 0 : on.ordinal());
        result = PRIME * result + ((qName == null) ? 0 : qName.hashCode());
        result =
          PRIME * result + ((required == null) ? 0 : required.hashCode());
        hashCode = result;
      }
      return hashCode;
    }
    ...
    public boolean equals(Object obj) {
      ...
      if (on == null) {
        if (other.on != null) return false;
      } else if (!on.equals(other.on)) return false;

      if (aspectQNames == null) {
        if (other.aspectQNames != null) return false;
      } else if (!aspectQNames.equals(other.aspectQNames)) return false;
   ...
   */
  public Set<PermissionReference> getGranteePermissions(
    PermissionReference permissionReference
  ) {
    mutableState.lock.readLock().lock();
    try {
   ...
   */
  public Set<PermissionReference> getImmediateGranteePermissions(
    PermissionReference permissionReference
  ) {
    mutableState.lock.readLock().lock();
    try {
   ...
   */
  public Set<PermissionReference> getRequiredPermissions(
    PermissionReference required,
    QName qName,
    Set<QName> aspectQNames,
    On on
    ...
    mutableState.lock.readLock().lock();
    try {
      return mutableState.getRequiredPermissions(
        required,
        qName,
        aspectQNames,
        on
   ...
   */
  @Override
  public Set<PermissionReference> getUnconditionalRequiredPermissions(
    PermissionReference required,
    On on
  ) {
    mutableState.lock.readLock().lock();

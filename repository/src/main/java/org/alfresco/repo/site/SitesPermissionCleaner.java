...
public class SitesPermissionCleaner {
  ...
  public void cleanSitePermissions(
    ...
    final Long targetNodeID = nodeDAO
      .getNodePair(tenantService.getName(targetNode))
      .getFirst();
    ...
    if (targetNodeAcl.getAclType() == ACLType.DEFINING) {
      AccessControlList targetNodeAccessControlList = aclDAO.getAccessControlList(
        targetNodeAclID
      );
      ...
      for (AccessControlEntry entry : targetNodeAclEntries) {
        ...
        String thisSiteGroupPrefix = siteServiceImpl.getSiteGroup(
          containingSite.getShortName(),
          true
        );
    ...
    // Recurse
    List<NodeIdAndAclId> childNodeIds = nodeDAO.getPrimaryChildrenAcls(
      targetNodeID
    );
    for (NodeIdAndAclId nextChild : childNodeIds) {
      cleanSitePermissions(
        nodeDAO.getNodePair(nextChild.getId()).getSecond(),
        containingSite
      );
    }
  }

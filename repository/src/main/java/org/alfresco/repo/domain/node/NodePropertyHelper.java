...
public class NodePropertyHelper {
  ...
  public Map<NodePropertyKey, NodePropertyValue> convertToPersistentProperties(
    ...
    for (Map.Entry<QName, Serializable> entry : in.entrySet()) {
      ...
      Long propertyQNameId = qnameDAO
        .getOrCreateQName(propertyQName)
        .getFirst();
  ...
  private void addValueToPersistedProperties(
    ...
    if (
      ...
      collectionIndex == IDX_NO_COLLECTION && value instanceof Collection<?>
    ) {
      ...
      if (collectionValues.size() == 0) {
        NodePropertyValue npValue = makeNodePropertyValue(
          null,
      ...
      for (Object collectionValueObj : collectionValues) {
        collectionIndex++;
        if (
          collectionValueObj != null &&
          !(collectionValueObj instanceof Serializable)
        ) {
      ...
      if (value instanceof MLText) {
        ...
        for (Map.Entry<Locale, String> mlTextEntry : mlTextValue.entrySet()) {
          ...
          Long mlTextLocaleId = localeDAO
            .getOrCreateLocalePair(mlTextLocale)
            .getFirst();
          // This is persisted against the current locale, but as a d:text instance
          // This is persisted against the current locale, but as a d:text instance
          NodePropertyValue npValue = new NodePropertyValue(
  ...
  public Serializable getPublicProperty(
    ...
    if (scratch.size() > 0) {
      ...
      Serializable collapsedValue = collapsePropertiesWithSameQName(
        propertyDef,
        scratch
      );
  ...
  public Map<QName, Serializable> convertToPublicProperties(
    ...
    Iterator<Map.Entry<NodePropertyKey, NodePropertyValue>> iterator = sortedPropertyValues
      .entrySet()
      .iterator();
    while (true) {
      ...
      if (
        ...
        PropertyDefinition currentPropertyDef = dictionaryService.getProperty(
          currentQName
        );
        ...
        if (scratch.size() == 1) {
          ...
          collapsedValue =
            collapsePropertiesWithSameQName(currentPropertyDef, scratch);
        }
  ...
  private Serializable collapsePropertiesWithSameQName(
    ...
    Map<NodePropertyKey, NodePropertyValue> scratch = new HashMap<NodePropertyKey, NodePropertyValue>(
      3
    );
  ...
  private Serializable collapsePropertiesWithSameQNameAndListIndex(
    ...
    Long defaultLocaleId = (defaultLocalePair == null)
      ? null
      : defaultLocalePair.getFirst();
    ...
    for (Map.Entry<NodePropertyKey, NodePropertyValue> entry : propertyValues.entrySet()) {
      ...
      if (listIndex == null) {
        ...
        throw new IllegalStateException(
          "Expecting to collapse properties with same list index: " +
          propertyValues
        );
      }
      ...
      boolean isDefaultLocale = EqualsHelper.nullSafeEquals(
        defaultLocaleId,
        localeId
      );

      // Get the local entry value
      Serializable entryValue = makeSerializableValue(
        propertyDef,
        propertyValue
      );
  ...
  public Serializable makeSerializableValue(
    ...
    try {
      ...
      if (value instanceof ContentDataId) {
        ...
        ContentData contentData = contentDataDAO
          .getContentData(contentDataId)
          .getSecond();

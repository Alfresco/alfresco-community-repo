...
public class PersonServiceTest extends BaseWebScriptTest {
    ...
    @Mock private ResultSet mockSearchServiceQueryResultSet;
    ...
    protected void tearDown() throws Exception {
        ...
        // Should be safe not to do the following as we don't have a search service, but it is
        // cleaner to remove the mock.
    ...
    private JSONObject updatePerson(
        ...
        Response response =
                sendRequest(
                        new PutRequest(
                                URL_PEOPLE + "/" + userName, person.toString(), "application/json"),
                        expectedStatus);
    ...
    private JSONObject deletePerson(String userName, int expectedStatus) throws Exception {
        ...
        Response response =
                sendRequest(new DeleteRequest(URL_PEOPLE + "/" + userName), expectedStatus);
    ...
    public void testGetPeopleSkipCount() throws Exception {
        ...
        // Ensure that the REST call with no filter will always be routed to a DB canned query
        // rather than a FTS
    ...
    private void checkPaging(int totalItems, int skipCount, int maxItems, JSONObject response)
            throws Exception {
        ...
        assertEquals(
                "skipCount was not correct in the response", skipCount, paging.getInt("skipCount"));
        assertEquals(
                "maxItems was not correct in the response", maxItems, paging.getInt("maxItems"));
    }
        ...
        for (int i = 0; i < peopleAsc.length() - 1; i++) {
            if (SORT_BY_USERNAME.equals(sortBy)) {
                ...
                assertTrue(
                        ...
                        (jobUser1 == null ? "" : jobUser1)
                                        .compareToIgnoreCase(jobUser2 == null ? "" : jobUser2)
                                <= 0);
                ...
                assertTrue(
                        ...
                        (emailUser1 == null ? "" : emailUser1)
                                        .compareToIgnoreCase(emailUser2 == null ? "" : emailUser2)
                                <= 0);
    ...
    private void assertSearchQuery(String term, boolean buildFilter) {
        ...
        String expectedQuery =
                "TYPE:\"{http://www.alfresco.org/model/content/1.0}person\" AND (" + term + ")";
        ArgumentCaptor<SearchParameters> searchParametersCaptor =
    ...
    private void addUserUsageContent(final String userName, final int stringDataLength) {
        RetryingTransactionHelper.RetryingTransactionCallback<Void> usageCallback =

...
public class DownloadsCleanupJob implements Job {
  ...
  private static final String CLEAN_All_SYS_DOWNLOAD_FOLDERS =
    "cleanAllSysDownloadFolders";
  ...
  public void execute(JobExecutionContext context)
    ...
    final DownloadService downloadService = (DownloadService) jobData.get(
      KEY_DOWNLOAD_SERVICE
    );
    final TenantAdminService tenantAdminService = (TenantAdminService) jobData.get(
      KEY_TENANT_ADMIN_SERVICE
    );
    final int maxAgeInMinutes = Integer.parseInt(
      (String) jobData.get(KEY_MAX_AGE)
    );
    final int batchSize = Integer.parseInt((String) jobData.get(BATCH_SIZE));
    final boolean cleanAllSysDownloadFolders = Boolean.parseBoolean(
      (String) jobData.get(CLEAN_All_SYS_DOWNLOAD_FOLDERS)
    );

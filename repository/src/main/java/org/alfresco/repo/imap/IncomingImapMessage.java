...
/**
 * This class is used to serve incoming IMAP message. E.g. when message is copied /moved into some
 * IMAP older.
 *
...
public class IncomingImapMessage extends AbstractMimeMessage {
     ...
     */
    public IncomingImapMessage(
            FileInfo fileInfo, ServiceRegistry serviceRegistry, MimeMessage message)
            throws MessagingException {
        super(Session.getDefaultInstance(new Properties()));
        this.wrappedMessage =
                message; // temporary save it and then destroyed in writeContent() (to avoid memory
        // leak with byte[] MimeMessage.content field)
    ...
    public void buildMessageInternal() throws MessagingException {
        ...
        nodeService.addAspect(
                this.messageFileInfo.getNodeRef(), ImapModel.ASPECT_IMAP_CONTENT, null);
    ...
    private void writeContent() throws MessagingException {
        ContentWriter writer =
                serviceRegistry
                        .getContentService()
                        .getWriter(messageFileInfo.getNodeRef(), ContentModel.PROP_CONTENT, true);
        ...
        try {
            ...
            outputStream.close();
            wrappedMessage =
                    null; // it is not used any more and it is available to GC (to avoid memory leak
            // with byte[] MimeMessage.content field)
            this.contentReader =
                    serviceRegistry
                            .getContentService()
                            .getReader(messageFileInfo.getNodeRef(), ContentModel.PROP_CONTENT);

...
/**
 * Task related Rest api tests using http client to communicate with the rest apis in the
 * repository.
 *
...
public class TaskWorkflowApiTest extends EnterpriseWorkflowTestApi {
    ...
    public void testGetTaskById() throws Exception {
        ...
        String otherPerson =
                getOtherPersonInNetwork(
                                requestContext.getRunAsUser(), requestContext.getNetworkId())
                        .getId();
        RequestContext otherContext =
                new RequestContext(requestContext.getNetworkId(), otherPerson);
        ...
        ProcessInstance processInstance =
                startAdhocProcess(
                        requestContext.getRunAsUser(), requestContext.getNetworkId(), null);
        ...
        try {
            ...
            assertEquals(
                    ...
                    taskJSONObject.get("processDefinitionId"));
            ...
            activitiProcessEngine
                    .getTaskService()
                    .setAssignee(task.getId(), requestContext.getRunAsUser());
            activitiProcessEngine
                    .getTaskService()
                    .setOwner(task.getId(), requestContext.getRunAsUser());
            activitiProcessEngine
                    .getTaskService()
                    .delegateTask(task.getId(), otherContext.getRunAsUser());
            taskJSONObject = tasksClient.findTaskById(task.getId());
            assertNotNull(taskJSONObject);
            assertEquals(task.getId(), taskJSONObject.get("id"));
            ...
            TenantUtil.runAsUserTenant(
                    ...
                    requestContext.getRunAsUser(),
                    requestContext.getNetworkId());
            taskJSONObject = tasksClient.findTaskById(task.getId());
            assertNotNull(taskJSONObject);
            assertEquals(task.getId(), taskJSONObject.get("id"));
            assertEquals(requestContext.getRunAsUser(), taskJSONObject.get("assignee"));
    ...
    public void testGetTaskByIdAuthorization() throws Exception {
        ...
        // Start process by one user and try to access the task as the task assignee instead of the
        // process
        // initiator to see if the assignee is authorized to get the task
        ProcessInstance processInstance =
                startAdhocProcess(initiator, requestContext.getNetworkId(), null);
        try {
            Task task =
                    activitiProcessEngine
                            .getTaskService()
            ...
            try {
                ...
                assertEquals(
                        HttpStatus.FORBIDDEN.value(), expected.getHttpResponse().getStatusCode());
            ...
            String tenantAdmin =
                    AuthenticationUtil.getAdminUserName() + "@" + requestContext.getNetworkId();
            publicApiClient.setRequestContext(
                    new RequestContext(TenantUtil.DEFAULT_TENANT, tenantAdmin));
    ...
    public void testGetUnexistingTaskById() throws Exception {
        ...
        try {
            ...
            assertErrorSummary(
                    "The entity with id: unexisting was not found", expected.getHttpResponse());
        }
    }
    ...
    public void testUpdateTask() throws Exception {
        ...
        RequestContext adminContext =
                new RequestContext(requestContext.getNetworkId(), tenantAdmin);
    ...
    public void testUpdateTaskMnt13276() throws Exception {
        ...
        // startReviewPooledProcess() uses initiator's site id and role name for construct
        // bpm_groupAssignee, thus we need appropriate things for created users
        transactionHelper.doInTransaction(
                        ...
                        TenantUtil.runAsUserTenant(
                                new TenantRunAsWork<Void>() {
                                    ...
                                    public Void doWork() throws Exception {
                                        ...
                                        initiatorSite.inviteToSite(
                                                persons.get(0).getId(), memberOfSite.getRole());
                                        initiatorSite.inviteToSite(
                                                persons.get(1).getId(), memberOfSite.getRole());
                                        // this user wouldn't be in group
                                        initiatorSite.inviteToSite(
                                                persons.get(2).getId(),
                                                SiteRole.SiteConsumer == memberOfSite.getRole()
    ...
    public void testAssociationDefinitionExtraction() throws Exception {
        ...
        // The "wbpm:delegatee" custom association is defined in bpmDelegateeModel.xml. This
        // association has "cm:person" target type.
        ...
        // Get person
        final ActivitiScriptNode person =
                TenantUtil.runAsUserTenant(
                        new TenantRunAsWork<ActivitiScriptNode>() {
                            @Override
        ...
        ProcessInstance processInstance =
                TenantUtil.runAsUserTenant(
                        new TenantRunAsWork<ProcessInstance>() {
                            @Override
                            public ProcessInstance doWork() throws Exception {
                                ...
                                return activitiProcessEngine
                                        ...
                                        .startProcessInstanceByKey(
                                                processDefinitionKey, "myProcessKey", variables);
                            }
                        },
                        user,
                        networkId);
        ...
        try {
            ...
            // Define the "wbpm_delegatee" variable for the taskDefine delegatee user name. POST
            ...
            try {
                ...
                JSONObject response =
                        tasksClient.createTaskVariables(activeTask.getId(), variablesArray);
                ...
                response =
                        publicApiClient
                                .processesClient()
                                .getProcessvariables(processInstance.getId());
                assertNotNull(response);
                assertTrue(
                        delegatee + " variable was not set",
                        response.toJSONString().contains(delegatee));
                ...
                for (Object entry : entries) {
                    ...
                    if (variable.get("name").equals(delegatee)) {
                        ...
                        // It means that assoc type def was retrieved successfully from the
                        // dictionary.
    ...
    public void testUpdateTaskInvalidProperty() throws Exception {
        ...
        try {
            ...
            try {
                ...
                assertEquals(
                        HttpStatus.BAD_REQUEST.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "The property selected for update does not exist for this resource:"
                                + " unexisting",
            ...
            try {
                ...
                assertEquals(
                        HttpStatus.BAD_REQUEST.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "The property selected for update is read-only: id",
                        expected.getHttpResponse());
            }
        } finally {
            cleanupProcessInstance(processInstance);
    ...
    public void testClaimTask() throws Exception {
        ...
        try {
            ...
            List<MemberOfSite> memberships =
                    getTestFixture()
                            .getNetwork(requestContext.getNetworkId())
                            .getSiteMemberships(requestContext.getRunAsUser());
            assertTrue(memberships.size() > 0);
            MemberOfSite memberOfSite = memberships.get(0);
            String group =
                    "GROUP_site_" + memberOfSite.getSiteId() + "_" + memberOfSite.getRole().name();
            activitiProcessEngine.getTaskService().addCandidateGroup(task.getId(), group);

            // Claiming the task when part of candidate-group but another person has this task
            // assigned results in conflict
            try {
                tasksClient.updateTask(task.getId(), taskBody, selectedFields);
                fail("Exception expected");
            } catch (PublicApiException expected) {
                assertEquals(
                        HttpStatus.CONFLICT.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "The task is already claimed by another user.", expected.getHttpResponse());
            }

            // Claiming the task when part of candidate-group and NO assignee is currenlty set
            // should work
            activitiProcessEngine.getTaskService().setAssignee(task.getId(), null);
            taskBody = new JSONObject();
            taskBody.put("state", "claimed");
            ...
            activitiProcessEngine
                    .getTaskService()
                    .deleteGroupIdentityLink(task.getId(), group, IdentityLinkType.CANDIDATE);
            activitiProcessEngine
                    .getTaskService()
                    .addCandidateUser(task.getId(), requestContext.getRunAsUser());
            result = tasksClient.updateTask(task.getId(), taskBody, selectedFields);
            assertNotNull(result);
            assertEquals(requestContext.getRunAsUser(), result.get("assignee"));
            assertEquals(
    ...
    public void testCompleteTask() throws Exception {
        ...
        ProcessInstance processCompleteAsAssignee =
                startAdhocProcess(initiator, requestContext.getNetworkId(), null);
        ProcessInstance processCompleteAsOwner =
                startAdhocProcess(initiator, requestContext.getNetworkId(), null);
        ProcessInstance processCompleteAsInitiator =
                startAdhocProcess(initiator, requestContext.getNetworkId(), null);
        ProcessInstance processCompleteAsAdmin =
                startAdhocProcess(initiator, requestContext.getNetworkId(), null);
        ProcessInstance processCompleteWithVariables =
                startAdhocProcess(initiator, requestContext.getNetworkId(), null);
        try {
            Task asAssigneeTask =
                    activitiProcessEngine
                            .getTaskService()
                            .createTaskQuery()
                            .processInstanceId(processCompleteAsAssignee.getId())
                            .singleResult();
            Task asOwnerTask =
                    activitiProcessEngine
                            .getTaskService()
                            .createTaskQuery()
                            .processInstanceId(processCompleteAsOwner.getId())
                            .singleResult();
            Task asInitiatorTask =
                    activitiProcessEngine
                            .getTaskService()
                            .createTaskQuery()
                            .processInstanceId(processCompleteAsInitiator.getId())
                            .singleResult();
            Task asAdminTask =
                    activitiProcessEngine
                            .getTaskService()
                            .createTaskQuery()
                            .processInstanceId(processCompleteAsAdmin.getId())
                            .singleResult();
            Task withVariablesTask =
                    activitiProcessEngine
                            .getTaskService()
                            .createTaskQuery()
                            .processInstanceId(processCompleteWithVariables.getId())
                            .singleResult();
            TasksClient tasksClient = publicApiClient.tasksClient();
            ...
            assertNotNull(
                    "The outcome should not be null for completed task.",
            ...
            cleanupProcessInstance(
                    processCompleteAsAssignee,
                    processCompleteAsAdmin,
                    processCompleteAsInitiator,
                    processCompleteAsOwner,
                    processCompleteWithVariables);
        }
    }
        ...
        try {
            ...
            try {
                ...
                assertEquals(
                        HttpStatus.BAD_REQUEST.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "When delegating a task, assignee should be selected and provided in the"
                                + " request.",
                        expected.getHttpResponse());
            }
    ...
    public void testSetOutcome() throws Exception {
        ...
        Task task =
                activitiProcessEngine
                        .getTaskService()
                        .createTaskQuery()
                        .processInstanceId(processInf.getId())
                        .singleResult();
        TasksClient tasksClient = publicApiClient.tasksClient();
        activitiProcessEngine.getTaskService().saveTask(task);
        ...
        HttpResponse response =
                tasksClient.update(
                        ...
                        null,
                        null,
                        "{\"state\":\"completed\",\"variables\":[{\"name\":\"wf_reviewOutcome\",\"value\":\"Approve\",\"scope\":\"local\"},{\"name\":\"bpm_comment\",\"value\":\"approved"
                            + " by me\",\"scope\":\"local\"}]}",
                        params,
        ...
        try {
            ...
            HistoricTaskInstance historyTask =
                    activitiProcessEngine
                            ...
                            .singleResult();
            assertNotNull(
                    "The outcome should not be null for resolved task.",
                    historyTask.getTaskLocalVariables().get("bpm_outcome"));
    ...
    public void testGetTasks() throws Exception {
        ...
        try {
            ...
            // experiment
            assertEquals(
                    createdCal.getTime().toString(),
                    parseDate(taskJSONObject, "startedAt").toString());
            assertEquals(createdCal.getTime(), parseDate(taskJSONObject, "startedAt"));
            assertEquals(2l, taskJSONObject.get("priority"));
            assertEquals("wf:adhocTask", taskJSONObject.get("formResourceKey"));
            assertNull(taskJSONObject.get("endedAt"));
    ...
    public void testGetTasksWithParams() throws Exception {
        ...
        ProcessInstance processInstance =
                startAdhocProcess(
                        requestContext.getRunAsUser(), requestContext.getNetworkId(), businessKey);
        ...
        try {
            ...
            final Task reviewPooledTask =
                    activitiProcessEngine
                            .getTaskService()
                            .createTaskQuery()
                            .processInstanceId(otherInstance.getId())
                            .singleResult();

            assertNotNull(reviewPooledTask);
            // MNT-12221 case, due date should be set as task property, not as variable!!!
            assertNotNull(
                    "Due date was not set for review pooled task.", reviewPooledTask.getDueDate());
            ...
            activitiProcessEngine
                    .getTaskService()
                    .addCandidateUser(activeTask.getId(), anotherUserId);
            activitiProcessEngine.getTaskService().addCandidateGroup(activeTask.getId(), "sales");
            activitiProcessEngine
                    .getTaskService()
                    .setVariableLocal(activeTask.getId(), "numberVar", 10);
            ...
            params.put(
                    "where",
                    "(status = 'active' AND processId = '" + processInstance.getId() + "')");
            ...
            params.put(
                    "where",
                    "(status = 'completed' AND processId = '" + processInstance.getId() + "')");
            ...
            params.clear();
            params.put(
                    "where", "(status = 'any' AND processId = '" + processInstance.getId() + "')");
            assertTasksPresentInTaskQuery(
                    params, tasksClient, false, activeTask.getId(), completedTask.getId());
            ...
            try {
                ...
                assertEquals(
                        HttpStatus.BAD_REQUEST.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "Invalid status parameter: alfrescorocks", expected.getHttpResponse());
            }
            ...
            params.put(
                    "where",
                    "(status = 'active' AND assignee = '" + requestContext.getRunAsUser() + "')");
            ...
            params.put(
                    "where",
                    "(status = 'active' AND owner = '" + requestContext.getRunAsUser() + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, activeTask.getId());

            params.put(
                    "where",
                    "(status = 'completed' AND owner = '" + requestContext.getRunAsUser() + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());

            params.put(
                    "where",
                    "(status = 'any' AND owner = '" + requestContext.getRunAsUser() + "')");
            assertTasksPresentInTaskQuery(
                    params, tasksClient, activeTask.getId(), completedTask.getId());

            // Candidate user filtering, only available for active tasks. When used with
            // completed/any 400 is returned
            params.clear();
            ...
            params.put(
                    "where",
                    "(status = 'active' AND candidateUser = '"
                            + otherContext.getRunAsUser()
                            + "')");
            // No tasks expected since assignee is set
            ...
            params.clear();
            params.put(
                    "where", "(status = 'completed' AND candidateUser = '" + anotherUserId + "')");
            try {
                tasksClient.findTasks(params);
                fail("Exception expected");
            } catch (PublicApiException expected) {
                assertEquals(
                        HttpStatus.BAD_REQUEST.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "Filtering on candidateUser is only allowed in combination with"
            ...
            // Candidate group filtering, only available for active tasks. When used with
            // completed/any 400 is returned
            params.clear();
            params.put(
                    "where",
            ...
            params.put(
                    "where",
                    "(status = 'completed' AND candidateGroup = 'sales' AND processId='"
                            + processInstance.getId()
                            + "')");
            try {
                tasksClient.findTasks(params);
                ...
                assertEquals(
                        HttpStatus.BAD_REQUEST.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "Filtering on candidateGroup is only allowed in combination with"
                                + " status-parameter 'active'",
                        expected.getHttpResponse());
            }
            ...
            params.put(
                    "where",
                    "(status = 'any' AND candidateGroup = 'sales' AND processId='"
                            + processInstance.getId()
                            + "')");
            try {
                tasksClient.findTasks(params);
            ...
            params.put(
                    "where",
                    "(status = 'active' AND name = 'Task name' AND processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, activeTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'completed' AND name = 'Another task name' AND processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'any' AND name = 'Another task name' AND processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'active' AND description = 'This is a test description' AND"
                            + " processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, activeTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'completed' AND description = 'This is another test description' AND"
                            + " processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'any' AND description = 'This is another test description' AND"
                            + " processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'active' AND priority = 2 AND processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, activeTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'completed' AND priority = 3 AND processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'any' AND priority = 3 AND processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());
            ...
            params.clear();
            params.put(
                    "where", "(status = 'active' AND processBusinessKey = '" + businessKey + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, activeTask.getId());

            params.clear();
            params.put(
                    "where",
                    "(status = 'completed' AND processBusinessKey = '" + businessKey + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());

            params.clear();
            params.put("where", "(status = 'any' AND processBusinessKey = '" + businessKey + "')");
            assertTasksPresentInTaskQuery(
                    params, tasksClient, completedTask.getId(), activeTask.getId());

            params.clear();
            params.put(
                    "where",
                    "(status = 'any' AND processBusinessKey MATCHES('" + businessKey + "'))");
            assertTasksPresentInTaskQuery(
                    params, tasksClient, completedTask.getId(), activeTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'any' AND processBusinessKey MATCHES('"
            ...
            params.put(
                    "where",
                    "(status = 'active' AND activityDefinitionId = 'verifyTaskDone' AND processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, activeTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'completed' AND activityDefinitionId = 'adhocTask' AND processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'any' AND activityDefinitionId = 'adhocTask' AND processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'active' AND processDefinitionId = '"
                            + processInstance.getProcessDefinitionId()
                            + "' AND processId='"
                            + processInstance.getId()
                            + "')");
            ...
            params.put(
                    "where",
                    "(status = 'completed' AND processDefinitionId = '"
                            + processInstance.getProcessDefinitionId()
                            + "' AND processId='"
                            + processInstance.getId()
                            + "')");
            ...
            params.put(
                    "where",
                    "(status = 'any' AND processDefinitionId = '"
                            + processInstance.getProcessDefinitionId()
                            + "' AND processId='"
                            + processInstance.getId()
                            + "')");
            ...
            params.put(
                    "where",
                    "(status = 'active' AND processDefinitionName = 'Adhoc Activiti Process' AND"
                            + " processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, activeTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'completed' AND processDefinitionName = 'Adhoc Activiti Process' AND"
                            + " processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'any' AND processDefinitionName = 'Adhoc Activiti Process' AND"
                            + " processId='"
                            + processInstance.getId()
                            + "')");
            assertTasksPresentInTaskQuery(
            ...
            params.put(
                    "where",
                    "(status = 'active' AND dueAt = '"
                            + ISO8601DateFormat.format(activeTaskDue.getTime())
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, activeTask.getId());
            ...
            params.put(
                    "where",
                    "(status = 'active' AND startedAt > '"
                            + ISO8601DateFormat.format(compareCal.getTime())
                            + "')");
            assertTasksPresentInTaskQuery(
                    params, tasksClient, activeTask.getId(), otherTask.getId());

            params.clear();
            params.put(
                    "where",
                    "(status = 'completed' AND startedAt > '"
                            + ISO8601DateFormat.format(compareCal.getTime())
                            + "')");
            assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());
            ...
            params.put(
                    "where",
                    "(variables/bpm_dueDate = 'd:datetime "
            ...
            params.clear();
            params.put(
                    "where", "(variables/bpm_dueDate = 'd:datetime 2013-09-15T12:22:31.866+0000')");
            assertEquals(0, getResultSizeForTaskQuery(params, tasksClient));

            params.clear();
            params.put(
                    "where", "(variables/bpm_dueDate > 'd:datetime 2013-09-15T12:22:31.866+0000')");
            // MNT-12221 fix, due date is not saved as variable for review pooled workflow, so
            ...
            params.put(
                    "where",
                    "(dueAt = '" + ISO8601DateFormat.format(reviewPooledTask.getDueDate()) + "')");
            ...
            params.put(
                    "where",
                    "(status = 'completed' AND processDefinitionName = 'Adhoc Activiti Process')");
            JSONObject response =
                    publicApiClient.processesClient().getTasks(processInstance.getId(), params);
            assertNotNull(response);
    ...
    public void testGetTasksWithPaging() throws Exception {
        ...
        try {
            ...
            params.put(
                    ...
                    "(processDefinitionId = '"
                            + processDefinitionId
                            + "' AND includeProcessVariables = true AND includeTaskVariables ="
                            + " true)");
    ...
    public void testGetTasksWithPagingAndVariablesLimit() throws Exception {
        ...
        // system.workflow.engine.activiti.taskvariableslimit is set to 200 in
        // test-resources/alfresco-global.properties
        try {
            // MNT-16040: Create tasks with a number of variables just below the taskvariableslimit
            // and test that skipCount is working as expected.
            ...
            for (int i = 0; i < numberOfTasks; i++) {
                Task task =
                        taskService
                                .createTaskQuery()
                                .processInstanceId(startedProcesses.get(i).getProcessInstanceId())
                                .singleResult();
                taskService.setPriority(task.getId(), (i + 1) * 10);

                // Add an extra variable to the task, there are other 12 added, so a total of 13
    ...
    public void testGetTasksWithSorting() throws Exception {
        ...
        try {
            ...
            activitiProcessEngine
                    .getTaskService()
                    .setPriority(taskList.get(numberOfTasks - 1).getId(), 1);
            ...
            params.put(
                    "where",
                    "(processDefinitionId = '"
                            + startedProcesses.get(0).getProcessDefinitionId()
                            + "')");
    ...
    public void testGetTaskCandidates() throws Exception {
        ...
        try {
            ...
            JSONArray candidateArrayJSON =
                    (JSONArray) ((JSONObject) taskCandidatesJSONObject.get("list")).get("entries");
            ...
            for (int i = 0; i < candidateArrayJSON.size(); i++) {
                JSONObject entry =
                        (JSONObject) ((JSONObject) candidateArrayJSON.get(i)).get("entry");
    ...
    public void testGetTaskVariablesAuthentication() throws Exception {
        ...
        // Start process by one user and try to access the task variables as the task assignee
    ...
    public void testGetTaskVariables() throws Exception {
        ...
        try {
            ...
            Map<String, Object> actualLocalVariables =
                    activitiProcessEngine.getTaskService().getVariablesLocal(task.getId());
            Map<String, Object> actualGlobalVariables =
                    activitiProcessEngine.getRuntimeService().getVariables(processInstance.getId());
            ...
            // Should contain one variable less than the actual variables in the engine,
            // tenant-domain var is filtered out
            assertEquals(
                    actualLocalVariables.size() + actualGlobalVariables.size() - 1, entries.size());
    ...
    public void testUpdateTaskVariablesPresentInModel() throws Exception {
        ...
        try {
            ...
            JSONObject resultEntry =
                    tasksClient.updateTaskVariable(
                            task.getId(), "bpm_percentComplete", variableBody);
            ...
            assertEquals(
                    ...
                    activitiProcessEngine
                            .getRuntimeService()
                            .getVariable(processInstance.getId(), "bpm_percentComplete"));

            // Update a local value that is present in the model with name and scope, omitting type
            // to see if type is deducted from model
            variableBody = new JSONObject();
            variableBody.put("name", "bpm_percentComplete");
            ...
            assertEquals(
                    ...
                    activitiProcessEngine
                            .getTaskService()
                            .getVariable(task.getId(), "bpm_percentComplete"));
    ...
    public void testUpdateTaskVariablesExplicitTyped() throws Exception {
        ...
        try {
            ...
            assertEquals(
                    ...
                    activitiProcessEngine
                            .getRuntimeService()
                            .getVariable(processInstance.getId(), "newVariable"));
        } finally {
            cleanupProcessInstance(processInstance);
        }
    }
    ...
    public void testUpdateTaskVariablesNoExplicitType() throws Exception {
        ...
        try {
            ...
            assertEquals(
                    ...
                    activitiProcessEngine
                            .getTaskService()
                            .getVariable(task.getId(), "newVariable"));
        ...
        try {
            ...
            try {
                ...
                assertEquals(
                        HttpStatus.BAD_REQUEST.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "Variable scope is required and can only be 'local' or 'global'.",
                        expected.getHttpResponse());
            }
            ...
            try {
                ...
                assertEquals(
                        HttpStatus.BAD_REQUEST.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "Illegal value for variable scope: 'illegal'.", expected.getHttpResponse());
            }
            ...
            try {
                ...
                assertEquals(
                        HttpStatus.BAD_REQUEST.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "Unsupported type of variable: 'd:unexisting'.",
                        expected.getHttpResponse());
            }
            ...
            try {
                ...
                assertEquals(
                        HttpStatus.BAD_REQUEST.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "Unsupported type of variable: ' 12unexisting'.",
                        expected.getHttpResponse());
            }
        } finally {
            cleanupProcessInstance(processInstance);
        ...
        try {
            List<Task> tasks =
                    activitiProcessEngine
                            .getTaskService()
                            .createTaskQuery()
                            .processInstanceId(processRest.getId())
                            .list();
            assertNotNull(tasks);
            ...
            try {
                publicApiClient
                        .tasksClient()
                        .updateTaskVariable(taskId, "wf_requiredApprovePercent", variableJson);
                fail("Exception expected");
            ...
            assertEquals(
                    ...
                    activitiProcessEngine
                            .getRuntimeService()
                            .getVariable(processRest.getId(), "wf_requiredApprovePercent"));
            ...
            resultEntry =
                    publicApiClient
                            .tasksClient()
                            .updateTaskVariable(taskId, "testVariable", variableJson);
            assertNotNull(resultEntry);
            result = (JSONObject) resultEntry.get("entry");
            ...
            assertEquals(
                    "text",
                    activitiProcessEngine.getTaskService().getVariable(taskId, "testVariable"));
            ...
            resultEntry =
                    publicApiClient
                            .tasksClient()
                            .updateTaskVariable(taskId, "bpm_assignees", updateAssigneesJson);
            assertNotNull(resultEntry);
            ...
            try {
                publicApiClient
                        .tasksClient()
                        .updateTaskVariable(taskId, "bpm_assignees", updateAssigneeJson);
                fail("Exception expected");
            } catch (PublicApiException e) {
                assertEquals(HttpStatus.BAD_REQUEST.value(), e.getHttpResponse().getStatusCode());
            }
    ...
    public void testCreateTaskVariablesPresentInModel() throws Exception {
        ...
        try {
            ...
            JSONObject firstResultObject =
                    (JSONObject) ((JSONObject) resultList.get(0)).get("entry");
            ...
            JSONObject secondResultObject =
                    (JSONObject) ((JSONObject) resultList.get(1)).get("entry");
            ...
            assertEquals(
                    ...
                    activitiProcessEngine
                            .getTaskService()
                            .getVariable(task.getId(), "bpm_workflowPriority"));
        } finally {
            cleanupProcessInstance(processInstance);
        }
    }
    ...
    public void testGetTaskVariablesRawVariableTypes() throws Exception {
        ...
        try {
            ...
            JSONObject variables =
                    tasksClient.findTaskVariables(
                            task.getId(), Collections.singletonMap("where", "(scope = local)"));
            assertNotNull(variables);
            JSONObject list = (JSONObject) variables.get("list");
            assertNotNull(list);
            JSONArray entries = (JSONArray) list.get("entries");
            ...
            assertEquals(
                    actualLocalVariables.size(), ((Long) pagination.get("totalItems")).intValue());
            assertEquals(0L, pagination.get("skipCount"));
            assertFalse((Boolean) pagination.get("hasMoreItems"));
            ...
            for (int i = 0; i < entries.size(); i++) {
                ...
                entriesByName.put(
                        (String) ((JSONObject) var.get("entry")).get("name"),
    ...
    public void testGetTaskVariablesScoped() throws Exception {
        ...
        try {
            ...
            variables =
                    tasksClient.findTaskVariables(
                            task.getId(), Collections.singletonMap("where", "(scope = global)"));
            assertNotNull(variables);
    ...
    public void testDeleteTaskVariable() throws Exception {
        ...
        activitiProcessEngine
                .getRuntimeService()
                .setVariable(
                        processInstance.getId(), "overlappingVariable", "Value set in process");
        try {
            Task task =
                    activitiProcessEngine
                            .getTaskService()
            ...
            activitiProcessEngine
                    .getTaskService()
                    .setVariableLocal(task.getId(), "myVariable", "This is a variable");
            activitiProcessEngine
                    .getTaskService()
                    .setVariableLocal(task.getId(), "overlappingVariable", "Value set in task");
            ...
            assertFalse(
                    activitiProcessEngine
                            .getTaskService()
                            .hasVariableLocal(task.getId(), "myVariable"));

            // Delete a task-variable that has the same name as a global process-variable - which
            // should remain untouched after delete
            tasksClient.deleteTaskVariable(task.getId(), "overlappingVariable");
            assertFalse(
                    activitiProcessEngine
                            .getTaskService()
                            .hasVariableLocal(task.getId(), "overlappingVariable"));
            assertTrue(
                    activitiProcessEngine
                            .getRuntimeService()
                            .hasVariable(processInstance.getId(), "overlappingVariable"));
            assertEquals(
    ...
    public void testDeleteTaskVariableExceptions() throws Exception {
        ...
        try {
            ...
            try {
                ...
                assertEquals(
                        HttpStatus.NOT_FOUND.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "The entity with id: unexisting was not found", expected.getHttpResponse());
            }
            ...
            try {
                ...
                assertEquals(
                        HttpStatus.NOT_FOUND.value(), expected.getHttpResponse().getStatusCode());
                assertErrorSummary(
                        "The entity with id: unexistingVarName was not found",
                        expected.getHttpResponse());
            }
    ...
    public void testDeleteTaskVariableAuthentication() throws Exception {
        ...
        try {
            ...
            activitiProcessEngine
                    .getTaskService()
                    .setVariableLocal(task.getId(), "existingVariable", "Value");
            ...
            assertTrue(
                    activitiProcessEngine
                            .getTaskService()
                            .hasVariableLocal(task.getId(), "existingVariable"));
    ...
    protected void testModelEntries(Map<String, JSONObject> modelFieldsByName) {
        ...
        assertEquals(
                "{http://www.alfresco.org/model/bpm/1.0}package", modelEntry.get("qualifiedName"));
    ...
    public void testGetTaskItems() throws Exception {
        ...
        try {
            ...
            activitiProcessEngine
                    .getTaskService()
                    .addCandidateUser(task.getId(), otherContext.getRunAsUser());
            publicApiClient.setRequestContext(otherContext);
            itemsJSON = tasksClient.findTaskItems(task.getId());
            assertNotNull(itemsJSON);
            entriesJSON = (JSONArray) itemsJSON.get("entries");
            ...
            List<MemberOfSite> memberships =
                    getTestFixture()
                            .getNetwork(otherContext.getNetworkId())
            ...
            activitiProcessEngine
                    .getTaskService()
                    .deleteCandidateUser(task.getId(), otherContext.getRunAsUser());
            activitiProcessEngine.getTaskService().addCandidateGroup(task.getId(), group);
            publicApiClient.setRequestContext(otherContext);
            itemsJSON = tasksClient.findTaskItems(task.getId());
            assertNotNull(itemsJSON);
    ...
    protected ProcessInstance startAdhocProcess(
            final String user, final String networkId, final String businessKey) {
        return TenantUtil.runAsUserTenant(
                new TenantRunAsWork<ProcessInstance>() {
                    @Override
                    public ProcessInstance doWork() throws Exception {
    ...
    protected int getResultSizeForTaskQuery(Map<String, String> params, TasksClient tasksClient)
            throws Exception {
        JSONObject taskListJSONObject = tasksClient.findTasks(params);
        assertNotNull(taskListJSONObject);
        JSONArray jsonEntries = (JSONArray) taskListJSONObject.get("entries");
    ...
    protected void assertTasksPresentInTaskQuery(
            Map<String, String> params, TasksClient tasksClient, String... taskIds)
            throws Exception {
    ...
    protected void assertTasksPresentInTaskQuery(
        ...
        if (countShouldMatch) {
            assertEquals(
                    "Wrong amount of tasks returned by query", taskIds.length, jsonEntries.size());
        }
        ...
        assertEquals(
                "Not all tasks have been found in query response, missing: "
                        + StringUtils.join(tasksToFind, ","),
                0,
                tasksToFind.size());
    }

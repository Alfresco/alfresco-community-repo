...
public class CannedQueryDAOImpl extends AbstractCannedQueryDAOImpl {
   ...
   */
  private final String makeQueryName(
    final String sqlNamespace,
  ...
  public <R> R executeQueryUnique(
    ...
    try {
      ...
      throw new IllegalArgumentException(
        "Return type of query does not match expected type.",
        e
      );
    }
  }
  ...
  public <R> List<R> executeQuery(
    ...
    if (offset < 0 || offset == Integer.MAX_VALUE) {
      throw new IllegalArgumentException(
        "Query result offset must be zero or greater."
      );
    }

    if (limit <= 0) {
      throw new IllegalArgumentException(
        "Query results limit must be greater than zero."
      );
    }
  ...
  public <R> void executeQuery(
    ...
    String query = makeQueryName(sqlNamespace, queryName);
    ResultHandlerTranslator<R> resultHandler = new ResultHandlerTranslator<R>(
      handler
    );
    try {
      if ((offset == 0) && (limit == Integer.MAX_VALUE)) {

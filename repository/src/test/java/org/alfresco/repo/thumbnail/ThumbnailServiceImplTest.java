 ...
 * @deprecated The thumbnails code is being moved out of the codebase and replaced by the new async
...
public class ThumbnailServiceImplTest extends BaseAlfrescoSpringTest {
    ...
    /**
     * A test Thumbnail that is not know to the new RenditionService2, so is processed the very old
     * way.
     */
    ...
    public void before() throws Exception {
        ...
        this.renditionService =
                (RenditionService) this.applicationContext.getBean("RenditionService");
        this.thumbnailService =
                (ThumbnailService) this.applicationContext.getBean("ThumbnailService");
        this.scriptThumbnailService =
                (ScriptThumbnailService) this.applicationContext.getBean("thumbnailServiceScript");
        ...
        converter =
                (TransformationOptionsConverter)
                        applicationContext.getBean("transformOptionsConverter");

        // Create a folder and some content
        Map<QName, Serializable> folderProps = new HashMap<QName, Serializable>(1);
        folderProps.put(ContentModel.PROP_NAME, "testFolder");
        this.folder =
                this.secureNodeService
                        .createNode(
                                this.rootNodeRef,
                                ContentModel.ASSOC_CHILDREN,
                                QName.createQName(
                                        NamespaceService.CONTENT_MODEL_1_0_URI, "testFolder"),
    ...
    public static void createTestThumbnail(ThumbnailRegistry thumbnailRegistry) {
        ...
        if (thumbnailRegistry.getThumbnailDefinition(TEST_THUMBNAIL) == null) {
            ...
            ThumbnailDefinition testThumbnailDefinition =
                    new ThumbnailDefinition(
                            doclib.getMimetype(),
                            doclib.getTransformationOptions(),
                            TEST_THUMBNAIL);
            ...
            testThumbnailDefinition.setMimeAwarePlaceHolderResourcePath(
                    doclib.getMimeAwarePlaceHolderResourcePath());
    ...
    private void checkTransformer() {
        if (!synchronousTransformClient.isSupported(
                ...
                null)) {
            fail(
                    "Image transformer is not working.  Please check your image conversion command"
                            + " setup.");
        }
    }
    ...
    public void testCreateRenditionThumbnailFromImage() throws Exception {
        ...
        ThumbnailDefinition details =
                thumbnailService
                        ...
                        .getThumbnailDefinition(qname.getLocalName());
        ...
        assertEquals(
                ...
                details.getPlaceHolderResourcePath());
        ...
        checkRenditioned(
                ...
                Collections.singletonList(
                        new ExpectedAssoc(RegexQNamePattern.MATCH_ALL, "doclib", 1)));
    ...
    public void testCreateRenditionThumbnailFromPdfPage2() throws Exception {
        ...
        ThumbnailDefinition thumbnailDefinition =
                new ThumbnailDefinition(MimetypeMap.MIMETYPE_PDF, options, "doclib_2");
        ...
        checkRenditioned(
                pdfOrig,
                Collections.singletonList(
                        new ExpectedAssoc(RegexQNamePattern.MATCH_ALL, "doclib_2", 1)));
        ...
        assertTrue(
                "Page 2 should be blank and less than 4500 bytes. It was "
                        + size
                        + " bytes. tempFile="
                        + tempFile.getPath(),
                size < 4500);
    ...
    public void testCreateThumbnailFromImage() throws Exception {
        ...
        checkRenditioned(
                jpgOrig,
                Collections.singletonList(
                        new ExpectedAssoc(RegexQNamePattern.MATCH_ALL, "small", 1)));
        ...
        checkRenditioned(
                jpgOrig,
                Collections.singletonList(
                        new ExpectedAssoc(RegexQNamePattern.MATCH_ALL, "small2", 1)));
        checkRendition("small2", thumbnail2);
        outputThumbnailTempContentLocation(
                thumbnail2, "jpg", "small2 - 64x64, aspect not maintained");
        ...
        checkRenditioned(
                jpgOrig,
                Collections.singletonList(
                        new ExpectedAssoc(RegexQNamePattern.MATCH_ALL, "half", 1)));
        ...
        checkRenditioned(
                gifOrig,
                Collections.singletonList(
                        new ExpectedAssoc(RegexQNamePattern.MATCH_ALL, "half2", 1)));
    ...
    public void testCreateFailingThumbnail() throws Exception {
        ...
        assertFalse(
                secureNodeService.hasAspect(
                        corruptNode, ContentModel.ASPECT_FAILED_THUMBNAIL_SOURCE));
                        ...
                        new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                            public Void execute() throws Throwable {
                                ThumbnailDefinition thumbnailDef =
                                        thumbnailService
                                                .getThumbnailRegistry()
                                                .getThumbnailDefinition(TEST_THUMBNAIL);

                                Action createThumbnailAction =
                                        ThumbnailHelper.createCreateThumbnailAction(
                                                thumbnailDef, services);
                                actionService.executeAction(
                                        createThumbnailAction, corruptNode, true, true);
                                return null;
                            }
                        });
        // The thumbnail attempt has now failed. But a compensating action should have been
        // scheduled that will mark the
        // source node with a failure aspect. As that is an asynchronous action, we need to wait for
        // that to complete.
        ...
        transactionService
                ...
                .doInTransaction(
                        new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                            public Void execute() throws Throwable {
                                assertFalse(
                                        "corrupt node should not have renditioned aspect",
                                        secureNodeService.hasAspect(
                                                corruptNode, RenditionModel.ASPECT_RENDITIONED));
                                assertTrue(
                                        "corrupt node should have failed thumbnails aspect",
                                        secureNodeService.hasAspect(
                                                corruptNode,
                                                ContentModel.ASPECT_FAILED_THUMBNAIL_SOURCE));

                                Map<String, FailedThumbnailInfo> failedThumbnails =
                                        thumbnailService.getFailedThumbnails(corruptNode);
                                assertEquals(
                                        "Wrong number of failed thumbnails",
                                        1,
                                        failedThumbnails.size());

                                assertTrue(
                                        "Missing QName for failed thumbnail",
                                        failedThumbnails.containsKey(TEST_THUMBNAIL));
                                final FailedThumbnailInfo doclibFailureInfo =
                                        failedThumbnails.get(TEST_THUMBNAIL);
                                assertNotNull("Failure info was null", doclibFailureInfo);
                                assertEquals(
                                        "Failure count was wrong.",
                                        1,
                                        doclibFailureInfo.getFailureCount());
                                assertEquals(
                                        "thumbnail name was wrong.",
                                        TEST_THUMBNAIL,
                                        doclibFailureInfo.getThumbnailDefinitionName());

                                return null;
                            }
                        });

        // If you uncomment this line and set the timeout to a value greater than
        // ${system.thumbnail.retryPeriod} * 1000.
        // Then the retry period will have passed, the below re-thumbnail attempt will be made and
        // the test will fail with a
        // failureCount == 2.
        //
        // Thread.sleep(150 * 1000);

        // Run the thumbnail again. It should not run because the action condition should prevent
        // it.
    ...
    public void testRuleExecutionOnFailedThumbnailChild() throws Exception {
        ...
        ActionCondition condition =
                this.actionService.createActionCondition(NoConditionEvaluator.NAME, null);
        ...
        final NodeRef failedThumbnailNode =
                transactionService
                        .getRetryingTransactionHelper()
                        .doInTransaction(
                                new RetryingTransactionHelper.RetryingTransactionCallback<
        ...
        assertTrue(
                "Rule must not be executed on document",
                secureNodeService.hasAspect(corruptNode, ContentModel.ASPECT_GEN_CLASSIFIABLE));
        assertFalse(
                "Rule must not be executed on failed thumbnail",
                secureNodeService.hasAspect(
     ...
     * cannot attempt to perform the transformation for some reason (e.g. process/service
     * unavailable) and wishes to decline the request. Such 'failures' should not lead to the
     * addition of the {@link ContentModel#ASPECT_FAILED_THUMBNAIL_SOURCE} aspect.
     *
     * @since 4.0.1
     */
    @Test
    public void testCreateTransientlyFailingThumbnail() throws Exception {
                ...
                this.secureNodeService
                        .createNode(
                                ...
                                QName.createQName(
                                        ...
                                        "transientThumbnail.transientThumbnail"),
                                ContentModel.TYPE_CONTENT,
                                props)
                        .getChildRef();

        // Modified test to add content. Having no content was failing to find a transformer with
        // Legacy, but now
        // does not with both Legacy and Local transforms. As a result the test was passing for the
        // wrong reason.
        ...
        assertFalse(
                secureNodeService.hasAspect(testNode, ContentModel.ASPECT_FAILED_THUMBNAIL_SOURCE));
                            ...
                            public Void execute() throws Throwable {
                                ...
                                Action createThumbnailAction =
                                        ThumbnailHelper.createCreateThumbnailAction(
                                                thumbnailDef, services);
                                actionService.executeAction(
                                        createThumbnailAction, testNode, true, true);
                                return null;
                            }
                        });
        // The thumbnail attempt has now failed. But in this case the compensating action should NOT
        // have been scheduled.
        // We'll wait briefly in case it has erroneously been scheduled.

        Thread.sleep(
                3000); // This should be long enough for the compensating action to run - if it has
        // been scheduled, which it shouldn't.
        ...
        transactionService
                ...
                .doInTransaction(
                        new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                            public Void execute() throws Throwable {
                                assertFalse(
                                        "Node should not have renditioned aspect",
                                        secureNodeService.hasAspect(
                                                testNode, RenditionModel.ASPECT_RENDITIONED));
                                assertFalse(
                                        "Node should not have failed thumbnails aspect",
                                        secureNodeService.hasAspect(
                                                testNode,
                                                ContentModel.ASPECT_FAILED_THUMBNAIL_SOURCE));
    ...
    public void testGetThumbnailByName() throws Exception {
        ...
        NodeRef result1 =
                this.thumbnailService.getThumbnailByName(
                        jpgOrig, ContentModel.PROP_CONTENT, "small");
        ...
        NodeRef result3 =
                this.thumbnailService.getThumbnailByName(
                        jpgOrig, ContentModel.PROP_CONTENT, "anotherone");
     ...
     *
     * <p>That is useful if you need to use the log output for your tests.
     */
    ...
    public void testIfNodesExistsAfterCreateThumbnail() throws IOException {
        ...
        transactionService
                ...
                .doInTransaction(
                        new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                            public Void execute() throws Throwable {
                                // Delete the content node (pdfOrig) before the afterCommit code is
                                // executed
                                TestNodeDeleterListener testNodeDeleterListener =
                                        new TestNodeDeleterListener(pdfOrig);
                                // It needs to have a higher priority as the implemented
                                // afterCommit. The priority in order are (0,1,2,3,4)
    ...
    public void testLastThumbnailModificationDataContentUpdates() throws Exception {
        ...
        String lastThumbnailDataV1 =
                ((List<String>)
                                this.secureNodeService.getProperty(
                                        pdfOrig,
                ...
                .doInTransaction(
                        new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                            public Void execute() throws Throwable {
                                // Update file content
                                setNewContent(
                                        pdfOrig, "quick-size-limit.", MimetypeMap.MIMETYPE_PDF);
                                return null;
                            }
                        },
                        false,
        ...
        assertFalse(
                "Property 'Last thumbnail modification data' has not changed",
                lastThumbnailDataV1.equals(lastThumbnailDataV2));
    }
    ...
    public void testLastThumbnailModificationDataContentCopy() throws Exception {
        ...
        // Get initial value of property "Last thumbnail modification data"
        List<String> lastThumbnailData =
                (List<String>)
                        this.secureNodeService.getProperty(
                                pdfOrig, ContentModel.PROP_LAST_THUMBNAIL_MODIFICATION_DATA);
        assertNotNull(lastThumbnailData);
        assertEquals(1, lastThumbnailData.size());
        assertTrue(lastThumbnailData.get(0).contains("doclib:"));

        final NodeRef pdfCopy =
                copyService.copy(
                        pdfOrig,
                        this.folder,
                        ContentModel.ASSOC_CONTAINS,
                        QName.createQName("copyOfOriginal"));
        List<String> lastThumbnailDataCopy =
                (List<String>)
                        this.secureNodeService.getProperty(
                                pdfCopy, ContentModel.PROP_LAST_THUMBNAIL_MODIFICATION_DATA);
    ...
    public void testLockedContent() throws Exception {
        ...
        NodeRef pdfOrig =
                createOriginalContent(
                        sharedHomeNodeRef,
                        "testLockedContent-" + GUID.generate(),
        ...
        assertEquals(
                ...
                permissionService.hasPermission(pdfOrig, PermissionService.READ));
        assertEquals(
                AccessStatus.DENIED,
                permissionService.hasPermission(pdfOrig, PermissionService.WRITE));

        QName qname = QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "doclib");

        assertNull(
                thumbnailService.getThumbnailByName(
                        pdfOrig, ContentModel.PROP_CONTENT, qname.getLocalName()));
        assertNull(
    ...
    private void setNewContent(NodeRef noderef, String quickFileName, String mimetype)
            throws IOException {
    ...
    private static class ExpectedAssoc {
        ...
        public String toString() {
            return "ExpectedAssoc [assocTypeQName="
                    + assocTypeQName
                    + ", assocName="
                    + assocName
                    + ", count="
    ...
    private void checkRenditioned(NodeRef contentNodeRef, List<ExpectedAssoc> expectedAssocs) {
        assertTrue(
                "Renditioned aspect should have been applied",
                this.secureNodeService.hasAspect(
                        contentNodeRef, RenditionModel.ASPECT_RENDITIONED));

        for (ExpectedAssoc expectedAssoc : expectedAssocs) {
            QNamePattern qNamePattern =
                    expectedAssoc.getAssocName() != null
                            ? QName.createQName(
                                    NamespaceService.CONTENT_MODEL_1_0_URI,
                                    expectedAssoc.getAssocName())
                            : null;
            List<ChildAssociationRef> assocs =
                    this.secureNodeService.getChildAssocs(
                            contentNodeRef, expectedAssoc.getAssocTypeQName(), qNamePattern);
            assertNotNull(assocs);
            assertEquals(
                    expectedAssoc + " association count mismatch",
                    expectedAssoc.getCount(),
                    assocs.size());
        }
    }
    ...
    private void checkRendition(String thumbnailName, NodeRef thumbnail) {
        ...
        assertTrue(
                "Thumbnail should have been a rendition", renditionService.isRendition(thumbnail));

        // Check the name
        if (thumbnailName != null) {
            assertEquals(
                    thumbnailName,
                    this.secureNodeService.getProperty(thumbnail, ContentModel.PROP_NAME));
        }
        ...
        assertEquals(
                ContentModel.PROP_CONTENT,
                this.secureNodeService.getProperty(
                        thumbnail, ContentModel.PROP_CONTENT_PROPERTY_NAME));

        // Check the thumbnail is of type cm:thumbnail.
        assertEquals(
                "The thumbnail node should be of type cm:thumbnail!",
                ContentModel.TYPE_THUMBNAIL,
                secureNodeService.getType(thumbnail));

        // Check the thumbnail name property is correctly set on thumbnail.
        assertEquals(
                thumbnailName,
                secureNodeService.getProperty(thumbnail, ContentModel.PROP_THUMBNAIL_NAME));

        ContentData thumbnailData =
                (ContentData) secureNodeService.getProperty(thumbnail, ContentModel.PROP_CONTENT);
    ...
    /**
     * This method creates a node under the specified folder whose content is taken from the quick
     * file corresponding to the specified MIME type.
     *
     * @param parentFolder
     * @param mimetype
     * @return
     * @throws IOException
     */
    private NodeRef createOriginalContent(NodeRef parentFolder, String mimetype)
            throws IOException {
        return createOriginalContent(parentFolder, "original", mimetype);
    }

    private NodeRef createOriginalContent(NodeRef parentFolder, String baseName, String mimetype)
            throws IOException {
        String ext = this.mimetypeMap.getExtension(mimetype);
        ...
        NodeRef node =
                this.secureNodeService
                        .createNode(
                                ...
                                ContentModel.ASSOC_CONTAINS,
                                QName.createQName(
                                        NamespaceService.CONTENT_MODEL_1_0_URI, "original." + ext),
                                ContentModel.TYPE_CONTENT,
                                props)
                        .getChildRef();
    ...
    private NodeRef createCorruptedContent(NodeRef parentFolder) throws IOException {
        // The below pdf file has been truncated such that it is identifiable as a PDF but otherwise
        // corrupt.
        File corruptPdfFile =
                AbstractContentTransformerTest.loadNamedQuickTestFile("quickCorrupt.pdf");
        ...
        NodeRef node =
                this.secureNodeService
                        .createNode(
                                ...
                                ContentModel.ASSOC_CONTAINS,
                                QName.createQName(
                                        NamespaceService.CONTENT_MODEL_1_0_URI, "quickCorrupt.pdf"),
                                ContentModel.TYPE_CONTENT,
                                props)
                        .getChildRef();
        ...
        secureNodeService.setProperty(
                node,
                ContentModel.PROP_CONTENT,
                new ContentData(null, MimetypeMap.MIMETYPE_PDF, 0L, null));
    ...
    public void testAutoUpdate() throws Exception {
        ...
        ThumbnailDefinition details =
                this.thumbnailService.getThumbnailRegistry().getThumbnailDefinition("medium");
        @SuppressWarnings("unused")
        final NodeRef thumbnail =
                this.thumbnailService.createThumbnail(
                        jpgOrig,
                        ContentModel.PROP_CONTENT,
                        details.getMimetype(),
                ...
                .doInTransaction(
                        new RetryingTransactionCallback<Object>() {
                            public Object execute() throws Exception {
                                String ext =
                                        ThumbnailServiceImplTest.this.mimetypeMap.getExtension(
                                                MimetypeMap.MIMETYPE_IMAGE_JPEG);
                                File origFile =
                                        AbstractContentTransformerTest.loadQuickTestFile(ext);

                                ContentWriter writer =
                                        ThumbnailServiceImplTest.this.contentService.getWriter(
                                                jpgOrig, ContentModel.PROP_CONTENT, true);
                                writer.putContent(origFile);
    ...
    public void testHTMLToImageAndSWF() throws Exception {
        ...
        ThumbnailDefinition def =
                this.thumbnailService.getThumbnailRegistry().getThumbnailDefinition("medium");
        TransformationOptions transformationOptions = def.getTransformationOptions();
        Map<String, String> options =
                converter.getOptions(
                        transformationOptions, MimetypeMap.MIMETYPE_HTML, def.getMimetype());
        String targetMimetype = def.getMimetype();
        boolean supported =
                synchronousTransformClient.isSupported(
                        MimetypeMap.MIMETYPE_HTML, -1, null, targetMimetype, options, null, null);
        if (supported) {
            NodeRef thumb =
                    this.thumbnailService.createThumbnail(
                            nodeRef,
    ...
    public void testThumbnailServiceCreateApi() throws Exception {
        ...
        NodeRef otherFolder =
                this.secureNodeService
                        .createNode(
                                ...
                                ContentModel.ASSOC_CHILDREN,
                                QName.createQName(
                                        NamespaceService.CONTENT_MODEL_1_0_URI, "otherTestFolder"),
                                ContentModel.TYPE_FOLDER)
                        .getChildRef();
        ...
        checkRenditioned(
                jpgOrig,
                Collections.singletonList(
                        new ExpectedAssoc(RegexQNamePattern.MATCH_ALL, "smallJpeg", 1)));
        checkRendition("smallJpeg", thumbnail1);
        outputThumbnailTempContentLocation(
                thumbnail1, "jpg", "smallJpeg - 64x64, marked as thumbnail");
        ...
        checkRenditioned(
                jpgOrig,
                Collections.singletonList(
                        new ExpectedAssoc(RegexQNamePattern.MATCH_ALL, "smallPng", 1)));
        checkRendition("smallPng", thumbnail1);
        outputThumbnailTempContentLocation(
                thumbnail1, "png", "smallPng - 64x64, marked as thumbnail");

        //      Removd code: We now automatically discard all extra command options for security
        // reasons.

        // Create thumbnail - different target assoc details
        ThumbnailParentAssociationDetails tpad =
                new ThumbnailParentAssociationDetails(
                        otherFolder,
        ...
        checkRenditioned(
                jpgOrig,
                Collections.singletonList(
                        new ExpectedAssoc(RegexQNamePattern.MATCH_ALL, "targetDetails", 1)));
        checkRendition("targetDetails", thumbnail1);
        outputThumbnailTempContentLocation(
                thumbnail1, "png", "targetDetails - 64x64, marked as thumbnail");
        ...
        outputThumbnailTempContentLocation(
                thumbnail1, "png", "'null' - 64x64, marked as thumbnail");
    }

    @Test
    public void testRegistry() {
        ThumbnailRegistry thumbnailRegistry = this.thumbnailService.getThumbnailRegistry();
        List<ThumbnailDefinition> defs =
                thumbnailRegistry.getThumbnailDefinitions(MimetypeMap.MIMETYPE_HTML, -1);
    ...
    public void testJSAPI() throws Exception {
        ...
        ScriptLocation location =
                new ClasspathScriptLocation(
                        "org/alfresco/repo/thumbnail/script/test_thumbnailAPI.js");
        this.scriptService.executeScript(location, model);
    }

    /**
     * This test method tests the thumbnail placeholders which are handled in the {@link
     * ScriptThumbnailService}. See ALF-6566.
     */
    @Test
    public void testPlaceHoldersByMimeType() throws Exception {
        ...
        String docxDoclibIcon =
                scriptThumbnailService.getMimeAwarePlaceHolderResourcePath(
                        "doclib", MimetypeMap.MIMETYPE_WORD);
        ...
        String fallbackDoclibIcon =
                scriptThumbnailService.getMimeAwarePlaceHolderResourcePath(
                        "doclib", "application/wibble");
        ...
        final String standardMediumIcon =
                "alfresco/thumbnail/thumbnail_placeholder_medium.jpg"; // This one jpg, not png
        assertEquals(standardMediumIcon, mediumIcon);
    }

    protected void performLongRunningThumbnailTest(
            final List<ExpectedThumbnail> expectedThumbnails,
            final List<ExpectedAssoc> expectedAssocs,
            final LongRunningConcurrentWork concurrentWork,
            final Integer retryPeriod,
            final Integer quietPeriod)
            throws Exception {
        ...
        try {
            ...
            final NodeRef source =
                    secureNodeService
                            ...
                            .getChildRef();
            ContentWriter writer =
                    contentService.getWriter(source, ContentModel.PROP_CONTENT, true);
            ...
            for (ExpectedThumbnail expectedThumbnail : expectedThumbnails) {
                ...
                Action createThumbnailAction =
                        ThumbnailHelper.createCreateThumbnailAction(thumbnailDef, services);
                ...
                while ((endTime - startTime) < (TEST_LONG_RUNNING_TRANSFORM_TIME * multiples)) {
                    thumbnail =
                            transactionService
                                    .getRetryingTransactionHelper()
                                    .doInTransaction(
                                            new RetryingTransactionHelper
                                                            .RetryingTransactionCallback<
                                                    NodeRef>() {
                                                public NodeRef execute() throws Throwable {
                                                    return thumbnailService.getThumbnailByName(
                                                            source,
                                                            ContentModel.PROP_CONTENT,
                                                            expectedThumbnail.getThumbnailName());
                                                }
                    ...
                    if (thumbnail == null) {
                        ...
                        logger.debug(
                                "Elapsed "
                                        + (endTime - startTime)
                                        + " ms of "
                                        + TEST_LONG_RUNNING_TRANSFORM_TIME * multiples
                                        + " ms waiting for "
                                        + expectedThumbnail.getThumbnailName());
                    ...
                    .doInTransaction(
                            new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                                public Void execute() throws Throwable {
                                    ...
                                    for (final ExpectedThumbnail expectedThumbnail :
                                            expectedThumbnails) {
    ...
    /**
     * Verifies that our long-running test setup passes with simple behavior of a single thumbnail
     * requested with no other concurrent work.
     *
     * @throws Exception
     */
    @Test
    public void testLongRunningThumbnails() throws Exception {
        ...
        performLongRunningThumbnailTest(
                ...
                Collections.singletonList(
                        new ExpectedAssoc(RegexQNamePattern.MATCH_ALL, "imgpreview", 1)),
                new EmptyLongRunningConcurrentWork(),
                60,
                null);
    }

    /**
     * Verifies that a property can be updated on the source node during a long-running thumbnailing
     * process without affecting the thumbnailing or property update.
     *
     * @throws Exception
    ...
    public void testUpdatePropertyDuringLongRunningThumbnail() throws Exception {
        ...
        LongRunningConcurrentWork updatePropertyWork =
                new LongRunningConcurrentWork() {
                    @Override
                    ...
                    public void verify(NodeRef source) throws Exception {
                        ...
                        assertEquals(
                                "The node's property was not updated.",
                                TEST_LONG_RUNNING_PROPERTY_VALUE,
                                description);
                    }
                };
     ...
     *
     * <p>Note: the current architecture of the thumbnail and rendition services can't guarantee
     * that there won't be failed thumbnails for the scenario covered by this test. In particular,
     * given long-running thumbnails/renditions, the concurrent creation of more than one thumbnail
     * may fail with a primary key constraint exception because both transactions try to add the
     * same aspect to the same parent content node. Whilst the retrying transaction handler
     * correctly handles this scenario, the {@link org.alfresco.service.cmr.action.ActionService}
     * incorrectly generates a compensating action (failed thumbnail) when in fact the thumbnail
     * creation is recoverable.
     *
     * @throws Exception
     */
    @Test
    public void testCreateMultipleLongRunningThumbnails() throws Exception {
        ...
        // MNT-15135 note: at least one of the thumbnails will fail due to a deadlock/primary key
        // constraint exception.
        // This is expected, given the current structure of the thumbnail/rendition/action code and
        // can't be avoided
        ...
        //        expectedAssocs.add(new ExpectedAssoc(ContentModel.ASSOC_FAILED_THUMBNAIL, null,
        // 1));

        performLongRunningThumbnailTest(
                expectedThumbnails, expectedAssocs, new EmptyLongRunningConcurrentWork(), 1, 1);
    }

    /**
     * Defines the work to be done while long running transformations are being performed and the
     * means to verify that work completed successfully.
     *
     * <p>Implementations might update a property then verify the updated value for example.
     */

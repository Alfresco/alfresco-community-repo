...
public class SiteMembersCannedQuery
  ...
  protected SiteMembersCannedQuery(
  ...
  @Override
  protected List<SiteMembership> queryAndFilter(
    CannedQueryParameters parameters
  ) {
    ...
    final CQSiteMembersCallback callback = new CQSiteMembersCallback(
      siteShortName,
      sortPairs
    );
  ...
  private class CQSiteMembersCallback implements SiteMembersCallback {
    ...
    public void siteMember(
      ...
      if (personService.personExists(authority)) {
        ...
        lastName =
          (String) nodeService.getProperty(nodeRef, ContentModel.PROP_LASTNAME);
      }

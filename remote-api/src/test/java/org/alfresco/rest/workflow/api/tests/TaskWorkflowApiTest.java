...
public class TaskWorkflowApiTest extends EnterpriseWorkflowTestApi {
  ...
  public void testGetTaskById() throws Exception {
    ...
    String otherPerson = getOtherPersonInNetwork(
      requestContext.getRunAsUser(),
      requestContext.getNetworkId()
    )
      .getId();
    RequestContext otherContext = new RequestContext(
      requestContext.getNetworkId(),
      otherPerson
    );
    ...
    Clock actiClock = activitiProcessEngine
      .getProcessEngineConfiguration()
      .getClock();
    actiClock.setCurrentTime(createdCal.getTime());

    ProcessInstance processInstance = startAdhocProcess(
      requestContext.getRunAsUser(),
      requestContext.getNetworkId(),
      null
    );
    ...
    try {
      final Task task = activitiProcessEngine
        .getTaskService()
        .createTaskQuery()
  ...
  public void testGetTaskByIdAuthorization() throws Exception {
    ...
    String initiator = getOtherPersonInNetwork(
      requestContext.getRunAsUser(),
      requestContext.getNetworkId()
    )
      .getId();
    ...
    try {
      Task task = activitiProcessEngine
        .getTaskService()
        .createTaskQuery()
        .processInstanceId(processInstance.getId())
        .singleResult();
      ...
      try {
        ...
        assertEquals(
          ...
          expected.getHttpResponse().getStatusCode()
        );
      ...
      TestNetwork anotherNetwork = getOtherNetwork(
        requestContext.getNetworkId()
      );
      tenantAdmin =
        AuthenticationUtil.getAdminUserName() + "@" + anotherNetwork.getId();
      publicApiClient.setRequestContext(
        new RequestContext(TenantUtil.DEFAULT_TENANT, tenantAdmin)
      );
      try {
  ...
  public void testGetUnexistingTaskById() throws Exception {
    ...
    try {
      ...
      assertErrorSummary(
        "The entity with id: unexisting was not found",
  ...
  public void testUpdateTask() throws Exception {
    ...
    try {
      ...
      JSONObject result = tasksClient.updateTask(
        task.getId(),
        taskBody,
        selectedFields
      );
      ...
      assertEquals(
        1234,
        Integer.valueOf(result.get("priority").toString()).intValue()
      );
  ...
  public void testUpdateTaskMnt13276() throws Exception {
    ...
    // create test users
    final List<TestPerson> persons = transactionHelper.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<List<TestPerson>>() {
    ...
    final MemberOfSite memberOfSite = currentNetwork
      .getSiteMemberships(initiatorId)
      .get(0);
    ...
    try {
      ...
      try {
        ...
        result = tasksClient.updateTask(task.getId(), taskBody, selectedFields);
        fail(
          "User from group should not see items for claimed task by another user."
        );
      } catch (PublicApiException expected) {
        assertEquals(
          HttpStatus.FORBIDDEN.value(),
  ...
  public void testAssociationDefinitionExtraction() throws Exception {
    ...
    // Get person
    final ActivitiScriptNode person = TenantUtil.runAsUserTenant(
    ...
    ProcessInstance processInstance = TenantUtil.runAsUserTenant(
    ...
    // Get task
    Task activeTask = activitiProcessEngine
      .getTaskService()
      .createTaskQuery()
      .processInstanceId(processInstance.getId())
      .singleResult();
    ...
    try {
      ...
      try {
        ...
        JSONObject response = tasksClient.createTaskVariables(
          activeTask.getId(),
          variablesArray
        );
  ...
  public void testUpdateTaskInvalidProperty() throws Exception {
    ...
    try {
      ...
      try {
        ...
        assertErrorSummary(
          "The property selected for update does not exist for this resource: unexisting",
          expected.getHttpResponse()
        );
      }
  ...
  public void testClaimTask() throws Exception {
    ...
    try {
      ...
      List<String> selectedFields = new ArrayList<String>();
      selectedFields.addAll(
        Arrays.asList(new String[] { "state", "assignee" })
      );
      try {
        tasksClient.updateTask(task.getId(), taskBody, selectedFields);
        fail("Exception expected");
      ...
      List<MemberOfSite> memberships = getTestFixture()
      ...
      String group =
        "GROUP_site_" +
        memberOfSite.getSiteId() +
        "_" +
        memberOfSite.getRole().name();
      activitiProcessEngine
        .getTaskService()
  ...
  public void testUnClaimTask() throws Exception {
    ...
    try {
      ...
      activitiProcessEngine
        .getTaskService()
        .setAssignee(task.getId(), initiator);
      assertNotNull(
        activitiProcessEngine
          .getTaskService()
          .createTaskQuery()
  ...
  public void testCompleteTask() throws Exception {
    ...
    ProcessInstance processCompleteAsAssignee = startAdhocProcess(
      initiator,
      requestContext.getNetworkId(),
      null
    );
    ProcessInstance processCompleteAsOwner = startAdhocProcess(
      initiator,
      requestContext.getNetworkId(),
      null
    );
    ProcessInstance processCompleteAsInitiator = startAdhocProcess(
      initiator,
      requestContext.getNetworkId(),
      null
    );
    ProcessInstance processCompleteAsAdmin = startAdhocProcess(
      initiator,
      requestContext.getNetworkId(),
      null
    );
    ProcessInstance processCompleteWithVariables = startAdhocProcess(
      initiator,
      requestContext.getNetworkId(),
      null
    );
    try {
      Task asAssigneeTask = activitiProcessEngine
        .getTaskService()
        .createTaskQuery()
        .processInstanceId(processCompleteAsAssignee.getId())
        .singleResult();
      Task asOwnerTask = activitiProcessEngine
        .getTaskService()
        .createTaskQuery()
        .processInstanceId(processCompleteAsOwner.getId())
        .singleResult();
      Task asInitiatorTask = activitiProcessEngine
        .getTaskService()
        .createTaskQuery()
        .processInstanceId(processCompleteAsInitiator.getId())
        .singleResult();
      Task asAdminTask = activitiProcessEngine
        .getTaskService()
        .createTaskQuery()
        .processInstanceId(processCompleteAsAdmin.getId())
        .singleResult();
      Task withVariablesTask = activitiProcessEngine
        .getTaskService()
        .createTaskQuery()
      ...
      selectedFields.addAll(Arrays.asList(new String[] { "state" }));
      try {
        tasksClient.updateTask(
          asAssigneeTask.getId(),
          taskBody,
          selectedFields
        );
        fail("Exception expected");
      ...
      activitiProcessEngine
        .getTaskService()
        .setAssignee(asAssigneeTask.getId(), user);
      JSONObject result = tasksClient.updateTask(
        asAssigneeTask.getId(),
        taskBody,
        selectedFields
      ...
      activitiProcessEngine
        .getTaskService()
        .setAssignee(asInitiatorTask.getId(), null);
      result =
        tasksClient.updateTask(
          asInitiatorTask.getId(),
          taskBody,
          selectedFields
        );
      assertEquals("completed", result.get("state"));
      ...
      activitiProcessEngine
        .getTaskService()
        .setAssignee(withVariablesTask.getId(), null);
      ...
      HistoricTaskInstance historyTask = activitiProcessEngine
        .getHistoryService()
      ...
      assertEquals(
        1234,
        historyTask.getProcessVariables().get("newGlobalVariable")
      );
      assertEquals(
        5678,
        historyTask.getTaskLocalVariables().get("newLocalVariable")
      );
      assertNotNull(
        "The outcome should not be null for completed task.",
        historyTask.getTaskLocalVariables().get("bpm_outcome")
      );
      JSONObject variables = tasksClient.findTaskVariables(
        withVariablesTask.getId()
      );
      ...
      for (Object entry : entries) {
        JSONObject variableObject = (JSONObject) ((JSONObject) entry).get(
            "entry"
          );
  ...
  public void testSetOutcome() throws Exception {
    ...
    HttpResponse response = tasksClient.update(
      "tasks",
      task.getId(),
      null,
      null,
    ...
    HistoricTaskInstance historyTask = activitiProcessEngine
      ...
      .includeTaskLocalVariables()
      .singleResult();
    String outcome = (String) historyTask
  ...
  public void testGetTasks() throws Exception {
    ...
    try {
      ...
      JSONObject taskJSONObject = (JSONObject) (
        (JSONObject) jsonEntries.get(0)
      ).get("entry");
      assertNotNull(taskJSONObject);
      assertEquals(task.getId(), taskJSONObject.get("id"));
      assertEquals(processInstance.getId(), taskJSONObject.get("processId"));
  ...
  public void testGetTasksWithParams() throws Exception {
    ...
    try {
      ...
      final Task completedTask = activitiProcessEngine
        .getTaskService()
        .createTaskQuery()
        .processInstanceId(processInstance.getId())
        .singleResult();
      ...
      final Task reviewPooledTask = activitiProcessEngine
        .getTaskService()
        .createTaskQuery()
      ...
      assertNotNull(
        "Due date was not set for review pooled task.",
        reviewPooledTask.getDueDate()
      );
      ...
      activitiProcessEngine
        .getTaskService()
        .addCandidateGroup(activeTask.getId(), "sales");
      activitiProcessEngine
        .getTaskService()
        .setVariableLocal(activeTask.getId(), "numberVar", 10);

      final Task otherTask = activitiProcessEngine
        .getTaskService()
        .createTaskQuery()
        .processInstanceId(otherInstance.getId())
        .singleResult();
      ...
      params.put(
        "where",
        "(status = 'completed' AND processId = '" +
      ...
      params.put(
        "where",
        "(status = 'any' AND processId = '" + processInstance.getId() + "')"
      );
      assertTasksPresentInTaskQuery(
        params,
        tasksClient,
      ...
      try {
        ...
        assertErrorSummary(
          "Invalid status parameter: alfrescorocks",
          expected.getHttpResponse()
        );
      }
      ...
      params.put(
        "where",
        "(status = 'completed' AND assignee = '" + anotherUserId + "')"
      );
      ...
      params.put(
        "where",
        "(status = 'any' AND assignee = '" + anotherUserId + "')"
      );
      assertTasksPresentInTaskQuery(params, tasksClient, completedTask.getId());
      ...
      params.put(
        "where",
        "(status = 'active' AND candidateUser = '" + anotherUserId + "')"
      );
      ...
      activitiProcessEngine
        .getTaskService()
        .setAssignee(activeTask.getId(), null);
      assertTasksPresentInTaskQuery(params, tasksClient, activeTask.getId());
      ...
      params.put(
        "where",
        "(status = 'completed' AND candidateUser = '" + anotherUserId + "')"
      );
      try {
        tasksClient.findTasks(params);
        fail("Exception expected");
      ...
      params.put(
        "where",
        "(status = 'any' AND candidateUser = '" + anotherUserId + "')"
      );
      try {
        tasksClient.findTasks(params);
        fail("Exception expected");
      ...
      params.put(
        "where",
        "(status = 'active' AND description = 'This is a test description' AND processId='" +
        processInstance.getId() +
        "')"
      );
      assertTasksPresentInTaskQuery(params, tasksClient, activeTask.getId());
      ...
      params.put(
        "where",
        "(status = 'active' AND processBusinessKey = '" + businessKey + "')"
      );
      assertTasksPresentInTaskQuery(params, tasksClient, activeTask.getId());
      ...
      params.put(
        "where",
        "(status = 'any' AND processBusinessKey = '" + businessKey + "')"
      );
      assertTasksPresentInTaskQuery(
        params,
        tasksClient,
      ...
      params.put(
        "where",
        "(variables/bpm_dueDate = 'd:datetime 2013-09-15T12:22:31.866+0000')"
      );
      assertEquals(0, getResultSizeForTaskQuery(params, tasksClient));
      ...
      params.put(
        "where",
        "(dueAt = '" +
        ISO8601DateFormat.format(reviewPooledTask.getDueDate()) +
        "')"
      );
      ...
      JSONObject response = publicApiClient
        .processesClient()
        .getTasks(processInstance.getId(), params);
      assertNotNull(response);
      ...
      params.put(
        "where",
        "(status = 'any' AND variables/numberVar < 'd:int 5')"
      );
      assertEquals(0, getResultSizeForTaskQuery(params, tasksClient));
    } finally {
  ...
  public void testGetTasksWithPaging() throws Exception {
    ...
    try {
      ...
      String processDefinitionId = startedProcesses
        .get(0)
      ...
      assertNotNull(taskListJSONObject);
      JSONObject paginationJSON = (JSONObject) taskListJSONObject.get(
        "pagination"
      );
      ...
      cleanupProcessInstance(
        startedProcesses.toArray(new ProcessInstance[] {})
      );
    }
  }
  ...
  public void testGetTasksWithPagingAndVariablesLimit() throws Exception {
    ...
    try {
      ...
      params.put(
        "where",
        "(includeProcessVariables = true AND includeTaskVariables = true)"
      );
      JSONObject taskListJSONObject = tasksClient.findTasks(params);
      assertNotNull(taskListJSONObject);
      JSONObject paginationJSON = (JSONObject) taskListJSONObject.get(
  ...
  public void testGetTasksWithSorting() throws Exception {
    ...
    try {
      ...
      JSONObject paginationJSON = (JSONObject) tasksResponseJSON.get(
        "pagination"
      );
      assertEquals(6l, paginationJSON.get("count"));
      assertEquals(6l, paginationJSON.get("totalItems"));
      ...
      JSONObject taskJSON = (JSONObject) (
        (JSONObject) tasksListJSON.get(0)
      ).get("entry");
  ...
  public void testGetTaskCandidates() throws Exception {
    ...
    try {
      ...
      activitiProcessEngine
        .getTaskService()
        .addCandidateUser(task.getId(), "testuser");
      activitiProcessEngine
        .getTaskService()
        .addCandidateUser(task.getId(), "testuser2");
      activitiProcessEngine
        .getTaskService()
        .addCandidateGroup(task.getId(), "testgroup");

      TasksClient tasksClient = publicApiClient.tasksClient();

      JSONObject taskCandidatesJSONObject = tasksClient.findTaskCandidates(
        task.getId()
      );
      assertNotNull(taskCandidatesJSONObject);
      JSONArray candidateArrayJSON = (JSONArray) (
        (JSONObject) taskCandidatesJSONObject.get("list")
      ).get("entries");
      ...
      for (int i = 0; i < candidateArrayJSON.size(); i++) {
        JSONObject entry = (JSONObject) (
          (JSONObject) candidateArrayJSON.get(i)
        ).get("entry");
  ...
  public void testGetTaskVariables() throws Exception {
    ...
    try {
      ...
      Map<String, Object> actualLocalVariables = activitiProcessEngine
        .getTaskService()
        .getVariablesLocal(task.getId());
      Map<String, Object> actualGlobalVariables = activitiProcessEngine
        .getRuntimeService()
      ...
      assertEquals(
        actualLocalVariables.size() + actualGlobalVariables.size() - 1,
        entries.size()
      );
      ...
      for (int i = 0; i < entries.size(); i++) {
        JSONObject entry = (JSONObject) ((JSONObject) entries.get(i)).get(
            "entry"
          );
        assertNotNull(entry);
  ...
  public void testUpdateTaskVariablesPresentInModel() throws Exception {
    ...
    try {
      ...
      TasksClient tasksClient = publicApiClient.tasksClient();
      JSONObject resultEntry = tasksClient.updateTaskVariable(
        task.getId(),
        "bpm_percentComplete",
        variableBody
      );
  ...
  public void testUpdateTaskVariablesExceptions() throws Exception {
    ...
    try {
      ...
      try {
        ...
        assertErrorSummary(
          "Illegal value for variable scope: 'illegal'.",
          expected.getHttpResponse()
        );
      }
  ...
  public void testUpdateTaskVariableWithWrongType() throws Exception {
    ...
    try {
      List<Task> tasks = activitiProcessEngine
        .getTaskService()
        .createTaskQuery()
      ...
      JSONObject resultEntry = publicApiClient
        .tasksClient()
      ...
      JSONObject taskVariables = publicApiClient
        .tasksClient()
      ...
      JSONObject approvePercentObject = variablesByName.get(
        "wf_requiredApprovePercent"
      );
      ...
      assertEquals(
        ...
        activitiProcessEngine
          .getTaskService()
          .getVariable(taskId, "testVariable")
      );
      ...
      assertNotNull(resultEntry);
      final JSONObject updateAssigneeResult = (JSONObject) resultEntry.get(
        "entry"
      );
      ...
      TenantUtil.runAsUserTenant(
        new TenantRunAsWork<Void>() {
          @Override
          public Void doWork() throws Exception {
            JSONArray assigneeArray = (JSONArray) updateAssigneeResult.get(
              "value"
            );
  ...
  public void testUpdateTaskVariablesAuthentication() throws Exception {
    ...
    try {
      ...
      activitiProcessEngine
        .getTaskService()
        .setAssignee(task.getId(), requestContext.getRunAsUser());
      JSONObject jsonObject = tasksClient.updateTaskVariable(
        task.getId(),
        "newVariable",
        variableBody
      );
  ...
  public void testCreateTaskVariablesPresentInModel() throws Exception {
    ...
    try {
      ...
      TasksClient tasksClient = publicApiClient.tasksClient();
      JSONObject result = tasksClient.createTaskVariables(
        task.getId(),
        variablesArray
      );
      assertNotNull(result);
      ...
      JSONObject firstResultObject = (JSONObject) (
        (JSONObject) resultList.get(0)
      ).get("entry");
      ...
      JSONObject secondResultObject = (JSONObject) (
        (JSONObject) resultList.get(1)
      ).get("entry");
  ...
  public void testGetTaskVariablesRawVariableTypes() throws Exception {
    ...
    try {
      ...
      variablesToSet.put(
        "testVarNodeRef",
        new ActivitiScriptNode(
          new NodeRef("workspace:///testNode"),
          serviceRegistry
        )
      );
      variablesToSet.put(
      ...
      activitiProcessEngine
        .getTaskService()
        .setVariablesLocal(task.getId(), variablesToSet);
      Map<String, Object> actualLocalVariables = activitiProcessEngine
        .getTaskService()
        .getVariablesLocal(task.getId());
      ...
      assertNotNull(pagination);
      assertEquals(
        actualLocalVariables.size(),
        ((Long) pagination.get("count")).intValue()
      );
      assertEquals(
        actualLocalVariables.size(),
        ((Long) pagination.get("totalItems")).intValue()
      );
      assertEquals(0L, pagination.get("skipCount"));
      assertFalse((Boolean) pagination.get("hasMoreItems"));
  ...
  public void testGetTaskModel() throws Exception {
    ...
    try {
      ...
      JSONObject model = publicApiClient
        .tasksClient()
        .findTaskFormModel(task.getId());
      assertNotNull(model);
  ...
  protected void testModelEntries(Map<String, JSONObject> modelFieldsByName) {
    ...
    assertEquals(
      "{http://www.alfresco.org/model/bpm/1.0}package",
      modelEntry.get("qualifiedName")
    );
    ...
    try {
      ...
      for (Object entryObject : entriesJSON) {
        ...
        if (entryJSON.get("name").equals("Test Doc1")) {
          ...
          assertEquals(
            requestContext.getRunAsUser(),
            entryJSON.get("createdBy")
          );
          assertNotNull(entryJSON.get("modifiedAt"));
          assertEquals(
            requestContext.getRunAsUser(),
            entryJSON.get("modifiedBy")
          );
  ...
  public void testAddTaskItem() throws Exception {
    ...
    try {
      ...
      JSONObject result = tasksClient.addTaskItem(
        task.getId(),
        createItemObject.toJSONString()
      );
      ...
      JSONObject itemJSON = tasksClient.findTaskItem(
        task.getId(),
        docNodeRefs[0].getId()
      );
      ...
      activitiProcessEngine
        .getTaskService()
        .addCandidateUser(task.getId(), otherPerson);
      publicApiClient.setRequestContext(otherContext);
      result =
        tasksClient.addTaskItem(task.getId(), createItemObject.toJSONString());
      assertNotNull(result);
      ...
      activitiProcessEngine
        .getTaskService()
        .deleteCandidateUser(task.getId(), otherPerson);
      publicApiClient.setRequestContext(otherContext);
      try {
        tasksClient.addTaskItem(task.getId(), createItemObject.toJSONString());
        fail("Expected exception");
  ...
  protected int getResultSizeForTaskQuery(
  ...
  protected void assertTasksPresentInTaskQuery(
    ...
    if (countShouldMatch) {
      assertEquals(
        "Wrong amount of tasks returned by query",
        taskIds.length,
        jsonEntries.size()
      );
    }
    ...
    for (int i = 0; i < jsonEntries.size(); i++) {
      JSONObject entry = (JSONObject) ((JSONObject) jsonEntries.get(i)).get(
          "entry"
        );
      assertNotNull(entry);
  ...
  protected void cleanupProcessInstance(String... processInstances) {
    ...
    try {
      for (String instanceId : processInstances) {
        activitiProcessEngine
          .getRuntimeService()
          .deleteProcessInstance(instanceId, null);
        activitiProcessEngine
          .getHistoryService()
          .deleteHistoricProcessInstance(instanceId);
      }
    } catch (Throwable t) {
      // Ignore error during cleanup to prevent swallowing potential assetion-exception
      log("Error while cleaning up process instance", t);

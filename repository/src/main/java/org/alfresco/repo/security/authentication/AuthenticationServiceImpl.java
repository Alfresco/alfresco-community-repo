...
public class AuthenticationServiceImpl extends AbstractAuthenticationService
    ...
    private static final String BRUTE_FORCE_ATTACK_DETECTED =
            "Brute force attack was detected for user: %s";
    ...
    public void authenticate(String userName, char[] password) throws AuthenticationException {
        try {
            ...
            // clear context - to avoid MT concurrency issue (causing domain mismatch) - see also
            // 'validate' below
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Exception in authenticating user: "
    ...
    public boolean isUserProtected(String userName) {
        ...
        if (protectionEnabled) {
            ...
            if (protectedUser != null) {
                ...
                isProtected =
                        protectedUser.getNumLogins() >= protectionLimit
                                && currentTimeStamp - protectedUser.getTimeStamp()
                                        < protectionPeriodSeconds * 1000;
            }
        }
        return isProtected;
    }
    ...
    /**
     * Method records a failed login attempt. If the number of recorded failures exceeds {@link
     * AuthenticationServiceImpl#protectionLimit} the user will be considered 'protected'.
     */
    ...
    /**
     * Creates a key by combining the service instance ID with the username. This are the type of
     * keys maintained by protectedUsersCache map.
     */
    ...
    public void validate(String ticket) throws AuthenticationException {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Exception validating ticket: " + ticket + ", cause:" + ae.getMessage(),
                        ae);
            }
            clearCurrentSecurityContext();
            throw ae;
    ...
    /**
     * This method is called from the {@link #validate(String)} method. If this method returns null
     * then the user's tenant will be obtained from the username. This is generally correct in the
     * case where the user can be associated with just one tenant. Override this method in order to
     * force the selection of a different tenant (for whatever reason).
     *
     * @return String
     */
    ...
    public String getCurrentTicket() throws AuthenticationException {
        ...
        // So that preAuthenticationCheck can constrain the creation of new tickets, we first ask
        // for the current ticket without auto-creation
        String ticket = ticketComponent.getCurrentTicket(userName, false);
        if (ticket == null) {
            // If we get through the authentication check then it's safe to issue a new ticket (e.g.
            // for SSO/external-based login)
            if (logger.isTraceEnabled()) {
                logger.trace(
                        "Ticket was null, but, if we got through the authentication check, then"
                                + " it's safe to issue a new ticket for user: "
                                + AuthenticationUtil.maskUsername(userName));
            }
            return getNewTicket();
        }
    ...
    public void authenticateAsGuest() throws AuthenticationException {
        ...
        ticketComponent.clearCurrentTicket();
        ticketComponent.getCurrentTicket(
                guestUser,
                true); // to ensure new ticket is created (even if client does not explicitly call
        // getCurrentTicket)
        if (logger.isTraceEnabled()) {

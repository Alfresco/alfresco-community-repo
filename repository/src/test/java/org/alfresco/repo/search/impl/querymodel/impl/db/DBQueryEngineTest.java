...
public class DBQueryEngineTest {
  ...
  private static final String SQL_TEMPLATE_PATH =
    "alfresco.metadata.query.select_byDynamicQuery";
  ...
  public void shouldGetAFilteringResultSetFromAcceleratedNodeSelection() {
    ...
    ResultSet result = engine.acceleratedNodeSelection(
      options,
      dbQuery,
      assessor
    );
  ...
  public void shouldResultSetHaveExpectedAmountOfRequiredNodesBasedOnMaxItems() {
    ...
    FilteringResultSet result = engine.acceleratedNodeSelection(
      options,
      dbQuery,
      assessor
    );
  ...
  private void prepareTemplate(DBQuery dbQuery, List<Node> nodes) {
    doAnswer(invocation -> {
        ResultHandler<Node> handler = (ResultHandler<Node>) invocation.getArgument(
          2
        );
        for (Node node : nodes) {
      ...
      .when(template)
      .select(eq(SQL_TEMPLATE_PATH), eq(dbQuery), any());
  }
  ...
  private void mockStores() {
    Pair<Long, StoreRef> spacesStore = new Pair<>(
      6L,

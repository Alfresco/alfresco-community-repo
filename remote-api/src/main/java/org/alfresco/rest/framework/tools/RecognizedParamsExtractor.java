...
public interface RecognizedParamsExtractor {
    ...
    @Deprecated public static final String PARAM_FILTER_PROPERTIES = "properties";
    ...
    public static final List<String> KNOWN_PARAMS =
            Arrays.asList(
                    PARAM_RELATIONS,
    ...
    /**
     * Finds the formal set of params that any rest service could potentially have passed in as
     * request params
     *
    ...
    default Params.RecognizedParams getRecognizedParams(WebScriptRequest req) {
        ...
        Map<String, BeanPropertiesFilter> relationFilter =
                getRelationFilter(req.getParameter(PARAM_RELATIONS));
            ...
            if (rpeLogger().isWarnEnabled()) {
                rpeLogger()
                        .warn(
                                "Taking 'fields' param ["
                                        + fields
                                        + "] and ignoring deprecated 'properties' param ["
                                        + properties
                                        + "]");
            }
        }
        ...
        BeanPropertiesFilter filter =
                getFilter((fields != null ? fields : properties), includedFields);

        return new Params.RecognizedParams(
                requestParams,
                paging,
                filter,
                relationFilter,
    ...
    /**
     * Takes the web request and looks for a "fields" parameter (otherwise deprecated "properties"
     * parameter). Parses the parameter and produces a list of bean properties to use as a filter A
     * SimpleBeanPropertyFilter it returned that uses the bean properties. If no filter param is set
     * then a default BeanFilter is returned that will never filter fields (ie. Returns all bean
     * properties). If selectList is provided then it will take precedence (ie. be included) over
     * the fields/properties filter for top-level entries (bean properties). For example, this will
     * return entries from both select & properties, eg. select=abc,def&properties=id,name,ghi Note:
     * it should be noted that API-generic "fields" clause does not currently work for sub-entries.
     * Hence, even if the API-specific "select" clause allows selection of a sub-entries this cannot
     * be used with "fields" filtering. For example, an API-specific method may implement and return
     * "abc/blah", eg. select=abc/blah However the following will not return "abc/blah" if used with
     * fields filtering, eg. select=abc/blah&fields=id,name,ghi If fields filtering is desired then
     * it would require "abc" to be selected and returned as a whole, eg.
    ...
    default BeanPropertiesFilter getFilter(String filterParams, List<String> selectList) {
        if (filterParams != null) {
            ...
            // if supplied, the select takes precedence over the filter (fields/properties) for
            // top-level bean properties
            if (selectList != null) {
    ...
    default List<String> getClause(String param, String paramName) {
        ...
        try {
            ...
            if (selectedPropsTree instanceof CommonErrorNode) {
                rpeLogger()
                        .debug("Error parsing the " + paramName + " clause " + selectedPropsTree);
        ...
        try {
            ...
            return new QueryImpl(whereTree);
                ...
                RewriteCardinalityException
                        re) { // Catch any error so it doesn't get thrown up the stack
    ...
    /**
     * Takes the Sort parameter as a String and parses it into a List of SortColumn objects. The
     * format is a comma seperated list of "columnName sortDirection", e.g. "name DESC, age ASC". It
     * is not case sensitive and the sort direction is optional It default to sort ASCENDING.
     *
            ...
            while (st.hasMoreTokens()) {
                ...
                if (columnDesc.countTokens() <= 2) {
                    ...
                    if (columnDesc.hasMoreTokens()) {
                        ...
                        if (SortColumn.ASCENDING.equals(sortDef)
                                || SortColumn.DESCENDING.equals(sortDef)) {
                            sortOrder = sortDef;
                        } else {
                            rpeLogger()
                                    .debug(
                            ...
                            throw new InvalidArgumentException(
                                    "Unknown sort order direction '"
                                            + sortDef
                                            + "', expected asc or desc");
                        }
                    }
                    sortedColumns.add(
                            new SortColumn(columnName, SortColumn.ASCENDING.equals(sortOrder)));
                } else {
                    rpeLogger().debug("Invalid sort order definition (" + token + ")");
                    throw new InvalidArgumentException(
                            "Unknown sort order definition '"
                                    + token
                                    + "', expected 'field1,field2' or 'field1 asc,field2 desc' or"
                                    + " similar");
                }
                // filteredProperties.add();
            }
            //            logger.debug("Filtering using the following properties: " +
            // filteredProperties);
            //            BeanPropertiesFilter filter = new
            // BeanPropertiesFilter(filteredProperties);
    ...
    default Paging getPaging(String skip, String maxItems) {
        ...
        try {
            ...
            String errorMsg =
                    "Invalid paging parameters skipCount: " + skip + ", maxItems:" + maxItems;
    ...
    /**
     * Takes the web request and looks for a "relations" parameter Parses the parameter and produces
     * a list of bean properties to use as a filter A SimpleBeanPropertiesFilter it returned that
     * uses the properties If no filter param is set then a default BeanFilter is returned that will
     * never filter properties (ie. Returns all bean properties).
     *
     * @param filterParams String
     * @return BeanPropertiesFilter - if no parameter then returns a new ReturnAllBeanProperties
     *     class
     */
    default Map<String, BeanPropertiesFilter> getRelationFilter(String filterParams) {
        if (filterParams != null) {
            ...
            Map<String, BeanPropertiesFilter> filterMap =
                    new HashMap<String, BeanPropertiesFilter>(relations.length);
    ...
    default Map<String, String[]> getRequestParameters(WebScriptRequest req) {
        if (req != null) {
            ...
            if (paramNames != null) {
                Map<String, String[]> requestParameteters =
                        new HashMap<String, String[]>(paramNames.length);

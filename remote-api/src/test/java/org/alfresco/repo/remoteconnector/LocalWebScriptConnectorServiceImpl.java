...
/**
 * Testing implementation of {@link RemoteConnectorService} which talks to the local webscripts only
 *
 * @author Nick Burch
 * @since 4.0.2
 */
public class LocalWebScriptConnectorServiceImpl implements RemoteConnectorService {
    ...
    public RemoteConnectorResponse executeRequest(RemoteConnectorRequest request)
        ...
        if (logger.isInfoEnabled())
            logger.info(
                    "Performing local " + request.getMethod() + " request to " + request.getURL());
        ...
        if (authHeader != null) {
            ...
            if (logger.isDebugEnabled())
                logger.debug(
                        "HTTP Authorization found for the request, clearing security context, Auth"
                                + " is "
                                + authHeader);
        }
        ...
        private WebScriptHelper(BaseWebScriptTest test) throws Exception {
            ...
            sendRequest =
                    BaseWebScriptTest.class.getDeclaredMethod(
                            "sendRequest", Request.class, Integer.TYPE);
    ...
    /**
     * A wrapper around {@link BasicHttpAuthenticator}, which uses the Authentication Context if
     * present, otherwise HTTP Auth
     */

...
public class JobLockServiceTest extends TestCase {
  ...
  public static final String NAMESPACE =
    "http://www.alfresco.org/test/JobLockServiceTest";
  ...
  public void setUp() throws Exception {
    ...
    lockAAA =
      QName.createQName(
        NAMESPACE,
        "a-" + testName + ".a-" + testName + ".a-" + testName
      );
    lockAAB =
      QName.createQName(
        NAMESPACE,
        "a-" + testName + ".a-" + testName + ".b-" + testName
      );
    lockAAC =
      QName.createQName(
        NAMESPACE,
        "a-" + testName + ".a-" + testName + ".c-" + testName
      );
    lockAB = QName.createQName(NAMESPACE, "a-" + testName + ".b-" + testName);
    lockABA =
      QName.createQName(
        NAMESPACE,
        "a-" + testName + ".b-" + testName + ".a-" + testName
      );
    lockABB =
      QName.createQName(
        NAMESPACE,
        "a-" + testName + ".b-" + testName + ".b-" + testName
      );
    lockABC =
      QName.createQName(
        NAMESPACE,
        "a-" + testName + ".b-" + testName + ".c-" + testName
      );
  }
  ...
  public synchronized void testDeadlockPrevention() throws Throwable {
    ...
    assertNotNull(
      "T2 should have failed as the unordered locker.",
      t2.lockFailure
    );
  }
  ...
  public synchronized void testLockCallbackReleaseInactive() throws Exception {
    ...
    wait(2000L);
    assertEquals(
      "Lock callback timer was not terminated",
      checkedCount,
      checked[0]
    );
    assertEquals(
  ...
  public synchronized void testLockCallbackReleaseSelf() throws Exception {
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "checkedCount=" + checkedCount + ",releasedCount=" + releasedCount
        );
      }
      wait(10000L);
      assertEquals(
  ...
  private class TestCallback implements JobLockRefreshCallback {
    ...
    public boolean isActive() {
      isActiveCount++;
      logger.debug(
        "TestCallback.isActive => " + isActive + " (" + isActiveCount + ")"
      );
      return isActive;
    }

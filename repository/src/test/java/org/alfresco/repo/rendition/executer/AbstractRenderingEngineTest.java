...
public class AbstractRenderingEngineTest extends TestCase {
  ...
  public void testCreateRenditionNodeAssoc() throws Exception {
    ...
    assertEquals(
      renditionAssoc.getQName().getLocalName(),
    ...
    assertEquals(
      ...
      props.get(ContentModel.PROP_CONTENT_PROPERTY_NAME)
    );

    // Check the returned result is the association created by the call to
    // nodeServcie.createNode().
    Serializable result = definition.getParameterValue(
      ActionExecuter.PARAM_RESULT
    );
    assertEquals(
      "The returned rendition association is not the one created by the node service!",
      renditionAssoc,
      result
    );
  ...
  public void testCheckSourceNodeExists() {
    ...
    RenditionDefinitionImpl definition = new RenditionDefinitionImpl(
      "id",
      null,
      TestRenderingEngine.NAME
    );
    try {
      ...
      assertTrue(
        e
          .getMessage()
          .endsWith(
            "Cannot execute action as node does not exist: http://test/sourceId"
          )
      );
    }
  }
  ...
  public void testRenderingContext() {
    ...
    when(
      nodeService.createNode(
        eq(source),
        eq(renditionAssoc.getTypeQName()),
        any(QName.class),
        any(QName.class),
        anyMap()
      )
    ...
    try {
      ...
      assertTrue(
        e
          .getMessage()
          .endsWith(
            "The parameter: Some-param must be of type: java.lang.Booleanbut was of type: java.lang.String"
          )
      );
    }
  }
   ...
   */
  private RenditionDefinition makeRenditionDefinition(
    ChildAssociationRef renditionAssoc
  ) {
    String id = "definitionId";
    RenditionDefinition definition = new RenditionDefinitionImpl(
      id,

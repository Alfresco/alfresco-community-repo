...
public class SubscriptionServiceImpl implements SubscriptionService {
    ...
    public void setFollowingEmailTemplateLocation(
            RepositoryLocation followingEmailTemplateLocation) {
        this.followingEmailTemplateLocation = followingEmailTemplateLocation;
    }

    @Override
    public PagingSubscriptionResults getSubscriptions(
    ...
    public void subscribe(String userId, NodeRef node) {
        ...
        if (userId.equalsIgnoreCase(AuthenticationUtil.getRunAsUser())) {
            ...
            activityService.postActivity(
                    ActivityType.SUBSCRIPTIONS_SUBSCRIBE, null, ACTIVITY_TOOL, activityDataJSON);
        }
    }
    ...
    public void follow(String userId, String userToFollow) {
        ...
        if (userId.equalsIgnoreCase(AuthenticationUtil.getRunAsUser())) {
            try {
                ...
                activityData.put(
                        FOLLOWER_FIRSTNAME,
                        nodeService.getProperty(followerNode, ContentModel.PROP_FIRSTNAME));
                activityData.put(
                        FOLLOWER_LASTNAME,
                        nodeService.getProperty(followerNode, ContentModel.PROP_LASTNAME));
                activityData.put(USER_USERNAME, userToFollow);
                activityData.put(
                        USER_FIRSTNAME,
                        nodeService.getProperty(userNode, ContentModel.PROP_FIRSTNAME));
                activityData.put(
                        USER_LASTNAME,
                        nodeService.getProperty(userNode, ContentModel.PROP_LASTNAME));
                activityDataJSON = activityData.toString();

                activityService.postActivity(
                        ActivityType.SUBSCRIPTIONS_FOLLOW, null, ACTIVITY_TOOL, activityDataJSON);
    ...
    public void setSubscriptionListPrivate(String userId, boolean isPrivate) {
        ...
        nodeService.setProperty(
                getUserNodeRef(userId), ContentModel.PROP_SUBSCRIPTIONS_PRIVATE, isPrivate);
    }
    ...
    public boolean isSubscriptionListPrivate(String userId) {
        ...
        Serializable privateList =
                nodeService.getProperty(
                        getUserNodeRef(userId), ContentModel.PROP_SUBSCRIPTIONS_PRIVATE);
        if (privateList == null) {
            return false;
        }
    ...
    protected void checkRead(String userId, boolean checkPrivate) {
        ...
        if (currentUser.equalsIgnoreCase(userId)
                ...
                || !isSubscriptionListPrivate(userId)) {
            return;
        }
    ...
    protected void sendFollowingMail(String userId, String userToFollow) {
        ...
        Serializable emailFeedDisabled =
                nodeService.getProperty(userNode, ContentModel.PROP_EMAIL_FEED_DISABLED);
        ...
        model.put(
                FOLLOWER_JOBTITLE,
                nodeService.getProperty(followerNode, ContentModel.PROP_JOBTITLE));
        ...
        mail.setParameterValue(
                MailActionExecuter.PARAM_SUBJECT, "subscription.notification.email.subject");
        mail.setParameterValue(
                MailActionExecuter.PARAM_SUBJECT_PARAMS, new Object[] {followerFullName});
    ...
    /**
     * Returns the NodeRef of the email template or <code>null</code> if the template coudln't be
     * found.
     */
            ...
            try {
                ...
                List<NodeRef> nodeRefs =
                        searchService.selectNodes(
                                rootNodeRef, xpath, null, namespaceService, false);
                if (nodeRefs.size() > 1) {

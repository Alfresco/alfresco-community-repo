...
public class FeedNotifierTest {
    ...
    public void before() throws Exception {
        ...
        ObjectFactory<ActivitiesFeedModelBuilder> feedModelBuilderFactory =
                (ObjectFactory<ActivitiesFeedModelBuilder>)
                        activitiesFeedCtx.getBean("feedModelBuilderFactory");

        Scheduler scheduler = (Scheduler) ctx.getBean("schedulerFactory");

        JobDetail feedGeneratorJobDetail =
                (JobDetail) activitiesFeedCtx.getBean("feedGeneratorJobDetail");
        JobDetail postLookupJobDetail =
                (JobDetail) activitiesFeedCtx.getBean("postLookupJobDetail");
        JobDetail feedCleanerJobDetail =
                (JobDetail) activitiesFeedCtx.getBean("feedCleanerJobDetail");
        JobDetail postCleanerJobDetail =
                (JobDetail) activitiesFeedCtx.getBean("postCleanerJobDetail");
        JobDetail feedNotifierJobDetail =
        ...
        // one of the users follows the other user (so that we can test that notification failures
        // for one of the users does not
        // affect other users)
        transactionService
                .getRetryingTransactionHelper()
                .doInTransaction(
    ...
    public void testFailedNotifications() {
        ...
        assertEquals(
                1,
                errorProneActionExecutor.getNumFailed()); // should be a single notification failure
        assertTrue(
                errorProneActionExecutor.getNumSuccess() > 0); // others should have gone through ok
    ...
    public void testNullSiteNetworkStatus() throws Exception {
                ...
                .doInTransaction(
                        new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                            ...
                            public Void execute() throws Throwable {
                                ...
                                activityService.postActivity(
                                        activityType, siteId, appTool, jsonActivityData);
        ...
        try {
            feeds =
                    activityService.getUserFeedEntries(
                            userName3, null, false, false, null, null, -1);
        } catch (Exception e) {
        ...
        for (ActivityFeedEntity feed : feeds) {
            if (feed.getActivityType().equals(activityType)

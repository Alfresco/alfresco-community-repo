...
public class TemporaryNodes extends ExternalResource {
  ...
  protected void after() {
    ...
    final RetryingTransactionHelper transactionHelper = springContext.getBean(
      "retryingTransactionHelper",
      RetryingTransactionHelper.class
    );
    final CheckOutCheckInService cociService = springContext.getBean(
      "CheckOutCheckInService",
      CheckOutCheckInService.class
    );
    final DictionaryService dictionaryService = springContext.getBean(
      "DictionaryService",
      DictionaryService.class
    );
    final NodeService nodeService = springContext.getBean(
      "NodeService",
      NodeService.class
    );
    final SiteService siteService = springContext.getBean(
      "SiteService",
      SiteService.class
    );
    final VersionService versionService = springContext.getBean(
      "VersionService",
                ...
                for (NodeRef node : temporaryNodeRefs) {
                  ...
                  if (nodeService.exists(node)) {
                    ...
                    if (versionService.isVersioned(node)) {
                      log.debug(
                        "Deleting version history of temporary node " +
                        nodeService.getProperty(node, ContentModel.PROP_NAME)
                      );
                    ...
                    if (
                      nodeType.equals(SiteModel.TYPE_SITE) ||
  ...
  public NodeRef createNode(
    ...
    return this.createNodeWithTextContent(
        parentNode,
        nodeCmName,
        nodeType,
        nodeCreator,
  ...
  public NodeRef createNodeWithTextContent(
    ...
    NodeRef newNodeRef = transactionHelper.doInTransaction(
      new RetryingTransactionCallback<NodeRef>() {
        public NodeRef execute() throws Throwable {
          final NodeService nodeService = (NodeService) appContextRule
            .getApplicationContext()
            .getBean("nodeService");
          ...
          if (textContent != null) {
            ContentService contentService = appContextRule
              .getApplicationContext()
  ...
  public NodeRef createFolder(
    ...
    NodeRef newNodeRef = transactionHelper.doInTransaction(
      new RetryingTransactionCallback<NodeRef>() {
        public NodeRef execute() throws Throwable {
          final NodeRef result = createNode(
            nodeCmName,
            parentNode,
    ...
    NodeRef newNodeRef = transactionHelper.doInTransaction(
      new RetryingTransactionCallback<NodeRef>() {
        public NodeRef execute() throws Throwable {
          ...
          if (isVersionable) {
            NodeService nodeService = appContextRule
              .getApplicationContext()
          ...
          File quickFile = AbstractContentTransformerTest.loadNamedQuickTestFile(
  ...
  public NodeRef createQuickFileByName(
    final String quickFileName,
    final NodeRef parentNode,
    final String nodeCreator
  ) {
    final MimetypeMap mimetypeService = (MimetypeMap) appContextRule
      .getApplicationContext()
      .getBean("mimetypeService");
    final RetryingTransactionHelper transactionHelper = (RetryingTransactionHelper) appContextRule
      .getApplicationContext()
      .getBean("retryingTransactionHelper");

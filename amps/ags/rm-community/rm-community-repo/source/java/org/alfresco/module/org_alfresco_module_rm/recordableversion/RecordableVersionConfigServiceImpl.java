...
public class RecordableVersionConfigServiceImpl
  ...
  public List<Version> getVersions(NodeRef nodeRef) {
    ...
    for (RecordableVersionPolicy recordableVersionPolicy : recordableVersionPolicies) {
      ...
      boolean selected = isVersionPolicySelected(
  ...
  public void setVersion(NodeRef nodeRef, String version) {
    ...
    RecordableVersionPolicy recordableVersionPolicy = RecordableVersionPolicy.valueOf(
      version
    );
    getNodeService()
      .setProperty(
  ...
  private boolean isVersionPolicySelected(
    RecordableVersionPolicy recordableVersionPolicy,
    NodeRef nodeRef
  ) {
    boolean isVersionPolicySelected = false;
    String policy = (String) getNodeService()
      .getProperty(nodeRef, PROP_RECORDABLE_VERSION_POLICY);
    if (isNotBlank(policy)) {
      if (
        RecordableVersionPolicy.valueOf(policy).equals(recordableVersionPolicy)
      ) {

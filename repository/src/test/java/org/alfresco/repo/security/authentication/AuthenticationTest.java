...
public class AuthenticationTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        ChildApplicationContextFactory sysAdminSubsystem =
                (ChildApplicationContextFactory) ctx.getBean("sysAdmin");
        ...
        // Let's look inside the alfresco authentication subsystem to get the DAO-wired
        // authentication manager
        ChildApplicationContextManager authenticationChain =
                (ChildApplicationContextManager) ctx.getBean("Authentication");
        ApplicationContext subsystem =
                authenticationChain.getApplicationContext(
        ...
        systemNodeRef =
                nodeService.createNode(rootNodeRef, children, system, container).getChildRef();
        typesNodeRef =
        ...
        personAndyNodeRef =
                nodeService
                        .createNode(
                                typesNodeRef, children, ContentModel.TYPE_PERSON, container, props)
                        .getChildRef();
    ...
    public void testCreateUsers() {
        authenticationService.createAuthentication(
                AuthenticationUtil.getGuestUserName(), DONT_CARE_PASSWORD);
        authenticationService.authenticate(
                AuthenticationUtil.getGuestUserName(), DONT_CARE_PASSWORD);
        // Guest is treated like any other user
        assertEquals(
                AuthenticationUtil.getGuestUserName(), authenticationService.getCurrentUserName());
        ...
        if (!tenantService.isEnabled()) {
            authenticationService.createAuthentication(
                    "Mr.Woof.Banana@chocolate.chip.cookie.com", DONT_CARE_PASSWORD);
            authenticationService.authenticate(
                    "Mr.Woof.Banana@chocolate.chip.cookie.com", DONT_CARE_PASSWORD);
            assertEquals(
    ...
    /**
     * Test for ALF-20680 Test of the {@link
     * RepositoryAuthenticationDao#getUserFolderLocation(String)} in multitenancy
     */
                    ...
                    public Object doWork() throws Exception {
                        if (!tenantAdminService.existsTenant(TEST_TENANT_DOMAIN)) {
                            tenantAdminService.createTenant(
                                    TEST_TENANT_DOMAIN, TENANT_ADMIN_PW.toCharArray(), null);
                        }
                        return null;
                    }
                },
    ...
    public void testCreateAndyUserAndOtherCRUD()
        ...
        assertTrue(
                compositePasswordEncoder.matchesPassword(
                        "carrot", newDetails.getPassword(), null, rau.getHashIndicator()));
        ...
        assertTrue(
                compositePasswordEncoder.matchesPassword(
                        "potato", newDetails.getPassword(), null, rau.getHashIndicator()));

        dao.deleteUser("Andy");
        assertFalse(
                "Should not be a cache entry for 'Andy'.", authenticationCache.contains("Andy"));
        assertNull("DAO should report that 'Andy' does not exist.", dao.getUserOrNull("Andy"));
    }

    /**
     * <a href="https://issues.alfresco.com/jira/browse/ALF-19301">ALF-19301: Unsafe usage of
     * transactions around authenticationCache</a>
     */
    public void testStaleAuthenticationCacheRecovery() {
        RepositoryAuthenticationDao dao = createRepositoryAuthenticationDao();

        assertFalse(
                "Must start with no cache entry for 'Andy'.", authenticationCache.contains("Andy"));
        ...
        // So the cache should be populated.  Now, remove Andy's node but without having policies
        // fire.
    ...
    public void testAuthentication() {
        ...
        UsernamePasswordAuthenticationToken token =
                new UsernamePasswordAuthenticationToken("GUEST", new String(DONT_CARE_PASSWORD));
    ...
    public void testCreateAuthenticationWhileRunningAsSystem() throws Exception {
        userTransaction.rollback();
        RunAsWork<Object> authWorkAsMuppet =
                new RunAsWork<Object>() {
                    public Object doWork() throws Exception {
                        RunAsWork<Object> authWorkAsSystem =
                                new RunAsWork<Object>() {
                                    public Object doWork() throws Exception {
                                        RetryingTransactionCallback<Object> txnWork =
                        ...
                        return AuthenticationUtil.runAs(
                                authWorkAsSystem, AuthenticationUtil.getSystemUserName());
                    }
                };
    ...
    public void testAuthenticationFailure() {
        ...
        UsernamePasswordAuthenticationToken token =
                new UsernamePasswordAuthenticationToken("Andy", "turnip");
        token.setAuthenticated(false);
    ...
    public void testTicket() {
        ...
        UsernamePasswordAuthenticationToken token =
                new UsernamePasswordAuthenticationToken("Andy", "ticket");
        token.setAuthenticated(false);
    ...
    public void testAuthenticationService() {
        ...
        authenticationService.updateAuthentication(
                "Andy", "auth3".toCharArray(), "auth4".toCharArray());
        ...
        authenticationService.updateAuthentication(
                "Andy", "auth4".toCharArray(), "auth5".toCharArray());
    ...
    public void testAlwaysGetNewTicket() {
        ...
        // it means that we should be able to use the application (for external SSO case, for
        // example)
        ...
        try {
            ...
            fail(
                    " Previous call to validate should have cleared the context, so no new tickets"
                            + " should be issued");
        } catch (Exception e) {
            // we expect this here
        }
    ...
    /**
     * ACE-3542: test that "server.maxusers" setting limits the number of unique logins to that
     * number.
     */
    ...
    /**
     * Tests the scenario where a user logs in after the system has been upgraded. Their password
     * should get re-hashed using the preferred encoding.
     */
    public void testRehashedPasswordOnAuthentication() throws Exception {
        ...
        List<String> hashIndicatorProp =
                (List<String>) userProps.get(ContentModel.PROP_HASH_INDICATOR);
    ...
    public void testCreateUserWithHashedPassword() throws Exception {
                ...
                new TenantUtil.TenantRunAsWork<Object>() {
                    public Object doWork() throws Exception {
                        if (!tenantAdminService.existsTenant(myTestDomain)) {
                            tenantAdminService.createTenant(
                                    myTestDomain, TENANT_ADMIN_PW.toCharArray(), null);
                        }
                        return null;
                    }
                },
        ...
        for (String enc : encs) {
            ...
            assertCreateHashed(
                    SOME_PASSWORD, hash, null, user1 + TenantService.SEPARATOR + myTestDomain);
            assertCreateHashed(
                    SOME_PASSWORD,
                    null,
    ...
    private void assertCreateHashed(
            String rawString, String hash, char[] rawPassword, String user) {

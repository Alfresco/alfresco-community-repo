...
public class MultiFileDumperTest {
    ...
    public void exceptionThrownWhenZeroPrefixesUsed() {
        ...
        new MultiFileDumper(
                new String[] {}, TempFileProvider.getTempDir(), "", dbToXMLFactory, null);
    }
    ...
    public void canDumpSchemaToFiles() {
        ...
        MultiFileDumper dumper =
                new MultiFileDumper(prefixes, directory, fileNamePattern, dbToXMLFactory, null);

        when(dbToXMLFactory.create(
                        argThat(isFileNameStartingWith("SchemaDump-MySQL-a_-")), eq("a_")))
                .thenReturn(dbToXMLForA);
        when(dbToXMLFactory.create(
                        argThat(isFileNameStartingWith("SchemaDump-MySQL-b_-")), eq("b_")))
                .thenReturn(dbToXMLForB);
        when(dbToXMLFactory.create(
                        argThat(isFileNameStartingWith("SchemaDump-MySQL-c_-")), eq("c_")))
    ...
    public void canDumpSchemaToFilesForDefaultDBPrefixes() {
        ...
        MultiFileDumper dumper =
                new MultiFileDumper(directory, fileNamePattern, dbToXMLFactory, null);

        Map<String, DbToXML> xmlExporters =
                new HashMap<String, DbToXML>(MultiFileDumper.DEFAULT_PREFIXES.length);
        ...
        // Check that each DEFAULT_PREFIX prefix resulted in its associated DbToXML object being
        // used.
    ...
    /**
     * Check that actualFile has the expected directory and file name prefix, e.g. if the actual
     * file is /temp/my_file_123.xml and we call:
     *
     * <pre>
     ...
     * @param actualFile
     */
    private void assertPathCorrect(
            String expectedFileNamePrefix, File expectedDirectory, File actualFile) {

...
public class CheckOutCheckInServiceImplTest extends BaseSpringTest {
    ...
    private static final QName PROP_NAME_QNAME =
    ...
    public void before() {
        ...
        RetryingTransactionCallback<Void> processInitWork =
    ...
    private NodeRef checkout() {
        ...
        String expectedWorkingCopyName =
                CheckOutCheckInServiceImpl.createWorkingCopyName(name, expectedWorkingCopyLabel);
        String workingCopyName =
                (String) this.nodeService.getProperty(workingCopy, PROP_NAME_QNAME);
        assertEquals(expectedWorkingCopyName, workingCopyName);
        // Check a record has been kept of the working copy label used to create the working copy
        // name
        assertEquals(
        ...
        assertEquals(
                "The content string of the working copy should match the original immediatly after"
                        + " checkout.",
    ...
    public void testCheckIn() {
        ...
        NodeRef origNodeRef =
                cociService.checkin(workingCopy3, versionProperties3, contentUrl, true);
    ...
    public void testCheckInVersionedNode_MNT_8789() {
        ...
        // Admin checked in the node, but as the working copy was not modified, the modifier should
        // not change
    ...
    public void testCheckInWithNameChange() {
        ...
        assertEquals(
                "Working copy not found ", fileWorkingCopyNodeRef, fileWorkingCopyNodeRefCheck);
    ...
    public void testAutoCancelCheckOut() {
        Date modifiedDateBeforeCheckOut =
                (Date) this.nodeService.getProperty(this.nodeRef, ContentModel.PROP_MODIFIED);
        NodeRef workingCopy = checkout();
        assertNotNull(workingCopy);
        ...
        Date modifiedDateAfterCheckOut =
                (Date) this.nodeService.getProperty(this.nodeRef, ContentModel.PROP_MODIFIED);
    ...
    public void testMultipleCheckoutsCheckInsWithPropChange() {
        // Note: this test assumes cm:autoVersionProps=true by default (refer to
        // cm:versionableAspect in contentModel.xml)
                        ...
                        new RetryingTransactionCallback<Object>() {
                            public Object execute() throws Exception {
                                nodeService.setProperty(
                                        workingCopy1, ContentModel.PROP_AUTHOR, "author1");

                                Map<String, Serializable> versionProperties =
                                        new HashMap<String, Serializable>();
                                versionProperties.put(
                                        Version.PROP_DESCRIPTION, "This is a test version 1");
        ...
        transactionService
                ...
                .doInTransaction(
                        new RetryingTransactionCallback<Object>() {
                            public Object execute() throws Exception {
                                nodeService.setProperty(
                                        workingCopy2, ContentModel.PROP_AUTHOR, "author2");

                                Map<String, Serializable> versionProperties =
                                        new HashMap<String, Serializable>();
                                versionProperties.put(
                                        Version.PROP_DESCRIPTION, "This is a test version 2");
        ...
        transactionService
                ...
                .doInTransaction(
                        new RetryingTransactionCallback<Object>() {
                            public Object execute() throws Exception {
                                nodeService.setProperty(
                                        workingCopy3, ContentModel.PROP_AUTHOR, "author3");

                                Map<String, Serializable> versionProperties =
                                        new HashMap<String, Serializable>();
                                versionProperties.put(
                                        Version.PROP_DESCRIPTION, "This is a test version 3");
    ...
    public void testAlfrescoCheckoutDoesNotModifyNode() {
        ...
        assertEquals(
                "Checkout should not cause the modified date to change!", initModified, modified);
        ...
        modifier = nodeService.getProperty(nodeRef, ContentModel.PROP_MODIFIER);
        // See MNT-8789, nothing has changed in the working copy, so the modifier should be left
        // untouched
        assertEquals(
                "The modifier should NOT change to Admin after checkin!", initModifier, modifier);
    }
    ...
    public void testCheckOutPermissions_ALF7680_ALF535() {
        ...
        NodeRef folder1 =
                createFolderWithPermission(rootNodeRef, userName, PermissionService.CONSUMER);
        ...
        NodeRef folder2 =
                createFolderWithPermission(
                        rootNodeRef, userName, PermissionService.ALL_PERMISSIONS);
        ...
        try {
            ...
            fail(
                    "Node can't be checked out to folder where user has no permissions to create"
                            + " children");
        } catch (Exception e) {
    ...
    public void testCheckInLockableAspectDoesntCopies_ALF16194() {
        ...
        if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_LOCKABLE)) {
            fail(
                    "Lockable aspect should not be copied from the working copy to the original"
                            + " document");
        }
    }
    ...
    private NodeRef createNodeWithPermission(NodeRef parent, String username, String permission) {
        ...
        NodeRef node =
                copyService.copy(
                        nodeRef, parent, ContentModel.ASSOC_CHILDREN, ContentModel.TYPE_CONTENT);
     ...
     * MNT-2641
     *
     * <p>Creating a document and working copy. Then try to move working copy to another place. Test
     * is passed, if a working copy was moved to another place with original document. Only the lock
     * owner can move documents.
     */
    @Test
    ...
    public void testDeleteUpdateOriginalOfCheckedOutDocument() {
        // Create a FolderA
        final NodeRef folderA =
                createFolder("DeleteUpdateOriginalOfCheckedOutDocument_" + GUID.generate());
    ...
    /**
     * MNT-2641 The working copy delete is equivalent to "cancelCheckout". This should fail for
     * everyone except the lock owner.
     */
    @Test
    ...
    /**
     * MNT-2641: The {@link ContentModel#ASPECT_WORKING_COPY} aspect cannot be removed from a
     * working copy
     */
    @Test
    public void testDeleteWorkingCopyAspect() {
        // Create a FolderA
        final NodeRef folderA =
                createFolder("DeleteCopiedFromAspectFromWorkingCopy_" + GUID.generate());
    ...
    public void testDeleteWorkingCopyLinkAssociation() {
        // Create a FolderA
        final NodeRef folderA =
                createFolder("DeleteOriginalAssociationFromCopy_" + GUID.generate());
        ...
        try {
            ...
            fail(
                    "Should not have been allowed to remove the association from working copy to"
                            + " original");
    ...
    /**
     * <br>
     * Creating node - CheckOut - Add write lock to working copy - Unlock working copy -
     * CancelCheckOut
     */
    @Test
    public void testCancelCheckoutUnlockedWCopy() {
        ServiceRegistry serviceRegistry =
                (ServiceRegistry) this.applicationContext.getBean(ServiceRegistry.SERVICE_REGISTRY);
    ...
    public void testDeleteAndRestore() {
        ...
        permissionService.setPermission(
                spacesRootNodeRef, userName, PermissionService.ALL_PERMISSIONS, true);
        ...
        assertEquals(
                initialText,
                contentService.getReader(contentRef, ContentModel.PROP_CONTENT).getContentString());
        assertEquals(
                updatedText,
                contentService
        ...
        nodeService.moveNode(
                folderRef, folderRef2, ContentModel.ASSOC_CONTAINS, ContentModel.ASSOC_CONTAINS);

...
public class SiteExportGet extends AbstractWebScript {
    private static final List<String> USERS_NOT_TO_EXPORT =
            Arrays.asList(new String[] {"admin", "guest"});
    ...
    public void execute(WebScriptRequest req, WebScriptResponse res) throws IOException {
        ...
        if (site == null) {
            throw new WebScriptException(
                    Status.STATUS_NOT_FOUND, "No Site found with that short name");
        }
        ...
        if (peopleNodes.isEmpty()) {
            ...
            String text =
                    "Person nodes were not exported because the site does not contain\n"
        ...
        // Export the User (authentication) details for the site users that have user(authenticator)
        // nodes associated
        ...
        for (String contextName : authenticationContextManager.getInstanceIds()) {
            ApplicationContext ctx =
                    authenticationContextManager.getApplicationContext(contextName);
            try {
                authenticationDao =
        ...
        // authenticationDao is initialized only when using a repository-based authentication
        // subsystem
        if (authenticationDao == null) {
            mainZip.putNextEntry(new ZipEntry("Users_Skipped_As_Wrong_Authentication.txt"));
            String text =
                    "Users were not exported because the Authentication\n"
            ...
            String text =
                    "User nodes were not exported because the site does not contain\n"
                            + "any members other than the built-ins e.g. admin, guest.";
            outputForNesting.write(text.getBytes("ASCII"));
        } else {
    ...
    protected void doSiteACPExport(SiteInfo site, CloseIgnoringOutputStream writeTo)
            throws IOException {
        ...
        ACPExportPackageHandler handler =
    ...
    protected void doPeopleACPExport(
            final List<NodeRef> peopleNodes, SiteInfo site, CloseIgnoringOutputStream writeTo)
            throws IOException {
        if (!peopleNodes.isEmpty()) {
            // Build the parameters
            ExporterCrawlerParameters parameters = new ExporterCrawlerParameters();
            parameters.setExportFrom(
                    new Location(peopleNodes.toArray(new NodeRef[peopleNodes.size()])));
            parameters.setCrawlChildNodes(true);
            parameters.setCrawlSelf(true);
            parameters.setCrawlContent(true);
    ...
    private List<NodeRef> getPersonNodesInSiteGroup(SiteInfo site) {
        ...
        Set<String> siteUsers =
                authorityService.getContainedAuthorities(AuthorityType.USER, siteGroup, false);
     ...
     * @return
     */
    private List<NodeRef> getUserNodesInSiteGroup(
            SiteInfo site, RepositoryAuthenticationDao authenticationDao) {
    ...
    protected void doGroupExport(SiteInfo site, CloseIgnoringOutputStream writeTo)
            throws IOException {
        // Find the root group
        String siteGroup = AbstractSiteWebScript.buildSiteGroup(site);
        ...
        Set<String> siteGroups =
                authorityService.getContainedAuthorities(AuthorityType.GROUP, siteGroup, true);
    ...
    protected void doUserACPExport(
            List<NodeRef> userNodes, SiteInfo site, CloseIgnoringOutputStream writeTo)
            throws IOException {
        // Build the parameters
        ExporterCrawlerParameters parameters = new ExporterCrawlerParameters();
    ...
    public void setAuthenticationContextManager(
            ChildApplicationContextManager authenticationContextManager) {

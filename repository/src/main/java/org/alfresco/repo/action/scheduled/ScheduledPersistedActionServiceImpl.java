...
/**
 * A service which handles the scheduling of the execution of persisted actions. It handles
 * registering them with the Quartz scheduler on repository start, and handles the edit, creation
...
public class ScheduledPersistedActionServiceImpl implements ScheduledPersistedActionService {
    ...
    protected static final Set<QName> ACTION_TYPES =
            new HashSet<QName>(Arrays.asList(new QName[] {ActionModel.TYPE_ACTION_SCHEDULE}));
    ...
    protected void locatePersistanceFolder() {
        // TODO: Use SearchService.selectNodes(repositoryHelper.getCompanyHome(),
        // "/app:dictionary/Scheduled Actions");
        //       Log error if result not found
        //       Log warning if multiple results found
        List<ChildAssociationRef> dictionaryAssocs =
                startupNodeService.getChildAssocs(
        ...
        List<ChildAssociationRef> scheduledAssocs =
                startupNodeService.getChildAssocs(
                        dataDictionary,
                        ContentModel.ASSOC_CONTAINS,
                        QName.createQName(
                                NamespaceService.CONTENT_MODEL_1_0_URI, "Scheduled Actions"));
    ...
    /**
     * Find all our previously persisted scheduled actions, and tell the scheduler to start handling
     * them. Called by spring when startup is complete.
     */
    ...
    /**
     * Saves the changes to the schedule to the repository, and updates the Scheduler with any
     * changed details.
     */
    ...
    private void updatePersistentSchedule(ScheduledPersistedActionImpl schedule) {
        ...
        if (nodeRef == null) throw new IllegalStateException("Must be persisted first");

        // update schedule properties
        nodeService.setProperty(nodeRef, ActionModel.PROP_START_DATE, schedule.getScheduleStart());
        nodeService.setProperty(
                nodeRef, ActionModel.PROP_INTERVAL_COUNT, schedule.getScheduleIntervalCount());
        IntervalPeriod period = schedule.getScheduleIntervalPeriod();
        nodeService.setProperty(
                nodeRef, ActionModel.PROP_INTERVAL_PERIOD, period == null ? null : period.name());
        ...
        // NOTE: can only associate to a single action from a schedule (as specified by the action
        // model)
            ...
            if (actionNodeRef == null) {
                if (actionAssoc != null) {
                    ...
                    nodeService.removeAssociation(
                            actionAssoc.getSourceRef(),
                            actionAssoc.getTargetRef(),
                            actionAssoc.getTypeQName());
                }
            } else {
                if (actionAssoc == null) {
                    // create associated action
                    nodeService.createAssociation(
                            nodeRef, actionNodeRef, ActionModel.ASSOC_SCHEDULED_ACTION);
    ...
    /**
     * Removes the schedule for the action, and cancels future executions of it. The persisted
     * action is unchanged.
     */
    ...
    public ScheduledPersistedAction getSchedule(NodeRef persistedActionNodeRef) {
        ...
        List<AssociationRef> assocs =
                nodeService.getSourceAssocs(
                        persistedActionNodeRef, ActionModel.ASSOC_SCHEDULED_ACTION);
    ...
    private List<ScheduledPersistedAction> listSchedules(NodeService nodeService) {
        List<ChildAssociationRef> childAssocs =
                nodeService.getChildAssocs(SCHEDULED_ACTION_ROOT_NODE_REF, ACTION_TYPES);

        List<ScheduledPersistedAction> scheduledActions =
                new ArrayList<ScheduledPersistedAction>(childAssocs.size());
        for (ChildAssociationRef actionAssoc : childAssocs) {
            ScheduledPersistedActionImpl scheduleImpl =
                    loadPersistentSchedule(actionAssoc.getChildRef());
    ...
    protected ScheduledPersistedActionImpl loadPersistentSchedule(NodeRef schedule) {
        if (!nodeService.exists(schedule)) return null;
        ...
        scheduleImpl.setScheduleLastExecutedAt(
                (Date) nodeService.getProperty(schedule, ActionModel.PROP_LAST_EXECUTED_AT));

        scheduleImpl.setScheduleStart(
                (Date) nodeService.getProperty(schedule, ActionModel.PROP_START_DATE));
        scheduleImpl.setScheduleIntervalCount(
                (Integer) nodeService.getProperty(schedule, ActionModel.PROP_INTERVAL_COUNT));
        String period =
                (String) nodeService.getProperty(schedule, ActionModel.PROP_INTERVAL_PERIOD);
        if (period != null) {
    ...
    private AssociationRef findActionAssociationFromSchedule(NodeRef schedule) {
        List<AssociationRef> assocs =
                nodeService.getTargetAssocs(schedule, ActionModel.ASSOC_SCHEDULED_ACTION);
    ...
    protected void removeFromScheduler(ScheduledPersistedActionImpl schedule) {
        ...
        if (schedule.getPersistedAtNodeRef() == null) return;

        // Ask to remove it
        try {
            scheduler.deleteJob(
                    new JobKey(schedule.getPersistedAtNodeRef().toString(), SCHEDULER_GROUP));
    ...
    /**
     * Builds up the Quartz details, and adds it to the Quartz scheduler when the transaction
     * completes. We have to wait for the transaction to finish, otherwise Quartz may end up trying
     * and failing to load the details of a job that hasn't been committed to the repo yet!
     */
    ...
    /**
     * This is used to trigger the loading of previously persisted schedules on an application
     * startup. It is an additional bean to make the context files cleaner.
     */
    public static class ScheduledPersistedActionServiceBootstrap extends AbstractLifecycleBean {
        ...
        public void setScheduledPersistedActionService(
                ScheduledPersistedActionServiceImpl scheduledPersistedActionService) {
    ...
    /**
     * The thing that Quartz runs when the schedule fires. Handles fetching the action, and having
    ...
    public static class ScheduledJobWrapper implements Job, ApplicationContextAware {
        ...
        public void setApplicationContext(ApplicationContext applicationContext) {
            ...
            runtimeActionService =
                    (RuntimeActionService) applicationContext.getBean("actionService");
        }
                    ...
                    .doInTransaction(
                            new RetryingTransactionCallback<Void>() {
                                public Void execute() throws Throwable {
                                    ...
                                    NodeRef scheduleNodeRef =
                                            new NodeRef(
                                                    jobContext
                                                            .getMergedJobDataMap()
                                                            .getString(JOB_SCHEDULE_NODEREF));
                                    nodeService.setProperty(
                                            scheduleNodeRef,
                                    ...
                                    NodeRef actionNodeRef =
                                            new NodeRef(
                                                    jobContext
                                                            .getMergedJobDataMap()
                                                            .getString(JOB_ACTION_NODEREF));
                                    Action action =
                                            runtimeActionService.createAction(actionNodeRef);

                                    // Have it executed asynchronously
                                    actionService.executeAction(
                                            action, (NodeRef) null, false, true);

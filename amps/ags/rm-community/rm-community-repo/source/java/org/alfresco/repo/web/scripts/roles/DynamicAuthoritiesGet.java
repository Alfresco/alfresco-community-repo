...
public class DynamicAuthoritiesGet
  ...
  private static final String MESSAGE_PARAMETER_BATCHSIZE_GREATER_THAN_ZERO =
    "Parameter batchsize should be a number greater than 0.";
  ...
  private static final String MESSAGE_PROCESSING_RECORD_END_TEMPLATE =
    "Processing record {0} - END";
  private static final String MESSAGE_PROCESSING_RECORD_BEGIN_TEMPLATE =
    "Processing record {0} - BEGIN";
  private static final String MESSAGE_BATCHSIZE_IS_INVALID =
    "Parameter batchsize is invalid.";
  private static final String MESSAGE_BATCHSIZE_IS_MANDATORY =
    "Parameter batchsize is mandatory";
  private static final String MESSAGE_NODE_REF_DOES_NOT_EXIST_TEMPLATE =
    "Parameter parentNodeRef = {0} does not exist.";
  ...
  private static final String MESSAGE_PARTIAL_TEMPLATE =
    "Processed first {0} records.";
  private static final String MESSAGE_NO_RECORDS_TO_PROCESS =
    "There where no records to be processed.";
  ...
  protected Map<String, Object> buildModel(
    ...
    final Pair<Long, QName> recordAspectPair = qnameDAO.getQName(
    ...
    Long totalNumberOfRecordsToProcess = getMaxToProccessParameter(
      req,
      batchSize
    );
      ...
      if (parentNodeRef != null) {
        processedNodes =
          processChildrenNodes(
            ...
            batchSize.intValue(),
            recordAspectPair,
            totalNumberOfRecordsToProcess.intValue(),
            out,
            attach
          );
      } else {
    ...
    String message = "";
    if (
      totalNumberOfRecordsToProcess == 0 ||
      (
        totalNumberOfRecordsToProcess > 0 &&
        processedNodesSize < totalNumberOfRecordsToProcess
      )
    ) {
    ...
    if (
      totalNumberOfRecordsToProcess > 0 &&
      totalNumberOfRecordsToProcess == processedNodesSize
    ) {
      message =
        MessageFormat.format(
          MESSAGE_PARTIAL_TEMPLATE,
          totalNumberOfRecordsToProcess
        );
    }
    model.put(MODEL_STATUS, SUCCESS_STATUS);
  ...
  public void execute(WebScriptRequest req, WebScriptResponse res)
    ...
    try {
      ...
      if (
        statusCode != HttpServletResponse.SC_OK && !req.forceSuccessStatus()
      ) {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Force success status header in response: " +
      ...
      if (format.equals(WebScriptResponse.JSON_FORMAT) && callback != null) {
        if (logger.isDebugEnabled()) logger.debug(
          "Rendering JSON callback response: content type=" +
          Format.JAVASCRIPT.mimetype() +
          ", status=" +
          statusCode +
          ", callback=" +
          callback
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "Rendering response: content type=" +
          mimetype +
          ", status=" +
          statusCode
        );
  ...
  protected void renderFormatTemplate(
    String format,
   ...
   */
  protected Long getMaxToProccessParameter(
    WebScriptRequest req,
    final Long batchSize
  ) {
    String totalToBeProcessedRecordsStr = req.getParameter(
    ...
    if (StringUtils.isNotBlank(totalToBeProcessedRecordsStr)) {
      try {
        totalNumberOfRecordsToProcess =
          Long.parseLong(totalToBeProcessedRecordsStr);
  ...
  protected Long getBatchSizeParameter(WebScriptRequest req) {
    ...
    if (StringUtils.isBlank(batchSizeStr)) {
      ...
      throw new WebScriptException(
        ...
        MESSAGE_BATCHSIZE_IS_MANDATORY
      );
    }
    try {
    ...
    if (StringUtils.isNotBlank(parentNodeRefStr)) {
      ...
      if (!nodeService.exists(parentNodeRef)) {
        String message = MessageFormat.format(
          MESSAGE_NODE_REF_DOES_NOT_EXIST_TEMPLATE,
  ...
  protected List<NodeRef> processNodes(
    final Long batchSize,
    Long maxNodeId,
    final Pair<Long, QName> recordAspectPair,
    ...
    for (Long i = 0L; i < maxNodeId; i += batchSize) {
      if (
        maxRecordsToProcess != 0 && processedNodes.size() >= maxRecordsToProcess
      ) {
        break;
      }
            ...
            public Void execute() throws Throwable {
              ...
              for (Long nodeId : nodeIds) {
                if (
                  ...
                  processedNodes.size() >= maxRecordsToProcess
                ) {
                  break;
                }
                NodeRef record = nodeDAO.getNodePair(nodeId).getSecond();
                String recordName = (String) nodeService.getProperty(
                  record,
  ...
  protected List<NodeRef> processChildrenNodes(
    NodeRef parentNodeRef,
    final int batchSize,
    final Pair<Long, QName> recordAspectPair,
    final int maxRecordsToProcess,
    final BufferedWriter out,
    final boolean attach
  ) {
    final List<NodeRef> processedNodes = new ArrayList<>();
    final List<FileInfo> children = fileFolderService.search(
      parentNodeRef,
      "*",
      /*filesSearch*/true,
      /*folderSearch*/true,
    ...
    for (int i = 0; i < children.size(); i += batchSize) {
      ...
      transactionService
        ...
        .doInTransaction(
          new RetryingTransactionCallback<Void>() {
            public Void execute() throws Throwable {
              List<FileInfo> nodes = children.subList(
                currentIndex,
                Math.min(currentIndex + batchSize, children.size())
              );
              // process each one
              for (FileInfo node : nodes) {
                ...
                if (
                  nodeService.hasAspect(record, recordAspectPair.getSecond())
                ) {
                  String recordName = (String) nodeService.getProperty(
                    record,
                    ContentModel.PROP_NAME
  ...
  protected void processNode(NodeRef nodeRef) {
    ...
    Map<String, Integer> readers = (Map<String, Integer>) nodeService.getProperty(
      nodeRef,
      PROP_READERS
    );
    Map<String, Integer> writers = (Map<String, Integer>) nodeService.getProperty(
      nodeRef,
      PROP_WRITERS
    );
    ...
    permissionService.clearPermission(
      nodeRef,
      ExtendedReaderDynamicAuthority.EXTENDED_READER
    );
    permissionService.clearPermission(
      nodeRef,

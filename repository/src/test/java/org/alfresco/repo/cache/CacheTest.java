...
public class CacheTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        backingCacheNoStats =
                (SimpleCache<String, ValueHolder<Object>>) ctx.getBean("backingCacheNoStats");
        transactionalCache = (TransactionalCache<String, Object>) ctx.getBean("transactionalCache");
        transactionalCacheNoStats =
                (TransactionalCache<String, Object>) ctx.getBean("transactionalCacheNoStats");
    ...
    public void testTransactionalCacheNoTxn() throws Exception {
        ...
        assertEquals(
                "Backing cache not used for put when no transaction present",
                value,
                TransactionalCache.getSharedCacheValue(backingCache, key, null));

        // remove the value from the backing cache and check that it is removed from the transaction
        // cache
        backingCache.remove(key);
        assertNull(
                "Backing cache not used for removed when no transaction present",
        ...
        assertNull(
                "Non-transactional remove didn't go to backing cache",
                TransactionalCache.getSharedCacheValue(backingCache, key, null));
    }
    ...
    public void testTransactionalCacheWithSingleTxn() throws Throwable {
        ...
        TransactionalCache.putSharedCacheValue(
                backingCache, NEW_GLOBAL_THREE, NEW_GLOBAL_THREE, null);
        ...
        try {
            ...
            transactionalCache.remove(NEW_GLOBAL_ONE);
            assertFalse(
                    "Item was not removed from txn cache",
                    transactionalCache.contains(NEW_GLOBAL_ONE));
            assertNull("Get didn't return null", transactionalCache.get(NEW_GLOBAL_ONE));
            assertTrue(
                    "Item was removed from backing cache", backingCache.contains(NEW_GLOBAL_ONE));

            // read 2 from the cache
            assertEquals(
                    "Item not read from backing cache",
                    NEW_GLOBAL_TWO,
                    transactionalCache.get(NEW_GLOBAL_TWO));
            // Change the backing cache
            TransactionalCache.putSharedCacheValue(
                    backingCache, NEW_GLOBAL_TWO, NEW_GLOBAL_TWO + "-updated", null);
            // Ensure read-committed
            assertEquals(
                    "Read-committed not preserved",
                    NEW_GLOBAL_TWO,
                    transactionalCache.get(NEW_GLOBAL_TWO));
            ...
            assertFalse(
                    "Transactionally removed item found in keys",
                    transactionalKeys.contains(NEW_GLOBAL_ONE));
            assertTrue(
                    "Transactionally added item not found in keys",
                    transactionalKeys.contains(UPDATE_TXN_THREE));

            // Register a post-commit cache reader to make sure that nothing blows up if the cache
            // is hit in post-commit
            PostCommitCacheReader listenerReader =
                    new PostCommitCacheReader(transactionalCache, UPDATE_TXN_THREE);
            AlfrescoTransactionSupport.bindListener(listenerReader);

            // Register a post-commit cache reader to make sure that nothing blows up if the cache
            // is hit in post-commit
            PostCommitCacheWriter listenerWriter =
                    new PostCommitCacheWriter(transactionalCache, UPDATE_TXN_FOUR, "FOUR");
            ...
            assertFalse(
                    "Item was not removed from backing cache",
                    backingCache.contains(NEW_GLOBAL_ONE));
            assertNull(
                    "Item could still be fetched from backing cache",
                    TransactionalCache.getSharedCacheValue(backingCache, NEW_GLOBAL_ONE, null));
            assertEquals(
                    "Item not updated in backing cache",
                    "XXX",
            ...
            assertEquals(
                    "Transactional cache must serve post-commit get requests",
                    "XXX",
                    transactionalCache.get(UPDATE_TXN_THREE));
        } catch (Throwable e) {
    ...
    /**
     * This transaction listener attempts to read from the cache in the afterCommit phase.
     * Technically the transaction has finished, but the transaction resources are still available.
     *
     * @author Derek Hulley
     * @since 2.1
     */
    ...
    /**
     * This transaction listener attempts to write to the cache in the afterCommit phase.
     * Technically the transaction has finished, but the transaction resources are still available.
     *
     * @author Derek Hulley
     * @since 2.1
    ...
    private class PostCommitCacheWriter extends TransactionListenerAdapter {
        ...
        private PostCommitCacheWriter(
                SimpleCache<String, Object> transactionalCache, String key, Object value) {
            this.transactionalCache = transactionalCache;
            this.key = key;
            this.value = value;
        }
    ...
    public void testTransactionalCacheDisableSharedCaches() throws Throwable {
        ...
        try {
            ...
            assertNull(
                    "Read of mutable shared cache MUST NOT use backing cache",
                    transactionalCache.get(NEW_GLOBAL_ONE));
            assertNull(
                    "Value should not be in any cache", transactionalCache.get(UPDATE_TXN_THREE));
            ...
            assertNull(
                    ...
                    transactionalCache.get(NEW_GLOBAL_ONE));
            assertEquals(
                    "Value should be in transactional cache",
                    "An update",
                    transactionalCache.get(NEW_GLOBAL_TWO));
            assertEquals(
                    "Value should be in transactional cache",
            ...
            assertEquals(
                    "Out-of-txn read must return shared value",
                    NEW_GLOBAL_ONE,
                    transactionalCache.get(NEW_GLOBAL_ONE));
            assertNull(
                    "Value should be removed from shared cache",
                    transactionalCache.get(NEW_GLOBAL_TWO));
            assertEquals(
                    "New values must be written to shared cache",
                    UPDATE_TXN_THREE,
                    transactionalCache.get(UPDATE_TXN_THREE));
        } catch (Throwable e) {
            if (txn.getStatus() == Status.STATUS_ACTIVE) {
    ...
    /**
     * Preloads the cache, then performs a simultaneous addition of N new values and removal of the
     * N preloaded values.
     *
     * @param cache
     * @param objectCount
    ...
    /**
     * Tests a straight Ehcache adapter against a transactional cache both in and out of a
     * transaction. This is done repeatedly, pushing the count up.
     */
    ...
    /**
     * Starts off with a <tt>null</tt> in the backing cache and adds a value to the transactional
     * cache. There should be no problem with this.
     */
    ...
    /**
     * Add 50K objects into the transactional cache and checks that the first object added has been
     * discarded.
     */
    public void testMaxSizeOverrun() throws Exception {
        ...
        try {
            ...
            assertEquals(
                    "The start value isn't correct", startValue, transactionalCache.get(startKey));
    ...
    private void executeAndCheck(
            ...
            boolean mustContainKey)
            throws Throwable {
        if (expectedValue != null && !mustContainKey) {
            throw new IllegalArgumentException(
                    "Why have a value when the key should not be there?");
        }
    ...
    public void testValueLockingInTxn() throws Exception {
        ...
        TransactionalCache.putSharedCacheValue(
                backingCache, DEFINITIVE_THREE, "initial_three", null);
        ...
        try {
            ...
            // Add
            {
                ...
                assertFalse(
                        "Key should not be locked, yet.",
                        transactionalCache.isValueLocked(DEFINITIVE_ONE));
                // Mark it as definitive
                transactionalCache.lockValue(DEFINITIVE_ONE);
                assertTrue(
                        "Key should be locked.", transactionalCache.isValueLocked(DEFINITIVE_ONE));
                // Attempt update
                transactionalCache.put(DEFINITIVE_ONE, "update_one");
                assertEquals(
                        "Update values should be locked.",
                        DEFINITIVE_ONE,
            ...
            // Update
            {
                ...
                assertFalse(
                        "Key should not be locked, yet.",
                        transactionalCache.isValueLocked(DEFINITIVE_TWO));
                // Mark it as definitive
                transactionalCache.lockValue(DEFINITIVE_TWO);
                assertTrue(
                        "Key should be locked.", transactionalCache.isValueLocked(DEFINITIVE_TWO));
                // Attempt update
            ...
            // Remove
            {
                ...
                assertFalse(
                        "Key should not be locked, yet.",
                        transactionalCache.isValueLocked(DEFINITIVE_THREE));
                // Mark it as definitive
                transactionalCache.lockValue(DEFINITIVE_THREE);
                assertTrue(
                        "Key should be locked.",
                        transactionalCache.isValueLocked(DEFINITIVE_THREE));
                // Attempt update
                transactionalCache.put(DEFINITIVE_THREE, "add_three");
                assertEquals(
                        "Removal should be locked.",
                        null,
                        transactionalCache.get(DEFINITIVE_THREE));
            }
            ...
            assertEquals(
                    "Definitive change not written through.",
                    DEFINITIVE_ONE,
     ...
     *
     * <ul>
     *   <li>Add to the transaction cache
     *   <li>Add to the backing cache
     *   <li>Commit
     * </ul>
     */
    public void testConcurrentAddAgainstAdd() throws Throwable {
        RetryingTransactionCallback<Object> callback =
                new RetryingTransactionCallback<Object>() {
                    public Object execute() throws Throwable {
                        ...
                        TransactionalCache.putSharedCacheValue(
                                backingCache, COMMON_KEY, VALUE_ONE_B, null);
                        return null;
                    }
                };
        transactionalCache.setAllowEqualsChecks(false);
        ...
        executeAndCheck(
                ...
                COMMON_KEY,
                VALUE_ONE_B,
                true); // Immutable: Assume backing cache is correct
        executeAndCheck(
                callback,
                true,
                COMMON_KEY,
        ...
        executeAndCheck(
                ...
                COMMON_KEY,
                VALUE_ONE_B,
                true); // Mutable: Shared cache value checked
        executeAndCheck(
                callback,
                true,
                COMMON_KEY,
    ...
    public void testConcurrentAddAgainstAddSame() throws Throwable {
        RetryingTransactionCallback<Object> callback =
                new RetryingTransactionCallback<Object>() {
                    public Object execute() throws Throwable {
                        transactionalCache.put(COMMON_KEY, VALUE_ONE_A);
                        TransactionalCache.putSharedCacheValue(
                                backingCache, COMMON_KEY, VALUE_ONE_A, null);
                        return null;
                    }
                };
        transactionalCache.setAllowEqualsChecks(false);
        ...
        transactionalCache.setMutable(false);
        executeAndCheck(
                callback, false, COMMON_KEY, VALUE_ONE_A, true); // Immutable: Assumed to be same
        executeAndCheck(
                callback, true, COMMON_KEY, VALUE_ONE_A, true); // Immutable: Assumed to be same

        transactionalCache.setAllowEqualsChecks(true);
        transactionalCache.setMutable(true);
        executeAndCheck(
                callback, false, COMMON_KEY, VALUE_ONE_A, true); // Mutable: Equality check done
        executeAndCheck(
                callback, true, COMMON_KEY, VALUE_ONE_A, true); // Mutable: Equality check done
        transactionalCache.setMutable(false);
        executeAndCheck(
                callback, false, COMMON_KEY, VALUE_ONE_A, true); // Immutable: Assumed to be same
        executeAndCheck(
     ...
     * <ul>
     *   <li>Add to the transaction cache
     *   <li>Add <tt>null</tt> to the backing cache
     *   <li>Commit
     * </ul>
     */
        ...
        RetryingTransactionCallback<Object> callback =
                new RetryingTransactionCallback<Object>() {
                    public Object execute() throws Throwable {
                        transactionalCache.put(COMMON_KEY, VALUE_ONE_A);
                        TransactionalCache.putSharedCacheValue(
                                backingCache, COMMON_KEY, null, null);
                        return null;
                    }
                };
        transactionalCache.setAllowEqualsChecks(false);
    ...
    public void testConcurrentAddAgainstClear() throws Throwable {
        ...
        executeAndCheck(
                ...
                COMMON_KEY,
                VALUE_ONE_A,
                true); // Mutable: Add back to backing cache
        executeAndCheck(
                callback,
                true,
                COMMON_KEY,
        ...
        executeAndCheck(
                ...
                COMMON_KEY,
                VALUE_ONE_A,
                true); // Immutable: Add back to backing cache
        executeAndCheck(
                callback,
                true,
                COMMON_KEY,
     ...
     * <ul>
     *   <li>Add to the backing cache
     *   <li>Update the transactional cache
     *   <li>Update the backing cache
     *   <li>Commit
     * </ul>
     */
    public void testConcurrentUpdateAgainstUpdate() throws Throwable {
        RetryingTransactionCallback<Object> callback =
                new RetryingTransactionCallback<Object>() {
                    public Object execute() throws Throwable {
                        ...
                        TransactionalCache.putSharedCacheValue(
                                backingCache, COMMON_KEY, VALUE_TWO_A, null);
                        return null;
                    }
                };
        transactionalCache.setAllowEqualsChecks(false);
        ...
        executeAndCheck(
                ...
                COMMON_KEY,
                null,
                false); // Mutable: Shared cache value checked failed
        executeAndCheck(
                callback,
                true,
                COMMON_KEY,
     ...
     *   <li>Add to the backing cache
     *   <li>Update the transactional cache
     *   <li>Update the backing cache with a <tt>null</tt> value
     *   <li>Commit
     * </ul>
     */
     ...
     * <ul>
     *   <li>Add to the backing cache
     *   <li>Update the transactional cache with a <tt>null</tt> value
     *   <li>Update the backing cache
     *   <li>Commit
     * </ul>
     */
     ...
     *   <li>Add to the backing cache
     *   <li>Update the transactional cache
     *   <li>Remove from the backing cache
     *   <li>Commit
     * </ul>
     */
     ...
     * <ul>
     *   <li>Remove from the backing cache
     *   <li>Remove from the transactional cache
     *   <li>Add to the backing cache
     *   <li>Commit
     * </ul>
     */
    public void testConcurrentRemoveAgainstUpdate_NoPreExisting() throws Throwable {
        ...
        transactionalCache.setMutable(false);
        executeAndCheck(
                callback, false, COMMON_KEY, null, false); // Immutable: Remove from backing cache
        executeAndCheck(
                callback, true, COMMON_KEY, null, false); // Immutable: Remove from backing cache
     ...
     * <ul>
     *   <li>Remove from the backing cache
     *   <li>Add to the transactional cache
     *   <li>Add to the backing cache
     *   <li>Commit
     * </ul>
     */
    ...
    public void testConcurrentRemoveAgainstRemove() throws Throwable {
        ...
        transactionalCache.setMutable(true);
        executeAndCheck(
                callback, false, COMMON_KEY, null, false); // Mutable: Remove from backing cache
        executeAndCheck(
                callback, true, COMMON_KEY, null, false); // Mutable: Remove from backing cache
        transactionalCache.setMutable(false);
        executeAndCheck(
                callback, false, COMMON_KEY, null, false); // Immutable: Remove from backing cache
        executeAndCheck(
    ...
    public void testTransactionalCacheStatsOnCommit() throws Throwable {
        ...
        try {
            ...
            // TODO: remove is called twice for each remove (in beforeCommit and afterCommit) -
            // check this is correct.
            assertEquals(
                    removesAtStart + 16, cacheStats.count("transactionalCache", OpType.REMOVE));
            assertEquals(hitsAtStart + 3, cacheStats.count("transactionalCache", OpType.GET_HIT));
            assertEquals(
                    missesAtStart + 4, cacheStats.count("transactionalCache", OpType.GET_MISS));
    ...
    public void testTransactionalCacheStatsForClears() throws Throwable {
        ...
        try {
            ...
            // There are no removes or puts propagated to the shared cache, as a result of the
            // clears.

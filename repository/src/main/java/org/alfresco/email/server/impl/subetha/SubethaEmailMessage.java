...
public class SubethaEmailMessage implements EmailMessage {
    ...
    public SubethaEmailMessage(InputStream dataInputStream) {
        ...
        try {
            mimeMessage =
                    new MimeMessage(
                            Session.getDefaultInstance(System.getProperties()), dataInputStream);
        ...
        if (body != null) {
            attachmentList.add(
                    new SubethaEmailMessagePart(
                            messagePart,
                            getPartFileName(
                                    getSubject() + " (part " + ++bodyNumber + ")", messagePart)));
            ...
            body =
                    new SubethaEmailMessagePart(
                            messagePart, getPartFileName(getSubject(), messagePart));
            if (log.isDebugEnabled()) {
    ...
    private void addAttachment(Part messagePart) throws MessagingException {
        String fileName =
                getPartFileName(FILENAME_ATTACHMENT_PREFIX + attachmentNumber, messagePart);
    ...
    /**
     * Method extracts file name from a message part for saving its as aa attachment. If the file
     * name can't be extracted, it will be generated based on defaultPrefix parameter.
     *
    ...
    private String getPartFileName(String defaultPrefix, Part messagePart)
        ...
        if (fileName != null) {
            ...
            if (messagePart.isMimeType(MIME_PLAIN_TEXT)) fileName += ".txt";
            else if (messagePart.isMimeType(MIME_HTML_TEXT)) fileName += ".html";
            else if (messagePart.isMimeType(MIME_XML_TEXT)) fileName += ".xml";
            else if (messagePart.isMimeType(MIME_IMAGE)) fileName += ".gif";
        }
        return fileName;
    }
    ...
    public void setRmiRegistry(String rmiRegistryHost, int rmiRegistryPort) {
        ...
        for (EmailMessagePart attachment : attachments) {
            if (attachment instanceof SubethaEmailMessagePart) {
                ((SubethaEmailMessagePart) attachment)
                        .setRmiRegistry(rmiRegistryHost, rmiRegistryPort);
            }
        }
    }

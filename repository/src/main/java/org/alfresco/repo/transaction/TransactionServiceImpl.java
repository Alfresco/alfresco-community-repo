 ...
 *
 * <p>Default retry behaviour: see {@link RetryingTransactionHelper#RetryingTransactionHelper()}
 *
 * @author David Caruana
 */
public class TransactionServiceImpl implements TransactionService {
    private static VmShutdownListener shutdownListener =
            new VmShutdownListener("TransactionService");
    ...
    private final QName generalVetoName =
            QName.createQName(NamespaceService.APP_MODEL_1_0_URI, "GeneralVeto");
     ...
     *
     * <p>Intended for use by spring configuration only. Alfresco code should call the method which
     ...
     * Set the read-only mode for all generated transactions.
     *
     * <p>By default read/write transactions are allowed however vetos may be applied that make the
     * transactions read only.
     *
     * <p>Prevent writes by calling allowWrite with false and a given veto name.
     *
     * <p>The veto is removed by calling allowWrite with true for the given veto name when all vetos
     * are removed then read/write transactions are allowed.
     *
     * @param allowWrite false if all transactions must be read-only
     * @param nameOfVeto the name of the veto
     */
            ...
            if (allowWrite) {
                ...
                if (removed) {
                    logger.warn(
                            "setAllowWrite:"
                                    + allowWrite
                                    + ".  Removing veto on write operations: "
                                    + nameOfVeto);
                }
            } else {
                ...
                logger.warn(
                        "setAllowWrite:"
                                + allowWrite
                                + ".  Applying veto on write operations: "
                                + nameOfVeto);
            }
        } finally {
    ...
    public UserTransaction getUserTransaction(boolean readOnly, boolean ignoreSystemReadOnly) {
        if (ignoreSystemReadOnly) {
            SpringAwareUserTransaction txn =
                    new SpringAwareUserTransaction(
                            transactionManager,
                            (readOnly),
    ...
    /**
     * Creates a new helper instance. It can be reused or customized by the client code: each
     * instance is new and initialized afresh.
     */

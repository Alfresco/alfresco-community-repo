...
public class RM3993Test extends BaseRMTestCase {
  ...
  public void testAutoDeclareAutoFileCreateRecordFolderOnly() throws Exception {
    doTestInTransaction(
      new Test<Void>() {
        ...
        public Void run() {
          ...
          ruleFolder =
            fileFolderService
              .create(documentLibrary, "mytestfolder", ContentModel.TYPE_FOLDER)
              .getNodeRef();
    ...
    List<NodeRef> records = new ArrayList<>(
      NUMBER_OF_BATCHES * NUMBER_IN_BATCH
    );
          ...
          new Test<Void>() {
            ...
            public Void run() throws Exception {
              while (temp.hasNext()) {
                ...
                if (
                  nodeService.hasAspect(record, ASPECT_RECORD) &&
    ...
    Integer numberOfRecords = AuthenticationUtil.runAsSystem(
      new RunAsWork<Integer>() {
        @Override
        public Integer doWork() throws Exception {
          List<NodeRef> containedRecordFolders = filePlanService.getContainedRecordFolders(
            nodeRefCategory1
          );
          int numberOfRecords = 0;
          for (NodeRef recordFolder : containedRecordFolders) {
            numberOfRecords =
              numberOfRecords + fileFolderService.list(recordFolder).size();
          }

...
public class LinksRestApiTest extends BaseWebScriptTest {
    ...
    private JSONObject createLink(
            String title, String description, String url, boolean internal, int expectedStatus)
            throws Exception {
    ...
    private JSONObject updateLink(
        ...
        Response response =
                sendRequest(
                        new PutRequest(
                                URL_LINKS_UPDATE + name, json.toString(), "application/json"),
                        expectedStatus);
        if (expectedStatus == Status.STATUS_OK) {
            JSONObject result = new JSONObject(response.getContentAsString());
            if (result.has("links")) {
    ...
    public void testCreateEditDeleteEntry() throws Exception {
        ...
        link =
                updateLink(
                        name, LINK_TITLE_ONE, "More Thing 1", LINK_URL_ONE, true, Status.STATUS_OK);
    ...
    public void testXssLinks() throws Exception {
        ...
        mapForCheck.put(
                "<iframe src=http://ha.ckers.org/scriptlet.html <", Status.STATUS_BAD_REQUEST);
        ...
        mapForCheck.put(
                "BODY onload!#$%&()*~+-_.,:;?@[/|\\]^`=alert(\"XSS\")>", Status.STATUS_BAD_REQUEST);
    ...
    public void testOverallListing() throws Exception {
        ...
        this.authenticationComponent.setCurrentUser(USER_TWO);
        JSONObject link3 =
                createLink(LINK_TITLE_THREE, "Thing 3", LINK_URL_THREE, true, Status.STATUS_OK);
        String name3 = getNameFromLink(link3);
        updateLink(
                name3, LINK_TITLE_THREE, "More Where 3", LINK_URL_THREE, false, Status.STATUS_OK);
        this.authenticationComponent.setCurrentUser(USER_ONE);
    ...
    public void testCreateLinkPermission() throws Exception {
        ...
        this.siteService.createSite(
                "LinkSitePreset", siteName, "SiteTitle", "SiteDescription", SiteVisibility.PUBLIC);
    ...
    public void testCommentLink() throws Exception {
        ...
        JSONObject link =
                createLink(LINK_TITLE_ONE, "commented link", LINK_URL_ONE, false, Status.STATUS_OK);
        postLookup.execute();

...
public class HttpAlfrescoContentReader extends AbstractContentReader {
  ...
  private static final String ERR_NO_CONNECTION =
    "content.http_reader.err.no_connection";
  private static final String ERR_NO_AUTHENTICATION =
    "content.http_reader.err.no_authentication";
  private static final String ERR_CHECK_CLUSTER =
    "content.http_reader.err.check_cluster";
  private static final String ERR_UNRECOGNIZED =
    "content.http_reader.err.unrecognized";
  ...
  private static Log logger = LogFactory.getLog(
    HttpAlfrescoContentReader.class
  );
  ...
  public String toString() {
    ...
    sb
      .append("HttpAlfrescoContentReader")
      .append("[ contentUrl=")
  ...
  private void getInfoImpl() {
    ...
    try {
      String ticket = transactionService
        .getRetryingTransactionHelper()
        .doInTransaction(ticketCallback, false, true);
      String url = HttpAlfrescoContentReader.generateURL(
        baseHttpUrl,
        contentUrl,
        ticket,
        true
      ...
      if (statusCode == HttpServletResponse.SC_OK) {
        ...
        String responseSize = method
          .getResponseHeader("alfresco.dr.size")
          .getValue();
        String responseLastModified = method
          .getResponseHeader("alfresco.dr.lastModified")
          .getValue();
        String responseMimetype = method
          .getResponseHeader("alfresco.dr.mimetype")
          .getValue();
        String responseEncoding = method
          .getResponseHeader("alfresco.dr.encoding")
          .getValue();
        String responseLocale = method
          .getResponseHeader("alfresco.dr.locale")
          .getValue();
        // Fill in this reader's values
        cachedSize =
          DefaultTypeConverter.INSTANCE.convert(Long.class, responseSize);
        cachedLastModified =
          DefaultTypeConverter.INSTANCE
            .convert(Date.class, responseLastModified)
            .getTime();
        setMimetype(
          DefaultTypeConverter.INSTANCE.convert(String.class, responseMimetype)
        );
        setEncoding(
          DefaultTypeConverter.INSTANCE.convert(String.class, responseEncoding)
        );
        setLocale(
          DefaultTypeConverter.INSTANCE.convert(Locale.class, responseLocale)
        );
        // It exists
        cachedExists = true;
        // Done
      ...
      throw new AlfrescoRuntimeException(
        "Reader exists check failed: " + this,
        e
      );
    } finally {
      if (method != null) {
  ...
  protected ContentReader createReader() throws ContentIOException {
    return new HttpAlfrescoContentReader(
      transactionService,
  ...
  private ReadableByteChannel getDirectReadableChannelImpl()
    throws ContentIOException {
    String contentUrl = getContentUrl();
    ...
    try {
      ...
      if (statusCode == HttpServletResponse.SC_OK) {
        ...
        throw new IOException(
          "Failed to get content remote content that supposedly exists."
        );
      }
    } catch (Throwable e) {
   ...
   */
  public static final String generateURL(
    String baseHttpUrl,
    String contentUrl,
    String ticket,
    boolean infoOnly

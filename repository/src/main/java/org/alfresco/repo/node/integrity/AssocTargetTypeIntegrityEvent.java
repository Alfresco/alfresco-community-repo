...
public class AssocTargetTypeIntegrityEvent extends AbstractIntegrityEvent {
  ...
  private static Log logger = LogFactory.getLog(
    AssocTargetTypeIntegrityEvent.class
  );
  ...
  public void checkIntegrity(List<IntegrityRecord> eventResults) {
    ...
    if (targetNodeTypeQName == null) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Ignoring integrity check - node gone: \n" + "   event: " + this
        );
      }
      return;
    }
  ...
  protected void checkTargetType(
    ...
    if (targetDef instanceof TypeDefinition) {
      ...
      if (
        !dictionaryService.isSubClass(targetNodeTypeQName, targetDef.getName())
      ) {
        IntegrityRecord result = new IntegrityRecord(
      ...
      if (!found) {
        ...
        if (assocDef.isChild()) {
          ...
          if (
            nodeService
              .getSourceAssocs(targetNodeRef, assocDef.getName())
              .size() ==
            0
          ) {
            // The association does not exist any more

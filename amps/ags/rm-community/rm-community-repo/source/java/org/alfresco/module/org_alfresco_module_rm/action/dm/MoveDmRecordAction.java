...
public class MoveDmRecordAction
  ...
  protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
    ...
    if (
      !getNodeService().hasAspect(actionedUponNodeRef, ASPECT_RECORD) &&
      ...
      getInplaceRecordService()
        .moveRecord(actionedUponNodeRef, getTargetNodeRef(action));
    }
  }
  ...
  private NodeRef getTargetNodeRef(Action action) {
    String targetNodeRef = (String) action.getParameterValue(
      PARAM_TARGET_NODE_REF
    );

    if (StringUtils.isBlank(targetNodeRef)) {
      throw new AlfrescoRuntimeException(
        "Could not find target node reference."
      );
    }

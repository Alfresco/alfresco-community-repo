...
public class LockServiceImpl
  ...
  public void setLockableAspectInterceptor(
  ...
  public void init() {
    ...
    beforeLock =
      policyComponent.registerClassPolicy(LockServicePolicies.BeforeLock.class);
    beforeUnlock =
      policyComponent.registerClassPolicy(
        LockServicePolicies.BeforeUnlock.class
      );
  ...
  private void addToIgnoreSet(NodeRef nodeRef) {
    Set<NodeRef> ignoreNodeRefs = (Set<NodeRef>) AlfrescoTransactionSupport.getResource(
      KEY_IGNORE_NODES
    );
   ...
   */
  @Extend(
    traitAPI = LockServiceTrait.class,
    extensionAPI = LockServiceExtension.class
  )
  ...
  public void lock(
    ...
    if (
      lifetime.equals(Lifetime.EPHEMERAL) &&
      (timeToExpire > MAX_EPHEMERAL_LOCK_SECONDS)
    ) {
    ...
    if (
      lifetime.equals(Lifetime.EPHEMERAL) &&
      (timeToExpire > ephemeralExpiryThreshold)
    ) {
    ...
    Pair<LockState, LockStatus> statusAndState = getLockStateAndStatus(
      nodeRef,
      userName
    );
    ...
    if (LockStatus.LOCKED.equals(currentLockStatus) == true) {
      ...
      LockStatus.NO_LOCK.equals(currentLockStatus) == true ||
      ...
      if (lifetime == Lifetime.PERSISTENT) {
        ...
        LockState lock = LockState.createLock(
          nodeRef,
          lockType,
          userName,
          expiryDate,
  ...
  public void unlock(
    ...
    if (lockState.isLockInfo()) {
      ...
      if (lifetime == Lifetime.PERSISTENT) {
        ...
        throw new IllegalStateException(
          "Unhandled Lifetime value: " + lifetime
        );
      }
    }
  ...
  public LockStatus getLockStatus(NodeRef nodeRef, String userName) {
    Pair<LockState, LockStatus> stateAndStatus = getLockStateAndStatus(
      nodeRef,
      userName
    );
  ...
  private Pair<LockState, LockStatus> getLockStateAndStatus(
    NodeRef nodeRef,
    String userName
  ) {
  ...
  public LockType getLockType(NodeRef nodeRef) {
    ...
    if (
      this.nodeService.hasAspect(nodeRef, ContentModel.ASPECT_LOCKABLE) == true
    ) {
      String lockTypeString = (String) this.nodeService.getProperty(
          nodeRef,
          ContentModel.PROP_LOCK_TYPE
        );
    ...
    if (nodeRef != null && userName != null) {
      ...
      if (
        ...
        try {
          ...
          if (
            LockType.WRITE_LOCK.equals(lockType) == true &&
            ...
            LockType.READ_ONLY_LOCK.equals(lockType) == true &&
            (
              LockStatus.LOCKED.equals(currentLockStatus) == true ||
              LockStatus.LOCK_OWNER.equals(currentLockStatus) == true
            )
            ...
            LockType.NODE_LOCK.equals(lockType) == true &&
            (
              LockStatus.LOCKED.equals(currentLockStatus) == true ||
              LockStatus.LOCK_OWNER.equals(currentLockStatus) == true
            )
  ...
  public LockState getLockState(NodeRef nodeRef) {
    ...
    if (lockState != null) {
      String lockOwner = lockState.getOwner();
      Date expiryDate = lockState.getExpires();
      LockStatus status = LockUtils.lockStatus(
        lockOwner,
        lockOwner,
        expiryDate
      );
    ...
    if (lockState == null || !lockState.isLockInfo()) {
      ...
      if (
        ...
        String lockOwner = (String) nodeService.getProperty(
          nodeRef,
          ContentModel.PROP_LOCK_OWNER
        );

        Date expiryDate = (Date) nodeService.getProperty(
          nodeRef,
          ContentModel.PROP_EXPIRY_DATE
        );
        String lockTypeStr = (String) nodeService.getProperty(
          nodeRef,
          ContentModel.PROP_LOCK_TYPE
        );
        LockType lockType = lockTypeStr != null
          ...
          : null;
        String lifetimeStr = (String) nodeService.getProperty(
          nodeRef,
          ContentModel.PROP_LOCK_LIFETIME
        );
        Lifetime lifetime = lifetimeStr != null
          ...
          : null;
        String additionalInfo = (String) nodeService.getProperty(
          nodeRef,
          ContentModel.PROP_LOCK_ADDITIONAL_INFO
        );
  ...
  public void afterRollback() {
    ...
    Map<NodeRef, LockState> lockedNodes = TransactionalResourceHelper.getMap(
      KEY_MODIFIED_NODES
    );

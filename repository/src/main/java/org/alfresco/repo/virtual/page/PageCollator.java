...
public class PageCollator<R> {
  ...
  public PagingResults<R> collate(
    ...
    try {
      PagingRequest preemptiveRequest = new PagingRequest(
        preemptiveSkip,
        preemptiveSize,
        pagingRequest.getQueryExecutionId()
      );
      preemptiveRequest.setRequestTotalCountMax(
  ...
  private PagingResults<R> collate(
    ...
    final boolean endOfCollation = collate(
      objects,
      inPageList,
      pageSkip,
      pageSize,
    ...
    final Pair<Integer, Integer> total = new Pair<>(
    ...
    final boolean hasMoreItems =
      objectPageSurce.hasMoreItems() || !endOfCollation;
  ...
  private boolean collate(
    ...
    final int zeroPageSize =
      (pageSize == 0 ? collationArray.length - pageSkip : pageSize);
    final int to = Math.min(pageSkip + zeroPageSize, collationArray.length);

    collatedResult.addAll(
      Arrays.asList(Arrays.copyOfRange(collationArray, pageSkip, to))
    );

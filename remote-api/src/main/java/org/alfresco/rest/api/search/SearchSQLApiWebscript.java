...
public class SearchSQLApiWebscript
  ...
  implements
    RecognizedParamsExtractor, RequestReader, ResponseWriter, InitializingBean {
  ...
  public void execute(WebScriptRequest webScriptRequest, WebScriptResponse res)
    ...
    try {
      ...
      SearchSQLQuery searchQuery = extractJsonContent(
        webScriptRequest,
        assistant.getJsonHelper(),
        SearchSQLQuery.class
      );
      ...
      if (searchQuery.getFormat().equalsIgnoreCase("solr")) {
        ...
        CollectionWithPagingInfo<TupleList> nodes = resultMapper.toCollectionWithPagingInfo(
          ssjr.getDocs(),
          searchQuery
        );
      ...
      if (exception instanceof QueryParserException) {
        ...
        renderException(
          new WebScriptException(400, exception.getMessage()),
          res,
          assistant
        );
      }

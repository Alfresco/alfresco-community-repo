...
public class WorkflowPermissionInterceptor implements MethodInterceptor {
  ...
  public Object invoke(MethodInvocation invocation) throws Throwable {
    ...
    if (methodName.equals("getTaskById")) {
      ...
      if (
        isInitiatorOrAssignee(wt, currentUser) ||
    ...
    if (methodName.equals("getStartTask")) {
      ...
      if (
        isInitiatorOrAssignee(wt, currentUser) ||
        isUserPartOfProcess(wt, currentUser)
      ) {
        return result;
      } else {
        String taskId = (String) invocation.getArguments()[0];
    ...
    if (
      methodName.equals("getAssignedTasks") ||
  ...
  private boolean isInitiatorOrAssignee(WorkflowTask wt, String userName) {
    ...
    accessUseres.add(
      getUserGroupRef(props.get(WorkflowModel.ASSOC_GROUP_ASSIGNEE))
    );
    accessUseres.addAll(
      getUserGroupRefs(props.get(WorkflowModel.ASSOC_GROUP_ASSIGNEES))
    );
    accessUseres.addAll(
      getUserGroupRefs(props.get(WorkflowModel.ASSOC_ASSIGNEES))
    );
    accessUseres.addAll(
      getUserGroupRefs(
        wt.getProperties().get(WorkflowModel.ASSOC_POOLED_ACTORS)
      )
    );
  ...
  private boolean isStartTaskOfProcessInvolvedIn(
    WorkflowTask wt,
    String userName
  ) {
    return (
      wt.getId().contains(ActivitiConstants.START_TASK_PREFIX) &&
  ...
  private boolean fromSameParallelReviewWorkflow(
    ...
    if (
      ...
      List<WorkflowTask> allWorkflowTasks = workflowService.queryTasks(
        tasksQuery,
        true
      );

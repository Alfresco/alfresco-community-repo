...
/**
 * Requests an extract of metadata via a remote async transform using {@link
 * RenditionService2#transform(NodeRef, TransformDefinition)}. The properties that will extracted
 * are defined by the transform. This allows out of process metadata extracts to be defined without
 * the need to apply an AMP. The actual transform is a request to go from the source mimetype to
 * {@code "alfresco-metadata-extract"}. The resulting transform is a Map in json of properties and
 * values to be set on the source node.
 *
 * <p>As with other sub-classes of {@link AbstractMappingMetadataExtracter} it also supports
 * embedding of metadata in a source node. In this case the remote async transform states that it
 * supports a transform from a source mimetype to {@code "alfresco-metadata-embed"}. The resulting
...
public class AsynchronousExtractor extends AbstractMappingMetadataExtracter {
    ...
    private List<MetadataExtractorPropertyMappingOverride>
            metadataExtractorPropertyMappingOverrides = Collections.emptyList();
    ...
    public void setRenditionDefinitionRegistry2(
            RenditionDefinitionRegistry2Impl renditionDefinitionRegistry2) {
    ...
    public void setMetadataExtractorPropertyMappingOverrides(
            List<MetadataExtractorPropertyMappingOverride>
                    metadataExtractorPropertyMappingOverrides) {
    ...
    protected Map<String, Set<QName>> getDefaultMapping() {
        return Collections
                .emptyMap(); // Mappings are done by the transform, but a non null value must be
        // returned.
    }

    public boolean isSupported(String sourceMimetype, long sourceSizeInBytes) {
        return isEnabled(sourceMimetype)
                && isSupported(sourceMimetype, sourceSizeInBytes, MIMETYPE_METADATA_EXTRACT);
    }
    ...
    private boolean isSupported(
            String sourceMimetype, long sourceSizeInBytes, String targetMimetype) {
        return transformServiceRegistry.isSupported(
                sourceMimetype,
                sourceSizeInBytes,
                targetMimetype,
    ...
    public static String getTargetMimetypeFromTransformName(String transformName) {
        return transformName == null
                ? null
                : transformName.startsWith(MIMETYPE_METADATA_EXTRACT)
    ...
    public static String getSourceMimetypeFromTransformName(String transformName) {
        return transformName == null
                ? null
                : transformName.startsWith(MIMETYPE_METADATA_EXTRACT)
                        ? transformName.substring(MIMETYPE_METADATA_EXTRACT.length() + 1)
                        : transformName.startsWith(MIMETYPE_METADATA_EMBED)
                                ? transformName.substring(MIMETYPE_METADATA_EMBED.length() + 1)
                                : null;
    }

    /**
     * Returns a file extension used as the target in a transform. The normal extension is changed
     * if the {@code targetMimetype} is an extraction or embedding type.
     *
     * @param targetMimetype the target mimetype
     * @param sourceExtension normal source extension
     * @param targetExtension current target extension (normally {@code "bin" for embedding and
     *     extraction})
    ...
    public static String getExtension(
        ...
        return isMetadataExtractMimetype(targetMimetype)
                ? "json"
                : isMetadataEmbedMimetype(targetMimetype) ? sourceExtension : targetExtension;
    }

    /**
     * Returns a rendition name used in {@link TransformerDebug}. The normal name is changed if it
     * is a metadata extract or embed. The name in this case is actually the {@code
     * "alfresco-metadata-extract/"} {@code "alfresco-metadata-embed/"} followed by the source
     * mimetype.
     *
     * @param renditionName the normal name, or a special one based on the source mimetype and a
     *     prefixed.
    ...
    private Map<String, String> getExtractOptions(
            NodeRef nodeRef, ContentReader reader, MetadataExtracterLimits limits) {
        long timeoutMs = limits.getTimeoutMs();

        // This is to allow the AGS (RM) AMP to specify the mapping of properties from the
        // repository
        // rather than doing it out of process in the T-Engine.
        String sourceMimetype = reader.getMimetype();
        for (MetadataExtractorPropertyMappingOverride override :
                metadataExtractorPropertyMappingOverrides) {
    ...
    /**
     * As T-Engines do the mapping, all this method can do is convert QNames to fully qualified
     * Strings and the values to Strings or a Collection of Strings.
     *
    ...
    protected Map<String, Serializable> mapSystemToRaw(Map<QName, Serializable> systemMetadata) {
        ...
        for (Map.Entry<QName, Serializable> entry : systemMetadata.entrySet()) {
            ...
            if (serializableValue instanceof Collection<?>) {
                ...
                try {
                    String value =
                            DefaultTypeConverter.INSTANCE.convert(String.class, serializableValue);
    ...
    private void transformInBackground(
            NodeRef nodeRef,
            ContentReader reader,
            String targetMimetype,
            String embedOrExtract,
    ...
    private void transform(
        ...
        try {
            ...
            if (e.getMessage()
                    .endsWith("The supplied sourceNodeRef " + nodeRef + " does not exist.")) {
    ...
    public void setMetadata(NodeRef nodeRef, InputStream transformInputStream) {
        ...
        OverwritePolicy overwritePolicy =
                removeOverwritePolicy(metadata, "sys:overwritePolicy", OverwritePolicy.PRAGMATIC);
        ...
        List<String> stringTaggingSeparators =
                removeTaggingSeparators(
                        metadata,
                        ...
                        () ->
                                transactionService
                                        ...
                                        .doInTransaction(
                                                () -> {
                                                    // Based on:
                                                    // AbstractMappingMetadataExtracter.extract
                                                    ...
                                                    Map<QName, Serializable> systemProperties =
                                                            convertKeysToQNames(metadata);
                                                    // Convert the properties according to the
                                                    // dictionary types
                                                    systemProperties =
                                                            convertSystemPropertyValues(
                                                                    systemProperties);
                                                    // There is no last filter in the
                                                    // AsynchronousExtractor.
                                                    // Now use the proper overwrite policy
                                                    Map<QName, Serializable> changedProperties =
                                                            overwritePolicy.applyProperties(
                                                                    systemProperties,
                                                                    nodeProperties);

                                                    // Based on:
                                                    // ContentMetadataExtracter.executeImpl
                                                    // If none of the properties where changed, then
                                                    // there is nothing more to do
                                                    if (changedProperties.size() == 0) {
                                                        return null;
                                                    }
                                                    boolean transformerDebugEnabled =
                                                            transformerDebug.isEnabled();
    ...
    private Map<String, Serializable> readMetadata(InputStream transformInputStream) {
        try {
            TypeReference<HashMap<String, Serializable>> typeRef =
    ...
    private OverwritePolicy removeOverwritePolicy(
            Map<String, Serializable> map, String key, OverwritePolicy defaultValue) {
    ...
    private Boolean removeBoolean(
            Map<String, Serializable> map, Serializable key, boolean defaultValue) {
    ...
    private List<String> removeTaggingSeparators(
            Map<String, Serializable> map, String key, List<String> defaultValue) {
        Serializable value = map.remove(key);
        if (value == null) {
            return defaultValue;
        }
    ...
    private Map<QName, Serializable> convertKeysToQNames(
            Map<String, Serializable> documentMetadata) {
                                ...
                                .doInTransaction(
                                        () -> {
                                            try {
                                                ...
                                                if (logger.isTraceEnabled()) {
                                                    logger.trace(
                                                            "Embedded Metadata on "
                                                                    + nodeRef
                                                                    + " complete");
                                                }
                                            } catch (Exception e) {
                                                logger.error(
                                                        "Failed to copy embedded metadata transform"
                                                                + " InputStream into "
                                                                + nodeRef);
                                                throw e;
                                            }

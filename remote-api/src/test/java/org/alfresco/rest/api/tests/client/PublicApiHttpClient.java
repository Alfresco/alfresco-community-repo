...
/**
 * A http client for talking to the rest apis. The caller can pass in a rest api implementation
 * class to the http method (get, post, put, delete supported) and the url will be generated.
 *
 * @author steveglover
 */
public class PublicApiHttpClient {
    ...
    private static final String PUBLICAPI_CMIS_SERVICE_URL =
            "{0}://{1}:{2}{3}{4}cmis/versions/{5}/{6}";
    private static final String PUBLICAPI_CMIS_URL =
            "{0}://{1}:{2}{3}{4}{5}/{6}/cmis/versions/{7}/{8}";
    ...
    public PublicApiHttpClient(
        ...
        if (this.servletName != null
                && !this.servletName.isEmpty()
                && !this.servletName.endsWith("/")) {
        ...
        if (repositoryId == null) {
            url.append(
                    MessageFormat.format(
                            ATOM_PUB_URL,
                            new Object[] {scheme, host, String.valueOf(port), contextPath}));
        } else {
            url.append(
                    MessageFormat.format(
                            ATOM_PUB_URL,
    ...
    public String getPublicApiCmisUrl(
            String networkId, Binding binding, String version, String operation) {
        StringBuilder url = new StringBuilder();
        if (networkId == null) {
            url.append(
                    MessageFormat.format(
                            PUBLICAPI_CMIS_SERVICE_URL,
                            new Object[] {
                                scheme,
                                host,
                                String.valueOf(port),
    ...
    public String getPublicApiCmisUrlSuffix(
            String networkId, Binding binding, String version, String operation) {
        StringBuilder url = new StringBuilder();

        url.append(
                MessageFormat.format(
                        PUBLICAPI_CMIS_URL_SUFFIX,
                        new Object[] {
    ...
    public HttpResponse submitRequest(HttpMethod req, final RequestContext rq)
            throws HttpException, IOException {
        try {
            final long start = System.currentTimeMillis();

            final HttpRequestCallback<HttpResponse> callback =
                    new HttpRequestCallback<HttpResponse>() {
                        @Override
            ...
            if (rq.getPassword() != null) {
                response =
                        authenticatedHttp.executeHttpMethodAuthenticated(
                                req, rq.getRunAsUser(), rq.getPassword(), callback);
            } else if (rq.getRunAsUser() != null) {
                response =
                        authenticatedHttp.executeHttpMethodAuthenticated(
                                req, rq.getRunAsUser(), callback);
            } else {
    ...
    public HttpResponse get(
            final RequestContext rq, final String urlSuffix, Map<String, String> params)
            throws IOException {
    ...
    public HttpResponse get(
            final Class<?> c,
            final RequestContext rq,
            final Object entityId,
            final Object relationshipEntityId,
            Map<String, String> params)
            throws IOException {
        RestApiEndpoint endpoint =
                new RestApiEndpoint(c, rq.getNetworkId(), entityId, relationshipEntityId, params);
        String url = endpoint.getUrl();
    ...
    public HttpResponse get(
            ...
            final Object entityId,
            final String relationCollectionName,
            final Object relationshipEntityId,
            Map<String, String> params)
            throws IOException {
        return get(
    ...
    public HttpResponse get(
            final String urlSuffix, final RequestContext rq, Map<String, String> params)
            throws IOException {
    ...
    public HttpResponse post(
            ...
            throws IOException {
        RestApiEndpoint endpoint =
                new RestApiEndpoint(c, rq.getNetworkId(), entityId, relationshipEntityId, null);
        String url = endpoint.getUrl();

        PostMethod req = new PostMethod(url.toString());
        if (body != null) {
            StringRequestEntity requestEntity =
                    new StringRequestEntity(body, "application/json", "UTF-8");
            req.setRequestEntity(requestEntity);
        }
        return submitRequest(req, rq);
    }

    public HttpResponse post(final RequestContext rq, final String urlSuffix, String body)
            throws IOException {
    ...
    public HttpResponse post(
            ...
            throws IOException {
        RestApiEndpoint endpoint =
                new RestApiEndpoint(rq.getNetworkId(), cmisBinding, version, cmisOperation, null);
        String url = endpoint.getUrl();
    ...
    public HttpResponse get(
        ...
        RestApiEndpoint endpoint =
                new RestApiEndpoint(
                        rq.getNetworkId(), cmisBinding, version, cmisOperation, parameters);
        String url = endpoint.getUrl();
    ...
    public HttpResponse putBinary(
        ...
        PutMethod req = new PutMethod(url);
        if (payload != null) {
            BinaryRequestEntity requestEntity =
                    new BinaryRequestEntity(
                            payload.getFile(), payload.getMimeType(), payload.getCharset());
            req.setRequestEntity(requestEntity);
        }
        return submitRequest(req, rq);
    }
    ...
    private class RestApiEndpoint {
            ...
            StringBuilder sb =
                    new StringBuilder(
                            MessageFormat.format(
                                    INDEX_URL,
                                    new Object[] {
                                        scheme, host, String.valueOf(port), contextPath, servletName
                                    }));
            if (url != null) {
                sb.append(url);
            }
        ...
        RestApiEndpoint(String tenantDomain, String url, Map<String, String> params)
                throws IOException {
            StringBuilder sb =
                    new StringBuilder(
                            MessageFormat.format(
        ...
        RestApiEndpoint(
            ...
            Pair<String, String> relationshipCollectionInfo =
                    getRelationCollectionInfo(resourceClass);
        ...
        private void addParams(StringBuilder sb, Map<String, String> params)
        ...
        private Pair<String, String> getRelationCollectionInfo(Class<?> resourceClass)
                throws IOException {
            AnnotationMetadata annotationMetaData =
                    getAnnotationMetadata(resourceClass.getCanonicalName());
            if (annotationMetaData.isConcrete() && annotationMetaData.isIndependent()) {
                if (annotationMetaData
                        .getAnnotationTypes()
                        .contains(RelationshipResource.class.getCanonicalName())) {
                    Map<String, Object> attrs =
                            annotationMetaData.getAnnotationAttributes(
                                    RelationshipResource.class.getName());
                    ...
                    String entityCollectionName =
                            getEntityCollectionInfo(entityResource.getCanonicalName());

                    Pair<String, String> ret =
                            new Pair<String, String>(
                                    entityCollectionName, relationshipCollectionName);
                    return ret;
                } else {
                    return null;
                }
        ...
        private String getEntityCollectionInfo(String className) throws IOException {
            ...
            if (annotationMetaData.isConcrete() && annotationMetaData.isIndependent()) {
                if (annotationMetaData
                        .getAnnotationTypes()
                        .contains(EntityResource.class.getCanonicalName())) {
                    Map<String, Object> attrs =
                            annotationMetaData.getAnnotationAttributes(
        ...
        protected void setRequestHeaderIfAny(T methodBase) {
            if (headers != null) {
                for (Entry<String, String> headerNameValue : headers.entrySet()) {
                    methodBase.addRequestHeader(
                            headerNameValue.getKey(), headerNameValue.getValue());
                }
            }
        }
    ...
    public class GetRequestBuilder extends RequestBuilder<GetMethod> {
        ...
        public GetMethod getHttpMethod() throws IOException {
            RestApiEndpoint endpoint =
                    new RestApiEndpoint(
                            getRequestContext().getNetworkId(),
                            getScope(),
                            getApiName(),
                            getVersion(),
                            getEntityCollectionName(),
                            getEntityId(),
                            getRelationCollectionName(),
                            getRelationshipEntityId(),
                            getParams());
            String url = endpoint.getUrl();
    ...
    public class PostRequestBuilder extends PostPutRequestBuilder<PostMethod> {
        ...
        public PostMethod getHttpMethod() throws IOException {
            ...
            if (getBodyAsString() != null) {
                ...
                StringRequestEntity requestEntity =
                        new StringRequestEntity(getBodyAsString(), contentType, "UTF-8");
                req.setRequestEntity(requestEntity);
                ...
                ByteArrayRequestEntity requestEntity =
                        new ByteArrayRequestEntity(getBodyAsByteArray(), contentType);
                req.setRequestEntity(requestEntity);
            }
            setRequestHeaderIfAny(req);
            return req;
    ...
    public class PutRequestBuilder extends PostPutRequestBuilder<PutMethod> {
        ...
        public PutMethod getHttpMethod() throws IOException {
            ...
            if (getBodyAsString() != null) {
                StringRequestEntity requestEntity =
                        new StringRequestEntity(getBodyAsString(), "application/json", "UTF-8");
                req.setRequestEntity(requestEntity);
            } else if (binaryPayload != null) {
                BinaryRequestEntity requestEntity =
                        new BinaryRequestEntity(
                                binaryPayload.getFile(),
                                binaryPayload.getMimeType(),

...
public class ServerStatusWebScript extends AbstractWebScript implements ApplicationContextAware {
    ...
    public void execute(WebScriptRequest req, WebScriptResponse res) throws IOException {
        ChildApplicationContextFactory subsystem =
                (ChildApplicationContextFactory) applicationContext.getBean("imap");

        // note: getting property (rather than getting imapService bean to check isEnabled) does not
        // cause subsystem startup (if stopped)
        // hence providing ability for subsystem to be disabled (whilst still supporting ability to
        // check status and/or dynamically start via JMX)

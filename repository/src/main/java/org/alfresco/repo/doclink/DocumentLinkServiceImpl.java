...
/**
 * Implementation of the document link service In addition to the document link service, this class
 * also provides a BeforeDeleteNodePolicy
 *
 * @author Ana Bozianu
 * @since 5.1
 */
public class DocumentLinkServiceImpl
    ...
    public NodeRef createDocumentLink(final NodeRef source, NodeRef destination) {
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Creating document link. source: " + source + ", destination: " + destination);
        }
        ...
        if (!nodeService.exists(destination)) {
            throw new IllegalArgumentException(
                    "Destination NodeRef '" + destination + "' does not exist");
        }
        ...
        if (checkOutCheckInService.isWorkingCopy(source)
                || nodeService.hasAspect(source, ContentModel.ASPECT_LOCKABLE)) {
            throw new IllegalArgumentException(
            ...
            if (dictionaryService.isSubClass(sourceType, ContentModel.TYPE_CONTENT)) {
                ...
                childRef =
                    ...
                    && dictionaryService.isSubClass(
                            nodeService.getType(source), ContentModel.TYPE_FOLDER)) {
                ...
                throw new IllegalArgumentException(
                        "Unsupported source node type : " + nodeService.getType(source));
            }
    ...
    public List<Long> getNodeLinksIds(NodeRef nodeRef) {
        ...
        if (nameQName != null) {
            ...
            GetDoclinkNodesCannedQueryParams parameterBean =
                    new GetDoclinkNodesCannedQueryParams(
                            nodeRef.toString(), nameQName.getFirst(), pagingRequest.getMaxItems());
            ...
            CannedQuery<Long> query = new GetDoclinkNodesCannedQuery(cannedQueryDAO, params);
    ...
    public DeleteLinksStatusReport deleteLinksToDocument(NodeRef document) {
        ...
        for (Long linkId : linkNodeIds) {
                                ...
                                public NodeRef doWork() throws Exception {
                                    ...
                                    isNodePendingDelete =
                                            nodeService.hasAspect(
                                                    nodeRef, ContentModel.ASPECT_PENDING_DELETE);
                                    return nodeRef;
                                }
                            },
                            AuthenticationUtil.getSystemUserName());
            ...
            if (!isNodePendingDelete) {
                if (permissionService.hasPermission(linkNodeRef, PermissionService.DELETE_NODE)
                        == AccessStatus.DENIED) {
                    report.addErrorDetail(
                            ...
                            new AccessDeniedException(
                                    "User '"
                                            ...
                                            + "' doesn't have permission to create discussion on"
                                            + " node '"
                                            + linkNodeRef
                                            + "'"));
                } else {
                    ...
                    new AuthenticationUtil.RunAsWork<Object>() {
                        public Void doWork() throws Exception {
                            behaviourFilter.disableBehaviour(
                                    nodeRef, ContentModel.ASPECT_AUDITABLE);
                            ...
                            try {
                                ...
                                behaviourFilter.enableBehaviour(
                                        nodeRef, ContentModel.ASPECT_AUDITABLE);
                                behaviourFilter.enableBehaviour(
                                        nodeRef, ContentModel.ASPECT_LOCKABLE);
                            }

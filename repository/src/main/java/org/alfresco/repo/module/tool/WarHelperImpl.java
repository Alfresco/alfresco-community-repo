...
public class WarHelperImpl implements WarHelper {
    ...
    public void checkCompatibleVersion(TFile war, ModuleDetails installingModuleDetails) {
        ...
        if (propsFile != null && propsFile.exists()) {
            ...
            VersionNumber warVersion =
                    new VersionNumber(
                            warVers.getProperty("version.major")
                                    + "."
                                    + warVers.getProperty("version.minor")
                                    + "."
                                    + warVers.getProperty("version.revision"));
    ...
    /**
     * Checks if the module is compatible using the entry in the manifest. This is more accurate and
     * works for both alfresco.war and share.war, however valid manifest entries weren't added until
     * 3.4.11, 4.1.1 and Community 4.2
     *
     * @param war TFile
    ...
    protected void checkCompatibleVersionUsingManifest(
        ...
        if (version != null && version.length() > 0) {
            if (version.matches(REGEX_NUMBER_OR_DOT)) {
                ...
                // A non-numeric version number.  Currently our VersionNumber class doesn't support
                // Strings in the version
                ...
                if (edition.endsWith(MANIFEST_COMMUNITY)) {
                    ...
                    log.info(
                            "WARNING: Community edition war detected, the version number is"
                                    + " non-numeric so we will not validate it.");
                } else {
                    throw new ModuleManagementToolException(
                            "Invalid version number specified: " + version);
                }
            }

        } else {
            log.info(
                    "WARNING: No version information detected in war, therefore version validation"
                        + " is disabled, continuing anyway.  Is this war prior to 3.4.11, 4.1.1 and"
                        + " Community 4.2 ?");
        }
    }
     ...
     */
    protected String findManifestArtibute(TFile war, String attributeName)
            throws ModuleManagementToolException {
    ...
    public Manifest findManifest(TFile war) throws ModuleManagementToolException {
        ...
        try {
            ...
            throw new ModuleManagementToolException(
                    "Unabled to read a manifest for the war file: " + war);
        } finally {
            if (is != null) {
                try {
                    is.close();
    ...
    /**
     * Compares the version information with the module details to see if their valid. If they are
     * invalid then it throws an exception.
     *
     * @param warVersion VersionNumber
     * @param installingModuleDetails ModuleDetails
     * @throws ModuleManagementToolException
     */
    private void checkVersions(VersionNumber warVersion, ModuleDetails installingModuleDetails)
            throws ModuleManagementToolException {
        if (warVersion.compareTo(installingModuleDetails.getRepoVersionMin()) == -1) {
            throw new ModuleManagementToolException(
                    "The module ("
                            + installingModuleDetails.getTitle()
                            + ") must be installed on a war version equal to or greater than "
                            + installingModuleDetails.getRepoVersionMin()
                            + ". This war is version: "
                            + warVersion
                            + ".");
        }
        if (warVersion.compareTo(installingModuleDetails.getRepoVersionMax()) == 1) {
            throw new ModuleManagementToolException(
                    "The module ("
                            + installingModuleDetails.getTitle()
                            + ") cannot be installed on a war version greater than "
                            + installingModuleDetails.getRepoVersionMax()
                            + ". This war is version: "
                            + warVersion
                            + ".");
        }
        ...
        if (installableEditions != null && installableEditions.size() > 0) {
            ...
            if (propsFile != null && propsFile.exists()) {
                ...
                throw new ModuleManagementToolException(
                        "The module ("
                                + installingModuleDetails.getTitle()
                                + ") can only be installed in one of the following editions"
                                + installableEditions);
            } else {
    ...
    /**
     * Checks to see if the module that is being installed is compatible with the war, (using the
     * entry in the manifest). This is more accurate and works for both alfresco.war and share.war,
     * however valid manifest entries weren't added until 3.4.11, 4.1.1 and Community 4.2
     *
     * @param war TFile
     * @param installingModuleDetails ModuleDetails
     */
    public void checkCompatibleEditionUsingManifest(
        ...
        if (installableEditions != null && installableEditions.size() > 0) {
            ...
            if (warEdition != null && warEdition.length() > 0) {
                ...
                log.info(
                        "WARNING: No edition information detected in war, edition validation is"
                            + " disabled, continuing anyway. Is this war prior to 3.4.11, 4.1.1 and"
                            + " Community 4.2 ?");
            }
        }
    }
    ...
    public void checkModuleDependencies(TFile war, ModuleDetails installingModuleDetails) {
        ...
        for (ModuleDependency dependency : installingModuleDependencies) {
            ...
            // Check the dependency.  The API specifies that a null returns false, so no null check
            // is required
    ...
    public ModuleDetails getModuleDetailsOrAlias(TFile war, ModuleDetails installingModuleDetails) {
        ...
        if (installedModuleDetails == null) {
            ...
            for (String installingAlias : installingAliases) {
                ...
                // outputMessage("Module '" + installingAlias + "' is installed and is an alias of
                // '" + installingModuleDetails + "'", false);
                break;
            }
        }
            ...
            for (java.io.File dir : dirs) {
                if (dir.isDirectory() == true) {
                    TFile moduleProperties =
                            new TFile(dir.getPath() + WarHelper.MODULE_CONFIG_IN_WAR);
                    if (moduleProperties.exists() == true) {
                        ...
                        try {
                            ...
                            moduleDetails.add(
                                    ModuleDetailsHelper.createModuleDetailsFromPropertiesStream(
                                            is));
                        } catch (AlfrescoRuntimeException exception) {
                            throw new ModuleManagementToolException(
                                    "Unable to open module properties file '"
                                            + moduleProperties.getPath()
                                            + "' "
    ...
    private Properties loadProperties(TFile propertiesFile) {
        ...
        try {
            ...
            throw new ModuleManagementToolException(
                    "Unable to load properties from the war file; " + propertiesFile.getPath(),
                    exception);
        } finally {
            if (is != null) {
                try {

...
public class RuleTypeImpl extends CommonResourceAbstractBase implements RuleType {
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.rule.RuleType#triggerRuleType(org.alfresco.service.cmr.repository.NodeRef,
    ...
    public void triggerRuleType(
        ...
        if (ruleService.isEnabled() == true
            ...
            List<Rule> rules = ruleService.getRules(nodeRef, true, this.name);

            String ruleContext = null;
            if (logger.isDebugEnabled() == true) {
                if (actionedUponNodeRef != null) {
                    ruleContext =
                            (executeRuleImmediately ? " now" : "    ")
                                    + " on "
                                    + nodeService
                                            .getPath(actionedUponNodeRef)
                                            .toString()
                                            .replaceAll("\\{[^}]*}", "");
                }
            }
            ...
            if (rules.size() != 0) {
                for (Rule rule : rules) {
                    ...
                    if (exists
                            && rule.getRuleDisabled() == false
                            && ruleService.rulesEnabled(ruleService.getOwningNodeRef(rule))) {
                        if (logger.isDebugEnabled() == true) {
                            logger.debug("Triggering rule" + ruleContext);
                        }
                        if (executeRuleImmediately == false) {
                            // Queue the rule to be executed at the end of the transaction (but
                            // still in the transaction)
                            ((RuntimeRuleService) ruleService)
                                    .addRulePendingExecution(nodeRef, actionedUponNodeRef, rule);
                        } else {
                            // Execute the rule now
                            ((RuntimeRuleService) ruleService)
                                    .executeRule(rule, actionedUponNodeRef, null);
                        }

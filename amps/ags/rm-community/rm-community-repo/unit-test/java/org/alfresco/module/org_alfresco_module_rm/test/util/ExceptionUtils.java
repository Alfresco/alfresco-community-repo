...
public class ExceptionUtils {
    ...
    /**
     * Utility method to help with expected exceptions (unchecked - see below) in test code. This
     * can be used in place of {@code try/catch} blocks within test code and can sometimes make code
     * more readable. A single expected exception would usually be let escape from the test method
     * and be handled e.g. by JUnit's {@code @Test(expected="Exception.class")} pattern. However if
     * you have multiple expected exceptions in a sequence, you need to either add a sequence of
     * {@code try/catch} or use this method. Likewise if you need to make assertions about state
     * within the expected exception, such as root cause or other internal state, this method will
     * be useful.
     *
     * <p>Examples:
     *
     * <ul>
     *   <li>Calling a local method which throws a {@code RuntimeException}. (An expression lambda)
     *       <pre>
     * expectedException(RuntimeException.class, () -> badMethod() );
     *         </pre>
     *   <li>Executing a block of code. (Requires return statement)
     *       <pre>
     ...
     * });
     *         </pre>
     *   <li>Examining the expected exception e.g. to assert the root cause is correct.
     *       <pre>
     * UnsupportedOperationException e = expectedException(UnsupportedOperationException.class, () -> badMethod2() );
     * assertEquals(RuntimeException.class, e.getCause().getClass());
     *         </pre>
     *   <li>Note that if your lambda expression returns 'void' then you cannot use an expression
     ...
     * </ul>
     *
     * A note on checked exceptions: currently this method does not provide any support for working
     * around the normal integration of Java 8 lambdas and checked exceptions. If your {@code code}
     * block must deal with checked exceptions, you must add {@code try}/{@code catch} blocks within
     * your lambda which obviously makes this method less useful. This may change in the future.
     *
     ...
     * @param <R> the return type of the code block (which should not matter as it should not
     *     complete).
     * @param <T> the type of the expected throwable (subtypes will also match).
     * @return the expected throwable object if it was thrown.
     * @throws UnexpectedThrowableException if a non-matching throwable was thrown out of the code
     *     block.
     * @throws MissingThrowableException if the expected throwable was not thrown out of the code
     *     block.
     */
    @SuppressWarnings("unchecked")
    public static <R, T extends Throwable> T expectedException(
            final Class<T> expected, final Supplier<R> code) {
        ...
        Throwable thrownByCode =
                maybeThrownByCode.orElseThrow(() -> new MissingThrowableException(expected));
     ...
     */
    public static <R, T extends Exception> R smuggleCheckedExceptions(
            final ThrowingSupplier<R, T> code) {
        try {
            return code.get();
    ...
    /**
     * Equivalent to `java.util.function.Supplier` but its method declares that it throws checked
     * exceptions.
     *
    ...
    /**
     * A wrapper for checked exceptions so that they can be handled as unchecked exceptions, namely
     * by not requiring try/catch blocks etc.
     *
     * <p>This type is expected to be most useful when handling Java 8 lambdas containing code which

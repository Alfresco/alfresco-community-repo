...
public class FolderNode extends CMISNode implements Serializable {
  ...
  private Tree<FileableCmisObject> findTree(
    String nodeId,
    List<Tree<FileableCmisObject>> nodes
  ) {
  ...
  private void checkChildrenImpl(
    Map<String, FolderNode> expectedFolderNodes,
    Map<String, CMISNode> expectedDocumentNodes
  ) {
    for (FolderNode expectedFolderNode : expectedFolderNodes.values()) {
      FolderNode actualFolderNode = getFolderNode(
        expectedFolderNode.getNodeId()
      );
      assertNotNull(
        "Unable to find node " + expectedFolderNode,
        actualFolderNode
      );
      ...
      AssertUtil.assertEquals(
        "cmis:objectTypeId",
        expectedProperties.get("cmis:objectTypeId"),
        actualProperties.get("cmis:objectTypeId")
      );
      AssertUtil.assertEquals(
        "cmis:path",
        expectedProperties.get("cmis:path"),
        actualProperties.get("cmis:path")
      );
      AssertUtil.assertEquals(
        "cmis:name",
        expectedProperties.get("cmis:name"),
        actualProperties.get("cmis:name")
      );
    }

    for (CMISNode expectedDocumentNode : expectedDocumentNodes.values()) {
      CMISNode actualDocumentNode = getDocumentNode(
        expectedDocumentNode.getNodeId()
      );
      assertNotNull(
        "Unable to find node " + expectedDocumentNode.getNodeId(),
        actualDocumentNode
      );
  ...
  public void checkChildren(ItemIterable<CmisObject> expectedChildren) {
    ...
    for (CmisObject child : expectedChildren) {
      ...
      if (dn instanceof FolderNode) {
        expectedFolderNodes.put(
          getBareObjectId(dn.getNodeId()),
          (FolderNode) dn
        );
      } else {
  ...
  public void checkChildren(
    List<String> expectedChildFolderIds,
    List<String> expectedChildDocumentIds
  ) {
    Map<String, FolderNode> expectedFolderNodes = new HashMap<String, FolderNode>();
  ...
  public void checkDescendants(List<Tree<FileableCmisObject>> descendants) {
    for (FolderNode expectedFolderNode : folderNodes.values()) {
      Tree<FileableCmisObject> item = findTree(
        expectedFolderNode.getNodeId(),
        descendants
      );
      assertNotNull(item);
      FolderNode actualFolderNode = (FolderNode) CMISNode.createNode(
        item.getItem()
      );

...
public class OpenCmisLocalTest extends TestCase {
    ...
    private Repository getRepository(String user, String password) {
        ...
        parameters.put(
                ...
                "org.alfresco.opencmis.OpenCmisLocalTest$TestCmisServiceFactory");
    ...
    public void DISABLED_testBasicFileOps() {
        ...
        Folder folder =
                rootFolder.createFolder(folderProps, null, null, null, session.getDefaultContext());
        ...
        ContentStreamImpl fileContent = new ContentStreamImpl();
        {
            ContentWriter writer =
                    new FileContentWriter(TempFileProvider.createTempFile(getName(), ".txt"));
    ...
    public void testDownloadEvent() throws InterruptedException {
        ...
        ContentStream contentStream =
                new ContentStreamImpl(
                        docname, BigInteger.valueOf(byteContent.length), "text/plain", stream);
    ...
    public void testALF10085() throws InterruptedException {
        ...
        Calendar doc1LastModifiedBefore =
                (Calendar) doc1.getProperty(PropertyIds.LAST_MODIFICATION_DATE).getFirstValue();
        assertNotNull(doc1LastModifiedBefore);

        doc2.refresh();
        Calendar doc2LastModifiedBefore =
                (Calendar) doc2.getProperty(PropertyIds.LAST_MODIFICATION_DATE).getFirstValue();
        ...
        doc1.refresh();
        Calendar doc1LastModifiedAfter =
                (Calendar) doc1.getProperty(PropertyIds.LAST_MODIFICATION_DATE).getFirstValue();
        assertNotNull(doc1LastModifiedAfter);

        doc2.refresh();
        Calendar doc2LastModifiedAfter =
                (Calendar) doc2.getProperty(PropertyIds.LAST_MODIFICATION_DATE).getFirstValue();
    ...
    private ContentStreamImpl makeContentStream(String filename, String mimetype, String content)
            throws IOException {
        ...
        ContentStreamImpl contentStream =
                new ContentStreamImpl(
                        filename,
                        BigInteger.valueOf(tos.getLength()),
    ...
    /**
     * Simulates the pattern of advice created by
     * AlfrescoCmisServiceFactory.getService(CallContext), optionally including a
     * AlfrescoCmisStreamInterceptor which changes ContentStream parameter values into
     * ReusableContentStream parameters which unlike the original may be closed and then opened
     * again. This is important as retrying transaction advice is also added (simulated here by the
     * afterAdvice and the test target object. See MNT-285
     *
    ...
    private void simulateCallWithAdvice(boolean includeStreamInterceptor) throws Exception {
        ...
        MethodInterceptor beforeAdvice =
        ...
        TestStreamTarget target =
                new TestStreamTarget() {
                    @Override
                    public void methodA(
                            ContentStream csa,
                            String str,
                        ...
                        try {
                            ...
                            // Similar test to that in
                            // AlfrescoCmisServiceImpl.copyToTempFile(ContentStream)
                            if ((csa.getLength() > -1)
                                    && (a == null || csa.getLength() != a.length())) {
        ...
        TempStoreOutputStreamFactory streamFactory =
                TempStoreOutputStreamFactory.newInstance(tempDir, 1024, 1024, false);
        TempStoreOutputStream tos = streamFactory.newOutputStream();
        OutputStreamWriter writer = new OutputStreamWriter(tos);
        writer.write("The cat sat on the mat");

        ContentStreamImpl csa =
                makeContentStream(
                        "file1", MimetypeMap.MIMETYPE_TEXT_PLAIN, "The cat sat on the mat");
        ContentStreamImpl csb =
                makeContentStream(
                        "file2",
                        MimetypeMap.MIMETYPE_TEXT_PLAIN,
    ...
    private interface TestStreamTarget {
        void methodA(ContentStream csa, String str, ContentStream csb, ContentStream csc, int i)
                throws Exception;
    }

    /**
     * MNT-14687 - Creating a document as checkedout and then cancelling the checkout should delete
     * the document.
     *
     * <p>This test would have fit better within CheckOutCheckInServiceImplTest but was added here
     * to make use of existing methods
     */
    ...
    public void testCancelCheckoutWhileInCheckedOutState() {
        ...
        AuthenticationComponent authenticationComponent =
                (AuthenticationComponent) ctx.getBean(BEAN_NAME_AUTHENTICATION_COMPONENT);
        ...
        ContentStream contentStream =
                new ContentStreamImpl(
                        docname, BigInteger.valueOf(byteContent.length), MIME_PLAIN_TEXT, stream);
    ...
    public void testEncodingForCreateContentStream() {
        ...
        try {
            ...
            Folder rootFolder = session.getRootFolder();
            Document document =
                    createDocument(rootFolder, "test_file_" + GUID.generate() + ".txt", session);
        ...
        ContentStream contentStream =
                session.getObjectFactory()
                        .createContentStream(
                                ...
                                input); // additionally set the charset here
        // NOTE that we intentionally specified the wrong charset here (as UTF-8)
        // because Alfresco does automatic charset detection, so we will ignore this explicit
        // request

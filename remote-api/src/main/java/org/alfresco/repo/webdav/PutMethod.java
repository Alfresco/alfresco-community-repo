...
public class PutMethod extends WebDAVMethod implements ActivityPostProducer {
    ...
    /**
     * Clears the aspect added by a LOCK request for a new file, so that the Timer started by the
     * LOCK request will not remove the node now that the PUT request has been received. This is
     * needed for large content.
     *
     * @exception WebDAVServerException
     */
                        ...
                        .doInTransaction(
                                new RetryingTransactionCallback<Boolean>() {
                                    public Boolean execute() throws Throwable {
                                        ...
                                        try {
                                            contentNodeInfo =
                                                    getNodeForPath(getRootNodeRef(), getPath());
    ...
    protected void executeImpl() throws WebDAVServerException, Exception {
        ...
        try {
            ...
            if (fileFolderService.isHidden(contentNodeRef)
                    && !getDAVHelper().isRenameShuffle(getPath())) {
            ...
            try {
                ...
                // ALF-7079 fix, retry: it looks like concurrent access (file not found but file
                // exists)
        ...
        try {
            ...
            ContentData contentData =
                    (ContentData) getNodeService().getProperty(nodeRef, ContentModel.PROP_CONTENT);
            if ((contentData == null || contentData.getSize() == 0)
                    && getNodeService().hasAspect(nodeRef, ContentModel.ASPECT_VERSIONABLE)) {
                getDAVHelper()
                        .getPolicyBehaviourFilter()
                        .disableBehaviour(nodeRef, ContentModel.ASPECT_VERSIONABLE);
            ...
            // ALF-16756: To avoid firing inbound rules too early (while a node is still locked)
            // apply the no content aspect
            //                     Note, for MNT-15801, that the aspect is only applied if:
            //                         - the node is locked AND
            //                         - the node does not have any content (zero length binaries
            // included)
            if (nodeLockInfo != null
                    && nodeLockInfo.isExclusive()
                    && !(ContentData.hasContent(contentData) && contentData.getSize() > 0)) {
                getNodeService()
                        .addAspect(
                                contentNodeInfo.getNodeRef(), ContentModel.ASPECT_NO_CONTENT, null);
            }

            // Ask for the document metadata to be extracted
            Action extract =
                    getActionService().createAction(ContentMetadataExtracter.EXECUTOR_NAME);
            if (extract != null) {
            ...
            if (m_strContentType == null || !m_strContentType.equals(writer.getMimetype())) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Mimetype originally specified as "
                                    + oldMimeType
                                    + ", now guessed to be "
                                    + m_strContentType);
                }
            ...
            m_response.setStatus(
                    created ? HttpServletResponse.SC_CREATED : HttpServletResponse.SC_NO_CONTENT);
            ...
            if (disabledVersioning) {
                getDAVHelper()
                        .getPolicyBehaviourFilter()
                        .enableBehaviour(
                                contentNodeInfo.getNodeRef(), ContentModel.ASPECT_VERSIONABLE);
            }
        }
    ...
    /**
     * Can be used after a successful {@link #execute()} invocation to check whether the resource
     * was new (created) or over-writing existing content.
     *
    ...
    /**
     * Retrieve the mimetype of the content sent for the PUT request. The initial value specified in
     * the request may be updated after the file contents have been uploaded if the repository has
     * determined a different mimetype for the content.
     *
     * @return content-type
     */

...
public interface RuleService {
    ...
    /**
     * Indicates whether the rules for a given node are enabled or not. If the rules are not enabled
     * then they will not be executed.
     *
    ...
    /**
     * Disables the rules for a given node reference. When the rules are disabled they will not
     * execute.
     *
     * @param nodeRef the node reference
     */
    ...
    /**
     * Enables the rules for a given node reference. When the rules are enabled they will execute as
     * usual. By default all rules are enabled.
     *
     * @param nodeRef the node reference
     */
    @Auditable(parameters = {"nodeRef"})
     ...
     *
     * <p>An exception is raised if the actionable aspect is not present on the passed node.
     *
     * @param nodeRef the node reference
     ...
     *
     * <p>Optionally this list includes rules inherited from its parents.
     *
     * <p>An exception is raised if the actionable aspect is not present on the passed node.
     *
     * @param nodeRef the node reference
     * @param includeInhertied indicates whether the inherited rules should be included in the
     *     result list or not
     * @return a list of the rules associated with the node
     */
     ...
     * @param ruleTypeName the name of the rule type, if null is passed all rule types are returned
     * @return a list of the rules associated with the node
     */
     ...
     *
     * <p>If the rule is already associated with the node, the details are updated with those
     * specified.
     *
    ...
    /**
     * Returns the owning node reference for an action. Returns null for an unsaved action or one
     * that is not parented by a rule.
     *
     * <p>NOTE: this method is temporary and will be removed in future versions. It should only be
    ...
    /**
     * Get the node reference to the rule node which the rule node links to. Returns null if rules
     * are not linked.
     *
    ...
    /**
     * Get a list of the all the rule nodes that link to the passed rule node. Returns an empty list
     * if none link.
     *
     * @param nodeRef node reference of a rule node

...
public class AuditWebScriptTest extends BaseWebScriptTest {
  ...
  protected void setUp() throws Exception {
    ...
    AuditComponent auditComponent = (AuditComponent) ctx.getBean(
      "auditComponent"
    );
    auditComponent.setUserAuditFilter(userAuditFilter);
    AuditServiceImpl auditServiceImpl = (AuditServiceImpl) ctx.getBean(
      "auditService"
    );
    ...
    AuditModelRegistryImpl auditModelRegistry = (AuditModelRegistryImpl) ctx.getBean(
      "auditModel.modelRegistry"
    );
    URL testModelUrl = ResourceUtils.getURL(
      "classpath:alfresco/testaudit/alfresco-audit-test-repository.xml"
    );
    URL testModelUrl1 = ResourceUtils.getURL(
      "classpath:alfresco/testaudit/alfresco-audit-test-mnt-16748.xml"
    );
    ...
    wasRepoEnabled =
      auditService.isAuditEnabled(APP_REPOTEST_NAME, APP_REPOTEST_PATH);
    wasSearchEnabled =
      auditService.isAuditEnabled(APP_SEARCHTEST_NAME, APP_SEARCHTEST_PATH);
  ...
  protected void tearDown() throws Exception {
    ...
    try {
      ...
      throw new RuntimeException(
        "Failed to set audit back to globally enabled/disabled state",
        e
      );
    }
    try {
      ...
      throw new RuntimeException(
        "Failed to set repo audit back to enabled/disabled state",
        e
      );
    }
    try {
      ...
      throw new RuntimeException(
        "Failed to set search audit back to enabled/disabled state",
        e
      );
    }
  }
  ...
  public void testGetWithoutPermissions() throws Exception {
    ...
    TestWebScriptServer.GetRequest req = new TestWebScriptServer.GetRequest(
      url
    );
  ...
  public void testGetIsAuditEnabledGlobally() throws Exception {
    ...
    JSONArray apps = json.getJSONArray(
      AbstractAuditWebScript.JSON_KEY_APPLICATIONS
    );
    assertEquals(
      "Incorrect number of applications reported",
      checkApps.size(),
      apps.length()
    );
  }
  ...
  public void testSetAuditEnabledGlobally() throws Exception {
    ...
    if (wasEnabled) {
      ...
      TestWebScriptServer.PostRequest req = new TestWebScriptServer.PostRequest(
        url,
        "",
  ...
  public void testGetIsAuditEnabledRepo() throws Exception {
    ...
    if (wasEnabled) {
      ...
      boolean appEnabled = app.getBoolean(
        AbstractAuditWebScript.JSON_KEY_ENABLED
      );
      assertEquals(
        "Mismatched application audit enabled",
        wasEnabled,
        appEnabled
      );
      assertEquals(
        "Mismatched application audit name",
        APP_REPOTEST_NAME,
        appName
      );
      assertEquals(
        "Mismatched application audit path",
        APP_REPOTEST_PATH,
        appPath
      );
    }
  ...
  public void testGetAuditSearchService() throws Exception {
    ...
    TestWebScriptServer.PostRequest postReq = new TestWebScriptServer.PostRequest(
      url,
      "",
      MimetypeMap.MIMETYPE_JSON
    );
    ...
    url =
      "/api/audit/query/" +
      APP_SEARCHTEST_NAME +
      "/searchaudit/queryX/searchParametersX?verbose=true";
    TestWebScriptServer.GetRequest getReq = new TestWebScriptServer.GetRequest(
      url
    );
    ...
    JSONArray jsonEntries = json.getJSONArray(
      AbstractAuditWebScript.JSON_KEY_ENTRIES
    );
    assertEquals(
      "Incorrect number of entries reported",
      1,
      jsonEntries.length()
    );

    JSONObject values = (JSONObject) ((JSONObject) jsonEntries.get(0)).get(
        AbstractAuditWebScript.JSON_KEY_ENTRY_VALUES
      );
    assertTrue(
      "Audit entry was not found",
      values.toString(0).contains("query==cm:name:" + filename)
    );
  ...
  public synchronized void testClearAuditRepo() throws Exception {
    ...
    String url =
      "/api/audit/clear/" + APP_REPOTEST_NAME + "?fromTime=" + future;
    TestWebScriptServer.PostRequest req = new TestWebScriptServer.PostRequest(
      url,
      "",
      MimetypeMap.MIMETYPE_JSON
    ...
    for (int i = 0; i < 60; i++) {
      ...
      url =
        ...
        future;
      req =
        new TestWebScriptServer.PostRequest(url, "", MimetypeMap.MIMETYPE_JSON);
  ...
  public void testQueryAuditRepo() throws Exception {
    ...
    assertEquals(
      "Entry count and physical count don't match",
      new Long(jsonEntries.length()),
      entryCount
    );
    ...
    String entryTimeStr = jsonEntry.getString(
      AbstractAuditWebScript.JSON_KEY_ENTRY_TIME
    );
    ...
    JSONObject jsonValues = jsonEntry.getJSONObject(
      AbstractAuditWebScript.JSON_KEY_ENTRY_VALUES
    );
    String entryUsername = jsonValues.getString(
      "/repositorytest/login/error/user"
    );
    assertEquals(
      "Didn't find the login-failure-user",
      getName(),
      entryUsername
    );
    ...
    url =
      "/api/audit/query/" +
      APP_REPOTEST_NAME +
      "/repositorytest/login/error/userXXX" +
      "?verbose=true";
    req = new TestWebScriptServer.GetRequest(url);
    response = sendRequest(req, Status.STATUS_OK, admin);
    json = new JSONObject(response.getContentAsString());
    ...
    url =
      "/api/audit/query/" +
      APP_REPOTEST_NAME +
      "?forward=false&limit=1&verbose=true&fromTime=" +
      now;
    // ALF-3055 : auditing of failures is now asynchronous, so loop 60 times with a
    // 1 second sleep to ensure that the audit is processed
    for (int i = 0; i < 60; i++) {
    ...
    assertNotNull("No entry ID", entryId);
    jsonValues =
      jsonEntry.getJSONObject(AbstractAuditWebScript.JSON_KEY_ENTRY_VALUES);

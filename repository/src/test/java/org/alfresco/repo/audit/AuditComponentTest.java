...
public class AuditComponentTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        final RetryingTransactionCallback<Void> resetDisabledPathsCallback =
    ...
    public void testAreAuditValuesRequired() {
        ...
        assertFalse(
                "Path 'bogus' should not have any audit associated with it.",
                auditRequiredForBogus);
        ...
        RetryingTransactionCallback<Void> testCallback =
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        Map<String, Serializable> auditedValues =
                                auditComponent.recordAuditValues("/bogus", null);
                        assertNotNull(auditedValues);
                        assertTrue(
                                "Invalid application should not audit anything",
                                auditedValues.isEmpty());
    ...
    public void testAudit_Basic() throws Exception {
        final RetryingTransactionCallback<Void> testCallback =
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
    ...
    private Map<String, Serializable> auditTestAction(
            final String action, NodeRef nodeRef, Map<String, Serializable> parameters) {
        final Map<String, Serializable> adjustedValues =
                new HashMap<String, Serializable>(parameters.size() * 2);
        // Add the noderef
        ...
        return transactionService
                .getRetryingTransactionHelper()
                .doInTransaction(auditCallback, true, false);
    }

    /** Utility method to compare a 'results' map with a map of expected values */
    private void checkAuditMaps(
            Map<String, Serializable> result, Map<String, Serializable> expected) {
    ...
    public void testAudit_EnableDisableAuditPaths() throws Exception {
        ...
        RetryingTransactionCallback<Void> disableAuditCallback =
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        Map<String, Serializable> expectedInner =
                                new HashMap<String, Serializable>(expected);

                        auditComponent.disableAudit(
                                APPLICATION_ACTIONS_TEST,
                                "/actions-test/actions/action-01/params/A");
                        expectedInner.remove("/actions-test/actions/action-01/params/A/value");
                        Map<String, Serializable> result =
                                auditTestAction("action-01", nodeRef, parameters);
                        checkAuditMaps(result, expectedInner);

                        auditComponent.disableAudit(
                                APPLICATION_ACTIONS_TEST,
                                "/actions-test/actions/action-01/params/B");
        ...
        transactionService
                .getRetryingTransactionHelper()
                .doInTransaction(disableAuditCallback, false);
    }
    ...
    public void testAuditAuthenticationService() throws Exception {
        ...
        URL testModelUrl =
                ResourceUtils.getURL(
                        "classpath:alfresco/testaudit/alfresco-audit-test-authenticationservice.xml");
        auditModelRegistry.registerModel(testModelUrl);
        auditModelRegistry.loadAuditModels();

        final List<Long> results = new ArrayList<Long>(5);
        final StringBuilder sb = new StringBuilder();
        AuditQueryCallback auditQueryCallback =
                new AuditQueryCallback() {
                    public boolean valuesRequired() {
                        return true;
                    }
        ...
        logger.debug(sb.toString());
        assertTrue(
                "There should be no audit entries for the API test after a clear",
                results.isEmpty());
        ...
        RunAsWork<Void> createAuthenticationWork =
                new RunAsWork<Void>() {
                    public Void doWork() throws Exception {
                        if (!authenticationService.authenticationExists(getName())) {
                            authenticationService.createAuthentication(
                                    getName(), getName().toCharArray());
                        }
                        return null;
                    }
                };
        ...
        logger.debug(sb.toString());
        assertEquals(
                "Incorrect number of audit entries after failed login", iterations, results.size());
        ...
        logger.debug(sb.toString());
        assertEquals(
                "Range of audit entries were not deleted", remainingResults.size(), results.size());
    ...
    public void testAuditFailedNodeAccess() throws Exception {
        ...
        URL testModelUrl =
                ResourceUtils.getURL(
                        "classpath:alfresco/testaudit/alfresco-audit-test-alf-12638.xml");
        auditModelRegistry.registerModel(testModelUrl);
        auditModelRegistry.loadAuditModels();
    ...
    public void testApplication() throws Exception {
        ...
        URL testModelUrl =
                ResourceUtils.getURL(
                        "classpath:alfresco/testaudit/alfresco-audit-test-mnt-10070.xml");
        auditModelRegistry.registerModel(testModelUrl);
        auditModelRegistry.loadAuditModels();
        ...
        AuditApplication applicationOne =
                auditModelRegistry.getAuditApplicationByName(APPLICATION_ONE);
        assertNotNull("Application 'app1' dosn't exist", applicationOne);
        AuditApplication applicationTwo =
                auditModelRegistry.getAuditApplicationByName(APPLICATION_TWO);
        assertNotNull("Application 'app2' dosn't exist", applicationTwo);
        AuditApplication applicationThree =
                auditModelRegistry.getAuditApplicationByName(APPLICATION_THREE);
        ...
        Map<String, Serializable> recordedAuditMap =
                auditComponent.recordAuditValues("/alfresco-access", auditMap);
    ...
    public void testAuditSubordinateCall() throws Exception {
        ...
        URL testModelUrl =
                ResourceUtils.getURL(
                        "classpath:alfresco/testaudit/alfresco-audit-test-mnt-10767.xml");
        auditModelRegistry.registerModel(testModelUrl);
        auditModelRegistry.loadAuditModels();
        // There should be a log entry for the application
        final List<Long> results = new ArrayList<Long>(5);
    ...
    public void onCreateFolderMNT10767(ChildAssociationRef childAssocRef) {
        ...
        fileFolderService.create(
                newFolderRef,
                "testcontent-" + System.currentTimeMillis(),
                ...
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        values.put(
                                ...
                                (Serializable) Collections.unmodifiableCollection(list));
    ...
    /** Clear the audit log as 'admin' */
    private Integer deleteAuditEntries(
            final String applicationName, final long fromId, final long toId) {
        RunAsWork<Integer> work =
                new RunAsWork<Integer>() {
                    @Override
                    public Integer doWork() throws Exception {
                        return new Integer(
                                auditService.clearAuditByIdRange(applicationName, fromId, toId));
                    }
                };
    ...
    private void queryAuditLog(
            final AuditQueryCallback callback,
            final AuditQueryParameters parameters,
    ...
    public void testAuditTruncatedValues() {
        ...
        final String stringValue =
                RandomStringUtils.randomAlphanumeric(SchemaBootstrap.DEFAULT_MAX_STRING_LENGTH + 1);
        final MLText mlTextValue = new MLText();
        mlTextValue.put(Locale.ENGLISH, stringValue);

        final RetryingTransactionCallback<Map<String, Serializable>> testCallback =
                new RetryingTransactionCallback<Map<String, Serializable>>() {
                    public Map<String, Serializable> execute() throws Throwable {
        ...
        RunAsWork<Map<String, Serializable>> testRunAs =

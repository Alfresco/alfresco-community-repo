...
public interface LockService {
     ...
     *
     * <p>The lock prevents any other user or process from comitting updates to the node until the
     * lock is released.
     *
     * <p>The lock will be owned by the current user.
     *
     * <p>A lock made with this call will never expire.
     *
     * @param nodeRef a reference to a node
     * @param lockType the lock type
     * @throws UnableToAquireLockException thrown if the lock could not be obtained
     */
     ...
     * <p>The lock will be owned by the current user.
     *
     * <p>If the time to expire is 0 then the lock will never expire. Otherwise the timeToExpire
     * indicates the number of seconds before the lock expires. When a lock expires the lock is
     * considered to have been released.
     *
     * <p>If the node is already locked and the user is the lock owner then the lock will be renewed
     * with the passed timeToExpire.
     *
     * @param nodeRef a reference to a node
     * @param lockType the lock type
     ...
     *
     * <p>The lock prevents any other user or process from committing updates to the node until the
     * lock is released.
     *
     * <p>The lock will be owned by the current user.
     *
     * <p>If any one of the child locks can not be taken then an exception will be raised and all
     * locks cancelled.
     *
     * <p>If the time to expire is 0 then the lock will never expire. Otherwise the timeToExpire
     * indicates the number of seconds before the lock expires. When a lock expires the lock is
     ...
     * @param lockType the lock type
     * @param timeToExpire the number of seconds before the locks expires.
     * @param lockChildren if true indicates that all the children (and grandchildren, etc) of the
     *     node will also be locked, false otherwise
     * @throws UnableToAquireLockException thrown if the lock could not be obtained
     */
     ...
     * considered to have been released.
     *
     * <p>If the node is already locked and the current user is the lock owner then the lock will be
     * renewed with the passed timeToExpire.
     *
     ...
     *
     * <p>The user must have sufficient permissions to remove the lock (ie: be the owner of the lock
     * or have admin rights) and the node must not be checked out. Otherwise an exception will be
     * raised.
     *
     * @param nodeRef a reference to a node
     * @throws UnableToReleaseLockException thrown if the lock could not be released
     */
     ...
     *
     * <p>The user must have sufficient permissions to remove the lock(s) (ie: be the owner of the
     * lock(s) or have admin rights) and the node(s) must not be checked out. Otherwise an exception
     * will be raised.
     *
     * <p>If one of the child nodes is not locked then it will be ignored and the process continue
     * without error.
     *
     * <p>If the lock on any one of the child nodes cannot be released then an exception will be
     * raised.
     *
     * @param nodeRef a node reference
     * @param lockChildren if true then all the children (and grandchildren, etc) of the node will
     *     also be unlocked, false otherwise
     * @throws UnableToReleaseLockException thrown if the lock could not be released
     */
     ...
     *
     * <p>The user must have sufficient permissions to remove the lock(s) (ie: be the owner of the
     * lock(s) or have admin rights) otherwise an exception will be raised.
     *
     * <p>If one of the child nodes is not locked then it will be ignored and the process continue
     * without error.
     *
     ...
     *
     * <p>The user must have sufficient permissions to remove the locks (ie: be the owner of the
     * locks or have admin rights) otherwise an exception will be raised.
     *
     * <p>If one of the nodes is not locked then it will be ignored and the process will continue
     * without an error.
     *
     * <p>If the lock on any one of the nodes cannot be released than an exception will be raised
     * and the process rolled back.
     *
     ...
     *
     * <p>Returns null if the node is not locked.
     *
     * <p>Throws an exception if the node does not have the lock aspect.
     *
     * @param nodeRef the node reference
     * @return the lock type, null is returned if the object in question has no lock
     */
    @Auditable(parameters = {"nodeRef"})
     ...
     *
     * <p>If the node is locked by another user then a NodeLockedException is thrown.
     *
     * <p>Gets the user reference from the current session.
     *
     * @param nodeRef the node reference
     * @throws NodeLockedException thrown if the node is locked by someone else. This is based on
     *     the lock status of the lock, the user ref and the lock type.
     */
    @Auditable(parameters = {"nodeRef"})
     ...
     *
     * <p>Used for the system to be able to update locked nodes.
     */
     ...
     *
     * <p>Do NOT use the returned information to determine the actual state of a lock, use {@link
     * #getLockStatus(NodeRef)} and other LockService methods for this purpose.
     *
     * <p>The returned data is intended for information purposes only, e.g. returning the timeout in
     * a WebDAV response.
     *
     * @param nodeRef NodeRef
     * @return LockState
     ...
     * Requests for ephemeral locks with expiry times greater than this value will be automatically
     * converted to a request for a persistent lock.
     *

...
public class DbOrIndexSwitchingQueryLanguageTest {
  ...
  public void hybridSearch() {
    when(indexQueryLang.executeQuery(argThat(isSearchParamsSinceTxId(null))))
      .thenReturn(indexResults);
    when(indexResults.getLastIndexedTxId()).thenReturn(80L);
    when(dbQueryLang.executeQuery(argThat(isSearchParamsSinceTxId(80L))))
      .thenReturn(dbResults);
    when(
      solrDAO.getNodes(argThat(isNodeParamsFromTxnId(81L)), eq(null), eq(null))
    )
      .thenReturn(changedNodes);
  ...
  public void findAfts() throws Exception {
    java.util.regex.Matcher matcher = LuceneQueryLanguageSPI.AFTS_QUERY.matcher(
    ...
    matcher =
      LuceneQueryLanguageSPI.AFTS_QUERY.matcher(
   ...
   */
  private Matcher<SearchParameters> isSearchParamsSinceTxId(
    final Long sinceTxId
  ) {
    return new BaseMatcher<SearchParameters>() {
      @Override
      public void describeTo(Description description) {
        description.appendText(
          SearchParameters.class.getSimpleName() +
          "[sinceTxId=" +
          sinceTxId +
          "]"
        );
  ...
  private Matcher<NodeParameters> isNodeParamsFromTxnId(final Long fromTxnId) {
    return new BaseMatcher<NodeParameters>() {
      ...
      public void describeTo(Description description) {
        description.appendText(
          NodeParameters.class.getSimpleName() + "[fromTxId=" + fromTxnId + "]"
        );
      }

...
public class FullTest {
  ...
  public void canUseCachingContentStore() {
    ...
    assertEquals(
      "Reader and writer should have same URLs",
      writer.getContentUrl(),
      reader.getContentUrl()
    );
    assertEquals(
      "Reader should get correct content",
      content,
  ...
  public void writeToCacheWithContentContext() {
    ...
    ContentWriter writer = store.getWriter(
      new ContentContext(null, proposedUrl)
    );
    final String content = makeContent();
    writer.putContent(content);
    assertEquals(
      "Writer should have correct URL",
      proposedUrl,
      writer.getContentUrl()
    );
  ...
  public void writeToCacheWithExistingReader() {
    ...
    final String proposedUrl = FileContentStore.createNewFileStoreUrl();
    ContentWriter writer = store.getWriter(
      new ContentContext(existingReader, proposedUrl)
    );
    final String content = makeContent();
    writer.putContent(content);
    assertEquals(
    ...
    assertFalse(
      "Old and new writers must have different URLs",
  ...
  public void canGuessMimeType() {
    ...
    ContentService contentService = (ContentService) ctx.getBean(
      "ContentService"
    );

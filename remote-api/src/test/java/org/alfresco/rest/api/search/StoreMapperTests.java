...
public class StoreMapperTests {
    ...
    public void testGetStoreRef() throws Exception {
        ...
        assertEquals(
                StoreMapper.STORE_REF_VERSION2_SPACESSTORE, storeMapper.getStoreRef("Versions"));
        assertEquals(
                StoreMapper.STORE_REF_VERSION2_SPACESSTORE, storeMapper.getStoreRef("versions"));
        assertEquals(
                StoreMapper.STORE_REF_VERSION2_SPACESSTORE, storeMapper.getStoreRef("VERSIONS"));
        ...
        assertEquals(
                StoreRef.STORE_REF_ARCHIVE_SPACESSTORE, storeMapper.getStoreRef("Deleted-nodes"));
        assertEquals(
                StoreRef.STORE_REF_ARCHIVE_SPACESSTORE, storeMapper.getStoreRef("deleted-nodes"));
        assertEquals(
                StoreRef.STORE_REF_ARCHIVE_SPACESSTORE, storeMapper.getStoreRef("DELETED-NODES"));
    ...
    public void testGetStore() throws Exception {
        assertEquals(
                ...
                storeMapper.getStore(
                        new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, "not interested")));
        assertEquals(
                storeMapper.VERSIONS,
                storeMapper.getStore(
                        new NodeRef(StoreMapper.STORE_REF_VERSION2_SPACESSTORE, "not interested")));
        assertEquals(
                storeMapper.DELETED,
                storeMapper.getStore(
                        new NodeRef(StoreRef.STORE_REF_ARCHIVE_SPACESSTORE, "not interested")));

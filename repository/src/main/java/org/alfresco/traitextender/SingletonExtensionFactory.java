...
/**
 * Creates singleton extension sub classes that are extension API implementors. The singleton
 * extensions continue to exist after the extensible has been collected.
 *
 * @author Bogdan Horje
 */
public class SingletonExtensionFactory<E, S extends SingletonExtension<E, T>, T extends Trait>
    ...
    private class TraiSingletontHandler implements InvocationHandler {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            ...
            try {
                return method.invoke(this.singleton, args);
    ...
    public SingletonExtensionFactory(S singleton, Class<E> extensionAPI) throws InvalidExtension {
        ...
        if (!extensionAPI.isAssignableFrom(singleton.getClass())) {
            throw new InvalidExtension(
                    singleton.getClass() + " is not an " + extensionAPI + " extension.");
        }
        this.singleton = singleton;
    ...
    public <TO extends Trait> E createExtension(TO traitObject) {
        ...
        if (!singleton.acceptsTrait(traitObject)) {
            throw new InvalidExtension(
                    "Extension factory error : "
                            + singleton.getClass()
                            + " does not support trait "
                            + traitObject);
        }

        if (logger.isDebugEnabled()) {
            String traitTrace =
                    traitObject != null ? traitObject.getClass().toString() : "<null> trait";
        ...
        return (E)
                Proxy.newProxyInstance(
                        ...
                        new TraiSingletontHandler(singleton, tTrait));
    }

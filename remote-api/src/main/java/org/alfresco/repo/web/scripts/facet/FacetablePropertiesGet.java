...
public class FacetablePropertiesGet extends AbstractSolrFacetConfigAdminWebScript {
    ...
    protected Map<String, Object> unprotectedExecuteImpl(
        ...
        // We use any provided locale to localise some elements of the webscript response, but not
        // all.
        final String userLocaleString = req.getParameter(QUERY_PARAM_LOCALE);
        final Locale userLocale =
                (userLocaleString == null) ? Locale.getDefault() : new Locale(userLocaleString);

        // There are multiple defined URIs for this REST endpoint. Some define a "classname"
        // template var.
        ...
        try {
            ...
            throw new WebScriptException(
                    Status.STATUS_NOT_FOUND, "Unrecognised classname: " + contentClassName, e);
        }

        // Build an FTL model of facetable properties - this includes normal Alfresco properties and
        // also
        ...
        if (contentClassQName == null) {
            facetableProperties =
                    toFacetablePropertyModel(facetService.getFacetableProperties(), userLocale);

            final List<SyntheticPropertyDefinition> facetableSyntheticProperties =
                    facetService.getFacetableSyntheticProperties();
            facetableProperties.addAll(
                    toFacetablePropertyModel_(facetableSyntheticProperties, userLocale));
        } else {
            facetableProperties =
                    toFacetablePropertyModel(
                            facetService.getFacetableProperties(contentClassQName), userLocale);

            final List<SyntheticPropertyDefinition> facetableSyntheticProperties =
                    facetService.getFacetableSyntheticProperties(contentClassQName);
            facetableProperties.addAll(
                    toFacetablePropertyModel_(facetableSyntheticProperties, userLocale));
        }
                    ...
                    new ResultFilter() {
                        ...
                        public boolean filter(FacetablePropertyFTL<?> facetableProperty) {
                            ...
                            Collection<String> prefixes =
                                    namespaceService.getPrefixes(propQName.getNamespaceURI());
                            return prefixes.contains(namespaceFilter);
                        }
                    });
        }

        List<FacetablePropertyFTL<?>> filteredFacetableProperties =
                filter(facetableProperties, filters);

        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Retrieved "
                            + facetableProperties.size()
                            + " available facets; filtered to "
        ...
        ScriptPagingDetails paging =
                new ScriptPagingDetails(
                        getNonNegativeIntParameter(
                                req, QUERY_PARAM_MAX_ITEMS, DEFAULT_MAX_ITEMS_PER_PAGE),
    ...
    /**
     * This type defines the (inclusion) filtering of {@link FacetablePropertyFTL property data} in
     * the response to this webscript.
     */
    ...
    /**
     * This method returns a new List instance containing only those {@link FacetablePropertyFTL
     * property data} that satisfy all {@link ResultFilter filters}.
     */
    private List<FacetablePropertyFTL<?>> filter(
            Collection<FacetablePropertyFTL<?>> propsData, List<ResultFilter> filters) {
    ...
    /**
     * This method returns a {@link FacetablePropertyFTL} for the specified {@link
     * PropertyDefinition}.
     */
    private FacetablePropertyFTL<?> toFacetablePropertyModel(
            PropertyDefinition propDef, Locale locale) {
     ...
     */
    private FacetablePropertyFTL<?> toFacetablePropertyModel(
            SyntheticPropertyDefinition propDef, Locale locale) {
        ...
        final String localisedTitle =
                I18NUtil.getMessage(i18nKeyPrefix + propDef.syntheticPropertyName, locale);

        return new SyntheticFacetablePropertyFTL(
                propDef.containingPropertyDef,
                localisedTitle,
                propDef.syntheticPropertyName,
                propDef.dataTypeDefinition);
    }

    private SortedSet<FacetablePropertyFTL<?>> toFacetablePropertyModel(
            Collection<PropertyDefinition> propDefs, Locale locale) {
        SortedSet<FacetablePropertyFTL<?>> result =
                new TreeSet<>(new FacetablePropertyFTLComparator());
    ...
    // Note: the trailing underscore in this method name is to prevent a clash between this method
    // and the
    // one that takes a Collection<PropertyDefinition> as Java's type erasure means that both

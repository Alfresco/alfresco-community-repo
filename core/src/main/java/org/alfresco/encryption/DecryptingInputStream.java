...
/**
 * An input stream that encrypts data produced by a {@link EncryptingOutputStream}. A lightweight
 * yet secure hybrid encryption scheme is used. A random symmetric key is decrypted using the
 * receiver's private key. The supplied data is then decrypted using the symmetric key and read on a
 * streaming basis. When the end of the stream is reached or the stream is closed, a HMAC checksum
 * of the entire stream contents is validated.
 */
public class DecryptingInputStream extends InputStream {
     ...
     *
     * @param wrapped the input stream to decrypt
     * @param privKey the receiver's private key for decrypting the symmetric key
     ...
     */
    public DecryptingInputStream(final InputStream wrapped, final PrivateKey privKey)
    ...
    public DecryptingInputStream(
        ...
        final SecretKey macKey =
                new SecretKeySpec(this.inputCipher.doFinal(readBlock()), "HMACSHA1");
    ...
    /**
     * Reads the next block of data, adding it to the HMAC checksum. Strips the header recording the
    ...
    /**
     * Reads the next block of data, optionally adding it to the HMAC checksum. Strips the header
     * recording the number of bytes in the block.
     *
     * @param updateMac should the block be added to the HMAC checksum?
     * @return the data block, or <code>null</code> if the end of the stream has been reached
     * @throws IOException Signals that an I/O exception has occurred.
     */
    ...
    public int read(final byte b[], int off, final int len) throws IOException {
        ...
        while (bytesToRead > 0) {
            ...
            while (this.currentDataBlock == null
                    || this.currentDataPos >= this.currentDataBlock.length) {
                byte[] newDataBlock;
            ...
            final int bytesRead =
                    Math.min(bytesToRead, this.currentDataBlock.length - this.currentDataPos);
    ...
    public int available() throws IOException {
        return this.currentDataBlock == null
                ? 0
                : this.currentDataBlock.length - this.currentDataPos;
    }

...
/**
 * A base class for chaining authentication components. Where appropriate, methods will 'chain'
 * across multiple {@link AuthenticationComponent} instances, as returned by {@link
...
public abstract class AbstractChainingAuthenticationComponent
    ...
    /**
     * Chain authentication with user name and password - tries all in order until one works, or
     * fails.
     *
    ...
    protected void authenticateImpl(String userName, char[] password) {
        if (logger.isTraceEnabled()) {
            logger.trace(
                    "Chaining authentication for user: "
        ...
        for (AuthenticationComponent authComponent : getUsableAuthenticationComponents()) {
            try {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Not that important: Exception chaining authentication: "
                                    + e.getMessage(),
                            e);
                }
            }
    ...
    public void testAuthenticate(String authenticatorName, String userName, char[] password) {
        ...
        AuthenticationComponent authenticationComponent =
                getAuthenticationComponent(authenticatorName);
        if (authenticationComponent != null) {
            if (authenticationComponent instanceof ActivateableBean) {
                if (!((ActivateableBean) authenticationComponent).isActive()) {
                    ...
                    diagnostic.addStep(
                            AuthenticationDiagnostic.STEP_KEY_VALIDATION_AUTHENTICATOR_NOT_ACTIVE,
                            false,
                            args);
                    throw new AuthenticationException(
                            "authentication.err.validation.authenticator.notactive",
                            args,
                            diagnostic);
                }
            }
        ...
        Object[] args = {authenticatorName};
        diagnostic.addStep(
                AuthenticationDiagnostic.STEP_KEY_VALIDATION_AUTHENTICATOR_NOT_FOUND, false, args);
        throw new AuthenticationException(
                "authentication.err.validation.authenticator.notfound", args, diagnostic);
    }
    ...
    public Authentication setCurrentUser(String userName, UserNameValidationMode validationMode) {
        for (AuthenticationComponent authComponent : getUsableAuthenticationComponents()) {
            try {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Not that important: Exception chaining set current user: "
                                    + e.getMessage(),
                            e);
                }
            }
        ...
        throw new AuthenticationException(
                "Failed to set current user " + AuthenticationUtil.maskUsername(userName));
    }
    ...
    public Authentication setCurrentUser(String userName) {
        ...
        throw new AuthenticationException(
                "Failed to set current user " + AuthenticationUtil.maskUsername(userName), last);
    }

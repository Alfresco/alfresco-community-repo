...
public class DynamicallySizedThreadPoolExecutorTest extends TestCase {
  ...
  private static Log logger = LogFactory.getLog(
    DynamicallySizedThreadPoolExecutorTest.class
  );
  ...
  public void testUpToCore() throws Exception {
    DynamicallySizedThreadPoolExecutor exec = createInstance(
      5,
      10,
  ...
  public void offTestToExpandThenContract() throws Exception {
    ...
    assertTrue(
      "Pool size should be 0-2 as everything is idle, was " +
      exec.getPoolSize(),
      exec.getPoolSize() >= 0
    );
    assertTrue(
    ...
    Thread.sleep(100);
    assertTrue(
      "Pool size should be 0 or 1 as everything is idle, was " +
      exec.getPoolSize(),
      exec.getPoolSize() >= 0
    );
    assertTrue(

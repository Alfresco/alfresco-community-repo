...
public abstract class AbstractAuthorityBridgeDAO implements AuthorityBridgeDAO {
  ...
  public List<AuthorityBridgeLink> getAuthorityBridgeLinks() {
    ...
    Pair<Long, QName> authorityContainerTypeQNamePair = qnameDAO.getQName(
      ContentModel.TYPE_AUTHORITY_CONTAINER
    );
    if (authorityContainerTypeQNamePair != null) {
      authorityContainerTypeQNameId =
        authorityContainerTypeQNamePair.getFirst();
    }

    Long memberAssocQNameId = Long.MIN_VALUE;
    Pair<Long, QName> memberAssocQNamePair = qnameDAO.getQName(
    ...
    Pair<Long, QName> authorityNameQNamePair = qnameDAO.getQName(
    ...
    StoreRef tenantSpecificStoreRef = tenantService.getName(
   ...
   */
  @Override
  public List<AuthorityBridgeLink> getDirectAuthoritiesForUser(
    NodeRef authRef
  ) {
    Long authorityContainerTypeQNameId = Long.MIN_VALUE;
    Pair<Long, QName> authorityContainerTypeQNamePair = qnameDAO.getQName(
    ...
    Pair<Long, NodeRef> pair = (authRef == null)
      ? null
      : nodeDAO.getNodePair(tenantService.getName(authRef));

    return selectDirectAuthoritiesForUser(
      authorityContainerTypeQNameId,
      memberAssocQNameId,
      authorityNameQNameId,
      storeId,
      (pair == null) ? -1L : pair.getFirst()
    );
  }

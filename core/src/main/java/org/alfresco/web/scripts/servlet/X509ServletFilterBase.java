...
public abstract class X509ServletFilterBase implements Filter {
  ...
  public void doFilter(
    ...
    if (this.enforce) {
      ...
      X509Certificate[] certs = (X509Certificate[]) httpRequest.getAttribute(
  ...
  protected abstract boolean checkEnforce(ServletContext servletContext)
    throws IOException;
  ...
  private boolean validCert(X509Certificate[] certs) {
    ...
    if (name.contains(this.certContains)) {
      ...
      logger.error(
        "Cert: " + name + "  does not contain:  " + this.certContains
      );
      return false;
    }
  }
  ...
  private void handleClientAuth() {
    try {
      ...
      if (objectNames != null && objectNames.size() == 0) {
        logger.warn(
          "clientAuth does not appear to be set for Tomcat. clientAuth must be set to 'want' for X509 Authentication"
        );
      ...
      logger.warn(
        "An error occurred while checking for clientAuth. Turn on debug logging to see the stacktrace."
      );

...
public class RecordFolderTests extends BaseRMRestTest {
    ...
    public Object[][] getInvalidNodesForRecordFolders() {
        return new String[][] {
            ...
            },
            {
                createUnfiledContainerChild(
                    UNFILED_RECORDS_CONTAINER_ALIAS,
                    "Unfiled Record " + getRandomAlphanumeric(),
                    CONTENT_TYPE
                )
                    .getId(),
            },
    ...
    public void createInvalidChildrenForFolder(String nodeType) {
        ...
        RecordCategoryChild folder = createRecordFolder(
            rootCategory.getId(),
        ...
        getRestAPIFactory()
            .getRecordFolderAPI()
            .createRecord(record, folder.getId());
    ...
    public void checkRecordFolderDetails() {
        ...
        RecordFolder recordFolder = getRestAPIFactory()
            .getRecordFolderAPI()
            .getRecordFolder(
        ...
        assertEquals(
            recordFolder.getModifiedByUser().getId(),
            getAdminUser().getUsername()
        );
        assertEquals(
            recordFolder.getProperties().getTitle(),
            TITLE_PREFIX + RECORD_FOLDER_NAME
        );
        assertNotNull(
            recordFolder.getProperties().getIdentifier(),
            "The record folder doesn't have a identifier"
        );
        assertFalse(
            recordFolder.getProperties().getVitalRecordIndicator(),
            "The record folder has the vital record identifier"
        );
        assertFalse(
            recordFolder.getProperties().getIsClosed(),
            "The record folder is closed"
        );
    }
    ...
    public void updateRecordFolderDetails() {
        ...
        String folderDescription =
            "The folder description is updated" + getRandomAlphanumeric();
        String folderName =
            "The folder name is updated" + getRandomAlphanumeric();
        String folderTitle = "Update title " + getRandomAlphanumeric();
        String location = "Location " + getRandomAlphanumeric();

        // Create the record folder properties to update
        RecordFolder recordFolder = RecordFolder
            .builder()
            .name(folderName)
            .properties(
        ...
        RecordFolder updatedRecordFolder = getRestAPIFactory()
            .getRecordFolderAPI()
            .updateRecordFolder(recordFolder, recordCategoryChild.getId());
        ...
        AssertJUnit.assertEquals(
            recordFolderProperties.getDescription(),
            folderDescription
        );
        AssertJUnit.assertEquals(
            recordFolderProperties.getTitle(),
            folderTitle
        );
        assertTrue(recordFolderProperties.getVitalRecordIndicator());
        AssertJUnit.assertEquals(
            recordFolderProperties.getLocation(),
            location
        );
    ...
    public void updateOtherNodeTypesDetails(String nodeId) {
        ...
        getRestAPIFactory()
            .getRecordFolderAPI()
            .updateRecordFolder(recordFolder, nodeId);
    ...
    public void openClosedRecordFolder() {
        ...
        RecordFolder updatedRecordFolder = recordFolderAPI.updateRecordFolder(
            recordFolderModel,
            recordFolder.getId()
        );
        ...
        recordFolderModel =
            RecordFolder
                .builder()
                .properties(
                    RecordFolderProperties.builder().isClosed(false).build()
                )
                .build();

        // Make a request to reopen the record folder
        updatedRecordFolder =
            recordFolderAPI.updateRecordFolder(
                recordFolderModel,
                recordFolder.getId()
            );
    ...
    public void listRecordsFromRecordFolder() {
        ...
        String containerId = createRecordFolder(
            rootCategory.getId(),
            getRandomName("recFolder")
        )
            .getId();
        ...
        for (int i = 0; i < NUMBER_OF_RECORDS; i++) {
            ...
            Record record = Record
                ...
                .build();
            //create a child
            Record child = recordFolderAPI.createRecord(
                record,
                containerId,
                createTempFile(
        ...
        RecordFolderCollection apiChildren = (RecordFolderCollection) recordFolderAPI
            .getRecordFolderChildren(containerId, "include=properties")
            .assertThat()
        ...
        apiChildren
            ...
            .forEach(c -> {
                ...
                try {
                    ...
                    Record createdComponent = children
                        .stream()
                    ...
                    assertEquals(
                        ...
                        "Record Name" +
                        record.getName() +
                        " doesn't match the one returned on create"
                    );
                    assertTrue(
                        createdComponent
                            .getName()
                        ...
                        "Record Name" +
                        createdComponent.getName() +
                        " doesn't contain the record identifier in response when creating"
                    );
                    assertEquals(

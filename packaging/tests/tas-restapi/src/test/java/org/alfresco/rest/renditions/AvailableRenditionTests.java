 ...
 * Renditions are requested based on supported renditions according to GET
 * '/nodes/{nodeId}/renditions'. <br>
 */
@Test(groups = {TestGroup.RENDITIONS})
public class AvailableRenditionTests extends RenditionIntegrationTests {
    /**
     * List of parameters used as an input to supportedRenditionTest - fileName, nodeId,
     * renditionId, expectedMimeType *
     */
    ...
    public void dataPreparation() throws Exception {
        ...
        List<String> toTest =
                Arrays.asList(
                        "doc", "xls", "ppt", "docx", "xlsx", "pptx", "msg", "pdf", "png", "gif",
                        "jpg");
    ...
    private List<Object[]> uploadFileAndGetAvailableRenditions(String sourceFile) throws Exception {
        ...
        RestRenditionInfoModelCollection renditionsInfo =
                restClient.withCoreAPI().usingNode(file).getNodeRenditionsInfo();
        for (RestRenditionInfoModel m : renditionsInfo.getEntries()) {
            ...
            renditionsForFile.add(
                    new Object[] {sourceFile, fileNode.getId(), renditionId, targetMimeType});
        }
    ...
    /**
     * Check that a particular rendition can be created for the file and that it has the expected
     * Mime type
     */
    @Test(
            dataProvider = "RenditionTestDataProvider",
            groups = {TestGroup.REST_API, TestGroup.RENDITIONS, TestGroup.SANITY})
    @TestRail(
            section = {TestGroup.REST_API, TestGroup.RENDITIONS},
            executionType = ExecutionType.SANITY,
            description =
                    "Verify renditions created for a selection of test files via POST"
                            + " nodes/{nodeId}/renditions")
    public void supportedRenditionTest(
            String fileName, String nodeId, String renditionId, String expectedMimeType)
            throws Exception {
    ...
    public void renditionsAvailableTest() {
        Assert.assertFalse(
                renditionsToTest.isEmpty(),
                "No available renditions were reported for any of the test files.");
    }

    @Test(
            dataProvider = "UnsupportedRenditionTestDataProvider",
            groups = {TestGroup.REST_API, TestGroup.RENDITIONS, TestGroup.SANITY})
    @TestRail(
            section = {TestGroup.REST_API, TestGroup.RENDITIONS},
            executionType = ExecutionType.SANITY,
    ...
    public void unsupportedRenditionTest(String sourceFile, String renditionId) throws Exception {
        ...
        Assert.assertEquals(
                Integer.valueOf(restClient.getStatusCode()).intValue(),
                HttpStatus.BAD_REQUEST.value(),
                "Expected to see the rendition rejected. ["
                        + sourceFile
                        + ", "
                        + renditionId
                        + "] [source file, rendition ID] ");
    }

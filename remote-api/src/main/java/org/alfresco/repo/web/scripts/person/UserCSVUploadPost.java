...
public class UserCSVUploadPost extends DeclarativeWebScript {
  ...
  private static final String ERROR_CORRUPT_FILE =
    "person.err.userCSV.corruptFile";
  private static final String ERROR_GENERAL = "person.err.userCSV.general";
  private static final String ERROR_BLANK_COLUMN =
    "person.err.userCSV.blankColumn";
  ...
  private void doAddUsers(
    final MutableInt totalUsers,
    final MutableInt addedUsers,
    final Map<String, String> results,
    final List<Map<QName, String>> users,
    ...
    for (Map<QName, String> user : users) {
      ...
      try {
        username =
          PersonServiceImpl.updateUsernameForTenancy(username, tenantService);
      ...
      if (personService.personExists(username)) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Not creating user as already exists: " + username + " for " + user
          );
        }
      } else {
        ...
        try {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug(
              "Creating user from upload: " + username + " for " + user
            );
          }
  ...
  protected void processCSVUpload(
    InputStream input,
    List<Map<QName, String>> users
  ) throws IOException {
  ...
  protected void processXLSUpload(
    InputStream input,
    List<Map<QName, String>> users
  ) throws IOException {
  ...
  protected void processXLSXUpload(
    InputStream input,
    List<Map<QName, String>> users
  ) throws IOException {
  ...
  private void processSpreadsheetUpload(
    Workbook wb,
    List<Map<QName, String>> users
  ) throws IOException {
    if (wb.getNumberOfSheets() > 1) {
      logger.info(
        "Uploaded Excel file has " +
    ...
    if (
      pane != null && pane.isFreezePane() && pane.getHorizontalSplitTopRow() > 0
    ) {
  ...
  private void processSpreadsheetUpload(
    ...
    PropertyDefinition unPD = dictionaryService.getProperty(
      ContentModel.PROP_USERNAME
    );
    if (unPD != null) {
      if (unPD.getTitle(dictionaryService) != null) usernameIsUsername.add(
        unPD.getTitle(dictionaryService).toLowerCase()
      );
      if (
        unPD.getDescription(dictionaryService) != null
      ) usernameIsUsername.add(
        unPD.getDescription(dictionaryService).toLowerCase()
      );
    }
  ...
  protected static class ResourceBundleWebScriptException
    ...
    public ResourceBundleWebScriptException(
      int status,
      ResourceBundle rb,
      String msgId,
      Object... args
    ...
    private void buildMessageIfPossible(
      ResourceBundle rb,
      String msgId,
      Object... args
    ) {

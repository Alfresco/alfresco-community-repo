...
public class MultilingualContentServiceImplTest
  ...
  public void testMakeTranslation() throws Exception {
    ...
    NodeRef mlContainerNodeRef = multilingualContentService.getTranslationContainer(
      chineseContentNodeRef
    );
    // Check it
  ...
  public void testAddTranslationUsingContent() throws Exception {
    ...
    NodeRef newMLContainerNodeRef = multilingualContentService.getTranslationContainer(
      frenchContentNodeRef
    );
  ...
  public void testGetMissingTranslation() throws Exception {
    ...
    List<Locale> missing = multilingualContentService.getMissingTranslations(
      nodeRef1,
      false
    );
    ...
    missing =
      multilingualContentService.getMissingTranslations(nodeRef1, false);
  ...
  public void testCreateEmptyTranslation() throws Exception {
    ...
    NodeRef emptyNodeRef = multilingualContentService.addEmptyTranslation(
      chineseContentNodeRef,
      "Document.txt",
      Locale.CANADA
    );
    ...
    String emptyName = DefaultTypeConverter.INSTANCE.convert(
      String.class,
    ...
    ContentData chineseContentData = fileFolderService
      .getReader(chineseContentNodeRef)
      .getContentData();
    ContentData emptyContentData = fileFolderService
  ...
  public void testCreateEmptyTranslationNames() throws Exception {
    ...
    assertEquals(
      "Empty translation name not generated correctly.",
      "Document_en_CA.txt",
      nullName
    );
    ...
    String differentName = fileFolderService
      .getFileInfo(differentNameNodeRef)
      .getName();
    assertEquals(
      "Empty translation name not generated correctly.",
      "Document2.txt",
  ...
  public void testDeleteMultilingualContent() throws Exception {
    ...
    NodeRef archivedChineseContentNodeRef = nodeArchiveService.getArchivedNode(
      chineseContentNodeRef
    );
    NodeRef archivedFrenchContentNodeRef = nodeArchiveService.getArchivedNode(
      frenchContentNodeRef
    );
    NodeRef archivedJapaneseContentNodeRef = nodeArchiveService.getArchivedNode(
      japaneseContentNodeRef
    );
    NodeRef archivedEmptyGermanContentNodeRef = nodeArchiveService.getArchivedNode(
      emptyGermanContentNodeRef
    );
    NodeRef archivedMlContainerNodeRef = nodeArchiveService.getArchivedNode(
      mlContainerNodeRef
    );
    ...
    assertFalse(
      ...
      Locale.JAPANESE,
      nodeService.hasAspect(
        archivedJapaneseContentNodeRef,
        ContentModel.ASPECT_MULTILINGUAL_DOCUMENT
      )
    );
    assertFalse(
  ...
  public void testCopyMLContainerInNewSpace() throws Exception {
    ...
    // create a new space
    NodeRef destinationSpace = fileFolderService
      .create(
        folderNodeRef,
    ...
    NodeRef newMLContainer = multilingualContentService.copyTranslationContainer(
      mlContainerNodeRef,
      destinationSpace,
      ""
    );
    ...
    assertTrue(
      "The new mlContainer is not created ",
      nodeService.exists(newMLContainer)
    );
    ...
    assertEquals(
      "The newMLContainer is not a copy of the source mlContainer, the locales are not the same " +
      newMLContainer,
      nodeService.getProperty(mlContainerNodeRef, ContentModel.PROP_LOCALE),
      nodeService.getProperty(newMLContainer, ContentModel.PROP_LOCALE)
    );
    assertEquals(
      "The newMLContainer is not a copy of the source mlContainer, the authors are not the same " +
      newMLContainer,
    ...
    Map<Locale, NodeRef> sourceTranslations = multilingualContentService.getTranslations(
      mlContainerNodeRef
    );
    // get the copies
    Map<Locale, NodeRef> copyTranslations = multilingualContentService.getTranslations(
      newMLContainer
    );

    // Ensure that the translations are copies from the source translations
    assertEquals(
      "They are not the same number of translation in the source mlContainer and in its copy",
    ...
    for (Map.Entry<Locale, NodeRef> entry : sourceTranslations.entrySet()) {
      ...
      NodeRef sourceParent = nodeService
        .getPrimaryParent(sourceNodeRef)
        .getParentRef();

      NodeRef copyTranslation = multilingualContentService.getTranslationForLocale(
        newMLContainer,
        locale
      );
      NodeRef copyParent = nodeService
        .getPrimaryParent(copyTranslation)
        .getParentRef();

      // Ensure that the copy exists
      assertNotNull("No copy found for the locale " + locale, copyTranslation);
      assertTrue(
        "No copy exists for the locale " + locale,
        nodeService.exists(copyTranslation)
      );
      ...
      assertEquals(
        "The call of nodeService.hasAspect(nodeRef, ContentModel.ASPECT_MULTILINGUAL_EMPTY_TRANSLATION) must return the same result for the source and the copy",
        nodeService.hasAspect(
          sourceNodeRef,
          ContentModel.ASPECT_MULTILINGUAL_EMPTY_TRANSLATION
        ),
      ...
      assertNotSame(
        "The copy has the same ref as the source",
        sourceNodeRef,
        copyTranslation
      );
      ...
      assertEquals(
        "The copy is not in the right space",
        copyParent,
        destinationSpace
      );
    }
  ...
  public void testCopyAndRenameMLContainer() throws Exception {
    ...
    for (Map.Entry<Locale, NodeRef> entry : sourceTranslations.entrySet()) {
      ...
      NodeRef sourceNodeRef = entry.getValue();
      NodeRef copyNodeRef = multilingualContentService.getTranslationForLocale(
        newMLContainer,
        locale
      );
      ...
      String copyName = (String) nodeService.getProperty(
        copyNodeRef,
        ContentModel.PROP_NAME
      );
  ...
  public void testMoveMLContainer() throws Exception {
    ...
    multilingualContentService.moveTranslationContainer(
      mlContainerNodeRef,
      destinationSpace
    );

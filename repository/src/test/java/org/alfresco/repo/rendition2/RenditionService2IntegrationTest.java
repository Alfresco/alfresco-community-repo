...
public class RenditionService2IntegrationTest extends AbstractRenditionIntegrationTest {
    ...
    public void changedSourceToNonNull() {
        ...
        ContentData contentData1 =
                DefaultTypeConverter.INSTANCE.convert(
                        ContentData.class, nodeService.getProperty(rendition1, PROP_CONTENT));
        ...
        ContentData contentData2 =
                DefaultTypeConverter.INSTANCE.convert(
                        ContentData.class, nodeService.getProperty(rendition2, PROP_CONTENT));

        assertEquals("The rendition node should not change", rendition1, rendition2);
        assertNotEquals(
                "The content should have change", contentData1.toString(), contentData2.toString());
    }
                ...
                .doInTransaction(
                        () -> {
                            permissionService.setPermission(
                                    sourceNodeRef, otherUserName, PermissionService.READ, true);
                            return null;
                        });
    ...
    public void testUpgradeRenditionService() throws InterruptedException {
        String ownerUserName = createRandomUser();
        NodeRef sourceNodeRef = createSource(ownerUserName, "quick.jpg");
        final QName doclibRendDefQName =
                QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "doclib");
        transactionService
                .getRetryingTransactionHelper()
                .doInTransaction(
                        () ->
                                AuthenticationUtil.runAs(
                                        () ->
                                                renditionService.render(
                                                        sourceNodeRef, doclibRendDefQName),
                                        ownerUserName));
        ...
        assertTrue(
                "The rendition should be generated by new Rendition Service", hasRenditionedAspect);
    }
    ...
    public void testUseOldService() throws InterruptedException {
        ...
        try {
            ...
            assertNotNull(
                    ...
                    waitForRendition(ADMIN, sourceNodeRef, DOC_LIB, true));
            List<String> lastThumbnailModification =
                    transactionService
                            .getRetryingTransactionHelper()
                            .doInTransaction(
                                    () ->
            ...
            assertFalse(
                    ...
                    AuthenticationUtil.runAs(
                            () ->
                                    nodeService.hasAspect(
                                            renditionNodeRef, RenditionModel.ASPECT_RENDITION2),
                            ADMIN));
        } finally {
    ...
    public void testSwitchToNewServiceViaContentUpdate() {
        ...
        try {
            ...
            NodeRef renditionNodeRef = waitForRendition(ADMIN, sourceNodeRef, DOC_LIB, true);
            boolean hasAspect =
                    nodeService.hasAspect(renditionNodeRef, RenditionModel.ASPECT_RENDITION2);
    ...
    public void testDowngradeRenditionService() throws InterruptedException {
        ...
        assertTrue(
                ...
                AuthenticationUtil.runAs(
                        () ->
                                nodeService.hasAspect(
                                        sourceNodeRef, RenditionModel.ASPECT_RENDITIONED),
                        ownerUserName));

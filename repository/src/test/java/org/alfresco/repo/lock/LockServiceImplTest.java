...
public class LockServiceImplTest extends BaseSpringTest {
  ...
  private class LockServicePoliciesImpl
    implements
      LockServicePolicies.BeforeLock, LockServicePolicies.BeforeUnlock {

    @Override
    public void beforeLock(NodeRef nodeRef, LockType lockType) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Invoked beforeLock() for nodeRef: " + nodeRef + lockType != null
  ...
  public void before() throws Exception {
    ...
    this.securedLockService =
      (LockService) applicationContext.getBean("LockService");
    ...
    AuthenticationComponent authComponent = (AuthenticationComponent) this.applicationContext.getBean(
  ...
  public void testLockServicePolicies() {
    LockServicePoliciesImpl mockedLockServicePoliciesImpl = mock(
      LockServicePoliciesImpl.class
    );

    BehaviourDefinition<ClassBehaviourBinding> lockDef =
      this.policyComponent.bindClassBehaviour(
          LockServicePolicies.BeforeLock.QNAME,
    ...
    BehaviourDefinition<ClassBehaviourBinding> unlockDef =
      this.policyComponent.bindClassBehaviour(
          LockServicePolicies.BeforeUnlock.QNAME,
          ContentModel.TYPE_BASE,
    ...
    verify(mockedLockServicePoliciesImpl, times(1))
      .beforeLock(this.parentNode, LockType.WRITE_LOCK);

    this.lockService.unlock(this.parentNode);

    verify(mockedLockServicePoliciesImpl, times(1))
      .beforeUnlock(this.parentNode);
  ...
  public void testLock() {
    ...
    assertEquals(
      LockType.WRITE_LOCK.toString(),
      props.get(ContentModel.PROP_LOCK_TYPE)
    );
    assertEquals(
      Lifetime.PERSISTENT.toString(),
      props.get(ContentModel.PROP_LOCK_LIFETIME)
    );
    ...
    try {
      this.lockService.lock(this.parentNode, LockType.WRITE_LOCK);
      fail(
        "The user should not be able to lock the node since it is already locked by another user."
      );
    ...
    try {
      ...
      fail(
        "No error should be thrown when a node is re-locked by the current lock owner."
      );
    }
  ...
  public void testEphemeralLock() {
    ...
    assertEquals(
      false,
      nodeService.hasAspect(noAspectNode, ContentModel.ASPECT_LOCKABLE)
    );
    ...
    assertEquals(
      "some extra data",
      lockService.getAdditionalInfo(noAspectNode)
    );
    ...
    assertEquals(
      LockStatus.LOCK_OWNER,
      lockService.getLockStatus(noAspectNode)
    );
    assertTrue(lockService.isLocked(noAspectNode));
    ...
    NodeService fullNodeService = (NodeService) applicationContext.getBean(
      "nodeService"
    );
    assertEquals(
      true,
      fullNodeService.hasAspect(noAspectNode, ContentModel.ASPECT_LOCKABLE)
    );
  ...
  public void testEphemeralLockIndexing() {
    ...
    IndexerAndSearcher indexerAndSearcher = (IndexerAndSearcher) applicationContext.getBean(
    ...
    final String query = String.format(
      "+@cm\\:lockOwner:\"%s\" +@cm\\:lockType:\"WRITE_LOCK\"",
      GOOD_USER_NAME
    );
  ...
  public void testEphemeralLockModifyNode() {
    ...
    try {
      ...
      props.put(
        ...
        "descr" + System.currentTimeMillis()
      );
    ...
    /* removeProperty test */
    try {
      fullNodeService.removeProperty(
        noAspectNode,
        ContentModel.PROP_DESCRIPTION
      );
  ...
  public void testExpiredEphemeralLockAndPersitentLock() {
    ...
    assertEquals(
      LockStatus.NO_LOCK,
      securedLockService.getLockStatus(noAspectNode)
    );
    ...
    assertEquals(
      LockType.READ_ONLY_LOCK,
      securedLockService.getLockType(noAspectNode)
    );
    // Check that we can retrieve LockState
  ...
  public void testUnlock() {
    ...
    try {
      ...
      fail(
        "Unlocking an unlocked node should not result in an exception being raised."
      );
    }
  ...
  public void testGetLocks() {
    ...
    List<NodeRef> locked3 = lockService.getLocks(
      this.storeRef,
      LockType.WRITE_LOCK
    );
    ...
    List<NodeRef> locked4 = lockService.getLocks(
      this.storeRef,
      LockType.READ_ONLY_LOCK
    );
  ...
  public void testEphemeralExpiryThreshold() {
    ...
    final int origThresh =
      ((LockServiceImpl) lockService).getEphemeralExpiryThreshold();
  ...
  private void checkLifetimeForExpiry(
  ...
  public void testCreateChildrenOfLockedNodes() throws Exception {
    ...
    try {
      ...
      logger.debug(
        "exception while trying to create a child of a read only lock",
        e
      );
    }
    ...
    for (Lifetime lt : new Lifetime[] {
      ...
      final NodeRef testNode =
        this.nodeService.createNode(
            parentNode,
            ContentModel.ASSOC_CONTAINS,
            QName.createQName("{}testNode"),
            ContentModel.TYPE_CONTAINER
          )
          .getChildRef();

      // lock it as GOOD user
      this.securedLockService.lock(
          testNode,
          LockType.WRITE_LOCK,
          2 * 86400,
          lt,

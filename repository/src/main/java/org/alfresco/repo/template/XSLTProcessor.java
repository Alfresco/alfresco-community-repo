...
public class XSLTProcessor extends BaseProcessor implements TemplateProcessor {
    ...
    public static final QName ROOT_NAMESPACE = QName.createQName(null, "root_namespace");
    ...
    private void process(TemplateSource templateSource, Object model, Writer out) {
        ...
        System.setProperty(
                "org.apache.xalan.extensions.bsf.BSFManager", BSFManager.class.getName());
        ...
        try {
            ...
            throw new TemplateException(
                    MSG_UNABLE_TO_PARSE_TEMPLATE, new Object[] {sax.getMessage()}, sax);
        } finally {
            try {
        ...
        try {
            ...
            if (errors.size() != 0) {
                final StringBuilder msg =
                        new StringBuilder("errors encountered creating tranformer ... \n");
        ...
        if (errors.size() != 0) {
            final StringBuilder msg =
                    new StringBuilder("errors encountered during transformation ... \n");
            for (TransformerException te : errors) {
                msg.append(te.getMessageAndLocation()).append("\n");
            }
            throw new TemplateException(msg.toString());
    ...
    /**
     * Adds a script element to the xsl which makes static methods on this object available to the
     * xsl tempalte.
     *
     * @param xslTemplate the xsl template
     */
    protected List<String> addScripts(
            final XSLTemplateModel xsltModel, final Document xslTemplate) {
        ...
        for (QName ns : methods.keySet()) {
            ...
            final Element compEl =
                    xslTemplate.createElementNS(XALAN_NS, XALAN_NS_PREFIX + ":component");
            compEl.setAttribute("prefix", prefix);
            docEl.appendChild(compEl);
            String functionNames = null;
            final Element scriptEl =
                    xslTemplate.createElementNS(XALAN_NS, XALAN_NS_PREFIX + ":script");
            ...
            final StringBuilder js =
                    new StringBuilder(
                            "var _xsltp_invoke = java.lang.Class.forName('"
                                    ...
                                    + "var java_array ="
                                    + " java.lang.reflect.Array.newInstance(java.lang.Object,"
                                    + " js_array.length);\n"
                                    + "for (var i = 0; i < js_array.length; i++) { java_array[i] ="
                                    + " js_array[i]; }\n"
                                    + "return java_array; }\n");
            for (final Map.Entry<QName, Object> entry : methods.get(ns)) {
                ...
                XSLTProcessorMethodInvoker.addMethod(
                        id, (TemplateProcessorMethod) entry.getValue());
                result.add(id);
            }
     ...
     *
     * @param xsltModel the variables to place within the xsl template
     * @param xslTemplate the xsl template
     */

...
public class BasicCorrespondingNodeResolverImpl implements CorrespondingNodeResolver {
    ...
    public ResolvedParentChildPair resolveCorrespondingNode(
        ...
        if (!nodeService.exists(parentNodeRef)) {
            ...
            parentNodeRef =
                    resolveParentPath(primaryAssoc.getParentRef().getStoreRef(), parentPath);
        } else {
            if (log.isDebugEnabled()) {
        ...
        if ((parentNodeRef != null) && (result.resolvedChild == null)) {
            ...
            List<ChildAssociationRef> children =
                    nodeService.getChildAssocs(
                            parentNodeRef, RegexQNamePattern.MATCH_ALL, primaryAssoc.getQName());
            if (!children.isEmpty()) {
    ...
    private NodeRef resolveParentPath(StoreRef store, Path parentPath) {
        ...
        while (index < parentPath.size()) {
            ...
            List<ChildAssociationRef> children =
                    nodeService.getChildAssocs(node, RegexQNamePattern.MATCH_ALL, name);

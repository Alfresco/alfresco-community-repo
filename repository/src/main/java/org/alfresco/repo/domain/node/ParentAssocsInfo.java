...
public class ParentAssocsInfo implements Serializable {
  ...
  ParentAssocsInfo(
    boolean isRoot,
    boolean isStoreRoot,
    List<? extends ChildAssocEntity> parents
  ) {
    this.isRoot = isRoot;
  ...
  public ChildAssocEntity getPrimaryParentAssoc() {
    return (primaryAssocId != null)
      ? parentAssocsById.get(primaryAssocId)
      : null;
  }
  ...
  public ParentAssocsInfo addAssoc(Long assocId, ChildAssocEntity parentAssoc) {
    Map<Long, ChildAssocEntity> parentAssocs = new HashMap<Long, ChildAssocEntity>(
      parentAssocsById
    );

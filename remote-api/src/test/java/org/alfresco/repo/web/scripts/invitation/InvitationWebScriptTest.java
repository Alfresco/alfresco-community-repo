...
public class InvitationWebScriptTest extends BaseWebScriptTest {
    private static final QName avatarQName =
            QName.createQName(NamespaceService.APP_MODEL_1_0_URI, "test");
    ...
    private final Map<String, Map<String, String>> userProperties =
            new HashMap<String, Map<String, String>>(3);
    ...
    private JSONObject createSite(
        ...
        Response response =
                sendRequest(
                        new PostRequest(URL_SITES, site.toString(), "application/json"),
                        expectedStatus);
    ...
    protected void testInvitationsGet(boolean requireAcceptance) throws Exception {
        ...
        createSite(
                "myPreset", shortNameSiteA, "myTitle", "myDescription", SiteVisibility.PUBLIC, 200);

        String shortNameSiteB = GUID.generate();
        createSite(
                "myPreset", shortNameSiteB, "myTitle", "myDescription", SiteVisibility.PUBLIC, 200);
        ...
        String moderatedIdAUSER_TWO =
                createModeratedInvitation(shortNameSiteA, inviteeComments, userName, roleName);

        // Create a moderated invitation on SiteB, USER_TWO
        String moderatedIdBUSER_TWO =
                createModeratedInvitation(shortNameSiteB, inviteeComments, userName, roleName);
        ...
        String moderatedIdBUSER_THREE =
                createModeratedInvitation(shortNameSiteB, inviteeCommentsB, userNameB, roleNameB);
        ...
        // search by user - find USER_TWO's three invitations
        {
            Response response =
                    sendRequest(
                            new GetRequest(URL_INVITATIONS + "?inviteeUserName=" + userTwo), 200);
        ...
        {
            Response response =
                    sendRequest(new GetRequest(URL_INVITATIONS + "?invitationType=MODERATED"), 200);
            JSONObject top = new JSONObject(response.getContentAsString());
            // System.out.println(response.getContentAsString());
            JSONArray data = top.getJSONArray("data");
        ...
        // Search by type and site
        {
            Response response =
                    sendRequest(
                            new GetRequest(
                                    URL_INVITATIONS
                                            + "?invitationType=MODERATED&resourceName="
                                            + shortNameSiteA
    ...
    /**
     * Detailed Test of List Invitation Web Script using legacy workflow which requires user
     * acceptance Using URL: /api/invitations
     *
     * @throws Exception
     */
    ...
    protected void testSiteInvitationsGet(boolean requireAcceptance) throws Exception {
        ...
        {
            String siteAModeratedUrl =
                    URL_SITES + "/" + shortNameSiteA + "/invitations?invitationType=MODERATED";
            ...
            assertEquals(
                    "Wrong invitation type", "MODERATED", invitation.getString("invitationType"));
            JSONObject moderatedATwoInv = getInvitation(moderatedIdAUSER_TWO, data);
            assertNotNull("first is null", moderatedATwoInv);
        }
        ...
        if (requireAcceptance) {
            String siteANominatedUrl =
                    URL_SITES + "/" + shortNameSiteA + "/invitations?invitationType=NOMINATED";
            ...
            JSONObject invitation = data.getJSONObject(0);
            assertEquals(
                    "Wrong invitation type", "NOMINATED", invitation.getString("invitationType"));
        ...
        {
            String siteBUser2Url =
                    URL_SITES + "/" + shortNameSiteB + "/invitations?inviteeUserName=" + userTwo;
        ...
        {
            String siteBUser2Url =
                    URL_SITES + "/" + shortNameSiteB + "/invitations?inviteeUserName=" + userThree;
            Response response = sendRequest(new GetRequest(siteBUser2Url), 200);
            JSONObject top = new JSONObject(response.getContentAsString());
            // System.out.println(response.getContentAsString());
            JSONArray data = top.getJSONArray("data");
            ...
            JSONObject invitation = data.getJSONObject(0);
            assertEquals(
                    "Wrong invitation user", userThree, invitation.getString("inviteeUserName"));
            JSONObject moderatedBThreeInv = getInvitation(moderatedIdBUSER_THREE, data);
    ...
    /**
     * Detailed Test of List Invitation Web Script using legacy workflow which requires user
     * acceptance. Using URL: /api/sites/{shortname}/invitations
     *
     * @throws Exception
     */
    ...
    private void checkJSONInvitations(JSONArray data) throws JSONException {
        for (int i = 0; i < data.length(); i++) {
            ...
            assertEquals(
                    "User name is wrong for user: " + i, userId, invitee.getString("userName"));
            assertEquals(
                    "Avatar URI is wrong for user: " + i,
                    expectedProps.get("avatar"),
                    invitee.getString("avatar"));
            assertEquals(
                    "First name is wrong!",
                    expectedProps.get("firstName"),
                    invitee.getString("firstName"));
            assertEquals(
                    "Last name is wrong!",
                    expectedProps.get("lastName"),
                    invitee.getString("lastName"));
        }
    }
    ...
    public void testDeleteInvitation() throws Exception {
        ...
        // search for all invitations to site A: one nominated should be found for user Buffy...
        // Summers
        {
    ...
    private JSONArray queryCurrentInvitationList(String shortNameSiteA)
    ...
    void deleteInvitation(String invitationID, String siteShortName, int expectedStatus)
            throws Exception {

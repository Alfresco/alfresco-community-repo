...
public class LinksRestApiTest extends BaseWebScriptTest {
  ...
  private static final String URL_LINKS_BASE =
    "/api/links/site/" + SITE_SHORT_NAME_LINKS + "/links";
  ...
  private static final String URL_LINKS_DELETE =
    "/api/links/delete/site/" + SITE_SHORT_NAME_LINKS + "/links";
  private static final String URL_LINKS_FETCH =
    "/api/links/link/site/" + SITE_SHORT_NAME_LINKS + "/links/"; // plus path
  ...
  private JSONObject getLink(String name, int expectedStatus) throws Exception {
    Response response = sendRequest(
      new GetRequest(URL_LINKS_FETCH + name),
      expectedStatus
    );
  ...
  private JSONObject createLink(
    ...
    Response response = sendRequest(
      new PostRequest(URL_LINKS_CREATE, json.toString(), "application/json"),
      expectedStatus
    );
    if (expectedStatus == Status.STATUS_OK) {
      JSONObject result = new JSONObject(response.getContentAsString());
   ...
   * Deletes the link
   */
  private JSONObject deleteLink(String name, int expectedStatus)
    throws Exception {
    Response response = sendRequest(
      new DeleteRequest(URL_LINKS_FETCH + name),
      expectedStatus
    );
    if (expectedStatus == Status.STATUS_OK) {
      JSONObject result = new JSONObject(response.getContentAsString());
   ...
   * Deletes the links
   */
  private JSONObject deleteLinks(List<String> names, int expectedStatus)
    throws Exception {
    ...
    Response response = sendRequest(
      new PostRequest(URL_LINKS_DELETE, json.toString(), "application/json"),
      expectedStatus
    );
    if (expectedStatus == Status.STATUS_OK) {
      JSONObject result = new JSONObject(response.getContentAsString());
   ...
   */
  private void pushLinkCreatedDateBack(String name, int daysAgo)
    throws Exception {
  ...
  public void testCreateEditDeleteEntry() throws Exception {
    ...
    assertEquals(
      "Incorrect JSON: " + link.toString(),
      true,
      link.getString("message").contains("updated")
    );
    ...
    assertEquals(
      "Incorrect JSON: " + link.toString(),
      true,
      link.getString("message").contains("deleted")
    );
    ...
    sendRequest(
      new PutRequest(URL_LINKS_UPDATE + name, "{}", "application/json"),
  ...
  public void testXssLinks() throws Exception {
    ...
    mapForCheck.put(
      "http:javasc\\ript:alert('mail.ru')",
      Status.STATUS_BAD_REQUEST
    );
    mapForCheck.put(
      "javas\\0cr\\ip\\00t:alert('dd')",
      Status.STATUS_BAD_REQUEST
    );
    mapForCheck.put("alfresco.my", Status.STATUS_OK);
    mapForCheck.put(
      "javascript:alert('http://somedata.html')",
      Status.STATUS_BAD_REQUEST
    );
    ...
    mapForCheck.put(
      "http:java\\00script:alert('XSS')",
      Status.STATUS_BAD_REQUEST
    );
    mapForCheck.put(
      "http:javas\\0cript:alert('XSS')",
      Status.STATUS_BAD_REQUEST
    );
    mapForCheck.put(
    ...
    link =
      createLink(LINK_TITLE, "Link desc", LINK_URL, false, Status.STATUS_OK);
  ...
  public void testOverallListing() throws Exception {
    ...
    assertEquals(
      LINK_TITLE_THREE + "z",
      entries.getJSONObject(0).getString("title")
    );
    assertEquals(
      LINK_TITLE_THREE + "a",
      entries.getJSONObject(1).getString("title")
    );
  ...
  private void checkLinkPermissions(String siteName) throws Exception {
    ...
    assertTrue(
      "The user sould have permission to create a new link.",
      result
        .getJSONObject("metadata")
        .getJSONObject("linkPermissions")
        .getBoolean("create")
    );
  }
  ...
  public void testCommentLink() throws Exception {
    ...
    int activityNumStart = activityService
      .getSiteFeedEntries(SITE_SHORT_NAME_LINKS)
      .size();
    String name = getNameFromLink(link);
    link = getLink(name, Status.STATUS_OK);
    ...
    feedGenerator.execute();
    activityNumNext =
      activityService.getSiteFeedEntries(SITE_SHORT_NAME_LINKS).size();
    assertEquals(

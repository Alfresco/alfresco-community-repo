...
public class ConfigDataCache
  extends AbstractMTAsynchronouslyRefreshedCache<ConfigData> {
   ...
   * be done by code.
   */
  public void setRepoXMLConfigService(
  ...
  protected ConfigData buildCache(String tenantId) {
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Received request to rebuild config data for tenant: " + tenantId
      );
    }
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Rebuilt config data for tenant: " + tenantId + " (" + configData + ")."
      );
    }
    return configData;
  }
  ...
  public static class ImmutableConfigData extends ConfigData {
    ...
    public ImmutableConfigData(ConfigData configData) {
      ...
      if (configDeployments != null) {
        List<ConfigDeployment> configDeploymentsLocked = Collections.unmodifiableList(
          configDeployments
        );
      ...
      if (elementReaders != null) {
        Map<String, ConfigElementReader> elementReadersLocked = Collections.unmodifiableMap(
          elementReaders
        );
      ...
      if (evaluators != null) {
        Map<String, Evaluator> evaluatorsLocked = Collections.unmodifiableMap(
          evaluators
        );
      ...
      if (sections != null) {
        List<ConfigSection> sectionsLocked = Collections.unmodifiableList(
          sections
        );
      ...
      if (sectionsByArea != null) {
        Map<String, List<ConfigSection>> sectionsByAreaLocked = Collections.unmodifiableMap(
          sectionsByArea
        );

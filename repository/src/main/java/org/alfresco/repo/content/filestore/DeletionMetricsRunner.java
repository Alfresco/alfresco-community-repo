...
public class DeletionMetricsRunner {
  ...
  public DeletionMetricsRunner() {
    ...
    // and a listener
    List<ContentStoreCleanerListener> listeners = new ArrayList<ContentStoreCleanerListener>(
      2
    );
  ...
  private void time(String description) {
    ...
    System.out.println();
    System.out.println(
      String.format("%s took %6.0fms", description, timeTaken)
    );
  }
  ...
  private void cleanContent() {
    ...
    if (deletedUrls < numOrphans) throw new IllegalStateException(
      "Not all the orphans were cleaned."
    );
  }

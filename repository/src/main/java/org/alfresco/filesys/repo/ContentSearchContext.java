 ...
 * outside the context instance itself - this class merely maintains the state of the search results
...
public class ContentSearchContext extends SearchContext implements InFlightCorrectable {
    ...
    public boolean nextFileInfo(FileInfo info) {
        ...
        try {
            // Loop until we get a valid node, might have been deleted since the initial folder
            // search
                ...
                try {
                    ...
                    nextInfo =
                            cifsHelper.getFileInformation(
                                    nextNodeRef, "", false, lockedFilesAsOffline);
            ...
            if (nextInfo.isLinkNode()) {
                // Set a dummy file size for the link data that will be generated if/when the file
                // is opened
            ...
            } else info.setFileType(FileType.RegularFile);
    ...
    public boolean restartAt(FileInfo info) {
        ...
        if (m_lastFileName != null && info.getFileName().equalsIgnoreCase(m_lastFileName)) {
            ...
            if (logger.isDebugEnabled()) logger.debug("Fast search restart - " + m_lastFileName);
            return true;
        }
    ...
    public boolean restartAt(int resumeIdParameter) {
        // Resume ids are one based as zero has special meaning for some protocols, adjust the
        // resume id

...
public final class Export extends Tool {
      ...
      if (args[i].equals("-h") || args[i].equals("-help")) {
        ...
        if (i == args.length || args[i].length() == 0) {
          throw new ToolArgumentException(
            "The value <store> for the parameter -store must be specified"
          );
        }
        ...
        if (i == args.length || args[i].length() == 0) {
          throw new ToolArgumentException(
            "The value <path> for the parameter -path must be specified"
          );
        }
        context.path = args[i];
        ...
        if (i == args.length || args[i].length() == 0) {
          throw new ToolArgumentException(
            "The value <dir> for the parameter -dir must be specified"
          );
        }
        ...
        if (i == args.length || args[i].length() == 0) {
          throw new ToolArgumentException(
            "The value <packagedir> for the parameter -packagedir must be specified"
          );
        }
   ...
   */
  protected @Override /*package*/int execute() throws ToolException {
  ...
  private class ExportContext extends ToolContext {
    ...
    /*package*/void validate() {
      ...
      if (storeRef == null) {
        throw new ToolArgumentException(
          "Store to export from has not been specified."
        );
      }
      if (packageName == null) {
    ...
    private File getPackageFile() {
      String packageFile = (packageName.indexOf('.') != -1)
        ? packageName
        : packageName + ".xml";
    ...
    private File getZipFile() {
      ...
      String zipFile =
        ((iExt != -1) ? packageName.substring(0, iExt) : packageName) + ".acp";

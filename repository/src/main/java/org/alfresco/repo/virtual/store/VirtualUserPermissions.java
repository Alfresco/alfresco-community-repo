...
public class VirtualUserPermissions {
  ...
  public VirtualUserPermissions(VirtualUserPermissions userPermissions) {
    ...
    this.denyReadonlySmartNodes =
      new HashSet<>(userPermissions.denyReadonlySmartNodes);
    init();
  }
  ...
  public void init() {
    ...
    this.denyReadonlySmartNodesFull =
      asFullNamePermissions(denyReadonlySmartNodes);
  }
  ...
  public AccessStatus hasVirtualNodePermission(
    String permission,
    boolean readonly
  ) {
    if (readonly) {
      if (
        denyReadonlySmartNodesFull.contains(permission) ||
        denyReadonlySmartNodes.contains(permission)
      ) {
    ...
    if (
      denySmartNodesFull.contains(permission) ||
      denySmartNodes.contains(permission)
    ) {
      return AccessStatus.DENIED;
    } else if (
      allowSmartNodesFull.contains(permission) ||
      allowSmartNodes.contains(permission)
    ) {
  ...
  public AccessStatus hasQueryNodePermission(String permission) {
    if (
      denyQueryNodesFull.contains(permission) ||
      denyQueryNodes.contains(permission)
    ) {
      return AccessStatus.DENIED;
    } else if (
      allowQueryNodesFull.contains(permission) ||
      allowQueryNodes.contains(permission)
    ) {
      return AccessStatus.ALLOWED;
    } else {
      return AccessStatus.UNDETERMINED;

...
public class HomeFolderProviderSynchronizerTest {
  ...
  private static final QName PROP_PARENT_PATH = QName.createQName(
  ...
  public static void classSetup() throws Exception {
    ...
    personService =
      (PersonServiceImpl) applicationContext.getBean("personService");
    ...
    homeFolderManager =
      (PortableHomeFolderManager) applicationContext.getBean(
        "homeFolderManager"
      );
    largeHomeFolderProvider =
      (RegexHomeFolderProvider) applicationContext.getBean(
        "largeHomeFolderProvider"
      );
    ...
    testHomeFolderProvider =
      new RegexHomeFolderProvider() {
        @Override
        public List<String> getHomeFolderPath(NodeRef person) {
          String parentPath = DefaultTypeConverter.INSTANCE.convert(
            String.class,
            nodeService.getProperty(person, PROP_PARENT_PATH)
          );
          String propPath =
            (
            ...
            homeFolderManager.getPersonProperty(
              person,
    ...
    testHomeFolderProvider.setPropertyName(
    ...
    testHomeFolderProvider.setOnCreatePermissionsManager(
      (PermissionsManager) applicationContext.getBean(
        "defaultOnCreatePermissionsManager"
      )
    );
    testHomeFolderProvider.setOnCreatePermissionsManager(
      (PermissionsManager) applicationContext.getBean(
    ...
    testHomeFolderProvider.setOnReferencePermissionsManager(
      (PermissionsManager) applicationContext.getBean(
        "defaultOnReferencePermissionsManager"
      )
    );
    ...
    homeFolderProviderSynchronizer =
      new HomeFolderProviderSynchronizer(
        transactionService,
        authorityService,
        personService,
        fileFolderService,
  ...
  public void setUp() throws Exception {
    ...
    if (firstTest) {
      ...
      trans.begin();
      rootNodeRef =
        homeFolderManager.getRootPathNodeRef(largeHomeFolderProvider);
      trans.commit();
      trans = null;
  ...
  public void tearDown() throws Exception {
    if (trans != null) {
      try {
        ...
        if (
          (trans.getStatus() == Status.STATUS_ACTIVE) ||
  ...
  private Set<NodeRef> deleteNonAdminGuestUsers() {
    ...
    for (final NodeRef nodeRef : personService.getAllPeople()) {
      final String username = DefaultTypeConverter.INSTANCE.convert(
        String.class,
  ...
  private void deleteUser(
    final Set<NodeRef> adminGuestUserHomeFolders,
    NodeRef person,
    String username,
  ...
  private void deleteNonAdminGuestFolders(
    final Set<NodeRef> adminGuestUserHomeFolders
  ) {
    ...
    // double check.
    for (ChildAssociationRef childAssocs : nodeService.getChildAssocs(
      rootNodeRef
    )) {
      ...
      if (!adminGuestUserHomeFolders.contains(nodeRef)) {
        System.out.println(
          "TearDown remove '" +
          childAssocs.getQName().getLocalName() +
  ...
  private NodeRef createUser(String parentPath, String username)
    throws Exception {
  ...
  private NodeRef createUser(
    String parentPath,
    String username,
    String homeFolderProviderName,
    boolean createHomeDirectory
  ) throws Exception {
    return createUser(
  ...
  private NodeRef createUser(
    ...
    final String homeFolderProviderName,
    final boolean createHomeDirectory
  ) throws Exception {
    final String domainUsername = tenantService.getDomainUser(
      username,
      tenantDomain
    );
    ...
    if (path.length() > 0) {
      ...
      for (String pathElement : path.split("/")) {
        ...
        if (nodeRef == null) {
          ...
          assertTrue(
            "Expected " + currentPath + " to be a folder",
  ...
  private NodeRef createContent(String parentPath, String name)
    throws Exception {
  ...
  private String toPath(NodeRef root, NodeRef homeFolder) {
    ...
    for (int i = rootSize; i < homeFolderSize; i++) {
      ...
      QName folderQName =
        ((Path.ChildAssocElement) element).getRef().getQName();
      if (sb.length() > 0) {
        sb.append('/');
      }
        ...
        public Object doWork() throws Exception {
          if (!tenantAdminService.existsTenant(tenantDomain)) {
            tenantAdminService.createTenant(
              tenantDomain,
              ("admin " + tenantDomain).toCharArray(),
              null
            );
          }
          return null;
  ...
  private void assertHomeFolderLocation(String username, String expectedPath)
    throws Exception {
    assertHomeFolderLocation(
      TenantService.DEFAULT_DOMAIN,
      username,
      expectedPath
    );
  ...
  private void assertHomeFolderLocation(
    String tenantDomain,
    final String username,
    ...
    try {
        ...
        new TenantRunAsWork<Object>() {
          public NodeRef doWork() throws Exception {
            ...
            if (expectedPath != null) {
              assertNotNull(
                "User: " + domainUsername + " home folder should exist",
                homeFolder
              );
            }

            NodeRef rootPath = homeFolderManager.getRootPathNodeRef(
              largeHomeFolderProvider
            );
            String actualPath = toPath(rootPath, homeFolder);
            assertEquals(
              "User: " + domainUsername + " home folder location",
              expectedPath,
              actualPath
            );
            return null;
      ...
      if (
        cause instanceof ComparisonFailure || cause instanceof AssertionError
      ) {
  ...
  public void test07RemoveEmptyParents() throws Exception {
    ...
    assertFalse(
      "Expected the empty parent 'a' to have been removed.",
      exists("a")
    );
  }
  ...
  public void test08KeepEmptyParents() throws Exception {
    ...
    assertHomeFolderLocation("peter", "pe/peter");
    assertTrue(
      "Expected the empty parent 'a/bb/ccc' to still exist as global " +
      "property was set.",
      exists("a/bb/ccc")
    );
  }
  ...
  public void test09KeepNonEmptyParents() throws Exception {
    ...
    assertHomeFolderLocation("peter", "pe/peter");
    assertFalse(
      "Expected the empty parent 'a/bb/ccc' to have been removed.",
      exists("a/bb/ccc")
    );
    assertTrue(
      "Expected the non empty parent 'a/bb' to have been kept.",
      exists("a/bb/ddd")
    );
  }
  ...
  public void test11PathAlreadyInUseByContent() throws Exception {
    System.out.println(
      "testPathAlreadyInUseByContent: EXPECT TO SEE AN EXCEPTION IN THE LOG ======================== "
    );
    ...
    assertFalse(
      "Did not expect there to be a folder in the prefered location.",
      exists("fr/fred")
    );
  ...
  public void test12PathInUseByUser() throws Exception {
    ...
    assertFalse(
      "The Temporary-1 folder should have been removed",
      exists("Temporary-1")
    );
  }
  ...
  public void test13UseFirstAvailableTemporaryFolder() throws Exception {
    ...
    assertTrue(
      "The existing Temporary-1 folder should still exist",
      exists("Temporary-1")
    );
    assertTrue(
      "The existing Temporary-2 folder should still exist",
      exists("Temporary-2")
    );
    assertTrue(
      "The existing Temporary-3 folder should still exist",
      exists("Temporary-3")
    );
    assertTrue(
      "The existing Temporary-4 folder should still exist",
      exists("Temporary-4")
    );
  }
  ...
  public void test14Exception() throws Exception {
    System.out.println(
      "testException: EXPECT TO SEE AN EXCEPTION IN THE LOG ======================== "
    );
  ...
  public void test20OverrideProvider() throws Exception {
    ...
    String providerName = (String) nodeService.getProperty(
      person,
      ContentModel.PROP_HOME_FOLDER_PROVIDER
    );
  ...
  public void test21NoOriginalProvider() throws Exception {
    ...
    assertTrue(
      "Expected the empty parent 'a/b/c' to still exist as original " +
  ...
  public void test23ExtendsAbstractHomeFolderProvider() throws Exception {
    ...
    v1Provider.setOnCreatePermissionsManager(
      largeHomeFolderProvider.getOnCreatePermissionsManager()
    );
    v1Provider.setOnReferencePermissionsManager(
      largeHomeFolderProvider.getOnReferencePermissionsManager()
    );
  ...
  public void test24TenantService() throws Exception {
    ...
    if (tenantAdminService.isEnabled()) {
      ...
      assertHomeFolderLocation(
        tenant1,
        "fred",
        "fr/" + tenantService.getDomainUser("fred", tenant1)
      );
      assertHomeFolderLocation(
        tenant2,
        "fred",
        "fr/" + tenantService.getDomainUser("fred", tenant2)
      );
    }
  }
  ...
  public void test27UpdateTenancy() throws Exception {
    ...
    String tUser = personService.updateUsernameForTenancy(
      "Bloggz",
      tenantService
    );

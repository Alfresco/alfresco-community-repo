 ...
 *
 * <p>Uses the commons-net ftp client library to connect to the Alfresco FTP server.
 */
...
public class FTPServerTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        ServerConfigurationAccessor fileServerConfiguration =
                (ServerConfigurationAccessor) applicationContext.getBean("fileServerConfiguration");
        ftpConfigSection =
                (FTPConfigSection)
                        fileServerConfiguration.getConfigSection(FTPConfigSection.SectionName);
                    ...
                    public Void execute() throws Throwable {
                        {
                            ...
                            permissionService.setPermission(
                                    userOneHome, USER_TWO, PermissionService.CONTRIBUTOR, true);
                            permissionService.setPermission(
                                    userOneHome, USER_TWO, PermissionService.WRITE, true);
                        }
                        return null;
                    }
                };
    ...
    /**
     * Simple negative test that connects to the inbuilt ftp server and attempts to log on with the
     * wrong password.
     *
     * @throws Exception
     */
    ...
    public void testCWD() throws Exception {
        ...
        try {
            ...
            reply = ftp.cwd("/Alfresco/User*Homes");
            assertTrue(
                    "unable to change to /Alfresco User*Homes/",
                    FTPReply.isPositiveCompletion(reply));

            //            // Single char pattern match
            //            reply = ftp.cwd("/Alfre?co");
            //            assertTrue("Unable to match single char /Alfre?co",
            // FTPReply.isPositiveCompletion(reply));

            // two level folder
            reply = ftp.cwd("/Alfresco/Data Dictionary");
            assertTrue(
                    "unable to change to /Alfresco/Data Dictionary",
                    FTPReply.isPositiveCompletion(reply));
     ...
     *
     * <p>RFC959 states that paths are constructed thus... <string> ::= <char> | <char><string>
     * <pathname> ::= <string> <char> ::= any of the 128 ASCII characters except <CR> and <LF>
     *
     * <p>So we need to check how high characters and problematic are encoded
     */
     ...
     *
     * <p>Create a folder containing test.docx as user one Update that file as user two. Check user
     * one can see user two's changes.
     *
     * @throws Exception
     */
    public void testTwoUserUpdate() throws Exception {
        ...
        try {
            ...
            success =
                    ftpOne.appendFile(
                            FILE1_NAME,
                            new ByteArrayInputStream(FILE1_CONTENT_1.getBytes("UTF-8")));
            ...
            success =
                    ftpTwo.storeFile(
                            FILE1_NAME,
                            new ByteArrayInputStream(FILE1_CONTENT_2.getBytes("UTF-8")));
    ...
    /**
     * Test a quota failue exception over FTP. A file should not exist after a create and quota
     * exception.
     */
    public void testFtpQuotaAndFtp() throws Exception {
        // Enable usages
        ContentUsageImpl contentUsage =
                (ContentUsageImpl) applicationContext.getBean("contentUsageImpl");
        contentUsage.setEnabled(true);
        contentUsage.init();
        UserUsageTrackingComponent userUsageTrackingComponent =
                (UserUsageTrackingComponent)
                        applicationContext.getBean("userUsageTrackingComponent");
        ...
        try {
            ...
            String FILE3_CONTENT_3 =
                    "test file 3 content that needs to be greater than 100 bytes to result in a"
                            + " quota exception being thrown";
            String FILE1_NAME = "test.docx";
            ...
            success =
                    ftpOne.appendFile(
                            FILE1_NAME,
                            new ByteArrayInputStream(FILE3_CONTENT_3.getBytes("UTF-8")));
    ...
    public void testModificationTime() throws Exception {
        ...
        try {
            ...
            // Feature of the commons ftp library ExtractedTime has a "status code" first and is
            // followed by newline chars

            assertTrue(
                    "time not set correctly by explicit set time",
                    extractedTime.contains(olympicTime));
            ...
            assertFalse(
                    "time not moved on if time not explicitly set",
                    extractedTime.contains(olympicTime));
    ...
    /**
     * Test for Passive Mode -> FTPCommand.Pasv command with external address functionality. see
     * MNT-16433
     */
    public void testFTPConnectExternalAddressSet() throws Exception {
        ...
        try {
            ...
            try {
                ...
                assertTrue(
                        "Client should be in passive mode now",
                        ftp.getDataConnectionMode()
                                == FTPClient.PASSIVE_REMOTE_DATA_CONNECTION_MODE);
                ...
                assertTrue(
                        "Pasv command should contain the set external address encoded",
                        replyLine.contains(encodedImprobableIPAddress));
                ...
                assertTrue(
                        "The passive host should be the one set earlier.",
                        improbableIPAddress.equals(ftp.getPassiveHost()));
            } finally {
                safeDisconnect(ftp);
            }
        } finally {
     ...
     *
     * <p>Upload a file less than the quota.
     *
     * <p>Upload a file greater than the quota.
     *
     * @throws Exception
     */

...
public class Repository implements ApplicationContextAware {
    ...
    /**
     * Gets the currently fully authenticated person, Excludes any overlay authentication set by
     * runas
     *
     * @return person node ref
     */
     ...
     *
     * <p>1) Node - {store_type}/{store_id}/{node_id} <br>
    ...
    public NodeRef findNodeRef(String referenceType, String[] reference) {
        ...
        if (reference.length < 3) {
            throw new AlfrescoRuntimeException(
                    "Reference " + Arrays.toString(reference) + " is not properly formed");
        }
        ...
        if (nodeService.exists(storeRef)) {
            if (referenceType.equals("node")) {
                ...
                if (reference.length == 3) {
                    if (reference[2].equals(
                            nodeService
                                    .getPrimaryParent(rootNodeRef)
                                    .getQName()
                ...
                throw new AlfrescoRuntimeException(
                        "Web Script Node URL specified an invalid reference style of '"
                                + referenceType
                                + "'");
            }
        }

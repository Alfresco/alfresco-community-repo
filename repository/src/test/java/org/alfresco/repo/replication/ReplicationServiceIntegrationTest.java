...
/**
 * Unit tests for the Replication Service. Handles its own transactions, as in a few cases it needs
...
public class ReplicationServiceIntegrationTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        replicationActionExecutor =
                (ReplicationActionExecutor) ctx.getBean("replicationActionExecutor");
        ...
        scheduledPersistedActionService =
                (ScheduledPersistedActionService) ctx.getBean("scheduledPersistedActionService");
    ...
    protected void tearDown() throws Exception {
        RetryingTransactionCallback<Void> cleanupCallback =
                new RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        // Zap the destination folder, which may well contain entries transfered
                        // over which are locked
        ...
        RetryingTransactionCallback<Void> cleanupTargetCallback =
                new RetryingTransactionCallback<Void>() {
                    @Override
                    public Void execute() throws Throwable {
        ...
        try {
            transactionService
                    ...
                    .doInTransaction(cleanupTargetCallback);
    ...
    public void testCreateSaveLoad() throws Exception {
        ...
        ReplicationDefinition retrieved = replicationService.loadReplicationDefinition(ACTION_NAME);
        ...
        ReplicationDefinition second = replicationService.loadReplicationDefinition(ACTION_NAME);
    ...
    public void testLoadList() throws Exception {
        ...
        // Create and store
        ReplicationDefinition rd1 =
                replicationService.createReplicationDefinition(ACTION_NAME, "Test 1");
        ReplicationDefinition rd2 =
                replicationService.createReplicationDefinition(ACTION_NAME2, "Test 2");
    ...
    public void testLoadByTarget() throws Exception {
        ...
        // Store some
        ReplicationDefinition rdTT =
                replicationService.createReplicationDefinition(ACTION_NAME, "Test");
    ...
    public void testDeletion() throws Exception {
        ...
        // Create and save two
        ReplicationDefinition rd2 =
                replicationService.createReplicationDefinition(ACTION_NAME2, "Test2");
        replicationService.saveReplicationDefinition(rd1);
        replicationService.saveReplicationDefinition(rd2);
        assertEquals(2, replicationService.loadReplicationDefinitions().size());
    ...
    /**
     * Test that the action service can find the executor for us, and that it has everything it
     * needs
     */
    public void testBasicExecution() throws Exception {
        ...
        ReplicationDefinition rd2 =
                replicationService.createReplicationDefinition(ACTION_NAME2, "Test");
    ...
    /**
     * Check that the locking works. Take a 10 second lock on the job, then execute. Ensure that we
     * really wait a little over 10 seconds.
     */
    ...
    /**
     * Check that cancelling works. Does this by taking a lock on the job, cancelling, releasing and
     * seeing it abort.
     *
     * <p>Tests that when we ask for a replication task to be cancelled, that it starts, cancels,
     * and the status is correctly recorded for it.
     */
    ...
    public void testExecutionResult() throws Exception {
        ...
        String folder2Name =
                (String) nodeService.getProperties(folder2).get(ContentModel.PROP_NAME);
        ...
        if (nodeService
                .getProperty(folder1, ContentModel.PROP_NAME)
    ...
    /**
     * Test that we turn a list of payload node starting points into the correct set of nodes to
     * pass to the transfer service.
     */
    ...
    public void testScheduling() throws Exception {
                ...
                .doInTransaction(
                        new RetryingTransactionCallback<Void>() {
                            public Void execute() throws Throwable {
                                ...
                                replicationService.deleteReplicationDefinition(
                                        replicationDefinition);
                                assertEquals(
                                        count,
                                        scheduledPersistedActionService.listSchedules().size());
                ...
                .doInTransaction(
                        new RetryingTransactionCallback<Void>() {
                            public Void execute() throws Throwable {
                                ...
                                assertEquals(
                                        ActionStatus.New,
                                        replicationDefinition.getExecutionStatus());
                                return null;
                            }
                        },
                        false,
        ...
        for (int i = 0; i < 150; i++) {
            rd = replicationService.loadReplicationDefinition(ACTION_NAME);
            if (rd.getExecutionStatus().equals(ActionStatus.Failed)) break;
            if (rd.getExecutionStatus().equals(ActionStatus.Completed)) break;
            Thread.sleep(10);
        }
    ...
    public void testJavascriptAPI() throws Exception {
        ...
        ReplicationDefinition empty =
                replicationService.createReplicationDefinition(ACTION_NAME, "Empty");

        ReplicationDefinition persisted =
                replicationService.createReplicationDefinition(ACTION_NAME2, "Persisted");
        ...
        ReplicationDefinition persisted2 =
                replicationService.createReplicationDefinition(ACTION_NAME3, "Persisted2");
        ...
        model.put(
                "Empty",
                new ScriptReplicationDefinition(serviceRegistry, replicationService, null, empty));
        model.put("EmptyName", ACTION_NAME);
        model.put(
                "Persisted",
                new ScriptReplicationDefinition(
                        serviceRegistry, replicationService, null, persisted));
        ...
        model.put(
                "Persisted2",
                new ScriptReplicationDefinition(
                        serviceRegistry, replicationService, null, persisted2));
        ...
        ScriptLocation location =
                new ClasspathScriptLocation(
                        "org/alfresco/repo/replication/script/test_replicationService.js");
    ...
    private NodeRef makeNode(NodeRef parent, QName nodeType, String name) {
        ...
        ChildAssociationRef assoc =
                nodeService.createNode(
                        parent, ContentModel.ASSOC_CONTAINS, newName, nodeType, props);
    ...
    private void makeTransferServiceLocal() {
        ...
        TransferManifestNodeFactory transferManifestNodeFactory =
                (TransferManifestNodeFactory) ctx.getBean("transferManifestNodeFactory");
        TransferServiceImpl2 transferServiceImpl =
                (TransferServiceImpl2) ctx.getBean("transferService2");

...
public class CreateDownloadArchiveAction extends ActionExecuterAbstractBase {
  ...
  private static final Logger log = LoggerFactory.getLogger(
    CreateDownloadArchiveAction.class
  );
  ...
  // Dependency setters
  public void setCheckOutCheckInSerivce(
    CheckOutCheckInService checkOutCheckInService
  ) {
    this.checkOutCheckInService = checkOutCheckInService;
  }

  public void setContentServiceHelper(
  ...
  protected void executeImpl(Action action, final NodeRef actionedUponNodeRef) {
    ...
    final DownloadRequest downloadRequest = downloadStorage.getDownloadRequest(
      actionedUponNodeRef
    );
    ...
    AuthenticationUtil.runAs(
      new RunAsWork<Object>() {
        ...
        public Object doWork() throws Exception {
          ...
          Location exportFrom = new Location(
            downloadRequest.getRequetedNodeRefs()
          );
          ...
          if (
            maximumContentSize > 0 && estimator.getSize() > maximumContentSize
          ) {
  ...
  private void maximumContentSizeExceeded(
    ...
    log.debug(
      "Maximum contentent size ({}), exceeded ({})",
      maximumContentSize,
      size
    );
  ...
  private void createDownload(
    ...
    final File tempFile = TempFileProvider.createTempFile(
      TEMP_FILE_PREFIX,
      TEMP_FILE_SUFFIX
    );
    final ZipDownloadExporter handler = new ZipDownloadExporter(
      tempFile,
      checkOutCheckInService,
      nodeService,
      transactionHelper,

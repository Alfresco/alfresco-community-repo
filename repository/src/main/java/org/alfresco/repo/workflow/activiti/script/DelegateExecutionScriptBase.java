...
public class DelegateExecutionScriptBase extends ActivitiScriptBase {
  ...
  protected void runScript(DelegateExecution execution) throws Exception {
    if (script != null) {
      ...
      // owner us used
      boolean clearAuthenticationContext = checkFullyAuthenticatedUser(
        execution
      );
      ...
      try {
        Object scriptOutput = executeScript(
          scriptString,
          scriptModel,
          scriptProcessorName,
          runAsUser
  ...
  protected Map<String, Object> getInputMap(
    ...
    if (personNode != null) {
      ...
      NodeRef userHomeNode = (NodeRef) registry
        .getNodeService()
        .getProperty(personNode.getNodeRef(), ContentModel.PROP_HOMEFOLDER);
      if (userHomeNode != null) {
        scriptModel.put(
          USERHOME_BINDING_NAME,
          new ActivitiScriptNode(userHomeNode, registry)
        );
      }
    }
   ...
   */
  private boolean checkFullyAuthenticatedUser(
    final DelegateExecution execution
  ) {
    if (AuthenticationUtil.getFullyAuthenticatedUser() == null) {
      NamespaceService namespaceService = getServiceRegistry()
        .getNamespaceService();
      WorkflowQNameConverter qNameConverter = new WorkflowQNameConverter(
        namespaceService
      );
      String ownerVariableName = qNameConverter.mapQNameToName(

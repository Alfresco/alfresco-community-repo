...
public abstract class AbstractServerConfigurationBean
  ...
  implements
    ExtendedServerConfigurationAccessor,
    ApplicationListener,
  ...
  public void init() {
    ...
    if (m_authenticationComponent == null) {
      ...
      throw new AlfrescoRuntimeException(
        "Property 'authenticationService' not set"
      );
      ...
      throw new AlfrescoRuntimeException(
        "Property 'transactionService' not set"
      );
    ...
    try {
      debugConfig.setDebug(
        "org.alfresco.filesys.debug.FileServerDebugInterface",
        new GenericConfigElement("params")
      );
    ...
    try {
      ...
      logger.error(
        "File server configuration error (WINS), " + ex.getMessage(),
        ex
      );
    }
    ...
    try {
      ...
      // Configuration error
      throw new AlfrescoRuntimeException(
        "File server configuration error, " + ex.getMessage(),
        ex
      );
    }
    ...
    try {
      ...
      logger.info(
        "FTP server " + (isFTPServerEnabled() ? "" : "NOT ") + "started"
      );
    } catch (Exception ex) {
      // Configuration error
  ...
  private final void determinePlatformType() {
    if (m_platform == Platform.Type.Unchecked) m_platform =
   ...
   */
  protected final EnumSet<Platform.Type> parsePlatformString(
    String platformStr
  ) {
    ...
    StringTokenizer token = new StringTokenizer(
      platformStr.toUpperCase(Locale.ENGLISH),
      ","
    );
    String typ = null;
    ...
    try {
      while (token.hasMoreTokens()) {
        ...
        if (platform != Platform.Type.Unknown) platformTypes.add(
          platform
        ); else throw new AlfrescoRuntimeException(
          "Invalid platform type, " + typ
        );
      }
  ...
  public final String getLocalServerName(boolean trimDomain) {
    ...
    if (m_localName != null) return m_localName;
    ...
    if (trimDomain && srvName != null) {
      ...
      if (pos != -1) srvName = srvName.substring(0, pos);
    }
  ...
  private String getLocalServerName() {
    ...
    if (m_localNameFull != null) return m_localNameFull;
  ...
  public final String getLocalDomainName() {
    ...
    if (m_localDomain != null) return m_localDomain;
    ...
    if (getPlatformType() == Platform.Type.WINDOWS && !isNativeCodeDisabled()) {
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "Local domain name is " + domainName + " (via JNI)"
      );
    } else {
      ...
      try {
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "Found browse master at " + nbName.getIPAddressString(0)
        );
  ...
  protected final InetAddress parseAdapterName(String adapter)
    ...
    try {
      ...
      throw new InvalidConfigurationException(
        "Invalid adapter name, " + adapter
      );
    }

    if (ni == null) throw new InvalidConfigurationException(
    ...
    while (addrEnum.hasMoreElements() && adapAddr == null) {
      ...
      if (IPAddress.isNumericAddress(addr.getHostAddress())) adapAddr = addr;
    }

    // Check if we found the IP address to bind to

    if (adapAddr == null) throw new InvalidConfigurationException(
      "Adapter " + adapter + " does not have a valid IP address"
    );

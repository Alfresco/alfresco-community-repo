...
public class RFC822MetadataExtracterTest extends AbstractMetadataExtracterTest {
  ...
  private NodeRef nodeRefWithDodRecord = new NodeRef(
    "workspace://spacesStore/test-dod"
  );
  private NodeRef nodeRefWithRecord = new NodeRef(
    "workspace://spacesStore/test-rm"
  );
  private NodeRef nodeRefWithBoth = new NodeRef(
    "workspace://spacesStore/test-both"
  );
  private NodeRef nodeRefWithNeither = new NodeRef(
    "workspace://spacesStore/test-neither"
  );

  private static final QName MESSAGE_FROM_TEST_PROPERTY = QName.createQName(
    "MessageToTest"
  );
  private static final QName MESSAGE_TO_TEST_PROPERTY = QName.createQName(
    "MessageFromTest"
  );
  private static final QName MESSAGE_CC_TEST_PROPERTY = QName.createQName(
    "MessageCCTest"
  );
  ...
  public void setUp() throws Exception {
    ...
    when(
      mockNodeService.hasAspect(nodeRefWithDodRecord, ASPECT_DOD_5015_RECORD)
    )
      .thenReturn(true);
    when(mockNodeService.hasAspect(nodeRefWithRecord, ASPECT_RECORD))
      .thenReturn(true);
    when(mockNodeService.hasAspect(nodeRefWithBoth, ASPECT_DOD_5015_RECORD))
      .thenReturn(true);
    when(mockNodeService.hasAspect(nodeRefWithBoth, ASPECT_RECORD))
      .thenReturn(true);
  ...
  @Override
  protected void testFileSpecificMetadata(
    String mimetype,
  ...
  public void testMatch() {
    assertFalse(
      "Normal class should never match",
      extracter.match(MimetypeMap.MIMETYPE_RFC822)
    );
    assertTrue(
  ...
  private int countSystemProperties(NodeRef nodeRef) {
    Map<String, Set<String>> extractMapping = rmExtracter.getExtractMapping(
      nodeRef
    );

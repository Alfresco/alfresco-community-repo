 ...
 *
 * <p>NOTE: this has been added to resolve a specific issue and needs re-consideration
 *
...
public class ScriptCommentService extends BaseScopableProcessorExtension {
    ...
    public ScriptNode createCommentsFolder(ScriptNode node) {
        ...
        if (permissionService.hasPermission(nodeRef, PermissionService.ADD_CHILDREN)
                == AccessStatus.DENIED) {
        ...
        NodeRef commentsFolder =
                AuthenticationUtil.runAs(
                        new AuthenticationUtil.RunAsWork<NodeRef>() {
                            public NodeRef doWork() throws Exception {
                                NodeRef commentsFolder = null;

                                // ALF-5240: turn off auditing round the discussion node creation to
                                // prevent
                                // the source document from being modified by the first user leaving
                                // a comment

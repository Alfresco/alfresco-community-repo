...
public class RemoteTransformerClient {
  ...
  public RemoteTransformerClient(String name, String baseUrl) {
    this.name = name;
    this.baseUrl =
      baseUrl == null || baseUrl.trim().isEmpty() ? null : baseUrl.trim();
  }
  ...
  public void request(
    ...
    if (args.length % 2 != 0) {
      throw new IllegalArgumentException(
        "There should be a value for each request property"
      );
    }

    StringJoiner sj = new StringJoiner(" ");
    HttpEntity reqEntity = getRequestEntity(
      reader,
      sourceMimetype,
      sourceExtension,
      targetExtension,
    ...
    try {
      try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
        try (CloseableHttpResponse response = execute(httpclient, httppost)) {
          ...
          if (resEntity != null) {
            ...
            if (statusCode == 200) {
              try {
                ...
                throw new AlfrescoRuntimeException(
                  name + " failed to read the returned content",
                  e
                );
              }
            } else {
          ...
          connectionFailed();
          throw new AlfrescoRuntimeException(
            name + " failed to connect or to read the response",
            e
          );
        }
      } catch (IOException e) {
        throw new AlfrescoRuntimeException(
          name + " failed to create an HttpClient",
          e
        );
      }
    ...
    try {
      try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
        try (CloseableHttpResponse response = execute(httpclient, httpGet)) {
          ...
          if (resEntity != null) {
            ...
            if (statusCode == 200) {
              try {
                ...
                throw new AlfrescoRuntimeException(
                  name + " check failed to read the returned content",
                  e
                );
              }
            } else {
          ...
          throw new AlfrescoRuntimeException(
            name + " check failed to connect or to read the response",
            e
          );
        }
      } catch (IOException e) {
        throw new AlfrescoRuntimeException(
          name + " check failed to create an HttpClient",
          e
        );
      }
    } catch (AlfrescoRuntimeException e) {

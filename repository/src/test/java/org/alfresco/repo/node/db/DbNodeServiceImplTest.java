...
public class DbNodeServiceImplTest extends BaseNodeServiceTest {
  ...
  protected NodeService getNodeService() {
    // Force cascading
    DbNodeServiceImpl dbNodeServiceImpl = (DbNodeServiceImpl) applicationContext.getBean(
      "dbNodeServiceImpl"
    );
  ...
  public void testNodeCleanupRegistry() throws Exception {
    ...
    NodeCleanupRegistry cleanupRegistry = (NodeCleanupRegistry) applicationContext.getBean(
      "nodeCleanupRegistry"
    );
  ...
  public synchronized void testTxnCommitTime() throws Exception {
    ...
    assertEquals(
      "Should not have found a written txn",
      0L,
  ...
  private class TestTxnCommitTimeTxnListener
    ...
    public long getTxnCommitTime(String txnId, long fromTime) {
      List<Transaction> startTxns =
        ((NodeDAOImpl) nodeDAO).selectTxns(
            fromTime,
            null,
            Integer.MAX_VALUE,
            null,
      ...
      public Object execute() {
        ...
        if (!n6Status.isDeleted()) {
          throw new RuntimeException(
            "Deleted node does not have deleted status"
          );
        }
  ...
  private void executeAndCheck(
    ...
    try {
      ...
      // check
      assertEquals(
        "Change didn't update status",
        currentTxnId,
        newStatus.getChangeTxnId()
      );
      ...
      nodeDAO.getParentAssocs(
        ...
        null,
        null,
        new DummyChildAssocRefQueryCallback()
      );
  ...
  public void testMLTextValues() throws Exception {
    ...
    Map<QName, Serializable> propertiesDirect = nodeService.getProperties(
      rootNodeRef
    );
    assertEquals(
  ...
  public void testStringIntoMLTextProperty() throws Exception {
    ...
    Serializable mlTextCheck = nodeService.getProperty(
      rootNodeRef,
    ...
    assertEquals(
      "Default MLText value was not set correctly",
      text,
      mlTextDefaultCheck
    );
    ...
    Serializable nullValueCheck = nodeService.getProperty(
      rootNodeRef,
      PROP_QNAME_ML_TEXT_VALUE
    );
  ...
  public void testSingleStringMLTextProperty() throws Exception {
    ...
    nodeService.setProperty(rootNodeRef, PROP_QNAME_ML_TEXT_VALUE, mlText);
    // Get it back and check
    MLText mlTextCheck = (MLText) nodeService.getProperty(
      rootNodeRef,
      PROP_QNAME_ML_TEXT_VALUE
    );
    assertEquals("Setting of MLText over String failed.", mlText, mlTextCheck);
  ...
  public void testAspectRemovalWithCommit() throws Throwable {
    ...
    try {
      ...
      assertEquals(
        "Expected exactly one less aspect",
        noAspectsBefore - 1,
        aspects.size()
      );
  ...
  public void testGetChildAssocsByPropertyValue() throws Exception {
    ...
    ChildAssociationRef rootRef = assocRefs.get(
      QName.createQName(BaseNodeServiceTest.NAMESPACE, "root")
    );
    ChildAssociationRef n1Ref = assocRefs.get(
      QName.createQName(BaseNodeServiceTest.NAMESPACE, "root_p_n1")
    );
    ChildAssociationRef n2Ref = assocRefs.get(
      QName.createQName(BaseNodeServiceTest.NAMESPACE, "root_p_n2")
    );
     ...
     */
    {
      ...
      List<ChildAssociationRef> refs = nodeService.getChildAssocsByPropertyValue(
        parentNodeRef,
        ContentModel.PROP_SUBJECT,
        subject
      );
  ...
  public void testMySQLInnoDBNodeStringLengthWorker() throws Exception {
    ...
    SchemaBootstrap schemaBootstrap = (SchemaBootstrap) applicationContext.getBean(
      "schemaBootstrap"
    );
    assertEquals(
    ...
    NodeStringLengthWorker worker = (NodeStringLengthWorker) applicationContext.getBean(
    ...
    schemaBootstrap.setMaximumStringLength(
      SchemaBootstrap.DEFAULT_MAX_STRING_LENGTH
    );
    schemaBootstrap.onApplicationEvent(
    ...
    String checkLongString = txnService
      .getRetryingTransactionHelper()
      .doInTransaction(
            ...
            AuthenticationUtil.runAs(
              new RunAsWork<Void>() {
                ...
                public Void doWork() throws Exception {
                  ...
                  fail(
                    "Should have generated a DuplicateChildNodeNameException"
                  );

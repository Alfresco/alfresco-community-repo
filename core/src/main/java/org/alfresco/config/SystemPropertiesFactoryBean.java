...
/**
 * Like the parent <code>PropertiesFactoryBean</code>, but overrides or augments the resulting
 * property set with values from VM system properties. As with the Spring {@link
 * PropertyPlaceholderConfigurer} the following modes are supported:
 *
 * <ul>
 *   <li><b>SYSTEM_PROPERTIES_MODE_NEVER: </b>Don't use system properties at all.
 *   <li><b>SYSTEM_PROPERTIES_MODE_FALLBACK: </b>Fallback to a system property only for undefined
 *       properties.
 *   <li><b>SYSTEM_PROPERTIES_MODE_OVERRIDE: (DEFAULT)</b>Use a system property if it is available.
 * </ul>
 *
 * Note that system properties will only be included in the property set if defaults for the
 * property have already been defined using {@link #setProperties(Properties)} or {@link
 * #setLocations(org.springframework.core.io.Resource[])} or their names have been included
 * explicitly in the set passed to {@link #setSystemProperties(Set)}.
 *
 * @author Derek Hulley
 */
public class SystemPropertiesFactoryBean extends PropertiesFactoryBean {
    ...
    public void setSystemPropertiesModeName(String constantName) throws IllegalArgumentException {
        this.systemPropertiesMode =
                SystemPropertiesFactoryBean.constants.asNumber(constantName).intValue();
    }
     ...
     *
     * @param systemProperties a set of properties that can be fetched from the system properties
     */
    ...
    protected Properties mergeProperties() throws IOException {
        ...
        if (this.systemPropertiesMode
                == PropertyPlaceholderConfigurer.SYSTEM_PROPERTIES_MODE_NEVER) {
            ...
            // Otherwise, we allow unset properties to drift through from the systemProperties set
            // and potentially set
    ...
    /**
     * Override hook. Allows subclasses to resolve a merged property from an alternative source,
     * whilst still respecting the chosen system property fallback path.
     *

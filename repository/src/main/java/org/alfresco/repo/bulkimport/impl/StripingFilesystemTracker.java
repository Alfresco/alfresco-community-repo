...
public class StripingFilesystemTracker extends AbstractFilesystemTracker {
  ...
  private Map<Integer, List<ImportableItem>> directoriesToProcess = new HashMap<Integer, List<ImportableItem>>(
    10
  );
  ...
  protected void addDirectoriesToProcess(
    Collection<ImportableItem> dirsToAdd,
    int level
  ) {
  ...
  protected List<ImportableItem> getImportableItems(int count) {
    while (toProcess.size() < count) {
      ...
      if (directory != null) {
        AnalysedDirectory analysedDirectory = getImportableItemsInDirectory(
          directory
        );

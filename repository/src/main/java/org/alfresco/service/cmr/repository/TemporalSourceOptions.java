...
/**
 * Time-based content conversion options to specify an offset and duration. Useful for audio and
 * video.
 *
 * <p>If only the offset is specified transformers should attempt a transform from that offset to
 * the end if possible.
 *
 * <p>If only a duration is specified transformers should attempt a transform from the start until
 * that duration is reached if possible.
 *
 * @author Ray Gauss II
 * @deprecated The transformations code is being moved out of the codebase and replaced by the new
...
public class TemporalSourceOptions extends AbstractTransformationSourceOptions {
    ...
    private static final String VALID_TIME_STRING_REGEX =
            "\\d{2}:[0-5][0-9]:[0-5][0-9](\\.\\d{1,3})?";
    ...
    public boolean isApplicableForMimetype(String sourceMimetype) {
        return ((sourceMimetype != null && sourceMimetype.startsWith(MimetypeMap.PREFIX_VIDEO)
                        || sourceMimetype.startsWith(MimetypeMap.PREFIX_AUDIO))
                || super.isApplicableForMimetype(sourceMimetype));
    }

    /**
     * Gets the offset time code from which to start the transformation with a format of
     * hh:mm:ss[.xxx]
     *
     * @return the offset
     */
    ...
    /**
     * Sets the offset time code from which to start the transformation with a format of
     * hh:mm:ss[.xxx]
     *
     * @param offset String
     */
    ...
    public static void validateTimeString(String value) {
        if (value != null && !value.matches(VALID_TIME_STRING_REGEX)) {
            throw new AlfrescoRuntimeException(
                    "'" + value + "' is not a valid time specification of the form hh:mm:ss[.xxx]");
        }
    }

    @Override
    public TransformationSourceOptions mergedOptions(
            TransformationSourceOptions overridingOptions) {
        if (overridingOptions instanceof TemporalSourceOptions) {
            TemporalSourceOptions mergedOptions =
                    (TemporalSourceOptions) super.mergedOptions(overridingOptions);
            ...
            if (((TemporalSourceOptions) overridingOptions).getDuration() != null) {
                mergedOptions.setDuration(
                        ((TemporalSourceOptions) overridingOptions).getDuration());
            }
    ...
    public class TemporalSourceOptionsSerializer implements TransformationSourceOptionsSerializer {
        ...
        public TransformationSourceOptions deserialize(
            ...
            String offset =
                    serializedOptions.getCheckedParam(PARAM_SOURCE_TIME_OFFSET, String.class);
            String duration =
                    serializedOptions.getCheckedParam(PARAM_SOURCE_TIME_DURATION, String.class);
        ...
        public void serialize(
                TransformationSourceOptions sourceOptions, Map<String, Serializable> parameters) {
            if (parameters == null || sourceOptions == null) return;

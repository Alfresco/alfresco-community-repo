...
public class TemporaryNodesTest {
  ...
  private static final String SITE_SHORT_NAME =
    "siteShortName" +
    TemporaryNodesTest.class.getSimpleName() +
  ...
  public static void initStaticData() throws Exception {
    COCI_SERVICE =
      APP_CONTEXT_INIT
        ...
        .getBean("checkOutCheckInService", CheckOutCheckInService.class);
    CONTENT_SERVICE =
      APP_CONTEXT_INIT
        .getApplicationContext()
    ...
    VERSION_SERVICE =
      APP_CONTEXT_INIT
        .getApplicationContext()
        .getBean("VersionService", VersionService.class);

    Repository repositoryHelper = APP_CONTEXT_INIT
      .getApplicationContext()
      .getBean("repositoryHelper", Repository.class);
      ...
      new RetryingTransactionCallback<Void>() {
        public Void execute() throws Throwable {
          assertTrue(
            ...
            NODE_SERVICE.exists(testNode1)
          );
          assertTrue(
            "Test node does not exist",
            NODE_SERVICE.exists(testNode2)
          );

          Map<QName, Serializable> node1Props = NODE_SERVICE.getProperties(
            testNode1
          );
          Map<QName, Serializable> node2Props = NODE_SERVICE.getProperties(
            testNode2
          );
          ...
          assertEquals(
            "cm:name was wrong",
            "doc 1",
            node1Props.get(ContentModel.PROP_NAME)
          );
          assertEquals(
            "cm:name was wrong",
            "doc 2",
            node2Props.get(ContentModel.PROP_NAME)
          );

          // creator
          assertEquals(
            "cm:creator was wrong",
            TEST_USER1.getUsername(),
            node1Props.get(ContentModel.PROP_CREATOR)
          );
          assertEquals(
            "cm:creator was wrong",
            TEST_USER2.getUsername(),
            node2Props.get(ContentModel.PROP_CREATOR)
          );

          // content
          ContentReader reader = CONTENT_SERVICE.getReader(
            testNode1,
          ...
          reader =
            CONTENT_SERVICE.getReader(testNode2, ContentModel.PROP_CONTENT);
          assertEquals(
            "Content was wrong",
            "Hello world",
            reader.getContentString("Hello world".length())
          );
  ...
  public void ensureCheckedOutNodesAreCleanedUp() throws Throwable {
    ...
    nodesThatShouldBeDeletedByRule.add(
      myTemporaryNodes.createNode(
        COMPANY_HOME,
        "normal node",
        ContentModel.TYPE_CONTENT,
        TEST_USER1.getUsername()
    ...
    final NodeRef checkedoutNode = myTemporaryNodes.createNode(
      COMPANY_HOME,
      "checkedout node",
      ContentModel.TYPE_CONTENT,
      TEST_USER1.getUsername()
        ...
        public Void execute() throws Throwable {
          for (NodeRef node : nodesThatShouldBeDeletedByRule) {
            if (NODE_SERVICE.exists(node)) {
              fail(
                "Node '" +
                NODE_SERVICE.getProperty(node, ContentModel.PROP_NAME) +
                "' still exists."
              );
            }
          }
  ...
  public void ensureVersionedNodesAreFullyCleanedUp() throws Throwable {
    ...
    // Therefore we have to manually poke the Rule to get it to cleanup during test execution.
    // NOTE! This is *not* how a JUnit Rule would normally be used.
    final TemporaryNodes myTemporaryNodes = new TemporaryNodes(
      APP_CONTEXT_INIT
    );
    ...
    final NodeRef versionedTempNode = myTemporaryNodes.createQuickFile(
    ...
    TRANSACTION_HELPER.doInTransaction(
      new RetryingTransactionCallback<Void>() {
        public Void execute() throws Throwable {
          assertTrue(
            "The test node was not versioned as it should be.",
            VERSION_SERVICE.isVersioned(versionedTempNode)
          );
  ...
  public void ensureSiteNodesAreCleanedUp() throws Throwable {
    ...
    SiteInfo createdSite = TRANSACTION_HELPER.doInTransaction(
      new RetryingTransactionCallback<SiteInfo>() {
        public SiteInfo execute() throws Throwable {
          return SITE_SERVICE.createSite(
            "sitePreset",
            SITE_SHORT_NAME,
            "site title",
            "site description",
      ...
      new RetryingTransactionCallback<Void>() {
        public Void execute() throws Throwable {
          if (NODE_SERVICE.exists(siteNodeRef)) {
            fail(
              "Node '" +
              NODE_SERVICE.getProperty(siteNodeRef, ContentModel.PROP_NAME) +
              "' still exists."
            );
          }
          return null;
  ...
  public void testCreateFolderAndQuickFiles() throws Throwable {
    ...
    final NodeRef folder = myTemporaryNodes.createFolder(
      COMPANY_HOME,
      "testFolder",
      AuthenticationUtil.getAdminUserName()
    );
    ...
    final NodeRef quickTxt = myTemporaryNodes.createQuickFile(
      MimetypeMap.MIMETYPE_TEXT_PLAIN,
      folder,
      "quickFile",
      AuthenticationUtil.getAdminUserName()
    ...
    final NodeRef namedQuickTxt = myTemporaryNodes.createQuickFileByName(
      "quickCorrupt.jpg",
      folder,
      AuthenticationUtil.getAdminUserName()
    );
    ...
    TRANSACTION_HELPER.doInTransaction(
      new RetryingTransactionCallback<Void>() {
        public Void execute() throws Throwable {
          ...
          assertEquals(
            ContentModel.TYPE_CONTENT,
            NODE_SERVICE.getType(quickTxt)
          );
          assertEquals(
            ContentModel.TYPE_CONTENT,
            NODE_SERVICE.getType(namedQuickTxt)
          );

          assertEquals(
            AuthenticationUtil.getAdminUserName(),
            NODE_SERVICE.getProperty(folder, ContentModel.PROP_CREATOR)
          );
          assertEquals(
            AuthenticationUtil.getAdminUserName(),
            NODE_SERVICE.getProperty(quickTxt, ContentModel.PROP_CREATOR)
          );
          assertEquals(
            AuthenticationUtil.getAdminUserName(),
            NODE_SERVICE.getProperty(namedQuickTxt, ContentModel.PROP_CREATOR)
          );
          ...
          ContentReader reader2 = CONTENT_SERVICE.getReader(
            namedQuickTxt,
            ContentModel.PROP_CONTENT
          );

...
public class ModuleManagementToolTest extends TestCase {
  ...
  private void installerSharedTests(
    String warLocation,
    String ampLocation,
    String ampV2Location
  ) {
    ...
    files2.add(
      "/WEB-INF/classes/alfresco/module/test/version2/version2-context.xml"
    );
  ...
  public void testDependencySuccess() throws Exception {
    ...
    String testAmpDepV1Location = getFileLocation(
      ".amp",
      "module/dependent_on_test_v1.amp"
    );
    String testAmpDepV2Location = getFileLocation(
      ".amp",
      "module/dependent_on_test_v2.amp"
    );

    // Install V1
    this.manager.installModule(
        testAmpV1Location,
        warLocation,
        false,
        false,
  ...
  public void testForcedInstall() throws Exception {
    ...
    try {
      ...
      assertTrue(
        e.getMessage().contains("The amp will overwrite an existing file")
      );
    }
  ...
  public void testExistingFilesInWar() throws Exception {
    ...
    try {
      ...
      assertTrue(
        exception
          .getMessage()
          .contains("will overwrite an existing file in the war")
      );
    }
  ...
  private String extractToDir(String extension, String location) {
    ...
    try {
      TFile zipFile = new TFile(
        this.getClass().getClassLoader().getResource(location).getPath()
      );
      TFile outDir = new TFile(
        tmpDir.getAbsolutePath() +
        "/moduleManagementToolTestDir" +
  ...
  private void checkForFileExistance(String warLocation, List<String> files) {
    for (String file : files) {
      ...
      assertTrue(
        "The file/dir " + file + " does not exist in the WAR.",
        file0.exists()
      );
    }
  }

  private void checkForFileNonExistance(
    String warLocation,
    List<String> files
  ) {
    for (String file : files) {
      File file0 = new TFile(warLocation + file);
      assertFalse(
        "The file/dir " + file + " does exist in the WAR.",
        file0.exists()
      );
    }
  }
  ...
  private void checkContentsOfFile(String location, String expectedContents)
    ...
    try {
      reader =
        new BufferedReader(new InputStreamReader(new TFileInputStream(file)));

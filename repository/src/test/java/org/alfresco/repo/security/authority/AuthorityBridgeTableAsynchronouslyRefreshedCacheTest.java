...
public class AuthorityBridgeTableAsynchronouslyRefreshedCacheTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        authorityService =
                (AuthorityService) ctx.getBean(ServiceRegistry.AUTHORITY_SERVICE.getLocalName());
    ...
    public void testAuthorityBridgeTableCacheForTenants() throws Exception {
                        ...
                        .doInTransaction(
                                new RetryingTransactionHelper.RetryingTransactionCallback<
                                    ...
                                    public String execute() throws Throwable {
                                        ...
                                        String tenantChildGroup =
                                                authorityService.createAuthority(
                                                        AuthorityType.GROUP, childGroupName);
                                        assertNotNull(
                                                authorityService.getAuthorityNodeRef(
                                                        tenantChildGroup));
                                        authorityService.addAuthority(
                                                tenantChildGroup, tenantPersonName);
                                        authorityBridgeTableCache
        ...
        // The transaction is required, because the AuthorityBridgeTableAsynchronouslyRefreshedCache
        // is cleared in the end of transaction asynchronously using FutureTask.
        final String tenantParentGroup =
                transactionService
                        .getRetryingTransactionHelper()
                        .doInTransaction(
                                new RetryingTransactionHelper.RetryingTransactionCallback<
                                    ...
                                    public String execute() throws Throwable {
                                        String tenantParentGroup =
                                                authorityService.createAuthority(
                                                        AuthorityType.GROUP, parentGroupName);
                                        assertNotNull(
                                                authorityService.getAuthorityNodeRef(
                                                        tenantParentGroup));
                                        authorityService.addAuthority(
                                                tenantParentGroup, tenantChildGroup);
                                        authorityBridgeTableCache
                                                .forceInChangesForThisUncommittedTransaction();
        ...
        Set<String> authorities =
                authorityService.getContainingAuthorities(null, tenantPersonName, false);
                            ...
                            public Void execute() throws Throwable {
                                if (!tenantAdminService.existsTenant(tenantDomain)) {
                                    tenantAdminService.createTenant(
                                            tenantDomain, "password".toCharArray());
                                } else {
    ...
    public void testCyclicGroups() {
        ...
        AuthorityBridgeTableAsynchronouslyRefreshedCache cache =
                new AuthorityBridgeTableAsynchronouslyRefreshedCache();

...
/**
 * This Heartbeat job collects data and passes it to the {@link HBDataSenderService}. Collectors
 * using this job will collect data in one cluster node per execution, this is achieved by a job
 * lock on collector id. The job lock is obtained by the first repository node which requests it for
 * its collector id.
 */
public class LockingJob implements Job {
    ...
    public void execute(final JobExecutionContext jobexecutioncontext)
        ...
        QName lockQname =
                QName.createQName(
                        NamespaceService.SYSTEM_MODEL_1_0_URI, collector.getCollectorId());
        ...
        try {
            ...
            // instead the job lock service will check the callback (running) flag every LOCK_TTL/2
            // ms from lock acquisition
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Skipping collect and send data (could not get lock): " + e.getMessage());
            }
        } finally {
            if (logger.isDebugEnabled()) {
    ...
    private void collectAndSendDataLocked(
            final HBBaseDataCollector collector, final HBDataSenderService hbDataSenderService)
            throws JobExecutionException {
        try {

...
/**
 * Provides methods specific to manipulating {@link org.alfresco.model.ContentModel#TYPE_CONTENT
 * files} and {@link org.alfresco.model.ContentModel#TYPE_FOLDER folders}.
 *
 * <p>So this interface provides a simple way of accessing simple trees of files and folders in
 * Alfresco.
 *
...
public interface FileFolderService {
     ...
     *
     * <p>Note: this could be a long list (and will be trimmed at a pre-configured maximum). You
     * should consider using a paging request.
     *
    ...
    /**
     * Lists page of immediate child files and/or folders of the given context node with optional
     * filtering (exclusion of certain child file/folder subtypes) and sorting <br>
     * <br>
     * author janv
     *
     * @since 4.0
    ...
    /**
     * Lists page of immediate child files and/or folders of the given context node with pattern
     * matching and optional filtering (exclusion of certain child file/folder subtypes) and sorting
     *
     * <p>Pattern uses '*' as a wildcard
     *
     * @since 4.0
     */
    @Auditable(
     ...
     *
     * <p>The filter parameter allows subfolders to be excluded from the search.
     *
     * @param contextNodeRef the node to start searching in
    ...
    /**
     * Uses the <b>cm:name</b> of the given node and attempts to find a sibling node with a more
     * specific localized name. The node passed in must represent the base of the possible
     * translations i.e. the base name for the resource names will be calculated using the filename
     * without extension. The locale used will come from {@link I18NUtil#getLocale() the thread's
     * default locale}.
     *
    ...
    /**
     * Searches for all files and folders with the matching name pattern, using wildcard characters
     * <b>*</b> and <b>?</b>.
     *
     * <p>Warning: Please avoid using this method with any "namePattern" other than "*". Although it
     * works, its performance is poor, which is why this method is deprecated.
     *
     * @param contextNodeRef the context of the search. This node will never be returned as part of
     *     the search results.
     * @param namePattern the name of the file or folder to search for, or a {@link
     *     org.alfresco.util.SearchLanguageConversion#DEF_LUCENE wildcard} pattern to search for.
     ...
     * @deprecated for shallow search use list, listFolders, listFiles, searchSimple. For deep
     *     listing use listDeepFolders. Avoid calling this method with any name pattern except for
     *     "*".
     */
    ...
    /**
     * Perform a search against the name of the files or folders within a hierarchy. Wildcard
     * characters are <b>*</b> and <b>?</b>.
     *
     * <p>Warning: Please avoid using this method with any "namePattern" other than "*". Although it
     * works, its performance is poor which is why this method is deprecated.
     *
     ...
     *
     * <p>If both the parent folder and name remain the same, then nothing is done.
     *
     ...
     * <p>If both the parent folder and name remain the same, then nothing is done.
     *
     * <p>It is possible to specify <i>which</i> is the parent node when moving nodes; nodes can
     * reside in multiple locations.
     *
     * @param sourceNodeRef the file or folder to move
     * @param sourceParentRef the source parent of node - <tt>null</tt> means move from primary
     *     parent
     * @param targetParentRef the new parent node to move the node to - null means rename in situ
     * @param newName the name to change the file or folder to - null to keep the existing name
     * @return Returns the new file info
    ...
    /**
     * @deprecated From 3.4.2, use {@link #moveFrom(NodeRef, NodeRef, NodeRef, String)} or {@link
     *     #move(NodeRef, NodeRef, String)}. See <a
     *     href="https://issues.alfresco.com/jira/browse/ALF-7692">ALF-7692</a>
     */
    @Auditable(parameters = {"sourceNodeRef", "sourceParentRef", "targetParentRef", "newName"})
    ...
    /**
     * Copy a source file or folder. The source can be optionally renamed and optionally moved into
     * another folder.
     *
     * <p>If both the parent folder and name remain the same, then nothing is done.
     *
     ...
     *
     * <p>The association QName for the patch defaults to <b>cm:filename</b> i.e. the <b>Content
     * Model</b> namespace with the filename as the local name.
     *
     * @param parentNodeRef the parent node. The parent must be a valid {@link
     *     org.alfresco.model.ContentModel#TYPE_FOLDER folder}.
     ...
     *
     * <ul>
     *   <li>The root node can be of any type and is not included in the path list.
     *   <li>Only the primary path is considered. If the target node is not a descendant of the root
     *       along purely primary associations, then an exception is generated.
     *   <li>If an invalid type is encountered along the path, then an exception is generated.
     * </ul>
     *
     * @param rootNodeRef the start of the returned path, or null if the <b>store</b> root node must
     *     be assumed.
     * @param nodeRef a reference to the file or folder
     * @return Returns a list of file/folder infos from the root (excluded) down to and including
     ...
     *     be assumed.
     * @param nodeRef a reference to the file or folder
     * @return Returns a list of file/folder names from the root (excluded) down to and including
     *     the destination file or folder
     * @throws FileNotFoundException if the node could not be found
     */
    @Auditable(parameters = {"rootNodeRef", "nodeRef"})
     ...
     *
     * @param rootNodeRef the start point node - a cm:folder type or subtype, e.g. the Company
     *     Home's nodeRef
     * @param pathElements a list of names in the path. Do not include the referenced rootNodeRef's
     *     path element.
     ...
     * @param pathElements a list of names in the path. Do not include the referenced rootNodeRef's
     *     path element.
     * @return Returns the info of the file or folder or <code>null</code> if <code>mustExist</code>
     *     is <code>false</code> and the file does not exist
     * @throws FileNotFoundException if no file or folder exists along the path and <code>mustExist
     *     </code> is <code>true</code>
     */
    @Auditable(parameters = {"rootNodeRef", "pathElements", "mustExist"})
    public FileInfo resolveNamePath(
            NodeRef rootNodeRef, List<String> pathElements, boolean mustExist)
            throws FileNotFoundException;
    ...
    /**
     * Get the reader to the file represented by the node according to the File/Folder model. (This
     * is not the same as the method on the ContentService)
     *
    ...
    /**
     * Get the writer to the file represented by the node according to the File/Folder model. (This
     * is not the same as the method on the ContentService)
     *
     * @param nodeRef the content node
     * @return Returns a handle to the content associated with the node
    ...
    /**
     * Checks the type for whether it is a recognised file or folder type or is invalid for the
     * FileFolderService.
     *
    ...
    /**
     * @deprecated <a href="https://issues.alfresco.com/jira/browse/MNT-8704">MNT-8704:
     *     WebDAV:Content does not disappear after being deleted</a>
     */
    @Deprecated
    ...
    /**
     * Lists page of immediate child objects of the given context node with specification of which
     * types to list and optional filtering (exclusion of certain child file/folder subtypes) and
     * sorting
     *

...
public class TestUserPreferences extends EnterpriseTestApi {
    ...
    public void testUserPreferences() throws Exception {
        ...
        expectedPreferences.add(
                new Preference(
                        "org.alfresco.share.documentList.testPreference2", String.valueOf(true)));
        expectedPreferences.add(
                new Preference(
                        "org.alfresco.share.documentList.testPreference1", String.valueOf(true)));
        expectedPreferences.add(
                new Preference(
                        "org.alfresco.share.documentList.sortAscending", String.valueOf(true)));
        expectedPreferences.add(
                new Preference(
                        "org.alfresco.share.documentList.testPreference3", String.valueOf(true)));
                ...
                new TenantRunAsWork<Void>() {
                    ...
                    public Void doWork() throws Exception {
                        for (Preference pref : expectedPreferences) {
                            ...
                            repoService.addPreference(
                                    person1.getId(), pref.getId(), pref.getValue());
                        }
        ...
        try {
            ...
            Paging paging =
                    getPaging(
                            ...
                            expectedPreferences.size(),
                            expectedPreferences.size());
            //	        ListResponse<Preference> resp = peopleProxy.getPreferences(person1.getId(),
            // createParams(paging, null));
            //	        checkList(expectedPreferences.subList(skipCount, skipCount +
            // paging.getExpectedPaging().getCount()), paging.getExpectedPaging(), resp);
            peopleProxy.getPreferences(person1.getId(), createParams(paging, null));
            fail();
        } catch (PublicApiException e) {
        ...
        try {
            publicApiClient.setRequestContext(
                    new RequestContext(network1.getId(), person1.getId()));
            peopleProxy.remove(
                    "people", person1.getId(), "preferences", null, "Unable to DELETE preferences");
            fail();
        } catch (PublicApiException e) {
            assertEquals(HttpStatus.SC_METHOD_NOT_ALLOWED, e.getHttpResponse().getStatusCode());
        }

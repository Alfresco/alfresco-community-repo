...
public class DefaultComparisonUtils implements ComparisonUtils {
  ...
  @Override
  public List<DbObject> findEquivalentObjects(
    DbObject rootObject,
    DbObject objToMatch
  ) {
    EquivalentObjectSeeker objectSeeker = new EquivalentObjectSeeker(
      objToMatch
    );
  ...
  public void compareSimpleOrderedLists(
    ...
    ArrayList<Object> refList = new ArrayList<Object>(
      (List<Object>) refProp.getPropertyValue()
    );
    @SuppressWarnings("unchecked")
    ArrayList<Object> targetList = new ArrayList<Object>(
      (List<Object>) targetProp.getPropertyValue()
    );
    ...
    for (int i = 0; i < maxSize; i++) {
      if (i < refList.size() && i < targetList.size()) {
        DbProperty refIndexedProp = new DbProperty(
          refProp.getDbObject(),
          refProp.getPropertyName(),
          i
        );
        DbProperty targetIndexedProp = new DbProperty(
          targetProp.getDbObject(),
        ...
        DbProperty indexedProp = new DbProperty(
          refProp.getDbObject(),
          refProp.getPropertyName(),
          i
        );
  ...
  public void compareSimpleCollections(
    ...
    ArrayList<? extends Object> leftList = new ArrayList<Object>(
      leftCollection
    );
    ArrayList<? extends Object> rightList = new ArrayList<Object>(
      rightCollection
    );
    ...
    for (int leftIndex = 0; leftIndex < leftList.size(); leftIndex++) {
      ...
      DbProperty leftIndexedProp = new DbProperty(
        leftProp.getDbObject(),
        leftProp.getPropertyName(),
        leftIndex
      );
      ...
      if ((rightIndex = rightList.indexOf(leftObj)) != -1) {
        ...
        DbProperty rightIndexedProp = new DbProperty(
          rightProp.getDbObject(),
          rightProp.getPropertyName(),
          rightIndex
        );
        differences.add(
          Where.IN_BOTH_NO_DIFFERENCE,
  ...
  private void checkNotDbObject(Object obj) {
    if (obj != null && DbObject.class.isAssignableFrom(obj.getClass())) {
      throw new IllegalArgumentException(
        "Property value is a DbObject - this method shouldn't be used to compare this type: " +
        obj
      );
    }
  }

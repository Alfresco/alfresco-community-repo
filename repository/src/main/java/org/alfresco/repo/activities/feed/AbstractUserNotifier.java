...
public abstract class AbstractUserNotifier implements UserNotifier {
    ...
    public void setActivitiesFeedModelBuilderFactory(
            ObjectFactory<ActivitiesFeedModelBuilder> activitiesFeedModelBuilder) {
    ...
    protected void checkProperties() {
        PropertyCheck.mandatory(
                this, "activitiesFeedModelBuilderFactory", activitiesFeedModelBuilderFactory);
    ...
    public Pair<Integer, Long> notifyUser(
        ...
        // own + others (note: template can be changed to filter out user's own activities if
        // needed)
        ...
        List<ActivityFeedEntity> feedEntries =
                activityService.getUserFeedEntries(
                        feedUserId, null, false, false, null, null, feedDBID);
        ...
        if (feedEntries.size() > 0) {
            ...
            for (ActivityFeedEntity feedEntry : feedEntries) {
                try {
                    ...
                    logger.warn(
                            "Skip feed entry for user (" + feedUserId + "): " + je.getMessage());
                    continue;
                }
            }
            ...
            if (activityCount > 0) {
                ...
                Map<String, Serializable> personPrefixProps =
                        new HashMap<String, Serializable>(personProps.size());

...
public abstract class AbstractTestFixture implements TestFixture {
  ...
  public void setup(boolean createTestData) throws Exception {
    ...
    this.transactionHelper =
      (RetryingTransactionHelper) repoService
        ...
        new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
          ...
          public Void execute() throws Throwable {
            ...
            if (testAccount.getPersonIds().size() > 0) {
              ...
              List<String> people = testAccount.peopleSample(
                testAccount.getPersonIds().size()
              );
              ...
              for (final SiteInformation site : getSites().values()) {
                final Iterator<String> peopleIterator = RepoService.getWrappingIterator(
                  i++,
                  people
                );

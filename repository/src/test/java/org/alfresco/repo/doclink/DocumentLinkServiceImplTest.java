...
public class DocumentLinkServiceImplTest extends TestCase {
  ...
  private static final String TEST_USER =
    DocumentLinkServiceImplTest.class.getSimpleName() + "_testuser";
  ...
  public void setUp() throws Exception {
    ...
    site1Folder1 =
      fileFolderService
        .create(site1, site1Folder1Name, ContentModel.TYPE_FOLDER)
        .getNodeRef();
    site1File1 =
      fileFolderService
        .create(site1Folder1, site1File1Name, ContentModel.TYPE_CONTENT)
        .getNodeRef();
    site1File2 =
      fileFolderService
        .create(site1Folder1, GUID.generate(), ContentModel.TYPE_CONTENT)
        .getNodeRef();
    site1Folder2 =
      fileFolderService
        .create(site1, GUID.generate(), ContentModel.TYPE_FOLDER)
        .getNodeRef();
    site1Folder3 =
      fileFolderService
        .create(site1, GUID.generate(), ContentModel.TYPE_FOLDER)
    ...
    site2File =
      fileFolderService
        .create(site2, GUID.generate(), ContentModel.TYPE_CONTENT)
        .getNodeRef();
    site2Folder1 =
      fileFolderService
        .create(site2, GUID.generate(), ContentModel.TYPE_FOLDER)
        .getNodeRef();
    site2Folder2 =
      fileFolderService
        .create(site2, GUID.generate(), ContentModel.TYPE_FOLDER)
    ...
    // without permission
    linkOfFile1Site2 =
      documentLinkService.createDocumentLink(site1File2, site2Folder2);
  }
  ...
  public void testCreateFileLink() throws Exception {
    ...
    NodeRef linkNodeRef = documentLinkService.createDocumentLink(
      site1File1,
      site1Folder2
    );
    ...
    String site1File1LinkName = I18NUtil.getMessage(
      "doclink_service.link_to_label",
      (site1File1Name + ".url")
    );
    NodeRef linkNodeRef2 = fileFolderService.searchSimple(
      site1Folder2,
      site1File1LinkName
    );
    ...
    assertEquals(
      nodeService.getType(linkNodeRef),
      ApplicationModel.TYPE_FILELINK
    );

    // check if the link destination is site1File1_1
    NodeRef linkDestination = documentLinkService.getLinkDestination(
      linkNodeRef
    );
  ...
  public void testCreateFolderLink() throws Exception {
    ...
    // test if the link node is listed as a child of site1Folder2
    String site1Folder1LinkName = I18NUtil.getMessage(
      "doclink_service.link_to_label",
      (site1Folder1Name + ".url")
    );
    NodeRef linkNodeRef2 = fileFolderService.searchSimple(
      site1Folder2,
      site1Folder1LinkName
    );
    assertNotNull(linkNodeRef2);
    assertEquals(linkNodeRef, linkNodeRef2);
  ...
  public void testInvalidDestination() throws Exception {
    try {
      ...
      fail(
        "invalid destination argument must generate IllegalArgumentException."
      );
   ...
   * @throws Exception
   */
  public void testCreateDocLinkWithoutReadPermissionOnSource()
    throws Exception {
    try {
      ...
      fail(
        "no read permission on the source node must generate AccessDeniedException."
      );
   ...
   * @throws Exception
   */
  public void testCreateDocLinkWithoutWritePermissionOnDestination()
    throws Exception {
    try {
      AuthenticationUtil.runAs(
        new RunAsWork<Void>() {
      ...
      fail(
        "no write permission on the destination node must generate AccessDeniedException."
      );
    } catch (AccessDeniedException ex) {
      // Expected
    }
  ...
  public void testDeleteLinks() throws Exception {
    DeleteLinksStatusReport report = AuthenticationUtil.runAs(
      new RunAsWork<DeleteLinksStatusReport>() {
        @Override
    ...
    assertEquals(
      true,
      nodeService.hasAspect(site1File2, ApplicationModel.ASPECT_LINKED)
    );
  ...
  public void testCreateLinksNotAllowed() throws Exception {
    ...
    try {
      invalidLinkNodeRef =
        documentLinkService.createDocumentLink(site1, site2Folder1);
      fail("unsupported source node type : " + nodeService.getType(site1));
    } catch (IllegalArgumentException e) {
      // Expected
    }

    NodeRef firstLinkNodeRef = documentLinkService.createDocumentLink(
      site1File1,
      site1Folder1
    );
    assertEquals(
      true,
      nodeService.hasAspect(site1File1, ApplicationModel.ASPECT_LINKED)
    );
    ...
    try {
      ...
      fail(
        "Cannot perform operation since the node (id:" +
        workingCopyNodeRef.getId() +
        ") is locked."
      );
    } catch (IllegalArgumentException e) {
      // Expected
    ...
    try {
      invalidLinkNodeRef =
        documentLinkService.createDocumentLink(site1File1, site1Folder1);
      fail(
        "Cannot perform operation since the node (id:" +
        site1File1.getId() +
        ") is locked."

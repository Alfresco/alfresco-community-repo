 ...
 *
 * <p>Provides a DiskInterface that deals with "shuffles". Shuffles are implemented by the Rules
 * Engine.
 *
 * <p>Sits on top of the repository and is non-retryable and non-transactional. It is, however
 * thread safe and multiple callers may call in parallel.
 */
public class NonTransactionalRuleContentDiskDriver implements ExtendedDiskInterface {
    ...
    private class DriverState {
        ...
        Map<String, EvaluatorContext> contextMap =
                new ConcurrentHashMap<String, EvaluatorContext>();
    }

    private static final Log logger =
            LogFactory.getLog(NonTransactionalRuleContentDiskDriver.class);
    ...
    private void releaseEvaluatorContextIfEmpty(
            DriverState driverState, EvaluatorContext ctx, String folder) {
        synchronized (driverState.contextMap) {
            if (ctx != null) {

...
public class LocalFeedGenerator extends AbstractFeedGenerator {
    ...
    protected boolean generate() throws Exception {
        ...
        AuthenticationUtil.runAs(
                ...
                AuthenticationUtil.getSystemUserName()); // need web scripts to support System-level
        // authentication ... see RepositoryContainer !

        // process the activity posts using the batch processor {@link BatchProcessor}
        BatchProcessor.BatchProcessWorker<JobSettings> worker =
                new BatchProcessor.BatchProcessWorker<JobSettings>() {
                    @Override
                        ...
                        txHelper.doInTransaction(
                                new RetryingTransactionCallback<Void>() {
                                    public Void execute() throws Throwable {
                                        ...
                                        // FeedTaskProcessor takes JobSettings parameters instead
                                        // collection of ActivityPost. FeedTaskProcessor can be
                                        // refactored.
                                        feedTaskProcessor.process(
                                                jobTaskNode, minSeq, maxSeq, webScriptsCtx);
                                        return null;
                                    }
                                },
                                false,
        ...
        BatchProcessWorkProvider<JobSettings> provider =
                new BatchProcessWorkProvider<JobSettings>() {
                    ...
                    public Collection<JobSettings> getNextWork() {
                        ...
                        // One JobSettings object will be returned. Because FeedTaskProcessor
                        // fetches list activity posts by itself before processing.
        ...
        // batchSize and loggingInterval parameters are equal 1 because provider always will provide
        // collection with one JobSettings object.
        // FeedTaskProcessor fetches list activity posts by itself before processing. It needs only
        // JobSettings parameters. FeedTaskProcessor can be refactored.
        new BatchProcessor<JobSettings>(
                        "LocalFeedGenerator", txHelper, provider, numThreads, 1, null, logger, 1)
                .process(worker, true);

 ...
 *
 * <p>Implements a low-level node browser client for the Admin Console tool.
 *
 * @author Kevin Roast
 * @since 5.1
 */
public class NodeBrowserPost extends DeclarativeWebScript implements Serializable {
    ...
    public boolean getInheritPermissions(NodeRef nodeRef) {
        Boolean inheritPermissions =
                this.getPermissionService().getInheritParentPermissions(nodeRef);
    ...
    public List<Permission> getPermissions(NodeRef nodeRef) {
        ...
        AccessStatus readPermissions =
                this.getPermissionService()
                        .hasPermission(nodeRef, PermissionService.READ_PERMISSIONS);
        if (readPermissions.equals(AccessStatus.ALLOWED)) {
            List<Permission> nodePermissions = new ArrayList<Permission>();
            for (Iterator<AccessPermission> iterator =
                            getPermissionService().getAllSetPermissions(nodeRef).iterator();
                    iterator.hasNext(); ) {
    ...
    public List<PeerAssociation> getAssocs(NodeRef nodeRef) {
        ...
        for (AssociationRef ref : refs) {
            assocs.add(
                    new PeerAssociation(
                            ref.getTypeQName(), ref.getSourceRef(), ref.getTargetRef()));
        }
        return assocs;
    }
    ...
    protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
        ...
        try {
                                    ...
                                    .doInTransaction(
                                            new RetryingTransactionCallback<Void>() {
                                                ...
                                                public Void execute() throws Throwable {
                                                    // delete the node using the standard
                                                    // NodeService
                                                    nodeService.deleteNode(
                                                            new NodeRef(executeValue));
                                                    return null;
                                                }
                                            },
                                            false,
                            ...
                            transactionService
                                    ...
                                    .doInTransaction(
                                            new RetryingTransactionCallback<Void>() {
                                                @Override
                                                public Void execute() throws Throwable {
                                                    // delete the node - but ensure that it is not
                                                    // archived
                            ...
                            transactionService
                                    ...
                                    .doInTransaction(
                                            new RetryingTransactionCallback<Void>() {
                                                @Override
                                                public Void execute() throws Throwable {
                                                    // argument value contains "NodeRef|QName"
                                                    // packed string
                                                    String[] parts = executeValue.split("\\|");
                                                    nodeService.removeProperty(
                                                            new NodeRef(parts[0]),
                                                            QName.createQName(parts[1]));
                                                    return null;
                                                }
                                            },
                                            false,
                                            ...
                                            new RetryingTransactionCallback<Void>() {
                                                ...
                                                public Void execute() throws Throwable {
                                                    ...
                                                    if (cociService.isCheckedOut(ref)) {
                                                        NodeRef wcRef =
                                                                cociService.getWorkingCopy(ref);
                                                        if (wcRef != null) {
            ...
            // 'action' is a view action that request an update of the admin console view state e.g.
            // 'search' or 'children'
                                ...
                                case "noderef":
                                    {
                                        ...
                                        if (!exists) {
                                            throw new AlfrescoRuntimeException(
                                                    "Node " + nodeRef + " does not exist.");
                                        }
                                        currentNode = nodeRef;
                                        // this is not really a search for results, it is a direct
                                        // node reference
                                        // so gather the child assocs as usual and update the action
                                        // value for the UI location
        ...
        if (currentNode != null) {
            ...
            info.put(
                    "path",
                    getNodeService().getPath(currentNode).toPrefixString(getNamespaceService()));
            info.put(
                    "type",
                    getNodeService().getType(currentNode).toPrefixString(getNamespaceService()));
            ChildAssociationRef parent = getNodeService().getPrimaryParent(currentNode);
            info.put(
                    "parent",
                    parent.getParentRef() != null ? parent.getParentRef().toString() : "");
        ...
        // The logic to generate the navigation section and server meta-data is all tied into
        // alfresco-common.lib.js
        // which is great for writing JS based JMX surfaced pages, but not so great for Java backed
        // WebScripts.
        status.setCode(301);
        ...
        status.setLocation(
                buildUrl(
                        req,
                        returnParams,
                        execute != null && execute.length() != 0 ? execute : action));
    ...
    public class QNameBean implements Serializable {
        ...
        public String getPrefixedName() {
            try {
                return prefixString != null
                        ? prefixString
                        : (prefixString = name.toPrefixString(getNamespaceService()));
    ...
    public class ChildAssociation extends Association implements Serializable {
        ...
        public ChildAssociation(ChildAssociationRef ref) {
            super(
                    ref.getQName() != null ? ref.getQName() : null,
                    ref.getTypeQName() != null ? ref.getTypeQName() : null);

            this.childRef = ref.getChildRef();
            this.parentRef = ref.getParentRef(); // could be null
            if (childRef != null) this.childType = new QNameBean(getNodeType(childRef));
            if (parentRef != null) this.parentType = new QNameBean(getNodeType(parentRef));
    ...
    public class ChildAssocRefWrapper implements Serializable {
        ...
        public String getTypeQName() {
            return typeqname != null
                    ? typeqname
                    : (typeqname =
                            ref.getTypeQName() != null
                                    ? ref.getTypeQName().toPrefixString(getNamespaceService())
                                    : "");
        }
        ...
        public String getQName() {
            return qname != null
                    ...
                    : (qname =
                            ref.getQName() != null
                                    ? ref.getQName().toPrefixString(getNamespaceService())
                                    : "");
        }
    ...
    public class PeerAssociation extends Association implements Serializable {
        ...
        public PeerAssociation(QName typeName, NodeRef sourceRef, NodeRef targetRef) {
            ...
            if (sourceRef != null) this.sourceType = new QNameBean(getNodeType(sourceRef));
            if (targetRef != null) this.targetType = new QNameBean(getNodeType(targetRef));
        }
    ...
    public class Property implements Serializable {
        ...
        public Property(QName qname, Serializable value, NodeRef nodeRef) {
            ...
            if (propDef != null) {
                ...
                // ALF-21950 We need to check if the property belongs to the type of the node or to
                // their ancestors
                if (propDef.getContainerClass().isAspect()) {
                    residual = false;
                } else {
                    ClassDefinition classDef =
                            getDictionaryService().getClass(getNodeService().getType(nodeRef));
                    boolean found = false;
            ...
            if (value instanceof Collection) {
                ...
                for (Serializable multiValue : oldValues) {
                    values.add(
                            new Value(
                                    multiValue instanceof QName
                                            ? new QNameBean((QName) multiValue)
                                            : multiValue));
                }
            } else {
                values =
        ...
        public boolean isAny() {
            return (getTypeName() == null)
                    ? false
                    : getTypeName().getName().equals(DataTypeDefinition.ANY.toString());
        }
        ...
        public class Value implements Serializable {
                ...
                if (datatype == null || datatype.equals(DataTypeDefinition.ANY.toString())) {
                    if (value != null) {
                        DataTypeDefinition dataTypeDefinition =
                                getDictionaryService().getDataType(value.getClass());
                        if (dataTypeDefinition != null) {
                            datatype =
                                    getDictionaryService()
                                            .getDataType(value.getClass())
                                            .getName()
                                            .toString();
                        }
                    }
            ...
            public boolean isContent() {
                ...
                return (datatype == null)
                        ? false
                        : datatype.equals(DataTypeDefinition.CONTENT.toString());
            }
            ...
            public boolean isNodeRef() {
                ...
                return (datatype == null)
                        ? false
                        : datatype.equals(DataTypeDefinition.NODE_REF.toString())
                                || datatype.equals(DataTypeDefinition.CATEGORY.toString());
            }

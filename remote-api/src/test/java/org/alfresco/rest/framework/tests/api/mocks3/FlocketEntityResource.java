...
@EntityResource(
        name = "flocket",
        title = "A resource used for testing binary properties with lost of properties")
public class FlocketEntityResource
        implements BinaryResourceAction.Read,
                BinaryResourceAction.Delete,
                BinaryResourceAction.Update<Flocket> {

    @Override
    @WebApiDescription(title = "Updates a flocket")
    @BinaryProperties({"photo", "album"})
    public Flocket updateProperty(
            String entityId, BasicContentInfo contentInfo, InputStream stream, Parameters params) {
        return null;
    }

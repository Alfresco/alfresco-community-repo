...
public class ScriptReplicationService extends BaseScopableProcessorExtension {
    ...
    /**
     * Creates a new {@link ScriptReplicationDefinition} and sets the replication name and the
     * description to the specified values.
     *
     * @param replicationName A unique identifier used to specify the created {@link
     ...
     * @see
     *     org.alfresco.service.cmr.replication.ReplicationService#createReplicationDefinition(String,
     *     String)
     */
    public ScriptReplicationDefinition createReplicationDefinition(
            String replicationName, String description) {
        if (logger.isDebugEnabled()) {
            StringBuilder msg = new StringBuilder();
            msg.append("Creating ScriptReplicationDefinition [")
                    .append(replicationName)
                    .append(", ")
                    .append(description)
                    .append("]");
        ...
        ReplicationDefinition replicationDefinition =
                replicationService.createReplicationDefinition(replicationName, description);
        return new ScriptReplicationDefinition(
                serviceRegistry, replicationService, this.getScope(), replicationDefinition);
    }
    ...
    public void saveReplicationDefinition(ScriptReplicationDefinition definition) {
        if (logger.isDebugEnabled()) {
            ...
            msg.append("Saving ScriptReplicationDefinition [")
                    .append(definition.getReplicationName())
                    .append(", ")
    ...
    public ScriptReplicationDefinition loadReplicationDefinition(String replicationName) {
        ReplicationDefinition replicationDefinition =
                replicationService.loadReplicationDefinition(replicationName);
        if (replicationDefinition == null) return null;
        return new ScriptReplicationDefinition(
                serviceRegistry, replicationService, this.getScope(), replicationDefinition);
    }
    ...
    public ScriptReplicationDefinition[] loadReplicationDefinitions(String targetName) {
        List<ReplicationDefinition> definitions =
                replicationService.loadReplicationDefinitions(targetName);
        return toScriptReplicationDefinitions(definitions);
    }
    ...
    private ScriptReplicationDefinition[] toScriptReplicationDefinitions(
            List<ReplicationDefinition> definitions) {
        ScriptReplicationDefinition[] scriptDefs =
                new ScriptReplicationDefinition[definitions.size()];
        for (int i = 0; i < scriptDefs.length; i++) {
            ReplicationDefinition def = definitions.get(i);
            scriptDefs[i] =
                    new ScriptReplicationDefinition(
                            serviceRegistry, replicationService, this.getScope(), def);
        }
        return scriptDefs;
    }
}

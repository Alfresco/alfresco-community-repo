...
public class GetChildrenCannedQueryFactory extends AbstractCannedQueryFactory<NodeRef> {
    ...
    public CannedQuery<NodeRef> getCannedQuery(CannedQueryParameters parameters) {
        ...
        return (CannedQuery<NodeRef>)
                new GetChildrenCannedQuery(
                        nodeDAO,
                        qnameDAO,
                        cannedQueryDAO,
                        nodePropertyHelper,
    ...
    /**
     * Retrieve an optionally filtered/sorted instance of a {@link CannedQuery} based on parameters
     * including request for a total count (up to a given max)
     *
     * <p>Note: if both filtering and sorting is required then the combined total of unique QName
     * properties should be the 0 to 3.
     *
     ...
     * @param pagingRequest skipCount, maxItems - optionally queryExecutionId and
     ...
     * @param assocTypeQNames qnames of assocs to include (may be null)
     * @param childTypeQNames type qnames of children nodes (pre-filter)
     * @param inclusiveAspects If not null, only child nodes with any aspect in this collection will
     *     be included in the results.
     * @param exclusiveAspects If not null, any child nodes with any aspect in this collection will
     *     be excluded in the results.
     * @param filterProps filter properties
     * @param sortProps sort property pairs (QName and Boolean - true if ascending)
     * @param pagingRequest skipCount, maxItems - optionally queryExecutionId and
     *     requestTotalCountMax
    ...
    public CannedQuery<NodeRef> getCannedQuery(
        ...
        // specific query params - context (parent) and inclusive filters (child types, property
        // values)
        GetChildrenCannedQueryParams paramBean =
                new GetChildrenCannedQueryParams(
                        tenantService.getName(parentRef),
                        assocTypeQNames,
                        childTypeQNames,
                        inclusiveAspects,
                        exclusiveAspects,
    ...
    /**
     * Retrieve an unsorted instance of a {@link CannedQuery} based on parameters including request
     * for a total count (up to a given max)
     *
     * @param parentRef parent node ref
     * @param pattern the pattern to use to filter children (wildcard character is '*')
     * @param assocTypeQNames qnames of assocs to include (may be null)

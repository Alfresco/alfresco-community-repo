...
public class LockServiceImpl
    ...
    public LockServiceImpl() {
        this.lockServiceTrait =
                new ExtendedTrait<LockServiceTrait>(
                        AJProxyTrait.create(this, LockServiceTrait.class));
    }
    ...
    private void addToIgnoreSet(NodeRef nodeRef) {
        Set<NodeRef> ignoreNodeRefs =
                (Set<NodeRef>) AlfrescoTransactionSupport.getResource(KEY_IGNORE_NODES);
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.lock.LockService#lock(org.alfresco.service.cmr.repository.NodeRef,
     ...
     * @see
     *     org.alfresco.service.cmr.lock.LockService#lock(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.cmr.lock.LockType, int)
     */
    @Override
    @Extend(traitAPI = LockServiceTrait.class, extensionAPI = LockServiceExtension.class)
     ...
     * @see
     *     org.alfresco.service.cmr.lock.LockService#lock(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.cmr.lock.LockType, int, Lifetime, String)
     */
    @Override
    @Extend(traitAPI = LockServiceTrait.class, extensionAPI = LockServiceExtension.class)
     ...
     * @see
     *     org.alfresco.service.cmr.lock.LockService#lock(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.cmr.lock.LockType, int, Lifetime, boolean)
     */
    @Override
    @Extend(traitAPI = LockServiceTrait.class, extensionAPI = LockServiceExtension.class)
    public void lock(
    ...
    public void lock(
        ...
        if (lifetime.equals(Lifetime.EPHEMERAL) && (timeToExpire > MAX_EPHEMERAL_LOCK_SECONDS)) {
            throw new IllegalArgumentException(
                    "Attempt to create ephemeral lock for "
                            + timeToExpire
                            + " seconds - exceeds maximum allowed time.");
        }
        ...
        if (LockStatus.LOCKED.equals(currentLockStatus) == true) {
            ...
            if (lifetime == Lifetime.PERSISTENT) {
                ...
                try {
                    ...
                    persistLockProps(
                            nodeRef, lockType, lifetime, userName, expiryDate, additionalInfo);
                } finally {
                ...
                LockState lock =
                        LockState.createLock(
                                nodeRef, lockType, userName, expiryDate, lifetime, additionalInfo);
                ...
                TransactionalResourceHelper.getMap(KEY_MODIFIED_NODES)
                        .put(nodeRef, currentLockInfo);
                AlfrescoTransactionSupport.bindListener(this);
            } else {
                throw new IllegalStateException(
                        lifetime.getClass().getSimpleName()
    ...
    private void persistLockProps(
        ...
        try {
            ...
            this.nodeService.setProperty(
                    nodeRef, ContentModel.PROP_LOCK_LIFETIME, lifetime.toString());
            this.nodeService.setProperty(nodeRef, ContentModel.PROP_EXPIRY_DATE, expiryDate);
            this.nodeService.setProperty(
                    nodeRef, ContentModel.PROP_LOCK_ADDITIONAL_INFO, additionalInfo);
        } finally {
     ...
     * @see
     *     org.alfresco.service.cmr.lock.LockService#lock(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.cmr.lock.LockType, int, boolean)
     */
    @Extend(traitAPI = LockServiceTrait.class, extensionAPI = LockServiceExtension.class)
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.lock.LockService#unlock(org.alfresco.service.cmr.repository.NodeRef,
     *     boolean)
     */
    @Override
    @Extend(traitAPI = LockServiceTrait.class, extensionAPI = LockServiceExtension.class)
    ...
    public void unlock(NodeRef nodeRef, boolean unlockChildren, boolean allowCheckedOut)
        ...
        if (lockState.isLockInfo()) {
            ...
            if (!allowCheckedOut
                    && nodeService.hasAspect(nodeRef, ContentModel.ASPECT_CHECKED_OUT)) {
    ...
    public LockType getLockType(NodeRef nodeRef) {
        ...
        if (this.nodeService.hasAspect(nodeRef, ContentModel.ASPECT_LOCKABLE) == true) {
            String lockTypeString =
                    (String) this.nodeService.getProperty(nodeRef, ContentModel.PROP_LOCK_TYPE);
    ...
    public void checkForLock(NodeRef nodeRef) throws NodeLockedException {
        ...
        if (nodeRef != null && userName != null) {
            ...
            // Check to see if should just ignore this node - note: special MT System due to
            // AuditableAspect
            if (!(ignore(nodeRef)
                    || tenantService
                            .getBaseNameUser(effectiveUserName)
     ...
     *
     * <p>Ensures that the property values of the lock aspect are not 'frozen' in the version store.
     */
    ...
    private List<NodeRef> getLocks(StoreRef storeRef, String query) {
        ...
        try {
            resultSet = this.searchService.query(storeRef, SearchService.LANGUAGE_LUCENE, query);
    ...
    public List<NodeRef> getLocks(StoreRef storeRef, LockType lockType) {
        return getLocks(
                ...
                "ASPECT:\""
                        ...
                        + "\""
                        + " +@\\{http\\://www.alfresco.org/model/content/1.0\\}"
                        + ContentModel.PROP_LOCK_TYPE.getLocalName()
                        + ":\""
                        + lockType.toString()
                        + "\"");
    }
    ...
    public LockState getLockState(NodeRef nodeRef) {
        ...
        // ALF-20361: It is possible that a rollback has resulted in a "non-lock" lock state being
        // added to
        // the lock store. Because of that, we check both whether the retrieved lockState is null
        // and, if it isn't,
        ...
        if (lockState == null || !lockState.isLockInfo()) {
            ...
            if (nodeService.exists(nodeRef)
                ...
                String lockOwner =
                        (String) nodeService.getProperty(nodeRef, ContentModel.PROP_LOCK_OWNER);

                Date expiryDate =
                        (Date) nodeService.getProperty(nodeRef, ContentModel.PROP_EXPIRY_DATE);
                String lockTypeStr =
                        (String) nodeService.getProperty(nodeRef, ContentModel.PROP_LOCK_TYPE);
                LockType lockType = lockTypeStr != null ? LockType.valueOf(lockTypeStr) : null;
                String lifetimeStr =
                        (String) nodeService.getProperty(nodeRef, ContentModel.PROP_LOCK_LIFETIME);
                Lifetime lifetime = lifetimeStr != null ? Lifetime.valueOf(lifetimeStr) : null;
                String additionalInfo =
                        (String)
                                nodeService.getProperty(
                                        nodeRef, ContentModel.PROP_LOCK_ADDITIONAL_INFO);

                // Mark lockstate as PERSISTENT as it was in the persistent storage!
                lockState =
                        LockState.createLock(
                                nodeRef, lockType, lockOwner, expiryDate, lifetime, additionalInfo);
            } else {
    ...
    public void afterRollback() {
        // As rollback has occurred we are unable to keep hold of any ephemeral locks set during
        // this transaction.
        Map<NodeRef, LockState> lockedNodes =
                TransactionalResourceHelper.getMap(KEY_MODIFIED_NODES);

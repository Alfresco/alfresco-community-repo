...
public class TransformerConfigMBeanImplTest {
    @Mock private AdminUiTransformerDebug transformerDebug;

    @Mock private MimetypeService mimetypeService;

    // Would like to use mockito, see getTransformationLogTest(),
    // but getting the error WrongTypeOfReturnValue: String[] cannot be returned by
    // getUpperMaxEntries()
    ...
    public void setUp() throws Exception {
        ...
        mockMimetypes(
                mimetypeService, "application/pdf", "pdf", "image/png", "png", "text/plain", "txt");
    }

    /**
     * Mock up the responses from the mimetypeService so that it: a) returns all the supplied
     * mimetypes b) returns the extension given the mimetype c) returns the mimetype given the
     * extension.
     *
     ...
     */
    public static void mockMimetypes(
            MimetypeService mimetypeService, String... mimetypesAndExtensions) {
        if (mimetypesAndExtensions.length % 2 != 0) {
            // Not using IllegalArgumentException as this is thrown by classes under test
            throw new java.lang.IllegalStateException(
                    "There should be an extension for every mimetype");
        }
        ...
        for (int i = 0; i < mimetypesAndExtensions.length; i += 2) {
            ...
            when(mimetypeService.getExtension(mimetypesAndExtensions[i]))
                    .thenReturn(mimetypesAndExtensions[i + 1]);
            when(mimetypeService.getMimetype(mimetypesAndExtensions[i + 1]))
                    .thenReturn(mimetypesAndExtensions[i]);
        }
    ...
    public void getExtensionsAndMimetypesTest() {
        when(mimetypeService.getMimetypes(null))
                .thenReturn(Arrays.asList(new String[] {"application/pdf", "image/png"}));
    ...
    private void setupForGetTransformationsByExtension() {
        when(transformerDebug.transformationsByExtension("pdf", "png", true))
                .thenReturn("One result");
        when(transformerDebug.transformationsByExtension(null, "png", true))
                .thenReturn("Lots of results to png");
        when(transformerDebug.transformationsByExtension("pdf", null, true))
                .thenReturn("Lots of results from pdf");
    }
    ...
    public void getTransformationLogTest() {
        ...
        assertArrayEquals(
                new String[] {"test message 1", "test message 2"}, mbean.getTransformationLog(5));
    }

    @Test
    public void getTransformationLogZeroTest() {
        assertArrayEquals(
                new String[] {"No transformations to report"}, mbean.getTransformationLog(5));
    }
    ...
    public void getTransformationDebugLogTest() {
        ...
        logEntries.add("test message 2");
        assertArrayEquals(
                new String[] {"test message 1", "test message 2"},
                mbean.getTransformationDebugLog(5));
    }

    @Test
    public void getTransformationDebugLogZeroTest() {
        assertArrayEquals(
                new String[] {"No transformations to report"}, mbean.getTransformationDebugLog(5));
    }
    ...
    public void testTransformAnyTransformerBadExtensionTest() {
        when(transformerDebug.testTransform("bad", "png"))
                .thenThrow(new IllegalArgumentException("Unknown source extension: bad"));

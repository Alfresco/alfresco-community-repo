...
public class MultiTAdminServiceImpl
  ...
  public void setRepositoryExporterService(
    RepositoryExporterService repositoryExporterService
  ) {
  ...
  private static final String WARN_MSG =
    "system.mt.warn.upgrade_mt_admin_context";
  ...
  public void startTenants() {
    ...
    try {
      ...
      for (Tenant tenant : tenants) {
        if (
          (!(isTenantRoutingContentStore(tenantFileContentStore))) &&
          (
            !tenantFileContentStore
              .getRootLocation()
      ...
      if (logger.isInfoEnabled() && ((enabledCount + disabledCount) > 0)) {
        logger.info(
          String.format(
            "Alfresco Multi-Tenant startup - %d enabled tenants, %d disabled tenants",
            enabledCount,
            disabledCount
          )
        );
  ...
  public void createTenant(
    ...
    if (
      (contentRootContainerPath != null) &&
      (!contentRootContainerPath.isEmpty())
    ) {
          ...
          public Object doWork() {
            ...
            RetryingTransactionCallback<Object> doImportCallback = new RetryingTransactionCallback<Object>() {
              public Object execute() throws Throwable {
                ...
                ImporterBootstrap userImporterBootstrap = (ImporterBootstrap) ctx.getBean(
                  "userBootstrap-mt"
                );
                ...
                ImporterBootstrap systemImporterBootstrap = (ImporterBootstrap) ctx.getBean(
                  "systemBootstrap-mt"
                );
                ...
                // deprecated
                ImporterBootstrap versionImporterBootstrap = (ImporterBootstrap) ctx.getBean(
                  "versionBootstrap-mt"
                );
                ...
                ImporterBootstrap version2ImporterBootstrap = (ImporterBootstrap) ctx.getBean(
                  "version2Bootstrap-mt"
                );
                bootstrapVersionTenantStore(
                  version2ImporterBootstrap,
                  tenantDomain
                );

                ImporterBootstrap spacesArchiveImporterBootstrap = (ImporterBootstrap) ctx.getBean(
                  "spacesArchiveBootstrap-mt"
                );
                bootstrapSpacesArchiveTenantStore(
                  spacesArchiveImporterBootstrap,
                  tenantDomain
                );

                ImporterBootstrap spacesImporterBootstrap = (ImporterBootstrap) ctx.getBean(
                  "spacesBootstrap-mt"
                );
  ...
  public void enableTenant(String tenantDomain) {
    ...
    TenantUpdateEntity tenantUpdateEntity = tenantAdminDAO.getTenantForUpdate(
      tenantDomain
    );
  ...
  public void deleteTenant(String tenantDomain) {
    ...
    if (!existsTenant(tenantDomain)) {
      ...
      try {
        ...
        throw new AlfrescoRuntimeException(
          "Failed to delete tenant: " + tenantDomain,
          t
        );
      }
  ...
  private void importBootstrapSystemTenantStore(
    String tenantDomain,
    ...
    bootstrapView.put(
      "location",
      directorySource.getPath() + "/" + tenantDomain + "_system.acp"
    );
  ...
  private void bootstrapSystemTenantStore(
    ...
    StoreRef tenantBootstrapStoreRef = new StoreRef(
  ...
  private void importBootstrapUserTenantStore(
    String tenantDomain,
    File directorySource
  ) {
    ...
    bootstrapView.put(
      "location",
      directorySource.getPath() + "/" + tenantDomain + "_users.acp"
    );
  ...
  private void bootstrapUserTenantStore(
    ...
    if (tenantAdminRawPassword != null) {
      ...
      String sha256Password = sha256PasswordEncoder.encodePassword(
        new String(tenantAdminRawPassword),
        salt
      );
  ...
  private void importBootstrapVersionTenantStore(
    String tenantDomain,
    File directorySource
  ) {
    // Import Bootstrap (restore) Tenant-Specific Version Store
    ...
    bootstrapView.put(
      "location",
      directorySource.getPath() + "/" + tenantDomain + "_versions2.acp"
    );
  ...
  private void importBootstrapSpacesArchiveTenantStore(
    ...
    bootstrapView.put(
      "location",
      directorySource.getPath() + "/" + tenantDomain + "_spaces_archive.acp"
    );
  ...
  private void importBootstrapSpacesModelsTenantStore(
    String tenantDomain,
    File directorySource
  ) {
    ...
    bootstrapView.put(
      "location",
      directorySource.getPath() + "/" + tenantDomain + "_models.acp"
    );
  ...
  private void importBootstrapSpacesTenantStore(
    String tenantDomain,
    File directorySource
  ) {
    // Import Bootstrap (restore) Tenant-Specific Spaces Store
    ...
    bootstrapView.put(
      "location",
      directorySource.getPath() + "/" + tenantDomain + "_spaces.acp"
    );
  ...
  private void bootstrapSpacesTenantStore(
    ...
    props.put(
      ...
      getTenantGuestUser(tenantDomain)
    );
    ...
    UserUsageTrackingComponent userUsageTrackingComponent = (UserUsageTrackingComponent) ctx.getBean(
  ...
  protected TenantRoutingContentStore tenantRoutingContentStore(
  ...
  private void initTenant(
    ...
    if (contentRoot == null) {
      ...
      if (!isTenantRoutingContentStore(tenantFileContentStore)) {
        ...
        throw new AlfrescoRuntimeException(
          "MT: cannot initialse tenant - TenantRoutingContentStore is not configured AND tenant is not using co-mingled content store (ie. default root location)"
        );
      }
  ...
  protected void validateTenantName(String tenantDomain) {
    ...
    if (!Pattern.matches(REGEX_CONTAINS_ALPHA, tenantDomain)) {
      throw new IllegalArgumentException(
        tenantDomain +
        " is not a valid tenant name (must contain at least one alpha character)"
      );
    }

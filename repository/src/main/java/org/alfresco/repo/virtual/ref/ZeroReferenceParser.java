...
public class ZeroReferenceParser implements ReferenceParser, ZeroEncoding {
     ...
     * Reference}. Parsing is done with the help of the {@link Cursor} class, which keeps track of
     * the String elements that make up the {@link Reference} String representation. This method
     * instantiates and initiates the cursor and checks the validity of the referenceString and
     * relies upon the {@link ZeroReferenceParser#parseReference} for the actual parsing.
     *
     ...
     * Parses a String representation of a {@link Reference} and returns the corresponding {@link
     * Reference}. Parsing is done with the help of the {@link Cursor} class, which keeps track of
     * the String elements that make up the {@link Reference} String representation.
     *
     * @param cursor
    ...
    private Reference parseReference(Cursor cursor) throws ReferenceParseException {
        try {
            ...
            cursor.i++;
            final Resource resource = parseResource(protocolResourceEncoding, cursor);
            final List<Parameter> parameters = parseParameters(cursor);

            Reference reference =
                    new Reference(Encodings.ZERO.encoding, protocol, resource, parameters);
            return reference;
    ...
    /**
     * Parses a {@link Resource} which is an instance of {@link RepositoryResource} or {@link
     * ClasspathResource} if the current token is contained in the array {@link
     * ZeroEncoding#RESOURCE_PARAMETER}.
     *
     ...
     * @throws ReferenceParseException
     */
    private Resource parseResource(int resourceEncoding, Cursor cursor)
            throws ReferenceParseException {
    ...
    /**
     * Parses a {@link Resource} reference which is an instance of {@link RepositoryResource} or
     * {@link ClasspathResource}, depending on the {@link ZeroEncoding#RESOURCE_PARAMETER}
     *
     * @param cursor
    ...
    /**
     * Creates a list of {@link Parameter}s by parsing the individual parameters given by the
     * cursor's tokens. Stops parsing parameters upon encountering {@link
     * ZeroEncoding#REFERENCE_DELIMITER}, which signals the end of a {@link ReferenceParameter}
     *
     * @param cursor
    ...
    /**
     * Creates a {@link Parameter} reference that is an instance of {@link StringParameter} by
     * parsing the resource given by the cursor's current token.
     *
     * @param cursor
    ...
    /**
     * Creates a {@link Parameter} reference that is an instance of {@link ResourceParameter} by
     * parsing the resource given by the cursor's current token.
     *
     * @param cursor
    ...
    /**
     * Creates a {@link RepositoryNodeRef} reference from the default {@link StoreRef}, SpacesStore
     * and from the node id given by the cursor's current token.
     *
     * @param cursor
    ...
    /**
     * Creates a {@link ReferenceParameter} reference from the cursor's tokens up to the * token
     * which signifies the end of the current {@link ReferenceParameter}.
     *
     * @param cursor

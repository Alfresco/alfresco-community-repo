 ...
 */
public class SiteServiceImpl extends AbstractLifecycleBean
        implements SiteServiceInternal,
                SiteModel,
                NodeArchiveServicePolicies.BeforePurgeNodePolicy,
                NodeServicePolicies.OnRestoreNodePolicy,
                NodeArchiveServicePolicies.BeforeRestoreArchivedNodePolicy,
                NodeArchiveServicePolicies.OnRestoreArchivedNodePolicy {
    /** Logger */
    ...
    private static final String GROUP_SITE_ADMINISTRATORS_AUTHORITY =
            PermissionService.GROUP_PREFIX + SITE_ADMINISTRATORS_AUTHORITY;
    ...
    private static final String MSG_VISIBILITY_GROUP_MISSING =
            "site_service.visibility_group_missing";
    ...
    private static final String MSG_SITE_CONTAINER_NOT_FOLDER =
            "site_service.site_container_not_folder";
    ...
    public void setPublicServiceAccessService(
    ...
    public void setCannedQueryRegistry(
            NamedObjectRegistry<CannedQueryFactory<? extends Object>> cannedQueryRegistry) {
    ...
    public boolean hasCreateSitePermissions() {
        // NOTE: see ALF-13580 - since 3.4.6 PermissionService.CONTRIBUTOR is no longer used as the
        // default on the Sites folder
        // instead the ability to call createSite() and the Spring configured ACL is the mechanism
        // used to protect access.
        return (publicServiceAccessService.hasAccess(
                        "SiteService", "createSite", "", "", "", "", true)
                == AccessStatus.ALLOWED);
    }

    /**
     * @see org.alfresco.service.cmr.site.SiteService#createSite(java.lang.String, java.lang.String,
     *     java.lang.String, java.lang.String, boolean)
     */
    public SiteInfo createSite(
            final String sitePreset,
    ...
    public SiteInfo createSite(
        ...
        return createSite(
                sitePreset, passedShortName, title, description, visibility, SiteModel.TYPE_SITE);
    }
    ...
    public SiteInfo createSite(
        ...
        if (existingSite != null
                || authorityService.authorityExists(getSiteGroup(shortName, true))) {
                        ...
                        new RunAsWork<NodeRef>() {
                            ...
                            public NodeRef doWork() throws Exception {
                                ...
                                behaviourFilter.disableBehaviour(
                                        siteParent, ContentModel.ASPECT_AUDITABLE);
                                try {
                                    return nodeService
                                            .createNode(
                                                    siteParent,
        ...
        eventPublisher.publishEvent(
                new EventPreparator() {
                    ...
                    public Event prepareEvent(String user, String networkId, String transactionId) {
                        return new SiteManagementEvent(
                                "site.create",
                                transactionId,
                                networkId,
                                new Date().getTime(),
     ...
     *
     * <p>Creates the top-level site group, plus all the Role groups required for users of the site.
     *
     * <p>Note - Changes here likely need to be replicated to the {@link #updateSite(SiteInfo)}
    ...
    private void setupSitePermissions(
            ...
            final SiteVisibility visibility,
            final Map<String, Set<String>> memberships,
            final boolean ignoreExistingGroups) {
        ...
        AuthenticationUtil.runAs(
                new AuthenticationUtil.RunAsWork<Object>() {
                    public String doWork() throws Exception {
                        ...
                        // From Alfresco 3.4 the 'site public' group is configurable. Out of the box
                        // it is
                        // GROUP_EVERYONE so unconfigured behaviour is unchanged. But from 3.4,
                        // admins
                        // can change the value of property site.public.group via JMX/properties
                        // files
                        ...
                        boolean publicGroupExists =
                                authorityService.authorityExists(sitePublicGroup);
                        if (!PermissionService.ALL_AUTHORITIES.equals(sitePublicGroup)
                                && !publicGroupExists
                                && !SiteVisibility.PRIVATE.equals(visibility)) {
                            // If the group specified in the settings does not exist, we cannot
                            // create the site.
                            throw new SiteServiceException(
                                    MSG_VISIBILITY_GROUP_MISSING, new Object[] {sitePublicGroup});
                        }
                        ...
                        String siteGroup =
                                authorityService.getName(AuthorityType.GROUP, siteGroupShortName);
                        ...
                        Set<String> permissions =
                                permissionService.getSettablePermissions(siteType);
                        for (String permission : permissions) {
                            // Create a group for the permission
                            String permissionGroupShortName =
                                    getSiteRoleGroup(shortName, permission, false);
                            /* MNT-11289 fix - group is probably already exists. we should check it for existence */
                            String authorityName =
                                    authorityService.getName(
                                            AuthorityType.GROUP, permissionGroupShortName);
                            ...
                            permissionService.setPermission(
                                    siteNodeRef, permissionGroup, permission, true);
                        }
                        ...
                        if (SiteVisibility.PUBLIC.equals(visibility) == true
                            ...
                            permissionService.setPermission(
                                    siteNodeRef, sitePublicGroup, SITE_CONSUMER, true);
                        ...
                        Set<String> currentManagers =
                                authorityService.getContainedAuthorities(
                                        AuthorityType.USER,
                                        getSiteRoleGroup(shortName, SiteModel.SITE_MANAGER, true),
                                        false);
        ...
        if (siteRoot == null) {
            ...
            if (filterIsPresent) {
                ...
                String escNameFilter =
                        SearchLanguageConversion.escapeLuceneQuery(filter.replace('"', ' '));
    ...
    public List<SiteInfo> findSites(String filter, String sitePresetFilter, int size) {
        ...
        if (siteRoot == null) {
            ...
            // A search for name/title/description = "*" does not need to be put into the Lucene
            // query.
            ...
            final boolean sitePresetFilterIsPresent =
                    sitePresetFilter != null && sitePresetFilter.length() > 0;
            ...
            if (filterIsPresentAndNecessary || sitePresetFilterIsPresent) {
                ...
                if (sitePresetFilterIsPresent) {
                    String escPresetFilter =
                            SearchLanguageConversion.escapeLuceneQuery(
                                    sitePresetFilter.replace('"', ' '));
    ...
    public List<SiteInfo> listSites(final String filter, final String sitePresetFilter, int size) {
        ...
        if (siteRoot != null) {
            ...
            final boolean sitePresetFilterHasValue =
                    sitePresetFilter != null && sitePresetFilter.length() > 0;
            ...
            if (filterHasValue) {
                filterProps.add(
                        new FilterPropString(
                                ContentModel.PROP_NAME,
                                filter,
                                FilterTypeString.STARTSWITH_IGNORECASE));
                filterProps.add(
                        new FilterPropString(
    ...
    public List<SiteInfo> listSites(final String userName, final int size) {
        ...
        if (tenantService.isEnabled()
            ...
            return TenantUtil.runAsSystemTenant(
                    new TenantRunAsWork<List<SiteInfo>>() {
    ...
    /**
     * This method uses {@link CannedQuery canned queries} to retrieve {@link SiteModel#TYPE_SITE
     * st:site} NodeRefs with support for {@link PagingRequest result paging}.
     */
    @Override
    public PagingResults<SiteInfo> listSites(
        ...
        GetChildrenCannedQueryFactory getChildrenCannedQueryFactory =
                (GetChildrenCannedQueryFactory) cannedQueryRegistry.getNamedObject(cQBeanName);

        GetChildrenCannedQuery cq =
                (GetChildrenCannedQuery)
                        getChildrenCannedQueryFactory.getCannedQuery(
                                getSiteRoot(),
                                null,
                                null,
                                searchTypeQNames,
    ...
    public String resolveSite(String group) {
        // purge non Site related Groups and strip the group name down to the site "shortName" it
        // relates too
    ...
    /**
     * This method returns the {@link SiteInfo siteInfos} for sites to which the specified user has
     * access. Note that if the user has access to more than 1000 sites, the list will be truncated
     * to 1000 entries.
     *
    ...
    private SiteInfo createSiteInfo(NodeRef siteNodeRef) {
        ...
        String title =
                DefaultTypeConverter.INSTANCE.convert(
                        String.class, properties.get(ContentModel.PROP_TITLE));
        String description =
                DefaultTypeConverter.INSTANCE.convert(
        ...
        siteInfo =
                new SiteInfoImpl(
                        ...
                        customProperties,
                        siteNodeRef);
        siteInfo.setCreatedDate(
                DefaultTypeConverter.INSTANCE.convert(
                        Date.class, properties.get(ContentModel.PROP_CREATED)));
        siteInfo.setLastModifiedDate(
                DefaultTypeConverter.INSTANCE.convert(
    ...
    /**
     * Helper method to get the visibility of the site. If no value is present in the repository
     * then it is calculated from the set permissions. This will maintain backwards compatibility
     * with earlier versions of the service implementation.
     *
     * @param siteNodeRef site node reference
    ...
    private SiteVisibility getSiteVisibility(NodeRef siteNodeRef) {
        ...
        String visibilityValue =
                (String)
                        this.directNodeService.getProperty(
                                siteNodeRef, SiteModel.PROP_SITE_VISIBILITY);
    ...
    public SiteInfo getSite(final String shortName) {
        ...
        if (tenantService.isEnabled()
            ...
            return TenantUtil.runAsSystemTenant(
                    new TenantRunAsWork<SiteInfo>() {
                        public SiteInfo doWork() throws Exception {
                            SiteInfo site = getSiteImpl(sName);
                            return new SiteInfoImpl(
                                    site.getSitePreset(),
                                    shortName,
                                    site.getTitle(),
                                    site.getDescription(),
                                    site.getVisibility(),
                                    site.getCustomProperties(),
                                    site.getNodeRef());
                        }
                    },
                    tenantDomain);
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.site.SiteService#getSite(org.alfresco.service.cmr.repository.NodeRef)
     */
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.site.SiteService#getSiteShortName(org.alfresco.service.cmr.repository.NodeRef)
     */
    public String getSiteShortName(NodeRef nodeRef) {
        ...
        if (siteNodeRef != null) {
            shortName =
                    (String)
                            this.directNodeService.getProperty(siteNodeRef, ContentModel.PROP_NAME);
        }
        return shortName;
    }

    /**
     * This method gets the <code>st:site</code> NodeRef for the Share Site which contains the given
     * NodeRef. If the given NodeRef is not contained within a Share Site, then <code>null</code> is
     * returned.
     *
                    ...
                    AuthenticationUtil.runAs(
                            new AuthenticationUtil.RunAsWork<NodeRef>() {
                                public NodeRef doWork() throws Exception {
                                    ...
                                    NodeRef siteNode =
                                            directNodeService.getChildByName(
                                                    siteRoot,
                                                    ContentModel.ASSOC_CONTAINS,
                                                    shortName);

                                    // cache the result if found - null results will be required to
                                    // ensure new sites are found later
                                    if (siteNode != null) {
        ...
        if (enforcePermissions) {
            ...
            return siteNodeRef == null
                            || !(this.permissionService
                                            .hasPermission(
                                                    siteNodeRef, PermissionService.READ_PROPERTIES)
                                            .equals(AccessStatus.ALLOWED)
                                    || isSiteAdmin(AuthenticationUtil.getRunAsUser()))
                    ? null
                    : siteNodeRef;
        } else {
            return siteNodeRef;
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.site.SiteService#updateSite(org.alfresco.service.cmr.site.SiteInfo)
     */
    public void updateSite(SiteInfo siteInfo) {
        ...
        if (siteNodeRef == null) {
            throw new SiteServiceException(
                    MSG_CAN_NOT_UPDATE, new Object[] {siteInfo.getShortName()});
        }
        ...
        if (currentVisibility.equals(updatedVisibility) == false) {
            ...
            if (SiteVisibility.PUBLIC.equals(currentVisibility) == true
                ...
                this.permissionService.deletePermission(
                        siteNodeRef, sitePublicGroup, SITE_CONSUMER);
                if (sitePublicGroup.equals(PermissionService.ALL_AUTHORITIES)) {
                    this.permissionService.deletePermission(
                            siteNodeRef, PermissionService.ALL_AUTHORITIES, SITE_CONSUMER);
                }
            }

            // If the site was moderated/private before, undo the work of
            // #setModeratedPermissions/#setPrivatePermissions
            ...
            if (SiteVisibility.MODERATED.equals(currentVisibility) == true
                    || SiteVisibility.PRIVATE.equals(currentVisibility) == true) {
    ...
    protected void deleteSiteImpl(final String shortName) {
        ...
        // no need to retain the membership of the site as we postpone delete of authorities until
        // purge from the trashcan

        // The default behaviour is that sites cannot be deleted. But we disable that behaviour here
        // in order to allow site deletion only via this service. Share calls this service for
        // deletion.
        //
                    ...
                    public Void doWork() throws Exception {
                        ...
                        if (authorityService.authorityExists(siteGroup)) {
                            ...
                            Set<String> permissions =
                                    permissionService.getSettablePermissions(siteType);
                            for (String permission : permissions) {
                                String siteRoleGroup =
                                        getSiteRoleGroup(shortName, permission, true);
    ...
    public void listMembers(
            ...
            final SiteMembersCallback callback) {
        ...
        if (tenantService.isEnabled()
            ...
            listMembersImpl(
                    shortName, nameFilter, roleFilter, collapseGroups, callback, true, true);
        }
    }

    // note that this may return an authority more than once
    protected void listMembersImpl(
            String shortName,
            String nameFilter,
            String roleFilter,
        ...
        for (String permission : permissions) {
            ...
            if ((roleFilter == null || roleFilter.length() == 0 || roleFilter.equals(permission))
                    && this.authorityService.authorityExists(groupName)) {
                Set<String> authorities =
                        this.authorityService.getContainedAuthorities(null, groupName, true);
                        ...
                        case GROUP:
                            if (expandGroups) {
                                ...
                                if (nameFilter != null && nameFilter.length() != 0) {
                                    ...
                                    if (authority
                                            .substring(GROUP_PREFIX_LENGTH)
                                            .toLowerCase()
                                            .contains(nameFilterLower)) {
                                        callback.siteMember(authority, permission, false);
                                    } else {
                                        // Does it match on the Group Display Name part instead?
                                        String displayName =
                                                authorityService.getAuthorityDisplayName(authority);
                                        if (displayName != null
                                                && displayName
                                                        .toLowerCase()
                                                        .contains(nameFilterLower)) {
                            ...
                            break;
                        default:
                            // TODO: Should error but do not want to break anything here, so will
                            // fall through (DH)
                            break;
                    }
                }
        ...
        if (expandGroups && includeUsers) {
            for (Map.Entry<String, String> entry : groupsToExpand.entrySet()) {
                Set<String> subUsers =
                        this.authorityService.getContainedAuthorities(
                                AuthorityType.USER, entry.getKey(), false);
                for (String subUser : subUsers) {
                    if (isAcceptedName(nameFilter, subUser)) {
                        // Add the collapsed user into the members list if they do not already
                        // appear in the list
    ...
    public PagingResults<SiteMembership> listMembersPaged(
            ...
            List<Pair<SiteService.SortFields, Boolean>> sortProps,
            PagingRequest pagingRequest) {
        CannedQueryParameters params =
                getCannedQueryParameters(shortName, collapseGroups, sortProps, pagingRequest);

        CannedQuery<SiteMembership> query =
                new SiteMembersCannedQuery(this, personService, nodeService, params);
    ...
    /**
     * @see
     *     org.alfresco.repo.node.NodeServicePolicies.OnRestoreNodePolicy#onRestoreNode(org.alfresco.service.cmr.repository.ChildAssociationRef)
     */
    ...
    public void onRestoreNode(ChildAssociationRef childAssocRef) {
        ...
        setupSitePermissions(
                ...
                (Map<String, Set<String>>)
                        directNodeService.getProperty(
                                siteRef, QName.createQName(null, "memberships")),
                true);
    }

    /**
     * @see
     *     org.alfresco.repo.node.NodeArchiveServicePolicies.BeforeRestoreArchivedNodePolicy#beforeRestoreArchivedNode(org.alfresco.service.cmr.repository.NodeRef)
     */
    @Override
    ...
    /**
     * @see
     *     org.alfresco.repo.node.NodeArchiveServicePolicies.OnRestoreArchivedNodePolicy#onRestoreArchivedNode(org.alfresco.service.cmr.repository.NodeRef)
     */
    @Override
    ...
    /**
     * @see org.alfresco.service.cmr.site.SiteService#listMembers(java.lang.String,
    ...
    public Map<String, String> listMembers(
        ...
        if (tenantService.isEnabled()
            ...
            return TenantUtil.runAsSystemTenant(
                    new TenantRunAsWork<Map<String, String>>() {
                        public Map<String, String> doWork() throws Exception {
                            return listMembersImpl(
                                    sName, nameFilter, roleFilter, size, expandGroups);
                        }
                    },
                    tenantDomain);
        } else {
    ...
    /**
     * @see org.alfresco.service.cmr.site.SiteService#listMembersInfo(String, String, String, int,
     *     boolean)
     */
    ...
    protected Map<String, String> listMembersImpl(
            String shortName,
            String nameFilter,
            String roleFilter,
            int size,
            boolean expandGroups) {
        List<SiteMemberInfo> list =
                listMembersInfoImpl(shortName, nameFilter, roleFilter, size, expandGroups);
    ...
    protected List<SiteMemberInfo> listMembersInfoImpl(
            String shortName,
            String nameFilter,
            String roleFilter,
            int size,
                        ...
                        case GROUP:
                            if (expandGroups) {
                                ...
                                if (nameFilter != null && nameFilter.length() != 0) {
                                    // found a filter - does it match Group name part?
                                    if (matchByFilter(
                                            authority.substring(GROUP_PREFIX_LENGTH).toLowerCase(),
                                            nameFilterLower)) {
                                        members.add(
                                                new SiteMemberInfoImpl(
                                    ...
                                    // No name filter add this group
                                    members.add(
                                            new SiteMemberInfoImpl(authority, permission, false));
                                }
        ...
        if ((expandGroups) && (members.size() < size)) {
            ...
            for (Map.Entry<String, String> entry : groupsToExpand.entrySet()) {
                ...
                for (String subUser : subUsers) {
                    if (isAcceptedName(nameFilter, subUser)) {
                        SiteMemberInfo memberInfo =
                                new SiteMemberInfoImpl(subUser, entry.getValue(), true);
                        // Add the collapsed user into the members list if they do not already
                        // appear in the list
                        if (members.contains(memberInfo) == false) {
                            ...
                            int priority =
                                    roleComparator.compare(
                                            members.get(index).getMemberRole(),
     ...
     *
     * <p>One of the user's firstname or lastname must match at least one of the filters given.
     *
    ...
    private boolean matchPerson(final String[] nameFilters, final String username) {
        ...
        try {
            ...
            String firstName =
                    (String) directNodeService.getProperty(person, ContentModel.PROP_FIRSTNAME);
            String lastName =
                    (String) directNodeService.getProperty(person, ContentModel.PROP_LASTNAME);
            String userName =
                    (String) directNodeService.getProperty(person, ContentModel.PROP_USERNAME);
    ...
    /**
     * @see org.alfresco.service.cmr.site.SiteService#getMembersRoleInfo(java.lang.String,
     *     java.lang.String)
     */
    public SiteMemberInfo getMembersRoleInfo(String shortName, String authorityName) {
        ...
        QName siteType = directNodeService.getType(siteNodeRef);
        Set<String> permissions = this.permissionService.getSettablePermissions(siteType);
        // This set is a lazily evaluated one, so merely getting it in advance as we do here is not
        // expensive
        ...
        for (String role : permissions) {
            ...
            Set<String> authorities =
                    this.authorityService.getContainedAuthorities(null, roleGroup, true);
    ...
    /**
     * Helper method to get the permission groups for a given authority on a site. Returns empty
     * List if the user does not have a explicit membership to the site.
     *
     * <p>A user permission will take precedence over a permission obtained via a group.
     *
    ...
    private List<String> getPermissionGroups(String siteShortName, String authorityName) {
        ...
        // First use the authority's cached recursive group memberships to answer the question
        // quickly
        ...
        // Unfortunately, due to direct membership taking precedence, we can't answer the question
        // quickly if more than one role has been inherited
        ...
        List<String> result = new ArrayList<String>(5);
        Set<String> authorityGroups =
                this.authorityService.getContainingAuthorities(
    ...
    /**
     * @see org.alfresco.service.cmr.site.SiteService#removeMembership(java.lang.String,
     *     java.lang.String)
     */
    public void removeMembership(final String shortName, final String authorityName) {
        ...
        if (role != null) {
            ...
            if ((currentUserName.equals(authorityName) == true)
                    || isSiteAdmin(currentUserName)
                    || (permissionService.hasPermission(
                                    siteNodeRef, PermissionService.CHANGE_PERMISSIONS)
                            == AccessStatus.ALLOWED)) {
                // Run as system user
                AuthenticationUtil.runAs(
                        new AuthenticationUtil.RunAsWork<Object>() {
                ...
                if (authorityType == AuthorityType.USER) {
                    activityService.postActivity(
                            ...
                            shortName,
                            ACTIVITY_TOOL,
                            getActivityUserData(authorityName, ""),
                            authorityName);
                    ...
                    activityService.postActivity(
                            ActivityType.SITE_GROUP_REMOVED,
                            shortName,
                            ACTIVITY_TOOL,
                            getActivityGroupData(authorityDisplayName, ""));
                }
            } else {
                // Throw an exception
    ...
    public boolean canAddMember(
            final String shortName, final String authorityName, final String role) {
        final NodeRef siteNodeRef = getSiteNodeRef(shortName);
        if (siteNodeRef == null) {
            throw new SiteDoesNotExistException(shortName);
        }
        ...
        return ((permissionService.hasPermission(siteNodeRef, PermissionService.CHANGE_PERMISSIONS)
                        == AccessStatus.ALLOWED)
                || isSiteAdmin(currentUserName)
                        ...
                        new AuthenticationUtil.RunAsWork<Object>() {
                            public Object doWork() throws Exception {
                                if (currentRole != null) {
                                    ...
                                    // permission group
                                    String currentGroup =
                                            getSiteRoleGroup(shortName, currentRole, true);
                                    authorityService.removeAuthority(currentGroup, authorityName);
                                }
                ...
                if (currentRole == null) {
                    if (authorityType == AuthorityType.USER) {
                        activityService.postActivity(
                                ...
                                shortName,
                                ACTIVITY_TOOL,
                                getActivityUserData(authorityDisplayName, role),
                                authorityName);
                    } else if (authorityType == AuthorityType.GROUP) {
                        activityService.postActivity(
                                ActivityType.SITE_GROUP_ADDED,
                                shortName,
                                ACTIVITY_TOOL,
                                getActivityGroupData(authorityDisplayName, role));
                    }
                } else {
                    if (authorityType == AuthorityType.USER) {
                        activityService.postActivity(
                                ActivityType.SITE_USER_ROLE_UPDATE,
                                shortName,
                                ACTIVITY_TOOL,
                                getActivityUserData(authorityDisplayName, role));
                    } else if (authorityType == AuthorityType.GROUP) {
                        activityService.postActivity(
                                ActivityType.SITE_GROUP_ROLE_UPDATE,
                                shortName,
                                ACTIVITY_TOOL,
                                getActivityGroupData(authorityDisplayName, role));
                    }
     ...
     *     java.lang.String, org.alfresco.service.namespace.QName, java.util.Map)
     */
    public NodeRef createContainer(
        ...
        // MNT-15743
        componentId =
                componentId.toLowerCase().equals(DOCUMENT_LIBRARY.toLowerCase())
                        ? DOCUMENT_LIBRARY
                        : componentId;
        ...
        if (containerNodeRef == null) {
            ...
            if (containerProperties != null) {
                Map<QName, Serializable> props =
                        this.directNodeService.getProperties(containerNodeRef);
    ...
    /**
     * This method recursively cleans the site permissions on the specified NodeRef and all its
     * primary descendants. This consists of
     *
     * <ul>
     *   <li>the removal of all site permissions pertaining to a site other than the containingSite
     * </ul>
     *
     * If the containingSite is <code>null</code> then the targetNode's current containing site is
     * used.
     *
     * @param targetNode NodeRef
     * @param containingSite the site which the site is a member of. If <code>null</code>, it will
     *     be calculated.
     */
    @Override
    ...
    /**
     * Moderated sites have separate ACLs on each component and don't inherit from the site which
     * has consumer role for everyone.
     */
    ...
    public PagingResults<FileInfo> listContainers(String shortName, PagingRequest pagingRequest) {
        SiteContainersCannedQueryFactory sitesContainersCannedQueryFactory =
                (SiteContainersCannedQueryFactory)
                        cannedQueryRegistry.getNamedObject("siteContainersCannedQueryFactory");

        CannedQueryPageDetails pageDetails =
                new CannedQueryPageDetails(
                        pagingRequest.getSkipCount(), pagingRequest.getMaxItems());
        CannedQuerySortDetails sortDetails =
                new CannedQuerySortDetails(
                        new Pair<Object, SortOrder>(
                                SiteContainersCannedQueryParams.SortFields.ContainerName,
                                SortOrder.ASCENDING));
        SiteContainersCannedQueryParams parameterBean =
                new SiteContainersCannedQueryParams(getSiteNodeRef(shortName));
        CannedQueryParameters params =
    ...
    private NodeRef findContainer(NodeRef siteNodeRef, String componentId)
        ...
        if (!fileInfo.isFolder()) {
            throw new SiteServiceException(
                    MSG_SITE_CONTAINER_NOT_FOLDER, new Object[] {fileInfo.getName()});
        }
        return fileInfo.getNodeRef();
    }

    /**
     * Helper method to create a container if missing, and mark it as a tag scope if it isn't
     * already one
     */
    public static NodeRef getSiteContainer(
            ...
            if (!create) {
                ...
                throw new AlfrescoRuntimeException(
                        "Unable to create the "
                                + componentName
                                + " container from a hidden or non-existant site");
            }
        }
            ...
            if (create) {
                if (transactionService.isReadOnly()) {
                    throw new AlfrescoRuntimeException(
                            "Unable to create the "
                                    + componentName
                                    + " container from a read only transaction");
                }

                // Have the site container created
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Creating " + componentName + " container in site " + siteShortName);
                }
    ...
    private String getActivityUserData(String userName, String role) {
        ...
        if (person != null) {
            memberFN = (String) directNodeService.getProperty(person, ContentModel.PROP_FIRSTNAME);
            memberLN = (String) directNodeService.getProperty(person, ContentModel.PROP_LASTNAME);
        }
    ...
    public int countAuthoritiesWithRole(String shortName, String role) {
        ...
        Set<String> siteUsers =
                this.authorityService.getContainedAuthorities(AuthorityType.USER, group, true);
        Set<String> siteGroups =
                this.authorityService.getContainedAuthorities(AuthorityType.GROUP, group, true);
     ...
     * @param role String
     */
    private void checkLastManagerRemoval(
            final String shortName, final String authorityName, final String role) {
        // Check that we are not about to remove the last site manager
    ...
    public List<SiteInfo> listSites(Set<String> siteNames) {
        List<ChildAssociationRef> assocs =
                this.nodeService.getChildrenByName(
                        getSiteRoot(), ContentModel.ASSOC_CONTAINS, siteNames);
    ...
    public List<SiteMembership> listSiteMemberships(String userName, int size) {
        ...
        Set<String> containingAuthorities =
                authorityService.getContainingAuthorities(
                        AuthorityType.GROUP, actualUserName, false);
        ...
        // (MNT-19035) When querying the site memberships for users other than the authenticated
        // user or site admins, an extra check on the site visibility is enforced.
     ...
     * @return list of child associations between the site root and sites having the requested site
     *     names
     */
    private List<ChildAssociationRef> getSitesAssocsByName(final List<String> siteNames) {
        ...
        while (sitesIterator.hasNext()) {
            ...
            if (iterationPage.size() >= GET_CHILD_ASSOCS_PAGE_SIZE) {
                ...
                childRefPage =
                        directNodeService.getChildrenByName(
                                siteRoot, ContentModel.ASSOC_CONTAINS, iterationPage);
    ...
    public PagingResults<SiteMembership> listSitesPaged(
        ...
        if (sortProps == null) {
            ...
            List<Pair<? extends Object, SortOrder>> sortPairs =
                    new ArrayList<Pair<? extends Object, SortOrder>>(sortProps.size());
            for (Pair<SiteService.SortFields, Boolean> sortProp : sortProps) {
                sortPairs.add(
                        new Pair<SiteService.SortFields, SortOrder>(
                                sortProp.getFirst(),
                                (sortProp.getSecond()
                                        ? SortOrder.ASCENDING
                                        : SortOrder.DESCENDING)));
            }
            TreeSet<SiteMembership> sortedSet =
                    new TreeSet<SiteMembership>(
                            new SiteMembershipComparator(
                                    sortPairs, SiteMembershipComparator.Type.SITES));
            sortedSet.addAll(siteMembers);
            resultList = new ArrayList<SiteMembership>(sortedSet);
        }

        PagingResults<SiteMembership> res =
                new PagingResults<SiteMembership>() {
    ...
    private <T extends Object> PagingResults<T> getPagingResults(
            PagingRequest pagingRequest, final CannedQueryResults<T> results) {
    ...
    /**
     * Private sites have separate ACLs on each component and don't inherit from the site which has
     * consumer role for everyone.
     */
    ...
    public boolean isSiteAdmin(String userName) {
        ...
        return this.authorityService.isAdminAuthority(userName)
                || this.authorityService
                        ...
                        .contains(GROUP_SITE_ADMINISTRATORS_AUTHORITY);
    }
    ...
    @Override
    public PagingResults<SiteGroupMembership> listGroupMembersPaged(
            String shortName,
            List<Pair<SortFields, Boolean>> sortProps,
            PagingRequest pagingRequest) {
        CannedQueryParameters params =
                getCannedQueryParameters(shortName, false, sortProps, pagingRequest);
        CannedQuery<SiteGroupMembership> query =
                new SiteGroupCannedQuery(this, authorityService, params);
    ...
    private CannedQueryParameters getCannedQueryParameters(
            String shortName,
            boolean expandGroups,
            List<Pair<SortFields, Boolean>> sortProps,
            PagingRequest pagingRequest) {
        ...
        SiteMembersCannedQueryParams parameterBean =
                new SiteMembersCannedQueryParams(shortName, expandGroups);
        return new CannedQueryParameters(
                parameterBean,
                pageDetails,
                sortDetails,
                pagingRequest.getRequestTotalCountMax(),

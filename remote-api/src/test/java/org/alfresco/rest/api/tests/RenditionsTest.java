...
public class RenditionsTest extends AbstractBaseApiTest {
  ...
  public void setup() throws Exception {
    ...
    synchronousTransformClient =
      applicationContext.getBean(
        "synchronousTransformClient",
        SynchronousTransformClient.class
      );
  ...
  public void testListNodeRenditions() throws Exception {
    ...
    String folder_Id = addToDocumentLibrary(
      userOneN1Site,
      folderName,
      TYPE_CM_FOLDER,
      userOneN1.getId()
    ...
    assertEquals(
      "Should've only returned the 'doclib' rendition.",
      1,
      renditions.size()
    );
    ...
    getAll(
      getNodeRenditionsUrl(UUID.randomUUID().toString()),
      paging,
      params,
      404
    );
    ...
    String emptyContentNodeId = addToDocumentLibrary(
      userOneN1Site,
      "emptyDoc.txt",
      TYPE_CM_CONTENT,
      userOneN1.getId()
  ...
  public void testGetNodeRendition() throws Exception {
    ...
    Rendition rendition = RestApiUtil.parseRestApiEntry(
      response.getJsonResponse(),
      Rendition.class
    );
    ...
    assertEquals("PNG Image", contentInfo.getMimeTypeName());
    assertNull(
      "Shouldn't have returned the encoding, as the rendition hasn't been created yet.",
      contentInfo.getEncoding()
    );
    assertNull(
      "Shouldn't have returned the size, as the rendition hasn't been created yet.",
      contentInfo.getSizeInBytes()
    );
    ...
    Document jpgImage = RestApiUtil.parseRestApiEntry(
      response.getJsonResponse(),
      Document.class
    );
    ...
    // List all available renditions (includes those that have been created and those that are yet to be created)
    response =
      getAll(getNodeRenditionsUrl(jpgImageNodeId), getPaging(0, 50), 200);
    List<Rendition> renditions = RestApiUtil.parseRestApiEntries(
      response.getJsonResponse(),
      Rendition.class
    );
  ...
  public void testCreateRendition() throws Exception {
    ...
    // Get rendition (not created yet) information for node
    response =
      getSingle(getNodeRenditionsUrl(contentNodeId), "imgpreview", 200);
    Rendition rendition = RestApiUtil.parseRestApiEntry(
      response.getJsonResponse(),
      Rendition.class
    );
    ...
    if (areLocalTransformsAvailable()) {
      ...
      post(
        getNodeRenditionsUrl(emptyContentNodeId),
        toJsonAsString(renditionRequest),
        202
      );
      ...
      multiPartBuilder =
        MultiPartBuilder
          .create()
          .setFileData(new FileData("binaryFileName", file));
      ...
      Document binaryDocument = RestApiUtil.parseRestApiEntry(
        response.getJsonResponse(),
        Document.class
      );
      post(
        getNodeRenditionsUrl(binaryDocument.getId()),
        toJsonAsString(renditionRequest),
        202
      );
    }
    ...
    response =
      post(
        getNodeRenditionsUrl(contentNodeId),
        toJsonAsString(new Rendition().setId("imgpreview")),
        409
      );
    ExpectedErrorResponse errorResponse = RestApiUtil.parseErrorResponse(
      response.getJsonResponse()
    );
    ...
    // nodeId in the path parameter does not represent a file
    post(
      getNodeRenditionsUrl(folder_Id),
      toJsonAsString(renditionRequest),
      400
    );
    ...
    post(
      getNodeRenditionsUrl(contentNodeId),
      toJsonAsString(new Rendition().setId(randomRenditionId)),
      404
    );
    ...
    post(
      getNodeRenditionsUrl(contentNodeId),
      toJsonAsString(new Rendition().setId(null)),
      400
    );
    ...
    post(
      getNodeRenditionsUrl(contentNodeId),
      toJsonAsString(new Rendition().setId("")),
      400
    );
    ...
    post(
      getNodeRenditionsUrl(contentNodeId),
      toJsonAsString(multipleRenditionRequest),
      400
    );
    ...
    assertRenditionCreatedWithWait(
      contentNodeId,
      "doclib",
      "medium",
      "avatar",
    ...
    RenditionService2Impl renditionService2 = applicationContext.getBean(
      "renditionService2",
      RenditionService2Impl.class
    );
    ...
    try {
      ...
      Document txtDocument = RestApiUtil.parseRestApiEntry(
        response.getJsonResponse(),
        Document.class
      );
      ...
      response =
        post(
          getNodeRenditionsUrl(txtDocument.getId()),
          toJsonAsString(renditionRequest),
          501
        );
      errorResponse =
  ...
  public void testCreateRenditionOnUpload() throws Exception {
    ...
    ThumbnailService thumbnailService = applicationContext.getBean(
      "thumbnailService",
  ...
  protected void assertRenditionCreatedWithWait(
    String contentNodeId,
    String... renditionNames
  ) throws Exception {
  ...
  public void testCreateRenditionForNewVersion() throws Exception {
    ...
    Document document1 = RestApiUtil.parseRestApiEntry(
      response.getJsonResponse(),
      Document.class
    );
    ...
    // Get rendition (not created yet) information for node
    response =
      getSingle(getNodeRenditionsUrl(contentNodeId), RENDITION_NAME, 200);
    Rendition rendition = RestApiUtil.parseRestApiEntry(
      response.getJsonResponse(),
      Rendition.class
    );
    ...
    Document document1b = RestApiUtil.parseRestApiEntry(
      response.getJsonResponse(),
      Document.class
    );
    ...
    assertEquals(
      document1b.getModifiedByUser().getId(),
      document1.getModifiedByUser().getId()
    );
    assertEquals(
      document1b.getModifiedByUser().getDisplayName(),
      document1.getModifiedByUser().getDisplayName()
    );

    assertNotEquals(
      document1b.getProperties().get(PROP_LTM),
      document1.getProperties().get(PROP_LTM)
    );
    ...
    Document document2 = RestApiUtil.parseRestApiEntry(
      response.getJsonResponse(),
      Document.class
    );
    ...
    params = Collections.singletonMap("include", "properties");
    response = getSingle(NodesEntityResource.class, contentNodeId, params, 200);
    Document document2b = RestApiUtil.parseRestApiEntry(
      response.getJsonResponse(),
      Document.class
    );
    ...
    assertEquals(
      document2b.getModifiedByUser().getId(),
      document1.getModifiedByUser().getId()
    );
    assertEquals(
      document2b.getModifiedByUser().getDisplayName(),
      document1.getModifiedByUser().getDisplayName()
    );

    // check last thumbnail modification property has changed ! (REPO-1644)
    assertNotEquals(
      document2b.getProperties().get(PROP_LTM),
      document1b.getProperties().get(PROP_LTM)
    );
  }
  ...
  public void testDownloadRendition() throws Exception {
    ...
    response =
      getSingle(getNodeRenditionsUrl(contentNodeId), "doclib/content", 200);
    assertNotNull(response.getResponseAsBytes());
    responseHeaders = response.getHeaders();
    assertNotNull(responseHeaders);
  ...
  private String addToDocumentLibrary(
    Site testSite,
    String name,
    String nodeType,
    String userId

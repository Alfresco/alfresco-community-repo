...
public final class Classification extends BaseScopableProcessorExtension {
  ...
  public Scriptable getAllCategoryNodes(String aspect) {
    Object[] cats = buildCategoryNodes(
      services
        .getCategoryService()
        .getCategories(storeRef, createQName(aspect), CategoryService.Depth.ANY)
    );
  ...
  public String[] getAllClassificationAspects() {
    Collection<QName> aspects = services
      .getCategoryService()
  ...
  public CategoryNode createRootCategory(String aspect, String name) {
    NodeRef categoryNodeRef = services
      .getCategoryService()
      .createRootCategory(storeRef, createQName(aspect), name);
  ...
  public CategoryNode getCategory(String categoryRef) {
    ...
    if (
      services.getNodeService().exists(categoryNodeRef) == true &&
      services
        .getDictionaryService()
        .isSubClass(
          ContentModel.TYPE_CATEGORY,
          services.getNodeService().getType(categoryNodeRef)
        ) ==
      true
    ) {
  ...
  public Scriptable getRootCategories(String aspect) {
    Object[] cats = buildCategoryNodes(
      services
        .getCategoryService()
        .getRootCategories(storeRef, createQName(aspect))
    );
    return Context.getCurrentContext().newArray(getScope(), cats);
  }
   ...
   * @return
   */
  public Scriptable getRootCategories(
    String aspect,
    String filter,
    int maxItems,
    int skipCount
    ...
    List<ChildAssociationRef> rootCategories = services
      .getCategoryService()
      .getRootCategories(
        storeRef,
        createQName(aspect),
  ...
  public Scriptable getCategoryUsage(String aspect, int maxCount) {
    List<Pair<NodeRef, Integer>> topCats = services
      .getCategoryService()
      .getTopCategories(storeRef, createQName(aspect), maxCount);
    Object[] tags = new Object[topCats.size()];
    int i = 0;
    for (Pair<NodeRef, Integer> topCat : topCats) {
      tags[i++] =
        new Tag(
          new CategoryNode(topCat.getFirst(), this.services, getScope()),
          topCat.getSecond()
        );
    }
  ...
  private Object[] buildCategoryNodes(Collection<ChildAssociationRef> cars) {
    ...
    for (ChildAssociationRef car : cars) {
      categoryNodes[i++] =
        new CategoryNode(car.getChildRef(), this.services, getScope());
    }

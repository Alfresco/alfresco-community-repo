 ...
 *
 * <p>Repository level locking for CIFS, prevents files open via CIFS/FTP/JLAN being interfered with
 * by the alfresco "back end".
 *
 * <p>Delegates ephemeral locking requests to the lockService.
 *
 * @author mrogers
 */
public class LockKeeperImpl implements LockKeeper {
    ...
    public void init() {
        ...
        PropertyCheck.mandatory(
                this, "lockKeeperTransactionalCache", getLockKeeperTransactionalCache());
        ...
        if (lockEnabled) {
            ...
            getLockService()
                    .lock(
                            ...
                            Lifetime.EPHEMERAL,
                            LOCK_KEEPER_KEY);
            lockKeeperTransactionalCache.put(
                    nodeRef, new KeeperInfo(AuthenticationUtil.getFullyAuthenticatedUser()));
        }
    }
                                    ...
                                    if (LOCK_KEEPER_KEY.equalsIgnoreCase(additionalInfo)) {
                                        ...
                                        switch (lockService.getLockStatus(nodeRefToRefresh)) {
                                            case LOCK_OWNER:
                                                if (logger.isDebugEnabled()) {
                                                    logger.debug(
                                                            "refresh ephemeral lock nodeRef: "
                                                                    + nodeRefToRefresh);
                                                }
                                                ...
                                                break;
                                            case LOCKED:
                                                // Locked by somebody else? Something has gone wrong
                                                // here
                                            case LOCK_EXPIRED:
                                            default:
                                                if (logger.isDebugEnabled()) {
                                                    logger.debug(
                                                            "remove lock from lock keeper cache,"
                                                                    + " nodeRef: "
                                                                    + nodeRefToRefresh);
                                                }
                                                lockKeeperTransactionalCache.remove(
                                                        nodeRefToRefresh);
                                        }
                                    } else {
                                        if (logger.isDebugEnabled()) {
                                            logger.debug(
                                                    "not a lock keeper lock, remove lock from lock"
                                                            + " keeper cache, nodeRef: "
                                                            + nodeRefToRefresh);
                                        }

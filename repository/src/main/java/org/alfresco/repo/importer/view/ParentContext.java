...
public class ParentContext extends ElementContext implements ImportParent {
  ...
  public ParentContext(
    ...
    if (typeDef != null) {
      ...
      for (AspectDefinition typeAspect : parent
        .getTypeDefinition()
        .getDefaultAspects()) {
        allAspects.add(typeAspect.getName());
      }
      allAspects.addAll(parent.getNodeAspects());
      TypeDefinition anonymousType = getDictionaryService()
        .getAnonymousType(parent.getTypeDefinition().getName(), allAspects);
      ...
      if (nodeAssociations.containsKey(assocDef.getName()) == false) {
        throw new ImporterException(
          "Association " +
          assocDef.getName() +
          " is not valid for node " +
          parent.getTypeDefinition().getName()
        );
      }
    }
  ...
  public String toString() {
    return (
      "ParentContext[parent=" + parentRef + ",assocType=" + getAssocType() + "]"
    );
  }
}

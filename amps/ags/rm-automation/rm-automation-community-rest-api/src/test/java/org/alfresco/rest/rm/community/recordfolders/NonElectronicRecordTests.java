...
public class NonElectronicRecordTests extends BaseRMRestTest {
    ...
    public void canCreateInValidContainers(String folderId, String type) throws Exception {
        ...
        if (RECORD_FOLDER_TYPE.equalsIgnoreCase(type)) {
            ...
            Record nonElectrinicRecordModel =
                    createFullNonElectronicRecordModel(
                            name,
                            title,
                            description,
                            box,
            ...
            nonElectronicId =
                    recordFolderAPI.createRecord(nonElectrinicRecordModel, folderId).getId();
        } else if (UNFILED_CONTAINER_TYPE.equalsIgnoreCase(type)) {
            // Set values of all available properties for the non electronic records
            UnfiledContainerChild nonElectrinicRecordModel =
                    createFullNonElectronicUnfiledContainerChildRecordModel(
                            name,
                            title,
                            description,
                            box,
            ...
            nonElectronicId =
                    unfiledContainersAPI
                            .createUnfiledContainerChild(nonElectrinicRecordModel, folderId)
                            .getId();
            ...
            nonElectronicId =
                    unfiledRecordFoldersAPI
                            .createUnfiledRecordFolderChild(nonElectrinicRecordModel, folderId)
                            .getId();
        } else {
    ...
    @Test(
            dataProvider = "validRootContainers",
            description =
                    "Non-electronic record can only be created if all mandatory properties are"
                            + " given")
    public void allMandatoryPropertiesRequired(String folderId, String type) {
        ...
        if (type.equals(RECORD_FOLDER_TYPE)) {
            ...
            asList(noNameOrTitle, titleOnly)
                    .forEach(
                            c -> {
                                logger.info(
                                        "Creating non-electronic record with body:\n" + toJson(c));
            ...
            // Component without name and title
            UnfiledContainerChild noNameOrTitle =
                    UnfiledContainerChild.builder().nodeType(NON_ELECTRONIC_RECORD_TYPE).build();

            // Component with title only
            UnfiledContainerChild titleOnly =
                    ...
                    .forEach(
                            c -> {
                                ...
                                getRestAPIFactory()
                                        ...
                                        .createUnfiledContainerChild(c, folderId);
                                // Verify the status code is BAD_REQUEST
                                assertStatusCode(BAD_REQUEST);
                            });
        } else {
                    ...
                    .forEach(
                            c -> {
                                ...
                                getRestAPIFactory()
                                        ...
                                        .createUnfiledRecordFolderChild(c, folderId);
    ...
    @Test(
            dataProvider = "validRootContainers",
            description =
                    "Non-electronic record can't be created if user doesn't have RM privileges")
    public void cantCreateIfNoRmPrivileges(String folderId, String type) {
        ...
        if (type.equals(RECORD_FOLDER_TYPE)) {
            ...
            getRestAPIFactory()
                    ...
                    .createUnfiledContainerChild(recordModel, folderId);
        } else {
            // Try to create a record model
            UnfiledContainerChild recordModel =
                    UnfiledContainerChild.builder()
            ...
            getRestAPIFactory()
                    ...
                    .createUnfiledRecordFolderChild(recordModel, folderId);
        }
    ...
    /**
     * Create user with site manager role and add it to RM site <br>
     * Checks whether the user exists in RM site and creates it if required, with password identical
     * to user name. Note the role is a Core API role, not an RM role. <br>

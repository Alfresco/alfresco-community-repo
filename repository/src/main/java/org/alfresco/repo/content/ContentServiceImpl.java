...
/**
 * Service implementation acting as a level of indirection between the client and the underlying
 * content store.
 *
 * <p>Note: This class was formerly the {@link RoutingContentService} but the 'routing'
 * functionality has been pushed into the {@link AbstractRoutingContentStore store} implementations.
 *
 * @author Derek Hulley
 * @since 3.2
 */
public class ContentServiceImpl implements ContentService, ApplicationContextAware {
    ...
    ClassPolicyDelegate<ContentServicePolicies.OnContentPropertyUpdatePolicy>
            onContentPropertyUpdateDelegate;
    ...
    public void init() {
        ...
        this.tempStore =
                new FileContentStore(
                        this.applicationContext, TempFileProvider.getTempDir().getAbsolutePath());
        ...
        this.onContentUpdateDelegate =
                this.policyComponent.registerClassPolicy(OnContentUpdatePolicy.class);
        this.onContentPropertyUpdateDelegate =
                this.policyComponent.registerClassPolicy(OnContentPropertyUpdatePolicy.class);
        this.onContentReadDelegate =
                this.policyComponent.registerClassPolicy(OnContentReadPolicy.class);
    }
    ...
    public void onUpdateProperties(
        ...
        for (QName propertyQName : after.keySet()) {
            ...
            try {
                ...
                boolean hasContentBefore =
                        ContentData.hasContent(beforeValue)
                                && (!ignoreEmptyContent || beforeValue.getSize() > 0);
                boolean hasContentAfter =
                        ContentData.hasContent(afterValue)
                ...
                propertyPolicy.onContentPropertyUpdate(
                        nodeRef, propertyQName, beforeValue, afterValue);
    ...
    private ContentReader getReader(
            NodeRef nodeRef, QName propertyQName, boolean fireContentReadPolicy) {
    ...
    private ContentData getContentData(NodeRef nodeRef, QName propertyQName) {
        ...
        if (contentData == null) {
            ...
            // if no value or a value other content, and a property definition has been provided,
            // ensure that it's CONTENT or ANY
            if (contentPropDef != null
                    && (!(contentPropDef.getDataType().getName().equals(DataTypeDefinition.CONTENT)
                            || contentPropDef
                                    .getDataType()
                                    .getName()
                                    .equals(DataTypeDefinition.ANY)))) {
    ...
    public ContentWriter getWriter(NodeRef nodeRef, QName propertyQName, boolean update) {
        ...
        ContentContext ctx =
                new NodeContentContext(existingContentReader, null, nodeRef, propertyQName);
        ContentWriter writer = store.getWriter(ctx);
        // Register the new URL for rollback cleanup
        eagerContentStoreCleaner.registerNewContentUrl(writer.getContentUrl());
        ...
        if (update) {
            ...
            WriteStreamListener listener =
                    new WriteStreamListener(nodeService, nodeRef, propertyQName, writer);
    ...
    /**
     * Ensures that, upon closure of the output stream, the node is updated with the latest URL of
        ...
        public void contentStreamClosedImpl() throws ContentIOException {
            try {
                ...
                throw new ContentIOException(
                        "Failed to set content property on stream closure: \n"
                                + "   node: "
                                + nodeRef
                                + "\n"
                                + "   property: "
        ...
        if (systemWideDirectUrlConfig.isEnabled()) {
            ...
            if (contentData == null || contentData.getContentUrl() == null) {
                throw new IllegalArgumentException(
                        "The supplied nodeRef " + nodeRef + " has no content.");
            }

            contentDirectUrlEnabled =
                    (store.isContentDirectUrlEnabled(contentData.getContentUrl()));
        }
    ...
    public DirectAccessUrl requestContentDirectUrl(
            NodeRef nodeRef, boolean attachment, Long validFor) {
        if (!systemWideDirectUrlConfig.isEnabled()) {
        ...
        if (store.isContentDirectUrlEnabled()) {
            try {
                directAccessUrl =
                        store.requestContentDirectUrl(
                                contentUrl, attachment, fileName, contentMimetype, validFor);

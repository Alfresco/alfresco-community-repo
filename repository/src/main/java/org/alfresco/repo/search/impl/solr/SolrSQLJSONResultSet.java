...
public class SolrSQLJSONResultSet
  implements ResultSet, SearchEngineResultMetadata {
  ...
  public SolrSQLJSONResultSet(
    ...
    try {
      ...
      if (obj1.has(SOLR_STREAM_EXCEPTION)) {
        ...
        if (
          error.equalsIgnoreCase("/sql handler only works in Solr Cloud mode")
        ) {
          ...
          throw new RuntimeException(
            "Unable to execute the query, error caused by: " + error
          );
        }
      }

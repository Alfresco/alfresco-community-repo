...
public class BroadcastDispositionActionDefinitionUpdateActionUnitTest {
  ...
  private static final NodeRef DISPOSITION_ACTION_DEF_NODE = new NodeRef(
    "disposition://Action/Def"
  );
  /** The node containing the details of the next disposition step for the content. */
  private static final NodeRef NEXT_ACTION_NODE_REF = new NodeRef(
    "next://Step/"
  );
  /** The node being subject to the disposition step. */
  private static final NodeRef CONTENT_NODE_REF = new NodeRef(
    "content://Node/Ref"
  );
  ...
  private DispositionService mockDispositionService = mock(
    DispositionService.class
  );
  ...
  public void setUp() {
    ...
    when(mockNodeService.getPrimaryParent(NEXT_ACTION_NODE_REF))
      .thenReturn(mockAssocRef);
  ...
  public void testPersistPeriodChanges() {
    ...
    when(
      mockDispositionService.calculateAsOfDate(
        CONTENT_NODE_REF,
    ...
    verify(mockNodeService)
      .setProperty(NEXT_ACTION_NODE_REF, PROP_DISPOSITION_AS_OF, asOfDate);
  }
  ...
  public void testChangePeriodProperty() {
    ...
    NodeRef definitionNode = new NodeRef(
      "definition://node/" + definitionNodeId
    );
    DispositionSchedule mockDispositionSchedule = mock(
    ...
    ChildAssociationRef scheduleDefinitionRelationship = new ChildAssociationRef(
      null,
      scheduleNode,
      null,
      definitionNode
    ...
    ChildAssociationRef categoryScheduleRelationship = new ChildAssociationRef(
      null,
      categoryNode,
      null,
      scheduleNode
    ...
    ChildAssociationRef recordStepRelationship = new ChildAssociationRef(
      null,
      recordNode,
      null,
      stepNode
    );
    when(mockNodeService.getPrimaryParent(stepNode))
      .thenReturn(recordStepRelationship);
    ...
    when(mockDispositionService.getDispositionSchedule(recordNode))
      .thenReturn(mockDispositionSchedule);
    // Set up the record.
    when(mockNodeService.hasAspect(recordNode, ASPECT_DISPOSITION_LIFECYCLE))
      .thenReturn(true);
    ...
    when(mockDispositionService.getNextDispositionAction(recordNode))
      .thenReturn(nextAction);
    DispositionActionDefinition mockActionDefinition = mock(
      DispositionActionDefinition.class
    );
    when(nextAction.getDispositionActionDefinition())
      .thenReturn(mockActionDefinition);
    ...
    when(
      mockDispositionService.calculateAsOfDate(recordNode, mockActionDefinition)
    )
    ...
    verify(mockNodeService)
      .setProperty(stepNode, PROP_DISPOSITION_AS_OF, newAsOfDate);
  }
}

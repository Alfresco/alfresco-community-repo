            ...
            for (int i = 0; i < parameterTypes.length; i++) {
                if (ContentStream.class.isAssignableFrom(parameterTypes[i])) {
                    ...
                    if (contentStream != null) {
                        ...
                        ReusableContentStream reusableContentStream =
                                new ReusableContentStream(contentStream);

                        // ALF-18006
                        if (contentStream.getMimeType() == null) {
                            String mimeType =
                                    mimetypeService.guessMimetype(
                                            reusableContentStream.getFileName(),
                                            new FileContentReader(reusableContentStream.file));
        ...
        public ReusableContentStream(ContentStream contentStream) throws Exception {
            ...
            file =
                    TempFileProvider.createTempFile(
                            contentStream.getStream(), "cmis", "contentStream");
        }
            ...
            if (stream == null && file != null) {
                try {
                    ...
                    throw new AlfrescoRuntimeException(
                            "Expected to be able to reopen temporary file", e);
                }
                setStream(stream);
            }
            return stream;

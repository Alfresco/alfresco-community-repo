...
public class ScriptRatingService extends BaseScopableProcessorExtension {
    ...
    /**
     * This method checks whether self-rating is allowed for the specified rating scheme. If
     * self-rating is allowed in the specified scheme, then the cm:creator of a node can apply a
     * rating, otherwise they cannot.
     *
    ...
    /**
     * Applies the given rating to the specified node using the specified ratingScheme. It is the
     * responsibility of the caller to ensure that the rating scheme exists and that the rating is
     * within the limits defined for that scheme.
     *
     * <p>Furthermore, only one rating scheme per user per target node is supported. Any attempt by
     * one user to apply a second rating in a different scheme will result in a {@link
    ...
    /**
     * Gets the rating applied to the specified node in the specified scheme by the currently
     * authenticated user.
     *
     * @param node ScriptNode
     * @param ratingSchemeName String
     * @return rating if there is one, else -1. TODO -1 could be a valid rating.
     */
    public float getRating(ScriptNode node, String ratingSchemeName) {
        final Rating ratingByCurrentUser =
                ratingService.getRatingByCurrentUser(node.getNodeRef(), ratingSchemeName);
        return ratingByCurrentUser == null ? -1f : ratingByCurrentUser.getScore();
    }

    /**
     * Gets the rating applied date for the specified node in the specified scheme by the currently
     * authenticated user.
     *
     * @param node ScriptNode
     * @param ratingSchemeName String

...
public class DictionaryRepositoryBootstrap extends AbstractLifecycleBean
        implements TenantDeployer,
                DictionaryListener, /*TenantDictionaryListener, */
                MessageDeployer {
    // Logging support
    ...
    private List<RepositoryLocation> repositoryModelsLocations =
            new ArrayList<RepositoryLocation>();

    /** Locations in the repository from which messages should be loaded */
    private List<RepositoryLocation> repositoryMessagesLocations =
            new ArrayList<RepositoryLocation>();
     ...
     *
     * @param repositoryLocations list of the repository messages locations
     */
        ...
        if (onLoadDynamicModelDelegate == null) {
            onLoadDynamicModelDelegate =
                    policyComponent.registerClassPolicy(
                            DynamicModelPolicies.OnLoadDynamicModel.class);
        }
    ...
    private void onDictionaryInitInTxn() {
        if (AlfrescoTransactionSupport.getTransactionReadState() == TxnReadState.TXN_NONE) {
            throw new IllegalStateException(
                    "The Repository-based dictionary initialization has to be done in the context"
                            + " of a transaction.");
        }
        ...
        if (this.repositoryModelsLocations != null) {
            ...
            for (RepositoryLocation repositoryLocation : this.repositoryModelsLocations) {
                ...
                if (repositoryLocation
                        .getQueryLanguage()
                        .equals(RepositoryLocation.LANGUAGE_PATH)) {
                    nodeRefs =
                            getNodes(
                                    storeRef,
                                    repositoryLocation,
                    ...
                    if (nodeRefs.size() > 0) {
                        for (NodeRef dictionaryModel : nodeRefs) {
                            try {
                                // Ignore if the node is a working copy or archived, or if its
                                // inactive
                                if (!(nodeService.hasAspect(
                                                dictionaryModel, ContentModel.ASPECT_WORKING_COPY)
                                        || nodeService.hasAspect(
                                                dictionaryModel, ContentModel.ASPECT_ARCHIVED))) {
                                    Boolean isActive =
                                            (Boolean)
    ...
    public void notifyDynamicModelLoaded(DynamicModelInfo entry) {
        ...
        try {
            ...
            // note: skip with warning - to allow server to start, and hence allow the possibility
            // of fixing the broken model(s)
            // do print the stacktrace so there is some information about what went wrong, in order
            // to fix it
            logger.warn(
                    "Failed to notify dynamic model loaded'"
                            + (entry.model != null ? entry.model.getName() : "null")
                            + "' : "
                            + e,
                    e);
        }
                ...
                if (nodeRefs.size() > 0) {
                    for (NodeRef dictionaryModel : nodeRefs) {
                        try {
                            ...
                            if (logger.isDebugEnabled()) {
                                logger.debug(
                                        "getModelRefs: " + inre + " (assume concurrently deleted)");
                            }
    ...
    protected List<NodeRef> getNodes(
        ...
        if (folderNodeRef != null) {
            ...
            List<ChildAssociationRef> childAssocRefs =
                    nodeService.getChildAssocs(folderNodeRef, types);
    ...
    private void loadModel(
        ...
        if (loadedModels.contains(modelName) == false) {
            for (M2Namespace importNamespace : model.getImports()) {
                ...
                // else we can assume that the imported model is already loaded, if this not the
                // case then
            ...
            try {
                ...
                // note: skip with warning - to allow server to start, and hence allow the
                // possibility of fixing the broken model(s)
                // do print the stacktrace so there is some information about what went wrong, in
                // order to fix it
    ...
    public M2Model createM2Model(NodeRef nodeRef) {
        ...
        ContentReader contentReader =
                this.contentService.getReader(nodeRef, ContentModel.PROP_CONTENT);
    ...
    protected void onBootstrap(ApplicationEvent event) {
        // Reset the dictionary (destroy and reload) in order to ensure that we have a basic version
        // of
        ...
        // However, the listeners will be needing to access the dictionary themselves, hence the
        // earlier 'reset'

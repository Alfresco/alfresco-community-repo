...
public class TransferServiceImpl2 implements TransferService2 {
    ...
    private Map<String, TransferStatus> transferMonitoring =
            Collections.synchronizedMap(new TreeMap<String, TransferStatus>());
    ...
    private TransferTarget createTransferTarget(TransferTarget newTarget) {
        ...
        ChildAssociationRef ref =
                nodeService.createNode(
                        ...
                        QName.createQName(
                                TransferModel.TRANSFER_MODEL_1_0_URI, newTarget.getName()),
    ...
    public Set<TransferTarget> getTransferTargets(String groupName) {
        ...
        NodeRef groupNode =
                nodeService.getChildByName(home, ContentModel.ASSOC_CONTAINS, groupName);
    ...
    private Set<TransferTarget> getTransferTargets(NodeRef groupNode) {
        ...
        List<ChildAssociationRef> children =
                nodeService.getChildAssocs(
                        groupNode, ContentModel.ASSOC_CONTAINS, RegexQNamePattern.MATCH_ALL);
    ...
    public void transferAsync(
        ...
        /**
         * Execute transfer async in its own transaction. The action service only runs actions in
         * the post commit which is why there's a separate transaction here.
         */
    ...
    private TransferEndEvent transferImpl(
        ...
        TransferCallback reportCallback =
            ...
            try {
                    ...
                    case Prepare:
                        {
                            ...
                            if (!this.transferVersionChecker.checkTransferVersions(
                                    fromVersion, toVersion)) {
                                throw new TransferException(
                                        MSG_INCOMPATIBLE_VERSIONS,
                                        new Object[] {
                                            transfer.getTransferId(), fromVersion, toVersion
                                        });
                            }
                    ...
                    case Poll:
                        {
                            ...
                            if (progress.getStatus() == TransferProgress.Status.ERROR) {
                                ...
                                // NOTE: it's possible the error is not returned from pre v3.4
                                // target repositories
                                if (targetError == null) {
                    ...
                    case Finished:
                        {
                            try {
                                ...
                                try {
                                    ...
                                    // manually add the terminal event to the transfer report event
                                    // list
                                    ...
                                    reportName = "error";
                                    logger.warn(
                                            "Exception - unable to notify end transfer state", e);
                                }
                ...
                if (transfer != null
                        && (clientState == ClientTransferState.Begin
                                || clientState == ClientTransferState.Prepare
                                || clientState == ClientTransferState.Commit)) {
                    // we must first inform the target repository that a client failure has occurred
                    // to allow it to
    ...
    private File createManifest(
        ...
        // If a noderef exists in both the "nodes" set and the "nodesToRemove" set then the
        // nodesToRemove wins. It is removed
        // from the "nodes" set...
        ...
        if (nodes != null) {
            for (NodeRef nodeRef : nodes) {
                TransferManifestNode node =
                        transferManifestNodeFactory.createTransferManifestNode(
                                nodeRef, definition, transferContext);
        ...
        if (nodesToRemove != null) {
            for (NodeRef nodeRef : nodesToRemove) {
                TransferManifestNode node =
                        transferManifestNodeFactory.createTransferManifestNode(
                                nodeRef, definition, transferContext, true);
                formatter.writeTransferManifestNode(node);
            }
        }
    ...
    private void sendContent(
         ...
         *
         * <p>ManifestFile -> Manifest Processor -> Chunker -> Transmitter
         *
         * <p>Step 1: Create a chunker and wire it up to the transmitter
         */
        final ContentChunker chunker = new ContentChunkerImpl();
        final Long removeNodesRange =
                Long.valueOf(
                        definition.getNodesToRemove() != null
                                ? definition.getNodesToRemove().size()
                                : 0);
        final Long nodesRange =
                Long.valueOf(definition.getNodes() != null ? definition.getNodes().size() : 0);
    ...
    private NodeRef persistTransferReport(
        ...
        RetryingTransactionCallback<NodeRef> writeReportCallback =
    ...
    private NodeRef persistDestinationTransferReport(
            final String transferName, final Transfer transfer, final TransferTarget target) {
        ...
        RetryingTransactionCallback<NodeRef> writeReportCallback =
                new RetryingTransactionCallback<NodeRef>() {
                    ...
                    public NodeRef execute() throws Throwable {
                        File tempDir = TempFileProvider.getLongLifeTempDir(FILE_DIRECTORY);
                        File destReportFile =
                                TempFileProvider.createTempFile("TRX-DREP", FILE_SUFFIX, tempDir);
                        ...
                        NodeRef reportNode =
                                transferReporter.writeDestinationReport(
                                        transferName, target, destReportFile);

...
public class FilesFoldersConstraint extends VirtualQueryConstraintDecorator {
    ...
    public FilesFoldersConstraint(
            VirtualQueryConstraint decoratedConstraint, boolean files, boolean folders) {
    ...
    private String filter(String language, String query, boolean files, boolean folders)
        ...
        if (files ^ folders) {
            if (SearchService.LANGUAGE_FTS_ALFRESCO.equals(language)) {
                ...
                throw new VirtualizationException(
                        "Disjunctive file-folder filters are only supported on "

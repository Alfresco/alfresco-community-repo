...
public class NodeServiceXPath extends BaseXPath {
    ...
    public NodeServiceXPath(
            String xpath, DocumentNavigator documentNavigator, QueryParameterDefinition[] paramDefs)
            throws JaxenException {
        ...
        if (logger.isDebugEnabled()) {
            ...
            for (int i = 0; paramDefs != null && i < paramDefs.length; i++) {
                sb.append("      Parameter: \n")
                        .append("         name: ")
                        .append(paramDefs[i].getQName())
                        .append("\n")
                        .append("         value: ")
                        .append(paramDefs[i].getDefault())
                        .append("\n");
            }
        ...
        for (String prefix : documentNavigator.getNamespacePrefixResolver().getPrefixes()) {
            addNamespace(
                    prefix, documentNavigator.getNamespacePrefixResolver().getNamespaceURI(prefix));
        }
    }
     ...
     *
     * <p>So, duplicates are eliminated here before the results are returned.
     */
    ...
    static class SubTypeOf implements Function {
        public Object call(Context context, List args) throws FunctionCallException {
            if (args.size() != 1) {
                throw new FunctionCallException(
                        "subtypeOf() requires one argument: subtypeOf(QName typeQName)");
            }
        ...
        public Object evaluate(List nodes, Object qnameObj, Navigator nav) {
            ...
            if (qnameStr.startsWith("{")) {
                ...
                typeQName =
                        QName.createQName(
                                qnameStr, ((DocumentNavigator) nav).getNamespacePrefixResolver());
            }
            // resolve the noderef
    ...
    static class HasAspect implements Function {
        public Object call(Context context, List args) throws FunctionCallException {
            if (args.size() != 1) {
                throw new FunctionCallException(
                        "hasAspect() requires one argument: hasAspect(QName typeQName)");
            }
            return evaluate(context.getNodeSet(), args.get(0), context.getNavigator());
        }
            ...
            if ((attributeValue != null) && (attributeValue.length() > 0)) {
                ...
                if (patternValue.equals("*")) {
                    ...
                    QNamePattern qNamePattern =
                            new JCRPatternMatch(patternValue, dNav.getNamespacePrefixResolver());
    ...
    /**
     * A boolean function to determine if a node property matches a pattern and/or the node text
     * matches the pattern.
     *
     * <p>The default is JSR170 compliant. The optional boolean allows searching only against the
     * property value itself.
     *
     * <p>The search is always case-insensitive.
     *
     * @author Derek Hulley
     */
    static class Like implements Function {
        ...
        public Object evaluate(
                List nodes, Object obj, Object patternObj, Object includeFtsObj, Navigator nav) {
            ...
            QName qname =
                    QName.createQName(
                            nav.getAttributeNamespaceUri(attribute),
                            ISO9075.decode(nav.getAttributeName(attribute)));
    ...
    public static class XPathFunctionContext extends SimpleFunctionContext {
         ...
         * Construct.
         *
         * <p>Construct with all core XPath functions registered.
         */

...
public abstract class DictionaryWebServiceBase extends DeclarativeWebScript {
    ...
    public String getFullNamespaceURI(String classname) {
        try {
            ...
            throw new WebScriptException(
                    ...
                    "The exact classname - "
                            + classname
                            + "  parameter has not been provided in the URL");
        }
    }
    ...
    /**
     * @param classname - checks whether the classname is valid , gets the classname as input e.g
     *     cm_person
        ...
        for (QName qnameObj : this.dictionaryservice.getAllModels()) {
            if (qnameObj.getLocalName().equals(modelname)) {
                Collection<String> prefixes =
                        this.namespaceService.getPrefixes(qnameObj.getNamespaceURI());
                if (!prefixes.isEmpty()) {
    ...
    /**
     * @param modelname - gets the modelname as the input (modelname is without prefix ie.
     *     cm:contentmodel => where modelname = contentmodel)
    ...
    /**
     * @param classname - returns the prefix from the classname of the format namespaceprefix:name
     *     eg. cm_person
     ...
     * @return the shortname from the classname of the format cm_person here person represents the
     *     shortname
     */
    ...
    /**
     * Returns dependent collections (properties or associations) in order that complies to order of
     * class definitions
     *
     ...
     */
    protected <T> Collection<T> reorderedValues(
            List<ClassDefinition> sortedClassDefs, Map<QName, T> dependent) {

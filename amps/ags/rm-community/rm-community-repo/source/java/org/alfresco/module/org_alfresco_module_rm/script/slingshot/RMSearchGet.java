...
public class RMSearchGet extends DeclarativeWebScript {
  ...
  public void setClassificationReasonsUtil(
    ClassificationReasonsUtil classificationReasonsUtil
  ) {
  ...
  protected Map<String, Object> executeImpl(
    ...
    try {
      ...
      // Execute search
      List<Pair<NodeRef, NodeRef>> results = recordsManagementSearchService.search(
        siteId,
        query,
        searchParameters
      );
      ...
      for (Pair<NodeRef, NodeRef> pair : results) {
        ...
        try {
          ...
          LOGGER.debug(
            "Ignoring failed attempt to add item to search results.",
            e
          );
        }
      }
  ...
  public class Item {
    ...
    public Item(NodeRef parent, NodeRef nodeRef) {
      ...
      isContainer = true;
      if (
        dictionaryService.isSubClass(nodeRefType, ContentModel.TYPE_CONTENT)
      ) {
        isContainer = false;
      }
      ...
      if (isContainer) {
        ...
        String displayPath = nodeService
          .getPath(nodeRef)
      ...
      for (Map.Entry<QName, Serializable> entry : nodeProperties.entrySet()) {
        ...
        if (
          !NamespaceService.SYSTEM_MODEL_1_0_URI.equals(qName.getNamespaceURI())
        ) {
      ...
      properties.put(
        ...
        recordCategoryUtil.getCategoryIdFromNodeId(nodeRef, false)
      );
    }

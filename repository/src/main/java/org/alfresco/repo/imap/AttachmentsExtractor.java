...
public class AttachmentsExtractor {
                ...
                AuthenticationUtil.runAs(
                        new AuthenticationUtil.RunAsWork<NodeRef>() {
                            public NodeRef doWork() throws Exception {
                                ...
                                RetryingTransactionCallback<NodeRef> getDescriptorCallback =
                                        new RetryingTransactionCallback<NodeRef>() {
                                            public NodeRef execute() {
                                                NodeRef attFolderRef =
                                                        attachmentsFolder.getOrCreateFolderPath(
                                                                serviceRegistry
                                                                        .getNamespaceService(),
                                                                nodeService,
                                                                serviceRegistry.getSearchService(),
                                                                fileFolderService);
                                                if (attachmentsExtractorMode != null
    ...
    public void extractAttachments(NodeRef messageRef, MimeMessage originalMessage)
        ...
        switch (attachmentsExtractorMode) {
            ...
            default:
                String messageName =
                        (String) nodeService.getProperty(messageRef, ContentModel.PROP_NAME);
        ...
        if (!createFolder) {
            nodeService.createAssociation(
                    messageRef, attachmentsFolderRef, ImapModel.ASSOC_IMAP_ATTACHMENTS_FOLDER);
        }
        ...
        if (content instanceof Multipart) {
            ...
            for (int i = 0, n = multipart.getCount(); i < n; i++) {
                ...
                if ("attachment".equalsIgnoreCase(part.getDisposition())) {
                    if (createFolder) {
                        attachmentsFolderRef =
                                createAttachmentFolder(messageRef, attachmentsFolderName);
                        createFolder = false;
                    }
    ...
    private NodeRef createAttachmentFolder(NodeRef messageRef, String attachmentsFolderName) {
        ...
        attachmentsFolderRef =
                fileFolderService
                        .create(parentFolder, attachmentsFolderName, ContentModel.TYPE_FOLDER)
                        .getNodeRef();
        nodeService.createAssociation(
                messageRef, attachmentsFolderRef, ImapModel.ASSOC_IMAP_ATTACHMENTS_FOLDER);
        ...
        if (contentType.getBaseType().equalsIgnoreCase("application/ms-tnef")) {
            ...
            for (org.apache.poi.hmef.Attachment attachment : attachments) {
                ...
                NodeRef attachmentNode =
                        fileFolderService.searchSimple(attachmentsFolderRef, subName);
                if (attachmentNode == null) {
                    ...
                    FileInfo createdFile =
                            fileFolderService.create(
                                    attachmentsFolderRef, subName, ContentModel.TYPE_CONTENT);
                    ...
                    serviceRegistry
                            .getNodeService()
                            .createAssociation(
                                    messageFile, attachmentNode, ImapModel.ASSOC_IMAP_ATTACHMENT);
             ...
             */
            // And another one behaviour which will overwrite the content of the existing file. It
            // is performance preferable.
            if (attachmentFile == null) {
                FileInfo createdFile =
                        fileFolderService.create(
                                attachmentsFolderRef, fileName, ContentModel.TYPE_CONTENT);
                nodeService.createAssociation(
                        messageFile, createdFile.getNodeRef(), ImapModel.ASSOC_IMAP_ATTACHMENT);
                attachmentFile = createdFile.getNodeRef();
            } else {

                String newFileName =
                        imapService.generateUniqueFilename(attachmentsFolderRef, fileName);

                FileInfo createdFile =
                        fileFolderService.create(
                                attachmentsFolderRef, newFileName, ContentModel.TYPE_CONTENT);
                nodeService.createAssociation(
                        messageFile, createdFile.getNodeRef(), ImapModel.ASSOC_IMAP_ATTACHMENT);
                attachmentFile = createdFile.getNodeRef();
            }

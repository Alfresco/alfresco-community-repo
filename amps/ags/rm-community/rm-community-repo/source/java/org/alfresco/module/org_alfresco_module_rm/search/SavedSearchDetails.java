 ...
 *
 * <p>Example format of posted Saved Search JSON:
 *
 * <p>{ "siteid" : "rm", "name": "search name", "description": "the search description", "search":
 * "the search sting as entered by the user", "public": boolean, "searchparams" : { "maxItems" :
 * 500, "records" : true, "undeclaredrecords" : false, "vitalrecords" : false, "recordfolders" :
 * false, "frozen" : false, "cutoff" : false, "containertypes" : [ "rma:recordSeries",
 * "rma:recordCategory" ] "sort" : [ { "field" : "cm:name", "ascending" : true } ] } }
 *
 * <p>where: name and query values are mandatory, searchparams contains the filters, sort, etc
 * information about the query query is there for backward compatibility note: "params":
 * "terms=keywords:xyz&amp;undeclared=true", "sort": "cm:name/asc" "query": "the complete search
 * query string", ... are sometimes found in the place of searchparams and are migrated to the new
 * format when re-saved params are in URL encoded name/value pair format sort is in comma separated
 * "property/dir" packed format i.e. "cm:name/asc,cm:title/desc"
 *
 * @author Roy Wetherall
 */
// Not @AlfrescoPublicApi at the moment as it requires RecordsManagementSearchServiceImpl which is
// not public API.
public class SavedSearchDetails extends ReportDetails {
        ...
        try {
            ...
            if (!search.has(NAME)) {
                throw new AlfrescoRuntimeException(
                        "Can not create saved search details from json, because required name is"
                                + " not present. "
                                + jsonString);
            }
            ...
            if (!search.has(SEARCH)) {
                ...
                if (search.has(PARAMS)) {
                    ...
                    throw new AlfrescoRuntimeException(
                            "Can not create saved search details from json, because required search"
                                    + " is not present. "
                                    + jsonString);
                }
            ...
            if (search.has(SEARCHPARAMS)) {
                searchParameters =
                        RecordsManagementSearchParameters.createFromJSON(
                                search.getJSONObject(SEARCHPARAMS), namespaceService);
            } else {
                ...
                if (search.has(PARAMS)) {
                    ...
                    searchParameters =
                            SavedSearchDetailsCompatibility.createSearchParameters(
                                    oldParams, oldSort, namespaceService);
                }
            }
            ...
            throw new AlfrescoRuntimeException(
                    "Can not create saved search details from json. " + jsonString, exception);
        }
    }
    ...
    /*package*/ SavedSearchDetails(
        ...
        this.compatibility =
                new SavedSearchDetailsCompatibility(this, namespaceService, searchService);
    ...
    public String toJSONString() {
        try {
            ...
            throw new AlfrescoRuntimeException(
                    "Can not convert saved search details into JSON.", exception);
        }
    }
}

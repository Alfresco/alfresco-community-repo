...
public class PersonServiceLoader {
    ...
    public static void main(String[] args) {
        ...
        sb.append("\n")
                .append("Usage\n")
                .append(
                        "   PersonServiceLoader  --user=<username> --pwd=<password>"
                                + " --batch-count=<batch-count> --batch-size=<batch-size>"
                                + " --threads=<threads>\n");
    ...
    static class Nester extends Thread {
        ...
        public void run() {
            ...
            try {
                RetryingTransactionCallback<Integer> makeUsersCallback =
                        new RetryingTransactionCallback<Integer>() {
                            public Integer execute() throws Throwable {
                                for (int i = 0; i < batchSize; i++) {
                                    ...
                                    String emailAddress =
                                            String.format("%s.%s@xyz.com", firstName, lastName);
                                    PropertyMap properties = new PropertyMap(7);
                                    properties.put(ContentModel.PROP_USERNAME, username);
                                    properties.put(ContentModel.PROP_FIRSTNAME, firstName);
                                    properties.put(ContentModel.PROP_LASTNAME, lastName);

...
public class WorkflowInterpreter extends BaseInterpreter {
        ...
        if (command[0].equals("help")) {
            ...
            if (command.length < 2) {
                ...
                if (id == null && command.length == 2) {
                    return "workflow definition not in use.  Enter command 'show workflows all' or"
                            + " 'use <workflowDefId>'.\n";
                }
                if (command.length == 3) {
                    if (command[2].equals("all")) {
                ...
                if ("all".equals(id)) {
                    for (WorkflowDefinition def : workflowService.getAllDefinitions()) {
                        List<WorkflowInstance> workflows =
                                workflowService.getActiveWorkflows(def.getId());
                ...
                for (WorkflowPath path : paths) {
                    ...
                    List<WorkflowTask> tasks =
                            workflowService.getTasksForWorkflowPath(path.getId());
                    for (WorkflowTask task : tasks) {
                        out.println(
                                " task id: "
                                        + task.getId()
                ...
                String id = (currentWorkflowDef != null) ? currentWorkflowDef.getId() : null;
                if (id == null && command.length == 2) {
                    return "workflow definition not in use.  Enter command 'show timers all' or"
                            + " 'use <workflowDefId>'.\n";
                }
                if (command.length == 3) {
                    if (command[2].equals("all")) {
                ...
                if (command[2].equals("tasks")) {
                    ...
                    List<WorkflowTask> tasks =
                            workflowService.getAssignedTasks(
                    ...
                    List<WorkflowTask> tasks =
                            workflowService.getAssignedTasks(
                                    AuthenticationUtil.getRunAsUser(), WorkflowTaskState.COMPLETED);
                    for (WorkflowTask task : tasks) {
                        out.println(
                                "id: "
                                        + task.getId()
                    ...
                    out.println(AuthenticationUtil.getRunAsUser() + ":");
                    List<WorkflowTask> tasks =
                            workflowService.getPooledTasks(AuthenticationUtil.getRunAsUser());
                    for (WorkflowTask task : tasks) {
                        out.println(
                                "id: "
                                        + task.getId()
            ...
            if (command[1].equals("task")) {
                ...
                out.println(
                        "transitions: " + task.getDefinition().getNode().getTransitions().length);
                ...
                for (int i = 2; i < command.length; i++) {
                    ...
                    if (predicateName.length == 1) {
                        if (predicate[0].equals("taskId")) {
                            ...
                            WorkflowTaskQuery.OrderBy[] queryOrderBy =
                                    new WorkflowTaskQuery.OrderBy[orderBy.length];
            ...
            WorkflowDeployment deployment =
                    workflowService.deployDefinition(
                            command[1], workflowDef.getInputStream(), MimetypeMap.MIMETYPE_XML);
            ...
            out.print(
                    executeCommand("deploy " + currentDeployEngine + " " + currentDeployResource));
                ...
                if (command.length == 3) {
                    ...
                    if (command[2].equals("name")) {
                        ...
                        List<WorkflowDefinition> defs =
                                workflowService.getAllDefinitionsByName(command[3]);
                        for (WorkflowDefinition def : defs) {
            ...
            if (command[1].equals("task")) {
                WorkflowTask task =
                        workflowService.endTask(
                                command[2], (command.length == 4) ? command[3] : null);
            ...
            if (command[1].equals("workflow")) {
                ...
                if (command[2].equals("workflows")) {
                    if (command.length < 4) {
                        return "Enter the command 'delete all workflows imeanit' to really delete"
                                + " all workflows\n";
                    }
                    ...
                    for (int i = 0; i < number; i++) {
                        FileInfo fileInfo =
                                fileFolderService.create(
                                        packageNodeRef, "Content" + i, ContentModel.TYPE_CONTENT);
    ...
    private void setupStartTaskParameters(TypeDefinition typeDef, Map<QName, Serializable> params) {
        ...
        ClassDefinition startTaskDef =
                dictionaryService.getAnonymousType(typeDef.getName(), aspectNames);
                    ...
                    params.put(
                            ...
                            (Serializable)
                                    DefaultTypeConverter.INSTANCE.convert(
                                            entry.getValue().getDataType(), defaultValue));
                }
            } else {
                params.put(
                        entry.getKey(),

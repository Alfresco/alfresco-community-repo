...
public class SearchDAOImpl implements SearchDAO {
  ...
  private static final String SELECT_CHANGESETS_SUMMARY =
    "alfresco.solr.select_ChangeSets_Summary";
  private static final String SELECT_ACLS_BY_CHANGESET_IDS =
    "alfresco.solr.select_AclsByChangeSetIds";
  ...
  public List<AclChangeSet> getAclChangeSets(
    ...
    if (maxResults <= 0 || maxResults == Integer.MAX_VALUE) {
      throw new IllegalArgumentException(
        "Maximum results must be a reasonable number."
      );
    }

    // We simulate an ID for the sys:deleted type
    Pair<Long, QName> deletedTypeQNamePair = qnameDAO.getQName(
      ContentModel.TYPE_DELETED
    );
    Long deletedTypeQNameId = deletedTypeQNamePair == null
      ? -1L
      : deletedTypeQNamePair.getFirst();
  ...
  public List<Acl> getAcls(
    ...
    if (aclChangeSetIds.size() > 512) {
      throw new IllegalArgumentException(
        "'aclChangeSetIds' cannot have more than 512 entries."
      );
    }
  ...
  public List<Node> getNodes(
    ...
    QName shardPropertyQName,
    QName shardPropertyTypeName
  ) {
    NodeParametersEntity params = new NodeParametersEntity(
      nodeParameters,
      qnameDAO
    );
    ...
    if (shardPropertyQName != null && shardPropertyTypeName != null) {
      if (shardPropertyQName.equals(ContentModel.PROP_CREATED)) {
        ...
        Pair<Long, QName> propertyQNamePair = qnameDAO.getQName(
          shardPropertyQName
        );
    ...
    if (
      nodeParameters.getMaxResults() != 0 &&
      nodeParameters.getMaxResults() != Integer.MAX_VALUE
    ) {
      return template.selectList(
        SELECT_NODES,
        params,

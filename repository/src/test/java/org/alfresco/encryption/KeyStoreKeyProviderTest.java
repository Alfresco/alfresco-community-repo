...
public class KeyStoreKeyProviderTest extends TestCase {
  ...
  private static final String FILE_ONE =
    "classpath:alfresco/keystore-tests/ks-test-1.jks";
  private static final String FILE_TWO =
    "classpath:alfresco/keystore-tests/ks-test-2.jks";
  private static final String FILE_THREE =
    "classpath:alfresco/keystore-tests/ks-test-3.jks";
  ...
  /* package */static KeystoreKeyProvider getTestKeyStoreProvider() {
    ...
    KeyStoreParameters encryptionParameters = new KeyStoreParameters(
      "test",
      "JCEKS",
      "SunJCE",
      null,
  ...
  public void testNoKeyStorePasswords() throws Exception {
    KeystoreKeyProvider keyProvider = getTestKeyStoreProvider(
      FILE_ONE,
  ...
  public void testKeyStoreWithOnlyAliasPasswords() throws Exception {
    ...
    // This has succeeded because we have not attempted to access it
    assertNotNull(
      "Should be able to key alias with same password",
      keyProvider.getKey(ALIAS_ONE)
    );
  }

  public void testAliasWithIncorrectPassword_One() throws Exception {
    try {
      getTestKeyStoreProvider(
        FILE_ONE,
  ...
  public void testAliasWithCorrectPassword_One() throws Exception {
    KeystoreKeyProvider ks = getTestKeyStoreProvider(
      FILE_ONE,
      Collections.singletonMap(ALIAS_ONE, "aliasPwd1")
    );

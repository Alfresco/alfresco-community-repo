...
public class RepoPrimaryManifestProcessorImpl
  ...
  private static final Log log = LogFactory.getLog(
    RepoPrimaryManifestProcessorImpl.class
  );

  private static final String MSG_NO_PRIMARY_PARENT_SUPPLIED =
    "transfer_service.receiver.no_primary_parent_supplied";
  private static final String MSG_ORPHANS_EXIST =
    "transfer_service.receiver.orphans_exist";
   ...
   */
  private Map<NodeRef, List<ChildAssociationRef>> orphans = new HashMap<NodeRef, List<ChildAssociationRef>>(
    89
  );
   ...
   */
  public RepoPrimaryManifestProcessorImpl(
  ...
  protected void processNode(TransferManifestDeletedNode node) {
    ...
    logComment(
      "Primary Processing incoming deleted node: " + node.getNodeRef()
    );

    ChildAssociationRef origPrimaryParent = node.getPrimaryParentAssoc();
    NodeRef origNodeRef = new NodeRef(
      origPrimaryParent.getParentRef().getStoreRef(),
    ...
    if (resolvedNodes.resolvedChild != null) {
      ...
      logDeleted(
        node.getNodeRef(),
        exNode,
        nodeService.getPath(exNode).toString()
      );
      logSummaryDeleted(
        node.getNodeRef(),
        exNode,
  ...
  protected void processNode(TransferManifestNormalNode node) {
    if (log.isDebugEnabled()) {
      log.debug(
        "Processing node with incoming noderef of " + node.getNodeRef()
      );
    }
    logComment(
  ...
  private void create(
    ...
    if (parentNodeRef == null) {
      ...
      // later processing
      ChildAssociationRef tempLocation = getTemporaryLocation(
        node.getNodeRef()
      );
    ...
    Map<QName, Serializable> contentProps = processProperties(
      null,
      props,
      null
    );
    ...
    ChildAssociationRef newNode = nodeService.createNode(
      parentNodeRef,
      parentAssocType,
      parentAssocName,
      node.getType(),
     ...
     */
    if (
      nodeService.hasAspect(parentNodeRef, TransferModel.ASPECT_TRANSFERRED) ||
  ...
  protected void delete(
    TransferManifestDeletedNode node,
    NodeRef nodeToDelete
  ) {
    if (alienProcessor.isAlien(nodeToDelete)) {
      logComment(
        "Node contains alien content and can't be deleted: " + nodeToDelete
      );
      if (log.isDebugEnabled()) {
        log.debug(
          "Node to be deleted is alien prune rather than delete: " +
          nodeToDelete
        );
      }
       ...
       */
      if (
        nodeService.hasAspect(nodeToDelete, TransferModel.ASPECT_TRANSFERRED)
      ) {
        String fromRepository = (String) nodeService.getProperty(
          nodeToDelete,
      ...
      logDeleted(
        node.getNodeRef(),
        nodeToDelete,
        nodeService.getPath(nodeToDelete).toString()
      );
      logSummaryDeleted(
        node.getNodeRef(),
        nodeToDelete,
  ...
  private void checkOrphans(NodeRef parentNode) {
    ...
    if (orphansToClaim != null) {
      ...
      for (ChildAssociationRef orphan : orphansToClaim) {
        ...
        ChildAssociationRef newRef = nodeService.moveNode(
          orphan.getChildRef(),
          orphan.getParentRef(),
          orphan.getTypeQName(),
          orphan.getQName()
  ...
  private void update(
    ...
    ChildAssociationRef currentParent = nodeService.getPrimaryParent(
      nodeToUpdate
    );
    if (
      !currentParent.getParentRef().equals(parentNodeRef) ||
      ...
      ChildAssociationRef newNode = nodeService.moveNode(
        nodeToUpdate,
        parentNodeRef,
        parentAssocType,
        parentAssocName
       ...
       */
      if (
        nodeService.hasAspect(newNode.getChildRef(), TransferModel.ASPECT_ALIEN)
      ) {
    ...
    if (updateNeeded(node, nodeToUpdate)) {
      ...
      List<AspectDefinition> aspectDefs = dictionaryService
      ...
      ManifestAccessControl acl = node.getAccessControl();
      if (acl != null) {
        boolean existInherit = permissionService.getInheritParentPermissions(
          nodeToUpdate
        );
        ...
        Set<AccessPermission> existingPermissions = permissionService.getAllSetPermissions(
          nodeToUpdate
        );
          ...
          if (newPermissions != null) {
            ...
            for (ManifestPermission permission : acl.getPermissions()) {
              if (!work.contains(permission)) {
                ...
                AccessStatus status = AccessStatus.valueOf(
  ...
  private Map<QName, Serializable> processProperties(
    ...
    for (Map.Entry<QName, Serializable> propEntry : props.entrySet()) {
      ...
      if (
        (value != null) && ContentData.class.isAssignableFrom(value.getClass())
      ) {
        if (existingProps != null) {
          ...
          if (stagedFile.exists()) {
            if (log.isDebugEnabled()) {
              log.debug(
                "replace content for node:" + nodeToUpdate + ", " + key
              );
            }
            ...
            if (props.containsKey(key)) {
              if (log.isDebugEnabled()) {
                log.debug(
                  "keep existing content for node:" + nodeToUpdate + ", " + key
                );
              }
  ...
  private void storeOrphanNode(ChildAssociationRef primaryParentAssoc) {
    List<ChildAssociationRef> orphansOfParent = orphans.get(
      primaryParentAssoc.getParentRef()
    );
  ...
  private void processCategories(
    ...
    if (manifestCategories != null) {
      for (Map.Entry<QName, Serializable> val : properties.entrySet()) {
        ...
        if (def != null) {
          if (
            def.getDataType().getName().isMatch(DataTypeDefinition.CATEGORY)
          ) {
              ...
              if (def.isMultiValued()) {
                if (thing instanceof java.util.Collection) {
                  ...
                  for (NodeRef sourceCategoryNodeRef : c) {
                    if (log.isDebugEnabled()) {
                      log.debug(
                        "sourceCategoryNodeRef" + sourceCategoryNodeRef
                      );
                    }
                    // substitute target node ref fot source node ref
                    NodeRef targetNodeRef = processCategory(
  ...
  private NodeRef processCategory(
    ...
    if (category != null) {
      ...
      final QName aspectQName = QName.createQName(
        aspectName.getElementString()
      );
      ...
      if (aspectQName.equals(ContentModel.ASPECT_TAGGABLE)) {
        ...
        RunAsWork<NodeRef> processCategory = new RunAsWork<NodeRef>() {
          ...
          public NodeRef doWork() throws Exception {
            QName rootCatName = QName.createQName(
              catPath.get(3).getElementString()
            );

            Collection<ChildAssociationRef> roots = categoryService.getRootCategories(
              sourceCategoryNodeRef.getStoreRef(),
              aspectQName
            );
            ...
            for (int i = 4; i < catPath.size(); i++) {
              ...
              ChildAssociationRef child = categoryService.getCategory(
                workingNodeRef,
                aspectQName,
  ...
  private void injectTransferred(Map<QName, Serializable> props) {
    ...
    for (Serializable value : props.values()) {
      if (
        ...
        if (
          srcContent.getContentUrl() != null &&
          !srcContent.getContentUrl().isEmpty()
        ) {

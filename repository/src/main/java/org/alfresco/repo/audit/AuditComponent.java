...
public interface AuditComponent {
    ...
    /**
     * Determine whether the audit infrastructure expects audit values to be passed in. This is a
     * helper method to allow optimizations in the client code. Reasons why this method might return
     * <tt>false</tt> are: auditing is disabled; no audit applications have been registered.
     * Sometimes, depending on the log level, this method may always return <tt>true</tt>.
     *
     * @return Returns <code>true</code> if the calling code (data producers) should go ahead and
     *     generate the data for {@link #recordAuditValues(String, Map) recording}.
     * @since 3.3
     */
     ...
     * @return Returns <tt>true</tt> if there is at least one audit application registered to handle
     *     the given path.
     * @since 3.4
     */
    ...
    /**
     * Check if an audit path is enabled. The path will be disabled if it or any higher path has
     * been explicitly disabled. Any disabled path will not be processed when data is audited.
     *
     * @param applicationName the name of the application being logged to
     * @param path the audit path to check or <tt>null</tt> to assume the application's root path
    ...
    /**
     * Enable auditing (if it is not already enabled) for all paths that contain the given path. The
     * path is the path as originally logged and not the path that the generated data may contain -
     * although this would be similarly enabled.
     *
     * <p>If the enabled
     *
     * @param applicationName the name of the application being logged to
     ...
     * The path is the path as originally logged and not the path that the generated data may
     * contain - although this would be similarly disabled.
     *
     * <p>If the path is <b>/x/y</b> then any data paths that start with <b>/x/y</b> will be
     * stripped out <u>before</u> data generators and data recorders are applied. If the path
     * represents the root path of the application, then auditing for that application is
     ...
     * disabled globally. This is primarily for test purposes; applications should know which paths
     * need {@link #enableAudit(String, String) enabling} or {@link #disableAudit(String, String)
     * disabled}.
     *
     * @param applicationName the name of the application
     * @since 3.2
     ...
     *
     * <p>The root path and value keys are combined to produce a map of data keyed by full path.
     * This fully-pathed map is then passed through the {@link
     * AuditModelRegistry#getAuditPathMapper() audit path mapper}. The result may yield data
     * destined for several different {@link AuditModelRegistry#getAuditApplicationByKey(String)
     * audit applications}. depending on the data extraction and generation defined in the
     * applications, values (or derived values) may be recorded against several audit entries (one
     * per application represented).
     *
     * <p>The return values reflect what was actually persisted and is controlled by the data
     * extractors defined in the audit configuration.
     *
     * <p>A new read-write transaction is started if there are values to write that there is not a
     * viable transaction present.
     *
     * @param rootPath a base path of {@link AuditPath} key entries concatenated with the path
     *     separator '/' ({@link AuditApplication#AUDIT_PATH_SEPARATOR})
     * @param values the values to audit mapped by {@link AuditPath} key relative to root path (may
     *     be <tt>null</tt>)
    ...
    /**
     * The same as {@link AuditComponent#recordAuditValues(String, Map)}, but with controlled usage
     * of userFilter
     *
     * @param rootPath a base path of {@link AuditPath} key entries concatenated with the path
     *     separator '/' ({@link AuditApplication#AUDIT_PATH_SEPARATOR})
     ...
     * @throws IllegalStateException if the transaction state could not be determined
     */
    Map<String, Serializable> recordAuditValuesWithUserFilter(
            String rootPath, Map<String, Serializable> values, boolean useUserFilter);

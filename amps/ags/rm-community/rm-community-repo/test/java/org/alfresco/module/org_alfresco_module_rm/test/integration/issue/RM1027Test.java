      ...
      new Test<NodeRef>() {
        ...
        public NodeRef run() {
          ...
          NodeRef recordFolder = recordFolderService.createRecordFolder(
            rmContainer,
            "My Closed Record Folder"
          );
          utils.closeFolder(recordFolder);

          assertTrue(
            (Boolean) nodeService.getProperty(recordFolder, PROP_IS_CLOSED)
          );
    ...
    doTestInTransaction(
      new Test<NodeRef>() {
        ...
        public NodeRef run() throws Exception {
          ...
          NodeRef destination = filePlanService.createRecordCategory(
            filePlan,
            "Copy Destination"
          );

          // try and copy the closed record folder
          FileInfo copyInfo = fileFolderService.copy(
            recordFolder,
            destination,
            null
          );
    ...
    final NodeRef recordFolder = doTestInTransaction(
      new Test<NodeRef>() {
        ...
        public NodeRef run() {
          ...
          assertTrue(
            nodeService.hasAspect(recordFolder, ASPECT_DISPOSITION_LIFECYCLE)
          );
    ...
    doTestInTransaction(
      new Test<NodeRef>() {
        ...
        public void test(NodeRef copy) throws Exception {
          ...
          assertNotNull(nodeService.getProperty(copy, PROP_IDENTIFIER));
          assertFalse(
            nodeService.hasAspect(copy, ASPECT_DISPOSITION_LIFECYCLE)
          );
        }
      }
    );

...
public class DynamicAuthoritiesGet extends AbstractWebScript implements RecordsManagementModel {
    ...
    protected Map<String, Object> buildModel(WebScriptRequest req, WebScriptResponse res)
            throws IOException {
    ...
    public void execute(WebScriptRequest req, WebScriptResponse res) throws IOException {
        ...
        try {
            String mimetype =
                    getContainer().getFormatRegistry().getMimeType(req.getAgent(), format);
            if (mimetype == null) {
            ...
            if (statusCode != HttpServletResponse.SC_OK && !req.forceSuccessStatus()) {
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Force success status header in response: " + req.forceSuccessStatus());
    ...
    /**
     * Obtain maximum of the records to be processed from the request if it is specified or bachsize
     * value otherwise
     *
     * @param req
     * @return maximum of the records to be processed from the request if it is specified or
     *     bachsize value otherwise
     */
    ...
    protected Long getBatchSizeParameter(WebScriptRequest req) {
        ...
        try {
            ...
            if (size <= 0) {
                ...
                throw new WebScriptException(
                        Status.STATUS_BAD_REQUEST, MESSAGE_PARAMETER_BATCHSIZE_GREATER_THAN_ZERO);
            }
        } catch (NumberFormatException ex) {
            ...
            if (!nodeService.exists(parentNodeRef)) {
                String message =
                        MessageFormat.format(
                                MESSAGE_NODE_REF_DOES_NOT_EXIST_TEMPLATE, parentNodeRef.toString());
                logger.info(message);
    ...
    /**
     * Process nodes all nodes or the maximum number of nodes specified by batchsize or
     * totalNumberOfRecordsToProcess parameters
     *
     * @param batchSize
     * @param maxNodeId
                                ...
                                public Void execute() throws Throwable {
                                    ...
                                    for (FileInfo node : nodes) {
                                        ...
                                        if (nodeService.hasAspect(
                                                record, recordAspectPair.getSecond())) {
                                            String recordName =
                                                    (String)
                                                            nodeService.getProperty(
                                                                    record, ContentModel.PROP_NAME);
    ...
    protected void processNode(NodeRef nodeRef) {
        ...
        Map<String, Integer> readers =
                (Map<String, Integer>) nodeService.getProperty(nodeRef, PROP_READERS);
        Map<String, Integer> writers =
                (Map<String, Integer>) nodeService.getProperty(nodeRef, PROP_WRITERS);

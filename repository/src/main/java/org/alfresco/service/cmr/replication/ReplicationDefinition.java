...
/**
 * This class is used to fully specify an inter-repository replication. It specifies which node(s)
 * should be transfered, which associated content should be sent with them, and which target they
 * should be sent to.
 *
 * <p>Every ReplicationDefinition has a <code>replicationName</code> attribute which uniquely
 * identifies it. It also has a single target.
 *
 * @author Nick Burch
 */
public interface ReplicationDefinition extends CancellableAction, SchedulableAction, Serializable {
    ...
    /**
     * Returns the local side of the report on the transfer. The transfer service generates two
     * reports, one on the local repository, and one on the remote repository. This returns the
    ...
    /**
     * Records the location on the local repository of the transfer service report on the
     * replication.
     */
    void setLocalTransferReport(NodeRef report);

    /**
     * Returns the remote side of the report on the transfer. The transfer service generates two
     * reports, one on the local repository, and one on the remote repository. This returns the
    ...
    /**
     * Records the location on the local repository of the transfer service report that was
     * generated on the remote repository for the replication.
     */
    void setRemoteTransferReport(NodeRef report);

    /**
     * Is scheduling currently enabled? See {@link
     * ReplicationService#enableScheduling(ReplicationDefinition)} and {@link

...
public class TestSiteMembers extends EnterpriseTestApi {
  ...
  public void testSiteMembers() throws Exception {
...

    {
      ...
      TestSite testSite = TenantUtil.runAsUserTenant(
      ...
      // Test Case cloud-1482
      {
        ...
        Paging paging = getPaging(
          skipCount,
          maxItems,
          expectedSiteMembers.size(),
          6
        );
        publicApiClient.setRequestContext(
          new RequestContext(testNetwork.getId(), personId)
        );
        ListResponse<SiteMember> siteMembers = sitesProxy.getSiteMembers(
          testSite.getSiteId(),
...

      {
        ...
        HttpResponse response = sitesProxy.getAll(
          "sites",
          testSite.getSiteId(),
          "members",
          null,
          createParams(
            paging,
            Collections.singletonMap("includeSource", "true")
          ),
        ...
        JSONObject source = sitesProxy.parseListSource(
          response.getJsonResponse()
        );
      ...
      // update site member
      {
        ...
        SiteMember ret = sitesProxy.updateSiteMember(
          testSite.getSiteId(),
          siteMember1
        );
        assertEquals(siteMember1.getRole(), ret.getRole());
        Person expectedSiteMember = repoService.getPerson(
          siteMember1.getMemberId()
        );
      ...
      // GET single site member
      {
        ...
        SiteMember ret = sitesProxy.getSingleSiteMember(
          testSite.getSiteId(),
          siteMember1.getMemberId()
        );
    ...
    // test: user is member of different tenant, but has site membership(s) in common with the http request user
    {
      ...
      try {
        ...
        sitesProxy.createSiteMember(
          site.getSiteId(),
          new SiteMember(person1.getId(), "dodgyRole")
        );
        fail("");
      } catch (PublicApiException e) {
        assertEquals(
...

      {
        ...
        SiteMember siteMember = sitesProxy.createSiteMember(
          site.getSiteId(),
          sm
        );
      ...
      try {
        ...
        sitesProxy.createSiteMember(
          site.getSiteId(),
          new SiteMember(person1.getId(), null)
        );
        fail("");
      } catch (PublicApiException e) {
        assertEquals(
    ...
    // test: create site membership, remove it, get list of site memberships
    {
...

      {
        ...
        SiteMember siteMemberAno = sitesProxy.createSiteMember(
          site.getSiteId(),
          new SiteMember(person3.getId(), SiteRole.SiteCollaborator.toString())
        );
        assertEquals(person3.getId(), siteMemberAno.getMemberId());
        assertEquals(
          SiteRole.SiteCollaborator.toString(),
          siteMemberAno.getRole()
        );
        ...
        try {
          ...
          sitesProxy.removeSiteMember(
            site.getSiteId(),
            new SiteMember(GUID.generate())
          );
          fail();
        } catch (PublicApiException e) {
          assertEquals(
        ...
        try {
          ...
          sitesProxy.updateSiteMember(
            site.getSiteId(),
            new SiteMember(person1.getId(), "invalidRole")
          );
          fail();
        } catch (PublicApiException e) {
          assertEquals(
        ...
        // successful update
        {
          ...
          // check site membership in GET
          SiteMember smToCheck = sitesProxy.getSingleSiteMember(
            site.getSiteId(),
            person1.getId()
          );

...
public class FeedControlEntity {
    ...
    // TODO - review - deleted feed controls are not kept and available feed controls are currently
    // retrieved during generation, hence
    // it is possible for a feed control to be applied even if lastModified is greater than postDate
    // - could check the date !
    // it is also possible for a feed control to not be applied if it is deleted just after the post
    // - would need to keep, at least until next generation

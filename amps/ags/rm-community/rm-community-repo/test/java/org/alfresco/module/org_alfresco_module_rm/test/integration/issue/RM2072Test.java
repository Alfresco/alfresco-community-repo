    ...
    doTestInTransaction(
      new Test<Void>() {
        ...
        public Void run() {
          ...
          NodeRef nodeRefA = filePlanService.createRecordCategory(
            filePlan,
            "A"
          );
          NodeRef nodeRefB = filePlanService.createRecordCategory(
            nodeRefA,
            "B"
          );
            ...
            public List<NodeRef> run() throws Exception {
              ...
              for (int j = 0; j < NUMBER_IN_BATCH; j++) {
                ...
                NodeRef record = fileFolderService
                  .create(ruleFolder, name, ContentModel.TYPE_CONTENT)
                  .getNodeRef();
                records.add(record);
              }
              return records;

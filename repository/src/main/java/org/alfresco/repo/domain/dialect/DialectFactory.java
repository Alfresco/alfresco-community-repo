 ...
 *
 * <p>This class is an Alfresco-specific version of the Hibernate class with the same name.
 *
...
public class DialectFactory {
     ...
     *
     * <p>The JDBC driver, the database name and version (obtained from connection metadata) are
     * used to make the determination.
     *
     * <p>An exception is thrown if a dialect was not explicitly set and the database name is not
     * known.
     *
     * @param databaseName The name of the database product (obtained from metadata).
     * @param databaseMajorVersion The major version of the database product (obtained from
     *     metadata).
     * @param driverName THe name of the JDBC driver
     * @return The appropriate dialect.
     */
    public static Dialect buildDialect(
            String databaseName, int databaseMajorVersion, String driverName) {
        if (databaseName == null) {
            throw new IllegalArgumentException("Database name must be explicitly set");
        }

        DatabaseDialectMapper mapper =
                MAPPERS.get(driverName) != null
    ...
    /**
     * For a given database product name, instances of DatabaseDialectMapper know which Dialect to
     * use for different versions.
     */
    ...
    /**
     * A simple DatabaseDialectMapper for dialects which are independent of the underlying database
     * product version.
     */
    ...
    // TODO : this is the stuff it'd be nice to move to a properties file or some other easily
    // user-editable place
    private static final Map<String, VersionInsensitiveMapper> MAPPERS =
    ...
    static {
        ...
        MAPPERS.put(
                "PostgreSQL",
                new VersionInsensitiveMapper("org.alfresco.repo.domain.dialect.PostgreSQLDialect"));
        MAPPERS.put(
                "MariaDB",
        ...
        MAPPERS.put(
                ...
                new VersionInsensitiveMapper("org.alfresco.repo.domain.dialect.SQLServerDialect"));
        MAPPERS.put(
                "Microsoft SQL Server",
                new VersionInsensitiveMapper("org.alfresco.repo.domain.dialect.SQLServerDialect"));
        MAPPERS.put(
                "Oracle",
                new VersionInsensitiveMapper("org.alfresco.repo.domain.dialect.Oracle9Dialect"));
    }
}

...
public class ThumbnailServiceTest extends BaseWebScriptTest {
    ...
    protected void setUp() throws Exception {
        ...
        try {
            this.transactionService =
                    (TransactionServiceImpl)
                            getServer().getApplicationContext().getBean("transactionComponent");
        ...
        FileInfo fileInfoPdf =
                this.fileFolderService.create(
                        this.testRoot, "test_doc" + guid + ".pdf", ContentModel.TYPE_CONTENT);
        this.pdfNode = fileInfoPdf.getNodeRef();
        ContentWriter contentWriter =
                this.contentService.getWriter(
                        fileInfoPdf.getNodeRef(), ContentModel.PROP_CONTENT, true);
        ...
        FileInfo fileInfoJpg =
                this.fileFolderService.create(
                        this.testRoot, "test_image" + guid + ".jpg", ContentModel.TYPE_CONTENT);
        this.jpgNode = fileInfoJpg.getNodeRef();
        contentWriter =
                this.contentService.getWriter(
                        fileInfoJpg.getNodeRef(), ContentModel.PROP_CONTENT, true);
        contentWriter.setEncoding("UTF-8");
    ...
    public void testCreateThumbnail() throws Exception {
        ...
        if (synchronousTransformClient.isSupported(
            ...
            Response response =
                    sendRequest(new PostRequest(url, tn.toString(), "application/json"), 200);
    ...
    public void testUpdateThumbnail() throws Exception {
        ...
        sendRequest(
                new PutRequest(
                        getThumbnailsURL(jpgNode) + "/incorrectname", "", "application/json"),
                404);
        // Request for update of correct thumbnail
        sendRequest(
                new PutRequest(getThumbnailsURL(jpgNode) + "/doclib", "", "application/json"), 200);
    }

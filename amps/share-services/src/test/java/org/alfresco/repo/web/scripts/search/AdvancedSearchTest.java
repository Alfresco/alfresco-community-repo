...
public class AdvancedSearchTest extends BaseWebScriptTest {
    ...
    protected void setUp() throws Exception {
        ...
        this.searchService =
                (SearchService) getServer().getApplicationContext().getBean("SearchService");
        this.namespaceService =
                (NamespaceService) getServer().getApplicationContext().getBean("namespaceService");
        ...
        List<NodeRef> results =
                searchService.selectNodes(
                        this.rootNodeRef, "/app:company_home", null, namespaceService, false);
        if (results.size() == 0) {
        ...
        folderNodeRef =
                fileFolderService
                        .create(companyHomeNodeRef, TEST_FOLDER, ContentModel.TYPE_FOLDER)
                        .getNodeRef();

        testNodeRef1 =
                fileFolderService
                        .create(folderNodeRef, TEST_FILE_NAME1, ContentModel.TYPE_CONTENT)
                        .getNodeRef();
        testNodeRef2 =
                fileFolderService
                        .create(folderNodeRef, TEST_FILE_NAME2, ContentModel.TYPE_CONTENT)
                        .getNodeRef();
    ...
    public void testSearchFile() throws IOException, JSONException {
        ...
        String url =
                "/slingshot/search?site=&term=&tag=&maxResults=251&sort=&query={\"prop_cm_name\":\""
                        + SEARCH_NAME
                        + "\",\"datatype\":\"cm:content\"}&repo=true&rootNode=alfresco://company/home";
    ...
    public void testSearchInSites() throws IOException, JSONException {
        ...
        NodeRef sitefolderNodeRef =
                fileFolderService
                        .create(sitesNodeRef, TEST_FOLDER, ContentModel.TYPE_FOLDER)
                        .getNodeRef();
        NodeRef nodeRef =
                fileFolderService
                        .create(
                                sitefolderNodeRef,
                                TEST_IN_SITES_FILE_NAME + time,
                                ContentModel.TYPE_CONTENT)
                        .getNodeRef();
        // String url = "/slingshot/search?site=&term=" + SEARCH_NAME +
        // "&tag=&maxResults=251&sort=&query=&repo=false&rootNode=alfresco%3A%2F%2Fcompany%2Fhome&pageSize=50&startIndex=0";
        String url =
                "/slingshot/search?site=&term=&tag=&maxResults=251&sort=&query={\"prop_cm_name\":\""
                        + SEARCH_IN_SITES
                        + time

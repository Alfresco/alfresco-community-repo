        ...
        doBehaviourDrivenTest(
                new BehaviourDrivenTest() {
                    ...
                    public void given() {
                        ...
                        assertTrue(
                                "Primary parent of newly created should be a records management"
                                        + " artifact.",
                                ...
                                new RunAsWork<Void>() {
                                    public Void doWork() throws Exception {
                                        ...
                                        inplaceRecordService.moveRecord(
                                                dmDocument, destinationDmFolder);
                    ...
                    public void then() {
                        ...
                        // Check that the source folder is empty now and the destination folder has
                        // the document
                        assertEquals(0, nodeService.getChildAssocs(dmFolder).size());
                        List<ChildAssociationRef> destinationFolderChildAssocs =
                                nodeService.getChildAssocs(destinationDmFolder);
                        ...
                        NodeRef primaryParentAfterMove =
                                nodeService.getPrimaryParent(dmDocument).getParentRef();
                        assertTrue(
                                "Primary parent of record after inplace move should be a records"
                                        + " management artifact.",
                                filePlanService.isFilePlanComponent(primaryParentAfterMove));
                        assertEquals(
                                "Primary parent of record after inplace move should remain the"
                                        + " same.",
                                primaryParentBeforeMove,
                                primaryParentAfterMove);

                        // Check extended readers/writers
                        Set<String> extendedReadersAfterMove =
                                extendedSecurityService.getReaders(dmDocument);
                        Set<String> extendedWritersAfterMove =
                                extendedSecurityService.getWriters(dmDocument);

                        assertEquals(
                                extendedReadersBeforeMove.size(), extendedReadersAfterMove.size());
                        assertEquals(
                                extendedWritersBeforeMove.size(), extendedWritersAfterMove.size());

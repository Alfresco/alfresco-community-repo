...
public class SolrSQLHttpClientTest {
  ...
  public void setUp() throws Exception {
    ...
    when(mockSearchParameters.getStores())
      .thenReturn(new ArrayList(asList(STORE_REF)));
    ...
    when(mockHttpClient.getHostConfiguration())
      .thenReturn(mockHostConfiguration);
    when(mockHostConfiguration.getHostURL()).thenReturn("hostURL");
    Pair<HttpClient, String> httpClientAndBaseUrl = new Pair<>(
      mockHttpClient,
      "baseURL"
    );
    ...
    when(mockTenantService.getCurrentUserDomain())
      .thenReturn("currentUserDomain");
  ...
  public void testExecuteQuery_minimalQuery() {
    // Call the method under test.
    ResultSet resultSet = solrSQLHttpClient.executeQuery(
      mockSearchParameters,
      LANGUAGE
    );

    assertEquals(
      "Expected result to come back from HTTP call.",
      mockResultSet,
      resultSet
    );
  }
  ...
  public void testExecuteQuery_filterQueries() throws JSONException {
    when(mockSearchParameters.getFilterQueries())
      .thenReturn(asList("FQ1", "FQ2"));
    ...
    List<String> actual = stringJsonArrayToList(
      bodyCaptor.getValue().getJSONArray("filterQueries")
    );
    assertEquals(
      "Unexpected filter queries in HTTP request.",
      actual,
      asList("FQ1", "FQ2")
    );
  }
   ...
   */
  private List<String> stringJsonArrayToList(JSONArray jsonArray)

 ...
 * @author Andy
 */
public class DBFTSPhrase extends FTSPhrase implements DBQueryBuilderComponent {
  ...
  public void prepare(
    ...
    PropertyArgument propArg = (PropertyArgument) functionArgs.get(
      ARG_PROPERTY
    );
    ...
    if ((propArg == null) || (propArg.getPropertyName() == null)) {
      ...
      parentSupport.setCommandType(
        DBQueryBuilderPredicatePartCommandType.EQUALS
      );
      ...
      aspectSupport.setQnameIds(
        DBQuery.findAspectIds(
          term,
          namespaceService,
          dictionaryService,
          qnameDAO,
      ...
      propArg.getPropertyName().equals(QueryConstants.FIELD_EXACTTYPE)
    ) {
      TypeSupport typeSupport = new TypeSupport();
      typeSupport.setQnameIds(
        DBQuery.findTypeIds(
      ...
      propArg.getPropertyName().equals(QueryConstants.FIELD_EXACTASPECT)
    ) {
      AspectSupport aspectSupport = new AspectSupport();
      aspectSupport.setQnameIds(
        DBQuery.findAspectIds(
      ...
      if (mode != AnalysisMode.IDENTIFIER) {
        throw new QueryModelException(
          "Analysis mode not supported for DB " + mode
        );
      }
      ...
      QName propertyQName = QName.createQName(
        DBQuery.expandQName(
          functionContext.getAlfrescoPropertyName(propArg.getPropertyName()),
          namespaceService
        )
      );

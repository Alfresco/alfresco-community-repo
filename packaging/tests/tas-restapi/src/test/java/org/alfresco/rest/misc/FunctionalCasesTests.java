...
public class FunctionalCasesTests extends RestTest {
  ...
  public void managerIsAbleToUpdateManagerWithDifferentRoles() {
    ...
    updatedMember =
      restClient.withCoreAPI().usingSite(publicSite).updateSiteMember(testUser);
  ...
  public void approveRequestAddAndDeleteSiteFromFavorites() throws Exception {
    ...
    returnedCollection
      .assertThat()
      .entriesListDoesNotContain("id", moderatedSite.getId());
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.PEOPLE },
    executionType = ExecutionType.REGRESSION,
    description = "Reject request, add moderated site to favorites, create request again and approve it"
  )
  @Test(groups = { TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.REGRESSION })
  ...
  public void addUpdateDeleteCommentThenGetCommentDetails() throws Exception {
    ...
    file =
      dataContent
        ...
        .usingSite(publicSite)
        .createContent(DocumentType.TEXT_PLAIN);
    RestCommentModel newComment = restClient
      .authenticateUser(manager)
      .withCoreAPI()
      .usingResource(file)
      .addComment("new comment");
    ...
    RestCommentModelsCollection fileComments = restClient
      .withCoreAPI()
      .usingResource(file)
      .getNodeComments();
    restClient.assertStatusCodeIs(HttpStatus.OK);
    fileComments
      .assertThat()
      .entriesListContains("content", newComment.getContent());

    RestCommentModel updatedComment = restClient
      .withCoreAPI()
      .usingResource(file)
  ...
  public void joinUserToSiteThenGetPersonActivities() throws Exception {
    ...
    activities =
      restClient
        .withCoreAPI()
        .usingAuthUser()
        .getPersonActivitiesUntilEntriesCountIs(1);
    activities
      .assertThat()
      .entriesListIsNotEmpty()
      .and()
      ...
      .and()
      .entriesListContains(
        "activitySummary.memberPersonId",
        userJoinSite.getUsername()
      );
  }
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.PEOPLE },
    executionType = ExecutionType.REGRESSION,
    description = "Verify membership request by user after it was removed from site gets status code 404"
  )
  @Test(groups = { TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.REGRESSION })
  public void userCanNotCreateMembershipRequestIfItWasRemovedFromPrivateSite()
    throws Exception {
    UserModel newMember = dataUser.createRandomTestUser();
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.COMMENTS },
    executionType = ExecutionType.REGRESSION,
    description = "Check that a comment of a document was also removed after deleting the document"
  )
  @Test(
    groups = { TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION }
  )
  public void checkTheCommentOfADocumentThatWasDeletedDoesNotExist()
    throws Exception {
    SiteModel publicSite = dataSite
      .usingUser(dataUser.getAdminUser())
      .createPublicRandomSite();
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.COMMENTS },
    executionType = ExecutionType.REGRESSION,
    description = "Check that a comment of a document from a private site is not deleted after user is removed"
  )
  @Test(
    groups = { TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION }
  )
  public void checkThatCommentIsNotDeletedWhenPrivateSiteMemberIsRemoved()
    throws Exception {
    ...
    file =
      dataContent
        ...
        .createContent(CMISUtil.DocumentType.TEXT_PLAIN);
    String newContent =
      "This is a new comment added by " + newUser.getUsername();
    ...
    RestCommentModelsCollection comments = restClient
      .authenticateUser(dataUser.getAdminUser())
      .withCoreAPI()
      .usingResource(file)
      .getNodeComments();
  ...
  public void changeFavoriteSiteVisibilityThenCheckFavorites()
    throws Exception {
    UserModel manager = dataUser.createRandomTestUser();
    SiteModel publicSite = dataSite
      .usingUser(dataUser.getAdminUser())
      .createPublicRandomSite();
    dataUser.addUserToSite(manager, publicSite, UserRole.SiteManager);
    SiteModel favoriteSite = dataSite
      .usingUser(manager)
      .createPublicRandomSite();
    ...
    RestPersonFavoritesModelsCollection userFavoriteSites = restClient
      .withCoreAPI()
      .usingAuthUser()
      .where()
      .targetSiteExist()
    ...
    userFavoriteSites
      ...
      .entriesListContains(
        ...
        favoriteSite.getVisibility().name()
      )
      .assertThat()
    ...
    dataSite
      .usingUser(manager)
      .updateSiteVisibility(favoriteSite, Visibility.MODERATED);
    ...
    dataSite
      .usingUser(manager)
      .updateSiteVisibility(favoriteSite, Visibility.PRIVATE);
    ...
    dataSite
      .usingUser(manager)
      .updateSiteVisibility(favoriteSite, Visibility.PUBLIC);
  ...
  public void checkNewManagerActions() throws Exception {
    ...
    returnedCollection =
      restClient.withCoreAPI().usingMe().getSiteMembershipRequests();
    restClient.assertStatusCodeIs(HttpStatus.OK);
    returnedCollection
      .assertThat()
      .entriesListDoesNotContain("id", moderatedSite.getId());
    ...
    RestRatingModel returnedRatingModel = restClient
      .withCoreAPI()
      .usingResource(file)
      .likeDocument();
    restClient.assertStatusCodeIs(HttpStatus.CREATED);
    ...
    returnedRatingModel =
      restClient.withCoreAPI().usingResource(file).rateStarsToDocument(5);
    restClient.assertStatusCodeIs(HttpStatus.CREATED);
    returnedRatingModel
      .assertThat()
      .field("myRating")
  ...
  @TestRail(
    ...
    executionType = ExecutionType.REGRESSION,
    description = "Verify user gets its activities with Rest API and response is successful"
  )
  public void userGetsItsPeopleActivities() throws Exception {
    UserModel newUser = dataUser.createRandomTestUser();
    SiteModel userSiteModel = dataSite
      .usingUser(dataUser.getAdminUser())
      .createPublicRandomSite();
    ...
    FileModel fileInSite = dataContent
      .usingUser(newUser)
      .usingSite(userSiteModel)
      .createContent(DocumentType.TEXT_PLAIN);
    String newContent =
      "This is a new comment added by " + newUser.getUsername();
    RestCommentModel commentModel = restClient
      .authenticateUser(newUser)
      .withCoreAPI()
    ...
    RestActivityModelsCollection restActivityModelsCollection = restClient
      .authenticateUser(newUser)
      .withCoreAPI()
      .usingMe()
    ...
    restActivityModelsCollection
      ...
      .entriesListContains(
        "activityType",
        ActivityType.COMMENT_CREATED.toString()
      )
      .assertThat()
      ...
      .entriesListContains(
        "activityType",
        ActivityType.COMMENT_UPDATED.toString()
      )
      .assertThat()
      .entriesListContains(
        "activityType",
        ActivityType.USER_ROLE_CHANGED.toString()
      )
      .assertThat()
      .entriesListContains(
        "activityType",
        ActivityType.COMMENT_DELETED.toString()
      )
      .assertThat()

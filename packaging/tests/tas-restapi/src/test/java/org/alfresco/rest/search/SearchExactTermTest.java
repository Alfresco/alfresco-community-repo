...
/**
 * Tests Exact Term queries against standard properties (cm:name, cm:title, cm:description, cm:content).
 * These tests are engine-agnostic and do not require Cross Locale or Solr-specific tokenisation configuration.
 */
public class SearchExactTermTest extends AbstractSearchExactTermTest
{

    /**
     * Note these tests are searching in cm:name property only (default for exact term without cross locale).
     * 1 result expected for =run  >> Document #2 >> name: "Run"
     * 1 result expected for =jump >> Document #4 >> name: "Jump"
     */
    @Test
    ...
    public void exactSearch_exactPhrase_shouldReturnResultsContainingExactPhrase() throws Exception
    {
        ...
        /*
         * 5 results are expected for non-exact phrase search:
         * - Document #1 >> name: "Running", title: "Running jumping"
         * - Document #2 >> name: "Run", description: "you are supposed to run jump"

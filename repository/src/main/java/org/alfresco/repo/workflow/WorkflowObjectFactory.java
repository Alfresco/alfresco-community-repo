...
public class WorkflowObjectFactory {
   ...
   */
  public WorkflowDeployment createDeployment(
  ...
  public WorkflowInstance createInstance(
    ...
    String description = (String) getVariable(
      variables,
      WorkflowModel.PROP_WORKFLOW_DESCRIPTION
    );

    NodeRef initiator = null;
    ScriptNode initiatorSN = (ScriptNode) getVariable(
      variables,
    ...
    NodeRef workflowPackage = getNodeVariable(
      variables,
  ...
  private NodeRef getNodeVariable(Map<String, Object> variables, QName qName) {
    ...
    String message =
      "Variable " +
      qName +
      " should be of type ScriptNode but was " +
      obj.getClass();
  ...
  public boolean isDefaultDomain() {
    return TenantService.DEFAULT_DOMAIN.equals(
   ...
   */
  public TypeDefinition getTaskFullTypeDefinition(
    String name,
    boolean isStart
  ) {
    ...
    if (typeDef == null) {
      ...
      if (typeDef == null) {
        String msg = messageService.getMessage(
          "workflow.get.task.definition.metadata.error",
          name
        );

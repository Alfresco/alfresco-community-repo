...
public class WikiPageListGet extends AbstractWikiWebScript {
  ...
  protected Map<String, Object> executeImpl(
    ...
    boolean pageMetaOnly = strPageMetaOnly != null
      ? Boolean.parseBoolean(strPageMetaOnly)
      : false;
    ...
    if (userFiltering) {
      ...
      if (dateIsCreated) {
        pages =
          wikiService.listWikiPagesByCreated(
            site.getShortName(),
            from,
            to,
            paging
        ...
        pages =
          wikiService.listWikiPagesByModified(
            site.getShortName(),
            from,
            to,
            paging
    ...
    if (
      pages.getTotalResultCount() != null &&
      pages.getTotalResultCount().getFirst() != null
    ) {
    ...
    if (pages.getPage().size() > 0) {
      ...
      container =
        siteService.getContainer(
          site.getShortName(),
          WikiServiceImpl.WIKI_COMPONENT
        );

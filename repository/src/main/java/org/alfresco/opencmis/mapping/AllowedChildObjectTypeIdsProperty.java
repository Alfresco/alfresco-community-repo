...
public class AllowedChildObjectTypeIdsProperty extends AbstractProperty {
   ...
   */
  public AllowedChildObjectTypeIdsProperty(
  ...
  public Serializable getValueInternal(CMISNodeInfo nodeInfo) {
    ...
    TypeDefinition type = getServiceRegistry()
      .getDictionaryService()
      .getType(nodeInfo.getType().getAlfrescoClass());
    if (
      (type != null) &&
      (type.getChildAssociations() != null) &&
      (!type.getChildAssociations().isEmpty())
    ) {
      ArrayList<String> result = new ArrayList<String>();

      for (ChildAssociationDefinition cad : type
        .getChildAssociations()
        .values()) {
        String typeId = cmisMapping.getCmisTypeId(
          cad.getTargetClass().getName()
        );
        if (typeId != null) {
          result.add(typeId);
        }

...
public class GetPeopleActivitiesCoreTests extends RestTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify user cannot get activities for inexistent user with Rest API and"
                            + " response is 404")
    public void userCannotGetPeopleActivitiesForInexistentPersonId() throws Exception {
        ...
        restClient
                ...
                .containsSummary(
                        String.format(
                                RestErrorModel.ENTITY_NOT_FOUND, inexistentUserName.getUsername()))
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user gets its activities for inexistent siteId with Rest API and"
                            + " response is 404")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user gets activities with invald skipCount parameter with Rest API and"
                            + " response is 400")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user gets activities with invalid maxItems parameter with Rest API and"
                            + " response is 400")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user gets activities using invalid value for parameter 'who'with Rest"
                            + " API and response is 400")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user gets activities successfully using parameter 'who' with 'others'"
                            + " value with Rest API")
    public void userGetsPeopleActivitiesUsingOthersForWhoParameter() throws Exception {
        restActivityModelsCollection =
                restClient
                        ...
                        .getPersonActivities();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        // after repo-4250 a file is created first and then update it with content when using
        // dataprep, so there are more entries than previously
        restActivityModelsCollection.assertThat().paginationField("count").is("3");
        restActivityModelsCollection
                .assertThat()
                .entriesListDoesNotContain("postPersonId", userModel.getUsername().toLowerCase())
                .and()

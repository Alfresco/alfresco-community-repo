 ...
 *
 * <p>TODO: transaction rollback handling, deletion of nodes (currently tied to beforeDeleteNode).
 *
 * @author steveglover
 */
public class EventsServiceImpl extends AbstractEventsService implements EventsService {
    ...
    private NodeRenamedEvent nodeRenamedEvent(NodeInfo nodeInfo, String oldName, String newName) {
        ...
        // For site display name (title) rename events we don't want the path to be changed to the
        // display name (title); leave it to name (id)
    ...
    public void nodeMoved(
        ...
        if (nodeInfo.checkNodeInfo()) {
            ...
            String oldParentNodeName =
                    (String) nodeService.getProperty(oldParentNodeRef, ContentModel.PROP_NAME);
            String newParentNodeName =
                    (String) nodeService.getProperty(newParentNodeRef, ContentModel.PROP_NAME);
            String oldNodeName =
                    (String) nodeService.getProperty(oldNodeRef, ContentModel.PROP_NAME);
            String newNodeName =
                    (String) nodeService.getProperty(newNodeRef, ContentModel.PROP_NAME);
            List<Path> newParentPaths = nodeService.getPaths(newParentNodeRef, false);
            List<String> newPaths =
                    getPaths(newParentPaths, Arrays.asList(newParentNodeName, newNodeName));

            // renames are handled by an onUpdateProperties callback, we just deal with real moves
            // here.
            if (!oldParentNodeRef.equals(newParentNodeRef)) {
                ...
                List<String> previousPaths =
                        getPaths(oldParentPaths, Arrays.asList(oldParentNodeName, oldNodeName));
    ...
    public void nodeUnFavourited(NodeRef nodeRef) {
        ...
        if (nodeInfo.checkNodeInfo()) {
            ...
            NodeUnFavouritedEvent event =
                    new NodeUnFavouritedEvent(
                            nextSequenceNumber(),
                            name,
                            txnId,
                            timestamp,
    ...
    public void secondaryAssociationCreated(final ChildAssociationRef secAssociation) {
        ...
        if (nodeInfo.checkNodeInfo()) {
            ...
            String secParentNodeName =
                    (String)
                            nodeService.getProperty(
                                    secAssociation.getParentRef(), ContentModel.PROP_NAME);
            List<Path> secParentPath = nodeService.getPaths(secParentNodeRef, true);
            List<String> nodePaths =
                    getPaths(secParentPath, Arrays.asList(secParentNodeName, name));
    ...
    public void nodeCheckedOut(NodeRef workingCopyNodeRef) {
        NodeInfo workingCopyNodeInfo =
                getNodeInfo(workingCopyNodeRef, NodeCheckedOutEvent.EVENT_TYPE);
    ...
    public void nodeCheckOutCancelled(NodeRef nodeRef) {
        ...
        if (nodeInfo.checkNodeInfo()) {
            ...
            Event event =
                    new NodeCheckOutCancelledEvent(
                            nextSequenceNumber(),
                            name,
                            txnId,
                            timestamp,
    ...
    public void authorityRemovedFromGroup(String parentGroup, String childAuthority) {
        if (includeEventType(AuthorityRemovedFromGroupEvent.EVENT_TYPE)) {
            ...
            Event event =
                    AuthorityRemovedFromGroupEvent.builder()
    ...
    public void authorityAddedToGroup(String parentGroup, String childAuthority) {
        if (includeEventType(AuthorityAddedToGroupEvent.EVENT_TYPE)) {
            ...
            Event event =
                    AuthorityAddedToGroupEvent.builder()
                            .parentGroup(parentGroup)
                            .authorityName(childAuthority)
                            .seqNumber(nextSequenceNumber())
                            .txnId(txnId)
    ...
    public void inheritPermissionsEnabled(NodeRef nodeRef) {
        ...
        if (nodeInfo.checkNodeInfo()) {
            ...
            Event event =
                    InheritPermissionsEnabledEvent.builder()
                            .seqNumber(nextSequenceNumber())
                            .name(name)
                            .txnId(txnId)
                            .timestamp(timestamp)
    ...
    public void inheritPermissionsDisabled(NodeRef nodeRef, boolean async) {
        ...
        if (nodeInfo.checkNodeInfo()) {
            ...
            Event event =
                    InheritPermissionsDisabledEvent.builder()
                            .async(async)
                            .seqNumber(nextSequenceNumber())
                            .name(name)
                            .txnId(txnId)
    ...
    public void revokeLocalPermissions(NodeRef nodeRef, String authority, String permission) {
        ...
        if (nodeInfo.checkNodeInfo()) {
            ...
            Event event =
                    LocalPermissionRevokedEvent.builder()
                            .authority(authority)
    ...
    public void grantLocalPermission(NodeRef nodeRef, String authority, String permission) {
        ...
        if (nodeInfo.checkNodeInfo()) {
            ...
            LocalPermissionGrantedEvent event =
                    LocalPermissionGrantedEvent.builder()
                            .authority(authority)
                            .permission(permission)
                            .seqNumber(nextSequenceNumber())
                            .name(name)
    ...
    public void recordRejected(NodeRef nodeRef) {
        ...
        if (nodeInfo.checkNodeInfo()) {
            ...
            // Since the record creation, the record might've been hidden on the collaboration site,
            // thus removing the secondary parent-child association,
            ...
            if (PROP_RMA_RECORD_ORIGINATING_LOCATION == null) {
                logger.error(
                        format(
                                "Could not generate %s event for node %s because %s property is not"
                                        + " found.",
            ...
            NodeRef recordOriginatingLocation =
                    (NodeRef)
                            nodeService.getProperty(nodeRef, PROP_RMA_RECORD_ORIGINATING_LOCATION);
            String recordOriginatingParentName =
                    (String)
                            nodeService.getProperty(
                                    recordOriginatingLocation, ContentModel.PROP_NAME);
            Path originatingParentPath = nodeService.getPath(recordOriginatingLocation);

            Event event =
                    RecordRejectedEvent.builder()
                            .seqNumber(nextSequenceNumber())
                            .name(nodeInfo.getName())
                            .txnId(AlfrescoTransactionSupport.getTransactionId())
                            .timestamp(System.currentTimeMillis())
     ...
     * </pre>
     *
     * @return the QName of the RM property pointing to the original location of a node that has
     *     been declared as a record
     */
    private static QName getRmPropOriginatingLocation() {
        ...
        try {
            ...
            logger.info(
                    format(
                            "Could not retrieve property %s from class %s. Maybe RM isn't"
                                    + " installed, the property %s will be null.",
                            RM_MODEL_PROP_NAME_RECORD_ORIGINATING_LOCATION,
    ...
    public void recordCreated(NodeRef nodeRef) {
        ...
        if (nodeInfo.checkNodeInfo()) {
            // The event should contain the path that points to the original location of the file.
            // When a node is declared as a record, a secondary association is created for the
            // original location, hence use just that.
    ...
    public void nodeUnlocked(NodeRef nodeRef) {
        ...
        if (nodeInfo.checkNodeInfo()) {
            NodeUnlockedEvent event =
                    new NodeUnlockedEvent(
                            nextSequenceNumber(),
                            nodeInfo.getName(),
                            AlfrescoTransactionSupport.getTransactionId(),
                            System.currentTimeMillis(),

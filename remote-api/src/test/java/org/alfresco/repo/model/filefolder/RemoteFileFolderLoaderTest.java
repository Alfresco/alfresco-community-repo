...
public class RemoteFileFolderLoaderTest extends BaseWebScriptTest {
  ...
  protected void tearDown() throws Exception {
    ...
    try {
      transactionService
        .getRetryingTransactionHelper()
        .doInTransaction(deleteFolderWork);
    } finally {
  ...
  public void testLoad_15_default() throws Exception {
    ...
    for (FileInfo fileInfo : fileInfos) {
      ...
      assertTrue(
        reader.getContentUrl().startsWith(FileContentStore.SPOOF_PROTOCOL)
      );
      assertTrue(
      ...
      assertEquals(
        "hhoudini",
        nodeService.getProperty(fileNodeRef, ContentModel.PROP_CREATOR)
      );
      // We also expect the default language description to be present
      String description = (String) nodeService.getProperty(
        fileNodeRef,
  ...
  public void testLoad_15_16bytes() throws Exception {
    ...
    String contentUrlCheck = SpoofedTextContentReader.createContentUrl(
      Locale.ENGLISH,
      0L,
      16L
    );
    ...
    for (FileInfo fileInfo : fileInfos) {
      ...
      assertTrue(
        reader.getContentUrl().startsWith(FileContentStore.STORE_PROTOCOL)
      );
      // Check text

...
public class ReadRecordTests extends BaseRMRestTest {
    ...
    private Record electronicRecord =
            Record.builder().name(ELECTRONIC_RECORD_NAME).nodeType(CONTENT_TYPE).build();

    private Record nonelectronicRecord =
            Record.builder()
                    .properties(
                            RecordProperties.builder()
    ...
    /**
     * Given a record category or a container which can't contain records When I try to read the
     * children filtering the results to records Then I receive an empty list
     */
    @DataProvider(name = "invalidContainersForRecords")
    public Object[][] getInvalidContainersForRecords() {
        return new String[][] {{FILE_PLAN_ALIAS}, {TRANSFERS_ALIAS}, {rootCategory.getId()}};
    }
    ...
    public void readRecordsFromInvalidContainers(String container) {
        ...
        if (FILE_PLAN_ALIAS.equals(container)) {
            ...
            RecordCategoryChildCollection children =
                    getRestAPIFactory().getRecordCategoryAPI().getRecordCategoryChildren(container);
            // check response status code
            assertStatusCode(OK);
            children.assertThat() // check the list returned is not empty because there is a record
                    // folder created inside
    ...
    /**
     * Given a record When I try to read the meta-data Then I successfully receive the meta-data
    ...
    public void readRecordMetadata() {
        ...
        RecordCategory recordCategoryModel =
                createRecordCategoryModel(CATEGORY_NAME, CATEGORY_NAME);
        ...
        Record recordWithContent =
                getRestAPIFactory()
                        .getRecordsAPI()
                        .getRecord(recordWithContentId, "include=" + IS_COMPLETED + "," + CONTENT);
        ...
        Record nonElectronicRecord =
                getRestAPIFactory()
                        .getRecordsAPI()
                        .getRecord(
                                nonElectronicRecordId, "include=" + IS_COMPLETED + "," + CONTENT);
        ...
        assertEquals(
                nonElectronicRecord.getProperties().getDescription(), NONELECTRONIC_RECORD_NAME);
    ...
    /**
     * Given an electronic record When I try to read the content Then I successfully receive the
    ...
    public void readRecordContent() throws Exception {
        ...
        Record recordText = Record.builder().name(RECORD_ELECTRONIC).nodeType(CONTENT_TYPE).build();
        RecordFolderAPI recordFolderAPI = getRestAPIFactory().getRecordFolderAPI();
        String recordId =
                recordFolderAPI
                        .createRecord(
        ...
        Record recordBinary =
                Record.builder().name(RECORD_ELECTRONIC_BINARY).nodeType(CONTENT_TYPE).build();

        String binaryRecordId =
                recordFolderAPI.createRecord(recordBinary, folderId, getFile(IMAGE_FILE)).getId();
        // binary content, therefore compare respective SHA1 checksums in order to verify this is
        // identical content
        try (InputStream recordContentStream =
                        recordsAPI.getRecordContent(binaryRecordId).asInputStream();
        ...
        Record recordNoContent =
                Record.builder().name(RECORD_ELECTRONIC).nodeType(CONTENT_TYPE).build();
    ...
    /**
     * Given a non-electronic record When I try to read the content Then I am informed that the
    ...
    public void readNonElectronicRecordContent() {
        ...
        RecordFolderAPI recordFolderAPI = getRestAPIFactory().getRecordFolderAPI();
        String nonElectronicRecord =
                recordFolderAPI.createRecord(record, recordFolder.getId()).getId();
    ...
    /**
     * Given a container (eg record folder, record category, etc) When I try to read the content

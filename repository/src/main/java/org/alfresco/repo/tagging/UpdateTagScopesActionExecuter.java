 ...
 *
 * <p>NOTE: This action is used to facilitate the async update of tag scopes. It is not intended for
 * general usage.
 *
...
public class UpdateTagScopesActionExecuter extends ActionExecuterAbstractBase {
    ...
    private static final String noderefPath =
            TaggingServiceImpl.TAGGING_AUDIT_ROOT_PATH
                    + "/"
                    + TaggingServiceImpl.TAGGING_AUDIT_KEY_NODEREF
                    + "/value";
    private static final String tagsPath =
            TaggingServiceImpl.TAGGING_AUDIT_ROOT_PATH
                    + "/"
                    + TaggingServiceImpl.TAGGING_AUDIT_KEY_TAGS
                    + "/value";
    ...
    protected void executeImpl(final Action action, final NodeRef actionedUponNodeRef) {
        try {
            ...
            List<NodeRef> tagScopeNodes =
                    (List<NodeRef>) action.getParameterValue(PARAM_TAG_SCOPES);
                ...
                try {
                                                ...
                                                .doInTransaction(
                                                        new RetryingTransactionCallback<Void>() {
                                                            public Void execute() throws Throwable {
                                                                ...
                                                                updatesRemain.setValue(
                                                                        updatesRemain.intValue()
                                                                                + 1);

                                                                // Update the tags
                                                                performUpdates(tagScope, updates);

                                                                // Mark these entries as finished
                                                                // with
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "Tag scope "
                                        + tagScope
                                        + " is already being processed by another action,"
                                        + " skipping");
                    }
                }
    ...
    /**
     * For the given tag scope node, which should have been locked, ask the Audit Service for work
     * that needs to be done on it. Should fetch up to one chunk of work, and should coalesce
     * multiple updates of one tag into a single change.
     */
    private List<Long> searchForUpdates(
            final NodeRef tagScopeNode, final Map<String, Integer> updates) {
        // Build the query
                ...
                new AuditQueryCallback() {
                    ...
                    public boolean handleAuditEntry(
                        ...
                        if (values.containsKey(tagsPath)) {
                            ...
                            HashMap<String, Integer> changes =
                                    (HashMap<String, Integer>) values.get(tagsPath);
    ...
    /**
     * For the given tag scope node, which should have been locked, update the tag scope cache with
     * the given updates.
     */
    private void performUpdates(NodeRef tagScopeNode, Map<String, Integer> updates) {
        if (nodeService.exists(tagScopeNode)) {
            ...
            ContentReader contentReader =
                    contentService.getReader(tagScopeNode, ContentModel.PROP_TAGSCOPE_CACHE);
            ...
            for (String tagName : updates.keySet()) {
                ...
                if (change > 0) {
                    ...
                    for (int i = 0; i < change; i++) currentTag.incrementCount();
                } else {
                    if (currentTag != null) {
            ...
            // ACE-1979: emptying tag scope cache by setting content property for the cache to null
            // to avoid zero-size writes. Orphaned content will be deleted with content store
            // cleaner job
            if (tags.isEmpty()) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Updated tag scope: '"
                                    + tagScopeNode
                                    + "'. No tags were found. Emptying tags cache by setting"
                ...
                ContentWriter contentWriter =
                        contentService.getWriter(
                                tagScopeNode, ContentModel.PROP_TAGSCOPE_CACHE, true);
    ...
    /**
     * Checks several batches of updates in the Audit event log, and returns the list of Tag Scope
     * Node References found there. You should generally call this action to process the list of tag
     * nodes before re-calling this method. You may need to call this method several times if lots
     * of work is backed up, when an empty list is returned then you know that all work is handled.
     */
                                ...
                                auditService.auditQuery(
                                        new AuditQueryCallback() {
                                            ...
                                            public boolean handleAuditEntryError(
                                                    ...
                                                    Throwable error) {
                                                logger.warn(
                                                        "Error fetching tagging update entry - "
                                                                + errorMsg,
                                                        error);
                                                // Keep trying
                                                return true;
                                            ...
                                            public boolean handleAuditEntry(
                                                ...
                                                if (values.containsKey(noderefPath)) {
                                                    String nodeRefStr =
                                                            (String) values.get(noderefPath);
                                                    if (!tagNodesStrs.contains(nodeRefStr))
                                                        tagNodesStrs.add(nodeRefStr);
                                                } else {
                                                    logger.warn(
                                                            "Unexpected Tag Scope update entry of "
                                                                    + values);
                                                }
    ...
    protected String lockTagScope(NodeRef tagScope) {
        String lock = jobLockService.getLock(tagScopeToLockQName(tagScope), tagScopeLockTime, 0, 0);
        return lock;
    }
    ...
    protected void addParameterDefinitions(List<ParameterDefinition> paramList) {
        paramList.add(
                new ParameterDefinitionImpl(
                        ...
                        true,
                        getParamDisplayLabel(PARAM_TAG_SCOPES)));
    }
}

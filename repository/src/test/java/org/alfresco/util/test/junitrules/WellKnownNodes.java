...
/**
 * A JUnit rule designed to help with finding well known nodes in the system. It provides
 * functionality similar to {@link Repository}, but with extra nodes needed for testing
 *
 * @author Nick Burch
 * @since Odin
 */
public class WellKnownNodes extends ExternalResource {
    ...
    public WellKnownNodes(ApplicationContextInit appContextRule) {
        ...
        this.repositoryHelper =
                (Repository) appContextRule.getApplicationContext().getBean("repositoryHelper");
        this.nodeService =
    ...
    public NodeRef getSystemRoot() {
        ...
        List<ChildAssociationRef> sysRefs =
                nodeService.getChildAssocs(root, ContentModel.ASSOC_CHILDREN, SYSTEM_FOLDER_QNAME);
        if (sysRefs.size() != 1) {
    ...
    public NodeRef getSystemContainer(QName containerName) {
        ...
        List<ChildAssociationRef> containerRefs =
                nodeService.getChildAssocs(system, ContentModel.ASSOC_CHILDREN, containerName);
        if (containerRefs.size() != 1) {
            throw new IllegalStateException(
                    "System Container "
                            + containerName
                            + " missing / duplicated! Found "
                            + containerRefs);
        }

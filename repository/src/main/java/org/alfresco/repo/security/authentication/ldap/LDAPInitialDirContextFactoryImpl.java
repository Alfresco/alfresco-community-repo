...
public class LDAPInitialDirContextFactoryImpl
    ...
    private static Set<Map<String, String>> checkedEnvs =
            Collections.synchronizedSet(new HashSet<Map<String, String>>(11));
    ...
    public InitialDirContext getDefaultIntialDirContext(
            int pageSize, AuthenticationDiagnostic diagnostic) throws AuthenticationException {
    ...
    private InitialDirContext buildInitialDirContext(
            Hashtable<String, String> env, int pageSize, AuthenticationDiagnostic diagnostic)
            throws AuthenticationException {
        ...
        try {
            ...
            throw new AuthenticationException(
                    "authentication.err.authentication", diagnostic, args2, ax);
            ...
            throw new AuthenticationException(
                    "authentication.err.communication", diagnostic, args, cause);
            ...
            throw new AuthenticationException(
                    "authentication.err.connection", diagnostic, args1, nx);
        } catch (IOException e) {
            Object[] args = {providerURL, securityPrincipal};
            diagnostic.addStep(AuthenticationDiagnostic.STEP_KEY_LDAP_CONNECTED, true, args);
    ...
    public boolean hasNextPage(DirContext ctx, int pageSize) {
        if (pageSize > 0) {
            try {
                ...
                throw new AuthenticationException(
                        "Unable to connect to LDAP Server; check LDAP configuration", nx);
            } catch (IOException e) {
                throw new AuthenticationException(
    ...
    public InitialDirContext getInitialDirContext(
            String principal, String credentials, AuthenticationDiagnostic diagnostic)
            throws AuthenticationException {
        if (diagnostic == null) {
            diagnostic = new AuthenticationDiagnostic();
        }
        ...
        Hashtable<String, String> env =
                new Hashtable<String, String>(authenticatedEnvironment.size());
    ...
    public static void main(String[] args) {
        ...
        // We make the people and groups to represent this and stick them into LDAP ...used to
        // populate a test data base for user and groups

        int userMembers = Integer.parseInt(args[3]);

        ApplicationContext applicationContext = ApplicationContextHelper.getApplicationContext();
        LDAPInitialDirContextFactory factory =
                (LDAPInitialDirContextFactory)
                        applicationContext.getBean("ldapInitialDirContextFactory");
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        try {
            ...
            throw new AuthenticationException(
                    "The ldap server at "
                            ...
                            + " falls back to use anonymous bind if invalid security credentials"
                            + " are presented. This is not supported.");
        } catch (javax.naming.AuthenticationException ax) {
            logger.info(
                    "LDAP server does not fall back to anonymous bind for a string uid and password"
                            + " at "
                            + env.get(Context.PROVIDER_URL));
        } catch (AuthenticationNotSupportedException e) {
            logger.info(
        ...
        try {
            ...
            logger.info(
                    "LDAP server does not fall back to anonymous bind for a simple dn and password"
                            + " at "
                            + env.get(Context.PROVIDER_URL));
        } catch (AuthenticationNotSupportedException e) {
            logger.info(
            ...
            if (!checkedEnvs.contains(env)) {
                ...
                try {
                    ...
                    throw new AuthenticationException(
                            "The ldap server at "
                                    + env.get(Context.PROVIDER_URL)
                                    + " falls back to use anonymous bind for a known principal if "
                                    + " invalid security credentials are presented. This is not"
                                    + " supported.");
                } catch (javax.naming.AuthenticationException ax) {
                    logger.info(
                            "LDAP server does not fall back to anonymous bind for known principal"
                                    + " and invalid credentials at "
                                    + env.get(Context.PROVIDER_URL));
                } catch (AuthenticationNotSupportedException e) {
                    logger.info(
                ...
                // Record this environment as checked so that we don't check it again on further
                // restarts / other subsystem
                // instances
                checkedEnvs.add(env);
            }
        }
     ...
     * Required for LDAPS configuration. The <code>ldap.authentication.java.naming.security.protocol
     * </code> should be set to "ssl" for LDAPS. <br>
    ...
    private boolean isSSLSocketFactoryRequired() {
        ...
        if (protocol != null && protocol.equals("ssl")) {
            if (getTrustStoreType() != null
                ...
                logger.warn(
                        "The SSL configuration for LDAPS is not full, the default configuration"
                                + " will be used.");
            }
        }
        return result;
    ...
    private KeyStore initTrustStore() {
        ...
        try {
            ...
            throw new AlfrescoRuntimeException(
                    "The certificates cannot be loaded from truststore.", ce);
        }
        return ks;
    }
}

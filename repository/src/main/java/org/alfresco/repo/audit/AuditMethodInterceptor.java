...
public class AuditMethodInterceptor implements MethodInterceptor {
  ...
  private static final Class[] NON_RECORDABLE_CLASSES = new Class[] {
    InputStream.class,
    OutputStream.class,
  };

  private static final Log logger = LogFactory.getLog(
    AuditMethodInterceptor.class
  );
  ...
  private Object proceed(MethodInvocation mi) throws Throwable {
    ...
    try {
      ...
      Map<String, Serializable> namedArguments = getInvocationArguments(
        auditableDef,
        args
      );
  ...
  private Map<String, Serializable> auditInvocationBefore(
    ...
    if (namedArguments == null || namedArguments.isEmpty()) {
      rootPath =
        AuditApplication.buildPath(AUDIT_PATH_API_PRE, serviceName, methodName);
    ...
    Map<String, Serializable> auditedData = auditComponent.recordAuditValues(
      rootPath,
      auditData
    );
    // Done
  ...
  private void auditInvocationAfter(
    ...
    final String rootPath = AuditApplication.buildPath(
      AUDIT_PATH_API_POST,
      serviceName,
      methodName
    );
    ...
    for (Entry<String, Serializable> entry : preAuditedData.entrySet()) {
      String path = AuditApplication.buildPath(
            ...
            RetryingTransactionCallback<Map<String, Serializable>> auditCallback = new RetryingTransactionCallback<Map<String, Serializable>>() {
              public Map<String, Serializable> execute() throws Throwable {
                ...
                if (thrown instanceof AuthenticationException) {
                  return auditComponent.recordAuditValuesWithUserFilter(
                    rootPath,
                    auditData,
                    false
                  );

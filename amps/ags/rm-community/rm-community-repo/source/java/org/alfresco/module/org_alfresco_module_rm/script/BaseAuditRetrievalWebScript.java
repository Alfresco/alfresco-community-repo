...
public class BaseAuditRetrievalWebScript extends StreamContent {
  ...
  private static Log logger = LogFactory.getLog(
    BaseAuditRetrievalWebScript.class
  );
   ...
   */
  protected RecordsManagementAuditQueryParameters parseQueryParameters(
    ...
    if (MimetypeMap.MIMETYPE_JSON.equals(req.getContentType())) {
      try {
        ...
        if (logger.isWarnEnabled()) {
          logger.warn(
            "Failed to parse JSON parameters for audit query: " +
            ioe.getMessage()
          );
        }
    ...
    if (size != null && size.length() > 0) {
      try {
        ...
        if (logger.isWarnEnabled()) {
          logger.warn(
            "Ignoring size parameter as '" + size + "' is not a number!"
          );
        }
      }
    }

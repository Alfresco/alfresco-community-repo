...
public class ImapMoveFolderTests extends EmailTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify move folder at location where the folder already exists is successful")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify move folder with Contributor will create the new folder, but is NOT"
                            + " able to delete the current one")
    @Test(groups = {TestGroup.PROTOCOLS, TestGroup.IMAP, TestGroup.CORE})
    public void siteContributorCannotMoveFolder() throws Exception {
        ...
        dataUser.usingUser(testUser)
                .addUserToSite(siteContributor, testSite, UserRole.SiteContributor);
        ...
        imapProtocol
                .authenticateUser(siteContributor)
                .usingSite(testSite)
                .usingResource(testFolder)
                .moveTo(moveToFolder)
                .usingResource(moveToFolder)
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify move folder with Collaborator will create the new folder, but is NOT"
                            + " able to delete the current one")
    @Test(groups = {TestGroup.PROTOCOLS, TestGroup.IMAP, TestGroup.CORE})
    public void siteCollaboratorCannotMoveFolder() throws Exception {
        UserModel siteCollaborator = dataUser.createRandomTestUser();
        dataUser.usingUser(testUser)
                .addUserToSite(siteCollaborator, testSite, UserRole.SiteContributor);
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify move folder with Consumer will create the new folder, but is NOT able"
    ...
    public void siteConsumerCannotMoveFolder() throws Exception {
        ...
        dataUser.usingUser(testUser)
                .addUserToSite(siteConsumer, testSite, UserRole.SiteContributor);
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify move folder fails using user that doesn't have permission to the IMAP"
                            + " site")
    @Test(

...
/**
 * A {@link WorkflowTask} instance that will only fecth the {@link WorkflowPath} if needed and will
 * only fetch the properties when properties are needed which cannot be returned by the wrapped
 * {@link Task} or {@link HistoricTaskInstance} object.
 *
 * <p>The instance can only be used inside of the alfresco-context/transaction where the list of
...
public class LazyActivitiWorkflowTask extends WorkflowTask {
    ...
    public LazyActivitiWorkflowTask(
            Task task,
            ActivitiTypeConverter typeConverter,
            TenantService tenantService,
            String workflowDefinitionName) {
        super(
                BPMEngineRegistry.createGlobalId(ActivitiConstants.ENGINE_ID, task.getId()),
                null,
                null,
                null,
                null,
        ...
        // Fetch task-definition and a partially-initialized WorkflowTask (not including properties
        // and path)
        WorkflowTaskDefinition taskDefinition = activitiTypeConverter.getTaskDefinition(task);

        WorkflowTask partiallyInitialized =
                typeConverter
    ...
    public LazyActivitiWorkflowTask(
            ...
            TenantService tenantService) {
        super(
                BPMEngineRegistry.createGlobalId(ActivitiConstants.ENGINE_ID, historicTask.getId()),
                null,
                null,
                null,
                null,
        ...
        WorkflowTaskDefinition taskDefinition =
                activitiTypeConverter.getTaskDefinition(
                        historicTask.getTaskDefinitionKey(), historicTask.getProcessDefinitionId());

        String workflowDefinitionName =
                activitiTypeConverter.getWorkflowDefinitionName(
    ...
    /**
     * Property-map that returns properties that are known from the {@link Task} instance without
     * having to fetch the actual task-properties. Once a property is required that cannot be
     * deducted from the {@link Task} instance, all properties are fetched.
     *
     * @author Frederik Heremans
     */
    @SuppressWarnings("deprecation")
    private class LazyPropertiesMap implements Map<QName, Serializable> {
        ...
        public Serializable get(Object key) {
            if (WorkflowModel.PROP_DUE_DATE.equals(key)) {
                ...
                // Description-property is based on the task.getDescription(). Revert to the default
                // task(type) description, if missing.
                if (task != null) {
                    return (task.getDescription() != null && !task.getDescription().isEmpty())
                            ? task.getDescription()
                            : getDescription();
                } else {
                    return (historicTask.getDescription() != null
                                    && !historicTask.getDescription().isEmpty())
                            ? historicTask.getDescription()
                            : getDescription();
                }
            } else if (ContentModel.PROP_CREATED.equals(key)

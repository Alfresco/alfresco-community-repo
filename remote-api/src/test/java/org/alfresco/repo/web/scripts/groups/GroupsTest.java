...
public class GroupsTest extends BaseWebScriptTest {
     ...
     *
     * <p>TEST_ROOTGROUP GROUPA GROUPB GROUPD GROUPE (in Share Zone) USER_TWO USER_THREE GROUPC
     * USER_TWO GROUPF GROUPD GROUPG GROUPD
     */
    private synchronized String createTestTree() {
        ...
        if (!authorityService.authorityExists(rootGroupName)) {
            ...
            String groupE =
                    authorityService.createAuthority(
                            AuthorityType.GROUP, TEST_GROUPE, TEST_GROUPE, shareZones);
    ...
    protected void setUp() throws Exception {
        ...
        this.authorityService =
                (AuthorityService) getServer().getApplicationContext().getBean("AuthorityService");
    ...
    public void testGetRootGroup() throws Exception {
         ...
         */
        {
            ...
            for (int i = 0; i < data.length(); i++) {
                ...
                if (rootGroup.getString("shortName").equals(TEST_ROOTGROUP)) {
                    ...
                    assertEquals(
                            "shortName wrong", TEST_ROOTGROUP, rootGroup.getString("shortName"));
                    assertEquals(
                            "displayName wrong",
                            TEST_ROOTGROUP_DISPLAY_NAME,
                            rootGroup.getString("displayName"));
                    assertEquals(
                            "authorityType wrong", "GROUP", rootGroup.getString("authorityType"));
                    gotRootGroup = true;
                }
        ...
        /** Get all root groups in the application zone "APP.DEFAULT" */
        {
            Response response =
                    sendRequest(
                            new GetRequest(URL_ROOTGROUPS + "?zone=APP.DEFAULT"), Status.STATUS_OK);
            JSONObject top = new JSONObject(response.getContentAsString());
            logger.debug(response.getContentAsString());
            // System.out.println(response.getContentAsString());
            JSONArray data = top.getJSONArray("data");
        ...
        /** Get all root groups in the admin zone */
        {
            Response response =
                    sendRequest(
                            new GetRequest(URL_ROOTGROUPS + "?zone=AUTH.ALF"), Status.STATUS_OK);
            JSONObject top = new JSONObject(response.getContentAsString());
            logger.debug(response.getContentAsString());
            JSONArray data = top.getJSONArray("data");
            assertTrue(data.length() > 0);
        ...
        {
            Response response =
                    sendRequest(new GetRequest(URL_ROOTGROUPS + "?zone=WIBBLE"), Status.STATUS_OK);
            JSONObject top = new JSONObject(response.getContentAsString());
            logger.debug(response.getContentAsString());
            JSONArray data = top.getJSONArray("data");
            assertTrue(data.length() == 0);
    ...
    public void testGetGroup() throws Exception {
        ...
        {
            Response response =
                    sendRequest(new GetRequest(URL_GROUPS + "/" + TEST_GROUPB), Status.STATUS_OK);
            JSONObject top = new JSONObject(response.getContentAsString());
            logger.debug(response.getContentAsString());
            JSONObject data = top.getJSONObject("data");
            assertTrue(data.length() > 0);
        ...
        {
            Response response =
                    sendRequest(new GetRequest(URL_GROUPS + "/" + TEST_GROUPE), Status.STATUS_OK);
            JSONObject top = new JSONObject(response.getContentAsString());
            logger.debug(response.getContentAsString());
            JSONObject data = top.getJSONObject("data");
            assertTrue(data.length() > 0);
    ...
    public void testCreateRootGroup() throws Exception {
        ...
        try {
            /** Create a root group */
            {
                ...
                assertEquals(
                        "displayName wrong", myDisplayName, rootGroup.getString("displayName"));
            }
            ...
            sendRequest(
                    new DeleteRequest(URL_ROOTGROUPS + "/" + myGroupName), Status.STATUS_NOT_FOUND);
    ...
    public void testLinkChild() throws Exception {
        ...
        try {
            ...
            {
                Response response =
                        sendRequest(new GetRequest(URL_GROUPS + "/" + TEST_LINK), Status.STATUS_OK);
                JSONObject top = new JSONObject(response.getContentAsString());
                logger.debug(response.getContentAsString());
                JSONObject data = top.getJSONObject("data");
                assertTrue(data.length() > 0);
            ...
            /**
             * Link the group again - this fails - duplicate groups (children with the same name)
             * are not allowed
             */
            {
                JSONObject newGroupJSON = new JSONObject();
    ...
    public void testUpdateGroup() throws Exception {
        ...
        try {
            ...
            /** Now get it and verify that the name has changed */
            {
                Response response =
                        sendRequest(
                                new GetRequest(URL_GROUPS + "/" + myGroupName), Status.STATUS_OK);
                JSONObject top = new JSONObject(response.getContentAsString());
                logger.debug(response.getContentAsString());
                JSONObject data = top.getJSONObject("data");
                assertTrue(data.length() > 0);
     ...
     * <li>if the optional includeInternal parameter is true then will include internal groups,
     *     otherwise internalGroups are not returned.
     * <li>If the optional shortNameFilter parameter is set then returns those root groups with a
     *     partial match on shortName.
     */
    public void testSearchGroups() throws Exception {
        ...
        //    	{
        //		    Response response = sendRequest(new GetRequest(URL_GROUPS + "?zone=" +
        // AuthorityService.ZONE_APP_SHARE), Status.STATUS_OK);
        //		    JSONObject top = new JSONObject(response.getContentAsString());
        //		    logger.debug(response.getContentAsString());
        //		    System.out.println(response.getContentAsString());
        //		    JSONArray data = top.getJSONArray("data");
    ...
    public void testGetParents() throws Exception {
        ...
        //      /**
        //    	 * Get GROUP E   Which should be a child of GROUPB child of TESTROOT but in a
        // different zone
        //    	 */
        //    	{
        //    		Response response = sendRequest(new GetRequest(URL_GROUPS + "/" + TEST_GROUPE +
        // "/parents?level=ALL"), Status.STATUS_OK);
        //    		JSONObject top = new JSONObject(response.getContentAsString());
        //    		logger.debug(response.getContentAsString());
        //    		JSONArray data = top.getJSONArray("data");
        //    		assertTrue(data.length() >= 2);
        ...
        {
            sendRequest(
                    new GetRequest(URL_GROUPS + "/" + TEST_GROUPD + "/parents?level=rubbish"),
                    Status.STATUS_BAD_REQUEST);
        }
    ...
    public void testGetChildren() throws Exception {
        ...
        {
            sendRequest(
                    new GetRequest(URL_GROUPS + "/" + TEST_GROUPB + "/children?authorityType=XXX"),
                    Status.STATUS_BAD_REQUEST);
        }

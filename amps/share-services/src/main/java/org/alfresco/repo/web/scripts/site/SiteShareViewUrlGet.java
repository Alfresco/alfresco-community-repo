...
public class SiteShareViewUrlGet extends DeclarativeWebScript {
  ...
  protected Map<String, Object> executeImpl(
    ...
    if (nodeRefS == null) throw new WebScriptException(
      Status.STATUS_BAD_REQUEST,
      "NodeRef must be supplied"
    );
    if (!NodeRef.isNodeRef(nodeRefS)) throw new WebScriptException(
      Status.STATUS_BAD_REQUEST,
      "Invalid NodeRef"
    );
    ...
    if (!nodeService.exists(nodeRef)) throw new WebScriptException(
    ...
    if (site == null) throw new WebScriptException(
      Status.STATUS_BAD_REQUEST,
      "Node isn't part of a site"
    );
  ...
  private static QName TYPE_LINK = QName.createQName(
    NamespaceService.LINKS_MODEL_1_0_URI,
    "link"
  );

  protected String identifySharePage(
    NodeRef nodeRef,
    SiteInfo site,
    ...
    if (
      ...
      QName containerName = nodeService
        .getPrimaryParent(siteContainer)
        .getQName();
      if (
        containerName.equals(
          QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "blog")
        )
      ) {
      ...
      if (
        containerName.equals(
          QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "wiki")
        )
      ) {
    ...
    if (dictionaryService.isSubClass(nodeType, ForumModel.TYPE_POST)) {
      ...
      String topicName = (String) nodeService.getProperty(
        postTopic,
        ContentModel.PROP_NAME
      );
    ...
    if (dictionaryService.isSubClass(nodeType, ContentModel.TYPE_FOLDER)) {
      ...
      while (
        current != null &&
        !current.equals(siteContainer) &&
        !current.equals(site.getNodeRef())
      ) {
        paths.add(
          (String) nodeService.getProperty(current, ContentModel.PROP_NAME)
        );
        current = nodeService.getPrimaryParent(current).getParentRef();
      }
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "COuldn't identify specific URL for Node " +
      nodeRef +
      " of type " +
      nodeType
    );

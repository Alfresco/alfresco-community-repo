...
/**
 * Interceptor that enforces restrictions on updates of properties of type content (cm:content and
 * alike) on NodeService.
 *
 * <p>Can be configured by using global enabled flag - <code>contentPropertyRestrictions.enabled
 * </code>, or comma separated white list of callers <code>contentPropertyRestrictions.whitelist
 * </code>, which can be either packages or fully qualified class names.
 *
 * @author Alex Mukha
...
public class ContentPropertyRestrictionInterceptor implements MethodInterceptor {
    ...
    private Class[] defaultWhiteList =
            new Class[] {
        ...
        if (globalContentPropertyRestrictions && !isCallerWhiteListed()) {
            if (methodName.equals("setProperties")) {
                Map<QName, Serializable> properties =
                        args[1] != null
                                ? Collections.unmodifiableMap((Map<QName, Serializable>) args[1])
                ...
                if (nodeRef != null) {
                    for (QName propQname : properties.keySet()) {
                        if (isContentProperty(propQname, properties.get(propQname))
                                ...
                                && isContentChanged(
                                        nodeRef, propQname, properties.get(propQname))) {
                            throw new InvalidTypeException(
                                    "The node's content can't be updated via"
                                            + " NodeService#setProperties directly: \n"
                                            + "   node: "
                                            + args[0]
                                            + "\n"
                                            + "   property name: "
                ...
                if (nodeRef != null) {
                    for (QName propQname : properties.keySet()) {
                        if (isContentProperty(propQname, properties.get(propQname))
                            ...
                            throw new InvalidTypeException(
                                    "The node's content can't be updated via"
                                            + " NodeService#addProperties directly: \n"
                                            + "   node: "
                                            + args[0]
                                            + "\n"
                                            + "   property name: "
                ...
                Map<QName, Serializable> properties =
                        args[4] != null
                                ? Collections.unmodifiableMap((Map<QName, Serializable>) args[4])
                                : Collections.emptyMap();
                for (QName propQname : properties.keySet()) {
                    if (isContentProperty(propQname, properties.get(propQname))
                            && isContentNotNullOrEmpty(properties.get(propQname))) {
                        throw new InvalidTypeException(
                                "The node's content can't be updated via NodeService#createNode"
                                        + " directly: \n"
                                        + "   node: "
                                        + args[0]
                                        + "\n"
                ...
                if (nodeRef != null) {
                    if (isContentProperty(propQname, value)
                        ...
                        throw new InvalidTypeException(
                                "The node's content can't be updated via NodeService#setProperty"
                                        + " directly: \n"
                                        + "   node: "
                                        + args[0]
                                        + "\n"
                ...
                Map<QName, Serializable> properties =
                        args[2] != null
                                ? Collections.unmodifiableMap((Map<QName, Serializable>) args[2])
                                : Collections.emptyMap();
                NodeRef nodeRef = (NodeRef) args[0];
                if (nodeRef != null) {
                    for (QName propQname : properties.keySet()) {
                        if (isContentProperty(propQname, properties.get(propQname))
                                ...
                                && isContentChanged(
                                        nodeRef, propQname, properties.get(propQname))) {
                            throw new InvalidTypeException(
                                    "The node's content can't be updated via NodeService#addAspect"
                                            + " directly: \n"
                                            + "   node: "
                                            + args[0]
                                            + "\n"

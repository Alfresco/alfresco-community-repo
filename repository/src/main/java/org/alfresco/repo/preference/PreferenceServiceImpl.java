    ...
    private JSONObject getPreferencesObject(String userName) throws JSONException {
        ...
        if (isSystem
            ...
            throw new AccessDeniedException(
                    "The current user "
                            + currentUserName
                            + " does not have sufficient permissions to get the preferences of the"
                            + " user "
                            + userName);
        }
            ...
            if (jsonPrefs != null) {
                ...
                while (keys.hasNext()) {
                    ...
                    if (key.startsWith(SHARE_SITES_PREFERENCE_KEY)) {
                        ...
                        //   <SHARE_SITES_PREFERENCE_KEY>.<siteId>.favourited ->
                        // <SHARE_SITES_PREFERENCE_KEY>.<siteId>
                        //   <SHARE_SITES_PREFERENCE_KEY>.<siteId>.createdAt ->
                        // <EXT_SITES_PREFERENCE_KEY>.<siteId>.createdAt
                        if (key.endsWith(".favourited")) {
                            ...
                            // Ensure that the values of the following form (the only other
                            // important form in this case) does not
        ...
        if (userCanWritePreferences(userName, personNodeRef)) {
            AuthenticationUtil.runAs(
                    new RunAsWork<Object>() {
                        public Object doWork() throws Exception {
                            ...
                            if (PreferenceServiceImpl.this.nodeService.hasAspect(
                                            personNodeRef, ContentModel.ASPECT_PREFERENCES)
                                    == false) {
                                PreferenceServiceImpl.this.nodeService.addAspect(
                                        personNodeRef, ContentModel.ASPECT_PREFERENCES, null);
                            }
                            ...
                            try {
                                ...
                                for (Map.Entry<String, Serializable> entry :
                                    ...
                                    if (key.startsWith(SHARE_SITES_PREFERENCE_KEY)) {
                                        ...
                                        String siteId =
                                                key.substring(SHARE_SITES_PREFERENCE_KEY_LEN);

                                        StringBuilder sb =
                                                new StringBuilder(SHARE_SITES_PREFERENCE_KEY);
                                        sb.append(siteId);
                                ...
                                throw new AlfrescoRuntimeException(
                                        "Can not update preferences for "
                                                + userName
                                                + " because there was an error pasing the JSON"
                                                + " data.",
                                        exception);
                            }
            ...
            throw new AccessDeniedException(
                    "The current user "
                            ...
                            + " does not have sufficient permissions to update the preferences of"
                            + " the user "
                            + userName);
        }
    }
    ...
    public PreferenceServiceTrait createPreferenceServiceTrait() {
        return new PreferenceServiceTrait() {
            ...
            public void setPreferences(
                    final String userName, final Map<String, Serializable> preferences)
                    throws Throwable {

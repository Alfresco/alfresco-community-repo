...
public class ActivitiPropertyConverter {
    ...
    public Map<QName, Serializable> getTaskProperties(Task task) {
        ...
        Map<QName, Serializable> properties =
                mapArbitraryProperties(variables, localVariables, taskProperties, taskAssociations);
        ...
        if (variables.get(outcomeVarName) != null) {
            properties.put(
                    WorkflowModel.PROP_OUTCOME, (Serializable) variables.get(outcomeVarName));
        }
    ...
    private void mapPooledActors(
            Collection<IdentityLink> links, Map<QName, Serializable> properties) {
    ...
    public Map<QName, Serializable> getTaskProperties(
            DelegateTask task, TypeDefinition typeDefinition, boolean localOnly) {
    ...
    public Map<QName, Serializable> getTaskProperties(
            HistoricTaskInstance historicTask, Map<String, Object> localVariables) {
        ...
        Map<String, Object> allVariables =
                getHistoricProcessVariables(historicTask.getProcessInstanceId());
        allVariables.putAll(localVariables);

        // Map the arbitrary properties
        Map<QName, Serializable> properties =
                mapArbitraryProperties(
                        allVariables, localVariables, taskProperties, taskAssociations);
        ...
        if (localVariables.get(outcomeVarName) != null) {
            properties.put(
                    WorkflowModel.PROP_OUTCOME, (Serializable) localVariables.get(outcomeVarName));
        }

        // History of pooled actors is stored in task variable
        List<NodeRef> pooledActors = new ArrayList<NodeRef>();
        List<String> pooledActorRefIds =
                (List<String>) localVariables.get(ActivitiConstants.PROP_POOLED_ACTORS_HISTORY);
    ...
    //    /**
    //     * Sets all default workflow properties that should be set based on the given
    // taskproperties.
    //     * When the currentValues already contains a value for a certain key, this value is
    // retained
    ...
    // factory.mapQNameToName(WorkflowModel.PROP_WORKFLOW_DUE_DATE);
    ...
    // factory.mapQNameToName(WorkflowModel.PROP_WORKFLOW_PRIORITY);
    ...
    public void setDefaultTaskProperties(DelegateTask task) {
        ...
        if (description == null || description.length() == 0) {
            ...
            String processDefinitionKey =
                    ((ProcessDefinition) ((TaskEntity) task).getExecution().getProcessDefinition())
                            .getKey();
            description =
    ...
    public Map<QName, Serializable> getStartTaskProperties(
        ...
        Map<QName, Serializable> properties =
                mapArbitraryProperties(variables, variables, taskProperties, taskAssociations);
        ...
        ActivitiScriptNode ownerNode =
                (ActivitiScriptNode) variables.get(WorkflowConstants.PROP_INITIATOR);
    ...
    public Map<String, Object> getHistoricTaskVariables(String taskId) {
        HistoricDetailQuery query =
                activitiUtil.getHistoryService().createHistoricDetailQuery().taskId(taskId);
        return getHistoricVariables(query);
    }
    ...
    private Map<QName, Serializable> mapArbitraryProperties(
        ...
        EntryTransformer<String, Object, QName, Serializable> transformer =
                new EntryTransformer<String, Object, QName, Serializable>() {
                    @Override
     ...
     * If the type of the property is known, use {@link #convertValueToPropertyType(Task,
     * Serializable, QName)}
     *
    ...
    public Serializable convertValueToPropertyType(
            Task task, Serializable value, QName propertyName) {
        TypeDefinition taskDef = typeManager.getFullTaskDefinition(task);
        PropertyDefinition propDef = taskDef.getProperties().get(propertyName);
        if (propDef != null) {
            return (Serializable)
                    DefaultTypeConverter.INSTANCE.convert(propDef.getDataType(), value);
        }
        return value;
    }
        ...
        if (add != null || remove != null) {
                    ...
                    if (existingAdd == null) {
                        ...
                        if (existingAdd instanceof List<?>) {
                            ...
                            if (toAdd.getValue().size() > 0) {
                                newProperties.put(
                                        toAdd.getKey(), (Serializable) toAdd.getValue().get(0));
                            }
                        }
                    }
                }
            ...
            if (remove != null) {
                ...
                for (Entry<QName, List<NodeRef>> toRemove : remove.entrySet()) {
                    ...
                    // association
                    Serializable existingRemove =
                            (Serializable) newProperties.get(toRemove.getKey());
    ...
    public void setTaskProperties(DelegateTask task, Map<QName, Serializable> properties) {
        ...
        Map<String, Object> variablesToSet =
                handlerRegistry.handleVariablesToSet(properties, type, task, DelegateTask.class);
    ...
    public void setTaskProperties(Task task, Map<QName, Serializable> properties) {
        ...
        TypeDefinition type = typeManager.getFullTaskDefinition(task);
        Map<String, Object> variablesToSet =
                handlerRegistry.handleVariablesToSet(properties, type, task, Task.class);
    ...
    public Map<String, Object> convertHistoricDetails(List<HistoricDetail> details) {
        HashMap<String, HistoricVariableUpdate> updateMap =
        ...
        for (HistoricDetail detail : details) {
            ...
            // Add to the map if value is more recent than existing value or is the first value for
            // this update
            if (isMoreRecent) {
    ...
    public Map<String, Object> getStartVariables(
            String processDefId, Map<QName, Serializable> properties) {
        ...
        if (description == null) {
            String wfDescription =
                    (String) defaultProperties.get(WorkflowModel.PROP_WORKFLOW_DESCRIPTION);
            String procDefKey = procDef.getKey();
            ReadOnlyProcessDefinition deployedDef =
                    activitiUtil.getDeployedProcessDefinition(processDefId);
            String startEventName = deployedDef.getInitial().getId();
            String wfDefKey = factory.buildGlobalId(procDefKey);
            description =
                    factory.getTaskDescription(
                            startTaskType, wfDefKey, wfDescription, startEventName);
        ...
        return handlerRegistry.handleVariablesToSet(
                defaultProperties, startTaskType, null, Void.class);
    }
    ...
    public Task updateTask(
        ...
        Map<QName, Serializable> newProperties =
                getNewTaskProperties(task, properties, add, remove);
        if (newProperties != null) {

...
public class IntegrationWithCmisTests extends IntegrationTest {
  ...
  @TestRail(
    ...
    description = "Verify getChildren action for a large number of files from CMIS returns only unique values with few retries"
  )
  ...
  public void verifyContentAndThumbnailForTifFile() throws Exception {
    ...
    RestNodeModel fileNode = restAPI
      .authenticateUser(user)
      .withCoreAPI()
      .usingNode(folder)
      .createNode();
    ...
    STEP(
      "4. Verify thumbnail and content of TIF files are created using RESTAPI"
    );
    cmisAPI
      .authenticateUser(user)
      .usingSite(site)
    ...
    RestRenditionInfoModel renditionInfo = restAPI
      .withCoreAPI()
      .usingNode(file)
      .getNodeRendition("pdf");
    restAPI.assertStatusCodeIs(HttpStatus.OK);
    renditionInfo.assertThat().field("status").is("CREATED");
    renditionInfo =
      restAPI.withCoreAPI().usingNode(file).getNodeRendition("doclib");
    restAPI.assertStatusCodeIs(HttpStatus.OK);
    renditionInfo.assertThat().field("status").is("CREATED");
  }
  ...
  public void verifyContentDispositionForContentThatAreWhiteListed()
    throws Exception {
    ...
    STEP(
      "2. Upload the .pdf file and verify the reponse header adding the download=inline/attachement parameters (accepted both)."
    );
    cmisAPI
      .authenticateUser(user)
      .usingResource(sharedFolder)
      .createFile(pdfFile);

    RestResponse response = restAPI
      .authenticateUser(user)
      .withCMISApi()
    ...
    STEP(
      "3. Upload the .html file and verify the reponse header adding the download=inline/attachement parameters (accepted only attachment) ."
    );
  ...
  @TestRail(
    ...
    description = "Verify that alfresco returns the correct encoding for files created via CMIS."
  )
  public void verifyFileEncodingUsingCMIS() throws Exception {
    STEP(
      "1. Create a folder, two text file with specific encoding content and define the expected encoding."
    );
    FileModel utf8File = FileModel.getRandomFileModel(
      FileType.TEXT_PLAIN,
      " ∮ E⋅da = Q"
    );
    FileModel iso8859File = FileModel.getRandomFileModel(
      FileType.TEXT_PLAIN,
    ...
    String fileCreationWebScript =
      "alfresco/service/api/node/content/workspace/SpacesStore/";
    RestAssured.basePath = "";

    STEP(
      "3. Retrieve the nodes via webscripts and verify that the content type is the expected one (GET alfresco/service/api/node/content/workspace/SpacesStore/)."
    );
    restAPI.configureRequestSpec().setBasePath(RestAssured.basePath);

...
public abstract class QNameFieldProcessor<Type extends ClassAttributeDefinition>
    ...
    public QNameFieldProcessor(
            NamespaceService namespaceService, DictionaryService dictionaryService) {
     ...
     */
    @Override
    protected Field generateTypedField(
            String fieldName, FormCreationData formData, ContentModelItemData<?> typedData) {
        Field field = null;
        try {
            ...
            // ignore fields with an invalid namespace - the model may no longer be present in the
            // repository
        }
        return field;
    }
    ...
    public Field generateField(
            QName fullName, ContentModelItemData<?> itemData, boolean isForcedField) {
    ...
    /**
     * Sets several properties on the {@link FieldDefinition}, including name, label, description,
     * dataKeyName and whether the field is protected. These values are derived from the <code>
     * attribDef</code> parameter.
     *
     * @param attribDef Used to set the values of name, description, label, dataKeyName and
     *     isProtected properties on the returned object.
     * @param fieldDef A factory object used to create the FieldDefinition to be returned.
     * @param group Used to set the group on the returned FieldDefinition.
     */
    protected void populateFieldDefinition(
            Type attribDef, FieldDefinition fieldDef, FieldGroup group, String dataKeyPrefix) {
    ...
    protected abstract Type getTypeDefinition(
            QName fullName, ContentModelItemData<?> itemData, boolean isForcedField);
}

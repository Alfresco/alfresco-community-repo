 ...
 *
 * <p>When a cache file has been written that results in cleanThresholdPct (default 80%) of
 * maxUsageBytes being exceeded then the cached content cleaner is invoked (if not already running)
 * in a new thread.
 *
 * <p>When the CachingContentStore is about to write a cache file but the disk usage is in excess of
 * panicThresholdPct (default 90%) then the cache file is not written and the cleaner is started (if
 * not already running) in a new thread.
 *
 * <p>This quota manager works in conjunction with the cleaner to update disk usage levels in
 * memory. When the quota manager shuts down the current disk usage is saved to disk in
 * {ContentCacheImpl.cacheRoot}/cache-usage.ser
 *
 * <p>Upon startup, if the cache-usage.ser file exists then the current usage is seeded with that
 * value and the cleaner is invoked in a new thread so that the value can be updated more accurately
 * (perhaps some files were deleted manually after shutdown for example).
 *
 * @author Matt Ward
 */
public class StandardQuotaStrategy implements QuotaManagerStrategy, UsageTracker {
    ...
    /**
     * Lifecycle method. Should be called immediately after constructing objects of this type (e.g.
     * by the Spring framework's application context).
     */
    public void init() {
        ...
        if (maxUsageBytes < (10 * FileUtils.ONE_MB)) {
            if (log.isWarnEnabled()) {
                log.warn(
                        "Low maxUsageBytes of "
                                + maxUsageBytes
                                + "bytes - did you mean to specify in MB?");
            }
        }
    ...
    /**
     * Lifecycle method. Should be called when finished using an object of this type and before the
     * application container is shutdown (e.g. using a Spring framework destroy method).
     */
    ...
    public boolean afterWritingCacheFile(long contentSizeBytes) {
        ...
        if (getCurrentUsageBytes() >= maxUsageBytes) {
            ...
            if (log.isDebugEnabled()) {
                log.debug(
                        "Usage has reached or exceeded quota limit, limit: "
                                + maxUsageBytes
                                + " bytes, current usage: "
                                + getCurrentUsageBytes()
                                + " bytes.");
            ...
            if (log.isDebugEnabled()) {
                log.debug(
                        "Usage has reached "
                                + cleanThresholdPct
                                + "% - starting cached content cleaner.");
            }
    ...
    private void signalCleanerStart(final String reason, final boolean aggressive) {
        if (aggressive) {
            ...
            if (timePassedFromLastClean < normalCleanThresholdSec * 1000) {
                if (log.isDebugEnabled()) {
                    log.debug(
                            "Skipping a normal clean as it is too soon. The last cleanup was run "
                                    + timePassedFromLastClean / 1000f
                                    + " seconds ago.");
                }
            } else {

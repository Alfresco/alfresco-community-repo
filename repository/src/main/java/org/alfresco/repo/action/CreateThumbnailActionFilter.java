...
/**
 * This filter class is used to identify equivalent create-thumbnail actions. Two create-thumbnail
 * actions are considered equivalent if they are on the same NodeRef and if they have the same
 * thumbnail-name parameter.
 *
...
public class CreateThumbnailActionFilter extends AbstractAsynchronousActionFilter {
    ...
    public int compare(OngoingAsyncAction nodeAction1, OngoingAsyncAction nodeAction2) {
        ...
        if (n1.equals(n2) == false) {
            ...
            String thName1 =
                    (String) nodeAction1.getAction().getParameterValue(PARAM_THUMBNAIL_NAME);
            String thName2 =
                    (String) nodeAction2.getAction().getParameterValue(PARAM_THUMBNAIL_NAME);

            // Need to allow for the possibility of null values for thumbnail-name. (ALF-4946)
            if (thName1 == null && thName2 == null) {
                // Two null-valued thumbnail-names are considered equal under the compareTo
                // contract.
                return 0;

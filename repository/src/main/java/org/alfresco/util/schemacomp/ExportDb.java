...
public class ExportDb {
    private static final String SCHEMA_REFERENCE_SEQUENCES_SQL_FILE =
            "classpath:alfresco/dbscripts/create/${db.script.dialect}/Schema-Reference-Sequences.sql";
    ...
    /**
     * Only top-level tables starting with namePrefix will be exported, set to empty string for all
     * objects
     */
    ...
    private void attemptInit()
        ...
        final Map<Integer, String> forwardMap2 =
                (Map<Integer, String>) defaultsField.get(typeNames);
        ...
        final Map<Integer, Map<Integer, String>> forwardMap1 =
                (Map<Integer, Map<Integer, String>>) weightedField.get(typeNames);
    ...
    private void extractSchema(DatabaseMetaData dbmd, String schemaName, String prefixFilter)
        ...
        // No tables are returned when using MySQL8 - maybe this issue can be solved if we update
        // the MySQL JDBC driver that we use (the new driver class is `com.mysql.cj.jdbc.Driver').
    ...
    private void processTables(DatabaseMetaData dbmd, ResultSet tables)
        ...
        while (tables.next()) {
            ...
            final ResultSet columns =
                    dbmd.getColumns(null, tables.getString("TABLE_SCHEM"), tableName, "%");
            while (columns.next()) {
            ...
            // Primary key - beware that getPrimaryKeys gets primary keys ordered by their column
            // name
            final ResultSet primarykeycols =
                    dbmd.getPrimaryKeys(null, tables.getString("TABLE_SCHEM"), tableName);
            ...
            List<Integer> keySeqSortedColumnOrders =
                    keySeqsAndColumnNames.keySet().stream().sorted().collect(Collectors.toList());
            ...
            // Indexes
            final ResultSet indexes =
                    dbmd.getIndexInfo(
                            null, tables.getString("TABLE_SCHEM"), tableName, false, true);
            ...
            final ResultSet foreignkeys =
                    dbmd.getImportedKeys(null, tables.getString("TABLE_SCHEM"), tableName);
            String lastKeyName = "";
    ...
    protected String generateType(final String dbTypeRaw, int size, final int digits, int sqlType)
        ...
        if (this.reverseTypeMap.containsKey(precisionScaleType)) {
            ...
            for (String key : reverseTypeMap.keySet()) {
                ...
                if (popKey.equals(precisionScaleType)
                        || popKey.equals(sizeType)
                        || popKey.equals(charSizeType)) {
                    return popKey;
                }
            }
        }
    ...
    /**
     * Attempts to find the schema reference sequences sql file. If not found, the dialect hierarchy
     * will be walked until a compatible resource is found. This makes it possible to have resources
     * that are generic to all dialects.
     *
     * @return The Resource, otherwise null
     */
    private Resource getSequencesReferenceResource() {
        return DialectUtil.getDialectResource(
                rpr, dialect.getClass(), SCHEMA_REFERENCE_SEQUENCES_SQL_FILE);
    }

    /**
     * Retrieves and process DB sequences for the schema validation check.
     *
     * @param dbmd the database meta data
     * @param resource the resource to load the SQL script from
     * @param schemaName the DB schema name
     * @param prefixFilter the DB tables prefix filter
     */
    private void retrieveAndProcessSequences(
            DatabaseMetaData dbmd, Resource resource, String schemaName, String prefixFilter)
            throws SQLException, IllegalArgumentException, IllegalAccessException, IOException {
     ...
     */
    private void retrieveAndProcessSequences(
            DatabaseMetaData dbmd, String script, String schemaName, String prefixFilter)
            throws SQLException, IllegalArgumentException, IllegalAccessException {
        if (log.isDebugEnabled()) {

 ...
 *
 * <p>In this case, compiled means that a) all references between model items have been resolved b)
 * inheritence of class features have been flattened c) overridden class features have been resolved
 *
 * <p>A compiled model also represents a valid model.
 *
...
public class CompiledModel implements ModelQuery {
    ...
    private static final String ERR_COMPILE_MODEL_FAILURE =
            "d_dictionary.compiled_model.err.compile.failure";
    private static final String ERR_DUPLICATE_PROPERTY_TYPE =
            "d_dictionary.compiled_model.err.duplicate_property_type";
    private static final String ERR_DUPLICATE_TYPE =
            "d_dictionary.compiled_model.err.duplicate_type";
    private static final String ERR_DUPLICATE_ASPECT =
            "d_dictionary.compiled_model.err.duplicate_aspect";
    private static final String ERR_DUPLICATE_CONSTRAINT =
            "d_dictionary.compiled_model.err.duplicate_constraint";
    ...
    private Map<QName, ConstraintDefinition> constraints =
            new HashMap<QName, ConstraintDefinition>();
        ...
        try {
            ...
            for (ConstraintDefinition def : constraints.values()) {
                ((M2ConstraintDefinition) def)
                        .resolveDependencies(query, enableConstraintClassLoading);
            }
    ...
    private void constructDefinitions(
            M2Model model, NamespaceDAO namespaceDAO, DictionaryDAO dictionaryDAO) {
        NamespacePrefixResolver localPrefixes = createLocalPrefixResolver(model, namespaceDAO);
        ...
        for (M2DataType propType : model.getPropertyTypes()) {
            M2DataTypeDefinition def =
                    new M2DataTypeDefinition(modelDefinition, propType, localPrefixes);
            if (dataTypes.containsKey(def.getName())) {
                throw new DuplicateDefinitionException(
                        ERR_DUPLICATE_PROPERTY_TYPE, propType.getName());
            }
        ...
        for (M2Type type : model.getTypes()) {
            M2TypeDefinition def =
                    new M2TypeDefinition(
                            modelDefinition, type, localPrefixes, properties, associations);
        ...
        for (M2Aspect aspect : model.getAspects()) {
            M2AspectDefinition def =
                    new M2AspectDefinition(
                            modelDefinition, aspect, localPrefixes, properties, associations);
            if (classes.containsKey(def.getName())) {
        ...
        for (M2Constraint constraint : model.getConstraints()) {
            ...
            if (constraints.containsKey(qname)) {
                throw new DuplicateDefinitionException(
                        ERR_DUPLICATE_CONSTRAINT, constraint.getName());
            }
    ...
    /**
     * Create a local namespace prefix resolver containing the namespaces defined and imported in
     * the model
     *
     * @param model model definition
     * @param namespaceDAO namespace DAO
        ...
        for (M2Namespace imported : model.getImports()) {
            ...
            if (model.getNamespace(uri) != null) {
                throw new NamespaceException(
                        "URI "
                                + uri
                                + " cannot be imported as it is already contained in the model's"
                                + " namespaces");
            }
    ...
    private void resolveInheritance(
            ...
            while (parentName != null) {
                ...
                if (traversedNodes.contains(parentClass)) {
                    throw new DictionaryException(
                            ERR_CYCLIC_REFERENCE, parentClass.getName(), model.getName());
                }
                depth = depth + 1;
        ...
        for (int depth = 0; depth < order.size(); depth++) {
            for (ClassDefinition def : order.get(depth)) {
                ((M2ClassDefinition) def)
                        .resolveInheritance(query, prefixResolver, modelConstraints);
            }
        }
    }

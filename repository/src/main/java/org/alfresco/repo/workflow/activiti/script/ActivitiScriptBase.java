...
public class ActivitiScriptBase {
  ...
  protected Object executeScript(
    ...
    if (scriptProcessorName != null) {
      ...
      scriptResult =
        getServiceRegistry()
          .getScriptService()
          .executeScriptString(theScript, model);
    }
  ...
  private void validateRunAsUser(final String runAsUser) {
    Boolean runAsExists = AuthenticationUtil.runAs(
    ...
    if (!runAsExists) {
      throw new WorkflowException(
        "runas user '" + runAsUser + "' does not exist."
      );
    }
  }
  ...
  protected ActivitiScriptNode getPersonNode(String runAsUser) {
    ...
    if (
      userName != null && !AuthenticationUtil.SYSTEM_USER_NAME.equals(userName)
    ) {

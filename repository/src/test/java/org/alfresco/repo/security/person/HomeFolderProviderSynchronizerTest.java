...
public class HomeFolderProviderSynchronizerTest {
    private static final QName PROP_PARENT_PATH =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "parentPath");
    ...
    public static void classSetup() throws Exception {
        ...
        homeFolderManager =
                (PortableHomeFolderManager) applicationContext.getBean("homeFolderManager");
        largeHomeFolderProvider =
                (RegexHomeFolderProvider) applicationContext.getBean("largeHomeFolderProvider");
        ...
        testHomeFolderProvider.setOnCreatePermissionsManager(
                (PermissionsManager)
                        applicationContext.getBean("defaultOnCreatePermissionsManager"));
        testHomeFolderProvider.setOnCreatePermissionsManager(
                (PermissionsManager)
                        applicationContext.getBean("defaultOnCreatePermissionsManager"));
        testHomeFolderProvider.setOnReferencePermissionsManager(
                (PermissionsManager)
                        applicationContext.getBean("defaultOnReferencePermissionsManager"));
    ...
    public void tearDown() throws Exception {
        ...
        RetryingTransactionCallback<Void> cleanup =
    ...
    private NodeRef createUser(String tenantDomain, String parentPath, String username)
            throws Exception {
                ...
                new TenantRunAsWork<NodeRef>() {
                    public NodeRef doWork() throws Exception {
                        ...
                        if (parentPath != null) {
                            ...
                            properties.put(
                                    ContentModel.PROP_HOME_FOLDER_PROVIDER, homeFolderProviderName);
                        }
            ...
            for (String pathElement : path.split("/")) {
                ...
                NodeRef nodeRef =
                        nodeService.getChildByName(
                                parent, ContentModel.ASSOC_CONTAINS, pathElement);
                if (nodeRef == null) {
                    parent =
                            fileFolderService
                    ...
                    public Object doWork() throws Exception {
                        if (!tenantAdminService.existsTenant(tenantDomain)) {
                            tenantAdminService.createTenant(
                                    tenantDomain, ("admin " + tenantDomain).toCharArray(), null);
                        }
                        return null;
                    }
                },
    ...
    private void assertHomeFolderLocation(
            String tenantDomain, final String username, final String expectedPath)
            throws Exception {
        try {
            final String domainUsername = tenantService.getDomainUser(username, tenantDomain);
            TenantUtil.runAsSystemTenant(
    ...
    public void test09KeepNonEmptyParents() throws Exception {
        ...
        assertHomeFolderLocation("peter", "pe/peter");
        assertFalse(
                "Expected the empty parent 'a/bb/ccc' to have been removed.", exists("a/bb/ccc"));
    ...
    public void test11PathAlreadyInUseByContent() throws Exception {
        ...
        assertFalse(
                "Did not expect there to be a folder in the prefered location.", exists("fr/fred"));
    ...
    public void test20OverrideProvider() throws Exception {
        ...
        String providerName =
                (String) nodeService.getProperty(person, ContentModel.PROP_HOME_FOLDER_PROVIDER);
    ...
    public void test21NoOriginalProvider() throws Exception {
        ...
        assertTrue(
                "Expected the empty parent 'a/b/c' to still exist as original root was unknown,"
                        + " because the original home folder provider was not set.",
                exists("a/b/c"));
    }
    ...
    public void test24TenantService() throws Exception {
        ...
        if (tenantAdminService.isEnabled()) {
            ...
            assertHomeFolderLocation(
                    tenant1, "fred", "fr/" + tenantService.getDomainUser("fred", tenant1));
            assertHomeFolderLocation(
                    tenant2, "fred", "fr/" + tenantService.getDomainUser("fred", tenant2));
        }
    }

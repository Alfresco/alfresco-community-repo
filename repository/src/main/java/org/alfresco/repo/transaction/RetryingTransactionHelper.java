...
/**
 * A helper that runs a unit of work inside a UserTransaction, transparently retrying the unit of
 * work if the cause of failure is an optimistic locking or deadlock condition.
 *
 * <p>Defaults:
 *
 * <ul>
 *   <li><b>maxRetries: 20</b>
 *   <li><b>minRetryWaitMs: 100</b>
 *   <li><b>maxRetryWaitMs: 2000</b>
 *   <li><b>retryWaitIncrementMs: 100</b>
 * </ul>
 *
 * <p>To get details of 'why' transactions are retried use the following log level:<br>
...
public class RetryingTransactionHelper {
        ...
        Class<?>[] coreClasses =
                new Class[] {
                    ...
                    JdbcUpdateAffectedIncorrectNumberOfRowsException
                            .class, // Similar to StaleObjectState
                    ...
                    TooManyResultsException
                            .class, // Expected one result but found multiple (bad key alert)
    ...
    /**
     * Use reflection to load a list of enterprise-specific exception classes to add to the core
     * list specified in this class.
     *
     * <p>This is used to decouple this class from enterprise-specific libraries.
     *
    ...
    /**
     * Optional time limit for execution time. When non-zero, retries will not continue when the
     * projected time is beyond this time.
     */
    private long maxExecutionMs;

    /**
     * Map of transaction start times to thread stack traces. Only maintained when maxExecutionMs is
     * set.
     */
    private SortedMap<Long, List<Throwable>> txnsInProgress = new TreeMap<Long, List<Throwable>>();

    /**
     * The number of concurrently exeucting transactions. Only maintained when maxExecutionMs is
     * set.
     */
    ...
    public void setRetryWaitIncrementMs(int retryWaitIncrementMs) {
        if (retryWaitIncrementMs <= 0) {
            throw new IllegalArgumentException(
                    "'retryWaitIncrementMs' must be a positive integer.");
        }
     ...
     *
     * <p><b>NOTE: </b> This method may not be used to circumvent the Alfresco License policy.
     *
     * @param forceWritable <tt>true</tt> to force transactions to be writable regardless of system
     *     read-only mode
     */
    ...
    /**
     * Execute a callback in a transaction until it succeeds, fails because of an error not the
     * result of an optimistic locking failure, or a deadlock loser failure, or until a maximum
     * number of retries have been attempted.
     *
     * <p>If there is already an active transaction, then the callback is merely executed and any
     * retry logic is left to the caller. The transaction will attempt to be read-write.
     *
     ...
     * number of retries have been attempted.
     *
     * <p>It is possible to force a new transaction to be created or to partake in any existing
     * transaction.
     *
     * @param cb The callback containing the unit of work.
     * @param readOnly Whether this is a read only transaction.
     * @param requiresNew <tt>true</tt> to force a new transaction or <tt>false</tt> to partake in
    ...
    public <R> R doInTransaction(
        ...
        if (!requiresNew) {
            ...
            switch (readState) {
                case TXN_READ_ONLY:
                    if (!readOnly) {
                        // The current transaction is read-only, but a writable transaction is
                        // requested
                        throw new AlfrescoRuntimeException(
                                "Read-Write transaction started within read-only transaction");
                    }
                    // We are in a read-only transaction and this is what we require so continue
                    // with it.
                    break;
                case TXN_READ_WRITE:
        ...
        // If we need a new transaction, then we have to check that the read-write request can be
        // served
        if (requiresNew) {
        ...
        // If we are time limiting, set ourselves a time limit and maintain the count of concurrent
        // transactions
        long startTime = 0;
                    ...
                    if (oldestDuration > maxExecutionMs) {
                        throw new TooBusyException(
                                "Too busy: "
                                        ...
                                        + oldestDuration
                                        + " milliseconds",
                                txnsInProgress.get(oldestStart).get(0));
                    }
                }
            ...
            for (int count = 0; count == 0 || count < maxRetries; count++) {
                ...
                try {
                    if (requiresNew) {
                        ...
                        UserTransactionProtectionAdvise advise =
                                new UserTransactionProtectionAdvise();
                        ...
                        // Store the UserTransaction for static retrieval.  There is no need to
                        // unbind it
                    ...
                    if (txn == null) {
                        RuntimeException ee =
                                AlfrescoRuntimeException.makeRuntimeException(
                    ...
                    if (txn != null) {
                        try {
                            ...
                            // We can only rollback if a transaction was started (NOT
                            // NO_TRANSACTION) and
                            // if that transaction has not been rolled back (NOT ROLLEDBACK).
                            // If an exception occurs while the transaction is being created (e.g.
                            ...
                            // A rollback failure should not preclude a retry, but logging of the
                            // rollback failure is required
                            logger.error(
                                    "Rollback failure.  Normal retry behaviour will resume.", e1);
                        }
                    }
                        ...
                        lastException =
                                (e.getCause() instanceof RuntimeException)
                                        ...
                                        : new AlfrescoRuntimeException(
                                                "Exception in Transaction.", e.getCause());
                    } else {
                        lastException =
                    ...
                    if (retryCause == null
                            && extraExceptions != null
                            && !extraExceptions.isEmpty()) {
                        retryCause =
                                ExceptionStackUtil.getCause(
                                        e, extraExceptions.toArray(new Class[] {}));
                    }

                    if (retryCause != null) {
                        // Sleep a random amount of time before retrying.
                        // The sleep interval increases with the number of retries.
                        int sleepIntervalRandom =
                                (count > 0 && retryWaitIncrementMs > 0)
                        ...
                        if (logger.isInfoEnabled() && !logger.isDebugEnabled()) {
                            String msg =
                                    String.format(
                                            "Retrying %s: count %2d; wait: %1.1fs; msg: \"%s\";"
                                                    + " exception: (%s)",
                                            Thread.currentThread().getName(),
                                            count,
                                            (double) sleepInterval / 1000D,
    ...
    /**
     * Sometimes, the exception means retry and sometimes not. The stack of exceptions is also
     * checked for any occurence of {@link DoNotRetryException} and, if found, nothing is returned.
     *
        ...
        if (retryCause == null) {
            ...
            if (retryCause.getCause() != null && retryCause.getCause() != retryCause) {
                ...
                if (retryCause.getMessage().toLowerCase().contains("deadlock")) {
                    // The word "deadlock" is usually an indication that we need to resolve with a
                    // retry.
                    return retryCause;
     ...
     * marked for rollback.
     *
     * <p><b>NOTE:</b> Any attempt to actually commit or rollback the transaction will cause
     * failures.
     *
    ...
    public static UserTransaction getActiveUserTransaction() {
        ...
        // Get the current transaction.  There might not be one if the transaction was not started
        // using
        // this class i.e. it wasn't started with retries.
        UserTransaction txn =
                (UserTransaction) AlfrescoTransactionSupport.getResource(KEY_ACTIVE_TRANSACTION);
        if (txn == null) {
            return null;
        }
        // Done
        ...
        public void before(Method method, Object[] args, Object target) throws Throwable {
            ...
            if (methodName.equals("begin")
                ...
                throw new IllegalAccessException(
                        "The user transaction cannot be manipulated from within the transactional"
                                + " work load");
            }
        }
    }

...
public class SlowContentStoreTest {
  ...
  public void readsAreFasterFromCache() {
    ...
    storeReader.execute();
    assertTrue(
      "First read should take a while",
      storeReader.timeTakenMillis() > 1000
    );
    logger.debug(
      String.format("First read took %ds", storeReader.timeTakenMillis())
    );
    // The content came from the slow backing store...
    assertEquals(
      "This is the content for my slow ReadableByteChannel",
      storeReader.content
    );
    ...
    for (int i = 0; i < 5; i++) {
      ...
      storeReader.execute();
      assertTrue(
        "Subsequent reads should be fast",
        storeReader.timeTakenMillis() < 100
      );
      logger.debug(
        String.format("Cache read took %ds", storeReader.timeTakenMillis())
      );
  ...
  public void writeThroughCacheResultsInFastReadFirstTime() {
    ...
    cachingStore
      .getWriter(new ContentContext(null, "any-url"))
    ...
    assertEquals(
      "Content written from " + getClass().getSimpleName(),
      storeReader.content
    );

...
public interface DictionaryDAO extends ModelQuery {
    ...
    /**
     * Construct an anonymous type that combines a primary type definition and and one or more
     * aspects
     *
    ...
    /**
     * Adds a model to the dictionary. The model is compiled and validated. Constraints are not
     * loaded.
     *
     * <p>This method should only be used to load models where the enforcement of constraints is
     * never required. For example, SOLR read only use of the index where contraints are not
     * required and thier definitions may not be available.
     *
     * @param model the model to add
     * @return QName name of model
     */
    ...
    /**
     * Get all properties for the model and that are of the given data type. If dataType is null
     * then the all properties will be returned.
     *
    ...
    /**
     * @param modelName the model to retrieve namespaces for
    ...
    /**
     * @param model the model to retrieve constraint defs (optionally only referenceable
     *     constraints)
     * @return the constraints of the model
     */
    ...
    /**
     * Reset the Dictionary for the current tenant. The current dictionary will be discarded <b>and
     * reloaded</b> before the method returns i.e. upon return the dictionary will be current.
     */
    void reset();

    /**
     * Initialise a reload of the dictionary for the current tenant. The current version of the
     * dictionary will be accessible during this call, however it will only return once the
     * dictionary has undergone a reload for the current tenant.
     */
    void init();

    /**
     * Destroy the Dictionary. After this call, there will be no dictionary available for the
     * current tenant; reloading will be done lazily as required.
     *
     * <p><strong>WARNING: </strong>This method can cause 'stutter' on user threads as they wait for

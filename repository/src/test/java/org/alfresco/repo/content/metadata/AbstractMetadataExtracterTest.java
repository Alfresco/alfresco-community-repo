...
public abstract class AbstractMetadataExtracterTest extends TestCase {
    ...
    protected void testExtractFromMimetype(String mimetype) throws Exception {
        try {
            ...
            assertFalse(
                    "extractFromMimetype should return at least some properties, none found for "
                            + mimetype,
                    properties.isEmpty());
            ...
            System.err.println(
                    "No test file found for mime type "
                            + mimetype
                            + ", skipping extraction test - "
                            + e.getMessage());
        }
    }
    ...
    /**
     * Tests that we can get the common metadata correctly from the file. You only need to override
     * this if your test data file doesn't have the usual Nevin Nollop/quick brown fox data in it.
     */
        ...
        if (!skipAuthorCheck(mimetype)) {
            if (properties.containsKey(ContentModel.PROP_CREATOR)) {
                ...
                assertEquals(
                        ...
                        QUICK_CREATOR,
                        DefaultTypeConverter.INSTANCE.convert(
                                String.class, properties.get(ContentModel.PROP_AUTHOR)));
            } else {
                fail(
                        "Expected one property out of "
                                + ContentModel.PROP_CREATOR
                                + " and "
                                + ContentModel.PROP_AUTHOR
                                + " but found neither of them for "
                                + mimetype);
            }
        }
    ...
    protected abstract void testFileSpecificMetadata(
            String mimetype, Map<QName, Serializable> properties);

    /**
     * This method can be overridden to cause the author/creator property check to be skipped. The
     * default behaviour is for the check not to be skipped for all MIME types.
     *
    ...
    /**
     * This method can be overridden to cause the description property check to be skipped. The
     * default behaviour is for the check not to be skipped for all MIME types.
     *
     * @param mimetype String
     * @return <code>true</code> to skip the checks, else <code>false</code>
    ...
    public void testZeroLengthFile() throws Exception {
        ...
        // Note: Normally the reader would need to be fetched for each access, but we need to be
        // sure

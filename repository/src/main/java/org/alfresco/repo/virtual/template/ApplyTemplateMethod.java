...
/**
 * Creates a {@link VirtualFolderDefinition} by executing the template indicated by a virtualized
 * entity reference.
 *
 * @author Bogdan Horje
 */
public class ApplyTemplateMethod extends AbstractProtocolMethod<VirtualFolderDefinition> {
    ...
    public VirtualFolderDefinition execute(VirtualProtocol virtualProtocol, Reference reference)
        ...
        return execute(virtualProtocol, reference, context);
    }

    private VirtualContext createVirtualContext(Reference reference)
            throws ProtocolMethodException {
        return new VirtualContext(
                environment, reference.execute(new GetActualNodeRefMethod(environment)));
    }

    public VirtualFolderDefinition execute(
            VirtualProtocol virtualProtocol, Reference reference, VirtualContext context)
            throws ProtocolMethodException {
        Resource resource = reference.getResource();

        try {
            VirtualFolderDefinition theStructure =
                    resource.processWith(new TemplateResourceProcessor(context));
            ...
            if (!path.isEmpty()) {
                ...
                for (int i = startIndex; i < pathElements.length; i++) {
                    ...
                    if (theStructure == null) {
                        ...
                        throw new ProtocolMethodException(
                                "Invalid template path in " + reference.toString());
                    }
                }
            }
    ...
    public VirtualFolderDefinition execute(VanillaProtocol vanillaProtocol, Reference reference)
            throws ProtocolMethodException {
        InputStream vanillaIS =
                reference.execute(new GetVanillaScriptInputStreamMethod(environment));
        try {
            ...
            try {
                if (vanillaIS != null) vanillaIS.close();
    ...
    /**
     * Creates an empty {@link VirtualFolderDefinition} parameterized with a {@link NullFilingRule}
     * as this method is called for non-virtual nodes.
     *
     * @param protocol
     * @param reference
     ...
     */
    @Override
    public VirtualFolderDefinition execute(NodeProtocol protocol, Reference reference)
            throws ProtocolMethodException {

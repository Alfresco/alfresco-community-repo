...
public class ModuleVersionNumber implements Externalizable {
  ...
  public static final ModuleVersionNumber VERSION_ZERO = new ModuleVersionNumber(
    "0"
  );
  public static final ModuleVersionNumber VERSION_BIG = new ModuleVersionNumber(
    "999.999.999.99"
  );
  ...
  private static Pattern A_M_RC_VERSION_PATTERN = Pattern.compile(
    "((\\d+\\.){2,3}\\d+)(-(A|M|RC)\\d+)?"
  );
  ...
  public int hashCode() {
    ...
    int result = 1;
    result =
      prime * result + ((this.delegate == null) ? 0 : this.delegate.hashCode());
    return result;
  }

 ...
 *
 * <p><b><u>V3.2 Configuration</u>:</b> As of V3.2, the pre- and post-invocation values are passed
 * to the audit component for processing. Individual applications have to extract the desired audit
 * values. Values are audited before and after the invocation so that applications that desire to
 * extract derived data before the invocation can have a chance to do so; generally, however, the
 * post-invocation values will be the most useful.
 *
 * <p>The values passed to the audit component (assuming auditing is enabled and the new
 * configuration is being used) are:
 *
 * <pre>
 * /alfresco-api
 *    /pre
 ...
 *
 * <p>TODO: Audit configuration mapping needs to support conditionals
 *
 * @author Andy Hind
 * @author Derek Hulley
 */
public class AuditMethodInterceptor implements MethodInterceptor {
    ...
    private static final Class[] NON_RECORDABLE_CLASSES =
    ...
    /**
     * Allow the given method invocation to proceed, auditing values before invocation and after
     ...
     * @since 3.2
     */
    private Map<String, Serializable> getInvocationArguments(
            Auditable auditableDef, Object[] args) {
    ...
    /**
     * Returns a value that may be included in audit map data. Called for method arguments and
     * return values. Not all values may be audited. Some cannot be converted to something that can
     * be serialised or may only be read once (so would be damaged). String audit values may be
     * trimmed.
     *
     * @param value to be audited.
     * @return Return {@link #NOT_RECORDABLE} if it is not possible to audit the value,a value that
     *     may be audited. May be null.
        ...
        if (instanceofNonAuditClass(value)) {
            ...
            // trim MLText
                ...
                for (Locale locale : locales) {
                    mltext.put(
                            locale,
                            SchemaBootstrap.trimStringForTextFields(mltext.getValue(locale)));
                }
            }
        }
        return audit;
        ...
        if (namedArguments == null || namedArguments.isEmpty()) {
            ...
            rootPath =
                    AuditApplication.buildPath(
                            AUDIT_PATH_API_PRE, serviceName, methodName, AUDIT_SNIPPET_ARGS);
            auditData = namedArguments;
        }

        // Audit in a read-write txn
        Map<String, Serializable> auditedData =
                auditComponent.recordAuditValues(rootPath, auditData);
        // Done
    ...
    private void auditInvocationAfter(
        ...
        final String rootPath =
                AuditApplication.buildPath(AUDIT_PATH_API_POST, serviceName, methodName);
            ...
            threadPoolExecutor.execute(
                    new Runnable() {
                        public void run() {
                            ...
                            auditData.put(
                                    AUDIT_SNIPPET_ERROR,
                                    SchemaBootstrap.trimStringForTextFields(sb.toString()));
                            ...
                            RetryingTransactionCallback<Map<String, Serializable>> auditCallback =
                                    new RetryingTransactionCallback<Map<String, Serializable>>() {
                                        public Map<String, Serializable> execute()
                                                throws Throwable {
                                            // Record thrown exceptions regardless of userFilter in
                                            // case of failed authentication
                                            // see MNT-11760
                                            if (thrown instanceof AuthenticationException) {
                                                return auditComponent
                                                        .recordAuditValuesWithUserFilter(
                                                                rootPath, auditData, false);
                                            } else {
                                                return auditComponent.recordAuditValues(
                                                        rootPath, auditData);
                                            }
                                        }
                                    };

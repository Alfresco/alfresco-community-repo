...
public class ScheduledPersistedActionServiceImpl
  ...
  protected static final Set<QName> ACTION_TYPES = new HashSet<QName>(
    Arrays.asList(new QName[] { ActionModel.TYPE_ACTION_SCHEDULE })
  );

  protected static final String SCHEDULER_GROUP = "PersistedActions";

  private static final Log log = LogFactory.getLog(
    ScheduledPersistedActionServiceImpl.class
  );
  ...
  protected void locatePersistanceFolder() {
    ...
    if (scheduledAssocs.size() == 0) {
      throw new AlfrescoRuntimeException(
        "Failed to find 'cm:Scheduled Actions' location."
      );
    }
  ...
  private void updatePersistentSchedule(ScheduledPersistedActionImpl schedule) {
    ...
    if (nodeRef == null) throw new IllegalStateException(
    ...
    nodeService.setProperty(
      nodeRef,
      ActionModel.PROP_START_DATE,
      schedule.getScheduleStart()
    );
    nodeService.setProperty(
      nodeRef,
      ActionModel.PROP_INTERVAL_COUNT,
      schedule.getScheduleIntervalCount()
    );
    IntervalPeriod period = schedule.getScheduleIntervalPeriod();
    nodeService.setProperty(
      nodeRef,
      ActionModel.PROP_INTERVAL_PERIOD,
      period == null ? null : period.name()
    );
  ...
  private List<ScheduledPersistedAction> listSchedules(
    ...
    for (ChildAssociationRef actionAssoc : childAssocs) {
      ScheduledPersistedActionImpl scheduleImpl = loadPersistentSchedule(
  ...
  protected ScheduledPersistedActionImpl loadPersistentSchedule(
    NodeRef schedule
  ) {
    ...
    // create schedule
    ScheduledPersistedActionImpl scheduleImpl = new ScheduledPersistedActionImpl(
      action
    );
    ...
    String period = (String) nodeService.getProperty(
      schedule,
      ActionModel.PROP_INTERVAL_PERIOD
    );
    if (period != null) {
        ...
        .doInTransaction(
          new RetryingTransactionCallback<Void>() {
            public Void execute() throws Throwable {
              ...
              actionService.executeAction(action, (NodeRef) null, false, true);

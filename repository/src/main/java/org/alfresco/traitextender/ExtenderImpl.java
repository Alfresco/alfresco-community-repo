...
public class ExtenderImpl extends Extender {
    ...
    private Map<ExtensionPoint<?, ?>, ExtensionFactory<?>> pointFactories =
            new ConcurrentHashMap<ExtensionPoint<?, ?>, ExtensionFactory<?>>();
    ...
    public synchronized <E, M extends Trait> E getExtension(
            Extensible anExtensible, ExtensionPoint<E, M> point) {
        E extension = null;
        ...
        if (factory != null) {
            ...
            if (extension == null) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "trait extension leak trace : "
                                    + System.identityHashCode(extension)
                                    + " : "
                                    + System.identityHashCode(exTrait)
                                    + " : "
                                    + System.identityHashCode(extension));
                }
            }
    ...
    public synchronized void register(ExtensionPoint<?, ?> point, ExtensionFactory<?> factory) {
        ...
        if (!factory.canCreateExtensionFor(point)) {
            throw new InvalidExtension(
                    "Invalid extension factory registry entry : " + point + "->" + factory);
        }
        pointFactories.put(point, factory);
    }

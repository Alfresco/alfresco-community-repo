...
public class ThumbnailServiceImpl
    ...
    /** Error messages */
    private static final String ERR_NO_PARENT =
            "Thumbnail has no parent so update cannot take place.";
    ...
    public void init() {
        policyComponent.bindClassBehaviour(
                ...
                new JavaBehaviour(
                        this, "onCreateNode", Behaviour.NotificationFrequency.EVERY_EVENT));
        policyComponent.bindClassBehaviour(
                NodeServicePolicies.BeforeCreateNodePolicy.QNAME,
                ContentModel.TYPE_FAILED_THUMBNAIL,
                new JavaBehaviour(
                        this, "beforeCreateNode", Behaviour.NotificationFrequency.EVERY_EVENT));
        policyComponent.bindClassBehaviour(
    ...
    public void beforeCreateNode(
        ...
        // When a thumbnail has failed, we must delete any existing (successful) thumbnails of that
        // thumbnailDefinition.
        if (ContentModel.TYPE_FAILED_THUMBNAIL.equals(nodeTypeQName)) {
            ...
            List<ChildAssociationRef> existingThumbnails =
                    nodeService.getChildAssocs(parentRef, childNodeTypes);
        ...
        // We can't respond to the creation of a cm:thumbnail node at this point as they are created
        // with
    ...
    public void onCreateNode(ChildAssociationRef childAssoc) {
        ...
        String thumbnailName =
                (String) nodeService.getProperty(thumbnailNodeRef, ContentModel.PROP_NAME);
        ...
        if (nodeService.exists(thumbnailNodeRef)) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Caching "
                                + childAssoc
                                + " in transaction resources, thumbnail "
                                + thumbnailName);
            }
            ...
            // For new version/content the child association contains a temporary thumbnail node,
            // that gets removed later.
            // We need to add the original thumbnail node to the TransactionalResourceHelper
            // instead.
            QName thumbnailQname =
                    QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, thumbnailName);
            if (!childAssoc.getQName().equals(thumbnailQname)) {
                List<ChildAssociationRef> allAssocList =
        ...
        if (existingFailedThumbnail != null) {
            if (logger.isDebugEnabled()) {
                StringBuilder msg = new StringBuilder();
                msg.append("Deleting failedThumbnail node ")
                        .append(existingFailedThumbnail.getFailedThumbnailNode());
                logger.debug(msg.toString());
            }
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.thumbnail.ThumbnailService#createThumbnail(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.namespace.QName, java.lang.String,
     *     org.alfresco.service.cmr.repository.TransformationOptions, java.lang.String)
     */
    public NodeRef createThumbnail(
            NodeRef node,
        ...
        return createThumbnail(
                node, contentProperty, mimetype, transformationOptions, thumbnailName, null);
    }
    ...
    public NodeRef createThumbnail(
        ...
        if (logger.isDebugEnabled() == true) {
            logger.debug(
                    "Creating thumbnail (node="
                            + node.toString()
                            + "; contentProperty="
                            + contentProperty.toString()
                            + "; mimetype="
                            + mimetype);
        }
        ...
        if (thumbnailName != null) {
            ...
            if (existingThumbnail != null) {
                if (logger.isDebugEnabled() == true) {
                    logger.debug(
                            "Creating thumbnail: There is already a thumbnail with the name '"
                                    + thumbnailName
                                    + "' (node="
                                    + node.toString()
                                    + "; contentProperty="
    ...
    private QName getThumbnailQName(String localThumbnailName) {
        ...
        QName thumbnailQName =
                QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, localThumbnailName);
    ...
    /**
     * This method returns the NodeRef for the thumbnail, using the ChildAssociationRef that links
     * the sourceNode to its associated Thumbnail node.
     *
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.thumbnail.ThumbnailService#updateThumbnail(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.cmr.repository.TransformationOptions)
     */
    public void updateThumbnail(
            final NodeRef thumbnail, final TransformationOptions transformationOptions) {
        if (logger.isDebugEnabled() == true) {
        ...
        if (renditionService.isRendition(thumbnail)) {
            ...
            if (parentAssoc == null) {
                if (logger.isDebugEnabled() == true) {
                    logger.debug(
                            "Updating thumbnail: The thumbnails parent cannot be found (thumbnail="
                                    + thumbnail.toString()
                                    + ")");
                }
            ...
            // Do the thumbnail transformation. Rendition Definitions are persisted underneath the
            // Data Dictionary for which Group ALL
            ...
            if (rendDefn == null) {
                ...
                rendDefn =
                        renditionService.createRenditionDefinition(
                                parentAssoc.getQName(), renderingEngineName);
            }
            ...
            if (logger.isDebugEnabled() == true) {
                logger.debug(
                        "Updating thumbnail: cannot update a thumbnail node that isn't the correct"
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.thumbnail.ThumbnailService#getThumbnailByName(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.namespace.QName, java.lang.String)
     */
    public NodeRef getThumbnailByName(NodeRef node, QName contentProperty, String thumbnailName) {
        ...
        // NOTE:
        //
        // Since there is not an easy alternative and for clarity the node service is being used to
        ...
        if (logger.isDebugEnabled() == true) {
            logger.debug(
                    "Getting thumbnail by name (nodeRef="
                            + node.toString()
                            + "; contentProperty="
                            + contentProperty.toString()
                            + "; thumbnailName="
        ...
        QName namespacedThumbnailName =
                QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, thumbnailName);

        ChildAssociationRef existingRendition =
                renditionService.getRenditionByName(node, namespacedThumbnailName);
        NodeRef thumbnail = null;

        // Check the child to see if it matches the content property we are concerned about.
        // We can assume there will only ever be one per content property since createThumbnail
        // enforces this.
        if (existingRendition != null) {
            NodeRef child = existingRendition.getChildRef();
            Serializable contentPropertyName =
                    this.nodeService.getProperty(child, ContentModel.PROP_CONTENT_PROPERTY_NAME);
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.thumbnail.ThumbnailService#getThumbnails(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.namespace.QName, java.lang.String,
     *     org.alfresco.service.cmr.repository.TransformationOptions)
     */
    public List<NodeRef> getThumbnails(
        ...
        for (ChildAssociationRef assoc : renditions) {
            ...
            if (contentProperty.equals(
                                    this.nodeService.getProperty(
                                            child, ContentModel.PROP_CONTENT_PROPERTY_NAME))
                            == true
    ...
    private boolean validateThumbnail(NodeRef thumbnailNode) {
        ...
        ContentData content =
                (ContentData)
                        this.nodeService.getProperty(thumbnailNode, ContentModel.PROP_CONTENT);
        // (MNT-17162) A thumbnail with an empty content is cached for post-transaction removal, to
        // prevent the delete in read-only transactions.
    ...
    public Map<String, FailedThumbnailInfo> getFailedThumbnails(NodeRef sourceNode) {
        ...
        if (nodeService.hasAspect(sourceNode, ContentModel.ASPECT_FAILED_THUMBNAIL_SOURCE)) {
            List<ChildAssociationRef> failedThumbnailChildren =
                    nodeService.getChildAssocs(
                            sourceNode,
            ...
            for (ChildAssociationRef chAssRef : failedThumbnailChildren) {
                ...
                final FailedThumbnailInfo failedThumbnailInfo =
                        new FailedThumbnailInfo(
                                failedThumbnailName.getLocalName(),
                                failureDateTime,
                                failureCount,
                                failedThumbnailNode);
     ...
     *
     * <p>If mimetype and transformation options are null then match is guarenteed
     *
     * @param thumbnail thumbnail node reference
     * @param mimetype mimetype
     * @param options transformation options
     * @return boolean true if the mimetype and options match the thumbnail metadata, false
     *     otherwise
     */
    private boolean matchMimetypeOptions(
            NodeRef thumbnail, String mimetype, TransformationOptions options) {
        boolean result = true;
        ...
        if (mimetype != null) {
            ...
            String thumbnailMimetype =
                    ((ContentData)
                                    this.nodeService.getProperty(
                                            thumbnail, ContentModel.PROP_CONTENT))
                            .getMimetype();

            if (mimetype.endsWith(SUBTYPES_POSTFIX)) {
                String baseMimetype =
                        mimetype.substring(0, mimetype.length() - SUBTYPES_POSTFIX.length());
                if (thumbnailMimetype == null
                        || thumbnailMimetype.startsWith(baseMimetype) == false) {
                    result = false;
                }
            } else {
     ...
     */
    private RenditionDefinition createRawRenditionDefinition(
            QName thumbnailQName, final TransformationOptions transformationOptions) {
        // Create the renditionDefinition
        String renderingEngineName = getRenderingEngineNameFor(transformationOptions);
        RenditionDefinition definition =
                renditionService.createRenditionDefinition(thumbnailQName, renderingEngineName);
    ...
    private RenditionDefinition createRenditionDefinition(
            ...
            final String mimetype,
            final TransformationOptions transformationOptions,
            final QName thumbnailQName,
            final ThumbnailParentAssociationDetails assocDetails) {
        RenditionDefinition definition =
                createRawRenditionDefinition(thumbnailQName, transformationOptions);
        ...
        Map<String, Serializable> params =
                thumbnailRegistry
                        .getThumbnailRenditionConvertor()
                        .convert(transformationOptions, assocDetails);
    ...
    private NodeRef createThumbnailNode(
            final NodeRef node,
            final QName contentProperty,
            final String mimetype,
            final TransformationOptions transformationOptions,
    ...
    /**
     * Updates the parent of the supplied {@link NodeRef} to ensure that it has the
     * "cm:thumbnailModification" aspect and sets the last modification data for it.
     *
     * @param nodeRef A {@link NodeRef} representing a thumbnail to provide last modification data
     *     for.
     */
                ...
                if (modified != null) {
                    ...
                    for (ChildAssociationRef parent : nodeService.getParentAssocs(nodeRef)) {
                        ...
                        behaviourFilter.disableBehaviour(
                                parentNode, ContentModel.ASPECT_VERSIONABLE);
                        try {
                            if (nodeService.hasAspect(
                                    parentNode, ContentModel.ASPECT_THUMBNAIL_MODIFICATION)) {
                                // The node already has the aspect, check to see if the current
                                // thumbnail modification exists...
                                thumbnailMods =
                                        (List<String>)
                                                nodeService.getProperty(
                                                        parentNode,
                                ...
                                // If we have previously set last modified thumbnail data then it
                                // will exist as part of the multi-value
                                // property. The value will consist of the "cm:thumbnailName" value
                                // delimited with a ":" and then the
                                // timestamp. We need to find the appropriate entry in the
                                // multivalue property and then update it
                                String target = null;
                                ...
                                if (logger.isDebugEnabled()) {
                                    logger.debug(
                                            "Setting thumbnail last modified date to "
                                                    + lastModifiedValue
                                                    + " on parent node: "
                                                    + parentNode);
                                }
                                ...
                                try {
                                    nodeService.setProperty(
                                            ...
                                            (Serializable) thumbnailMods);
                                } finally {
                                    ruleService.enableRuleType(RuleType.UPDATE);
                                }
                            } else {
                                // If the aspect has not previously been added then we'll need to
                                // set it now...
                            ...
                            behaviourFilter.enableBehaviour(
                                    parentNode, ContentModel.ASPECT_AUDITABLE);
                            behaviourFilter.enableBehaviour(
                                    parentNode, ContentModel.ASPECT_VERSIONABLE);
                        }
                    }
    ...
    private class ThumbnailTransactionListenerAdapter extends TransactionListenerAdapter {
        ...
        public void afterCommit() {
            ...
            final Set<ChildAssociationRef> childAssocs =
                    TransactionalResourceHelper.getSet(THUMBNAIL_PARENT_NODES);

            AuthenticationUtil.runAs(
                    new AuthenticationUtil.RunAsWork<Void>() {
                        @Override
                        public Void doWork() throws Exception {
                            // MNT-15135: Do the property update in a new transaction, in case the
                            // parent node was already changed (has a different version) in another
                            // transaction.
                                        ...
                                        public Void execute() throws Throwable {
                                            for (ChildAssociationRef childAssoc : childAssocs) {
                                                ...
                                                if (thumbnailNodeRef == null
                                                        || !nodeService.exists(thumbnailNodeRef)) {
                                                    logger.debug(
                                                            "Thumbnail node "
                                                                    + thumbnailNodeRef
                                                ...
                                                if (sourceNodeRef == null
                                                        || !nodeService.exists(sourceNodeRef)) {
                                                    logger.debug(
                                                            "Parent node "
                                                                    + sourceNodeRef
                                                                    + " does not exist. It will be"
                                                ...
                                                // Update the parent node with the thumbnail
                                                // update...
                                                if (logger.isDebugEnabled()) {
                                                    logger.debug(
                                                            "Found cached parent node "
                                                                    + sourceNodeRef
                                                                    + " in transactional"
                                                                    + " resources");
                                                    logger.debug(
                                                            "Adding thumbnail modification data.");
                                                }
                                                addThumbnailModificationData(
                                                        thumbnailNodeRef, thumbnailName);
                                            }
                                            return null;
                                        }
                                    },
    ...
    private class ThumbnailCleanupTransactionListenerAdapter extends TransactionListenerAdapter {
        ...
        public void afterCommit() {
            ...
            final Set<NodeRef> thumbnailToDelete =
                    TransactionalResourceHelper.getSet(THUMBNAIL_TO_DELETE_NODES);
                                    ...
                                    new RetryingTransactionCallback<Void>() {
                                        ...
                                        public Void execute() throws Throwable {
                                            for (NodeRef node : thumbnailToDelete) {
                                                ...
                                                // so that the thumbnail will be recreated when
                                                // browsing share
                                                String thumbnailName =
                                                        (String)
                                                                nodeService.getProperty(

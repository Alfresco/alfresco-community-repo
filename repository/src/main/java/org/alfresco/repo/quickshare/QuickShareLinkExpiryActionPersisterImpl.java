...
/**
 * Default implementation of the {@link QuickShareLinkExpiryActionPersister}. It is responsible for
 * persisting and retrieving the quick share link expiry actions.
 *
 * @author Jamal Kaabi-Mofrad
 */
public class QuickShareLinkExpiryActionPersisterImpl
        implements QuickShareLinkExpiryActionPersister {
    protected static final NodeRef QUICK_SHARE_LINK_EXPIRY_ACTIONS_ROOT =
            new NodeRef(
                    StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, "shared_link_expiry_actions_space");
    ...
    public NodeRef getQuickShareLinkExpiryActionNode(QName linkExpiryActionName) {
        ...
        List<ChildAssociationRef> childAssocs =
                nodeService.getChildAssocs(
                        rootNodeRef, ContentModel.ASSOC_CONTAINS, linkExpiryActionName);
        if (!childAssocs.isEmpty()) {
            if (childAssocs.size() > 1) {
                throw new QuickShareLinkExpiryActionException(
                        "Multiple quick share link expiry actions with the name: "
    ...
    @Override
    public QuickShareLinkExpiryAction loadQuickShareLinkExpiryAction(
            NodeRef linkExpiryActionNodeRef) {
    ...
    /**
     * Gets the folder containing quick share link expiry action nodes. If it doesn't exist then it
     * tries to create it.
     *
    ...
    private NodeRef getOrCreateActionsRootNodeRef() {
        if (!nodeService.exists(QUICK_SHARE_LINK_EXPIRY_ACTIONS_ROOT)) {
            ...
            if (!nodeService.exists(QUICK_SHARE_LINK_EXPIRY_ACTIONS_ROOT)) {
                throw new QuickShareLinkExpiryActionException(
                        "Couldn't import the quick share link expiry actions root node.");
            }
        }

...
public class CachingContentStoreSpringTest
  ...
  public void before() throws Exception {
    ...
    cache.setCacheRoot(
      TempFileProvider.getLongLifeTempDir("cached_content_test")
    );
  ...
  public void testStoreWillReadFromCacheWhenAvailable() {
    ...
    assertFalse(
      "Original content should have been deleted",
      backingStore.exists(contentUrl)
    );
  ...
  public void testCacheOnInbound() {
    ...
    ContentWriter writer = store.getWriter(
      new ContentContext(null, contentUrl)
    );
    writer.putContent(content);

    assertTrue(
      "Cache should contain content after write",
      cache.contains(contentUrl)
    );
    ...
    String fromBackingStore = backingStore
      .getReader(contentUrl)
      .getContentString();
    assertEquals(
      "Content should be in backing store",
      content,
      fromBackingStore
    );
  ...
  public void testQuotaOnOverlimitCacheFile() {
    ...
    assertFalse(
      "Overlimit content should be deleted from cache",
      cache.contains(contentUrl)
    );
    assertFalse(
      "Overlimit content should be deleted from cache",
      writer.getReader().exists()
    );
  ...
  public void testBackingStoreBehavesLikeCenteraContentStore() {
    ...
    assertEquals(
      TestCenteraLikeContentWriter.UNKNOWN_ID,
      writer1.getContentData().getContentUrl()
    );
    ...
    assertEquals(
      TestCenteraLikeContentWriter.UNKNOWN_ID,
      writer2.getContentData().getContentUrl()
    );
    ...
    ContentReader reader = store.getReader(
      writer1.getContentData().getContentUrl()
    );

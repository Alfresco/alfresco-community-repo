...
public class Aspect
  extends org.alfresco.rest.api.model.Aspect
  implements Serializable, ExpectedComparison {

  @Override
  public void expected(Object model) {
    assertTrue(
      "model is an instance of " + model.getClass(),
      model instanceof Aspect
    );
    ...
    if (getModel() != null && other.getModel() != null) {
      AssertUtil.assertEquals(
        "modelId",
        getModel().getId(),
        other.getModel().getId()
      );
      AssertUtil.assertEquals(
        "author",
        getModel().getAuthor(),
        other.getModel().getAuthor()
      );
      AssertUtil.assertEquals(
        "namespaceUri",
  ...
  public static Aspect parseAspect(JSONObject jsonObject) {
    ...
    List<PropertyDefinition> properties = (List<PropertyDefinition>) jsonObject.get(
      "properties"
    );
    List<String> mandatoryAspects = jsonObject.get("mandatoryAspects") != null
    ...
    Boolean includedInSupertypeQuery = (Boolean) jsonObject.get(
      "includedInSupertypeQuery"
    );
    ...
    if (jsonObject.get("associations") != null) {
      ...
      for (int i = 0; i < jsonArray.size(); i++) {
        ...
        if (sourceModel != null) {
          ...
          source.setIsMandatoryEnforced(
            (Boolean) sourceModel.get("isMandatoryEnforced")
          );
        ...
        JSONObject targetModel = (JSONObject) object.get("target");
        {
          ...
          target.setIsMandatoryEnforced(
            (Boolean) targetModel.get("isMandatoryEnforced")
          );
  ...
  @SuppressWarnings("unchecked")
  public static PublicApiClient.ListResponse<Aspect> parseAspects(
    ...
    PublicApiClient.ExpectedPaging paging = PublicApiClient.ExpectedPaging.parsePagination(
      jsonList
    );

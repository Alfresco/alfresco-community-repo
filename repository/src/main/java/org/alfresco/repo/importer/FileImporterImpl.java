...
public class FileImporterImpl implements FileImporter {
  ...
  public int loadFile(NodeRef container, File file)
    throws FileImporterException {
    try {
      Counter counter = new Counter();
  ...
  private NodeRef create(
    ...
    if (file.isDirectory()) {
      ...
      if (recurse) {
        File[] files =
          ((filter == null) ? file.listFiles() : file.listFiles(filter));
  ...
  private NodeRef createFile(NodeRef parentNodeRef, File file) {
    ...
    contentProps.put(
      ...
      currentUser == null ? "unknown" : currentUser
    );
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Created file node for file: " + file.getName()
    );
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Added titled aspect with properties: " + titledProps
    );
    ...
    try {
      ...
      throw new FileImporterException(
        "Failed to load content from " + file.getPath(),
        e
      );
    } catch (FileNotFoundException e) {
      throw new FileImporterException(
        "Failed to load content (file not found) " + file.getPath(),
        e
      );
    }
  ...
  private NodeRef createDirectory(
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Created folder node with name: " + name
    );
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Added uifacets aspect with properties: " + uiFacetsProps
    );

...
public class PropertyValueDAOImpl extends AbstractPropertyValueDAOImpl {
  ...
  private static final String SELECT_PROPERTY_BY_ID =
    "alfresco.propval.select_PropertyById";
  ...
  private static final String UPDATE_PROPERTY_ROOT =
    "alfresco.propval.update_PropertyRoot";
  ...
  private static final String INSERT_PROPERTY_LINK =
    "alfresco.propval.insert_PropertyLink";
  ...
  protected PropertySerializableValueEntity findSerializableValueById(Long id) {
    ...
    entity.setId(id);
    entity =
      template.selectOne(SELECT_PROPERTY_SERIALIZABLE_VALUE_BY_ID, entity);
    // Done
    return entity;
  }

  @Override
  protected PropertySerializableValueEntity createSerializableValue(
    Serializable value
  ) {
    PropertySerializableValueEntity entity = new PropertySerializableValueEntity();
  ...
  protected PropertyValueEntity findPropertyValueById(Long id) {
    ...
    if (size == 0) {
      ...
      logger.error(
        "Found property value linked to multiple raw types: " + results
      );
      return results.get(0);
    }
  }
    ...
    switch (persistedType) {
      ...
      default:
        throw new IllegalStateException(
          "Unhandled PersistedType value: " + persistedType
        );
    }
  ...
  private PropertyValueEntity createPropertyValueInternal(Serializable value) {
    ...
    switch (insertEntity.getPersistedTypeEnum()) {
      case DOUBLE:
        ...
        Pair<Long, Double> insertDoublePair = getOrCreatePropertyDoubleValue(
          doubleValue
        );
      ...
      case ENUM:
        ...
        Pair<Long, String> insertStringPair = getOrCreatePropertyStringValue(
          stringValue
        );
        insertEntity.setLongValue(insertStringPair.getFirst());
        break;
      case SERIALIZABLE:
        Pair<Long, Serializable> insertSerializablePair = createPropertySerializableValue(
          value
        );
  ...
  private static final String[] KEY_COLUMNS_FINDBYIDS = new String[] {
    "propId",
  };

  @Override
  protected void findPropertiesByIds(
    List<Long> ids,
    final PropertyFinderCallback callback
  ) {
  ...
  protected void getPropertyUniqueContextByValues(
    final PropertyUniqueContextCallback callback,
    Long... valueIds
  ) {
    PropertyUniqueContextEntity entity = new PropertyUniqueContextEntity();
    ...
    template.select(
      SELECT_PROPERTY_UNIQUE_CTX_BY_VALUES,
      entity,
      valueResultHandler
    );
    // Done

...
public class OnPropertyUpdateRuleTrigger
  ...
  private boolean havePropertiesBeenModified(
    ...
    for (QName name : keys) {
      ...
      if (
        name.equals(ContentModel.PROP_PREFERENCE_VALUES) ||
      ...
      PropertyDefinition propertyDefinition =
        this.dictionaryService.getProperty(name);
      ...
      // Ignore protected properties
        ...
        if (
          ...
          if (this.ignoreEmptyContent) {
            ...
            if (
              !ContentData.hasContent(beforeContent) ||
              beforeContent.getSize() == 0
            ) {
              beforeValue = null;
            }
            if (
              !ContentData.hasContent(afterContent) ||
              afterContent.getSize() == 0
            ) {
              afterValue = null;
            }
          }
  ...
  public void onUpdateProperties(
    ...
    Set<NodeRef> newNodeRefSet = TransactionalResourceHelper.getSet(
      RULE_TRIGGER_NEW_NODES
    );
    boolean wasCreatedInTxn = newNodeRefSet.contains(nodeRef);
    if (wasCreatedInTxn) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Receiving property update for node created in transaction: " +
          nodeRef
        );
      }

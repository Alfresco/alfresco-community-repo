...
public class ServiceBaseImpl
  ...
  public FilePlanComponentKind getFilePlanComponentKind(NodeRef nodeRef) {
    ...
    Map<NodeRef, FilePlanComponentKind> map = transactionalResourceHelper.getMap(
      "rm.transaction.filePlanComponentByNodeRef"
    );
    if (map.containsKey(nodeRef)) {
      ...
      if (isFilePlanComponent(nodeRef)) {
        ...
        if (isFilePlan(nodeRef)) {
          ...
          instanceOf(nodeRef, TYPE_DISPOSITION_SCHEDULE) ||
  ...
  public FilePlanComponentKind getFilePlanComponentKindFromType(QName type) {
    ...
    if (ASPECT_FILE_PLAN_COMPONENT.equals(type)) {
      ...
      instanceOf(type, TYPE_DISPOSITION_SCHEDULE) ||
  ...
  public boolean isFilePlanComponent(NodeRef nodeRef) {
    ...
    if (
      getInternalNodeService().exists(nodeRef) &&
  ...
  public boolean isHold(NodeRef nodeRef) {
    ...
    if (
      getInternalNodeService().exists(nodeRef) && instanceOf(nodeRef, TYPE_HOLD)
    ) {
      isHold = true;
    }
    return isHold;
  ...
  public NodeRef getFilePlan(final NodeRef nodeRef) {
    NodeRef result = null;
    if (nodeRef != null) {
      Map<NodeRef, NodeRef> transactionCache = transactionalResourceHelper.getMap(
        "rm.servicebase.getFilePlan"
      );
      if (transactionCache.containsKey(nodeRef)) {
        result = transactionCache.get(nodeRef);
      } else {
        result =
          (NodeRef) getInternalNodeService()
            .getProperty(nodeRef, PROP_ROOT_NODEREF);
        if (result == null || !instanceOf(result, TYPE_FILE_PLAN)) {
          if (instanceOf(nodeRef, TYPE_FILE_PLAN)) {
            result = nodeRef;
          } else {
            ChildAssociationRef parentAssocRef = getInternalNodeService()
  ...
  protected int getNextCount(NodeRef nodeRef) {
    ...
    if (!nodeService.hasAspect(nodeRef, ASPECT_COUNTABLE)) {
      ...
      Integer value = (Integer) this.nodeService.getProperty(
          nodeRef,
          PROP_COUNT
        );
      if (value != null) {

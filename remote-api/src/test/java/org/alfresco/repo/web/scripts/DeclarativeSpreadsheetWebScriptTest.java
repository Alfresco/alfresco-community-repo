...
public class DeclarativeSpreadsheetWebScriptTest extends BaseWebScriptTest {
  ...
  public void testCSVFormat() throws Exception {
    ...
    assertEquals(
      "The response CSV body was not correct.",
      "User Name,First Name,Last Name\n",
      response.getContentAsString()
    );

    req =
      new TestWebScriptServer.GetRequest(
        URL + "?" + DeclarativeSpreadsheetWebScript.PARAM_REQ_DELIMITER + "=%2C"
      );
    ...
    req =
      new TestWebScriptServer.GetRequest(
        URL + "?" + DeclarativeSpreadsheetWebScript.PARAM_REQ_DELIMITER + "=%09"
      );
    response = sendRequest(req, Status.STATUS_OK, admin);
    ...
    req =
      new TestWebScriptServer.GetRequest(
        URL + "?" + DeclarativeSpreadsheetWebScript.PARAM_REQ_DELIMITER + "=%3B"
      );
    response = sendRequest(req, Status.STATUS_OK, admin);

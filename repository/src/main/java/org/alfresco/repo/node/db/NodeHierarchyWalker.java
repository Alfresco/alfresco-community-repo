 ...
 *
 * <p>This class is not threadsafe and should be used sequentially on a single thread and then
 * discarded.
 *
 * @author Derek Hulley
 * @since 4.1.1
...
public class NodeHierarchyWalker {
    ...
    /** Store for all nodes by ID */
    private final Map<NodeRef, VisitedNode> nodesVisitedByNodeRef =
            new HashMap<NodeRef, VisitedNode>(59);
    ...
    public void walkHierarchy(
            Pair<Long, NodeRef> nodePair, Pair<Long, ChildAssociationRef> parentAssocPair) {
        ...
        VisitedNode visitedNode =
                new VisitedNode(nodeId, nodeRef, nodeType, nodeAclId, parentAssocPair);
    ...
    private void walkNode(Long nodeId) {
        ...
        NodeDAO.ChildAssocRefQueryCallback walkChildAssocs =
                        ...
                        if (childAssocPair.getSecond().isPrimary()) {
                            ...
                            if (nodeVisitedWorking == null) {
                                ...
                                throw new IllegalStateException(
                                        "Came to secondary association without having found primary"
                                                + " parent before: \n"
                                                + "   parent: "
                                                + parentNodePair
                                                + "\n"
        ...
        NodeDAO.ChildAssocRefQueryCallback getParentAssocs =
                new NodeDAO.ChildAssocRefQueryCallback() {
                    @Override
                    public final boolean preLoadNodes() {
                        return false;

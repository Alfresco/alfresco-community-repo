...
public abstract class AbstractSolrFacetConfigAdminWebScript
  ...
  private static final Log logger = LogFactory.getLog(
    AbstractSolrFacetConfigAdminWebScript.class
  );
  ...
  protected <T> T getValue(Class<T> clazz, Object value, T defaultValue)
  ...
  protected Set<CustomProperties> getCustomProperties(
    JSONObject customPropsJsonObj
  ) throws JSONException {
    ...
    for (int i = 0, length = keys.length(); i < length; i++) {
      JSONObject jsonObj = customPropsJsonObj.getJSONObject(
        (String) keys.get(i)
      );

      QName name = resolveToQName(
        getValue(String.class, jsonObj.opt(CUSTOM_PARAM_NAME), null)
      );
  ...
  protected Set<String> getScopedSites(JSONArray scopedSitesJsonArray)
    throws JSONException {
    if (scopedSitesJsonArray == null) {
      return null;
    }

    Set<String> scopedSites = new HashSet<String>(
      scopedSitesJsonArray.length()
    );
  ...
  private void validateMandatoryCustomProps(Object obj, String paramName)
    throws JSONException {
    if (obj == null) {
      throw new JSONException(
  ...
  private Serializable getSerializableValue(Object object)
    throws JSONException {
  ...
  private QName resolveToQName(String qnameStr) throws JSONException {
    ...
    if (
      qnameStr.charAt(0) == QName.NAMESPACE_BEGIN &&

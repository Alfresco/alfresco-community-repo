...
public class CreateTransferFolderAsNonAdminUserTest extends BaseRMTestCase {
    ...
    public void testCreateTransferFolderAsNonAdminUser() {
        doBehaviourDrivenTest(
                new BehaviourDrivenTest(testUser) {
                    // Records folder
                    ...
                    /**
                     * @see
                     *     org.alfresco.module.org_alfresco_module_rm.test.util.BaseRMTestCase.BehaviourDrivenTest#given()
                     */
                    @Override
                    public void given() {
                        runAs(
                                new RunAsWork<Void>() {
                                    public Void doWork() {
                                        ...
                                        // Give filing permissions for the test users on the
                                        // category
                                        filePlanPermissionService.setPermission(
                                                category, testUser, FILING);
                                        ...
                                        rmActionService.executeRecordsManagementAction(
                                                recordsFolder, CompleteEventAction.NAME, params);

                                        // Cut off folder
                                        rmActionService.executeRecordsManagementAction(
                                                recordsFolder, CutOffAction.NAME);
                                ...
                                (NodeRef)
                                        rmActionService
                                                .executeRecordsManagementAction(
                                                        recordsFolder, TransferAction.NAME)
                                                .getValue();
                    }

                    /**
                     * @see
                     *     org.alfresco.module.org_alfresco_module_rm.test.util.BaseRMTestCase.BehaviourDrivenTest#when()
                     */
                    @Override
                    public void when() {
                        // FIXME: If the transfer step is executed here the test fails?!? See
                        // RM-3931
                        // transferFolder = (NodeRef)
                        // rmActionService.executeRecordsManagementAction(recordsFolder,
                        // TransferAction.NAME).getValue();
                    }

                    /**
                     * @see
                     *     org.alfresco.module.org_alfresco_module_rm.test.util.BaseRMTestCase.BehaviourDrivenTest#then()
                     */
                    @Override
                    public void then() {
                        ...
                        assertEquals(
                                ALLOWED, permissionService.hasPermission(transferFolder, FILING));
                    }
                });
    }
}

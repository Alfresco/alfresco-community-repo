...
public class StandardRenditionLocationResolverTest
  ...
  public void before() throws Exception {
    ...
    this.renditionService =
      (RenditionService) applicationContext.getBean("RenditionService");
    this.repositoryHelper =
      (Repository) applicationContext.getBean("repositoryHelper");
    this.companyHome =
      this.applicationContext.getBean("repositoryHelper", Repository.class)
        .getCompanyHome();
  ...
  public void testChildAssociationFinder() {
    QName renditionKind = QName.createQName(
    ...
    RenditionLocation location = locationResolver.getRenditionLocation(
      sourceNode,
      renditionDef,
      tempRenditionNode
    );
    ...
    String companyHomeName = (String) nodeService.getProperty(
      companyHome,
      ContentModel.PROP_NAME
    );
    String targetFolderName = (String) nodeService.getProperty(
      targetFolder,
      ContentModel.PROP_NAME
    );
    String template =
      "/" + companyHomeName + "/" + targetFolderName + "/brian.xml";
    renditionDef.setParameterValue(
      RenditionService.PARAM_DESTINATION_PATH_TEMPLATE,
      template
    );
    location =
      locationResolver.getRenditionLocation(
        sourceNode,
        renditionDef,
        tempRenditionNode
      );
    ...
    String destinationName = (String) nodeService.getProperty(
      destinationNode,
      ContentModel.PROP_NAME
    );
  ...
  public void testCreatesFoldersForTemplatedLocation() throws Exception {
    QName fooName = QName.createQName(
      NamespaceService.CONTENT_MODEL_1_0_URI,
      "testFooFolder"
    );
    QName barName = QName.createQName(
      NamespaceService.CONTENT_MODEL_1_0_URI,
      "testBarFolder"
    );
    ...
    assertTrue(
      "Folder " + fooPath + " should not exist!",
      childAssocs.isEmpty()
    );
    ...
    NodeRef tempRenditionNode = makeNode(
      companyHome,
    ...
    NodeRef finalFolderNode = checkFolder(
  ...
  private NodeRef checkFolder(
    QName folderName,
    NodeRef parentName,
    String folderMessageName
  ) {
    ...
    assertEquals(
      "Folder " + folderMessageName + " should exist!",
      1,
      folderAssocs.size()
    );

...
/**
 * A service provider interface to provide both acegi integration via AuthenticationDao and
 * SaltSource and mutability support for user definitions.
 *
 * @author Andy Hind
 */
public interface MutableAuthenticationDao extends AuthenticationDao, SaltSource {
    ...
    /**
     * Create a user with the given userName and password hash If hashedPassword is passed in then
     * this is used, otherwise it falls back to using the rawPassword. It is assumed the hashed
     * password has been encoded using system.preferred.password.encoding and doesn't use its own
     * salt.
     */

...
public class VirtualPermissionServiceExtension
  ...
  private static Log logger = LogFactory.getLog(
    VirtualPermissionServiceExtension.class
  );
  ...
  public AccessStatus hasPermission(NodeRef nodeRef, String perm) {
    ...
    if (reference == null) {
      ...
      AccessStatus virtualAccessStatus = smartStore.hasPermission(
        reference,
        perm
      );
  ...
  public NodePermissionEntry getSetPermissions(NodeRef nodeRef) {
    ...
    if (reference == null) {
      ...
      NodePermissionEntry virtualSetPermissions = smartStore.getSetPermissions(
        reference
      );
      ...
      if (nodeToAdhereTo != null) {
        NodePermissionEntry actualSetPermissions = theTrait.getSetPermissions(
          nodeToAdhereTo
        );
      ...
      for (PermissionEntry permissionEntry : virtualPermissionEntries) {
        overridenPermissions.add(
          permissionEntry.getPermissionReference().getQName()
        );
        mergedEntries.add(permissionEntry);
      }
      for (PermissionEntry permissionEntry : actualPermissionEntries) {
        if (
          !overridenPermissions.contains(
            permissionEntry.getPermissionReference().getQName()
          )
        ) {
          mergedEntries.add(permissionEntry);
  ...
  private NodeRef establishPermisisonAdherence(Reference reference) {
    NodeRef nodeToAdhereTo = smartStore.adhere(
      reference,
    ...
    if (logger.isDebugEnabled()) {
      if (nodeToAdhereTo == null) {
        logger.debug(
          "Could not establish permission adherence for " + reference.toString()
        );
      }
    }
  ...
  public NodePermissionEntry explainPermission(
    ...
    return getTrait()
      .explainPermission(smartStore.materializeIfPossible(nodeRef), perm);
  }
  ...
  public Set<AccessPermission> getPermissions(NodeRef nodeRef) {
    ...
    if (reference == null) {
      ...
      Set<AccessPermission> virtualSetPermissions = smartStore.getAllSetPermissions(
        reference
      );
      NodeRef nodeToAdhereTo = establishPermisisonAdherence(reference);
      Set<AccessPermission> mergedEntries = new HashSet<>(
        virtualSetPermissions
      );
      if (nodeToAdhereTo != null) {
        Set<AccessPermission> actualSetPermissions = theTrait.getPermissions(
          nodeToAdhereTo
        );
  ...
  public AccessStatus hasPermission(
    ...
    return getTrait().hasPermission(aclID, context, permission);
  }
  ...
  public void clearPermission(NodeRef nodeRef, String authority) {
    if (Reference.fromNodeRef(nodeRef) == null) {
      getTrait().clearPermission(nodeRef, authority);
    } else {
      // no action taken on virtual nodes
    }
  }
  ...
  public void deletePermission(
    ...
    if (Reference.fromNodeRef(nodeRef) == null) {
      getTrait().deletePermission(nodeRef, authority, permission);
    } else {
      // no action taken on virtual nodes
    }
  }
  ...
  public void setPermission(
    ...
    if (Reference.fromNodeRef(nodeRef) == null) {
      getTrait().setPermission(nodeRef, authority, permission, allow);
    } else {
      // no action taken on virtual nodes
    }
  }
  ...
  public void setInheritParentPermissions(
    ...
    if (Reference.fromNodeRef(nodeRef) == null) {
      getTrait().setInheritParentPermissions(nodeRef, inheritParentPermissions);
    } else {
      // no action taken on virtual nodes
    }
  }
  ...
  public void setPermission(
    ...
    getTrait().setPermission(storeRef, authority, permission, allow);
  }
  ...
  public void deletePermission(
    ...
    getTrait().deletePermission(storeRef, authority, permission);
  }

  @Override
  public void clearPermission(StoreRef storeRef, String authority) {
    getTrait().clearPermission(storeRef, authority);
  }

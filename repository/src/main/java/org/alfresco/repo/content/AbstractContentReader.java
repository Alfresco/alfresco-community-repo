...
public abstract class AbstractContentReader
  ...
  private ReadableByteChannel getCallbackReadableChannel(
    ...
    if (directChannel instanceof FileChannel) {
      callbackChannel =
        getCallbackFileChannel((FileChannel) directChannel, listeners);
    } else {
      // introduce an advistor to handle the callbacks to the listeners
      ChannelCloseCallbackAdvise advise = new ChannelCloseCallbackAdvise(
        listeners
      );
   ...
   */
  public final synchronized ReadableByteChannel getReadableChannel()
    throws ContentIOException {
      ...
      ContentStreamListener spoofListener = new ContentStreamListener() {
        public void contentStreamClosed() throws ContentIOException {
          try {
            ...
            throw new ContentIOException(
              "Failed to close underlying channel",
              e
            );
          }
        }

...
/**
 * A JUnit rule designed to help with the automatic revert of test objects with mocked fields. This
 * is intended to be used when writing test code and you wish to set a mock object on a spring
 * singleton bean. By mocking out fields on spring singletons beans, if you don't remember to revert
 * them to their original values, then any subsequent tests that expect to see 'proper' services
 * plugged in, may fail when they are instead given a mock.
 *
 * <p>Example usage:
 *
 * <pre>
...
public class TemporaryMockOverride extends ExternalResource {
    ...
    protected void after() {
        ...
        for (int i = pristineFieldValues.size() - 1; i >= 0; i--) {
            ...
            if (log.isDebugEnabled()) {
                log.debug(
                        "Reverting mocked field '"
                                + override.fieldName
                                + "' on object "
                                + override.objectContainingField
                                + " to original value '"
                                + override.fieldPristineValue
                                + "'");
            }
            ...
            Field f =
                    ReflectionUtils.findField(
                            override.objectContainingField.getClass(), override.fieldName);
            ReflectionUtils.makeAccessible(f);
            // and revert its value.
            ReflectionUtils.setField(
                    f, override.objectContainingField, override.fieldPristineValue);
        }
    }

    public void setTemporaryField(
            Object objectContainingField, String fieldName, Object fieldValue) {
        if (log.isDebugEnabled()) {
            log.debug(
                    "Overriding field '"
                            + fieldName
        ...
        pristineFieldValues.add(
                new FieldValueOverride(objectContainingField, fieldName, pristineValue));
    ...
    private static class FieldValueOverride {
        public FieldValueOverride(
                Object objectContainingField, String fieldName, Object pristineValue) {

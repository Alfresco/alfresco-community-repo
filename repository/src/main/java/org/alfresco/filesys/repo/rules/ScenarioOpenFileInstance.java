  ...
  public Command evaluate(Operation operation) {
    ...
    if (operation instanceof DeleteFileOperation) {
      ...
      if (name.equalsIgnoreCase(d.getName())) {
        logger.debug(
          "Anti-Pattern - delete of the open file, scenario:" + this
        );
        isComplete = true;
        return null;
      }
            ...
            if (isReadOnly(file)) {
              ...
              if (openReadOnlyCount == 1) {
                ...
                if (file instanceof TempNetworkFile) {
                  logger.debug(
                    "this is the last close of a temp read only file"
                  );
                  ArrayList<Command> commands = new ArrayList<Command>();
                  ArrayList<Command> postCommitCommands = new ArrayList<Command>();
          ...
          if (name != null && name.equalsIgnoreCase(o.getName())) {
            if (o.getMode() == OpenFileMode.READ_WRITE) {
              ...
              if (openReadOnlyCount == 0) {
                ...
                openReadOnlyCount++;
                logger.debug(
                  "Return already open only file handle from scenario:" + this
                );
      ...
      public void execute(Object result) {
        if (result instanceof NetworkFile) {
          ...
          if (isReadOnly(fileHandle)) {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "file opened read only:" + result + ", name:" + name
              );
            }
          } else {
            openReadWriteCount++;
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                "file opened read write :" + result + ", name:" + name
              );
            }
          }
        }

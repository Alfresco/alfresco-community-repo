...
public class RM1030Test extends BaseRMTestCase {
  ...
  public void testRM1030() throws Exception {
    final NodeRef recordHold = doTestInTransaction(
      new Test<NodeRef>() {
        @Override
        public NodeRef run() {
          Set<String> auths = filePlanRoleService.getAllAssignedToRole(
            filePlan,
    ...
    final NodeRef recordFolderHold = doTestInTransaction(
      new Test<NodeRef>() {
        @Override
        public NodeRef run() {
          // freeze the record folder that contains the frozen record
          NodeRef folderHold = holdService.createHold(
            filePlan,
            "my hold 3",
            "innit but",
            "my decription"

...
public class RepositoryExporterComponent implements RepositoryExporterService {
  ...
  public FileExportHandle[] export(String packageName) {
    List<FileExportHandle> exportHandles = exportStores(
      exportStores,
      packageName,
      new TempFileExporter()
    );
   ...
   */
  public FileExportHandle[] export(
    ...
    List<FileExportHandle> exportHandles = exportStores(
      exportStores,
      packageName,
      new FileExporter(directoryDestination)
    );
    return exportHandles.toArray(new FileExportHandle[exportHandles.size()]);
  }
   ...
   */
  public RepositoryExportHandle[] export(
    NodeRef repositoryDestination,
    String packageName
  ) {
    ...
    List<RepositoryExportHandle> repoExportHandles = new ArrayList<RepositoryExportHandle>(
      exportHandles.size()
    );
    for (FileExportHandle exportHandle : exportHandles) {
      String name =
        exportHandle.packageName +
        "." +
        mimetypeExtensions.get(exportHandle.mimeType);
      ...
      NodeRef repoExportFile = addExportFile(
    ...
    return repoExportHandles.toArray(
      new RepositoryExportHandle[repoExportHandles.size()]
    );
  }
  ...
  private <ExportHandleType extends ExportHandle> List<ExportHandleType> exportStores(
    ...
    List<ExportHandleType> exportHandles = new ArrayList<ExportHandleType>(
      stores.size() + 1
    );

    // export repository system info
    {
      String completePackageName = (packageName == null)
        ? "systeminfo"
        : packageName + "_systeminfo";
      ExportHandleType systemInfoHandle = exportStore.exportSystem(
        completePackageName
      );
    ...
    for (Properties store : stores) {
      ...
      String completePackageName = (packageName == null)
        ? storePackageName
        : packageName + "_" + storePackageName;
      ...
      ExporterCrawlerParameters exportParameters = getExportParameters(
        storeRef,
        includedPaths
      );
      ExportHandleType exportHandle = exportStore.exportStore(
        exportParameters,
        completePackageName,
        null
      );

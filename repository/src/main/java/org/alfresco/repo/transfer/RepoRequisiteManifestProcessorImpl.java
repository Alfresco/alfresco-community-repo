...
/**
 * @author mrogers
 *     <p>The requisite manifest processor performs a parse of the manifest file to determine which
 *     resources are required. In particular it returns a list of nodes which require content to be
 *     transferred.
 */
public class RepoRequisiteManifestProcessorImpl extends AbstractManifestProcessorBase {
     ...
     */
    public RepoRequisiteManifestProcessorImpl(
    ...
    protected void processNode(TransferManifestNormalNode node) {
        ...
        logComment(
                "Requisite Processing incoming node: "
                        + node.getNodeRef()
                        + " --  Source path = "
                        + node.getParentPath()
                        + "/"
        ...
        if (resolvedNodes.resolvedChild != null) {
            /**
             * there is a corresponding node so we need to check whether we already have the part
            ...
            for (Map.Entry<QName, Serializable> propEntry : node.getProperties().entrySet()) {
                ...
                if ((value != null) && ContentData.class.isAssignableFrom(value.getClass())) {
                    ...
                    if (srcContent.getContentUrl() != null
                        ...
                        if (destSer != null
                                && ContentData.class.isAssignableFrom(destSer.getClass())) {
                            /**
                             * Content property not empty and content property already exists on
                             * destination
                             */
                            ContentData destContent = (ContentData) destSer;

                            Serializable destFromContents =
                                    destinationProps.get(TransferModel.PROP_FROM_CONTENT);

                            if (destFromContents != null
                                    && Collection.class.isAssignableFrom(
                                            destFromContents.getClass())) {
                                ...
                                if (contents.contains(partName)) {
                                    if (log.isDebugEnabled()) {
                                        log.debug(
                                                "part already transferred, no need to send it"
                                                        + " again, partName:"
                                                        + partName
                                                        + ", nodeRef:"
                                                        + node.getNodeRef());
                                    ...
                                    if (log.isDebugEnabled()) {
                                        log.debug(
                                                "part name not transferred, requesting new content"
                                                        + " item partName:"
                                                        + partName
                                                        + ", nodeRef:"
                                                        + node.getNodeRef());
                                ...
                                if (log.isDebugEnabled()) {
                                    log.debug(
                                            "from contents is null, requesting new content item"
                                                    + " partName:"
                                                    + partName
                                                    + ", nodeRef:"
                                                    + node.getNodeRef());
                            ...
                            if (log.isDebugEnabled()) {
                                log.debug(
                                        "no content on destination, all content is required"
                                                + propEntry.getKey()
                                                + srcContent.getContentUrl());
                            }
            ...
            for (Map.Entry<QName, Serializable> propEntry : node.getProperties().entrySet()) {
                ...
                if ((value != null) && ContentData.class.isAssignableFrom(value.getClass())) {
                    ...
                    if (srcContent.getContentUrl() != null
                        ...
                        if (log.isDebugEnabled()) {
                            log.debug(
                                    "no node on destination, content is required"
                                            + propEntry.getKey()
                                            + srcContent.getContentUrl());
                        }
                        out.missingContent(

...
public class FacetQNameUtilsTest {
  ...
  private final NamespacePrefixResolver resolver = new NamespacePrefixResolver() {
    private final List<String> uris = Arrays.asList(
      new String[] { "http://www.alfresco.org/model/foo/1.0" }
    );
  ...
  public void canCreateFromLongForm() throws Exception {
    assertEquals(
      ...
      FacetQNameUtils.createQName(
        "{http://www.alfresco.org/model/foo/1.0}localName",
        resolver
      )
    );
  }
  ...
  public void canCreateLongFormQnameWithUnrecognisedUri() throws Exception {
    ...
    assertEquals(
      QName.createQName(
        "http://www.alfresco.org/model/illegal/1.0",
        "localName"
      ),
      FacetQNameUtils.createQName(
        "{http://www.alfresco.org/model/illegal/1.0}localName",
        resolver
      )
    );
  }

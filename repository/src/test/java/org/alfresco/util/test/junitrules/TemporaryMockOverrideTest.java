...
public class TemporaryMockOverrideTest {
    ...
    @Test
    public void mockFieldsWithinServiceAndThenEnsureTheProperFieldValuesAreRestoredAfterCleanup()
            throws Throwable {
        TemporaryMockOverride mockRule = new TemporaryMockOverride();
        mockRule.before();

        assertEquals(
                "Original BarService giving wrong data.",
                REAL_DATA,
                realFooService.getBarService().getString());
        assertEquals(
                "Original AbcService giving wrong data.",
                REAL_DATA,
                realFooService.getAbcService().getString());
        ...
        // ...and then set the mocked values via the rule, which will remember the old values and
        // revert them for us automatically.
        ...
        assertEquals(
                "Mocked BarService giving wrong data.",
                MOCKED_DATA,
                fooServiceWithMockedServices.getBarService().getString());
        assertEquals(
                "Mocked AbcService giving wrong data.",
                MOCKED_DATA,
                fooServiceWithMockedServices.getAbcService().getString());

        mockRule.after();

        // Now it should all be magically reverted.
        assertEquals(
                "BarService giving wrong data.",
                REAL_DATA,
                fooServiceWithMockedServices.getBarService().getString());
        assertEquals(
                "AbcService giving wrong data.",
                REAL_DATA,
                fooServiceWithMockedServices.getAbcService().getString());
    }

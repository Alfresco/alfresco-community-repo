...
public class AuditComponentTest extends TestCase {
  ...
  private static final String APPLICATION_API_TEST =
  ...
  public void testAreAuditValuesRequired() {
    ...
    boolean auditRequiredForBogus = auditComponent.areAuditValuesRequired(
      "bogus"
    );
    assertFalse(
    ...
    boolean auditRequiredForTest = auditComponent.areAuditValuesRequired(
      "/test"
    );
  ...
  public void testAuditWithBadPath() throws Exception {
    ...
    auditComponent.recordAuditValues(
      "/test",
    ...
    RetryingTransactionCallback<Void> testCallback = new RetryingTransactionCallback<Void>() {
      public Void execute() throws Throwable {
        ...
        Map<String, Serializable> auditedValues = auditComponent.recordAuditValues(
          "/bogus",
          null
        );
  ...
  private Map<String, Serializable> auditTestAction(
    ...
    final Map<String, Serializable> adjustedValues = new HashMap<String, Serializable>(
      parameters.size() * 2
    );
    // Add the noderef
  ...
  private void checkAuditMaps(
    ...
    Map<String, Serializable> expected
  ) {
  ...
  private void auditAction01(String actionName) throws Exception {
    ...
    Map<String, Serializable> result = auditTestAction(
      actionName,
      nodeRef,
      parameters
    );
  ...
  public void testAudit_EnableDisableAuditPaths() throws Exception {
    ...
    RetryingTransactionCallback<Void> disableAuditCallback = new RetryingTransactionCallback<Void>() {
      public Void execute() throws Throwable {
        Map<String, Serializable> expectedInner = new HashMap<String, Serializable>(
          expected
        );
  ...
  public void testAuditAuthenticationService() throws Exception {
    ...
    logger.debug(sb.toString());
    assertFalse(
      "Did not get any audit results after successful login",
      results.isEmpty()
    );
    ...
    logger.debug(sb.toString());
    assertEquals(
      "Incorrect number of audit entries after failed login",
      iterations,
      results.size()
    );
    ...
    List<Long> remainingResults = new ArrayList<>(
      results.subList(100, results.size())
    );
    ...
    logger.debug(sb.toString());
    assertEquals(
      "Range of audit entries were not deleted",
  ...
  public void testAuditFailedNodeAccess() throws Exception {
    ...
    assertTrue(
      "There should be exactly one audit entry for the API test",
      success
    );
  }
  ...
  public void testApplication() throws Exception {
    ...
    AuditApplication applicationOne = auditModelRegistry.getAuditApplicationByName(
      APPLICATION_ONE
    );
    assertNotNull("Application 'app1' dosn't exist", applicationOne);
    AuditApplication applicationTwo = auditModelRegistry.getAuditApplicationByName(
      APPLICATION_TWO
    );
    assertNotNull("Application 'app2' dosn't exist", applicationTwo);
    AuditApplication applicationThree = auditModelRegistry.getAuditApplicationByName(
      APPLICATION_THREE
    );
    ...
    Map<String, Serializable> recordedAuditMap = auditComponent.recordAuditValues(
      "/alfresco-access",
      auditMap
    );
    ...
    String failure = EqualsHelper.getMapDifferenceReport(
      recordedAuditMap,
      expected
    );
    if (failure != null) {
  ...
  public void testAuditTruncatedValues() {
    ...
    final String stringValue = RandomStringUtils.randomAlphanumeric(
      SchemaBootstrap.DEFAULT_MAX_STRING_LENGTH + 1
    );
    ...
    Map<String, Serializable> result = AuthenticationUtil.runAs(
      testRunAs,
      "SomeOtherUser"
    );

...
/**
 * This class provides support for {@link CannedQuery canned queries} used by the {@link
 * CalendarService}.
 *
 * @author Nick Burch
 * @since 4.0
...
public class GetCalendarEntriesCannedQuery extends AbstractCannedQueryPermissions<CalendarEntry> {
    ...
    protected List<CalendarEntry> queryAndFilter(CannedQueryParameters parameters) {
        ...
        GetCalendarEntriesCannedQueryParams paramBean =
                (GetCalendarEntriesCannedQueryParams) paramBeanObj;
        ...
        List<CalendarEntity> results =
                cannedQueryDAO.executeQuery(
                        QUERY_NAMESPACE, QUERY_SELECT_GET_BLOGS, paramBean, 0, Integer.MAX_VALUE);
        ...
        for (CalendarEntity result : results) {
            ...
            Date recurringLastDate =
                    DefaultTypeConverter.INSTANCE.convert(
                            Date.class, result.getRecurrenceLastMeeting());
            ...
            if (recurringRule != null && !nextNodeIsAcceptable) {
                if (entriesToDate != null || recurringLastDate != null) {
                    ...
                    List<ChildAssociationRef> ignoreEventList =
                            nodeService.getChildAssocs(result.getNodeRef(), childNodeTypeQNames);
                        ...
                        Date ignoredDate =
                                (Date)
                                        nodeService.getProperty(
                                                nodeRef, CalendarModel.PROP_IGNORE_EVENT_DATE);
        ...
        if (sortPairs != null && !sortPairs.isEmpty()) {
            ...
            for (Pair<? extends Object, SortOrder> sortPair : sortPairs) {
                ...
                final CalendarEntityComparator comparator =
                        new CalendarEntityComparator(sortProperty);
                comparators.add(
                        new Pair<Comparator<CalendarEntity>, SortOrder>(
                                comparator, sortPair.getSecond()));
            }
            NestedComparator<CalendarEntity> comparator =
                    new NestedComparator<CalendarEntity>(comparators);

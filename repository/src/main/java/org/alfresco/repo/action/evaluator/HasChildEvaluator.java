...
public class HasChildEvaluator extends ActionConditionEvaluatorAbstractBase {
  ...
  public boolean evaluateImpl(
    ...
    if (this.nodeService.exists(actionedUponNodeRef) == true) {
      ...
      QName assocTypeParam = (QName) ruleCondition.getParameterValue(
        PARAM_ASSOC_TYPE
      );
      QName assocNameParam = (QName) ruleCondition.getParameterValue(
        PARAM_ASSOC_NAME
      );

      // Use the specified QNames if there are any, else default to match_all.
      QNamePattern assocType = assocTypeParam == null
        ? matchAll
        : assocTypeParam;
      QNamePattern assocName = assocNameParam == null
        ? matchAll
        : assocNameParam;

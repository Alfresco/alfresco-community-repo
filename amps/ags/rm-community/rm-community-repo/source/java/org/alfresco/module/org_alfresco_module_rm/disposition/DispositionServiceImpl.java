...
public class DispositionServiceImpl
  ...
  private Map<QName, DispositionProperty> dispositionProperties = new HashMap<>(
    4
  );
  ...
  public void refreshDispositionAction(NodeRef nodeRef) {
    ...
    if (di != null) {
      ...
      if (!dispositionActionDefinitions.isEmpty()) {
        ...
        DispositionActionDefinition nextDispositionActionDefinition = dispositionActionDefinitions.get(
          0
        );
   ...
   */
  @Override
  public void registerDispositionProperty(
    DispositionProperty dispositionProperty
  ) {
    dispositionProperties.put(
      dispositionProperty.getQName(),
      dispositionProperty
    );
  }
  ...
  public Collection<DispositionProperty> getDispositionProperties(
    ...
    for (DispositionProperty dispositionProperty : values) {
      boolean test = dispositionProperty.applies(
        isRecordLevel,
        dispositionAction
      );
      if (test) {
  ...
  public DispositionSchedule getDispositionSchedule(final NodeRef nodeRef) {
    ...
    if (isRecord(nodeRef)) {
      ...
      DispositionSchedule originDispositionSchedule = AuthenticationUtil.runAsSystem(
      ...
      if (
        originDispositionSchedule == null ||
        isNotTrue(originDispositionSchedule.isRecordLevelDisposition())
      ) {
        return null;
      }

      final NextActionFromDisposition dsNextAction = getDispositionActionByNameForRecord(
        nodeRef
      );
      ...
      if (dsNextAction != null) {
        ...
        if (
          isNotTrue(
            (Boolean) nodeService.getProperty(action, PROP_MANUALLY_SET_AS_OF)
          )
        ) {
  ...
  private NodeRef getDispositionScheduleImpl(NodeRef nodeRef) {
    ...
    if (result == null) {
      NodeRef parent =
        this.nodeService.getPrimaryParent(nodeRef).getParentRef();
  ...
  public DispositionSchedule getOriginDispositionSchedule(NodeRef nodeRef) {
    ...
    if (parent != null) {
      if (filePlanService.isRecordCategory(parent)) {
        ...
        return new DispositionScheduleImpl(
          serviceRegistry,
          nodeService,
          result
        );
  ...
  public DispositionSchedule getAssociatedDispositionSchedule(NodeRef nodeRef) {
    ...
    if (nodeService.exists(nodeRef)) {
      ...
      if (dsNodeRef != null) {
        ...
        ds =
          new DispositionScheduleImpl(serviceRegistry, nodeService, dsNodeRef);
      }
    }
  ...
  private NodeRef getAssociatedDispositionScheduleImpl(NodeRef nodeRef) {
    ...
    if (!filePlanService.isFilePlanComponent(nodeRef)) {
      throw new AlfrescoRuntimeException(
        "Can not find the associated retention schedule for a non records management component. (nodeRef=" +
        nodeRef.toString() +
        ")"
      );
    }
      ...
      if (!assocs.isEmpty()) {
        if (assocs.size() != 1) {
          ...
          if (LOGGER.isWarnEnabled()) {
            LOGGER.warn(
              "Retention schedule has more than one associated records management container.  " +
  ...
  public boolean hasDisposableItems(DispositionSchedule dispositionSchdule) {
    ...
    NodeRef rmContainer = getAssociatedRecordsManagementContainer(
      dispositionSchdule
    );

    return hasDisposableItemsImpl(
      dispositionSchdule.isRecordLevelDisposition(),
      rmContainer
    );
  }
   ...
   * @return
   */
  private boolean hasDisposableItemsImpl(
    ...
    for (NodeRef item : items) {
      if (recordFolderService.isRecordFolder(item)) {
        ...
        filePlanService.isRecordCategory(item) &&
   ...
   */
  @Override
  public List<NodeRef> getDisposableItems(
    DispositionSchedule dispositionSchedule
  ) {
    ParameterCheck.mandatory("dispositionSchedule", dispositionSchedule);
  ...
  public DispositionSchedule createDispositionSchedule(
    ...
    if (
      !TYPE_RECORD_CATEGORY.equals(nodeRefType) &&
      !dictionaryService.isSubClass(nodeRefType, TYPE_RECORD_CATEGORY)
    ) {
      throw new AlfrescoRuntimeException(
        "Unable to create retention schedule on a node that is not a records management container."
      );
    }
    ...
    try {
      ...
      if (assocs.isEmpty()) {
        DispositionSchedule currentDispositionSchdule = getDispositionSchedule(
          nodeRef
        );
        if (currentDispositionSchdule != null) {
          List<NodeRef> items = getDisposableItemsImpl(
            currentDispositionSchdule.isRecordLevelDisposition(),
            nodeRef
          );
          if (!items.isEmpty()) {
            throw new AlfrescoRuntimeException(
              "Can not create a retention schedule if there are disposable items already under the control of an other retention schedule"
            );
          }
        }
        ...
        throw new AlfrescoRuntimeException(
          "Unable to create retention schedule on node that already has a retention schedule."
        );
      }
    } finally {
  ...
  public DispositionActionDefinition addDispositionActionDefinition(
    ...
    String name = (String) actionDefinitionParams.get(
      PROP_DISPOSITION_ACTION_NAME
    );
    ...
    DispositionSchedule updatedSchedule =
      this.getDispositionSchedule(scheduleParent);
    return updatedSchedule.getDispositionActionDefinition(
      actionNodeRef.getId()
    );
  }
  ...
  public DispositionActionDefinition updateDispositionActionDefinition(
    ...
    this.nodeService.addProperties(
        actionDefinition.getNodeRef(),
        actionDefinitionParams
      );
    ...
    NodeRef ds =
      this.nodeService.getPrimaryParent(actionDefinition.getNodeRef())
        .getParentRef();
    DispositionSchedule updatedSchedule = new DispositionScheduleImpl(
      serviceRegistry,
      nodeService,
      ds
    );
  ...
  private DispositionAction initialiseDispositionAction(
    ...
    if (childAssocs != null && !childAssocs.isEmpty()) {
      return new DispositionActionImpl(
        serviceRegistry,
  ...
  private DispositionAction createDispositionAction(
    ...
    NodeRef dispositionActionNodeRef = nodeService
      .createNode(
        nodeRef,
        ASSOC_NEXT_DISPOSITION_ACTION,
        ASSOC_NEXT_DISPOSITION_ACTION,
    ...
    if (period != null) {
      ...
      if (periodProperty != null) {
        if (
          ...
          DispositionAction lastCompletedDispositionAction = getLastCompletedDispostionAction(
            nodeRef
          );
          if (lastCompletedDispositionAction != null) {
            ...
            contextDate =
              (Date) this.nodeService.getProperty(nodeRef, periodProperty);
          }
        } else {
  ...
  public boolean isNextDispositionActionEligible(NodeRef nodeRef) {
    ...
    if (
      ...
      if (nextDa.getName().equals("accession")) {
        NodeRef accessionNodeRef = di
          .getDispositionActionDefinitionByName("accession")
          .getNodeRef();
        if (accessionNodeRef != null) {
          Boolean combineStepsProp = (Boolean) getInternalNodeService()
            .getProperty(
              accessionNodeRef,
      ...
      Date asOf = (Date) getInternalNodeService()
      ...
      DispositionAction da = new DispositionActionImpl(
        serviceRegistry,
        nextDa.getNodeRef()
      );
      DispositionActionDefinition dad = da.getDispositionActionDefinition();
      if (dad != null) {
        boolean firstComplete = authenticationUtil.runAsSystem(() ->
          dad.eligibleOnFirstCompleteEvent()
        );

        List<ChildAssociationRef> assocs = getInternalNodeService()
          .getChildAssocs(
            nextDa.getNodeRef(),
        ...
        for (ChildAssociationRef assoc : assocs) {
          ...
          Boolean isCompleteValue = (Boolean) getInternalNodeService()
            .getProperty(eventExecution, PROP_EVENT_EXECUTION_COMPLETE);
  ...
  public DispositionAction getNextDispositionAction(NodeRef nodeRef) {
    ...
    if (dispositionActionNodeRef != null) {
      result =
        new DispositionActionImpl(serviceRegistry, dispositionActionNodeRef);
    }
    return result;
  }
   ...
   */
  @Override
  public List<DispositionAction> getCompletedDispositionActions(
    NodeRef nodeRef
  ) {
    List<ChildAssociationRef> assocs = nodeService.getChildAssocs(
      nodeRef,
  ...
  public void updateNextDispositionAction(final NodeRef nodeRef) {
    ...
    RunAsWork<Void> runAsWork = new RunAsWork<Void>() {
      ...
      public Void doWork() {
        // Get this disposition instructions for the node
        DispositionSchedule dispositionSchedule = getDispositionSchedule(
          nodeRef
        );
  ...
  public void updateNextDispositionAction(
    final NodeRef nodeRef,
    final DispositionSchedule dispositionSchedule
  ) {
    RunAsWork<Void> runAsWork = new RunAsWork<Void>() {
      /**
       * @see org.alfresco.repo.security.authentication.AuthenticationUtil.RunAsWork#doWork()
      ...
      public Void doWork() {
        if (dispositionSchedule != null) {
          ...
          if (currentDispositionAction == null) {
            ...
            String currentADId = (String) nodeService.getProperty(
              currentDispositionAction,
              PROP_DISPOSITION_ACTION_ID
            );
            currentDispositionActionDefinition =
              dispositionSchedule.getDispositionActionDefinition(currentADId);
            ...
            if (currentDispositionActionDefinition == null) {
              String currentADName = (String) nodeService.getProperty(
                currentDispositionAction,
                PROP_DISPOSITION_ACTION
              );
              currentDispositionActionDefinition =
                dispositionSchedule.getDispositionActionDefinitionByName(
                  currentADName
                );
            }
  ...
  public void cutoffDisposableItem(final NodeRef nodeRef) {
    ...
    if (
      FilePlanComponentKind.RECORD_FOLDER.equals(
  ...
  public Date getDispositionActionDate(
    ...
    DispositionSchedule ds = new DispositionScheduleImpl(
      serviceRegistry,
      nodeService,
      dispositionSchedule
    );
    List<ChildAssociationRef> assocs = getInternalNodeService()
      .getChildAssocs(dispositionSchedule);
    ...
    if (assocs != null && !assocs.isEmpty()) {
      for (ChildAssociationRef assoc : assocs) {
        if (
          ...
          DispositionActionDefinition actionDefinition = ds.getDispositionActionDefinition(
  ...
  public void recalculateNextDispositionStep(NodeRef record) {
    ...
    if (nextDispositionAction != null) {
      NextActionFromDisposition dsNextAction = getNextDispositionAction(
        record,
        recordFolders,
  ...
  protected NextActionFromDisposition getDispositionActionByNameForRecord(
    NodeRef record
  ) {
    List<NodeRef> recordFolders = recordFolderService.getRecordFolders(record);
    DispositionAction nextDispositionAction = getNextDispositionAction(record);
    ...
    if (nextDispositionAction == null) {
      ...
      return getNextDispositionAction(
        record,
        recordFolders,
        nextDispositionAction
      );
  ...
  private NextActionFromDisposition getNextDispositionAction(
    ...
    for (NodeRef folder : recordFolders) {
      ...
      if (dsNodeRef != null) {
        Date dispActionDate = getDispositionActionDate(
          record,
          dsNodeRef,
    ...
    WriteMode mode = determineWriteMode(
  ...
  private WriteMode determineWriteMode(
    Date recordNextDispositionActionDate,
    Date nextDispositionActionDate
  ) {
  ...
  private NextActionFromDisposition getFirstDispositionAction(
    ...
    for (NodeRef folder : recordFolders) {
      ...
      if (folderDS != null) {
        ...
        if (
          dispositionActionDefinitions != null &&
          dispositionActionDefinitions.size() > 0
        ) {
          DispositionActionDefinition firstDispositionActionDef = dispositionActionDefinitions.get(
            0
          );
          ...
          if (newAction == null) {
            ...
            DispositionAction firstDispositionAction = initialiseDispositionAction(
              recordOrFolder,
              firstDispositionActionDef
            );
            ...
            Date firstActionDate = calculateAsOfDate(
              record,
              firstDispositionActionDef
            );
            if (
    ...
    if (
      newDispositionActionName == null ||
      dispositionNodeRef == null ||
      newAction == null
    ) {
      return null;
    }

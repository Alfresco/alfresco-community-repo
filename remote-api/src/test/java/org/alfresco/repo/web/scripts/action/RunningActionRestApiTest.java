...
public class RunningActionRestApiTest extends BaseWebScriptTest {
  ...
  private static final String URL_RUNNING_REPLICATION_ACTIONS =
    "api/running-replication-actions";
  ...
  public void testRunningActionsGet() throws Exception {
    ...
    response =
      sendRequest(
        new GetRequest(URL_RUNNING_ACTIONS),
    ...
    ReplicationDefinition rd = replicationService.createReplicationDefinition(
      "Test1",
      "Testing"
    );
    ...
    String id = rd.getId();
    String instance = Integer.toString(
      ((ActionImpl) rd).getExecutionInstance()
    );
    ...
    for (int i = 0; i < keys.length(); i++) {
      ...
      if (
        key.equals("actionId") ||
        key.equals("actionType") ||
        key.equals("actionInstance") ||
        key.equals("actionNodeRef") ||
        key.equals("startedAt") ||
        key.equals("cancelRequested") ||
        key.equals("details")
      ) {
        // All good
      } else {
        fail(
          "Unexpected key '" + key + "' found in json, raw json is\n" + jsonStr
        );
      }
    }
    ...
    actionTrackingService.recordActionExecuting(rd);
    String id2 = rd.getId();
    String instance2 = Integer.toString(
      ((ActionImpl) rd).getExecutionInstance()
    );
    String startedAt2 = ISO8601DateFormat.format(rd.getExecutionStartDate());

    rd =
      replicationService.createReplicationDefinition(
        "AnotherTest",
        "3rd Testing"
      );
    replicationService.saveReplicationDefinition(rd);
    ...
    response =
      sendRequest(
        new GetRequest(
          URL_RUNNING_ACTIONS + "?nodeRef=" + rd.getNodeRef().toString()
        ),
        200
      );
    assertEquals(Status.STATUS_OK, response.getStatus());
    ...
    response =
      sendRequest(
        new GetRequest(URL_RUNNING_ACTIONS + "?type=replicationActionExecutor"),
        200
      );
    assertEquals(Status.STATUS_OK, response.getStatus());
    jsonStr = response.getContentAsString();
    ...
    // 0 if doesn't exist
    response =
      sendRequest(new GetRequest(URL_RUNNING_ACTIONS + "?type=MadeUp4"), 200);
    assertEquals(Status.STATUS_OK, response.getStatus());
    jsonStr = response.getContentAsString();
    json = new JSONObject(jsonStr);
    results = json.getJSONArray("data");
    ...
    response =
      sendRequest(new GetRequest(URL_RUNNING_ACTIONS + "?type=MadeUp1"), 200);
    assertEquals(Status.STATUS_OK, response.getStatus());
    jsonStr = response.getContentAsString();
    json = new JSONObject(jsonStr);
    results = json.getJSONArray("data");
    ...
    response =
      sendRequest(new GetRequest(URL_RUNNING_ACTIONS + "?type=MadeUp2"), 200);
    assertEquals(Status.STATUS_OK, response.getStatus());
    jsonStr = response.getContentAsString();
    json = new JSONObject(jsonStr);
    results = json.getJSONArray("data");
    ...
    assertEquals(false, jsonRD.getBoolean("cancelRequested"));
    assertEquals(
      "/" + URL_RUNNING_ACTION + "MadeUp2=54321=" + instanceAlt2,
      jsonRD.get("details")
    );
  }
  ...
  public void testRunningReplicationActionsGet() throws Exception {
    ...
    response =
      sendRequest(
        new GetRequest(URL_RUNNING_REPLICATION_ACTIONS),
        Status.STATUS_UNAUTHORIZED
      );
    assertEquals(Status.STATUS_UNAUTHORIZED, response.getStatus());
    ...
    response =
      sendRequest(new GetRequest(URL_RUNNING_REPLICATION_ACTIONS), 200);
    assertEquals(Status.STATUS_OK, response.getStatus());
    ...
    response =
      sendRequest(
        new GetRequest(URL_RUNNING_REPLICATION_ACTIONS + "?name=Test1"),
        200
      );
    assertEquals(Status.STATUS_OK, response.getStatus());
  ...
  public void testRunningActionGet() throws Exception {
    ...
    response =
      sendRequest(
        new GetRequest(URL_RUNNING_ACTION + "MadeUp"),
        Status.STATUS_UNAUTHORIZED
      );
    assertEquals(Status.STATUS_UNAUTHORIZED, response.getStatus());
    ...
    response =
      sendRequest(new GetRequest(URL_RUNNING_ACTION + key1), Status.STATUS_OK);
    assertEquals(Status.STATUS_OK, response.getStatus());
    ...
    response =
      sendRequest(new GetRequest(URL_RUNNING_ACTION + key2), Status.STATUS_OK);
    assertEquals(Status.STATUS_OK, response.getStatus());
    ...
    actionTrackingService.recordActionPending(rd);
    String id3 = rd.getId();
    String instance3 = Integer.toString(
      ((ActionImpl) rd).getExecutionInstance()
    );
    String key3 = "replicationActionExecutor=" + id3 + "=" + instance3;

    response =
      sendRequest(new GetRequest(URL_RUNNING_ACTION + key3), Status.STATUS_OK);
    assertEquals(Status.STATUS_OK, response.getStatus());
  ...
  public void testRunningActionCancel() throws Exception {
    ...
    response =
      sendRequest(
        new DeleteRequest(URL_RUNNING_ACTION + "MadeUp"),
        Status.STATUS_UNAUTHORIZED
      );
    assertEquals(Status.STATUS_UNAUTHORIZED, response.getStatus());
    ...
    response =
      sendRequest(
        new DeleteRequest(URL_RUNNING_ACTION + key),
    ...
    response =
      sendRequest(
        new DeleteRequest(URL_RUNNING_ACTION + key2),
        Status.STATUS_NO_CONTENT
      );
    assertEquals(Status.STATUS_NO_CONTENT, response.getStatus());
  ...
  public void testRunningActionsPost() throws Exception {
    ...
    response =
      sendRequest(
        new PostRequest(URL_RUNNING_ACTIONS, "{}", JSON),
        Status.STATUS_UNAUTHORIZED
      );
    assertEquals(Status.STATUS_UNAUTHORIZED, response.getStatus());
    ...
    response =
      sendRequest(
        new PostRequest(URL_RUNNING_ACTIONS, json.toString(), JSON),
        Status.STATUS_OK
      );
    assertEquals(Status.STATUS_OK, response.getStatus());
  ...
  public void testRunningReplicationsActionsPost() throws Exception {
    ...
    response =
      sendRequest(
        new PostRequest(URL_RUNNING_REPLICATION_ACTIONS, "{}", JSON),
        Status.STATUS_UNAUTHORIZED
      );
    assertEquals(Status.STATUS_UNAUTHORIZED, response.getStatus());
    ...
    response =
      sendRequest(
        new PostRequest(URL_RUNNING_REPLICATION_ACTIONS, json.toString(), JSON),
        Status.STATUS_OK
      );
    assertEquals(Status.STATUS_OK, response.getStatus());
  ...
  protected void setUp() throws Exception {
    ...
    replicationService =
      (ReplicationService) appContext.getBean("ReplicationService");
    actionTrackingService =
      (ActionTrackingService) appContext.getBean("actionTrackingService");
    repositoryHelper = (Repository) appContext.getBean("repositoryHelper");
    transactionService =
      (TransactionService) appContext.getBean("transactionService");
    executingActionsCache =
      (SimpleCache<String, ExecutionDetails>) appContext.getBean(
        "executingActionsCache"
      );

...
public abstract class AbstractTransferProgressMonitor
  ...
  private static final Log log = LogFactory.getLog(
    AbstractTransferProgressMonitor.class
  );
  ...
  protected abstract void updateProgressInternal(
    String transferId,
    int currPos,
    int endPos
  );
  ...
  protected abstract void updateStatusInternal(
    String transferId,
    Status status
  );
  ...
  private TransferDestinationReportWriter getLogWriter(String transferId) {
    TransferDestinationReportWriter writer =
      this.transferLogWriters.get(transferId);
    if (writer == null) {
      writer = new XMLTransferDestinationReportWriter();
      writer.startTransferReport(
        "UTF-8",
        createUnderlyingLogWriter(transferId)
      );

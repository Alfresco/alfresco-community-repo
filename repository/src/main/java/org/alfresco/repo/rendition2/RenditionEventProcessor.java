...
public class RenditionEventProcessor implements Processor {
  ...
  public void process(Exchange exchange) throws Exception {
    ...
    try {
      ...
      try {
        event =
          messagingObjectMapper.readValue(
            body,
  ...
  private String getExecutingUserOrDefault(OnContentUpdatePolicyEvent event) {
    if (
      event.getExecutingUser() != null && !event.getExecutingUser().isEmpty()
    ) {
    ...
    AuthenticationUtil.runAs(
      (AuthenticationUtil.RunAsWork<Void>) () ->
        transactionService
          ...
          .doInTransaction(() -> {
            renditionService2.onContentUpdate(
              new NodeRef(event.getNodeRef()),
              event.isNewContent()
            );

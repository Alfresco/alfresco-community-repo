...
public abstract class AbstractArchivedNodeWebScript extends DeclarativeWebScript {
    ...
    /**
     * * This method gets all nodes from the archive which were originally contained within the
     * specified StoreRef.
     *
     ...
     */
    protected PagingResults<NodeRef> getArchivedNodesFrom(
            StoreRef storeRef, ScriptPagingDetails paging, String filter) {
        ...
        ArchivedNodesCannedQueryBuilder queryBuilder =
                new ArchivedNodesCannedQueryBuilder.Builder(archiveStoreRootNodeRef, paging)
                        .filter(filter)
    ...
    protected void validatePermission(NodeRef nodeRef, String currentUser) {
        if (!nodeArchiveService.hasFullAccess(nodeRef)) {
            throw new WebScriptException(
                    Status.STATUS_FORBIDDEN, "You don't have permission to act on the node.");
        }
    }
}

...
public class DescriptorServiceImpl
   ...
   */
  public void setInstalledRepoDescriptorDAO(
    DescriptorDAO installedRepoDescriptorDAO
  ) {
  ...
  public void setHbDataCollectorService(
    HBDataCollectorService hbDataCollectorService
  ) {
  ...
  private boolean currentRepoDescriptorNullOrModeHasChanged(
    LicenseMode newMode
  ) {
    // Should not be needed as all updates take place within "synchronized (licenseLock)" but this is clearer
    currentRepoDescriptorLock.readLock().lock();
  ...
  protected void onBootstrap(ApplicationEvent event) {
    ...
    ((ApplicationContext) event.getSource()).publishEvent(
        new DescriptorServiceAvailableEvent(this)
      );
  }
  ...
  private void bootstrap() {
    ...
    Descriptor installed = helper.doInTransaction(
      getDescriptorCallback,
      false,
      false
    );
    if (installed != null) {
     ...
     */
    licenseService =
      (LicenseService) constructSpecialService(
        "org.alfresco.enterprise.license.LicenseComponent"
      );
    ...
    licenseService.registerOnLicenseChange(
      (LicenseChangeHandler) hbDataCollectorService
    );
  ...
  private Object constructSpecialService(String className) {
    try {
      ...
      throw new AlfrescoRuntimeException(
        "Failed to initialise " + className,
        e
      );
    }
  }
    ...
    synchronized (licenseLock) {
      ...
      RetryingTransactionCallback<RepoUsage> updateLicenseCallback = new RetryingTransactionCallback<RepoUsage>() {
        public RepoUsage execute() {
          ...
          Long expiryTime = licenseDescriptor.getValidUntil() == null
            ? null

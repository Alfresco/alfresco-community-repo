...
public class DeclarativeReportGenerator extends BaseReportGenerator {
    ...
    protected static final NodeRef TEMPLATE_ROOT =
            new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, "rm_report_templates");
    ...
    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.report.generator.BaseReportGenerator#generateReportName(org.alfresco.service.cmr.repository.NodeRef,
     *     String)
     */
    @Override
    ...
    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.report.generator.BaseReportGenerator#generateReportContent(NodeRef,
     *     String, Map)
     */
    @Override
    protected ContentReader generateReportContent(
        ...
        Map<String, Serializable> model =
                createTemplateModel(reportTemplateNodeRef, reportedUponNodeRef, properties);

        // run the template
        String result =
                templateService.processTemplate(
                        "freemarker", reportTemplateNodeRef.toString(), model);
     ...
     * @return
     */
    protected Map<String, Serializable> createTemplateModel(
        ...
        templateService.buildDefaultModel(
                person,
        ...
        model.put(
                "reportDate",
                SimpleDateFormat.getDateInstance(SimpleDateFormat.MEDIUM).format(now.getTime()));
    ...
    private NodeRef getReportTemplate(String mimetype) {
        ...
        if (!fileFolderService.exists(TEMPLATE_ROOT)) {
            throw new AlfrescoRuntimeException(
                    "Unable to get report template, because the template root folder does not exist"
                            + " in the data dictionary.");
        }

        String reportTemplateName = getReportTemplateName(mimetype);

        NodeRef reportTemplateNodeRef =
                fileFolderService.searchSimple(TEMPLATE_ROOT, reportTemplateName);
        if (reportTemplateNodeRef == null) {
            throw new AlfrescoRuntimeException(
                    "Unable to get report template, because report template "
                            + reportTemplateName
                            + " does not exist.");
        }
    ...
    private String getReportTemplateName(String mimetype) {
        String typePrefixName =
                reportType.getPrefixedQName(namespaceService).getPrefixString().replace(":", "_");
        String extension = mimetypeService.getExtension(mimetype);
    ...
    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.report.generator.BaseReportGenerator#checkReportApplicability(org.alfresco.service.cmr.repository.NodeRef)
     */
    @Override
    protected void checkReportApplicability(NodeRef reportedUponNodeRef) {
        if (applicableTypes != null && applicableTypes.size() != 0) {
            ...
            if (!isTypeApplicable) {
                // throw an exception
                throw new AlfrescoRuntimeException(
                        "Can't generate report, because the provided reported upon node reference"
                                + " is type "
                                + type.toString()
                                + " which is not an applicable type for a "
                                + reportType.toString()
                                + " report.");
            }
        }
    ...
    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.report.generator.BaseReportGenerator#generateReportTemplateContext(org.alfresco.service.cmr.repository.NodeRef)
     */
    ...
    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.report.generator.BaseReportGenerator#generateReportMetadata(org.alfresco.service.cmr.repository.NodeRef)
     */
    @SuppressWarnings("unchecked")
    @Override

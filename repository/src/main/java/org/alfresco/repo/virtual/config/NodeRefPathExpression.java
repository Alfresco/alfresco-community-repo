 ...
 * The given name or qname path is relative to a {@link NodeRefContext} repository location. The
 * default context is set to {@link CompanyHomeContext#COMPANY_HOME_CONTEXT_NAME}. Other contexts
 * can be set using their name with {@link #setContext(String)}.<br>
...
public class NodeRefPathExpression implements NodeRefExpression {
    ...
    public NodeRefPathExpression(NodeRefResolver resolver, Map<String, NodeRefContext> contexts) {
        this(resolver, contexts, CompanyHomeContext.COMPANY_HOME_CONTEXT_NAME, null);
    }

    public NodeRefPathExpression(
            NodeRefResolver resolver,
     ...
     * @throws AlfrescoRuntimeException if the given path is inconsistent (i.e. a combination of
     *     qnames and names)
     */
    ...
    public NodeRef resolve() {
        ...
        if (this.namePath != null) {
            return theContext.resolveNamePath(this.namePath, resolver);
        } else {
            return theContext.resolveQNamePath(this.qNamePath, resolver);
        }
    }
    ...
    public NodeRef resolve(boolean createIfNotFound) {
        ...
        if (nodeRef == null && createIfNotFound) {
            ...
            if (this.namePath != null) {
                return theContext.createNamePath(this.namePath, resolver);
            } else {
                return theContext.createQNamePath(this.qNamePath, this.createNamePath, resolver);
            }
        }
        return nodeRef;
    }

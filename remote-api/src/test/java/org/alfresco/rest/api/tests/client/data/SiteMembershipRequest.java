...
public class SiteMembershipRequest
  implements ExpectedComparison, Comparable<SiteMembershipRequest> {
  ...
  public static SiteMembershipRequest parseSiteMembershipRequest(
    ...
    siteMembershipRequest.setCreatedAt(
      PublicApiDateFormat.getDateFormat().parse(createdAt)
    );
    siteMembershipRequest.setMessage(message);
    if (modifiedAt != null) {
      siteMembershipRequest.setModifiedAt(
        PublicApiDateFormat.getDateFormat().parse(modifiedAt)
      );
    }
    if (siteJSON != null) {
  ...
  public static ListResponse<SiteMembershipRequest> parseSiteMembershipRequests(
    JSONObject jsonObject
  ) throws ParseException {
    ...
    for (int i = 0; i < jsonEntries.size(); i++) {
      ...
      siteMembershipRequests.add(
        SiteMembershipRequest.parseSiteMembershipRequest(entry)
      );
    }

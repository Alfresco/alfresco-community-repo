...
/**
 * Component API for indexing. Delegates to the real index retrieved from the {@link
...
public class IndexerComponent extends AbstractLifecycleBean implements Indexer {
    ...
    private static final String KEY_READ_THROUGH =
            IndexerComponent.class.getName() + "READ_THROUGH";

    public void setStoreRedirectorProxyFactory(
    ...
    private Indexer getIndexer(StoreRef storeRef) {
        ...
        indexer.setReadThrough(
                AlfrescoTransactionSupport.getResource(KEY_READ_THROUGH) == Boolean.TRUE);
        return indexer;
    }
    ...
    public void detectNodeChanges(
        ...
        Indexer indexer = getIndexer(nodeRef.getStoreRef());
        indexer.detectNodeChanges(
                nodeRef, searcher, addedParents, deletedParents, createdNodes, updatedNodes);
    }

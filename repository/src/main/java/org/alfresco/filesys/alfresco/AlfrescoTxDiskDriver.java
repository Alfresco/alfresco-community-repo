...
public abstract class AlfrescoTxDiskDriver
   ...
   * @throws IOException
   */
  public <T> T doInWriteTransaction(
    SrvSession sess,
    final CallableIO<T> callback
  ) throws IOException {
  ...
  public void endTransaction(SrvSession sess, Object tx) {
    ...
    if (filesysTx != null && filesysTx.hasTransaction()) {
      ...
      try {
        ...
        if (
          ftx.getStatus() == Status.STATUS_MARKED_ROLLBACK ||
          ...
          if (logger.isDebugEnabled()) logger.debug("End transaction (commit)");
        }
      } catch (Exception ex) {
        if (logger.isDebugEnabled()) logger.debug(
          "Failed to end transaction, " + ex.getMessage()
        );
  ...
  private final void beginTransaction(SrvSession sess, boolean readOnly)
    ...
    FilesysTransaction filesysTx = (FilesysTransaction) sess
    ...
    if (filesysTx.hasTransaction() == false) {
      try {
        ...
        UserTransaction userTrans = m_transactionService.getUserTransaction(
          readOnly
        );
        userTrans.begin();
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "Created transaction readOnly=" + readOnly
        );
      } catch (Exception ex) {
        throw new AlfrescoRuntimeException(
          "Failed to create transaction, " + ex.getMessage()
        );
      }
    }

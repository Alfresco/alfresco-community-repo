...
public class SubscriptionsDAOImpl extends AbstractSubscriptionsDAO {
  ...
  private static final QName PROP_SYS_NODE_DBID = QName.createQName(
  ...
  public int countSubscriptions(String userId, SubscriptionItemTypeEnum type) {
    ...
    Number count = template.selectOne(
      "alfresco.subscriptions.select_countSubscriptions",
      map
    );
  ...
  public void insertSubscription(String userId, NodeRef node) {
    ...
    Number count = template.selectOne(
      "alfresco.subscriptions.select_hasSubscribed",
      se
    );
  ...
  @Override
  public PagingFollowingResults selectFollowing(
    String userId,
    PagingRequest pagingRequest
  ) {
    if (userId == null) {
    ...
    @SuppressWarnings("unchecked")
    List<String> userList = template.selectList(
  ...
  @Override
  public PagingFollowingResults selectFollowers(
    String userId,
    PagingRequest pagingRequest
  ) {
    if (userId == null) {
  ...
  public int countFollowers(String userId) {
    ...
    Number count = template.selectOne(
      "alfresco.subscriptions.select_countFollowers",
      map
    );
    return count == null ? 0 : count.intValue();
  }

...
public class ActivitiWorkflowEngine extends BPMEngine implements WorkflowEngine {
    ...
    public static final QName QNAME_INITIATOR =
            QName.createQName(NamespaceService.DEFAULT_URI, WorkflowConstants.PROP_INITIATOR);
    ...
    public WorkflowInstance cancelWorkflow(String workflowId) {
        ...
        try {
            ...
            runtimeService.deleteProcessInstance(
                    processInstance.getId(), WorkflowConstants.PROP_CANCELLED);
    ...
    public WorkflowInstance deleteWorkflow(String workflowId) {
        String localId = createLocalId(workflowId);
        try {
            // Delete the runtime process instance if still running, this calls the end-listeners if
            // any
            ProcessInstance processInstance =
                    runtimeService
                            .createProcessInstanceQuery()
            ...
            if (processInstance != null) {
                runtimeService.deleteProcessInstance(
                        processInstance.getId(), ActivitiConstants.DELETE_REASON_DELETED);
            }
        ...
        try {
            ...
            if (definitionList != null && definitionList.size() > 0) {
                ...
                for (ProcessDefinition processDefinition : definitionList) {
                    if (WorkflowDeployer.CATEGORY_ALFRESCO_INTERNAL.equals(
                ...
                if (internalCategory) {
                    repoService.setDeploymentCategory(
                            deployment.getId(), WorkflowDeployer.CATEGORY_ALFRESCO_INTERNAL);
                }
            }
    ...
    public List<WorkflowInstance> getActiveWorkflows(String workflowDefinitionId) {
        try {
            ...
            String message =
                    messageService.getMessage(ERR_GET_ACTIVE_WORKFLOW_INSTS, workflowDefinitionId);
            if (logger.isDebugEnabled()) {
                logger.debug(message, ae);
            }
            throw new WorkflowException(message, ae);
    ...
    public List<WorkflowDefinition> getAllDefinitionsByName(String workflowName) {
        try {
            ...
            List<ProcessDefinition> definitions =
                    repoService.createProcessDefinitionQuery().processDefinitionKey(key).list();
        ...
        try {
            ...
            String message =
                    messageService.getMessage(
                            ERR_GET_COMPLETED_WORKFLOW_INSTS, workflowDefinitionId);
            if (logger.isDebugEnabled()) {
                logger.debug(message, ae);
            }
            throw new WorkflowException(message, ae);
    ...
    public WorkflowDefinition getDefinitionById(String workflowDefinitionId) {
        try {
            ...
            String msg =
                    messageService.getMessage(ERR_GET_WORKFLOW_DEF_BY_ID, workflowDefinitionId);
            if (logger.isDebugEnabled()) {
                logger.debug(msg, ae);
            }
            throw new WorkflowException(msg, ae);
    ...
    public byte[] getDefinitionImage(String workflowDefinitionId) {
        try {
            ...
            if (processDefinition == null) {
                throw new WorkflowException(
                        messageService.getMessage(
                                ERR_GET_DEF_UNEXISTING_IMAGE, workflowDefinitionId));
            }

            String diagramResourceName =
                    ((ReadOnlyProcessDefinition) processDefinition).getDiagramResourceName();
            if (diagramResourceName != null) {
                ByteArrayOutputStream out = new ByteArrayOutputStream();
                InputStream resourceInputStream =
                        repoService.getResourceAsStream(processDefinitionId, diagramResourceName);
    ...
    public List<WorkflowDefinition> getDefinitions() {
        try {
            ProcessDefinitionQuery query =
                    repoService.createProcessDefinitionQuery().latestVersion();
            if (activitiUtil.isMultiTenantWorkflowDeploymentEnabled()
                    && !TenantUtil.isCurrentDomainDefault()) {
                query.processDefinitionKeyLike("@" + TenantUtil.getCurrentDomain() + "%");
            }
    ...
    public List<WorkflowTaskDefinition> getTaskDefinitions(String workflowDefinitionId) {
        ...
        defs.add(
                typeConverter.getTaskDefinition(
                        startEvent, startTaskName, processDefinition.getId(), true));
        ...
        for (PvmActivity act : taskActivities) {
            ...
            defs.add(
                    typeConverter.getTaskDefinition(
                            act, formKey, processDefinition.getId(), false));
        }
    ...
    private String getFormKey(PvmActivity act) {
        if (act instanceof ActivityImpl) {
            ...
            if (actImpl.getActivityBehavior() instanceof UserTaskActivityBehavior) {
                UserTaskActivityBehavior uta =
                        (UserTaskActivityBehavior) actImpl.getActivityBehavior();
    ...
    public List<WorkflowTimer> getTimers(String workflowId) {
        try {
            ...
            // Only fetch process-instance when timers are available, to prevent extra unneeded
            // query
    ...
    private WorkflowTask getTaskForTimer(
            Job job, ProcessInstance processInstance, Execution jobExecution) {
        if (job instanceof TimerEntity) {
            ReadOnlyProcessDefinition def =
                    activitiUtil.getDeployedProcessDefinition(
                            processInstance.getProcessDefinitionId());
            List<String> activeActivityIds =
                    runtimeService.getActiveActivityIds(jobExecution.getId());
    ...
    public WorkflowInstance getWorkflowById(String workflowId) {
        try {
            ...
            String processInstanceId = createLocalId(workflowId);
            ProcessInstance processIntance =
                    runtimeService
                            .createProcessInstanceQuery()
                            .processInstanceId(processInstanceId)
                            .singleResult();
    ...
    public List<WorkflowInstance> getWorkflows(String workflowDefinitionId) {
        try {
            ...
            String message =
                    messageService.getMessage(ERR_GET_WORKFLOW_INSTS, workflowDefinitionId);
            if (logger.isDebugEnabled()) {
                logger.debug(message, ae);
            }
            throw new WorkflowException(message, ae);
    ...
    public WorkflowPath startWorkflow(
        ...
        try {
            ...
            // Set start task properties. This should be done before instance is started, since it's
            // id will be used
            Map<String, Object> variables =
                    propertyConverter.getStartVariables(processDefId, parameters);
            ...
            if (initiator != null) {
                variables.put(
                        WorkflowConstants.PROP_INITIATOR, nodeConverter.convertNode(initiator));
            ...
            if (context != null && context.getException() == null) {
                // MNT-11926: push null context to stack to avoid context reusage when new instance
                // is not flushed
    ...
    public void undeployDefinition(String workflowDefinitionId) {
        try {
            ...
            if (procDef == null) {
                String msg =
                        messageService.getMessage(
                                ERR_UNDEPLOY_WORKFLOW_UNEXISTING, workflowDefinitionId);
    ...
    public boolean hasWorkflowImage(String workflowInstanceId) {
        ...
        if (pi != null) {
            ...
            hasImage =
                    (processDefinition != null && processDefinition.isGraphicalNotationDefined());
        }
    ...
    /**
     * Converts the given list of {@link ProcessDefinition}s to a list of {@link
     * WorkflowDefinition}s that have a valid domain.
     *
    ...
    /**
     * Converts the given list of {@link Task}s to a list of {@link WorkflowTask}s that have a valid
     * domain.
     *
     * <p>Note that this method is not very performant. It will do many DB queries for each of the
     * Task item in the list of tasks as parameter. Make sure that you limit the size of the tasks
     * list depending on your needs. Hint: use the WorkflowTaskQuery query parameter setLimit()
     * method to limit the number of Tasks to process
     *
        ...
        return typeConverter.filterByDomainAndConvert(
                ...
                new Function<HistoricTaskInstance, String>() {
                    public String apply(HistoricTaskInstance task) {
                        ...
                        ProcessDefinition definition =
                                (ProcessDefinition)
                                        activitiUtil.getDeployedProcessDefinition(defId);
                        return definition.getKey();
                    }
                });
    }
    ...
    private WorkflowTask endNormalTask(String taskId, String localTaskId, String transition) {
        ...
        // Check if the assignee is equal to the current logged-in user. If not, assign task before
        // ending
        String currentUserName = AuthenticationUtil.getFullyAuthenticatedUser();
        if (task.getAssignee() == null || !task.getAssignee().equals(currentUserName)) {
            ...
            // Re-fetch the task-entity since it's revision has been updated by the setAssignee()
            // call
    ...
    private void setOutcome(Task task, String transition) {
        ...
        boolean isDefaultTransition =
                transition == null || ActivitiConstants.DEFAULT_TRANSITION_NAME.equals(transition);
        ...
        if (outcomePropName != null) {
            if (isDefaultTransition == false) {
                ...
                Serializable transitionValue =
                        propertyConverter.convertValueToPropertyType(
    ...
    private WorkflowTask endStartTask(String localTaskId) {
        ...
        runtimeService.setVariable(
                processInstanceId, ActivitiConstants.PROP_START_TASK_END_DATE, new Date());
        ...
        ReadOnlyProcessDefinition procDef =
                activitiUtil.getDeployedProcessDefinition(processInstance.getProcessDefinitionId());
            ...
            if (state == WorkflowTaskState.IN_PROGRESS) {
                ...
                for (HistoricTaskInstance historicTask : historicTasks) {
                    ...
                    if (lazyInitialization) {
                        resultingTasks.add(
                                new LazyActivitiWorkflowTask(
                                        historicTask, typeConverter, tenantService));
                    } else {
    ...
    public List<WorkflowTask> getPooledTasks(List<String> authorities, boolean lazyInitialization) {
        try {
            if (authorities != null && authorities.size() > 0) {
                ...
                // others are groups to which the user (or group) belongs. This way, we don't have
                // to
                // check for the type of the authority.

                String firstAuthority = authorities.get(0);
                // Use a map, can be that a task has multiple candidate-groups, which are inside the
                // list
                // of authorities
                ...
                if (authorityManager.isUser(firstAuthority)) {
                    ...
                    if (authorities.size() > 1) {
                        List<String> remainingAuthorities =
                                authorities.subList(1, authorities.size());
    ...
    private void addTasksForCandidateGroups(
            List<String> groupNames, Map<String, Task> resultingTasks) {
        if (groupNames != null && groupNames.size() > 0) {

            TaskQuery query = taskService.createTaskQuery().taskCandidateGroupIn(groupNames);

            // Additional filtering on the tenant-property in case workflow-definitions are shared
            // across tenants
            if (!activitiUtil.isMultiTenantWorkflowDeploymentEnabled()
    ...
    private void addProcessPropertiesToQuery(
        ...
        for (Entry<QName, Object> customProperty : processCustomProps.entrySet()) {
            ...
            if (name != ActivitiConstants.VAR_TENANT_DOMAIN) {
                ...
                Object converted =
                        propertyConverter.convertPropertyToValue(customProperty.getValue());
    ...
    private TaskQuery createRuntimeTaskQuery(WorkflowTaskQuery query) {
        ...
        if (!activitiUtil.isMultiTenantWorkflowDeploymentEnabled()) {
            ...
            taskQuery.processVariableValueEquals(
                    ActivitiConstants.VAR_TENANT_DOMAIN, TenantUtil.getCurrentDomain());
        } else if (tenantService.isEnabled()
                && !TenantUtil.isCurrentDomainDefault()
    ...
    private HistoricTaskInstanceQuery createHistoricTaskQuery(WorkflowTaskQuery query) {
        HistoricTaskInstanceQuery historicQuery =
    ...
    private void addTaskPropertiesToQuery(
            Map<QName, Object> taskCustomProps, HistoricTaskInstanceQuery taskQuery) {
        for (Entry<QName, Object> customProperty : taskCustomProps.entrySet()) {
            String name = factory.mapQNameToName(customProperty.getKey());
    ...
    private void addProcessPropertiesToQuery(
            Map<QName, Object> processCustomProps, HistoricTaskInstanceQuery taskQuery) {
        for (Entry<QName, Object> customProperty : processCustomProps.entrySet()) {
            String name = factory.mapQNameToName(customProperty.getKey());
    ...
    private boolean isStartTaskMatching(WorkflowTask workflowTask, WorkflowTaskQuery query) {
        ...
        if (query.getProcessCustomProps() != null
            ...
            // Get properties for process instance, based on path of start task, which is
            // process-instance
    ...
    private boolean checkPropertiesPresent(
            Map<QName, Object> expectedProperties, Map<QName, Serializable> props) {
    ...
    private List<WorkflowInstance> getWorkflowsInternal(
        ...
        List<WorkflowInstance> completedResults =
                typeConverter.doSpecialTenantFilterAndSafeConvert(
                        completedInstances,
                        new Function<HistoricProcessInstance, String>() {
                            public String apply(HistoricProcessInstance historicProcessInstance) {
                                ProcessDefinition procDef =
                                        activitiUtil.getProcessDefinition(
    ...
    private HistoricProcessInstanceQuery createQuery(
        ...
        // MNT-9074 My Tasks fails to render if tasks quantity is excessive
        String processDefId =
                workflowInstanceQuery.getWorkflowDefinitionId() == null
                        ? null
                        : createLocalId(workflowInstanceQuery.getWorkflowDefinitionId());

        HistoricProcessInstanceQuery query;
        if (isActive) {
            // Don't use ProcessInstanceQuery here because in any case they will be converted to
            // WorkflowInstance thro HistoricProcessInstance.
            ...
            for (Map.Entry<QName, Object> prop : customProps.entrySet()) {
                ...
                if (prop.getValue() == null) {
                    ...
                    PropertyDefinition propertyDefinition =
                            dictionaryService.getProperty(prop.getKey());
                    if (propertyDefinition == null) {
                        Object converted =
                                propertyConverter.convertPropertyToValue(prop.getValue());
                        ...
                        if (propertyType.equals("java.util.Date")) {
                            Map<DatePosition, Date> dateProps =
                                    (Map<DatePosition, Date>) prop.getValue();
                                ...
                                if (dateProp.getValue() != null) {
                                    if (dateProp.getKey() == DatePosition.BEFORE) {
                                        query.variableValueLessThanOrEqual(
                                                propertyName, dateProp.getValue());
                                    }

...
public class ExtendedSecurityServiceImpl
  ...
  static final String READER_GROUP_PREFIX =
    ExtendedSecurityService.IPR_GROUP_PREFIX + "R";
  static final String WRITER_GROUP_PREFIX =
    ExtendedSecurityService.IPR_GROUP_PREFIX + "W";
  ...
  public void set(
    NodeRef nodeRef,
    Pair<Set<String>, Set<String>> readersAndWriters
  ) {
    ParameterCheck.mandatory("nodeRef", nodeRef);
  ...
  private Pair<String, String> getIPRGroups(NodeRef nodeRef) {
    ...
    for (AccessPermission permission : permissions) {
      ...
      if (
        permission.getAuthority().startsWith(GROUP_PREFIX + READER_GROUP_PREFIX)
      ) {
        iprReaderGroup = permission.getAuthority();
      }
      // look for the presence of the writer group
      else if (
        permission.getAuthority().startsWith(GROUP_PREFIX + WRITER_GROUP_PREFIX)
      ) {
   ...
   */
  private Pair<String, String> createOrFindIPRGroups(
    Set<String> readers,
    Set<String> writers
  ) {
    return new Pair<>(
   ...
   */
  private String createOrFindIPRGroup(
    String groupPrefix,
   ...
   */
  private Pair<String, Integer> findIPRGroup(
    String groupPrefix,
    Set<String> authorities
  ) {
    ...
    String groupShortNamePrefix = getIPRGroupPrefixShortName(
      groupPrefix,
      authorities
    );
  ...
  private boolean isIPRGroupTrueMatch(String group, Set<String> authorities) {
    ...
    Set<String> contained = authorityService.getContainedAuthorities(
      null,
      group,
      true
    );
   ...
   */
  /*package*/String getIPRGroupPrefixShortName(
    String prefix,
    Set<String> authorities
  ) {
   ...
   */
  /*package*/String getIPRGroupShortName(
    String prefix,
    Set<String> authorities,
    int index
  ) {
   ...
   * @return String   group short name
   */
  private String getIPRGroupShortName(
    String prefix,
    Set<String> authorities,
    String index
  ) {
  ...
  private String createIPRGroup(
    ...
    // get the group name
    String groupShortName = getIPRGroupShortName(
      groupNamePrefix,
      children,
      index
    );
    ...
    try {
      ...
      if (children != null) {
        for (String child : children) {
          if (
            authorityService.authorityExists(child) &&
   ...
   * @param nodeRef   node reference
   */
  private void assignIPRGroupsToNode(
    Pair<String, String> iprGroups,
    NodeRef nodeRef
  ) {

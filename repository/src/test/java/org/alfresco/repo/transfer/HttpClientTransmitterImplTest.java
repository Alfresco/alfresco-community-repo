...
public class HttpClientTransmitterImplTest extends TestCase {
  ...
  protected void setUp() throws Exception {
    ...
    when(mockedNodeService.getType(any(NodeRef.class)))
      .thenReturn(TransferModel.TYPE_TRANSFER_TARGET);
  ...
  public void testSuccessfulVerifyTargetOverHttp() throws Exception {
    ...
    when(
      mockedHttpClient.executeMethod(
        any(HostConfiguration.class),
    ...
    ArgumentCaptor<HostConfiguration> hostConfig = ArgumentCaptor.forClass(
      HostConfiguration.class
    );
    ArgumentCaptor<HttpMethod> httpMethod = ArgumentCaptor.forClass(
      HttpMethod.class
    );
    ArgumentCaptor<HttpState> httpState = ArgumentCaptor.forClass(
      HttpState.class
    );
  ...
  public void testSuccessfulVerifyTargetOverHttps() throws Exception {
    ...
    assertTrue(
      ...
      hostConfig
        .getValue()
        .getProtocol()
        .getSocketFactory() instanceof SecureProtocolSocketFactory
    );
    assertEquals(
      "protocol",
      HTTPS_PROTOCOL.toLowerCase(),
      hostConfig.getValue().getProtocol().getScheme().toLowerCase()
    );
  }
  ...
  public void testHttpsVerifyTargetWithCustomSocketFactory() throws Exception {
    ...
    assertTrue(
      "socket factory",
      hostConfig.getValue().getProtocol().getSocketFactory() == socketFactory
    );
    assertEquals(
      "protocol",
      HTTPS_PROTOCOL.toLowerCase(),
  ...
  public void testGetStatusErrorRehydration() throws Exception {
    ...
    final TransferException expectedException = new TransferException(
      "my message id",
    ...
    doAnswer(
      ...
      .when(mockedHttpMethodFactory.latestPostMethod)
    ...
    assertTrue(
      Arrays.deepEquals(
        expectedException.getMsgParams(),
        receivedException.getMsgParams()
      )
    );
  }
  ...
  public void testBeginFailure() throws Exception {
    ...
    try {
      transmitter.begin(
        target,
        "1234",
        new TransferVersionImpl("2", "2", "2", "Dummy")
      );
      fail();
    } catch (TransferException ex) {
      ...
      assertTrue(
        Arrays.deepEquals(expectedException.getMsgParams(), ex.getMsgParams())
      );
    }
  }

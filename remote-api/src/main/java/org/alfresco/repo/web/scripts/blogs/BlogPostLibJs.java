...
public class BlogPostLibJs {
   ...
   */
  public static boolean hasExternalBlogConfiguration(
    NodeRef node,
    ...
    if (node == null) {
      ...
      return hasExternalBlogConfiguration(
        services.getNodeService().getPrimaryParent(node).getParentRef(),
        services
      );
    }
  }

  public static Map<String, Object> getBlogPostData(
    NodeRef node,
    ServiceRegistry services
  ) {
    Map<String, Object> data = new HashMap<String, Object>();
    data.put("node", node);
    String creator = (String) services
      .getNodeService()
      .getProperty(node, ContentModel.PROP_CREATOR);
    //ALF-18527
    ...
    Serializable published = services
      .getNodeService()
      .getProperty(node, ContentModel.PROP_PUBLISHED);
    ...
    Date updatedDate = (Date) services
      .getNodeService()
      .getProperty(node, ContentModel.PROP_UPDATED);
    ...
    if (isPublished && lastUpdate != null) {
      ...
      data.put(
        "outOfDate",
        modifiedDate.getTime() - lastUpdate.getTime() > 5000L
      );
    } else {

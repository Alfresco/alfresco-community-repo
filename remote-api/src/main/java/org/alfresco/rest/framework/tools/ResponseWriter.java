...
public interface ResponseWriter {
    ...
    final WithResponse DEFAULT_SUCCESS =
            new WithResponse(Status.STATUS_OK, DEFAULT_JSON_CONTENT, CACHE_NEVER);
        ...
        if (contentInfo != null) {
            ...
            if (res instanceof WebScriptServletResponse) {
                ...
                if (contentInfo.getLength() > 0) {
                    if (contentInfo.getLength() > 0
                            && contentInfo.getLength() < Integer.MAX_VALUE) {
                        servletResponse
                                .getHttpServletResponse()
                                .setContentLength((int) contentInfo.getLength());
                    }
                }
    ...
    /**
     * The response status must be set before the response is written by Jackson (which will by
     * default close and commit the response). In a r/w txn, web script buffered responses ensure
     * that it doesn't really matter but for r/o txns this is important. If you set content
     * information via the contentInfo object and ALSO the headers then "headers" will win because
     * they are set last.
     *
     * @param res
     * @param status
     ...
     * @throws IOException
     */
    default void renderErrorResponse(
        ...
        final ErrorResponse errorToWrite =
                new ErrorResponse(
        ...
        // Status must be set before the response is written by Jackson (which will by default close
        // and commit the response).
        // In a r/w txn, web script buffered responses ensure that it doesn't really matter but for
        // r/o txns this is important.
        res.setStatus(errorToWrite.getStatusCode());

        jsonHelper.withWriter(
                res.getOutputStream(),
                new JacksonHelper.Writer() {
     ...
     * @throws IOException
     */
    default void renderException(
            final Exception exception,
            final WebScriptResponse response,
            final ApiAssistant assistant)
            throws IOException {
        renderErrorResponse(
                assistant.resolveException(exception), response, assistant.getJsonHelper());
    }
     ...
     * @throws IOException
     */
    default void renderJsonResponse(
            final WebScriptResponse res, final Object toSerialize, final JacksonHelper jsonHelper)
            throws IOException {
        jsonHelper.withWriter(
                res.getOutputStream(),
                new JacksonHelper.Writer() {

...
/**
 * Property value based virtual nodes {@link Comparator}. Compares two virtualized nodes based on an
 * ordered collection of property names. The property with the lower index in the collection of
 * properties is the more significant (with the highest-order) one in the comparison process.
 *
...
public class ReferenceComparator implements Comparator<Reference> {
    ...
    public int compare(Reference r1, Reference r2) {
        try {
            return compareImpl(r1, r2, sortProps);
        } catch (VirtualizationException e) {
            throw new RuntimeException(e);
        }
    }

    private int compareImpl(
            Reference ref1In, Reference ref2In, List<Pair<QName, Boolean>> sortProps)

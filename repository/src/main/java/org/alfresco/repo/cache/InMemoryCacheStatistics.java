...
/**
 * Simple non-persistent implementation of {@link CacheStatistics}. Statistics are empty at
 * repository startup.
 *
 * @since 5.0
 * @author Matt Ward
...
public class InMemoryCacheStatistics implements CacheStatistics, ApplicationContextAware {
    ...
    /**
     * Gets a {@link ReentrantReadWriteLock} for a specific cache, lazily creating the lock if
     * necessary. Locks may be created per cache (rather than hashing to a smaller pool) since the
     * number of caches is not too large.
     *

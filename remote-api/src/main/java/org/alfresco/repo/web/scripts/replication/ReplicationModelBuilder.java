...
public class ReplicationModelBuilder {
  ...
  public static class SimpleSorterByStatus
    ...
    public int compare(
      Map<String, Object> simpleA,
      Map<String, Object> simpleB
    ) {
  ...
  protected Map<String, Object> buildSimpleList(
    ...
    if (replicationDefinitions.size() > 0) {
      List<ExecutionSummary> executing = actionTrackingService.getExecutingActions(
  ...
  protected Map<String, Object> buildDetails(ReplicationDefinition rd) {
    ...
    if (rd.isSchedulingEnabled()) {
      rdm.put(
        ...
        ISO8601DateFormat.format(rd.getScheduleStart())
      );
      ...
      if (rd.getScheduleIntervalPeriod() != null) {
        rdm.put(
          ...
          rd.getScheduleIntervalPeriod().toString()
        );
      } else {
  ...
  protected void setStatus(
    ...
    if (details == null) {
      ...
      model.put(
        DEFINITION_STATUS,
        replicationDefinition.getExecutionStatus().toString()
      );
    ...
    if (details.getStartedAt() != null) {
      model.put(
        ...
        ISO8601DateFormat.format(details.getStartedAt())
      );
    } else {
      model.put(DEFINITION_STARTED_AT, null);
    }
    ...
    if (model.containsKey(DEFINITION_FAILURE_MESSAGE)) model.put(
      DEFINITION_FAILURE_MESSAGE,
      null
    );
    if (model.containsKey(DEFINITION_TRANSFER_LOCAL_REPORT)) model.put(
      DEFINITION_TRANSFER_LOCAL_REPORT,
      null
    );
    if (model.containsKey(DEFINITION_TRANSFER_REMOTE_REPORT)) model.put(
  ...
  private ExecutionDetails getExecutionDetails(
    ...
    for (ExecutionSummary es : executing) {
      if (
        es
          .getActionType()
          .equals(replicationDefinition.getActionDefinitionName()) &&

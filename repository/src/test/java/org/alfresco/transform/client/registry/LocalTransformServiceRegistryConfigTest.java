...
public class LocalTransformServiceRegistryConfigTest extends TransformRegistryTest {
    ...
    private class TestLocalTransformServiceRegistry extends LocalTransformServiceRegistry {
        ...
        public synchronized void setMockSuccessReadingConfig(boolean mockSuccessReadingConfig) {
            System.out.println(
                    "\n"
                            + getMs()
                            + ": set next mock read to "
                            + (mockSuccessReadingConfig ? "success" : "failure"));
        ...
        // It looks for the alfresco global property or system property with a localTransform.
        // prefix,
        // just like the normal code.
        // If setResetBaseUrl(false) is called, the baseUrl remains null
        // If we are using transforms called "t-engine" only the first one has its baseUrl set -
        ...
        public String getBaseUrlIfTesting(String name, String baseUrl) {
            ...
            return baseUrl == null && resetBaseUrl && !isTEngine
                    ...
                    : isTEngine && tEngineCount == 1 ? HARD_CODED_VALUE : baseUrl;
        }
        ...
        public boolean readConfig() throws IOException {
            ...
            System.out.println(
                    getMs()
                            + "readConfig() success="
                            + mockSuccessReadingRemoteConfig
                            + " reads="
                            + readConfigCount);
    ...
    private static final String LOCAL_TRANSFORM_SERVICE_CONFIG =
            "alfresco/local-transform-service-config-test.json";
    private static final String LOCAL_TRANSFORM_SERVICE_CONFIG_PIPELINE =
            "alfresco/local-transform-service-config-pipeline-test.json";
    ...
    protected int getExpectedTransformsForTestJsonPipeline() {
        ...
        // libreoffice
        //         {"sourceMediaType": "application/vnd.ms-outlook", "targetMediaType":
        // "application/pdf"}
        // officeToImageViaPdf
    ...
    private void initTestData() {
        ...
        targetMimetype = new ArrayList<>(targetMimetype);
        targetMimetype.add(
                "alfresco-metadata-extract"); // Metadata extract and embed types should be excluded
        // from pipeline cartesian products
        ...
        tikaSupportedTransformation.put(
        ...
        libreofficeSupportedTransformation.put(
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                targetMimetype);
        libreofficeSupportedTransformation.put(
                "application/vnd.openxmlformats-officedocument.presentationml.presentation",
        ...
        officeToImageViaPdfSupportedTransformation.put(
                "application/vnd.ms-powerpoint", targetMimetype);
        officeToImageViaPdfSupportedTransformation.put(
                "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                targetMimetype);
        officeToImageViaPdfSupportedTransformation.put(
        ...
        officeToImageViaPdfSupportedTransformation.put(
                ...
                targetMimetype);
        officeToImageViaPdfSupportedTransformation.put(
                "application/vnd.ms-outlook", targetMimetype);
    }
    ...
    public void testJsonConfig() throws IOException {
        ...
        assertEquals(
                "The number of UNIQUE source to target mimetypes transforms has changed. Config"
                        + " change?",
                64,
                countSupportedTransforms(true));
        assertEquals(
                "The number of source to target mimetypes transforms has changed. There may be"
                        + " multiple transformers for the same combination. Config change?",
                70,
                countSupportedTransforms(false));
        ...
        assertSupported(
                DOC, 1234, PDF, invalidPdfOptions, null, "Invalid as there is a extra option");
    }
    ...
    public void testJsonPipeline() throws IOException {
        ...
        Set<TransformOption> transformOptionsSet =
                supportedTransform.getTransformOptions().getTransformOptions();
        ...
        TransformOptionValue alphaRemove =
                (TransformOptionValue)
                        retrieveTransformOptionByPropertyName(
                                imagemagick, "alphaRemove", "TransformOptionValue");
        TransformOptionGroup crop =
                (TransformOptionGroup)
                        retrieveTransformOptionByPropertyName(
                                imagemagick, "crop", "TransformOptionGroup");
        TransformOptionValue cropGravity =
                (TransformOptionValue)
                        retrieveTransformOptionByPropertyName(
                                crop, "cropGravity", "TransformOptionValue");
        TransformOptionValue cropWidth =
                (TransformOptionValue)
                        retrieveTransformOptionByPropertyName(
                                crop, "cropWidth", "TransformOptionValue");

        assertTrue(
                "The holding group should be required",
                supportedTransform.getTransformOptions().isRequired());
        ...
        assertFalse(
                "alphaRemove should be optional as required is not set", alphaRemove.isRequired());
    ...
    private TransformOption retrieveTransformOptionByPropertyName(
            TransformOptionGroup transformOptionGroup, String propertyName, String propertyType) {
        ...
        for (TransformOption t : transformOptionsList) {
            if (t instanceof TransformOptionValue) {
                ...
                if (propertyType.equalsIgnoreCase("TransformOptionValue")) {
                    if (value.getName().equalsIgnoreCase(propertyName)) return value;
                } else {
                    if (value.getName().contains(propertyName)) return transformOptionGroup;
                }
            } else {
                TransformOption result =
                        retrieveTransformOptionByPropertyName(
                                (TransformOptionGroup) t, propertyName, propertyType);
    ...
    private boolean containsTransformOptionValueName(
            TransformOptionGroup transformOptionGroup, String propertyName) {
        return retrieveTransformOptionByPropertyName(
                        transformOptionGroup, propertyName, "TransformOptionValue")
                != null;
    }
    ...
    private int countSupportedTransforms(boolean unique) {
        int count = 0;
        int uniqueCount = 0;
        for (Map<String, List<SupportedTransform>> targetMap :
                registry.getData().getTransforms().values()) {
    ...
    public void testReadJsonConfig() {
        ...
        for (CombinedConfig.TransformAndItsOrigin t : transformerList) {
            assertTrue(
                    t.transformer.getTransformerName()
                            + " should be an expected local transformer.",
                    listOfExpectedTransformersName.contains(t.transformer.getTransformerName()));
            ...
            switch (t.transformer.getTransformerName()) {
                case "imagemagick":
                    assertEquals(
                            t.transformer.getTransformerName()
                                    + " incorrect number of supported transform",
                            18,
                            t.transformer.getSupportedSourceAndTargetList().size());
                    assertEquals(
                            t.transformer.getTransformerName()
                                    + "incorrect number of transform option names",
                            1,
                            t.transformer.getTransformOptions().size());
                    assertEquals(
                            t.transformer.getTransformerName()
                                    + "incorrect number of transform options",
                            6,
                            countTopLevelOptions(t.transformer.getTransformOptions()));
                    assertEquals(
                            t.transformer.getTransformerName()
                                    + " expected to not be a transformer pipeline",
                            t.transformer.getTransformerPipeline().size(),
                            0);
                    assertEquals(
                            t.transformer.getTransformerName()
                                    + " expected to not be a failover pipeline",
                            t.transformer.getTransformerFailover().size(),
                            0);

                    // Test supportedSourceAndTargetList
                    for (SupportedSourceAndTarget ssat :
                            t.transformer.getSupportedSourceAndTargetList()) {
                        assertTrue(
                                ssat.getSourceMediaType()
                                        + " not expected to be a supported transform source.",
                                imagemagickSupportedTransformation.containsKey(
                                        ssat.getSourceMediaType()));
                        assertTrue(
                                ssat.getTargetMediaType()
                                        + " not expected to be a supported transform target for "
                                        + ssat.getSourceMediaType(),
                                imagemagickSupportedTransformation
                ...
                case "tika":
                    ...
                    for (SupportedSourceAndTarget ssat :
                        ...
                        assertTrue(
                                ssat.getSourceMediaType()
                                        + " not expected to be a supported transform source.",
                                tikaSupportedTransformation.containsKey(ssat.getSourceMediaType()));
                        assertTrue(
                                ssat.getTargetMediaType()
                                        + " not expected to be a supported transform target for "
                                        + ssat.getSourceMediaType(),
                ...
                case "pdfrenderer":
                    ...
                    for (SupportedSourceAndTarget ssat :
                        ...
                        assertTrue(
                                ssat.getSourceMediaType()
                                        + " not expected to be a supported transform source.",
                                pdfRendererSupportedTransformation.containsKey(
                                        ssat.getSourceMediaType()));
                        assertTrue(
                                ssat.getTargetMediaType()
                                        + " not expected to be a supported transform target for "
                ...
                case "libreoffice":
                    ...
                    for (SupportedSourceAndTarget ssat :
                        ...
                        assertTrue(
                                ssat.getSourceMediaType()
                                        + " not expected to be a supported transform source.",
                                libreofficeSupportedTransformation.containsKey(
                                        ssat.getSourceMediaType()));
                        assertTrue(
                                ssat.getTargetMediaType()
                                        + " not expected to be a supported transform target for "
                ...
                case "officeToImageViaPdf":
                    // Note we will get 35 entries in getSupportedSourceAndTargetList() if the
                    // metadata transforms are not excluded
                    assertEquals(
                            t.transformer.getTransformerName()
                                    + " incorrect number of supported transform",
                            28,
                    ...
                    assertEquals(
                            t.transformer.getTransformerName()
                                    + " expected to be a transformer pipeline",
                            t.transformer.getTransformerPipeline().size(),
                            3);
                    ...
                    for (SupportedSourceAndTarget ssat :
                        ...
                        assertTrue(
                                ssat.getSourceMediaType()
                                        + " not expected to be a supported transform source.",
                                officeToImageViaPdfSupportedTransformation.containsKey(
                                        ssat.getSourceMediaType()));
                        assertTrue(
                                ssat.getTargetMediaType()
                                        + " not expected to be a supported transform target for "
                                        + ssat.getSourceMediaType(),
                                officeToImageViaPdfSupportedTransformation
                                        .get(ssat.getSourceMediaType())
                                        .contains(ssat.getTargetMediaType()));
                    }
                    break;
        ...
        assertEquals(
                "Transformer expected but not found in config file",
                0,
                listOfExpectedTransformersName.size());
    }
    ...
    public void testReadTransformProperties() {
        ...
        for (CombinedConfig.TransformAndItsOrigin t : transformerList) {
            if (t.transformer.getTransformerPipeline() == null) {
                assertNotNull(
                        t.transformer.getTransformerName() + " JVM property not set.",
                        System.getProperty(
                                LOCAL_TRANSFORM + t.transformer.getTransformerName() + URL));
            }
        }
        assertEquals(
                "Unexpected pdfrenderer JVM property value",
                "http://localhost:8090/",
                System.getProperty(LOCAL_TRANSFORM + "pdfrenderer" + URL));
        assertEquals(
                "Unexpected imagemagick JVM property value",
                "http://localhost:8091/",
                System.getProperty(LOCAL_TRANSFORM + "imagemagick" + URL));
        assertEquals(
                "Unexpected libreoffice JVM property value",
                "http://localhost:8092/",
                System.getProperty(LOCAL_TRANSFORM + "libreoffice" + URL));
        assertEquals(
                "Unexpected tika JVM property value",
                "http://localhost:8093/",
                System.getProperty(LOCAL_TRANSFORM + "tika" + URL));

        for (CombinedConfig.TransformAndItsOrigin t : transformerList) {
            if (t.transformer.getTransformerPipeline() == null) {
                assertNotNull(
                        t.transformer.getTransformerName() + " alfresco-global property not set.",
                        properties.getProperty(
                                LOCAL_TRANSFORM + t.transformer.getTransformerName() + URL));
            }
        }
        assertEquals(
                "Unexpected pdfrenderer alfresco-global property value",
                "http://localhost:8090/",
                properties.getProperty(LOCAL_TRANSFORM + "pdfrenderer" + URL));
        assertEquals(
                "Unexpected imagemagick alfresco-global property value",
                "http://localhost:8091/",
                properties.getProperty(LOCAL_TRANSFORM + "imagemagick" + URL));
        assertEquals(
                "Unexpected libreoffice alfresco-global property value",
                "http://localhost:8092/",
                properties.getProperty(LOCAL_TRANSFORM + "libreoffice" + URL));
        assertEquals(
                "Unexpected tika alfresco-global property value",
                "http://localhost:8093/",
                properties.getProperty(LOCAL_TRANSFORM + "tika" + URL));
    }

    @Test
    // Simulates the reading of config which changes over time and sometimes T-Engines fail to
    // reply.
    public void testAdditionAndRemovalOfTEngines() throws Exception {
        ...
        CronExpression origInitialAndOnErrorCronExpression =
                registry.getInitialAndOnErrorCronExpression();
        ...
        try {
            ...
            registry.setInitialAndOnErrorCronExpression(
                    new CronExpression(
                            ("0/2 * * ? * * *"))); // every 2 seconds rather than 10 seconds
            registry.setCronExpression(
                    new CronExpression(
                            ("0/4 * * ? * * *"))); // every 4 seconds rather than every hour

            // Sleep until a 6 second boundary, in order to make testing clearer.
            // It avoids having to work out schedule offsets and extra quick runs that can otherwise
            // take place.
            ...
            data =
                    registry.assertDataChanged(
                            data,
                            "There should have been a read after a few milliseconds that fails");

            Thread.sleep(2000); // 3 seconds
            data =
                    registry.assertDataChanged(
                            data, "There should have been a read after 2 seconds that fails");

            Thread.sleep(2000); // 5 seconds
            data =
                    registry.assertDataChanged(
                            data, "There should have been a read after 4 seconds that fails");

            Thread.sleep(2000); // 7 seconds
            data =
                    registry.assertDataChanged(
                            data, "There should have been a read after 6 seconds that fails");

            // Should switch to normal 4s schedule after the next read, so the read at 12 seconds
            // will be on that schedule.
            // It is always possible that another quick one gets scheduled almost straight away
            // after the next read.
            registry.setMockSuccessReadingConfig(true);
            Thread.sleep(2000); // 9 seconds
            data =
                    registry.assertDataChanged(
                            data, "There should have been a read after 8 seconds that succeeds");

            Thread.sleep(2000); // 11 seconds
            data =
                    registry.assertDataUnchanged(
                            data, "There really should not have been a read until 12 seconds");

            Thread.sleep(2000); // 13 seconds
            data =
                    registry.assertDataChanged(
                            data, "There should have been a read after 12 seconds that succeeds");
            ...
            data =
                    registry.assertDataChanged(
                            data, "There should have been a read after 16 seconds that fails");

            Thread.sleep(2000); // 19 seconds
            data =
                    registry.assertDataChanged(
                            data, "There should have been a read after 18 seconds");
        } finally {
            registry.setMockSuccessReadingConfig(true);
    ...
    @Test
    // Checks that only options expected by individual transforms in the pipeline are actually
    ...
    public void testStripExtraOptions() {
        ...
        LocalPipelineTransform officeToImageViaPdf =
                (LocalPipelineTransform)
                        ((LocalTransformServiceRegistry) registry)
                                .getLocalTransform(
                                        "application/msword", -1, "image/gif", actualOptions, null);
        assertEquals(
                "Original number of options officeToImageViaPdf",
                21,
                officeToImageViaPdf.getTransformsTransformOptionNames().size());

        AbstractLocalTransform libreoffice =
                (AbstractLocalTransform) officeToImageViaPdf.getIntermediateTransformer(0);
        assertEquals(
                "libreoffice options", 0, libreoffice.getTransformsTransformOptionNames().size());
        assertEquals(
                "libreoffice actual options",
                0,
                libreoffice.getStrippedTransformOptions(actualOptions).size());

        AbstractLocalTransform pdfrenderer =
                (AbstractLocalTransform) officeToImageViaPdf.getIntermediateTransformer(1);
        assertEquals(
                "pdfrenderer options", 5, pdfrenderer.getTransformsTransformOptionNames().size());
        assertEquals(
                "pdfrenderer actual options",
                2,
                pdfrenderer.getStrippedTransformOptions(actualOptions).size()); // width, height

        AbstractLocalTransform imagemagick =
                (AbstractLocalTransform) officeToImageViaPdf.getIntermediateTransformer(2);
        assertEquals(
                "imagemagick options", 16, imagemagick.getTransformsTransformOptionNames().size());
        assertEquals(
                "imagemagick actual options",
                1,
                imagemagick.getStrippedTransformOptions(actualOptions).size()); // autoOrient
    }
    ...
    public void testPriority() {
        ...
        assertEquals(
                ...
                ((AbstractLocalTransform)
                                registry.getLocalTransform(
                                        "source", -1, "target1", Collections.emptyMap(), null))
                        .getName());
        ...
        assertEquals(
                ...
                ((AbstractLocalTransform)
                                registry.getLocalTransform(
                                        "source", -1, "target2", Collections.emptyMap(), null))
                        .getName());
        ...
        assertEquals(
                ...
                ((AbstractLocalTransform)
                                registry.getLocalTransform(
                                        "source", -1, "target3", Collections.emptyMap(), null))
                        .getName());
    }
    ...
    public void testPipelineAndFailover() {
        retrieveLocalTransformList(
                "alfresco/local-transform-service-config-pipeline-and-failover-test.json");
        registry.assertErrorLogged(
                "Transformer .* cannot have pipeline and failover"
                        + " sections.*pipeline-and-failover.*");
    }
    ...
    public void testTEngineNoBaseUrls() {
        ...
        registry.assertErrorLogged(
                "Single step transformers \\(such as \"pdfrenderer\"\\) must be defined in a"
                        + " T-Engine rather than in a pipeline file, unless they are overriding an"
                        + " existing single step definition.*no-base-url.*");
    }
    ...
    public void testPipelineMissingStepTransform() {
        ...
        registry.assertErrorLogged(
                "Transformer \"missingPdfrenderer\" ignored as step transforms"
                        + " \\(\"pdfrenderer\"\\) do not exist.*pipeline-missing-step-test.*");
    }
    ...
    public void testFailoverMissingStepTransform() {
        ...
        registry.assertErrorLogged(
                "Transformer \"missingPdfrenderer\" ignored as step transforms"
                        + " \\(\"pdfrenderer\"\\) do not exist.*failover-missing-step-test.*");
    }

    @Test
    public void testOverrideTEngine() {
        // This json file contains two transformers with the same name.
        // * The second one should override the first which has different supported source
        // mimetypes, target mimetypes
        //   and max sizes. It also has different transform options.
        // * There is special code in getBaseUrlIfTesting mocking up the baseUrl just for the first
        // one. It should be
        ...
        assertNotNull(
                ...
                ((AbstractLocalTransform)
                        registry.getLocalTransform(
                                "text/csv", 1000, "text/html", Collections.emptyMap(), null)));

        assertNotNull(
                "Increased max size be supported",
                ((AbstractLocalTransform)
                        registry.getLocalTransform(
                                "text/csv", 2000, "text/html", Collections.emptyMap(), null)));

        assertNull(
                "Increased max size is now 2000",
                ((AbstractLocalTransform)
                        registry.getLocalTransform(
                                "text/csv", 3000, "text/html", Collections.emptyMap(), null)));

        assertNotNull(
                "Should have been added",
                ((AbstractLocalTransform)
                        registry.getLocalTransform(
                                "text/csv", -1, "application/pdf", Collections.emptyMap(), null)));
        ...
        assertNotNull(
                "options1 should still exist, even if not used",
                mapOfTransformOptions.get("options1"));
        ...
        Map<String, String> actualOptions =
                Map.of(
                        ...
                        "height", "50");
        assertNull(
                "width from options1 is no longer used, so should find no transformer",
                ((AbstractLocalTransform)
                        registry.getLocalTransform(
                                "text/csv", -1, "application/pdf", actualOptions, null)));
        ...
        actualOptions =
                Map.of(
                        ...
                        "height", "50");
        assertNotNull(
                "Both options are in options2, so should we should find the transformer",
                ((AbstractLocalTransform)
                        registry.getLocalTransform(
                                "text/csv", -1, "application/pdf", actualOptions, null)));

        LocalTransformImpl localTransform =
                (LocalTransformImpl)
                        registry.getLocalTransform(
                                "text/csv", -1, "application/pdf", Collections.emptyMap(), null);
        assertEquals(
        ...
        assertTrue(
                "The baseUrl should have been copied",
                localTransform.remoteTransformerClientConfigured());
    }

    @Test
    public void testOverrideTEngineWithPipeline() {
        // This json file contains two transformers with the same name, plus two others libreoffice
        // and pdfrenderer
        // * The first transform which should be overridden has a baseUrl as it talks to a T-Engine
        // and has 5
        //   supported source to target mimetypes.
        // * The second one should override the first with a pipeline of libreoffice and pdfrenderer
        // but only has 1
        //   supported source to target mimetype. It should not have a baseUrl as it will not talk
        // to a T-Engine.
        //   THIS IS WHAT IS BASICALLY DIFFERENT TO testOverrideTEngine, resulting in a
        // LocalPipelineTransform rather
        //   rather than a LocalTransformImpl.
        // * There is special code in getBaseUrlIfTesting mocking up the baseUrl just for the first
        // one. It should be
        //   copied to the second one.
        retrieveLocalTransformList(
                "alfresco/local-transform-service-config-override-with-pipeline-test.json");

        LocalPipelineTransform pipelineTransform =
                (LocalPipelineTransform)
                        registry.getLocalTransform(
                                "text/csv", -1, "image/png", Collections.emptyMap(), null);

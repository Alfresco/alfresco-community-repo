...
public class JSONConversionComponent {
    ...
    protected Map<QName, PropertyDecorator> propertyDecorators =
            new HashMap<QName, PropertyDecorator>(8);
    ...
    public JSONObject toJSONObject(final NodeRef nodeRef, final boolean useShortQNames) {
        ...
        if (this.nodeService.exists(nodeRef)) {
            if (publicServiceAccessService.hasAccess(
                            ServiceRegistry.NODE_SERVICE.getLocalName(), "getProperties", nodeRef)
    ...
    protected void setRootValues(
        ...
        rootJSONObject.put(
                "isContainer",
                nodeInfo.isFolder()); // node.getIsContainer() || node.getIsLinkToContainer());
        ...
        // TODO should this be moved to the property output since we may have more than one content
        // property
        ...
        if (nodeInfo.isFolder() == false) {
            ...
            if (cdata != null) {
                ...
                if (mimetypeDescriptions.containsKey(cdata.getMimetype())) {
                    rootJSONObject.put(
                            "mimetypeDisplayName", mimetypeDescriptions.get(cdata.getMimetype()));
                }
    ...
    protected JSONObject permissionsToJSON(final NodeRef nodeRef) {
        ...
        if (AccessStatus.ALLOWED.equals(
                        permissionService.hasPermission(
                                nodeRef, PermissionService.READ_PERMISSIONS))
                == true) {
            permissionsJSON.put(
    ...
    protected Object propertyToJSON(
        ...
        if (value != null) {
            ...
            if (propertyDecorators.containsKey(propertyName)) {
                JSONAware jsonAware =
                        propertyDecorators.get(propertyName).decorate(propertyName, nodeRef, value);
                if (jsonAware != null) {
                    return jsonAware;
                }
            } else {
                ...
                if (value instanceof Date) {
                    ...
                    dateObj.put(
                            "iso8601", JSONObject.escape(ISO8601DateFormat.format((Date) value)));
                    return dateObj;
                } else if (value instanceof List) {
                    // Convert the List to a JSON list by recursively calling propertyToJSON
                    List<Object> jsonList =
                            new ArrayList<Object>(((List<Serializable>) value).size());

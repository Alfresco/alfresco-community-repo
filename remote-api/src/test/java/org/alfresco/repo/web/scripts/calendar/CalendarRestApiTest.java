...
public class CalendarRestApiTest extends BaseWebScriptTest {
    ...
    private static final String URL_EVENT_BASE =
            "/calendar/event/" + SITE_SHORT_NAME_CALENDAR + "/";
    private static final String URL_EVENTS_LIST = "/calendar/eventList";
    private static final String URL_EVENTS_LIST_ICS =
            "/calendar/eventList-" + SITE_SHORT_NAME_CALENDAR + ".ics?format=calendar";
    ...
    private static final String URL_USER_SITE_EVENTS_LIST =
            "/calendar/events/" + SITE_SHORT_NAME_CALENDAR + "/user";
    ...
    protected void setUp() throws Exception {
        ...
        if (!siteService.hasContainer(
                SITE_SHORT_NAME_CALENDAR, CalendarServiceImpl.CALENDAR_COMPONENT)) {
            siteService.createContainer(
                    SITE_SHORT_NAME_CALENDAR, CalendarServiceImpl.CALENDAR_COMPONENT, null, null);
        }
    ...
    private JSONObject createEntry(
            String name, String where, String description, int expectedStatus) throws Exception {
    ...
    private JSONObject createAllDayEntryDifferentTimeZone(
            String name, String where, String description, int expectedStatus) throws Exception {
        String date = "2011-06-29"; // A wednesday
        String start = "00:00";
        String end = "00:00";
    ...
    private JSONObject createAllDayEntry(
            String name, String where, String description, int expectedStatus) throws Exception {
        String date = "2011-06-29"; // A wednesday
        String start = "00:00";
        String end = "00:00";
    ...
    private JSONObject createMultiAllDayEntry(
            String name, String where, String description, int expectedStatus) throws Exception {
    ...
    private JSONObject createEntry(
            String name, String where, String description, JSONObject datesJSON, int expectedStatus)
            throws Exception {
        JSONObject json = new JSONObject();
        ...
        Response response =
                sendRequest(
                        new PostRequest(URL_EVENT_CREATE, json.toString(), "application/json"),
                        expectedStatus);
        if (expectedStatus == Status.STATUS_OK) {
            JSONObject result = validateAndParseJSON(response.getContentAsString());
            if (result.has("event")) {
    ...
    private JSONObject updateEntry(
            String name,
            String what,
            String where,
            String description,
        ...
        Response response =
                sendRequest(
                        new PutRequest(URL_EVENT_BASE + name, json.toString(), "application/json"),
                        expectedStatus);
        if (expectedStatus == Status.STATUS_OK) {
            JSONObject result = validateAndParseJSON(response.getContentAsString());
            if (result.has("event")) {
    ...
    private String getNameFromEntry(JSONObject entry) throws Exception {
        ...
        String name =
                uri.substring(
                        uri.indexOf(SITE_SHORT_NAME_CALENDAR)
                                + SITE_SHORT_NAME_CALENDAR.length()
                                + 1);
    ...
    public void testCreateEditDeleteEntry() throws Exception {
        ...
        assertEquals(
                "2011-06-29",
                entry.getJSONObject("startAt").getString("legacyDate")); // Different format!
        assertEquals(
                "2011-06-29",
                entry.getJSONObject("endAt").getString("legacyDate")); // Different format!
        ...
        assertEquals(
                "2011-06-29T12:00:00.000+01:00", entry.getJSONObject("startAt").get("iso8601"));
        ...
        entry =
                updateEntry(
                        name, EVENT_TITLE_ONE, "More Where", "More Thing", false, Status.STATUS_OK);
        assertEquals("Error found " + entry.toString(), false, entry.has("error"));
        ...
        assertEquals(
                "2011-06-28T11:30", entry.getJSONObject("startAt").getString("legacyDateTime"));
        ...
        entry =
                updateEntry(
                        name, EVENT_TITLE_ONE, "More Where", "More Thing", true, Status.STATUS_OK);
        ...
        sendRequest(
                new PutRequest(URL_EVENT_BASE + name, "{}", "application/json"), Status.STATUS_OK);
        assertEquals(true, entry.has("error"));
    }
    ...
    public void testCreateAllDayEntry() throws Exception {
        ...
        entry =
                createAllDayEntryDifferentTimeZone(
                        EVENT_TITLE_ONE, "Where", "Thing", Status.STATUS_OK);
        name = getNameFromEntry(entry);
    ...
    public void testPermissions() throws Exception {
        ...
        assertEquals(
                ...
                siteService.getMembersRole(
                        SITE_SHORT_NAME_CALENDAR, authenticationComponent.getCurrentUserName()));
        assertEquals(
    ...
    /**
     * Checks that we can work with old style two part date times, ISO8601 datetimes with offsets,
     * and ISO8601 datetimes with an explicit timezone.
     */
    public void testDifferentDateStyles() throws Exception {
        ...
        assertEquals(
                "2011-06-21T11:30:05.000+01:00",
                entry.getJSONObject("startAt").getString("iso8601"));
        assertEquals(
                "2011-06-21T12:45:25.000+01:00", entry.getJSONObject("endAt").getString("iso8601"));
        ...
        // Check new style dates and times
        assertEquals(
                "2011-06-22T11:30:05.000+01:00",
                entry.getJSONObject("startAt").getString("iso8601"));
        assertEquals(
                "2011-06-22T12:45:25.000+01:00", entry.getJSONObject("endAt").getString("iso8601"));
        ...
        // Check new style dates and times
        assertEquals(
                "2011-06-20T09:35:05.000+01:00",
                entry.getJSONObject("startAt").getString("iso8601"));
        assertEquals(
                "2011-06-20T10:35:25.000+01:00", entry.getJSONObject("endAt").getString("iso8601"));
        ...
        // Check new style dates and times
        assertEquals(
                "2011-06-24T09:30:05.000+01:00",
                entry.getJSONObject("startAt").getString("iso8601"));
        assertEquals(
                "2011-06-24T10:45:25.000+01:00", entry.getJSONObject("endAt").getString("iso8601"));
    ...
    public void testOverallListing() throws Exception {
        ...
        // Check the dates
        assertEquals(
                "2011-06-29T12:00:00.000+01:00",
                entries.getJSONObject(0).getJSONObject("startAt").getString("iso8601"));
        assertEquals(
                "2011-06-29T13:00:00.000+01:00",
                entries.getJSONObject(0).getJSONObject("endAt").getString("iso8601"));
        ...
        updateEntry(
                name3, EVENT_TITLE_THREE, "More Where 3", "More Thing 3", false, Status.STATUS_OK);
    ...
    public void testUserListing() throws Exception {
        ...
        Response response =
                sendRequest(new GetRequest(URL_USER_EVENTS_LIST + "?from=2000/01/01"), 200);
    ...
    public void testRepeatingEventsInListings() throws Exception {
        ...
        assertEquals(
                ...
                events.getJSONObject(0)
                        .getJSONObject("startAt")
                        .getString("iso8601")
                        .substring(0, 11));
        // 1st repeat Wednesday
        assertEquals(
                "2011-06-29T",
                events.getJSONObject(1)
                        .getJSONObject("startAt")
                        .getString("iso8601")
                        .substring(0, 11));
        ...
        assertEquals(
                EVENT_TITLE_TWO, result.getJSONArray("6/28/2011").getJSONObject(0).get("name"));
        // 1st repeat Wednesday, then Non-repeating original
        assertEquals("6/29/2011", getKeys(result, true)[1]);
        assertEquals(2, result.getJSONArray("6/29/2011").length());
        assertEquals(
                EVENT_TITLE_TWO, result.getJSONArray("6/29/2011").getJSONObject(0).get("name"));
        assertEquals(
                EVENT_TITLE_ONE, result.getJSONArray("6/29/2011").getJSONObject(1).get("name"));
        // 1st repeat Friday
        ...
        assertEquals(
                EVENT_TITLE_TWO, result.getJSONArray("8/24/2011").getJSONObject(0).get("name"));

        assertEquals("8/26/2011", getKeys(result, true)[1]);
        assertEquals(1, result.getJSONArray("8/26/2011").length());
        assertEquals(
                EVENT_TITLE_TWO, result.getJSONArray("8/26/2011").getJSONObject(0).get("name"));

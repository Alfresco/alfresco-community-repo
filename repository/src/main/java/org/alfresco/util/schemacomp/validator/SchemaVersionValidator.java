 ...
 *
 * <p>For the target schema to pass validation its version number must be greater than or equal to
 * the reference schema's version number.
 *
 * @author Matt Ward
 */
public class SchemaVersionValidator extends AbstractDbValidator {
    ...
    public void validate(DbObject referenceObj, DbObject targetObj, DiffContext ctx) {
        ...
        if (target.getVersion() < reference.getVersion()) {
            ...
            String message =
                    I18NUtil.getMessage(
                            "system.schema_comp.schema_version_validator", reference.getVersion());

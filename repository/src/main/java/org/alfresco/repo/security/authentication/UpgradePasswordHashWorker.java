...
public class UpgradePasswordHashWorker
  ...
  private static final QName LOCK = QName.createQName(
  ...
  private static Log logger = LogFactory.getLog(
    UpgradePasswordHashWorker.class
  );
  ...
  public boolean processPasswordHash(Map<QName, Serializable> properties) {
    ...
    Pair<List<String>, String> passwordHash = RepositoryAuthenticationDao.determinePasswordHash(
      properties
    );
    ...
    if (!passwordEncoder.lastEncodingIsPreferred(passwordHash.getFirst())) {
      String username = (String) properties.get(
      ...
      if (passwordEncoder.isSafeToEncodeChain(nowHashed)) {
        ...
        properties.put(
          ...
          passwordEncoder.encodePreferred(
            new String(passwordHash.getSecond()),
            salt
          )
        );
        properties.put(
          ContentModel.PROP_HASH_INDICATOR,
          (Serializable) nowHashed
        );
    ...
    if (hashIndicator == null) {
      ...
      properties.put(
        ContentModel.PROP_HASH_INDICATOR,
        (Serializable) passwordHash.getFirst()
      );
   ...
   */
  private synchronized void doWork(UpgradePasswordHashWorkResult progress)
    throws Exception {
    // Build batch processor
    BatchProcessWorkProvider<Long> workProvider = new UpgradePasswordHashWorkProvider(
      progress
    );
  ...
  private class UpgradePasswordHashWorkProvider
    ...
    private UpgradePasswordHashWorkProvider(
    ...
    public int getTotalEstimatedWorkSize() {
      ...
      long totalUserCount = patchDAO.getCountNodesWithTypId(
    ...
    public Collection<Long> getNextWork() {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Upgrade password hash work provider found " + ret.size() + " users."
        );
      }
  ...
  private class UpgradePasswordHashBatch
    extends BatchProcessWorkerAdaptor<Long> {
    ...
    public void process(Long nodeId) throws Throwable {
      ...
      try {
        ...
        // get the username
        String username = (String) userProps.get(
          ContentModel.PROP_USER_USERNAME
        );
    ...
    public String getIdentifier(Long nodeId) {
      return (String) nodeDAO.getNodeProperty(
        nodeId,
        ContentModel.PROP_USER_USERNAME
      );
    }
  ...
  public static class UpgradePasswordHashJob implements Job {
    ...
    public void execute(JobExecutionContext context)
      ...
      if (
        upgradePasswordHashWorkerObj == null ||

...
public class RemoteUserAuthenticatorFactory
  ...
  private static Log logger = LogFactory.getLog(
    RemoteUserAuthenticatorFactory.class
  );
  ...
  long getRemoteUserTimeoutMilliseconds =
  ...
  public void setAlwaysAllowBasicAuthForAdminConsole(
    ...
    this.alwaysAllowBasicAuthForAdminConsole =
      alwaysAllowBasicAuthForAdminConsole;
  }
  ...
  public void setAdminConsoleScriptFamilies(
    List<String> adminConsoleScriptFamilies
  ) {
  ...
  public void setGetRemoteUserTimeoutMilliseconds(
    long getRemoteUserTimeoutMilliseconds
  ) {
  ...
  public class RemoteUserAuthenticator extends BasicHttpAuthenticator {
    ...
    public boolean authenticate(
      ...
      if (logger.isTraceEnabled()) {
        logger.trace(
          "Authenticate level required: " + required + " is guest: " + isGuest
        );
      }
      ...
      if (isRemoteUserMapperActive()) {
        if (isAlwaysAllowBasicAuthForAdminConsole()) {
          final boolean useTimeoutForAdminAccessingAdminConsole = shouldUseTimeoutForAdminAccessingAdminConsole(
            required,
            isGuest
          );
          ...
          try {
            userId =
              getRemoteUserWithTimeout(useTimeoutForAdminAccessingAdminConsole);
      ...
      if (userId != null) {
        try {
          ...
          Throwable disabledCause = ExceptionStackUtil.getCause(
            authErr,
        ...
        HttpSession session = servletReq
          ...
          try {
            ...
            if (user != null) {
              ...
              if (logger.isDebugEnabled()) {
                logger.debug(
                  "Ticket is valid. Retaining cached user in session."
                );
              }
              listener.userAuthenticated(
    ...
    private boolean callBasicAuthForAdminConsoleAccess(
      ...
      if (logger.isTraceEnabled()) {
        logger.trace(
          "An Admin Console request has come in with Basic Auth headers present for an admin user."
        );
      }
    ...
    protected String getRemoteUserWithTimeout(boolean useTimeout)
      ...
      try {
        ...
        logger.warn(
          "Exception trying to get the remote user: " + e.getMessage(),
          e
        );
      }
      ...
      if (workerGettingTheRemoteUser.isAlive()) {
        ...
        if (logger.isWarnEnabled()) {
          logger.warn(
            "Returning basic auth challenge for Admin Console. Cause: " +
            message
          );
        }
    ...
    protected String getRemoteUser() {
      ...
      if (isRemoteUserMapperActive()) {
        userId =
          remoteUserMapper.getRemoteUser(

 ...
 * All mimetypes and extensions are stored and handled as lowercase.
 *
 * @author Derek Hulley
 */
public class MimetypeMap implements MimetypeService {
    ...
    public void init() {
        ...
        if (mimetypeJsonConfigDir != null
            ...
            jsonConfigFileFinder =
                    new ConfigFileFinder(jsonObjectMapper) {
                        @Override
                        protected void readJson(
                                JsonNode jsonNode, String readFromMessage, String baseUrl)
                                throws IOException {
                            try {
                                    ...
                                    for (JsonNode mediaType : mediaTypes) {
                                        MediaTypeDef def =
                                                jsonObjectMapper.convertValue(
                                                        mediaType, MediaTypeDef.class);
                                        GenericConfigElement mimetype =
                                                new GenericConfigElement(ATTR_MIMETYPE);
                                        mimetype.addAttribute(ATTR_DISPLAY, def.name);
                                        mimetype.addAttribute(ATTR_MIMETYPE, def.mediaType);
                                        if (def.text) {
                                            mimetype.addAttribute(
                                                    ATTR_TEXT, Boolean.TRUE.toString());
                                        }
                                ...
                                logger.error(
                                        "Error reading " + readFromMessage + " " + e.getMessage());
                            }
                        }
                    };
        }
    ...
    public boolean readConfig() {
        // Config in XML
        Config config =
                configService.getConfig(CONFIG_CONDITION, new ConfigLookupContext(CONFIG_AREA));
        ...
        if (jsonConfigFileFinder != null) {
            ...
            if (mimetypeJsonConfigDir != null && !mimetypeJsonConfigDir.isBlank()) {
                successReadingConfig &=
                        jsonConfigFileFinder.readFiles(mimetypeJsonConfigDir, logger);
            }
        }
     ...
     * @return Returns the default extension for the mimetype. Returns the {@link #MIMETYPE_BINARY
     *     binary} mimetype extension.
     * @see #MIMETYPE_BINARY
    ...
    // We have a problem with .ai files, as Tika detects them as .pdf, but if we can use the
    // filename
    ...
    private MediaType typeBasedOnDetectedTypeAndExtension(MediaType type, String filename) {
        if (filename != null && type != null) {
            String[] detectedAndPossibleTypes =
                    new String[] {
    ...
    /**
     * Use Apache Tika to check if the mime type of the document really matches what it claims to
     * be. This is typically used when a transformation or metadata extractions fails, and you want
     * to know if someone has renamed a file and consequently it has the wrong mime type.
     *
    ...
    /**
     * Takes a guess at the mimetype based exclusively on the file extension, which can (and often
     * is) wrong...
     *
     * @see #MIMETYPE_BINARY
     */
    ...
    /**
     * Uses Tika to try to identify the mimetype of the file, falling back on {@link
     * #guessMimetype(String)} for an extension based one if Tika can't help.
     */
    ...
    public String guessMimetype(String filename, InputStream input) {
        ...
        // If Tika doesn't know what the type is, or file is password protected, go with the
        // filename one
        if (type == null
                || MediaType.OCTET_STREAM.equals(type)
                || MIMETYPE_ENCRYPTED_OFFICE.equals(type.toString())) {
        ...
        if ((MediaType.TEXT_PLAIN.equals(type) || MediaType.APPLICATION_XML.equals(type))
                && (!filenameGuess.equals(MIMETYPE_BINARY))) {
            return filenameGuess;
        }
        ...
        logger.info(
                "Tika detected a type of "
                        + tikaType
                        + " for file "
                        + filename
                        + " which Alfresco doesn't know about. Consider "

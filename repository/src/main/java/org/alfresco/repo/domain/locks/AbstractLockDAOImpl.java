...
public abstract class AbstractLockDAOImpl implements LockDAO {
  ...
  public void getLock(QName lockQName, String lockToken, long timeToLive) {
    ...
    if (!qnameLocalName.toLowerCase().equals(qnameLocalName)) {
      lockQName =
        QName.createQName(qnameNamespaceUri, qnameLocalName.toLowerCase());
    ...
    Long qnameNamespaceId = qnameDAO
      .getOrCreateNamespace(qnameNamespaceUri)
      .getFirst();

    // Get the lock resource for the exclusive lock.
    // All the locks that are created will need the exclusive case.
    LockResourceEntity exclusiveLockResource = getLockResource(
      qnameNamespaceId,
      qnameLocalName
    );
    if (exclusiveLockResource == null) {
      ...
      exclusiveLockResource =
        createLockResource(qnameNamespaceId, qnameLocalName);
    }
    ...
    for (QName lockQNameIter : lockQNames) {
      ...
      LockResourceEntity lockResource = getLockResource(
        qnameNamespaceId,
        localname
      );
    ...
    List<LockEntity> existingLocks = getLocksBySharedResourceIds(
      requiredLockResourceIds
    );
    ...
    for (LockEntity existingLock : existingLocks) {
      boolean canTakeLock = canTakeLock(
        existingLock,
        lockToken,
  ...
  private boolean canTakeLock(
    LockEntity existingLock,
    String lockToken,
    Long desiredExclusiveLock
  ) {
    if (EqualsHelper.nullSafeEquals(existingLock.getLockToken(), lockToken)) {
      ...
      desiredExclusiveLock.equals(existingLock.getSharedResourceId())
    ) {
   ...
   * @return                          Returns the lock, if it exists, otherwise <tt>null</tt>
   */
  protected abstract LockEntity getLock(

...
public class AuthorityBridgeTableAsynchronouslyRefreshedCacheTest
  ...
  private static final String TENANT_ADMIN_USER =
    AuthenticationUtil.getAdminUserName() + "@" + TENANT_DOMAIN;
  ...
  public void setUp() throws Exception {
    ...
    authorityService =
      (AuthorityService) ctx.getBean(
        ServiceRegistry.AUTHORITY_SERVICE.getLocalName()
      );
    tenantAdminService =
      ctx.getBean("tenantAdminService", TenantAdminService.class);
    personService =
      (PersonService) ctx.getBean(
        ServiceRegistry.PERSON_SERVICE.getLocalName()
      );
  ...
  public void testAuthorityBridgeTableCacheForTenants() throws Exception {
    ...
    final String tenantChildGroup = transactionService
      ...
      .doInTransaction(
        new RetryingTransactionHelper.RetryingTransactionCallback<String>() {
          ...
          public String execute() throws Throwable {
            ...
            String tenantChildGroup = authorityService.createAuthority(
              AuthorityType.GROUP,
              childGroupName
            );
            assertNotNull(
              authorityService.getAuthorityNodeRef(tenantChildGroup)
            );
    ...
    final String tenantParentGroup = transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(
        new RetryingTransactionHelper.RetryingTransactionCallback<String>() {
          @Override
          public String execute() throws Throwable {
            String tenantParentGroup = authorityService.createAuthority(
              AuthorityType.GROUP,
              parentGroupName
            );
            assertNotNull(
              authorityService.getAuthorityNodeRef(tenantParentGroup)
            );
    ...
    Set<String> authorities = authorityService.getContainingAuthorities(
      null,
      tenantPersonName,
      false
    );
  ...
  public void testCyclicGroups() {
    ...
    when(authorityBridgeDAOMock.getAuthorityBridgeLinks())
      .thenReturn(cyclicLinks);
    ...
    doAnswer(
      ...
      .when(authorityDAOMock)
      .removeAuthority(anyString(), anyString(), anyBoolean());
  ...
  private AuthorityBridgeLink createAuthorityBridgeLink(
    String parentName,
    String childName
  ) {

...
public class DictionaryRepositoryBootstrap
  ...
  implements
    TenantDeployer,
    DictionaryListener,
    /*TenantDictionaryListener, */MessageDeployer {

  // Logging support
  private static Log logger = LogFactory.getLog(
    DictionaryRepositoryBootstrap.class
  );
  ...
  private void onDictionaryInitInTxn() {
    ...
    if (this.repositoryModelsLocations != null) {
      ...
      if (logger.isTraceEnabled()) {
        logger.trace(
          "onDictionaryInit: locations=" + this.repositoryModelsLocations
        );
      }
  ...
  public void notifyDynamicModelLoaded(DynamicModelInfo entry) {
    ...
    DynamicModelPolicies.OnLoadDynamicModel policy = onLoadDynamicModelDelegate.get(
    ...
    if (this.repositoryMessagesLocations != null) {
      ...
      for (RepositoryLocation repositoryLocation : this.repositoryMessagesLocations) {
        ...
        if (
          ...
          List<NodeRef> nodeRefs = getNodes(
            storeRef,
            repositoryLocation,
            ContentModel.TYPE_CONTENT
          );
          ...
          if (nodeRefs.size() > 0) {
            ...
            for (NodeRef messageResource : nodeRefs) {
              ...
              String bundleBaseName = messageService.getBaseBundleName(
                resourceName
              );
  ...
  protected List<NodeRef> getNodes(
    ...
    if (repositoryLocation instanceof DynamicCreateRepositoryLocation) {
      ((DynamicCreateRepositoryLocation) repositoryLocation).checkAndCreate(
          rootNodeRef
        );
    }
  ...
  public M2Model createM2Model(NodeRef nodeRef) {
    ...
    if (contentReader != null) {
      if (contentReader instanceof EmptyContentReader) {
        // belts-and-braces
        logger.error(
          "Failed to create model (due to EmptyContentReader): " + nodeRef
        );
      } else {
        InputStream is = null;
        try {
  ...
  protected void onBootstrap(ApplicationEvent event) {
    ...
    ((ApplicationContext) event.getSource()).publishEvent(
        new DictionaryRepositoryBootstrappedEvent(this)
      );
  }

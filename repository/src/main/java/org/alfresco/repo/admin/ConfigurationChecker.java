 ...
 *
 * <ul>
 *   <li>Checks that an absolute path is used
 *   <li>Ensures that the system descriptor content is available (created at bootstrap)
 * </ul>
 *
 * @author Derek Hulley
 */
public class ConfigurationChecker extends AbstractLifecycleBean {
     ...
     * Generally, the system should be <b>strict</b>, but this can be changed if indexes are going
     * to be recovered, or if missing content is acceptable.
     *
    ...
    protected void onBootstrap(ApplicationEvent event) {
        RetryingTransactionCallback<Object> checkWork =
        ...
        if (descriptorNodeRef != null) {
            ...
            ContentReader reader =
                    contentService.getReader(
                            descriptorNodeRef, ContentModel.PROP_SYS_VERSION_PROPERTIES);
        ...
        if (nodeService.exists(systemStoreRef)) {
            ...
            nodeRefs =
                    searchService.selectNodes(
                            rootNodeRef, searchPath, null, namespaceService, false);
            if (nodeRefs.size() > 0) {

 ...
 *
 * <p>Each filter is called before and after the processor generates and persists the form, thus
 * allowing the form and the effected objects to be manipulated prior to generation or persistence
 * or after the fact.
 *
 * <p>Each filter is responsible for determing whether it applies to the item being processed.
 *
...
public class FilterRegistry<ItemType, PersistType> {
    ...
    public void addFilter(Filter<ItemType, PersistType> filter) {
        if (filter.isActive()) {
            ...
            logger.warn(
                    "Ignored registration of filter " + filter + " as it was marked as inactive");
        }
    }
    ...
    public List<Filter<ItemType, PersistType>> getFilters() {
        List<Filter<ItemType, PersistType>> activeFilters =
                new ArrayList<Filter<ItemType, PersistType>>(4);

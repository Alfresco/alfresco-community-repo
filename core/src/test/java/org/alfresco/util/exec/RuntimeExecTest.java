...
public class RuntimeExecTest extends TestCase {
    ...
    private void longishRunningProcess(int runFor, long timeout) {
        ...
        assertTrue(
                "The timeout when set plus " + marginOfError + "ms must less than the runFor value",
        ...
        commandMap.put(
                "Windows.*",
                new String[] {
                    "ping", "-n", "" + (runFor / 1000 + 1), "127.0.0.1"
        ...
        if (shouldComplete) assertEquals("Didn't expect error code", 0, ret.getExitValue());

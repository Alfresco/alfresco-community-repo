 ...
 *
 * <p>The {@linkplain #setSelectors(List) selectors} are used to find an extracter most appropriate
 * of a given XML document. The chosen extracter is then asked to extract the values, passing
 * through the {@code MetadataExtracter.OverwritePolicy} as {@linkplain
 * #setOverwritePolicy(org.alfresco.repo.content.metadata.MetadataExtracter.OverwritePolicy)} on
 * this instance. The overwrite policy of the embedded extracters is not relevant unless they are
 * used separately in another context.
 *
...
public class XmlMetadataExtracter extends AbstractMappingMetadataExtracter {
     ...
     * The evaluations are done in the order that they occur in the list.
     *
    ...
    public Map<QName, Serializable> extract(
        ...
        for (ContentWorkerSelector<MetadataExtracter> selector : selectors) {
            ...
            try {
                ...
                if (reader.isChannelOpen()) {
                    logger.error(
                            "Content reader not closed by MetadataExtractor selector: \n"
                                    + "   reader:   "
                                    + reader
                                    + "\n"
                                    + "   selector: "
        ...
        if (extracter == null) {
            ...
            try {
                modifiedProperties =
                        extracter.extract(reader, overwritePolicy, destination, mapping);
            } finally {
                if (reader.isChannelOpen()) {
                    logger.error(
                            "Content reader not closed by MetadataExtractor: \n"
                                    + "   Reader:   "
                                    + reader
                                    + "\n"
                                    + "   extracter: "

...
public class SSOFallbackBasicAuthenticationDriver
  ...
  private Log logger = LogFactory.getLog(
    SSOFallbackBasicAuthenticationDriver.class
  );
  ...
  public boolean authenticateRequest(
    ...
    SessionUser user = session == null
      ? null
      : (SessionUser) session.getAttribute(userAttributeName);
    if (user == null) {
      if (
        authHdr != null &&
        authHdr.length() > 5 &&
        ...
        try {
          ...
          final RetryingTransactionCallback<SessionUser> callback = new RetryingTransactionCallback<SessionUser>() {
            ...
            public SessionUser execute() throws Throwable {
              NodeRef personNodeRef = personService.getPerson(
              ...
              NodeRef homeSpaceRef = (NodeRef) nodeService.getProperty(
                personNodeRef,
  ...
  public void restartLoginChallenge(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Including Basic HTTP authentication into response headers..."
      );
    }

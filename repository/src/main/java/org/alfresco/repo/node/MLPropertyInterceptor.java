...
/**
 * Interceptor to filter out multilingual text properties from getter methods and transform to
 * multilingual text for setter methods.
 *
 * <p>This interceptor ensures that all multilingual (ML) text is transformed to the locale chosen
 * for the request for getters and transformed to the default locale type for setters.
 *
 * <p>Where {@link org.alfresco.service.cmr.repository.MLText ML text} has been passed in, this will
 * be allowed to pass.
 *
...
public class MLPropertyInterceptor implements MethodInterceptor {
    ...
    /**
     * Change the filtering behaviour of this interceptor on the curren thread. Use this to switch
     * off the filtering and just pass out properties as handed out of the node service.
     *
     * @param mlAwareVal <tt>true</tt> if the current thread is able to handle {@link MLText
     *     d:mltext} property types, otherwise <tt>false</tt>.
    ...
    /**
     * @return Returns <tt>true</tt> if the current thread has marked itself as being able to handle
     *     {@link MLText d:mltext} types properly.
     */
    ...
    public Object invoke(final MethodInvocation invocation) throws Throwable {
        ...
        if (methodName.equals("getProperty")) {
            ...
            Map<QName, Serializable> convertedProperties =
                    new HashMap<QName, Serializable>(properties.size() * 2);
            // Check each return value type
            for (Map.Entry<QName, Serializable> entry : properties.entrySet()) {
                QName propertyQName = entry.getKey();
                Serializable value = entry.getValue();
                Serializable convertedValue =
                        convertOutboundProperty(nodeRef, pivotNodeRef, propertyQName, value);
            ...
            Map<QName, Serializable> convertedProperties =
                    convertInboundProperties(
                            null, newProperties, contentLangLocale, nodeRef, pivotNodeRef);
            // Now complete the call by passing the converted properties
            ret =
    ...
    /**
     * @param nodeRef a potential empty translation
     * @return the pivot translation node or <tt>null</tt>
     */
    private NodeRef getPivotNodeRef(NodeRef nodeRef) {
        if (nodeRef == null) {
            throw new IllegalArgumentException(
                    "NodeRef may not be null for calls to NodeService.  Check client code.");
        }
        ...
        } else if (pivotNodeRef != null) // It is an empty translation
        {
            if (propertyQName.equals(ContentModel.PROP_MODIFIED)) {
                ...
                Date pivotLastModified =
                        (Date) nodeService.getProperty(pivotNodeRef, ContentModel.PROP_MODIFIED);
    ...
    private Map<QName, Serializable> convertInboundProperties(
            ...
            NodeRef pivotNodeRef) {
        Map<QName, Serializable> convertedProperties =
                new HashMap<QName, Serializable>(newProperties.size() * 2);
        for (Map.Entry<QName, Serializable> entry : newProperties.entrySet()) {
            ...
            Serializable currentValue =
                    currentProperties == null ? null : currentProperties.get(propertyQName);
    ...
    private Serializable convertInboundProperty(
        ...
        if (propertyDef == null) {
            ...
            if (inboundValue instanceof MLText) {
                ...
                if (isCollectionOfMLText(inboundValue)) {
                    ...
                    if (currentValue != null) {
                        Collection<MLText> currentCollection =
                                DefaultTypeConverter.INSTANCE.getCollection(
                                        MLText.class, currentValue);
                        returnMLList.addAll(currentCollection);
                    }
                    Collection<String> inboundCollection =
                            DefaultTypeConverter.INSTANCE.getCollection(String.class, inboundValue);
                    int count = 0;
                ...
                if (currentValue != null) {
                    MLText currentMLValue =
                            DefaultTypeConverter.INSTANCE.convert(MLText.class, currentValue);
                    returnMLValue.putAll(currentMLValue);
                }
                // Force the inbound value to be a String (it isn't MLText)
                String inboundValueStr =
                        DefaultTypeConverter.INSTANCE.convert(String.class, inboundValue);
            ...
            ContentData pivotContentData =
                    (ContentData) nodeService.getProperty(pivotNodeRef, ContentModel.PROP_CONTENT);
            ContentData emptyContentData = (ContentData) inboundValue;
            String pivotContentUrl =
                    pivotContentData == null ? null : pivotContentData.getContentUrl();
            String emptyContentUrl =
                    emptyContentData == null ? null : emptyContentData.getContentUrl();
    ...
    /**
     * Replace any text in mlText having the same language (but any variant) as contentLocale with
     * updatedText keyed by the language of contentLocale. This ensures that the mlText will have no
     * more than one entry for the particular language.
     *

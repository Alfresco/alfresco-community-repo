...
public class NodeHierarchyWalker {
  ...
  private final Map<Long, VisitedNode> nodesVisitedById = new HashMap<Long, VisitedNode>(
    59
  );
  /** Store for all nodes by ID */
  private final Map<NodeRef, VisitedNode> nodesVisitedByNodeRef = new HashMap<NodeRef, VisitedNode>(
    59
  );
  /** Store all the nodes visited from the leaf nodes up */
  private final List<VisitedNode> nodesLeafToParent = new ArrayList<VisitedNode>(
    67
  );
  /** Store all the nodes visited from parent down */
  private final List<VisitedNode> nodesParentToLeaf = new ArrayList<VisitedNode>(
    67
  );
  ...
  public void walkHierarchy(
    ...
    VisitedNode visitedNode = new VisitedNode(
      nodeId,
      nodeRef,
      nodeType,
      nodeAclId,
  ...
  private void walkNode(Long nodeId) {
    ...
    if (nodeVisited == null) {
      throw new IllegalStateException(
        "Parent node has not been visited: " + nodeId
      );
    }
    ...
    NodeDAO.ChildAssocRefQueryCallback getParentAssocs = new NodeDAO.ChildAssocRefQueryCallback() {
      ...
      public boolean handle(
        ...
        VisitedNode visitedNode = nodesVisitedById.get(
          childNodePair.getFirst()
        );
        if (visitedNode == null) {
          throw new IllegalStateException(
            "Querying upwards found nodes not visited: " + childNodePair
          );
        }
        if (childAssocPair.getSecond().isPrimary()) {
          ...
          if (!visitedNode.primaryParentAssocPair.equals(childAssocPair)) {
            ...
            throw new ConcurrencyFailureException(
              "Node parent changed while hierarchy was being examined: " +
              childNodePair
            );
          }
        } else {
    ...
    if (visitedNode == null) {
      throw new IllegalStateException(
        "Querying upwards found nodes not visited: " + nodeId
      );
    }
  ...
  public class VisitedNode {
    ...
    private VisitedNode(
      ...
      this.secondaryParentAssocs =
        new ArrayList<Pair<Long, ChildAssociationRef>>(17);
      this.secondaryChildAssocs =
        new ArrayList<Pair<Long, ChildAssociationRef>>(17);

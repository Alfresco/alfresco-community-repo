...
public class WebDAVLockServiceImpl implements WebDAVLockService {
  ...
  public void sessionDestroyed() {
    ...
    final List<Pair<String, NodeRef>> lockedResources = (List<Pair<String, NodeRef>>) session.getAttribute(
      LOCKED_RESOURCES
    );
    ...
    if (lockedResources != null && lockedResources.size() > 0) {
                  ...
                  new RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                      ...
                      if (nodeService.exists(nodeRef)) {
                        if (logger.isDebugEnabled()) {
                          logger.debug(
                            "Trying to release lock for: " + nodeRef
                          );
                        }

                        // check the lock status of document
                        LockStatus lockStatus = lockService.getLockStatus(
                          nodeRef
                        );

                        // check if document was checked out
                        boolean hasWorkingCopy =
                          checkOutCheckInService.getWorkingCopy(nodeRef) !=
                          null;
                        ...
                        if (
                          (
                            lockStatus.equals(LockStatus.LOCKED) ||
                            lockStatus.equals(LockStatus.LOCK_OWNER)
                          ) &&
                          !hasWorkingCopy &&
                          !isWorkingCopy
                        ) {
                        ...
                        if (logger.isDebugEnabled()) {
                          logger.debug(
                            "Skip lock releasing for an unexisting node: " +
                            nodeRef
                          );
                        }
                      }
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "No locked resources were found for session: " + session.getId()
        );
      }
    }
  }
  ...
  public void lock(NodeRef nodeRef, LockInfo lockInfo) {
    ...
    if (
      timeout >= WebDAV.TIMEOUT_24_HOURS || timeout == WebDAV.TIMEOUT_INFINITY
    ) {
  ...
  public void unlock(NodeRef nodeRef) {
    ...
    boolean removed = removeObjectFromSessionList(
      session,
      LOCKED_RESOURCES,
      new Pair<String, NodeRef>(AuthenticationUtil.getRunAsUser(), nodeRef)
    );
    ...
    if (removed && logger.isDebugEnabled()) {
      logger.debug(
        nodeRef + " was removed from the session " + session.getId()
      );
    }
  }

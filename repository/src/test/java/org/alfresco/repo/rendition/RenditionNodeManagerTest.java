...
public class RenditionNodeManagerTest extends TestCase {
  ...
  private final RenditionService renditionService = mock(
    RenditionService.class
  );
  ...
  private final NodeRef oldRendition = new NodeRef(
    "http://test/oldRenditionId"
  );

  private final QName renditionName = QName.createQName(
    NamespaceService.RENDITION_MODEL_1_0_URI,
    "renditionName"
  );
  private final RenditionDefinition definition = new RenditionDefinitionImpl(
    "definitionId",
    renditionName,
    "engineName"
  );
  ...
  public void testNoOldRenditionAndDestinationSpecified() {
    ...
    when(nodeService.getPrimaryParent(any(NodeRef.class)))
      .thenReturn(parentAssoc);
    RenditionLocation location = new RenditionLocationImpl(
      source,
      destination,
      "destinationName"
    );
    RenditionNodeManager manager = new RenditionNodeManager(
      source,
      null,
      location,
      definition,
  ...
  public void testNoOldRenditionAndNoDestinationSpecifiedAndParentIsSource() {
    Map<QName, Serializable> indexProps = Collections.singletonMap(
  ...
  public void testNoOldRenditionAndNoDestinationSpecifiedAndParentIsNotSource() {
    ...
    ChildAssociationRef parentAssoc = makeAssoc(
      parent,
      destination,
      assocName,
      false
  ...
  public void testHasOldRenditionMatchesSpecifiedDestinationNode() {
    ...
    when(renditionService.getRenditionByName(source, renditionName))
  ...
  public void testHasOldRenditionWrongParentSpecified() {
    ...
    ChildAssociationRef newParentAssoc = makeAssoc(
      newParent,
      oldRendition,
      false
    );
  ...
  public void testHasOldRenditionCorrectParentWrongNameSpecified() {
    ...
    assertEquals(parentAssoc, result);
    verify(nodeService)
      .moveNode(oldRendition, parent, ContentModel.ASSOC_CONTAINS, assocName);
  }

  private ChildAssociationRef makeAssoc(
    NodeRef parent,
    NodeRef child,
    boolean isRenditionAssoc
  ) {
  ...
  private ChildAssociationRef makeAssoc(
    ...
    QName assocType = isRenditionAssoc
      ? RenditionModel.ASSOC_RENDITION
      : ContentModel.ASSOC_CONTAINS;

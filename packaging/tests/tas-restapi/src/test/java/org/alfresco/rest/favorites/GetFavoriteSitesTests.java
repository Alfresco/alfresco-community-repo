...
public class GetFavoriteSitesTests extends RestTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify manager user fails to get an user favorite sites with Rest API (403)")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify collaborator user fails to get an user favorite sites with Rest API"
                            + " (403)")
    public void collaboratorUserFailsToGetAnUserFavoriteSites() throws Exception {
        UserModel collaboratorUser = dataUser.usingAdmin().createRandomTestUser();
        dataUser.usingUser(userModel)
                .addUserToSite(collaboratorUser, siteModel, UserRole.SiteCollaborator);
        UserModel contributorUser = dataUser.usingAdmin().createRandomTestUser();
        dataUser.usingUser(userModel)
                .addUserToSite(contributorUser, siteModel, UserRole.SiteContributor);
        dataSite.usingUser(contributorUser).usingSite(siteModel).addSiteToFavorites();

        restClient
                .authenticateUser(collaboratorUser)
                .withCoreAPI()
                .usingUser(contributorUser)
                .getFavoriteSites();
        restClient
                .assertStatusCodeIs(HttpStatus.FORBIDDEN)
                .assertLastError()
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify contributor user fails to get an user favorite sites with Rest API"
                            + " (403)")
    public void contributorUserFailsToGetAnUserFavoriteSites() throws Exception {
        ...
        dataUser.usingUser(userModel)
                .addUserToSite(contributorUser, siteModel, UserRole.SiteContributor);
        dataUser.usingUser(userModel)
                .addUserToSite(contributorUser2, siteModel, UserRole.SiteContributor);
        dataSite.usingUser(contributorUser2).usingSite(siteModel).addSiteToFavorites();

        restClient
                .authenticateUser(contributorUser)
                .withCoreAPI()
                .usingUser(contributorUser2)
                .getFavoriteSites();
        restClient
                .assertStatusCodeIs(HttpStatus.FORBIDDEN)
                .assertLastError()
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify consumer user fails to get an user favorite sites with Rest API (403)")
    public void consumerUserFailsToGetAnUserFavoriteSites() throws Exception {
        ...
        dataUser.usingUser(collaboratorUser)
                .addUserToSite(collaboratorUser, siteModel, UserRole.SiteCollaborator);
    ...
    @TestRail(
            ...
            description =
                    "Verify any user gets its own user favorite sites with Rest API and response is"
                            + " successful (200)")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Verify any user is NOT Authorized to get its favorite sites with Rest API when"
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that getFavoriteSites request status code is 404 for a personId that"
                            + " does not exist")
    @Test(groups = {TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.REGRESSION})
    public void getFavoriteSitesReturns404ForAPersonIdThatDoesNotExist() throws Exception {
        ...
        restClient
                ...
                .containsSummary(
                        String.format(RestErrorModel.ENTITY_NOT_FOUND, invalidUser.getUsername()))
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that getFavoriteSites request status code is 403 if the user doesn't"
                            + " have access to the person favorite sites")
    @Test(groups = {TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.REGRESSION})
    ...
    public void regularUserCannotGetFavoriteSitesForAdminUser() throws Exception {
        restClient
                ...
                .withCoreAPI()
                .usingUser(dataUser.getAdminUser())
                .getFavoriteSites();
        restClient
                .assertStatusCodeIs(HttpStatus.FORBIDDEN)
                .assertLastError()
    ...
    public void verifyThatFavoriteSiteCanBeRemoved() throws Exception {
        ...
        restSiteModelsCollection =
                restClient
                        .authenticateUser(randomTestUser)
                        .withCoreAPI()
                        .usingAuthUser()
                        .getFavoriteSites();
        restClient.assertStatusCodeIs(HttpStatus.OK);
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that if maxItems param is invalid status code returned is BAD_REQUEST"
                            + " (400)")
    @Test(groups = {TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that if skipCount param is invalid status code returned is BAD_REQUEST"
                            + " (400)")
    @Test(groups = {TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that getFavoriteSites request response status using -me- string in"
                            + " place of personId is OK (200)")
    @Test(groups = {TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.REGRESSION})
    public void userGetsFavoriteSiteWithSuccessUsingMEForRequest() throws Exception {
        restSiteModelsCollection =
                restClient.authenticateUser(testUser1).withCoreAPI().usingMe().getFavoriteSites();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        restSiteModelsCollection
                .assertThat()
                .entriesListIsNotEmpty()
                .and()
                .entriesListContains("id", testSite1.getId())
                .and()
                .paginationExist();
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that getFavoriteSites request response status for a user that has no"
                            + " favorite sites is OK (200)")
    @Test(groups = {TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that getFavoriteSites request response status for a user with several"
                            + " favorite sites is OK (200)")
    @Test(groups = {TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.REGRESSION})
    public void getFavoriteSiteRequestForUserWithSeveralFavoriteSitesIsSuccessful()
            throws Exception {
        restSiteModelsCollection =
                restClient
                        .authenticateUser(testUser1)
        ...
        restSiteModelsCollection
                ...
                .entriesListContains("id", testSite1.getId())
                .and()
                .entriesListContains("id", testSite2.getId())
                .and()
                .entriesListContains("id", testSite3.getId())
                .and()
                .paginationExist();
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that getFavoriteSites request can only be called with a positive"
                            + " maxItems param")
    @Test(groups = {TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that getFavoriteSites request response status code for a high skipCount"
                            + " param is OK (200)")
    @Test(groups = {TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.REGRESSION})
    ...
    public void userCanRetrieveLast2FavoriteSites() throws Exception {
        restSiteModelsCollection =
                restClient
                        .authenticateUser(testUser1)
                        .withParams("skipCount=1&maxCount=2")
                        .withCoreAPI()
                        .usingAuthUser()
                        .getFavoriteSites();
        restClient.assertStatusCodeIs(HttpStatus.OK);
    ...
    public void getFavoriteSitesRequestWithValidPropertiesParam() throws Exception {
        RestSiteModel restSiteModel =
                restClient
                        ...
                        .onModel();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        restSiteModel.assertThat().fieldsCount().is(1).assertThat().field("title").isNotEmpty();
    }

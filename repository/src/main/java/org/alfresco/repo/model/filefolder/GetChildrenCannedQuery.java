 ...
 *
 * <p>To get paged list of files and folders of a parent folder filtered by child type. Also
 * optionally filtered and/or sorted by one or more properties (up to three).
 *
 * <p>This is the same as the nodes getchildren canned query, except it takes into account hidden
 * files and folders (with respect to client visibility) and other aspect(s) to ignore - eg.
 * optionally "cm:checkedOut" in case of Share DocLib.
 *
 * @since 4.1.1
...
public class GetChildrenCannedQuery
    ...
    protected UnsortedChildQueryCallback getUnsortedChildQueryCallback(
        ...
        UnsortedChildQueryCallback callback =
                new FileFolderUnsortedChildQueryCallback(
    ...
    protected FilterSortChildQueryCallback getFilterSortChildQuery(
            ...
            GetChildrenCannedQueryParams paramBean) {
        FilterSortChildQueryCallback callback =
                new FileFolderFilterSortChildQueryCallback(children, filterProps);
        return callback;
    }

    private class FileFolderFilterSortChildQueryCallback
            extends DefaultFilterSortChildQueryCallback {
        private Map<QName, Boolean> isTypeFolderMap = new HashMap<QName, Boolean>(10);

        public FileFolderFilterSortChildQueryCallback(
                List<FilterSortNode> children, List<FilterProp> filterProps) {
        ...
        public boolean handle(FilterSortNode node) {
            ...
            if (nodeTypeQName != null) {
                ...
                if (isFolder == null) {
                    isFolder =
                            dictionaryService.isSubClass(nodeTypeQName, ContentModel.TYPE_FOLDER);
    ...
    private class FileFolderUnsortedChildQueryCallback extends DefaultUnsortedChildQueryCallback {
        public FileFolderUnsortedChildQueryCallback(
                List<NodeRef> rawResult,
                int requestedCount,

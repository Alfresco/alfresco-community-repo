...
public class RMEntryVoterUnitTest extends BaseUnitTest {
    ...
    public void before() throws Exception {
        ...
        // don't run as system
        when(mockedAuthenticationUtil.isRunAsUserTheSystemUser()).thenReturn(false);

        // indicate that "vote" transaction value is not set
        when(mockedTransactionalResourceHelper.isResourcePresent("voting")).thenReturn(false);
    }

    /** Given that the system is already voting When I vote Then access granted */
    @Test
    public void alreadyVoting() throws Exception {
        ...
        MethodInvocation mockedMethodInvocation =
                createMethodInvoation("myTestMethod", NodeRef.class);
        net.sf.acegisecurity.ConfigAttributeDefinition mockedConfigDef =
                createConfigDefinition("RM.invalid");
    ...
    /** Given that I am running this as the system user When I evaluate Then access granted */
    @Test
    public void runAsSystem() throws Exception {
        // run as system
        when(mockedAuthenticationUtil.isRunAsUserTheSystemUser()).thenReturn(true);
    ...
    /**
     * Given that we have provided an invalid policy When I evaluate the voter Then an
    ...
    /**
     * Given that I have provided a valid policy When I evaluate the voter Then the corresponding
    ...
    public void validPolicy() throws Exception {
        ...
        MethodInvocation mockedMethodInvocation =
                createMethodInvoation("myTestMethod", NodeRef.class);
        net.sf.acegisecurity.ConfigAttributeDefinition mockedConfigDef =
                createConfigDefinition("RM." + POLICY_NAME);

        // call vote
        entryVoter.vote(mockedAuthentication, mockedMethodInvocation, mockedConfigDef);

        // verify that the policy was executed
        verify(mockedPolicy, times(1))
                .evaluate(
                        eq(mockedMethodInvocation),
                        any(Class[].class),
                        any(ConfigAttributeDefinition.class));
    }
    ...
    private net.sf.acegisecurity.ConfigAttributeDefinition createConfigDefinition(String value) {
        net.sf.acegisecurity.ConfigAttributeDefinition mockedConfig =
                mock(net.sf.acegisecurity.ConfigAttributeDefinition.class);
    ...
    private MethodInvocation createMethodInvoation(String methodName, Class<?>... parameterTypes)
        ...
        when(mockedMethodInvocation.getMethod()).thenReturn(method);

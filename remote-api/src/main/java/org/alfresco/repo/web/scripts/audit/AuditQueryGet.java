        ...
        if (appName == null) {
            ...
            if (app == null) {
                throw new WebScriptException(
                        Status.STATUS_NOT_FOUND, "audit.err.app.notFound", appName);
            }
        }
        ...
        if (value != null && valueType != null) {
            try {
                ...
                Class<? extends Serializable> clazz =
                        (Class<? extends Serializable>) Class.forName(valueType);
                value = DefaultTypeConverter.INSTANCE.convert(clazz, value);
            } catch (ClassNotFoundException e) {
                throw new WebScriptException(
                        Status.STATUS_BAD_REQUEST, "audit.err.value.classNotFound", valueType);
            } catch (Throwable e) {
                throw new WebScriptException(
                        Status.STATUS_BAD_REQUEST,
                        "audit.err.value.convertFailed",
                        value,
                        valueType);
            }
        }
                        ...
                        if (values != null) {
                            ...
                            for (Map.Entry<String, Serializable> mapEntry : values.entrySet()) {
                                ...
                                try {
                                    ...
                                    // Fix for symptoms of MNT-20992. It is possible to have MLText
                                    // values whose underlying
                                    // map's keys are not Locale, as MLText map keys should be. In
                                    // this case we can expect
                                    // a ClassCastException.
                                    if (!(value instanceof MLText)) {
                                        // Rethrow if the exception was not caused by the expected
                                        // MLText conversion.
                                        throw e;
                                    }

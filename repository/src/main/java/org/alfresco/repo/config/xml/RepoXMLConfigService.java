...
public class RepoXMLConfigService extends XMLConfigService implements TenantDeployer {
     ...
     * This is required because the super classes call back into this mechanism after receiving the
     * initial object, etc.
     */
    ...
    /**
     * Get the repository configuration data for a given tenant. This does the low-level
     * initialization of the configuration and does not do any caching.
     *
    ...
    public ConfigData getRepoConfig(final String tenantDomain) {
        final RetryingTransactionCallback<Void> getConfigWork =
        ...
        TenantUtil.TenantRunAsWork<Void> getConfigRunAs =
                new TenantUtil.TenantRunAsWork<Void>() {
                    @Override
                    public Void doWork() throws Exception {
                        transactionService
                                .getRetryingTransactionHelper()
                                .doInTransaction(getConfigWork, true);
                        return null;
                    }
                };
        ...
        try {
            ...
            // Put some mutable config onto the current thread and have the superclasses mess with
            // that.
    ...
    /**
     * Fetch the tenant-specific config data from the cache. If nothing is available then the config
     * will be created.
     *
     * <p>Note that this method is used during construction of the config as well. When this occurs,
     * a thread local value is used in place of the cached value. It's not pretty.
     *
    ...
    protected void removeGlobalConfig() {
        throw new UnsupportedOperationException(
                "'destroy' method must destroy all config.  Piecemeal destruction is not"
                        + " supported.");
    }

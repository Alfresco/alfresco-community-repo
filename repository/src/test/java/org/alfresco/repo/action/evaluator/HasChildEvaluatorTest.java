...
public class HasChildEvaluatorTest extends BaseSpringTest {
    ...
    public void before() throws Exception {
        ...
        this.evaluator =
                (HasChildEvaluator) this.applicationContext.getBean(HasChildEvaluator.NAME);
    }
    ...
    public void testPass() {
        ...
        condition.setParameterValue(
                HasChildEvaluator.PARAM_ASSOC_TYPE, ContentModel.ASSOC_CHILDREN);
        assertTrue(this.evaluator.evaluate(condition, this.parentNodeRef));
    ...
    public void testFail() {
        ...
        condition.setParameterValue(
                HasChildEvaluator.PARAM_ASSOC_TYPE, ContentModel.ASSOC_ATTACHMENTS);
        assertFalse(this.evaluator.evaluate(condition, this.parentNodeRef));

        // node has child of unmatched assoc name
        condition = new ActionConditionImpl(ID, HasChildEvaluator.NAME, null);
        condition.setParameterValue(
                HasChildEvaluator.PARAM_ASSOC_NAME, QName.createQName("{foo}noSuchName"));
        assertFalse(this.evaluator.evaluate(condition, this.parentNodeRef));
    }
}

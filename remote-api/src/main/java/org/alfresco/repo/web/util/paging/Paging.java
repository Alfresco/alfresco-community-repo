 ...
 * are 0 based.
 *
 * <p>At any time, -1 is returned to represent "out of range" i.e. for next, previous, last page.
 *
 * <p>Pseudo-code for traversing through a collection of N items (10 at a time):
 *
 * <p>Paging paging = new Paging(); Cursor page = paging.createCursor(N, paging.createPage(1, 10));
 * while (page.isInRange()) { for (long i = page.getStartRow(); i <= page.getEndRow(); i++) {
 * ...collection[i]... } page = paging.createCursor(N, paging.createPage(page.getNextPage(),
 * page.getPageSize()); }
 *
 * <p>Cursor window = paging.createCursor(N, paging.createWindow(0, 10)); while (window.isInRange())
 * { for (long i = window.getStartRow(); i <= window.getEndRow(); i++) { ...collection[i]... }
...
public class Paging {
     ...
     *
     * <p>For Paged based index (take precedence over window based index, if both are specified):
     *
     * <p>- request args pageNo => page number index pageSize => size of page
     *
     * <p>For Window based index (as defined by CMIS):
     *
     * <p>- request args (take precedence over header values if both are specified) skipCount => row
     * number start index maxItems => size of page
     *
     ...
     * @return page (if pageNumber driven) or window (if skipCount driven)
     */
    public Page createPageOrWindow(
            Integer pageNumber, Integer pageSize, Integer skipCount, Integer maxItems) {
        if (pageNumber != null || pageSize != null) {
            return createPage(
                    pageNumber == null ? isZeroBasedPage() ? 0 : 1 : pageNumber,
                    pageSize == null ? -1 : pageSize);
        } else if (skipCount != null || maxItems != null) {
            return createWindow(
                    skipCount == null ? isZeroBasedRow() ? 0 : 1 : skipCount,
                    maxItems == null ? -1 : maxItems);
        }
    ...
    public Cursor createCursor(int totalRows, Page page) {
        if (page.getType() == PageType.PAGE) {
            return new PagedCursor(
                    zeroBasedRow, totalRows, page.zeroBasedIdx, page.startIdx, page.pageSize);

...
public class FeedControlTest extends BaseWebScriptTest {
  ...
  protected void createFeedControl(String siteId, String appToolId)
    throws Exception {
    ...
    Response response = sendRequest(
      new PostRequest(URL_CONTROL, feedControl.toString(), "application/json"),
      expectedStatus
    );
  ...
  protected void deleteFeedControl(String siteId, String appToolId)
    throws Exception {
    // Unset (delete) feed control
    int expectedStatus = 200;
    Response response = sendRequest(
      new DeleteRequest(
        URL_CONTROL + "?s=" + TEST_SITE_ID + "&a=" + TEST_APP_TOOL_ID
      ),
      expectedStatus
    );

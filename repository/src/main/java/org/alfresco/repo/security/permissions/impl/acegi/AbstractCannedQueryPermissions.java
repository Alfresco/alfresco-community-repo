...
public abstract class AbstractCannedQueryPermissions<R> extends AbstractCannedQuery<R> {
    ...
    protected AbstractCannedQueryPermissions(
            CannedQueryParameters parameters, MethodSecurityBean<R> methodSecurity) {
        super(parameters);
     ...
     * {@inheritDoc}
     *
     * <p>By default, the is a permission checking class. Override the method if you wish to switch
    ...
    protected List<R> applyPostQueryPermissions(List<R> results, int requestedCount) {
        ...
        if ((context == null) || (!(context instanceof AlfrescoSecureContext))) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Ignoring post-query permissions.  The secure context is empty: " + this);
            }
            return results;
        }
        ...
        List<R> resultsOut =
                (List<R>) methodSecurity.applyPermissions(results, authentication, requestedCount);
        // Done
        return resultsOut;
    }

    /**
     * Overrides the default implementation to check for the permission data that will allow a good
     * guess as to the maximum number of results in the event of a permission-based cut-off.
     */
    @Override

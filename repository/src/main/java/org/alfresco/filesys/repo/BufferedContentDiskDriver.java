 ...
 *
 * <p>Decorates ContentDiskDriver with a performance cache of some frequently used results. In
 * particular for getFileInformation and fileExists
 */
/*
...
public class BufferedContentDiskDriver
    ...
    private FileInfo getFileInformationInternal(SrvSession sess, TreeConnection tree, String path)
            throws IOException {
    ...
    public FileInfo getFileInformation(SrvSession sess, TreeConnection tree, String path)
        ...
        if (tctx.hasStateCache()) {
            ...
            if (fstate != null) {
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "state cache available - overwriting from state cache: isDirectory="
                                    + info.isDirectory());
                }
                ...
                if (!info.isDirectory()) {
                    ...
                    if (fstate.hasAccessDateTime()) {
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "replace access date "
                                            + new Date(finfo.getAccessDateTime())
                                            + " with "
                                            + new Date(fstate.getAccessDateTime()));
                        }
                    ...
                    if (fstate.hasChangeDateTime()) {
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "replace change date "
                                            + new Date(finfo.getChangeDateTime())
                                            + " with "
                                            + new Date(fstate.getChangeDateTime()));
                        }
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "replace modified date "
                                            + new Date(finfo.getModifyDateTime())
                                            + " with "
                                            + new Date(fstate.getModifyDateTime()));
                        }
                    ...
                    if (fstate.hasAllocationSize()) {
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "replace allocation size"
                                            + finfo.getAllocationSize()
                                            + " with "
                                            + fstate.getAllocationSize());
                        }
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Return getFileInformation, path: "
                                    + path
                                    + ", returning:"
                                    + finfo
                                    + ", readOnly:"
                                    ...
                                    + ", createdDate: "
                                    + new Date(finfo.getCreationDateTime())
                                    + ", accessDate:"
                                    + new Date(finfo.getAccessDateTime())
                                    + ", modifiedDate:"
                                    + new Date(finfo.getModifyDateTime())
                                    ...
                                    + new Date(finfo.getChangeDateTime())
                                    + ", fileAttributes: 0x"
                                    + Integer.toHexString(info.getFileAttributes())
                                    + ", mode: 0x"
                                    + Integer.toHexString(finfo.getMode()));
                }
    ...
    public DataBuffer processIOControl(
        ...
        return ioctlInterface.processIOControl(
                sess, tree, ctrlCode, fid, dataBuf, isFSCtrl, filter);
    }

...
public class Version2ServiceImpl
  ...
  @Override
  @Extend(
    extensionAPI = VersionServiceExtension.class,
    traitAPI = VersionServiceTrait.class
  )
  ...
  protected Version createVersion(
    ...
    String versionDescription = (String) versionProperties.get(
    ...
    if (versionHistoryRef == null) {
      ...
      // the current version
      Pair<Boolean, Version> result = getCurrentVersionImpl(
        versionHistoryRef,
        nodeRef
      );
      ...
      if (!headVersion) {
        ...
        VersionHistory versionHistory = buildVersionHistory(
          versionHistoryRef,
          nodeRef
        );
    ...
    String versionLabel = invokeCalculateVersionLabel(
      classRef,
      currentVersion,
      versionNumber,
      versionProperties
    ...
    long nodeDbId = (Long) this.nodeService.getProperty(
        nodeRef,
  ...
  protected NodeRef createNewVersion(
    ...
    try {
      ...
      childAssocRef =
        this.dbNodeService.createNode(
            ...
            QName.createQName(
              ...
              Version2Model.CHILD_VERSIONS + "-" + versionNumber
      ...
      freezeChildAssociations(
        versionNodeRef,
        nodeDetails.getChildAssociations()
      );
    ...
    if (
      dbNodeService.hasAspect(versionNodeRef, ContentModel.ASPECT_AUDITABLE) ==
      false
    ) {
    ...
    if (logger.isTraceEnabled()) {
      logger.trace(
        "newVersion created (" + versionNumber + ") " + versionNodeRef
      );
    }
  ...
  private void freezeAspects(
    ...
    for (QName aspect : aspects) {
      ...
      if (aspect.equals(ContentModel.ASPECT_AUDITABLE)) {
        ...
        for (Map.Entry<QName, Serializable> entry : nodeDetails
          .getProperties(aspect)
          .entrySet()) {
        ...
        dbNodeService.addAspect(
          versionNodeRef,
          aspect,
          nodeDetails.getProperties(aspect)
        );
      }
      ...
      freezeChildAssociations(
        versionNodeRef,
        nodeDetails.getChildAssociations(aspect)
      );
   ...
   * @since 3.3 (Ent)
   */
  private void freezeAssociations(
    NodeRef versionNodeRef,
    ...
    for (AssociationRef targetAssocRef : associations) {
      ...
      properties.put(
        Version2Model.PROP_QNAME_ASSOC_DBID,
        targetAssocRef.getId()
      );
  ...
  protected List<Version> getAllVersions(NodeRef versionHistoryRef) {
    List<ChildAssociationRef> versionAssocs = getVersionAssocs(
      versionHistoryRef,
      true
    );
  ...
  private List<ChildAssociationRef> getVersionAssocs(
    NodeRef versionHistoryRef,
    boolean preLoad
  ) {
   ...
   */
  @Override
  protected VersionHistory buildVersionHistory(
    NodeRef versionHistoryRef,
    NodeRef nodeRef
  ) {
    VersionHistory versionHistory = null;
    ...
    if (versionComparatorDesc != null) {
      Collections.sort(
        versions,
        Collections.reverseOrder(versionComparatorDesc)
      );
    }
  ...
  protected Version getVersion(NodeRef versionRef) {
    ...
    Map<QName, Serializable> nodeProperties =
      this.dbNodeService.getProperties(versionRef);
    ...
    for (QName key : nodeProperties.keySet()) {
      ...
      if (idx == 0) {
        ...
        versionProperties.put(
          keyName.substring(Version2Model.PROP_METADATA_PREFIX.length()),
          value
        );
      } else {
        if (key.equals(Version2Model.PROP_QNAME_VERSION_DESCRIPTION)) {
          ...
          if (
            keyName.equals(Version.PROP_DESCRIPTION) ||
    ...
    NodeRef newNodeRef = new NodeRef(
  ...
  private Pair<Boolean, Version> getCurrentVersionImpl(
    ...
    if (
      ...
      throw new IllegalArgumentException(
        "The node reference " +
        nodeRef +
        " is pointing to a version node, when a reference to a live node is expected."
      );
    }

    Pair<Boolean, Version> result = null;

    String versionLabel = (String) this.nodeService.getProperty(
        nodeRef,
        ContentModel.PROP_VERSION_LABEL
      );
    ...
    for (int i = versionCount; i > 0; i--) {
      ...
      String tempLabel = (String) this.dbNodeService.getProperty(
      ...
      if (tempLabel != null && tempLabel.equals(versionLabel) == true) {
        ...
        result =
          new Pair<Boolean, Version>(
            headVersion,
            getVersion(versionAssoc.getChildRef())
          );
        break;
      }
    }
   ...
   * @param nodeRef a node reference
   */
  private void checkForCorruptedVersions(
    NodeRef versionHistory,
    NodeRef nodeRef
  ) {
    ...
    if (versionLabel != null && versionLabel.equals("0")) {
      ...
      Collections.sort(
        ...
        new Comparator<Version>() {
          public int compare(Version v1, Version v2) {
            ...
            if (result == 0) {
              Long dbid1 = (Long) nodeService.getProperty(
                v1.getFrozenStateNodeRef(),
                ContentModel.PROP_NODE_DBID
              );
              Long dbid2 = (Long) nodeService.getProperty(
      ...
      for (Version version : versions) {
        ...
        NodeRef versionNodeRef = new NodeRef(
  ...
  public void revert(NodeRef nodeRef, Version version, boolean deep) {
    ...
    try {
      ...
      Map<QName, Serializable> currentVersionProp =
        this.nodeService.getProperties(nodeRef);
      forumProps = new HashMap<QName, Serializable>();
      for (QName key : currentVersionProp.keySet()) {
        if (
          key.getNamespaceURI().equals(NamespaceService.FORUMS_MODEL_1_0_URI)
        ) {
      ...
      for (QName aspect : oldAspectQNames) {
        ...
        if (aspectDef != null) {
          if (
            getRevertAspectAction(aspect, revertDetails) ==
            RevertAspectAction.IGNORE
          ) {
            propsToLeaveAlone.addAll(aspectDef.getProperties().keySet());
          }
          for (QName assocName : aspectDef.getAssociations().keySet()) {
            if (
              getRevertAssocAction(aspect, assocName, revertDetails) ==
              RevertAssocAction.IGNORE
            ) {
      ...
      List<ChildAssociationRef> children = new ArrayList<ChildAssociationRef>(
        this.nodeService.getChildAssocs(nodeRef)
      );
      ...
      for (ChildAssociationRef versionedChild : versionedChildren) {
        if (children.contains(versionedChild) == false) {
          ...
          if (this.nodeService.exists(versionedChild.getChildRef()) == true) {
            ...
            // be a primary child
            String childRefName = (String) this.nodeService.getProperty(
            ...
            if (childRef == null) {
              List<ChildAssociationRef> allAssocs = nodeService.getParentAssocs(
                versionedChild.getChildRef(),
                versionedChild.getTypeQName(),
                RegexQNamePattern.MATCH_ALL
              );
            ...
            if (versionedChild.isPrimary() == true) {
              ...
              if (
                deep == true &&
                getVersionHistoryNodeRef(versionedChild.getChildRef()) != null
              ) {
      ...
      for (AssociationRef assocRef : this.nodeService.getTargetAssocs(
          nodeRef,
          RegexQNamePattern.MATCH_ALL
        )) {
      ...
      for (AssociationRef versionedAssoc : this.nodeService.getTargetAssocs(
          versionNodeRef,
          RegexQNamePattern.MATCH_ALL
        )) {
  ...
  public NodeRef restore(
    ...
    QName type = (QName) dbNodeService.getType(
      VersionUtil.convertNodeRef(version.getFrozenStateNodeRef())
    );
  ...
  public boolean isAVersion(NodeRef nodeRef) {
    ...
    if (
      nodeRef
        .getStoreRef()
        .getProtocol()
        .equals(VersionBaseModel.STORE_PROTOCOL)
    ) {
      realNodeRef = VersionUtil.convertNodeRef(nodeRef);
    }
    return this.dbNodeService.hasAspect(
        realNodeRef,
        Version2Model.ASPECT_VERSION
      );
  }
  ...
  @Override
  public <T extends Trait> ExtendedTrait<T> getTrait(
    Class<? extends T> traitAPI
  ) {

...
/**
 * This abstract class forms a basis for all rendering engines that are built around the Template
 * Service.<br>
 * A template must be specified either as a {@link String} using the PARAM_TEMPLATE parameter, as a
 * {@link NodeRef} using the PARAM_TEMPLATE_NODE parameter or as a file path location using the
 * PARAM_TEMPLATE_PATH parameter. The RenderingEngine reads from these parameters with the following
 * priority: PARAM_TEMPLATE > PARAM_TEMPLATE_NODE > PARAM_TEMPLATE_PATH.
 *
...
public abstract class BaseTemplateRenderingEngine extends AbstractRenderingEngine {
    ...
    /**
     * This optional {@link Map}<{@link String}, {@link Serializable}> parameter can be used to pass
     * additional arguments to the templating engine when processing a template.
     */
    public static final String PARAM_MODEL = "model";

    /**
     * This optional {@link String} parameter specifies the template in a simple {@link String}
     * format.<br>
     * If this parameter is set the Rendering Engine will use it in preference to templates
     * specified by either the PARAM_TEMPLATE_NODE or the PARAM_TEMPLATE_PATH parameters.
     */
    public static final String PARAM_TEMPLATE = "template_string";

    /**
     * This optional {@link NodeRef} parameter specifies a node containing the template to be
     * processed.<br>
     * If a value is specified for PARAM_TEMPLATE then this parameter will be ignored.<br>
     * If a value is specified for this parameter it will be used in preference to values specified
     * for the PARAM_TEMPLATE_PATH parameter.
     */
    public static final String PARAM_TEMPLATE_NODE = "template_node";

    /**
     * This optional {@link String} parameter specifies a file path location for the template to be
     * processed.<br>
            ...
            if (path != null && path.length() > 0) {
                ...
                if (refs.size() != 1) {
                    throw new RenditionServiceException(
                            "Could not find template node for path: " + path);
                }
                node = refs.get(0);
            }
        }
    ...
    /**
     * Create the model that will be passed to the template service for rendering with the
    ...
    /**
     * This method gets the parameter definition display label from the properties file. It looks
     * first for a property whose key has a fixed rendition service-specific prefix and if that gets
     * null, it then delegates to the standard bean name-based approach.
     *
    ...
    protected String getParamDisplayLabel(String paramName) {
        // First we try to get the message using a common prefix for all template-based rendering
        // engines.
        final String commonPropertiesPrefix = "baseTemplateRenderingAction";
        String message =
                I18NUtil.getMessage(commonPropertiesPrefix + "." + paramName + "." + DISPLAY_LABEL);
    ...
    protected Collection<ParameterDefinition> getParameterDefinitions() {
        ...
        ParameterDefinitionImpl modelParamDef =
                new ParameterDefinitionImpl(
                        PARAM_MODEL,
                        DataTypeDefinition.ANY,
                        false,
                        getParamDisplayLabel(PARAM_MODEL));
        ParameterDefinitionImpl templateParamDef =
                new ParameterDefinitionImpl( //
                        PARAM_TEMPLATE,
                        DataTypeDefinition.TEXT,
                        false,
                        getParamDisplayLabel(PARAM_TEMPLATE));
        ParameterDefinitionImpl templateNodeParamDef =
                new ParameterDefinitionImpl(
                        PARAM_TEMPLATE_NODE,
                        DataTypeDefinition.NODE_REF,
                        false,
                        getParamDisplayLabel(PARAM_TEMPLATE_NODE));
        ParameterDefinitionImpl templatePathParamDef =
                new ParameterDefinitionImpl(
                        PARAM_TEMPLATE_PATH,
                        DataTypeDefinition.TEXT,
                        false,

...
public class AuditApplication {
  ...
  public static final Pattern AUDIT_KEY_PATTERN = Pattern.compile(
    "[a-zA-Z0-9\\-\\_\\.]+"
  );
  public static final Pattern AUDIT_PATH_PATTERN = Pattern.compile(
    "(/[a-zA-Z0-9:\\-\\_\\.]+)+"
  );
  public static final Pattern AUDIT_INVALID_PATH_COMP_CHAR_PATTERN = Pattern.compile(
    "[^a-zA-Z0-9:\\-\\_\\.]"
  );
  ...
  /** Derived expaned map for fast lookup */
  private Map<String, Map<String, DataGenerator>> dataGenerators = new HashMap<String, Map<String, DataGenerator>>(
    11
  );
  ...
  public static String buildPath(String... pathComponents) {
    ...
    if (!AUDIT_PATH_PATTERN.matcher(path).matches()) {
      ...
      msg
        .append(
          "The audit path is invalid and must be matched by regular expression: "
        )
  ...
  public static String getRootKey(String path) {
    if (!path.startsWith(AUDIT_PATH_SEPARATOR)) {
      throw new AuditModelException(
        "The path must start with the path separator '" +
  ...
  public List<DataExtractorDefinition> getDataExtractors() {
    List<DataExtractorDefinition> extractors = Collections.unmodifiableList(
      dataExtractors
    );
  ...
  public Map<String, DataGenerator> getDataGenerators(Set<String> paths) {
    Map<String, DataGenerator> amalgamatedGenerators = new HashMap<String, DataGenerator>(
      13
    );
  ...
  private void buildAuditPaths(
    ...
    upperGeneratorsByPath =
      new HashMap<String, DataGenerator>(upperGeneratorsByPath);
    ...
    for (RecordValue element : auditPath.getRecordValue()) {
      ...
      if (extractor == null) {
        generateException(
          extractorPath,
          "No data extractor exists for name: " + extractorName
        );
      }
      ...
      DataExtractorDefinition extractorDef = new DataExtractorDefinition(
        dataTrigger,
        sourcePath,
        extractorPath,
        extractor
    ...
    for (GenerateValue element : auditPath.getGenerateValue()) {
      ...
      if (generator == null) {
        generateException(
          generatorPath,
          "No data generator exists for name: " + generatorName
        );
      }
  ...
  private void generateException(String path, String msg)

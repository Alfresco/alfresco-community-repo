...
public class CompiledModel implements ModelQuery {
  ...
  private static final String ERR_CYCLIC_REFERENCE =
    "d_dictionary.compiled_model.err.cyclic_ref";
  ...
  /*package*/CompiledModel(
    ...
    try {
      ...
      NamespacePrefixResolver localPrefixes = createLocalPrefixResolver(
        model,
        namespaceDAO
      );
      ...
      for (ConstraintDefinition def : constraints.values()) {
        ((M2ConstraintDefinition) def).resolveDependencies(
            query,
  ...
  private void constructDefinitions(
    ...
    modelDefinition =
      new M2ModelDefinition(model, localPrefixes, dictionaryDAO);

    // Construct Property Types
    for (M2DataType propType : model.getPropertyTypes()) {
      M2DataTypeDefinition def = new M2DataTypeDefinition(
        modelDefinition,
        propType,
        localPrefixes
      );
    ...
    for (M2Type type : model.getTypes()) {
      M2TypeDefinition def = new M2TypeDefinition(
        modelDefinition,
        type,
        localPrefixes,
        properties,
    ...
    for (M2Aspect aspect : model.getAspects()) {
      M2AspectDefinition def = new M2AspectDefinition(
        modelDefinition,
        aspect,
        localPrefixes,
        properties,
    ...
    for (M2Constraint constraint : model.getConstraints()) {
      M2ConstraintDefinition def = new M2ConstraintDefinition(
        modelDefinition,
        null,
        constraint,
        localPrefixes
  ...
  private NamespacePrefixResolver createLocalPrefixResolver(
    ...
    DynamicNamespacePrefixResolver prefixResolver = new DynamicNamespacePrefixResolver(
      null
    );
    for (M2Namespace imported : model.getImports()) {
      ...
      if (model.getNamespace(uri) != null) {
        throw new NamespaceException(
          "URI " +
          uri +
          " cannot be imported as it is already contained in the model's namespaces"
        );
      }
  ...
  private void resolveDependencies(
    ...
    NamespacePrefixResolver prefixResolver = createLocalPrefixResolver(
      model,
      namespaceDAO
    );
    ...
    for (ClassDefinition def : classes.values()) {
      ((M2ClassDefinition) def).resolveDependencies(
          query,
          prefixResolver,
          constraints
        );
  ...
  private void resolveInheritance(
    ...
    for (int depth = 0; depth < order.size(); depth++) {
      for (ClassDefinition def : order.get(depth)) {
        ((M2ClassDefinition) def).resolveInheritance(
            query,
            prefixResolver,
            modelConstraints
          );

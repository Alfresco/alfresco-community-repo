 ...
 *
 * @author Roy Wetheral
 *     <p>NOTE: deprecated since 3.1 (migrate and use Version2 Service)
 */
@SuppressWarnings("deprecation")
public abstract class VersionServiceImpl extends AbstractVersionServiceImpl
    ...
    protected static final String MSGID_ERR_RESTORE_NO_VERSION =
            "version_service.err_restore_no_version";
            ...
            try {
                versionComparatorDesc =
                        (Comparator<Version>)
                                getClass()
                                        .getClassLoader()
                                        .loadClass(versionComparatorClass.trim())
                                        .newInstance();
            } catch (Exception e) {
    ...
    /**
     * The version's are created from the children upwards with the parent being created first. This
     * will ensure that the child version references in the version node will point to the version
     * history nodes for the (possibly) newly created version histories.
     */
    @Extend(extensionAPI = VersionServiceExtension.class, traitAPI = VersionServiceTrait.class)
    public Collection<Version> createVersion(
            NodeRef nodeRef, Map<String, Serializable> versionProperties, boolean versionChildren)
            throws ReservedVersionNameException, AspectMissingException {
        if (logger.isDebugEnabled()) {
            logger.debug("Run as user " + AuthenticationUtil.getRunAsUser());
            logger.debug("Fully authenticated " + AuthenticationUtil.getFullyAuthenticatedUser());
        ...
        Collection<Version> versions =
                createVersion(nodeRef, versionProperties, versionChildren, versionNumber);

        if (logger.isDebugEnabled()) {
            Version[] versionsArray = versions.toArray(new Version[0]);
            Version version =
                    versionsArray[versionsArray.length - 1]; // last item is the new parent version
            logger.debug(
                    "created version ("
                            + VersionUtil.convertNodeRef(version.getFrozenStateNodeRef())
                            + ") in "
    ...
    /**
     * Helper method used to create the version when the versionChildren flag is provided. This
     * method ensures that all the children (if the falg is set to true) are created with the same
     * version number, this ensuring that the version stripe is correct.
     *
     * @param nodeRef the parent node reference
     * @param versionProperties the version properties
     * @param versionChildren indicates whether to version the children of the parent node
     * @param versionNumber the version number
     * @return a collection of the created versions
     * @throws ReservedVersionNameException thrown if there is a reserved version property name
     *     clash
    ...
    private Collection<Version> createVersion(
        ...
        if (versionChildren == true) {
            ...
            for (ChildAssociationRef childAssoc : children) {
                ...
                Collection<Version> childVersions =
                        createVersion(
    ...
    /**
     * Note: we can't control the order of the list, so if we have children and parents in the list
     * and the parents get versioned before the children and the children are not already versioned
     * then the parents child references will be pointing to the node ref, rather than the verison
     * history.
     */
    public Collection<Version> createVersion(
            Collection<NodeRef> nodeRefs, Map<String, Serializable> versionProperties)
            throws ReservedVersionNameException, AspectMissingException {
        if (logger.isDebugEnabled()) {
            logger.debug("Run as user " + AuthenticationUtil.getRunAsUser());
            logger.debug("Fully authenticated " + AuthenticationUtil.getFullyAuthenticatedUser());
     ...
     * @throws ReservedVersionNameException thrown if there is a name clash in the version
     *     properties
     */
    protected Version createVersion(
            NodeRef nodeRef, Map<String, Serializable> origVersionProperties, int versionNumber)
        ...
        if (versionHistoryRef == null) {
            ...
            // Need to check that we are not about to create branch since this is not currently
            // supported
        ...
        NodeRef newVersionRef =
                createNewVersion(
                        nodeRef,
                        versionHistoryRef,
                        getStandardVersionProperties(
                                versionProperties, nodeRef, currentVersionRef, versionNumber),
                        versionProperties,
                        nodeDetails);
        ...
        if (currentVersionRef == null) {
            ...
            this.dbNodeService.createAssociation(
                    versionHistoryRef, newVersionRef, VersionServiceImpl.ASSOC_ROOT_VERSION);
        } else {
            // Relate the new version to the current version as its successor
            this.dbNodeService.createAssociation(
                    currentVersionRef, newVersionRef, VersionServiceImpl.ASSOC_SUCCESSOR);
        }
        ...
        this.nodeService.setProperty(
                nodeRef, ContentModel.PROP_VERSION_LABEL, version.getVersionLabel());
     ...
     */
    private Map<QName, Serializable> getStandardVersionProperties(
            Map<String, Serializable> versionProperties,
            NodeRef nodeRef,
        ...
        result.put(
                VersionModel.PROP_QNAME_FROZEN_NODE_STORE_PROTOCOL,
                nodeRef.getStoreRef().getProtocol());

        // Set the versionable node store id
        result.put(
                VersionModel.PROP_QNAME_FROZEN_NODE_STORE_ID,
     ...
     * @param versionProperties the version properties
     */
    private void storeVersionMetaData(
            NodeRef versionNodeRef, Map<String, Serializable> versionProperties) {
    ...
    protected Map<String, Serializable> getVersionMetaData(NodeRef versionNodeRef) {
        ...
        List<ChildAssociationRef> metaData =
                this.dbNodeService.getChildAssocs(
                        versionNodeRef, RegexQNamePattern.MATCH_ALL, CHILD_QNAME_VERSION_META_DATA);

        Map<String, Serializable> versionProperties =
                new HashMap<String, Serializable>(metaData.size());
            ...
            String name =
                    (String)
                            this.dbNodeService.getProperty(
                                    metaDataValue, PROP_QNAME_META_DATA_NAME);
            Serializable value =
                    this.dbNodeService.getProperty(metaDataValue, PROP_QNAME_META_DATA_VALUE);
     ...
     */
    private void freezeAspects(
            PolicyScope nodeDetails, NodeRef versionNodeRef, Set<QName> aspects) {
     ...
     */
    private void freezeChildAssociations(
            NodeRef versionNodeRef, List<ChildAssociationRef> childAssociations) {
     ...
     *
     * <p>The node ref is passed to enable the version history to be scoped to the appropriate
     * branch in the version history.
     *
    ...
    protected VersionHistory buildVersionHistory(NodeRef versionHistoryRef, NodeRef nodeRef) {
        ...
        if (this.nodeService.exists(nodeRef)) {
            ...
            currentVersion =
                    VersionUtil.convertNodeRef(getLatestVersion(nodeRef).getFrozenStateNodeRef());
        }
        ...
        while (currentVersion != null) {
            ...
            List<AssociationRef> preceedingVersions =
                    this.dbNodeService.getSourceAssocs(
    ...
    protected Version getVersion(NodeRef versionRef) {
        ...
        NodeRef newNodeRef =
                new NodeRef(new StoreRef(STORE_PROTOCOL, STORE_ID), versionRef.getId());
    ...
    protected NodeRef getVersionHistoryNodeRef(NodeRef nodeRef) {
        if (nodeService.exists(nodeRef)) {
            return this.dbNodeService.getChildByName(
                    getRootNode(),
                    CHILD_QNAME_VERSION_HISTORIES,
                    (String) nodeService.getProperty(nodeRef, ContentModel.PROP_NODE_UUID));
        } else {
            return this.dbNodeService.getChildByName(
                    getRootNode(), CHILD_QNAME_VERSION_HISTORIES, nodeRef.getId());
        }
    }
     ...
     *
     * <p>This uses the version label as a mechanism for looking up the version node in the version
     * history.
     *
     * @param nodeRef a node reference
    ...
    private NodeRef getCurrentVersionNodeRef(NodeRef versionHistory, NodeRef nodeRef) {
        NodeRef result = null;
        String versionLabel =
                (String) this.nodeService.getProperty(nodeRef, ContentModel.PROP_VERSION_LABEL);

        Collection<ChildAssociationRef> versions =
                this.dbNodeService.getChildAssocs(versionHistory);
        for (ChildAssociationRef version : versions) {
            String tempLabel =
                    (String)
                            this.dbNodeService.getProperty(
                                    version.getChildRef(), VersionModel.PROP_QNAME_VERSION_LABEL);
    ...
    public void ensureVersioningEnabled(
            NodeRef nodeRef, Map<QName, Serializable> versionProperties) {
        if (logger.isDebugEnabled()) {
            logger.debug("Run as user " + AuthenticationUtil.getRunAsUser());
            logger.debug("Fully authenticated " + AuthenticationUtil.getFullyAuthenticatedUser());
        }
        ...
        // Do we need to create the initial version history entry? By convention this is always a
        // major version.
        if (getVersionHistoryNodeRef(nodeRef) == null) {
            createVersion(
                    nodeRef,
                    Collections.<String, Serializable>singletonMap(
                            VersionModel.PROP_VERSION_TYPE, VersionType.MAJOR));
        }
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.version.VersionService#revert(org.alfresco.service.cmr.repository.NodeRef,
     *     boolean)
     */
     ...
     * @see
     *     org.alfresco.service.cmr.version.VersionService#revert(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.cmr.version.Version, boolean)
     */
    public void revert(NodeRef nodeRef, Version version, boolean deep) {
        ...
        try {
            ...
            // Re-set the version label property (since it should not be modified from the
            // origional)
            this.nodeService.setProperty(
                    nodeRef, ContentModel.PROP_VERSION_LABEL, currentVersionLabel);

            // Add/remove the child nodes
            List<ChildAssociationRef> children =
                    new ArrayList<ChildAssociationRef>(this.nodeService.getChildAssocs(nodeRef));
            for (ChildAssociationRef versionedChild :
                    this.nodeService.getChildAssocs(versionNodeRef)) {
                if (children.contains(versionedChild) == false) {
                    if (this.nodeService.exists(versionedChild.getChildRef()) == true) {
                        // The node was a primary child of the parent, but that is no longer the
                        // case.  Dispite this
                        // the node still exits so this means it has been moved.
                        // The best thing to do in this situation will be to re-add the node as a
                        // child, but it will not
                        // be a primary child.
                        ...
                        if (versionedChild.isPrimary() == true) {
                            ...
                            if (deep == true
                                    && getVersionHistoryNodeRef(versionedChild.getChildRef())
                                            != null) {
                                // We're going to try and restore the missing child node and
                                // recreate the assoc
                                restore(
                                        versionedChild.getChildRef(),
                                        nodeRef,
                            ...
                            // else the deleted child did not have a version history so we can't
                            // restore the child
                            // and so we can't revert the association
                        }

                        // else
                        // Since this was never a primary assoc and the child has been deleted we
                        // won't recreate
                        // the missing node as it was never owned by the node and we wouldn't know
                        // where to put it.
                    }
                } else {
            ...
            for (AssociationRef assocRef :
                    this.nodeService.getTargetAssocs(nodeRef, RegexQNamePattern.MATCH_ALL)) {
                this.nodeService.removeAssociation(
                        assocRef.getSourceRef(), assocRef.getTargetRef(), assocRef.getTypeQName());
            }
            for (AssociationRef versionedAssoc :
                    this.nodeService.getTargetAssocs(versionNodeRef, RegexQNamePattern.MATCH_ALL)) {
                if (this.nodeService.exists(versionedAssoc.getTargetRef()) == true) {
                    this.nodeService.createAssociation(
                            nodeRef, versionedAssoc.getTargetRef(), versionedAssoc.getTypeQName());
                }
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.version.VersionService#restore(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.cmr.repository.NodeRef, org.alfresco.service.namespace.QName,
     *     org.alfresco.service.namespace.QName)
     */
    public NodeRef restore(
            NodeRef nodeRef, NodeRef parentNodeRef, QName assocTypeQName, QName assocQName) {
     ...
     *     org.alfresco.service.cmr.version.VersionService#restore(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.cmr.repository.NodeRef, org.alfresco.service.namespace.QName,
     *     org.alfresco.service.namespace.QName, boolean)
     */
    public NodeRef restore(
            NodeRef nodeRef,
        ...
        if (this.nodeService.exists(nodeRef) == true) {
            ...
            throw new VersionServiceException(
                    MSGID_ERR_RESTORE_EXISTS, new Object[] {nodeRef.toString()});
        }
        ...
        if (version == null) {
            ...
            throw new VersionServiceException(
                    MSGID_ERR_RESTORE_NO_VERSION, new Object[] {nodeRef.toString()});
        }
        ...
        props.put(
                ...
                version.getVersionProperty(VersionModel.PROP_FROZEN_NODE_ID));
        ...
        try {
            ...
            restoredNodeRef =
                    this.nodeService
                            .createNode(parentNodeRef, assocTypeQName, assocQName, type, props)
                            .getChildRef();
        } finally {
    ...
    private Version getHeadVersion(NodeRef nodeRef) {
        ...
        if (versionHistoryNodeRef != null) {
            List<ChildAssociationRef> versionsAssoc =
                    this.dbNodeService.getChildAssocs(
                            versionHistoryNodeRef,
                            RegexQNamePattern.MATCH_ALL,
                            VersionModel.CHILD_QNAME_VERSIONS);
            for (ChildAssociationRef versionAssoc : versionsAssoc) {
                NodeRef versionNodeRef = versionAssoc.getChildRef();
                List<AssociationRef> successors =
                        this.dbNodeService.getTargetAssocs(
                                versionNodeRef, VersionModel.ASSOC_SUCCESSOR);
    ...
    private Version getLatestVersion(NodeRef nodeRef) {
        ...
        if (versionHistoryNodeRef != null) {
            ...
            for (ChildAssociationRef versionAssoc : versionsAssoc) {
                NodeRef versionNodeRef = versionAssoc.getChildRef();
                List<AssociationRef> predecessors =
                        this.dbNodeService.getSourceAssocs(
                                versionNodeRef, VersionModel.ASSOC_SUCCESSOR);
    ...
    public void deleteVersionHistory(NodeRef nodeRef) throws AspectMissingException {
        ...
        if (versionHistoryNodeRef != null) {
            ...
            if (this.nodeService.exists(nodeRef) == true
                    && this.nodeService.hasAspect(nodeRef, ContentModel.ASPECT_VERSIONABLE)
                            == true) {
    ...
    public void deleteVersion(NodeRef nodeRef, Version version) {
        ...
        throw new UnsupportedOperationException(
                "Delete version is unsupported by the old (deprecated) version store"
                        + " implementation");
    }
    ...
    protected void defaultOnCreateVersion(
        ...
        if (classDefinition != null) {
            ...
            List<AssociationRef> nodeAssocRefs =
                    this.nodeService.getTargetAssocs(nodeRef, RegexQNamePattern.MATCH_ALL);

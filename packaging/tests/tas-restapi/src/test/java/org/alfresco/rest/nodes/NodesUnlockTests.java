...
public class NodesUnlockTests extends RestTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify Collaborator canot unlock EPHEMERAL lock made by different user, but"
                            + " can unlock EPHEMERAL lock made by same user")
    public void lockEphemeralAndUnlock() throws Exception {
        STEP(
                "1. Add user(s) as collaborators to the site created by administrator and add a"
                        + " file in this site.");
        file1 =
                dataContent
                        .usingUser(adminUser)
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Verify Collaborator canot unlock PERSISTENT lock made by different user, but"
                            + " can unlock PERSISTENT lock made by same user")

...
public class AlfrescoImapServer extends AbstractLifecycleBean {
    private class SecureImapServer extends ImapServer {
        ...
        /**
         * Use Java's default SSL Server SocketFactory controlled via System Properties
                    ...
                    ret =
                            (SSLServerSocket)
                                    SSLServerSocketFactory.getDefault()
                                            .createServerSocket(setup.getPort(), 0, bindTo);
                } catch (IOException e) {
                    if (logger.isErrorEnabled()) {
                        logger.error(
                                "Unable to open socket bindTo:"
                                        + bindTo
                                        + "port "
                                        + setup.getPort(),
                                e);
        ...
        protected synchronized ServerSocket openServerSocket() throws IOException {
            ...
            if (setup.isSecure()) {
                try {
                    ret =
                            (SSLServerSocket)
                                    DummySSLServerSocketFactory.getDefault()
                                            .createServerSocket(setup.getPort(), 0, bindTo);
                } catch (IOException e) {
                    if (logger.isErrorEnabled()) {
                        logger.error(
    ...
    public void startup() {
        if (serverImpl == null) {
            final Managers imapManagers =
                    new Managers() {
                        // We create a new Host Manager instance per session to allow for session
                        // state tracking
            ...
            if (isImapEnabled()) {
                ...
                serverImpl =
                        new DefaultImapServer(
                                new ServerSetup(port, host, ServerSetup.PROTOCOL_IMAP),
                                imapManagers,
                                serverOpeningExceptionRef);
            ...
            if (isImapsEnabled()) {
                AtomicReference<Exception> serverOpeningExceptionRef =
                        new AtomicReference<Exception>();
                secureServerImpl =
                        new SecureImapServer(
                                new ServerSetup(securePort, host, ServerSetup.PROTOCOL_IMAPS),
                                imapManagers,
                                serverOpeningExceptionRef);
                ...
                if (logger.isInfoEnabled()) {
                    logger.info(
                            "IMAPS service started on host:port " + host + ":" + this.securePort);
                }
            }

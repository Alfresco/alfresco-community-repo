 ...
 * @author gkspencer
 */
public abstract class AbstractServerConfigurationBean extends ServerConfiguration
    ...
    public void init() {
        ...
        // We need to check for a WINS server configuration in the CIFS server config section to
        // initialize
        // the NetBIOS name lookups to use WINS rather broadcast lookups, which may be used to get
        // the local
        // domain
        ...
        try {
            ...
            throw new AlfrescoRuntimeException(
                    "File server configuration error, " + ex.getMessage(), ex);
        }
    ...
    private final void determinePlatformType() {
        if (m_platform == Platform.Type.Unchecked) m_platform = Platform.isPlatformType();
    }
    ...
    protected final EnumSet<Platform.Type> parsePlatformString(String platformStr) {
        ...
        try {
            while (token.hasMoreTokens()) {
                ...
                else throw new AlfrescoRuntimeException("Invalid platform type, " + typ);
            }
            ...
            try {
                ...
                nbName =
                        NetBIOSSession.FindName(
                                NetBIOSName.BrowseMasterName, NetBIOSName.BrowseMasterGroup, 5000);
                ...
                NetBIOSNameList nbNameList =
                        NetBIOSSession.FindNamesForAddress(nbName.getIPAddressString(0));
                if (nbNameList != null) {

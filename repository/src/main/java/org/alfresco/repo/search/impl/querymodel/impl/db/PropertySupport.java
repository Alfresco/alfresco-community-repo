...
public class PropertySupport implements DBQueryBuilderComponent {
  ...
  private DBQueryBuilderJoinCommandType joinCommandType =
    DBQueryBuilderJoinCommandType.PROPERTY;
   ...
   */
  public void setJoinCommandType(
      ...
      switch (joinCommandType) {
        ...
        case PROPERTY:
          ...
          if (propertyDataType != null) {
            if (
              propertyDataType.getName().equals(DataTypeDefinition.ASSOC_REF)
            ) {
              command.setValue(value);
              command.setValues(values);
            } else if (
              propertyDataType.getName().equals(DataTypeDefinition.CATEGORY)
            ) {
              command.setValue(value);
              command.setValues(values);
            } else if (
              propertyDataType.getName().equals(DataTypeDefinition.DATE)
            ) {
              command.setValue(value);
              command.setValues(values);
            } else if (
              propertyDataType.getName().equals(DataTypeDefinition.DATETIME)
            ) {
              command.setValue(value);
              command.setValues(values);
            } else if (
              propertyDataType.getName().equals(DataTypeDefinition.LOCALE)
            ) {
              command.setValue(value);
              command.setValues(values);
            } else if (
              propertyDataType.getName().equals(DataTypeDefinition.MLTEXT)
            ) {
              command.setValue(value);
              command.setValues(values);
            } else if (
              propertyDataType.getName().equals(DataTypeDefinition.NODE_REF)
            ) {
              command.setValue(value);
              command.setValues(values);
            } else if (
              propertyDataType.getName().equals(DataTypeDefinition.PERIOD)
            ) {
              command.setValue(value);
              command.setValues(values);
            } else if (
              propertyDataType.getName().equals(DataTypeDefinition.QNAME)
            ) {
              command.setValue(value);
              command.setValues(values);
            } else {
              command.setValue(
                DefaultTypeConverter.INSTANCE.convert(propertyDataType, value)
              );
              Collection<?> collection = DefaultTypeConverter.INSTANCE.convert(
                propertyDataType,
                values
              );
              command.setValues(
                collection == null ? null : collection.toArray()
              );
            }
          } else {
            command.setValue(value);
        ...
        case CONTENT_URL:
          ...
          if (commandType == DBQueryBuilderPredicatePartCommandType.ORDER) {
            ...
            command.setValue(
              DefaultTypeConverter.INSTANCE.convert(Long.class, value)
            );
            command.setValues(
              values == null
                ? null
                : DefaultTypeConverter.INSTANCE
                  .convert(Integer.class, Arrays.asList(values))
                  .toArray(new Integer[] {})
            );
          }
          break;

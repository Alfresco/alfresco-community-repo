...
public class InviteModeratedSenderTest extends TestCase {
    ...
    /**
     * Test that the mail action is correctly constructed when sending notifications emails about
     * users requesting access to a specific site
     *
     * @throws Exception
     */
    public void testSendModeratedEmail() throws Exception {
        ...
        verify(mailAction)
                .setParameterValue(eq(MailActionExecuter.PARAM_TO_MANY), eq(SiteManagerGroup));
        verify(mailAction)
                .setParameterValue(
        ...
        verify(mailAction)
                .setParameterValue(eq(MailActionExecuter.PARAM_TEMPLATE), eq(emailTemplateNodeRef));

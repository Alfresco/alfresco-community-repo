...
public abstract class BaseAuthenticationFilter {
    ...
    /**
     * The default session attribute used to cache the user. Subclasses may override this with
     * {@link #setUserAttributeName(String)}.
     */
     ...
     *
     * @param authenticationService the authService to set
     */
     ...
     *
     * @param remoteUserMapper the remote user mapper
     */
     ...
     * @return SessionUser
     */
    protected SessionUser createUserObject(
            String userName, String ticket, NodeRef personNode, NodeRef homeSpaceRef) {
    ...
    protected SessionUser getSessionUser(
        ...
        if (remoteUserMapper != null
            ...
            if (getLogger().isTraceEnabled()) {
                getLogger()
                        .trace("Found a remote user: " + AuthenticationUtil.maskUsername(userId));
            }
        }
        ...
        if (sessionUser != null) {
            try {
                ...
                if (getLogger().isDebugEnabled()) {
                    getLogger()
                            .debug(
                                    "The ticket may have expired or the person could have been"
                                            + " removed, invalidating session.",
                                    e);
                }
        ...
        if (userId != null) {
            if (getLogger().isDebugEnabled()) {
                getLogger()
                        .debug(
                                "We have a previously-cached user with the wrong identity - replace"
                                        + " them.");
            }
            ...
            if (sessionUser == null) {
                // If we have been authenticated by other means, just propagate through the user
                // identity
                if (getLogger().isDebugEnabled()) {
                    getLogger()
                            .debug(
                                    "Propagating through the user identity: "
     ...
     */
    protected <T> T doInSystemTransaction(
            final RetryingTransactionHelper.RetryingTransactionCallback<T> callback) {
        return AuthenticationUtil.runAs(
                new AuthenticationUtil.RunAsWork<T>() {
                    public T doWork() throws Exception {
                        return transactionService
                                .getRetryingTransactionHelper()
                                .doInTransaction(callback, transactionService.isReadOnly());
                    }
                },
     ...
     *
     * @param userAttr the user object session attribute name
     */
     ...
     */
    protected SessionUser createUserEnvironment(
            HttpSession session, final String userName, final String ticket, boolean externalAuth)
            throws IOException, ServletException {
        if (getLogger().isTraceEnabled()) {
            getLogger()
                    .trace(
                            "Create the User environment for: "
     ...
     */
    protected SessionUser createUserEnvironment(final HttpSession session, final String userName)
            throws IOException, ServletException {
    ...
    protected boolean handleLoginForm(HttpServletRequest req, HttpServletResponse res)
        ...
        try {
            ...
            createUserEnvironment(
                    session, username, authenticationService.getCurrentTicket(), false);

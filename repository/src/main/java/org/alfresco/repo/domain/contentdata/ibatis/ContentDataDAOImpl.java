...
public class ContentDataDAOImpl extends AbstractContentDataDAOImpl {
    ...
    protected ContentUrlEntity createContentUrlEntity(
            String contentUrl, long size, ContentUrlKeyEntity contentUrlKeyEntity) {
        ContentUrlEntity contentUrlEntity = new ContentUrlEntity();
        contentUrlEntity.setContentUrl(contentUrl);
    ...
    public void getContentUrlsOrphaned(
        ...
        List<ContentUrlEntity> results =
                template.selectList(
                        SELECT_CONTENT_URLS_ORPHANED, query, new RowBounds(0, maxResults));
        // Pass the result to the callback
        for (ContentUrlEntity result : results) {
            contentUrlHandler.handle(
                    result.getId(), result.getContentUrl(), result.getOrphanTime());
        }
    }

    @Override
    public void getContentUrlsKeepOrphaned(
            final ContentUrlHandler contentUrlHandler, final int maxResults) {
        List<ContentUrlEntity> results =
                template.selectList(
                        SELECT_CONTENT_URLS_KEEP_ORPHANED, new RowBounds(0, maxResults));
        // Pass the result to the callback
        for (ContentUrlEntity result : results) {
            contentUrlHandler.handle(
                    result.getId(), result.getContentUrl(), result.getOrphanTime());
    ...
    protected ContentUrlEntity getContentUrlEntityUnreferenced(String contentUrl) {
        ...
        contentUrlEntity =
                (ContentUrlEntity)
                        template.selectOne(
                                SELECT_CONTENT_URL_BY_KEY_UNREFERENCED, contentUrlEntity);
        // Done
        return contentUrlEntity;
    }
    ...
    protected ContentDataEntity createContentDataEntity(
        ...
        try {
            ...
            throw new AlfrescoRuntimeException(
                    "Failed to insert ContentData: " + contentDataEntity, e);
        }
        // Done
    ...
    protected ContentDataEntity getContentDataEntity(Long id) {
        ...
        ContentDataEntity contentDataEntity =
                (ContentDataEntity) template.selectOne(SELECT_CONTENT_DATA_BY_ID, params);
        // Done
        return contentDataEntity;
    }
    ...
    public List<ContentUrlKeyEntity> getSymmetricKeysByMasterKeyAlias(
            String masterKeyAlias, long fromId, int maxResults) {
        ContentUrlKeyEntity entity = new ContentUrlKeyEntity();
        entity.setMasterKeyAlias(masterKeyAlias);
        entity.setId(fromId);
        List<ContentUrlKeyEntity> results =
                template.selectList(
                        SELECT_SYMMETRIC_KEYS_BY_MASTER_KEY, entity, new RowBounds(0, maxResults));
        return results;
    }

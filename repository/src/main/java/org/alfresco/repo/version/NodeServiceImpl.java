...
public class NodeServiceImpl implements NodeService, VersionModel {
  ...
  private static final String MSG_UNSUPPORTED_V1 =
    "Versioning V1 is not implemented or supported. Patches exist to upgrade your data to use Versioning V2. Please contact support.";

  /**
   * The name of the spoofed root association
   */
  private static final QName rootAssocName = QName.createQName(
   ...
   */
  @Override
  public boolean removeSecondaryChildAssociation(
    ChildAssociationRef childAssocRef
  ) {
    throw new UnsupportedOperationException(MSG_UNSUPPORTED);
  }
  ...
  public QName getType(NodeRef nodeRef) throws InvalidNodeRefException {
    return (QName) this.dbNodeService.getProperty(
        VersionUtil.convertNodeRef(nodeRef),
        PROP_QNAME_FROZEN_NODE_TYPE
      );
  }
  ...
  public Set<QName> getAspects(NodeRef nodeRef) throws InvalidNodeRefException {
    return new HashSet<QName>(
      (ArrayList<QName>) this.dbNodeService.getProperty(
          VersionUtil.convertNodeRef(nodeRef),
          PROP_QNAME_FROZEN_ASPECTS
        )
    );
  }
  ...
  public Map<QName, Serializable> getProperties(NodeRef nodeRef)
    ...
    for (ChildAssociationRef child : children) {
      ...
      QName qName = (QName) this.dbNodeService.getProperty(
          versionedAttribute,
          PROP_QNAME_QNAME
        );
      PropertyDefinition propDef = this.dicitionaryService.getProperty(qName);

      Boolean isMultiValue = (Boolean) this.dbNodeService.getProperty(
          versionedAttribute,
          PROP_QNAME_IS_MULTI_VALUE
        );
      if (isMultiValue.booleanValue() == false) {
        value =
          this.dbNodeService.getProperty(versionedAttribute, PROP_QNAME_VALUE);

        if (propDef != null) {
          DataTypeDefinition dataTypeDef = propDef.getDataType();
          if (dataTypeDef != null) {
            if (
              dataTypeDef.getName().equals(DataTypeDefinition.CONTENT) &&
  ...
  public List<ChildAssociationRef> getChildAssocs(
    ...
    for (ChildAssociationRef childAssocRef : childAssocRefs) {
      ...
      NodeRef referencedNode = (NodeRef) this.dbNodeService.getProperty(
          childRef,
      ...
      if (this.dbNodeService.exists(referencedNode) == true) {
        ...
        if (qnamePattern.isMatch(qName) == true) {
          ...
          QName assocType = (QName) this.dbNodeService.getProperty(
              childRef,
              PROP_QNAME_ASSOC_TYPE_QNAME
            );
          boolean isPrimary =
            (
              (Boolean) this.dbNodeService.getProperty(
                  childRef,
          ...
          int nthSibling =
            (
              (Integer) this.dbNodeService.getProperty(
                  childRef,
  ...
  public List<ChildAssociationRef> getChildAssocs(
    ...
    List<ChildAssociationRef> result = getChildAssocs(
      nodeRef,
      typeQName,
      qname
    );
  ...
  @Override
  public int countChildAssocs(NodeRef nodeRef, boolean isPrimary)
    throws InvalidNodeRefException {
    // This operation is not supported for a version store
    throw new UnsupportedOperationException(MSG_UNSUPPORTED);
  }

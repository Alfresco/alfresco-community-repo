...
public class DbObjectXMLTransformer {
    ...
    private void attemptOutput(DbObject dbObject) throws SAXException {
        ...
        if (dbObject instanceof Schema) {
            ...
            attribs.addAttribute(
                    "", "", "xmlns", "CDATA", "http://www.alfresco.org/repo/db-schema");
            attribs.addAttribute(
    ...
    private void addAttributes(DbObject dbObject, AttributesImpl attribs) {
        if (dbObject instanceof Schema) {
            ...
            attribs.addAttribute(
                    "", "", XML.ATTR_VERSION, "CDATA", Integer.toString(schema.getVersion()));
            attribs.addAttribute(
                    "",
                    "",
                    XML.ATTR_TABLE_COLUMN_ORDER,
                    "CDATA",
                    Boolean.toString(schema.isCheckTableColumnOrder()));
        } else if (dbObject instanceof Index) {
            Index index = (Index) dbObject;
            attribs.addAttribute(
                    "", "", XML.ATTR_UNIQUE, "CDATA", Boolean.toString(index.isUnique()));
        } else if (dbObject instanceof Column) {
            Column column = (Column) dbObject;
            attribs.addAttribute(
                    "", "", XML.ATTR_ORDER, "CDATA", Integer.toString(column.getOrder()));
        }
    }
     ...
     *
     * <p>The columnOrders parameter will provide a corresponding list of integers that will be
     * provided in each columnname element's order attribute. This parameter may be null in which
     * case order attributes will be ommitted.
     *
     * @param columnNames List<String>
     * @param columnOrders List<Integer>
     * @throws SAXException
     */
    private void columnNameList(List<String> columnNames, List<Integer> columnOrders)
            throws SAXException {
        ...
        for (int i = 0; i < columnNames.size(); i++) {
            ...
            if (columnOrders != null) {
                ...
                attribs.addAttribute(
                        "", "", XML.ATTR_ORDER, "CDATA", Integer.toString(columnOrder));
            }

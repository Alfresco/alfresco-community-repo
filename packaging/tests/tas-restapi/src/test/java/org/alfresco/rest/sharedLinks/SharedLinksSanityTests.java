...
/**
 * Class includes Sanity tests for the shared-links api. Detailed tests would be covered in the
 * alfresco-remote-api test project
 *
 * @author meenal bhave
 */
public class SharedLinksSanityTests extends RestTest {
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Sanity tests for GET {sharedId}/content and POST {sharedId}/email endpoints")
    @Test(groups = {TestGroup.REST_API, TestGroup.SHAREDLINKS, TestGroup.SANITY})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Sanity tests for GET /renditions, GET /renditions/{renditionId} and GET"
                            + " /renditions/{renditionId}/content endpoints")
    @Test(
            groups = {
                TestGroup.REST_API,
                TestGroup.SHAREDLINKS,
    ...
    public void testGetSharedLinkRendition() throws Exception {
        ...
        // GET /renditions/{renditionId}: get specific rendition information for the file with
        // shared link
        nodeRenditionInfo =
                restClient
                        .authenticateUser(testUser1)
        ...
        restClient.assertStatusCodeIs(HttpStatus.OK);
        nodeRenditionInfo.assertThat().field("id").is("pdf").and().field("status").is("CREATED");

        // GET /renditions/{renditionId}/content: get the rendition content for file with shared
        // link
        restClient
                .authenticateUser(testUser1)
                .withCoreAPI()
        ...
        Assert.assertTrue(
                restClient.onResponse().getResponse().body().asInputStream().available() > 0);
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Sanity tests for Range reuest header on   GET"
                            + " shared-links/{sharedId}/renditions/{renditionId}/content endpoints")
    @Test(
            groups = {
                TestGroup.REST_API,
                TestGroup.SHAREDLINKS,
    ...
    public void testGetVerifyRangeReguestOnSharedLinks() throws Exception {
        ...
        // GET /renditions/{renditionId}/content: get the Range request header for file with shared
        // links endpoints.
        Utility.sleep(
                500,
                30000,

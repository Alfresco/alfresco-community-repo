 ...
 *
 * <p>Completed records of type pdf can be transferred
 *
 * @author Ramona Popa
 * @since 2.6
 */
public class RM4804Test extends BaseRMTestCase {
    ...
    /**
     * Given a category with disposition schedule applied on folder with Cut Of and Transfer, a
     * record folder and a file PDF document to folder, complete the record When execute disposition
     * schedule steps Then the Transfer step is successfully finished
     */
    @org.junit.Test
                ...
                new BehaviourDrivenTest() {
                    public void given() {
                        ...
                        DispositionSchedule dispositionSchedule =
                                dispositionService.createDispositionSchedule(
                                        recordCategory, dsProps);

                        // cutoff immediately
                        Map<QName, Serializable> dispositionActionCutOff = new HashMap<>(3);
                        dispositionActionCutOff.put(
                                PROP_DISPOSITION_ACTION_NAME, CutOffAction.NAME);
                        ...
                        dispositionService.addDispositionActionDefinition(
                                dispositionSchedule, dispositionActionCutOff);

                        // transfer immediately
                        Map<QName, Serializable> dispositionActionTransfer = new HashMap<>(4);
                        dispositionActionTransfer.put(
                                PROP_DISPOSITION_ACTION_NAME, TransferAction.NAME);
                        dispositionActionTransfer.put(
                                PROP_DISPOSITION_DESCRIPTION, GUID.generate());
                        dispositionActionTransfer.put(
                                PROP_DISPOSITION_PERIOD, CommonRMTestUtils.PERIOD_IMMEDIATELY);
                        dispositionActionTransfer.put(PROP_DISPOSITION_LOCATION, StringUtils.EMPTY);

                        dispositionService.addDispositionActionDefinition(
                                dispositionSchedule, dispositionActionTransfer);
                        ...
                        rmActionService.executeRecordsManagementAction(
                                recordFolder, CutOffAction.NAME, null);
                        transferFolder =
                                (NodeRef)
                                        rmActionService
                                                .executeRecordsManagementAction(
                                                        recordFolder, TransferAction.NAME)
                                                .getValue();
                    }

...
public class RenditionServicePermissionsTest {
  ...
  public RunAsFullyAuthenticatedRule runTestsAsUser = new RunAsFullyAuthenticatedRule(
    AuthenticationUtil.getAdminUserName()
  );

  // Tie them together in a static Rule Chain
  @Rule
  public RuleChain nonStaticRuleChain = RuleChain
    .outerRule(runTestsAsUser)
    .around(testNodes)
    .around(testSites);
  ...
  public static void initStaticData() throws Exception {
    contentService =
      (ContentService) APP_CONTEXT_INIT
    ...
    nodeService =
      (NodeService) APP_CONTEXT_INIT
        ...
        .getBean("NodeService");
    permissionService =
      (PermissionService) APP_CONTEXT_INIT
        .getApplicationContext()
        .getBean("PermissionService");
    renditionService =
      (RenditionService) APP_CONTEXT_INIT
        .getApplicationContext()
        .getBean("renditionService");
    repositoryHelper =
      (Repository) APP_CONTEXT_INIT
        .getApplicationContext()
    ...
    thumbnailRegistry =
      (ThumbnailRegistry) APP_CONTEXT_INIT
        .getApplicationContext()
        .getBean("thumbnailRegistry");
  ...
  public void testRenditionAccessPermissions() throws Exception {
    ...
    final NodeRef rendition = transactionHelper.doInTransaction(
    ...
    transactionHelper.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
        public Void execute() throws Throwable {
          ...
          final Serializable renditionCreator = nodeService.getProperty(
            rendition,
    ...
    transactionHelper.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
        public Void execute() throws Throwable {
          ...
          ThumbnailDefinition details = thumbnailRegistry.getThumbnailDefinition(
            TEST_THUMBNAIL
          );
          ...
          services
            .getActionService()
            .executeAction(action, brokenJpg, true, true);
          return null;
        }
      }
    );
    ...
    transactionHelper.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
        public Void execute() throws Throwable {
          ...
          NodeRef failedThumbnailChildNode = failedThumbnailChildren
            .get(0)
            .getChildRef();
  ...
  public void userWithoutDeleteAccessToNodeShouldNotCauseFailedThumbnailProblemsOnUpdate()
    ...
    final NodeRef imgNode = transactionHelper.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<NodeRef>() {
        public NodeRef execute() throws Throwable {
          NodeRef imgNode = testNodes.createQuickFile(
    ...
    for (int i = 1; i <= numRetries; i++) {
      ...
      Boolean hasNoRenditions = transactionHelper.doInTransaction(
  ...
  private RenditionDefinition makeRescaleImageAction() {
    RenditionDefinition result = renditionService.createRenditionDefinition(

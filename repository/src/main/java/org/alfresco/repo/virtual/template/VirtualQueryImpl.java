...
public class VirtualQueryImpl implements VirtualQuery {
  ...
  public PagingResults<Reference> perform(
    ...
    if (!files && !folders) {
      ...
      constraint =
        new IgnoreConstraint(constraint, ignoreTypeQNames, ignoreAspectQNames);
  ...
  private PagingResults<Reference> asPagingResults(
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Unsupported ResultSet.getStart() when trying to create query paging result"
        );
      }
    ...
    final int totlaSecond = !hasMore
      ? (int) result.getNumberFound()
      : (int) (start + result.getNumberFound() + 1);
    final Pair<Integer, Integer> total = new Pair<Integer, Integer>(
      totalFirst,
      totlaSecond
    );

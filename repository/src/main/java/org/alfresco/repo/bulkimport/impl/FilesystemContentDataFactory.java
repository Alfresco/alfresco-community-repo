 ...
 *
 * <br>
 * The mimetype will be guessed from the file extension, or fall back to binary. The encoding will
 * be guessed from the file itself, or fall back to {@link #defaultEncoding}.
 *
 * @since 4.0
 */
public class FilesystemContentDataFactory implements ContentDataFactory, InitializingBean {
        ...
        try {
            ...
            if (!contentLocation.startsWith(rootLocation + File.separator)) {
                throw new IllegalArgumentException(
                        "Can't create content URL : file '"
                                + contentLocation
                                + "' is not located within the store's tree ! The store's root is"
                                + " :'"
                                + rootLocation);
            }
            String relativeFilePath =
                    contentLocation.substring(rootLocation.length() + File.separator.length());
            ...
            ContentData contentData =
                    new ContentData(
                            storeProtocol + PROTOCOL_DELIMITER + relativeFilePath,
                            mimetype,
                            contentFile.length(),
                            encoding);
    ...
    private String guessEncoding(File file, String mimetype) {
        ...
        if (file.isDirectory()) return defaultEncoding; // not necessary to guess folder encoding
        InputStream is = null;
        ...
        try {
            ...
            if (logger.isWarnEnabled())
                logger.warn(
                        "Failed to guess character encoding of file: '"
                                + file.getName()
                                + "'. Falling back to configured default encoding ("
                                + defaultEncoding
                                + ")");
        } finally {
            if (is != null) {

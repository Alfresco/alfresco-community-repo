...
public class RFC822MetadataExtracterUnitTest extends BaseUnitTest {
  ...
  public void testRemoveSensitivePropertiesFromCommonNodes() {
    ...
    when(
      mockedNodeService.hasAspect(node, RecordsManagementModel.ASPECT_RECORD)
    )
      .thenReturn(false);
    ...
    // When
    Map<QName, Serializable> systemProperties = new HashMap<>(
      COMMON_PROPERTIES
    );
    systemProperties.putAll(RECORD_PROPERTIES);
    systemProperties.putAll(DOD_PROPERTIES);
    metadataExtracter.filterSystemProperties(
      systemProperties,
      generateTargetProperties(node)
    );

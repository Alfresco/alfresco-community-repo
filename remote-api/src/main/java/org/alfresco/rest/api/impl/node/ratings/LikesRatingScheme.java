...
public class LikesRatingScheme extends AbstractRatingScheme {
    ...
    public Float getRatingServiceRating(Object rating) {
        ...
        if (rating instanceof Boolean) {
            ...
            throw new InvalidArgumentException(
                    "Rating should be non-null and a boolean for 'likes' rating scheme.");
        }
    ...
    public void applyRating(NodeRef nodeRef, Object rating) {
        try {
            ...
            boolean isContainer =
                    dictionaryService.isSubClass(nodeType, ContentModel.TYPE_FOLDER)
                            && !dictionaryService.isSubClass(
                                    nodeType, ContentModel.TYPE_SYSTEM_FOLDER);
            postActivity(
                    nodeRef, isContainer ? ActivityType.FOLDER_LIKED : ActivityType.FILE_LIKED);

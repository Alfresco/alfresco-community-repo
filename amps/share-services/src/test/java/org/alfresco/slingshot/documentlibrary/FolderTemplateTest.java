...
public class FolderTemplateTest extends BaseWebScriptTest {
  ...
  protected void setUp() throws Exception {
    ...
    Map<QName, Serializable> propsTemplate = new HashMap<QName, Serializable>(
      1
    );
    ...
    // Create destination
    Map<QName, Serializable> propsDestination = new HashMap<QName, Serializable>(
      1
    );
    ...
    destination =
      nodeService
        .createNode(
          companyHome,
          ContentModel.ASSOC_CHILDREN,
          QName.createQName(destinationName),
          ContentModel.TYPE_FOLDER,
          propsDestination
        )
        .getChildRef();
  ...
  public void testFolderTemplatesPost() throws Exception {
    ...
    String newFolderQuery =
      "/app:company_home/" + destinationName + "/" + newName;
    ...
    FileInfo newFolder = fileFolderService.getFileInfo(
      result.getRow(0).getNodeRef()
    );
    ...
    assertEquals(
      "The folder's description should be " +
      newDescription +
      ", but was " +
      newFolder.getProperties().get(ContentModel.PROP_DESCRIPTION),
      newDescription,
      newFolder.getProperties().get(ContentModel.PROP_DESCRIPTION)
    );
    assertEquals(
      "The folder's title should be " +
      newTitle +
      ", but was " +
      newFolder.getProperties().get(ContentModel.PROP_TITLE),
      newTitle,
      newFolder.getProperties().get(ContentModel.PROP_TITLE)
    );
    ...
    Object contentJsonObject = jsonParser.parse(response.getContentAsString());
    JSONObject jsonData = (JSONObject) contentJsonObject;
    String persistedObject = (String) jsonData.get("persistedObject");
    assertEquals(
      "The response's persistedObject should be " +
      newFolder.getNodeRef().toString() +
      " but it was " +
      persistedObject,
      newFolder.getNodeRef().toString(),
      persistedObject

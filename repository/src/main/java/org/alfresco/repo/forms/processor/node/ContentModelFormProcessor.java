...
public abstract class ContentModelFormProcessor<ItemType, PersistType>
   ...
   */
  protected Pattern propertyNamePattern = Pattern.compile(
    PROP_DATA_PREFIX + "([a-zA-Z0-9-]+)_(.*)"
  );
   ...
   */
  protected Pattern transientPropertyPattern = Pattern.compile(
    PROP_DATA_PREFIX + "(.*){1}?"
  );
   ...
   * parts.
   */
  protected Pattern associationNamePattern = Pattern.compile(
    ASSOC_DATA_PREFIX + "([a-zA-Z0-9-]+)_(.*)(_[a-zA-Z]+)"
  );
  ...
  private String makePropDataKey(QName propName) {
    ...
    String dataKey =
      FormFieldConstants.PROP_DATA_PREFIX + propPrefixName.replace(':', '_');
    return dataKey;
  }
  ...
  protected ContentModelItemData<ItemType> makeItemData(ItemType item) {
    ...
    TypeDefinition anonType = dictionaryService.getAnonymousType(
      baseType.getName(),
      aspects
    );
  ...
  protected List<String> getDefaultIgnoredFields() {
    ...
    fields.add(
      ContentModel.PROP_NODE_DBID.toPrefixString(this.namespaceService)
    );
    fields.add(
      ContentModel.PROP_NODE_UUID.toPrefixString(this.namespaceService)
    );
    fields.add(
      ContentModel.PROP_STORE_IDENTIFIER.toPrefixString(this.namespaceService)
    );
    fields.add(
      ContentModel.PROP_STORE_PROTOCOL.toPrefixString(this.namespaceService)
    );

    // ignore associations that are system maintained
    fields.add(
      RenditionModel.ASSOC_RENDITION.toPrefixString(this.namespaceService)
    );
  ...
  protected abstract Map<QName, Serializable> getAssociationValues(
    ItemType item
  );
  ...
  protected void persistNode(NodeRef nodeRef, FormData data) {
    ...
    TypeDefinition typeDef =
      this.dictionaryService.getAnonymousType(
          type,
  ...
  protected void processPropertyPersist(
    ...
    if (getLogger().isDebugEnabled()) getLogger()
      .debug("Processing field " + fieldData + " for property persistence");
    ...
    Matcher m =
      this.propertyNamePattern.matcher(
          fieldData
            .getName()
            .replaceAll(DOT_CHARACTER_REPLACEMENT, DOT_CHARACTER)
        );
    if (m.matches()) {
      String qNamePrefix = m.group(1);
      String localName = m.group(2);
      QName fullQName = QName.createQName(
        qNamePrefix,
        localName,
        namespaceService
      );
      ...
      if (propDef != null) {
        ...
        if (fullQName.equals(ContentModel.PROP_NAME)) {
          ...
          propDef.getDataType().getName().equals(DataTypeDefinition.CONTENT)
        ) {
          ...
          if (propDef.isMultiValued()) {
            ...
            propDef.getDataType().getName().equals(DataTypeDefinition.BOOLEAN)
          ) {
            ...
            if (
              !propDef
                .getDataType()
                .getName()
                .equals(DataTypeDefinition.TEXT) &&
      ...
      if (tppm.matches()) {
        ...
        if (fieldName.equals(MimetypeFieldProcessor.KEY)) {
          ...
          getLogger()
            .warn("Ignoring unrecognised field '" + fieldData.getName() + "'");
        }
      } else if (getLogger().isWarnEnabled()) {
        getLogger()
          .warn("Ignoring unrecognised field '" + fieldData.getName() + "'");
  ...
  protected void processAssociationPersist(
    ...
    if (getLogger().isDebugEnabled()) getLogger()
      .debug("Processing field " + fieldData + " for association persistence");
          ...
          if (getLogger().isWarnEnabled()) {
            getLogger()
              .warn(
                "Ignoring field '" +
                fieldName +
                "' as a valid association definition can not be found"
              );
          }
  ...
  protected void processNamePropertyPersist(
    ...
    if (fileInfo != null) {
      try {
        ...
        throw new FormException(
          "Failed to persist field '" + fieldData.getName() + "'",
          fnne
        );
      }
    } else {
      ...
      propsToPersist.put(
        ...
        (Serializable) fieldData.getValue()
      );
    }
  }
  ...
  protected void processMimetypePropertyPersist(
    ...
    ContentData contentData = (ContentData) propsToPersist.get(
    ...
    if (contentData != null) {
      ...
      contentData =
        ContentData.setMimetype(contentData, (String) fieldData.getValue());
  ...
  protected void processEncodingPropertyPersist(
    ...
    if (contentData != null) {
      // update content data if we found the property
      contentData =
        ContentData.setEncoding(contentData, (String) fieldData.getValue());
      propsToPersist.put(ContentModel.PROP_CONTENT, contentData);
    }
  }
      ...
      if (defaultMimetypeRequired) {
        ...
        if (contentData != null) {
          ...
          if (propsToPersist.containsKey(ContentModel.PROP_CONTENT)) {
            ContentData mimetypeEncoding = (ContentData) propsToPersist.get(
              ContentModel.PROP_CONTENT
            );
            contentData =
              ContentData.setMimetype(
  ...
  protected String determineDefaultMimetype(FormData data) {
    ...
    if (data != null) {
      FieldData mimetypeField = data.getFieldData(
        PROP_DATA_PREFIX + MimetypeFieldProcessor.KEY
      );
...
class AddAssocCommand extends AbstractAssocCommand {
  ...
  protected void updateAssociations(NodeService nodeService) {
    List<AssociationRef> existingAssocs = nodeService.getTargetAssocs(
      sourceNodeRef,
      assocQName
    );
    for (AssociationRef assoc : existingAssocs) {
      if (assoc.getTargetRef().equals(targetNodeRef)) {
        if (logger.isWarnEnabled()) {
          logger.warn(
            "Attempt to add existing association prevented. " + assoc
          );
        }
        return;
      }
  ...
  protected void updateAssociations(NodeService nodeService) {
    ...
    if (assocDoesNotExist) {
      if (logger.isWarnEnabled()) {
        StringBuilder msg = new StringBuilder();
        msg
          .append("Attempt to remove non-existent association prevented. ")
...
class AddChildAssocCommand extends AbstractAssocCommand {
  ...
  private static final Log logger = LogFactory.getLog(
    AddChildAssocCommand.class
  );

  public AddChildAssocCommand(
    NodeRef sourceNodeRef,
    NodeRef targetNodeRef,
    QName assocQName
  ) {
  ...
  @Override
  protected void updateAssociations(NodeService nodeService) {
    List<ChildAssociationRef> existingChildren = nodeService.getChildAssocs(
      sourceNodeRef
    );
    ...
    for (ChildAssociationRef assoc : existingChildren) {
      if (assoc.getChildRef().equals(targetNodeRef)) {
        if (logger.isWarnEnabled()) {
          logger.warn(
            "Attempt to add existing child association prevented. " + assoc
          );
        }
        return;
      }
...
class RemoveChildAssocCommand extends AbstractAssocCommand {
  ...
  private static final Log logger = LogFactory.getLog(
    RemoveChildAssocCommand.class
  );

  public RemoveChildAssocCommand(
    NodeRef sourceNodeRef,
    NodeRef targetNodeRef,
    QName assocQName
  ) {
  ...
  protected void updateAssociations(NodeService nodeService) {
    ...
    if (childAssocDoesNotExist) {
      if (logger.isWarnEnabled()) {
        ...
        msg
          .append(
            "Attempt to remove non-existent child association prevented. "
          )
          .append(sourceNodeRef)
          .append("|")
          .append(targetNodeRef)

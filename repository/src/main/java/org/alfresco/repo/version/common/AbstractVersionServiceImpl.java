...
public abstract class AbstractVersionServiceImpl {
    ...
    protected void invokeOnCreateVersion(
            NodeRef nodeRef, Map<String, Serializable> versionProperties, PolicyScope nodeDetails) {
    ...
    protected RevertAspectAction getRevertAspectAction(
            QName aspectName, VersionRevertDetails revertDetails) {
        // first check for a policy for the node type
        Collection<OnRevertVersionPolicy> policies =
                onRevertVersionDelegate.getList(revertDetails.getNodeRef(), aspectName);
    ...
    protected RevertAssocAction getRevertAssocAction(
        ...
        Collection<OnRevertVersionPolicy> policies =
                onRevertVersionDelegate.getList(revertDetails.getNodeRef(), className);
        for (OnRevertVersionPolicy policy : policies) {
    ...
    /**
     * Default implementation of the on create version policy. Override if you wish to supply your
     * own policy.
     *
     * @param classRef QName
    ...
    protected String invokeCalculateVersionLabel(
        ...
        if (behaviours.size() == 0) {
            ...
            CalculateVersionLabelPolicy[] arr =
                    behaviours.toArray(new CalculateVersionLabelPolicy[] {});
            versionLabel =
            ...
            throw new VersionServiceException(
                    "More than one CalculateVersionLabelPolicy behaviour has been registered for"
                            + " the type "
                            + classRef.toString());
        }

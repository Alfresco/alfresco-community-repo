...
public class DbCmisQueryLanguage extends AbstractLuceneQueryLanguage {
    ...
    public ResultSet executeQuery(SearchParameters searchParameters) {
        if (metadataIndexCheck1.getPatchApplied()) {
            ...
            throw new QueryModelException(
                    "The patch to add the indexes to support in-transactional metadata queries has"
                            + " not been applied");
        }
    }
    ...
    private ResultSet executeQueryImpl(SearchParameters searchParameters) {
        ...
        org.alfresco.repo.search.impl.querymodel.Query queryModelQuery =
                parser.parse(queryEngine.getQueryModelFactory(), functionContext);

        QueryEngineResults results =
                queryEngine.executeQuery(queryModelQuery, options, functionContext);

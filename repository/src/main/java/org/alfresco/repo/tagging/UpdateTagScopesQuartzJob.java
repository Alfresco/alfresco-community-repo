...
public class UpdateTagScopesQuartzJob implements Job {
  ...
  public void execute(JobExecutionContext context)
    ...
    if (
      transactionServiceO == null ||
      !(transactionServiceO instanceof TransactionService)
    ) {
      throw new AlfrescoRuntimeException(
    ...
    if (transactionService.isReadOnly()) {
      logger.info(
        "Skipping Tag Scopes scheduled update as the system is Read Only"
      );
      return;
    }
  ...
  protected void execute(
    ...
    while (true) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Checked for tag scopes with pending tag updates, found " + tagNodes
        );
      }

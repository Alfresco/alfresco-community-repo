...
public class PasswordHashingTest {
  ...
  public void testRehashedPassword() throws Exception {
    ...
    assertFalse(
      "Should be empty",
      properties.containsKey(ContentModel.PROP_PASSWORD_HASH)
    );
    //No hashing to do but we need to update the Indicator
    assertTrue(passwordHashWorker.processPasswordHash(properties));
    assertEquals(
      CompositePasswordEncoder.MD4,
      properties.get(ContentModel.PROP_HASH_INDICATOR)
    );
    assertTrue(
      "Should now contain the password",
      properties.containsKey(ContentModel.PROP_PASSWORD_HASH)
    );
    assertFalse(
      "Should remove the property",
      properties.containsKey(ContentModel.PROP_PASSWORD)
    );
    assertFalse(
      "Should remove the property",
      properties.containsKey(ContentModel.PROP_PASSWORD_SHA256)
    );
  ...
  public void testRehashedPasswordBcrypt() throws Exception {
    ...
    properties.put(
      ...
      (Serializable) Arrays.asList("md4")
    );
  ...
  public void testGetPasswordHash() throws Exception {
    ...
    try {
      ...
      assertTrue(
        are.getMessage().contains("Unable to find a password for user")
      );
    }
    ...
    Pair<List<String>, String> passwordHashed = RepositoryAuthenticationDao.determinePasswordHash(
      properties
    );
    ...
    properties.put(
      ContentModel.PROP_HASH_INDICATOR,
      (Serializable) Arrays.asList("myencoding")
    );
  ...
  private static boolean matches(
    ...
    return cpe.matchesPassword(
      password,
      (String) properties.get(ContentModel.PROP_PASSWORD_HASH),
      (String) properties.get(ContentModel.PROP_SALT),

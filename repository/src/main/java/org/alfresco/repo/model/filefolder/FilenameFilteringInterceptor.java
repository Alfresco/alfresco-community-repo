...
/**
 * An interceptor that intercepts FileFolderService methods, ensuring system, temporary and hidden
 * files and paths are marked with the correct aspects.
 *
 * @author alex.mukha
 */
public class FilenameFilteringInterceptor implements MethodInterceptor {
    ...
    private static final Pattern XSL_MACOS_TEMPORARY_FILENAME_FITLER =
            Pattern.compile(
        ...
        if (methodName.startsWith("create")) {
            ...
            if (getMode() == Mode.ENHANCED) {
                if (systemPaths.isFiltered(filename)) {
                    // it's a system file/folder, create as system and allow full control to all
                    // authorities
                    ...
                    hiddenAspect.hideNode(
                            fileInfo, getSystemFileVisibilityMask(), false, false, false);
                } else {
                    ...
                    if (isSystemPath(nodeRef, filename)) {
                        // it's on a system path, check whether temporary, hidden and noindex
     ...
     *   <li>it has Mimetype equal to
     *       ‘application/vnd.openxmlformats-officedocument.spreadsheetml.sheet’.
     * </ul>
     *
     * @param name - {@link String} value which contains name of node
     * @param nodeRef - {@link NodeRef} instance of the node
     * @return {@link boolean} value. <code>true</code> if <code>name</code> is name of temporary
     *     object.
     */

...
public class RmRestApiTest
  ...
  protected static final String GET_NODE_AUDITLOG_URL_FORMAT =
    "/api/node/{0}/rmauditlog";
  protected static final String GET_TRANSFER_URL_FORMAT =
    "/api/node/{0}/transfers/{1}";
  protected static final String TRANSFER_REPORT_URL_FORMAT =
    "/api/node/{0}/transfers/{1}/report";
  protected static final String REF_INSTANCES_URL_FORMAT =
    "/api/node/{0}/customreferences";
  protected static final String RMA_AUDITLOG_URL = "/api/rma/admin/rmauditlog";
  protected static final String RMA_AUDITLOG_STATUS_URL =
    "/api/rma/admin/rmauditlog/status";
  protected static final String GET_LIST_URL = "/api/rma/admin/listofvalues";
  protected static final String RMA_ACTIONS_URL =
    "/api/rma/actions/ExecutionQueue";
  protected static final String APPLICATION_JSON = "application/json";
  protected static final String RMA_CUSTOM_PROPS_DEFINITIONS_URL =
    "/api/rma/admin/custompropertydefinitions";
  protected static final String RMA_CUSTOM_REFS_DEFINITIONS_URL =
  ...
  public void testPostActionToNonExistentNode() throws Exception {
    NodeRef nonExistentNode = new NodeRef(
      "workspace://SpacesStore/09ca1e02-1c87-4a53-97e7-xxxxxxxxxxxx"
    );
    ...
    sendRequest(
      new PostRequest(RMA_ACTIONS_URL, jsonPostString, APPLICATION_JSON),
      expectedStatus
    );
  }
  ...
  public void testPostReviewedAction() throws IOException, JSONException {
    ...
    Serializable pristineReviewAsOf =
      this.nodeService.getProperty(testRecord, PROP_REVIEW_AS_OF);
    ...
    Response rsp = sendRequest(
      new PostRequest(RMA_ACTIONS_URL, jsonString, APPLICATION_JSON),
      expectedStatus
    );
    ...
    Serializable newReviewAsOfDate =
      this.nodeService.getProperty(testRecord, PROP_REVIEW_AS_OF);
    assertFalse(
      "The reviewAsOf property should have changed. Was " + pristineReviewAsOf,
  ...
  public void testPostMultiReviewedAction() throws IOException, JSONException {
    ...
    Serializable pristineReviewAsOf =
      this.nodeService.getProperty(testRecord, PROP_REVIEW_AS_OF);
    Serializable pristineReviewAsOf2 =
      this.nodeService.getProperty(testRecord2, PROP_REVIEW_AS_OF);
    Serializable pristineReviewAsOf3 =
      this.nodeService.getProperty(testRecord3, PROP_REVIEW_AS_OF);
    ...
    Serializable newReviewAsOfDate2 =
      this.nodeService.getProperty(testRecord2, PROP_REVIEW_AS_OF);
    assertFalse(
      "The reviewAsOf property should have changed. Was " + pristineReviewAsOf2,
      pristineReviewAsOf2.equals(newReviewAsOfDate2)
    );
    Serializable newReviewAsOfDate3 =
      this.nodeService.getProperty(testRecord3, PROP_REVIEW_AS_OF);
    assertFalse(
      "The reviewAsOf property should have changed. Was " + pristineReviewAsOf3,
  ...
  public void testActionParams() throws Exception {
    ...
    String jsonString = new JSONStringer()
      ...
      .key("params")
      .object()
      .key(TestActionParams.PARAM_DATE)
      .object()
      .key("iso8601")
  ...
  public void testPostCustomReferenceDefinitions()
   ...
   */
  private String[] postCustomReferenceDefinitions()
    ...
    String jsonString = new JSONStringer()
      .object()
      .key("referenceType")
      .value(RelationshipType.PARENTCHILD)
      .key("source")
      .value(CHILD_SRC)
      .key("target")
      .value(CHILD_TGT)
    ...
    Response rsp = sendRequest(
      new PostRequest(
        RMA_CUSTOM_REFS_DEFINITIONS_URL,
        jsonString,
        APPLICATION_JSON
      ),
      expectedStatus
    ...
    String generatedChildRefId = jsonRsp
      .getJSONObject("data")
      .getString("refId");
    ...
    jsonString =
      new JSONStringer()
        .object()
        .key("referenceType")
        .value(RelationshipType.BIDIRECTIONAL)
        .key("label")
        .value(BI_DI)
        .endObject()
        .toString();
    ...
    String generatedBidiRefId = jsonRsp
      .getJSONObject("data")
      .getString("refId");
    result[1] = generatedBidiRefId;

    // Now assert that both have appeared in the data dictionary.
    AspectDefinition customAssocsAspect = dictionaryService.getAspect(
    ...
    assertTrue(
      "Custom child assoc not returned by dataDictionary.",
      associations.containsKey(newRefQname)
    );

    newRefQname = adminService.getQNameForClientId(generatedBidiRefId);
    assertTrue(
      "Custom std assoc not returned by dataDictionary.",
      customAssocsAspect.getAssociations().containsKey(newRefQname)
    );
  ...
  public void testPutCustomPropertyDefinition() throws Exception {
    ...
    Response rsp = sendRequest(
      new PutRequest(propDefnUrl, jsonString, APPLICATION_JSON),
      200
    );
    ...
    assertEquals(
      "Wrong customProperties length.",
      1,
      customPropsObject.length()
    );

    Object keyToSoleProp = customPropsObject.keys().next();

    JSONObject newPropObject = customPropsObject.getJSONObject(
      (String) keyToSoleProp
    );
    assertEquals(
      "Wrong property label.",
      updatedLabel,
      newPropObject.getString("label")
    );
    JSONArray constraintRefsArray = newPropObject.getJSONArray(
      "constraintRefs"
    );
    assertEquals(
      "ConstraintRefsArray wrong length.",
      1,
      constraintRefsArray.length()
    );
    String retrievedUpdatedTitle = constraintRefsArray
      .getJSONObject(0)
      .getString("name");
    assertEquals(
      "Constraints had wrong name.",
      "rmc:tlList",
      retrievedUpdatedTitle
    );
    ...
    String readdedUpdatedTitle = constraintRefsArray
      .getJSONObject(0)
      .getString("name");
    assertEquals(
      "Constraints had wrong name.",
  ...
  public void testGetCustomReferences() throws IOException, JSONException {
    ...
    final int expectedStatus = 200;
    Response rsp = sendRequest(
      new GetRequest(RMA_CUSTOM_REFS_DEFINITIONS_URL),
      expectedStatus
    );

    JSONObject jsonRsp = new JSONObject(
      new JSONTokener(rsp.getContentAsString())
    );
    ...
    assertTrue(
      "There should be at least two custom references. Found " + customRefsObj,
      customRefsObj.length() >= 2
    );
    ...
    rsp =
      sendRequest(
        new GetRequest(
          RMA_CUSTOM_REFS_DEFINITIONS_URL +
          "/" +
          CUSTOM_REF_VERSIONS.getLocalName()
        ),
        expectedStatus
      );
    ...
    assertTrue(
      "There should be exactly 1 custom references. Found " +
      customRefsObj.length(),
      customRefsObj.length() == 1
    );
  }
  ...
  public void testGetDodCustomTypes() throws IOException, JSONException {
    final int expectedStatus = 200;
    Response rsp = sendRequest(
      new GetRequest("/api/rma/admin/dodcustomtypes"),
      expectedStatus
    );
  ...
  public void testGetPostAndRemoveCustomReferenceInstances() throws Exception {
    NodeRef testRecord1 = utils.createRecord(
      recordFolder,
      "testRecord1" + System.currentTimeMillis(),
      "The from recørd"
    );
    NodeRef testRecord2 = utils.createRecord(
      recordFolder,
      "testRecord2" + System.currentTimeMillis(),
      "The to récord"
    );

    String node1Url = testRecord1.toString().replace("://", "/");
    String refInstancesRecord1Url = MessageFormat.format(
    ...
    String jsonString = new JSONStringer()
      ...
      .value(testRecord2.toString())
      .key("refId")
      .value(generatedRefIds[1])
      .endObject()
      .toString();

    Response rsp = sendRequest(
      new PostRequest(refInstancesRecord1Url, jsonString, APPLICATION_JSON),
      200
    );
    ...
    JSONArray customRefsFromArray = (JSONArray) dataObj.get(
      "customReferencesFrom"
    );
    assertNotNull(
      "JSON 'customReferencesFrom' object was null",
      customRefsFromArray
    );

    int customRefsCount = customRefsFromArray.length();
    assertTrue(
      "There should be at least one custom reference. Found " +
      customRefsFromArray,
      customRefsCount > 0
    );
    ...
    assertNotNull(
      "JSON 'customReferencesTo' object was null",
      customRefsToArray
    );
    assertEquals(
      "customReferencesTo wrong length.",
      0,
      customRefsToArray.length()
    );

    // 2. Back-references on the 'to' record
    String node2Url = testRecord2.toString().replace("://", "/");
    String refInstancesRecord2Url = MessageFormat.format(
      REF_INSTANCES_URL_FORMAT,
      node2Url
    );
    ...
    assertEquals(
      "customReferencesFrom wrong length.",
      0,
      customRefsFromArray.length()
    );
    ...
    String urlQueryString = MessageFormat.format(
      queryFormat,
      protocol,
      identifier,
      recId
    ...
    rsp =
      sendRequest(
        new DeleteRequest(
          refInstancesRecord1Url + "/" + generatedRefIds[1] + urlQueryString
        ),
        200
      );
    ...
    rsp =
      sendRequest(
        new DeleteRequest(
          refInstancesRecord1Url + "/" + generatedRefIds[0] + urlQueryString
        ),
        200
      );
    assertTrue(rsp.getContentAsString().contains("success"));
    ...
    customRefsFromArray = (JSONArray) dataObj.get("customReferencesFrom");
    assertNotNull(
      "JSON 'customReferences' object was null",
      customRefsFromArray
    );
    assertTrue(
      "customRefsArray was unexpectedly not empty.",
      customRefsFromArray.length() == 0
    );
  }

  public void testMob1630ShouldNotBeAbleToCreateTwoSupersedesReferencesOnOneRecordPair()
    throws Exception {
    ...
    rsp =
      sendRequest(
        new DeleteRequest(
          refInstancesRecord1Url + "/" + supersedesRefLocalName + urlQueryString
        ),
        200
      );
    assertTrue(rsp.getContentAsString().contains("success"));
  ...
  public void testPostCustomPropertyDefinition() throws Exception {
    ...
    postCustomPropertyDefinition(
      "customProperty" + currentTimeMillis,
      "prop" + currentTimeMillis
    );
  }
   ...
   */
  private String postCustomPropertyDefinition(
    ...
    if (propId == null) {
      jsonString =
        new JSONStringer()
          ...
          .value(propertyLabel)
          .key("description")
          .value("Dynamically defined test property")
          .key("mandatory")
          .value(false)
          .key("dataType")
          .value("d:text")
    ...
    Response rsp = sendRequest(
      new PostRequest(
        "/api/rma/admin/custompropertydefinitions?element=record",
        jsonString,
        APPLICATION_JSON
      ),
      expectedStatus
  ...
  public void testPutCustomReferenceDefinition() throws Exception {
    ...
    String refDefnUrl =
      "/api/rma/admin/customreferencedefinitions/" + bidiRefId;
    ...
    rsp =
      sendRequest(
        new PutRequest(refDefnUrl, jsonString, APPLICATION_JSON),
        200
      );
    ...
    assertEquals(
      "Wrong customReferences length.",
      1,
      customRefsObject.length()
    );
    ...
    jsonString =
      new JSONStringer()
        .object()
        .key("source")
        .value(updatedPcSource)
        .key("target")
        .value(updatedPcTarget)
        .endObject()
        .toString();
    ...
    assertEquals(
      "Wrong reference source.",
      updatedPcSource,
      newRefObject.getString("source")
    );
    assertEquals(
      "Wrong reference target.",
      updatedPcTarget,
      newRefObject.getString("target")
    );
  }
  ...
  private String getCustomProperties()
    throws Exception, IOException, UnsupportedEncodingException, JSONException {
    // Ensure that there is at least one custom property.
    this.testPostCustomPropertyDefinition();

    final int expectedStatus = 200;
    Response rsp = sendRequest(
      new GetRequest("/api/rma/admin/custompropertydefinitions?element=record"),
      expectedStatus
    );
    ...
    assertTrue(
      "There should be at least one custom property. Found " + customPropsObj,
      customPropsCount > 0
    );
  ...
  public void testGetRecordMetaDataAspects() throws Exception {
    Response rsp = sendRequest(
      new GetRequest("/api/rma/recordmetadataaspects"),
      200
    );
    String contentAsString = rsp.getContentAsString();
    System.out.println(contentAsString);
  ...
  public void testExport() throws Exception {
    ...
    Response rsp = sendRequest(
      new PostRequest(exportUrl, jsonPostString, APPLICATION_JSON),
      200
    );
  ...
  public void testAudit() throws Exception {
    ...
    JSONObject jsonParsedObject = new JSONObject(
      new JSONTokener(rsp.getContentAsString())
    );
    ...
    String auditUrl = MessageFormat.format(
      GET_NODE_AUDITLOG_URL_FORMAT,
      nodeUrl
    );
    ...
    String filteredAuditUrl =
      auditUrl +
      "?user=gavinc&size=5&from=2009-01-01&to=2009-12-31&event=Login";
    ...
    filteredAuditUrl =
      auditUrl + "?user=fred&size=abc&from=2009&to=2010&property=wrong";
    rsp = sendRequest(new GetRequest(filteredAuditUrl), 200);
    assertEquals("application/json", rsp.getContentType());
    jsonRsp = new JSONObject(new JSONTokener(rsp.getContentAsString()));
    ...
    rsp =
      sendRequest(
        new PutRequest(RMA_AUDITLOG_URL, jsonPostString, APPLICATION_JSON),
        200
      );
  ...
  private void checkAuditStatus(boolean expected) throws Exception {
    ...
    assertEquals(
      "Audit log status does not match expected status.",
      expected,
      enabled
    );
  }
  ...
  public void testFileAuditLogAsRecord() throws Exception {
    ...
    String jsonPostString = jsonPostData.toString();
    Response rsp = sendRequest(
      new PostRequest(RMA_AUDITLOG_URL, jsonPostString, APPLICATION_JSON),
      404
    );
  ...
  public void testPropertyLabelWithAccentedChars() throws Exception {
    ...
    JSONObject accentedPropertyObj = customPropertiesObj.getJSONObject(
      RecordsManagementCustomModel.RM_CUSTOM_PREFIX + ":" + accentedPropId
    );
  ...
  private void putCustomPropDefinition(String label, String id)
    throws JSONException, IOException, UnsupportedEncodingException {
    String jsonString = new JSONStringer()
      .object()
      .key("label")

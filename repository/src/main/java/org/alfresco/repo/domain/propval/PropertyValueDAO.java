...
public interface PropertyValueDAO
{
     ...
     * 
     * @param value
     *            the value to find the ID for (may be <tt>null</tt>)
     */
     ...
     * Use for accessing unique properties; see interface comments.
     * <p/>
     * <b>alf_prop_value</b> accessor: find or create a property based on the value. <b>Note:</b> This method will not recurse into maps or collections. Use the dedicated methods if you want recursion; otherwise maps and collections will be serialized and probably stored as BLOB values.
     * <p/>
     ...
     * 
     * @param ids
     *            the IDs (may not be <tt>null</tt>; may be empty)
     * @param callback
     *            the callback to handle the results
     * @throws DataIntegrityViolationException
     *             if any of the the IDs are invalid
     */
     ...
     * 
     * @param value
     *            the value to create (may be <tt>null</tt>)
     ...
     * 
     * @param id
     *            the ID of the root property to change
     * @param value
    ...
    // ================================
    /**
     * <b>alf_prop_unique_ctx</b> accessor: create a unique context with an optional associated value.
     * <p/>
     ...
     *            a value to store against the key (may be <tt>null</tt>)
    ...
    /**
     * Get the unique context ID and associated shared property ID, or <tt>null</tt> if no such context exists. The associated property may be <tt>null</tt> even if the unique context exists.
     *
     * @param value1
     *            first value
     * @param value2
    ...
    /**
     * Get unique contexts (unique context ID and associated shared property ID), if any, based on one, two or three context values. The associated property may be <tt>null</tt> even if the unique context exists.
     *
     * @param values
     ...
     * <p/>
     * If the rows passed in don't constitute a valid, full property - they don't contain all the link entities for the property - then the result may be <tt>null</tt>.
     * 
     * @param rows
     *            the search results for a single root property
     * @return Returns the root property as originally persisted, or <tt>null</tt> if the rows don't represent a complete property

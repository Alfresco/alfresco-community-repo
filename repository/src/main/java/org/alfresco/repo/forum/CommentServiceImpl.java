...
public class CommentServiceImpl extends AbstractLifecycleBean
    ...
    private static final QName FORUM_TO_TOPIC_ASSOC_QNAME =
            ...
            if (this.cannedQueryRegistry == null) {
                this.cannedQueryRegistry =
                        (NamedObjectRegistry<CannedQueryFactory<? extends Object>>)
                                ctx.getBean("commentsCannedQueryRegistry");
            }
    ...
    public NodeRef getDiscussableAncestor(NodeRef descendantNodeRef) {
        // For "Share comments" i.e. fm:post nodes created via the Share commenting UI, the
        // containment structure is as follows:
        // fm:discussable
        //    - fm:forum
        //        - fm:topic
        //            - fm:post
        // For other fm:post nodes the ancestor structure may be slightly different. (cf. Share
        // discussions, which don't have 'forum')
        //
        // In order to ensure that we only return the discussable ancestor relevant to Share
        // comments, we'll climb the
        // containment tree in a controlled manner.
        // We could navigate up looking for the first fm:discussable ancestor, but that might not
        // find the correct node - it could,
        ...
        if (nodeService.getType(descendantNodeRef).equals(ForumModel.TYPE_POST)) {
            ...
            if (nodeService.getType(topicNode).equals(ForumModel.TYPE_TOPIC)) {
                ChildAssociationRef forumToTopicChildAssocRef =
                        nodeService.getPrimaryParent(topicNode);
                ...
                if (forumToTopicChildAssocRef.getQName().equals(FORUM_TO_TOPIC_ASSOC_QNAME)) {
                    ...
                    if (nodeService.getType(forumNode).equals(ForumModel.TYPE_FORUM)) {
                        NodeRef discussableNode =
                                nodeService.getPrimaryParent(forumNode).getParentRef();
            ...
            GetChildrenCannedQuery cq =
                    (GetChildrenCannedQuery)
                            getChildrenCannedQueryFactory.getCannedQuery(
                                    commentsFolder, null, null, null, null, sort, paging);
    ...
    public NodeRef getShareCommentsTopic(NodeRef discussableNode) {
        ...
        if (nodeService.hasAspect(discussableNode, ForumModel.ASPECT_DISCUSSABLE)) {
            // We navigate down the "Share comments" containment model, which is based on the more
            // general forum model,
            // but with certain naming conventions.
            List<ChildAssociationRef> fora =
            ...
            if (!fora.isEmpty()) {
                ...
                List<ChildAssociationRef> topics =
                        nodeService.getChildAssocs(
                                firstForumNode,
                                ContentModel.ASSOC_CONTAINS,
                                FORUM_TO_TOPIC_ASSOC_QNAME,
                                true);
    ...
    //
    //        NodeRef commentsFolder = AuthenticationUtil.runAs(new
    ...
    // QName.createQName(NamespaceService.FORUMS_MODEL_1_0_URI, "discussable"), null);
    ...
    // COMMENTS_TOPIC_NAME),
    //                                QName.createQName(NamespaceService.FORUMS_MODEL_1_0_URI,
    // "topic"),
    //                                props).getChildRef();
    //                    }
    //                }
    ...
    private JSONObject getActivityData(String siteId, final NodeRef nodeRef) {
        if (siteId != null) {
            ...
            try {
                ...
                // MNT-11667 "createComment" method creates activity for users who are not supposed
                // to see the file
    ...
    public NodeRef createComment(
        ...
        final List<ChildAssociationRef> existingTopics =
                nodeService.getChildAssocs(
                        forumNode, ContentModel.ASSOC_CONTAINS, FORUM_TO_TOPIC_ASSOC_QNAME);
    ...
    public void updateComment(NodeRef commentNodeRef, String title, String comment) {
        ...
        try {
            ContentWriter writer =
                    contentService.getWriter(commentNodeRef, ContentModel.PROP_CONTENT, true);
        ...
        if (discussableNodeRef != null) {
            ...
            logger.warn(
                    "Unable to determine discussable node for the comment with nodeRef "
                            + commentNodeRef
                            + ", not posting an activity");
        }
    }
    ...
    public void deleteComment(NodeRef commentNodeRef) {
        ...
        // determine the siteId and activity data of the comment NodeRef (do this before removing
        // the comment NodeRef)
        String siteId = getSiteId(commentNodeRef);
        NodeRef discussableNodeRef = getDiscussableAncestor(commentNodeRef);
    ...
    public Map<String, Boolean> getCommentPermissions(
            NodeRef discussableNode, NodeRef commentNodeRef) {
        ...
        if (discussableNodeRef.equals(discussableNode)) {
            if (!isWorkingCopyOrLocked(discussableNode) || isLockOwner(discussableNode)) {
    ...
    private boolean canEditPermission(NodeRef commentNodeRef) {
        String creator =
                (String) nodeService.getProperty(commentNodeRef, ContentModel.PROP_CREATOR);
        Serializable owner = nodeService.getProperty(commentNodeRef, ContentModel.PROP_OWNER);
        String currentUser = AuthenticationUtil.getFullyAuthenticatedUser();

        boolean isSiteManager =
                permissionService.hasPermission(commentNodeRef, SiteModel.SITE_MANAGER)
                        == (AccessStatus.ALLOWED);
        boolean isCoordinator =
                permissionService.hasPermission(commentNodeRef, PermissionService.COORDINATOR)
                        == (AccessStatus.ALLOWED);
        return (isSiteManager
                || isCoordinator
                || currentUser.equals(creator)
                || currentUser.equals(owner));
    }

    private boolean canDeletePermission(NodeRef commentNodeRef) {
        return permissionService.hasPermission(commentNodeRef, PermissionService.DELETE)
                == AccessStatus.ALLOWED;
    }
    ...
    public void onUpdateProperties(
        ...
        if (discussableNodeRef != null) {
            if (behaviourFilter.isEnabled(ContentModel.ASPECT_LOCKABLE)
                ...
                // check if comment updated (rather than some other change, eg. addition of lockable
                // aspect only)
                ...
                for (QName changedProperty : changedProperties) {
                    ...
                    if (propertyDef != null) {
                        if (propertyDef
                                .getContainerClass()
                                .getName()
                                .equals(ContentModel.TYPE_CONTENT)) {
                            commentUpdated = true;
                            break;
                        }
                    }
    ...
    public void beforeDeleteNode(final NodeRef commentNodeRef) {
        ...
        if (discussableNodeRef != null) {
            ...
            if (behaviourFilter.isEnabled(
                            ContentModel.ASPECT_LOCKABLE) // eg. delete site (MNT-14671)
                    && isWorkingCopyOrLocked(discussableNodeRef)
                    && !isLockOwner(discussableNodeRef)
                    && canDelete) {
                throw new NodeLockedException(discussableNodeRef);

...
public class PeopleImpl implements People {
  ...
  public void setResetPasswordService(
  ...
  protected void processPersonProperties(
    String userName,
    final Map<QName, Serializable> nodeProps
  ) {
    ...
    final ContentData personDescription = (ContentData) nodeProps.get(
      ...
      AuthenticationUtil.runAsSystem(
        new RunAsWork<Void>() {
          ...
          public Void doWork() throws Exception {
            ContentReader reader = contentService.getRawReader(
              personDescription.getContentUrl()
            );
  ...
  private NodeRef getAvatarOriginal(NodeRef personNode) {
    ...
    List<ChildAssociationRef> avatarChildAssocs = nodeService.getChildAssocs(
      personNode,
      Collections.singleton(ContentModel.ASSOC_PREFERENCE_IMAGE)
    );
    if (avatarChildAssocs.size() > 0) {
      ...
      List<AssociationRef> avatorAssocs = nodeService.getTargetAssocs(
        personNode,
  ...
  @Override
  public BinaryResource downloadAvatarContent(
    String personId,
    ...
    return renditions.getContentNoValidation(
      avatarNodeRef,
      "avatar",
      parameters
    );
  ...
  public Person uploadAvatarContent(
    ...
    QName origAvatarQName = QName.createQName(
      NamespaceService.CONTENT_MODEL_1_0_URI,
      "origAvatar"
    );
    ...
    Node n = nodes.updateContent(
      avatarOriginalNodeId,
      contentInfo,
      stream,
      parameters
    );
  ...
  @Override
  public CollectionWithPagingInfo<Person> getPeople(
    ...
    // please see REPO-555
    final PagingResults<PersonService.PersonInfo> pagingResult = personService.getPeople(
      null,
      null,
      sortProps,
      pagingRequest
    ...
    List<Person> people = new AbstractList<Person>() {
      ...
      public Person get(int index) {
        ...
        Person person = getPersonWithProperties(
    ...
    if ((sortCols != null) && (sortCols.size() > 0)) {
      for (SortColumn sortCol : sortCols) {
        ...
        if (sortPropQName == null) {
          throw new InvalidArgumentException(
            "Invalid sort field: " + sortCol.column
          );
        }
        sortProps.add(
          new Pair<>(
            sortPropQName,
            (sortCol.asc ? Boolean.TRUE : Boolean.FALSE)
          )
        );
      }
    } else {
  ...
  private Person getPersonWithProperties(
    String personId,
    List<String> include
  ) {
    Person person = null;
    ...
    if (personNode != null) {
      Map<QName, Serializable> nodeProps = nodeService.getProperties(
        personNode
      );
      ...
      if (include.contains(PARAM_INCLUDE_PROPERTIES)) {
        ...
        Map<String, Object> custProps = nodes.mapFromNodeProperties(
          nodeProps,
          new ArrayList<>(),
          new HashMap<>(),
          EXCLUDED_NS,
  ...
  public Person create(Person person) {
    ...
    mas.createAuthentication(
      person.getUserName(),
      person.getPassword().toCharArray()
    );
   ...
   * @param nodeRef
   */
  private void savePersonDescription(
  ...
  private void validateCreatePersonData(Person person) {
    ...
    nodes.validateAspects(
      person.getAspectNames(),
      EXCLUDED_NS,
      EXCLUDED_ASPECTS
    );
    nodes.validateProperties(
  ...
  public Person update(String personId, final Person person) {
    ...
    if (person.isEnabled() != null) {
      if (isAdminAuthority(personIdToUpdate)) {
        throw new PermissionDeniedException(
          "Admin authority cannot be disabled."
        );
      }
    ...
    Set<QName> immutableProperties = userRegistrySynchronizer.getPersonMappedProperties(
      personIdToUpdate
    );
  ...
  private void validateUpdatePersonData(Person person) {
    ...
    if (
      person.wasSet(ContentModel.PROP_ENABLED) && (person.isEnabled() == null)
    ) {
      throw new IllegalArgumentException("'enabled' field cannot be empty.");
    }

    if (
      person.wasSet(ContentModel.PROP_EMAIL_FEED_DISABLED) &&
  ...
  private void updatePassword(
    ...
    if (isPassword || isOldPassword) {
      if (
        isOldPassword &&
        (
          (person.getOldPassword() == null) ||
          (person.getOldPassword().isEmpty())
        )
      ) {
        throw new IllegalArgumentException(
      ...
      if (!isAdmin) {
        ...
        if (!isOldPassword) {
          throw new IllegalArgumentException(
            "To change password, both 'oldPassword' and 'password' fields are required."
          );
        }
        ...
        mutableAuthenticationService.setAuthentication(
          personIdToUpdate,
          newPassword
        );
      }
  ...
  private boolean isMutableAuthority(String authorityName) {
    ...
    if (
      currentUser.equals(authorityName) ||

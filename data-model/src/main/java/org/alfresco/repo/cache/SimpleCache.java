 ...
 * <p>
 * All implementations <b>must</b> be thread-safe. Additionally, the use of the <tt>Serializable</tt> for both keys and values ensures that the underlying cache implementations can support both clustered caches as well as persistent caches.
 * <p>
...
public interface SimpleCache<K extends Serializable, V extends Object>
{
     ...
     * @param key
     *            K
     * @return Returns the value associated with the key. It will be <tt>null</tt> if the value is <tt>null</tt> or if the cache doesn't have an entry.
     */
    public V get(K key);
     ...
     * <p/>
     * Be sure to use {@link #remove(Serializable) remove} if cache entries need to be removed as the cache implementations must treat a <tt>null</tt> value as a first class object in exactly the same way as a {@link Map} will allow storage and retrieval of <tt>null</tt> values.
     * 
     * @param key

...
public class AddToHoldsTests extends BaseRMRestTest {
    ...
    private static final String ACCESS_DENIED_ERROR_MESSAGE =
            "Access Denied.  You do not have the appropriate "
                    + "permissions to perform this operation.";
    private static final String INVALID_TYPE_ERROR_MESSAGE =
            "Items added to a hold must be either a record, a "
                    + "record folder or active content.";
    private static final String LOCKED_FILE_ERROR_MESSAGE =
    ...
    public void preconditionForAddContentToHold() {
        ...
        holdsAPI.addItemToHold(
                ...
                documentHeld.getNodeRefWithoutVersion(),
                HOLD);

        STEP("Create users");
        userAddHoldPermission =
    ...
    /**
     * Given a hold that contains at least one active content When I use the existing REST API to
     * retrieve the contents of the hold Then I should see all the active content on hold
     */
    @Test
    public void retrieveTheContentOfTheHoldUsingV1API() throws Exception {
        STEP(
                "Retrieve the list of children from the hold and collect the entries that have the"
                        + " name of the active content held");
        List<RestNodeModel> documentsHeld =
                restClient
                        ...
                        .getEntries()
                        .stream()
                        .filter(child -> child.onModel().getName().contains(documentHeld.getName()))
                        .collect(Collectors.toList());
        STEP("Check the list of active content");
        assertEquals(
                documentsHeld.size(),
                1,
                "The active content is not retrieve when getting the children from the "
                        + "hold folder");
    ...
    /**
     * Given a hold that contains at least one active content When I use the existing REST API to
     * retrieve the holds the content is added Then the hold where the content held is returned
     */
    @Test
    public void retrieveTheHoldWhereTheContentIsAdded() {
        List<HoldEntry> holdEntries =
                holdsAPI.getHolds(
                        getAdminUser().getUsername(),
                        getAdminUser().getPassword(),
                        documentHeld.getNodeRefWithoutVersion(),
                        ...
                        null);
        assertTrue(
                holdEntries.stream().anyMatch(holdEntry -> holdEntry.getName().contains(HOLD)),
    ...
    public Object[][] getValidNodesForAddToHold() {
        ...
        getRestAPIFactory()
                .getRMUserAPI()
                .addUserPermission(recordFolder.getId(), userAddHoldPermission, PERMISSION_FILING);

        RecordCategoryChild folderToHold = createCategoryFolderInFilePlan();
        getRestAPIFactory()
                .getRMUserAPI()
                .addUserPermission(folderToHold.getId(), userAddHoldPermission, PERMISSION_FILING);
    ...
    /**
     * Given record folder/record/document not on hold And a hold And file permission on the hold
     * And the appropriate capability to add to hold When I use the existing REST API to add the
     * node to the hold Then the record folder/record/document is added to the hold And the item is
     * frozen
     *
     * @throws Exception
     */
    ...
    public void addValidNodesToHoldWithAllowedUser(String nodeId) throws Exception {
        ...
        holdsAPI.addItemToHold(
                userAddHoldPermission.getUsername(),
                userAddHoldPermission.getPassword(),
                nodeId,
                HOLD);
    ...
    /**
     * Data provider with user without correct permission to add to hold and the node ref to be
     * added to hold
     *
    ...
    public Object[][] getUserWithoutPermissionForAddToHold() {
        ...
        UserModel user =
                roleService.createUserWithRMRoleAndRMNodePermission(
                        ROLE_RM_MANAGER.roleId, recordFolder.getId(), PERMISSION_READ_RECORDS);
        ...
        return new Object[][] { // user without write permission on the content
            ...
            // user with write permission on the content, filling permission on a hold without add
            // to
            // hold capability
            {
                roleService.createUserWithSiteRoleRMRoleAndPermission(
    ...
    /**
     * Given a node not on hold And a hold And user without right permission to add to hold When I
     * use the existing REST API to add the node to the hold Then the node is not added to the hold
     * And the node is not frozen
     *
     * @throws Exception
     */
    @Test(dataProvider = "userWithoutPermissionForAddToHold")
    public void addContentToHoldWithUserWithoutHoldPermission(
            UserModel userModel, String nodeToBeAddedToHold) throws Exception {
        users.add(userModel);
        STEP("Add the node to the hold with user without permission.");
        String response =
                holdsAPI.addToHoldAndGetMessage(
    ...
    public Object[][] getInvalidNodesForAddToHold() {
        ...
        return new Object[][] { // file plan node id
            ...
            // folder,
            {
                dataContent.usingAdmin().usingSite(testSite).createFolder().getNodeRef(),
                SC_BAD_REQUEST,
                INVALID_TYPE_ERROR_MESSAGE
            },
            // document locked
            {
                contentLocked.getNodeRefWithoutVersion(),
                SC_INTERNAL_SERVER_ERROR,
    ...
    /**
     * Given a node that is not a document/record/ record folder ( a valid node type to be added to
     * hold) And a hold And user without right permission to add to hold When I use the existing
     * REST API to add the node to the hold Then the node is not added to the hold And the node is
     * not frozen
     *
     * @throws Exception
     */
    @Test(dataProvider = "invalidNodesForAddToHold")
    public void addInvalidNodesToHold(String itemNodeRef, int responseCode, String errorMessage)
            throws Exception {
        ...
        String responseErrorMessage =
                holdsAPI.addToHoldAndGetMessage(
                        ...
                        holdNodeRef);
        assertTrue(
                responseErrorMessage.contains(errorMessage),

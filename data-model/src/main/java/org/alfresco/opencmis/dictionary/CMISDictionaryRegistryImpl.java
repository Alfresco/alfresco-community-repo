...
public class CMISDictionaryRegistryImpl implements CMISDictionaryRegistry {
  ...
  protected static final Log logger = LogFactory.getLog(
    CMISDictionaryRegistryImpl.class
  );
  ...
  public CMISDictionaryRegistryImpl(
  ...
  @Override
  public TypeDefinitionWrapper getTypeDefByTypeId(
    String typeId,
    ...
    if (classDef != null) {
      ...
      if (defaultAspects != null && defaultAspects.size() > 0) {
        ...
        for (AspectDefinition aspectDef : defaultAspects) {
          ...
          TypeDefinitionWrapper aspectType = getTypeDefByQName(
            cmisMapping.getCmisType(aspectQName)
          );
          if (aspectType == null) {
            continue;
          }

          mandatoryAspectsExtensions.add(
  ...
  public void addModel(CompiledModel model) {
    Collection<AbstractTypeDefinitionWrapper> types = dictionaryInitializer.createDefinitions(
      this,
      model
    );
    addTypes(types);
  ...
  private void addTypes(Collection<AbstractTypeDefinitionWrapper> types) {
    ...
    for (AbstractTypeDefinitionWrapper objectTypeDef : types) {
      List<TypeDefinitionWrapper> children = objectTypeDef.connectParentAndSubTypes(
        cmisMapping,
        this,
        dictionaryService
      );
    ...
    for (AbstractTypeDefinitionWrapper typeDef : types) {
      if (
        typeDef.getTypeDefinition(false).getParentTypeId() == null ||
  ...
  public void init() {
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
        "Initialized CMIS Dictionary " +
        cmisMapping.getCmisVersion() +
        " tenant " +
        tenant +
        " in " +
        (end - start) +
        "ms. Types:" +
        typeDefsByTypeId.size() +
        ", Base Types:" +
        baseTypes.size()
      );
    }
  ...
  public void registerTypeDefinition(AbstractTypeDefinitionWrapper typeDef) {
    TypeDefinitionWrapper existingTypeDef = typeDefsByTypeId.get(
      typeDef.getTypeId()
    );
    ...
    if (typeQName != null) {
      if (
        (typeDef instanceof RelationshipTypeDefintionWrapper) &&
        !typeDef.isBaseType()
      ) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        ...
        typeDef.getTypeId() +
        " (scope=" +
        typeDef.getBaseTypeId() +
        ")"
      );
      logger.debug(" QName: " + typeDef.getAlfrescoName());
      logger.debug(
        " Table: " + typeDef.getTypeDefinition(false).getQueryName()
      );
      logger.debug(
        " Action Evaluators: " + typeDef.getActionEvaluators().size()
      );
    }
  }
  ...
  public void registerPropertyDefinitions(
    AbstractTypeDefinitionWrapper typeDef
  ) {

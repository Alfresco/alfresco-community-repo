...
/**
 * This class wires up all the active {@link org.alfresco.filesys.auth.ftp.FTPAuthenticatorBase}
 * beans in a chain.
 *
 * @author alex.mukha
 * @since 4.2.1
    ...
    protected List<FTPAuthenticatorBase> getUsableFtpAuthenticators() {
        ...
        for (String instance : this.applicationContextManager.getInstanceIds()) {
            try {
                ...
                FTPAuthenticatorBase authenticator =
                        (FTPAuthenticatorBase) context.getBean(sourceBeanName);

                if (this.getClass().isInstance(authenticator)) {
                    continue;
                }
                // Only add active authenticators. E.g. we might have a FTP authenticator that is
                // disabled.

...
public class ScriptRenditionService extends BaseScopableProcessorExtension {
  ...
  private RenditionDefinition loadRenditionDefinitionImpl(
    String shortOrLongFormQName
  ) {
  ...
  public ScriptRenditionDefinition createRenditionDefinition(
    ...
    if (logger.isDebugEnabled()) {
      ...
      msg
        .append("Creating ScriptRenditionDefinition [")
    ...
    RenderingEngineDefinition engineDefinition = renditionService.getRenderingEngineDefinition(
      renderingEngineName
    );
    RenditionDefinition rendDef = renditionService.createRenditionDefinition(
      renditionQName,
      renderingEngineName
    );
  ...
  public ScriptNode render(ScriptNode sourceNode, String renditionDefQName) {
    ...
    RenditionDefinition rendDef = loadRenditionDefinitionImpl(
      renditionDefQName
    );
    ChildAssociationRef result =
      this.renditionService.render(sourceNode.getNodeRef(), rendDef);
  ...
  public ScriptNode[] getRenditions(ScriptNode node) {
    ...
    for (int i = 0; i < renditions.size(); i++) {
      renditionObjs[i] =
        new ScriptNode(renditions.get(i).getChildRef(), serviceRegistry);
    }
  ...
  public ScriptNode getRenditionByName(ScriptNode node, String renditionName) {
    ...
    return result == null
      ? null
      : new ScriptNode(result.getChildRef(), serviceRegistry);
  }
  ...
  private QName createQName(String qnameString) {
    ...
    if (qnameString.startsWith(Character.toString(QName.NAMESPACE_BEGIN))) {
      ...
      result =
        QName.createQName(qnameString, serviceRegistry.getNamespaceService());
    }
    return result;
  }
}

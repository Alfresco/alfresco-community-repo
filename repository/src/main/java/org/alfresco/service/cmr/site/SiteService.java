 ...
 *
 * <p>This service API is designed to support the public facing Site APIs
 *
...
public interface SiteService {
    ...
    public interface SiteMembersCallback {
        ...
        /**
         * @deprecated from 7.0.0 onwards, use #siteMember(String, String, boolean) instead A site
         *     member along with his/her Site permission
         */
         ...
         */
        public default void siteMember(
                String authority, String permission, boolean isMemberOfGroup) {
     ...
     * @deprecated since version 3.2, replaced by {@link #createSite(String, String, String, String,
     *     SiteVisibility)}
     */
    ...
    /**
     * Can the current user add the authority "authorityName" to the site "shortName" with role
     * "role"?
     *
     * @param shortName site short name, must be unique
    ...
    /**
     * This method will find all {@link SiteInfo sites} available to the currently authenticated
     * user based on the specified site filter, site preset filter and result set size. The filter
     * parameter will match any sites whose {@link ContentModel#PROP_NAME cm:name}, {@link
     * ContentModel#PROP_TITLE cm:title} or {@link ContentModel#PROP_DESCRIPTION cm:description}
     * <i>contain</i> the specified string (ignoring case).
     *
     * <p>Note that this method uses <a href="http://wiki.alfresco.com/wiki/Search">Alfresco Full
     * Text Search</a> to retrieve results and depending on server Lucene, SOLR configuration may
     * only offer eventually consistent results.
     *
     * @param filter Any supplied filter will be wrapped in asterisks (e.g. '*foo*') and used to
     *     match the sites' cm:name, cm:title or cm:description.
    ...
    /**
     * This method will find all {@link SiteInfo sites} available to the currently authenticated
     * user based on the specified site filter and result set size. The filter parameter will match
     * any sites whose {@link ContentModel#PROP_NAME cm:name}, {@link ContentModel#PROP_TITLE
     * cm:title} or {@link ContentModel#PROP_DESCRIPTION cm:description} <i>contain</i> the
    ...
    /**
     * List the available sites. This list can optionally be filtered by site name/title/description
     * and/or site preset.
     *
     * <p>Note: Starting with Alfresco 4.0, the filter parameter will only match sites whose {@link
     * ContentModel#PROP_NAME cm:name} or {@link ContentModel#PROP_TITLE cm:title} or {@link
     * ContentModel#PROP_DESCRIPTION cm:description} <i>start with</i> the specified string
     * (ignoring case). The listing of sites whose cm:names (or titles or descriptions)
     * <i>contain</i> the specified string is no longer supported. To retrieve sites whose cm:names
     * etc contain a substring, {@link SiteService#findSites(String, String, int)} should be used
     * instead.
     *
     * <p><b>THIS METHOD CAN RETURN INCOMPLETE RESULTS WHILE CACHES CATCH UP WITH REALITY</b> (<a
     * href=https://issues.alfresco.com/jira/browse/ACE-196>BM-0012: Run v420b1494_01: (CMIS)
     * GetSites is Slow</a>).
     *
    ...
    /**
     * This method returns {@link PagingResults paged result sets} of {@link SiteInfo} objects,
     * which should be more efficient than the unpaged methods also available on this interface. It
     * is also guaranteed to return fully consistent results.
     *
     ...
     */
    @NotAuditable
    PagingResults<SiteInfo> listSites(
     ...
     *
     * <p>Returns null if the site can not be found.
     *
    ...
    /**
     * This method gets the {@link SiteInfo} for the Share Site which contains the given NodeRef. If
     * the given NodeRef is not contained within a Share Site, then <code>null</code> is returned.
     *
     * @param nodeRef the node whose containing site's info is to be found.
     * @return SiteInfo site information for the containing site or <code>null</code> if node is not
     *     in a site.
     */
    @NotAuditable
    ...
    /**
     * This method gets the shortName for the Share Site which contains the given NodeRef. If the
     * given NodeRef is not contained within a Share Site, then <code>null</code> is returned.
     *
     * @param nodeRef the node whose containing site's info is to be found.
     * @return String site short name for the containing site or <code>null</code> if node is not in
     *     a site.
     */
    @NotAuditable
    ...
    /**
     * Returns true if the site exists. This allows create scripts to confirm the existence of
     * private sites - they would not normally be returned from getSite() if the user does not have
     * permission on the site noderef.
     *
     * @param shortName the site short name
     ...
     *
     * <p>Note that the short name and site preset of a site can not be updated once the site has
     * been created.
     *
    ...
    /**
     * @deprecated from 7.0.0, use #listMembers(String, String, String, boolean, boolean, boolean,
     *     SiteMembersCallback) instead List the members of the site. This includes both users and
     *     groups.
     *     <p>Name and role filters are optional and if not specified all the members of the site
     *     are returned.
     * @param shortName site short name
    ...
    /**
     * List the members of the site. This includes both users and groups. Users and groups can be
     * controlled by passing params
     *
     * <p>Name and role filters are optional and if not specified all the members of the site are
     * returned.
     *
     * @param shortName site short name
     * @param nameFilter name filter
    ...
    /**
     * List the members of the site. This includes both users and groups if collapseGroups is set to
     * false, otherwise all groups that are members are collapsed into their component users and
     * listed.
     *
     * @param shortName site short name
     * @param nameFilter name filter
     ...
     */
    @NotAuditable
    List<SiteMemberInfo> listMembersInfo(
            String shortName,
            String nameFilter,
            String roleFilter,
            int size,
    ...
    /**
     * Gets the role of the specified user. Returns a paged list of the members of the site. This
     * includes both users and groups if collapseGroups is set to false, otherwise all groups that
     * are members are collapsed into their component users and listed.
     *
     * @param shortName site short name
     ...
     */
    @NotAuditable
    PagingResults<SiteMembership> listMembersPaged(
            String shortName,
            boolean collapseGroups,
    ...
    /**
     * Creates a container for a component is a site of the given container type (must be a sub-type
     * of st:siteContainer)
     *
     * <p>If no container type is specified then a node of type st:siteContainer is created.
     *
     * <p>The map of container properties are set on the created container node. Null can be
     * provided when no properties need to be set.
     *
     * @param shortName site short name
     ...
     * This will generally only differ from {@link #getSiteRoles()} if your site is of a custom
     * type.
     *
     * @return list of available roles
     */
    ...
    /**
     * Gets the sites role group. All members assigned the given role will be members of the
     * returned group.
     *
     * @param shortName site short name
     ...
     */
    @NotAuditable
    PagingResults<SiteMembership> listSitesPaged(
     ...
     *
     * <p>Note: The super/repo admin is considered to be a site administrator too.
     *
     ...
     */
    @NotAuditable
    PagingResults<SiteGroupMembership> listGroupMembersPaged(
            String shortName,

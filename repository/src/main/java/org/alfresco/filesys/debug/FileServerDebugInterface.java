...
public class FileServerDebugInterface implements DebugInterface {
  ...
  public void debugPrintln(String str, int level) {
    ...
    if (level <= getLogLevel()) if (m_printBuf.length() > 0) {
  ...
  public int getLogLevel() {
    ...
    if (logger.isDebugEnabled()) logLevel = Debug.Debug; else if (
      logger.isInfoEnabled()
    ) logLevel = Debug.Info; else if (logger.isWarnEnabled()) logLevel =
      Debug.Warn; else if (logger.isErrorEnabled()) logLevel =
      Debug.Error; else if (logger.isFatalEnabled()) logLevel = Debug.Fatal;

...
public class PatchTest extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    authenticationContext =
      (AuthenticationContext) ctx.getBean("authenticationContext");
  ...
  public void testApplyOutstandingPatches() throws Exception {
    ...
    for (AppliedPatch appliedPatch : appliedPatches) {
      if (appliedPatch.getId().equals("Sample01")) {
        found01 = true;
        assertTrue(
          "Patch info didn't indicate success: " + appliedPatch,
          appliedPatch.getSucceeded()
        );
  ...
  public void testGetPatchesByDate() throws Exception {
    ...
    List<AppliedPatch> appliedPatches = patchService.getPatches(null, null);
    assertTrue(
      "Expected at least 2 applied patches",
      appliedPatches.size() >= 2
    );

    // now requery using null dates
    List<AppliedPatch> appliedPatchesAllDates = patchService.getPatches(
      null,
      null
    );
    assertEquals(
      "Applied patches by all dates doesn't match all applied patches",
      appliedPatches.size(),
      appliedPatchesAllDates.size()
    );

    // perform another query with dates that should return no results
    List<AppliedPatch> appliedPatchesFutureDates = patchService.getPatches(
      new Date(),
      new Date()
    );
    assertEquals(
      "Query returned results for dates when no patches should exist",
      0,
      appliedPatchesFutureDates.size()
    );
  }
}

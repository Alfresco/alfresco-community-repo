...
/**
 * Basic HTTP web authentication implementation. Main purpose to use as fallback authentication with
 * SSO filters.
 *
...
public class SSOFallbackBasicAuthenticationDriver implements AuthenticationDriver {
    ...
    public boolean authenticateRequest(
        ...
        SessionUser user =
                session == null ? null : (SessionUser) session.getAttribute(userAttributeName);
        if (user == null) {
            if (authHdr != null
                    && authHdr.length() > 5
                ...
                try {
                    ...
                    final RetryingTransactionCallback<SessionUser> callback =
                            new RetryingTransactionCallback<SessionUser>() {
                                @Override
                    ...
                    user =
                            AuthenticationUtil.runAs(
                                    new AuthenticationUtil.RunAsWork<SessionUser>() {

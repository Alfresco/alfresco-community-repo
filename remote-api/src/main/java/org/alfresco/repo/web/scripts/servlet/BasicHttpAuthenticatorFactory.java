  ...
  public class BasicHttpAuthenticator implements Authenticator {
    ...
    public boolean authenticate(
      ...
      if (logger.isDebugEnabled()) {
        ...
        logger.debug(
          "URL ticket provided: " + (ticket != null && ticket.length() > 0)
        );
      }
      ...
      // authenticate as specified by HTTP Basic Authentication
        ...
        if (!authorizationParts[0].equalsIgnoreCase("basic")) {
          throw new WebScriptException(
            401,
            "Authorization '" + authorizationParts[0] + "' not supported."
          );
        }

        String decodedAuthorisation = new String(
        ...
        try {
          if (auth.isTicket()) {
            if (logger.isDebugEnabled()) logger.debug(
              "Authenticating (BASIC HTTP) ticket " + auth.getTicket()
            );
            ...
            if (logger.isDebugEnabled()) logger.debug(
              "Authenticating (BASIC HTTP) user " + auth.getUserName()
            );
      ...
      if (!authorized) {
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "Requesting authorization credentials"
        );
    ...
    private boolean isTicketValid() {
      try {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "User ticket is not valid. Passing to the Basic authentication handling. Reqeust information:\n" +
            "    ticket: " +
            ticket +
            "\n" +
            "    request: " +

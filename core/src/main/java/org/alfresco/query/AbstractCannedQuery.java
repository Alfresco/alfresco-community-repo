...
public abstract class AbstractCannedQuery<R> implements CannedQuery<R> {
  ...
  public String toString() {
    return (
      "AbstractCannedQuery [parameters=" +
      parameters +
      ", class=" +
      this.getClass() +
      "]"
  ...
  public final synchronized CannedQueryResults<R> execute() {
    ...
    if (isApplyPostQuerySorting()) {
      rawResults =
        applyPostQuerySorting(rawResults, parameters.getSortDetails());
    }
   ...
   */
  protected List<R> applyPostQuerySorting(
    List<R> results,
   ...
   */
  protected List<R> applyPostQueryPermissions(
    List<R> results,
    int requestedCount
  ) {
    throw new UnsupportedOperationException(

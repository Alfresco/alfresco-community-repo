...
public class UpgradePasswordHashTest extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    SimpleCache<String, RepositoryAuthenticationDao.CacheEntry> authenticationCache = (SimpleCache<String, RepositoryAuthenticationDao.CacheEntry>) ctx.getBean(
      "authenticationCache"
    );
    SimpleCache<String, NodeRef> immutableSingletonCache = (SimpleCache<String, NodeRef>) ctx.getBean(
    ...
    PolicyComponent policyComponent = (PolicyComponent) ctx.getBean(
      "policyComponent"
    );

    repositoryAuthenticationDao = new RepositoryAuthenticationDao();
    repositoryAuthenticationDao.setTransactionService(
      serviceRegistry.getTransactionService()
    );
    repositoryAuthenticationDao.setAuthorityService(
    ...
    repositoryAuthenticationDao.setNamespaceService(
      serviceRegistry.getNamespaceService()
    );
    repositoryAuthenticationDao.setCompositePasswordEncoder(
      compositePasswordEncoder
    );
    ...
    upgradePasswordHashWorker =
      (UpgradePasswordHashWorker) ctx.getBean("upgradePasswordHashWorker");
  ...
  protected void createTestUsers(String encoding) throws Exception {
    ...
    testUsers.add(
      createUser("king" + encoding, "king".toCharArray(), encoding)
    );
    testUsers.add(createUser("kin" + encoding, "Kong".toCharArray(), encoding));
    testUsers.add(
      createUser("ding" + encoding, "dong".toCharArray(), encoding)
    );
    testUsers.add(
      createUser("ping" + encoding, "pong".toCharArray(), encoding)
    );
  ...
  private NodeRef createUser(
    ...
    NodeRef userNodeRef = repositoryAuthenticationDao.getUserOrNull(
      caseSensitiveUserName
    );
    if (userNodeRef == null) {
      throw new AuthenticationException(
        "User name does not exist: " + caseSensitiveUserName
      );
    }
  ...
  private void runWorker(List<String> expectedEncoding) throws Exception {
    //set preferred
    compositePasswordEncoder.setPreferredEncoding(
      expectedEncoding.get(expectedEncoding.size() - 1)
    );
    this.upgradePasswordHashWorker.setCompositePasswordEncoder(
        compositePasswordEncoder
      );
    ...
    for (NodeRef testUser : testUsers) {
      assertNull(
        "The hash indicator should not be set",
        nodeService.getProperty(testUser, ContentModel.PROP_HASH_INDICATOR)
      );
      assertNull(
        "The password hash should not be set",
        nodeService.getProperty(testUser, ContentModel.PROP_PASSWORD_HASH)
      );
    }
    ...
    for (NodeRef testUser : testUsers) {
      assertNotNull(
        "The password hash should be set",
        nodeService.getProperty(testUser, ContentModel.PROP_PASSWORD_HASH)
      );
      assertEquals(
        expectedEncoding,
      ...
      assertNull(
        "The md4 password should not be set",
        nodeService.getProperty(testUser, ContentModel.PROP_PASSWORD)
      );
      assertNull(
        "The sh256 password should not be set",
        nodeService.getProperty(testUser, ContentModel.PROP_PASSWORD_SHA256)
      );
    }
  }
}

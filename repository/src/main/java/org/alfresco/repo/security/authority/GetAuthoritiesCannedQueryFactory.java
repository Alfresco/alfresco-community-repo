...
public class GetAuthoritiesCannedQueryFactory extends AbstractCannedQueryFactory<AuthorityInfo> {
    ...
    public CannedQuery<AuthorityInfo> getCannedQuery(CannedQueryParameters parameters) {
        return (CannedQuery<AuthorityInfo>)
                new GetAuthoritiesCannedQuery(
                        cannedQueryDAO, tenantService, methodSecurity, parameters);
    }
    ...
    public CannedQuery<AuthorityInfo> getCannedQuery(
        ...
        Long qnameAuthDisplayNameId =
                Long.MIN_VALUE; // We query but using a value that won't return results
        Pair<Long, QName> qnameAuthDisplayNamePair =
                qnameDAO.getQName(ContentModel.PROP_AUTHORITY_DISPLAY_NAME);
        if (qnameAuthDisplayNamePair != null) {
            qnameAuthDisplayNameId = qnameAuthDisplayNamePair.getFirst();
        }

        // this can be null, in which case, there is no filtering on type, done at the database
        // level
        ...
        if (sortBy != null) {
            ...
            sortPairs.add(
                    new Pair<String, SortOrder>(
                            sortBy, (sortAscending ? SortOrder.ASCENDING : SortOrder.DESCENDING)));

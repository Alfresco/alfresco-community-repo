...
/**
 * Holds a FormProcessor implementation for each of the types of form that can be processed. By
 * default a node, type, task, and workflow form processor are available.
 *
 * <p>Given an item the registry selects the relevant form processor, the match is performed via
 * pattern matching on the supplied string.
 *
...
public class FormProcessorRegistry {
    ...
    public void addProcessor(FormProcessor processor) {
        if (processor.isActive()) {
            ...
            if (logger.isDebugEnabled()) logger.debug("Registered processor: " + processor);
        } else if (logger.isWarnEnabled()) {
            logger.warn(
                    "Ignored registration of processor "
                            + processor
                            + "as it was marked as inactive");
        }
    }
     ...
     *
     * <p>Each registered processors is asked if it is applicable for the given item, the first
     * processor to positively respond that is also active is selected and returned.
     *

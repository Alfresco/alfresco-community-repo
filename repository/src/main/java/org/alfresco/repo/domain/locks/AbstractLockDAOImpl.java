...
public abstract class AbstractLockDAOImpl implements LockDAO {
    ...
    public void getLock(QName lockQName, String lockToken, long timeToLive) {
        ...
        LockResourceEntity exclusiveLockResource =
                getLockResource(qnameNamespaceId, qnameLocalName);
        ...
        for (LockEntity existingLock : existingLocks) {
            boolean canTakeLock =
                    canTakeLock(existingLock, lockToken, requiredExclusiveLockResourceId);
            if (!canTakeLock) {
     ...
     * well.
     *
     * @param optimistic <tt>true</tt> if a mismatch in the number of locked rows should be ignored.
     * @return <tt>true</tt> if the lock was successfully and fully updated using the lock token
     *     provided
    ...
    private boolean updateLocks(
        ...
        if (exclusiveLockResource == null) {
            ...
            throw new LockAcquisitionException(
                    LockAcquisitionException.ERR_LOCK_RESOURCE_MISSING, lockQName, lockToken);
        }
        ...
        if (updateCount != requiredUpdateCount) {
            ...
            if (LOCK_TOKEN_RELEASED.equals(newLockToken)) {
                throw new LockAcquisitionException(
                        LockAcquisitionException.ERR_FAILED_TO_RELEASE_LOCK, lockQName, lockToken);
            } else {
                throw new LockAcquisitionException(
                        ...
                        new Integer(requiredUpdateCount));
            }
        } else {
    ...
    private boolean canTakeLock(
            LockEntity existingLock, String lockToken, Long desiredExclusiveLock) {
     ...
     * @return Returns the lock resource entity, or <tt>null</tt> if it doesn't exist
     */
    protected abstract LockResourceEntity getLockResource(
            Long qnameNamespaceId, String qnameLocalName);
     ...
     */
    protected abstract LockResourceEntity createLockResource(
            Long qnameNamespaceId, String qnameLocalName);
     ...
     * may be expired.
     *
     * @param sharedLockResourceIds a list of shared resource IDs for which to retrieve the current
     *     locks
     * @return Returns a list of locks (expired or not) for the given lock resources
     */
    protected abstract List<LockEntity> getLocksBySharedResourceIds(
    ...
    protected abstract LockEntity updateLock(
            LockEntity lockEntity, String lockToken, long timeToLive);
    ...
    /**
     * Split a lock's qualified name into the component parts using the '.' (period) as a separator
     * on the localname. The namespace is preserved. The provided qualified name will always be the
     * last component in the returned list.
     *

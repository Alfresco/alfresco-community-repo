...
public abstract class AbstractContentTransformerTest extends TestCase {
    ...
    public static File loadNamedQuickTestFile(String quickname) throws IOException {
        ...
        URL url =
                AbstractContentTransformerTest.class.getClassLoader().getResource(quickNameAndPath);
        if (url == null) {
            return null;
        }
        if (ResourceUtils.isJarURL(url)) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Using a temp file for quick resource that's in a jar." + quickNameAndPath);
            }
            try {
                InputStream is =
                        AbstractContentTransformerTest.class
                                .getClassLoader()

...
public class SolrSQLHttpClient
  ...
  public ResultSet executeQuery(
    ...
    try {
      ...
      if (
        searchParameters.getTimezone() != null &&
        !searchParameters.getTimezone().isEmpty()
      ) {
        url
          .append("&TZ=")
          .append(encoder.encode(searchParameters.getTimezone(), "UTF-8"));
      }
      ...
      boolean includeGroups = includeGroupsForRoleAdmin
        ? true
      ...
      for (Locale currentLocale : searchParameters.getLocales()) {
        locales.put(
          DefaultTypeConverter.INSTANCE.convert(String.class, currentLocale)
        );
      }
      if (locales.length() == 0) {
  ...
  public void setUseDynamicShardRegistration(
    boolean useDynamicShardRegistration
  ) {

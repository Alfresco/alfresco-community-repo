...
public class EqualsHelperTest extends TestCase {
  ...
  public void testMapComparison() throws Exception {
    ...
    // LEFT_ONLY
    left.put(6, "A");
    Map<Integer, MapValueComparison> diff = EqualsHelper.getMapComparison(
      left,
      right
    );
    assertEquals("'EQUAL' check failed", MapValueComparison.EQUAL, diff.get(0));
    assertEquals(
      "'NOT_EQUAL' check failed",
      MapValueComparison.NOT_EQUAL,
      diff.get(1)
    );
    ...
    assertEquals(
      "'RIGHT_ONLY' check failed",
      MapValueComparison.RIGHT_ONLY,
      diff.get(5)
    );
    assertEquals(
      "'LEFT_ONLY' check failed",
      MapValueComparison.LEFT_ONLY,
      diff.get(6)
    );
  }
}

 ...
 */
/*package*/class ViewXMLExporter implements Exporter {
  ...
  ViewXMLExporter(
    ...
    PROPERTIES_QNAME =
      QName.createQName(
        NamespaceService.REPOSITORY_VIEW_PREFIX,
        PROPERTIES_LOCALNAME,
        namespaceService
      );
    ASSOCIATIONS_QNAME =
      QName.createQName(
        NamespaceService.REPOSITORY_VIEW_PREFIX,
        ASSOCIATIONS_LOCALNAME,
        namespaceService
      );
    DATATYPE_QNAME =
      QName.createQName(
    ...
    PERMISSION_QNAME =
      QName.createQName(
        NamespaceService.REPOSITORY_VIEW_PREFIX,
        PERMISSION_LOCALNAME,
        namespaceService
      );
    INHERITPERMISSIONS_QNAME =
      QName.createQName(
        NamespaceService.REPOSITORY_VIEW_PREFIX,
  ...
  public void start(ExporterContext context) {
    try {
      ...
      contentHandler.characters(
        context.getExportedBy().toCharArray(),
        0,
        context.getExportedBy().length()
      );
      ...
      String date = DefaultTypeConverter.INSTANCE.convert(
        String.class,
        context.getExportedDate()
      );
      ...
      contentHandler.characters(
        context.getExporterVersion().toCharArray(),
        0,
        context.getExporterVersion().length()
      );
      contentHandler.endElement(
        NamespaceService.REPOSITORY_VIEW_PREFIX,
        EXPORTERVERSION_LOCALNAME,
      ...
      for (int i = 0; i < exportList.length; i++) {
        ...
        contentHandler.characters(
          ArrayUtils.addAll(path.toCharArray(), ",".toCharArray()),
          0,
          path.length() + comma
        );
      }
  ...
  public void startNamespace(String prefix, String uri) {
    try {
      ...
      throw new ExporterException(
        "Failed to process start namespace event - prefix " +
        prefix +
        " uri " +
        uri,
        e
  ...
  public void endNamespace(String prefix) {
    try {
      ...
      throw new ExporterException(
        "Failed to process end namespace event - prefix " + prefix,
        e
      );
    }
  }
  ...
  public void startNode(NodeRef nodeRef) {
    try {
      ...
      throw new ExporterException(
        "Failed to process start node event - node ref " + nodeRef.toString(),
        e
      );
    }
  }
  ...
  public void endNode(NodeRef nodeRef) {
    try {
      ...
      throw new ExporterException(
        "Failed to process end node event - node ref " + nodeRef.toString(),
        e
      );
    }
  }
  ...
  public void startAspects(NodeRef nodeRef) {
    try {
      contentHandler.startElement(
        ASPECTS_QNAME.getNamespaceURI(),
        ASPECTS_LOCALNAME,
        toPrefixString(ASPECTS_QNAME),
        EMPTY_ATTRIBUTES
      );
    } catch (SAXException e) {
  ...
  public void startAspect(NodeRef nodeRef, QName aspect) {
    try {
      ...
      throw new ExporterException(
        "Failed to process start aspect event - node ref " +
        nodeRef.toString() +
        "; aspect " +
        toPrefixString(aspect),
        e
  ...
  public void startACL(NodeRef nodeRef) {
    try {
      ...
      contentHandler.startElement(
        ACL_QNAME.getNamespaceURI(),
        ACL_QNAME.getLocalName(),
        toPrefixString(ACL_QNAME),
        attrs
      );
    } catch (SAXException e) {
      throw new ExporterException(
  ...
  public void permission(NodeRef nodeRef, AccessPermission permission) {
    try {
      ...
      contentHandler.startElement(
        ACE_QNAME.getNamespaceURI(),
        ACE_QNAME.getLocalName(),
      ...
      contentHandler.characters(
        strPermission.toCharArray(),
        0,
        strPermission.length()
      );
      contentHandler.endElement(
        PERMISSION_QNAME.getNamespaceURI(),
        PERMISSION_QNAME.getLocalName(),
  ...
  public void startProperties(NodeRef nodeRef) {
    try {
      contentHandler.startElement(
        PROPERTIES_QNAME.getNamespaceURI(),
        PROPERTIES_LOCALNAME,
        toPrefixString(PROPERTIES_QNAME),
        EMPTY_ATTRIBUTES
      );
    } catch (SAXException e) {
  ...
  public void startProperty(NodeRef nodeRef, QName property) {
    try {
      QName encodedProperty = QName.createQName(
        property.getNamespaceURI(),
        ISO9075.encode(property.getLocalName())
      );
      contentHandler.startElement(
        encodedProperty.getNamespaceURI(),
        encodedProperty.getLocalName(),
        toPrefixString(encodedProperty),
        EMPTY_ATTRIBUTES
      );
    } catch (SAXException e) {
      throw new ExporterException(
  ...
  public void startValueCollection(NodeRef nodeRef, QName property) {
    try {
      ...
      contentHandler.startElement(
        NamespaceService.REPOSITORY_VIEW_PREFIX,
        VALUES_LOCALNAME,
        toPrefixString(VALUES_QNAME),
        EMPTY_ATTRIBUTES
      );
    } catch (SAXException e) {
      throw new ExporterException(
        "Failed to process start collection event - nodeRef " +
        nodeRef +
        "; property " +
        toPrefixString(property),
        e
  ...
  public void endValueCollection(NodeRef nodeRef, QName property) {
    try {
      ...
      throw new ExporterException(
        "Failed to process end collection event - nodeRef " +
        nodeRef +
        "; property " +
        toPrefixString(property),
        e
  ...
  public void value(NodeRef nodeRef, QName property, Object value, int index) {
    try {
      ...
      DataTypeDefinition dataTypeDef = (propDef == null)
        ? null
        : propDef.getDataType();
      if (
        dataTypeDef == null ||
        dataTypeDef.getName().equals(DataTypeDefinition.ANY)
      ) {
        dataTypeDef =
          (value == null)
            ? null
            : dictionaryService.getDataType(value.getClass());
        if (dataTypeDef != null) {
      ...
      boolean isMLText =
        (
          dataTypeDef != null &&
          dataTypeDef.getName().equals(DataTypeDefinition.MLTEXT)
        );

      // convert node references to paths
      if (
        value instanceof NodeRef && referenceType.equals(ReferenceType.PATHREF)
      ) {
        ...
        if (nodeRef.getStoreRef().equals(valueNodeRef.getStoreRef())) {
          ...
          if (property.equals(ContentModel.PROP_CATEGORIES)) {
            ...
            nodeRefPath =
              createPath(context.getExportParent(), nodeRef, valueNodeRef);
          }
          value =
            (nodeRefPath == null)
              ? null
              : nodeRefPath.toPrefixString(namespaceService);
        }
      }
      ...
      // output value
      String strValue = (String) DefaultTypeConverter.INSTANCE.convert(
        String.class,
        value
      );
      if (strValue != null) {
      ...
      if (
        (value == null || valueDataType != null || index != -1) && !isMLText
      ) {
        // only output for non MLTEXT values
        contentHandler.endElement(
          NamespaceService.REPOSITORY_VIEW_PREFIX,
  ...
  public void startAssocs(NodeRef nodeRef) {
    try {
      contentHandler.startElement(
        ASSOCIATIONS_QNAME.getNamespaceURI(),
        ASSOCIATIONS_LOCALNAME,
        toPrefixString(ASSOCIATIONS_QNAME),
        EMPTY_ATTRIBUTES
      );
    } catch (SAXException e) {
      ...
      if (relativePath == null) {
        ...
        if (i == rootPath.size()) {
          ...
          for (NodeRef nodeRef : context.getExportParentList()) {
            int j = 0;
            Path tryPath = createIndexedPath(
              nodeRef,
            ...
            while (
              j < tryPath.size() &&
              j < toPath.size() &&
              tryPath.get(j).equals(toPath.get(j))
            ) {
              j++;
            }
            if (j == tryPath.size()) {
  ...
  private Path createIndexedPath(NodeRef nodeRef, Path path) {
    ...
    for (int i = path.size() - 1; i >= 0; i--) {
      ...
      if (i > 0 && pathElement instanceof Path.ChildAssocElement) {
        ...
        List<NodeRef> siblings = searchService.selectNodes(
          nodeRef,
          searchPath,
          null,
          namespaceService,
        ...
        if (siblings.size() > 1) {
          ChildAssociationRef childAssoc =
            ((Path.ChildAssocElement) pathElement).getRef();

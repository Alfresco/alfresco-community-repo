...
public class VirtualFileFolderServiceExtensionTest extends VirtualizationIntegrationTest {
    ...
    public void testSearch() throws Exception {
        ...
        List<FileInfo> search =
                fileAndFolderService.search(
                        virtualFolder1NodeRef, "testfile.txt", true, false, true);
        // one physical file and one from node1
        assertEquals(2, search.size());

        search =
                fileAndFolderService.search(
                        virtualFolder1NodeRef, "testfile-1.txt", true, false, true);
        // one physical file and one from node2
        assertEquals(2, search.size());

        search =
                fileAndFolderService.search(
                        virtualFolder1NodeRef, "testfile.txt", true, false, false);

        assertEquals(1, search.size());

        search = fileAndFolderService.search(virtualFolder1NodeRef, null, true, false, true);
        assertEquals(4, search.size());

        search = fileAndFolderService.search(virtualFolder1NodeRef, null, true, false, false);

        assertEquals(2, search.size());

        search = fileAndFolderService.search(virtualFolder1NodeRef, null, false, true, false);

        assertEquals(2, search.size());

        search = fileAndFolderService.search(virtualFolder1NodeRef, null, false, true, true);
    ...
    public void testListVirtualizedVirtualChild() throws Exception {
        ...
        final String virtualizedChildFolderName =
                "TestVirtualFileFolderService_testVirtualFolderVirtualChild_CVF";
        // CM-533 Suppress options to create folders in a virtual folder (repo)
        // we programmatically replicate the process
        NodeRef virtulizedFolder =
                createVirtualizedFolder(
                        vf, virtualizedChildFolderName, TEST_TEMPLATE_3_JSON_SYS_PATH);
        nodeService.setProperty(
                virtulizedFolder, ContentModel.PROP_DESCRIPTION, "Node1_content_FR");

        NodeRef vfchildvf =
                nodeService.getChildByName(
                        node1, ContentModel.ASSOC_CHILDREN, virtualizedChildFolderName);

        assertNotNull("Virtual child expected.", vfchildvf);

        {
            List<FileInfo> vfchildvfList = fileAndFolderService.list(vfchildvf);
            assertContainsNames(vfchildvfList, "Node1", "Node2");
        }

        {
            PagingResults<FileInfo> vfchildvfResults =
                    fileAndFolderService.list(vfchildvf, null, null, null, new PagingRequest(100));
            List<FileInfo> page = vfchildvfResults.getPage();
            assertContainsNames(page, "Node1", "Node2");
        }

        {
            PagingResults<FileInfo> vfchildvfResults =
                    fileAndFolderService.list(
                            vfchildvf, true, true, null, null, null, new PagingRequest(100));
            List<FileInfo> page = vfchildvfResults.getPage();
            assertContainsNames(page, "Node1", "Node2");
        }
    }
    ...
    public void testListFileFolderDisjunction() throws Exception {
        ...
        properties.put(ContentModel.PROP_DESCRIPTION, "Node2_folder_FR");
        ChildAssociationRef createFolder = createFolder(vf, folderName, properties);

        final String contentName = "ContentVirtualChild";
        ChildAssociationRef createContent = createContent(node2, contentName);

        prepareMocks("Node2", createFolder.getChildRef());
        try {
            PagingResults<FileInfo> folderChildren =
                    fileAndFolderService.list(
                            node2,
                            false,
                            true,
            ...
            assertContainsNames(page, folderName);
            assertMissesNames(page, contentName);
        } finally {
            resetMocks();
        }
        ...
        try {
            PagingResults<FileInfo> contentChildren =
                    fileAndFolderService.list(
                            node2,
                            true,
                            false,
            ...
            assertMissesNames(page, folderName);
            assertContainsNames(page, contentName);
        } finally {
            resetMocks();
        }
    }
    ...
    public void testListWithIgnores() throws Exception {
        ...
        final String contentName = "ContentVirtualChild";
        NodeRef contentNodeRef = createContent(node2, contentName).getChildRef();
        ...
        PagingResults<FileInfo> node2List =
                fileAndFolderService.list(
                        node2, searchTypeQNames, ignoreAspectQNames, sortProps, pagingRequest);

        assertEquals(1, node2List.getPage().size());
    }
    ...
    public void testCreateTempFile() throws Exception {
        ...
        NodeRef childByName =
                nodeService.getChildByName(node2, ContentModel.ASSOC_CHILDREN, contentName);
    ...
    public void testRename() throws Exception {
        NodeRef vf =
                createVirtualizedFolder(
                        ...
                        "TestVirtualFileFolderService_testVirtualFolderVirtualChild",
                        TEST_TEMPLATE_3_JSON_SYS_PATH);
        NodeRef node2 = nodeService.getChildByName(vf, ContentModel.ASSOC_CONTAINS, "Node2");
        String contentName = "ContentVirtualChild";
        createContent(node2, contentName);
        NodeRef childByName =
                nodeService.getChildByName(node2, ContentModel.ASSOC_CONTAINS, contentName);
        // rename file in virtual context
        FileInfo renamedFileInfo = fileAndFolderService.rename(childByName, NEW_FILE_NAME_1);
        assertNotNull(renamedFileInfo);
        assertEquals(NEW_FILE_NAME_1, nodeService.getProperty(childByName, ContentModel.PROP_NAME));
        assertNull(nodeService.getChildByName(node2, ContentModel.ASSOC_CONTAINS, contentName));
        assertNull(nodeService.getChildByName(vf, ContentModel.ASSOC_CONTAINS, contentName));
        assertNotNull(
                nodeService.getChildByName(node2, ContentModel.ASSOC_CONTAINS, NEW_FILE_NAME_1));
        assertNotNull(nodeService.getChildByName(vf, ContentModel.ASSOC_CONTAINS, NEW_FILE_NAME_1));

        // rename physical file
        childByName = nodeService.getChildByName(vf, ContentModel.ASSOC_CONTAINS, NEW_FILE_NAME_1);
        renamedFileInfo = fileAndFolderService.rename(childByName, NEW_FILE_NAME_2);
        assertNotNull(renamedFileInfo);
        assertEquals(NEW_FILE_NAME_2, nodeService.getProperty(childByName, ContentModel.PROP_NAME));
        assertNull(nodeService.getChildByName(node2, ContentModel.ASSOC_CONTAINS, NEW_FILE_NAME_1));
        assertNull(nodeService.getChildByName(vf, ContentModel.ASSOC_CONTAINS, NEW_FILE_NAME_1));
        assertNotNull(
                nodeService.getChildByName(node2, ContentModel.ASSOC_CONTAINS, NEW_FILE_NAME_2));
        assertNotNull(nodeService.getChildByName(vf, ContentModel.ASSOC_CONTAINS, NEW_FILE_NAME_2));
    }
    ...
    public void testListNamePattern() throws Exception {
        ...
        NodeRef vf =
                createVirtualizedFolder(
                        ...
                        "TestVirtualFileFolderService_testVirtualFolderVirtualChild",
                        TEST_TEMPLATE_3_JSON_SYS_PATH);
        NodeRef node1 = nodeService.getChildByName(vf, ContentModel.ASSOC_CONTAINS, "Node1");
        ChildAssociationRef createContent =
                createContent(node1, name, "0", MimetypeMap.MIMETYPE_TEXT_PLAIN, "UTF-8");

        ChildAssociationRef createContent1 =
                createContent(node1, name1, "01", MimetypeMap.MIMETYPE_TEXT_PLAIN, "UTF-8");

        createContent(node1, nameAfter, "1", MimetypeMap.MIMETYPE_TEXT_PLAIN, "UTF-8");
        ...
        try {
            ...
            assertEquals(page.get(0).getName(), name1);
            assertEquals(page.get(1).getName(), name);
        } finally {
            resetMocks();
        }
    }
    ...
    public void testLastDuplicate() throws Exception {
        ...
        NodeRef node1 = nodeService.getChildByName(vf, ContentModel.ASSOC_CONTAINS, "Node1");
        createContent(node1, name, "0", MimetypeMap.MIMETYPE_TEXT_PLAIN, "UTF-8");
        NodeRef aNameNodeRef = nodeService.getChildByName(node1, ContentModel.ASSOC_CHILDREN, name);

        createContent(node1, nameAfter, "1", MimetypeMap.MIMETYPE_TEXT_PLAIN, "UTF-8");
        NodeRef nameAfterNodeRef =
                nodeService.getChildByName(node1, ContentModel.ASSOC_CHILDREN, nameAfter);
        ...
        try {
            PagingResults<FileInfo> results =
                    fileAndFolderService.list(
                            nodeService.getPrimaryParent(aNameNodeRef).getParentRef(),
                            true,
                            false,
                            namePattern,
                            null,
    ...
    public void testListUsedWithCMIS() throws Exception {
        ...
        PagingResults<FileInfo> results =
                fileAndFolderService.list(vf, true, true, null, sortProps, pagingRequest);
        ...
        createContent(node2, contentName1);

        results = fileAndFolderService.list(vf, true, true, null, sortProps, pagingRequest);

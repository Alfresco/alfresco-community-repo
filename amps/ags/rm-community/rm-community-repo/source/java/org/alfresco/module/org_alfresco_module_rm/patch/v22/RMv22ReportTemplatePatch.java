...
public class RMv22ReportTemplatePatch extends AbstractModulePatch {
  ...
  private static final String REPORT_TEMPLATE_PATH =
    "alfresco/module/org_alfresco_module_rm/bootstrap/report/report_rmr_transferReport.html.ftl";
  ...
  public void applyInternal() {
    NodeRef transferReport = new NodeRef(
      StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
      TRANSFER_REPORT
    );
    NodeRef destructionReport = new NodeRef(
      StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
      DESTRUCTION_REPORT
    );
    if (
      !nodeService.exists(transferReport) &&
      nodeService.exists(destructionReport)
    ) {
      NodeRef parent = nodeService
        .getPrimaryParent(destructionReport)
        .getParentRef();

      // get the assoc qname
      QName assocQName = QName.createQName(
        NamespaceService.CONTENT_MODEL_1_0_URI,
        QName.createValidLocalName("report_rmr_transferReport.html.ftl")
      );
      ...
      InputStream is = getClass()
        .getClassLoader()
        .getResourceAsStream(REPORT_TEMPLATE_PATH);

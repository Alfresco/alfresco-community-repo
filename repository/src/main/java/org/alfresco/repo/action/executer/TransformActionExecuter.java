...
public class TransformActionExecuter extends ActionExecuterAbstractBase {
  ...
  protected void executeImpl(Action ruleAction, NodeRef actionedUponNodeRef) {
    ...
    if (
      this.dictionaryService.isSubClass(typeQName, ContentModel.TYPE_CONTENT) ==
      false
    ) {
    ...
    TransformationOptions transformationOptions = newTransformationOptions(
      ruleAction,
      actionedUponNodeRef
    );
    ...
    transformationOptions.setUse(
      Thread.currentThread().getName().contains("Async")
        ? "asyncRule"
        : "syncRule"
    );
    ...
    NodeRef destinationParent = (NodeRef) ruleAction.getParameterValue(
      PARAM_DESTINATION_FOLDER
    );
    QName destinationAssocTypeQName = (QName) ruleAction.getParameterValue(
      PARAM_ASSOC_TYPE_QNAME
    );
    QName destinationAssocQName = (QName) ruleAction.getParameterValue(
      PARAM_ASSOC_QNAME
    );
    ...
    Boolean overwriteValue = (Boolean) ruleAction.getParameterValue(
      PARAM_OVERWRITE_COPY
    );
    ...
    if (copyNodeRef == null) {
      ...
      String originalTitle = (String) nodeService.getProperty(
        actionedUponNodeRef,
    ...
    if (contentReader != null) {
      ...
      try {
        ...
        throw new RuleServiceException(
          TRANSFORMING_ERROR_MESSAGE + e.getMessage()
        );
      }
    }
  }
  ...
  public static String transformName(
    ...
    if (dotIndex > -1) {
      ...
      String originalExtension = original.substring(
        dotIndex + 1,
        original.length()
      );
      ...
      boolean originalExtensionIsReasonable = isExtensionReasonable(
        originalExtension
      );
   ...
   * @since 3.3
   */
  private static boolean isExtensionReasonable(
    String potentialExtensionString
  ) {
    return (
      potentialExtensionString.length() > 0 &&
      potentialExtensionString.indexOf(' ') == -1
    );
  }

...
public abstract class AbstractSubClassesGet extends DictionaryWebServiceBase {
  ...
  protected Map<String, Object> executeImpl(
    ...
    String recursiveValue = getValidInput(
      req.getParameter(REQ_URL_TEMPL_IMMEDIATE_SUB_TYPE_CHILDREN)
    );
    ...
    if (recursiveValue == null) {
      ...
      throw new WebScriptException(
        ...
        "Check the value for the parameter recursive=> " +
        recursiveValue +
    ...
    if (name != null) {
      if (isValidModelName(name) == false) {
        throw new WebScriptException(
          Status.STATUS_NOT_FOUND,
          "Check the name parameter - " + name + " in the URL"
        );
      }
    }
    ...
    if (namespacePrefix == null && name != null) {
      namespaceUri =
        namespaceService.getNamespaceURI(getPrefixFromModelName(name));
    }
   ...
   */
  protected abstract Collection<QName> getQNameCollection(
    WebScriptRequest req,
    boolean recursive
  );
   ...
   */
  protected abstract void validateClassname(

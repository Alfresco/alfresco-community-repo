...
/**
 * Delegate for a Class-level Policy. Provides access to Policy Interface implementations which
 * invoke the appropriate bound behaviours.
 *
...
public class ClassPolicyDelegate<P extends ClassPolicy> {
    ...
    /*package*/ ClassPolicyDelegate(
            DictionaryService dictionary,
            Class<P> policyClass,
            BehaviourIndex<ClassBehaviourBinding> index,
            long tryLockTimeout) {
     ...
     *
     * <p>When multiple behaviours are bound to the policy for the class, an aggregate policy
     * implementation is returned which invokes each policy in turn.
     *
    ...
    public P get(NodeRef nodeRef, QName classQName) {
        ...
        if (classDefinition == null) {
            throw new IllegalArgumentException(
                    "Class " + classQName + " has not been defined in the data dictionary");
        }
    ...
    /**
     * Gets the policy implementation for the given classes. The single <tt>Policy</tt> will be a
     * wrapper of multiple appropriate policies.
     *

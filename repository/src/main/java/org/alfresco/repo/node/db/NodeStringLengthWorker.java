 ...
 * <h2>What it is</h2>
 *
 * A worker for a scheduled job that checks and adjusts string storage for persisted strings in the
 * system.
 *
 * <p>
 *
 ...
 *
 * <ul>
 *   <li><b>${system.maximumStringLength}</b> - the maximum length of a string that can be persisted
 *       in the *alf_node_properties.string_value* column.
 *   <li><b>${system.maximumStringLength.jobQueryRange}</b> - the node ID range to query for. The
 *       process will repeat from the first to the last node, querying for up to this many nodes.
 *       Only reduce the value if the NodeDAO query takes a long time.
 *   <li><b>${system.maximumStringLength.jobThreadCount}</b> - the number of threads that will
 *       handle persistence checks and changes. Increase or decrease this to allow for free CPU
 *       capacity on the machine executing the job.
 * </ul>
 *
 * <h2>How to use it</h2>
 *
...
public class NodeStringLengthWorker implements ApplicationContextAware {
    private static final QName LOCK =
            QName.createQName(NamespaceService.SYSTEM_MODEL_1_0_URI, "NodeStringLengthWorker");
    ...
    public NodeStringLengthWorkResult execute() {
        ...
        JobLockRefreshCallback lockCallback =
    ...
    private synchronized void doWork(NodeStringLengthWorkResult progress) throws Exception {
        ...
        BatchProcessWorkProvider<NodePropertyEntity> workProvider =
                new NodeStringLengthWorkProvider(progress);
        ...
        BatchProcessor<NodePropertyEntity> batchProcessor =
    ...
    private class NodeStringLengthWorkProvider
        ...
        public Collection<NodePropertyEntity> getNextWork() {
            ...
            // Keep shifting the query window up until we get results or we hit the original max
            // node ID
            ...
            while (ret.isEmpty() && progress.currentMinNodeId.get() < maxNodeId) {
                ...
                ret =
                        nodeDAO.selectNodePropertiesByDataType(
                                DataTypeDefinition.TEXT, minNodeId, maxNodeId);
            }

            // Done
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Node string length work provider found "
                                + ret.size()
    ...
    private class NodeStringLengthBatch extends BatchProcessWorkerAdaptor<NodePropertyEntity> {
        private final int typeOrdinalText =
                NodePropertyValue.convertToTypeOrdinal(DataTypeDefinition.TEXT);
        private final int typeOrdinalAny =
                NodePropertyValue.convertToTypeOrdinal(DataTypeDefinition.ANY);
        private final NodeStringLengthWorkResult progress;
        ...
        public void process(NodePropertyEntity entry) throws Throwable {
            ...
            try {
                ...
                if (text.length() > SchemaBootstrap.getMaxStringLength()) {
                    ...
                    // The text is shorter than the current max, so it should be stored as a
                    // string_value (TEXT)
                ...
                if (repersist) {
                    ...
                    if (logger.isTraceEnabled()) {
                        logger.trace(
                                "Fixing property " + getIdentifier(entry) + ".  Value: " + text);
        ...
        public String getIdentifier(NodePropertyEntity entry) {
            Long nodeId = entry.getNodeId();
            NodePropertyValue prop = entry.getValue();
            return ("Property with persisted type "
                    + prop.getPersistedType()
                    + " on node "
                    + nodeDAO.getNodePair(nodeId));
        }
    }
    ...
    public static class NodeStringLengthWorkResult {
        ...
        public String toString() {
            ...
            String part2 =
                    String.format(
                            " %4d out of a potential %4d properties. ",
                            propertiesChanged.get(), propertiesProcessed.get());
     ...
     *
     * <ul>
     *   <li><b>nodeStringLengthWorker</b> - The worker that performs the actual processing.
     * </ul>
     *
     * @author Derek Hulley
     * @since 4.1.9.2
    ...
    public static class NodeStringLengthJob implements Job {
        ...
        public void execute(JobExecutionContext context) throws JobExecutionException {
            ...
            if (nodeStringLengthWorkerObj == null
                    || !(nodeStringLengthWorkerObj instanceof NodeStringLengthWorker)) {
                throw new AlfrescoRuntimeException(
                        "MaxStringLengthJob data '"
                                + JOB_DATA_NODE_WORKER
                                + "' must reference a "
                                + NodeStringLengthWorker.class.getSimpleName());
            }

...
public class TestActions extends AbstractBaseApiTest {
  ...
  public void setUp() throws Exception {
    ...
    ownableService =
      applicationContext.getBean("OwnableService", OwnableService.class);
    actionService =
      applicationContext.getBean("ActionService", ActionService.class);
  ...
  public void canGetActionDefinitions() throws PublicApiException {
    ...
    publicApiClient.setRequestContext(
      new RequestContext(account1.getId(), person1)
    );

    {
      ListResponse<ActionDefinition> actionDefs = actions.getActionDefinitions(
        emptyParams,
        200
      );
      ...
      assertFalse(
        "Action definition list should not be empty",
        actionDefs.getList().isEmpty()
      );

      // Check defaults, given that no paging params were sent in the request
      assertEquals(
        Paging.DEFAULT_MAX_ITEMS,
        actionDefs.getPaging().getMaxItems().intValue()
      );
      assertEquals(
        Paging.DEFAULT_SKIP_COUNT,
        actionDefs.getPaging().getSkipCount().intValue()
      );

      // Check ActionDefinition fields
      List<ActionDefinition> actionDefinitions = actionDefs
        .getList()
        .stream()
        .filter(ad -> ad.getName().equals("add-features"))
      ...
      assertEquals(
        "This will add an aspect to the matched item.",
      ...
      ActionDefinition.ParameterDefinition paramDefs = action
    ...
    checkBasicPagingAndSorting(
      ...
      () ->
        actionService
          ...
          .stream()
          .sorted(
            comparing(org.alfresco.service.cmr.action.ActionDefinition::getName)
          )
    ...
    checkSorting(
      ...
      () ->
        actionService
          ...
          .sorted(
            comparing(
              org.alfresco.service.cmr.action.ActionDefinition::getTitle,
      ...
      // Actual results
      (paging, orderBy) ->
        actions.getActionDefinitions(createParams(paging, orderBy), 200),
      "title"
    );
    ...
    checkSorting(
      ...
      () ->
        actionService
          ...
          .sorted(
            comparing(
              org.alfresco.service.cmr.action.ActionDefinition::getName,
              nullsFirst(naturalOrder())
            )
          )
          .map(act -> new Pair<>(act.getName(), act.getTitle()))
    ...
    {
      publicApiClient.setRequestContext(
        new RequestContext(account1.getId(), person1, "invalid-password")
      );
  ...
  public void canGetActionDefinition() throws PublicApiException {
    ...
    ActionDefinition actionDef = actions.getActionDefinition(
      "add-features",
      200
    );
    ...
    ActionDefinition.ParameterDefinition paramDefs = actionDef
      .getParameterDefinitions()
      .get(0);
    assertEquals(
      AddFeaturesActionExecuter.PARAM_ASPECT_NAME,
  ...
  public void canGetActionDefinitionsForNode() throws Exception {
    ...
    {
      ListResponse<ActionDefinition> actionDefs = actions.getActionDefinitionsForNode(
        "-my-",
        emptyParams,
        200
      );
    ...
    {
      NodeRef nodeForCheckout = nodeService
        .createNode(
        ...
        .getChildRef();
      CheckOutCheckInService coci = applicationContext.getBean(
      ...
      List<ActionDefinition> actionDefinitions = actionDefs
        .getList()
        .stream()
        .filter(ad -> ad.getName().equals("check-in"))
        .collect(Collectors.toList());
      assertEquals(1, actionDefinitions.size());
      ...
      assertEquals(
        CheckInActionExecuter.PARAM_MINOR_CHANGE,
        paramDefs.getName()
      );
    ...
    checkBasicPagingAndSorting(
      ...
      () ->
        actionService
          .getActionDefinitions(validNode)
          .stream()
          .sorted(
            comparing(org.alfresco.service.cmr.action.ActionDefinition::getName)
          )
      ...
      // Actual results
      paging ->
        actions.getActionDefinitionsForNode(
          validNode.getId(),
    ...
    // Non-existent node ID
    {
      ...
      assertFalse(
        "Test pre-requisite: node must not exist",
  ...
  private void checkBasicPagingAndSorting(
    ...
    List<String> actionNames = actionDefs
      .getList()
      .stream()
    ...
    if (expectedNames.size() < ((pageSize * 2) + 1)) { // need at least 3 pages worth
      ...
      assertEquals(
        expectedNames.size(),
        (int) actionDefs.getPaging().getTotalItems()
      );
  ...
  private void checkSorting(
    ...
    ListResponse<ActionDefinition> actionDefs = actionsFun.apply(
      paging,
      orderBy
    );

    List<Pair<String, String>> retrievedActions = actionDefs
      .getList()
      .stream()
      .map(act -> new Pair<>(act.getName(), act.getTitle()))
      .collect(Collectors.toList());
    ...
    assertEquals(
      expectedActions.subList(pageSize, pageSize * 2),
      retrievedActions
    );
  }
  ...
  public void testExecuteAction() throws Throwable {
    ...
    NodeRef validNode = nodeService
      .createNode(
        new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, myNode),
        ContentModel.ASSOC_CONTAINS,
        QName.createQName("test", "test-ea-node"),
        ContentModel.TYPE_CONTENT
      )
      .getChildRef();
    ...
    // Missing mandatory params - action not executed.
    {
      ...
      assertFalse(
        "Aspect versionable wasn't expected !",
        nodeService.hasAspect(validNode, ContentModel.ASPECT_VERSIONABLE)
      );
    }

...
public class LockDAOTest extends TestCase {
        ...
        try {
            ...
            if (expectSuccess) {
                ...
                throw new RuntimeException(
                        "Expected to get lock " + lockName + " with TTL of " + timeToLive, e);
            } else {
                return null;
            }
        }
    ...
    public void testLockTableScaling() throws Exception {
        ...
        for (int i = 1; i <= count; i++) {
            QName lockName =
                    QName.createQName(lockAAA.getNamespaceURI(), lockAAA.getLocalName() + "-" + i);
            lock(lockName, 500L, true);
            if (i % 100 == 0) {
                long after = System.currentTimeMillis();
                System.out.println(
                        "Creation of " + i + " locks took " + (after - before) / 1000 + "s");
            }
        }
    }
    ...
    public synchronized void testLockExpiryAndRelease() throws Exception {
        ...
        release(
                lockAAA,
                grabbedTokenAAAA,
                true); // Proof that expiry, on it's own, doesn't prevent release
    }

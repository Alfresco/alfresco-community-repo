...
public class TestWebScriptRepoServer extends TestWebScriptServer {
   ...
   * @return  Test Server
   */
  public static synchronized TestWebScriptServer getTestServer(
    String appendTestConfigLocation
  ) {
      ...
      if (configChanged) {
        ...
        try {
          ...
          throw new RuntimeException(
            "Failed to shut down existing application context",
            e
          );
        }
      } else {
    ...
    if (
      TestWebScriptRepoServer.ctx == null ||
      (
        TestWebScriptRepoServer.ctx != null &&
        !TestWebScriptRepoServer.ctx.isActive()
      )
    ) {
      // Restart it
    ...
    // Get the bean
    TestWebScriptServer testServer = (TestWebScriptRepoServer) TestWebScriptRepoServer.ctx.getBean(
      "webscripts.test"
    );
    return testServer;
  }

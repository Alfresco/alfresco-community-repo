...
public class RemoteCredentialsServiceImpl implements RemoteCredentialsService
{
    ...
    /**
     * The name of the System Container used to hold Shared Credentials. This isn't final, as unit tests change it to avoid trampling on the real credentials.
     */
    ...
    /**
     * Controls which Factory will be used to create {@link BaseCredentialsInfo} instances for a given node, based on the type. eg rc:passwordCredentials -> PasswordCredentialsFactory
     */
    ...
    /**
     * Registers a new Factory to produce {@link BaseCredentialsInfo} objects for a given data type. This provides an alternative to {@link #setCredentialsFactories(Map)} to allow the registering of a new type without overriding all of them.
     * 
    ...
    private static QName SHARED_CREDENTIALS_CONTAINER_QNAME = QName.createQName(NamespaceService.SYSTEM_MODEL_1_0_URI, SHARED_CREDENTIALS_CONTAINER_NAME);
    ...
    /**
     * Ensure the appropriate aspect is applied to the node which will hold the Remote Credentials System
     */
        ...
        else
        {
            if (lazyCreate)
            {
                ...
                system = nodeService.createNode(
                        ...
                        QName.createQName(remoteSystem), RemoteCredentialsModel.TYPE_REMOTE_CREDENTIALS_SYSTEM).getChildRef();
    ...
    private BaseCredentialsInfo createCredentials(String remoteSystem, NodeRef remoteSystemNodeRef, BaseCredentialsInfo credentials)
    {
        ...
        NodeRef nodeRef = nodeService.createNode(
                ...
                name, credentials.getCredentialsType(), properties).getChildRef();
    ...
    public BaseCredentialsInfo getPersonCredentials(String remoteSystem)
    {
        ...
        // Grab the children
        List<ChildAssociationRef> credentials = nodeService.getChildAssocs(personContainer, RemoteCredentialsModel.ASSOC_CREDENTIALS, RegexQNamePattern.MATCH_ALL);
    ...
    /**
     * TODO This would probably be better done as a dedicated Canned Query We want to filter by Assoc Type and Child Node Type, and the node service currently only allows you to do one or the other
     */

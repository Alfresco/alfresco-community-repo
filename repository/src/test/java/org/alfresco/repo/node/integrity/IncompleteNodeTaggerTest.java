...
public class IncompleteNodeTaggerTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        if (!nodeService.exists(storeRef)) {
            ...
            permissionService.setPermission(
                    rootNodeRef, user, PermissionService.ALL_PERMISSIONS, true);
        } else {
            rootNodeRef = nodeService.getRootNode(storeRef);
        }
    ...
    private void checkTagging(final NodeRef nodeRef, final boolean mustBeTagged) {
        ...
        RunAsWork<Void> checkWork =
                new RunAsWork<Void>() {
                    public Void doWork() throws Exception {
                        assertEquals(
                                nodeService.hasAspect(nodeRef, ContentModel.ASPECT_INCOMPLETE),
                                mustBeTagged);
                        return null;
                    }
                };
    ...
    public void testChangeToTypeWithMandatoryProperties() {
        ...
        // Change type to a one with mandatory properties and check the node to have sys:incomplete
        // aspect
    ...
    public void testCreateWithoutAssoc() throws Exception {
        NodeRef nodeRef =
                createNode(
                        "abc", IntegrityTest.TEST_TYPE_WITH_NON_ENFORCED_CHILD_ASSOCS, properties);
        // The node should not be tagged as by default it has enforced=true
        // see IncompleteNodeTagger.checkProperties(Collection<PropertyDefinition>, Map<QName,
        // Serializable>)
        checkTagging(nodeRef, false);
    }

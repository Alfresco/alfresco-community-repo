 ...
 *
 * <p>Cluster-wide invalidation messages mean that we have to be very careful about only making
 * updates to caches when entries really change. However, the nature of the {@link ConfigService}
 * hierarchy makes it very difficult to do this in a deterministic manner without performing write
 * locks that run across tenants. By receiving asynchronous messages to refresh we no longer have to
 * rely on a cluster-aware cache.
 *
 * @author Derek Hulley
 * @author Andy Hind
 * @since 4.1.5
...
public class ConfigDataCache extends AbstractMTAsynchronouslyRefreshedCache<ConfigData> {
    ...
    /**
     * Set the config service. Depending on the order of injection, this might have to be done by
     * code.
     */
    ...
    /**
     * Immutable version of {@link ConfigData} to ensure cast-iron safety of data being put into the
     * caches.
     *
     * @author Derek Hulley
     * @since 4.1.5
    ...
    public static class ImmutableConfigData extends ConfigData {
        ...
        public ImmutableConfigData(ConfigData configData) {
            ...
            if (configDeployments != null) {
                List<ConfigDeployment> configDeploymentsLocked =
                        Collections.unmodifiableList(configDeployments);
            ...
            if (elementReaders != null) {
                Map<String, ConfigElementReader> elementReadersLocked =
                        Collections.unmodifiableMap(elementReaders);
            ...
            if (sectionsByArea != null) {
                Map<String, List<ConfigSection>> sectionsByAreaLocked =
                        Collections.unmodifiableMap(sectionsByArea);

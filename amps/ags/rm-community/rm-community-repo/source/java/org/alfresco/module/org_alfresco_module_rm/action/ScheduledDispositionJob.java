...
public class ScheduledDispositionJob implements Job {
  ...
  public void execute(JobExecutionContext context)
    ...
    RecordsManagementActionService rmActionService = (RecordsManagementActionService) context
      .getJobDetail()
      .getJobDataMap()
      .get("recordsManagementActionService");
    NodeService nodeService = (NodeService) context
      .getJobDetail()
      .getJobDataMap()
      .get("nodeService");
    ...
    String dateRange =
      "[\"1970-01-01T00:00:00.00Z\" TO \"" + currentDate + "\"]";

    // Execute the query and process the results
    String query =
      "+ASPECT:\"rma:record\" +ASPECT:\"rma:dispositionSchedule\" +@rma\\:dispositionAsOf:" +
      dateRange;

    SearchService search = (SearchService) context
      .getJobDetail()
      .getJobDataMap()
      .get("searchService");
    ResultSet results = search.query(
    ...
    for (NodeRef node : resultNodes) {
      String dispActionName = (String) nodeService.getProperty(
        node,
      ...
      if (dispActionName != null && dispActionName.equalsIgnoreCase("cutoff")) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Performing " +
            dispActionName +
            " dispoition action on disposable item " +
            node.toString()
          );
        }
      }

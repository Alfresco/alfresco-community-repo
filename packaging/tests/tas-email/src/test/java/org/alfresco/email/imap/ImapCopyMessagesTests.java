...
public class ImapCopyMessagesTests extends EmailTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify site manager can copy non-empty file via IMAP client to a different"
                            + " location")
    ...
    public void siteManagerShouldCopyNonEmptyFile() throws Exception {
        ...
        FileModel fileModel =
                dataContent.usingSite(testSite).createContent(CMISUtil.DocumentType.TEXT_PLAIN);
        imapProtocol
                ...
                .existsInImap()
                .then()
                .copyMessageTo(testFolder)
                .assertThat()
                .containsMessages(fileModel);
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Verify site manager can copy non-empty files via IMAP client to a different"
                            + " location")
    @Test(groups = {TestGroup.PROTOCOLS, TestGroup.IMAP, TestGroup.SANITY})
    public void siteManagerShouldCopyNonEmptyFiles() throws Exception {
        ...
        imapProtocol
                ...
                .then()
                .usingSite(testSite)
                .copyMessagesTo(testFolder)
                .assertThat()
                .containsMessages(fileModel1, fileModel2);
    }
    ...
    @TestRail(
            ...
            description =
                    "Verify site manager can copy empty file via IMAP client to a different"
                            + " location")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user can copy file via IMAP client to a location where the message"
                            + " already exists")
    @Test(groups = {TestGroup.PROTOCOLS, TestGroup.IMAP, TestGroup.CORE})
    public void userShouldCopyFileWhereAlreadyExists() throws Exception {
        testFolder = dataContent.usingUser(testUser).usingSite(testSite).createFolder();
        FileModel fileModel =
                new FileModel("CopyFile.txt", FileType.TEXT_PLAIN, "content of copied file");
        ...
        imapProtocol
                ...
                .copyMessageTo(testFolder)
                .assertThat()
                .containsMessages(fileToBeCopied);
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user cannot copy a file via IMAP client if it was already deleted from"
                            + " repository")
    @Test(
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user cannot copy file via IMAP client to a location where you don't"
                            + " have permissions")
    @Test(
            groups = {TestGroup.PROTOCOLS, TestGroup.IMAP, TestGroup.CORE},
            expectedExceptions = MessagingException.class,
            expectedExceptionsMessageRegExp =
                    ".*NO APPEND failed. Can't append message - Permission denied.*")

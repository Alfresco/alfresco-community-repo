...
public class WebDAVServlet extends HttpServlet {
  ...
  private static final String KEY_WEBDAV_ROOT_NODEREF =
    "key.webdavRoot.noderef";
    ...
    try {
      ...
      if (method == null) {
        if (logger.isErrorEnabled()) logger.error(
          "WebDAV method not implemented - " + request.getMethod()
        );
   ...
   * @return WebDAVMethod
   */
  protected WebDAVMethod createMethod(
    HttpServletRequest request,
    HttpServletResponse response
  ) {
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "WebDAV request " + strHttpMethod + " on path " + request.getRequestURI()
    );
  ...
  public void init(ServletConfig config) throws ServletException {
    ...
    WebApplicationContext wc = WebApplicationContextUtils.getRequiredWebApplicationContext(
      getServletContext()
    );
    ...
    serviceRegistry =
      (ServiceRegistry) context.getBean(ServiceRegistry.SERVICE_REGISTRY);
    ...
    ActivityPoster poster = (ActivityPoster) context.getBean(
      "activitiesPoster"
    );
    singletonCache =
      (SimpleCache<String, NodeRef>) context.getBean("immutableSingletonCache");
  ...
  private void initializeRootNode(
    ...
    AuthenticationContext authComponent = (AuthenticationContext) context.getBean(

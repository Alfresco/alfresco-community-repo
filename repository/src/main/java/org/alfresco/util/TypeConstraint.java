...
/**
 * Stores a set of expected and excluded types, by full type name. The localName can be a wildcard
 * (*) to indicate that the whole namespace should be expected/excluded.
 *
 * <p>A node is tested to ensure that its type is in the expected list and not in the excluded list.
 * Its aspects are also tested to ensure that they are in the expected list and not in the excluded
 * list.
 *
 * <p>Adapted some code from QNameFilter.
 *
 * @author steveglover
 */
public class TypeConstraint {
    ...
    private boolean isExcluded(QName typeQName) {
        return excludedQNames != null && excludedQNames.contains(typeQName)
                || excludedModels != null && excludedModels.contains(typeQName.getNamespaceURI());
    }

    private boolean matchesExpected(QName typeQName) {
        if (expectedQNames == null
                || expectedQNames.contains(typeQName)
                || expectedModels == null
                || expectedModels.contains(typeQName.getNamespaceURI())) {
            return true;
        }
                        ...
                        new TenantRunAsWork<Boolean>() {
                            ...
                            public Boolean doWork() throws Exception {
                                ...
                                if (typeExcluded) {
                                    ...
                                    if (isExpected) {
                                        // type is expected and not excluded, check the aspects for
                                        // exclusion
                                        ...
                                        // type not in expected list, check aspects for exclusion
                                        // and in the expected list
                                        ...
                                        if (!isExpected) {
                                            // neither the type nor any of the aspects in the
                                            // expected list, no match
                                            matches = false;
                                        }
                                    }
                                }

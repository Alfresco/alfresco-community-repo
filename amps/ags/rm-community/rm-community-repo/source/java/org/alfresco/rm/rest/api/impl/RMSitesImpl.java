...
public class RMSitesImpl extends SitesImpl implements RMSites {
    ...
    /**
     * Even if the method it will be protected in core, we still need to override since we don't
     * need to check if the visibility is set since for RM site it is always PUBLIC. We also don't
     * need to generate the id from title, or to check the id, since the id is always rm.
     *
     * @param site
     * @return
     */
    ...
    /**
     * Method used for solving rm site nodeRef caching problem that affected rm site update and get
     * from rest api, after site deletion from rest api. See RM-4289 issue for details.
     */
    private void solveRMSiteNodeRefCaching() {
        // since we do not have access to SiteServiceImpl.getSiteNodeRef(String shortName, boolean
        // enforcePermissions) method we can use hasSite method to solve caching problem

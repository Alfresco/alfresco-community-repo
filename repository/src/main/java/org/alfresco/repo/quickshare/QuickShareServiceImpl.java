...
public class QuickShareServiceImpl
  ...
  private static final String DEFAULT_EMAIL_SUBJECT =
    "quickshare.notifier.email.subject";
  ...
  public void setQuickShareLinkExpiryActionPersister(
    ...
    this.quickShareLinkExpiryActionPersister =
  ...
  public void setExpiryDatePeriod(String expiryDatePeriod) {
    if (expiryDatePeriod != null) {
      this.expiryDatePeriod =
        ExpiryDatePeriod.valueOf(expiryDatePeriod.toUpperCase());
    }
  }
  ...
  public QuickShareDTO shareContent(NodeRef nodeRef, Date expiryDate)
    ...
    if (
      !nodeService.getAspects(nodeRef).contains(QuickShareModel.ASPECT_QSHARE)
    ) {
      ...
      if (logger.isInfoEnabled()) {
        logger.info(
          "QuickShare - shared content: " + sharedId + " [" + nodeRef + "]"
        );
      }
    } else {
      sharedId =
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "QuickShare - content already shared: " +
          sharedId +
          " [" +
          nodeRef +
          "]"
  ...
  private void checkEnabled() {
    if (enabled == false) {
      throw new QuickShareDisabledException(
  ...
  public Map<String, Object> getMetaData(NodeRef nodeRef) {
    ...
    String modifierUserName = (String) nodeProps.get(
    ...
    List<NodeRef> thumbnailRefs = thumbnailService.getThumbnails(
      nodeRef,
      ContentModel.PROP_CONTENT,
      null,
      null
    ...
    if (nodeProps.containsKey(QuickShareModel.PROP_QSHARE_SHAREDID)) {
      metadata.put(
        "sharedId",
        nodeProps.get(QuickShareModel.PROP_QSHARE_SHAREDID)
      );
      metadata.put(
        "expiryDate",
        nodeProps.get(QuickShareModel.PROP_QSHARE_EXPIRY_DATE)
      );
    } else {
  ...
  @Override
  public Pair<String, NodeRef> getTenantNodeRefFromSharedId(
    final String sharedId
  ) {
    NodeRef nodeRef = TenantUtil.runAsDefaultTenant(
      new TenantRunAsWork<NodeRef>() {
        public NodeRef doWork() throws Exception {
          return (NodeRef) attributeService.getAttribute(
            ATTR_KEY_SHAREDIDS_ROOT,
            sharedId
          );
        }
  ...
  public void beforeDeleteNode(final NodeRef beforeDeleteNodeRef) {
    AuthenticationUtil.runAsSystem(
      new RunAsWork<Void>() {
        public Void doWork() throws Exception {
          String sharedId = (String) nodeService.getProperty(
            beforeDeleteNodeRef,
            QuickShareModel.PROP_QSHARE_SHAREDID
          );
          if (sharedId != null) {
            try {
              Pair<String, NodeRef> pair = getTenantNodeRefFromSharedId(
                sharedId
              );
  ...
  public void onRestoreNode(ChildAssociationRef childAssocRef) {
    ...
    AuthenticationUtil.runAsSystem(
      new RunAsWork<Void>() {
        public Void doWork() throws Exception {
          if (
            nodeService.hasAspect(childNodeRef, QuickShareModel.ASPECT_QSHARE)
          ) {
            // Disable audit to preserve modifier and modified date
            behaviourFilter.disableBehaviour(
              childNodeRef,
    ...
    TenantUtil.runAsSystemTenant(
      new TenantRunAsWork<Void>() {
        public Void doWork() throws Exception {
          ...
          String nodeSharedId = (String) nodeService.getProperty(
            nodeRef,
            QuickShareModel.PROP_QSHARE_SHAREDID
          );
  ...
  public void sendEmailNotification(final QuickShareEmailRequest emailRequest) {
    ...
    final NodeRef senderNodeRef = personService.getPerson(
      authenticatedUser,
      false
    );
    final Map<QName, Serializable> senderProps = nodeService.getProperties(
      senderNodeRef
    );
    final String senderFirstName = (String) senderProps.get(
      ContentModel.PROP_FIRSTNAME
    );
    final String senderLastName = (String) senderProps.get(
    ...
    final String templateAssetsUrl = getUrl(
      clientApp.getTemplateAssetsUrl(),
    ...
    actionParams.put(
      MailActionExecuter.PARAM_FROM_PERSONAL_NAME,
      senderFullName
    );
    ...
    actionParams.put(
      ...
      new Object[] {
        senderFirstName,
        senderLastName,
        emailRequest.getSharedNodeName(),
      }
    );
    actionParams.put(
      MailActionExecuter.PARAM_IGNORE_SEND_FAILURE,
      emailRequest.isIgnoreSendFailure()
    );
    // Pick the template
  ...
  public boolean canDeleteSharedLink(NodeRef nodeRef, String sharedByUserId) {
    ...
    if (siteName != null) {
      ...
      isSharedByCurrentUser || (authorityService.isAdminAuthority(currentUser))
    ) {
  ...
  private String getSiteName(NodeRef nodeRef) {
    ...
    while (
      parent != null && !nodeService.getType(parent).equals(SiteModel.TYPE_SITE)
    ) {
  ...
  protected void saveSharedLinkExpiryAction(String sharedId, Date expiryDate) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Creating shared link expiry action for the sharedId:" + sharedId
      );
    }

    final NodeRef expiryActionNodeRef = getQuickShareLinkExpiryActionNode(
      sharedId
    );
    ...
    final QuickShareLinkExpiryAction expiryAction = new QuickShareLinkExpiryActionImpl(
      java.util.UUID.randomUUID().toString(),
      sharedId,
      "QuickShare link expiry action"
    );
    // Create the persisted schedule
    final ScheduledPersistedAction schedule = scheduledPersistedActionService.createSchedule(
      expiryAction
    );
    ...
    try {
      ...
      throw new QuickShareLinkExpiryActionException(
        "Couldn't create quick share link expiry action.",
        ex
      );
    }
   ...
   */
  protected void deleteQuickShareLinkExpiryAction(
  ...
  private void deleteQuickShareLinkExpiryActionImpl(
    QuickShareLinkExpiryAction linkExpiryAction
  ) {
    ...
    quickShareLinkExpiryActionPersister.deleteQuickShareLinkExpiryAction(
      linkExpiryAction
    );
  }
  ...
  private QuickShareLinkExpiryAction attachSchedule(
    ...
    if (quickShareLinkExpiryAction.getSchedule() == null) {
      ScheduledPersistedAction schedule = scheduledPersistedActionService.getSchedule(
  ...
  private NodeRef getQuickShareLinkExpiryActionNode(String sharedId) {
    final QName expiryActionQName = QuickShareLinkExpiryActionImpl.createQName(
      sharedId
    );
    ...
    DAYS {
      ...
      int getDuration(DateTime now, DateTime expiryDate) {
        Interval interval = new Interval(
          now.withSecondOfMinute(0).withMillisOfSecond(0),
          expiryDate
        );

...
public class SurfConfigTest extends AbstractSiteServiceTest {
  ...
  public void testSurfConfigPermissions() throws Exception {
    ...
    sendRequest(
      new PostRequest(
        URL_ADM +
        "CREATE/alfresco/site-data/pages/site/" +
        shortName +
        "/dashboard.xml?s=sitestore",
    ...
    NodeRef surfConfigFolderRef = nodeService.getChildByName(
      siteService.getSite(shortName).getNodeRef(),
      ContentModel.ASSOC_CONTAINS,
      "surf-config"
    );
    assertEquals(
      "surf-config",
      nodeService.getProperty(surfConfigFolderRef, ContentModel.PROP_NAME)
    );
    ...
    assertFalse(
      "Inherit Permissions should be off.",
      permissionService.getInheritParentPermissions(surfConfigFolderRef)
    );
    ...
    PagingResults<FileInfo> pageResults = fileFolderService.list(
      surfConfigFolderRef,
      true,
      true,
      null,
    ...
    try {
      ...
      fail(
        "USER_TWO dose not have the appropriate permissions to perform this operation."
      );
    ...
    try {
      ...
      fail(
        "USER_ONE is not the owner and he is no longer a site manager, so does not have the appropriate permissions to perform this operation"
      );
    } catch (AccessDeniedException ex) {
      //expected
    }

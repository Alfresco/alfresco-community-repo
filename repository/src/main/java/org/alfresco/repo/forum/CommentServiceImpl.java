...
public class CommentServiceImpl
   ...
   */
  private static final QName FORUM_TO_TOPIC_ASSOC_QNAME = QName.createQName(
    NamespaceService.CONTENT_MODEL_1_0_URI,
    "Comments"
  );
  private static final String COMMENTS_TOPIC_NAME = "Comments";

  private static final String CANNED_QUERY_GET_CHILDREN =
    "commentsGetChildrenCannedQueryFactory";
    ...
    if (ctx != null) {
      ...
      if (this.cannedQueryRegistry == null) {
        this.cannedQueryRegistry =
          (NamedObjectRegistry<CannedQueryFactory<? extends Object>>) ctx.getBean(
            "commentsCannedQueryRegistry"
          );
      }
  ...
  public NodeRef getDiscussableAncestor(NodeRef descendantNodeRef) {
    ...
    if (nodeService.getType(descendantNodeRef).equals(ForumModel.TYPE_POST)) {
      NodeRef topicNode = nodeService
        .getPrimaryParent(descendantNodeRef)
        .getParentRef();

      if (nodeService.getType(topicNode).equals(ForumModel.TYPE_TOPIC)) {
        ChildAssociationRef forumToTopicChildAssocRef = nodeService.getPrimaryParent(
          topicNode
        );
  ...
  @Override
  public PagingResults<NodeRef> listComments(
  ...
  public NodeRef getShareCommentsTopic(NodeRef discussableNode) {
    ...
    if (nodeService.hasAspect(discussableNode, ForumModel.ASPECT_DISCUSSABLE)) {
      ...
      List<ChildAssociationRef> fora = nodeService.getChildAssocs(
        discussableNode,
  ...
  private String getSiteId(final NodeRef nodeRef) {
    String siteId = AuthenticationUtil.runAsSystem(
  ...
  private JSONObject getActivityData(String siteId, final NodeRef nodeRef) {
    if (siteId != null) {
      ...
      logger.warn(
        "Unable to determine site in which node " + nodeRef + " resides."
      );
      return null;
    }
  }
  ...
  public void updateComment(
    ...
    if (!nodeType.equals(ForumModel.TYPE_POST)) {
      throw new IllegalArgumentException(
        "Node to update is not a comment node."
      );
    }
  ...
  public Map<String, Boolean> getCommentPermissions(
    ...
    if (discussableNodeRef.equals(discussableNode)) {
      if (
        !isWorkingCopyOrLocked(discussableNode) || isLockOwner(discussableNode)
      ) {
  ...
  private boolean canEditPermission(NodeRef commentNodeRef) {
    ...
    Serializable owner = nodeService.getProperty(
      commentNodeRef,
      ...
      if (!isWorkingCopy) {
        if (aspects.contains(ContentModel.ASPECT_LOCKABLE)) {
          ...
          if (
            ...
            lockStatus == LockStatus.LOCK_OWNER
          ) {
            isLocked = true;
          }
        }
  ...
  public void onUpdateProperties(
    ...
    if (discussableNodeRef != null) {
      if (
        behaviourFilter.isEnabled(ContentModel.ASPECT_LOCKABLE) &&
  ...
  public void beforeDeleteNode(final NodeRef commentNodeRef) {
    ...
    if (discussableNodeRef != null) {
      ...
      if (
        behaviourFilter.isEnabled(ContentModel.ASPECT_LOCKABLE) && // eg. delete site (MNT-14671)
        isWorkingCopyOrLocked(discussableNodeRef) &&
        !isLockOwner(discussableNodeRef) &&
        canDelete
      ) {

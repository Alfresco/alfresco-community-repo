...
/**
 * This {@link ActionExecuter} implementation is used internally to delete rendition nodes when a
 * rendition update has failed. The scenario is as follows: a content node exists in the repository
 * and has a number of rendition nodes associated with it. When the content node is given new
 * content, each of the rendition nodes must be updated to reflect the new source content. But if
 * one or more of those re-renditions fail, then the old rendition nodes now refer to out of date
 * content and should be deleted.
 *
 * <p>This class executes the deletion of the specified rendition node.
 *
...
public class DeleteRenditionActionExecuter extends ActionExecuterAbstractBase {
    ...
    public void executeImpl(final Action action, final NodeRef actionedUponNodeRef) {
        // It is possible that the user who triggered the thumbnail update does not have delete
        // permissions on the node
        // but does have write permissions. e.g. if a SiteCollaborator updates a node which they do
        // not own, any failure
        // to update renditions would trigger this DeleteRendition action and they would not have
                ...
                new RunAsWork<Void>() {
                    ...
                    public Void doWork() throws Exception {
                        ...
                        if (nodeExists) {
                            ...
                            final QName renditionDefName =
                                    (QName) paramValues.get(PARAM_RENDITION_DEFINITION_NAME);

                            ChildAssociationRef existingRendition =
                                    renditionService.getRenditionByName(
                                            actionedUponNodeRef, renditionDefName);

                            if (existingRendition != null) {
                                if (log.isDebugEnabled()) {
                                    StringBuilder msg = new StringBuilder();
                                    msg.append("Deleting rendition node: ")
                                            .append(existingRendition);
    ...
    protected void addParameterDefinitions(List<ParameterDefinition> paramList) {
        paramList.add(
                new ParameterDefinitionImpl(
                        ...
                        true,
                        getParamDisplayLabel(PARAM_RENDITION_DEFINITION_NAME),
                        false));
    }
}

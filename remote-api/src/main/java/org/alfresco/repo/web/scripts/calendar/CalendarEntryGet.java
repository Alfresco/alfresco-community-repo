...
public class CalendarEntryGet extends AbstractCalendarWebScript {
  ...
  protected String buildRecurrenceString(CalendarEntry event) {
    ...
    Map<String, String> days = CalendarRecurrenceHelper.buildLocalRecurrenceDaysOfTheWeek(
      I18NUtil.getLocale()
    );

    // Get our weeks names, in the current locale
    Map<Integer, String> weeks = CalendarRecurrenceHelper.buildLocalRecurrenceWeekNames(
      I18NUtil.getLocale()
    );

    // Turn the string into a useful map
    Map<String, String> params = CalendarRecurrenceHelper.extractRecurrenceRule(
      event
    );
    ...
    if (params.containsKey("FREQ")) {
      ...
      if ("WEEKLY".equals(freq)) {
        ...
        if (params.get("BYMONTHDAY") != null) {
          ...
          text.append(
            weeks.get((Integer.parseInt(params.get("BYSETPOS")))) + " "
          );

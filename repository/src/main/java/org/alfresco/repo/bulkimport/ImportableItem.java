...
/**
 * This class is a DTO that represents an "importable item" - a series of files that represent a
 * single node (content OR space) in the repository.
 *
 * @since 4.0
 */
public final class ImportableItem {
    ...
    public boolean isValid() {
        return (headRevision.contentFileExists()
                || headRevision.metadataFileExists()
                || hasVersionEntries());
    }
    ...
    public class ContentAndMetadata {
                ...
                if (contentFileExists) {
                    try {
                        BasicFileAttributes attrs =
                                Files.readAttributes(contentFile, BasicFileAttributes.class);
                        ...
                        logger.error(
                                "Attributes for file '"
                                        + FileUtils.getFileName(contentFile)
                                        + "' could not be read.",
                                e);
                    }
                }
        ...
        public final FileType getContentFileType() {
            if (!contentFileExists()) {
                throw new IllegalStateException(
                        "Cannot determine content file type if content file doesn't exist.");
            }
        ...
        public final long getContentFileSize() {
            if (!contentFileExists()) {
                throw new IllegalStateException(
                        "Cannot determine content file size if content file doesn't exist.");
            }
        ...
        public final Date getContentFileCreatedDate() {
            if (!contentFileExists()) {
                throw new IllegalStateException(
                        "Cannot determine content file creation date if content file doesn't"
                                + " exist.");
            }
        ...
        public final Date getContentFileModifiedDate() {
            if (!contentFileExists()) {
                throw new IllegalStateException(
                        "Cannot determine content file modification date if content file doesn't"
                                + " exist.");
            }
        ...
        public final long getMetadataFileSize() {
            if (!metadataFileExists()) {
                throw new IllegalStateException(
                        "Cannot determine metadata file size if metadata file doesn't exist.");
            }
    ...
    /** @since 4.0 */
    public class VersionedContentAndMetadata extends ContentAndMetadata

...
/**
 * Public REST API: Centralises access to favourites functionality and maps between representations
 * repository and api representations.
 *
 * @author steveglover
 * @since publicapi1.0
 */
public class FavouritesImpl implements Favourites {
    ...
    private CollectionWithPagingInfo<Favourite> wrap(
            Paging paging, PagingResults<PersonFavourite> personFavourites, Parameters parameters) {
        final List<PersonFavourite> page = personFavourites.getPage();
        final List<Favourite> list =
                new AbstractList<Favourite>() {
                    @Override
        ...
        return CollectionWithPagingInfo.asPaged(
                paging, list, personFavourites.hasMoreItems(), total);
    }
    ...
    public Favourite addFavourite(String personId, Favourite favourite, Parameters parameters) {
        ...
        if (target == null) {
            ...
            SiteInfo siteInfo =
                    sites.validateSite(new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, guid));
            ...
            if (!nodes.nodeMatches(
                    nodeRef, Collections.singleton(ContentModel.TYPE_CONTENT), null)) {
    ...
    public CollectionWithPagingInfo<Favourite> getFavourites(
            String personId, final Parameters parameters) {
        personId = people.validatePerson(personId, true);
        ...
        QueryHelper.walk(
                ...
                new WalkerCallbackAdapter() {
                    ...
                    public void or() {
                        ...
                        // need to do anything here.  If we don't override it then it will be
                        // assumed
                    ...
                    public void exists(String filteredByClient, boolean negated) {
                        if (filteredByClient != null) {
                            ...
                            if (idx == -1 || idx == filteredByClient.length()) {
                                ...
                                String filtertype =
                                        filteredByClient.substring(idx + 1).toUpperCase();
        ...
        final PagingResults<PersonFavourite> favourites =
                favouritesService.getPagedFavourites(
                        personId, filterTypes, sortProps, Util.getPagingRequest(paging));
    ...
    private Parameters getDefaultParameters(String personId, String favouriteId) {
        Params.RecognizedParams recognizedParams =
                new Params.RecognizedParams(
                        null, null, null, null, Collections.emptyList(), null, null, null, false);

...
public class LuceneUtils {
   ...
   * @since 4.0
   */
  private static final SimpleDateFormat LUCENE_DATETIME_FORMAT = new SimpleDateFormat(
  ...
  public static String createDateRangeQuery(
    ...
    if (propDef == null) {
      throw new NullPointerException(
        "dateProperty '" + dateProperty + "' not recognised."
      );
    } else {
      final QName propDefType = propDef.getDataType().getName();
      if (
        !DataTypeDefinition.DATE.equals(propDefType) &&
    ...
    final String prefix = shortFormQName.substring(
      0,
      shortFormQName.indexOf(QName.NAMESPACE_PREFIX)
    );

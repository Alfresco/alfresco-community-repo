...
public class CustomModelServiceImplTest {
    ...
    public RunAsFullyAuthenticatedRule runAsRule =
            new RunAsFullyAuthenticatedRule(AuthenticationUtil.getAdminUserName());
    ...
    public void testCreateBasicInactiveModel() throws Exception {
        ...
        try {
            ...
            createModel(model, false);
            fail(
                    "Shouldn't be able to create a model with the same name as the bootstrapped"
                            + " model.");
        } catch (Exception e) {
            // Expected
        }
        ...
        PagingResults<CustomModelDefinition> result =
                transactionHelper.doInTransaction(
    ...
    public void testListTypesAspects_Empty() throws Exception {
        ...
        final M2Model model =
                M2Model.createModel(namespacePair.getSecond() + QName.NAMESPACE_PREFIX + modelName);
        model.createNamespace(namespacePair.getFirst(), namespacePair.getSecond());
    ...
    public void testCreateModelWithTypesAndAspects() throws Exception {
        ...
        M2Type m2Type =
                model.createType(namespacePair.getSecond() + QName.NAMESPACE_PREFIX + typeName);
        ...
        try {
            createModel(model, false);
            fail(
                    "Shouldn't be able to create a model without namespace imports, when type"
                            + " and/or aspect have parent name.");
        } catch (Exception ex) {
            // Expected
        }
        ...
        assertEquals(
                typeName,
                modelDefinition.getTypeDefinitions().iterator().next().getName().getLocalName());
        assertEquals(
                "cm:content",
                modelDefinition
                        .getTypeDefinitions()
        ...
        assertEquals(
                aspectName,
                modelDefinition.getAspectDefinitions().iterator().next().getName().getLocalName());
        ...
        // list all custom models's types and aspects
        {
            ...
            final PagingRequest pagingRequest = new PagingRequest(0, Integer.MAX_VALUE);
            PagingResults<TypeDefinition> allTypes =
                    transactionHelper.doInTransaction(
            ...
            PagingResults<AspectDefinition> allAspects =
                    transactionHelper.doInTransaction(
...

        {
            ...
            try {
                updateModel(modelName, model, false);
                fail(
                        "Shouldn't be able to create a type and an aspect with the same name in a"
                                + " model.");
            } catch (Exception ex) {
                // Expected
            }
    ...
    public void testUpdateModel() throws Exception {
        ...
        model.createNamespace(
                "http://www.alfresco.org/model/contenttest/1.0", namespacePair.getSecond());
        try {
            updateModel(modelName, model, false);
        ...
        model.createNamespace(
                "http://www.alfresco.org/model/content/1.0", namespacePair.getSecond());
        try {
            updateModel(modelName, model, false);
        ...
        // Test that the cache is updated correctly. This means the cache should have removed the
        // old namespace URI.
        QName aspectQName = QName.createQName("{" + namespacePair.getFirst() + "}" + aspectName);
        AspectDefinition aspectDefinition = getAspect(aspectQName);
        ...
        model =
                M2Model.createModel(
                        activeModelNamespacePair.getSecond() + QName.NAMESPACE_PREFIX + modelName);
        ...
        model =
                M2Model.createModel(
                        activeModelNamespacePair.getSecond() + QName.NAMESPACE_PREFIX + modelName);
        model.createNamespace(
                activeModelNamespacePair.getFirst(), activeModelNamespacePair.getSecond());
        try {
            updateModel(modelName, model, true); // true => as we activated the model
            fail(
                    "Shouldn't be able to update the namespace URI and namespace prefix of an"
                            + " active model.");
        } catch (ActiveModelConstraintException ax) {
            // Expected
        }
    ...
    public void testCreateDownload() throws Exception {
        ...
        String shareExtEntry =
                cmmDownloadTestUtil.getDownloadEntry(entries, shareExtExportFileName);
    ...
    private CustomModelDefinition updateModel(
            final String modelName, final M2Model m2Model, final boolean activate) {
        return transactionHelper.doInTransaction(
                new RetryingTransactionCallback<CustomModelDefinition>() {
                    public CustomModelDefinition execute() throws Exception {
    ...
    private NodeRef createDownload(final String modelName, final boolean withShareExtModule) {
        return transactionHelper.doInTransaction(
                new RetryingTransactionCallback<NodeRef>() {
                    public NodeRef execute() throws Exception {
                        NodeRef nodeRef =
                                customModelService.createDownloadNode(
                                        modelName, withShareExtModule);
                        return nodeRef;
                    }
                });
    }

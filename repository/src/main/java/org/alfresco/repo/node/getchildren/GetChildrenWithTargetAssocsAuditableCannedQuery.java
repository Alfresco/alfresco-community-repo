...
public class GetChildrenWithTargetAssocsAuditableCannedQuery
  ...
  @Override
  protected List<NodeWithTargetsEntity> queryAndFilter(
    ...
    List<NodeWithTargetsEntity> results = cannedQueryDAO.executeQuery(
      QUERY_NAMESPACE,
      QUERY_SELECT_GET_NODES,
      paramBean,
      0,
      Integer.MAX_VALUE
    );

    List<NodeWithTargetsEntity> filtered = new ArrayList<NodeWithTargetsEntity>(
      results.size()
    );
    ...
    List<Pair<? extends Object, SortOrder>> sortPairs = parameters
      .getSortDetails()
      .getSortPairs();
    ...
    if (sortPairs != null && !sortPairs.isEmpty()) {
      ...
      for (Pair<? extends Object, SortOrder> sortPair : sortPairs) {
        ...
        final NodeBackedEntityComparator comparator = new NodeBackedEntityComparator(
          sortProperty
        );
        comparators.add(
      ...
      NestedComparator<NodeBackedEntity> comparator = new NestedComparator<NodeBackedEntity>(
        comparators
      );

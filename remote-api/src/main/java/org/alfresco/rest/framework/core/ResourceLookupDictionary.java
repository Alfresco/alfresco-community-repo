...
/**
 * Used for locating resources, implements ResourceLocator Contains a ResourceDictionary (which is a
 * reference to all the resources available)
 *
 * @author Gethin James
 */
public class ResourceLookupDictionary implements ResourceLocator {
    ...
    public Map<String, String> parseTemplateVars(Map<String, String> templateVars) {
        if ("preferences".equals(templateVars.get(ResourceLocator.RELATIONSHIP_RESOURCE))) {
            // REPO-855: special case for backwards-compatibility (see
            // PersonPreferencesRelation.java)
            return templateVars;
        }
    ...
    @Override
    public ResourceWithMetadata locateEntityResource(
            Api api, String entityResource, HttpMethod httpMethod)
            throws NotFoundException, UnsupportedResourceOperationException {
        return locateRelationResource(api, entityResource, (String) null, httpMethod);
    }

    @Override
    public ResourceWithMetadata locateRelationPropertyResource(
            Api api,
        ...
        if (resource == null) {
            if (relationResource != null) {
                // Get entity/relationship resource and check if we are referencing a relation on
                // it.
            ...
            logger.warn(
                    "Unable to locate resource resource for :"
                                    ...
                                    == null
                            ? ""
                            : relationResource + " " + property == null ? "" : property);
    ...
    @Override
    public ResourceWithMetadata locateRelationResource(
            Api api, String entityResource, String relationResource, HttpMethod httpMethod)
            throws NotFoundException, UnsupportedResourceOperationException {
        String resourceKey = ResourceDictionary.resourceKey(entityResource, relationResource);
        ...
        if (resource == null) {
            if (relationResource != null) {
                ...
                resourceKey =
                        ResourceDictionary.propertyResourceKey(entityResource, relationResource);
            ...
            throw new NotFoundException(
                    "Unable to locate resource resource for :"
                            + entityResource
                            + " "
                            + (relationResource == null ? "" : relationResource));
        } else {
            ResourceOperation op = resource.getMetaData().getOperation(httpMethod);
            if (op == null) {
                throw new UnsupportedResourceOperationException();
     ...
     *
     * <p>This will probably get refactored later when we work out what we are doing with the
     * discoverability model. It shouldn't create a new instance every time.
     */
    @Override
    public ResourceWithMetadata locateResource(
            Api api, Map<String, String> templateVars, HttpMethod httpMethod) {
        ...
        if (StringUtils.isNotBlank(property2)) {
            ...
            return locateRelationPropertyResource(
                    api, collectionName, resourceName2, property2, httpMethod);
        }
        if (StringUtils.isNotBlank(property)) {
            return locateRelationPropertyResource(
                    api, collectionName, resourceName, property, httpMethod);
        }
    ...
    @Override
    public Map<String, ResourceWithMetadata> locateEmbeddedResources(
            Api api, Map<String, String> embeddedKeys) {
        if (embeddedKeys != null && !embeddedKeys.isEmpty()) {
            ...
            for (Entry<String, String> embeddedEntry : embeddedKeys.entrySet()) {
                ResourceWithMetadata res =
                        locateEntityResource(api, embeddedEntry.getValue(), HttpMethod.GET);
    ...
    @Override
    public Map<String, ResourceWithMetadata> locateRelationResource(
            Api api, String entityKey, Collection<String> relationshipKeys, HttpMethod httpMethod) {

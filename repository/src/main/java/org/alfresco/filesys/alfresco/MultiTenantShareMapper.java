...
public class MultiTenantShareMapper
  implements ShareMapper, ConfigurationListener, InitializingBean {

  //	Server configuration and configuration sections
  private static final Log logger = LogFactory.getLog(
    MultiTenantShareMapper.class
  );
  ...
  public void afterPropertiesSet() {
    ...
    m_filesysConfig =
      (FilesystemsConfigSection) m_config.getConfigSection(
    ...
    m_alfrescoConfig =
      (AlfrescoConfigSection) m_config.getConfigSection(
        AlfrescoConfigSection.SectionName
      );

    if (
      m_filesysConfig == null || m_alfrescoConfig == null
  ...
  public SharedDevice findShare(
    ...
    if (
      m_alfrescoConfig.getTenantService().isEnabled() &&
    ...
    SharedDevice share = m_filesysConfig
      .getShares()
      .findShare(name, typ, false);
    ...
    if (
      ...
      share.getContext().isAvailable() == false
    ) share = null;
  ...
  public void deleteShares(SrvSession sess) {
    ...
    if (sess.hasDynamicShares() == false) return;
   ...
   */
  public SharedDeviceList getShareList(
    String host,
    SrvSession sess,
    boolean allShares
  ) {
    ...
    if (m_filesysConfig == null) return null;
  ...
  public int configurationChanged(
    ...
    if (id == ConfigId.ConfigSection) {
      ...
      if (newVal instanceof FilesystemsConfigSection) m_filesysConfig =
        (FilesystemsConfigSection) newVal;
      // Or the Alfresco config

      else if (newVal instanceof AlfrescoConfigSection) m_alfrescoConfig =
        (AlfrescoConfigSection) newVal;
   ...
   * @exception InvalidUserException
   */
  private final SharedDevice findTenantShare(
    String host,
    String name,
    int typ,
    SrvSession sess,
   ...
   * @return SharedDeviceList
   */
  private final SharedDeviceList getTenantShareList(
    String host,
    SrvSession sess,
    boolean allShares
  ) {
    ...
    String tenantDomain = m_alfrescoConfig
      .getTenantService()
  ...
  private final DiskSharedDevice createTenantShare(String tenantDomain) {
    ...
    NodeRef rootNodeRef = new NodeRef(
    ...
    ContentContext diskCtx = new ContentContext(
      m_tenantShareName,
      getStoreName(),
      getRootPath(),
      rootNodeRef
    ...
    if (m_config instanceof ServerConfigurationBean) {
      ...
      throw new AlfrescoRuntimeException(
        "configuration error, unknown configuration bean"
      );
    }

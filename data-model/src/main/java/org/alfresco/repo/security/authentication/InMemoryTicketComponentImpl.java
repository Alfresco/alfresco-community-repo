...
public class InMemoryTicketComponentImpl implements TicketComponent {
  ...
  private static Log logger = LogFactory.getLog(
    InMemoryTicketComponentImpl.class
  );
   ...
   */
  public void setUsernameToTicketIdCache(
    SimpleCache<String, String> usernameToTicketIdCache
  ) {
  ...
  public void setOneOff(boolean oneOff) {
    ...
    if (this.oneOff) {
      logger.warn(
        "The 'oneOff' feature has been deprecated and will be removed in a future version. " +
  ...
  private void removeFromCache(String ticketId) {
    ...
    if (ticket != null) {
      ...
      String actualUserTicketIdFromCache = usernameToTicketIdCache.get(
        username
      );
      if (ticketId.equals(actualUserTicketIdFromCache)) {
        if (logger.isTraceEnabled()) {
          logger.trace(
            "Removing ticketId from usernameToTicketIdCache for: " + username
          );
        }
  ...
  public String getNewTicket(String userName) throws AuthenticationException {
    ...
    String ticketString =
      GRANTED_AUTHORITY_TICKET_PREFIX + ticket.getTicketId();
  ...
  private String getTicketKey(String ticketString) {
    if (ticketString == null) {
      ...
      ticketString.length() < GRANTED_AUTHORITY_TICKET_PREFIX.length()
    ) {
      throw new AuthenticationException(
        ticketString + " is an invalid ticket format"
      );
    }

    String key = ticketString.substring(
  ...
  public static class Ticket implements Serializable {
    ...
    Ticket(
      ...
      final String guid = UUIDGenerator
        .getInstance()
    ...
    private static String computeTicketId(
      ...
      String encode =
        expires.toString() +
        getExpireDateAsString(expiryDate) +
        userName +
        guid;
      ...
      try {
        ...
        ticketId =
          new String(Hex.encodeHex(digester.digest(encode.getBytes())));
        ...
        case AFTER_INACTIVITY:
          ...
          if (hasExpired(now)) {
            ...
            if (remaining.compareTo(testDuration) < 0) {
              if (logger.isTraceEnabled()) {
                logger.trace(
                  "AFTER_INACTIVITY case, Creating new ticket based on the current one that expires at: " +
                  expiryDate
                );
              }
              return new Ticket(

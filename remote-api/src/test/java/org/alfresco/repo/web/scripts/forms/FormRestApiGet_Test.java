...
public class FormRestApiGet_Test extends AbstractTestFormRestApi {
  ...
  protected JSONObject createItemJSON(NodeRef nodeRef) throws Exception {
    ...
    builder
      .append(nodeRef.getStoreRef().getProtocol())
      .append("/")
      .append(nodeRef.getStoreRef().getIdentifier())
      .append("/")
  ...
  public void testResponseContentType() throws Exception {
    ...
    Response rsp = sendRequest(
      new PostRequest(FORM_DEF_URL, jsonPostString, APPLICATION_JSON),
      200
    );
  ...
  public void testGetFormForNonExistentNode() throws Exception {
    ...
    String missingId =
      this.referencingDocNodeRef.getId().replaceAll("\\d", "x");
    NodeRef missingNodeRef = new NodeRef(
      this.referencingDocNodeRef.getStoreRef(),
      missingId
    );
  ...
  public void testJsonUpperStructure() throws Exception {
    ...
    JSONObject jsonParsedObject = new JSONObject(
      new JSONTokener(jsonResponseString)
    );
  ...
  public void testJsonFormData() throws Exception {
    ...
    for (Iterator iter = formDataObject.keys(); iter.hasNext();) {
      ...
      assertEquals(
        "Did not expect to find a colon char in " + nextFieldName,
        -1,
        nextFieldName.indexOf(':')
      );
    ...
    assertTrue(
      "Expected more than " +
      threshold +
      " entries in formData. Actual: " +
      actualKeyCount,
      actualKeyCount > threshold
    );
  }
    ...
    for (int i = 0; i < fieldsArray.length(); i++) {
      ...
      for (String s : fieldKeys) {
        if (s.equals("mandatory") || s.equals("protectedField")) {
          assertEquals(
            "JSON booleans should be actual booleans.",
            java.lang.Boolean.class,
            nextJsonObject.get(s).getClass()
          );
        }
      }
    }
  ...
  public void testJsonSelectedFields() throws Exception {
    ...
    JSONObject formDataObject = (JSONObject) rootDataObject.get("formData");
    assertNotNull(
      "Expected to find cm:name data",
      formDataObject.get(nameFieldDataKey)
    );
    assertNotNull(
      "Expected to find cm:title data",
      formDataObject.get(titleFieldDataKey)
    );

...
public class TypedPropertyValueGetter {
  ...
  public Serializable getValue(Object value, PropertyDefinition propDef) {
    ...
    if (isBooleanProperty(propDef)) {
      ...
      if (
        isDateProperty(propDef) &&
        !ISO8601DateFormat.isTimeComponentDefined(valStr)
      ) {
    ...
    if (value instanceof Serializable) {
      ...
      throw new FormException(
        "Property values must be of a Serializable type! Value type: " +
        value.getClass()
      );
    }
  }
  ...
  private boolean isTextProperty(PropertyDefinition propDef) {
    return (
      propDef.getDataType().getName().equals(DataTypeDefinition.TEXT) ||
  ...
  private Serializable processLocaleValue(Object value) {
    if (value instanceof String) {
      ...
      throw new FormException(
        "Locale property values must be represented as a String! Value is of type: " +
        value.getClass()
      );
    }
  }
  ...
  private Serializable processMultiValuedType(Object value) {
    ...
    if (value instanceof String) {
      ...
      throw new FormException(
        "The value is an unsupported multi-value type: " + value
      );
    }
  }

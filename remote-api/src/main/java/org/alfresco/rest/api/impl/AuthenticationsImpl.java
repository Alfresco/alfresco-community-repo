...
public class AuthenticationsImpl implements Authentications {
    ...
    public LoginTicketResponse createTicket(LoginTicket loginRequest, Parameters parameters) {
        ...
        try {
            ...
            authenticationService.authenticate(
                    loginRequest.getUserId(), loginRequest.getPassword().toCharArray());
    ...
    @Override
    public LoginTicketResponse validateTicket(
    ...
    protected void validateLoginRequest(LoginTicket loginTicket) {
        if (loginTicket == null
                || loginTicket.getUserId() == null
                || loginTicket.getPassword() == null) {
    ...
    private String getTicketFromRemoteUserMapperUserId(Parameters parameters) {
        // If we got to execute any of the code in this class, it means that, somehow, the user is
        // authenticated;
        ...
        // * the authorization type in the header is not "basic"; therefore the user was not
        // authenticated with basic auth

        // We could end up here authenticated with some other mechanism (kerberos (SSO) or other
        // custom authenticators)
        // We need to validate the bearer token so as not to open an exploit where we return the
        // alf_ticket even if
        // the value of the bearer access token is not valid;

        // Validate the bearer access token again and
        // confirm that the current authenticated user is the same user specified in the bearer
        // token
        ...
        if (httpServletRequest != null && isRemoteUserMapperActive()) {
            ...
            // We accept that the remoteUserMapper may have not been the
            // IdentityServiceRemoteUserMapper,
            // and could have been DefaultRemoteUserMapper (using External authentication), but that
            // is ok
        ...
        throw new InvalidArgumentException(
                "Can't use Alfresco Identity Services to validate the user in the bearer access"
                        + " token");
    }

    private HttpServletRequest extractHttpServletRequestFromParameters(Parameters parameters) {
        // An alternative solution would be to create some sort of ServletHttpFacade object based on
        // the information present
        // in the parameters object. But for that we need to write a lot of code and check that we
        // pass all the data required
        // by the keycloak library;

        // Parameters object is clearly not designed to give us access to the HttpServletRequest
        // object,
        ...
        if (parameters.getRequest() instanceof BufferedRequest
                && ((BufferedRequest) parameters.getRequest()).getNext()
                        instanceof PublicApiTenantWebScriptServletRequest) {
            return ((PublicApiTenantWebScriptServletRequest)
                            ((BufferedRequest) parameters.getRequest()).getNext())
    ...
    private boolean isRemoteUserMapperActive() {
        return remoteUserMapper instanceof ActivateableBean

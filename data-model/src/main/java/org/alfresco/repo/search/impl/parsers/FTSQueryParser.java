...
public class FTSQueryParser {
  ...
  public static Constraint buildFTS(
    ...
    for (String name : templates.keySet()) {
      ...
      try {
        ...
        parser.setMode(mode);
        parser.setDefaultFieldConjunction(
          defaultFieldConnective == Connective.AND ? true : false
        );
        ...
        if (templateTrees.containsKey(name.toLowerCase())) {
          throw new FTSQueryException(
            "Duplicate template of property: " + name
          );
        }
  ...
  private static CommonTree autoPhraseReWrite(
    CommonTree node,
    boolean defaultConjunction,
    RerankPhase rerankPhase
  ) {
    if (node == null) {
      return node;
    ...
    if (isAutoPhrasable(node, defaultConjunction)) {
      ...
      for (Object current : node.getChildren()) {
        ...
        if (
          (child.getType() == FTSParser.MANDATORY) ||
          (child.getType() == FTSParser.DEFAULT)
        ) {
          if (child.getChildCount() > 0) {
            CommonTree term = (CommonTree) child.getChild(0);
            if (
              (term.getType() == FTSParser.TERM) && (term.getChildCount() == 1)
            ) {
              ...
              CommonTree newChild = autoPhraseReWrite(
                child,
                defaultConjunction,
                rerankPhase
              );
      ...
      if (rerankPhase == RerankPhase.SINGLE_PASS_WITH_AUTO_PHRASE) {
        ...
        for (CommonTree phrased : autoPhrased) {
          CommonTree newPhrased = autoPhraseReWrite(
            phrased,
            defaultConjunction,
            rerankPhase
          );
  ...
  private static boolean isAutoPhrasable(
    CommonTree node,
    boolean defaultConjunction
  ) {
    if (
      (node.getType() == FTSParser.CONJUNCTION) && (node.getChildCount() > 1)
    ) {
      ...
      for (Object current : node.getChildren()) {
        ...
        if (
          ...
          if (child.getChildCount() > 0) {
            ...
            if (
              (item.getType() == FTSParser.TERM) && (item.getChildCount() == 1)
            ) {
              simpleTermCount++;
            }
          }
  ...
  private static Constraint buildFTSConnective(
    CommonTree fieldReferenceNode,
    CommonTree node,
    QueryModelFactory factory,
    FunctionEvaluationContext functionEvaluationContext,
    Selector selector,
    Map<String, Column> columnMap,
    Map<String, CommonTree> templateTrees,
    String defaultField
  ) {
    ...
    for (int i = 0; i < node.getChildCount(); i++) {
      ...
      switch (subNode.getType()) {
        ...
        default:
          throw new FTSQueryException(
            "Unsupported FTS option " + subNode.getText()
          );
      }
      constraints.add(constraint);
    }
    if (
      constraints.size() == 1 && Occur.EXCLUDE != constraints.get(0).getOccur()
    ) {
  ...
  protected static class TestNodeBuilder {
    ...
    protected CommonTree build(
      ...
      if (parg != null) {
        CommonTree template = templateTrees.get(
          parg.getPropertyName().toLowerCase()
        );
        if (template != null) {
        ...
        if (hasOptionalFieldReference(testNode)) {
          if (isStarWithNoField(testNode)) {
            testNode =
              copyAndReplace(
                templateTrees.get(KEY_REPLACELONESTAR.toLowerCase()),
                testNode
              );
          } else {
  ...
  private static Constraint buildFTSTest(
    CommonTree fieldReferenceNode,
    CommonTree argNode,
    QueryModelFactory factory,
    FunctionEvaluationContext functionEvaluationContext,
    ...
    CommonTree testNode = testNodeBuilder.build(
      fieldReferenceNode,
      argNode,
      factory,
      functionEvaluationContext,
    ...
    switch (testNode.getType()) {
      ...
      case FTSParser.FG_TERM:
        ...
        if (fuzzy == null) {
          ...
          switch (termNode.getType()) {
            ...
            case FTSParser.QUESTION_MARK:
              throw new FTSQueryException(
                "Fuzzy queries are not supported with wild cards"
              );
            default:
              return buildFuzzyTerm(
                fuzzy,
                fieldReferenceNode,
                testNode,
                factory,
      ...
      default:
        throw new FTSQueryException(
          "Unsupported FTS option " + testNode.getText()
        );
    }
  }

  private static Constraint buildRange(
    CommonTree fieldReferenceNode,
    CommonTree testNode,
    QueryModelFactory factory,
    FunctionEvaluationContext functionEvaluationContext,
    ...
    LiteralArgument larg = factory.createLiteralArgument(
      FTSRange.ARG_FROM_INC,
      DataTypeDefinition.BOOLEAN,
      testNode.getChild(0).getType() == FTSParser.INCLUSIVE
    );
    functionArguments.put(larg.getName(), larg);
    larg =
      factory.createLiteralArgument(
        FTSRange.ARG_FROM,
        DataTypeDefinition.TEXT,
        getText(testNode.getChild(1))
      );
    functionArguments.put(larg.getName(), larg);
    larg =
      factory.createLiteralArgument(
    ...
    larg =
      factory.createLiteralArgument(
        FTSRange.ARG_TO_INC,
        DataTypeDefinition.BOOLEAN,
        testNode.getChild(3).getType() == FTSParser.INCLUSIVE
      );
    functionArguments.put(larg.getName(), larg);
    if (fieldReferenceNode != null) {
      PropertyArgument parg = buildFieldReference(
        FTSRange.ARG_PROPERTY,
        fieldReferenceNode,
        factory,
        functionEvaluationContext,
  ...
  private static Constraint buildProximity(
    CommonTree fieldReferenceNode,
    CommonTree testNode,
    QueryModelFactory factory,
    FunctionEvaluationContext functionEvaluationContext,
    ...
    Map<String, Argument> functionArguments = new LinkedHashMap<String, Argument>();
    LiteralArgument larg = factory.createLiteralArgument(
      FTSProximity.ARG_FIRST,
      DataTypeDefinition.TEXT,
      getText(testNode.getChild(0))
    );
    functionArguments.put(larg.getName(), larg);
    if (testNode.getChild(1).getChildCount() > 0) {
      larg =
        factory.createLiteralArgument(
          FTSProximity.ARG_SLOP,
          DataTypeDefinition.INT,
          getText(testNode.getChild(1).getChild(0))
        );
      functionArguments.put(larg.getName(), larg);
    }
    larg =
      factory.createLiteralArgument(
        FTSProximity.ARG_LAST,
        DataTypeDefinition.TEXT,
        getText(testNode.getChild(2))
      );
    functionArguments.put(larg.getName(), larg);
  ...
  private static Constraint buildExpandTerm(
    CommonTree fieldReferenceNode,
    CommonTree testNode,
    QueryModelFactory factory,
    FunctionEvaluationContext functionEvaluationContext,
    ...
    larg =
      factory.createLiteralArgument(
        FTSTerm.ARG_TOKENISATION_MODE,
        DataTypeDefinition.ANY,
        AnalysisMode.TOKENISE
      );
    functionArguments.put(larg.getName(), larg);
    if (fieldReferenceNode != null) {
      PropertyArgument parg = buildFieldReference(
        FTSTerm.ARG_PROPERTY,
        fieldReferenceNode,
        factory,
        functionEvaluationContext,
        selector,
  ...
  private static Constraint buildPhrase(
    Float fuzzy,
    CommonTree fieldReferenceNode,
    CommonTree testNode,
    QueryModelFactory factory,
    ...
    larg =
      factory.createLiteralArgument(
        FTSPhrase.ARG_TOKENISATION_MODE,
        DataTypeDefinition.ANY,
    ...
    if (fuzzy != null) {
      larg =
        factory.createLiteralArgument(
          FTSPhrase.ARG_SLOP,
          DataTypeDefinition.INT,
          Integer.valueOf(fuzzy.intValue())
        );
      functionArguments.put(larg.getName(), larg);
    }
    if (fieldReferenceNode != null) {
      PropertyArgument parg = buildFieldReference(
        FTSPhrase.ARG_PROPERTY,
        fieldReferenceNode,
        factory,
        functionEvaluationContext,
        selector,
  ...
  private static Constraint buildExactPhrase(
    Float fuzzy,
    CommonTree fieldReferenceNode,
    CommonTree testNode,
    QueryModelFactory factory,
  ...
  private static Constraint buildExactTerm(
    CommonTree fieldReferenceNode,
    CommonTree testNode,
    QueryModelFactory factory,
    FunctionEvaluationContext functionEvaluationContext,
    ...
    LiteralArgument larg = factory.createLiteralArgument(
      FTSTerm.ARG_TERM,
      DataTypeDefinition.TEXT,
      getText((List<Tree>) testNode.getChildren())
    );
    functionArguments.put(larg.getName(), larg);
    larg =
      factory.createLiteralArgument(
  ...
  @SuppressWarnings("unchecked")
  private static Constraint buildTerm(
    CommonTree fieldReferenceNode,
    CommonTree testNode,
    QueryModelFactory factory,
    FunctionEvaluationContext functionEvaluationContext,
  ...
  private static CommonTree copyAndReplace(
    CommonTree source,
    CommonTree insert
  ) {
        ...
        if (child.getType() == FTSParser.TEMPLATE) {
          if (child.getChildCount() > 1) {
            ...
            for (Object currentfieldReferenceNode : child.getChildren()) {
              ...
              CommonTree newChild = insertTreeAndFixFieldRefs(
                insert,
                fieldReferenceNode
              );
  ...
  private static CommonTree insertTreeAndFixFieldRefs(
    CommonTree source,
    CommonTree fieldReferenceNode
  ) {
  ...
  private static Constraint buildFuzzyTerm(
    Float fuzzy,
    CommonTree fieldReferenceNode,
    CommonTree testNode,
    QueryModelFactory factory,
    ...
    larg =
      factory.createLiteralArgument(
        FTSFuzzyTerm.ARG_MIN_SIMILARITY,
  ...
  @SuppressWarnings("unchecked")
  private static Constraint buildWildTerm(
    CommonTree fieldReferenceNode,
    CommonTree testNode,
    QueryModelFactory factory,
    FunctionEvaluationContext functionEvaluationContext,
  ...
  @SuppressWarnings("unchecked")
  private static Constraint buildExactWildTerm(
    CommonTree fieldReferenceNode,
    CommonTree testNode,
    QueryModelFactory factory,
    FunctionEvaluationContext functionEvaluationContext,
  ...
  @SuppressWarnings("unchecked")
  private static Constraint buildPrefixTerm(
    CommonTree fieldReferenceNode,
    CommonTree testNode,
    QueryModelFactory factory,
    FunctionEvaluationContext functionEvaluationContext,
  ...
  @SuppressWarnings("unchecked")
  private static Constraint buildExactPrefixTerm(
    CommonTree fieldReferenceNode,
    CommonTree testNode,
    QueryModelFactory factory,
    FunctionEvaluationContext functionEvaluationContext,
  ...
  public static PropertyArgument buildFieldReference(
    String argumentName,
    CommonTree fieldReferenceNode,
    QueryModelFactory factory,
    FunctionEvaluationContext functionEvaluationContext,
    ...
    if (fieldReferenceNode.getType() != FTSParser.FIELD_REF) {
      throw new FTSQueryException(
        "Not column ref  ..." + fieldReferenceNode.getText()
      );
    }
    ...
    if (columnMap != null) {
      for (Column column : columnMap.values()) {
        if (column.getAlias().equals(fieldName)) {
          ...
          PropertyArgument arg = (PropertyArgument) column
    ...
    return factory.createPropertyArgument(
      argumentName,
      functionEvaluationContext.isQueryable(fieldName),
      functionEvaluationContext.isOrderable(fieldName),
      alias,
      fieldName
    );
  }
  ...
  private static String unescape(String string) {
      ...
      if (lastWasEscape) {
        if (c == 'u') {
          if ((i + 4) < string.length()) {
            char encoded = (char) Integer.parseInt(
              string.substring(i + 1, i + 5),
              16
            );
    ...
    if (lastWasEscape) {
      throw new FTSQueryException(
        "Escape character at end of string " + string
      );
    }

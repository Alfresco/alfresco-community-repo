...
public class ActivitiPropertyConverter {
  ...
  private static Log logger = LogFactory.getLog(
    ActivitiPropertyConverter.class
  );
  private static final String ERR_CONVERT_VALUE =
    "activiti.engine.convert.value.error";
  private static final String ERR_SET_TASK_PROPS_INVALID_VALUE =
    "activiti.engine.set.task.properties.invalid.value";
  private static final String ERR_MANDATORY_TASK_PROPERTIES_MISSING =
    "activiti.engine.mandatory.properties.missing";
  ...
  public ActivitiPropertyConverter(
    ...
    this.typeManager =
      new ActivitiTaskTypeManager(factory, activitiUtil.getFormService());
  }
  ...
  public Map<QName, Serializable> getTaskProperties(Task task) {
    ...
    Map<String, Object> localVariables = taskService.getVariablesLocal(
      task.getId()
    );

    // Map the arbitrary properties
    Map<QName, Serializable> properties = mapArbitraryProperties(
      variables,
      localVariables,
      taskProperties,
      taskAssociations
    ...
    if (variables.get(outcomeVarName) != null) {
      properties.put(
        ...
        (Serializable) variables.get(outcomeVarName)
      );
    }

    List<IdentityLink> links = taskService.getIdentityLinksForTask(
      task.getId()
    );
  ...
  public Map<QName, Serializable> getPathProperties(String executionId) {
    Map<String, Object> variables = activitiUtil.getExecutionVariables(
      executionId
    );
  ...
  public List<NodeRef> getPooledActorsReference(
  ...
  private void mapPooledActors(
    ...
    if (pooledActorRefs != null) {
      properties.put(
        ...
        (Serializable) pooledActorRefs
      );
    }
  }

  public Map<QName, Serializable> getTaskProperties(
    DelegateTask task,
    ...
    if (localOnly == false) {
      ...
      Map<String, Object> executionVariables = task
        .getExecution()
        .getVariables();
  ...
  public Map<QName, Serializable> getTaskProperties(
    ...
    String formKey = (String) localVariables.get(
      ActivitiConstants.PROP_TASK_FORM_KEY
    );
    ...
    Map<String, Object> allVariables = getHistoricProcessVariables(
    ...
    properties.put(
      WorkflowModel.PROP_DESCRIPTION,
    ...
    properties.put(
      WorkflowModel.PROP_COMPLETION_DATE,
      historicTask.getEndTime()
    );
    ...
    List<String> pooledActorRefIds = (List<String>) localVariables.get(
      ActivitiConstants.PROP_POOLED_ACTORS_HISTORY
    );
  ...
  public void setDefaultTaskProperties(DelegateTask task) {
    ...
    Map<QName, Serializable> existingValues = getTaskProperties(
      task,
      typeDefinition,
      true
    );
    ...
    Serializable existingValue = existingValues.get(
      ...
      if (priorDef != null) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Task priority value (" +
            existingValue +
            ") was invalid so it was set to the default value of " +
            defaultVal +
            ". Task:" +
            task.getName()
          );
    ...
    String description = (String) existingValues.get(
      WorkflowModel.PROP_DESCRIPTION
    );
    if (description == null || description.length() == 0) {
      ...
      String processDefinitionKey =
        (
          (ProcessDefinition) ((TaskEntity) task).getExecution()
            .getProcessDefinition()
        ).getKey();
      description =
        factory.getTaskDescription(
          typeDefinition,
          factory.buildGlobalId(processDefinitionKey),
          null,
      ...
      if (description != null && description.length() > 0) {
        ...
        String descriptionKey = factory.mapQNameToName(
  ...
  public Map<QName, Serializable> getStartTaskProperties(
    ...
    properties.put(
      ...
      ActivitiConstants.START_TASK_PREFIX + historicProcessInstance.getId()
    );
    ...
    String wfDueDateKey = factory.mapQNameToName(
      WorkflowModel.PROP_WORKFLOW_DUE_DATE
    );
    ...
    if (priority == null) {
      String wfPriorityKey = factory.mapQNameToName(
    ...
    ActivitiScriptNode ownerNode = (ActivitiScriptNode) variables.get(
  ...
  public Map<String, Object> getStartVariables(
    ...
    HistoricActivityInstance startEvent = activitiUtil
      .getHistoryService()
    ...
    Map<String, Object> variables = getHistoricActivityVariables(
      startEvent.getId()
    );
    return variables;
  }
  ...
  public Map<String, Object> getHistoricProcessVariables(String processId) {
    HistoricDetailQuery query = activitiUtil
      .getHistoryService()
  ...
  private Map<QName, Serializable> mapArbitraryProperties(
    ...
    EntryTransformer<String, Object, QName, Serializable> transformer = new EntryTransformer<String, Object, QName, Serializable>() {
      ...
      public Pair<QName, Serializable> apply(Entry<String, Object> entry) {
        ...
        // on task
        if (
          taskProperties.containsKey(qname) ||
   ...
   * @return Serializable
   */
  public Serializable convertValueToPropertyType(
    Task task,
    Serializable value,
    QName propertyName
  ) {
    ...
    if (propDef != null) {
      return (Serializable) DefaultTypeConverter.INSTANCE.convert(
        propDef.getDataType(),
        value
      );
    }
  ...
  @SuppressWarnings("unchecked")
  private Map<QName, Serializable> getNewTaskProperties(
    Task task,
    Map<QName, Serializable> properties,
    ...
    if (add != null || remove != null) {
          ...
          if (existingAdd == null) {
            ...
            if (existingAdd instanceof List<?>) {
              ...
              if (toAdd.getValue().size() > 0) {
                newProperties.put(
                  toAdd.getKey(),
                  (Serializable) toAdd.getValue().get(0)
                );
              }
            }
          }
      ...
      if (remove != null) {
        ...
        for (Entry<QName, List<NodeRef>> toRemove : remove.entrySet()) {
          ...
          // association
          Serializable existingRemove = (Serializable) newProperties.get(
            toRemove.getKey()
          );
          ...
          if (existingRemove == null) {
            existingRemove =
              (Serializable) existingProperties.get(toRemove.getKey());
          }
          ...
          if (existingRemove != null) {
            if (existingRemove instanceof List<?>) {
              existingRemove =
                new ArrayList<NodeRef>((List<NodeRef>) existingRemove);
  ...
  public void setTaskProperties(
    ...
    Map<String, Object> variablesToSet = handlerRegistry.handleVariablesToSet(
      properties,
      type,
      task,
      DelegateTask.class
  ...
  private WorkflowException getInvalidPropertyValueException(
    QName key,
    Serializable value
  ) {
    String msg = messageService.getMessage(
      ERR_SET_TASK_PROPS_INVALID_VALUE,
      value,
      key
    );
  ...
  private Map<QName, Serializable> filterTaskProperties(
    ...
    if (properties != null) {
      properties.remove(
        QName.createQName(null, ActivitiConstants.PROP_POOLED_ACTORS_HISTORY)
      );
      properties.remove(
        QName.createQName(null, ActivitiConstants.PROP_TASK_FORM_KEY)
      );
    }
    return properties;
  }
   ...
   */
  public Map<String, Object> convertHistoricDetails(
      ...
      if (previous == null) {
        ...
        if (current.getTime().equals(previous.getTime())) {
          if (current.getRevision() == previous.getRevision()) {
            ...
            isMoreRecent =
              Long.valueOf(current.getId()).longValue() >
              Long.valueOf(previous.getId()).longValue();
          } else {
  ...
  public Map<String, Object> getStartVariables(
    ...
    TypeDefinition startTaskType = factory.getTaskFullTypeDefinition(
      startTaskTypeName,
      true
    );
    ...
    String description = (String) defaultProperties.get(
      WorkflowModel.PROP_DESCRIPTION
    );
    if (description == null) {
      String wfDescription = (String) defaultProperties.get(
        WorkflowModel.PROP_WORKFLOW_DESCRIPTION
      );
      String procDefKey = procDef.getKey();
      ReadOnlyProcessDefinition deployedDef = activitiUtil.getDeployedProcessDefinition(
        processDefId
      );
    ...
    if (
      !defaultProperties.containsKey(WorkflowModel.PROP_WORKFLOW_DUE_DATE) &&
    ...
    return handlerRegistry.handleVariablesToSet(
      defaultProperties,
      startTaskType,
      null,
      Void.class
  ...
  public void checkMandatoryProperties(DelegateTask task) {
    ...
    if (missingProps != null && missingProps.size() > 0) {
      String missingPropString = StringUtils.join(
        missingProps.iterator(),
        ", "
      );
      throw new WorkflowException(
        messageService.getMessage(
          ERR_MANDATORY_TASK_PROPERTIES_MISSING,
          missingPropString
        )
      );
    }
  ...
  private List<QName> getMissingMandatoryTaskProperties(DelegateTask task) {
    ...
    List<QName> missingProps = findMissingProperties(
      existingValues,
      propertyDefs
    );
    List<QName> missingAssocs = findMissingProperties(
      existingValues,
      assocDefs
    );
  ...
  private List<QName> findMissingProperties(
    Map<QName, Serializable> existingValues,
    ...
    for (Map.Entry<QName, ? extends ClassAttributeDefinition> entry : definitions.entrySet()) {
      ...
      if (
        (
          name
            .getNamespaceURI()
            .equals(NamespaceService.CONTENT_MODEL_1_0_URI) ||
  ...
  public Task updateTask(
    ...
    Map<QName, Serializable> newProperties = getNewTaskProperties(
      task,
      properties,
      add,
      remove

...
public abstract class AbstractCannedQuery<R> implements CannedQuery<R> {
     ...
     *
     * <p>A random GUID query execution ID will be generated.
     *
    ...
    public final synchronized CannedQueryResults<R> execute() {
        ...
        // Has more items beyond requested pages ? ... ie. at least one more page (with at least one
        // result)
        final boolean hasMoreItems =
                (rawResults.size() > pagingDetails.getResultsRequiredForPaging())
                        || (totalCount.getFirst() > pagingDetails.getResultsRequiredForPaging());

        results =
                new CannedQueryResults<R>() {
                    @Override
                    ...
                    public Pair<Integer, Integer> getTotalResultCount() {
                        if (parameters.getTotalResultCountMax() > 0) {
                            ...
                            throw new IllegalStateException(
                                    "Total results were not requested in parameters.");
                        }
                    }
                    ...
                    public List<R> getPage() {
                        if (finalPages.size() != 1) {
                            throw new IllegalStateException(
                                    "There must be exactly one page of results available.");
                        }
     ...
     *
     * <p>The implementation may optimally select, filter, sort and apply permissions. If not,
     * however, the subsequent post-query methods ({@link #applyPostQuerySorting(List,
     * CannedQuerySortDetails)}, {@link #applyPostQueryPermissions(List, int)} and {@link
     * #applyPostQueryPaging(List, CannedQueryPageDetails)}) can be used to trim the results as
     * required.
     *
    ...
    /**
     * Called before {@link #applyPostQueryPermissions(List, int)} to allow the results to be sorted
     * prior to permission checks. Note that the query implementation may optimally sort results
     * during retrieval, in which case this method does not need to be implemented.
     *
    ...
    protected List<R> applyPostQuerySorting(List<R> results, CannedQuerySortDetails sortDetails) {
        throw new UnsupportedOperationException(
                "Override this method if post-query sorting is required.");
    }
    ...
    /**
     * Called after the query to filter out results based on permissions. Note that the query
     * implementation may optimally only select results based on available privileges, in which case
     * this method does not need to be implemented.
     *
     * <p>Permission evaluations should continue until the requested number of results are retrieved
     * or all available results have been examined.
     *
     * @param results the results to apply permissions to
     * @param requestedCount the minimum number of results to pass the permission checks in order to
     *     fully satisfy the paging requirements
    ...
    protected List<R> applyPostQueryPermissions(List<R> results, int requestedCount) {
        throw new UnsupportedOperationException(
                "Override this method if post-query filtering is required.");
    }

    /**
     * Get the total number of available results after querying, filtering, sorting and permission
     * checking.
     *
     * <p>The default implementation assumes that the given results are the final total possible.
     *
     * @param results the results after filtering and sorting, but before paging
     * @return pair representing (a) the total number of results and (b) the estimated (or actual)
     *     number of maximum results possible for this query.
    ...
    /**
     * Called after the {@link #applyPostQuerySorting(List, CannedQuerySortDetails) sorting phase}
     * to pull out results specific to the required pages. Note that the query implementation may
     * optimally create page-specific results, in which case this method does not need to be
     * implemented.
     *
     * <p>The base implementation assumes that results are not paged and that the current results
     ...
     */
    protected List<List<R>> applyPostQueryPaging(
            List<R> results, CannedQueryPageDetails pageDetails) {
        ...
        if (skipResults == 0 && pageSize > availableResults) {
            return Collections.singletonList(
                    results); // Requesting more results in one page than are available
        ...
        int countTotal = 0;
        List<R> page =
                new ArrayList<R>(Math.min(results.size(), pageSize)); // Prevent memory blow-out
        pages.add(page);

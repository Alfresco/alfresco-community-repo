...
public class AspectsImpl extends AbstractClassImpl<Aspect> implements Aspects {
    ...
    public CollectionWithPagingInfo<Aspect> listAspects(Parameters params) {
        ...
        if (query != null && query.getModelIds() != null) {
            ...
            Set<Pair<QName, Boolean>> modelsFilter =
                    parseModelIds(query.getModelIds(), PARAM_INCLUDE_SUBASPECTS);
            aspectStream =
                    modelsFilter.stream().map(this::getModelAspects).flatMap(Collection::stream);
    ...
    public Aspect getAspect(String aspectId) {
        ...
        try {
            aspectDefinition =
                    dictionaryService.getAspect(QName.createQName(aspectId, this.namespaceService));

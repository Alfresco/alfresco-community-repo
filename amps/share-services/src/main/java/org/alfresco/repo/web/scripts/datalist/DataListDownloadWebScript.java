 ...
 * Data List Download
 *
 * <p>Exports the contents of a Data List as an Excel file
 *
 * @author Nick Burch
 */
public class DataListDownloadWebScript extends DeclarativeSpreadsheetWebScript
        ...
        if (args.get("store_type") != null) {
            ...
            if (site == null) {
                throw new WebScriptException(
                        Status.STATUS_NOT_FOUND, "Site not found with supplied name");
            }

            // Now find the data list container with in
            NodeRef container =
                    nodeService.getChildByName(
                            site.getNodeRef(), ContentModel.ASSOC_CONTAINS, args.get("container"));
            if (container == null) {
                throw new WebScriptException(
                        Status.STATUS_NOT_FOUND, "Container not found within site");
            }

            // Now get the data list itself
            list =
                    nodeService.getChildByName(
                            container, ContentModel.ASSOC_CONTAINS, args.get("list"));
        }
        if (list == null || !nodeService.exists(list)) {
            throw new WebScriptException(
                    Status.STATUS_NOT_FOUND, "The Data List could not be found");
        }
    ...
    private QName buildType(NodeRef list) {
        ...
        if (!typeS.startsWith(NamespaceService.DATALIST_MODEL_PREFIX + ":")) {
            throw new WebScriptException(
                    Status.STATUS_NOT_IMPLEMENTED, "Unexpected list type " + typeS);
        }
        QName type =
                QName.createQName(
    ...
    @Override
    protected void populateBody(Object resource, CSVPrinter csv, List<QName> properties)
            throws IOException {
            ...
            for (QName prop : properties) {
                ...
                if (val == null) {
                    ...
                    if (assocs.size() > 0) {
                        ...
                        for (AssociationRef ref : assocs) {
                            ...
                            if (ContentModel.TYPE_PERSON.equals(type)) {
                                ...
                                text.append(
                                        nodeService.getProperty(child, ContentModel.PROP_USERNAME));
                                ...
                                text.append(
                                        nodeService.getProperty(child, ContentModel.PROP_TITLE));
                            } else {
                    ...
                    if (val instanceof String) {
                        ...
                        // TODO
                        System.err.println(
                                "TODO: handle " + val.getClass().getName() + " - " + val);
                    }
                }

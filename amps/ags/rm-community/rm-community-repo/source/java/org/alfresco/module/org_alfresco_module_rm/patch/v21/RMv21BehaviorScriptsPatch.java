...
public class RMv21BehaviorScriptsPatch
  ...
  protected static final NodeRef RM_CONFIG = new NodeRef(
    StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
    "rm_config_folder"
  );

  /** old behavior scripts folder root lookup */
  protected static final NodeRef OLD_BEHAVIOR_SCRIPTS_FOLDER = new NodeRef(
    StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
    "rm_scripts"
  );

  /** new behavior scripts folder root lookup */
  private static NodeRef newBehaviorScriptsFolder = new NodeRef(
    StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
    "rm_behavior_scripts"
  );
          ...
          public Object doWork() {
            RetryingTransactionCallback<Void> callback = new RetryingTransactionCallback<Void>() {
              public Void execute() throws Throwable {
                ...
                List<FileInfo> oldBehaviorScripts = fileFolderService.listFiles(
                  OLD_BEHAVIOR_SCRIPTS_FOLDER
                );

                if (
                  oldBehaviorScripts != null && !oldBehaviorScripts.isEmpty()
                ) {
                  if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug(
                      " ... moving files from RM Scripts folder to RM Behavior Scripts folder"
                    );
                  }
                  ...
                  for (FileInfo script : oldBehaviorScripts) {
                    ...
                    fileFolderService.moveFrom(
                      script.getNodeRef(),
                      OLD_BEHAVIOR_SCRIPTS_FOLDER,
                      RMv21BehaviorScriptsPatch.newBehaviorScriptsFolder,
                      script.getName()
                    );

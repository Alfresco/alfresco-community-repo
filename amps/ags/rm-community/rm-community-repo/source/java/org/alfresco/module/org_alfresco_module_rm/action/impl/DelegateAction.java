 ...
 *
 * <p>Useful for creating a RM version of an existing action implementation.
 *
...
public class DelegateAction extends RMActionExecuterAbstractBase {
    ...
    /**
     * @param checkFrozen true if we check whether the actioned upon node reference is frozen, false
     *     otherwise
     */
        ...
        if (getNodeService().exists(actionedUponNodeRef)
            ...
            if (isAllowParameterSubstitutions()) {
                getParameterProcessorComponent()
                        .process(
                                action,
                                delegateActionExecuter.getActionDefinition(),
                                actionedUponNodeRef);
            }

...
public class WorkflowInterpreter extends BaseInterpreter {
  ...
  protected String executeCommand(String line) throws IOException {
    ...
    if (command[0].equals("help")) {
      ...
      if (command.length < 2) {
        ...
        String id = (currentWorkflowDef != null)
          ? currentWorkflowDef.getId()
          : null;
          ...
          for (WorkflowDefinition def : workflowService.getAllDefinitions()) {
            ...
            for (WorkflowInstance workflow : workflows) {
              out.println(
                ...
                " , def: " +
                workflow.getDefinition().getName() +
                " v" +
                workflow.getDefinition().getVersion()
              );
            }
          }
        } else {
        ...
        String workflowId = (command.length == 3)
          ? command[2]
          : (currentPath == null) ? null : currentPath.getInstance().getId();
        if (workflowId == null) {
        ...
        for (WorkflowPath path : paths) {
          out.println(
            "path id: " + path.getId() + " , node: " + path.getNode().getName()
          );
        }
      } else if (command[1].equals("tasks")) {
        String pathId = (command.length == 3)
          ? command[2]
          : (currentPath == null) ? null : currentPath.getId();
        if (pathId == null) {
        ...
        for (WorkflowPath path : paths) {
          ...
          for (WorkflowTransition transition : path
            .getNode()
            .getTransitions()) {
            out.println(
              " transition id: " +
              (
                (transition.getId() == null || transition.getId().equals(""))
                  ? "[default]"
                  : transition.getId()
              ) +
              " , title: " +
        ...
        for (WorkflowTimer timer : timers) {
          out.print(
            ...
            timer.getPath().getId() +
            " , node: " +
            timer.getPath().getNode().getName() +
            " , process: " +
            timer.getPath().getInstance().getId()
          );
        ...
        if (command[2].equals("tasks")) {
          ...
          out.println(AuthenticationUtil.getRunAsUser() + ":");
          List<WorkflowTask> tasks = workflowService.getPooledTasks(
            AuthenticationUtil.getRunAsUser()
          );
          for (WorkflowTask task : tasks) {
            out.println(
      ...
      if (command[1].equals("task")) {
        ...
        out.println(
          "transitions: " +
          task.getDefinition().getNode().getTransitions().length
        );
        for (WorkflowTransition transition : task
          .getDefinition()
          .getNode()
          .getTransitions()) {
          out.println(
        ...
        for (Map.Entry<QName, Serializable> prop : task
          .getProperties()
          .entrySet()) {
        ...
        for (int i = 2; i < command.length; i++) {
          ...
          if (predicateName.length == 1) {
            if (predicate[0].equals("taskId")) {
              ...
              query.setTaskName(
                QName.createQName(predicate[1], namespaceService)
              );
              ...
              for (int iOrderBy = 0; iOrderBy < orderBy.length; iOrderBy++) {
                queryOrderBy[iOrderBy] =
                  WorkflowTaskQuery.OrderBy.valueOf(orderBy[iOrderBy]);
            ...
            if (predicateName[0].equals("task")) {
              taskProps.put(
                QName.createQName(predicateName[1], namespaceService),
                predicate[1]
              );
      ...
      out.print(
        executeCommand(
          "deploy " + currentDeployEngine + " " + currentDeployResource
        )
      );
      ...
      if (command.length == 1) {
        out.println(
          ...
          (
            (currentWorkflowDef == null)
              ...
              : currentWorkflowDef.getId() +
              " , name: " +
              currentWorkflowDef.getTitle() +
              " , version: " +
              currentWorkflowDef.getVersion()
          )
        );
        out.println(
          "workflow: " +
          (
            (currentPath == null)
              ? "None"
              : currentPath.getInstance().getId() +
              " , active: " +
              currentPath.getInstance().isActive()
          )
        );
        out.println(
          "path: " +
        ...
        if (command[1].equals("definition")) {
          ...
          WorkflowDefinition def = workflowService.getDefinitionById(
            command[2]
          );
          if (def == null) {
            return "Not found.\n";
          ...
          currentPath =
            workflowService.getWorkflowPaths(instance.getId()).get(0);
          out.print(executeCommand("use"));
        } else {
          return "Syntax Error.\n";
        }
      ...
      setupStartTaskParameters(
        currentWorkflowDef.getStartTaskDefinition().metadata,
        params
      );
      ...
      out.println(
        ...
        " , def: " +
        path.getInstance().getDefinition().getTitle()
      );
      currentPath = path;
      ...
      if (command[1].equals("task")) {
        ...
        WorkflowTask task = workflowService.updateTask(
          command[2],
          params,
          null,
          null
      ...
      WorkflowPath path = workflowService.signal(
        command[1],
        getTransition(command)
      );
      ...
      if (command[1].equals("task")) {
        WorkflowTask task = workflowService.endTask(
          command[2],
          (command.length == 4) ? command[3] : null
        );
      ...
      if (command.length == 1) {
        ...
        if (command[2].equals("person")) {
          ...
          if (!multi) {
            ...
            if (auth == null) {
              throw new WorkflowException(
                "Group " + strValues[0] + " does not exist."
              );
            }
            vars.put(qname, auth);
          } else {
  ...
  private void setupStartTaskParameters(
    ...
    ClassDefinition startTaskDef = dictionaryService.getAnonymousType(
      typeDef.getName(),
      aspectNames
    );
  ...
  private void getMandatoryAspects(
  ...
  private void endStartTaskForPath(WorkflowPath path) {
    if (path != null) {
      List<WorkflowTask> tasks =
        this.workflowService.getTasksForWorkflowPath(path.id);
      if (tasks.size() == 1) {

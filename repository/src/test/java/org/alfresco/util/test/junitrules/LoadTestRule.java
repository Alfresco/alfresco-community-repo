...
/**
 * This JUnit rule can be used to turn existing test code into Load Tests. It does this in
 * conjunction with the {@link AlfrescoPeople} JUnit rule. That rule is used to {@link
 * AlfrescoPeople#AlfrescoPeople(ApplicationContextInit, int) create} a fixed number of Alfresco
 * users. Then {@link LoadTestRule} will do the following for each of your JUnit 4 &#64;Test
 * methods:
 *
 * <ul>
 *   <li>if they are annotated with the {@link LoadTest} marker annotation:
 *       <ol>
 *         <li>one Java thread for each of the users created by the {@link AlfrescoPeople} rule will
 *             be created and started.
 *         <li>each of those threads will start by authenticating as a different user from the above
 *             set.
 *         <li>each of those threads will concurrently execute the same JUnit &#64;Test method.
 *         <li>if all the concurrent threads complete execution successfully, the &#64;Test method
 *             is passed.
 *         <li>but if one or more of those concurrent threads fail, the error messages will be
 *             aggregated together into a single error for that method.
 *       </ol>
 *   <li>else they will be executed as normal and will pass or fail as normal.
 * </ul>
 *
 * <p>Example usage, where we have a 'normal' feature test and a load test for the same feature.:
 *
 * <pre>

...
public class GetDiscussionTopcisWithPostsCannedQuery
  ...
  private static final String QUERY_SELECT_GET_NODES =
    "select_GetDiscussionTopcisWithPosts";
  ...
  @Override
  protected List<NodeWithChildrenEntity> queryAndFilter(
    ...
    List<NodeWithChildrenEntity> results = cannedQueryDAO.executeQuery(
      QUERY_NAMESPACE,
    ...
    // Filter
    List<NodeWithChildrenEntity> filtered = new ArrayList<NodeWithChildrenEntity>(
      results.size()
    );
    for (NodeWithChildrenEntity result : results) {
      // Filter by topic date
      if (filterByTopicCreatedDate) {
        Date createdDate = DefaultTypeConverter.INSTANCE.convert(
          Date.class,
    ...
    Collections.sort(
      filtered,
      new Comparator<NodeWithChildrenEntity>() {
        @Override
        public int compare(
          NodeWithChildrenEntity o1,
          NodeWithChildrenEntity o2
        ) {

...
/**
 * Provides a store of node content directly to the file system. The writers are generated using
 * information from the {@link ContentContext simple content context}.
 *
 * <p>The file names obey, as they must, the URL naming convention as specified in the {@link
 * org.alfresco.repo.content.ContentStore ContentStore interface}.<br>
 * The protocols handled are:
 *
 * <ul>
 *   <li><b>{@link #STORE_PROTOCOL store}</b>: These URLs can be generated by this implementation
 *       and are file references within the root directory.
 *   <li><b>{@link #SPOOF_PROTOCOL spoof}</b>: These URLs are never generated by the implementation
 *       but represent spoofed binary text stream data.
 * </ul>
 *
 * @author Derek Hulley
 */
...
public class FileContentStore extends AbstractContentStore
     ...
     *
     * @param rootDirectoryStr the root under which files will be stored. The directory will be
     ...
     * Private: for Spring-constructed instances only.
     *
     * @param rootDirectory the root under which files will be stored. The directory will be created
    ...
    /**
     * Stores may optionally produce readers and writers that support random access. Switch this off
     * for this store by setting this to <tt>false</tt>.
     *
     * <p>This switch is primarily used during testing to ensure that the system has the ability to
     * spoof random access in cases where the store is unable to produce readers and writers that
     * allow random access. Typically, stream-based access would be an example.
     *
    ...
    /**
     * File stores may optionally be declared read-only. This is useful when configuring a store,
     * possibly temporarily, to act as a source of data but to preserve it against any writes.
     *
     ...
     *
     * <p>The store prefix is stripped off the URL and the rest of the URL used directly to create a
     * file.
     *
     * @param newContentUrl the specific URL to use, which may not be in use
     * @return Returns a new and unique file
     * @throws IOException if the file or parent directories couldn't be created or if the URL is
     *     already in use.
    ...
    /**
     * Takes the file absolute path, strips off the root path of the store and appends the store URL
     * prefix.
     *
     ...
     * @throws UnsupportedContentUrlException if the URL is invalid and doesn't support the {@link
     *     FileContentStore#STORE_PROTOCOL correct protocol}
     */
    /*package*/ File makeFile(String contentUrl) {
        ...
        if (!protocol.equals(FileContentStore.STORE_PROTOCOL)) {
            throw new UnsupportedContentUrlException(
                    this, protocol + PROTOCOL_DELIMITER + relativePath);
        }
        // get the file
    ...
    /**
     * Performs a direct check against the file for its existence. For {@link #SPOOF_PROTOCOL
     * spoofed} URLs, the URL always exists.
     */
    @Override
    ...
    /**
     * This implementation requires that the URL start with {@link FileContentStore#STORE_PROTOCOL }
     * or {@link FileContentStore#SPOOF_PROTOCOL }
     */
     ...
     * @param newContentUrl the new content url
    ...
    public ContentWriter getWriterInternal(
        ...
        try {
            ...
            FileContentWriter writer =
                    new FileContentWriter(file, contentUrl, existingContentReader);
    ...
    /**
     * Attempts to delete the content. The actual deletion is optional on the interface so it just
     * returns the success or failure of the underlying delete.
     *
     * @throws UnsupportedOperationException if the store is read-only
     * @see #setReadOnly(boolean)
     */
    ...
    /**
     * Creates a new content URL. This must be supported by all stores that are compatible with
     * Alfresco.
     *
    ...
    /**
     * Publishes an event to the application context that will notify any interested parties of the
     * existence of this content store.
     *
     * @param context the application context
     * @param extendedEventParams
     */
    private void publishEvent(
            ApplicationContext context, Map<String, Serializable> extendedEventParams) {
        context.publishEvent(new ContentStoreCreatedEvent(this, extendedEventParams));
    }

    public void onApplicationEvent(ApplicationEvent event) {
        // Once the context has been refreshed, we tell other interested beans about the existence
        // of this content store
    ...
    /**
     * Configure the FileContentStore to delete empty parent directories upon deleting a content
     * URL.
     *
    ...
    private void ensureFileInContentStore(File file) {
        ...
        if (!fileNormalizedAbsoultePath.startsWith(rootNormalizedAbsolutePath)) {
            throw new ContentIOException(
                    "Access to files outside of content store root is not allowed: " + file);
        }
    }
}

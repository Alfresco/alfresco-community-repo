...
/**
 * Repo Specific Helper class to manage transaction synchronization. This provides helpers to ensure that the necessary <code>TransactionSynchronization</code> instances are registered on behalf of the application code.
 * <p>
...
{
    /* The registrations of services is very explicit on the interface. This is to convey the idea that the execution of these services when the transaction completes is very explicit. As we only have a finite list of types of services that need registration, this is still OK. */
     ...
     *            <tt>true</tt> if the transaction must be read-write
     * 
     * @since 3.2
     */
    ...
    /**
     * Method that registers a <tt>NodeDaoService</tt> against the transaction. Setting this will ensure that the pre- and post-commit operations perform the necessary cleanups against the <tt>NodeDaoService</tt>.
     * <p>
     * This method can be called repeatedly as long as the service being bound implements <tt>equals</tt> and <tt>hashCode</tt>.
     * 
     * @param daoService
     *            TransactionalDao
     */
    ...
    /**
     * Method that registers an <tt>IntegrityChecker</tt> against the transaction. Setting this will ensure that the pre- and post-commit operations perform the necessary cleanups against the <tt>IntegrityChecker</tt>.
     * <p>
     * This method can be called repeatedly as long as the service being bound implements <tt>equals</tt> and <tt>hashCode</tt>.
     * 
    ...
    /**
     * Method maintained for backward compatibility: <a href="https://issues.alfresco.com/jira/browse/ACE-2801">ACE-2801: Package change for TransactionListener</a>.
     * 
    ...
    /**
     * Method that registers a <tt>Listener</tt> against the transaction.
     * <p>
     * will be better for the caller to only bind once per transaction, if possible.
     * 
     * @param listener
     *            the transaction listener tasks on
     * 
     * @since 5.0
     */
     ...
     * @param bound
     *            true if the service was just bound; false if it was previously bound
     */

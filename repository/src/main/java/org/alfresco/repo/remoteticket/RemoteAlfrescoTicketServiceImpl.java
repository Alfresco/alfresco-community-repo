...
/**
 * Service for working with a Remote Alfresco instance, which holds user credentials for the remote
 * system via the {@link RemoteCredentialsService}, and handles ticket negotiation for you.
 *
 * <p>Note - this service will be moved to the Repository Core once it has stabilised (likely after
 * OAuth support is added)
 *
 * <p>TODO OAuth support
 *
 * @author Nick Burch
 * @since 4.0.2
 */
public class RemoteAlfrescoTicketServiceImpl implements RemoteAlfrescoTicketService {
    ...
    private Map<String, Map<String, String>> remoteSystemsReqHeaders =
    ...
    /**
     * Sets the Retrying Transaction Helper, used to write changes to Credentials which turn out to
     * be invalid
     */
    ...
    /**
     * Registers the details of a new Remote System with the service. Any previous details for the
     * system will be overridden
     */
    public synchronized void registerRemoteSystem(
            String remoteSystemId, String baseUrl, Map<String, String> requestHeaders) {
    ...
    protected PasswordCredentialsInfo ensureCredentialsFound(
            String remoteSystemId, BaseCredentialsInfo credentails) {
        ...
        if (!(credentails instanceof PasswordCredentialsInfo)) {
            throw new AlfrescoRuntimeException(
                    "Credentials found, but of the wrong type, needed PasswordCredentialsInfo but"
                            + " got "
                            + credentails);
        }
    ...
    public BaseCredentialsInfo storeRemoteCredentials(
            String remoteSystemId, String username, String password)
        ...
        if (existing != null) {
            ...
            if (existing instanceof PasswordCredentialsInfoImpl) {
                ...
                if (logger.isDebugEnabled())
                    logger.debug(
                            "Unable to update existing credentials from "
                                    + existing.getNodeRef()
                                    + ", replacing");
     ...
     */
    public BaseCredentialsInfo getRemoteCredentials(String remoteSystemId)
    ...
    public boolean deleteRemoteCredentials(String remoteSystemId) throws NoSuchSystemException {
        ...
        if (logger.isDebugEnabled())
            logger.debug(
                    "Deleting credentials for "
                            + credentials.getRemoteUsername()
                            + " on "
                            + remoteSystemId);
    ...
    /**
     * Returns the current Alfresco Ticket for the current user on the remote system, fetching if it
     * isn't already cached.
     */
    public RemoteAlfrescoTicketInfo getAlfrescoTicket(String remoteSystemId)
            throws AuthenticationException, NoCredentialsFoundException, NoSuchSystemException,
        ...
        if (ticket == null) {
            ...
            if (logger.isDebugEnabled())
                logger.debug(
                        "Cached ticket found for "
                                + creds.getRemoteUsername()
                                + " on "
                                + remoteSystemId);
    ...
    /**
     * Forces a re-fetch of the Alfresco Ticket for the current user, if possible, and marks the
     * credentials as failing if not.
     */
    ...
    protected RemoteAlfrescoTicketInfo refreshTicket(
            final String remoteSystemId, final PasswordCredentialsInfo credentials)
            throws AuthenticationException, NoSuchSystemException,
                    RemoteSystemUnavailableException {
        // Check we know about the system
        String baseUrl = remoteSystemsUrls.get(remoteSystemId);
        if (baseUrl == null) {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) logger.debug("JSON Ticket Response Received: " + response);
            ...
            if (!(data instanceof JSONObject)) {
                throw new RemoteSystemUnavailableException(
                        "Invalid JSON part received: " + data.getClass() + " - from: " + response);
            }

            Object ticketJSON = ((JSONObject) data).get("ticket");
            if (ticketJSON == null) {
                throw new RemoteSystemUnavailableException(
                        "Invalid JSON received, ticket missing: " + response);
            }
            if (!(ticketJSON instanceof String)) {
                throw new RemoteSystemUnavailableException(
                        "Invalid JSON part received: "
                                + ticketJSON.getClass()
                                + " from: "
                                + response);
            }
            ...
            if (logger.isDebugEnabled())
                logger.debug(
                        "Problem communicating with remote Alfresco instance " + remoteSystemId,
                        ioEx);
            ...
            if (logger.isDebugEnabled())
                logger.debug(
                        "Invalid JSON from remote Alfresco instance " + remoteSystemId, jsonEx);

            throw new RemoteSystemUnavailableException(
                    "Invalid JSON response from remote system", jsonEx);
            ...
            if (credentials.getNodeRef() != null && credentials.getLastAuthenticationSucceeded()) {
                retryingTransactionHelper.doInTransaction(
                        new RetryingTransactionCallback<Void>() {
                            public Void execute() {
                                remoteCredentialsService.updateCredentialsAuthenticationSucceeded(
                                        false, credentials);
                                return null;
                            }
                        },

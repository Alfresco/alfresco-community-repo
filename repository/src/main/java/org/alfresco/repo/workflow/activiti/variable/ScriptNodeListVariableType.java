...
public class ScriptNodeListVariableType extends SerializableType {
    ...
    if (value != null) {
      if (!(value instanceof ActivitiScriptNodeList)) {
        throw new ActivitiException(
          "Passed value is not an instance of ActivitiScriptNodeList, cannot set variable value."
        );
      }

      // Extract all node references
      List<NodeRef> nodeRefs =
        ((ActivitiScriptNodeList) value).getNodeReferences();
  ...
  public Object getValue(ValueFields valueFields) {
    ...
    if (!(serializable instanceof List<?>)) {
      throw new ActivitiException(
        "Serializable stored in variable is not instance of List<NodeRef>, cannot get value."
      );
    }

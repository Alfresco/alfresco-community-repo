...
public class ActionsImpl implements Actions {
  ...
  public ActionDefinition getActionDefinitionById(String actionDefinitionId) {
    ...
    try {
      result =
        getActionDefinition(
          actionService.getActionDefinition(actionDefinitionId)
        );
  ...
  private ActionDefinition getActionDefinition(
    ...
    List<ActionDefinition.ParameterDefinition> paramDefs = actionDefinitionId
  ...
  @Override
  public CollectionWithPagingInfo<ActionDefinition> getActionDefinitions(
    NodeRef nodeRef,
    Parameters params
  ) {
    return actionDefinitions(
      actionService.getActionDefinitions(nodeRef),
      params
    );
  }
  ...
  private CollectionWithPagingInfo<ActionDefinition> actionDefinitions(
    ...
    if (sorting != null && !sorting.isEmpty()) {
      if (sorting.size() > 1) {
        throw new IllegalArgumentException(
          "Only a single sort field ('name' or 'title') is supported."
        );
      }
    ...
    switch (sortKey) {
      case TITLE:
        comparator =
          comparing(ActionDefinition::getTitle, nullsFirst(naturalOrder()));
        break;
      case NAME:
        comparator =
          comparing(ActionDefinition::getName, nullsFirst(naturalOrder()));
        break;
      default:
        throw new IllegalArgumentException(
          "Invalid sort key, must be either 'title' or 'name'."
        );
    }
    ...
    List<ActionDefinition> sortedPage = actionDefinitions
      ...
      .map(actionDefinition -> {
        List<ActionDefinition.ParameterDefinition> paramDefs = actionDefinition
          .getParameterDefinitions()
          .stream()
          .map(this::toModel)
          .collect(Collectors.toList());
  ...
  public Action executeAction(Action action, Parameters parameters) {
    ...
    if (
      ...
      action.getActionDefinitionId().isEmpty()
    ) {
      throw new InvalidArgumentException(
        "action.actionDefinitionId is null or empty"
      );
    }
    ...
    try {
      actionDef =
        actionService.getActionDefinition(action.getActionDefinitionId());
    } catch (NoSuchBeanDefinitionException nsbdx) {
      // Intentionally empty.
    }
    ...
    if (action.getParams() != null && !action.getParams().isEmpty()) {
      cmrAction =
        actionService.createAction(
          ...
          extractActionParams(actionDef, action.getParams())
        );
    } else {
  ...
  private Map<String, Serializable> extractActionParams(
    org.alfresco.service.cmr.action.ActionDefinition actionDefinition,
    ...
    try {
      for (Map.Entry<String, String> entry : params.entrySet()) {
        ...
        ParameterDefinition paramDef = actionDefinition.getParameterDefintion(
          propertyName
        );
  ...
  private Serializable convertValue(QName typeQName, Object propertyValue)
    ...
    if (propertyValue instanceof JSONArray) {
      ...
      if (
        typeQName.equals(DataTypeDefinition.QNAME) &&
        typeQName.toString().contains(":")
      ) {

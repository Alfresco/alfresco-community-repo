...
/**
 * Query walker that adds all properties for "equals" comparison to a map. If an unsupported
 * property or comparison operation is encountered, an {@link InvalidArgumentException} is thrown
 * unless the method {@link #handleUnmatchedComparison(int, String, String)} returns true (default
...
public class MapBasedQueryWalker extends WalkerCallbackAdapter {
    ...
    public void setSupportedGreaterThanOrEqualParameters(
            Set<String> supportedGreaterThanOrEqualParameters) {
    ...
    public void setSupportedLessThanOrEqualParameters(
            Set<String> supportedLessThanOrEqualParameters) {
    ...
    public void enableVariablesSupport(
    ...
    public void matches(String property, String value, boolean negated) {
        if (negated) {
            throw new InvalidArgumentException(
                    "Cannot use negated matching for property: " + property);
        }
        if (variablesEnabled && property.startsWith("variables/")) {
            processVariable(property, value, WhereClauseParser.MATCHES);
        } else if (supportedMatchesParameters != null
                && supportedMatchesParameters.contains(property)) {
    ...
    public void comparison(int type, String propertyName, String propertyValue, boolean negated) {
        ...
        if (type == WhereClauseParser.EQUALS) {
            if (supportedEqualsParameters != null
                    && supportedEqualsParameters.contains(propertyName)) {
            ...
            if (supportedMatchesParameters != null
                    && supportedMatchesParameters.contains(propertyName)) {
                matchesProperties.put(propertyName, propertyValue);
            } else {
                throwError = !handleUnmatchedComparison(type, propertyName, propertyValue);
            }
        } else if (type == WhereClauseParser.GREATERTHAN) {
            if (supportedGreaterThanParameters != null
                    && supportedGreaterThanParameters.contains(propertyName)) {
                greaterThanProperties.put(propertyName, propertyValue);
            } else {
                throwError = !handleUnmatchedComparison(type, propertyName, propertyValue);
            }
        } else if (type == WhereClauseParser.GREATERTHANOREQUALS) {
            if (supportedGreaterThanOrEqualParameters != null
                    && supportedGreaterThanOrEqualParameters.contains(propertyName)) {
                greaterThanOrEqualProperties.put(propertyName, propertyValue);
            } else {
                throwError = !handleUnmatchedComparison(type, propertyName, propertyValue);
            }
        } else if (type == WhereClauseParser.LESSTHAN) {
            if (supportedLessThanParameters != null
                    && supportedLessThanParameters.contains(propertyName)) {
                lessThanProperties.put(propertyName, propertyValue);
            } else {
                throwError = !handleUnmatchedComparison(type, propertyName, propertyValue);
            }
        } else if (type == WhereClauseParser.LESSTHANOREQUALS) {
            if (supportedLessThanOrEqualParameters != null
                    && supportedLessThanOrEqualParameters.contains(propertyName)) {
        ...
        if (throwError) {
            throw new InvalidArgumentException(
                    "framework.exception.InvalidProperty",
                    new Object[] {propertyName, propertyValue, WhereClauseParser.tokenNames[type]});
        } else if (negated) {
            // Throw error for the unsupported negation only if the property was valid for
            // comparison, show the more meaningful error first.
            throw new InvalidArgumentException(
                    "Cannot use NOT for " + WhereClauseParser.tokenNames[type] + " comparison.");
        }
    }
    ...
    public String getProperty(String propertyName, int type) {
        if (type == WhereClauseParser.EQUALS) {
            ...
            return greaterThanProperties.get(propertyName);
                ...
                && greaterThanOrEqualProperties != null) {
            ...
            return lessThanProperties.get(propertyName);
                ...
                && lessThanOrEqualProperties != null) {
     ...
     * @throws InvalidArgumentException when conversion to the given type was not possible due to an
        ...
        try {
            ...
            if (stringValue != null) {
                ...
                if ((result instanceof String) && (!returnType.equals(String.class))) {
                    // If a string is returned, no converter has been found (for non-String return
                    // type)
    ...
    protected void processVariable(String propertyName, String propertyValue, int type) {
            ...
            if ((propertyValue.contains("_") && indexOfSpace > propertyValue.indexOf("_"))
                ...
                try {
                    ...
                    throw new ApiException(
                            "Error translating propertyName "
                                    + propertyName
                                    + " with value "
                                    + propertyValue);
                }
        ...
        variableProperties.add(
                new QueryVariableHolder(localPropertyName, type, actualValue, scopeDef));
    }

    /**
     * Called when unsupported property is encountered or comparison operator other than equals.
     *
     * @return true, if the comparison is handles successfully. False, if an exception should be
     *     thrown because the comparison can't be handled.
     */
    protected boolean handleUnmatchedComparison(
            int type, String propertyName, String propertyValue) {
        return false;
    }
    ...
    public static class QueryVariableHolder implements Serializable {
        ...
        public QueryVariableHolder(
                String propertyName, int operator, Object propertyValue, String scope) {

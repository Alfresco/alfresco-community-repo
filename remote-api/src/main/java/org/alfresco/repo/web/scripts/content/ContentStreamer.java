...
public class ContentStreamer implements ResourceLoaderAware {
  ...
  public void streamContent(
    ...
    long lastModified = modifiedTime == null
      ? file.lastModified()
      : modifiedTime;
  ...
  protected void streamContent(
    ...
    long resourceLastModified = resourceLoader
  ...
  public void streamContentImpl(
    ...
    try {
      ...
      if (range != null) {
        if (logger.isDebugEnabled()) logger.debug(
          "Found content range header: " + range
        );
        ...
        if (range.length() > 6) {
          if (
            range.indexOf(',') != -1 &&
            (nodeRef == null || propertyQName == null)
          ) {
            if (logger.isInfoEnabled()) logger.info(
              "Multi-range only supported for nodeRefs"
            );
          } else {
  ...
  public void setAttachment(
    ...
    if (attach == true) {
      ...
      if (attachFileName != null && attachFileName.length() > 0) {
        if (logger.isDebugEnabled()) logger.debug(
          "Attaching content using filename: " + attachFileName
        );
  ...
  protected void setResponseCache(
    ...
    if (
      model != null &&
      (obj = model.get(KEY_CACHE_DIRECTIVE)) instanceof CacheDirective
    ) {
      ...
      model == null || !getBooleanValue(model.get(KEY_ALLOW_BROWSER_TO_CACHE))
    ) {

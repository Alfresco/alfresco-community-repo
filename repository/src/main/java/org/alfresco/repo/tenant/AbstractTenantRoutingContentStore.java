 ...
 *
 * <p>Note: Need to initialise before the dictionary service, in the case that models are
 * dynamically loaded for the tenant.
 */
public abstract class AbstractTenantRoutingContentStore extends AbstractRoutingContentStore
        implements ApplicationContextAware, TenantRoutingContentStore {
    ...
    protected ContentStore selectWriteStore(ContentContext ctx) {
        RetryingTransactionCallback<ContentStore> callback =
                new RetryingTransactionCallback<ContentStore>() {
                    @Override
    ...
    public List<ContentStore> getAllStores() {
        RetryingTransactionCallback<List<ContentStore>> callback =
                new RetryingTransactionCallback<List<ContentStore>>() {
                    @Override
    ...
    /**
     * Work method to build a list of stores and <b>must always be called from withing an active
     * transaction</b>.
     */
    private List<ContentStore> getAllStoresImpl() {
        ...
        if ((currentUser == null)
                || (tenantService
                        .getBaseNameUser(currentUser)
                        .equals(AuthenticationUtil.getSystemUserName()))) {
            // return enabled stores across all tenants, if running as system/null user, for
            // example, ContentStoreCleaner scheduled job
                    ...
                    TenantUtil.runAsSystemTenant(
                            new TenantRunAsWork<Void>() {
                                public Void doWork() throws Exception {
                                    allEnabledStores.add(
                                            getTenantContentStore()); // note: cache should only
                                    // contain enabled stores
                                    return null;
                                }
                            },
                            tenantDomain);

 ...
 *
 * <p>topics = 'mine' (searches for posts by the user) or 'all' (ignores the author in the search)
 * history = days in the past to search resultSize = the number of topics returned in the results
 *
...
public class ForumTopicsFilteredGet extends AbstractDiscussionWebScript {
    ...
    protected static final StoreRef SPACES_STORE =
     ...
     */
    protected PagingResults<TopicInfo> doSearch(
            Pair<String, String> searchQuery, boolean sortAscending, PagingRequest paging) {
        ...
        if (paging.getMaxItems() > 0) {
            // Multiply maxItems by 10.  This is to catch topics that have multiple replies and
            // ensure that the maximum number of topics is shown.
    ...
    protected Pair<String, String> getSearchQuery(SiteInfo site, String author, int daysAgo) {
        ...
        Pair<String, String> searchQuery =
                new Pair<String, String>(search, SearchService.LANGUAGE_FTS_ALFRESCO);
    ...
    protected PagingResults<TopicInfo> wrap(final ResultSet finalResults, PagingRequest paging) {
        ...
        Comparator<TopicInfo> lastPostDesc =

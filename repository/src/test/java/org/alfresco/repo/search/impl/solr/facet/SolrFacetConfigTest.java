...
public class SolrFacetConfigTest {
  ...
  public static void setUp() throws Exception {
    context =
      new ClassPathXmlApplicationContext(
        new String[] { "classpath:facets/test-facet-property-context.xml" },
        ApplicationContextHelper.getApplicationContext()
      );

    rawProperties =
      context.getBean("solrFacetRawPropertiesTest", Properties.class);
    facetConfig =
      context.getBean("solrFacetConfigsTest", SolrFacetConfig.class);
  }
  ...
  public void testBasic() throws Exception {
    ...
    config.setNamespaceService(
      context.getBean("namespaceService", NamespaceService.class)
    );
    try {
  ...
  public void testDefault() throws Exception {
    ...
    SolrFacetProperties contentSizeFP = defaultProps.get(
      "test_filter_content_size"
    );
    assertEquals(
      "Incorrect QNAME",
      "{http://www.alfresco.org/model/content/1.0}content.size",
      contentSizeFP.getFacetQName().toString()
    );
    assertEquals(
      "faceted-search.facet-menu.facet.size",
      contentSizeFP.getDisplayName()
    );
    assertEquals(
      "alfresco/search/FacetFilters",
      contentSizeFP.getDisplayControl()
    );
    ...
    String customValue = (String) contentSizeFP
      .getCustomProperties()
      .iterator()
      .next()
      .getValue();
    ...
    assertEquals(
      ...
      mimeTypeFP.getFacetQName().toString()
    );
    assertEquals(
      "faceted-search.facet-menu.facet.formats",
      mimeTypeFP.getDisplayName()
    );
    assertEquals(
      "alfresco/search/FacetFilters",
      mimeTypeFP.getDisplayControl()
    );
    ...
    assertEquals(
      "There shouldn't be any namespace (solr special).",
      "{}SITE",
      siteFP.getFacetQName().toString()
    );
    ...
    assertEquals(
      ...
      creatorFP.getFacetQName().toString()
    );

    String msg =
      "The value has not been overridden with the value from the custom properties";
  ...
  public void testOverrideOrder() throws Exception {
     ...
     */
    SolrFacetConfig config = new SolrFacetConfig(
      rawProperties,
      "default,custom"
    );
    config.setNamespaceService(
    ...
    SolrFacetProperties creatorFP = config
      .getDefaultFacets()
      .get("test_filter_creator");
    assertEquals(
      "Incorrect QNAME",
      "{http://www.alfresco.org/model/content/1.0}creator",
      creatorFP.getFacetQName().toString()

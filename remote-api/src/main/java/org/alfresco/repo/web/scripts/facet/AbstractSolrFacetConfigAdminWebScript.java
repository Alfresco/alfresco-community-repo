...
/**
 * This class is an abstract base class for the various web script controllers in the
 * SolrFacetService.
 *
...
public abstract class AbstractSolrFacetConfigAdminWebScript extends DeclarativeWebScript {
    private static final Log logger =
            LogFactory.getLog(AbstractSolrFacetConfigAdminWebScript.class);
    ...
    protected Map<String, Object> executeImpl(
            final WebScriptRequest req, final Status status, final Cache cache) {
        validateCurrentUser();
    ...
    protected <T> T getValue(Class<T> clazz, Object value, T defaultValue) throws JSONException {
        ...
        try {
            ...
            throw new JSONException(
                    "JSONObject[" + value + "] is not an instance of [" + clazz.getName() + "]");
        }
    }

    protected Set<CustomProperties> getCustomProperties(JSONObject customPropsJsonObj)
        ...
        for (int i = 0, length = keys.length(); i < length; i++) {
            ...
            QName name =
                    resolveToQName(getValue(String.class, jsonObj.opt(CUSTOM_PARAM_NAME), null));
    ...
    private void validateMandatoryCustomProps(Object obj, String paramName) throws JSONException {
        if (obj == null) {
            throw new JSONException(
                    "Invalid JSONObject in the Custom Properties JSON. ["
                            + paramName
                            + "] cannot be null.");
        }
    }
    ...
    protected void validateFilterID(String filterID) {
        ...
        if (matcher.find()) {
            throw new WebScriptException(
                    ...
                    "Invalid Filter Id. The characters \" * \\ < > ? / : | are not allowed. The"
                            + " Filter Id cannot end with a dot.");
        }
    }
    ...
    private Serializable getSerializableValue(Object object) throws JSONException {
        if (!(object instanceof Serializable)) {
            throw new JSONException(
                    "Invalid value in the Custom Properties JSON. ["
                            + object
                            + "] must be an instance of Serializable.");
        }
    ...
    private QName resolveToQName(String qnameStr) throws JSONException {
        ...
        if (qnameStr.charAt(0) == QName.NAMESPACE_BEGIN
                && qnameStr.indexOf("solrfacetcustomproperty") < 0) {
            throw new JSONException(
                    "Invalid name in the Custom Properties JSON. Namespace URL must be ["
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Resolved facet's custom property name ["
                            + qnameStr
                            + "] into ["
                            + typeQName
                            + "]");
    ...
    /**
     * Retrieves the named parameter as an integer, if the parameter is not present the default
     * value is returned.
     *
     ...
     */
    protected int getNonNegativeIntParameter(
            WebScriptRequest req, String paramName, int defaultValue) {
    ...
    protected abstract Map<String, Object> unprotectedExecuteImpl(

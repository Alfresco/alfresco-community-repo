...
public class ProcessDefinitionWorkflowApiTest extends EnterpriseWorkflowTestApi {
    ...
    public void testGetProcessDefinitions() throws Exception {
        ...
        ProcessDefinitionsClient processDefinitionsClient =
                publicApiClient.processDefinitionsClient();
        ListResponse<ProcessDefinition> processDefinitionsResponse =
        ...
        org.activiti.engine.repository.ProcessDefinition activitiDefinition =
        ...
        assertEquals(
                ((ProcessDefinitionEntity) activitiDefinition).isGraphicalNotationDefined(),
        ...
        JSONObject definitionListObject =
    ...
    public void testGetProcessDefinitionsSorting() throws Exception {
        ...
        List<ProcessDefinition> processDefinitions =
                getProcessDefinitions(
                        processDefinitionsClient, "(category = 'http://alfresco.org')", "name");
        ...
        processDefinitions =
                getProcessDefinitions(
                        processDefinitionsClient, "(category = 'http://alfresco.org')", "id ASC");
        assertEquals(5, processDefinitions.size());
        List<String> ids =
                collect(
                        processDefinitions,
        ...
        processDefinitions =
                getProcessDefinitions(
                        processDefinitionsClient, "(category = 'http://alfresco.org')", "key DESC");
        assertEquals(5, processDefinitions.size());
        List<String> keys =
                collect(
                        processDefinitions,
    ...
    public void testGetProcessDefinitionsWhereClause() throws Exception {
        ...
        Map<String, ProcessDefinition> processDefinitionMap =
                getProcessDefinitions(
                        processDefinitionsClient, "(category = 'http://alfresco.org')");
        ...
        processDefinitionMap =
                getProcessDefinitions(processDefinitionsClient, "(category = 'unexisting')");
        assertEquals(0, processDefinitionMap.size());

        // Filter on name equals
        processDefinitionMap =
                getProcessDefinitions(
                        processDefinitionsClient, "(name = 'Adhoc Activiti Process')");
        assertTrue(processDefinitionMap.containsKey("activitiAdhoc"));
        ...
        processDefinitionMap =
                getProcessDefinitions(processDefinitionsClient, "(deploymentId='unexisting')");
        assertEquals(0, processDefinitionMap.size());

        // Filter on category matches
        processDefinitionMap =
                getProcessDefinitions(
                        processDefinitionsClient, "(category matches('%alfresco.o%'))");
        ...
        processDefinitionMap =
                getProcessDefinitions(processDefinitionsClient, "(category matches('unexisting'))");
        assertEquals(0, processDefinitionMap.size());

        // Filter on name matches
        processDefinitionMap =
                getProcessDefinitions(
                        processDefinitionsClient, "(name matches('Adhoc Activiti %'))");
        assertTrue(processDefinitionMap.containsKey("activitiAdhoc"));
        assertEquals(1, processDefinitionMap.size());

        processDefinitionMap =
                getProcessDefinitions(processDefinitionsClient, "(name matches('unexisting'))");
        assertEquals(0, processDefinitionMap.size());

        // Filter on key matches
        processDefinitionMap =
                getProcessDefinitions(processDefinitionsClient, "(key matches('activitiAd%'))");
        assertTrue(processDefinitionMap.containsKey("activitiAdhoc"));
        assertEquals(1, processDefinitionMap.size());
        ...
        try {
            processDefinitionMap =
                    getProcessDefinitions(
                            processDefinitionsClient,
                            "(category = 'http://alfresco.org' OR name = 'Adhoc Activiti"
                                    + " Process')");
            fail("Expected exception");
        } catch (PublicApiException e) {
            assertEquals(400, e.getHttpResponse().getStatusCode());
    ...
    public void testGetProcessDefinitionById() throws Exception {
        ...
        ProcessDefinitionsClient processDefinitionsClient =
                publicApiClient.processDefinitionsClient();
        ProcessDefinition adhocDefinition =
                processDefinitionsClient.findProcessDefinitionById(activitiDefinition.getId());
    ...
    public void testGetProcessDefinitionsImage() throws Exception {
        ...
        try {
            HttpResponse response =
                    processDefinitionsClient.findImageById(activitiDefinition.getId());
    ...
    public void testGetProcessDefinitionStartModel() throws Exception {
        ...
        assertEquals(
                "{http://www.alfresco.org/model/bpm/1.0}status", modelEntry.get("qualifiedName"));
        assertEquals("d:text", modelEntry.get("dataType"));
    ...
    public void testMethodNotAllowedURIs() throws Exception {
        RequestContext requestContext = initApiClientWithTestUser();
        HttpResponse response =
                publicApiClient.get("public", "process-definitions", null, null, null, null);
        ...
        assertEquals(405, response.getStatusCode());
        response =
                publicApiClient.put("public", "process-definitions", null, null, null, null, null);
        assertEquals(405, response.getStatusCode());
        ...
        response =
                publicApiClient.delete(
                        "public", "process-definitions", processDefinition.getId(), null, null);
        assertEquals(405, response.getStatusCode());
        response =
                publicApiClient.put(
                        "public",
    ...
    public void testAuthenticationAndAuthorization() throws Exception {
        ...
        // Fetching a specific process definitions as admin from another tenant should not be
        // possible
        publicApiClient.setRequestContext(otherContext);
        try {
            adhocDefinition =
    ...
    protected Map<String, ProcessDefinition> getProcessDefinitions(
            ProcessDefinitionsClient processDefinitionsClient, String whereClause)
        ...
        ListResponse<ProcessDefinition> processDefinitionsResponse =
                processDefinitionsClient.getProcessDefinitions(params);
    ...
    protected List<ProcessDefinition> getProcessDefinitions(
            ProcessDefinitionsClient processDefinitionsClient, String whereClause, String sort)
            throws PublicApiException {
        Map<String, String> params = null;
        if (whereClause != null) {
            params = Collections.singletonMap("where", whereClause);

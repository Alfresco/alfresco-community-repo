...
public class ContentDataDAOTest extends TestCase {
        ...
        RetryingTransactionCallback<Pair<Long, ContentData>> callback =
                new RetryingTransactionCallback<Pair<Long, ContentData>>() {
                    public Pair<Long, ContentData> execute() throws Throwable {
                        Pair<Long, ContentData> contentDataPair =
                                contentDataDAO.createContentData(contentData);
    ...
    private Pair<Long, ContentData> getAndCheck(
            final Long contentDataId, ContentData checkContentData) {
        RetryingTransactionCallback<Pair<Long, ContentData>> callback =
                new RetryingTransactionCallback<Pair<Long, ContentData>>() {
                    public Pair<Long, ContentData> execute() throws Throwable {
                        Pair<Long, ContentData> contentDataPair =
                                contentDataDAO.getContentData(contentDataId);
                        return contentDataPair;
                    }
                };
        ...
        assertEquals(
                "ContentData retrieved not the same as persisted: ",
                checkContentData,
    ...
    /**
     * the caveat to {@link #testEnsureCaseSensitiveStorage()} is that mimetypes must be normalized
     * to lowercase.
     *
     * @throws Exception
     */
    ...
    public void testUpdate() throws Exception {
        ...
        // Update
        contentData =
                ContentData.setMimetype(contentData, "TEXT/HTML"); // Note the upper case mimetype
    ...
    public void testContentUrlCrud() throws Exception {
        ...
        assertNull(
                "Expect null return fetching a URL",
                contentDataDAO.getContentUrl("store://someURL"));
        // Update and create
        ContentUrlEntity contentUrlEntity =
                contentDataDAO.getOrCreateContentUrl("store://" + GUID.generate());
                ...
                new ContentUrlHandler() {
                    public void handle(Long id, String contentUrl, Long orphanTime) {
                        ...
                        if (id == null || contentUrl == null || orphanTime == null) {
                            fail(
                                    "Invalid orphan data returned to handler: "
                                            + id
                                            + "-"
                                            + contentUrl
                                            + "-"
    ...
    private static final String[] ENCODINGS =
            new String[] {"utf-8", "ascii", "latin1", "wibbles", "iso-whatever"};
    private static final Locale[] LOCALES =
            new Locale[] {
                Locale.FRENCH, Locale.CHINESE, Locale.ITALIAN, Locale.JAPANESE, Locale.ENGLISH
            };
    ...
    private List<Pair<Long, ContentData>> speedTestWrite(String name, int total) {
        ...
        for (int i = 0; i < (total / 200 / 5); i++) {
            ...
            String msg =
                    String.format(
                            "   Wrote %7d rows; average is %5.2f ms per row or %5.2f rows per"
                                    + " second",
                            pairs.size(), aveMs, 1000.0 / aveMs);
    ...
    private void speedTestRead(String name, List<Pair<Long, ContentData>> pairs) {
        ...
        for (Pair<Long, ContentData> pair : pairs) {
            ...
            if (num % 1000 == 0) {
                ...
                String msg =
                        String.format(
                                "   Read %7d rows; average is %5.2f ms per row or %5.2f rows per"
                                        + " second",
                                num, aveMs, 1000.0 / aveMs);
                System.out.println(msg);
            }
            num++;
        }
    ...
    public void testCreateSpeedSingleTxn() {
        RetryingTransactionCallback<List<Pair<Long, ContentData>>> writeCallback =
                new RetryingTransactionCallback<List<Pair<Long, ContentData>>>() {
                    public List<Pair<Long, ContentData>> execute() throws Throwable {
                        return speedTestWrite(getName(), 10000);
                    }
                };
        final List<Pair<Long, ContentData>> pairs =
                txnHelper.doInTransaction(writeCallback, false, false);
        RetryingTransactionCallback<Void> readCallback =
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        speedTestRead(getName(), pairs);
                        return null;

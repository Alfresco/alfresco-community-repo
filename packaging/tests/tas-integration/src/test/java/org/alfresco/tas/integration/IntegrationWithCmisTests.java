...
public class IntegrationWithCmisTests extends IntegrationTest {
    ...
    @TestRail(
            section = {TestGroup.INTEGRATION, TestGroup.CMIS},
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify getChildren action for a large number of files from CMIS returns only"
                            + " unique values with few retries")
    public void verifyGetChildrenReturnsUniqueValues() throws Exception {
        ...
        RestRequest request =
                RestRequest.requestWithBody(HttpMethod.POST, postBody, fileCreationWebScript);
    ...
    @TestRail(
            section = {TestGroup.INTEGRATION, TestGroup.CMIS},
            executionType = ExecutionType.REGRESSION,
            description = "Verify content and thumbnail of TIF files are retrieved by CMIS ")
    ...
    public void verifyContentAndThumbnailForTifFile() throws Exception {
        ...
        RestNodeModel fileNode =
                restAPI.authenticateUser(user).withCoreAPI().usingNode(folder).createNode();
        ...
        cmisAPI.authenticateUser(user)
                ...
                .usingResource(file)
                .assertThat()
                .contentContains("Adobe Photoshop CC 2015");
        RestRenditionInfoModel renditionInfo =
                restAPI.withCoreAPI().usingNode(file).getNodeRendition("pdf");
    ...
    public void verifyContentDispositionForContentThatAreWhiteListed() throws Exception {
        ...
        STEP(
                "2. Upload the .pdf file and verify the reponse header adding the"
                        + " download=inline/attachement parameters (accepted both).");
        cmisAPI.authenticateUser(user).usingResource(sharedFolder).createFile(pdfFile);

        RestResponse response =
                restAPI.authenticateUser(user).withCMISApi().getRootObjectByLocation(pdfFile);
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        response.assertThat()
                .header(
                        "Content-Disposition",
                        String.format("inline; filename=%s", pdfFile.getName()));

        response =
                restAPI.authenticateUser(user)
                        .withCMISApi()
                        .usingParams("download=inline")
                        .getRootObjectByLocation(pdfFile);
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        response.assertThat()
                .header(
                        "Content-Disposition",
        ...
        response =
                restAPI.authenticateUser(user)
                        .withCMISApi()
                        .usingParams("download=attachment")
                        .getRootObjectByLocation(pdfFile);
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        response.assertThat()
                .header(
                        "Content-Disposition",
                        String.format("attachment; filename=%s", pdfFile.getName()));

        STEP(
                "3. Upload the .html file and verify the reponse header adding the"
                        + " download=inline/attachement parameters (accepted only attachment) .");
        ...
        response.assertThat()
                .header(
                        "Content-Disposition",
                        String.format("attachment; filename=%s", htmlFile.getName()));

        response =
                restAPI.authenticateUser(user)
                        .withCMISApi()
                        .usingParams("download=attachment")
                        .getRootObjectByLocation(htmlFile);
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        response.assertThat()
                .header(
                        "Content-Disposition",
    ...
    @TestRail(
            ...
            description =
                    "Verify that alfresco returns the correct encoding for files created via CMIS.")
    public void verifyFileEncodingUsingCMIS() throws Exception {
        STEP(
                "1. Create a folder, two text file with specific encoding content and define the"
                        + " expected encoding.");
        FileModel utf8File = FileModel.getRandomFileModel(FileType.TEXT_PLAIN, " ∮ E⋅da = Q");
        FileModel iso8859File =
                FileModel.getRandomFileModel(
                        FileType.TEXT_PLAIN,
                        "<html><head><title>aegif Mind Share Leader Generating New Paradigms by"
                                + " aegif corporation</title></head><body><p> Test"
        ...
        STEP(
                "3. Retrieve the nodes via webscripts and verify that the content type is the"
                        + " expected one (GET"
        ...
        RestRequest request =
                RestRequest.simpleRequest(
                        HttpMethod.GET,
                        fileCreationWebScript + utf8File.getNodeRefWithoutVersion());
        RestResponse response = restAPI.authenticateUser(user).process(request);
        Assert.assertEquals(response.getResponse().getStatusCode(), HttpStatus.OK.value());
        ...
        request =
                RestRequest.simpleRequest(
                        HttpMethod.GET,
                        fileCreationWebScript + iso8859File.getNodeRefWithoutVersion());
        response = restAPI.process(request);
        Assert.assertEquals(response.getResponse().getStatusCode(), HttpStatus.OK.value());
        Assert.assertEquals(response.getResponse().getContentType(), iso8859Type);

        // Commented the use of v1 RestAPI GET (nodes/{nodeId}), because it works on alfresco 5.2.N
        // or above.
        // STEP("3. Retrieve the nodes and verify that the content type is the expected one (GET
        // nodes/{nodeId}).");

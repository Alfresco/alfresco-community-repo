...
public class RulesAspect
        ...
        try {
            ...
            if (count == 0) {
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "rules folder does not exist: create new rules folder for: " + nodeRef);
                }
    ...
    /**
     * The rule folder & below will be deleted automatically in the normal way, so we don't need to
     * worry about them. But we need additional handling for any other folders which have rules
     * linked to this folder's rules. See ALF-11923, ALF-15262.
     *
     * @see
    ...
    public void beforeRemoveAspect(NodeRef nodeRef, QName aspectTypeQName) {
        ...
        this.ruleService.disableRules(nodeRef);
        try {
            for (ChildAssociationRef childAssocRef :
                    nodeService.getChildAssocs(
                            nodeRef,
                            RuleModel.ASSOC_RULE_FOLDER,
                            RuleModel.ASSOC_RULE_FOLDER,
                            false)) {
                // We are only interested in the deletion of primary associations to a rule folder,
                // which usually
                // happens when all rules in a folder are deleted and the ASPECT_RULES aspect is
                // removed
                ...
                List<ChildAssociationRef> linkedAssocs =
                        nodeService.getParentAssocs(savedRuleFolderRef);
                for (ChildAssociationRef linkAssoc : linkedAssocs) {
                    if (!linkAssoc.isPrimary()) {
                        // Remove the aspect from linked parents; this will also delete the linking
                        // secondary
                        // association
    ...
    public void beforeDeleteNode(NodeRef nodeRef) {
        // In case the event isn't triggered automatically by node service (e.g. on a cascaded node
        // tree deletion),
    ...
    private class RulesAspectCopyBehaviourCallback extends DefaultCopyBehaviourCallback {
         ...
         *
         * @return Returns {@link ChildAssocCopyAction#COPY_CHILD} for {@link
         *     RuleModel#ASSOC_RULE_FOLDER}
         */
        @Override
        public ChildAssocCopyAction getChildAssociationCopyAction(
            ...
            if (childAssocRef.getTypeQName().equals(RuleModel.ASSOC_RULE_FOLDER)) {
                ...
                super.throwExceptionForUnexpectedBehaviour(
                        copyDetails, childAssocCopyDetails.toString());

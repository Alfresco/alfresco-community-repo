 ...
 *
 * <p>Client facing API for interacting with Alfresco Repository Admin services.
 */
public interface RepoAdminService {
    ...
    /**
     * Deploy custom model (to the 'Models' space). Allows creation of new models and incremental
     * update of existing models.
     */
    @Auditable(
            parameters = {"modelStream", "modelFileName"},
     ...
     */
    @Auditable(
            parameters = {"modelStream", "modelFileName", "activate"},
            recordable = {false, true, false})
    public NodeRef deployModel(InputStream modelStream, String modelFileName, boolean activate);

    /**
     * Undeploy custom model (from the 'Models' space). Allows delete of existing models, if not
     * used. Permanently removes the model definition from the repository (all versions).
     */
    ...
    /**
     * Force an update of the usages, providing a hint on the specific updates required. If another
     * client is already performing the update, then the calling code will need to determine the
     * severity i.e. is an updated value <b>really</b> needed. Generally clients should accept that
     * the data might be slightly stale, especially since there is no way to guarantee visibility of
     * data being put into the database by other transactions.
     *
     * @param usageType the type of usage update to perform
     * @return <tt>true</tt> if the update succeeded or <tt>false</tt> if some other client was
     *     already performing the same update
     */
    public boolean updateUsage(UsageType usageType);

    /**
     * Get full information on the state of the usage limits, including errors and warnings about
     * limits in play.
     *

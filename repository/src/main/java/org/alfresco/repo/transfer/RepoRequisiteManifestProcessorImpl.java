...
public class RepoRequisiteManifestProcessorImpl
  ...
  private static final Log log = LogFactory.getLog(
    RepoRequisiteManifestProcessorImpl.class
  );
  ...
  protected void processNode(TransferManifestNormalNode node) {
    ...
    if (resolvedNodes.resolvedChild != null) {
      ...
      Map<QName, Serializable> destinationProps = nodeService.getProperties(
        destinationNode
      );
      /**
       * For each property on the source node
       */
      for (Map.Entry<QName, Serializable> propEntry : node
        .getProperties()
        .entrySet()) {
        ...
        if (
          (value != null) &&
          ContentData.class.isAssignableFrom(value.getClass())
        ) {
          /**
          ...
          if (
             ...
             */
            String partName = TransferCommons.URLToPartName(
            ...
            if (
              ...
              Serializable destFromContents = destinationProps.get(
                TransferModel.PROP_FROM_CONTENT
              );

              if (
                destFromContents != null &&
                ...
                if (contents.contains(partName)) {
                  if (log.isDebugEnabled()) {
                    log.debug(
                      "part already transferred, no need to send it again, partName:" +
                      partName +
                      ", nodeRef:" +
                      node.getNodeRef()
                    );
                  ...
                  if (log.isDebugEnabled()) {
                    log.debug(
                      "part name not transferred, requesting new content item partName:" +
                      partName +
                      ", nodeRef:" +
                      node.getNodeRef()
                    );
                ...
                if (log.isDebugEnabled()) {
                  log.debug(
                    "from contents is null, requesting new content item partName:" +
                    partName +
                    ", nodeRef:" +
                    node.getNodeRef()
                  );
      ...
      log.debug(
        "Node does not exist on destination nodeRef:" + node.getNodeRef()
      );
      ...
      for (Map.Entry<QName, Serializable> propEntry : node
        ...
        if (log.isDebugEnabled()) {
          if (value == null) {
            log.debug(
              "Received a null value for property " + propEntry.getKey()
            );
          }
        }
        if (

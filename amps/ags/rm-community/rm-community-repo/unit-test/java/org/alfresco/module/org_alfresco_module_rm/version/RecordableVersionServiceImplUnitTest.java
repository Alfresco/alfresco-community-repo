...
public class RecordableVersionServiceImplUnitTest extends BaseUnitTest {
  ...
  public void before() throws Exception {
    ...
    doReturn(123l)
      ...
      .getProperty(nodeRef, ContentModel.PROP_NODE_DBID);
    ...
    doReturn(
      generateChildAssociationRef(
        null,
        generateNodeRef(Version2Model.TYPE_QNAME_VERSION_HISTORY)
      )
    )
  ...
  public void policyAllVersionMinor() throws Exception {
    ...
    verify(mockedRecordService, times(1))
      .createRecordFromCopy(filePlan, nodeRef);
  }
  ...
  private void verifyNormalVersion() throws Exception {
    ...
    verify(mockedFilePlanService, never())
      .getUnfiledContainer(any(NodeRef.class));
    ...
    verify(mockedNodeService, never())
      .addAspect(
        eq(version),
        eq(RecordableVersionModel.PROP_RECORD_NODE_REF),
        anyMap()
      );
  }
  ...
  public void filePlanSpecifiedWithPolicy() throws Exception {
    ...
    // then the recorded version is created
    verify(mockedRecordService, times(0))
      .createRecordFromCopy(filePlan, nodeRef);
  }
  ...
  public void notVersionableCreateRecordFromVersion() {
    ...
    doReturn(false)
      .when(mockedNodeService)
      .hasAspect(nodeRef, ContentModel.ASPECT_VERSIONABLE);

    // create record from version
    recordableVersionService.createRecordFromLatestVersion(filePlan, nodeRef);

    // nothing happens
    verify(mockedRecordService, never())
      .createRecordFromCopy(eq(filePlan), any(NodeRef.class));
  }
  ...
  public void notRecordedCreateRecordFromVersion() {
    ...
    doReturn(mockedVersion)
      .when(recordableVersionService)
      .getCurrentVersion(nodeRef);
    doReturn(versionNodeRef)
      .when(recordableVersionService)
      .convertNodeRef(any(NodeRef.class));
    makePrimaryParentOf(
      versionNodeRef,
    ...
    doReturn(
      generateChildAssociationRef(versionHistoryNodeRef, newVersionNodeRef)
    )
      .when(mockedDbNodeService)
      .createNode(
    ...
    verify(mockedNodeService, times(1))
      .addAspect(
        eq(newVersionNodeRef),
        eq(Version2Model.ASPECT_VERSION),
        any(Map.class)
      );
    verify(mockedNodeService, times(1))
      .addAspect(
  ...
  public void propNotSetVersionNotDestroyed() {
    ...
    // set prop not set
    when(
      mockedDbNodeService.getProperty(
        versionNodeRef,
        RecordableVersionModel.PROP_DESTROYED
      )
    )
      .thenReturn(null);
    ...
    assertFalse(
      recordableVersionService.isRecordedVersionDestroyed(mockedVersion)
    );
  }

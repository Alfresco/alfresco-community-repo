...
public class RepoSecondaryManifestProcessorImpl
  ...
  private static final Log log = LogFactory.getLog(
    RepoSecondaryManifestProcessorImpl.class
  );
   ...
   */
  public RepoSecondaryManifestProcessorImpl(
  ...
  protected void processNode(TransferManifestNormalNode node) {
    ...
    NodeRef correspondingNodeRef = nodeResolver.resolveCorrespondingNode(
      node.getNodeRef(),
    ...
    List<ChildAssociationRef> currentAssocs = nodeService.getParentAssocs(
      correspondingNodeRef
    );
    processParentChildAssociations(
      requiredAssocs,
      currentAssocs,
      correspondingNodeRef,
      false
    ...
    List<AssociationRef> currentTargetAssocs = nodeService.getTargetAssocs(
      correspondingNodeRef,
      RegexQNamePattern.MATCH_ALL
    );
    processPeerAssociations(
      requiredTargetAssocs,
      currentTargetAssocs,
      correspondingNodeRef,
      true
    );
    ...
    List<AssociationRef> currentSourceAssocs = nodeService.getSourceAssocs(
      correspondingNodeRef,
      RegexQNamePattern.MATCH_ALL
    );
    processPeerAssociations(
      requiredSourceAssocs,
      currentSourceAssocs,
      correspondingNodeRef,
      false
    );
  ...
  private void processParentChildAssociations(
    ...
    for (ChildAssociationRef currentAssoc : currentAssocs) {
      if (!currentAssoc.isPrimary()) {
        NodeRef key = isParent
          ? currentAssoc.getChildRef()
          : currentAssoc.getParentRef();
    ...
    for (ChildAssociationRef requiredAssoc : requiredAssocs) {
      ...
      if (!requiredAssoc.isPrimary()) {
        NodeRef otherNode = isParent
          ? requiredAssoc.getChildRef()
          : requiredAssoc.getParentRef();
        ChildAssociationRef existingAssociation = currentAssocMap.remove(
          otherNode
        );
        if (existingAssociation != null) {
          ...
          if (
            !existingAssociation.getQName().equals(requiredAssoc.getQName()) ||
            !existingAssociation
              .getTypeQName()
  ...
  private class AssociationRefKey {
    ...
    public int hashCode() {
      ...
      hashCode =
        37 * hashCode + ((sourceRef == null) ? 0 : sourceRef.hashCode());
      hashCode =
        37 *
        hashCode +
        ((assocTypeQName == null) ? 0 : assocTypeQName.hashCode());
      return hashCode;
    }
  }
}

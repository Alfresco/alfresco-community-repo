...
public class OpenRecordFolderAction extends RMActionExecuterAbstractBase {
  ...
  /** I18N */
  private static final String MSG_NO_OPEN_RECORD_FOLDER =
    "rm.action.no-open-record-folder";
  ...
  protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
    if (
      ...
      if (getRecordService().isRecord(actionedUponNodeRef)) {
        ChildAssociationRef assocRef = getNodeService()
      ...
      if (getRecordFolderService().isRecordFolder(actionedUponNodeRef)) {
        Boolean isClosed = (Boolean) getNodeService()
          .getProperty(actionedUponNodeRef, PROP_IS_CLOSED);
        if (Boolean.TRUE.equals(isClosed)) {
          getNodeService()
            .setProperty(actionedUponNodeRef, PROP_IS_CLOSED, false);
        }
      } else {

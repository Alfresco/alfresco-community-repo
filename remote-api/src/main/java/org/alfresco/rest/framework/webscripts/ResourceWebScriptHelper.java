...
public class ResourceWebScriptHelper {
    ...
    /**
     * Set the id of theObj to the uniqueId. Attempts to find a set method and invoke it. If it
     * fails it just swallows the exceptions and doesn't throw them further.
     *
     * @param theObj Object
    ...
    /**
     * Looks at the object passed in and recursively expands any @EmbeddedEntityResource annotations
     * or related relationship. {@link org.alfresco.rest.framework.resource.EmbeddedEntityResource
     * EmbeddedEntityResource} is expanded by calling the ReadById method for this entity.
     *
     * <p>Either returns a ExecutionResult object or a CollectionWithPagingInfo containing a
     * collection of ExecutionResult objects.
     *
     * @param api Api
    ...
    public Object processAdditionsToTheResponse(
        ...
        if (objectToWrap instanceof CollectionWithPagingInfo<?>) {
            CollectionWithPagingInfo<?> collectionToWrap =
                    (CollectionWithPagingInfo<?>) objectToWrap;
            Object sourceEntity =
                    executeIncludedSource(api, params, entityCollectionName, collectionToWrap);
            Collection<Object> resultCollection =
                    new ArrayList(collectionToWrap.getCollection().size());
            if (!collectionToWrap.getCollection().isEmpty()) {
                for (Object obj : collectionToWrap.getCollection()) {
                    resultCollection.add(
                            processAdditionsToTheResponse(
                                    res, api, entityCollectionName, params, obj));
                }
            }
            return CollectionWithPagingInfo.asPaged(
                    collectionToWrap.getPaging(),
                    resultCollection,
                    collectionToWrap.hasMoreItems(),
                    collectionToWrap.getTotalItems(),
                    sourceEntity,
                    collectionToWrap.getContext());
        } else {
            if (BeanUtils.isSimpleProperty(objectToWrap.getClass())
                    || objectToWrap instanceof Collection) {
            ...
            Map<String, Pair<String, Method>> embeddded =
                    ResourceInspector.findEmbeddedResources(objectToWrap.getClass());
            if (embeddded != null && !embeddded.isEmpty()) {
                Map<String, Object> results =
                        executeEmbeddedResources(api, params, objectToWrap, embeddded);
                execRes.addEmbedded(results);
            }

            if (params.getRelationsFilter() != null && !params.getRelationsFilter().isEmpty()) {
                Map<String, ResourceWithMetadata> relationshipResources =
                        locator.locateRelationResource(
                                api,
                                entityCollectionName,
                                params.getRelationsFilter().keySet(),
                                HttpMethod.GET);
                String uniqueEntityId = ResourceInspector.findUniqueId(objectToWrap);
                Map<String, Object> relatedResources =
                        executeRelatedResources(api, params, relationshipResources, uniqueEntityId);
    ...
    private Object executeIncludedSource(
            Api api,
            Params params,
            String entityCollectionName,
            CollectionWithPagingInfo<?> collectionToWrap) {
        if (params.includeSource()) {
            ...
            ResourceWithMetadata res =
                    locator.locateEntityResource(api, entityCollectionName, HttpMethod.GET);
            if (res != null) {
    ...
    /**
     * Loops through the embedded Resources and executes them. The results are added to list of
     * embedded results used by the ExecutionResult object.
     *
     * @param api Api
     * @param params Params
     * @param objectToWrap Object
     ...
     * @return Map
     */
    private Map<String, Object> executeEmbeddedResources(
            Api api,
            Params params,
            Object objectToWrap,
            Map<String, Pair<String, Method>> embeddded) {
        ...
        for (Entry<String, Pair<String, Method>> embeddedEntry : embeddded.entrySet()) {
            ResourceWithMetadata res =
                    locator.locateEntityResource(
                            api, embeddedEntry.getValue().getFirst(), HttpMethod.GET);
            if (res != null) {
                Object id =
                        ResourceInspectorUtil.invokeMethod(
                                embeddedEntry.getValue().getSecond(), objectToWrap);
                if (id != null) {
                    Object execEmbeddedResult =
                            executeResource(
                                    api, params, String.valueOf(id), embeddedEntry.getKey(), res);
                    ...
                    logger.warn(
                            "Cannot embed resource with path "
                                    + embeddedEntry.getKey()
                                    + ". No unique id because the method annotated with"
                                    + " @EmbeddedEntityResource returned null.");
                }
            }
        }
        return results;
    ...
    /**
     * Loops through the related Resources and executed them. The results are added to list of
     * embedded results used by the ExecutionResult object.
     *
     * @param api Api
     * @param params Params
     ...
     * @return Map
     */
    private Map<String, Object> executeRelatedResources(
            final Api api,
            Params params,
        ...
        for (final Entry<String, ResourceWithMetadata> relation : relatedResources.entrySet()) {
            Object execResult =
                    executeResource(
                            api, params, uniqueEntityId, relation.getKey(), relation.getValue());
            if (execResult != null) {
     ...
     * @return Object
     */
    private Object executeResource(
            final Api api,
            Params params,
            final String uniqueEntityId,
        ...
        try {
            ...
            final Params executionParams =
                    Params.valueOf(paramFilter, uniqueEntityId, params.getRequest());
            final WithResponse callBack =
                    new WithResponse(
                            Status.STATUS_OK,
            ...
            logger.warn(
                    "Failed to execute a RelatedResource for "
                            + resourceKey
                            + " "

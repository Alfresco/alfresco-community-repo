 ...
 *
 * <p>The extracters themselves know how well they are able to extract metadata.
 *
...
public class MetadataExtracterRegistry {
    ...
    /**
     * Force the registry to drop its cache of extractors. This is useful for the case where an
     * extractor becomes available only after the registry has initialized the cache.
     */
    ...
    /**
     * Returns the {@link AsynchronousExtractor} if it is able to perform the extraction and is
     * enabled. Failing that it calls {@link #getExtracter(String)}.
     *
    ...
    public MetadataExtracter getExtractor(String sourceMimetype, long sourceSizeInBytes) {
        return asyncExtractEnabled
                        && asynchronousExtractor != null
    ...
    /**
     * Gets the best metadata extracter. This is a combination of the most reliable and the most
     * performant extracter.
     *
     * <p>The result is cached for quicker access next time.
     *
    ...
    /**
     * Returns the {@link AsynchronousExtractor} if it is able to perform the embedding and is
     * enabled. Failing that it calls {@link #getEmbedder(String)}.
     *
     * @param sourceSizeInBytes size of the source content.
     * @param sourceMimetype the source MIMETYPE of the extraction
     * @return Returns a metadata extractor that can extract metadata from the chosen MIME type.
    ...
    public MetadataEmbedder getEmbedder(String sourceMimetype, long sourceSizeInBytes) {
        return asyncEmbedEnabled
                        && asynchronousExtractor != null
                        && asynchronousExtractor.isEmbedderSupported(
                                sourceMimetype, sourceSizeInBytes)
                ? asynchronousExtractor
                : getEmbedder(sourceMimetype);
    }

    /**
     * Gets the best metadata embedder. This is a combination of the most reliable and the most
     * performant embedder.
     *
     * <p>The result is cached for quicker access next time.
     *

...
public class TestService implements Extensible {
    ...
    public TestService(String psmv2) {
        super();
        this.testTrait =
                new ExtendedTrait<TestTrait>(
                        AJProxyTrait.create(createTestTrait(), TestTrait.class));
        this.psmv2 = psmv2;
    }
        ...
        return new TestTrait() {
            ...
            public String traitImplOf_privateServiceMethod1(final String s) {
                return AJExtender.run(
                        new AJExtender.ExtensionBypass<String>() {
            ...
            @Override
            public void traitImplOf_publicServiceMethod3(
                    final TestService s, final List<Integer> traitIdentities) {
                AJExtender.run(
                        new AJExtender.ExtensionBypass<Void>() {

                            @Override
                            public Void run() {
                                TestService.this.publicServiceMethod3(s, traitIdentities);
                                return null;
                            }
                            ;
                        });

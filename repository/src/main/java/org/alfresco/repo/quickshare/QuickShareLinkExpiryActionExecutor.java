...
/**
 * This action executor unshares the shared link when the quick share link expiry action is
 * triggered.
 *
...
public class QuickShareLinkExpiryActionExecutor extends ActionExecuterAbstractBase {
    ...
    protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
        if (!(action instanceof QuickShareLinkExpiryAction)) {
            if (action.getActionDefinitionName()
                    .equals(QuickShareLinkExpiryActionImpl.EXECUTOR_NAME)) {
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(
                    "Unsharing the shared id ["
                            + sharedId
                            + "] for the node:"
                            + quickShareService.getTenantNodeRefFromSharedId(sharedId).getSecond());
        }
        ...
        try {
            ...
            if (ex instanceof QuickShareLinkExpiryActionException) {
                LOGGER.error(
                        "Couldn't delete the quick share expiry action ["
                                + quickShareLinkExpiryAction.getNodeRef()
                                + "] for the sharedId:"
                                + sharedId);
            } else {

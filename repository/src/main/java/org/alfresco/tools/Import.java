...
public class Import extends Tool {
     ...
     */
    protected @Override
    /*package*/ ToolContext processArgs(String[] args) throws ToolArgumentException {
            ...
            if (args[i].equals("-h") || args[i].equals("-help")) {
                ...
                if (i == args.length || args[i].length() == 0) {
                    throw new ToolArgumentException(
                            "The value <store> for the option -store must be specified");
                }
                ...
                if (i == args.length || args[i].length() == 0) {
                    throw new ToolArgumentException(
                            "The value <path> for the option -path must be specified");
                }
                context.path = args[i];
                ...
                if (i == args.length || args[i].length() == 0) {
                    throw new ToolArgumentException(
                            "The value <dir> for the option -dir must be specified");
                }
                ...
                try {
                    ...
                    throw new ToolArgumentException(
                            "The value " + args[i] + " is an invalid uuidBinding");
                }
    ...
    /*package*/ void displayHelp() {
        ...
        logError(
                " -uuidBinding CREATE_NEW, REMOVE_EXISTING, REPLACE_EXISTING, UPDATE_EXISTING,"
                        + " THROW_ON_COLLISION (default: CREATE_NEW)");
    ...
    protected int execute() throws ToolException {
        ...
        for (int i = 0; i < context.packageNames.length; i++) {
            importHandler =
                    new ZipHandler(
                            context.getSourceDir(), context.getPackageFile(i), context.encoding);
            try {
                if (context.zipFile[i]) {
                    importHandler =
                            new ZipHandler(
                                    context.getSourceDir(),
                                    context.getPackageFile(i),
                                    context.encoding);
                } else {
                    importHandler =
                            new FileHandler(
                ...
                try {
                    ...
                    importer.importView(
                            importHandler, context.getLocation(), binding, new ImportProgress());
    ...
    private class ImportProgress implements ImporterProgress {
        ...
        public void nodeCreated(
                NodeRef nodeRef, NodeRef parentRef, QName assocName, QName childName) {
            logVerbose(
                    "Imported node "
                            + nodeRef
                            + " (parent="

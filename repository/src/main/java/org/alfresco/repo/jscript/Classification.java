...
/**
 * Support class for finding categories, finding root nodes for categories and creating root
 * categories.
 *
 * @author Andy Hind
 */
public final class Classification extends BaseScopableProcessorExtension {
    ...
    public Scriptable getRootCategories(String aspect, String filter, int maxItems, int skipCount) {
        ...
        List<ChildAssociationRef> rootCategories =
                services.getCategoryService()
                        .getRootCategories(
                                storeRef, createQName(aspect), pagingRequest, true, filter)
                        .getPage();
    ...
    public Scriptable getCategoryUsage(String aspect, int maxCount) {
        List<Pair<NodeRef, Integer>> topCats =
                services.getCategoryService()

...
/* package */class M2ConstraintDefinition implements ConstraintDefinition {
  ...
  public static final String ERR_CYCLIC_REF =
    "d_dictionary.constraint.err.cyclic_ref";
  public static final String ERR_TYPE_AND_REF =
    "d_dictionary.constraint.err.type_and_ref";
  public static final String ERR_TYPE_OR_REF =
    "d_dictionary.constraint.err.type_or_ref";
  public static final String ERR_REF_NOT_FOUND =
    "d_dictionary.constraint.err.ref_not_found";
  public static final String ERR_ANON_NEEDS_PROPERTY =
    "d_dictionary.constraint.err.anon_needs_property";
  public static final String ERR_INVALID_TYPE =
    "d_dictionary.constraint.err.invalid_type";
  public static final String ERR_SIMPLE_AND_LIST =
    "d_dictionary.constraint.err.property_simple_and_list";
  public static final String ERR_CONSTRUCT_FAILURE =
    "d_dictionary.constraint.err.construct_failure";
  public static final String ERR_PROPERTY_MISMATCH =
    "d_dictionary.constraint.err.property_mismatch";
  public static final String ERR_RESERVED_PROPERTY =
    "d_dictionary.constraint.err.reserved_property";
  public static final String ERR_NAMESPACE_NOT_DEFINED =
    "d_dictionary.constraint.err.namespace_not_defined";
  ...
  /* package */M2ConstraintDefinition(
    ...
    if (constraintName == null) {
      ...
      String localName =
        m2PropertyDef.getName().getLocalName() + "_anon_" + (++anonPropCount);
      this.name =
        QName.createQName(m2PropertyDef.getName().getNamespaceURI(), localName);
      m2Constraint.setName(
        this.name.getPrefixedQName(prefixResolver).toPrefixString()
      );
    } else {
      this.name = QName.createQName(m2Constraint.getName(), prefixResolver);
      if (!model.isNamespaceDefined(name.getNamespaceURI())) {
        throw new DictionaryException(
          ERR_NAMESPACE_NOT_DEFINED,
          name.toPrefixString(),
  ...
  /* package */synchronized void resolveDependencies(
    ModelQuery query,
  ...
  private synchronized void resolveInternal(
    ModelQuery query,
    boolean enableConstraintClassLoading
  ) {
    if (constraint != null) {
    ...
    if (ref != null && type != null) {
      ...
      M2ConstraintDefinition constraintDef = (M2ConstraintDefinition) query.getConstraint(
        qnameRef
      );
      ...
      if (constraint != null) {
        ...
        if (constraintNamedValues != null) {
          for (M2NamedValue namedValue : constraintNamedValues) {
            ...
            Object value = null;
            if (
              namedValue.getSimpleValue() != null &&
              namedValue.getListValue() != null
            ) {
              throw new DictionaryException(
                ERR_SIMPLE_AND_LIST,
                shortName,
          ...
          if (
            (title != null) &&
            (beanWrapper.isWritableProperty(PROP_DESCRIPTION))
          ) {
  ...
  /* package */M2ModelDiff diffConstraint(ConstraintDefinition conDef) {
    ...
    // check title
    if (
      !EqualsHelper.nullSafeEquals(getTitle(null), conDef.getTitle(null), false)
    ) {
  ...
  /*package*/static Collection<M2ModelDiff> diffConstraintLists(
    Collection<ConstraintDefinition> previousConstraints,
    Collection<ConstraintDefinition> newConstraints
  ) {
    ...
    for (ConstraintDefinition previousConstraint : previousConstraints) {
      ...
      for (ConstraintDefinition newConstraint : newConstraints) {
        if (newConstraint.getName().equals(previousConstraint.getName())) {
          modelDiffs.add(
            ((M2ConstraintDefinition) previousConstraint).diffConstraint(
                previousConstraint
              )
          );
          found = true;

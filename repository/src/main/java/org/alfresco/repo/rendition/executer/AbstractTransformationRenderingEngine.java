...
public abstract class AbstractTransformationRenderingEngine
  ...
  private static Log logger = LogFactory.getLog(
    AbstractTransformationRenderingEngine.class
  );
   ...
   */
  public static final String PARAM_TIMEOUT_MS =
   ...
   */
  public static final String PARAM_MAX_PAGES =
   ...
   */
  public static final String PARAM_PAGE_LIMIT =
  ...
  protected void render(RenderingContext context) {
    ...
    while (true) {
      try {
        ...
        if (
          transformationOptions.getTimeoutMs() > 0 &&
          new Date().getTime() -
          startTime >
          (
        ...
        if (executionSummary != null) {
          ExecutionDetails executionDetails = actionTrackingService.getExecutionDetails(
            executionSummary
          );
    ...
    if (actionCompleted) {
      ...
      try {
        ...
        if (tempTarget == null) {
          ...
          throw new RenditionServiceException(
            "Target of transformation not present"
          );
        }
        ...
        throw new RenditionServiceException(
          TRANSFORMING_ERROR_MESSAGE + e.getCause().getMessage(),
          e.getCause()
        );
      } catch (InterruptedException e) {
  ...
  protected abstract TransformationOptions getTransformOptions(
  ...
  protected TransformationOptions getTransformOptionsImpl(
    ...
    Long readLimitTimeMs = context.getCheckedParam(
    ...
    Long maxSourceSizeKBytes = context.getCheckedParam(
    ...
    Long readLimitKBytes = context.getCheckedParam(
    ...
    Integer pageLimit = context.getCheckedParam(
      PARAM_PAGE_LIMIT,
      Integer.class
    );
    if (pageLimit != null) {
    ...
    if (getSourceOptionsSerializers() != null) {
      for (TransformationSourceOptionsSerializer sourceSerializer : getSourceOptionsSerializers()) {
        TransformationSourceOptions sourceOptions = sourceSerializer.deserialize(
          context
        );
  ...
  protected class TransformationCallable implements Callable<ContentWriter> {
    ...
    public ContentWriter call() throws Exception {
      ...
      Serializable runAsParam = context
        .getDefinition()
        .getParameterValue(AbstractRenderingEngine.PARAM_RUN_AS);
      String runAsName = runAsParam == null
        ...
        new AuthenticationUtil.RunAsWork<ContentWriter>() {
          ...
          public ContentWriter doWork() throws Exception {
            ...
            try {
              ...
              NoTransformerException | UnsupportedTransformationException ntx
            ) {
              {
                logger.debug(
              ...
              throw new RenditionServiceException(
                TRANSFORMING_ERROR_MESSAGE + ntx.getMessage(),
                ntx
              );
            }
          }

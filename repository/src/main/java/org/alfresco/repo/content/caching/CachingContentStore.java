...
public class CachingContentStore
  ...
  private ContentReader attemptCacheAndRead(
    String url,
    ContentReader backingStoreReader
  ) {
  ...
  public ContentWriter getWriter(final ContentContext context) {
    if (cacheOnInbound) {
      ...
      final BackingStoreAwareCacheWriter cacheWriter = new BackingStoreAwareCacheWriter(
        cache.getWriter(url),
        bsWriter
      );
      ...
      cacheWriter.addListener(
        new ContentStreamListener() {
          ...
          public void contentStreamClosed() throws ContentIOException {
            ...
            if (
              !quota.afterWritingCacheFile(cacheWriter.getSize()) ||
              contentUrlChanged
            ) {

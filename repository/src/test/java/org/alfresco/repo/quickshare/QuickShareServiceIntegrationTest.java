...
public class QuickShareServiceIntegrationTest {
    ...
    private static final String MODEL =
            "<?xml version='1.0' encoding='UTF-8'?><model name='lx:lxmodel'"
                + " xmlns='http://www.alfresco.org/model/dictionary/1.0'><description>LX"
                + " model</description><author>Peter"
                + " Löfgren</author><version>1.0</version><imports><import"
                + " uri='http://www.alfresco.org/model/dictionary/1.0' prefix='d' /><import"
                + " uri='http://www.alfresco.org/model/content/1.0' prefix='cm'"
                + " /></imports><namespaces><namespace uri='http://bugtestmodel' prefix='lx'"
                + " /></namespaces><constraints></constraints><types><type name='lx:doc'><title>LX"
                + " dokument</title><parent>cm:content</parent><mandatory-aspects>"
                + "<aspect>cm:generalclassifiable</aspect></mandatory-aspects></type><type"
                + " name='lx:doc2'><title>LX dokument 2</title><parent>cm:cmobject</parent></type>"
                + "</types></model>";
    ...
    public static RuleChain classChain =
            RuleChain.outerRule(testContext).around(user1).around(user2);
                    ...
                    public Void doWork() throws Exception {
                        ...
                        assertNotNull(
                                nodeService.getProperty(
                                        testNode, QuickShareModel.PROP_QSHARE_SHAREDID));
                        assertEquals(
                                user1.getUsername(),
                                nodeService.getProperty(
                                        testNode, QuickShareModel.PROP_QSHARE_SHAREDBY));
                        return null;
                    }
                });
    }
    ...
    public void testDeleteAndRestoreSharedNode() {
                ...
                AuthenticationUtil.runAs(
                        new RunAsWork<NodeRef>() {
                            ...
                            public NodeRef doWork() throws Exception {
                                ...
                                final NodeRef archivedNode =
                                        nodeArchiveService.getArchivedNode(testNode);
                                assertNotNull(archivedNode);

                                // Restore the deleted shared node from trashcan
                                RestoreNodeReport restoreNodeReport =
                                        nodeArchiveService.restoreArchivedNode(archivedNode);
        ...
        /**
         * Tests the scenario where the shared node has been deleted and restored before the fix
         * (MNT-16224). In this scenario the user should be able to un-share the restored node.
         */
        {
            // Share the test node again
            final QuickShareDTO dto = share(testNode, user1.getUsername());

            // Delete only the sharedId without removing the 'shared' aspect!(The cause of MNT-16224
            // and RA-1093)
    ...
    private QuickShareDTO share(
            final NodeRef nodeRef, final String username, final Date expiryDate) {
        return AuthenticationUtil.runAs(
                () -> quickShareService.shareContent(nodeRef, expiryDate), username);
    }
        ...
        AuthenticationUtil.runAs(
                new RunAsWork<Object>() {
                    ...
                    public Object doWork() throws Exception {
                        ...
                        Map<QName, Serializable> originalProps =
                        ...
                        NodeRef copyNodeRef =
                                copyService.copyAndRename(
                                        ...
                                        QName.createQName(
                                                NamespaceService.APP_MODEL_1_0_URI, "copy"),
                                        true);
                        ...
                        Assert.assertNull(
                                nodeService.getProperty(
                                        copyNodeRef, QuickShareModel.PROP_QSHARE_SHAREDBY));
                        Assert.assertNull(
                                nodeService.getProperty(
                                        copyNodeRef, QuickShareModel.PROP_QSHARE_SHAREDID));
                        ...
                        for (QName property : originalProps.keySet()) {
                            ...
                            Assert.assertTrue(
                                    "Mising property " + property, copyProps.containsKey(property));
                        }
                        return null;
                    }
                },
    ...
    public void cloud928() {
        ...
        attributeService.removeAttribute(
                QuickShareServiceImpl.ATTR_KEY_SHAREDIDS_ROOT, dto.getId());
                ...
                AuthenticationUtil.runAsSystem(
                        new RunAsWork<Serializable>() {
                            ...
                            public Serializable doWork() throws Exception {
                                return nodeService.getProperty(
                                        testNode, ContentModel.PROP_MODIFIED);
                            }
                        });
                ...
                AuthenticationUtil.runAsSystem(
                        new RunAsWork<String>() {
                            ...
                            public String doWork() throws Exception {
                                return (String)
                                        nodeService.getProperty(
                                                testNode, ContentModel.PROP_MODIFIER);
                            }
                        });
    ...
    public void testSharedLinkExpiryScheduling() throws Exception {
        ...
        assertEquals(
                quickShareDTO.getId(), getProperty(testNode, QuickShareModel.PROP_QSHARE_SHAREDID));
        ...
        QuickShareLinkExpiryAction expiryAction =
                getExpiryActionAndAttachSchedule(quickShareDTO.getId());
        ...
        // Check that restoring the node hasn't brought back the shared aspect or the persisted
        // expiry action
        assertFalse(hasQuickShareAspect(testNode));
        assertNull(getExpiryAction(quickShareDTO.getId()));
        assertEquals(numOfSchedules, listSchedules());
    ...
    public void testSharedLinkExpiryDateValidator() throws Exception {
        ...
        final String defaultExpiryDatePeriod =
        ...
        try {
             ...
             */
            {
                ...
                // Set the expiry date to be in the next 2 days, even though we did set the date
                // period to HOURS.
            ...
            ((QuickShareServiceImpl) directQuickShareService)
                    .setExpiryDatePeriod(defaultExpiryDatePeriod);
        }
    }

    // Test SharedLink deletion by admin user based on REPO-2819 - test does not relay on whether a
    // sharedLink
    // exists or not.
    @Test
    public void testCanDeleteSharedLinkWithAdminUserForPrivateNodes() throws Exception {
        ...
        try {
            ...
            boolean canDeleteSharedLink =
                    userCanDeleteSharedLink(nodeRefOnPrivateSite, user1.getUsername());

...
public class CacheTest extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    backingCache =
      (SimpleCache<String, ValueHolder<Object>>) ctx.getBean("backingCache");
    backingCacheNoStats =
      (SimpleCache<String, ValueHolder<Object>>) ctx.getBean(
        "backingCacheNoStats"
      );
    transactionalCache =
      (TransactionalCache<String, Object>) ctx.getBean("transactionalCache");
    transactionalCacheNoStats =
      (TransactionalCache<String, Object>) ctx.getBean(
        "transactionalCacheNoStats"
      );
  ...
  public void testRollbackCleanup() throws Exception {
    ...
    assertFalse(
      "Remove not done after rollback",
      transactionalCache.contains(NEW_GLOBAL_ONE)
    );
    assertFalse(
      "Update happened after rollback",
      transactionalCache.contains(NEW_GLOBAL_TWO)
    );
  }
  ...
  public void testTransactionalCacheWithSingleTxn() throws Throwable {
    ...
    try {
      ...
      assertNull(
        "Get didn't return null",
        transactionalCache.get(NEW_GLOBAL_ONE)
      );
      assertTrue(
        "Item was removed from backing cache",
      ...
      PostCommitCacheReader listenerReader = new PostCommitCacheReader(
        transactionalCache,
        UPDATE_TXN_THREE
      );
      ...
      // Register a post-commit cache reader to make sure that nothing blows up if the cache is hit in post-commit
      PostCommitCacheWriter listenerWriter = new PostCommitCacheWriter(
        transactionalCache,
        UPDATE_TXN_FOUR,
        "FOUR"
      );
  ...
  private class PostCommitCacheReader extends TransactionListenerAdapter {
    ...
    private PostCommitCacheReader(
      SimpleCache<String, Object> transactionalCache,
      String key
    ) {
  ...
  public void testTransactionalCacheDisableSharedCaches() throws Throwable {
    ...
    try {
      ...
      assertNull(
        "Value should not be in any cache",
        transactionalCache.get(UPDATE_TXN_THREE)
      );
   ...
   */
  public long runPerformanceTestOnCache(
    SimpleCache<String, Object> cache,
    int objectCount
  ) {
    // preload
  ...
  public void testMaxSizeOverrun() throws Exception {
    ...
    try {
      ...
      assertEquals(
        "The start value isn't correct",
        startValue,
        transactionalCache.get(startKey)
      );
      ...
      assertNull(
        "The start value didn't drop out of the cache",
        checkStartValue
      );
  ...
  private void executeAndCheck(
    ...
    Object actualValue = TransactionalCache.getSharedCacheValue(
      backingCache,
      key,
      null
    );
    assertEquals(
      "Backing cache value was not correct",
      expectedValue,
      actualValue
    );
    assertEquals(
      "Backing cache contains(key): ",
      mustContainKey,
      backingCache.contains(key)
    );
  ...
  public void testTransactionalCacheStatsOnCommit() throws Throwable {
    ...
    final long hitsAtStart = cacheStats.count(
      "transactionalCache",
      OpType.GET_HIT
    );
    final long missesAtStart = cacheStats.count(
      "transactionalCache",
      OpType.GET_MISS
    );
    final long putsAtStart = cacheStats.count("transactionalCache", OpType.PUT);
    final long removesAtStart = cacheStats.count(
      "transactionalCache",
      OpType.REMOVE
    );
    final long clearsAtStart = cacheStats.count(
      "transactionalCache",
      OpType.CLEAR
    );
    ...
    try {
      ...
      assertEquals(
        hitsAtStart,
        cacheStats.count("transactionalCache", OpType.GET_HIT)
      );
      assertEquals(
        missesAtStart,
        cacheStats.count("transactionalCache", OpType.GET_MISS)
      );
      assertEquals(
        putsAtStart,
        cacheStats.count("transactionalCache", OpType.PUT)
      );
      assertEquals(
        removesAtStart,
        cacheStats.count("transactionalCache", OpType.REMOVE)
      );
      assertEquals(
        clearsAtStart,
        cacheStats.count("transactionalCache", OpType.CLEAR)
      );

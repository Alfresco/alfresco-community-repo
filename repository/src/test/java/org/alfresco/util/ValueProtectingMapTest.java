...
public class ValueProtectingMapTest extends TestCase {
  ...
  protected void setUp() throws Exception {
    ...
    map =
      new ValueProtectingMap<String, Serializable>(
        holyMap,
        moreImmutableClasses
      );
  }
  ...
  private void checkMaps(boolean expectMapClone) {
    ...
    if (expectMapClone) {
      ...
      assertTrue(
        "Expect holy map to have been released: ",
        map.getProtectedMap() != holyMap
      );
      ...
      map.put("TWO", "TWO");
      assertTrue(
        "Backing map was changed again!",
        mapClone == map.getProtectedMap()
      );
      mapClone.containsKey("TWO");
    } else {
      // Make sure that the holy map is still acting as the backing map
      assertTrue(
        "Expect holy map to still be in use: ",
        map.getProtectedMap() == holyMap
      );
    }
  }
  ...
  public void testMapWrapping() {
    ValueProtectingMap<String, Serializable> mapTwo = new ValueProtectingMap<String, Serializable>(
      map
    );
    assertTrue(
      "Backing map must be shared: ",
      mapTwo.getProtectedMap() == map.getProtectedMap()
    );
    ValueProtectingMap<String, Serializable> mapThree = new ValueProtectingMap<String, Serializable>(
      map
    );
    assertTrue(
      "Backing map must be shared: ",
  ...
  public void testMapGetImmutable() {
    assertTrue(
      "Immutable value instance incorrect",
      map.get("IMMUTABLE") == valueImmutable
    );
    checkMaps(false);
  }
  ...
  public void testMapGetMutable() {
    ...
    mutable.increment();
    assertEquals(
      "Backing mutable should not have changed: ",
      0,
      valueMutable.i
    );
  }

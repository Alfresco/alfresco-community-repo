...
public class EditionServiceImpl implements EditionService {
    ...
    public static final QName[] ML_CONTAINER_PROPERTIES_TO_VERSION = {
        ContentModel.PROP_AUTHOR, ContentModel.PROP_LOCALE
    };

    /** {@inheritDoc} */
    public NodeRef createEdition(
            NodeRef startingTranslationNodeRef, Map<String, Serializable> versionProperties) {
        if (nodeService.hasAspect(
                startingTranslationNodeRef, ContentModel.ASPECT_MULTILINGUAL_DOCUMENT)) {
            return createEditionImpl(startingTranslationNodeRef, versionProperties);
        } else {
            throw new IllegalArgumentException(
                    "The node " + startingTranslationNodeRef + " is not multilingual.");
        }
    }

    private NodeRef createEditionImpl(
            NodeRef startingTranslationNodeRef, Map<String, Serializable> versionProperties) {

        // 1. First step: prepare and version the mlContainer

        // Get the ml container to version
        NodeRef mlContainerToVersion =
                multilingualContentService.getTranslationContainer(startingTranslationNodeRef);
        ...
        String name =
                (String)
                        nodeService.getProperty(startingTranslationNodeRef, ContentModel.PROP_NAME);
        Locale locale =
                (Locale)
                        nodeService.getProperty(
                                startingTranslationNodeRef, ContentModel.PROP_LOCALE);
        String author =
                (String)
                        nodeService.getProperty(
                                startingTranslationNodeRef, ContentModel.PROP_AUTHOR);
        ...
        for (int count = 0; ; count++) {
            try {
                ...
                startNode =
                        fileFolderService
                                .copy(startingTranslationNodeRef, space, tempName)
                                .getNodeRef();
    ...
    public VersionHistory getEditions(NodeRef mlContainer) {
        ...
        if (nodeService.getType(mlContainer).equals(ContentModel.TYPE_MULTILINGUAL_CONTAINER)) {
            // get the editions of the mlContainer
            editionHistory = versionService.getVersionHistory(mlContainer);
        } else {
            throw new IllegalArgumentException(
                    "The type of the node must be " + ContentModel.TYPE_CONTAINER);
        }
    ...
    public Map<QName, Serializable> getVersionedMetadatas(Version version) {
        ...
        if (ContentModel.TYPE_MULTILINGUAL_CONTAINER.equals(nodeService.getType(frozenNodeRef))) {
            ...
            Map<QName, Serializable> convertedProperties =
                    new HashMap<QName, Serializable>(properties.size());

            // perform the convertion
            for (Map.Entry<String, Serializable> entry : properties.entrySet()) {
                convertedProperties.put(QName.createQName(entry.getKey()), entry.getValue());
            }
    ...
    public List<VersionHistory> getVersionedTranslations(Version mlContainerEdition) {
        ...
        if (!ContentModel.TYPE_MULTILINGUAL_CONTAINER.equals(
                nodeService.getType(mlContainerEdition.getVersionedNodeRef()))) {
            throw new IllegalArgumentException(
                    "The type of the node must be " + ContentModel.TYPE_CONTAINER);
        }

        Map<QName, Serializable> properties =
                versionNodeService.getProperties(mlContainerEdition.getFrozenStateNodeRef());
        ...
        if (versionService
            ...
            throw new AlfrescoRuntimeException(
                    "Unexpected versionstore: "
                            + versionService.getVersionStoreReference().getIdentifier());
        }

        if (versionHistories == null) {
            // the initial edition doesn't content translations (at the creation time of the
            // mlContainer).
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Found all translations : \n"
                            + "   Versioned mlContainer: "
                            + mlContainerEdition.getVersionedNodeRef()
                            + "\n"
                            + "   Edition:                "
                            + mlContainerEdition
                            + "   Translations:           "
    ...
    /**
     * Util method to add the version histories of translations as a property of the frozen
     * mlContainer
     */
    private void addVersionHistoryProperty(
            Version edition, List<ChildAssociationRef> childAssocRefs) {
        List<VersionHistory> translationVersionHistories =
                new ArrayList<VersionHistory>(childAssocRefs.size());
        ...
        if (versionService
            ...
            this.nodeService.setProperty(
                    versionNodeRef,
                    Version2Model.PROP_QNAME_TRANSLATION_VERSIONS,
                    (Serializable) translationVersionHistories);
        } else if (versionService
                .getVersionStoreReference()
                .getIdentifier()
                .equals(VersionModel.STORE_ID)) {
            ...
            // add the version history of the translation as property of the Edition
            properties.put(
                    VersionModel.PROP_QNAME_QNAME, VersionModel.PROP_QNAME_TRANSLATION_VERSIONS);
    ...
    /**
     * Util method to add the usefull properties to the existing properties of the given,
     * mlContainer
     *
     * @link {@link EditionServiceImpl#ML_CONTAINER_PROPERTIES_TO_VERSION}
     */
    private void addPropertiesToVersion(
            Map<String, Serializable> versionProperties, NodeRef mlContainerToVersion) {
        // add useful properties
        for (QName prop : ML_CONTAINER_PROPERTIES_TO_VERSION) {
            versionProperties.put(
                    prop.toString(), nodeService.getProperty(mlContainerToVersion, prop));
        }
    }
    ...
    /** @param versionNodeService the Version Store Node Service to set */

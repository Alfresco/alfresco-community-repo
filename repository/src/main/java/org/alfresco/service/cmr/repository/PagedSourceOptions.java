 ...
 *
 * <p>The page numbering index starts with 1.
 *
 * <p>If only the start page number is specified transformers should attempt a page range from that
 * page number to the end if possible.
 *
 * <p>If only an end page number is specified transformers should attempt a page range from the
 * start to that page if possible.
 *
 * @author Ray Gauss II
 */
public class PagedSourceOptions extends AbstractTransformationSourceOptions {
    ...
    public TransformationSourceOptions mergedOptions(
        ...
        if (overridingOptions instanceof PagedSourceOptions) {
            PagedSourceOptions mergedOptions =
                    (PagedSourceOptions) super.mergedOptions(overridingOptions);

            if (((PagedSourceOptions) overridingOptions).getStartPageNumber() != null) {
                mergedOptions.setStartPageNumber(
                        ((PagedSourceOptions) overridingOptions).getStartPageNumber());
            }
            if (((PagedSourceOptions) overridingOptions).getEndPageNumber() != null) {
                mergedOptions.setEndPageNumber(
                        ((PagedSourceOptions) overridingOptions).getEndPageNumber());
            }

 ...
 *
 * <p>Adapts JLAN's debug information out to Alfresco's logging
 *
 * @author gkspencer
 */
public class FileServerDebugInterface implements DebugInterface {
    ...
    // MER TODO - Not thread safe - probably needs to be in a thread slot.   Would be much better to
    // fix DebugInterface.
    ...
    public void debugPrint(String str, int level) {
        if (level <= getLogLevel()) m_printBuf.append(str);
    }
    ...
    public int getLogLevel() {
        ...
        if (logger.isDebugEnabled()) logLevel = Debug.Debug;
        else if (logger.isInfoEnabled()) logLevel = Debug.Info;
        else if (logger.isWarnEnabled()) logLevel = Debug.Warn;
        else if (logger.isErrorEnabled()) logLevel = Debug.Error;
        else if (logger.isFatalEnabled()) logLevel = Debug.Fatal;

...
public class ForumPostBehaviours
  ...
  public void onUpdateProperties(
    ...
    if (newCommentCount != null) {
      ...
      if (newCommentCountInt == COUNT_TRIGGER_VALUE) {
        ...
        final Integer realCommentTotal = calculateCommentTotalByNodeCounting(
          commentsRollupNode
        );
  ...
  private Integer calculateCommentTotalByNodeCounting(NodeRef discussableNode) {
    ...
    if (nodeService.hasAspect(discussableNode, ForumModel.ASPECT_DISCUSSABLE)) {
      ...
      if (topicNode != null) {
        ...
        List<ChildAssociationRef> fmPostChildren = rawNodeService.getChildAssocs(
          topicNode,
          childNodeTypeQNames
        );
  ...
  private void adjustCommentCount(NodeRef fmPostNode, boolean incrementing) {
    ...
    NodeRef discussableAncestor = commentService.getDiscussableAncestor(
      fmPostNode
    );

    if (discussableAncestor != null) {
      if (discussableNodeRequiresFullRecount(discussableAncestor)) {
        Integer recount = calculateCommentTotalByNodeCounting(
          discussableAncestor
        );
        ...
        Integer existingCommentCount = (Integer) nodeService.getProperty(
          discussableAncestor,
          ForumModel.PROP_COMMENT_COUNT
        );
        int existingCommentCountInt = existingCommentCount == null
          ? 0
          : existingCommentCount.intValue();
  ...
  private boolean discussableNodeRequiresFullRecount(NodeRef discussableNode) {
    ...
    if (
      ...
      result =
        existingCommentCount == null ||
        existingCommentCount <= COUNT_TRIGGER_VALUE;
    }

...
public class FTPServerTest extends TestCase {
  ...
  protected void setUp() throws Exception {
    ...
    repositoryHelper =
      (Repository) applicationContext.getBean("repositoryHelper");
    permissionService =
      (PermissionService) applicationContext.getBean("permissionService");
    ServerConfigurationAccessor fileServerConfiguration = (ServerConfigurationAccessor) applicationContext.getBean(
      "fileServerConfiguration"
    );
    ftpConfigSection =
      (FTPConfigSection) fileServerConfiguration.getConfigSection(
        FTPConfigSection.SectionName
      );
    ...
    RetryingTransactionCallback<Void> createTestDirCB = new RetryingTransactionCallback<Void>() {
      ...
      public Void execute() throws Throwable {
        {
          NodeRef userOneHome = repositoryHelper.getUserHome(
            personService.getPerson(USER_ONE)
          );
  ...
  public void testFtpQuotaAndFtp() throws Exception {
    // Enable usages
    ContentUsageImpl contentUsage = (ContentUsageImpl) applicationContext.getBean(
      "contentUsageImpl"
    );
    contentUsage.setEnabled(true);
    contentUsage.init();
    UserUsageTrackingComponent userUsageTrackingComponent = (UserUsageTrackingComponent) applicationContext.getBean(
    ...
    try {
      ...
      String FILE3_CONTENT_3 =
        "test file 3 content that needs to be greater than 100 bytes to result in a quota exception being thrown";
      String FILE1_NAME = "test.docx";
  ...
  public void testModificationTime() throws Exception {
    ...
    try {
      ...
      String pathname =
        "/Alfresco/User Homes" + "/" + PATH1 + "/" + PATH2 + "/" + FILE1_NAME;

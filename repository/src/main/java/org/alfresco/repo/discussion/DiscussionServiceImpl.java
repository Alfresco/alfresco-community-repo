...
public class DiscussionServiceImpl implements DiscussionService {
   ...
   */
  protected NodeRef getSiteDiscussionsContainer(
  ...
  public TopicInfo getTopic(NodeRef parentNodeRef, String topicName) {
    NodeRef topic = nodeService.getChildByName(
      parentNodeRef,
  ...
  public PostInfo createReply(PostInfo parentPost, String contents) {
    ...
    if (parentPost.getNodeRef() == null) {
      throw new IllegalArgumentException(
        "Can't reply to a post that was never persisted"
      );
    }
    if (parentPost.getTopic() == null) {
      throw new IllegalArgumentException(
        "Can't reply to a post with no attached topic"
      );
    }
  ...
  public void deleteTopic(TopicInfo topic) {
    if (topic.getNodeRef() == null) {
      throw new IllegalArgumentException(
        "Can't delete a topic that was never persisted"
      );
    }
  ...
  public PostInfo getMostRecentPost(TopicInfo topic) {
    ...
    CannedQueryResults<NodeBackedEntity> results = listEntries(
      topic.getNodeRef(),
      ForumModel.TYPE_POST,
      null,
      null,
    ...
    // Wrap and return
    return buildPost(
      tenantService.getBaseName(node.getNodeRef()),
      topic,
      node.getName(),
      null
    );
  ...
  public PagingResults<TopicInfo> listTopics(
    ...
    CannedQueryResults<NodeBackedEntity> nodes = listEntries(
      nodeRef,
      ForumModel.TYPE_TOPIC,
      null,
      null,
  ...
  public PagingResults<Pair<TopicInfo, Integer>> listHotTopics(
    ...
    // Do the query
    GetDiscussionTopcisWithPostsCannedQueryFactory getCQFactory = (GetDiscussionTopcisWithPostsCannedQueryFactory) cannedQueryRegistry.getNamedObject(
  ...
  @Override
  public PagingResults<PostInfo> listPosts(
    TopicInfo topic,
    PagingRequest paging
  ) {
    // Do the listing, oldest first
  ...
  public PostWithReplies listPostReplies(PostInfo primaryPost, int levels) {
    // Grab the factory
    GetChildrenWithTargetAssocsAuditableCannedQueryFactory cqFactory = (GetChildrenWithTargetAssocsAuditableCannedQueryFactory) cannedQueryRegistry.getNamedObject(
  ...
  private CannedQueryResults<NodeBackedEntity> listEntries(
    ...
    // Grab the factory
    GetChildrenAuditableCannedQueryFactory getChildrenCannedQueryFactory = (GetChildrenAuditableCannedQueryFactory) cannedQueryRegistry.getNamedObject(
    ...
    if (oldestFirst) {
      sorting =
        getChildrenCannedQueryFactory.createDateAscendingCQSortDetails();
    } else {
      sorting =
        getChildrenCannedQueryFactory.createDateDescendingCQSortDetails();
    }
  ...
  private PagingResults<TopicInfo> wrap(
    ...
    return new PagingResults<TopicInfo>() {
      ...
      public Pair<Integer, Integer> getTotalResultCount() {
        ...
        final int totalItemsInUnpagedResultSet =
          skipCount + itemsRemainingAfterThisPage;
        return new Pair<Integer, Integer>(
          totalItemsInUnpagedResultSet,
          totalItemsInUnpagedResultSet
        );
      }
   ...
   */
  private PagingResults<TopicInfo> wrap(
    final PagingResults<NodeBackedEntity> results,
    final NodeRef container
  ) {
  ...
  private PagingResults<Pair<TopicInfo, Integer>> wrapWithCount(
    final PagingResults<NodeWithChildrenEntity> results,
    final NodeRef container
  ) {
    // Pre-load the nodes before we create them
    List<Long> ids = new ArrayList<Long>();

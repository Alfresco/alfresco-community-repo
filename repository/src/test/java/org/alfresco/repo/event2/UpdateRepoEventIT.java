...
public class UpdateRepoEventIT extends AbstractContextAwareRepoEvent {
  ...
  public void testUpdateNodeResourceContent() {
    ...
    assertEquals(
      "Wrong repo event type.",
      EventType.NODE_UPDATED.getType(),
      resultRepoEvent.getType()
    );
    ...
    assertNotNull(
      "Content should not have been null.",
      resourceBefore.getContent()
    );
  ...
  public void testUpdateContentTitleFromNull() {
    ...
    assertEquals(
      expectedResourceBeforeProperties,
      resourceBefore.getProperties()
    );
  ...
  public void testUpdateNodeType() {
    ...
    // old node's type
    assertEquals(
      "Created node does not have the correct type",
      ContentModel.TYPE_CONTENT,
  ...
  public void testUpdateNodeTypeWithCustomType() {
    ...
    // Create active model
    CustomModelDefinition modelDefinition = retryingTransactionHelper.doInTransaction(() ->
      customModelService.createCustomModel(model, true)
    );
  ...
  public void testMoveFile() {
    ...
    final String moveFileParentBeforeMove = resourceBefore
      .getPrimaryHierarchy()
      .get(0);
    final String moveFileParentAfterMove = resource
      .getPrimaryHierarchy()
      .get(0);
  ...
  public void testMoveFolder() {
    ...
    final String moveFolderParentBeforeMove = getNodeResourceBefore(4)
      .getPrimaryHierarchy()
      .get(0);
    final String moveFolderParentAfterMove = getNodeResource(4)
      .getPrimaryHierarchy()
      .get(0);
  ...
  public void testMoveFolderStructure() {
    ...
    final String grandParentParentAfterMove = getNodeResource(6)
      .getPrimaryHierarchy()
      .get(0);
    assertEquals("Wrong node parent.", root2ID, grandParentParentAfterMove);

    final String grandParentID = getNodeResource(3).getId();
    final String parentIDOfTheParentFolder = getNodeResource(4)
      .getPrimaryHierarchy()
      .get(0);
    assertEquals(
      "Wrong node parent.",
      grandParentID,
      parentIDOfTheParentFolder
    );

    final String parentID = getNodeResource(4).getId();
    final String contentParentID = getNodeResource(5)
      .getPrimaryHierarchy()
      .get(0);
  ...
  public void testMoveNodeWithAspects() {
    ...
    assertTrue(
      "Wrong aspect.",
      resource.getAspectNames().contains("cm:versionable")
    );

    final String folder2ID = getNodeResource(2).getId();
    final String moveFileParentAfterMove = getNodeResource(5)
      .getPrimaryHierarchy()
      .get(0);
  ...
  public void testMoveNodeWithProperties() {
    ...
    final String folder2ID = getNodeResource(2).getId();
    final String moveFileParentAfterMove = getNodeResource(4)
      .getPrimaryHierarchy()
      .get(0);
  ...
  public void testCreateAndMoveFileInTheSameTransaction() {
    retryingTransactionHelper.doInTransaction(() -> {
      ...
      NodeRef fileToMove = nodeService
        .createNode(
          folder1,
          ContentModel.ASSOC_CONTAINS,
          QName.createQName(TEST_NAMESPACE),
      ...
      assertEquals(
        folder2,
        nodeService.getPrimaryParent(fileToMove).getParentRef()
      );
    ...
    final String folder2ID = getNodeResource(2).getId();
    final String moveFileParentAfterMove = getNodeResource(3)
      .getPrimaryHierarchy()
      .get(0);
  ...
  public void testAddAndRemovePropertyInTheSameTransaction() {
    ...
    List<RepoEvent<EventData<NodeResource>>> nodeUpdatedEvents = getFilteredEvents(

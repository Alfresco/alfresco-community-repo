...
public class EventConsolidator implements EventSupportedPolicies {
   ...
   */
  public RepoEvent<DataAttributes<NodeResource>> getRepoEvent(
    EventInfo eventInfo
  ) {
    NodeResource resource = buildNodeResource();
    EventType eventType = getDerivedEvent();

    DataAttributes<NodeResource> eventData = buildEventData(
      eventInfo,
      resource,
      eventType
    );
    ...
    return RepoEvent
      .<DataAttributes<NodeResource>>builder()
  ...
  protected DataAttributes<NodeResource> buildEventData(
    ...
    EventData.Builder<NodeResource> eventDataBuilder = EventData
      .<NodeResource>builder()
      .setEventGroupId(eventInfo.getTxnId())
      .setResource(resource);

    if (eventType == EventType.NODE_UPDATED) {
      eventDataBuilder.setResourceBefore(
        buildNodeResourceBeforeDelta(resource)
      );
    }
  ...
  protected NodeResource buildNodeResourceBeforeDelta(NodeResource after) {
    ...
    Map<QName, Serializable> changedPropsBefore = getBeforeMapChanges(
      propertiesBefore,
      propertiesAfter
    );
    if (!changedPropsBefore.isEmpty()) {
      // Set only the changed properties
      Map<String, Serializable> mappedProps = helper.mapToNodeProperties(
        changedPropsBefore
      );
      ...
      UserInfo modifier = helper.getUserInfo(
        (String) changedPropsBefore.get(ContentModel.PROP_MODIFIER)
      );
      if (modifier != null) {
  ...
  private boolean hasChangedAspect() {
    if (
      (aspectsRemoved.isEmpty() && aspectsAdded.isEmpty()) ||
  ...
  private <K, V> Map<K, V> getBeforeMapChanges(
    Map<K, V> before,
    Map<K, V> after
  ) {
    if (before == null) {
  ...
  public boolean isTemporaryNode() {
    return (
      eventTypes.contains(EventType.NODE_CREATED) &&
  ...
  protected void setResourceBeforeAllFieldsNull(
    boolean resourceBeforeAllFieldsNull
  ) {

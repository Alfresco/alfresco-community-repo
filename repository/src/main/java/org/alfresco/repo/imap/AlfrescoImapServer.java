...
public class AlfrescoImapServer extends AbstractLifecycleBean {
  ...
  private class DefaultImapServer extends ImapServer {
    ...
    protected synchronized ServerSocket openServerSocket() throws IOException {
      ...
      if (setup.isSecure()) {
        try {
          ret =
            (SSLServerSocket) DummySSLServerSocketFactory
              .getDefault()
              .createServerSocket(setup.getPort(), 0, bindTo);
        } catch (IOException e) {
          if (logger.isErrorEnabled()) {
    ...
    if (serverImpl == null) {
      final Managers imapManagers = new Managers() {
        ...
        public ImapHostManager getImapHostManager() {
          return new AlfrescoImapHostManager(
            AlfrescoImapServer.this.imapService
          );
        }
      ...
      if (isImapEnabled()) {
        ...
        if (logger.isInfoEnabled()) {
          logger.info(
            "IMAP service started on host:port " + host + ":" + this.port
          );
        }
      }
      if (isImapsEnabled()) {
  ...
  public void checkForOpeningExceptions(
    AtomicReference<Exception> serverOpeningExceptionRef
  ) {

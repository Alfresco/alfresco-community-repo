...
public class MetadataExtracterLimitsTest {
  ...
  public void testNoTimeout() throws Exception {
    File file = AbstractContentTransformerTest.loadNamedQuickTestFile(
      "quick.txt"
    );

    Map<QName, Serializable> properties = extractFromFile(
      file,
    ...
    assertEquals(
      "value1",
      properties.get(
        QName.createQName("http://DummyMappingMetadataExtracter", "a1")
      )
    );
  }
  ...
  public void testWildcardTimeout() throws Exception {
    ...
    HashMap<String, MetadataExtracterLimits> mimetypeLimits = new HashMap<String, MetadataExtracterLimits>(
      1
    );
    mimetypeLimits.put("*", limits);
    ((MockDelayedMetadataExtracter) getExtracter()).setMimetypeLimits(
        mimetypeLimits
      );
    ...
    assertTrue(
      ...
      timeoutMs +
      "ms",
      extractionTime < (timeoutMs + 50)
    ); // bit of wiggle room for logging, cleanup, etc.
  }
  ...
  public void testInterrupted() throws Exception {
    ...
    assertTrue(
      ...
      interruptMs +
      "ms",
      extractionTime < (interruptMs + 500)
    ); // bit of wiggle room for logging, cleanup, etc.
  }

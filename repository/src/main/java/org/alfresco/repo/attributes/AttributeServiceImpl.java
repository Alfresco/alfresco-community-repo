...
public class AttributeServiceImpl implements AttributeService {
  ...
  public void getAttributes(
    final AttributeQueryCallback callback,
    Serializable... keys
  ) {
      ...
      public void handle(Long id, Long valueId, Serializable[] resultKeyIds) {
        ...
        for (int i = 0; i < resultKeyIds.length; i++) {
          if (resultKeyIds[i] != null) {
            Pair<Long, Serializable> keyValuePair = propertyValueDAO.getPropertyValueById(
              (Long) resultKeyIds[i]
            );
            resultsKeyValues[i] =
    ...
    propertyValueDAO.getPropertyUniqueContext(
      propertyUniqueContextCallback,
      keys
    );
    // Done
  }
  ...
  public void setAttribute(Serializable value, Serializable... keys) {
    ...
    if (pair == null) {
      ...
      propertyValueDAO.updatePropertyUniqueContext(
        keys[0],
        keys[1],
        keys[2],
        value
  ...
  public void updateOrCreateAttribute(
    ...
    // Done
    if (logger.isDebugEnabled()) {
      Serializable[] keysBefore = normalizeKeys(
        keyBefore1,
        keyBefore2,
        keyBefore3
      );
  ...
  public void removeAttribute(Serializable... keys) {
    keys = normalizeKeys(keys);
    int deleted = propertyValueDAO.deletePropertyUniqueContext(
      keys[0],
      keys[1],
      keys[2]
    );

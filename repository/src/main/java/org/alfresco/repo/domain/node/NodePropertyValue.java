...
public class NodePropertyValue implements Cloneable, Serializable {
  ...
  private static Log loggerOracle = LogFactory.getLog(
    NodePropertyValue.class.getName() + ".oracle"
  );
      ...
      protected ValueType getPersistedType(Serializable value) {
        if (value instanceof MLText) {
          throw new IllegalArgumentException(
            "MLText must be split up before persistence."
          );
        }
        return ValueType.STRING;
      }
    ...
    CONTENT {
      ...
      protected ValueType getPersistedType(Serializable value) {
        if (value instanceof ContentData) {
          ...
          throw new RuntimeException(
            "ContentData persistence must be by ContentDataId."
          );
        }
      }
      ...
      Serializable convert(Serializable value) {
        if (value instanceof Long) {
          ...
          logger.warn(
            "Content URL converter has not run to completion: " + value
          );
    ...
    SEALED_OBJECT {
      ...
      Serializable convert(Serializable value) {
        if (value == null) {
          ...
          throw new IllegalArgumentException(
            "Encrypted properties must be encrypted by the client."
          );
        }
      }
    };
  ...
  static {
    ...
    for (ValueType valueType : ValueType.values()) {
      ...
      if (valueTypesByOrdinalNumber.containsKey(ordinalNumber)) {
        throw new RuntimeException(
          "ValueType has duplicate ordinal number: " + valueType
        );
      } else if (ordinalNumber.intValue() == -1) {
        throw new RuntimeException(
          "ValueType doesn't have an ordinal number: " + valueType
        );
      }
   ...
   *      e.g. <b>CONTENT.getOrdinalNumber()</b> for type <b>d:content</b>.
   */
  public static int convertToPersistedTypeOrdinal(
    QName typeQName,
    Serializable value
  ) {
    ValueType valueType = makeValueType(typeQName);
    if (valueType == null) {
      throw new AlfrescoRuntimeException(
        "Unable to get value type from type qname " +
        typeQName +
        " and value " +
        value
      );
  ...
  public boolean equals(Object obj) {
    ...
    if (obj instanceof NodePropertyValue) {
      ...
      return (
        this.actualType.equals(that.actualType) &&
        EqualsHelper.nullSafeEquals(
          this.getPersistedValue(),
          that.getPersistedValue()
        )
      );
    } else {
      return false;
  ...
  public int hashCode() {
    int h = 0;
    if (actualType != null) h = actualType.hashCode();
    Serializable persistedValue = getPersistedValue();
    if (persistedValue != null) h += 17 * persistedValue.hashCode();
    return h;
  }
  ...
  public void setActualType(Integer actualType) {
    ValueType type = NodePropertyValue.valueTypesByOrdinalNumber.get(
      actualType
    );
    if (type == null) {
      logger.error(
        "Unknown property actual type ordinal number: " + actualType
      );
    }
  ...
  public void setPersistedType(Integer persistedType) {
    ...
    if (type == null) {
      logger.error(
        "Unknown property persisted type ordinal number: " + persistedType
      );
    }
  ...
  private Serializable cloneSerializable(Serializable original) {
    ...
    try {
      ...
      objectIn =
        new ObjectInputStream(new ByteArrayInputStream(byteOut.toByteArray()));
      Object target = objectIn.readObject();
      // Done
      return (Serializable) target;
    } catch (Throwable e) {
      throw new AlfrescoRuntimeException(
        "Failed to clone serializable object: " + original,
        e
      );
    } finally {
      if (objectOut != null) {
  ...
  public boolean getBooleanValue() {
    if (
      booleanValue == null
    ) return false; else return booleanValue.booleanValue();
  }
  ...
  public long getLongValue() {
    if (longValue == null) return 0; else return longValue.longValue();
  }
  ...
  public float getFloatValue() {
    if (floatValue == null) return 0.0F; else return floatValue.floatValue();
  }
  ...
  public double getDoubleValue() {
    if (doubleValue == null) return 0.0; else return doubleValue.doubleValue();
  }

...
public class ExportRecordsTests extends BaseRMRestTest {
    ...
    @Autowired private ExportAPI exportAPI;
    ...
    /**
     * Given a record with size > 4 MB When I export the record using API Then the request is
     * successful
     */
    @Test(
            description =
                    "Testing the RM Export functionality for records of size >4MB and Record "
    ...
    public void exportRMNodeTest(String nodeID) {
        STEP(
                "Export the created record/record folder with size greater than 4 MB and verifying"
                        + " the expected response code");
        exportAPI.exportRMNode(
                getAdminUser().getUsername(), getAdminUser().getPassword(), SC_OK, nodeID);
    }

    /**
     * I would change this to Given a list of records with a size > 4MB When I export the records
     * Then the request is succesfull
     */
    @Test(
            description =
                    "Testing the RM Export functionality using API for a list of Records at once"
                            + " with collective size of more than 4MB")
    ...
    public Record createRecord(String recordName, int sizeInMegaBytes) {
        return getRestAPIFactory()
                .getRecordFolderAPI()
                .createRecord(
                        Record.builder().name(recordName).nodeType(CONTENT_TYPE).build(),
                        recordFolder.getId(),
    ...
    public void exportRecordsTestsAfter() {
        STEP(
                "Delete the created rootCategory along with corresponding record folders/records"
                        + " present in it");

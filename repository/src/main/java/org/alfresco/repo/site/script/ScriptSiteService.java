...
public class ScriptSiteService extends BaseScopableProcessorExtension {
     ...
     * @deprecated as of version 3.2, replaced by {@link #createSite(String, String, String, String,
     *     String)}
     */
    public Site createSite(
            ...
            boolean isPublic) {
        SiteInfo siteInfo =
                this.siteService.createSite(sitePreset, shortName, title, description, isPublic);
     ...
     * Create a new site.
     *
     * <p>The site short name will be used to uniquely identify the site so it must be unique.
     *
     * @param sitePreset site preset
     * @param shortName site short name
     * @param title site title
    ...
    public Site createSite(
        ...
        SiteInfo siteInfo =
                this.siteService.createSite(
                        sitePreset, shortName, title, description, siteVisibility);
        return new Site(siteInfo, this.serviceRegistry, this.siteService, getScope());
    }
    ...
    public Site createSite(
        ...
        SiteInfo siteInfo =
                this.siteService.createSite(
                        sitePreset, shortName, title, description, siteVisibility, siteTypeQName);
        return new Site(siteInfo, this.serviceRegistry, this.siteService, getScope());
    }
     ...
     *
     * @return <code>true</code> if the current user is a Contributor to "Sites" node, else <code>
     *     false</code>.
     * @since 3.4
     */
    ...
    public boolean isSiteManager(String siteId) {
        String role =
                siteService.getMembersRole(siteId, AuthenticationUtil.getFullyAuthenticatedUser());
        return (SiteServiceImpl.SITE_MANAGER.equals(role));
    }

    /**
     * Retrieves the sites available in the repository. The returned list can optionally be filtered
     * by name and site preset. If no filters are specified then all the available sites are
     * returned.
     *
     * <p>NOTE: If the filter starts with a * a Lucene based search will be performed, this may
     * discover a wider range of results i.e. those sites that contain the search term as opposed to
     * those that start with the search term, but newly created sites may not be found until the
     * underlying search indexes are updated.
     *
     * @param filter inclusion filter for returned sites. Only sites whose cm:name OR cm:title OR
     *     cm:description start with the filter string will be returned.
    ...
    /**
     * List the sites available in the repository. The returned list can optionally be filtered by
     * name and site preset.
     *
     * <p>If no filters are specified then all the available sites are returned.
     *
     * @param filter inclusion filter for returned sites. Only sites whose cm:name OR cm:title OR
     *     cm:description start with the filter string will be returned.
     * @param sitePresetFilter site preset filter
     * @return Site[] a list of the site filtered as appropriate
     * @see SiteService#listSites(String, String, int) for a description of the limitations of this
     *     method.
     */
    ...
    /**
     * Find (search) the sites available in the repository. The returned list can optionally be
     * filtered by name and site preset.
     *
     * <p>If no filters are specified then all the available sites are returned.
     *
     * @param filter inclusion filter for returned sites. Only sites whose cm:name OR cm:title OR
     *     cm:description CONTAIN the filter string will be returned.
     * @param sitePresetFilter site preset filter
     * @param size max results size crop if >0
     * @return Site[] a list of the site filtered as appropriate
     * @see SiteService#findSites(String, String, int) for a description of the limitations of this
     *     method.
     * @since 4.0
     */
     ...
     *
     * <p>Returns null if the site does not exist.
     *
        ...
        if (siteService.isSiteAdmin(AuthenticationUtil.getFullyAuthenticatedUser())) {
            siteInfo =
                    AuthenticationUtil.runAs(
                            new AuthenticationUtil.RunAsWork<SiteInfo>() {
    ...
    /**
     * Get a site for a provided site short name. If the current user does not have permission to
     * view the site content, the info will still be returned, but none of the operations that would
     * modify site information on save() will work. As usual ACLs will apply on any attempt to
    ...
    /**
     * This method cleans up the permissions on the specified node and all its primary children. It
     * removes permissions which pertain to sites other than the node's current site.
     *
     * @since 3.4.2
     * @see SiteService#cleanSitePermissions(NodeRef, SiteInfo)
     */

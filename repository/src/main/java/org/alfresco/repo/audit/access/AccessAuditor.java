...
public class AccessAuditor
  ...
  private static final String AUDIT_SUB_ACTIONS =
    "audit.alfresco-access.sub-actions.enabled";
  ...
  public void setProperties(Properties properties) {
    ...
    auditSubActions =
      properties
        .getProperty(AUDIT_SUB_ACTIONS, "false")
  ...
  public void onUpdateProperties(
    ...
    if (auditEnabled()) {
      getNodeChange(nodeRef)
        .onUpdateProperties(nodeRef, fromProperties, toProperties);
    }
  }
  ...
  private NodeChange getNodeChange(NodeRef nodeRef) {
    Map<NodeRef, NodeChange> accessAuditNodes = TransactionalResourceHelper.getMap(
      transactionListener
    );
   ...
   */
  private boolean recordAuditValues(
    String action,
        ...
        for (String key : new TreeSet<String>(auditMap.keySet())) {
          if (!devOutput || !NodeChange.SUMMARY_KEYS.contains(key)) {
            StringBuilder output = (
                key.startsWith(NodeChange.SUB_ACTION_PREFIX)
              )
              ? subActions
              : sb;
  ...
  private class AccessTransactionListener extends TransactionListenerAdapter {
    ...
    public void afterCommit() {
      ...
      final Map<NodeRef, NodeChange> changedNodes = TransactionalResourceHelper.getMap(
        this
      );

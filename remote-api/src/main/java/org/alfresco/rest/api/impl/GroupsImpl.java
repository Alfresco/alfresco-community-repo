...
public class GroupsImpl implements Groups {
    ...
    private static final String ERR_MSG_MODIFY_FIXED_AUTHORITY =
    ...
    private static final Set<String> LIST_GROUPS_EQUALS_QUERY_PROPERTIES =
            new HashSet<>(Arrays.asList(new String[] {PARAM_IS_ROOT}));

    private static final Set<String> LIST_GROUP_MEMBERS_QUERY_PROPERTIES =
            new HashSet<>(Arrays.asList(new String[] {PARAM_MEMBER_TYPE}));
    ...
    public Group create(Group group, Parameters parameters) {
        ...
        String authority =
                authorityService.createAuthority(
                        AuthorityType.GROUP, group.getId(), authorityDisplayName, authorityZones);
    ...
    public CollectionWithPagingInfo<Group> getGroups(final Parameters parameters) {
        ...
        try {
            pagingResult =
                    getAuthoritiesInfo(
                            authorityType, groupsFilters, rootAuthorities, sortProp, paging);
        ...
        List<Group> groups =
                createGroupsResponse(
                        pagingResult.getPage(), parameters.getInclude(), rootAuthorities);

        int totalItems = pagingResult.getTotalResultCount().getFirst();

        return CollectionWithPagingInfo.asPaged(
                paging, groups, pagingResult.hasMoreItems(), totalItems);
    }

    private List<Group> createGroupsResponse(
            final List<AuthorityInfo> page,
            final List<String> includeParam,
            final Set<String> rootAuthorities) {
        List<Group> groups =
    ...
    private GroupsFilter getGroupsFilterFromQueryParameters(Query queryParameters) {
        ...
        if (queryParameters != null) {
            ...
            groupsFilterBuilder
                    .withIsRoot(propertyWalker.getIsRoot())
                    .withZoneFilter(
                            getStringFilterFromList(propertyWalker.getZones(), ZONE, MAX_ZONES))
                    .withDisplayNameFilter(
                            getStringFilterFromList(
                                    propertyWalker.getDisplayNames(),
                                    DISPLAY_NAME,
                                    MAX_DISPLAY_NAMES));
        }
    ...
    private void validateListParam(List<String> listParam, String paramName, int maxItems) {
        if (listParam.size() > maxItems) {
            throw new IllegalArgumentException(
                    "A maximum of " + maxItems + " " + paramName + "s may be specified.");
        } else if (listParam.isEmpty()) {
            throw new IllegalArgumentException(
                    StringUtils.capitalize(paramName) + "s filter list cannot be empty.");
        }
    ...
    @Override
    public CollectionWithPagingInfo<Group> getGroupsByPersonId(
            String requestedPersonId, Parameters parameters) {
        ...
        Set<String> userAuthorities =
                runAsSystem(() -> authorityService.getAuthoritiesForUser(personId));
        ...
        List<AuthorityInfo> groupAuthorities =
                userAuthorities.stream()
                        ...
                        .sorted(
                                new AuthorityInfoComparator(
                                        sortProp.getFirst(), sortProp.getSecond()))
                        .collect(Collectors.toList());

        PagingResults<AuthorityInfo> pagingResult =
                Util.wrapPagingResults(paging, groupAuthorities);
    ...
    private PagingResults<AuthorityInfo> getAuthoritiesInfo(
            AuthorityType authorityType,
            GroupsFilter groupsFilter,
            Set<String> rootAuthorities,
        ...
        if (isRootParam != null || displayNameFilter != null) {
            ...
            if (isRootParam != null && isRootParam) {
                ...
                // service layer.
                AuthorityInfoComparator authorityComparator =
                        new AuthorityInfoComparator(sortProp.getFirst(), sortProp.getSecond());
                Collections.sort(groupList, authorityComparator);
            } else {
                PagingRequest pagingNoMaxItems =
                ...
                PagingResults<AuthorityInfo> nonPagingResult =
    ...
    /**
     * Checks to see if the named group authority should be included in results when filtered by
     * zone.
     *
     ...
     *
     * <p>If isRootParam is null, then no results will be filtered. Otherwise results will be
     * filtered to return only those that are root or non-root, depending on the value of
     * isRootParam.
     *
     * @param isRootParam
     ...
     * @return
     */
    private boolean isRootPredicate(
            Boolean isRootParam, Set<String> rootAuthorities, String authority) {
        if (isRootParam != null) {
    ...
    /**
     * Checks a list of zones to see if it matches the supplied zone filter ({@code requiredZone}
     * parameter).
     *
     * <p>If the requiredZone parameter is null, then the filter will not be applied (returns true.)
     *
     * <p>If the requiredZone parameter is not null (i.e. a filter must be applied) and the {@code
     * zones}) list is {@code null} then the predicate will return false.
     *
     * @param zones
     * @param requiredZone
     * @return true if result should be included.
     ...
     * displayName.
     *
     * <p>If the requiredDisplayName parameter is null, then the filter will not be applied (returns
     * true.)
     *
     * <p>
     *
    ...
    private boolean displayNamePredicate(String groupDisplayName, String requiredDisplayName) {
        if (requiredDisplayName != null) {
            return groupDisplayName != null
                    && groupDisplayName.equalsIgnoreCase(requiredDisplayName);
        }
        return true;
    }
    ...
    /**
     * Retrieve authority info by name. <b>Node id field isn't used at this time and it is set to
     * null.</b>
     *
     ...
     *
     * @param id The authority name.
     * @param defaultDisplayNameIfNull True if we would like to get a default value (e.g. shortName
     *     of the authority) if the authority display name is null.
     * @return The authority info.
     */
    private AuthorityInfo getAuthorityInfo(String id, boolean defaultDisplayNameIfNull) {
        ...
        if (!id.equals(PermissionService.ALL_AUTHORITIES)
                && !authorityService.authorityExists(id)) {
    ...
    private String getAuthorityDisplayName(String id, boolean defaultDisplayNameIfNull) {
        return defaultDisplayNameIfNull
                ? authorityService.getAuthorityDisplayName(id)
                : authorityDAO.getAuthorityDisplayName(id);
    }

    private Group getGroup(
            AuthorityInfo authorityInfo, List<String> includeParam, Set<String> rootAuthorities) {
        ...
        if (authorityDisplayName == null || authorityDisplayName.isEmpty()) {
            authorityDisplayName =
                    authorityService.getAuthorityDisplayName(authorityInfo.getAuthorityName());
        }
        ...
        if (includeParam != null) {
                ...
                if (!authority.equalsIgnoreCase(PermissionService.ALL_AUTHORITIES)) {
                    containingAuthorities =
                            authorityService.getContainingAuthorities(
                                    AuthorityType.GROUP, authority, true);
                }
            ...
            if (includeParam.contains(PARAM_INCLUDE_ZONES)) {
                Set<String> authorityZones =
                        authorityService.getAuthorityZones(authorityInfo.getAuthorityName());
    ...
    public CollectionWithPagingInfo<GroupMember> getGroupMembers(
            String groupId, final Parameters parameters) {
        validateGroupId(groupId, false);
        ...
        Query q = parameters.getQuery();
        if (q != null) {
            MapBasedQueryWalkerOrSupported propertyWalker =
                    new MapBasedQueryWalkerOrSupported(LIST_GROUP_MEMBERS_QUERY_PROPERTIES, null);
            QueryHelper.walk(q, propertyWalker);

            String memberTypeStr =
                    propertyWalker.getProperty(
                            PARAM_MEMBER_TYPE, WhereClauseParser.EQUALS, String.class);
            authorityType = getAuthorityType(memberTypeStr);
        }

        PagingResults<AuthorityInfo> pagingResult =
                getAuthoritiesInfo(authorityType, groupId, sortProp, paging);

        // Create response.
        final List<AuthorityInfo> page = pagingResult.getPage();
        int totalItems = pagingResult.getTotalResultCount().getFirst();
        List<GroupMember> groupMembers =
                new AbstractList<GroupMember>() {
                    @Override
        ...
        return CollectionWithPagingInfo.asPaged(
                paging, groupMembers, pagingResult.hasMoreItems(), totalItems);
    }
    ...
    public GroupMember createGroupMember(String groupId, GroupMember groupMember) {
        ...
        if (existingAuthorityType != authorityType) {
            throw new IllegalArgumentException(
                    "Incorrect group member type, "
                            + (AuthorityType.USER.equals(existingAuthorityType)
                                    ? Groups.PARAM_MEMBER_TYPE_PERSON
                                    : existingAuthorityType)
    ...
    public void deleteGroupMembership(String groupId, String groupMemberId) {
        ...
        Set<String> parents =
                authorityService.getContainingAuthorities(AuthorityType.GROUP, groupMemberId, true);
            ...
            switch (memberType) {
                ...
                default:
                    throw new InvalidArgumentException(
                            "MemberType is invalid (expected eg. GROUP, PERSON)");
            }
        }
    ...
    private PagingResults<AuthorityInfo> getAuthoritiesInfo(
        ...
        try {
            authorities =
                    authorityService.findAuthorities(authorityType, groupId, true, null, null);
        } catch (UnknownAuthorityException e) {
            authorities = Collections.emptySet();
        }

        List<AuthorityInfo> authorityInfoList = new ArrayList<>(authorities.size());
        authorityInfoList.addAll(
                authorities.stream().map(this::getAuthorityInfo).collect(Collectors.toList()));
    ...
    private GroupMember getGroupMember(AuthorityInfo authorityInfo) {
        ...
        AuthorityType authorityType =
                AuthorityType.getAuthorityType(authorityInfo.getAuthorityName());
    ...
    private void validateGroupId(String groupId, boolean inferPrefix) {
        ...
        if (!PermissionService.ALL_AUTHORITIES.equals(groupId)
                && !groupAuthorityExists(groupId, inferPrefix)) {
    ...
    private void validateGroup(Group group, boolean isUpdate) {
        ...
        if (!isUpdate) {
            ...
            if (group.wasSet(Group.PARENT_IDS)) {
                throw new InvalidArgumentException(
                        "Group update does not support field: parentIds");
            }
    ...
    private boolean authorityExists(
            AuthorityType authorityType, String authorityName, boolean inferPrefix) {
        String name =
                inferPrefix
                        ? authorityService.getName(authorityType, authorityName)
                        : authorityName;
    ...
    private boolean isGroupAuthority(String authorityName) {
        ...
        return AuthorityType.GROUP.equals(authorityType)
                || AuthorityType.EVERYONE.equals(authorityType);
    }

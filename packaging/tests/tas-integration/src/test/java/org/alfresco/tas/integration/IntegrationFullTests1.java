...
public class IntegrationFullTests1 extends IntegrationTest {
    ...
    /**
     * Scenario 87 1. Using RestApi Admin user creates a new group1 2. Using CMIS create test users:
     * U1, U2 and U3 3. Using RestApi add users U2 and U3 to group1 4. U1 creates public test site1
     * using CMIS 5. U1 creates file1 using CMIS, WebDav, FTP 6. U1 apply Acl permission in the new
     * documents for testGroup with role Site Consumer using CMIS 7. Get permissions for U2 using
     * CMIS 8. U2 edits files created with CMIS, WebDav, FTP using WebDav
     */
    ...
    public void addDocumentPermissionsForGroupOfUsers() throws Exception {
        ...
        STEP(
                "6. U1 apply Acl permission in the new documents for testGroup with role Site"
                        + " Consumer using CMIS");
        ...
        cmisAPI.usingResource(cmisFile)
                .assertThat()
                .permissionIsSetForGrup(group1, UserRole.SiteConsumer);
        cmisAPI.usingResource(webDavFile)
                .assertThat()
                .permissionIsSetForGrup(group1, UserRole.SiteConsumer);
        cmisAPI.usingResource(ftpFile)
                .assertThat()
                .permissionIsSetForGrup(group1, UserRole.SiteConsumer);
        ...
        webDavProtocol
                ...
                .usingResource(cmisFile)
                .update(newContent)
                .assertThat()
                .hasStatus(org.apache.commons.httpclient.HttpStatus.SC_FORBIDDEN)
                .usingResource(webDavFile)
                .update(newContent)
                .assertThat()
                .hasStatus(org.apache.commons.httpclient.HttpStatus.SC_FORBIDDEN)
                .usingResource(ftpFile)
                .update(newContent)
                .assertThat()
                .hasStatus(org.apache.commons.httpclient.HttpStatus.SC_FORBIDDEN);
    }
    ...
    /**
     * Scenario 88 1. Using RestApi Admin user creates a new group1 2. Using CMIS create test users:
     * U1, U2 and U3 3. Using RestApi add users U2 and U3 to group1 4. U1 creates public test site1
     * using CMIS 5. U1 creates folder1 using CMIS, WebDav, FTP 6. U1 apply Acl permission in the
     * new folders for testGroup with role Site Consumer using CMIS 7. Get permissions for U2 using
     * CMIS 8. U2 edits folders created with CMIS, WebDav, FTP using WebDav
     */
    @Test(groups = {TestGroup.INTEGRATION, TestGroup.FULL})
    @TestRail(
            section = {TestGroup.INTEGRATION, TestGroup.CONTENT},
    ...
    public void addFolderPermissionsForGroupOfUsers() throws Exception {
        ...
        STEP(
                "6. U1 apply Acl permission in the new folders for testGroup with role Site"
                        + " Consumer using CMIS");
        ...
        STEP("7. Get permissions for U2 using CMIS");
        cmisAPI.usingResource(cmisFolder)
                .assertThat()
                .permissionIsSetForGrup(group1, UserRole.SiteConsumer);
        cmisAPI.usingResource(webDavFolder)
                .assertThat()
                .permissionIsSetForGrup(group1, UserRole.SiteConsumer);
        cmisAPI.usingResource(ftpFolder)
                .assertThat()
                .permissionIsSetForGrup(group1, UserRole.SiteConsumer);
        ...
        webDavProtocol
                .authenticateUser(testUser2)
                .usingResource(cmisFolder)
                .rename(newName)
                .assertThat()
                .hasStatus(org.apache.commons.httpclient.HttpStatus.SC_FORBIDDEN)
                .usingResource(webDavFolder)
                .rename(newName)
                .assertThat()
                .hasStatus(org.apache.commons.httpclient.HttpStatus.SC_FORBIDDEN)
                .usingResource(ftpFolder)
                .rename(newName)
                .assertThat()
                .hasStatus(org.apache.commons.httpclient.HttpStatus.SC_FORBIDDEN);
    }
    ...
    /**
     * Scenario 102 1. Using CMIS create test users: U1 and U2 2. U1 creates moderated test site1
     * using CMIS 3. U2 creates site membership request to site1 with RestAPI 4. U2 gets site
     * membership request, check new message using RestAPI 5. U2 updates site membership request
     * with a new message using RestAPI 6. U2 gets site membership request, check new message using
     * RestAPI 7. U1 changes site visibility to private site using RestAPI 8. U2 updates site
     * membership request with a new message using RestAPI 9. U2 gets site membership request, check
     * new message using RestAPI 10. U1 change site visibility to moderated site RestAPI 11. U2 gets
     * site membership request, check new message using RestAPI 12. U2 updates site membership
     * request with a new message using RestAPI 13. U2 get site membership request, check new
     * message using RestAPI 14. U1 user1 changes site visibility to public using RestAPI 15. U2
     * updates site membership request with a new message with RestAPI 16. U2 gets site membership
     * request, check new message using RestAPI 17. U1 changes site visibility to moderated site
     * RestAPI"); 18. U2 gets site membership request, check new message using RestAPI"); 19. U2
     * updates new site membership request using RestAPI 20. U1 approves site membership using
     * RestAPI 21. U2 gets site membership request - no request using RestAPI
     */
    @Test(groups = {TestGroup.INTEGRATION, TestGroup.FULL})
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify user can update or not site membership request when site visibility is"
                            + " updated")
    public void updateSiteMembershipRequestWhenSiteVisibilityIsUpdated() throws Exception {
        ...
        request =
                restAPI.withCoreAPI()
                        .usingMe()
                        .updateSiteMembershipRequest(testSiteModerated, "Please add me 2");
        ...
        request =
                restAPI.withCoreAPI()
                        .usingMe()
                        .updateSiteMembershipRequest(testSiteModerated, "Please add me 3");
        ...
        restAPI.withCoreAPI()
                .usingMe()
                .updateSiteMembershipRequest(testSiteModerated, "Please add me 4");
        restAPI.assertStatusCodeIs(HttpStatus.NOT_FOUND);
        ...
        request =
                restAPI.withCoreAPI()
                        .usingMe()
                        .updateSiteMembershipRequest(testSiteModerated, "Please add me 5");
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        ...
        RestTaskModel taskModel =
                restAPI.withWorkflowAPI().getTasks().getTaskModelByDescription(testSiteModerated);
    ...
    /**
     * Scenario 104 1. Using CMIS create test user A1 2. A1 creates public test site1 using CMIS 3.
     * Using CMIS create test user U1 4. A1 adds U1 to site1 as site manager using RestAPI 5. A1 get
     * site members using RestAPI 6. Using CMIS create test user U2 7. A1 adds U2 to site1 as site
     * collaborator using RestAPI 8. A1 get site members using RestAPI 9. Using CMIS create test
     * user U3 10. A1 add U3 to site1 as site contributor using RestAPI 11. A1 get site members
     * using RestAPI 12. Using CMIS create test user U4 13. A1 add U4 to site1 as site consumer
     * using RestAPI 14. A1 get site members using RestAPI 15. Using CMIS delete test user U1 16. A1
     * get site members using RestAPI 17. Using CMIS delete test user U2 18. A1 get site members
     * using RestAPI 19. Using CMIS delete test user U3 20. A1 get site members using RestAPI 21.
     * Using CMIS delete test user U4 22. A1 get site members using RestAPI
     */
    @Test(groups = {TestGroup.INTEGRATION, TestGroup.FULL})
    @TestRail(
            section = {TestGroup.INTEGRATION, TestGroup.SITES},
    ...
    public void getSiteMembersWhenUsersAreAddedOrDeleted() throws Exception {
        ...
        RestSiteMemberModelsCollection members =
                restAPI.authenticateUser(testUser)
                        .withCoreAPI()
                        .usingSite(testSitePublic)
                        .getSiteMembers();
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        ...
        members.assertThat()
                .entriesListContains("id", user1.getUsername())
                .and()
        ...
        members.assertThat()
                .entriesListContains("id", user2.getUsername())
                .and()
        ...
        members.assertThat()
                .entriesListContains("id", user3.getUsername())
                .and()
        ...
        members.assertThat()
                .entriesListContains("id", user4.getUsername())
                .and()
    ...
    /**
     * Scenario 105 1. Using CMIS create test user U1 2. U1 creates public test site1 using CMIS 3.
     * U1 creates file1 using Webdav 4. U1 updates file1 properties using CMIS 5. U1 gets file1
     * properties using CMIS 6. U1 renames file1 using Webdav 7. U1 updates file1 content using FTP
     * 8. U1 adds tags to file1 using RestAPI 9. U1 updates file1 properties using CMIS 10. U1 gets
     * file1 properties using CMIS
     */
    @Test(groups = {TestGroup.INTEGRATION, TestGroup.FULL})
    @TestRail(
            section = {TestGroup.INTEGRATION, TestGroup.CONTENT},
    ...
    public void checkFilePropertiesWhenFileIsUpdated() throws Exception {
        ...
        cmisAPI.authenticateUser(testUser)
                .usingSite(testSitePublic)
                .usingResource(file1)
                .updateProperty("cmis:description", newDescription)
        ...
        cmisAPI.authenticateUser(testUser)
                .usingResource(file1)
                .assertThat()
                .contentPropertyHasValue("cmis:description", newDescription)
                .assertThat()
                .contentPropertyHasValue("cm:title", newTitle);

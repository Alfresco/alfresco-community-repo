...
public class RMSiteTests extends BaseRMRestTest {
    /**
     * Given that RM module is installed When I want to create the RM site with specific title,
     * description and compliance Then the RM site is created
     */
    @Test(description = "Create RM site with Standard Compliance as admin user", priority = 2)
    // Run after createRMSiteAsAnotherAdminUser. In this way the Dod site is deleted and standard
    // site is created for the rest of the tests
    ...
    /**
     * Given that RM site exists When I want to create the RM site Then the response code 409 (Site
     * with the given identifier already exists) is return
     */
    ...
    /**
     * Given that RM site exists When I want to delete the RM site Then RM site is successfully
     * deleted
     */
    ...
    /**
     * Given that RM site exists When I GET the retrieve the RM site details Then RM site details
     * are returned
     */
    ...
    /**
     * Given that an user is created and RM site doesn't exist When the user wants to create a RM
     * site with DOD compliance Then RM site is created
     */
    // Run test after deleteRMSite. In this way rmSiteAPI.deleteRMSite isn't called because site is
    // already deleted
    ...
    public void createRMSiteAsAnotherAdminUser() {
        ...
        getRestAPIFactory()
                .getRMUserAPI()
                .createUser(ANOTHER_ADMIN, TestData.DEFAULT_PASSWORD, TestData.DEFAULT_EMAIL);

        // Create the RM site
        RMSite rmSiteModel =
                getRestAPIFactory()
                        .getRMSiteAPI(new UserModel(ANOTHER_ADMIN, DEFAULT_PASSWORD))
    ...
    /**
     * Given that RM site exist When a non-RM user wants to update the RM site details (title or
     * description) Then 403 response status code is return When the admin user wants to update the
     * RM site details (title or description) Then RM site details are updated
     */
    ...
    public void updateRMSiteDetails() {
        ...
        RMSite rmSiteToUpdate =
                RMSite.builder().title(NEW_TITLE).description(NEW_DESCRIPTION).build();

        // Create the RM site
        getRestAPIFactory()
                .getRMSiteAPI(getDataUser().createRandomTestUser("testUser"))
                .updateRMSite(rmSiteToUpdate);
    ...
    /**
     * Given that RM site exist When the admin user wants to update the RM site compliance Then RM
     * site compliance is not updated
     */
    @Test(priority = 3) // Run test after the other tests with priority 0, 1 or 2

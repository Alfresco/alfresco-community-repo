...
/**
 * Base class for rest-implementations related to workflow. Contains utility-methods that can be
 * used, regardless of the type of resources the implementing class can handle.
 *
...
public class WorkflowRestImpl {
    ...
    protected List<String> excludeModelTypes =
            new ArrayList<String>(Arrays.asList("bpm_priority", "bpm_description", "bpm_dueDate"));
    ...
    public Item createItemInProcess(String itemId, String processId) {
        ...
        activitiWorkflowEngine.dispatchPackageUpdatedEvent(
                packageScriptNode, null, null, processId, null);
        return responseItem;
    }
     ...
     * @throws IllegalArgumentException when no conversion for the given returnType is available or
    ...
    public <T extends Object> T getParameter(
            Parameters parameters, String parameterName, Class<T> returnType) {
        if (returnType == null) {
    ...
    public CollectionWithPagingInfo<FormModelElement> getFormModelElements(
            TypeDefinition type, Paging paging) {
        ...
        for (Entry<QName, PropertyDefinition> entry : taskProperties.entrySet()) {
            ...
            if (!typesToExclude.contains(entry.getValue().getContainerClass().getName())
                ...
                element.setDataType(
                        entry.getValue().getDataType().getName().toPrefixString(namespaceService));
    ...
    protected Set<QName> getTypesToExclude(TypeDefinition taskType) {
        ...
        while (parentClassDefinition != null) {
            if (contentClassFound) {
                ...
                // All parents of "cm:content" should be ignored as well for fetching
                // start-properties
    ...
    /**
     * Validates if the logged in user is allowed to get information about a specific process
     * instance. If the user is not allowed an exception is thrown.
     *
    ...
    protected List<HistoricVariableInstance> validateIfUserAllowedToWorkWithProcess(
        ...
        if (authorityService.isAdminAuthority(username)) {
            ...
            // If non-admin user, involvement in the task is required (either owner, assignee or
            // externally involved).
    ...
    private boolean isUserInGroup(String username, NodeRef group) {
        // Get the group name
        String groupName =
                (String) nodeService.getProperty(group, ContentModel.PROP_AUTHORITY_NAME);

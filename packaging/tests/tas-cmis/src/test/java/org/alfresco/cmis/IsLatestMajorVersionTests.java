...
public class IsLatestMajorVersionTests extends CmisTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify is latest major version for document created with VersioningState set"
                            + " to major")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that major version doesn't change for document created with"
                            + " VersioningState set to none")
    @Test(groups = {TestGroup.REGRESSION, TestGroup.CMIS})
    public void verifyMajorVersionNotChangedForNoneState() throws Exception {
        ...
        cmisApi.authenticateUser(managerUser)
                .usingSite(testSite)
                .createFile(testFile, VersioningState.NONE)
                .and()
                .assertThat()
                .existsInRepo()
                .and()
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify collaborator can verify is latest major version for document created by"
                            + " manager")
    @Test(groups = {TestGroup.REGRESSION, TestGroup.CMIS})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify contributor can verify is latest major version for document created by"
                            + " manager")
    @Test(groups = {TestGroup.REGRESSION, TestGroup.CMIS})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify consumer can verify is latest major version for document created by"
                            + " manager")
    @Test(groups = {TestGroup.REGRESSION, TestGroup.CMIS})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify non invited user can verify is latest major version for document in"
                            + " public site")
    @Test(groups = {TestGroup.REGRESSION, TestGroup.CMIS})
    public void nonInvitedUserCanVerifyIsLatestMajorVersionInPublicSite() throws Exception {
        cmisApi.authenticateUser(nonInviteUser)
                .usingResource(majorVersionDoc)
                .usingResource(majorVersionDoc)
                .and()
                .assertThat()
    ...
    public void nonInvitedUserCannotVerifyIsLatestMajorVersionInPrivateSite() throws Exception {
        ...
        cmisApi.authenticateUser(managerUser)
                ...
                .then()
                .authenticateUser(nonInviteUser)

...
public class NodeVersionsApiTest extends AbstractSingleNetworkSiteTest {
     ...
     *
     * <p>GET: {@literal
     * <host>:<port>/alfresco/api/<networkId>/public/alfresco/versions/1/nodes/<nodeId>/versions}
     * {@literal
     * <host>:<port>/alfresco/api/<networkId>/public/alfresco/versions/1/nodes/<nodeId>/versions/<versionId>}
     * {@literal
     * <host>:<port>/alfresco/api/<networkId>/public/alfresco/versions/1/nodes/<nodeId>/versions/<versionId>/content}
     */
    @Test
    public void testUploadFileVersionCreateWithOverwrite() throws Exception {
        ...
        try {
            ...
            // upload text file - versioning is currently auto enabled on upload (create file via
            ...
            {
                // -ve test - do not allow overwrite (using POST upload) if the file is not
                // versionable
                cnt++;
    ...
    public void testUploadFileVersionAsMinor() throws Exception {
        ...
        try {
            ...
            versionLabel =
                    updateFileVersions(
                            user1, docId, cnt, textContentSuffix, verCnt, null, versionLabel);
            verCnt = verCnt + cnt;
            ...
            versionLabel =
                    updateFileVersions(
                            user1, docId, cnt, textContentSuffix, verCnt, true, versionLabel);
            verCnt = verCnt + cnt;
     ...
     * Test delete version
     *
     * <p>Note: see also VersionServiceImplTest.testDeleteLastVersion
     *
     * @throws Exception
     */
    @Test
    public void testDeleteVersion() throws Exception {
        ...
        try {
            ...
            Pair<String, String> pair =
                    uploadTextFileVersions(
                            user1, f1Id, contentName, cnt, textContentSuffix, 0, null, null);
            ...
            // -ve test - cannot delete last version (via delete version api call) (see REPO-835 &
            // REPO-834)
            ...
            */
            ...
            // re-enable versioning (default model properties should cause initial version to be
            // created as 1.0)
            response = getSingle(URL_NODES, docId, 200);
            nodeResp = RestApiUtil.parseRestApiEntry(response.getJsonResponse(), Node.class);
    ...
    /**
     * This test helper method uses "overwrite=true" to create one or more new versions, including
     * the initial create if needed.
     *
     * <p>If the file does not already exist (currentVersionLabel should be null) and majorVersionIn
     * is also null then the first version is created as MAJOR (1.0) and subsequent versions are
     * created as MINOR.
     *
     * @param userId
     ...
     * @param currentVersionCounter - overall version counter, used as a suffix in text content and
     *     version comment
     * @param majorVersionIn - if null then false, if true then create MAJOR versions else if false
     *     create MINOR versions
     * @param currentVersionLabel - the current version label (if file already exists)
     * @return
     * @throws Exception
     */
    private Pair<String, String> uploadTextFileVersions(
            String userId,
            String parentFolderNodeId,
            String fileName,
            int cnt,
            String textContentPrefix,
            int currentVersionCounter,
            final Boolean majorVersionIn,
        ...
        for (int i = 1; i <= cnt; i++) {
            ...
            Document documentResp =
                    createTextFile(parentFolderNodeId, fileName, textContent, "UTF-8", params);
            ...
            HttpResponse response =
                    getSingle(getNodeVersionsUrl(docId), currentVersionLabel, null, 200);
            Node nodeResp = RestApiUtil.parseRestApiEntry(response.getJsonResponse(), Node.class);
            assertEquals(currentVersionLabel, nodeResp.getProperties().get("cm:versionLabel"));
            assertEquals(
                    (expectedMajorVersion ? "MAJOR" : "MINOR"),
    ...
    public void testUploadFileVersionUpdate() throws Exception {
        ...
        try {
            ...
            Document documentResp =
                    createTextFile(myFolderNodeId, contentName, content, "UTF-8", null);
            ...
            // check total version count - also get properties so that we can check version label
            // etc
            params = new HashMap<>();
            params.put("include", "properties");
            response = getAll(getNodeVersionsUrl(d1Id), paging, params, 200);
     ...
     *
     * <p>POST: {@literal
     * <host>:<port>/alfresco/api/-default-/public/alfresco/versions/1/nodes/<nodeId>/versions/<versionId>/revert}
     */
    @Test
    public void testRevert() throws Exception {
        ...
        try {
            f1Id =
                    createFolder(sharedFolderNodeId, "testRevert-f1-" + System.currentTimeMillis())
                            .getId();
            ...
            // check version history count - also get properties so that we can check version label
            // etc
            params = new HashMap<>();
            params.put("include", "properties");
            ...
            for (int i = 1; i <= revertCnt; i++) {
                ...
                post(
                        getNodeVersionRevertUrl(d1Id, revertVersionId),
                        toJsonAsStringNonNull(versionOptions),
                        null,
                        200);
            ...
            //  check version labels and content - most recently reverted, eg. version labels 4.0,
            // 3.0, 2.0
            ...
            checkVersionHistoryAndContent(
                    ...
                    updateCnt,
                    textContentSuffix,
                    revertVerCommentSuffix,
                    majorVersion,
                    0,
                    true);

            // check version labels and content - the rest of the version history (prior to
            // reverted), eg. version labels 1.3, 1.2, 1.1, 1.0
            minorVersion = 3;
            List<Node> originalUpdatedNodes = nodes.subList(revertCnt, nodes.size());
            checkVersionHistoryAndContent(
                    d1Id,
                    originalUpdatedNodes,
                    updateCnt + 1,
                    textContentSuffix,
                    updateVerCommentSuffix,
                    1,
            ...
            // Currently, we also allow the most recent version to be reverted (ie. not disallowed
            // by underlying VersionService)
    ...
    private void checkVersionHistoryAndContent(
            String docId,
            List<Node> nodesWithProps,
            int verCnt,
            String textContentSuffix,
            String verCommentSuffix,
            int majorVersion,
            int minorVersion,
        ...
        for (Node versionNode : nodesWithProps) {
            ...
            assertEquals(
                    (verCommentSuffix != null ? verCommentSuffix + verCnt : null),
                    versionNode.getVersionComment());

            // Download version content - by default with Content-Disposition header
            HttpResponse response =
                    getSingle(getNodeVersionsUrl(docId), versionId + "/content", null, 200);
    ...
    public void testCreateEmptyFileVersionUpdate() throws Exception {
        ...
        try {
            ...
            // note: we do not disallow listing version history on non-content node - however
            // currently no API method to version say a folder
            ...
            // note: alternatively can use version params ("comment" &/or "majorVersion") on update
            // (see separate test below)
            ...
            checkVersionHistoryAndContent(
                    d1Id, nodes, verCnt, textContentSuffix, null, 1, minorVersion, false);
...

            {
                ...
                //
                // -ve tests for non-versionable node with no version history (could be content,
                // folder or some other node type)
                //
    ...
    public void testUpdateFileVersionCreate() throws Exception {
        ...
        // Update again - with version comment - versioning is enabled here by using version params
        // ("comment" &/or "majorVersion")
        // note: alternatively could add versionable aspect before doing the update (see separate
        // test above)
        cnt++;
        int majorVersion = 1;
        int minorVersion = 0;
        ...
        // note: no version params (comment &/or majorVersion) needed since versioning is enabled on
        // this content
     ...
     *
     * <p>GET: {@literal
     * <host>:<port>/alfresco/api/-default-/public/alfresco/versions/1/nodes/<nodeId>/versions}
     */
    @Test

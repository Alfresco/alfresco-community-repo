...
public class PutMethod extends WebDAVMethod implements ActivityPostProducer {
                ...
                if (
                  getNodeService()
                    .hasAspect(
                      ...
                      ContentModel.ASPECT_WEBDAV_NO_CONTENT
                    )
                ) {
                  getNodeService()
                    .removeAspect(
  ...
  protected void executeImpl() throws WebDAVServerException, Exception {
    ...
    try {
      ...
      if (
        fileFolderService.isHidden(contentNodeRef) &&
      ...
      try {
        ...
        throw new ConcurrencyFailureException(
          "Concurrent access was detected.",
          ee
        );
      }
    }
    ...
    String userName = getDAVHelper()
      .getAuthenticationService()
      .getCurrentUserName();
    LockInfo lockInfo = getDAVLockService()
      .getLockInfo(contentNodeInfo.getNodeRef());
    ...
    try {
      ...
      ContentData contentData = (ContentData) getNodeService()
      ...
      Action extract = getActionService()
      ...
      if (
        ...
        !m_strContentType.equals(writer.getMimetype())
      ) {
  ...
  protected void postActivity() throws WebDAVServerException {
    WebDavService davService = getDAVHelper()
      .getServiceRegistry()
      .getWebDavService();
    ...
    try {
      ...
      if (!getNodeService().hasAspect(nodeRef, ContentModel.ASPECT_HIDDEN)) {
        if (isCreated()) {
          // file added
          activityPoster.postFileFolderAdded(
            siteId,
            tenantDomain,
            null,
            contentNodeInfo

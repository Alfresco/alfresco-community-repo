 ...
 *
 * <p>Properties are stored as a set of well-defined types defined by the enumeration {@link
 * PersistedType}. Ultimately, data can be persisted as BLOB data, but must be the last resort.
 *
 * @author Derek Hulley
 * @since 3.2
 */
public class DefaultPropertyTypeConverter implements PropertyTypeConverter {
    ...
    public DefaultPropertyTypeConverter() {
        persistenceMapping =
                new HashMap<Class<?>, PersistedType>(
     ...
     * For example, a <b>java.util.HashMap</b> is constructable provided that the map is empty.
     *
     * <p>Subclasses can override this to handle any well-known types, and in conjunction with
     * {@link #constructInstance(String)}, even choose to return <tt>true</tt> if it needs a
     * non-default constructor.
     *
     * @param value the value to check
     * @return Returns <tt>true</tt> if the value can be reconstructed by instantiation using a
     *     default constructor
     */
    ...
    public Serializable constructInstance(String clazzName) {
        try {
            ...
            throw new AlfrescoRuntimeException(
                    "The constructed property is not serializable: " + clazzName);
        } catch (Throwable e) {
            throw new AlfrescoRuntimeException(
                    "Unable to construct property for class: " + clazzName);
        }
    }
    ...
    /**
     * Performs the conversion using {@link DefaultTypeConverter} but also adds special handling for
     * {@link Enum enum types}.
     */

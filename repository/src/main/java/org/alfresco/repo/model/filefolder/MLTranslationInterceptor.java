...
public class MLTranslationInterceptor implements MethodInterceptor {
  ...
  private NodeRef getTranslatedNodeRef(NodeRef nodeRef) {
    ...
    NodeRef translatedNodeRef = multilingualContentService.getTranslationForLocale(
      nodeRef,
      filterLocale
    );
    // Done
    if (logger.isDebugEnabled()) {
      if (nodeRef.equals(translatedNodeRef)) {
        logger.debug(
          "NodeRef substitution: " + nodeRef + " --> " + translatedNodeRef
        );
      } else {
  ...
  private List<FileInfo> processList(List<FileInfo> fileInfos) {
    ...
    Map<FileInfo, FileInfo> translatedFileInfos = new HashMap<FileInfo, FileInfo>(
      17
    );
    ...
    Set<FileInfo> alreadyPresent = new HashSet<FileInfo>(
      fileInfos.size() * 2 + 1
    );
  ...
  private PagingResults<FileInfo> processPagingResults(
    final PagingResults<FileInfo> fileInfos
  ) {
  ...
  public Object invoke(MethodInvocation invocation) throws Throwable {
    ...
    if (I18NUtil.getContentLocaleOrNull() == null) {
      ...
      throw new RuntimeException(
        "Method not handled by interceptor: " + methodName
      );
    }

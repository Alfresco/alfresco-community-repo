...
public class MTPolicyComponentTest extends TestCase {
  ...
  private static final String TEST_MODEL =
    "org/alfresco/repo/policy/policycomponenttest_model.xml";
  private static final String TEST_NAMESPACE =
    "http://www.alfresco.org/test/policycomponenttest/1.0";
  private static QName BASE_TYPE = QName.createQName(TEST_NAMESPACE, "base");
  private static QName BASE_PROP_A = QName.createQName(
    TEST_NAMESPACE,
    "base_a"
  );
  private static QName BASE_ASSOC_A = QName.createQName(
    TEST_NAMESPACE,
    "base_assoc_a"
  );
  private static QName FILE_TYPE = QName.createQName(TEST_NAMESPACE, "file");
  private static QName FILE_PROP_B = QName.createQName(
    TEST_NAMESPACE,
    "file_b"
  );
  private static QName FOLDER_TYPE = QName.createQName(
    TEST_NAMESPACE,
    "folder"
  );
  private static QName FOLDER_PROP_D = QName.createQName(
    TEST_NAMESPACE,
    "folder_d"
  );
  private static QName TEST_ASPECT = QName.createQName(
    TEST_NAMESPACE,
    "aspect"
  );
  private static QName ASPECT_PROP_A = QName.createQName(
    TEST_NAMESPACE,
    "aspect_a"
  );
  private static QName INVALID_TYPE = QName.createQName(
    TEST_NAMESPACE,
    "classdoesnotexist"
  );
  ...
  protected void setUp() throws Exception {
    ...
    when(mockTenantService.getDomainUser(any(String.class), any(String.class)))
      .thenReturn("System");
    when(mockTenantService.getBaseName(any(NodeRef.class)))
      .thenReturn(new NodeRef(BASE_PROTOCOL, BASE_IDENTIFIER, BASE_ID));
    when(mockTenantService.getBaseName(any(StoreRef.class)))
      .thenReturn(new StoreRef(BASE_PROTOCOL, BASE_IDENTIFIER));
  ...
  public void testJavaBehaviour() {
    ...
    TestClassPolicyResult result = policy.test(
      "argument",
      nodeRef,
      date,
      storeRef
  ...
  public void testRegisterDefinitions() {
    try {
      ...
      ClassPolicyDelegate<InvalidMetaDataPolicy> delegate = policyComponent.registerClassPolicy(
        InvalidMetaDataPolicy.class
      );
    ...
    try {
      @SuppressWarnings("unused")
      ClassPolicyDelegate<NoMethodPolicy> delegate = policyComponent.registerClassPolicy(
        NoMethodPolicy.class
      );
    ...
    try {
      @SuppressWarnings("unused")
      ClassPolicyDelegate<MultiMethodPolicy> delegate = policyComponent.registerClassPolicy(
        MultiMethodPolicy.class
      );
      fail("Failed to catch multiple methods defined in policy");
    } catch (PolicyException e) {}

    QName policyName = QName.createQName(TEST_NAMESPACE, "test");
    boolean isRegistered = policyComponent.isRegisteredPolicy(
      PolicyType.Class,
      policyName
    );
    assertFalse(isRegistered);
    ClassPolicyDelegate<TestClassPolicy> delegate = policyComponent.registerClassPolicy(
      TestClassPolicy.class
    );
    assertNotNull(delegate);
    isRegistered =
      policyComponent.isRegisteredPolicy(PolicyType.Class, policyName);
    assertTrue(isRegistered);
    PolicyDefinition definition = policyComponent.getRegisteredPolicy(
      PolicyType.Class,
      policyName
    );
  ...
  public void testBindBehaviour() {
    ...
    try {
      BehaviourDefinition<ClassBehaviourBinding> definition = policyComponent.bindClassBehaviour(
        policyName,
        FILE_TYPE,
        validBehaviour
      );
  ...
  public void testClassDelegate() {
    ...
    TestClassPolicyResult baseResult = basePolicy.test(
      "womble",
      nodeRef,
      date,
      storeRef
    ...
    TestClassPolicyResult fileResult = filePolicy.test(
      "womble",
      nodeRef,
      date,
      storeRef
    ...
    assertEquals(
      "argument type of NodeRef not replaced by base node ref",
      fileResult.getNodeRef(),
      baseNodeRef
    );
    ...
    TestClassPolicyResult fileResult2 = filePolicy2.test(
      "womble",
      nodeRef,
      date,
      storeRef
    ...
    TestClassPolicyResult fileResult3 = filePolicy3.test(
      "womble",
      nodeRef,
      date,
      storeRef
  ...
  public interface TestClassPolicy extends ClassPolicy {
    ...
    public TestClassPolicyResult test(
      String argument,
      NodeRef nodeRef,
      Date date,
      StoreRef storeRef
  ...
  public TestClassPolicyResult validClassTest(
    String argument,
    NodeRef nodeRef,
    Date date,
    StoreRef storeRef
  ...
  public TestClassPolicyResult fileTest(
    String argument,
    NodeRef nodeRef,
    Date date,
    StoreRef storeRef
  ...
  public TestClassPolicyResult fileTest2(
    String argument,
    NodeRef nodeRef,
    Date date,
    StoreRef storeRef
  ...
  public TestClassPolicyResult serviceTest(
    String argument,
    NodeRef nodeRef,
    Date date,
    StoreRef storeRef

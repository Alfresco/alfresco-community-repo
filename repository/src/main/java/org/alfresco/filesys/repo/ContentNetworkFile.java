...
public class ContentNetworkFile extends NodeRefNetworkFile {
  ...
  public static ContentNetworkFile createFile(
    ...
    try {
      ...
      throw new AlfrescoRuntimeException(
        "File not found when creating network file: " + nodeRef,
        e
      );
    }
    ...
    if (fileInfo.hasCreationDateTime()) netFile.setCreationDate(
      fileInfo.getCreationDateTime()
    );

    if (
      fileInfo.hasModifyDateTime() && fileInfo.getModifyDateTime() > 0L
    ) netFile.setModifyDate(
      fileInfo.getModifyDateTime()
    ); else netFile.setModifyDate(fileInfo.getCreationDateTime());

    if (
      fileInfo.hasAccessDateTime() && fileInfo.getAccessDateTime() > 0L
    ...
    if (
      netFile.isReadOnly() &&
      netFile.getGrantedAccess() == NetworkFile.READWRITE
    ) netFile.setGrantedAccess(NetworkFile.READONLY);

    // DEBUG

    if (logger.isDebugEnabled()) logger.debug(
      "Create file node=" + nodeRef + ", path=" + path + ", netfile=" + netFile
    );
  ...
  public void openContent(boolean write, boolean trunc)
    ...
    synchronized (this) {
      ...
      if (write) {
        ...
        content =
          contentService.getReader(getNodeRef(), ContentModel.PROP_CONTENT);
  ...
  public void truncateFile(long size) throws IOException {
    ...
    try {
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "Error opening file " + getFullName() + " for write",
        ex
      );
      ...
      throw new DiskFullException(
        "Failed to open " + getFullName() + " for write"
      );
    }
  ...
  public long seekFile(long pos, int typ) throws IOException {
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Seek file=" + this + ", pos=" + pos + ", type=" + typ
    );

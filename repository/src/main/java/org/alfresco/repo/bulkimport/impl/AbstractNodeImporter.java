...
public abstract class AbstractNodeImporter implements NodeImporter {
    ...
    protected abstract NodeRef importImportableItemImpl(
            ImportableItem importableItem, BulkImportParameters.ExistingFileMode existingFileMode);

    protected abstract void importContentAndMetadata(
            NodeRef nodeRef,
    ...
    /**
     * Returns the name of the given importable item. This is the final name of the item, as it
     * would appear in the repository, after metadata renames are taken into account.
     *
     ...
     */
    protected final String getImportableItemName(
            ImportableItem importableItem, MetadataLoader.Metadata metadata) {
        String result = null;
        ...
        if (result == null && importableItem != null && importableItem.getHeadRevision() != null) {
            ...
            if (metadataFile != null) {
                ...
                result =
                        metadataFileName.substring(
                                0,
                                metadataFileName.length()
                                        - (MetadataLoader.METADATA_SUFFIX.length()
                                                + metadataLoader
    ...
    protected final int importImportableItemFile(
            NodeRef nodeRef,
        ...
        if (nodeState == NodeState.REPLACED
            ...
            // It is being replaced, and ADD_VERSION is the selected method of dealing with
            // overwrites.
    ...
    protected final int importContentVersions(
            NodeRef nodeRef, ImportableItem importableItem, NodeState nodeState) {
        int result = 0;
        ...
        // versionProperties.put(ContentModel.PROP_VERSION_LABEL.toPrefixString(),
        // String.valueOf(versionEntry.getVersion()));
        versionProperties.put(
                VersionModel.PROP_VERSION_TYPE,
                VersionType.MAJOR); // Load every version as a major version for now - see
        // http://code.google.com/p/alfresco-bulk-filesystem-import/issues/detail?id=84

        // handle versions
        for (final ImportableItem.VersionedContentAndMetadata versionEntry :
                importableItem.getVersionEntries()) {
            ...
            if (logger.isDebugEnabled())
                logger.debug(
                        "Creating v"
                                ...
                                + " of node '"
                                + nodeRef.toString()
                                + "' (note: version label in Alfresco will not be the same - it is"
                                + " not currently possible to explicitly force a particular version"
                                + " label).");

            versionService.createVersion(nodeRef, versionProperties);
            result +=
                    metadata.getProperties().size()
                            + 4; // Add 4 for "standard" metadata properties read from filesystem
        }

        // handle head version, if exists
        ImportableItem.ContentAndMetadata headRevContentAndMetadata =
                importableItem.getHeadRevision();
        if (headRevContentAndMetadata != null
                && (headRevContentAndMetadata.contentFileExists()
    ...
    protected final Triple<NodeRef, Boolean, NodeState> createOrFindNode(
            ...
            BulkImportParameters.ExistingFileMode existingFileMode,
            MetadataLoader.Metadata metadata) {
        // ADD_VERSION isn't strictly a replacement option, but we need to deal with it as such, at
        // least as an
        // interim measure while the new ExistingFileMode options are introduced (MNT-17703)
        boolean replaceExisting =
                (existingFileMode == BulkImportParameters.ExistingFileMode.REPLACE
                        || existingFileMode == BulkImportParameters.ExistingFileMode.ADD_VERSION);
        ...
        if (nodeName == null) {
            throw new IllegalStateException(
                    "Unable to determine node name for " + String.valueOf(importableItem));
        }

        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Searching for node with name '"
                            + nodeName
                            + "' within node '"
                            + target.toString()
                            + "'.");
        ...
        if (nodeRef == null) {
            // But only if the content file exists - we don't create new nodes based on
            // metadata-only importableItems
            if (importableItem.getHeadRevision().contentFileExists()) {
                isDirectory =
                        ImportableItem.FileType.DIRECTORY.equals(
                                importableItem.getHeadRevision().getContentFileType());

                try {
                    if (logger.isDebugEnabled())
                        logger.debug(
                                "Creating new node of type '"
                    ...
                    nodeRef =
                            fileFolderService
                                    .create(target, nodeName, metadata.getType())
                                    .getNodeRef();
                    ...
                    if (logger.isWarnEnabled())
                        logger.warn(
                                "Node with name '"
                                        ...
                                        + "' within node '"
                                        + target.toString()
                                        + "' was created concurrently to the bulk import.  Skipping"
                                        + " importing it.",
                                fee);
                    nodeRef = null;
                ...
                if (logger.isWarnEnabled())
                    logger.warn(
                            "Skipping creation of new node '"
                                    + nodeName
                                    + "' within node '"
                                    + target.toString()
                                    + "' since it doesn't have a content file.");
                nodeRef = null;
                nodeState = NodeState.SKIPPED;
            }
        }
        // We found the node in the repository.  Make sure we return the NodeRef, so that recursive
        // loading works (we need the NodeRef of all sub-spaces, even if we didn't create them).
        else {
            if (replaceExisting) {
                boolean targetNodeIsSpace = fileFolderService.getFileInfo(nodeRef).isFolder();

                if (importableItem.getHeadRevision().contentFileExists()) {
                    // If the source file exists, ensure that the target node is of the same type
                    // (i.e. file or folder) as it.
                    isDirectory =
                            ImportableItem.FileType.DIRECTORY.equals(
                                    importableItem.getHeadRevision().getContentFileType());
    ...
    protected final void importImportableItemMetadata(
            NodeRef nodeRef, Path parentFile, MetadataLoader.Metadata metadata) {
        // Attach aspects
        if (metadata.getAspects() != null) {
            for (final QName aspect : metadata.getAspects()) {
                ...
                nodeService.addAspect(
                        nodeRef, aspect,
                        null); // Note: we set the aspect's properties separately, hence null for
                // the third parameter
            }
        }
        ...
        if (metadata.getProperties() != null) {
            ...
            try {
                ...
                if (!nodeRef.equals(inre.getNodeRef())) {
                    ...
                    throw new IllegalStateException(
                            "Invalid nodeRef found in metadata for '"
                                    + getFileName(parentFile)
                                    + "'.  Probable cause: an association is being populated via"
                                    + " metadata, but the NodeRef for the target of that"
                                    + " association ('"
                                    + inre.getNodeRef()
                                    + "') is invalid.  "
                                    + "Please double check your metadata file and try again.",
                            inre);
                } else {
    ...
    protected final void importImportableItemDirectory(
            NodeRef nodeRef, ImportableItem importableItem, MetadataLoader.Metadata metadata) {
        if (importableItem.hasVersionEntries()) {
            logger.warn(
                    "Skipping versions for directory '"
        ...
        importImportableItemMetadata(
                nodeRef, importableItem.getHeadRevision().getContentFile(), metadata);
    }

    protected final MetadataLoader.Metadata loadMetadata(
            ImportableItem.ContentAndMetadata contentAndMetadata) {
        ...
        if (contentAndMetadata != null && contentAndMetadata.contentFileExists()) {
            final String filename =
                    contentAndMetadata
                            ...
                            .trim()
                            .replaceFirst(
                                    DirectoryAnalyser.VERSION_SUFFIX_REGEX,
                                    ""); // Strip off the version suffix (if any)
            ...
            result.setType(
                    ImportableItem.FileType.FILE.equals(contentAndMetadata.getContentFileType())
    ...
    public NodeRef importImportableItem(
            ImportableItem importableItem, BulkImportParameters.ExistingFileMode existingFileMode) {
        if (logger.isDebugEnabled()) {
    ...
    protected void skipImportableDirectory(ImportableItem importableItem) {
        if (logger.isInfoEnabled()) {
            logger.info(
                    "Skipping '" + getFileName(importableItem.getHeadRevision().getContentFile()));
        }

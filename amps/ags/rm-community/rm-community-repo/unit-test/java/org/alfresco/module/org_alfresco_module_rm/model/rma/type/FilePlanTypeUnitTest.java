...
public class FilePlanTypeUnitTest extends BaseUnitTest {
  ...
  public void testAddFileplanToFileplan() {
    ChildAssociationRef childAssoc = createFileplanContainerChild(
      TYPE_FILE_PLAN
    );
  ...
  public void testAddCategoriesToFileplan() {
    // add the first child
    ChildAssociationRef childAssoc1 = createFileplanContainerChild(
      TYPE_RECORD_CATEGORY
    );
    filePlanType.onCreateChildAssociation(childAssoc1, true);

    // add the second child
    ChildAssociationRef childAssoc2 = createFileplanContainerChild(
      TYPE_RECORD_CATEGORY
    );
  ...
  public void testCreateHoldContainers() {
    ...
    when(
      mockedNodeService.getChildAssocs(
        filePlanContainer,
        Sets.newHashSet(TYPE_HOLD_CONTAINER)
      )
    )
  ...
  public void testCreateMultipleHoldContainers() {
    ChildAssociationRef existingHoldAssoc = createFileplanContainerChild(
      TYPE_HOLD_CONTAINER
    );
    ChildAssociationRef childAssoc = createFileplanContainerChild(
      TYPE_HOLD_CONTAINER
  ...
  public void testCreateTransferContainers() {
    ...
    when(
      mockedNodeService.getChildAssocs(
        filePlanContainer,
        Sets.newHashSet(TYPE_TRANSFER_CONTAINER)
      )
    )
      .thenReturn(Arrays.asList(childAssoc));
    filePlanType.onCreateChildAssociation(childAssoc, true);
  ...
  public void testCreateUnfiledRecordsContainers() {
    ...
    when(
      mockedNodeService.getChildAssocs(
        filePlanContainer,
        Sets.newHashSet(TYPE_UNFILED_RECORD_CONTAINER)
      )
    )
      .thenReturn(Arrays.asList(childAssoc));
    filePlanType.onCreateChildAssociation(childAssoc, true);

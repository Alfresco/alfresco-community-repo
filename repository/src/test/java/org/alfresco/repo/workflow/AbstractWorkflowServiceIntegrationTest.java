...
public abstract class AbstractWorkflowServiceIntegrationTest extends BaseSpringTest {
    ...
    protected static final QName customStringProp =
            QName.createQName(NamespaceService.WORKFLOW_MODEL_1_0_URI, "customStringProp");
    ...
    public void testGetPathProperties() throws Exception {
        ...
        assertEquals(
                false, properties.get(QName.createQName("{}" + WorkflowConstants.PROP_CANCELLED)));
        assertEquals(
                instanceId,
                properties.get(
        ...
        NodeRef initiator =
                (NodeRef)
                        properties.get(QName.createQName("{}" + WorkflowConstants.PROP_INITIATOR));
        assertEquals(personManager.get(USER1), initiator);

        NodeRef expInitiatorHome =
                (NodeRef) nodeService.getProperty(initiator, ContentModel.PROP_HOMEFOLDER);
        NodeRef initiatorHome =
                (NodeRef)
                        properties.get(
                                QName.createQName("{}" + WorkflowConstants.PROP_INITIATOR_HOME));
        assertEquals(expInitiatorHome, initiatorHome);

        NodeRef actualCompanyHome =
                (NodeRef)
                        properties.get(
    ...
    public void testAssociateWorkflowPackage() {
        ...
        NodeRef rootNode =
                nodeService.getRootNode(new StoreRef(StoreRef.PROTOCOL_WORKSPACE, "SpacesStore"));
        ...
        List<WorkflowInstance> existingInstances =
                workflowService.getWorkflowsForContent(contentNode, true);
        ...
        String workflowDefId =
                (String)
                        nodeService.getProperty(
                                pckgNode, WorkflowModel.PROP_WORKFLOW_DEFINITION_ID);
        assertEquals(workflowDefId, workflowDef.getId());
        String workflowDefName =
                (String)
                        nodeService.getProperty(
                                pckgNode, WorkflowModel.PROP_WORKFLOW_DEFINITION_NAME);
        assertEquals(workflowDefName, workflowDef.getName());
        String workflowInstanceId =
                (String) nodeService.getProperty(pckgNode, WorkflowModel.PROP_WORKFLOW_INSTANCE_ID);
        ...
        List<WorkflowInstance> completedInstances =
    ...
    public void testPooledTaskCapabilities() {
        ...
        // MNT-9147: test service in read only state
        transactionService.setAllowWrite(false);
        // check nothing can be done to the task by the members of the group and sub group as
        ...
        Collection<?> newActors =
                (Collection<?>) taskProperties.get(WorkflowModel.ASSOC_POOLED_ACTORS);
    ...
    protected void checkTaskQueryInProgress(
            String workflowInstanceId, WorkflowTask expectedTask, String workflowInstanceId2) {
    ...
    private void checkWorkflowsDontContain(
            List<WorkflowInstance> workflows, String... expectedIds) {
        List<String> expIds = Arrays.asList(expectedIds);
    ...
    protected void checkTaskNameQuery(
        ...
        QName unexistingTaskName =
                QName.createQName(NamespaceService.WORKFLOW_MODEL_1_0_URI, "unexistingTask");
    ...
    protected void checkProcessIdQuery(
            String workflowInstanceId, List<String> expectedTaskIds, WorkflowTaskState state) {
        WorkflowTaskQuery taskQuery = createWorkflowTaskQuery(state);
        taskQuery.setProcessId(workflowInstanceId);
        checkTasksFoundUsingQuery(expectedTaskIds, taskQuery);
    ...
    protected void checkIsActiveQuery(
            List<String> expectedTaskIds, WorkflowTaskState state, String optionalProcessId) {
        WorkflowTaskQuery taskQuery = createWorkflowTaskQuery(state);
        taskQuery.setActive(true);
        if (optionalProcessId != null) {
            taskQuery.setProcessId(optionalProcessId);
    ...
    protected void checkTasksFoundUsingQuery(
            List<String> taskIds, WorkflowTaskQuery workflowTaskQuery) {
    ...
    private void checkDefinitions(
        ...
        if (expContainsDef) fail("The definitions did not contain expected id: " + id);
    }
    ...
    public void before() throws Exception {
        ...
        try {
            ...
            throw new AlfrescoRuntimeException(
                    "The AbstractWorkflowServiceIntegrationTest needs direct access to the"

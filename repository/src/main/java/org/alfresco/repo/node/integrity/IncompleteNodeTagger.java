 ...
 * @author Derek Hulley
 */
public class IncompleteNodeTagger extends TransactionListenerAdapter
     ...
     *     IncompleteNodeTagger. Note: assumes associations are within a store.
     */
    ...
    /**
     * Ensures that this service is registered with the transaction and saves the node reference for
     * use (property check) later.
     *
     ...
     * {@inheritDoc}
     *
     * <p>Save the node for checking of properties. The {@link
     * org.alfresco.model.ContentModel#ASPECT_INCOMPLETE incomplete} aspect is not processed.
     */
     ...
     * {@inheritDoc}
     *
     * <p>This only saves the node for checking if it is <i>not</i> new. The create of the node will
     * handle it.
     */
    ...
    public void beforeCommit(boolean readOnly) {
        ...
        RunAsWork<Void> processNodesWork =
                new RunAsWork<Void>() {
    ...
    private boolean checkProperties(
            Collection<PropertyDefinition> propertyDefs, Map<QName, Serializable> properties) {
            ...
            if (assocDef.isChild()) {
                ...
                List<ChildAssociationRef> childAssocRefs =
                        nodeService.getChildAssocs(
                                nodeRef, assocDef.getName(), RegexQNamePattern.MATCH_ALL);
                actualSize = childAssocRefs.size();
            } else {
                // check the target assocs present
                List<AssociationRef> targetAssocRefs =
                        nodeService.getTargetAssocs(nodeRef, assocDef.getName());
    ...
    /**
     * Adds or removes the {@link ContentModel#ASPECT_INCOMPLETE incomplete} marker aspect. This
     * only performs the operation if the tag aspect is or is not present, depending on the
     * operation required.
     *
     * @param nodeRef the node to apply the change to
     * @param addTag <tt>true</tt> to add the tag and <tt>false</tt> to remove the tag
     * @param isTagged <tt>true</tt> if the node already has the tag aspect applied, otherwise
     *     <tt>false</tt>
     */

 ...
 *
 * <p>To get paged list of children of a parent node filtered by child type. Also optionally
 * filtered and/or sorted by one or more properties (up to three).
 *
 * @author janv
 * @since 4.0
 */
public class GetChildrenCannedQuery extends AbstractCannedQueryPermissions<NodeRef> {
    ...
    // note: special qnames - originally from Share DocLib default config (however, we do not
    // support arbitrary "fts-alfresco" special sortable fields)
    public static final QName SORT_QNAME_CONTENT_SIZE =
            QName.createQName("http://www.alfresco.org/model/content/1.0", "content.size");
    ...
    public static final QName SORT_QNAME_NODE_IS_FOLDER =
            QName.createQName("", "IS_FOLDER"); // ALF-13968
    ...
    private boolean applyPostQueryPermissions =
            false; // if true, the permissions will be applied post-query (else should be applied as
    // part of the "queryAndFilter")
    ...
    public GetChildrenCannedQuery(
        ...
        GetChildrenCannedQueryParams paramBean =
                (GetChildrenCannedQueryParams) params.getParameterBean();
    ...
    protected FilterSortChildQueryCallback getFilterSortChildQuery(
        ...
        return new DefaultFilterSortChildQueryCallback(
                children, filterProps, inclusiveAspects, exclusiveAspects);
    }
    ...
    protected UnsortedChildQueryCallback getUnsortedChildQueryCallback(
        ...
        return new DefaultUnsortedChildQueryCallback(
                rawResult, requestedCount, inclusiveAspects, exclusiveAspects);
    }
    ...
    protected List<NodeRef> queryAndFilter(CannedQueryParameters parameters) {
        ...
        // Get parameters
        GetChildrenCannedQueryParams paramBean =
                (GetChildrenCannedQueryParams) parameters.getParameterBean();
        ...
        if (nodePair == null) {
            throw new InvalidNodeRefException(
                    "Parent node does not exist: " + parentRef, parentRef);
        }
        Long parentNodeId = nodePair.getFirst();

        // Set query params - note: currently using SortableChildEntity to hold (supplemental-)
        // query params
        ...
        filterProps.addAll(
                paramBean.getFilterProps()); // clone (to allow special handling for isPrimary)
        ...
        try {
            if ((childNodeTypeQNames != null) && (childNodeTypeQNames.size() > 0)) {
                ...
                Set<Long> childNodeTypeQNameIds =
                        qnameDAO.convertQNamesToIds(childNodeTypeQNames, false);
            ...
            if (pattern != null) {
                // TODO, check that we should be tied to the content model in this way. Perhaps a
            ...
            if (filterSortPropCnt > 0) {
                ...
                final FilterSortChildQueryCallback c =
                        getFilterSortChildQuery(children, filterProps, paramBean);
                ...
                // unsorted (apart from any implicit order) - note: permissions are applied during
                // result handling to allow early cutoff

                final int requestedCount = parameters.getResultsRequired();

                final List<NodeRef> rawResult =
                        new ArrayList<NodeRef>(Math.min(1000, requestedCount));
                UnsortedChildQueryCallback callback =
                        getUnsortedChildQueryCallback(rawResult, requestedCount, paramBean);
    ...
    private Long getQNameId(QName sortPropQName) {
        if (sortPropQName.equals(SORT_QNAME_CONTENT_SIZE)
        ...
        private int compareImpl(
            ...
            if (pv1 instanceof String) {
                result =
                        collator.compare(
                                (String) pv1,
                                (String) pv2); // TODO use collation keys (re: performance)
    ...
    private boolean includeAspects(
            NodeRef nodeRef, Set<QName> inclusiveAspects, Set<QName> exclusiveAspects) {
    ...
    protected boolean isApplyPostQueryPermissions() {
        return applyPostQueryPermissions; // true if sorted (if unsorted then permissions are
        // applied as part of the query impl)
    }
    ...
    protected List<NodeRef> applyPostQueryPermissions(List<NodeRef> results, int requestedCount) {
        ...
        // note: assume user has read access to most/majority of the items hence pre-load up to max
        // checks
    ...
    protected class DefaultFilterSortChildQueryCallback implements FilterSortChildQueryCallback {
        ...
        public DefaultFilterSortChildQueryCallback(
                final List<FilterSortNode> children, final List<FilterProp> filterProps) {
        ...
        private void preloadFilterSort() {
            ...
            for (FilterSortNodeEntity result : results) {
                ...
                Map<NodePropertyKey, NodePropertyValue> propertyValues =
                        new HashMap<NodePropertyKey, NodePropertyValue>(3);
                    ...
                    if (entry.getValue() instanceof MLText) {
                        propVals.put(
                                ...
                                DefaultTypeConverter.INSTANCE.convert(
                                        String.class, (MLText) entry.getValue()));
                    }
                }
                ...
                if (auditableProps != null) {
                    for (Map.Entry<QName, Serializable> entry :
                            auditableProps.getAuditableProperties().entrySet()) {
    ...
    protected class FilterSortNode {
        ...
        private Map<QName, Serializable>
                propVals; // subset of nodes properties - used for filtering and/or sorting
    ...
    private class UnsortedResultHandler implements CannedQueryDAO.ResultHandler<NodeEntity> {
        ...
        private void preloadAndApplyPermissions() {
            ...
            // TODO track total time for incremental permission checks ... and cutoff (eg. based on
            // some config)

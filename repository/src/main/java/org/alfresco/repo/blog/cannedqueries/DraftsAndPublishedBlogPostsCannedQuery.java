...
/**
 * This is a {@link CannedQuery} for the rather particular 'get my drafts and all published'
 * blog-post query.
 *
 * @author Neil Mc Erlean
 * @since 4.0
 * @see org.alfresco.service.cmr.blog.BlogService#getMyDraftsAndAllPublished(NodeRef, Date, Date,
 *     org.alfresco.query.PagingRequest)
 */
public class DraftsAndPublishedBlogPostsCannedQuery
    ...
    protected List<BlogEntity> queryAndFilter(CannedQueryParameters parameters) {
        ...
        if (paramBeanObj == null) throw new NullPointerException("Null GetBlogPosts query params");

        DraftsAndPublishedBlogPostsCannedQueryParams paramBean =
                (DraftsAndPublishedBlogPostsCannedQueryParams) paramBeanObj;
        ...
        for (BlogEntity result : results) {
            ...
            Date actualPublishedDate =
                    DefaultTypeConverter.INSTANCE.convert(Date.class, result.getPublishedDate());
            ...
            if (actualPublishedDate != null) {
                ...
                // We're relying on cm:published being null below i.e. we are dealing with draft
                // blog posts.

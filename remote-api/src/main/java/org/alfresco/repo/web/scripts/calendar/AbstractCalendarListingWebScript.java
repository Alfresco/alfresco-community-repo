...
public abstract class AbstractCalendarListingWebScript extends AbstractCalendarWebScript {
    ...
    /**
     * Returns a Comparator for (re-)sorting events, typically used after expanding out recurring
     * instances.
     */
    protected static Comparator<Map<String, Object>> getEventDetailsSorter() {
        return new Comparator<Map<String, Object>>() {
            public int compare(Map<String, Object> resultA, Map<String, Object> resultB) {
                ...
                // check and parse iso8601 date without time zone (All day events are stripped of
                // time zone)
                DateTime sa =
                        startA.length() > 23
    ...
    protected boolean handleRecurring(
        ...
        List<ChildAssociationRef> ignoreEventList =
                nodeService.getChildAssocs(entry.getNodeRef(), childNodeTypeQNames);
        ...
        for (ChildAssociationRef ignoreEvent : ignoreEventList) {
            ...
            Date ignoredDate =
                    (Date) nodeService.getProperty(nodeRef, CalendarModel.PROP_IGNORE_EVENT_DATE);
            ignoredDates.add(ignoredDate);
        }

        // Get it's recurring instances
        List<Date> dates =
                CalendarRecurrenceHelper.getRecurrencesOnOrAfter(
        ...
        List<ChildAssociationRef> updatedEventList =
                nodeService.getChildAssocs(entry.getNodeRef(), childNodeTypeQNames);
        ...
        for (ChildAssociationRef updatedEvent : updatedEventList) {
            ...
            Date updatedDate =
                    (Date) nodeService.getProperty(nodeRef, CalendarModel.PROP_UPDATED_EVENT_DATE);
            Date newStart =
                    (Date) nodeService.getProperty(nodeRef, CalendarModel.PROP_UPDATED_START);
            Date newEnd = (Date) nodeService.getProperty(nodeRef, CalendarModel.PROP_UPDATED_END);
            String newWhere =
                    (String) nodeService.getProperty(nodeRef, CalendarModel.PROP_UPDATED_WHERE);
            String newWhat =
                    (String) nodeService.getProperty(nodeRef, CalendarModel.PROP_UPDATED_WHAT);
    ...
    private void updateRepeating(
        ...
        if (updatedDates.keySet().contains(fmt.format(date))) {
            ...
            entryResult.put(
                    RESULT_TITLE, (String) updatedDates.get(fmt.format(date).toString() + "what"));
            entryResult.put(
                    "where", (String) updatedDates.get(fmt.format(date).toString() + "where"));

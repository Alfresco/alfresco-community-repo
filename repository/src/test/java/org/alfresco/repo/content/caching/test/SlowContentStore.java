 ...
 *
 * <p>Simulates a slow content store such as Amazon S3 or XAM. The ContentStore does not provide
 * genuine facilities to store or retrieve content.
 *
 * <p>Asking for content using {@link #getReader(String)} will result in (generated) content being
 * retrieved for any URL. A counter records how many times each arbitrary URL has been asked for.
 *
 * <p>Attempts to write content using any of the getWriter() methods will succeed. Though the
 * content does not actually get stored anywhere.
 *
 * <p>Both reads and writes are slow - the readers and writers returned by this class sleep for
 * {@link #pauseMillis} after each operation.
 *
 * @author Matt Ward
 */
class SlowContentStore extends AbstractContentStore {
        ...
        protected ReadableByteChannel getDirectReadableChannel() throws ContentIOException {
            return new ReadableByteChannel() {
                private final byte[] content =
                        "This is the content for my slow ReadableByteChannel".getBytes();
                private int index = 0;
                private boolean closed = false;
    ...
    /**
     * Length of time in milliseconds that ReadableByteChannel and WriteableByteChannel objects
     * returned by SlowContentStore will pause for during read and write operations respectively.
     *

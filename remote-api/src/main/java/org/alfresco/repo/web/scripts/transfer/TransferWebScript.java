...
public class TransferWebScript extends AbstractWebScript {
  ...
  public void execute(WebScriptRequest req, WebScriptResponse res)
    ...
    if (enabled) {
      log.debug(
        "Transfer webscript invoked by user: " +
        AuthenticationUtil.getFullyAuthenticatedUser() +
        " running as " +
        AuthenticationUtil.getRunAsAuthentication().getName()
      );
      processCommand(
        req.getServiceMatch().getTemplateVars().get("command"),
        req,
        res
      );
    } else {
  ...
  private void processCommand(
    ...
    if (command == null || (command = command.trim()).length() == 0) {
      log.warn(
        "Empty or null command received by the transfer script. Returning \"Not Found\""
      );
      res.setStatus(Status.STATUS_NOT_FOUND);
    } else {
      ...
      if (processor != null) {
        ...
        log.warn(
          "No processor found for requested command: " +
          command +
          ". Returning \"Not Found\""
        );
        res.setStatus(Status.STATUS_NOT_FOUND);

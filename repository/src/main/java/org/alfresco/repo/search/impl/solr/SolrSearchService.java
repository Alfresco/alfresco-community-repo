...
public class SolrSearchService implements SearchService {
  ...
  public ResultSet query(
    ...
    String queryString = parameterise(
      definition.getQuery(),
  ...
  private void checkParameters(
    ...
    if (missing.size() > 0) {
      ...
      buffer.append(
        "The query is missing values for the following parameters: "
      );
  ...
  private String parameterise(
    ...
    while ((index = buffer.indexOf("${", index)) != -1) {
      ...
      if (parameterDefinition == null) {
        ...
        if (it == null) {
          ...
          value =
            DefaultTypeConverter.INSTANCE.convert(String.class, it.next());
        }
    ...
    if (missing.size() > 0) {
      ...
      error.append(
        "The query uses the following parameters which are not defined: "
      );
      for (QName qName : missing) {
        error.append(qName);
        error.append(", ");
  ...
  public ResultSet query(SearchParameters searchParameters) {
    if (searchParameters.getStores().size() == 0) {
      throw new IllegalStateException(
        "At least one store must be defined to search"
      );
    }
    ...
    LuceneQueryLanguageSPI language = queryLanguages.get(
      searchParameters.getLanguage().toLowerCase()
    );
    if (language != null) {
  ...
  public boolean like(
    ...
    if (includeFTS) {
      ...
      String pattern = SearchLanguageConversion.convertXPathLikeToLucene(
        sqlLikePattern.toLowerCase()
      );
      ...
      // convert the SQL-like pattern into a Lucene-compatible string
      String pattern = SearchLanguageConversion.convertXPathLikeToRegex(
        sqlLikePattern.toLowerCase()
      );

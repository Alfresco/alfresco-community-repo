...
public interface ResponseWriter {
  String UTF8 = "UTF-8";
  ContentInfo DEFAULT_JSON_CONTENT = new ContentInfoImpl(
    Format.JSON.mimetype(),
    UTF8,
    0,
    null
  );
  Cache CACHE_NEVER = new Cache(
  ...
  final WithResponse DEFAULT_SUCCESS = new WithResponse(
    Status.STATUS_OK,
   ...
   */
  default void setContentInfoOnResponse(
    ...
    if (contentInfo != null) {
      ...
      if (res instanceof WrappingWebScriptResponse) {
        WrappingWebScriptResponse wrappedRes =
          ((WrappingWebScriptResponse) res);
      ...
      if (res instanceof WebScriptServletResponse) {
        ...
        if (contentInfo.getLocale() != null) {
          servletResponse
            .getHttpServletResponse()
            .setLocale(contentInfo.getLocale());
        }
      }
    }
  }
  ...
  default void renderErrorResponse(
    ...
    if (
      Status.STATUS_INTERNAL_SERVER_ERROR == errorResponse.getStatusCode() ||
    ...
    String stackMessage = I18NUtil.getMessage(
      DefaultExceptionResolver.STACK_MESSAGE_ID
    );

    final ErrorResponse errorToWrite = new ErrorResponse(
  ...
  default void renderException(
    ...
    renderErrorResponse(
      assistant.resolveException(exception),
      response,

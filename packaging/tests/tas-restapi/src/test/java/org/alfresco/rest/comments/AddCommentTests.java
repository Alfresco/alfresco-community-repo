...
public class AddCommentTests extends RestTest {
  ...
  public void adminIsAbleToAddComment() throws Exception {
    restClient.authenticateUser(adminUserModel);
    String newContent =
      "This is a new comment added by " + adminUserModel.getUsername();
    restClient
      .withCoreAPI()
    ...
    restClient
      .onResponse()
      .assertThat()
      .body("entry.edited", org.hamcrest.Matchers.is(false));
  }
  ...
  public void managerIsAbleToAddComment() throws Exception {
    ...
    RestCommentModel createdComment = restClient
      .withCoreAPI()
      .usingResource(document)
      .addComment(contentSiteManger);
    restClient.assertStatusCodeIs(HttpStatus.CREATED);
    createdComment
      .assertThat()
      .field("content")
  ...
  public void contributorIsAbleToAddComment() throws Exception {
    ...
    String contentSiteContributor =
      "This is a new comment added by user with role" +
      UserRole.SiteContributor;
    RestCommentModel createdComment = restClient
      .withCoreAPI()
      .usingResource(document)
      .addComment(contentSiteContributor);
    restClient.assertStatusCodeIs(HttpStatus.CREATED);
    createdComment
      .assertThat()
      .field("content")
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.COMMENTS },
    executionType = ExecutionType.REGRESSION,
    description = "Verify Consumer user can't add comments with Rest API and status code is 403"
  )
  @Test(
    groups = { TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION }
  )
  public void consumerIsNotAbleToAddComment() throws Exception {
    ...
    restClient
      .withCoreAPI()
      .usingResource(document)
      .addComment(contentSiteConsumer);
    restClient

...
public class NodePermissionAssessor {
  ...
  public NodePermissionAssessor(
    ...
    Authority authority,
    EntityLookupCache<Long, Node, NodeRef> nodeCache
  ) {
  ...
  public boolean shouldQuitChecks() {
    if (checksPerformed >= maxPermissionChecks) {
      logger.warn(
        "Maximum permission checks exceeded (" + maxPermissionChecks + ")"
      );
      return true;
    }

    if (
      (System.currentTimeMillis() - startTime) >= maxPermissionCheckTimeMillis
    ) {
      logger.warn(
        "Maximum permission checks time exceeded (" +

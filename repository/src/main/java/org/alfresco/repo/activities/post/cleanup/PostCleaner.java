...
public class PostCleaner {
    ...
    private static VmShutdownListener vmShutdownListener =
            new VmShutdownListener(PostCleaner.class.getName());
    ...
    public void executeWithLock() throws JobExecutionException {
        ...
        try {
            ...
            long keepTimeOffset =
                    nowTimeOffset
                            - (maxAgeMins * 60 * 1000); // millsecs = mins * 60 secs * 1000 msecs
            Date keepDate = new Date(keepTimeOffset);

            // clean old entries - PROCESSED - does not clean POSTED or PENDING, which will need to
            // be done manually, if stuck

...
public interface TransferReceiver {
     ...
     */
    String start(String fromRepositoryId, boolean allowTransferToSelf, TransferVersion fromVersion)
    ...
    /**
     * Store the specified snapshot file into the transfer staging area. The specified transfer must
     * currently be the holder of the transfer lock, otherwise an exception is thrown. This
     * operation does not close the supplied stream, so the caller must do it as appropriate. The
     * caller should assume that the supplied stream has been fully read when this operation
     * returns.
     *
     ...
     * @throws TransferException
     */
    void saveContent(String transferId, String contentId, InputStream contentStream)
            throws TransferException;

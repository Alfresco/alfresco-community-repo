...
public class SOLRWebScriptTest extends BaseWebScriptTest {
    ...
    public void testAclsGet() throws Exception {
        List<AclChangeSet> aclChangeSets =
                searchTrackingComponent.getAclChangeSets(null, null, null, null, 100);
        ...
        TestWebScriptServer.PostRequest req =
                new TestWebScriptServer.PostRequest(url, json.toString(), "application/json");
        Response response = sendRequest(req, Status.STATUS_OK, admin);
        if (logger.isDebugEnabled()) {
            logger.debug(response.getContentAsString());
        }
    ...
    private void aclReadersGetImpl() throws Exception {
        List<AclChangeSet> aclChangeSets =
                searchTrackingComponent.getAclChangeSets(null, null, null, null, 1024);
        ...
        assertEquals(
                "Should have same number of ACLs as supplied", aclIds.size(), aclsReaders.size());
    ...
    private JSONArray getNodes(GetNodesParameters parameters, int maxResults, int expectedNumNodes)
            throws Exception {
        ...
        TestWebScriptServer.PostRequest req =
                new TestWebScriptServer.PostRequest(
                        url.toString(), json.toString(), "application/json");

        long startTime = System.currentTimeMillis();
        Response response = sendRequest(req, Status.STATUS_OK, admin);
        long endTime = System.currentTimeMillis();

        //      assertEquals("Expected application/json content type",
        // "application/json[;charset=UTF-8]", response.getContentType());
        ...
        txnHelper.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        for (int i = 0; i < 100; i++) {
                            FileInfo content1Info =
                                    fileFolderService.create(
                                            container3, "Content" + i, ContentModel.TYPE_CONTENT);
                ...
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        FileInfo contentInfo =
                                fileFolderService.create(
                                        container6, "Content1", ContentModel.TYPE_CONTENT);
    ...
    private JSONArray getNodesMetaData(List<Long> nodeIds, int maxResults, int numMetaDataNodes)
            throws Exception {
                ...
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        for (int i = 0; i < 2000; i++) {
                            FileInfo content1Info =
                                    fileFolderService.create(
                                            container6, "Content" + i, ContentModel.TYPE_CONTENT);
                            NodeRef nodeRef = content1Info.getNodeRef();
                            contents.add(nodeRef);
        ...
        txnHelper.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        FileInfo contentInfo =
                                fileFolderService.create(
                                        container7, "Content1", ContentModel.TYPE_CONTENT);
                        contents.add(contentInfo.getNodeRef());
    ...
    private boolean containsProperty(
            Map<QName, String> propertyMap, QName propName, String propValue) throws Exception {
        if (propName == null) {

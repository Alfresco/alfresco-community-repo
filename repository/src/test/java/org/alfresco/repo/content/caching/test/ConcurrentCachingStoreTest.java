...
public class ConcurrentCachingStoreTest {
  ...
  public void setUp() {
    ...
    ctx =
      ApplicationContextHelper.getApplicationContext(new String[] { slowconf });
  ...
  public void concurrentReadsWillReadCacheOncePerURL()
    ...
    for (Map.Entry<String, AtomicLong> entry : backingStore
      .getUrlHits()
      .entrySet()) {
    ...
    if (failedURLs > 0) Assert.fail(
      failedURLs + " URLs were requested more than once."
    );
  }
  ...
  private class CacheReaderThread extends Thread {
    ...
    public void run() {
      while (reads > 0) {
        ...
        log.debug(
          "Thread: " + getName() + ", URL: " + url + ", content: " + content
        );
        reads--;
      }
    }

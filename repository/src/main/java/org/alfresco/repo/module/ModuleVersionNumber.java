...
/**
 * The exising Alfresco VersionNumber can only be numeric. ModuleVersionNumber allows string
 * literals in the version number.
 *
 * <p>It follows maven conventions and actually uses the ComparableVersion class from the maven code
 * base.
 *
 * @author Gethin James
 */
public class ModuleVersionNumber implements Externalizable {
    ...
    private static Pattern A_M_RC_VERSION_PATTERN =
            Pattern.compile("((\\d+\\.){2,3}\\d+)(-(A|M|RC)\\d+)?");
    ...
    /**
     * Now that we create internal releases of the form {@code M.m.r-M9} (milestone), {@code
     * M.m.r-A9} (alpha) and {@code M.m.r-RC9} (release candidate) during development and testing,
     * we need to ensure we can upgrade from any of these to any other, as they may occur in any
     * order and also to the final external release {@code M.m.r}. We also will allow a change from
     * the final release back to an internal one for regression testing.
     *
     * <p>The code within {@link ModuleComponentHelper} which calls this method, checks if it is the
     * same version ({@code 0}) and then if it is downgrading ({@code > 0}), so if they share the
     * same {@code M.m.r} part matches AND is one of these formats, we return {@code <0}.
     *

    ...
    public void onDictionaryInit() {
        ...
        Collection<QName> modelsBefore =
                dictionaryDAO.getModels(
                        true); // note: on first bootstrap will init empty dictionary
            ...
            for (String bootstrapModel : models) {
                ...
                if (modelStream == null) {
                    throw new DictionaryException(
                            "d_dictionary.bootstrap.model_not_found", bootstrapModel);
                }
                try {
                    ...
                    throw new DictionaryException(
                            "d_dictionary.bootstrap.model_not_imported", e, bootstrapModel);
                } finally {
                    try {
                        modelStream.close();
                    } catch (IOException ioe) {
                        logger.warn(
                                "Failed to close model input stream for '"
                                        + bootstrapModel
                                        + "': "
                                        + ioe);
                    }

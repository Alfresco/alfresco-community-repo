 ...
 * @author kroast
 */
public abstract class BaseSSOAuthenticationFilter extends BaseAuthenticationFilter
    ...
    public void doFilter(
            ...
            getLogger()
                    .debug(
                            req.getMethod()
                                    + " "
                                    + getScriptUrl(req)
                                    + "not found in Public API Container.");
        }
        ...
        // check the authentication required - if none then we don't want any of the filters down
        // the chain to require any authentication checks
        else if ((match != null)
                && (match.getWebScript() != null)
                && (RequiredAuthentication.none
                        == match.getWebScript().getDescription().getRequiredAuthentication())) {
            if (getLogger().isDebugEnabled()) {
                getLogger()
                        .debug(
                                "Found webscript with no authentication - set NO_AUTH_REQUIRED"
                                        + " flag.");
            }
    ...
    /**
     * Initializes the filter. Only called if the filter is active, as indicated by {@link
     * #isActive()}. Subclasses should override.
     */
     ...
     */
    protected boolean onLoginComplete(
            ServletContext sc, HttpServletRequest req, HttpServletResponse res, boolean userInit)
            throws IOException {
        return true;
    }
     ...
     * @return boolean
     */
    protected boolean checkForTicketParameter(
            ...
            try {
                ...
                if (user == null) {
                    if (getLogger().isDebugEnabled()) {
                        getLogger()
                                .debug(
                                        "There is no valid cached user, validate the ticket and"
                                                + " create one.");
                    }
                ...
                if (getLogger().isDebugEnabled()) {
                    getLogger()
                            .debug(
                                    "Failed to authenticate user ticket: " + authErr.getMessage(),
                                    authErr);
                }
            } catch (Throwable e) {
                if (getLogger().isDebugEnabled()) {
                    getLogger()
                            .debug(
                                    "Error during ticket validation and user creation: "
                                            + e.getMessage(),
                                    e);
                }
            } finally {
    ...
    protected void redirectToLoginPage(HttpServletRequest req, HttpServletResponse res)
        ...
        if (hasLoginPage()) res.sendRedirect(req.getContextPath() + "/faces" + getLoginPage());
    }
    ...
    protected final boolean isNTLMSSPBlob(byte[] byts, int offset) {
        ...
        if ((byts.length - offset) >= NTLM.Signature.length) {
            ...
            int idx = 0;
            while (idx < NTLM.Signature.length && byts[offset + idx] == NTLM.Signature[idx]) idx++;

            if (idx == NTLM.Signature.length) isNTLMSSP = true;
        }
    ...
    /**
     * Because the file server configuration may change during the lifetime of this filter, this
     * method checks against the last configured server name before returning a cached result
     *
    ...
    protected synchronized String getServerName() {
        ...
        if (m_lastResolvedServerName != null
                && (m_lastConfiguredServerName == null && srvName == null
                        || m_lastConfiguredServerName.equals(srvName))) {
            ...
            if (m_lastConfiguredServerName != null) {
                try {
                    ...
                    if (getLogger().isWarnEnabled()) {
                        getLogger()
                                .warn(
                                        "NTLM filter, error resolving CIFS host name"
                                                + m_lastConfiguredServerName);
                    }
                }
            }

            // If we still do not have a name use the DNS name of the server, with the domain part
            // removed
    ...
    protected SecurityConfigSection getSecurityConfigSection() {
        return serverConfiguration == null
                ...
                : (SecurityConfigSection)
                        serverConfiguration.getConfigSection(SecurityConfigSection.SectionName);
    }

    /**
     * Writes link to login page and refresh tag which cause user to be redirected to the login
     * page.
     *
     ...
     * @throws IOException
     */
    protected void writeLoginPageLink(
     ...
     * @throws IOException
     */
    protected void includeFallbackAuth(
            ServletContext context, HttpServletRequest req, HttpServletResponse resp)
            throws IOException {
     ...
     */
    protected boolean performFallbackAuthentication(
            ServletContext context, HttpServletRequest req, HttpServletResponse resp)
            throws IOException, ServletException {
        if (getLogger().isTraceEnabled()) {
    ...
    private String getScriptUrl(HttpServletRequest req) {
        ...
        if (pathInfo.length() < 2 || pathInfo.equals("/")) {
            ...
            if ((pathInfo.length() > 5 && !pathInfo.substring(0, 6).toLowerCase().equals("/cmis/"))

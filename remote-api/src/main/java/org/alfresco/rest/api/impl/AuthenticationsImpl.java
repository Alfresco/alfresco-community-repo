...
public class AuthenticationsImpl implements Authentications {
  ...
  @Override
  public LoginTicketResponse createTicket(
  ...
  protected String getTicket(Parameters parameters) {
    ...
    final String authorization = parameters
      .getRequest()
  ...
  private String getTicketFromRemoteUserMapperUserId(Parameters parameters) {
    ...
    HttpServletRequest httpServletRequest = extractHttpServletRequestFromParameters(
      parameters
    );
    ...
    throw new InvalidArgumentException(
      "Can't use Alfresco Identity Services to validate the user in the bearer access token"
    );
  }

  private HttpServletRequest extractHttpServletRequestFromParameters(
    Parameters parameters
  ) {
    ...
    if (
      parameters.getRequest() instanceof BufferedRequest &&
      (
        (BufferedRequest) parameters.getRequest()
      ).getNext() instanceof PublicApiTenantWebScriptServletRequest
    ) {
      return (

...
/**
 * Test case which shows that the user who did not create a transfer folder will be able to see and
 * action on it if he gets filing permission on the transfer folder.
 *
 * @author Tuna Aksoy
 * @since 2.3
 */
                                ...
                                new RunAsWork<Void>() {
                                    public Void doWork() {
                                        ...
                                        filePlanPermissionService.setPermission(
                                                category, testUser1, FILING);
                                        filePlanPermissionService.setPermission(
                                                category, testUser2, FILING);
                        ...
                        runAs(
                                new RunAsWork<Void>() {
                                    public Void doWork() {
                                        ...
                                        // testUser2 should have read permissions on the transfers
                                        // container
                                        assertEquals(
                                                ALLOWED,
                                                permissionService.hasPermission(
                                                        transfersContainer, READ_RECORDS));

                                        // Check if testUser2 has read permissions on the transfer
                                        // folder
                                        assertEquals(
                                                ALLOWED,
                                                permissionService.hasPermission(
                                                        transferFolder, READ_RECORDS));

                                        // Check if testUser2 has filing permissions on the transfer
                                        // folder
                                        assertEquals(
                                                ALLOWED,
                                                permissionService.hasPermission(
                                                        transferFolder, FILING));

                                        // FIXME: Should be able to execute the action. Failing
                                        // intermittently. See RM-3931
                                        // Execute transfer complete action as testUser2 who has
                                        // filing permissions on the transfer folder

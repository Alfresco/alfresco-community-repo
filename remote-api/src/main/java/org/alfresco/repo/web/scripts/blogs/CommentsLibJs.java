...
class CommentsLibJs {
   ...
   */
  public static List<ChildAssociationRef> getComments(
    NodeRef node,
    ...
    if (commentsFolder != null) {
      List<ChildAssociationRef> children = services
        .getNodeService()
        .getChildAssocs(
          commentsFolder,
   ...
   */
  public static NodeRef getCommentsFolder(
    NodeRef node,
    ServiceRegistry services
  ) {
    ...
    NodeRef result = null;
    if (
      services.getNodeService().hasAspect(node, ForumModel.ASPECT_DISCUSSABLE)
    ) {
      List<ChildAssociationRef> forumFolders = services
        .getNodeService()
        .getChildAssocs(
          node,
      ...
      List<ChildAssociationRef> topicFolder = services
        .getNodeService()
        .getChildAssocs(
          forumFolder,
          ContentModel.ASSOC_CONTAINS,

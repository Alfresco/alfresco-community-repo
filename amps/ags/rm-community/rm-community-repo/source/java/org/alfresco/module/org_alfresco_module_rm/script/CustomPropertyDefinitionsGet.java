...
public class CustomPropertyDefinitionsGet extends BaseCustomPropertyWebScript {
  ...
  private static Log logger = LogFactory.getLog(
    CustomPropertyDefinitionsGet.class
  );
  ...
  public Map<String, Object> executeImpl(
    ...
    if (logger.isDebugEnabled() && elementName != null) {
      logger.debug(
        "Getting custom property definitions for elementName " + elementName
      );
    ...
    if (propId != null) {
      ...
      PropertyDefinition propDefn = rmAdminService
        .getCustomPropertyDefinitions()
        .get(propQName);
      if (propQName == null || propDefn == null) {
        throw new WebScriptException(
          Status.STATUS_NOT_FOUND,
          "Property definition for " + propId + " not found."
        );
      }
      propData.add(propDefn);
    } else if (elementName != null) {
      QName customisableType = mapToTypeQName(elementName);
      Map<QName, PropertyDefinition> currentCustomProps = rmAdminService.getCustomPropertyDefinitions(
        customisableType
      );
      ...
      throw new WebScriptException(
        ...
        "Either elementName or propId must be specified."
      );
    }

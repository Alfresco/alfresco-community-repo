...
public class GetPeoplePreferenceFullTests extends RestTest {
  ...
  public void propertiesParameterIsAppliedWhenRetrievingPreference()
    throws Exception {
    restPreferenceModel =
      restClient
    ...
    restPreferenceModel =
      restClient
        .authenticateUser(userModel)
        .withParams("properties=id,value")
        .withCoreAPI()
        .usingUser(userModel)
        .getPersonPreferenceInformation(
          String.format(
  ...
  public void validateIdElementInGetFolderPreferenceResponse()
    throws Exception {
    FolderModel folderFavorite = new FolderModel("favoriteFolder");
    folderFavorite =
      dataContent.usingSite(siteModel).createFolder(folderFavorite);
    dataContent
      .getContentActions()
      .setFolderAsFavorite(
    ...
    restPreferenceModel
      .assertThat()
      .field("id")
      .is(PreferenceName.FOLDERS_FAVORITES_PREFIX)
      .and()
      .field("value")
      .is(
        Utility.removeLastSlash(
          Utility.buildPath(
            "workspace://SpacesStore",
            folderFavorite.getNodeRef()
          )
        )
      );
  }
  ...
  public void validateIdElementInGetFilePreferenceResponse() throws Exception {
    ...
    restPreferenceModel
      .assertThat()
      .field("id")
      .is(PreferenceName.DOCUMENTS_FAVORITES_PREFIX)
      .and()
      .field("value")
      .is(
        Utility.removeLastSlash(
          Utility.buildPath(
            "workspace://SpacesStore",
            fileFavorite.getNodeRefWithoutVersion()
          )
        )
      );
  }
  ...
  public void getPreferenceForUserWithoutPreferences() throws Exception {
    ...
    restClient
      ...
      .containsSummary(
        String.format(
          "The relationship resource was not found for the" +
          " entity with id: %s and a relationship id of %s",
          newUser.getUsername(),
          PreferenceName.DOCUMENTS_FAVORITES_PREFIX.toString()
        )
      );
  ...
  public void changePreferenceThenPerformGetPreferenceCall() throws Exception {
    ...
    dataContent
      ...
      .setFileAsFavorite(
        ...
        String.format(
          "%s.%s",
          fileFavorite.getName(),
          fileFavorite.getFileType().extension
        )
      );
    ...
    dataContent
      ...
      .removeFavorite(
        ...
        newUser.getPassword(),
        site.getId(),
        Paths.get(fileFavorite.getCmisLocation()).getFileName().toString()
      );
  ...
  public void regularUserIsNotAbleToGetAdminPreference() throws Exception {
    ...
    restClient
      ...
      .usingUser(dataUser.getAdminUser())
      .getPersonPreferenceInformation(
        PreferenceName.SITES_FAVORITES_PREFIX + newSite.getId()
      );

...
/**
 * Search SQL API end point. An implementation of the
 * {{baseUrl}}/{{networkId}}/public/search/versions/1/sql
 *
 * @author Michael Suzuki
 */
public class SearchSQLApiWebscript extends AbstractWebScript
    ...
    @Override
    public void execute(WebScriptRequest webScriptRequest, WebScriptResponse res)
            throws IOException {
        try {
            // Turn JSON into a Java object representation
            SearchSQLQuery searchQuery =
                    extractJsonContent(
                            webScriptRequest, assistant.getJsonHelper(), SearchSQLQuery.class);
            ...
            if (searchQuery.getFormat().equalsIgnoreCase("solr")) {
                ...
                CollectionWithPagingInfo<TupleList> nodes =
                        resultMapper.toCollectionWithPagingInfo(ssjr.getDocs(), searchQuery);
            ...
            if (exception instanceof QueryParserException) {
                ...
                renderException(
                        new WebScriptException(400, exception.getMessage()), res, assistant);
            }
        }
    }

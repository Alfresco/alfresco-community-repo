 ...
 * @author gkspencer
 */
public abstract class AlfrescoTxDiskDriver extends AlfrescoDiskDriver
     ...
     *
     * <p>WARNING : side effect - that the current transaction, if any, is ended.
     *
     * @param sess the server session
     * @param callback callback for the retryable operation
                                    ...
                                    new RetryingTransactionHelper.RetryingTransactionCallback<T>() {
                                        public T execute() throws Throwable {
                                            try {
                                                ...
                                                // Ensure original checked IOExceptions get
                                                // propagated
        ...
        if (filesysTx != null && filesysTx.hasTransaction()) {
            ...
            try {
                ...
                if (ftx.getStatus() == Status.STATUS_MARKED_ROLLBACK
                    ...
                    if (logger.isDebugEnabled()) logger.debug("End transaction (rollback)");
                } else {

...
public class DescriptorServiceImpl extends AbstractLifecycleBean
    ...
    // License changes must hold a synchronized lock on licenseLock so that these changes can by
    // synchronized on
    // startup (in a single JVM) BUT at the point of setting the currentRepoDescriptor they must
    // also obtain the
    // currentRepoDescriptorLock.writeLock. This is required for the situation where the database
    // change takes a long
    // time or there are reties. This update has been known to clash with authorize user updates
    // which also updates the
    // current user count. To avoid HTTP requests from hanging, when looking up the
    // currentRepoDescriptor they now
    // obtain the currentRepoDescriptorLock.readLock rather than the same synchronized lock. The
    // writeLock is only held
    // for as long as it takes to update the currentRepoDescriptor reference. The database update
    // takes place inside
    // the synchronized lock but more importantly in the database as there may be a cluster. This
     ...
     *
     * @param serverDescriptorDAO the new server descriptor DAO
     */
     ...
     *
     * @param currentRepoDescriptorDAO the new current repo descriptor DAO
     */
     ...
     *
     * @param installedRepoDescriptorDAO the new installed repo descriptor DAO
     */
    ...
    public Descriptor getCurrentRepositoryDescriptor() {
        ...
        try {
            // this should be updated when the License is verified - but it is possible that no
            // License at all
            // is available and this will be in an known state - do not allow a null value to be
            // returned
            return this.currentRepoDescriptor != null
                    ? this.currentRepoDescriptor
    ...
    private boolean isCurrentRepoDescriptorIsNull() {
        // Should not be needed as all updates take place within "synchronized (licenseLock)" but
        // this is clearer
        currentRepoDescriptorLock.readLock().lock();
        try {
    ...
    private boolean currentRepoDescriptorNullOrModeHasChanged(LicenseMode newMode) {
        ...
        try {
            return currentRepoDescriptor == null
                    || newMode != currentRepoDescriptor.getLicenseMode();
        } finally {
            currentRepoDescriptorLock.readLock().unlock();
        }
    }
    ...
    public String loadLicense() {
        ...
        final RetryingTransactionCallback<String> loadCallback =
     ...
     *
     * <p>Also set installedRepoDescriptor and update current
     */
    @Override
    protected void onBootstrap(ApplicationEvent event) {
        AuthenticationUtil.RunAsWork<Void> bootstrapWork =
                new RunAsWork<Void>() {
                    @Override
        ...
        ((ApplicationContext) event.getSource())
                .publishEvent(new DescriptorServiceAvailableEvent(this));
    }
    ...
    private void bootstrap() {
        ...
        RetryingTransactionCallback<Descriptor> getDescriptorCallback =
                new RetryingTransactionCallback<Descriptor>() {
        ...
        licenseService =
                (LicenseService)
                        constructSpecialService("org.alfresco.enterprise.license.LicenseComponent");
        ...
        if (applicationContext instanceof ConfigurableApplicationContext) {
            ((ConfigurableApplicationContext) applicationContext)
                    .getBeanFactory()
                    .registerSingleton("licenseService", licenseService);
        }
    ...
    /**
     * Constructs a special service whose dependencies cannot or should not be injected
     * declaratively. Examples include the license component and heartbeat service that are
     * intentionally left unconfigurable.
     *
    ...
    private Object constructSpecialService(String className) {
        try {
            ...
            Constructor<?> constructor =
                    componentClass.getConstructor(new Class[] {ApplicationContext.class});
            return constructor.newInstance(new Object[] {getApplicationContext()});
    ...
    private class NOOPLicenseService implements LicenseService {
        /**
         * Ensures that a repository Descriptor is available. This is done here to prevent
         * flip-flopping on the license mode during startup i.e. the Enterprise mode will be set
         * once when the real license is validated.
         */
        public void verifyLicense() throws LicenseException {
            synchronized (licenseLock) {
                if (isCurrentRepoDescriptorIsNull()) {
                    ...
                    RetryingTransactionCallback<Void> nopLoadLicense =
                            new RetryingTransactionCallback<Void>() {
                                @Override
                                public Void execute() {
                                    Descriptor newRepoDescriptor =
                                            currentRepoDescriptorDAO.updateDescriptor(
                                                    serverDescriptor, LicenseMode.UNKNOWN);
     ...
     * @author gavinc
     */
    public abstract static class BaseDescriptor implements Descriptor {
        ...
        protected int getSchema(String schemaStr) {
            ...
            try {
                ...
                throw new AlfrescoRuntimeException(
                        "Schema must be a positive integer '" + schemaStr + "' is not!");
            }
        }
    }
    ...
    public void onLicenseChange(final LicenseDescriptor licenseDescriptor) {
        synchronized (licenseLock) {
            ...
            RetryingTransactionCallback<RepoUsage> updateLicenseCallback =
                    new RetryingTransactionCallback<RepoUsage>() {
                        public RepoUsage execute() {
                            // Configure the license restrictions
                            RepoUsage.LicenseMode newMode = licenseDescriptor.getLicenseMode();
                            Long expiryTime =
                                    licenseDescriptor.getValidUntil() == null
                                            ? null
                                            : licenseDescriptor.getValidUntil().getTime();
                            ...
                            // persist the server descriptor values in the current repository
                            // descriptor
                            if (currentRepoDescriptorNullOrModeHasChanged(newMode)) {
                                if (logger.isDebugEnabled()) {
                                    logger.debug(
                                            "Changing license mode in current repo descriptor to: "
                                                    + newMode);
                                }
                                Descriptor newRepoDescriptor =
                                        currentRepoDescriptorDAO.updateDescriptor(
                                                serverDescriptor, newMode);
                                setCurrentRepoDescriptor(newRepoDescriptor);
                            }
                            if (logger.isDebugEnabled()) {
     ...
     * {@inheritDoc}
     *
     * <p>Restrictions are not changed; previous restrictions remain in place.
     */
    @Override

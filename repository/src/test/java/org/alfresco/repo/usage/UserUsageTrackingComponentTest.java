...
public class UserUsageTrackingComponentTest extends TestCase {
  ...
  public static StoreRef SPACES_STORE = new StoreRef(
  ...
  protected void setUp() throws Exception {
    ...
    contentUsageService =
      (ContentUsageService) applicationContext.getBean("ContentUsageService");

    userUsageTrackingComponent =
      (UserUsageTrackingComponent) applicationContext.getBean(
        "userUsageTrackingComponent"
      );
    ...
    try {
      ...
      for (int i = 1; i <= MAX_USERS; i++) {
        ...
        if (!authenticationService.authenticationExists(userName)) {
          ...
          NodeRef content = addTextContent(
            homeFolder,
            namePrefix + "-" + userName + "-1.txt",
            sb.toString()
          );
          contentSize =
            contentService
              .getReader(content, ContentModel.PROP_CONTENT)
              .getSize();

          content =
            addTextContent(
              homeFolder,
              namePrefix + "-" + userName + "-2.txt",
              sb.toString()
            );
          contentSize +=
            contentService
        ...
        if (((i % PROGRESS_SIZE) == 0) && (i != MAX_USERS)) {
          logger.debug(
            ...
            PROGRESS_SIZE +
            " users created in " +
            ((System.currentTimeMillis() - progressStart) / 1000) +
            " secs"
          );
  ...
  public void test1EnableDisableCollapse() {
    ...
    for (int i = 1; i <= MAX_USERS; i++) {
      ...
      NodeRef childNodeRef = nodeService.getChildByName(
        homeFolder,
        ContentModel.ASSOC_CONTAINS,
        fileNamePrefix + "-" + userName + "-1.txt"
      );
  ...
  public void test2RecalculateUserUsage() throws Exception {
    ...
    for (int i = 1; i <= MAX_USERS; i++) {
      String userName = TEST_USER_PREFIX + i;
      Long totalExpected =
        expectedResultsA.get(userName) + expectedResultsB.get(userName);
      assertEquals(
        totalExpected,
        Long.valueOf(contentUsageService.getUserUsage(userName))
      );
    }
  }
  ...
  private void checkCalculated(long factor) {
    for (int i = 1; i <= MAX_USERS; i++) {
      ...
      Long sizeProp = (Long) nodeService.getProperty(
        personNodeRef,
        ContentModel.PROP_SIZE_CURRENT
      );
      assertNotNull(
        "Property " + ContentModel.PROP_SIZE_CURRENT + "does not exist, yet",
        sizeProp
      );
  ...
  private void checkUsage(long factor) {
    for (int i = 1; i <= MAX_USERS; i++) {
      ...
      assertEquals(
        userName,
        i * factor,
        contentUsageService.getUserUsage(userName)
      );
    }
  }
  ...
  private void checkCleared(boolean isCleared) {
    for (int i = 1; i <= MAX_USERS; i++) {
      String userName = TEST_USER_PREFIX + i;
      NodeRef personNodeRef = personService.getPerson(userName);
      Long sizeCurrent = (Long) nodeService.getProperty(
        personNodeRef,
        ContentModel.PROP_SIZE_CURRENT
      );
  ...
  private NodeRef getHomeSpaceFolderNode(String userName) {
    return (NodeRef) this.nodeService.getProperty(

...
public class FileFolderLoaderTest extends TestCase {
                ...
                new RunAsWork<Void>() {
                    ...
                    public Void doWork() throws Exception {
                        ...
                        NodeRef companyHomeNodeRef =
                                fileFolderLoader.getRepository().getCompanyHome();
                        NodeRef sharedHomeNodeRef =
                                fileFolderLoader.getRepository().getSharedHome();
                        List<FileInfo> sharedHomeFileInfos =
                                fileFolderService.getNamePath(
                                        companyHomeNodeRef, sharedHomeNodeRef);
                        sharedHomePath = "/" + sharedHomeFileInfos.get(0).getName();

                        // Create a folder that will be invisible to all normal users
                        FileInfo hiddenFolderInfo =
                                fileFolderService.create(
                                        sharedHomeNodeRef, "HideThis", ContentModel.TYPE_FOLDER);
                        ...
                        permissionService.setPermission(
                                readOnlyFolderNodeRef,
                                PermissionService.ALL_AUTHORITIES,
                                PermissionService.READ,
                                true);

                        // Create a folder to write to
                        FileInfo writeFolderInfo =
                                fileFolderService.create(
                                        sharedHomeNodeRef, "WriteThis", ContentModel.TYPE_FOLDER);
    ...
    public void testIllegalArgs_MinMax() throws Exception {
        try {
            fileFolderLoader.createFiles(
                    sharedHomePath, 1, 256, 100L, 10L, Long.MAX_VALUE, false, 10, 256);
    ...
    public void testIllegalArgs_DescriptionSize() throws Exception {
        try {
            fileFolderLoader.createFiles(
                    sharedHomePath, 1, 256, 1024L, 10L, Long.MAX_VALUE, false, 10, Long.MAX_VALUE);
    ...
    public void testNoPermissionsAtAll() throws Exception {
        try {
            fileFolderLoader.createFiles(
                    sharedHomePath, 0, 256, 1024L, 1024L, Long.MAX_VALUE, false, 10, 256L);
    ...
    public void testNoPermissionsToFindFolder() throws Exception {
        try {
            ...
            fileFolderLoader.createFiles(
                    hiddenFolderPath, 0, 256, 1024L, 1024L, Long.MAX_VALUE, false, 10, 256L);
            fail("No permissions to see folder.");
    ...
    public void testNoPermissionsToWriteToFolder() throws Exception {
        try {
            ...
            AuthenticationUtil.setFullyAuthenticatedUser("BOB-1");
            fileFolderLoader.createFiles(
                    readOnlyFolderPath, 1, 256, 1024L, 1024L, Long.MAX_VALUE, false, 10, 256L);
    ...
    public void testLoad_ZeroFiles() throws Exception {
        try {
            ...
            int created =
                    fileFolderLoader.createFiles(
                            writeFolderPath, 0, 256, 1024L, 1024L, Long.MAX_VALUE, false, 10, 256L);
    ...
    public void testLoad_OneFile() throws Exception {
        try {
            ...
            int created =
                    fileFolderLoader.createFiles(
                            writeFolderPath, 1, 256, 1024L, 1024L, Long.MAX_VALUE, false, 10, 256L);
            assertEquals("Incorrect number of files generated.", 1, created);
            // Check the descriptions
            RetryingTransactionCallback<Void> checkCallback =
                    new RetryingTransactionCallback<Void>() {
                        @Override
                        public Void execute() throws Throwable {
                            MLPropertyInterceptor.setMLAware(true);
                            List<ChildAssociationRef> childAssocs =
                                    nodeService.getChildAssocs(writeFolderNodeRef);
                            // Count
                            ...
                            MLText descriptions =
                                    (MLText)
                                            nodeService.getProperty(
                                                    fileNodeRef, ContentModel.PROP_DESCRIPTION);
                            assertNotNull("No descriptions added", descriptions);
                            assertEquals(
                                    "Incorrect number of unique descriptions added: ",
                                    10,
                                    descriptions.size());
                            assertTrue(
                                    "Expect the default language to be present. ",
                                    descriptions.containsKey(
    ...
    public void testLoad_03() throws Exception {
        try {
            ...
            int created =
                    fileFolderLoader.createFiles(
                            writeFolderPath, 15, 10, 1024L, 1024L, Long.MAX_VALUE, false, 10, 256L);
            ...
            for (FileInfo fileInfo : fileInfos) {
                ...
                if (lastText == null) {
                    ...
                    assertNotEquals(
                            "All text must differ due to varying seed. ", lastText, currentStr);
                    lastText = currentStr;
                }
                // Check description
                if (lastDescr == null) {
                    lastDescr =
                            DefaultTypeConverter.INSTANCE.convert(
                                    ...
                                    nodeService.getProperty(
                                            fileNodeRef, ContentModel.PROP_DESCRIPTION));
                    assertEquals(
                            "cm:description length is incorrect. ",
                            256,
                            lastDescr.getBytes().length);
                } else {
                    String currentDescr =
                            DefaultTypeConverter.INSTANCE.convert(
                                    String.class,
                                    nodeService.getProperty(
                                            fileNodeRef, ContentModel.PROP_DESCRIPTION));
                    assertNotEquals(
                            "All descriptions must differ due to varying seed. ",
                            lastDescr,
                            currentDescr);
    ...
    public void testLoad_04() throws Exception {
        try {
            ...
            int created =
                    fileFolderLoader.createFiles(
                            writeFolderPath, 10, 10, 1024L, 1024L, 1L, true, 10, 256L);
            ...
            for (FileInfo fileInfo : fileInfos) {
                ...
                if (lastText == null) {
                    ...
                    String currentStr = reader.getContentString();
                    assertEquals(
                            "All text must be identical due to same seed. ", lastText, currentStr);
                    lastText = currentStr;
                }
                // Check description
                if (lastDescr == null) {
                    ...
                    String currentDescr =
                            DefaultTypeConverter.INSTANCE.convert(
                                    ...
                                    nodeService.getProperty(
                                            fileNodeRef, ContentModel.PROP_DESCRIPTION));
                    assertEquals(
                            "All descriptions must be identical due to varying seed. ",
                            lastDescr,
                            currentDescr);
                    lastDescr = currentDescr;
                }

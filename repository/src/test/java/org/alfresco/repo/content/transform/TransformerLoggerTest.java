...
public class TransformerLoggerTest {
  ...
  private TransformerLogger<String> newLogger(
    final AtomicInteger numberOfMessagesToAdd,
  ...
  public void noEntriesDisabledTest() {
    assertArrayEquals(
      new String[] {
        "No entries are available. property.name must be set to a number between 1 and 176",
      },
      log.getEntries(10)
    );
  }
  ...
  public void addTwoEntriesTest() {
    ...
    log.debug("two");
    assertArrayEquals(
      new String[] { "two", "two", "one", "one" },
      log.getEntries(10)
    );

    log.debug("three");
    assertArrayEquals(
      new String[] { "three", "three", "two", "two", "one" },
      log.getEntries(10)
    );
  }
  ...
  public void addRemoveModifyTest() {
    ...
    log.debug("three");
    assertArrayEquals(
      new String[] { "three", "three", "two", "two", "one", "one" },
      log.getEntries(10)
    );

...
/**
 * Retrieves statistics using solr. For a list of potential facets call it with
 * /api/solr/stats?listFacets=true You can pass one of these facets in eg. facet=content.creator .
 * The facet name can be used as a I18n resource bundle key, it also has a predefined structure:
 * group.property[.type] eg. content.created.datetime. The [.type] is optional, the default is
 * String.</description>
 *
 * @author Gethin James
 */
public class StatsGet extends DeclarativeWebScript {
    ...
    protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
        ...
        Pair<LocalDate, LocalDate> startAndEnd =
                getStartAndEndDates(req.getParameter("startDate"), req.getParameter("endDate"));
        query = buildQuery(siteInfo, facetKey, startAndEnd);

        StatsParameters params =
                new StatsParameters(SearchService.LANGUAGE_INDEX_FTS_ALFRESCO, query, false);
        // params.addSort(new SortDefinition(SortDefinition.SortType.FIELD, this.statsField,
        // false));
        params.addStatsParameter(StatsParameters.PARAM_FIELD, this.statsField);
        params.addStatsParameter(
                StatsParameters.PARAM_FACET, StatsParameters.FACET_PREFIX + propFacet.toString());
    ...
    protected String buildQuery(
            SiteInfo siteInfo, String facetKey, Pair<LocalDate, LocalDate> startEndDate) {

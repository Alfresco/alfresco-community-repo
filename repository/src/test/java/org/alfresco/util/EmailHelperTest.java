...
public class EmailHelperTest {
    ...
    private static final String FALLBACK_TEMPLATE_PATH =
            "alfresco/templates/email-templates/test-email-template.ftl";
    private static final String CLIENT_NAME = "test-client";

    @Mock private ServiceRegistry serviceRegistryMock;
    @Mock private NodeService nodeServiceMock;
    @Mock private SearchService searchServiceMock;
    @Mock private NamespaceService namespaceServiceMock;
    @Mock private FileFolderService fileFolderServiceMock;
    @Mock private PersonService personServiceMock;
    @Mock private PreferenceService preferenceServiceMock;
    @Mock private Repository repositoryHelperMock;
    @Mock private TemplateLoader templateLoaderMock;
    ...
    public void setup() throws Exception {
        ...
        when(repositoryHelperMock.getRootHome())
                .thenReturn(new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, GUID.generate()));
        when(fileFolderServiceMock.getLocalizedSibling(dummyTemplateNodeRef))
                .thenReturn(dummyTemplateNodeRef);
    ...
    public void testGetEmailTemplate() throws Exception {
        ...
        assertEquals(
                "The given template path is null, so the fallback template should be returned.",
                result,
                FALLBACK_TEMPLATE_PATH);

        // XPath
        {
            final String emailTemplateXPath =
                    "app:company_home/app:dictionary/app:email_templates/example-email.ftl";

            when(searchServiceMock.selectNodes(
                            repositoryHelperMock.getRootHome(),
                            emailTemplateXPath,
                            null,
            ...
            result =
                    emailHelper.getEmailTemplate(
                            CLIENT_NAME, emailTemplateXPath, FALLBACK_TEMPLATE_PATH);
            assertEquals(
                    "Couldn't find email template with the given XPath, so the fallback template"
                            + " should be returned.",
                    result,
                    FALLBACK_TEMPLATE_PATH);
            ...
            result =
                    emailHelper.getEmailTemplate(
                            CLIENT_NAME, emailTemplateXPath, FALLBACK_TEMPLATE_PATH);
            // XPath search returned 3 results. But after logging an error, we just return the first
            // element
            assertEquals(result, dummyTemplateNodeRef.toString());
        }
        ...
        // NodeRef
        {
            ...
            result =
                    emailHelper.getEmailTemplate(
                            CLIENT_NAME, dummyTemplateNodeRef.toString(), FALLBACK_TEMPLATE_PATH);
            assertEquals(
                    "Couldn't find email template with the given NodeRef, so the fallback template"
                            + " should be returned.",
                    result,
                    FALLBACK_TEMPLATE_PATH);
        ...
        // Class path
        {
            ...
            result =
                    emailHelper.getEmailTemplate(
                            CLIENT_NAME, classPathTemplate, FALLBACK_TEMPLATE_PATH);
            assertEquals(
                    "Couldn't find email template with the given class path, so the fallback"
            ...
            when(templateLoaderMock.findTemplateSource(classPathTemplate))
                    .thenThrow(new IOException());
            result =
                    emailHelper.getEmailTemplate(
                            CLIENT_NAME, classPathTemplate, FALLBACK_TEMPLATE_PATH);
            assertEquals(
                    "Error occurred while finding the email template with the class path, so the"
                            + " fallback template should be returned.",
                    result,
                    FALLBACK_TEMPLATE_PATH);
        }

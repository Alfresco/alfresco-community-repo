...
public class Schema extends AbstractDbObject implements Iterable<DbObject> {
    ...
    /**
     * Construct a schema with the given name and database prefix. The database prefix specifies
     * what filtering applies to the high-level (tables and sequences) objects in the schema. If for
     * example dbPrefix is "alf_" then only tables and sequences whose names begin with "alf_" will
     * be represented by this schema. Therefore any comparisons should be performed against another
     * similarly filtered Schema object.
     *
     * @param name String
    ...
    protected void doDiff(DbObject right, DiffContext ctx) {
        ...
        comparisonUtils.compareSimple(
                new DbProperty(this, "version"), new DbProperty(rightSchema, "version"), ctx);

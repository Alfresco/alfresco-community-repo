...
public class EventBehaviourTest extends BaseSpringTest {
    ...
    @Autowired private AuthenticationComponent authenticationComponent;
        ...
        retryingTransactionHelper.doInTransaction(
                () -> {
                    ...
                    try {
                        ...
                        // First check that the policy is not fired when the versionable aspect is
                        // not
                        // present
                        ...
                        this.nodeService.addAspect(
                                this.contentNodeRef, ContentModel.ASPECT_VERSIONABLE, null);
                        ...
                        ContentWriter contentWriter3 =
                                this.contentService.getWriter(
                                        contentNodeRef, ContentModel.PROP_CONTENT, false);
                        contentWriter3.putContent("content update three");
                        assertFalse(this.policyFired);

                        // Now check that the policy isn't fired when the versionable aspect is
                        // present
                        // (because it's triggered only for the first event regardless how many
                        // times it
                        // is updated in that transaction)
                        this.nodeService.addAspect(
                                this.contentNodeRef, ContentModel.ASPECT_VERSIONABLE, null);
                        ContentWriter contentWriter4 =
                                this.contentService.getWriter(
                                        contentNodeRef, ContentModel.PROP_CONTENT, true);
    ...
    /**
     * Tests that the content update policy is triggered correctly for transaction commit (the
        ...
        retryingTransactionHelper.doInTransaction(
                () -> {
                    ...
                    try {
                        ...
                        // The policy is fired when the versionable aspect is present (on
                        // transaction
                        // commit)
                        this.nodeService.addAspect(
                                this.contentNodeRef, ContentModel.ASPECT_VERSIONABLE, null);

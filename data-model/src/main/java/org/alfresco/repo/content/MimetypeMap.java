...
public class MimetypeMap implements MimetypeService {
  ...
  static class Data {
    ...
    private Map<String, String> extensionsByMimetype = new HashMap<String, String>(
      59
    );
    ...
    private Map<String, String> displaysByExtension = new HashMap<String, String>(
      59
    );
   ...
   */
  public void setContentCharsetFinder(
    ContentCharsetFinder contentCharsetFinder
  ) {
  ...
  public void init() {
            ...
            try {
              ...
              if (mediaTypes != null && mediaTypes.isArray()) {
                ...
                for (JsonNode mediaType : mediaTypes) {
                  MediaTypeDef def = jsonObjectMapper.convertValue(
                    mediaType,
                    MediaTypeDef.class
                  );
                  GenericConfigElement mimetype = new GenericConfigElement(
                    ATTR_MIMETYPE
                  );
    ...
    if (tikaConfig == null) {
      logger.warn(
        "TikaConfig spring parameter not supplied, using default config"
      );
  ...
  public boolean readConfig() {
    ...
    Config config = configService.getConfig(
      CONFIG_CONDITION,
      new ConfigLookupContext(CONFIG_AREA)
    );
    ...
    if (jsonConfigFileFinder != null) {
      successReadingConfig &=
        jsonConfigFileFinder.readFiles("alfresco/mimetypes", logger);
  ...
  private void registerMimetypes(List<ConfigElement> mimetypes) {
    ...
    for (ConfigElement mimetypeElement : mimetypes) {
      ...
      boolean isText =
        Boolean.parseBoolean(isTextStr) || mimetype.startsWith(PREFIX_TEXT);
      boolean prevIsText = replacement
        ? data.textMimetypes.contains(mimetype)
        : !isText;
      ...
      for (ConfigElement extensionElement : extensions) {
        ...
        boolean isDefault =
          Boolean.parseBoolean(isDefaultStr) || prevExtension == null;
        if (isDefault) {
  ...
  private MediaType typeBasedOnDetectedTypeAndExtension(
    MediaType type,
    String filename
  ) {
  ...
  public String getMimetypeIfNotMatches(ContentReader reader) {
    ...
    if (
      tikaConfig.getMediaTypeRegistry().isSpecializationOf(claimed, type) ||
    ...
    SortedSet<MediaType> aliases = tikaConfig
  ...
  public String guessMimetype(String filename, InputStream input) {
    ...
    if (
      (
        ...
        MediaType.APPLICATION_XML.equals(type)
      ) &&

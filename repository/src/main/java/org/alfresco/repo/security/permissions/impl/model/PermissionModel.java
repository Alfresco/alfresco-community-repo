...
/**
 * The implementation of the model DAO Reads and stores the top level model information Encapsulates
...
public class PermissionModel implements ModelDAO {
     ...
     * PermissionReference)
     */
    private static PermissionReference ALL =
    ...
    private static class MutableState {
        ...
        private Map<PermissionReference, Set<PermissionReference>> grantingPermissions =
                new HashMap<PermissionReference, Set<PermissionReference>>(256);

        // Cache grantees
        private Map<PermissionReference, Set<PermissionReference>> granteePermissions =
                new HashMap<PermissionReference, Set<PermissionReference>>(256);

        // Cache the mapping of extended groups to the base
        private Map<PermissionGroup, PermissionGroup> groupsToBaseGroup =
                new HashMap<PermissionGroup, PermissionGroup>(256);

        private Map<RequiredKey, Set<PermissionReference>> requiredPermissionsCache =
                new HashMap<RequiredKey, Set<PermissionReference>>(1024);

        private Map<Pair<PermissionReference, RequiredPermission.On>, Set<PermissionReference>>
                unconditionalRequiredPermissionsCache =
                        new HashMap<
                                Pair<PermissionReference, RequiredPermission.On>,
                                Set<PermissionReference>>(1024);

        private Map<QName, Set<PermissionReference>> cachedTypePermissionsExposed =
                new HashMap<QName, Set<PermissionReference>>(256);
                ...
                if (pg.isExtends()) {
                    if (pg.getTypeQName() != null) {
                        ...
                        while ((parent = classDefinition.getParentName()) != null) {
                            ...
                            PermissionGroup attempt =
                                    getPermissionGroupOrNull(
                                            SimplePermissionReference.getPermissionReference(
                                                    parent, pg.getName()));
        ...
        private void addTypePermissions(
                QName type, Set<PermissionReference> permissions, boolean exposedOnly) {
        ...
        private void mergeGeneralAspectPermissions(
                Set<PermissionReference> target, boolean exposedOnly) {
        ...
        /**
         * Query the model for a base permission group Uses the Data Dictionary to reolve
         * inheritance
         *
         * @param pg PermissionGroup
         * @return the permission group
        ...
        private Set<PermissionReference> getGranteePermissionsImpl(
            ...
            for (PermissionSet ps : permissionSets.values()) {
                        ...
                        if (pg.isExtends()) {
                            if (pg.getTypeQName() != null) {
                                ...
                                ClassDefinition classDefinition =
                                        dictionaryService.getClass(pg.getQName());
                ...
                PermissionGroup baseGroup =
                        getBasePermissionGroupOrNull(getPermissionGroupOrNull(permissionReference));
                if (baseGroup != null) {
                    for (Permission p : ps.getPermissions()) {
                        for (PermissionReference grantedTo : p.getGrantedToGroups()) {
                            PermissionGroup base =
                                    getBasePermissionGroupOrNull(
        ...
        private Set<PermissionReference> getGranteePermissions(
            ...
            if (grantees == null) {
                ...
                try {
                    ...
                    if (grantees == null) {
                        Set<PermissionReference> internal =
                        ...
                        for (PermissionReference grantee : internal) {
                            grantees.add(
                                    SimplePermissionReference.getPermissionReference(
                                            grantee.getQName(), grantee.getName()));
                        }
        ...
        private Set<PermissionReference> getImmediateGranteePermissions(
            ...
            Set<PermissionReference> internal =
                    getImmediateGranteePermissionsImpl(permissionReference);
        ...
        private Set<PermissionReference> getRequiredPermissions(
            ...
            if (answer == null) {
                ...
                try {
                    ...
                    if (answer == null) {
                        PermissionGroup pg =
                                getBasePermissionGroupOrNull(getPermissionGroupOrNull(required));
                        if (pg == null) {
        ...
        private Set<PermissionReference> getUnconditionalRequiredPermissions(
                PermissionReference required, RequiredPermission.On on) {
            // Cache lookup as this is static
            if (required == null) {
                return Collections.<PermissionReference>emptySet();
            }
            Pair<PermissionReference, RequiredPermission.On> key =
                    new Pair<PermissionReference, RequiredPermission.On>(required, on);
        ...
        private Set<PermissionReference> getRequirementsForPermissionGroup(
                ...
                QName qName,
                Set<QName> aspectQNames) {
            HashSet<PermissionReference> requiredPermissions =
                    new HashSet<PermissionReference>(16, 1.0f);
            if (target == null) {
                return requiredPermissions;
            }
            for (PermissionSet ps : permissionSets.values()) {
                ...
                for (Permission p : ps.getPermissions()) {
                    for (PermissionReference grantedTo : p.getGrantedToGroups()) {
                        PermissionGroup base =
                                getBasePermissionGroupOrNull(getPermissionGroupOrNull(grantedTo));
                        if ((target.equals(base) || target.isAllowFullControl())
                                && (!base.isTypeRequired()
                                        || isPartOfDynamicPermissionGroup(
                                                grantedTo, qName, aspectQNames))) {
        ...
        private Set<PermissionReference> getUnconditionalRequirementsForPermissionGroup(
                PermissionGroup target, RequiredPermission.On on) {
            HashSet<PermissionReference> requiredPermissions =
                    new HashSet<PermissionReference>(16, 1.0f);
            if (target == null) {
                return requiredPermissions;
        ...
        private boolean isPartOfDynamicPermissionGroup(
                PermissionReference pr, QName typeQname, Set<QName> aspects) {
        ...
        private void buildUniquePermissionMap() {
            ...
            if (uniqueMap.containsKey(PermissionService.ALL_PERMISSIONS)) {
                throw new IllegalStateException(
                        "There must not be a permission with the same name as the ALL_PERMISSION"
                                + " constant: "
                                + PermissionService.ALL_PERMISSIONS);
            }
            uniqueMap.put(
                ...
                if (group.isAllowFullControl()) {
                    ...
                    if (group.isExtends()) {
                        if (group.getTypeQName() != null) {
                            return hasFull(
                                    SimplePermissionReference.getPermissionReference(
                                            group.getTypeQName(), group.getName()));
                        } else {
                            ClassDefinition classDefinition =
                                    dictionaryService.getClass(group.getQName());
                            QName parent;
                            while ((parent = classDefinition.getParentName()) != null) {
                                classDefinition = dictionaryService.getClass(parent);
                                PermissionGroup attempt =
    ...
    public void addPermissionModel(String model) {
        ...
        try {
            ...
            for (Iterator<Element> nsit = root.elementIterator(NAMESPACES); nsit.hasNext(); /**/ ) {
                ...
                for (Iterator<Element> it = namespacesElement.elementIterator(NAMESPACE);
                        it.hasNext(); /**/ ) {
            ...
            for (Iterator<Element> psit = root.elementIterator(PERMISSION_SET);
                    psit.hasNext(); /**/ ) {
            ...
            for (Iterator<Element> npit = root.elementIterator(GLOBAL_PERMISSION);
                    npit.hasNext(); /**/ ) {
    ...
    private Document createDocument(String model) {
        ...
        try {
            ...
            throw new PermissionModelException(
                    "Failed to create permission model document: " + model, e);
        } catch (IOException e) {
            throw new PermissionModelException(
                    "Failed to close permission model document: " + model, e);
        }
        ...
        //                    {
        //                        throw new PermissionModelException("Failed to read DTD/schema: " +
        // dtdSchema, e);
        //                    }
        //                    catch (ParserConfigurationException e)
        //                    {
        //                        throw new PermissionModelException("Failed to configure
        // DTD/schema: " + dtdSchema, e);
        //                    }
        //                }
        //                else
        //                {
        //                    throw new PermissionModelException("Couldn't obtain DTD/schema to
        ...
        //        catch (DocumentException e)
        //        {
        //            throw new PermissionModelException("Failed to create permission model
        // document: " + model, e);
        //        }
        //        finally
        //        {
        ...
        //                catch (IOException e)
        //                {
        //                    throw new PermissionModelException("Failed to close permission model
        // document: " + model, e);
        //                }
        //            }
        //            if (dtdSchemaIs != null)
        ...
        //                catch (IOException e)
        //                {
        //                    throw new PermissionModelException("Couldn't close DTD/schema to
        // validate permission model.");
        //                }
        //            }
        //        }
    ...
    public Set<PermissionReference> getAllPermissions(NodeRef nodeRef) {
        return getAllPermissionsImpl(
                nodeService.getType(nodeRef), nodeService.getAspects(nodeRef), false);
    }

    public Set<PermissionReference> getExposedPermissions(NodeRef nodeRef) {
        return getAllPermissionsImpl(
                nodeService.getType(nodeRef), nodeService.getAspects(nodeRef), true);
    }
    ...
    private Set<PermissionReference> getAllPermissionsImpl(
            QName typeName, Set<QName> aspects, boolean exposedOnly) {
    ...
    public Set<PermissionReference> getGrantingPermissions(
        ...
        Set<PermissionReference> granters =
                mutableState.grantingPermissions.get(permissionReference);
        if (granters == null) {
            mutableState.lock.readLock().unlock();
            mutableState.lock.writeLock().lock();
            ...
            try {
                ...
                if (granters == null) {
                    Set<PermissionReference> internal =
                            mutableState.getGrantingPermissionsImpl(permissionReference);
    ...
    public static class RequiredKey {
        ...
        private static Map<
                        PermissionReference,
                        Map<QName, Map<Set<QName>, EnumMap<RequiredPermission.On, RequiredKey>>>>
                instances =
                        new HashMap<
                                PermissionReference,
                                Map<
                                        ...
                                        Map<
                                                Set<QName>,
                                                EnumMap<RequiredPermission.On, RequiredKey>>>>();
            ...
            try {
                ...
                if (byPermRef == null) {
                    byPermRef =
                            new HashMap<
                                    QName,
                                    Map<Set<QName>, EnumMap<RequiredPermission.On, RequiredKey>>>();
    ...
    public Set<PermissionReference> getRequiredPermissions(
            PermissionReference required, QName qName, Set<QName> aspectQNames, On on) {
        mutableState.lock.readLock().lock();
        try {
    ...
    public Set<PermissionReference> getUnconditionalRequiredPermissions(
            PermissionReference required, On on) {
        mutableState.lock.readLock().lock();
        try {

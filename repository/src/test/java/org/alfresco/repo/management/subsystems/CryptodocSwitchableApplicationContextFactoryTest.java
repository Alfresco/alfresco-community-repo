...
public class CryptodocSwitchableApplicationContextFactoryTest {
  ...
  private static final String UNENCRYPTED_STORE_SUBSYSTEM =
    "unencryptedContentStore";
  private static final String NEW_UNENCRYPTED_STORE_SUBSYSTEM =
    "newUnencryptedContentStore";
  private static final String ENCRYPTED_STORE_SUBSYSTEM =
  ...
  public void setUp() throws Exception {
    ...
    when(parentContext.containsBean(NEW_UNENCRYPTED_STORE_SUBSYSTEM))
      .thenReturn(true);
    when(
      parentContext.getBean(
        NEW_UNENCRYPTED_STORE_SUBSYSTEM,
        ChildApplicationContextFactory.class
      )
    )
      .thenReturn(newUnencrytedContentStore);

    when(parentContext.containsBean(ENCRYPTED_STORE_SUBSYSTEM))
      .thenReturn(true);
    when(parentContext.getBean(ENCRYPTED_STORE_SUBSYSTEM))
      .thenReturn(cryptodocContentStore);
    when(
      parentContext.getBean(
        ENCRYPTED_STORE_SUBSYSTEM,
        ChildApplicationContextFactory.class
      )
    )
      .thenReturn(cryptodocContentStore);
    ...
    when(parentContext.getBean(NEW_STORE_SUBSYSTEM))
      .thenReturn(newContentStore);
    when(
      parentContext.getBean(
        NEW_STORE_SUBSYSTEM,
        ChildApplicationContextFactory.class
  ...
  public void canSwitchFromUnencryptedToUnencrypted() {
    ...
    switchableContext.setProperty(
      SOURCE_BEAN_NAME_PROPERTY,
      NEW_UNENCRYPTED_STORE_SUBSYSTEM
    );
    assertEquals(
      NEW_UNENCRYPTED_STORE_SUBSYSTEM,
  ...
  public void canSwitchFromUnencryptedToEncrypted_Supported() {
    ...
    DescriptorServiceAvailableEvent event = new DescriptorServiceAvailableEvent(
      descriptorService
    );
    when(descriptorService.getLicenseDescriptor())
      .thenReturn(licenseDescriptor);
  ...
  public void cannotSwitchFromUnencryptedToEncrypted_NotSupported() {
    ...
    try {
      ...
      fail(
        "It shouldn't be possible to switch to an encrypted content store when the license doesn't support it."
      );
  ...
  public void cannotSwitchFromEncryptedToUnencrypted() {
    ...
    try {
      ...
      fail(
        "It shouldn't be possible to switch to an unencrypted content store from an encrypted one."
      );
    } catch (IllegalStateException e) {
      // expected
    }

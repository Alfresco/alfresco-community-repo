...
public class RecordsManagementContainerTypeUnitTest extends BaseUnitTest {
    ...
    public void before() throws Exception {
        ...
        when(mockedDictionaryService.isSubClass(ContentModel.TYPE_FOLDER, ContentModel.TYPE_FOLDER))
                .thenReturn(true);
        when(mockedDictionaryService.isSubClass(
                        ContentModel.TYPE_FOLDER, ContentModel.TYPE_SYSTEM_FOLDER))
                .thenReturn(false);

        when(mockedDictionaryService.isSubClass(
                        TestModel.NOT_RM_FOLDER_TYPE, ContentModel.TYPE_FOLDER))
                .thenReturn(true);
        when(mockedDictionaryService.isSubClass(
                        TestModel.NOT_RM_FOLDER_TYPE, ContentModel.TYPE_SYSTEM_FOLDER))
                .thenReturn(false);

        when(mockedDictionaryService.isSubClass(TYPE_RECORD_CATEGORY, ContentModel.TYPE_FOLDER))
                .thenReturn(true);
        when(mockedDictionaryService.isSubClass(
                        TYPE_RECORD_CATEGORY, ContentModel.TYPE_SYSTEM_FOLDER))
                .thenReturn(false);
    }

    /**
     * Having the Unfilled Record container and a non RM folder subtype node When adding a child
     * association between the folder and the container Then the new folder should not be altered
     *
     * <p>Outlook creates a hidden folder subtype to store attachments and we don't want to change
     * the type of those folders
     */
    @Test
    ...
    /**
     * Having the fileplan and a non RM folder node When adding a child association between the
     * fileplan and the folder Then the new folder should be converted to a record category
     */
    @Test
    public void testAddFolderInFilePlan() {
        ...
        ChildAssociationRef childAssoc =
                new ChildAssociationRef(
                        ContentModel.ASSOC_CONTAINS, fileplan, ContentModel.ASSOC_CONTAINS, folder);
        recordManagementContainerType.onCreateChildAssociation(childAssoc, true);
    ...
    /**
     * Having a record category and a non RM folder node When adding a child association between the
     * record category and the folder Then the new folder should be converted to a record folder
     */
    @Test
    public void testAddFolderInRecordCategory() {
        ...
        ChildAssociationRef childAssoc =
                new ChildAssociationRef(
                        ContentModel.ASSOC_CONTAINS, category, ContentModel.ASSOC_CONTAINS, folder);
        recordManagementContainerType.onCreateChildAssociation(childAssoc, true);
    ...
    /**
     * Having an unfiled record container and a non RM folder node When adding a child association
     * between the container and the folder Then the new folder should be converted to a unfiled
     * record folder
     */
    @Test
    public void testAddFolderInUnfiledRecordContainer() {
        NodeRef unfiledRecordContainer = generateNodeRef();
        when(mockedNodeService.getType(unfiledRecordContainer))
    ...
    /**
     * Having an unfiled record folder and a non RM folder node When adding a child association
     * between the unfiled record folder and the regular folder Then the new folder should be
     * converted to a unfiled record folder
     */
    @Test
    ...
    private NodeRef generateRMContainer() {
        ...
        when(mockedNodeService.getType(rmContainer))
                .thenReturn(RecordsManagementModel.TYPE_UNFILED_RECORD_CONTAINER);
        return rmContainer;
    }
    ...
    private NodeRef generateNonRmFolderSubtypeNode() {
        ...
        when(mockedNodeService.hasAspect(nonRmFolder, ASPECT_FILE_PLAN_COMPONENT))
                .thenReturn(false);
        return nonRmFolder;
    }
    ...
    private NodeRef generateNonRmFolderNode() {
        ...
        when(mockedNodeService.hasAspect(regularFolder, ASPECT_FILE_PLAN_COMPONENT))
                .thenReturn(false);

...
public class ContentUsageImpl
    ...
    private void recordDelete(NodeRef nodeRef) {
        Set<NodeRef> deletedNodes =
                (Set<NodeRef>) AlfrescoTransactionSupport.getResource(KEY_DELETED_NODES);
        ...
        Set<NodeRef> updatedNodes =
                (Set<NodeRef>) AlfrescoTransactionSupport.getResource(KEY_CREATED_NODES);
    ...
    public void onUpdateProperties(
        ...
        if (stores.contains(tenantService.getBaseName(nodeRef.getStoreRef()).toString())
            ...
            Long contentSizeBefore =
                    (contentDataBefore == null ? null : contentDataBefore.getSize());
            ...
            if (contentSizeBefore == null
                    && contentSizeAfter != null
                    && contentSizeAfter != 0
                    && ownerAfter != null) {
                // new size has been added - note: ownerBefore does not matter since the
                // contentSizeBefore is null
                if (logger.isDebugEnabled())
                    logger.debug(
                            "onUpdateProperties: updateSize (null -> "
                                    + contentSizeAfter
                                    + "): nodeRef="
                                    + nodeRef
                                    + ", ownerAfter="
            ...
            } else if (contentSizeAfter == null
                    && contentSizeBefore != null
                    && contentSizeBefore != 0
                    && ownerBefore != null) {
                // old size has been removed - note: ownerAfter does not matter since
                // contentSizeAfter is null
                if (logger.isDebugEnabled())
                    logger.debug(
                            "onUpdateProperties: updateSize ("
                                    + contentSizeBefore
                                    + " -> null): nodeRef="
                                    + nodeRef
                                    + ", ownerBefore="
                ...
                if (contentSizeBefore.equals(contentSizeAfter) == false) {
                    ...
                    if (ownerBefore == null
                        ...
                        if (logger.isDebugEnabled())
                            logger.debug(
                                    "onUpdateProperties: updateOwner (null -> "
                                            + ownerAfter
                                            + "): nodeRef="
                                            + nodeRef
                                            + ", contentSize="
                                            + contentSizeAfter);
                        incrementUserUsage(ownerAfter, contentSizeAfter, nodeRef);
                    } else if (ownerAfter == null
                            && ownerBefore != null
                            && contentSizeBefore != 0) {
                        // old owner has been removed
                        if (logger.isDebugEnabled())
                            logger.debug(
                                    "onUpdateProperties: updateOwner ("
                                            + ownerBefore
                                            + " -> null): nodeRef="
                                            + nodeRef
                                            + ", contentSize="
                ...
                String owner =
                        (String)
                                nodeService.getProperty(
                                        nodeRef,
                                        ContentModel
                                                .PROP_ARCHIVED_ORIGINAL_OWNER); // allow for case
                // where someone
                // else is deleting
                // the node
                if (owner == null || owner.equals(OwnableService.NO_OWNER)) {
    ...
    */
    ...
    private void incrementUserUsage(String userName, long contentSize, NodeRef contentNodeRef) {
        if (!authenticationContext.isSystemUserName(userName)
                && !excludedTypes.contains(nodeService.getType(contentNodeRef))) {
    ...
    public void setUserStoredUsage(NodeRef personNodeRef, long currentUsage) {
        if (personNodeRef != null) {
            nodeService.setProperty(
                    personNodeRef, ContentModel.PROP_SIZE_CURRENT, new Long(currentUsage));
        }
    }
    ...
    public long getUserUsage(NodeRef personNodeRef, boolean removeDeltas) {
        ...
        if (currentUsage != -1) {
            long deltaSize =
                    removeDeltas
                            ? usageService.getAndRemoveTotalDeltaSize(personNodeRef)
    ...
    /**
     * Set user's current quota. Usually called by Web Client (Admin Console) if admin is
     * changing/setting a user's quota.
     *
    ...
    public void setUserQuota(String userName, long currentQuota) {
        ...
        if (personNodeRef != null) {
            nodeService.setProperty(
                    personNodeRef, ContentModel.PROP_SIZE_QUOTA, new Long(currentQuota));
        }
    }
    ...
    public long getUserQuota(String userName) {
        ...
        if (personNodeRef != null) {
            currentQuota =
                    (Long) nodeService.getProperty(personNodeRef, ContentModel.PROP_SIZE_QUOTA);
        }
    ...
    private NodeRef getPerson(String userName) {
        ...
        try {
            ...
            // The boolean parameter to getPerson does two things. It should really be split into
            // two booleans.
            personNodeRef = null;
        } catch (RuntimeException e) {
            // workaround for ETHREEOH-1457 where existing tenants (created using 3.0.1) may have
            // been bootstrapped with creator set
            // to super admin (eg. "admin") rather than "System@xxx". This workaround should remain
            // until we patch any such existing tenants

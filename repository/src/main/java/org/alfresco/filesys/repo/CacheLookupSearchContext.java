...
public class CacheLookupSearchContext extends DotDotContentSearchContext {
  ...
  private static final Log logger = LogFactory.getLog(
    CacheLookupSearchContext.class
  );
  ...
  public boolean nextFileInfo(FileInfo info) {
    ...
    if (fstate != null) {
      ...
      if (fstate.hasAccessDateTime()) info.setAccessDateTime(
        fstate.getAccessDateTime()
      );

      if (fstate.hasModifyDateTime()) info.setModifyDateTime(
        fstate.getModifyDateTime()
      );
      ...
      if (
        fstate.hasAllocationSize() &&
        fstate.getAllocationSize() > info.getSize()
      ) info.setAllocationSize(fstate.getAllocationSize());

      // DEBUG

      if (logger.isDebugEnabled()) logger.debug(
        "Search timestamps from cache, path=" + info.getFileName()
      );
    }

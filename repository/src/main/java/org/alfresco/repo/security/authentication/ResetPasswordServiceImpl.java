...
public class ResetPasswordServiceImpl implements ResetPasswordService {
    ...
    public void initiateResetPassword(ResetPasswordDetails resetDetails) {
        ...
        // So now we know that the workflow instance exists, is active and has the correct key. We
        // can proceed.
        ...
        // Set the provided password into the task. We will remove this after we have updated the
        // user's authentication details.
        Map<QName, Serializable> props =
                Collections.singletonMap(
                        WorkflowModelResetPassword.WF_PROP_PASSWORD, resetDetails.getPassword());
        ...
        // Remove the previous task from Activiti's history - so that the password will not be in
        // the database.
        // See http://www.activiti.org/userguide/index.html#history for a description of how
        // Activiti stores historical records of
    ...
    private void validateIdAndKey(String id, String key, String userId) {
        ...
        if (workflowInstance == null) {
            throw new ResetPasswordWorkflowNotFoundException(
                    "The reset password workflow instance with the id [" + id + "] is not found.");
        }
        ...
        if (username == null || recoveredKey == null || !recoveredKey.equals(key)) {
            ...
            if (username == null) {
                msg =
                        "The recovered user name is null for the reset password workflow instance"
                                + " with the id ["
                                + id
                                + "]";
            } else if (recoveredKey == null) {
                msg =
                        "The recovered key is null for the reset password workflow instance with"
                                + " the id ["
                                + id
                                + "]";
            } else {
    ...
    public void sendResetPasswordEmail(
            DelegateExecution execution, String fallbackEmailTemplatePath, String emailSubject) {
        Map<String, Object> variables = execution.getVariables();
        final String userName =
                (String) variables.get(WorkflowModelResetPassword.WF_PROP_USERNAME_ACTIVITI);
        final String toEmail =
                (String) variables.get(WorkflowModelResetPassword.WF_PROP_USER_EMAIL_ACTIVITI);
        final String clientName =
                (String) variables.get(WorkflowModelResetPassword.WF_PROP_CLIENT_NAME_ACTIVITI);
        ...
        Map<String, Serializable> emailTemplateModel =
                Collections.singletonMap(
                        FTL_RESET_PASSWORD_URL, createResetPasswordUrl(clientApp, id, key));

        final String templatePath =
                emailHelper.getEmailTemplate(
                        clientName,
                        getResetPasswordEmailTemplate(clientApp),
                        fallbackEmailTemplatePath);

        ResetPasswordEmailDetails emailRequest =
    ...
    public void performResetPassword(DelegateExecution execution) {
        ...
        final String userName =
                (String)
                        execution.getVariable(WorkflowModelResetPassword.WF_PROP_USERNAME_ACTIVITI);

        // But we cannot get the password from the execution as we have intentionally not stored the
        // password there.
        ...
        final String password =
                (String)
                        activitiTaskService.getVariable(
                                activitiTaskId,
        ...
        ParameterCheck.mandatoryString(
                WorkflowModelResetPassword.WF_PROP_USERNAME_ACTIVITI, userName);
        ParameterCheck.mandatoryString(
                WorkflowModelResetPassword.WF_PROP_PASSWORD_ACTIVITI, password);
    ...
    public void sendResetPasswordConfirmationEmail(
        ...
        final ClientApp clientApp = getClientAppConfig(clientName);
        Map<String, Serializable> emailTemplateModel =
                Collections.singletonMap(FTL_USER_NAME, userName);
    ...
    protected void sendEmail(ResetPasswordEmailDetails emailRequest) {
        ...
        final String templateAssetsUrl =
                getUrl(
        ...
        actionParams.put(
                MailActionExecuter.PARAM_IGNORE_SEND_FAILURE, emailRequest.ignoreSendFailure);
        // Now send the email
    ...
    protected String createResetPasswordUrl(
            ClientApp clientApp, final String id, final String key) {
    ...
    public static class ResetPasswordDetails {
        ...
        public String toString() {
            ...
            sb.append("ResetPasswordDetails [userId=")
                    .append(userId)
                    .append(", workflowId=")
                    .append(workflowId)
                    .append(", workflowKey=")
    ...
    public static class ResetPasswordEmailDetails {
        ...
        public String toString() {
            ...
            sb.append("ResetPasswordEmailDetails [userName=")
                    .append(userName)
                    .append(", userEmail=")
                    .append(userEmail)
                    .append(", fromEmail=")

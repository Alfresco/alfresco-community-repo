...
public class IntegrityChecker
  ...
  private static final String KEY_WARN_IN_TRANSACTION =
    "IntegrityChecker.WarnInTransaction";
  ...
  public static boolean isWarnInTransaction() {
    Boolean warnInTransaction = (Boolean) TransactionSupportUtil.getResource(
  ...
  private void save(IntegrityEvent event) {
    ...
    // get the event list
    Map<IntegrityEvent, IntegrityEvent> events = (Map<IntegrityEvent, IntegrityEvent>) AlfrescoTransactionSupport.getResource(
      KEY_EVENT_SET
    );
    if (events == null) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "" +
        (
          existingEvent != null ? "Event already present in" : "Added event to"
        ) +
        " event set: \n" +
        "   event: " +
        event
  ...
  public void onCreateNode(ChildAssociationRef childAssocRef) {
    ...
    if (
      ...
      event =
        new PropertiesIntegrityEvent(nodeService, dictionaryService, childRef);
      save(event);
      ...
      event =
        new AspectsIntegrityEvent(nodeService, dictionaryService, childRef);
      save(event);

      // check for associations defined on the new node (child)
      QName childNodeTypeQName = nodeService.getType(childRef);
      ClassDefinition nodeTypeDef = dictionaryService.getClass(
        childNodeTypeQName
      );
      if (nodeTypeDef == null) {
  ...
  public void onUpdateProperties(
    ...
    if (
      ...
      event =
        new PropertiesIntegrityEvent(nodeService, dictionaryService, nodeRef);
      save(event);
    }
  }
  ...
  public void onRemoveAspect(NodeRef nodeRef, QName aspectTypeQName) {
    if (
      ...
      // check mandatory aspects
      event =
        new AspectsIntegrityEvent(nodeService, dictionaryService, nodeRef);
      save(event);
    }
  }
  ...
  public void onCreateChildAssociation(
    ...
    if (
      !storesToIgnore.contains(
        tenantService
          .getBaseName(childAssocRef.getChildRef().getStoreRef())
          .toString()
      )
    ) {
      IntegrityEvent event = null;
  ...
  public void onCreateAssociation(AssociationRef nodeAssocRef) {
    if (
      !storesToIgnore.contains(
        tenantService
          .getBaseName(nodeAssocRef.getSourceRef().getStoreRef())
          .toString()
      )
    ) {
      IntegrityEvent event = null;
  ...
  private List<IntegrityRecord> processAllEvents() {
    // the results
    ArrayList<IntegrityRecord> allIntegrityResults = new ArrayList<IntegrityRecord>(
      0
    ); // generally empty
    ...
    for (IntegrityEvent event : events.keySet()) {
      try {
        ...
        Throwable retryThrowable = RetryingTransactionHelper.extractRetryCause(
          e
        );
        ...
        IntegrityRecord exceptionRecord = new IntegrityRecord(
          "" + e.getMessage()
        );

...
public class ScheduledPersistedActionImpl implements ScheduledPersistedAction {
    ...
    /**
     * Get the first date that the action should be run on or after, or null if it should start
     * shortly after each startup.
     */
    ...
    /**
     * Sets the first date that the action should be run on or after. Set to null if the action
    ...
    /**
     * How many {@link #getScheduleIntervalPeriod()} periods should we wait between executions? Will
     * be null if the action isn't scheduled to be repeated.
     */
    ...
    /**
     * Sets how many periods should be waited between each execution, or null if it shouldn't be
     * repeated.
     */
    ...
    /** How long are {@link #getScheduleIntervalCount()} counts measured in? */
    ...
    /**
     * Returns the interval in a form like 1Day (1 day) or 2Hour (2 hours), or null if a
     * period+count hasn't been set
     */
    ...
    /**
     * Returns a Quartz trigger definition based on the current scheduling details. May only be
     * called once this object has been persisted
     */
    public Trigger asTrigger() {
        if (persistedAtNodeRef == null) throw new IllegalStateException("Must be persisted first");
                            ...
                            TriggerBuilder.newTrigger()
                                    ...
                                    .withSchedule(
                                            CalendarIntervalScheduleBuilder
                                                    ...
                                                    .withInterval(
                                                            intervalCount,
                                                            DateBuilder.IntervalUnit.valueOf(
                                                                    intervalPeriod
                                                                            .toString()
                                                                            .toUpperCase())))
                                    .startAt(scheduleStart)

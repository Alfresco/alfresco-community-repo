...
public class RecordsManagementNotificationHelper
  ...
  private static Log logger = LogFactory.getLog(
    RecordsManagementNotificationHelper.class
  );

  /** I18n */
  private static final String MSG_SUBJECT_RECORDS_DUE_FOR_REVIEW =
    "notification.dueforreview.subject";
  private static final String MSG_SUBJECT_RECORD_SUPERCEDED =
    "notification.superseded.subject";
  private static final String MSG_SUBJECT_RECORD_REJECTED =
    "notification.rejected.subject";
  ...
  private NodeRef supersededTemplate = new NodeRef(
    StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
    "record_superseded_template"
  );
  private NodeRef dueForReviewTemplate;
  private NodeRef rejectedTemplate = new NodeRef(
    StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
  ...
  public NodeRef getDueForReviewTemplate() {
    if (dueForReviewTemplate == null) {
      List<NodeRef> nodeRefs = searchService.selectNodes(
        getRootNode(),
        "app:company_home/app:dictionary/cm:records_management/cm:records_management_email_templates/cm:notify-records-due-for-review-email.ftl",
        null,
        namespaceService,
        false
      );
   ...
   */
  public void recordsDueForReviewEmailNotification(
    ...
    if (!records.isEmpty()) {
      if (
        ...
        if (doesGroupContainUsers(groupName)) {
          ...
          notificationContext.setSubject(
            I18NUtil.getMessage(MSG_SUBJECT_RECORDS_DUE_FOR_REVIEW)
          );
          notificationContext.setAsyncNotification(false);
          notificationContext.setIgnoreNotificationFailure(true);

          notificationContext.setBodyTemplate(
            getDueForReviewTemplate().toString()
          );
  ...
  public void recordSupersededEmailNotification(final NodeRef record) {
    ...
    if (doesGroupContainUsers(groupName)) {
      NotificationContext notificationContext = new NotificationContext();
      notificationContext.setSubject(
        I18NUtil.getMessage(MSG_SUBJECT_RECORD_SUPERCEDED)
      );
      notificationContext.setAsyncNotification(false);
      notificationContext.setIgnoreNotificationFailure(true);
   ...
   */
  public void recordRejectedEmailNotification(
    NodeRef record,
    String recordId,
    ...
    if (canSendRejectEmail(record, recordCreator)) {
      ...
      String rejectReason = (String) nodeService.getProperty(
        record,
        PROP_RECORD_REJECTION_REASON
      );
      String rejectedPerson = (String) nodeService.getProperty(
        record,
        PROP_RECORD_REJECTION_USER_ID
      );
      Date rejectDate = (Date) nodeService.getProperty(
        record,
      ...
      notificationContext.setSubject(
        I18NUtil.getMessage(MSG_SUBJECT_RECORD_REJECTED)
      );
  ...
  private boolean canSendRejectEmail(NodeRef record, String recordCreator) {
    ...
    if (siteService.getSite(record) == null) {
      ...
      logger.warn(
        msg1 +
        "the site which should contain the node '" +
        record.toString() +
        "'" +
        msg2
      );
    ...
    if (
      StringUtils.isBlank(
        (String) nodeService.getProperty(record, PROP_RECORD_REJECTION_REASON)
      )
    ) {
      result = false;
    ...
    if (
      StringUtils.isBlank(
        (String) nodeService.getProperty(record, PROP_RECORD_REJECTION_USER_ID)
      )
    ) {
      result = false;
    ...
    if (
      ((Date) nodeService.getProperty(record, PROP_RECORD_REJECTION_DATE)) ==
      null
    ) {
      result = false;

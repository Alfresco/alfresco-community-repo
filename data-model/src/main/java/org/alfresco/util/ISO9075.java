...
public class ISO9075 {
  ...
  public static String encode(String toEncode) {
    if ((toEncode == null) || (toEncode.length() == 0)) {
      ...
      XMLChar.isValidName(toEncode) &&
      (toEncode.indexOf("_x") == -1) &&
      (toEncode.indexOf(':') == -1)
    ) {
      return toEncode;
    } else {
  ...
  public static String decode(String toDecode) {
    ...
    for (int i = 0, l = toDecode.length(); i < l; i++) {
      if (matchesEncodedPattern(toDecode, i)) {
        decoded.append(
          ((char) Integer.parseInt(toDecode.substring(i + 2, i + 6), 16))
        );
  ...
  public static String getXPathName(QName qName, NamespacePrefixResolver nspr) {
    ...
    if (prefixes.size() == 0) {
      throw new NamespaceException(
        "A namespace prefix is not registered for uri " +
  ...
  public static String getXPathName(QName qName) {
    return (
      "{" + qName.getNamespaceURI() + "}" + ISO9075.encode(qName.getLocalName())
    );
  }

...
public class QuickShareServiceIntegrationTest {
  ...
  private static AlfrescoPerson user1 = new AlfrescoPerson(
    testContext,
    "UserOne"
  );
  private static AlfrescoPerson user2 = new AlfrescoPerson(
    testContext,
    "UserTwo"
  );
  ...
  @ClassRule
  public static RuleChain classChain = RuleChain
    .outerRule(testContext)
    .around(user1)
    .around(user2);
  ...
  private static void findServices() {
    ...
    directQuickShareService =
      ctx.getBean("quickShareService", QuickShareService.class);
    quickShareService =
      ctx.getBean("QuickShareService", QuickShareService.class);
    ...
    nodeArchiveService =
      ctx.getBean("nodeArchiveService", NodeArchiveService.class);
  ...
  public void getMetaDataFromNodeRefByOwner() {
    Map<String, Object> metadata = AuthenticationUtil.runAs(
      ...
      new RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          assertTrue(
            nodeService
              .getAspects(testNode)
              .contains(QuickShareModel.ASPECT_QSHARE)
          );
          assertNotNull(
  ...
  public void testDeleteAndRestoreSharedNode() {
    ...
    AuthenticationUtil.runAsSystem(
      new RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          assertTrue(
            nodeService.hasAspect(testNode, QuickShareModel.ASPECT_QSHARE)
          );
          return null;
        }
      }
      ...
      AuthenticationUtil.runAs(
        new RunAsWork<NodeRef>() {
          ...
          public NodeRef doWork() throws Exception {
            ...
            final NodeRef archivedNode = nodeArchiveService.getArchivedNode(
              testNode
            );
            assertNotNull(archivedNode);

            // Restore the deleted shared node from trashcan
            RestoreNodeReport restoreNodeReport = nodeArchiveService.restoreArchivedNode(
              archivedNode
            );
  ...
  private QuickShareDTO share(
    final NodeRef nodeRef,
    final String username,
    final Date expiryDate
  ) {
    return AuthenticationUtil.runAs(
      () -> quickShareService.shareContent(nodeRef, expiryDate),
      username
    );
  }
    ...
    AuthenticationUtil.runAs(
      new RunAsWork<Object>() {
        ...
        public Object doWork() throws Exception {
          ...
          Map<QName, Serializable> originalProps = nodeService.getProperties(
            testNode
          );
          ...
          Map<QName, Serializable> copyProps = nodeService.getProperties(
            copyNodeRef
          );
          ...
          for (QName property : originalProps.keySet()) {
            if (
              property.equals(QuickShareModel.PROP_QSHARE_SHAREDBY) ||
              property.equals(QuickShareModel.PROP_QSHARE_SHAREDID)
            ) {
              continue;
            }
            Assert.assertTrue(
              "Mising property " + property,
              copyProps.containsKey(property)
            );
          }
          return null;
        }
  ...
  public void testWithCustomContentType() {
    ByteArrayInputStream modelStream = new ByteArrayInputStream(
      MODEL.getBytes()
    );
    ...
    final NodeRef sharableNode = testNodes.createNodeWithTextContent(
      userHome,
    ...
    final NodeRef unsharableNode = testNodes.createNodeWithTextContent(
      userHome,
  ...
  private Map<String, Object> getMetadata(
    ...
    Map<String, Object> container = AuthenticationUtil.runAs(
      new RunAsWork<Map<String, Object>>() {
        @Override
        public Map<String, Object> doWork() throws Exception {
  ...
  public void cloud928() {
    final NodeRef node = testNodes.createNodeWithTextContent(
      userHome,
      "CLOUD-928 Test Node",
      ContentModel.TYPE_CONTENT,
      user1.getUsername(),
    ...
    attributeService.removeAttribute(
      QuickShareServiceImpl.ATTR_KEY_SHAREDIDS_ROOT,
      dto.getId()
    );
    ...
    assertNotNull(
      "Node " +
      node.toString() +
      " hasn't been archived hence the deletion was unsuccessful",
      archivedNode
    );
  }
  ...
  public void testModifierAfterSharing() {
    ...
    final Serializable modifiedDate = AuthenticationUtil.runAsSystem(
  ...
  public void testModifierAfterUnSharing() {
    ...
    String modifier = AuthenticationUtil.runAsSystem(
  ...
  public void testSharedLinkExpiryScheduling() throws Exception {
    ...
    QuickShareDTO quickShareDTO = share(
      testNode,
      user1.getUsername(),
      expiryDate
    );
    ...
    assertEquals(
      quickShareDTO.getId(),
      getProperty(testNode, QuickShareModel.PROP_QSHARE_SHAREDID)
    );
    ...
    QuickShareLinkExpiryAction expiryAction = getExpiryActionAndAttachSchedule(
      quickShareDTO.getId()
    );
    ...
    assertNull(
      "We haven't set interval count.",
      expiryAction.getScheduleIntervalCount()
    );
    assertNull(
      "We haven't set interval period.",
      expiryAction.getScheduleIntervalPeriod()
    );
    ...
    assertEquals(
      expiryDate,
      getProperty(testNode, QuickShareModel.PROP_QSHARE_EXPIRY_DATE)
    );
    assertTrue(hasQuickShareAspect(testNode));
    assertEquals(
      quickShareDTO.getId(),
    ...
    QuickShareLinkExpiryAction deletedExpiryAction = getExpiryAction(
      quickShareDTO.getId()
    );
  ...
  public void testSharedLinkExpiryDateValidator() throws Exception {
    ...
    try {
      ...
      QuickShareLinkExpiryActionException.InvalidExpiryDateException ex
    ) {
      // Expected
    }

    final String defaultExpiryDatePeriod = globalProperties.getProperty(
      "system.quickshare.expiry_date.enforce.minimum.period"
    );
    ...
    try {
       ...
       */
      {
        ((QuickShareServiceImpl) directQuickShareService).setExpiryDatePeriod(
            "DAYS"
          );
        ...
        try {
          ...
          fail(
            "Should have failed as the expiry date is invalid (less than 1 day)."
          );
        } catch (
          QuickShareLinkExpiryActionException.InvalidExpiryDateException ex
        ) {
       ...
       */
      {
        ...
        try {
          ...
          fail(
            "Should have failed as the expiry date is invalid (less than 1 hour)."
          );
        } catch (
          QuickShareLinkExpiryActionException.InvalidExpiryDateException ex
        ) {
       ...
       */
      {
        ...
        try {
          ...
          fail(
            "Should have failed as the expiry date is invalid (less than 1 minute)."
          );
        } catch (
          QuickShareLinkExpiryActionException.InvalidExpiryDateException ex
        ) {
  ...
  public void testCanDeleteSharedLinkWithAdminUserForPrivateNodes()
    ...
    try {
      ...
      NodeRef nodeRefOnPrivateSite = testNodes.createNode(
        siteService.getSite(siteName).getNodeRef(),
        nodeName,
        ContentModel.TYPE_CONTENT,
        user1.getUsername()
      );
      NodeRef nodeRefOnUserHome = testNodes.createNode(
        userHome,
        nodeName + "userHome",
        ContentModel.TYPE_CONTENT,
        user1.getUsername()
      ...
      boolean canDeleteSharedLink = userCanDeleteSharedLink(
        nodeRefOnPrivateSite,
        user1.getUsername()
      );
      assertEquals(true, canDeleteSharedLink);
      canDeleteSharedLink =
        userCanDeleteSharedLink(nodeRefOnUserHome, user1.getUsername());
      assertEquals(true, canDeleteSharedLink);
  ...
  private QuickShareLinkExpiryAction getExpiryActionAndAttachSchedule(
    ...
    assertNotNull(
      "Expiry action should have been persisted.",
      expiryAction.getNodeRef()
    );
    assertNull(
      "The schedule hasn't been attached yet.",
      expiryAction.getSchedule()
    );
    ScheduledPersistedAction scheduledPersistedAction = getSchedule(
      expiryAction
    );
    assertNotNull(
      "Scheduled action should have been persisted.",
  ...
  private ScheduledPersistedAction getSchedule(
    final QuickShareLinkExpiryAction linkExpiryAction
  ) {
    return AuthenticationUtil.runAsSystem(() ->
  ...
  private void deleteExpiryAction(
    final QuickShareLinkExpiryAction linkExpiryAction
  ) {
  ...
  private boolean hasQuickShareAspect(NodeRef nodeRef) {
    return AuthenticationUtil.runAsSystem(() ->
      nodeService.hasAspect(nodeRef, QuickShareModel.ASPECT_QSHARE)
    );
  }

  private Serializable getProperty(NodeRef nodeRef, QName property) {
    return AuthenticationUtil.runAsSystem(() ->
      nodeService.getProperty(nodeRef, property)
    );
  }

  private void forceSaveNewExpiryTime(
    final QuickShareLinkExpiryAction linkExpiryAction
  ) {
    transactionHelper.doInTransaction(() -> {
      AuthenticationUtil.runAsSystem(() -> {
  ...
  private boolean userCanDeleteSharedLink(
    NodeRef nodeRef,

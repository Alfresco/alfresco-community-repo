...
/**
 * A filesystem walker that returns all files and directories in subsequent levels of a filesystem
 * tree; it returns all directories and files in a given level, at which point it moves on to the
 * next level and starts returning directories and files in that level.
 *
 * @since 4.0
 */
public class StripingFilesystemTracker extends AbstractFilesystemTracker {
    ...
    private Map<Integer, List<ImportableItem>> directoriesToProcess =
            new HashMap<Integer, List<ImportableItem>>(10);
    private List<ImportableItem> toProcess = new ArrayList<ImportableItem>();

    public StripingFilesystemTracker(
            DirectoryAnalyser directoryAnalyser, NodeRef target, File sourceFolder, int batchSize) {
    ...
    protected List<ImportableItem> getImportableItems(int count) {
        while (toProcess.size() < count) {
            ...
            if (directory != null) {
                ...
                getDirectoriesToProcess(currentLevel + 1)
                        .addAll(analysedDirectory.getImportableDirectories());
            } else {
                break;
            }
        }
        ...
        int i = size;
        while (i > 0) {
            // we can assume that there are items in toProcess to remove because the size has been
    ...
    public BatchProcessWorkProvider<ImportableItem> getWorkProvider() {
        BatchProcessWorkProvider<ImportableItem> provider =
                new BatchProcessWorkProvider<ImportableItem>() {
                    @Override

...
public abstract class AbstractAssociationsGet extends DictionaryWebServiceBase {
    ...
    protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
        ...
        if ((name == null && namespacePrefix != null)
            ...
            throw new WebScriptException(
                    Status.STATUS_NOT_FOUND,
                    "Missing either name or namespaceprefix parameter in the URL - both combination"
                            + " of name and namespaceprefix is needed");
        }
        ...
        if (associationFilter.equals("child")) {
            model.put(
                    ...
                    this.dictionaryservice.getClass(classQname).getChildAssociations().values());
        } else if (associationFilter.equals("general")) {
            for (AssociationDefinition assocname :
        ...
        // if both namespaceprefix and name parameters are given then, the combination
        // namespaceprefix_name is used as the index to create the qname
        if (name != null && namespacePrefix != null) {
            ...
            if (this.dictionaryservice.getClass(classQname).getAssociations().get(associationQname)
                    == null) {
                throw new WebScriptException(
                        Status.STATUS_NOT_FOUND, "not a Valid - namespaceprefix_name combination");
            }

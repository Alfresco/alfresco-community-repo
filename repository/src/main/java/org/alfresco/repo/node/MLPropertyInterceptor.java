...
public class MLPropertyInterceptor implements MethodInterceptor {
  ...
  public Object invoke(final MethodInvocation invocation) throws Throwable {
    ...
    if (methodName.equals("getProperty")) {
      ...
      for (Map.Entry<QName, Serializable> entry : properties.entrySet()) {
        ...
        Serializable convertedValue = convertOutboundProperty(
          nodeRef,
          pivotNodeRef,
          propertyQName,
          value
      ...
      Map<QName, Serializable> currentProperties = nodeService.getProperties(
        nodeRef
      );
  ...
  private Serializable convertOutboundProperty(
    ...
    if (outboundValue instanceof MLText) {
      ...
      if (propertyQName.equals(ContentModel.PROP_MODIFIED)) {
        ...
        Date pivotLastModified = (Date) nodeService.getProperty(
          pivotNodeRef,
  ...
  private Serializable getClosestValue(MLText mlText) {
    ...
    if (contentLocaleLang != null) {
      ...
      logger.warn(
        "contentLocaleLang is null in getClosestValue. This is not expected."
      );
    }
  ...
  private Serializable convertInboundProperty(
    ...
    PropertyDefinition propertyDef =
      this.dictionaryService.getProperty(propertyQName);
    //if no type definition associated to the name then just proceed
    if (propertyDef == null) {
      ret = inboundValue;
    } else if (
      propertyDef.getDataType().getName().equals(DataTypeDefinition.MLTEXT)
    ) {
      ...
      if (inboundValue instanceof MLText) {
        ...
        if (isCollectionOfMLText(inboundValue)) {
          ...
          if (currentValue != null) {
            Collection<MLText> currentCollection = DefaultTypeConverter.INSTANCE.getCollection(
              MLText.class,
              currentValue
            );
            returnMLList.addAll(currentCollection);
          }
          Collection<String> inboundCollection = DefaultTypeConverter.INSTANCE.getCollection(
            String.class,
            inboundValue
          );
          int count = 0;
        ...
        if (currentValue != null) {
          MLText currentMLValue = DefaultTypeConverter.INSTANCE.convert(
            MLText.class,
            currentValue
          );
        ...
        String inboundValueStr = DefaultTypeConverter.INSTANCE.convert(
          String.class,
          inboundValue
        );
      ...
      pivotNodeRef != null && propertyQName.equals(ContentModel.PROP_CONTENT)
    ) {
      // It is an empty translation.  The content must not change if it matches
      // the content of the pivot translation
      ContentData pivotContentData = (ContentData) nodeService.getProperty(
        pivotNodeRef,
        ContentModel.PROP_CONTENT
      );
      ContentData emptyContentData = (ContentData) inboundValue;
      String pivotContentUrl = pivotContentData == null
        ? null
        : pivotContentData.getContentUrl();
      String emptyContentUrl = emptyContentData == null
        ? null
        : emptyContentData.getContentUrl();
   ...
   * @param mlText MLText
   */
  private void replaceTextForLanguage(
    Locale contentLocale,
    String updatedText,
    MLText mlText
  ) {

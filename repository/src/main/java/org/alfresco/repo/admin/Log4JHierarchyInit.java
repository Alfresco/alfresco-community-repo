 ...
 * log4.properties files. The actual implementation uses introspection to avoid any hard-coded
 * references to Log4J classes. If Log4J is not present, this class will do nothing.
 *
 ...
 * Module</a>
 *
 * <p>For example, if {module.id} is "org.alfresco.module.someModule", then within your source code
 * you'll have:
 *
 * <pre>
...
public class Log4JHierarchyInit implements ApplicationContextAware {
     ...
     * array of Srping URLS.
     *
     * <p>This function supports Spring's syntax for retrieving multiple class path resources with
     * the same name, via the "classpath&#042;:" prefix. For details, see: {@link
    ...
    private void importLogSettings() {
        try {
            ...
            throw new RuntimeException(
                    "Unable to find method 'configure' on class"
                            + " 'org.apache.log4j.PropertyConfigurator'");
        } catch (Throwable t) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Failed to add extra Logger configuration: \n"
                                + "   Error: "
                                + t.getMessage(),
                        t);
            }

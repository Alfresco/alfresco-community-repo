...
public class PostLookup {
  ...
  private static Map<String, String> rollupTypes = new HashMap<String, String>(
    3
  );
  ...
  public void execute() throws JobExecutionException {
    ...
    try {
      ...
      final List<ActivityPostEntity> activityPosts = postDAO.selectPosts(
        params,
        maxItemsPerCycle
      );
      ...
      if (activityPosts.size() > 0) {
        ...
        List<ActivityPostEntity> activityPostsToUpdate = transactionService
          .getRetryingTransactionHelper()
          .doInTransaction(
            new RetryingTransactionCallback<List<ActivityPostEntity>>() {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "execute: Can't get lock. Assume post lookup job already underway: " +
          e
        );
      }
  ...
  private class UserRollupActivity {
    ...
    public int hashCode() {
      ...
      result =
        prime * result + ((activityType == null) ? 0 : activityType.hashCode());
      result =
        prime *
        result +
        ((parentNodeRef == null) ? 0 : parentNodeRef.hashCode());
      return result;
    }
  ...
  private List<ActivityPostEntity> lookupPosts(
    ...
    for (final ActivityPostEntity activityPost : activityPosts) {
      ...
      try {
        ...
        final JSONObject jo = new JSONObject(
          new JSONTokener(activityPost.getActivityData())
        );
        ...
        TenantUtil.runAsSystemTenant(
          new TenantUtil.TenantRunAsWork<Void>() {
            public Void doWork() throws Exception {
                ...
                if (parentNodeRef == null) {
                  ...
                  if (jo.isNull(JSON_PARENT_NODEREF)) {
                    if (!jo.isNull(JSON_NODEREF)) {
                      parentNodeRef =
                        lookupParentNodeRef(
                          new NodeRef(jo.getString(JSON_NODEREF))
                        );
                      if (parentNodeRef != null) {
              ...
              if (
                (activityPost.getActivityData() != null) &&
                (
              ...
              if (
                (activityPost.getSiteNetwork() != null) &&
                (
        ...
        logger.error(
          "Skipping activity post " + activityPost.getId() + ": " + e
        );
  ...
  private List<ActivityPostEntity> rollupPosts(
    List<ActivityPostEntity> activityPosts
  ) throws SQLException {
    Map<UserRollupActivity, List<ActivityPostEntity>> rollupPosts = new HashMap<UserRollupActivity, List<ActivityPostEntity>>();

    List<ActivityPostEntity> result = new ArrayList<ActivityPostEntity>(
      activityPosts.size()
    );

    for (final ActivityPostEntity post : activityPosts) {
      if (
        rollupTypes.containsKey(post.getActivityType()) &&
        (post.getParentNodeRef() != null)
      ) {
        UserRollupActivity key = new UserRollupActivity(
          post.getUserId(),
        ...
        TenantUtil.runAsSystemTenant(
          new TenantUtil.TenantRunAsWork<Void>() {
            public Void doWork() throws Exception {
              ...
              newPost.setActivityType(
                rollupTypes.get(oldPost.getActivityType())
              );
  ...
  private void updatePosts(List<ActivityPostEntity> activityPosts)
    throws SQLException {
    for (final ActivityPostEntity activityPost : activityPosts) {
      // MT share
        ...
        new TenantUtil.TenantRunAsWork<Object>() {
          public Object doWork() throws Exception {
            try {
              ...
              ActivityPostEntity.STATUS status = ActivityPostEntity.STATUS.valueOf(
              ...
              switch (status) {
                ...
                case PENDING:
                  ...
                  activityPost.setStatus(
                    ActivityPostEntity.STATUS.POSTED.toString()
                  ); // for debug output
                  break;
                default:
  ...
  private JSONObject lookupNode(
    ...
    jo.put(
      JSON_PARENT_NODEREF,
      (parentNodeRef != null ? parentNodeRef.toString() : null)
    );

...
public class LockingJobTest {
  ...
  public void testJobInClusterNotLocked() throws Exception {
    ...
    JobExecutionContext mockJobExecutionContext = mock(
      JobExecutionContext.class
    );
    // create the hb collector
    SimpleHBDataCollector simpleCollector = spy(
      new SimpleHBDataCollector("simpleCollector")
    );
    ...
    verify(mockJobLockService, Mockito.times(2))
      .getLock(any(QName.class), anyLong());
    verify(mockJobLockService, Mockito.times(2))
      .refreshLock(
        eq(lockToken),
        any(QName.class),

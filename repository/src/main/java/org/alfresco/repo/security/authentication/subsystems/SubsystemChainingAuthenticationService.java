...
/**
 * An authentication service that chains across beans in multiple child application contexts
 * corresponding to different 'subsystems' in a chain determined by a {@link
 * ChildApplicationContextManager}. The first authentication service in the chain will always be
 * considered to be the 'mutable' authentication service.
 *
 * @author dward
 */
public class SubsystemChainingAuthenticationService extends AbstractChainingAuthenticationService {
    ...
    // Bring our cached copies of the source beans in line with the application context manager,
    // using a RW lock to
    // ensure consistency
    private void refreshBeans() {
        ...
        try {
            if (this.instanceIds == null
                    || !this.instanceIds.equals(this.applicationContextManager.getInstanceIds())) {
            ...
            for (String instance : this.instanceIds) {
                ...
                try {
                    ...
                    // This subsystem won't start. The reason would have been logged. Ignore and
                    // continue.
                    newContext = null;
                }
                ...
                if (context != newContext) {
                    ...
                    try {
                        ...
                        // This subsystem won't start or the bean doesn't exist. The reason would
                        // have been logged. Ignore and continue.
        ...
        try {
            ...
            for (String instance : this.instanceIds) {
                AuthenticationService authenticationService =
                        (AuthenticationService) this.sourceBeans.get(instance);
                // Only add active authentication services. E.g. we might have an ldap context that
                // is only used for
                // synchronizing
                if (authenticationService instanceof MutableAuthenticationService
                        && (!(authenticationService instanceof ActivateableBean)

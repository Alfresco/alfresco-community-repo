 ...
 *
 * <p>Provides special handling for MS Office files that are written to by the app even though the
 * user does not change the file.
 *
 * @author gkspencer
 */
    ...
    public void writeFile(byte[] buffer, int length, int position, long fileOffset)
        ...
        if (m_writeList != null) {
            ...
            if (getReadCount() > 0 && m_writeList.size() < 2) {
                ...
                // Buffer the write, looks like a file open update. Do not buffer zero length
                // writes.
            ...
            if (m_writeList.size() > 0) {
                ...
                if (logger.isDebugEnabled()) logger.debug("MSOfficeFile: Buffered writes flushed");

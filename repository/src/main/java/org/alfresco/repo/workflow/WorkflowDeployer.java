...
public class WorkflowDeployer extends AbstractLifecycleBean {
    ...
    private WorkflowDeployerTransactionListener workflowDeployerTransactionListener =
            new WorkflowDeployerTransactionListener();
    ...
    // used by TenantAdminService when creating a new tenant and bootstrapping the pre-defined
    // workflows
    ...
    public void init() {
        ...
        try {
            ...
            if (models != null
                ...
                // MNT-10537 fix, since new model was deployed we need to destroy dictionary to
                // force lazy re-init
            ...
            if (workflowDefinitions != null) {
                for (Properties workflowDefinition : workflowDefinitions) {
                    ...
                    if (workflowAdminService.isEngineEnabled(engineId)) {
                        Boolean redeploy =
                                Boolean.valueOf(workflowDefinition.getProperty(REDEPLOY));
                        ...
                        if (!redeploy
                                && workflowService.isDefinitionDeployed(
                                        engineId, workflowResource.getInputStream(), mimetype)) {
            ...
            Boolean value =
                    (Boolean)
                            nodeService.getProperty(
                                    nodeRef, WorkflowModel.PROP_WORKFLOW_DEF_DEPLOYED);
            if ((value != null) && (value.booleanValue() == true)) {
                String engineId =
                        (String)
                                nodeService.getProperty(
                                        nodeRef, WorkflowModel.PROP_WORKFLOW_DEF_ENGINE_ID);
    ...
    /**
     * Validate that the workflow definition node is a child of the correct workflow location node,
     * e.g. "/Company Home/Data Dictionary/Workflows"
     */
    ...
    public void undeploy(NodeRef nodeRef) {
        ...
        if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_WORKING_COPY) == false) {
            String defName =
                    (String) nodeService.getProperty(nodeRef, WorkflowModel.PROP_WORKFLOW_DEF_NAME);
            if (defName != null) {
        ...
        for (String workflowName : workflowNames) {
            ...
            if (workflowAdminService.isEngineEnabled(engineId)) {
                ...
                List<WorkflowDefinition> defs =
                        workflowService.getAllDefinitionsByName(workflowName);
                if (defs.size() > 0) {
                    undeployed++;
                }
                for (WorkflowDefinition def : defs) {
                                    ...
                                    new RunAsWork<Object>() {
                                        public Object doWork() {
                                            ...
                                            if (logger.isTraceEnabled()) {
                                                logger.trace(
                                                        "Workflow deployer afterCommit: Dictionary"
                                                                + " destroyed ["

...
public class FilePlanComponentsApiUtils {
    ...
    public static final List<String> CONTAINERS_FOR_CLASSIFIABLE_CHILDREN_ALIAS =
            Arrays.asList(FILE_PLAN_ALIAS, UNFILED_ALIAS);
    // public static String PARAM_RELATIVE_PATH = "relativePath";

    // excluded properties
    public static final List<QName> TYPES_CAN_CREATE =
            Arrays.asList(
     ...
     */
    public NodeRef lookupAndValidateNodeType(String nodeId, QName expectedNodeType)
            throws EntityNotFoundException {
     ...
     * @throws EntityNotFoundException
     */
    public NodeRef lookupAndValidateNodeType(
            String nodeId, QName expectedNodeType, String relativePath)
            throws EntityNotFoundException {
        ...
        if (!nodeType.equals(expectedNodeType)) {
            throw new InvalidArgumentException(
                    "The given id:'"
                            ...
                            + "' (nodeType:"
                            + nodeType.toString()
                            + ") is not valid for this endpoint. Expected nodeType is:"
                            + expectedNodeType.toString());
        }

        if (StringUtils.isNotBlank(relativePath)) {
            nodeRef =
                    lookupAndValidateRelativePath(
                            nodeRef, relativePath, readOnlyRelativePath, expectedNodeType);
        }
        return nodeRef;
    }
    ...
    /**
     * Helper method that creates a relative path if it doesn't already exist The relative path will
     * be build with nodes of the type specified in nodesType If the relative path already exists
     * the method validates if the last element is of type nodesType The method does not validate
     * the type of parentNodeRef
     *
     * @param parentNodeRef the first node of the path
     * @param relativePath a string representing the relative path in the format
     *     "Folder1/Folder2/Folder3"
     * @param nodesType the type of all the containers in the path
     * @return the last element of the relative path
     */
    public NodeRef lookupAndValidateRelativePath(
            final NodeRef parentNodeRef, String relativePath, QName nodesType) {
        return lookupAndValidateRelativePath(parentNodeRef, relativePath, false, nodesType);
    }

    /**
     * Helper method that creates a relative path if it doesn't already exist and if relative path
     * is not read only. If relative path is read only an exception will be thrown if the provided
     * relative path does not exist. The relative path will be build with nodes of the type
     * specified in nodesType If the relative path already exists the method validates if the last
     * element is of type nodesType The method does not validate the type of parentNodeRef
     *
     * @param parentNodeRef the first node of the path
     * @param relativePath a string representing the relative path in the format
     *     "Folder1/Folder2/Folder3"
     * @param readOnlyRelativePath the flag that indicates if the relativePath should be created if
    ...
    public NodeRef lookupAndValidateRelativePath(
        ...
        if (i == pathElements.size()) {
            ...
            if (!nodeType.equals(nodesType)) {
                throw new InvalidArgumentException(
                        "The given id:'"
                                ...
                                + relativePath
                                + "' reach a node type invalid for this endpoint."
                                + " Expected nodeType is:"
                                + nodesType.toString()
                                + ". Actual nodeType is:"
                                + nodeType);
            }
            ...
            if (!readOnlyRelativePath) {
                ...
                throw new NotFoundException(
                        "The entity with relativePath: " + relativePath + " was not found.");
            }
        }
        ...
        if (nodesType.equals(RecordsManagementModel.TYPE_UNFILED_RECORD_FOLDER)) {
            ...
            throw new InvalidArgumentException(
                    "Creating relative path of type '"
                            + nodesType
                            + "' not suported for this endpoint");
        }
    ...
    /**
     * Helper method that parses a string representing a file path and returns a list of element
     * names
     *
    ...
    public NodeRef createRMNode(NodeRef parentNodeRef, RMNode nodeInfo, Parameters parameters) {
        ...
        try {
            ...
            Map<QName, Serializable> qnameProperties =
                    mapToNodeProperties(nodeInfo.getProperties());
            ...
            if (aspectNames != null) {
                nodes.addCustomAspects(
                        newNodeRef, aspectNames, ApiNodesModelFactory.EXCLUDED_ASPECTS);
            }
     ...
     */
    public NodeRef uploadRecord(
            NodeRef parentNodeRef, UploadInfo uploadInfo, Parameters parameters) {
        mandatory("parentNodeRef", parentNodeRef);
        ...
        NodeRef newNodeRef =
                fileFolderService.create(parentNodeRef, nodeName, typeQName).getNodeRef();
     ...
     */
    public List<FilterProp> getListChildrenFilterProps(
        ...
        if (q != null) {
            ...
            Boolean isPrimary =
                    propertyWalker.getProperty(
                            RMNode.PARAM_ISPRIMARY, WhereClauseParser.EQUALS, Boolean.class);
            ...
            Boolean isClosed =
                    propertyWalker.getProperty(
                            RMNode.PARAM_IS_CLOSED, WhereClauseParser.EQUALS, Boolean.class);
            if (isClosed != null) {
                filterProps = new ArrayList<>(1);
                filterProps.add(
                        new FilterPropBoolean(RecordsManagementModel.PROP_IS_CLOSED, isClosed));
            }
            // TODO see how we can filter for categories that have retention schedule
            //            Boolean hasRetentionSchedule =
            // propertyWalker.getProperty(RMNode.PARAM_HAS_RETENTION_SCHEDULE,
    ...
    public NodeRef validateRecord(String recordId) throws InvalidArgumentException {
        ...
        if (!recordService.isRecord(nodeRef)) {
            throw new IllegalArgumentException(
                    "The given id:'"
                            + recordId
                            + "' is not valid for this endpoint. This endpoint only supports"
                            + " records.");
        }
        return nodeRef;
    }
     ...
     * @param parameters request parameters
     */
    public void updateTransferContainer(
            NodeRef nodeRef, TransferContainer transferContainerInfo, Parameters parameters) {
        Map<QName, Serializable> props = new HashMap<>(0);
     ...
     * @return a sublist of [{@link Nodes#OP_DELETE}, {@link Nodes#OP_CREATE}, {@link
     *     Nodes#OP_UPDATE}] representing the allowable operations for the provided node
     */
    protected List<String> getAllowableOperations(NodeRef nodeRef, QName typeQName) {
        ...
        boolean isTransferContainer =
                typeQName.equals(RecordsManagementModel.TYPE_TRANSFER_CONTAINER);
        boolean isUnfiledContainer =
                typeQName.equals(RecordsManagementModel.TYPE_UNFILED_RECORD_CONTAINER);
        boolean isHoldsContainer = typeQName.equals(RecordsManagementModel.TYPE_HOLD_CONTAINER);
        boolean isSpecialContainer =
                isFilePlan || isTransferContainer || isUnfiledContainer || isHoldsContainer;

        // DELETE
        if (!isSpecialContainer
                && capabilityService.getCapability("Delete").evaluate(nodeRef)
        ...
        if (TYPES_CAN_CREATE.contains(typeQName)
                && capabilityService.getCapability("FillingPermissionOnly").evaluate(nodeRef)
                        == AccessDecisionVoter.ACCESS_GRANTED) {
            allowableOperations.add(Nodes.OP_CREATE);
        }

        // UPDATE
        if (capabilityService.getCapability("Update").evaluate(nodeRef)
                == AccessDecisionVoter.ACCESS_GRANTED) {
    ...
    public void postActivity(FileInfo fileInfo, NodeRef parentNodeRef, String activityType) {
        ...
        if (rmSite != null && !rmSite.getId().equals("")) {
            if (fileInfo != null) {
                boolean isContent =
                        dictionaryService.isSubClass(fileInfo.getType(), ContentModel.TYPE_CONTENT);
        ...
        if (activityType != null) {
            activityPoster.postFileFolderActivity(
                    activityType,
                    null,

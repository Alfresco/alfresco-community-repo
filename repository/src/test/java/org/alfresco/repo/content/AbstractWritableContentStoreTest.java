...
public abstract class AbstractWritableContentStoreTest extends AbstractReadOnlyContentStoreTest {
     ...
     * {@inheritDoc}
     *
     * <p>This implementation creates some content in the store and returns the new content URL.
     */
    ...
    /**
     * Get a writer into the store. This test class assumes that the store is writable and that it
     * therefore supports the ability to write content.
     *
    ...
    public void testSetUp() throws Exception {
        ...
        assertTrue(
                "The same instance of the store must be returned for getStore",
                store == getStore());
    }
    ...
    private void checkIllegalWritableContentUrl(ContentStore store, String contentUrl) {
        assertFalse(
                "This check is for unsupported content URLs only",
                store.isContentUrlSupported(contentUrl));
    ...
    public void testSimpleUse() {
        ...
        assertTrue(
                "URL is not valid: " + contentUrlBefore,
                AbstractContentStore.isValidContentUrl(contentUrlBefore));
        // Write something
        writer.putContent(content);
        String contentUrlAfter = writer.getContentUrl();
        assertTrue(
                "URL is not valid: " + contentUrlBefore,
                AbstractContentStore.isValidContentUrl(contentUrlAfter));
        assertEquals(
                "The content URL may not change just because the writer has put content",
                contentUrlBefore,
                contentUrlAfter);
        // Get the readers
    ...
    public synchronized void testGetReader() throws Exception {
        ...
        assertNotNull(
                "A reader must always be available from the store", readerFromStoreBeforeWrite);

        // check that a reader is available from the writer
        ContentReader readerFromWriterBeforeWrite = writer.getReader();
        assertNotNull(
                "A reader must always be available from the writer", readerFromWriterBeforeWrite);
    ...
    /**
     * Check that a reader is immutable, i.e. that a reader fetched before a write doesn't suddenly
     * become aware of the content once it has been written.
     */
    @Test
    public void testReaderImmutability() {
        ...
        assertTrue(
                "New reader after write should be directed to new content",
                readerAfterWrite.exists());
    }
    ...
    public void testMimetypAndEncodingAndLocale() throws Exception {
        ...
        assertEquals(
                "Writer -> Reader content URL mismatch",
                writer.getContentUrl(),
                reader.getContentUrl());
        assertEquals(
                "Writer -> Reader mimetype mismatch", writer.getMimetype(), reader.getMimetype());
        assertEquals(
                "Writer -> Reader encoding mismatch", writer.getEncoding(), reader.getEncoding());
        assertEquals("Writer -> Reader locale mismatch", writer.getLocale(), reader.getLocale());

        // now get the string directly from the reader
        String contentCheck =
                reader.getContentString(); // internally it should have taken care of the encoding
    ...
    public void testClosedState() throws Exception {
        ...
        assertFalse(
                "Before-content reader should not be affected by content updates",
                readerBeforeWrite.isClosed());
        ...
        assertEquals(
                "Readers should refer to same URL",
                readerBeforeWrite.getContentUrl(),
                readerAfterWrite.getContentUrl());
        ...
        assertFalse(
                "Before-content reader stream should not be closed", readerBeforeWrite.isClosed());
        assertTrue(
                "After-content reader should be closed after reading", readerAfterWrite.isClosed());
    }
    ...
    public void testDeleteSimple() throws Exception {
        ...
        try {
            ...
            logger.warn(
                    "Store test testDeleteSimple not possible on " + store.getClass().getName());
            return;
        }
     ...
     *
     * <p>Only applies when {@link #getStore()} returns a value.
     */
    @Test
    public void testDeleteReaderStates() throws Exception {
        ...
        reader = store.getReader(contentUrl);
        assertNotNull("Reader must be returned even when underlying content is missing", reader);
        assertFalse("Content should not exist", reader.exists());
        try {
            is = reader.getContentInputStream();
    ...
    /**
     * The simplest test. Write a string and read it again, checking that we receive the same
     * values. If the resource accessed by {@link #getReader(String)} and {@link #getWriter()} is
     * not the same, then values written and read won't be the same.
     */
    @Test
     ...
     *
     * <p>Only executes if the writer implements {@link RandomAccessContent}.
     */
    @Test
    public void testRandomAccessWrite() throws Exception {
        ...
        // get a new writer from the store, using the existing content and perform a truncation
        // check
        ...
        // get a new writer from the store, using the existing content and perform a non-truncation
        // check
     ...
     *
     * <p>Only executes if the reader implements {@link RandomAccessContent}.
     */
    @Test

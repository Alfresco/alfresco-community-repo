...
public class UserGroupMembershipUtils {
    ...
    public static final QName PARENT_GROUP =
            QName.createQName(RecordsManagementModel.RM_URI, "Parent Group");
    /** A QName to display for a child group's name. */
    private static final QName CHILD_GROUP =
            QName.createQName(RecordsManagementModel.RM_URI, "Child Group");
    ...
    public static Map<QName, Serializable> makePropertiesMap(
            ChildAssociationRef childAssocRef, NodeService nodeService) {
        ...
        String childUserName =
                (String)
                        nodeService.getProperty(
                                childAssocRef.getChildRef(), ContentModel.PROP_USERNAME);
        if (!isBlank(childUserName)) {
            auditProperties.put(ContentModel.PROP_USERNAME, childUserName);
        }
        // Set the parent group name.
        auditProperties.put(
                PARENT_GROUP, getUserGroupName(childAssocRef.getParentRef(), nodeService));

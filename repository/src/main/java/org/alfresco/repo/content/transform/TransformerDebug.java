 ...
 *
 * <p>As transformations are frequently composed of lower level transformations, log messages
 * include a prefix to identify the transformation. A numeric dot notation is used (such as {@code
 * 123.1.2} indicating the second third level transformation of the 123rd top level transformation).
 *
 * @author Alan Davis
 */
public class TransformerDebug {
    ...
    protected class UnavailableTransformer implements Comparable<UnavailableTransformer> {
        ...
        UnavailableTransformer(
                String name, String priority, long maxSourceSizeKBytes, boolean debug) {
            this.name = name;
        ...
        public boolean equals(Object obj) {
            if (this == obj) {
                ...
                return EqualsHelper.nullSafeEquals(name, that.name)
    ...
    /**
     * Adds a new level to the stack to get a new request number or nesting number. Called prior to
     * working out what transformers are active and prior to listing the supported mimetypes for an
     * active transformer.
     */
    public void pushMisc() {
        if (isEnabled()) {
            push(null, null, null, null, -1, null, null, null, Call.AVAILABLE);
        }
    }
    ...
    void push(
        ...
        boolean origDebugOutput =
                ThreadInfo.setDebugOutput(ThreadInfo.getDebugOutput() && sourceSize != 0);
        frame =
                new Frame(
                        frame,
                        transformerName,
    ...
    protected void logBasicDetails(
            Frame frame,
            long sourceSize,
            Map<String, String> options,
            String renditionName,
        ...
        log(
                getSourceAndTargetExt(frame.sourceMimetype, frame.targetMimetype)
                        + ((fileName != null) ? fileName + ' ' : "")
                        + ((sourceSize >= 0) ? fileSize(sourceSize) + ' ' : "")
                        + (firstLevel ? getRenditionName(renditionName) : "")
                        + message);
        if (firstLevel) {
            log(options);
    ...
    public void debug(
        ...
        log(
                "              "
                        ...
                        + ((fileName != null) ? fileName + ' ' : "")
                        + ((sourceSize >= 0) ? fileSize(sourceSize) + ' ' : "")
                        + (getRenditionName(renditionName))
                        + message);
        log(options);
    }
    ...
    /**
     * Removes a frame from the stack. Called prior to working out what transformers are active and
     * prior to listing the supported mimetypes for an active transformer.
     */
    public void popMisc() {
        if (isEnabled()) {
            ...
            if ((frame.callType == callType)
                ...
                if (!suppressFinish && (firstLevel || logger.isTraceEnabled())) {
                    log(
                            FINISHED_IN
                                    + ms
                                    + (frame.callType == Call.AVAILABLE && !suppressChecking
                                            ? " Just checking if a transformer is available"
                                            : "")
                                    + (firstLevel ? "\n" : ""),
                            firstLevel);
                }
        ...
        if (info.isDebugEnabled()) {
            ...
            if (NO_TRANSFORMERS.equals(failureReason)) {
                    ...
                    if (frame.unavailableTransformers != null) {
                        ...
                        for (UnavailableTransformer unavailable : frame.unavailableTransformers) {
                            if (smallestMaxSourceSizeKBytes > unavailable.maxSourceSizeKBytes
                                    && unavailable.maxSourceSizeKBytes > 0) {
                                smallestMaxSourceSizeKBytes = unavailable.maxSourceSizeKBytes;
                            }
                        }
                        smallestMaxSourceSizeKBytes =
                                smallestMaxSourceSizeKBytes == Long.MAX_VALUE
                                        ? 0
                                        : smallestMaxSourceSizeKBytes;
                        failureReason =
                                "No transformers as file is > "
                                        + fileSize(smallestMaxSourceSizeKBytes * 1024);
                    }
                }
                ...
                debug =
                        size == 1
                                || (size == 2
                                        && ThreadInfo.getStack().peekLast().callType
                                                != Call.TRANSFORM);
            }

            if (level != null) {
                infoLog(
                        getReference(debug, false, false),
                        sourceAndTargetExt,
                        level,
                        fileName,
                        sourceSize,
    ...
    public boolean isEnabled() {
        // Don't check ThreadInfo.getDebugOutput() as availableTransformers() may upgrade from trace
        // to debug.
        return logger.isDebugEnabled()
                || info.isDebugEnabled()
                || ThreadInfo.getStringBuilder() != null;
    }

    /**
     * Enable or disable debug log output. Normally used to hide calls to getTransformer as trace
     * rather than debug level log messages. There are lots of these and it makes it hard to see
     * what is going on.
     *
     * @param debugOutput if {@code true} both debug and trace is generated. Otherwise all output is
     *     trace.
    ...
    /**
     * Log a message prefixed with the current transformation reference and include a exception,
     * suppressing the stack trace if repeated as we return up the stack of transformers.
     *
     * @param message
     */
    ...
    /**
     * Sets the cause of a transformation failure, so that only the message of the Throwable is
     * reported later rather than the full stack trace over and over.
     */
    ...
    /**
     * Returns the current StringBuilder (if any) being used to capture debug information for the
     * current Thread.
     */
     ...
     *
     * @param firstLevelOnly indicates if only the top level should be included and no extra
     *     padding.
     * @param overrideFirstLevel if the first level id should just be set to 1 (used in test
     *     methods)
     * @param usePreviousRef if the reference of the last transform performed by this Thread should
     *     be used.
     * @return a padded (fixed length) reference.
     */
    private String getReference(
            boolean firstLevelOnly, boolean overrideFirstLevel, boolean usePreviousRef) {
        if (usePreviousRef) {
    ...
    private String getFileNameOrNodeRef(
            NodeRef sourceNodeRef, boolean firstLevel, long sourceSize, boolean getName) {
        ...
        if (sourceNodeRef != null) {
            try {
                ...
                // ignore (normally InvalidNodeRefException) but we should ignore other
    ...
    public int debugTransformServiceRequest(
            String sourceMimetype,
            long sourceSize,
            NodeRef sourceNodeRef,
            int contentHashcode,
    ...
    public void debugTransformServiceResponse(
            NodeRef sourceNodeRef,
            int contentHashcode,
            long requested,
            int id,

...
public class Site implements Serializable {
  ...
  public String getSiteGroup() {
    if (this.siteGroup == null) {
      this.siteGroup =
        this.siteService.getSiteGroup(this.siteInfo.getShortName());
    }
  ...
  public ScriptableHashMap<String, String> getSitePermissionGroups() {
    if (this.siteRoleGroups == null) {
      List<String> roles =
        this.siteService.getSiteRoles(this.siteInfo.getShortName());
  ...
  public ScriptNode getContainer(final String componentId) {
    ...
    NodeRef containerNodeRef = AuthenticationUtil.runAs(
    ...
    if (containerNodeRef != null) {
      container =
        new ScriptNode(containerNodeRef, this.serviceRegistry, this.scope);
    }
    return container;
  }
  ...
  public ScriptNode createContainer(
    ...
    if (
      Site.this.serviceRegistry.getPermissionService()
    ...
    return AuthenticationUtil.runAs(
      new RunAsWork<NodeRef>() {
        public NodeRef doWork() throws Exception {
          ...
          QName folderQName = (folderType == null)
            ? null
            : QName.createQName(
              folderType,
            ...
            for (int i = 0; i < propIds.length; i++) {
              ...
              if (propId instanceof String) {
                ...
                if (value instanceof String) {
                  ...
                  Site.this.serviceRegistry.getPermissionService()
                    .setPermission(containerNode, key, (String) value, true);
                }
              }
            }
          }
    ...
    if (container == null) {
      RetryingTransactionCallback<NodeRef> txnCallback = new RetryingTransactionCallback<NodeRef>() {
        public NodeRef execute() throws Throwable {
          final NodeRef containerRef = createContainerImpl(
            componentId,
            folderType,
            null
          );
            ...
            for (int i = 0; i < propIds.length; i++) {
              ...
              if (propId instanceof String) {
                ...
                if (value instanceof String) {
                  ...
                  QName qname = QName.resolveToQName(
                    serviceRegistry.getNamespaceService(),
                    key
                  );
                  serviceRegistry
                    .getNodeService()
                    .setProperty(containerRef, qname, (Serializable) value);
                }
              }
            }
          }
      ...
      final NodeRef containerNodeRef = serviceRegistry
        .getRetryingTransactionHelper()
        .doInTransaction(txnCallback, false, true);
      if (
        this.serviceRegistry.getPermissionService()
          .hasPermission(containerNodeRef, PermissionService.READ_PROPERTIES) ==
        AccessStatus.ALLOWED
      ) {
        container =
        ...
        AuthenticationUtil.runAs(
          new RunAsWork<Void>() {
            public Void doWork() throws Exception {
              ...
              String managers = siteService.getSiteRoleGroup(
                getShortName(),
  ...
  public ScriptableQNameMap<String, CustomProperty> getCustomProperties() {
    if (this.customProperties == null) {
      ...
      ScriptNode siteNode = new ScriptNode(
        this.siteInfo.getNodeRef(),
        this.serviceRegistry
      );
      ...
      for (QName qname : props.keySet()) {
        ...
        Serializable value = valueConverter.convertValueForScript(
          qname,
          propValue
        );
  ...
  public ScriptInvitation<?> inviteModerated(
    ...
    if (invitations.isEmpty()) {
      Invitation invitation = invitationService.inviteModerated(
        inviteeComments,
        inviteeUserName,

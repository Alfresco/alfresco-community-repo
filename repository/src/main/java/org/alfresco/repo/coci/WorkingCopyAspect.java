...
public class WorkingCopyAspect
   ...
   */
  public void onDeleteCmisCreatedCheckoutWorkingCopyAssociation(
  ...
  public void beforeArchiveNode(NodeRef workingCopyNodeRef) {
    NodeRef checkedOutNodeRef = checkOutCheckInService.getCheckedOut(
      workingCopyNodeRef
    );
    ...
    if (checkedOutNodeRef != null) {
      try {
        ...
        if (
          nodeService.hasAspect(checkedOutNodeRef, ContentModel.ASPECT_LOCKABLE)
        ) {
          Map<QName, Serializable> checkedOutNodeProperties = nodeService.getProperties(
            checkedOutNodeRef
          );
          Map<QName, Serializable> workingCopyProperties = nodeService.getProperties(
            workingCopyNodeRef
          );
          ...
          String lockOwner = (String) checkedOutNodeProperties.get(
            ContentModel.PROP_LOCK_OWNER
          );
          Date expiryDate = (Date) checkedOutNodeProperties.get(
            ContentModel.PROP_EXPIRY_DATE
          );
          String lockTypeStr = (String) checkedOutNodeProperties.get(
            ContentModel.PROP_LOCK_TYPE
          );
          LockType lockType = lockTypeStr != null
            ? LockType.valueOf(lockTypeStr)
            : null;
          String lifetimeStr = (String) checkedOutNodeProperties.get(
            ContentModel.PROP_LOCK_LIFETIME
          );
          Lifetime lifetime = lifetimeStr != null
            ? Lifetime.valueOf(lifetimeStr)
            : null;
          String additionalInfo = (String) checkedOutNodeProperties.get(
          ...
          workingCopyProperties.put(
            ContentModel.PROP_ARCHIVED_LOCK_OWNER,
            lockOwner
          );
          workingCopyProperties.put(
            ContentModel.PROP_ARCHIVED_LOCK_TYPE,
            lockType
          );
          workingCopyProperties.put(
            ContentModel.PROP_ARCHIVED_LOCK_LIFETIME,
            lifetime
          );
          workingCopyProperties.put(
            ContentModel.PROP_ARCHIVED_EXPIRY_DATE,
            expiryDate
          );
          workingCopyProperties.put(
          ...
          // Target associations
          Collection<Pair<Long, AssociationRef>> targetAssocs = nodeDAO.getTargetNodeAssocs(
            nodeId,
            null
          );
          for (Pair<Long, AssociationRef> targetAssocPair : targetAssocs) {
            if (
              ContentModel.ASSOC_ORIGINAL.equals(
          ...
          // Source associations
          Collection<Pair<Long, AssociationRef>> sourceAssocs = nodeDAO.getSourceNodeAssocs(
            nodeId,
            null
          );
          for (Pair<Long, AssociationRef> sourceAssocPair : sourceAssocs) {
            if (
              ContentModel.ASSOC_WORKING_COPY_LINK.equals(
  ...
  public void onRestoreNode(ChildAssociationRef childAssocRef) {
    ...
    if (
      ...
      try {
        ...
        String lockOwner = (String) workingCopyProperties.get(
          ContentModel.PROP_ARCHIVED_LOCK_OWNER
        );
        workingCopyProperties.remove(ContentModel.PROP_ARCHIVED_LOCK_OWNER);
        Date expiryDate = (Date) workingCopyProperties.get(
          ContentModel.PROP_ARCHIVED_EXPIRY_DATE
        );
        workingCopyProperties.remove(ContentModel.PROP_ARCHIVED_EXPIRY_DATE);
        String lockTypeStr = (String) workingCopyProperties.get(
          ContentModel.PROP_ARCHIVED_LOCK_TYPE
        );
        ...
        LockType lockType = lockTypeStr != null
          ? LockType.valueOf(lockTypeStr)
          : null;
        String lifetimeStr = (String) workingCopyProperties.get(
          ContentModel.PROP_ARCHIVED_LOCK_LIFETIME
        );
        ...
        Lifetime lifetime = lifetimeStr != null
          ? Lifetime.valueOf(lifetimeStr)
          : null;
        String additionalInfo = (String) workingCopyProperties.get(
          ContentModel.PROP_ARCHIVED_LOCK_ADDITIONAL_INFO
        );
        workingCopyProperties.remove(
          ContentModel.PROP_ARCHIVED_LOCK_ADDITIONAL_INFO
        );

        List<AssociationRef> targetAssocList = (ArrayList<AssociationRef>) workingCopyProperties.get(
          ContentModel.PROP_ARCHIVED_TARGET_ASSOCS
        );
        ...
        ArrayList<AssociationRef> sourceAssocList = (ArrayList<AssociationRef>) workingCopyProperties.get(
          ContentModel.PROP_ARCHIVED_SOURCE_ASSOCS
        );
  ...
  private class WorkingCopyAspectCopyBehaviourCallback
     ...
     */
    @Override
    public boolean isTopLevelCanBeRenamed(
      QName classQName,
      CopyDetails copyDetails
    ) {
      return true;
    ...
    public Map<QName, Serializable> getCopyProperties(
      ...
      if (classQName.equals(ContentModel.ASPECT_WORKING_COPY)) {
        ...
        if (copyDetails.isTargetNodeIsNew()) {
          ...
          NodeRef checkedOutFrom = checkOutCheckInService.getCheckedOut(
            copyDetails.getSourceNodeRef()
          );
          if (checkedOutFrom != null) {
            String oldName = (String) nodeService.getProperty(
              checkedOutFrom,

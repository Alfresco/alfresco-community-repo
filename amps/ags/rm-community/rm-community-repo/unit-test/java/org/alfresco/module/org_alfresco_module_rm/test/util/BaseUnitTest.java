 ...
 * Base unit test.
 *
 * <p>Contains core and records management service mocks ready for injection. Helper methods provide
 * an easy way to build RM or Alfresco constructs for use in tests.
 *
...
public class BaseUnitTest implements RecordsManagementModel, ContentModel {
    ...
    public void before() throws Exception {
        ...
        doReturn(CollectionUtils.unmodifiableSet(RM_PREFIX))
                .when(mockedNamespaceService)
                .getPrefixes(RM_URI);
        ...
        doReturn(result)
                ...
                .getChildAssocs(
                        eq(recordFolder), eq(ContentModel.ASSOC_CONTAINS), any(QNamePattern.class));
     ...
     *
     * <p>Assumes the cm:contains assoc type.
     *
     * @param child
     * @param parent
     */
    ...
    protected void makePrimaryParentOf(
            NodeRef child, NodeRef parent, QName assocType, QName assocName) {
    ...
    protected void makeChildrenOf(NodeRef parent, NodeRef... children) {
        ...
        for (NodeRef child : children) {
            assocs.add(
                    new ChildAssociationRef(
                            ContentModel.ASSOC_CONTAINS, parent, generateQName(), child));
            doReturn(assocs).when(mockedNodeService).getParentAssocs(child);
        }
        doReturn(assocs)
                .when(mockedNodeService)
                .getChildAssocs(parent, ContentModel.ASSOC_CONTAINS, RegexQNamePattern.MATCH_ALL);
    }

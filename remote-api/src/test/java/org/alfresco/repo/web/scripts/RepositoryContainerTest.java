...
public class RepositoryContainerTest extends BaseWebScriptTest {
  ...
  private static final Pattern HIDDEN_EXCEPTION_PATTERN = Pattern.compile(
  ...
  public void testReset() throws Exception {
    RepositoryContainer repoContainer = (RepositoryContainer) getServer()
      .getApplicationContext()
      .getBean("webscripts.container");
    repoContainer.reset();
  }
  ...
  public void testLargeContentRequest() throws Exception {
    ...
    Response response = sendRequest(
      new PutRequest("/test/largecontenttest", content, "text/plain"),
      STATUS_OK
    );
    assertEquals(SUCCESS, response.getContentAsString());

    // trigger the webscript temp folder cleaner job
    CronTrigger webscriptsTempFileCleanerJobTrigger = (CronTrigger) getServer()
      .getApplicationContext()
      .getBean("webscripts.tempFileCleanerTrigger");

    Scheduler scheduler = (Scheduler) getServer()
      .getApplicationContext()
  ...
  public void testHideExceptions() throws Exception {
    final String messageFormException =
      "Failed to persist field 'prop_cm_name'";
    final String messageAuthenticationException =
    ...
    try {
      ...
      try {
        ...
        assertTrue(
          "FormException should be visible for client",
      ...
      repoContainerMock.setNotPublicExceptions(
        Arrays.asList(defaultConfiguration)
      );
    }
  }
    ...
    try {
      ...
      try {
        ...
        assertNull(
          ...
          ExceptionStackUtil.getCause(
            e,
            new Class[] { ContentIOException.class }
          )
        );
        assertTrue(
          "Details should be in the server logs.",
        ...
        .doAnswer(
          new Answer<Object>() {
            public Object answer(InvocationOnMock invocation) {
              throw new AlfrescoRuntimeException(
                "AlfrescoRuntimeException",
                new ContentIOException(commandExecutionResult.toString())
              );
            }
          }
        )
      ...
      List<Class<?>> updatedNotPublicExceptions = new LinkedList<Class<?>>(
      ...
      try {
        ...
        assertTrue(
          "'AlfrescoRuntimeException' cause should not be hidden for client. Exception class: " +
          e.getClass().getName(),

...
public class DeclarativeCapabilityTest extends BaseRMTestCase {
  ...
  protected void setupTestDataImpl() {
    ...
    // Closed folder
    closedFolder =
      recordFolderService.createRecordFolder(rmContainer, "closedFolder");
    ...
    frozenRecord2 =
      utils.createRecord(recordFolderContainsFrozen, "frozen2.txt");
    frozenRecordFolder =
      recordFolderService.createRecordFolder(rmContainer, "frozenRecordFolder");

    // MoveTo artifacts
    moveToFolder =
      recordFolderService.createRecordFolder(rmContainer, "moveToFolder");
    moveToCategory =
      filePlanService.createRecordCategory(rmContainer, "moveToCategory");
  }
  ...
  public void testDeclarativeCapabilities() {
    ...
    for (Capability capability : capabilities) {
      if (
        capability instanceof DeclarativeCapability &&
  ...
  private void testDeclarativeCapability(
    ...
    doTestInTransaction(
      new Test<Void>() {
        @Override
        public Void run() {
          AccessStatus accessStatus = capability.hasPermission(
            filePlanComponent
          );
          ...
          if (roles.isEmpty()) {
            assertEquals(
              "User " +
              userName +
              " has no RM role so we expect access to be denied for capability " +
              capability.getName(),
              AccessStatus.DENIED,
              accessStatus
            );
            ...
            FilePlanComponentKind actualKind = filePlanService.getFilePlanComponentKind(
              filePlanComponent
            );
            ...
            if (kinds == null || kinds.contains(actualKind.toString())) {
              ...
              assertEquals(
                "User is expected to only have one role.",
                1,
                roles.size()
              );
              ...
              if (roleCapabilities.contains(capability) && conditionResult) {
                assertEquals(
                  ...
                  " so we expect access to be allowed for capability " +
                  capability.getName() +
                  " on the object " +
    ...
    doTestInTransaction(
      new Test<Void>() {
        ...
        public Void run() {
          ...
          assertEquals(
            AccessStatus.DENIED,
            capability.hasPermission(frozenRecordFolder)
          );
          assertEquals(
            AccessStatus.DENIED,
          ...
          assertEquals(
            AccessStatus.DENIED,
            capability.hasPermission(frozenRecord)
          );
          assertEquals(
            AccessStatus.DENIED,
            capability.hasPermission(closedFolder)
          );
      ...
      new Test<Void>() {
        ...
        public Void run() {
          ...
          assertEquals(
            ...
            capability.hasPermission(record)
          );
          assertEquals(
            AccessStatus.UNDETERMINED,
            capability.hasPermission(declaredRecord)

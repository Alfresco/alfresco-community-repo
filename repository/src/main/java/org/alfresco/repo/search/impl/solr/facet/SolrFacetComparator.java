...
public class SolrFacetComparator implements Comparator<SolrFacetProperties> {
  ...
  public int compare(SolrFacetProperties facet1, SolrFacetProperties facet2) {
    ...
    if (bothSorted(facetIndicesInSortedList)) {
      ...
      return (
        facetIndicesInSortedList.getFirst() -
        facetIndicesInSortedList.getSecond()
      );
  ...
  private Pair<Integer, Integer> find(
    SolrFacetProperties facet1,
    SolrFacetProperties facet2
  ) {
    return new Pair<>(
      sortedIDs.indexOf(facet1.getFilterID()),

...
/**
 * Extends the MyBatis-Spring support by allowing a choice of {@link
 * org.springframework.core.io.ResourceLoader}. The {@link
 * #setResourceLoader(HierarchicalResourceLoader) ResourceLoader} will be used to load the
 * <b>SqlMapConfig</b> file and use a {@link HierarchicalXMLConfigBuilder} to read the individual
 * MyBatis (3.x) resources.
 *
 * <p>Pending a better way to extend/override, much of the implementation is a direct copy of the
 * MyBatis-Spring {@link SqlSessionFactoryBean}; some of the <tt>protected</tt> methods do not have
 * access to the object's state and can therefore not be overridden successfully.
 *
 * <p>This is equivalent to HierarchicalSqlMapClientFactoryBean which extended iBatis (2.x). See
 * also: <a href=https://issues.apache.org/jira/browse/IBATIS-589>IBATIS-589</a> and: <a
 * href=http://code.google.com/p/mybatis/issues/detail?id=21</a>
 *
...
public class HierarchicalSqlSessionFactoryBean extends SqlSessionFactoryBean {
    ...
    private SqlSessionFactoryBuilder sqlSessionFactoryBuilder =
            new AlfrescoSqlSessionFactoryBuilder();

    private SqlSessionFactory sqlSessionFactory;

    private String environment =
            SqlSessionFactoryBean.class.getSimpleName(); // EnvironmentAware requires spring 3.1
    ...
    /**
     * Set the resource loader to use. To use the <b>&#35;resource.dialect&#35</b> placeholder, use
     * the {@link HierarchicalResourceLoader}.
     *
    ...
    /**
     * If true, a final check is done on Configuration to assure that all mapped statements are
     * fully loaded and there is no one still pending to resolve includes. Defaults to false.
     *
     * @since 1.0.1
     ...
     *
     * <p>This is an alternative to specifying "&lt;sqlmapper&gt;" entries in an MyBatis config
     * file. This property being based on Spring's resource abstraction also allows for specifying
     ...
     * DataSource} should match the one used by the {@code SqlSessionFactory}: for example, you
     ...
     * transactions for, not a {@code TransactionAwareDataSourceProxy}. Only data access code may
     * work with {@code TransactionAwareDataSourceProxy}, while the transaction manager needs to
     * work on the underlying target {@code DataSource}. If there's nevertheless a {@code
     ...
     *
     * <p>This is mainly meant for testing so that mock SqlSessionFactory classes can be injected.
     * By default, {@code SqlSessionFactoryBuilder} creates {@code DefaultSqlSessionFactory}
     * instances.
     */
     ...
     *
     * <p>The default {@code SpringManagedTransactionFactory} should be appropriate for all cases:
    ...
    /**
     * <b>NOTE:</b> This class <em>overrides</em> any {@code Environment} you have set in the
     * MyBatis config file. This is used only as a placeholder name. The default value is {@code
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        if (sqlSessionFactoryBuilder instanceof AlfrescoSqlSessionFactoryBuilder) {
            ((AlfrescoSqlSessionFactoryBuilder) sqlSessionFactoryBuilder)
                    .setDbMetricsReporter(this.dbMetricsReporter);
        }
     ...
     *
     * <p>The default implementation uses the standard MyBatis {@code XMLConfigBuilder} API to build
     * a {@code SqlSessionFactory} instance based on an Reader.
     *
    ...
    protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
        ...
        if (this.configLocation != null) {
            try {
                xmlConfigBuilder =
                        new HierarchicalXMLConfigBuilder(
                                resourceLoader,
        ...
        */
        ...
        if (!isEmpty(this.mapperLocations)) {
            ...
            if (this.logger.isDebugEnabled()) {
                this.logger.debug(
                        "Property 'mapperLocations' was not specified, only MyBatis mapper files"
                                + " specified in the config xml were loaded");
            }
        }

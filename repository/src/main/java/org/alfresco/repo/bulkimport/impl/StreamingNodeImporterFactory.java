...
public class StreamingNodeImporterFactory extends AbstractNodeImporterFactory {
    ...
    private static class StreamingNodeImporter extends AbstractNodeImporter {
        ...
        protected final void importContentAndMetadata(
                NodeRef nodeRef,
                ...
                try {
                    ...
                    throw new ContentIOException(
                            "Failed to copy content from file: \n"
                                    ...
                                    + "\n"
                                    + "   file: "
                                    + contentAndMetadata.getContentFile(),
                            e);
                }
            } else {
                if (logger.isDebugEnabled())
                    logger.debug(
                            "No content to stream into node '"
                                    + nodeRef.toString()
        ...
        protected NodeRef importImportableItemImpl(
            ...
            if (target == null) {
                ...
                throw new AlfrescoRuntimeException(
                        "Bulk importer: target is not known for importable item: "
                                + importableItem.getParent());
            }
            NodeRef result = null;
            MetadataLoader.Metadata metadata = loadMetadata(importableItem.getHeadRevision());

            // TODO: we'll get NodeState.REPLACED back from this method (i.e. the node WILL be
            // replaced)
            // even if we're using ExistingFileMode.ADD_VERSION - we need to do this currently,
            // otherwise
            // the file would be SKIPPED and various other checks that are only computed if replace
            // is being used,
            // wouldn't happen.
            // TODO: sort this out.
            Triple<NodeRef, Boolean, NodeState> node =
                    createOrFindNode(target, importableItem, existingFileMode, metadata);
            boolean isDirectory =
                    node.getSecond() == null
                            ? false
                            : node.getSecond(); // Watch out for NPEs during unboxing!
            ...
            if (result != null && nodeState != NodeState.SKIPPED) {
                ...
                if (isDirectory) {
                    ...
                    numVersionProperties =
                            importImportableItemFile(
                                    result, importableItem, metadata, nodeState, existingFileMode);
                }

                importStatus.incrementNodesWritten(
                        importableItem,
                        isDirectory,
                        nodeState,
                        metadata.getProperties().size() + 4,
                        numVersionProperties);

 ...
 *
 * <p>This means any node service can do xpath style navigation. Given any context node we can
 * navigate between nodes using xpath.
 *
 * <p>This allows simple path navigation and much more.
 *
 * @author Andy Hind
 */
public class DocumentNavigator extends DefaultNavigator implements NamedAccessNavigator {
    ...
    /**
     * @param dictionaryService used to resolve the <b>subtypeOf</b> function and other type-related
     *     functions
     * @param nodeService the <tt>NodeService</tt> against which to execute
     * @param searchService the service that helps resolve functions such as <b>like</b> and
     *     <b>contains</b>
     * @param nspr resolves namespaces in the xpath
     * @param followAllParentLinks true if the XPath should traverse all parent associations when
     *     going up the hierarchy; false if the only the primary parent-child association should be
     *     traversed
     */
    public DocumentNavigator(
            DictionaryService dictionaryService,
    ...
    public Iterator getChildAxisIterator(
        ...
        // MNT-10730
        if (localName != null
                && (localName.equalsIgnoreCase("true") || localName.equalsIgnoreCase("false"))) {
            return Collections.singletonList(new Boolean(Boolean.parseBoolean(localName)))
                    .iterator();
        }
    ...
    private static final UnsupportedAxisException EXCEPTION_NOT_SUPPORTED =
            new UnsupportedAxisException("");
        ...
        if (o instanceof ChildAssociationRef) {
            ...
            if (contextRef.getParentRef() != null) {
                if (followAllParentLinks) {
                    for (ChildAssociationRef car :
                            nodeService.getParentAssocs(contextRef.getChildRef())) {

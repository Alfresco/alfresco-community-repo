...
public class DeleteNotExistsExecutorTest {
    ...
    private DeleteNotExistsExecutor createDeleteNotExistsExecutor(
            Connection connection, String sql, int line, File scriptFile, Properties properties) {
        if (dialect instanceof MySQLInnoDBDialect) {
            return new MySQLDeleteNotExistsExecutor(
                    connection, sql, line, scriptFile, properties, dataSource);
        }
    ...
    public void testDefaultBehaviour() throws Exception {
        scriptExecutor.executeScriptUrl(
                "scriptexec/${db.script.dialect}/delete-not-exists/test-data1.sql");

        String sql =
                "--DELETE_NOT_EXISTS"
                    + " temp_tst_tbl_1.id,temp_tst_tbl_2.tbl_2_id,temp_tst_tbl_3.tbl_3_id,temp_tst_tbl_4.tbl_4_id"
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            // Test read only
            {
                when(properties.getProperty(DeleteNotExistsExecutor.PROPERTY_READ_ONLY))
                        .thenReturn("true");
                when(properties.getProperty(DeleteNotExistsExecutor.PROPERTY_TIMEOUT_SECONDS))
                        .thenReturn("-1");
                DeleteNotExistsExecutor deleteNotExistsExecutor =
                        createDeleteNotExistsExecutor(
                                connection, sql, line, scriptFile, properties);
    ...
    public void testDeleteBatch() throws Exception {
        ...
        try (Connection connection = dataSource.getConnection()) {
            connection.setAutoCommit(true);
            {
                when(properties.getProperty(DeleteNotExistsExecutor.PROPERTY_DELETE_BATCH_SIZE))
                        .thenReturn("1");
                when(properties.getProperty(DeleteNotExistsExecutor.PROPERTY_READ_ONLY))
                        .thenReturn("false");
                DeleteNotExistsExecutor deleteNotExistsExecutor =
    ...
    public void testBatchExecute() throws Exception {
        ...
        try (Connection connection = dataSource.getConnection()) {
            connection.setAutoCommit(true);
            {
                when(properties.getProperty(DeleteNotExistsExecutor.PROPERTY_BATCH_SIZE))
                        .thenReturn("2");
                when(properties.getProperty(DeleteNotExistsExecutor.PROPERTY_READ_ONLY))
                        .thenReturn("false");
                when(properties.getProperty(DeleteNotExistsExecutor.PROPERTY_TIMEOUT_SECONDS))

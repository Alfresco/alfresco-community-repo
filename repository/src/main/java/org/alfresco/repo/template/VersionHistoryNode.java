...
public class VersionHistoryNode
  ...
  public VersionHistoryNode(
    ...
    if (version == null) {
      throw new IllegalArgumentException(
        "Version history descriptor is mandatory."
      );
    }
    if (parent == null) {
  ...
  public QName getType() {
    return parent.services
      .getNodeService()
      .getType(this.version.getFrozenStateNodeRef());
  }
  ...
  public Map<String, Serializable> getProperties() {
    if (propsRetrieved == false) {
      Map<QName, Serializable> props = parent.services
        .getNodeService()
        .getProperties(this.version.getFrozenStateNodeRef());

      for (QName qname : props.keySet()) {
        Serializable propValue =
          parent.new TemplatePropertyConverter().convertProperty(
              props.get(qname),
              qname,
              parent.services,
  ...
  public boolean hasAspect(String aspect) {
    ...
    if (aspect.startsWith(parent.NAMESPACE_BEGIN)) {
      ...
      for (QName qname : this.aspects) {
        if (
          qname
            .toPrefixString(parent.services.getNamespaceService())
            .equals(aspect)
        ) {
          found = true;
          break;

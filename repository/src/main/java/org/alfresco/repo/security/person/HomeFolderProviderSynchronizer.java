...
public class HomeFolderProviderSynchronizer extends AbstractLifecycleBean {
  ...
  private static final Log logger = LogFactory.getLog(
    HomeFolderProviderSynchronizer.class
  );
  private static final Log batchLogger = LogFactory.getLog(
    HomeFolderProviderSynchronizer.class + ".batch"
  );

  private static final String GUEST_HOME_FOLDER_PROVIDER =
    "guestHomeFolderProvider";
  private static final String BOOTSTRAP_HOME_FOLDER_PROVIDER =
  ...
  public void setOverrideHomeFolderProviderName(
    String overrideHomeFolderProviderName
  ) {
  ...
  private void scanPeople(
    ...
    RunAsWorker[] workers = new RunAsWorker[] {
      ...
      new RunAsWorker(
        systemUserName,
        tenantDomain,
        moveFolderThatClashesPhaseName
      ) {
        @Override
        public void doWork(NodeRef person) throws Exception {
          moveHomeFolderThatClashesWithParentFolderStructure(
   ...
   */
  private void moveHomeFolderIfRequired(
    NodeRef person,
    String overrideProviderName
  ) {
    new HomeFolderHandler(person, overrideProviderName) {
      ...
      protected void handleInPreferredLocation() {
        if (logger.isInfoEnabled()) {
          logger.info(
            "     # " +
            toPath(actualPath) +
            " is already in preferred location."
          );
        }
      }
      ...
      protected void handleRootOrAbove() {
        if (logger.isInfoEnabled()) {
          logger.info(
            "     # " +
            userName +
            " has a home folder that is the provider's root directory (or is above it). " +
      ...
      protected void handleHomeFolderNotSet() {
        if (logger.isInfoEnabled()) {
          logger.info(
            "     # " + userName + " Home folder is not set - ignored"
          );
        }
      }
    }
  ...
  private void moveHomeFolder(
    ...
    try {
      ...
      NodeRef oldParent = nodeService
        .getPrimaryParent(homeFolder)
        .getParentRef();
      ...
      homeFolder =
        fileFolderService
          .move(homeFolder, newParent, homeFolderName)
          .getNodeRef();
  ...
  private class WorkProvider implements BatchProcessWorkProvider<NodeRef> {
    ...
    private final VmShutdownListener vmShutdownLister = new VmShutdownListener(
      "getHomeFolderProviderSynchronizerWorkProvider"
    );
          ...
          while (results.size() < BATCH_SIZE && iterator.hasNext()) {
            ...
            try {
              ...
              if (logger.isTraceEnabled()) {
                logger.trace(
                  "The user " + userName + " no longer exists - ignored."
                );
              }
            }
          }
  ...
  private abstract class RunAsWorker
    extends BatchProcessWorkerAdaptor<NodeRef> {
  ...
  private abstract class HomeFolderHandler {
    ...
    public void doWork() {
      ...
      if (homeFolder != null) {
        ...
        if (
          !BOOTSTRAP_HOME_FOLDER_PROVIDER.equals(originalProviderName) &&
          ...
          if (provider != null) {
            ...
            if (preferredPath == null || preferredPath.isEmpty()) {
              ...
              originalRoot = null;
              HomeFolderProvider2 originalProvider = homeFolderManager.getHomeFolderProvider2(
                originalProviderName
              );
              ...
              if (originalProvider != null) {
                ...
                originalPreferredPath =
                  originalProvider.getHomeFolderPath(person);
              }
      ...
      public void add(List<String> path) {
        if (!includesRoot) {
          ...
          if (parentSize == 0) {
            ...
            if (logger.isInfoEnabled()) {
              logger.info(
                "   # Recorded root as parent - no need to record other parents as all home folders will clash"
              );
            }
          } else {
            add(path, 0);

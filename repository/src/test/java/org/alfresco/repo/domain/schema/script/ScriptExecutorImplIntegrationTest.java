...
public class ScriptExecutorImplIntegrationTest {
    ...
    /**
     * Check that a script designed to be run for all varieties of DBMS (i.e. in subdirectory
     * org.hibernate.dialect.Dialect) will run regardless of specific dialect (e.g. MySQL or
     * PostgreSQL)
     *
     * @throws Exception
     */
    ...
    /**
     * Test the case of executing a specific (e.g. PostgreSQL) database script when no general
     * script is present (therefore no overriding mechanism is required).
     *
     * @throws Exception
     */
    @Test
    public void canExecuteSpecificDialectScript() throws Exception {
        ...
        if (dialect.getClass().equals(MySQLInnoDBDialect.class)) {
            ...
            log.warn(
                    "No suitable dialect-specific DB script for test"
                            + " canExecuteSpecificDialectScript()");
        }
    }

    /**
     * Test the case of executing a specific database script (e.g. PostgreSQL) when a more generic
     * script also exists -- the more generic script is not run.
     *
     * @throws Exception
     */
    @Test
    public void canExecuteSpecificDialectOverridingGenericScript() throws Exception {
        ...
        if (dialect.getClass().equals(MySQLInnoDBDialect.class)) {
            ...
            log.warn(
                    "No suitable dialect-specific DB script for test"
                            + " canExecuteSpecificDialectOverridingGenericScript()");
        }
    }

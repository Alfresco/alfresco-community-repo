...
public class DiscussableAspect
  ...
  private static final String KEY_WORKING_COPIES =
    DiscussableAspect.class.getName() + ".WorkingCopies";
  ...
  private static class DiscussableAspectCopyBehaviourCallback
    ...
    public boolean getMustCopy(QName classQName, CopyDetails copyDetails) {
      if (
        ...
        Set<NodeRef> nodeRefs = TransactionalResourceHelper.getSet(
          KEY_WORKING_COPIES
        );
  ...
  public void onAddAspect(NodeRef discussableNodeRef, QName aspectTypeQName) {
    ...
    String forumName = I18NUtil.getMessage(
      "discussion.discussion_for",
      new Object[] { name }
    );
  ...
  public void onCopyComplete(
    ...
    Set<NodeRef> workingCopyNodeRefs = TransactionalResourceHelper.getSet(
      KEY_WORKING_COPIES
    );
    ...
    for (ChildAssociationRef topicAssocRef : topicAssocRefs) {
      ...
      try {
        // work out the name for the copied topic
        String topicName;
        String topicNodeName = nodeService
          .getProperty(topicNodeRef, ContentModel.PROP_NAME)
          .toString();
        Serializable labelProp = nodeService.getProperty(
          targetNodeRef,
          ContentModel.PROP_VERSION_LABEL
        );
        if (labelProp == null) {
        ...
        if (
          fileFolderService.searchSimple(targetForumNodeRef, topicName) != null
        ) {
        ...
        throw new ConcurrencyFailureException(
          "Target topic exists: " + e.getMessage(),
          e
        );
      } catch (FileNotFoundException e) {
        // The node was there, but now it's gone
        throw new ConcurrencyFailureException(
          "Forum was deleted: " + e.getMessage(),
          e
        );
      }
    }
  ...
  public void afterVersionRevert(NodeRef nodeRef, Version version) {
    ...
    List<ChildAssociationRef> childAssocRefs =
      this.nodeService.getChildAssocs(
          VersionUtil.convertNodeRef(versionNodeRef),
          ForumModel.ASSOC_DISCUSSION,
          RegexQNamePattern.MATCH_ALL
        );
    for (ChildAssociationRef childAssocRef : childAssocRefs) {
      ...
      if (
        referencedNode != null &&
        this.nodeService.exists(referencedNode) == false
      ) {
        StoreRef orginalStoreRef = referencedNode.getStoreRef();
        NodeRef archiveRootNodeRef =
          this.nodeService.getStoreArchiveNode(orginalStoreRef);
        ...
        if (
          !this.nodeService.exists(archivedNodeRef) ||

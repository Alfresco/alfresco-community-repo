...
/**
 * Manages persistence and retrieval of Descriptors whose meta-data are retrieved from the
 * repository stores.
 *
 * @author dward
 */
public class RepositoryDescriptorDAOImpl implements DescriptorDAO {
     ...
     *
     * @param systemBootstrap the new system bootstrap
     */
        ...
        try {
            ...
            if (descriptorNodeRef != null) {
                final Map<QName, Serializable> properties =
                        this.nodeService.getProperties(descriptorNodeRef);
    ...
    public Descriptor updateDescriptor(final Descriptor serverDescriptor, LicenseMode licenseMode) {
        ...
        try {
            ...
            if (!this.transactionService.isReadOnly()) {
                ...
                props.put(
                        ContentModel.PROP_SYS_VERSION_REVISION,
                ...
                // In order to migrate properly, this property needs to be d:content.  We will
                // rewrite the property with the
                // license update code.  There is no point attempting to rewrite the property here.
                final Serializable value =
                        this.nodeService.getProperty(
                                currentDescriptorNodeRef, ContentModel.PROP_SYS_VERSION_EDITION);
                if (value == null) {
                ...
                if (RepositoryDescriptorDAOImpl.logger.isDebugEnabled()) {
                    RepositoryDescriptorDAOImpl.logger.debug(
                            "Updated current repository descriptor properties: \n"
                                    + "   node: "
                                    + currentDescriptorNodeRef
                                    + "\n"
                                    + "   descriptor: "
            ...
            final Map<QName, Serializable> properties =
                    this.nodeService.getProperties(currentDescriptorNodeRef);
            descriptor = new RepositoryDescriptor(properties);
        } catch (final RuntimeException e) {
            if (logger.isErrorEnabled()) {
    ...
    public byte[] getLicenseKey() {
        ...
        try {
            ...
            final ContentReader reader =
                    this.contentService.getReader(
                            descriptorRef, ContentModel.PROP_SYS_VERSION_EDITION);
            ...
            if (exists) {
                ...
                try {
                    ...
                    if (os != null) {
                        try {
                            ...
                            logger.debug(
                                    "getLicenseKey: Error closing ByteArrayOutputStream", ignore);
                        }
                    }
                }
            } else {
                if (logger.isDebugEnabled()) {
                    ...
                    logger.debug(
                            "getLicenseKey: reader "
                                    + (reader == null
                                            ? "is null"
                                            : " file does " + (exists ? "" : "NOT ") + "exist"));
                }
            }
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "getLicenseKey: key " + (key == null ? "is null" : "length=" + key.length));
        }
        return key;
    }
    ...
    public void updateLicenseKey(final byte[] key) {
        try {
            ...
            if (key == null) {
                this.nodeService.setProperty(
                        descriptorRef, ContentModel.PROP_SYS_VERSION_EDITION, null);
            } else {
                final ContentWriter writer =
                        this.contentService.getWriter(
                                descriptorRef, ContentModel.PROP_SYS_VERSION_EDITION, true);
    ...
    private NodeRef getDescriptorNodeRef(final boolean create) {
        ...
        if (this.nodeService.exists(storeRef)) {
            ...
            final List<NodeRef> nodeRefs =
                    this.searchService.selectNodes(
                            rootNodeRef, searchPath, null, this.namespaceService, false);
            if (nodeRefs.size() == 1) {
                ...
                descriptorNodeRef =
                        this.nodeService
                                .createNode(
                                        ...
                                        QName.createQName("sys:descriptor", this.namespaceService))
                                .getChildRef();
                if (RepositoryDescriptorDAOImpl.logger.isDebugEnabled()) {
                    RepositoryDescriptorDAOImpl.logger.debug(
                            "Created missing descriptor node: " + descriptorNodeRef);
                }
            }
        }
    ...
    private class RepositoryDescriptor extends BaseDescriptor {
        ...
        public String getDescriptor(final String key) {
            ...
            final QName qname =
                    QName.createQName(key, RepositoryDescriptorDAOImpl.this.namespaceService);
        ...
        public LicenseMode getLicenseMode() {
            ...
            return licenseModeStr == null
                    ? LicenseMode.UNKNOWN
                    : LicenseMode.valueOf(licenseModeStr);
        }
    }
}

...
public class MigrateTenantsFromAttrsToTablePatch extends AbstractPatch {
  ...
  private static final String MSG_SUCCESS =
    "patch.migrateTenantsFromAttrsToTable.result";
      ...
      new AttributeQueryCallback() {
        public boolean handleAttribute(
          ...
          if ((keys.length >= 2) && (value != null)) {
            ...
            Boolean isEnabled = (Boolean) valueMap.get(
              MultiTAdminServiceImpl.TENANT_ATTRIBUTE_ENABLED
            );
            String rootDir = (String) valueMap.get(
              MultiTAdminServiceImpl.TENANT_ATTRIBUTE_ROOT_CONTENT_STORE_DIR
            );

            TenantEntity tenantEntity = new TenantEntity(
              new String(tenantDomain)
            );
            tenantEntity.setEnabled((isEnabled != null ? isEnabled : false));
            tenantEntity.setContentRoot(new String(rootDir));

            TenantEntity tenantEntityCreated = tenantAdminDAO.createTenant(
              tenantEntity
            );
    ...
    if (tenantCount > 0) {
      ...
      if (checkTenants.size() > 0) {
        throw new AlfrescoRuntimeException(
          "Incomplete migration of tenant attributes to tenant table: " +
          checkTenants.size()
        );
      }

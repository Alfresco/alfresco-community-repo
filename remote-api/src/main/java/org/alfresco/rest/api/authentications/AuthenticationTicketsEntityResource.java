...
public class AuthenticationTicketsEntityResource
  implements
    EntityResourceAction.Create<LoginTicket>,
  ...
  @WebApiNoAuth
  @Override
  public List<LoginTicket> create(
    List<LoginTicket> entity,
    Parameters parameters
  ) {
    if (entity == null || entity.size() != 1) {
      throw new InvalidArgumentException(
        "Please specify one login request only."
      );
    }
    LoginTicket result = authentications.createTicket(
      entity.get(0),
      parameters
    );
  ...
  @WebApiDescription(
    title = "Validate login ticket",
    description = "Validate login ticket."
  )
  @Override
  public LoginTicket readById(
    String me,
    Parameters parameters,

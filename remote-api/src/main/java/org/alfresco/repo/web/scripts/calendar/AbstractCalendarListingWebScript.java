...
public abstract class AbstractCalendarListingWebScript
  ...
  protected static Comparator<Map<String, Object>> getEventDetailsSorter() {
    return new Comparator<Map<String, Object>>() {
      public int compare(
        ...
        DateTimeFormatter fmtNoTz = DateTimeFormat.forPattern(
          "yyyy-MM-dd'T'HH:mm:ss.SSS"
        );
        DateTimeFormatter fmtTz = DateTimeFormat.forPattern(
        ...
        DateTime sa = startA.length() > 23
          ? fmtTz.parseDateTime(startA)
          : fmtNoTz.parseDateTime(startA);
        DateTime sb = startB.length() > 23
          ? fmtTz.parseDateTime(startB)
          : fmtNoTz.parseDateTime(startB);
        ...
        if (cmp == 0) {
          ...
          DateTime ea = endA.length() > 23
            ? fmtTz.parseDateTime(endA)
            : fmtNoTz.parseDateTime(endA);
          DateTime eb = endB.length() > 23
            ? fmtTz.parseDateTime(endB)
            : fmtNoTz.parseDateTime(endB);
   ...
   */
  protected boolean handleRecurring(
    CalendarEntry entry,
    ...
    List<ChildAssociationRef> ignoreEventList = nodeService.getChildAssocs(
      entry.getNodeRef(),
      childNodeTypeQNames
    );
    ...
    for (ChildAssociationRef ignoreEvent : ignoreEventList) {
      ...
      Date ignoredDate = (Date) nodeService.getProperty(
        nodeRef,
        CalendarModel.PROP_IGNORE_EVENT_DATE
      );
    ...
    if (entry.getStart().getTime() >= from.getTime()) {
      if (
        dates.size() == 0 ||
        dates.get(0).getTime() != entry.getStart().getTime()
      ) {
    ...
    List<ChildAssociationRef> updatedEventList = nodeService.getChildAssocs(
      entry.getNodeRef(),
      childNodeTypeQNames
    );
    ...
    for (ChildAssociationRef updatedEvent : updatedEventList) {
      ...
      Date updatedDate = (Date) nodeService.getProperty(
        nodeRef,
        CalendarModel.PROP_UPDATED_EVENT_DATE
      );
      Date newStart = (Date) nodeService.getProperty(
        nodeRef,
        CalendarModel.PROP_UPDATED_START
      );
      Date newEnd = (Date) nodeService.getProperty(
        nodeRef,
        CalendarModel.PROP_UPDATED_END
      );
      String newWhere = (String) nodeService.getProperty(
        nodeRef,
        CalendarModel.PROP_UPDATED_WHERE
      );
      String newWhat = (String) nodeService.getProperty(
        nodeRef,
        CalendarModel.PROP_UPDATED_WHAT
      );
      updatedDates.put(
        fmt.format(updatedDate),
  ...
  private void updateRepeatingStartEnd(
    Date newStart,
    long duration,
  ...
  private void updateRepeating(
    CalendarEntry entry,
    Map<String, Object> updatedDates,
    Map<String, Object> entryResult,
    long duration,
    ...
    if (updatedDates.keySet().contains(fmt.format(date))) {
      ...
      entryResult.put(
        RESULT_TITLE,
        (String) updatedDates.get(fmt.format(date).toString() + "what")
      );
      entryResult.put(
        "where",
        (String) updatedDates.get(fmt.format(date).toString() + "where")
      );

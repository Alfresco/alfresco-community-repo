    ...
    public void testSiteMembers() throws Exception {
...

        {
                    ...
                    TenantUtil.runAsUserTenant(
                            new TenantRunAsWork<TestSite>() {
                                ...
                                public TestSite doWork() throws Exception {
                                    TestSite testSite =
                                            testNetwork.createSite(SiteVisibility.PRIVATE);
                                    for (int i = 1; i <= 5; i++) {
            ...
            // Test Case cloud-1482
            {
                ...
                publicApiClient.setRequestContext(
                        new RequestContext(testNetwork.getId(), personId));
...

            {
                ...
                HttpResponse response =
                        sitesProxy.getAll(
                                ...
                                null,
                                createParams(
                                        paging, Collections.singletonMap("includeSource", "true")),
                                "Failed to get all site members");
                checkList(
                        expectedSiteMembers.subList(
                                skipCount, skipCount + paging.getExpectedPaging().getCount()),
                        paging.getExpectedPaging(),
                        SiteMember.parseSiteMembers(
                                testSite.getSiteId(), response.getJsonResponse()));
            ...
            // GET single site member
            {
                ...
                publicApiClient.setRequestContext(
                        new RequestContext(testNetwork.getId(), personId));
                SiteMember ret =
                        sitesProxy.getSingleSiteMember(
                                testSite.getSiteId(), siteMember1.getMemberId());
                siteMember1.expected(ret);
            }
        }

        // test: user is member of different tenant, but has site membership(s) in common with the
        // http request user
        {
            ...
            try {
                publicApiClient.setRequestContext(
                        new RequestContext(network1.getId(), person2.getId()));
                sitesProxy.createSiteMember(
                        site.getSiteId(), new SiteMember(person1.getId(), "dodgyRole"));
                fail("");
            } catch (PublicApiException e) {
            ...
            try {
                publicApiClient.setRequestContext(
                        new RequestContext(network1.getId(), person1.getId()));
                sitesProxy.createSiteMember(
                        site.getSiteId(),
            ...
            try {
                publicApiClient.setRequestContext(
                        new RequestContext(network1.getId(), person4.getId()));
                sitesProxy.createSiteMember(
                        site.getSiteId(),
                        new SiteMember(person1.getId(), SiteRole.SiteContributor.toString()));
                fail("");
                ...
                assertEquals(
                        ...
                        e.getHttpResponse()
                                .getStatusCode()); // TODO check that 404 is correct here - external
                // user of network can't see public site??
            }
            ...
            try {
                ...
                sitesProxy.createSiteMember(
                        site.getSiteId(),
                        new SiteMember(null, SiteRole.SiteContributor.toString()));
                fail("");
            } catch (PublicApiException e) {
                assertEquals(HttpStatus.SC_BAD_REQUEST, e.getHttpResponse().getStatusCode());
            }
            ...
            try {
                publicApiClient.setRequestContext(
                        new RequestContext(network1.getId(), person2.getId()));
                sitesProxy.createSiteMember(
                        site.getSiteId(), new SiteMember(person1.getId(), null));
                fail("");
            } catch (PublicApiException e) {
                assertEquals(HttpStatus.SC_BAD_REQUEST, e.getHttpResponse().getStatusCode());
            }
        ...
        // test: create site membership, remove it, get list of site memberships
        {
            ...
            // for -me- user (PUBLICAPI-90)
            {
                ...
                SiteMember siteMember =
                        sitesProxy.createSiteMember(
                                site.getSiteId(),
                                new SiteMember(
                                        person1.getId(), SiteRole.SiteContributor.toString()));
                assertEquals(person1.getId(), siteMember.getMemberId());
...

            {
                ...
                assertEquals(person1.getId(), siteMember.getMemberId());
                assertEquals(SiteRole.SiteContributor.toString(), siteMember.getRole());
                siteMember.setSiteId(
                        site.getSiteId()); // note: needed for contains check below, ugh

                // create another site member
                publicApiClient.setRequestContext(
                        new RequestContext(network1.getId(), person2.getId()));
                SiteMember siteMemberAno =
                        sitesProxy.createSiteMember(
                                site.getSiteId(),
                                new SiteMember(
                                        person3.getId(), SiteRole.SiteCollaborator.toString()));
                ...
                // ACE-5444
                // cannot update site member (without appropriate site "permission" - see
                // SiteService)
                try {
                    publicApiClient.setRequestContext(
                            new RequestContext(network1.getId(), person1.getId()));
                    sitesProxy.updateSiteMember(
                            site.getSiteId(),
                            new SiteMember(person1.getId(), SiteRole.SiteCollaborator.toString()));
                    fail();
                } catch (PublicApiException e) {
                    assertEquals(
                ...
                // ACE-5444
                // cannot remove another site member (without appropriate site "permission" - see
                // SiteService)
                try {
                    publicApiClient.setRequestContext(
                            new RequestContext(network1.getId(), person1.getId()));
                ...
                try {
                    ...
                    sitesProxy.updateSiteMember(
                            site.getSiteId(),
                            new SiteMember(person2.getId(), SiteRole.SiteContributor.toString()));
                    fail();
                } catch (PublicApiException e) {
                    assertEquals(
                            HttpStatus.SC_UNPROCESSABLE_ENTITY,
                ...
                // successful update
                {
                    ...
                    assertEquals(
                            sm.getRole(), smToCheck.getRole()); // check that the role is correct

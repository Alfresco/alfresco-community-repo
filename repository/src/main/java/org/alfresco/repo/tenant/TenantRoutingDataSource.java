...
public class TenantRoutingDataSource extends AbstractRoutingDataSource {
  ...
  public void afterPropertiesSet() {
    ...
    try {
      // default tenant
      DataSource defaultTargetDataSource = new TenantBasicDataSource(
        baseDataSource,
        dbUrl,
        -1
      );
  ...
  public synchronized void addTenantDataSource(
    String tenantDomain,
    String dbUrl
  ) throws SQLException {
    ...
    if (!TenantService.DEFAULT_DOMAIN.equals(currentTenantDomain)) {
      throw new RuntimeException(
        "Unexpected - should not be in context of a tenant [" +
        currentTenantDomain +
        "]"
      );
    }
    ...
    tenantDataSources.put(
      tenantDomain,
      new TenantBasicDataSource(baseDataSource, dbUrl, -1)
    );

    super.afterPropertiesSet(); // to update resolved data sources
  }

  public synchronized void removeTenantDataSource(String tenantDomain)
    throws SQLException {
    String currentTenantDomain = tenantService.getCurrentUserDomain();
    if (!TenantService.DEFAULT_DOMAIN.equals(currentTenantDomain)) {
      throw new RuntimeException(

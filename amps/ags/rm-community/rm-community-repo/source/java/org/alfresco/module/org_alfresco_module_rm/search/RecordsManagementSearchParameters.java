...
public class RecordsManagementSearchParameters {
  ...
  public static RecordsManagementSearchParameters createFromJSON(
    ...
    try {
      ...
      return RecordsManagementSearchParameters.createFromJSON(
        jsonObject,
        namespaceService
      );
  ...
  public static RecordsManagementSearchParameters createFromJSON(
    ...
    try {
      ...
      if (jsonObject.has(JSON_VITALRECORDS)) {
        searchParameters.setIncludeVitalRecords(
          jsonObject.getBoolean(JSON_VITALRECORDS)
        );
      }
      ...
      if (jsonObject.has(JSON_SORT)) {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
          ...
          if (
            sortJSONObject.has(JSON_FIELD) && sortJSONObject.has(JSON_ASCENDING)
          ) {
            sortOrder.add(
              new SortItem(
                QName.createQName(
  ...
  public JSONObject toJSONObject(NamespaceService namespaceService) {
    try {
      ...
      throw new AlfrescoRuntimeException(
        "Unable to generate json string for records management search parameters.",
        e
      );
    }
  }

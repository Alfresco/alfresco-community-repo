...
/**
 * The post lookup component is responsible for updating posts that require a secondary lookup (to
...
public class PostLookup {
    ...
    private static VmShutdownListener vmShutdownListener =
            new VmShutdownListener(PostLookup.class.getName());

    /**
     * The name of the lock used to ensure that post lookup does not run on more than one node at
     * the same time
     */
    private static final QName LOCK_QNAME =
            QName.createQName(NamespaceService.SYSTEM_MODEL_1_0_URI, "ActivityPostLookup");
    ...
    public void execute() throws JobExecutionException {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Selecting activity posts with status: "
                                + ActivityPostEntity.STATUS.PENDING.toString());
            }

            // get all pending post (for this job run)
            final List<ActivityPostEntity> activityPosts =
                    postDAO.selectPosts(params, maxItemsPerCycle);
            ...
            if (activityPosts.size() > 0) {
                ...
                List<ActivityPostEntity> activityPostsToUpdate =
                        transactionService
                                .getRetryingTransactionHelper()
                                .doInTransaction(
                                        new RetryingTransactionCallback<
                                                List<ActivityPostEntity>>() {
                                            public List<ActivityPostEntity> execute()
                                                    throws Throwable {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "execute: Can't get lock. Assume post lookup job already underway: " + e);
            }
    ...
    private class UserRollupActivity {
        ...
        public boolean equals(Object obj) {
            ...
            return this.userId.equals(that.userId)
                    && this.activityType.equals(that.activityType)
                    && this.parentNodeRef.equals(that.parentNodeRef);
        }
    }
    ...
    private List<ActivityPostEntity> lookupPosts(final List<ActivityPostEntity> activityPosts) {
        for (final ActivityPostEntity activityPost : activityPosts) {
            ...
            try {
                ...
                final JSONObject jo =
                        new JSONObject(new JSONTokener(activityPost.getActivityData()));
                            ...
                            public Void doWork() throws Exception {
                                ...
                                if (!jo.isNull(JSON_NODEREF_LOOKUP)) {
                                    ...
                                    if ((jo.isNull(JSON_FIRSTNAME)) || (jo.isNull(JSON_LASTNAME))) {
                                        Pair<String, String> firstLastName =
                                                lookupPerson(postUserId);
                                    ...
                                    if (parentNodeRef == null) {
                                        ...
                                        if (jo.isNull(JSON_PARENT_NODEREF)) {
                                            if (!jo.isNull(JSON_NODEREF)) {
                                                ...
                                                if (parentNodeRef != null) {
                                                    ...
                                                    // note: currently only required during
                                                    // lookup/rollup
                                                    // joLookup = jo;
                                                }
                                            }
                                        ...
                                        if (parentNodeRefStr != null) {
                                            activityPost.setParentNodeRef(
                                                    new NodeRef(parentNodeRefStr));
                                        }
                                    }
                                ...
                                if ((activityPost.getActivityData() != null)
                                        && (activityPost.getActivityData().length()
                                ...
                                if ((activityPost.getSiteNetwork() != null)
                                        && (activityPost.getSiteNetwork().length()
                                                > ActivityPostDAO.MAX_LEN_SITE_ID)) {
                                    // belts-and-braces - should not get here since checked during
                                    // post (and not modified)
                                    throw new IllegalArgumentException(
    ...
    private List<ActivityPostEntity> rollupPosts(List<ActivityPostEntity> activityPosts)
            throws SQLException {
        Map<UserRollupActivity, List<ActivityPostEntity>> rollupPosts =
                new HashMap<UserRollupActivity, List<ActivityPostEntity>>();
        ...
        for (final ActivityPostEntity post : activityPosts) {
            if (rollupTypes.containsKey(post.getActivityType())
                    && (post.getParentNodeRef() != null)) {
                UserRollupActivity key =
                        new UserRollupActivity(
                                post.getUserId(), post.getActivityType(), post.getParentNodeRef());
        ...
        for (final Map.Entry<UserRollupActivity, List<ActivityPostEntity>> entry :
                rollupPosts.entrySet()) {
                ...
                TenantUtil.runAsSystemTenant(
                        new TenantUtil.TenantRunAsWork<Void>() {
                            public Void doWork() throws Exception {
                                ...
                                // rollup - create a new 'posted' event that represents the
                                // rolled-up activity (and set others to 'processed')
                                ...
                                try {
                                    ...
                                    jo.put(
                                            JSON_NODEREF_PARENT,
                                            oldPost.getParentNodeRef().toString());
                                    ...
                                    if (path != null) {
                                        ...
                                        if (displayPath != null) {
                                            // note: PathUtil.getDisplayPath returns prefix path as:
                                            // '/company_home/sites/' rather than /Company
                                            // Home/Sites'
                                            String prefix =
                                                    "/company_home/sites/"
                                                            + tenantService.getBaseName(
                                                                    oldPost.getSiteNetwork())
                                                            + "/documentLibrary";
                                            ...
                                            jo.put(
                                                    JSON_PAGE,
                                                    "documentlibrary?path=" + displayPath);
                                        }
                                    }
                        ...
                        public Object doWork() throws Exception {
                            try {
                                ...
                                ActivityPostEntity.STATUS status =
                                        ActivityPostEntity.STATUS.valueOf(activityPost.getStatus());
                                ...
                                switch (status) {
                                    ...
                                    case PENDING:
                                        postDAO.updatePost(
                                                ...
                                                activityPost.getSiteNetwork(),
                                                activityPost.getActivityData(),
                                                ActivityPostEntity.STATUS.POSTED);
                                        activityPost.setStatus(
                                                ActivityPostEntity.STATUS.POSTED
                                ...
                                postDAO.updatePostStatus(
                                        activityPost.getId(), ActivityPostEntity.STATUS.ERROR);
    ...
    /**
     * Generic node lookup - note: not currently used (see ActivityService.postActivity when
     * activityData is not supplied)
     */
    private JSONObject lookupNode(
            final NodeRef nodeRef, final String postUserId, final JSONObject jo)
        ...
        if ((nodeRef != null) && (nodeService.exists(nodeRef))) {
            ...
            if (typeQName.length() == 0) {
                // lookup type
                typeQName =
                        nodeService.getType(nodeRef).toPrefixString(); // TODO: missing the prefix ?
            }
    ...
    private synchronized String acquireLock(LockCallback lockCallback)

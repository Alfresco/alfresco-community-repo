...
/**
 * <code>QName</code> represents the qualified name of a Repository item. Each QName consists of a
 * local name qualified by a namespace.
 *
 * <p>The {@link org.alfresco.service.namespace.QNamePattern QNamePattern} is implemented to allow
 * instances of this class to be used for direct pattern matching where required on interfaces.
 *
...
public final class QName implements QNamePattern, Serializable, Cloneable, Comparable<QName> {
    ...
    public static QName createQName(
            String prefix, String localName, NamespacePrefixResolver prefixResolver)
    ...
    /**
     * Create a QName from its internal string representation of the following format: <code>
     * {namespaceURI}localName</code>
     *
        ...
        if (namespaceBegin != -1) {
            if (namespaceBegin != 0) {
                throw new InvalidQNameException(
                        "QName '" + qname + "' must start with a namespaceURI");
            }
            ...
            if (namespaceEnd == -1) {
                throw new InvalidQNameException(
                        "QName '"
                                + qname
                                + "' is missing the closing namespace "
                                + NAMESPACE_END
                                + " token");
            }
    ...
    private QName(String namespace, String name, String prefix) {
        this.namespaceURI =
                ((namespace == null) || (namespace.equals(QName.EMPTY_URI_SUBSTITUTE)))
    ...
    public QName getPrefixedQName(NamespacePrefixResolver resolver) {
        ...
        if (prefixes.size() == 0) {
            throw new NamespaceException(
                    "A namespace prefix is not registered for uri " + namespaceURI);
        }
     ...
     *
     * <p>Note: The prefix is ignored during the comparison.
     */
    @Override
    ...
    /**
     * Calculate hashCode. Follows pattern used by String where hashCode is cached (QName is
     * immutable).
     */
    @Override
    public int hashCode() {
    ...
    /**
     * Render string representation of QName using format: <code>{namespace}name</code>
     *
    ...
    /**
     * Uses the {@link #getNamespaceURI() namespace URI} and then the {@link #getLocalName()
     * localname} to do the comparison i.e. the comparison is alphabetical.
     */
    ...
    /**
     * Render string representation of QName using format: <code>prefix:name</code>
     *
     * @return the string representation
     */
    ...
    /**
     * Render string representation of QName using format: <code>prefix:name</code> according to
     * namespace prefix mappings of specified namespace resolver.
     *
    ...
    /**
     * Creates a QName representation for the given String. If the String has no namespace the
     * Alfresco namespace is added. If the String has a prefix an attempt to resolve the prefix to
     * the full URI will be made.
     *
    ...
    /**
     * Creates a string representation of a QName for the given string. If the given string already
     * has a namespace, either a URL or a prefix, nothing the given string is returned. If it does
     * not have a namespace the Alfresco namespace is added.
     *
     * @param str The string to convert

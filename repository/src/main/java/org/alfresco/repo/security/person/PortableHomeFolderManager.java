...
public class PortableHomeFolderManager implements HomeFolderManager {
  ...
  public void makeHomeFolder(final ChildAssociationRef childAssocRef) {
    ...
    if (v2Provider != null) {
      ...
      if (v2Provider instanceof AbstractHomeFolderProvider.V2Adaptor) {
        ((AbstractHomeFolderProvider.V2Adaptor) v2Provider).onCreateNode(
            childAssocRef
          );
      } else {
        homeFolderCreateAndSetPermissions(
          v2Provider,
          childAssocRef.getChildRef()
        );
      }
  ...
  void homeFolderCreateAndSetPermissions(
    ...
    if (homeFolder.getNodeRef() != null) {
      ...
      String owner = (providerSuppliedOwner == null)
        ? uid
        : providerSuppliedOwner;
      // If created..
  ...
  public String getPersonProperty(NodeRef person, QName name) {
    String value = DefaultTypeConverter.INSTANCE.convert(
      String.class,
      nodeService.getProperty(person, name)
    );

    if (value == null || value.length() == 0) {
      throw new PersonException(
        "Can not create a home folder when the " +
        name +
  ...
  public HomeSpaceNodeRef getHomeFolder(
    ...
    if (existingHomeFolder != null) {
      ...
      return new HomeSpaceNodeRef(
        homeFolderNodeRef,
        HomeSpaceNodeRef.Status.CREATED
      );
    }
   ...
   */
  public void modifyHomeFolderNameIfItExists(
    NodeRef root,
    ...
    try {
      do {
        ...
        fileFolderService.resolveNamePath(root, homeFolderPath, false) != null
      );
  ...
  private FileInfo createTree(
    ...
    NodeRef newParent = createNewParentIfRequired(
      root,
      homeFolderPath,
      fileFolderService
    );
    ...
    if (templateNodeRef == null) {
      ...
      try {
        fileInfo =
          fileFolderService.copy(templateNodeRef, newParent, homeFolderName);
      } catch (FileNotFoundException e) {

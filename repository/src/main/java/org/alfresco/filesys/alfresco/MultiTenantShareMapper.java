...
public class MultiTenantShareMapper
    ...
    public void afterPropertiesSet() {
        //  Filesystem configuration will usually be initialized after the security configuration so
        // we need to plug in
        //  a listener to initialize it later

        m_filesysConfig =
                (FilesystemsConfigSection)
                        m_config.getConfigSection(FilesystemsConfigSection.SectionName);

        //  Get the Alfresco configuration section

        m_alfrescoConfig =
                (AlfrescoConfigSection)
                        m_config.getConfigSection(AlfrescoConfigSection.SectionName);

        if (m_filesysConfig == null || m_alfrescoConfig == null) m_config.addListener(this);
    ...
    public SharedDevice findShare(
            String host, String name, int typ, SrvSession sess, boolean create)
        ...
        if (share != null
                ...
                && share.getContext().isAvailable() == false) share = null;
    ...
    private final SharedDeviceList getTenantShareList(
            String host, SrvSession sess, boolean allShares) {
    ...
    private final DiskSharedDevice createTenantShare(String tenantDomain) {
        ...
        NodeRef rootNodeRef =
                new NodeRef(storeRef.getProtocol(), storeRef.getIdentifier(), "dummy");
        ...
        ContentContext diskCtx =
                new ContentContext(m_tenantShareName, getStoreName(), getRootPath(), rootNodeRef);
        ...
        //  Create a temporary shared device for the user to access the tenant company home
        // directory

...
public class OwnableServiceImpl
  ...
  public String getOwner(NodeRef nodeRef) {
    ...
    if (userName == null) {
      ...
      if (isRendition(nodeRef)) {
        userName =
          getOwner(nodeService.getPrimaryParent(nodeRef).getParentRef());
    ...
    AuthenticationUtil.runAs(
      new RunAsWork<Void>() {
        public Void doWork() throws Exception {
          List<ChildAssociationRef> renditions = renditionService.getRenditions(
            nodeRef
          );
   ...
   */
  public CopyBehaviourCallback onCopyNode(
    QName classRef,

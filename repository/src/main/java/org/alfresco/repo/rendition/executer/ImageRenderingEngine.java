...
/**
 * This class is the implementation of the {@link RenditionService}'s "imageRenderingEngine"
 * rendering engine. This action renders a piece of content in the same MIME type as its source
 * node, having been rescaled as requested.
 *
...
public class ImageRenderingEngine extends AbstractTransformationRenderingEngine {
    ...
    // Resize params
    /**
     * This optional {@link Integer} or {@link Float} parameter specifies the width of the image
     * after resizing. This may be expressed as pixels or it may represent a percentage of the
     * original image width, depending on the value of the PARAM_IS_PERCENT_RESIZE parameter. <br>
     * If no value is specified for this parameter then the width of the image will be unchanged.
     * <br>
     * If an image is being cropped and resized then the cropping happens first, followed by
     * resizing of the cropped image.
     */
    public static final String PARAM_RESIZE_WIDTH = "xsize"; // RESIZE_WIDTH

    /**
     * This optional {@link Integer} or {@link Float} parameter specifies the height of the image
     * after resizing. This may be expressed as pixels or it may represent a percentage of the
     * original image width, depending on the value of the PARAM_IS_PERCENT_RESIZE parameter. <br>
    ...
    /**
     * This optional {@link Boolean} flag parameter specifies how the PARAM_RESIZE_HEIGHT and
     * PARAM_RESIZE_WIDTH parameters are interpreted. If this parameter is set to <code>true</code>
     * then the rendition height and width are represented as a percentage of the original image
     * height and width. If this parameter is set to <code>false</code> then the rendition height
     * and width are represented as pixels. This parameter defaults to <code>false</code>.
     */
    public static final String PARAM_IS_PERCENT_RESIZE = "isAbsolute";

    /**
     * This optional {@link Boolean} flag parameter determines whether the rendered image maintains
     * its original aspect ratio or is stretched to fit the specified height and width. <br>
     * If this parameter is <code>true</code> then the rendered image will always maintain its
     * aspect ratio and will be resized to best fit within the given width and height. For example
     * if an image starts at 100x200 pixels and it is resized to 50x50 pixels then the rendered
     * image will actually be 25x50 pixels. <br>
     * If this parameter is <code>false</code> then the image will be stretched or compressed to fit
     * the given height and width, regardless of the original aspect ratio. <br>
     * This parameter defaults to <code>false</code>
     */
    public static final String PARAM_MAINTAIN_ASPECT_RATIO = MAINTAIN_ASPECT_RATIO;

    /**
     * This optional {@link Boolean} flag parameter specifies a mode for dramatically shrinking
     * large images in a performant way.<br>
     * If set to <code>true</code> the rendering process will be more performant for large images
     * but the rendered image will be of lower quality. <br>
     * If set to <code>false</code> the rendering process will take longer but the resulting image
     * will usually be of better quality.
     */
    public static final String PARAM_RESIZE_TO_THUMBNAIL = "resizeToThumbnail";

    /**
     * This optional {@link Boolean} flag parameter specifies whether image resizing should produce
     * an enlarged image, based on the resizing parameters and the size of the original image. If
     * true (the default), images may be enlarged. If false, resize operations that would enlarge
     * the image will instead produce a copy of the original image at the same size.
     *
     * @since 4.0
     */
    ...
    /**
     * This optional {@link String} parameter specifies any additional ImageMagick commands, that
     * the user wishes to add. These commands are appended after the various crop and resize
     * options.
     */
    public static final String PARAM_COMMAND_OPTIONS = "commandOptions";

    /**
     * This optional {@link Boolean} flag parameter specifies if the engine should automatically
     * rotate and image based on the EXIF orientation flag. If this parameter is set to <code>true
     * </code> then the engine reads and resets the EXIF image profile setting 'Orientation' and
     * then performs the appropriate 90 degree rotation on the image to orient the image, for
     * correct viewing. This parameter defaults to <code>true</code>.
     */
    ...
    @Override
    protected TransformationOptions getTransformOptionsImpl(
        ...
        ImageTransformationOptions imageTransformationOptions =
                (ImageTransformationOptions) options;
    ...
    private ImageResizeOptions getImageResizeOptions(RenderingContext context) {
        ...
        boolean maintainAspectRatio =
                context.getParamWithDefault(PARAM_MAINTAIN_ASPECT_RATIO, false);
    ...
    protected void checkParameterValues(Action action) {
        ...
        if (mimeTypeParam != null && !mimeTypeParam.startsWith("image")) {
            ...
            msg.append("Parameter ")
                    .append(PARAM_MIME_TYPE)
                    .append(" had illegal non-image MIME type: ")
                    .append(mimeTypeParam);
            throw new IllegalArgumentException(msg.toString());
        }
    }

    /**
     * This method checks that if the specified parameter is non-null, that it has a positive
     * numerical value. That is it is non-zero and positive.
     *
     * @param action Action
    ...
    private void checkNumericalParameterIsPositive(Action action, String numericalParamName) {
        ...
        if (param != null && param.longValue() <= 0) {
            StringBuilder msg = new StringBuilder();
            msg.append("Parameter ")
                    .append(numericalParamName)
                    .append(" had illegal non-positive value: ")
                    .append(param.intValue());
            throw new IllegalArgumentException(msg.toString());
        }
    }
    ...
    private void checkTargetMimeType(RenderingContext context) {
        ...
        if (targetMimeType == null || !targetMimeType.startsWith("image")) {
            StringBuilder msg = new StringBuilder();
            msg.append("Target has illegal non-image MIME type: ")
                    .append(targetMimeType)
                    .append(". ")
                    .append("Use image as a source or configure target parameter \"")
                    .append(PARAM_MIME_TYPE)
                    .append("\"");
            throw new IllegalArgumentException(msg.toString());
        }

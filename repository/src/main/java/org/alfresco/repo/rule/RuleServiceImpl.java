 ...
 *
 * <p>This service automatically binds to the transaction flush hooks. It will therefore participate
 * in any flushes that occur during the transaction as well.
 *
...
public class RuleServiceImpl
    ...
    private RegexQNamePattern ASSOC_NAME_RULES_REGEX =
            new RegexQNamePattern(RuleModel.RULE_MODEL_URI, "^" + ASSOC_NAME_RULES_PREFIX + ".*");
    ...
    /**
     * List of disabled rules. Any rules that appear in this list will not be added to the pending
     * list and therefore not fired.
     */
    ...
    /**
     * Set the cache to hold node's individual rules. This cache <b>must not be shared</b> across
     * transactions.
     *
    ...
    /**
     * Registers to listen for events of interest. For instance, the creation or deletion of a rule
     * folder will affect the caching of rules.
     */
    public void init() {
    ...
    public ChildAssociationRef getSavedRuleFolderAssoc(NodeRef nodeRef) {
        ...
        List<ChildAssociationRef> assocs =
                this.runtimeNodeService.getChildAssocs(
                        nodeRef, RuleModel.ASSOC_RULE_FOLDER, RuleModel.ASSOC_RULE_FOLDER);
        if (assocs.size() > 1) {
    ...
    public boolean rulesEnabled(NodeRef nodeRef) {
        Set<NodeRef> disabledRuleNodes =
                TransactionalResourceHelper.getSet(KEY_DISABLED_RULE_NODES);
    ...
    public List<Rule> getRules(
            final NodeRef nodeRef, final boolean includeInherited, final String ruleTypeName) {
                    ...
                    public List<Rule> doWork() throws Exception {
                        ...
                        if (includeInherited == true
                                && runtimeNodeService.hasAspect(
                                                nodeRef, RuleModel.ASPECT_IGNORE_INHERITED_RULES)
                                        == false) {
                        ...
                        for (Rule rule : nodeRules) {
                            if ((rules.contains(rule) == false)
                                    && (ruleTypeName == null
                                            || rule.getRuleTypes().contains(ruleTypeName)
                                                    == true)) {
                                rules.add(rule);
                            }
                        }
    ...
    /**
     * Looks at the type of the node and indicates whether the node can have rules associated with
     * it
     *
     * @param nodeRef the node reference
    ...
    private List<Rule> getInheritedRules(
            NodeRef nodeRef, String ruleTypeName, Set<NodeRef> visitedNodeRefs) {
        ...
        if (this.runtimeNodeService.hasAspect(nodeRef, RuleModel.ASPECT_IGNORE_INHERITED_RULES)
            ...
            if (visitedNodeRefs.contains(nodeRef) == false) {
                ...
                for (ChildAssociationRef parent : parents) {
                    // We are not interested in following potentially massive person group
                    // membership trees!
                    ...
                    for (Rule rule :
                            getInheritedRules(
                                    parent.getParentRef(), ruleTypeName, visitedNodeRefs)) {
                        // Ensure that we don't get any rule duplication (don't use a set cos we
    ...
    public Rule getRule(NodeRef ruleNodeRef) {
        ...
        String title =
                DefaultTypeConverter.INSTANCE.convert(
                        String.class, props.get(ContentModel.PROP_TITLE));
        String description =
                DefaultTypeConverter.INSTANCE.convert(
                        String.class, props.get(ContentModel.PROP_DESCRIPTION));
        rule.setTitle(title);
        ...
        List<ChildAssociationRef> actions =
                this.nodeService.getChildAssocs(
                        ruleNodeRef, RuleModel.ASSOC_ACTION, RuleModel.ASSOC_ACTION);
        if (actions.size() == 0) {
    ...
    public void saveRule(NodeRef nodeRef, Rule rule) {
        ...
        if (this.permissionService.hasPermission(nodeRef, PermissionService.CHANGE_PERMISSIONS)
                == AccessStatus.ALLOWED) {
            disableRules();
            try {
                ...
                this.nodeService.setProperty(
                        ruleNodeRef, ContentModel.PROP_DESCRIPTION, rule.getDescription());
                this.nodeService.setProperty(
                        ruleNodeRef, RuleModel.PROP_RULE_TYPE, (Serializable) rule.getRuleTypes());
                this.nodeService.setProperty(
                        ruleNodeRef, RuleModel.PROP_APPLY_TO_CHILDREN, rule.isAppliedToChildren());
                this.nodeService.setProperty(
                        ruleNodeRef, RuleModel.PROP_EXECUTE_ASYNC, rule.getExecuteAsynchronously());
                this.nodeService.setProperty(
                        ruleNodeRef, RuleModel.PROP_DISABLED, rule.getRuleDisabled());
    ...
    public void setRulePosition(NodeRef nodeRef, NodeRef ruleNodeRef, int index) {
        ...
        if (ruleFolder != null) {
            List<ChildAssociationRef> assocs =
                    this.runtimeNodeService.getChildAssocs(
                            ruleFolder, RegexQNamePattern.MATCH_ALL, ASSOC_NAME_RULES_REGEX);
            List<ChildAssociationRef> orderedAssocs =
                    new ArrayList<ChildAssociationRef>(assocs.size());
    ...
    private void saveAction(NodeRef ruleNodeRef, Rule rule) {
        ...
        if (actions.size() == 1) {
            ...
            throw new RuleServiceException(
                    "The rule has become corrupt.  More than one action is associated with the"
                            + " rule.");
        }
        ...
        if (actionNodeRef == null) {
            actionNodeRef =
                    this.runtimeActionService.createActionNodeRef(
                            action, ruleNodeRef, RuleModel.ASSOC_ACTION, RuleModel.ASSOC_ACTION);
        }
            ...
            if (this.nodeService.exists(nodeRef) == true
                ...
                if (folder != null) {
                    List<ChildAssociationRef> ruleChildAssocs =
                            this.nodeService.getChildAssocs(
                                    folder, RegexQNamePattern.MATCH_ALL, ASSOC_NAME_RULES_REGEX);
    ...
    public void addRulePendingExecution(
        ...
        if (this.isEnabled() == true
            ...
            PendingRuleData pendingRuleData =
                    new PendingRuleData(actionableNodeRef, actionedUponNodeRef, rule, executeAtEnd);
            pendingRuleData.setRunAsUser(AuthenticationUtil.getRunAsUser());

            List<PendingRuleData> pendingRules =
                    (List<PendingRuleData>)
    ...
    public void executePendingRules() {
        if (AlfrescoTransactionSupport.getResource(KEY_RULES_EXECUTED) == null) {
            ...
            AlfrescoTransactionSupport.bindResource(
                    KEY_RULES_EXECUTED, new HashSet<ExecutedRuleData>());
        } else {
            if (logger.isDebugEnabled() == true) {
    ...
    private void executePendingRuleImpl(PendingRuleData pendingRule) {
        ...
        NodeRef ruleNodeRef = rule.getNodeRef();
        if (!ruleNodeRef.getStoreRef().equals(actionedUponNodeRef.getStoreRef())
                && !nodeService.exists(ruleNodeRef)) {
            NodeRef newRuleNodeRef =
                    new NodeRef(actionedUponNodeRef.getStoreRef(), ruleNodeRef.getId());
    ...
    public void executeRule(
        ...
        if (this.actionService.evaluateAction(action, actionedUponNodeRef) == true) {
            if (executedRules != null) {
                ...
                // (do this before this is executed to prevent rules being added to the pending
                // list)
            ...
            // current transaction is committed. This only deals with the bug fix and a more
            // thorough approach
            // (but one with potentially wide ranging consequences) is to replace the boolean
            // executeAsynchronously
            // property on Rules and Actions with an ExecutionTime property - which would
            // be an enumerated type with members SYNCHRONOUSLY, SYNCRHONOUSLY_AFTER_COMMIT and
            // ASYNCHRONOUSLY.
            //
            // NOTE: this code is not at the Action level (i.e. ActionService) since the logic of
            // sending after
                ...
                for (Action subAction : ((CompositeAction) action).getActions()) {
                    if (subAction.getActionDefinitionName().equals(MailActionExecuter.NAME)) {
                        subAction.setParameterValue(
                                MailActionExecuter.PARAM_SEND_AFTER_COMMIT, true);
                    }
                }
    ...
    private boolean canExecuteRule(
            Set<ExecutedRuleData> executedRules, NodeRef actionedUponNodeRef, Rule rule) {
        boolean result = true;
    ...
    private boolean checkForCopy(
        ...
        if (this.nodeService.exists(actionedUponNodeRef)
                && this.permissionService
                        .hasPermission(actionedUponNodeRef, PermissionService.READ)
        ...
        if (nodeService.hasAspect(nodeRef, RuleModel.ASPECT_RULES) == true) {
            ...
            if (assoc.isPrimary() == true) {
                List<ChildAssociationRef> linkedAssocs =
                        nodeService.getParentAssocs(assoc.getChildRef());

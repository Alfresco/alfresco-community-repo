...
public class SolrClientUtil {
  ...
  public static StoreRef extractStoreRef(
  ...
  public static String extractLanguageFragment(
    ...
    if (languageUrlFragment == null) {
      throw new AlfrescoRuntimeException(
        "No solr query support for language " + language
      );
    }
    return languageUrlFragment;
  }

  public static SolrStoreMappingWrapper extractMapping(
    StoreRef store,
    ...
    if ((shardRegistry != null) && useDynamicShardRegistration) {
      ...
      if ((slice == null) || (slice.size() == 0)) {
        logger.error(
          "No available shards for solr query of store " +
          store +
          " - trying non-dynamic configuration"
        );
        SolrStoreMappingWrapper mappings = mappingLookup.get(store);
        if (mappings == null) {
          throw new QueryParserException(
            "No solr query support for store " + store
          );
        }
        return mappings;
      }

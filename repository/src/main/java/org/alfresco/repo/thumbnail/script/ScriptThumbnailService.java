...
public class ScriptThumbnailService extends BaseScopableProcessorExtension {
    ...
    public boolean isThumbnailNameRegistered(String thumbnailName) {
        return (this.serviceRegistry
                        ...
                        .getThumbnailDefinition(thumbnailName)
                != null);
    }
    ...
    /**
     * Gets the resource path for the place holder thumbnail for the given named thumbnail and the
     * given mime type. If there is no icon available for the specified MIME type, a generic icon
     * will be used instead. The generic icon is that returned by {@link
     * #getPlaceHolderResourcePath(String)} If neither a MIME-specific icon nor a generic icon is
    ...
    public String getMimeAwarePlaceHolderResourcePath(String thumbnailName, String mimetype) {
        ...
        Map<String, String> extensionsByMimetype =
                serviceRegistry.getMimetypeService().getExtensionsByMimetype();
        // We get the extension for the mime type directly from the Map as we need
        // to know if a mimetype is not recognised by the system. (The MimetypeService gives us
        // ".bin"
        ...
        if (extension != null) {
            ...
            if (details != null) {
                ...
                if (resourcePath != null) {
                    // It's possible that the mimetype service recognises mime types for which we
                    // have no icon.
                    String formattedResourcePath = MessageFormat.format(resourcePath, mimeFragment);
                    boolean iconResourceExists =
                            classpathResourceExists("/" + formattedResourcePath);
    ...
    /**
     * This method returns <code>true</code> if the specified classpath resource exists, else <code>
     * false</code>.
     */

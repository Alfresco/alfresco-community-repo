 ...
 *
 * <p>Provides a way to record information about the capabilities being executed and report when an
...
public class RMMethodSecurityInterceptor extends MethodSecurityInterceptor {
     ...
     *
     * <p>Used to generate the capability error report.
     */
    private static final ThreadLocal<Map<String, CapabilityReport>> CAPABILITIES =
            new ThreadLocal<Map<String, CapabilityReport>>() {
                @Override
    ...
    private static final ThreadLocal<Boolean> IS_RM_SECURITY_CHECK =
    ...
    private static final ThreadLocal<List<String>> MESSAGES =
    ...
    /**
     * Get capability report object from the thread local, creating one for the given capability
     ...
     */
    public static void reportCapabilityCondition(
            String name, String conditionName, boolean expected, boolean actual) {
        if (LOGGER.isDebugEnabled()) {
            CapabilityReport capability = getCapabilityReport(name);
            if (!expected) {
    ...
    public String getFailureReport() {
        ...
        if (LOGGER.isDebugEnabled()) {
            Collection<CapabilityReport> capabilities =
                    RMMethodSecurityInterceptor.CAPABILITIES.get().values();
    ...
    /**
     * @see
     *     net.sf.acegisecurity.intercept.AbstractSecurityInterceptor#beforeInvocation(java.lang.Object)
     */
    @Override
    protected InterceptorStatusToken beforeInvocation(Object object) {
        ...
        try {
            ...
            if (LOGGER.isDebugEnabled()) {
                ...
                boolean first = true;
                methodDetails
                        .append("Failed on method:  ")
                        .append(mi.getMethod().getName())
                        .append("(");
                ...
                if (failureReport == null) {
                    ...
                    throw new AccessDeniedException(
                            exception.getMessage() + methodDetails, exception);
                } else {
                    // rethrow with additional information
                    throw new AccessDeniedException(
                            exception.getMessage() + methodDetails + getFailureReport(), exception);
                }
            } else {
                throw exception;
            }
    ...
    /**
     * @see
     *     net.sf.acegisecurity.intercept.method.aopalliance.MethodSecurityInterceptor#invoke(org.aopalliance.intercept.MethodInvocation)
     */
    @Override

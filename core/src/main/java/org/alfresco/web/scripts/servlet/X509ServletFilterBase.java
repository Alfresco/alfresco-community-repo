 ...
 *
 * <p>Optional Init Param: <b>cert-contains</b> : Ensure that the principal subject of the cert
 * contains a specific string.
 *
 * <p>The X509ServletFilter will also ensure that the cert is present in the request, which will
 * only happen if there is a successful SSL handshake which includes client authentication. This
 * handshake is handled by the Application Server. A SSL handshake that does not include client
 * Authentication will receive a 403 error response.
 *
 * <p>The checkInforce method must be implemented to determine if the X509 Authentication is turned
 * on. This allows applications to turn on/off X509 Authentication based on parameters outside of
 * the web.xml.
 */
public abstract class X509ServletFilterBase implements Filter {
        ...
        if (this.enforce) {
            ...
            if (validCert(certs)) {
                ...
                httpResponse.sendError(
                        HttpServletResponse.SC_FORBIDDEN, "X509 Authentication failure");
            }
        } else {
            /*
     ...
     * @throws IOException
     *     <p>The checkInforce method is called during the initialization of the Filter. Implement
     *     this method to decide if X509 security is being enforced.
     */
    ...
    private void handleClientAuth() {
        try {
            ...
            QueryExp query =
                    Query.or(
                            Query.eq(Query.attr("clientAuth"), Query.value("want")),
            ...
            if (objectNames != null && objectNames.size() == 0) {
                logger.warn(
                        "clientAuth does not appear to be set for Tomcat. clientAuth must be set to"
                                + " 'want' for X509 Authentication");
                ...
                if (objectNames1 != null) {
                    for (ObjectName objectName : objectNames1) {
                        if (objectName.toString().contains("ProtocolHandler")) {
                            logger.warn(
                                    "Setting clientAuth=want on MBean:" + objectName.toString());
                            mBeanServer.setAttribute(
                                    objectName, new Attribute("clientAuth", "want"));
                            return;
                        }
                    }
                }
            ...
            logger.warn(
                    "An error occurred while checking for clientAuth. Turn on debug logging to see"
                            + " the stacktrace.");

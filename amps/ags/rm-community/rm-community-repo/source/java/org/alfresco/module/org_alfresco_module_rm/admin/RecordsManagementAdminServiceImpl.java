...
public class RecordsManagementAdminServiceImpl
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(
    RecordsManagementAdminServiceImpl.class
  );

  /** I18N messages*/
  private static final String MSG_SERVICE_NOT_INIT =
    "rm.admin.service-not-init";
  private static final String MSG_PROP_EXIST = "rm.admin.prop-exist";
  private static final String MSG_CUSTOM_PROP_EXIST =
    "rm.admin.custom-prop-exist";
  private static final String MSG_UNKNOWN_ASPECT = "rm.admin.unknown-aspect";
  private static final String MSG_CONSTRAINT_EXISTS =
    "rm.admin.constraint-exists";
  private static final String MSG_CANNOT_FIND_CONSTRAINT =
    "rm.admin.contraint-cannot-find";
  ...
  private static final String CUSTOM_CONSTRAINT_TYPE =
    org.alfresco.module.org_alfresco_module_rm.caveat.RMListOfValuesConstraint.class.getName();
  private static final String CAPABILITY_CUSTOM_CONSTRAINT_TYPE =
    org.alfresco.module.org_alfresco_module_dod5015.caveat.RMListOfValuesConstraint.class.getName();
  ...
  public Set<QName> getCustomisable(NodeRef nodeRef) {
    ...
    for (QName aspect : aspects) {
      ...
      while (tempAspect != null) {
        ...
        AspectDefinition aspectDef = getDictionaryService()
          .getAspect(tempAspect);
        if (aspectDef != null) {
  ...
  private void initCustomMap() {
    ...
    Collection<QName> aspects = getDictionaryService()
      .getAspects(RM_CUSTOM_MODEL);
    for (QName aspect : aspects) {
      ...
      if (name.endsWith("Properties")) {
        ...
        if (prefixString == null) {
          ...
          if (CompatibilityModel.NAME_CUSTOM_RECORD_PROPERTIES.equals(name)) {
            ...
            CompatibilityModel.NAME_CUSTOM_RECORD_FOLDER_PROPERTIES.equals(name)
          ) {
            type = RecordsManagementModel.TYPE_RECORD_FOLDER;
          } else if (
            CompatibilityModel.NAME_CUSTOM_RECORD_CATEGORY_PROPERTIES.equals(
              name
            )
          ) {
            type = RecordsManagementModel.TYPE_RECORD_CATEGORY;
          } else if (
            CompatibilityModel.NAME_CUSTOM_RECORD_SERIES_PROPERTIES.equals(
              name
            ) &&
            // Only add the deprecated record series type as customisable if
            // a v1.0 installation has added custom properties
            aspectDef.getProperties().size() !=
            0
          ) {
    ...
    if (
      pendingCustomisableTypes != null && pendingCustomisableTypes.size() != 0
    ) {
      NodeRef modelRef = getCustomModelRef(
        RecordsManagementModel.RM_CUSTOM_URI
      );
      ...
      try {
        for (QName customisableType : pendingCustomisableTypes) {
          ...
          M2Aspect aspect = model.createAspect(
            customAspect.toPrefixString(getNamespaceService())
          );
          aspect.setDescription(
            customisableType.toPrefixString(getNamespaceService())
          );
  ...
  private QName getCustomAspectImpl(QName customisableType) {
    String localName = customisableType
  ...
  public void unmakeCustomisable(QName type) {
    ...
    if (customisableTypes == null) {
      throw new AlfrescoRuntimeException(
        I18NUtil.getMessage(MSG_SERVICE_NOT_INIT)
      );
    }
  ...
  public boolean existsCustomProperty(QName propertyName) {
    ...
    if (
      RM_CUSTOM_URI.equals(propertyName.getNamespaceURI()) &&
  ...
  public Map<QName, PropertyDefinition> getCustomPropertyDefinitions() {
    ...
    for (QName customisableType : getCustomisable()) {
      Map<QName, PropertyDefinition> props = getCustomPropertyDefinitions(
        customisableType
      );
      if (props != null) {
        result.putAll(props);
  ...
  public QName addCustomPropertyDefinition(
    ...
    M2Aspect customPropsAspect = deserializedModel.getAspect(
      customAspect.toPrefixString(getNamespaceService())
    );
    ...
    if (lovConstraint != null) {
      ...
      String lovConstraintQNameAsString = lovConstraint.toPrefixString(
        getNamespaceService()
      );
   ...
   */
  public QName updateCustomPropertyDefinitionName(
    QName propQName,
    String newName
  ) throws CustomMetadataException {
    ...
    if (propDefn == null) {
      throw new AlfrescoRuntimeException(
        I18NUtil.getMessage(MSG_PROP_EXIST, propQName)
      );
    }
    ...
    if (newPropQName != null) {
      PropertyDefinition newPropDefn = getDictionaryService()
        .getProperty(newPropQName);
      if (newPropDefn != null && !propDefn.equals(newPropDefn)) {
        // The requested QName is already in use
        String propIdAsString = newPropQName.toPrefixString(
          getNamespaceService()
        );
        throw new PropertyAlreadyExistsMetadataException(propIdAsString);
      }
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
        "setCustomPropertyDefinitionLabel: " + propQName + "=" + newName
      );
    }
   ...
   */
  public QName setCustomPropertyDefinitionLabel(
    QName propQName,
    String newLabel
  ) {
    mandatory("propQName", propQName);
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
        "setCustomPropertyDefinitionLabel: " + propQName + "=" + newLabel
      );
    }
   ...
   */
  public QName setCustomPropertyDefinitionConstraint(
    QName propQName,
    ...
    if (
      !dataType.equals(
        DataTypeDefinition.TEXT.toPrefixString(getNamespaceService())
      )
    ) {
      throw new AlfrescoRuntimeException(
        I18NUtil.getMessage(
    ...
    String lovConstraintQNameAsString = newLovConstraint.toPrefixString(
      getNamespaceService()
    );
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
        "addCustomPropertyDefinitionConstraint: " + lovConstraintQNameAsString
      );
    }
  ...
  private M2Property findProperty(QName propQName, M2Model deserializedModel) {
    ...
    for (M2Aspect aspect : aspects) {
      for (M2Property prop : aspect.getProperties()) {
        if (
          propQName.toPrefixString(getNamespaceService()).equals(prop.getName())
        ) {
          return prop;
        }
      }
    ...
    throw new AlfrescoRuntimeException(
      I18NUtil.getMessage(MSG_CUSTOM_PROP_EXIST, propQName)
    );
  }
  ...
  public void addCustomReference(
    ...
    getRelationshipService()
      .addRelationship(refId.getLocalName(), fromNode, toNode);
  }
  ...
  public void removeCustomReference(
    ...
    getRelationshipService()
      .removeRelationship(assocId.getLocalName(), fromNode, toNode);
  }
  ...
  public QName updateCustomChildAssocDefinition(
    ...
    RelationshipDefinition relationshipDefinition = getRelationshipService()
      .updateRelationshipDefinition(localName, displayName);
    return QName.createQName(
      RM_CUSTOM_PREFIX,
      relationshipDefinition.getUniqueName(),
      getNamespaceService()
  ...
  public void addCustomConstraintDefinition(
    ...
    String constraintNameAsPrefixString = constraintName.toPrefixString(
      getNamespaceService()
    );

    M2Constraint customConstraint = deserializedModel.getConstraint(
    ...
    M2Constraint newCon = deserializedModel.createConstraint(
      constraintNameAsPrefixString,
   ...
   */
  public void changeCustomConstraintValues(
    QName constraintName,
  ...
  public List<ConstraintDefinition> getCustomConstraintDefinitions(
    ...
    Collection<ConstraintDefinition> conDefs = getDictionaryService()
      .getConstraints(modelQName, true);
  ...
  public void removeCustomConstraintDefinition(QName constraintName) {
    ...
    if (logger.isInfoEnabled()) {
      logger.info(
        "deleteCustomConstraintDefinition: " + constraintNameAsPrefixString
      );
    }
  }
  ...
  private QName generateQNameFor(String clientId) {
    if (getQNameForClientId(clientId) != null) {
      ...
      throw new IllegalArgumentException(
        I18NUtil.getMessage(MSG_ERROR_CLIENT_ID, clientId)
      );
    }

    String newGUID = GUID.generate();
    QName newQName = QName.createQName(
      RM_CUSTOM_PREFIX,
      newGUID,
      getNamespaceService()
    );

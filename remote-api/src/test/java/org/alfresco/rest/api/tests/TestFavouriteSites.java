...
public class TestFavouriteSites extends EnterpriseTestApi {
  ...
  public void testFavouriteSites() throws Exception {
    ...
    TestSite testSite = transactionHelper.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<TestSite>() {
    ...
    TestSite testSite1 = transactionHelper.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<TestSite>() {
        @SuppressWarnings("synthetic-access")
        public TestSite execute() throws Throwable {
          return TenantUtil.runAsUserTenant(
    ...
    TestSite testSite3 = transactionHelper.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<TestSite>() {
        @SuppressWarnings("synthetic-access")
        public TestSite execute() throws Throwable {
          return TenantUtil.runAsUserTenant(
    ...
    TestSite testSite4 = transactionHelper.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<TestSite>() {
        @SuppressWarnings("synthetic-access")
        public TestSite execute() throws Throwable {
          return TenantUtil.runAsUserTenant(
    ...
    try {
      ...
      sitesProxy.create(
        ...
        testSite.getSiteId(),
        fs.toJSON().toString(),
        "Unable to POST to a favorite-site"
      );
      fail();
    ...
    try {
      ...
      sitesProxy.remove(
        ...
        "favorite-sites",
        null,
        "Unable to DELETE favorite-sites"
      );
      fail();
    } catch (PublicApiException e) {
      assertEquals(
    ...
    try {
      ...
      sitesProxy.createFavouriteSite(
        person1.getId(),
        new FavouriteSite(testSite.getSiteId())
      );
      fail();
    } catch (PublicApiException e) {
    ...
    // "-me" user
    {
      ...
      FavouriteSite fs = new FavouriteSite(testSite.getSiteId());
      FavouriteSite resp = sitesProxy.createFavouriteSite(
        org.alfresco.rest.api.People.DEFAULT_USER,
        fs
      );
      fs.expected(resp);

      final List<FavouriteSite> expectedFavouriteSites = TenantUtil.runAsUserTenant(
        new TenantRunAsWork<List<FavouriteSite>>() {
          @Override
    ...
    // GET favourite sites
    {
      ...
      try {
        ...
        publicApiClient.setRequestContext(
          new RequestContext(network1.getId(), GUID.generate(), "password")
        );
        sitesProxy.getFavouriteSites(
          person1.getId(),
          createParams(paging, null)
      ...
      try {
        ...
        ListResponse<FavouriteSite> response = sitesProxy.getFavouriteSites(
          person1.getId(),
          createParams(paging, null)
        );
    ...
    {
      publicApiClient.setRequestContext(
        new RequestContext(network2.getId(), person5.getId())
      );

      List<FavouriteSite> expectedFavouriteSites = new ArrayList<FavouriteSite>(
        1
      );
    ...
    // remove
    {
      ...
      {
        publicApiClient.setRequestContext(
          new RequestContext(network1.getId(), person3.getId())
        );
      ...
      // known user
      {
        ...
        List<FavouriteSite> expectedFavouriteSites = TenantUtil.runAsUserTenant(
          new TenantRunAsWork<List<FavouriteSite>>() {
            @Override
            public List<FavouriteSite> doWork() throws Exception {
              return repoService.getFavouriteSites(person1);
      ...
      // "-me-" user
      {
        ...
        sitesProxy.removeFavouriteSite(
          org.alfresco.rest.api.People.DEFAULT_USER,
          fs
        );

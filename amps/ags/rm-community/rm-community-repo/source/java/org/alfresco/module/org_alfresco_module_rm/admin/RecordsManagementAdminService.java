...
public interface RecordsManagementAdminService {
    ...
    /**
     * Get a list of all the registered customisable types and aspects present on a given node
     * reference.
     *
     ...
     *
     * <p>Note: the custom property definitions are retrieved from the dictionaryService which is
     * notified of any newly created definitions on transaction commit. Therefore custom properties
     * created in the current transaction will not appear in the result of this method.
     *
    ...
    /**
     * This method returns the custom properties that have been defined for all of the specified
     * customisable RM elements.
     *
     * <p>Note: the custom property definitions are retrieved from the dictionaryService which is
     * notified of any newly created definitions on transaction commit. Therefore custom properties
     * created in the current transaction will not appear in the result of this method.
     ...
     *
     * <p>Note: no default value, single valued, optional, not system protected, no constraints
     *
     * @param propId - If a value for propId is provided it will be used to identify property
     *     definitions within URLs and in QNames. Therefore it must contain URL/QName-valid
     *     characters only. It must also be unique. If a null value is passed, an id will be
     *     generated.
     * @param typeName - mandatory. The aspect within which the property is to be defined. This must
     *     be one of the CustomisableRmElements.
     ...
     * @return the propId.
     */
    QName setCustomPropertyDefinitionLabel(QName propQName, String newLabel)
     ...
     * @throws CustomMetadataException
     */
    QName updateCustomPropertyDefinitionName(QName propQName, String newName)
            throws CustomMetadataException;
    ...
    /**
     * This method returns the custom references that have been defined in the custom model. Note:
     * the custom reference definitions are retrieved from the dictionaryService which is notified
     * of any newly created definitions on transaction commit. Therefore custom references created
     * in the current transaction will not appear in the results.
     *
     * @return The Map of custom references (both parent-child and standard).
     * @deprecated as of RM 2.3, please use {@link RelationshipService#getRelationshipDefinitions()}
     *     instead.
     */
     ...
     * @deprecated as of RM 2.3, please use{@link NodeService#getTargetAssocs(NodeRef,
     *     QNamePattern)} with QNamePattern RegexQNamePattern.MATCH_ALL instead
     */
    List<AssociationRef> getCustomReferencesFrom(NodeRef node);

    /**
     * Fetches all child associations of the given source. i.e. all associations where the given
     * node is the parent.
     *
     * @param node
     * @return
     ...
     * @return a List of associations.
     * @deprecated as of RM 2.3, please use {@link NodeService#getSourceAssocs(NodeRef,
     *     QNamePattern)} with QNamePattern RegexQNamePattern.MATCH_ALL instead.
     */
    ...
    /**
     * This method adds the specified custom reference instance between the specified nodes. Only
     * one instance of any custom reference type is allowed in a given direction between two given
     * records.
     *
     * @param fromNode
     * @param toNode
     * @param assocId the server-side qname e.g.
     *     {http://www.alfresco.org/model/rmcustom/1.0}abcd-12-efgh-4567
     * @throws AlfrescoRuntimeException if an instance of the specified reference type already
     *     exists from fromNode to toNode.
     * @deprecated as of RM 2.3, please use {@link RelationshipService#addRelationship(String,
     *     NodeRef, NodeRef)} instead.
     */
     ...
     * @param assocId the server-side qname e.g.
     *     {http://www.alfresco.org/model/rmcustom/1.0}abcd-12-efgh-4567
     * @deprecated as of RM 2.3, please use {@link RelationshipService#removeRelationship(String,
     *     NodeRef, NodeRef)} instead.
     */
     ...
     * @deprecated as of RM 2.3, please use {@link
    ...
    /**
     * This method creates a new custom child association, combining the given source and target and
     * using the combined String as the title.
     *
     * @param source
     * @param target
     * @return the QName of the newly-created association.
    ...
    /**
     * This method updates the source and target values for the specified child association. The
     * source and target will be combined into a single string and stored in the title property.
     ...
     * @deprecated as of RM 2.3, please use {@link
     *     RelationshipService#updateRelationshipDefinition(String,
    ...
    /**
     * This method updates the label value for the specified association. The label will be stored
     * in the title property. Label is String metadata for bidirectional custom references.
     *
     * @param refQName qname of the child association.
    ...
    /**
     * This method returns ConstraintDefinition objects defined in the given model (note: not
     * property references or in-line defs) The custom constraint definitions are retrieved from the
     * dictionaryService which is notified of any newly created definitions on transaction commit.
     * Therefore custom constraints created in the current transaction will not appear in the
     * results.
     */
    List<ConstraintDefinition> getCustomConstraintDefinitions(QName modelQName);

    /**
     * This method adds a Constraint definition to the custom model. The implementation of this
     * method would have to go into the M2Model and insert the relevant M2Objects for this new
     * constraint.
     *
     * <p>param type not included as it would always be RMListOfValuesConstraint for RM.
     *
     ...
     */
    void addCustomConstraintDefinition(
            QName constraintName,
            String title,
    ...
    /**
     * This method iterates over the custom properties, references looking for one whose id exactly
    ...
    /**
     * Given a compound id for source and target strings (as used with parent/child custom
     * references), this method splits the String and returns an array containing the source and
     * target IDs separately.
     *

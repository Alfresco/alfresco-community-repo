...
public interface ImapService {
    ...
    /**
     * Helper enumeration to handle email body format text/html and text/plain for Alfresco/Share
     * webapp
     */
    ...
    /**
     * Deletes an existing MailBox. Specified mailbox must already exist on this server, and the
     * user must have rights to delete it. This method serves DELETE command of the IMAP protocol.
     *
     * @param user User making the request.
    ...
    /**
     * Renames an existing mailbox. The specified mailbox must already exist, the requested name
     * must not exist already but must be able to be created and the user must have rights to delete
     * the existing mailbox and create a mailbox with the new name. Any inferior hierarchical names
     * must also be renamed. If INBOX is renamed, the contents of INBOX are transferred to a new
     * mailbox with the new name, but INBOX is not deleted. If INBOX has inferior mailbox these are
     * not renamed. This method serves RENAME command of the IMAP protocol.
     *
     * @param user User making the request.
     ...
     * <br>
     * <br>
     * <code>mailbox</code> parameter may specify absolute or relative path to a folder. Absolute
     * path uniquely identifies some directory, whereas relative path implies that root folder
     * should be <code>IMAP home</code> directory for the specified <code>user</code> rather than
     * <code>IMAP root</code> (i.e. <code>IMAP mount point</code>). Mailbox will be found or created
     * (<code>mayCreate=true</code>, <code>mayExist=false</code> or ) in <code>user</code>'s <code>
     * IMAP home</code> directory if relative path is specified.<br>
     * <br>
     * <code>mayExist</code> and mayCreate parameters' combinations and results:
     *
     * <ul>
     *   <li><code>mayCreate=false</code>, <code>mayExist=true</code> - mailbox is found and not
     *       created if it doesn't exist. Error is thrown if mailbox doesn't not exist;
     *   <li><code>mayCreate=true</code>, <code>mayExist=true</code> - mailbox is created if it
     *       doesn't exist or it is just found in other case. No error is thrown;
     *   <li><code>mayCreate=true</code>, <code>mayExist=false</code> - mailbox is created if it
     *       doesn't exist. Error is thrown if it is already created;
     *   <li><code>mayCreate=false</code>, <code>mayExist=false</code> - error is thrown that
     *       mailbox cannot be created if doesn't exist. Error is thrown that mailbox should not
     *       exist in other case.<br>
     *       <b>It's a very shady combination!</b>
     * </ul>
     *
     * @param user User making the request.
     * @param mailboxName String name of the target.
     * @param mayExist Is the mailbox allowed to exist already? If <code>false</code> and the
     *     mailbox already exists, an error will be thrown
     * @param mayCreate If the mailbox does not exist, can one be created? If <code>false</code>
     *     then an error is thrown if the folder does not exist
    ...
    /**
     * Subscribes a user to a mailbox. The mailbox must exist locally and the user must have rights
     * to modify it.
     *
     * <p>This method serves SUBSCRIBE command of the IMAP protocol.
     *
     * @param user User making the request
     ...
     *
     * <p>This method serves UNSUBSCRIBE command of the IMAP protocol.
     *
     * @param user User making the request
     * @param mailbox String representation of a mailbox name.
     */
    ...
    //     */
    //    public List<FileInfo> searchFiles(NodeRef contextNodeRef, String namePattern, boolean
    // includeSubFolders);
    ...
    //     */
    //    public List<FileInfo> searchFolders(NodeRef contextNodeRef, String namePattern, boolean
    // includeSubFolders, ImapViewMode viewMode);
     ...
     * @return a message.
     */
    public SimpleStoredMessage createImapMessage(FileInfo messageFileInfo, boolean generateBody)
    ...
    /**
     * @return Path to the folder containing templates, that will be used for generating body of
     *     message in VIRTUAL and MIXED views.
     */
    ...
    /**
     * Returns a template for email body. It is either classpath path or NodeRef.toString(). This
     * method trying to find a template on the path in the repository first e.g. {@code "Data
     * Dictionary > IMAP Templates >"}. This path should be set as the property of the "imapHelper"
     * bean. In this case it returns {@code NodeRef.toString()} of the template. If there are no
     * template in the repository it returns a default template on the classpath.
     *
     ...
     * @return nodeRef of the node's site container or null if provided node does not belong to
     *     sites.
     */
    ...
    static class FolderStatus {
        ...
        public FolderStatus(
                ...
                long uidValidity,
                String changeToken,
                NavigableMap<Long, FileInfo> search) {

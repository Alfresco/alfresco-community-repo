...
public class DispositionActionImpl
  implements DispositionAction, RecordsManagementModel {
  ...
  public String getLabel() {
    ...
    RecordsManagementAction action = services
      .getRecordsManagementActionService()
      .getDispositionAction(name);
    if (action != null) {
  ...
  public boolean isEventsEligible() {
    return (
      (Boolean) services
        .getNodeService()
        .getProperty(this.dispositionNodeRef, PROP_DISPOSITION_EVENTS_ELIGIBLE)
    ).booleanValue();
  }
  ...
  public List<EventCompletionDetails> getEventCompletionDetails() {
    List<ChildAssociationRef> assocs = services
      .getNodeService()
      .getChildAssocs(
        this.dispositionNodeRef,
   ...
   */
  private EventCompletionDetails getEventCompletionDetailsFromNodeRef(
    NodeRef nodeRef
  ) {
    // get the properties
    Map<QName, Serializable> props =
      this.services.getNodeService().getProperties(nodeRef);
    ...
    if (!assocs.isEmpty()) {
      if (assocs.size() != 1) {
        throw new AlfrescoRuntimeException(
          "Unable to get event completion details, because more than one child was found for event " +
          eventName
        );
      }

      result =
        getEventCompletionDetailsFromNodeRef(assocs.get(0).getChildRef());
    }
      ...
      AuthenticationUtil.runAsSystem(
        new RunAsWork<Void>() {
          ...
          public Void doWork() {
            ...
            Map<QName, Serializable> props = services
              .getNodeService()
              .getProperties(eventNodeRef);
    ...
    AuthenticationUtil.runAsSystem(
      new RunAsWork<Void>() {
        ...
        public Void doWork() {
          ...
          List<String> stepEvents = (List<String>) services
            .getNodeService()
            .getProperty(
              getDispositionActionDefinition().getNodeRef(),
          ...
          for (EventCompletionDetails event : eventsList) {
            ...
            if (
              stepEvents != null && !stepEvents.contains(event.getEventName())
            ) {
  ...
  private void setDeclassificationReview(
    NodeRef eventNodeRef,
    Date completedAtValue,
    String completedByValue
  ) {
    NodeRef nextDispositionActionNodeRef = services
      .getNodeService()
      .getPrimaryParent(eventNodeRef)
      .getParentRef();
    NodeRef nodeRef = services
      .getNodeService()
    ...
    if (
      ...
      List<ChildAssociationRef> records = services
        .getNodeService()
        .getChildAssocs(
          nodeRef,
  ...
  private void setPropsOnContent(
    ...
    Map<QName, Serializable> nodeProps = services
      .getNodeService()
      .getProperties(nodeRef);
    nodeProps.put(
      PROP_RS_DECLASSIFICATION_REVIEW_COMPLETED_AT,
      completedAtValue
    );
    nodeProps.put(
      PROP_RS_DECLASSIFICATION_REVIEW_COMPLETED_BY,
      completedByValue
    );

 ...
 *
 * <p>Files are created in a temporary directory and then a delete and move.
 */
public class ScenarioTempDeleteShuffle implements Scenario {
    ...
    protected static final String SCENARIO_KEY =
            "org.alfresco.filesys.repo.rules.ScenarioTempDeleteShuffle";
    ...
    public ScenarioInstance createInstance(final EvaluatorContext ctx, Operation operation) {
        ...
        if (operation instanceof CreateFileOperation) {
            ...
            if (d.matches()) {
                logger.debug(
                        "pattern matches temp dir folder so this is a new create in a temp dir");
                Matcher m = pattern.matcher(c.getName());
                if (m.matches()) {
                    // and how to lock - since we are already have one lock on the scenarios/folder
                    // here
                    // this is a potential deadlock and synchronization bottleneck
                    Map<String, String> createdTempFiles =
                            (Map<String, String>) ctx.getSessionState().get(SCENARIO_KEY);

                    if (createdTempFiles == null) {
                        synchronized (ctx.getSessionState()) {
                            logger.debug(
                                    "created new temp file map and added it to the session state");
                            createdTempFiles =
                                    (Map<String, String>) ctx.getSessionState().get(SCENARIO_KEY);
                            if (createdTempFiles == null) {
                                createdTempFiles =
                                        Collections.synchronizedMap(
                                                new MaxSizeMap<String, String>(5, false));
                ...
                if (createdTempFiles != null) {
                    if (createdTempFiles.containsKey(c.getName())) {
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "New Scenario Temp Delete Shuffle Instance:" + c.getName());
                        }

                        ScenarioTempDeleteShuffleInstance instance =
                                new ScenarioTempDeleteShuffleInstance();

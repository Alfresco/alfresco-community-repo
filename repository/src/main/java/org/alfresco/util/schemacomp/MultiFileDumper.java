...
/**
 * Given a set of database object prefixes (e.g. "alf_", "act_") and a file name template (e.g.
 * "AlfrescoSchema-MySQL-{0}-") will produce a set of files, one per database object prefix of the
 * form:
 *
 * <pre>
 ...
 * </pre>
 *
 * Where the database object prefix is substituted for parameter {0} and the random number is
 * produced by the File.createTempFile() method. The suffix .xml is always used.
 *
 * @author Matt Ward
 */
public class MultiFileDumper {
    ...
    public static final String[] DEFAULT_PREFIXES = new String[] {"alf_", "act_"};
    ...
    public List<File> dumpFiles() {
        ...
        for (String dbPrefix : dbPrefixes) {
            ...
            File outputFile =
                    TempFileProvider.createTempFile(fileNamePrefix, fileNameSuffix, directory);

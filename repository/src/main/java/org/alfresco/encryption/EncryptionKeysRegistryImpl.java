...
/**
 * Registered Encryption Keys are stored in the AttributeService directly under a top level key
 * defined by TOP_LEVEL_KEY (which means that all key aliases must be unique across however many
 * keystores are being used).
 *
 * @since 4.0
 */
// TODO caching? This will probably not be used extensively.
// TODO instead of persisting the Pair when registering a key, create two attributes per key (one
// for the
// guid and one for the encrypted value of the guid). This means a custom class does not need to be
// bound to
...
public class EncryptionKeysRegistryImpl implements EncryptionKeysRegistry {
    ...
    public void registerKey(String keyAlias, Key key) {
        ...
        final RetryingTransactionCallback<Void> createAttributeCallback =
    ...
    public List<String> getRegisteredKeys(final Set<String> keyStoreKeys) {
        ...
        attributeService.getAttributes(
                new AttributeQueryCallback() {
                    public boolean handleAttribute(
                            Long id, Serializable value, Serializable[] keys) {
    ...
    public KEY_STATUS checkKey(String keyAlias, Key key) {
        ...
        if (attributeService.exists(TOP_LEVEL_KEY, keyAlias)) {
            try {
                ...
                try {
                    keyCheck =
                            (Pair<String, Serializable>)
                                    attributeService.getAttribute(TOP_LEVEL_KEY, keyAlias);
                } catch (Throwable e) {
                    // there is an issue getting the attribute. Remove it.
                    attributeService.removeAttribute(TOP_LEVEL_KEY, keyAlias);
                ...
                return EqualsHelper.nullSafeEquals(storedGUID, keyCheck.getFirst())
                        ? KEY_STATUS.OK
                        : KEY_STATUS.CHANGED;
    ...
    public void removeRegisteredKeys(final Set<String> keys) {
        RetryingTransactionHelper retryingTransactionHelper =
                transactionService.getRetryingTransactionHelper();
        final RetryingTransactionCallback<Void> removeKeysCallback =
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {

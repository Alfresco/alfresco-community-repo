...
public class ReplicationDefinitionsPost extends AbstractReplicationWebscript {
  ...
  protected Map<String, Object> buildModel(
    ...
    try {
      ...
      if (!json.has("name")) throw new WebScriptException(
        Status.STATUS_BAD_REQUEST,
        "name is required but wasn't supplied"
      );
      if (!json.has("description")) throw new WebScriptException(
        Status.STATUS_BAD_REQUEST,
        "description is required but wasn't supplied"
      );
      ...
      if (replicationService.loadReplicationDefinition(name) != null) {
        throw new WebScriptException(
          Status.STATUS_BAD_REQUEST,
          "A replication definition already exists with that name"
        );
      }

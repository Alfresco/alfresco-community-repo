...
/**
 * Rateable aspect behaviour bean. When any node with the rateable aspect is copied, then ensure the
 * ratings and roll ups are not copied.
 *
 * @author Alex Miller
 */
public class RateableAspect implements CopyServicePolicies.OnCopyNodePolicy {
    ...
    public void init() {
        ...
        for (RatingScheme ratingScheme : ratingSchemeRegistry.getRatingSchemes().values()) {
            if (ratingScheme.getPropertyRollups() != null
                    && ratingScheme.getPropertyRollups().size() > 0) {
                QName rollupAspectName =
                        ratingNamingConventions.getRollupAspectNameFor(ratingScheme);

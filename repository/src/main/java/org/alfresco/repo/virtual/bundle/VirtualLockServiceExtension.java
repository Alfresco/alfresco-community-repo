...
public class VirtualLockServiceExtension
        extends SpringBeanExtension<LockServiceExtension, LockServiceTrait>
    ...
    public void lock(NodeRef nodeRef, LockType lockType) throws UnableToAquireLockException {
        getTrait().lock(smartStore.materializeIfPossible(nodeRef), lockType);
    }

    @Override
    public void lock(NodeRef nodeRef, LockType lockType, int timeToExpire)
            throws UnableToAquireLockException {
        getTrait().lock(smartStore.materializeIfPossible(nodeRef), lockType, timeToExpire);
    }
    ...
    public void lock(NodeRef nodeRef, LockType lockType, int timeToExpire, Lifetime lifetime)
            throws UnableToAquireLockException {
        getTrait()
                .lock(smartStore.materializeIfPossible(nodeRef), lockType, timeToExpire, lifetime);
    }
    ...
    public void lock(Collection<NodeRef> nodeRefs, LockType lockType, int timeToExpire)
            throws UnableToAquireLockException {
        getTrait().lock(smartStore.materializeIfPossible(nodeRefs), lockType, timeToExpire);
    }
    ...
    public void unlock(NodeRef nodeRef, boolean lockChildren) throws UnableToReleaseLockException {
        getTrait().unlock(smartStore.materializeIfPossible(nodeRef), lockChildren);
    }

    @Override
    public void unlock(NodeRef nodeRef, boolean lockChildren, boolean allowCheckedOut)
            throws UnableToReleaseLockException {
        getTrait().unlock(smartStore.materializeIfPossible(nodeRef), lockChildren, allowCheckedOut);
    }
    ...
    public LockStatus getLockStatus(NodeRef nodeRef, String userName) {
        return getTrait().getLockStatus(smartStore.materializeIfPossible(nodeRef), userName);
    }

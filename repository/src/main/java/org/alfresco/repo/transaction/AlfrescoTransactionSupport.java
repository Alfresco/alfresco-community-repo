...
/**
 * Repo Specific Helper class to manage transaction synchronization. This provides helpers to ensure
 * that the necessary <code>TransactionSynchronization</code> instances are registered on behalf of
 * the application code.
 *
 * <p>This class remains for backward API compatibility, the majority of transaction support has
 * been moved to TransactionSupportUtil in the Core project.
 *
 * @author Derek Hulley
 * @author mrogers
 */
public abstract class AlfrescoTransactionSupport extends TransactionSupportUtil {
    ...
    /**
     * Checks the state of the current transaction and throws an exception if a transaction is not
     * present or if the transaction is not read-write, if required.
     *
    ...
    public static void checkTransactionReadState(boolean requireReadWrite) {
        ...
        switch (readState) {
            ...
            case TXN_READ_ONLY:
                if (requireReadWrite) {
                    throw new IllegalStateException(
                            "The current operation requires an active read-write transaction.");
                }
            case TXN_READ_WRITE:
                // All good
        }
    ...
    /**
     * Method that registers a <tt>NodeDaoService</tt> against the transaction. Setting this will
     * ensure that the pre- and post-commit operations perform the necessary cleanups against the
     * <tt>NodeDaoService</tt>.
     *
     * <p>This method can be called repeatedly as long as the service being bound implements
    ...
    /**
     * Method that registers an <tt>IntegrityChecker</tt> against the transaction. Setting this will
     * ensure that the pre- and post-commit operations perform the necessary cleanups against the
     * <tt>IntegrityChecker</tt>.
     *
     * <p>This method can be called repeatedly as long as the service being bound implements
     * <tt>equals</tt> and <tt>hashCode</tt>.
     *
    ...
    public static void bindIntegrityChecker(IntegrityChecker integrityChecker) {
        ...
        // bind the service in
        boolean bound =
                bindListener((TransactionListener) integrityChecker, COMMIT_ORDER_INTEGRITY);
    ...
    /**
     * Method maintained for backward compatibility: <a
     * href="https://issues.alfresco.com/jira/browse/ACE-2801">ACE-2801: Package change for
     * TransactionListener</a>.
     *
    ...
    public static void bindListener(org.alfresco.repo.transaction.TransactionListener listener) {
        AlfrescoTransactionSupport.bindListener(
                (org.alfresco.util.transaction.TransactionListener) listener);
    }
     ...
     *
     * @param listener the transaction listener tasks on
     * @since 5.0
     */

...
public class CMMDownloadTestUtil {
    ...
    private static final String SHARE_EXTENSIONS_FOLDER =
            "extensions"; // app:company_home/st:sites/cm:surf-config/cm:extensions
    private static final String SHARE_PERSISTED_EXTENSION_FILE =
            "default-persisted-extension.xml"; // app:company_home/st:sites/cm:surf-config/cm:extensions/cm:default-persisted-extension.xml
    ...
    private static final String MODULE =
            "<extension>"
                    ...
                    + "<id>CMM_"
                    + MKR
                    + "</id><auto-deploy>true</auto-deploy><configurations><config"
                    + " evaluator=\"string-compare\" condition=\"DocumentLibrary\""
                    + " replace=\"false\"><types><type name=\"cm:content\"><subtype label=\"type1"
                    + " title\" name=\"testprefix:type1\"/></type></types></config><config"
                    + " evaluator=\"string-compare\" condition=\"FormDefinition\"></config>"
    ...
    public synchronized void createShareExtModule(final String moduleId) {
        ...
        this.extensionsNodeRef =
                nodeService.getChildByName(
                        surfConfigNodeRef, ContentModel.ASSOC_CONTAINS, SHARE_EXTENSIONS_FOLDER);
        if (this.extensionsNodeRef == null) {
            ...
            logger.info(
                    "Created 'cm:extensions' folder within the"
                            + " 'app:company_home/st:sites/cm:surf-config'");
        }

        this.sharePersistedExtNodeRef =
                nodeService.getChildByName(
                        this.extensionsNodeRef,
                        ContentModel.ASSOC_CONTAINS,
                        SHARE_PERSISTED_EXTENSION_FILE);
        if (this.sharePersistedExtNodeRef == null) {
            ...
            logger.info(
                    "Created 'cm:default-persisted-extension.xml' file within the"
                            + " 'app:company_home/st:sites/cm:surf-config/cm:extensions'");
                        ...
                        public Void execute() throws Throwable {
                            ContentReader reader =
                                    contentService.getReader(
                                            sharePersistedExtNodeRef, ContentModel.PROP_CONTENT);
                            originalShareExtFile =
                                    TempFileProvider.createTempFile(
                                            CustomModelServiceImplTest.class.getName(), ".xml");
                    ...
                    public Void execute() throws Throwable {
                        ContentWriter writer =
                                contentService.getWriter(
                                        sharePersistedExtNodeRef, ContentModel.PROP_CONTENT, true);
    ...
    public synchronized void cleanup() {
        if (isExtFolderCreated) {
            ...
            logger.info(
                    "Deleted 'cm:extensions' folder within the"
                            + " 'app:company_home/st:sites/cm:surf-config");
        } else if (originalShareExtFile != null) {
            transactionHelper.doInTransaction(
                    new RetryingTransactionCallback<Void>() {
                        ...
                        public Void execute() throws Throwable {
                            ContentWriter writer =
                                    contentService.getWriter(
                                            sharePersistedExtNodeRef,
                                            ContentModel.PROP_CONTENT,
                                            true);
                            writer.setMimetype(MimetypeMap.MIMETYPE_XML);
                            writer.setEncoding("UTF-8");
                            writer.putContent(
                                    originalShareExtFile); // put back the original extension file
                            return null;
                        }
                    });
        ...
        return transactionHelper.doInTransaction(
                new RetryingTransactionCallback<Set<String>>() {
                    ...
                    public Set<String> execute() throws Throwable {
                        ...
                        ContentReader reader =
                                contentService.getReader(downloadNode, ContentModel.PROP_CONTENT);
                        try (ZipArchiveInputStream zipInputStream =
                                new ZipArchiveInputStream(reader.getContentInputStream())) {
    ...
    public DownloadStatus getDownloadStatus(final NodeRef downloadNode) {
        return AuthenticationUtil.runAsSystem(
                new RunAsWork<DownloadStatus>() {
                    @Override
                    public DownloadStatus doWork() throws Exception {
                        return transactionHelper.doInTransaction(
                                new RetryingTransactionCallback<DownloadStatus>() {
                                    @Override

 ...
 * to import snippets into the system. These snippets would otherwise have been bootstrapped by a
 * clean install.
 *
 * <p>By providing this class with a bootstrap view and an importer, it can check whether the path
...
public class GenericBootstrapPatch extends AbstractPatch {
    ...
    /**
     * Set the XPath statement that must be executed to check whether the import data is already
     * present or not.
     *
        ...
        if (checkPath != null) {
            List<NodeRef> results =
                    searchService.selectNodes(
                            rootNodeRef, checkPath, null, namespaceService, false);
            if (results.size() > 1) {

...
public class DownloadServiceIntegrationTest {
  ...
  public static AlfrescoPerson TEST_USER = new AlfrescoPerson(
    APP_CONTEXT_INIT,
    "User"
  );
  ...
  public void createContent() {
    ...
    ChildAssociationRef assoc = NODE_SERVICE.addChild(
      rootFolder,
      secondaryNode,
    ...
    try {
      modelStream =
        new ByteArrayInputStream(CUSTOM_FOLDER_MODEL_XML.getBytes("UTF-8"));
  ...
  public void createDownload() throws IOException, InterruptedException {
    ...
    final NodeRef downloadNode = DOWNLOAD_SERVICE.createDownload(
      new NodeRef[] { rootFile, rootFolder },
      true
    );
    ...
    TRANSACTION_HELPER.doInTransaction(
      new RetryingTransactionCallback<Object>() {
        ...
        public Object execute() throws Throwable {
          Map<QName, Serializable> properties = NODE_SERVICE.getProperties(
            downloadNode
          );
          Assert.assertEquals(
            Boolean.TRUE,
            properties.get(DownloadModel.PROP_RECURSIVE)
          );

          List<AssociationRef> associations = NODE_SERVICE.getTargetAssocs(
            downloadNode,
          ...
          Assert.assertEquals(
            Boolean.FALSE,
            properties.get(ContentModel.PROP_IS_INDEXED)
          );
          Assert.assertEquals(
            Boolean.FALSE,
    ...
    Assert.assertTrue(
      "Maximum creation time exceeded!",
  ...
  private long waitForDownload(final NodeRef downloadNode)
  ...
  public void deleteBeforeDateAsSystem() throws InterruptedException {
    ...
    afterNodeRef =
      DOWNLOAD_SERVICE.createDownload(new NodeRef[] { level1Folder2 }, true);
  ...
  public void deleteBeforeDateAsNormalUserFromAllSysDownloadFolders()
    ...
    NamespaceService namespaceService = APP_CONTEXT_INIT
      .getApplicationContext()
    ...
    FileFolderService fileFolderService = APP_CONTEXT_INIT
      .getApplicationContext()
    ...
    final NodeRef problematicDuplicateSystemDownloadNode = createProblematicDuplicateSystemNode(
      container,
      NODE_SERVICE,
      repositoryHelper
    );
    ...
    try {
      ...
      assertNotEquals(
        problematicDuplicateSystemDownloadNode.getId(),
        "downloads_container"
      );
      ...
      deleteDownloadsAndCheckParameters(
        beforeNodeRef,
        afterNodeRef,
        beforeTime
      );
  ...
  private void moveDownloadedFileToProblematicFolder(
    ...
    FileInfo fileInfo = TRANSACTION_HELPER.doInTransaction(
          ...
          return AuthenticationUtil.runAsSystem(
            new AuthenticationUtil.RunAsWork<NodeRef>() {
              ...
              public NodeRef doWork() throws Exception {
                NodeRef system = SystemNodeUtils.getSystemContainer(
                  nodeService,
                  repositoryHelper
                );
  ...
  private void cleanProblematicFolder(
    NodeRef problematicDuplicateSystemDownloadNode
  ) {
    TRANSACTION_HELPER.doInTransaction(
      new RetryingTransactionCallback<Void>() {
  ...
  public void workingCopies() throws InterruptedException {
    ...
    try {
      ...
      postCheckoutExpectedEntries.add(
        "level1Folder2/fileToCheckout (Working Copy).txt"
      );
      validateWorkingCopyFolder(
        postCheckoutExpectedEntries,
        level1Folder2,
        TEST_USER.getUsername()
      );
    } finally {
      ...
      new RetryingTransactionCallback<Void>() {
        ...
        public Void execute() throws Throwable {
          try {
            ...
            final List<AssociationRef> assocsList = NODE_SERVICE.getTargetAssocs(
              nodeRef,
            ...
            fail(
              "The association should have been removed successfully from the target node."
            );
          }
          return null;
        }
  ...
  public void checkNormalUsersCanNotAccessSysDownloadFolder() throws Exception {
    ...
    try {
      try {
        ...
        fail(
          "The normal user should not be able to read the sys:Download node properties"
        );
      ...
      try {
        ...
        fail(
          "The normal user should not be able to list the sys:Download node children"
        );
      } catch (AccessDeniedException e) {
        // we expect this to happen.
  ...
  private void createUser(String username) {
    PersonService personService = APP_CONTEXT_INIT
      .getApplicationContext()
      .getBean("PersonService", PersonService.class);

    MutableAuthenticationService mutableAuthenticationService = APP_CONTEXT_INIT
      .getApplicationContext()
    ...
    mutableAuthenticationService.createAuthentication(
      username,

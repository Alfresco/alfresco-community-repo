...
public class XmlMetadataExtracterTest extends TestCase {
  ...
  private static final String FILE_ALFRESCO_MODEL =
    "xml-metadata/alfresco-model-sample.xml";
  private static final String FILE_DITA_FILE =
    "xml-metadata/dita-concept-quick.xml";
  private static final String FILE_ECLIPSE_PROJECT =
    "xml-metadata/eclipse-project-sample.xml";
  private static final String FILE_EMPTY = "xml-metadata/empty-sample.xml";
  private static final String FILE_MALFORMED =
    "xml-metadata/malformed-sample.xml";

  private static final String CTX_LOCATION =
    "classpath:xml-metadata/xml-metadata-test-context.xml";
   ...
   */
  private static final ContentReader getReader(String fileName)
    throws FileNotFoundException {
    URL url =
      AbstractContentTransformerTest.class.getClassLoader()
        .getResource(fileName);
    if (url == null) {
      throw new FileNotFoundException(
        "Could not find file on classpath: " + fileName
      );
    }
  ...
  public void setUp() throws Exception {
    ...
    alfrescoModelMetadataExtracter =
      (XPathMetadataExtracter) ctx.getBean(
        "extracter.xml.AlfrescoModelMetadataExtracter"
      );
    eclipseProjectMetadataExtracter =
      (XPathMetadataExtracter) ctx.getBean(
        "extracter.xml.EclipseProjectMetadataExtracter"
      );
    ditaConceptMetadataExtracter =
      (XPathMetadataExtracter) ctx.getBean(
        "extracter.xml.DITAConceptMetadataExtracter"
      );
    rootElementNameMetadataExtracterSelector =
      (RootElementNameContentWorkerSelector<MetadataExtracter>) ctx.getBean(
        "extracter.xml.selector.RootElementSelector"
      );
    xpathMetadataExtracterSelector =
      (XPathContentWorkerSelector<MetadataExtracter>) ctx.getBean(
        "extracter.xml.selector.XPathSelector"
      );
    xmlMetadataExtracter =
      (XmlMetadataExtracter) ctx.getBean("extracter.xml.XMLMetadataExtracter");
  ...
  public void testExtractAlfresocModel() throws Exception {
    ...
    assertEquals(
      "Gavin Cornwell",
      getPropertyValue(checkProperties, ContentModel.PROP_AUTHOR)
    );
    assertEquals(
      "fm:forummodel",
      getPropertyValue(checkProperties, ContentModel.PROP_TITLE)
    );
    assertEquals(
      "Forum Model",
      getPropertyValue(checkProperties, ContentModel.PROP_DESCRIPTION)
    );
  }
  ...
  public void testDITAFileWithDoctype() throws Exception {
    ...
    assertTrue(
      "DOCTYPE should be present but wasn't",
      contents.indexOf("<!DOCTYPE") > -1
    );
  ...
  public void testEmptyFile() throws Exception {
    ...
    assertNotNull(
      "Properties changed by empty file extraction",
      checkProperties.get(ContentModel.PROP_TITLE)
    );
    assertEquals(
      "Properties changed by empty file extraction",
      getName(),
  ...
  public void testMalformedFile() throws Exception {
    ...
    // The map should be unaffected
    assertNotNull(
      "Properties changed by malformed file extraction",
      checkProperties.get(ContentModel.PROP_TITLE)
    );
    assertEquals(
      "Properties changed by malformed file extraction",
  ...
  public void testRootElementNameSelector() throws Exception {
    ...
    MetadataExtracter alfrescoModelExtracter = rootElementNameMetadataExtracterSelector.getWorker(
      alfrescoModelReader
    );
    assertNotNull("Failed to select correct extracter", alfrescoModelExtracter);
    assertTrue(
      "Incorrect extracter instance selected",
      alfrescoModelMetadataExtracter == alfrescoModelExtracter
    );
    assertFalse("Read channel not closed", alfrescoModelReader.isChannelOpen());

    // Check with an eclipse project document
    MetadataExtracter eclipseProjectExtracter = rootElementNameMetadataExtracterSelector.getWorker(
      eclipseProjectReader
    );
    assertNotNull(
      "Failed to select correct extracter",
      eclipseProjectExtracter
    );
    assertTrue(
      "Incorrect extracter instance selected",
      eclipseProjectMetadataExtracter == eclipseProjectExtracter
    );
    assertFalse(
      "Read channel not closed",
      eclipseProjectReader.isChannelOpen()
    );
  }
  ...
  public void testXpathSelector() throws Exception {
    ...
    // Check with an alfresco model document
    MetadataExtracter alfrescoModelExtracter = xpathMetadataExtracterSelector.getWorker(
      alfrescoModelReader
    );
    assertNotNull("Failed to select correct extracter", alfrescoModelExtracter);
    assertTrue(
    ...
    // Check with an eclipse project document
    MetadataExtracter eclipseProjectExtracter = xpathMetadataExtracterSelector.getWorker(
      eclipseProjectReader
    );
    assertNotNull(
      "Failed to select correct extracter",
  ...
  public void testXmlMetadataExtracter() throws Exception {
    ...
    xmlMetadataExtracter.extract(
      alfrescoModelReader,
      checkAlfrescoModelProperties
    );
    // Check the values
    assertEquals(
      "Gavin Cornwell",
      getPropertyValue(checkAlfrescoModelProperties, ContentModel.PROP_AUTHOR)
    );
    assertEquals(
      "fm:forummodel",
      getPropertyValue(checkAlfrescoModelProperties, ContentModel.PROP_TITLE)
    );
    assertEquals(
      "Forum Model",
      getPropertyValue(
    ...
    xmlMetadataExtracter.extract(
      eclipseProjectReader,
      checkEclipseProjectProperties
    );
    // Check the values
    assertEquals(
      "Repository",
      getPropertyValue(checkEclipseProjectProperties, ContentModel.PROP_TITLE)
    );
    assertEquals(
      "JavaCC Nature",
      getPropertyValue(
  ...
  public void testLifecycleOfXmlMetadataExtraction() throws Exception {
    ...
    NodeRef contentNodeRef = nodeService
      .createNode(
        rootNodeRef,
    ...
    // properties.
    String checkTitle = (String) nodeService.getProperty(
      contentNodeRef,
      ContentModel.PROP_TITLE
    );
    String checkDescription = (String) nodeService.getProperty(
      contentNodeRef,
      ContentModel.PROP_DESCRIPTION
    );
  ...
  private String getPropertyValue(PropertyMap properties, QName qname) {
    ...
    assertNotNull(
      "Property " + qname + " missing, properties are " + properties.keySet(),
      val
    );

    if (val instanceof String) return (String) val;
    if (val instanceof MLText) return ((MLText) val).getDefaultValue();
    return val.toString();
  }
}

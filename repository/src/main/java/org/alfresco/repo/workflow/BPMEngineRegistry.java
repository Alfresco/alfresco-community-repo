 ...
 * BPM Engine Registry
 *
 * <p>Responsible for managing the list of registered BPM Engines for the following components:
 *
 * <p>- Workflow Component - Task Component
 *
 * @author davidc
 */
public class BPMEngineRegistry {
    ...
    public void registerWorkflowComponent(String engineId, WorkflowComponent engine) {
        if (workflowComponents.containsKey(engineId)) {
            throw new WorkflowException(
                    "Workflow Component already registered for engine id '" + engineId + "'");
        }
        workflowComponents.put(engineId, engine);
        if (logger.isDebugEnabled())
            logger.debug(
                    "Registered Workflow Component '" + engineId + "' (" + engine.getClass() + ")");
    }
    ...
    public void registerTaskComponent(String engineId, TaskComponent engine) {
        if (taskComponents.containsKey(engineId)) {
            throw new WorkflowException(
                    "Task Component already registered for engine id '" + engineId + "'");
        }
        taskComponents.put(engineId, engine);
        if (logger.isDebugEnabled())
            logger.debug(
                    "Registered Task Component '" + engineId + "' (" + engine.getClass() + ")");
    }
    ...
    /**
     * Returns <code>true</code> if the globalId parameter is a valid global Id for the given
     * engineId.
     *

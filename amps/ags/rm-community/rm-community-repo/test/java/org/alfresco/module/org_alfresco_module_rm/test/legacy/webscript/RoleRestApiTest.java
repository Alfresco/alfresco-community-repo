...
public class RoleRestApiTest extends BaseRMWebScriptTestCase implements RecordsManagementModel {
    ...
    public void testGetRoles() throws Exception {
        ...
        filePlanRoleService.createRole(
                filePlan, role2, "My Test Role Too", getListOfCapabilities(5));

        // The user can either enter a plain text label or a key to look up in a property file.
        filePlanRoleService.createRole(
                filePlan, role3, "System Administrator", getListOfCapabilities(5));

        // create test group
        String groupName = GUID.generate();
        String group =
                authorityService.createAuthority(AuthorityType.GROUP, groupName, "monkey", null);
    ...
    public void testPutRole() throws Exception {
        ...
        try {
            ...
            sendRequest(
                    new PutRequest(
                            getRolesUrlBySite() + "/cheese", obj.toString(), APPLICATION_JSON),
                    404);
        } finally {
            // Clean up
            filePlanRoleService.deleteRole(filePlan, role1);

...
/**
 * This patch creates a new "Records Management Behavior Scripts" folder and moves existing behavior
 * scripts from the old "Records Management Scripts" folder to the new folder. This is to compensate
 * for any non-behavior RM scripts so that they can live in the old "Records Management Scripts"
 * folder for its intended purpose and be picked up by the execute script rule action.
 *
 * @author Craig Tan
 * @since 2.1
 */
public class RMv21BehaviorScriptsPatch extends RMv21PatchComponent implements BeanNameAware {
    ...
    protected static final NodeRef OLD_BEHAVIOR_SCRIPTS_FOLDER =
            new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, "rm_scripts");

    /** new behavior scripts folder root lookup */
    private static NodeRef newBehaviorScriptsFolder =
            new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, "rm_behavior_scripts");
    ...
    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.patch.compatibility.ModulePatchComponent#executePatch()
     */
    @Override
    protected void executePatch() {
        ...
        if (!nodeService.exists(newBehaviorScriptsFolder)) {
            ...
            newBehaviorScriptsFolderProps.put(
                    ContentModel.PROP_NODE_UUID, newBehaviorScriptsNodeUUID);
            newBehaviorScriptsFolderProps.put(ContentModel.PROP_NAME, newBehaviorScriptsFolderName);
            newBehaviorScriptsFolderProps.put(
                    ContentModel.PROP_TITLE, newBehaviorScriptsFolderName);
            newBehaviorScriptsFolderProps.put(
                    ContentModel.PROP_DESCRIPTION,
                    "Scripts intended for execution in response to RM events.");

            newBehaviorScriptsFolder =
                    nodeService
                            .createNode(
                                    RM_CONFIG,
                                    ...
                                    QName.createQName(
                                            ...
                                            newBehaviorScriptsAssocQName),
                                    ContentModel.TYPE_FOLDER,
                                    newBehaviorScriptsFolderProps)
                            .getChildRef();
        }

        // move to the new behavior scripts folder if the old behavior scripts folder exists and
        // contains files
                                    ...
                                    new RetryingTransactionCallback<Void>() {
                                        public Void execute() throws Throwable {
                                            ...
                                            nodeService.setProperty(
                                                    ...
                                                    ContentModel.PROP_DESCRIPTION,
                                                    "Scripts specific to RM that can also be"
                                                            + " executed by RM rules.");

                                            // Move files from RM Scripts folder to RM Behavior
                                            // Scripts folder.
                                            List<FileInfo> oldBehaviorScripts =
                                                    fileFolderService.listFiles(
                                                            OLD_BEHAVIOR_SCRIPTS_FOLDER);

                                            if (oldBehaviorScripts != null
                                                    && !oldBehaviorScripts.isEmpty()) {
                                                if (LOGGER.isDebugEnabled()) {
                                                    LOGGER.debug(
                                                            " ... moving files from RM Scripts"
                                                                + " folder to RM Behavior Scripts"
                                                                + " folder");
                                                }

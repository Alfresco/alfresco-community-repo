...
/**
 * This class contains tests for the class <code>{@link SolrFacetHelper}</code> and <code>
 * {@link FacetLabelDisplayHandler}</code>.
 *
...
public class SolrFacetQueriesDisplayHandlersTest {
    ...
    @ClassRule public static ApplicationContextInit context = new ApplicationContextInit();

    // Rule will allow us to create Share sites and users and have them automatically cleaned up for
    // us.
    @Rule public TemporarySites temporarySites = new TemporarySites(context);
    ...
    public void testCreateFacetQueryFromSearchQuery() throws Exception {
        String searchQueryWithCreatedDate =
                "query=(test  AND"
                    + " ({http://www.alfresco.org/model/content/1.0}created:(\"NOW/DAY-7DAYS\"..\"NOW/DAY+1DAY\""
                    + " )) AND (+TYPE:\"cm:content\" OR +TYPE:\"cm:folder\")) AND"
                    + " -TYPE:\"cm:thumbnail\" AND -TYPE:\"cm:failedThumbnail\" AND"
                    + " -TYPE:\"cm:rating\" AND -TYPE:\"st:site\" AND -ASPECT:\"st:siteContainer\""
                    + " AND -ASPECT:\"sys:hidden\" AND -cm:creator:system,"
        ...
        assertEquals(
                "@{http://www.alfresco.org/model/content/1.0}created:[NOW/DAY-7DAYS TO"
                        + " NOW/DAY+1DAY]",
                result);
        ...
        String searchQueryWithCreatedAndModifiedDate =
                "query=(test  AND"
                    + " ({http://www.alfresco.org/model/content/1.0}created:(\"NOW/DAY-7DAYS\"..\"NOW/DAY+1DAY\""
                    + " ) AND"
                    + " ({http://www.alfresco.org/model/content/1.0}modified:(\"NOW/DAY-1DAY\"..\"NOW/DAY+1DAY\""
                    + " )) AND (+TYPE:\"cm:content\" OR +TYPE:\"cm:folder\")) AND"
                    + " -TYPE:\"cm:thumbnail\" AND -TYPE:\"cm:failedThumbnail\" AND"
                    + " -TYPE:\"cm:rating\" AND -TYPE:\"st:site\" AND -ASPECT:\"st:siteContainer\""
                    + " AND -ASPECT:\"sys:hidden\" AND -cm:creator:system,"
        ...
        result =
                helper.createFacetQueriesFromSearchQuery(
                        fields[1], searchQueryWithCreatedAndModifiedDate);
        assertNotNull(result);
        assertEquals(
                "@{http://www.alfresco.org/model/content/1.0}modified:[NOW/DAY-1DAY TO"
                        + " NOW/DAY+1DAY]",
                result);
    }
    ...
    public void testGetDefaultFacetQueries() throws Exception {
        ...
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}created:[NOW/DAY-1DAY TO"
                                + " NOW/DAY+1DAY]"));
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}created:[NOW/DAY-7DAYS TO"
        ...
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}created:[NOW/DAY-1MONTH TO"
                                + " NOW/DAY+1DAY]"));
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}created:[NOW/DAY-6MONTHS TO"
                                + " NOW/DAY+1DAY]"));
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}created:[NOW/DAY-1YEAR TO"
                                + " NOW/DAY+1DAY]"));
        ...
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}modified:[NOW/DAY-7DAYS TO"
                                + " NOW/DAY+1DAY]"));
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}modified:[NOW/DAY-1MONTH TO"
                                + " NOW/DAY+1DAY]"));
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}modified:[NOW/DAY-6MONTHS TO"
                                + " NOW/DAY+1DAY]"));
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}modified:[NOW/DAY-1YEAR TO"
                                + " NOW/DAY+1DAY]"));
        ...
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}content.size:[10240 TO"
                                + " 102400]"));
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}content.size:[102400 TO"
                                + " 1048576]"));
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}content.size:[1048576 TO"
                                + " 16777216]"));
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}content.size:[16777216 TO"
                                + " 134217728]"));
        assertTrue(
                result.contains(
                        "@{http://www.alfresco.org/model/content/1.0}content.size:[134217728 TO"
                                + " MAX]"));
    }
    ...
    public void testGetUserNameDisplayHandler() throws Exception {
        FacetLabelDisplayHandler userNameHandler =
                displayHandlerRegistry.getDisplayHandler(
    ...
    public void testGetMimetypeDisplayHandler() throws Exception {
        // Mimetype handler
        FacetLabelDisplayHandler mimeTypeHandler =
                displayHandlerRegistry.getDisplayHandler(
        ...
        assertEquals(
                "someMimetype123 is not a registered mimetype, hence, the handler should return the"
                        + " passed-in mimetype.",
                "someMimetype123",
                mimetype.getLabel());
    ...
    public void testGetCreatedDateBucketsDisplayHandler() throws Exception {
        ...
        FacetLabelDisplayHandler dateBucketeHandler =
                displayHandlerRegistry.getDisplayHandler(createdDateField);
        assertNotNull(dateBucketeHandler);

        FacetLabel dateLabel =
    ...
    public void testGetContentSizeBucketsDisplayHandler() throws Exception {
        ...
        FacetLabelDisplayHandler contentSizeBucketeHandler =
                displayHandlerRegistry.getDisplayHandler(contentSizeField);

...
public class AdminUserPatch extends AbstractPatch implements InitializingBean {
  ...
  private static final String MSG_START =
    "patch.updateAdminUserWhenDefault.start";
  private static final String MSG_RESULT =
    "patch.updateAdminUserWhenDefault.result";
  private static final String MSG_NO_ACTION =
    "patch.updateAdminUserWhenDefault.noaction";
  private static final Log logger = LogFactory.getLog(AdminUserPatch.class);

  public static String DEFAULT_SHA =
    "f378d5d7b947d5c26f478e21819e7ec3a6668c8149b050d086c64447bc40173b";
  ...
  protected String applyInternal() throws Exception {
    ...
    if (authenticationDao != null) {
      ...
      final NodeRef userNodeRef = authenticationDao.getUserOrNull(
        adminUsername
      );

      if (userNodeRef != null) {
        Map<QName, Serializable> userProperties = nodeService.getProperties(
          userNodeRef
        );
        String sha256 = (String) userProperties.get(
        ...
        if (DEFAULT_SHA.equals(sha256)) {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug(
              "Removing password sha256 hash for user: " + adminUsername
            );
          }

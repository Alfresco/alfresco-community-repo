...
public class ActionsImpl implements Actions {
    ...
    private ActionDefinition getActionDefinition(
        ...
        List<ActionDefinition.ParameterDefinition> paramDefs =
                actionDefinitionId.getParameterDefinitions().stream()
                        .map(this::toModel)
    ...
    private CollectionWithPagingInfo<ActionDefinition> actionDefinitions(
        ...
        List<ActionDefinition> sortedPage =
                actionDefinitions.stream()
                        .map(
                                actionDefinition -> {
                                    List<ActionDefinition.ParameterDefinition> paramDefs =
                                            actionDefinition.getParameterDefinitions().stream()
                                                    .map(this::toModel)
                                                    .collect(Collectors.toList());
                                    return new ActionDefinition(
        ...
        return CollectionWithPagingInfo.asPaged(
                params.getPaging(), sortedPage, hasMoreItems, actionDefinitions.size());
    }
    ...
    public Action executeAction(Action action, Parameters parameters) {
        ...
        if (action.getTargetId() != null && !action.getTargetId().isEmpty()) {
            ...
            actionedUponNodeRef =
                    new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, action.getTargetId());

...
public class RatingsGet extends AbstractRatingWebScript {
  ...
  protected Map<String, Object> executeImpl(
    ...
    for (String schemeName : ratingService.getRatingSchemes().keySet()) {
      final Rating ratingByCurrentUser = ratingService.getRatingByCurrentUser(
        nodeRef,
        schemeName
      );
      ...
      averageRatings.put(
        schemeName,
        ratingService.getAverageRating(nodeRef, schemeName)
      );
      ratingsTotals.put(
        schemeName,
        ratingService.getTotalRating(nodeRef, schemeName)
      );
      ratingsCounts.put(
        schemeName,
        ratingService.getRatingsCount(nodeRef, schemeName)
      );
    }

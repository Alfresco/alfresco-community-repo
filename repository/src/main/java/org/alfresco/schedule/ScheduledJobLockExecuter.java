...
/**
 * This class encapsulates the {@link org.alfresco.repo.lock.JobLockService JobLockService} usage in
 * order to guarantee that a job is not executed simultaneously in more than one node in a cluster.
 * After instantiated passing in constructor {@link org.alfresco.schedule.AbstractScheduledLockedJob
 * job} to be executed, as well as the name of the to be locked job and the {@link
 * org.alfresco.repo.lock.JobLockService JobLockService}, the execute method of this class will
 * execute the job taking care of all cluster aware lockings.
 *
 * <p>This code is based on original code by Derek Hulley on {@link
 * org.alfresco.repo.content.cleanup.ContentStoreCleaner ContentStoreCleaner}, extracting the
 * generic locking code in order to be reused and avoid code duplication.
 *
...
public class ScheduledJobLockExecuter {
    ...
    private static ThreadLocal<Pair<Long, String>> lockThreadLocal =
            new ThreadLocal<Pair<Long, String>>();
     ...
     */
    public ScheduledJobLockExecuter(
            JobLockService jobLockService, String name, AbstractScheduledLockedJob job) {
    ...
    /**
     * It will execute the {@link AbstractScheduledLockedJob job} passed on instantiation taking
     * care of all cluster aware lockings.
     *
    ...
    public void execute(JobExecutionContext jobContext) throws JobExecutionException {
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        String.format("   Job %s already underway.", lockQName.getLocalName()));
            }

...
public class SearchDAOImpl implements SearchDAO {
    ...
    public List<AclChangeSet> getAclChangeSets(
        ...
        Long deletedTypeQNameId =
                deletedTypeQNamePair == null ? -1L : deletedTypeQNamePair.getFirst();
    ...
    public List<Acl> getAcls(List<Long> aclChangeSetIds, Long minAclId, int maxResults) {
        ...
        if (maxResults <= 0 || maxResults == Integer.MAX_VALUE) {
            ...
            source =
                    template.selectList(
                            SELECT_ACLS_BY_CHANGESET_IDS, params, new RowBounds(0, maxResults));
        }
    ...
    public List<Node> getNodes(
            NodeParameters nodeParameters, QName shardPropertyQName, QName shardPropertyTypeName) {
        ...
        if (nodeParameters.getMaxResults() != 0
                && nodeParameters.getMaxResults() != Integer.MAX_VALUE) {
            return template.selectList(
                    SELECT_NODES, params, new RowBounds(0, nodeParameters.getMaxResults()));
        } else {

...
public class AttributeServiceImpl implements AttributeService {
    ...
    private Serializable[] normalizeKeys(Serializable... keys) {
        ...
        return new Serializable[] {
            keys[0], keys.length > 1 ? keys[1] : null, keys.length > 2 ? keys[2] : null
        };
    }
    ...
    public boolean exists(Serializable... keys) {
        ...
        Pair<Long, Long> pair =
                propertyValueDAO.getPropertyUniqueContext(keys[0], keys[1], keys[2]);
    ...
    public void getAttributes(final AttributeQueryCallback callback, Serializable... keys) {
        PropertyUniqueContextCallback propertyUniqueContextCallback =
                new PropertyUniqueContextCallback() {
                    ...
                    public void handle(Long id, Long valueId, Serializable[] resultKeyIds) {
                        ...
                        for (int i = 0; i < resultKeyIds.length; i++) {
                            if (resultKeyIds[i] != null) {
                                Pair<Long, Serializable> keyValuePair =
                                        propertyValueDAO.getPropertyValueById(
                                                (Long) resultKeyIds[i]);
                                resultsKeyValues[i] =
                                        (keyValuePair != null ? keyValuePair.getSecond() : null);
                            }
                        }
    ...
    public void updateOrCreateAttribute(
        ...
        Pair<Long, Long> pair =
                propertyValueDAO.getPropertyUniqueContext(keyBefore1, keyBefore2, keyBefore3);
        try {
            if (pair == null) {
                pair =
                        propertyValueDAO.createPropertyUniqueContext(
                                keyAfter1, keyAfter2, keyAfter3, null);
            } else {
                Long id = pair.getFirst();
                propertyValueDAO.updatePropertyUniqueContextKeys(
                        id, keyAfter1, keyAfter2, keyAfter3);
            }
        } catch (PropertyUniqueConstraintViolation e) {
        ...
        if (logger.isDebugEnabled()) {
            ...
            logger.debug(
                    "Updated attribute: \n"
                            + "   Before:   "
                            + Arrays.asList(keysBefore)
                            + "\n"
                            + "   After:    "
                            + Arrays.asList(keysAfter));
        }
    }

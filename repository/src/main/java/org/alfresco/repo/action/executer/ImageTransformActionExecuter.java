 ...
 * backward compatibility in 15/04/2008). Now removed. Did not find any internal usage (22/11/2012)
 * but it is possible that there may be some in Javascript calling ScriptNode or custom code using
 * the context passed to the ImageRenderingEngine. That said, it will almost always be ImagMagick
 * that does the transform so it will still be used. These command line options were to overcome
 * issues with ImageMagick. Other transformers will just ignore them.
 *
    ...
    protected void addParameterDefinitions(List<ParameterDefinition> paramList) {
        ...
        paramList.add(
                new ParameterDefinitionImpl(
                        ...
                        false,
                        getParamDisplayLabel(PARAM_CONVERT_COMMAND)));
    }

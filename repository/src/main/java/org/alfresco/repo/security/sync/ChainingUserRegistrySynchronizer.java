...
/**
 * A <code>ChainingUserRegistrySynchronizer</code> is responsible for synchronizing Alfresco's local
 * user (person) and group (authority) information with the external subsystems in the
 * authentication chain (most typically LDAP directories). When the {@link #synchronize(boolean,
 * boolean)} method is called, it visits each {@link UserRegistry} bean in the 'chain' of
 * application contexts, managed by a {@link ChildApplicationContextManager}, and compares its
 * timestamped user and group information with the local users and groups last retrieved from the
 * same source. Any updates and additions made to those users and groups are applied to the local
 * copies. The ordering of each {@link UserRegistry} in the chain determines its precedence when it
 * comes to user and group name collisions. The {@link JobLockService} is used to ensure that in a
 * cluster, no two nodes actually run a synchronize at the same time.
 *
 * <p>The <code>force</code> argument determines whether a complete or partial set of information is
 * queried from the {@link UserRegistry}. When <code>true</code> then <i>all</i> users and groups
 * are queried. With this complete set of information, the synchronizer is able to identify which
 * users and groups have been deleted, so it will delete users and groups as well as update and
 * create them. Since processing all users and groups may be fairly time consuming, it is
 * recommended this mode is only used by a background scheduled synchronization job. When the
 * argument is <code>false</code> then only those users and groups modified since the most recent
 * modification date of all the objects last queried from the same {@link UserRegistry} are
 * retrieved. In this mode, local users and groups are created and updated, but not deleted (except
 * where a name collision with a lower priority {@link UserRegistry} is detected). This
 * 'differential' mode is much faster, and by default is triggered on subsystem startup and also by
 * {@link #createMissingPerson(String)} when a user is successfully authenticated who doesn't yet
 * have a local person object in Alfresco. This should mean that new users and their group
...
public class ChainingUserRegistrySynchronizer extends AbstractLifecycleBean
    ...
    /**
     * The name of the lock used to ensure that a synchronize does not run on more than one node at
     * the same time.
     */
    public static final QName LOCK_QNAME =
            QName.createQName(
                    NamespaceService.SYSTEM_MODEL_1_0_URI, "ChainingUserRegistrySynchronizer");

    /**
     * The time this lock will persist for in the database (now only 2 minutes but refreshed at
     * regular intervals).
     */
     ...
     *
     * @param attributeService the new attribute service
     */
     ...
     *
     * @param autoCreatePeopleOnLogin <code>true</code> if we should auto create a missing person on
     *     log in
     */
     ...
     *
     * @param syncWhenMissingPeopleLogIn <code>true</code> if we should trigger a sync when missing
     *     people log in
     */
     ...
     *
     * @param syncOnStartup <code>true</code> if we should trigger a sync on startup
     */
     ...
     *
     * @param loggingInterval the number of entries to process before reporting progress or zero to
     ...
     *
     * @param allowDeletions If <b>true</b> the entries are deleted from alfresco. If <b>false</b>
     *     then they are unlinked from their LDAP authentication zone but remain within alfresco.
     */
    ...
    /**
     * Controls whether to query for users and groups that have been deleted in LDAP. For large LDAP
     * directories the delete query is expensive and time consuming, needing to read the entire LDAP
     * directory. By default is set to true.
     *
     * @param syncDelete If <b>false</b> then LDAP sync does not even attempt to search for deleted
     *     users.
     */
    ...
    public SynchronizeDiagnostic testSynchronize(String authenticatorName) {
        ...
        if (instanceIds.contains(authenticatorName)) {
            ...
            ApplicationContext context =
                    this.applicationContextManager.getApplicationContext(authenticatorName);
        ...
        throw new AuthenticationException(
                "authentication.err.validation.authenticator.notfound", params);
    }
    ...
    private void synchronizeInternal(
            boolean forceUpdate, boolean isFullSync, final boolean splitTxns) {
        if (ChainingUserRegistrySynchronizer.logger.isDebugEnabled()) {
            ...
            if (this.transactionService.isReadOnly()) {
                ChainingUserRegistrySynchronizer.logger.warn(
                        "Unable to proceed with user registry synchronization. Repository is read"
                                + " only.");
                return;
            }
        }
        ...
        // Create a background executor that will refresh our lock. This means we can request a lock
        // with a relatively
        // small persistence time and not worry about it lasting after server restarts. Note we use
        // an independent
        // executor because this is a compound operation that spans accross multiple batch
        // processors.
        ...
        try {
            ...
            try {
                if (splitTxns) {
                    // If this is an automated sync on startup or scheduled sync, don't even wait
                    // around for the lock.
                                    ...
                                    .doInTransaction(
                                            new RetryingTransactionCallback<String>() {
                                                public String execute() throws Throwable {
                                                    return ChainingUserRegistrySynchronizer.this
                ...
                ChainingUserRegistrySynchronizer.logger.warn(
                        "User registry synchronization already running in another thread."
                                + " Synchronize aborted");
                return;
            }
            ...
            lockRefresher.scheduleAtFixedRate(
                    new Runnable() {
                        public void run() {
                            ChainingUserRegistrySynchronizer.this
                                    .transactionService
                                    .getRetryingTransactionHelper()
                                    .doInTransaction(
                    ...
                    ChainingUserRegistrySynchronizer.LOCK_TTL / 2,
                    ChainingUserRegistrySynchronizer.LOCK_TTL / 2,
            ...
            // Work out the set of all zone IDs in the authentication chain so that we can decide
            // which users / groups
            // need 're-zoning'
            ...
            for (String id : instanceIds) {
                ...
                try {
                    ApplicationContext context =
                            this.applicationContextManager.getApplicationContext(id);
                    plugin = (UserRegistry) context.getBean(this.sourceBeanName);
                } catch (RuntimeException e) {
                    // The bean doesn't exist or this subsystem won't start. The reason would have
                    // been logged. Ignore and continue.
                    continue;
                }
                ...
                if (plugin != null) {
                    ...
                    if (ChainingUserRegistrySynchronizer.logger.isDebugEnabled()) {
                        mbeanServer =
                                (MBeanServerConnection)
                                        getApplicationContext().getBean("alfrescoMBeanServer");
                        try {
                            ...
                            if (mbeanServer != null && mbeanServer.isRegistered(name)) {
                                ...
                                for (MBeanAttributeInfo attribute : attributes) {
                                    Object value =
                                            mbeanServer.getAttribute(name, attribute.getName());
                    ...
                    if (ChainingUserRegistrySynchronizer.logger.isInfoEnabled()) {
                        ChainingUserRegistrySynchronizer.logger.info(
                                "Synchronizing users and groups with user registry '" + id + "'");
                    }
                    if (isFullSync && ChainingUserRegistrySynchronizer.logger.isWarnEnabled()) {
                        if (allowDeletions) {
                            ...
                            ChainingUserRegistrySynchronizer.logger.warn(
                                    "Deletions are disabled. Users and groups removed from this"
                                        + " registry will be logged only and will remain in the"
                                        + " repository. Users previously found in a different"
                                        + " registry will be moved in the repository rather than"
                                        + " recreated.");
                        }
                    }
                    // Work out whether we should do the work in a separate transaction (it's most
                    // performant if we
                    // bunch it into small transactions, but if we are doing a sync on login, it has
                    // to be the same
                    // transaction)
                    boolean requiresNew =
                            splitTxns
                    ...
                    try {
                        ...
                        this.applicationEventPublisher.publishEvent(
                                new SynchronizeDirectoryEndEvent(this, id));
            ...
            notifySyncEnd(e);
            ChainingUserRegistrySynchronizer.logger.error(
                    "Synchronization aborted due to error", e);
            throw e;
        } finally {
            // Release the lock if necessary
            if (lockToken != null) {
                // Cancel the lock refresher
                // Because we may hit a perfect storm when trying to interrupt workers in their
                // unsynchronized getTask()
                // method we can't wait indefinitely and may have to retry the shutdown
                int trys = 0;
                do {
                    lockRefresher.shutdown();
                    try {
                        lockRefresher.awaitTermination(
                                ChainingUserRegistrySynchronizer.LOCK_TTL, TimeUnit.MILLISECONDS);
                ...
                if (!lockRefresher.isTerminated()) {
                    ...
                    ChainingUserRegistrySynchronizer.logger.error(
                            "Failed to shut down lock refresher");
                }
                ...
                this.transactionService
                        ...
                        .doInTransaction(
                                new RetryingTransactionCallback<Object>() {
                                    public Object execute() throws Throwable {
                                        ChainingUserRegistrySynchronizer.this.jobLockService
                                                .releaseLock(
                                                        token,
                                                        ChainingUserRegistrySynchronizer
                                                                .LOCK_QNAME);
    ...
    public Set<QName> getPersonMappedProperties(String username) {
        ...
        // Visit the user registries in priority order and return the person mapping of the first
    ...
    /**
     * Synchronizes local groups and users with a {@link UserRegistry} for a particular zone,
     * optionally handling deletions.
     *
     * @param zone the zone id. This identifier is used to tag all created groups and users, so that
     *     in the future we can tell those that have been deleted from the registry.
     * @param userRegistry the user registry for the zone.
     ...
     *     maximum performance? If <code>true</code>, users and groups are created/updated in
     *     batches for increased performance. If <code>false</code>, all users and groups are
     *     processed in the current transaction. This is required if calling synchronously (e.g. in
     *     response to an authentication event in the same transaction).
     * @param visitedZoneIds the set of zone ids already processed. These zones have precedence over
     *     the current zone when it comes to group name 'collisions'. If a user or group is queried
     *     that already exists locally but is tagged with one of the zones in this set, then it will
     *     be ignored as this zone has lower priority.
     * @param allZoneIds the set of all zone ids in the authentication chain. Helps us work out
     *     whether the zone information recorded against a user or group is invalid for the current
     *     authentication chain and whether the user or group needs to be 're-zoned'.
     */
    private void syncWithPlugin(
            final String zone,
        ...
        if (ChainingUserRegistrySynchronizer.logger.isInfoEnabled()) {
            if (lastModified == null) {
                ChainingUserRegistrySynchronizer.logger.info(
                        "Retrieving all groups from user registry '" + zone + "'");
            } else {
                ChainingUserRegistrySynchronizer.logger.info(
                        "Retrieving groups changed since "
        ...
        // First, analyze the group structure. Create maps of authorities to their parents for
        ...
        final BatchProcessor<NodeDescription> groupProcessor =
                new BatchProcessor<NodeDescription>(
                        SyncProcess.GROUP_ANALYSIS.getTitle(zone),
        ...
        class Analyzer extends BaseBatchProcessWorker<NodeDescription> {
            ...
            public void process(NodeDescription group) throws Throwable {
                ...
                String groupShortName =
                        ChainingUserRegistrySynchronizer.this.authorityService.getShortName(
                                groupName);
                Set<String> groupZones =
                        ChainingUserRegistrySynchronizer.this.authorityService.getAuthorityZones(
                                groupName);
                ...
                if (groupZones == null) {
                    ...
                    // Check whether the group is in any of the higher priority authentication chain
                    // zones
                    ...
                    if (groupZones.contains(zoneId)) {
                        ...
                        // Deletions are disallowed or the group exists, but not in a zone that's in
                        // the authentication
                        ...
                        if (ChainingUserRegistrySynchronizer.logger.isWarnEnabled()) {
                            ChainingUserRegistrySynchronizer.logger.warn(
                                    "Updating group '"
                                            + groupShortName
                                            + "'. This group will in future be assumed to originate"
                                            + " from user registry '"
                                            + zone
                                            + "'.");
                        }
                        ...
                        if (ChainingUserRegistrySynchronizer.logger.isWarnEnabled()) {
                            ChainingUserRegistrySynchronizer.logger.warn(
                                    "Recreating occluded group '"
                                            + groupShortName
                                            + "'. This group was previously created through"
                                            + " synchronization with a lower priority user"
                                            + " registry.");
                        }
                        ChainingUserRegistrySynchronizer.this.authorityService.deleteAuthority(
                                groupName);
            ...
            // Recursively walks and caches the authorities relating to and from this group so that
            // we can later detect potential cycles
            ...
            private synchronized void updateGroup(NodeDescription group, boolean existed) {
                ...
                String groupDisplayName =
                        (String) groupProperties.get(ContentModel.PROP_AUTHORITY_DISPLAY_NAME);
                ...
                // Divide the child associations into person and group associations, dealing with
                // case sensitivity
                ...
                if (existed) {
                    ...
                    ChainingUserRegistrySynchronizer.this.authorityService.setAuthorityDisplayName(
                ...
                // Mark as created if new
                    ...
                    // Make sure each group to be created features in the association deletion map
                    // (as these are handled in the same phase)
                ...
                for (String child : newChildPersons) {
                    // Make sure each person with association changes features as a key in the
                    // deletion map
                    recordParentAssociationDeletion(child, null);
                    recordParentAssociationCreation(child, groupName);
                }
                for (String child : newChildGroups) {
                    // Make sure each group with association changes features as a key in the
                    // deletion map
                    recordParentAssociationDeletion(child, null);
                    recordParentAssociationCreation(child, groupName);
                }
            ...
            private void recordParentAssociationDeletion(String child, String parent) {
                ...
                if (AuthorityType.getAuthorityType(child) == AuthorityType.USER) {
                    ...
                    // Reflect the change in the map of final group associations (for cycle
                    // detection later)
            ...
            private void recordParentAssociationCreation(String child, String parent) {
                Map<String, Set<String>> parentAssocs =
                        AuthorityType.getAuthorityType(child) == AuthorityType.USER
            ...
            private void validateGroupParentAssocsToCreate() {
                Iterator<Map.Entry<String, Set<String>>> i =
                        this.groupParentAssocsToCreate.entrySet().iterator();
                while (i.hasNext()) {
                            ...
                            if (logger.isWarnEnabled()) {
                                ChainingUserRegistrySynchronizer.logger.warn(
                                        "Not adding group '"
                                                ...
                                                + "' to group '"
                                                + ChainingUserRegistrySynchronizer.this
                                                        .authorityService.getShortName(parent)
                ...
                // Sort the group associations in parent-first order (root groups first) to minimize
                // reindexing overhead
                Map<String, Set<String>> sortedGroupAssociations =
                        new LinkedHashMap<String, Set<String>>(
                                this.groupParentAssocsToCreate.size() * 2);
                Deque<String> visited = new LinkedList<String>();
                for (String authority : this.groupParentAssocsToCreate.keySet()) {
                    visitGroupParentAssocs(
                            visited,
                            authority,
                            this.groupParentAssocsToCreate,
            ...
            /**
             * Visits the given authority by recursively visiting its parents in associationsOld and
             * then adding the authority to associationsNew. Used to sort associationsOld into
             * 'parent-first' order to minimize reindexing overhead.
             *
             * @param visited The ancestors that form the path to the authority to visit. Allows
             *     detection of cyclic child associations.
             * @param authority the authority to visit
             * @param associationsOld the association map to sort
             * @param associationsNew the association map to add to in parent-first order
             */
            private boolean visitGroupParentAssocs(
                    Deque<String> visited,
                    String authority,
                    Map<String, Set<String>> associationsOld,
                    ...
                    if (!associationsNew.containsKey(authority)) {
                        ...
                        if (oldParents != null) {
                            ...
                            for (String parent : oldParents) {
                                if (visitGroupParentAssocs(
                                        visited, parent, associationsOld, associationsNew)) {
            ...
            private Set<String> newPersonSet() {
                return ChainingUserRegistrySynchronizer.this.personService
            ...
            private void logRetainParentAssociations(
                    Map<String, Set<String>> parentAssocs, Set<String> toRetain) {
            ...
            private void processGroups(
                    UserRegistry userRegistry, boolean isFullSync, boolean splitTxns) {
                // MNT-12454 fix. If syncDelete is false, there is no need to pull all users and all
                // groups from LDAP during the full synchronization.
                if ((syncDelete || !groupsToCreate.isEmpty())
                    ...
                    ChainingUserRegistrySynchronizer.this
                            ...
                            .doInTransaction(
                                    new RetryingTransactionCallback<Void>() {
                                        public Void execute() throws Throwable {
                                            allZonePersons.addAll(
                                                    ChainingUserRegistrySynchronizer.this
                                                            .authorityService
                                                            .getAllAuthoritiesInZone(
                                                                    zoneId, AuthorityType.USER));
                                            allZoneGroups.addAll(
                                                    ChainingUserRegistrySynchronizer.this
                                                            .authorityService
                                                            .getAllAuthoritiesInZone(
                                                                    zoneId, AuthorityType.GROUP));
                                            return null;
                                        }
                                    },
                                    true,
                    ...
                    if (isFullSync) {
                        ...
                        if (allowDeletions) {
                            ...
                            BatchProcessor<String> groupScanner =
                                    new BatchProcessor<String>(
                                            ...
                                            ChainingUserRegistrySynchronizer.this.workerThreads,
                                            20,
                                            ChainingUserRegistrySynchronizer.this
                                                    .applicationEventPublisher,
                                            ChainingUserRegistrySynchronizer.logger,
                                            ChainingUserRegistrySynchronizer.this.loggingInterval);
                            groupScanner.process(
                                    new BaseBatchProcessWorker<String>() {
                                        ...
                                        public void process(String authority) throws Throwable {
                                            // MNT-12454 fix. Modifies an authority's zone. Move
                                            // authority from AUTH.EXT.LDAP1 to AUTH.ALF.
                                            updateAuthorityZones(
                                                    authority,
                    ...
                    // Pruning person associations will have to wait until we have passed over all
                    // persons and built up
                    // this set
                    ...
                    if (!this.groupParentAssocsToDelete.isEmpty()) {
                        ...
                        BatchProcessor<Map.Entry<String, Set<String>>> groupCreator =
                                new BatchProcessor<Map.Entry<String, Set<String>>>(
                                        SyncProcess.GROUP_CREATION_AND_ASSOCIATION_DELETION
                                                .getTitle(zone),
                                        ChainingUserRegistrySynchronizer.this.transactionService
                                                .getRetryingTransactionHelper(),
                                        ...
                                        ChainingUserRegistrySynchronizer.logger,
                                        ChainingUserRegistrySynchronizer.this.loggingInterval);
                        groupCreator.process(
                                new BaseBatchProcessWorker<Map.Entry<String, Set<String>>>() {
                                    ...
                                    public void process(Map.Entry<String, Set<String>> entry)
                                        ...
                                        String groupDisplayName =
                                                Analyzer.this.groupsToCreate.get(child);
                                        if (groupDisplayName != null) {
                                            String groupShortName =
                                                    ChainingUserRegistrySynchronizer.this
                                                            .authorityService.getShortName(child);
                                            if (ChainingUserRegistrySynchronizer.logger
                                                    .isDebugEnabled()) {
                                                ChainingUserRegistrySynchronizer.logger.debug(
                                                        "Creating group '" + groupShortName + "'");
                                            }
                                            // create the group
                                            ChainingUserRegistrySynchronizer.this.authorityService
                                                    .createAuthority(
                                                            AuthorityType.getAuthorityType(child),
                                                            groupShortName,
                                                            groupDisplayName,
                                                            zoneSet);
                                        } else {
                                            // Maintain association deletions now. The creations
                                            // will have to be done later once
                                            // we have performed all the deletions in order to avoid
                                            // creating cycles
            ...
            private void finalizeAssociations(UserRegistry userRegistry, boolean splitTxns) {
                // First validate the group associations to be created for potential cycles. Remove
                // any offending association
                ...
                // Filter out associations to authorities that simply can't exist (and log if
            ...
            private void maintainAssociationDeletions(String authorityName) {
                ...
                Set<String> parentsToDelete =
                        isPerson
                                ? this.personParentAssocsToDelete.get(authorityName)
                ...
                if (parentsToDelete != null && !parentsToDelete.isEmpty()) {
                    for (String parent : parentsToDelete) {
                        ...
                        ChainingUserRegistrySynchronizer.this.authorityService.removeAuthority(
            ...
            private void maintainAssociationCreations(String authorityName) {
                ...
                Set<String> parents =
                        isPerson
                                ? this.personParentAssocsToCreate.get(authorityName)
                ...
                if (parents != null && !parents.isEmpty()) {
                    ...
                    try {
                        ChainingUserRegistrySynchronizer.this.authorityService.addAuthority(
                                parents, authorityName);
                    } catch (UnknownAuthorityException e) {
                        // Let's force a transaction retry if a parent doesn't exist. It may be
                        // because we are
                        // waiting for another worker thread to create it
                        throw new ConcurrencyFailureException(
                                "Forcing batch retry for unknown authority", e);
                    } catch (InvalidNodeRefException e) {
                        // Another thread may have written the node, but it is not visible to this
                        // transaction
                        // See: ALF-5471: 'authorityMigration' patch can report 'Node does not
                        // exist'
                        throw new ConcurrencyFailureException(
                                "Forcing batch retry for invalid node", e);
                    }
                }
        ...
        if (ChainingUserRegistrySynchronizer.logger.isInfoEnabled()) {
            if (lastModified == null) {
                ChainingUserRegistrySynchronizer.logger.info(
                        "Retrieving all users from user registry '" + zone + "'");
            } else {
                ChainingUserRegistrySynchronizer.logger.info(
                        "Retrieving users changed since "
                                + DateFormat.getDateTimeInstance().format(lastModified)
                                + " from user registry '"
                                + zone
                                + "'");
        ...
        final BatchProcessor<NodeDescription> personProcessor =
                new BatchProcessor<NodeDescription>(
                        SyncProcess.USER_CREATION.getTitle(zone),
                        this.transactionService.getRetryingTransactionHelper(),
        ...
        class PersonWorker extends BaseBatchProcessWorker<NodeDescription> {
            ...
            public void process(NodeDescription person) throws Throwable {
                // Make a mutable copy of the person properties, since they get written back to by
                // person service
                HashMap<QName, Serializable> personProperties =
                        new HashMap<QName, Serializable>(person.getProperties());
                String personName =
                        personProperties.get(ContentModel.PROP_USERNAME).toString().trim();
                ...
                if (Boolean.parseBoolean(ChainingUserRegistrySynchronizer.this.externalUserControl)
                        && ChainingUserRegistrySynchronizer.this.externalUserControlSubsystemName
                                .equals(zone)
                        ...
                        if (ldapUserRegistry.getUserAccountStatusInterpreter() != null) {
                            ...
                            if (personProperties.get(propertyNameToCheck) != null
                                ...
                                personProperties.put(
                                        ContentModel.PROP_ENABLED, !isUserAccountDisabled);
                            }
                        }
                    } catch (IllegalArgumentException iae) {
                        // Can be thrown by certain implementations of
                ...
                // for invalid names will throw ConstraintException that will be catched by
                // BatchProcessor$TxnCallback
                ...
                if (zones == null) {
                    ...
                    if (ChainingUserRegistrySynchronizer.logger.isDebugEnabled()) {
                        ChainingUserRegistrySynchronizer.logger.debug(
                                "Creating user '" + personName + "'");
                    }
                    ChainingUserRegistrySynchronizer.this.personService.createPerson(
                            personProperties, zoneSet);
                } else if (zones.contains(zoneId)) {
                    // The person already existed in this zone: update the person
                    if (ChainingUserRegistrySynchronizer.logger.isDebugEnabled()) {
                        ChainingUserRegistrySynchronizer.logger.debug(
                                "Updating user '" + personName + "'");
                    }
                    ChainingUserRegistrySynchronizer.this.personService.setPersonProperties(
                            personName, personProperties, false);
                } else {
                    // Check whether the user is in any of the authentication chain zones
                    Set<String> intersection = new TreeSet<String>(zones);
                    intersection.retainAll(allZoneIds);
                    // Check whether the user is in any of the higher priority authentication chain
                    // zones
                    Set<String> visited = new TreeSet<String>(intersection);
                    visited.retainAll(visitedZoneIds);
                    if (visited.size() > 0) {
                        ...
                        // The person exists, but in a different zone. Either deletions are
                        // disallowed or the zone is
                        // not in the authentication chain. May be due to upgrade or zone changes.
                        // Let's re-zone them
                        if (ChainingUserRegistrySynchronizer.logger.isWarnEnabled()) {
                            ChainingUserRegistrySynchronizer.logger.warn(
                                    "Updating user '"
                                            + personName
                                            + "'. This user will in future be assumed to originate"
                                            + " from user registry '"
                                            + zone
                                            + "'.");
                        }
                        ...
                        if (ChainingUserRegistrySynchronizer.logger.isWarnEnabled()) {
                            ChainingUserRegistrySynchronizer.logger.warn(
                                    "Recreating occluded user '"
                                            + personName
                                            + "'. This user was previously created through"
                                            + " synchronization with a lower priority user"
                                            + " registry.");
                        }
                        ChainingUserRegistrySynchronizer.this.personService.deletePerson(
                                personName);
                        ChainingUserRegistrySynchronizer.this.personService.createPerson(
                                personProperties, zoneSet);
                    }
                ...
                // Maintain association deletions and creations in one shot (safe to do this with
                // persons as we can't
                // create cycles)
        ...
        // Only now that the whole tree has been processed is it safe to persist the last modified
        // dates
        long latestTime = groupAnalyzer.getLatestTime();
        if (latestTime != -1) {
            setMostRecentUpdateTime(
                    ChainingUserRegistrySynchronizer.GROUP_LAST_MODIFIED_ATTRIBUTE,
                    zoneId,
                    latestTime,
                    splitTxns);
        ...
        if (isFullSync && allowDeletions && !deletionCandidates.isEmpty()) {
            ...
            BatchProcessor<String> authorityDeletionProcessor =
                    new BatchProcessor<String>(
                            SyncProcess.AUTHORITY_DELETION.getTitle(zone),
                            this.transactionService.getRetryingTransactionHelper(),
                            deletionCandidates,
        ...
        final String statusMessage =
                I18NUtil.getMessage("synchronization.summary.status", statusParams);

        if (ChainingUserRegistrySynchronizer.logger.isInfoEnabled()) {
            ChainingUserRegistrySynchronizer.logger.info(
                    "Finished synchronizing users and groups with user registry '" + zone + "'");
     ...
     * @param splitTxns split transactions, if true run this in a separate transaction
     * @return the most recent update time in milliseconds
     */
    private long getMostRecentUpdateTime(
            final String label, final String zoneId, boolean splitTxns) {
     ...
     * @param label the label
     * @param zoneId the zone id
     * @param lastModifiedMillis the update time in milliseconds
     * @param splitTxns Can the modifications to Alfresco be split across multiple transactions for
     *     maximum performance? If <code>true</code>, the attribute is persisted in a new
     *     transaction for increased performance and reliability.
     */
    private void setMostRecentUpdateTime(
            final String label,
            final String zoneId,
    ...
    /**
     * Gets the default set of zones to set on a person or group belonging to the user registry with
     * the given zone ID. We add the default zone as well as the zone corresponding to the user
     * registry so that the users and groups are visible in the UI.
     *
     * @param zoneId the zone id
     * @return the zone set
     */
     ...
     * (avoiding unnecessary reindexing cost).
     */
    private void updateAuthorityZones(
            String authorityName, Set<String> oldZones, final Set<String> newZones) {
        Set<String> zonesToRemove = new HashSet<String>(oldZones);
        zonesToRemove.removeAll(newZones);
        // Let's keep the authority in the alfresco auth zone if it was already there. Otherwise we
        // may have to
    ...
    protected void onBootstrap(ApplicationEvent event) {
        // Do an initial differential sync on startup, using transaction splitting. This ensures
        // that on the very
        // first startup, we don't have to wait for a very long login operation to trigger the first
        // sync!
                        ...
                        new RetryingTransactionCallback<Void>() {
                            ...
                            public Void execute() throws Throwable {
                                ...
                                for (String zoneId : toSync) {
                                    ChainingUserRegistrySynchronizer.this.attributeService
                                            .setAttribute(
    ...
    private void notifyZoneDeleted(final String zoneId) {
        ...
        // SynchronizeDirectoryDeleteZoneEvent(this, zoneId, batchProcessNames));
        this.transactionService
                .getRetryingTransactionHelper()
                .doInTransaction(
                        new RetryingTransactionCallback<Void>() {
    ...
    private void notifySyncDirectoryStart(final String zoneId, final String[] batchProcessNames) {
        this.applicationEventPublisher.publishEvent(
                new SynchronizeDirectoryStartEvent(this, zoneId, batchProcessNames));
        this.transactionService
                .getRetryingTransactionHelper()
                .doInTransaction(
                        new RetryingTransactionCallback<Void>() {
    ...
    private void notifySyncDirectoryEnd(final String zoneId, final Exception e) {
        this.applicationEventPublisher.publishEvent(
                new SynchronizeDirectoryEndEvent(this, zoneId, e));
    ...
    public Date getSyncStartTime() {
        Long start =
                (Long)
                        ChainingUserRegistrySynchronizer.this.attributeService.getAttribute(
                                ChainingUserRegistrySynchronizer.ROOT_ATTRIBUTE_PATH,
                                ChainingUserRegistrySynchronizer.START_TIME_ATTRIBUTE);

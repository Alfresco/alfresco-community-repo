...
public abstract class AbstractSubscriptionServiceWebScript
  ...
  public void execute(WebScriptRequest req, WebScriptResponse res)
    ...
    try {
      ...
      throw new WebScriptException(
        404,
        "Subscription service is disabled!",
        sde
      );
    } catch (NoSuchPersonException nspe) {
      throw new WebScriptException(
        404,
        "Unknown user '" + nspe.getUserName() + "'!",
        nspe
      );
  ...
  public abstract Object executeImpl(
    String userId,
  ...
  protected JSONObject getUserDetails(String username) {
    ...
    result.put(
      "firstName",
      nodeService.getProperty(node, ContentModel.PROP_FIRSTNAME)
    );
    result.put(
      "lastName",
      nodeService.getProperty(node, ContentModel.PROP_LASTNAME)
    );
    result.put(
      "jobtitle",
      nodeService.getProperty(node, ContentModel.PROP_JOBTITLE)
    );
    result.put(
      "organization",
      nodeService.getProperty(node, ContentModel.PROP_ORGANIZATION)
    );

    String status = (String) nodeService.getProperty(
      node,
    ...
    Date statusTime = (Date) nodeService.getProperty(
      node,
    ...
    List<AssociationRef> assocRefs =
      this.nodeService.getTargetAssocs(node, ContentModel.ASSOC_AVATAR);

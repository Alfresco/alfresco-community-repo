...
public class SearchDocumentsV1Test extends BaseRMRestTest {
    ...
    public static Object[][] queryTypes() {
        ...
        cmisQueryModel.setQuery(
                "select * from cmis:document WHERE cmis:name LIKE '%" + SEARCH_TERM + ".txt'");
     ...
     * paging And setting the skipCount and maxItems to reach the number of total items Then
    ...
    /**
     * Given some documents ending with a particular text When executing the search query with
     * paging And setting skipCount and maxItems to exceed the number of total items Then
     * hasMoreItems will be set to false
     */
    @Test(dataProvider = "queryTypes")

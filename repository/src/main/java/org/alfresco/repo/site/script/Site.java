...
public class Site implements Serializable {
    ...
    /*package*/ Site(
        ...
        this.scriptInvitationFactory =
                new ScriptInvitationFactory(invitationService, nodeService, personService);
    }
    ...
    /**
     * Gets a map of members of the site with their role within the site. This list can be filtered
     * by name and/or role.
     *
     * <p>If no name or role filter is specified all members of the site are listed.
     *
     * <p>This list includes both users and groups.
     *
    ...
    public ScriptableHashMap<String, String> listMembers(
            String nameFilter, String roleFilter, int size) {
     ...
     * <p>If no name or role filter is specified all members of the site are listed.
     *
     * <p>This list includes both users and groups if collapseGroups is set to false, otherwise all
     ...
     */
    public ScriptableHashMap<String, String> listMembers(
            String nameFilter, String roleFilter, int size, boolean collapseGroups) {
        Map<String, String> members =
                this.siteService.listMembers(
                        getShortName(), nameFilter, roleFilter, size, collapseGroups);
     ...
     *
     * <p>If the user is not a member of the site then null is returned.
     *
     ...
     *
     * <p>If the user is not already a member of the site then they are added with the role given.
     * If the user is already a member of the site then their role is updated to the new role.
     *
     * <p>Only a site manager can modify memberships and there must be at least one site manager at
     * all times.
     *
     * @param authorityName authority name
     ...
     *
     * <p>Only a site manager can remove a user's membership and the last site manager can not be
     * removed.
     *
     * @param authorityName authority name
     */
     ...
     * @return node representing the "container" folder (or null, if for some reason the container
     *     can not be created - probably due to permissions)
     */
                ...
                AuthenticationUtil.runAs(
                        new RunAsWork<NodeRef>() {
                            public NodeRef doWork() throws Exception {
                                return Site.this.siteService.getContainer(
                                        getShortName(), componentId);
                            }
                        },
     ...
     * @return ScriptNode the created container
     */
    public ScriptNode createContainer(
            final String componentId, final String folderType, final Object permissions) {
        final NodeRef containerNodeRef =
                this.createContainerImpl(componentId, folderType, permissions);
        if (Site.this
                        .serviceRegistry
                        .getPermissionService()
                        .hasPermission(containerNodeRef, PermissionService.READ_PROPERTIES)
    ...
    private NodeRef createContainerImpl(
            final String componentId, final String folderType, final Object permissions) {
                    ...
                    public NodeRef doWork() throws Exception {
                        ...
                        QName folderQName =
                                (folderType == null)
                                        ? null
                                        : QName.createQName(
                                                folderType, serviceRegistry.getNamespaceService());

                        // Create the container node
                        NodeRef containerNode =
                                Site.this.siteService.createContainer(
                                        getShortName(), componentId, folderQName, null);
                                ...
                                if (propId instanceof String) {
                                    ...
                                    if (value instanceof String) {
                                        ...
                                        Site.this
                                                ...
                                                .getPermissionService()
                                                .setPermission(
                                                        containerNode, key, (String) value, true);
                                    }
                                }
                            }
                        }
                        ...
                        serviceRegistry
                                .getOwnableService()
                                .setOwner(containerNode, AuthenticationUtil.getAdminUserName());
    ...
    /**
     * Gets and if missing, creates a new site container. The Site container is created in a new
     * readwrite txn.
     *
     * @param componentId component id
     * @return ScriptNode the created container
     ...
     * @return ScriptNode the created container
     */
    public ScriptNode aquireContainer(
            final String componentId, final String folderType, final Object properties) {
        ScriptNode container = this.getContainer(componentId);
        if (container == null) {
            RetryingTransactionCallback<NodeRef> txnCallback =
                    new RetryingTransactionCallback<NodeRef>() {
                        public NodeRef execute() throws Throwable {
                            final NodeRef containerRef =
                                    createContainerImpl(componentId, folderType, null);
            ...
            final NodeRef containerNodeRef =
                    serviceRegistry
                            ...
                            .doInTransaction(txnCallback, false, true);
    ...
    /**
     * This method creates a container of the specified id and type, sets the cm:description on that
     * container node to the specified value and saves the container node updates to the repository.
     ...
     * @since 3.4
     */
    public ScriptNode createAndSaveContainer(
            String containerId, String containerType, final String description) {
        // Implementation node. See ALF-4282 for details.
        //
        // The container for the "data lists" page within a Share site is lazily created the first
        // time
        // that a user navigates to that page. However if the first Share user to look at the data
        // lists
        // page for a site is not a site manager then they will not have the necessary permissions
        // to
        // create the container node.
        // For this reason we need to create the node, set its cm:description and save those changes
        // as system.

        // The container creation is already run as system, so we don't need to double-wrap this
        // first call
        // in a RunAs class.
                        ...
                        new RunAsWork<Void>() {
                            public Void doWork() throws Exception {
                                ...
                                for (int i = 0; i < propIds.length; i++) {
                                    ...
                                    if (propId instanceof String) {
                                        // Get the value out for the specified key - it must be
                                        // String
                                        final String key = (String) propId;
                                        final Object value = scriptable.get(key, scriptable);
                                        if (value instanceof String) {
                                ...
                                String managers =
                                        siteService.getSiteRoleGroup(
                                                getShortName(), SiteModel.SITE_MANAGER);
                                permissionService.setPermission(
                                        nodeRef, managers, SiteModel.SITE_MANAGER, true);
    ...
    public void resetAllPermissions(ScriptNode node) {
        ...
        if (permissionService
            ...
            AuthenticationUtil.runAs(
                    new RunAsWork<Void>() {
                        public Void doWork() throws Exception {
                            // Ensure node isn't inheriting permissions from an ancestor before
                            // deleting
                            if (!permissionService.getInheritParentPermissions(nodeRef)) {
                                permissionService.deletePermissions(nodeRef);
    ...
    /**
     * Get the value of a custom property, null if the custom property has not been set or doesn't
     * exist.
     *
        ...
        if (this.customProperties == null) {
            ...
            for (QName qname : props.keySet()) {
                ...
                PropertyDefinition propDef =
                        this.serviceRegistry.getDictionaryService().getProperty(qname);
                if (propDef != null) {
                ...
                CustomProperty customProp =
                        new CustomProperty(qname.toString(), value, type, title);

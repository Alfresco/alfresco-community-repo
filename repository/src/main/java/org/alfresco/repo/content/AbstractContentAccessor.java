...
public abstract class AbstractContentAccessor implements ContentAccessor {
  ...
  private static final Log loggerTrace = LogFactory.getLog(
    AbstractContentAccessor.class.getName() + ".trace"
  );

  static {
    if (loggerTrace.isDebugEnabled()) {
      loggerTrace.warn(
        "Trace channel assignment logging is on and will affect performance"
      );
    }
  }
  ...
  protected class ChannelCloseCallbackAdvise implements AfterReturningAdvice {
    ...
    private void fireChannelClosed() {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "" + listeners.size() + " content listeners called: close"
        );
      }
    }
  }

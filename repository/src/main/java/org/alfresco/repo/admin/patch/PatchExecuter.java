...
public class PatchExecuter extends AbstractLifecycleBean {
    ...
    public void applyOutstandingPatches() {
        // Apply patches even if we are in read only mode.   The system may not work safely
        // otherwise.
        ...
        if (applySucceeded && appliedPatches.size() == 0) {
            ...
            for (AppliedPatch patchInfo : appliedPatches) {
                if (!patchInfo.getWasExecuted()) {
                    ...
                    logger.debug(
                            I18NUtil.getMessage(
                                    MSG_NOT_EXECUTED, patchInfo.getId(), patchInfo.getReport()));
                } else if (patchInfo.getSucceeded()) {
                    logger.info(
                            I18NUtil.getMessage(
                                    MSG_EXECUTED, patchInfo.getId(), patchInfo.getReport()));
                } else {
                    allPassed = false;
                    logger.error(
                            I18NUtil.getMessage(
                                    MSG_FAILED, patchInfo.getId(), patchInfo.getReport()));
                }
            }

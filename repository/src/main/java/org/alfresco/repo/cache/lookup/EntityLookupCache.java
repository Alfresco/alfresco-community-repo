...
public class EntityLookupCache<K extends Serializable, V extends Object, VK extends Serializable> {
   ...
   * @since 3.2
   */
  public abstract static class EntityLookupCallbackDAOAdaptor<K2 extends Serializable, V2 extends Object, VK2 extends Serializable>
    ...
    public int deleteByKey(K2 key) {
      throw new UnsupportedOperationException(
        "Entity deletion by key is not supported"
      );
    }
    ...
    public int deleteByValue(V2 value) {
      throw new UnsupportedOperationException(
        "Entity deletion by value is not supported"
      );
    }
  }
  ...
  public Pair<K, V> getByKey(K key) {
    if (key == null) {
      throw new IllegalArgumentException(
        "An entity lookup key may not be null"
      );
    }
    ...
    if (entityPair == null) {
      ...
      // Get the value key
      VK valueKey = (value == null)
        ? (VK) VALUE_NULL
        : entityLookup.getValueKey(value);
      // Check if the value has a good key
      if (valueKey != null) {
        CacheRegionValueKey valueCacheKey = new CacheRegionValueKey(
          cacheRegion,
          valueKey
        );
  ...
  public Pair<K, V> createOrGetByValue(V value, ControlDAO controlDAO) {
    ...
    Savepoint savepoint = controlDAO.createSavepoint(
      "EntityLookupCache.createOrGetByValue"
    );
    try {

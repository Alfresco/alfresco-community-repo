...
public class AuthenticationComponentImpl
  ...
  protected void authenticateImpl(
    ...
    if (logger.isTraceEnabled()) {
      logger.trace(
        ...
        AuthenticationUtil.maskUsername(userNameIn)
      );
    }
    try {
      ...
      String normalized = getTransactionService()
              ...
              return TenantUtil.runAsSystemTenant(
                new TenantRunAsWork<String>() {
                  public String doWork() throws Exception {
                    String normalized = getPersonService()
                      .getUserIdentifier(userName);
                    String finalUserName = normalized == null
                      ? userName
                      : normalized;
                    ...
                    UserDetails userDetails = authenticationDao.loadUserByUsername(
                      finalUserName
                    );
                    if (userDetails instanceof RepositoryAuthenticatedUser) {
                      List<String> hashIndicator =
                        (
                          (RepositoryAuthenticatedUser) userDetails
                        ).getHashIndicator();
                      ...
                      if (hashIndicator != null && !hashIndicator.isEmpty()) {
                        ...
                        if (
                          ...
                          !passwordEncoder.lastEncodingIsPreferred(
                            hashIndicator
                          )
                        ) {
                          // add transaction listener to re-hash the users password
                          HashPasswordTransactionListener txListener = new HashPasswordTransactionListener(
                            userName,
                            password
                          );
                          txListener.setTransactionService(
  ...
  public Authentication authenticate(Authentication token)
    ...
    throw new AlfrescoRuntimeException(
      "Authentication via token not supported"
    );
  }

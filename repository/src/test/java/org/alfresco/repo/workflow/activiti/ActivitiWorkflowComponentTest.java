...
public class ActivitiWorkflowComponentTest
  ...
  public void testDeployDefinition() throws Exception {
    ProcessDefinition defInDB = repo
    ...
    ProcessDefinition processDef2 = repo
      .createProcessDefinitionQuery()
  ...
  public void testUndeployDefinition() throws Exception {
    ...
    defCount =
      repo.createProcessDefinitionQuery().processDefinitionId(localId).count();
    assertEquals(
      "The undeployed process definition should not exist!",
      0,
      defCount
    );
  }
  ...
  public void testGetDefinitionById() throws Exception {
    WorkflowDefinition definition = deployTestTaskDefinition();
    WorkflowDefinition result = workflowEngine.getDefinitionById(
      definition.getId()
    );
    ...
    assertEquals(
      "Start task Id does not match!",
      originalStartDef.getId(),
      resultStartDef.getId()
    );

    WorkflowNode resultNode = resultStartDef.getNode();
    assertNotNull("Start Task Node is null!", resultNode);
    assertEquals(
      "Start Task Node Name does not match!",
      originalStartDef.getNode().getName(),
      resultNode.getName()
    );
    ...
    assertEquals(
      "Start Task Metadata name does not match!",
      originalStartDef.getMetadata().getName(),
      metaData.getName()
    );

    workflowEngine.undeployDefinition(definition.getId());
    WorkflowDefinition nullResult = workflowEngine.getDefinitionById(
      definition.getId()
    );
    assertNull(
      "The workflow definition was found but should be null!",
      nullResult
    );
  }
  ...
  public void testGetDefinitionByName() throws Exception {
    WorkflowDefinition definition = deployTestTaskDefinition();
    WorkflowDefinition result = workflowEngine.getDefinitionByName(
      definition.getName()
    );
    ...
    workflowEngine.undeployDefinition(definition.getId());
    WorkflowDefinition nullResult = workflowEngine.getDefinitionByName(
      definition.getName()
    );
    assertNull(
      "The workflow definition was found but should be null!",
  ...
  public void testStartWorkflow() throws Exception {
    ...
    ProcessInstance procInstance = runtime
    ...
    ScriptNode companyHome = (ScriptNode) runtime.getVariable(
      procInstanceId,
      "companyhome"
    );
    ...
    assertEquals(
      "companyHome",
      companyHome.getNodeRef().getStoreRef().getIdentifier()
    );

    // Check if the initiator is added as variable
    ScriptNode initiator = (ScriptNode) runtime.getVariable(
      procInstanceId,
      "initiator"
    );
    ...
    ScriptNode initiatorHome = (ScriptNode) runtime.getVariable(
      procInstanceId,
      "initiatorhome"
    );
    ...
    assertEquals(
      "admin-home",
      initiatorHome.getNodeRef().getStoreRef().getIdentifier()
    );
    ...
    String defaultSetVariable = (String) taskService.getVariableLocal(
      task.getId(),
      "test_myProp"
    );
  ...
  public void testSignal() throws Exception {
    ...
    ProcessInstance processInstance = runtime.startProcessInstanceById(
      BPMEngineRegistry.getLocalId(def.getId())
    );
    ...
    String pathId = BPMEngineRegistry.createGlobalId(
  ...
  public void testCancelWorkflow() throws Exception {
    ...
    List<WorkflowInstance> instances = workflowEngine.getActiveWorkflows(
      def.getId()
    );
    ...
    assertEquals(
      ...
      BPMEngineRegistry.getLocalId(instances.get(0).getId())
    );

    // Call cancel method on component
    WorkflowInstance cancelledWorkflow = workflowEngine.cancelWorkflow(
    ...
    HistoricProcessInstance historicProcessInstance = historyService
      ...
      .singleResult();
    if (
      workflowEngine.activitiUtil.isRetentionHistoricProcessInstanceEnabled() ==
      false
    ) {
  ...
  public void testDeleteFinishedWorkflow() throws Exception {
    ...
    WorkflowTask adhocTask = workflowEngine
      .getTasksForWorkflowPath(instance.getId())
      .get(0);
  ...
  public void testGetActiveWorkflows() throws Exception {
    WorkflowDefinition def = deployTestAdhocDefinition();
    String activitiProcessDefinitionId = BPMEngineRegistry.getLocalId(
      def.getId()
    );

    ProcessInstance activeInstance = runtime.startProcessInstanceById(
      activitiProcessDefinitionId
    );
    ProcessInstance completedInstance = runtime.startProcessInstanceById(
      activitiProcessDefinitionId
    );
    ProcessInstance cancelledInstance = runtime.startProcessInstanceById(
      activitiProcessDefinitionId
    );
    ProcessInstance deletedInstance = runtime.startProcessInstanceById(
      activitiProcessDefinitionId
    );
    ...
    while (isActive) {
      ...
      ProcessInstance instance = runtime
        .createProcessInstanceQuery()
        .processInstanceId(completedId)
        .singleResult();
    ...
    workflowEngine.cancelWorkflow(
      workflowEngine.createGlobalId(cancelledInstance.getId())
    );
    workflowEngine.deleteWorkflow(
      workflowEngine.createGlobalId(deletedInstance.getId())
    );
    ...
    assertEquals(
      ...
      BPMEngineRegistry.getLocalId(instanceId)
    );
  }
  ...
  public void testGetCompletedWorkflows() throws Exception {
    ...
    // Validate if a workflow exists
    List<WorkflowInstance> instances = workflowEngine.getCompletedWorkflows(
      def.getId()
    );
    assertNotNull(instances);
    assertEquals(1, instances.size());
  ...
  public void testGetWorkflowById() throws Exception {
    ...
    Date startTime = new SimpleDateFormat("dd-MM-yyy hh:mm:ss")
      .parse("01-01-2011 12:11:10");
    ...
    putVariable(
      ...
      new ActivitiScriptNode(testWorkflowContext, serviceRegistry)
    );
    putVariable(
      variables,
      WorkflowModel.ASSOC_PACKAGE,
      new ActivitiScriptNode(testWorkflowPackage, serviceRegistry)
    );
    putVariable(variables, WorkflowModel.PROP_WORKFLOW_PRIORITY, 3);
    variables.put(
      WorkflowConstants.PROP_INITIATOR,
      new ActivitiScriptNode(adminHomeNode, serviceRegistry)
    );
    ...
    WorkflowInstance workflowInstance = workflowEngine.getWorkflowById(
      globalProcessInstanceId
    );
  ...
  public void testGetCompletedWorkflowById() throws Exception {
    ...
    Date endTime = new SimpleDateFormat("dd-MM-yyy hh:mm:ss")
      .parse("01-01-2011 02:03:04");
  ...
  public void testGetTimers() throws Exception {
    ...
    String workflowInstanceId = BPMEngineRegistry.createGlobalId(
      ActivitiConstants.ENGINE_ID,
      processInstance.getProcessInstanceId()
    );
    ...
    Job timerJob = managementService
      ...
      .processInstanceId(processInstance.getId())
      .singleResult();
    String globalJobId = BPMEngineRegistry.createGlobalId(
      ActivitiConstants.ENGINE_ID,
      timerJob.getId()
    );
    ...
    String expectedTimerPathId = BPMEngineRegistry.createGlobalId(
      ActivitiConstants.ENGINE_ID,
    ...
    assertEquals(
      BPMEngineRegistry.createGlobalId(
        ActivitiConstants.ENGINE_ID,
        waitingTask.getId()
      ),
      timer.getTask().getId()
  ...
  public void testGetWorkflowImage() {
    ...
    ProcessInstance processInstance = runtime.startProcessInstanceById(
      BPMEngineRegistry.getLocalId(definitionWithoutImage.getId())
    );
    String worklfowId = BPMEngineRegistry.createGlobalId(
      ActivitiConstants.ENGINE_ID,
      processInstance.getId()
    );
    ...
    ProcessInstance processInstanceWithImage = runtime.startProcessInstanceById(
      BPMEngineRegistry.getLocalId(definitionWithImage.getId())
    );
    String worklfowWithImageId = BPMEngineRegistry.createGlobalId(
      ActivitiConstants.ENGINE_ID,
      processInstanceWithImage.getId()
    );
  ...
  private void checkDefinitions(
    List<WorkflowDefinition> actual,
    List<WorkflowDefinition> startDefs,
    WorkflowDefinition... expected
  ) {
    assertEquals(
      "The number of process definitions expected does not match the actual number!",
      startDefs.size() + expected.length,
      actual.size()
    );
    ...
    for (WorkflowDefinition exp : expected) {
      assertTrue(
        "Results did not contain expected definition: " + exp,
    ...
    for (WorkflowDefinition exp : expected) {
      assertFalse(
        "Starting Definitions should not contain expected definition: " + exp,
        startIds.contains(exp.getId())
      );
    }
  }
}

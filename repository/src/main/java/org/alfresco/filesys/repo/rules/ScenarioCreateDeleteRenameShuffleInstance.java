...
/**
 * This is an instance of a create, delete, rename shuffle" triggered by a create of a file matching
 * a specified pattern.
 *
 * <p>a) New file created. Typically with an obscure name. b) Existing file deleted c) New file
 * moved into place.
 *
 * <p>If this filter is active then this is what happens. a) New file created. New file created. b)
 * Existing file deleted. File moved to temporary location instead. c) Rename - Scenario fires -
 * File moved back from temporary location - Content updated. - temporary file deleted
 */
    ...
    public Command evaluate(Operation operation) {
        ...
        switch (internalState) {
            ...
            case ACTIVE:
                 ...
                 *
                 * <p>Need to intervene and replace delete with a rename to temp file.
                 */
                if (operation instanceof DeleteFileOperation) {
                    DeleteFileOperation d = (DeleteFileOperation) operation;
                    ...
                    /**
                     * For Mac 2011 powerpoint files - add an extra check based on the filename e.g
                     * FileA1 - delete FileA. For Mac 2011 excel files - check that file to delete
                     * is not temporary
                     */
                    if (checkFilename) {
                        ...
                        if (i > 0) {
                            ...
                            String createExt =
                                    (j > 0) ? createName.substring(j + 1, createName.length()) : "";
                                ...
                                isRightTarget =
                                        (i < createName.length())
                                                && deleteName
                                                        .substring(0, i)
                                                        .equalsIgnoreCase(
                                                                createName.substring(0, i));
                            } else if (deleteExt.startsWith("xls") && createExt.isEmpty()) {
                                isRightTarget =
                                        !deleteName.startsWith("._")
                                                && !deleteName.startsWith("~$");
                            }
                        }
                        ...
                        if (isRightTarget) {
                            ...
                            if (logger.isDebugEnabled()) {
                                logger.debug(
                                        "check filename patterns do not match - Ignore"
                                                + createName
                                                + deleteName);
                            }
                            return null;
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "got a delete : replace with rename createName:"
                                        + createName
                                        + "deleteName:"
                                        + deleteName);
                    }
                ...
                if (operation instanceof RenameFileOperation) {
                    ...
                    if (deletes.containsKey(targetFile)) {
                        ...
                        /**
                         * This is where the scenario fires. a) Rename the temp file back to the
                         * targetFile b) Copy content from moved file c) Delete rather than move
                         * file
                         */
                        if (logger.isDebugEnabled()) {
                        ...
                        DeleteFileCommand d1 =
                                new DeleteFileCommand(
                                        m.getFrom(), m.getRootNodeRef(), m.getFromPath());

...
public class SolrOpenCMISQueryServiceImpl implements CMISQueryService {
  ...
  public void setAlfrescoDictionaryService(
  ...
  public CMISResultSet query(CMISQueryOptions options) {
    ...
    searchParameters.addExtraParameter(
      "cmisVersion",
      options.getCmisVersion().toString()
    );
    ResultSet rs = solrQueryLanguage.executeQuery(searchParameters);

    CapabilityJoin joinSupport = getJoinSupport();
    if (
      options.getQueryMode() ==
      CMISQueryOptions.CMISQueryMode.CMS_WITH_ALFRESCO_EXTENSIONS
    ) {
    ...
    CMISResultSet cmis = new CMISResultSet(
      wrapped,
      options,
      limitBy,
      nodeService,

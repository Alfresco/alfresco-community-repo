 ...
 *
 * <p>a) Existing file renamed out of the way. X.fm to X.backup.fm b) New file moved renamed into
 * place. X.fm.C29
 *
 * <p>Scenario is triggered by the first rename matching a pattern.
 */
public class ScenarioDoubleRenameShuffle implements Scenario {
        ...
        if (operation instanceof RenameFileOperation) {
            ...
            if (m.matches()) {
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "New Scenario Double Rename Shuffle Instance strPattern:"
                                    + pattern
                                    + " matches"
                                    + r.getTo());
                }
                ScenarioDoubleRenameShuffleInstance instance =
                        new ScenarioDoubleRenameShuffleInstance();
    ...
    public void setInterimPattern(String intermediateMovePattern) {
        if (null != intermediateMovePattern) {
            this.interimPattern =
                    Pattern.compile(intermediateMovePattern, Pattern.CASE_INSENSITIVE);

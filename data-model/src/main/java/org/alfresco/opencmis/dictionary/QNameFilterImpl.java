...
public class QNameFilterImpl implements QNameFilter {
  ...
  public Collection<QName> filterQName(Collection<QName> typesToFilter) {
    ...
    if (!excludedQNames.isEmpty() || !excludedModels.isEmpty()) {
      ...
      for (QName classQName : typesToFilter) {
        if (!isExcluded(classQName)) { //                if (!excludedQNames.contains(classQName) && !excludedModels.contains(classQName.getNamespaceURI()))
  ...
  public boolean isExcluded(QName typeQName) {
    ...
    return (
      isExcluded != null &&
      isExcluded.booleanValue() ||
      excludedModels.contains(typeQName.getNamespaceURI())
    );
  }
  ...
  public static List<String> listOfHardCodedExcludedTypes() {
    ...
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/publishingworkflow/1.0}*"
    );
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/publishing/twitter/1.0}*"
    );
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/publishing/slideshare/1.0}*"
    );
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/publishing/facebook/1.0}*"
    );
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/publishing/youtube/1.0}*"
    );
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/publishing/linkedin/1.0}*"
    );
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/publishing/flickr/1.0}*"
    );
    ...
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/blogintegration/1.0}*"
    );
    ...
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/workflow/invite/moderated/1.0}*"
    );
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/workflow/invite/nominated/1.0}*"
    );
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/workflow/cloud/resetpassword/1.0}*"
    );
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/workflow/cloud/siteinvitation/1.0}*"
    );
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/workflow/signup/selfsignup/1.0}*"
    );
    hardCodeListOfTypes.add(
    ...
    hardCodeListOfTypes.add(
      "{http://www.alfresco.org/model/hybridworkflow/1.0}*"
    );

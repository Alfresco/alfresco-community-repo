 ...
 *
 * <p>There are two types of cursor:
 *
 * <p>a) Paged
 *
 * <p>This type of cursor is driven from a page number and page size. Random access within the
 * collection is possible by jumping straight to a page. A simple scroll through the collection is
 * supported by iterating through each next page.
 *
 * <p>b) Rows
 *
 * <p>This type of cursor is driven from a skip row count and maximum number of rows. Random access
 * is not supported. The collection of items is simply scrolled through from start to end by
 * iterating through each next set of rows.
 *
 * <p>In either case, a paging cursor provides a start row and end row which may be used to extract
 * the items for the page from the collection of N items.
 *
 * <p>A zero (or less) page size or row maximum means "unlimited".
 *
 * <p>Zero or one based Page and Rows indexes are supported. By default, Pages are 1 based and Rows
 * are 0 based.
 *
 * <p>At any time, -1 is returned to represent "out of range" i.e. for next, previous, last page and
 * next skip count.
 *
 * <p>Pseudo-code for traversing through a collection of N items (10 at a time):
 *
 * <p>PagingCursor cursor = new PagingCursor(); Page page = cursor.createPageCursor(N, 10, 1); while
 * (page.isInRange()) { for (long i = page.getStartRow(); i <= page.getEndRow(); i++) {
 * ...collection[i]... } page = cursor.createPageCursor(N, 10, page.getNextPage()); }
 *
 * <p>Rows rows = cursor.createRowsCursor(N, 10, 0); while (rows.isInRange()) { for (long i =
 * page.getStartRow(); i <= page.getEndRow(); i++) { ...collection[i]... } rows =
 * cursor.createRowsCursor(N, 10, rows.getNextSkipRows()); }
 *
 * @author davidc
 */
public class PagingCursor {
     ...
     *
     * <p>Note: scoped to this paging cursor instance
     *
     ...
     * @return Rows Cursor
     */
    public Rows createRowsCursor(
            long totalRows, long maxRows, long skipRows, boolean zeroBasedRow) {
        return new Rows(totalRows, maxRows, skipRows, zeroBasedRow);
    }

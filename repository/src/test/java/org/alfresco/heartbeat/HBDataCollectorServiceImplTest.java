...
public class HBDataCollectorServiceImplTest {
  ...
  private final HBBaseDataCollector validCollector1 = new SimpleHBDataCollector(
    "validCollector1",
    VALID_CRON
  );
  private final HBBaseDataCollector validCollector2 = new SimpleHBDataCollector(
    "validCollector2",
    VALID_CRON
  );
  ...
  public void testInitialEnabledEqualsDefaultState() throws Exception {
    HBDataCollectorServiceImpl dataCollectorService = new HBDataCollectorServiceImpl(
      true
    );
  ...
  public void testJobSchedulingWhenEnabled() throws Exception {
    ...
    final HBDataCollectorServiceImpl collectorService = new HBDataCollectorServiceImpl(
      true
    );
  ...
  public void testDeregister() throws Exception {
    ...
    assertTrue(
      ...
      isJobScheduledForCollector(validCollector1)
    );
    assertTrue(
      "Job was not scheduled",
      isJobScheduledForCollector(validCollector2)
    );

    collectorService.deregisterCollector(validCollector1);

    assertFalse(
      "Job should have be unregistered",
      isJobScheduledForCollector(validCollector1)
    );
    assertTrue(
      "Job was not scheduled",
  ...
  private void registerValidCollectors(
    HBDataCollectorServiceImpl collectorService
  ) {
    collectorService.registerCollector(validCollector1);
  ...
  private void assertCollectorJobsNotScheduled(String message)
    throws Exception {
  ...
  private void enabledHbInLicense(
    HBDataCollectorServiceImpl collectorService,
    boolean activate
  ) {
  ...
  private boolean isJobScheduledForCollector(HBBaseDataCollector collector)
    throws Exception {

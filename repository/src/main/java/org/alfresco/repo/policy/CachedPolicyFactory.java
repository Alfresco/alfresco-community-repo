...
public class CachedPolicyFactory<B extends BehaviourBinding, P extends Policy>
   ...
   */
  /*package*/CachedPolicyFactory(
    Class<P> policyClass,
    BehaviourIndex<B> index
  ) {
  ...
  public P create(B binding) {
    ...
    LockHelper.tryLock(
      lock.readLock(),
      tryLockTimeout,
      "getting policy from cache in 'CachedPolicyFactory.create()'"
    );
    try {
    ...
    LockHelper.tryLock(
      lock.writeLock(),
      tryLockTimeout,
      "putting new policy to cache in 'CachedPolicyFactory.create()'"
    );
    try {
      P policyInterface = singleCache.get(binding);
      if (policyInterface != null) {
  ...
  public Collection<P> createList(B binding) {
    ...
    LockHelper.tryLock(
      lock.readLock(),
      tryLockTimeout,
      "getting policy list from cache in 'CachedPolicyFactory.createList()'"
    );
    try {
    ...
    LockHelper.tryLock(
      lock.writeLock(),
      tryLockTimeout,
      "putting policy list to cache in 'CachedPolicyFactory.createList()'"
    );
    try {
      Collection<P> policyInterfaces = listCache.get(binding);
      if (policyInterfaces != null) {
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "Cached delegate interface collection " +
        policyInterfaces +
        " for " +
        binding +
        " and policy " +
  ...
  private void clearCache(String cacheDescription, Map<B, ?> cache, B binding) {
    if (binding == null) {
      LockHelper.tryLock(
        lock.writeLock(),
        tryLockTimeout,
        "clearing policy cache in 'CachedPolicyFactory.clearCache()'"
      );
      try {
        // A specific binding has not been provided, so clear all entries
        cache.clear();

        if (logger.isDebugEnabled() && cache.isEmpty() == false) logger.debug(
          "Cleared " +
          cacheDescription +
          " cache (all class bindings) for policy " +
          getPolicyClass()
        );
      } finally {
        lock.writeLock().unlock();
      ...
      if (invalidBindings.size() > 0) {
        LockHelper.tryLock(
          lock.writeLock(),
          tryLockTimeout,
          "removing invalid policy bindings from cache in 'CachedPolicyFactory.clearCache()'"
        );
        try {

...
public class ReadRecordTests extends BaseRMRestTest {
    ...
    public static final String CATEGORY_NAME =
        TestData.RECORD_CATEGORY_NAME + getRandomAlphanumeric();

    public static final String ELECTRONIC_RECORD_NAME =
        "Record electronic" + getRandomAlphanumeric();
    public static final String NONELECTRONIC_RECORD_NAME =
        "Record nonelectronic" + getRandomAlphanumeric();

    private Record electronicRecord = Record
        .builder()
        .name(ELECTRONIC_RECORD_NAME)
        .nodeType(CONTENT_TYPE)
        .build();

    private Record nonelectronicRecord = Record
        .builder()
        .properties(
            RecordProperties
                .builder()
    ...
    public void readRecordsFromInvalidContainers(String container) {
        Record electronicRecord = Record
            .builder()
            .name(ELECTRONIC_RECORD_NAME)
            .nodeType(CONTENT_TYPE)
        ...
        if (FILE_PLAN_ALIAS.equals(container)) {
            getRestAPIFactory()
                .getFilePlansAPI()
                .getRootRecordCategories(container, "")
            ...
            getRestAPIFactory()
                ...
                .getTransfers(container, "where=(isRecord=true)")
            ...
            RecordCategoryChildCollection children = getRestAPIFactory()
                .getRecordCategoryAPI()
                .getRecordCategoryChildren(container);
            //check response status code
            assertStatusCode(OK);
            children
                .assertThat() //check the list returned is not empty because there is a record folder created inside
                .entriesListCountIs(1)
                .assertThat()
                .paginationExist();
            assertEquals(
                children.getEntries().get(0).getEntry().getNodeType(),
                RECORD_FOLDER_TYPE
            );
        }
    }
    ...
    public void readRecordMetadata() {
        String RELATIVE_PATH =
            "/" + CATEGORY_NAME + getRandomAlphanumeric() + "/folder";

        RecordCategory recordCategoryModel = createRecordCategoryModel(
            CATEGORY_NAME,
            CATEGORY_NAME
        );
        String recordCategoryId = getRestAPIFactory()
            .getFilePlansAPI()
        ...
        String recordFolderId = getRestAPIFactory()
            .getRecordCategoryAPI()
        ...
        String recordWithContentId = getRestAPIFactory()
            .getRecordFolderAPI()
            .createRecord(electronicRecord, recordFolderId, getFile(IMAGE_FILE))
            .getId();

        //Get the record created
        Record recordWithContent = getRestAPIFactory()
            .getRecordsAPI()
            .getRecord(
                recordWithContentId,
                "include=" + IS_COMPLETED + "," + CONTENT
            );

        //Check the metadata returned
        assertTrue(
            recordWithContent.getName().startsWith(ELECTRONIC_RECORD_NAME)
        );
        ...
        assertTrue(
            recordWithContent
                .getName()
                .contains(recordWithContent.getProperties().getIdentifier())
        );
        assertStatusCode(OK);

        //create non-electronic record
        String nonElectronicRecordId = getRestAPIFactory()
            .getRecordFolderAPI()
            .createRecord(nonelectronicRecord, recordFolderId)
            .getId();
        //Get the record created
        Record nonElectronicRecord = getRestAPIFactory()
            .getRecordsAPI()
            .getRecord(
                nonElectronicRecordId,
                "include=" + IS_COMPLETED + "," + CONTENT
            );

        //Check the metadata returned
        assertTrue(
            nonElectronicRecord.getName().startsWith(NONELECTRONIC_RECORD_NAME)
        );
        assertNull(nonElectronicRecord.getContent());
        assertEquals(
            nonElectronicRecord.getNodeType(),
            NON_ELECTRONIC_RECORD_TYPE
        );
        assertNotNull(nonElectronicRecord.getAspectNames());
        assertEquals(
            nonElectronicRecord.getProperties().getDescription(),
            NONELECTRONIC_RECORD_NAME
        );
        assertNotEquals(
            NONELECTRONIC_RECORD_NAME,
            nonElectronicRecord.getName()
        );
        assertTrue(
            nonElectronicRecord
                .getName()
                .contains(nonElectronicRecord.getProperties().getIdentifier())
        );
        assertStatusCode(OK);
    }
    ...
    public void readRecordContent() throws Exception {
        ...
        String RECORD_ELECTRONIC_BINARY =
            "Binary Record" + getRandomAlphanumeric();

        String RELATIVE_PATH =
            "/" + CATEGORY_NAME + getRandomAlphanumeric() + "/folder";

        // create the containers from the relativePath
        RecordCategoryChild recordFolder = RecordCategoryChild
            .builder()
            .name(RECORD_FOLDER_NAME)
            .nodeType(RECORD_FOLDER_TYPE)
            .relativePath(RELATIVE_PATH)
        ...
        RecordCategoryAPI recordCategoryAPI = getRestAPIFactory()
            .getRecordCategoryAPI();
        String folderId = recordCategoryAPI
            .createRecordCategoryChild(recordFolder, rootCategory2.getId())
            .getId();
        ...
        RecordFolderAPI recordFolderAPI = getRestAPIFactory()
            .getRecordFolderAPI();
        String recordId = recordFolderAPI
            .createRecord(
                recordText,
                folderId,
                createTempFile(RECORD_ELECTRONIC, RECORD_ELECTRONIC)
            )
            .getId();
        assertEquals(
            recordsAPI.getRecordContent(recordId).asString(),
            RECORD_ELECTRONIC
        );
        // Check status code
        assertStatusCode(OK);
        ...
        Record recordBinary = Record
            .builder()
            .name(RECORD_ELECTRONIC_BINARY)
            .nodeType(CONTENT_TYPE)
            .build();

        String binaryRecordId = recordFolderAPI
            .createRecord(recordBinary, folderId, getFile(IMAGE_FILE))
            .getId();
        // binary content, therefore compare respective SHA1 checksums in order to verify this is identical content
        try (
            InputStream recordContentStream = recordsAPI
                .getRecordContent(binaryRecordId)
                .asInputStream();
            FileInputStream localFileStream = new FileInputStream(
                getFile(IMAGE_FILE)
            )
        ) {
            assertEquals(
                DigestUtils.sha1(recordContentStream),
                DigestUtils.sha1(localFileStream)
            );
        }
        assertStatusCode(OK);

        // electronic record with no content
        Record recordNoContent = Record
            .builder()
            .name(RECORD_ELECTRONIC)
            .nodeType(CONTENT_TYPE)
            .build();
        String recordNoContentId = recordFolderAPI
            .createRecord(recordNoContent, folderId)
            .getId();
        assertTrue(
            recordsAPI.getRecordContent(recordNoContentId).asString().isEmpty()
        );
        assertStatusCode(OK);
    }
    ...
    public void readNonElectronicRecordContent() {
        ...
        RecordFolderAPI recordFolderAPI = getRestAPIFactory()
            .getRecordFolderAPI();
        String nonElectronicRecord = recordFolderAPI
            .createRecord(record, recordFolder.getId())
            .getId();

        getRestAPIFactory()
            .getRecordsAPI()
            .getRecordContent(nonElectronicRecord);
        assertStatusCode(BAD_REQUEST);
    }

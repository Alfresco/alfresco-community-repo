...
public abstract class CMISAbstractDictionaryService
  ...
  protected static final Log logger = LogFactory.getLog(
    CMISAbstractDictionaryService.class
  );
  ...
  private final String KEY_OPENCMIS_DICTIONARY_REGISTRY =
    "key.openCmisDictionaryRegistry";
   ...
   *            mapping
   */
  public void setPropertyAccessorMapping(
   ...
   *            mapping
   */
  public void setPropertyLuceneBuilderMapping(
  ...
  protected interface DictionaryInitializer {
    Collection<AbstractTypeDefinitionWrapper> createDefinitions(
  ...
  protected void onBootstrap(ApplicationEvent event) {
    ...
    if (dictionaryDAO != null) {
      ...
      logger.error(
        "DictionaryDAO is null - hence CMIS Dictionary not registered for updates"
      );
    }
  }
  ...
  protected CMISDictionaryRegistry createCoreDictionaryRegistry() {
    CMISDictionaryRegistryImpl cmisRegistry = new CMISDictionaryRegistryImpl(
      this,
      cmisMapping,
      dictionaryService,
  ...
  protected CMISDictionaryRegistry createTenantDictionaryRegistry(
    String tenant
  ) {
    CMISDictionaryRegistryImpl cmisRegistry = new CMISDictionaryRegistryImpl(
      this,
  ...
  public TypeDefinitionWrapper findType(String typeId) {
    ...
    if (
      typeDef != null &&
      typeDef.getTypeDefinition(false).getDisplayName() == null
    ) {
  ...
  @Override
  public TypeDefinitionWrapper findTypeForClass(
    QName clazz,
    BaseTypeId... matchingScopes
  ) {
    ...
    TypeDefinitionWrapper matchingTypeDef = (matchingScopes.length == 0)
      ? typeDef
      : null;
    if (typeDef != null) {
      for (BaseTypeId scope : matchingScopes) {
  ...
  public TypeDefinitionWrapper findTypeByQueryName(String queryName) {
    ...
    return getRegistry()
      .getTypeDefByQueryName(ISO9075.lowerCaseEncodedSQL(queryName));
  }
  ...
  public PropertyDefinitionWrapper findPropertyByQueryName(String queryName) {
    return getRegistry()
      .getPropDefByQueryName(ISO9075.lowerCaseEncodedSQL(queryName));
  }
  ...
  public List<TypeDefinitionWrapper> getBaseTypes(boolean includeParent) {
    List<TypeDefinitionWrapper> types = getRegistry()
      .getBaseTypes(includeParent);
  ...
  public List<TypeDefinitionWrapper> getAllTypes() {
    ...
    return Collections.unmodifiableList(
      new ArrayList<TypeDefinitionWrapper>(getRegistry().getTypeDefs())
    );
  }
  ...
  public List<TypeDefinitionWrapper> getAllTypes(boolean includeParent) {
    // TODO is there a way of not having to reconstruct this every time?
    return Collections.unmodifiableList(
      new ArrayList<TypeDefinitionWrapper>(
        getRegistry().getTypeDefs(includeParent)
      )
    );
  }
  ...
  public List<TypeDefinitionWrapper> getChildren(String typeId) {
    ...
    for (TypeDefinitionWrapper child : children) {
      if (
        child != null && child.getTypeDefinition(false).getDisplayName() == null
      ) {

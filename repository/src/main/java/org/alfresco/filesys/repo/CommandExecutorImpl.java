 ...
 *
 * <p>Executes commands against the repository.
 */
public class CommandExecutorImpl implements CommandExecutor {
    ...
    @Override
    public Object execute(final SrvSession sess, final TreeConnection tree, final Command command)
            throws IOException {
    ...
    private Object executeInternal(
            SrvSession sess, TreeConnection tree, Command command, Object result)
            throws IOException {
        ...
        try {
            if (command instanceof CompoundCommand) {
                ...
                return repositoryDiskInterface.createFile(
                        create.getRootNode(),
                        create.getPath(),
                        create.getAllocationSize(),
                        create.isHidden());
                ...
                return repositoryDiskInterface.restoreFile(
                        sess,
                        tree,
                        restore.getRootNode(),
                        restore.getPath(),
                        restore.getAllocationSize(),
                        restore.getOriginalNodeRef());
                ...
                return repositoryDiskInterface.deleteFile2(
                        sess, tree, delete.getRootNode(), delete.getPath());
                ...
                return repositoryDiskInterface.openFile(
                        sess, tree, o.getRootNodeRef(), o.getPath(), mode, o.isTruncate());

            } else if (command instanceof CloseFileCommand) {
                logger.debug("close file command");
                CloseFileCommand c = (CloseFileCommand) command;
                return repositoryDiskInterface.closeFile(
                        tree, c.getRootNodeRef(), c.getPath(), c.getNetworkFile());
                ...
                repositoryDiskInterface.renameFile(
                        rename.getRootNode(),
                        rename.getFromPath(),
                        rename.getToPath(),
                        rename.isSoft(),
                ...
                repositoryDiskInterface.copyContent(
                        copy.getRootNode(), copy.getFromPath(), copy.getToPath());

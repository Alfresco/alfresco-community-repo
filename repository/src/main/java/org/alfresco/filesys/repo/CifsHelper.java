...
public class CifsHelper {
    ...
    /**
     * Controls whether the read only flag is set on folders. This flag, when set, may cause
     * problematic # behaviour in Windows clients and doesn't necessarily mean a folder can't be
     * written to. See ALF-6727. Should we ever set the read only flag on folders?
     *
     * @param setReadOnlyFlagOnFolders the setReadOnlyFlagOnFolders to set
     */
    ...
    /**
     * Extract a single node's file info, where the node is reference by a path relative to an
     * ancestor node.
     *
     ...
     */
    public ContentFileInfo getFileInformation(
            final NodeRef pathRootNodeRef,
            final String path,
            final boolean readOnly,
            final boolean lockedFilesAsOffline)
            throws FileNotFoundException {

        RetryingTransactionCallback<ContentFileInfo> cb =
                new RetryingTransactionCallback<ContentFileInfo>() {
                    /**
                    ...
                    public ContentFileInfo execute() throws IOException {
                        try {
                            return getFileInformationImpl(
                                    pathRootNodeRef, path, readOnly, lockedFilesAsOffline);
    ...
    public ContentFileInfo getFileInformation(
            final NodeRef nodeRef, final boolean readOnly, final boolean lockedFilesAsOffline)
            throws FileNotFoundException {
        RetryingTransactionCallback<ContentFileInfo> cb =
                new RetryingTransactionCallback<ContentFileInfo>() {
                    /**
     ...
     * @throws FileNotFoundException
     */
    public ContentFileInfo getFileInformationImpl(
            NodeRef pathRootNodeRef, String path, boolean readOnly, boolean lockedFilesAsOffline)
            throws FileNotFoundException {
    ...
    private ContentFileInfo getFileInformationImpl(
            ...
            throws FileNotFoundException {
        // get the file info
        org.alfresco.service.cmr.model.FileInfo fileFolderInfo =
     ...
     *
     * <p>If the directory path doesn't exist, then all the parent directories will be created. If
     * the file path is <code>null</code>, then the file will not be created
     *
    ...
    public NodeRef createNode(NodeRef rootNodeRef, String path, QName typeQName)
        ...
        try {
            NodeRef nodeRef =
                    fileFolderService.create(parentFolderNodeRef, name, typeQName).getNodeRef();
    ...
    private void addDescendents(
            List<NodeRef> pathRootNodeRefs, Stack<String> pathElements, List<NodeRef> results) {
     ...
     *
     * <p>The path may contain wild cards
     *
     * <p>
     *
    ...
    public String getFileName(final NodeRef nodeRef) {
        RetryingTransactionCallback<String> cb =

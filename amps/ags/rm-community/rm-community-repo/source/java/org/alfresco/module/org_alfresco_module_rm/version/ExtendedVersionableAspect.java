...
/**
 * Extend versionable aspect auto-version behaviour to allow versions to be created when the content
 * type is changed.
 *
 * <p>Note: this behaviour should be merged into core asap
 *
...
public class ExtendedVersionableAspect implements NodeServicePolicies.OnSetNodeTypePolicy {
    ...
    public void onSetNodeType(NodeRef nodeRef, QName oldType, QName newType) {
        if (isAutoVersionOnTypeChange
            ...
            if (versionedNodeRefs == null || !versionedNodeRefs.containsKey(nodeRef)) {
                ...
                // NOTE: auto version on type change is a global setting, if thins extension was
                // moved into the
                //       core then cm:versionable could be extended with a property consistent with
                // the current
                //       implementation
    ...
    private void createVersionImpl(
            final NodeRef nodeRef, final Map<String, Serializable> versionProperties) {

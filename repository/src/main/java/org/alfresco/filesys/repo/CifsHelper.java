...
public class CifsHelper {
  ...
  public boolean isDirectory(NodeRef nodeRef) {
    ...
    if (dictionaryService.isSubClass(nodeTypeQName, ContentModel.TYPE_FOLDER)) {
      ...
      dictionaryService.isSubClass(nodeTypeQName, ContentModel.TYPE_CONTENT)
    ) {
      return false;
    } else {
  ...
  private ContentFileInfo getFileInformationImpl(
    ...
    // get the file info
    org.alfresco.service.cmr.model.FileInfo fileFolderInfo = fileFolderService.getFileInfo(
      nodeRef
    );
    ...
    long id = DefaultTypeConverter.INSTANCE.convert(
      Long.class,
    ...
    if (fileFolderInfo.isFolder()) {
      ...
      ContentData contentData = (ContentData) nodeProperties.get(
      ...
      if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_LOCKABLE)) {
        ...
        if (lockType != null) {
          switch (lockType) {
            case NODE_LOCK:
              if ((attr & FileAttribute.ReadOnly) == 0) attr +=
                FileAttribute.ReadOnly;
              break;
            case WRITE_LOCK:
              LockStatus lockStatus = lockService.getLockStatus(nodeRef);
              if (lockStatus == LockStatus.LOCK_OWNER) {} else {
    ...
    if (name != null) {
      ...
      if (
        hiddenAspect.getVisibility(Client.cifs, fileInfo.getNodeRef()) ==
    ...
    if (fileInfo.getFileAttributes() == 0) fileInfo.setFileAttributes(
  ...
  public NodeRef createNode(NodeRef rootNodeRef, String path, QName typeQName)
    ...
    try {
      NodeRef nodeRef = fileFolderService
        .create(parentFolderNodeRef, name, typeQName)
        .getNodeRef();
  ...
  private void addDescendents(
    ...
    for (NodeRef pathRootNodeRef : pathRootNodeRefs) {
      ...
      List<NodeRef> directDescendents = getDirectDescendents(
        pathRootNodeRef,
        pathElement
      );
   ...
   */
  private List<NodeRef> getDirectDescendents(
    NodeRef pathRootNodeRef,
    String pathElement
  ) {
    if (logger.isDebugEnabled()) {
    ...
    if (!WildCard.containsWildcards(pathElement)) {
      ...
      NodeRef foundNodeRef = fileFolderService.searchSimple(
        pathRootNodeRef,
        pathElement
      );
      ...
      String escapedPathElement = SearchLanguageConversion.convertCifsToLucene(
        pathElement
      );
      // do the lookup
   ...
   */
  public NodeRef getNodeRef(NodeRef pathRootNodeRef, String path)
    throws FileNotFoundException {
  ...
  public void relinkNode(
    ...
    org.alfresco.service.cmr.model.FileInfo tempFileInfo = fileFolderService.getFileInfo(
      tempNodeRef
    );
    org.alfresco.service.cmr.model.FileInfo fileToMoveInfo = fileFolderService.getFileInfo(
      nodeToMoveRef
    );
   ...
   * @throws FileExistsException
   */
  public void rename(NodeRef nodeToRenameRef, String newName)
    throws FileExistsException {
    try {

...
public class MultiTServiceImpl implements TenantService {
        ...
        if (tenantDomain != null) {
            storeRef =
                    new StoreRef(
                            ...
                            getName(storeRef.getIdentifier(), tenantDomain, checkTenantEnabled));
        }
    ...
    public NodeRef getBaseName(NodeRef nodeRef, boolean forceForNonTenant) {
        ...
        return new NodeRef(
                nodeRef.getStoreRef().getProtocol(),
                getBaseName(nodeRef.getStoreRef().getIdentifier(), forceForNonTenant),
                nodeRef.getId());
    }
    ...
    @Override
    public ChildAssociationRef getBaseName(
            ChildAssociationRef childAssocRef, boolean forceForNonTenant) {
        if (childAssocRef == null) {
            return null;
        }
    ...
    public void checkDomainUser(String username) {
        ...
        if (!tenantDomain.equals(DEFAULT_DOMAIN)) {
            ...
            if ((idx2 > 0) && (idx2 < (username.length() - 1))) {
                ...
                if ((tenantUserDomain == null)
                        || (!tenantDomain.equalsIgnoreCase(tenantUserDomain))) {
     ...
     *
     * <p>For user names of the form "user@tenantdomain", the tenant domain the part of the string
     * after the @ symbol. A check is then made to see if tenant with that domain name exists. If it
     * does, then the identified domain is returned. If no tenant exists then null is returned.
     *
     * <p>If the username does not end with a domain, as described above, then the default domain is
     * returned.
     */
    @Override
    ...
    protected void checkTenantEnabled(String tenantDomain) {
        ...
        if (tenant == null
                || !AuthenticationUtil.isRunAsUserTheSystemUser() && !tenant.isEnabled()) {

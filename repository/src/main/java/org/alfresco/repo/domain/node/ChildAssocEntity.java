...
public class ChildAssocEntity implements Serializable {
  ...
  public static String getChildNodeNameShort(String childNodeName) {
    ...
    if (length <= 50) {
      ...
      ret
        .append(childNodeName.substring(0, 47))
        .append(TRUNCATED_NAME_INDICATOR);
      return ret.toString();
    }
  }
  ...
  public static Pair<String, Long> getChildNameUnique(
    ...
    if (childName == null) {
      throw new IllegalArgumentException(
        "Child name may not be null.  Use the Node ID ..."
      );
    }
    ...
    if (assocDef == null || !assocDef.isChild()) {
      if (logger.isWarnEnabled()) {
        logger.warn(
          "No child association of this type could be found: " + assocTypeQName
        );
      }
  ...
  public ChildAssociationRef getRef(QNameDAO qnameDAO) {
    ...
    QName qname = QName.createQName(
      qnameDAO.getNamespace(qnameNamespaceId).getSecond(),
      qnameLocalName
    );
   ...
   */
  public boolean setTypeQNameAll(
    QNameDAO qnameDAO,
    QName typeQName,
    boolean forUpdate
  ) {
  ...
  public void setChildNodeNameAll(
    ...
    if (dictionaryService != null) {
      ...
      this.childNodeName =
        ChildAssocEntity.getChildNodeNameShort(childNameNewLower);
      this.childNodeNameCrc =
  ...
  public boolean setQNameAll(
    ...
    if (forUpdate) {
      assocQNameNamespaceId =
        qnameDAO.getOrCreateNamespace(assocQNameNamespace).getFirst();
    } else {

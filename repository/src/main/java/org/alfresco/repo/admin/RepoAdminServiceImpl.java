...
public class RepoAdminServiceImpl implements RepoAdminService {
  ...
  private static final String MODEL_NOT_FOUND =
    "repoadmin_service.model_not_found";
  private static final String MODELS_MULTIPLE_FOUND =
    "repoadmin_service.models_multiple_found";
  ...
  private static final String MODEL_NO_LONGER_EXISTS =
    "repoadmin_service.model_no_longer_exists";
  ...
  public void setRepositoryMessagesLocation(
    RepositoryLocation repoMessagesLocation
  ) {
  ...
  public List<RepoModelDefinition> getModels() {
    ...
    if (nodeRefs.size() > 0) {
      for (NodeRef nodeRef : nodeRefs) {
        String modelFileName = (String) nodeService.getProperty(
          nodeRef,
          ContentModel.PROP_NAME
        );
        String repoVersion = (String) nodeService.getProperty(
          nodeRef,
        ...
        try {
          ...
          logger.warn(
            "Skip model: " + modelFileName + " (" + t.getMessage() + ")"
          );
        }
      }
    }
      ...
      if (nodeRefs.size() == 1) {
        ...
        try {
          ...
          ChildAssociationRef association = nodeService.createNode(
            ...
            contentProps
          ); // also invokes policies for DictionaryModelType - e.g. onUpdateProperties
    ...
    if (activate) {
      ...
      if (isActive) {
        if (modelDef != null) {
          ...
          throw new AlfrescoRuntimeException(
            ...
            new Object[] { modelQName }
          );
        } else {
          logger.warn(
            "Model is set to active but not loaded in Dictionary - trying to load..."
          );
        }
      } else {
        if (modelDef != null) {
          logger.warn(
            "Model is loaded in Dictionary but is not set to active - trying to activate..."
          );
        }
      }
    } else {
      ...
      if (!isActive) {
        if (modelDef == null) {
          ...
          throw new AlfrescoRuntimeException(
            MODEL_ALREADY_DEACTIVATED,
            new Object[] { modelQName }
          );
        } else {
          logger.warn(
            "Model is set to inactive but loaded in Dictionary - trying to unload..."
          );
        }
      } else {
        if (modelDef == null) {
          logger.warn(
            "Model is not loaded in Dictionary but is set to active - trying to deactivate..."
          );
        }
      }
    }
  ...
  public QName undeployModel(String modelFileName) {
    ...
    try {
      ...
      if (isActive) {
        if (modelDef == null) {
          logger.warn(
            "Model is set to active but not loaded in Dictionary - trying to undeploy..."
          );
        }
      }
    ...
    for (NodeRef nodeRef : nodeRefs) {
      ...
      if (registeredBundles != null) {
        for (String registeredBundlePath : registeredBundles) {
          if (
            registeredBundlePath.endsWith(resourceBundleBaseName) &&
        ...
        // unexpected
        logger.error(
          "Message bundle not registered: " + resourceBundleBaseName
        );
      }
    }
  ...
  public String deployMessageBundle(String resourceClasspath) {
    ...
    String pattern =
      ...
      resourceClasspath +
      "*" +
      MessageServiceImpl.PROPERTIES_FILE_SUFFIX;
    ...
    try {
      ...
      if ((resources != null) && (resources.length > 0)) {
        ...
        throw new AlfrescoRuntimeException(
          ...
          new Object[] { resourceClasspath }
        );
      }
    } catch (Throwable e) {
      throw new AlfrescoRuntimeException(
        MSG_RESOURCES_DEPLOYMENT_FAILED,
        new Object[] { resourceClasspath },
        e
      );
    }
  ...
  public void undeployMessageBundle(String bundleBaseName) {
    ...
    try {
      ...
      for (NodeRef nodeRef : nodeRefs) {
        ...
        if (
          bundleBaseName.equals(messageService.getBaseBundleName(resourceName))
        ) {

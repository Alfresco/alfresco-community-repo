...
public class FilePlanServiceImpl
  ...
  private static final String MSG_CONTAINER_PARENT_TYPE =
    "rm.service.container-parent-type";
  private static final String MSG_CONTAINER_TYPE = "rm.service.container-type";
  private static final String MSG_CONTAINER_EXPECTED =
    "rm.service.container-expected";
  ...
  public NodeRef getFilePlanBySiteId(String siteId) {
    ...
    if (
      siteInfo != null &&
      getSiteService().hasContainer(siteId, FILE_PLAN_CONTAINER)
    ) {
      NodeRef nodeRef = getSiteService()
        .getContainer(siteId, FILE_PLAN_CONTAINER);
   ...
   */
  private NodeRef getFilePlanRootContainer(
    NodeRef filePlan,
  ...
  public NodeRef createTransferContainer(NodeRef filePlan) {
    return createFilePlanRootContainer(
      filePlan,
      TYPE_TRANSFER_CONTAINER,
      NAME_TRANSFER_CONTAINER
    );
  }
  ...
  private NodeRef createFilePlanRootContainer(
    ...
    if (!isFilePlan(filePlan)) {
      throw new AlfrescoRuntimeException(
        "Unable to create file plan root container, because passed node is not a file plan."
      );
    }

    String allRoles = getFilePlanRoleService()
      .getAllRolesContainerGroup(filePlan);
  ...
  public NodeRef createFilePlan(
    ...
    if (
      !TYPE_FILE_PLAN.equals(type) &&
      !dictionaryService.isSubClass(type, TYPE_FILE_PLAN)
    ) {
      throw new AlfrescoRuntimeException(
        I18NUtil.getMessage(MSG_ROOT_TYPE, type.toString())
      );
    }
  ...
  public List<NodeRef> getNodeRefPath(NodeRef nodeRef) {
    ...
    try {
      ...
      throw new AlfrescoRuntimeException(
        I18NUtil.getMessage(MSG_PATH_NODE, nodeRef),
        e
      );
    }
    return nodeRefPath;
   ...
   */
  private void getNodeRefPathRecursive(
    NodeRef nodeRef,
    Deque<NodeRef> nodeRefPath
  ) {
  ...
  public NodeRef createRecordCategory(
    ...
    if (
      !TYPE_RECORDS_MANAGEMENT_CONTAINER.equals(parentType) &&
    ...
    if (
      !TYPE_RECORD_CATEGORY.equals(type) &&
   ...
   */
  private List<NodeRef> getContained(
    NodeRef container,
    QName typeFilter,
    boolean deep
  ) {
    ...
    if (!isRecordCategory(container)) {
      throw new AlfrescoRuntimeException(
        I18NUtil.getMessage(MSG_CONTAINER_EXPECTED)
      );
    }
   ...
   */
  @Override
  public List<NodeRef> getContainedRecordCategories(
    NodeRef container,
    boolean deep
  ) {
   ...
   */
  @Override
  public List<NodeRef> getContainedRecordFolders(
    NodeRef container,
    boolean deep
  ) {

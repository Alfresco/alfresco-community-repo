...
public class SOLRWebScriptTest extends BaseWebScriptTest {
  ...
  protected void setUp() throws Exception {
    ...
    searchTrackingComponent =
      (SearchTrackingComponent) ctx.getBean("searchTrackingComponent");
    ...
    storeRef =
      nodeService.createStore(
        ...
        getName() + ".1." + System.currentTimeMillis()
      );
  ...
  public void testAclsGet() throws Exception {
    List<AclChangeSet> aclChangeSets = searchTrackingComponent.getAclChangeSets(
      null,
      null,
      null,
      null,
    ...
    // Check
    List<Acl> aclsCheck = searchTrackingComponent.getAcls(
      aclChangeSetIds,
      null,
      512
    );
    assertEquals(
      "Script and API returned different number of results",
      aclsCheck.size(),
      acls.length()
    );
  }
  ...
  private void aclReadersGetImpl() throws Exception {
    ...
    assertEquals(
      "Should have same number of ACLs as supplied",
      aclIds.size(),
      aclsReaders.size()
    );
    ...
    for (AclReaders aclReaders : aclsReaders) {
      assertNotNull(
        "AclReaders should not contain null denial set",
        aclReaders.getDenied()
      );
    ...
    for (int i = 0; i < aclsReadersJSON.length(); i++) {
      ...
      assertEquals(
        "Readers list for ACL " + aclIdJSON + " is wrong. ",
        readersCheck.size(),
        readersJSON.length()
      );
      for (int j = 0; j < readersJSON.length(); j++) {
        ...
        assertTrue(
          "Found reader not in check set: " + readerJSON,
          readersCheck.contains(readerJSON)
        );
      }
      ...
      assertEquals(
        "Denied list for ACL " + aclIdJSON + " is wrong. ",
        deniedCheck.size(),
        deniedJSON.length()
      );
      for (int j = 0; j < deniedJSON.length(); j++) {
        ...
        assertTrue(
          "Found denied authority not in check set: " + denyJSON,
          deniedCheck.contains(denyJSON)
        );
      }
    }
  }
  ...
  private JSONArray getNodes(
    ...
    logger.debug(
      "Got " + nodes.length() + " nodes in " + (endTime - startTime) + " ms"
    );

    assertEquals(
      "Number of returned node meta data results is incorrect",
      expectedNumNodes,
      nodes.length()
    );
  ...
  private List<Long> getTransactionIds(JSONArray transactions)
    ...
    txnHelper.doInTransaction(
      new RetryingTransactionCallback<Void>() {
        public Void execute() throws Throwable {
          ...
          for (int i = 0; i < 100; i++) {
            FileInfo content1Info = fileFolderService.create(
              container3,
              "Content" + i,
  ...
  private JSONArray getNodesMetaData(
    List<Long> nodeIds,
    int maxResults,
    int numMetaDataNodes
  ) throws Exception {
    ...
    txnHelper.doInTransaction(
      new RetryingTransactionCallback<Void>() {
        public Void execute() throws Throwable {
          PropertyMap props = new PropertyMap();
          props.put(ContentModel.PROP_NAME, "Container6");
          NodeRef container6 = nodeService
            .createNode(
              rootNodeRef,
              ContentModel.ASSOC_CHILDREN,
              ContentModel.ASSOC_CHILDREN,
  ...
  public void testNodeMetaData() throws Exception {
    ...
    assertTrue(
      "Expected author aspect",
      containsAspect(aspects, ContentModel.ASPECT_AUTHOR)
    );
    assertTrue(
      "Expected author property",
      containsProperty(propertyMap, ContentModel.PROP_AUTHOR, "steve")
    );
  ...
  public void testNodeMetaDataStringEscaping() throws Exception {
    ...
    assertTrue(
      "Expected author property",
      containsProperty(propertyMap, ContentModel.PROP_AUTHOR, "ste\"ve")
    );
  }
  ...
  private boolean containsAspect(JSONArray aspectsArray, QName aspect)
    throws Exception {
    if (aspect == null) {
  ...
  private Map<QName, String> getPropertyMap(JSONObject properties)
    throws Exception {
    Map<QName, String> propertyMap = new HashMap<QName, String>(
      properties.length()
    );
  ...
  public void DISABLED_testGetContent() throws Exception {
    ...
    String url =
      "/api/solr/content?nodeId=" + nodeId + "&propertyName=" + propertyName;
    TestWebScriptServer.GetRequest req = new TestWebScriptServer.GetRequest(
      url
    );
    Response response = sendRequest(req, Status.STATUS_OK, admin);

...
/**
 * Tests {@link GetMethod} in real environment, using {@link Mock} HTTP request and {@link Mock}
 * HTTP response
 *
...
public class GetMethodRegressionTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        auditRegistry =
                (AuditModelRegistryImpl) applicationContext.getBean(AUDIT_REGISTRY_BEAN_NAME);
        ...
        rootTestFolder =
                fileFolderService
                        .create(companyHomeNodeRef, ROOT_TEST_FOLDER_NAME, ContentModel.TYPE_FOLDER)
                        .getNodeRef();
    }
    ...
    public void testAuditRecordsAdditionAsbsence() throws Exception {
        String url =
                new StringBuilder(TEST_WEBDAV_URL_PREFIX).append(ROOT_TEST_FOLDER_NAME).toString();
        ...
        boolean alfrescoAccessEnabled =
                Boolean.valueOf(auditRegistry.getProperty(PROP_AUDIT_ALFRESCO_ACCESS_ENABLED));
        ...
        try {
            ...
            if (null == expectedLastTime) {
                assertNull(
                        "Audit entry table is not empty after 'GetMethod.executeImpl()' invocation."
            ...
            if (!alfrescoAccessEnabled) {
                setAuditRegistryProperty(
                        PROP_AUDIT_ALFRESCO_ACCESS_ENABLED, Boolean.FALSE.toString());
            }
    ...
    private void createTestContent(NodeRef parentNode, int documentsAmount) {
        for (int i = 0; i < documentsAmount; i++) {
            String testDocumentName =
                    String.format(TEST_DOCUMENT_NAME_PATTERN, i, System.currentTimeMillis());
            FileInfo testDocument =
                    fileFolderService.create(
                            parentNode, testDocumentName, ContentModel.TYPE_CONTENT);
    ...
    /**
     * Commits or rolls back or does nothing with the current transaction and begins a new {@link
     * UserTransaction}
     *
     * @param transactionAction - one of the {@link TransactionActionEnum} values which specifies
     *     action to be done for the current transaction
     * @throws Exception
     */

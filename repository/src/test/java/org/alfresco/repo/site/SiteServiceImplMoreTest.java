 ...
 *
 * <p>TODO Refactor the two classes together into one common approach.
 *
...
public class SiteServiceImplMoreTest {
    ...
    @ClassRule
    public static RuleChain staticRuleChain =
    ...
    // Need to ensure the rules are in the correct order so that we're authenticated as admin before
    // trying to create the person.
    @Rule
    public RuleChain ruleChain =
            RuleChain.outerRule(runAllTestsAsAdmin).around(perMethodTestSites).around(testUser);
    ...
    public static void initStaticData() throws Exception {
        ...
        PERMISSION_SERVICE =
                APP_CONTEXT_INIT
                        .getApplicationContext()
                        .getBean("permissionServiceImpl", PermissionService.class);
        ...
        FILE_FOLDER_SERVICE =
                APP_CONTEXT_INIT
                        .getApplicationContext()
                        .getBean("FileFolderService", FileFolderService.class);
        AUTHENTICATION_COMPONENT =
                APP_CONTEXT_INIT
                        .getApplicationContext()
                        .getBean("authenticationComponent", AuthenticationComponent.class);
        LOCK_SERVICE =
                APP_CONTEXT_INIT.getApplicationContext().getBean("lockService", LockService.class);
        ...
        TEST_SITE_WITH_MEMBERS =
                STATIC_TEST_SITES.createTestSiteWithUserPerRole(
                        SiteServiceImplMoreTest.class.getSimpleName(),
                        "sitePreset",
                        SiteVisibility.PUBLIC,
                        admin);
    }
    ...
    /**
     * This method ensures that {@link SiteService#listSites(String)} includes content subtypes of
     * {@link SiteModel#TYPE_SITE st:site}.
     */
    @Test
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        PagingResults<SiteInfo> sites =
                                SITE_SERVICE.listSites(null, null, new PagingRequest(0, 1024));
                        ...
                        assertNotNull(
                                "subtype of st:site missing.", sitesByName.get(TEST_SUB_SITE_NAME));
    ...
    /**
     * This method ensures that sites can be deleted by any SiteManager, not just the Site Creator
     * (ALF-15257).
     */
    @Test
    public void anySiteManagerShouldBeAbleToDeleteASite() throws Exception {
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        // We already have a site with 4 users created by the TestSiteAndMemberInfo
                        // rule above.
                        // We'll use the testUser from the rule above - oh how I wish JUnit
                        // supported @Test-level rules, so I didn't have to create that user for all
                        // tests.
                        SITE_SERVICE.setMembership(
                                TEST_SITE_WITH_MEMBERS.siteInfo.getShortName(),
    ...
    /**
     * This test ensures that site deleted before MNT-10109 fix (i.e. deleted with their associated
     * authorities) could be restored with site-groups recreation
     *
     * @throws Exception
     */
    @Test
    public void deleteSiteDeleteAuthoritiesAndRestoreEnsuringSiteGroupsWasRecreated()
            throws Exception {
        ...
        final TestSiteAndMemberInfo testSiteAndMemberInfo =
                perMethodTestSites.createTestSiteWithUserPerRole(
                        siteShortName,
                        "sitePreset",
                        SiteVisibility.PUBLIC,
        ...
        final Map<String, String> membersBefore =
                TRANSACTION_HELPER.doInTransaction(
                        new RetryingTransactionCallback<Map<String, String>>() {
                            public Map<String, String> execute() throws Throwable {
                                ...
                                Map<String, String> membersBefore =
                                        SITE_SERVICE.listMembers(
                                                siteShortName, null, null, 0, true);
                                log.debug(membersBefore.size() + " members...");
                                for (Map.Entry<String, String> entry : membersBefore.entrySet()) {
                                    log.debug(entry);
                                }

                                Map<String, Set<String>> groupsMemberships =
                                        new HashMap<String, Set<String>>();

                                log.debug("About to delete site-related groups.");
                                // delete authorities
                                Set<String> permissions =
                                        PERMISSION_SERVICE.getSettablePermissions(
                                                SiteModel.TYPE_SITE);
                                for (String permission : permissions) {
                                    String prefixSiteRoleGroup =
                                            siteServiceImpl.getSiteRoleGroup(
                                                    siteShortName, permission, true);

                                    Set<String> groupUsers =
                                            AUTHORITY_SERVICE.getContainedAuthorities(
                                                    null, prefixSiteRoleGroup, true);
                                ...
                                NODE_SERVICE.setProperty(
                                        ...
                                        (Serializable) groupsMemberships);
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        assertThatArchivedNodeExists(
                                testSiteAndMemberInfo.siteInfo.getNodeRef(),
                                "Site node not found in archive.");

                        // ensure there are no authorities
                        Set<String> permissions =
                                PERMISSION_SERVICE.getSettablePermissions(SiteModel.TYPE_SITE);
                        for (String permission : permissions) {
                            String permissionGroupShortName =
                                    siteServiceImpl.getSiteRoleGroup(
                                            siteShortName, permission, false);
                            String authorityName =
                                    AUTHORITY_SERVICE.getName(
                                            AuthorityType.GROUP, permissionGroupShortName);

                            assertTrue(
                                    "Authotiry should not exist : " + authorityName,
                                    !AUTHORITY_SERVICE.authorityExists(authorityName));
                        }

                        log.debug("About to restore site node from archive");

                        final NodeRef archivedSiteNode =
                                NODE_ARCHIVE_SERVICE.getArchivedNode(
                                        testSiteAndMemberInfo.siteInfo.getNodeRef());
                        RestoreNodeReport report =
                                NODE_ARCHIVE_SERVICE.restoreArchivedNode(archivedSiteNode);
                        // ...which should work
                        assertEquals(
                                "Failed to restore site from archive",
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Map<String, String>>() {
                    public Map<String, String> execute() throws Throwable {
                        ...
                        assertTrue(
                                "The site noderef was not restored as expected",
                                NODE_SERVICE.exists(testSiteAndMemberInfo.siteInfo.getNodeRef()));

                        Map<String, String> members =
                                SITE_SERVICE.listMembers(siteShortName, null, null, 0, true);
                        assertEquals(
                                "Not all member have been restored",
                                membersBefore.size(),
                                members.size());
                        log.debug(members.size() + " members...");
                        for (Map.Entry<String, String> entry :
                                SITE_SERVICE
                                        .listMembers(siteShortName, null, null, 0, true)
                                        .entrySet()) {
                            log.debug(entry);
                        }
                        ...
                        for (String role : SITE_SERVICE.getSiteRoles()) {
                            final String siteGroup =
                                    SITE_SERVICE.getSiteRoleGroup(siteShortName, role);
                            assertTrue(
                                    "Site group for role "
                                            + role
                                            + " did not exist after site restoration",
                ...
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        NodeRef archivedNodeRef =
                                new NodeRef(
                                        storeRef,
                                        testSiteAndMemberInfo.siteInfo.getNodeRef().getId());
     ...
     *
     * <p>This test also checks that after restore the locks are restored correctly for all the
     * locked files;
     *
     * <p>MNT-15855: Checks the case when there is a working copy (simulate lock for offline edit)
     *
     * @throws Exception
     */
    @Test
    public void deleteSiteDeleteAuthoritiesAndRestoreEnsuringLocksAreRestored() throws Exception {
        ...
        final TestSiteAndMemberInfo testSiteAndMemberInfo =
                perMethodTestSites.createTestSiteWithUserPerRole(
                        siteShortName, "sitePreset", SiteVisibility.PUBLIC, userOwner);
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Object>() {
                    public Object execute() throws Throwable {
                        SITE_SERVICE.setMembership(
                                siteShortName, userCollaborator, SiteModel.SITE_COLLABORATOR);
                        return null;
                    }
                });
        ...
        final FileInfo fileInfoForCollaboratorUser =
                FILE_FOLDER_SERVICE.create(
                        folder2Info.getNodeRef(),
                        fileFolderPrefix + "userCollaborator.txt",
                        ContentModel.TYPE_CONTENT);
        ContentWriter writer4 =
                FILE_FOLDER_SERVICE.getWriter(fileInfoForCollaboratorUser.getNodeRef());
        writer4.putContent("Just some old content that doesn't mean anything");

        // Check that the node is not currently locked
        assertEquals(
                LockStatus.NO_LOCK,
                LOCK_SERVICE.getLockStatus(fileInfoForCollaboratorUser.getNodeRef()));
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        LOCK_SERVICE.lock(
                                fileInfoForCollaboratorUser.getNodeRef(), LockType.WRITE_LOCK);
                        return null;
                    }
                });
        ...
        final FileInfo fileEditOffline =
                FILE_FOLDER_SERVICE.create(
                        siteContainer,
        ...
        assertEquals(
                LockStatus.LOCK_OWNER, LOCK_SERVICE.getLockStatus(fileEditOffline.getNodeRef()));
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        // if we apply the sys:temporary aspect to the site, the NodeService will
                        // not
                        // archive it i.e. this bit of clean up will be a bit faster.
                        NODE_SERVICE.addAspect(
                                testSiteAndMemberInfo.siteInfo.getNodeRef(),
    ...
    private void checkThatNonMembersCanNotCreateFiles(
            final String userCollaborator, String fileFolderPrefix, FileInfo folder2Info) {
        // now, as another user, that is not yet a member, try to create a file in this site
        // this use case is not really relevant to test method, but it is a good test for
        // permissions
        AUTHENTICATION_COMPONENT.setCurrentUser(userCollaborator);
        try {
            FileInfo fileInfoForTestNewFileAsAnotherUser =
                    FILE_FOLDER_SERVICE.create(
                            folder2Info.getNodeRef(),
                            fileFolderPrefix + "user2.txt",
                            ContentModel.TYPE_CONTENT);
            ContentWriter writer3_user2 =
                    FILE_FOLDER_SERVICE.getWriter(fileInfoForTestNewFileAsAnotherUser.getNodeRef());
    ...
    /**
     * This test ensures that when sites are deleted (moved to the trashcan) and then restored, that
     * the 4 role-based groups are restored correctly and that any users who were members of those
     * groups are made members once more.
     *
    ...
    public void deleteSiteAndRestoreEnsuringSiteGroupsAreRecovered() throws Exception {
        // Implementation note: as of Alfresco 4.2, authorities cannot be archived and are always
        // hard-deleted.
        // Therefore, on site restore, the SiteService *recreates* the deleted groups associated
        // with the restored site.
        // Having recreated them, it then needs to add any old members (users, TODO what about
        // groups?) into those newly created site groups.
        // It does this by writing the site members and their roles onto a property on the st:site
        // node in the archive & reading them on restore.
        ...
        // Now get the various site-related data that we want to examine after deletion &
        // restoration
        final TestData testData =
                TRANSACTION_HELPER.doInTransaction(
                        new RetryingTransactionCallback<TestData>() {
                            public TestData execute() throws Throwable {
                                ...
                                properties.put(
                                        ...
                                        "content - test doc for test");
                                ChildAssociationRef testDoc =
                                        NODE_SERVICE.createNode(
                                                testSiteAndMemberInfo.doclib,
                                ...
                                PERMISSION_SERVICE.setInheritParentPermissions(
                                        testDocNodeRef, false);
                                assertTrue(
                                        "Permissions should be cleared",
                                        PERMISSION_SERVICE
                                                .getAllSetPermissions(testDocNodeRef)
                                                .isEmpty());
                                ...
                                for (String permission : permissions) {
                                    String siteRoleGroup =
                                            SITE_SERVICE.getSiteRoleGroup(
                                                    siteShortName, permission);
                                    PERMISSION_SERVICE.setPermission(
                                            testDocNodeRef, siteRoleGroup, "SiteContributor", true);
                                }
                                ...
                                for (String role : SITE_SERVICE.getSiteRoles()) {
                                    // putAll here shouldn't overwrite any keys in the above map as
                                    // each authority should only have one role in a site.
                                    userNameToRoleMap.putAll(
                                            SITE_SERVICE.listMembers(
                                                    siteShortName, null, role, 0, true));
                                }

                                // Some sanity checking before we delete the site
                                final String siteContributorGroup =
                                        SITE_SERVICE.getSiteRoleGroup(
                                                siteShortName, "SiteContributor");
                                assertTrue(
                                        "Site contributor user was not in site contributors group",
                                        AUTHORITY_SERVICE
                                                .getContainedAuthorities(
                                                        AuthorityType.USER,
                                                        siteContributorGroup,
                                                        true)
                                                .contains(testSiteAndMemberInfo.siteContributor));
                                assertEquals(
                                        "Site contributor user did not have expected Contributor"
                                                + " role",
                                        SiteModel.SITE_CONTRIBUTOR,
                                        userNameToRoleMap.get(
                                                testSiteAndMemberInfo.siteContributor));
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        // At this point we might assert that the groups associated with the site
                        // were gone, but that's an implementation detail really.
        ...
        final Map<String, String> associatedGroups =
                TRANSACTION_HELPER.doInTransaction(
                        new RetryingTransactionCallback<Map<String, String>>() {
                            public Map<String, String> execute() throws Throwable {
                                // The site itself should have been restored, of course...
                                ...
                                // But the group authority nodes should be restored (recreated) as
                                // well.
                                for (String role : SITE_SERVICE.getSiteRoles()) {
                                    final String siteGroup =
                                            SITE_SERVICE.getSiteRoleGroup(siteShortName, role);
                                ...
                                // And finally, the original members of the site should have been
                                // given the same membership that they had before.
                                for (Map.Entry<String, String> entry :
                                        testData.userNameToRoleMap.entrySet()) {
                                    assertEquals(
                                            "Unexpected role for site user: " + entry.getKey(),
                                            entry.getValue(),
                                            SITE_SERVICE.getMembersRole(
                                                    siteShortName, entry.getKey()));
                                }

                                // When the site is restored custom permissions on contents should
                                // be also restored
                                Set<AccessPermission> accessPermissions =
                                        PERMISSION_SERVICE.getAllSetPermissions(
                                                testData.testDocNodeRef);
                                Map<String, String> associatedGroups =
                                        new HashMap<String, String>();
                                for (AccessPermission access : accessPermissions) {
                                    associatedGroups.put(
                                            access.getAuthority(), access.getPermission());
                                }
                                Set<String> permissions =
                                        PERMISSION_SERVICE.getSettablePermissions(
                                                SiteModel.TYPE_SITE);
                                for (String permission : permissions) {
                                    String siteRoleGroup =
                                            SITE_SERVICE.getSiteRoleGroup(
                                                    siteShortName, permission);
                                    assertTrue(
                                            "all groups should have the permissions from a"
                                                    + " contributor on test content",
                                            "SiteContributor"
                                                    .equals(associatedGroups.get(siteRoleGroup)));
                                }
                ...
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        for (String authority : associatedGroups.keySet()) {
                            assertTrue(
                                    "Associated groups should remain after site delete",
                                    AUTHORITY_SERVICE.authorityExists(authority));
                        }
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        for (String authority : associatedGroups.keySet()) {
                            assertTrue(
                                    "Associated groups should be deleted on site purge",
                                    !AUTHORITY_SERVICE.authorityExists(authority));
                        }
                ...
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        List<Pair<SiteService.SortFields, Boolean>> sortProps =
                                new ArrayList<Pair<SiteService.SortFields, Boolean>>(1);
                        sortProps.add(
                                new Pair<SiteService.SortFields, Boolean>(
                                        SiteService.SortFields.FirstName, true));
                        PagingResults<SiteMembership> pagedMembers =
                                SITE_SERVICE.listMembersPaged(
                                        siteShortName, true, sortProps, new PagingRequest(100));
    ...
    public void testSiteGroupsPaged() {
        ...
        TRANSACTION_HELPER.doInTransaction(
                () -> {
                    perMethodTestSites.createTestSiteWithGroups(
                            siteShortName,
                            "sitePreset",
                            SiteVisibility.PUBLIC,
                            AuthenticationUtil.getAdminUserName(),
                    ...
                    sortProps.add(
                            new Pair<SiteService.SortFields, Boolean>(
                                    SiteService.SortFields.DisplayName, true));
                    PagingResults<SiteGroupMembership> pagedMembers =
                            SITE_SERVICE.listGroupMembersPaged(
                                    siteShortName, sortProps, new PagingRequest(5));
                    assertNotNull(pagedMembers);
                    assertNotNull(pagedMembers.getQueryExecutionId());
    ...
    public void testSiteMembersPagedV2() {
        ...
        TRANSACTION_HELPER.doInTransaction(
                () -> {
                    ...
                    PagingResults<SiteMembership> pagedMembers =
                            SITE_SERVICE.listMembersPaged(
                                    siteShortName, true, sortProps, new PagingRequest(25));
                    assertNotNull(pagedMembers);
                    assertNotNull(pagedMembers.getQueryExecutionId());
                    assertFalse(pagedMembers.hasMoreItems());
                    assertEquals(pagedMembers.getPage().size(), 11);

                    List<SiteMembership> users =
                            pagedMembers.getPage().stream()
                                    .filter((member) -> !member.isMemberOfGroup())
                                    .collect(Collectors.toList());
                    List<SiteMembership> groupsUsers =
                            pagedMembers.getPage().stream()
                                    .filter(SiteMembership::isMemberOfGroup)
                                    .collect(Collectors.toList());
                    ...
                    pagedMembers =
                            SITE_SERVICE.listMembersPaged(
                                    siteShortName, false, sortProps, new PagingRequest(100));
                    assertNotNull(pagedMembers);
                    assertNotNull(pagedMembers.getQueryExecutionId());
                    assertFalse(pagedMembers.hasMoreItems());
    ...
    public void testSiteRolesPermissionsToDeleteWorkingCopy() throws Exception {
        ...
        final TestSiteAndMemberInfo testSiteAndMemberInfo =
                perMethodTestSites.createTestSiteWithUserPerRole(
                        siteShortName, "sitePreset", SiteVisibility.PUBLIC, userSiteOwner);
        ...
        NodeRef siteContainer =
                SITE_SERVICE.createContainer(
                        siteShortName, "doclib", ContentModel.TYPE_FOLDER, null);
        final FileInfo fileInfo1 =
                FILE_FOLDER_SERVICE.create(
                        siteContainer, "fileInfo1.txt", ContentModel.TYPE_CONTENT);
        ...
        final FileInfo fileInfo2 =
                FILE_FOLDER_SERVICE.create(
                        siteContainer, "fileInfo2.txt", ContentModel.TYPE_CONTENT);
        ContentWriter writer2 = FILE_FOLDER_SERVICE.getWriter(fileInfo2.getNodeRef());
        writer2.putContent(dummyContent);

        final FileInfo fileInfo3 =
                FILE_FOLDER_SERVICE.create(
                        siteContainer, "fileInfo3.txt", ContentModel.TYPE_CONTENT);
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        SITE_SERVICE.setMembership(
                                siteShortName, userSiteCollaborator, SiteModel.SITE_COLLABORATOR);
                        ...
                        try {
                            ...
                            fail(
                                    "You do not have the appropriate permissions to perform this"
                                            + " operation");
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        SITE_SERVICE.setMembership(
                                siteShortName, userSiteManager, SiteModel.SITE_MANAGER);
    ...
    /**
     * REPO-1688: Restore Site should disable lockable behaviour (to be symmetric with
     * delete/purge). See also MNT-17093 for background on this.
     *
     * @throws Exception
     */
    @Test
    public void deleteSiteRestoreSiteWithLocks() throws Exception {
        ...
        // Add custom behaviour that triggers onCreateNode to cause update ... and hence triggers
        // REPO-1688
    ...
    /**
     * REPO-2811 / ALF-21924: "Internal Server Error" when listSites is invoked for users belonging
     * to a deleted Site.
     *
     * @throws Exception
     */
    ...
    public void testListMembershipOnSitesDifferentCase() throws Exception {
        ...
        perMethodTestSites.createSite(
                "sitePreset", siteShortName, null, null, SiteVisibility.PUBLIC, userSiteOwner);
        perMethodTestSites.createSite(
                "sitePreset",
                secondSiteShortName,
                null,
        ...
        perMethodTestSites.createSite(
                "sitePreset", siteShortName1, null, null, SiteVisibility.PUBLIC, userSiteOwner);

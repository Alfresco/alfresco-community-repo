...
public class ResultMapperTests {
  ...
  public static final Params EMPTY_PARAMS = Params.valueOf(
    (String) null,
    null,
    null
  );
  ...
  public static void setupTests() {
    ...
    when(versionService.getVersionHistory(any(NodeRef.class)))
      .thenReturn(versionHistory);
    ...
    when(nodeService.getProperties(any(NodeRef.class)))
      .thenAnswer(invocation -> {
        ...
        if (
          StoreMapper.STORE_REF_VERSION2_SPACESSTORE.equals(aNode.getStoreRef())
        ) {
          nodeProps.put(
    ...
    when(
      nodes.validateOrLookupNode(nullable(String.class), nullable(String.class))
    )
      .thenAnswer(invocation -> {
        Object[] args = invocation.getArguments();
    ...
    when(
      ...
      .thenAnswer(
        (Answer<Node>) invocation -> {
          Object[] args = invocation.getArguments();
          NodeRef aNode = (NodeRef) args[0];
          if (
            StoreRef.STORE_REF_ARCHIVE_SPACESSTORE.equals(aNode.getStoreRef())
          ) {
    ...
    when(
      ...
      .thenAnswer(
        (Answer<Node>) invocation -> {
          ...
          if (FROZEN_ID.equals(nodeId)) throw new EntityNotFoundException(
            nodeId
          );
          ...
          return new Node(
            aNode,
            new NodeRef(StoreRef.STORE_REF_ARCHIVE_SPACESSTORE, "unknown"),
            nodeProps,
            mapUserInfo,
            sr
          );
  ...
  public void testNoResults() {
    ...
    CollectionWithPagingInfo<Node> collection = mapper.toCollectionWithPagingInfo(
      EMPTY_PARAMS,
      searchRequest,
      null,
      new EmptyResultSet()
  ...
  public void testToCollectionWithPagingInfo() {
    ...
    CollectionWithPagingInfo<Node> collectionWithPage = mapper.toCollectionWithPagingInfo(
      EMPTY_PARAMS,
      searchRequest,
      SearchQuery.EMPTY,
      results
    ...
    assertEquals(
      found.intValue(),
      collectionWithPage.getTotalItems().intValue()
    );
    Node firstNode = collectionWithPage
      .getCollection()
      .stream()
      .findFirst()
      .get();
  ...
  public void testToSearchContext() {
    ...
    SearchContext searchContext = mapper.toSearchContext(
    ...
    assertEquals(
      "alfresco",
      searchContext.getSpellCheck().getSuggestions().get(0)
    );
    assertEquals(1, searchContext.getFacetsFields().size());
    assertEquals(
      "content.size",
      searchContext.getFacetsFields().get(0).getLabel()
    );

    //Facet intervals
    List<GenericFacetResponse> intervalFacets = searchContext
      .getFacets()
      .stream()
    ...
    Metric[] metrics = intervalFacets
      .get(0)
      .getBuckets()
      .get(0)
      .getMetrics()
      .toArray(
        new Metric[intervalFacets
          .get(0)
          .getBuckets()
          .get(0)
          .getMetrics()
    ...
    metrics =
      pivotBucket
        .getMetrics()
        .toArray(new Metric[pivotBucket.getMetrics().size()]);
    ...
    metrics =
      nestedBucket
        .getMetrics()
        .toArray(new Metric[nestedBucket.getMetrics().size()]);
    ...
    metrics =
      nestedBucket2
        .getMetrics()
        .toArray(new Metric[nestedBucket2.getMetrics().size()]);
    ...
    assertTrue(
      statsMetrics.contains(new SimpleMetric(METRIC_TYPE.countValues, 990))
    );
    assertTrue(
      statsMetrics.contains(new SimpleMetric(METRIC_TYPE.missing, 290))
    );
    assertTrue(
      statsMetrics.contains(
        new SimpleMetric(
          METRIC_TYPE.sum,
          BigDecimal.valueOf(1.458318720769983E15)
        )
      )
    );
    assertTrue(
      statsMetrics.contains(
        new SimpleMetric(
          METRIC_TYPE.stddev,
          BigDecimal.valueOf(5.6250677994522545E10)
        )
      )
    );
    ...
    assertTrue(
      statsMetrics.contains(new SimpleMetric(METRIC_TYPE.sumOfSquares, 0))
    );
    assertTrue(
      statsMetrics.contains(new SimpleMetric(METRIC_TYPE.countValues, 0))
    );
    ...
    assertTrue(
      statsMetrics.contains(new ListMetric(METRIC_TYPE.distinctValues, dVals))
    );
    JSONArray pers = new JSONArray(asList("0.99", 20.0685, "0.0", 12.0));
    assertTrue(
      statsMetrics.contains(new PercentileMetric(METRIC_TYPE.percentiles, pers))
    );
  ...
  public void testIsNullContext() {
    assertTrue(
      mapper.isNullContext(new SearchContext(0l, null, null, null, null, null))
    );
    assertFalse(
      mapper.isNullContext(new SearchContext(1l, null, null, null, null, null))
    );
    assertFalse(
      mapper.isNullContext(
        new SearchContext(
          ...
          null,
          null,
          new SpellCheckContext(null, null),
          null
        )
      )
    );
  ...
  public void testHighlight() {
    ...
    fields.add(
      new FieldHighlightParameters("myfield", null, null, null, "(", ")")
    );
    highlightParameters =
    ...
    assertEquals(
      true,
      sp.getHighlight().getUsePhraseHighlighter().booleanValue()
    );
  ...
  public void testInterval() {
    ...
    assertEquals(
      "earlier",
      intervalFacets.get(1).getBuckets().get(0).getLabel()
    );
    assertEquals(
      "cm:created:[\"*\" TO \"2016\">",
    ...
    assertEquals(
      "lastYear",
      intervalFacets.get(1).getBuckets().get(1).getLabel()
    );
    assertEquals(
    ...
    assertEquals(
      "currentYear",
      intervalFacets.get(1).getBuckets().get(2).getLabel()
    );
    assertEquals(
  ...
  public void testRange() {
    ...
    //Numeric facet range
    List<GenericFacetResponse> rangeFacets = searchContext
      .getFacets()
      .stream()
    ...
    assertEquals(
      null,
      rangeFacets.get(0).getBuckets().get(0).getBucketInfo().get("count")
    );
    assertEquals(
      "false",
    ...
    assertEquals(
      "[0 - 100)",
      rangeFacets.get(1).getBuckets().get(0).getLabel()
    );
    Object[] metrics = rangeFacets
      .get(1)
      .getBuckets()
      .get(0)
      .getMetrics()
    ...
    assertEquals(
      null,
      rangeFacets.get(1).getBuckets().get(0).getBucketInfo().get("count")
    );
    Map<String, String> facetInfo = rangeFacets
      .get(1)
      .getBuckets()
      .get(0)
      .getBucketInfo();
    ...
    assertEquals(
      "[100 - 200)",
      rangeFacets.get(1).getBuckets().get(1).getLabel()
    );
    ...
    assertEquals(
      null,
      rangeFacets.get(1).getBuckets().get(1).getBucketInfo().get("count")
    );
    assertEquals(
      "false",
    ...
    assertEquals(
      "[200 - 300)",
      rangeFacets.get(1).getBuckets().get(2).getLabel()
    );
  ...
  public void testFacetingGroupResponse() {
    ...
    assertEquals(
      "small",
      searchContext.getFacets().get(0).getBuckets().get(0).getLabel()
    );
    assertEquals(
  ...
  public void testFacetingGroupResponseV1() {
    String jsonQuery =
      "{\"query\": {\"query\": \"alfresco\"}, \"facetFormat\":\"V1\"," +
      "\"facetQueries\": [" +
  ...
  private ResultSet mockResultSet(String json) {
    ...
    SearchParameters sp = new SearchParameters();
    sp.setBulkFetchEnabled(false);
    ResultSet results = new SolrJSONResultSet(
      jsonObj,
      sp,
      nodeService,
      null,
  ...
  private ResultSet mockResultSet(
    List<Long> archivedNodes,
    List<Long> versionNodes
  ) {
    NodeService nodeService = mock(NodeService.class);
    when(nodeService.getNodeRef(any()))
      .thenAnswer(
        (Answer<NodeRef>) invocation -> {
          Object[] args = invocation.getArguments();
          //If the DBID is in the list archivedNodes, instead of returning a noderef return achivestore noderef
          if (archivedNodes.contains(args[0])) return new NodeRef(
            StoreRef.STORE_REF_ARCHIVE_SPACESSTORE,
            GUID.generate()
          );
          if (versionNodes.contains(args[0])) return new NodeRef(
            StoreMapper.STORE_REF_VERSION2_SPACESSTORE,
  ...
  public void facetFormatTest() {
    ...
    assertEquals(
      124,
      searchContext.getFacetsFields().get(0).getBuckets().get(0).getCount()
    );
    assertEquals(
      "creator:\"System\"",
      searchContext
    ...
    assertEquals(
      "System",
      searchContext.getFacetsFields().get(0).getBuckets().get(0).getLabel()
    );
  ...
  public void testFacetingWithPartialGroup() {
    ...
    assertEquals(
      "small",
      searchContext.getFacets().get(1).getBuckets().get(0).getLabel()
    );
    assertEquals(
      "medium",
      searchContext.getFacets().get(2).getBuckets().get(0).getLabel()
    );
    assertEquals(
      "content.size:[o TO 102400]",
  ...
  public void testSqlResponse() {
    ...
    SearchSQLQuery query = new SearchSQLQuery(
    ...
    CollectionWithPagingInfo<TupleList> info = mapper.toCollectionWithPagingInfo(
      docs,
      query
    );
  ...
  public void testGetNode() {
    ...
    assertNull(
      "Expected node to be filtered due to permission exception.",
      node
    );
  }
}

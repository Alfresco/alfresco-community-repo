...
public interface LuceneConfig {
    ...
    /**
     * The factor by which the target overlay count is multiplied to determine the allowable number
     * of overlays before blocking.
     *
     * @return the factor by which the target overlay count is multiplied to determine the allowable
     *     number of overlays before blocking
     */
    ...
    /**
     * If we are using the DateAnalyser then lucene sort is only to the date, as that is all that is
     * in the index. If this is true, a query that defines a sort on a datetime field will do a post
     * sort in Java.
     *
     * <p>For the DateTimeAnalyser no post sort is done. (The default config does do a post sort)
     *
     * <p>In the future, this behaviour may also be set per query on the SearchParameters object.
     *
     * @return boolean
     */
    ...
    /**
     * Should we use a 'fair' locking policy, giving queue-like access behaviour to the indexes and
     * avoiding starvation? Default is <code>false</code> since fair locking appears to cause
     * deadlock on old JVMs.
     *
    ...
    /**
     * Enable/Disable the indexing of content Content is not indexed and FTS disabled When disabled,
     * documents are marked as requiring FTS indexing. When enabled the normal FTS process will
     * catch up with content that was not indexed.
     *

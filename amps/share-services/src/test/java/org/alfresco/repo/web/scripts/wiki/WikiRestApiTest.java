...
public class WikiRestApiTest extends BaseWebScriptTest {
    ...
    private String getPageAtVersion(String name, String version, int expectedStatus)
        ...
        Response response =
                sendRequest(
                        new GetRequest(URL_WIKI_VERSION + name + "/" + version), expectedStatus);
        if (expectedStatus == Status.STATUS_OK) {
    ...
    private JSONObject createOrUpdatePage(
            String title, String contents, String version, int expectedStatus) throws Exception {
    ...
    private JSONObject renamePage(String oldTitle, String newTitle, int expectedStatus)
        ...
        Response response =
                sendRequest(
                        new PostRequest(
                                URL_WIKI_RENAME + name, json.toString(), "application/json"),
                        expectedStatus);
        if (expectedStatus == Status.STATUS_OK) {
            JSONObject result = new JSONObject(response.getContentAsString());
            return result;
    ...
    public void testRenamePageWithEmptyTitle() throws Exception {
        ...
        double vNum =
                Double.parseDouble(versions.getJSONObject(maxVersionIndex).getString("version"));
    ...
    public void testVersioning() throws Exception {
        ...
        assertEquals(
                true,
                nodeService.hasAspect(wikiInfo.getNodeRef(), ContentModel.ASPECT_VERSIONABLE));
        ...
        page =
                createOrUpdatePage(
                        PAGE_TITLE_TWO, PAGE_CONTENTS_CHANGED3, "force", Status.STATUS_OK);
    ...
    public void test_MNT11595() throws Exception {
        ...
        try {
            ...
            JSONObject resultConsumerRole =
                    new JSONObject(responseConsumerRole.getContentAsString());
    ...
    public void testXSSInjection() throws Exception {
        ...
        try {
            wikiPage =
                    this.wikiService.createWikiPage(SITE_SHORT_NAME_WIKI, "test_wiki", "[[Test]]");
            wikiPageNew =
                    this.wikiService.createWikiPage(SITE_SHORT_NAME_WIKI, "Test", "test_content");
            ...
            while (m.find()) {
                ...
                WikiPageInfo wikiPage1 =
                        this.wikiService.getWikiPage(
                                SITE_SHORT_NAME_WIKI, StringEscapeUtils.unescapeHtml(link));

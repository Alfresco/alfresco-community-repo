...
public class LockKeeperImpl implements LockKeeper {
  ...
  public void addLock(NodeRef nodeRef) {
    if (lockEnabled) {
      ...
      lockKeeperTransactionalCache.put(
        nodeRef,
        new KeeperInfo(AuthenticationUtil.getFullyAuthenticatedUser())
      );
    }
  }
  ...
  public void refreshAllLocks() {
    ...
    for (NodeRef nodeRef : nodes) {
      ...
      final KeeperInfo keeperInfo = lockKeeperTransactionalCache.get(
        nodeRefToRefresh
      );
      final String additionalInfo = lockService.getAdditionalInfo(
        nodeRefToRefresh
      );
              ...
              if (LOCK_KEEPER_KEY.equalsIgnoreCase(additionalInfo)) {
                ...
                switch (lockService.getLockStatus(nodeRefToRefresh)) {
                  case LOCK_OWNER:
                    if (logger.isDebugEnabled()) {
                      logger.debug(
                        "refresh ephemeral lock nodeRef: " + nodeRefToRefresh
                      );
                    }
                  ...
                  default:
                    if (logger.isDebugEnabled()) {
                      logger.debug(
                        "remove lock from lock keeper cache, nodeRef: " +
                        nodeRefToRefresh
                      );
                    }
                ...
                if (logger.isDebugEnabled()) {
                  logger.debug(
                    "not a lock keeper lock, remove lock from lock keeper cache, nodeRef: " +
                    nodeRefToRefresh
                  );
                }
                lockKeeperTransactionalCache.remove(nodeRefToRefresh);

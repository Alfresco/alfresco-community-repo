...
public class AlfrescoImapHostManager implements ImapHostManager {
   ...
   */
  public Collection<MailFolder> listMailboxes(
    GreenMailUser user,
    String mailboxPattern
  ) throws FolderException {
    try {
      AlfrescoImapUser alfrescoUser = new AlfrescoImapUser(
        user.getEmail(),
        user.getLogin(),
        user.getPassword()
      );
   ...
   * @throws com.icegreen.greenmail.store.FolderException
   */
  public Collection<MailFolder> listSubscribedMailboxes(
    GreenMailUser user,
    String mailboxPattern
  ) throws FolderException {
    try {
  ...
  public void renameMailbox(
    ...
    try {
      ...
      String oldFolderPath = getUnqualifiedMailboxPattern(
        alfrescoUser,
        oldMailboxName
      );
      String newFolderpath = getUnqualifiedMailboxPattern(
        alfrescoUser,
        newMailboxName
      );
   ...
   */
  public void createPrivateMailAccount(GreenMailUser user)
    throws FolderException {
   ...
   */
  public void subscribe(GreenMailUser user, String mailbox)
    throws FolderException {
    try {
      AlfrescoImapUser alfrescoUser = new AlfrescoImapUser(
        user.getEmail(),
      ...
      imapService.subscribe(
        alfrescoUser,
        getUnqualifiedMailboxPattern(alfrescoUser, mailbox)
      );
    } catch (Throwable e) {
      logger.debug(e.getMessage(), e);
      throw new FolderException(e.getMessage());
   ...
   * @param mailbox String representation of a mailbox name.
   */
  public void unsubscribe(GreenMailUser user, String mailbox)
    throws FolderException {
    try {
      AlfrescoImapUser alfrescoUser = new AlfrescoImapUser(
        user.getEmail(),
  ...
  private String getUnqualifiedMailboxPattern(
    ...
    if (mailboxPattern.startsWith(AlfrescoImapConst.NAMESPACE_PREFIX)) {
      int sepIndex = mailboxPattern.indexOf(
   ...
   */
  private Collection<MailFolder> registerMailboxes(
    Collection<AlfrescoImapFolder> mailboxes
  ) {

...
public class AlfrescoImapFolderAccessInterceptor implements MethodInterceptor {
  ...
  private Log logger = LogFactory.getLog(
    AlfrescoImapFolderAccessInterceptor.class
  );
  ...
  public Object invoke(MethodInvocation mi) throws Throwable {
    ...
    if ("contains".equals(methodName)) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Check the cache [" + methodName + "] for '" + mailboxName + "'"
        );
      }
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "The cache " +
          (containsResult ? "contains" : "does't contain") +
          " '" +
          mailboxName +
          "'"
        );
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "The cache " +
          (folder != null ? "contains" : "does't contain") +
          " '" +
          mailboxName +
          "'"
        );

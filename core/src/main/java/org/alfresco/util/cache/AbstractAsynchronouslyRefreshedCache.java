...
public abstract class AbstractAsynchronouslyRefreshedCache<T>
  ...
  private static final String RESOURCE_KEY_TXN_DATA =
    "AbstractAsynchronouslyRefreshedCache.TxnData";

  private static Log logger = LogFactory.getLog(
    AbstractAsynchronouslyRefreshedCache.class
  );
  ...
  public T get(String key) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "get() miss, scheduling and waiting for key " + key + " on " + this
      );
    }
  ...
  public void refresh(String key) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Async cache refresh request for tenant " + key + " on " + this
      );
    }
    registry.broadcastEvent(
      new RefreshableCacheRefreshEvent(cacheId, key),
      true
    );
  }
  ...
  public void onRefreshableCacheEvent(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Async cache onRefreshableCacheEvent " +
  ...
  private TransactionData getTransactionData() {
    TransactionData data = (TransactionData) TransactionSupportUtil.getResource(
      resourceKeyTxnData
    );
    if (data == null) {
  ...
  private void queueRefreshAndSubmit(LinkedHashSet<String> tenantIds) {
    ...
    try {
      for (String tenantId : tenantIds) {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Async cache adding refresh to queue for tenant " +
            tenantId +
            " on " +
            this
          );
  ...
  private void doRefresh(Refresh refresh) {
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Building cache for tenant" + refresh.getKey() + ": " + this
      );
    }
  ...
  public void broadcastEvent(RefreshableCacheEvent event) {
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Notifying cache listeners for " + getCacheId() + " " + event
      );
    }

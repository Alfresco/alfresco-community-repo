...
public class AspectsImpl extends AbstractClassImpl<Aspect> implements Aspects {
  ...
  public CollectionWithPagingInfo<Aspect> listAspects(Parameters params) {
    ...
    if (query != null && query.getModelIds() != null) {
      ...
      Set<Pair<QName, Boolean>> modelsFilter = parseModelIds(
        query.getModelIds(),
        PARAM_INCLUDE_SUBASPECTS
      );
      aspectStream =
        modelsFilter
          .stream()
          .map(this::getModelAspects)
  ...
  public Aspect getAspect(String aspectId) {
    if (aspectId == null) throw new InvalidArgumentException(
      "Invalid parameter: unknown scheme specified"
    );
    ...
    try {
      aspectDefinition =
        dictionaryService.getAspect(
          QName.createQName(aspectId, this.namespaceService)
        );
    } catch (NamespaceException exception) {
      throw new EntityNotFoundException(aspectId);
    }

    if (aspectDefinition == null) throw new EntityNotFoundException(aspectId);

    return this.convertToAspect(aspectDefinition, ALL_PROPERTIES);
  }

  public Aspect convertToAspect(
  ...
  private Collection<QName> getModelAspects(Pair<QName, Boolean> model) {
    ...
    Collection<QName> aspects =
      this.dictionaryService.getAspects(modelDefinition.getName());

    if (!model.getSecond()) return aspects; // look for model aspects alone

    Stream<QName> aspectStream = aspects.stream();
    Stream<QName> childrenStream = aspects
      .stream()
      .map(aspect -> this.dictionaryService.getSubAspects(aspect, false))
      .flatMap(Collection::stream);

    return Stream
      .concat(aspectStream, childrenStream)
      .collect(Collectors.toList());
  }

...
/**
 * This is an instance of a "classic shuffle" triggered by a create of a file matching a specified
 * pattern.
 *
 * <p>a) New file created. Typically with an obscure name. b) Existing file moved out of the way c)
 * New file moved into place. d) Old file deleted.
 *
 * <p>If this filter is active then this is what happens. a) New file created. New file created (X).
 * b) Existing file moved out of the way (Y to Z). Raname tracked. c) New file moved into place (X
 * to Y). Scenario kicks in to change commands. d) Old file deleted.
 */
    ...
    public Command evaluate(Operation operation) {
        ...
        if (createName != null) {
            if (operation instanceof DeleteFileOperation) {
                ...
                if (d.getName().equals(createName)) {
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "Anti-pattern : Shuffle file deleted createName:" + createName);
                    }
                    isComplete = true;
                    return null;
                }
        ...
        switch (internalState) {
                ...
                if (operation instanceof RenameFileOperation) {
                    ...
                    if (middle != null) {
                        ...
                        if (end != null) {
                             ...
                             * content only. Finally it moves the temp file into place for the
                             * subsequent delete. a) Rename Z to Y (Reverse previous move) b) Copy
                             * Content from X to Y c) Rename X to Z (move temp file out to old
                             * location)
                             */
                            if (logger.isDebugEnabled()) {
                                logger.debug(
                                        "Go and shuffle! createName:"
                                                + createName
                                                + " move1 "
                                                + move1
                                                + " move2 "
                            ...
                            CopyContentCommand copyContent =
                                    new CopyContentCommand(
                                            ...
                                            move1,
                                            r.getRootNodeRef(),
                                            oldFolder + "\\" + createName,
                                            oldFolder + "\\" + middle);
                            RenameFileCommand r2 =
                                    new RenameFileCommand(
                                            createName,
                                            end,
                                            r.getRootNodeRef(),
            ...
            case DELETE:
                ...
                /**
                 * Todo consider create shuffle with backup file which will never calls delete - do
                 * we need to pattern match on "Backup*". At the moment the delete state does
                 * nothing - hence we can simply set complete here for all situations.
                 */
                isComplete = true;

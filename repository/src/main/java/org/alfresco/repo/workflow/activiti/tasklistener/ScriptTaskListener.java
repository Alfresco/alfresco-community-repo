 ...
 * than the default by setting the field 'runAs'. By default, the user this script is executed with
 * is the task's assignee. If no assignee is set, the current logged-in user is used. If no user is
...
public class ScriptTaskListener extends ActivitiScriptBase implements TaskListener {
    ...
    public void notify(DelegateTask delegateTask) {
        if (script != null) {
            ...
            // the script is executed using no 'runAs' from a job-executor thread, the task's
            // assignee
            ...
            try {
                Object scriptOutput =
                        executeScript(scriptString, scriptModel, scriptProcessorName, runAsUser);
                ...
                if (clearAuthenticationContext) {
                    // If the current user has been set to the Task's assignee, we should clear it
                    // agian
            ...
            NodeRef userHomeNode =
                    (NodeRef)
                            registry.getNodeService()
                                    .getProperty(
                                            personNode.getNodeRef(), ContentModel.PROP_HOMEFOLDER);
            if (userHomeNode != null) {
                scriptModel.put(
                        USERHOME_BINDING_NAME, new ActivitiScriptNode(userHomeNode, registry));
            }
        }
    ...
    /**
     * Checks a valid Fully Authenticated User is set. If none is set then attempts to set the task
     * assignee as the Fully Authenticated User.
     *
     * @param delegateTask the delegate task
     * @return <code>true</code> if the Fully Authenticated User was changed, otherwise <code>false
     *     </code>.
     */

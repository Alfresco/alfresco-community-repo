...
public class BlogPostLibJs {
    ...
    /**
     * Checks whether a blog configuration is available This should at some point also check whether
     * the configuration is enabled.
     *
     * @param node the node that should be checked. Will check all parents if the node itself
     *     doesn't contain a configuration.
    ...
    public static boolean hasExternalBlogConfiguration(NodeRef node, ServiceRegistry services) {
        if (node == null) {
            return false;
        } else if (services.getNodeService()
                .hasAspect(node, BlogIntegrationModel.ASPECT_BLOG_DETAILS)) {
            return true;
        } else {
            return hasExternalBlogConfiguration(
                    services.getNodeService().getPrimaryParent(node).getParentRef(), services);
        }
    }
    ...
    public static Map<String, Object> getBlogPostData(NodeRef node, ServiceRegistry services) {
        ...
        data.put("node", node);
        String creator =
                (String) services.getNodeService().getProperty(node, ContentModel.PROP_CREATOR);
        // ALF-18527
        ...
        Serializable published =
                services.getNodeService().getProperty(node, ContentModel.PROP_PUBLISHED);
        ...
        Date updatedDate =
                (Date) services.getNodeService().getProperty(node, ContentModel.PROP_UPDATED);
        ...
        data.put(
                "createdDate",
                services.getNodeService().getProperty(node, ContentModel.PROP_CREATED));
        data.put(
                "modifiedDate",
                services.getNodeService().getProperty(node, ContentModel.PROP_MODIFIED));

        // does the external post require an update?
        Date lastUpdate =
                (Date)
                        services.getNodeService()
                                .getProperty(node, BlogIntegrationModel.PROP_LAST_UPDATE);

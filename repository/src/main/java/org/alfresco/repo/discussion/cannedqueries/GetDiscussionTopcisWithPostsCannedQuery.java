...
/**
 * This class provides support for {@link CannedQuery canned queries} which filter topics by their
 * posts
 *
 * @author Nick Burch
 * @since 4.0
...
public class GetDiscussionTopcisWithPostsCannedQuery
    ...
    protected List<NodeWithChildrenEntity> queryAndFilter(CannedQueryParameters parameters) {
        ...
        GetDiscussionTopcisWithPostsCannedQueryParams paramBean =
                (GetDiscussionTopcisWithPostsCannedQueryParams) paramBeanObj;
        ...
        List<NodeWithChildrenEntity> results =
                cannedQueryDAO.executeQuery(
                        QUERY_NAMESPACE, QUERY_SELECT_GET_NODES, paramBean, 0, Integer.MAX_VALUE);

        // Filter
        List<NodeWithChildrenEntity> filtered =
                new ArrayList<NodeWithChildrenEntity>(results.size());
        for (NodeWithChildrenEntity result : results) {
            // Filter by topic date
            if (filterByTopicCreatedDate) {
                Date createdDate =
                        DefaultTypeConverter.INSTANCE.convert(Date.class, result.getCreatedDate());
            ...
            if (filterByPostCreatedDate) {
                ...
                for (NameAndCreatedAt post : result.getChildren()) {
                    Date createdDate =
                            DefaultTypeConverter.INSTANCE.convert(Date.class, post.getCreatedAt());

...
public class PublicApiTenantWebScriptServletRuntime extends TenantWebScriptServletRuntime
    ...
    private static final Pattern CMIS_URI_PATTERN =
            Pattern.compile(".*/cmis/versions/[0-9]+\\.[0-9]+/.*");
    private ApiAssistant apiAssistant;

    public PublicApiTenantWebScriptServletRuntime(
    ...
    protected WebScriptRequest createRequest(Match match) {
        //    	try
        //    	{
        // make the request input stream a BufferedInputStream so that the first x bytes can be
        // reused.
        //	    	PublicApiHttpServletRequest wrapped = new PublicApiHttpServletRequest(req);

        // TODO: construct
        // org.springframework.extensions.webscripts.servlet.WebScriptServletResponse when
    ...
    @Override
    protected void renderErrorResponse(
            Match match,
            Throwable exception,
        ...
        if (CMIS_URI_PATTERN.matcher(req.getRequestURI()).matches()

...
public class LockableAspectInterceptorTest {
    ...
    public void setUp() {
        ...
        // The user that will create locks, this should be different from the user that queries them
        // (ALF-19465)
        lockOwner = "jbloggs";
        // The 'current' user.
    ...
    public void testHasAspectEphemeralLock() {
        QName nodeName =
                QName.createQName(
                        "http://www.alfresco.org/test/" + getClass().getSimpleName(), "testNode");
        NodeRef nodeRef =
                nodeService
                        .createNode(
                                rootNode,
        ...
        assertFalse(
                "Node should not be reported as lockable",
                nodeService.hasAspect(nodeRef, ContentModel.ASPECT_LOCKABLE));

        lockStore.set(
                nodeRef,
                LockState.createLock(
                        nodeRef, LockType.WRITE_LOCK, lockOwner, null, Lifetime.EPHEMERAL, null));

        assertTrue(
                "Node should be reported as lockable",
                nodeService.hasAspect(nodeRef, ContentModel.ASPECT_LOCKABLE));
    }
    ...
    public void testHasAspectPersistentLock() {
        ...
        lockStore.set(
                nodeRef,
                LockState.createLock(
                        nodeRef, LockType.WRITE_LOCK, lockOwner, null, Lifetime.PERSISTENT, null));
        ...
        nodeService.addAspect(
                nodeRef, ContentModel.ASPECT_LOCKABLE, new HashMap<QName, Serializable>(0));
    ...
    public void testGetAspectsEphemeralLock() {
        ...
        // We want to check that the spoofed cm:lockable aspect is joined with the other aspects for
        // the node.
        nodeService.addAspect(
                nodeRef, ContentModel.ASPECT_AUDITABLE, new HashMap<QName, Serializable>());
        ...
        assertTrue(
                "Node should have auditable aspect",
        ...
        assertTrue(
                "Node should have lockable aspect", aspects.contains(ContentModel.ASPECT_LOCKABLE));
        assertTrue(
                "Node should have auditable aspect",
                aspects.contains(ContentModel.ASPECT_AUDITABLE));
    }
    ...
    public void testGetPropertiesPersistentLock() {
        ...
        lockProps.put(
                ContentModel.PROP_LOCK_ADDITIONAL_INFO, "{ \"fieldName\": \"extra lock info\" }");
        ...
        assertEquals(
                LockType.READ_ONLY_LOCK.toString(), readProps.get(ContentModel.PROP_LOCK_TYPE));
        assertEquals(now, readProps.get(ContentModel.PROP_EXPIRY_DATE));
        // Spoofed - wasn't explicitly added.
        assertEquals(
                Lifetime.PERSISTENT.toString(), readProps.get(ContentModel.PROP_LOCK_LIFETIME));
        // Double check - not really present
        ensurePropertyNotPresent(nodeRef, ContentModel.PROP_LOCK_LIFETIME);
        assertEquals(
                "{ \"fieldName\": \"extra lock info\" }",
                readProps.get(ContentModel.PROP_LOCK_ADDITIONAL_INFO));
    }
    ...
    private void ensurePropertyNotPresent(final NodeRef nodeRef, final QName propQName) {
        RetryingTransactionCallback<Boolean> check =
    ...
    public void testGetPropertiesEphemeralLock() {
        ...
        assertFalse(
                "Node should not have lockOwner property",
                properties.containsKey(ContentModel.PROP_LOCK_OWNER));
        assertTrue(
                "Node should have created property",
                properties.containsKey(ContentModel.PROP_CREATED));

        // Set a lock on the node and reload the properties
        // Make sure the ephemeral lock won't expire before asserting cm:lockable properties
        // (spoofed).
        Date expire = makeExpiryDate(2);
        lockStore.set(
                nodeRef,
                LockState.createLock(
                        nodeRef, LockType.WRITE_LOCK, lockOwner, expire, Lifetime.EPHEMERAL, null));
        ...
        // Spoofed - wasn't explicitly added.
        assertEquals(
                Lifetime.EPHEMERAL.toString(), properties.get(ContentModel.PROP_LOCK_LIFETIME));
    ...
    public void testGetPropertyEphemeralLock() {
        ...
        assertEquals(
                ...
                nodeService.getProperty(nodeRef, ContentModel.PROP_LOCK_TYPE));
        assertEquals(expire, nodeService.getProperty(nodeRef, ContentModel.PROP_EXPIRY_DATE));
        assertEquals(
                Lifetime.EPHEMERAL.toString(),
                nodeService.getProperty(nodeRef, ContentModel.PROP_LOCK_LIFETIME));
    }
    ...
    public void testEnableDisableForThread() throws InterruptedException, ExecutionException {
        ...
        assertTrue(
                "Node should be reported as lockable (main thread)",
                nodeService.hasAspect(nodeRef, ContentModel.ASPECT_LOCKABLE));
    ...
    public void testSetPropertiesPersistentLock() {
        ...
        // With the exception of cm:lockLifetime, lock properties should be unaffected after
        // setProperties()
    ...
    public void testEphemeralLock() {
        ...
        QName nodeName =
                QName.createQName(
                        "http://www.alfresco.org/test/" + getClass().getSimpleName(), fName);
        final NodeRef nodeRef =
                nodeService
                        .createNode(
                                rootNode,
        ...
        // unlock node
        RetryingTransactionCallback<Void> unlockExecuteImplCallBack =
                new RetryingTransactionCallback<Void>() {
    ...
    private void writeData(String fileName, final NodeRef fileNodeRef) {
        ...
        InputStream is =
                getClass().getClassLoader().getResourceAsStream("farmers_markets_list_2003.doc");

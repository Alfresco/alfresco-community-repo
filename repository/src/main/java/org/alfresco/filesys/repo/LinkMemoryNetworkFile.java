...
public class LinkMemoryNetworkFile extends NodeRefNetworkFile {
  ...
  public LinkMemoryNetworkFile(
    ...
    m_data = data;
    if (m_data == null) m_data = new byte[0];
  ...
  public boolean isEndOfFile() throws java.io.IOException {
    ...
    if (m_filePos == m_data.length) return true;
    return false;
  }
  ...
  public int readFile(byte[] buf, int len, int pos, long fileOff)
    ...
    if (fileOff >= fileLen) return 0;

    // Calculate the actual read length

    if ((fileOff + len) > fileLen) len = (int) (fileLen - fileOff);
  ...
  public long seekFile(long pos, int typ) throws IOException {
    ...
    switch (typ) {
      ...
      case SeekType.StartOfFile:
        if (currentPosition() != pos) m_filePos = pos;
        break;
  ...
  public FileState getFileState() {
    ...
    if (super.getFileState() == null) setFileState(
      new LocalFileState(getFullName(), false)
    );

...
public class LDAPInitialDirContextFactoryImpl
  implements LDAPInitialDirContextFactory, InitializingBean {

  private static final Log logger = LogFactory.getLog(
    LDAPInitialDirContextFactoryImpl.class
  );

  private static Set<Map<String, String>> checkedEnvs = Collections.synchronizedSet(
    new HashSet<Map<String, String>>(11)
  );
  ...
  static {
    System.setProperty(
      "javax.security.auth.useSubjectCredentialsOnly",
      "false"
    );
  }
  ...
  public void setInitialDirContextEnvironment(
    Map<String, String> initialDirContextEnvironment
  ) {
    this.authenticatedEnvironment = initialDirContextEnvironment;
    this.authenticatedEnvironment.values()
  ...
  public void setDefaultIntialDirContextEnvironment(
    Map<String, String> defaultEnvironment
  ) {
    this.defaultEnvironment =
      new LinkedHashMap<String, String>(defaultEnvironment.size());
  ...
  public void setPoolSystemProperties(
    Map<String, String> poolSystemProperties
  ) {
  ...
  public InitialDirContext getDefaultIntialDirContext(int pageSize)
    throws AuthenticationException {
  ...
  private InitialDirContext buildInitialDirContext(
    ...
    if (isSSLSocketFactoryRequired()) {
      ...
      env.put(
        ...
        AlfrescoSSLSocketFactory.class.getName()
      );
    }
    ...
    try {
      ...
      if (pageSize > 0) {
        ...
        diagnostic.addStep(
          AuthenticationDiagnostic.STEP_KEY_LDAP_CONNECTED,
          true,
          args
        );
        return ret;
      ...
      diagnostic.addStep(
        AuthenticationDiagnostic.STEP_KEY_LDAP_AUTHENTICATION,
        false,
        args1
      );
      ...
      diagnostic.addStep(
        AuthenticationDiagnostic.STEP_KEY_LDAP_CONNECTING,
        false,
        args1
      );
      ...
      throw new AuthenticationException(
        "authentication.err.communication",
        diagnostic,
        args,
        cause
      );
      ...
      throw new AuthenticationException(
        "Unable to encode LDAP v3 request controls",
        e
      );
    }
  }
  ...
  public boolean hasNextPage(DirContext ctx, int pageSize) {
    if (pageSize > 0) {
      try {
        ...
        if (controls != null) {
          for (Control control : controls) {
            if (control instanceof PagedResultsResponseControl) {
              byte[] cookie =
                ((PagedResultsResponseControl) control).getCookie();
              if (cookie != null) {
        ...
        throw new AuthenticationException(
          "Unable to encode LDAP v3 request controls; check LDAP configuration",
          e
        );
      }
    }
  ...
  public InitialDirContext getInitialDirContext(
    ...
    if (principal == null) {
      ...
      diagnostic.addStep(
        AuthenticationDiagnostic.STEP_KEY_VALIDATION,
        false,
        null
      );
  ...
  public static void main(String[] args) {
    ...
    LDAPInitialDirContextFactory factory = (LDAPInitialDirContextFactory) applicationContext.getBean(
    ...
    try {
      ctx =
        factory.getInitialDirContext("cn=" + args[0] + "," + args[2], args[1]);
      ...
      for (int i = 0; i < userMembers; i++) {
        ...
        Attribute telephoneNumber = new BasicAttribute(
          "telephoneNumber",
          "123"
        );
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    try {
      ...
      logger.warn(
        "LDAP server supports anonymous bind " + env.get(Context.PROVIDER_URL)
      );
    } catch (javax.naming.AuthenticationException ax) {} catch (
      AuthenticationNotSupportedException e
    ) {} catch (NamingException nx) {
      logger.error(
        "Unable to connect to LDAP Server; check LDAP configuration",
        nx
    ...
    try {
      ...
      logger.info(
        "LDAP server does not fall back to anonymous bind for a string uid and password at " +
        env.get(Context.PROVIDER_URL)
      );
      ...
      logger.info(
        "LDAP server does not support simple string user ids and invalid credentials at " +
        env.get(Context.PROVIDER_URL)
      );
    }
    ...
    try {
      ...
      logger.info(
        "LDAP server does not fall back to anonymous bind for a simple dn and password at " +
        env.get(Context.PROVIDER_URL)
      );
    } catch (AuthenticationNotSupportedException e) {
      logger.info(
      ...
      logger.info(
        "LDAP server does not support simple DN and invalid password at " +
        env.get(Context.PROVIDER_URL)
      );
    }
    ...
    if (principal != null) {
      ...
      if (!checkedEnvs.contains(env)) {
        try {
          ...
          logger.info(
            "LDAP server does not support the required authentication mechanism"
          );
        } catch (NamingException nx) {
          // already done
        }
  ...
  private boolean isSSLSocketFactoryRequired() {
    ...
    if (protocol != null && protocol.equals("ssl")) {
      if (
        ...
        logger.warn(
          "The SSL configuration for LDAPS is not full, the default configuration will be used."
        );
      }
    }
    return result;
  ...
  private KeyStore initTrustStore() {
    ...
    try {
      ks.load(
        new FileInputStream(getTrustStorePath()),
        getTrustStorePassPhrase().toCharArray()
      );
    } catch (FileNotFoundException fnfe) {
      throw new AlfrescoRuntimeException(
        "The truststore file is not found.",
        fnfe
      );
    } catch (IOException ioe) {
      throw new AlfrescoRuntimeException(
        "The truststore file cannot be read.",
        ioe
      );
    } catch (NoSuchAlgorithmException nsae) {
      throw new AlfrescoRuntimeException(
        "Algorithm used to check the integrity of the truststore cannot be found.",
        nsae
      );
    } catch (CertificateException ce) {
      throw new AlfrescoRuntimeException(
        "The certificates cannot be loaded from truststore.",
        ce
      );
    }
    return ks;

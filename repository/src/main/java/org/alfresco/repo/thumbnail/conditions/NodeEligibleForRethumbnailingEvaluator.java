 ...
 *
 * <ul>
 *   <li>All content nodes are eligible for thumbnailing initially. Of course thumbnails can only
 *       ever be attempted for those content mime types that have at least one registered and active
 *       {@link ContentTransformer}.
 *   <li>If the first attempt to produce a thumbnail for a node fails, then it may be retried up to
 *       a maximum of {@link FailureHandlingOptions#getRetryCount() system.thumbnail.retryCount}
 *       times.
 *   <li>These initial retries to produce a thumbnail will occur not more often than every {@link
 *       FailureHandlingOptions#getRetryPeriod() system.thumbnail.retryPeriod} seconds and will use
 *       which ever content transformers the ContentService#getTransformer(String, String content
 *       service gives.
 *   <li>If a thumbnail is not successfully produced for a node after these attempts then it is
 *       considered to be a 'difficult' piece of content with respect to thumbnailing and the
 *       assumption is that a thumbnail may never be available for it. However, in order to allow
 *       for the possibility of software upgrades or similiar, which may make the content
 *       thumbnailable at a later date, further attempts will be made, but at a much reduced
 *       frequency.
 *   <li>Difficult pieces of content will not be attempted more often than every {@link
 *       FailureHandlingOptions#getQuietPeriod() system.thumbnail.quietPeriod} seconds.
 *   <li>The attempts to thumbnail difficult pieces of content can be disabled by setting {@link
 *       FailureHandlingOptions#getQuietPeriodRetriesEnabled()
 *       system.thumbnail.quietPeriodRetriesEnabled} to false.
 * </ul>
 *
 * At all times, thumbnails will be attempted when a user navigates to a page which needs to show
 * the relevant thumbnail (lazy production).
 *
...
public class NodeEligibleForRethumbnailingEvaluator extends ActionConditionEvaluatorAbstractBase {
    ...
    public boolean evaluateImpl(ActionCondition actionCondition, NodeRef actionedUponNodeRef) {
        ...
        Serializable paramThumbnailDefnName =
                actionCondition.getParameterValue(PARAM_THUMBNAIL_DEFINITION_NAME);
        ...
        final Serializable parameterValue =
                actionCondition.getParameterValue(PARAM_QUIET_PERIOD_RETRIES_ENABLED);
        Serializable paramQuietPeriodRetriesEnabled =
                parameterValue != null
                        ? parameterValue
                        : FailureHandlingOptions.DEFAULT_QUIET_PERIOD_RETRIES_ENABLED;

        final QName thumbnailDefnQName =
                QName.createQName(
                        NamespaceService.CONTENT_MODEL_1_0_URI, (String) paramThumbnailDefnName);
        ...
        Map<String, FailedThumbnailInfo> failures =
                thumbnailService.getFailedThumbnails(actionedUponNodeRef);
        ...
        final FailedThumbnailInfo failedThumbnailInfo =
                failures.get(thumbnailDefnQName.getLocalName());

        // There is a cm:failedThumbnail child, so there has been at least one failed execution of
        // the given
    ...
    private boolean timeSinceFailureExceedsLimit(
            Serializable failurePeriod, long timeSinceFailure) {

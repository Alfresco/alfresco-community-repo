...
public class SubscriptionServiceActivitiesTest {
    ...
    // We'll suffix each user id with a unique number to allow for repeated re-runs of this test
    // class.
    ...
    public static RuleChain STATIC_RULE_CHAIN =
            RuleChain.outerRule(APP_CONTEXT_INIT).around(TEST_USER1).around(TEST_USER2);
    ...
    private SiteInfo publicSite, privateSite1, privateSite2, modSite1, modSite2;
    ...
    public static void setUp() throws Exception {
        ...
        // feed generator jobs
        // Note that to ensure this test class can be run within a suite, that we do not want to
        // actually 'shutdown' the scheduler.
        // It may be needed by other test classes and must be restored to life after this class has
        // run.
        ...
        ChildApplicationContextFactory activitiesFeed =
                (ChildApplicationContextFactory) ctx.getBean("ActivitiesFeed");
        ...
        LocalFeedTaskProcessor feedProcessor =
    ...
    public void createTestSites() throws Exception {
        ...
        // admin creates the test sites. This is how this test case was before refactoring. TODO
        // Probably better to have a non-admin user create the sites.
        publicSite =
                testSites.createSite(
                        "sitePreset", "pub" + guid, "", "", SiteVisibility.PUBLIC, ADMIN);
        privateSite1 =
                testSites.createSite(
                        "sitePreset", "priv1" + guid, "", "", SiteVisibility.PRIVATE, ADMIN);
        privateSite2 =
                testSites.createSite(
                        "sitePreset", "priv2" + guid, "", "", SiteVisibility.PRIVATE, ADMIN);
        modSite1 =
                testSites.createSite(
                        "sitePreset", "mod1" + guid, "", "", SiteVisibility.MODERATED, ADMIN);
        modSite2 =
                testSites.createSite(
                        "sitePreset", "mod2" + guid, "", "", SiteVisibility.MODERATED, ADMIN);
    ...
    public void testFollowingActivity() throws Exception {
        // We'll change things in the system in order to cause the generation of activity events and
        // compare the feeds with these totals as we go.
        // Initially, both users have zero activity feed entries.
        // Java's requirement for final modifiers on variables accessed within inner classes means
        // we can't use simple ints here.
        ...
        doWorkAs(
                ...
                new RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        for (SiteInfo s :
                                new SiteInfo[] {
                                    publicSite, privateSite1, privateSite2, modSite1, modSite2
                                }) {
                            siteService.setMembership(
                                    s.getShortName(), USER_ONE_NAME, SiteModel.SITE_MANAGER);
                        }
                        return null;
                    }
                });
        ...
        doWorkAs(
                ...
                new RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        log.debug(
                                "Now to check if the activity tables have the correct number of"
                                        + " entries for our test users");

                        List<String> feed =
                                activityService.getUserFeedEntries(
                                        USER_ONE_NAME, null, false, false, null, null);
                        assertEquals(USER_ONE_NAME + " had wrong feed size.", 0, feed.size());

                        feed =
                                activityService.getUserFeedEntries(
                                        USER_TWO_NAME, null, false, false, null, null);
        ...
        doWorkAs(
                ...
                new RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        // note: userId2 should not see activities from followers in moderated sites
                        // that they do not belong do (ALF-16460)
                        feed =
                                activityService.getUserFeedEntries(
                                        USER_TWO_NAME, null, false, false, null, null);
                        log.debug(USER_TWO_NAME + "'s feed: " + prettyJson(feed));
                        ...
                        siteService.setMembership(
                                privateSite2.getShortName(),
                                USER_TWO_NAME,
                                SiteModel.SITE_CONSUMER);
                        siteService.setMembership(
                                modSite2.getShortName(), USER_TWO_NAME, SiteModel.SITE_MANAGER);
        ...
        doWorkAs(
                ...
                new RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        // note: userId2 should not see activities from followers in moderated sites
                        // that they do not belong to (ALF-16460)
                        feed =
                                activityService.getUserFeedEntries(
                                        USER_TWO_NAME, null, false, false, null, null);
        ...
        doWorkAs(
                ...
                new RetryingTransactionCallback<Void>() {
                    @Override
                    public Void execute() throws Throwable {
                        // Feeds should now be reduced to 'follow' events only - see FeedCleaner's
                        // behaviours/policies/transaction listeners.
                        List<String> feed =
                                activityService.getUserFeedEntries(
                                        USER_ONE_NAME, null, false, false, null, null);
    ...
    // Just adding a little helper method to make the above code more readable. Oh Java 8, how we
    // need you! (Or Groovy, or Scala...)
    private <T> T doWorkAs(final String userName, final RetryingTransactionCallback<T> work) {
        return AuthenticationUtil.runAs(
                new AuthenticationUtil.RunAsWork<T>() {
                    @Override

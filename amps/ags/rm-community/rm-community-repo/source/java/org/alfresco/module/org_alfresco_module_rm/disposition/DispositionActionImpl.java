...
public class DispositionActionImpl implements DispositionAction, RecordsManagementModel {
     ...
     */
    public DispositionActionImpl(
            RecordsManagementServiceRegistry services, NodeRef dispositionActionNodeRef) {
    ...
    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.disposition.DispositionAction#getDispositionActionDefinition()
     */
            ...
            String id =
                    (String)
                            services.getNodeService()
                                    .getProperty(
                                            this.dispositionNodeRef, PROP_DISPOSITION_ACTION_ID);

            // Get the disposition instructions for the owning node
            NodeRef recordNodeRef =
                    services.getNodeService()
                            .getPrimaryParent(this.dispositionNodeRef)
                            .getParentRef();
            if (recordNodeRef != null) {
                DispositionSchedule ds =
                        services.getDispositionService().getDispositionSchedule(recordNodeRef);
    ...
    public String getLabel() {
        ...
        RecordsManagementAction action =
                services.getRecordsManagementActionService().getDispositionAction(name);
        if (action != null) {
    ...
    public String getId() {
        return (String)
                services.getNodeService()
                        .getProperty(this.dispositionNodeRef, PROP_DISPOSITION_ACTION_ID);
    }
    ...
    public String getName() {
        return (String)
                services.getNodeService()
                        .getProperty(this.dispositionNodeRef, PROP_DISPOSITION_ACTION);
    }
    ...
    public Date getAsOfDate() {
        return (Date)
                services.getNodeService()
                        .getProperty(this.dispositionNodeRef, PROP_DISPOSITION_AS_OF);
    }

    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.disposition.DispositionAction#isEventsEligible()
     */
    public boolean isEventsEligible() {
        return ((Boolean)
                        services.getNodeService()
                                .getProperty(
                                        this.dispositionNodeRef, PROP_DISPOSITION_EVENTS_ELIGIBLE))
                .booleanValue();
    }

    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.disposition.DispositionAction#getCompletedAt()
     */
    public Date getCompletedAt() {
        return (Date)
                services.getNodeService()
                        .getProperty(this.dispositionNodeRef, PROP_DISPOSITION_ACTION_COMPLETED_AT);
    }

    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.disposition.DispositionAction#getCompletedBy()
     */
    public String getCompletedBy() {
        return (String)
                services.getNodeService()
                        .getProperty(this.dispositionNodeRef, PROP_DISPOSITION_ACTION_COMPLETED_BY);
    }
    ...
    public Date getStartedAt() {
        return (Date)
                services.getNodeService()
                        .getProperty(this.dispositionNodeRef, PROP_DISPOSITION_ACTION_STARTED_AT);
    }
    ...
    public String getStartedBy() {
        return (String)
                services.getNodeService()
                        .getProperty(this.dispositionNodeRef, PROP_DISPOSITION_ACTION_STARTED_BY);
    }

    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.disposition.DispositionAction#getEventCompletionDetails()
     */
     ...
     *
     * <p>Returns null if event can not be found.
     *
     * @param eventName name of the event
     * @return {@link EventCompletionDetails} event completion details for named event, null
     *     otherwise
     * @since 2.2
     */
    @Override
        ...
        List<ChildAssociationRef> assocs =
                services.getNodeService()
                        .getChildAssocsByPropertyValue(
                                dispositionNodeRef, PROP_EVENT_EXECUTION_NAME, eventName);

        if (!assocs.isEmpty()) {
            if (assocs.size() != 1) {
                throw new AlfrescoRuntimeException(
                        "Unable to get event completion details, because more than one child was"
                                + " found for event "
                                + eventName);
            }
    ...
    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.disposition.DispositionAction#completeEvent(java.lang.String,
     *     java.util.Date, java.lang.String)
     */
    @Override
    public void completeEvent(
            final String eventName, final Date completedAt, final String completedBy) {
                    ...
                    new RunAsWork<Void>() {
                        ...
                        public Void doWork() {
                            ...
                            Map<QName, Serializable> props =
                                    services.getNodeService().getProperties(eventNodeRef);
                            ...
                            if (eventName.equals("declassification_review")) {
                                setDeclassificationReview(
                                        eventNodeRef, completedAtValue, completedByValue);
                            }
    ...
    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.disposition.DispositionAction#undoEvent(java.lang.String)
     */
    @Override
    ...
    public void refreshEvents() {
        AuthenticationUtil.runAsSystem(
                new RunAsWork<Void>() {
                    @Override
                    public Void doWork() {
                        // go through the current events on the next action and remove any that are
                        ...
                        for (EventCompletionDetails event : eventsList) {
                            ...
                            if (stepEvents != null && !stepEvents.contains(event.getEventName())) {
                                ...
                                services.getNodeService()
                                        .removeChild(getNodeRef(), event.getNodeRef());
                        ...
                        // go through the disposition action definition step events and add any new
                        // ones
                        if (stepEvents != null) {
                        ...
                        // finally since events may have changed re-calculate the events eligible
                        // flag
                        boolean eligible = updateEventEligible();

                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "Set events eligible flag to '"
                                            + eligible
                                            + "' for next action '"
                                            + getName()
                                            + "' ("
    ...
    /**
     * @see
     *     org.alfresco.module.org_alfresco_module_rm.disposition.DispositionAction#addEventCompletionDetails(org.alfresco.module.org_alfresco_module_rm.event.RecordsManagementEvent)
     */
    @Override
    public void addEventCompletionDetails(RecordsManagementEvent event) {
        ...
        // TODO display label
        eventProps.put(
                PROP_EVENT_EXECUTION_AUTOMATIC,
                event.getRecordsManagementEventType().isAutomaticEvent());
    ...
    /**
     * Calculates and updates the <code>rma:dispositionEventsEligible</code> property for the given
    ...
    private boolean updateEventEligible() {
        ...
        services.getNodeService()
                .setProperty(getNodeRef(), PROP_DISPOSITION_EVENTS_ELIGIBLE, eligible);
     ...
     */
    private void setDeclassificationReview(
            NodeRef eventNodeRef, Date completedAtValue, String completedByValue) {
        NodeRef nextDispositionActionNodeRef =
                services.getNodeService().getPrimaryParent(eventNodeRef).getParentRef();
        NodeRef nodeRef =
                services.getNodeService()
                        .getPrimaryParent(nextDispositionActionNodeRef)
                        .getParentRef();
        setPropsOnContent(nodeRef, completedAtValue, completedByValue);

        // check if the node is a record folder then set the declassification review on the records
        // also
        if (services.getNodeService()
                .getType(nodeRef)
                .equals(RecordsManagementModel.TYPE_RECORD_FOLDER)) {
            // get all the records inside the record folder
            List<ChildAssociationRef> records =
                    services.getNodeService()
                            .getChildAssocs(
                                    nodeRef,
    ...
    private void setPropsOnContent(
            NodeRef nodeRef, Date completedAtValue, String completedByValue) {

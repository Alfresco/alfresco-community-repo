...
public class NodeServiceXPath extends BaseXPath {
  ...
  public NodeServiceXPath(
      ...
      for (int i = 0; i < paramDefs.length; i++) {
        if (!paramDefs[i].hasDefaultValue()) {
          throw new AlfrescoRuntimeException(
            "Parameter must have default value"
          );
        }
        Object value = null;
        if (
          paramDefs[i].getDataTypeDefinition()
            .getName()
    ...
    for (String prefix : documentNavigator
      .getNamespacePrefixResolver()
      .getPrefixes()) {
      addNamespace(
        prefix,
        documentNavigator.getNamespacePrefixResolver().getNamespaceURI(prefix)
      );
    }
  }
    ...
    public Object evaluate(List nodes, Object qnameObj, Navigator nav) {
      ...
      if (qnameStr.startsWith("{")) {
        ...
        typeQName =
          QName.createQName(
            qnameStr,
            ((DocumentNavigator) nav).getNamespacePrefixResolver()
          );
      }
      // resolve the noderef
    ...
    public Object evaluate(
      ...
      if ((attributeValue != null) && (attributeValue.length() > 0)) {
        ...
        if (patternValue.equals("*")) {
          ...
          QNamePattern qNamePattern = new JCRPatternMatch(
            patternValue,
            dNav.getNamespacePrefixResolver()
          );
  ...
  static class Contains implements Function {
    ...
    public Object call(Context context, List args)
      ...
      if (args.size() != 1) {
        throw new FunctionCallException(
          "contains() usage: contains('pattern')"
        );
      }
      return evaluate(
        context.getNodeSet(),

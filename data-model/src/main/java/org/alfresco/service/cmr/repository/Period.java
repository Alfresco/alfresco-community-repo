...
public class Period implements Serializable {
  ...
  private static transient ConcurrentHashMap<String, PeriodProvider> providers = new ConcurrentHashMap<String, PeriodProvider>();
  ...
  public static PeriodProvider getProvider(String periodType) {
    ...
    if (provider == null) {
      throw new IllegalStateException(
        "No period provider for period type " + periodType
      );
    }
    return provider;
  }
  ...
  public Period(Map<String, String> source) {
    ...
    if (source.get("periodType") == null) {
      throw new IllegalArgumentException(
        "Cannot create Period with null periodType"
      );
    }
  ...
  public Date getNextDate(Date date) {
    ...
    return provider.getNextDate(
      date,
      expression != null ? expression : provider.getDefaultExpression()
    );
  }
  ...
  public int hashCode() {
    ...
    result =
      prime * result + ((periodType == null) ? 0 : periodType.hashCode());
    return result;
  }

...
public class RMEntryVoter
  ...
  public int vote(
    ...
    try {
      ...
      List<ConfigAttributeDefinition> supportedDefinitions = extractSupportedDefinitions(
        config
      );
      ...
      for (ConfigAttributeDefinition cad : supportedDefinitions) {
        ...
        if (cad.getTypeString().equals(ConfigAttributeDefinition.RM_DENY)) {
          ...
          cad.getTypeString().equals(ConfigAttributeDefinition.RM_ABSTAIN)
        ) {
          return AccessDecisionVoter.ACCESS_ABSTAIN;
        } else if (
          cad.getTypeString().equals(ConfigAttributeDefinition.RM_ALLOW)
        ) {
          return AccessDecisionVoter.ACCESS_GRANTED;
        }
        ...
        // It is distinguished from RM_ALLOW so query may have additional behaviour in the future
          ...
          cad.getTypeString().equals(ConfigAttributeDefinition.RM_QUERY)
        ) {
          return AccessDecisionVoter.ACCESS_GRANTED;
        }
        ...
        // These entries effectively abstain
          ...
          (
            (cad.getParameters().get(0) != null) &&
            (cad.getParameters().get(0) >= invocation.getArguments().length)
          ) ||
          (
            (cad.getParameters().get(1) != null) &&
            (cad.getParameters().get(1) >= invocation.getArguments().length)
          )
        ) {
          continue;
        } else if (
          cad.getTypeString().equals(ConfigAttributeDefinition.RM_CAP)
        ) {

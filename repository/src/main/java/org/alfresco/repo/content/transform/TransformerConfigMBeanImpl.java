...
public class TransformerConfigMBeanImpl implements TransformerConfigMBean {
    ...
    public String getTransformationsByExtension(String sourceExtension, String targetExtension) {
        try {
            return transformerDebug.transformationsByExtension(
                    nullDefaultLowerParam(sourceExtension),
                    nullDefaultLowerParam(targetExtension),
                    true);
     ...
     * and forces other values to lower case.
     */
    ...
    public String help() {
        return "getTransformationDebugLog(n)\n"
                ...
                + "   - n the number of entries to include. If blank all available entries are"
                + " listed\n"
                + "\n"
                ...
                + "   Transforms a small test file from one mimetype to another and then shows"
                + " the \n"
                + "   debug of the transform, which would indicate if it was successful or even"
                + " if \n"
                + "   it was possible.\n"
                ...
                + "     \"index\", \"webpreview\", \"syncRule\", \"asyncRule\"...) or blank"
                + " for\n"
                + "     the default.\n"
                + "\n"
                + "getTransformationsByExtension(sourceExtension, targetExtension, use)\n"
                + "   Lists all possible transformations sorted by source and then target"
                + " mimetype\n"
                + "   extension.\n"
                + "   - sourceExtension to be checked. If blank all source mimetypes are"
                + " included\n"
                + "   - targetExtension to be checked. If blank all target mimetypes are"
                + " included.\n"

...
/**
 * A <code>UserTransaction</code> that will allow the thread using it to participate in transactions
 * that are normally only begun and committed by the <b>SpringFramework</b> transaction aware
 * components.
 *
 * <p>Client code can use this class directly, but should be very careful to handle the exception
 * conditions with the appropriate <code>finally</code> blocks and <code>rollback</code> code. It is
 * recommended that clients use this class indirectly via an instance of the {@link
 * org.alfresco.repo.transaction.RetryingTransactionHelper}.
 *
 * <p>This class is thread-safe in that it will detect multithreaded access and throw exceptions.
 * Therefore </b>do not use on multiple threads</b>. Instances should be used only for the duration
 * of the required user transaction and then discarded. Any attempt to reuse an instance will result
 * in failure.
 *
 * <p>Nested user transaction are allowed.
 *
 * <p><b>Logging:</b><br>
 * To dump exceptions during commits, turn debugging on for this class.<br>
 * To log leaked transactions i.e. a begin() is not matched by a commit() or rollback(), add
 * <i>.trace</i> to the usual classname-based debug category and set to WARN log level. This will
 * log the first detection of a leaked transaction and automatically enable transaction call stack
 * logging for subsequent leaked transactions. To enforce call stack logging from the start set the
 * <i>.trace</i> log level to DEBUG. Call stack logging will hamper performance but is useful when
 * it appears that something is eating connections or holding onto resources - usually a sign that
 * client code hasn't handled all possible exception conditions.
 *
 * @see org.springframework.transaction.PlatformTransactionManager
 * @see org.springframework.transaction.support.DefaultTransactionDefinition
 * @author Derek Hulley
 */
public class SpringAwareUserTransaction extends TransactionAspectSupport
     ...
     */
    private static final Log traceLogger =
            LogFactory.getLog(SpringAwareUserTransaction.class.getName() + ".trace");
    ...
    /**
     * Creates a user transaction that defaults to {@link
     * TransactionDefinition#PROPAGATION_REQUIRED}.
     *
     * @param transactionManager the transaction manager to use
     * @param readOnly true to force a read-only transaction
     * @param isolationLevel one of the {@link TransactionDefinition#ISOLATION_DEFAULT
     *     TransactionDefinition.ISOLATION_XXX} constants
     * @param propagationBehaviour one of the {@link TransactionDefinition#PROPAGATION_MANDATORY
     *     TransactionDefinition.PROPAGATION__XXX} constants
    ...
    /**
     * Return a qualifier value associated with this transaction attribute. This is not used as the
     * transaction manager has been selected for us.
     *
     * @return null always
     */
     ...
     *
     * <p>This may be used for applying specific transactional behavior or follow a purely
     * descriptive nature.
     */
    ...
    /**
     * The {@link UserTransaction } must rollback regardless of the error. The {@link #rollback()
     * rollback} behaviour is implemented by simulating a caught exception. As this method will
     * always return <code>true</code>, the rollback behaviour will be to rollback the transaction
     ...
     *
     * <p>A check is done to ensure that the transaction info on the stack is exactly the same
     * instance used when this transaction was started. The internal status is also checked against
     * the transaction info. These checks ensure that the transaction demarcation is done correctly
     * and that thread safety is adhered to.
     *
    ...
    private TransactionInfo getTransactionInfo() {
        ...
        try {
            ...
            // No transaction.  It is possible that the transaction threw an exception during
            // commit.
        }
        // perform checks for active transactions
        if (internalStatus == Status.STATUS_ACTIVE) {
            if (Thread.currentThread().getId() != threadId) {
                // the internally stored transaction info (retrieved in begin()) should match the
                // info
                // on the thread
                throw new RuntimeException(
        ...
        if (!finalized) {
            try {
                ...
                RollbackException re =
                        new RollbackException("Transaction didn't commit: " + e.getMessage());
        ...
        if (!isBeginMatched) {
            if (isCallStackTraced) {
                if (beginCallStack == null) {
                    traceLogger.error(
                            "UserTransaction being garbage collected without a commit() or"
                                + " rollback(). NOTE: Prior to transaction call stack logging.");
                } else {
                ...
                traceLogger.error(
                        "Detected first UserTransaction which is being garbage collected without a"
                                + " commit() or rollback()");
                traceLogger.error(
                        "Logging of transaction call stack is now enabled and will affect"
                                + " performance");
                isCallStackTraced = true;
            }
        }

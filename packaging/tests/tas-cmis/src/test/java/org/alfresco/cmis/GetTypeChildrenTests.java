...
public class GetTypeChildrenTests extends CmisTest {
    ...
    /**
     * Get type children for a valid type id and includePropertyDefinitions set to false (verify
     * that Map<String, PropertyDefinition<?>> is empty)
     */
    @TestRail(
            section = {"cmis-api"},
            executionType = ExecutionType.SANITY,
            description =
                    "Verify admin can get type children for BaseTypeId.CMIS_DOCUMENT and"
    ...
    public void getTypeChildrenWithoutPropertyDefinitions() {
        cmisApi.authenticateUser(dataUser.getAdminUser())
                ...
                .hasChildren("D:srft:facetField")
                .propertyDefinitionIsEmpty();
    }

    /**
     * Get type children for a valid type id and includePropertyDefinitions set to true (verify that
     * Map<String, PropertyDefinition<?>> is not empty)
     */
    @TestRail(
            section = {"cmis-api"},
            executionType = ExecutionType.SANITY,
            description =
                    "Verify admin can get type children for valid type id and"
    ...
    public void getTypeChildrenWithPropertyDefinitions() {
        cmisApi.authenticateUser(dataUser.getAdminUser())
                ...
                .hasChildren("F:pub:DeliveryChannel")
                .propertyDefinitionIsNotEmpty();
        cmisApi.usingObjectType(BaseTypeId.CMIS_FOLDER.value())
                .withPropertyDefinitions()
    ...
    public void getValidTypeChildrenForInvalidTypeId() {
        cmisApi.authenticateUser(dataUser.getAdminUser())
                .usingObjectType("cmis:invalidfolder")
                .withPropertyDefinitions()
                .hasChildren("F:pub:DeliveryChannel")
                .propertyDefinitionIsNotEmpty();
        ;
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify deleted user is not authorized to get type children for valid type id")
    @Test(
            groups = {TestGroup.CMIS, TestGroup.REGRESSION},
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify disabled user is not authorized to get type children for valid type id")
    @Test(
            groups = {TestGroup.CMIS, TestGroup.REGRESSION},
            expectedExceptions = CmisUnauthorizedException.class)

...
public class EventFilterUnitTest {
  ...
  public static void setUp() {
    ...
    when(dictionaryService.getSubTypes(any(), anyBoolean()))
      .thenAnswer(
        (Answer<Collection<QName>>) invocation -> {
    ...
    namespaceService.registerNamespace(
      NamespaceService.FORUMS_MODEL_PREFIX,
    ...
    caseInsensitive_userFilter =
      new EventUserFilter("System, john.doe, null", false);
    caseSensitive_userFilter =
      new EventUserFilter("System, john.doe, null", true);
  }

  @Test
  public void nodePropertyFilter() {
    assertTrue(
      "System properties are excluded by default.",
  ...
  public void nodeTypeFilter() {
    assertTrue(
      "Thumbnail node type should have been filtered.",
      typeFilter.isExcluded(ContentModel.TYPE_THUMBNAIL)
    );

    assertTrue(
      "System folder node types are excluded by default.",
      typeFilter.isExcluded(ContentModel.TYPE_SYSTEM_FOLDER)
    );

    assertTrue(
      "System node type should have been filtered (sys:*).",
      typeFilter.isExcluded(
        QName.createQName("sys:testSomeSystemType", namespaceService)
      )
    );

    assertTrue(
      "Forum node type should have been filtered (fm:*).",
  ...
  public void nodeAspectFilter() {
    assertTrue(
      "Working copy aspect should have been filtered.",
  ...
  public void childAssociationTypeFilter() {
    assertTrue(
      "Rendition child association type should have been filtered.",
      childAssociationTypeFilter.isExcluded(RenditionModel.ASSOC_RENDITION)
    );

    assertFalse(
      childAssociationTypeFilter.isExcluded(ContentModel.ASSOC_CONTAINS)
    );
  }
  ...
  public void userFilter_case_insensitive() {
    assertTrue(
      "System user should have been filtered.",
      caseInsensitive_userFilter.isExcluded("System")
    );

    assertTrue(
      "System user should have been filtered (case-insensitive).",
      caseInsensitive_userFilter.isExcluded("SYSTEM")
    );

    assertTrue(
      "'null' user should have been filtered.",
      caseInsensitive_userFilter.isExcluded("null")
    );

    assertTrue(
      "john.doe user should have been filtered.",
      caseInsensitive_userFilter.isExcluded("John.Doe")
    );

    assertFalse(
      "'jane.doe' user should not have been filtered.",
  ...
  public void userFilter_case_sensitive() {
    assertFalse(
      "'system' user should not have been filtered.",
    ...
    assertFalse(
      "'John.Doe' user should not have been filtered.",

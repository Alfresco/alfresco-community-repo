...
public class ScriptTaggingService extends BaseScopableProcessorExtension {
  ...
  public String[] getTags(String store) {
    ...
    List<String> result =
      this.serviceRegistry.getTaggingService().getTags(storeRef);
   ...
   */
  public PagedTagsWrapper getPagedTags(
    String store,
    int fromTag,
    int pageSize
  ) {
    StoreRef storeRef = new StoreRef(store);
    Pair<List<String>, Integer> page =
      this.serviceRegistry.getTaggingService()
        .getPagedTags(storeRef, fromTag, pageSize);
    List<String> result = page.getFirst();
    return new PagedTagsWrapper(
  ...
  public String[] getTags(String store, String filter) {
    StoreRef storeRef = new StoreRef(store);
    List<String> result =
      this.serviceRegistry.getTaggingService().getTags(storeRef, filter);
    return (String[]) result.toArray(new String[result.size()]);
  }
  ...
  public PagedTagsWrapper getPagedTags(
    ...
    Pair<List<String>, Integer> page =
      this.serviceRegistry.getTaggingService()
        .getPagedTags(storeRef, filter, fromTag, pageSize);
    List<String> result = page.getFirst();
    return new PagedTagsWrapper(
      (String[]) result.toArray(new String[result.size()]),
      page.getSecond()
  ...
  public ScriptNode getTag(String store, String tag) {
    StoreRef storeRef = new StoreRef(store);
    NodeRef result =
      this.serviceRegistry.getTaggingService().getTagNodeRef(storeRef, tag);
    if (result != null) {
  ...
  public ScriptNode createTag(String store, String tag) {
    StoreRef storeRef = new StoreRef(store);
    NodeRef result =
      this.serviceRegistry.getTaggingService().createTag(storeRef, tag);
    if (result != null) {

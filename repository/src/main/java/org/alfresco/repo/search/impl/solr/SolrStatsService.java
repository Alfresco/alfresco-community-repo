...
public class SolrStatsService implements StatsService {
  ...
  public StatsResultSet query(StatsParameters searchParameters) {
    ...
    LuceneQueryLanguageSPI language = searcher
      .getQueryLanguages()
      .get(searchParameters.getLanguage().toLowerCase());
    if (
      language != null &&
      SearchService.LANGUAGE_INDEX_FTS_ALFRESCO.equals(language.getName())
    ) {
      ...
      throw new SearcherException(
        "Unknown stats query language: " + searchParameters.getLanguage()
      );
    }
  }
}

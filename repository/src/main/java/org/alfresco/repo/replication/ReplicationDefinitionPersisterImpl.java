...
public class ReplicationDefinitionPersisterImpl
  ...
  private static final StoreRef SPACES_STORE = new StoreRef(
    StoreRef.PROTOCOL_WORKSPACE,
    "SpacesStore"
  );
  protected static final NodeRef REPLICATION_ACTION_ROOT_NODE_REF = new NodeRef(
    SPACES_STORE,
   ...
   */
  public void setRuntimeActionService(
  ...
  public List<ReplicationDefinition> loadReplicationDefinitions() {
    ...
    List<ReplicationDefinition> replicationActions = new ArrayList<ReplicationDefinition>(
      childAssocs.size()
    );
    for (ChildAssociationRef actionAssoc : childAssocs) {
      Action nextAction = runtimeActionService.createAction(
        actionAssoc.getChildRef()
      );
  ...
  public List<ReplicationDefinition> loadReplicationDefinitions(
    ...
    List<ReplicationDefinition> allReplicationDefinitions =
      this.loadReplicationDefinitions();
  ...
  public ReplicationDefinition loadReplicationDefinition(
    String replicationDefinitionName
  ) {
    return loadReplicationDefinition(
      buildReplicationQName(replicationDefinitionName)
    );
  }

  public ReplicationDefinition loadReplicationDefinition(
    QName replicationDefinitionName
  ) {
  ...
  public void renameReplicationDefinition(
    String oldReplicationName,
  ...
  public void renameReplicationDefinition(
    QName oldReplicationName,
    QName newReplicationName
  ) {
    ...
    if (findActionNode(newReplicationName) != null) {
      throw new ReplicationServiceException(
        "Can't rename to '" +
        newReplicationName +
  ...
  private NodeRef findActionNode(QName replicationDefinitionName) {
    ...
    if (childAssocs.isEmpty()) {
      ...
      if (childAssocs.size() > 1) {
        throw new ReplicationServiceException(
          "Multiple replication definitions with the name: " +
          replicationDefinitionName +
          " exist!"
        );
      }
  ...
  private NodeRef findOrCreateActionNode(
    ReplicationDefinition replicationAction
  ) {
    QName actionName = replicationAction.getReplicationQName();
    NodeRef actionNode = findActionNode(actionName);
  ...
  private void checkReplicationActionRootNodeExists() {
    if (nodeService.exists(REPLICATION_ACTION_ROOT_NODE_REF) == false) {
      throw new ReplicationServiceException(
        "Unable to find replication action root node."
      );
    }
  }

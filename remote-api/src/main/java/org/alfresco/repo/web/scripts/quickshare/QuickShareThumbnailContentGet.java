 ...
 *
 * <p>GET web script to stream "shared" thumbnail content (ie. enabled for public/unauthenticated
    ...
    protected void executeImpl(
        ...
        if (thumbnailName == null) {
            ...
            throw new WebScriptException(
                    HttpServletResponse.SC_NOT_FOUND, "Unable to find " + nodeRef);
        }

        // Indicate whether or not the thumbnail can be cached by the browser. Caching is allowed if
        // the lastModified
        // argument is provided as this is an indication of request uniqueness and therefore the
        // browser will have
        ...
        NodeRef thumbnailNodeRef =
                thumbnailService.getThumbnailByName(
                        nodeRef, ContentModel.PROP_CONTENT, thumbnailName);
        ...
        if (thumbnailNodeRef == null) {
            ...
            Scriptable scope =
                    new BaseScopableProcessorExtension()
                            .getScope(); // note: required for ValueConverter (collection)
            ...
            if (thumbnailNodeRef == null) {
                if (ph == true) {
                    // Try and get the place holder resource. We use a method in the thumbnail
                    // service
                    ...
                    if (contentData != null) {
                        phPath =
                                scriptThumbnailService.getMimeAwarePlaceHolderResourcePath(
                                        thumbnailName, contentData.getMimetype());
                    }
                    ...
                    if (phPath == null) {
                        ...
                        throw new WebScriptException(
                                ...
                                "Thumbnail was not found and no place holder resource set for '"
                                        + thumbnailName
                                        + "'");
                    } else {
                        // Set the resouce path in the model ready for the content stream to send
                        // back to the client
                    ...
                    // 404 since no thumbnail was found
                    throw new WebScriptException(
                            HttpServletResponse.SC_NOT_FOUND, "Thumbnail was not found");
                }
            }
        }
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "QuickShare - retrieved thumbnail content: "
                            + thumbnailNodeRef
                            + " ["
                            + nodeRef
                            + ","

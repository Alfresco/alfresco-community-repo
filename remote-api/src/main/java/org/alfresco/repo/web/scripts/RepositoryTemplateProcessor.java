...
public class RepositoryTemplateProcessor extends FreeMarkerProcessor
    ...
    /**
     * @param updateDelay the time in seconds between checks on the modified date for cached
     *     templates
     */
        ...
        for (Store apiStore : searchPath.getStores()) {
            ...
            if (loader == null) {
                throw new WebScriptException(
                        "Unable to retrieve template loader for Web Script store "
                                + apiStore.getBasePath());
            }
            loaders.add(loader);
        }
        MultiTemplateLoader loader =
    ...
    private void initProcessorExtensions() {
        for (ProcessorExtension processorExtension :
                this.freeMarkerProcessor.getProcessorExtensions()) {

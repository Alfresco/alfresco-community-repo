    ...
    try {
      ...
      if (json.has(RATING) == false) {
        throw new WebScriptException(
          ...
          "rating parameter missing when applying rating"
        );
      }
      if (json.has(RATING_SCHEME) == false) {
        throw new WebScriptException(
          Status.STATUS_BAD_REQUEST,
          "schemeName parameter missing when applying rating"
        );
      }
      ...
      if (scheme == null) {
        throw new WebScriptException(
          Status.STATUS_BAD_REQUEST,
          "Unknown scheme name: " + schemeName
        );
      }
      ...
      String ratedNodeUrlFragment = nodeRefToBeRated
        .toString()
        .replace("://", "/");
      String ratedNodeUrl = MessageFormat.format(
        NODE_RATINGS_URL_FORMAT,
        ratedNodeUrlFragment
      );
      ...
      model.put(
        AVERAGE_RATING,
        ratingService.getAverageRating(nodeRefToBeRated, schemeName)
      );
      model.put(
        RATINGS_TOTAL,
        ratingService.getTotalRating(nodeRefToBeRated, schemeName)
      );
      model.put(
        RATINGS_COUNT,
        ratingService.getRatingsCount(nodeRefToBeRated, schemeName)
      );

...
public class CompositeRenderingEngine extends AbstractRenderingEngine {
  ...
  protected void executeRenditionImpl(Action action, NodeRef sourceNode) {
    ...
    if (action instanceof CompositeRenditionDefinition) {
      ...
      ChildAssociationRef renditionAssoc = executeCompositeRendition(
        compositeDefinition,
        sourceNode
      );
      ...
      String msg =
        "This method requires that the RenditionDefinition be of type CompositeRenditionDefinition";
      logger.warn(msg);
  ...
  private ChildAssociationRef executeCompositeRendition(
    CompositeRenditionDefinition definition,
    NodeRef sourceNode
  ) {
    ...
    for (RenditionDefinition subDefinition : definition.getActions()) {
      ChildAssociationRef nextResult = executeSubDefinition(
        source,
        subDefinition,
        parent,
        assocType
  ...
  private ChildAssociationRef executeSubDefinition(
    ...
    subDefinition.setParameterValue(
      RenditionService.PARAM_IS_COMPONENT_RENDITION,
      true
    );
    actionService.executeAction(subDefinition, source);
    ChildAssociationRef newResult = (ChildAssociationRef) subDefinition.getParameterValue(
      PARAM_RESULT
    );
    return newResult;
  }

...
public class FileFolderServiceImplTest extends TestCase {
  ...
  private static final String IMPORT_VIEW =
    "filefolder/filefolder-test-import.xml";
  ...
  private static final String NAME_DISCUSSION_FOLDER =
    "CHECK_DISCUSSION_RENAME";
  ...
  public void setUp() throws Exception {
    ...
    InputStream is = getClass()
      .getClassLoader()
      .getResourceAsStream(IMPORT_VIEW);
    if (is == null) {
  ...
  private void checkFileList(
    ...
    assertTrue(
      "Name list was not exact - remaining: " + check,
      check.size() == 0
    );
  ...
  private FileInfo getByName(String name, boolean isFolder) throws Exception {
    List<FileInfo> results = fileFolderService.search(
      workingRootNodeRef,
      name,
      !isFolder,
      isFolder,
  ...
  public void testRenameWithoutAssocQNameChange() throws Exception {
    ...
    NodeRef newFileNodeRef = fileFolderService
      .create(
        folderNodeRef,
        "AnotherFile.txt",
        ContentModel.TYPE_CONTENT,
  ...
  public void testRenameDiscussionALF5569() throws Exception {
    ...
    FileInfo newFileInfo = fileFolderService.rename(
      discussionNodeRef,
      NAME_DISCUSSION_FOLDER
    );

    // get the name now
    String newName = (String) nodeService.getProperty(
  ...
  public void testMove() throws Exception {
    ...
    List<FileInfo> checkFileInfos = fileFolderService.search(
      workingRootNodeRef,
      NAME_L1_FOLDER_A,
      false
    );
    ...
    // rename properly
    FileInfo checkFileInfo = fileFolderService.move(
      folderToMoveRef,
      null,
      "new name"
    );
  ...
  public void testMovePermissions() throws Exception {
    ...
    final NodeRef targetNodeRef = AuthenticationUtil.runAs(
      createTargetWork,
    ...
    NodeRef movingNodeRef = fileFolderService
  ...
  public void testCopy() throws Exception {
    ...
    // copy it to the root
    folderToCopyInfo =
      fileFolderService.copy(folderToCopyRef, workingRootNodeRef, null);
    ...
    // copy properly
    FileInfo checkFileInfo = fileFolderService.copy(
      folderToCopyRef,
      null,
      "new name"
    );
  ...
  public void testMakeFolders() throws Exception {
    ...
    FileInfo lastFileInfo = FileFolderServiceImpl.makeFolders(
      fileFolderService,
      rootNodeRef,
      namePath,
      ContentModel.TYPE_FOLDER
    ...
    FileInfo lastFileInfoAgain = FileFolderServiceImpl.makeFolders(
      fileFolderService,
      rootNodeRef,
      namePath,
      ContentModel.TYPE_FOLDER
    ...
    List<FileInfo> checkInfos = fileFolderService.search(
      rootNodeRef,
      "DDD",
      false,
      true,
    ...
    // get the path
    List<FileInfo> checkPathInfos = fileFolderService.getNamePath(
      rootNodeRef,
      checkInfos.get(0).getNodeRef()
    );
    assertEquals(
      "Path created is incorrect",
      namePath.size(),
      checkPathInfos.size()
    );
    int i = 0;
  ...
  public void testGetNamePath() throws Exception {
    ...
    List<FileInfo> infoPaths = fileFolderService.getNamePath(
      workingRootNodeRef,
      nodeRef
    );
    assertEquals("Not enough elements", 2, infoPaths.size());
    assertEquals(
      "First level incorrent",
      NAME_L0_FOLDER_A,
      infoPaths.get(0).getName()
    );
    assertEquals(
      "Second level incorrent",
      NAME_L1_FILE_A,
      infoPaths.get(1).getName()
    );
    ...
    assertEquals(
      "Third level incorrent",
      NAME_L1_FILE_A,
      infoPaths.get(2).getName()
    );
  ...
  public void testGetNameOnlyPath() throws Exception {
    ...
    List<String> infoPaths = fileFolderService.getNameOnlyPath(
      workingRootNodeRef,
      nodeRef
    );
    assertEquals("Not enough elements", 2, infoPaths.size());
  ...
  public void testGetNamePathDoesNotReturnPathContainingNonLeafFileNode()
    ...
    FileInfo dirInfo = fileFolderService.create(
      parentFolderRef,
      "newDir",
      ContentModel.TYPE_FOLDER
    );
    ...
    FileInfo fileInfo2 = fileFolderService.create(
      fileInfo.getNodeRef(),
      "newFile2",
      ContentModel.TYPE_CONTENT
    );
  ...
  public void testGetNamePathDoesNotCrossIntoNonFileFolderHierarchy()
    ...
    NodeRef cmContainer = nodeService
      .createNode(
        rootNodeRef,
        ContentModel.ASSOC_CHILDREN,
  ...
  public void testSearchSimple() throws Exception {
    ...
    NodeRef phantomNodeRef = fileFolderService.searchSimple(
      folderNodeRef,
      "aaaaaaa"
    );
    assertNull("Found non-existent node by name", phantomNodeRef);
    // search for a file that is there
    NodeRef fileNodeRef = fileFolderService.searchSimple(
      folderNodeRef,
      NAME_L1_FILE_A
    );
  ...
  public void testResolveNamePath() throws Exception {
    ...
    FileInfo fileInfoCheck = fileFolderService.resolveNamePath(
      workingRootNodeRef,
      pathElements
    );
  ...
  public void testGetType() throws Exception {
    ...
    try {
      I18NUtil.setContentLocale(Locale.CANADA);
      FileFolderServiceType type = fileFolderService.getType(
        ContentModel.TYPE_FOLDER
      );
      assertEquals(
        "Type incorrect for folder",
        FileFolderServiceType.FOLDER,
        type
      );
    } finally {
      I18NUtil.setContentLocale(savedLocale);
  ...
  public void testAlf6560MimetypeSetting() throws Exception {
    ...
    out.write(
      "<html><body>hall\u00e5 v\u00e4rlden</body></html>".getBytes("UnicodeBig")
    );
    out.close();
    ...
    assertEquals(
      "Encoding was not automatically set",
      "UTF-16BE",
      reader.getEncoding()
    );
  }
  ...
  public void testGetLocalizedSibling() throws Exception {
    FileInfo base = fileFolderService.create(
      workingRootNodeRef,
      "Something.ftl",
      ContentModel.TYPE_CONTENT
    );
    NodeRef node = base.getNodeRef();
    NodeRef nodeFr = fileFolderService
      .create(workingRootNodeRef, "Something_fr.ftl", ContentModel.TYPE_CONTENT)
      .getNodeRef();
    NodeRef nodeFrFr = fileFolderService
      .create(
        workingRootNodeRef,
        "Something_fr_FR..ftl",
        ContentModel.TYPE_CONTENT
    ...
    FileInfo mbase = fileFolderService.create(
      workingRootNodeRef,
      "Another.get.html.ftl",
      ContentModel.TYPE_CONTENT
    );
    NodeRef mnode = mbase.getNodeRef();
    NodeRef mnodeFr = fileFolderService
      .create(
        workingRootNodeRef,
  ...
  public synchronized void testAlf7421TimestampPropagation() throws Exception {
    ...
    String creatorExpected = (String) nodeService.getProperty(
      folderNodeRef,
      ContentModel.PROP_CREATOR
    );
    Date createdExpected = (Date) nodeService.getProperty(
      folderNodeRef,
      ContentModel.PROP_CREATED
    );
    String modifierExpected = (String) nodeService.getProperty(
      folderNodeRef,
      ContentModel.PROP_MODIFIER
    );
    Date modifiedExpected = (Date) nodeService.getProperty(
      folderNodeRef,
      ContentModel.PROP_MODIFIED
    );

    // Get the current dates for the parent folder (one level up)
    String creatorTooHigh = (String) nodeService.getProperty(
      workingRootNodeRef,
      ContentModel.PROP_CREATOR
    );
    Date createdTooHigh = (Date) nodeService.getProperty(
      workingRootNodeRef,
      ContentModel.PROP_CREATED
    );
    String modifierTooHigh = (String) nodeService.getProperty(
      workingRootNodeRef,
      ContentModel.PROP_MODIFIER
    );
    Date modifiedTooHigh = (Date) nodeService.getProperty(
      workingRootNodeRef,
      ContentModel.PROP_MODIFIED
    );
    ...
    FileInfo sourceFolderInfo = fileFolderService.create(
      workingRootNodeRef,
      "SourceFolder",
      ContentModel.TYPE_FOLDER
    );
    ...
    FileInfo destinationFolderInfo = fileFolderService.create(
      workingRootNodeRef,
      "DestinationFolder",
      ContentModel.TYPE_FOLDER
    );
    ...
    String sourceFolderCreatorExpected = (String) nodeService.getProperty(
      sourceFolderNodeRef,
      ContentModel.PROP_CREATOR
    );
    Date sourceFolderCreatedExpected = (Date) nodeService.getProperty(
      sourceFolderNodeRef,
      ContentModel.PROP_CREATED
    );

    String destinationFolderCreatorExpected = (String) nodeService.getProperty(
      destinationFolderNodeRef,
      ContentModel.PROP_CREATOR
    );
    Date destinationFolderCreatedExpected = (Date) nodeService.getProperty(
      destinationFolderNodeRef,
      ContentModel.PROP_CREATED
    );

    FileInfo relocatableFileInfo = fileFolderService.create(
      sourceFolderNodeRef,
      "MoveMePlease.html",
      ContentModel.TYPE_CONTENT
    );
    ...
    String sourceFolderModifierExpected = (String) nodeService.getProperty(
      sourceFolderNodeRef,
      ContentModel.PROP_MODIFIER
    );

    // Get the dates for the destination folder
    String destinationFolderModifierExpected = (String) nodeService.getProperty(
      destinationFolderNodeRef,
      ContentModel.PROP_MODIFIER
    );
  ...
  public void testPatterns() {
    ...
    PagingResults<FileInfo> pagingResults = fileFolderService.list(
      workingRootNodeRef,
      true,
      true,
      "L0*",
  ...
  public void testList_HiddenFiles() {
    ...
    try {
      ...
      NodeRef nodeRef = fileFolderService
        ...
        .getNodeRef();
      NodeRef nodeRef1 = fileFolderService
  ...
  public void testListNotIgnoreSpaces() {
    ...
    for (String folder : foldersToTest) {
      fileFolderService
        .create(testFolder, folder, ContentModel.TYPE_FOLDER)
        .getNodeRef();
    }
    ...
    for (int i = 1; i <= pageCount; i++) {
      ...
      assertEquals(
        (
          i != pageCount
            ? pageSize
            : (totalItems - ((pageCount - 1) * pageSize))
        ),
  ...
  public void testCopyOfWorkingCopy_ALF_8863() throws Exception {
    ...
    NodeRef testNodeRef = fileFolderService
      .create(
        workingRootNodeRef,
    ...
    FileInfo destDirInfo = fileFolderService.create(
      workingRootNodeRef,
      "destDir",
      ContentModel.TYPE_FOLDER
    );
    ...
    FileInfo copyInfo = fileFolderService.copy(
      workingCopyRef,
      destDirInfo.getNodeRef(),
      null
    );
    String checkedOutName = (String) nodeService.getProperty(
      testNodeRef,
      ContentModel.PROP_NAME
    );
  ...
  public void testSortingCustomFields() {
    ...
    NodeRef nodeRef = fileFolderService
      ...
      .getNodeRef();
    NodeRef parentTestRef = fileFolderService
      .create(nodeRef, "parent", ContentModel.TYPE_CONTENT)
      .getNodeRef();
    ...
    expectedNames =
      new String[] { "A-foo", "A-bar", "A-null", "B-null", "B-baz", "B-biz" };
    checkFileList(pageRes, 6, 0, expectedNames);
    ...
    expectedNames =
      new String[] { "B-biz", "B-baz", "A-null", "B-null", "A-bar", "A-foo" };
    checkFileList(pageRes, 6, 0, expectedNames);
    ...
    expectedNames =
      new String[] { "A-foo", "A-bar", "B-biz", "B-baz", "A-null", "B-null" };
    checkFileList(pageRes, 6, 0, expectedNames);
    ...
    expectedNames =
      new String[] { "A-null", "B-null", "A-bar", "A-foo", "B-baz", "B-biz" };
    checkFileList(pageRes, 6, 0, expectedNames);
    ...
    expectedNames =
      new String[] { "B-biz", "B-baz", "A-foo", "A-bar", "A-null", "B-null" };
    checkFileList(pageRes, 6, 0, expectedNames);
    ...
    expectedNames =
      new String[] { "B-null", "A-foo", "A-bar", "B-baz", "A-null", "B-biz" };
    checkFileList(pageRes, 6, 0, expectedNames);
  }

  public void testMoveCopyLotsOfFiles() throws FileNotFoundException {
    final String CONTAINING_FOLDER =
      "CONTAINING FOLDER " + GUID.generate(), FOLDER_1 =
      "FOLDER 1 " + GUID.generate(), FOLDER_2 = "FOLDER 2 " + GUID.generate();

    FileInfo containingFolder = fileFolderService.create(
      workingRootNodeRef,
      CONTAINING_FOLDER,
      ContentModel.TYPE_FOLDER
    ), folder1 = fileFolderService.create(
      containingFolder.getNodeRef(),
      FOLDER_1,
      ContentModel.TYPE_FOLDER
    ), folder2 = fileFolderService.create(
      containingFolder.getNodeRef(),
      FOLDER_2,
      ContentModel.TYPE_FOLDER
    );
    ...
    assertEquals(
      COUNT,
      fileFolderService.listFiles(folder1.getNodeRef()).size()
    );
    ...
    assertEquals(
      2,
      fileFolderService.list(containingFolder.getNodeRef()).size()
    );
    ...
    List<FileInfo> folders = fileFolderService.listFolders(
      folder2.getNodeRef()
    );
    assertEquals(1, folders.size());
    assertEquals(
      COUNT,
      fileFolderService.listFiles(folders.get(0).getNodeRef()).size()
    );
    ...
    // copy back
    FileInfo newFolder = fileFolderService.copy(
      folder1.getNodeRef(),
      containingFolder.getNodeRef(),
      null
    );

...
public class AuditLogPost extends BaseAuditRetrievalWebScript {
   ...
   */
  private void writeResponse(WebScriptResponse res, String response)
    throws IOException {
    // setup response
  ...
  private NodeRef fileAuditTrail(NodeRef destination, WebScriptRequest req) {
    NodeRef record = rmAuditService.fileAuditTrailAsRecord(
      parseQueryParameters(req),
      destination,
      parseReportFormat(req)
    );
   ...
   */
  private JSONObject getJSONObjectFromRequest(WebScriptRequest req)
    throws IOException {
    JSONObject json;
    ...
    try {
      ...
      throw new AlfrescoRuntimeException(
        "Error creating json object from request content.",
        error
      );
    }
  ...
  private NodeRef getDestination(JSONObject json) {
    ...
    try {
      ...
      throw new AlfrescoRuntimeException(
        "Error extracting 'destination' from parameter.",
        error
      );
    }
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Filing audit trail as record in record folder: '" + destination + "'."
      );
    }

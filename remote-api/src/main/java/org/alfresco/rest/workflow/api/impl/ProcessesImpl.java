...
public class ProcessesImpl extends WorkflowRestImpl implements Processes {
    ...
    protected static final Set<String> PROCESS_STATUS_LIST =
            new HashSet<String>(
                    Arrays.asList(
                            PROCESS_STATUS_ANY,
    ...
    protected static final Set<String> PROCESS_COLLECTION_GREATERTHAN_QUERY_PROPERTIES =
            new HashSet<String>(Arrays.asList("startedAt", "endedAt"));
        ...
        if (handlerRegistry == null) {
            handlerRegistry =
                    new WorkflowPropertyHandlerRegistry(
                            getDefaultPropertyHandler(), getQNameConverter());
        }
    ...
    public CollectionWithPagingInfo<ProcessInfo> getProcesses(Parameters parameters) {
        ...
        MapBasedQueryWalker propertyWalker =
                new MapBasedQueryWalker(PROCESS_COLLECTION_EQUALS_QUERY_PROPERTIES, null);
        ...
        Boolean includeVariables =
                propertyWalker.getProperty(
                        "includeVariables", WhereClauseParser.EQUALS, Boolean.class);
        ...
        final HistoricProcessInstanceQuery query =
                activitiProcessEngine.getHistoryService().createHistoricProcessInstanceQuery();
            ...
            for (QueryVariableHolder queryVariableHolder : variableProperties) {
                if (queryVariableHolder.getOperator() == WhereClauseParser.EQUALS) {
                    ...
                    throw new InvalidArgumentException(
                            "variable "
                                    + queryVariableHolder.getPropertyName()
                                    + " can only be used with an =, >, >=, <=, <, not, matches"
                                    + " comparison type");
                }
            }
        }
        ...
        if (authorityService.isAdminAuthority(AuthenticationUtil.getRunAsUser())) {
            ...
            // If non-admin user, involvement in the process is required (either owner, assignee or
            // externally involved).
        ...
        for (HistoricProcessInstance processInstance : processInstances) {
            ...
            if (includeVariables != null && includeVariables) {
                ...
                if (definitionTypeMap.containsKey(processInfo.getProcessDefinitionId())) {
                    ...
                    List<Variable> resultingVariables =
                            restVariableHelper.getVariables(
    ...
    public ProcessInfo create(ProcessInfo process) {
        ...
        StartFormData startFormData =
                activitiProcessEngine.getFormService().getStartFormData(processDefinitionId);
        if (startFormData != null) {
            if (CollectionUtils.isEmpty(process.getVariables()) == false) {
                TypeDefinition startTaskType =
                        getWorkflowFactory()
                    ...
                    if (propNameMap.containsKey(variableName)) {
                        ...
                        if (taskAssociations.containsKey(propNameMap.get(variableName))) {
                            AssociationDefinition associationDef =
                                    taskAssociations.get(propNameMap.get(variableName));
                            variableValue =
                                    convertAssociationDefinitionValue(
                                            associationDef, variableName, variableValue);
                        } else if (taskProperties.containsKey(propNameMap.get(variableName))) {
                            PropertyDefinition propDef =
                                    taskProperties.get(propNameMap.get(variableName));
                            DataTypeDefinition propDataType = propDef.getDataType();
                            if ("java.util.Date"
                                    .equalsIgnoreCase(propDataType.getJavaClassName())) {
                                // fix for different ISO 8601 Date format classes in Alfresco
                                // (org.alfresco.util and Spring Surf)
                        ...
                        if (variableValue instanceof Serializable) {
                            startParams.put(
                                    propNameMap.get(variableName), (Serializable) variableValue);
                        }
                    }
                }
            }
        ...
        // Set start task properties. This should be done before instance is started, since it's id
        // will be used
        Map<String, Object> variables =
                getPropertyConverter().getStartVariables(processDefinitionId, startParams);
        ...
        ProcessInstance processInstance =
                runtimeService.startProcessInstanceById(
                        processDefinitionId, process.getBusinessKey(), variables);
    ...
    public CollectionWithPagingInfo<Variable> getVariables(String processId, Paging paging) {
        ...
        // Convert raw variables to Variable objects
        List<Variable> resultingVariables =
                restVariableHelper.getVariables(variables, startTaskTypeDefinition);
    ...
    public Variable updateVariable(String processId, Variable variable) {
        ...
        return updateVariableInProcess(
                processId, processInstance.getProcessDefinitionId(), variable);
    }
    ...
    protected Variable updateVariableInProcess(
            String processId, String processDefinitionId, Variable variable) {
            ...
            if (context.getAssociationDefinition(variable.getName()) != null) {
                ...
                actualValue =
                        DefaultTypeConverter.INSTANCE.convert(
                                dataTypeDefinition, variable.getValue());
            }
        }
        ...
        try {
            ...
            if (model != null && model.getLocationMap().size() > 0) {
                List<String> activeActivities =
                        activitiProcessEngine.getRuntimeService().getActiveActivityIds(processId);
                ProcessDiagramGenerator generator = new DefaultProcessDiagramGenerator();
                InputStream generateDiagram =
                        generator.generateDiagram(model, "png", activeActivities);
    ...
    protected ProcessInfo createProcessInfo(HistoricProcessInstance processInstance) {
        ...
        ProcessDefinition definitionEntity =
                activitiProcessEngine
                        .getRepositoryService()

...
public class UserFeedRetrieverWebScript extends DeclarativeWebScript {
  ...
  private static final Log logger = LogFactory.getLog(
    UserFeedRetrieverWebScript.class
  );
  ...
  protected Map<String, Object> executeImpl(
    ...
    String[] extParts = extensionPath == null
      ? new String[1]
      : extensionPath.split("/");
    ...
    if (extParts.length == 1) {
      ...
      throw new AlfrescoRuntimeException(
        "Unexpected extension: " + extensionPath
      );
    }
    ...
    if (
      (exclThisUserStr != null) &&
      (
        exclThisUserStr.equalsIgnoreCase("true") ||
        exclThisUserStr.equalsIgnoreCase("t")
      )
    ) {
      exclThisUser = true;
    }
    ...
    if (
      (exclOtherUsersStr != null) &&
      (
        exclOtherUsersStr.equalsIgnoreCase("true") ||
        exclOtherUsersStr.equalsIgnoreCase("t")
      )
    ) {
      exclOtherUsers = true;
    }
    ...
    if (
      (onlyFollowingStr != null) &&
      (
        onlyFollowingStr.equalsIgnoreCase("true") ||
        onlyFollowingStr.equalsIgnoreCase("t")
      )
    ) {
      userFilter = new HashSet<String>();
      if (subscriptionService.isActive()) {
        PagingFollowingResults following = subscriptionService.getFollowing(
        ...
        if (following.getPage() != null) {
          for (String userName : following.getPage()) {
            userFilter.add(
              this.userNamesAreCaseSensitive ? userName : userName.toLowerCase()
            );
          }
        }
      }
    ...
    try {
      List<String> feedEntries = activityService.getUserFeedEntries(
        feedUserId,
        siteId,
        exclThisUser,
        exclOtherUsers,
      ...
      if (format.equals(FeedTaskProcessor.FEED_FORMAT_JSON)) {
        ...
        try {
          for (String feedEntry : feedEntries) {
            activityFeedModels.add(
              JSONtoFmModel.convertJSONObjectToMap(feedEntry)
            );
          }

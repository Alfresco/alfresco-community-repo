...
public class CORSContextListener implements ServletContextListener {
  ...
  private static final String CORS_SUPPORT_CREDENTIALS =
    "cors.support.credentials";
  private static final String CORS_PREFLIGHT_CREDENTIALS =
    "cors.preflight.maxage";

  private Log logger = LogFactory.getLog(getClass());

  private final EnumSet<DispatcherType> DISPATCHER_TYPE = EnumSet.of(
  ...
  private void initCORS(ServletContext servletContext) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "CORS filter is" + (corsEnabled ? " " : " not ") + "enabled"
      );
    }
    if (corsEnabled) {
      ...
      corsFilter.setInitParameter(
        ...
        gP.getProperty(CORS_ALLOWED_ORIGINS)
      );
      corsFilter.setInitParameter(
        CORS_ALLOWED_METHODS,
        gP.getProperty(CORS_ALLOWED_METHODS)
      );
      corsFilter.setInitParameter(
        CORS_ALLOWED_HEADERS,
        gP.getProperty(CORS_ALLOWED_HEADERS)
      );
      corsFilter.setInitParameter(
        CORS_EXPOSED_HEADERS,
        gP.getProperty(CORS_EXPOSED_HEADERS)
      );
      corsFilter.setInitParameter(
        CORS_SUPPORT_CREDENTIALS,
        gP.getProperty(CORS_SUPPORT_CREDENTIALS)
      );
      corsFilter.setInitParameter(
        CORS_PREFLIGHT_CREDENTIALS,
        gP.getProperty(CORS_PREFLIGHT_CREDENTIALS)
      );

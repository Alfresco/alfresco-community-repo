 ...
 *
 * <p>This service will refuse to create any user authorities. These should be managed using the
 * AuthenticationService and PersonServce. Methods that try to change alter users will throw an
 * exception.
 *
 * <p>A string key is used to identify the authority. These follow the contract defined in
 * AuthorityType. If there are entities linked to these authorities this key should be used to find
 * them, as userName is used link user and person.
 *
 * @author Andy Hind
 */
@AlfrescoPublicApi
public interface AuthorityService {
     ...
     *
     * <p>There is no contract for who should have this authority, only that it can be tested here.
     ...
     *
     * <p>See also "getAuthorities" (paged) alternative
     *
     * @param type the type of authorities - cannot be null
     * @return all authorities by type
     * @deprecated use {@link #getAuthorities(AuthorityType, String, String, boolean, boolean,
     *     PagingRequest)} at least
     ...
     * @param displayNameFilter optional filter (startsWith / ignoreCase) for authority display name
     *     (note: implied trailing "*")
     * @param sortBy either "displayName", "shortName", "authorityName" or null if no sorting. note:
     *     for users, displayName/shortName is equivalent to the userName, for groups if the display
     *     is null then use the short name
     * @param sortAscending if true then sort ascending else sort descending (ignore if
     *     sortByDisplayName is false)
     * @param pagingRequest the requested page (skipCount, maxItems, queryExectionId)
     * @throws org.alfresco.repo.security.authority.UnknownAuthorityException - if zoneName is not
     *     null and does not exist <br>
     *     <br>
     *     author janv
     * @since 4.0
     */
     ...
     * @param displayNameFilter optional filter (startsWith / ignoreCase) for authority display name
     *     (note: implied trailing "*")
     * @param sortByDisplayName if true then sort (ignoring case) by the authority display name, if
     *     false then unsorted note: for users, displayName/shortName is equivalent to the userName,
     *     for groups if the display is null then use the short name
     * @param sortAscending if true then sort ascending else sort descending (ignore if
     *     sortByDisplayName is false)
     * @param pagingRequest the requested page (skipCount, maxItems, queryExectionId)
     * @throws org.alfresco.repo.security.authority.UnknownAuthorityException - if zoneName is not
    ...
    /**
     * Get all root authorities by type. Root authorities are ones that were created without an
     * authority as the parent authority;
     *
     * @param type - the type of the authority
     ...
     *
     * @param type - the type of the authority
     * @param shortName - the short name of the authority to create this will also be set as the
     *     default display name for the authority
     * @return the name of the authority (this will be the prefix, if any associated with the type
     *     appended with the short name)
     */
     ...
     *
     * @param type the type of the authority
     * @param shortName the short name of the authority to create
     * @param authorityDisplayName the display name for the authority
     * @param authorityZones identifier for external user registry owning the authority or <code>
     *     null</code> if not applicable
     * @return the full name of the authority (this will be the prefix, if any associated with the
     *     type appended with the short name)
     */
    ...
    /**
     * Set an authority to include another authority. For example, adding a group to a group or
     * adding a user to a group.
     *
     * @param parentName - the full name string identifier for the parent.
     * @param childName - the string identifier for the child.
     */
    @Auditable(parameters = {"parentName", "childName"})
    public void addAuthority(String parentName, String childName);

    /**
     * Set a given child authority to be included by the given parent authorities. For example,
     * adding a group to groups or adding a user to groups.
     *
     * @param parentNames - the full name string identifier for the parents.
     * @param childName - the string identifier for the child.
     */
    ...
    /**
     * Remove an authority as a member of another authority. The child authority will still exist.
     * If the child authority was not created as a root authority and you remove its creation link,
     * it will be moved to a root authority. If you want rid of it, use delete.
     *
     * @param parentName - the string identifier for the parent.
     * @param childName - the string identifier for the child.
     */
    @Auditable(parameters = {"parentName", "childName"})
    ...
    /**
     * Delete an authority and all its relationships, optionally recursively deleting child
     * authorities of the same type.
     *
     * @param name the authority long name
     * @param cascade should the delete be cascaded to child authorities of the same type?
     */
     ...
     *
     * <p>For a group you could get all the authorities it contains, just the users it contains or
     * just the other groups it includes.
     *
     * @param type - if not null, limit to the type of authority specified
     * @param name - the name of the containing authority
     * @param immediate - if true, limit the depth to just immediate child, if false find
     *     authorities at any depth
     */
    @Auditable(parameters = {"type", "name", "immediate"})
    public Set<String> getContainedAuthorities(AuthorityType type, String name, boolean immediate);

    /**
     * Get the authorities that contain the given authority, <b>but use {@code
     * getAuthoritiesForUser(userName).contains(authority)}</b> rather than {@code
     * getContainingAuthorities(type, userName, false).contains(authority)} or use {@link
     * #getContainingAuthoritiesInZone(AuthorityType, String, String, AuthorityFilter, int)} <b>as
     * they will be much faster</b>.
     *
     * <p>For example, this method can be used find out all the authorities that contain a group.
     *
     * @param type - if not null, limit to the type of authority specified
     * @param name - the name of the authority for which the containing authorities are required.
     * @param immediate - limit to immediate parents or any ancestor.
     */
    @Auditable(parameters = {"type", "name", "immediate"})
     ...
     * @param name if non-null, only return those authorities who contain this authority
     ...
     * @param size if greater than zero, the maximum results to return. The search strategy used is
     *     varied depending on this number.
    ...
    /**
     * Set the display name for the given authority. Setting the display name is only supported for
     * authorities of type group
     *
     ...
     *
     * @param name the authority long name
     * @return the the name of the zone containing the specified authority, {@link
     *     AuthorityService#ZONE_APP_DEFAULT} if the authority exists but has no zone, or <code>null
     *     </code> if the authority does not exist.
     */
    @Auditable(parameters = {"name"})
     ...
     *
     * <p>See also "getAuthorities" paged alternative (note: in that case, zone must exist)
     *
     * @param zoneName the zone name - note: if zone does not exist then will currently return empty
     *     set
    ...
    /**
     * Search for authorities by pattern matching (* and ?) against the authority name. Note: This
     * will use a search index to find the results (eg. via Lucene / SOLR).
     *
     * @param type AuthorityType
     * @param parentAuthority if non-null, will look only for authorities who are a child of the
     *     named parent
     * @param immediate if <code>true</code> then only search root groups if parentAuthority is
     *     null, or immediate children of parentAuthority if it is non-null.
     ...
     *     otherwise false
     * @throws UnsupportedOperationException if the implementing class (i.e. external clients)
     *     doesn't provide an implementation for the {@code hasSysAdminAuthority} operation
     * @since 7.1
     */
    @Auditable

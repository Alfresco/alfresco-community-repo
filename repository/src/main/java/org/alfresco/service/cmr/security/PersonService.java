 ...
 * <p>
 *
 * <p>People and groups may be managed entirely in the repository or entirely in some other
 * implementation such as LDAP or via NTLM. Some properties may in the repository and some in
...
public interface PersonService {
    /**
     * Get a person by userName. The person is store in the repository. The person may be created as
     * a side effect of this call, depending on the setting of {@link
     * #setCreateMissingPeople(boolean) to create missing people or not}. The home folder will also
     * be created as a side effect if it does not exist.
     *
     * @param userName - the userName key to find the person
     * @return Returns the person node, either existing or new
     * @throws NoSuchPersonException if the user doesn't exist and could not be created
     *     automatically
    ...
    /**
     * Get a person by userName. The person is store in the repository. No missing person objects
     * will be created as a side effect of this call. If the person is missing from the repository
    ...
    /**
     * Retrieve the person NodeRef for a {@code username}, optionally creating the home folder if it
     * does not exist and optionally creating the person if they don't exist AND the PersonService
     * is configured to allow the creation of missing persons {@link
     * #setCreateMissingPeople(boolean)}.
     *
     * <p>If not allowed to create missing persons and the person does not exist a {@code
     * NoSuchPersonException} exception will be thrown.
     *
     * @param userName of the person NodeRef to retrieve
     * @param autoCreateHomeFolderAndMissingPersonIfAllowed If the person exits: should we create
     *     the home folder if it does not exist? If the person exists AND the creation of missing
     *     persons is allowed should we create both the person and home folder.
     * @return NodeRef of the person as specified by the username
     * @throws NoSuchPersonException if the person doesn't exist and can't be created
     */
    ...
    /**
     * Does this service create people on demand if they are missing. If this is true, a call to
     * getPerson() will create a person if they are missing.
     *
    ...
    /**
     * Get the list of properties that are mutable. Some service may only allow a limited list of
     * properties to be changed. This may be those persisted in the repository or those that can be
     * changed in some other implementation such as LDAP.
     *
     ...
     * <b>the home folder is created if it doesn't exist</b>
     *
     ...
     * @param userName - the user for which the properties should be set.
     * @param properties - the map of properties to set (as the NodeService)
     * @param autoCreateHomeFolder should we auto-create the home folder if it doesn't exist.
     */
    ...
    /**
     * Create a new person with the given properties. The userName is one of the properties. Users
     * with duplicate userNames are not allowed.
     *
     * @return NodeRef
     */
    ...
    /**
     * Create a new person with the given properties, recording them against the given zone name
     * (usually identifying an external user registry from which the details were obtained). The
     * userName is one of the properties. Users with duplicate userNames are not allowed.
     *
     * @param properties the properties
     * @param zones a set if zones including the identifier for the external user registry owning
     *     the person information, or <code>null</code> or an empty set
     * @return the node ref
     */
    ...
    /**
     * Notifies a user by email that their account has been created, and the details of it. Normally
     * called after {@link #createPerson(Map)} or {@link #createPerson(Map, Set)} where email
     * notifications are required.
     *
     * @param userName of the person to notify
     * @param password of the person to notify
     * @throws NoSuchPersonException if the person doesn't exist
     */
    @Auditable(parameters = {"userName"})
    ...
    /**
     * Delete the person identified by the given ref, and optionally delete the associated
     * authentication, if one.
     *
     * @param personRef NodeRef
     ...
     *
     * <p>Note: the pattern is applied to filter props (0 to 3) as startsWithIgnoreCase, which are
     * OR'ed together, for example: cm:userName or cm:firstName or cm:lastName
     *
     ...
     * @param pagingRequest skip, max + optional query execution id <br>
     *     <br>
     *     author janv
     * @since 4.1.2
     */
     ...
     * Get paged list of people optionally filtered and/or sorted
     *
     * @param stringPropFilters list of filter properties (with "startsWith" values), eg.
     *     cm:username "al" might match "alex", "alice", ...
     * @param filterIgnoreCase true to ignore case when filtering, false to be case-sensitive when
     *     filtering
     * @param sortProps sort property, eg. cm:username ascending
     * @param pagingRequest skip, max + optional query execution id <br>
     *     <br>
     ...
     * @param filterStringProps list of filter properties (with "startsWith" values), eg.
     *     cm:username "al" might match "alex", "alice", ...
     ...
     *
     * <p>In order to get paging, use {@link #getPeople(List, boolean, List, PagingRequest)}
     *
    ...
    /**
     * Given the case sensitive user name find the approriate identifier from the person service. If
     * the system is case sensitive it will return the same string. If case insentive it will return
     * the common object. If the user does not exist it will return null;
     *
     ...
     *
     * @throws NoSuchPersonException if the user doesn't exist
     * @return true = enabled.
     */
    @NotAuditable

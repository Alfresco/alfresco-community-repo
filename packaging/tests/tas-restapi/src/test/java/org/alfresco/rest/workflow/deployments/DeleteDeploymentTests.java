...
/**
 * Tests for REST API call DELETE "/deployments/{deploymentId}" Class has priority 100 in order to
 * be executed last in the list of tests classes
 *
...
public class DeleteDeploymentTests extends RestTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify admin user deletes a specific deployment using REST API and status code"
                            + " is successful (204)")
    @Test(
            groups = {
                TestGroup.REST_API,
    ...
    public void adminDeletesDeploymentWithSuccess() throws Exception {
        ...
        // The deployment with name "customWorkflowExtentionForRest.bpmn" is created by Workflow
        // Extention Point
        RestDeploymentModelsCollection allDeployments =
                restClient.authenticateUser(adminUser).withWorkflowAPI().getDeployments();
        allDeployments
                .assertThat()
        ...
        RestProcessDefinitionModel processDefinitionAssociated =
                restClient
                        ...
                        .getProcessDefinitionById(deployment.getId());

        RestProcessModel addedProcess =
                restClient
                        .withWorkflowAPI()
                        .addProcess(
                                processDefinitionAssociated.getName(),
                                adminUser,
                                false,
        ...
        restClient
                ...
                .assertLastError()
                .containsSummary(
                        String.format(RestErrorModel.ENTITY_NOT_FOUND, deployment.getId()));

        // check that process definitions contained by deployment are deleted
        restClient
                .withWorkflowAPI()
                .usingProcessDefinitions(processDefinitionAssociated)
        ...
        restClient
                ...
                .containsSummary(
                        String.format(
                                ...
                                processDefinitionAssociated.getId()));
        ...
        restClient
                ...
                .assertLastError()
                .containsSummary(
                        String.format(RestErrorModel.ENTITY_NOT_FOUND, addedProcess.getId()));
        ...
        for (RestTaskModel processTask : processTasks) {
            ...
            restClient
                    ...
                    .assertLastError()
                    .containsSummary(
                            String.format(RestErrorModel.ENTITY_NOT_FOUND, processTask.getId()));
        }
        ...
        restClient
                .assertStatusCodeIs(HttpStatus.NOT_FOUND)
                .assertLastError()
                .containsSummary(String.format(RestErrorModel.ENTITY_NOT_FOUND, deployment.getId()))
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Verify admin user cannot delete an inexistent deployment using REST API and"
                            + " status code is successful (204)")
    @Test(
            groups = {
                TestGroup.REST_API,
                TestGroup.DEPLOYMENTS,
    ...
    public void adminCannotDeleteInexistentDeployment() throws Exception {
        ...
        restClient
                .assertStatusCodeIs(HttpStatus.NOT_FOUND)
                .assertLastError()
                .containsSummary(String.format(RestErrorModel.ENTITY_NOT_FOUND, "1000"));
    }
    ...
    @TestRail(
            ...
            description =
                    "Verify deleteDeployment is unsupported for empty deployment id with REST API"
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify deleteDeployment is forbidden using non admin user or different user"
                            + " than creator with REST API and status code is 403")
    @Test(
            groups = {
                TestGroup.REST_API,
                TestGroup.DEPLOYMENTS,

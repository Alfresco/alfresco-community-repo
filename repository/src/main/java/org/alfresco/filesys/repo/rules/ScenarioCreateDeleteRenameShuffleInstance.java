...
public class ScenarioCreateDeleteRenameShuffleInstance
  ...
  private static Log logger = LogFactory.getLog(
    ScenarioCreateDeleteRenameShuffleInstance.class
  );
      ...
      case ACTIVE:
        ...
        if (operation instanceof DeleteFileOperation) {
          ...
          if (checkFilename) {
            ...
            if (i > 0) {
              String deleteExt = deleteName.substring(
                i + 1,
                deleteName.length()
              );
              String createExt = (j > 0)
                ? createName.substring(j + 1, createName.length())
                : "";
              ...
              if (deleteExt.startsWith("ppt") && createExt.startsWith("ppt")) {
                isRightTarget =
                  ...
                  deleteName
                    .substring(0, i)
                    .equalsIgnoreCase(createName.substring(0, i));
              } else if (deleteExt.startsWith("xls") && createExt.isEmpty()) {
                isRightTarget =
                  !deleteName.startsWith("._") && !deleteName.startsWith("~$");
              }
            }

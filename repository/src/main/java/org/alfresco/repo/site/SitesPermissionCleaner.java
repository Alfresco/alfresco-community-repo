...
/**
 * This class handles the permissions cleanup which is sometimes necessary after a node has been
 * moved or copied between sites. It removes any permissions that pertain to the former site that
 * may be present on the relocated node.
 *
...
public class SitesPermissionCleaner {
    ...
    public void cleanSitePermissions(final NodeRef targetNode, SiteInfo containingSite) {
        ...
        // We can calculate the containing site at the start of a recursive call & then reuse it on
        // subsequent calls.
        ...
        // rather than the NodeService. Note: direct use of NodeDAO requires tenantService
        // (ALF-12732).
        ...
        if (targetNodeAcl.getAclType() == ACLType.DEFINING) {
            AccessControlList targetNodeAccessControlList =
                    aclDAO.getAccessControlList(targetNodeAclID);
            List<AccessControlEntry> targetNodeAclEntries =
                    targetNodeAccessControlList.getEntries();
            for (AccessControlEntry entry : targetNodeAclEntries) {
                String authority = entry.getAuthority();

                String thisSiteGroupPrefix =
                        siteServiceImpl.getSiteGroup(containingSite.getShortName(), true);
                ...
                if (authority.startsWith(PermissionService.GROUP_PREFIX)
                        ...
                        !authority.startsWith(PermissionService.ALL_AUTHORITIES)
                ...
                if (!permissionService.getInheritParentPermissions(targetNode)) {
                    // The site manager from the new site, where this node was moved to, has to have
                    // permission to this node
                    String siteManagerAuthority =
                            thisSiteGroupPrefix + "_" + SiteModel.SITE_MANAGER;
                    AuthenticationUtil.runAs(
                            new AuthenticationUtil.RunAsWork<Void>() {
                                public Void doWork() throws Exception {
                                    permissionService.setPermission(
                                            targetNode,
                                            siteManagerAuthority,
        ...
        for (NodeIdAndAclId nextChild : childNodeIds) {
            cleanSitePermissions(
                    nodeDAO.getNodePair(nextChild.getId()).getSecond(), containingSite);
        }
    }
}

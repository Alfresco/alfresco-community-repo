...
public class OnPropertyUpdateRuleTrigger extends RuleTriggerAbstractBase
    ...
    private boolean havePropertiesBeenModified(
            NodeRef nodeRef,
        ...
        // Compare all properties, ignoring protected properties and giving special treatment to
        // content properties
        ...
        for (QName name : keys) {
            ...
            if (name.equals(ContentModel.PROP_PREFERENCE_VALUES)
                    || name.equals(ContentModel.PROP_CASCADE_CRC)) {
                continue;
            }
            ...
            // Ignore protected properties
                ...
                if (propertyDefinition.getDataType().getName().equals(DataTypeDefinition.CONTENT)
                    ...
                    if (this.ignoreEmptyContent) {
                        ...
                        if (!ContentData.hasContent(beforeContent)
                                || beforeContent.getSize() == 0) {
                            beforeValue = null;
                        }
    ...
    /**
     * ALF-17483: It's possible that even for a single-valued contentdata property, its definition
     * may have been changed and the previous persisted value is multi-valued, so let's be careful
     * about converting to ContentData.
     *
     * @param object property value to convert
    ...
    public void onUpdateProperties(
        ...
        if (wasCreatedInTxn) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Receiving property update for node created in transaction: " + nodeRef);
            }
                ...
                if (havePropertiesBeenModified(nodeRef, before, after, true, true)) {
                    ...
                    if (logger.isDebugEnabled() == true) {
                        logger.debug(
                                "New node "
                                        + nodeRef.toString()
                                        + " confirmed to have no content properties or to have new"
                                        + " content so firing inbound rules.");
                    }
                    ...
                    if (logger.isDebugEnabled() == true) {
                        logger.debug(
                                "Removing the pending rules for the new node "
                                        + nodeRef.toString()
            ...
            if (!wasCreatedInTxn
                    && before.size() != 0
                    && // ALF-4846: Do not trigger for newly created nodes
                    havePropertiesBeenModified(nodeRef, before, after, false, this.onNewContent)) {
                // Keep track of name changes explicitly.  This prevents the later association
                // change from
                // triggering 'inbound' rules
                if (!EqualsHelper.nullSafeEquals(
                        before.get(ContentModel.PROP_NAME), after.get(ContentModel.PROP_NAME))) {
                    // Name has changed

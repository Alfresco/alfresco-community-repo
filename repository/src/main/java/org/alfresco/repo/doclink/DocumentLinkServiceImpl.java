...
public class DocumentLinkServiceImpl
  ...
  protected static final String CANNED_QUERY_GET_DOC_LINKS =
    "getDoclinkNodesCannedQueryFactory";
  ...
  public NodeRef createDocumentLink(final NodeRef source, NodeRef destination) {
    ...
    if (!nodeService.exists(source)) {
      throw new IllegalArgumentException(
        "Source NodeRef '" + source + "' does not exist"
      );
    }
    ...
    if (
      !dictionaryService.isSubClass(
        nodeService.getType(destination),
    ...
    if (
      checkOutCheckInService.isWorkingCopy(source) ||
    ...
    try {
      if (dictionaryService.isSubClass(sourceType, ContentModel.TYPE_CONTENT)) {
        ...
        !dictionaryService.isSubClass(sourceType, SiteModel.TYPE_SITE) &&
        dictionaryService.isSubClass(
          nodeService.getType(source),
          ContentModel.TYPE_FOLDER
        )
      ) {
  ...
  public NodeRef getLinkDestination(NodeRef linkNodeRef) {
    ...
    if (
      ...
      throw new IllegalArgumentException(
        "The provided node is not a document link"
      );
    }
  ...
  public List<Long> getNodeLinksIds(NodeRef nodeRef) {
    ...
    Pair<Long, QName> nameQName = qnameDAO.getQName(
      ContentModel.PROP_LINK_DESTINATION
    );
    if (nameQName != null) {
      // Execute the canned query if there are links in the database
      GetDoclinkNodesCannedQueryParams parameterBean = new GetDoclinkNodesCannedQueryParams(
        nodeRef.toString(),
        nameQName.getFirst(),
      ...
      CannedQuery<Long> query = new GetDoclinkNodesCannedQuery(
        cannedQueryDAO,
        params
      );
  ...
  public DeleteLinksStatusReport deleteLinksToDocument(NodeRef document) {
    ...
    for (Long linkId : linkNodeIds) {
      NodeRef linkNodeRef = AuthenticationUtil.runAs(
        ...
        if (
          ...
          report.addErrorDetail(
            ...
            new AccessDeniedException(
              ...
              "' doesn't have permission to create discussion on node '" +
              linkNodeRef +
              "'"
            )
          );
  ...
  public void beforeDeleteLinkNode(NodeRef linkNodeRef) {
    ...
    long linkNodeId = (Long) nodeService.getProperty(
      linkNodeRef,

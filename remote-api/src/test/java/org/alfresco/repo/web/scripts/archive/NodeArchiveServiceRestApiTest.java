...
public class NodeArchiveServiceRestApiTest extends BaseWebScriptTest {
  ...
  protected void setUp() throws Exception {
    ...
    archiveMap =
      (StoreArchiveMap) getServer()
        .getApplicationContext()
        .getBean("storeArchiveMap");
  ...
  public void testGetDeletedItems() throws Exception {
    ...
    JSONArray deletedNodesArray = (JSONArray) dataObj.get(
      AbstractArchivedNodeWebScript.DELETED_NODES
    );
    assertNotNull(
      "JSON 'deletedNodesArray' object was null",
      deletedNodesArray
    );
    assertTrue(
      "Unexpectedly found 0 items in archive store",
      0 != deletedNodesArray.length()
    );
    ...
    for (int i = 0; i < deletedNodesArray.length(); i++) {
      ...
      String nodeRefString = nextJSONObj.getString(
        AbstractArchivedNodeWebScript.NODEREF
      );
    ...
    for (int i = 0; i < deletedNodesArray.length(); i++) {
      ...
      String nextArchivedDateString = nextJSONObj.getString(
        AbstractArchivedNodeWebScript.ARCHIVED_DATE
      );
  ...
  private void doTest(JSONObject deletedNodeToTest, String archivedBy)
    throws Exception {
    assertNotNull(
      "Failed to find an expected NodeRef within the archive store.",
      deletedNodeToTest
    );

    assertEquals(
      archivedBy,
      deletedNodeToTest.getString(AbstractArchivedNodeWebScript.ARCHIVED_BY)
    );
    assertEquals(
      TEST_TITLE,
      deletedNodeToTest.getString(AbstractArchivedNodeWebScript.TITLE)
    );
    assertEquals(
      TEST_DESCRIPTION,
    ...
    assertNotNull(
      deletedNodeToTest.getString(AbstractArchivedNodeWebScript.NAME)
    );
    assertNotNull(
      deletedNodeToTest.getString(AbstractArchivedNodeWebScript.NODEREF)
    );
    assertNotNull(
      deletedNodeToTest.getString(AbstractArchivedNodeWebScript.DISPLAY_PATH)
    );
    assertNotNull(
      deletedNodeToTest.getString(AbstractArchivedNodeWebScript.ARCHIVED_DATE)
    );
    assertNotNull(
      deletedNodeToTest.getString(AbstractArchivedNodeWebScript.FIRST_NAME)
    );
    assertNotNull(
      deletedNodeToTest.getString(AbstractArchivedNodeWebScript.LAST_NAME)
    );
    assertNotNull(
      deletedNodeToTest.getString(AbstractArchivedNodeWebScript.NODE_TYPE)
    );
  }
  ...
  public void testGetDeletedItemsAsNonAdminUser() throws Exception {
    ...
    assertEquals(
      USER_ONE,
      archivedNode.getString(AbstractArchivedNodeWebScript.ARCHIVED_BY)
    );
  ...
  public void testPurgeDeletedItems() throws Exception {
    ...
    JSONArray archivedNodesArray = dataJsonObj.getJSONArray(
      AbstractArchivedNodeWebScript.DELETED_NODES
    );

    int archivedNodesLength = archivedNodesArray.length();
    assertTrue(
      "Insufficient archived nodes for test to run.",
      archivedNodesLength > 1
    );
    ...
    String nodeRefString = requiredNodeInArchive.getString(
      AbstractArchivedNodeWebScript.NODEREF
    );
    ...
    JSONArray purgedNodesArray = dataObj.getJSONArray(
      ArchivedNodesDelete.PURGED_NODES
    );
    assertEquals(
      "Only expected one NodeRef to have been purged.",
      1,
      purgedNodesArray.length()
    );
    ...
    assertEquals(
      "Archive store was unexpectedly not empty",
      0,
  ...
  public void testPurgeDeletedItemsAsNonAdminUser() throws Exception {
    ...
    // So we have identified a specific Node in the archive that we want to delete permanently (purge).
    String nodeRefString = archivedNode.getString(
      AbstractArchivedNodeWebScript.NODEREF
    );
    assertTrue("nodeRef string is invalid", NodeRef.isNodeRef(nodeRefString));
  ...
  private int getArchivedNodesCount()
    ...
    JSONArray deletedNodesArray = datatObject.getJSONArray(
      AbstractArchivedNodeWebScript.DELETED_NODES
    );
  ...
  public void testRestoreDeletedItems() throws Exception {
    ...
    String nodeRefString = firstArchivedNode.getString(
      AbstractArchivedNodeWebScript.NODEREF
    );
    assertTrue("nodeRef string is invalid", NodeRef.isNodeRef(nodeRefString));
    NodeRef nodeRef = new NodeRef(nodeRefString);
    ...
    Response rsp = sendRequest(
      new PutRequest(restoreUrl, jsonString, "application/json"),
      200
    );
            ...
            new RunAsWork<NodeRef>() {
              public NodeRef doWork() throws Exception {
                ...
                if (deleteNode) {
                  ...
                  NodeRef archivedNode = nodeArchiveService.getArchivedNode(
                    nodeRef
                  );

...
public class SpoofedTextContentReader extends AbstractContentReader {
  ...
  public static final String LEXICON_STEM_PATH =
    "alfresco/textgen/lexicon-stem-@@LOCALE@@.txt";
  ...
  private static final Log logger = LogFactory.getLog(
    SpoofedTextContentReader.class
  );
  ...
  public SpoofedTextContentReader(String url) {
    ...
    try {
      ...
      String jsonLocale = mappedData.containsKey(KEY_LOCALE)
        ? (String) mappedData.get(KEY_LOCALE)
        : Locale.ENGLISH.toString();
      String jsonSeed = mappedData.containsKey(KEY_SEED)
        ? (String) mappedData.get(KEY_SEED)
        : "0";
      String jsonSize = mappedData.containsKey(KEY_SIZE)
        ? (String) mappedData.get(KEY_SIZE)
        : "1024";
      JSONArray jsonWords = mappedData.containsKey(KEY_WORDS)
  ...
  protected ReadableByteChannel getDirectReadableChannel()
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Opened read channel to random text for URL: " + getContentUrl()
        );
      }
      return textChannel;
    } catch (Throwable e) {
  ...
  protected final void setContentUrl(String contentUrl) {
    throw new UnsupportedOperationException(
      "The URL is static and cannot be changed."
    );
  }
}

...
class CommentsLibJs {
     ...
     *
     * <p>We currently use the fm:discussable aspect where we add a "Comments" topic to it.
     */
    public static NodeRef getCommentsFolder(NodeRef node, ServiceRegistry services) {
        // FIXME These methods are from the original JavaScript. Should use the (soon to arrive)
        // CommentService.
        NodeRef result = null;
        if (services.getNodeService().hasAspect(node, ForumModel.ASPECT_DISCUSSABLE)) {
            List<ChildAssociationRef> forumFolders =
                    services.getNodeService()
                            .getChildAssocs(
                                    node, ForumModel.ASSOC_DISCUSSION, RegexQNamePattern.MATCH_ALL);
            // The JavaScript was retrieving the first child under this child-assoc so we'll do the
            // same.
            NodeRef forumFolder = forumFolders.get(0).getChildRef();

            List<ChildAssociationRef> topicFolder =
                    services.getNodeService()
                            .getChildAssocs(
                                    forumFolder,
                                    ContentModel.ASSOC_CONTAINS,

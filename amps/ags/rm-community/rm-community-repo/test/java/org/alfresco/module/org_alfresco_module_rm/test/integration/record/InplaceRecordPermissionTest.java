...
public class InplaceRecordPermissionTest extends BaseRMTestCase {
    ...
    protected void initServices() {
        ...
        editNonRecordMetadataCapability =
                capabilityService.getCapability(RMPermissionModel.EDIT_NON_RECORD_METADATA);
        editRecordMetadataCapability =
                capabilityService.getCapability(RMPermissionModel.EDIT_RECORD_METADATA);
    }

    /**
     * Given a document in a collaboration site When a user without write permissions on the
     * document tries to declare it as a record Then the declaration fails And the document does not
     * become a record
     */
    public void testUserWithOutWriteCantDeclareInPlaceRecord() {
        test().given()
                ...
                .from(
                        () ->
                                permissionService
                                        .hasPermission(dmDocument, PermissionService.WRITE)
                                        .toString())
    ...
    /**
     * Given a document in a collaboration site that is not a record And a contributor the didn't
     * create the document When the contributor tries to declare the document as a record Then the
     * document does not become a record
     */
    ...
    /**
     * Given a document in a collaboration site is not a record When the document is declared by a
     * site collaborator Then the document becomes a record And the site users have the appropriate
     * in-place permissions on the record
     */
    public void testCreateInplaceRecordFromCollabSite() {
        test()
                ...
                // And a site collaborator has filling permissions and filling capability on the
                // record
                .as(dmCollaborator)
                .perform(
                        () ->
    ...
    private void checkInPlaceAccess(NodeRef nodeRef, AccessStatus... accessStatus) {
        ...
        assertEquals(
                "Incorrect read record permission access.",
                accessStatus[0],
                permissionService.hasPermission(nodeRef, RMPermissionModel.READ_RECORDS));
        assertEquals(
                "Incorrect filling permission access.",
                accessStatus[1],
                permissionService.hasPermission(nodeRef, RMPermissionModel.FILING));

        // check capability access
        Map<Capability, AccessStatus> access =
                capabilityService.getCapabilitiesAccessState(nodeRef, CAPABILITIES);
        assertEquals(
                "Incorrect view records capability access",
                accessStatus[2],
                access.get(viewRecordsCapability));
        assertEquals(
                "Incorrect edit non record metadata capability access",
                accessStatus[3],
                access.get(editNonRecordMetadataCapability));
        assertEquals(
                "Incorrect edit record metadata capability access",
                accessStatus[4],
                access.get(editRecordMetadataCapability));
    }

    /**
     * Given that a document is created by contributor When it is declared as an inplace record Then
     * it becomes a record And it isn't filed And a site collaborator has filling permissions and
     * filling capability on the record And a site contributor has filling capability and
     * permissions And a site consumer has read permissions and view record capability on the record
     * And a user that is not a member of the site has no access to the inplace record
     */
    public void testCreateInplaceRecordFromCollabSiteWhenContribIsCreatorOfDocument() {
        test()
                ...
                .perform(
                        () -> {
                            ...
                            dbNodeService.addAspect(
                                    contribDoc, ContentModel.ASPECT_AUDITABLE, null);
                        })
                .expect(false)
                ...
                .from(
                        () ->
                                permissionService
                                        .hasPermission(contribDoc, PermissionService.WRITE)
                                        .toString())
    ...
    /**
     * Given an unfiled in-place record When the record is moved to the file plan (ie filed) Then
     * the site users still have the appropriate in-place permissions on the record
     */
    ...
    /**
     * Given an incomplete inplace record When it is completed Then the inplace users still have
     * access to the record And can't edit the records meta-data
     */
                ...
                .perform(
                        () ->
                                rmActionService.executeRecordsManagementAction(
                                        dmDocument, DeclareRecordAction.NAME))
                .expect(true)
                .from(() -> recordService.isDeclared(dmDocument))
                .because("Record is complete.")
                .then()

                // Then the collaborator has filling permissions, view record capability, but not
    ...
    /**
     * Given an inplace record ready for destruction When it is destroyed And it's metadata is
     * maintained Then the inplace users will no longer see the record
     */
                ...
                .perform(
                        () -> {
                            ...
                            utils.createBasicDispositionSchedule(
                                    recordCategory, GUID.generate(), GUID.generate(), true, true);
    ...
    /**
     * Given an inplace record And the collaborator has view and edit non-record capability And
     * doesn't have edit record capability When we add edit record metadata capability to the
     * extended writer role Then the collaborator now has edit record metadata capability
     */
    public void testAddUserToRole() {
        test().given()
                ...
                .perform(
                        () ->
                                filePlanRoleService.updateRole(
                                        filePlan,
    ...
    /**
     * Given an inplace record When the record is hidden Then the collaborator has no access to the
     * record And the consumer has no access to the record And a user that is not in the site has no
    ...
    /**
     * Given an inplace record When the record is rejected Then the collaborator has no access to
     * the record And the consumer has no access to the record And a user that is not in the site
     * has no permissions or capabilities
     */
    ...
    /**
     * Given a user is the cm:creator of a document And the user is deleted When the document is
     * declared as a record by a manager Then it successfully becomes a record
     */
                ...
                .perform(
                        () -> {
                            ...
                            siteService.setMembership(
                                    collabSiteId, deletedUser, SiteModel.SITE_CONTRIBUTOR);
                        })
                .as(deletedUser)
                .perform(
                        () -> {
                            deleteUserDoc =
                                    fileFolderService
                                            ...
                                            .getNodeRef();
                            dbNodeService.addAspect(
                                    deleteUserDoc, ContentModel.ASPECT_AUDITABLE, null);
                        })
                .asAdmin()
    ...
    /**
     * Given a document created by the collaborator And declared as a record by the collaborator And
     * filed by the records manager When the records manager copies the record Then the collaborator
     * has no access to the record copy And a site contributor has no access to the record copy And
     * the consumer has no access to the record copy And a user that is not in the site has no
     * access to the record copy
     */
        ...
        test().given()
                ...
                .perform(
                        () ->
                                copyDoc =
                                        fileFolderService
                                                .copy(copiedDoc, rmFolder, "newRecord.txt")
                                                .getNodeRef())
                .then()
                        ...
                        () ->
                                authorityService
                                        .getContainedAuthorities(
                                                null, "GROUP_INPLACE_RECORD_MANAGEMENT", true)
                                        .size())
    ...
    protected void tearDownImpl() {
        ...
        // clear up groups
        authorityService
                .getContainedAuthorities(null, "GROUP_INPLACE_RECORD_MANAGEMENT", true)
                .stream()

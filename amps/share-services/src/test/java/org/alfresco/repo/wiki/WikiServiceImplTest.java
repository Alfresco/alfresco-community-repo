...
public class WikiServiceImplTest {
    ...
    public void createNewEntry() throws Exception {
        ...
        ContentReader reader =
                CONTENT_SERVICE.getReader(page.getNodeRef(), ContentModel.PROP_CONTENT);
    ...
    public void createUpdateDeleteEntry() throws Exception {
        ...
        // Check it went
        assertEquals(
                null, WIKI_SERVICE.getWikiPage(WIKI_SITE.getShortName(), page.getSystemName()));
                    ...
                    public Void execute() throws Throwable {
                        ...
                        assertFalse(
                                SITE_SERVICE.hasContainer(
                                        TEST_SITE_NAME, WikiServiceImpl.WIKI_COMPONENT));

                        // Create a link
                        WIKI_SERVICE.createWikiPage(TEST_SITE_NAME, "Title", "TextTextText");
    ...
    public void pagesListing() throws Exception {
        ...
        // Add a few
        WikiPageInfo pageA =
                WIKI_SERVICE.createWikiPage(WIKI_SITE.getShortName(), "TitleA", "ContentA");
        WikiPageInfo pageB =
                WIKI_SERVICE.createWikiPage(WIKI_SITE.getShortName(), "TitleB", "ContentB");
        WikiPageInfo pageC =
                WIKI_SERVICE.createWikiPage(WIKI_SITE.getShortName(), "TitleC", "ContentC");
        ...
        AuthenticationUtil.setFullyAuthenticatedUser(ADMIN_USER);
        WikiPageInfo pageD =
                WIKI_SERVICE.createWikiPage(WIKI_SITE.getShortName(), "TitleD", "ContentD");
        ...
        // Very recent ones
        results =
                WIKI_SERVICE.listWikiPagesByCreated(
                        WIKI_SITE.getShortName(), yesterday, tomorrow, paging);
        ...
        results =
                WIKI_SERVICE.listWikiPagesByCreated(
                        WIKI_SITE.getShortName(), twoWeeksAgo, yesterday, paging);
        assertEquals(1, results.getPage().size());
        ...
        results =
                WIKI_SERVICE.listWikiPagesByCreated(
                        WIKI_SITE.getShortName(), twoWeeksAgo, tomorrow, paging);
        assertEquals(3, results.getPage().size());
        assertEquals("TitleD", results.getPage().get(0).getTitle());
        assertEquals("TitleA", results.getPage().get(1).getTitle());
        assertEquals("TitleB", results.getPage().get(2).getTitle());
        ...
        // Very recent ones
        results =
                WIKI_SERVICE.listWikiPagesByModified(
                        WIKI_SITE.getShortName(), yesterday, tomorrow, paging);
        assertEquals(2, results.getPage().size());
        assertEquals("TitleA", results.getPage().get(0).getTitle());
        assertEquals("TitleB", results.getPage().get(1).getTitle());
    ...
    public void pagesListingPermissionsChecking() throws Exception {
        ...
        TRANSACTION_HELPER.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        assertEquals(
                                true, SITE_SERVICE.isMember(WIKI_SITE.getShortName(), TEST_USER));
                        assertEquals(
                                false,
                                SITE_SERVICE.isMember(
                                        ALTERNATE_WIKI_SITE.getShortName(), TEST_USER));
                        return null;
                    }
                });
                ...
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        SITE_SERVICE.setMembership(
                                ALTERNATE_WIKI_SITE.getShortName(),
                                TEST_USER,
    ...
    private void pushAuditableDatesBack(WikiPageInfo page, int createdDaysAgo, int modifiedDaysAgo)
            throws Exception {
    ...
    private static void createTestSites() throws Exception {
        final WikiServiceImpl privateWikiPageService =
                (WikiServiceImpl) testContext.getBean("wikiService");
                        ...
                        new RetryingTransactionHelper.RetryingTransactionCallback<SiteInfo>() {
                            ...
                            public SiteInfo execute() throws Throwable {
                                SiteInfo site =
                                        SITE_SERVICE.createSite(
                                                TEST_SITE_PREFIX,
                                                WikiServiceImplTest.class.getSimpleName()
                                                        + "_testSite"
                                                        + System.currentTimeMillis(),
                                                "test site title",
                                                "test site description",
                                                SiteVisibility.PUBLIC);
                                privateWikiPageService.getSiteWikiContainer(

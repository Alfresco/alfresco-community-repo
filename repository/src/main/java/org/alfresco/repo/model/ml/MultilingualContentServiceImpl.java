...
public class MultilingualContentServiceImpl
  ...
  private static Log logger = LogFactory.getLog(
    MultilingualContentServiceImpl.class
  );
  ...
  private NodeRef getMLContainer(NodeRef mlDocumentNodeRef, boolean allowNull) {
    ...
    if (parentAssocRefs.size() == 0) {
      if (!allowNull) {
        throw new AlfrescoRuntimeException(
          "No multilingual container exists for document node: " +
          mlDocumentNodeRef
        );
      }
   ...
   */
  private NodeRef getOrCreateMLContainer(
    NodeRef mlDocumentNodeRef,
    boolean allowCreate
  ) {
    if (
    ...
    if (parentAssocRefs.size() == 0) {
      ...
      logger.warn(
        "Cleaning up multiple multilingual containers on node: " +
        mlDocumentNodeRef
      );
      ChildAssociationRef toKeepAssocRef = parentAssocRefs.get(0);
      mlContainerNodeRef = toKeepAssocRef.getParentRef();
  ...
  private NodeRef makeTranslationImpl(
    ...
    boolean forceNewVersion = nodeService.hasAspect(
      contentNodeRef,
      ContentModel.ASPECT_VERSIONABLE
    );
    ...
    if (mlContainerNodeRef == null) {
      ...
      Serializable containerFunctionalName = nodeService.getProperty(
        contentNodeRef,
      ...
      Map<Locale, NodeRef> existingLanguages =
        this.getTranslations(mlContainerNodeRef);
      if (existingLanguages.containsKey(locale)) {
        throw new AlfrescoRuntimeException(
          "Duplicate locale in document pool: " + locale
        );
      }
  ...
  private boolean isPivotTranslation(NodeRef contentNodeRef) {
    ...
    Locale containerLocale = (Locale) nodeService.getProperty(
      containerNodeRef,
      ContentModel.PROP_LOCALE
    );
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Node " + (isPivot ? "is" : "is not") + " pivot: " + contentNodeRef
      );
    }
    return isPivot;
  }
  ...
  public void makeTranslation(NodeRef contentNodeRef, Locale locale) {
    NodeRef mlContainerNodeRef = makeTranslationImpl(
      null,
      contentNodeRef,
      locale
    );
  ...
  public void deleteTranslationContainer(NodeRef mlContainerNodeRef) {
    if (
      !ContentModel.TYPE_MULTILINGUAL_CONTAINER.equals(
  ...
  public NodeRef getTranslationContainer(NodeRef translationNodeRef) {
    NodeRef mlContainerNodeRef = getOrCreateMLContainer(
      translationNodeRef,
      false
    );
    // done
  ...
  /** {@inheritDoc} */
  public NodeRef getTranslationForLocale(
  ...
  public List<Locale> getMissingTranslations(
    ...
    if (addThisNodeLocale) {
      Locale localeNode = (Locale) nodeService.getProperty(
        localizedNodeRef,
        ContentModel.PROP_LOCALE
      );
    ...
    if (foundLanguages.size() == 0) {
      ...
      missingLanguages =
        contentFilterLanguagesService.getMissingLanguages(foundLanguages);
    }
    // construct a list of locales
    List<Locale> missingLocales = new ArrayList<Locale>(
      missingLanguages.size() + 1
    );
  ...
  public NodeRef getPivotTranslation(NodeRef nodeRef) {
    ...
    if (
      ...
      containerLocale =
        (Locale) nodeService.getProperty(container, ContentModel.PROP_LOCALE);
    } else if (
      ContentModel.TYPE_MULTILINGUAL_CONTAINER.equals(
   ...
   * {@inheritDoc}
   */
  public NodeRef addEmptyTranslation(
    NodeRef translationOfNodeRef,
    String name,
    Locale locale
  ) {
    boolean hasMLAspect = nodeService.hasAspect(
      translationOfNodeRef,
      ContentModel.ASPECT_MULTILINGUAL_DOCUMENT
    );
    boolean isMLContainer = nodeService
    ...
    if (hasMLAspect || isMLContainer) {
      ...
      NodeRef pivotTranslationNodeRef = getPivotTranslation(
        translationOfNodeRef
      );
    ...
    FileInfo translationOfFileInfo = fileFolderService.getFileInfo(
      translationOfNodeRef
    );
    ...
    NodeRef parentNodeRef = nodeService
      .getPrimaryParent(translationOfNodeRef)
      .getParentRef();

    // Create the empty translation.
    // ALF-2200: Create the translation as the same type as the pivot
    QName newTranslationType = nodeService.getType(translationOfNodeRef);
    NodeRef newTranslationNodeRef = fileFolderService
      .create(parentNodeRef, name, newTranslationType)
      .getNodeRef();
  ...
  public NodeRef copyTranslationContainer(
    ...
    NodeRef spaceBefore = nodeService
    ...
    pivotCopyNodeRef =
      fileFolderService
        .copy(pivotNodeRef, newParentRef, pivotName)
        .getNodeRef();
    ...
    for (Map.Entry<Locale, NodeRef> entry : getTranslations(mlContainerNodeRef)
      .entrySet()) {
      ...
      if (!translationNodeRef.equals(pivotNodeRef)) {
        if (
          ...
          try {
            ...
            NodeRef copyNodeRef = fileFolderService
   ...
   * @inheritDoc
   */
  public void moveTranslationContainer(
    NodeRef mlContainerNodeRef,
    NodeRef newParentRef
    ...
    for (NodeRef translationToMove : getTranslations(mlContainerNodeRef)
      .values()) {
  ...
  private class MLContainerCleaner extends TransactionListenerAdapter {
    ...
    public void beforeCommit(boolean readOnly) {
      ...
      Set<NodeRef> mlContainerNodeRefs = TransactionalResourceHelper.getSet(
        KEY_ML_CONTAINERS_TO_DELETE
      );

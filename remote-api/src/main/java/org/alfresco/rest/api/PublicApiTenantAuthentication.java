...
public class PublicApiTenantAuthentication implements TenantAuthentication {
  ...
  public boolean tenantExists(final String tenant) {
    if (
      tenant == null || TenantService.DEFAULT_DOMAIN.equalsIgnoreCase(tenant)
    ) {
      return true;
    }
    ...
    return AuthenticationUtil.runAsSystem(
      new RunAsWork<Boolean>() {
        public Boolean doWork() throws Exception {
          return (
            tenantAdminService.existsTenant(tenant) &&
  ...
  public boolean authenticateTenant(String username, String networkId) {
    ...
    if (
      userNetworkId == null ||
      userNetworkId.equals(TenantService.DEFAULT_DOMAIN)
    ) {
      if (
        networkId.equalsIgnoreCase(TenantUtil.DEFAULT_TENANT) ||
        networkId.equalsIgnoreCase(TenantUtil.SYSTEM_TENANT)
      ) {
        authenticated = true;
      }
    } else {

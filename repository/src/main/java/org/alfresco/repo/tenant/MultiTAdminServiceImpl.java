...
public class MultiTAdminServiceImpl
        implements TenantAdminService, ApplicationContextAware, InitializingBean {
    // Logger
    ...
    // if set then tenant are not co-mingled and all content roots will appear below this container
    // (in <tenantdomain> sub-folder)
    ...
    public void startTenants() {
        ...
        try {
            ...
            for (Tenant tenant : tenants) {
                if ((!(isTenantRoutingContentStore(tenantFileContentStore)))
                        && (!tenantFileContentStore
                                .getRootLocation()
                                .equals(tenant.getRootContentStoreDir()))) {
                    // eg. ALF-14121 - MT will not work with
                    // replicating-content-services-context.sample if tenants are not co-mingled
                    throw new AlfrescoRuntimeException(
                            "MT: cannot start tenants - TenantRoutingContentStore is not configured"
                                    + " AND not all tenants use co-mingled content store");
                }
            ...
            if ((enabledCount + disabledCount) == 0) {
                AuthenticationUtil.setMtEnabled(
                        false); // explicitly disable if there are no tenants
            }

            if (logger.isInfoEnabled() && ((enabledCount + disabledCount) > 0)) {
                logger.info(
                        String.format(
                                "Alfresco Multi-Tenant startup - %d enabled tenants, %d disabled"
                                        + " tenants",
    ...
    public void createTenant(
            final String tenantDomain, final char[] tenantAdminRawPassword, String contentRoot) {
        createTenant(tenantDomain, tenantAdminRawPassword, contentRoot, null);
    }

    @Override
    public void createTenant(
            final String tenantDomainIn,
            final char[] tenantAdminRawPassword,
        ...
        if ((dbUrl != null) && (trds != null)) {
            try {
                // note: experimental - currently assumes a bootstrapped DB schema exists for this
                // dbUrl !
                trds.addTenantDataSource(tenantDomain, dbUrl);
            } catch (SQLException se) {
                throw new AlfrescoRuntimeException(
                        "Failed to create tenant '" + tenantDomain + "' for dbUrl '" + dbUrl + "'",
                        se);
            }
        }

        try {
            // note: runAs would cause auditable property "creator" to be "admin" instead of
            // "System@xxx"
            ...
            TenantUtil.runAsSystemTenant(
                    new TenantRunAsWork<Object>() {
                        public Object doWork() {
                            ...
                            // callback
                            RetryingTransactionCallback<Object> doImportCallback =
                                    new RetryingTransactionCallback<Object>() {
                                        public Object execute() throws Throwable {
                                            // create tenant-specific stores
                                            ImporterBootstrap userImporterBootstrap =
                                                    (ImporterBootstrap)
                                                            ctx.getBean("userBootstrap-mt");
                                            bootstrapUserTenantStore(
                                                    userImporterBootstrap,
                                                    tenantDomain,
                                                    tenantAdminRawPassword);

                                            ImporterBootstrap systemImporterBootstrap =
                                                    (ImporterBootstrap)
                                                            ctx.getBean("systemBootstrap-mt");
                                            bootstrapSystemTenantStore(
                                                    systemImporterBootstrap, tenantDomain);

                                            // deprecated
                                            ImporterBootstrap versionImporterBootstrap =
                                                    (ImporterBootstrap)
                                                            ctx.getBean("versionBootstrap-mt");
                                            bootstrapVersionTenantStore(
                                                    versionImporterBootstrap, tenantDomain);

                                            ImporterBootstrap version2ImporterBootstrap =
                                                    (ImporterBootstrap)
                                                            ctx.getBean("version2Bootstrap-mt");
                                            bootstrapVersionTenantStore(
                                                    version2ImporterBootstrap, tenantDomain);

                                            ImporterBootstrap spacesArchiveImporterBootstrap =
                                                    (ImporterBootstrap)
                                                            ctx.getBean(
                                                                    "spacesArchiveBootstrap-mt");
                                            bootstrapSpacesArchiveTenantStore(
                                                    spacesArchiveImporterBootstrap, tenantDomain);

                                            ImporterBootstrap spacesImporterBootstrap =
                                                    (ImporterBootstrap)
                                                            ctx.getBean("spacesBootstrap-mt");
                                            bootstrapSpacesTenantStore(
                                                    spacesImporterBootstrap, tenantDomain);
                                            ...
                                            */
                                            ...
                                            // bootstrap workflows
                                            for (WorkflowDeployer workflowDeployer :
                                                    workflowDeployers) {
                            ...
                            // if not default DB (ie. dbUrl != null) then run in new Spring managed
                            // txn (to ensure datasource is switched)
                            transactionService
    ...
    /**
     * Export tenant - equivalent to the tenant admin running a 'complete repo' export from the Web
     * Client Admin
     */
    @Override
    ...
    /**
     * Create tenant by restoring from a complete repository export. This is equivalent to a
     * bootstrap import using restore-context.xml.
     */
    @Override
    public void importTenant(
            final String tenantDomainIn, final File directorySource, String contentRoot) {
        final String tenantDomain = getTenantDomain(tenantDomainIn);
    ...
    public void disableTenant(String tenantDomain) {
        ...
        // update tenant attributes / tenant cache - need to disable after notifying listeners (else
        // they cannot disable)
        TenantUpdateEntity tenantUpdateEntity = tenantAdminDAO.getTenantForUpdate(tenantDomain);
        ...
        if (!existsTenant(tenantDomain)) {
            ...
            try {
                TenantUtil.runAsSystemTenant(
                        new TenantRunAsWork<Object>() {
                            public Object doWork() {
                                // Only undeploy tenant-workflows when MT-workflow deployment is
                                // enabled
                                if (workflowService.isMultiTenantWorkflowDeploymentEnabled()) {
                ...
                try {
                    ...
                    nodeService.deleteStore(
                            tenantService.getName(
                                    new StoreRef(PROTOCOL_STORE_WORKSPACE, STORE_BASE_ID_VERSION1),
                                    tenantDomain,
                                    false));
                    nodeService.deleteStore(
                            tenantService.getName(
                                    new StoreRef(PROTOCOL_STORE_WORKSPACE, STORE_BASE_ID_VERSION2),
                                    tenantDomain,
                                    false));
                    nodeService.deleteStore(
                            tenantService.getName(
    ...
    private void importBootstrapSystemTenantStore(String tenantDomain, File directorySource) {
        ...
        bootstrapView.put(
                "location", directorySource.getPath() + "/" + tenantDomain + "_system.acp");

        List<Properties> bootstrapViews = new ArrayList<Properties>(1);
        bootstrapViews.add(bootstrapView);

        ImporterBootstrap systemImporterBootstrap =
                (ImporterBootstrap) ctx.getBean("systemBootstrap");
        systemImporterBootstrap.setBootstrapViews(bootstrapViews);

        bootstrapSystemTenantStore(systemImporterBootstrap, tenantDomain);
    }

    private void bootstrapSystemTenantStore(
            ImporterBootstrap systemImporterBootstrap, String tenantDomain) {
        // Bootstrap Tenant-Specific System Store
        StoreRef bootstrapStoreRef = systemImporterBootstrap.getStoreRef();
        StoreRef tenantBootstrapStoreRef =
                new StoreRef(
                        bootstrapStoreRef.getProtocol(),
                        tenantService.getName(bootstrapStoreRef.getIdentifier(), tenantDomain));
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Bootstrapped store: "
                            + tenantService.getBaseName(tenantBootstrapStoreRef)
                            + " (Tenant: "
                            + tenantDomain
                            + ")");
        }
    ...
    private void importBootstrapUserTenantStore(String tenantDomain, File directorySource) {
        ...
        bootstrapView.put("path", "/");
        bootstrapView.put(
                "location", directorySource.getPath() + "/" + tenantDomain + "_users.acp");
    ...
    private void bootstrapUserTenantStore(
            ImporterBootstrap userImporterBootstrap,
            String tenantDomain,
        ...
        if (tenantAdminRawPassword != null) {
            ...
            props.put(
                    ...
                    passwordEncoder.encodePassword(new String(tenantAdminRawPassword), null));
            ...
            String sha256Password =
                    sha256PasswordEncoder.encodePassword(new String(tenantAdminRawPassword), salt);
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Bootstrapped store: "
                            + tenantService.getBaseName(bootstrapStoreRef)
                            + " (Tenant: "
                            + tenantDomain
                            + ")");
        }
    ...
    private void importBootstrapVersionTenantStore(String tenantDomain, File directorySource) {
        ...
        bootstrapView.put("path", "/");
        bootstrapView.put(
                "location", directorySource.getPath() + "/" + tenantDomain + "_versions2.acp");

        List<Properties> bootstrapViews = new ArrayList<Properties>(1);
        bootstrapViews.add(bootstrapView);

        ImporterBootstrap versionImporterBootstrap =
                (ImporterBootstrap) ctx.getBean("versionBootstrap");
    ...
    private void bootstrapVersionTenantStore(
            ImporterBootstrap versionImporterBootstrap, String tenantDomain) {
    ...
    private void importBootstrapSpacesArchiveTenantStore(
            String tenantDomain, File directorySource) {
        // Import Bootstrap (restore) Tenant-Specific Spaces Archive Store
        Properties bootstrapView = new Properties();
        bootstrapView.put("path", "/");
        bootstrapView.put(
                "location", directorySource.getPath() + "/" + tenantDomain + "_spaces_archive.acp");

        List<Properties> bootstrapViews = new ArrayList<Properties>(1);
        bootstrapViews.add(bootstrapView);

        ImporterBootstrap spacesArchiveImporterBootstrap =
                (ImporterBootstrap) ctx.getBean("spacesArchiveBootstrap");
    ...
    private void bootstrapSpacesArchiveTenantStore(
            ImporterBootstrap spacesArchiveImporterBootstrap, String tenantDomain) {
    ...
    private void importBootstrapSpacesModelsTenantStore(String tenantDomain, File directorySource) {
        ...
        bootstrapView.put("path", "/");
        bootstrapView.put(
                "location", directorySource.getPath() + "/" + tenantDomain + "_models.acp");

        List<Properties> bootstrapViews = new ArrayList<Properties>(1);
        bootstrapViews.add(bootstrapView);

        ImporterBootstrap spacesImporterBootstrap =
                (ImporterBootstrap) ctx.getBean("spacesBootstrap");
    ...
    private void importBootstrapSpacesTenantStore(String tenantDomain, File directorySource) {
        ...
        bootstrapView.put("path", "/");
        bootstrapView.put(
                "location", directorySource.getPath() + "/" + tenantDomain + "_spaces.acp");
    ...
    private void bootstrapSpacesTenantStore(
            ImporterBootstrap spacesImporterBootstrap, String tenantDomain) {
        ...
        UserUsageTrackingComponent userUsageTrackingComponent =
                (UserUsageTrackingComponent) ctx.getBean("userUsageTrackingComponent");
    ...
    private void initTenant(String tenantDomain, String contentRoot, String dbUrl) {
        ...
        if (contentRoot == null) {
            ...
            if (!isTenantRoutingContentStore(tenantFileContentStore)) {
                ...
                throw new AlfrescoRuntimeException(
                        "MT: cannot initialse tenant - TenantRoutingContentStore is not configured"
                            + " AND tenant is not using co-mingled content store (ie. default root"
                            + " location)");
            }
    ...
    protected void validateTenantName(String tenantDomain) {
        ...
        if (tenantDomain.length() > MAX_LEN) {
            throw new IllegalArgumentException(
                    tenantDomain
                            + " is not a valid tenant name (must be less than "
                            + MAX_LEN
                            + " characters)");
        }
        ...
        if (!Pattern.matches(REGEX_CONTAINS_ALPHA, tenantDomain)) {
            throw new IllegalArgumentException(
                    tenantDomain
                            + " is not a valid tenant name (must contain at least one alpha"
                            + " character)");
        }
        ...
        if (dnsLabels.length != 0) {
            for (int i = 0; i < dnsLabels.length; i++) {
                if (!Pattern.matches(REGEX_VALID_DNS_LABEL, dnsLabels[i])) {
                    throw new IllegalArgumentException(
                            dnsLabels[i]
                                    + " is not a valid DNS label (must match "
    ...
    private String getBaseAdminUser() {
        // default for backwards compatibility only - eg. upgrade of existing MT instance

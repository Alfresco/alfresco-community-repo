...
public class ThumbnailRegistry
  ...
  private Map<String, List<ThumbnailDefinitionLimits>> mimetypeMap = new HashMap<String, List<ThumbnailDefinitionLimits>>(
    17
  );
  ...
  public void setThumbnailDefinitions(
    final List<ThumbnailDefinition> thumbnailDefinitions
  ) {
      ...
      new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
        ...
        public Void execute() throws Throwable {
          for (String thumbnailDefName : thumbnailDefinitions.keySet()) {
            final ThumbnailDefinition thumbnailDefinition = thumbnailDefinitions.get(
              thumbnailDefName
            );
            ...
            RenditionDefinition renditionDef = thumbnailRenditionConvertor.convert(
              thumbnailDefinition,
              null
            );
  ...
  public List<ThumbnailDefinition> getThumbnailDefinitions() {
    return new ArrayList<ThumbnailDefinition>(
      this.thumbnailDefinitions.values()
    );
  }
  ...
  public List<ThumbnailDefinition> getThumbnailDefinitions(
    ...
    List<ThumbnailDefinition> result = new ArrayList<ThumbnailDefinition>(
      thumbnailDefinitionsLimitsForMimetype.size()
    );
  ...
  public boolean isThumbnailDefinitionAvailable(
    ...
    RenditionDefinition2 renditionDefinition = getEquivalentRenditionDefinition2(
      thumbnailDefinition
    );
    ...
    return AuthenticationUtil.runAs(
      new RunAsWork<Boolean>() {
        public Boolean doWork() throws Exception {
          return (
            (getThumbnailDefinitions().size() > 0) &&
            (
              redeployStaticDefsOnStartup ||
              renditionService.loadRenditionDefinitions().size() == 0
            )
          );
        }
      },
    ...
    protected void onBootstrap(ApplicationEvent event) {
      if (redeploy()) {
        ...
        if (transactionService.isReadOnly()) {
          if (logger.isDebugEnabled()) {
            logger.debug(
              "TransactionService is in read-only mode. Therefore no thumbnail definitions have been initialised."
            );
          }
          return;
        }

...
public class SiteServiceImpl
  ...
  private static final String GROUP_SITE_PREFIX =
    PermissionService.GROUP_PREFIX + SITE_PREFIX;
   ...
   */
  private static final String SITE_ADMINISTRATORS_AUTHORITY =
    "SITE_ADMINISTRATORS";
  ...
  private static final String MSG_UNABLE_TO_CREATE =
    "site_service.unable_to_create";
  private static final String MSG_SITE_SHORT_NAME_TOO_LONG =
    "site_service.short_name_too_long";
  private static final String MSG_VISIBILITY_GROUP_MISSING =
    "site_service.visibility_group_missing";
  private static final String MSG_CAN_NOT_UPDATE =
    "site_service.can_not_update";
  private static final String MSG_CAN_NOT_DELETE =
    "site_service.can_not_delete";
  private static final String MSG_CAN_NOT_REMOVE_MSHIP =
    "site_service.can_not_remove_membership";
  private static final String MSG_DO_NOT_CHANGE_MGR =
    "site_service.do_not_change_manager";
  private static final String MSG_CAN_NOT_CHANGE_MSHIP =
    "site_service.can_not_change_membership";
  private static final String MSG_SITE_CONTAINER_NOT_FOLDER =
    "site_service.site_container_not_folder";
  private static final String MSG_INVALID_SITE_TYPE =
    "site_service.invalid_site_type";
  ...
  public void setSiteNodeRefCache(
    SimpleCache<String, NodeRef> siteNodeRefCache
  ) {
   ...
   */
  public void setTransactionHelper(
  ...
  public void setSitesPermissionsCleaner(
    SitesPermissionCleaner sitesPermissionsCleaner
  ) {
  ...
  public SiteInfo createSite(
    ...
    for (String permission : permissionService.getSettablePermissions(
      siteType
    )) {
      if (
        permission.length() > longestPermissionLength
    ...
    if (getSiteGroup(shortName, true).length() > maximumPermisionGroupLength) {
      throw new SiteServiceException(
        ...
        new Object[] {
          shortName,
          maximumPermisionGroupLength - getSiteGroup("", true).length(),
        }
      );
    }
    ...
    final NodeRef siteNodeRef = AuthenticationUtil.runAs(
    ...
    Map<QName, Serializable> customProperties = getSiteCustomProperties(
      siteNodeRef
    );
    SiteInfo siteInfo = new SiteInfoImpl(
      sitePreset,
      shortName,
      title,
      description,
    ...
    AuthenticationUtil.runAs(
      new AuthenticationUtil.RunAsWork<Object>() {
        public String doWork() throws Exception {
          ...
          boolean publicGroupExists = authorityService.authorityExists(
            sitePublicGroup
          );
          if (
            !PermissionService.ALL_AUTHORITIES.equals(sitePublicGroup) &&
            !publicGroupExists &&
            !SiteVisibility.PRIVATE.equals(visibility)
          ) {
            // If the group specified in the settings does not exist, we cannot create the site.
            throw new SiteServiceException(
              MSG_VISIBILITY_GROUP_MISSING,
              new Object[] { sitePublicGroup }
            );
          }
          ...
          String siteGroup = authorityService.getName(
            AuthorityType.GROUP,
            siteGroupShortName
          );
          if (
            !ignoreExistingGroups ||
            !authorityService.authorityExists(siteGroup)
          ) {
            siteGroup =
          ...
          Set<String> permissions = permissionService.getSettablePermissions(
            siteType
          );
          for (String permission : permissions) {
            // Create a group for the permission
            String permissionGroupShortName = getSiteRoleGroup(
              shortName,
              permission,
              false
            );
            ...
            String siteRoleGroup = getSiteRoleGroup(
              shortName,
              permission,
              true
            );
          ...
          if (
            ...
            SiteVisibility.MODERATED.equals(visibility) == true &&
            permissions.contains(SITE_CONSUMER)
          ) {
          ...
          Set<String> currentManagers = authorityService.getContainedAuthorities(
            AuthorityType.USER,
   ...
   */
  private Map<QName, Serializable> getSiteCustomProperties(
    ...
    for (Map.Entry<QName, Serializable> entry : properties.entrySet()) {
      if (
        entry.getKey().getNamespaceURI().equals(SITE_CUSTOM_PROPERTY_URL) ==
        true
      ) {
  ...
  private Map<QName, Serializable> getSiteCustomProperties(
    ...
    Map<QName, Serializable> properties = directNodeService.getProperties(
      siteNodeRef
    );
   ...
   */
  public String getSiteRoleGroup(
    String shortName,
    String permission,
              ...
              return retryingTransactionHelper.doInTransaction(
                new RetryingTransactionCallback<NodeRef>() {
                  public NodeRef execute() throws Exception {
                    ...
                    NodeRef rootNodeRef = directNodeService.getRootNode(
                      SITE_STORE
                    );
  ...
  public List<SiteInfo> findSites(String filter, int size) {
    ...
    if (siteRoot == null) {
      ...
      if (filterIsPresent) {
        query.append(" AND (");
        String escNameFilter = SearchLanguageConversion.escapeLuceneQuery(
          filter.replace('"', ' ')
        );
        String[] tokenizedFilter = SearchLanguageConversion.tokenizeString(
          escNameFilter
        );
   ...
   */
  @Override
  public List<SiteInfo> findSites(
    String filter,
    ...
    if (siteRoot == null) {
      ...
      final boolean filterIsPresentAndNecessary =
        filterIsPresent && !filter.equals("*");
      ...
      if (filterIsPresentAndNecessary || sitePresetFilterIsPresent) {
        ...
        if (filterIsPresentAndNecessary) {
          ...
          query
            .append(" @cm\\:name:\"*" + escNameFilter + "*\"")
            .append(" @cm\\:title:\"" + escNameFilter + "\"")
            .append(" @cm\\:description:\"" + escNameFilter + "\"");
        }
        if (sitePresetFilterIsPresent) {
          String escPresetFilter = SearchLanguageConversion.escapeLuceneQuery(
      ...
      try {
        ...
        for (NodeRef site : results.getNodeRefs()) {
          ...
          if (
            this.dictionaryService.isSubClass(
                siteClassName,
                SiteModel.TYPE_SITE
              )
          ) {
  ...
  public List<SiteInfo> listSites(
    ...
    if (siteRoot != null) {
      ...
      PagingResults<SiteInfo> allSites = listSites(
        filterProps,
        sortProps,
        pagingRequest
      );
  ...
  public List<SiteInfo> listSites(final String userName, final int size) {
    ...
    if (
      ...
      (
        AuthenticationUtil.SYSTEM_USER_NAME.equals(
  ...
  public PagingResults<SiteInfo> listSites(
    ...
    searchTypeQNames.addAll(
      dictionaryService.getSubTypes(SiteModel.TYPE_SITE, true)
    );

    // get canned query
    final String cQBeanName = "siteGetChildrenCannedQueryFactory";
    GetChildrenCannedQueryFactory getChildrenCannedQueryFactory = (GetChildrenCannedQueryFactory) cannedQueryRegistry.getNamedObject(
      cQBeanName
    );
  ...
  private SiteInfo createSiteInfo(NodeRef siteNodeRef) {
    ...
    // Get the properties
    Map<QName, Serializable> properties =
      this.directNodeService.getProperties(siteNodeRef);
    String shortName = (String) properties.get(ContentModel.PROP_NAME);
    String sitePreset = (String) properties.get(PROP_SITE_PRESET);
    String title = DefaultTypeConverter.INSTANCE.convert(
      String.class,
      properties.get(ContentModel.PROP_TITLE)
    );
    String description = DefaultTypeConverter.INSTANCE.convert(
      String.class,
  ...
  private SiteVisibility getSiteVisibility(NodeRef siteNodeRef) {
    ...
    String visibilityValue = (String) this.directNodeService.getProperty(
        siteNodeRef,
    ...
    if (visibilityValue == null) {
      ...
      for (AccessPermission permission : permissions) {
        if (
          permission.getAuthority().equals(PermissionService.ALL_AUTHORITIES) ==
          true &&
  ...
  public String getSiteShortName(NodeRef nodeRef) {
    ...
    if (siteNodeRef != null) {
      shortName =
        (String) this.directNodeService.getProperty(
            siteNodeRef,
            ContentModel.PROP_NAME
          );
    }
   ...
   * @return NodeRef node reference
   */
  private NodeRef getSiteNodeRef(
    final String shortName,
        ...
        AuthenticationUtil.runAs(
          new AuthenticationUtil.RunAsWork<NodeRef>() {
            public NodeRef doWork() throws Exception {
              ...
              NodeRef siteNode = directNodeService.getChildByName(
                siteRoot,
                ContentModel.ASSOC_CONTAINS,
                shortName
              );
  ...
  public void updateSite(SiteInfo siteInfo) {
    ...
    if (siteNodeRef == null) {
      throw new SiteServiceException(
        MSG_CAN_NOT_UPDATE,
        new Object[] { siteInfo.getShortName() }
      );
    }
    ...
    if (currentVisibility.equals(updatedVisibility) == false) {
      ...
      if (
        SiteVisibility.PUBLIC.equals(currentVisibility) == true ||
      ...
      properties.put(
        SiteModel.PROP_SITE_VISIBILITY,
        siteInfo.getVisibility().toString()
      );
    }
  ...
  protected void listMembersImpl(
    ...
    String nameFilterLower = nameFilters.length > 0
      ? nameFilter.toLowerCase()
      : null;

    QName siteType = directNodeService.getType(siteNodeRef);
    Set<String> permissions =
      this.permissionService.getSettablePermissions(siteType);
              ...
              if (expandGroups) {
                ...
                if (nameFilter != null && nameFilter.length() != 0) {
                  ...
                  if (
                    ...
                    String displayName = authorityService.getAuthorityDisplayName(
                      authority
                    );
                    if (
                      displayName != null &&
                      displayName.toLowerCase().contains(nameFilterLower)
                    ) {
                      callback.siteMember(authority, permission, false);
                    }
                  }
  ...
  public PagingResults<SiteMembership> listMembersPaged(
    ...
    CannedQueryParameters params = getCannedQueryParameters(
      shortName,
      collapseGroups,
      sortProps,
      pagingRequest
    ...
    CannedQuery<SiteMembership> query = new SiteMembersCannedQuery(
      this,
      personService,
      nodeService,
      params
  ...
  public void onRestoreNode(ChildAssociationRef childAssocRef) {
    ...
    setupSitePermissions(
      ...
      (Map<String, Set<String>>) directNodeService.getProperty(
        siteRef,
        QName.createQName(null, "memberships")
      ),
      true
    );
  }
  ...
  protected Map<String, String> listMembersImpl(
    ...
    List<SiteMemberInfo> list = listMembersInfoImpl(
      shortName,
      nameFilter,
      roleFilter,
      size,
    ...
    for (SiteMemberInfo info : list) members.put(
      info.getMemberName(),
      info.getMemberRole()
    );
  ...
  protected List<SiteMemberInfo> listMembersInfoImpl(
              ...
              if (expandGroups) {
                ...
                if (nameFilter != null && nameFilter.length() != 0) {
                  ...
                  if (
                    ...
                    if (
                      displayName != null &&
                      matchByFilter(displayName.toLowerCase(), nameFilterLower)
                    ) {
                      members.add(
                        new SiteMemberInfoImpl(authority, permission, false)
                      );
    ...
    if ((expandGroups) && (members.size() < size)) {
      GROUP_EXPAND:for (Map.Entry<String, String> entry : groupsToExpand.entrySet()) {
        ...
        for (String subUser : subUsers) {
          if (isAcceptedName(nameFilter, subUser)) {
            SiteMemberInfo memberInfo = new SiteMemberInfoImpl(
              subUser,
              entry.getValue(),
              true
            );
            ...
            if (members.contains(memberInfo) == false) {
              ...
              int priority = roleComparator.compare(
  ...
  private boolean isAcceptedName(String nameFilter, String name) {
    boolean addUser = true;
    if (
      nameFilter != null && nameFilter.length() != 0 && !nameFilter.equals(name)
    ) {
      // found a filter - does it match person first/last name?
  ...
  private boolean matchPerson(
    ...
    try {
      ...
      String firstName = (String) directNodeService.getProperty(
        person,
        ContentModel.PROP_FIRSTNAME
      );
      String lastName = (String) directNodeService.getProperty(
        person,
        ContentModel.PROP_LASTNAME
      );
      String userName = (String) directNodeService.getProperty(
        person,
        ContentModel.PROP_USERNAME
      );

      final String lowFirstName =
        (firstName != null ? firstName.toLowerCase() : "");
      final String lowLastName =
        (lastName != null ? lastName.toLowerCase() : "");
      final String lowUserName =
        (userName != null ? userName.toLowerCase() : "");
      for (int i = 0; i < nameFilters.length; i++) {
        if (
          matchByFilter(lowUserName, nameFilters[i]) ||
   ...
   */
  public SiteMemberInfo getMembersRoleInfo(
    String shortName,
    ...
    Set<String> userAuthoritySet =
      this.authorityService.getAuthoritiesForUser(authorityName);
  ...
  public List<String> getMembersRoles(String shortName, String authorityName) {
    ...
    for (String group : groups) {
      ...
      int index =
        group.lastIndexOf(shortNamePattern) + (shortNamePattern).length();
      if (index != -1) {
   ...
   */
  private List<String> getPermissionGroups(
  ...
  public void removeMembership(
    ...
    if (role != null) {
      ...
      if (
        (currentUserName.equals(authorityName) == true) ||
        ...
        if (authorityType == AuthorityType.USER) {
          ...
          String authorityDisplayName = authorityService.getAuthorityDisplayName(
            authorityName
          );
          activityService.postActivity(
   ...
   */
  @Override
  public void cleanSitePermissions(
    final NodeRef targetNode,
    SiteInfo containingSite
  ) {
    this.sitesPermissionsCleaner.cleanSitePermissions(
        targetNode,
        containingSite
      );
  }
   ...
   */
  private void setModeratedPermissions(
    String shortName,
  ...
  @SuppressWarnings("unchecked")
  public PagingResults<FileInfo> listContainers(
    String shortName,
    PagingRequest pagingRequest
  ) {
    SiteContainersCannedQueryFactory sitesContainersCannedQueryFactory = (SiteContainersCannedQueryFactory) cannedQueryRegistry.getNamedObject(
      "siteContainersCannedQueryFactory"
    );

    CannedQueryPageDetails pageDetails = new CannedQueryPageDetails(
      pagingRequest.getSkipCount(),
      pagingRequest.getMaxItems()
    );
    CannedQuerySortDetails sortDetails = new CannedQuerySortDetails(
      new Pair<Object, SortOrder>(
        SiteContainersCannedQueryParams.SortFields.ContainerName,
        SortOrder.ASCENDING
      )
    );
    SiteContainersCannedQueryParams parameterBean = new SiteContainersCannedQueryParams(
      getSiteNodeRef(shortName)
    );
    ...
    CannedQuery<FileInfo> query = sitesContainersCannedQueryFactory.getCannedQuery(
      params
    );
  ...
  public boolean hasContainer(
    ...
    NodeRef containerRef = AuthenticationUtil.runAs(
      new RunAsWork<NodeRef>() {
        public NodeRef doWork() throws Exception {
          return retryingTransactionHelper.doInTransaction(
            new RetryingTransactionCallback<NodeRef>() {
  ...
  public static NodeRef getSiteContainer(
    ...
    if (!siteService.hasContainer(siteShortName, componentName)) {
      if (create) {
        ...
        NodeRef container = AuthenticationUtil.runAs(
          new AuthenticationUtil.RunAsWork<NodeRef>() {
            public NodeRef doWork() throws Exception {
  ...
  private String getActivityUserData(String userName, String role) {
    ...
    if (person != null) {
      memberFN =
        (String) directNodeService.getProperty(
          person,
          ContentModel.PROP_FIRSTNAME
        );
      memberLN =
    ...
    try {
      ...
      activityData.put(
        "title",
        (memberFN + " " + memberLN + " (" + userName + ")").trim()
      );
  ...
  private void checkLastManagerRemoval(
    ...
    if (SiteModel.SITE_MANAGER.equals(role) == true) {
      int siteAuthorities = countAuthoritiesWithRole(
        shortName,
        SiteModel.SITE_MANAGER
      );
      if (siteAuthorities <= 1) {
        throw new SiteServiceException(
          MSG_DO_NOT_CHANGE_MGR,
  ...
  public List<SiteMembership> listSiteMemberships(String userName, int size) {
    ...
    Set<String> containingAuthorities = authorityService.getContainingAuthorities(
      AuthorityType.GROUP,
      actualUserName,
      false
    );
    ...
    boolean isMe = authenticationContext
      .getCurrentUserName()
    ...
    for (NodeRef site : siteNodes) {
      ...
      if (
        this.dictionaryService.isSubClass(siteClassName, SiteModel.TYPE_SITE)
      ) {
        if (checkAccess) {
   ...
   */
  private List<ChildAssociationRef> getSitesAssocsByName(
  ...
  public PagingResults<SiteMembership> listSitesPaged(
    ...
    final PageDetails pageDetails = PageDetails.getPageDetails(
      pagingRequest,
      totalSize
    );
  ...
  private <T extends Object> PagingResults<T> getPagingResults(
    PagingRequest pagingRequest,
    final CannedQueryResults<T> results
  ) {
   ...
   * site which has consumer role for everyone.
   */
  private void setPrivatePermissions(
    String shortName,
    NodeRef containerNodeRef
  ) {
    setNonPublicSitePermissions(shortName, containerNodeRef);
  ...
  private void setNonPublicSitePermissions(
    String shortName,
    NodeRef containerNodeRef
  ) {
    NodeRef siteNodeRef = getSiteNodeRef(shortName);
  ...
  public boolean isSiteAdmin(String userName) {
    ...
    return (
      this.authorityService.isAdminAuthority(userName) ||
      this.authorityService.getAuthoritiesForUser(userName)
  ...
  public PagingResults<SiteGroupMembership> listGroupMembersPaged(
    ...
    CannedQuery<SiteGroupMembership> query = new SiteGroupCannedQuery(
      this,
      authorityService,
      params
    );
  ...
  private CannedQueryParameters getCannedQueryParameters(
    ...
    List<Pair<? extends Object, SortOrder>> sortPairs = sortProps
      .stream()
      .map(props ->
        new Pair<SortFields, SortOrder>(
          props.getFirst(),
          (props.getSecond() ? SortOrder.ASCENDING : SortOrder.DESCENDING)
        )
      )
      .collect(Collectors.toList());
    CannedQuerySortDetails sortDetails = new CannedQuerySortDetails(sortPairs);

    SiteMembersCannedQueryParams parameterBean = new SiteMembersCannedQueryParams(
      shortName,
      expandGroups
    );

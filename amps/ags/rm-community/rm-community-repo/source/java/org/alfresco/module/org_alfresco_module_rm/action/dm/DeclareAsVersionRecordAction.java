...
public class DeclareAsVersionRecordAction extends AuditableActionExecuterAbstractBase
    ...
    protected void executeImpl(final Action action, final NodeRef actionedUponNodeRef) {
        if (!nodeService.exists(actionedUponNodeRef)) {
            ...
            if (logger.isDebugEnabled()) {
                ...
                nodeService.getType(actionedUponNodeRef), ContentModel.TYPE_CONTENT)) {
            // TODO eventually we should support other types .. either as record folders or as
            // composite records
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Can not declare version as record, because "
            ...
            if (filePlan == null) {
                filePlan =
                        RecordActionUtils.getDefaultFilePlan(
                                authenticationUtil, filePlanService, NAME);
            }
            // verify that the provided file plan is actually a file plan
            else if (!filePlanService.isFilePlan(filePlan)) {
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Can not declare version record, because the provided file plan node"
                                    + " reference is not a file plan.");
                }
            ...
            if (pathParameter != null && !pathParameter.isEmpty()) {
                RecordActionUtils.Services services =
                        new Services(nodeService, filePlanService, authenticationUtil);
                destinationRecordFolder =
                        RecordActionUtils.resolvePath(services, filePlan, pathParameter, NAME);
            }
            ...
            if (destinationRecordFolder != null) {
                boolean hasCapability =
                        capabilityService.hasCapability(
                                destinationRecordFolder, FILE_VERSION_RECORDS_CAPABILITY);
                // validate destination record folder
                if (hasCapability) {
                    NodeRef recordedVersion =
                            recordableVersionService.createRecordFromLatestVersion(
                                    destinationRecordFolder, actionedUponNodeRef);
    ...
    protected void addParameterDefinitions(List<ParameterDefinition> params) {
        // NOTE:  commented out for now so that it doesn't appear in the UI ... enable later when
        // multi-file plan support is added
        // params.add(new ParameterDefinitionImpl(PARAM_FILE_PLAN, DataTypeDefinition.NODE_REF,
        // false, getParamDisplayLabel(PARAM_FILE_PLAN)));
        params.add(

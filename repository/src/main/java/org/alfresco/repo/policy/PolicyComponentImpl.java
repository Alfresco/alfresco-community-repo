...
public class PolicyComponentImpl implements PolicyComponent {
  ...
  public <P extends ClassPolicy> ClassPolicyDelegate<P> registerClassPolicy(
    ...
    registeredPolicies.put(
      new PolicyKey(definition.getType(), definition.getName()),
      definition
    );
    ClassPolicyDelegate<P> delegate = new ClassPolicyDelegate<P>(
      dictionary,
      policy,
  ...
  public void removeClassDefinition(
    ...
    getClassBehaviourIndex(definition.getPolicy())
      .removeClassBehaviour(definition);
  }
  ...
  public <P extends PropertyPolicy> PropertyPolicyDelegate<P> registerPropertyPolicy(
    ...
    PropertyPolicyDelegate<P> delegate = new PropertyPolicyDelegate<P>(
      dictionary,
      policy,
  ...
  public <P extends AssociationPolicy> AssociationPolicyDelegate<P> registerAssociationPolicy(
    ...
    AssociationPolicyDelegate<P> delegate = new AssociationPolicyDelegate<P>(
      dictionary,
      policy,
   ...
   */
  public PolicyDefinition getRegisteredPolicy(
  ...
  public BehaviourDefinition<ClassBehaviourBinding> bindClassBehaviour(
    ...
    ClassBehaviourBinding binding = new ClassBehaviourBinding(
      dictionary,
      classRef
    );
    BehaviourDefinition<ClassBehaviourBinding> definition = createBehaviourDefinition(
      PolicyType.Class,
      policy,
      binding,
      behaviour
    ...
    if (logger.isInfoEnabled()) logger.info(
      "Bound " + behaviour + " to policy " + policy + " for class " + classRef
    );
  ...
  public BehaviourDefinition<ServiceBehaviourBinding> bindClassBehaviour(
    ...
    BehaviourDefinition<ServiceBehaviourBinding> definition = createBehaviourDefinition(
      PolicyType.Class,
      policy,
      binding,
      behaviour
    ...
    if (logger.isInfoEnabled()) logger.info(
      "Bound " + behaviour + " to policy " + policy + " for service " + service
    );
  ...
  public BehaviourDefinition<ClassFeatureBehaviourBinding> bindPropertyBehaviour(
    ...
    // Validate Binding
    PropertyDefinition propertyDefinition = dictionary.getProperty(
      className,
      propertyName
    );
    ...
    // Create behaviour definition and bind to policy
    ClassFeatureBehaviourBinding binding = new ClassFeatureBehaviourBinding(
      dictionary,
      className,
      propertyName
    );
    BehaviourDefinition<ClassFeatureBehaviourBinding> definition = createBehaviourDefinition(
      PolicyType.Property,
      policy,
      binding,
      behaviour
  ...
  public BehaviourDefinition<ClassFeatureBehaviourBinding> bindAssociationBehaviour(
    ...
    // Validate Binding
    AssociationDefinition assocDefinition = dictionary.getAssociation(
      assocName
    );
  ...
  private synchronized ClassBehaviourIndex<ClassBehaviourBinding> getClassBehaviourIndex(
    ...
    ClassBehaviourIndex<ClassBehaviourBinding> index = classBehaviours.get(
      policy
    );
    if (index == null) {
   ...
   */
  private synchronized ClassBehaviourIndex<ClassFeatureBehaviourBinding> getPropertyBehaviourIndex(
    QName policy
  ) {
    ClassBehaviourIndex<ClassFeatureBehaviourBinding> index = propertyBehaviours.get(
      policy
    );
    if (index == null) {
      index =
        new ClassBehaviourIndex<ClassFeatureBehaviourBinding>(behaviourFilter);
      index.setTryLockTimeout(tryLockTimeout);
   ...
   */
  private synchronized ClassBehaviourIndex<ClassFeatureBehaviourBinding> getAssociationBehaviourIndex(
    QName policy
  ) {
    ClassBehaviourIndex<ClassFeatureBehaviourBinding> index = associationBehaviours.get(
      policy
    );
    if (index == null) {
      index =
  ...
  private PolicyDefinition createPolicyDefinition(Class policyIF) {
    ...
    return new PolicyDefinitionImpl(
      QName.createQName(namespaceURI, name),
      policyIF,
      args
    );
  }
  ...
  /*package*/class PolicyDefinitionImpl implements PolicyDefinition {
    ...
    public Arg getArgument(int index) {
      if (index < 0 || index > args.length - 1) {
        throw new IllegalArgumentException(
          "Argument index " + index + " is invalid"
        );
      }
      return args[index];
    }

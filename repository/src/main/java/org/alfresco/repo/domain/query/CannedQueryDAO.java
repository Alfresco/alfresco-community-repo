...
/**
 * DAO services for general-use database canned queries. Note that this is specifically targeted at
 * low-level queries to the persistance layer.
 *
 * @author Derek Hulley
 * @since 3.5
 */
public interface CannedQueryDAO {
    ...
    /**
     * Execute a <b>count(*)</b>-style query returning a count value. The implementation will ensure
     * that <tt>null</tt> is substituted with <tt>0</tt>, if required.
     *
     * <p>All exceptions can be safely caught and handled as required.
     *
     * @param sqlNamespace the query namespace (defined by config file) e.g.
     *     <b>alfresco.query.usage</b>
    ...
    /**
     * Execute a query that returns exactly one result. The assumption is that the parameters
     * provided uniquely identify the object.
     *
     ...
     * @throws IllegalArgumentException concurrency-related exception if a single object was not
     *     found
     */
     ...
     * @param limit the maximum number of results to retrieve or <code>Integer.MAX_VALUE</code> for
     *     no limit
    ...
    public interface ResultHandler<R> {
        /**
         * Allow implementations to process a result. Note that the interface contract will be met,
         * but internally the querying mechanism <i>might not</i> be able to optimise out all result
         * fetching.
         *
         * @return <tt>true</tt> if more results are required or <tt>false</tt> to terminate result
         *     fetching.
         */

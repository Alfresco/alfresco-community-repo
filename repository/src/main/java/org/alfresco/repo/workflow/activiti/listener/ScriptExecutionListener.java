 ...
 *
 * <p>The script that is executed can be set using field 'script'. A non-default script-processor
 * can be set in the field 'scriptProcessor'. Optionally, you can run the script as a different user
 * than the default by setting the field 'runAs'. By default, the user this script as the current
 * logged-in user. If no user is currently logged in (eg. flow triggered by timer) the system user
 ...
 * @since 3.4.e
 */
public class ScriptExecutionListener extends DelegateExecutionScriptBase
    ...
    protected Map<String, Object> getInputMap(DelegateExecution execution, String runAsUser) {
        ...
        if (ActivitiConstants.DELETE_REASON_DELETED.equals(listenerExecution.getDeleteReason())) {
            deleted = true;
        } else if (ActivitiConstants.DELETE_REASON_CANCELLED.equals(
                listenerExecution.getDeleteReason())) {
            cancelled = true;
        }

...
public class SiteGroupCannedQuery
  ...
  @Override
  protected List<SiteGroupMembership> queryAndFilter(
    CannedQueryParameters parameters
  ) {
    ...
    final CQSiteGroupsCallback callback = new CQSiteGroupsCallback(
      siteShortName,
      sortDetails.getSortPairs()
    );
  ...
  private class CQSiteGroupsCallback implements SiteMembersCallback {
    ...
    public void siteMember(String authority, String role) {
      if (authorityService.authorityExists(authority)) {
        ...
        siteGroups.add(
          new SiteGroupMembership(siteInfo, authority, role, displayName)
        );
      }
    }

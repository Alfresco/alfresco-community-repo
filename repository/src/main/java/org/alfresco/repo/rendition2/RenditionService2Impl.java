...
public class RenditionService2Impl
    ...
    // As Async transforms and renditions are so similar, this class provides a way to provide the
    ...
    public void setRenditionPreventionRegistry(
    ...
    public void transform(NodeRef sourceNodeRef, TransformDefinition transformDefinition) {
        requestAsyncTransformOrRendition(
                sourceNodeRef,
                new RenderOrTransformCallBack() {
                    @Override
                    ...
                    public RenditionDefinition2 getRenditionDefinition() {
                        ...
                        if (renditionDefinition == null) {
                            throw new IllegalArgumentException(
                                    getName() + " has not been registered.");
                        }
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    getName()
                                            ...
                                            + " rendition "
                                            + renditionContentHashCode
                                            + " hashCodes");
                        }
                        if (renditionContentHashCode == sourceContentHashCode) {
                            throw new IllegalStateException(
                                    getName() + " has already been created.");
                        }
                    }
                });
    }

    private void requestAsyncTransformOrRendition(
            NodeRef sourceNodeRef, RenderOrTransformCallBack renderOrTransform) {
        try {
            if (!isEnabled()) {
                throw new RenditionService2Exception(
                        "Async transforms and renditions are disabled"
                                + " (system.thumbnail.generate=false or"
                                + " renditionService2.enabled=false).");
            }

            if (!nodeService.exists(sourceNodeRef)) {
                throw new IllegalArgumentException(
                        renderOrTransform.getName()
                                + ": The supplied sourceNodeRef "
                                + sourceNodeRef
                                + " does not exist.");
            }
            ...
            ContentData contentData =
                    (ContentData) nodeService.getProperty(sourceNodeRef, ContentModel.PROP_CONTENT);
            if (contentData != null && contentData.getContentUrl() != null) {
                ...
                try {
                    transformClient.checkSupported(
                            sourceNodeRef, renditionDefinition, sourceMimetype, size, contentUrl);
                    ...
                    () -> {
                        ...
                        if (!supported.get()) {
                            if (logger.isDebugEnabled()) {
                                logger.debug(
                                        renderOrTransform.getName()
                                                + " is not supported. The content might be too big"
                                                + " or the source mimetype cannot be converted.");
                            }
                            failure(sourceNodeRef, renditionDefinition, sourceContentHashCode);
                        } else {
                            renderOrTransform.throwIllegalStateExceptionIfAlreadyDone(
                                    sourceContentHashCode);
    ...
    private void consumeExtractedMetadata(
            NodeRef nodeRef,
        ...
        if (transformInputStream == null) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Ignore transform for metadata extraction on " + nodeRef + " as it failed");
            }
        } else if (transformContentHashCode != sourceContentHashCode) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Ignore transform for metadata extraction on "
                                + nodeRef
    ...
    private void consumeEmbeddedMetadata(
            NodeRef nodeRef,
            int sourceContentHashCode,
            InputStream transformInputStream,
            TransformDefinition transformDefinition,
        ...
        if (transformInputStream == null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Ignore transform for metadata embed on "
                                + nodeRef
                                + " as it is no longer needed");
            }
        } else {
    ...
    private void consumeTransformReply(
            NodeRef sourceNodeRef,
            InputStream transformInputStream,
            TransformDefinition transformDefinition,
            int transformContentHashCode) {
        ...
        transformReplyProvider.produceTransformEvent(
                sourceNodeRef, transformInputStream, transformDefinition, transformContentHashCode);
    }

    /**
     * Takes a transformation (InputStream) and attaches it as a rendition to the source node. Does
     * nothing if there is already a newer rendition. If the transformInputStream is null, this is
     * taken to be a transform failure.
     */
    private void consumeRendition(
            NodeRef sourceNodeRef,
            int sourceContentHashCode,
            InputStream transformInputStream,
            RenditionDefinition2 renditionDefinition,
        ...
        if (transformContentHashCode != sourceContentHashCode) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Ignore transform for rendition "
                                + renditionName
                                + " on "
                                + sourceNodeRef
                                + " as it is no longer needed");
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Set the content of rendition "
                                + renditionName
                                + " on "
                                + sourceNodeRef
                                + (transformInputStream == null
                                        ? " to null as the transform failed"
                                        : " to the transform result"));
            }
                    ...
                    (AuthenticationUtil.RunAsWork<Void>)
                            () ->
                                    transactionService
                                            ...
                                            .doInTransaction(
                                                    () -> {
                                                        // Ensure that the creation of a rendition
                                                        // does not cause updates to the modified,
                                                        // modifier properties on the source node
                                                        NodeRef renditionNode =
                                                                getRenditionNode(
                                                                        sourceNodeRef,
                                                                        renditionName);
                                                        boolean createRenditionNode =
                                                                renditionNode == null;
                                                        boolean sourceHasAspectRenditioned =
                                                                nodeService.hasAspect(
                                                                        sourceNodeRef,
                                                                        RenditionModel
                                                                                .ASPECT_RENDITIONED);
                                                        boolean sourceChanges =
                                                                !sourceHasAspectRenditioned
                                                                        || createRenditionNode
                                                                        || transformInputStream
                                                                                == null;
                                                        try {
                                                            if (sourceChanges) {
                                                                ruleService.disableRuleType(
                                                                        RuleType.UPDATE);
                                                            ...
                                                            // If they do not exist create the
                                                            // rendition association and the
                                                            // rendition node.
                                                            ...
                                                            if (logger.isDebugEnabled()) {
                                                                logger.debug(
                                                                        "Set rendition hashcode "
                                                                                + transformContentHashCode
                                                                                + " and ThumbnailLastModified"
                                                                                + " for "
                                                                                + renditionName);
                                                            }
                                                            nodeService.setProperty(
                                                                    ...
                                                                    transformContentHashCode);
                                                            setThumbnailLastModified(
                                                                    sourceNodeRef, renditionName);

                                                            if (transformInputStream != null) {
                                                                try {
                                                                    // Set or replace rendition
                                                                    // content
                                                                    ...
                                                                    String targetMimetype =
                                                                            renditionDefinition
                                                                                    .getTargetMimetype();
                                                                    contentWriter.setMimetype(
                                                                            targetMimetype);
                                                                    contentWriter.setEncoding(
                                                                            DEFAULT_ENCODING);
                                                                    ContentWriter renditionWriter =
                                                                            contentWriter;
                                                                    renditionWriter.putContent(
                                                                            transformInputStream);
                                                                } catch (Exception e) {
                                                                    logger.error(
                                                                            "Failed to copy"
    ...
    private NodeRef createRenditionNode(
            NodeRef sourceNode, RenditionDefinition2 renditionDefinition) {
        String renditionName = renditionDefinition.getRenditionName();
        ...
        ChildAssociationRef childAssoc =
                nodeService.createNode(sourceNode, assocType, assocName, nodeType, nodeProps);
    ...
    // Based on code from
    // org.alfresco.repo.thumbnail.ThumbnailServiceImpl.addThumbnailModificationData
    private void setThumbnailLastModified(NodeRef sourceNodeRef, String renditionName) {
        ...
        if (nodeService.hasAspect(sourceNodeRef, ContentModel.ASPECT_THUMBNAIL_MODIFICATION)) {
            ...
            properties.put(
                    ...
                    (Serializable) thumbnailMods);
            nodeService.addAspect(
                    sourceNodeRef, ContentModel.ASPECT_THUMBNAIL_MODIFICATION, properties);
        }
    }

    /**
     * Returns the hash code of the source node's content url. As transformations may be returned in
     * a different sequences to which they were requested, this is used work out if a rendition
     * should be replaced.
     */
    private int getSourceContentHashCode(NodeRef sourceNodeRef) {
        ...
        if (contentData != null) {
            // Originally we used the contentData URL, but that is not enough if the mimetype
            // changes.
    ...
    /**
     * Returns the hash code of source node's content url on the rendition node (node may be null)
     * if it does not exist. Used work out if a rendition should be replaced. {@code -2} is returned
     * if the rendition does not exist or was not created by RenditionService2. {@code -1} is
     * returned if there was no source content or the rendition failed.
     */
    private int getRenditionContentHashCode(NodeRef renditionNode) {
        ...
        Serializable hashCode =
                nodeService.getProperty(renditionNode, PROP_RENDITION_CONTENT_HASH_CODE);
        return hashCode == null ? SOURCE_HAS_NO_CONTENT : (int) hashCode;
    }
    ...
    private NodeRef getRenditionNode(NodeRef sourceNodeRef, String renditionName) {
        ...
        List<ChildAssociationRef> renditionAssocs =
                nodeService.getChildAssocs(
                        sourceNodeRef, RenditionModel.ASSOC_RENDITION, renditionQName);
    ...
    // Only called by the old service, so that it can take over. Normally RenditionService2 just
    // updates
    // rendition nodes to new versions, or to mark them as failed. The older service creates new
    // nodes.
    ...
    /**
     * This method checks whether the specified source node is of a content class which has been
     * registered for rendition prevention.
     *
     * @param sourceNode the node to check.
     * @throws RenditionService2PreventedException if the source node is configured for rendition
     *     prevention.
     */
            ...
            for (QName contentClass : nodeContentClasses) {
                if (renditionPreventionRegistry.isContentClassRegistered(contentClass)) {
                    String msg =
                            "Node "
                                    + sourceNode
                                    + " cannot be renditioned as it is of class "
                                    + contentClass;
                    logger.debug(msg);
    ...
    @Override
    // Only returns valid renditions. These may be from RenditionService2 or original
    // RenditionService.
    ...
    /**
     * Indicates if the rendition is available. Failed renditions (there was an error) don't have a
     * contentUrl and out of date renditions or those still being created don't have a matching
     * contentHashCode.
     */
    public boolean isRenditionAvailable(NodeRef sourceNodeRef, NodeRef renditionNode) {
        boolean available = true;
        if (nodeService.hasAspect(renditionNode, RenditionModel.ASPECT_RENDITION2)) {
            Serializable contentUrl =
                    nodeService.getProperty(renditionNode, ContentModel.PROP_CONTENT);
            if (contentUrl == null) {
                available = false;
            } else {
                int sourceContentHashCode = getSourceContentHashCode(sourceNodeRef);
    ...
    @Override
    // Only returns a valid renditions. This may be from RenditionService2 or original
    // RenditionService.
    public ChildAssociationRef getRenditionByName(NodeRef sourceNodeRef, String renditionName) {
        // Based on code from the original RenditionService. renditionName is a String rather than a
        // QName.
        ...
        if (isEnabled()) {
            if (nodeService.exists(sourceNodeRef)) {
                ...
                List<ChildAssociationRef> childAssocs =
                        getRenditionChildAssociations(sourceNodeRef);
                for (ChildAssociationRef childAssoc : childAssocs) {
                    NodeRef renditionNodeRef = childAssoc.getChildRef();
                    // TODO: This check will not be needed once the original RenditionService is
                    // removed.
                    if (nodeService.hasAspect(renditionNodeRef, RenditionModel.ASPECT_RENDITION2)) {
                        ...
                        if (renditionDefinition != null) {
                            ...
                            logger.debug(
                                    "onContentUpdate rendition "
                                            + renditionName
                                            + " only exists in the original rendition service.");
                        }
                    }
                }
            } else {

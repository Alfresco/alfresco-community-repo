...
public class CalendarEntryImpl extends CalendarEntryDTO {
    ...
    protected void recordStorageDetails(
            NodeRef nodeRef, NodeRef containerNodeRef, String systemName) {
        this.nodeRef = nodeRef;
        this.systemName = systemName;
        this.containerNodeRef = containerNodeRef;
    }
    ...
    /**
     * Builds up the node properties for a given Calendar Entry Doesn't handle tags, those must be
     * done in the service itself.
     */
    ...
    /**
     * Populates a Calendar Entry from the given node properties. Doesn't handle tags, those must be
     * done in the service itself.
     */
        ...
        if (entry instanceof CalendarEntryDTO) {
            if (properties.containsKey(ContentModel.PROP_CREATED)) {
                ((CalendarEntryDTO) entry)
                        .setCreatedAt((Date) properties.get(ContentModel.PROP_CREATED));
            }
            if (properties.containsKey(ContentModel.PROP_MODIFIED)) {
                ((CalendarEntryDTO) entry)
                        .setModifiedAt((Date) properties.get(ContentModel.PROP_MODIFIED));
            }
        }
    }

    /**
     * Populates this entry from the given node properties Doesn't handle tags, those are done in
     * the service itself.
     */

...
public class FormRestApiJsonPost_Test extends AbstractTestFormRestApi {
  ...
  private static final String ASSOC_CM_REFERENCES_ADDED =
    "assoc_cm_references_added";
  private static final String ASSOC_CM_REFERENCES_REMOVED =
    "assoc_cm_references_removed";
  private static final String ASSOC_SYS_CHILDREN = "assoc_sys_children";
  private static final String ASSOC_SYS_CHILDREN_ADDED =
    "assoc_sys_children_added";
  private static final String ASSOC_SYS_CHILDREN_REMOVED =
    "assoc_sys_children_removed";

  public void testSimpleJsonPostRequest() throws IOException, JSONException {
    // Retrieve and store the original property value.
    Serializable originalDescription = nodeService.getProperty(
    ...
    Serializable modifiedDescription = nodeService.getProperty(
      referencingDocNodeRef,
      ContentModel.PROP_DESCRIPTION
    );
  ...
  public void testAddNewAssociationsToNode() throws Exception {
    ...
    JSONObject jsonPostData = new JSONObject();
    String assocsToAdd =
      associatedDoc_C + "," + associatedDoc_D + "," + associatedDoc_E;
    ...
    List<AssociationRef> modifiedAssocs = nodeService.getTargetAssocs(
      referencingDocNodeRef,
  ...
  public void testAddNewChildAssociationsToNode() throws Exception {
    ...
    sendRequest(
      new PostRequest(
        containingNodeUpdateUrl.toString(),
        jsonPostString,
        APPLICATION_JSON
      ),
      200
    ...
    List<ChildAssociationRef> modifiedAssocs = nodeService.getChildAssocs(
      containerNodeRef
    );
    assertEquals(5, modifiedAssocs.size());
  ...
  private void checkOriginalAssocsBeforeChanges() {
    List<AssociationRef> originalAssocs = nodeService.getTargetAssocs(
      referencingDocNodeRef,
      RegexQNamePattern.MATCH_ALL
    );
  ...
  private void checkOriginalChildAssocsBeforeChanges() {
    List<ChildAssociationRef> originalChildAssocs = nodeService.getChildAssocs(
      containerNodeRef
    );

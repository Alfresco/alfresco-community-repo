...
public class RangeResultMapper {
   ...
   */
  public static List<GenericFacetResponse> getGenericFacetsForRanges(
    Map<String, List<Map<String, String>>> facetFields,
    ...
    if (facetFields != null && !facetFields.isEmpty() && ranges != null) {
      for (Entry<String, List<Map<String, String>>> facet : facetFields.entrySet()) {
        ...
        ffcs.add(
          new GenericFacetResponse(FACET_TYPE.range, facet.getKey(), buckets)
        );
      }
    }
    return ffcs;
   ...
   * @return
   */
  public static GenericBucket buildGenericBucketFromRange(
    String facetField,
    ...
    facet.put(
      GenericFacetResponse.START_INC.toString(),
      Boolean.toString(startInclusive)
    );
    facet.put(
      GenericFacetResponse.END_INC.toString(),
      Boolean.toString(endInclusive)
    );
    ...
    Set<Metric> metrics = new HashSet<Metric>(
      Arrays.asList(
        new SimpleMetric(
          METRIC_TYPE.count,
          facet.get(GenericFacetResponse.COUNT)
        )
      )
    );
    facet.remove("count");

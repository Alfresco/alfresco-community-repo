...
public class MimetypeMapTest extends TestCase {
  ...
  public void testExtensions() throws Exception {
    ...
    assertEquals(
      "sds",
      extensionsByMimetype.get("application/vnd.stardivision.chart")
    );
  }
  ...
  public void testMimetypeFromExtension() throws Exception {
    ...
    assertEquals(
      ...
      mimetypeService.getMimetype(null)
    );
    assertEquals(
      MimetypeMap.MIMETYPE_BINARY,
      mimetypeService.getMimetype("unknownext")
    );
  }
  ...
  public void testGuessMimetypeForFilename() throws Exception {
    assertEquals(
      "application/msword",
      mimetypeService.guessMimetype("something.doc")
    );
    assertEquals(
      "application/msword",
      mimetypeService.guessMimetype("SOMETHING.DOC")
    ...
    assertEquals(
      MimetypeMap.MIMETYPE_BINARY,
      mimetypeService.guessMimetype("noextension")
    );
    assertEquals(
      MimetypeMap.MIMETYPE_BINARY,
      mimetypeService.guessMimetype("file.unknownext")
    );

    // Without a content reader, the behaviour is the same
    assertEquals(
      "application/msword",
      mimetypeService.guessMimetype("something.doc", (ContentReader) null)
    );
    assertEquals(
      "application/msword",
      mimetypeService.guessMimetype("SOMETHING.DOC", (ContentReader) null)
    ...
    assertEquals(
      MimetypeMap.MIMETYPE_BINARY,
      mimetypeService.guessMimetype("noextension", (ContentReader) null)
    );
    assertEquals(
      MimetypeMap.MIMETYPE_BINARY,
      mimetypeService.guessMimetype("file.unknownext", (ContentReader) null)
    );
  }
  ...
  public void testNoDuplicates() throws Exception {
    setConfigService(MIMETYPE_1A + MIMETYPE_2A + MIMETYPE_3A);
    ((MimetypeMap) mimetypeService).init();

    assertFalse(
      "mimetype1 should not be text",
      mimetypeService.isText("mimetype1")
    );
    assertEquals("ext1a", mimetypeService.getExtension("mimetype1"));
    assertEquals("mimetype1", mimetypeService.getMimetype("ext1a"));
    assertEquals(
      "Mimetype ONE",
      mimetypeService.getDisplaysByMimetype().get("mimetype1")
    );
    assertEquals(
      "Extension ONE",
      mimetypeService.getDisplaysByExtension().get("ext1a")
    );
  ...
  public void testTypes() throws Exception {
    ...
    assertNull(
      mimetypeService.getMimetypeIfNotMatches(new DummyContentReader())
    );
  }

  public void testMisc() throws Exception {
    MimetypeMap m = new MimetypeMap(null);
    ContentReader reader = new DummyContentReader(
      MimetypeMap.MIMETYPE_VIDEO_QUICKTIME
    );
    assertEquals(
      MimetypeMap.MIMETYPE_TEXT_PLAIN,
      mimetypeService.getMimetypeIfNotMatches(reader)
    );
    assertEquals(
      MimetypeMap.MIMETYPE_TEXT_PLAIN,
      mimetypeService.guessMimetype("file.rm", reader.getContentInputStream())
    );
    assertEquals(
      MimetypeMap.MIMETYPE_VIDEO_QUICKTIME,
      mimetypeService.guessMimetype("file.rm", reader)
    );
  }
  ...
  public void testTypeBasedOnDetectedTypeAndExtension() throws Exception {
    ...
    assertEquals(
      ...
      mimetypeService.guessMimetype("file.ai", reader.getContentInputStream())
    );
    assertEquals(
      MimetypeMap.MIMETYPE_PDF,
      mimetypeService.guessMimetype("file.pdf", reader.getContentInputStream())
    );

    reader =
      new DummyContentReader(MimetypeMap.MIMETYPE_APPLICATION_PS, "%!PS");
    assertEquals(
      MimetypeMap.MIMETYPE_APPLICATION_EPS,
      mimetypeService.guessMimetype("file.eps", reader.getContentInputStream())
    );
    assertEquals(
      MimetypeMap.MIMETYPE_APPLICATION_PS,
      mimetypeService.guessMimetype("file.ps", reader.getContentInputStream())
    );
  }
  ...
  public void testDuplicates() throws Exception {
    setConfigService(
      ...
      MIMETYPE_1D +
      MIMETYPE_1E +
      MIMETYPE_1F + // Change all values
      MIMETYPE_2A +
      MIMETYPE_2B + // duplicate removes isText
      MIMETYPE_3A +
      MIMETYPE_3A
    ); // identical
    ((MimetypeMap) mimetypeService).init();
    ...
    assertTrue(
      "mimetype1 should have be reset to text",
      mimetypeService.isText("mimetype1")
    );
    ...
    assertFalse(
      "mimetype2 should have be reset to not text",
      mimetypeService.isText("mimetype2")
    );
    assertEquals("mimetype2", mimetypeService.getMimetype("ext2a"));
  ...
  private void setConfigService(final String mimetypes) {
    ConfigSource configSource = new ConfigSource() {
      ...
      public List<ConfigDeployment> getConfigDeployments() {
        ...
        configs.add(
          new ConfigDeployment("name", new ByteArrayInputStream(xml.getBytes()))
        );
        return configs;
      }
    };
  ...
  public void testJsonRead() throws Exception {
    ...
    //  ]
    //}
    ((MimetypeMap) mimetypeService).setMimetypeJsonConfigDir(
        "alfresco/test/mimetypes/testMimetype.json"
      );
    ((MimetypeMap) mimetypeService).init();
    ...
    assertEquals(
      "There should be 2 more mimetypes from the JSON file",
      beforeCount + 2,
      afterCount
    );

...
public class TransactionServiceImplTest extends BaseSpringTest {
  ...
  private final QName vetoName = QName.createQName(
  ...
  public void testPropagatingTxn() throws Exception {
    ...
    assertEquals(
      "Inner txn not marked rolled back",
  ...
  public void testGetRetryingTransactionHelper() {
    ...
    ServiceRegistry serviceRegistry = (ServiceRegistry) applicationContext.getBean(
    ...
    try {
      transactionService
        ...
        .doInTransaction(callback, false);
      fail(
        "Expected AccessDeniedException when starting to write to a read-only transaction service."
      );
  ...
  public void testSystemUserHasWritePermissionsInReadOnlyMode() {
    ...
    transactionService.setAllowWrite(false, vetoName);
    try {
      Boolean isReadOnly = AuthenticationUtil.runAs(
      ...
      assertFalse(
        "SystemUser must has write permissions in read-only mode",
        isReadOnly
      );
    } finally {
      transactionService.setAllowWrite(true, vetoName);

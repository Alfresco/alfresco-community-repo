 ...
 * CRUD operations.
 *
 * <p>The DAO deals in {@link ContentData} instances. The cache is primarily present to decode IDs
 * into <code>ContentData</code> instances.
 *
 * @author Derek Hulley
 * @author sglover
 * @since 3.2
...
public abstract class AbstractContentDataDAOImpl implements ContentDataDAO {
    ...
    public AbstractContentDataDAOImpl() {
        ...
        this.contentDataCache =
                new EntityLookupCache<Long, ContentData, Serializable>(contentDataCallbackDAO);
        this.contentUrlCache =
                new EntityLookupCache<Long, ContentUrlEntity, String>(contentUrlCallbackDAO);
    }
    ...
    public void setContentDataCache(SimpleCache<Long, ContentData> contentDataCache) {
        this.contentDataCache =
                new EntityLookupCache<Long, ContentData, Serializable>(
                        contentDataCache, CACHE_REGION_CONTENT_DATA, contentDataCallbackDAO);
    }
    ...
    public void setContentUrlCache(SimpleCache<Long, ContentUrlEntity> contentUrlCache) {
        this.contentUrlCache =
                new EntityLookupCache<Long, ContentUrlEntity, String>(
                        contentUrlCache, CACHE_REGION_CONTENT_URL, contentUrlCallbackDAO);
    }

    /**
     * A <b>content_url</b> entity was dereferenced. This makes no assumptions about the current
     * references - dereference deletion is handled in the commit phase.
     */
    protected void registerDereferencedContentUrl(String contentUrl) {
        Set<String> contentUrls =
                TransactionalResourceHelper.getSet(KEY_PRE_COMMIT_CONTENT_URL_DELETIONS);
        if (contentUrls.size() == 0) {
            ContentUrlDeleteTransactionListener listener =
                    new ContentUrlDeleteTransactionListener();
        ...
        public Pair<Long, ContentUrlEntity> createValue(ContentUrlEntity value) {
            ContentUrlEntity contentUrlEntity =
                    createContentUrlEntity(
                            value.getContentUrl(), value.getSize(), value.getContentUrlKey());
            // Done
    ...
    private ContentData makeContentData(ContentDataEntity contentDataEntity) {
        ...
        long size =
                contentDataEntity.getSize() == null ? 0L : contentDataEntity.getSize().longValue();
    ...
    protected ContentDataEntity createContentDataEntity(ContentData contentData) {
        ...
        if (contentUrl != null) {
            ...
            Pair<Long, ContentUrlEntity> pair =
                    contentUrlCache.createOrGetByValue(contentUrlEntity, controlDAO);
        ...
        ContentDataEntity contentDataEntity =
                createContentDataEntity(contentUrlId, mimetypeId, encodingId, localeId);
        // Done
    ...
    protected int updateContentDataEntity(
            ContentDataEntity contentDataEntity, ContentData contentData) {
        // Resolve the content URL
        ...
        if (!EqualsHelper.nullSafeEquals(oldContentUrl, newContentUrl)) {
            ...
            if (newContentUrl != null) {
                ...
                Pair<Long, ContentUrlEntity> pair =
                        contentUrlCache.getOrCreateByValue(contentUrlEntity);
    ...
    protected abstract ContentUrlEntity createContentUrlEntity(
            String contentUrl, long size, ContentUrlKeyEntity contentUrlKey);
     ...
     * @return Return the entity or <tt>null</tt> if it doesn't exist or is still referenced by a
     *     <b>content_data</b> entity
     */
     ...
     * @param oldOrphanTime the orphan time we expect to update for optimistic locking (may be
     *     <tt>null</tt>)
    ...
    protected abstract ContentDataEntity createContentDataEntity(
            Long contentUrlId, Long mimetypeId, Long encodingId, Long localeId);
    ...
    protected abstract int updateContentUrlEntity(
            ContentUrlEntity existing, ContentUrlEntity entity);
        ...
        public void beforeCommit(boolean readOnly) {
            ...
            for (String contentUrl : contentUrls) {
                ...
                if (!isEagerCleanup) {
                    ...
                    int updated =
                            updateContentUrlOrphanTime(contentUrlId, orphanTime, oldOrphanTime);
                    if (updated != 1) {
                        throw new ConcurrencyFailureException(
                    ...
                    // We need to make sure that the URL is not re-referenced by another
                    // transaction.

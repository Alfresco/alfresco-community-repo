...
public class SearchRecordsV1CmisTests extends BaseRMRestTest {
    ...
    @Autowired private UserTrashcanAPI userTrashcanAPI;
    ...
    public void setupSearchRecordsV1Cmis() throws Exception {
        ...
        for (int i = 0; ++i <= 10; ) {
            fileModel =
                    new FileModel(
                            String.format(
                                    "%s.%s",
                                    "Record" + SEARCH_TERM + i, FileType.TEXT_PLAIN.extension));
            fileModel =
                    dataContent
                            .usingUser(nonRMUser)
        ...
        queryModel.setQuery(
                "select * from cmis:document WHERE cmis:name LIKE 'Record" + SEARCH_TERM + "%'");
        ...
        Utility.sleep(
                ...
                80000,
                () -> {
                    SearchRequestBuilder sqlRequest =
                    ...
                    SearchResponse searchResponse =
                            getRestAPIFactory().getSearchAPI(null).search(sqlRequest);
                    assertEquals(
    ...
    /**
     * Given some documents with names starting with a particular test When executing the search
     * query with paging And setting the skipCount and maxItems to reach the number of total items
     * Then hasMoreItems will be set to false
     */
    @Test
    public void searchWhenTotalItemsReach() {
        final SearchRequestBuilder sqlRequest =
                new SearchRequestBuilder()
                        .setQueryBuilder(queryModel)
    ...
    public void searchWhenTotalItemsReachWithNonRM() {
        ...
        SearchResponse searchResponse =
                getRestAPIFactory().getSearchAPI(nonRMUser).search(sqlRequest);
        assertEquals(searchResponse.getPagination().getCount(), 5, "Expected maxItems to be five");
        assertEquals(
                searchResponse.getPagination().getSkipCount(), 5, "Expected skip count to be five");
        assertFalse(
                searchResponse.getPagination().isHasMoreItems(),
                "Expected hasMoreItems to be false");
        assertEquals(searchResponse.getEntries().size(), 5, "Expected total entries to be five");
    ...
    /**
     * Given some documents with names starting with a particular text When executing the search
     * query with paging And setting skipCount and maxItems to exceed the number of total items Then
     * hasMoreItems will be set to false
     */
    @Test
    ...
    public void searchWhenTotalItemsExceedNonRMUser() {
        ...
        assertEquals(searchResponse.getPagination().getCount(), 4, "Expected maxItems to be four");
        assertEquals(
                searchResponse.getPagination().getSkipCount(), 6, "Expected skip count to be six");
        assertFalse(
                searchResponse.getPagination().isHasMoreItems(),
                "Expected hasMoreItems to be false");
        assertEquals(searchResponse.getEntries().size(), 4, "Expected total entries to be four");
    ...
    /**
     * Given some documents ending with a particular text When executing the search query with
     * paging And setting skipCount and maxItems under the number of total items Then hasMoreItems
     * will be set to true
     */
    @Test

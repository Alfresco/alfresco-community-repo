...
public class AuditMethodInterceptorTest extends TestCase {
     ...
     * Use NodeService, as it is wrapped by the AuditMethodInterceptor, to get node props in
    ...
    private void checkAuditEntries(
            String applicationName, String language, String query, int resultsNumber) {
        final StringBuilder sb = new StringBuilder();
        final MutableInt rowCount = new MutableInt();
        AuditQueryCallback callback =
                new AuditQueryCallback() {
        ...
        assertTrue("The used query should be in the audit entry.", sb.toString().contains(query));
        if (logger.isDebugEnabled()) {
            logger.debug(sb.toString());
        }
    }

...
public class RepoAdminServiceImplTest extends TestCase {
  ...
  public static final String MODEL_MKR_XML =
    ...
    "   </imports>" +
    "   <namespaces>" +
    "      <namespace uri='http://www.alfresco.org/test/testmodel" +
    MKR +
    "/1.0' prefix='ratest-" +
    MKR +
    "'/>" +
  ...
  protected void setUp() throws Exception {
    ...
    DbNodeServiceImpl dbNodeService = (DbNodeServiceImpl) ctx.getBean(
      "dbNodeService"
    );
  ...
  public void testConsequentDeploy() throws Exception {
    ...
    final QName typeName = QName.createQName(
      "{http://www.alfresco.org/test/testmodel" + X + "/1.0}base"
    );
    // getModelsForUri creates NamespaceLocal that is not cleared in MNT-8930 issue
    dictionaryService.getProperty(typeName);

    final NamespaceDAO namespaceDAO = (NamespaceDAO) ctx.getBean(
      "namespaceDAO"
    );
  ...
  public void testSimpleDynamicModelViaNodeService() throws Exception {
    ...
    final QName modelName = QName.createQName(
      "{http://www.alfresco.org/test/testmodel" + X + "/1.0}testModel" + X
    );
    ...
    try {
      ...
      try {
        ...
        assertTrue(
          ...
          are
            .getMessage()
            .contains(" is in use and cannot be deleted or deactivated.")
        );
      }
      ...
      NodeRef archiveNode1 = new NodeRef(
      ...
      NodeRef archiveModel1 = new NodeRef(
        archiveRootNode.getStoreRef(),
        model1.getId()
      );
  ...
  public void testSimpleDynamicModelViaRepoAdminService() throws Exception {
    ...
    try {
      ...
      try {
        ...
        assertTrue(
          are.getCause().getMessage().contains("is already deactivated")
        );
      }
      ...
      try {
        ...
        assertTrue(
          are.getCause().getMessage().contains("is already activated")
        );
      }
      ...
      try {
        ...
        assertTrue(are.getMessage().contains("Model undeployment failed"));
        assertTrue(
          are.getCause().getMessage().contains("Could not find custom model")
        );
      }
    } finally {
      if (isModelDeployed(modelFileName)) {
  ...
  private void deployModels(int modelCnt) throws UnsupportedEncodingException {
    for (int i = 1; i <= modelCnt; i++) {
      if (!isModelDeployed(modelPrefix + i)) {
        ...
        logger.warn(
          "[" + i + "] Already deployed - test model: " + modelPrefix + i
        );
      }
    }
  }
  ...
  public void testConcurrentDynamicModelCreate() throws Exception {
    ...
    logger.info(
      "Before deploy: deployed custom model count: " + deployedModelCount
    );
    ...
    logger.info(
      "After deploy: deployed custom model count: " + newDeployedModelCount
    );
    ...
    logger.info(
      "After undeploy: deployed custom model count: " + newDeployedModelCount
    );
  ...
  public void testConcurrentDynamicModelDelete() throws Exception {
    ...
    assertEquals(
      "assert A: deployed model count not equal to the repoAdminService",
      deployedModelCount + n,
      repoAdminService.getModels().size()
    );
    ...
    assertEquals(
      "assert after concurrent undeploy",
      deployedModelCount,
      repoAdminService.getModels().size()
    );
  ...
  private void runConcurrentOps(int threadCnt, int opType)
            ...
            new RetryingTransactionHelper.RetryingTransactionCallback<Object>() {
              public Object execute() throws Throwable {
                if (opType == 1) {
                  ...
                  logger.info(
                    "[" + i + "] Deploying - test model: " + modelPrefix + i
                  );
                } else if (opType == 2) {
                  // Undeploy model
                  repoAdminService.undeployModel(modelPrefix + i);
                  ...
                  logger.info(
                    "[" + i + "] Undeployed - test model: " + modelPrefix + i
                  );
                }

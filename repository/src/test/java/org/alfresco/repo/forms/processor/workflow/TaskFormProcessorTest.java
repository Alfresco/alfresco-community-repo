...
public class TaskFormProcessorTest extends FormProcessorTest {
  ...
  private static final NodeRef USER_NODE = new NodeRef(
    NamespaceService.CONTENT_MODEL_1_0_URI + "/admin"
  );
  ...
  public void testGenerateSetsItemAndUrl() throws Exception {
    Form form =
      ((TaskFormProcessor) processor).generate(item, null, null, null);
  ...
  public void testGenerateDefaultForm() throws Exception {
    ...
    assertTrue(
      fieldDefs.contains(ASSOC_WITH_.toPrefixString(namespaceService))
    );
    assertTrue(fieldDefs.contains(DESC_NAME.toPrefixString(namespaceService)));
    assertTrue(
      fieldDefs.contains(STATUS_NAME.toPrefixString(namespaceService))
    );
    ...
    assertEquals(
      ...
      formData.getFieldData("prop_bpm_status").getValue()
    );
  }
  ...
  public void testPersistTransitions() throws Exception {
    ...
    verify(workflowService, times(1))
      .updateTask(eq(TASK_ID), anyMap(), anyMap(), anyMap());
  ...
  private Form processForm(List<String> fields) {
    Form form =
      ((TaskFormProcessor) processor).generate(item, fields, null, null);
    return form;
  }
  ...
  private TaskFormProcessor makeTaskFormProcessor(
    ...
    processor1 =
      (TaskFormProcessor) super.makeTaskFormProcessor(
        processor1,
        dictionaryService,
        fieldProcessorRegistry,
        defaultProcessor
  ...
  private Map<QName, PropertyDefinition> makeTaskPropertyDefs() {
    ...
    PropertyDefinition priorityDef = MockClassAttributeDefinition.mockPropertyDefinition(
      priorityName,
  ...
  private DictionaryService makeDictionaryService() {
    ...
    when(mock.getAnonymousType((QName) any(), (Collection<QName>) any()))
      .thenReturn(task.getDefinition().getMetadata());
    return mock;
  }
  ...
  private WorkflowService makeWorkflowService() {
    ...
    when(service.getTaskById(any()))
      .thenAnswer(
        new Answer<WorkflowTask>() {
          public WorkflowTask answer(InvocationOnMock invocation)
            throws Throwable {
            String id = (String) invocation.getArguments()[0];
            if (TASK_ID.equals(id)) return task; else {
    ...
    this.newTask =
      new WorkflowTask(TASK_ID, null, null, null, null, null, null, null);

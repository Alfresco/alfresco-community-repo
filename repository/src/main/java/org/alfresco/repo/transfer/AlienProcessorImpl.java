...
public class AlienProcessorImpl implements AlienProcessor {
  ...
  public void onCreateChild(
    ...
    if (!nodeService.hasAspect(parentNodeRef, TransferModel.ASPECT_ALIEN)) {
      ...
      String parentFromRepo = (String) nodeService.getProperty(
        parentNodeRef,
      ...
      );
      {
        if (repositoryId.equalsIgnoreCase(parentFromRepo)) {
          log.debug(
            "parent was not alien and this node is from the same repo - do nothing"
          );
          return;
        }
      }
        ...
        if (!isInvaded(parentNodeRef, repositoryId)) {
          ...
          RunAsWork<Void> actionRunAs = new RunAsWork<Void>() {
            public Void doWork() throws Exception {
              getBehaviourFilter()
                .disableBehaviour(newAlien, ContentModel.ASPECT_AUDITABLE);
              getBehaviourFilter()
                .disableBehaviour(newAlien, ContentModel.ASPECT_LOCKABLE);
  ...
  protected void logInvasionHierarchy(NodeRef parentRef, NodeRef childRef) {
    ...
    Map<QName, Serializable> parentProperties = nodeService.getProperties(
      parentRef
    );
  ...
  public void beforeDeleteAlien(
    NodeRef deletedNodeRef,
    ChildAssociationRef oldAssoc
  ) {
    log.debug("before delete node - need to check for alien invaders");

    List<String> stuff = (List<String>) nodeService.getProperty(
      deletedNodeRef,
    ...
    while (
      currentAssoc != null && exInvaders != null && exInvaders.size() > 0
    ) {
      NodeRef parentNodeRef = currentAssoc.getParentRef();
      ...
      if (nodeService.hasAspect(parentNodeRef, TransferModel.ASPECT_ALIEN)) {
        ...
        while (i.hasNext()) {
          ...
          for (ChildAssociationRef ref : refs) {
            ...
            List<String> invadedBy = (List<String>) nodeService.getProperty(
              childRef,
              TransferModel.PROP_INVADED_BY
            );
        ...
        if (exInvaders.size() > 0) {
          ...
          List<String> parentInvaders = (List<String>) nodeService.getProperty(
            parentNodeRef,
            TransferModel.PROP_INVADED_BY
          );
  ...
  public void afterMoveAlien(ChildAssociationRef newAssocRef) {
    ...
    List<String> childInvadedBy = (List<String>) nodeService.getProperty(
      childNodeRef,
      TransferModel.PROP_INVADED_BY
    );
    ...
    if (
      ...
      if (!newAssocRef.getTypeQName().equals(ContentModel.ASSOC_CONTAINS)) {
        ...
        if (!subAspects.contains(newAssocRef.getTypeQName())) {
          ...
          String parentRepoId = descriptorService
      ...
      if (nodeService.hasAspect(parentNodeRef, TransferModel.ASPECT_ALIEN)) {
        ...
        List<String> parentInvadedBy = (List<String>) nodeService.getProperty(
          parentNodeRef,
          TransferModel.PROP_INVADED_BY
        );
        ...
        );
        {
          for (String invader : childInvadedBy) {
            if (invader.equalsIgnoreCase(parentFromRepo)) {
              ...
              log.debug(
                "child node is from the same repo as a non invaded node"
              );
  ...
  private void retreatDownwards(NodeRef nodeRef, String fromRepositoryId) {
    ...
    while (!nodesToRetreat.isEmpty()) {
      ...
      if (
        getNodeService().hasAspect(currentNodeRef, TransferModel.ASPECT_ALIEN)
      ) {
        // Yes this is an alien node
        List<String> invadedBy = (List<String>) getNodeService()
        ...
        if (fromRepositoryId.equalsIgnoreCase(parentRepoId)) {
          ...
          if (invadedBy.size() == 1) {
            ...
            getNodeService()
              .removeAspect(currentNodeRef, TransferModel.ASPECT_ALIEN);
          } else {
  ...
  public void pruneNode(NodeRef nodeToPrune, String fromRepositoryId) {
    ...
    ChildAssociationRef startingParent = nodeService.getPrimaryParent(
      nodeToPrune
    );
    ...
    while (!nodesToPrune.isEmpty()) {
      ...
      if (
        ...
        List<String> invadedBy = (List<String>) getNodeService()
          .getProperty(currentNodeRef, TransferModel.PROP_INVADED_BY);
        String initialRepoId = (String) getNodeService()
          .getProperty(currentNodeRef, TransferModel.PROP_FROM_REPOSITORY_ID);
        ...
        if ((null != invadedBy) && invadedBy.contains(fromRepositoryId)) {
          if (log.isDebugEnabled()) {
            log.debug(
              "Current nodeRef's PROP_INVADED_BY contains current manifestId (name: \"" +
              properties.get(ContentModel.PROP_NAME) +
              "\", fromRepositoryId: \"" +
              properties.get(TransferModel.PROP_FROM_REPOSITORY_ID) +
              "\", manifestId: \"" +
           ...
           */
          getNodeService()
            .hasAspect(currentNodeRef, TransferModel.ASPECT_TRANSFERRED);
          {
            if (fromRepositoryId.equalsIgnoreCase(initialRepoId)) {
              ...
              List<ChildAssociationRef> refs = getNodeService()
          ...
          .hasAspect(currentNodeRef, TransferModel.ASPECT_TRANSFERRED);
        {
          ...
          String initialRepoId = (String) getNodeService()
            .getProperty(currentNodeRef, TransferModel.PROP_REPOSITORY_ID);
          if (fromRepositoryId.equalsIgnoreCase(initialRepoId)) {
            if (log.isDebugEnabled()) {
              log.debug(
    ...
    while (ripple != null) {
      ...
      if (
        nodeService.hasAspect(ripple.getParentRef(), TransferModel.ASPECT_ALIEN)
      ) {
   ...
   */
  private boolean recalcInvasion(
    ...
    List<String> folderInvadedBy = (List<String>) nodeService.getProperty(
      folderNodeRef,
      TransferModel.PROP_INVADED_BY
    );
    ...
    String parentRepositoryId = (String) nodeService.getProperty(
      folderNodeRef,
      TransferModel.PROP_FROM_REPOSITORY_ID
    );
    for (ChildAssociationRef ref : refs) {
      ...
      List<String> childInvadedBy = (List<String>) properties.get(
        TransferModel.PROP_INVADED_BY
      );
      String childRepositoryId = (String) properties.get(
        TransferModel.PROP_FROM_REPOSITORY_ID
      );

      hasAlienChild =
        hasAlienChild ||
        !parentRepositoryId.equalsIgnoreCase(childRepositoryId);
    ...
    if (!stillInvaded) {
      if (log.isDebugEnabled()) {
        log.debug(
          "Current folder is not invaded by this repository. Updating 'invadedBy' property..."
        );
      ...
      if (folderInvadedBy.size() > 0) {
        if (log.isDebugEnabled()) {
          log.debug(
            "Current folder HAS ANOTHER invasions. Updating the 'invadedBy' property..."
          );
        ...
        getNodeService()
          .removeAspect(folderNodeRef, TransferModel.ASPECT_ALIEN);
      }
    }

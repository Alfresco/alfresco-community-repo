...
public class RepositoryExporterComponent implements RepositoryExporterService {
    ...
    public FileExportHandle[] export(String packageName) {
        List<FileExportHandle> exportHandles =
                exportStores(exportStores, packageName, new TempFileExporter());
    ...
    public FileExportHandle[] export(File directoryDestination, String packageName) {
        ...
        List<FileExportHandle> exportHandles =
                exportStores(exportStores, packageName, new FileExporter(directoryDestination));
        return exportHandles.toArray(new FileExportHandle[exportHandles.size()]);
    }
    ...
    public RepositoryExportHandle[] export(NodeRef repositoryDestination, String packageName) {
        ...
        if (destInfo == null || !destInfo.isFolder()) {
            throw new ExporterException(
                    "Repository destination " + repositoryDestination + " is not a folder.");
        }

        List<FileExportHandle> exportHandles =
                exportStores(exportStores, packageName, new TempFileExporter());
        Map<String, String> mimetypeExtensions = mimetypeService.getExtensionsByMimetype();
        List<RepositoryExportHandle> repoExportHandles =
                new ArrayList<RepositoryExportHandle>(exportHandles.size());
        for (FileExportHandle exportHandle : exportHandles) {
            String name =
                    exportHandle.packageName + "." + mimetypeExtensions.get(exportHandle.mimeType);
            String title = exportHandle.packageName;
            String description;
            if (exportHandle.storeRef != null) {
                description =
                        I18NUtil.getMessage(
                                "export.store.package.description",
                                new Object[] {exportHandle.storeRef.toString()});
            } else {
                description = I18NUtil.getMessage("export.generic.package.description");
            }

            NodeRef repoExportFile =
                    addExportFile(
                            repositoryDestination,
                            name,
                            title,
                            description,
                            exportHandle.mimeType,
                            exportHandle.exportFile);
     ...
     */
    private NodeRef addExportFile(
            NodeRef repoDestination,
            String name,
            String title,
            String description,
            String mimeType,
        ...
        try {
            FileInfo fileInfo =
                    fileFolderService.create(repoDestination, name, ContentModel.TYPE_CONTENT);
    ...
    private interface ExportStore<ExportHandleType extends ExportHandle> {
        public ExportHandleType exportStore(
                ExporterCrawlerParameters exportParameters, String packageName, Exporter progress);
     ...
     */
    private <ExportHandleType extends ExportHandle> List<ExportHandleType> exportStores(
            List<Properties> stores,
            String packageName,
            ExportStore<ExportHandleType> exportStore) {
        ...
        // export repository system info
        {
            String completePackageName =
                    (packageName == null) ? "systeminfo" : packageName + "_systeminfo";
        ...
        for (Properties store : stores) {
            ...
            String completePackageName =
                    (packageName == null) ? storePackageName : packageName + "_" + storePackageName;

            // retrieve included paths (optional)
            // note: the default exporter will currently include parents and children, relative to
            // the path (to support bootstrap import of Dynamic Models)
            String includedPathsStr = (String) store.get(INCLUDED_PATHS);
            String[] includedPaths =
                    (includedPathsStr != null ? includedPathsStr.split(",\\s*") : null);
            ...
            ExportHandleType exportHandle =
                    exportStore.exportStore(exportParameters, completePackageName, null);
    ...
    private ExporterCrawlerParameters getExportParameters(
            StoreRef storeRef, String[] includedPaths) {
    ...
    private class TempFileExporter implements ExportStore<FileExportHandle> {
        ...
        public FileExportHandle exportStore(
            ...
            try {
                ...
                tempFile.delete();
                throw new ExporterException(
                        "Failed to create temporary file for holding export of store "
        ...
        public FileExportHandle exportSystem(String packageName) {
            ...
            File systemTempDir = TempFileProvider.getSystemTempDir();
            File tempFile =
                    TempFileProvider.createTempFile("repoExpSystemInfo", ".xml", systemTempDir);
            ...
            try {
                ...
                tempFile.delete();
                throw new ExporterException(
                        "Failed to create temporary file for holding export of system info");
            }

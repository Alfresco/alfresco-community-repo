...
public class MoveTests extends WebDavTest {
    ...
    @TestRail(
            ...
            description = "Verify that admin user can move folder")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Verify that user with contributor role cannot move folder with content in site"
                            + " folder")
    @Test(groups = {TestGroup.PROTOCOLS, TestGroup.WEBDAV, TestGroup.SANITY})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Verify that user with collaborator role cannot move folder with content in"
                            + " site folder")
    @Test(groups = {TestGroup.PROTOCOLS, TestGroup.WEBDAV, TestGroup.SANITY})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Verify that user with consumer role cannot move folder with content in site"
                            + " folder")
    @Test(groups = {TestGroup.PROTOCOLS, TestGroup.WEBDAV, TestGroup.SANITY})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify the response code for a conflict when a file is moved in a folder that"
    ...
    public void conflictWhileMovingFile() throws Exception {
        ...
        webDavProtocol
                ...
                .then()
                .usingResource(sourceFolder)
                .createFile(testFileSource)
                .assertThat()
                .existsInRepo()
                .then()
                .usingResource(destinationFolder)
                .createFile(testFileDestination)
                .assertThat()
                .existsInRepo()
                .when()
                .usingResource(testFileSource)
                .doNotOverwriteIfExists()
                .moveTo(destinationFolder)
                .and()
                .assertThat()
                .hasStatus(HttpStatus.SC_PRECONDITION_FAILED);
    }
}

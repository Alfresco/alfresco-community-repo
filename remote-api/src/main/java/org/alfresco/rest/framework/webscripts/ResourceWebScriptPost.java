 ...
 * @author Gethin James
 */
public class ResourceWebScriptPost extends AbstractResourceWebScript
        implements ParamsExtractor, RecognizedParamsExtractor, RequestReader {
    ...
    public Params extractParams(ResourceMetadata resourceMeta, WebScriptRequest req) {
        final Map<String, String> resourceVars =
                locator.parseTemplateVars(req.getServiceMatch().getTemplateVars());
        ...
        switch (resourceMeta.getType()) {
            case ENTITY:
                if (StringUtils.isNotBlank(entityId)) {
                    throw new UnsupportedResourceOperationException(
                            "POST is executed against a collection URL");
                } else {
            ...
            case RELATIONSHIP:
                if (StringUtils.isNotBlank(propertyName)
                        && (StringUtils.isNotBlank(relationshipId))) {
    ...
    /**
     * If the request content-type is <i><b>multipart/form-data</b></i> then it returns the {@link
     * FormData}, otherwise it tries to extract the required object from the JSON payload.
     */
    private Object processRequest(
            ResourceMetadata resourceMeta, ResourceOperation operation, WebScriptRequest req) {
    ...
    /**
     * If the @WebApiParam has been used and set allowMultiple to false then this will get a single
     * entry. It should error if an array is passed in.
     *
     * @param resourceMeta ResourceMetadata
     * @param req WebScriptRequest
     * @return Either an object
     */
    private Object extractObjFromJson(
            ResourceMetadata resourceMeta, ResourceOperation operation, WebScriptRequest req) {
        if (operation == null) {
            return null;
        }
        ...
        boolean isTypeOperation =
                resourceMeta.getType().equals(ResourceMetadata.RESOURCE_TYPE.OPERATION);
        List<ResourceParameter> params = operation.getParameters();

        if (!params.isEmpty()) {
            for (ResourceParameter resourceParameter : params) {
                // POST to collection may or may not support List as json body, Operations don't
                // support a List as json body
                ...
                if (ResourceParameter.KIND.HTTP_BODY_OBJECT.equals(resourceParameter.getParamType())
                    ...
                    try {
                        ...
                        if (objType != null) {
                            ...
                            if (!resourceParameter.isRequired()
                                    && Integer.valueOf(req.getHeader("content-length")) <= 0) {
                                // in some cases the body is optional and the json doesn't need to
                                // be extracted
                                return null;
                            } else {
                                jsonContent =
                                        extractJsonContent(req, assistant.getJsonHelper(), objType);
                            }
                        }
                        ...
                        if (iae.getMessage().contains("START_ARRAY")
                                && iae.getMessage().contains("line: 1, column: 1")) {
                            throw new UnsupportedResourceOperationException(
                                    "Only 1 entity is supported in the HTTP request body");
                        } else {
                            throw iae;
                        }
                    }
    ...
    private Object executeOperation(
            ResourceWithMetadata resource, Params params, WithResponse withResponse)
            throws Throwable {
        OperationResourceMetaData operationResourceMetaData =
                (OperationResourceMetaData) resource.getMetaData();

        switch (operationResourceMetaData.getOperationMethod().getParameterTypes().length) {
            case 4:
                // EntityResource operation by id
                return ResourceInspectorUtil.invokeMethod(
                        operationResourceMetaData.getOperationMethod(),
        ...
        throw new UnsupportedResourceOperationException(
                "The operation method has an invalid signature");
    }
        ...
        switch (resource.getMetaData().getType()) {
            case ENTITY:
                if (resObj instanceof MultiPartResourceAction.Create<?>
                        && params.getPassedIn() instanceof FormData) {
                    MultiPartResourceAction.Create<Object> creator =
                            (MultiPartResourceAction.Create<Object>) resObj;
                    return creator.create((FormData) params.getPassedIn(), params, withResponse);

                } else {
                    if (EntityResourceAction.Create.class.isAssignableFrom(
                            resource.getResource().getClass())) {
                        if (resource.getMetaData().isDeleted(EntityResourceAction.Create.class)) {
                            throw new DeletedResourceException(
                                    "(DELETE) " + resource.getMetaData().getUniqueId());
                        }
                        EntityResourceAction.Create<Object> creator =
                                (EntityResourceAction.Create<Object>) resObj;
                        List<Object> created =
                                creator.create((List<Object>) params.getPassedIn(), params);
                    ...
                    if (EntityResourceAction.CreateWithResponse.class.isAssignableFrom(
                            resource.getResource().getClass())) {
                        if (resource.getMetaData()
                                .isDeleted(EntityResourceAction.CreateWithResponse.class)) {
                            throw new DeletedResourceException(
                                    "(DELETE) " + resource.getMetaData().getUniqueId());
                        }
                        EntityResourceAction.CreateWithResponse<Object> creator =
                                (EntityResourceAction.CreateWithResponse<Object>) resObj;
                        List<Object> created =
                                creator.create(
                                        (List<Object>) params.getPassedIn(), params, withResponse);
                        if (created != null && created.size() == 1) {
                            // return just one object instead of an array
                            return created.get(0);
                        } else {
            ...
            case RELATIONSHIP:
                if (resObj instanceof MultiPartRelationshipResourceAction.Create<?>
                        && params.getPassedIn() instanceof FormData) {
                    MultiPartRelationshipResourceAction.Create<Object> creator =
                            (MultiPartRelationshipResourceAction.Create<Object>) resObj;
                    return creator.create(
                            params.getEntityId(),
                            (FormData) params.getPassedIn(),
                            params,
                            withResponse);
                } else {
                    if (RelationshipResourceAction.Create.class.isAssignableFrom(
                            resource.getResource().getClass())) {
                        if (resource.getMetaData()
                                .isDeleted(RelationshipResourceAction.Create.class)) {
                            throw new DeletedResourceException(
                                    "(DELETE) " + resource.getMetaData().getUniqueId());
                        }

                        RelationshipResourceAction.Create<Object> createRelation =
                                (RelationshipResourceAction.Create<Object>) resource.getResource();
                        List<Object> createdRel =
                                createRelation.create(
                                        params.getEntityId(),
                                        (List<Object>) params.getPassedIn(),
                                        params);
                    ...
                    if (RelationshipResourceAction.CreateWithResponse.class.isAssignableFrom(
                            resource.getResource().getClass())) {
                        if (resource.getMetaData()
                                .isDeleted(RelationshipResourceAction.CreateWithResponse.class)) {
                            throw new DeletedResourceException(
                                    "(DELETE) " + resource.getMetaData().getUniqueId());
                        }

                        RelationshipResourceAction.CreateWithResponse<Object> createRelation =
                                (RelationshipResourceAction.CreateWithResponse<Object>)
                                        resource.getResource();
                        List<Object> createdRel =
                                createRelation.create(
                                        params.getEntityId(),

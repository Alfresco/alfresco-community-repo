...
public class WebScriptOptionsMetaData
  ...
  public void execute(
    ...
    String collectionName = templateVars.get(
      ResourceLocator.COLLECTION_RESOURCE
    );
    String resourceName = templateVars.get(
    ...
    if (resource == null) {
      ...
      resourceKey =
        ResourceDictionary.propertyResourceKey(collectionName, resourceName);
  ...
  protected ResourceMetaDataWriter chooseWriter(WebScriptRequest req) {
    if (writers != null) {
      ResourceMetaDataWriter theWriter = writers.get(
      ...
      for (ResourceWithMetadata aResource : apiResources.values()) {
        if (
          resource
            .getMetaData()
            .getUniqueId()
            .equals(aResource.getMetaData().getParentResource())
        ) {

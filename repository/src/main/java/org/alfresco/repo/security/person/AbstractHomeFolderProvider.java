...
/**
 * Common support for creating home folders This is hooked into node creation events from Person
 * type objects via the homeFolderManager. Provider must all be wired up to the homeFolderManager.
 *
    ...
    public class V2Adaptor implements HomeFolderProvider2 {
        ...
        public List<String> getHomeFolderPath(NodeRef person) {
            return (abstractHomeFolderProvider instanceof UIDBasedHomeFolderProvider)
                    ? ((UIDBasedHomeFolderProvider) abstractHomeFolderProvider)
                            .getHomeFolderPath(person)
                    : null;
        }

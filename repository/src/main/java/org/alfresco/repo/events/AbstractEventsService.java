...
public abstract class AbstractEventsService extends TransactionListenerAdapter {
    ...
    public void setIncludeEventTypes(String includeEventTypesStr) {
        this.includeEventTypes =
                Sets.newHashSet(
                        Splitter.on(",")
                                .trimResults()
                                .omitEmptyStrings()
                                .split(includeEventTypesStr));
    }
    ...
    public void beforeCommit(boolean readOnly) {
        if (sendEventsBeforeCommit) {
            // send all events
            final TxnEvents transactionEvents =
                    (TxnEvents) AlfrescoTransactionSupport.getResource(EVENTS_KEY);
    ...
    protected Client getAlfrescoClient(Client knownClientType) {
        ...
        // We can use this information to avoid setting "alfrescoClientId" in this case, otherwise
        // its presence
        // will cause the filtering-out of the events that are generated as a result of a rule
        // execution;
        //
        // We should find a better, cleaner solution in the future...
        Object noAlfrescoClientIdHint =
    ...
    public void afterCommit() {
        if (sendEventsBeforeCommit) {
            ...
            // Need to execute this in another read txn because Camel/JMS expects it (the config now
            // seems to
            // require a txn)
            transactionService
    ...
    /**
     * Determines whether it still makes sense to create and send a TransactionCommittedEvent after
     * the transaction is committed. (e.g. if there are no events for the specific transaction makes
     * no sense to signal the end of transaction with TransactionCommittedEvent)
     *
     ...
     */
    private void updateTransactionEvents(
            final TxnEvents transactionEvents, List<Event> filteredEvents) {
    ...
    protected class NodeInfo {
        ...
        public Map<String, Serializable> getProperties() {
            ...
            String workingCopyOwner =
                    (String) nodeService.getProperty(nodeRef, ContentModel.PROP_WORKING_COPY_OWNER);
    ...
    /**
     * For each path, construct a list of ancestor node ids starting with the leaf path element node
     * id first.
     *
     * @param nodePaths
     * @return
    ...
    protected TxnEvents getTxnEvents() {
        ...
        if (events == null) {
            ...
            // so it will give a chance to other listeners to finish execution and have all events
            // generated before sending them.
            // See MNT-17302
                            ...
                            public NodeInfo doWork() throws Exception {
                                ...
                                if (!includeEventType(eventType)) {
                                    ...
                                    if (!typeMatches(type)) {
                                        ...
                                        String siteId =
                                                (siteInfo != null ? siteInfo.getShortName() : null);

 ...
 * examples of which include: schemas, tables, indexes, primary keys, foreign keys, sequences and
 * columns.
 *
 * @author Matt Ward
 */
public interface DbObject {
     ...
     * different names, but are the same index as they both index the same columns for the same
     * table.
     *
     * <p>If two objects a and b have the same logical identity, it does not mean that <code>
     * a.equals(b) == true</code>. The two objects may well have differences and will be flagged as
     * such by the schema comparison tool. When <code>a.sameAs(b) == true</code> it makes it easier
     * to show the differences as related, i.e. a and b are different rather than, a is only in the
     * 'left' tree and b is only in the 'right' tree.
     *
     ...
     * Differences between the left and right objects under inspection are captured in the {@link
     * Results} object passed in to this method.
     *
    ...
    /**
     * Allows a visitor to be invoked against this DbObject. Implementations should ensure that
     * child objects are visited first (by calling accept on them) before invoking the visitor on
     * itself.
     *
    ...
    /**
     * Get the parent object for which this object is a child. If this is the root object then null
     * should be returned.
     *
     ...
     * requirement for reporting of differences for this object?
     *
     * @return boolean
     */

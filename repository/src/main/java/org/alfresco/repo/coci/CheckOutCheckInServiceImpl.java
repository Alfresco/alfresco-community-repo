...
public class CheckOutCheckInServiceImpl
  implements CheckOutCheckInService, Extensible {

  /**
   * I18N labels
   */
  private static final String MSG_ERR_BAD_COPY = "coci_service.err_bad_copy";
  private static final String MSG_WORKING_COPY_LABEL =
  ...
  private static final String MSG_ERR_NOT_AUTHENTICATED =
    "coci_service.err_not_authenticated";
  private static final String MSG_ERR_WORKINGCOPY_HAS_NO_CONTENT =
    "coci_service.err_workingcopy_has_no_content";
  private static final String MSG_ALREADY_CHECKEDOUT =
    "coci_service.err_already_checkedout";
  private static final String MSG_ERR_CANNOT_RENAME =
    "coci_service.err_cannot_rename";
  ...
  private static Log logger = LogFactory.getLog(
    CheckOutCheckInServiceImpl.class
  );
  ...
  public void init() {
    ...
    onCheckIn =
      policyComponent.registerClassPolicy(
        CheckOutCheckInServicePolicies.OnCheckIn.class
      );
  ...
  private void invokeBeforeCancelCheckOut(NodeRef workingCopy) {
    List<QName> classes = getInvokeClasses(workingCopy);
    for (QName invokeClass : classes) {
      Collection<BeforeCancelCheckOut> policies = beforeCancelCheckOut.getList(
        invokeClass
      );
  ...
  private void invokeOnCancelCheckOut(NodeRef nodeRef) {
    List<QName> classes = getInvokeClasses(nodeRef);
    for (QName invokeClass : classes) {
      Collection<OnCancelCheckOut> policies = onCancelCheckOut.getList(
        invokeClass
      );
  ...
  public NodeRef checkout(
    ...
    if (
      lockStatus != LockStatus.NO_LOCK && lockStatus != LockStatus.LOCK_EXPIRED
    ) {
  ...
  private NodeRef doCheckout(
    ...
    try {
      ...
      String copyName = (String) this.nodeService.getProperty(
          nodeRef,
      ...
      final QName copyQName = QName.createQName(
      ...
      Map<QName, Serializable> workingCopyProperties = new HashMap<QName, Serializable>(
        1
      );
      workingCopyProperties.put(ContentModel.PROP_WORKING_COPY_OWNER, userName);
      workingCopyProperties.put(
        ContentModel.PROP_WORKING_COPY_LABEL,
        workingCopyLabel
      );
  ...
  public NodeRef checkin(
    ...
    if (
      ...
      throw new AspectMissingException(
        ContentModel.ASPECT_WORKING_COPY,
        workingCopyNodeRef
      );
    }
    ...
    if (contentUrl != null) {
      ...
      if (contentData == null) {
        throw new AlfrescoRuntimeException(
          MSG_ERR_WORKINGCOPY_HAS_NO_CONTENT,
          new Object[] { workingCopyNodeRef }
        );
      } else {
        contentData =
          new ContentData(
    ...
    if (fileFolderService.getFileInfo(workingCopyNodeRef) != null) {
      String origName = (String) nodeService.getProperty(
        nodeRef,
        ContentModel.PROP_NAME
      );
      ...
      String wcLabel = (String) this.nodeService.getProperty(
          workingCopyNodeRef,
          ContentModel.PROP_WORKING_COPY_LABEL
        );
      if (wcLabel == null) {
        ...
        if (!name.contains(" " + wcLabel)) {
          try {
            fileFolderService.rename(
              workingCopyNodeRef,
              createWorkingCopyName(name, wcLabel)
            );
          } catch (FileExistsException e) {
            throw new CheckOutCheckInServiceException(
              e,
              MSG_ERR_CANNOT_RENAME,
              name,
              wcLabel
            );
  ...
  public NodeRef getWorkingCopy(NodeRef nodeRef) {
    ...
    if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_CHECKED_OUT)) {
      ...
      if (assocs.size() == 0) {
        logger.warn(
          "Found node with cm:checkedOut aspect but no association.  Current node state: " +
  ...
  public NodeRef getCheckedOut(NodeRef nodeRef) {
    ...
    if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_WORKING_COPY)) {
      ...
      if (assocs.size() == 0) {
        logger.warn(
          "Found node with cm:workingcopy aspect but no association.  Current node state: " +
          nodeService.getNodeStatus(nodeRef)
        );
      } else if (assocs.size() > 1) {
        logger.warn(
   ...
   * @return String
   */
  public static String createWorkingCopyName(
    String name,
    final String workingCopyLabel
  ) {
   ...
   */
  private String getNameFromWorkingCopyName(
    ...
    if (workingCopyName.contains(" " + workingCopyLabel)) {
      workingCopyName =
        workingCopyName.replaceFirst(" " + workingCopyLabelRegEx, "");

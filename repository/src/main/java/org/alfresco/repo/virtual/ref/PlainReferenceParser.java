...
public class PlainReferenceParser implements ReferenceParser, PlainEncoding {
  ...
  @Override
  public Reference parse(String referenceString)
  ...
  private Reference parseReference(Cursor cursor)
    throws ReferenceParseException {
    try {
   ...
   * @throws ReferenceParseException
   */
  private List<Parameter> parseParameters(Cursor cursor)
    throws ReferenceParseException {
    ...
    while (cursor.i < cursor.tokens.length) {
      if (RESOURCE_PARAMETER.equals(cursor.tokens[cursor.i])) {
        ...
        throw new ReferenceParseException(
          "Invalid parameter " + cursor.tokens[cursor.i]
        );
      }
    }
    return parameters;
   ...
   * @throws ReferenceParseException
   */
  private Parameter parseResourceParameter(Cursor cursor)
    throws ReferenceParseException {
  ...
  private Resource parseResource(Cursor cursor) throws ReferenceParseException {
    if (REPOSITORY.equals(cursor.tokens[cursor.i])) {
      ...
      throw new ReferenceParseException(
        "Invalid resource " + cursor.tokens[cursor.i]
      );
    }
  }
  ...
  private RepositoryLocation parseRepositoryLocation(Cursor cursor)
    ...
    if (PATH.equals(cursor.tokens[cursor.i])) {
      ...
      throw new ReferenceParseException(
        "Invalid location " + cursor.tokens[cursor.i]
      );
    }
  }
  ...
  private RepositoryNodeRef parseRepositoryNode(Cursor cursor) {
    ...
    return new RepositoryNodeRef(
      new NodeRef(storeProtocol, storeIdentifier, id)
    );
  }

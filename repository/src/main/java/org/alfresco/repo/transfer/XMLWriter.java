...
/**
 * A simple class whose primary purpose is to wrap the fairly unfriendly interface presented by the
 * dom4j XMLWriter with one that is simpler to use.
 *
 * @author Brian
 */
public class XMLWriter {
    ...
    public XMLWriter(OutputStream outputStream, boolean prettyPrint, String encoding)
        ...
        OutputFormat format =
                prettyPrint ? OutputFormat.createPrettyPrint() : OutputFormat.createCompactFormat();
    ...
    public void startElement(QName name, Attributes attrs) {
        ...
        try {
            dom4jWriter.startElement(
                    name.getNamespaceURI(),
                    name.getLocalName(),
                    name.toPrefixString(prefixResolver),
                    attrs);
        } catch (SAXException e) {
            // Can't happen
        }
    ...
    /**
     * A local namespace prefix resolver class that allows the XMLWriter interface to be simplified
     * to use proper QNames
     */

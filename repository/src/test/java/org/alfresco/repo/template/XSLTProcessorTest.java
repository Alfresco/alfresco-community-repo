...
public class XSLTProcessorTest extends BaseAlfrescoSpringTest {
    private static final Log log = LogFactory.getLog(XSLTProcessorTest.class);
    ...
    public void testSimplestStringTemplate() throws Exception {
        try {
            ...
            model.put(
                    XSLTProcessor.ROOT_NAMESPACE, XMLUtil.parse(file.getNodeRef(), contentService));
            ...
            assertEquals(
                    "Avocado DipBagels, New York StyleBeef Frankfurter, Quarter PoundChicken Pot"
                            + " PieCole SlawEggsHazelnut SpreadPotato ChipsSoy Patties,"
                            + " GrilledTruffles, Dark Chocolate",
                    output);
        } catch (Exception ex) {
            log.error("Error!", ex);
            fail();
    ...
    public void testSimplestNodeTemplate() throws Exception {
        try {
            ...
            XSLTemplateModel model = new XSLTemplateModel();
            model.put(
                    XSLTProcessor.ROOT_NAMESPACE,
                    XMLUtil.parse(xmlFile.getNodeRef(), contentService));
    ...
    public void testLocalisedNodeTemplate() throws Exception {
        // This should have the same result as testSimplestNodeTemplate as the localization should
        // be ignored for node templates.
        try {
            FileInfo xmlFile = createXmlFile(companyHome);
            FileInfo xslFile = createXmlFile(companyHome, verySimpleXSLT);
            XSLTemplateModel model = new XSLTemplateModel();
    ...
    public void testLocalisedClasspathTemplate() throws Exception {
        try {
            ...
            xsltProcessor.process(
                    "org/alfresco/repo/template/test_template1.xsl",
                    model,
                    writer,
                    new Locale("en", "AU"));
            ...
            log.debug("XSLT Processor output: " + output);
            assertEquals(
                    "G'day, Avocado DipBagels, New York StyleBeef Frankfurter, Quarter PoundChicken"
                            + " Pot PieCole SlawEggsHazelnut SpreadPotato ChipsSoy Patties,"
                            + " GrilledTruffles, Dark Chocolate",
                    output);
            ...
            log.debug("XSLT Processor output: " + output);
            assertEquals(
                    "Bonjour, Avocado DipBagels, New York StyleBeef Frankfurter, Quarter"
                            + " PoundChicken Pot PieCole SlawEggsHazelnut SpreadPotato ChipsSoy"
                            + " Patties, GrilledTruffles, Dark Chocolate",
                    output);
        } catch (Exception ex) {
            log.error("Error!", ex);
            fail();
    ...
    public void testTemplateServiceBinding() throws Exception {
        try {
            ...
            templateService.processTemplate(
                    "xslt", "org/alfresco/repo/template/test_template1.xsl", model, writer);
    ...
    private FileInfo createXmlFile(NodeRef folder, String content) {
        ...
        FileInfo testXmlFile =
                fileFolderService.create(folder, name + ".xml", ContentModel.TYPE_CONTENT);
        ContentWriter writer =
                contentService.getWriter(testXmlFile.getNodeRef(), ContentModel.PROP_CONTENT, true);

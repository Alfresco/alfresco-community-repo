...
public class NodeParameterProcessor
  ...
  public String process(String value, NodeRef actionedUponNodeRef) {
    ...
    if (!value.isEmpty()) {
      ...
      if (ArrayUtils.contains(supportedDataTypes, type)) {
        Serializable propertyValue = nodeService.getProperty(
          actionedUponNodeRef,
          qname
        );
      ...
      for (QName definition : this.suggestionDefinitions) {
        if (
          getSubstitutionSuggestions(
            definition,
            substitutionFragment.toLowerCase(),
            suggestionSet
          )
        ) {
          break;
  ...
  private boolean getSubstitutionSuggestions(
    ...
    ClassDefinition definition =
      this.dictionaryService.getAspect(definitionName);
    if (definition == null) {
    ...
    if (
      recordsManagementAdminService.isCustomisable(definitionName) &&
      !gotMaximumSuggestions
    ) {
      gotMaximumSuggestions =
  ...
  private boolean getSubstitutionSuggestionsForDefinition(
    ...
    boolean gotMaximumSuggestions = processPropertyDefinitions(
  ...
  private boolean processPropertyDefinitions(
    ...
    if (properties != null) {
      for (Map.Entry<QName, PropertyDefinition> entry : properties.entrySet()) {
        ...
        if (ArrayUtils.contains(supportedDataTypes, type)) {
          String suggestion =
            getName() + "." + entry.getKey().getPrefixString();

...
/**
 * Represents maximum source values (that result in exceptions if exceeded) or limits on source
 * values (that result in EOF (End Of File) being returned early). Options exist for elapse time,
 * KBytes read or number of pages read.
 *
...
public class TransformationOptionLimits implements Serializable {
    ...
    public static final String OPT_MAX_SOURCE_SIZE_K_BYTES =
            RenditionDefinition2.MAX_SOURCE_SIZE_K_BYTES;
    ...
    public static final String TIME_MESSAGE =
            "Both timeoutMs and readLimitTimeMs should not be set.";
    public static final String KBYTES_MESSAGE =
            "Both maxSourceSizeKBytes and readLimitKBytes should not be set.";
    ...
    private TransformationOptionLimits(
            TransformationOptionLimits a, TransformationOptionLimits b, boolean lower) {
        if (lower) {
     ...
     *
     * <p>A call to this method overrides any values in the supplied limits parameter with those in
     * this Object. The supplied limits parameter is being gradually built up by initially setting
     * the most general limits and then more specific values for each level. As a result 'default'
    ...
    /**
     * Indicates if the limits allow a transformation to take place at all. If any of the limits are
     * 0, it would not be possible.
     *
    ...
    /**
     * Returns a TransformationOptionLimits that has getter methods that combine the the values from
     * the getter methods of this and the supplied TransformationOptionLimits.
     */
    ...
    /**
     * Returns a TransformationOptionLimits that has getter methods that combine the the values from
     * the getter methods of this and the supplied TransformationOptionLimits so that they return
     * the lowest common denominator of the limits .
     */
    public TransformationOptionLimits combineUpper(final TransformationOptionLimits that) {
        return combine(that, false);
    }

    private TransformationOptionLimits combine(
            final TransformationOptionLimits that, boolean lower) {

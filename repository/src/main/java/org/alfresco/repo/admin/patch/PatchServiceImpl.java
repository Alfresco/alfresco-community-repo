 ...
 *
 * <p>Patches are injected into this class and any attempted applications are recorded for later
 * auditing.
 *
 * @since 1.2
 * @author Derek Hulley
...
public class PatchServiceImpl implements PatchService {
    ...
    private static final String MSG_PRECEEDED_BY_ALTERNATIVE =
            "patch.service.preceeded_by_alternative";
    ...
    private final QName vetoName =
            QName.createQName(NamespaceService.APP_MODEL_1_0_URI, "PatchServiceImpl");
            ...
            if (patch.getFixesToSchema() > serverSchemaVersion) {
                logger.error(
                        I18NUtil.getMessage(
                                ...
                                patch.getId(),
                                serverSchemaVersion,
                                patch.getFixesToSchema(),
                                patch.getTargetSchema()));
                success = false;
            }
        }
        if (!success) {
    ...
    public boolean applyOutstandingPatches() {
        ...
        try {
            ...
            try {
                ...
                Map<String, AppliedPatch> appliedPatchesById =
                        new HashMap<String, AppliedPatch>(250);
                ...
                for (final AppliedPatch appliedPatch : appliedPatches) {
                    ...
                    if (appliedPatch.getAppliedOnDate() == null) {
                        RetryingTransactionCallback<Date> callback =
                                new RetryingTransactionCallback<Date>() {
    ...
    /**
     * Reentrant method that ensures that a patch and all its dependencies get applied. The process
     * terminates on the first failure.
     *
     ...
     */
    private boolean applyPatchAndDependencies(
            final Patch patch, Map<String, AppliedPatch> appliedPatchesById) {
    ...
    public List<AppliedPatch> getPatches(Date fromDate, Date toDate) {
        ...
        List<? extends AppliedPatch> appliedPatches =
                appliedPatchDAO.getAppliedPatches(fromDate, toDate);
        // done
     ...
     *
     * <p>This has been split out from applyPatch because it's easier to perform the transaction
     * wrapping required when requiresTransaction == false
     */
    private static class PatchWork {
        ...
        private String preceededByAlternative(Patch patch) {
            ...
            for (Patch alternative : alternatives) {
                ...
                AppliedPatch appliedAlternative =
                        appliedPatchDAO.getAppliedPatch(alternative.getId());
                if (appliedAlternative != null
                        && appliedAlternative.getWasExecuted()
                        && appliedAlternative.getSucceeded()) {
        ...
        private boolean applies(Descriptor repoDescriptor, Patch patch) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Patch schema version number check against repo version: \n"
                                + "   repo schema version: "
        ...
        private boolean patchSucceeded() {
            return state == STATE.ALREADY_APPLIED
        ...
        private boolean savePatch() {
            return state == STATE.DOES_NOT_APPLY
                    || state == STATE.APPLIED
                    || state == STATE.PRECEEDED
                    || state == STATE.FAILED;
        }
        ...
        public void execute() {
            ...
            if (!patch.requiresTransaction()
                    && AlfrescoTransactionSupport.isActualTransactionActive()) {
                throw new AlfrescoRuntimeException(
                        "Patch "
                                + patch.getId()
                                + " has been configured with requiresTransaction set to false but"
                                + " is being called in a transaction");
            }
        ...
        /**
         * Perform some setup before applying the patch e.g. check whether the patch needs to be
         * applied.
         */
        private void setup() {
            transactionService
                    ...
                    .doInTransaction(
                            new RetryingTransactionCallback<Object>() {
                                ...
                                public Object execute() throws Throwable {
                                    ...
                                    preceededByAlternative =
                                            forcePatch ? null : preceededByAlternative(patch);
                                    if (preceededByAlternative != null) {
                                        report =
                                                I18NUtil.getMessage(
                                                        MSG_PRECEEDED_BY_ALTERNATIVE,
                                                        preceededByAlternative);
                                        state = STATE.PRECEEDED;
                                    } else {
                                        if (applies) {
                    ...
                    .doInTransaction(
                            new RetryingTransactionCallback<Object>() {
                                ...
                                public Object execute() throws Throwable {
                                    ...
                                    if (server.length() > 64) {
                                        logger.error(
                                                "Server version '"
                                                        + server
                                                        + "' is too long for the"
                                                        + " 'applied_to_server' column therefore"
                                                        + " patch '"
                                                        + patch.getId()
                                    ...
                                    appliedPatch.setTargetSchema(
                                            patch.getTargetSchema()); // the schema the server is
                                    // expecting
                                    appliedPatch.setAppliedToSchema(
                                            repoDescriptor
                                                    .getSchema()); // the old schema of the repo
                                    appliedPatch.setAppliedToServer(
                                            server); // the current version and label of the server
                                    appliedPatch.setAppliedOnDate(new Date()); // the date applied
                                    appliedPatch.setSucceeded(
                                            patchSucceeded()); // whether or not the patch succeeded
                                    appliedPatch.setWasExecuted(
                                            applies); // whether or not the patch was executed
                                    appliedPatch.setReport(
                                            report); // additional, human-readable, status

...
/**
 * A {@link BpmnParseHandler} that adds execution listeners to a {@link UserTask} which are
 * specifically for Alfresco usage.
 *
 * @author Joram Barrez
...
public class AlfrescoUserTaskBpmnParseHandler extends AbstractBpmnParseHandler<UserTask> {
    ...
    protected void executeParse(BpmnParse bpmnParse, UserTask userTask) {
        ...
        if (activitybehaviour instanceof UserTaskActivityBehavior) {
            ...
            MultiInstanceActivityBehavior multiInstance =
                    (MultiInstanceActivityBehavior) activitybehaviour;
    ...
    protected void addListeners(UserTaskActivityBehavior activityBehavior) {
        ...
        if (completeTaskListener != null) {
            addTaskListenerAsFirst(
                    completeTaskListener, TaskListener.EVENTNAME_COMPLETE, activityBehavior);
        }
        if (notificationTaskListener != null) {
            addTaskListenerAsLast(
                    notificationTaskListener, TaskListener.EVENTNAME_CREATE, activityBehavior);
        }
    }
    ...
    protected void addTaskListenerAsFirst(
            TaskListener taskListener, String eventName, UserTaskActivityBehavior userTask) {
        getOrCreateListenerList(eventName, userTask).add(0, taskListener);
    }

    protected void addTaskListenerAsLast(
            TaskListener taskListener, String eventName, UserTaskActivityBehavior userTask) {
        getOrCreateListenerList(eventName, userTask).add(taskListener);
    }

    protected List<TaskListener> getOrCreateListenerList(
            String eventName, UserTaskActivityBehavior userTask) {
        List<TaskListener> taskEventListeners =
                userTask.getTaskDefinition().getTaskListeners().get(eventName);

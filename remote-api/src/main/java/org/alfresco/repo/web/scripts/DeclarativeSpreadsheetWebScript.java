...
/**
 * Parent of Declarative Webscripts that generate Excel files, usually based on some sort of
 * dictionary model.
 *
 * @author Nick Burch
 */
public abstract class DeclarativeSpreadsheetWebScript extends DeclarativeWebScript {
    ...
    /**
     * If the format is requested as HTML, should an exception be raised, or should an HTML version
     * be called?
     */
    protected abstract boolean allowHtmlFallback();

    /**
     * Returns the QNames of the model properties to be output in the header, and if they're
     * required or not
     */
    protected abstract List<Pair<QName, Boolean>> buildPropertiesForHeader(
            Object resource, String format, WebScriptRequest req);

    /**
     * Populates the body of the Excel Workbook, once the header has been output. This is called if
     * the format is .xls or .xlsx
     */
    protected abstract void populateBody(
            Object resource, Workbook workbook, Sheet sheet, List<QName> properties)
            throws IOException;

    /**
     * Populates the body of the CSV file, once the header has been output. This is called if the
     * format is .csv
     */
        ...
        if ("csv".equals(format)
            ...
            try {
                ...
                throw new WebScriptException(
                        Status.STATUS_BAD_REQUEST, "Unable to generate template file", e);
            }
        }
    ...
    public void generateSpreadsheet(
        ...
        List<Pair<QName, Boolean>> propertyDetails =
                buildPropertiesForHeader(resource, format, req);
        ...
        if ("csv".equals(format)) {
            ...
            CSVPrinter csv =
                    new CSVPrinter(sw, reqCSVFormat != null ? reqCSVFormat : getCsvFormat());
    ...
    protected Map<String, Object> createTemplateParameters(
            WebScriptRequest req, WebScriptResponse res, Map<String, Object> customParams) {

...
public class AsynchronousActionExecutionQueueImpl
  ...
  private static Log logger = LogFactory.getLog(
    AsynchronousActionExecutionQueueImpl.class
  );
  ...
  private void invokeOnAsyncActionExecutePolicy(
    Action action,
    NodeRef actionedUponNodeRef
  ) {
    // Execute the policy, passing it all details, firing as a general action case
    AsynchronousActionExecutionQueuePolicies.OnAsyncActionExecute policy = onAsyncActionExecuteDelegate.get(
      actionedUponNodeRef,
  ...
  public void registerActionFilter(AbstractAsynchronousActionFilter filter) {
    ...
    if (logger.isDebugEnabled()) {
      ...
      msg
        .append("Registered asynchronous action filter ")
    ...
    AbstractAsynchronousActionFilter existingFilter = actionFilters.get(
      filterName
    );
    if (logger.isDebugEnabled() && existingFilter != null) {
      StringBuilder msg = new StringBuilder();
      msg
        .append("This replaces previous filter ")
  ...
  public void executeAction(
    ...
    if (logger.isDebugEnabled()) {
      StringBuilder msg = new StringBuilder();
      msg
        .append("Received request to execute async action ")
      ...
      msg
        .append("ThreadPool's active count = ")
        .append(this.threadPoolExecutor.getActiveCount());
      logger.debug(msg.toString());

      msg = new StringBuilder();
      msg
        .append("ThreadPool's queue size = ")
        .append(this.threadPoolExecutor.getQueue().size());
      logger.debug(msg.toString());
    }

    Set<RuleServiceImpl.ExecutedRuleData> executedRules = (Set<RuleServiceImpl.ExecutedRuleData>) AlfrescoTransactionSupport.getResource(
    ...
    OngoingAsyncAction nodeBeingNewlyActioned = new OngoingAsyncAction(
      actionedUponNodeRef,
      action
    );
    ...
    for (Entry<String, AbstractAsynchronousActionFilter> entry : actionFilters.entrySet()) {
      ...
      if (
        actionDefinitionName.equals(action.getActionDefinitionName()) == false
      ) {
        ...
        synchronized (this.ongoingActions) {
          for (OngoingAsyncAction ongoingAction : this.ongoingActions) {
            if (
              comparator.compare(ongoingAction, nodeBeingNewlyActioned) == 0
            ) {
    ...
    if (newActionShouldBeFilteredOut) {
      if (logger.isDebugEnabled()) {
        StringBuilder msg = new StringBuilder();
        msg
          .append("Dropping action ")
          .append(action)
          .append(" as equivalent is ongoing.");
        logger.debug(msg.toString());
      }
      return;
    } else {
  ...
  private class ActionExecutionWrapper implements Runnable {
    ...
    public void run() {
      try {
        ...
        (
          (ActionImpl) ActionExecutionWrapper.this.action
        ).setExecuteAsynchronously(true);
        ...
        Throwable rootCause = (e instanceof AlfrescoRuntimeException)
        ...
        if (
          !ActionExecutionWrapper.this.actionService.onLogException(
              action,
              logger,
              rootCause,
              message

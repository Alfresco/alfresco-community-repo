...
public class RecordServiceImplTest
    ...
    doTestInTransaction(
      new Test<Void>() {
        ...
        public void test(Void result) {
          ...
          // location
          assertTrue(
            nodeService.hasAspect(dmDocument, ASPECT_RECORD_ORIGINATING_DETAILS)
          );
          assertEquals(
            originalLocation,
          ...
          Capability editRecordMetadata = capabilityService.getCapability(
            "EditNonRecordMetadata"
          );
          assertEquals(
            AccessStatus.ALLOWED,
            editRecordMetadata.hasPermission(dmDocument)
          );

          Capability updateProperties = capabilityService.getCapability(
            "UpdateProperties"
          );
          assertEquals(
            AccessStatus.ALLOWED,
            updateProperties.hasPermission(dmDocument)
          );
        }
      },
      dmCollaborator
  ...
  public void testCreateRecordNoLink() throws Exception {
    ...
    // create record from document
    final NodeRef originalLocation = doTestInTransaction(
      new Test<NodeRef>() {
        @Override
        public NodeRef run() {
          NodeRef originalLocation = nodeService
            .getPrimaryParent(dmDocument)
            .getParentRef();
  ...
  private void checkPermissions(
    ...
    assertEquals(
      unfiledExpected,
      permissionService.hasPermission(unfiledContainer, permission)
    );
    assertEquals(
      recordCatExpected,
      permissionService.hasPermission(rmContainer, permission)
    );
    assertEquals(
      recordFolderExpected,
      permissionService.hasPermission(rmFolder, permission)
    );
    assertEquals(
      recordExpected,
      permissionService.hasPermission(dmDocument, permission)
    );
  }
        ...
        public void runImpl() throws Exception {
          assertTrue(
            recordService.isPropertyEditable(
              recordOne,
              RecordsManagementModel.PROP_LOCATION
            )
          );
          assertTrue(
            recordService.isPropertyEditable(recordOne, PROP_DESCRIPTION)
          );
          assertTrue(
            recordService.isPropertyEditable(
              recordDeclaredOne,
  ...
  private void cantEditProperty(
    final NodeRef nodeRef,
    final QName property,
    String user
  ) throws Exception {
    ...
    doTestInTransaction(
      new Test<Void>() {
        ...
        public Void run() {
          ...
          policyComponent.removeClassDefinition(

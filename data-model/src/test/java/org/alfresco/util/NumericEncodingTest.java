...
public class NumericEncodingTest extends TestCase {
        ...
        for (int exponent = 0; exponent <= 0xFF; exponent++) {
          for (int mantissa = 0; mantissa <= 0x007FFFFF; mantissa++) {
            ...
            if (!next.equals(Float.NaN)) {
              ...
              if (
                (lastString != null) && (lastString.compareTo(nextString) > 0)
              ) {
  ...
  public void testLongEncoding() {
    ...
    assertEquals(
      NumericEncoder.decodeLong(NumericEncoder.encode(Long.MIN_VALUE)),
      Long.MIN_VALUE
    );
    assertEquals(
      NumericEncoder.decodeLong(NumericEncoder.encode(Long.MIN_VALUE + 1)),
      Long.MIN_VALUE + 1
    );
    ...
    assertEquals(NumericEncoder.decodeLong(NumericEncoder.encode(1L)), 1L);
    assertEquals(
      NumericEncoder.decodeLong(NumericEncoder.encode(Long.MAX_VALUE - 1)),
      Long.MAX_VALUE - 1
    );
    assertEquals(
      NumericEncoder.decodeLong(NumericEncoder.encode(Long.MAX_VALUE)),
      Long.MAX_VALUE
    );
  }
  ...
  public void testDoubleEncoding() {
    ...
    assertTrue(
      NumericEncoder.encode(-0.9).compareTo(NumericEncoder.encode(0.88)) < 0
    );
    assertTrue(
      NumericEncoder.encode(-0.9).compareTo(NumericEncoder.encode(0.91)) < 0
    );
    assertTrue(

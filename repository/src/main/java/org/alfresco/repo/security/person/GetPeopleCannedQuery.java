...
public class GetPeopleCannedQuery extends AbstractCannedQuery<NodeRef> {
    ...
    protected List<NodeRef> queryAndFilter(CannedQueryParameters parameters) {
        ...
        // Get parameters
        GetPeopleCannedQueryParams paramBean =
                (GetPeopleCannedQueryParams) parameters.getParameterBean();
        ...
        if ((pattern == null) || (pattern.equals(""))) {
            // note: although no pattern means no filtering required, set to match all in case sort
            // required
            pattern = "%";
        ...
        if (paramBean.getExclusiveAspects() != null) {
            Set<Long> qnamesIds =
                    qnameDAO.convertQNamesToIds(paramBean.getExclusiveAspects(), false);
            params.setExcludeAspectIds(new ArrayList<Long>(qnamesIds));
        }

        if (paramBean.getInclusiveAspects() != null) {
            Set<Long> qnamesIds =
                    qnameDAO.convertQNamesToIds(paramBean.getInclusiveAspects(), false);
        ...
        for (Pair<QName, SortOrder> sort : sortPairs) {
            ...
            if ((filterProps.size() > 0) && (!filterProps.contains(sortQName))) {
                throw new AlfrescoRuntimeException(
                        "GetPeople: cannot sort by a non-filter property: "
                                + sortQName
                                + " (filterStringProps="
                                + filterProps
                                + ")");
        ...
        if (filterSortPropCnt > MAX_FILTER_SORT_PROPS) {
            throw new AlfrescoRuntimeException(
                    "GetPeople: exceeded maximum number filter/sort properties: (max="
        ...
        final PersonQueryCallback c =
                new DefaultPersonQueryCallback(result, paramBean.getIncludeAdministrators());
        ...
        int origLimit =
                totalResultCountMax > 0
                        ? totalResultCountMax
                        : parameters.getPageDetails().getPageSize();
    ...
    private int setFilterSortParams(
            ...
            Map<QName, Boolean> sortAsc,
            FilterSortPersonEntity params) {
        int cnt = 0;
        int propCnt = 0;
        ...
        for (QName filterSortProp : filterSortProps) {
            ...
            Boolean sortOrder =
                    sortAsc.get(filterSortProp); // true = ascending, false = descending, null =
            // unsorted
            ...
            if (sortQNameId != null) {
                if (propCnt == 0) {
                    ...
                    // belts and braces
                    throw new AlfrescoRuntimeException(
                            "GetPeople: unexpected - cannot set sort parameter: " + cnt);
                }
    ...
    protected class DefaultPersonQueryCallback implements PersonQueryCallback {
        ...
        public DefaultPersonQueryCallback(
                final List<NodeRef> children, boolean includeAdministrators) {
        ...
        public boolean handle(NodeRef personRef) {
            ...
            if (includeAdministrators == false) {
                String userName =
                        (String) nodeService.getProperty(personRef, ContentModel.PROP_USERNAME);
    ...
    protected class PersonResultHandler implements CannedQueryDAO.ResultHandler<String> {
        ...
        public boolean handleResult(String uuid) {
            // Call back
            return resultsCallback.handle(
                    new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, uuid));
        }

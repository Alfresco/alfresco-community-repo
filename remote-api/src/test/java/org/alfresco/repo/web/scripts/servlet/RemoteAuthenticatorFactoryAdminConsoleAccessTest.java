...
public class RemoteAuthenticatorFactoryAdminConsoleAccessTest
  ...
  public void before() throws Exception {
    ...
    DefaultChildApplicationContextManager childApplicationContextManager = (DefaultChildApplicationContextManager) applicationContext.getBean(
      "Authentication"
    );
    remoteUserAuthenticatorFactory =
      (RemoteUserAuthenticatorFactory) applicationContext.getBean(
        "webscripts.authenticator.remoteuser"
      );
    remoteUserAuthenticatorFactory.setRemoteUserMapper(
      blockingRemoteUserMapper
    );
    remoteUserAuthenticatorFactory.setGetRemoteUserTimeoutMilliseconds(
      (long) (BlockingRemoteUserMapper.BLOCKING_FOR_MILLIS / 2)
    ); //highly impatient
    ...
    authenticationService =
      applicationContext.getBean(
        "AuthenticationService",
        MutableAuthenticationService.class
      );
    ...
    authorityService =
      applicationContext.getBean("AuthorityService", AuthorityService.class);
    ...
    childApplicationContextFactory.setProperty(
  ...
  public void testAdminGuestAccess() {
    ...
    final boolean authenticated = authenticateWithGuestParameters(
      required,
      isGuest
    );
  ...
  public void testUserCanAccessAdminConsoleScript() {
    ...
    checkAdminConsoleFamilyWithBasicAuthHeaderPresentUser(
      families,
      headerToAdd
    );
  }
  ...
  private void checkAdminConsoleFamilyWithBasicAuthHeaderButWrongPassword(
    Set<String> families
  ) {
    ...
    final boolean authenticated = authenticate(
      families,
      headerToAddWithWrongPassword
    );

    assertFalse(
      "It is an AdminConsole webscript now and Admin basic auth header was present BUT with wrong password. Should fail.",
      authenticated
    );
    assertEquals("Status should be 401", 401, setStatusCode);
    final String message =
      "The code from blockingRemoteUserMapper shouldn't have been called";
  ...
  private void checkAdminConsoleFamilyPage(Set<String> families) {
    ...
    assertFalse(
      "It is an AdminConsole webscript now, but Admin basic auth header was not present. It should return 401",
      authenticated
    );
    assertEquals("Status should be 401", 401, setStatusCode);
    assertTrue(
  ...
  private void checkAdminConsoleFamilyWithBasicAuthHeaderPresent(
    ...
    assertTrue(
      "It is an AdminConsole webscript now and Admin basic auth header was present. It should succeed.",
      authenticated
    );
  ...
  private void checkAdminConsoleFamilyWithBasicAuthHeaderPresentUser(
    ...
    assertTrue(
      "It is an AdminConsole webscript and a User with Admin access and basic auth header was present. It should succeed.",
      authenticated
    );
    // status is not checked here, as it is not returned by this framework we are testing. It should eventually be 200
    final String message =
  ...
  private void checkAdminConsoleFamilyPageWithRemoteUserMapperDisabled(
    Set<String> families
  ) {
    blockingRemoteUserMapper.reset();
    ...
    assertEquals("Status should be 401", 401, setStatusCode);
    assertFalse(
      "The interrupt should have not been called because the RemoteUserMapper is not enabled.",
      blockingRemoteUserMapper.isWasInterrupted()
    );
    assertEquals(
      "RemoteUserMapper not called",
      blockingRemoteUserMapper.getTimePassed(),
      0
    );
  }
  ...
  private void checkGenericResourceAccess() {
    ...
    assertFalse(
      "This should not be authenticated as it is not an Admin Console requested. And no credentials have been provided",
      authenticated
    );
    assertFalse(
      "Because it is not an Admin Console, the timeout feature from BasicHttpAuthenticator should not be requested. " +
      "Therefore the interrupt should not have been called. ",
      blockingRemoteUserMapper.isWasInterrupted()
    );
    assertTrue(
  ...
  private void checkTimeOutFeaturesWasNotUsed() {
    assertFalse(
      "The timeout feature from BasicHttpAuthenticator should not be requested. Therefore the interrupt should not have been called. ",
      blockingRemoteUserMapper.isWasInterrupted()
    );
    assertTrue(
      "No interrupt should have been called.",
  ...
  private boolean authenticate(Set<String> families, String headerToAdd) {
    WebScriptServletRequest mockRequest = prepareMockRequest(
      families,
      headerToAdd
    );
    WebScriptServletResponse mockResponse = prepareMockResponse();

    Authenticator authenticator = remoteUserAuthenticatorFactory.create(
      mockRequest,
      mockResponse
    );
  ...
  private WebScriptServletRequest prepareMockRequest(
    Set<String> families,
    String headerToAdd
  ) {
  ...
  private void checkExtAuthStillWorks(
    RequiredAuthentication required,
    Set<String> families
  ) {
    blockingRemoteUserMapper.reset();
    ...
    assertTrue(
      "This should be authenticating with external auth",
      authenticated
    );
    assertFalse(
      "We have been using the DefaultRemoteUserMapper, so our BlockingRemoteUserMapper shouldn't have been called",
      blockingRemoteUserMapper.isWasInterrupted()
    );
    assertEquals(
  ...
  private WebScriptServletResponse prepareMockResponse() {
    ...
    WebScriptServletResponse mockResponse = mock(
      WebScriptServletResponse.class
    );

...
/**
 * Delegate for a Class Feature-level (Property and Association) Policies. Provides access to Policy
 * Interface implementations which invoke the appropriate bound behaviours.
 *
...
public class PropertyPolicyDelegate<P extends PropertyPolicy> {
    ...
    /*package*/ PropertyPolicyDelegate(
            DictionaryService dictionary,
            Class<P> policyClass,
            BehaviourIndex<ClassFeatureBehaviourBinding> index,
            long tryLockTimeout) {
    ...
    private void checkPropertyType(QName propertyQName) throws IllegalArgumentException {
        ...
        if (propertyDef == null) {
            throw new IllegalArgumentException(
                    "Property " + propertyQName + " has not been defined in the data dictionary");
        }
    }
     ...
     *
     * <p>When multiple behaviours are bound to the policy for the class feature, an aggregate
     * policy implementation is returned which invokes each policy in turn.
     *
    ...
    public P get(NodeRef nodeRef, QName classQName, QName propertyQName) {
        ...
        return factory.create(
                new ClassFeatureBehaviourBinding(dictionary, nodeRef, classQName, propertyQName));
    }
    ...
    public Collection<P> getList(NodeRef nodeRef, Set<QName> classQNames, QName propertyQName) {
        ...
        for (QName classQName : classQNames) {
            P policy =
                    factory.create(
                            new ClassFeatureBehaviourBinding(
                                    dictionary, nodeRef, classQName, propertyQName));

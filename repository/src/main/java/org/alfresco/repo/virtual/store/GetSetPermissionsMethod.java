...
public class GetSetPermissionsMethod extends AbstractProtocolMethod<NodePermissionEntry> {
    ...
    public GetSetPermissionsMethod(
    ...
    private NodePermissionEntry execute(
        ...
        for (String permission : toAllow) {
            PermissionReference permissionReference =
                    PermissionReferenceImpl.getPermissionReference(
                            userPermissions.getPermissionTypeQName(), permission);
            permissions.add(
                    new SimplePermissionEntry(
                            rNodeRef, permissionReference, authority, AccessStatus.ALLOWED));
        }
        ...
        for (String permission : toDeny) {
            ...
            permissions.add(
                    new SimplePermissionEntry(
                            rNodeRef, permissionReference, authority, AccessStatus.DENIED));
        }
    ...
    @Override
    public NodePermissionEntry execute(NodeProtocol protocol, Reference reference)
            throws ProtocolMethodException {

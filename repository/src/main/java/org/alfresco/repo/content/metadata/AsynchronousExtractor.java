...
public class AsynchronousExtractor extends AbstractMappingMetadataExtracter {
  ...
  private static final String MIMETYPE_METADATA_EXTRACT =
    "alfresco-metadata-extract";
  private static final String MIMETYPE_METADATA_EMBED =
  ...
  public void setTransformServiceRegistry(
    TransformServiceRegistry transformServiceRegistry
  ) {
  ...
  public void setMetadataExtractorPropertyMappingOverrides(
    List<MetadataExtractorPropertyMappingOverride> metadataExtractorPropertyMappingOverrides
  ) {
    this.metadataExtractorPropertyMappingOverrides =
  ...
  public static String getTargetMimetypeFromTransformName(
  ...
  public static String getSourceMimetypeFromTransformName(
    String transformName
  ) {
    return transformName == null
      ? null
  ...
  private Map<String, String> getExtractOptions(
    ...
    for (MetadataExtractorPropertyMappingOverride override : metadataExtractorPropertyMappingOverrides) {
      if (override.match(sourceMimetype)) {
        Map<String, Set<String>> extractMapping = override.getExtractMapping(
          nodeRef
        );
  ...
  protected Map<String, Serializable> mapSystemToRaw(
    ...
    Map<String, Serializable> metadataProperties = new HashMap<>(
      systemMetadata.size()
    );
  ...
  private void transform(
    ...
    TransformDefinition transformDefinition = (TransformDefinition) renditionDefinitionRegistry2.getRenditionDefinition(
      renditionName
    );
    ...
    try {
      ...
      if (
        e
          .getMessage()
          .endsWith(
            "The supplied sourceNodeRef " + nodeRef + " does not exist."
          )
      ) {
  ...
  public void setMetadata(NodeRef nodeRef, InputStream transformInputStream) {
    ...
    OverwritePolicy overwritePolicy = removeOverwritePolicy(
      metadata,
      "sys:overwritePolicy",
      OverwritePolicy.PRAGMATIC
    );
    Boolean enableStringTagging = removeBoolean(
      metadata,
      "sys:enableStringTagging",
      false
    );
    Boolean carryAspectProperties = removeBoolean(
      metadata,
      "sys:carryAspectProperties",
      true
    );
    List<String> stringTaggingSeparators = removeTaggingSeparators(
      metadata,
        ...
        transactionService
          ...
          .doInTransaction(
            () -> {
              ...
              Map<QName, Serializable> systemProperties = convertKeysToQNames(
                metadata
              );
              ...
              Map<QName, Serializable> changedProperties = overwritePolicy.applyProperties(
                systemProperties,
                nodeProperties
              );
              ...
              if (transformerDebugEnabled || debugEnabled) {
                for (Map.Entry<QName, Serializable> entry : changedProperties.entrySet()) {
                  ...
                  String prefixString = qname.toPrefixString(
                    namespacePrefixResolver
                  );
                  String debugMessage =
                    prefixString + "=" + (value == null ? "" : value);
                  if (transformerDebugEnabled) {
                    transformerDebug.debugUsingPreviousReference(
                      "  " + debugMessage
                    );
                  }
                  if (debugEnabled) {
              ...
              ContentMetadataExtracter.addExtractedMetadataToNode(
                nodeRef,
                nodeProperties,
                changedProperties,
                nodeService,
  ...
  private List<String> removeTaggingSeparators(
    ...
    try (
      CSVParser parser = CSVParser.parse((String) value, CSVFormat.RFC4180)
    ) {
      Iterator<CSVRecord> iterator = parser.iterator();
      CSVRecord record = iterator.next();
      if (iterator.hasNext()) {
        logger.error(
          key + "=" + value + " is invalid. Should only have one record"
        );
        return null;
      }
      ...
      logger.error(
        key + "=" + value + " is invalid. Must be a CSV using CSVFormat.RFC4180"
      );
      return null;
    }
    return list;
  ...
  public void setEmbeddedMetadata(
    NodeRef nodeRef,
    InputStream transformInputStream
  ) {
        ...
        .doInTransaction(
          () -> {
            try {
              ...
              logger.error(
                "Failed to copy embedded metadata transform InputStream into " +
                nodeRef
              );
              throw e;
            }

...
public class CalendarAllDayEventDatesCorrectingPatch extends AbstractPatch {
  ...
  private static final Logger LOGGER = Logger.getLogger(
    CalendarAllDayEventDatesCorrectingPatch.class
  );
  ...
  protected String applyInternal() throws Exception {
    ...
    List<ChildAssociationRef> allSites = (null != siteRoot)
      ? (nodeService.getChildAssocs(siteRoot))
      : (null);
      ...
      for (ChildAssociationRef siteAssoc : allSites) {
        ...
        if (null != site) {
          ...
          do {
            ...
            List<CalendarEntry> page = (null != entries)
              ? (entries.getPage())
              : (null);
            ...
            if (null != page) {
              if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(
                  "Processing " + page.size() + " Calendar Events..."
                );
              }
              ...
              for (CalendarEntry entry : page) {
                if (isAllDay(entry)) {
                  ...
                  if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug(
                      "'All Day' Calendar event has been detected: [title: " +
                      entry.getTitle() +
                      ", start: " +
                      entry.getStart() +
                      ", end: " +
  ...
  public static boolean isAllDay(CalendarEntry entry) {
    ...
    if (
      startLocal.get(Calendar.HOUR_OF_DAY) == 0 &&

...
public class CannedQueryTest extends TestCase {
  ...
  public void setUp() throws Exception {
    ...
    // ONE
    TestCannedQueryFactory<String> namedQueryFactoryOne = new TestCannedQueryFactory(
      RESULTS_ONE
    );
    ...
    // TWO
    TestCannedQueryFactory<Long> namedQueryFactoryTwo = new TestCannedQueryFactory(
      RESULTS_TWO
    );
  ...
  public void testRegistry() throws Exception {
    CannedQueryFactory<String> one = namedQueryFactoryRegistry.getNamedObject(
      QUERY_TEST_ONE
    );
    assertNotNull("No factory for " + QUERY_TEST_ONE, one);
    CannedQueryFactory<String> two = namedQueryFactoryRegistry.getNamedObject(
      QUERY_TEST_TWO
    );
    assertNotNull("No factory for " + QUERY_TEST_TWO, two);
    // Kick out registrations with incorrect naming convention
    try {
      TestCannedQueryFactory<Long> namedQueryFactoryBogus = new TestCannedQueryFactory(
        RESULTS_TWO
      );
  ...
  public void testQueryAllResults() throws Exception {
    ...
    CannedQueryFactory<String> qfOne = namedQueryFactoryRegistry.getNamedObject(
      QUERY_TEST_ONE
    );
    ...
    try {
      ...
      fail(
        "Expected failure when requesting total count without explicit request."
      );
    } catch (IllegalStateException e) {
      // Expected
    }
    ...
    assertEquals(
      "No sorting was specified in the parameters",
      "ONE_0",
      qrOne.getPages().get(0).get(0)
    );
  ...
  public void testQueryPagedResults() throws Exception {
    ...
    CannedQueryPageDetails qPageDetails = new CannedQueryPageDetails(
      0,
      5,
      1,
      2
  ...
  public void testQueryPermissionCheckedResults() throws Exception {
    ...
    assertEquals(
      "Incorrect result order",
      "ONE_1",
      qrOne.getPages().get(0).get(1)
    );
    assertEquals(
      "Incorrect result order",
      "ONE_2",
      qrOne.getPages().get(0).get(2)
    );
    assertEquals(
      "Incorrect result order",
      "ONE_3",
      qrOne.getPages().get(0).get(3)
    );
    assertEquals(
      "Incorrect result order",
      "ONE_4",
      qrOne.getPages().get(0).get(4)
    ); // << missing 5!
    assertEquals(
      "Incorrect result order",
      "ONE_6",
      qrOne.getPages().get(0).get(5)
    );
    assertEquals(
      "Incorrect result order",
      "ONE_7",
      qrOne.getPages().get(0).get(6)
    );
    assertEquals(
      "Incorrect result order",
      "ONE_8",
      qrOne.getPages().get(0).get(7)
    );
    assertEquals(
      "Incorrect result order",
      "ONE_9",
  ...
  public void testQueryPermissionCheckedPagedTotalCount() throws Exception {
    ...
    // Check pages
    assertEquals(
      "Incorrect number of total results",
      new Pair<Integer, Integer>(9, 9),
      qrOne.getTotalResultCount()
    ); // Pre-paging
    assertEquals(
      "Incorrect number of paged results",
      1,
      qrOne.getPagedResultCount()
    ); // Skipped 5
    assertEquals(
      "Incorrect result order",
      "ONE_3",
  ...
  private static class TestCannedQuery<T> extends AbstractCannedQuery<T> {
    ...
    @Override
    protected List<T> applyPostQuerySorting(
      List<T> results,
    ...
    @Override
    protected List<T> applyPostQueryPermissions(
      List<T> results,
      int requestedCount
    ) {

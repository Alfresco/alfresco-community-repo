...
public class ImapCopyFolderTests extends EmailTest {
    ...
    public void adminShouldCopyNonEmptyFolder() throws Exception {
        ...
        copiedFolder.setProtocolLocation(
                Utility.buildPath(copyFolder.getProtocolLocation(), testFolder.getName()));
    ...
    public void siteManagerShouldCopyFolder() throws Exception {
        ...
        copiedFolder.setCmisLocation(
                Utility.buildPath(copyFolder.getCmisLocation(), testFolder.getName()));
        copiedFolder.setProtocolLocation(
                Utility.buildPath(copyFolder.getProtocolLocation(), testFolder.getName()));
        imapProtocol
    ...
    public void collaboratorShouldCopyFolder() throws Exception {
        ...
        testFolder = dataContent.usingUser(testUser).usingSite(testSite).createFolder();
        FolderModel copyFolder =
                dataContent.usingUser(collaboratorUser).usingSite(testSite).createFolder();
        FolderModel copiedFolder = testFolder;
        copiedFolder.setProtocolLocation(
                Utility.buildPath(copyFolder.getProtocolLocation(), testFolder.getName()));
        dataContent.usingResource(testFolder).createContent(CMISUtil.DocumentType.TEXT_PLAIN);
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify copy folder to a location for which the user does not have permission")
    @Test(
            groups = {TestGroup.PROTOCOLS, TestGroup.IMAP, TestGroup.CORE},
    ...
    public void userCopyFolderToALocationThatHeDoesNotHaveAccess() throws Exception {
        ...
        dataContent.usingResource(testFolder).createContent(CMISUtil.DocumentType.TEXT_PLAIN);
        imapProtocol.authenticateUser(testUser).usingResource(testFolder).copyTo(copyFolder);
    }
}

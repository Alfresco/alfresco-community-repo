...
public class LuceneIn<Q, S, E extends Throwable>
  ...
  public Q addComponent(
    ...
    String modeString = DefaultTypeConverter.INSTANCE.convert(
      String.class,
      modeArgument.getValue(functionContext)
    );
    ...
    Q query = functionContext.buildLuceneIn(
      lqpa,
    ...
    if (query == null) {
      throw new QueryModelException(
        "No query time mapping for property  " +
        propertyArgument.getPropertyName() +
        ", it should not be allowed in predicates"
      );
    }

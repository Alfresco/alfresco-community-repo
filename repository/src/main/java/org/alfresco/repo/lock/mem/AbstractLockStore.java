...
public abstract class AbstractLockStore<T extends ConcurrentMap<NodeRef, LockState>>
    ...
    public void set(NodeRef nodeRef, LockState lockState) {
        ...
        if (!updated) {
            String msg =
                    String.format(
                            "Attempt to update lock state failed, old=%s, new=%s, noderef=%s",
    ...
    protected Map<NodeRef, LockState> getTxMap() {
        ...
        Map<NodeRef, LockState> map =
                TransactionalResourceHelper.getMap(getClass().getName() + ".repeatableReadMap");
        return map;
    }

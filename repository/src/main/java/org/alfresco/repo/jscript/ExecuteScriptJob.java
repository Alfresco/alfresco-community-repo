...
public class ExecuteScriptJob implements Job {
  ...
  private static final String PARAM_AUTHENTICATION_COMPONENT =
  ...
  public void execute(JobExecutionContext context)
    ...
    if (
      scriptServiceObj == null || !(scriptServiceObj instanceof ScriptService)
    ) {
    ...
    if (
      scriptLocationObj == null ||
      !(scriptLocationObj instanceof ScriptLocation)
    ) {
      throw new AlfrescoRuntimeException(
    ...
    Object authenticationComponentObj = jobData.get(
      PARAM_AUTHENTICATION_COMPONENT
    );
    if (
      authenticationComponentObj == null ||
      !(authenticationComponentObj instanceof AuthenticationComponent)
    ) {
      throw new AlfrescoRuntimeException(
    ...
    (
      (AuthenticationComponent) authenticationComponentObj
    ).setSystemUserAsCurrentUser();
    try {
      // Execute the script
      ((ScriptService) scriptServiceObj).executeScript(
          (ScriptLocation) scriptLocationObj,
          null
        );
    } finally {
      (

...
public class TaskFormProcessorTest extends FormProcessorTest {
    ...
    private static final NodeRef USER_NODE =
            new NodeRef(NamespaceService.CONTENT_MODEL_1_0_URI + "/admin");
    ...
    public void testGenerateDefaultForm() throws Exception {
        ...
        assertEquals(
                WorkflowTaskState.IN_PROGRESS, formData.getFieldData("prop_bpm_status").getValue());
    }
    ...
    public void testGeneratePackageItems() throws Exception {
        ...
        packageItems =
                (Serializable)
                        Arrays.asList(
                                FAKE_NODE.toString(), FAKE_NODE2.toString(), FAKE_NODE3.toString());
        checkSingleAssociation(form, fieldName, packageItems);
    }
    ...
    protected void setUp() throws Exception {
        ...
        MockFieldProcessorRegistry fieldProcessorRegistry =
                new MockFieldProcessorRegistry(namespaceService, dictionaryService);
        DefaultFieldProcessor defaultProcessor = super.makeDefaultFieldProcessor(dictionaryService);
        super.processor =
                makeTaskFormProcessor(dictionaryService, fieldProcessorRegistry, defaultProcessor);
    }
    ...
    private WorkflowTask makeTask(WorkflowTransition... transitions) {
        ...
        WorkflowDefinition definition =
                new WorkflowDefinition("42", "Test", "1.0", "Test", "Test", null);
        NodeRef wfPackage = PCKG_NODE;
        WorkflowInstance instance =
                new WorkflowInstance(
                        null, definition, null, null, wfPackage, null, true, null, null);
    ...
    private Map<QName, PropertyDefinition> makeTaskPropertyDefs() {
        ...
        PropertyDefinition descValue =
                MockClassAttributeDefinition.mockPropertyDefinition(DESC_NAME, textType);
        properties.put(DESC_NAME, descValue);

        // Add a Status property
        PropertyDefinition titleValue =
                MockClassAttributeDefinition.mockPropertyDefinition(STATUS_NAME, textType);
        properties.put(STATUS_NAME, titleValue);

        // Add a Status property
        PropertyDefinition with_ =
                MockClassAttributeDefinition.mockPropertyDefinition(PROP_WITH_, textType);
        ...
        PropertyDefinition pckgItemAction =
                MockClassAttributeDefinition.mockPropertyDefinition(
                        pckgItemActionGroup, textType, "read_package_item_actions");
        ...
        PropertyDefinition priorityDef =
                MockClassAttributeDefinition.mockPropertyDefinition(
                        priorityName, DataTypeDefinition.INT, Integer.class, "0");
    ...
    private NamespaceService makeNamespaceService() {
        ...
        nsService.registerNamespace(
                NamespaceService.RENDITION_MODEL_PREFIX, NamespaceService.RENDITION_MODEL_1_0_URI);
        nsService.registerNamespace(
                ...
                .thenAnswer(
                        new Answer<WorkflowTask>() {
                            public WorkflowTask answer(InvocationOnMock invocation)
                                ...
                                if (TASK_ID.equals(id)) return task;
                                else {
                                    // if TaskId is not found then Activity returns null, does not
                ...
                .thenAnswer(
                        new Answer<WorkflowTask>() {
                            public WorkflowTask answer(InvocationOnMock invocation)
                                ...
                                Map<QName, List<NodeRef>> added =
                                        (Map<QName, List<NodeRef>>) args[2];
                                actualAdded = new HashMap<QName, List<NodeRef>>(added);
                                Map<QName, List<NodeRef>> removed =
                                        (Map<QName, List<NodeRef>>) args[3];
    ...
    private NodeService makeNodeService() {
        ...
        when(service.hasAspect(PCKG_NODE, ASPECT_WORKFLOW_PACKAGE)).thenReturn(true);

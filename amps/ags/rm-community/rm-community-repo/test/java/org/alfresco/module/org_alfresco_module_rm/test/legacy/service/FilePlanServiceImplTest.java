...
public class FilePlanServiceImplTest extends BaseRMTestCase {
  ...
  public void testIsFilePlanComponent() throws Exception {
    doTestInTransaction(
      new Test<NodeRef>() {
        ...
        public NodeRef run() {
          assertTrue(
            "The rm root container should be a rm component",
            filePlanService.isFilePlanComponent(filePlan)
          );
          assertTrue(
            "The rm container should be a rm component",
            filePlanService.isFilePlanComponent(rmContainer)
          );
          assertTrue(
            "The rm folder should be a rm component",
            filePlanService.isFilePlanComponent(rmFolder)
          );
    ...
    doTestInTransaction(
      new Test<NodeRef>() {
        ...
        public void test(NodeRef result) throws Exception {
          assertEquals(
            FilePlanComponentKind.FILE_PLAN,
            filePlanService.getFilePlanComponentKind(filePlan)
          );
          assertEquals(
            FilePlanComponentKind.RECORD_CATEGORY,
            filePlanService.getFilePlanComponentKind(rmContainer)
          );
          assertEquals(
            FilePlanComponentKind.RECORD_FOLDER,
            filePlanService.getFilePlanComponentKind(rmFolder)
          );
          assertEquals(
            FilePlanComponentKind.RECORD,
            filePlanService.getFilePlanComponentKind(result)
          );
  ...
  public void testIsRecordCategory() throws Exception {
    doTestInTransaction(
      new Test<NodeRef>() {
        ...
        public NodeRef run() {
          assertFalse(
            "This should not be a record category.",
            filePlanService.isRecordCategory(filePlan)
          );
          assertTrue(
            "This is a record category.",
            filePlanService.isRecordCategory(rmContainer)
          );
          assertFalse(
            "This should not be a record category.",
            filePlanService.isRecordCategory(rmFolder)
          );
  ...
  private void basicRMContainerCheck(NodeRef nodeRef, String name, QName type) {
    ...
    assertNotNull(
      "RM id has not been set",
      nodeService.getProperty(nodeRef, PROP_IDENTIFIER)
    );
  ...
  public void testCreateRecordCategory() throws Exception {
    ...
    doTestInTransaction(
      new Test<NodeRef>() {
        ...
        public void test(NodeRef result) {
          assertNotNull(
            "Unable to create records management container",
            result
          );
    ...
    doTestInTransaction(
      new Test<Void>() {
        ...
        public Void run() {
          ...
          NodeRef series = filePlanService.createRecordCategory(
            rmContainer,
            "rmSeries"
          );
          NodeRef seriesChildFolder = recordFolderService.createRecordFolder(
            series,
            "seriesRecordFolder"
          );
          NodeRef seriesChildContainer = filePlanService.createRecordCategory(
            series,
            "childContainer"
          );
  ...
  public void testGetContainedRecordCategories() throws Exception {
    ...
    doTestInTransaction(
      new Test<Void>() {
        ...
        @Override
        public void test(Void result) throws Exception {
          List<NodeRef> nodes = filePlanService.getContainedRecordCategories(
            rmContainer
          );
          assertNotNull(nodes);
          ...
          nodes =
            filePlanService.getContainedRecordCategories(rmContainer, false);
          assertNotNull(nodes);
          assertEquals(1, nodes.size());
          assertTrue(nodes.contains(getNodeRef("series")));

          nodes =
            filePlanService.getContainedRecordCategories(rmContainer, true);
          assertNotNull(nodes);
          assertEquals(2, nodes.size());
          assertTrue(nodes.contains(getNodeRef("series")));
          assertTrue(nodes.contains(getNodeRef("seriesChildContainer")));
  ...
  public void testGetContainedRecordFolders() throws Exception {
    ...
    doTestInTransaction(
      new Test<Void>() {
        ...
        @Override
        public void test(Void result) throws Exception {
          List<NodeRef> nodes = filePlanService.getContainedRecordFolders(
            rmContainer
          );
          assertNotNull(nodes);
          assertEquals(1, nodes.size());
    ...
    doTestInTransaction(
      new Test<Void>() {
        ...
        public Void run() {
          ...
          NodeRef levelOne = setNodeRef(
            "container1",
            filePlanService.createRecordCategory(filePlan, "container1")
          );
          assertNotNull("Unable to create container", levelOne);
          NodeRef levelTwo = setNodeRef(
            "container2",
            filePlanService.createRecordCategory(levelOne, "container2")
          );
          assertNotNull("Unable to create container", levelTwo);
          NodeRef levelThree = setNodeRef(
            "container3",
            filePlanService.createRecordCategory(levelTwo, "container3")
          );
          assertNotNull("Unable to create container", levelThree);
          NodeRef levelThreeRecordFolder = setNodeRef(
            "recordFolder3",
            recordFolderService.createRecordFolder(levelThree, "recordFolder3")
          );
          assertNotNull(
            "Unable to create record folder",
            levelThreeRecordFolder
          );
        ...
        public void test(Void result) {
          ...
          basicRMContainerCheck(
            getNodeRef("container1"),
            "container1",
            TYPE_RECORD_CATEGORY
          );
          basicRMContainerCheck(
            getNodeRef("container2"),
            "container2",
            TYPE_RECORD_CATEGORY
          );
          basicRMContainerCheck(
            getNodeRef("container3"),
            "container3",
            TYPE_RECORD_CATEGORY
          );
          basicRMContainerCheck(
            getNodeRef("recordFolder3"),
            "recordFolder3",
            TYPE_RECORD_FOLDER
          );

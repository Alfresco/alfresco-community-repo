...
/**
 * A store providing support for content store implementations that provide routing of content read
 * and write requests based on context.
 *
 * @see ContentContext
 * @since 2.1
 * @author Derek Hulley
...
public abstract class AbstractRoutingContentStore implements ContentStore {
    ...
    /**
     * Get a content store based on the context provided. The applicability of the context and even
     * the types of context allowed are up to the implementation, but normally there should be a
     * fallback case for when the parameters are not adequate to make a decision.
     *
     * @param ctx the context to use to make the choice
     * @return Returns the store most appropriate for the given context and <b>never
     *     <tt>null</tt></b>
     */
            ...
            if (store != null) {
                ...
                try {
                    ...
                    throw new AlfrescoRuntimeException(
                            "Found a content store that previously supported a URL, but no longer"
                                    + " does: \n"
                                    + "   Store:       "
                                    + store
                                    + "\n"
    ...
    public boolean isContentUrlSupported(String contentUrl) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "The url "
                            + (supported ? "is" : "is not")
                            + " supported by at least one store.");
        }
        return supported;
    }
    ...
    /**
     * @return Returns a valid reader from one of the stores otherwise a {@link EmptyContentReader}
     *     is returned.
     */
    ...
    /**
     * This operation has to be performed on all the stores in order to maintain the {@link
    ...
    public boolean delete(String contentUrl) throws ContentIOException {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Deleted content URL from stores: \n"
                            + "   Stores:  "
                            + stores.size()
                            + "\n"
                            + "   Deleted: "

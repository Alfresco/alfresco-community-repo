...
public abstract class AbstractForumEmailMessageHandler extends AbstractEmailMessageHandler {
    ...
    protected NodeRef addPostNode(NodeRef nodeRef, EmailMessage message) {
        ...
        Date now = new Date();
        String nodeName =
                "posted-" + new SimpleDateFormat("dd-MM-yyyy-hh-mm-ss").format(now) + ".html";

        PropertyMap properties = new PropertyMap(3);
        properties.put(ContentModel.PROP_NAME, nodeName);

        NodeRef postNodeRef =
                nodeService.getChildByName(nodeRef, ContentModel.ASSOC_CONTAINS, nodeName);
        if (postNodeRef == null) {
    ...
    protected NodeRef getTopicNode(NodeRef nodeRef, String name) {
        ...
        NodeRef ret =
                getNodeService().getChildByName(nodeRef, ContentModel.ASSOC_CONTAINS, workingName);
        return ret;
    }
    ...
    protected NodeRef addTopicNode(NodeRef parentNode, String name) {
        ...
        NodeRef topicNode =
                nodeService.getChildByName(parentNode, ContentModel.ASSOC_CONTAINS, workingName);
        if (topicNode == null) {

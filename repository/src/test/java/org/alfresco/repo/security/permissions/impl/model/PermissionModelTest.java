...
public class PermissionModelTest extends AbstractPermissionTest {
    ...
    public void testWoof() {
        ...
        Set<PermissionReference> answer =
                permissionModelDAO.getRequiredPermissions(ref, typeQname, aspectQNames, On.NODE);
        assertEquals(1, answer.size());
    }

    public void testIncludePermissionGroups() {
        Set<PermissionReference> grantees =
                permissionModelDAO.getGranteePermissions(
                        SimplePermissionReference.getPermissionReference(
                                QName.createQName("cm", "cmobject", namespacePrefixResolver),
                                "Consumer"));
    ...
    public void testGetGrantingPermissions() {
        Set<PermissionReference> granters =
                permissionModelDAO.getGrantingPermissions(
                        SimplePermissionReference.getPermissionReference(
    ...
    public void testGlobalPermissions() {
        Set<? extends PermissionEntry> globalPermissions =
                permissionModelDAO.getGlobalPermissionEntries();
        assertEquals(5, globalPermissions.size());
    }

    public void testRequiredPermissions() {
        Set<PermissionReference> required =
                permissionModelDAO.getRequiredPermissions(
                        SimplePermissionReference.getPermissionReference(
                                QName.createQName("sys", "base", namespacePrefixResolver), "Read"),
                        QName.createQName("sys", "base", namespacePrefixResolver),
    ...
    class Nester extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                RetryingTransactionCallback<Void> queryPermissionModel =
                        new RetryingTransactionCallback<Void>() {
                            public Void execute() throws Throwable {
                                Random random = new Random();
                                Set<PermissionReference> toTest =
                                        permissionModelDAO.getAllPermissions(
                                                QName.createQName(
                                                        "sys", "base", namespacePrefixResolver));
    ...
    public void testNulls() {
        ...
        permissionModelDAO.getRequiredPermissions(
                ...
                On.NODE);
        permissionModelDAO.getRequiredPermissions(
                null, null, Collections.<QName>emptySet(), On.NODE);

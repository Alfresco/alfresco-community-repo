...
public class SubsystemChainingFtpAuthenticatorTest extends BaseSpringTest {
    ...
    public void testNumberOfAuthenticatorsInChain() {
        ...
        // 1) authType1 - ftp auth disabled, the user is always NOT authenticated as that bean is
        // switched off
        ...
        List<FTPAuthenticatorBase> authenticators =
                chainingAuthenticator.getUsableFtpAuthenticators();
        assertTrue(
                "The context configuration was created for 3 test FTP authenticators with 1"
                        + " disabled.",
                authenticators.size() == 2);

        // The contexts are configured for 2 subsystems:
        // 1) authType1 - ftp auth disabled, the user is always NOT authenticated as that bean is
        // switched off
        // 2) authType2 - ftp auth disabled, the user is always NOT authenticated as that bean is
        // switched off
        ...
        assertTrue(
                "The context configuration was created for 2 test FTP authenticators - all"
                        + " disabled.",
    ...
    public void testAuthenticatorChain() {
        ...
        assertTrue(
                ...
                chainingAuthenticator.authenticateUser(info, session));
    ...
    private void setContextForSubsystem(String beanName) {
        ...
        ChildApplicationContextManager applicationContextManager =
                (ChildApplicationContextManager) applicationContext.getBean(beanName);

 ...
 *
 * <p>In addition to the quick share service, this class also provides a BeforeDeleteNodePolicy and
...
public class QuickShareServiceImpl
    ...
    private static final String EMAIL_TEMPLATE_REF =
            "alfresco/templates/quickshare-email-templates/quickshare-email.default.template.ftl";
     ...
     */
    public void setScheduledPersistedActionService(
     ...
     */
    public void setQuickShareLinkExpiryActionPersister(
            QuickShareLinkExpiryActionPersister quickShareLinkExpiryActionPersister) {
    ...
    @Override
    public QuickShareDTO shareContent(NodeRef nodeRef, Date expiryDate)
            throws QuickShareDisabledException, InvalidNodeRefException {
        checkEnabled();
        ...
        if (!nodeService.getAspects(nodeRef).contains(QuickShareModel.ASPECT_QSHARE)) {
                        ...
                        new RunAsWork<Void>() {
                            public Void doWork() {
                                nodeService.addAspect(
                                        nodeRef, QuickShareModel.ASPECT_QSHARE, props);
                                return null;
                            }
                        });
            } finally {
                    ...
                    new TenantRunAsWork<Void>() {
                        public Void doWork() throws Exception {
                            attributeService.setAttribute(
                                    tenantNodeRef, ATTR_KEY_SHAREDIDS_ROOT, sharedId);
                            return null;
                        }
                    });
            ...
            sharedId =
                    (String) nodeService.getProperty(nodeRef, QuickShareModel.PROP_QSHARE_SHAREDID);
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "QuickShare - content already shared: " + sharedId + " [" + nodeRef + "]");
            }
        }
            ...
            AuthenticationUtil.runAsSystem(
                    (RunAsWork<Void>)
                            () -> {
                                ...
                                // if we get here, it means the expiry date is validated and the
                                // action
                                ...
                                try {
                                    nodeService.setProperty(
                                            nodeRef,
                                            QuickShareModel.PROP_QSHARE_EXPIRY_DATE,
                                            expiryDate);
                                } finally {
    ...
    public Map<String, Object> getMetaData(NodeRef nodeRef) {
        // TODO This functionality MUST be available when quickshare is also disabled, therefor
        // refactor it out from the quickshare package to a more common package.
        ...
        if (contentData != null) {
            // Note: thumbnail defs only appear in this list if they can produce a thumbnail for the
            // content
            // found in the content property of this node. This will be determined by looking at the
        ...
        List<NodeRef> thumbnailRefs =
                thumbnailService.getThumbnails(nodeRef, ContentModel.PROP_CONTENT, null, null);
        List<String> thumbnailNames = new ArrayList<String>(thumbnailRefs.size());
        for (NodeRef thumbnailRef : thumbnailRefs) {
            thumbnailNames.add(
                    (String) nodeService.getProperty(thumbnailRef, ContentModel.PROP_NAME));
        }
        metadata.put("thumbnailNames", thumbnailNames);

        metadata.put(
                "lastThumbnailModificationData",
                (List<String>) nodeProps.get(ContentModel.PROP_LAST_THUMBNAIL_MODIFICATION_DATA));
    ...
    public Pair<String, NodeRef> getTenantNodeRefFromSharedId(final String sharedId) {
                            ...
                            public NodeRef doWork() throws Exception {
                                return (NodeRef)
                                        attributeService.getAttribute(
                                                ATTR_KEY_SHAREDIDS_ROOT, sharedId);
                            }
                        });
        ...
        if (nodeRef == null) {
            ...
            // TMDQ
            final String query =
                    "+TYPE:\"cm:content\" AND +ASPECT:\"qshare:shared\" AND =qshare:sharedId:\""
                            + sharedId
                            + "\"";
    ...
    // note: will remove "share" even if node is only being archived (ie. moved to trash) => a
    // subsequent restore will *not* restore the "share"
                    ...
                    public Void doWork() throws Exception {
                        ...
                        if (sharedId != null) {
                            try {
                                ...
                                // note: deleted nodeRef might not match, eg. for upload new version
                                // -> checkin -> delete working copy
                                if (nodeRef.equals(beforeDeleteNodeRef)) {
                                    ...
                                    try {
                                        nodeService.removeAspect(
                                                nodeRef, QuickShareModel.ASPECT_QSHARE);
                                    } finally {
                                        behaviourFilter.enableBehaviour(
                                                nodeRef, ContentModel.ASPECT_AUDITABLE);
                                    }
                ...
                new RunAsWork<Void>() {
                    public Void doWork() throws Exception {
                        if (nodeService.hasAspect(childNodeRef, QuickShareModel.ASPECT_QSHARE)) {
                            // Disable audit to preserve modifier and modified date
                            behaviourFilter.disableBehaviour(
                                    childNodeRef, ContentModel.ASPECT_AUDITABLE);
                            try {
                                nodeService.removeAspect(
                                        childNodeRef, QuickShareModel.ASPECT_QSHARE);
                            } finally {
                                behaviourFilter.enableBehaviour(
                                        childNodeRef, ContentModel.ASPECT_AUDITABLE);
                            }
    ...
    private void removeSharedId(final String sharedId) {
        ...
        try {
            ...
            throw new QuickShareLinkExpiryActionException(
                    "Couldn't delete the quick share link expiry action for the sharedId:"
                            + sharedId);
        }
    }
        ...
        return TenantUtil.runAsTenant(
                new TenantRunAsWork<Boolean>() {
                    public Boolean doWork() throws Exception {
                        try {
                            checkQuickShareNode(nodeRef);
                            return permissionService.hasPermission(nodeRef, PermissionService.READ)
    ...
    public void sendEmailNotification(final QuickShareEmailRequest emailRequest) {
        ...
        if (clientApp == null) {
            throw new ClientAppNotFoundException(
                    "Client was not found [" + emailRequest.getClient() + "]");
        }
        ...
        final String senderFullName =
                ((senderFirstName != null ? senderFirstName + " " : "")
        ...
        final String sharedNodeUrl =
                getUrl(
                                clientApp.getProperty(CONFIG_SHARED_LINK_BASE_URL),
                                CONFIG_SHARED_LINK_BASE_URL)
                        + '/'
        ...
        final String templateAssetsUrl =
                getUrl(clientApp.getTemplateAssetsUrl(), ClientAppConfig.PROP_TEMPLATE_ASSETS_URL);
        templateModel.put(FTL_TEMPLATE_ASSETS_URL, templateAssetsUrl);

        // Set the email details
        Map<String, Serializable> actionParams = new HashMap<>();
        // Email sender. By default the current-user's email address will not be used to send this
        // mail.
        ...
        actionParams.put(
                ...
                new Object[] {senderFirstName, senderLastName, emailRequest.getSharedNodeName()});
        actionParams.put(
                MailActionExecuter.PARAM_IGNORE_SEND_FAILURE, emailRequest.isIgnoreSendFailure());
        // Pick the template
        final String templatePath =
                emailHelper.getEmailTemplate(
                        clientApp.getName(),
                        getSharedLinkEmailTemplatePath(clientApp),
                        EMAIL_TEMPLATE_REF);
        ...
        actionParams.put(
                MailActionExecuter.PARAM_LOCALE,
                getDefaultIfNull(
    ...
    public boolean canDeleteSharedLink(NodeRef nodeRef, String sharedByUserId) {
        ...
        if (siteName != null) {
            // node belongs to a site - current user must be a manager or collaborator or someone
            // who shared the link
            String role = siteService.getMembersRole(siteName, currentUser);
            if (isSharedByCurrentUser
                    || (role != null
                            && (role.equals(SiteModel.SITE_MANAGER)
                                    || role.equals(SiteModel.SITE_COLLABORATOR)))
            ...
            // node does not belongs to a site - current user must be the person who shared the link
            // or an admin
            canDeleteSharedLink = true;
        }
    ...
    private String getSiteName(NodeRef nodeRef) {
        ...
        while (parent != null && !nodeService.getType(parent).equals(SiteModel.TYPE_SITE)) {
            ...
            if (permissionService.hasReadPermission(parent) == AccessStatus.ALLOWED) {
                String parentName =
                        (String) nodeService.getProperty(parent, ContentModel.PROP_NAME);
            } else {
                return null;
            }
    ...
    protected void saveSharedLinkExpiryAction(String sharedId, Date expiryDate) {
        ...
        // If an expiry action already exists for the specified shared Id, first remove it, before
        // creating a new one.
        if (expiryActionNodeRef != null) {
            deleteQuickShareLinkExpiryAction(expiryActionNodeRef);
        }
        ...
        final QuickShareLinkExpiryAction expiryAction =
                new QuickShareLinkExpiryActionImpl(
                        java.util.UUID.randomUUID().toString(),
                        sharedId,
                        "QuickShare link expiry action");
        // Create the persisted schedule
        final ScheduledPersistedAction schedule =
                scheduledPersistedActionService.createSchedule(expiryAction);
        ...
        try {
            ...
            throw new QuickShareLinkExpiryActionException(
                    "Couldn't create quick share link expiry action.", ex);
        }

        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Quick share link expiry action is created for sharedId["
                            + sharedId
                            + "] and it's scheduled to be executed on: "
                            + expiryDate);
        }
    }
    ...
    public void deleteQuickShareLinkExpiryAction(QuickShareLinkExpiryAction linkExpiryAction) {
        ...
        try {
            Pair<String, NodeRef> pair =
                    getTenantNodeRefFromSharedId(linkExpiryAction.getSharedId());
        ...
        TenantUtil.runAsSystemTenant(
                () -> {
                    ...
                    // As the method is called directly (ie. not via unshareContent method which
                    // removes the aspect properties),
                    // then we have to remove the 'expiryDate' property as well.
                    if (sharedNodeRef != null
                            && nodeService.getProperty(
                                            sharedNodeRef, QuickShareModel.PROP_QSHARE_EXPIRY_DATE)
                                    != null) {
                        behaviourFilter.disableBehaviour(
                                sharedNodeRef, ContentModel.ASPECT_AUDITABLE);
                        try {
    ...
    /**
     * Removes (hard deletes) the previously persisted {@link QuickShareLinkExpiryAction} and its
     * related schedule {@link ScheduledPersistedAction} from the repository.
     */
    ...
    private QuickShareLinkExpiryAction attachSchedule(
            QuickShareLinkExpiryAction quickShareLinkExpiryAction) {
        if (quickShareLinkExpiryAction.getSchedule() == null) {
            ScheduledPersistedAction schedule =
                    scheduledPersistedActionService.getSchedule(quickShareLinkExpiryAction);
    ...
    private NodeRef getQuickShareLinkExpiryActionNode(String sharedId) {
        ...
        return TenantUtil.runAsDefaultTenant(
                () ->
                        quickShareLinkExpiryActionPersister.getQuickShareLinkExpiryActionNode(
                                expiryActionQName));
    }
    ...
    private void checkExpiryDate(Date expiryDate) {
        ...
        if (now.isAfter(expiryDate.getTime())) {
            throw new QuickShareLinkExpiryActionException.InvalidExpiryDateException(
                    "Invalid expiry date. Expiry date can't be in the past.");
        }
        if (expiryDatePeriod.getDuration(now, new DateTime(expiryDate)) < 1) {
            throw new QuickShareLinkExpiryActionException.InvalidExpiryDateException(
                    "Invalid expiry date. Expiry date can't be less then 1 "
                            + expiryDatePeriod.getMessage()
                            + '.');
        }
    }
    ...
    private enum ExpiryDatePeriod {
        DAYS {
            ...
            int getDuration(DateTime now, DateTime expiryDate) {
                Interval interval =
                        new Interval(now.withSecondOfMinute(0).withMillisOfSecond(0), expiryDate);

...
public class FilePlanComponentAspect extends BaseBehaviourBean
    ...
    /**
     * This method examines the old and new property sets and for those properties which have
     * changed, looks for script resources corresponding to those properties. Those scripts are then
     * called via the ScriptService.
     *
     ...
     */
    private void lookupAndExecuteScripts(
            NodeRef nodeWithChangedProperties,
    ...
    /**
     * This method determines which properties have changed and for each such property looks for a
     * script resource in a well-known location.
     *
     * @param oldProps the old properties and their values.
     * @param newProps the new properties and their values.
     ...
     */
    private List<NodeRef> lookupScripts(
            Map<QName, Serializable> oldProps, Map<QName, Serializable> newProps) {
        List<NodeRef> result = new ArrayList<>();

        Map<QName, Serializable> changedProps =
                PropertyMap.getChangedProperties(oldProps, newProps);
    ...
    public void onAddAspect(final NodeRef nodeRef, final QName aspectTypeQName) {
        AuthenticationUtil.runAs(
                new RunAsWork<Void>() {
                    @Override
                    public Void doWork() {
                        // Check if the node exists and the aspect hasn't been removed in the same
                        // transaction (see RM-3266)
                        if (nodeService.exists(nodeRef)
                                && nodeService.hasAspect(nodeRef, aspectTypeQName)) {
                            ...
                            // If the node has any renditions, they inherit the file plan from their
                            // source node.
                            ...
                            for (ChildAssociationRef chAssRef : renditions) {
                                ...
                                if (nodeService.exists(rendition)) {
                                    ...
                                    nodeService.addAspect(
                                            rendition, ASPECT_FILE_PLAN_COMPONENT, null);
                                }
                            }
                        }
    ...
    public void onMoveNode(
            final ChildAssociationRef oldChildAssocRef,
            final ChildAssociationRef newChildAssocRef) {
        AuthenticationUtil.runAs(
                new RunAsWork<Void>() {
                    @Override
                    public Void doWork() {
                        if (nodeService.exists(newChildAssocRef.getParentRef())
                            ...
                            NodeRef root =
                                    filePlanService.getFilePlan(newChildAssocRef.getParentRef());
                            // NOTE: set the null value if no root found
                            nodeService.setProperty(
                                    newChildAssocRef.getChildRef(), PROP_ROOT_NODEREF, root);
                        }
    ...
    public CopyBehaviourCallback getCopyCallback(QName classRef, CopyDetails copyDetails) {
        return new AbstractCopyBehaviourCallback() {
            /**
             * @see
             *     org.alfresco.repo.copy.CopyBehaviourCallback#getChildAssociationCopyAction(org.alfresco.service.namespace.QName,
            ...
            /**
             * @see
             *     org.alfresco.repo.copy.CopyBehaviourCallback#getCopyProperties(org.alfresco.service.namespace.QName,
             *     org.alfresco.repo.copy.CopyDetails, java.util.Map)
             */
            public Map<QName, Serializable> getCopyProperties(
                    QName classQName,
                    CopyDetails copyDetails,
                    Map<QName, Serializable> properties) {
                // Only copy the root node reference if the new value can be looked up via the
                // parent
            ...
            /**
             * @see
             *     org.alfresco.repo.copy.CopyBehaviourCallback#getMustCopy(org.alfresco.service.namespace.QName,
             *     org.alfresco.repo.copy.CopyDetails)
             */

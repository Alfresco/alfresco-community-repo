...
public class FTSQueryParser {
    ...
    private static CommonTree autoPhraseReWrite(
            CommonTree node, boolean defaultConjunction, RerankPhase rerankPhase) {
        if (node == null) {
            return node;
        }
        ...
        if (isAutoPhrasable(node, defaultConjunction)) {
            ...
            for (Object current : node.getChildren()) {
                ...
                if ((child.getType() == FTSParser.MANDATORY)
                    ...
                    if (child.getChildCount() > 0) {
                        ...
                        if ((term.getType() == FTSParser.TERM) && (term.getChildCount() == 1)) {
                            ...
                            CommonTree newChild =
                                    autoPhraseReWrite(child, defaultConjunction, rerankPhase);
            ...
            if (rerankPhase == RerankPhase.SINGLE_PASS_WITH_AUTO_PHRASE) {
                ...
                for (CommonTree phrased : autoPhrased) {
                    CommonTree newPhrased =
                            autoPhraseReWrite(phrased, defaultConjunction, rerankPhase);
    ...
    private static Constraint buildRange(
        ...
        larg =
                factory.createLiteralArgument(
                        FTSRange.ARG_FROM, DataTypeDefinition.TEXT, getText(testNode.getChild(1)));
        functionArguments.put(larg.getName(), larg);
        larg =
                factory.createLiteralArgument(
                        FTSRange.ARG_TO, DataTypeDefinition.TEXT, getText(testNode.getChild(2)));
        functionArguments.put(larg.getName(), larg);
        larg =
                factory.createLiteralArgument(
                        FTSRange.ARG_TO_INC,
    ...
    private static Constraint buildExpandTerm(
        ...
        LiteralArgument larg =
                factory.createLiteralArgument(
                        FTSTerm.ARG_TERM, DataTypeDefinition.TEXT, getText(testNode.getChild(0)));
        functionArguments.put(larg.getName(), larg);
        larg =
                factory.createLiteralArgument(
                ...
                if (child.getType() == FTSParser.TEMPLATE) {
                    if (child.getChildCount() > 1) {
                        ...
                        for (Object currentfieldReferenceNode : child.getChildren()) {
                            ...
                            CommonTree newChild =
                                    insertTreeAndFixFieldRefs(insert, fieldReferenceNode);
    ...
    private static CommonTree insertTreeAndFixFieldRefs(
            CommonTree source, CommonTree fieldReferenceNode) {
    ...
    private static Constraint buildFuzzyTerm(
        ...
        larg =
                factory.createLiteralArgument(
                        FTSFuzzyTerm.ARG_MIN_SIMILARITY, DataTypeDefinition.FLOAT, fuzzy);
        functionArguments.put(larg.getName(), larg);
        if (fieldReferenceNode != null) {
            PropertyArgument parg =
                    buildFieldReference(

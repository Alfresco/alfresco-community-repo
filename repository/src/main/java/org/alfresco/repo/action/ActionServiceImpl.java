...
public class ActionServiceImpl
    ...
    private Map<String, AsynchronousActionExecutionQueue> asynchronousActionExecutionQueues =
            new HashMap<String, AsynchronousActionExecutionQueue>();
     ...
     *
     * @param asynchronousActionExecutionQueues the asynchronous action execution queues
     * @deprecated Rather than inject a map, it is preferable to inject individual {@link
     *     AsynchronousActionExecutionQueue} instances during bean initialisation in a spring
     *     init-method.
     */
    ...
    /**
     * This method registers an {@link AsynchronousActionExecutionQueue} with the {@link
     * ActionService}.
     *
     * @param key String
     ...
     */
    public void registerAsynchronousActionExecutionQueue(
            String key, AsynchronousActionExecutionQueue asyncExecQueue) {
    ...
    private NodeRef getSavedActionFolderRef(NodeRef nodeRef) {
        List<ChildAssociationRef> assocs =
                this.nodeService.getChildAssocs(
                        nodeRef, RegexQNamePattern.MATCH_ALL, ActionModel.ASSOC_ACTION_FOLDER);
        if (assocs.size() != 1) {
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.action.ActionService#getActionDefinitions(org.alfresco.service.cmr.repository.NodeRef)
     */
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.action.ActionService#getActionConditionDefinition(java.lang.String)
     */
    ...
    public ActionCondition createActionCondition(String name) {
        if (logger.isDebugEnabled()) logger.debug("Creating Action Condition - [" + name + "]");
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.action.ActionService#evaluateAction(org.alfresco.service.cmr.action.Action,
    ...
    public boolean evaluateAction(Action action, NodeRef actionedUponNodeRef) {
        ...
        if (logger.isDebugEnabled()) logger.debug("\tAll Condition Evaluation Result - " + result);

        return result;
    }

    /**
     * Evaluates the actions by finding corresponding actionEvaluators in applicationContext
     * (registered through Spring). Composite conditions are evaluated here as well. It is also
     * possible to have composite actions inside composite actions.
     *
     * @see
     *     org.alfresco.service.cmr.action.ActionService#evaluateActionCondition(org.alfresco.service.cmr.action.ActionCondition,
     *     org.alfresco.service.cmr.repository.NodeRef)
     */
    public boolean evaluateActionCondition(ActionCondition condition, NodeRef actionedUponNodeRef) {
        if (condition instanceof CompositeActionCondition) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Evaluating Composite Condition - BOOLEAN CONDITION IS "
            ...
            for (ActionCondition simplecondition : compositeCondition.getActionConditions()) {
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Evaluating composite condition "
                                    + simplecondition.getActionConditionDefinitionName());
                }

                if (compositeCondition.isORCondition()) {
                    result =
                            result || evaluateSimpleCondition(simplecondition, actionedUponNodeRef);
    ...
    private boolean evaluateSimpleCondition(
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Evaluating simple condition " + condition.getActionConditionDefinitionName());
        }
        // Evaluate the condition
        ActionConditionEvaluator evaluator =
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.action.ActionService#executeAction(org.alfresco.service.cmr.action.Action,
    ...
    public void executeAction(Action action, NodeRef actionedUponNodeRef, boolean checkConditions) {
        executeAction(
                action, actionedUponNodeRef, checkConditions, action.getExecuteAsychronously());
    }
    ...
    public void executeAction(
        ...
        // Like emails (see RuleServiceImpl), metadata extraction is now normally performed
        // asynchronously.
        // As a result we need to override the executeAsychronously value if this is the case so
        // that
        // changes to the actionedUponNodeRef will have been committed before the extract is
        // performed.
        ...
        if (executeAsychronously == false) {
            ...
            addPostTransactionPendingAction(
                    action, actionedUponNodeRef, checkConditions, actionChain);
        }
    }

    private boolean isExecuteAsynchronously(
            Action action, NodeRef actionedUponNodeRef, boolean executeAsynchronously) {
        if (executeAsynchronously == false) {
            ...
            if (actionDefinitionName.equals(CompositeActionExecuter.NAME)) {
                ...
                ActionExecuter executer =
                        (ActionExecuter) this.applicationContext.getBean(actionDefinitionName);
    ...
    private AsynchronousActionExecutionQueue getQueue(Action action) {
        ActionExecuter executer =
                (ActionExecuter) this.applicationContext.getBean(action.getActionDefinitionName());
        ...
        if (queue == null) {
            // can't get queue
            throw new ActionServiceException(
                    "Unable to get AsynchronousActionExecutionQueue name: " + queueName);
        }
            ...
            if (logger.isDebugEnabled() == true) {
                logger.debug(
                        "Node = "
                                ...
                                + " skip action because node doesn't exist");
            }
            return;
        }
            ...
            try {
                ...
                try {
                    ...
                    if (checkConditions == false
                            || evaluateAction(action, actionedUponNodeRef) == true) {
                        if (getTrackStatus(action)) {
                            // Mark the action as starting
                            actionTrackingService.recordActionExecuting(
                                    action, actionedUponNodeRef);
                        }
                ...
                if (executedAsynchronously == true) {
                    ...
                    if (compensatingAction != null) {
                        // Set the current user & tenant. These should be the same for the primary
                        // action and the compensating action.
                        ((ActionImpl) compensatingAction).setRunAsUser(currentUserName);
                        ((ActionImpl) compensatingAction)
                                .setTenantId(((ActionImpl) action).getTenantId());
    ...
    /**
     * @see
     *     org.alfresco.repo.action.RuntimeActionService#directActionExecution(org.alfresco.service.cmr.action.Action,
     *     org.alfresco.service.cmr.repository.NodeRef)
     */
    public void directActionExecution(Action action, NodeRef actionedUponNodeRef) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "The action is being executed as the user: "
    ...
    /**
     * @see
     *     org.alfresco.repo.action.RuntimeActionService#registerActionConditionEvaluator(org.alfresco.repo.action.evaluator.ActionConditionEvaluator)
     */
    public void registerActionConditionEvaluator(
            ActionConditionEvaluator actionConditionEvaluator) {
    ...
    /**
     * @see
     *     org.alfresco.repo.action.RuntimeActionService#registerActionExecuter(org.alfresco.repo.action.executer.ActionExecuter)
     */
    ...
    /**
     * @see
     *     org.alfresco.repo.action.RuntimeActionService#registerParameterConstraint(org.alfresco.service.cmr.action.ParameterConstraint)
     */
    ...
    private NodeRef getActionNodeRefFromId(NodeRef nodeRef, String actionId) {
        ...
        if (this.nodeService.hasAspect(nodeRef, ActionModel.ASPECT_ACTIONS) == true) {
            DynamicNamespacePrefixResolver namespacePrefixResolver =
            ...
            List<NodeRef> nodeRefs =
                    searchService.selectNodes(
                            ...
                            "*[@sys:"
                                    + ContentModel.PROP_NODE_UUID.getLocalName()
                                    + "='"
                                    + actionId
                                    + "']",
                            null,
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.action.ActionService#saveAction(org.alfresco.service.cmr.repository.NodeRef,
    ...
    public NodeRef createActionNodeRef(
        ...
        NodeRef actionNodeRef =
                this.nodeService
                        .createNode(parentNodeRef, assocTypeName, assocName, actionType, props)
                        .getChildRef();
                ...
                .setCreator(
                        (String)
                                this.nodeService.getProperty(
                                        actionNodeRef, ContentModel.PROP_CREATOR));
        ((ActionImpl) action)
                .setCreatedDate(
                        (Date)
                                this.nodeService.getProperty(
                                        actionNodeRef, ContentModel.PROP_CREATED));
    ...
    /**
     * @see
     *     org.alfresco.repo.action.RuntimeActionService#saveActionImpl(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.cmr.action.Action)
     */
    public void saveActionImpl(NodeRef actionNodeRef, Action action) {
        ...
        ((ActionImpl) action)
                .setModifier(
                        (String)
                                this.nodeService.getProperty(
                                        actionNodeRef, ContentModel.PROP_MODIFIER));
        ((ActionImpl) action)
                .setModifiedDate(
                        (Date)
                                this.nodeService.getProperty(
                                        actionNodeRef, ContentModel.PROP_MODIFIED));
    }
    ...
    private void saveCompositeActions(NodeRef compositeActionNodeRef, ActionList<?> action2) {
        ...
        List<ChildAssociationRef> actionRefs =
                this.nodeService.getChildAssocs(
    ...
    private void saveActionConditionList(
            NodeRef actionNodeRef,
            List<ActionCondition> actionConditionsList,
            boolean isComposite) {
        if (logger.isDebugEnabled())
            logger.debug(
                    "SaveActionCondition list, "
                            + actionConditionsList.size()
        ...
        List<ChildAssociationRef> conditionRefs =
                this.nodeService.getChildAssocs(
                        actionNodeRef,
                        RegexQNamePattern.MATCH_ALL,
                        !isComposite
        ...
        for (ChildAssociationRef conditionRef : conditionRefs) {
            ...
            if (idToCondition.containsKey(conditionNodeRef.getId()) == false) {
                ...
                saveConditionProperties(
                        conditionNodeRef, idToCondition.get(conditionNodeRef.getId()));
        ...
        for (String nextId : orderedIds) {
            ...
            if (!isComposite && actionCondition instanceof CompositeActionCondition) {
                if (logger.isDebugEnabled()) logger.debug("Saving Composite Condition");
                ...
                NodeRef conditionNodeRef =
                        saveActionCondition(
                                ...
                                actionCondition,
                                ActionModel.ASSOC_CONDITIONS,
                                ActionModel.TYPE_COMPOSITE_ACTION_CONDITION);
                saveActionConditionList(
                        conditionNodeRef,
                        ((CompositeActionCondition) actionCondition).getActionConditions(),
                        true);
            } else {
                if (logger.isDebugEnabled())
                    logger.debug(
    ...
    private NodeRef saveActionCondition(
            NodeRef actionNodeRef,
        ...
        Map<QName, Serializable> props = new HashMap<QName, Serializable>(2);
        props.put(
                ActionModel.PROP_DEFINITION_NAME,
                actionCondition.getActionConditionDefinitionName());
        ...
        NodeRef conditionNodeRef =
                this.nodeService
                        .createNode(
                                actionNodeRef, AssociationQName, AssociationQName, typeName, props)
                        .getChildRef();
    ...
    private void saveConditionProperties(NodeRef conditionNodeRef, ActionCondition condition) {
        this.nodeService.setProperty(
                conditionNodeRef,
                ActionModel.PROP_CONDITION_INVERT,
                condition.getInvertCondition());

        if (condition instanceof CompositeActionCondition) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "SAVING OR = " + ((CompositeActionCondition) condition).isORCondition());
            }
            this.nodeService.setProperty(
                    conditionNodeRef,
                    ActionModel.PROP_CONDITION_ANDOR,
                    new Boolean(((CompositeActionCondition) condition).isORCondition()));
        }
    }
    ...
    private void saveParameters(NodeRef parameterizedNodeRef, ParameterizedItem item) {
        ...
        List<ChildAssociationRef> parameters =
                this.nodeService.getChildAssocs(
        ...
        for (ChildAssociationRef ref : parameters) {
            ...
            Map<QName, Serializable> nodeRefParameterMap =
                    this.nodeService.getProperties(paramNodeRef);
            ...
            if (parameterMap.containsKey(paramName) == false) {
                ...
                nodeRefParameterMap.put(
                        ActionModel.PROP_PARAMETER_VALUE, parameterMap.get(paramName));
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.action.ActionService#getActions(org.alfresco.service.cmr.repository.NodeRef)
     */
    public List<Action> getActions(NodeRef nodeRef) {
        ...
        if (this.nodeService.exists(nodeRef) == true
            ...
            List<ChildAssociationRef> actions =
                    this.nodeService.getChildAssocs(
                            getSavedActionFolderRef(nodeRef),
                            RegexQNamePattern.MATCH_ALL,
                            ActionModel.ASSOC_NAME_ACTIONS);
    ...
    private void populateAction(NodeRef actionNodeRef, Action action) {
        ...
        // Set the conditions
        List<ChildAssociationRef> conditions =
                this.nodeService.getChildAssocs(
                        actionNodeRef, RegexQNamePattern.MATCH_ALL, ActionModel.ASSOC_CONDITIONS);
    ...
    private void populateActionProperties(NodeRef actionNodeRef, Action action) {
        ...
        ((ActionImpl) action)
                .setExecutionStartDate((Date) props.get(ActionModel.PROP_EXECUTION_START_DATE));
        ((ActionImpl) action)
                .setExecutionEndDate((Date) props.get(ActionModel.PROP_EXECUTION_END_DATE));
        ((ActionImpl) action)
                .setExecutionStatus(
                        ActionStatus.valueOf(props.get(ActionModel.PROP_EXECUTION_ACTION_STATUS)));
        ((ActionImpl) action)
                .setExecutionFailureMessage(
                        (String) props.get(ActionModel.PROP_EXECUTION_FAILURE_MESSAGE));
    ...
    private void populateParameters(
            NodeRef parameterizedItemNodeRef, ParameterizedItem parameterizedItem) {
        List<ChildAssociationRef> parameters =
                this.nodeService.getChildAssocs(
                        parameterizedItemNodeRef,
                        RegexQNamePattern.MATCH_ALL,
                        ActionModel.ASSOC_PARAMETERS);
    ...
    private ActionCondition createActionCondition(NodeRef conditionNodeRef) {
        ...
        if (ActionModel.TYPE_COMPOSITE_ACTION_CONDITION.equals(conditionType) == false) {
            condition =
                    new ActionConditionImpl(
                            conditionNodeRef.getId(),
                            (String) properties.get(ActionModel.PROP_DEFINITION_NAME));
        } else {
            if (logger.isDebugEnabled()) {
        ...
        Boolean invert =
                (Boolean)
                        this.nodeService.getProperty(
                                conditionNodeRef, ActionModel.PROP_CONDITION_INVERT);
    ...
    private void populateCompositeActionCondition(
            NodeRef compositeNodeRef, CompositeActionCondition compositeActionCondition) {
        List<ChildAssociationRef> conditions =
                this.nodeService.getChildAssocs(
                        compositeNodeRef,
                        RegexQNamePattern.MATCH_ALL,
        ...
        Boolean OR =
                (Boolean)
                        this.nodeService.getProperty(
                                compositeNodeRef, ActionModel.PROP_CONDITION_ANDOR);

        if (logger.isDebugEnabled()) {
            logger.debug(
                    "\tPopulating Composite Condition with subconditions, Condition OR = " + OR);
        }
        ...
        for (ChildAssociationRef conditionNodeRef : conditions) {
            ...
            if (logger.isDebugEnabled())
                logger.debug(
                        "\t\tAdding subcondition "
                                + currentCondition.getActionConditionDefinitionName());
    ...
    public void populateCompositeAction(NodeRef compositeNodeRef, CompositeAction compositeAction) {
        ...
        List<ChildAssociationRef> actions =
                this.nodeService.getChildAssocs(
                        compositeNodeRef, RegexQNamePattern.MATCH_ALL, ActionModel.ASSOC_ACTIONS);
        for (ChildAssociationRef action : actions) {
            NodeRef actionNodeRef = action.getChildRef();
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.action.ActionService#getAction(org.alfresco.service.cmr.repository.NodeRef,
     *     java.lang.String)
     */
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.action.ActionService#removeAction(org.alfresco.service.cmr.repository.NodeRef,
     *     org.alfresco.service.cmr.action.Action)
     */
    ...
    /**
     * @see
     *     org.alfresco.service.cmr.action.ActionService#removeAllActions(org.alfresco.service.cmr.repository.NodeRef)
     */
        ...
        if (this.nodeService.exists(nodeRef) == true
            ...
            for (ChildAssociationRef action : actions) {
                this.nodeService.removeChild(
                        getSavedActionFolderRef(nodeRef), action.getChildRef());
            }
        }
    }

    /**
     * Add a pending action to the list to be queued for execution once the transaction is
     * completed.
     *
     * @param action the action
    ...
    private void addPostTransactionPendingAction(
        ...
        if (logger.isDebugEnabled() == true) {
            StringBuilder builder =
                    new StringBuilder("addPostTransactionPendingAction action chain = ");
            if (actionChain == null) {
                builder.append("null");
            } else {
                for (String value : actionChain) {
        ...
        if (actionChain == null || actionChain.contains(action.getId()) == false) {
            ...
            if (pendingActions == null) {
                ...
                AlfrescoTransactionSupport.bindResource(
                        POST_TRANSACTION_PENDING_ACTIONS, pendingActions);
            }

            // Check that action has only been added to the list once
            PendingAction pendingAction =
                    new PendingAction(action, actionedUponNodeRef, checkConditions, actionChain);
    ...
    private List<PendingAction> getPostTransactionPendingActions() {
        return (List<PendingAction>)
                AlfrescoTransactionSupport.getResource(POST_TRANSACTION_PENDING_ACTIONS);
    }
    ...
    private static class ActionParameterTypeCopyBehaviourCallback
        ...
        private static final ActionParameterTypeCopyBehaviourCallback INSTANCE =
                new ActionParameterTypeCopyBehaviourCallback();
        ...
        public Map<QName, Serializable> getCopyProperties(
            ...
            recordNodeRefsForRepointing(
                    sourceNodeRef, properties, ActionModel.PROP_PARAMETER_VALUE);
    ...
    /**
     * Ensures that <b>d:noderef</b> properties are repointed if the target was also copied as part
     * of the hierarchy.
     */
    public void onCopyComplete(
        ...
        ActionParameterTypeCopyBehaviourCallback.INSTANCE.repointNodeRefs(
                sourceNodeRef,
                targetNodeRef,
    ...
    public boolean onLogException(Action action, Log logger, Throwable t, String message) {
        LoggingAwareExecuter executer =
                (LoggingAwareExecuter)
                        this.applicationContext.getBean(action.getActionDefinitionName());

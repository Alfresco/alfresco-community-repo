...
public class AuditRemoveFromHoldTests extends BaseRMRestTest {
    ...
    private final String PREFIX = generateTestPrefix(
        AuditRemoveFromHoldTests.class
    );
    ...
    public void preconditionForAuditRemoveFromHoldTests() {
        ...
        heldRecordFolder =
            createRecordFolder(
                recordCategory.getId(),
                PREFIX + "heldRecFolder"
            );
        heldRecord =
            createElectronicRecord(recordFolder.getId(), PREFIX + "record");
    ...
    public void removeFromHoldEventIsAudited(
        String nodeId,
        String nodeName,
        String nodePath
    ) {
        ...
        STEP(
            "Check the audit log contains the entry for the remove from hold event."
        );
    ...
    public void unsuccessfulRemoveFromHoldIsNotAudited() {
        ...
        STEP(
            "Check the audit log doesn't contain the entry for the unsuccessful remove from hold."
        );
        assertTrue(
    ...
    public void removeFromHoldNotAuditedForRecordFolderChildren() {
        ...
        STEP(
            "Check the audit log contains only an entry for remove from hold."
        );
        assertEquals(
    ...
    public void removeFromHoldAuditEntryNotVisible() {
        ...
        FileModel heldFile = dataContent
            .usingAdmin()
            .usingSite(privateSite)
            .createContent(CMISUtil.DocumentType.TEXT_PLAIN);
        holdsAPI.addItemToHold(
        ...
        STEP(
            "Check that an user with no Read permissions can't see the entry for the remove from hold event."
        );
        assertTrue(
            "The list of events should not contain Remove from Hold entry ",
            rmAuditService
    ...
    public void removeFromHoldAuditEntryHoldNameNotVisible() {
        ...
        STEP(
            "Check that an user with no Read permissions can't see the hold name in the remove from hold event."
        );

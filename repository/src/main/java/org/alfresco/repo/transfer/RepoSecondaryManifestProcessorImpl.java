...
/**
 * @author brian
 *     <p>The secondary manifest processor performs a second parse of the snapshot file.
 *     <p>It is responsible for linking nodes together.
 *     <p>At the point that this processor runs both ends (source and target) of the nodes'
 *     associations should be available in the receiving repository.
 */
public class RepoSecondaryManifestProcessorImpl extends AbstractManifestProcessorBase {
    ...
    protected void processNode(TransferManifestNormalNode node) {
        ...
        NodeRef correspondingNodeRef =
                nodeResolver.resolveCorrespondingNode(
                                node.getNodeRef(),
                                TransferManifestNodeHelper.getPrimaryParentAssoc(node),
                                node.getParentPath())
                        .resolvedChild;
        ...
        List<AssociationRef> currentTargetAssocs =
                nodeService.getTargetAssocs(correspondingNodeRef, RegexQNamePattern.MATCH_ALL);
        processPeerAssociations(
                requiredTargetAssocs, currentTargetAssocs, correspondingNodeRef, true);

        // Process "source" peer associations (associations *to* this node)
        List<AssociationRef> requiredSourceAssocs = node.getSourceAssocs();
        List<AssociationRef> currentSourceAssocs =
                nodeService.getSourceAssocs(correspondingNodeRef, RegexQNamePattern.MATCH_ALL);
        processPeerAssociations(
                requiredSourceAssocs, currentSourceAssocs, correspondingNodeRef, false);
    }
     ...
     */
    private void processPeerAssociations(
            List<AssociationRef> requiredAssocs,
         ...
         *
         * <p>Need to compare on sourceNodeRef, targetNodeRef and qname but ignore, irrelevant id
         * property which is why we need to introduce AssociationRefKey
         */
         ...
         *
         * <p>Only remove the assocs for this node.
         *
         * <p>TODO ALF-6543 - What if there is a local, non transferred, peer assoc, for example a
         * rendition or a rule or something? Is there some way to say that that link was not
         * transferred?
         */
        //        if(isSource)
        //        {
        for (AssociationRefKey ref : keysCurrent) {
            if (!keysRequired.contains(ref)) {
                if (log.isDebugEnabled()) {
                    log.debug(
                            "need to remove peer assoc from:"
                                    + ref.sourceRef
                                    + ", to:"
                                    + ref.targetRef
                                    + ", qname:"
    ...
    private void processParentChildAssociations(
            List<ChildAssociationRef> requiredAssocs,
        ...
        Map<NodeRef, ChildAssociationRef> currentAssocMap =
                new HashMap<NodeRef, ChildAssociationRef>();
        ...
        for (ChildAssociationRef requiredAssoc : requiredAssocs) {
            ...
            if (!requiredAssoc.isPrimary()) {
                NodeRef otherNode =
                        isParent ? requiredAssoc.getChildRef() : requiredAssoc.getParentRef();
        ...
        // Once we get here, any entries remaining in currentParentMap are associations that need to
        // be deleted.
        ...
        for (ChildAssociationRef assocToAdd : assocsToAdd) {
            ...
            nodeService.addChild(parent, child, assocToAdd.getTypeQName(), assocToAdd.getQName());
        }
    }
     ...
     *
     * <p>In particular is differs from AssociationRef in that it does not have the "id" property.
     */

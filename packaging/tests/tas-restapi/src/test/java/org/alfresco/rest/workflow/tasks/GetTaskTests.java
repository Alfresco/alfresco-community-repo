...
public class GetTaskTests extends RestTest {
  ...
  @TestRail(
    ...
    description = "Verify admin user gets any existing task with Rest API and response is successfull (200)"
  )
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.WORKFLOW, TestGroup.TASKS },
    executionType = ExecutionType.SANITY,
    description = "Verify assignee user gets its assigned task with Rest API and response is successfull (200)"
  )
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.WORKFLOW, TestGroup.TASKS },
    executionType = ExecutionType.SANITY,
    description = "Verify user that started the task gets the started task with Rest API and response is successfull (200)"
  )
  ...
  @TestRail(
    ...
    description = "Verify any user with no relation to task id forbidden to get other task with Rest API (403)"
  )
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.WORKFLOW, TestGroup.TASKS },
    executionType = ExecutionType.SANITY,
    description = "Verify candidate user gets its specific task and no other user claimed the task with Rest API and response is successfull (200)"
  )
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.WORKFLOW, TestGroup.TASKS },
    executionType = ExecutionType.SANITY,
    description = "Verify involved user in a task without claim it gets the task with Rest API and response is successfull (200)"
  )
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.WORKFLOW, TestGroup.TASKS },
    executionType = ExecutionType.REGRESSION,
    description = "Verify user who started a task gets the task with empty taskId with Rest API"
  )
  public void starterUserGetsTaskWithEmptyTaskId() throws Exception {
    ...
    taskModel =
      dataWorkflow
        ...
        .usingSite(siteModel)
        .usingResource(fileModel)
        .createNewTaskAndAssignTo(userWhoStartsTask);
    ...
    RestTaskModelsCollection tasks = restClient.processModels(
      RestTaskModelsCollection.class,
      request
    );
    ...
    tasks
      ...
      .and()
      .entriesListContains("id", taskModel.getId());
  }
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.WORKFLOW, TestGroup.TASKS },
    executionType = ExecutionType.REGRESSION,
    description = "Verify user who started a task gets the task with properties parameter with Rest API"
  )
  public void starterUserGetsTaskWithPropertiesParameter() throws Exception {
    restTaskModel =
      restClient
        ...
        .withWorkflowAPI()
        .usingTask(taskModel)
        .usingParams("properties=id,assignee,formResourceKey")
        .getTask();
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.WORKFLOW, TestGroup.TASKS },
    executionType = ExecutionType.REGRESSION,
    description = "Verify user who started a task gets the task after it is deleted with Rest API"
  )
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.WORKFLOW, TestGroup.TASKS },
    executionType = ExecutionType.REGRESSION,
    description = "Verify if using invalid taskId status code 404 is returned."
  )
  @Test(
    groups = {
      TestGroup.REST_API,
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.WORKFLOW, TestGroup.TASKS },
    executionType = ExecutionType.REGRESSION,
    description = "Verify if using empty taskId status code 200 is returned."
  )
  @Test(
    groups = {
      TestGroup.REST_API,

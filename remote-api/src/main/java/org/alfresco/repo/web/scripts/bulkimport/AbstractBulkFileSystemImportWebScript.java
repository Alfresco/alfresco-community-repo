...
public class AbstractBulkFileSystemImportWebScript extends DeclarativeWebScript {
    ...
    protected NodeRef getTargetNodeRef(String targetNodeRefStr, String targetPath)
    ...
    protected NodeRef convertPathToNodeRef(String targetPath) throws FileNotFoundException {
        ...
        if (cleanTargetPath.startsWith("/")) cleanTargetPath = cleanTargetPath.substring(1);

        if (cleanTargetPath.endsWith("/"))
            cleanTargetPath = cleanTargetPath.substring(0, cleanTargetPath.length() - 1);

        if (cleanTargetPath.length() == 0) result = companyHome;
        else {
            FileInfo info =
                    fileFolderService.resolveNamePath(
                            companyHome, Arrays.asList(cleanTargetPath.split("/")));
            if (info == null)
                throw new WebScriptException(
    ...
    protected class MultithreadedImportWebScriptLogic {
        ...
        public Map<String, Object> executeImport() {
            ...
            try {
                ...
                throw new WebScriptException(
                        Status.STATUS_INTERNAL_SERVER_ERROR, buildTextMessage(t), t);
            }
        ...
        private void waitForImportToBegin() throws InterruptedException {
            // ACE-3047 fix, since bulk import is started asynchronously there is a chance that
            // client
        ...
        private BulkImportParameters getBulkImportParameters() throws FileNotFoundException {
            final BulkImportParametersExtractor extractor =
                    new BulkImportParametersExtractor(
    ...
    protected static class BulkImportParametersExtractor {
        ...
        public BulkImportParameters extract() throws FileNotFoundException {
            ...
            result.setNumThreads(
                    getOptionalPositiveInteger(PARAMETER_NUM_THREADS).orElse(defaultNumThreads));
            result.setBatchSize(
                    getOptionalPositiveInteger(PARAMETER_BATCH_SIZE).orElse(defaultBatchSize));
            ...
            if (!isNullOrEmpty(replaceExistingStr) && !isNullOrEmpty(existingFileModeStr)) {
                ...
                throw new IllegalStateException(
                        String.format(
                                ...
                                PARAMETER_REPLACE_EXISTING, PARAMETER_EXISTING_FILE_MODE));
            }

...
/**
 * Servlet responsible for streaming node content from the repo directly to the response stream. The
 * appropriate mimetype is calculated based on filename extension.
 *
 * <p>The URL to the servlet should be generated thus:
 *
 ...
 *
 * <p>The 'attach' or 'direct' element is used to indicate whether to display the stream directly in
 * the browser or download it as a file attachment.
 *
 * <p>Since ACS 6.X, this Servlet redirects to GET /nodes/{nodeId}/content V1 REST API.
 *
 * @author Kevin Roast
 * @author gavinc
 */
public class DownloadContentServlet extends HttpServlet {
    ...
    protected void doGet(final HttpServletRequest req, final HttpServletResponse res)
        ...
        if (logger.isDebugEnabled()) {
            ...
            logger.debug(
                    "Authenticating (GET) request to URL: "
                            + req.getRequestURI()
        ...
        if (tokenCount < 6) {
            throw new IllegalArgumentException(
                    "Download URL did not contain all required args: " + requestURI);
        }

        // attachment mode (either 'attach' or 'direct')
        String attachToken = t.nextToken();
        boolean isAttachment =
                URL_ATTACH.equalsIgnoreCase(attachToken)
                        || URL_ATTACH_LONG.equalsIgnoreCase(attachToken);
        boolean isDirect =
                URL_DIRECT.equalsIgnoreCase(attachToken)
                        || URL_DIRECT_LONG.equalsIgnoreCase(attachToken);
        if (!(isAttachment || isDirect)) {
            throw new IllegalArgumentException(
                    "Attachment mode is not properly specified: " + requestURI);
        }

        // allow only nodes from workspace://SpaceStore/ storeRef
        StoreRef storeRef =
                new StoreRef(URLDecoder.decode(t.nextToken()), URLDecoder.decode(t.nextToken()));
        ...
        if (!isWorkspaceStoreType || !isSpacesStoreStoreId) {
            throw new IllegalArgumentException(
                    "Servlet accepts only nodes from workspace://SpaceStore/ storeRef: "
                            + requestURI);
        }
        ...
        String redirectUrl =
                String.format(
                        "%s/api/-default-/public/alfresco/versions/1/nodes/%s/content?attachment=%b",

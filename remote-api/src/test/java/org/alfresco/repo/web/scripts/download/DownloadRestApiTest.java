...
public class DownloadRestApiTest extends BaseWebScriptTest {
    ...
    public void setUp() {
        ...
        contentService =
                getServer().getApplicationContext().getBean("ContentService", ContentService.class);
        ...
        if (this.authenticationService.authenticationExists(TEST_USERNAME) == false) {
            ...
            this.authenticationService.createAuthentication(
                    TEST_USERNAME, "password".toCharArray());
    ...
    @Test
    public void testCreateAndGetDownload()
            throws UnsupportedEncodingException, IOException, JSONException {
        // CReate the download
        String postData =
                "[{ \"nodeRef\": \"" + rootFile + "\"}, { \"nodeRef\": \"" + rootFolder + "\"}]";
        Response response =
                sendRequest(new PostRequest(URL_DOWNLOADS, postData, "application/json"), 200);
        ...
        String statusUrl =
                MessageFormat.format(
                        URL_DOWNLOAD_STATUS,
                        downloadNodeRef.getStoreRef().getProtocol(),
                        downloadNodeRef.getStoreRef().getIdentifier(),
                        downloadNodeRef.getId());

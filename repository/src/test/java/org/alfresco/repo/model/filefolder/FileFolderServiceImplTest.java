...
public class FileFolderServiceImplTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        workingRootNodeRef1 =
                nodeService
                        .createNode(
                                ...
                                QName.createQName(
                                        "http://www.alfresco.org/test/filefoldertest/1.0",
                                        "folder"))
                        .getChildRef();
        nodeService
                .createNode(
        ...
        nodeService
                .createNode(
                        workingRootNodeRef1,
                        QName.createQName(
                                "http://www.alfresco.org/test/filefoldertest/1.0", "contains1"),
    ...
    public void testShallowFoldersOnlyList() throws Exception {
        ...
        // check
        String[] expectedNames =
                new String[] {NAME_L0_FOLDER_A, NAME_L0_FOLDER_B, NAME_L0_FOLDER_C};
        checkFileList(files, 0, 3, expectedNames);
    }

    public void testShallowFileSearch() throws Exception {
        List<FileInfo> files =
                fileFolderService.search(workingRootNodeRef, NAME_L0_FILE_B, true, false, false);
        // check
    ...
    public void testDeepFilesAndFoldersSearch() throws Exception {
        ...
        {
            List<FileInfo> files =
                    fileFolderService.search(workingRootNodeRef, "?1-*", true, true, true);
            // check
            String[] expectedNames =
                    new String[] {
                        NAME_L1_FOLDER_A,
        ...
        {
            List<FileInfo> files =
                    fileFolderService.search(workingRootNodeRef, NAME_L1_FILE_B, true, true, true);
            // check
        ...
        {
            List<FileInfo> files =
                    fileFolderService.search(workingRootNodeRef, "*", true, true, true);
            // check
            String[] expectedNames =
                    new String[] {
                        NAME_CHECK_FOLDER,
    ...
    public void testDeepFilesOnlySearch() throws Exception {
        List<FileInfo> files =
                fileFolderService.search(workingRootNodeRef, "?1-*", true, false, true);
        // check
    ...
    private FileInfo getByName(String name, boolean isFolder) throws Exception {
        List<FileInfo> results =
                fileFolderService.search(workingRootNodeRef, name, !isFolder, isFolder, true);
        if (results.size() > 1) {
    ...
    public void testRenamePattern() throws Exception {
        ...
        assertFalse(
                nodeService
                        .getAspects(checkInfo.getNodeRef())
                        .contains(ContentModel.ASPECT_TEMPORARY));
    }
    ...
    public void testRenameDiscussionALF5569() throws Exception {
        ...
        String currentName =
                (String) nodeService.getProperty(discussionNodeRef, ContentModel.PROP_NAME);
        ...
        // get the name now
        String newName =
                (String) nodeService.getProperty(newFileInfo.getNodeRef(), ContentModel.PROP_NAME);
    ...
    public void testMove() throws Exception {
        ...
        List<FileInfo> checkFileInfos =
                fileFolderService.search(workingRootNodeRef, NAME_L1_FOLDER_A, false);
        ...
        checkFileInfos =
                fileFolderService.search(workingRootNodeRef, checkFileInfo.getName(), false);
    ...
    public void testMovePermissions() throws Exception {
        ...
        RunAsWork<NodeRef> createTargetWork =
                new RunAsWork<NodeRef>() {
                    @Override
                    public NodeRef doWork() throws Exception {
                        // Create folder TARGET
                        return fileFolderService
                                .create(workingRootNodeRef, "TARGET", ContentModel.TYPE_FOLDER)
                                .getNodeRef();
                    }
                };
        final NodeRef targetNodeRef =
                AuthenticationUtil.runAs(createTargetWork, AuthenticationUtil.getSystemUserName());
        ...
        fileFolderService
                .create(targetNodeRef, "SOURCE ONE", ContentModel.TYPE_CONTENT)
                .getNodeRef();
        // Deny anyone access to the target
        RunAsWork<Void> setPermissionsWork =
                new RunAsWork<Void>() {
                    @Override
        ...
        try {
            fileFolderService
                    .create(targetNodeRef, "SOURCE TWO", ContentModel.TYPE_CONTENT)
                    .getNodeRef();
        ...
        NodeRef movingNodeRef =
                fileFolderService
                        .create(workingRootNodeRef, "SOURCE THREE", ContentModel.TYPE_CONTENT)
                        .getNodeRef();
        // Move it
        try {
            fileFolderService.moveFrom(
                    movingNodeRef, workingRootNodeRef, targetNodeRef, "SOURCE THREE");
    ...
    public void testCreateFolder() throws Exception {
        ...
        try {
            ...
            fileFolderService.create(
                    parentFolderRef, "illegal folder", ContentModel.TYPE_SYSTEM_FOLDER);
        ...
        try {
            ...
            fileFolderService.create(
                    parentFolderRef,
                    "Legal subtype of folder",
                    QName.createQName(testNs, "subfolder"));
        } catch (Throwable e) {
        ...
        // create a file
        FileInfo fileInfo =
                fileFolderService.create(parentFolderRef, "newFile", ContentModel.TYPE_CONTENT);
        // check
    ...
    public void testMakeFolders() throws Exception {
        ...
        FileInfo lastFileInfo =
                FileFolderServiceImpl.makeFolders(
                        fileFolderService, rootNodeRef, namePath, ContentModel.TYPE_FOLDER);
        assertNotNull("First makeFolder failed", lastFileInfo);
        // check that a repeat works

        FileInfo lastFileInfoAgain =
                FileFolderServiceImpl.makeFolders(
                        fileFolderService, rootNodeRef, namePath, ContentModel.TYPE_FOLDER);
        assertNotNull("Repeat makeFolders failed", lastFileInfoAgain);
        assertEquals(
                "Repeat created new leaf",
                lastFileInfo.getNodeRef(),
                lastFileInfoAgain.getNodeRef());
        ...
        // get the path
        List<FileInfo> checkPathInfos =
                fileFolderService.getNamePath(rootNodeRef, checkInfos.get(0).getNodeRef());
    ...
    public void testGetNamePath() throws Exception {
        ...
        assertEquals(
                "First level incorrent", workingRootNodeRef.getId(), infoPaths.get(0).getName());
    ...
    public void testGetNamePathDoesNotReturnPathContainingNonLeafFileNode() throws Exception {
        ...
        FileInfo dirInfo =
                fileFolderService.create(parentFolderRef, "newDir", ContentModel.TYPE_FOLDER);
        FileInfo fileInfo =
                fileFolderService.create(
                        dirInfo.getNodeRef(), "newFile", ContentModel.TYPE_CONTENT);
        ...
        FileInfo fileInfo2 =
                fileFolderService.create(
                        fileInfo.getNodeRef(), "newFile2", ContentModel.TYPE_CONTENT);
    ...
    public void testResolveNamePath() throws Exception {
        ...
        FileInfo fileInfoCheck =
                fileFolderService.resolveNamePath(workingRootNodeRef, pathElements);
        assertNotNull("File info not found", fileInfoCheck);
        assertEquals(
                "Path not resolved to correct node",
                fileInfo.getNodeRef(),
                fileInfoCheck.getNodeRef());
    }
    ...
    public void testLongFileNames() throws Exception {
        String fileName =
                "12345678901234567890123456789012345678901234567890"
                        ...
                        + "12345678901234567890123456789012345678901234567890";
        FileInfo fileInfo =
                fileFolderService.create(workingRootNodeRef, fileName, ContentModel.TYPE_CONTENT);
    ...
    public void testETHREEOH_3088_MoveIntoSelf() throws Exception {
        FileInfo folderInfo =
                fileFolderService.create(
                        workingRootNodeRef, "NotGood.txt", ContentModel.TYPE_FOLDER);
        NodeRef folderNodeRef = folderInfo.getNodeRef();
        // Move into self
        try {
    ...
    public void testAlf6560MimetypeSetting() throws Exception {
        FileInfo fileInfo =
                fileFolderService.create(
                        workingRootNodeRef, "Something.html", ContentModel.TYPE_CONTENT);
        ...
        assertEquals(
                "Mimetype was not automatically set",
    ...
    public void testGetLocalizedSibling() throws Exception {
        FileInfo base =
                fileFolderService.create(
                        workingRootNodeRef, "Something.ftl", ContentModel.TYPE_CONTENT);
        NodeRef node = base.getNodeRef();
        NodeRef nodeFr =
                fileFolderService
                        .create(workingRootNodeRef, "Something_fr.ftl", ContentModel.TYPE_CONTENT)
                        .getNodeRef();
        NodeRef nodeFrFr =
                fileFolderService
                        .create(
        ...
        NodeRef nodeEn =
                fileFolderService
                        .create(workingRootNodeRef, "Something_en.ftl", ContentModel.TYPE_CONTENT)
                        .getNodeRef();
        NodeRef nodeEnUs =
                fileFolderService
                        .create(
        ...
        assertEquals(
                "Match fail for " + I18NUtil.getLocale(),
                nodeEnUs,
                fileFolderService.getLocalizedSibling(node));
        ...
        assertEquals(
                "Match fail for " + I18NUtil.getLocale(),
                nodeFr,
                fileFolderService.getLocalizedSibling(nodeFr));
        I18NUtil.setLocale(Locale.UK);
        assertEquals(
                "Match fail for " + I18NUtil.getLocale(),
                nodeFr,
        ...
        FileInfo mbase =
                fileFolderService.create(
                        workingRootNodeRef, "Another.get.html.ftl", ContentModel.TYPE_CONTENT);
        NodeRef mnode = mbase.getNodeRef();
        NodeRef mnodeFr =
                fileFolderService
                        .create(
                                workingRootNodeRef,
                                "Another_fr.get.html.ftl",
                                ContentModel.TYPE_CONTENT)
                        .getNodeRef();
        ...
        assertEquals(
                "Match fail for " + I18NUtil.getLocale(),
                mnode,
                fileFolderService.getLocalizedSibling(mnode));
    ...
    public synchronized void testAlf7421TimestampPropagation() throws Exception {
        ...
        FileInfo folderInfo =
                fileFolderService.create(
                        workingRootNodeRef, "SomeFolder", ContentModel.TYPE_FOLDER);
        NodeRef folderNodeRef = folderInfo.getNodeRef();
        // Get the dates for the folder we are using
        String creatorExpected =
                (String) nodeService.getProperty(folderNodeRef, ContentModel.PROP_CREATOR);
        Date createdExpected =
                (Date) nodeService.getProperty(folderNodeRef, ContentModel.PROP_CREATED);
        String modifierExpected =
                (String) nodeService.getProperty(folderNodeRef, ContentModel.PROP_MODIFIER);
        Date modifiedExpected =
                (Date) nodeService.getProperty(folderNodeRef, ContentModel.PROP_MODIFIED);

        // Get the current dates for the parent folder (one level up)
        String creatorTooHigh =
                (String) nodeService.getProperty(workingRootNodeRef, ContentModel.PROP_CREATOR);
        Date createdTooHigh =
                (Date) nodeService.getProperty(workingRootNodeRef, ContentModel.PROP_CREATED);
        String modifierTooHigh =
                (String) nodeService.getProperty(workingRootNodeRef, ContentModel.PROP_MODIFIER);
        Date modifiedTooHigh =
                (Date) nodeService.getProperty(workingRootNodeRef, ContentModel.PROP_MODIFIED);
        ...
        assertTrue(
                "cm:modified should have changed",
                beforeSleep.compareTo(
                                (Date)
                                        nodeService.getProperty(
                                                folderNodeRef, ContentModel.PROP_MODIFIED))
                        < 0);
        ...
        assertEquals(
                "cm:creator should not have changed (level too high)",
                creatorTooHigh,
                nodeService.getProperty(workingRootNodeRef, ContentModel.PROP_CREATOR));
        assertEquals(
                "cm:created should not have changed (level too high)",
                createdTooHigh,
                nodeService.getProperty(workingRootNodeRef, ContentModel.PROP_CREATED));
        assertEquals(
                "cm:modifier should not have changed (level too high)",
                modifierTooHigh,
                nodeService.getProperty(workingRootNodeRef, ContentModel.PROP_MODIFIER));
        assertEquals(
                "cm:modified should not have changed (level too high)",
                modifiedTooHigh,
                nodeService.getProperty(workingRootNodeRef, ContentModel.PROP_MODIFIED));

        // Now let's test file moving:
        // Create source folder
        FileInfo sourceFolderInfo =
                fileFolderService.create(
                        workingRootNodeRef, "SourceFolder", ContentModel.TYPE_FOLDER);
        NodeRef sourceFolderNodeRef = sourceFolderInfo.getNodeRef();

        // Create destination folder
        FileInfo destinationFolderInfo =
                fileFolderService.create(
                        workingRootNodeRef, "DestinationFolder", ContentModel.TYPE_FOLDER);
        NodeRef destinationFolderNodeRef = destinationFolderInfo.getNodeRef();

        String sourceFolderCreatorExpected =
                (String) nodeService.getProperty(sourceFolderNodeRef, ContentModel.PROP_CREATOR);
        Date sourceFolderCreatedExpected =
                (Date) nodeService.getProperty(sourceFolderNodeRef, ContentModel.PROP_CREATED);

        String destinationFolderCreatorExpected =
                (String)
                        nodeService.getProperty(
                                destinationFolderNodeRef, ContentModel.PROP_CREATOR);
        Date destinationFolderCreatedExpected =
                (Date) nodeService.getProperty(destinationFolderNodeRef, ContentModel.PROP_CREATED);

        FileInfo relocatableFileInfo =
                fileFolderService.create(
                        sourceFolderNodeRef, "MoveMePlease.html", ContentModel.TYPE_CONTENT);
        ...
        String sourceFolderModifierExpected =
                (String) nodeService.getProperty(sourceFolderNodeRef, ContentModel.PROP_MODIFIER);

        // Get the dates for the destination folder
        String destinationFolderModifierExpected =
                (String)
                        nodeService.getProperty(
                                destinationFolderNodeRef, ContentModel.PROP_MODIFIER);
    ...
    public void testPatterns() {
        ...
        PagingResults<FileInfo> pagingResults =
                fileFolderService.list(
                        workingRootNodeRef, true, true, "L0*", null, null, pagingRequest);
        ...
        pagingResults =
                fileFolderService.list(
                        workingRootNodeRef, true, true, "L1*", null, null, pagingRequest);
        ...
        pagingResults =
                fileFolderService.list(
                        workingRootNodeRef, true, true, "L0*File*", null, null, pagingRequest);
    ...
    public void testALF12758() {
        ...
        PagingResults<FileInfo> pagingResults =
                fileFolderService.list(
                        workingRootNodeRef1, true, true, null, null, null, pagingRequest);
        assertNotNull(pagingResults);
    ...
    public void testListPage() throws Exception {
        ...
        PagingResults<FileInfo> pagingResults =
                fileFolderService.list(
                        workingRootNodeRef, true, true, null, null, null, pagingRequest);

        assertNotNull(pagingResults);
        assertFalse(pagingResults.hasMoreItems());
        assertTrue(
                (pagingResults.getQueryExecutionId() != null)
                        && (pagingResults.getQueryExecutionId().length() > 0));
        assertNull(pagingResults.getTotalResultCount());
    ...
    public void testList_HiddenFiles() {
        ...
        try {
            ...
            NodeRef nodeRef1 =
                    fileFolderService
                            .create(nodeRef, "parent", ContentModel.TYPE_CONTENT)
                            .getNodeRef();
            for (int i = 0; i < 10; i++) {
                fileFolderService
                        .create(nodeRef1, ".child" + i, ContentModel.TYPE_CONTENT)
                        .getNodeRef();
            }
            ...
            for (int i = 0; i < 10; i++) {
                fileFolderService
                        .create(nodeRef1, "visiblechild" + i, ContentModel.TYPE_CONTENT)
                        .getNodeRef();
            }
            ...
            PagingResults<FileInfo> results =
                    fileFolderService.list(nodeRef1, true, true, null, null, pagingRequest);
            Pair<Integer, Integer> totalResultCount = results.getTotalResultCount();
            assertNotNull(totalResultCount.getFirst());
            assertEquals(
                    "Total result lower count should be 10",
                    10,
                    totalResultCount.getFirst().intValue());
            assertNotNull(totalResultCount.getSecond());
            assertEquals(
                    "Total result upper count should be 10",
                    10,
    ...
    public void testList_notCheckedOut_ALF_13602() {
        // Test that, eg. in the case of Share doclib, when listing files that have been checked-out
        // we only list the working copy (ie. not the original checkedOut copy)
        ...
        NodeRef nodeRef =
                fileFolderService
                        ...
                        .getNodeRef();
        NodeRef nodeRef1 =
                fileFolderService.create(nodeRef, "parent", ContentModel.TYPE_CONTENT).getNodeRef();
        ...
        for (int i = 0; i < totalItems; i++) {
            ...
            children[i] =
                    fileFolderService
                            .create(nodeRef1, "child-" + suffix, ContentModel.TYPE_CONTENT)
                            .getNodeRef();
        }
    ...
    public void testListNotIgnoreSpaces() {
        String[] foldersToTest = new String[] {"A B", "AA", "AC"};
        ...
        PagingResults<FileInfo> pagingResults =
                fileFolderService.list(
                        testFolder, true, true, null, null, sortProps, pagingRequest);
        List<FileInfo> files = pagingResults.getPage();
    ...
    private void checkPages(
            ...
            int pageSize,
            int totalItems,
            boolean hideCheckedOut,
            int checkedOutChildIdx) {
        ...
        for (int i = 1; i <= pageCount; i++) {
            ...
            PagingResults<FileInfo> results =
                    fileFolderService.list(
                            parentRef, true, true, ignoreQNameTypes, sortProps, pagingRequest);
            ...
            assertEquals(
                    (i != pageCount ? pageSize : (totalItems - ((pageCount - 1) * pageSize))),
            ...
            for (FileInfo fileInfo : results.getPage()) {
                ...
                assertTrue(
                        "Expected: " + expected + ", Actual: " + actual + " (j=" + j + ")",
    ...
    public void testCopyOfWorkingCopy_ALF_8863() throws Exception {
        ...
        FileInfo destDirInfo =
                fileFolderService.create(workingRootNodeRef, "destDir", ContentModel.TYPE_FOLDER);

        NodeRef workingCopyRef = cociService.checkout(testNodeRef);
        String workingCopyName =
                (String) nodeService.getProperty(workingCopyRef, ContentModel.PROP_NAME);

        FileInfo copyInfo = fileFolderService.copy(workingCopyRef, destDirInfo.getNodeRef(), null);
        String checkedOutName =
                (String) nodeService.getProperty(testNodeRef, ContentModel.PROP_NAME);
        ...
        assertTrue(
                copyInfo.getName()
                        .substring(0, copyExtIndex)
                        .startsWith(checkedOutName.substring(0, origExtIndex)));
    }
    ...
    public void testSortingCustomFields() {
        ...
        HashMap<String, Pair<QName, String>> customProps =
                new HashMap<String, Pair<QName, String>>();
        ...
        NodeRef nodeRef =
                fileFolderService
                        .create(
                                ...
                                ContentModel.TYPE_CONTENT)
                        .getNodeRef();
        NodeRef parentTestRef =
                fileFolderService.create(nodeRef, "parent", ContentModel.TYPE_CONTENT).getNodeRef();
        for (Entry<String, Pair<QName, String>> entry : customProps.entrySet()) {
            NodeRef child =
                    fileFolderService
                            .create(parentTestRef, entry.getKey(), ContentModel.TYPE_CONTENT)
                            .getNodeRef();
            nodeService.setProperty(
                    child, entry.getValue().getFirst(), entry.getValue().getSecond());
        }
        ...
        PagingResults<FileInfo> results =
                fileFolderService.list(parentTestRef, true, false, null, sortProps, pagingRequest);
        List<FileInfo> pageRes = results.getPage();
        String[] expectedNames =
                new String[] {"A-null", "B-null", "B-baz", "B-biz", "A-bar", "A-foo"};
    ...
    public void testMoveCopyLotsOfFiles() throws FileNotFoundException {
        ...
        FileInfo
                containingFolder =
                        fileFolderService.create(
                                workingRootNodeRef, CONTAINING_FOLDER, ContentModel.TYPE_FOLDER),
                folder1 =
                        fileFolderService.create(
                                containingFolder.getNodeRef(), FOLDER_1, ContentModel.TYPE_FOLDER),
                folder2 =
                        fileFolderService.create(
                                containingFolder.getNodeRef(), FOLDER_2, ContentModel.TYPE_FOLDER);
        ...
        if (dialect instanceof MySQLClusterNDBDialect) {
            // note: to increase the file count on NDB, may need to further bump-up NDB cluster
            // config
            // eg. DataMemory, IndexMemory, MaxNoOfConcurrentOperations, ...
            // also consider splitting into separate txns (eg. after each bulk create, move, delete,
            // copy, ...)
            COUNT = 1000;
        }
        ...
        for (int index = 0; index < COUNT; index++) {
            fileFolderService.create(
                    folder1.getNodeRef(), "Name " + index, ContentModel.TYPE_CONTENT);
        }
        ...
        // copy back
        FileInfo newFolder =
                fileFolderService.copy(folder1.getNodeRef(), containingFolder.getNodeRef(), null);

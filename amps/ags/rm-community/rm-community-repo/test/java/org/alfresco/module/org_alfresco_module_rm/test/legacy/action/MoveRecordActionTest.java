        ...
        doTestInTransaction(
                new Test<Void>() {
                    public Void run() {
                        ...
                        // Create a document so that the user has the write permissions for that
                        // document
                        NodeRef document =
                                fileFolderService
                                        .create(
                        ...
                        assertNotNull(
                                nodeService.getProperty(
                                        document, PROP_RECORD_ORIGINATING_LOCATION));

                        // Check the parents. In this case the document should have two parents
                        // (doclib and fileplan)
                        ...
                        // Move the record
                        Action moveRecordAction =
                                actionService.createAction(MoveDmRecordAction.NAME);
                        moveRecordAction.setParameterValue(
                                MoveDmRecordAction.PARAM_TARGET_NODE_REF, destination);

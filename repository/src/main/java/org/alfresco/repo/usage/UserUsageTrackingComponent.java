 ...
 *
 * <p>- used by UserUsageCollapseJob to collapse usage deltas. - used on bootstrap to either clear
 * all usages or (re-)calculate all missing usages.
 */
public class UserUsageTrackingComponent extends AbstractLifecycleBean
    ...
    public void onCreateNode(ChildAssociationRef childAssocRef) {
        if (enabled == true) {
            ...
            if (userName != null) {
                RetryingTransactionCallback<Long> updateUserWithUsage =
                            ...
                            public Long execute() throws Throwable {
                                ...
                                for (String store : stores) {
                                    final StoreRef storeRef =
                                            tenantService.getName(new StoreRef(store));

                                    Long contentSize =
                                            usageDAO.getContentSizeForStoreForUser(
                                                    storeRef, userName);

                                    if (contentSize != null) {
                                        currentUsage =
                                                (currentUsage == null ? 0L : currentUsage)
                                                        + contentSize;
                                    }
                                }

                                if (currentUsage != null && currentUsage > 0L) {
                                    List<Pair<NodeRef, Long>> batchUserUsages =
                                            new ArrayList<Pair<NodeRef, Long>>(1);
                                    batchUserUsages.add(
                                            new Pair<NodeRef, Long>(personRef, currentUsage));
                ...
                transactionService
                        ...
                        .doInTransaction(updateUserWithUsage, false);
            }
        }
    }
    ...
    private void clearAllUsages() {
        ...
        RetryingTransactionCallback<Object> getUsersWithUsage =
                new RetryingTransactionCallback<Object>() {
                    public Object execute() throws Throwable {
                        // get people (users) with calculated usage
                        MapHandler userHandler =
                                new MapHandler() {
                        ...
                        usageDAO.getUsersWithUsage(
                                tenantService.getName(personStoreRef), userHandler);
    ...
    private int clearUsages(final List<NodeRef> personNodeRefs) {
        RetryingTransactionCallback<Integer> clearPersonUsage =
                new RetryingTransactionCallback<Integer>() {
                    public Integer execute() throws Throwable {
                        int clearCount = 0;
                        for (NodeRef personNodeRef : personNodeRefs) {
                            nodeService.setProperty(
                                    personNodeRef, ContentModel.PROP_SIZE_CURRENT, new Long(-1L));
        ...
        return transactionService
                .getRetryingTransactionHelper()
                .doInTransaction(clearPersonUsage, false);
    }

    /**
     * Recalculate content usage for all users that have no usage. Required if upgrading an existing
     * Alfresco, for users that have not had their initial usage calculated.
     */
    private void calculateMissingUsages() {
        ...
        RetryingTransactionCallback<Object> getUsersWithoutUsage =
                new RetryingTransactionCallback<Object>() {
                    public Object execute() throws Throwable {
        ...
        transactionService
                .getRetryingTransactionHelper()
                .doInTransaction(getUsersWithoutUsage, true);
    ...
    private int recalculateUsages(final Map<String, NodeRef> users) {
        ...
        RetryingTransactionCallback<Long> calculateCurrentUsages =
                new RetryingTransactionCallback<Long>() {
                    public Long execute() throws Throwable {
                        List<String> stores = contentUsageImpl.getStores();
                            ...
                            MapHandler userContentUrlHandler =
                                    new MapHandler() {
                                        public void handle(Map<String, Object> result) {
                                            ...
                                            currentUserUsages.put(
                                                    owner, currentUsage + contentSize);
                                        }
                                    };
        ...
        transactionService
                .getRetryingTransactionHelper()
                .doInTransaction(calculateCurrentUsages, true);
        ...
        List<Pair<NodeRef, Long>> batchUserUsages =
                new ArrayList<Pair<NodeRef, Long>>(updateBatchSize);
    ...
    private int updateUsages(final List<Pair<NodeRef, Long>> userUsages) {
        RetryingTransactionCallback<Integer> updateCurrentUsages =
                new RetryingTransactionCallback<Integer>() {
                    public Integer execute() throws Throwable {
                        int updateCount = 0;
        ...
        return transactionService
                .getRetryingTransactionHelper()
                .doInTransaction(updateCurrentUsages, false);
    }
    ...
    private void collapseUsages() {
        ...
        RetryingTransactionCallback<Set<NodeRef>> getUsageNodeRefs =
                new RetryingTransactionCallback<Set<NodeRef>>() {
        ...
        // execute in READ-ONLY txn
        Set<NodeRef> usageNodeRefs =
                transactionService
                        .getRetryingTransactionHelper()
                        .doInTransaction(getUsageNodeRefs, true);
    ...
    private boolean collapseUsage(final NodeRef usageNodeRef) {
        RetryingTransactionCallback<Boolean> collapseUsages =
                    ...
                    public Boolean execute() throws Throwable {
                        ...
                        if (nodeType.equals(ContentModel.TYPE_PERSON)) {
                            ...
                            if (currentUsage != -1) {
                                ...
                                // Calculate and remove deltas in one go to guard against deletion
                                // of
                                // deltas from another transaction that have not been included in
                                // the
                                // calculation
                                ...
                                if (logger.isWarnEnabled()) {
                                    logger.warn(
                                            "Initial usage for user has not yet been calculated: "
                                                    + userName);
                                }
                            }
                        }

...
public class SubsystemChainingAuthenticationService
  ...
  public MutableAuthenticationService getMutableAuthenticationService() {
    ...
    try {
      refreshBeans();
      for (String instance : this.instanceIds) {
        AuthenticationService authenticationService = (AuthenticationService) this.sourceBeans.get(
            instance
          );
        // Only add active authentication services. E.g. we might have an ldap context that is only used for
        // synchronizing
        if (
          authenticationService instanceof MutableAuthenticationService &&
          (

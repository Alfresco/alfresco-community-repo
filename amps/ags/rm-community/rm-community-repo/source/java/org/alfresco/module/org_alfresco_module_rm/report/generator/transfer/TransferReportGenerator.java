...
public class TransferReportGenerator extends DeclarativeReportGenerator {
   ...
   */
  @Override
  protected Map<String, Serializable> generateReportTemplateContext(
   ...
   */
  private Map<String, Serializable> getTransferNodeProperties(
    NodeRef childRef
  ) {
    Map<String, Serializable> transferNodeProperties = new HashMap<>(6);

    boolean isFolder = dictionaryService.isSubClass(
      nodeService.getType(childRef),
      ContentModel.TYPE_FOLDER
    );
    transferNodeProperties.put("isFolder", isFolder);

    if (isFolder) {
      Map<String, Serializable> folderProperties = getFolderProperties(
        childRef
      );
      transferNodeProperties.putAll(folderProperties);
    } else {
      Map<String, Serializable> recordProperties = getRecordProperties(
        childRef
      );
  ...
  private Map<String, Serializable> getCommonProperties(NodeRef nodeRef) {
    ...
    String identifier = (String) properties.get(
  ...
  private Map<String, Serializable> getFolderProperties(NodeRef folder) {
    ...
    ArrayList<TransferNode> records = (ArrayList<TransferNode>) getRecords(
      folder
    );
  ...
  private Map<String, Serializable> getRecordProperties(NodeRef record) {
    ...
    String declaredBy = (String) properties.get(
      RecordsManagementModel.PROP_DECLARED_BY
    );
    Date declaredOn = (Date) properties.get(
  ...
  private String getDispositionAuthority(List<TransferNode> transferNodes) {
    ...
    return dispositionAuthority == null
      ? StringUtils.EMPTY

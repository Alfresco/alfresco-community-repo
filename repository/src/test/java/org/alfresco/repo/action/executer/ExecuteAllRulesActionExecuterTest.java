...
public class ExecuteAllRulesActionExecuterTest extends BaseSpringTest {
    ...
    protected void before() throws Exception {
        ...
        fileFolderService =
                applicationContext.getBean("fileFolderService", FileFolderService.class);
    ...
    /**
     * ALF-17517: if inbound "Add classifiable Aspect" rule is applied on folder test that no
     * InvalidNodeRefException is thrown during Checkout/Checkin for the document in folder. Such
     * case can happen during revert to older document version.
     */
                ...
                new RetryingTransactionCallback<Object>() {
                    public Object execute() throws Throwable {
                        ...
                        Map<String, Serializable> props =
                                new HashMap<String, Serializable>(2, 1.0f);
                        ...
                        // Execute the action
                        ActionImpl action =
                                new ActionImpl(null, ID, ExecuteAllRulesActionExecuter.NAME, null);
    ...
    public void testExecution() {
        ...
        action1.setParameterValue(
                AddFeaturesActionExecuter.PARAM_ASPECT_NAME, ContentModel.ASPECT_VERSIONABLE);
        ...
        action3.setParameterValue(
                AddFeaturesActionExecuter.PARAM_ASPECT_NAME, ContentModel.ASPECT_TITLED);
        ...
        transactionHelper.doInTransaction(
                new RetryingTransactionCallback<Object>() {
                    public Object execute() throws Throwable {
                        ...
                        assertTrue(nodeService.hasAspect(doc2, ContentModel.ASPECT_CLASSIFIABLE));
                        assertFalse(nodeService.hasAspect(doc2, ContentModel.ASPECT_TITLED));
                        // Running recursively (PARAM_RUN_ALL_RULES_ON_CHILDREN), so folder2 has
                        // folder1's rules applied
     ...
     * Test for MNT-13055:
     *
     * <p>We create a folder structure thus:
     *
     * <pre>
     ...
     *
     * <p>We then explicitly execute folderA's rules, including for subfolders. Ultimately, we want
     * to know that fileB did indeed have the ruleset executed on it, and therefore gained the
     * classifiable aspect.
     */
    public void testRulesFireOnNonInheritedFolderUsingLinkedRuleSetInstead() {
        final NodeRef rootFolder =
                this.nodeService
                        .createNode(
                                this.rootNodeRef,
                                ContentModel.ASSOC_CHILDREN,
        ...
        final NodeRef folderA =
                fileFolderService
                        .create(rootFolder, "folderA", ContentModel.TYPE_FOLDER)
                        .getNodeRef();
        ...
        final NodeRef folderB =
                fileFolderService.create(folderA, "folderB", ContentModel.TYPE_FOLDER).getNodeRef();
        ...
        transactionHelper.doInTransaction(
                () -> {
                    ...
                    actionParams.put(
                            ExecuteAllRulesActionExecuter.PARAM_RUN_ALL_RULES_ON_CHILDREN, true);
                    // In Share, when you manually trigger rules on a folder, this parameter
                    // (execute-inherited-rules)
                    // will be false if the aspect rule:ignoreInheritedRules is present, and true
                    // otherwise.
                    actionParams.put(
                            ExecuteAllRulesActionExecuter.PARAM_EXECUTE_INHERITED_RULES, false);
                    Action rulesAction =
                            actionService.createAction(
                                    ExecuteAllRulesActionExecuter.NAME, actionParams);
    ...
    private FileInfo createFile(NodeRef parent, String name) {
        ...
        ContentWriter writer =
                contentService.getWriter(file.getNodeRef(), ContentModel.PROP_CONTENT, true);
     ...
     *
     * <p>When we use the ExecuteAllRulesActionExecuter to explicitly (e.g. "manually") execute all
     * the rules on a folder and its subfolders, the rule must correctly recurse into those
     * subfolders. This is a more simple test case than the steps described in the above issue, but
     * is crucial to the more complicated case succeeding.
     */
        ...
        transactionHelper.doInTransaction(
                () -> {
                    ...
                    actionParams.put(
                            ExecuteAllRulesActionExecuter.PARAM_RUN_ALL_RULES_ON_CHILDREN, true);
                    actionParams.put(
                            ExecuteAllRulesActionExecuter.PARAM_EXECUTE_INHERITED_RULES, true);
                    Action action =
                            actionService.createAction(
                                    ExecuteAllRulesActionExecuter.NAME, actionParams);

 ...
 *
 * @author Andy Hind, Alan Davis (support v1 and v2 HomeFolderProviders - code from v1
 *     HomeFolderProviders moved into HomeFolderManager).
 */
public class PortableHomeFolderManager implements HomeFolderManager {
    ...
    private Map<String, HomeFolderProvider2> v2Providers =
            new HashMap<String, HomeFolderProvider2>();
    ...
    /**
     * Helper method for {@link HomeFolderProvider2#getHomeFolder} (so that it does not need its own
     * NodeService) that returns a person property value.
     */
    public String getPersonProperty(NodeRef person, QName name) {
        String value =
                DefaultTypeConverter.INSTANCE.convert(
                        String.class, nodeService.getProperty(person, name));

        if (value == null || value.length() == 0) {
            throw new PersonException(
                    "Can not create a home folder when the " + name + " property is null or empty");
        }
        return value;
    }
    ...
    NodeRef resolvePath(HomeFolderProvider2 provider, String pathToResolve) {
        ...
        if (refs.size() != 1) {
            throw new IllegalStateException(
                    "Non-unique path: found : " + pathToResolve + " " + refs.size());
        }
        return refs.get(0);
    }

    /**
     * Helper method for {@link HomeFolderProvider2#getHomeFolder(NodeRef)} implementations to
     * return a {@link HomeSpaceNodeRef}
     *
     * @param referenceRootNode indicates that a reference to the root node should be returned if
     *     the home folder property on the person has not yet been set.
     */
    public HomeSpaceNodeRef getHomeFolder(
            HomeFolderProvider2 provider, NodeRef person, boolean referenceRootNode) {
        HomeSpaceNodeRef homeSpaceNodeRef = null;
        NodeRef existingHomeFolder =
                DefaultTypeConverter.INSTANCE.convert(
                        NodeRef.class,
                        nodeService.getProperty(person, ContentModel.PROP_HOMEFOLDER));
        if (existingHomeFolder != null) {
            homeSpaceNodeRef =
                    new HomeSpaceNodeRef(existingHomeFolder, HomeSpaceNodeRef.Status.VALID);
        } else if (referenceRootNode) {
            homeSpaceNodeRef =
                    new HomeSpaceNodeRef(
                            getRootPathNodeRef(provider), HomeSpaceNodeRef.Status.REFERENCED);
        } else {
            ...
            FileInfo fileInfo =
                    createTree(
                            provider,
                            getRootPathNodeRef(provider),
                            homeFolderPath,
                            provider.getTemplateNodeRef(),
                            fileFolderService);
    ...
    /**
     * Modifies (if required) the leaf folder name in the {@code homeFolderPath} by appending {@code
     * "-N"} (where N is an integer starting with 1), so that a new folder will be created.
     *
     * @param root folder.
    ...
    private FileInfo createTree(
        ...
        if (templateNodeRef == null) {
            fileInfo =
                    fileFolderService.create(newParent, homeFolderName, ContentModel.TYPE_FOLDER);
        } else {
            try {
    ...
    private NodeRef createNewParentIfRequired(
            NodeRef root, List<String> homeFolderPath, FileFolderService fileFolderService) {
        if (homeFolderPath.size() > 1) {
            List<String> parentPath = new ArrayList<String>(homeFolderPath);
            parentPath.remove(parentPath.size() - 1);
            return FileFolderUtil.makeFolders(
                            fileFolderService, root, parentPath, ContentModel.TYPE_FOLDER)
                    .getNodeRef();
        } else {
            return root;
        }

...
public class CachingCorrespondingNodeResolverImpl
  ...
  private static final Log log = LogFactory.getLog(
    CachingCorrespondingNodeResolverImpl.class
  );

  private Map<NodeRef, ResolvedParentChildPair> cache = new HashMap<NodeRef, ResolvedParentChildPair>(
    359
  );
  ...
  public ResolvedParentChildPair resolveCorrespondingNode(
    ...
    if (result != null) {
      if (log.isDebugEnabled()) {
        log.debug(
          "Found fully-resolved entry in cache for node " + sourceNodeRef
        );
      }
      return result;
    }
    ...
    result =
      delegateResolver.resolveCorrespondingNode(
        sourceNodeRef,
        primaryAssoc,
        parentPath
      );

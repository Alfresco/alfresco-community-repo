...
public class DeleteMethod extends WebDAVMethod implements ActivityPostProducer {
    ...
    @Deprecated private static final Timer deleteDelayTimer = new Timer();
    ...
    protected void executeImpl() throws WebDAVServerException, Exception {
        ...
        if (permissionService.hasPermission(nodeRef, PermissionService.DELETE)
            ...
            // As this content will be deleted, we need to extract some info before it's no longer
            // available.
            ...
            if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_WORKING_COPY)
                ...
                // Mark content as hidden.  This breaks many contracts and will be fixed for
                // "ALF-18619 WebDAV/SPP file shuffles"
                fileFolderService.setHidden(nodeRef, true);
                {
                    // Workaround for MNT-8704: WebDAV:Content does not disappear after being
                    // deleted
                            ...
                            new TimerTask() {
                                ...
                                public void run() {
                                    RunAsWork<Void> deleteDelayRunAs =
                                            new RunAsWork<Void>() {
                                                ...
                                                public Void doWork() throws Exception {
                                                    // Ignore if it is NOT hidden: the shuffle may
                                                    // have finished; the operation may have failed
                                                    if (!nodeService.exists(nodeRef)
                                                            || !fileFolderService.isHidden(
                                                                    nodeRef)) {
                                                        return null;
                                                    }

                                                    // Since this will run in a different thread,
                                                    // the client thread-local must be set
                                                    // or else unhiding the node will not unhide it
                                                    // for WebDAV.
                                                    FileFilterMode.setClient(
                                                            FileFilterMode.Client.webdav);
                                    ...
                                    try {
                                        ...
                                        // consume exception to avoid it leaking from the TimerTask
                                        // and causing the Timer to
                                        // no longer accept tasks to be scheduled.
                                        logger.info(
                                                "Exception thrown during WebDAV delete timer task.",
                                                e);
                                    }
                                }
                            };
                ...
                // MNT-16454: It should be possible to delete files with versionable aspects using
                // webdav; check that
    ...
    protected void postActivity(FileInfo parent, FileInfo deletedFile, String siteId)
        ...
        if (!siteId.equals(WebDAVHelper.EMPTY_SITE_ID)) {
            ...
            activityPoster.postFileFolderDeleted(
                    siteId, tenantDomain, parentPath, parent, deletedFile);
        }
    }

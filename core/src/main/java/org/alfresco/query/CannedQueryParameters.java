...
public class CannedQueryParameters
{
    ...
    /**
     * Construct all the parameters for executing a named query, using values from the {@link PagingRequest}.
     * 
     * @param parameterBean
     *            the values that the query will be based on or <tt>null</tt> if not relevant to the query
     * @param sortDetails
     *            the type of sorting to be applied or <tt>null</tt> for none
     * @param pagingRequest
     *            the type of paging to be applied or <tt>null</tt> for none
     */
    public CannedQueryParameters(
            Object parameterBean,
            CannedQuerySortDetails sortDetails,
     ...
     * @param sortDetails
     *            the type of sorting to be applied or <tt>null</tt> for none
     * @param totalResultCountMax
     *            greater than zero if the query should not only return the required rows but should also return the total number of possible rows up to the given maximum.
    ...
    /**
     * @return if > 0 then the query should not only return the required rows but should also return the total count (number of possible rows) up to the given max if 0 then query does not need to return the total count
     */
    ...
    /**
     * Helper method to get the total number of query results that need to be obtained in order to satisfy the {@link #getPageDetails() paging requirements}, the maximum result count ... and an extra to provide 'hasMore' functionality.
     * 
    ...
    /**
     * @return parameterBean the values that the query will be based on or <tt>null</tt> if not relevant to the query
     */

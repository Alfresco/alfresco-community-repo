...
/**
 * Test enabling (recalculating) and disabling (clearing) and user usages, and also collapsing user
 * usage deltas.
 */
...
public class UserUsageTrackingComponentTest extends TestCase {
                ...
                if (!authenticationService.authenticationExists(userName)) {
                    ...
                    NodeRef content =
                            addTextContent(
                                    ...
                                    sb.toString());
                    contentSize =
                            contentService.getReader(content, ContentModel.PROP_CONTENT).getSize();
    ...
    private void checkCalculated(long factor) {
        for (int i = 1; i <= MAX_USERS; i++) {
            ...
            Long sizeProp =
                    (Long) nodeService.getProperty(personNodeRef, ContentModel.PROP_SIZE_CURRENT);
            assertNotNull(
                    "Property " + ContentModel.PROP_SIZE_CURRENT + "does not exist, yet", sizeProp);

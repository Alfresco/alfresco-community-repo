...
public class CachingDateFormat extends SimpleDateFormat {
  ...
  static ThreadLocal<SimpleDateFormat> S_LOCAL_DATE_FORMAT = ThreadLocal.withInitial(() ->
    newDateFormat(FORMAT_FULL_GENERIC)
  );

  static ThreadLocal<SimpleDateFormat> S_LOCAL_DATEONLY_FORMAT = ThreadLocal.withInitial(() ->
    newDateFormat(FORMAT_DATE_GENERIC)
  );

  static ThreadLocal<SimpleDateFormat> S_LOCAL_TIMEONLY_FORMAT = ThreadLocal.withInitial(() ->
    newDateFormat(FORMAT_TIME_GENERIC)
  );

  static ThreadLocal<SimpleDateFormat> S_LOCAL_CMIS_SQL_DATETIME = ThreadLocal.withInitial(() ->
    newDateFormat(FORMAT_CMIS_SQL)
  );

  static ThreadLocal<SimpleDateFormat> S_LOCAL_SOLR_DATETIME = ThreadLocal.withInitial(() -> {
      CachingDateFormat formatter = newDateFormatWithLocale(
        FORMAT_SOLR,
        Locale.ENGLISH
      );
      /*
  ...
  static ThreadLocal<SimpleDateFormatAndResolution[]> S_LENIENT_PARSERS = ThreadLocal.withInitial(() ->
  ...
  public static SimpleDateFormat getDateFormat(
    ...
    SimpleDateFormat dateFormat = (SimpleDateFormat) CachingDateFormat.getDateInstance(
      length,
      locale
    );
  ...
  public static SimpleDateFormat getDateTimeFormat(
    ...
    SimpleDateFormat dateFormat = (SimpleDateFormat) CachingDateFormat.getDateTimeInstance(
      dateLength,
      timeLength,
      locale
    );
  ...
  public static Pair<Date, Integer> lenientParse(
    String text,
   ...
   * @return new non-lenient {@link CachingDateFormat} instance.
   */
  private static CachingDateFormat newDateFormatWithLocale(
    String pattern,
    Locale locale
  ) {

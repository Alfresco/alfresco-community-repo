...
public class ContentPropertyRestrictionInterceptor
  ...
  public void setGlobalContentPropertyRestrictions(
    boolean globalContentPropertyRestrictions
  ) {
      ...
      if (methodName.equals("setProperties")) {
        ...
        if (nodeRef != null) {
          for (QName propQname : properties.keySet()) {
            if (
              isContentProperty(propQname, properties.get(propQname)) &&
              isContentNotNullOrEmpty(properties.get(propQname)) &&
              isContentChanged(nodeRef, propQname, properties.get(propQname))
            ) {
              throw new InvalidTypeException(
                "The node's content can't be updated via NodeService#setProperties directly: \n" +
                "   node: " +
                args[0] +
                "\n" +
                "   property name: " +
        ...
        for (QName propQname : properties.keySet()) {
          if (
            ...
            throw new InvalidTypeException(
              "The node's content can't be updated via NodeService#createNode directly: \n" +
              "   node: " +
              args[0] +
              "\n" +
              "   property name: " +
        ...
        if (nodeRef != null) {
          if (
            isContentProperty(propQname, value) &&
        ...
        if (nodeRef != null) {
          for (QName propQname : properties.keySet()) {
            if (
              ...
              throw new InvalidTypeException(
                "The node's content can't be updated via NodeService#addAspect directly: \n" +
                "   node: " +
                args[0] +
                "\n" +
                "   property name: " +
  ...
  private boolean isContentChanged(
    ...
    Serializable existingValue = internalNodeService.getProperty(
      nodeRef,
      qname
    );
    if (existingValue == null) {
      ...
      if (
        newValue instanceof ContentData && existingValue instanceof ContentData
      ) {
  ...
  private boolean isContentProperty(
    QName propertyQName,

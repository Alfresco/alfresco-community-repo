...
/**
 * A base class for {@link PropertyBackedBean}s. Gets its category from its Spring bean name and
 * automatically propagates and resolves property defaults on initialization. Automatically destroys
 * itself on server shutdown. Communicates its creation and destruction and start and stop events to
 * a {@link PropertyBackedBeanRegistry}. Listens for start and stop events from remote nodes in
 * order to keep the bean in sync with edits made on a remote node. On receiving a start event from
 * a remote node, the bean is completely reinitialized, allowing it to be resynchronized with any
 * persisted changes.
 *
 * @author dward
 */
public abstract class AbstractPropertyBackedBean
    ...
    private List<String> instancePath =
            Collections.singletonList(AbstractPropertyBackedBean.DEFAULT_INSTANCE_NAME);
    ...
    public void setEarlyPropertyCheckers(
            Map<String, SubsystemEarlyPropertyChecker> earlyPropertyCheckers) {
        this.earlyPropertyCheckers = earlyPropertyCheckers;
    }

    /**
     * Used in conjunction with {@link #localSetProperties} to control setting of properties from
     * either a JMX client or by code in the local Alfresco node calling {@link
     * AbstractPropertyBackedBean#setProperties(Map)} or {@link
     * AbstractPropertyBackedBean#setProperty(String, String)}. Is <code>true</code> when there is a
     * nested call to either of these methods. This is the case when there is an MBean AND one of
     * these method was NOT originally called from that MBean (it is a local code).
     */
    private ThreadLocal<Boolean> nestedCall =
    ...
    /**
     * Used in conjunction with {@link #nestedCall} to control setting of properties from either a
     * JMX client or by code in the local Alfresco node calling {@link
     * AbstractPropertyBackedBean#setProperties(Map)} or {@link
     * AbstractPropertyBackedBean#setProperty(String, String)}. Is set to <code>true</code> when
     * there is a nested call back from a JMX bean.
     */
    private ThreadLocal<Boolean> localSetProperties =
            new ThreadLocal<Boolean>() {
                @Override
                protected Boolean initialValue() {
                    return false;
     ...
     *
     * @param autoStart <code>true</code> if the bean should be started on startup of the parent
     *     application context
     */
     ...
     *
     * @param propertyDefaults the property defaults
     */
    ...
    /**
     * Resolves the default value of a property, if there is one, expanding placholders as
     * necessary.
     *
    ...
    protected String resolveDefault(String name) {
        ...
        if (encryptedPropertyDefaults != null) {
            for (Object key : encryptedPropertyDefaults.keySet()) {
                props.setProperty(
                        (String) key, encryptedPropertyDefaults.getProperty((String) key));
            }
        }
     ...
     * Gets the state.
     *
     * @param start are we making use of the state? I.e. should we start it if it has not been
     *     already?
     * @return the state
     */
    ...
    /**
     * When set to true, calls to setProperties / setProperty are persisted to the MBean if it
     * exists.
     */
    ...
    public String performEarlyPropertyChecks(Map<String, String> properties) {
        if (properties != null && !properties.isEmpty() && earlyPropertyCheckers != null) {
            List<InvalidPropertyValueException> exceptions =
                    new ArrayList<InvalidPropertyValueException>();

            for (String property : properties.keySet()) {
                if (earlyPropertyCheckers.containsKey(property)) {
                    try {
                        SubsystemEarlyPropertyChecker propertyChecker =
                                earlyPropertyCheckers.get(property);

                        if (propertyChecker != null) {
                            if (propertyChecker.getPairedPropertyName() != null
                                    && properties.containsKey(
                                            propertyChecker.getPairedPropertyName())) {
                                propertyChecker.checkPropertyValue(
                                        property,
                                        properties.get(property),
                                        properties.get(propertyChecker.getPairedPropertyName()));
                            } else {
                                propertyChecker.checkPropertyValue(
                                        property, properties.get(property), null);
                            }
                        }
     ...
     *
     * @param isPermanent is the component being destroyed forever, i.e. should persisted values be
    ...
    public void onApplicationEvent(ApplicationEvent event) {
        if (this.autoStart
                ...
                && event.getSource() == this.parent) {
            this.lock.writeLock().lock();
            try {
                start(false, false);
            } catch (Exception e) {
                // Let's log and swallow auto-start exceptions so that they are non-fatal. This
                // means that the system
                // can hopefully be brought up to a level where its configuration can be edited and
                // corrected
            ...
            this.lock.writeLock().lock();
            try {
                // If we aren't started, reinitialize so that we pick up state changes from the
                // database
            ...
            this.lock.writeLock().lock();
            try {
                // Completely destroy the state so that it will have to be reinitialized should the
                // bean be put back in
                // to use by this node
                destroy(false);
            } finally {
    ...
    private void setPropertiesInternal(Map<String, String> properties) {
        ...
        try {
            ...
            // We still haven't broadcast the start - a persist is required first so this will be
            // done by the caller
        } catch (Exception e) {
     ...
     * {@inheritDoc}
     *
     * <p>When called from code within the local node the values are saved to the database in the
     * Enterprise edition and will be visible in a JMX client.
     *
     * <p>
     *
     * @param name
            ...
            try {
                ...
                if (localSetProperties.get()) {
                    if (mBeanInfoChange) {
                        ...
                        //                      Commented out to avoid "UserTransaction is not
                        // visible from class loader" as it drops the context.
     ...
     *
     * @param broadcastNow Should the event be broadcast immediately?
     * @param broadcastLater Should the event be broadcast ever?
     */
            ...
            try {
                switch (this.runtimeState) {
                    ...
                    case STOPPED:
                        ...
                        this.runtimeState =
                                broadcastLater
                                        ? RuntimeState.PENDING_BROADCAST_START
     ...
     *
     * @param broadcast Should the event be broadcast?
     */
    ...
    /**
     * Uses a Spring {@link PropertyPlaceholderHelper} to resolve placeholders in the property
     * defaults. This means that placeholders need not be displayed in the configuration UI or JMX
     * console.
     */

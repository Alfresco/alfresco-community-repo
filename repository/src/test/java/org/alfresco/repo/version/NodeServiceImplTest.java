...
public class NodeServiceImplTest extends BaseVersionStoreTest {
    ...
    private static final String MSG_ERR =
            "This operation is not supported by a version store implementation of the node"
                    + " service.";
    ...
    public void before() throws Exception {
        ...
        this.versionStoreNodeService =
                (NodeService) this.applicationContext.getBean("versionNodeService");
    ...
    public void testGetProperties() {
        ...
        Map<QName, Serializable> versionedProperties =
                this.versionStoreNodeService.getProperties(version.getFrozenStateNodeRef());

        if (logger.isDebugEnabled()) {
            logger.debug("original (" + origProps.size() + "):  " + origProps.keySet());
            logger.debug(
                    "versioned ("
                            + versionedProperties.size()
                            + "): "
                            + versionedProperties.keySet());
        }
        ...
        assertEquals(
                "first version label",
                "0.1",
                versionedProperties.get(ContentModel.PROP_VERSION_LABEL));
    ...
    public void testGetProperty() {
        ...
        Serializable value1 =
                this.versionStoreNodeService.getProperty(version.getFrozenStateNodeRef(), PROP_1);
        ...
        Collection<String> multiValue =
                (Collection<String>)
                        this.versionStoreNodeService.getProperty(
                                version.getFrozenStateNodeRef(), MULTI_PROP);
    ...
    public void testGetChildAssocs() {
        if (logger.isTraceEnabled()) {
            ...
            logger.trace(
                    NodeStoreInspector.dumpNodeStore(
                                    this.dbNodeService,
        ...
        // Create a new versionable node
        NodeRef versionableNode = createNewVersionableNode();
        Collection<ChildAssociationRef> originalChildren =
                this.dbNodeService.getChildAssocs(versionableNode);
        assertNotNull(originalChildren);

        // Store the original children in a map for easy navigation later
        HashMap<String, ChildAssociationRef> originalChildAssocRefs =
                new HashMap<String, ChildAssociationRef>();
        ...
        if (logger.isTraceEnabled()) {
            ...
            logger.trace(
                    NodeStoreInspector.dumpNodeStore(
                            this.dbNodeService, this.versionService.getVersionStoreReference()));
            logger.trace("");
        }

        // Get the children of the versioned node
        Collection<ChildAssociationRef> versionedChildren =
                this.versionStoreNodeService.getChildAssocs(version.getFrozenStateNodeRef());
        ...
        for (ChildAssociationRef versionedChildRef : versionedChildren) {
            ChildAssociationRef origChildAssocRef =
                    originalChildAssocRefs.get(versionedChildRef.getChildRef().getId());
            assertNotNull(origChildAssocRef);

            assertEquals(origChildAssocRef.getChildRef(), versionedChildRef.getChildRef());
            assertEquals(origChildAssocRef.isPrimary(), versionedChildRef.isPrimary());
            assertEquals(origChildAssocRef.getNthSibling(), versionedChildRef.getNthSibling());
        }
    }
    ...
    public void testGetAssociationTargets() {
        ...
        List<AssociationRef> origAssocs =
                this.dbNodeService.getTargetAssocs(versionableNode, RegexQNamePattern.MATCH_ALL);

        // Create a new version
        Version version = createVersion(versionableNode, this.versionProperties);

        List<AssociationRef> assocs =
                this.versionStoreNodeService.getTargetAssocs(
                        version.getFrozenStateNodeRef(), RegexQNamePattern.MATCH_ALL);
    ...
    public void testGetTargetAssocsByPropertyValue() {
        ...
        // Store the current details of the target associations
        List<AssociationRef> origAssocs =
                this.dbNodeService.getTargetAssocsByPropertyValue(
                        versionableNode, RegexQNamePattern.MATCH_ALL, propertyQName, propertyValue);

        // Create a new version
        Version version = createVersion(versionableNode, this.versionProperties);

        List<AssociationRef> assocs =
                this.versionStoreNodeService.getTargetAssocsByPropertyValue(
    ...
    public void testHasAspect() {
        ...
        boolean test1 =
                this.versionStoreNodeService.hasAspect(
                        version.getFrozenStateNodeRef(), ApplicationModel.ASPECT_UIFACETS);
        assertFalse(test1);

        boolean test2 =
                this.versionStoreNodeService.hasAspect(
                        version.getFrozenStateNodeRef(), ContentModel.ASPECT_VERSIONABLE);
        assertTrue(test2);
    }
    ...
    public void testGetAspects() {
        ...
        Set<QName> aspects =
                this.versionStoreNodeService.getAspects(version.getFrozenStateNodeRef());
    ...
    public void testGetParentAssocs() {
        ...
        NodeRef versionStoreRoot =
                this.dbNodeService.getRootNode(this.versionService.getVersionStoreReference());
     ...
     * operations are not supported as expected.
     */
    ...
    public void testCreateNode() {
        try {
            this.versionStoreNodeService.createNode(
                    dummyNodeRef, null, dummyQName, ContentModel.TYPE_CONTENT);
            fail("This operation is not supported.");
        } catch (UnsupportedOperationException exception) {
            if (exception.getMessage() != MSG_ERR) {
                fail(
                        "Unexpected exception raised during method excution: "
    ...
    public void testAddChild() {
        try {
            this.versionStoreNodeService.addChild(
                    this.dummyNodeRef, this.dummyNodeRef, this.dummyQName, this.dummyQName);
            fail("This operation is not supported.");
        } catch (UnsupportedOperationException exception) {
            if (exception.getMessage() != MSG_ERR) {
                fail(
    ...
    public void testSetProperties() {
        try {
            this.versionStoreNodeService.setProperties(
                    this.dummyNodeRef, new HashMap<QName, Serializable>());
            fail("This operation is not supported.");
        } catch (UnsupportedOperationException exception) {
            if (exception.getMessage() != MSG_ERR) {
                fail(
    ...
    public void testCreateAssociation() {
        try {
            this.versionStoreNodeService.createAssociation(
                    this.dummyNodeRef, this.dummyNodeRef, this.dummyQName);
            fail("This operation is not supported.");
        } catch (UnsupportedOperationException exception) {
            if (exception.getMessage() != MSG_ERR) {
                fail(
    ...
    /**
     * Tests that we can store and retrieve unicode properties and association names. If there's
     * something wrong with how we're setting up the database or database connection WRT unicode,
     * this is a test that'll hopefully break in testing and alert us!
     */
    @Test
    public void testUnicodeNamesAndProperties() {
        ...
        cachesToClear.add(
                (TransactionalCache) this.applicationContext.getBean("propertyValueCache"));
        cachesToClear.add((TransactionalCache) this.applicationContext.getBean("node.nodesCache"));
        cachesToClear.add(
                (TransactionalCache) this.applicationContext.getBean("node.propertiesCache"));
        ...
        assertEquals(
                1,
                nodeService
                        .getChildAssocs(this.rootNodeRef, ContentModel.ASSOC_CONTAINS, engQName)
                        .size());
        assertEquals(
                engNode,
                nodeService.getChildByName(rootNodeRef, ContentModel.ASSOC_CONTAINS, engProp));

        // Now French
        String frProp =
                "C'est une propri\u00e9t\u00e9 en fran\u00e7ais"; // C'est une propri̩t̩ en
        // fran̤ais
        ...
        assertEquals(
                1,
                nodeService
                        .getChildAssocs(this.rootNodeRef, ContentModel.ASSOC_CONTAINS, frQName)
                        .size());
        assertEquals(
                frNode,
                nodeService.getChildByName(rootNodeRef, ContentModel.ASSOC_CONTAINS, frProp));
        ...
        // Next Spanish
        String esProp =
                "Esta es una propiedad en Espa\u00f1ol"; // Esta es una propiedad en Espa̱ol
        ...
        assertEquals(
                1,
                nodeService
                        .getChildAssocs(this.rootNodeRef, ContentModel.ASSOC_CONTAINS, esQName)
                        .size());
        assertEquals(
                esNode,
                nodeService.getChildByName(rootNodeRef, ContentModel.ASSOC_CONTAINS, esProp));
        ...
        assertEquals(
                1,
                nodeService
                        .getChildAssocs(this.rootNodeRef, ContentModel.ASSOC_CONTAINS, jpQName)
                        .size());
        assertEquals(
                jpNode,
                nodeService.getChildByName(rootNodeRef, ContentModel.ASSOC_CONTAINS, jpProp));
    ...
    public void testALF1793AddVersionableAspect() {
        ...
        newProperties.put(
                ContentModel.PROP_NODE_UUID, oldProperties.get(ContentModel.PROP_NODE_UUID));
    ...
    public void testTakeOwnershipPermission() {
        ...
        TestWithUserUtils.createUser(
                userName, PWD, this.rootNodeRef, this.nodeService, this.authenticationService);
        ...
        TestWithUserUtils.authenticateUser(
                userName, PWD, this.rootNodeRef, this.authenticationService);

        // Check if a user has not the "take ownership" permission directly through
        // permissionService
        boolean isAble =
                AccessStatus.ALLOWED
                        == permissionService.hasPermission(
                                docRef, PermissionService.TAKE_OWNERSHIP);
        ...
        String updatedOwner =
                (String) this.nodeService.getProperty(docRef, ContentModel.PROP_OWNER);

        boolean isUserOwner = updatedOwner == null || !updatedOwner.equals(userName) ? false : true;
        assertEquals(
                "Ownership's rights to the document have been taken by the user that has Editor"
                        + " permissions (addAspect).",
                false,
                isUserOwner);
        ...
        try {
            proxyNodeService.setProperty(
                    docRef, ContentModel.ASPECT_OWNABLE, (Serializable) userName);
        } catch (AccessDeniedException e) {
        }

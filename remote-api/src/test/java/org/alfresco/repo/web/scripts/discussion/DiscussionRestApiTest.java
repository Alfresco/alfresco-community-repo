...
public class DiscussionRestApiTest extends BaseWebScriptTest {
  ...
  private static final String SITE_SHORT_NAME_DISCUSSION =
    "DiscussionSiteShortNameThree";
  ...
  private static final String URL_FORUM_SITE_POSTS =
    ...
    COMPONENT_DISCUSSION +
    "/posts";
  private static final String URL_FORUM_NODE_POST_BASE =
    "/api/forum/post/node/"; // Plus node id
   ...
   */
  private JSONObject createSitePost(
    String title,
    String content,
    int expectedStatus
  ) throws Exception {
  ...
  private JSONObject doCreatePost(
    ...
    Response response = sendRequest(
      new PostRequest(url, post.toString(), "application/json"),
      expectedStatus
    );
  ...
  private JSONObject doUpdatePost(
    String url,
    String title,
    String content,
    int expectedStatus
    ...
    post.put("content", content);
    Response response = sendRequest(
      new PutRequest(url, post.toString(), "application/json"),
      expectedStatus
    );
  ...
  private JSONObject getPost(NodeRef nodeRef, int expectedStatus)
    throws Exception {
    return doGetPost(getPostUrl(nodeRef), expectedStatus);
  }

  private JSONObject doGetPost(String url, int expectedStatus)
    throws Exception {
  ...
  private JSONObject getReplies(String name, int expectedStatus)
    throws Exception {
    return doGetReplies(getRepliesUrl(name), expectedStatus);
  }

  private JSONObject getReplies(NodeRef nodeRef, int expectedStatus)
    throws Exception {
    return doGetReplies(getRepliesUrl(nodeRef), expectedStatus);
  }

  private JSONObject doGetReplies(String url, int expectedStatus)
    throws Exception {
    Response response = sendRequest(new GetRequest(url), expectedStatus);
    if (expectedStatus == Status.STATUS_OK) {
      JSONObject result = new JSONObject(response.getContentAsString());
  ...
  private JSONObject getPosts(String type, int expectedStatus)
    throws Exception {
    return doGetPosts(URL_FORUM_SITE_POSTS, type, expectedStatus);
  }

  private JSONObject getPosts(NodeRef nodeRef, String type, int expectedStatus)
    throws Exception {
    return doGetPosts(getPostsUrl(nodeRef), type, expectedStatus);
  }

  private JSONObject doGetPosts(
    String baseUrl,
    String type,
    int expectedStatus
  ) throws Exception {
  ...
  private JSONObject deletePost(String name, int expectedStatus)
    throws Exception {
    return doDeletePost(URL_FORUM_SITE_POST + name, expectedStatus);
  }

  private JSONObject deletePost(NodeRef nodeRef, int expectedStatus)
    throws Exception {
    return doDeletePost(getPostUrl(nodeRef), expectedStatus);
  }

  private JSONObject doDeletePost(String url, int expectedStatus)
    throws Exception {
  ...
  private void pushCreatedDateBack(NodeRef node, int daysAgo) throws Exception {
    ...
    Date newCreated = new Date(
      created.getTime() - daysAgo * 24 * 60 * 60 * 1000
    );
    Date published = (Date) nodeService.getProperty(
      node,
      ContentModel.PROP_PUBLISHED
    );
    if (published == null) published = created;
    Date newPublished = new Date(
      published.getTime() - daysAgo * 24 * 60 * 60 * 1000
    );
    ...
    internalNodeService.setProperty(
      node,
      ContentModel.PROP_CREATED,
      newCreated
    );
  ...
  public void testAddReply() throws Exception {
    ...
    assertEquals(
      topicNodeRef,
      nodeService.getPrimaryParent(replyNodeRef).getParentRef()
    );
    assertEquals(
      topicNodeRef,
      nodeService.getPrimaryParent(reply2NodeRef).getParentRef()
    );
    ...
    assertEquals(
      0,
      nodeService
        .getSourceAssocs(reply2NodeRef, RegexQNamePattern.MATCH_ALL)
        .size()
    );
    assertEquals(
      1,
    ...
    assertEquals(
      1,
      nodeService
        .getTargetAssocs(replyNodeRef, RegexQNamePattern.MATCH_ALL)
        .size()
    );
    assertEquals(
      reply2NodeRef,
  ...
  public void testListings() throws Exception {
    ...
    item =
      createNodePost(FORUM_NODE, "NodeTitle1", "Content", Status.STATUS_OK);
    NodeRef nodeTopic1 = new NodeRef(item.getString("nodeRef"));
    this.authenticationComponent.setCurrentUser(USER_ONE);
    item =
      createNodePost(FORUM_NODE, "NodeTitle2", "Content", Status.STATUS_OK);
    NodeRef nodeTopic2 = new NodeRef(item.getString("nodeRef"));
    item =
      createNodePost(FORUM_NODE, "NodeTitle3", "Content", Status.STATUS_OK);
    ...
    assertEquals(
      "SiteTitle1",
      result.getJSONArray("items").getJSONObject(1).getString("title")
    );
    assertEquals(
      "SiteTitle2",
    ...
    assertEquals(
      2,
      result.getJSONArray("items").getJSONObject(1).getInt("replyCount")
    );
    assertEquals(
      3,
      result.getJSONArray("items").getJSONObject(0).getInt("replyCount")
    );
    ...
    assertEquals(
      "NodeTitle1",
      result.getJSONArray("items").getJSONObject(2).getString("title")
    );
    assertEquals(
      "NodeTitle2",
      result.getJSONArray("items").getJSONObject(1).getString("title")
    ...
    assertEquals(
      1,
      result.getJSONArray("items").getJSONObject(2).getInt("replyCount")
    );
    assertEquals(
      0,
      result.getJSONArray("items").getJSONObject(1).getInt("replyCount")
  ...
  public void testContributorCanEditReply() throws Exception {
    ...
    JSONObject post = createSitePost(
  ...
  private void checkForumPermissions(String siteName) throws Exception {
    String url =
      "/api/forum/site/" + siteName + "/" + COMPONENT_DISCUSSION + "/posts";

...
/**
 * Base implementation of the patch. This class ensures that the patch is thread- and
 * transaction-safe.
 *
 * @author Derek Hulley
 */
public abstract class AbstractPatch implements Patch, ApplicationEventPublisherAware {
    ...
    public AbstractPatch() {
        ...
        this.applied = false;
        this.applyToTenants =
                true; // by default, apply to each tenant, if tenant service is enabled
    ...
    public String toString() {
        ...
        sb.append("Patch")
                ...
                .append(", description=")
                .append(description)
                .append(", fixesFromSchema=")
                .append(fixesFromSchema)
                .append(", fixesToSchema=")
                .append(fixesToSchema)
                .append(", targetSchema=")
                .append(targetSchema)
                .append(", ignored=")
    ...
    /**
     * This ensures that this bean gets registered with the appropriate {@link PatchService
     * service}.
     */
    public void init() {
    ...
    public void setFixesFromSchema(int version) {
        if (version < 0) {
            throw new IllegalArgumentException(
                    "The 'fixesFromSchema' property may not be less than 0");
        }
     ...
     *
     * @param version a schema version number not smaller than the {@link #setFixesFromSchema(int)
     *     from version} number.
     */
    public void setFixesToSchema(int version) {
        if (version < fixesFromSchema) {
            throw new IllegalArgumentException(
                    "'fixesToSchema' must be greater than or equal to 'fixesFromSchema'");
        }
    ...
    /**
     * Set the schema version that this patch attempts to take the existing schema to. This is for
     * informational purposes only, acting as an indicator of intention rather than having any
     * specific effect.
     *
     * @param version a schema version number that must be greater than the {@link #fixesToSchema
     *     max fix schema number}
     */
    public void setTargetSchema(int version) {
        if (version <= fixesToSchema) {
            throw new IllegalArgumentException(
                    "'targetSchema' must be greater than 'fixesToSchema'");
        }
    ...
    /**
     * Set the flag that forces the patch to be forcefully applied. This allows patches to be
     * overridden to induce execution regardless of the upgrade or installation versions, or even if
     * the patch has been executed before.
     *
    ...
    /**
     * Set all the dependencies for this patch. It should not be executed before all the
    ...
    /**
     * Check that the schema version properties have been set appropriately. Derived classes can
     * override this method to perform their own validation provided that this method is called by
     * the derived class.
     */
    ...
    private String applyWithTxns() throws Exception {
        ...
        RetryingTransactionCallback<String> patchWork =
        ...
        if ((tenantAdminService != null) && tenantAdminService.isEnabled() && applyToTenants) {
            ...
            BatchProcessWorkProvider<Tenant> provider =
                    new BatchProcessWorkProvider<Tenant>() {
            ...
            BatchProcessor<Tenant> batchProcessor =
                    new BatchProcessor<Tenant>(
            ...
            BatchProcessWorker<Tenant> worker =
                    new BatchProcessWorker<Tenant>() {
                        @Override
            ...
            if (batchProcessor.getTotalErrorsLong() > 0) {
                sb.append(
                        "\n"
                                + " and failure during update of tennants total success: "
                                + batchProcessor.getSuccessfullyProcessedEntriesLong()
                                + " number of errors: "
                                + batchProcessor.getTotalErrorsLong()
                                + " lastError"
                                + batchProcessor.getLastError());
            } else {
                sb.append(
                        "\n"
                                + " and successful batch update of "
                                + batchProcessor.getTotalResultsLong()
                                + "tennants");
            }
        }
    ...
    private String apply(boolean async) {
        ...
        try {
            AuthenticationUtil.RunAsWork<String> applyPatchWork =
            ...
            String report =
                    AuthenticationUtil.runAs(
                            applyPatchWork, AuthenticationUtil.getSystemUserName());
    ...
    /**
     * This method does the work. All transactions and thread-safety will be taken care of by this
     * class. Any exception will result in the transaction being rolled back. Integrity checks are
     * downgraded for the duration of the transaction.
     *

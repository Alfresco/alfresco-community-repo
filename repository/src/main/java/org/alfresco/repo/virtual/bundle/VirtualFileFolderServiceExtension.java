...
public class VirtualFileFolderServiceExtension
  ...
  public FileInfo asFileInfo(
    ...
    return getTrait()
      .createFileInfo(nodeRef, qNameType, isFolder, false, properties);
  }
  ...
  public List<FileInfo> list(NodeRef contextNodeRef) {
    if (canVirtualize(contextNodeRef)) {
      ...
      List<FileInfo> searchResult = asFileInfos(
        virtualNodes,
        smartStore,
        environment
      );
      ...
      if (mergeActualNode(reference)) {
        List<FileInfo> actualSearch = getTrait()
          .list(actualNodeFrom(reference));
  ...
  protected boolean mergeActualNode(Reference reference)
    throws VirtualizationException {
    return smartStore.canMaterialize(reference);
  }

  protected NodeRef actualNodeFrom(Reference reference)
    throws VirtualizationException {
    return smartStore.materialize(reference);
  }

  protected boolean canVirtualize(NodeRef nodeRef)
    throws VirtualizationException {
  ...
  private Set<QName>[] buildSearchAndIgnore(
    ...
    Set<QName>[] searchAndIgnore = (Set<QName>[]) Array.newInstance(
      Set.class,
      3
    );

    Pair<Set<QName>, Set<QName>> searchTypesAndIgnoreAspects = getTrait()
      .buildSearchTypesAndIgnoreAspects(files, folders, ignoreQNames);
    if (searchTypesAndIgnoreAspects != null) {
      ...
      if (
        (searchTypesQNames != null || ignoreAspectsQNames != null) &&
        ignoreQNames != null
      ) {
  ...
  public PagingResults<FileInfo> list(
    ...
    if (canVirtualize(contextNodeRef)) {
      ...
      Set<QName>[] searchAndIgnore = buildSearchAndIgnore(
        files,
        folders,
        ignoreQNames
      );
      if (mergeActualNode(reference)) {
        PagingResults<Reference> virtualChildren = smartStore.list(
          reference,
          false,
          true,
          files,
        ...
        PagingResultsSource<FileInfo> superSource = new PagingResultsSource<FileInfo>() {
          ...
          public PagingResults<FileInfo> retrieve(PagingRequest pr)
            ...
            try {
              PagingResults<FileInfo> result = theTrait.list(
        ...
        PagingResults<Reference> children = smartStore.list(
          reference,
          true,
          true,
          files,
      ...
      if (mergeActualNode(reference)) {
        ...
        PagingResultsSource<FileInfo> superSource = new PagingResultsSource<FileInfo>() {
          ...
          public PagingResults<FileInfo> retrieve(PagingRequest pr)
            throws PageCollationException {
            try {
              PagingResults<FileInfo> result = getTrait()
                .list(
                  actualNodeFrom(reference),
                  searchTypeQNames,
                  ignoreAspectQNames,
        ...
        FileInfoPropsComparator comparator = new FileInfoPropsComparator(
          sortingPropoerties
        );
  ...
  public FileInfo rename(NodeRef sourceNodeRef, String newName)
    ...
    return getTrait()
      .rename(smartStore.materializeIfPossible(sourceNodeRef), newName);
  }

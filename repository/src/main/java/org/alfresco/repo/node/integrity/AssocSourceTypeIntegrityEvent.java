 ...
 *
 * <p>Checks are ignored if the source node has been deleted.
 *
 * @author Derek Hulley
 */
        ...
        if (sourceDef instanceof TypeDefinition) {
            ...
            if (!found) {
                ...
                if (assocDef.isChild()) {
                    ...
                    if (nodeService.getTargetAssocs(sourceNodeRef, assocDef.getName()).size()
                            == 0) {
                        // The association does not exist any more
                        return;
                    }
                ...
                IntegrityRecord result =
                        new IntegrityRecord(
                                "The association source is missing the aspect required for this"
                                        + " association: \n"
                                        + "   Source Node: "
                                        + sourceNodeRef
                                        + "\n"

...
/**
 * Node class specific for use by Template pages that support Bean objects as part of the model. The
 * default template engine FreeMarker can use these objects and they are provided to support it. A
 * single method is completely freemarker specific - getXmlNodeModel()
 *
 * <p>The class exposes Node properties, children as dynamically populated maps and lists.
 *
 ...
 *
 * <p>See <a href="http://wiki.alfresco.com/wiki/Template_Guide">Template_Guide</a>
 *
 * @author Kevin Roast
 */
public class TemplateNode extends BasePermissionsNode implements NamespacePrefixResolverProvider {
    ...
    public boolean getIsWorkingCopy() {
        return this.services
                .getNodeService()
                .hasAspect(this.nodeRef, ContentModel.ASPECT_WORKING_COPY);
    }
    ...
    public String getParentTypeShort() {
        if (this.parentType == null) {
            this.parentType =
                    this.services.getDictionaryService().getType(getType()).getParentName();
        }
        ...
        if (this.name == null) {
            ...
            if (this.name == null) {
                ChildAssociationRef parentRef =
                        this.services.getNodeService().getPrimaryParent(this.nodeRef);
    ...
    public Map<String, Serializable> getProperties() {
        if (this.propsRetrieved == false) {
            Map<QName, Serializable> props =
                    this.services.getNodeService().getProperties(this.nodeRef);

            for (QName qname : props.keySet()) {
                Serializable value =
                        this.propertyConverter.convertProperty(
    ...
    public Map<String, List<TemplateNode>> getAssocs() {
        if (this.targetAssocs == null) {
            List<AssociationRef> refs =
                    this.services
                            .getNodeService()
                            .getTargetAssocs(this.nodeRef, RegexQNamePattern.MATCH_ALL);
    ...
    public Map<String, List<TemplateNode>> getSourceAssocs() {
        if (this.sourceAssocs == null) {
            List<AssociationRef> refs =
                    this.services
                            .getNodeService()
                            .getSourceAssocs(this.nodeRef, RegexQNamePattern.MATCH_ALL);
    ...
    /**
     * @return The child associations for this Node. As a Map of assoc name to a List of
     *     TemplateNodes.
     */
    public Map<String, List<TemplateNode>> getChildAssocs() {
        if (this.childAssocs == null) {
            List<ChildAssociationRef> refs =
                    this.services.getNodeService().getChildAssocs(this.nodeRef);
    ...
    public List<TemplateNode> getChildAssocsByType(String type) {
        ...
        List<ChildAssociationRef> refs =
                this.services.getNodeService().getChildAssocs(this.nodeRef, types);
    ...
    public TemplateProperties getParent() {
        if (parent == null) {
            NodeRef parentRef =
                    this.services.getNodeService().getPrimaryParent(nodeRef).getParentRef();
    ...
    /**
     * @return the primary parent association so we can access the association QName and association
     *     type QName.
     */
        ...
        if (this.getAspects().contains(ContentModel.ASPECT_VERSIONABLE)) {
            ...
            if (history != null) {
                ...
                for (Version version : history.getAllVersions()) {
                    ...
                    VersionHistoryNode record =
                            new VersionHistoryNode(version, this, this.services);
    ...
    /**
     * @return FreeMarker NodeModel for the XML content of this node, or null if no parsable XML
     *     found
     */
    public NodeModel getXmlNodeModel() {
        try {
            ...
            if (logger.isDebugEnabled()) logger.debug(err.getMessage(), err);
    ...
    /**
     * @return A map capable of returning the TemplateNode at the specified Path as a child of this
     *     node.
     */
    ...
    /**
     * @return A map capable of returning a List of TemplateNode objects from an XPath query as
    ...
    /**
     * @return A map capable of returning a List of TemplateNode objects from an NodeRef to a Saved
     *     Search object. The Saved Search is executed and the resulting nodes supplied as a
     *     sequence.
     */
    ...
    /**
     * @return A map capable of returning a List of TemplateNode objects from an NodeRef to a Lucene
     *     search string. The Saved Search is executed and the resulting nodes supplied as a
     *     sequence.
     */
    ...
    public List<TemplateAuditInfo> getAuditTrail() {
        ...
        final AuditQueryCallback callback =
                new AuditQueryCallback() {
                    ...
                    public boolean handleAuditEntry(
                        ...
                        TemplateAuditInfo auditInfo =
                                new TemplateAuditInfo(applicationName, user, time, values);
                ...
                new RunAsWork<Object>() {
                    public Object doWork() throws Exception {
                        ...
                        services.getAuditService()
                                .auditQuery(callback, pathParams, Integer.MAX_VALUE);
                        ...
                        copyFromPathParams.addSearchKey(
                                "/alfresco-access/transaction/copy/from/path", nodePath);
                        // unfortunately, the getAuditTrail API forces us to get them all
                        services.getAuditService()
                                .auditQuery(callback, copyFromPathParams, Integer.MAX_VALUE);
                        ...
                        moveFromPathParams.addSearchKey(
                                "/alfresco-access/transaction/move/from/path", nodePath);
                        // unfortunately, the getAuditTrail API forces us to get them all
                        services.getAuditService()
                                .auditQuery(callback, moveFromPathParams, Integer.MAX_VALUE);
                        return null;
                    }
                },
        ...
        Collections.sort(
                result,
                new Comparator<TemplateAuditInfo>() {
    ...
    /**
     * This method returns a URL string which resolves to an Alfresco Share view of this node. Note
     * that in order for this method to return meaningful data, the {@link SysAdminParams
     * sysAdminParams} bean must have been configured.
     *
     * <p>Currently this method only produces valid URls for documents and not for folders.
     *
    ...
    public String getShareUrl() {
        ...
        result.append("document-details?nodeRef=").append(getNodeRef());

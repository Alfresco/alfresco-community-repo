...
public class RecordsManagementNodeFormFilter
  ...
  private static Log logger = LogFactory.getLog(
    RecordsManagementNodeFormFilter.class
  );
  ...
  public void afterGenerate(
    ...
    if (getFilePlanService().isFilePlanComponent(nodeRef)) {
      ...
      FilePlanComponentKind kind = getFilePlanService()
      ...
      if (FilePlanComponentKind.RECORD.equals(kind)) {
        ...
        if (
          this.nodeService.hasAspect(nodeRef, ImapModel.ASPECT_IMAP_CONTENT)
        ) {
        ...
        DispositionSchedule schedule = new DispositionScheduleImpl(
          this.rmServiceRegistry,
          this.nodeService,
          nodeRef
        );
   ...
   * @param nodeRef
   */
  protected void addRecordMetadataPropertyFieldsToGroup(
    Form form,
    NodeRef nodeRef
  ) {
    ...
    for (QName aspect : aspects) {
      if (nodeService.hasAspect(nodeRef, aspect)) {
        ...
        addPropertyFieldsToGroup(
          form,
          dictionaryService.getPropertyDefs(aspect),
          setId,
          setLabel
        );
      }
   ...
   */
  protected void forceSupplementalMarkingListProperty(
    Form form,
    NodeRef nodeRef
  ) {
    if (
   ...
   * @param nodeRef
   */
  protected void addTransientProperties(
    final Form form,
    ...
    runAsSystem(
      new RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          DispositionSchedule ds = getDispositionService()
          ...
          if (ds != null) {
            ...
            NodeRef recordCategory = getDispositionService()
              .getAssociatedRecordsManagementContainer(ds);
            if (recordCategory != null) {
              String categoryId = (String) nodeService.getProperty(
                recordCategory,
                PROP_IDENTIFIER
              );
              if (categoryId != null) {
   ...
   * @param value
   */
  protected void addTransientPropertyField(
    Form form,
    String name,
    QName type,
    Object value
  ...
  protected void protectRecordProperties(Form form, NodeRef nodeRef) {
    ...
    for (FieldDefinition fieldDef : fieldDefs) {
      if (!fieldDef.isProtectedField()) {
        ...
        if (
          "size".equals(name) ||
          "mimetype".equals(name) ||
          "encoding".equals(name)
        ) {
  ...
  protected void protectRecordLevelDispositionPropertyField(Form form) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Set 'rma:recordLevelDisposition' field to be protected as record folders or records are present"
      );
    }
  }
}

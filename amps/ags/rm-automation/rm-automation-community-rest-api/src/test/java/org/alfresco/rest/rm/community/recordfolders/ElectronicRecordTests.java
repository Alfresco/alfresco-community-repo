...
public class ElectronicRecordTests extends BaseRMRestTest {
    ...
    public void cantCreateElectronicRecordsInInvalidContainers(
        String container
    ) {
    ...
    public void canCreateElectronicRecordOnlyWithMandatoryProperties(
        String folderId,
        String type
    ) throws Exception {
        ...
        if (RECORD_FOLDER_TYPE.equalsIgnoreCase(type)) {
            ...
            UnfiledContainerChild recordModel = UnfiledContainerChild
                .builder()
                .nodeType(CONTENT_TYPE)
                .build();
            unfiledContainersAPI.createUnfiledContainerChild(
                recordModel,
                folderId
            );
    ...
    public void canCreateElectronicRecordsInValidContainers(
        String folderId,
        String type
    ) throws Exception {
        String newRecordId;
        ...
        if (RECORD_FOLDER_TYPE.equalsIgnoreCase(type)) {
            ...
            newRecordId =
                recordFolderAPI
                    .createRecord(recordModel, folderId, getFile(IMAGE_FILE))
                    .getId();
            ...
            newRecordId =
                unfiledContainersAPI
                    .uploadRecord(recordModel, folderId, getFile(IMAGE_FILE))
                    .getId();
            expectedName = recordModel.getName();
        } else if (UNFILED_RECORD_FOLDER_TYPE.equalsIgnoreCase(type)) {
            UnfiledRecordFolderAPI unfiledRecordFoldersAPI = getRestAPIFactory()
    ...
    public void recordNameDerivedFromFileName(String folderId, String type)
        throws Exception {
        String newRecordId;
        if (RECORD_FOLDER_TYPE.equalsIgnoreCase(type)) {
        ...
        // Get newly created electronic record and verify its properties
        Record electronicRecord = getRestAPIFactory()
            .getRecordsAPI()
            .getRecord(newRecordId);
        ...
        assertTrue(
            electronicRecord
                .getName()
                .startsWith(IMAGE_FILE.substring(0, IMAGE_FILE.indexOf(".")))
        );
        assertTrue(
            electronicRecord
                .getName()
                .contains(electronicRecord.getProperties().getIdentifier())
        );
    }
    ...
    public void createElectronicRecordWithRelativePath() {
        ...
        String parentUbnfiledRecordFolderName =
            "ParentUnfiledRecordFolder" + getRandomAlphanumeric();
        String unfiledRecordFolderPathEl1 =
            "UnfiledRecordFolderPathEl1" + getRandomAlphanumeric();
        String unfiledRecordFolderPathEl2 =
            "UnfiledRecordFolderPathEl2" + getRandomAlphanumeric();
        String unfiledRecordFolderPathEl3 =
            "UnfiledRecordFolderPathEl3" + getRandomAlphanumeric();

        String parentUnfiledRecordFolderId = createUnfiledContainerChild(
            UNFILED_RECORDS_CONTAINER_ALIAS,
            parentUbnfiledRecordFolderName,
            UNFILED_RECORD_FOLDER_TYPE
        )
            .getId();

        String relativePath =
            unfiledRecordFolderPathEl1 +
            "/" +
            unfiledRecordFolderPathEl2 +
            "/" +
            unfiledRecordFolderPathEl3;
        UnfiledContainerChild unfiledContainerChildModel = UnfiledContainerChild
            .builder()
        ...
        UnfiledRecordFolderAPI unfiledRecordFoldersAPI = getRestAPIFactory()
            .getUnfiledRecordFoldersAPI();
        UnfiledContainerChild recordCreated = unfiledRecordFoldersAPI.uploadRecord(
            unfiledContainerChildModel,
            parentUnfiledRecordFolderId,
        ...
        assertEquals(
            unfiledRecordFoldersAPI
                .getUnfiledRecordFolder(record.getParentId())
                .getName(),
            unfiledRecordFolderPathEl3
        );

        // The first relative path element exists and the second one does not exist
        String unfiledRecordFolderPathEl4 =
            "UnfiledRecordFolderPathEl4" + getRandomAlphanumeric();
        relativePath =
            unfiledRecordFolderPathEl1 + "/" + unfiledRecordFolderPathEl4;
        ...
        assertTrue(
            unfiledRecordFoldersAPI
                .getUnfiledRecordFolder(record.getParentId())
                .getName()
                .equals(unfiledRecordFolderPathEl4)
        );

        //the containers from the RELATIVE PATH exists
        unfiledContainerChildModel.setName(
            ELECTRONIC_RECORD_NAME + getRandomAlphanumeric()
        );
        recordCreated =
            unfiledRecordFoldersAPI.uploadRecord(
                unfiledContainerChildModel,
     ...
     */
    @Test(
        description = "Electronic records can be created in record folder with duplicate name"
    )
    ...
    public void canCreateElectronicRecordsWithDuplicateName() {
        ...
        getRestAPIFactory()
            .getRecordFolderAPI()
            .createRecord(recordModel, recordFolder.getId());
        // Verify the status code
        assertStatusCode(CREATED);

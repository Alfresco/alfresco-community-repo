...
public class ActionServiceImpl
   ...
   */
  private static final String POST_TRANSACTION_PENDING_ACTIONS =
    "postTransactionPendingActions";

  /**
   * Error message
   */
  private static final String ERR_FAIL =
    "The action failed to execute due to an error.";
   ...
   */
  private ActionTransactionListener transactionListener = new ActionTransactionListener(
    this
  );
  ...
  public List<ActionConditionDefinition> getActionConditionDefinitions() {
    return new ArrayList<ActionConditionDefinition>(
      this.conditionDefinitions.values()
    );
  }
  ...
  public List<ParameterConstraint> getParameterConstraints() {
    return new ArrayList<ParameterConstraint>(
      this.parameterConstraints.values()
    );
  }
  ...
  public ActionCondition createActionCondition(String name) {
    if (logger.isDebugEnabled()) logger.debug(
      "Creating Action Condition - [" + name + "]"
    );
  ...
  public boolean evaluateAction(Action action, NodeRef actionedUponNodeRef) {
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "\tAll Condition Evaluation Result - " + result
    );
   ...
   *      org.alfresco.service.cmr.repository.NodeRef)
   */
  public boolean evaluateActionCondition(
    ...
    if (condition instanceof CompositeActionCondition) {
      ...
      if (!compositeCondition.hasActionConditions()) {
        throw new IllegalStateException(
          "CompositeActionCondition has no subconditions."
        );
      }
      ...
      for (ActionCondition simplecondition : compositeCondition.getActionConditions()) {
        ...
        if (compositeCondition.isORCondition()) {
          result =
            result ||
            evaluateSimpleCondition(simplecondition, actionedUponNodeRef);
  ...
  private boolean evaluateSimpleCondition(
    ...
    ActionConditionEvaluator evaluator = (ActionConditionEvaluator) this.applicationContext.getBean(
        condition.getActionConditionDefinitionName()
      );
  ...
  private boolean isExecuteAsynchronously(
    ...
    if (executeAsynchronously == false) {
      ...
      if (actionDefinitionName.equals(CompositeActionExecuter.NAME)) {
        ...
        ActionExecuter executer = (ActionExecuter) this.applicationContext.getBean(
            actionDefinitionName
          );
        executeAsynchronously =
          executer.isExecuteAsynchronously(actionedUponNodeRef);
      }
    }
  ...
  private boolean getTrackStatus(Action action) {
    ...
    // action definition.
    ActionDefinition actionDef = getActionDefinition(
      action.getActionDefinitionName()
    );
    if (actionDef == null) {
  ...
  public void executeActionImpl(
    ...
    if (logger.isDebugEnabled() == true) {
      StringBuilder builder = new StringBuilder(
        "Execute action impl action chain = "
      );
      if (actionChain == null) {
        builder.append("null");
      } else {
    ...
    if (
      ...
      !this.nodeService.exists(actionedUponNodeRef)
    ) {
      if (logger.isDebugEnabled() == true) {
        logger.debug(
          "Node = " +
      ...
      try {
        ...
        if (executedAsynchronously == true) {
          ...
          if (compensatingAction != null) {
            ...
            ((ActionImpl) compensatingAction).setTenantId(
                ((ActionImpl) action).getTenantId()
              );
   ...
   *      org.alfresco.service.cmr.repository.NodeRef)
   */
  public void directActionExecution(
    Action action,
    NodeRef actionedUponNodeRef
  ) {
    // Debug output
   ...
   */
  public void registerParameterConstraint(
    ParameterConstraint parameterConstraint
  ) {
    this.parameterConstraints.put(
  ...
  public void saveAction(NodeRef nodeRef, Action action) {
    ...
    if (actionNodeRef == null) {
      if (
        this.nodeService.hasAspect(nodeRef, ActionModel.ASPECT_ACTIONS) == false
      ) {
  ...
  public NodeRef createActionNodeRef(
    ...
    ((ActionImpl) action).setCreator(
        (String) this.nodeService.getProperty(
            actionNodeRef,
            ContentModel.PROP_CREATOR
          )
      );
    ((ActionImpl) action).setCreatedDate(
        (Date) this.nodeService.getProperty(
            actionNodeRef,
  ...
  public void saveActionImpl(NodeRef actionNodeRef, Action action) {
    ...
    ((ActionImpl) action).setModifier(
        (String) this.nodeService.getProperty(
            actionNodeRef,
            ContentModel.PROP_MODIFIER
          )
      );
    ((ActionImpl) action).setModifiedDate(
        (Date) this.nodeService.getProperty(
            actionNodeRef,
  ...
  private void saveActionProperties(NodeRef actionNodeRef, Action action) {
    ...
    Map<QName, Serializable> props =
      this.nodeService.getProperties(actionNodeRef);
    ...
    props.put(
      ActionModel.PROP_EXECUTE_ASYNCHRONOUSLY,
      action.getExecuteAsychronously()
    );

    props.put(
      ActionModel.PROP_EXECUTION_START_DATE,
      action.getExecutionStartDate()
    );
    props.put(
      ActionModel.PROP_EXECUTION_END_DATE,
      action.getExecutionEndDate()
    );
    props.put(
      ActionModel.PROP_EXECUTION_ACTION_STATUS,
      action.getExecutionStatus()
    );
    props.put(
      ActionModel.PROP_EXECUTION_FAILURE_MESSAGE,
      action.getExecutionFailureMessage()
    );
    ...
    if (assocs.size() == 0) {
      if (compensatingAction != null) {
        ...
        NodeRef compensatingActionNodeRef = createActionNodeRef(
          compensatingAction,
          actionNodeRef,
          ActionModel.ASSOC_COMPENSATING_ACTION,
          ActionModel.ASSOC_COMPENSATING_ACTION
   ...
   */
  private void saveCompositeActions(
    NodeRef compositeActionNodeRef,
    ActionList<?> action2
  ) {
  ...
  private void saveActionConditionList(
    ...
    for (String nextId : orderedIds) {
      ...
      if (!isComposite && actionCondition instanceof CompositeActionCondition) {
        ...
        NodeRef conditionNodeRef = saveActionCondition(
          actionNodeRef,
          actionCondition,
          ActionModel.ASSOC_CONDITIONS,
   ...
   */
  private void saveConditionProperties(
    NodeRef conditionNodeRef,
    ActionCondition condition
  ) {
  ...
  private void saveParameters(
    ...
    for (ChildAssociationRef ref : parameters) {
      ...
      String paramName = (String) nodeRefParameterMap.get(
        ActionModel.PROP_PARAMETER_NAME
      );
    ...
    for (Map.Entry<String, Serializable> entry : parameterMap.entrySet()) {
      Map<QName, Serializable> nodeRefProperties = new HashMap<QName, Serializable>(
        2
      );
  ...
  private void populateActionProperties(NodeRef actionNodeRef, Action action) {
    ...
    action.setDescription(
      (String) props.get(ActionModel.PROP_ACTION_DESCRIPTION)
    );
    ...
    Boolean executeAsynchObj = (Boolean) props.get(
      ActionModel.PROP_EXECUTE_ASYNCHRONOUSLY
    );
    boolean executeAsynch = executeAsynchObj == null
      ? false
      : executeAsynchObj.booleanValue();
    ...
    ((ActionImpl) action).setModifier(
        (String) props.get(ContentModel.PROP_MODIFIER)
      );
    ((ActionImpl) action).setModifiedDate(
    ...
    ((ActionImpl) action).setExecutionStartDate(
        (Date) props.get(ActionModel.PROP_EXECUTION_START_DATE)
      );
    ((ActionImpl) action).setExecutionEndDate(
        (Date) props.get(ActionModel.PROP_EXECUTION_END_DATE)
      );
    ((ActionImpl) action).setExecutionStatus(
        ActionStatus.valueOf(
          props.get(ActionModel.PROP_EXECUTION_ACTION_STATUS)
        )
      );
  ...
  private void populateParameters(
    NodeRef parameterizedItemNodeRef,
    ...
    for (ChildAssociationRef parameter : parameters) {
      ...
      Map<QName, Serializable> properties =
        this.nodeService.getProperties(parameterNodeRef);
  ...
  private ActionCondition createActionCondition(NodeRef conditionNodeRef) {
    if (logger.isDebugEnabled()) logger.debug(
      "\tCreateActionCondition: Retrieving Conditions from repository"
    );

    Map<QName, Serializable> properties =
      this.nodeService.getProperties(conditionNodeRef);
    QName conditionType = this.nodeService.getType(conditionNodeRef);

    ActionCondition condition = null;
    if (
      ActionModel.TYPE_COMPOSITE_ACTION_CONDITION.equals(conditionType) == false
    ) {
      condition =
      ...
      CompositeActionCondition compositeCondition = new CompositeActionConditionImpl(
        GUID.generate()
      );
    ...
    Boolean invert = (Boolean) this.nodeService.getProperty(
        conditionNodeRef,
        ActionModel.PROP_CONDITION_INVERT
      );
    condition.setInvertCondition(
      invert == null ? false : invert.booleanValue()
    );
  ...
  private void populateCompositeActionCondition(
    ...
    Boolean OR = (Boolean) this.nodeService.getProperty(
        compositeNodeRef,
        ActionModel.PROP_CONDITION_ANDOR
      );
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "\tPopulating Composite Condition with subconditions, Condition OR = " +
        OR
      );
    }

    compositeActionCondition.setORCondition(
      OR == null ? false : OR.booleanValue()
    );
   ...
   */
  public void populateCompositeAction(
    NodeRef compositeNodeRef,
  ...
  public void removeAllActions(NodeRef nodeRef) {
    if (
      ...
      List<ChildAssociationRef> actions = new ArrayList<ChildAssociationRef>(
        this.nodeService.getChildAssocs(
            getSavedActionFolderRef(nodeRef),
            RegexQNamePattern.MATCH_ALL,
            ActionModel.ASSOC_NAME_ACTIONS
  ...
  private void addPostTransactionPendingAction(
    ...
    if (logger.isDebugEnabled() == true) {
      StringBuilder builder = new StringBuilder(
        "addPostTransactionPendingAction action chain = "
      );
      if (actionChain == null) {
        builder.append("null");
      } else {
    ...
    if (actionChain == null || actionChain.contains(action.getId()) == false) {
      ...
      if (logger.isDebugEnabled() == true) {
        logger.debug(
          "The current user is: " + AuthenticationUtil.getRunAsUser()
        );
      }
      ...
      List<PendingAction> pendingActions = (List<PendingAction>) AlfrescoTransactionSupport.getResource(
      ...
      if (pendingActions.contains(pendingAction) == false) {
        ...
        if (getTrackStatus(action)) {
          actionTrackingService.recordActionPending(
            action,
            actionedUponNodeRef
          );
        }
  ...
  private List<PendingAction> getPostTransactionPendingActions() {
    return (List<PendingAction>) AlfrescoTransactionSupport.getResource(
      POST_TRANSACTION_PENDING_ACTIONS
    );
  }
  ...
  @Override
  public boolean onLogException(
    Action action,
    Log logger,
    Throwable t,
    String message
  ) {
    LoggingAwareExecuter executer = (LoggingAwareExecuter) this.applicationContext.getBean(
        action.getActionDefinitionName()
      );

...
public class Classification extends BaseTemplateProcessorExtension {
  ...
  public List<CategoryTemplateNode> getAllCategoryNodes(QName aspect) {
    return buildCategoryNodes(
      services
        .getCategoryService()
        .getCategories(storeRef, aspect, CategoryService.Depth.ANY)
    );
  }
  ...
  private List<CategoryTemplateNode> buildCategoryNodes(
    Collection<ChildAssociationRef> cars
  ) {
    ArrayList<CategoryTemplateNode> categoryNodes = new ArrayList<CategoryTemplateNode>(
      cars.size()
    );

...
public class JscriptWorkflowDefinition implements Serializable {
   ...
   */
  @SuppressWarnings("unchecked")
  public JscriptWorkflowPath startWorkflow(
    ScriptNode workflowPackage,
    Object properties
  ) {
    ...
    if (properties instanceof ScriptableObject) {
      ...
      workflowParameters =
        new HashMap<QName, Serializable>(scriptableProps.getIds().length);
    ...
    if (context == null) {
      workflowParameters.put(
        ...
        workflowPackage.getNodeRef()
      );
    }

    WorkflowPath cmrWorkflowPath = workflowService.startWorkflow(
      this.id,
      workflowParameters
    );

    return new JscriptWorkflowPath(
      cmrWorkflowPath,
  ...
  public synchronized Scriptable getActiveInstances() {
    ...
    List<WorkflowInstance> cmrWorkflowInstances = workflowService.getActiveWorkflows(
      this.id
    );
    ...
    Scriptable activeInstancesScriptable = (Scriptable) getValueConverter()

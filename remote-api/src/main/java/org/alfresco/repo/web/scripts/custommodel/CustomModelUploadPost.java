...
public class CustomModelUploadPost extends DeclarativeWebScript {
    ...
    protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
        if (!customModelService.isModelAdmin(AuthenticationUtil.getFullyAuthenticatedUser())) {
            throw new WebScriptException(
                    Status.STATUS_FORBIDDEN, PermissionDeniedException.DEFAULT_MESSAGE_ID);
        }

        FormData formData = (FormData) req.parseContent();
        if (formData == null || !formData.getIsMultiPart()) {
            throw new WebScriptException(
                    Status.STATUS_BAD_REQUEST, "cmm.rest_api.model.import_not_multi_part_req");
        }
            ...
            if (field.getIsFile()) {
                ...
                try (ZipFile zipFile = new ZipFile(tempFile, StandardCharsets.UTF_8)) {
                    ...
                    throw new WebScriptException(
                            ...
                            "cmm.rest_api.model.import_not_zip_format",
                            new Object[] {fileName});
                } catch (IOException io) {
                    throw new WebScriptException(
                            Status.STATUS_INTERNAL_SERVER_ERROR,
                            "cmm.rest_api.model.import_process_zip_file_failure",
                            io);
                } finally {
        ...
        if (!processed) {
            throw new WebScriptException(
                    Status.STATUS_BAD_REQUEST, "cmm.rest_api.model.import_no_zip_file_uploaded");
        }
        ...
        try {
            File tempFile =
                    TempFileProvider.createTempFile(
                            inputStream, TEMP_FILE_PREFIX, TEMP_FILE_SUFFIX);
            return tempFile;
        } catch (Exception ex) {
            throw new WebScriptException(
                    Status.STATUS_INTERNAL_SERVER_ERROR,
    ...
    protected ImportResult processUpload(ZipFile zipFile, String filename) throws IOException {
        if (zipFile.size() > 2) {
            throw new WebScriptException(
                    Status.STATUS_BAD_REQUEST, "cmm.rest_api.model.import_invalid_zip_package");
        }
        ...
        while (entries.hasMoreElements()) {
            ...
            if (!entry.isDirectory()) {
                ...
                try (InputStream input =
                        new BufferedInputStream(zipFile.getInputStream(entry), BUFFER_SIZE)) {
                    if (!(entryName.endsWith(CustomModelServiceImpl.SHARE_EXT_MODULE_SUFFIX))
                            && customModel == null) {
                        try {
                            ...
                            if (shareExtModule == null) {
                                // Get the input stream again, as the zip file doesn't support
                                // reset.
                                try (InputStream moduleInputStream =
                                        new BufferedInputStream(
                                                zipFile.getInputStream(entry), BUFFER_SIZE)) {
                                    shareExtModule =
                                            getExtensionModule(moduleInputStream, entryName);
                                }

                                if (shareExtModule == null) {
                                    throw new WebScriptException(
                                            Status.STATUS_BAD_REQUEST,
                                            "cmm.rest_api.model.import_invalid_zip_entry_format",
                        ...
                        if (shareExtModule == null) {
                            throw new WebScriptException(
                                    Status.STATUS_BAD_REQUEST,
                                    "cmm.rest_api.model.import_invalid_ext_module_entry",
                                    new Object[] {entryName});
                        }
                    }
                }
    ...
    protected CustomModel importModel(M2Model m2Model) {
        ...
        try {
            ...
            msg =
                    (msg != null)
                            ? msg.replaceAll("\\d{8,}", "").trim()
                            : "cmm.rest_api.model.import_failure";
    ...
    protected String getExtensionModule(InputStream inputStream, String fileName) {
        ...
        if (rootElement != null
                && SHARE_EXT_MODULE_ROOT_ELEMENT.equals(rootElement.getNodeName())) {

...
public class ReplicationServiceIntegrationTest extends TestCase {
  ...
  private static final Log log = LogFactory.getLog(
    ReplicationServiceIntegrationTest.class
  );
  ...
  protected void setUp() throws Exception {
    ...
    scheduledPersistedActionService =
      (ScheduledPersistedActionService) ctx.getBean(
        "scheduledPersistedActionService"
      );
    ...
    folder1 =
      makeNode(repositoryHelper.getCompanyHome(), ContentModel.TYPE_FOLDER);
    folder2 =
      makeNode(repositoryHelper.getCompanyHome(), ContentModel.TYPE_FOLDER);
  ...
  protected void tearDown() throws Exception {
    ...
    transactionService
      ...
      .doInTransaction(cleanupCallback);
  ...
  public void testCreateSaveLoad() throws Exception {
    ...
    replicationAction
      .getPayload()
      .add(new NodeRef("workspace://SpacesStore/Testing"));
    replicationAction
      .getPayload()
      .add(new NodeRef("workspace://SpacesStore/Testing2"));
    ...
    ReplicationDefinition retrieved = replicationService.loadReplicationDefinition(
      ACTION_NAME
    );
    ...
    ReplicationDefinition second = replicationService.loadReplicationDefinition(
      ACTION_NAME
    );
  ...
  public void testLoadList() throws Exception {
    ...
    // Create and store
    ReplicationDefinition rd1 = replicationService.createReplicationDefinition(
      ACTION_NAME,
      "Test 1"
    );
    ReplicationDefinition rd2 = replicationService.createReplicationDefinition(
      ACTION_NAME2,
      "Test 2"
    );
  ...
  public void testLoadByTarget() throws Exception {
    ...
    assertEquals(
      0,
      replicationService.loadReplicationDefinitions("TestTarget").size()
    );
    assertEquals(
      0,
      replicationService.loadReplicationDefinitions("TestTarget2").size()
    );

    // Store some
    ReplicationDefinition rdTT = replicationService.createReplicationDefinition(
      ACTION_NAME,
      "Test"
    );
  ...
  public void testBasicExecution() throws Exception {
    ...
    try {
      ...
      if (actionService instanceof RuntimeActionService) {
        ...
        assertTrue(
          runtimeActionService.onLogException(rd, log, e, e.getMessage())
        );
      }
    }
    txn.rollback();
    ...
    for (int i = 0; i < 100; i++) {
      ...
      if (
        rd2.getExecutionStatus().equals(ActionStatus.New) ||
  ...
  public void testExecutionResult() throws Exception {
    ...
    String folder2Name = (String) nodeService
      .getProperties(folder2)
      .get(ContentModel.PROP_NAME);
    NodeRef folderT2 = makeNode(
      destinationFolder,
      ContentModel.TYPE_FOLDER,
      folder2Name
    );
    ...
    NodeRef c1 = nodeService
      .getChildAssocs(destinationFolder)
      .get(0)
      .getChildRef();
    NodeRef c2 = nodeService
      .getChildAssocs(destinationFolder)
      .get(1)
      .getChildRef();
    ...
    for (ChildAssociationRef r : nodeService.getChildAssocs(folderT1)) {
      if (
        nodeService.getType(r.getChildRef()).equals(ContentModel.TYPE_AUTHORITY)
      ) {
        fail(
          "Found authority as " +
          r.getChildRef() +
          " but it shouldn't be transfered!"
        );
      }
    }
    ...
    NodeRef folderT2a = nodeService
      .getChildAssocs(folderT2)
      .get(0)
      .getChildRef();
    assertEquals(
    ...
    ContentReader localReader = contentService.getReader(
      localReport,
    ...
    assertTrue(
      "XML not found in:\n" + localReportContent,
      localReportContent.contains("<?xml")
    );
    assertTrue(
    ...
    ContentReader remoteReader = contentService.getReader(
      remoteReport,
      ContentModel.PROP_CONTENT
    );
    ...
    assertTrue(
      "XML not found in:\n" + remoteReportContent,
      remoteReportContent.contains("<?xml")
    );
    assertTrue(
  ...
  public void testTransferDefinitionBuilding() throws Exception {
    ...
    TransferDefinition td = replicationActionExecutor.buildTransferDefinition(
      rd,
      nodes
    );
  ...
  private abstract class DoInTransaction
    implements RetryingTransactionCallback<Void> {
  ...
  public void testScheduling() throws Exception {
    ...
    ReplicationDefinition rd2 = replicationService.loadReplicationDefinition(
      ACTION_NAME
    );
    ...
    assertEquals(
      1,
      replicationService.loadReplicationDefinitions("Target").size()
    );
  ...
  public void testJavascriptAPI() throws Exception {
    ...
    ReplicationDefinition empty = replicationService.createReplicationDefinition(
      ACTION_NAME,
      "Empty"
    );

    ReplicationDefinition persisted = replicationService.createReplicationDefinition(
      ACTION_NAME2,
      "Persisted"
    );
    ...
    ReplicationDefinition persisted2 = replicationService.createReplicationDefinition(
      ACTION_NAME3,
      "Persisted2"
    );
  ...
  private NodeRef makeNode(NodeRef parent, QName nodeType, String name) {
    ...
    NodeRef existing = nodeService.getChildByName(
      parent,
    ...
    if (existing != null) {
      System.err.println(
        "Zapped existing node " + existing + " for name " + name
      );
  ...
  private void makeTransferServiceLocal() {
    TransferReceiver receiver = (TransferReceiver) ctx.getBean(
      "transferReceiver"
    );
    TransferManifestNodeFactory transferManifestNodeFactory = (TransferManifestNodeFactory) ctx.getBean(
      "transferManifestNodeFactory"
    );
    TransferServiceImpl2 transferServiceImpl = (TransferServiceImpl2) ctx.getBean(
      "transferService2"
    );

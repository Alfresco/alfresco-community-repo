...
/**
 * Tasklistener that is notified when a task is created. This will set all default properties for
 * this task.
 *
...
public class TaskCreateListener implements TaskListener {
    ...
    public void notify(DelegateTask task) {
        ...
        // Fetch definition and extract name again. Possible that the default is used if the
        // provided is missing
        TypeDefinition typeDefinition =
                propertyConverter
                        .getWorkflowObjectFactory()
                        .getTaskTypeDefinition(taskFormKey, false);
        ...
        ActivitiScriptNode initiatorNode =
                (ActivitiScriptNode)
                        task.getExecution().getVariable(WorkflowConstants.PROP_INITIATOR);
        if (initiatorNode != null) {
            task.addUserIdentityLink(
                    (String)
                            initiatorNode
                                    .getProperties()
                                    .get(ContentModel.PROP_USERNAME.toPrefixString()),

...
public class InviteServiceTest extends BaseWebScriptTest {
    ...
    private static final String WF_DEFINITION_INVITE =
    ...
    protected void setUp() throws Exception {
        ...
        this.workflowService =
                (WorkflowService) getServer().getApplicationContext().getBean("WorkflowService");
        this.mutableAuthenticationDao =
                (MutableAuthenticationDao)
                        getServer().getApplicationContext().getBean("authenticationDao");
        this.transactionService =
                (TransactionService)
                        getServer().getApplicationContext().getBean("TransactionService");
        this.nodeArchiveService =
                (NodeArchiveService)
                        getServer().getApplicationContext().getBean("nodeArchiveService");
        this.invitationServiceImpl =
                (InvitationServiceImpl)
                        getServer().getApplicationContext().getBean("invitationService");
        ScriptInvitationService scriptInvitationService =
                (ScriptInvitationService)
                        getServer().getApplicationContext().getBean("invitationServiceScript");
        ...
        // This MailExecutorAction bean is named "test-mail" but is in all other respects equivalent
        // to the
                ...
                .doInTransaction(
                        new RetryingTransactionCallback<Void>() {
                            ...
                            public Void execute() throws Throwable {
                                ...
                                // The creation of sites is heavily dependent on the authenticated
                                // user.  We must ensure that,
                                // when doing the runAs below, the user both 'runAs' and
                                // 'fullyAuthenticated'.  In order for
                                // this to be the case, the security context MUST BE EMPTY now.  We
                                // could do the old
                                // "defensive clear", but really there should not be any lurking
                                // authentications on this thread
                                // after the context starts up.  If there are, that is a bug, and we
                                // fail explicitly here.
                                String residuallyAuthenticatedUser =
                                        AuthenticationUtil.getFullyAuthenticatedUser();
                                assertNull(
                                        "Residual authentication on context-initiating thread (this"
                                                + " thread):"
                                ...
                                AuthenticationUtil.runAs(
                                        new RunAsWork<Object>() {
                                            public Object doWork() throws Exception {
                                                // Create first site for Inviter to invite Invitee
                                                // to
                                                SiteInfo siteInfo =
                                                        siteService.getSite(
                                                ...
                                                // Create second site for inviter to invite invitee
                                                // to
                                                siteInfo =
                                                        siteService.getSite(
                                                                SITE_SHORT_NAME_INVITE_2);
                                                if (siteInfo == null) {
                                                    siteService.createSite(
                                                            "InviteSitePreset",
                                                            SITE_SHORT_NAME_INVITE_2,
                                                ...
                                                // Create third site for inviter to invite invitee
                                                // to
                                                siteInfo =
                                                        InviteServiceTest.this.siteService.getSite(
                                                                SITE_SHORT_NAME_INVITE_3);
                                                if (siteInfo == null) {
                                                    siteService.createSite(
                                                            "InviteSitePreset",
                                                            SITE_SHORT_NAME_INVITE_3,
                                                ...
                                                String inviterSiteRole =
                                                        siteService.getMembersRole(
                                                                SITE_SHORT_NAME_INVITE_3,
                                                                USER_INVITER_2);
                                                if ((inviterSiteRole == null)
                                                        || (inviterSiteRole.equals(
                                ...
                                InviteServiceTest.this.authenticationComponent.setCurrentUser(
                                        USER_INVITER);
                                return null;
                            }
                        });
    ...
    public static void configureMailExecutorForTestMode(TestWebScriptServer server) {
        ...
        // 3. Hack into the "mail" bean by programmatically going through the known
        // applicationContext
        ...
        ChildApplicationContextFactory outboundSmptSubsystem =
                (ChildApplicationContextFactory)
                        server.getApplicationContext().getBean("OutboundSMTP");
    ...
    protected void tearDown() throws Exception {
        ...
        RunAsWork<SiteInfo[]> runAsWork =
                new RunAsWork<SiteInfo[]>() {
                    public SiteInfo[] doWork() throws Exception {
                        SiteInfo[] siteInfos = {
                            siteService.getSite(SITE_SHORT_NAME_INVITE_1),
                        ...
                        new RetryingTransactionCallback<Void>() {
                            ...
                            public Void execute() throws Throwable {
                                RunAsWork<Object> runAsWork =
                                        new RunAsWork<Object>() {
                                            public Object doWork() throws Exception {
                                                ...
                                                for (String inviteeEmail : inviteeEmailAddrs) {
                                                    ...
                                                    for (NodeRef person : people) {
                                                        ...
                                                        try {
                                                            ...
                                                            // sometimes, when running single tests,
                                                            // not all users are initialized
                                                            // properly
                                                            exp.printStackTrace();
                                                        }
                                                    }
                                                ...
                                                // Sleep to ensure asynchronous invite deletion
                                                // completes
                                                Thread.sleep(5000);
                                ...
                                WorkflowDefinition wfDef =
                                        InviteServiceTest.this.workflowService.getDefinitionByName(
                                                WF_DEFINITION_INVITE);
                                List<WorkflowInstance> workflowList =
                                        InviteServiceTest.this.workflowService.getActiveWorkflows(
                                                wfDef.id);
                                for (WorkflowInstance workflow : workflowList) {
                                    InviteServiceTest.this.workflowService.cancelWorkflow(
                                            workflow.id);
                                }
                                return null;
                            }
    ...
    private JSONObject startInvite(
    ...
    private JSONObject cancelInvite(String inviteId, String siteShortName, int expectedStatus)
            throws Exception {
        return deleteInvitation(inviteId, siteShortName, expectedStatus);
    }

    /**
     * Adapted from similar method in
     * org.alfresco.repo.web.scripts.invitation.InvitationWebScriptTest
     */
    JSONObject deleteInvitation(String invitationID, String siteShortName, int expectedStatus)
            throws Exception {
        ...
        Response response =
                sendRequest(
                        new DeleteRequest(
                                URL_SITES + "/" + siteShortName + "/invitations/" + invitationID),
                        expectedStatus);
     ...
     * org.alfresco.repo.web.scripts.invitation.InvitationWebScriptTest
     */
    private JSONObject createNominatedInvitation(
            String siteName,
            String inviteeFirstName,
            String inviteeLastName,
            String inviteeEmail,
            String inviteeUserName,
            String inviteeRoleName,
            String serverPath,
            String acceptURL,
            String rejectURL,
            int expectedStatus)
        ...
        Response response =
                sendRequest(
                        new PostRequest(
                                URL_SITES + "/" + siteName + "/invitations",
                                newInvitation.toString(),
                                "application/json"),
                        expectedStatus);
    ...
    private JSONObject rejectInvite(String inviteId, String inviteTicket, int expectedStatus)
            throws Exception {
        // Invitee rejects invitation to a Site from Inviter
        String rejectInviteUrl = URL_INVITE + "/" + inviteId + "/" + inviteTicket + "/reject";
        Response response =
                sendRequest(new PutRequest(rejectInviteUrl, (byte[]) null, null), expectedStatus);
        JSONObject result = new JSONObject(response.getContentAsString());
        return result;
    }
    ...
    private JSONObject getInvitesByInviterUserName(String inviterUserName, int expectedStatus)
            throws Exception {
        // construct get invites URL
    ...
    private JSONObject getInvitesByInviteeUserName(String inviteeUserName, int expectedStatus)
            throws Exception {
        // construct get invites URL
    ...
    private JSONObject getInvitesBySiteShortName(String siteShortName, int expectedStatus)
            throws Exception {
        // construct get invites URL
        String getInvitesUrl = URL_INVITES + "?siteShortName=" + siteShortName;
    ...
    private JSONObject listInvitations(
            String siteShortName, String userNameSearch, int expectedStatus) throws Exception {
        // construct get invites URL
        String getInvitesUrl =
                "/api/sites/"
                        + siteShortName
                        + "/potentialmembers?authorityType=USER&sortBy=fullName&dir=asc&filter="
                        + userNameSearch
                        + "&maxResults=250";
    ...
    private JSONObject getInviteInfo(String inviteId, String inviteTicket, String inviteeUid)
            throws Exception {
        String url =
                "/api/invite/" + inviteId + "/" + inviteTicket + "?inviteeUserName=" + inviteeUid;
    ...
    public void testStartInvite() throws Exception {
        ...
        assertEquals(
                this.inviteeEmailAddrs.get(this.inviteeEmailAddrs.size() - 1),
    ...
    public void testStartInviteWhenInviteeIsAlreadyMemberOfSite() throws Exception {
        ...
        final String inviteeEmailAddr =
                INVITEE_EMAIL_PREFIX + randomStr + "@" + INVITEE_EMAIL_DOMAIN;
                ...
                new RunAsWork<Object>() {
                    public Object doWork() throws Exception {
                        ...
                        InviteServiceTest.this.siteService.setMembership(
                                SITE_SHORT_NAME_INVITE_1, inviteeUserName, INVITEE_SITE_ROLE);
                        return null;
                    }
                },
                USER_INVITER);
    ...
    public void testStartInviteForSameInviteeButTwoDifferentSites() throws Exception {
        ...
        AuthenticationUtil.runAs(
                new RunAsWork<Object>() {
                    public Object doWork() throws Exception {
                        createPerson(
                                INVITEE_FIRSTNAME, INVITEE_LASTNAME, inviteeUsername, inviteeEmail);
                        return null;
                    }
                },
                AuthenticationUtil.getSystemUserName());
    ...
    public void testGetInvitationStatus() throws Exception {
        for (String invitationStatus :
                new String[] {
                    InviteInfo.INVITATION_STATUS_REJECTED, InviteInfo.INVITATION_STATUS_ACCEPTED
                }) {
            // inviter starts invite (sends out invitation)
            JSONObject result =
                    startInvite(
            ...
            if (invitationStatus.equals(InviteInfo.INVITATION_STATUS_REJECTED)) {
                ...
                String acceptInviteUrl =
                        URL_INVITE + "/" + inviteId + "/" + inviteTicket + "/accept";
                sendRequest(new PutRequest(acceptInviteUrl, (byte[]) null, null), Status.STATUS_OK);
            } else {
                fail();
            }
    ...
    public void testGetInvitesByInviterUserName() throws Exception {
        ...
        // above
        JSONObject getInvitesResult = getInvitesByInviterUserName(USER_INVITER, Status.STATUS_OK);
    ...
    public void testGetInvitesByInviteeUserName() throws Exception {
        ...
        JSONObject getInvitesResult =
                getInvitesByInviteeUserName(inviteeUserName, Status.STATUS_OK);
    ...
    public void testGetInvitesBySiteShortName() throws Exception {
        ...
        JSONObject getInvitesResult = getInvitesBySiteShortName(siteShortName, Status.STATUS_OK);
                                ...
                                .doInTransaction(
                                        new RetryingTransactionCallback<Void>() {
                                            public Void execute() throws Throwable {
                                                assertEquals(
                                                        false,
                                                        mutableAuthenticationDao.userExists(
                                                                inviteeUserName));
                                                assertEquals(
                                                        false,
                ...
                new RunAsWork<Object>() {
                    public Object doWork() throws Exception {
                        createPerson(
                                INVITEE_FIRSTNAME,
                                INVITEE_LASTNAME,
                                INVITEE_FIRSTNAME + "_" + INVITEE_LASTNAME,
                                inviteeEmail);
                        return null;
                    }
                },
        ...
        try {
            ...
            JSONObject collInv =
                    startInvite(
                            ...
                            Status.STATUS_CREATED);
            siteService.setMembership(
                    SITE_SHORT_NAME_INVITE_1, collaborator, SiteModel.SITE_COLLABORATOR);

            // get pending invites matching inviter user name used in invite started
            InviteServiceTest.this.authenticationComponent.setCurrentUser(managerUsersArr[1]);
            JSONObject getInvitesResult =
                    getInvitesBySiteShortName(SITE_SHORT_NAME_INVITE_1, Status.STATUS_OK);
            assertEquals(true, getInvitesResult.length() == 1);

            // get site member
            JSONObject userList =
                    listInvitations(SITE_SHORT_NAME_INVITE_1, "user", Status.STATUS_OK);
    ...
    public void testDontOwerrideModeratedSitePermissions() throws Exception {
        ...
        String inviteId =
                createModeratedInvitation(
                        MODERATED_SITE_NAME, "", secondUser, SiteModel.SITE_CONSUMER);
    ...
    private String createModeratedInvitation(
            String siteName, String inviteeComments, String inviteeUserName, String inviteeRoleName)
            throws Exception {

...
public class VersionableAspectTest extends TestCase {
  ...
  public static final String AUTO_VERSION_KEY =
    "version.store.enableAutoVersioning";
  public static final String AUTO_VERSION_PROPS_KEY =
  ...
  private static final String DOCUMENT_NAME =
    "ChildDocumentWithVersionLabel-.txt";

  private static final String PARENT_FOLDER_NAME =
    "ParentFolder-" + System.currentTimeMillis();
  ...
  private NodeService nodeService = (NodeService) applicationContext.getBean(
    "nodeService"
  );
  private LockService lockService = (LockService) applicationContext.getBean(
    "lockService"
  );
  private SearchService searchService = (SearchService) applicationContext.getBean(
    "searchService"
  );
  private ContentService contentService = (ContentService) applicationContext.getBean(
    "contentService"
  );
  private TransactionService transactionService = (TransactionService) applicationContext.getBean(
    "transactionService"
  );
  private CheckOutCheckInService checkOutCheckInService = (CheckOutCheckInService) applicationContext.getBean(
    "checkOutCheckInService"
  );
  private AuthenticationService authenticationService = (AuthenticationService) applicationContext.getBean(
    "authenticationService"
  );
  private Properties globalProperties = (Properties) applicationContext.getBean(
    "global-properties"
  );
    ...
    transactionService
      ...
      .doInTransaction(
        new RetryingTransactionCallback<Void>() {
          @Override
          public Void execute() throws Throwable {
            Map<QName, Serializable> properties = getAndAssertProperties(
              document,
              "0.1"
            );

            Serializable autoVersionProps = properties.get(
              ContentModel.PROP_AUTO_VERSION_PROPS
            );
            assertNotNull(
              (
                "Autoversion property is NULL! NodeRef = '" +
                document.toString() +
                "'"
              ),
              autoVersionProps
            ...
            assertTrue(
              (
                "Autoversion must be TRUE! NodeRef = '" +
                document.toString() +
                "'"
              ),
        ...
        new RetryingTransactionCallback<Void>() {
          ...
          public Void execute() throws Throwable {
            ...
            assertEquals(
              ...
              lockService.getLockStatus(document)
            );
  ...
  private void assertDocumentVersionAndName(
        ...
        new RetryingTransactionCallback<Void>() {
          ...
          public Void execute() throws Throwable {
            ...
            if (versionType != null) {
              ...
              assertTrue(
                versionTypeProperty.toString().equals(versionType.toString())
              );
            }
  ...
  private Map<QName, Serializable> getAndAssertProperties(
    ...
    assertNotNull(
      ("Properties must not be NULL! NodeRef = '" + nodeRef.toString() + "'"),
      properties
    );
    assertFalse(
      (
        "Version specific properties can't be found! NodeRef = '" +
        nodeRef.toString() +
        "'"
      ),
      properties.isEmpty()
  ...
  private String generateDocumentName(String namePattern, String versionLabel) {
    ...
    StringBuilder result = new StringBuilder(nameAndExt[i++])
      .append(versionLabel)
      .append(NAME_AND_EXT_DELIMETER)
      .append(nameAndExt[i++]);

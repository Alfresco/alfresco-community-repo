...
public class MultiFileDumperTest {
  ...
  @Mock
  private DbToXMLFactory dbToXMLFactory;
  ...
  public void canDumpSchemaToFiles() {
    ...
    MultiFileDumper dumper = new MultiFileDumper(
      prefixes,
      directory,
      fileNamePattern,
      dbToXMLFactory,
    ...
    when(
      dbToXMLFactory.create(
        argThat(isFileNameStartingWith("SchemaDump-MySQL-a_-")),
        eq("a_")
      )
    )
    ...
    when(
      dbToXMLFactory.create(
        argThat(isFileNameStartingWith("SchemaDump-MySQL-b_-")),
        eq("b_")
      )
    )
    ...
    when(
      dbToXMLFactory.create(
        argThat(isFileNameStartingWith("SchemaDump-MySQL-c_-")),
        eq("c_")
      )
    )
  ...
  public void canDumpSchemaToFilesForDefaultDBPrefixes() {
    ...
    Map<String, DbToXML> xmlExporters = new HashMap<String, DbToXML>(
      MultiFileDumper.DEFAULT_PREFIXES.length
    );
    ...
    for (String prefix : MultiFileDumper.DEFAULT_PREFIXES) {
      ...
      when(dbToXMLFactory.create(any(File.class), eq(prefix)))
        .thenReturn(dbToXML);
    }
  ...
  private void assertPathCorrect(
    ...
    if (
      !actualFile.getAbsolutePath().startsWith(expectedPath.getAbsolutePath())
    ) {
      String failureMsg =
        "File path " +

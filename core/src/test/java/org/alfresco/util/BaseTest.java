...
public abstract class BaseTest extends TestCase {
  ...
  public BaseTest() {
    ...
    this.resourcesDir =
      resourcesDir + ((loadFromClasspath) ? "" : File.separator);
  }
  ...
  protected void assertFileIsValid(String fullFileName) {
    if (loadFromClasspath) {
      ...
      String resourceName = fullFileName.substring(
        fullFileName.indexOf(":") + 1
      );
      assertNotNull(resourceName);
      URL configResourceUrl = getClass()
        .getClassLoader()
  ...
  protected XMLConfigService initXMLConfigService(
    List<String> xmlConfigFilenames
  ) {
    ...
    ConfigSource configSource = (loadFromClasspath)

...
public class RuntimeExecBeansTest extends TestCase {
    ...
    public void testDeprecatedSetCommandMap() throws Exception {
        ClassPathXmlApplicationContext ctx = new ClassPathXmlApplicationContext(APP_CONTEXT_XML);
        try {
            RuntimeExec deprecatedExec =
                    (RuntimeExec) ctx.getBean("commandCheckDeprecatedSetCommandMap");
    ...
    public void testSplitArguments() throws Exception {
        ...
        try {
            ...
            assertTrue(
                    "Command arguments not split into 'dir', '.' and '..' :"
                            + Arrays.deepToString(splitCommand),
    ...
    // ClassPathXmlApplicationContext(APP_CONTEXT_XML);
    //        try
    //        {
    //            RuntimeExec dirRootExec = (RuntimeExec) ctx.getBean("commandListRootDir");
    //            assertNotNull(dirRootExec);
    ...
    //            assertNotSame("Differently encoded strings should differ", defaultStdOut,
    // mangledStdOut);
    //
    //            // Now convert the Shift-JIS string and ensure it's the same as originally
    // expected
    ...
    //            assertTrue("Expected to be able to convert value back to default charset.",
    // convertedStrOut.contains(defaultStdOut));
    //        }
    //        finally
    //        {
    //            ctx.close();

...
public class NodeEligibleForRethumbnailingEvaluatorTest extends BaseSpringTest {
    private final QName thumbnailDef1 =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "thumbDef1");
    ...
    public void before() throws Exception {
        ...
        // We'll fake this. We won't actually run any thumbnails - just set up the various aspect
        // and child-node
        // conditions we need to test the evaluator.
        this.newUnthumbnailedNodeRef =
                this.nodeService
                        .createNode(
                                this.rootNodeRef,
        ...
        nodeService.addAspect(
                recentlyFailedNodeRef, ContentModel.ASPECT_FAILED_THUMBNAIL_SOURCE, null);
        NodeRef thumbDef1FailureNode =
                nodeService
                        .createNode(
        ...
        nodeService.setProperty(
                thumbDef1FailureNode, ContentModel.PROP_FAILED_THUMBNAIL_TIME, twoSecondsAgo);
    ...
    public void testNodeWithNoFailedThumbnails() {
        ...
        ActionCondition condition =
                new ActionConditionImpl(
                        GUID.generate(), NodeEligibleForRethumbnailingEvaluator.NAME);
        condition.setParameterValue(
                NodeEligibleForRethumbnailingEvaluator.PARAM_THUMBNAIL_DEFINITION_NAME,
                thumbnailDef1.getLocalName());
        // Offset values don't matter here.
        condition.setParameterValue(NodeEligibleForRethumbnailingEvaluator.PARAM_RETRY_PERIOD, 0L);
        condition.setParameterValue(
                NodeEligibleForRethumbnailingEvaluator.PARAM_RETRY_COUNT,
        ...
        condition.setParameterValue(
                NodeEligibleForRethumbnailingEvaluator.PARAM_QUIET_PERIOD_RETRIES_ENABLED, true);

        NodeEligibleForRethumbnailingEvaluator evaluator =
                (NodeEligibleForRethumbnailingEvaluator)
                        this.applicationContext.getBean(
                                NodeEligibleForRethumbnailingEvaluator.NAME);
    ...
    public void testNodeWithFailedThumbnails() {
        ...
        condition.setParameterValue(
                ...
                thumbnailDef1.getLocalName());
        condition.setParameterValue(
                NodeEligibleForRethumbnailingEvaluator.PARAM_RETRY_PERIOD,
                failureHandlingOptions.getRetryPeriod() * 1000);
        condition.setParameterValue(
                NodeEligibleForRethumbnailingEvaluator.PARAM_RETRY_COUNT,
                failureHandlingOptions.getRetryCount());
        condition.setParameterValue(
                NodeEligibleForRethumbnailingEvaluator.PARAM_QUIET_PERIOD,
        ...
        Map<String, FailedThumbnailInfo> failures =
                thumbnailService.getFailedThumbnails(recentlyFailedNodeRef);
        assertFalse(failures.isEmpty());
        final FailedThumbnailInfo failedThumbnailInfo = failures.get(thumbnailDef1.getLocalName());

        final long timeBeforeTheLimit =
                new Date().getTime() - (failureHandlingOptions.getRetryPeriod() * 1000l) - 5000l;
        nodeService.setProperty(
                failedThumbnailInfo.getFailedThumbnailNode(),
        ...
        final long timeBeforeTheLongLimit =
                new Date().getTime() - (failureHandlingOptions.getQuietPeriod() * 1000l) - 5000l;
        nodeService.setProperty(
                failedThumbnailInfo.getFailedThumbnailNode(),
                ContentModel.PROP_FAILED_THUMBNAIL_TIME,
                timeBeforeTheLongLimit);

        assertTrue(evaluator.evaluate(condition, recentlyFailedNodeRef));

        // 5. Unless the retries during the quiet period are disabled...
        condition.setParameterValue(
                NodeEligibleForRethumbnailingEvaluator.PARAM_QUIET_PERIOD_RETRIES_ENABLED, false);

...
/**
 * This JUnit rule can be used to make test methods run as a particular user. A username can be
 * provided on construction to the rule and then all <code>@Test</code> methods will be run as that
 * user.
 *
 * <p>Furthermore, if an individual test method is annotated like this <code>
 * @RunAsUser(userName="John")</code> than that method (and only that method) will be run as "John".
 *
 * <p>Example usage:
 *
 * <pre>
...
public class RunAsFullyAuthenticatedRule implements TestRule {
    /**
     * This annotation can be used to mark an individual {@link Test} method for running as a named
     * user.
     *
     * @author Neil Mc Erlean
     * @since Odin
    ...
    /**
     * This constructs a rule where there is no specified user to run as. For this to be useful (or
     * legal) a user must be specified on every test method using {@link RunAsUser}.
     */
    ...
    /**
     * @param personRule the rule which will provide the username which all test methods should run
     *     as.
     */
            ...
            public void evaluate() throws Throwable {
                ...
                if (runAsUser != null) {
                    ...
                    if (fixedUserName != null) {
                        ...
                        throw new Exception(
                                "Illegal rule: must provide username or "
                                        + AlfrescoPerson.class.getSimpleName()
                                        + " at rule construction or else a "
                                        + RunAsUser.class.getSimpleName()
                                        + " annotation.");
                    }
                }
                ...
                try {
                    // Execute the test method or whatever other rules are configured further down
                    // the stack.
                    base.evaluate();
                } finally {
     ...
     * @return the username specified in the {@link RunAsUser} annotation, if there was one, else
     *     <code>null</code>.
     */
    private String getMethodAnnotatedUserName(Description description)

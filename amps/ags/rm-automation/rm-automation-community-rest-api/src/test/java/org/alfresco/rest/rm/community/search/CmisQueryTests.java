...
public class CmisQueryTests extends BaseRMRestTest {
    ...
    public void setupCmisQuery() throws Exception {
        ...
        for (int i = 0; ++i <= 10; ) {
            FileModel fileModel =
                    new FileModel(
                            String.format(
                                    "%s%s%s.%s",
                                    "Doc", i, SEARCH_TERM, FileType.TEXT_PLAIN.extension));
        ...
        for (int i = 0; ++i <= 10; ) {
            FileModel fileModel =
                    new FileModel(
                            String.format(
                                    "%s%s%s.%s",
                                    "InPlace ", SEARCH_TERM, i, FileType.TEXT_PLAIN.extension));
            fileModel =
                    dataContent
                            .usingUser(nonRMUser)
        ...
        for (int i = 0; ++i <= 10; ) {
            createElectronicRecord(
                    recordFolder.getId(),
                    String.format(
                            "%s%s%s.%s", "Record ", SEARCH_TERM, i, FileType.TEXT_PLAIN.extension));
        }
        STEP(
                "Create an rm user with read permission over the category created and contributor"
                        + " role within the collaboration site");
        rmUser =
                roleService.createUserWithSiteRoleRMRoleAndPermission(
                        collaborationSite,
                        UserRole.SiteContributor,
                        recordFolder.getParentId(),
                        ROLE_RM_MANAGER,
                ...
                () -> {
                    ItemIterable<QueryResult> results =
                            contentActions
                                    ...
                                    .query(sqlWithName, false);
                    assertEquals(
                            "Total number of items is not 30, got  "
    ...
    public void getAllDocumentsNamesCmisQuery() {
        ...
        ItemIterable<QueryResult> results =
                contentActions
                        .getCMISSession(getAdminUser().getUsername(), getAdminUser().getPassword())
                        .query(cq, false);
        ...
        assertTrue(
                "Total number of items is not greater than 100. Total number of items received"
                        + results.getTotalNumItems(),
                results.getTotalNumItems() > 100);
        assertEquals(
                "Expected 100 items per page and got " + results.getPageNumItems() + " per page.",
                100,
    ...
    public void getDocumentsWithSpecificNamesCmisQuery() {
        ...
        ItemIterable<QueryResult> results =
                contentActions
                        .getCMISSession(nonRMUser.getUsername(), nonRMUser.getPassword())
                        .query(sqlWithName, false);
        assertEquals(
                "Total number of items is not 20, got  "
                        + results.getTotalNumItems()
                        + " total items",
                20,
                results.getTotalNumItems());
        ...
        assertEquals(
                "Expected 20 items per page and got " + results.getPageNumItems() + " per page.",
                20,
                results.getPageNumItems());
    }
    ...
    public void getDocumentsCmisQueryWithPagination() {
        ...
        ItemIterable<QueryResult> results =
                contentActions
                        .getCMISSession(rmUser.getUsername(), rmUser.getPassword())
                        .query(sqlWithName, false, oc);

        // check the total number of items and has more items is true
        assertTrue("Has more items not true. ", results.getHasMoreItems());
        assertEquals(
                "Total number of items is not 30, got " + results.getTotalNumItems(),
                30,
                results.getTotalNumItems());
        assertEquals(
                "Expected 10 items per page and got " + results.getPageNumItems() + " per page.",
                10,
                results.getPageNumItems());
    }

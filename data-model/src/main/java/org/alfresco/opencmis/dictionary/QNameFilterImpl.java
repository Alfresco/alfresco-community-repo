...
/**
 * Filters a QName and excludes any that are defined using the excludedTypes parameter. The list of
 * types can either be defined using a property name such as "cm:name" or with a wildcard: "cm:*" It
 * validates the definitions against the DataDictionary.
 *
 * @author steveglover
 * @author Gethin James
 */
public class QNameFilterImpl implements QNameFilter {
    ...
    public Collection<QName> filterQName(Collection<QName> typesToFilter) {
        ...
        if (!excludedQNames.isEmpty() || !excludedModels.isEmpty()) {
            ...
            for (QName classQName : typesToFilter) {
                ...
                //                if (!excludedQNames.contains(classQName) &&
                // !excludedModels.contains(classQName.getNamespaceURI()))
                {
    ...
    public boolean isExcluded(QName typeQName) {
        ...
        return (isExcluded != null && isExcluded.booleanValue()
    ...
    /**
     * I don't like hard code values, but have been persuaded its less pain than having to keep a
     * config file in sync with both sides of SOLR.
     */

...
public class RMAfterInvocationProvider
  ...
  private static Log logger = LogFactory.getLog(
    RMAfterInvocationProvider.class
  );
  ...
  public boolean supports(ConfigAttribute configAttribute) {
    ...
    return (
      StringUtils.isNotBlank(attribute) && attribute.startsWith(AFTER_RM)
    );
  }
      ...
      if (authenticationUtil.isRunAsUserTheSystemUser()) {
        ...
        if (rootNodeRef == null) {
          throw new AlfrescoRuntimeException(
            "Root node reference of '" + returnedObject + "' is null."
          );
        }
        ...
        AssociationRef.class.isAssignableFrom(returnedObject.getClass())
      ) {
        return decide(
          authentication,
          object,
          config,
          (AssociationRef) returnedObject
        );
      } else if (
  ...
  private void checkSupportedDefinitions(
    ...
    for (ConfigAttributeDefintion cad : supportedDefinitions) {
      if (
        (cad.parent && parentResult == AccessDecisionVoter.ACCESS_DENIED) ||
  ...
  private ChildAssociationRef decide(
    ...
    for (ConfigAttributeDefintion cad : supportedDefinitions) {
      ...
      if (
        (cad.parent && parentReadCheck != AccessDecisionVoter.ACCESS_GRANTED) ||
  ...
  private ResultSet decide(
    ...
    ResultSet raw =
      (
        (FilteringResultSet) returnedObject.getWrapped()
    ...
    PagingLuceneResultSet plrs = new PagingLuceneResultSet(
  ...
  private ResultSet decide(
    ...
    if (
      returnedObject
        ...
        .getSearchParameters()
        .getLanguage()
        .equalsIgnoreCase(LANGUAGE_INDEX_SQL)
    ) {
      return returnedObject;
    }
    ...
    RMFilteringResultSet filteringResultSet = new RMFilteringResultSet(
      returnedObject,
      inclusionMask
    );
    ...
    if (
      ...
      maxSize =
        Integer.valueOf(
          returnedObject.getResultSetMetaData().getSearchParameters().getLimit()
        );
    }
    // Allow for skip
    if (
    ...
    for (int i = 0; i < returnedObject.length(); i++) {
      ...
      if (!nodeService.exists(returnedObject.getNodeRef(i))) {
        ...
        int parentCheckRead = checkRead(
          returnedObject.getChildAssocRef(i).getParentRef()
        );
        ...
        for (ConfigAttributeDefintion cad : supportedDefinitions) {
          ...
          if (
            inclusionMask.get(i) &&
            (testNodeRef != null) &&
            (checkRead != AccessDecisionVoter.ACCESS_GRANTED)
          ) {
            inclusionMask.set(i, false);
          }
        }
      ...
      if (
        (maxSize != null) && (filteringResultSet.length() > maxSize.intValue())
      ) {
        ...
        if (cad.mode.equalsIgnoreCase("FilterNode")) {
          ...
          if (cad.parent) {
            if (StoreRef.class.isAssignableFrom(nextObject.getClass())) {
              ...
              testNodeRef =
                nodeService
                  .getPrimaryParent((NodeRef) nextObject)
                  .getParentRef();
            } else if (

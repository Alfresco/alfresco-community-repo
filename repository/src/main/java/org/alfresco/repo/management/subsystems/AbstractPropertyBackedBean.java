...
public abstract class AbstractPropertyBackedBean
  ...
  private List<String> instancePath = Collections.singletonList(
  ...
  private static Log logger = LogFactory.getLog(
    AbstractPropertyBackedBean.class
  );
  ...
  protected String resolveDefault(String name) {
    ...
    if (propertyDefaults != null) {
      for (Object key : propertyDefaults.keySet()) {
        props.setProperty(
          (String) key,
          propertyDefaults.getProperty((String) key)
        );
      }
    }
    ...
    if (encryptedPropertyDefaults != null) {
      for (Object key : encryptedPropertyDefaults.keySet()) {
        props.setProperty(
          (String) key,
          encryptedPropertyDefaults.getProperty((String) key)
        );
      }
    }
    ...
    if (
      ...
      for (String property : properties.keySet()) {
        if (earlyPropertyCheckers.containsKey(property)) {
          try {
            SubsystemEarlyPropertyChecker propertyChecker = earlyPropertyCheckers.get(
              property
            );

            if (propertyChecker != null) {
              if (
                propertyChecker.getPairedPropertyName() != null &&
  ...
  public void setProperties(Map<String, String> properties) {
    ...
    if (!nestedCall.get()) {
      ...
      try {
        boolean mBeanInfoChange = !getPropertyNames()

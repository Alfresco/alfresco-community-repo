...
public class RelationshipServiceImpl
  ...
  public Set<RelationshipDefinition> getRelationshipDefinitions() {
    ...
    Set<Entry<QName, AssociationDefinition>> associationsEntrySet = getCustomAssociations()
      .entrySet();
  ...
  public RelationshipDefinition getRelationshipDefinition(String uniqueName) {
    ...
    AssociationDefinition associationDefinition = getAssociationDefinition(
      uniqueName
    );
  ...
  public RelationshipDefinition createRelationshipDefinition(
    ...
    String title;
    RelationshipType type = determineRelationshipTypeFromDisplayName(
      displayName
    );
    ...
    if (existsTitle(title)) {
      ...
      sb
        .append(
          "Cannot create a relationship definition for the display name: '"
        )
        .append(displayName.toString())
        .append(
          "' as there is already a relationship definition with this display name."
        );
      throw new AlfrescoRuntimeException(sb.toString());
    }
    ...
    QName relationshipDefinitionQName = generateRelationshipDefinitionQNameFor(
      title
    );
    String generatedShortQName = relationshipDefinitionQName.toPrefixString(
      getNamespaceService()
    );

    M2ClassAssociation customAssoc = customAssocsAspect.getAssociation(
      generatedShortQName
    );
    if (customAssoc != null) {
      StringBuilder sb = new StringBuilder();
    ...
    switch (type) {
      ...
      case PARENTCHILD:
        newAssoc =
          customAssocsAspect.createChildAssociation(generatedShortQName);
        break;
      default:
        StringBuilder sb = new StringBuilder();
        sb
    ...
    return new RelationshipDefinitionImpl(
      relationshipDefinitionQName.getLocalName(),
      type,
      displayName
    );
  }
  ...
  public RelationshipDefinition updateRelationshipDefinition(
    ...
    RelationshipDefinition relationshipDefinition = getRelationshipDefinition(
      uniqueName
    );
    if (relationshipDefinition == null) {
      StringBuilder sb = new StringBuilder();
      sb
        .append("The relationship definition for the unique name '")
        .append(uniqueName)
    ...
    if (existsTitle(title)) {
      StringBuilder sb = new StringBuilder();
      sb
        .append("Cannot update the relationship definition as '")
        .append(title)
        .append("' already exists.");
      throw new AlfrescoRuntimeException(sb.toString());
    }
    ...
    QName updatedAssociationDefinitionQName = persistUpdatedAssocTitle(
      associationDefinitionQName,
      title
    );
    RelationshipDefinition updatedRelationshipDefinition = getRelationshipDefinition(
      updatedAssociationDefinitionQName.getLocalName()
    );
  ...
  public boolean removeRelationshipDefinition(String uniqueName) {
    ...
    throw new UnsupportedOperationException(
      "It is not possible to remove a relationship."
    );
  }
   ...
   */
  @Override
  public Set<Relationship> getRelationshipsFrom(
    NodeRef nodeRef,
    String nameFilter
  ) {
    ...
    List<AssociationRef> customReferencesFrom = getNodeService()
    ...
    List<ChildAssociationRef> customChildReferences = getNodeService()
      .getChildAssocs(nodeRef);
    relationships.addAll(
      generateRelationshipFromParentChildAssociationRef(
        customChildReferences,
        nameFilter
      )
    );
   ...
   */
  @Override
  public Set<Relationship> getRelationshipsTo(
    NodeRef nodeRef,
    String nameFilter
  ) {
    mandatory("nodeRef", nodeRef);
  ...
  public void addRelationship(
    ...
    if (getNodeService().hasAspect(target, ASPECT_FROZEN)) {
      StringBuilder sb = new StringBuilder();
      sb
        .append("Relationship cannot be created as the target '")
    ...
    if (associationDefinition == null) {
      StringBuilder sb = new StringBuilder();
      sb
        .append("No association definition found for '")
        .append(uniqueName)
        .append("'.");
    ...
    boolean associationAlreadyExists = associationExists(
      associationDefinition,
      source,
      target
    );
    ...
    if (associationDefinition.isChild()) {
      ...
      getNodeService()
        .createAssociation(source, target, associationDefinitionName);
    }
    ...
    if (associationDefinition.isChild()) {
      AuthenticationUtil.runAsSystem(
        new RunAsWork<Void>() {
          @Override
          public Void doWork() {
            List<ChildAssociationRef> children = getNodeService()
              .getChildAssocs(sourceNode);
            for (ChildAssociationRef chRef : children) {
              if (
                associationDefinitionName.equals(chRef.getTypeQName()) &&
      ...
      getNodeService()
        .removeAssociation(source, targetNode, associationDefinitionName);
    }
  ...
  private RelationshipDefinition createRelationshipDefinition(
    ...
    if (associationDefinition != null) {
      ...
      RelationshipDisplayName displayName = getRelationshipDisplayName(
        type,
        title
      );

      relationshipDefinition =
        new RelationshipDefinitionImpl(uniqueName, type, displayName);
    }
  ...
  private Set<Relationship> generateRelationshipFromAssociationRef(
    ...
    for (AssociationRef associationRef : associationRefs) {
      ...
      if (
        existsRelationshipDefinition(uniqueName) &&
  ...
  private Set<Relationship> generateRelationshipFromParentChildAssociationRef(
    List<ChildAssociationRef> childAssociationRefs,
    String nameFilter
  ) {
    Set<Relationship> relationships = new HashSet<Relationship>();
  ...
  private RelationshipType determineRelationshipTypeFromDisplayName(
    ...
    String errorMsg =
      "Relationship type could not be determined from the display name. It is neither biderectional nor parent/child relationship";
  ...
  private void invokeBeforeCreateReference(
    NodeRef source,
    NodeRef target,
    QName associationDefinitionName
  ) {
    // Get QNames to invoke against
    Set<QName> qnames = PoliciesUtil.getTypeAndAspectQNames(
      getNodeService(),
      source
    );
  ...
  private boolean associationExists(
    ...
    if (associationDefinition.isChild()) {
      ...
      List<AssociationRef> assocs = getNodeService()
        .getTargetAssocs(source, associationDefinitionName);
   ...
   */
  private QName persistUpdatedAssocTitle(
    QName associationDefinitionQName,
    String newTitle
  ) {
    mandatory("associationDefinitionQName", associationDefinitionQName);

    AssociationDefinition assocDefn = getDictionaryService()
      .getAssociation(associationDefinitionQName);
    if (assocDefn == null) {
      StringBuilder sb = new StringBuilder();
      sb
        .append("Cannot find the association definiton for '")
  ...
  private QName generateRelationshipDefinitionQNameFor(String uniqueName) {
    ...
    if (existingQName != null) {
      StringBuilder sb = new StringBuilder();
      sb
        .append("Cannot create qualified name for given unique name '")
        .append(uniqueName)

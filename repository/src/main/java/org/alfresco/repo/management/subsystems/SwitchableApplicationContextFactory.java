...
/**
 * A configurable proxy for a set of {@link ApplicationContextFactory} beans that allows dynamic
 * selection of one or more alternative subsystems via a <code>sourceBeanName</code> property. As
 * with other {@link PropertyBackedBean}s, can be stopped, reconfigured, started and tested.
 */
public class SwitchableApplicationContextFactory extends AbstractPropertyBackedBean
        implements ApplicationContextFactory {

    /**
     * The name of the property holding the bean name of the source {@link
    ...
    public String getCurrentSourceBeanName() {
        return getState(false)
                .getProperty(SwitchableApplicationContextFactory.SOURCE_BEAN_PROPERTY);
    }
    ...
    protected class SwitchableState implements PropertyBackedBeanState {
         ...
         *
         * @param sourceBeanName the source bean name
         */
        ...
        public void start() {
            if (this.sourceApplicationContextFactory == null) {
                this.sourceApplicationContextFactory =
                        (ApplicationContextFactory) getParent().getBean(this.sourceBeanName);

...
public class GroupsImpl implements Groups {
  ...
  private static final Set<String> LIST_GROUPS_EQUALS_QUERY_PROPERTIES = new HashSet<>(
    Arrays.asList(new String[] { PARAM_IS_ROOT })
  );

  private static final Set<String> LIST_GROUP_MEMBERS_QUERY_PROPERTIES = new HashSet<>(
    Arrays.asList(new String[] { PARAM_MEMBER_TYPE })
  );
  ...
  public Group create(Group group, Parameters parameters) {
    ...
    String authority = authorityService.createAuthority(
      AuthorityType.GROUP,
      group.getId(),
  ...
  public Group getGroup(String groupId, Parameters parameters)
    throws EntityNotFoundException {
    AuthorityInfo authorityInfo = getAuthorityInfo(groupId);

    final Set<String> rootAuthorities = getAllRootAuthorities(
      AuthorityType.GROUP
    );
  ...
  public CollectionWithPagingInfo<Group> getGroups(
    ...
    GroupsFilter groupsFilters = getGroupsFilterFromQueryParameters(
      parameters.getQuery()
    );
    ...
    List<Group> groups = createGroupsResponse(
      pagingResult.getPage(),
  ...
  private GroupsFilter getGroupsFilterFromQueryParameters(
  ...
  private String getStringFilterFromList(
  ...
  private void validateListParam(
    List<String> listParam,
    String paramName,
    int maxItems
  ) {
    ...
    listParam
      ...
      .ifPresent(displayName -> {
        throw new IllegalArgumentException(
          StringUtils.capitalize(paramName) + " cannot be empty (i.e. '')"
        );
      });
  }
  ...
  public CollectionWithPagingInfo<Group> getGroupsByPersonId(
    ...
    List<AuthorityInfo> groupAuthorities = userAuthorities
      .stream()
    ...
    PagingResults<AuthorityInfo> pagingResult = Util.wrapPagingResults(
      paging,
      groupAuthorities
    );
  ...
  private PagingResults<AuthorityInfo> getAuthoritiesInfo(
    ...
    if (isRootParam != null || displayNameFilter != null) {
      ...
      if (isRootParam != null && isRootParam) {
        ...
        List<AuthorityInfo> authorities = rootAuthorities
          .stream()
          .map(this::getAuthorityInfo)
        ...
        // service layer.
        AuthorityInfoComparator authorityComparator = new AuthorityInfoComparator(
          sortProp.getFirst(),
          sortProp.getSecond()
        );
        Collections.sort(groupList, authorityComparator);
      } else {
        PagingRequest pagingNoMaxItems = new PagingRequest(
        ...
        PagingResults<AuthorityInfo> nonPagingResult = authorityService.getAuthoritiesInfo(
          authorityType,
          zoneFilter,
          null,
          sortProp.getFirst(),
   ...
   * @return true if result should be included.
   */
  private boolean displayNamePredicate(
   ...
   * @return The authority info.
   */
  private AuthorityInfo getAuthorityInfo(
    String id,
    boolean defaultDisplayNameIfNull
  ) {
    ...
    if (
      !id.equals(PermissionService.ALL_AUTHORITIES) &&
      !authorityService.authorityExists(id)
    ) {
      throw new EntityNotFoundException(id);
    }

    String authorityDisplayName = getAuthorityDisplayName(
      id,
      defaultDisplayNameIfNull
    );

    return new AuthorityInfo(null, authorityDisplayName, id);
  }

  private String getAuthorityDisplayName(
    String id,
    boolean defaultDisplayNameIfNull
  ) {
  ...
  private Group getGroup(
    ...
    if (authorityDisplayName == null || authorityDisplayName.isEmpty()) {
      authorityDisplayName =
        authorityService.getAuthorityDisplayName(
          authorityInfo.getAuthorityName()
        );
    }
    ...
    group.setIsRoot(
      isRootAuthority(rootAuthorities, authorityInfo.getAuthorityName())
    );
    ...
    if (includeParam != null) {
      ...
      if (includeParam.contains(PARAM_INCLUDE_ZONES)) {
        Set<String> authorityZones = authorityService.getAuthorityZones(
          authorityInfo.getAuthorityName()
        );
  ...
  public CollectionWithPagingInfo<GroupMember> getGroupMembers(
    ...
    if (q != null) {
      ...
      String memberTypeStr = propertyWalker.getProperty(
        PARAM_MEMBER_TYPE,
    ...
    PagingResults<AuthorityInfo> pagingResult = getAuthoritiesInfo(
      authorityType,
      groupId,
      sortProp,
      paging
  ...
  public GroupMember createGroupMember(
    ...
    String authority = authorityService.getName(
      authorityType,
      groupMember.getId()
    );
  ...
  public void deleteGroupMembership(String groupId, String groupMemberId) {
    ...
    if (!parents.contains(groupId)) {
      throw new NotFoundException(
        groupMemberId + " is not member of " + groupId
      );
    }
  ...
  private PagingResults<AuthorityInfo> getAuthoritiesInfo(
    ...
    try {
      authorities =
        authorityService.findAuthorities(
          authorityType,
          groupId,
          true,
          null,
  ...
  private void validateGroupId(String groupId, boolean inferPrefix) {
    ...
    if (
      !PermissionService.ALL_AUTHORITIES.equals(groupId) &&
  ...
  private void validateGroupMemberId(String groupMemberId) {
    ...
    if (
      !(
        personAuthorityExists(groupMemberId) ||
        groupAuthorityExists(groupMemberId, false)
      )
    ) {
  ...
  private void validateGroup(Group group, boolean isUpdate) {
    ...
    if (!isUpdate) {
      ...
      if (group.wasSet(Group.ID)) {
        throw new InvalidArgumentException(
          "Group update does not support field: id"
        );
      }

      if (group.wasSet(Group.IS_ROOT)) {
        throw new InvalidArgumentException(
          "Group update does not support field: isRoot"
        );
      }
      ...
      if (group.wasSet(Group.ZONES)) {
        throw new InvalidArgumentException(
          "Group update does not support field: zones"
        );
      }
    }
  }
  ...
  private void validateGroupMember(GroupMember groupMember) {
    ...
    if (
      groupMember.getMemberType() == null ||
      groupMember.getMemberType().isEmpty()
    ) {
  ...
  private boolean groupAuthorityExists(
    String authorityName,
    boolean inferPrefix
  ) {
  ...
  private boolean isGroupAuthority(String authorityName) {
    ...
    return (
      AuthorityType.GROUP.equals(authorityType) ||

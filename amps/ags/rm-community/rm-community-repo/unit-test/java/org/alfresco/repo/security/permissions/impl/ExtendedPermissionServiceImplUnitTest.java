...
public class ExtendedPermissionServiceImplUnitTest extends BaseUnitTest {
  ...
  private static final Set<String> READERS = Stream
    .of(generateText(), generateText())
    .collect(Collectors.toSet());
  private static final Set<String> WRITERS = Stream
    .of(generateText(), generateText())
    .collect(Collectors.toSet());
  ...
  @InjectMocks
  @Spy
  private ExtendedPermissionServiceImpl extendedPermissionServiceImpl = new ExtendedPermissionServiceImpl() {
  ...
  @InjectMocks
  private ExtendedPermissionServiceImpl extendedPermissionServiceImplWithReaderWritersSet = new ExtendedPermissionServiceImpl() {
    protected AccessStatus hasPermissionImpl(NodeRef nodeRef, String perm) {
      return AccessStatus.UNDETERMINED;
    }
  ...
  public void preProcessorDoesNotDeny() {
    ...
    AccessStatus result = extendedPermissionServiceImpl.hasPermission(
      nodeRef,
      perm
    );
  ...
  public void preProcessorDenys() {
    ...
    verify(mockedPermissionPreProcessor).process(nodeRef, perm);
    verify(extendedPermissionServiceImpl, never())
      .hasPermissionImpl(nodeRef, perm);
  }
  ...
  public void postProcessorRegistered() {
    ...
    List<String> configuredFilePermissions = asList(
      "WriteProperties",
      "AddChildren"
    );

    extendedPermissionServiceImpl.setConfiguredReadPermissions(
      "ReadProperties,ReadChildren"
    );
    extendedPermissionServiceImpl.setConfiguredFilePermissions(
      "WriteProperties,AddChildren"
    );

    when(mockedPermissionProcessorRegistry.getPermissionPostProcessors())
      .thenReturn(asList(mockedPermissionPostProcessor));
    when(
      mockedPermissionPostProcessor.process(
  ...
  public void getReadersAndWritersForNodeWithNoOwnerAspect() {
    ...
    // when
    Pair<Set<String>, Set<String>> result = extendedPermissionServiceImplWithReaderWritersSet.getReadersAndWriters(
      nodeRef
    );

...
public class PageCollatorTest extends TestCase {
    ...
    private int[] createMergedPage(int skip, int count, List<Integer> s1, int[] s2) {
        int[] s1Primitive =
                (s1 == null || s1.isEmpty())
                        ? new int[] {}
                        : ArrayUtils.toPrimitive((Integer[]) s1.toArray(new Integer[s1.size()]));

        return createMergedPage(skip, count, s1Primitive, s2);
    }

    /**
     * Creates an page-array by concatenating the given arrays than skipping and trimming the
     * resulted array.
     *
     * @param skip
     * @param count
     * @param s1
     * @param s2
     * @return a page-array obtained by merging <code>s1</code> and <code>s2</code>, sorting the
     *     result than copying <code>count</code> elements from the concatenation starting with the
     *     element at index <code>skip</code> into the returned array of size <code>count</code>. If
     *     <code>skip</code> value is out of bounds an empty array is returned.
     */
    private int[] createMergedPage(int skip, int count, int[] s1, int[] s2) {
        final int[] m = Arrays.copyOf(s1, s1.length + s2.length);
        System.arraycopy(s2, 0, m, s1.length, s2.length);
        Arrays.sort(m);

        final int zeroCount = (count == 0 ? m.length : count);

        final int len = Math.min(m.length - skip, zeroCount);
        if (len > 0) {
            int[] p = new int[len];
    ...
    private void assertEqualPages(int[] expected, int[] actual) {
        assertEqualPages(
                Arrays.toString(expected) + " vs " + Arrays.toString(actual), expected, actual);
    }
    ...
    public void testCreateMergedPage() throws Exception {
        ...
        assertEqualPages(new int[] {2, 3, 4}, createMergedPage(2, 3, s1, s2));

        assertEqualPages(new int[] {9, 10}, createMergedPage(7, 2, s1, s2));

        assertEqualPages(new int[] {0, 1, 2, 3, 4}, createMergedPage(0, 5, s1, s2));

        assertEqualPages(new int[] {0, 1, 2, 3, 4}, createMergedPage(0, 5, s1, s2));

        assertEqualPages(new int[] {4, 7, 8, 9}, createMergedPage(4, 4, s1, s2));

        assertEqualPages(new int[] {}, createMergedPage(10, 4, s1, s2));

        assertEqualPages(new int[] {}, createMergedPage(9, 1, s1, s2));

        assertEqualPages(new int[] {}, createMergedPage(9, 1, s1, s2));

        assertEqualPages(new int[] {}, createMergedPage(11, 3, s1, s2));

        final int[] s3 = new int[] {2, 3};
        final int[] s4 = new int[] {7, 10, 13, 11};

        assertEqualPages(new int[] {13}, createMergedPage(5, 2, s3, s4));

        final int[] s5 = new int[] {1, 2, 3};
        final int[] s6 = new int[] {5};

        assertEqualPages(new int[] {5}, createMergedPage(3, 1, s5, s6));

        assertEqualPages(new int[] {1, 2, 3, 5}, createMergedPage(0, 0, s5, s6));
    }

    /**
     * A page source that uses a generic array for the entire possible page range.
     *
     * @author Bogdan Horje
     * @param <R>
     */
    class ArrayPageSource<R> implements PagingResultsSource<R> {
        ...
        public PagingResults<R> retrieve(PagingRequest pr) throws PageCollationException {
            ...
            return new ListBackedPagingResults<R>(Arrays.asList(array), pr);
        }
    }

    public void assertCollate(List<Integer> results, Integer[] source, int skip, int pageSize)
            throws Exception {
        assertCollate(results, source, skip, pageSize, false);
    }

    public void assertCollate(
            List<Integer> results, Integer[] source, int skip, int pageSize, boolean boundsError)
            throws Exception {
        Arrays.sort(source);
        Collections.sort(results);
        int[] expected = createMergedPage(skip, pageSize, results, source);
        PagingResults<Integer> actualResults =
                new PageCollator<Integer>()
                        .collate(
                                results,
                                new ArrayPageSource(source),
                                new PagingRequest(skip, pageSize),
        ...
        if (boundsError
                && !new Pair<Integer, Integer>(null, null)
                        .equals(actualResults.getTotalResultCount())) {
            assertEquals(
                    "Invalid total info",
                    new Pair<Integer, Integer>(expectedTotal, expectedTotal),
    ...
    public void testCollate() throws Exception {
        final List<Integer> s1 = Arrays.asList(2, 4, 9, 10);
        final Integer[] s2 = new Integer[] {0, 1, 3, 5, 7, 8};

        assertCollate(s1, s2, 0, 3);
        assertCollate(s1, s2, 2, 3);

        final List<Integer> s3 = Arrays.asList(2, 4, 9, 10, 12, 15, 17, 18);
        final Integer[] s4 = new Integer[] {0, 1, 3, 8, 16, 19};

        assertCollate(s3, s4, 0, 3);
        assertCollate(s3, s4, 2, 3);
        assertCollate(s3, s4, 6, 3);
        assertCollate(s3, s4, 7, 3);

        final List<Integer> s5 = Arrays.asList(2, 3, 6, 8);
        final Integer[] s6 = new Integer[] {7, 10, 13, 11, 17, 19};

        assertCollate(s5, s6, 0, 1);
        assertCollate(s5, s6, 1, 2);
    }
    ...
    public void testCollateBoundary2() throws Exception {
        ...
        assertCollate(s1, s2, 5, 2, true);
    }

    public void testCollateBoundary3() throws Exception {
        final List<Integer> s1 = Arrays.asList(1, 2, 3);
    ...
    public void testCollateBoundary13() throws Exception {
        final List<Integer> s1 = Arrays.asList(6, 7, 8, 9, 10, 11, 12, 13, 14);
        final Integer[] s2 = new Integer[] {1};

        assertCollate(s1, s2, 3, 5, false);
    }

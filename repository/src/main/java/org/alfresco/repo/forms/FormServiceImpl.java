...
public class FormServiceImpl implements FormService {
  ...
  public Form getForm(
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Retrieving form for item: " + item
    );

    FormProcessor processor =
      this.processorRegistry.getApplicableFormProcessor(item);

    if (processor == null) {
      throw new FormException(
        "Failed to find appropriate FormProcessor to generate Form for item: " +
        item
      );
    } else {
  ...
  public Object saveForm(Item item, FormData data) {
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Saving form for item '" + item + "': " + data
    );
    ...
    if (processor == null) {
      throw new FormException(
        "Failed to find appropriate FormProcessor to persist Form for item: " +
        item
      );
    } else {

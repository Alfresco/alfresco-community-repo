...
public class JscriptWorkflowInstance implements Serializable {
  ...
  public Scriptable getPaths() {
    ...
    for (WorkflowPath cmrPath : cmrPaths) {
      paths.add(
        new JscriptWorkflowPath(cmrPath, this.serviceRegistry, this.scope)
      );
    }

    Scriptable pathsScriptable = (Scriptable) new ValueConverter()
      .convertValueForScript(this.serviceRegistry, this.scope, null, paths);

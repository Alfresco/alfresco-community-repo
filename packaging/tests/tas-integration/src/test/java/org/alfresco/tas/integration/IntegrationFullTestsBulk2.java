...
public class IntegrationFullTestsBulk2 extends IntegrationTest {
    ...
    /**
     * Scenario 73 1. Create file1 using ftp 2. Open document for edit using CMIS 3. Try to edit
     * document using Webdav while checked-out with CMIS 4. Copy document to testFolder2 with ftp 5.
     * Update document from folder2, check its content is updated with Webdav 6. Update document
     * with WebDAV - should fail since document is checked out
     *
     * @throws Exception
     */
    ...
    public void updateFileUsingDifferentProtocolsWhileDocumentIsCheckedOut() throws Exception {
        ...
        STEP(
                "3. Try to edit document using Webdav while checked-out with CMIS - content should"
                        + " be updated");
        webDavProtocol
                .authenticateUser(testUser1)
                .usingResource(wordFilePWC)
                .update("update")
                .and()
                .assertThat()
                .contentIs("update");
    ...
    /**
     * Scenario 84 1. Create folder1 with webdav 2. Upload new file, file1 with ftp 3. Set content
     * for file1 with cmis 4. CheckOut file1 with cmis 5. Try to edit file while checked-out with
     * ftp 6. Cancel checkout with cmis 7. Try again to edit file with ftp 8. Edit file content
     * using webDav 9. Get content using cmis, validate it
     */
    @Test(groups = {TestGroup.INTEGRATION, TestGroup.FULL})
    @TestRail(
            section = {TestGroup.INTEGRATION, TestGroup.CONTENT},
    ...
    public void updateCheckedOutFile() throws Exception {
        ...
        cmisAPI.authenticateUser(testUser1)
                ...
                .and()
                .assertThat()
                .contentIs("testContentcmisUpdate");

        STEP("4. CheckOut file1 with cmis");
        cmisAPI.usingResource(testFile).checkOut();

        STEP(
                "5. Try to edit file while checked-out with ftp - content should not be updated"
                        + " since file is checked out");
        ftpProtocol
                .usingResource(testFile)
                .update("ftpUpdate")
                .and()
    ...
    /**
     * 1. Create folder1 with cmis 2. Add new file with webdav 3. Open file with Manager, add some
     * content, save with webdav 4. Open file with Consumer, add some content, save with ftp 5. Open
     * file with Contributor, delete some content, save with webdav 6. Open file with Collaborator,
     * add some content, save with WebDAV 7. Open file with Consumer, delete some content, save with
     * webdav 8. Open file with Manager, add new content, save with cmis 9. Open file with
     * Collaborator, delete content that Manager added with ftp 10. Open file with Manager, add new
     * content with webdav 11. Delete all content with admin using FTP 12. Delete file with cmis 13.
     * Check file is deleted with WebDAV
     */
    @Test(groups = {TestGroup.INTEGRATION, TestGroup.FULL})
    @TestRail(
            section = {TestGroup.INTEGRATION, TestGroup.CONTENT},
    ...
    public void updateFileWithDifferentRolesUsingDifferentProtocols() throws Exception {
        ...
        cmisAPI.authenticateUser(usersWithRoles.getOneUserWithRole(UserRole.SiteManager))
                ...
                .and()
                .assertThat()
                .contentIs("webdavUpdate2cmisUpdate");
    ...
    /**
     * 1. Add file1 to document library with ftp 2. Update file content with WebDAV 3. Add a minor
     * change to a document with cmis 4. Add a change with webdav, check document version with cmis
     * 5. Add a major change to a document with cmis 6. Delete document last version with cmis 7.
     * Try to edit previous version with cmis 8. Update content with ftp, check version 9. Check
     * document content with WebDAV
     */
    @Test(groups = {TestGroup.INTEGRATION, TestGroup.FULL})
    @TestRail(
            section = {TestGroup.INTEGRATION, TestGroup.CONTENT},
    ...
    public void documentVersioningTest() throws Exception {
        ...
        cmisAPI.authenticateUser(testUser1)
                ...
                .and()
                .assertThat()
                .contentIs("WebDAVUpdatecmisUpdate");
        ...
        cmisAPI.usingResource(testFile)
    ...
    /**
     * Scenario 82 1. Create user1, user2 2. User1 creates site1 and invites user2 as manager 3.
     * User1 adds document1 and tag1 to doc 4. User2 gets tags and verifies tag1 appears 5. User2
     * delete tag1 6. User1 tries to update tag1 7. User1 add new tag tag2 8. User2 verifies tag2
     * appears and tag1 is not in the list 9. User2 deletes document1 10. User1 tries to delete tag2
     */
    @Test(groups = {TestGroup.INTEGRATION, TestGroup.FULL})
    @TestRail(
            section = {TestGroup.INTEGRATION, TestGroup.CONTENT, TestGroup.TAGS},
            executionType = ExecutionType.REGRESSION,
            description = "Check negative scenarios for tags")
    public void tagsNegativeScenariosTest() throws Exception {
        ...
        restAPI.authenticateUser(user2)
                .withCoreAPI()
                .usingResource(testFile)
                .getNodeTags()
                .assertThat()
        ...
        restAPI.withCoreAPI()
                ...
                .getNodeTags()
                .assertThat()
                .entriesListDoesNotContain("tag", "tag1");
    ...
    /**
     * Scenario 83 1. Create user1, user2 2. User1 creates site1 and invites user2 as manager with
     * rest api 3. User1 adds document1 with webdav 4. User2 reads the document1 with ftp 5. User1
     * update document1 with WebDAV 6. User2 update the document1 with WebDAV 7. User1 deletes the
     * document1 with ftp 8. Verify user2 cannot update document1 with cmis
     */
    @Test(
    ...
    @TestRail(
            section = {TestGroup.INTEGRATION, TestGroup.CONTENT},
            executionType = ExecutionType.REGRESSION,
            description = "Negative scenarios for update document")
    public void fileUpdateNegativeScenariosTest() throws Exception {
        ...
        ftpProtocol
                ...
                .usingResource(testFile)
                .assertThat()
                .contentIs("testContent");

        STEP("5. User1 update document1 with WebDAV");
        webDavProtocol
                .authenticateUser(user1)
                .usingResource(testFile)
                .update("WebDAVUpdate1")
                .assertThat()
                .contentIs("WebDAVUpdate1");
    ...
    /**
     * Scenario 85 1. Upload file with CMIS 2. Add some content with ftp 3. Add a minor change to
     * document, 1.2 with cmis 4. Add a major change to document, 2.0 with cmis 5. Add a minor
     * change to document, 2.1 with cmis 6. Delete version 2.0 with cmis with cmis 7. Get document
     * version with cmis 8. Try to edit version 2.0 with cmis 9. Delete version 2.1 with cmis 10.
     * Get document version with cmis 11. Try to edit version 2.0 with cmis 12. Check document
     * content with ftp 13. Check document version with cmis
     */
    @Test(groups = {TestGroup.INTEGRATION, TestGroup.FULL})
    @TestRail(
            section = {TestGroup.INTEGRATION, TestGroup.CONTENT},
    ...
    public void fileVersioningScenariosTest() throws Exception {
        ...
        cmisAPI.usingResource(testFile)
                ...
                .and()
                .assertThat()
                .contentIs("cmisUpdatecmisUpdate")
                .and()
                .assertThat()
                .documentHasVersion(2.1);
    ...
    /**
     * 1. Upload file with cmis 2. Add some content with WebDAV 3. Add a minor change to document,
     * 1.2 with cmis 4. Add a major change to document, 2.0 with cmis 5. Add a minor change to
     * document, 2.1 with cmis 6. Delete version 2.1 with cmis 7. Try to edit the last version
     * (which is now 2.0) with webdav 8. Check version with CMIS 9. Add new major change, version
     * 3.0 with cmis 10. Delete previous version with cmis 11. Delete document with ftp 12. Check
     * document version with cmis
     */
    @Test(
            groups = {TestGroup.INTEGRATION, TestGroup.FULL},
            expectedExceptionsMessageRegExp = ".*Version Series not found.*",
            expectedExceptions = CmisObjectNotFoundException.class)
    @TestRail(
            section = {TestGroup.INTEGRATION, TestGroup.CONTENT},
            executionType = ExecutionType.REGRESSION,

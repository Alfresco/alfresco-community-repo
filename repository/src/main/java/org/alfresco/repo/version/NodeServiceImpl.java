...
public class NodeServiceImpl implements NodeService, VersionModel {
    ...
    private static final String MSG_UNSUPPORTED_V1 =
            "Versioning V1 is not implemented or supported. Patches exist to upgrade your data to"
                    + " use Versioning V2. Please contact support.";

    /** The name of the spoofed root association */
    private static final QName rootAssocName =
            QName.createQName(VersionModel.NAMESPACE_URI, "versionedState");
    ...
    public QName getType(NodeRef nodeRef) throws InvalidNodeRefException {
        return (QName)
                this.dbNodeService.getProperty(
                        VersionUtil.convertNodeRef(nodeRef), PROP_QNAME_FROZEN_NODE_TYPE);
    }

    /**
     * @see
     *     org.alfresco.service.cmr.repository.NodeService#setType(org.alfresco.service.cmr.repository.NodeRef,
    ...
    public void addAspect(
            NodeRef nodeRef, QName aspectRef, Map<QName, Serializable> aspectProperties)
    ...
    public Set<QName> getAspects(NodeRef nodeRef) throws InvalidNodeRefException {
        return new HashSet<QName>(
                (ArrayList<QName>)
                        this.dbNodeService.getProperty(
                                VersionUtil.convertNodeRef(nodeRef), PROP_QNAME_FROZEN_ASPECTS));
    }
    ...
    public Map<QName, Serializable> getProperties(NodeRef nodeRef) throws InvalidNodeRefException {
        ...
        for (ChildAssociationRef child : children) {
            ...
            QName qName =
                    (QName) this.dbNodeService.getProperty(versionedAttribute, PROP_QNAME_QNAME);
            PropertyDefinition propDef = this.dicitionaryService.getProperty(qName);

            Boolean isMultiValue =
                    (Boolean)
                            this.dbNodeService.getProperty(
                                    versionedAttribute, PROP_QNAME_IS_MULTI_VALUE);
                        ...
                        if (dataTypeDef.getName().equals(DataTypeDefinition.CONTENT)
                            ...
                            try {
                                ...
                                logger.warn(
                                        "ContentData with ID "
                                                + value
                                                + " no longer exists for versioned node "
                                                + nodeRef);
                            }
                        } else {
                            value =
    ...
    /** @throws UnsupportedOperationException always */
    public void setProperty(NodeRef nodeRef, QName qame, Serializable value)
            throws InvalidNodeRefException {
        // This operation is not supported for a version store
        throw new UnsupportedOperationException(MSG_UNSUPPORTED);
    }
        ...
        if (qnamePattern.isMatch(rootAssocName) == true) {
            result.add(
                    new ChildAssociationRef(
                            ...
                            dbNodeService.getRootNode(
                                    new StoreRef(StoreRef.PROTOCOL_WORKSPACE, STORE_ID)),
                            rootAssocName,
                            nodeRef));
        }
        return result;
    ...
    public List<ChildAssociationRef> getChildAssocs(
        ...
        List<ChildAssociationRef> result =
                new ArrayList<ChildAssociationRef>(childAssocRefs.size());
        for (ChildAssociationRef childAssocRef : childAssocRefs) {
            // Get the child reference
            NodeRef childRef = childAssocRef.getChildRef();
            NodeRef referencedNode =
                    (NodeRef) this.dbNodeService.getProperty(childRef, ContentModel.PROP_REFERENCE);

            if (this.dbNodeService.exists(referencedNode) == true) {
                // get the qualified name of the frozen child association and filter out unwanted
                // names
                QName qName =
                        (QName) this.dbNodeService.getProperty(childRef, PROP_QNAME_ASSOC_QNAME);
                ...
                if (qnamePattern.isMatch(qName) == true) {
                    ...
                    QName assocType =
                            (QName)
                                    this.dbNodeService.getProperty(
                                            childRef, PROP_QNAME_ASSOC_TYPE_QNAME);
                    boolean isPrimary =
                            ((Boolean)
                                            this.dbNodeService.getProperty(
                                                    childRef, PROP_QNAME_IS_PRIMARY))
                                    .booleanValue();
                    int nthSibling =
                            ((Integer)
                                            this.dbNodeService.getProperty(
                                                    childRef, PROP_QNAME_NTH_SIBLING))
                                    .intValue();
    ...
    public List<AssociationRef> getTargetAssocs(NodeRef sourceRef, QNamePattern qnamePattern) {
        ...
        for (ChildAssociationRef childAssocRef : childAssocRefs) {
            ...
            if (this.dbNodeService.exists(referencedNode) == true) {
                // get the qualified type name of the frozen child association and filter out
                // unwanted names
                QName qName =
                        (QName)
                                this.dbNodeService.getProperty(
                ...
                if (qnamePattern.isMatch(qName) == true) {
                    AssociationRef newAssocRef =
                            new AssociationRef(null, sourceRef, qName, referencedNode);

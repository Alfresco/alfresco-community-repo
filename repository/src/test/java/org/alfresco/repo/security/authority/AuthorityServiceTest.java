...
public class AuthorityServiceTest extends TestCase
{
    ...
    public void testGetAuthorityNodeRef()
    {
        ...
        {
            if (assoc.getChildRef().equals(newNode))
                found = true;
        }
        assertTrue(
    ...
    /**
     * MNT-25474: Verify that getAuthorityDisplayName returns firstName + lastName
     * for person authorities instead of the user ID.
     */
    ...
    /**
     * The test is not actual as we cannot create a user with a tenant domain in the name, if the tenant was not precreated
     */
    public void testMNT_11766()
    {
        /* Set<String> admins = authenticationComponent.getDefaultAdministratorUserNames();
         * 
         * for (String admin : admins) { // create user with MT format name (i.e. username@domain) String user = admin + "@" + System.currentTimeMillis();
         * 
         * Map<QName, Serializable> props = new HashMap<QName, Serializable>(4, 1.0f); props.put(ContentModel.PROP_USERNAME, user); props.put(ContentModel.PROP_FIRSTNAME, user); props.put(ContentModel.PROP_LASTNAME, user); props.put(ContentModel.PROP_EMAIL, user + "@gmail.com");
         * 
         * personService.createPerson(props); authenticationService.createAuthentication(user, "123123".toCharArray());
         * 
         * authenticationComponent.setCurrentUser(user); assertFalse("User should not have administrator role.", authorityService.hasAdminAuthority()); assertFalse("User should not have administrator role.", pubAuthorityService.hasAdminAuthority()); } */
    }

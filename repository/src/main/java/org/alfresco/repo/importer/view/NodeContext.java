...
public class NodeContext extends ElementContext implements ImportNode {
  ...
  public NodeContext(
    ...
    super(
      elementName,
      parentContext.getDictionaryService(),
      parentContext.getImporter()
    );
  ...
  public void addPropertyCollection(QName property) {
    ...
    ClassDefinition classDef = (propDef == null)
      ? null
      : propDef.getContainerClass();
    if (classDef != null) {
  ...
  public void addProperty(QName property, Serializable value) {
    ...
    if (
      uuid == null &&
      propDef != null &&
      propDef.getName().equals(ContentModel.PROP_NODE_UUID)
    ) {
      uuid = value.toString();
    }
  ...
  public DataTypeDefinition getPropertyDataType(QName propertyName) {
    ...
    if (valueDataType == null) {
      PropertyDefinition propDef = getDictionaryService()
  ...
  public PropertyDefinition determineProperty(QName defName) {
    ...
    if (nodeProperties.containsKey(defName) == false) {
      def =
        (typeDef == null)
          ? null
          : getDictionaryService().getProperty(typeDef.getName(), defName);
      if (def == null) {
        ...
        for (AspectDefinition aspectDef : allAspects) {
          def =
            getDictionaryService().getProperty(aspectDef.getName(), defName);
          if (def != null) {
            break;
          }
        }
  ...
  public String toString() {
    return (
      "NodeContext[childName=" +
      getChildName() +
      ",type=" +
      (typeDef == null ? "null" : typeDef.getName()) +
      ",nodeRef=" +
      nodeRef +
      ",aspects=" +
      nodeAspects.values() +

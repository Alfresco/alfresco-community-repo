...
public class RejectRecordTests extends BaseRMRestTest {
    ...
    public void setUp() {
        ...
        linkRecordFolder =
            createFolder(
                ...
                getRandomName("linkRecordFolder")
            );
    }
    ...
    public void rejectLinkedRecord() {
        ...
        Record record = getRestAPIFactory()
            .getFilesAPI()
            .usingParams(
                String.format("%s=%s", PARENT_ID_PARAM, recordFolder.getId())
            )
            .declareAsRecord(testFile.getNodeRefWithoutVersion());
        assertStatusCode(CREATED);

        STEP("Link record to new folder");
        RecordBodyFile linkRecordBody = RecordBodyFile
            .builder()
            .targetParentId(linkRecordFolder.getId())
            .build();
        getRestAPIFactory()
            .getRecordsAPI()
            .fileRecord(linkRecordBody, record.getId());
        ...
        assertFalse(
            isMatchingRecordInRecordFolder(testFile, recordFolder),
    ...
    public void rejectCompletedRecord() {
        ...
        RecordCategoryChild folderWithRule = createFolder(
            recordCategory.getId(),
        ...
        RuleDefinition ruleDefinition = RuleDefinition
            ...
            .actions(
                Collections.singletonList(ActionsOnRule.REJECT.getActionValue())
            );
        rulesAPI.createRule(
            getAdminUser().getUsername(),
            getAdminUser().getPassword(),
        ...
        RecordBodyFile recordBodyFile = RecordBodyFile
            .builder()
            .targetParentId(folderWithRule.getId())
            .build();
        getRestAPIFactory()
            .getRecordsAPI()
            .fileRecord(recordBodyFile, record.getId());
        assertStatusCode(CREATED);

...
/**
 * Set of tests that ensure Slingshot GET APIs are run successfully in a read-only transaction
 * (ALF-10179).
 *
 * <p>Some webscripts have a side effect of creating a "container" these tests are to ensure this is
 * handled gracefully in a way that allows the main transaction to be declared as readonly for
 * performance reasons.
 *
...
public class ReadOnlyTransactionInGetSlingshotApiTest extends BaseWebScriptTest {
    ...
    private static final String URL_GET_DOCLIB_DOCLIST =
            "/slingshot/doclib/doclist/documents/site/" + TEST_SITE_NAME + "/documentLibrary";
    private static final String URL_GET_DOCLIB2_DOCLIST =
            "/slingshot/doclib2/doclist/documents/site/" + TEST_SITE_NAME + "/documentLibrary";
                ...
                sendRequest(
                        new GetRequest(
                                MessageFormat.format(
                                        URL_GET_DOCLIB_CATEGORYNODE, testSiteNodeRefString)),
                        200);
        logResponse(response);
        assertEquals(Status.STATUS_OK, response.getStatus());
    }
    ...
    public void testGetDoclibNodeLocation() throws Exception {
        Response response =
                sendRequest(
                        new GetRequest(
                                MessageFormat.format(
                                        URL_GET_DOCLIB_LOCATION, testSiteNodeRefString)),
                        200);
        logResponse(response);
        assertEquals(Status.STATUS_OK, response.getStatus());
    }

...
/**
 * Integration tests that check transaction related functionality of {@link LockStore}
 * implementations.
 *
 * @author Matt Ward
 */
public abstract class AbstractLockStoreTxTest<T extends LockStore> {
    ...
    /**
     * Concrete subclasses must implement this method to provide the tests with a LockStore
     * instance.
     *
     ...
     * <ul>
     *   <li>Start outer txn
     *   <li>Modify lock in outer txn
     *   <li>Start inner txn
     *   <li>Modify lock in inner txn
     * </ul>
     *
    ...
    public void testRepeatableReadsInTransaction() throws NotSupportedException, SystemException {
        ...
        final LockState lockState1 =
                LockState.createLock(
                        nodeRef, LockType.WRITE_LOCK, "jbloggs", expires, Lifetime.EPHEMERAL, null);
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            try {
                                ...
                                // Wait, while txA checks whether it can see lock for nodeRef2
        ...
        try {
            ...
            AuthenticationUtil.setFullyAuthenticatedUser(
                    "jbloggs"); // Current lock owner needed to change lock.
            ...
            // Another update
            AuthenticationUtil.setFullyAuthenticatedUser(
                    "another"); // Current lock owner needed to change lock.
    ...
    @Test
    public void testCannotSetLockWhenChangedByAnotherTx()
                        ...
                        try {
                            ...
                            try {
                                ...
                                try {
                                    ...
                                    // when it has been modified by another tx since this tx last
                                    // inspected it.
                                    AuthenticationUtil.setFullyAuthenticatedUser(
                                            "jbloggs"); // Current lock owner
                                    lockStore.set(
                                            nodeRef,
                                            LockState.createLock(
                                                    nodeRef,
    ...
    @Test
    public void testCanChangeLockIfLatestValueIsHeldEvenIfAlreadyChangedByAnotherTx()
            throws NotSupportedException, SystemException {
        final TransactionService txService = (TransactionService) ctx.getBean("TransactionService");
        UserTransaction txA = txService.getUserTransaction();
        final NodeRef nodeRef = new NodeRef("workspace://SpacesStore/UUID-1");
        ...
        final LockState lockState1 =
                LockState.createLock(
                        nodeRef, LockType.WRITE_LOCK, "jbloggs", expired, Lifetime.EPHEMERAL, null);
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            try {
                                ...
                                final LockState newUserLockState =
                                        LockState.createLock(
                                                nodeRef,
                                                LockType.WRITE_LOCK,
                                                "new-user",
    ...
    @Test
    public void testNotOnlyCurrentLockOwnerCanChangeInfo()
            throws NotSupportedException, SystemException {
        final TransactionService txService = (TransactionService) ctx.getBean("TransactionService");
        UserTransaction txA = txService.getUserTransaction();
        final NodeRef nodeRef = new NodeRef("workspace://SpacesStore/UUID-1");
    ...
    @Test
    public void testOtherUserCanChangeLockInfoOnceExpired()
            throws NotSupportedException, SystemException {
        final TransactionService txService = (TransactionService) ctx.getBean("TransactionService");
        UserTransaction txA = txService.getUserTransaction();
        final NodeRef nodeRef = new NodeRef("workspace://SpacesStore/UUID-1");

...
public abstract class AbstractEmailMessageHandler
  ...
  private static Log logger = LogFactory.getLog(
    AbstractEmailMessageHandler.class
  );
      ...
      if (childNodeRef != null) {
        ...
        if (
          ...
          workingName =
            baseName.substring(
              0,
              QName.MAX_LENGTH - postFix.length() - extension.length() - 1
            ) +
            postFix;
        } else {
  ...
  protected NodeRef addAttachment(
    ...
    NodeRef attachmentNode = addContentNode(
      nodeService,
      folder,
      fileName,
      false
    ...
    nodeService.addAspect(
      mainContentNode,
      ContentModel.ASPECT_ATTACHABLE,
      null
    );
    // Add the association
   ...
   */
  private String getAppropriateNodeName(
    NodeRef parent,
    String name,
    QName assocType
  ) {
  ...
  protected void addAttachments(
    ...
    for (EmailMessagePart attachment : attachments) {
      ...
      NodeRef attachmentNode = addAttachment(
        getNodeService(),
        spaceNodeRef,
        nodeRef,
        fileName
  ...
  static final String[][] dubiousChars = new String[][] {
    { "\\", "%5c" },
    { "/", "%2f" },
    { "*", "%2a" },
    { "|", "%7c" },

 ...
 *
 * <p>Manage creation and deletion of ACL entries for the new ADM ACL implementation
 *
 * @author andyh
 */
public class ADMPermissionsDaoComponentImpl extends AbstractPermissionsDaoComponentImpl {
            ...
            case SHARED:
                ...
                changes.add(
                        new AclDAOImpl.AclChangeImpl(
                                existing.getId(), id, existing.getAclType(), acl.getAclType()));
                changes.addAll(
                        ...
                        if (inheritsFrom != null) {
                            ...
                            getACLDAO(nodeRef)
                                    .setAccessControlList(
                                            nodeRef, aclDaoComponent.getAcl(inheritsFrom));
                            List<AclChange> changes = new ArrayList<AclChange>();
                            changes.addAll(
                                    getACLDAO(nodeRef)
                                            .setInheritanceForChildren(
    ...
    private void replaceWithCleanDefiningAcl(NodeRef nodeRef, Acl acl) {
        ...
        List<AclChange> changes = new ArrayList<AclChange>();
        changes.addAll(
                getACLDAO(nodeRef).setInheritanceForChildren(nodeRef, id, acl.getInheritedAcl()));
        getACLDAO(nodeRef).updateChangedAcls(nodeRef, changes);
        aclDaoComponent.deleteAccessControlList(acl.getId());
    }
}

...
/*package*/class M2AssociationDefinition implements AssociationDefinition {
  ...
  /*package*/M2AssociationDefinition(
    ...
    this.targetClassName =
      QName.createQName(assoc.getTargetClassName(), resolver);
    this.sourceRoleName =
      QName.createQName(assoc.getSourceRoleName(), resolver);
    this.targetRoleName =
      QName.createQName(assoc.getTargetRoleName(), resolver);
  }
  ...
  /*package*/void resolveDependencies(ModelQuery query) {
    if (targetClassName == null) {
      throw new DictionaryException(
        "d_dictionary.association.target_class_not_specified",
        name.toPrefixString()
      );
    }
  ...
  /* package */M2ModelDiff diffAssoc(AssociationDefinition assocDef) {
    ...
    if (
      !EqualsHelper.nullSafeEquals(
        ...
        assocDef.getSourceClass().getName()
      )
    ) {
      isUpdated = true;
    }
    ...
    if (
      !EqualsHelper.nullSafeEquals(
        ...
        assocDef.getTargetClass().getName()
      )
    ) {
      isUpdated = true;
    }
    ...
    for (AssociationDefinition previousAssoc : previousAssocs) {
      ...
      for (AssociationDefinition newAssoc : newAssocs) {
        if (newAssoc.getName().equals(previousAssoc.getName())) {
          modelDiffs.add(
            ((M2AssociationDefinition) previousAssoc).diffAssoc(newAssoc)
          );
          found = true;
          break;
        }

...
public class ImporterComponentTest extends BaseSpringTest {
  ...
  public void before() throws Exception {
    ...
    importerService =
      (ImporterService) applicationContext.getBean(
        ServiceRegistry.IMPORTER_SERVICE.getLocalName()
      );

    importerBootstrap =
      (ImporterBootstrap) applicationContext.getBean("spacesBootstrap");
  ...
  public void testImportWithAuditableProperties() throws Exception {
    ...
    String createdDate = DefaultTypeConverter.INSTANCE.convert(
      String.class,
      nodeProps.get(ContentModel.PROP_CREATED)
    );
    String creator = DefaultTypeConverter.INSTANCE.convert(
      String.class,
      nodeProps.get(ContentModel.PROP_CREATOR)
    );
    String modifiedDate = DefaultTypeConverter.INSTANCE.convert(
      String.class,
      nodeProps.get(ContentModel.PROP_MODIFIED)
    );
    String modifier = DefaultTypeConverter.INSTANCE.convert(
      String.class,
      nodeProps.get(ContentModel.PROP_MODIFIER)
    );
    // Check that the cm:auditable properties are correct
    assertEquals(
      "cm:created not preserved during import",
      ISO8601DateFormat.format(
        ISO8601DateFormat.parse("2009-04-30T23:00:00.000Z")
      ),
      createdDate
    );
    assertEquals(
      "cm:creator not preserved during import",
      "Import Creator",
      creator
    );
    assertEquals(
      "cm:modified not preserved during import",
      ISO8601DateFormat.format(
        ISO8601DateFormat.parse("2009-05-01T23:00:00.000Z")
      ),
      modifiedDate
    );
    assertEquals(
      "cm:modifier not preserved during import",
      "Import Modifier",
      modifier
    );
  ...
  public void testImportWithVersioning() throws Exception {
    ...
    assertEquals(
      "1.15",
      nodeService.getProperty(versionedNode, ContentModel.PROP_VERSION_LABEL)
    );
  ...
  public void testImportWithUuidBinding() throws Exception {
    ...
    try {
      ...
      fail(
        "Failed to detected collision of UUID on import with THROW_ON_COLLISION"
      );
    } catch (Throwable e) {
      // Expected
    } finally {
  ...
  public void testBootstrap() {
    StoreRef bootstrapStoreRef = new StoreRef(
      StoreRef.PROTOCOL_WORKSPACE,
      "Test_" + System.currentTimeMillis()
    );
    ...
    System.out.println(
      NodeStoreInspector.dumpNodeStore(nodeService, bootstrapStoreRef)
    );
  }

  @Test
  public void testImportFoldersUuidBindingNullUuidNullLocationPath()
    throws Exception {
    Location location = new Location(storeRef);

    InputStream test1 = getClass()
      .getClassLoader()
      .getResourceAsStream("org/alfresco/repo/importer/import_folders.xml");
    InputStreamReader testReader1 = new InputStreamReader(test1, "UTF-8");
    try {
      importerService.importView(
        testReader1,

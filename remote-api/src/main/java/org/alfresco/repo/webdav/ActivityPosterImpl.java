...
public class ActivityPosterImpl implements WebDAVActivityPoster {
    ...
    public void postFileFolderAdded(
            String siteId, String tenantDomain, String path, FileInfo nodeInfo)
            throws WebDAVServerException {
        postFileFolderActivity(
                nodeInfo.isFolder() ? ActivityType.FOLDER_ADDED : ActivityType.FILE_ADDED,
                siteId,
                tenantDomain,
                nodeInfo.isFolder() ? path : null,
                null,
                nodeInfo);
    }
    ...
    /** {@inheritDoc} */
    @Override
    public void postFileFolderUpdated(String siteId, String tenantDomain, FileInfo nodeInfo)
            throws WebDAVServerException {
        if (!nodeInfo.isFolder()) {
            postFileFolderActivity(
                    ActivityType.FILE_UPDATED, siteId, tenantDomain, null, null, nodeInfo);
        }
    }
    ...
    public void postFileFolderDeleted(
            ...
            throws WebDAVServerException {
        postFileFolderActivity(
                nodeInfo.isFolder() ? ActivityType.FOLDER_DELETED : ActivityType.FILE_DELETED,
                siteId,
                tenantDomain,
                parentPath,
                parentNodeInfo.getNodeRef(),
                nodeInfo);
    }

...
public class ScriptUtils extends BaseScopableProcessorExtension {
    ...
    public String displayPath(ScriptNode node) {
        return this.unprotNodeService
                .getPath(node.nodeRef)
                .toDisplayPath(this.unprotNodeService, new PermissionServiceNOOPImpl());
    }
    ...
    public ScriptNode getNodeFromString(String nodeRefString) {
        ...
        return (ScriptNode)
                new ValueConverter()
                        .convertValueForScript(this.services, getScope(), null, nodeRef);
    }

    /**
     * Use the Node Locator Service to find the a node reference from a number of possible locator
     * types. This method is responsible for determining the locator type and then calling the
     * Service as the Service does not know how to guess which locator to use.
     *
     * <p>This service supports 'virtual' nodes including the following:
     *
     * <p>alfresco://company/home The Company Home root node<br>
     ...
     *
     * @param reference The node reference - See above for list of possible node references
     *     supported.
    ...
    /**
     * Given a long-form QName string, this method uses the namespace service to create a short-form
     * QName string.
     *
    ...
    /**
     * Builds a paging object, from the supplied Args object. Requires that the parameters have
     * their standard names, i.e. "maxItems" and "skipCount"
     *
     * @param args Mandatory hash of paging arguments
     *     <p>Possible arguments include:
     *     <p>maxItems - max count of items to return, default -1 (all)<br>

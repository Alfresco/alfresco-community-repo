 ...
 *
 * <p>This component provides the ability to:
 *
 * <p>
 *
 * <ul>
 *   <li>a) Register policies
 *   <li>b) Bind behaviours to policies
 *   <li>c) Invoke policy behaviours
 * </ul>
 *
 * <p>A behaviour may be bound to a Policy before the Policy is registered. In this case, the
 * behaviour is not validated (i.e. checked to determine if it supports the policy interface) until
 * the Policy is registered. Otherwise, the behaviour is validated at bind-time.
 *
 * <p>Policies may be selectively "turned off" by the Behaviour Filter.
 *
...
public interface PolicyComponent {
     ...
     */
    public <P extends PropertyPolicy> PropertyPolicyDelegate<P> registerPropertyPolicy(
            Class<P> policy);
     ...
     */
    public <P extends AssociationPolicy> AssociationPolicyDelegate<P> registerAssociationPolicy(
            Class<P> policy);
     ...
     *
     * <p>So when the named policy, happens on the specified aspect or type, the specified behaviour
     * is executed.
     *
     * <p>Example of calling this method
     *
     * <pre>
     ...
     */
    public BehaviourDefinition<ClassBehaviourBinding> bindClassBehaviour(
            QName policy, QName className, Behaviour behaviour);
     ...
     * @return the registered behaviour definition
     */
    public BehaviourDefinition<ServiceBehaviourBinding> bindClassBehaviour(
            QName policy, Object service, Behaviour behaviour);
     ...
     * @return the registered behaviour definition
     */
    public BehaviourDefinition<ClassFeatureBehaviourBinding> bindPropertyBehaviour(
            QName policy, QName className, QName propertyName, Behaviour behaviour);
     ...
     * @return the registered behaviour definition
     */
    public BehaviourDefinition<ServiceBehaviourBinding> bindPropertyBehaviour(
            QName policy, Object service, Behaviour behaviour);

    /**
     * Bind an Association specific behaviour to an Association-level Policy
     *
     * <p>For example, before a rule folder association is created.
     *
     * <pre>
    ...
    public BehaviourDefinition<ClassFeatureBehaviourBinding> bindAssociationBehaviour(
            QName policy, QName className, QName assocName, Behaviour behaviour);

    /**
     * Bind an Association specific behaviour to an Association-level Policy (for all associations
     * of a Class)
     *
     * @param policy the policy name
     * @param className the name of the class (type or aspect) to bind against
     ...
     * @return the registered behaviour definition
     */
    public BehaviourDefinition<ServiceBehaviourBinding> bindAssociationBehaviour(
            QName policy, Object service, Behaviour behaviour);

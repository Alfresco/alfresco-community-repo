...
public abstract class AbstractJettyComponent implements JettyComponent {
  ...
  protected WebApplicationContext createWebApplicationContext(
    ...
    GenericWebApplicationContext wac = (GenericWebApplicationContext) BeanUtils.instantiateClass(
  ...
  public ConfigurableApplicationContext getApplicationContext() {
    return (ConfigurableApplicationContext) webAppContext
      .getServletContext()
      .getAttribute(
  ...
  public void start() {
    if (logger.isDebugEnabled()) {
      logger.debug(
        "[" + new Date() + "] startJetty: starting embedded Jetty server ..."
      );
    }
  ...
  protected void configure(WebAppContext webAppContext) {
    try {
      ClassLoader classLoader = BaseApplicationContextHelper.buildClassLoader(
        classLocations
      );
      ...
      try {
        socket =
          new ServerSocket(
            JETTY_STOP_PORT,
            1,
            InetAddress.getByName(JETTY_LOCAL_IP)
          );
      } catch (Exception e) {

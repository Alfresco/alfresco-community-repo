 ...
 *
 * <p>Amongst other things, this service must enforce the unique name check as mandated by the
 * <b>duplicate</b> entity in the model.
 *
 * <pre>
 *    &lt;type name="cm:folder"&gt;
 *       ...
 ...
 * </pre>
 *
 * When duplicates are not allowed, and the <b>cm:name</b> property of a node changes, then the
 * {@link org.alfresco.service.cmr.repository.DuplicateChildNodeNameException} exception must be
 * thrown. Client code can catch this exception and deal with it appropriately.
 *
 * @author Derek Hulley
 */
@AlfrescoPublicApi
public interface NodeService {
    ...
    /**
     * Create a new store for the given protocol and identifier. The implementation may create the
     * store in any number of locations, including a database or Subversion.
     *
    ...
    /**
     * Gets the ID of the last transaction that caused the node to change. This includes deletions,
     * so it is possible that the node being referenced no longer exists. If the node never existed,
     ...
     * @param assocTypeQName the type of the association to create. This is used for verification
     ...
     *
     * <p>This involves changing the node's primary parent and possibly the name of the association
     * referencing it.
     *
     * <p>If the new parent is in a different store from the original, then the entire node
     * hierarchy is moved to the new store. Inter-store associations are not affected.
     *
     ...
     * @throws org.alfresco.service.cmr.repository.CyclicChildRelationshipException if the child
     *     partakes in a cyclic relationship after the add
    ...
    /**
     * Set the ordering index of the child association. This affects the ordering of of the return
     * values of methods that return a set of children or child associations.
     *
     ...
     *
     * <p>This should be used with caution since calling it changes the type of the node and thus
    ...
    /**
     * Applies an aspect to the given node. After this method has been called, the node with have
     * all the aspect-related properties present
     *
     * @param nodeRef NodeRef
     * @param aspectTypeQName the aspect to apply to the node
     * @param aspectProperties a minimum of the mandatory properties required for the aspect
     * @throws InvalidNodeRefException
     ...
     * @throws InvalidNodeRefException if the node could not be found
     * @throws InvalidAspectException if the the aspect is unknown or if the aspect is mandatory for
     *     the <b>class</b> of the <b>node</b>
     */
    ...
    /**
     * Determines if a given aspect is present on a node. Aspects may only be removed if they are
     * <b>NOT</b> mandatory.
     *
     * @param nodeRef NodeRef
     ...
     *
     * <p>All associations (both children and regular node associations) will be deleted, and where
     * the given node is the primary parent, the children will also be cascade deleted.
     *
     * <p>Node stores <i>may</i> be mapped to an associated archive node store. For example,
     * Alfresco ships with the mapping<br>
     * {@link StoreRef#STORE_REF_WORKSPACE_SPACESSTORE workspace://SpacesStore} .. maps to .. {@link
     * StoreRef#STORE_REF_ARCHIVE_SPACESSTORE archive://SpacesStore}.<br>
     * When a node is deleted:
     *
     * <ul>
     *   <li>If there *is a mapping* from the node's current store to an archive store, the node is
     *       first copied to the archive store
     *   <li>If there *is no mapping* from the node's current store, then the node is permanently
     *       deleted
     * </ul>
     *
     * When nodes have been archived, they contain all the details of the original location. Use the
     * {@link #restoreNode(NodeRef, NodeRef, QName, QName) restore} feature to return a node back to
     * its original store.
     *
    ...
    /**
     * Makes a parent-child association between the given nodes. Both nodes must belong to the same
     * store.
     *
     * @param parentRef the parent node
     * @param childRef the child node
     * @param assocTypeQName the qualified name of the association type as defined in the
     *     datadictionary
    ...
    /**
     * Associates a given child node with a given collection of parents. All nodes must belong to
     * the same store.
     *
     ...
     *
     * <p>The child node will be cascade deleted if the association was the primary association,
     * i.e. the one with which the child node was created.
     *
    ...
    /**
     * Replace all current properties on the node with the given properties. The properties given
     * must still fulfill the requirements of the class and aspects relevant to the node.
     *
     * <p><b>NOTE:</b> Null values <u>are</u> allowed.
     *
     ...
     *
     * <p><b>NOTE:</b> Null values <u>are</u> allowed and will replace the existing value.
     *
     ...
     * @return Returns a list of all parent-child associations that exist where the given node is
     *     the child
     * @throws InvalidNodeRefException if the node could not be found
     ...
     *
     * <p>The resultant list is ordered by (a) explicit index and (b) association creation time.
     *
     * @param nodeRef the child node
     * @param typeQNamePattern the pattern that the type qualified name of the association must
     *     match
     ...
     * @return Returns a collection of <code>ChildAssocRef</code> instances. If the node is not a
     *     <b>container</b> then the result will be empty.
     * @throws InvalidNodeRefException if the node could not be found
     * @see #getChildAssocs(NodeRef, QNamePattern, QNamePattern)
     * @see #setChildAssociationIndex(ChildAssociationRef, int)
     * @see ChildAssociationRef#getNthSibling()
     ...
     * Using a {@link org.alfresco.service.namespace.RegexQNamePattern#MATCH_ALL wildcard} for the
     * type and a specific {@link QName qualified name} for the association is akin to using the
     * XPath browse expression <b>./{url}localname</b> in the context of the parent node.
     *
     * <p>Note that all child associations are fetched and preloading is done for all results.
     *
     * @param nodeRef the parent node - usually a <b>container</b>
     * @param typeQNamePattern the pattern that the type qualified name of the association must
     *     match
     * @param qnamePattern the pattern that the qnames of the assocs must match
     * @return Returns a list of <code>ChildAssociationRef</code> instances. If the node is not a
     *     <b>container</b> then the result will be empty.
     * @throws InvalidNodeRefException if the node could not be found
     * @see QName
     * @see org.alfresco.service.namespace.RegexQNamePattern#MATCH_ALL
    ...
    /**
     * Gets all child associations where the pattern of the association qualified name is an exact
     * match.
     *
     * @param nodeRef the parent node - usually a <b>container</b>
     ...
     */
    @Auditable(
            parameters = {"nodeRef", "typeQNamePattern", "qnamePattern", "maxResults", "preload"})
    public List<ChildAssociationRef> getChildAssocs(
            NodeRef nodeRef,
    ...
    /**
     * Gets all child associations where the pattern of the association qualified names match the
     * patterns provided.
     *
     * <p>Note that all results are retrieved but preloading is optional.
     *
     * @param nodeRef the parent node - usually a <b>container</b>
    ...
    /**
     * Retrieve immediate children of a given node where the child nodes are in the given inclusive
     * list.
     *
     * @param nodeRef the parent node - usually a <b>container</b>
    ...
    /**
     * Get the node with the given name within the context of the parent node. The name is
     * case-insensitive as Alfresco has to support case-insensitive clients as standard.
     *
     * <p>That API method getChildByName only works for associations that don't allow duplicate
     * child names. See <b>cm:folder</b> and the <b>duplicate</b> tag. Child associations without
     * this allow duplicate child names and therefore it is possible to have multiple children with
     * the same name stored against the given association type.
     *
     ...
     *
     * <p>For a root node, the parent node reference will be null.
     *
     * @param nodeRef NodeRef
    ...
    public Collection<ChildAssociationRef> getChildAssocsWithoutParentAssocsOfType(
            final NodeRef parent, final QName assocTypeQName);

    /**
     * Create a peer association between two nodes.
     *
     * <p>The ordering will be natural. To change association ordering, use {@link
     * #setAssociations(NodeRef, QName, List) setAssociations}.
     *
     ...
     * @throws AssociationExistsException if an association of the same type exists between the
     *     nodes
    ...
    /**
     * Fetches all associations <i>from</i> the given source where the associations' qualified names
     * match the pattern provided.
     *
     * <p>The results are ordered if a specific type of association is requested
     *
     ...
     * @param sourceRef the association source
     * @param qnamePattern the association qname pattern to match against
     * @param propertyQName the fully qualified name of the property of the association target.
     *     Optional parameter - can be null.
     * @param propertyValue association target property value to search for. Must be a simple type
     *     such as String, Number, Date or Boolean, it cannot be a collection, a content property,
     *     MLText or a float. If <b>propertyQName</b> is null the <b>value</b> is ignored.
    ...
    /**
     * Fetches all associations <i>to</i> the given target where the associations' qualified names
     * match the pattern provided.
     *
     * @param targetRef the association target
     * @param qnamePattern the association qname pattern to match against
     * @return Returns a list of <code>NodeAssocRef</code> instances for which the given node is a
     *     target
     * @throws InvalidNodeRefException
     * @see QName
     * @see org.alfresco.service.namespace.RegexQNamePattern#MATCH_ALL
    ...
    /**
     * The root node has an entry in the path(s) returned. For this reason, there will always be
     * <b>at least one</b> path element in the returned path(s). The first element will have a null
     * parent reference and qname.
     *
     * @param nodeRef NodeRef
     ...
     *
     * @param nodeRef NodeRef
     * @param primaryOnly true if only the primary path must be retrieved. If true, the result will
    ...
    /**
     * Restore an individual node (along with its sub-tree nodes) to the target location. The
     * archived node must have the {@link org.alfresco.model.ContentModel#ASPECT_ARCHIVED archived
     * aspect} set against it. This would have been applied when a node was originally {@link
     * #deleteNode(NodeRef) deleted}.
     *
     * @param archivedNodeRef the archived node
     * @param destinationParentNodeRef the parent to move the node into or <tt>null</tt> to use the
     *     original
     * @param assocTypeQName the primary association type name to use in the new location or
     *     <tt>null</tt> to use the original
     * @param assocQName the primary association name to use in the new location or <tt>null</tt> to
     *     use the original
    ...
    public class FindNodeParameters {
        ...
        /**
         * Set the number of results to return. If this too large, then results will be trimmed by
         * wrapping classes or might cause excessive resource usage; limit the results and requery
         * using the ID-bases limiters.
         *

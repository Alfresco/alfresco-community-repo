 ...
 * <p>
 * The basic structure supported is that of a hidden container of type <b>cm:mlContainer</b> containing one or more secondary children of type <b>cm:mlDocument</b>. One of these will have a matching locale and is referred to as the <i>pivot translation</i>. It is also possible to have several transient <b>cm:emptyTranslation</b> instances that live and die with the container until they get their own content.
 * <p>
 * It is not possible to guarantee that there is always a pivot translation available in the set of sibling translations. The strategy is to hide all translations when there isn't a pivot translation available. A background task should be cleaning up the empty or invalid <b>cm:mlContainer</b> instances.
 *
 * @author Derek Hulley
...
public class MultilingualContentServiceImpl implements MultilingualContentService
{
    ...
    /**
     * Retrieve or create a <b>cm:mlDocument</b> container for the given node, which must have the <b>cm:mlDocument</b> already applied.
     *
     * @param mlDocumentNodeRef
     *            an existing <b>cm:mlDocument</b>
     * @param allowCreate
     *            <tt>true</tt> if a <b>cm:mlContainer</b> must be created if on doesn't exist, otherwise <tt>false</tt> if a parent <b>cm:mlContainer</b> is expected to exist.
    ...
    /**
     * Does the work of making the translation a simple node again. No parent-child relationships are modified and the pivot-container logic is not done here.
     * 

...
public class PropTablesCleaner {
    private static final String PROPERTY_PROP_TABLE_CLEANER_ALG =
            "system.prop_table_cleaner.algorithm";
    ...
    private static final QName LOCK_QNAME =
            QName.createQName(
                    NamespaceService.SYSTEM_MODEL_1_0_URI, PropTablesCleaner.class.getName());
    ...
    /**
     * Get {@link #LOCK_QNAME a lock} for {@link #LOCK_TTL a long-running job} and {@link
     * PropertyValueDAO#cleanupUnusedValues() call through} to get the unused data cleaned up.
     */
    public void execute() {
        checkProperties();
        ...
        try {
            ...
            ldapSyncLockTocken =
                    jobLockService.getLock(ChainingUserRegistrySynchronizer.LOCK_QNAME, LOCK_TTL);
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Skipping prop tables cleaning (could not get lock): " + e.getMessage());
            }
        } finally {
            ...
            if (ldapSyncLockTocken != null) {
                try {
                    jobLockService.releaseLock(
                            ldapSyncLockTocken, ChainingUserRegistrySynchronizer.LOCK_QNAME);
                } catch (LockAcquisitionException e) {
                    // Ignore
                }
            }

...
public class RMv22RemoveInPlaceRolesFromAllPatchUnitTest extends BaseUnitTest {
    ...
    /** Given there are no file plans to update then the 'all roles' group should not be changed. */
    @Test
    ...
    /**
     * Given that there is one file plan whose 'all roles' group does not contain the in-place roles
     * the 'all roles' groups should not be changed.
     */
    @Test
    public void rolesDontNeedRemovingFromGroup() {
        ...
        doReturn(getMockedRole(FilePlanRoleService.ROLE_EXTENDED_READERS))
                .when(mockedFilePlanRoleService)
                .getRole(filePlan, FilePlanRoleService.ROLE_EXTENDED_READERS);
        doReturn(getMockedRole(FilePlanRoleService.ROLE_EXTENDED_WRITERS))
                .when(mockedFilePlanRoleService)
                .getRole(filePlan, FilePlanRoleService.ROLE_EXTENDED_WRITERS);
        doReturn(ALL_ROLES).when(mockedFilePlanRoleService).getAllRolesContainerGroup(filePlan);
        doReturn(Collections.EMPTY_SET)
                .when(mockedAuthorityService)
                .getContainedAuthorities(null, ALL_ROLES, true);
    ...
    /**
     * Given that there is one file plan whose 'all roles' group contains the in-place roles then
     * they should be revoved.
     */
    @Test
    public void removeRolesFromGroup() {
        ...
        verify(mockedAuthorityService, times(1))
                .removeAuthority(ALL_ROLES, FilePlanRoleService.ROLE_EXTENDED_READERS);
        verify(mockedAuthorityService, times(1))
                .removeAuthority(ALL_ROLES, FilePlanRoleService.ROLE_EXTENDED_WRITERS);
        verifyNoMoreInteractions(mockedAuthorityService);
    }

...
public class PublicApiHttpClient {
  ...
  private static final String PUBLICAPI_CMIS_URL_SUFFIX =
    "{0}/{1}/cmis/versions/{2}/{3}";
  ...
  protected AnnotationMetadata getAnnotationMetadata(String classname)
    throws IOException {
  ...
  public HttpResponse submitRequest(HttpMethod req, final RequestContext rq)
    ...
    try {
      ...
      if (rq.getPassword() != null) {
        ...
        response =
          authenticatedHttp.executeHttpMethodUnauthenticated(req, callback);
      }
      return response;
    } finally {
      if (req != null) {
  ...
  public HttpResponse putBinary(
    ...
    PutMethod req = new PutMethod(url);
    if (payload != null) {
      BinaryRequestEntity requestEntity = new BinaryRequestEntity(
        payload.getFile(),
        payload.getMimeType(),
        payload.getCharset()
      );
  ...
  public HttpResponse execute(RequestBuilder requestBuilder)
    ...
    return submitRequest(
      ...
      requestBuilder.getRequestContext()
    );
  }
  ...
  private class RestApiEndpoint {
    ...
    RestApiEndpoint(
      ...
      Pair<String, String> relationshipCollectionInfo = getRelationCollectionInfo(
        resourceClass
      );
    ...
    RestApiEndpoint(
      ...
      if (networkId != null) {
        sb.append(
          getPublicApiCmisUrl(networkId, cmisBinding, version, cmisOperation)
        );
      } else {
    ...
    private String encodeToString(Object o)
      throws UnsupportedEncodingException {
      String ret = null;
    ...
    private Pair<String, String> getRelationCollectionInfo(
      Class<?> resourceClass
    ) throws IOException {
      ...
      if (
        annotationMetaData.isConcrete() && annotationMetaData.isIndependent()
      ) {
        if (
          annotationMetaData
            .getAnnotationTypes()
          ...
          String entityCollectionName = getEntityCollectionInfo(
            entityResource.getCanonicalName()
          );
    ...
    private String getEntityCollectionInfo(Class<?> resourceClass)
      throws IOException {
      return getEntityCollectionInfo(resourceClass.getCanonicalName());
    }

    private String getEntityCollectionInfo(String className)
      throws IOException {
  ...
  public abstract class RequestBuilder<T extends HttpMethod> {
    ...
    public RequestBuilder setRelationCollectionName(

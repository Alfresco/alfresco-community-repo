...
public class FixBpmPackagesPatch extends AbstractPatch {
    ...
    private static final String ERR_MSG_INVALID_BOOTSTRAP_STORE =
            "patch.fixBpmPackages.invalidBootsrapStore";
    ...
    protected String applyInternal() throws Exception {
        ...
        try {
            ...
            // Get /sys:system container path, if it doesn't exist there is something wrong with the
            // repo
            String sysContainer =
                    importerBootstrap
                            .getConfiguration()
                            .getProperty("system.system_container.childname");
            ...
            List<ChildAssociationRef> refs =
                    nodeService.getChildAssocs(
                            rootRef, ContentModel.ASSOC_CHILDREN, sysContainerQName);
            ...
            // Get /sys:system/sys:workflow container, if it doesn't exist there is something wrong
            // with the repo
            String sysWorkflowContainer =
                    importerBootstrap
                            .getConfiguration()
                            .getProperty("system.workflow_container.childname");
            QName sysWorkflowQName = QName.createQName(sysWorkflowContainer, namespaceService);

            refs =
                    nodeService.getChildAssocs(
                            sysNodeRef, ContentModel.ASSOC_CHILDREN, sysWorkflowQName);
            ...
            // Try to get /sys:system/sys:workflow/cm:packages, if there is no such node, then it
            // wasn't created yet,
            ...
            if (refs == null || refs.size() == 0) {
                if (logger.isDebugEnabled())
                    logger.debug(
                            "There are no any packages in the container " + sysWorkflowContainer);
    ...
    private class FixBpmPackagesPatchHelper {
        ...
        public String fixBpmPackages(List<ChildAssociationRef> references) throws Exception {
            ...
            BatchProcessWorkProvider<ChildAssociationRef> workProvider =
                    new BatchProcessWorkProvider<ChildAssociationRef>() {
                        @Override
            ...
            BatchProcessor<ChildAssociationRef> batchProcessor =
                    new BatchProcessor<ChildAssociationRef>(
                            "FixBpmPackagesPatch",
                            transactionHelper,
                            workProvider,
                            batchThreads,
            ...
            BatchProcessor.BatchProcessWorker<ChildAssociationRef> worker =
                    new BatchProcessor.BatchProcessWorker<ChildAssociationRef>() {
                        @Override
                        ...
                        public void process(ChildAssociationRef assocRef) throws Throwable {
                            ...
                            for (ChildAssociationRef itemAssoc : packageItemsAssocs) {
                                ...
                                String itemName =
                                        (String)
                                                nodeService.getProperty(
                                                        childRef, ContentModel.PROP_NAME);
                                // To avoid unnecessary deletion of the child item, we check if the
                                // association is not primary
                                // For the package item it should be not primary association.
                                if (itemAssoc.isPrimary()) {
                                    logger.error(
                                            "Association between package: "
                                                    + name
                                                    + " and item: "
                                                    + itemName
                                                    + " is primary association, so removing this"
                                                    + " assiciation will result in child node"
                                                    + " deletion");
                                    continue;
                                }
                                ...
                                if (itemAssoc
                                        .getTypeQName()
                                        .equals(WorkflowModel.ASSOC_PACKAGE_CONTAINS)) {
                                    continue;
                                }

                                boolean assocRemoved =
                                        nodeService.removeChildAssociation(itemAssoc);
                                if (assocRemoved) {
                                    if (logger.isDebugEnabled())
                                        logger.debug(
                                                "Association between package: "
                                ...
                                // Recreate new association between package and particular item as
                                // bpm:packageContains
                                /* ChildAssociationRef newChildAssoc = */ nodeService.addChild(
                                        parentRef,
                                        childRef,
                                ...
                                if (logger.isDebugEnabled()) {
                                    logger.debug(
                                            "New association has been created between package: "
                                                    + name
                                                    + " and item: "
                                                    + itemName);
                                }

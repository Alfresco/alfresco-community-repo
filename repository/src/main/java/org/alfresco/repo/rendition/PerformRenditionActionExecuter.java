...
public class PerformRenditionActionExecuter extends ActionExecuterAbstractBase {
  ...
  private static final Log log = LogFactory.getLog(
    PerformRenditionActionExecuter.class
  );
  ...
  protected void executeImpl(
    final Action containingAction,
    final NodeRef actionedUponNodeRef
  ) {
    final RenditionDefinition renditionDefinition = getRenditionDefinition(
      containingAction
    );
    if (log.isDebugEnabled()) {
      ...
      if (renditionDefinition.getParameterValues().isEmpty() == false) {
        for (String paramKey : renditionDefinition
          .getParameterValues()
          .keySet()) {
          msg
            .append("    ")
            .append(paramKey)
            .append("=")
            .append(renditionDefinition.getParameterValue(paramKey))
            .append("\n");
        }
      } else {
        msg.append("    [None]");
    ...
    ChildAssociationRef result = executeRendition(
      actionedUponNodeRef,
      renditionDefinition
    );
  ...
  private RenditionDefinition getRenditionDefinition(
    ...
    Serializable rendDefObj = containingAction.getParameterValue(
   ...
   */
  private ChildAssociationRef executeRendition(
    NodeRef sourceNode,
    RenditionDefinition definition
  ) {
    actionService.executeAction(definition, sourceNode);
    // Extract the result from the action
    Serializable serializableResult = definition.getParameterValue(

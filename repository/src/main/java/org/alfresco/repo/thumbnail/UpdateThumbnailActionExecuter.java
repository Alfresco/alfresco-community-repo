...
public class UpdateThumbnailActionExecuter extends ActionExecuterAbstractBase {
  ...
  private static Log logger = LogFactory.getLog(
    UpdateThumbnailActionExecuter.class
  );
   ...
   */
  public void setMimetypeMaxSourceSizeKBytes(
    HashMap<String, Long> mimetypeMaxSourceSizeKBytes
  ) {
  ...
  public void setGenerateThumbnails(boolean generateThumbnails) {
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Thumbnail generation is " +
        (generateThumbnails ? "enabled" : "disabled") +
        "via deprecated method in " +
  ...
  protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
    ...
    // Get the thumbnail
    NodeRef thumbnailNodeRef = (NodeRef) action.getParameterValue(
    ...
    if (
      this.nodeService.exists(thumbnailNodeRef) == true &&
      renditionService.isRendition(thumbnailNodeRef)
    ) {
      // Get the thumbnail Name
      ChildAssociationRef parent = renditionService.getSourceNode(
        thumbnailNodeRef
      );
      ...
      if (contentProp == null) {
        logger.info(
          "Creation of thumbnail, null content for " + details.getName()
        );
        return;
      }
      ...
      if (contentProp instanceof ContentData) {
        ...
        if (mimetypeMaxSourceSizeKBytes != null) {
          ...
          if (
            maxSourceSizeKBytes != null &&
            maxSourceSizeKBytes >= 0 &&
            maxSourceSizeKBytes < (content.getSize() / 1024L)
          ) {
            logger.debug(
              ...
              "' for " +
              mimetype +
              " as content is too large (" +
              (content.getSize() / 1024L) +
              "K > " +
              maxSourceSizeKBytes +
              "K)"
            );
  ...
  protected void addParameterDefinitions(List<ParameterDefinition> paramList) {
    ...
    paramList.add(
      new ParameterDefinitionImpl(
        ...
        DataTypeDefinition.QNAME,
        false,
        getParamDisplayLabel(PARAM_THUMBNAIL_NODE)
      )
    );
  }
}

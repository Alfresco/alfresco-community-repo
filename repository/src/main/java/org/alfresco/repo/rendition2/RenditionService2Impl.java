...
public class RenditionService2Impl
  ...
  public static final QName DEFAULT_RENDITION_CONTENT_PROP =
  ...
  public void setRenditionRequestSheduler(
    PostTxnCallbackScheduler renditionRequestSheduler
  ) {
    this.renditionRequestSheduler = renditionRequestSheduler;
  }

  public void setTransformReplyProvider(
    TransformReplyProvider transformReplyProvider
  ) {
    this.transformReplyProvider = transformReplyProvider;
  }

  public void setAsynchronousExtractor(
    AsynchronousExtractor asynchronousExtractor
  ) {
    ...
    requestAsyncTransformOrRendition(
      ...
      new RenderOrTransformCallBack() {
        ...
        public String getName() {
          ...
          return (
            "Transform" + (transformName == null ? "" : " " + transformName)
          );
        }
    ...
    requestAsyncTransformOrRendition(
      ...
      new RenderOrTransformCallBack() {
        ...
        public void handleUnsupported(UnsupportedOperationException e) {
          ...
          NodeRef renditionNode = getRenditionNode(
            sourceNodeRef,
            renditionName
          );
        ...
        @Override
        public void throwIllegalStateExceptionIfAlreadyDone(
          ...
          int renditionContentHashCode = getRenditionContentHashCode(
            renditionNode
          );
  ...
  private void requestAsyncTransformOrRendition(
    NodeRef sourceNodeRef,
    RenderOrTransformCallBack renderOrTransform
  ) {
    try {
      if (!isEnabled()) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          renderOrTransform.getName() + ": transform " + sourceNodeRef
        );
      }
      ...
      RetryingTransactionHelper.RetryingTransactionCallback callback = () -> {
        ...
        if (!supported.get()) {
          ...
          if (sourceContentHashCode != SOURCE_HAS_NO_CONTENT) {
            ...
            if (logger.isDebugEnabled()) {
              logger.debug(
                renderOrTransform.getName() + ": Source had no content."
              );
            }
            failure(sourceNodeRef, renditionDefinition, sourceContentHashCode);
          }
      ...
      renditionRequestSheduler.scheduleRendition(
        callback,
  ...
  public void consume(
    ...
    if (renditionDefinition instanceof TransformDefinition) {
      ...
      if (AsynchronousExtractor.isMetadataExtractMimetype(targetMimetype)) {
        ...
        AsynchronousExtractor.isMetadataEmbedMimetype(targetMimetype)
      ) {
  ...
  private void consumeEmbeddedMetadata(
    ...
    if (transformInputStream == null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Ignore transform for metadata embed on " + nodeRef + " as it failed"
        );
      }
    } else if (transformContentHashCode != sourceContentHashCode) {
      if (logger.isDebugEnabled()) {
          ...
          transactionService
            ...
            .doInTransaction(
              () -> {
                ...
                boolean sourceHasAspectRenditioned = nodeService.hasAspect(
                  sourceNodeRef,
                ...
                try {
                  ...
                  if (createRenditionNode) {
                    renditionNode =
                      createRenditionNode(sourceNodeRef, renditionDefinition);
                  } else if (
                    ...
                    if (logger.isDebugEnabled()) {
                      logger.debug(
                        "Added rendition2 aspect to rendition " +
                        renditionName +
                        " on " +
                        sourceNodeRef
                      );
                  ...
                  if (logger.isDebugEnabled()) {
                    logger.debug(
                      ...
                      transformContentHashCode +
                      " and ThumbnailLastModified for " +
                      renditionName
                    );
                  }
                  nodeService.setProperty(
                    renditionNode,
                    RenditionModel.PROP_RENDITION_CONTENT_HASH_CODE,
                    transformContentHashCode
                  );
                  ...
                  if (transformInputStream != null) {
                    try {
                      ...
                      logger.error(
                        "Failed to copy transform InputStream into rendition " +
                        renditionName +
                        " on " +
                        sourceNodeRef
                      );
  ...
  private void setThumbnailLastModified(
    NodeRef sourceNodeRef,
    ...
    if (
      ...
      List<String> thumbnailMods = (List<String>) nodeService.getProperty(
        sourceNodeRef,
  ...
  private int getRenditionContentHashCode(NodeRef renditionNode) {
    ...
    Serializable hashCode = nodeService.getProperty(
      renditionNode,
      PROP_RENDITION_CONTENT_HASH_CODE
    );
  ...
  private NodeRef getRenditionNode(
    NodeRef sourceNodeRef,
    String renditionName
  ) {
    ...
    List<ChildAssociationRef> renditionAssocs = nodeService.getChildAssocs(
      sourceNodeRef,
      RenditionModel.ASSOC_RENDITION,
      renditionQName
    );
    return renditionAssocs.isEmpty()
      ? null
      : renditionAssocs.get(0).getChildRef();
  }

  // Only called by the old service to work out if a rendition was created by the new service.
  public boolean isCreatedByRenditionService2(
    NodeRef sourceNodeRef,
    String renditionName
  ) {
  ...
  private void checkSourceNodeForPreventionClass(NodeRef sourceNode) {
    if (sourceNode != null && nodeService.exists(sourceNode)) {
      ...
      for (QName contentClass : nodeContentClasses) {
        if (
          renditionPreventionRegistry.isContentClassRegistered(contentClass)
        ) {
          String msg =
            "Node " +
            sourceNode +
  ...
  public List<ChildAssociationRef> getRenditions(NodeRef sourceNodeRef) {
    ...
    List<ChildAssociationRef> childAsocs = getRenditionChildAssociations(
      sourceNodeRef
    );
  ...
  public boolean isRenditionAvailable(
    ...
    if (
      ...
      Serializable contentUrl = nodeService.getProperty(
        renditionNode,
        ContentModel.PROP_CONTENT
      );
      if (contentUrl == null) {
  ...
  public ChildAssociationRef getRenditionByName(
    NodeRef sourceNodeRef,
    String renditionName
  ) {
    ...
    if (renditions.isEmpty()) {
      ...
      NodeRef renditionNode = childAssoc.getChildRef();
      return !isRenditionAvailable(sourceNodeRef, renditionNode)
        ? null
        : childAssoc;
    }
  }
  ...
  public void onContentUpdate(NodeRef sourceNodeRef, boolean newContent) {
    if (isEnabled()) {
      if (nodeService.exists(sourceNodeRef)) {
        ...
        List<ChildAssociationRef> childAssocs = getRenditionChildAssociations(
          sourceNodeRef
        );
        ...
        logger.debug(
          "onContentUpdate rendition " + sourceNodeRef + " does not exist."
        );
      }
    }
  }

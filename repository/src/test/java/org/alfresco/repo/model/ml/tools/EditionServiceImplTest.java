...
public class EditionServiceImplTest extends AbstractMultilingualTestCases {
  ...
  public void testEditionLabels() {
    ...
    NodeRef pivot = multilingualContentService.getPivotTranslation(
      mlContainerNodeRef
    );
    ...
    Version rootEdition = editionService
      .getEditions(mlContainerNodeRef)
      .getAllVersions()
      .iterator()
      .next();
    ...
    assertTrue(
      "The edition label would be 1.1 and not " +
      firstEdition.getVersionLabel(),
    ...
    assertTrue(
      "The edition label would be 2.0 and not " +
      secondEdition.getVersionLabel(),
    ...
    assertTrue(
      "The edition label would be 2.1 and not " +
      thirdEdition.getVersionLabel(),
  ...
  public void testCreateEdition() throws Exception {
    ...
    NodeRef frenchContentNodeRef = multilingualContentService.getTranslationForLocale(
      mlContainerNodeRef,
      Locale.FRENCH
    );
    ...
    NodeRef newStartingPoint = editionService.createEdition(
      frenchContentNodeRef,
      null
    );
    // get the edition history
    VersionHistory editionHistory = editionService.getEditions(
      mlContainerNodeRef
    );
    ...
    List<VersionHistory> rootVersionTranslations = editionService.getVersionedTranslations(
      rootEdition
    );
    ...
    assertNotNull(
      "The translations list of the root edition can't be null",
      rootVersionTranslations
    );

    // Ensure that the new starting document noderef is different that the initial one
    assertFalse(
      "The created starting document must be different that the starting document of the edition",
    ...
    numberOfTranslations =
      editionService.getVersionedTranslations(actualEdition).size();
    assertEquals(
  ...
  public void testEditionServiceWithContent() {
    ...
    NodeRef chineseContentNodeRef = createContent(
      CHINESE_CONTENT + "-" + Locale.CHINESE + "-" + "1.2" + "-1.0"
    );
    ...
    NodeRef japaneseContentNodeRef = createContent(
      JAPANESE_CONTENT + "-" + Locale.JAPANESE + "-" + "1.2" + "-1.0"
    );
    multilingualContentService.addTranslation(
    ...
    japaneseContentNodeRef =
      editionService.createEdition(japaneseContentNodeRef, null);
    checkContentVersionValuesForEditions(mlContainerNodeRef);
    japaneseContentNodeRef =
      multilingualContentService.getTranslationForLocale(
        mlContainerNodeRef,
    ...
    chineseContentNodeRef =
      createContent(
        CHINESE_CONTENT + "-" + Locale.CHINESE + "-" + "1.3" + "-1.0"
      );
    ...
    frenchContentNodeRef =
      createContent(
        FRENCH_CONTENT + "-" + Locale.FRENCH + "-" + "1.3" + "-1.0"
      );
    multilingualContentService.addTranslation(
      frenchContentNodeRef,
      mlContainerNodeRef,
    ...
    Collection<Version> editions = editionService
      .getEditions(mlContainerNodeRef)
      .getAllVersions();
    ...
    frenchContentNodeRef =
      createContent(
        FRENCH_CONTENT + "-" + Locale.FRENCH + "-" + "1.0" + "-1.0"
      );
    modifyContent(
      frenchContentNodeRef,
    ...
    assertTrue(
      "The edition label would be 3.0 and not " +
      secondEdition.getVersionLabel(),
  ...
  private void checkFirstVersion(NodeRef mlContainerNodeRef) {
    ...
    assertTrue(
      "The versioned mlContainer noderef of the editon must be the noderef of the created mlContainer",
      currentEdition.getVersionedNodeRef().equals(mlContainerNodeRef)
    );

    // get the list of translations
    List<VersionHistory> translations = editionService.getVersionedTranslations(
      currentEdition
    );

    // Ensure that the get versioned translations is empty
    assertNotNull(
      "The translations list of the current edition can't be null",
      translations
    );
  ...
  private NodeRef createMLContainerWithContent() {
    ...
    return multilingualContentService.getTranslationContainer(
      chineseContentNodeRef
    );
  }

  private void checkContentVersionValuesForEditions(
    NodeRef mlContainerNodeRef
  ) {
    ...
    for (Version editionVersion : editionHistory.getAllVersions()) {
      ...
      if (editionHistory.getHeadVersion() == editionVersion) {
        ...
        Map<Locale, NodeRef> translations = multilingualContentService.getTranslations(
          mlContainerNodeRef
        );
        ...
        for (Locale locale : keySet) {
          ...
          for (Version version : versionHistory.getAllVersions()) {
            ...
            Locale versionLocale = (Locale) nodeService.getProperty(
              frozenNodeRef,
              ContentModel.PROP_LOCALE
            );
            // get content as string and compare
            ...
            if (
              ...
              Version previousEditionVersion = editionHistory.getPredecessor(
                editionVersion
              );
              ...
              if (previousEditionVersion != null) {
                ...
                List<VersionHistory> versionTranslations = editionService.getVersionedTranslations(
        ...
        Locale editionPivotlanguage = (Locale) (
          editionService
        ...
        for (VersionHistory versionTranslation : versionTranslations) {
          ...
          for (Version version : versions) {
            ...
            Locale currentVersionLocale = (Locale) nodeService.getProperty(
              frozenNodeRef,
              ContentModel.PROP_LOCALE
            );
            System.out.println(
            ...
            if (
              ...
              if (previousEditionVersion != null) {
                ...
                List<VersionHistory> versionTranslations2 = editionService.getVersionedTranslations(
                  previousEditionVersion
                );
                // for all languages iterate to find the corresponding language
  ...
  private NodeRef createMLContainerWithContent(String editionSuffix) {
    ...
    NodeRef frenchContentNodeRef = createContent(
      FRENCH_CONTENT + "-" + Locale.FRENCH + "-" + editionSuffix + "-1.0"
    );
    NodeRef japaneseContentNodeRef = createContent(
  ...
  private void dumpFrozenMetaData(Version editionVersion) {
    ...
    for (String q : mapOfFrozenProps.keySet()) {
      String val = mapOfFrozenProps.get(q) == null
        ? "null"
        : mapOfFrozenProps.get(q).toString();

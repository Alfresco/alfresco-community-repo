...
public abstract class AbstractUsageDAOImpl implements UsageDAO {
  ...
  public long getTotalDeltaSize(NodeRef nodeRef, boolean removeDeltas) {
    ...
    if (removeDeltas) {
      ...
      if (
        entity.getDeltaCount() != null &&
        entity.getDeltaCount().intValue() != deleted
      ) {
        throw new ConcurrencyFailureException(
          "The number of usage deltas was " +
          entity.getDeltaCount() +
          " but only " +
          deleted +
          " were deleted."
        );
  ...
  public Long getContentSizeForStoreForUser(
    StoreRef storeRef,
    String userName
  ) {
  ...
  protected abstract UsageDeltaEntity insertUsageDeltaEntity(
    UsageDeltaEntity entity
  );

  protected abstract UsageDeltaEntity selectTotalUsageDeltaSize(
    long nodeEntityId
  );

  protected abstract List<Long> selectUsageDeltaNodes();

  protected abstract void selectUsersWithoutUsage(
    StoreRef storeRef,
    MapHandler handler
  );

  protected abstract void selectUsersWithUsage(
    StoreRef storeRef,
    MapHandler handler
  );

  protected abstract void selectUserContentSizesForStore(
    StoreRef storeRef,
    MapHandler resultsCallback
  );

  protected abstract Long selectContentSizeForStoreForUser(
    StoreRef storeRef,
    String userName
  );

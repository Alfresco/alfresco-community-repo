...
/**
 * This is an instance of a "locked delete shuffle" triggered by a create of a file matching a
 * specified pattern.
 *
 * <p>First implemented for TextEdit from MacOS Lion
 *
 * <p>Sequence of operations. a) Lock file created. Typically with an obscure name. b) Temp file
 * created in temporary folder c) Target file deleted d) Temp file renamed to target file. e) Lock
 * file deleted
 *
 * <p>If this filter is active then this is what happens. a) Lock file created. Lock file created
 * (X). b) Temp file created - in another folder. c) Existing file deleted. Scenario kicks in to
 * rename rather than delete. d) New file moved into place (X to Y). Scenario kicks in 1) renames
 * file from step c 2) copies content from temp file to target file 3) deletes temp file. e) Lock
 * file deleted.
 */

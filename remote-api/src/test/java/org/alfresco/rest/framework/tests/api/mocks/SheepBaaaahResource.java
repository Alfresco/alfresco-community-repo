...
@RelationshipResource(
        ...
        title = "Sheep baaah")
public class SheepBaaaahResource
        implements RelationshipResourceAction.Read<Sheep>,
                RelationshipResourceAction.ReadById<Sheep>,
                RelationshipResourceBinaryAction.Read,
                RelationshipResourceBinaryAction.Delete,
                RelationshipResourceBinaryAction.Update {
    ...
    public CollectionWithPagingInfo<Sheep> readAll(String entityResourceId, Parameters params) {
        ...
        toReturn =
                toReturn.subList(
                        0,
                        params.getPaging().getMaxItems() > toReturn.size()
                                ? toReturn.size()
                                : params.getPaging().getMaxItems());
        return CollectionWithPagingInfo.asPaged(
                params.getPaging(), toReturn, toReturn.size() != 3, 3);
    }

    @Override
    @WebApiDescription(title = "Reads a photo")
    @BinaryProperties("photo")
    public BinaryResource readProperty(String entityId, String id, Parameters parameters)

...
public class GetCopiesCannedQueryFactory
  extends AbstractCopyCannedQueryFactory<CopyInfo> {

  @Override
  public CannedQuery<CopyInfo> getCannedQuery(
  ...
  private class GetCopiesCannedQuery
    extends AbstractCannedQueryPermissions<CopyInfo> {

    private GetCopiesCannedQuery(
      CannedQueryParameters parameters,
      MethodSecurityBean<CopyInfo> methodSecurity
    ) {
    ...
    protected List<CopyInfo> queryAndFilter(CannedQueryParameters parameters) {
      CopyCannedQueryDetail detail =
        GetCopiesCannedQueryFactory.this.getDetail(parameters);
      // Build parameters
      CopyParametersEntity queryParameters = new CopyParametersEntity();
      // Original node
      Pair<Long, NodeRef> originalNodePair = nodeDAO.getNodePair(
        detail.originalNodeRef
      );
      ...
      Pair<Long, QName> assocTypeQNamePair = qnameDAO.getQName(
      ...
      // cm:name property ID
      Pair<Long, QName> propQNamePair = qnameDAO.getQName(
      ...
      if (detail.copyParentNodeRef != null) {
        Pair<Long, NodeRef> copyParentNodePair = nodeDAO.getNodePair(
          detail.copyParentNodeRef
        );
      ...
      // Convert them
      List<CopyInfo> results = new ArrayList<CopyService.CopyInfo>(
        copies.size()
      );
      for (CopyEntity copy : copies) {
        CopyInfo result = new CopyInfo(
          copy.getCopy().getNodeRef(),
          copy.getCopyName()
        );
        results.add(result);
      }

...
public class ImapServiceImplTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        user =
                new AlfrescoImapUser(
                        anotherUserName + "@alfresco.com", anotherUserName, anotherUserName);
    ...
    private void importInternal(String acpName, NodeRef space) throws IOException {
        ...
        ACPImportPackageHandler acpHandler =
                new ACPImportPackageHandler(acpResource.getFile(), null);
    ...
    //    {
    //        List<FileInfo> fi = imapService.searchFolders(testImapFolderNodeRef, FOLDER_PATTERN,
    // true, ImapViewMode.ARCHIVE);
    //        assertNotNull("Can't find folders in Archive Mode", fi);
    //        assertEquals("Can't find folders in Archive Mode", fi.size(), 2);
    //
    //        fi = imapService.searchFolders(testImapFolderNodeRef, FOLDER_PATTERN, false,
    ...
    //    {
    //        List<FileInfo> fi = imapService.searchFiles(testImapFolderNodeRef, FILE_PATTERN,
    // true);
    //        assertNotNull(fi);
    ...
    //    {
    //        List<FileInfo> fi = imapService.searchMails(testImapFolderNodeRef,
    // ImapViewMode.MIXED);
    //        assertNotNull(fi);
    //        assertTrue(fi.size() > 0);
    //    }
        ...
        if (fis != null && fis.size() > 0) {
            ...
            Serializable prop =
                    nodeService.getProperty(
                            messageFileInfo.getNodeRef(), ImapModel.PROP_FLAG_RECENT);
    ...
    public void testContentRecovery() throws Exception {
        ...
        NodeRef nodeRef =
                nodeService
                        .createNode(
                                ...
                                QName.createQName(
                                        NamespaceService.CONTENT_MODEL_1_0_URI, "content_recover"),
        ...
        if (nodeService.exists(archivedNodeRef)) {
            NodeRef restoredNode =
                    nodeService.restoreNode(archivedNodeRef, testImapFolderNodeRef, null, null);

            Map<QName, Serializable> props = nodeService.getProperties(restoredNode);

            if (props.containsKey(ImapModel.PROP_FLAG_DELETED)
    ...
    public void testAttachmentExtraction() throws Exception {
        ...
        List<AssociationRef> targetAssocs =
                nodeService.getTargetAssocs(
                        f2.getNodeRef(), ImapModel.ASSOC_IMAP_ATTACHMENTS_FOLDER);
    ...
    public void testMailboxRenamingInTheRootForMNT9055() throws Exception {
        reauthenticate(USER_NAME, USER_PASSWORD);
        AlfrescoImapUser poweredUser =
                new AlfrescoImapUser((USER_NAME + "@alfresco.com"), USER_NAME, USER_PASSWORD);
        ...
        FileInfo targetNode =
                fileFolderService.create(root, targetNodeName, ContentModel.TYPE_FOLDER);
        ...
        AlfrescoImapFolder actualMailbox =
                imapService.getOrCreateMailbox(poweredUser, renamedNodePath, true, false);

        List<ChildAssociationRef> parentAssocs =
                nodeService.getParentAssocs(actualMailbox.getFolderInfo().getNodeRef());
    ...
    public void testMailboxRenamingInTheUserImapHomeForMNT9055() throws Exception {
        ...
        AlfrescoImapFolder mailbox =
                imapService.getOrCreateMailbox(poweredUser, targetNodeName, false, true);
        assertMailboxNotNull(mailbox);

        List<String> pathBeforeRenaming =
                fileFolderService.getNameOnlyPath(root, mailbox.getFolderInfo().getNodeRef());
    ...
    public void testMiddleMailboxRenamingInDeepHierarchyForMNT9055() throws Exception {
        ...
        List<String> fullLeftPathList =
                new LinkedList<String>(); // Path elements to the middle folder
        List<String> fullRightPathList =
                new LinkedList<String>(); // Path from the middle to the leaf folder
        String renamedNodeName = "Renamed-" + System.currentTimeMillis();
        ...
        pathAfterRenaming =
                fileFolderService.getNameOnlyPath(middleNode.getNodeRef(), targetNode.getNodeRef());
    ...
    public void testMailboxMoving() throws Exception {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            AlfrescoImapFolder mailbox =
                    imapService.getOrCreateMailbox(
                            poweredUser, (imapUserHomePath + targetNodeName), false, true);
            assertNotNull(mailbox);

            targetNode = mailbox.getFolderInfo();
            imapUserHomePath =
                    fullPath.toString() + targetNodeName + AlfrescoImapConst.HIERARCHY_DELIMITER;
        }
        ...
        List<String> pathAfterRenaming =
                fileFolderService.getNameOnlyPath(root, targetNode.getNodeRef());
        fullPathList.add(newName);
        assertPathHierarchy(fullPathList, pathAfterRenaming);
    }

    /**
     * Test for MNT-12259 There is a 5s gap to run the test, see {@link
     * ImapServiceImpl#hideAndDelete}
     *
     * @throws Exception
     */
    public void testMoveViaDeleteAndAppend() throws Exception {
        ...
        ContentWriter contentWriter =
                contentService.getWriter(origFile.getNodeRef(), ContentModel.PROP_CONTENT, true);
        contentWriter.setMimetype("text/plain");
        contentWriter.setEncoding("UTF-8");
        contentWriter.putContent(nodeContent);

        FileInfo destinationNode =
                fileFolderService.create(root, destinationName, ContentModel.TYPE_FOLDER);
        ...
        AlfrescoImapFolder destinationMailbox =
                imapService.getOrCreateMailbox(poweredUser, destinationPath, true, false);
        ...
        NodeRef newNodeRef =
                nodeService.getChildAssocs(destinationNode.getNodeRef()).get(0).getChildRef();
    ...
    public void testMoveViaAppendAndDelete() throws Exception {
        ...
        origMessage
                .getMimeMessage()
                .addHeader(AlfrescoImapConst.X_ALF_NODEREF_ID, origFile.getNodeRef().getId());
        origMessage.getMimeMessage().saveChanges();
        ...
        NodeRef copiedParentNodeRef =
                nodeService.getPrimaryParent(copiedNode.getNodeRef()).getParentRef();
        assertEquals(
        ...
        assertTrue(
                ...
                nodeService.hasAspect(copiedNode.getNodeRef(), ContentModel.ASPECT_TAGGABLE));
    }
    ...
    /**
     * @param user - {@link AlfrescoImapUser} instance, which determines a user who has enough
     *     permissions to rename a node
     * @param root - {@link NodeRef} instance, which determines <code>Alfresco IMAP</code> root node
     * @param targetNodePath - {@link String} value, which determines a path in IMAP notation to a
     *     node which should be renamed
     * @param targetNode - {@link FileInfo} instance, which determines a node, located at the <code>
     *     targetNodePath</code> path
    ...
    private void assertMailboxRenaming(
            ...
            throws FileNotFoundException {
        AlfrescoImapFolder mailbox =
                imapService.getOrCreateMailbox(user, targetNodePath, true, false);
        assertNotNull(
        ...
        assertNotNull(
                ("Can't receive renamed node by full path: '" + renamedNodePath + "'"), actualNode);
     ...
     * @throws FileNotFoundException
     */
    private void assertMailboxInUserImapHomeDirectory(NodeRef root, NodeRef actualNode)
            throws FileNotFoundException {
    ...
    /**
     * @param pathBeforeRenaming - {@link List}&lt;{@link String}&gt; instance, which represents a
     *     path to some node <b>before</b> some action
     * @param pathAfterRenaming - {@link List}&lt;{@link String}&gt; instance, which represents a
     *     path to some node <b>after</b> some action
     */
    private void assertPathHierarchy(
            List<String> pathBeforeRenaming, List<String> pathAfterRenaming) {
    ...
    private NodeRef findCompanyHomeNodeRef() {
        ...
        List<NodeRef> nodeRefs =
                searchService.selectNodes(
                        storeRootNodeRef, APP_COMPANY_HOME, null, namespaceService, false);
    ...
    public void testSearchTerms() throws AddressException {
        List<AlfrescoImapFolder> mf =
                imapService.listMailboxes(
                        user, IMAP_ROOT + "/" + TEST_IMAP_FOLDER_NAME + "/_*", false);
    ...
    private void extractSearchTermResultsToList(
            List<AlfrescoImapFolder> folders, SearchTerm searchTerm, ArrayList<Long> resList) {

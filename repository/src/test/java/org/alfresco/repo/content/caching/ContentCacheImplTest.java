...
public class ContentCacheImplTest {
  ...
  public void willCreateNonExistentCacheRoot() {
    ...
    assertFalse(
      "Pre-condition of test is that cacheRoot does not exist",
      cacheRoot.exists()
    );
  ...
  public void getContentUrl() {
    ...
    Mockito
      .when(lookupTable.get(Key.forCacheFile(cacheFile)))
  ...
  public void putForNonEmptyFile() {
    ...
    Mockito
      ...
      .when(contentReader)
      .getContent(cacheFileArg.capture());
    ...
    String cacheFilePath = cacheFileArg
      .getValue()
      .getAbsolutePath()
      .replace(".tmp", ".bin");
  ...
  public void deleteFile() {
    ...
    Mockito
      .when(contentCache.getCacheFilePath("url"))
      .thenReturn(cacheFile.getAbsolutePath());
  ...
  public void getWriter() {
    ...
    Mockito
      .verify(lookupTable)
      .put(Key.forUrl(url), writer.getFile().getAbsolutePath());
    Mockito
      .verify(lookupTable)
      .put(Key.forCacheFile(writer.getFile().getAbsolutePath()), url);
  }
  ...
  public void compareNumericFileNames() {
    ...
    assertEquals(
      -1,
      comparator.compare(new File("3"), new File("non-numeric"))
    );
  ...
  public void canVisitOldestDirsFirst() {
    ...
    File f1 = tempfile(
      createDirs("2000/3/30/17/45/31"),
      "files-are-unsorted.bin"
    );
    File f2 = tempfile(createDirs("2000/3/4/17/45/31"), "another-file.bin");
    File f3 = tempfile(
      createDirs("2010/12/24/23/59/58"),
      "a-second-before.bin"
    );

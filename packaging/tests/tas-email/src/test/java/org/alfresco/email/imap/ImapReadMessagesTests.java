...
public class ImapReadMessagesTests extends EmailTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify folders created in repository in other folder via IMAP client by admin"
                            + " user")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Verify file and its content are displayed via IMAP client when the file is"
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Verify that file content in IMAP client contains creator, title, description,"
                            + " created date, modifier, modified date, size, three links to content"
                            + " folder, to content url, to download url")
    @Test(groups = {TestGroup.PROTOCOLS, TestGroup.IMAP, TestGroup.SANITY})
    public void siteManagerShouldVerifyFileContent() throws Exception {
        ...
        imapProtocol
                ...
                .then()
                .assertThat()
                .messageContentMatchesFileModelData(fileModel);
    }
    ...
    public void adminCanMarkMessageAsRead() throws Exception {
        ...
        imapProtocol
                ...
                .then()
                .assertThat()
                .messageContainsFlags(Flags.Flag.SEEN);
    }
    ...
    public void fileWithSpacesInNameIsDisplayedInImap() throws Exception {
        testFile =
                dataContent
                        .usingUser(testUser)
                        .usingSite(testSite)
                        .createContent(new FileModel("name with spaces.txt", FileType.TEXT_PLAIN));
        imapProtocol
                .authenticateUser(testUser)
                .usingSite(testSite)
                .assertThat()
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that a file created with name which contains special characters is"
                            + " visible in IMAP")
    ...
    public void userCanViewFileWithSpecialCharactersInNameViaIMAP() throws Exception {
        testFile =
                dataContent
                        .usingUser(testUser)
                        .usingSite(testSite)
                        .createContent(new FileModel("(a)[b]!#%^.txt", FileType.TEXT_PLAIN));
        imapProtocol
                .authenticateUser(testUser)
                .usingSite(testSite)
                .assertThat()
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that a file created with name which contains symbols is visible in"
                            + " IMAP")
    @Test(groups = {TestGroup.PROTOCOLS, TestGroup.IMAP, TestGroup.FULL})
    public void userCanViewFileWithSymbolsInNameViaIMAP() throws Exception {
        testFile =
                dataContent
                        .usingUser(testUser)
                        .usingSite(testSite)
                        .createContent(new FileModel("a£¥€$♊♎♏♐♑♒♓Ω.txt", FileType.TEXT_PLAIN));
        imapProtocol
                .authenticateUser(testUser)
                .usingSite(testSite)
                .assertThat()
    ...
    public void siteManagerCanViewLinks() throws Exception {
        ...
        imapProtocol
                .authenticateUser(testUser)
                .usingSiteLinksContainer(testSite)
                .assertThat()
                .countMessagesIs(1);
    }
    ...
    public void siteManagerCanViewCalendarEvents() throws Exception {
        ...
        imapProtocol
                .authenticateUser(testUser)
                .usingSiteCalendarContainer(testSite)
                .assertThat()
                .countMessagesIs(1);
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that site manager cannot read file via IMAP if it is already deleted"
                            + " from repository")
    @Test(
    ...
    public void siteManagerCannotReadFileInImapIfItWasDeletedFromRepository() throws Exception {
        testFile =
                dataContent
                        ...
                        .createContent(
                                new FileModel(
                                        RandomData.getRandomName("File"), FileType.TEXT_PLAIN));
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that site manager cannot read file via IMAP client if it is locked by"
                            + " an user in repository")
    @Test(
            groups = {TestGroup.PROTOCOLS, TestGroup.IMAP, TestGroup.FULL},

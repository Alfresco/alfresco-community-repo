...
public class PublicApiAlfrescoCmisService extends AlfrescoCmisServiceImpl {
    ...
    public PublicApiAlfrescoCmisService(
    ...
    public List<RepositoryInfo> getRepositoryInfos(ExtensionsData extension) {
        ...
        PagingResults<Network> networks =
                networksService.getNetworks(new PagingRequest(0, Integer.MAX_VALUE));
    ...
    public RepositoryInfo getRepositoryInfo(String repositoryId, ExtensionsData extension) {
        ...
        try {
            ...
            // ACE-2540: Avoid information leak. Same response if repository does not exist or if
    ...
    private RepositoryInfo getRepositoryInfo(final Network network) {
        ...
        final String tenantDomain =
                (networkId.equals(TenantUtil.SYSTEM_TENANT)
                                || networkId.equals(TenantUtil.DEFAULT_TENANT))
                        ? TenantService.DEFAULT_DOMAIN
                        : networkId;

        return TenantUtil.runAsSystemTenant(
                new TenantRunAsWork<RepositoryInfo>() {
                    public RepositoryInfo doWork() {
                        RepositoryInfoImpl repoInfo =
                                (RepositoryInfoImpl)
                                        connector.getRepositoryInfo(getContext().getCmisVersion());

                        repoInfo.setId(
                                !networkId.equals("") ? networkId : TenantUtil.SYSTEM_TENANT);
    ...
    public void checkRepositoryId(String repositoryId) {
        if (repositoryId.equals(TenantUtil.DEFAULT_TENANT)
                || repositoryId.equals(TenantUtil.SYSTEM_TENANT)) {
            // TODO check for super admin
            return;
        }

        if (!tenantAdminService.existsTenant(repositoryId)
                || !tenantAdminService.isEnabledTenant(repositoryId)) {
            throw new CmisObjectNotFoundException("Unknown repository '" + repositoryId + "'!");
        }
    }
    ...
    public void beforeCall() {
        // NOTE: Don't invoke super beforeCall to exclude authentication which is already supported
        // by
        //       Web Script F/W
        // super.beforeCall();
    }
    ...
    public void afterCall() {
        // NOTE: Don't invoke super afterCall to exclude authentication which is already supported
        // by
        //       Web Script F/W
        // super.afterCall();
    }

...
public class ExecuteAllRulesActionExecuterTest extends BaseSpringTest
{
    ...
    /**
     * ALF-17517: if inbound "Add classifiable Aspect" rule is applied on folder test that no InvalidNodeRefException is thrown during Checkout/Checkin for the document in folder. Such case can happen during revert to older document version.
     */
     ...
     * <p>
     * We then explicitly execute folderA's rules, including for subfolders. Ultimately, we want to know that fileB did indeed have the ruleset executed on it, and therefore gained the classifiable aspect.
     */
    public void testRulesFireOnNonInheritedFolderUsingLinkedRuleSetInstead()
    {
        ...
        final NodeRef folderA = fileFolderService.create(rootFolder, "folderA", ContentModel.TYPE_FOLDER).getNodeRef();
        ...
        final NodeRef folderB = fileFolderService.create(folderA, "folderB", ContentModel.TYPE_FOLDER).getNodeRef();
     ...
     * <p>
     * When we use the ExecuteAllRulesActionExecuter to explicitly (e.g. "manually") execute all the rules on a folder and its subfolders, the rule must correctly recurse into those subfolders. This is a more simple test case than the steps described in the above issue, but is crucial to the more complicated case succeeding.
     */

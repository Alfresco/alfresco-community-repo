 ...
 *
 * <p>Acts as an adaptor between JLAN's configuration requirements and the spring configuration of
 * the Alfresco filesystem subsystem.
 *
 * <p>Also contains an amount of initialisation logic.
 *
 * @author gkspencer
 * @author dward
 * @author mrogers
...
public class ServerConfigurationBean extends AbstractServerConfigurationBean
    ...
    protected void processFTPServerConfig() {
        ...
        try {
            ...
            if (bindText != null && bindText.length() > 0 && !bindText.equals(BIND_TO_IGNORE)) {
                ...
                try {
                    ...
                    throw new AlfrescoRuntimeException(
                            "Unable to find FTP bindto address, " + bindText, ex);
                }
            }
            ...
            if (flags != null) {
                ...
                while (token.hasMoreTokens()) {
                    ...
                    while (idx < m_ftpDebugStr.length
                            && m_ftpDebugStr[idx].equalsIgnoreCase(dbg) == false) idx++;
            ...
            } else throw new AlfrescoRuntimeException("FTP authenticator not specified");
            ...
            if (ftpConfigBean.getKeyStorePath() != null
                ...
                if (ftpConfigBean.getKeyStoreType() != null
                    ...
                    try {
                        ...
                        throw new InvalidConfigurationException(
                                "FTPS key store type is invalid, " + keyStoreType, ex);
                    }
            ...
            if (ftpConfigBean.getTrustStorePath() != null
                ...
                if (ftpConfigBean.getTrustStoreType() != null
            ...
            // MNT-7301 FTPS server requires unnecessarly to have a trustStore while a keyStore
    ...
    protected void processFilesystemsConfig() {
        ...
        SecurityConfigSection secConfig =
                (SecurityConfigSection) getConfigSection(SecurityConfigSection.SectionName);
                ...
                try {
                    ...
                    if (filesysContext.hasStateCache()) {
                        ...
                        fsysConfig.addFileStateCache(
                                filesystem.getDeviceName(), filesysContext.getStateCache());

                        // Create the lock manager for the context.
                        FileStateLockManager lockMgr =
                                new FileStateLockManager(filesysContext.getStateCache());
                    ...
                    filesys =
                            new DiskSharedDevice(
                                    filesystem.getDeviceName(), filesysDriver, filesysContext);
                    ...
                    throw new AlfrescoRuntimeException(
                            "Error creating filesystem " + filesystem.getDeviceName(), ex);
                } catch (InvalidConfigurationException ex) {
    ...
    /**
     * Returns true if either: the disk interface is a ContentDiskDriver2; or the disk interface is
     * a {@link BufferedContentDiskDriver} and its disk interface is a ContentDiskDriver2 (wrapped
     * by several other DiskInterface objects).
     *
    ...
    protected void processCoreServerConfig() throws InvalidConfigurationException {
        ...
        if (coreServerConfigBean.getThreadPoolDebug()) coreConfig.getThreadPool().setDebug(true);
        ...
        if (packetSizes != null) {
            ...
            for (MemoryPacketConfigBean curChild : packetSizes) {
                ...
                if (elemIdx > 0 && pktSizes[elemIdx - 1] >= pktSize)
                    throw new InvalidConfigurationException(
                            "Invalid packet size specified, less than/equal to previous packet"
                                    + " size");
     ...
     *
     * <p>TODO - what about desktop actions etc?
     *
        ...
        if (diskCtx.getStateCache() == null) {
            ...
            FilesystemsConfigSection filesysConfig =
                    (FilesystemsConfigSection)
                            this.getConfigSection(FilesystemsConfigSection.SectionName);
            ...
            if (filesysConfig != null) {
                ...
                try {
                    ...
                    filesysConfig.addFileStateCache(
                            diskCtx.getShareName(), diskCtx.getStateCache());

                    FileStateLockManager lockMgr =
                            new FileStateLockManager(diskCtx.getStateCache());

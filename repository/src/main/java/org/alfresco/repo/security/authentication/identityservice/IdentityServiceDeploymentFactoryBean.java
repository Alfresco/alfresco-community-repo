...
public class IdentityServiceDeploymentFactoryBean implements FactoryBean<KeycloakDeployment> {
    ...
    public KeycloakDeployment getObject() throws Exception {
        ...
        // Set client with custom timeout values if client was created by the
        // KeycloakDeploymentBuilder.
        // This can be removed if the future versions of Keycloak accept timeout values through the
        // config.
        if (deployment.getClient() != null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Created HttpClient for Keycloak deployment with connection timeout: "
                                + connectionTimeout
                                + " ms, socket timeout: "
                                + socketTimeout
                                + " ms.");

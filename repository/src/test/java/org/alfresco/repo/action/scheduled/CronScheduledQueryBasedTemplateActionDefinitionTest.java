...
/**
 * Test for {@link CronScheduledQueryBasedTemplateActionDefinition} class. The test assumes that not
 * all test-cases require scheduling. Hence, {@link CronScheduledQueryBasedTemplateActionDefinition}
 * instance is not registered as scheduled job. But in the same time this instance is unregistered
 * as scheduled job after every test-case execution. Also default query template is not set for
 * {@link CronScheduledQueryBasedTemplateActionDefinition} instance
 *
...
public class CronScheduledQueryBasedTemplateActionDefinitionTest extends TestCase {
    ...
    private static final String SCRIPT_TEST_ACTION_NAME =
            "scriptTestActionName-" + System.currentTimeMillis();
    ...
    private static final String SCHEDULER_TRIGGER_NAME =
            "triggerTestName-" + System.currentTimeMillis();

    /** {@link CronScheduledQueryBasedTemplateActionDefinition#setTriggerGroup(String)} */
    private static final String SCHEDULER_TRIGGER_GROUP =
            "triggerTestGroup-" + System.currentTimeMillis();
    ...
    private static final String FRESH_TEST_DOCUMENT_NAME_TEMPLATE =
            "Fresh" + TEST_DOCUMENT_NAME_TEMPLATE;

    private static final String YESTERDAY_TEST_DOCUMENT_NAME_TEMPLATE =
            "Yesterday" + TEST_DOCUMENT_NAME_TEMPLATE;

    private static final String MNT_11598_QUERY_TEMPLATE =
            "@cm\\:created:\\$\\{luceneDateRange(\"%s\", \"-P10Y\")\\}";

    private static final String TEST_FOLDER_NAME =
            String.format(ROOT_TEST_FOLDER_NAME_TEMPLATE, System.currentTimeMillis());
    ...
    /**
     * Initializes target {@link CronScheduledQueryBasedTemplateActionDefinition} instance for
     * testing. <b>Default query template is not set!</b> It must be set in test method. No need to
     * register for every test for scheduling. Hence, {@link
     * CronScheduledQueryBasedTemplateActionDefinition#afterPropertiesSet()} is omitted here and
     * must be invoked for test which requires scheduling
     */
    private void initializeScheduler() {
        Scheduler factory = (Scheduler) applicationContext.getBean(SCHEDULER_FACTORY_BEAN_NAME);

        FreeMarkerWithLuceneExtensionsModelFactory templateActionModelFactory =
                new FreeMarkerWithLuceneExtensionsModelFactory();
        templateActionModelFactory.setServiceRegistry(registry);

        SimpleTemplateActionDefinition templateActionDefinition =
                new SimpleTemplateActionDefinition();
        ...
        scheduler.setStores(
                Collections.singletonList(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE.toString()));
    ...
    /**
     * Creates <code>rootTestFolder</code> test folder as start of test content hierarchy. Then it
     * creates {@link CronScheduledQueryBasedTemplateActionDefinitionTest#TEST_DOCUMENTS_AMOUNT}
     * documents in the root folder with "today" creation date and the same amount of documents with
     * "yesterday" creation date
     */
    private void createTestContent() throws Exception {
        ...
        NodeRef companyHomeNodeRef =
                registry.getNodeLocatorService().getNode(CompanyHomeNodeLocator.NAME, null, null);
        rootTestFolder =
                fileFolderService
                        .create(companyHomeNodeRef, TEST_FOLDER_NAME, ContentModel.TYPE_FOLDER)
                        .getNodeRef();

        for (int i = 0; i < TEST_DOCUMENTS_AMOUNT; i++) {
            String freshDocName =
                    String.format(FRESH_TEST_DOCUMENT_NAME_TEMPLATE, i, System.currentTimeMillis());
            FileInfo fileInfo =
                    fileFolderService.create(
                            rootTestFolder, freshDocName, ContentModel.TYPE_CONTENT);
        ...
        BehaviourFilter policyBehaviourFilter =
                (BehaviourFilter) applicationContext.getBean(POLICY_BEHAVIOUR_FILTER_BEAN_NAME);
        policyBehaviourFilter.disableBehaviour(ContentModel.ASPECT_AUDITABLE);
        try {
            for (int i = 0; i < TEST_DOCUMENTS_AMOUNT; i++) {
                String yesterdayDocName =
                        String.format(
                                YESTERDAY_TEST_DOCUMENT_NAME_TEMPLATE,
                                i,
                                System.currentTimeMillis());
                FileInfo fileInfo =
                        fileFolderService.create(
                                rootTestFolder, yesterdayDocName, ContentModel.TYPE_CONTENT);
                nodeService.setProperty(
                        fileInfo.getNodeRef(), ContentModel.PROP_CREATED, yesterdayTime);
    ...
    public void testQueryTemplateFunctionsUnescapingMnt11598() throws Exception {
        ...
        assertTrue(
                ("Result set must contain at least " + TEST_DOCUMENTS_AMOUNT + " nodes!"),
                actualNodes.size() >= yesterdayNodes.size());

        for (FileInfo fileInfo : freshNodes) {
            assertFalse(
                    "No one of the nodes created \"today\" is expected in result set!",
                    actualNodes.contains(fileInfo.getNodeRef()));
        }

        for (FileInfo fileInfo : yesterdayNodes) {
            assertTrue(
                    "One of the nodes created \"yesteday\" is missing in result set!",
                    actualNodes.contains(fileInfo.getNodeRef()));
        }
    }
}

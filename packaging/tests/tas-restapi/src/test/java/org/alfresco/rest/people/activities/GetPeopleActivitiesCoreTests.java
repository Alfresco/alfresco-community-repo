...
public class GetPeopleActivitiesCoreTests extends RestTest {
  ...
  @TestRail(
    ...
    description = "Verify user cannot get activities for inexistent user with Rest API and response is 404"
  )
  public void userCannotGetPeopleActivitiesForInexistentPersonId()
    throws Exception {
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.ACTIVITIES },
    executionType = ExecutionType.REGRESSION,
    description = "Verify user gets its activities for inexistent siteId with Rest API and response is 404"
  )
  public void userGetItsPeopleActivitiesForInexistentSite() throws Exception {
    restActivityModelsCollection =
      restClient
        ...
        .withCoreAPI()
        .usingUser(userModel)
        .usingParams(String.format("siteId=inexistent"))
        .getPersonActivities();
    restClient
      .assertStatusCodeIs(HttpStatus.NOT_FOUND)
      .assertLastError()
      .containsErrorKey(RestErrorModel.ENTITY_NOT_FOUND_ERRORKEY)
      .containsSummary(
        String.format(RestErrorModel.ENTITY_NOT_FOUND, "inexistent")
      )
      .descriptionURLIs(RestErrorModel.RESTAPIEXPLORER)
      .stackTraceIs(RestErrorModel.STACKTRACE);
  }
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.ACTIVITIES },
    executionType = ExecutionType.REGRESSION,
    description = "Verify user gets activities with invald skipCount parameter with Rest API and response is 400"
  )
  public void userGetPeopleActivitiesUsingInvalidSkipCountParameter()
    throws Exception {
    restActivityModelsCollection =
      restClient
        .authenticateUser(userModel)
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.ACTIVITIES },
    executionType = ExecutionType.REGRESSION,
    description = "Verify user gets activities with invalid maxItems parameter with Rest API and response is 400"
  )
  public void userGetPeopleActivitiesUsingInvalidMaxItemsParameter()
    throws Exception {
    restActivityModelsCollection =
      restClient
        .authenticateUser(userModel)
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.ACTIVITIES },
    executionType = ExecutionType.REGRESSION,
    description = "Verify user gets activities using invalid value for parameter 'who'with Rest API and response is 400"
  )
  public void userGetsPeopleActivitiesUsingInvalidValueForWhoParameter()
    throws Exception {
    restActivityModelsCollection =
      restClient
        .authenticateUser(userModel)
    ...
    restClient
      .assertStatusCodeIs(HttpStatus.BAD_REQUEST)
      .assertLastError()
      .containsErrorKey(RestErrorModel.INVALID_PARAMETER_WHO)
  ...
  @TestRail(
    section = { TestGroup.REST_API, TestGroup.PEOPLE, TestGroup.ACTIVITIES },
    executionType = ExecutionType.REGRESSION,
    description = "Verify user gets activities successfully using parameter 'who' with 'others' value with Rest API"
  )

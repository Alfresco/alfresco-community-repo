...
public class UpdateRecordsTests extends BaseRMRestTest {
    ...
    public void preconditionUpdateRecordsTests() {
        ...
        // Grant updateUser Filing privileges on root category, this will be inherited to record
        // folders
        updateUser =
                roleService.createUserWithRMRoleAndCategoryPermission(
    ...
    /**
     * Incomplete electronic and non electronic records created in one record folder, unfiled
     * records container and one unfiled record folder
     */
    ...
    public Object[][] getIncompleteRecords() {
        ...
        String recordFolderId =
                createRecordFolder(rootCategory.getId(), getRandomName("recFolder1")).getId();
        ...
        UnfiledContainerChild electronicRecord1 =
        ...
        UnfiledContainerChild nonElectronicRecord1 =
        ...
        UnfiledContainerChild electronicRecord2 =
        ...
        UnfiledContainerChild nonElectronicRecord2 =
    ...
    /**
     * Complete electronic and non electronic records created in one record folder, unfiled records
     * container and one unfiled record folder
     */
    @DataProvider(name = "completeRecords")
    public Object[][] getCompleteRecords() {
        // create electronic and nonElectronic record in record folder
        String recordFolderId =
                createRecordFolder(rootCategory.getId(), getRandomName("recFolder2")).getId();
        RecordFolderAPI recordFolderAPI = getRestAPIFactory().getRecordFolderAPI();
    ...
    public void incompleteRecordsCanBeUpdated(String recordId) {
        ...
        recordsAPI.updateRecord(
                createRecordModel(newName, newDescription, newTitle), record.getId());
        assertStatusCode(OK);
    ...
    @Test(
            description =
                    "User with Edit Metadata capabilities can update incomplete record's metadata")
    @AlfrescoTest(jira = "RM-4362")
    public void userWithEditMetadataCapsCanUpdateMetadata() {
        // Create random folder
        RecordCategoryChild recFolder =
                createRecordFolder(rootCategory.getId(), getRandomName("recFolder"));

        // Create electronic and non-electronic records in a folder
        RecordFolderAPI recordFolderAPI = getRestAPIFactory().getRecordFolderAPI();
        Record electronicRecord =
                recordFolderAPI.createRecord(
                        createElectronicRecordModel(), recFolder.getId(), getFile(IMAGE_FILE));
        assertStatusCode(CREATED);
        Record nonElectronicRecord =
                recordFolderAPI.createRecord(createNonElectronicRecordModel(), recFolder.getId());
        assertStatusCode(CREATED);
        ...
        for (Record record : asList(electronicRecord, nonElectronicRecord)) {
            ...
            String newDescription =
                    getModifiedPropertyValue(record.getProperties().getDescription());
    ...
    @AlfrescoTest(jira = "RM-4362")
    @Bug(id = "APPS-132")
    public void completeRecordsCantBeUpdated(
            String electronicRecordId, String nonElectronicRecordId) {
        // Get the recordsAPI
        RecordsAPI recordsAPI = getRestAPIFactory().getRecordsAPI();
        ...
        for (Record record : asList(electronicRecord, nonElectronicRecord)) {
            ...
            assertEquals(
                    updatedRecord.getProperties().getTitle(), record.getProperties().getTitle());
            assertEquals(
                    updatedRecord.getProperties().getDescription(),
                    record.getProperties().getDescription());
        }
    }
    ...
    public void electronicRecordMandatoryAspectsCannotBeRemoved() {
        final List<String> expectedAspects =
                asList(
                        "rma:record",
        ...
        // Create random folder
        String recordFolderId =
                createRecordFolder(rootCategory.getId(), getRandomName("recordFolder")).getId();
        ...
        // Create an electronic record in the unfiled record folder and check it has all the records
        // aspects
        UnfiledContainerChild unfiledRecordModel =
        ...
        UnfiledContainerChild unfiledRecord =
                getRestAPIFactory()
                        ...
                        .createUnfiledRecordFolderChild(
                                unfiledRecordModel, unfiledRecordFolder.getId());

...
public class Duration implements Comparable, Serializable {
  ...
  public static Date add(Date date, Duration duration) {
    ...
    c.add(
      ...
      (duration.m_positive ? 1 : -1) * duration.m_hours
    );
    ...
    c.add(
      ...
      (duration.m_positive ? 1 : -1) * duration.m_nanos / 1000000
    );
    return c.getTime();
  }
          ...
          if (token == StreamTokenizer.TT_NUMBER) {
            ...
            if (tok.ttype == '-') {
              ...
              if (nval != null) {
                ...
                m_nanos =
                  (int) ((long) (nval.doubleValue() * 1000000000) % 1000000000);
              } else {
                throw new RuntimeException(
                  "IO Error parsing Duration: " + duration
                );
  ...
  public Duration(Date start_in, Date end_in) {
    ...
    int millis =
      cend.get(Calendar.MILLISECOND) - cstart.get(Calendar.MILLISECOND);
    if (millis < 0) {
    ...
    int hours =
      cend.get(Calendar.HOUR_OF_DAY) - cstart.get(Calendar.HOUR_OF_DAY);
    if (hours < 0) {
    ...
    int days =
      cend.get(Calendar.DAY_OF_MONTH) - cstart.get(Calendar.DAY_OF_MONTH);
    if (days < 0) {
  ...
  public Duration(
    ...
    if ((months < 0) && (seconds + nanos < 0)) {
      ...
      throw new RuntimeException(
        "Can not convert to period - incompatible signs for year_to_momth and day_to_second elements"
      );
    } else if ((months < 0) && (seconds + nanos > 0)) {
      throw new RuntimeException(
        "Can not convert to period - incompatible signs for year_to_momth and day_to_second elements"
  ...
  public boolean hasTime() {
    return (
      (m_hours != 0) || (m_mins != 0) || (m_seconds != 0) || (m_nanos != 0)
    );
  }
  ...
  public int compareTo(Object o) {
    if (!(o instanceof Duration)) {
      throw new RuntimeException(
        "Can not compare Duration and " + o.getClass().getName()
      );
    }
    ...
    if (this.getTotalMonths() != d.getTotalMonths()) {
      return (
        (m_positive ? 1 : -1) *
        ((int) (this.getTotalMonths() - d.getTotalMonths()))
      );
    } else if (this.getTotalSeconds() != d.getTotalSeconds()) {
      return (
        (m_positive ? 1 : -1) *
        ((int) (this.getTotalSeconds() - d.getTotalSeconds()))
      );
    } else if (this.getTotalNanos() != d.getTotalNanos()) {
      return (
        (m_positive ? 1 : -1) *
        ((int) (this.getTotalNanos() - d.getTotalNanos()))
      );
    } else {
      return 0;
    }
  ...
  public String toString() {
    ...
    if (hasTime()) {
      ...
      if ((m_seconds != 0) || (m_nanos != 0)) {
        ...
        a =
          a.add(
            b.divide(new BigDecimal(1000000000), 9, BigDecimal.ROUND_HALF_EVEN)
          );

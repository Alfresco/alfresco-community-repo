...
public class QNameTypeEditorTest {
  ...
  private static final String[] CONFIG_LOCATIONS = ArrayUtils.add(
  ...
  public void testPopulateBeanWithNamespace() throws Exception {
    QNameContainer qNameContainer = (QNameContainer) applicationContext.getBean(
      "qNameContainerWithNamespace"
    );

    QName expectedQName = QName.createQName(
      "http://www.alfresco.org/model/content/1.0",
      "namespacedName"
    );
    assertEquals(
      "Loading String as QName failed.",
      expectedQName,
      qNameContainer.getQName()
    );
  }

...
/**
 * This interceptor gathers basic method call statistics and calls the {@link
 * org.alfresco.util.resource.MethodResourceManager resource managers} for further processing. The
 * resource managers are called <i>after</i> the method invocations, but it doesn't matter too much
 * since they are called on a time frequency basis.
 *
 * <p>It acts as a sampling tool, but doesn't make any decisions or take any action with regard
 * system or transaction-related resources. All samples are stored against the current transaction.
 * If there is no current transaction then no action will be taken with respect to the resource
 * management.
 *
 * <p>The default is to activate after 10s and call through every 5s.
 *
 * <p>This class supports both interceptor-based calling as well as manual calling. Long-running
 * processes can call an this manually on every iteration and get the same behaviour of regular
 * calls to the resouce managers.
 *
 * <p>The current thread is marked on first entry and all subsequent nested re-entries will just get
 * passed down to the underlying delegate invocation method.
 *
...
public class SingleEntryTransactionResourceInterceptor implements MethodInterceptor {
    ...
    /**
     * A resource key unique to this interceptor. This avoids clashes with other instances up and
     * down the stack operating in the same transaction.
     */
    ...
    /**
     * Set the minimum number of seconds that a transaction must have been running for before method
     * sampling begins. The interceptor does nothing prior to this.
     *
    ...
    /**
     * Set the approximate time between calls to the {@link #setMethodResourceManagers(List)
     * registered resource managers}. This applies to this instance of the interceptor and <u>not to
     * the transaction</u>. If a single instance of this class is used in multiple places, then the
     * resource managers will still only get called at a steady rate. This is mainly in order to
     * streamline the interception prior to the activation phase, but suits the resource managers
     * since they get given the exact methods that were called anyway.
     *
     * @param resourceManagerCallFrequencyMillis an approximate time between calls to the resource
     *     managers
     */
    ...
    /**
     * An alternative method allowing a manual call to check the resources. This is useful in the
     * cases where long running iterations don't necessarily pass through the necessary API stack,
     * or where the specific resources in hand can't be dealt with blindly before and after resource
     * management. The elapsed time should be that of the iteration within the method (it is assumed
     * that there won't be more than one).
     *
     * <p>If you have a loop in a method that doesn't call anything that can be intercepted and
     * handle safely, then get a pre-configured instance (usually from the application context) and
     * mimic the interceptor call.
     *
     * <p>You should get the <code>Method</code>, which is used for informational purposes, in a
     * single call when you calling code is loaded by the classloader. Introspecting every time you
     * wish to call this method is unnecessary.
     *

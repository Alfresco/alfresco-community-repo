...
public interface RepoUsageComponent {
    ...
    /**
     * Record changes to the restrictions imposed on the repository. These <i>may</i> be cached for
     * fast access. This method should only be called if the {@link #getRestrictions() current
    ...
    /**
     * Force an update of the current repository usage. Usage data will be gathered and stored as
     * required.
     *
     ...
     * @return <tt>true</tt> if the reset succeeded or <tt>false</tt> if some other client was
     *     already performing the same reset
     */
    boolean resetUsage(UsageType usageType);

    /**
     * Get the current repository usage data. This will not trigger an update of the data if it is
     * not available; only pre-loaded data will be used.
     *

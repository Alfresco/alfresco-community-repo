...
public class FeedCleaner implements NodeServicePolicies.BeforeDeleteNodePolicy {
  ...
  private void checkProperties() {
    ...
    if ((maxAgeMins <= 0) && (maxFeedSize <= 0)) {
      logger.warn(
        "Neither maxAgeMins or maxFeedSize set - feeds will not be cleaned"
      );
    }
  }
  ...
  public void init() {
    ...
    deletePersonTransactionListener =
      new FeedCleanerDeletePersonTransactionListener();
   ...
   */
  private int executeWithLock(final AtomicBoolean keepGoing)
    ...
    try {
      ...
      if (maxIdRange > 0 && keepGoing.get()) {
        ...
        if (logger.isTraceEnabled()) {
          logger.trace(
            "Cleaned " +
            maxIdRangeDeletedCount +
            " entries to keep ID range of " +
            maxIdRange +
            "."
          );
        }
      ...
      if (maxFeedSize > 0 && keepGoing.get()) {
        ...
        if (logger.isTraceEnabled()) {
          logger.trace(
            "Selecting user+format feeds exceeding the required maximum of " +
            maxFeedSize +
            " entries."
          );
        }
        List<ActivityFeedEntity> userFeedsTooMany = feedDAO.selectUserFeedsToClean(
          maxFeedSize
        );
        for (ActivityFeedEntity userFeedTooMany : userFeedsTooMany) {
          ...
          if (logger.isTraceEnabled()) {
            logger.trace(
              "Found user activity feed entity: " + userFeedTooMany.toString()
            );
          }
          ...
          if (logger.isTraceEnabled()) {
            logger.trace(
              "Get the feeds to keep for user for all sites, not exluding users."
            );
          }
          List<ActivityFeedEntity> feedsToKeep = feedDAO.selectUserFeedEntries(
            feedUserId,
            null,
            false,
            false,
          ...
          if (logger.isTraceEnabled()) {
            for (ActivityFeedEntity feedToKeep : feedsToKeep) {
              logger.trace(
                "Found user activity feed entity to keep: " +
                feedToKeep.toString()
              );
            }
          }
          ...
          if (feedsToKeep.size() < maxFeedSize) {
            if (logger.isTraceEnabled()) {
              logger.trace(
                "Found less then " +
                maxFeedSize +
                " .The feeds were removed, ignoring."
              );
            }
            continue;
          }
          ...
          if (logger.isTraceEnabled()) {
            logger.trace(
              "Deleting the oldest feed entry: " + oldestFeedEntry.toString()
            );
          }
          int deletedCount = feedDAO.deleteUserFeedEntries(
            feedUserId,
            oldestFeedEntry
          );
          if (logger.isTraceEnabled()) {
        ...
        if (logger.isTraceEnabled()) {
          logger.trace(
            "Selecting site+format feeds exceeding the required maximum of " +
            maxFeedSize +
            " entries."
          );
        }
        List<ActivityFeedEntity> siteFeedsTooMany = feedDAO.selectSiteFeedsToClean(
          maxFeedSize
        );
        for (ActivityFeedEntity siteFeedTooMany : siteFeedsTooMany) {
          ...
          if (logger.isTraceEnabled()) {
            logger.trace(
              "Found site activity feed entity: " + siteFeedTooMany.toString()
            );
          }
          ...
          List<ActivityFeedEntity> feedsToKeep = feedDAO.selectSiteFeedEntries(
            siteId,
            maxFeedSize
          );
          if (logger.isTraceEnabled()) {
            for (ActivityFeedEntity feedToKeep : feedsToKeep) {
              logger.trace(
                "Found site activity feed entity to keep: " +
                feedToKeep.toString()
              );
            }
          }
          ...
          int deletedCount = feedDAO.deleteSiteFeedEntries(
            siteId,
            oldestFeedEntry
          );
          if (logger.isTraceEnabled()) {
            logger.trace(
              "Cleaned " + deletedCount + " entries for site '" + siteId + "'."
            );
          }
          maxSizeDeletedCount += deletedCount;
        }
  ...
  public void beforeDeleteNodePerson(NodeRef personNodeRef) {
    String userId = (String) nodeService.getProperty(
      personNodeRef,
    ...
    Set<String> deletedUserIds = (Set<String>) AlfrescoTransactionSupport.getResource(
      KEY_DELETED_USER_IDS
    );
  ...
  public void beforeDeleteNodeSite(NodeRef siteNodeRef) {
    String siteId = (String) nodeService.getProperty(
      siteNodeRef,
      ContentModel.PROP_NAME
    );

    Set<String> deletedSiteIds = (Set<String>) AlfrescoTransactionSupport.getResource(
      KEY_DELETED_SITE_IDS
    );
  ...
  class FeedCleanerDeleteSiteTransactionListener
    ...
    public void afterCommit() {
      Set<String> deletedSiteIds = TransactionalResourceHelper.getSet(
        KEY_DELETED_SITE_IDS
      );
      if (deletedSiteIds != null) {
              ...
              new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                public Void execute() throws Throwable {
                  try {
                    ...
                    int deletedCnt = feedDAO.deleteSiteFeedEntries(
                      tenantService.getName(siteId)
                    );
                    ...
                    logger.error(
                      "Activities feed cleanup for site '" +
                      siteId +
                      "' failed: ",
                      e
                    );
  ...
  class FeedCleanerDeletePersonTransactionListener
    ...
    @Override
    public void afterCommit() {
      Set<String> deletedUserIds = TransactionalResourceHelper.getSet(
        KEY_DELETED_USER_IDS
      );
      if (deletedUserIds != null) {
              ...
              new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                public Void execute() throws Throwable {
                  try {
                    ...
                    logger.error(
                      "Activities feed cleanup for user '" +
                      userId +
                      "' failed: ",
                      e
                    );

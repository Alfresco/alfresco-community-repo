...
public class NodeWebScripTest extends BaseWebScriptTest {
  ...
  protected void setUp() throws Exception {
    ...
    this.nodeArchiveService =
      (NodeArchiveService) ctx.getBean("nodeArchiveService");
  ...
  private void createUser(final String userName, boolean contributor) {
    ...
    if (contributor) {
      this.siteService.setMembership(
          TEST_SITE_NAME,
          userName,
  ...
  private JSONObject asJSON(Response response) throws Exception {
    ...
    throw new IllegalArgumentException(
      "Expected JSONObject, got " + o + " from " + json
    );
  }
  ...
  public void testFolderCreation() throws Exception {
    ...
    Request req = new Request(
      "POST",
      "/api/node/folder/" +
      testFolder.getStoreRef().getProtocol() +
      "/" +
      testFolder.getStoreRef().getIdentifier() +
      "/" +
      testFolder.getId()
    );
    ...
    assertEquals(
      testNodeName,
      nodeService.getProperty(folder, ContentModel.PROP_TITLE)
    );
    assertEquals(
      null,
      nodeService.getProperty(folder, ContentModel.PROP_DESCRIPTION)
    );

    assertEquals(
      testFolder,
      nodeService.getPrimaryParent(folder).getParentRef()
    );
  ...
  public void testLinkCreation() throws Exception {
    ...
    NodeRef testFolder1 = nodeService
      .createNode(
        siteDocLib,
        ContentModel.ASSOC_CONTAINS,
        QName.createQName("testingLinkCreationFolder1"),
        ContentModel.TYPE_FOLDER,
        testFolderProps
      )
      .getChildRef();
    ...
    NodeRef testFile1 = createNode(
      testFolder1,
      "testingLinkCreationFile1",
      ContentModel.TYPE_CONTENT,
      AuthenticationUtil.getAdminUserName()
    );
    NodeRef testFile2 = createNode(
      testFolder1,
      "testingLinkCreationFile2",
      ContentModel.TYPE_CONTENT,
      AuthenticationUtil.getAdminUserName()
    );
    NodeRef testFile3 = createNode(
      testFolder1,
      "testingLinkCreationFile3",
      ContentModel.TYPE_CONTENT,
      AuthenticationUtil.getAdminUserName()
    );
    ...
    NodeRef testFolder2 = nodeService
      .createNode(
        siteDocLib,
        ContentModel.ASSOC_CONTAINS,
        QName.createQName("testingLinkCreationFolder2"),
        ContentModel.TYPE_FOLDER,
        testFolderProps
      )
      .getChildRef();
    ...
    Request req = new Request(
      "POST",
      CREATE_LINK_API +
      testFile1.getStoreRef().getProtocol() +
      "/" +
      testFile1.getStoreRef().getIdentifier() +
      "/" +
      testFile1.getId()
    );
    jsonReq = new JSONObject();
    ...
    assertEquals(
      true,
      nodeService.hasAspect(testFile1, ApplicationModel.ASPECT_LINKED)
    );
    ...
    req =
      new Request(
        "POST",
        CREATE_LINK_API +
        testFolder2.getStoreRef().getProtocol() +
        "/" +
        testFolder2.getStoreRef().getIdentifier() +
        "/" +
        testFolder2.getId()
      );
    jsonReq = new JSONObject();
    ...
    assertEquals(
      true,
      nodeService.hasAspect(testFolder2, ApplicationModel.ASPECT_LINKED)
    );
    ...
    req =
      new Request(
        "POST",
        CREATE_LINK_API +
        testFolder1.getStoreRef().getProtocol() +
        "/" +
        testFolder1.getStoreRef().getIdentifier() +
        "/" +
        testFolder1.getId()
      );
    jsonReq = new JSONObject();
    ...
    NodeRef site2DocLib = siteService.getContainer(
      TEST_SITE.getShortName(),
      SiteService.DOCUMENT_LIBRARY
    );
    NodeRef testFileSite2 = createNode(
      site2DocLib,
      "testingLinkCreationFileInSite2",
      ContentModel.TYPE_CONTENT,
      AuthenticationUtil.getAdminUserName()
    );
    req =
    ...
    nodeArchiveService.purgeArchivedNode(
      nodeArchiveService.getArchivedNode(siteNodeRef)
    );

    // Links can be created in Shared Files, My Files and Repository
    NodeRef testFile4 = createNode(
      testFolder1,
      "testingLinkCreationFile4",
      ContentModel.TYPE_CONTENT,
      AuthenticationUtil.getAdminUserName()
    );
    req =
      new Request(
        "POST",
        CREATE_LINK_API +
        testFile4.getStoreRef().getProtocol() +
        "/" +
        testFile4.getStoreRef().getIdentifier() +
        "/" +
        testFile4.getId()
      );
    jsonReq = new JSONObject();
    ...
    assertEquals(
      false,
      nodeService.hasAspect(testFile4, ApplicationModel.ASPECT_LINKED)
    );

    AuthenticationUtil.setFullyAuthenticatedUser(USER_ONE);

    //you should not be able to create links for locked nodes but you can delete links
    final NodeRef testFile5 = createNode(
      testFolder1,
      "testingLinkCreationFileWithLock",
      ContentModel.TYPE_CONTENT,
      AuthenticationUtil.getAdminUserName()
    );
    ...
    req =
      new Request(
        "POST",
        CREATE_LINK_API +
        testFile5.getStoreRef().getProtocol() +
        "/" +
        testFile5.getStoreRef().getIdentifier() +
        "/" +
        testFile5.getId()
      );
    jsonReq = new JSONObject();
    ...
    assertEquals(
      true,
      nodeService.hasAspect(testFile5, ApplicationModel.ASPECT_LINKED)
    );
    assertEquals(true, nodeService.exists(file5Link));

    // checkout the node testFile5
    final NodeRef workingCopy = retryingTransactionHelper.doInTransaction(
    ...
    req =
      new Request(
        "POST",
        CREATE_LINK_API +
        workingCopy.getStoreRef().getProtocol() +
        "/" +
        workingCopy.getStoreRef().getIdentifier() +
        "/" +
        workingCopy.getId()
      );
    jsonReq = new JSONObject();
    ...
    assertEquals(
      false,
      nodeService.hasAspect(testFile5, ContentModel.ASPECT_LOCKABLE)
    );

    // test that Consumers can create and delete links if they have only read permission
    NodeRef testFile6 = createNode(
      testFolder1,
      "testingLinkCreationFile6",
      ContentModel.TYPE_CONTENT,
      AuthenticationUtil.getAdminUserName()
    );
    ...
    AuthenticationUtil.setFullyAuthenticatedUser(USER_TWO);
    assertTrue(
      permissionService.hasPermission(testFile6, PermissionService.READ) ==
      AccessStatus.ALLOWED
    );
    assertTrue(
      permissionService.hasPermission(testFile6, PermissionService.WRITE) ==
      AccessStatus.DENIED
    );
    ...
    req =
      new Request(
        "POST",
        CREATE_LINK_API +
        testFile6.getStoreRef().getProtocol() +
        "/" +
        testFile6.getStoreRef().getIdentifier() +
        "/" +
        testFile6.getId()
      );
    jsonReq = new JSONObject();
    ...
    assertEquals(
      true,
      nodeService.hasAspect(testFile6, ApplicationModel.ASPECT_LINKED)
    );
  }

...
public class ImporterActionExecuterTest {
    ...
    public static void setup() throws Exception {
        serviceRegistry =
                ctx.getApplicationContext()
                        .getBean(ServiceRegistry.SERVICE_REGISTRY, ServiceRegistry.class);
        contentService = serviceRegistry.getContentService();
        nodeService = serviceRegistry.getNodeService();
        importerActionExecuter =
                ctx.getApplicationContext()
                        .getBean(ImporterActionExecuter.NAME, ImporterActionExecuter.class);
        ...
        storeRef =
                serviceRegistry
                        ...
                        .doInTransaction(
                                new RetryingTransactionCallback<StoreRef>() {
        ...
        retryingTransactionHelper.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() {
                        ...
                        NodeRef zipFileNodeRef =
                                nodeService
                                        .createNode(
                                                rootNodeRef,
                                                ContentModel.ASSOC_CHILDREN,
                                                QName.createQName(
                                                        "http://www.alfresco.org/test/ImporterActionExecuterTest",
                                                        "testAssocQName1"),
                        ...
                        Action action =
                                createAction(
                                        zipFileNodeRef,
                                        "ImporterActionExecuterTestActionDefinition",
                                        targetFolderNodeRef);
        ...
        retryingTransactionHelper.doInTransaction(
                new RetryingTransactionCallback<Void>() {
                    public Void execute() {
                        ...
                        putContent(
                                zipFileNodeRef,
                                "import-archive-test/folderCmTitledAspectArchive.zip");
                        ...
                        try {
                            ...
                            NodeRef importedFolder =
                                    nodeService.getChildByName(
                                            targetFolderNodeRef,
                                            ContentModel.ASSOC_CONTAINS,
                                            "folderCmTitledAspectArchive");
                            assertNotNull("import action failed", importedFolder);

                            // check if aspect is set
                            boolean hasAspectTitled =
                                    nodeService.hasAspect(
                                            importedFolder, ContentModel.ASPECT_TITLED);
                            assertTrue(
                                    "folder didn't get the cm:titled aspect applied",
                                    hasAspectTitled);

                            // MNT-17017 check ContentModel.PROP_TITLE is not set on the top level
                            // folder, just like Share
                            String title =
                                    (String)
                                            nodeService.getProperty(
                                                    importedFolder, ContentModel.PROP_TITLE);
    ...
    private void putContent(NodeRef zipFileNodeRef, String resource) {
        ...
        ContentWriter writer =
                contentService.getWriter(zipFileNodeRef, ContentModel.PROP_CONTENT, true);
        writer.setMimetype(MimetypeMap.MIMETYPE_ZIP);
        writer.putContent(file);
    }

    private Action createAction(
            NodeRef nodeRef, String actionDefinitionName, NodeRef targetNodeRef) {

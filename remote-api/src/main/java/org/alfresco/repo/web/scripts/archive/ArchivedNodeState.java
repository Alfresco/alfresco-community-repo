...
public class ArchivedNodeState {
  ...
  public static ArchivedNodeState create(
    ...
    result.archivedDate =
      (Date) properties.get(ContentModel.PROP_ARCHIVED_DATE);
    ...
    if (
      result.archivedBy != null && personService.personExists(result.archivedBy)
    ) {
    ...
    ChildAssociationRef originalParentAssoc = (ChildAssociationRef) properties.get(
    ...
    if (
      serviceRegistry
        ...
        .hasPermission(
          originalParentAssoc.getParentRef(),
      ...
      result.displayPath =
        PathUtil.getDisplayPath(
          nodeService.getPath(originalParentAssoc.getParentRef()),
          true
        );
    } else {
      result.displayPath = "";

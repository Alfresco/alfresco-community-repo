...
/**
 * Creates extension sub classes that are extension API implementors once per extensible-extension
 * point definition.
 *
 * @author Bogdan Horje
 */
public class InstanceExtensionFactory<I extends InstanceExtension<E, T>, T extends Trait, E>
    ...
    public <C extends I> InstanceExtensionFactory(
            Class<C> extensionClass, Class<T> traitAPI, Class<? extends E> extensionAPI) {
        super();
        ...
        if (!extensionAPI.isAssignableFrom(extensionClass)) {
            throw new InvalidExtension(
                    "Extension class "
                            + extensionClass
                            + " is incompatible with extensio API "
                            + extensionAPI);
        }
    }

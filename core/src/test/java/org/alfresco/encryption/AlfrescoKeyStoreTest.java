...
public class AlfrescoKeyStoreTest {
  ...
  public void testSysPropConfig() {
    ...
    System.setProperty(
      keyStoreId + "." + alias1 + "." + "password",
      "aliasPwd1"
    );
    System.setProperty(
      keyStoreId + "." + alias2 + "." + "password",
      "aliasPwd2"
    );

    try {
      AlfrescoKeyStore alfrescoKeyStore = new AlfrescoKeyStoreImpl(
        keyStoreParameters,
        new SpringKeyResourceLoader()
      );
      ...
      Assert.assertNotNull(
        "Failed to retrieve a key from keystore.",
        alfrescoKeyStore.getKey(alias1)
      );
      Assert.assertNotNull(
        "Failed to retrieve a key from keystore.",
        alfrescoKeyStore.getKey(alias2)
      );
    } finally {
  ...
  public void testSysPropConfigWithoutAliases() {
    ...
    try {
      AlfrescoKeyStore keyStore = new AlfrescoKeyStoreImpl(
        keyStoreParameters,
        new SpringKeyResourceLoader()
      );
  ...
  public void testValidateKeysWrongAliasMetadataFile() throws Exception {
    ...
    AlfrescoKeyStoreImpl alfrescoKeyStore = new AlfrescoKeyStoreImpl(
      keyStoreParameters,
      new SpringKeyResourceLoader()
    );
  ...
  public void testValidateKeysWrongAliasSysProps() throws Exception {
    ...
    System.setProperty(
      keyStoreId + "." + "aliases",
      "wrong-alias,another-wrong-alias"
    );
    System.setProperty(keyStoreId + "." + "password", "ksPwd1");
    System.setProperty(
      keyStoreId + "." + alias1 + "." + "password",
  ...
  public void testEmptyKeysSysProps() {
    ...
    System.setProperty(
      keyStoreId + "." + "aliases",
      "empty-alias,another-empty-alias"
    );
    System.setProperty(keyStoreId + "." + "password", "ksPwd1");
  ...
  public void testConfigEmptyKeystore() {
    ...
    keyStoreParameters.setType("JCEKS");
    keyStoreParameters.setProvider("SunJCE");
    keyStoreParameters.setLocation(
      "classpath:non-existing-path/some-keystore.jks"
    );
  ...
  public void testValidateKeysEmptyAliasSysProps() throws Exception {
    ...
    try {
      ...
      alfrescoKeyStore.setValidateKeyChanges(true);
      alfrescoKeyStore.setKeysToValidate(
        Collections.singleton("non-existing-alias")
      );
      alfrescoKeyStore.setEncryptionKeysRegistry(encryptionKeysRegistry);
      alfrescoKeyStore.validateKeys();
    } finally {

...
public class CachingContentStoreTest {
  ...
  public void getWriterWhenCacheOnInbound()
    ...
    ArgumentCaptor<ContentStreamListener> arg = ArgumentCaptor.forClass(
      ContentStreamListener.class
    );
  ...
  public void quotaManagerCanVetoInboundCaching() {
    ...
    assertSame(
      "Should be writing direct to backing store",
      backingStoreWriter,
      returnedWriter
    );
    verify(quota, never()).afterWritingCacheFile(anyLong());
  ...
  public void exceptionRaisedWhenCopyingTempToBackingStoreIsPropogatedCorrectly()
    ...
    doThrow(new RuntimeException())
      .when(bsWriter)
      .putContent(any(ContentReader.class));
  ...
  public void spoofedGetReader() {
    cachingStore = new CachingContentStore(backingStore, cache, true);
    String url = SpoofedTextContentReader.createContentUrl(
      Locale.ENGLISH,
      0L,
      1024L
    );
  ...
  public void getRequestContentDirectUrlUnsupported() {
    try {
      when(
        backingStore.requestContentDirectUrl(
          anyString(),
          eq(true),
          anyString(),
          anyString(),
      ...
      cachingStore.requestContentDirectUrl(
        "url",
        true,
        "someFile",
        "someMimetype",

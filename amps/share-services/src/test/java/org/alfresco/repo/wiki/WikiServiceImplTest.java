...
public class WikiServiceImplTest {
  ...
  private static final String TEST_USER =
    WikiServiceImplTest.class.getSimpleName() + "_testuser";
  ...
  public void createNewEntry() throws Exception {
    ...
    PagingResults<WikiPageInfo> results = WIKI_SERVICE.listWikiPages(
      WIKI_SITE.getShortName(),
    ...
    NodeRef container = PUBLIC_NODE_SERVICE
      .getPrimaryParent(page.getNodeRef())
      .getParentRef();
    ...
    page =
      WIKI_SERVICE.getWikiPage(WIKI_SITE.getShortName(), page.getSystemName());
    assertEquals("Title", page.getTitle());
    assertEquals("This Is Some Content", page.getContents());
    assertEquals(ADMIN_USER, page.getCreator());
    assertEquals(0, page.getTags().size());
  ...
  public void pagesListing() throws Exception {
    ...
    results =
      WIKI_SERVICE.listWikiPages(WIKI_SITE.getShortName(), TEST_USER, paging);
    assertEquals(3, results.getPage().size());
    assertEquals("TitleC", results.getPage().get(0).getTitle());
    assertEquals("TitleB", results.getPage().get(1).getTitle());
    assertEquals("TitleA", results.getPage().get(2).getTitle());
    ...
    results =
      WIKI_SERVICE.listWikiPages(WIKI_SITE.getShortName(), ADMIN_USER, paging);
    assertEquals(1, results.getPage().size());
    assertEquals("TitleD", results.getPage().get(0).getTitle());
    ...
    results =
      WIKI_SERVICE.listWikiPages(ALTERNATE_WIKI_SITE.getShortName(), paging);
    for (WikiPageInfo link : results.getPage()) {
      PUBLIC_NODE_SERVICE.deleteNode(link.getNodeRef());
    }
  }
    ...
    TRANSACTION_HELPER.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
        ...
        public Void execute() throws Throwable {
          assertEquals(
            true,
            SITE_SERVICE.isMember(WIKI_SITE.getShortName(), TEST_USER)
          );
          assertEquals(
            false,
            SITE_SERVICE.isMember(ALTERNATE_WIKI_SITE.getShortName(), TEST_USER)
          );
          return null;
        }
      }
  ...
  private static void createTestSites() throws Exception {
    final WikiServiceImpl privateWikiPageService = (WikiServiceImpl) testContext.getBean(
      "wikiService"
    );

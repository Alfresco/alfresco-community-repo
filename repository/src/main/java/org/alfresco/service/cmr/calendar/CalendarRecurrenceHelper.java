...
/**
 * This class provides helper functions for when working with recurring {@link CalendarEntry}
 * instances. It provides support for working with key parts of the Outlook/SharePoint recurrence
 * rules
 *
 * @author Nick Burch
 * @since 4.0
...
public class CalendarRecurrenceHelper {
    ...
    protected static final Map<String, Integer> d2cd =
    ...
    @SuppressWarnings("serial")
    protected static final Map<Integer, String> WEEK_NUMBER_TO_WEEK_NAME =
            Collections.unmodifiableMap(
    ...
    /**
     * Returns the parsed calendar recurrence rule WARNING - Alfresco use only. Return type will
     * likely shift to a real object in the near future
     */
    ...
    /**
     * Outlook does some crazy stuff, which is only just about permitted by the specification, and
     * is hard to parse, especially for yearly events. Fix these to more normal cases where possible
     *
     * <p>TODO: This method modifies the input, returning the Map perhaps implies a copy is
     * returned. Decide whether this should be of return type 'void', or make a defensive copy
     * before modification.
     */
    protected static Map<String, String> fixOutlookRecurrenceQuirks(Map<String, String> params) {
        if (params.containsKey("FREQ")) {
            ...
            if ("MONTHLY".equals(params.get("FREQ")) && params.get("BYMONTH") != null) {
                ...
                // ALF-18928: The interval is likely present, we should change it from months to
                // years
                // See
                // org.alfresco.module.vti.web.ws.AbstractMeetingFromICalEndpoint.getLastMeeting()
    ...
    /**
     * For the given Calendar Entry, return its subsequent Recurrence on or after the specified
     * date, until the given limit. If it doesn't have any recurrences on or after the start date
     * (either no recurrence rules, or the last recurrence date is before then), null will be
     * returned. (The onOrAfter and until dates are treat as inclusive) If requested, can stop after
     * the first hit
     *
    ...
    public static List<Date> getRecurrencesOnOrAfter(
        ...
        if (lastRecurrence == null && !firstOnly && until == null) {
            logger.info(
                    "No end date set on the recurring event, and no end date "
    ...
    protected static void buildMonthlyRecurrences(
        ...
        if (params.get("BYWEEKDAY") != null
                || params.get("BYWEEKENDDAY") != null
                || params.get("BYANYDAY") != null) {
     ...
     */
    private static void buildWeekdayAndWeekEndRecurence(
            Calendar currentDate,
            List<Date> dates,
            Map<String, String> params,
            Date until,
        ...
        while (!isCurrentDateAfterUntil) {
            ...
            if (currentDate.getTime().before(until)) {
                ...
                // The sequence number for "BYSETPOS" parameter from recurrence rule for current
                // date.
                int dayCount = 0;
                // week day position, e.q.: first, second. third, forth, last. If the weekday
                // position is 'last' the weekDayPos
                ...
                if (weekDayPos > 0) {
                    // Setting the current date to the first day of month
                    currentDate.set(Calendar.DAY_OF_MONTH, firstMonthDay);
                    // Walk forward from the first day of the month to the required day position
                    // according the recurrence
                    // rule, skipping the unnecessary  days. F.ex, if we need only weekdays then
                    // weekends days should be skipped.
    ...
    protected static void buildYearlyRecurrences(
        ...
        // MNT-10006 fix. Added the support for recurrences rule "WEEKDAY", "WEEKEND DAY"
        else if (null != params.get("BYWEEKDAY")
                || null != params.get("BYWEEKENDDAY")
                || null != params.get("BYANYDAY")) {
    ...
    /**
     * Takes you to eg the 2nd Thursday in the month, which may involve going back before the
     * current date
     */

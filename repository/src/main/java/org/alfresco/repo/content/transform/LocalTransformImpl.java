 ...
 * files and returned from T-Engines which are themselves identified by global properties or system
 * properties the match the pattern localTransform.&lt;name>.url. The transforms take place in a
 * separate process (typically a Docker container).
 */
public class LocalTransformImpl extends AbstractLocalTransform {
    ...
    private String getAvailableMessage(boolean available, String suffix) {
        return "Local transformer "
                + name
                + " on "
                + remoteTransformerClient.getBaseUrl()
                + " is "
                + (available ? "" : "not ")
                + "available"
                + (suffix == null ? "." : ": " + suffix);
    ...
    protected void transformImpl(
        ...
        if (transformOptions.containsKey(SOURCE_NODE_REF)
                && transformOptions.get(SOURCE_NODE_REF) == null) {
        ...
        // These 3 values are commonly needed and are always supplied in the TransformRequest
        // (message to the T-Router).
        // The targetExtension is also supplied in the TransformRequest, but in the case of local
        // transforms
        // is added by the remoteTransformerClient.request call for historic reasons, so does not
        ...
        targetExtension =
                AsynchronousExtractor.getExtension(

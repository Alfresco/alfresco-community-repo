...
public class RenditionNodeManager {
  ...
  private static final List<QName> unchangedProperties = Arrays.asList(
    PROP_NODE_REF,
    PROP_STORE_NAME
  );
  ...
  public RenditionNodeManager(
    ...
    if (logger.isDebugEnabled()) {
      ...
      msg
        .append("Creating/updating rendition based on:")
        .append(LINE_BREAK)
        .append("    sourceNode: ")
        .append(sourceNode)
        .append(LINE_BREAK)
  ...
  public ChildAssociationRef findOrCreateRenditionNode() {
    ...
    if (existingLinkedRendition == null) {
      if (logger.isDebugEnabled()) {
        logger.debug(
          "No existing rendition was found to be linked from the source node."
        );
      }
  ...
  private ChildAssociationRef moveOldRendition(QName renditionName) {
    ...
    QName assocName = getAssociationName(
      parent.equals(sourceNode),
      renditionName
    );
    QName assocType = sourceNode.equals(parent)
      ? RenditionModel.ASSOC_RENDITION
      : ContentModel.ASSOC_CONTAINS;
    ChildAssociationRef result = nodeService.moveNode(
  ...
  private void orphanOldRendition(QNamePattern renditionName) {
    ...
    if (parents.size() == 1) {
      ...
      if (parentAssoc.getParentRef().equals(sourceNode)) {
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Orphaning old rendition node " + existingLinkedRendition
          );
        }
  ...
  private boolean isOrphaningRequiredWithoutLog() {
    ...
    if (
      location.getChildRef() != null
    ) return true; else return AbstractRenderingEngine.getParamWithDefault(
  ...
  private boolean isOldRenditionInCorrectLocationWithoutLog() {
    ...
    ChildAssociationRef oldParentAssoc = nodeService.getPrimaryParent(
      existingLinkedRendition
    );
    ...
    if (oldParent.equals(location.getParentRef())) {
      ...
      if (childName == null) return true; else {
        Serializable oldName = nodeService.getProperty(
          existingLinkedRendition,
  ...
  private ChildAssociationRef getSpecifiedRenditionOrCreateNewRendition(
    QName renditionName
  ) {
    ChildAssociationRef result;
    NodeRef destination = location.getChildRef();
    if (destination != null) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Destination was not null. Using primary parent of " + destination
        );
      }
    } else {
      QName nodeType = null;
  ...
  private void checkDestinationNodeIsAcceptable(NodeRef destination) {
    ...
    ChildAssociationRef sourceAssoc = renditionService.getSourceNode(
      destination
    );
    if (!sourceAssoc.getParentRef().equals(sourceNode)) {
      throw new RenditionServiceException(
        "Cannot perform a rendition to an existing rendition node whose source is different."
      );
    }
    if (
      !sourceAssoc.getQName().equals(renditionDefinition.getRenditionName())
    ) {
      throw new RenditionServiceException(
        "Cannot perform a rendition to an existing rendition node whose rendition name is different."
      );
    }
    if (logger.isDebugEnabled()) {
      logger.debug(
   ...
   */
  private ChildAssociationRef createNewRendition(
    QName renditionName,
    QName nodeTypeQName
  ) {
    ...
    QName assocTypeQName = parentIsSource
      ? renditionType
      : ContentModel.ASSOC_CONTAINS;
    ...
    if (parentIsSource == false) {
      ...
      if (logger.isDebugEnabled()) {
        StringBuilder msg = new StringBuilder();
        msg
          .append("Added new rendition node as child of source node ")
          .append(newChild);
        logger.debug(msg.toString());
      }
    }
   ...
   * @return QName
   */
  private QName getAssociationName(
    ...
    String childName = location.getChildName();
    if (
      parentIsSource == false && childName != null && childName.length() > 0
    ) {
      assocName =
  ...
  private NodeRef getExistingRendition() {
    ...
    NodeRef result = (renditionAssoc == null)
      ? null
      : renditionAssoc.getChildRef();
    if (logger.isDebugEnabled()) {
      StringBuilder msg = new StringBuilder();
      msg
        .append("Existing rendition with name ")
  ...
  private void transferNodeProperties(ChildAssociationRef finalRenditionAssoc) {
    ...
    if (logger.isDebugEnabled()) {
      StringBuilder msg = new StringBuilder();
      msg
        .append("Transferring some properties from ")
        .append(tempRenditionNode)
        .append(" to ")
        .append(targetNode);
      logger.debug(msg.toString());
    ...
    for (Entry<QName, Serializable> entry : nodeService
      .getProperties(tempRenditionNode)
      .entrySet()) {
      QName propKey = entry.getKey();
      if (
        unchangedProperties.contains(propKey) ||

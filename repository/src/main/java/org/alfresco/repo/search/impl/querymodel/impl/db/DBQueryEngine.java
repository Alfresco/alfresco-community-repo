...
public class DBQueryEngine implements QueryEngine {
     ...
     */
    @Override
    public QueryEngineResults executeQuery(
            Query query, QueryOptions options, FunctionEvaluationContext functionContext) {
        long start = 0;
        ...
        dbQuery.prepare(
                ...
                null,
                functionContext,
                metadataIndexCheck2.getPatchApplied());
        ...
        if (logger.isDebugEnabled()) {
            ...
            logger.debug(
                    "Selected "
                            + resultSet.length()
                            + " nodes with permission resolution in "
                            + ms
                            + " ms");
        }
    ...
    private ResultSet selectNodesWithPermissions(QueryOptions options, DBQuery dbQuery) {
        ...
        int maxPermsChecks =
                options.getMaxPermissionChecks() < 0
                        ? maxPermissionChecks
                        : options.getMaxPermissionChecks();
        long maxPermCheckTimeMillis =
                options.getMaxPermissionCheckTimeMillis() < 0
        ...
        FilteringResultSet resultSet =
                acceleratedNodeSelection(options, dbQuery, permissionAssessor);

        PagingLuceneResultSet plrs =
                new PagingLuceneResultSet(resultSet, options.getAsSearchParmeters(), nodeService);
    ...
    FilteringResultSet acceleratedNodeSelection(
            QueryOptions options, DBQuery dbQuery, NodePermissionAssessor permissionAssessor) {
        ...
        performTmdqSelect(
                pickQueryTemplate(options, dbQuery),
                dbQuery,
                requiredNodes,
                new ResultHandler<Node>() {
                    @Override
                    ...
                    private boolean shouldCache(
                            QueryOptions options, List<Node> nodes, int requiredNodes) {
                        if (nodes.size() > requiredNodes) {
                            return false;
                        } else {
    ...
    private void performTmdqSelect(
            String statement, DBQuery dbQuery, int requiredNodes, ResultHandler<Node> handler) {
        if (usePagingQuery) {
    ...
    private void performTmdqSelectStreaming(
            String statement, DBQuery dbQuery, ResultHandler<Node> handler) {
        template.select(statement, dbQuery, handler);
    }

    private void performTmdqSelectPaging(
            String statement, DBQuery dbQuery, int requiredNodes, ResultHandler<Node> handler) {
        int batchStart = 0;
    ...
    public void setNodesCache(SimpleCache<Serializable, Serializable> cache) {
        this.nodesCache =
                new EntityLookupCache<>(cache, CACHE_REGION_NODES, new ReadonlyLocalCallbackDAO());
    }

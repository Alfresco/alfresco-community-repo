...
public class AuditRemoveFromHoldTests extends BaseRMRestTest {
    ...
    public void preconditionForAuditRemoveFromHoldTests() {
        ...
        holdsAPI.addItemsToHolds(
                ...
                getAdminUser().getPassword(),
                asList(
                        heldContent.getNodeRefWithoutVersion(),
    ...
    /**
     * Given a document/record/record folder is removed from a hold When I view the audit log Then
     * an entry has been created in the audit log that contains the following: name of the hold name
     * of the document/record/record folder removed user who removed the content date the content
     * was removed path of the node
     */
    ...
    public void removeFromHoldEventIsAudited(String nodeId, String nodeName, String nodePath) {
        ...
        rmAuditService.checkAuditLogForEvent(
                ...
                nodePath,
                asList(
                        ImmutableMap.of("new", "", "previous", nodeName, "name", "Name"),
                        ImmutableMap.of("new", "", "previous", HOLD3, "name", "Hold Name")));
    }

    /**
     * Given an unsuccessful remove from hold action When I view the audit log Then the remove from
     * hold event isn't audited
     */
    @Test
    public void unsuccessfulRemoveFromHoldIsNotAudited() {
        ...
        holdsAPI.removeItemsFromHolds(
                rmManagerNoReadOnHold.getUsername(),
                rmManagerNoReadOnHold.getPassword(),
                SC_INTERNAL_SERVER_ERROR,
                Collections.singletonList(heldRecord.getId()),
                Collections.singletonList(hold1NodeRef));

        STEP(
                "Check the audit log doesn't contain the entry for the unsuccessful remove from"
                        + " hold.");
        assertTrue(
                "The list of events should not contain remove from hold entry ",
                rmAuditService
                        .getAuditEntriesFilteredByEvent(getAdminUser(), REMOVE_FROM_HOLD)
                        .isEmpty());
    }

    /**
     * Given a not empty record folder is removed from a hold When I view the audit log Then only an
     * entry has been created in the audit log for the record folder removed
     */
    @Test
    public void removeFromHoldNotAuditedForRecordFolderChildren() {
        STEP("Create a new record folder with a record inside");
        RecordCategoryChild notEmptyRecFolder =
                createRecordFolder(recordCategory.getId(), PREFIX + "notEmptyRecFolder");
        ...
        holdsAPI.addItemToHold(
                rmAdmin.getUsername(), rmAdmin.getPassword(), notEmptyRecFolder.getId(), HOLD1);

        rmAuditService.clearAuditLog();

        STEP("Remove record folder from hold.");
        holdsAPI.removeItemFromHold(
                rmAdmin.getUsername(), rmAdmin.getPassword(), notEmptyRecFolder.getId(), HOLD1);

        STEP("Get the list of audit entries for the remove from hold event.");
        auditEntries =
                rmAuditService.getAuditEntriesFilteredByEvent(getAdminUser(), REMOVE_FROM_HOLD);
        ...
        assertTrue(
                "The list of events should not contain Remove from Hold entry for the record",
                auditEntries.stream()
                        .noneMatch(entry -> entry.getNodeName().equals(record.getName())));
    }

    /**
     * Given a record folder is removed from multiple holds When I view the audit log Then multiple
     * entries have been created in the audit log for each remove from hold event
     */
    @Test
    public void removeFromHoldIsAuditedInBulkRemoval() {
        ...
        holdsAPI.removeItemsFromHolds(
                rmAdmin.getUsername(),
                rmAdmin.getPassword(),
                Collections.singletonList(heldRecordFolder.getId()),
                asList(HOLD1, HOLD2));
        ...
        assertEquals(
                "The list of events should contain remove from Hold entries for both holds",
                2,
                auditEntries.size());
        assertTrue(
                "The hold name value for the first remove from hold is not audited.",
                auditEntries.stream()
                        .anyMatch(
                                entry ->
                                        entry.getChangedValues()
                                                .contains(
                                                        ImmutableMap.of(
                                                                ...
                                                                "Hold Name"))));
        assertTrue(
                "The hold name value for the second remove from hold is not audited.",
                auditEntries.stream()
                        .anyMatch(
                                entry ->
                                        entry.getChangedValues()
    ...
    /**
     * Given a document/record/record folder is removed from a hold When I view the audit log as an
     * user with no Read permissions over the node Then the remove from hold entry isn't visible
     */
    @Test
    public void removeFromHoldAuditEntryNotVisible() {
        ...
        holdsAPI.addItemToHold(
                rmAdmin.getUsername(),
                rmAdmin.getPassword(),
                heldFile.getNodeRefWithoutVersion(),
                HOLD1);
        ...
        STEP(
                "Check that an user with no Read permissions can't see the entry for the remove"
                        + " from hold event.");
        assertTrue(
                "The list of events should not contain Remove from Hold entry ",
                rmAuditService
                        .getAuditEntriesFilteredByEvent(rmManagerNoReadOnNode, REMOVE_FROM_HOLD)
                        .isEmpty());
    }

    /**
     * Given a document/record/record folder is removed from a hold When I view the audit log as an
     * user with no Read permissions over the hold Then the the hold name is replaced in the remove
     * from hold entry
     */
    @Test
    public void removeFromHoldAuditEntryHoldNameNotVisible() {
        ...
        auditEntries =
                rmAuditService.getAuditEntriesFilteredByEvent(
                        rmManagerNoReadOnHold, REMOVE_FROM_HOLD);

        STEP(
                "Check that an user with no Read permissions can't see the hold name in the remove"
                        + " from hold event.");
        String replacementHoldName = "You don't have permission to view this hold.";
        assertEquals(
                "The list of events should contain the Remove from Hold entry",
                1,
                auditEntries.size());
        assertTrue(
                "The hold name should not be visible in the Remove from Hold entry ",
                auditEntries.stream()
                        .anyMatch(
                                entry ->
                                        entry.getChangedValues()

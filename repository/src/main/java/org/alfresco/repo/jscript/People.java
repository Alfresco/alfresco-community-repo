...
public class People
  ...
  public void afterPropertiesSet() throws Exception {
    Map<String, ValueDeriver<ScriptNode, Boolean>> capabilityTesters = new HashMap<String, ValueDeriver<ScriptNode, Boolean>>(
      5
    );
    ...
    capabilityTesters.put(
      ...
      new ValueDeriver<ScriptNode, Boolean>() {
        public Boolean deriveValue(ScriptNode source) {
          ...
          String sourceUser = (String) source
            .getProperties()
    ...
    this.valueDerivingMapFactory =
      new ValueDerivingMapFactory<ScriptNode, String, Boolean>(
        capabilityTesters
      );
  }
  ...
  public void enableAccount(String userName) {
    if (
      this.authorityService.isAdminAuthority(
  ...
  public void changePassword(String oldPassword, String newPassword) {
    ...
    this.services.getAuthenticationService()
  ...
  public void setPassword(String userName, String password) {
    ...
    MutableAuthenticationService authService =
      this.services.getAuthenticationService();
    if (
      this.authorityService.hasAdminAuthority() &&
  ...
  public Scriptable getPeople(
    ...
    return getPeoplePaging(
      filter,
      new ScriptPagingDetails(maxResults, 0),
      sortBy,
      Boolean.valueOf(sortAsc)
    );
  }
  ...
  public Scriptable getPeoplePaging(
    ...
    List<PersonInfo> persons = getPeopleImpl(
      filter,
      pagingRequest,
      sortBy,
      sortAsc
  ...
  protected List<PersonInfo> getPeopleImpl(
    ...
    if ((filter == null || filter.trim().isEmpty()) || useCQ) {
      ...
      try {
        // FTS
        List<NodeRef> personRefs = getPeopleImplSearch(
          term,
          tokens,
          pagingRequest,
          sortBy,
  ...
  protected List<PersonInfo> getPeopleImplDB(
    ...
    return personService
      .getPeople(filter, filterProps, sort, pagingRequest)
      .getPage();
  }
  ...
  protected List<NodeRef> getPeopleImplSearch(
    ...
    if (tokens.length == 1) {
      ...
      for (String token : tokens) {
        if (!propertySearch && token.indexOf(':') == -1) {
          ...
          query
            .append(token.substring(0, propIndex + 1))
            .append('"')
    ...
    try {
      ...
      personRefs =
        getSortedPeopleObjects(results.getNodeRefs(), sortBy, sortAsc);
      ...
      new Comparator<NodeRef>() {
        ...
        public Serializable getProperty(NodeRef nodeRef) {
          ...
          if ("fullName".equalsIgnoreCase(sortBy)) {
            ...
            result =
              nodeService.getProperty(nodeRef, ContentModel.PROP_JOBTITLE);
          } else if ("email".equalsIgnoreCase(sortBy)) {
            result = nodeService.getProperty(nodeRef, ContentModel.PROP_EMAIL);
          } else if ("usage".equalsIgnoreCase(sortBy)) {
            result =
              nodeService.getProperty(nodeRef, ContentModel.PROP_SIZE_CURRENT);
          } else if ("quota".equalsIgnoreCase(sortBy)) {
            result =
              nodeService.getProperty(nodeRef, ContentModel.PROP_SIZE_QUOTA);
          } else {
            // Default
            result =
  ...
  public ScriptNode getPerson(final String username) {
    ...
    return personRef == null
      ? null
      : new ScriptNode(personRef, services, getScope());
  }
  ...
  public ScriptNode createGroup(ScriptNode parentGroup, String groupName) {
    ...
    String actualName = services
      .getAuthorityService()
      .getName(AuthorityType.GROUP, groupName);
    if (authorityService.authorityExists(actualName) == false) {
      String result = authorityService.createAuthority(
        AuthorityType.GROUP,
        groupName
      );
      if (parentGroup != null) {
        String parentGroupName = (String) parentGroup
          .getProperties()
          .get(ContentModel.PROP_AUTHORITY_NAME);
  ...
  public void addAuthority(ScriptNode parentGroup, ScriptNode authority) {
    ...
    if (
      parentGroup.getQNameType().equals(ContentModel.TYPE_AUTHORITY_CONTAINER)
    ) {
      String parentGroupName = (String) parentGroup
        .getProperties()
        .get(ContentModel.PROP_AUTHORITY_NAME);
      ...
      if (
        authority.getQNameType().equals(ContentModel.TYPE_AUTHORITY_CONTAINER)
      ) {
        authorityName =
          (String) authority
            .getProperties()
        ...
        authorityName =
          (String) authority.getProperties().get(ContentModel.PROP_USERNAME);
      }
  ...
  public Scriptable getMembers(ScriptNode group, boolean recurse) {
    ParameterCheck.mandatory("Group", group);
    Object[] members = getContainedAuthorities(
      group,
      AuthorityType.USER,
      recurse
    );
  ...
  public ScriptableHashMap getImmutableProperties(String username) {
    Set<QName> props = userRegistrySynchronizer.getPersonMappedProperties(
      username
    );
  ...
  private Object[] getContainedAuthorities(
    ...
    if (
      container.getQNameType().equals(ContentModel.TYPE_AUTHORITY_CONTAINER)
    ) {
      String groupName = (String) container
        .getProperties()
        .get(ContentModel.PROP_AUTHORITY_NAME);

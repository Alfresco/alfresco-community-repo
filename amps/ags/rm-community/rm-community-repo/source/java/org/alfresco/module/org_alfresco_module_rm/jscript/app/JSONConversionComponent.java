...
public class JSONConversionComponent
  ...
  private static final String RECORD_CONTRIBUTOR_GROUP_NAME =
    "recordContributorGroupName";
  private static final String IS_VISIBLE_FOR_CURRENT_USER =
   ...
   */
  public void setRecordContributorsGroupName(
    String recordContributorsGroupName
  ) {
  ...
  protected void setRootValues(
    ...
    if (nodeInfo != null) {
      ...
      rootJSONObject.put(
        IS_RECORD_CONTRIBUTOR_GROUP_ENABLED,
        isRecordContributorsGroupEnabled
      );
      rootJSONObject.put(
        RECORD_CONTRIBUTOR_GROUP_NAME,
      ...
      if (
        AccessStatus.ALLOWED.equals(
          capabilityService.getCapabilityAccessState(
            nodeRef,
            ViewRecordsCapability.NAME
          )
        )
      ) {
        // Indicate whether the node is a RM object or not
        boolean isFilePlanComponent = filePlanService.isFilePlanComponent(
          nodeRef
        );
      ...
      if (!CollectionUtils.isEmpty(filePlans)) {
        ...
        if (filePlanNodeRef != null) {
          Set<Role> roles = filePlanRoleService.getRolesByUser(
            filePlanNodeRef,
  ...
  private void checkRmSiteExistence(JSONObject rootJSONObject) {
    if (!getJsonConversionComponentCache().contains(RM_SITE_EXISTS)) {
      ...
      rootJSONObject.put(
        IS_RM_SITE_CREATED,
        getJsonConversionComponentCache().get(RM_SITE_EXISTS)
      );
    }
  }
  ...
  private void addInfo(final FileInfo nodeInfo, JSONObject rootJSONObject) {
    ...
    NodeRef originatingLocation = AuthenticationUtil.runAsSystem(
    ...
    if (originatingLocation != null) {
      ...
      Object[] subPath = ArrayUtils.subarray(
        displayPathElements,
        5,
        displayPathElements.length
      );
      ...
      rootJSONObject.put(
        "originatingLocationPath",
        originatingLocationPath.toString()
      );
    }
  }
   ...
   */
  @SuppressWarnings("unchecked")
  private JSONObject setRmNodeValues(
    final NodeRef nodeRef,
    final boolean useShortQName
  ) {
    ...
    Map<String, Object> values = AuthenticationUtil.runAsSystem(
          ...
          if (filePlan != null) {
            ...
            if (unfiledRecordContainer != null) {
              ...
              result.put(
                ...
                propertiesToJSON(
                  unfiledRecordContainer,
                  nodeService.getProperties(unfiledRecordContainer),
                  useShortQName
                )
              );
              QName type = fileFolderService
    ...
    AuthenticationUtil.runAsSystem(
      (RunAsWork<Void>) () -> {
        ...
        if (dispositionService.getNextDispositionAction(nodeRef) != null) {
          for (EventCompletionDetails details : dispositionService
  ...
  protected JSONObject permissionsToJSON(final NodeRef nodeRef) {
    ...
    if (!filePlanService.isFilePlanComponent(nodeRef)) {
      ...
      if (
        ALLOWED.equals(permissionService.hasPermission(nodeRef, READ_RECORDS))
      ) {
        permissionsJSON.put(
          "inherited",

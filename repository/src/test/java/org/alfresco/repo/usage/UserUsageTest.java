...
public class UserUsageTest extends TestCase {
  ...
  private static final QName customType = QName.createQName(
    "{my.new.model}sop"
  ); // from exampleModel.xml
  ...
  protected void setUp() throws Exception {
    ...
    repoAdminService =
      (RepoAdminService) applicationContext.getBean("repoAdminService");
    ...
    InputStream modelStream = getClass()
      .getClassLoader()
      .getResourceAsStream("tenant/exampleModel.xml");
  ...
  public void testCreateUpdateDeleteInTx() throws Exception {
    ...
    NodeRef content1 = addTextContent(
      folder,
      "text1.txt",
    ...
    NodeRef content2 = addTextContent(
      folder,
      "text2.txt",
    ...
    NodeRef content3 = addTextContent(
      folder,
      "text3.txt",
  ...
  private void restore(NodeRef origfolderOrContentRef) {
    NodeRef archiveRootNode = nodeService.getStoreArchiveNode(
      this.rootNodeRef.getStoreRef()
    );

    NodeRef archiveNodeRef = new NodeRef(
      archiveRootNode.getStoreRef(),
      origfolderOrContentRef.getId()
    );
    nodeService.restoreNode(archiveNodeRef, null, null, null);
  }

  private NodeRef copy(
    NodeRef sourceFolderOrContentRef,
    NodeRef targetFolderRef,
    String newName
  ) throws FileNotFoundException {
    return fileFolderService
      .copy(sourceFolderOrContentRef, targetFolderRef, newName)
      .getNodeRef();
  }

...
/**
 * The one and only implementation of the ContentCache class. Binary content data itself is stored
 * on disk in the location specified by {@link #cacheRoot}.
 *
 * <p>The in-memory lookup table is provided by a SimpleCache implementation.
 *
 * @author Matt Ward
 */
public class ContentCacheImpl implements ContentCache {
    ...
    /**
     * Get the path of a cache file for the given content URL - will return null if there is no
     * entry in the cache for the specified URL.
     *
    ...
    /**
     * Creates a relative path for a new cache file. The path is based upon the current date/time:
     * year/month/day/hour/minute/guid.bin
     *
     * <p>e.g. 2011/12/3/13/55/27d56416-bf9f-4d89-8f9e-e0a52de0a59e.bin
     *
     ...
     *
     * <p>The returned array contains the (numerically sorted) directories first followed by the
     * (unsorted) plain files.
     *
     * @param dir File
     * @return File[]
     */
    ...
    protected static class NumericFileNameComparator implements Comparator<File> {
        ...
        /**
         * If unable to parse a String numerically then Integer.MAX_VALUE is returned. This results
         * in unexpected directories or files in the structure appearing after the expected
         * directories - so the files we know ought to be older will appear first in a sorted
         * collection.
         *
    ...
    /**
     * This FileContentWriter subclass allows for the temp cache file to be renamed to a cache file
     * proper, e.g filename.tmp becomes filename.bin
     *
     * @author Matt Ward
     */

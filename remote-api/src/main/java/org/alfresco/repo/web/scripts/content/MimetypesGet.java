...
public class MimetypesGet extends DeclarativeWebScript {
    ...
    protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
        ...
        List<String> mimetypes = new ArrayList<String>(Arrays.asList(mimetypesA));
    ...
    protected List<String> getTransformersFrom(
            String mimetype, long sourceSize, List<String> allMimetypes) {
        List<String> transforms = new ArrayList<String>();
        for (String toMT : allMimetypes) {
            if (toMT.equals(mimetype)) continue;

            String details = getTransformer(mimetype, sourceSize, toMT);
            if (details != null) transforms.add(toMT + " = " + details);
        }
        return transforms;
    }

    protected List<String> getTransformersTo(
            String mimetype, long sourceSize, List<String> allMimetypes) {
        List<String> transforms = new ArrayList<String>();
        for (String fromMT : allMimetypes) {
            if (fromMT.equals(mimetype)) continue;

            String details = getTransformer(fromMT, sourceSize, mimetype);
            if (details != null) transforms.add(fromMT + " = " + details);
        }
        return transforms;
    }
    ...
    protected String getTransformer(String from, long sourceSize, String to) {
        return localTransformServiceRegistry.findTransformerName(
                from, sourceSize, to, Collections.emptyMap(), null);
    }
}

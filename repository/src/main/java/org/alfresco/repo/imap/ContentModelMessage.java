...
public class ContentModelMessage extends AbstractMimeMessage {
  ...
  private void buildContentModelMessage() throws MessagingException {
    ...
    try {
      prop =
        (prop == null || prop.equals("")) ? messageFileInfo.getName() : prop;
  ...
  private Multipart buildContentModelMultipart() throws MessagingException {
    MimeMultipart rootMultipart = new AlfrescoMimeMultipart(
      "alternative",
      this.messageFileInfo
    );
  ...
  private MimeBodyPart getTextBodyPart(
    String bodyText,
    String subtype,
    String mimeType
  ) throws MessagingException {
    ...
    result.addHeader(
      AlfrescoImapConst.CONTENT_TYPE,
      mimeType + AlfrescoImapConst.CHARSET_UTF8
    );
    result.addHeader(
      AlfrescoImapConst.CONTENT_TRANSFER_ENCODING,
      AlfrescoImapConst.BASE_64_ENCODING
    );
    return result;
  }
  ...
  private InternetAddress[] buildRecipientToAddress() throws AddressException {
    ...
    try {
      ...
      logger.warn(
        String.format("Wrong email address '%s'.", defaultToAddress),
        e
      );
    }
  ...
  private InternetAddress[] buildSenderFromAddress() throws AddressException {
    ...
    try {
      ...
      logger.warn(
        String.format("Wrong email address '%s'.", defaultFromAddress),
        e
      );
    }

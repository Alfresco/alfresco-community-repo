...
public class ComparePropertyValueEvaluator
  ...
  private static Log logger = LogFactory.getLog(
    ComparePropertyValueEvaluator.class
  );
   ...
   * I18N message ID's
   */
  private static final String MSGID_INVALID_OPERATION =
    "compare_property_value_evaluator.invalid_operation";
  private static final String MSGID_NO_CONTENT_PROPERTY =
    "compare_property_value_evaluator.no_content_property";
   ...
   */
  public void setPropertyValueComparators(
    List<PropertyValueComparator> comparators
  ) {
  ...
  protected void addParameterDefinitions(List<ParameterDefinition> paramList) {
    ...
    paramList.add(
      new ParameterDefinitionImpl(
        ...
        true,
        getParamDisplayLabel(PARAM_VALUE)
      )
    );
    paramList.add(
      new ParameterDefinitionImpl(
        ...
        DataTypeDefinition.TEXT,
        false,
        getParamDisplayLabel(PARAM_OPERATION),
        false,
        "ac-compare-operations"
      )
    );
  }
  ...
  public boolean evaluateImpl(
    ...
    if (this.nodeService.exists(actionedUponNodeRef) == true) {
      ...
      QName propertyQName = (QName) ruleCondition.getParameterValue(
        PARAM_PROPERTY
      );
      if (propertyQName == null) {
        if (logger.isWarnEnabled()) logger.warn(
          "ComparePropertyValue - Property is NULL.  Setting to " +
          DEFAULT_PROPERTY
        );
      ...
      Serializable propertyValue =
        this.nodeService.getProperty(actionedUponNodeRef, propertyQName);
      Serializable compareValue = ruleCondition.getParameterValue(PARAM_VALUE);

      // Get the operation
      ComparePropertyValueOperation operation = null;
      String operationString = (String) ruleCondition.getParameterValue(
        PARAM_OPERATION
      );
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Evaluating Property Parameters, propertyQName - [" +
          propertyQName +
          "] getInverted? [" +
          ruleCondition.getInvertCondition() +
          "] operation [" +
          operation +
          "]"
        );
      ...
      if (DataTypeDefinition.CONTENT.equals(propertyTypeQName) == true) {
        ...
        String contentPropertyString = (String) ruleCondition.getParameterValue(
          PARAM_CONTENT_PROPERTY
        );
      ...
      if (propertyValue != null) {
        ...
        PropertyValueComparator comparator =
          this.comparators.get(propertyTypeQName);
        if (comparator != null) {
          ...
          if (propertyDef.isMultiValued()) {
            for (Serializable value : (
              (ArrayList<Serializable>) propertyValue
            )) {
              boolean success = comparator.compare(
                value,
                compareValue,
                operation
              );
          ...
          if (logger.isWarnEnabled()) {
            logger.warn(
              "Comparator not found for property type " + propertyTypeQName
            );
          }
        ...
        if (logger.isInfoEnabled()) {
          logger.info(
            "Condition Comparator encountered null value for property [" +
            propertyTypeQName +
            "]"
          );
        }

...
public class AssocSourceTypeIntegrityEvent extends AbstractIntegrityEvent {
  ...
  private static Log logger = LogFactory.getLog(
    AssocSourceTypeIntegrityEvent.class
  );
  ...
  protected void checkSourceType(
    ...
    if (sourceDef instanceof TypeDefinition) {
      ...
      if (
        !dictionaryService.isSubClass(sourceNodeTypeQName, sourceDef.getName())
      ) {
        IntegrityRecord result = new IntegrityRecord(
      ...
      if (!found) {
        ...
        if (assocDef.isChild()) {
          ...
          if (
            nodeService
              .getTargetAssocs(sourceNodeRef, assocDef.getName())
              .size() ==
            0
          ) {
            // The association does not exist any more

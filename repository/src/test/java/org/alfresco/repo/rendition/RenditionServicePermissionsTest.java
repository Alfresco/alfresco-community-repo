...
public class RenditionServicePermissionsTest {
    ...
    public static RuleChain staticRuleChain =
            RuleChain.outerRule(APP_CONTEXT_INIT).around(TEST_USER1);
    ...
    public RunAsFullyAuthenticatedRule runTestsAsUser =
            new RunAsFullyAuthenticatedRule(AuthenticationUtil.getAdminUserName());

    // Tie them together in a static Rule Chain
    @Rule
    public RuleChain nonStaticRuleChain =
            RuleChain.outerRule(runTestsAsUser).around(testNodes).around(testSites);
    ...
    public static void initStaticData() throws Exception {
        contentService =
                (ContentService) APP_CONTEXT_INIT.getApplicationContext().getBean("ContentService");
        nodeService = (NodeService) APP_CONTEXT_INIT.getApplicationContext().getBean("NodeService");
        permissionService =
                (PermissionService)
                        APP_CONTEXT_INIT.getApplicationContext().getBean("PermissionService");
        renditionService =
                (RenditionService)
                        APP_CONTEXT_INIT.getApplicationContext().getBean("renditionService");
        repositoryHelper =
        ...
        services =
                (ServiceRegistry)
                        APP_CONTEXT_INIT.getApplicationContext().getBean("ServiceRegistry");
        thumbnailRegistry =
                (ThumbnailRegistry)
                        APP_CONTEXT_INIT.getApplicationContext().getBean("thumbnailRegistry");
    ...
    public void initNonStaticData() throws Exception {
        ...
        testFolder =
                testNodes.createFolder(
                        companyHome, testFolderName, AuthenticationUtil.getAdminUserName());
        ...
        // This piece of content is malformed and it will not be possible to create thumbnails from
        // it.
        brokenJpg =
                testNodes.createQuickFileByName(
                        "quickCorrupt.jpg", siteDocLib, AuthenticationUtil.getAdminUserName());
    }

    /**
     * This test method uses the RenditionService to render a test document and place the rendition
     * under a folder for which the user does not have write permissions. This should be allowed as
     * all renditions are performed as system.
     */
    @Test
                ...
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        permissionService.setPermission(
                                testFolder, normalUser, PermissionService.CONSUMER, true);
                ...
                transactionHelper.doInTransaction(
                        new RetryingTransactionHelper.RetryingTransactionCallback<NodeRef>() {
                            public NodeRef execute() throws Throwable {
                                ...
                                logger.debug(
                                        "rendition's primary parent: "
                                                + nodeService.getPrimaryParent(renditionNode));
                                assertEquals(
                                        "The parent node was not correct",
                                        testFolder,
                                        nodeService.getPrimaryParent(renditionNode).getParentRef());
        ...
        transactionHelper.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        final Serializable renditionCreator =
                                nodeService.getProperty(rendition, ContentModel.PROP_CREATOR);
    ...
    /**
     * This test method uses the RenditionService to render a test document and check that the
     * content transformer has access to the original initiating username
     */
    @Test
        ...
        transactionHelper.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        // Restrict write access to the test folder
                        permissionService.setPermission(
                                testFolder, normalUser, PermissionService.COORDINATOR, true);
                            ...
                            public NodeRef execute() throws Throwable {
                                ...
                                RenditionDefinition action =
                                        renditionService.createRenditionDefinition(
                                                renditionName, MockRenderingEngine.NAME);
                                action.setParameterValue(
                                        RenditionService.PARAM_DESTINATION_PATH_TEMPLATE, path);
                                action.setParameterValue(
    ...
    @Test
    public void userWithReadOnlyAccessToNodeShouldNotCauseFailedThumbnailProblems()
            throws Exception {
        final String siteConsumer = testSiteInfo.siteConsumer;

        // Let's trigger the creation of a TEST_THUMBNAIL for the broken JPG node. Previously it was
        // doclib, but the
        // newer RenditionService2 knows how to create that and does not do failure recovery needed
        // by this test.
        // We know this cannot succeed. We also know the user triggering it does not have write
                ...
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        ...
                        ThumbnailDefinition details =
                                thumbnailRegistry.getThumbnailDefinition(TEST_THUMBNAIL);
                        Action action =
                                ThumbnailHelper.createCreateThumbnailAction(details, services);
        ...
        // The node in question should have no thumbnail/rendition. But it should be marked as
        // having had a failed rendition.
        transactionHelper.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        assertTrue(
                                "Expected an empty list of renditions for brokenJpg.",
                                renditionService.getRenditions(brokenJpg).isEmpty());

                        assertTrue(
                                "Expected brokenJpg to have FailedThumbnailSource aspect.",
                                nodeService.hasAspect(
                                        brokenJpg, ContentModel.ASPECT_FAILED_THUMBNAIL_SOURCE));
                        ...
                        assertEquals(
                                "Wrong number of failed thumbnail child nodes.",
                                1,
                                failedThumbnailChildren.size());

                        NodeRef failedThumbnailChildNode =
                                failedThumbnailChildren.get(0).getChildRef();

                        assertEquals(
                                1,
                                nodeService.getProperty(
                                        failedThumbnailChildNode, ContentModel.PROP_FAILURE_COUNT));
     ...
     *
     * <p>The bug is as follows.
     *
     * <ol>
     *   <li>A user creates a node and triggers rendition (thumbnail) creation.
     *   <li>It all succeeds.
     *   <li>Another user (Role SiteColaborator: Only has update, not delete privileges) updates the
     *       content with something that cannot be thumbnailed e.g. corrupt document.
     *   <li>There should be no failures in executing the DeleteRenditionActionExecuter.
     * </ol>
     */
    @Test
    public void userWithoutDeleteAccessToNodeShouldNotCauseFailedThumbnailProblemsOnUpdate()
            throws Exception {
                ...
                transactionHelper.doInTransaction(
                        new RetryingTransactionHelper.RetryingTransactionCallback<NodeRef>() {
                            public NodeRef execute() throws Throwable {
                                ...
                                // This is what ScriptNode.createThumbnail does
                                ThumbnailDefinition details =
                                        thumbnailRegistry.getThumbnailDefinition("doclib");
                                Action action =
                                        ThumbnailHelper.createCreateThumbnailAction(
                                                details, services);

                                // Creation of thumbnail
                                services.getActionService()
                                        .executeAction(action, imgNode, true, false);
                ...
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        final ContentWriter writer =
                                contentService.getWriter(imgNode, ContentModel.PROP_CONTENT, true);
                        final ContentReader reader =
                                contentService.getReader(brokenJpg, ContentModel.PROP_CONTENT);

                        writer.putContent(reader.getContentInputStream());

                        // Simply updating the content like this should trigger rendition updates,
                        // which in this case will fail.
                        return null;
                    }
                });
        ...
        for (int i = 1; i <= numRetries; i++) {
            ...
            // Now to check that the node has no renditions as the previous one should have been
            // deleted.
            Boolean hasNoRenditions =
                    transactionHelper.doInTransaction(

...
public class WebDAVMethodTest {
  ...
  protected void setUpApplicationContext() {
    ...
    this.tenantAdminService =
      (TenantAdminService) appContext.getBean("tenantAdminService");

    // Authenticate as system to create initial test data set
    AuthenticationComponent authenticationComponent = (AuthenticationComponent) appContext.getBean(
  ...
  private void checkLockedNodeTestWork() throws WebDAVServerException {
    ...
    NodeRef rootNodeRef = tenantService.getRootNode(
      nodeService,
      searchService,
      namespaceService,
      rootPath,
  ...
  public void checkLockedNodeTest() throws Exception {
        ...
        public Object doWork() throws Exception {
          if (!tenantAdminService.existsTenant(TEST_TENANT_DOMAIN)) {
            tenantAdminService.createTenant(
              TEST_TENANT_DOMAIN,
              (DEFAULT_ADMIN_PW + " " + TEST_TENANT_DOMAIN).toCharArray(),
              null
            );
          }
          return null;
    ...
    try {
      ...
      fail(
        "Failed to lock and put content as admin with error: " + e.getCause()
      );
    }
    ...
    try {
      ...
      fail(
        "Failed to lock and put content as tenant admin with error: " +
        e.getCause()
      );
    }
  }
  ...
  private void checkLockedNodeTestTenantWork() throws Exception {
    ...
    try {
      ...
      fail(
        "Locked node should be unlocked with correct lockTocken " + e.getCause()
      );
    }
    ...
    try {
      ...
      fail(
        "Locked node should be deleted with correct lockTocken " + e.getCause()
      );
    }
  }
  ...
  public void canGetStatusForAccessDeniedException() {
    ...
    assertStatusCode(
      500,
      "WebDAVFS/100.10.5 (01908000) Darwin/11.4.0 (x86_64)"
    );

    // Other processor architectures, e.g. x86_32 should work too.
    assertStatusCode(
      500,
      "WebDAVFS/100.10.5 (01908000) Darwin/109.6.3 (some_other_processor_arch)"
    );

    // Other clients should give 403.
    assertStatusCode(
      403,
      "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_6; en-us)"
    );
    // Mozilla-based Windows browser.
    assertStatusCode(
      403,
      "Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.2.12)"
    );
    assertStatusCode(
      403,
      "SomeBrowser/1.0 (Macintosh; U; Intel Mac OS X 10_6; en-us)"
    );
    assertStatusCode(
      403,
      "SomeBrowser/1.9.0 (01908000) Darwin/11.4.0 (x86_64)"
    );
  ...
  public void expiryLockTest() {
    ...
    Level repoWebdavSaveLogLevel = Logger
      .getLogger("org.alfresco.repo.webdav")
      .getLevel();
    Logger.getLogger("org.alfresco.repo.webdav").setLevel(Level.ALL);
    Level webdavProtocolSaveLogLevel = Logger
      .getLogger("org.alfresco.webdav.protocol")
      .getLevel();
    ...
    try {
      ...
      NodeRef folderNodeRef = nodeService
        .createNode(
          ...
          Collections.<QName, Serializable>singletonMap(
            ...
            "WebDavMethodExpiryLockTest" + System.currentTimeMillis()
          )
        )
        .getChildRef();
          ...
          new RetryingTransactionHelper.RetryingTransactionCallback<Object>() {
            ...
            public Object execute() throws Throwable {
              try {
                ...
                logger.debug(e);
                Assert.fail(
                  "Document was not locked again, when lock has expired."
                );
              }
              return null;
            }
      ...
      Logger
        .getLogger("org.alfresco.webdav.protocol")
        .setLevel(webdavProtocolSaveLogLevel);
      Logger
        .getLogger("org.alfresco.repo.webdav")
        .setLevel(repoWebdavSaveLogLevel);
    }
  }
  ...
  private void assertStatusCode(int expectedStatusCode, String userAgent) {
    ...
    assertEquals(
      "Incorrect status code for user-agent string \"" + userAgent + "\"",
      expectedStatusCode,
      statusCode
    );
  }

...
public abstract class VersionServiceImpl
  ...
  protected static final String MSGID_ERR_REVERT_MISMATCH =
    "version_service.err_revert_mismatch";
  ...
  public void setVersionComparatorClass(String versionComparatorClass) {
    if (
      versionComparatorClass != null &&
      versionComparatorClass.trim().length() != 0
    ) {
      try {
        versionComparatorDesc =
          (Comparator<Version>) getClass()
            .getClassLoader()
  ...
  public void registerVersionLabelPolicy(
    QName typeQName,
  ...
  public Collection<Version> createVersion(
    ...
    // Create the versions
    Collection<Version> versions = createVersion(
      nodeRef,
      versionProperties,
      versionChildren,
      versionNumber
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(
        ...
        (System.currentTimeMillis() - startTime) +
        " ms " +
        (versionChildren ? "(with " + (versions.size() - 1) + " children)" : "")
      );
    }
  ...
  private Collection<Version> createVersion(
    ...
    if (versionChildren == true) {
      ...
      Collection<ChildAssociationRef> children =
        this.dbNodeService.getChildAssocs(nodeRef);
  ...
  protected Version createVersion(
    ...
    Map<QName, Serializable> versionLabelAsMap = new HashMap<QName, Serializable>(
      1
    );
    versionLabelAsMap.put(
      ContentModel.PROP_VERSION_LABEL,
      version.getVersionLabel()
    );
  ...
  protected Map<String, Serializable> getVersionMetaData(
    NodeRef versionNodeRef
  ) {
    // Get the meta data
    ...
    for (ChildAssociationRef ref : metaData) {
      ...
      String name = (String) this.dbNodeService.getProperty(
          metaDataValue,
          PROP_QNAME_META_DATA_NAME
        );
      Serializable value =
  ...
  private void freezeChildAssociations(
    ...
    for (ChildAssociationRef childAssocRef : childAssociations) {
      ...
      properties.put(
        PROP_QNAME_IS_PRIMARY,
        Boolean.valueOf(childAssocRef.isPrimary())
      );
      properties.put(
        PROP_QNAME_NTH_SIBLING,
        Integer.valueOf(childAssocRef.getNthSibling())
      );
  ...
  protected VersionHistory buildVersionHistory(
    ...
    if (this.nodeService.exists(nodeRef)) {
      ...
      currentVersion =
        VersionUtil.convertNodeRef(
          getLatestVersion(nodeRef).getFrozenStateNodeRef()
        );
    }
  ...
  protected Version getVersion(NodeRef versionRef) {
    ...
    // Get the meta data
    Map<String, Serializable> versionMetaDataProperties = getVersionMetaData(
      versionRef
    );
    ...
    NodeRef newNodeRef = new NodeRef(
      new StoreRef(STORE_PROTOCOL, STORE_ID),
      versionRef.getId()
    );
   ...
   */
  private NodeRef getCurrentVersionNodeRef(
  ...
  public void ensureVersioningEnabled(
    ...
    boolean disableAuditable = policyBehaviourFilter.isEnabled(
    ...
    if (!nodeService.hasAspect(nodeRef, ContentModel.ASPECT_VERSIONABLE)) {
      ...
      AspectDefinition versionable = dictionaryService.getAspect(
        ContentModel.ASPECT_VERSIONABLE
      );
  ...
  public void revert(NodeRef nodeRef, Version version, boolean deep) {
    ...
    if (
      nodeRef
        .getId()
        .equals(version.getVersionProperty(VersionModel.PROP_FROZEN_NODE_ID)) ==
      false
    ) {
    ...
    try {
      ...
      this.nodeService.setProperties(
          nodeRef,
      ...
      for (ChildAssociationRef versionedChild : this.nodeService.getChildAssocs(
          versionNodeRef
        )) {
  ...
  public NodeRef restore(
    ...
    QName type = (QName) version.getVersionProperty(
  ...
  public void deleteVersion(NodeRef nodeRef, Version version) {
    ...
    throw new UnsupportedOperationException(
      "Delete version is unsupported by the old (deprecated) version store implementation"
    );
  }
      ...
      try {
        ...
        for (QName propertyName : propertyDefinitions.keySet()) {
          Serializable propValue =
            this.nodeService.getProperty(nodeRef, propertyName);
          nodeDetails.addProperty(classRef, propertyName, propValue);
        }
        // Also copy the aspect with no properties in its definition
        if (
          classDefinition.isAspect() &&
          !nodeDetails.getAspects().contains(classRef)
        ) {
      ...
      if (classDefinition.isContainer()) {
        ...
        for (ChildAssociationRef childAssocRef : childAssocRefs) {
          if (assocDefs.containsKey(childAssocRef.getTypeQName())) {
            nodeDetails.addChildAssociation(

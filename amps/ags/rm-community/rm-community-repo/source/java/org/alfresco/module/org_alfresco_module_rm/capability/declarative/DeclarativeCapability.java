...
public class DeclarativeCapability extends AbstractCapability {
   ...
   */
  protected boolean checkPermissionsImpl(
    NodeRef nodeRef,
    String... permissions
  ) {
    boolean result = true;
    ...
    for (String permission : permissions) {
      if (
        permissionService.hasPermission(filePlan, permission) !=
  ...
  protected boolean checkConditions(
    ...
    if (conditions != null) {
      for (Map.Entry<String, Boolean> entry : conditions.entrySet()) {
        ...
        CapabilityCondition condition = (CapabilityCondition) applicationContext.getBean(
          conditionName
        );
        if (condition == null) {
  ...
  protected boolean checkKinds(NodeRef nodeRef) {
    ...
    FilePlanComponentKind actualKind = getFilePlanService()
  ...
  public int evaluate(NodeRef nodeRef) {
    ...
    Map<String, Integer> map = TransactionalResourceHelper.getMap(
      "rm.declarativeCapability"
    );
    String key =
      getName() +
      "|" +
    ...
    if (map.containsKey(key)) {
      ...
      if (getFilePlanService().isFilePlanComponent(nodeRef)) {
        ...
        if (
          checkKinds(nodeRef) &&
          checkPermissions(nodeRef) &&
          checkConditions(nodeRef)
        ) {
      ...
      if (
        LOGGER.isDebugEnabled() && AccessDecisionVoter.ACCESS_DENIED == result
      ) {
        LOGGER.debug(
          "Capability " +
          getName() +
  ...
  public int evaluate(NodeRef source, NodeRef target) {
    ...
    if (
      targetCapability != null && result != AccessDecisionVoter.ACCESS_DENIED
    ) {

...
public class RequestInfoAction extends RMActionExecuterAbstractBase {
  ...
  protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
    if (
      ...
      String workflowDefinitionId = workflowService
      ...
      parameters.put(
        ...
        getWorkflowPackage(action, actionedUponNodeRef)
      );
      parameters.put(RMWorkflowModel.RM_MIXED_ASSIGNEES, getAssignees(action));
      parameters.put(
        RMWorkflowModel.RM_REQUESTED_INFORMATION,
        getRequestedInformation(action)
      );
   ...
   */
  private NodeRef getWorkflowPackage(
    Action action,
    ...
    ChildAssociationRef childAssoc = getNodeService()
  ...
  private Serializable getAssignees(Action action) {
    ...
    String assigneesAsString = (String) action.getParameterValue(
      PARAM_ASSIGNEES
    );

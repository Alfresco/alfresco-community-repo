...
public class StandardQuotaStrategyMockTest {
  ...
  public void testCanSetMaxUsageInMB() throws IllegalAccessException {
    ...
    assertEquals(
      0,
      (
        (Long) FieldUtils.readDeclaredField(quota, "maxUsageBytes", true)
      ).longValue()
    );
  ...
  public void testPanicThresholdForBeforeWritingCacheFile() {
    ...
    assertTrue(
      "Should allow writing of cache file",
      quota.beforeWritingCacheFile(899)
    );
    assertFalse(
      "Should not allow writing of cache file",
      quota.beforeWritingCacheFile(900)
    );

    quota.setCurrentUsageBytes(890);
    assertTrue(
      "Should allow writing of cache file",
      quota.beforeWritingCacheFile(9)
    );
    assertFalse(
      "Should not allow writing of cache file",
      quota.beforeWritingCacheFile(10)
    );

    quota.setCurrentUsageBytes(600);
    assertTrue(
      "Should allow writing of cache file",
      quota.beforeWritingCacheFile(299)
    );
    assertFalse(
      "Should not allow writing of cache file",
      quota.beforeWritingCacheFile(300)
    );
    ...
    assertFalse(
      "Should not allow writing of cache file",
      quota.beforeWritingCacheFile(1)
    );
    ...
    assertFalse(
      "Should not allow writing of cache file",
      quota.beforeWritingCacheFile(12300)
    );
  }
  ...
  public void attemptToWriteFileExceedingMaxFileSizeIsVetoed() {
    ...
    assertTrue(
      ...
      quota.beforeWritingCacheFile(20971520)
    );
    ...
    assertFalse(
      "File should be vetoed - too large",
      quota.beforeWritingCacheFile(1048577)
    );
    ...
    assertFalse(
      "File should be vetoed - too large",
      quota.beforeWritingCacheFile(20971521)
    );
  ...
  public void afterFileWrittenExceedingMaxFileSizeFileIsDeleted() {
    ...
    assertFalse(
      ...
      quota.afterWritingCacheFile(20971521)
    );

...
public class WikiRestApiTest extends BaseWebScriptTest {
  ...
  private static final String PAGE_CONTENTS_THREE =
    "http://share.alfresco.com/";
  private static final String PAGE_CONTENTS_LINK =
    "Text text [[TestPageOne|P1]] [[Test_Page_Two|P2]] [[Invalid|Invalid]] text";

  private static final String URL_WIKI_BASE = "/slingshot/wiki/page";
  private static final String URL_WIKI_LIST =
    URL_WIKI_BASE + "s/" + SITE_SHORT_NAME_WIKI;
  private static final String URL_WIKI_FETCH =
    URL_WIKI_BASE + "/" + SITE_SHORT_NAME_WIKI + "/"; // plus title
  private static final String URL_WIKI_UPDATE =
    URL_WIKI_BASE + "/" + SITE_SHORT_NAME_WIKI + "/"; // plus title
  private static final String URL_WIKI_DELETE =
    URL_WIKI_BASE + "/" + SITE_SHORT_NAME_WIKI + "/"; // plus title
  private static final String URL_WIKI_RENAME =
    URL_WIKI_BASE + "/" + SITE_SHORT_NAME_WIKI + "/"; // plus title
  private static final String URL_WIKI_VERSION =
    "/slingshot/wiki/version/" + SITE_SHORT_NAME_WIKI + "/";
  ...
  protected void setUp() throws Exception {
    ...
    this.wikiService =
      (WikiService) getServer().getApplicationContext().getBean("WikiService");
  ...
  private JSONObject getPage(String name, int expectedStatus) throws Exception {
    Response response = sendRequest(
      new GetRequest(URL_WIKI_FETCH + name),
      expectedStatus
    );
   ...
   * Note - not JSON based.
   */
  private String getPageAtVersion(
    String name,
    String version,
    int expectedStatus
  ) throws Exception {
    Response response = sendRequest(
      new GetRequest(URL_WIKI_VERSION + name + "/" + version),
      expectedStatus
    );
    if (expectedStatus == Status.STATUS_OK) {
   ...
   * Deletes the page
   */
  private JSONObject deletePage(String title, int expectedStatus)
    throws Exception {
    String name = title.replace(' ', '_');

    Response response = sendRequest(
      new DeleteRequest(URL_WIKI_DELETE + name),
      expectedStatus
    );
    if (expectedStatus == Status.STATUS_OK) {
      JSONObject result = new JSONObject(response.getContentAsString());
   ...
   */
  private void pushPageCreatedDateBack(String name, int daysAgo)
    throws Exception {
  ...
  public void testRenaming() throws Exception {
    ...
    assertEquals(PAGE_TITLE_TWO, page.getString("title"));
    assertEquals(
      "This page has been moved [[" + PAGE_TITLE_THREE + "|here]].",
      page.getString("pagetext")
    );
  ...
  public void testRenamePageWithEmptyTitle() throws Exception {
    ...
    double vNum = Double.parseDouble(
      versions.getJSONObject(maxVersionIndex).getString("version")
    );
    String newTitle = versions
      .getJSONObject(maxVersionIndex)
      .getString("title");
  ...
  public void testOverallListing() throws Exception {
    ...
    assertEquals(
      "Incorrect JSON: " + pages.toString(),
      true,
      pages.has("totalPages")
    );
    ...
    this.authenticationComponent.setCurrentUser(USER_TWO);
    JSONObject page3 = createOrUpdatePage(
      PAGE_TITLE_THREE,
      PAGE_CONTENTS_THREE,
      null,
      Status.STATUS_OK
    ...
    WikiPageInfo pageTwo = wikiService.getWikiPage(
      SITE_SHORT_NAME_WIKI,
      pageTwoName
    );
    nodeService.setProperty(
  ...
  public void test_MNT11595() throws Exception {
    ...
    try {
      ...
      MutableAuthenticationService mas = (MutableAuthenticationService) getServer()
        .getApplicationContext()
        .getBean("authenticationService");
      ...
      JSONObject resultManagerRole = new JSONObject(
        responseManagerRole.getContentAsString()
      );
      JSONObject permissionsManagerRole = resultManagerRole.getJSONObject(
        "permissions"
      );
      ...
      JSONObject resultConsumerRole = new JSONObject(
        responseConsumerRole.getContentAsString()
      );
      JSONObject permissionsConsumerRole = resultConsumerRole.getJSONObject(
        "permissions"
      );
  ...
  public void testXSSInjection() throws Exception {
    ...
    try {
      ...
      while (m.find()) {
        ...
        WikiPageInfo wikiPage2 =
          this.wikiService.getWikiPage(SITE_SHORT_NAME_WIKI, link);
        WikiPageInfo wikiPage1 =
          this.wikiService.getWikiPage(
              SITE_SHORT_NAME_WIKI,
              StringEscapeUtils.unescapeHtml(link)
            );

...
public class GetCopiesCannedQueryFactory extends AbstractCopyCannedQueryFactory<CopyInfo> {
    ...
    /**
     * Query to find nodes copied <i>from</i> a given node, optionally filtering out based on
     * specific values.
     *
     * @author Derek Hulley
     * @since 4.0
    ...
    private class GetCopiesCannedQuery extends AbstractCannedQueryPermissions<CopyInfo> {
        private GetCopiesCannedQuery(
                CannedQueryParameters parameters, MethodSecurityBean<CopyInfo> methodSecurity) {
        ...
        protected List<CopyInfo> queryAndFilter(CannedQueryParameters parameters) {
            ...
            if (detail.copyParentNodeRef != null) {
                Pair<Long, NodeRef> copyParentNodePair =
                        nodeDAO.getNodePair(detail.copyParentNodeRef);

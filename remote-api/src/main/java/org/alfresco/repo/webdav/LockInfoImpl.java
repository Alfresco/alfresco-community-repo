...
/**
 * Class to represent a WebDAV lock info. Instances of this class are accessible my multiple threads
 * as they are kept in the {@link LockStore}. Clients of this class are expected to synchronise
 * externally using the provided ReentrantReadWriteLock.
 *
    ...
    public void setExclusiveLockToken(String token) {
        if (isShared()) {
            throw new IllegalStateException(
                    "Cannot add exclusive lock token ["
                            + token
                            + "] to shared lock ["
                            + toString()
                            + "]");

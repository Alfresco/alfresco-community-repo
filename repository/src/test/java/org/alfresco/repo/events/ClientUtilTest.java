...
public class ClientUtilTest
{
    ...
    /**
     * If a new client is added to the FileFilterMode.Client then this unit test will throw a IllegalArgument exception. To fix it you will need to add to the org.alfresco.sync.events.Client.ClientType.
     */
    @Test
    ...
    /* @Test public void testClientTypeConversion() { //Loop through all the client types checking they work for (ClientType type : org.alfresco.events.Client.ClientType.values()) { equalsClientTypeConversion(type); } }
     * 
     * 
     * private void equalsClientTypeConversion(ClientType type) { FileFilterMode.Client ffClient = to(Client.asType(type)); org.alfresco.events.Client client = ClientUtil.from(ffClient); assertEquals(type, client.getType()); } */

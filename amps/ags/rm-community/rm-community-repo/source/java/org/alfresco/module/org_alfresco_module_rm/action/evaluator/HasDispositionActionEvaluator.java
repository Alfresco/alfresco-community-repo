...
/**
 * Records management hasDispositionAction evaluator that evaluates whether the given node's
 * disposition schedule has the specified disposition action.
 *
 * @author Craig Tan
 * @since 2.1
 */
public class HasDispositionActionEvaluator
    ...
    protected boolean evaluateImpl(ActionCondition actionCondition, NodeRef actionedUponNodeRef) {
        ...
        String position =
                ((QName)
                                actionCondition.getParameterValue(
                                        PARAM_DISPOSITION_ACTION_RELATIVE_POSITION))
                        .getLocalName();
        String action =
                ((QName) actionCondition.getParameterValue(PARAM_DISPOSITION_ACTION))
                        .getLocalName();
        ...
        if (dispositionService.isDisposableItem(actionedUponNodeRef)) {
            ...
            if (position.equals(DispositionActionRelativePositions.ANY.toString())) {
                ...
                DispositionSchedule dispositionSchedule =
                        dispositionService.getDispositionSchedule(actionedUponNodeRef);
                ...
                DispositionAction nextDispositionAction =
                        dispositionService.getNextDispositionAction(actionedUponNodeRef);
                ...
                DispositionAction lastCompletedDispositionAction =
                        dispositionService.getLastCompletedDispostionAction(actionedUponNodeRef);
    ...
    protected void addParameterDefinitions(List<ParameterDefinition> paramList) {
        paramList.add(
                new ParameterDefinitionImpl(
                        ...
                        true,
                        getParamDisplayLabel(PARAM_DISPOSITION_ACTION_RELATIVE_POSITION),
                        false,
                        "rm-ac-disposition-action-relative-positions"));
        paramList.add(
                new ParameterDefinitionImpl(
                        PARAM_DISPOSITION_ACTION,
                        DataTypeDefinition.QNAME,
                        true,
                        getParamDisplayLabel(PARAM_DISPOSITION_ACTION),
                        false,

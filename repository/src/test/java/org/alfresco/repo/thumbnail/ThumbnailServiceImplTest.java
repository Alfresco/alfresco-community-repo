...
public class ThumbnailServiceImplTest extends BaseAlfrescoSpringTest
{
    ...
    /**
     * From 4.0.1 we support 'transient' thumbnail failure. This occurs when the ContentTransformer cannot attempt to perform the transformation for some reason (e.g. process/service unavailable) and wishes to decline the request. Such 'failures' should not lead to the addition of the {@link ContentModel#ASPECT_FAILED_THUMBNAIL_SOURCE} aspect.
     * 
     * @since 4.0.1
     */
    @Test
    ...
    /**
     * This is a simple log error appender. You can simply add this appender to the root logger e.g. Logger.getRootLogger().addAppender(logErrorAppender);
     *
    ...
    private void checkRenditioned(NodeRef contentNodeRef, List<ExpectedAssoc> expectedAssocs)
    {
        ...
        for (ExpectedAssoc expectedAssoc : expectedAssocs)
        {
            QNamePattern qNamePattern = expectedAssoc.getAssocName() != null
                    ? QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, expectedAssoc.getAssocName())
                    : null;
    ...
    /**
     * This method creates a node under the specified folder whose content is taken from the quick file corresponding to the specified MIME type.
     *
     * @param parentFolder
     * @param mimetype
     * @return
    ...
    public void testThumbnailServiceCreateApi() throws Exception
    {
        ...
        ThumbnailParentAssociationDetails tpad = new ThumbnailParentAssociationDetails(otherFolder,
    ...
    /**
     * This test method tests the thumbnail placeholders which are handled in the {@link ScriptThumbnailService}. See ALF-6566.
     */
    @Test
    ...
    /**
     * Verifies that our long-running test setup passes with simple behavior of a single thumbnail requested with no other concurrent work.
     * 
     * @throws Exception
     */
    @Test
    ...
    /**
     * Verifies that a property can be updated on the source node during a long-running thumbnailing process without affecting the thumbnailing or property update.
     * 
     * @throws Exception
    ...
    /**
     * Defines the work to be done while long running transformations are being performed and the means to verify that work completed successfully.
     * <p>

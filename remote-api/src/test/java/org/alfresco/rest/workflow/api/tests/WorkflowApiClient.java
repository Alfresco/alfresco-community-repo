...
public class WorkflowApiClient extends PublicApiClient
{
    ...
    public class DeploymentsClient extends AbstractProxy
    {
        ...
        /* public ListResponse<Deployment> createNewDeployment(String name, byte[] bytes) throws PublicApiException { Part[] parts = new Part[]{ new StringPart("name", name), new FilePart("file", new ByteArrayPartSource("bytes", bytes)) }; RequestEntity requestEntity = new MultipartRequestEntity(parts, null); HttpResponse response = create("deployments", null, null, null, requestEntity, "Failed to create new deployment"); return DeploymentParser.INSTANCE.parseList(response.getJsonResponse()); } */

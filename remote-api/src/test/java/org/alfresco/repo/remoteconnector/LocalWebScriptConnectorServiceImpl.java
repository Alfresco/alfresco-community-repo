...
public class LocalWebScriptConnectorServiceImpl
  ...
  private static Log logger = LogFactory.getLog(
    LocalWebScriptConnectorServiceImpl.class
  );
  ...
  public LocalWebScriptConnectorServiceImpl(BaseWebScriptTest webScriptTest)
    throws Exception {
    ...
    if (url.startsWith(LOCAL_BASE_URL)) {
      ...
      if (url.startsWith(SERVICE_URL)) {
        ...
        throw new IllegalArgumentException(
          "Only /service/ local URLs are supported, can't handle " + url
        );
      }
    } else {
  ...
  public RemoteConnectorRequest buildRequest(
    ...
    String methodName;
    try {
      HttpMethodBase httpMethod = method
  ...
  public RemoteConnectorResponse executeRequest(RemoteConnectorRequest request)
    ...
    if (authHeader != null) {
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "HTTP Authorization found for the request, clearing security context, Auth is " +
        authHeader
      );
    }
    ...
    // Log
    if (logger.isInfoEnabled()) logger.info(
      "Response to request was " + resp.getStatus() + " - " + resp
    );
    ...
    if (resp.getStatus() >= 500 && resp.getStatus() <= 599) {
      throw new RemoteConnectorServerException(
        resp.getStatus(),
        "(not available)"
      );
    }
  ...
  private static class WebScriptHelper {
    ...
    private WebScriptHelper(BaseWebScriptTest test) throws Exception {
      ...
      sendRequest =
        BaseWebScriptTest.class.getDeclaredMethod(
            "sendRequest",
            Request.class,
            Integer.TYPE
          );
    ...
    private Response sendRequest(Request request, int expectedStatus)
      throws Exception {
  ...
  private static class LocalAndRemoteAuthenticator
    ...
    private LocalAndRemoteAuthenticator(BaseWebScriptTest test)
      throws Exception {
      // Get the test server
      ...
      httpAuthFactory =
        (BasicHttpAuthenticatorFactory) server

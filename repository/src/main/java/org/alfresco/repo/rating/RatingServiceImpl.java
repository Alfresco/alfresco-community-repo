...
public class RatingServiceImpl implements RatingService, Extensible {
    ...
    public RatingServiceImpl() {
        ratingServiceTrait =
                new ExtendedTrait<RatingServiceTrait>(
                        AJProxyTrait.create(this, RatingServiceTrait.class));
    }
    ...
    public void applyRating(
            final NodeRef targetNode, final float rating, final String ratingSchemeName)
        ...
        if (isCreator && this.getRatingScheme(ratingSchemeName).isSelfRatingAllowed() == false) {
            throw new RatingServiceException(
                    "Users can't rate their own content for scheme " + ratingSchemeName);
        }
    ...
    /**
     * This method checks if the current fully authenticated user is the cm:creator of the specified
     * node.
     */
    ...
    private void applyRating(
            NodeRef targetNode, float rating, String ratingSchemeName, final String userName)
            throws RatingServiceException {
        ...
        if (rating < ratingScheme.getMinRating() || rating > ratingScheme.getMaxRating()) {
            throw new RatingServiceException(
                    "Rating " + rating + " violates range for " + ratingScheme);
        }
        ...
        if (nodeService.hasAspect(targetNode, ContentModel.ASPECT_RATEABLE) == false) {
            ...
            try {
                ...
                // We'll also add the rollup aspect specific for this rating scheme - if one has
                // been defined in the content model.
        ...
        // We're looking for child cm:rating nodes whose assoc qname matches the current user &
        // rating scheme.
        ...
        final QName assocQName =
                ratingNamingConventions.getRatingAssocNameFor(userName, ratingScheme.getName());
        List<ChildAssociationRef> myRatingChildren =
                nodeService.getChildAssocs(targetNode, ContentModel.ASSOC_RATINGS, assocQName);
        if (myRatingChildren.isEmpty()) {
            // There are no previous ratings from this user/scheme combination, so we create a new
            // cm:rating child node.
    ...
    private void recalculateRatingRollups(NodeRef targetNode, final RatingScheme ratingScheme) {
        QName rollupAspectName = ratingNamingConventions.getRollupAspectNameFor(ratingScheme);
        AspectDefinition rollupAspect = dictionaryService.getAspect(rollupAspectName);

        // Only run the rating rollups for this node if the aspect which will hold the results has
        // been defined
        if ((rollupAspect != null)) {
            ...
            try {
                for (AbstractRatingRollupAlgorithm rollupAlgorithm :
                    ...
                    QName rollupPropertyName =
                            ratingNamingConventions.getRollupPropertyNameFor(
                                    ratingScheme, rollupAlgorithm.getRollupName());
                    nodeService.setProperty(targetNode, rollupPropertyName, s);

                    if (!rollupAspect.getProperties().containsKey(rollupPropertyName)
                            && log.isDebugEnabled()) {
                        StringBuilder msg = new StringBuilder();
                        msg.append("Rating property rollup property ")
                                .append(rollupPropertyName)
                                .append(" on aspect ")
                                .append(rollupAspectName)
            ...
            if (log.isDebugEnabled()) {
                StringBuilder msg = new StringBuilder();
                msg.append("Rating property rollup aspect ")
                        .append(rollupAspectName)
                        .append(
                                " is not defined in the content model & therefore the rollup was"
                                        + " not persisted.");
                log.debug(msg.toString());
            }
        }
    ...
    public List<Rating> getRatingsByCurrentUser(NodeRef targetNode) {
        ...
        List<ChildAssociationRef> children =
                getRatingNodeChildren(targetNode, null, fullyAuthenticatedUser);
    ...
    /**
     * This method gets the rating for the specified node, in the specified scheme by the specified
     * user.
     *
    ...
    private Rating getRating(NodeRef targetNode, String ratingSchemeName, String user) {
        List<ChildAssociationRef> ratingChildren =
                getRatingNodeChildren(targetNode, ratingSchemeName, user);
    ...
    /**
     * This method converts a NodeRef (which must be an instance of a cm:rating node) into a {@link
     * Rating} object.
     *
     * @param user String
    ...
    private Rating convertNodeRefToRating(String user, NodeRef ratingNode) {
        ...
        Rating result =
                new Rating(
                        getRatingScheme(existingRatingScheme),
                        existingRatingScore,
                        user,
                        existingRatingDate);
        return result;
    ...
    public float getTotalRating(NodeRef targetNode, String ratingSchemeName) {
        Serializable result =
                this.getRatingRollup(
                        targetNode, ratingSchemeName, RatingTotalRollupAlgorithm.ROLLUP_NAME);
        if (result == null) {
            result = new Float(0f);
        }
    ...
    public int getRatingsCount(NodeRef targetNode, String ratingSchemeName) {
        Serializable result =
                this.getRatingRollup(
                        targetNode, ratingSchemeName, RatingCountRollupAlgorithm.ROLLUP_NAME);
        if (result == null) {
     ...
     */
    @Extend(traitAPI = RatingServiceTrait.class, extensionAPI = RatingServiceExtension.class)
    public Serializable getRatingRollup(
        ...
        if (scheme == null) {
            throw new RatingServiceException(
                    "Cannot retrieve rollup. Unrecognized rating scheme " + ratingSchemeName);
        }
        ...
        if (nodeService.hasAspect(targetNode, rollupAspectName)) {
            QName rollupPropertyName =
                    ratingNamingConventions.getRollupPropertyNameFor(scheme, ratingRollupName);
    ...
    /**
     * This method gets all the cm:rating child nodes of the specified targetNode that have been
     * applied by the specified user in the specified rating scheme.
     *
     * @param targetNode the target node under which the cm:rating nodes reside.
     * @param user the user name of the user whose ratings are sought, <code>null</code> for all
     *     users.
     * @param ratingSchemeName the name of the rating scheme, <code>null</code> for all schemes.
     * @return List<ChildAssociationRef>
     */
    List<ChildAssociationRef> getRatingNodeChildren(
            NodeRef targetNode, String ratingSchemeName, String user) {
        QNamePattern qnamePattern =
                ratingNamingConventions.getRatingAssocPatternForUser(user, ratingSchemeName);
        List<ChildAssociationRef> results =
                nodeService.getChildAssocs(targetNode, ContentModel.ASSOC_RATINGS, qnamePattern);

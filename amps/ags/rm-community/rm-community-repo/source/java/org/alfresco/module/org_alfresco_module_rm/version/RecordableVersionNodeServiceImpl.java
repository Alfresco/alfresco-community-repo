...
public class RecordableVersionNodeServiceImpl
  ...
  protected Map<QName, Serializable> processProperties(
    ...
    properties.put(
      ...
      properties.get(RecordsManagementModel.PROP_ORIGIONAL_NAME)
    );

    // remove all rma, rmc, rmr and rmv properties
    for (QName property : cloneProperties.keySet()) {
      if (
        !PROP_RECORDABLE_VERSION_POLICY.equals(property) &&
   ...
   * @param properties                properties map
   */
  protected void processVersionProperties(
    NodeRef version,
    Map<QName, Serializable> properties
  ) throws InvalidNodeRefException {
    // get version properties
    Map<QName, Serializable> versionProperties = dbNodeService.getProperties(
      version
    );

    if (versionProperties != null) {
      String versionLabel = (String) versionProperties.get(
      ...
      NodeRef nodeRef = (NodeRef) versionProperties.get(
      ...
      Long dbid = (Long) versionProperties.get(
        Version2Model.PROP_QNAME_FROZEN_NODE_DBID
      );
      properties.put(ContentModel.PROP_NODE_DBID, dbid);

      // Convert frozen cm:auditable properties
      String creator = (String) versionProperties.get(
      ...
      Date created = (Date) versionProperties.get(
      ...
      String modifier = (String) versionProperties.get(
      ...
      Date modified = (Date) versionProperties.get(
      ...
      Date accessed = (Date) versionProperties.get(
  ...
  protected Set<QName> processAspects(Set<QName> aspects) {
    ...
    for (QName aspect : aspects) {
      if (
        !ASPECT_VERSIONABLE.equals(aspect) &&
        (

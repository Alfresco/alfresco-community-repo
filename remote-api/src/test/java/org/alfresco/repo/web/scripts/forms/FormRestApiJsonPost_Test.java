...
public class FormRestApiJsonPost_Test extends AbstractTestFormRestApi {
    ...
    public void testSimpleJsonPostRequest() throws IOException, JSONException {
        ...
        ContentData content =
                (ContentData)
                        this.nodeService.getProperty(
                                referencingDocNodeRef, ContentModel.PROP_CONTENT);
        if (content != null) {
        ...
        sendRequest(
                new PostRequest(referencingNodeUpdateUrl, jsonPostString, APPLICATION_JSON), 200);
    ...
    public void testAddNewAssociationsToNode() throws Exception {
        ...
        List<AssociationRef> modifiedAssocs =
                nodeService.getTargetAssocs(referencingDocNodeRef, RegexQNamePattern.MATCH_ALL);
    ...
    /**
     * This test method attempts to add the same association twice. This attempt will not succeed,
     * but the test case is to confirm that there is no exception thrown back across the REST API.
     */
    ...
    /**
     * This test method attempts to remove an association that does not exist. This attempt will not
     * succeed, but the test case is to confirm that there is no exception thrown back across the
     * REST API.
     */
    ...
    public void testAddNewChildAssociationsToNode() throws Exception {
        ...
        sendRequest(
                new PostRequest(
                        containingNodeUpdateUrl.toString(), jsonPostString, APPLICATION_JSON),
                200);
    ...
    public void testRemoveChildAssociationsFromNode() throws Exception {
        ...
        sendRequest(
                new PostRequest(containingNodeUpdateUrl, jsonPostString, APPLICATION_JSON), 200);
    ...
    /**
     * This test method attempts to add the same child association twice. This attempt will not
     * succeed, but the test case is to confirm that there is no exception thrown back across the
     * REST API.
     */
    ...
    /**
     * This test method attempts to remove a child association that does not exist. This attempt
     * will not succeed, but the test case is to confirm that there is no exception thrown back
     * across the REST API.
     */
    ...
    private void checkOriginalChildAssocsBeforeChanges() {
        List<ChildAssociationRef> originalChildAssocs =
                nodeService.getChildAssocs(containerNodeRef);

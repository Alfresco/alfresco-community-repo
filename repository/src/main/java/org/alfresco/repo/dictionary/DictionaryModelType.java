...
public class DictionaryModelType
  ...
  private static final String KEY_PENDING_MODELS =
    "dictionaryModelType.pendingModels";
  ...
  private static final String KEY_WORKING_COPY =
    "dictionaryModelType.workingCopy";
  ...
  public void init() {
    if (logger.isDebugEnabled()) {
      logger.debug(
        "init : bind class behaviours for " + ContentModel.TYPE_DICTIONARY_MODEL
      );
    }
    ...
    this.transactionListener =
      new DictionaryModelTypeTransactionListener(
        this.nodeService,
        this.contentService
      );
  }
  ...
  private void queueModel(NodeRef nodeRef) {
    Set<NodeRef> pendingModels = (Set<NodeRef>) AlfrescoTransactionSupport.getResource(
      KEY_PENDING_MODELS
    );
  ...
  public void beforeDeleteNode(NodeRef nodeRef) {
    boolean workingCopy = nodeService.hasAspect(
      nodeRef,
      ContentModel.ASPECT_WORKING_COPY
    );
    NodeRef wcNodeRef = (NodeRef) AlfrescoTransactionSupport.getResource(
      KEY_WORKING_COPY
    );
    ...
    if (!(workingCopy || isVersionNode)) {
      QName modelName = (QName) this.nodeService.getProperty(
          nodeRef,
      ...
      if (modelName != null) {
        ...
        Set<NodeRef> pendingModelDeletes = (Set<NodeRef>) AlfrescoTransactionSupport.getResource(
          KEY_PENDING_DELETE_MODELS
        );
  ...
  public void onCreateNode(ChildAssociationRef childAssocRef) {
    ...
    if (
      nodeService.getType(nodeRef).equals(ContentModel.TYPE_DICTIONARY_MODEL)
    ) {
      Boolean value = (Boolean) nodeService.getProperty(
        nodeRef,
        ContentModel.PROP_MODEL_ACTIVE
  ...
  public class DictionaryModelTypeTransactionListener
    ...
    public void afterCommit() {
      ...
      Set<NodeRef> pendingDeleteModels = (Set<NodeRef>) AlfrescoTransactionSupport.getResource(
        KEY_PENDING_DELETE_MODELS
      );
      ...
      if (pendingModels != null) {
        ...
        for (NodeRef nodeRef : pendingModels) {
          String tenantDomain = tenantService.getDomain(
    ...
    public void beforeCommit(boolean readOnly) {
      if (jobLockService != null) {
        ...
        try {
          jobLockService.getTransactionalLock(
            LOCK_QNAME,
            (1000 * 60),
            3000,
            10
      ...
      if (pendingModels != null) {
        ...
        for (NodeRef pendingNodeRef : pendingModels) {
          String tenantDomain = tenantService.getDomain(
            pendingNodeRef.getStoreRef().getIdentifier()
          );
          String tenantSystemUserName = tenantService.getDomainUser(
                ...
                if (
                  ...
                  if (isActive == true) {
                    ...
                    if (contentReader != null) {
                      ...
                      try {
                        ...
                        if (is != null) {
                          try {
                            ...
                            logger.error(
                              "Failed to close input stream for " + nodeRef
                            );
                          }
                        }
                      }
                      ...
                      CompiledModel compiledModel = m2Model.compile(
                        dictionaryDAO,
                        namespaceDAO,
                        true
                      );
                    ...
                    QName modelName = (QName) nodeService.getProperty(
                      nodeRef,
                      ContentModel.PROP_MODEL_NAME
                    );
                    if (modelName != null) {

...
public abstract class RenditionIntegrationTests extends RestTest {
    ...
    protected void checkRendition(
        ...
        Assert.assertEquals(
                ...
                "Failed to submit a request for rendition. ["
                        + fileName
                        + ", "
                        + renditionId
        ...
        RestResponse restResponse =
                restClient
                        .withCoreAPI()
                        .usingNode(file)
                        .getNodeRenditionContentUntilIsCreated(renditionId);
        Assert.assertEquals(
                Integer.valueOf(restClient.getStatusCode()).intValue(),
                HttpStatus.OK.value(),
                "Failed to produce rendition. ["
                        + fileName
                        + ", "
                        + renditionId
                        + "] [source file, rendition ID] ");
        ...
        Assert.assertEquals(
                restClient.getResponseHeaders().getValue("Content-Type"),
                expectedMimeType + ";charset=UTF-8",
                "Rendition was created but it has the wrong Content-Type. ["
                        + fileName
                        + ", "
                        + renditionId
                        + "] [source file, rendition ID]");
        ...
        Assert.assertTrue(
                ...
                "Rendition was created but its content is empty. ["
                        + fileName
                        + ", "
                        + renditionId
                        + "] [source file, rendition ID] ");

...
/**
 * A utility class providing a method to dump a local or remote MBeanServer's entire object tree for
 * support purposes. Nested arrays and CompositeData objects in MBean attribute values are handled.
 *
 * @author dward
 */
public class JmxDumpUtil {
    ...
    /**
     * Dumps a local or remote MBeanServer's entire object tree for support purposes. Nested arrays
     * and CompositeData objects in MBean attribute values are handled.
     *
     * @param connection the server connection (or server itself)
     * @param out PrintWriter to write the output to
     * @throws IOException Signals that an I/O exception has occurred.
     */
    public static void dumpConnection(MBeanServerConnection connection, PrintWriter out)
            throws IOException {
        ...
        Set<ObjectName> newObjectNames =
                new TreeSet<ObjectName>(
     ...
     * @param out PrintWriter to write the output to
     * @throws IOException Signals that an I/O exception has occurred.
     * @throws JMException Signals a JMX error
     */
    private static void printMBeanInfo(
            MBeanServerConnection connection, ObjectName objectName, PrintWriter out)
        ...
        if (objectName.getCanonicalName().equals("java.lang:type=Runtime")) {
            ...
            if (commandInputs != null) {
                try {
                    attributes.put(
                            INPUT_ARGUMENTS,
                            cleanPasswordsFromInputArguments(commandInputs, REDACTED_INPUTS));
    ...
    /**
     * Replaces strings with JmxDumpUtil.PROTECTED_VALUE, if any of the string that contains a
     * string from redactedInputs.
     *
     ...
     */
    static String[] cleanPasswordsFromInputArguments(
            String[] commandInputs, String[] redactedInputs) {
        Pattern passwordRedactPattern =
                Pattern.compile(createPasswordFindRegexString(redactedInputs));
    ...
    /**
     * Removes any characters the word/s provided in redactedInputs and replaces them with
     * JmxDumpUtil.PROTECTED_VALUE
     *
     * <p>Example:
     *
     * <p>Input: -Ddb.password=alfresco
     *
     * <p>Output: -Ddb.password=********
     *
     * @param input String
     * @param redactedInputs String[]
     * @return password redacted string if input matches a string in redactedInputs, an un-altered
     *     string will be returned if it does not match.
     */
    static String cleanPasswordFromInputArgument(String input, Pattern redactedInputPattern) {
        // Replace the whole string with just capture group 1 to remove the desired value and concat
        // the protected value.
    ...
    /**
     * Creates a regular expression that will select a string that contains one of the values
     * provided in argEndings, proceeding an "=" and defines two capture groups:
     *
     * <ul>
     *   <li>Group 1: An argEnding that is followed by an "=", including the "=" and all character
     *       prior to the argEnding.
     *   <li>Group 2: The characters that follow group 1, to the end of the string or new line.
     * </ul>
     *
     * <p>The argEnding can be the whole Input argument or the common characters proceeding the =
     * sign. Example argEndings:
     *
     * <ul>
     ...
     * <p>Example usage:
     *
     * <p>argEndings={"password", "pwd"}
     *
     * <p>This will create a regex that will match a string that contains either argEndings. The
     * following will be matched by the resulting regex:
     *
     * <p>"-Ddb.password=my_password"
     *
     * <p>For this example: group 1="-Ddb.password=" group 2="my_password"
     *
     ...
     * @param out PrintWriter to write the output to
     * @param nestLevel the nesting level
     * @throws IOException Signals that an I/O exception has occurred.
     */
    @SuppressWarnings("unchecked")
    private static void printCompositeInfo(CompositeData composite, PrintWriter out, int nestLevel)
            throws IOException {
        Map<String, Object> attributes = new TreeMap<String, Object>();
     ...
     * @param rows Map containing key value pairs forming the rows
     * @param out PrintWriter to write the output to
     * @param nestLevel the nesting level
     * @throws IOException Signals that an I/O exception has occurred.
     */
    ...
    /**
     * Outputs a single row in a two-column table. The first column is padded with spaces so that
     * the second column is aligned.
     *
     * @param out PrintWriter to write the output to
     * @param maxKeyLength maximum number of characters in the first column
     * @param key the first column value
     * @param value the second column value
     * @param nestLevel the nesting level
     * @throws IOException Signals that an I/O exception has occurred.
     */
    private static void outputRow(
            PrintWriter out, int maxKeyLength, String key, Object value, int nestLevel)
            throws IOException {
        indent(out, nestLevel);
        out.print(key);
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    private static void outputValue(PrintWriter out, Object value, int nestLevel)
            throws IOException {
        if (value == null) {

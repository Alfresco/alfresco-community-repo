...
public abstract class AbstractWritableContentStoreTest
  ...
  protected static Log logger = LogFactory.getLog(
    AbstractWritableContentStoreTest.class
  );
   ...
   */
  private void checkIllegalWritableContentUrl(
    ContentStore store,
    String contentUrl
  ) {
    assertFalse(
  ...
  public void testSimpleUse() {
    ...
    assertNotNull(
      "Content URL may not be null for unused writer",
      contentUrlBefore
    );
    assertTrue(
  ...
  public synchronized void testGetReader() throws Exception {
    ...
    assertNotNull(
      "A reader must always be available from the store",
      readerFromStoreBeforeWrite
    );

    // check that a reader is available from the writer
    ContentReader readerFromWriterBeforeWrite = writer.getReader();
    assertNotNull(
      "A reader must always be available from the writer",
      readerFromWriterBeforeWrite
    );
    ...
    assertEquals(
      "Reader content length is incorrect",
      length,
      readerFromWriter.getSize()
    );
    ...
    assertTrue(
      "Reader last modified is incorrect",
      beforeSeconds <= modifiedTimeCheckSeconds
    );
    assertTrue(
      "Reader last modified is incorrect",
      modifiedTimeCheckSeconds <= afterSeconds
    );
  }
  ...
  public void testReaderImmutability() {
    ...
    assertFalse(
      "Reader's state changed after write",
      readerBeforeWrite.exists()
    );
    try {
  ...
  public void testMimetypAndEncodingAndLocale() throws Exception {
    ...
    assertEquals(
      "Writer -> Reader mimetype mismatch",
      writer.getMimetype(),
      reader.getMimetype()
    );
    assertEquals(
      "Writer -> Reader encoding mismatch",
      writer.getEncoding(),
      reader.getEncoding()
    );
    assertEquals(
      "Writer -> Reader locale mismatch",
      writer.getLocale(),
      reader.getLocale()
    );
    ...
    assertEquals(
      "Encoding and decoding of strings failed",
      content,
      contentCheck
    );
  }
  ...
  public void testClosedState() throws Exception {
    ...
    assertFalse(
      "Reader stream should not be closed",
    ...
    assertFalse(
      "After content reader should not be closed",
      readerAfterWrite.isClosed()
    );
    ...
    assertFalse(
      "Reader must always be a new instance",
      newReaderA == newReaderB
    );
    ...
    assertFalse(
      "Before-content reader stream should not be closed",
      readerBeforeWrite.isClosed()
    );
    assertTrue(
      "After-content reader should be closed after reading",
      readerAfterWrite.isClosed()
    );
  }
  ...
  public void testDeleteSimple() throws Exception {
    ...
    try {
      ...
      logger.warn(
        "Store test testDeleteSimple not possible on " +
  ...
  public void testDeleteReaderStates() throws Exception {
    ...
    reader = store.getReader(contentUrl);
    assertNotNull(
      "Reader must be returned even when underlying content is missing",
      reader
    );
    assertFalse("Content should not exist", reader.exists());
    try {
  ...
  public void testWriteStreamListener() throws Exception {
    ...
    assertTrue(
      "Write stream listener was not called for the stream close",
      streamClosed[0]
    );
  }
  ...
  public void testWriteAndReadString() throws Exception {
    ...
    assertTrue(
      "Read and write may not share same resource",
      check.length() > 0
    );
  ...
  public void testReadAndWriteFile() throws Exception {
    ...
    File sinkFile = TempFileProvider.createTempFile(
      "testReadAndWriteFile",
      ".txt"
    );
  ...
  public void testRandomAccessWrite() throws Exception {
    ...
    ContentContext writerTruncateCtx = new ContentContext(
      writer.getReader(),
      null
    );
    ...
    ContentContext writerNoTruncateCtx = new ContentContext(
      writer.getReader(),
      null
    );

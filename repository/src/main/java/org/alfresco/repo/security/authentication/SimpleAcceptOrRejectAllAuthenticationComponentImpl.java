...
/**
 * This implementation of an AuthenticationComponent can be configured to accept or reject all
 * attempts to login.
 *
 * <p>This only affects attempts to login using a user name and password. Authentication filters
 * etc. could still support authentication but not via user names and passwords. For example, where
 * they set the current user using the authentication component. Then the current user is set in the
 * security context and asserted to be authenticated.
 *
 * <p>By default, the implementation rejects all authentication attempts.
 *
 * @author Andy Hind
 */
public class SimpleAcceptOrRejectAllAuthenticationComponentImpl
    ...
    protected UserDetails getUserDetails(String userName) {
        ...
        if (AuthenticationUtil.isMtEnabled()) {
            // ALF-9403 - "manual" runAs to avoid clearing ticket, eg. when called via "validate"
            ...
            try {
                ...
                if (originalFullAuthentication == null) {
                    ContextHolder.setContext(null); // note: does not clear ticket (unlike

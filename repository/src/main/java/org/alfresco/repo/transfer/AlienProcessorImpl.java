...
public class AlienProcessorImpl implements AlienProcessor {
    ...
    public void onCreateChild(
            ChildAssociationRef childAssocRef, final String repositoryId, boolean isNewNode) {
        ...
        if (!childAssocRef.getTypeQName().equals(ContentModel.ASSOC_CONTAINS)) {
            Collection<QName> subAspects =
                    dictionaryService.getSubAspects(ContentModel.ASSOC_CONTAINS, true);
            ...
            String parentFromRepo =
                    (String)
                            nodeService.getProperty(
                                    parentNodeRef, TransferModel.PROP_FROM_REPOSITORY_ID);
            {
                if (repositoryId.equalsIgnoreCase(parentFromRepo)) {
                    log.debug(
                            "parent was not alien and this node is from the same repo - do"
                                    + " nothing");
                    return;
                }
            }
        ...
        while (currentAssoc != null) {
            ...
            if (nodeService.hasAspect(parentNodeRef, TransferModel.ASPECT_TRANSFERRED)
                ...
                if (!isInvaded(parentNodeRef, repositoryId)) {
                    ...
                    /**
                     * Parent may be locked or not be editable by the current user turn off auditing
                     * and lock service for this transaction and run as admin.
                     */
                    RunAsWork<Void> actionRunAs =
                            new RunAsWork<Void>() {
                                public Void doWork() throws Exception {
                                    getBehaviourFilter()
                                            .disableBehaviour(
                                                    newAlien, ContentModel.ASPECT_AUDITABLE);
                                    getBehaviourFilter()
                                            .disableBehaviour(
                                                    newAlien, ContentModel.ASPECT_LOCKABLE);
    ...
    /**
     * Puts information about current <code>childRef</code> and its <code>parentRef</code> into log
     * in TRACE level. Information includes 'name', 'fromRepositoryId', 'aliened' and 'invadedBy'
     * properties. Additionally, collects the same information for children of <code>childRef</code>
     *
    ...
    protected void logInvasionHierarchy(NodeRef parentRef, NodeRef childRef) {
        ...
        StringBuilder message =
                new StringBuilder("Information about '")
                        .append(properties.get(ContentModel.PROP_NAME))
                        .append("' node:\n    fromRepositoryId: ")
                        .append(properties.get(TransferModel.PROP_FROM_REPOSITORY_ID))
                        .append("\n")
                        .append("    invadedBy: ")
                        .append(properties.get(TransferModel.PROP_INVADED_BY))
                        .append("\n")
                        .append("    alien: ")
                        .append(nodeService.hasAspect(childRef, TransferModel.ASPECT_ALIEN))
                        .append("\n")
                        .append("    repositoryId: ")
                        .append(properties.get(TransferModel.PROP_REPOSITORY_ID))
                        .append("\n")
                        .append("    parent: ")
                        .append(parentProperties.get(ContentModel.PROP_NAME))
                        .append("(")
                        .append(parentProperties.get(TransferModel.PROP_FROM_REPOSITORY_ID))
                        .append(")")
                        .append(parentProperties.get(TransferModel.PROP_INVADED_BY))
                        .append(": ")
                        .append(nodeService.hasAspect(parentRef, TransferModel.ASPECT_ALIEN))
                        .append("\n")
                        .append("    children:\n");
                ...
                message.append("        ")
                        ...
                        .append(
                                nodeService.hasAspect(
                                        child.getChildRef(), TransferModel.ASPECT_ALIEN))
                        .append("\n");
            }
        }
    ...
    public void beforeDeleteAlien(NodeRef deletedNodeRef, ChildAssociationRef oldAssoc) {
        ...
        List<String> stuff =
                (List<String>)
                        nodeService.getProperty(deletedNodeRef, TransferModel.PROP_INVADED_BY);
        ...
        while (currentAssoc != null && exInvaders != null && exInvaders.size() > 0) {
            ...
            if (nodeService.hasAspect(parentNodeRef, TransferModel.ASPECT_ALIEN)) {
                ...
                /**
                 * Remove the parent's origin from the list of exInvaders since the parent also
                 * invades.
                 */
                String parentRepoId;
                if (nodeService.hasAspect(parentNodeRef, TransferModel.ASPECT_TRANSFERRED)) {
                ...
                while (i.hasNext()) {
                    ...
                    /**
                     * Check the siblings of this node to see whether there are any other alien
                     * nodes for this invader.
                     */
                    ...
                    List<ChildAssociationRef> refs =
                            nodeService.getChildAssocsByPropertyValue(
                                    parentNodeRef, TransferModel.PROP_INVADED_BY, exInvader);

                    for (ChildAssociationRef ref : refs) {
                        NodeRef childRef = ref.getChildRef();
                        List<String> invadedBy =
                                (List<String>)
                                        nodeService.getProperty(
                                                childRef, TransferModel.PROP_INVADED_BY);
                ...
                if (exInvaders.size() > 0) {
                    ...
                    List<String> parentInvaders =
                            (List<String>)
                                    nodeService.getProperty(
                                            parentNodeRef, TransferModel.PROP_INVADED_BY);
                            ...
                            new RunAsWork<Void>() {
                                public Void doWork() throws Exception {
                                    behaviourFilter.disableBehaviour(
                                            oldAlien, ContentModel.ASPECT_AUDITABLE);
                                    behaviourFilter.disableBehaviour(
                                            oldAlien, ContentModel.ASPECT_LOCKABLE);
                                    if (newInvaders.size() > 0) {
                                        ...
                                        log.debug(
                                                "parent node is no longer alien nodeRef"
                                                        + oldAlien);
                                        nodeService.removeAspect(
                                                oldAlien, TransferModel.ASPECT_ALIEN);
                                    }
                                    return null;
                                }
                            };
    ...
    public void afterMoveAlien(ChildAssociationRef newAssocRef) {
        ...
        List<String> childInvadedBy =
                (List<String>) nodeService.getProperty(childNodeRef, TransferModel.PROP_INVADED_BY);
                ...
                if (nodeService.hasAspect(parentNodeRef, TransferModel.ASPECT_TRANSFERRED)
                    ...
                    while (i.hasNext()) {
                        ...
                        if (!isInvaded(parentNodeRef, alienRepoId)) {
                            ...
                            /**
                             * Parent may be locked or not be editable by the current user turn off
                             * auditing and lock service for this transaction and run as admin.
                             */
                            RunAsWork<Void> actionRunAs =
                                    new RunAsWork<Void>() {
                                        public Void doWork() throws Exception {
                            ...
                            AuthenticationUtil.runAs(
                                    actionRunAs, AuthenticationUtil.getSystemUserName());
                        } else {
     ...
     *
     * <p>Steps down the tree retreating from all the invaded nodes.
     *
     * <p>The retreat will stop is there is a "sub-invasion".
     *
     * <p>
     *
    ...
    private void retreatDownwards(NodeRef nodeRef, String fromRepositoryId) {
        ...
        while (!nodesToRetreat.isEmpty()) {
             ...
             *
             * <p>if they are "from" the retreating repository then
             */
            ...
            if (getNodeService().hasAspect(currentNodeRef, TransferModel.ASPECT_ALIEN)) {
                ...
                List<String> invadedBy =
                        (List<String>)
                                getNodeService()
                                        .getProperty(currentNodeRef, TransferModel.PROP_INVADED_BY);
                ...
                if (nodeService.hasAspect(currentNodeRef, TransferModel.ASPECT_TRANSFERRED)) {
                    ...
                    parentRepoId =
                            (String)
                                    nodeService.getProperty(
                                            currentNodeRef, TransferModel.PROP_FROM_REPOSITORY_ID);
                } else {
                ...
                if (fromRepositoryId.equalsIgnoreCase(parentRepoId)) {
                    ...
                    // List<ChildAssociationRef> refs =
                    // getNodeService().getChildAssocs(currentNodeRef);
                    List<ChildAssociationRef> refs =
                            nodeService.getChildAssocsByPropertyValue(
                                    currentNodeRef,
                                    TransferModel.PROP_INVADED_BY,
    ...
    public void pruneNode(NodeRef nodeToPrune, String fromRepositoryId) {
        ...
        while (!nodesToPrune.isEmpty()) {
             ...
             *
             * <p>if from the repo with no (other) aliens - delete
             *
             * <p>if from the repo with multiple alien invasions - leave alone but process children
             */
            ...
            if (getNodeService().hasAspect(currentNodeRef, TransferModel.ASPECT_ALIEN)) {
                if (log.isDebugEnabled()) {
                    log.debug(
                            "Current nodeRef has ASPECT_ALIEN (name: \""
                                    + properties.get(ContentModel.PROP_NAME)
                                    + "\", fromRepositoryId: \""
                                    + properties.get(TransferModel.PROP_FROM_REPOSITORY_ID)
                                    + "\", manifestId: \""
                ...
                if (log.isDebugEnabled()) {
                    log.debug(
                            "Current nodeRef has PROP_INVADED_BY (name: \""
                                    + properties.get(ContentModel.PROP_NAME)
                                    + "\", fromRepositoryId: \""
                                    + properties.get(TransferModel.PROP_FROM_REPOSITORY_ID)
                                    + "\", manifestId: \""
                ...
                if ((null != invadedBy) && invadedBy.contains(fromRepositoryId)) {
                    if (log.isDebugEnabled()) {
                        log.debug(
                                "Current nodeRef's PROP_INVADED_BY contains current manifestId"
                                        + " (name: \""
                                        + properties.get(ContentModel.PROP_NAME)
                                        + "\", fromRepositoryId: \""
                                        + properties.get(TransferModel.PROP_FROM_REPOSITORY_ID)
                    ...
                    // Yes we are invaded by fromRepositoryId
                    if ((1 == invadedBy.size())
                            && fromRepositoryId.equalsIgnoreCase(initialRepoId)) {
                        if (log.isDebugEnabled()) {
                            log.debug(
                                    "Current nodeRef has only 1 element in PROP_INVADED_BY. Also"
                                            + " MANIFEST_ID and INITIAL_REPOSITORY_ID are the same."
                                            + " Deleting the node... (name: \""
                                            + properties.get(ContentModel.PROP_NAME)
                                            + "\", fromRepositoryId: \""
                                            + properties.get(TransferModel.PROP_FROM_REPOSITORY_ID)
                                            + "\", manifestId: \""
                        ...
                        if (log.isDebugEnabled()) {
                            log.debug(
                                    "Current 'nodeRef' has more than 1 element in PROP_INVADED_BY."
                                        + " Adding its children to 'nodesToPrune' list... (name: \""
                                            + properties.get(ContentModel.PROP_NAME)
                                            + "\", fromRepositoryId: \""
                                            + properties.get(TransferModel.PROP_FROM_REPOSITORY_ID)
                                            + "\", manifestId: \""
                        ...
                        if (!foldersToRecalculate.contains(currentNodeRef)) {
                            if (log.isDebugEnabled()) {
                                log.debug(
                                        "Current 'nodeRef' is not in 'foldersToRecalculate' list."
                                                + " Adding it to the list... (name: \""
                                                + properties.get(ContentModel.PROP_NAME)
                                                + "\", fromRepositoryId: \""
                                                + properties.get(
                                                        TransferModel.PROP_FROM_REPOSITORY_ID)
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug(
                                "Current \"nodeRef\"'s PROP_INVADED_BY does not contain current"
                                        + " 'manifestId' (name: \""
                                        + properties.get(ContentModel.PROP_NAME)
                                        + "\", fromRepositoryId: \""
                                        + properties.get(TransferModel.PROP_FROM_REPOSITORY_ID)
                     ...
                     *
                     * <p>Need to check fromRepositoryId since its children may need to be pruned
                     */
                    getNodeService().hasAspect(currentNodeRef, TransferModel.ASPECT_TRANSFERRED);
                    {
                        if (fromRepositoryId.equalsIgnoreCase(initialRepoId)) {
                            if (log.isDebugEnabled()) {
                                ...
                                log.debug(
                                        "Current nodeRef has more than 1 element in"
                                                + " PROP_INVADED_BY. Adding its children to"
                                                + " 'nodesToPrune' list... (name: \""
                                                + properties.get(ContentModel.PROP_NAME)
                                                + "\", fromRepositoryId: \""
                                                + properties.get(
                                                        TransferModel.PROP_FROM_REPOSITORY_ID)
                            ...
                            for (ChildAssociationRef ref : refs) {
                                ...
                                /**
                                 * This folder can't be deleted so its invaded flag needs to be
                                 * re-calculated
                                 */
                                if (!foldersToRecalculate.contains(currentNodeRef)) {
                                    if (log.isDebugEnabled()) {
                                        log.debug(
                                                "Current 'nodeRef' is not in 'foldersToRecalculate'"
                                                    + " list. Adding it to the list... (name: \""
                                                        + properties.get(ContentModel.PROP_NAME)
                                                        + "\", fromRepositoryId: \""
                                                        + properties.get(
                                                                TransferModel
                ...
                getNodeService().hasAspect(currentNodeRef, TransferModel.ASPECT_TRANSFERRED);
                {
                    if (log.isDebugEnabled()) {
                        log.debug(
                                "Current 'nodeRef' does not have ASPECT_ALIEN (name: \""
                                        + properties.get(ContentModel.PROP_NAME)
                                        + "\", fromRepositoryId: \""
                                        + properties.get(TransferModel.PROP_FROM_REPOSITORY_ID)
                                        + "\", manifestId: \""
                    ...
                    if (fromRepositoryId.equalsIgnoreCase(initialRepoId)) {
                        if (log.isDebugEnabled()) {
                            log.debug(
                                    "Current \"nodeRef\"'s has PROP_FROM_REPOSITORY_ID equal to"
                                        + " current 'manifestId'. Deleting the node... (name: \""
                                            + properties.get(ContentModel.PROP_NAME)
                                            + "\", fromRepositoryId: \""
                                            + properties.get(TransferModel.PROP_FROM_REPOSITORY_ID)
                                            + "\", manifestId: \""
    ...
    private boolean isInvaded(NodeRef nodeRef, String invader) {
        List<String> invadedBy =
                (List<String>) nodeService.getProperty(nodeRef, TransferModel.PROP_INVADED_BY);
    ...
    private void setAlien(NodeRef newAlien, String invader) {
        ...
        List<String> invadedBy =
                (List<String>) nodeService.getProperty(newAlien, TransferModel.PROP_INVADED_BY);
    ...
    private boolean recalcInvasion(NodeRef folderNodeRef, String fromRepositoryId) {
        ...
        List<String> folderInvadedBy =
                (List<String>)
                        nodeService.getProperty(folderNodeRef, TransferModel.PROP_INVADED_BY);
        ...
        if (log.isDebugEnabled()) {
            ...
            log.debug(
                    "Is alien: "
                            + nodeService.hasAspect(folderNodeRef, TransferModel.ASPECT_ALIEN));
        }

        String parentRepositoryId =
                (String)
                        nodeService.getProperty(
                                folderNodeRef, TransferModel.PROP_FROM_REPOSITORY_ID);
        for (ChildAssociationRef ref : refs) {
            ...
            List<String> childInvadedBy =
                    (List<String>) properties.get(TransferModel.PROP_INVADED_BY);
            String childRepositoryId =
                    (String) properties.get(TransferModel.PROP_FROM_REPOSITORY_ID);

            hasAlienChild =
                    hasAlienChild || !parentRepositoryId.equalsIgnoreCase(childRepositoryId);

            if (!stillInvaded
                    && (null != childInvadedBy)
                    && (childInvadedBy.contains(fromRepositoryId)
                            || fromRepositoryId.equalsIgnoreCase(childRepositoryId))) {
                if (log.isDebugEnabled()) {
                    log.debug(
                            "This child contains current 'fromRepositoryId'. Current folder is"
                                    + " still invaded by this repository");
                }
        ...
        if (!stillInvaded) {
            if (log.isDebugEnabled()) {
                log.debug(
                        "Current folder is not invaded by this repository. Updating 'invadedBy'"
                                + " property...");
            ...
            if (folderInvadedBy.size() > 0) {
                if (log.isDebugEnabled()) {
                    log.debug(
                            "Current folder HAS ANOTHER invasions. Updating the 'invadedBy'"
                                    + " property...");
                ...
                if (log.isDebugEnabled()) {
                    ...
                    log.debug(
                            "This invasion was the last one for the current folder. Removing aspect"
                                    + " 'ALIEN' completely...");
                }

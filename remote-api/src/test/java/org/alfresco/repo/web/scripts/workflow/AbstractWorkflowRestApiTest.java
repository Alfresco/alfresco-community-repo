...
public abstract class AbstractWorkflowRestApiTest extends BaseWebScriptTest {
  ...
  protected static final String URL_USER_TASKS =
    "api/task-instances?authority={0}";
  protected static final String URL_USER_TASKS_PROPERTIES =
    "api/task-instances?authority={0}&properties={1}";
  protected static final String URL_TASKS_DUE_BEFORE =
    "api/task-instances?dueBefore={0}";
  protected static final String URL_TASKS_DUE_AFTER =
    "api/task-instances?dueAfter={0}";
  protected static final String URL_TASKS_DUE_AFTER_AND_SKIP =
    "api/task-instances?dueAfter={0}&skipCount={1}";
  protected static final String URL_WORKFLOW_TASKS =
    "api/workflow-instances/{0}/task-instances";
  protected static final String URL_WORKFLOW_DEFINITIONS =
    "api/workflow-definitions";
  protected static final String URL_WORKFLOW_DEFINITION =
    "api/workflow-definitions/{0}";
  protected static final String URL_WORKFLOW_INSTANCES =
    "api/workflow-instances";
  protected static final String URL_WORKFLOW_INSTANCES_FOR_DEFINITION =
    "api/workflow-definitions/{0}/workflow-instances";
  protected static final String URL_WORKFLOW_INSTANCES_FOR_NODE =
    "api/node/{0}/{1}/{2}/workflow-instances";
  ...
  protected static final String ADHOC_TASK_COMPLETED_TYPE =
  ...
  public void testTaskInstancesGet() throws Exception {
    ...
    Response response = sendRequest(
      new GetRequest(MessageFormat.format(URL_USER_TASKS, USER2)),
      200
    );
    ...
    WorkflowDefinition adhocDef = workflowService.getDefinitionByName(
    ...
    WorkflowPath adhocPath = workflowService.startWorkflow(
      adhocDef.getId(),
      params
    );
    ...
    List<WorkflowTask> tasks = workflowService.getAssignedTasks(
      USER2,
    ...
    assertEquals(
      "api/workflow-paths/" + adhocPath.getId(),
    ...
    assertEquals(
      personManager.getFirstName(USER2),
    ...
    checkTasksState(
      URL_TASKS + "?state=completed",
      WorkflowTaskState.COMPLETED
    );
    checkTasksState(
      URL_TASKS + "?state=in_progress",
      WorkflowTaskState.IN_PROGRESS
    );
    ...
    checkTasksPresent(
      MessageFormat.format(
        URL_TASKS_DUE_AFTER,
        ISO8601DateFormat.format(dueDateCal.getTime())
      ),
      true,
      task.getId()
    );
    ...
    checkTaskPropertyFiltering(
      customProperties,
      Arrays.asList("bpm_description", "bpm_priority")
    );
    ...
    for (int skipCount = 0; skipCount < totalItems; skipCount += maxItems) {
      ...
      // totalItems
      checkPaging(
        MessageFormat.format(URL_USER_TASKS, USER2) +
        "&maxItems=" +
        maxItems +
        "&skipCount=" +
        skipCount,
    ...
    response =
      sendRequest(new GetRequest(URL_TASKS + "?exclude=" + exclude), 200);
    assertEquals(Status.STATUS_OK, response.getStatus());
    jsonStr = response.getContentAsString();
    json = new JSONObject(jsonStr);
    results = json.getJSONArray("data");
    ...
    assertFalse(
      "Found wf:submitAdhocTask when they were supposed to be excluded",
      adhocTasksPresent
    );
    ...
    params.put(
      ...
      workflowService.createPackage(null)
    );

    WorkflowPath adhocPath2 = workflowService.startWorkflow(
      adhocDef.getId(),
      params
    );
  ...
  public void testTaskInstancesGetWithFiltering() throws Exception {
    ...
    personManager.setUser(USER1);
    WorkflowDefinition wfDefinition = workflowService.getDefinitionByName(
    ...
    WorkflowPath wfPath = workflowService.startWorkflow(
      wfDefinition.getId(),
      params
    );
    ...
    wfDefinition =
      workflowService.getDefinitionByName(
        getReviewPooledWorkflowDefinitionName()
      );
    params.put(WorkflowModel.ASSOC_GROUP_ASSIGNEE, groupManager.get(GROUP));
    params.put(
    ...
    response =
      sendRequest(
        new GetRequest(
          MessageFormat.format(URL_USER_TASKS, USER2) +
          "&property=bpm:description/descTest1"
        ),
        200
      );
    ...
    response =
      sendRequest(
        new GetRequest(
          MessageFormat.format(URL_USER_TASKS, USER2) +
          "&property=bpm:description/descTest2/withSlash"
        ),
        200
      );
    ...
    assertEquals(
      "descTest2/withSlash",
      properties.getString("bpm_description")
    );
    ...
    response =
      sendRequest(
        new GetRequest(
          MessageFormat.format(URL_USER_TASKS, USER2) +
          "&property=bpm:description/somePropValue"
        ),
        200
      );
    getJsonArray(response, 0);
    ...
    response =
      sendRequest(
        new GetRequest(
          MessageFormat.format(URL_USER_TASKS, USER2) +
          "&property=unknownPrefix:description/test"
        ),
        200
      );
    getJsonArray(response, 2);
    ...
    response =
      sendRequest(
        new GetRequest(
          MessageFormat.format(URL_USER_TASKS, USER2) +
          "&property=bpm:nonexistentProp/test"
        ),
        200
      );
    getJsonArray(response, 2);
    ...
    response =
      sendRequest(
        new GetRequest(
          MessageFormat.format(URL_USER_TASKS, USER2) + "&property=descTest1"
        ),
        200
      );
    getJsonArray(response, 2);
    ...
    response =
      sendRequest(
        new GetRequest(MessageFormat.format(URL_USER_TASKS, USER3)),
        200
      );
    getJsonArray(response, 0);
  }
  ...
  public void testWorkflowPermissions() throws Exception {
    ...
    personManager.setUser(USER2);
    Response response = sendRequest(
      new GetRequest(MessageFormat.format(URL_USER_TASKS, USER1)),
      200
    );
    assertEquals(Status.STATUS_OK, response.getStatus());
    String jsonStr = response.getContentAsString();
    ...
    assertNotNull(results);
    assertTrue(
      "User2 should not see any tasks if he is not initiator or assignee",
      results.length() == 0
    );
  }
  ...
  public void testTaskInstancesForWorkflowGet() throws Exception {
    ...
    personManager.setUser(USER2);
    sendRequest(
      new GetRequest(MessageFormat.format(URL_WORKFLOW_TASKS, "Foo")),
    ...
    String user1CompletedURL =
      user1Url + "&state=" + WorkflowTaskState.COMPLETED;
    checkTasksMatch(user1CompletedURL, startTaskId);

    String user1InProgressURL =
      user1Url + "&state=" + WorkflowTaskState.IN_PROGRESS;
    checkTasksMatch(user1InProgressURL);

    String user2CompletedURL =
      user2Url + "&state=" + WorkflowTaskState.COMPLETED;
    checkTasksMatch(user2CompletedURL);

    String user2InProgressURL =
      user2Url + "&state=" + WorkflowTaskState.IN_PROGRESS;
  ...
  public void testTaskInstanceGet() throws Exception {
    ...
    WorkflowTask startTask = workflowService
      .getTasksForWorkflowPath(adhocPath.getId())
      .get(0);

    // Get the start-task
    Response response = sendRequest(
      new GetRequest(URL_TASKS + "/" + startTask.getId()),
      Status.STATUS_OK
    );
    String jsonStr = response.getContentAsString();
    JSONObject json = new JSONObject(jsonStr);
    ...
    checkWorkflowInstance(
      startTask.getPath().getInstance(),
      result.getJSONObject("workflowInstance")
    );
    checkWorkflowTaskDefinition(
      startTask.getDefinition(),
      result.getJSONObject("definition")
    );
    ...
    WorkflowTask firstTask = workflowService
      .getTasksForWorkflowPath(adhocPath.getId())
      .get(0);

    response =
      sendRequest(new GetRequest(URL_TASKS + "/" + firstTask.getId()), 200);
    jsonStr = response.getContentAsString();
    json = new JSONObject(jsonStr);
    result = json.getJSONObject("data");
    assertNotNull(result);
    ...
    assertEquals(
      WorkflowTaskState.IN_PROGRESS.toString(),
    ...
    checkWorkflowInstance(
      firstTask.getPath().getInstance(),
      result.getJSONObject("workflowInstance")
    );
    checkWorkflowTaskDefinition(
      firstTask.getDefinition(),
      result.getJSONObject("definition")
    );
    ...
    assertEquals(firstTask.getState().name(), result.getString("state"));
    assertEquals(
      WorkflowTaskState.COMPLETED.toString(),
      result.getString("state")
    );
    assertEquals(
      "api/workflow-paths/" + adhocPath.getId(),
  ...
  private void checkWorkflowTaskPropertiesPresent(JSONObject taskJson)
    throws Exception {
  ...
  private void checkWorkflowTaskOwner(JSONObject taskJson, String user)
    throws Exception {
    JSONObject owner = taskJson.getJSONObject("owner");
    assertEquals(user, owner.getString("userName"));
    assertEquals(
      personManager.getFirstName(user),
      owner.getString("firstName")
    );
  ...
  private void checkWorkflowInstance(
    ...
    assertEquals(
      wfInstance.getDefinition().getName(),
      instance.getString("name")
    );
    assertEquals(
      wfInstance.getDefinition().getTitle(),
      instance.getString("title")
    );
    assertEquals(
      wfInstance.getDefinition().getDescription(),
    ...
    assertEquals(
      personManager.getFirstName(USER1),
      initiator.getString("firstName")
    );
    assertEquals(
      personManager.getLastName(USER1),
      initiator.getString("lastName")
    );
  }

  private void checkWorkflowTaskDefinition(
    WorkflowTaskDefinition wfDefinition,
    ...
    assertEquals(
      startType.getTitle(this.dictionaryService),
      type.getString("title")
    );
    assertEquals(
      startType.getDescription(this.dictionaryService),
      type.getString("description")
    );
    ...
    for (int i = 0; i < transitions.length(); i++) {
      ...
      assertEquals(
        ...
        transition.getString("description")
      );
      assertEquals(
        startTransition.isDefault(),
        transition.getBoolean("isDefault")
      );
  ...
  public void testTaskInstancePut() throws Exception {
    ...
    WorkflowTask startTask = workflowService.getStartTask(
      adhocPath.getInstance().getId()
    );
    ...
    Response response = sendRequest(
      new GetRequest(URL_TASKS + "/" + firstTask.getId()),
      200
    );

    JSONObject jsonProperties = new JSONObject(response.getContentAsString())
      .getJSONObject("data")
    ...
    jsonProperties.put(
      qnameToString(WorkflowModel.PROP_COMMENT),
      "Edited comment"
    );
    ...
    jsonProperties.put(
      qnameToString(WorkflowModel.PROP_DUE_DATE),
      ISO8601DateFormat.format(newDueDate.getTime())
    );
    jsonProperties.put(
      qnameToString(WorkflowModel.PROP_DESCRIPTION),
      "Edited description"
    );
    ...
    personManager.setUser(USER3);
    Response unauthResponse = sendRequest(
      new PutRequest(
        URL_TASKS + "/" + firstTask.getId(),
    ...
    personManager.setUser(USER2);
    Response putResponse = sendRequest(
      new PutRequest(
        URL_TASKS + "/" + firstTask.getId(),
        jsonProperties.toString(),
        "application/json"
  ...
  public void testTaskInstancePutCompletedTask() throws Exception {
    ...
    Response getResponse = sendRequest(
      new GetRequest(URL_TASKS + "/" + firstTask.getId()),
      200
    );

    JSONObject jsonProperties = new JSONObject(getResponse.getContentAsString())
      .getJSONObject("data")
      .getJSONObject("properties");
  ...
  public void testWorkflowDefinitionsGet() throws Exception {
    ...
    Response response = sendRequest(
      new GetRequest(URL_WORKFLOW_DEFINITIONS),
      200
    );
    assertEquals(Status.STATUS_OK, response.getStatus());
    ...
    assertTrue(
      "Adhoc workflow definition was not present!",
      adhocWorkflowPresent
    );
    ...
    response =
      sendRequest(
        new GetRequest(URL_WORKFLOW_DEFINITIONS + "?exclude=" + exclude),
        200
      );
    assertEquals(Status.STATUS_OK, response.getStatus());
    ...
    assertFalse(
      "Found adhoc workflow when it was supposed to be excluded",
      adhocWorkflowPresent
    );
  ...
  public void testWorkflowDefinitionGet() throws Exception {
    ...
    WorkflowDefinition wDef = workflowService.getDefinitionByName(
      getAdhocWorkflowDefinitionName()
    );
    ...
    assertEquals(
      getAdhocWorkflowDefinitionName(),
      workflowDefinitionJSON.getString("name")
    );
    ...
    String startTaskDefUrl = workflowDefinitionJSON.getString(
      "startTaskDefinitionUrl"
    );
    assertEquals(
      startTaskDefUrl,
      "api/classes/" + getSafeDefinitionName(ADHOC_START_TASK_TYPE)
    );

    assertTrue(workflowDefinitionJSON.has("startTaskDefinitionType"));
    assertEquals(
      ADHOC_START_TASK_TYPE,
      workflowDefinitionJSON.getString("startTaskDefinitionType")
    );

    // Check task-definitions
    JSONArray taskDefinitions = workflowDefinitionJSON.getJSONArray(
      "taskDefinitions"
    );
    ...
    checkTaskDefinitionTypeAndUrl(
      ADHOC_TASK_COMPLETED_TYPE,
      secondTaskDefinition
    );
  }

  private void checkTaskDefinitionTypeAndUrl(
    ...
    assertEquals(
      "api/classes/" + getSafeDefinitionName(expectedTaskType),
      taskDefinition.getString("url")
    );
  }
  ...
  public void testWorkflowInstanceGet() throws Exception {
    ...
    assertEquals(
      adhocInstance.getDefinition().getName(),
      result.getString("name")
    );
    assertEquals(
      adhocInstance.getDefinition().getTitle(),
      result.getString("title")
    );
    assertEquals(
      adhocInstance.getDefinition().getDescription(),
      result.getString("description")
    );
    assertEquals(adhocInstance.isActive(), result.getBoolean("isActive"));
    assertEquals(
      org.springframework.extensions.surf.util.ISO8601DateFormat.format(
        adhocInstance.getStartDate()
      ),
    ...
    assertEquals(
      adhocInstance.getContext().toString(),
      result.getString("context")
    );
    assertEquals(
      adhocInstance.getWorkflowPackage().toString(),
    ...
    assertEquals(
      adhocDefinition.getDescription(),
      jsonDefinition.getString("description")
    );
    assertEquals(
      adhocDefinition.getVersion(),
      jsonDefinition.getString("version")
    );
    assertEquals(
      adhocDefinition
      ...
      jsonDefinition.getString("startTaskDefinitionType")
    );
  ...
  public void testWorkflowInstancesGet() throws Exception {
    ...
    Response forDefinitionResponse = sendRequest(
      new GetRequest(
        MessageFormat.format(
    ...
    checkFiltering(
      URL_WORKFLOW_INSTANCES +
      "?startedAfter=" +
      ISO8601DateFormat.format(anHourAgo)
    );
    ...
    checkFiltering(
      URL_WORKFLOW_INSTANCES +
      "?startedBefore=" +
      ISO8601DateFormat.format(anHourLater)
    );
    ...
    if (adhocInstance.getEndDate() != null) {
      ...
      checkFiltering(
        URL_WORKFLOW_INSTANCES +
        "?completedAfter=" +
        ISO8601DateFormat.format(adhocInstance.getEndDate())
      );
    ...
    response =
      sendRequest(
        new GetRequest(URL_WORKFLOW_INSTANCES + "?exclude=" + exclude),
        200
      );
    assertEquals(Status.STATUS_OK, response.getStatus());
    jsonStr = response.getContentAsString();
    ...
    assertFalse(
      "Found adhoc workflows when they were supposed to be excluded",
      adhocWorkflowPresent
    );
  }
  ...
  public void testWorkflowInstancesForNodeGet() throws Exception {
    ...
    String url = MessageFormat.format(
      ...
      contentNodeRef.getStoreRef().getProtocol(),
      contentNodeRef.getStoreRef().getIdentifier(),
      contentNodeRef.getId()
    );
  ...
  public void testWorkflowInstanceDeleteAsAdministrator() throws Exception {
    ...
    String admin = authenticationComponent
    ...
    sendRequest(
      new DeleteRequest(URL_WORKFLOW_INSTANCES + "/" + adhocInstance.getId()),
      Status.STATUS_OK
    );

    WorkflowInstance instance = workflowService.getWorkflowById(
      adhocInstance.getId()
    );
    if (instance != null) {
    ...
    List<WorkflowInstance> instances = workflowService.getActiveWorkflows(
      adhocInstance.getDefinition().getId()
    );
  ...
  public void testWorkflowInstanceDelete() throws Exception {
    ...
    sendRequest(
      new DeleteRequest(URL_WORKFLOW_INSTANCES + "/" + instanceId),
  ...
  public void testWorkflowInstanceDeleteAsRecreatedUser() throws Exception {
    ...
    assertNotNull(
      workflowService.getWorkflowById(adhocPath.getInstance().getId())
    );
    ...
    sendRequest(
      new DeleteRequest(
        URL_WORKFLOW_INSTANCES + "/" + adhocPath.getInstance().getId()
      ),
      Status.STATUS_OK
    );
  ...
  protected void runReviewFlow(boolean approve) throws Exception {
    ...
    WorkflowPath reviewPath = workflowService.startWorkflow(
      reviewDef.getId(),
      params
    );
    ...
    WorkflowTask startTask = workflowService
      .getTasksForWorkflowPath(reviewPath.getId())
      .get(0);
  ...
  protected void setUp() throws Exception {
    ...
    namespaceService =
      (NamespaceService) appContext.getBean("NamespaceService");
    workflowService = (WorkflowService) appContext.getBean("WorkflowService");
    MutableAuthenticationService authenticationService = (MutableAuthenticationService) appContext.getBean(
      "AuthenticationService"
    );
    PersonService personService = (PersonService) appContext.getBean(
      "PersonService"
    );
    SearchService searchService = (SearchService) appContext.getBean(
      "SearchService"
    );
    FileFolderService fileFolderService = (FileFolderService) appContext.getBean(
      "FileFolderService"
    );
    nodeService = (NodeService) appContext.getBean("NodeService");

    // for the purposes of the tests make sure workflow engine is enabled/visible.
    WorkflowAdminServiceImpl workflowAdminService = (WorkflowAdminServiceImpl) appContext.getBean(
      "workflowAdminService"
    );
    this.wfTestHelper =
      new WorkflowTestHelper(workflowAdminService, getEngine(), true);

    AuthorityService authorityService = (AuthorityService) appContext.getBean(
      "AuthorityService"
    );
    personManager =
      new TestPersonManager(authenticationService, personService, nodeService);
    groupManager = new TestGroupManager(authorityService);

    authenticationComponent =
      (AuthenticationComponent) appContext.getBean("authenticationComponent");
    dictionaryService =
      (DictionaryService) appContext.getBean("dictionaryService");
    ...
    NodeRef companyHome = searchService
      .selectNodes(
    ...
    contentNodeRef =
      fileFolderService
        .create(
          companyHome,
          TEST_CONTENT + System.currentTimeMillis(),
  ...
  private void compareProperties(JSONObject before, JSONObject after)
      ...
      if (before.has(name)) {
        if (before.get(name) instanceof JSONArray) {
          for (int i = 0; i < before.getJSONArray(name).length(); i++) {
            assertEquals(
              before.getJSONArray(name).get(i),
  ...
  private void checkSimpleWorkflowInstanceResponse(JSONObject json)
    throws JSONException {
  ...
  private void checkPriorityFiltering(String url) throws Exception {
    ...
    for (int i = 0; i < result.length(); i++) {
      ...
      assertEquals(
        2,
        taskObject.getJSONObject("properties").get("bpm_priority")
      );
    }
  }

  private void checkTasksPresent(
    String url,
  ...
  private void checkTasksMatch(String url, String... ids) throws Exception {
    ...
    assertTrue(
      "Expected: " + taskIds + "\nActual: " + resultIds,
      resultIds.containsAll(taskIds)
    );
    assertTrue(
      "Expected: " + taskIds + "\nActual: " + resultIds,
      taskIds.containsAll(resultIds)
    );
  }

  private void checkTasksState(String url, WorkflowTaskState expectedState)
    throws Exception {
    JSONObject json = getDataFromRequest(url);
    JSONArray result = json.getJSONArray("data");
    assertNotNull(result);
  ...
  private void checkTaskPropertyFiltering(
    String propertiesParamValue,
    List<String> expectedProperties
  ) throws Exception {
    JSONObject data = getDataFromRequest(
      MessageFormat.format(
    ...
    JSONObject taskProperties = taskArray
      .getJSONObject(0)
      .getJSONObject("properties");
  ...
  private JSONArray getJsonArray(Response response, int expectedLength)
    throws Exception {
    String jsonStr = response.getContentAsString();
    JSONObject json = new JSONObject(jsonStr);
    JSONArray results = json.getJSONArray("data");

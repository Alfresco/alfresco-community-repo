...
public class SiteContainersCannedQuery extends AbstractCannedQueryPermissions<FileInfo> {
    ...
    public SiteContainersCannedQuery(
            FileFolderService fileFolderService,
            NodeService nodeService,
            CannedQueryParameters parameters,
            MethodSecurityBean<FileInfo> methodSecurity) {
    ...
    protected List<FileInfo> queryAndFilter(CannedQueryParameters parameters) {
        SiteContainersCannedQueryParams paramBean =
                (SiteContainersCannedQueryParams) parameters.getParameterBean();

        NodeRef siteNodeRef = paramBean.getSiteNodeRef();

        // need to get all folders to check for site container aspect since the FileFolderService
        // won't allow us to filter
        ...
        do {
            pagingResults =
                    fileFolderService.list(
                            siteNodeRef, false, true, null, sortProps, pagingRequest);
    ...
    protected List<FileInfo> applyPostQuerySorting(
            List<FileInfo> results, CannedQuerySortDetails sortDetails) {
    ...
    private static class FileInfoComparator implements Comparator<FileInfo> {
        ...
        public int compare(FileInfo o1, FileInfo o2) {
            ...
            for (Pair<? extends Object, SortOrder> pair : sortPairs) {
                if (pair.getFirst()
                        .equals(SiteContainersCannedQueryParams.SortFields.ContainerName)) {

...
public class MimetypeMapTest extends TestCase {
    private static ApplicationContext ctx =
            DataModelTestApplicationContextHelper.getApplicationContext();
     ...
     *
     * <p>Note - The test for checking by filename + content are in the repo project
     *
     * @see org.alfresco.repo.content.MimetypeMapContentTest
     */
    ...
    public void testMisc() throws Exception {
        ...
        assertEquals(
                MimetypeMap.MIMETYPE_TEXT_PLAIN, mimetypeService.getMimetypeIfNotMatches(reader));
        assertEquals(
    ...
    public void testDuplicates() throws Exception {
        setConfigService(
                MIMETYPE_1A
                        ...
                        MIMETYPE_2A
                        + MIMETYPE_2B
                        + // duplicate removes isText
                        MIMETYPE_3A
                        + MIMETYPE_3A); // identical
        ((MimetypeMap) mimetypeService).init();
        ...
        assertFalse(
                "mimetype2 should have be reset to not text", mimetypeService.isText("mimetype2"));
        assertEquals("mimetype2", mimetypeService.getMimetype("ext2a"));
        ...
        ConfigSource configSource =
                new ConfigSource() {
                    ...
                    public List<ConfigDeployment> getConfigDeployments() {
                        String xml =
                                "<alfresco-config area=\"mimetype-map\">  <config"
                                    + " evaluator=\"string-compare\" condition=\"Mimetype Map\">   "
                                    + " <mimetypes>"
                                        + mimetypes
                                        + "    </mimetypes>"
                                        + "  </config>"
                        ...
                        configs.add(
                                new ConfigDeployment(
                                        "name", new ByteArrayInputStream(xml.getBytes())));
                        return configs;
                    }
                };
    ...
    public void testJsonRead() throws Exception {
        ...
        ((MimetypeMap) mimetypeService)
                .setMimetypeJsonConfigDir("alfresco/test/mimetypes/testMimetype.json");
        ((MimetypeMap) mimetypeService).init();
        ...
        assertEquals(
                "There should be 2 more mimetypes from the JSON file", beforeCount + 2, afterCount);

...
public class AuditDAOImpl extends AbstractAuditDAOImpl {
    ...
    protected AuditApplicationEntity getAuditApplicationById(Long id) {
        ...
        params.put("id", id);
        AuditApplicationEntity entity = template.selectOne(SELECT_APPLICATION_BY_ID, params);
        // Done
    ...
    protected AuditApplicationEntity getAuditApplicationByName(String appName) {
        ...
        AuditApplicationEntity entity = template.selectOne(SELECT_APPLICATION_BY_NAME_ID, params);
        // Done
        if (logger.isDebugEnabled()) {
    ...
    protected AuditApplicationEntity createAuditApplication(
            Long appNameId, Long modelId, Long disabledPathsId) {
    ...
    protected AuditEntryEntity createAuditEntry(
            Long applicationId, long time, Long usernameId, Long valuesId) {
    ...
    protected void findAuditEntries(
        ...
        if (searchValue != null) {
            // Look up the ID of the search key
            Pair<Long, Serializable> searchValuePair =
                    propertyValueDAO.getPropertyValue(searchValue);
        ...
        if (maxResults > 0) {
            // Query without getting the values.  We gather all the results and batch-fetch the
            // audited
            // values afterwards.
            final Map<Long, AuditQueryResult> resultsByValueId =
                    new HashMap<Long, AuditQueryResult>(173);
            PropertyFinderCallback propertyFinderCallback =
            ...
            List<AuditQueryResult> rows =
                    template.selectList(
                            SELECT_ENTRIES_WITHOUT_VALUES, params, new RowBounds(0, maxResults));

...
/**
 * This class abstract the support required to set up and query the Acegi context for security
 * enforcement. There are some simple default method implementations to support simple
 * authentication.
 *
 * @author Andy Hind
 */
public abstract class AbstractAuthenticationComponent implements AuthenticationComponent {
    ...
    public void authenticate(String userName, char[] password) throws AuthenticationException {
        if (logger.isTraceEnabled()) {
            logger.trace(
                    "Authenticating user \"" + AuthenticationUtil.maskUsername(userName) + '"');
        }
        if (userName == null) {
    ...
    /**
     * Default unsupported authentication implementation - as of 2.1 this is the best way to
     * implement your own authentication component as it will support guest login - prior to this
     * direct over ride for authenticate(String , char[]) was used. This will still work.
     *
    ...
    public Authentication setCurrentUser(String userName, UserNameValidationMode validationMode) {
        ...
        if (validationMode == UserNameValidationMode.NONE || isSystemUserName(userName)) {
            ...
            CurrentUserCallback callback =
                    validationMode == UserNameValidationMode.CHECK_AND_FIX
                            ? new FixCurrentUserCallback(userName)
                            : new CheckCurrentUserCallback(userName);
            Authentication authentication;
            // If the repository is read only, we have to settle for a read only transaction. Auto
            // user creation
            ...
            if (transactionService.isReadOnly()) {
                authentication =
                        transactionService
                                ...
                                .doInTransaction(callback, true, false);
            }
            // Otherwise,
            // - for check-only mode we want a readable txn or
            // - for check-and-fix mode we want a writeable transaction, so if the current
            // transaction is read only we set the
    ...
    private Authentication setCurrentUserImpl(String userName) throws AuthenticationException {
        ...
        try {
            ...
            if (isGuestUserName(userName)) {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Setting the current user to the guest user of tenant domain \""
                                    + tenantDomain
                                    + '"');
                }
    ...
    private Authentication setGuestUserAsCurrentUser(String tenantDomain)
            throws AuthenticationException {
                                ...
                                if ((identifier = personService.getUserIdentifier(userName)) == null
                                    ...
                                    if (logger.isDebugEnabled()) {
                                        logger.debug(
                                                "User \""
                                                        + AuthenticationUtil.maskUsername(userName)
                                                        + "\" does not exist in Alfresco. Failing"
                                                        + " validation.");
                                    }
                                ...
                                TenantUtil.runAsSystemTenant(
                                        new TenantRunAsWork<String>() {
                                            public String doWork() throws Exception {
                                                                ...
                                                                "User \""
                                                                        + AuthenticationUtil
                                                                                .maskUsername(
                                                                                        userName)
                                                                        + "\" does not exist in"
                                                                        + " Alfresco. Attempting to"
                                                                        + " import / create the"
                                                                        + " user.");
                                                    }
                                                    if (!userRegistrySynchronizer
                                                            .createMissingPerson(userName)) {
                                                        if (logger.isDebugEnabled()) {
                                                            logger.debug(
                                                                    "Failed to import / create user"
                                                                            + " \""
                                                                            + AuthenticationUtil
                                                                                    .maskUsername(
                                                                                            userName)
                                                ...
                                                // Get the person name and use that as the current
                                                // user to line up with permission checks
                                                return (String)
    ...
    /**
     * Sets the user names who for this particular authentication system should be considered
     * administrators by default.
     *
     * @param defaultAdministratorUserNames a set of user names
     */
    ...
    /**
     * Convenience method to allow the administrator user names to be specified as a comma separated
     * list
     *
     ...
     * administrators by default.
     *
     * @param defaultGuestUserNames a set of user names
     */
    ...
    public Authentication setSystemUserAsCurrentUser(String tenantDomain) {
        if (logger.isTraceEnabled()) {
            logger.trace(
                    "Setting the current user to the system user of tenant domain \""
                            + tenantDomain
                            + '"');
        }

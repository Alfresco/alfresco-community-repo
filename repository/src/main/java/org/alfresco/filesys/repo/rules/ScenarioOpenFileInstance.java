 ...
 *
 * <p>1) open(readOnly) 2) close(readOnly)
 *
 * <p>1) open(readOnly) 2) open(readWrite) 3) close(readOnly) 4 close(readWrite) updates the repo
 *
 * <p>1) open(readOnly) 2) open(readWrite) 3) open(readWrite) - does nothing. Increments Open Count.
 * 4) close(readWrite) - does nothing. Decrements Open Count. 5) close(readWrite) - updates the
 * repo. 6) close(readOnly) - closes read only
 *
 * <p>1) open (readWrite) 2) open (readOnly) - file already open for read/write 3) close 4) close
 */
class ScenarioOpenFileInstance
        implements ScenarioInstance, DependentInstance, ScenarioInstanceRenameAware {
            ...
            case OPENING:
                if (operation instanceof OpenFileOperation) {
                    ...
                    if (name.equalsIgnoreCase(o.getName())) {
                        /**
                         * TODO What to do here - one thread is in the middle of opening a file
                         * while another tries to open the same file sleep for a bit? then check
                         * state again? What happens if file closes while sleeping. For now log an
                         * error.
                         */
                    ...
                    if (name.equalsIgnoreCase(c.getName())) {
                        ...
                        if (isReadOnly(file)) {
                                ...
                                if (file instanceof TempNetworkFile) {
                                    ...
                                    commands.add(
                                            new CloseFileCommand(
                                                    ...
                                                    c.getRootNodeRef(),
                                                    c.getPath()));
                                    postCommitCommands.add(
                                            new RemoveTempFileCommand((TempNetworkFile) file));
                                    return new CompoundCommand(commands, postCommitCommands);
                                } else {
                                    return new CloseFileCommand(
                                            c.getName(), file, c.getRootNodeRef(), c.getPath());
                                }
                            }

                            if (logger.isDebugEnabled()) {
                                logger.debug(
                                        "Only decrement count of read only file handle:" + this);
                            }
                            ...
                            if (openReadWriteCount == 1) {
                                ...
                                // postErrorCommands.add(new RemoveNoContentFileOnError(c.getName(),
                                // c.getRootNodeRef(), c.getPath()));

                                if (c.isDeleteOnClose()) {
                                    postCommitCommands.add(
                                            new ReduceQuotaCommand(
                                                    c.getName(),
                                                    file,
                                                    c.getRootNodeRef(),
                                                    c.getPath()));
                                ...
                                return new CompoundCommand(
                                        commands, postCommitCommands, postErrorCommands);
                            }

                            if (logger.isDebugEnabled()) {
                                logger.debug(
                                        "Only decrement count of read write file handle:" + this);
                            }
                    ...
                    if (name != null && name.equalsIgnoreCase(o.getName())) {
                        if (o.getMode() == OpenFileMode.READ_WRITE) {
                            ...
                            if (openReadWriteCount == 0) {
                                ...
                                openReadWriteCount++;
                                logger.debug(
                                        "Return already open read/write file handle from scenario:"
                                                + this);
                            ...
                            if (openReadOnlyCount == 0) {
                                ...
                                openReadOnlyCount++;
                                logger.debug(
                                        "Return already open only file handle from scenario:"
                                                + this);

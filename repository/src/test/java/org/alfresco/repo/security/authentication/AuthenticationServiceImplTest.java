...
public class AuthenticationServiceImplTest {
  ...
  private AuthenticationComponent authenticationComponent = mock(
  ...
  public void testProtectedUserBadPassword() {
    ...
    for (int i = 0; i < attempts; i++) {
      try {
        ...
        if (i < limit) {
          assertTrue(
            "Expected failure from AuthenticationComponent",
            ae == spoofedAE
          );
        } else {
    ...
    verify(authenticationComponent, times(limit))
      .authenticate(USERNAME, PASSWORD);
    assertTrue(
      "The user should be protected.",
      authService.isUserProtected(USERNAME)
    );

    final String protectedUserKey = authService.getProtectedUserKey(USERNAME);
    assertEquals(
      "The number of recorded logins did not match.",
      attempts,
      cache.get(protectedUserKey).getNumLogins()
    );
  ...
  public void testProtectedUserIfUserNamesAreNotCaseSensitive() {
    ...
    doThrow(spoofedAE)
      .when(authenticationComponent)
      .authenticate(USERNAME_CASE_SENSITIVE, PASSWORD);
    for (int i = 0; i < attempts; i++) {
      try {
    ...
    assertTrue(
      "The user should be protected.",
      authService.isUserProtected(USERNAME_CASE_SENSITIVE)
    );
  }
  ...
  public void testProtectedUserIfUserNamesAreCaseSensitive() {
    ...
    verify(authenticationComponent, times(0))
      .authenticate(USERNAME_CASE_SENSITIVE, PASSWORD);
    assertFalse(
  ...
  public void testProtectedUserCanLoginAfterProtection() throws Exception {
    ...
    for (int i = 0; i < attempts; i++) {
      try {
        authService.authenticate(USERNAME, PASSWORD);
        fail(
          "An " + AuthenticationException.class.getName() + " should be thrown."
        );
      } catch (AuthenticationException ae) {
        // normal
      }
    ...
    assertFalse(
      "The user should not be protected any more.",
      authService.isUserProtected(USERNAME)
    );
    assertEquals(
      "The number of recorded logins should stay the same after protection period ends.",
      attempts,
      cache.get(protectedUserKey).getNumLogins()
    );
    ...
    assertNull(
      "The user should be removed from the cache after successful login.",
  ...
  public void testAuthChainWorksIfFirstAuthFails() throws Exception {
    ...
    AuthenticationServiceImpl[] authenticationChain = {
      authService,
      authService2,
    };
    ...
    for (AuthenticationServiceImpl authentication : authenticationChain) {
      assertTrue(
        "The user should be protected.",
        authentication.isUserProtected(USERNAME)
      );
    }
    ...
    assertNotNull(
      "The user should not be removed from the cache for the corresponding authorization service after a failed login.",
      cache.get(authenticationChain[0].getProtectedUserKey(USERNAME))
    );
    assertNull(
      "The user should be removed from the cache for the corresponding authorization service after successful login.",
  ...
  public void testProtectionDisabledBadPassword() {
    ...
    verify(authenticationComponent, times(attempts))
      .authenticate(USERNAME, PASSWORD);

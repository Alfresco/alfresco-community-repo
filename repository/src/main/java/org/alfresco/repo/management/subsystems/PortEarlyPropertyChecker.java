...
public class PortEarlyPropertyChecker implements SubsystemEarlyPropertyChecker {
  ...
  private static final String PORT_CANT_BE_EMPTY_MESSAGE =
    "system.portcheck.err.empty_port";
  private static final String UNABLE_TO_PARSE_PORT_MESSAGE =
    "system.portcheck.err.parse_port";
  private static final String PORT_OUT_OF_BOUNDS_MESSAGE =
    "system.portcheck.err.port_out_of_bounds";
  private static final String PORTS_OUT_OF_BOUNDS_MESSAGE =
    "system.portcheck.err.ports_out_of_bounds";
  private static final String PORT_IN_USE_MESSAGE =
    "system.portcheck.err.port_in_use";
  private static final String HOST_PORT_IN_USE_MESSAGE =
    "system.portcheck.err.host_port_in_use";
  private static final String PORTS_IN_USE_MESSAGE =
    "system.portcheck.err.ports_in_use";
  private static final String UNKNOWN_OR_WRONG_HOST_MESSAGE =
    "system.portcheck.err.unknown_or_wrong_host";
  private static final String NETWORKING_ERROR_MESSAGE =
    "system.portcheck.err.networking_error";
  private static final String PORTS_WITH_NETWORKING_ERRORS_MESSAGE =
    "system.portcheck.err.ports_with_networking_errors";

  private static final Log logger = LogFactory.getLog(
    PortEarlyPropertyChecker.class
  );
   ...
   */
  public PortEarlyPropertyChecker(
  ...
  public void checkPropertyValue(
    ...
    try {
      if (!hasMultiplePorts) {
        ...
        if (portNumber < 1 || portNumber > 65535) {
          createLogAndThrowAnInvalidPropertyValueException(
            ...
            new String[] { subsystemName, "" + portNumber }
          );
          ...
          try {
            ...
            if (ioe instanceof java.net.BindException) {
              if (host == null || "0.0.0.0".equals(host)) {
                ...
                createLogAndThrowAnInvalidPropertyValueException(
                  HOST_PORT_IN_USE_MESSAGE,
                  new String[] { subsystemName, host, "" + portNumber }
                );
              }
            } else if (
              host != null && ioe instanceof java.net.UnknownHostException
            ) {
              createLogAndThrowAnInvalidPropertyValueException(
                UNKNOWN_OR_WRONG_HOST_MESSAGE,
                new String[] { subsystemName, "" + host }
              );
            } else {
              createLogAndThrowAnInvalidPropertyValueException(
                NETWORKING_ERROR_MESSAGE,
        ...
        for (String portStr : ports) {
          ...
          if (portNumber < 1 || portNumber > 65535) {
            portsOutOfBounds =
              appendToErrorString(portsOutOfBounds, ", ", "" + portNumber);
          } else if (shouldCheckForBlockedPort) {
            try {
              checkPort(portNumber, host);
            } catch (IOException ioe) {
              if (ioe instanceof java.net.BindException) {
                portsInUse =
                  appendToErrorString(portsInUse, ", ", "" + portNumber);
              } else {
                portsWithNetworkingErrors =
                  appendToErrorString(
                    portsWithNetworkingErrors,
                    " ; ",
        ...
        if (!portsOutOfBounds.equals("")) {
          String portsOutOfBoundsDisplayMessage = resolveMessage(
            PORTS_OUT_OF_BOUNDS_MESSAGE,
            new String[] { subsystemName, portsOutOfBounds }
          );
        ...
        if (!portsInUse.equals("")) {
          String portsInUseDisplayMessage = resolveMessage(
            PORTS_IN_USE_MESSAGE,
            new String[] { subsystemName, portsInUse }
          );
          ...
          completeErrorDisplayMessage =
            appendToErrorString(
              completeErrorDisplayMessage,
              " | ",
              portsInUseDisplayMessage
            );
        }

        if (!portsWithNetworkingErrors.equals("")) {
          String portsWithNetworkingErrorsDisplayMessage = resolveMessage(
            PORTS_WITH_NETWORKING_ERRORS_MESSAGE,
            new String[] { subsystemName, portsWithNetworkingErrors }
          );
          ...
          completeErrorDisplayMessage =
            appendToErrorString(
              completeErrorDisplayMessage,
              " | ",
              portsWithNetworkingErrorsDisplayMessage
            );
        }
      ...
      createLogAndThrowAnInvalidPropertyValueException(
        UNABLE_TO_PARSE_PORT_MESSAGE,
        new String[] { subsystemName, propertyValue }
      );
    }
  }
  ...
  private String appendToErrorString(
    String stringToAppendTo,
    String separator,
  ...
  private void createLogAndThrowAnInvalidPropertyValueException(
    ...
    InvalidPropertyValueException invalidPropertyValueException = new InvalidPropertyValueException(
      message,
      messageParams
    );

 ...
 *
 * <p>Since content URLs have to be consistent across all stores for reasons of replication and
 * backup, the most important functionality provided is the generation of new content URLs and the
 * checking of existing URLs.
 *
 * <p>Implementations must override either of the <b>getWriter</b> methods; {@link
 * #getWriter(ContentContext)} or {@link #getWriterInternal(ContentReader, String)}.
 *
...
public abstract class AbstractContentStore implements ContentStore {
    ...
    /**
     * Checks that the content conforms to the format <b>protocol://identifier</b> as specified in
     * the contract of the {@link ContentStore} interface.
     *
    ...
    /**
     * Splits the content URL into its component parts as separated by {@link
     * ContentStore#PROTOCOL_DELIMITER protocol delimiter}.
     *
     * @param contentUrl the content URL to split
     * @return Returns the protocol and identifier portions of the content URL, both of which will
     *     not be <tt>null</tt>
    ...
    protected Pair<String, String> getContentUrlParts(String contentUrl) {
        ...
        String identifier =
                contentUrl.substring(index + PROTOCOL_DELIMETER_LENGTH, contentUrl.length());
    ...
    /**
     * Override this method to supply a efficient and direct check of the URL supplied. The default
     * implementation checks whether {@link ContentStore#getReader(String)} throws the {@link
     * UnsupportedContentUrlException} exception.
     *
     * @since 2.1
     */
    @Override
    ...
    /**
     * Implement to supply a store-specific writer for the given existing content and optional
     * target content URL.
     *
     ...
     * @throws UnsupportedContentUrlException if the content URL supplied is not supported by the
     *     store
     * @throws ContentExistsException if the content URL is already in use
     * @throws ContentIOException if an IO error occurs
     * @since 2.1
     */
    protected ContentWriter getWriterInternal(
            ContentReader existingContentReader, String newContentUrl) {
        throw new UnsupportedOperationException(
                "Override getWriterInternal (preferred) or getWriter");
    }

    /**
     * An implementation that does some sanity checking before requesting a writer from the store.
     * If this method is not overridden, then an implementation of {@link
     * #getWriterInternal(ContentReader, String)} must be supplied.
     *
     * @see #getWriterInternal(ContentReader, String)
     * @since 2.1
     */
    ...
    public ContentWriter getWriter(ContentContext context) {
        ...
        if (!isWriteSupported()) {
            ...
            throw new UnsupportedOperationException(
                    "Write operations are not supported by this store: " + this);
        }
    ...
    /**
     * Simple implementation that uses the {@link ContentReader#exists() reader's exists} method as
     * its implementation. Override this method if a more efficient implementation is possible.
     */
    @Override

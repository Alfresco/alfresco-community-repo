...
public class UpdateTagScopesActionExecuterTest extends TestCase {
  ...
  private static final String ACTION_TRACKING_SERVICE_BEAN_NAME =
    "actionTrackingService";

  private static final String UPDATE_TAGSCOPE_ACTION_EXECUTER_BEAN_NAME =
    "update-tagscope";
  ...
  public void setUp() throws Exception {
    ...
    final ServiceRegistry registry = (ServiceRegistry) applicationContext.getBean(
    ...
    actionExecuter =
      (UpdateTagScopesActionExecuter) applicationContext.getBean(
        UPDATE_TAGSCOPE_ACTION_EXECUTER_BEAN_NAME
      );
    ...
    actionTrackingService =
      (ActionTrackingService) applicationContext.getBean(
        ACTION_TRACKING_SERVICE_BEAN_NAME
      );
  ...
  private void createTestContent(
    ...
    List<NodeRef> createdTagScopes
  ) {
    NodeRef rootNode = registry
    ...
    for (int i = 0; i < TAGSCOPE_LAYERS; i++) {
      FileInfo newFolder = fileFolderService.create(
        currentParent,
      ...
      for (int j = 0; j < TEST_DOCUMENTS_AMOUNT; j++) {
        FileInfo newDocument = fileFolderService.create(
          currentParent,
          String.format(TEST_DOCUMENT_NAME_PATTERN, i, j),
          ContentModel.TYPE_CONTENT
        );
        nodeService.addAspect(
          newDocument.getNodeRef(),
  ...
  public void testSimpleTagScopesUpsdate() throws Exception {
    Action tagScopeUpdateAction = actionService.createAction(
      UpdateTagScopesActionExecuter.NAME
    );
    tagScopeUpdateAction.setParameterValue(
      UpdateTagScopesActionExecuter.PARAM_TAG_SCOPES,
      (Serializable) expectedTagScopes
    );
    ...
    ContentData contentData = getTagScopeCacheContentDataProperty(
      taggedTagScope
    );
   ...
   */
  private ContentData getTagScopeCacheContentDataProperty(
    final NodeRef nodeRef
  ) {
    ContentData result = null;

    Serializable contentProperty = transactionService
      .getRetryingTransactionHelper()
      .doInTransaction(
        new RetryingTransactionCallback<Serializable>() {
          ...
          public Serializable execute() throws Throwable {
            return nodeService.getProperty(
              nodeRef,
              ContentModel.PROP_TAGSCOPE_CACHE
            );
          }
        },
        false,

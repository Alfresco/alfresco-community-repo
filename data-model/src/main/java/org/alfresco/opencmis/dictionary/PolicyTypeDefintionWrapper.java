...
public class PolicyTypeDefintionWrapper extends AbstractTypeDefinitionWrapper {
    ...
    public PolicyTypeDefintionWrapper(
        ...
        if (BaseTypeId.CMIS_POLICY.value().equals(typeId)) {
            ...
            if (parentQName == null) {
                ...
                typeDef.setParentTypeId(
                        cmisMapping.getCmisTypeId(BaseTypeId.CMIS_POLICY, parentQName));
            } else {
                throw new IllegalStateException(
                        "The CMIS type model should ignore aspects that inherit from excluded"
                                + " aspects");
            }
        }
    ...
    @Override
    public List<TypeDefinitionWrapper> connectParentAndSubTypes(
            CMISMapping cmisMapping,
        ...
        if (typeDef.getParentTypeId() != null) {
            ...
            //            if(registry.getTenant() != null && parent != null &&
            // registry.getTypeDefByTypeId(parentTypeId, false) == null)
            //            {
            //            	// this is a tenant registry and the parent is not defined locally so add
            // this type as a child of it
        ...
        if (isBaseType()) {
            ...
            childrenNames =
                    dictionaryService.getSubAspects(
                            cmisMapping.getAlfrescoClass(getAlfrescoName()), false);
        }
            ...
            if (cmisMapping.isValidCmisPolicy(childName)) {
                ...
                if (child == null) {
                    throw new AlfrescoRuntimeException(
                            "Failed to retrieve sub type for type id "
                                    + childName
                                    + " for parent type "
                                    + getAlfrescoName()
                                    + "!");
    ...
    public void resolveInheritance(
        ...
        if (parent != null) {
            for (PropertyDefinitionWrapper propDef : parent.getProperties(false)) {
                ...
                org.alfresco.service.cmr.dictionary.PropertyDefinition alfrescoPropDef =
                        dictionaryService.getProperty(
                                propDef.getOwningType().getAlfrescoName(),
                                propDef.getAlfrescoName());
                ...
                if (propertyDefintion != null) {
                    registerProperty(
                            new BasePropertyDefintionWrapper(
                                    propertyDefintion,
                                    alfrescoPropDef.getName(),
                                    propDef.getOwningType(),
                                    propDef.getPropertyAccessor(),
                                    propDef.getPropertyLuceneBuilder()));
                }
            }
        }
        ...
        for (TypeDefinitionWrapper child : children) {
            if (child instanceof AbstractTypeDefinitionWrapper) {
                ((AbstractTypeDefinitionWrapper) child)
                        .resolveInheritance(cmisMapping, registry, dictionaryService);
            }
        }
    }

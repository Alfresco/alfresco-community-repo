...
/**
 * An {@link JobHandler} which executes activiti timer-jobs authenticated against Alfresco. It runs
 * the timer execution as the task's assignee (if any) when the timer is applied to a task. If not,
 * system user is used to execute timer.
 *
 * <p>It wraps another JobHandler to which the actual execution is delegated to.
 *
...
public class AuthenticatedTimerJobHandler implements JobHandler {
     ...
     *     when only tenant is known. We can't use initiator ScriptNode for this, because this uses
     *     the protected {@link NodeService} which requires an authenticated user in that tenant
     *     (see {@link #getInitiator(ActivitiScriptNode)}).
     */
    ...
    public void execute(
        ...
        final ActivitiScriptNode initiatorNode =
                (ActivitiScriptNode) execution.getVariable(WorkflowConstants.PROP_INITIATOR);

        // Extracting the properties from the initiatornode should be done in correct tennant or as
        // administrator, since we don't
        // know who started the workflow yet (We can't access node-properties when no valid
        ...
        if (userName == null) {
            ...
            if (targetActivity != null) {
                ...
                String activityType =
                        (String) targetActivity.getProperty(ActivitiConstants.NODE_TYPE);
                if (ActivitiConstants.USER_TASK_NODE_TYPE.equals(activityType)) {
                    Task task =
                            new TaskQueryImpl(commandContext)

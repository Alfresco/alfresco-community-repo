...
public class LockDAOImpl extends AbstractLockDAOImpl {
  ...
  private static final String SELECT_LOCKRESOURCE_BY_QNAME =
    "alfresco.lock.select_LockResourceByQName";
  private static final String SELECT_LOCK_BY_ID =
    "alfresco.lock.select_LockByID";
  private static final String SELECT_LOCK_BY_KEY =
    "alfresco.lock.select_LockByKey";
  private static final String SELECT_LOCK_BY_SHARED_IDS =
    "alfresco.lock.select_LockBySharedIds";
  private static final String INSERT_LOCKRESOURCE =
    "alfresco.lock.insert.insert_LockResource";
  ...
  private static final String UPDATE_EXCLUSIVE_LOCK =
    "alfresco.lock.update_ExclusiveLock";
  ...
  @Override
  protected LockResourceEntity getLockResource(
    Long qnameNamespaceId,
    ...
    lockResource.setQnameLocalName(
      qnameLocalName == null ? null : qnameLocalName.toLowerCase()
    );
    lockResource =
      template.selectOne(SELECT_LOCKRESOURCE_BY_QNAME, lockResource);
    // Could be null
    return lockResource;
  }

  @Override
  protected LockResourceEntity createLockResource(
    Long qnameNamespaceId,
    String qnameLocalName
  ) {
    LockResourceEntity lockResource = new LockResourceEntity();
  ...
  @Override
  protected List<LockEntity> getLocksBySharedResourceIds(
    List<Long> sharedLockResourceIds
  ) {
    List<LockEntity> locks = template.selectList(
      SELECT_LOCK_BY_SHARED_IDS,
  ...
  @Override
  protected LockEntity updateLock(
    ...
    updateLockEntity.setExclusiveResourceId(
      lockEntity.getExclusiveResourceId()
    );
    updateLockEntity.setLockToken(
      lockToken == null ? null : lockToken.toLowerCase()
    );
    ...
    if (updated != 1) {
      ...
      throw new ConcurrencyFailureException(
        "Incorrect number of rows affected for updateLock: " +
        updateLockEntity +
        ": expected 1, actual " +
        updated
      );
  ...
  protected int updateLocks(
    ...
    params.put(
      "newLockToken",
      newLockToken == null ? null : newLockToken.toLowerCase()
    );
    long now = (timeToLive > 0) ? System.currentTimeMillis() : 0L;
    long exp = (timeToLive > 0) ? (now + timeToLive) : 0L;

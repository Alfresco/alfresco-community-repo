...
public class NodesContentAndVersioningTests extends RestTest {
    ...
    public void checkFileNameWithRegularCharsInHeader() throws Exception {
        ...
        restClient.assertHeaderValueContains(
                "Content-Disposition", String.format("filename=\"%s\"", file1.getName()));
    }
    ...
    @TestRail(
            ...
            description =
                    "Verify file name with special chars is escaped in Content-Disposition header")
    ...
    @TestRail(
            ...
            executionType = ExecutionType.SANITY,
            description =
                    "Verify that alfresco returns the correct encoding for files created via REST.")
    public void verifyFileEncodingUsingRestAPI() throws Exception {
        ...
        STEP(
                "2. Using multipart data upload (POST nodes/{nodeId}/children) the UTF-8 encoded"
                        + " file.");
        restClient
                .authenticateUser(user1)
                .configureRequestSpec()
                .addMultiPart("filedata", Utility.getResourceTestDataFile("UTF-8File.txt"));
        ...
        restClient
                .configureRequestSpec()
                .addMultiPart("filedata", Utility.getResourceTestDataFile("iso8859File.txt"));
        fileNode = restClient.withCoreAPI().usingNode(folder).createNode();
        restClient.assertStatusCodeIs(HttpStatus.CREATED);
        iso8859File.setNodeRef(fileNode.getId());

        STEP(
                "4. Retrieve the nodes and verify that the content type is the expected one (GET"
                        + " nodes/{nodeId}).");
        restClient
                .withCoreAPI()
                .usingNode(utf8File)
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that the node content is streamed directly to the client and not"
                            + " buffered in memory.")
    public void verifyUploadDownloadLargeFileUsingRestAPI() throws Exception {
        ...
        STEP(
                "3. Retrieve the content of the node without running out of memory (GET"
                        + " nodes/{nodeId}/content).");
        RestResponse nodeContent =
                restClient.withCoreAPI().usingNode(largeFileModel).getNodeContent();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        assertEquals(
    ...
    public void testUpdateNodeContent() throws Exception {
        STEP(
                "1. Retrieve the node in order to get data to compare after update GET"
                        + " /nodes/{nodeId}?include=path.");
        RestNodeModel initialNode =
                restClient
                        .authenticateUser(user1)
                        .withCoreAPI()
                        .usingNode(file1)
        ...
        STEP(
                "2. Update the node content (different from the initial one) PUT"
                        + " /nodes/{nodeId}/content?majorVersion=true&name=newfile.txt.");
        File updatedConentFile = Utility.getResourceTestDataFile("sampleContent.txt");
        RestNodeModel updatedBodyNode =
                restClient
                        .withCoreAPI()
                        .usingNode(file1)
        ...
        String initialNodeVersion =
                new JSONObject(initialNode.toJson())
                        .getJSONObject("properties")
                        .getString("cm:versionLabel");
        String updatedBodyNodeVersion =
                new JSONObject(updatedBodyNode.toJson())
                        .getJSONObject("properties")
                        .getString("cm:versionLabel");
    ...
    public void testCopyNode() throws Exception {
        ...
        STEP(
                "2. With another user(that has access to the file), copy the node to another path"
                        + " POST /nodes/{nodeId}/copy?include=path,isLocked.");
        String postBody = JsonBodyGenerator.keyValueJson("targetParentId", site2.getGuid());
        RestNodeModel copiedNode =
                restClient
                        .authenticateUser(user2)
                        .withCoreAPI()
                        .usingNode(file1)
        ...
        STEP(
                "3. ParentId, createdAt, path and lock are different, but the nodes have the same"
                        + " contentSize.");
    ...
    public void testGetVersionContent() throws Exception {
        ...
        STEP(
                "1. Update the node content in order to increase version(one minor and one major)"
        ...
        STEP(
                "2. List node version history GET /nodes/{nodeId}/versions. And verify that first"
                        + " (in the list) version is 2.0 and last is 1.0.");
        RestVersionModelsCollection versionListing =
                restClient.withCoreAPI().usingNode(file2).listVersionHistory();
        restClient.assertStatusCodeIs(HttpStatus.OK);
        ...
        STEP(
                "3. List node version using skipCount(1),maxItems(1) and include(properties) GET"
                        + " /nodes/{nodeId}/versions?include=properties&skipCount=1&maxItems=1");
        versionListing =
                restClient
                        .withCoreAPI()
        ...
        STEP(
                "4. Get version information for version 1.1 GET"
                        + " /nodes/{nodeId}/versions/{versionId} .");
        RestVersionModel version11 =
                restClient.withCoreAPI().usingNode(file2).getVersionInformation("1.1");
        restClient.assertStatusCodeIs(HttpStatus.OK);
        ...
        assertEquals(
                version.getContent().getSizeInBytes(), version11.getContent().getSizeInBytes());
        ...
        Utility.sleep(
                ...
                60000,
                () -> {
                    RestResponse versionContent =
                            restClient.withCoreAPI().usingNode(file2).getVersionContent("2.0");
                    restClient.assertStatusCodeIs(HttpStatus.OK);

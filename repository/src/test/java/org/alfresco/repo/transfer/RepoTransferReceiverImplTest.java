...
public class RepoTransferReceiverImplTest extends BaseAlfrescoSpringTest {
  ...
  private static final Log log = LogFactory.getLog(
    RepoTransferReceiverImplTest.class
  );
  ...
  public void before() throws Exception {
    ...
    this.receiver =
      (RepoTransferReceiverImpl) this.applicationContext.getBean(
          "transferReceiver"
        );
  ...
  public void testNodeDeleteAndRestore() throws Exception {
    ...
    final TransferServicePolicies.OnEndInboundTransferPolicy mockedPolicyHandler = mock(
    ...
    RetryingTransactionCallback<Void> validateFirstCB = new RetryingTransactionCallback<Void>() {
      ...
      public Void execute() throws Throwable {
        ...
        ArgumentCaptor<String> transferIdCaptor = ArgumentCaptor.forClass(
          String.class
        );
        ArgumentCaptor<Set> createdNodesCaptor = ArgumentCaptor.forClass(
          Set.class
        );
        ArgumentCaptor<Set> updatedNodesCaptor = ArgumentCaptor.forClass(
          Set.class
        );
        ArgumentCaptor<Set> deletedNodesCaptor = ArgumentCaptor.forClass(
          Set.class
        );
    ...
    RetryingTransactionCallback<Void> validateSecondCB = new RetryingTransactionCallback<Void>() {
      ...
      public Void execute() throws Throwable {
        ...
        log.debug(
          "Successfully tested existence of archive node: " + archiveNode8
        );
        ...
        log.debug(
          "Successfully tested existence of archive node: " + archiveNode2
        );
        ...
        log.debug(
          "Testing existence of original node: " + tc.node8.getNodeRef()
        );
        assertFalse(nodeService.exists(tc.node8.getNodeRef()));

        log.debug(
          "Testing existence of original node: " + tc.node2.getNodeRef()
        );
        assertFalse(nodeService.exists(tc.node2.getNodeRef()));

        log.debug(
          "Testing existence of original node: " + tc.node11.getNodeRef()
        );
  ...
  public void testJira_ALF_2772() throws Exception {
    ...
    RetryingTransactionCallback<Void> doSecondCB = new RetryingTransactionCallback<Void>() {
      ...
      public Void execute() throws Throwable {
        tc.transferId = receiver.start("1234", true, receiver.getVersion());
        String snapshot = createSnapshot(
          Arrays.asList(new TransferManifestNode[] { tc.deletedNode11 })
        );
        log.debug(snapshot);
        receiver.saveSnapshot(
          tc.transferId,
    ...
    RetryingTransactionCallback<Void> doThirdCB = new RetryingTransactionCallback<Void>() {
      ...
      public Void execute() throws Throwable {
        tc.transferId = receiver.start("1234", true, receiver.getVersion());
        String snapshot = createSnapshot(
          Arrays.asList(new TransferManifestNode[] { tc.node2, tc.node11 })
        );
        log.debug(snapshot);
        receiver.saveSnapshot(
          tc.transferId,
  ...
  public void testMNT11057() throws Exception {
    ...
    String transferIdA1 = receiver.start(
      "transferFromRepoA1",
      true,
      receiver.getVersion()
    );
    ...
    String transferIdB1 = receiver.start(
      "transferFromRepoB1",
      true,
      receiver.getVersion()
    );
    ...
    log.info("has Alien");
    log.info(
      nodeService.hasAspect(folder1A1.getNodeRef(), TransferModel.ASPECT_ALIEN)
    );

    assertTrue(nodeService.exists(folder2A1.getNodeRef()));
    log.info("has Alien");
    assertFalse(
      nodeService.hasAspect(folder2A1.getNodeRef(), TransferModel.ASPECT_ALIEN)
    );
    ...
    log.info("has Alien");
    assertTrue(
      nodeService.hasAspect(folder3B1.getNodeRef(), TransferModel.ASPECT_ALIEN)
    );
  ...
  public void testAsyncCommitWithSummaryReport() throws Exception {
    ...
    Properties properties = (Properties) this.applicationContext.getBean(
        "global-properties"
      );
    ...
    try {
      ...
      assertTrue(
        Boolean.parseBoolean(
          properties.getProperty(TransferCommons.TS_SIMPLE_REPORT)
        )
      );
  ...
  public void testAsyncCommitWithOutSummaryReport() throws Exception {
    ...
    try {
      properties.put(
        TransferCommons.TS_SIMPLE_REPORT,
  ...
  private void localTestDestinationReports(boolean testAlsoForSummaryReport)
    ...
    // generated
    FileFolderService fileFolderService = (FileFolderService) this.applicationContext.getBean(
        "fileFolderService"
      );
    ...
    String destinationReportName = (String) nodeService
      .getProperties(destinationReportNodeRef)
      .get(ContentModel.PROP_NAME);
    String destinationReportWithoutExtension = destinationReportName.substring(
      0,
      destinationReportName.lastIndexOf(".")
    );
    ...
    summaryReportsParentFolder =
      getSummaryReportsParentFolder(summaryReportsLocation);

    assertTrue(
      fileFolderService.getFileInfo(summaryReportsParentFolder).isFolder()
    );

    FileInfo simplifiedReportFile = null;
    for (FileInfo fi : fileFolderService.list(summaryReportsParentFolder)) {
      if (
        fi.getName().startsWith(destinationReportWithoutExtension) &&
  ...
  private NodeRef getSummaryReportsParentFolder(
    ...
    if (refs.size() > 0) {
      ...
      log.debug(
        "Found transfer summary report records folder: " + reportParentFolder
      );
    }
   ...
   * @return
   */
  private TransferManifestDeletedNode createDeletedNode(
    TransferManifestNode nodeToDelete
  ) {
  ...
  private void moveNode(
    TransferManifestNormalNode childNode,
    TransferManifestNormalNode newParent
  ) {
    ...
    for (ChildAssociationRef parent : currentParents) {
      if (!parent.isPrimary()) {
        ...
        ChildAssociationRef newPrimaryAssoc = new ChildAssociationRef(
          ContentModel.ASSOC_CONTAINS,
        ...
        newParentPath.append(
          new Path.ChildAssocElement(newParent.getPrimaryParentAssoc())
        );
  ...
  private void associatePeers(
    TransferManifestNormalNode source,
    TransferManifestNormalNode target
  ) {
    ...
    AssociationRef newAssoc = new AssociationRef(
      null,
      source.getNodeRef(),
      ContentModel.ASSOC_ATTACHMENTS,
      target.getNodeRef()
  ...
  private String createSnapshot(List<TransferManifestNode> nodes)
    throws Exception {
   ...
   * @return
   */
  private TransferManifestNormalNode createContentNode(/*String transferId*/)
    throws Exception {
  ...
  private TransferManifestNormalNode createFolderNode(String folderName)
    throws Exception {
    TransferManifestNormalNode node = new TransferManifestNormalNode();
    String uuid = GUID.generate();
    NodeRef nodeRef = new NodeRef(
    ...
    String nodeName = folderName == null
      ? uuid + ".folder" + getNameSuffix()
      : folderName;

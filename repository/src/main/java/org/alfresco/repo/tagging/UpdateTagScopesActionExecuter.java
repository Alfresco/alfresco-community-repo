...
public class UpdateTagScopesActionExecuter extends ActionExecuterAbstractBase {
  ...
  private static final Log logger = LogFactory.getLog(
    UpdateTagScopesActionExecuter.class
  );
  ...
  protected void executeImpl(
    ...
    try {
      ...
      List<NodeRef> tagScopeNodes = (List<NodeRef>) action.getParameterValue(
        PARAM_TAG_SCOPES
      );
      if (logger.isDebugEnabled()) {
        logger.debug(
          "About to process tag scope updates for scopes " + tagScopeNodes
        );
      }
                    ...
                    .doInTransaction(
                      new RetryingTransactionCallback<Void>() {
                        public Void execute() throws Throwable {
                          ...
                          List<Long> entryIds = searchForUpdates(
                            tagScope,
                            updates
                          );
                          ...
                          if (logger.isDebugEnabled()) {
                            if (updates.size() > 0) {
                              ...
                              logger.debug(
                                "All updates now processed for tag scope " +
                                tagScope
                              );
                            } else {
                              logger.debug(
                                "No updates needed for tag scope " + tagScope
                              );
                            }
                          }
  ...
  private List<Long> searchForUpdates(
    final NodeRef tagScopeNode,
    final Map<String, Integer> updates
  ) {
    // Build the query
    final AuditQueryParameters params = new AuditQueryParameters();
    params.setApplicationName(
      ...
      new AuditQueryCallback() {
        ...
        public boolean handleAuditEntry(
          ...
          if (values.containsKey(tagsPath)) {
            ...
            HashMap<String, Integer> changes = (HashMap<String, Integer>) values.get(
              tagsPath
            );
      ...
      if (tags.isEmpty()) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "Updated tag scope: '" +
            tagScopeNode +
            "'. No tags were found. Emptying tags cache by setting content property to null..."
          );
        }
      } else {
        // Order the list
              ...
              new AuditQueryCallback() {
                ...
                public boolean handleAuditEntry(
                  ...
                  if (values.containsKey(noderefPath)) {
                    ...
                    if (!tagNodesStrs.contains(nodeRefStr)) tagNodesStrs.add(
                      nodeRefStr
                    );
                  } else {
                    logger.warn(
                      "Unexpected Tag Scope update entry of " + values
                    );
                  }

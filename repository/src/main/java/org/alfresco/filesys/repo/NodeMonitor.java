...
public class NodeMonitor
  ...
  public final void init() {
    ...
    m_policyComponent.bindClassBehaviour(
    ...
    String rootPath = (String) m_nodeService.getProperty(
      m_filesysCtx.getRootNode(),
      ContentModel.PROP_NAME
    );

    StringBuilder pathBuilder = new StringBuilder();
    pathBuilder.append("/");
    if (rootPath != null && rootPath.length() > 0) pathBuilder.append(rootPath);
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Node monitor filesystem=" +
      m_filesysCtx.getDeviceName() +
      ", rootPath=" +
      m_rootPath
    );
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Node monitor installed for " + m_filesysCtx.getDeviceName()
    );
  }
  ...
  public void onCreateNode(ChildAssociationRef childAssocRef) {
    ...
    if (fType != FileFolderServiceType.INVALID) {
      ...
      String relPath = nodePath.toDisplayPath(
        m_nodeService,
        m_permissionService
      );
      String fName = (String) m_nodeService.getProperty(
        nodeRef,
        ContentModel.PROP_NAME
      );
  ...
  public void onUpdateProperties(
    ...
    if (fType != FileFolderServiceType.INVALID) {
      ...
      if (beforeName != null && !beforeName.equals(afterName)) {
        ...
        ChildAssociationRef childAssocRef = m_nodeService.getPrimaryParent(
          nodeRef
        );
        String relPath2 = buildRelativePathString(
          childAssocRef.getParentRef(),
          beforeName
        );
        String relPath3 = buildRelativePathString(
          childAssocRef.getParentRef(),
          afterName
        );
  ...
  public void onMoveNode(
    ...
    if (fType != FileFolderServiceType.INVALID) {
      ...
      String relPath = buildRelativePathString(
        oldChildAssocRef.getParentRef(),
        fName
      );
      String relPath2 = buildRelativePathString(
        newChildAssocRef.getParentRef(),
        fName
      );
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "OnMoveNode: nodeRef=" + oldNodeRef + ", relPath=" + relPath
      );
      ...
      if (relPath.startsWith(m_rootPath)) {
        ...
        NodeEvent nodeEvent = new MoveNodeEvent(
          fType,
          oldNodeRef,
          relPath,
          relPath2
  ...
  public void beforeDeleteNode(NodeRef nodeRef) {
    ...
    if (fType != FileFolderServiceType.INVALID) {
      ...
      if (relPath.startsWith(m_rootPath)) {
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "BeforeDeleteNode: nodeRef=" + nodeRef + ", relPath=" + relPath
        );
      }
    }
  }
    ...
    return AuthenticationUtil.runAs(
      new RunAsWork<StringBuilder>() {
        ...
        public StringBuilder doWork() throws Exception {
          ...
          result.append(
            nodePath.toDisplayPath(m_nodeService, m_permissionService)
          );
          if (
            (0 == result.length()) ||
            (
   ...
   * @return String
   */
  private String buildRelativePathString(
    ...
    pathStr
      .append(
        (String) m_nodeService.getProperty(
          parentNodeRef,
          ContentModel.PROP_NAME
        )
      )
  ...
  private void fireNodeEvent(NodeEvent nodeEvent) {
    List<NodeEvent> events = TransactionalResourceHelper.getList(
      FileSysNodeEvent
    );
    ...
    while (m_shutdown == false) {
      try {
        ...
        m_transService
          ...
          .doInTransaction(processEventCallback, true, true);
      } catch (InterruptedException ex) {} catch (Throwable e) {
    ...
    if (relPath.startsWith(m_rootPath)) {
      ...
      if (m_stateTable != null) {
        ...
        if (fState != null && fState.exists() == false) {
          ...
          if (
            createEvent.getFileType() == FileFolderServiceType.FILE
          ) fState.setFileStatus(
            FileStatus.FileExists
          ); else fState.setFileStatus(FileStatus.DirectoryExists);

          // DEBUG

          if (logger.isDebugEnabled()) logger.debug(
            "CreateNode updated file state - " + fState
          );
        }
      }
      ...
      if (m_filesysCtx.hasChangeHandler()) {
        ...
        if (m_filesysCtx.getChangeHandler().getGlobalNotifyMask() != 0) {
          ...
          if (
            ...
            .getChangeHandler()
            .notifyFileChanged(
              NotifyChange.ActionAdded,
              relPath
            ); else m_filesysCtx
            .getChangeHandler()
            .notifyDirectoryChanged(NotifyChange.ActionAdded, relPath);

          // DEBUG

          if (logger.isDebugEnabled()) logger.debug(
            "CreateNode queued change notification"
          );
        }
      }
    } else {
  ...
  private final void processDeleteNode(DeleteNodeEvent deleteEvent) {
    ...
    String relPath = deleteEvent
      .getPath()
      .substring(m_rootPath.length())
      .replace('/', '\\');

    // DEBUG

    if (logger.isDebugEnabled()) logger.debug(
      "DeleteNode nodeRef=" + deleteEvent.getNodeRef() + ", path=" + relPath
    );
    ...
    if (m_stateTable != null) {
      ...
      if (fState != null && fState.exists() == true) {
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "DeleteNode updated file state - " + fState
        );
      }
    }
    ...
    if (m_filesysCtx.hasChangeHandler()) {
      ...
      if (m_filesysCtx.getChangeHandler().getGlobalNotifyMask() != 0) {
        ...
        if (
          deleteEvent.getFileType() == FileFolderServiceType.FILE
        ) m_filesysCtx
          .getChangeHandler()
          .notifyFileChanged(
            NotifyChange.ActionRemoved,
            relPath
          ); else m_filesysCtx
          .getChangeHandler()
          .notifyDirectoryChanged(NotifyChange.ActionRemoved, relPath);

        // DEBUG

        if (logger.isDebugEnabled()) logger.debug(
          "DeleteNode queued change notification"
        );
      }
    }
  }
  ...
  private final void processMoveNode(MoveNodeEvent moveEvent) {
    ...
    String fromPath = moveEvent
      .getFromPath()
      .substring(m_rootPath.length())
      .replace('/', '\\');
    String toPath = moveEvent
      .getToPath()
      .substring(m_rootPath.length())
      .replace('/', '\\');
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "MoveNode fromPath=" + fromPath + ", toPath=" + toPath
    );
    ...
    if (m_stateTable != null) {
      ...
      if (fState != null && fState.exists() == true) {
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "MoveNode updated state for fromPath=" + fromPath
        );
      }
      ...
      if (fState != null && fState.exists() == false) {
        ...
        if (
          moveEvent.getFileType() == FileFolderServiceType.FILE
        ) fState.setFileStatus(
          FileStatus.FileExists
        ); else fState.setFileStatus(FileStatus.DirectoryExists);
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "MoveNode updated state for toPath=" + toPath
        );
      }
    }
    ...
    if (m_filesysCtx.hasChangeHandler()) {
      ...
      if (m_filesysCtx.getChangeHandler().getGlobalNotifyMask() != 0) {
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "MoveNode queued change notification"
        );
      }
    }
  }
  ...
  private final void processLockNode(LockNodeEvent lockEvent) {
    ...
    if (relPath.startsWith(m_rootPath)) {
      ...
      if (m_filesysCtx.hasChangeHandler()) {
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "LockNode queued change notification"
        );
      }
    }
  }

 ...
 *
 * <p>Unfortunately, we are tied to the PreferencesService and to the preference names and data
 * structure because Share uses the PreferenceService directly.
 *
 * @author steveglover
 */
public class FavouritesServiceImpl implements FavouritesService, InitializingBean {
    ...
    public interface OnAddFavouritePolicy extends ClassPolicy {
        public static final QName QNAME =
                QName.createQName(NamespaceService.ALFRESCO_URI, "onAddfavourite");
    ...
    public interface OnRemoveFavouritePolicy extends ClassPolicy {
        public static final QName QNAME =
                QName.createQName(NamespaceService.ALFRESCO_URI, "onRemovefavourite");
    ...
    private Comparator<PersonFavouriteKey> getComparator(
        ...
        Comparator<PersonFavouriteKey> comparator =
                new Comparator<PersonFavouriteKey>() {
                    @Override
                    public int compare(PersonFavouriteKey o1, PersonFavouriteKey o2) {
                        ...
                        if (ret == 0) {
                            // some favourites may not have a createdAt value, rendering this
                            // comparator less selective.
                            // If the favourites are still regarded as the same, differentiate on
                            // nodeRef.
    ...
    private PersonFavourite addFavouriteSite(String userName, NodeRef nodeRef) {
        ...
        if (siteInfo != null) {
            ...
            if (favourite == null) {
                ...
                // ISO8601 string format: PreferenceService works with strings only for dates it
                // seems
    ...
    private PersonFavourite getFavouriteSite(String userName, SiteInfo siteInfo) {
        ...
        if (isFavourited) {
            String createdAtStr =
                    (String) preferenceService.getPreference(userName, siteCreatedAtKey);
        ...
        if (isFavourite) {
            ...
            try {
                ...
                if (siteInfo != null) {
                    ...
                    PersonFavourite personFavourite =
                            new PersonFavourite(
                                    userName, siteInfo.getNodeRef(), Type.SITE, siteId, createdAt);
    ...
    private PersonFavourite addFavouriteDocumentOrFolder(
            String userName, Type type, NodeRef nodeRef) {
        Map<PersonFavouriteKey, PersonFavourite> personFavourites =
                getFavouriteNodes(userName, type);
    ...
    private boolean isFavouriteNode(String userName, Type type, NodeRef nodeRef) {
        Map<PersonFavouriteKey, PersonFavourite> personFavourites =
                getFavouriteNodes(userName, type);
        PersonFavouriteKey personFavouriteKey =
                new PersonFavouriteKey(userName, null, type, nodeRef);
    ...
    public PagingResults<PersonFavourite> getPagedFavourites(
        ...
        if (authenticationContext.isSystemUserName(currentUserName) == true
            ...
            // we may have more than one favourite that is considered the same w.r.t. the
            // PersonFavourite comparator
            ...
            for (String key : preferences.keySet()) {
                if (includeFiles && key.startsWith(documentsPrefKeys.sharePrefKey)) {
                    ...
                    // key is either of the form
                    // org.alfresco.share.sites.favourites.<siteId>.favourited or
                    // org.alfresco.share.sites.favourites.<siteId>
                    extractFavouriteSite(
                            userName, Type.SITE, sortedFavouriteNodes, preferences, key);
                }
            }
            ...
            final List<PersonFavourite> page =
                    new ArrayList<PersonFavourite>(pageDetails.getPageSize());
            ...
            // The current user does not have sufficient permissions to update the preferences for
            // this user
            throw new AccessDeniedException(
                    "The current user "
                            + currentUserName
                            + " does not have sufficient permissions to get the favourites of the"
                            + " user "
                            + userName);
        }
    }

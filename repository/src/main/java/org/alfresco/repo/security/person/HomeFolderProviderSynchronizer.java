 ...
 * {@link HomeFolderProvider2} or extend the now depreciated {@link AbstractHomeFolderProvider}.
 * Only users that use a HomeFolderProvider2 that don't provide a shared home folder (all user are
 * given the same home folder) will be moved. This allows existing home directories to be moved to
 * reflect changes in policy related to the location of home directories. Originally created for
 * ALF-7797 which related to the need to move large numbers of existing home directories created via
 * an LDAP import into a hierarchical folder structure with fewer home folder in each.
 *
 * <p>By default no action is taken unless the the global property {@code
 * home_folder_provider_synchronizer.enabled=true}.
 *
 * <p>The home folders for internal users (such as {@code admin} and {@code guest}) that use {@code
 * guestHomeFolderProvider} or {@code bootstrapHomeFolderProvider} are not moved, nor are any users
 * that use HomeFolderProviders create shared home folders (all user are given the same home
 * folder).
 *
 * <p>It is also possible change the HomeFolderProvider used by all other users by setting the
 * global property {@code home_folder_provider_synchronizer.override_provider=<providerBeanName>}.
 *
 * <p><b>Warning:</b> The LDAP synchronise process overwrites the home folder provider property.
 * This is not an issue as long as the root path of the overwriting provider is the same as the
 * overwritten provider or is not an ancestor of any of the existing home folders. This is important
 * because the root directory value is used by this class to tidy up empty 'parent' folders under
 * the root when a home folders are moved elsewhere. If you have any concerns that this may not be
 * true, set the global property {@code home_folder_provider_synchronizer.keep_empty_parents=true}
 * and tidy up any empty folders manually. Typically users created by the LDAP sync process are all
 * placed under the same root folder so there will be no parent folders anyway.
 *
 * <p>
 *
 * @author Alan Davis
...
public class HomeFolderProviderSynchronizer extends AbstractLifecycleBean {
    ...
    private static final Log batchLogger =
            LogFactory.getLog(HomeFolderProviderSynchronizer.class + ".batch");
    ...
    /**
     * Scans all {@code person} people objects and checks their home folder is located according to
     * the person's home folder provider preferred default location.
     *
     * @param systemUserName String the system user name with the tenant-specific ID attached.
     * @param tenantDomain String name of the tenant domain. Used to restrict the which people are
     *     processed.
     * @param overrideProvider String the bean name of a HomeFolderProvider to be used in place of
     *     the all home folders existing providers. If {@code null} the existing provider is used.
     */
    private void scanPeople(
            final String systemUserName, final String tenantDomain, final String overrideProvider) {
        /*
        ...
        final String moveFolderThatClashesPhaseName =
                "moveHomeFolderThatClashesWithParentFolderStructure";
        RunAsWorker[] workers =
                new RunAsWorker[] {
                    new RunAsWorker(
                            systemUserName, tenantDomain, "calculateParentFolderStructure") {
                        @Override
        ...
        for (RunAsWorker worker : workers) {
            ...
            if (logger.isInfoEnabled()) {
                logger.info(
                        "  -- "
                                + (TenantService.DEFAULT_DOMAIN.equals(tenantDomain)
                                        ? ""
                                        : tenantDomain + " ")
                                + name
                                + " --");
                        ...
                        RetryingTransactionCallback<Set<String>> restoreCallback =
                                new RetryingTransactionCallback<Set<String>>() {
                                    public Set<String> execute() throws Exception {
                                        // Returns a sorted set (using natural ordering) rather than
                                        // a hashCode
                                        // so that it is more obvious what the order is for
                                        // processing users.
    ...
    /**
     * Work out the preferred parent folder structure so we will be able to work out if any existing
     * home folders clash.
     */
    private ParentFolderStructure calculateParentFolderStructure(
            final ParentFolderStructure parentFolderStructure,
            NodeRef person,
            String overrideProviderName) {
    ...
    /**
     * Move any home folders (to a temporary folder) that clash with the new parent folder
     * structure.
     */
    ...
    /**
     * Creates the new home folder structure, before we move home folders so that we don't have race
     * conditions that result in unnecessary retries.
     *
    ...
    /**
     * If the home folder has been created but is not in its preferred location, the home folder is
     * moved. Empty parent folder's under the old root are only removed if the old root is known and
     * {@code home_folder_provider_synchronizer.keep_empty_parents=true} has not been set.
     *
     * @param person Person to be checked.
     * @param overrideProviderName String name of a provider to use in place of the one currently
     *     used. Ignored if {@code null}.
     */
    private void moveHomeFolderIfRequired(NodeRef person, String overrideProviderName) {
        new HomeFolderHandler(person, overrideProviderName) {
            ...
            protected void handleInPreferredLocation() {
                if (logger.isInfoEnabled()) {
                    logger.info(
                            "     # " + toPath(actualPath) + " is already in preferred location.");
                }
            }
                ...
                if (logger.isInfoEnabled()) {
                    logger.info(
                            "     # "
                                    ...
                                    + " "
                                    + providerName
                                    + " creates shared home folders - These are not moved.");
                }
            }
            ...
            protected void handleRootOrAbove() {
                if (logger.isInfoEnabled()) {
                    logger.info(
                            "     # "
                                    + userName
                                    + " has a home folder that is the provider's root directory (or"
                                    + " is above it). This is normally for users that origanally"
                                    + " had an internal provider or a provider that uses shared"
                                    + " home folders - These are not moved.");
                }
            }
                ...
                if (logger.isInfoEnabled()) {
                    logger.info(
                            "     # "
                                    ...
                                    + " "
                                    + providerName
                                    + " for is not a HomeFolderProvider2.");
                }
            }
                ...
                if (logger.isInfoEnabled()) {
                    logger.info(
                            "     # "
                                    ...
                                    + " Original "
                                    + originalProviderName
                                    + " is an internal type - These are not moved.");
                }
            }
    ...
    /**
     * @return the relative 'path' (a list of folder names) of the {@code homeFolder} from the
     *     {@code root} or {@code null} if the homeFolder is not under the root or is the root. An
     *     empty list is returned if the homeFolder is the same as the root or the root is below the
     *     homeFolder.
     */
    ...
    /**
     * Move an existing home folder from one location to another, removing empty parent folders and
     * reseting homeFolder and homeFolderProvider properties.
     */
    private void moveHomeFolder(
            NodeRef person,
            NodeRef homeFolder,
            NodeRef root,
        ...
        try {
            ...
            if (logger.isInfoEnabled()) {
                logger.info(
                        "     mv "
                                ...
                                + ((providerName != null
                                                && !providerName.equals(originalProviderName))
                                        ? "    # AND reset provider to " + providerName
                                        : ""));
            }

            // Perform the move
            homeFolder = fileFolderService.move(homeFolder, newParent, homeFolderName).getNodeRef();
            ...
            if (providerName != null && !providerName.equals(originalProviderName)) {
                nodeService.setProperty(
                        person, ContentModel.PROP_HOME_FOLDER_PROVIDER, providerName);
            }
    ...
    private NodeRef createNewParentIfRequired(NodeRef root, List<String> homeFolderPath) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            NodeRef nodeRef =
                    nodeService.getChildByName(parent, ContentModel.ASSOC_CONTAINS, pathElement);
            ...
            if (nodeRef == null) {
                ...
                parent =
                        fileFolderService
                                .create(parent, pathElement, ContentModel.TYPE_FOLDER)
                                .getNodeRef();
            } else {
    ...
    private class WorkProvider implements BatchProcessWorkProvider<NodeRef> {
        ...
        private final VmShutdownListener vmShutdownLister =
                new VmShutdownListener("getHomeFolderProviderSynchronizerWorkProvider");
        ...
        public synchronized Collection<NodeRef> getNextWork() {
            ...
            RetryingTransactionCallback<Collection<NodeRef>> restoreCallback =
                    new RetryingTransactionCallback<Collection<NodeRef>>() {
    ...
    private abstract class RunAsWorker extends BatchProcessWorkerAdaptor<NodeRef> {
        ...
        public void process(final NodeRef person) throws Throwable {
            // note: runAs before runAsTenant (to avoid clearing tenant context, if no previous
            // auth)
                ...
                originalProviderName =
                        DefaultTypeConverter.INSTANCE.convert(
                                ...
                                nodeService.getProperty(
                                        person, ContentModel.PROP_HOME_FOLDER_PROVIDER));
                if (!BOOTSTRAP_HOME_FOLDER_PROVIDER.equals(originalProviderName)
                    ...
                    if (provider != null) {
                        ...
                        if (preferredPath == null || preferredPath.isEmpty()) {
                            ...
                            originalRoot = null;
                            HomeFolderProvider2 originalProvider =
                                    homeFolderManager.getHomeFolderProvider2(originalProviderName);
                            List<String> originalPreferredPath = null;
                            if (originalProvider != null) {
                                originalRoot =
                                        homeFolderManager.getRootPathNodeRef(originalProvider);
                                originalPreferredPath = originalProvider.getHomeFolderPath(person);
                            }

                            if (originalProvider != null
                                    && (originalPreferredPath == null
                                            || originalPreferredPath.isEmpty())) {
    ...
    private class ParentFolderStructure {
        ...
        /**
         * Checks to see if there is a clash between the preferred paths and the existing folder
         * structure. If there is a clash, the existing home folder (the leaf folder) is moved to a
         * temporary structure. This allows any parent folders to be tidied up (if empty), so that
         * the new preferred structure can be recreated.
         *
         * <p>1. There is no clash if the path is null or empty.
         *
         * <p>2. There is a clash if there is a parent structure included the root folder itself.
         *
         * <p>3. There is a clash if the existing path exists in the parent structure. This
         * comparison ignores case as Alfresco does not allow duplicates regardless of case.
         *
         * <p>4. There is a clash if any of the folders in the existing path don't match the case of
         * the parent folders.
         *
         * <p>5. There is a clash there are different case versions of the parent folders themselves
         * or other existing folders.
         *
         * <p>When 4 takes place, we will end up with the first one we try to recreate being used
         * for all.
         */
        ...
        private class RootFolder extends Folder {
            ...
            public void add(List<String> path) {
                if (!includesRoot) {
                    ...
                    if (parentSize == 0) {
                        ...
                        if (logger.isInfoEnabled()) {
                            logger.info(
                                    "   # Recorded root as parent - no need to record other parents"
                                            + " as all home folders will clash");
                        }
                    } else {
                        add(path, 0);
                    }
             ...
             *
             * <p>Performs check 2 and then calls {@link Folder#clash(List, int)} to perform 3, 4
             * and 5.
             */
        ...
        private class Folder {
             ...
             * See description of {@link ParentFolderStructure#clash(NodeRef, List)}.
             *
             * <p>Performs checks 3, 4 and 5 for a single level and then recursively checks lower
             * levels.
             */
            protected boolean clash(List<String> path, int depth) {
                ...
                if (child == null) {
                    // Negation of check 3.
                    return false;
                } else if (child.duplicateWithDifferentCase) // if there folders using different
                // case!
                {
                    // Check 5.
                    return true;

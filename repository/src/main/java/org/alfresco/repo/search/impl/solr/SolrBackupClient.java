...
public class SolrBackupClient implements InitializingBean {
   ...
   */
  public void setFixNumberToKeepOffByOneError(
    boolean fixNumberToKeepOffByOneError
  ) {
  ...
  private void executeImpl(AtomicBoolean running) {
    if (
      (remoteBackupLocation == null) || (remoteBackupLocation.length() == 0)
    ) {
      if (logger.isInfoEnabled()) {
        logger.info(
          "Back up of SOLR core skipped - no remote backup localtion set for: " +
          core
        );
      }
    }
    ...
    if (fixNumberToKeepOffByOneError) {
      parameters.put(
        "numberToKeep",
        String.valueOf(numberToKeep > 1 ? (numberToKeep + 1) : numberToKeep)
      );
    } else {
      parameters.put("numberToKeep", String.valueOf(numberToKeep));
    }

    solrAdminClient.executeCommand(
      core,
      JSONAPIResultFactory.HANDLER.REPLICATION,
      JSONAPIResultFactory.COMMAND.BACKUP,
      parameters
    );

...
public class BatchProcessor<T> implements BatchMonitor {
  ...
  @Deprecated
  public int process(
    final BatchProcessWorker<T> worker,
    ...
    try {
      ...
      while (iterator.hasNext()) {
        ...
        if (batch.size() >= this.batchSize || !hasNext) {
          final TxnCallback callback = new TxnCallback(
            id++,
            worker,
            batch,
            splitTxns
   ...
   */
  @AlfrescoPublicApi
  public abstract static class BatchProcessWorkerAdaptor<TT>
  ...
  class TxnCallback
    ...
    public Object execute() throws Throwable {
      ...
      synchronized (BatchProcessor.this) {
        ...
        while (
          !BatchProcessor.this.retryTxns.isEmpty() &&
          (
        ...
        BatchProcessor.this.currentEntryId =
          this.worker.getIdentifier(this.batch.get(0));
    ...
    public void run() {
      try {} catch (Throwable e) {
        BatchProcessor.this.logger.error(
            "Failed to cleanup Worker after processing.",
            e
          );
      }
      ...
      try {
        ...
        if (this.splitTxns) {
          ...
          this.txnErrors++;
          if (BatchProcessor.this.logger.isWarnEnabled()) {
            String message = (t instanceof IntegrityException)
        ...
        // Otherwise, we have a retryable exception that we should propagate
          ...
          throw new AlfrescoRuntimeException(
            "Transactional error during " + getProcessName(),
            t
          );
        }
      }
      ...
      synchronized (BatchProcessor.this) {
        if (this.txnErrors > 0) {
          ...
          long currentIncrement =
            processed % BatchProcessor.this.loggingInterval;

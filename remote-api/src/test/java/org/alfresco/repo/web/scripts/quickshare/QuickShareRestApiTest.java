...
public class QuickShareRestApiTest extends BaseWebScriptTest {
    ...
    private static final String TEST_MIMETYPE_JPEG = MimetypeMap.MIMETYPE_IMAGE_JPEG;
    private static final String TEST_MIMETYPE_PNG = MimetypeMap.MIMETYPE_IMAGE_PNG;
    ...
    protected void setUp() throws Exception {
        ...
        synchronousTransformClient =
                (SynchronousTransformClient)
                        getServer().getApplicationContext().getBean("synchronousTransformClient");
                ...
                new RunAsWork<Void>() {
                    ...
                    public Void doWork() throws Exception {
                        if (!synchronousTransformClient.isSupported(
                                ...
                                null)) {
                            fail(
                                    "Image transformer is not working.  Please check your image"
                                            + " conversion command setup.");
                        }
    ...
    public void testSanityCheckUrls() throws Exception {
        ...
        final int expectedStatusServerError =
                500; // currently mapped from AccessDenied (should it be 403, 404 or does it depend
        // on use-case)
        ...
        //        rsp = sendRequest(new GetRequest(AUTH_CONTENT_URL.replace("{node_ref_3}",
        // testNodeRef_3)), expectedStatusOK, USER_ONE);
        ...
        // Commented out when removing original CMIS impl
        //      rsp = sendRequest(new GetRequest(AUTH_CONTENT_URL.replace("{node_ref_3}",
        // testNodeRef_3)), expectedStatusForbidden, USER_TWO);
        rsp =
                sendRequest(
                        new GetRequest(
                                AUTH_CONTENT_THUMBNAIL_URL
        ...
        assertEquals(
                22,
                sharedId.length()); // note: we may have to adjust/remove this check if we change
        // length of id (or it becomes variable length)
        ...
        // -ve test (should not be able to get metadata or content via sharedId) - whether
    ...
    public void testUnshareContributer()
        ...
        NodeRef siteDocLib =
                siteService.getContainer(siteInfo.getShortName(), SiteService.DOCUMENT_LIBRARY);
    ...
    /**
     * This test verifies that copying a shared node does not across the shared aspect and it's
    ...
    public void testCopy()
            throws UnsupportedEncodingException, IOException, JSONException, FileExistsException,
    ...
    public void testContentDispositionInResponseHeader() throws IOException, JSONException {
        ...
        // Thumbnail creation by user one to genuinely create the thumbnail and allow the sharedId
        // to get it
        sendRequest(
                new GetRequest(
                        AUTH_CONTENT_THUMBNAIL_URL
        ...
        // In case of requesting the content only, Content-Disposition should be present to force

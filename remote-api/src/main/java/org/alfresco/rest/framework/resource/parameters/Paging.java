...
public class Paging {
  ...
  public static final Paging DEFAULT = Paging.valueOf(
    DEFAULT_SKIP_COUNT,
    DEFAULT_MAX_ITEMS
  );
  ...
  private Paging(int skipCount, int maxItems) {
    ...
    if (skipCount < 0) {
      throw new InvalidArgumentException(
        "Negative values not supported for skipCount."
      );
    }
    if (maxItems < 1) {
      throw new InvalidArgumentException(
        "Only positive values supported for maxItems."
      );
    }

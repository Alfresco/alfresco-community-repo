...
public class CommentDelete extends AbstractCommentsWebScript {
  ...
  protected Map<String, Object> executeImpl(
    ...
    if (parentNodeRefStr != null) {
      parentNodeRef =
        new NodeRef((String) getOrNull(jsonPageParams, JSON_KEY_NODEREF));
    }
    ...
    if (parentNodeRef == null) {
      ...
      NodeRef topicNodeRef = nodeService
        .getPrimaryParent(nodeRef)
        .getParentRef();
      if (
        topicNodeRef == null ||
        !nodeService.getType(topicNodeRef).equals(ForumModel.TYPE_TOPIC)
      ) {
        throw new IllegalArgumentException(
          "The NodeRef specified is not a child of the topic."
        );
      }
      NodeRef forumNodeRef = nodeService
        .getPrimaryParent(topicNodeRef)
        .getParentRef();
      if (
        forumNodeRef == null ||
        !nodeService.getType(forumNodeRef).equals(ForumModel.TYPE_FORUM)
      ) {
        throw new IllegalArgumentException(
          "The NodeRef specified doesn't belong to a correct structure of forums."
        );
      }
  ...
  private void deleteComment(NodeRef commentNodeRef) {
    ...
    if (!nodeType.equals(ForumModel.TYPE_POST)) {
      throw new IllegalArgumentException(
        "Node to delete is not a comment node."
      );
    }

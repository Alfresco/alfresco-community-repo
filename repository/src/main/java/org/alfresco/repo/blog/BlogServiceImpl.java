...
public class BlogServiceImpl implements BlogService {
  ...
  private static final int MIN_NUMBER_OF_PAGES_FOR_THE_USER_TO_LOOP_THROUGH =
    10;
   ...
   */
  protected NodeRef getSiteBlogContainer(
  ...
  private BlogPostInfo buildBlogPost(
    ...
    BlogPostInfoImpl post = new BlogPostInfoImpl(
      nodeRef,
      parentNodeRef,
      postName
    );
  ...
  public boolean isDraftBlogPost(NodeRef blogPostNode) {
    return (
      nodeService.getProperty(blogPostNode, ContentModel.PROP_PUBLISHED) == null
    );
  }
  ...
  public BlogPostInfo createBlogPost(
    ...
    try {
      ...
      throw new ConcurrencyFailureException(
        "Blog post name already used: " + nodeName
      );
    }

    ContentWriter writer = contentService.getWriter(
      postNode.getChildRef(),
    ...
    BlogPostInfo post = new BlogPostInfoImpl(
      postNode.getChildRef(),
      blogContainerNode,
      nodeName
    );
  ...
  public void deleteBlogPost(BlogPostInfo post) {
    if (post.getNodeRef() == null) {
      throw new IllegalArgumentException(
        "Can't delete a post that was never persisted"
      );
    }
  ...
  public PagingResults<BlogPostInfo> getDrafts(
    ...
    // get canned query
    GetBlogPostsCannedQuery cq = (GetBlogPostsCannedQuery) draftPostsCannedQueryFactory.getGetDraftsCannedQuery(
      blogContainerNode,
      username,
      pagingReq
    );
  ...
  public PagingResults<BlogPostInfo> getPublished(
    ...
    // get canned query
    GetBlogPostsCannedQuery cq = (GetBlogPostsCannedQuery) publishedPostsCannedQueryFactory.getGetPublishedCannedQuery(
      blogContainerNode,
      fromDate,
      toDate,
      byUser,
  ...
  public PagingResults<BlogPostInfo> getMyDraftsAndAllPublished(
    ...
    DraftsAndPublishedBlogPostsCannedQuery cq = (DraftsAndPublishedBlogPostsCannedQuery) draftsAndPublishedBlogPostsCannedQueryFactory.getCannedQuery(
      blogContainerNode,
      createdFrom,
      createdTo,
      currentUser,
  ...
  private void setOrUpdateReleasedAndUpdatedDates(NodeRef blogPostNode) {
    ...
    if (!nodeService.hasAspect(blogPostNode, ContentModel.ASPECT_SYNDICATION)) {
      nodeService.addAspect(
        blogPostNode,
        ContentModel.ASPECT_SYNDICATION,
        null
      );
    }
  ...
  public PagingResults<BlogPostInfo> findBlogPosts(
    ...
    sp.setMaxItems(
      pagingReq.getMaxItems() * MIN_NUMBER_OF_PAGES_FOR_THE_USER_TO_LOOP_THROUGH
    );
    ...
    try {
      ...
      final List<NodeRef> nodeRefs = finalLuceneResults
        .getNodeRefs()
      ...
      results =
        new PagingResults<BlogPostInfo>() {
          ...
          public List<BlogPostInfo> getPage() {
            List<BlogPostInfo> blogPostInfos = new ArrayList<BlogPostInfo>(
              nodeRefs.size()
            );
            for (NodeRef nodeRef : nodeRefs) {
              String postName = (String) nodeService.getProperty(
                nodeRef,
  ...
  private PagingResults<BlogPostInfo> wrap(
    final CannedQueryResults<BlogEntity> results,
   ...
   */
  private String createDateRangeQuery(
    Date fromDate,
    Date toDate,
    QName dateProperty
  ) {

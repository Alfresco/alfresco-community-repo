...
public abstract class AbstractTypeDefinitionWrapper
  ...
  protected void setTypeDefinition(
    ...
    AbstractTypeDefinition typeDefInclProperties
  ) {
  ...
  public void updateProperty(
    ...
    if (
      propertyDefWrap != null &&
      propertyDefWrap.getPropertyDefinition().getDisplayName() == null
    ) {
      AbstractPropertyDefinition<?> property = (AbstractPropertyDefinition<?>) propertyDefWrap.getPropertyDefinition();
      org.alfresco.service.cmr.dictionary.PropertyDefinition propDef = dictionaryService.getProperty(
        QName.createQName(property.getLocalNamespace(), property.getLocalName())
      );
      if (propDef != null) {
        ...
        property.setDisplayName(
          displayName == null ? property.getId() : displayName
        );
        property.setDescription(
          description == null ? property.getDisplayName() : description
        );
      }
    }
  }
  ...
  public void updateTypeDefInclProperties() {
    for (PropertyDefinition<?> property : typeDefInclProperties
      .getPropertyDefinitions()
      .values()) {
      if (property.getDisplayName() == null) {
        typeDefInclProperties.addPropertyDefinition(
          getPropertyById(property.getId()).getPropertyDefinition()
        );
      }
    }
  }
  ...
  public void setTypeDefDisplayName(String name) {
    ...
    try {
      ...
      typeDefInclProperties.setDisplayName(
        name != null ? name : typeDef.getId()
      );
    } finally {
  ...
  public void setTypeDefDescription(String desc) {
    ...
    try {
      ...
      typeDefInclProperties.setDescription(
        desc != null ? desc : typeDef.getId()
      );
    } finally {
      lock.writeLock().unlock();
    }
  ...
  public abstract List<TypeDefinitionWrapper> connectParentAndSubTypes(
    CMISMapping cmisMapping,
    CMISDictionaryRegistry registry,
    DictionaryService dictionaryService
  );

  public abstract void resolveInheritance(
    CMISMapping cmisMapping,
    CMISDictionaryRegistry registry,
    DictionaryService dictionaryService
  );
  ...
  public void assertComplete() {
    if (typeDef == null) throw new IllegalStateException("typeDef is not set");
    if (typeDefInclProperties == null) throw new IllegalStateException(
      "typeDefInclProperties is not set"
    );
    if (alfrescoName == null) throw new IllegalStateException(
      "alfrescoName is not set"
    );
    if (alfrescoClass == null) throw new IllegalStateException(
      "alfrescoClass is not set"
    );
    if (propertiesById == null) throw new IllegalStateException(
      "propertiesById is not set"
    );
    if (propertiesByQueryName == null) throw new IllegalStateException(
      "propertiesByQueryName is not set"
    );
    if (propertiesByQName == null) throw new IllegalStateException(
      "propertiesByQName is not set"
    );
    //        if (propertiesById.size() == 0)
    //            throw new IllegalStateException("property map empty");
    if (
      propertiesById.size() != propertiesByQueryName.size()
    ) throw new IllegalStateException("property map mismatch");
    if (
      propertiesById.size() != propertiesByQName.size()
    ) throw new IllegalStateException("property map mismatch");
  }
   ...
   */
  protected void createOwningPropertyDefinitions(
    CMISMapping cmisMapping,
    ...
    for (org.alfresco.service.cmr.dictionary.PropertyDefinition alfrescoPropDef : cmisClassDef
      .getProperties()
      .values()) {
      if (!isBaseType()) {
      ...
      // compile property id
      String propertyId = cmisMapping.buildPrefixEncodedString(
        alfrescoPropDef.getName()
      );

      if (
        propertyId.equals("cmis:secondaryObjectTypeIds") &&
        cmisMapping.getCmisVersion() == CmisVersion.CMIS_1_0
      ) {
        continue;
      }
      ...
      if (propertyDefintion != null) {
        ...
        if (propertyAccessorMapping != null) {
          propertyAccessor =
            propertyAccessorMapping.getPropertyAccessor(propertyId);
          if (propertyAccessor == null) {
            propertyAccessor =
              propertyAccessorMapping.createDirectPropertyAccessor(
                propertyId,
                alfrescoPropDef.getName()
              );
          }
        ...
        if (luceneBuilderMapping != null) {
          luceneBuilder =
            luceneBuilderMapping.getPropertyLuceneBuilder(propertyId);
          if (luceneBuilder == null) {
            luceneBuilder =
              luceneBuilderMapping.createDirectPropertyLuceneBuilder(
                alfrescoPropDef.getName()
              );
          }
        }
        ...
        registerProperty(
          new BasePropertyDefintionWrapper(
            propertyDefintion,
            alfrescoPropDef.getName(),
            this,
            propertyAccessor,
  ...
  protected void registerProperty(PropertyDefinitionWrapper propDefWrapper) {
    ...
    if (propertiesById.containsKey(propDefWrapper.getPropertyId())) {
      throw new AlfrescoRuntimeException(
        "Property defintion " +
        propDefWrapper.getPropertyId() +
        " already exists on type " +
        typeDef.getId()
      );
    }

    propertiesById.put(propDefWrapper.getPropertyId(), propDefWrapper);
    propertiesByQueryName.put(
      propDefWrapper.getPropertyDefinition().getQueryName(),
      propDefWrapper
    );
    propertiesByQName.put(propDefWrapper.getAlfrescoName(), propDefWrapper);
    typeDefInclProperties.addPropertyDefinition(
      propDefWrapper.getPropertyDefinition()
    );
  }
  ...
  protected PropertyDefinition<?> createPropertyDefinition(
    ...
    if (
      id.equals(PropertyIds.OBJECT_TYPE_ID) ||
      id.equals(PropertyIds.SOURCE_ID) ||
      id.equals(PropertyIds.TARGET_ID)
    ) {
      // the CMIS spec requirement
      result.setUpdatability(Updatability.ONCREATE);
    } else {
      result.setUpdatability(
        propDef.isProtected() ? Updatability.READONLY : Updatability.READWRITE
      );
    }
    ...
    result.setCardinality(
      propDef.isMultiValued() ? Cardinality.MULTI : Cardinality.SINGLE
    );
    ...
    // query and order
    result.setQueryName(
      ISO9075.encodeSQL(cmisMapping.buildPrefixEncodedString(alfrescoPropName))
    );
          ...
          default:
            if (
              propDef
                .getDataType()
                .getName()
                .equals(DataTypeDefinition.BOOLEAN) ||
              ...
              propDef
                .getDataType()
                .getName()
                .equals(DataTypeDefinition.DOUBLE) ||
              propDef
                .getDataType()
                .getName()
                .equals(DataTypeDefinition.FLOAT) ||
    ...
    if (result instanceof PropertyIntegerDefinitionImpl) {
      if (propDef.getDataType().getName().equals(DataTypeDefinition.INT)) {
        ((PropertyIntegerDefinitionImpl) result).setMinValue(
            BigInteger.valueOf(Integer.MIN_VALUE)
          );
        ((PropertyIntegerDefinitionImpl) result).setMaxValue(
    ...
    for (ConstraintDefinition constraintDef : propDef.getConstraints()) {
      ...
      if (
        (constraint instanceof StringLengthConstraint) &&
        (result instanceof PropertyStringDefinitionImpl)
      ) {
        StringLengthConstraint slc = (StringLengthConstraint) constraint;
        ((PropertyStringDefinitionImpl) result).setMaxLength(
            BigInteger.valueOf(slc.getMaxLength())
          );
      }
      ...
      if (constraint instanceof NumericRangeConstraint) {
        ...
        if (result instanceof PropertyIntegerDefinitionImpl) {
          ((PropertyIntegerDefinitionImpl) result).setMinValue(
              BigInteger.valueOf(((Double) nrc.getMinValue()).longValue())
            );
          ((PropertyIntegerDefinitionImpl) result).setMaxValue(
              BigInteger.valueOf(((Double) nrc.getMaxValue()).longValue())
            );
        }
        if (result instanceof PropertyDecimalDefinitionImpl) {
          ((PropertyDecimalDefinitionImpl) result).setMinValue(
              BigDecimal.valueOf(nrc.getMinValue())
            );
          ((PropertyDecimalDefinitionImpl) result).setMaxValue(
              BigDecimal.valueOf(nrc.getMaxValue())
            );
        }
      }
    }
  ...
  protected void createActionEvaluators(
    PropertyAccessorMapping propertyAccessorMapping,
    BaseTypeId baseTypeId
  ) {
    if (propertyAccessorMapping != null) {
      actionEvaluators =
        propertyAccessorMapping.getActionEvaluators(baseTypeId);
    } else {
  ...
  private void addDefaultValue(String value, PropertyDefinition<?> propDef) {
    ...
    if (propDef instanceof PropertyBooleanDefinitionImpl) {
      ...
      propDefImpl.setDefaultValue(
        Collections.singletonList(
          (Boolean) convertValueFromString(value, PropertyType.BOOLEAN)
        )
      );
      ...
      propDefImpl.setDefaultValue(
        Collections.singletonList(
          (GregorianCalendar) convertValueFromString(
            value,
      ...
      propDefImpl.setDefaultValue(
        Collections.singletonList(
          (BigDecimal) convertValueFromString(value, PropertyType.DECIMAL)
        )
      );
      ...
      propDefImpl.setDefaultValue(
        Collections.singletonList(
          (String) convertValueFromString(value, PropertyType.HTML)
        )
      );
      ...
      propDefImpl.setDefaultValue(
        Collections.singletonList(
          (String) convertValueFromString(value, PropertyType.ID)
        )
      );
      ...
      propDefImpl.setDefaultValue(
        Collections.singletonList(
          (BigInteger) convertValueFromString(value, PropertyType.INTEGER)
        )
      );
      ...
      propDefImpl.setDefaultValue(
        Collections.singletonList(
          (String) convertValueFromString(value, PropertyType.STRING)
        )
      );
      ...
      propDefImpl.setDefaultValue(
        Collections.singletonList(
          (String) convertValueFromString(value, PropertyType.URI)
        )
      );
    }
  }
  ...
  private void addChoiceList(
    ...
    if (propDef instanceof PropertyBooleanDefinitionImpl) {
      ...
      for (String allowed : lovc.getAllowedValues()) {
        ...
        choice.setValue(
          Collections.singletonList(
            (Boolean) convertValueFromString(allowed, PropertyType.BOOLEAN)
          )
        );
      ...
      for (String allowed : lovc.getAllowedValues()) {
        ...
        choice.setValue(
          Collections.singletonList(
            (BigDecimal) convertValueFromString(allowed, PropertyType.DECIMAL)
          )
        );
      ...
      for (String allowed : lovc.getAllowedValues()) {
        ...
        choice.setValue(
          Collections.singletonList(
            (String) convertValueFromString(allowed, PropertyType.HTML)
          )
        );
      ...
      for (String allowed : lovc.getAllowedValues()) {
        ...
        choice.setValue(
          Collections.singletonList(
            (String) convertValueFromString(allowed, PropertyType.ID)
          )
        );
      ...
      for (String allowed : lovc.getAllowedValues()) {
        ...
        choice.setValue(
          Collections.singletonList(
            (BigInteger) convertValueFromString(allowed, PropertyType.INTEGER)
          )
        );
      ...
      for (String allowed : lovc.getAllowedValues()) {
        ...
        choice.setValue(
          Collections.singletonList(
            (String) convertValueFromString(allowed, PropertyType.STRING)
          )
        );
      ...
      for (String allowed : lovc.getAllowedValues()) {
        ...
        choice.setValue(
          Collections.singletonList(
            (String) convertValueFromString(allowed, PropertyType.URI)
          )
        );

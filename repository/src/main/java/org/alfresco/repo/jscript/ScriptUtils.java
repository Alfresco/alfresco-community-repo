...
public class ScriptUtils extends BaseScopableProcessorExtension {
  ...
  public String displayPath(ScriptNode node) {
    return this.unprotNodeService.getPath(node.nodeRef)
  ...
  public ScriptNode resolveNodeReference(final String reference) {
    if (reference == null) {
      throw new IllegalArgumentException(
        "Node 'reference' argument is mandatory."
      );
    }

    final NodeLocatorService locatorService =
      this.services.getNodeLocatorService();
    ...
    switch (reference) {
      case "alfresco://company/home":
        nodeRef =
          locatorService.getNode(CompanyHomeNodeLocator.NAME, null, null);
        break;
      ...
      case "alfresco://company/shared":
        nodeRef =
          locatorService.getNode(SharedHomeNodeLocator.NAME, null, null);
        break;
  ...
  public boolean moduleInstalled(String moduleName) {
    ModuleService moduleService = (ModuleService) this.services.getService(
        QName.createQName(NamespaceService.ALFRESCO_URI, "ModuleService")
      );
    if (moduleService != null) {
      ModuleDetails moduleDetail = (ModuleDetails) moduleService.getModule(
        moduleName
      );
   ...
   */
  public ScriptPagingDetails createPaging(
    int maxItems,
    int skipCount,

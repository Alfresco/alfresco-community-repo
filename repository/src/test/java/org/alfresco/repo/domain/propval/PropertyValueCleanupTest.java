...
public class PropertyValueCleanupTest {
    ...
    public synchronized void testRapidCreationDuringCleanup() throws Exception {
        ...
        assertNull(
                "Did not expect to find the attribute ",
                attributeService.getAttribute(toDeleteKey1, toDeleteKey2));
        assertNull(
                "Key2 should be NOT present as a property value",
                propertyValueDAO.getPropertyStringValue(toDeleteKey2));
        ...
        assertNotNull(
                "Key2 should be present as a property value",
                propertyValueDAO.getPropertyStringValue(toDeleteKey2));
        ...
        assertNotNull(
                "Key2 should be present as a property value (even if unreferenced)",
                propertyValueDAO.getPropertyStringValue(toDeleteKey2));
        ...
        assertNull(
                "Key2 should be NOT present as a property value (cleanup job)",
                propertyValueDAO.getPropertyStringValue(toDeleteKey2));
    ...
    /**
     * Simple runnable that continuously creates new serializable attributes until stopped. Each
     * thread has a unique second key value, a sequential third key and generates serializable
     * (unshared) property values.
     *
     * @author Derek Hulley
     */

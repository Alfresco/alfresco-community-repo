...
/**
 * @author mrogers
 *     <p>The tertiary manifest processor performs a third parse of the snapshot file.
 *     <p>For a complete transfer it is responsible for deleting any replicated nodes which exist in
 *     the target repository that do not exist in the source repository.
 *     <p>If the transfer is not "sync" then this processor does nothing.
 */
public class RepoTertiaryManifestProcessorImpl extends AbstractManifestProcessorBase {
    ...
    protected void processNode(TransferManifestNormalNode node) {
        ...
        logComment(
                "Tertiary Processing incoming node: "
                        + node.getNodeRef()
                        + " --  Source path = "
                        + node.getParentPath()
                        + "/"
        ...
        if (isSync) {
            ...
            if (nodeService.exists(nodeRef)) {
                ...
                if ((null != resolvedNodes.resolvedParent)
                        && nodeService.exists(resolvedNodes.resolvedParent)) {
                    if (log.isTraceEnabled()) {
                        logInvasionHierarchy(
                                resolvedNodes.resolvedParent, nodeRef, nodeService, log);
                    }
                } else {
                    ...
                    if (log.isTraceEnabled()) {
                        logInvasionHierarchy(
                                parentAssocs.iterator().next().getParentRef(),
                                nodeRef,
                                nodeService,
                                log);
                    }
                ...
                for (ChildAssociationRef ref : expectedChildren) {
                    ...
                    if (nodeService.exists(ref.getChildRef())) {
                        ...
                        ResolvedParentChildPair resolvedChild =
                                nodeResolver.resolveCorrespondingNode(
                                        ref.getChildRef(), ref, parentPath);
                        ...
                        if (null != resolvedChild.resolvedChild) {
                            ...
                            if (log.isDebugEnabled()) {
                                log.debug(
                                        "The node DOES NOT exist in current repository! Processing"
                                                + " will be made by its PATH!");
                            }
                ...
                for (ChildAssociationRef child : actualChildren) {
                    ...
                    if (child.isPrimary()) {
                        if (log.isTraceEnabled()) {
                            logInvasionHierarchy(
                                    child.getParentRef(), child.getChildRef(), nodeService, log);
                        }
                        ...
                        if (!expectedChildNodeRefs.contains(childNodeRef)
                                && !expectedChildNodePaths.contains(actualChildPath)) {
                            if (log.isDebugEnabled()) {
                                log.debug("This child IS NOT EXPECTED!");
                            }

                            /**
                             * An unexpected child - if this node has been transferred then it may
                             * need to be deleted.
                             *
                             * <p>If from another repository then we have to prune the alien
                             * children rather than deleting it.
                             */
                            if (nodeService.hasAspect(
                                    childNodeRef, TransferModel.ASPECT_TRANSFERRED)) {
                                log.debug("an unexpected transferred child node:" + child);
                                logComment(
                                        "Transfer sync mode - checking unexpected child node:"
                                                + child);
                                ...
                                // Yes this is a transferred node.  When syncing we only delete
                                // nodes that are "from"
                                ...
                                if (fromRepositoryId != null && manifestRepositoryId != null) {
                                    if (nodeService.hasAspect(
                                            childNodeRef, TransferModel.ASPECT_ALIEN)) {
                                        /**
                                         * This node can't be deleted since it contains alien
                                         * content it needs to be "pruned" of the transferring
                                         * repo's content instead.
                                         */
                                        log.debug(
                                                "node to be deleted contains alien content so needs"
                                                        + " to be pruned."
                                                        + childNodeRef);
                                        logComment(
                                                "Transfer sync mode - node contains alien content"
                                                        + " so can't be deleted. "
                                                        + childNodeRef);
                                        alienProcessor.pruneNode(
                                                childNodeRef, manifestRepositoryId);
                                    } else {
                                        // Node
                                        if (log.isDebugEnabled()) {
                                            log.debug(
                                                    "Node not alien. Trying to delete the node...");
                                        }
                                        ...
                                        if (manifestRepositoryId.equalsIgnoreCase(
                                                initialRepositoryId)) {
                                            if (log.isDebugEnabled()) {
                                                log.debug(
                                                        "Replication is initiated from the same"
                                                            + " repository from which this node was"
                                                            + " transferred! Deleting");
                                            }
                                            // Yes the manifest repository Id and the from
                                            // repository Id match.
                                            // Destination node if from the transferring repo and
                                            // needs to be deleted.
                                            logDeleted(
                                                    node.getNodeRef(),
                                                    childNodeRef,
                                            ...
                                            if (log.isDebugEnabled()) {
                                                log.debug(
                                                        "It is not an alien, but 'fromRepositoryId'"
                                                                + " is not equal to the"
                                                                + " 'manifestRepositoryId'! Cannot"
                                                                + " delete the foreign node...");
                                            }
                                        }
                                    }
                                } else {

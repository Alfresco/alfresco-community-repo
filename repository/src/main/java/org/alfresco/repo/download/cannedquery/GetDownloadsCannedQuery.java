...
/**
 * This class provides the GetDownloads canned queries} used by the {@link
...
public class GetDownloadsCannedQuery extends AbstractCannedQueryPermissions<DownloadEntity> {
    ...
    protected List<DownloadEntity> queryAndFilter(CannedQueryParameters parameters) {
        ...
        // note: refer to SQL for specific DB filtering (eg.parent node and optionally blog
        // integration aspect, etc)
        List<DownloadEntity> results =
                cannedQueryDAO.executeQuery(
                        QUERY_NAMESPACE,
                        QUERY_SELECT_GET_DOWNLOADS,
                        paramBeanObj,
                        0,
                        Integer.MAX_VALUE);
        ...
        for (DownloadEntity entity : results) {
            Date createdDate =
                    DefaultTypeConverter.INSTANCE.convert(Date.class, entity.getCreatedDate());
            Date modifiedDate =
                    DefaultTypeConverter.INSTANCE.convert(Date.class, entity.getModifiedDate());

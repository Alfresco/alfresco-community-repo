...
public class ScriptResourceHelper {
     ...
     *
     * <p>Multiple includes of the same resource are dealt with correctly and nested includes of
     * scripts is fully supported.
     *
     * <p>Note that for performance reasons the script import directive syntax and placement in the
     * file is very strict. The import lines <i>must</i> always be first in the file - even before
     * any comments. Immediately that the script service detects a non-import line it will assume
     * the rest of the file is executable script and no longer attempt to search for any further
     * import directives. Therefore all imports should be at the top of the script, one following
     * the other, in the correct syntax and with no comments present - the only separators valid
     * between import directives is white space.
     *
     ...
     */
    public static String resolveScriptImports(
            String script, ScriptResourceLoader loader, Log logger) {
        ...
        if (scriptlets.size() == 1) {
            ...
            if (logger.isTraceEnabled()) logger.trace("Script content resolved to:\r\n" + script);
    ...
    /**
     * Recursively resolve imports in the specified scripts, adding the imports to the specific list
     * of scriplets to combine later.
     *
    ...
    private static void recurseScriptImports(
        ...
        if (script.startsWith(IMPORT_PREFIX, index)) {
                    ...
                    if (script.charAt(index) == '"' && script.charAt(index + 1) == '>') {
                        ...
                        if (scripts.containsKey(resource) == false) {
                            ...
                            if (includedScript != null) {
                                ...
                                recurseScriptImports(
                                        resource, includedScript, loader, scripts, logger);
                            }
                        } else {
                            if (logger.isDebugEnabled())
                        ...
                        recurseScriptImports(
                                location, script.substring(index), loader, scripts, logger);
                        return;
                    }
                }
                // if we get here, we failed to find the end of an import line
                throw new ScriptException(
                        "Malformed 'import' line - must be first in file, no comments and strictly"
                                + " of the form:\r\n"
                                + "<import resource=\"...\">");
            } else {
                throw new ScriptException(
            ...
            if (logger.isTraceEnabled()) logger.trace(script);

...
public abstract class AbstractImapFolder implements MailFolder {
     ...
     * @return long
     */
    public long appendMessage(final MimeMessage message, final Flags flags, final Date internalDate)
            throws FolderException {
        if (isReadOnly()) {
            throw new FolderException("Can't append message - Permission denied");
        }

        CommandCallback<Long> command =
                new CommandCallback<Long>() {
    ...
    public void deleteAllMessages() throws FolderException {
        CommandCallback<Object> command =
    ...
    /**
     * Deletes messages marked with {@link javax.mail.Flags.Flag#DELETED}. Note that this message
     * deletes the messages with current uid
     */
    ...
    public SimpleStoredMessage getMessage(final long uid) {
        CommandCallback<SimpleStoredMessage> command =
                new CommandCallback<SimpleStoredMessage>() {
    ...
    public List<SimpleStoredMessage> getMessages() {
        CommandCallback<List<SimpleStoredMessage>> command =
                new CommandCallback<List<SimpleStoredMessage>>() {
    ...
    /**
     * Method is called before the deletion of the folder. Notifies {@link FolderListener} objects
     * with {@link FolderListener#mailboxDeleted()} method calls.
     */
    ...
    protected void notifyFlagUpdate(
            int msn, Flags flags, Long uidNotification, FolderListener silentListener) {
        synchronized (listeners) {
    ...
    protected abstract long appendMessageInternal(
            MimeMessage message, Flags flags, Date internalDate) throws Exception;
    ...
    protected abstract void replaceFlagsInternal(
            Flags flags, long uid, FolderListener silentListener, boolean addUid) throws Exception;

    protected abstract long[] searchInternal(SearchTerm searchTerm);

    protected abstract void setFlagsInternal(

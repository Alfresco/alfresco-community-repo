...
public class ContextDependencyLister {
  ...
  private void calculateDependencies() {
    ...
    System.out.println(
      "  Took " + df.format((endTime - startTime) / 1000) + " seconds"
    );
    System.out.println();
    ...
    for (String parentBean : instance.btl.beanAllDependencies.keySet()) {
      ...
      if (parentContext == null) {
        System.err.println(
          "Warning - I don't know where defined bean " +
          parentBean +
          " came from!"
        );
        continue;
      ...
      for (String depBean : instance.btl.beanAllDependencies.get(parentBean)) {
        ...
        if (depContext == null) {
          System.err.println(
            "Warning - I don't know where depdendency " +
            depBean +
            " came from!"
          );
   ...
   */
  public void graphVizDependencies(String outFile)
    ...
    for (String context : instance.btl.usedContextFiles) {
      if (contextBeans.containsKey(context)) {
        for (String ourBean : contextBeans.get(context)) {
          if (
            beanDependencies.containsKey(ourBean) ||
      ...
      if (contextBeans.containsKey(context)) {
        ...
        if (!displayBeans.isEmpty()) {
          for (String ourBean : displayBeans) {
            ...
            out.println(
              "    " + nodeId + " [label=\"" + ourBean + "\",fontsize=9];"
            );
            ...
            if (beanDependencies.containsKey(ourBean)) {
              for (String depBean : beanDependencies.get(ourBean)) {
                ...
                beanLinks.append(
                  " " + nodeId + " -> " + otherId + " [color=\"deepskyblue2\"];"
                );
                beanLinks.append('\n');
              }
            }
  ...
  public static class BeanTrackingListener implements ReaderEventListener {
    ...
    public void defaultsRegistered(
      DefaultsDefinition paramDefaultsDefinition
    ) {}
    ...
    public void importProcessed(ImportDefinition paramImportDefinition) {
      ...
      if (context.startsWith("classpath:")) {
        ...
        if (pathToSlash == null) {
          URL c =
            ContextDependencyLister.class.getClassLoader().getResource(context);

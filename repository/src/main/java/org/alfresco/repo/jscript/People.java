...
public class People extends BaseScopableProcessorExtension implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        Map<String, ValueDeriver<ScriptNode, Boolean>> capabilityTesters =
                new HashMap<String, ValueDeriver<ScriptNode, Boolean>>(5);
        capabilityTesters.put(
                "isAdmin",
                new ValueDeriver<ScriptNode, Boolean>() {
        ...
        capabilityTesters.put(
                ...
                new ValueDeriver<ScriptNode, Boolean>() {
                    public Boolean deriveValue(ScriptNode source) {
                        // Check whether the account is mutable according to the authentication
                        // service
                        String sourceUser =
                                (String) source.getProperties().get(ContentModel.PROP_USERNAME);
                        ...
                        if (currentUser.equals(sourceUser)
        ...
        this.valueDerivingMapFactory =
                new ValueDerivingMapFactory<ScriptNode, String, Boolean>(capabilityTesters);
    }
    ...
    /**
     * Allows customers to choose to use Solr or Lucene rather than a canned query in {@link
     * #getPeople(String, int, String, boolean)} when {@code " [hint:useCQ]"} is appended to the
     * search term (currently Share's User Console does this). The down side is that new users may
     * not appear as they will not have been indexed. This is similar to what happened in 4.1.1
     * prior to MNT-7548 (4.1.2 and 4.1.1.1). The down side of using a canned query at the moment is
     * that there is a bug, so that it is impossible to search for names such as {@code "Carlos
     * Allende García"} where the first or last names may contain spaces. See MNT-9719 for more
     * details. The alfresco global property {@code people.search.honor.hint.useCQ} is used to set
     ...
     * @param setAccountEnabled set to 'true' to create enabled user account, or 'false' to create
     *     disabled user account for created person.
     * @return the person node (type cm:person) created or null if the person could not be created
     */
    public ScriptNode createPerson(
            String userName,
            String firstName,
            String lastName,
            String emailAddress,
        ...
        return createPerson(
                userName, firstName, lastName, emailAddress, password, setAccountEnabled, false);
    }
     ...
     * @param setAccountEnabled set to 'true' to create enabled user account, or 'false' to create
     *     disabled user account for created person.
     * @param notifyByEmail set to 'true' to have the new user emailed to let them know their
     *     account details. Only applies if a username and password were supplied.
     * @return the person node (type cm:person) created or null if the person could not be created
     */
    public ScriptNode createPerson(
            String userName,
        ...
        if (userName != null) {
            try {
                ...
                throw new AuthenticationException(
                        "User must belong to same domain as admin: " + re.getTenantA());
            }
    ...
    public void enableAccount(String userName) {
        if (this.authorityService.isAdminAuthority(
    ...
    /**
     * Set a password for the given user. Note that only an administrator can perform this action,
    ...
    public void setPassword(String userName, String password) {
        ...
        if (this.authorityService.hasAdminAuthority()
                && (userName.equalsIgnoreCase(authService.getCurrentUserName()) == false)) {
    ...
    public void setQuota(ScriptNode person, String quota) {
        if (this.authorityService.isAdminAuthority(
                AuthenticationUtil.getFullyAuthenticatedUser())) {
            this.contentUsageService.setUserQuota(
    ...
    /**
     * Get the collection of people stored in the repository. An optional filter query may be
     * provided by which to filter the people collection. Space separate the query terms i.e. "john
     * bob" will find all users who's first or second names contain the strings "john" or "bob".
     *
     * @param filter filter query string by which to filter the collection of people. If
     *     <pre>null</pre>
    ...
    public Scriptable getPeople(String filter, int maxResults, String sortBy, boolean sortAsc) {
        return getPeoplePaging(
                filter, new ScriptPagingDetails(maxResults, 0), sortBy, Boolean.valueOf(sortAsc));
    }

    public Scriptable getPeoplePaging(
            String filter, ScriptPagingDetails pagingRequest, String sortBy, Boolean sortAsc) {
    ...
    protected List<PersonInfo> getPeopleImpl(
            String filter, ScriptPagingDetails pagingRequest, String sortBy, Boolean sortAsc) {
        ...
        // In order to use a SOLR/Lucene search, we must have a non-empty filter string - see
        // ALF-18876
        if ((filter == null || filter.trim().isEmpty()) || useCQ) {
            ...
            useCQ =
                    ((tokens.length == 1)
                            && (propIndex == -1)
                            && ((wildPosition == -1) || (wildPosition == (term.length() - 1))));

            try {
                // FTS
                List<NodeRef> personRefs =
                        getPeopleImplSearch(term, tokens, pagingRequest, sortBy, sortAsc);
                ...
                if (useCQ) {
                    ...
                    // simple non-FTS filter: firstname or lastname or username starting with term
                    // (ignoring case)
                    persons = getPeopleImplDB(filter, pagingRequest, sortBy, sortAsc);
                }
            }
    ...
    // canned query
    protected List<PersonInfo> getPeopleImplDB(
            String filter, ScriptPagingDetails pagingRequest, String sortBy, Boolean sortAsc) {
    ...
    // search query
    protected List<NodeRef> getPeopleImplSearch(
            String term,
            String[] tokens,
        ...
        if (tokens.length == 1) {
            ...
            for (String token : tokens) {
                if (!propertySearch && token.indexOf(':') == -1) {
                    ...
                    query.append(token.substring(0, propIndex + 1))
                            .append('"')
                            .append(token.substring(propIndex + 1))
                            .append('"')
                            .append(' ');
        ...
        if (logger.isDebugEnabled()) {
            if ((sortBy != null) && (!sortBy.isEmpty())) {
                logger.debug(
                        "getPeopleImplSearch: ignoring sortBy ("
                                + sortBy
                                + ")- not yet supported by model for search");
            }
        }
    ...
    private List<NodeRef> getSortedPeopleObjects(
            List<NodeRef> peopleRefs, final String sortBy, Boolean sortAsc) {
        if (sortBy == null) {
            return peopleRefs;
        }
                ...
                new Comparator<NodeRef>() {
                    ...
                    public Serializable getProperty(NodeRef nodeRef) {
                        ...
                        if ("fullName".equalsIgnoreCase(sortBy)) {
                            String firstName =
                                    (String)
                                            nodeService.getProperty(
                                                    nodeRef, ContentModel.PROP_FIRSTNAME);
                            String lastName =
                                    (String)
                                            nodeService.getProperty(
                                                    nodeRef, ContentModel.PROP_LASTNAME);
                            ...
                            result =
                                    nodeService.getProperty(
                                            nodeRef, ContentModel.PROP_SIZE_CURRENT);
    ...
    /**
     * Faster helper when the script just wants to build the Full name for a person. Avoids complete
     * getProperties() retrieval for a cm:person.
     *
    ...
    public String getPersonFullName(final String username) {
        ...
        if (personRef != null) {
            ...
            final String firstName =
                    (String) nodeService.getProperty(personRef, ContentModel.PROP_FIRSTNAME);
            final String lastName =
                    (String) nodeService.getProperty(personRef, ContentModel.PROP_LASTNAME);
        ...
        if (authorityService.authorityExists(actualName) == false) {
            ...
            if (parentGroup != null) {
                String parentGroupName =
                        (String) parentGroup.getProperties().get(ContentModel.PROP_AUTHORITY_NAME);
    ...
    public void addAuthority(ScriptNode parentGroup, ScriptNode authority) {
        ...
        if (parentGroup.getQNameType().equals(ContentModel.TYPE_AUTHORITY_CONTAINER)) {
            ...
            if (authority.getQNameType().equals(ContentModel.TYPE_AUTHORITY_CONTAINER)) {
                authorityName =
                        (String) authority.getProperties().get(ContentModel.PROP_AUTHORITY_NAME);
            } else {
    ...
    public Scriptable getContainerGroups(ScriptNode person) {
        ...
        Set<String> authorities =
                this.authorityService.getContainingAuthoritiesInZone(
                        AuthorityType.GROUP,
                        (String) person.getProperties().get(ContentModel.PROP_USERNAME),
    ...
    public boolean isAdmin(ScriptNode person) {
        ParameterCheck.mandatory("Person", person);
        return this.authorityService.isAdminAuthority(
                (String) person.getProperties().get(ContentModel.PROP_USERNAME));
    }
    ...
    public boolean isGuest(ScriptNode person) {
        ParameterCheck.mandatory("Person", person);
        return this.authorityService.isGuestAuthority(
                (String) person.getProperties().get(ContentModel.PROP_USERNAME));
    }
    ...
    /**
     * Return a map of the Person properties that are marked as immutable for the given user. This
     * enables a script to interogate which properties are dealt with by an external system such as
     * LDAP and should not be mutable in any client UI.
     *
     ...
     */
    private Object[] getContainedAuthorities(
            ScriptNode container, AuthorityType type, boolean recurse) {
        Object[] members = null;

        if (container.getQNameType().equals(ContentModel.TYPE_AUTHORITY_CONTAINER)) {
            String groupName =
                    (String) container.getProperties().get(ContentModel.PROP_AUTHORITY_NAME);
            Set<String> authorities =
                    authorityService.getContainedAuthorities(type, groupName, !recurse);

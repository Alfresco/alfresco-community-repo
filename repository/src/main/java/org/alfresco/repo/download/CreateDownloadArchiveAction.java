...
/**
 * {@link ActionExecuter} for creating an archive (ie. zip) file containing content from the
 * repository.
 *
 * <p>The maximum total size of the content which can be downloaded is controlled by the
 * maximumContentSie property. -1 indicates no limit.
 *
 * @author Alex Miller
 */
public class CreateDownloadArchiveAction extends ActionExecuterAbstractBase {
    ...
    private static final String CREATION_ERROR =
            "Unexpected error creating archive file for download";
    ...
    /**
     * Set the maximum total size of content that can be added to a single download. -1 indicates no
     * limit.
     */
     ...
     *
     * <p>Uses the {@link ExporterService} with custom exporters to create the archive files.
     *
     * @param actionedUponNodeRef Download node containing information required to create the
     *     archive file, and which will eventually have its content updated with the archive file.
     */
    @Override
    protected void executeImpl(Action action, final NodeRef actionedUponNodeRef) {
        ...
        final DownloadRequest downloadRequest =
                downloadStorage.getDownloadRequest(actionedUponNodeRef);
        ...
        AuthenticationUtil.runAs(
                new RunAsWork<Object>() {
                    ...
                    public Object doWork() throws Exception {
                        ...
                        ExporterCrawlerParameters crawlerParameters =
                                new ExporterCrawlerParameters();
                        ...
                        crawlerParameters.setExcludeChildAssocs(
                                new QName[] {
                                    RenditionModel.ASSOC_RENDITION, ForumModel.ASSOC_DISCUSSION
                                });
                        ...
                        SizeEstimator estimator =
                                new SizeEstimator(checkOutCheckInService, nodeService);
    ...
    private void maximumContentSizeExceeded(
            final NodeRef actionedUponNodeRef, final long size, final long fileCount) {
                ...
                new RetryingTransactionCallback<Object>() {
                    ...
                    public Object execute() throws Throwable {
                        DownloadStatus status =
                                new DownloadStatus(
                                        Status.MAX_CONTENT_SIZE_EXCEEDED,
                                        maximumContentSize,
                                        size,
                                        0,
                                        fileCount);
    ...
    private void archiveCreationComplete(
            final NodeRef actionedUponNodeRef,
            final File tempFile,
            final ZipDownloadExporter handler) {
        // Update the content and set the status to done.
                        ...
                        try {
                            ...
                            DownloadStatus status =
                                    new DownloadStatus(
                                            ...
                                            handler.getDone(),
                                            handler.getTotal(),
                                            handler.getFilesAdded(),
                                            handler.getTotalFiles());
                            updateService.update(
                                    actionedUponNodeRef, status, handler.getNextSequenceNumber());
    ...
    private void downloadCancelled(
            final NodeRef actionedUponNodeRef, final ZipDownloadExporter handler) {
        // Update the content and set the status to done.
        transactionHelper.doInTransaction(
                new RetryingTransactionCallback<Object>() {
                    @Override

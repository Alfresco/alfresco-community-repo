 ...
 *
 * <p>Adapted from
 * org.alfresco.module.org_alfresco_module_cloud.webscripts.TenantBasicHTTPAuthenticatorFactory.
 *
 * @author sglover
 */
public class PublicApiAuthenticatorFactory extends RemoteUserAuthenticatorFactory {
    ...
    /**
     * Whether to suggest that users use Basic auth. If set to true, then a 401 (unauthorized)
     * response will contain a WWW-Authentication header specifying the scheme "Basic". If this is
     * set to false, then the scheme "AlfTicket" will be used.
     *
     * <p>Set this to false to avoid getting Basic auth dialogue popups in browsers when using the
     * public API directly, for example.
     *
    ...
    public class PublicApiAuthenticator extends RemoteUserAuthenticator {
        ...
        // Proxy listener used to receive initial authentication events from the base
        ...
        public boolean authenticate(RequiredAuthentication required, boolean isGuest) {
            ...
            try {
                ...
                if (authenticatorKey != null && remoteUser != null) {
                    ...
                    authorized =
                            authenticateViaGateway(required, isGuest, authenticatorKey, remoteUser);
                } else {
                    ...
                    try {
                        ...
                        if (logger.isDebugEnabled())
                            logger.debug(
                                    "TenantBasicHttpAuthenticator: required="
                                            + required
                                            + ", isGuest="
                                            + isGuest
                                            + " - "
                                ...
                                retryingTransactionHelper.doInTransaction(
                                        new RetryingTransactionCallback<Boolean>() {
                                            public Boolean execute() throws Exception {
                                                return tenantAuthentication.authenticateTenant(
                                                        email, tenant);
                                            }
                                        },
                                        true,
                        ...
                        if (!authorized) {
                            listener.authenticationFailed(
                                    new TenantCredentials(
                                            tenant, email, proxyListener.getOrignalCredentials()));
                ...
                if (!authorized) {
                    ...
                    String challenge =
                            scheme + " realm=\"Alfresco " + servletReq.getTenant() + " tenant\"";
                    servletRes.setHeader("WWW-Authenticate", challenge);
                }
            }
        }

        private boolean authenticateViaGateway(
            ...
            if (validAuthenticatorKeys.contains(authenticatorKey)) {
                ...
                proxyListener.userAuthenticated(
                        new PublicApiCredentials(
                                authenticatorKey, remoteUser, getOutboundHeaders(servletReq)));
                return true;
            } else {
                logger.error("Invalid authenticator key:- " + authenticatorKey);
                proxyListener.authenticationFailed(
                        new PublicApiCredentials(
                                authenticatorKey, remoteUser, getOutboundHeaders(servletReq)));
                return false;
            }

 ...
 *
 * <p>Manages the modules installed in a war file. Allows modules to be installed, updated, enabled,
 * disabled and uninstalled. Information about the module installed is also available.
 *
 * @since 2.0
...
public class ModuleManagementTool implements LogOutput {
    ...
    private static final String DEFAULT_FILE_MAPPING_PROPERTIES =
            "org/alfresco/repo/module/tool/default-file-mapping.properties";
    /** Location of the AMP-specific mappings file */
    private static final String FILE_MAPPING_PROPERTIES = "file-mapping.properties";
    /**
     * The property to add to a custom {@link #FILE_MAPPING_PROPERTIES file-mapping.properties} to
     * inherit the default values. The default is <code>true</code>.
     */
    ...
    public ModuleManagementTool() {
        ...
        config.setArchiveDetector(
                new TArchiveDetector("war|amp", new JarDriver(IOPoolLocator.SINGLETON)));
        ...
        InputStream is =
                this.getClass()
                        .getClassLoader()
                        .getResourceAsStream(DEFAULT_FILE_MAPPING_PROPERTIES);
        try {
    ...
    public void installModules(
        ...
        if (dir.exists() == true) {
            ...
            throw new ModuleManagementToolException(
                    "Invalid directory '" + directoryLocation + "'");
        }
    }
        ...
        if (children != null) {
            for (java.io.File child : children) {
                if (child.isFile() == true
                        && child.getName().toLowerCase().endsWith(".amp") == true) {
                    installModule(
                            child.getPath(), warFileLocation, preview, forceInstall, backupWAR);
                } else {
     ...
     * @param warFileLocation the location of the WAR file into which the AMP file is to be
     *     installed
     */
     ...
     * @param warFileLocation the location of the WAR file into which the AMP file is to be
     *     installed.
     * @param preview indicates whether this should be a preview install. This means that the
     *     process of installation will be followed and reported, but the WAR file will not be
     *     modified.
     * @param forceInstall indicates whether the installed files will be replaced regardless of the
    ...
    public void installModule(
        ...
        try {
            outputVerboseMessage(
                    "Installing AMP '" + ampFileLocation + "' into WAR '" + warFileLocation + "'");
            if (!warFile.exists()) {
                throw new ModuleManagementToolException(
                        "The war file '" + warFile + "' does not exist.");
            }
            if (preview == false) {
            ...
            ModuleDetails installingModuleDetails =
                    ModuleDetailsHelper.createModuleDetailsFromPropertyLocation(
                            propertiesLocation, this);
            ...
            ModuleVersionNumber installingVersion =
                    installingModuleDetails.getModuleVersionNumber();
            ...
            ModuleDetails installedModuleDetails =
                    warHelper.getModuleDetailsOrAlias(warFile, installingModuleDetails);

            // Check module directory exists
            TFile moduleInstallDirectory =
                    new TFile(
                            warFileLocation + WarHelper.MODULE_NAMESPACE_DIR + "/" + installingId);
            ...
            throw new ModuleManagementToolException(
                    "An error was encountered during deployment of the AMP into the WAR: "
            ...
            try {
                ...
                throw new ModuleManagementToolException(
                        "Error when attempting to unmount WAR file: " + warFile.getPath(), e);
            }
        }
    }

    private void uninstallIfNecessary(
            String warFileLocation,
            ModuleDetails installedModuleDetails,
            boolean preview,
            boolean forceInstall,
        ...
        if (installedModuleDetails != null) {
            ...
            if (compareValue > 0) {
                ...
                outputVerboseMessage(
                        "WARNING: A later version of this module is already installed in the WAR."
                            + " Installation skipped.  You could force the installation by passing"
                            + " the -force option.",
                        false);
                return;
            }
            ...
            if (forceInstall == true) {
                ...
                outputVerboseMessage(
                        "WARNING: The installation of this module is being forced.  All files will"
                            + " be removed and replaced regardless of existing versions present.",
                        false);
            }
            ...
            if (compareValue == 0) {
                ...
                outputVerboseMessage(
                        "WARNING: This version of this module is already installed in the"
                                + " WAR..upgrading.",
                        false);
            }
    ...
    /** */
    private Properties calculateChanges(
            String ampFileLocation,
            String warFileLocation,
            boolean preview,
            boolean forceInstall,
    ...
    /**
     * Cleans the WAR file of all files relating to the currently installed version of the the
     * Module.
     *
    ...
    public void uninstallModule(
            String moduleId, String warFileLocation, boolean preview, boolean purge)
            throws IOException {
    ...
    private void removeFile(String warLocation, String filePath, boolean preview) {
        ...
        if (removeFile.exists() == true) {
            ...
            outputVerboseMessage(
                    "The file '"
                            + filePath
                            + "' was expected for removal but was not present in the war",
                    true);
        }
    }
     ...
     * @param forceInstall indicates whether the installed files will be replaces regardless of the
     *     currently installed version of the AMP.
     * @throws IOException throws any IOExpceptions thar are raised
     */
    private void calculateCopyToWar(
            String ampFileLocation,
            String warFileLocation,
            String sourceDir,
            ...
            for (java.io.File sourceChild : files) {
                ...
                if (sourceChild.isFile() == true) {
                    ...
                    if (destinationChild.exists() == false) {
                        ...
                        if (forceInstall) {
                            ...
                            if (preview == false) {
                                ...
                                TFile backupLocationDirectory =
                                        new TFile(warFileLocation + BACKUP_DIR);
                            ...
                            // Not a forced install, there is an existing file in the war, lets
                            // rollback the transaction,
                            // throw an error and explain the problem.
                            //                            File.
                            //                            ZipController zipController =
                            // ZipController.getInstance(warFile);
                            //                            zipController.reset();
                            throw new ModuleManagementToolException(
                                    "ERROR: The amp will overwrite an existing file in the war '"
                                            ...
                                            + "/"
                                            + sourceChild.getName()
                                            + "'. Execution halted.  By specifying -force , you can"
                                            + " force installation of AMP regardless of the current"
                                            + " war state.");
                        }
                    }
                    ...
                    if (createFile == true) {
                        ...
                        installedFiles.addUpdate(
                                destinationDir + "/" + sourceChild.getName(), backupLocation);
                        this.outputMessage(
                                "WARNING: The file '"
                                        + destinationDir
                                        + "/"
                                        + sourceChild.getName()
                                        + "' is being overwritten by this module. The original has"
                                        + " been backed-up to '"
                                        + backupLocation
                                        + "'",
                                true);
    ...
    public void listModules(String warLocation) {
        ...
        try {
            ...
            for (Iterator<ModuleDetails> iterator = modulesFound.iterator(); iterator.hasNext(); ) {
                ...
                outputVerboseMessage(
                        "   Version:      " + moduleDetails.getModuleVersionNumber(), true);
    ...
    /**
     * Outputs a message the console. Errors are always output, but others are only output in
     * verbose mode.
     *
     * @param message the message to output
     * @param indent indicates that the message should be formated with an indent
     * @param error indicates that the message is an error.
     * @param stdout indicates that the message should output to the console regardless of verbose
     *     setting
     */
    ...
    private static void outputUsage() {
        ...
        System.out.println("-----------------------------------------------------------\n");
        System.out.println(
                "install: Installs a AMP file(s) into an Alfresco WAR file, updates if an older"
        ...
        System.out.println(
                "                All amp files found in the directory and its sub directories are"
                        + " installed.");
        System.out.println(
                "   -force     : forces installation of AMP regardless of currently installed"
                        + " module version");
        System.out.println(

 ...
 *
 * <ul>
 *   <li>{@link #getReader()} to create a reader to the underlying content
 *   <li>{@link #getDirectWritableChannel()} to write content to the repository
 * </ul>
 *
 * @author Derek Hulley
 */
...
public abstract class AbstractContentWriter extends AbstractContentAccessor
     ...
     * the {@link ContentWriter#getReader()} method.
     *
     ...
     *     <b>always</b> be a new instance and never null.
    ...
    /**
     * This method returns the configured {@link ContentLimitProvider} for this writer. By default a
     * {@link NoLimitProvider} will be returned.
     *
     * @since Thor
     */
     ...
     *
     * <p>This is the only of the content <i>writing</i> methods that needs to be implemented by
    ...
    private WritableByteChannel getCallbackWritableChannel(
            WritableByteChannel directChannel, List<ContentStreamListener> listeners)
            throws ContentIOException {
    ...
    public FileChannel getFileChannel(boolean truncate) throws ContentIOException {
        ...
        if (channel instanceof FileChannel) {
            ...
            if (!truncate && existingContentReader != null) {
                ReadableByteChannel existingContentChannel =
                        existingContentReader.getReadableChannel();
                long existingContentLength = existingContentReader.getSize();
                // copy the existing content
                try {
                    clientFileChannel.transferFrom(
                            existingContentChannel, 0, existingContentLength);
                    // copy complete
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "Copied content for random access: \n"
                                        + "   writer: "
                                        + this
                                        + "\n"
                                        + "   existing: "
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Content writer provided direct support for FileChannel: \n"
                                + "   writer: "
                                + this);
            }
        } else {
            ...
            final FileContentWriter spoofWriter =
                    new FileContentWriter(
                            ...
                            getExistingContentReader()); // this ensures that the existing content
            // is pulled in
            // Attach a listener
                    ...
                    new ContentStreamListener() {
                        public void contentStreamClosed() throws ContentIOException {
                            ...
                            try {
                                ...
                                throw new ContentIOException(
                                        "Failed to copy from spoofed temporary channel to permanent"
                                                + " channel: \n"
                                                + "   writer: "
                                                + this
                                                + "\n"
    ...
    /**
     * When the content has been written, attempt to guess the mimetype of it, using the filename
     * and contents.
     *
    ...
    private void doGuessMimetype(String filename) {
        String mimetype;
        if (filename != null
                && filename.startsWith(MimetypeMap.MACOS_RESOURCE_FORK_FILE_NAME_PREFIX)) {
    ...
    /**
     * Our own listener that is always the first on the list, which lets us perform guessing
     * operations when the content has been written.
     */

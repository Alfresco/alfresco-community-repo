...
/**
 * The Workflow Manager serves as the main entry point for scripts to create and interact with
 * workflows. It is made available in the root scripting scope
 *
 * @author glenj
 */
public class WorkflowManager extends BaseScopableProcessorExtension {
    ...
    public JscriptWorkflowDefinition getDefinition(String id) {
        ...
        return definition == null
                ? null
                : new JscriptWorkflowDefinition(definition, services, getScope());
    }
    ...
    public JscriptWorkflowDefinition getDefinitionByName(String name) {
        WorkflowDefinition cmrWorkflowDefinition =
                services.getWorkflowService().getDefinitionByName(name);
        return cmrWorkflowDefinition == null
                ? null
                : new JscriptWorkflowDefinition(cmrWorkflowDefinition, services, getScope());
    }
    ...
    public JscriptWorkflowInstance getInstance(String workflowInstanceID) {
        WorkflowInstance instance =
                services.getWorkflowService().getWorkflowById(workflowInstanceID);
        return instance == null
                ? null
                : new JscriptWorkflowInstance(instance, services, getScope());
    }
    ...
    public Scriptable getPooledTasks(final String authority) {
        ...
        return (Scriptable)
                converter.convertValueForScript(services, getScope(), null, pooledTasks);
    }
    ...
    /**
     * Get task by id. Alternative method signature to <code>getTask(String id)</code> for those
     * used to the Template API
     *
     * @param id task id
     * @return the task (null if not found)
     */
    ...
    public Scriptable getLatestDefinitions() {
        ...
        return (Scriptable)
                new ValueConverter()
                        .convertValueForScript(services, getScope(), null, workflowDefs);
    }
    ...
    /**
     * Get tasks assigned to the current user, filtered by workflow task state. Only tasks having
    ...
    private Scriptable getAssignedTasksByState(WorkflowTaskState state) {
        ...
        return (Scriptable)
                new ValueConverter()
                        .convertValueForScript(services, getScope(), null, assignedTasks);
    }
}

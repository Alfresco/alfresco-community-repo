...
public class BasicCorrespondingNodeResolverImpl
  ...
  private static final Log log = LogFactory.getLog(
    BasicCorrespondingNodeResolverImpl.class
  );

  private static final String MSG_SPECIFIED_STORE_DOES_NOT_EXIST =
    "transfer_service.receiver.specified_store_nonexistent";
  ...
  public ResolvedParentChildPair resolveCorrespondingNode(
    ...
    if (log.isDebugEnabled()) {
      log.debug(
        "Attempting to resolve corresponding node for noderef " + sourceNodeRef
      );
    ...
    if (!nodeService.exists(parentNodeRef)) {
      ...
      parentNodeRef =
        resolveParentPath(
          primaryAssoc.getParentRef().getStoreRef(),
          parentPath
        );
    } else {
      if (log.isDebugEnabled()) {
        log.debug(
          "Node's parent has been resolved by noderef: " + parentNodeRef
        );
      }
    }
    if (log.isDebugEnabled()) {
  ...
  private NodeRef resolveParentPath(StoreRef store, Path parentPath) {
    ...
    while (index < parentPath.size()) {
      ...
      if (children.isEmpty()) {
        if (log.isDebugEnabled()) {
          log.debug(
            "Failed to resolve path element " + element.getElementString()
          );
        }
        return null;
      }

...
/**
 * This class provides support for {@link CannedQuery canned queries} which filter by Auditable
 * Properties and Target Assocs
 *
 * @author Nick Burch
 * @since 4.0
 */
public class GetChildrenWithTargetAssocsAuditableCannedQuery
    ...
    private static final String QUERY_SELECT_GET_NODES =
            "select_GetChildrenWithTargetAssocsAuditableCannedQuery";
    ...
    protected List<NodeWithTargetsEntity> queryAndFilter(CannedQueryParameters parameters) {
        ...
        if (paramBeanObj == null)
            throw new NullPointerException(
                    "Null GetChildrenWithTargetAssocsAuditable query params");

        GetChildrenWithTargetAssocsAuditableCannedQueryParams paramBean =
                (GetChildrenWithTargetAssocsAuditableCannedQueryParams) paramBeanObj;

        // note: refer to SQL for specific DB filtering (eg.parent nodes etc)
        List<NodeWithTargetsEntity> results =
                cannedQueryDAO.executeQuery(
                        QUERY_NAMESPACE, QUERY_SELECT_GET_NODES, paramBean, 0, Integer.MAX_VALUE);
        ...
        List<Pair<? extends Object, SortOrder>> sortPairs =
                parameters.getSortDetails().getSortPairs();
        ...
        if (sortPairs != null && !sortPairs.isEmpty()) {
            ...
            for (Pair<? extends Object, SortOrder> sortPair : sortPairs) {
                ...
                final NodeBackedEntityComparator comparator =
                        new NodeBackedEntityComparator(sortProperty);
                comparators.add(
                        new Pair<Comparator<NodeBackedEntity>, SortOrder>(
                                comparator, sortPair.getSecond()));
            }
            NestedComparator<NodeBackedEntity> comparator =
                    new NestedComparator<NodeBackedEntity>(comparators);

...
public interface RecordsManagementAuditService extends RecordsManagementAuditServiceDeprecated {
     ...
     *
     * <p>Creates an instance of a simple audit event and registers it with the service.
     *
    ...
    /**
     * Audits an event, assumes no properties where modified and that the event should not be
     * audited immediately.
     *
    ...
    /**
     * Audits an event, assumes that the event should not be audited immediately and not be removed
     * if no property is changed.
     *
     * @param nodeRef node reference
     * @param eventName event name
     * @param before property values before event (this must be modifiable and may be changed by the
     *     method).
     * @param after property values after event (this must be modifiable and may be changed by the
     *     method).
     */
    void auditEvent(
            NodeRef nodeRef,
     ...
     *     method).
     * @param immediate true if event is to be audited immediately, false otherwise
     * @param removeIfNoPropertyChanged true if event is not audited when there are no properties
    ...
    /**
     * Supply incremental changes as part of an event. This will either create a new event or update
     * the existing details to put any supplied properties into the map.
     *
     * @param nodeRef node reference
     * @param eventName event name
     * @param before additional property values before event (this must be modifiable and may be
     *     changed by the method).
     * @param after additional property values after event (this must be modifiable and may be
     *     changed by the method).
     * @param removeIfNoPropertyChanged true if event is not audited when there are no properties
     *     changed, false otherwise
     */
    void auditOrUpdateEvent(
            NodeRef nodeRef,
            String eventName,
            Map<QName, Serializable> before,
            Map<QName, Serializable> after,
    ...
    /**
     * Retrieves a list of audit log entries using the provided parameters represented by the
     * RecordsManagementAuditQueryParameters instance.
     *
     * <p>The parameters are all optional so an empty RecordsManagementAuditQueryParameters object
     * will result in ALL audit log entries for the RM system being returned. Setting the various
     * parameters effectively filters the full audit trail.
     *
    ...
    /**
     * Retrieves a list of audit log entries using the provided parameters represented by the
     * RecordsManagementAuditQueryParameters instance and then files the resulting log as an
     * undeclared record in the record folder represented by the given NodeRef.
     *
     * <p>The parameters are all optional so an empty RecordsManagementAuditQueryParameters object
     * will result in ALL audit log entries for the RM system being returned. Setting the various
     * parameters effectively filters the full audit trail.
     ...
     */
    NodeRef fileAuditTrailAsRecord(
            RecordsManagementAuditQueryParameters params, NodeRef destination, ReportFormat format);
}

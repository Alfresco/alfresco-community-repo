...
public class RecordActionUtils {
     ...
     */
    static NodeRef resolvePath(
            Services services, NodeRef filePlan, final String pathParameter, String actionName) {
        NodeRef destinationFolder;

        if (filePlan == null) {
            filePlan =
                    getDefaultFilePlan(
                            services.getAuthenticationUtil(),
                            services.getFilePlanService(),
                            actionName);
        }
        ...
        if (pathElementsArray.length > 0) {
            ...
            throw new AlfrescoRuntimeException(
                    "Unable to execute "
                            + actionName
                            + " action, because the destination path could not be found.");
        }
        return destinationFolder;
    }

    /**
     * Helper method to recursively get the next path element node reference from the action path
     * parameter
     *
     ...
     */
    static NodeRef resolvePath(
            NodeService nodeService, NodeRef parent, List<String> pathElements, String actionName) {
        NodeRef nodeRef;
        ...
        if (nodeType.equals(RecordsManagementModel.TYPE_HOLD_CONTAINER)
            ...
            throw new AlfrescoRuntimeException(
                    "Unable to execute "
                            + actionName
                            + " action, because the destination path is invalid.");
        }
     ...
     */
    static NodeRef getDefaultFilePlan(
            AuthenticationUtil authenticationUtil,
            FilePlanService filePlanService,
            String actionName) {
        NodeRef filePlan =
        ...
        if (filePlan == null) {
            final String logMessage =
                    String.format(
                            "Unable to execute %s action, because the fileplan path could not be"
                                + " determined. Make sure at least one file plan has been created.",
                            actionName);

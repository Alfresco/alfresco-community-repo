 ...
 * transactional batch process. It iterates over a collection, and queues jobs that fire a worker on
 * a batch of members. The queued jobs handle progress / error reporting, transaction delineation
 * and retrying. They are processed in parallel by a pool of threads of a configurable size. The job
 * processing is designed to be fault tolerant and will continue in the event of errors. When the
 * batch is complete a summary of the number of errors and the last error stack trace will be logged
 * at ERROR level. Each individual error is logged at WARN level and progress information is logged
 * at INFO level. Through the {@link BatchMonitor} interface, it also supports the real-time
 * monitoring of batch metrics (e.g. over JMX in the Enterprise Edition).
 *
 * @author dward
 */
@AlfrescoPublicApi
public class BatchProcessor<T> implements BatchMonitor {
    ...
    /**
     * What transactions need to be retried?. We do these single-threaded in order to avoid
     ...
     * @param batchSize the number of entries we process at a time in a transaction
     * @param applicationEventPublisher the application event publisher (may be <tt>null</tt>)
     * @param logger the logger to use (may be <tt>null</tt>)
     * @param loggingInterval the number of entries to process before reporting progress
     * @deprecated Since 3.4, use the {@link BatchProcessWorkProvider} instead of the
     *     <tt>Collection</tt>
     */
     ...
     * @param processName the process name
     * @param retryingTransactionHelper the retrying transaction helper
     * @param workProvider the object providing the work packets
     * @param workerThreads the number of worker threads
     * @param batchSize the number of entries we process at a time in a transaction
     * @param applicationEventPublisher the application event publisher (may be <tt>null</tt>)
     * @param logger the logger to use (may be <tt>null</tt>)
    ...
    /**
     * Invokes the worker for each entry in the collection, managing transactions and collating
     * success / failure information.
     *
     * @param worker the worker
     * @param splitTxns Can the modifications to Alfresco be split across multiple transactions for
     *     maximum performance? If <code>true</code>, worker invocations are isolated in separate
     *     transactions in batches for increased performance. If <code>false</code>, all invocations
     *     are performed in the current transaction. This is required if calling synchronously (e.g.
     *     in response to an authentication event in the same transaction).
    ...
    private long process(final BatchProcessWorker<T> worker, final boolean splitTxns, long count) {
        synchronized (this) {
            ...
            if (this.logger.isInfoEnabled()) {
                if (count >= 0) {
                    this.logger.info(
                            getProcessName() + ": Commencing batch of " + count + " entries");
                } else {
        ...
        // Create a thread pool executor with the specified number of threads and a finite blocking
        // queue of jobs
        ExecutorService executorService =
                splitTxns && this.workerThreads > 1
        ...
        try {
            ...
            synchronized (this) {
                ...
                if (this.logger.isInfoEnabled()) {
                    if (count >= 0) {
                        this.logger.info(
                                getProcessName() + ": Completed batch of " + count + " entries");
                    } else {
                ...
                if (this.totalErrors > 0 && this.logger.isErrorEnabled()) {
                    this.logger.error(
                            getProcessName()
                                    + ": "
                                    + this.totalErrors
                                    + " error(s) detected. Last error from entry \""
                                    + this.lastErrorEntryId
                                    + "\"",
                            this.lastError);
                }
            }
     ...
     *
     * @param last Have all jobs been processed? If <code>false</code> then progress is only
     *     reported after the number of entries indicated by {@link #loggingInterval}. If <code>true
     *     </code> then progress is reported if this is not one of the entries indicated by {@link
     *     #loggingInterval}.
     */
    ...
    public interface BatchProcessWorker<T> {
        ...
        /**
         * Callback to allow thread initialization before the work entries are {@link
         * #process(Object) processed}. Typically, this will include authenticating as a valid user
         * and disbling or enabling any system flags that might affect the entry processing.
         */
        ...
        /**
         * Callback to allow thread cleanup after the work entries have been {@link #process(Object)
         * processed}. Typically, this will involve cleanup of authentication and resetting any
         * system flags previously set.
         *
         * <p>This call is made regardless of the outcome of the entry processing.
         */
    ...
    @AlfrescoPublicApi
    public abstract static class BatchProcessWorkerAdaptor<TT> implements BatchProcessWorker<TT> {
    ...
    /**
     * Small iterator that repeatedly gets the next batch of work from a {@link
    ...
    class TxnCallback extends TransactionListenerAdapter
            implements RetryingTransactionCallback<Object>, Runnable {
         ...
         * @param splitTxns If <code>true</code>, the worker invocation is made in a new
         *     transaction.
         */
        ...
        public Object execute() throws Throwable {
            ...
            synchronized (BatchProcessor.this) {
                if (BatchProcessor.this.logger.isDebugEnabled()) {
                    BatchProcessor.this.logger.debug(
                            "RETRY TXNS: " + BatchProcessor.this.retryTxns);
                }
                // If we are retrying after failure, assume there are cross-dependencies and wait
                // for other
                    ...
                    if (BatchProcessor.this.logger.isDebugEnabled()) {
                        BatchProcessor.this.logger.debug(
                                Thread.currentThread().getName()
                                        + " Recoverable failure: waiting for other batches to"
                                        + " complete");
                    }
                ...
                if (BatchProcessor.this.logger.isDebugEnabled()) {
                    BatchProcessor.this.logger.debug(
                            Thread.currentThread().getName() + " ready to execute");
                }
        ...
        public void run() {
            ...
            try {
                ...
                try {
                    BatchProcessor.this.retryingTransactionHelper.doInTransaction(
                            callback, false, splitTxns);
                } catch (Throwable t) {
                ...
                if (this.splitTxns) {
                    this.txnLastError = t;
                    this.txnLastErrorEntryId =
                            (t instanceof IntegrityException) ? "unknown" : this.txnEntryId;
                    this.txnErrors++;
                    if (BatchProcessor.this.logger.isWarnEnabled()) {
                        String message =
                                (t instanceof IntegrityException)
                                        ? ": Failed on batch commit."
                ...
                // Otherwise, we have a retryable exception that we should propagate
                    ...
                    throw new AlfrescoRuntimeException(
                            "Transactional error during " + getProcessName(), t);
                }
            }
        ...
        private void commitProgress() {
            synchronized (BatchProcessor.this) {
                if (this.txnErrors > 0) {
                    long processed =
                            BatchProcessor.this.successfullyProcessedEntries
                    ...
                    long intervals =
                            (this.txnErrors + currentIncrement)
                                    / BatchProcessor.this.loggingInterval;
                    if (intervals > 0) {
                        BatchProcessor.this.totalErrors +=
                                BatchProcessor.this.loggingInterval - currentIncrement;
                ...
                if (this.txnSuccesses > 0) {
                    ...
                    long currentIncrement = processed % BatchProcessor.this.loggingInterval;
                    long newSuccess =
                            BatchProcessor.this.successfullyProcessedEntries + this.txnSuccesses;
                    // Work out the number of logging intervals we will cross and report them
                    long intervals =
                            (this.txnSuccesses + currentIncrement)
                                    / BatchProcessor.this.loggingInterval;
                    if (intervals > 0) {
                        BatchProcessor.this.successfullyProcessedEntries +=
                                BatchProcessor.this.loggingInterval - currentIncrement;
        ...
        public void afterCommit() {
            ...
            synchronized (BatchProcessor.this) {
                BatchProcessor.this.executingCount--;
                // We do the final notifications in commitProgress so we can handle a transaction
                // ending in a rollback
            }
        }

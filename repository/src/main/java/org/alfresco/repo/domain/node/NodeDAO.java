...
public interface NodeDAO extends NodeBulkLoader {
    ...
    /**
     * @return the commit time of the current transaction entry or <tt>null</tt> if there have not
     *     been any modifications to nodes registered in the transaction.
     */
    Long getCurrentTransactionCommitTime();

    /**
     * @param ensureNew <tt>true</tt> to ensure that a new transaction entry is created if the
     *     current transaction does not have one.
     * @return Returns the ID of the current transaction entry or <tt>null</tt> if there have not
     *     been any modifications to nodes registered in the transaction and <tt>forceNew</tt> is
     *     <tt>false</tt>
     */
    ...
    /**
     * Creates a unique store for the given protocol and identifier combination. The root node is
     * created with the "root" aspect.
     *
    ...
    /**
     * Find out if a node exists. Unpurged deleted nodes do not count as they are the DAO's concern
     * only.
     *
    ...
    /**
     * @return Returns <tt>true</tt> if the node was last modified in the current transaction,
     ...
     * @return Returns the current status of the reference. This will only be <tt>null</tt> if the
     *     node never existed or has been purged following deletion.
     */
    ...
    /**
     * Create a new node. Note that allowing the <b>uuid</b> to be assigned by passing in a
     ...
     * @param childNodeName the <b>cm:name</b> of the child node or <tt>null</tt> to use the node's
     *     UUID
     ...
     * @throws InvalidTypeException if the node type is invalid or if the node type is not a valid
     *     real node
     ...
     *
     * <p><b>**NEW**:</b> If the parent node's store differs from the child node's store, then a new
     ...
     * @throws NodeExistsException if the target UUID of the move (in case of a store move) already
     *     exists
     */
    public Pair<Pair<Long, ChildAssociationRef>, Pair<Long, NodeRef>> moveNode(
            Long childNodeId, Long newParentNodeId, QName assocTypeQName, QName assocQName);
    ...
    /**
     * @param nodeTypeQName the new type QName for the node or <tt>null</tt> to keep the existing
     *     one
    ...
    /**
     * An object that contains data giving a 'view' of the node to be deleted. It is used for
     * ensuring that node cleanup is only done for node-related data that has been handled by the
     * NodeService's policy and related code.
     *
     * <p>If new data is introduced to a node that has not been covered by the NodeService, then a
     * subsequent node deletion will fail.
     *
     * <p>This class is NOT thread-safe and should only be used within a transaction. Client code
     * should use the getters to get the ID sets and add any IDs that have been visited during
     * policy triggering.
     *
     * @author Derek Hulley
     * @since 4.1.1
    ...
    /**
     * Purge deleted nodes where their participating transactions are in-between the given time
     * interval.
     *
     ...
     * <b>cm:auditable</b> properties. The change may be done in the current transaction or in a
     * later transaction.
     *
     ...
     * @deprecated Use {@link #setModifiedProperties(Long, Date, String)} to also change the
     *     <b>cm:modifier</b> property
     */
     ...
     * @return Returns <tt>true</tt> if the <b>cm:modified</b> and <b>cm:modifier</b> properties
     *     were actually set
     */
     ...
     * @return Returns all the node associations where the node is the <b>source</b> or
     *     <b>target</b>
     */
     ...
     * @param propertyValue property value (<tt>null</tt> allowed only if the <b>propertyQName</b>
     *     is <tt>null</tt>)
     * @return Returns all the node associations where the node is the <b>source</b>.
     */
    ...
    /**
     * @return Returns a specific node association with the given ID or <tt>null</tt> if it doesn't
     *     exist
     */
    ...
    public interface ChildAssocRefQueryCallback {
        /** @return Return <tt>false</tt> to terminate the query i.e. stop receiving results */
        boolean handle(
        ...
        /** @return Return <tt>true</tt> if child associations must be ordered by index and ID */
    ...
    /**
     * Create a new child association. The unique enforcement for <b>cm:name</b> will be done as
     * part of the association creation i.e. there is no need to update it after the fact.
     *
     ...
     *
     * <p>The implementation may find multiple entries (there is no constraint to prevent it)
     * although the <b>cm:name</b> constraint will normally prevent the association from being
     * created twice. The lowest ID association will always be returned and the others will be
     * cleaned up if the transaction is read-write.
     *
    ...
    /**
     * Get the child associations of a given parent node, optionally filtering on association
     * <tt>QName</tt> and association type <tt>QName</tt>.
     *
     * <p>This is an efficient query for node paths.
     *
     * @param parentNodeId the parent node ID
     ...
     * @param isPrimary filter for primary (<tt>true</tt>) or secondary associations; <tt>null</tt>
     *     for no filtering.
     * @param sameStore <tt>null</tt> to ignore, <tt>true</tt> to only get children that are in the
     *     same store as the parent, or <tt>false</tt> to only get children that are in a different
     *     store from the parent.
    ...
    /**
     * Gets the first n child associations of a given parent node, optionally filtering on
     * association <tt>QName</tt> and association type <tt>QName</tt>.
     *
     * <p>This is an efficient query for node paths.
     *
     * @param parentNodeId the parent node ID
     * @param assocTypeQName the association type qname to filter on; <tt>null</tt> for no filtering
     * @param assocQName the association qname to filter on; <tt>null</tt> for no filtering
     * @param maxResults the maximum number of results to return. The query will be terminated
     *     efficiently after that number of results
     * @param resultsCallback the callback that will be called with the results
     */
    public void getChildAssocs(
            Long parentNodeId,
    ...
    /**
     * Get the child associations of a given parent node, optionally filtering on type
     * <tt>QName</tt>.
     *
     * @param parentNodeId the parent node ID
     * @param assocTypeQNames the association type qnames to filter on; <tt>null</tt> for no
     *     filtering
     * @param resultsCallback the callback that will be called with the results
     */
    public void getChildAssocs(
    ...
    /**
     * Get a child association for given parent node, association type and child node name
     * (<b>cm:name</b>).
     *
     * @param parentNodeId the parent Node ID
    ...
    /**
     * Get the child associations of a given parent node, filtering on type <tt>QName</tt> and the
     * <b>cm:name</b> of the child nodes.
     *
     * <p><b>NOTE: </b>This method only works if the association type fundamentally supports
     * unique-name enforcement.
     *
     * @param parentNodeId the parent node
     * @param assocTypeQName the type of the association to check; or <tt>null</tt> for no
     *     filtering. If the association type is not specified, then the same child node may be
     *     included several times.
     * @param childNames the names of the child nodes (<b>cm:name</b>). These will be matched
     *     exactly.
     * @param resultsCallback the callback that will be called with the results
     */
    public void getChildAssocs(
    ...
    /**
     * Gets the set of child associations of a certain parent node without parent associations of a
     * certain type to other nodes with the same parent! In effect the 'orphans' with respect to a
    ...
    /**
     * @param parentNodeId the parent node id
     * @param minNodeId the minimum node ID (inclusive), <tt>null</tt> for no limitation on the
     *     minimum value of the node id
     * @param maxNodeId the maximum node ID (exclusive), <tt>null</tt> for no limitation on the
     *     maximum value of the node id
     * @param assocToExcludeTypeQNames the node associations to exclude, <tt>null</tt> for no
     *     filtering of the associations types
     ...
     *
     * @return Returns the primary (defining) association or <tt>null</tt> if it is a root node
     */
    public Pair<Long, ChildAssociationRef> getPrimaryParentAssoc(Long childNodeId);

    /**
     * Get the parent association of a given parent node, optionally filtering on association
     * <tt>QName</tt> and association type <tt>QName</tt>.
     *
     * <p>This is an efficient query for node paths.
     *
     ...
     *
     * <p>When searching for <code>primaryOnly == true</code>, checks that there is exactly one
     * path.
     *
    ...
    /**
     * @return Returns the minimum commit time for deleted nodes or <tt>null</tt> if there are no
     *     transactions
     */
    ...
    /**
     * Returns the [minId, maxId] interval for nodes of a type, with the transaction time in the
     * given window time.
     *
     * @param type the node type
     * @param startTxnTime the starting transaction time, <tt>null</tt> is allowed, case in which no
     *     minimum transaction time is considered
     * @param endTxnTime the end transaction time, <tt>null</tt> is allowed, case in which no
     *     maximum transaction time is considered
    ...
    /**
     * Gets the minimum commit time from transactions including a node id in the range
     * [fromNodeId:toNodeId]
     *
    ...
    /**
     * Gets the maximum commit time from transactions including a node id in the range
     * [fromNodeId:toNodeId]
     *
     * @param fromNodeId Initial node id
     * @param toNodeId Final node id

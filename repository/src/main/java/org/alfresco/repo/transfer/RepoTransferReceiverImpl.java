 ...
 *
 * <p>Provides the implementation of the transfer commands on the destination repository.
 *
 * <p>Provides callback handlers for Aliens and Transferred Aspects.
 *
 * <p>Calls transfer policies.
 *
 * <p>Co-ordinates locking and logging as the transfer progresses.
 *
 * @author brian
 */
public class RepoTransferReceiverImpl
    ...
    private static final String MSG_FAILED_TO_CREATE_STAGING_FOLDER =
            "transfer_service.receiver.failed_to_create_staging_folder";
    private static final String MSG_ERROR_WHILE_STARTING = "transfer_service.receiver.error_start";
    private static final String MSG_TRANSFER_TEMP_FOLDER_NOT_FOUND =
            "transfer_service.receiver.temp_folder_not_found";
    private static final String MSG_TRANSFER_LOCK_UNAVAILABLE =
            "transfer_service.receiver.lock_unavailable";
    private static final String MSG_INBOUND_TRANSFER_FOLDER_NOT_FOUND =
            "transfer_service.receiver.record_folder_not_found";

    private static final String MSG_ERROR_WHILE_ENDING_TRANSFER =
            "transfer_service.receiver.error_ending_transfer";
    private static final String MSG_ERROR_WHILE_STAGING_SNAPSHOT =
            "transfer_service.receiver.error_staging_snapshot";
    private static final String MSG_ERROR_WHILE_STAGING_CONTENT =
            "transfer_service.receiver.error_staging_content";
    private static final String MSG_NO_SNAPSHOT_RECEIVED =
            "transfer_service.receiver.no_snapshot_received";
    private static final String MSG_ERROR_WHILE_COMMITTING_TRANSFER =
            "transfer_service.receiver.error_committing_transfer";
    private static final String MSG_ERROR_WHILE_GENERATING_REQUISITE =
            "transfer_service.receiver.error_generating_requisite";
    ...
    private static final String MSG_TRANSFER_TO_SELF =
            "transfer_service.receiver.error.transfer_to_self";
    private static final String MSG_INCOMPATIBLE_VERSIONS =
            "transfer_service.incompatible_versions";
    ...
    private final String KEY_TRANSFER_TEMP_NODEREF =
            "key.transferTempFolder.noderef"; // where temp files are stored
    private final String KEY_INBOUND_TRANSFER_RECORDS_NODEREF =
            "key.inboundTransferRecordsFolder.noderef"; // where the destination side transfer
    // report is generated

    private ClassPolicyDelegate<BeforeStartInboundTransferPolicy>
    ...
    /**
     * How long in mS to keep the lock before giving up and ending the transfer, possibly the client
     * has terminated?
     */
    ...
    public void init() {
        ...
        beforeStartInboundTransferDelegate =
                policyComponent.registerClassPolicy(
                        TransferServicePolicies.BeforeStartInboundTransferPolicy.class);
        onStartInboundTransferDelegate =
                policyComponent.registerClassPolicy(
                        TransferServicePolicies.OnStartInboundTransferPolicy.class);
        onEndInboundTransferDelegate =
                policyComponent.registerClassPolicy(
                        TransferServicePolicies.OnEndInboundTransferPolicy.class);

        /**
         * For every new child of a node with the trx:transferred aspect run
        ...
        this.getPolicyComponent()
                .bindClassBehaviour(
                        ...
                        TransferModel.ASPECT_TRANSFERRED,
                        new JavaBehaviour(
                                this, "onContentUpdate", NotificationFrequency.EVERY_EVENT));
        ...
        this.getPolicyComponent()
                .bindClassBehaviour(
                        ...
                        TransferModel.ASPECT_TRANSFERRED,
                        new JavaBehaviour(
                                this, "onCopyTransferred", NotificationFrequency.EVERY_EVENT));
        ...
        this.getPolicyComponent()
                .bindClassBehaviour(
                        ...
                        TransferModel.ASPECT_ALIEN,
                        new JavaBehaviour(
                                this, "beforeDeleteNode", NotificationFrequency.EVERY_EVENT));
        ...
        this.getPolicyComponent()
                .bindClassBehaviour(
                        ...
                        TransferModel.ASPECT_ALIEN,
                        new JavaBehaviour(
                                this, "onRestoreNode", NotificationFrequency.EVERY_EVENT));
        ...
        if (!tempFolder.exists()) {
            if (!tempFolder.mkdirs()) {
                ...
                throw new TransferException(
                        MSG_FAILED_TO_CREATE_STAGING_FOLDER, new Object[] {transferId});
            }
        }
        return tempFolder;
    }
    ...
    public NodeRef getTempFolder(String transferId) {
        ...
        if (transferTempFolder == null) {
            ...
            if (refs.size() > 0) {
                ...
                throw new TransferException(
                        ...
                        new Object[] {transferId, transferTempFolderPath});
            }
        }
        ...
        QName folderName =
                QName.createQName(NamespaceService.APP_MODEL_1_0_URI, tempTransferFolderName);

        // Do we already have a temp folder for this transfer?
        List<ChildAssociationRef> tempChildren =
                nodeService.getChildAssocs(
                        transferTempFolder, RegexQNamePattern.MATCH_ALL, folderName);
            ...
            tempFolderNode =
                    nodeService
                            .createNode(
                                    ...
                                    folderName,
                                    TransferModel.TYPE_TEMP_TRANSFER_STORE,
                                    props)
                            .getChildRef();
        } else {
     ...
     */
    public String start(
            String fromRepositoryId, boolean transferToSelf, TransferVersion fromVersion) {
        ...
        if (!getTransferVersionChecker().checkTransferVersions(fromVersion, toVersion)) {
            throw new TransferException(
                    MSG_INCOMPATIBLE_VERSIONS, new Object[] {"None", fromVersion, toVersion});
        }
        ...
        String lockStr =
                tenantDomain.isEmpty()
                        ? "transfer.server.default"
                        : "transfer.server.tenant." + tenantDomain;
                        ...
                        txHelper.doInTransaction(
                                new RetryingTransactionHelper.RetryingTransactionCallback<
                                    ...
                                    public String execute() throws Throwable {
                                        final NodeRef relatedTransferRecord =
                                        ...
                                        TransferServicePolicies.OnStartInboundTransferPolicy
                                                onStartPolicy =
                                                        onStartInboundTransferDelegate.get(
    ...
    private NodeRef createTransferRecord() {
        ...
        NodeRef inboundTransferRecordsFolder =
                singletonCache.get(KEY_INBOUND_TRANSFER_RECORDS_NODEREF);
        ...
        if (inboundTransferRecordsFolder == null) {
            ...
            if (refs.size() > 0) {
                ...
                singletonCache.put(
                        KEY_INBOUND_TRANSFER_RECORDS_NODEREF, inboundTransferRecordsFolder);
        ...
        props.put(
                TransferModel.PROP_TRANSFER_STATUS, TransferProgress.Status.PRE_COMMIT.toString());
     ...
     *
     * <p>This is the last chance to clean up.
     *
    ...
    private void timeout(final String transferId) {
         ...
         * timer thread.
         */
        final RetryingTransactionCallback<Void> timeoutCB =
                new RetryingTransactionCallback<Void>() {
                    ...
                    public Void execute() throws Throwable {
                        ...
                        if (progress.getStatus().equals(TransferProgress.Status.PRE_COMMIT)) {
                            ...
                            /**
                             * Did not get out of PRE_COMMIT. The client has probably "gone away"
                             * after calling "start", but before calling commit, cancel or error.
                             */
                            ...
                            getProgressMonitor()
                                    .logException(
                                            transferId,
                                            "transfer timeout",
                                            new TransferException(MSG_LOCK_TIMED_OUT, msgParams));
                            getProgressMonitor()
                                    .updateStatus(transferId, TransferProgress.Status.ERROR);
                        } else {
                            ...
                            // transfer in another thread.   Although why, in that case, are we
                            // here?
                            log.warn(
                                    "Inbound Transfer Lock Timeout - already past PRE-COMMIT - do"
                                            + " no cleanup transferId:"
                                            + transferId);
                        }
                        return null;
                    }
        ...
        AuthenticationUtil.runAs(
                new AuthenticationUtil.RunAsWork<String>() {
                    public String doWork() throws Exception {
                        transactionService
                                .getRetryingTransactionHelper()
                                .doInTransaction(timeoutCB, false, true);
                        return null;
                    }
                },
    ...
    public void end(final String transferId) {
        ...
        try {
            ...
            throw new TransferException(
                    MSG_ERROR_WHILE_ENDING_TRANSFER, new Object[] {transferId}, ex);
        }
    }
    ...
    private void removeTempFolders(final String transferId) {
        ...
        try {
            ...
            log.warn(
                    "Failed to delete temp store node for transfer id "
                            + transferId
                            + "\nTemp store noderef = "
                            + tempStoreNode);
        }
        ...
        try {
            ...
            log.warn(
                    "Failed to delete staging folder for transfer id "
                            + transferId
                            + "\nStaging folder = "
    ...
    public void saveSnapshot(String transferId, InputStream openStream) throws TransferException {
        ...
        try {
            ...
            try {
                ...
                throw new TransferException(
                        MSG_ERROR_WHILE_STAGING_SNAPSHOT, new Object[] {transferId}, ex);
            }
        } finally {
            lock.enableLockTimeout();
        }
    ...
    public void saveContent(String transferId, String contentFileId, InputStream contentStream)
        ...
        try {
            ...
            if (stagedFile.createNewFile()) {
                FileCopyUtils.copy(
                        contentStream, new BufferedOutputStream(new FileOutputStream(stagedFile)));
            }
        } catch (Exception ex) {
            throw new TransferException(
                    MSG_ERROR_WHILE_STAGING_CONTENT, new Object[] {transferId, contentFileId}, ex);
        } finally {
            lock.enableLockTimeout();
        }
    }
    ...
    public void commitAsync(String transferId) {
        ...
        try {
            ...
            commitAction.setParameterValue(
            ...
            throw new TransferException(
                    MSG_ERROR_WHILE_COMMITTING_TRANSFER, new Object[] {transferId}, error);
        }
    ...
    public void commit(final String transferId) throws TransferException {
        ...
        /**
         * A side-effect of checking the lock here is that it ensures that the lock timeout is
         * suspended.
         */
        ...
        try {
            ...
            RetryingTransactionHelper.RetryingTransactionCallback<Object> commitWork =
                    new RetryingTransactionCallback<Object>() {
                        public Object execute() throws Throwable {
                            AlfrescoTransactionSupport.bindListener(
                                    new TransferCommitTransactionListener(
                                            transferId, RepoTransferReceiverImpl.this));

                            List<TransferManifestProcessor> commitProcessors =
                                    manifestProcessorFactory.getCommitProcessors(
                                            RepoTransferReceiverImpl.this, transferId);

                            Set<TransferSummaryReport> summaryReports =
                                    new HashSet<TransferSummaryReport>();
                            ...
                            if (snapshotFile.exists()) {
                                ...
                                for (TransferManifestProcessor processor : commitProcessors) {
                                    XMLTransferManifestReader reader =
                                            new XMLTransferManifestReader(processor);

                                    // behaviourFilter.disableBehaviour(ContentModel.ASPECT_AUDITABLE);
                                    behaviourFilter.disableBehaviour();
                                    if (processor instanceof TransferSummaryAware) {
                                        summaryReports.add(
                                                ((TransferSummaryAware) processor)
                                ...
                                progressMonitor.logException(
                                        transferId,
                                        "Unable to start commit. No snapshot file received",
                                        new TransferException(
                                                MSG_NO_SNAPSHOT_RECEIVED,
            ...
            transactionService
                    .getRetryingTransactionHelper()
                    .doInTransaction(commitWork, false, true);
            ...
            try {
                ...
                log.error(
                        "Failed to clean up transfer. Lock may still be in place: " + transferId,
                        ex);
            }
        }
    }
    ...
    /** @param transferLockFolderPath the transferLockFolderPath to set */
    ...
    /** @param rootStagingDirectory the rootTransferFolder to set */
    public void setRootStagingDirectory(String rootStagingDirectory) {
        this.rootStagingDirectory = rootStagingDirectory;
    }

    /** @param inboundTransferRecordsPath the inboundTransferRecordsPath to set */
        ...
        try {
            ...
            if (snapshotFile.exists()) {
                ...
                TransferManifestProcessor processor =
                        manifestProcessorFactory.getRequsiteProcessor(
                                RepoTransferReceiverImpl.this, transferId, writer);
    ...
    /**
     * When a new node is created as a child of a Transferred or Alien node then the new node needs
     * to be marked as an alien.
     *
     * <p>Then the tree needs to be walked upwards to mark all parent transferred nodes as alien.
     */
     ...
     *
     * <p>Walk the tree checking the invasion status!
     */
    ...
    /**
     * When a transferred node is restored it may be a new invader or it may no longer be an
     * invader.
     *
     * <p>Walk the tree checking the invasion status!
     */
    ...
    /**
     * When an alien node is moved it may un-invade its old location and invade a new location. The
     * node may also cease to be alien.
     */
    ...
    private static class TransferredAspectCopyBehaviourCallback
        ...
        private static final CopyBehaviourCallback INSTANCE =
                new TransferredAspectCopyBehaviourCallback();
    ...
    private static class AlienAspectCopyBehaviourCallback extends DefaultCopyBehaviourCallback {
        private static final CopyBehaviourCallback INSTANCE =
                new AlienAspectCopyBehaviourCallback();
    ...
    private class Lock implements JobLockService.JobLockRefreshCallback {
        ...
        public synchronized void makeLock() {
            ...
            if (log.isDebugEnabled()) {
                ...
                log.debug(
                        "register lock callback, target lock refresh time :"
                                + getLockRefreshTime());
            }
         ...
         *
         * <p>Called on main transfer thread as transfer proceeds.
         *

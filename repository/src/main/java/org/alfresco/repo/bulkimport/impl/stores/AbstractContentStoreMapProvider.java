...
/**
 * Common elements of the role of a {@link ContentStoreMapProvider}. Extending classes should
 * implement {@link #setUpStoreMap()} to initialize the {@link Map}.
 *
 * @since 4.0
 */
public abstract class AbstractContentStoreMapProvider extends AbstractLifecycleBean
    ...
    /**
     * Check that the given store name is in the list. Also check it's an instance of {@link
     * FileContentStore}. If it's not, output a warning as non-file-based implementations have not
     * been tested and may be unsupported.
     *
    ...
    public ContentStore checkAndGetStore(String storeName) {
        ...
        if (store == null) {
            ...
            throw new IllegalArgumentException(
                    "given store name : '"
                            + storeName
                            + "' is not part of the registered stores : "
                            + validStores);
        }
        if (!(store instanceof FileContentStore)) {
            // letting you off with a warning :)
            // some people may have a custom content store for which the import could work in this
            // case too ...
            if (logger.isWarnEnabled()) {
                logger.warn(
                        "selected store '"
                                + storeName
                                + "' is not a FileContentStore. Is the implementation based on"
                                + " local files ?");
            }
        }

...
public class AddCommentTests extends RestTest {
    ...
    @TestRail(
            section = {TestGroup.REST_API, TestGroup.COMMENTS},
            executionType = ExecutionType.SANITY,
            description = "Verify unauthenticated user gets status code 401 on post comments call")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.SANITY})
    ...
    public void managerIsAbleToAddComment() throws Exception {
        ...
        String contentSiteManger =
                "This is a new comment added by user with role: " + UserRole.SiteManager;
        RestCommentModel createdComment =
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Contributor user adds comments with Rest API and status code is 201")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    @Bug(id = "ACE-4614")
    public void contributorIsAbleToAddComment() throws Exception {
        restClient.authenticateUser(usersWithRoles.getOneUserWithRole(UserRole.SiteContributor));
        String contentSiteContributor =
                "This is a new comment added by user with role" + UserRole.SiteContributor;
        RestCommentModel createdComment =
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Collaborator user adds comments with Rest API and status code is 201")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    public void collaboratorIsAbleToAddComment() throws Exception {
        restClient.authenticateUser(usersWithRoles.getOneUserWithRole(UserRole.SiteCollaborator));
        String contentSiteCollaborator =
                "This is a new comment added by user with role: " + UserRole.SiteCollaborator;
        restClient
                .withCoreAPI()
                .usingResource(document)
                .addComment(contentSiteCollaborator)
                .assertThat()
                .field("content")
                .isNotEmpty()
                .and()
                .field("content")
                .is(contentSiteCollaborator);
        restClient.assertStatusCodeIs(HttpStatus.CREATED);
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify Consumer user can't add comments with Rest API and status code is 403")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    public void consumerIsNotAbleToAddComment() throws Exception {
        restClient.authenticateUser(usersWithRoles.getOneUserWithRole(UserRole.SiteConsumer));
        String contentSiteConsumer =
                "This is a new comment added by user with role: " + UserRole.SiteConsumer;
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that invalid request returns status code 404 for nodeId that does not"
                            + " exist")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that request using nodeId that is neither document or folder returns"
                            + " 405")
    @Bug(id = "MNT-16904")
    @Test(groups = {TestGroup.REST_API, TestGroup.COMMENTS, TestGroup.REGRESSION})
    ...
    public void addCommentWithNonMemberOfPrivateSite() throws Exception {
        ...
        restClient.authenticateUser(member).withCoreAPI().usingResource(file).addComment(comment);
        restClient
                .assertStatusCodeIs(HttpStatus.FORBIDDEN)
                .assertLastError()
                .containsSummary(RestErrorModel.PERMISSION_WAS_DENIED);

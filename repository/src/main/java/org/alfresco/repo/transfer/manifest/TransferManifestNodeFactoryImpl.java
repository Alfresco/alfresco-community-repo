...
public class TransferManifestNodeFactoryImpl
  ...
  public TransferManifestNode createTransferManifestNode(
    ...
    if (status == null) {
      throw new TransferException(
        "Unable to get node status for node : " + nodeRef
      );
    }
    ...
    if (status.isDeleted()) {
      ...
      ChildAssociationRef dummyPrimaryParent = new ChildAssociationRef(
        ContentModel.ASSOC_CONTAINS,
        nodeRef,
        QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "dummy"),
        nodeRef,
        true,
        -1
      );
      ...
      if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_ARCHIVED)) {
        ...
        ChildAssociationRef car = (ChildAssociationRef) nodeService.getProperty(
          nodeRef,
      ...
      node.setProperties(
        getNodeProperties(
          nodeRef,
          definition == null ? null : definition.getExcludedAspects()
        )
      );
      node.setAspects(
        getNodeAspects(
      ...
      if (
        dictionaryService.isSubClass(node.getType(), ContentModel.TYPE_CONTENT)
      ) {
        node.setAncestorType(ContentModel.TYPE_CONTENT);
      } else {
        if (
          dictionaryService.isSubClass(node.getType(), ContentModel.TYPE_FOLDER)
        ) {
      ...
      List<ManifestPermission> mps = new ArrayList<ManifestPermission>(
        permissions.size()
      );
              ...
              if (thing instanceof java.util.Collection) {
                ...
                for (NodeRef categoryNodeRef : c) {
                  if (categoryNodeRef != null) {
                    categories.put(
                      categoryNodeRef,
                      getManifestCategory(transferContext, categoryNodeRef)
                    );
                  }
                }
              } else {
   ...
   */
  private Set<QName> getNodeAspects(
    NodeRef nodeRef,
   ...
   */
  private Map<QName, Serializable> getNodeProperties(
    NodeRef nodeRef,
    Set<QName> excludedAspects
  ) {
    Map<QName, Serializable> properties = mlAwareNodeService.getProperties(
      nodeRef
    );
    if (excludedAspects == null || excludedAspects.size() == 0) {
      return properties;
    } else {
      Map<QName, Serializable> filteredProperties = new HashMap<QName, Serializable>(
        properties.size()
      );
      for (Map.Entry<QName, Serializable> property : properties.entrySet()) {
        ...
        if (
          propDef == null ||
          !excludedAspects.contains(propDef.getContainerClass().getName())
        ) {
  ...
  private ManifestCategory getManifestCategory(
    TransferContext transferContext,
    NodeRef categoryNodeRef
  ) {
    ManifestCategory c = transferContext
      .getManifestCategoriesCache()
      .get(categoryNodeRef);

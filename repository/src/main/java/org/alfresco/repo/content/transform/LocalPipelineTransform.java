...
public class LocalPipelineTransform extends AbstractLocalTransform {
  ...
  public LocalTransform getIntermediateTransformer(int i) {
    return i >= transformers.size()
      ? null
      : transformers.get(i).intermediateTransformer;
  }
  ...
  protected void transformImpl(
    ...
    for (int i = 0; i <= lastI; i++) {
      ...
      if (i == lastI) {
        ...
        String sourceExt = mimetypeService.getExtension(
          currentReader.getMimetype()
        );
        String targetExt = mimetypeService.getExtension(

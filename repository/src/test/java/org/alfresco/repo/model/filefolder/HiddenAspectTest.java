...
public class HiddenAspectTest {
  ...
  public void setup() throws SystemException, NotSupportedException {
    ...
    interceptor =
      (FilenameFilteringInterceptor) ctx.getBean(
    ...
    cmisDisableHide =
      Boolean.getBoolean(
        properties.getProperty("cmis.disable.hidden.leading.period.files")
      );
  ...
  public void tearDown() throws Exception {
    try {
      if (
        txn.getStatus() != Status.STATUS_ROLLEDBACK &&
        txn.getStatus() != Status.STATUS_COMMITTED
      ) {
        txn.commit();
      }
    } catch (Throwable e) {
  ...
  public void testHiddenFilesEnhancedClient() {
    ...
    try {
      ...
      NodeRef parent = fileFolderService
        .create(topNodeRef, "New Folder", ContentModel.TYPE_FOLDER)
        .getNodeRef();
      NodeRef child = fileFolderService
        .create(parent, "file.tmp", ContentModel.TYPE_CONTENT)
        .getNodeRef();
      ...
      parent =
        fileFolderService
          .create(topNodeRef, "abc", ContentModel.TYPE_FOLDER)
          .getNodeRef();
      child =
        fileFolderService
          .create(parent, ".TemporaryItems", ContentModel.TYPE_FOLDER)
          .getNodeRef();
      NodeRef child1 = fileFolderService
        .create(child, "inTemporaryItems", ContentModel.TYPE_FOLDER)
        .getNodeRef();
      assertTrue(nodeService.hasAspect(child, ContentModel.ASPECT_TEMPORARY));
      assertTrue(nodeService.hasAspect(child, ContentModel.ASPECT_HIDDEN));
      assertTrue(
        nodeService.hasAspect(child, ContentModel.ASPECT_INDEX_CONTROL)
      );
      ...
      assertTrue(
        nodeService.hasAspect(child1, ContentModel.ASPECT_INDEX_CONTROL)
      );
      ...
      parent =
        fileFolderService
          .create(topNodeRef, "Folder 2", ContentModel.TYPE_FOLDER)
          .getNodeRef();
      child =
        fileFolderService
          .create(parent, "Thumbs.db", ContentModel.TYPE_CONTENT)
          .getNodeRef();
      ...
      assertEquals(
        Visibility.Visible,
        hiddenAspect.getVisibility(Client.ftp, child)
      );
      assertEquals(
        Visibility.Visible,
        hiddenAspect.getVisibility(Client.nfs, child)
      );
      assertEquals(
        Visibility.Visible,
        hiddenAspect.getVisibility(Client.webdav, child)
      );
      assertEquals(
        Visibility.HiddenAttribute,
        hiddenAspect.getVisibility(Client.cifs, child)
      );
      assertEquals(
        Visibility.NotVisible,
        hiddenAspect.getVisibility(Client.script, child)
      );
      assertEquals(
        Visibility.NotVisible,
        hiddenAspect.getVisibility(Client.webclient, child)
      );

      // surf-config should not be visible to any client
      NodeRef node = fileFolderService
        .create(topNodeRef, "surf-config", ContentModel.TYPE_FOLDER)
        .getNodeRef();
      assertTrue(nodeService.hasAspect(node, ContentModel.ASPECT_HIDDEN));
      assertTrue(
        nodeService.hasAspect(node, ContentModel.ASPECT_INDEX_CONTROL)
      );
      for (Client client : hiddenAspect.getClients()) {
        if (!client.equals(Client.admin)) {
          assertEquals(
            Visibility.NotVisible,
            hiddenAspect.getVisibility(client, node)
          );
        }
      }
      ...
      node =
        fileFolderService
          .create(topNodeRef, ".DS_Store", ContentModel.TYPE_CONTENT)
          .getNodeRef();
      assertTrue(nodeService.hasAspect(node, ContentModel.ASPECT_HIDDEN));
      assertTrue(
        nodeService.hasAspect(node, ContentModel.ASPECT_INDEX_CONTROL)
      ...
      for (Client client : hiddenAspect.getClients()) {
        if (client == Client.cifs) {
          assertEquals(
            "Should have hidden attribute set for client " + client,
            Visibility.HiddenAttribute,
            hiddenAspect.getVisibility(client, node)
          );
        } else if (
          ...
          assertEquals(
            "Should be visible for client " + client,
            Visibility.Visible,
            hiddenAspect.getVisibility(client, node)
          );
        } else if (client != Client.admin) {
          assertEquals(
            "Should not be visible for client " + client,
            Visibility.NotVisible,
            hiddenAspect.getVisibility(client, node)
          );
        }
      ...
      node =
        fileFolderService
          .create(topNodeRef, "._resourceFork", ContentModel.TYPE_FOLDER)
          .getNodeRef();
      assertTrue(nodeService.hasAspect(node, ContentModel.ASPECT_HIDDEN));
      assertTrue(
        nodeService.hasAspect(node, ContentModel.ASPECT_INDEX_CONTROL)
  ...
  public void testClientControlled() throws Exception {
    // test that a client controlled hidden node is not subject to hidden file patterns
    {
      ...
      try {
        ...
        child =
          fileFolderService
            .create(parent, "folder11", ContentModel.TYPE_FOLDER)
            .getNodeRef();
        child1 =
          fileFolderService
            .create(child, "folder21", ContentModel.TYPE_FOLDER)
            .getNodeRef();
        assertTrue(nodeService.hasAspect(child, ContentModel.ASPECT_HIDDEN));
        assertTrue(
          nodeService.hasAspect(child, ContentModel.ASPECT_INDEX_CONTROL)
        ...
        assertTrue(
          nodeService.hasAspect(parent, ContentModel.ASPECT_INDEX_CONTROL)
        );
        assertTrue(nodeService.hasAspect(child, ContentModel.ASPECT_HIDDEN));
        assertTrue(
          nodeService.hasAspect(child, ContentModel.ASPECT_INDEX_CONTROL)
    ...
    // (node is not client-controlled hidden)
    {
      ...
      try {
        parent =
          fileFolderService
            .create(topNodeRef, "." + GUID.generate(), ContentModel.TYPE_FOLDER)
            .getNodeRef();
        child =
          fileFolderService
            .create(parent, "folder11", ContentModel.TYPE_FOLDER)
      ...
      assertTrue(
        nodeService.hasAspect(child1, ContentModel.ASPECT_HIDDEN) !=
        cmisDisableHide
      );
      assertTrue(
        nodeService.hasAspect(child1, ContentModel.ASPECT_INDEX_CONTROL) !=
    ...
    if (imapEnabled) {
      ...
      try {
        ...
        assertEquals(
          ...
          0,
          numMailboxes(user, MAILBOX_NAME_A)
        );
        assertEquals(
          "Can't rename mailbox",
          1,
          numMailboxes(user, MAILBOX_NAME_B)
        );
      } finally {
        FileFilterMode.clearClient();
      }
  ...
  public void testRename() {
    ...
    try {
      ...
      String nodeName = GUID.generate();
      NodeRef node = fileFolderService
        .create(topNodeRef, nodeName, ContentModel.TYPE_FOLDER)
        .getNodeRef();
      NodeRef node11 = fileFolderService
        .create(node, nodeName + ".11", ContentModel.TYPE_FOLDER)
        .getNodeRef();
      NodeRef node12 = fileFolderService
        .create(node, nodeName + ".12", ContentModel.TYPE_CONTENT)
        .getNodeRef();
      NodeRef node21 = fileFolderService
        .create(node11, nodeName + ".21", ContentModel.TYPE_FOLDER)
        .getNodeRef();
      NodeRef node22 = fileFolderService
        .create(node11, nodeName + ".22", ContentModel.TYPE_CONTENT)
        .getNodeRef();
      NodeRef node31 = fileFolderService
        .create(node21, ".31", ContentModel.TYPE_FOLDER)
        .getNodeRef();
      NodeRef node41 = fileFolderService
        .create(node31, nodeName + ".41", ContentModel.TYPE_CONTENT)
        .getNodeRef();
      ...
      assertFalse(
        nodeService.hasAspect(node11, ContentModel.ASPECT_INDEX_CONTROL)
      );
      assertFalse(
        nodeService.hasAspect(node12, ContentModel.ASPECT_INDEX_CONTROL)
      );
      assertFalse(
        nodeService.hasAspect(node12, ContentModel.ASPECT_INDEX_CONTROL)
      );
      ...
      assertFalse(
        nodeService.hasAspect(node21, ContentModel.ASPECT_INDEX_CONTROL)
      );
      assertFalse(nodeService.hasAspect(node22, ContentModel.ASPECT_HIDDEN));
      assertFalse(
        nodeService.hasAspect(node22, ContentModel.ASPECT_INDEX_CONTROL)
      );
      assertTrue(nodeService.hasAspect(node31, ContentModel.ASPECT_HIDDEN));
      assertTrue(
        nodeService.hasAspect(node31, ContentModel.ASPECT_INDEX_CONTROL)
      );
      assertTrue(nodeService.hasAspect(node41, ContentModel.ASPECT_HIDDEN));
      assertTrue(
        nodeService.hasAspect(node41, ContentModel.ASPECT_INDEX_CONTROL)
      );
  ...
  public void testFolderRename() {
    ...
    try {
      ...
      NodeRef node = fileFolderService
        .create(topNodeRef, nodeName, ContentModel.TYPE_FOLDER)
        .getNodeRef();
      NodeRef checkedOutNode = fileFolderService
        .create(node, guid + ".lockedchild", ContentModel.TYPE_CONTENT)
        .getNodeRef();

      workingCopyNodeRef = cociService.checkout(checkedOutNode);
      assertNotNull(workingCopyNodeRef);
      assertTrue(
        nodeService.hasAspect(checkedOutNode, ContentModel.ASPECT_CHECKED_OUT)
      );
      assertTrue(
        nodeService.hasAspect(checkedOutNode, ContentModel.ASPECT_LOCKABLE)
      );
  ...
  public void testHiddenFilesBasicClient() {
    if (imapEnabled) {
      ...
      try {
        ...
        parent =
          fileFolderService
            .create(topNodeRef, ".TemporaryItems", ContentModel.TYPE_FOLDER)
            .getNodeRef();
        child =
          fileFolderService
            .create(parent, "inTemporaryItems", ContentModel.TYPE_FOLDER)
            .getNodeRef();
        assertFalse(
          nodeService.hasAspect(parent, ContentModel.ASPECT_TEMPORARY)
        );
        assertFalse(nodeService.hasAspect(parent, ContentModel.ASPECT_HIDDEN));
        assertFalse(
          nodeService.hasAspect(parent, ContentModel.ASPECT_INDEX_CONTROL)
        ...
        assertFalse(
          nodeService.hasAspect(child, ContentModel.ASPECT_TEMPORARY)
        );
        assertFalse(nodeService.hasAspect(child, ContentModel.ASPECT_HIDDEN));
        assertFalse(
          nodeService.hasAspect(child, ContentModel.ASPECT_INDEX_CONTROL)
        ...
        node =
          fileFolderService
            .create(topNodeRef, nodeName, ContentModel.TYPE_CONTENT)
            .getNodeRef();
        assertFalse(nodeService.hasAspect(node, ContentModel.ASPECT_HIDDEN));
        assertFalse(
          nodeService.hasAspect(node, ContentModel.ASPECT_INDEX_CONTROL)
  ...
  public void testCheckHidden() throws Exception {
    ...
    try {
      ...
      NodeRef node11 = fileFolderService
        ...
        .getNodeRef();
      NodeRef node12 = fileFolderService
        .create(node, ".12", ContentModel.TYPE_CONTENT)
        .getNodeRef();
      NodeRef node21 = fileFolderService
        .create(node11, nodeName + ".21", ContentModel.TYPE_FOLDER)
        .getNodeRef();
  ...
  public void testHideNodeExplicit() throws Exception {
    ...
    try {
      ...
      NodeRef parent = fileFolderService
        .create(topNodeRef, "New Folder", ContentModel.TYPE_FOLDER)
        .getNodeRef();
      NodeRef childA = fileFolderService
        .create(parent, "fileA", ContentModel.TYPE_CONTENT)
        .getNodeRef();
      NodeRef childB = fileFolderService
        .create(parent, "Thumbs.db", ContentModel.TYPE_CONTENT)
        .getNodeRef();
      ...
      assertTrue(
        "node a should be hidden",
        nodeService.hasAspect(childA, ContentModel.ASPECT_HIDDEN)
      );
  ...
  private List<NodeRef> getHiddenNodes(final StoreRef storeRef) {
    ...
    NodeRefQueryCallback resultsCallback = new NodeRefQueryCallback() {
      ...
      public boolean handle(Pair<Long, NodeRef> nodePair) {
        if (
          storeRef == null ||
          nodePair.getSecond().getStoreRef().equals(storeRef)
        ) {
    ...
    nodeDAO.getNodesWithAspects(
      Collections.singleton(ContentModel.ASPECT_HIDDEN),
      0l,
      Long.MAX_VALUE,
      resultsCallback
    );
  ...
  private int numMailboxes(AlfrescoImapUser user, String mailboxName) {
    ...
    try {
      List<AlfrescoImapFolder> folders = imapService.listMailboxes(
        user,
        mailboxName,
        false
      );

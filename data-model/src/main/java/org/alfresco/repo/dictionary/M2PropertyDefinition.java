...
/*package*/ class M2PropertyDefinition implements PropertyDefinition {
    ...
    /*package*/ M2PropertyDefinition(
        ...
        this.m2Property =
                createOverriddenProperty(propertyDef, override, prefixResolver, modelConstraints);
    }
    ...
    /*package*/ void resolveDependencies(
        ...
        if (propertyTypeName == null) {
            throw new DictionaryException(
                    "d_dictionary.property.err.property_type_not_specified", name.toPrefixString());
        }
        dataType = query.getDataType(propertyTypeName);
        if (dataType == null) {
            throw new DictionaryException(
                    "d_dictionary.property.err.property_type_not_found",
                    propertyTypeName.toPrefixString(),
                    name.toPrefixString());
        }
        ...
        constraintDefs =
                buildConstraints(
                        m2Property.getConstraints(), this, prefixResolver, modelConstraints);
    }
    ...
    private static List<ConstraintDefinition> buildConstraints(
        ...
        Map<QName, ConstraintDefinition> constraintsByQName =
                new HashMap<QName, ConstraintDefinition>(7);
        for (M2Constraint constraint : m2constraints) {
            // Consistent naming scheme for anonymous property constraints
            if (constraint.getName() == null) {
                StringBuilder builder = new StringBuilder();
                builder.append(m2PropertyDef.classDef.getModel().getName().getLocalName())
                        .append("_");
                ...
                QName newName =
                        QName.createQName(
                                m2PropertyDef.classDef.getModel().getName().getNamespaceURI(),
                                builder.toString());
                constraint.setName(newName.getPrefixedQName(prefixResolver).toPrefixString());
            }

            ConstraintDefinition def =
                    new M2ConstraintDefinition(m2PropertyDef, constraint, prefixResolver);
    ...
    private M2Property createOverriddenProperty(
        ...
        property.setDefaultValue(
                defaultValue == null ? propertyDef.getDefaultValue() : defaultValue);
        ...
        if (isOverrideMandatory != null && propertyDef.isMandatory()) {
            ...
            } else if ((isOverrideMandatoryEnforced != null)
                    && !isOverrideMandatoryEnforced
        ...
        property.setMandatory(
                isOverrideMandatory == null ? propertyDef.isMandatory() : isOverrideMandatory);
        property.setMandatoryEnforced(
                isOverrideMandatoryEnforced == null
                        ? propertyDef.isMandatoryEnforced()
                        : isOverrideMandatoryEnforced);
    ...
    public String getTitle(MessageLookup messageLookup) {
        String value =
                M2Label.getLabel(classDef.getModel(), messageLookup, "property", name, "title");
        if (value == null) {
    ...
    public String getTitle(MessageLookup messageLookup, Locale locale) {
        String value =
                M2Label.getLabel(
                        locale, classDef.getModel(), messageLookup, "property", name, "title");
        if (value == null) {
            value = m2Property.getTitle();
        }
        return value;
    ...
    public String getDescription(MessageLookup messageLookup) {
        String value =
                M2Label.getLabel(
                        classDef.getModel(), messageLookup, "property", name, "description");
        if (value == null) {
    ...
    /* package */ M2ModelDiff diffProperty(PropertyDefinition propDef) {
        ...
        if (this == propDef) {
            modelDiff =
                    new M2ModelDiff(name, M2ModelDiff.TYPE_PROPERTY, M2ModelDiff.DIFF_UNCHANGED);
            return modelDiff;
        }
        ...
        if (!EqualsHelper.nullSafeEquals(
                getDescription(null), propDef.getDescription(null), false)) {
            isUpdatedIncrementally = true;
        }
        ...
        if (!EqualsHelper.nullSafeEquals(
                getDataType().getName(), propDef.getDataType().getName())) {
            isUpdated = true;
        }

        // check container class qname
        if (!EqualsHelper.nullSafeEquals(
                getContainerClass().getName(), propDef.getContainerClass().getName())) {
            isUpdated = true;
        }
        ...
        if (!EqualsHelper.nullSafeEquals(
                getIndexTokenisationMode().toString(),
                propDef.getIndexTokenisationMode().toString(),
                false)) {
            isUpdated = true;
        }
        ...
        if (isUpdated) {
            ...
            modelDiff =
                    new M2ModelDiff(name, M2ModelDiff.TYPE_PROPERTY, M2ModelDiff.DIFF_UPDATED_INC);
        } else {
            modelDiff =
                    new M2ModelDiff(name, M2ModelDiff.TYPE_PROPERTY, M2ModelDiff.DIFF_UNCHANGED);
        }
    ...
    /*package*/ static Collection<M2ModelDiff> diffPropertyLists(
            Collection<PropertyDefinition> previousProperties,
            Collection<PropertyDefinition> newProperties) {
        ...
        for (PropertyDefinition previousProperty : previousProperties) {
            ...
            for (PropertyDefinition newProperty : newProperties) {
                if (newProperty.getName().equals(previousProperty.getName())) {
                    modelDiffs.add(
                            ((M2PropertyDefinition) previousProperty).diffProperty(newProperty));
                    found = true;
                    break;
                }
            }
            ...
            if (!found) {
                modelDiffs.add(
                        new M2ModelDiff(
                                previousProperty.getName(),
                                M2ModelDiff.TYPE_PROPERTY,

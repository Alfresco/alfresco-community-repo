...
public class PolicyTypeDefintionWrapper extends AbstractTypeDefinitionWrapper {
  ...
  protected static final Log logger = LogFactory.getLog(
    PolicyTypeDefintionWrapper.class
  );
  ...
  public PolicyTypeDefintionWrapper(
    ...
    if (BaseTypeId.CMIS_POLICY.value().equals(typeId)) {
      ...
      if (parentQName == null) {
        typeDef.setParentTypeId(
          cmisMapping.getCmisTypeId(CMISMapping.ASPECTS_QNAME)
        );
        ...
        throw new IllegalStateException(
          "The CMIS type model should ignore aspects that inherit from excluded aspects"
        );
      }
    }
    ...
    typeDef.setIsIncludedInSupertypeQuery(
      cmisClassDef.getIncludedInSuperTypeQuery()
    );
  ...
  public List<TypeDefinitionWrapper> connectParentAndSubTypes(
    ...
    if (typeDef.getParentTypeId() != null) {
      ...
      if (!isBaseType()) {
        throw new AlfrescoRuntimeException(
          "Type " + typeDef.getId() + " has no parent!"
        );
      }
    ...
    if (isBaseType()) {
      ...
      String aspectsTypeId = cmisMapping.getCmisTypeId(
      ...
      childrenNames =
        dictionaryService.getSubAspects(
          cmisMapping.getAlfrescoClass(getAlfrescoName()),
          false
        );
    }
  ...
  public void resolveInheritance(
    ...
    List<TypeDefinitionWrapper> children = registry.getChildren(
      typeDef.getId()
    );
    for (TypeDefinitionWrapper child : children) {
      if (child instanceof AbstractTypeDefinitionWrapper) {
        ((AbstractTypeDefinitionWrapper) child).resolveInheritance(
            cmisMapping,
            registry,
            dictionaryService
          );

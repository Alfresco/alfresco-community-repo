...
public class TransformerDebug {
  ...
  @Deprecated
  protected class UnavailableTransformer
    implements Comparable<UnavailableTransformer> {
  ...
  public void setTransformerDebugLog(Log transformerDebugLog) {
    logger =
      new LogTee(
        LogFactory.getLog(TransformerDebug.class),
        transformerDebugLog
      );
  }
  ...
  void push(
    ...
    if (
      callType == Call.TRANSFORM &&
      frame != null &&
      frame.callType == Call.AVAILABLE
    ) {
    ...
    boolean origDebugOutput = ThreadInfo.setDebugOutput(
      ThreadInfo.getDebugOutput() && sourceSize != 0
    );
    frame =
      new Frame(
        frame,
  ...
  private void logInfo(Frame frame, int size, String ms) {
    if (info.isDebugEnabled()) {
      ...
      boolean firstLevel = size == 1;
      String sourceAndTargetExt = getSourceAndTargetExt(
        frame.sourceMimetype,
        frame.targetMimetype
      );
      ...
      if (NO_TRANSFORMERS.equals(failureReason)) {
        ...
        debug =
          ...
          (
            size == 2 &&
            ThreadInfo.getStack().peekLast().callType != Call.TRANSFORM
          );
      }
  ...
  protected String getSourceAndTargetExt(
    String sourceMimetype,
    String targetMimetype
  ) {
    ...
    targetExt =
      AsynchronousExtractor.getExtension(targetMimetype, sourceExt, targetExt);

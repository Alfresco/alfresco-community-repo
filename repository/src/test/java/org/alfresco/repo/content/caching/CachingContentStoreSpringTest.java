...
/**
 * Tests for the CachingContentStore that benefit from a full set of tests defined in
...
public class CachingContentStoreSpringTest extends AbstractWritableContentStoreTest {
    ...
    public void testCacheOnInbound() {
        ...
        // Check DIRECTLY with the cache, since a getReader() from the CachingContentStore would
        // result
    ...
    public void testQuotaOnOverlimitCacheFile() {
        ...
        assertEquals("Content should be in backing store", content, fromBackingStore);
        assertEquals(
                "Content should be in backing store", overLimitSize, fromBackingStore.length());

        // cache writer should still return actual size
        // so that listeners could be executed correctly
        assertEquals(
                "Cache writer should still return actual size", overLimitSize, writer.getSize());
        assertEquals(
                "Cache writer should still return actual size",
                overLimitSize,
                writer.getContentData().getSize());
    }
    ...
    public void testBackingStoreBehavesLikeCenteraContentStore() {
        ...
        // create backing store that behaves like centera content store, i.e. contentUrl is known
        // only after content is written
        centeraLikeBackingStore =
                new TestCenteraLikeContentStore(
                        ctx,
        ...
        // for centera like backing content store all writers have the same content url before
        // content was written
        assertEquals(
                TestCenteraLikeContentWriter.UNKNOWN_ID, writer1.getContentData().getContentUrl());
        ...
        // content was written
        assertEquals(
                TestCenteraLikeContentWriter.UNKNOWN_ID, writer2.getContentData().getContentUrl());
        ...
        assertNotSame(
                writer1.getContentData().getContentUrl(), writer2.getContentData().getContentUrl());

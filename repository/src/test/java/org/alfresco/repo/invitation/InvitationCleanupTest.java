...
public class InvitationCleanupTest {
  ...
  public static TemporarySites TEST_SITES = new TemporarySites(
    APP_CONTEXT_INIT
  );
  ...
  public void pendingJoinRequestsToModeratedSitesShouldDisappearOnSiteDeletion()
    ...
    final String siteShortName =
      InvitationCleanupTest.class.getSimpleName() + "-test-site";
    ...
    AuthenticationUtil.runAs(
      new RunAsWork<Void>() {
        ...
        public Void doWork() throws Exception {
          ...
          NodeRef archivedNodeRef = TRANSACTION_HELPER.doInTransaction(
  ...
  private List<WorkflowTask> assertUserHasTasks(
    final String username,
    final int number
  ) {
    List<WorkflowTask> tasks = WORKFLOW_SERVICE.getAssignedTasks(
      username,
    ...
    assertEquals(
      "Wrong number of tasks assigned to user",
      number,
      allTasks.size()
    );
    return tasks;

...
/**
 * A generic utility for arranging a {@link List} of objects of type <code>R</code> into a <code>R
 * </code> objects page obtained from a {@link PagingResultsSource} considering a given {@link
 * PagingRequest} for the resulted {@link PagingResults}.
 *
 * @author Bogdan Horje
 * @param <R>
 */
public class PageCollator<R> {
     ...
     * <code>R</code> considering a given {@link PagingRequest}.
     *
     * @param <R>
     */
     ...
     * @param pagingRequest
     * @param comparator
     * @return a {@link PagingResults} R objects obtained from merging a collection of R objects
     *     with a paged result obtained from a {@link PagingResultsSource} considering the a merged
     *     result {@link PagingRequest}
     * @throws PageCollationException
     */
    public PagingResults<R> collate(
            List<R> objects,
            PagingResultsSource<R> objectPageSurce,
        ...
        try {
            PagingRequest preemptiveRequest =
                    new PagingRequest(
                            preemptiveSkip, preemptiveSize, pagingRequest.getQueryExecutionId());
        ...
        return collate(objects, pageResults, pageSkip, pagingRequest, comparator);
    }

    private PagingResults<R> collate(
            List<R> objects,
            final PagingResults<R> objectPageSurce,
            int pageSkip,
        ...
        final boolean endOfCollation =
                collate(objects, inPageList, pageSkip, pageSize, comparator, collatedPageList);
        ...
        final Pair<Integer, Integer> total =
                new Pair<>(
                        pageTotalFirst == null ? null : pageTotalFirst + resultsSize,

...
public abstract class AbstractReadOnlyContentStoreTest {
  ...
  private static Log logger = LogFactory.getLog(
    AbstractReadOnlyContentStoreTest.class
  );
   ...
   */
  private void checkIllegalReadContentUrl(
    ContentStore store,
    String contentUrl
  ) {
    assertFalse(
    ...
    try {
      ...
      fail(
        "Expected UnsupportedContentUrlException for getReader(), but got nothing: " +
        contentUrl
      );
    ...
    try {
      ...
      fail(
        "Expected UnsupportedContentUrlException for exists(), but got nothing: " +
        contentUrl
      );
    } catch (UnsupportedContentUrlException e) {
      // Expected
  ...
  public void testGetReaderForExistingContentUrl() throws Exception {
    ...
    if (contentUrl == null) {
      logger.warn(
        "Store test testGetReaderForExistingContentUrl not possible on " +
    ...
    // Get the reader
    assertTrue(
      "URL returned in set seems to no longer exist",
      store.exists(contentUrl)
    );
    ...
    assertFalse(
      "The reader channel should not be open yet",
    ...
    assertTrue(
      "The reader channel should be open during a read",
      reader.isChannelOpen()
    );
    ...
    assertFalse(
      "The reader channel should be closed after a read",
      reader.isChannelOpen()
    );
  }
  ...
  public void testRandomAccessRead() throws Exception {
    ...
    if (contentUrl == null) {
      logger.warn(
        "Store test testRandomAccessRead not possible on " +
        store.getClass().getName()
      );
      return;
    }
  ...
  public void testBlockedWriteOperations() throws Exception {
    ...
    if (contentUrl == null) {
      logger.warn(
        "Store test testBlockedWriteOperations not possible on " +
        store.getClass().getName()
      );
      return;
    }

...
public class XSLTProcessor extends BaseProcessor implements TemplateProcessor {
  ...
  private static final String MSG_UNABLE_TO_READ_TEMPLATE =
    "template.xslt.read_error";
  private static final String MSG_UNABLE_TO_PARSE_TEMPLATE =
    "template.xslt.parse_error";

  public static final QName ROOT_NAMESPACE = QName.createQName(
    null,
    "root_namespace"
  );
  ...
  public void process(String template, Object model, Writer out) {
    ...
    try {
      templateSource =
        (TemplateSource) templateLoader.findTemplateSource(template);
    } catch (IOException ex) {
      throw new TemplateException(
        MSG_UNABLE_TO_READ_TEMPLATE,
  ...
  private void process(
    ...
    if (
      (model == null) ||
      !XSLTemplateModel.class.isAssignableFrom(model.getClass())
    ) {
      throw new IllegalArgumentException(
        "\"model\" must be an XSLTemplateModel object: " + model
      );
    }
    ...
    try {
      ...
      if (errors.size() != 0) {
        final StringBuilder msg = new StringBuilder(
          "errors encountered creating tranformer ... \n"
        );
   ...
   *            the xsl template
   */
  protected List<String> addScripts(
    final XSLTemplateModel xsltModel,
    ...
    for (final Map.Entry<QName, Object> entry : xsltModel.entrySet()) {
      if (entry.getValue() instanceof TemplateProcessorMethod) {
        final String prefix = QName.splitPrefixedQName(
          entry.getKey().toPrefixString()
        )[0];
        final QName qn = QName.createQName(
    ...
    if (docEl.hasAttribute("exclude-result-prefixes")) {
      excludePrefixes.addAll(
        Arrays.asList(docEl.getAttribute("exclude-result-prefixes").split(" "))
      );
    }
    ...
    for (QName ns : methods.keySet()) {
      ...
      final Element compEl = xslTemplate.createElementNS(
        XALAN_NS,
      ...
      final Element scriptEl = xslTemplate.createElementNS(
        XALAN_NS,
        XALAN_NS_PREFIX + ":script"
      );
      scriptEl.setAttribute("lang", "javascript");
      final StringBuilder js = new StringBuilder(
        "var _xsltp_invoke = java.lang.Class.forName('" +
        XSLTProcessorMethodInvoker.class.getName() +
        "').newInstance();\n" +
      ...
      for (final Map.Entry<QName, Object> entry : methods.get(ns)) {
        ...
        final String id =
          entry.getKey().getLocalName() + entry.getValue().hashCode();
        js.append(
          "function " +
          entry.getKey().getLocalName() +
          "() { return _xsltp_invoke.invokeMethod('" +
          id +
          "', _xsltp_to_java_array(arguments)); }\n"
        );
        XSLTProcessorMethodInvoker.addMethod(
          id,
          (TemplateProcessorMethod) entry.getValue()
        );
        result.add(id);
      }
    ...
    docEl.setAttribute(
      ...
      StringUtils.join(
        excludePrefixes.toArray(new String[excludePrefixes.size()]),
        " "
      )
    );
    return result;
  ...
  protected void addParameters(
    ...
    for (Map.Entry<QName, Object> e : xsltModel.entrySet()) {
      ...
      final Element el = xslTemplate.createElementNS(
        XSL_NS,
        XSL_NS_PREFIX + ":variable"
      );
  ...
  protected Source getXMLSource(final Map<QName, Object> model) {
    ...
    if (!(o instanceof Document)) {
      throw new IllegalArgumentException(
        "expected root namespace object to be a  " +
  ...
  public void process(
    ...
    if (template.indexOf(StoreRef.URI_FILLER) != -1) {
      ...
      StringBuffer buf = new StringBuffer(
        template.length() + localeName.length()
      );
      buf.append(prefix);
      for (;;) {
        ...
        try {
          templateSource =
            (TemplateSource) templateLoader.findTemplateSource(path);
        } catch (IOException ex) {
          throw new TemplateException(
            MSG_UNABLE_TO_READ_TEMPLATE,
            new Object[] { ex.getMessage() },

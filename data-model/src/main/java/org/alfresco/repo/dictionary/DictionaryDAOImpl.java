...
public class DictionaryDAOImpl
  implements
    DictionaryDAO, NamespaceDAO, ApplicationListener<ApplicationEvent> {
  ...
  public void init() {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Triggered immediate reload of dictionary for tenant " + tenant
      );
    }
  }
  ...
  public ConstraintDefinition getConstraint(QName constraintQName) {
    ...
    if (constraintQName != null) {
      constraintDef =
        getTenantDictionaryRegistry().getConstraint(constraintQName);
    }
  ...
  @Override
  public TypeDefinition getAnonymousType(
    QName type,
    ...
    if (typeDef == null) {
      throw new DictionaryException(
        "d_dictionary.model.err.type_not_found",
        type
      );
    }
    ...
    if (aspects != null) {
      for (QName aspect : aspects) {
        ...
        if (aspectDef == null) {
          throw new DictionaryException(
            "d_dictionary.model.err.aspect_not_found",
            aspect
          );
        }
  ...
  @Override
  public Collection<PropertyDefinition> getPropertiesOfDataType(
    QName dataType
  ) {
   ...
   * @return model diffs (if any)
   */
  public List<M2ModelDiff> diffModel(
    M2Model model,
    boolean enableConstraintClassLoading
  ) {
    // Compile model definition
   ...
   * @return model diffs (if any)
   */
  /* package */List<M2ModelDiff> diffModel(
    CompiledModel previousVersion,
    CompiledModel model
  ) {
    ...
    if (previousVersion != null) {
      ...
      Collection<ConstraintDefinition> previousConDefs = getReferenceableConstraintDefs(
        previousVersion
      );
      Collection<NamespaceDefinition> previousImportedNamespaces = previousVersion
        .getModelDefinition()
      ...
      if (model == null) {
        ...
        Collection<ConstraintDefinition> conDefs = getReferenceableConstraintDefs(
          model
        );
        Collection<NamespaceDefinition> importedNamespaces = model
          .getModelDefinition()
          .getImportedNamespaces();
  ...
  public Collection<String> getURIs() {
    return Collections.unmodifiableCollection(
      getTenantDictionaryRegistry().getUrisCache()
    );
  }

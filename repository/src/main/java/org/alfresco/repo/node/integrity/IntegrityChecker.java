...
/**
 * Component that generates and processes integrity events, enforcing the dictionary model's node
 * structure.
 *
 * <p>In order to fulfill the contract of the interface, this class registers to receive
 * notifications pertinent to changes in the node structure. These are then store away in the
 * persistent store until the request to check integrity is made.
 *
 * <p>In order to ensure registration of these events, the {@link #init()} method must be called.
 *
 * <p>By default, this service is enabled, but can be disabled using {@link #setEnabled(boolean)}.
 * <br>
 ...
 * When enabled, the integrity check can either fail with a <tt>RuntimeException</tt> or not. In
 * either case, the integrity violations are logged as warnings or errors. This behaviour is
 * controleed using {@link #setFailOnViolation(boolean)} and is off by default. In other words, if
 * not set, this service will only log warnings about integrity violations.
 *
 * <p>Some integrity checks are not performed here as they are dealt with directly during the
 * modification operation in the {@link org.alfresco.service.cmr.repository.NodeService node
 * service}.
 *
...
public class IntegrityChecker
    ...
    /**
     * Downgrade violations to warnings within the current transaction. This is temporary and is
     * <u>dependent on there being a current transaction</u> active against the current thread. When
     * set, this will override the global {@link #setFailOnViolation(boolean) failure behaviour}.
     */
    ...
    /**
     * @return Returns true if the current transaction should only warn on violations. If <code>
     *     false</code>, the global setting will take effect.
     * @see #setWarnInTransaction()
     */
    public static boolean isWarnInTransaction() {
        Boolean warnInTransaction =
                (Boolean) TransactionSupportUtil.getResource(KEY_WARN_IN_TRANSACTION);
    ...
    /** @param traceOn set to <code>true</code> to enable stack traces recording of events */
    public void setTraceOn(boolean traceOn) {
        this.traceOn = traceOn;
    }

    /**
     * @param failOnViolation set to <code>true</code> to force failure by <tt>RuntimeException</tt>
    ...
    /**
     * @param maxLogNumberPerTransaction upper limit on how many violations are logged when multiple
    ...
    /**
     * @param storesToIgnore stores (eg. workspace://version2Store) which will be ignored by
     *     integrity checker. Note: assumes associations are within a store.
     */
    ...
    private void save(IntegrityEvent event) {
        ...
        Map<IntegrityEvent, IntegrityEvent> events =
                (Map<IntegrityEvent, IntegrityEvent>)
                        AlfrescoTransactionSupport.getResource(KEY_EVENT_SET);
        if (events == null) {
    ...
    public void onCreateNode(ChildAssociationRef childAssocRef) {
        ...
        if (!storesToIgnore.contains(
                tenantService.getBaseName(childRef.getStoreRef()).toString())) {
            ...
            if (nodeTypeDef == null) {
                throw new DictionaryException(
                        "The node type is not recognized: " + childNodeTypeQName);
            }
            ...
            for (AssociationDefinition assocDef : childAssocDefs.values()) {
                ...
                event =
                        new AssocTargetMultiplicityIntegrityEvent(
                                nodeService, dictionaryService, childRef, assocTypeQName, false);
                save(event);
            }
        }
    }
    ...
    public void onAddAspect(NodeRef nodeRef, QName aspectTypeQName) {
        if (!storesToIgnore.contains(tenantService.getBaseName(nodeRef.getStoreRef()).toString())) {
            ...
            if (aspectDef == null) {
                throw new DictionaryException(
                        "The aspect type is not recognized: " + aspectTypeQName);
            }
            ...
            for (AssociationDefinition assocDef : assocDefs.values()) {
                ...
                event =
                        new AssocTargetMultiplicityIntegrityEvent(
                                nodeService, dictionaryService, nodeRef, assocTypeQName, false);
                save(event);
            }
        }
    }
    ...
    public void onCreateChildAssociation(ChildAssociationRef childAssocRef, boolean isNew) {
        ...
        if (!storesToIgnore.contains(
                tenantService.getBaseName(childAssocRef.getChildRef().getStoreRef()).toString())) {
            IntegrityEvent event = null;
            // check source type
            event =
                    new AssocSourceTypeIntegrityEvent(
                            nodeService,
                            dictionaryService,
            ...
            // check target type
            event =
                    new AssocTargetTypeIntegrityEvent(
                            nodeService,
                            dictionaryService,
            ...
            event =
                    new AssocSourceMultiplicityIntegrityEvent(
                            nodeService,
                            dictionaryService,
                            childAssocRef.getChildRef(),
                            childAssocRef.getTypeQName(),
            ...
            // check target role
            event =
                    new AssocTargetRoleIntegrityEvent(
                            nodeService,
                            dictionaryService,
                            childAssocRef.getParentRef(),
                            childAssocRef.getTypeQName(),
    ...
    public void onCreateAssociation(AssociationRef nodeAssocRef) {
        if (!storesToIgnore.contains(
                tenantService.getBaseName(nodeAssocRef.getSourceRef().getStoreRef()).toString())) {
            IntegrityEvent event = null;
            // check source type
            event =
                    new AssocSourceTypeIntegrityEvent(
    ...
    /**
     * Runs several types of checks, querying specifically for events that will necessitate each
     * type of test.
     *
     * <p>The interface contracts also requires that all events for the transaction get cleaned up.
     */
     ...
     *
     * <p>The events are stored in a set, so there are no duplicates. Since each event performs a
     * particular type of check, this ensures that we don't duplicate checks.
     *
     * @return Returns a list of integrity violations, up to the {@link #maxErrorsPerTransaction the
     *     maximum defined}
     */
    @SuppressWarnings("unchecked")
    private List<IntegrityRecord> processAllEvents() {
        // the results
        ArrayList<IntegrityRecord> allIntegrityResults =
                new ArrayList<IntegrityRecord>(0); // generally empty

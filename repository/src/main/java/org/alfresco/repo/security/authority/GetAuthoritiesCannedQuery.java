...
public class GetAuthoritiesCannedQuery extends AbstractCannedQueryPermissions<AuthorityInfo> {
    ...
    protected List<AuthorityInfo> queryAndFilter(CannedQueryParameters parameters) {
        ...
        // Get parameters
        GetAuthoritiesCannedQueryParams params =
                (GetAuthoritiesCannedQueryParams) parameters.getParameterBean();
    ...
    private class AuthComparator implements Comparator<AuthorityInfo> {
        ...
        public AuthComparator(String fieldName, SortOrder sortOrder) {
            if (DISPLAY_NAME.equals(fieldName)) {
                ...
                throw new IllegalArgumentException(
                        "Authorities should be sorted by "
                                + DISPLAY_NAME
                                + ", "
                                + AUTHORITY_NAME
                                + " or "
        ...
        public int compare(AuthorityInfo auth1In, AuthorityInfo auth2In) {
            ...
            return collator.compare(
                    (String) s1, (String) s2); // TODO use collation keys (re: performance)
        }
    }
    ...
    protected List<AuthorityInfo> applyPostQuerySorting(
            List<AuthorityInfo> results, CannedQuerySortDetails sortDetails) {
        ...
        if (sortPairs.size() > 0) {
            ...
            Collections.sort(
                    results,
                    new AuthComparator(
                            (String) sortPairs.get(0).getFirst(), sortPairs.get(0).getSecond()));
        }

...
public class WorkflowRestImpl {
   ...
   */
  public CollectionWithPagingInfo<Item> getItemsFromProcess(
    String processId,
    Paging paging
  ) {
    ActivitiScriptNode packageScriptNode = null;
    try {
      HistoricVariableInstance variableInstance = activitiProcessEngine
        .getHistoryService()
  ...
  public Item createItemInProcess(String itemId, String processId) {
    ...
    try {
      packageScriptNode =
        (ActivitiScriptNode) activitiProcessEngine
          .getRuntimeService()
    ...
    if (packageScriptNode == null) {
      throw new InvalidArgumentException(
        "process doesn't contain a workflow package variable"
      );
    }
    ...
    try {
      ...
      throw new EntityNotFoundException(
        "item with id " + nodeRef.toString() + " not found"
      );
    }

    try {
      QName workflowPackageItemId = QName.createQName(
        "wpi",
        nodeRef.toString()
      );
      ...
      throw new ApiException(
        "could not add item to process " + e.getMessage(),
        e
      );
    }

    Item responseItem = createItemForNodeRef(nodeRef);
    activitiWorkflowEngine.dispatchPackageUpdatedEvent(
      packageScriptNode,
      null,
      null,
      processId,
  ...
  public void deleteItemFromProcess(String itemId, String processId) {
    ...
    if (itemIdFoundInPackage == false) {
      throw new EntityNotFoundException(
        "Item " + itemId + " not found in the process package variable"
      );
    }
   ...
   */
  public CollectionWithPagingInfo<FormModelElement> getFormModelElements(
    TypeDefinition type,
    Paging paging
  ) {
    ...
    for (Entry<QName, PropertyDefinition> entry : taskProperties.entrySet()) {
      ...
      if (
        !typesToExclude.contains(
          entry.getValue().getContainerClass().getName()
        ) &&
        ...
        if (entry.getValue().getConstraints() != null) {
          for (ConstraintDefinition constraintDef : entry
            .getValue()
            .getConstraints()) {
            Constraint constraint = constraintDef.getConstraint();
            if (
              constraint != null && constraint instanceof ListOfValuesConstraint
            ) {
              ListOfValuesConstraint valuesConstraint = (ListOfValuesConstraint) constraint;
              if (
                valuesConstraint.getAllowedValues() != null &&
    ...
    for (Entry<QName, AssociationDefinition> entry : taskAssociations.entrySet()) {
      ...
      if (
        !typesToExclude.contains(entry.getValue().getSourceClass().getName())
      ) {
        FormModelElement element = new FormModelElement();
        element.setName(
          entry.getKey().toPrefixString(namespaceService).replace(':', '_')
        );
        element.setQualifiedName(entry.getKey().toString());
        element.setTitle(entry.getValue().getTitle(dictionaryService));
  ...
  protected Set<QName> getTypesToExclude(TypeDefinition taskType) {
    ...
    while (parentClassDefinition != null) {
      if (contentClassFound) {
        ...
        ContentModel.TYPE_CONTENT.equals(parentClassDefinition.getName())
      ) {
   ...
   */
  protected List<HistoricVariableInstance> validateIfUserAllowedToWorkWithProcess(
    String processId
  ) {
    List<HistoricVariableInstance> variableInstances = activitiProcessEngine
      .getHistoryService()
      .createHistoricVariableInstanceQuery()
      .processInstanceId(processId)
      .list();
    ...
    if (tenantService.isEnabled()) {
      String tenantDomain = (String) variableMap.get(
        ActivitiConstants.VAR_TENANT_DOMAIN
      );
      if (TenantUtil.getCurrentDomain().equals(tenantDomain) == false) {
        throw new PermissionDeniedException(
          "Process is running in another tenant"
        );
      }
    }
    ...
    String nodeId =
      (
        (initiator instanceof ActivitiScriptNode)
          ? ((ActivitiScriptNode) initiator).getNodeRef().getId()
          : ((NodeRef) initiator).getId()
      );
    ...
    if (authorityService.isAdminAuthority(username)) {
      ...
      ActivitiScriptNode group = (ActivitiScriptNode) variableMap.get(
        "bpm_groupAssignee"
      );
      if (group != null) {
        ...
        if (
          ...
          isUserInGroup(username, group.getNodeRef())
        ) {
          return variableInstances;
        }
      }
  ...
  public void setActivitiWorkflowEngine(
    ActivitiWorkflowEngine activitiWorkflowEngine
  ) {
  ...
  private boolean isUserInGroup(String username, NodeRef group) {
    ...
    Set<String> groupMembers = authorityService.getContainedAuthorities(
      AuthorityType.USER,
      groupName,
      false
    );

...
public class FileFolderDuplicateChildTest extends TestCase {
  ...
  public void testDuplicateChildNameDetection() throws Exception {
    ...
    FileInfo fileInfo = retryingTransactionHelper.doInTransaction(
      callback,
      false,
      true
    );
  ...
  private class CreateFileCallback
    implements RetryingTransactionCallback<FileInfo> {
  ...
  private class WorkerThread extends Thread {
    ...
    public void run() {
      ...
      int number = 0;
      while (true) {
        RetryingTransactionCallback<FileInfo> callback = new CreateFileCallback(
          number
        );
        try {
          System.out.println(

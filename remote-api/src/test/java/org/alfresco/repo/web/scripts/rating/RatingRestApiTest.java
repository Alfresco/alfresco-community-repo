...
public class RatingRestApiTest extends BaseWebScriptTest {
  ...
  private static final String GET_RATING_DEFS_URL =
    "/api/rating/schemedefinitions";
  ...
  public void testGetRatingSchemeDefinitions() throws Exception {
    ...
    Response rsp = sendRequest(
      new GetRequest(GET_RATING_DEFS_URL),
      expectedStatus
    );
    ...
    assertNotNull(
      "JSON 'ratingSchemesArray' object was null",
      ratingSchemesArray
    );
    ...
    for (int i = 0; i < ratingSchemesArray.length(); i++) {
      ratingsMap.put(
        ratingSchemesArray.getJSONObject(i).getString(NAME),
        ratingSchemesArray.getJSONObject(i)
      );
    }
    ...
    assertNotNull(
      "The response did not contain " + LIKES_RATING_SCHEME,
      scheme1
    );
    ...
    assertNotNull(
      "The response did not contain " + FIVE_STAR_RATING_SCHEME,
      scheme2
    );
  ...
  public void testGetRatingsFromUnratedNodeRef() throws Exception {
    ...
    assertEquals(
      "Average rating was wrong.",
      -1.0,
      likesStats.getDouble(AVERAGE_RATING)
    );
    assertEquals(
      "Ratings count rating was wrong.",
      0,
      likesStats.getInt(RATINGS_COUNT)
    );
    assertEquals(
      "Ratings total was wrong.",
      0.0,
      likesStats.getDouble(RATINGS_TOTAL)
    );

    JSONObject fiveStarStats = statsObject.getJSONObject(
    ...
    assertEquals(
      "Ratings count rating was wrong.",
      0,
      fiveStarStats.getInt(RATINGS_COUNT)
    );
    assertEquals(
      "Ratings total was wrong.",
      0.0,
      fiveStarStats.getDouble(RATINGS_TOTAL)
    );
  }
  ...
  public void testApplyRatingsAsMultipleUsersAndRetrieve() throws Exception {
    ...
    String jsonString = new JSONStringer()
      .object()
      .key("rating")
      .value(userOneRatingValue)
      .key("ratingScheme")
      .value(FIVE_STAR_RATING_SCHEME)
      .endObject()
      .toString();

    Response postRsp = sendRequest(
      new PostRequest(testNodeRatingUrl, jsonString, APPLICATION_JSON),
      200
    );
    ...
    assertEquals(
      userOneRatingValue,
      (float) dataObj.getDouble("averageRating")
    );
    ...
    JSONObject recoveredRating = ratingsObject.getJSONObject(
      FIVE_STAR_RATING_SCHEME
    );
    assertEquals(
      userOneRatingValue,
      (float) recoveredRating.getDouble("rating")
    );
    ...
    assertEquals(
      "Average rating was wrong.",
      1f,
      (float) likesStats.getDouble(AVERAGE_RATING)
    );
    assertEquals(
      "Ratings count rating was wrong.",
      1,
    ...
    assertEquals(
      "Ratings total was wrong.",
      1f,
      (float) likesStats.getDouble(RATINGS_TOTAL)
    );
    ...
    assertEquals(
      (userOneRatingValue + userTwoRatingValue) / 2,
      (float) dataObj.getDouble("averageRating")
    );
    assertEquals(
      userOneRatingValue + userTwoRatingValue,
      (float) dataObj.getDouble("ratingsTotal")
    );
    ...
    JSONObject secondRating = (JSONObject) userTwoRatings.getJSONObject(
      FIVE_STAR_RATING_SCHEME
    );
    ...
    assertEquals(
      "Average rating was wrong.",
      (userOneRatingValue + userTwoRatingValue) / 2.0,
      fiveStarStats.getDouble(AVERAGE_RATING)
    );
    assertEquals(
      "Ratings count rating was wrong.",
    ...
    AuthenticationUtil.setFullyAuthenticatedUser(USER_TWO);
    sendRequest(
      new DeleteRequest(testNodeRatingUrl + "/" + FIVE_STAR_RATING_SCHEME),
      200
    );

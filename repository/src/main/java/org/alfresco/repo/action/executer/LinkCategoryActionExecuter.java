...
public class LinkCategoryActionExecuter extends ActionExecuterAbstractBase {
  ...
  protected void addParameterDefinitions(List<ParameterDefinition> paramList) {
    paramList.add(
      new ParameterDefinitionImpl(
        ...
        false,
        getParamDisplayLabel(PARAM_CATEGORY_ASPECT)
      )
    );
    paramList.add(
      new ParameterDefinitionImpl(
        ...
        true,
        getParamDisplayLabel(PARAM_CATEGORY_VALUE)
      )
    );
  }
  ...
  protected void executeImpl(Action ruleAction, NodeRef actionedUponNodeRef) {
    ...
    if (this.nodeService.exists(actionedUponNodeRef) == true) {
      ...
      QName categoryAspect = (QName) ruleAction.getParameterValue(
        PARAM_CATEGORY_ASPECT
      );
      ...
      NodeRef categoryValue = (NodeRef) ruleAction.getParameterValue(
        PARAM_CATEGORY_VALUE
      );
      ...
      if (
        ...
        Map<QName, PropertyDefinition> propertyDefs =
          this.dictionaryService.getAspect(categoryAspect).getProperties();
        for (Map.Entry<QName, PropertyDefinition> entry : propertyDefs.entrySet()) {
          if (
            DataTypeDefinition.CATEGORY.equals(
        ...
        if (categoryProperty == null) {
          throw new AlfrescoRuntimeException(
            "The category aspect " +
            categoryAspect.toPrefixString() +
            " does not have a category property to set."
          );
        }

        if (categoryAspect != null) {
          if (
            this.nodeService.hasAspect(actionedUponNodeRef, categoryAspect) ==
            false
          ) {

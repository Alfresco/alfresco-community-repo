...
public class ContentDataDAOTest extends TestCase {
  ...
  private Pair<Long, ContentData> create(final ContentData contentData) {
    RetryingTransactionCallback<Pair<Long, ContentData>> callback = new RetryingTransactionCallback<Pair<Long, ContentData>>() {
      public Pair<Long, ContentData> execute() throws Throwable {
        Pair<Long, ContentData> contentDataPair = contentDataDAO.createContentData(
          contentData
        );
  ...
  private Pair<Long, ContentData> update(
    final Long id,
  ...
  private Pair<Long, ContentData> getAndCheck(
    final Long contentDataId,
    ContentData checkContentData
  ) {
    RetryingTransactionCallback<Pair<Long, ContentData>> callback = new RetryingTransactionCallback<Pair<Long, ContentData>>() {
      public Pair<Long, ContentData> execute() throws Throwable {
        Pair<Long, ContentData> contentDataPair = contentDataDAO.getContentData(
          contentDataId
        );
        return contentDataPair;
      }
    };
    Pair<Long, ContentData> resultPair = txnHelper.doInTransaction(
      callback,
      true,
      false
    );
  ...
  public void testGetWithInvalidId() {
    try {
      ...
      fail(
        "Invalid ContentData IDs must generate DataIntegrityViolationException."
      );
  ...
  public void testEnsureCaseInsensitiveMimetypeStorage() throws Exception {
    ContentData contentData = getContentData();
    ContentData cdWithUpperMimetype = ContentData.setMimetype(
      contentData,
      "TEXT/MYFORMAT"
    );
    ContentData cdWithLowerMimetype = ContentData.setMimetype(
      contentData,
      "text/myformat"
    );
  ...
  public void testContentUrlCrud() throws Exception {
    assertNull(
      "Expect null return fetching a URL by ID",
      contentDataDAO.getContentUrl(0L)
    );
    assertNull(
    ...
    // Update and create
    ContentUrlEntity contentUrlEntity = contentDataDAO.getOrCreateContentUrl(
      "store://" + GUID.generate()
    );
    // Check that it exists, now
    contentUrlEntity =
      contentDataDAO.getContentUrl(contentUrlEntity.getContentUrl());
    ...
    assertEquals(
      "The content URL does not match.",
      url,
  ...
  public void testContentUrl_FetchingOrphansNoLimit() throws Exception {
    ...
    contentDataDAO.getContentUrlsOrphaned(
      handler,
      Long.MAX_VALUE,
      Integer.MAX_VALUE
    );
  ...
  private List<Pair<Long, ContentData>> speedTestWrite(String name, int total) {
    ...
    List<Pair<Long, ContentData>> pairs = new ArrayList<Pair<Long, ContentData>>(
      100000
    );
  ...
  public void testCreateSpeedSingleTxn() {
    ...
    final List<Pair<Long, ContentData>> pairs = txnHelper.doInTransaction(
      writeCallback,
      false,
      false
    );

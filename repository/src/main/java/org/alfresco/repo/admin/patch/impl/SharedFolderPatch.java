...
public class SharedFolderPatch extends GenericBootstrapPatch {
  ...
  public void executeAsync() {
    ...
    String lockToken = jobLockService.getLock(
      lockQName,
      LOCK_TIME_TO_LIVE,
      0,
      1
  ...
  protected String applyInternal() throws Exception {
    ...
    if (getRenamePath() != null) {
      ...
      if (results.size() > 1) {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "There is an existing node in the way path:" + getRenamePath()
          );
        }
        ...
        LinkedList<String> folderElements = new LinkedList<String>(
          Arrays.asList(getRenamePath().split("/"))
        );
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Node does not already exist, Running the Generic Bootstrap Patch"
      );
    }

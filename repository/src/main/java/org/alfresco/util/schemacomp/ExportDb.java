...
public class ExportDb {
  ...
  private void init() {
    try {
      ...
      throw new RuntimeException(
        "Unable to generate type map using hibernate.",
        error
      );
  ...
  private void attemptInit()
    ...
    final Map<Integer, String> forwardMap2 = (Map<Integer, String>) defaultsField.get(
      typeNames
    );
    ...
    final Map<Integer, Map<Integer, String>> forwardMap1 = (Map<Integer, Map<Integer, String>>) weightedField.get(
      typeNames
    );
  ...
  private void extractSchema(
    ...
    if (sequencesRefResource != null && sequencesRefResource.exists()) {
      ...
      retrieveAndProcessSequences(
        dbmd,
        sequencesRefResource,
        schemaName,
        prefixFilter
      );
    } else {
    ...
    final ResultSet tables = dbmd.getTables(
      null,
      schemaName,
      prefixFilter,
      tableTypes
  ...
  private void processTables(DatabaseMetaData dbmd, ResultSet tables)
    ...
    while (tables.next()) {
      ...
      // Table columns
      final ResultSet columns = dbmd.getColumns(
        null,
      ...
      final ResultSet primarykeycols = dbmd.getPrimaryKeys(
        null,
        tables.getString("TABLE_SCHEM"),
        tableName
      );
      ...
      List<Integer> keySeqSortedColumnOrders = keySeqsAndColumnNames
        .keySet()
        .stream()
        .sorted()
      ...
      // Indexes
      final ResultSet indexes = dbmd.getIndexInfo(
        null,
        tables.getString("TABLE_SCHEM"),
        tableName,
        false,
      ...
      final ResultSet foreignkeys = dbmd.getImportedKeys(
        null,
        tables.getString("TABLE_SCHEM"),
        tableName
      );
  ...
  protected String generateType(
    ...
    if (this.reverseTypeMap.containsKey(precisionScaleType)) {
      ...
      for (String key : reverseTypeMap.keySet()) {
        ...
        if (
          popKey.equals(precisionScaleType) ||
  ...
  private String[] namePrefixFilters(DatabaseMetaData dbmd)
    throws SQLException {
  ...
  private Resource getSequencesReferenceResource() {
    return DialectUtil.getDialectResource(
      rpr,
      dialect.getClass(),
      SCHEMA_REFERENCE_SEQUENCES_SQL_FILE
    );
  }
  ...
  private void retrieveAndProcessSequences(
    ...
    throws SQLException, IllegalArgumentException, IllegalAccessException, IOException {
    final String script = DBScriptUtil.readScript(

...
public class CommentsImpl implements Comments {
    private static final List<String> INCLUDE_FULL_PERSON =
            Arrays.asList(PARAM_INCLUDE_ASPECTNAMES, PARAM_INCLUDE_PROPERTIES);
    private Nodes nodes;
    ...
    private Comment toComment(NodeRef nodeRef, NodeRef commentNodeRef, List<String> include) {
        ...
        try {
            createdBy =
                    people.getPerson((String) nodeProps.get(ContentModel.PROP_CREATOR), include);
        ...
        try {
            modifiedBy =
                    people.getPerson((String) nodeProps.get(ContentModel.PROP_MODIFIER), include);
        } catch (EntityNotFoundException enfe) {
    ...
    public CollectionWithPagingInfo<Comment> getComments(
            String nodeId, Paging paging, List<String> include) {
        ...
        final PagingResults<NodeRef> pagingResults =
                commentService.listComments(nodeRef, pagingRequest);

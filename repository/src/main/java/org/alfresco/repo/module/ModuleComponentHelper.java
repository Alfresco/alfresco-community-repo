...
/**
 * Helper class to split up some of the code for managing module components. This class handles the
 * execution of the module components.
 *
 * @author Derek Hulley
 */
public class ModuleComponentHelper {
    ...
    private static final String ERR_UNSUPPORTED_REPO_VERSION =
            "module.err.unsupported_repo_version";
    private static final String ERR_NO_DOWNGRADE = "module.err.downgrading_not_supported";
    private static final String ERR_COMPONENT_ALREADY_REGISTERED =
            "module.err.component_already_registered";
    private static final String ERR_COMPONENT_IN_MISSING_MODULE =
            "module.err.component_in_missing_module";
    ...
    /**
     * @throws UnsupportedOperationException This feature was never active and cannot be used
     *     (ALF-19207)
     */
    public void setApplyToTenants(boolean applyToTenants) {
        throw new UnsupportedOperationException(
                "Applying modules to individual tenants is unsupported. See ALF-19207: MT module"
                        + " startup does not work");
    }

    /**
     * Add a managed module component to the registry of components. These will be controlled by the
     * {@link #startModules()} method.
     *
    ...
    /**
     * @return Returns the map of components keyed by name. The map could be empty but will never be
     *     <tt>null</tt>.
     */
                ...
                new RunAsWork<Object>() {
                    public Object doWork() throws Exception {
                        try {
                            ...
                            // Note: for system bootstrap this will be the default domain, else
                            // tenant domain for tenant create/import
                            final String tenantDomainCtx =
                                    tenantAdminService.getCurrentUserDomain();

                            if (tenantAdminService.isEnabled()
                                    && (!tenantDomainCtx.equals(TenantService.DEFAULT_DOMAIN))
                                    && (!applyToTenants)) {
                                // nothing to start (eg. when creating/importing tenant and
                                // applyToTenants = false)
                                return null;
                            }
                            ...
                            loggerService.info(
                                    I18NUtil.getMessage(MSG_FOUND_MODULES, modules.size()));

                            // Process each module in turn.  Ordering is not important.
                            final Map<String, Set<ModuleComponent>> mapExecutedComponents =
                                    new HashMap<String, Set<ModuleComponent>>(1);
                            final Map<String, Set<String>> mapStartedModules =
                                    new HashMap<String, Set<String>>(1);

                            mapExecutedComponents.put(
                                    tenantDomainCtx, new HashSet<ModuleComponent>(10));
                            mapStartedModules.put(tenantDomainCtx, new HashSet<String>(2));

                            List<Tenant> tenantsNonFinal = null;
                            if (tenantAdminService.isEnabled()) {
                                if (tenantDomainCtx.equals(TenantService.DEFAULT_DOMAIN)
                                        && applyToTenants) {
                                    tenantsNonFinal = tenantAdminService.getTenants(false);
                                    for (Tenant tenant : tenantsNonFinal) {
                                        mapExecutedComponents.put(
                                                tenant.getTenantDomain(),
                                                new HashSet<ModuleComponent>(10));
                                        mapStartedModules.put(
                                                tenant.getTenantDomain(), new HashSet<String>(2));
                                    }
                                }
                            }
                            ...
                            for (final ModuleDetails module : modules) {
                                RetryingTransactionCallback<Object> startModuleWork =
                                        new RetryingTransactionCallback<Object>() {
                                            public Object execute() throws Exception {
                                                startModule(
                                                        module,
                                                        mapStartedModules.get(tenantDomainCtx),
                                                        mapExecutedComponents.get(tenantDomainCtx));
                                ...
                                transactionService
                                        ...
                                        .doInTransaction(
                                                startModuleWork, transactionService.isReadOnly());
                            }
                            ...
                            if (tenants != null) {
                                for (Tenant tenant : tenants) {
                                    ...
                                    TenantUtil.runAsSystemTenant(
                                            new TenantRunAsWork<Object>() {
                                                public Object doWork() throws Exception {
                                                    checkForOrphanComponents(
                                                            mapExecutedComponents.get(
                                                                    tenantDomain));
                                                    return null;
                                                }
                                            },
    ...
    private void checkForOrphanComponents(Set<ModuleComponent> executedComponents) {
        ...
        for (Map.Entry<String, Map<String, ModuleComponent>> entry :
                componentsByNameByModule.entrySet()) {
            ...
            for (Map.Entry<String, ModuleComponent> entryInner : componentsByName.entrySet()) {
                ...
                if (executedComponents.contains(component)) {
                    ...
                    String msg =
                            I18NUtil.getMessage(
                                    ERR_COMPONENT_IN_MISSING_MODULE, componentName, moduleId);
                    logger.error(msg);
                }
            }
        }
    ...
    Collection<String> getRegistryModuleIDs() {
        ...
        RegistryKey moduleKeyAllIds =
                new RegistryKey(ModuleComponentHelper.URI_MODULES_1_0, REGISTRY_PATH_MODULES, null);
    ...
    ModuleVersionNumber getVersion(String moduleId) {
        RegistryKey moduleKeyCurrentVersion =
                new RegistryKey(
                        ModuleComponentHelper.URI_MODULES_1_0,
                        REGISTRY_PATH_MODULES,
                        moduleId,
                        REGISTRY_PROPERTY_CURRENT_VERSION);
    ...
    /**
     * Checks to see if there are any modules registered as installed that aren't in the list of
     * modules taken from the WAR.
     *
     * <p>Currently, the behaviour specified is that a warning is generated only.
     */
    ...
    /**
     * Copies, where necessary, the module registry details from the alias details and removes the
     * alias details.
     */
    private void renameModule(ModuleDetails module) {
        ...
        for (String moduleAlias : moduleAliases) {
            ...
            RegistryKey moduleKeyNew =
                    new RegistryKey(
                            ModuleComponentHelper.URI_MODULES_1_0,
                            REGISTRY_PATH_MODULES,
                            moduleId,
                            null);
            RegistryKey moduleKeyOld =
                    new RegistryKey(
                            ModuleComponentHelper.URI_MODULES_1_0,
                            REGISTRY_PATH_MODULES,
                            moduleAlias,
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Moved old module alias to new module ID: \n"
                                + "   Alias:  "
                                + moduleAlias
                                + "\n"
                                + "   Module: "
    ...
    /**
     * Does the actual work without fussing about transactions and authentication. Module
     * dependencies will be started first, but a module will only be started once.
     *
    ...
    private void startModule(
        ...
        for (ModuleDependency moduleDependency : moduleDependencies) {
            ...
            if (moduleDependencyDetails == null) {
                ...
                String msg =
                        I18NUtil.getMessage(
                                MSG_DEPENDENCIES, moduleId, moduleNewVersion, sb.toString());
                logger.info(msg);
                // Now fail
                throw AlfrescoRuntimeException.create(
                        ERR_MISSING_DEPENDENCY, moduleId, moduleNewVersion, moduleDependency);
            }
        ...
        VersionNumber repoVersionNumber =
                descriptorService.getServerDescriptor().getVersionNumber();
        ...
        if ((minRepoVersionNumber != null && repoVersionNumber.compareTo(minRepoVersionNumber) < 0)
                || (maxRepoVersionNumber != null
                        && repoVersionNumber.compareTo(maxRepoVersionNumber) > 0)) {
            ...
            throw AlfrescoRuntimeException.create(
                    ...
                    moduleNewVersion,
                    repoVersionNumber,
                    minRepoVersionNumber,
                    maxRepoVersionNumber);
        }

        // Get the module details from the registry
        RegistryKey moduleKeyInstalledVersion =
                new RegistryKey(
                        ModuleComponentHelper.URI_MODULES_1_0,
                        REGISTRY_PATH_MODULES,
                        moduleId,
                        REGISTRY_PROPERTY_INSTALLED_VERSION);
        RegistryKey moduleKeyCurrentVersion =
                new RegistryKey(
                        ModuleComponentHelper.URI_MODULES_1_0,
                        REGISTRY_PATH_MODULES,
        ...
        } else // It is an upgrade or is the same
        {
            ...
            if (moduleInstallVersion == null) {
                ...
                logger.warn(
                        I18NUtil.getMessage(
                                WARN_NO_INSTALL_VERSION, moduleId, moduleCurrentVersion));
                // Record the install version
                registryService.addProperty(moduleKeyInstalledVersion, currentModuleVersion);
                moduleInstallVersion = moduleCurrentVersion;
            }

            if (currentModuleVersion.compareTo(moduleNewVersion)
                    == 0) // The current version is the same
            {
                msg = I18NUtil.getMessage(MSG_STARTING, moduleId, moduleNewVersion);
            } else if (currentModuleVersion.compareTo(moduleNewVersion)
                    > 0) // Downgrading not supported
            {
                throw AlfrescoRuntimeException.create(
                        ERR_NO_DOWNGRADE, moduleId, moduleCurrentVersion, moduleNewVersion);
            } else // This is an upgrade
            {
                msg =
                        I18NUtil.getMessage(
                                MSG_UPGRADING, moduleId, moduleNewVersion, moduleCurrentVersion);
            }
        }
    ...
    private void executeComponent(
        ...
        if (executedComponents.contains(component)) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Skipping component already executed in this run: \n"
                                + "   Component:      "
                                + component);
            }
            return;
        ...
        String name = component.getName();
        RegistryKey executionDateKey =
                new RegistryKey(
                        ModuleComponentHelper.URI_MODULES_1_0,
                        REGISTRY_PATH_MODULES,
                        moduleId,
                        REGISTRY_PATH_COMPONENTS,
                        name,
        ...
        if (executionDate != null && component.isExecuteOnceOnly()) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Skipping already-executed module component: \n"
                                + "   Component:      "
                                + component
                                + "\n"
                                + "   Execution Time: "

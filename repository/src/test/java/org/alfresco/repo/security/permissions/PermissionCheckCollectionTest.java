    ...
    public void testBasicWrapping() throws Exception {
        ...
        Collection<Integer> proxiedList =
                PermissionCheckCollectionMixin.create(
                        list, targetResultCount, cutOffAfterTimeMs, cutOffAfterCount);
        ...
        assertTrue(
                "Proxied object must also be a PermissionCheckCollection",
                proxiedList instanceof PermissionCheckCollection);
        @SuppressWarnings("unchecked")
        PermissionCheckCollection<String> proxiedPermissionCheckCollection =
                (PermissionCheckCollection<String>) proxiedList;
        assertEquals(
                "targetResultCount value incorrect",
                targetResultCount,
                proxiedPermissionCheckCollection.getTargetResultCount());
        assertEquals(
                "cutOffAfterTimeMs value incorrect",
                cutOffAfterTimeMs,
                proxiedPermissionCheckCollection.getCutOffAfterTimeMs());
        assertEquals(
                "cutOffAfterCount value incorrect",
                cutOffAfterCount,
                proxiedPermissionCheckCollection.getCutOffAfterCount());
    }

...
public class ConnectionPoolOverloadTest {
  ...
  private static Log logger = LogFactory.getLog(
    ConnectionPoolOverloadTest.class
  );
  ...
  public void setUp() throws Exception {
    ...
    transactionService =
      ctx.getBean("transactionComponent", TransactionService.class);
    ...
    if (PropertyCheck.isValidPropertyString(dbPoolMaxProp)) {
      ...
      throw new IllegalArgumentException(
        "The db.pool.max property is not valid."
      );
    }
    ...
    if (PropertyCheck.isValidPropertyString(dbPoolWaitMaxProp)) {
      ...
      throw new IllegalArgumentException(
        "The db.pool.wait.max property is not valid."
      );
    }
  ...
  public void testOverload() throws Exception {
    ...
    try {
      ...
      assertTrue(
        "The number of failed threads should not be 0.",
        failCount.intValue() > 0
      );
      assertTrue(
        "The number of open transactions should not be more that the db pool maximum." +
        "(Maybe a configuration of DB connection limit is less then db.pool.max)" +
        " db.pool.max is " +
        dbPoolMax +
        ", number of threads is " +
        numThreads +
        ", number of failed threads is" +
        failCount.intValue(),
  ...
  private class TxnThread extends Thread {
    ...
    public void run() {
      ...
      try {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "The " + this.getName() + " failed with ConnectionPoolException."
          );
        }
        failCount.increment();
        interrupt();
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
            "The " + this.getName() + " failed with not expected exception."
          );
        }
        e.printStackTrace();
        failCount.increment();

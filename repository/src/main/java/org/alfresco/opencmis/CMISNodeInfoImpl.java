 ...
 *
 * <p>Tries to avoid getting the node's version history where possible (because it's not very
 * performant).
 */
public class CMISNodeInfoImpl implements CMISNodeInfo {
    ...
    protected void analyseVersionNode() {
        ...
        if (versionHistory == null) {
            ...
            objectId =
                    connector.constructObjectId(nodeRef, CMISConnector.UNVERSIONED_VERSION_LABEL);
            ...
            currentObjectId =
                    connector.constructObjectId(
                            headVersion.getVersionedNodeRef(), headVersion.getVersionLabel());
            ...
            hasPWC =
                    connector
                            ...
                            .isCheckedOut(headVersion.getVersionedNodeRef());
        }
    }
    ...
    protected void analyseObjectId() {
        ...
        try {
            ...
            if (objectId.startsWith(CMISConnector.ASSOC_ID_PREFIX)) {
                ...
                // nodeRef is a "live" node, the version label identifies the specific version of
                // the node
                ...
                if (isNodeWorkingCopy()) {
                    NodeRef checkedOut =
                            connector.getCheckOutCheckInService().getCheckedOut(nodeRef);
                ...
                if (versionLabel == null) {
                    if (isDocument()) {
                        // for a document, absence of a version label implies the current (head)
                        // version
                        if (connector.filter(nodeRef)) {
                            objecVariant = CMISObjectVariant.NOT_EXISTING;
                        } else {
                        ...
                        Version currentVersion =
                                connector.getVersionService().getCurrentVersion(nodeRef);
                        if (currentVersion != null) {
                            versionLabel = currentVersion.getVersionLabel();
                            versionHistory =
                                    connector.getVersionService().getVersionHistory(nodeRef);
                        } else {
                            versionLabel = CMISConnector.UNVERSIONED_VERSION_LABEL;
                        }
                ...
                if (!isNodeVersioned(nodeRef)) {
                    ...
                    // the node is versioned, determine whether the versionLabel refers to the head
                    // version or a
                ...
                hasPWC =
                        connector.getCheckOutCheckInService().isCheckedOut(getCurrentNodeNodeRef());
            }
    ...
    private void determineType(QName nodeType) {
        ...
        if ((objecVariant == CMISObjectVariant.INVALID_ID)
                || (objecVariant == CMISObjectVariant.NOT_A_CMIS_OBJECT)
                || (objecVariant == CMISObjectVariant.NOT_EXISTING)
                || (objecVariant == CMISObjectVariant.PERMISSION_DENIED)) {
            return;
        ...
        if (nodeRef != null) {
            QName typeQName =
                    (nodeType != null ? nodeType : connector.getNodeService().getType(nodeRef));
                    ...
                    if (versionHistory == null) {
                        ...
                        while (currentVersion != null) {
                            if (currentVersion.getVersionType() == VersionType.MAJOR) {
                                // ALF-11116: the current node (in the main store) and the frozen
                                // node (in the version store) are both represented as CMISNodeInfos
                                // but are indistinguishable apart from their storeRef (their
                                // objectVariant can be the same).
                                if (nodeRef.getStoreRef()
                                    ...
                                    isLatestMajorVersion =
                                            currentVersion.getFrozenStateNodeRef().equals(nodeRef);
                                }
                                break;
                            }
                ...
                while (i < path.size() - 1) {
                    ...
                    if (element instanceof ChildAssocElement) {
                        ...
                        try {
                            ...
                            // if the user does not have enough permissions to construct the entire
    ...
    private NodeRef getLatestNonMajorVersionNodeRef() {
        ...
        //        {
        //            return
        // connector.getCheckOutCheckInService().getWorkingCopy(getCurrentNodeNodeRef());
        //        } else
        //        {
    ...
    public List<CMISNodeInfo> getParents() {
        ...
        if (parents == null) {
            ...
            List<ChildAssociationRef> nodeParents =
                    connector
                            .getNodeService()
                            .getParentAssocs(
                                    nodeRefForParent,
            ...
            if (nodeParents != null) {
                for (ChildAssociationRef parent : nodeParents) {
                    if (connector.getType(parent.getParentRef())
                            instanceof FolderTypeDefintionWrapper) {
    ...
    public Set<QName> getNodeAspects() {
        ...
        // calls once again the aspects but only if a private copy is treated not ordinary versions
                ...
                && versionLabel.equals(CMISConnector.PWC_VERSION_LABEL)
                && nodeRef != null
                && !nodeRef.getId().equals(currentNodeId)) {
            nodeAspects = connector.getNodeService().getAspects(nodeRef);
        }
        return nodeAspects;
    }

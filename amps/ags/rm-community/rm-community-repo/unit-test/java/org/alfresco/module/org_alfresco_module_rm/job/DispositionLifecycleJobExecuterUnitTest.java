...
public class DispositionLifecycleJobExecuterUnitTest extends BaseUnitTest {
    ...
    public void before() throws Exception {
        ...
        Answer<Object> doInTransactionAnswer =
                invocation -> {
                    RetryingTransactionCallback callback =
                            (RetryingTransactionCallback) invocation.getArguments()[0];
                    return callback.execute();
                };
        doAnswer(doInTransactionAnswer)
                .when(mockedRetryingTransactionHelper)
                .doInTransaction(
                        any(RetryingTransactionCallback.class),
                        Matchers.anyBoolean(),
                        Matchers.anyBoolean());
    ...
    private void verifyQueryTimes(int numberOfInvocation) {
        ArgumentCaptor<SearchParameters> paramsCaptor =
    ...
    public void noResultsInQuery() {
        ...
        verifyZeroInteractions(
                mockedNodeService, mockedRecordFolderService, mockedRetryingTransactionHelper);
    }
    ...
    public void dispositionActionDoesNotMatch() {
        ...
        doReturn(DESTROY)
                ...
                .getProperty(node1, RecordsManagementModel.PROP_DISPOSITION_ACTION);
        doReturn(DESTROY)
                .when(mockedNodeService)
                .getProperty(node2, RecordsManagementModel.PROP_DISPOSITION_ACTION);

        when(mockedResultSet.getNodeRefs())
                .thenReturn(buildList(node1))
                .thenReturn(buildList(node2));

        when(mockedResultSet.hasMore()).thenReturn(true).thenReturn(false);
        ...
        verify(mockedRetryingTransactionHelper, times(2))
                .doInTransaction(
                        any(RetryingTransactionCallback.class),
                        Matchers.anyBoolean(),
                        Matchers.anyBoolean());
    ...
    /**
     * Brittle unit test that simply checks the generated query is an exact string when the supplied
     * disposition actions are "CUTOFF" and "RETAIN" (see {@link #before}).
     */
    @Test
    public void testGetQuery() {
        ...
        String expected =
                "TYPE:\"rma:dispositionAction\" AND (@rma\\:dispositionAction:(\"cutoff\" OR"
                        + " \"retain\")) AND ISUNSET:\"rma:dispositionActionCompletedAt\"  AND ("
                        + " @rma\\:dispositionEventsEligible:true OR @rma\\:dispositionAsOf:[MIN TO"
                        + " NOW] ) ";
    ...
    /**
     * Given the maximum page of elements for search service is 2 and search service finds more than
     * one page of elements When the job executer runs Then the executer retrieves both pages and
    ...
    public void testPagination() {
        ...
        when(mockedSearchService.query(any(SearchParameters.class)))
                .thenAnswer(
                        (Answer<ResultSet>)
                                invocation -> {
                                    SearchParameters params =
                                            invocation.getArgumentAt(0, SearchParameters.class);
                                    if (params.getSkipCount() == 0) {
                                        // mock first page
                                        ResultSet result1 = mock(ResultSet.class);
                                        when(result1.getNodeRefs())
                                        ...
                                        when(result2.getNodeRefs())
                                                .thenReturn(Arrays.asList(node3, node4));
                                        when(result2.hasMore()).thenReturn(false);
                                        return result2;
                                    }
                                    throw new IndexOutOfBoundsException(
                                            "Pagination did not stop after the second page!");
                                });

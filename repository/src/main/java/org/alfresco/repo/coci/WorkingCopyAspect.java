...
public class WorkingCopyAspect
    ...
    private WorkingCopyAspectCopyBehaviourCallback workingCopyAspectCopyBehaviourCallback =
            new WorkingCopyAspectCopyBehaviourCallback();
    ...
    public void beforeDeleteWorkingCopy(NodeRef nodeRef) {
        ...
        if (checkedOutNodeRef != null) {
            policyBehaviourFilter.disableBehaviour(
                    checkedOutNodeRef, ContentModel.ASPECT_AUDITABLE);
            try {
    ...
    /**
     * onDeleteAssociation policy behaviour If the node has the aspect
     * ASPECT_CMIS_CREATED_CHECKEDOUT and ASSOC_WORKING_COPY_LINK association is deleted, delete the
     * node. Fix for MNT-14850.
     *
     * @param nodeAssocRef ASSOC_WORKING_COPY_LINK association where the source is the checkedOut
     *     node and the target is the workingCopy
     */
     ...
     *
     * @param nodeRef the node reference about to be archived
     */
    @Override
    public void beforeArchiveNode(NodeRef workingCopyNodeRef) {
        ...
        if (checkedOutNodeRef != null) {
            try {
                policyBehaviourFilter.disableBehaviour(
                        workingCopyNodeRef, ContentModel.ASPECT_AUDITABLE);

                if (nodeService.hasAspect(checkedOutNodeRef, ContentModel.ASPECT_LOCKABLE)) {

                    Map<QName, Serializable> checkedOutNodeProperties =
                            nodeService.getProperties(checkedOutNodeRef);
                    ...
                    // get lock properties from checked out node and set them on working copy node
                    // in order to be available for restore
                    String lockOwner =
                            (String) checkedOutNodeProperties.get(ContentModel.PROP_LOCK_OWNER);
                    Date expiryDate =
                            (Date) checkedOutNodeProperties.get(ContentModel.PROP_EXPIRY_DATE);
                    String lockTypeStr =
                            (String) checkedOutNodeProperties.get(ContentModel.PROP_LOCK_TYPE);
                    LockType lockType = lockTypeStr != null ? LockType.valueOf(lockTypeStr) : null;
                    String lifetimeStr =
                            (String) checkedOutNodeProperties.get(ContentModel.PROP_LOCK_LIFETIME);
                    Lifetime lifetime = lifetimeStr != null ? Lifetime.valueOf(lifetimeStr) : null;
                    String additionalInfo =
                            (String)
                                    checkedOutNodeProperties.get(
                                            ContentModel.PROP_LOCK_ADDITIONAL_INFO);

                    nodeService.addAspect(
                            workingCopyNodeRef, ContentModel.ASPECT_ARCHIVE_LOCKABLE, null);
                    ...
                    workingCopyProperties.put(
                            ContentModel.PROP_ARCHIVED_LOCK_ADDITIONAL_INFO, additionalInfo);

                    // Target associations
                    Collection<Pair<Long, AssociationRef>> targetAssocs =
                            nodeDAO.getTargetNodeAssocs(nodeId, null);
                    for (Pair<Long, AssociationRef> targetAssocPair : targetAssocs) {
                        if (ContentModel.ASSOC_ORIGINAL.equals(
                                targetAssocPair.getSecond().getTypeQName())) {
                            workingCopyProperties.put(
                                    ContentModel.PROP_ARCHIVED_TARGET_ASSOCS,
                                    targetAssocPair.getSecond());
                        }
                    }
                    // Source associations
                    Collection<Pair<Long, AssociationRef>> sourceAssocs =
                            nodeDAO.getSourceNodeAssocs(nodeId, null);
                    for (Pair<Long, AssociationRef> sourceAssocPair : sourceAssocs) {
                        if (ContentModel.ASSOC_WORKING_COPY_LINK.equals(
                                sourceAssocPair.getSecond().getTypeQName())) {
                            workingCopyProperties.put(
                                    ContentModel.PROP_ARCHIVED_SOURCE_ASSOCS,
                                    sourceAssocPair.getSecond());
                        }
                    }
     ...
     *
     * @param nodeRef the node reference that was restored
     */
    ...
    public void onRestoreNode(ChildAssociationRef childAssocRef) {
        ...
        if (nodeService.hasAspect(workingCopyNodeRef, ContentModel.ASPECT_WORKING_COPY)) {
            try {
                ...
                String lockOwner =
                        (String) workingCopyProperties.get(ContentModel.PROP_ARCHIVED_LOCK_OWNER);
                workingCopyProperties.remove(ContentModel.PROP_ARCHIVED_LOCK_OWNER);
                Date expiryDate =
                        (Date) workingCopyProperties.get(ContentModel.PROP_ARCHIVED_EXPIRY_DATE);
                workingCopyProperties.remove(ContentModel.PROP_ARCHIVED_EXPIRY_DATE);
                String lockTypeStr =
                        (String) workingCopyProperties.get(ContentModel.PROP_ARCHIVED_LOCK_TYPE);
                workingCopyProperties.remove(ContentModel.PROP_ARCHIVED_LOCK_TYPE);
                LockType lockType = lockTypeStr != null ? LockType.valueOf(lockTypeStr) : null;
                String lifetimeStr =
                        (String)
                                workingCopyProperties.get(ContentModel.PROP_ARCHIVED_LOCK_LIFETIME);
                workingCopyProperties.remove(ContentModel.PROP_ARCHIVED_LOCK_LIFETIME);
                Lifetime lifetime = lifetimeStr != null ? Lifetime.valueOf(lifetimeStr) : null;
                String additionalInfo =
                        (String)
                                workingCopyProperties.get(
                                        ContentModel.PROP_ARCHIVED_LOCK_ADDITIONAL_INFO);
                workingCopyProperties.remove(ContentModel.PROP_ARCHIVED_LOCK_ADDITIONAL_INFO);

                List<AssociationRef> targetAssocList =
                        (ArrayList<AssociationRef>)
                                workingCopyProperties.get(ContentModel.PROP_ARCHIVED_TARGET_ASSOCS);
                if (targetAssocList != null && targetAssocList.get(0) != null) {
                    ...
                    nodeService.createAssociation(
                            targetAssoc.getSourceRef(),
                            targetAssoc.getTargetRef(),
                            ContentModel.ASSOC_ORIGINAL);
                }
                workingCopyProperties.remove(ContentModel.PROP_ARCHIVED_TARGET_ASSOCS);

                ArrayList<AssociationRef> sourceAssocList =
                        (ArrayList<AssociationRef>)
                                workingCopyProperties.get(ContentModel.PROP_ARCHIVED_SOURCE_ASSOCS);
                if (sourceAssocList != null && sourceAssocList.get(0) != null) {
                    ...
                    nodeService.createAssociation(
                            sourceAssoc.getSourceRef(),
                            sourceAssoc.getTargetRef(),
                            ContentModel.ASSOC_WORKING_COPY_LINK);
                }
                ...
                checkedOutNodeProperties.put(
                        ContentModel.PROP_LOCK_ADDITIONAL_INFO, additionalInfo);
    ...
    public void onRemoveAspect(NodeRef nodeRef, QName aspectTypeQName) {
        ...
        throw new UnsupportedOperationException(
                "Use CheckOutCheckInservice to manipulate working copies.");
    }
     ...
     * <b>cm:workingCopy</b> aspect, and to prevent the <b>cm:workingCopy</b> aspect from being
     * carried to the new node.
     *
     * @author Derek Hulley
     * @since 3.2
     */
    private class WorkingCopyAspectCopyBehaviourCallback extends DefaultCopyBehaviourCallback {
        /**
         * Disallows copying of the {@link ContentModel#ASPECT_WORKING_COPY <b>cm:workingCopy</b>}
         * aspect.
         */
        @Override
        ...
        public Map<QName, Serializable> getCopyProperties(
            ...
            if (classQName.equals(ContentModel.ASPECT_WORKING_COPY)) {
                return Collections.emptyMap();
                    ...
                    .contains(ContentModel.ASPECT_WORKING_COPY)) {
                // Generate a new name for a new copy of a working copy
                String newName = null;

                if (copyDetails.isTargetNodeIsNew()) {
                    // This is a copy of a working copy to a new node (not a check in). Try to
                    // derive a new name from the
                    // node it is checked out from
                    NodeRef checkedOutFrom =
                            checkOutCheckInService.getCheckedOut(copyDetails.getSourceNodeRef());
                    if (checkedOutFrom != null) {
                        String oldName =
                                (String)
                                        nodeService.getProperty(
                                                checkedOutFrom, ContentModel.PROP_NAME);
                        int extIndex = oldName.lastIndexOf('.');
                        newName =
                                extIndex == -1
                                        ? oldName + "_" + GUID.generate()
                                        : oldName.substring(0, extIndex)
                                                + "_"
                                                + GUID.generate()
                                                + oldName.substring(extIndex);
                    }
                } else {

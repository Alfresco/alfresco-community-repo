...
public class ForumTopicsFilteredGet extends AbstractDiscussionWebScript {
  ...
  protected static final StoreRef SPACES_STORE = new StoreRef(
   ...
   */
  protected Pair<String, String> getSearchQuery(
    SiteInfo site,
    String author,
    int daysAgo
  ) {
    ...
    Pair<String, String> searchQuery = new Pair<String, String>(
      search,
   ...
   * @param paging PagingRequest
   */
  protected PagingResults<TopicInfo> wrap(
    final ResultSet finalResults,
    PagingRequest paging
  ) {
    ...
    Comparator<TopicInfo> lastPostDesc = new Comparator<TopicInfo>() {
      ...
      public int compare(TopicInfo t1, TopicInfo t2) {
        ...
        if (discussionService.getMostRecentPost(t1) != null) {
          t1LastPostDate =
            discussionService.getMostRecentPost(t1).getCreatedAt();
        }

        Date t2LastPostDate = t2.getCreatedAt();
        if (discussionService.getMostRecentPost(t2) != null) {
          t2LastPostDate =
            discussionService.getMostRecentPost(t2).getCreatedAt();
        }
    ...
    for (ResultSetRow row : finalResults) {
      Pair<TopicInfo, PostInfo> pair = discussionService.getForNodeRef(
        row.getNodeRef()
      );

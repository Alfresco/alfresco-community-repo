 ...
 *
 * <p>Primarily tests the file plan permission service interaction with the permission service.
 *
...
public class FilePlanPermissionServiceImplUnitTest extends BaseUnitTest {
    ...
    public void before() throws Exception {
        ...
        doReturn(FilePlanComponentKind.FILE_PLAN)
                .when(filePlanPermissionService)
                .getFilePlanComponentKind(filePlan);
        doReturn(FilePlanComponentKind.RECORD_CATEGORY)
                .when(filePlanPermissionService)
                .getFilePlanComponentKind(rootRecordCategory);
        doReturn(FilePlanComponentKind.RECORD_CATEGORY)
                .when(filePlanPermissionService)
                .getFilePlanComponentKind(recordCategory);
        doReturn(FilePlanComponentKind.RECORD_FOLDER)
                .when(filePlanPermissionService)
                .getFilePlanComponentKind(newRecordFolder);
        doReturn(FilePlanComponentKind.RECORD)
                .when(filePlanPermissionService)
                .getFilePlanComponentKind(newRecord);
        doReturn(FilePlanComponentKind.UNFILED_RECORD_FOLDER)
                .when(filePlanPermissionService)
                .getFilePlanComponentKind(unfiledRecordFolder);
        doReturn(FilePlanComponentKind.UNFILED_RECORD_CONTAINER)
                .when(filePlanPermissionService)
                .getFilePlanComponentKind(unfiledRecordContainer);
        doReturn(FilePlanComponentKind.RECORD)
                .when(filePlanPermissionService)
                .getFilePlanComponentKind(unfiledRecord);
        doReturn(FilePlanComponentKind.HOLD_CONTAINER)
                .when(filePlanPermissionService)
                .getFilePlanComponentKind(holdContainer);
        doReturn(FilePlanComponentKind.HOLD)
                .when(filePlanPermissionService)
                .getFilePlanComponentKind(hold);
    }
    ...
    public void setReadPermissionOnUnfiledRecordFolder() {
        ...
        filePlanPermissionService.setPermission(
                unfiledRecordFolder, AUTHORITY, RMPermissionModel.READ_RECORDS);

        // verify permission set on target node
        verify(mockedPermissionService, times(1))
                .setPermission(
                        unfiledRecordFolder, AUTHORITY, RMPermissionModel.READ_RECORDS, true);
    }
    ...
    public void setReadAndFilePermissionOnUnfileRecordFolder() {
        // set read permission on unfiled record folder
        filePlanPermissionService.setPermission(
                unfiledRecordFolder, AUTHORITY, RMPermissionModel.FILING);

        // verify permission set on target node
        verify(mockedPermissionService, times(1))
                .setPermission(unfiledRecordFolder, AUTHORITY, RMPermissionModel.FILING, true);
    }
    ...
    public void deletePermissionFromUnfiledRecordFolder() {
        ...
        filePlanPermissionService.deletePermission(
                unfiledRecordFolder, AUTHORITY, RMPermissionModel.READ_RECORDS);

        // verify permission deleted on target node
        verify(mockedPermissionService, times(1))
                .deletePermission(unfiledRecordFolder, AUTHORITY, RMPermissionModel.READ_RECORDS);
    }
    ...
    public void setReadPermissionOnHoldContainer() {
        ...
        filePlanPermissionService.setPermission(
                holdContainer, AUTHORITY, RMPermissionModel.READ_RECORDS);

        // verify permission set on target node
        verify(mockedPermissionService, times(1))
                .setPermission(holdContainer, AUTHORITY, RMPermissionModel.READ_RECORDS, true);
    }
    ...
    public void setFilingPermissionOnHoldContainer() {
        ...
        // verify permission set on target node
        verify(mockedPermissionService, times(1))
                .setPermission(holdContainer, AUTHORITY, RMPermissionModel.FILING, true);
    }
    ...
    public void setReadPermissionOnHold() {
        ...
        // verify permission set on target node
        verify(mockedPermissionService, times(1))
                .setPermission(hold, AUTHORITY, RMPermissionModel.READ_RECORDS, true);
    }
    ...
    public void setFilingPermissionOnHold() {
        ...
        // verify permission set on target node
        verify(mockedPermissionService, times(1))
                .setPermission(hold, AUTHORITY, RMPermissionModel.FILING, true);
    }
    ...
    private void setupPermissions(NodeRef nodeRef) {
        ...
        perms.add(
                new AccessPermissionImpl(
                        RMPermissionModel.READ_RECORDS, AccessStatus.ALLOWED, AUTHORITY, 0));
        perms.add(
                new AccessPermissionImpl(
                        RMPermissionModel.FILING, AccessStatus.ALLOWED, AUTHORITY2, 1));
     ...
     */
    private void verifyInitPermissions(
            NodeRef parent,
            NodeRef child,
            VerificationMode read,
            VerificationMode filling,
            boolean isParentFilePlan,
            boolean isInherited) {
        ...
        if (isParentFilePlan) {
            ...
            // verify all the inherited permissions are set correctly (note read are not inherited
            // from fileplan)
            verify(mockedPermissionService, filling)
                    .setPermission(child, AUTHORITY2, RMPermissionModel.FILING, true);
            verify(mockedPermissionService, read)
                    .setPermission(child, AUTHORITY, RMPermissionModel.READ_RECORDS, true);
        }
    }
    ...
    public void initPermissionsForNewUnfiledRecordFolder() {
        ...
        verifyInitPermissions(
                unfiledRecordContainer, unfiledRecordFolder, never(), times(1), false, true);
    }

...
public class AuditQueryParameters {
    ...
    /**
     * @return Returns <tt>true</tt> if any query using these parameters will necessarily yield no
     *     results.
     */
    ...
    /**
     * @param forward <tt>true</tt> for results to ordered from first to last, or <tt>false</tt> to
     ...
     *     or the search value ({@link Pair#getSecond() second} value) may be <tt>null</tt>, but not
     *     both.
     */
    public void addSearchKey(String searchKey, Serializable searchValue) {
        if (searchKey == null && searchValue == null) {
            throw new IllegalArgumentException(
                    "A search key must have a 'searchKey' and/or a 'searchValue'.");
        }
        if (searchKeyValues.size() > 0) {
            throw new UnsupportedOperationException(
                    "Only one search key-value pair is currently supported.");
        }

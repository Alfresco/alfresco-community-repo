...
public class JSONConversionComponent {
  ...
  private static final String CONTENT_DOWNLOAD_API_URL =
    "/slingshot/node/content/{0}/{1}/{2}/{3}";

  /** Logger */
  private static Log logger = LogFactory.getLog(JSONConversionComponent.class);

  /** Registered decorators */
  protected Map<QName, PropertyDecorator> propertyDecorators = new HashMap<QName, PropertyDecorator>(
    8
  );
  ...
  public JSONObject toJSONObject(
    ...
    if (this.nodeService.exists(nodeRef)) {
      if (
        ...
        json.put(
          "properties",
          propertiesToJSON(nodeRef, nodeInfo.getProperties(), useShortQNames)
        );
  ...
  protected void setRootValues(
    ...
    rootJSONObject.put(
      "type",
      nameToString(nodeInfo.getType(), useShortQNames)
    );
    ...
    if (nodeInfo.isLink()) {
      ...
      if (targetNodeRef != null) {
        rootJSONObject.put(
          "linkedNode",
          toJSONObject(targetNodeRef, useShortQNames)
        );
      }
    }
    ...
    if (nodeInfo.isFolder() == false) {
      ...
      if (cdata != null) {
        ...
        if (mimetypeDescriptions.containsKey(cdata.getMimetype())) {
          rootJSONObject.put(
            "mimetypeDisplayName",
            mimetypeDescriptions.get(cdata.getMimetype())
          );
        }
  ...
  protected JSONObject userPermissionsToJSON(final NodeRef nodeRef) {
    ...
    for (String userPermission : this.userPermissions) {
      boolean hasPermission = AccessStatus.ALLOWED.equals(
        permissionService.hasPermission(nodeRef, userPermission)
      );
  ...
  protected Object propertyToJSON(
    final NodeRef nodeRef,
    ...
    if (value != null) {
      ...
      if (propertyDecorators.containsKey(propertyName)) {
        JSONAware jsonAware = propertyDecorators
          .get(propertyName)
          .decorate(propertyName, nodeRef, value);
        if (jsonAware != null) {
          return jsonAware;
        }
      } else {
        ...
        if (value instanceof Date) {
          ...
          return (
            Double.isInfinite((Double) value) || Double.isNaN((Double) value)
              ? null
              : value.toString()
          );
        } else if (value instanceof Float) {
          return (
            Float.isInfinite((Float) value) || Float.isNaN((Float) value)
              ? null
              : value.toString()
          );
        } else {
   ...
   */
  @SuppressWarnings("unchecked")
  protected JSONObject propertiesToJSON(
    NodeRef nodeRef,
    ...
    for (QName propertyName : properties.keySet()) {
      try {
        ...
        propertiesJSON.put(
          key,
          propertyToJSON(nodeRef, propertyName, key, value)
        );
        ...
        if (logger.isDebugEnabled()) logger.debug(
          "Ignoring property '" +
          propertyName +
          "' as its namespace is not registered"
        );
      }
    }
  ...
  protected JSONArray allSetPermissionsToJSON(NodeRef nodeRef) {
    Set<AccessPermission> acls = permissionService.getAllSetPermissions(
      nodeRef
    );
    ...
    if (isShortName) {
      ...
      if (prefix == null) {
        ...
        Collection<String> prefixes =
          this.namespaceService.getPrefixes(qname.getNamespaceURI());

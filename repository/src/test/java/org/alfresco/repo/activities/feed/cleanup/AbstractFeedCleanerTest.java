...
public abstract class AbstractFeedCleanerTest {
    ...
    public void setUp() throws Exception {
        ...
        // Let's shut down the scheduler so that we aren't competing with the scheduled versions of
        // jobs (ie. feed cleaner)
    ...
    public void testMaxAge() throws Exception {
        ...
        feedEntry.setPostDate(
                new Date(System.currentTimeMillis() - (20 * 60 * 1000L))); // 20 mins ago
        ...
        assertEquals(
                2, feedDAO.selectUserFeedEntries(TEST_USER_B, null, false, false, -1L, -1).size());
        ...
        assertEquals(
                1, feedDAO.selectUserFeedEntries(TEST_USER_B, null, false, false, -1L, -1).size());
    }
    ...
    public void testMaxSize() throws Exception {
        ...
        assertEquals(10, feedDAO.selectSiteFeedEntries(TEST_SITE_4, -1).size());
        assertEquals(
                10, feedDAO.selectUserFeedEntries(TEST_USER_D, null, false, false, -1L, -1).size());
        ...
        assertEquals(
                2, feedDAO.selectUserFeedEntries(TEST_USER_D, null, false, false, -1L, -1).size());
        ...
        assertEquals(
                10, feedDAO.selectUserFeedEntries(TEST_USER_F, null, false, false, -1L, -1).size());
    ...
    public void testSiteDelete() throws Exception {
        ...
        assertEquals(
                0, feedDAO.selectUserFeedEntries(TEST_USER_D, null, false, false, -1L, -1).size());
        ...
        assertEquals(
                site4FeedCnt + site5FeedCnt,
                feedDAO.selectUserFeedEntries(TEST_USER_D, null, false, false, -1L, -1).size());
                    ...
                    public Void execute() throws Throwable {
                        ...
                        assertEquals(
                                ...
                                feedDAO.selectUserFeedEntries(
                                                TEST_USER_D, null, false, false, -1L, -1)
                                        .size());
    ...
    protected void testPersonDelete(final String userId) throws Exception {
        ...
        assertEquals(
                site6FeedCnt + site7FeedCnt,
                feedDAO.selectUserFeedEntries(userId, null, false, false, -1L, -1).size());
        ...
        transactionHelper.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                    public Void execute() throws Throwable {
                        assertEquals(
                                site6FeedCnt,
                                feedDAO.selectSiteFeedEntries(TEST_SITE_6, -1).size());
                        assertEquals(
                                site7FeedCnt,
                                feedDAO.selectSiteFeedEntries(TEST_SITE_7, -1).size());

                        assertEquals(
                                0,
                                feedDAO.selectUserFeedEntries(userId, null, false, false, -1L, -1)
                                        .size());
            ...
            try {
                ...
                if (type == 0) {
                    ...
                    for (int i = 0; i < insertCount; i++) {
                        ...
                        feedEntry.setPostDate(
                                new Date(System.currentTimeMillis() - (i * 60 * 1000L)));
                        feedEntry.setSiteNetwork(TEST_SITE_4);
                        feedEntry.setActivityType("testActivityType");
                        feedEntry.setPostUserId(TEST_USER_C);
                        feedEntry.setFeedUserId("");
                            ...
                            transactionHelper.doInTransaction(
                                    new RetryingTransactionHelper.RetryingTransactionCallback<
                                        ...
                                        public Integer execute() throws Throwable {
                                            // query some entries
                                            int selectCount =
                                                    feedDAO.selectSiteFeedEntries(TEST_SITE_4, -1)
                                                            .size();
                                            return selectCount;
                                        }
                                    });

...
public class CreateFolderTests extends CmisTest {
    ...
    @TestRail(
            ...
            description =
                    "Verify site manager is able to create folder in DocumentLibrary with CMIS")
    ...
    public void inexistentUserShouldNotCreateFolder() throws Exception {
        testFolder = FolderModel.getRandomFolderModel();
        cmisApi.authenticateUser(new UserModel("random user", "random password"))
                .usingSite(testSite)
                .createFolder(testFolder);
    }
    ...
    public void shouldNotCreateFolderAtInvalidPath() throws Exception {
        testFolder = FolderModel.getRandomFolderModel();
        cmisApi.authenticateUser(testUser)
                .usingSite(new SiteModel("inexitentSite"))
                .createFolder(testFolder);
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify site manager is not able to create folder with invalid base type id"
                            + " with CMIS")
    @Test(
            groups = {TestGroup.REGRESSION, TestGroup.CMIS},
            expectedExceptions = CmisObjectNotFoundException.class)
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify site manager is not able to create folder with cmis:document base type"
                            + " id with CMIS")
    @Test(
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify that admin user is not able to create folder with invalid characters in"
                            + " name")
    @Test(
            groups = {TestGroup.REGRESSION, TestGroup.CMIS},
            expectedExceptions = CmisConstraintException.class)
    ...
    public void siteManagerCannotCreateFolderWithNoParamenters() throws Exception {
        ...
        testFolder = FolderModel.getRandomFolderModel();
        cmisApi.authenticateUser(testUser).usingSite(testSite).createFolder(testFolder, properties);
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify site manager is not able to create folder with properties specific for"
                            + " documents")
    @Test(
            groups = {TestGroup.REGRESSION, TestGroup.CMIS},
            expectedExceptions = IllegalArgumentException.class,
            expectedExceptionsMessageRegExp =
                    "Property 'cmis:isLatestMajorVersion' is not valid for this type or one of the"
                            + " secondary types!*")
    ...
    public void disabledUserShouldNotCreateFolder() throws Exception {
        ...
        cmisApi.usingShared().createFolder(testFolder);
    }
    ...
    @TestRail(
            ...
            executionType = ExecutionType.REGRESSION,
            description =
                    "Verify site manager is able to create folder with a name containing multi byte"
                            + " characters.")
    @Test(groups = {TestGroup.REGRESSION, TestGroup.CMIS})
    public void managerIsAbleToCreateFolderWithMultiByteName() throws Exception {
        testFolder =
                new FolderModel(RandomData.getRandomAlphanumeric() + "\ufeff\u6768\u6728\u91d1");
        cmisApi.authenticateUser(testUser)
                .usingSite(testSite)
                .createFolder(testFolder)
                .and()

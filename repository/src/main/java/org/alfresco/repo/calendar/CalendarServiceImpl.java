...
public class CalendarServiceImpl implements CalendarService {
    ...
    /**
     * For backwards compatibility with pre-Swift, we are asking the query to give us an accurate
     * total count of how many calendar nodes there are. This may need to change in the future -
     * certainly if the current 'brute force' query is replaced by a database query.
     */
    private static final int MAX_QUERY_ENTRY_COUNT = 10000;

    protected static final String CANNED_QUERY_GET_CHILDREN =
            "calendarGetChildrenCannedQueryFactory";
    protected static final String CANNED_QUERY_GET_ENTRIES =
            "calendarGetCalendarEntriesCannedQueryFactory";
    ...
    public CalendarEntry getCalendarEntry(String siteShortName, String entryName) {
        ...
        NodeRef event =
                nodeService.getChildByName(container, ContentModel.ASSOC_CONTAINS, entryName);
    ...
    public CalendarEntry createCalendarEntry(String siteShortName, CalendarEntry entry) {
        if (entry.getNodeRef() != null) {
            throw new IllegalArgumentException(
                    "Can't call create for a calendar entry that was previously persisted");
        }
        ...
        String name = (new Date()).getTime() + "-" + Math.round(Math.random() * 10000) + ".ics";
    ...
    public CalendarEntry updateCalendarEntry(CalendarEntry entry) {
        ...
        if (entry.getNodeRef() == null) {
            throw new IllegalArgumentException(
                    "Can't update a calendar entry that was never persisted, call create instead");
        }
        ...
        for (Map.Entry<QName, Serializable> prop :
                nodeService.getProperties(entry.getNodeRef()).entrySet()) {
        ...
        if (entry instanceof CalendarEntryDTO) {
                    ...
                    .setModifiedAt(
                            (Date)
                                    nodeService.getProperty(
                                            entry.getNodeRef(), ContentModel.PROP_MODIFIED));
            ...
            return getCalendarEntry(
                    entry.getNodeRef(), entry.getSystemName(), entry.getContainerNodeRef());
        }
    }
    ...
    public void deleteCalendarEntry(CalendarEntry entry) {
        if (entry.getNodeRef() == null) {
            throw new IllegalArgumentException(
                    "Can't delete a calendar entry that was never persisted");
        }
    ...
    public PagingResults<CalendarEntry> listCalendarEntries(
        ...
        GetChildrenCannedQueryFactory getChildrenCannedQueryFactory =
                (GetChildrenCannedQueryFactory)
                        cannedQueryRegistry.getNamedObject(CANNED_QUERY_GET_CHILDREN);
    ...
    public PagingResults<CalendarEntry> listCalendarEntries(
        ...
        // Run the canned query
        GetCalendarEntriesCannedQueryFactory cqFactory =
                (GetCalendarEntriesCannedQueryFactory)
                        cannedQueryRegistry.getNamedObject(CANNED_QUERY_GET_ENTRIES);
        GetCalendarEntriesCannedQuery cq =
                (GetCalendarEntriesCannedQuery)
                        cqFactory.getCannedQuery(containers, from, to, paging);
    ...
    @Override
    public PagingResults<CalendarEntry> listOutlookCalendarEntries(
            String siteShortName, String outlookUID, PagingRequest paging) {
        NodeRef container = getSiteCalendarContainer(siteShortName, false);
        if (container == null) {
            // No events
            return new EmptyPagingResults<CalendarEntry>();
    ...
    private PagingResults<CalendarEntry> wrap(
            final PagingResults<NodeRef> results, final NodeRef container) {

...
public class EventGeneratorQueue implements InitializingBean {
  ...
  public void setEvent2MessageProducer(
    Event2MessageProducer event2MessageProducer
  ) {
  ...
  public void accept(Callable<RepoEvent<?>> maker) {
    ...
    enqueueThreadPoolExecutor.execute(() -> {
      try {
        ...
        LOGGER.error(
          "Unexpected error while enqueuing maker function for repository event" +
          e
        );
      }
    });
        ...
        try {
          while (!Thread.interrupted()) {
            try {
              ...
              LOGGER.error(
                "Unexpected error while dequeuing and sending repository event" +
                e
              );
            }
          }

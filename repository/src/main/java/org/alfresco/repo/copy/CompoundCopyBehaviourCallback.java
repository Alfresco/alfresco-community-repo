 ...
 *
 * <p>When multiple policy handlers register callback for the same model class, an instance of this
 * class is used to resolve the calls. The behaviour is sometimes able to resolve conflicts and
 * sometimes not. Look at the individual callback methods to see how conflicts are handled.
 *
 * @author Derek Hulley
 * @since 3.2
 */
public class CompoundCopyBehaviourCallback extends AbstractCopyBehaviourCallback {
    ...
    public Pair<AssocCopySourceAction, AssocCopyTargetAction> getAssociationCopyAction(
        ...
        for (CopyBehaviourCallback callback : callbacks) {
            Pair<AssocCopySourceAction, AssocCopyTargetAction> action =
                    callback.getAssociationCopyAction(classQName, copyDetails, assocCopyDetails);
        ...
        Pair<AssocCopySourceAction, AssocCopyTargetAction> bestAction =
                new Pair<AssocCopySourceAction, AssocCopyTargetAction>(
                        bestSourceAction, bestTargetAction);
        // Done
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Association copy behaviour: "
                            + bestAction
                            + "\n"
                            + "   "
                            + assocCopyDetails
    ...
    /**
     * Individual callbacks effectively have a veto on the copy i.e. if one of the callbacks returns
     * <tt>false</tt> for {@link
     * CopyBehaviourCallback#getMustCopy(org.alfresco.service.namespace.QName,
     * org.alfresco.repo.copy.CopyDetails)}, then the copy will NOT proceed. However, a warning is
     * generated indicating that there is a conflict in the defined behaviour.
     *
     * @return Returns <tt>true</tt> if all registered callbacks return <tt>true</tt> or
     *     <b><tt>false</tt> if any of the registered callbacks return <tt>false</tt></b>.
     */
    public boolean getMustCopy(QName classQName, CopyDetails copyDetails) {
            ...
            if (mustCopyLocal && firstVeto != null) {
                ...
                logger.warn(
                        "CopyBehaviourCallback '"
                                ...
                                + "' "
                                + "is attempting to induce a copy when callback '"
                                + firstVeto.getClass().getName()
                                + "' has already vetoed it.  Copying of '"
        ...
        if (firstVeto == null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Copy behaviour vetoed for node "
                                + copyDetails.getSourceNodeRef()
                                + "\n"
                                + "   First veto: "
                                + firstVeto.getClass().getName()
    ...
    /**
     * Uses the ChildAssocCopyAction ordering to drive priority i.e. a vote to copy will override a
     * vote NOT to copy.
     *
    ...
    public ChildAssocCopyAction getChildAssociationCopyAction(
        ...
        for (CopyBehaviourCallback callback : callbacks) {
            ChildAssocCopyAction action =
                    callback.getChildAssociationCopyAction(
                            classQName, copyDetails, childAssocCopyDetails);
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Child association copy behaviour: "
                            + bestAction
                            + "\n"
                            + "   "
    ...
    public ChildAssocRecurseAction getChildAssociationRecurseAction(
        ...
        for (CopyBehaviourCallback callback : callbacks) {
            ChildAssocRecurseAction action =
                    callback.getChildAssociationRecurseAction(
                            classQName, copyDetails, childAssocCopyDetails);
            if (action.compareTo(bestAction) > 0) {
                // We've trumped the last best one
                bestAction = action;
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Child association recursion behaviour: "
                            + bestAction
                            + "\n"
                            + "   "
                            + childAssocCopyDetails
    ...
    /**
     * The lowest common denominator applies here. The properties are passed to each callback in
     * turn. The resulting map is then passed to the next callback and so on. If any callback
     * removes or alters properties, these will not be recoverable.
     *
    ...
    public Map<QName, Serializable> getCopyProperties(
        ...
        for (CopyBehaviourCallback callback : callbacks) {
            Map<QName, Serializable> propsToCopy =
                    callback.getCopyProperties(classQName, copyDetails, copyProperties);

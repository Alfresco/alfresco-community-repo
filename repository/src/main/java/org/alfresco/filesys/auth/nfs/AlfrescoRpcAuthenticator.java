...
public class AlfrescoRpcAuthenticator
  implements RpcAuthenticator, InitializingBean {

  // Debug logging

  private static final Log logger = LogFactory.getLog(
    "org.alfresco.nfs.protocol.auth"
  );
  ...
  public Object authenticateRpcClient(int authType, RpcPacket rpc)
    ...
    if (authType == AuthType.Unix) {
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "RpcAuth: Type=Unix uid=" + uid + ", gid=" + gid
      );
      ...
      if (userName == null) throw new RpcAuthenticationException(NFS.StsAccess);

      // Check if the Unix authentication session table is valid

      sessKey =
        new Long(
          (((long) rpc.getClientAddress().hashCode()) << 32) + (gid << 16) + uid
        );
    }
    ...
    if (sessKey == null) throw new RpcAuthenticationException(
      Rpc.AuthBadCred,
    ...
    if (logger.isDebugEnabled()) logger.debug(
      ...
      ", authType=" +
      AuthType.getTypeAsString(authType) +
      ", sessKey=" +
      sessKey
    );
  ...
  public ClientInfo getRpcClientInformation(Object sessKey, RpcPacket rpc) {
    ...
    if (authType == AuthType.Unix) {
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "RpcClientInfo: username=" + userName + ", uid=" + uid + ", gid=" + gid
      );
  ...
  public void setCurrentUser(SrvSession sess, final ClientInfo client) {
    try {
      ...
      doInTransaction(
        new RetryingTransactionCallback<Void>() {
          ...
          public Void execute() throws Throwable {
            ...
            if (
              client == null ||
              client.isNullSession() ||
              client instanceof AlfrescoClientInfo == false
            ) {
              ...
              if (logger.isDebugEnabled()) logger.debug(
                "Clear security context, client=" + client
              );
              ...
              if (alfClient.hasAuthenticationTicket()) {
                ...
                try {
                  ...
                  getAuthenticationService()
                    .validate(alfClient.getAuthenticationTicket());

                  // DEBUG

                  if (logger.isDebugEnabled()) logger.debug(
                    "Set user using auth ticket, ticket=" +
                    alfClient.getAuthenticationTicket()
                  );
                  ...
                  if (logger.isDebugEnabled()) {
                    logger.debug(
                      "Failed to set user using auth ticket, ticket=" +
                      alfClient.getAuthenticationTicket() +
                      ", re-authenticating"
                    );
              ...
              if (!alfClient.hasAuthenticationTicket()) {
                ...
                if (
                  ...
                  authenticationService.isAuthenticationCreationAllowed()
                ) {
                ...
                getAuthenticationComponent()
                  .setCurrentUser(client.getUserName());
                alfClient.setAuthenticationTicket(
                  getAuthenticationService().getCurrentTicket()
                );
              ...
              if (logger.isDebugEnabled()) logger.debug("Set guest user");
            }
            return null;
          }
        }
      ...
      if (logger.isErrorEnabled()) logger.error(
        "Error in RPC authenticator setting current user",
        ex
      );
      throw ex;
    }
  ...
  public void initialize(ServerConfiguration config, ConfigElement params)
    ...
    setAuthenticationService(
      (MutableAuthenticationService) alfrescoConfig.getAuthenticationService()
    );
      ...
      for (ConfigElement userElem : userMaps) {
        ...
        if (userElem.getName().equalsIgnoreCase("user")) {
          ...
          if (
            ...
            "Empty user name, or name not specified"
          );

          if (
            uidStr == null || uidStr.length() == 0
          ) throw new InvalidConfigurationException(
            "Invalid uid, or uid not specified, for user " + userName
          );

          if (
            gidStr == null || gidStr.length() == 0
          ) throw new InvalidConfigurationException(
            "Invalid gid, or gid not specified, for user " + userName
          );
          ...
          try {
            ...
            throw new InvalidConfigurationException(
              "Invalid uid value, " + uidStr + " for user " + userName
            );
          }
          ...
          try {
            ...
            throw new InvalidConfigurationException(
              "Invalid gid value, " + gidStr + " for user " + userName
            );
          }
  ...
  public void afterPropertiesSet() throws InvalidConfigurationException {
    ...
    if (this.userMappings != null) {
      ...
      for (UserMapping userElem : this.userMappings) {
        ...
        Integer idKey = new Integer(
          (userElem.getGid() << 16) + userElem.getUid()
        );
        if (m_idMap.containsKey(idKey) == false) {
          ...
          if (logger.isDebugEnabled()) logger.debug(
            "Added RPC user mapping for user " +
            userName +
            " uid=" +
            userElem.getUid() +
            ", gid=" +
          ...
          logger.debug(
            "Ignored duplicate mapping for uid=" +
            userElem.getUid() +
            ", gid=" +
            userElem.getGid()
          );
   ...
   */
  protected <T> T doInTransaction(
    RetryingTransactionHelper.RetryingTransactionCallback<T> callback
  ) {
    ...
    return txService
      ...
      .doInTransaction(callback, txService.isReadOnly());
  }

...
public class JmxDumpUtil {
  ...
  private static final String[] REDACTED_INPUTS = {
    "password",
    "token",
    "pwd",
  };
  ...
  public static void dumpConnection(
    ...
    Set<ObjectName> newObjectNames = new TreeSet<ObjectName>(
  ...
  private static void printMBeanInfo(
    ...
    if (
      objectName.getCanonicalName().equals("Alfresco:Name=SystemProperties")
    ) {
    ...
    tabulate(
      JmxDumpUtil.NAME_HEADER,
      JmxDumpUtil.VALUE_HEADER,
      attributes,
      out,
      0
    );
  ...
  static String[] cleanPasswordsFromInputArguments(
    ...
    String[] redactedInputs
  ) {
    Pattern passwordRedactPattern = Pattern.compile(
      createPasswordFindRegexString(redactedInputs)
    );
   ...
   */
  static String cleanPasswordFromInputArgument(
    String input,
    Pattern redactedInputPattern
  ) {
    //Replace the whole string with just capture group 1 to remove the desired value and concat the protected value.
    String output = redactedInputPattern
      .matcher(input)
      .replaceAll("$1" + PROTECTED_VALUE);
    return output;
  }
  ...
  private static void printCompositeInfo(
  ...
  private static void showStartBanner(PrintWriter out) {
    ...
    out.println(
      JmxDumpUtil.class.getSimpleName() + " started: " + df.format(new Date())
    );
    out.println();
  }
}

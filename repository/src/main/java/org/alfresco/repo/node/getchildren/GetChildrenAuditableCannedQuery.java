...
public class GetChildrenAuditableCannedQuery
  ...
  private static final String QUERY_SELECT_GET_NODES =
    "select_GetChildrenAuditableCannedQuery";
  ...
  @Override
  protected List<NodeBackedEntity> queryAndFilter(
    ...
    boolean filterByCreatedDate =
      (
        paramBean.getCreatedBefore() != null &&
        paramBean.getCreatedAfter() != null
      );
    boolean filterByModifiedDate =
      (
        paramBean.getModifiedBefore() != null &&
        paramBean.getModifiedAfter() != null
      );

    // note: refer to SQL for specific DB filtering (eg.parent nodes etc)
    List<NodeBackedEntity> results = cannedQueryDAO.executeQuery(
      QUERY_NAMESPACE,
    ...
    List<NodeBackedEntity> filtered = new ArrayList<NodeBackedEntity>(
      results.size()
    );
    for (NodeBackedEntity result : results) {
      ...
      if (filterByCreatedDate || filterByModifiedDate) {
        ...
        if (filterByCreatedDate) {
          if (
            createdDate.before(paramBean.getCreatedAfter()) ||
        ...
        if (filterByModifiedDate) {
          if (
            modifiedDate.before(paramBean.getModifiedAfter()) ||

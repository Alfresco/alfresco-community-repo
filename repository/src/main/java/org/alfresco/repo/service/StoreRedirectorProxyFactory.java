...
public class StoreRedirectorProxyFactory<I>
  ...
  private static final Log logger = LogFactory.getLog(
    StoreRedirectorProxyFactory.class
  );
  ...
  public void setRedirectedProtocolBindings(Map<String, I> protocolBindings) {
    if (protocolBindings != null) {
      this.redirectedProtocolBindings =
        Collections.unmodifiableMap(protocolBindings);
    } else {
  ...
  public void setRedirectedStoreBindings(Map<String, I> storeBindings) {
    Map<StoreRef, I> redirectedStoreBindingsMap = new HashMap<StoreRef, I>(
      storeBindings.size()
    );
    for (String ref : storeBindings.keySet()) {
      redirectedStoreBindingsMap.put(new StoreRef(ref), storeBindings.get(ref));
    }
    redirectedStoreBindings =
      Collections.unmodifiableMap(redirectedStoreBindingsMap);
  }
    ...
    public Object invoke(Object proxy, Method method, Object[] args)
      ...
      if (storeRef == null) {
        ...
        if (StoreRedirectorProxyFactory.this.redirectedStoreBindings != null) {
          binding =
            StoreRedirectorProxyFactory.this.redirectedStoreBindings.get(
                storeRef
              );
        }
        if (
        ...
        if (binding == null) {
          throw new ServiceException(
            "Store type " + storeRef + " is not supported"
          );
        }
      }

      if (logger.isDebugEnabled()) logger.debug(
        "Redirecting method " + method + " based on store type " + storeRef
      );

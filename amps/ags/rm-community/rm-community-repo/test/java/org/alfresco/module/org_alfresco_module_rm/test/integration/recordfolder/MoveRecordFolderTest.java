...
public class MoveRecordFolderTest extends BaseRMTestCase {
   ...
   */
  public void testMoveRecordFolderBeforeCutOffFolderLevelDisposition()
    throws Exception {
    doBehaviourDrivenTest(
      new BehaviourDrivenTest(null, false) {
          ...
          doTestInTransaction(
            new VoidTest() {
              public void runImpl() throws Exception {
                ...
                assertTrue(
                  nodeService.hasAspect(recordFolder, ASPECT_RM_SEARCH)
                );
                assertEquals(
                  CutOffAction.NAME,
                  nodeService.getProperty(
                    recordFolder,
                    PROP_RS_DISPOSITION_ACTION_NAME
                  )
                );
                assertNotNull(
                  nodeService.getProperty(
                    recordFolder,
                    PROP_RS_DISPOSITION_ACTION_AS_OF
                  )
                );
              }
            }
   ...
   *
   */
  public void testMoveRecordFolderBeforeCutOffIntoAFolderWithNoDisposition()
    throws Exception {
    doBehaviourDrivenTest(
      new BehaviourDrivenTest(null, false) {
        NodeRef recordFolder;
   ...
   *
   */
  public void testMoveRecordFolderWithRecordsBeforeCutOffRecordLevelDisposition()
    throws Exception {
    doBehaviourDrivenTest(
      new BehaviourDrivenTest(null, false) {
        NodeRef record;
          ...
          doTestInTransaction(
            new VoidTest() {
              public void runImpl() throws Exception {
                ...
                assertNotNull(dispositionAction.getAsOfDate());
                assertTrue(
                  dispositionService.isNextDispositionActionEligible(record)
                );
              }
            }
          );
  ...
  public void testMoveRecordFolderFromNoDisToDis() throws Exception {
    final NodeRef recordFolder = doTestInTransaction(
      new Test<NodeRef>() {
        @Override
        public NodeRef run() {
          // create a record category (no disposition schedule)
          NodeRef recordCategory = filePlanService.createRecordCategory(
            filePlan,
            "Caitlin Reed"
          );
        ...
        public void test(NodeRef result) throws Exception {
          ...
          assertFalse(
            nodeService.hasAspect(result, ASPECT_DISPOSITION_LIFECYCLE)
          );
          assertEquals(
            AccessStatus.ALLOWED,
            permissionService.hasPermission(result, FILING)
          );
        }
      }
    );
      ...
      new Test<NodeRef>() {
        ...
        public NodeRef run() {
          ...
          return fileFolderService
            .create(recordFolder, "mytest.txt", ContentModel.TYPE_CONTENT)
            .getNodeRef();
        }
      ...
      new Test<NodeRef>() {
        ...
        public NodeRef run() throws Exception {
          ...
          assertEquals(
            ...
            capability.evaluate(recordFolder)
          );
          assertEquals(
            AccessDecisionVoter.ACCESS_GRANTED,
            capability.evaluate(recordFolder, rmContainer)
          );
          ...
          assertEquals(
            AccessDecisionVoter.ACCESS_GRANTED,
            moveCapability.evaluate(recordFolder, rmContainer)
          );

          // move the node
          return fileFolderService
            .move(recordFolder, rmContainer, null)
            .getNodeRef();
        }
  ...
  public void testMoveCutoffRecordFolder() throws Exception {
    final NodeRef destination = doTestInTransaction(
      new Test<NodeRef>() {
        @Override
        public NodeRef run() {
          // create a record category (no disposition schedule)
    ...
    final NodeRef testFolder = doTestInTransaction(
      new Test<NodeRef>() {
        @Override
        public NodeRef run() {
          // create folder
          NodeRef testFolder = recordFolderService.createRecordFolder(
            rmContainer,
        ...
        public void test(NodeRef testFolder) throws Exception {
          // take a look at the move capability
          Capability moveCapability = capabilityService.getCapability(
            "MoveRecordFolder"
          );
          assertEquals(
            AccessDecisionVoter.ACCESS_DENIED,
            moveCapability.evaluate(testFolder, destination)
          );
        }
      }
    );
    ...
    doTestInTransaction(
      new Test<NodeRef>() {
        ...
        public NodeRef run() throws Exception {
          ...
          return fileFolderService
            .move(testFolder, destination, null)
            .getNodeRef();
        }
    ...
    final NodeRef destination = doTestInTransaction(
      new Test<NodeRef>() {
        ...
        public NodeRef run() {
          ...
          return filePlanService.createRecordCategory(
            filePlan,
            GUID.generate()
          );
        }
  ...
  private NodeRef createRecordCategory(boolean recordLevel) {
    ...
    DispositionSchedule dis = utils.createBasicDispositionSchedule(
      rc,
      GUID.generate(),
      GUID.generate(),
      recordLevel,

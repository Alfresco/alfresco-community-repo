...
public class RecordsManagementPermissionPostProcessor extends PermissionPostProcessorBaseImpl {
    ...
    /**
     * @see
     *     org.alfresco.repo.security.permissions.processor.PermissionPostProcessor#process(AccessStatus,
     *     NodeRef, String, List, List)
     */
    @Override
    public AccessStatus process(
        ...
        if (AccessStatus.DENIED.equals(accessStatus)
            ...
            if (PermissionService.READ.equals(perm)
                    || isPermissionContained(perm, configuredReadPermissions)) {
            ...
            // if write denied on rm artifact
                    ...
                    || isPermissionContained(perm, configuredFilePermissions)) {
    ...
    private boolean isPermissionContained(String perm, List<String> configuredPermissions) {
        ...
        for (String configuredPermission : configuredPermissions) {
            // TODO: Here we are assuming the permission name is unique across all contexts (but I
            // think we're doing that in the properties file anyway).
            PermissionReference permissionReference =
                    permissionModel.getPermissionReference(null, configuredPermission);
            for (PermissionReference granteePermission :
                    permissionModel.getGranteePermissions(permissionReference)) {

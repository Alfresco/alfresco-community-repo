...
/**
 * Spring-configured JUnit 4 test case. Uses Spring to load up a test context and runs each test
 * case in a transaction which gets rolled back. Loads up the activiti-context.xml and
...
public class ActivitiWorkflowComponentTest extends AbstractActivitiComponentTest {
    ...
    public void testDeployDefinition() throws Exception {
        ProcessDefinition defInDB =
                repo.createProcessDefinitionQuery()
        ...
        ProcessDefinition processDef =
                repo.createProcessDefinitionQuery().processDefinitionId(localDefId).singleResult();
        assertNotNull("Process Definition should have been deployed!", processDef);

        ProcessDefinition def2InDB =
                repo.createProcessDefinitionQuery()
        ...
        ProcessDefinition processDef2 =
                repo.createProcessDefinitionQuery().processDefinitionId(localDef2Id).singleResult();
    ...
    public void testUndeployDefinition() throws Exception {
        ...
        long defCount = repo.createProcessDefinitionQuery().processDefinitionId(localId).count();
        assertEquals("The deployed process definition should exist!", 1, defCount);

        workflowEngine.undeployDefinition(definition.getId());

        defCount = repo.createProcessDefinitionQuery().processDefinitionId(localId).count();
    ...
    public void testGetDefinitionById() throws Exception {
        ...
        assertEquals(
                "Start task Id does not match!", originalStartDef.getId(), resultStartDef.getId());
    ...
    public void testStartWorkflow() throws Exception {
        ...
        Execution execution =
                runtime.createExecutionQuery().executionId(executionId).singleResult();
        ...
        ScriptNode initiatorHome =
                (ScriptNode) runtime.getVariable(procInstanceId, "initiatorhome");
        ...
        String defaultSetVariable =
                (String) taskService.getVariableLocal(task.getId(), "test_myProp");
    ...
    public void testSignal() throws Exception {
        ...
        ProcessInstance processInstance =
                runtime.startProcessInstanceById(BPMEngineRegistry.getLocalId(def.getId()));
    ...
    public void testCancelWorkflow() throws Exception {
        ...
        assertEquals(
                processInstance.getId(), BPMEngineRegistry.getLocalId(instances.get(0).getId()));

        // Call cancel method on component
        WorkflowInstance cancelledWorkflow =
                workflowEngine.cancelWorkflow(instances.get(0).getId());
    ...
    public void testGetActiveWorkflows() throws Exception {
        ...
        ProcessInstance completedInstance =
                runtime.startProcessInstanceById(activitiProcessDefinitionId);
        ProcessInstance cancelledInstance =
                runtime.startProcessInstanceById(activitiProcessDefinitionId);
        ProcessInstance deletedInstance =
                runtime.startProcessInstanceById(activitiProcessDefinitionId);
        ...
        while (isActive) {
            Execution execution =
                    runtime.createExecutionQuery().processInstanceId(completedId).singleResult();
    ...
    public void testGetWorkflowById() throws Exception {
        ...
        String globalProcessInstanceId =
    ...
    public void testGetTimers() throws Exception {
        ...
        Job timerJob =
                managementService
                        ...
                        .singleResult();
        String globalJobId =
                BPMEngineRegistry.createGlobalId(ActivitiConstants.ENGINE_ID, timerJob.getId());
        ...
        String expectedTimerPathId =
                BPMEngineRegistry.createGlobalId(
                        ActivitiConstants.ENGINE_ID, timerJob.getExecutionId());
        ...
        assertEquals(
                BPMEngineRegistry.createGlobalId(ActivitiConstants.ENGINE_ID, waitingTask.getId()),
                timer.getTask().getId());
    ...
    public void testGetWorkflowImage() {
        ...
        String worklfowId =
                BPMEngineRegistry.createGlobalId(
                        ActivitiConstants.ENGINE_ID, processInstance.getId());
        ...
        ProcessInstance processInstanceWithImage =
                runtime.startProcessInstanceById(
                        BPMEngineRegistry.getLocalId(definitionWithImage.getId()));
        String worklfowWithImageId =
                BPMEngineRegistry.createGlobalId(
                        ActivitiConstants.ENGINE_ID, processInstanceWithImage.getId());

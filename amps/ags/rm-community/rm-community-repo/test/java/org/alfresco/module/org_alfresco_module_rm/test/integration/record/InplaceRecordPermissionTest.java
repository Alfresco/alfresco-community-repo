...
public class InplaceRecordPermissionTest extends BaseRMTestCase
{
    ...
    /**
     * Given a document in a collaboration site When a user without write permissions on the document tries to declare it as a record Then the declaration fails And the document does not become a record
     */
    ...
    /**
     * Given a document in a collaboration site that is not a record And a contributor the didn't create the document When the contributor tries to declare the document as a record Then the document does not become a record
     */
    ...
    /**
     * Given a document in a collaboration site is not a record When the document is declared by a site collaborator Then the document becomes a record And the site users have the appropriate in-place permissions on the record
     */
    public void testCreateInplaceRecordFromCollabSite()
    {
        test()
                ...
                .as(dmCollaborator)
                .perform(() -> checkInPlaceAccess(dmDocument,
    ...
    public void testCreateInplaceRecordFromCollabSiteWhenContribIsCreatorOfDocument()
    {
        test()
                ...
                // And a site collaborator has filling permissions and filling capability on the record
                .as(dmCollaborator)
                .perform(() -> checkInPlaceAccess(contribDoc,
                        AccessStatus.ALLOWED, // read record permission
                        AccessStatus.ALLOWED, // filing permission
                        AccessStatus.ALLOWED, // view record capability
                        AccessStatus.ALLOWED, // edit non record metadata capability
    ...
    /**
     * Given an unfiled in-place record When the record is moved to the file plan (ie filed) Then the site users still have the appropriate in-place permissions on the record
     */
    ...
    /**
     * Given an incomplete inplace record When it is completed Then the inplace users still have access to the record And can't edit the records meta-data
     */
    ...
    /**
     * Given an inplace record ready for destruction When it is destroyed And it's metadata is maintained Then the inplace users will no longer see the record
     */
    ...
    /**
     * Given an inplace record And the collaborator has view and edit non-record capability And doesn't have edit record capability When we add edit record metadata capability to the extended writer role Then the collaborator now has edit record metadata capability
     */
    ...
    /**
     * Given an inplace record When the record is hidden Then the collaborator has no access to the record And the consumer has no access to the record And a user that is not in the site has no permissions or capabilities
     */
    ...
    /**
     * Given an inplace record When the record is rejected Then the collaborator has no access to the record And the consumer has no access to the record And a user that is not in the site has no permissions or capabilities
     */
    ...
    /**
     * Given a user is the cm:creator of a document And the user is deleted When the document is declared as a record by a manager Then it successfully becomes a record
     */
    ...
    /**
     * Given a document created by the collaborator And declared as a record by the collaborator And filed by the records manager When the records manager copies the record Then the collaborator has no access to the record copy And a site contributor has no access to the record copy And the consumer has no access to the record copy And a user that is not in the site has no access to the record copy
     */
    public void testNoPermissionsOnCopy()
    {
        test()
                ...
                .as(dmCollaborator)
                .perform(() -> checkInPlaceAccess(copiedDoc,
                        AccessStatus.ALLOWED, // read record permission
                        AccessStatus.ALLOWED, // filing permission
                        AccessStatus.ALLOWED, // view record capability
                        AccessStatus.ALLOWED, // edit non record metadata capability
                ...
                .as(dmCollaborator)
                .perform(() -> checkInPlaceAccess(copyDoc,
                        AccessStatus.DENIED, // read record permission
                        AccessStatus.DENIED, // filing permission
                        AccessStatus.DENIED, // view record capability
                        AccessStatus.DENIED, // edit non record metadata capability

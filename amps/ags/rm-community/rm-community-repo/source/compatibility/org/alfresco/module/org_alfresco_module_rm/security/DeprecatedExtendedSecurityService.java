...
public interface DeprecatedExtendedSecurityService {
     ...
     *
     * <p>As of, 2.5 this method no longer applies the extended security to parents.
     *
     * @param nodeRef node reference
     ...
     * Add extended security for the specified authorities to a node.
     *
     * <p>If specified, the read and write extended permissions are applied to all parents up to the
     * file plan as extended read. This ensures parental read, but not parental write.
     *
     * @param nodeRef node reference
     * @param readers set of authorities to add extended read permissions
     * @param writers set of authorities to add extended write permissions
     * @param applyToParents true if extended security applied to parents (read only) false
     *     otherwise.
     * @deprecated as of 2.5, because extended security is no longer applied to parents. Note that
     *     calling this method will only apply the extended security to the node and the
     *     applyToParents parameter value will be ignored.
     * @see ExtendedSecurityService#set(NodeRef, Set, Set)
     */
    @Deprecated
    void addExtendedSecurity(
     ...
     * @deprecated as of 2.5, because partial removal of readers and writers from node or parents is
     *     no longer supported. Note that calling this method will now remove all extended security
     *     from the node and never applied to parents.
     ...
     * Remove the extended security for the specified authorities from a node.
     *
     * <p>If specified, extended security will also be removed from the parent hierarchy.(read
     * only). Note that extended security is records as a reference count, so security will only be
     * utterly removed from the parent hierarchy if all references to the authority are removed.
     *
     * @param nodeRef node reference
     * @param readers set of authorities to remove as extended readers
     * @param writers set of authorities to remove as extedned writers
     * @param applyToParents true if removal of extended security is applied to parent hierarchy
     ...
     *
     * @param nodeRef node reference
     * @param applyToParents if true then apply removal to parent hierarchy (read only) false
     *     otherwise.
     * @deprecated as of 2.5, because partial removal of readers and writers from node or parents is
     *     no longer supported. Note that calling this method will now remove all extended security
     *     from the node and never applied to parents.

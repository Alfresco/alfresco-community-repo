...
public abstract class ActionExecuterAbstractBase
  ...
  private static Log logger = LogFactory.getLog(
    ActionExecuterAbstractBase.class
  );
  ...
  public ActionDefinition getActionDefinition() {
    if (this.actionDefinition == null) {
      ...
      ((ActionDefinitionImpl) this.actionDefinition).setDescriptionKey(
          getDescriptionKey()
        );
      ((ActionDefinitionImpl) this.actionDefinition).setTrackStatus(
          getTrackStatus()
        );
      ((ActionDefinitionImpl) this.actionDefinition).setAdhocPropertiesAllowed(
          getAdhocPropertiesAllowed()
        );
      ((ActionDefinitionImpl) this.actionDefinition).setRuleActionExecutor(
          this.name
        );
      (
        (ActionDefinitionImpl) this.actionDefinition
      ).setLocalizedParameterDefinitions(getLocalizedParameterDefinitions());
      ((ActionDefinitionImpl) this.actionDefinition).setApplicableTypes(
  ...
  public void execute(Action action, NodeRef actionedUponNodeRef) {
    if (
      ...
      throw new IllegalStateException(
        "Actions invariably access the repository.  Doing so without a transaction is not recommended."
      );
    }
    ...
    if (
      ...
      nodeIsLockedForThisUser =
        lockService.isLockedAndReadOnly(actionedUponNodeRef);
    }

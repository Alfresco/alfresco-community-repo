...
/**
 * A unit test that verifies it's possible to start up the main Alfresco context, stop it, start a
 * different context, stop that, and finally return to the main context. To do a basic check of if
 * the repository works, use {@link RepositoryStartupTest}. This test allows you to verify that your
 * context is able to shut down cleanly, as well as that the minimal context is able to do the same.
 *
 * <p>As this test opens and closes lots of contexts, it is rather slow....
 *
 * @author Nick Burch
 */
...
public class RepositoryStartStopTest extends TestCase {
    ...
    /**
     * Using a minimal no-autostart context: Test that we can bring up and close down a context
     * twice without error, using it when running.
     */
    ...
    /**
     * Using a full autostarting context: Test that we can bring up and close down a context twice
     * without error, using it when running.
     */
    ...
    /**
     * Tests that we can open a context, use it, close it, and open a different one. Does this by
     * opening and closing contexts 4 different times, sometimes full ones, sometimes minimal ones
     * with no autostart.
     */
    ...
    public void doTestBasicWriteOperations(ApplicationContext ctx) throws Exception {
        ...
        // Check it looks fine
        assertFalse(
                "The transaction is read-only - further unit tests are pointless.",
                transactionService.isReadOnly());

        // A basic write operation on a node
        RetryingTransactionCallback<Void> addPropertyCallback =

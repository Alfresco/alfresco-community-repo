 ...
 *
 * <p>The user name and password are picked up from login.
 *
 * <p>The other configurable parameters are: realm - the authentication realm if required, and the
 * entry name to use from the login context.
 *
 * <p>You will need to be familiar with the JAAS authentication process to set this up.
 *
 * <p>In summary you will need to configure java.security (in the lib/security directory of the jre
 * you are using) to find a jaas configuration.
 *
 * <p>This entry could be used if you want to put the login configuration in the same place (in the
 * lib/security directory of the jre you are using) <code>
 * login.config.url.1=file:${java.home}/lib/security/java.login.config
 * </code> Example configuration entries for Kerberos would be: <code>
 * Alfresco {
 ...
 *    com.sun.security.auth.module.Krb5LoginModule sufficient;
 * };
 * </code> This sets up authentication using Kerberos for Alfresco and some defaults that would use
 * the same mechanism if sasl failed for example.
 *
 * <p>You could use kerberos and LDAP combined against an Active Directory server.
 *
 * @author Andy Hind
 */
public class JAASAuthenticationComponent extends AbstractAuthenticationComponent {
    ...
    protected void authenticateImpl(String userName, char[] password)
        ...
        try {
            lc =
                    new LoginContext(
                            jaasConfigEntryName, new SimpleCallback(userName, realm, password));

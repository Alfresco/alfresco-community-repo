...
/**
 * Test class which ensures that workflows created by {@link InvitationService} are correctly
...
public class InvitationCleanupTest {
    ...
    public static void initStaticData() throws Exception {
        INVITATION_SERVICE =
                APP_CONTEXT_INIT
                        ...
                        .getBean("InvitationService", InvitationService.class);
        SITE_SERVICE =
        ...
        NODE_ARCHIVE_SERVICE =
                APP_CONTEXT_INIT
                        .getApplicationContext()
                        .getBean("nodeArchiveService", NodeArchiveService.class);
    }

    /** See CLOUD-1824 for details on bug &amp; ALF-11872 for details on a related, older bug. */
    @Test
    public void pendingJoinRequestsToModeratedSitesShouldDisappearOnSiteDeletion()
            throws Exception {
                        ...
                        return TRANSACTION_HELPER.doInTransaction(
                                new RetryingTransactionCallback<Void>() {
                                    public Void execute() throws Throwable {
                                        INVITATION_SERVICE.inviteModerated(
                                                "Let me in!",
                ...
                new RunAsWork<Void>() {
                    ...
                    public Void doWork() throws Exception {
                        TRANSACTION_HELPER.doInTransaction(
                                new RetryingTransactionCallback<Void>() {
                                    public Void execute() throws Throwable {
                                        // Sanity check: UserOne should now have a task assigned to
                                        // them
                                ...
                                TRANSACTION_HELPER.doInTransaction(
                                        new RetryingTransactionCallback<NodeRef>() {
                                            public NodeRef execute() throws Throwable {
                                                SiteInfo siteInfo =
                                                        SITE_SERVICE.getSite(siteShortName);
                                                // UserOne ignores the task and instead deletes the
                                                // site.
                                                SITE_SERVICE.deleteSite(siteShortName);

                                                return NODE_ARCHIVE_SERVICE.getArchivedNode(
                                                        siteInfo.getNodeRef());
                                            }
                                        });
                        ...
                        // The pending inviations are deleted asynchronously and so we must wait for
                        // that deletion to complete.
    ...
    private List<WorkflowTask> assertUserHasTasks(final String username, final int number) {
        List<WorkflowTask> tasks =
                WORKFLOW_SERVICE.getAssignedTasks(username, WorkflowTaskState.IN_PROGRESS);

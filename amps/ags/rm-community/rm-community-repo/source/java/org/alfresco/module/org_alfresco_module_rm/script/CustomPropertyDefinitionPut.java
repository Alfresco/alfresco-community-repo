...
/**
 * Implementation for Java backed webscript to update RM custom property definitions in the custom
 * model.
 *
...
public class CustomPropertyDefinitionPut extends BaseCustomPropertyWebScript {
     ...
     */
    protected Map<String, Object> handlePropertyDefinitionUpdate(
            WebScriptRequest req, JSONObject json) throws JSONException, CustomMetadataException {
    ...
    /**
     * If label has a non-null value, it is set on the property def. If constraintRef has a non-null
     * value, it is set on this propDef. If constraintRef has a null value, all constraints for that
     * propDef are removed.
     *
     * @param params
     * @return
    ...
    protected QName updatePropertyDefinition(Map<String, Serializable> params)
        ...
        if (propQName == null) {
            throw new WebScriptException(
                    Status.STATUS_NOT_FOUND, "Could not find property definition for: " + propId);
        }

        if (params.containsKey(PARAM_CONSTRAINT_REF)) {
            String constraintRef = (String) params.get(PARAM_CONSTRAINT_REF);
            List<ConstraintDefinition> constraints =
                    rmAdminService.getCustomPropertyDefinitions().get(propQName).getConstraints();
            ...
            if (constraintRef == null) {
                ...
                for (ConstraintDefinition constraintDefinition : constraints) {
                    if (constraintDefinition
                            .getConstraint()
                            .getShortName()
                            .equalsIgnoreCase(constraintRef)) {
                        exists = true;
                        break;
                    }
                }
                if (!exists) {
                    QName constraintRefQName =
                            QName.createQName(constraintRef, getNamespaceService());
                    result =
                            rmAdminService.setCustomPropertyDefinitionConstraint(
                                    propQName, constraintRefQName);
                    updated = true;
                }
            }
        }
        ...
        if (params.containsKey(PARAM_LABEL)) {
            ...
            try {
                ...
                if (!updated) {
                    String propIdAsString =
                            rmAdminService
                                    .getQNameForClientId(label)

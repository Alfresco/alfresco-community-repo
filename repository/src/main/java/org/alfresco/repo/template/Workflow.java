...
public class Workflow extends BaseTemplateProcessorExtension {
  ...
  public List<WorkflowTaskItem> getAssignedTasks() {
    ...
    List<WorkflowTask> tasks = getWorkflowService()
      .getAssignedTasks(
  ...
  private List<WorkflowTaskItem> convertTasks(List<WorkflowTask> tasks) {
    List<WorkflowTaskItem> items = new ArrayList<WorkflowTaskItem>(
      tasks.size()
    );
    for (WorkflowTask task : tasks) {
      items.add(
        new WorkflowTaskItem(this.services, getTemplateImageResolver(), task)
      );
    }
  ...
  public static class WorkflowTaskItem
    ...
    public TemplateNode getInitiator() {
      return new TemplateNode(
        this.task.path.instance.initiator,
        this.services,
        this.resolver
      );
    }
    ...
    public List<TemplateContent> getPackageResources() {
      ...
      List<NodeRef> contents =
        this.services.getWorkflowService().getPackageContents(this.task.id);
      ...
      for (NodeRef nodeRef : contents) {
        ...
        if (ddService.getType(type) != null) {
          ...
          if (
            ddService.isSubClass(type, ContentModel.TYPE_CONTENT) ||
            ApplicationModel.TYPE_FILELINK.equals(type)
          ) {
            resources.add(
              new TemplateNode(nodeRef, this.services, this.resolver)
            );
          }
        }
      }
    ...
    public String getOutcome() {
      ...
      if (task.state.equals(WorkflowTaskState.COMPLETED)) {
        ...
        String transition = (String) task.properties.get(
          WorkflowModel.PROP_OUTCOME
        );
        if (transition != null) {
          WorkflowTransition[] transitions =
            this.task.definition.node.transitions;
      ...
      if (this.properties == null) {
        ...
        for (QName qname : this.task.properties.keySet()) {
          Serializable value = converter.convertProperty(
            this.task.properties.get(qname),
            qname,
            this.services,
            this.resolver
          );

...
public class JscriptWorkflowPath implements Serializable {
  ...
  public Scriptable getTasks() {
    ...
    for (WorkflowTask cmrTask : cmrTasks) {
      tasks.add(
        new JscriptWorkflowTask(cmrTask, this.serviceRegistry, this.scope)
      );
    }

    Scriptable tasksScriptable = (Scriptable) new ValueConverter()
  ...
  public JscriptWorkflowPath signal(String transitionId) {
    WorkflowPath path = serviceRegistry
      .getWorkflowService()
      .signal(this.id, transitionId);

    ...
    protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
        ...
        if (id == null) {
            throw new WebScriptException(
                    Status.STATUS_BAD_REQUEST, "audit.err.entry.id.notProvided");
        }
        final List<Long> auditEntryIds = new ArrayList<Long>();

        // Need to check that the audit entry actually exists - otherwise we get into a concurrency
        // retry loop
        ...
        if (auditEntryIds.size() > 0) {
            ...
            // Not found
            throw new WebScriptException(
                    Status.STATUS_NOT_FOUND, "audit.err.entry.id.notfound", id);
        }
    }
}

...
public class ChainingUserRegistrySynchronizer
  ...
  public void setExternalUserControlSubsystemName(
    String externalUserControlSubsystemName
  ) {
  ...
  public void setSyncWhenMissingPeopleLogIn(
    boolean syncWhenMissingPeopleLogIn
  ) {
  ...
  public SynchronizeDiagnostic testSynchronize(String authenticatorName) {
    ...
    if (instanceIds.contains(authenticatorName)) {
      ...
      Date groupLastModified = groupLastModifiedMillis == -1
        ? null
        : new Date(groupLastModifiedMillis);
      Date personLastModified = personLastModifiedMillis == -1
        ? null
        : new Date(personLastModifiedMillis);
  ...
  private void synchronizeInternal(
    ...
    if (ChainingUserRegistrySynchronizer.logger.isDebugEnabled()) {
      ...
      if (this.transactionService.isReadOnly()) {
        ChainingUserRegistrySynchronizer.logger.warn(
          "Unable to proceed with user registry synchronization. Repository is read only."
        );
        return;
      }
    }
    ...
    threadFactory.setNamePrefix(
      "ChainingUserRegistrySynchronizer lock refresh"
    );
    threadFactory.setThreadDaemon(true);
    ScheduledExecutorService lockRefresher = new ScheduledThreadPoolExecutor(
      1,
      threadFactory
    );
    ...
    try {
              ...
              .doInTransaction(
                new RetryingTransactionCallback<Object>() {
                  public Object execute() throws Throwable {
                    ChainingUserRegistrySynchronizer.this.jobLockService.refreshLock(
                        token,
                        ChainingUserRegistrySynchronizer.LOCK_QNAME,
                        ChainingUserRegistrySynchronizer.LOCK_TTL
                      );
      ...
      for (String id : instanceIds) {
        ...
        if (
          !(plugin instanceof ActivateableBean) ||
      ...
      for (String id : instanceIds) {
        ...
        if (plugin != null) {
          ...
          if (ChainingUserRegistrySynchronizer.logger.isDebugEnabled()) {
            mbeanServer =
              (MBeanServerConnection) getApplicationContext()
            ...
            try {
              ...
              if (mbeanServer != null && mbeanServer.isRegistered(name)) {
                ...
                for (MBeanAttributeInfo attribute : attributes) {
                  Object value = mbeanServer.getAttribute(
                    name,
                    attribute.getName()
                  );
                  ChainingUserRegistrySynchronizer.logger.debug(
          ...
          if (
            isFullSync &&
            ChainingUserRegistrySynchronizer.logger.isWarnEnabled()
          ) {
            if (allowDeletions) {
              ChainingUserRegistrySynchronizer.logger.warn(
      ...
      if (lockToken != null) {
        ...
        this.transactionService.getRetryingTransactionHelper()
          .doInTransaction(
            new RetryingTransactionCallback<Object>() {
              public Object execute() throws Throwable {
                ChainingUserRegistrySynchronizer.this.jobLockService.releaseLock(
                    token,
                    ChainingUserRegistrySynchronizer.LOCK_QNAME
                  );
                return null;
  ...
  public Set<QName> getPersonMappedProperties(String username) {
    Set<String> authorityZones =
      this.authorityService.getAuthorityZones(username);
    ...
    for (String id : instanceIds) {
      ...
      try {
        ApplicationContext context =
          this.applicationContextManager.getApplicationContext(id);
        UserRegistry plugin = (UserRegistry) context.getBean(
          this.sourceBeanName
        );
        if (
          !(plugin instanceof ActivateableBean) ||
    ...
    if (
      ...
      if (this.syncWhenMissingPeopleLogIn) {
        try {
          ...
          ChainingUserRegistrySynchronizer.logger.warn(
            "User authenticated but failed to sync with user registry",
            e
          );
        }
      ...
      if (
        this.autoCreatePeopleOnLogin && this.personService.createMissingPeople()
      ) {
  ...
  private enum SyncProcess {
    ...
    GROUP_CREATION_AND_ASSOCIATION_DELETION(
  ...
  private void syncWithPlugin(
    ...
    long lastModifiedMillis = forceUpdate
      ? -1
    ...
    Date lastModified = lastModifiedMillis == -1
      ? null
    ...
    class Analyzer extends BaseBatchProcessWorker<NodeDescription> {
      ...
      public void process(NodeDescription group) throws Throwable {
        ...
        String groupName = (String) groupProperties.get(
        ...
        synchronized (this) {
          ...
          if (groupLastModified != null) {
            this.latestTime =
              Math.max(this.latestTime, groupLastModified.getTime());
          }
        }
      }
      ...
      private Set<String> cacheContainedAuthorities(String groupName) {
        ...
        children =
          ChainingUserRegistrySynchronizer.this.authorityService.getContainedAuthorities(
              null,
              groupName,
              true
            );
      ...
      private synchronized void updateGroup(
        ...
        String groupDisplayName = (String) groupProperties.get(
        ...
        if (existed) {
          ...
          for (String child : new TreeSet<String>(
            getContainedAuthorities(groupName)
          )) {
            if (AuthorityType.getAuthorityType(child) == AuthorityType.USER) {
      ...
      private void recordParentAssociationDeletion(
        String child,
        String parent
      ) {
      ...
      private void recordParentAssociationCreation(
        String child,
        String parent
      ) {
        Map<String, Set<String>> parentAssocs = AuthorityType.getAuthorityType(
            child
          ) ==
          AuthorityType.USER
      ...
      private boolean validateAuthorityChildren(
      ...
      private void logRetainParentAssociations(
        ...
        Iterator<Map.Entry<String, Set<String>>> i = parentAssocs
          .entrySet()
          .iterator();
      ...
      private void processGroups(
        ...
        if (
          (syncDelete || !groupsToCreate.isEmpty()) &&
              ...
              new RetryingTransactionCallback<Void>() {
                public Void execute() throws Throwable {
                  allZonePersons.addAll(
                    ChainingUserRegistrySynchronizer.this.authorityService.getAllAuthoritiesInZone(
                        zoneId,
                        AuthorityType.USER
                      )
                  );
          ...
          if (!this.groupParentAssocsToDelete.isEmpty()) {
            ...
            BatchProcessor<Map.Entry<String, Set<String>>> groupCreator = new BatchProcessor<Map.Entry<String, Set<String>>>(
              SyncProcess.GROUP_CREATION_AND_ASSOCIATION_DELETION.getTitle(
                zone
              ),
              ChainingUserRegistrySynchronizer.this.transactionService.getRetryingTransactionHelper(),
              ...
              new BaseBatchProcessWorker<Map.Entry<String, Set<String>>>() {
                ...
                public void process(Map.Entry<String, Set<String>> entry)
                  ...
                  if (groupDisplayName != null) {
                    ...
                    if (
                      ChainingUserRegistrySynchronizer.logger.isDebugEnabled()
                    ) {
                      ChainingUserRegistrySynchronizer.logger.debug(
      ...
      private void finalizeAssociations(
        UserRegistry userRegistry,
        boolean splitTxns
      ) {
        ...
        this.personParentAssocsToDelete.keySet()
          .removeAll(this.personsProcessed);
      ...
      private void maintainAssociationDeletions(String authorityName) {
        ...
        Set<String> parentsToDelete = isPerson
        ...
        if (parents != null && !parents.isEmpty()) {
          ...
          try {
            ...
            throw new ConcurrencyFailureException(
              "Forcing batch retry for unknown authority",
              e
            );
            ...
            throw new ConcurrencyFailureException(
              "Forcing batch retry for invalid node",
              e
            );
          }
        }
    ...
    lastModified =
      lastModifiedMillis == -1 ? null : new Date(lastModifiedMillis);
    if (ChainingUserRegistrySynchronizer.logger.isInfoEnabled()) {
      if (lastModified == null) {
        ChainingUserRegistrySynchronizer.logger.info(
    ...
    class PersonWorker extends BaseBatchProcessWorker<NodeDescription> {
      ...
      public void process(NodeDescription person) throws Throwable {
        ...
        String personName = personProperties
        ...
        if (
          Boolean.parseBoolean(
            ChainingUserRegistrySynchronizer.this.externalUserControl
          ) &&
          ChainingUserRegistrySynchronizer.this.externalUserControlSubsystemName.equals(
              zone
            ) &&
          ...
          try {
            ...
            if (ldapUserRegistry.getUserAccountStatusInterpreter() != null) {
              QName propertyNameToCheck = QName.createQName(
                NamespaceService.CONTENT_MODEL_1_0_URI,
                "userAccountStatusProperty"
              );

              if (
                personProperties.get(propertyNameToCheck) != null ||
                ldapUserRegistry
                  .getUserAccountStatusInterpreter()
                  .acceptsNullArgument()
              ) {
                boolean isUserAccountDisabled = ldapUserRegistry
                  .getUserAccountStatusInterpreter()
        ...
        synchronized (this) {
          ...
          if (personLastModified != null) {
            this.latestTime =
              Math.max(this.latestTime, personLastModified.getTime());
          }
        }
      }
    }
    ...
    final String statusMessage = I18NUtil.getMessage(
      "synchronization.summary.status",
      statusParams
    );

    if (ChainingUserRegistrySynchronizer.logger.isInfoEnabled()) {
      ChainingUserRegistrySynchronizer.logger.info(
        "Finished synchronizing users and groups with user registry '" +
        zone +
        "'"
      );
  ...
  private void updateAuthorityZones(
    ...
    if (!zonesToRemove.isEmpty()) {
      this.authorityService.removeAuthorityFromZones(
          authorityName,
          zonesToRemove
        );
    }
  ...
  protected abstract class BaseBatchProcessWorker<T>
    implements BatchProcessWorker<T> {
  ...
  private void notifySyncStart(final Set<String> toSync) {
    ...
    this.applicationEventPublisher.publishEvent(
        new SynchronizeStartEvent(this)
      );
    this.transactionService.getRetryingTransactionHelper()
      .doInTransaction(
        new RetryingTransactionCallback<Void>() {
  ...
  private void notifySyncEnd(final Exception e) {
    this.applicationEventPublisher.publishEvent(
        new SynchronizeEndEvent(this, e)
      );
    this.transactionService.getRetryingTransactionHelper()
      .doInTransaction(
        new RetryingTransactionCallback<Void>() {
  ...
  private void notifySyncDirectoryStart(
    final String zoneId,
    final String[] batchProcessNames
  ) {
    this.applicationEventPublisher.publishEvent(
  ...
  private void notifySyncDirectoryEnd(
    final String zoneId,
    final String statusMessage
  ) {
    this.applicationEventPublisher.publishEvent(
        new SynchronizeDirectoryEndEvent(this, zoneId)
      );
  ...
  public Date getSyncStartTime() {
    ...
    Date lastUserUpdate = start.longValue() == -1
      ? null
      : new Date(start.longValue());
  ...
  public Date getSynchronizationLastUserUpdateTime(String id) {
    String zoneId = AuthorityService.ZONE_AUTH_EXT_PREFIX + id;
    long time = getMostRecentUpdateTime(
      ChainingUserRegistrySynchronizer.PERSON_LAST_MODIFIED_ATTRIBUTE,
      zoneId,
      false
    );

...
public class ADMPermissionsDaoComponentImpl
  ...
  @Override
  protected CreationReport createAccessControlList(
    NodeRef nodeRef,
    boolean inherit,
    Acl existing
  ) {
    if (existing == null) {
      ...
      changes.add(
        new AclDAOImpl.AclChangeImpl(null, id, null, acl.getAclType())
      );
      changes.addAll(
        getACLDAO(nodeRef).setInheritanceForChildren(nodeRef, id, null)
      );
    ...
    switch (existing.getAclType()) {
      case OLD:
        throw new IllegalStateException(
          "Can not mix old and new style permissions"
        );
      case DEFINING:
      ...
      case SHARED:
        ...
        changes.addAll(
          aclDaoComponent.mergeInheritedAccessControlList(existing.getId(), id)
        );
        ...
        changes.addAll(
          getACLDAO(nodeRef)
            .setInheritanceForChildren(
              nodeRef,
              id,
              aclDaoComponent.getInheritedAccessControlList(existing.getId())
            )
        );
      ...
      case LAYERED:
        throw new IllegalStateException(
          "Layering is not supported for DM permissions"
        );
      default:
        throw new IllegalStateException(
          "Unknown type " + existing.getAclType()
        );
    }
  }
  ...
  public void deletePermissions(NodeRef nodeRef) {
    ...
    if (acl != null) {
      switch (acl.getAclType()) {
        ...
        case DEFINING:
          if (acl.getInherits()) {
            ...
            if (inheritsFrom != null) {
              inheritsFrom =
                aclDaoComponent.getInheritedAccessControlList(inheritsFrom);
              getACLDAO(nodeRef)
                .setAccessControlList(
                  nodeRef,
                  aclDaoComponent.getAcl(inheritsFrom)
                );
              List<AclChange> changes = new ArrayList<AclChange>();
              changes.addAll(
                getACLDAO(nodeRef)
                  .setInheritanceForChildren(
                    nodeRef,
                    inheritsFrom,
                    aclDaoComponent.getInheritedAccessControlList(acl.getId())
                  )
              );
        ...
        case FIXED:
          throw new IllegalStateException(
            "Delete not supported for fixed permissions"
          );
        case GLOBAL:
          throw new IllegalStateException(
            "Delete not supported for global permissions"
          );
        case SHARED:
          // nothing to do
          return;
  ...
  private void replaceWithCleanDefiningAcl(NodeRef nodeRef, Acl acl) {
    ...
    changes.addAll(
      getACLDAO(nodeRef)
        .setInheritanceForChildren(nodeRef, id, acl.getInheritedAcl())
    );
    getACLDAO(nodeRef).updateChangedAcls(nodeRef, changes);
    aclDaoComponent.deleteAccessControlList(acl.getId());
  }

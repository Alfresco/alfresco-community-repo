...
public class CMISStrictDictionaryService extends CMISAbstractDictionaryService {
  ...
  public void init() {
    this.coreDictionaryInitializer =
      new DictionaryInitializer() {
        @Override
        public Collection<AbstractTypeDefinitionWrapper> createDefinitions(
          CMISDictionaryRegistry cmisRegistry
        ) {
          Collection<AbstractTypeDefinitionWrapper> ret = new LinkedList<>();
          ret.addAll(
            createTypeDefs(cmisRegistry, dictionaryService.getAllTypes(true))
          );

          Collection<QName> assocQNames = dictionaryService.getAllAssociations(
            true
          );

          // register base type
          String typeId = cmisMapping.getCmisTypeId(
          ...
          if (classDef == null) {
            if (assocQNames.size() != 0) {
              logger.warn(
                "Unexpected - no class for " +
                CMISMapping.RELATIONSHIP_QNAME +
                " - cannot create assocDefs for: " +
                assocQNames
              );
            }
          } else {
            RelationshipTypeDefintionWrapper objectTypeDef = new RelationshipTypeDefintionWrapper(
              cmisMapping,
              accessorMapping,
          ...
          ret.addAll(
            createTypeDefs(cmisRegistry, dictionaryService.getAllAspects(true))
          );
          return ret;
        }
        ...
        public Collection<AbstractTypeDefinitionWrapper> createDefinitions(
          ...
          for (TypeDefinition typeDef : model.getTypes()) {
            ...
            AbstractTypeDefinitionWrapper objectTypeDef = createTypeDef(
              classQName
            );
          ...
          for (AssociationDefinition assocDef : model.getAssociations()) {
            ...
            RelationshipTypeDefintionWrapper assocTypeDef = createAssocDef(
              classQName
            );
    ...
    this.tenantDictionaryInitializer =
      new DictionaryInitializer() {
        @Override
        public Collection<AbstractTypeDefinitionWrapper> createDefinitions(
          CMISDictionaryRegistry cmisRegistry
          ...
          Collection<AbstractTypeDefinitionWrapper> ret = new LinkedList<>();
          ret.addAll(
            createTypeDefs(cmisRegistry, dictionaryService.getAllTypes(false))
          );
          ret.addAll(
            createAssocDefs(
              cmisRegistry,
              dictionaryService.getAllAssociations(false)
            )
          );
          ret.addAll(
            createTypeDefs(cmisRegistry, dictionaryService.getAllAspects(false))
          );
          return ret;
        }
  ...
  private AbstractTypeDefinitionWrapper createTypeDef(QName classQName) {
    ...
    if (!cmisMapping.isRemappedType(classQName)) {
      ...
      if (cmisMapping.isValidCmisDocument(classQName)) {
        typeId =
          cmisMapping.getCmisTypeId(BaseTypeId.CMIS_DOCUMENT, classQName);
        objectTypeDef =
          new DocumentTypeDefinitionWrapper(
            cmisMapping,
            accessorMapping,
            luceneBuilderMapping,
            typeId,
        ...
        objectTypeDef =
          new FolderTypeDefintionWrapper(
            cmisMapping,
            accessorMapping,
            luceneBuilderMapping,
            typeId,
        ...
        cmisMapping.getCmisVersion().equals(CmisVersion.CMIS_1_1) &&
        cmisMapping.isValidCmisSecondaryType(classQName)
      ) {
        typeId =
          cmisMapping.getCmisTypeId(BaseTypeId.CMIS_SECONDARY, classQName);
        objectTypeDef =
          new SecondaryTypeDefinitionWrapper(
            cmisMapping,
            accessorMapping,
            luceneBuilderMapping,
            typeId,
        ...
        objectTypeDef =
          new PolicyTypeDefintionWrapper(
            cmisMapping,
            accessorMapping,
            luceneBuilderMapping,
            typeId,
  ...
  private Collection<AbstractTypeDefinitionWrapper> createTypeDefs(
  ...
  private RelationshipTypeDefintionWrapper createAssocDef(QName classQName) {
    ...
    if (cmisMapping.isValidCmisRelationship(classQName)) {
      // create appropriate kind of type definition
      AssociationDefinition assocDef = dictionaryService.getAssociation(
        classQName
      );
      String typeId = cmisMapping.getCmisTypeId(
        BaseTypeId.CMIS_RELATIONSHIP,
      ...
      assocTypeDef =
        new RelationshipTypeDefintionWrapper(
          cmisMapping,
          accessorMapping,
          luceneBuilderMapping,
          typeId,
   ...
   */
  private Collection<RelationshipTypeDefintionWrapper> createAssocDefs(
    CMISDictionaryRegistry registry,
    Collection<QName> classQNames
  ) {

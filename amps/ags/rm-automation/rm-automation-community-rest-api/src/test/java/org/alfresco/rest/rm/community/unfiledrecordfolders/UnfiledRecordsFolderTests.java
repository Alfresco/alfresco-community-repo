...
public class UnfiledRecordsFolderTests extends BaseRMRestTest {
    ...
    public static final String ELECTRONIC_RECORD_NAME = getRandomName(
        "Record electronic"
    );
    public static final String NONELECTRONIC_RECORD_NAME = getRandomName(
        "Record nonelectronic"
    );
    ...
    public void preconditionUnfiledRecordsFolderTests() {
        ...
        rootUnfiledRecordFolder =
            createUnfiledContainerChild(
                UNFILED_RECORDS_CONTAINER_ALIAS,
                getRandomName("RootUnfiledRecFolder"),
                UNFILED_RECORD_FOLDER_TYPE
            );
        unfiledRecord =
            createUnfiledContainerChild(
                UNFILED_RECORDS_CONTAINER_ALIAS,
                getRandomName("Unfiled Record"),
                CONTENT_TYPE
            );
    }
        ...
        return new String[][] {
            {
                getRestAPIFactory()
                    .getFilePlansAPI()
                    .getFilePlan(FILE_PLAN_ALIAS)
                    .getId(),
            },
            {
                getRestAPIFactory()
                    .getUnfiledContainersAPI()
                    .getUnfiledContainer(UNFILED_RECORDS_CONTAINER_ALIAS)
                    .getId(),
            },
            {
                getRestAPIFactory()
                    .getTransferContainerAPI()
                    .getTransferContainer(TRANSFERS_ALIAS)
                    .getId(),
            },
    ...
    public void createUnfiledRecordFolderWithRelativePath(String nodeType) {
        ...
        UnfiledContainerChild unfiledChildModel = UnfiledContainerChild
            .builder()
        ...
        UnfiledContainerChild unfiledRecordFolderChild = getRestAPIFactory()
            .getUnfiledRecordFoldersAPI()
            .createUnfiledRecordFolderChild(
                unfiledChildModel,
                rootUnfiledRecordFolder.getId(),
                "include=" + PATH
            );
        ...
        assertTrue(
            unfiledRecordFolderChild.getPath().getName().contains(relativePath)
        );
        ...
        assertEquals(
            getRestAPIFactory()
                .getUnfiledRecordFoldersAPI()
                .getUnfiledRecordFolder(unfiledRecordFolderChild.getParentId())
                .getNodeType(),
            UNFILED_RECORD_FOLDER_TYPE
        );
        ...
        // The record folder to be created
        UnfiledContainerChild newUnfiledFolderModel = UnfiledContainerChild
            .builder()
            .name(getRandomName("UnfiledRecordFolder"))
            .nodeType(nodeType)
        ...
        UnfiledContainerChild newUnfiledRecordFolderChild = getRestAPIFactory()
            .getUnfiledRecordFoldersAPI()
            .createUnfiledRecordFolderChild(
                newUnfiledFolderModel,
                rootUnfiledRecordFolder.getId(),
                "include=" + PATH
            );
        ...
        assertFalse(
            getRestAPIFactory()
                ...
                .getUnfiledRecordFolder(
                    newUnfiledRecordFolderChild.getParentId()
                )
                .equals(UNFILED_RECORD_FOLDER_TYPE)
        );
        // Verify the returned node type
    ...
    public void createInvalidUnfiledChildren(String filePlanComponentType) {
        ...
        UnfiledContainerChild unfiledFolderModel = createUnfiledContainerChildModel(
            getRandomName("UnfiledRecFolder"),
     ...
     */
    @Test(
        description = "Child unfiled records folder can be created in a parent unfiled records folder"
    )
    public void childUnfiledRecordsFolderCanBeCreated() {
        String unfiledParentFolderName =
            "UnfiledParentFolder" + getRandomAlphanumeric();
        String unfiledChildFolderName =
            "UnfiledChildFolder " + getRandomAlphanumeric();

        // No need for fine control, create it using utility function
        UnfiledContainerChild unfiledParentFolder = createUnfiledRecordsFolderChild(
            rootUnfiledRecordFolder.getId(),
            unfiledParentFolderName,
            UNFILED_RECORD_FOLDER_TYPE
        );
        assertEquals(unfiledParentFolderName, unfiledParentFolder.getName());

        // Build the unfiled records folder properties
        UnfiledContainerChild unfiledChildFolderModel = UnfiledContainerChild
            .builder()
            ...
            .properties(
                UnfiledContainerChildProperties
                    .builder()
                    .title(
                        FilePlanComponentsUtil.TITLE_PREFIX +
                        unfiledChildFolderName
                    )
                    .description(
                        FilePlanComponentsUtil.DESCRIPTION_PREFIX +
                        unfiledChildFolderName
                    )
                    .build()
            )
        ...
        UnfiledContainerChild unfiledChildFolder = getRestAPIFactory()
            .getUnfiledRecordFoldersAPI()
            .createUnfiledRecordFolderChild(
                unfiledChildFolderModel,
                unfiledParentFolder.getId()
            );
        ...
        assertEquals(
            unfiledChildFolder.getNodeType(),
            UNFILED_RECORD_FOLDER_TYPE
        );
        assertEquals(
            unfiledChildFolder.getCreatedByUser().getId(),
            getAdminUser().getUsername()
        );

        // Verify the returned file plan component properties
        UnfiledRecordFolder unfiledChildRecordFolder = getRestAPIFactory()
            .getUnfiledRecordFoldersAPI()
        ...
        assertEquals(
            unfiledChildFolderProperties.getTitle(),
            FilePlanComponentsUtil.TITLE_PREFIX + unfiledChildFolderName
        );
        assertEquals(
            unfiledChildFolderProperties.getDescription(),
            FilePlanComponentsUtil.DESCRIPTION_PREFIX + unfiledChildFolderName
        );

        // Does this child point to its parent?
        assertEquals(
            unfiledChildFolder.getParentId(),
            unfiledParentFolder.getId()
        );

        // Does child's parent point to it?
        // Perform another call as our parentFolder had been executed before childFolder existed
        UnfiledContainerChildCollection parentsChildren = getRestAPIFactory()
            .getUnfiledRecordFoldersAPI()
            .getUnfiledRecordFolderChildren(unfiledParentFolder.getId());
        assertStatusCode(OK);
        List<String> childIds = parentsChildren
            .getEntries()
            .stream()
    ...
    public void editUnfiledRecordsFolder() {
        ...
        String unfiledFolderName =
            "UnfiledFolderToModify" + getRandomAlphanumeric();

        // No need for fine control, create it using utility function
        UnfiledContainerChild unfiledFolderToModify = createUnfiledRecordsFolderChild(
            rootUnfiledRecordFolder.getId(),
            unfiledFolderName,
            UNFILED_RECORD_FOLDER_TYPE
        );
        ...
        UnfiledRecordFolder unfiledChildFolderModel = UnfiledRecordFolder
            .builder()
            ...
            .properties(
                UnfiledContainerChildProperties
                    ...
                    .title(
                        modified +
                        unfiledFolderToModify.getProperties().getTitle()
                    )
                    .description(
                        modified +
                        unfiledFolderToModify.getProperties().getDescription()
                    )
                    .build()
            )
            .build();
        ...
        UnfiledRecordFolder updatedRecordFolder = getRestAPIFactory()
            .getUnfiledRecordFoldersAPI()
            .updateUnfiledRecordFolder(
                unfiledChildFolderModel,
                unfiledFolderToModify.getId()
            );
        ...
        assertEquals(
            unfiledChildFolderModel.getName(),
            updatedRecordFolder.getName()
        );
        assertEquals(
            unfiledChildFolderModel.getProperties().getTitle(),
            updatedRecordFolder.getProperties().getTitle()
        );
        assertEquals(
            unfiledChildFolderModel.getProperties().getDescription(),
        ...
        UnfiledRecordFolder renamedUnfiledFolder = getRestAPIFactory()
            .getUnfiledRecordFoldersAPI()
            .getUnfiledRecordFolder(unfiledFolderToModify.getId());

        // Verify the returned file plan component
        assertEquals(
            modified + unfiledFolderToModify.getName(),
            renamedUnfiledFolder.getName()
        );
        assertEquals(
            modified + unfiledFolderToModify.getProperties().getTitle(),
            renamedUnfiledFolder.getProperties().getTitle()
        );
        assertEquals(
            modified + unfiledFolderToModify.getProperties().getDescription(),
            renamedUnfiledFolder.getProperties().getDescription()
        );
    }
    ...
    public void deleteUnfiledRecordsFolder() {
        String unfiledFolderName =
            "UnfiledFolderToDelete" + getRandomAlphanumeric();
        String nonElectronicRecordName =
            "NonElectronicRecord" + getRandomAlphanumeric();
        String electronicRecordName =
            "ElectronicRecord" + getRandomAlphanumeric();

        // Create unfiledFolderToDelete
        UnfiledContainerChild unfiledFolderToDelete = createUnfiledRecordsFolderChild(
            rootUnfiledRecordFolder.getId(),
            unfiledFolderName,
            UNFILED_RECORD_FOLDER_TYPE
        );
        ...
        UnfiledContainerChild nonElectronicRecord = createUnfiledRecordsFolderChild(
            unfiledFolderToDelete.getId(),
            nonElectronicRecordName,
            NON_ELECTRONIC_RECORD_TYPE
        );
        assertEquals(
            nonElectronicRecord.getParentId(),
            unfiledFolderToDelete.getId()
        );

        // Create an electronic record under unfiledFolderToDelete
        UnfiledContainerChild electronicRecord = createUnfiledRecordsFolderChild(
            unfiledFolderToDelete.getId(),
            electronicRecordName,
            CONTENT_TYPE
        );
        assertEquals(
            electronicRecord.getParentId(),
            unfiledFolderToDelete.getId()
        );
        ...
        getRestAPIFactory()
            .getUnfiledRecordFoldersAPI()
            .deleteUnfiledRecordFolder(unfiledFolderToDelete.getId());
        ...
        getRestAPIFactory()
            .getUnfiledRecordFoldersAPI()
            .getUnfiledRecordFolder(unfiledFolderToDelete.getId());
    ...
    public void deleteInvalidNodesUnfiled(String nodeId) {
        ...
        getRestAPIFactory()
            .getUnfiledRecordFoldersAPI()
            .deleteUnfiledRecordFolder(nodeId);
    ...
    public void readRecordsFromUnfiledRecordFolder() {
        unfiledRecordFolder =
            createUnfiledContainerChild(
                UNFILED_RECORDS_CONTAINER_ALIAS,
                getRandomName("UnfiledRecordFolder"),
                UNFILED_RECORD_FOLDER_TYPE
            );
        String containerId = unfiledRecordFolder.getId();

        //we have unfiled record folder
        UnfiledRecordFolderAPI unfiledRecordFoldersAPI = getRestAPIFactory()
            .getUnfiledRecordFoldersAPI();

        ArrayList<UnfiledContainerChild> children = new ArrayList<>();
        for (int i = 0; i < 5; i++) {
            // Create Electronic Records
            UnfiledContainerChild record = UnfiledContainerChild
                .builder()
                .name(ELECTRONIC_RECORD_NAME + i)
                .nodeType(CONTENT_TYPE)
                .build();
            UnfiledContainerChild child = unfiledRecordFoldersAPI.uploadRecord(
                record,
                containerId,
                createTempFile(
                    ELECTRONIC_RECORD_NAME + i,
                    ELECTRONIC_RECORD_NAME + i
                )
            );
            children.add(child);
            ...
            UnfiledContainerChild nonelectronicRecord = UnfiledContainerChild
                .builder()
                .properties(
                    UnfiledContainerChildProperties
                        .builder()
                ...
                .build();
            child =
                unfiledRecordFoldersAPI.createUnfiledRecordFolderChild(
                    nonelectronicRecord,
                    containerId
                );
            children.add(child);
        ...
        UnfiledContainerChildCollection apiChildren = (UnfiledContainerChildCollection) unfiledRecordFoldersAPI
            .getUnfiledRecordFolderChildren(containerId, "include=properties")
            .assertThat()
        ...
        apiChildren
            ...
            .forEach(c -> {
                ...
                try {
                    ...
                    UnfiledContainerChild createdComponent = children
                        .stream()
                    ...
                    assertEquals(
                        createdComponent.getName(),
                        record.getName(),
                        "The record name " +
                        record.getName() +
                        " is not equal with the record name returned when creating the record " +
                        createdComponent.getName()
                    );
                    String identifier =
                    ...
                    assertTrue(
                        createdComponent
                            .getName()
                            .contains(
                                createdComponent.getProperties().getIdentifier()
                            )
                    );
                    assertEquals(
    ...
    public void tearDown() {
        ...
        getRestAPIFactory()
            .getUnfiledRecordFoldersAPI()
            .deleteUnfiledRecordFolder(rootUnfiledRecordFolder.getId());
        getRestAPIFactory()
            .getUnfiledRecordFoldersAPI()
            .deleteUnfiledRecordFolder(unfiledRecordFolder.getId());
    }
}

...
public interface RecognizedParamsExtractor {
  ...
  default Params.RecognizedParams getRecognizedParams(WebScriptRequest req) {
    ...
    Map<String, BeanPropertiesFilter> relationFilter = getRelationFilter(
      req.getParameter(PARAM_RELATIONS)
    );
    Query whereQuery = getWhereClause(req.getParameter(PARAM_WHERE));
    Map<String, String[]> requestParams = getRequestParameters(req);
    boolean includeSource = Boolean.valueOf(
      req.getParameter(PARAM_INCLUDE_SOURCE_ENTITY)
    );

    List<String> includedFields = getIncludeClause(
      req.getParameter(PARAM_INCLUDE)
    );
    ...
    BeanPropertiesFilter filter = getFilter(
      (fields != null ? fields : properties),
      includedFields
    );
   ...
   * @return
   */
  default BeanPropertiesFilter getFilter(
    String filterParams,
    ...
    if (filterParams != null) {
      ...
      rpeLogger()
        .debug(
          "Filtering using the following properties: " + filteredProperties
        );
      BeanPropertiesFilter filter = new BeanPropertiesFilter(
        filteredProperties
      );
      return filter;
    }
  ...
  @Deprecated
  default List<String> getSelectClause(String selectParam)
   ...
   */
  @SuppressWarnings("unchecked")
  default List<String> getIncludeClause(String includeParam)
    throws InvalidArgumentException {
  ...
  default List<String> getClause(String param, String paramName) {
    ...
    try {
      ...
      if (selectedPropsTree instanceof CommonErrorNode) {
        rpeLogger()
          .debug(
            "Error parsing the " + paramName + " clause " + selectedPropsTree
          );
        throw new InvalidSelectException(paramName, selectedPropsTree);
      }
      if (
        selectedPropsTree.getChildCount() == 0 &&
        !selectedPropsTree.getText().isEmpty()
      ) {
      ...
      rpeLogger()
        .debug("Unhandled Error parsing the " + paramName + " clause: " + re);
  ...
  default List<SortColumn> getSort(String sortParams) {
    if (sortParams != null) {
      ...
      List<SortColumn> sortedColumns = new ArrayList<SortColumn>(
        st.countTokens()
      );
      while (st.hasMoreTokens()) {
        ...
        if (columnDesc.countTokens() <= 2) {
          ...
          if (columnDesc.hasMoreTokens()) {
            ...
            if (
              SortColumn.ASCENDING.equals(sortDef) ||
          ...
          throw new InvalidArgumentException(
            ...
            token +
            "', expected 'field1,field2' or 'field1 asc,field2 desc' or similar"
          );
        }
  ...
  default Paging getPaging(String skip, String maxItems) {
    ...
    try {
      ...
      String errorMsg =
        "Invalid paging parameters skipCount: " +
        skip +
        ", maxItems:" +
        maxItems;
   ...
   * ReturnAllBeanProperties class
   */
  default Map<String, BeanPropertiesFilter> getRelationFilter(
    String filterParams
  ) {
    if (filterParams != null) {
      ...
      for (String relation : relations) {
        ...
        if (bracketLocation != -1) {
          ...
          String props = relation.substring(
            bracketLocation + 1,
            relation.length() - 1
          );
  ...
  default Map<String, String[]> getRequestParameters(WebScriptRequest req) {
    if (req != null) {
      ...
      if (paramNames != null) {
        Map<String, String[]> requestParameteters = new HashMap<String, String[]>(
          paramNames.length
        );

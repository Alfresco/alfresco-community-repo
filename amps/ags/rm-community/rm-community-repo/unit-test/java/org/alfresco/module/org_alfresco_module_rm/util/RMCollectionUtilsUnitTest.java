...
public class RMCollectionUtilsUnitTest {
  ...
  public void getDuplicateElements() {
    ...
    assertEquals(
      "Failed to identify duplicate elements",
      asList("B", "A"),
      RMCollectionUtils.getDuplicateElements(l)
    );

    assertEquals(
      Collections.emptyList(),
      RMCollectionUtils.getDuplicateElements(asList("A", "B", "C"))
    );
  }
  ...
  public void elementsAsSerializableList() {
    ...
    Serializable s = RMCollectionUtils.<String, ArrayList<String>>asSerializableList(
      "one",
      "two",
      "three"
    );
    List<String> l = RMCollectionUtils.<String, ArrayList<String>>asSerializableList(
      "one",
      "two",
      "three"
    );
  ...
  public void toImmutableSet_nullElement() {
    ...
    assertEquals(
      "Unexpected handling of null input element",
      output,
      newHashSet("One", "Three")
    );
    assertEquals(
      "Input should not have been changed.",
      input,
      newHashSet("One", null, "Three")
    );
  }
}

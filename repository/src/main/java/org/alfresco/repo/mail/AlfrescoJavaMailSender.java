...
/**
 * This class extends Spring's {@link JavaMailSenderImpl} to pool the {@link Transport}s used to
 * send emails.
 *
 * <p>This is to overcome problems reported in CLOUD-313.
 *
 * @author Alex Miller
 */
public class AlfrescoJavaMailSender extends JavaMailSenderImpl {
    ...
    /**
     * {@link KeyedPoolableObjectFactory} which uses the {@link Session} returned by {@link
     * JavaMailSenderImpl#getSession()} to create a new {@link Transport}.
     */
    private final class TransportFactory implements KeyedPoolableObjectFactory {
        /**
         * Create a new {@link Transport} using the {@link Session} returned by {@link
         * JavaMailSenderImpl#getSession() getSession()}.
         *
    ...
    /**
     * Wrapper implementation of {@link Transport}, which borrows from a pool on connection, and
     * returns to the pool on close.
     *
    ...
    private static class PooledTransportWrapper extends Transport {
         ...
         */
        private PooledTransportWrapper(
                GenericKeyedObjectPool transportPool, Session session, String protocol) {
        ...
        public synchronized void close() throws MessagingException {
            ...
            try {
                ...
                throw new MessagingException(
                        "Unexpected exception returning transport to pool", error);
            } finally {
                super.close();
            }
        }
        ...
        protected boolean protocolConnect(String host, int port, String username, String password)
            ...
            try {
                ...
                throw new MessagingException(
                        "Unexpected exception borrowing connection from pool", ex);
            }
        }
    }

    private GenericKeyedObjectPool transportPool =
            new GenericKeyedObjectPool(new TransportFactory());
    ...
    /**
     * @return A new {@code PooledTransportWrapper} which borrows a pooled {@link Transport} on
     *     connect, and returns it to the pool on close.
     */
    @Override
    ...
    /**
     * Set the maximum number of active transports, managed by the pool. Use a negative value for no
     * limit. Default is -1.
     */
    ...
    /**
     * Set the maximum number of transports that can sit idle in the pool. Use a negative value for
     * no limit. Default is 8.
     */
    ...
    /**
     * Set the maximum amount of time (in milliseconds) to wait for a transport to be returned from
     * the pool. Set to 0 or less to block indefinitely.
     */
    ...
    /**
     * Set the time (in milliseconds) between runs of the idle object eviction thread. Set to
     * non-positive for no eviction. Default value is 30 seconds.
     */
     ...
     * Set to non positive prevent eviction based on idle time.
     *
     * <p>This value has no affect if timeBetweenEvictionRuns is non positive.
     *

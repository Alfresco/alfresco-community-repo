...
public class HiddenAspectTest {
    ...
    public void setup() throws SystemException, NotSupportedException {
        ...
        permissionService.setPermission(
                rootNodeRef, username, PermissionService.CREATE_CHILDREN, true);
    ...
    public void testHiddenFilesEnhancedClient() {
        ...
        try {
            ...
            children = fileFolderService.list(parent);
            assertEquals(1, children.size());
            // set hidden attribute for cifs, webdav. ftp, nfs should be able to see, other clients
            // not
            ...
            assertEquals(
                    Visibility.HiddenAttribute, hiddenAspect.getVisibility(Client.cifs, child));
            assertEquals(Visibility.NotVisible, hiddenAspect.getVisibility(Client.script, child));
            assertEquals(
                    Visibility.NotVisible, hiddenAspect.getVisibility(Client.webclient, child));
            ...
            // .DS_Store is a system path and so is visible in nfs and webdav, as a hidden file in
            // cifs and hidden to all other clients
            node =
                    fileFolderService
    ...
    public void testClientControlled() throws Exception {
        // test that a client controlled hidden node is not subject to hidden file patterns
        {
            ...
            try {
                ...
                // renaming from a hidden file pattern to a non-hidden file pattern should leave the
                // node as hidden
            ...
            // remove the client-controlled hidden aspect from the parent and check that it is no
            // longer hidden
            saveClient = FileFilterMode.setClient(Client.cmis);
            try {
        ...
        // test that a cascading hidden pattern defined in model-specific-services-content.xml
        // results in hidden files
        // (node is not client-controlled hidden)
        {
            ...
            assertTrue(
                    nodeService.hasAspect(child, ContentModel.ASPECT_INDEX_CONTROL)
                            != cmisDisableHide);
            assertTrue(
                    nodeService.hasAspect(child1, ContentModel.ASPECT_HIDDEN) != cmisDisableHide);
            assertTrue(
        ...
        try {
            ...
            NodeRef node22 =
                    fileFolderService
                            ...
                            .getNodeRef();
            NodeRef node31 =
                    fileFolderService.create(node21, ".31", ContentModel.TYPE_FOLDER).getNodeRef();
            NodeRef node41 =
                    fileFolderService
        ...
        try {
            ...
            NodeRef node11 =
                    fileFolderService
                            ...
                            .getNodeRef();
            NodeRef node12 =
                    fileFolderService.create(node, ".12", ContentModel.TYPE_CONTENT).getNodeRef();
            NodeRef node21 =
                    fileFolderService
                            .create(node11, nodeName + ".21", ContentModel.TYPE_FOLDER)
                            .getNodeRef();
    ...
    public void testHideNodeExplicit() throws Exception {
        ...
        try {
            ...
            //            assertFalse("node b should be hidden", nodeService.hasAspect(childB,

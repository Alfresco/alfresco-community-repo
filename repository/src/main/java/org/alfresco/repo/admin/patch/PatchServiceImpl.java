...
public class PatchServiceImpl implements PatchService {
  ...
  private static final String MSG_APPLYING_PATCH =
    "patch.service.applying_patch";
  ...
  private boolean validatePatchImpl(List<Patch> patches) {
    ...
    int serverSchemaVersion = descriptorService
      .getServerDescriptor()
      .getSchema();
  ...
  public boolean applyOutstandingPatches() {
    ...
    try {
      ...
      try {
        ...
        Map<String, AppliedPatch> appliedPatchesById = new HashMap<String, AppliedPatch>(
          250
        );
    ...
    try {
      ...
      try {
        ...
        if (patch.getDependsOn().isEmpty()) {
          AppliedPatch appliedPatch = appliedPatchDAO.getAppliedPatch(
            patch.getId()
          );
  ...
  private boolean applyPatchAndDependencies(
    ...
    for (Patch dependencyPatch : dependencies) {
      boolean success = applyPatchAndDependencies(
        dependencyPatch,
        appliedPatchesById
      );
      if (!success) {
  ...
  public List<AppliedPatch> getPatches(Date fromDate, Date toDate) {
    ...
    List<? extends AppliedPatch> appliedPatches = appliedPatchDAO.getAppliedPatches(
      fromDate,
      toDate
    );
    // done
  ...
  private static class PatchWork {
    ...
    private String preceededByAlternative(Patch patch) {
      ...
      for (Patch alternative : alternatives) {
        ...
        AppliedPatch appliedAlternative = appliedPatchDAO.getAppliedPatch(
          alternative.getId()
        );
        if (
            ...
            public Object execute() throws Throwable {
              ...
              if (server.length() > 64) {
                logger.error(
                  "Server version '" +
                  server +
                  "' is too long for the 'applied_to_server' column therefore patch '" +
                  patch.getId() +
              ...
              String patchDescription = I18NUtil.getMessage(

...
public class RenditionServiceImplTest extends TestCase {
  ...
  private final RenditionDefinitionPersisterImpl renditionDefinitionPersister = mock(
    RenditionDefinitionPersisterImpl.class
  );
  private RenditionServiceImpl renditionService;

  private final QName ACTION_NAME = QName.createQName(
  ...
  protected void setUp() throws Exception {
    ...
    renditionService.setRenditionDefinitionPersister(
  ...
  public void testGetRenderingEngineDefinition() throws Exception {
    ...
    when(actionService.getActionDefinition(ENGINE_NAME))
    ...
    ActionDefinition renderingDefinition = new RenderingEngineDefinitionImpl(
      ENGINE_NAME
    );
    when(actionService.getActionDefinition(ENGINE_NAME))
      .thenReturn(renderingDefinition);
    assertSame(
      renderingDefinition,
      renditionService.getRenderingEngineDefinition(ENGINE_NAME)
    );
  }
  ...
  public void testGetRenderingEngineDefinitions() throws Exception {
    ...
    assertTrue(
      "The list of rendering action definitions should be empty!",
      engineDefs.isEmpty()
    );
  ...
  public void testCreateRenditionDefinition() throws Exception {
    RenditionDefinition renderingAction = renditionService.createRenditionDefinition(
      ACTION_NAME,
      ENGINE_NAME
    );

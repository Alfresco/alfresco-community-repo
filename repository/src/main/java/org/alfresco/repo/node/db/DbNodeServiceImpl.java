...
public class DbNodeServiceImpl
  ...
  implements Extensible, NodeService {

  public static final String KEY_PENDING_DELETE_NODES =
    "DbNodeServiceImpl.pendingDeleteNodes";
   ...
   */
  public void setEnableTimestampPropagation(
    boolean enableTimestampPropagation
  ) {
  ...
  private Pair<Long, NodeRef> getNodePairNotNull(NodeRef nodeRef)
    ...
    if (unchecked == null) {
      ...
      throw new InvalidNodeRefException(
        "Node does not exist: " + nodeRef + " (status:" + nodeStatus + ")",
        nodeRef
      );
    }
    return unchecked;
  ...
  public void deleteStore(StoreRef storeRef) throws InvalidStoreRefException {
    ...
    StoreRef deletedStoreRef = new StoreRef(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
        "Marked store for deletion: " + storeRef + " --> " + deletedStoreRef
      );
    }
  }
  ...
  public NodeRef getRootNode(StoreRef storeRef)
    ...
    if (rootNodePair == null) {
      throw new InvalidStoreRefException(
        "Store does not exist: " + storeRef,
        storeRef
      );
    }
    // done
  ...
  public ChildAssociationRef createNode(
    ...
    Map<QName, Serializable> propertiesAfter = nodeDAO.getNodeProperties(
  ...
  private boolean addAspectsAndProperties(
    ...
    Set<QName> missingAspects = getMissingAspects(
      existingAspects,
      allProperties,
      classQName
    );
    ...
    Map<QName, Serializable> missingProperties = getMissingProperties(
      existingProperties,
      allClassQNames
    );
  ...
  private Set<QName> getMissingAspects(
    ...
    for (QName existingPropQName : existingProperties.keySet()) {
      PropertyDefinition existingPropDef = dictionaryService.getProperty(
        existingPropQName
      );
      if (
        existingPropDef == null ||
        !existingPropDef.getContainerClass().isAspect()
      ) {
        continue; // Property is undefined or belongs to a class
      }
      QName existingPropDefiningType = existingPropDef
        .getContainerClass()
        .getName();
  ...
  private Map<QName, Serializable> getMissingProperties(
    ...
    Map<QName, Serializable> allDefaultProperties = new HashMap<QName, Serializable>(
      17
    );
    for (QName classQName : classQNames) {
      ...
      Map<QName, Serializable> defaultProperties = getDefaultProperties(
        classQName
      );
    ...
    Map<QName, Serializable> missingProperties = new HashMap<QName, Serializable>(
  ...
  public void setChildAssociationIndex(
    ...
    // get nodes
    Pair<Long, NodeRef> parentNodePair = getNodePairNotNull(
      childAssocRef.getParentRef()
    );
    Pair<Long, NodeRef> childNodePair = getNodePairNotNull(
      childAssocRef.getChildRef()
    );
  ...
  public void setType(NodeRef nodeRef, QName typeQName)
    ...
    // Set the type
    boolean updatedNode = nodeDAO.updateNode(
      nodePair.getFirst(),
      typeQName,
      null
    );
    ...
    boolean updatedProps = addAspectsAndProperties(
      nodePair,
      typeQName,
      null,
      null,
  ...
  public void addAspect(
    ...
    if (aspectDef == null) {
      throw new InvalidAspectException(
        "The aspect is invalid: " + aspectTypeQName,
        aspectTypeQName
      );
    }
  ...
  public void removeAspect(NodeRef nodeRef, QName aspectTypeQName)
    ...
    if (aspectDef != null) {
      ...
      final List<Pair<Long, NodeRef>> nodesToDelete = new ArrayList<Pair<Long, NodeRef>>(
        5
      );
      NodeDAO.ChildAssocRefQueryCallback callback = new NodeDAO.ChildAssocRefQueryCallback() {
        ...
        public boolean handle(
          ...
          if (
            isPendingDelete(parentNodePair.getSecond()) ||
      ...
      Set<QName> assocTypeQNamesToRemove = new HashSet<QName>(
        aspectDef.getChildAssociations().keySet()
      );
      ...
      for (Map.Entry<QName, AssociationDefinition> entry : nodeAssocDefs.entrySet()) {
        ...
        Collection<Pair<Long, AssociationRef>> targetAssocRefs = nodeDAO.getTargetNodeAssocs(
          nodeId,
          assocTypeQName
        );
  ...
  private boolean isPendingDelete(NodeRef nodeRef) {
    ...
    Set<NodeRef> nodesPendingDelete = TransactionalResourceHelper.getSet(
  ...
  private void deleteNode(NodeRef nodeRef, boolean allowArchival) {
    ...
    if (nodeTypeQName.equals(ContentModel.TYPE_STOREROOT)) {
      throw new IllegalArgumentException(
        "A store root node cannot be deleted: " + nodeRef
      );
    }

    // get the primary parent-child relationship before it is gone
    Pair<Long, ChildAssociationRef> childAssocPair = nodeDAO.getPrimaryParentAssoc(
      nodeId
    );
    ...
    Set<NodeRef> nodesPendingDelete = new HashSet<NodeRef>(
      walker.getNodes(false).size()
    );
    for (VisitedNode visitedNode : walker.getNodes(true)) {
      nodesPendingDelete.add(visitedNode.nodeRef);
    }
    Set<NodeRef> nodesPendingDeleteTxn = TransactionalResourceHelper.getSet(
      KEY_PENDING_DELETE_NODES
    );
    ...
    if (!allowArchival) {
      ...
      while ((requiresDelete == null || !requiresDelete) && i.hasNext()) {
        ...
        AspectDefinition aspectDef = dictionaryService.getAspect(
          nodeAspectQName
        );
        if (aspectDef != null) {
    ...
    for (VisitedNode nodeToDelete : nodesToDelete) {
      ...
      for (Pair<Long, ChildAssociationRef> secondaryParentAssocPair : nodeToDelete.secondaryParentAssocs) {
        ...
        invokeBeforeDeleteChildAssociation(
          secondaryParentAssocPair.getSecond()
        );
      }
    ...
    for (VisitedNode nodeToDelete : nodesToDelete) {
      ...
      for (Pair<Long, AssociationRef> targetAssocPair : nodeToDelete.targetAssocs) {
        ...
        nodeDAO.removeNodeAssocs(
          Collections.singletonList(targetAssocPair.getFirst())
        );
      ...
      for (Pair<Long, AssociationRef> sourceAssocPair : nodeToDelete.sourceAssocs) {
        ...
        nodeDAO.removeNodeAssocs(
          Collections.singletonList(sourceAssocPair.getFirst())
        );
  ...
  public ChildAssociationRef addChild(
    ...
    return addChild(
      Collections.singletonList(parentRef),
      childRef,
      assocTypeQName,
      assocQName
    )
  ...
  public List<ChildAssociationRef> addChild(
    ...
    Long childNodeId = childNodePair.getFirst();
    Map<QName, Serializable> childNodeProperties = nodeDAO.getNodeProperties(
      childNodePair.getFirst()
    );
    ...
    List<ChildAssociationRef> childAssociationRefs = new ArrayList<ChildAssociationRef>(
      parentRefs.size()
    );
    List<Pair<Long, NodeRef>> parentNodePairs = new ArrayList<Pair<Long, NodeRef>>(
      parentRefs.size()
    );
  ...
  public void removeChild(NodeRef parentRef, NodeRef childRef)
    ...
    Pair<Long, ChildAssociationRef> primaryChildAssocPair = nodeDAO.getPrimaryParentAssoc(
      childNodeId
    );
    // We can shortcut if our parent is also the primary parent
    if (primaryChildAssocPair != null) {
      NodeRef primaryParentNodeRef = primaryChildAssocPair
        .getSecond()
        .getParentRef();
  ...
  public boolean removeChildAssociation(ChildAssociationRef childAssocRef) {
    ...
    Long parentNodeId = getNodePairNotNull(childAssocRef.getParentRef())
      .getFirst();
    Long childNodeId = getNodePairNotNull(childAssocRef.getChildRef())
      .getFirst();
    QName assocTypeQName = childAssocRef.getTypeQName();
    QName assocQName = childAssocRef.getQName();
   ...
   */
  private Map<QName, Serializable> getPropertiesImpl(
    Pair<Long, NodeRef> nodePair
  ) throws InvalidNodeRefException {
    Long nodeId = nodePair.getFirst();
   ...
   * Gets, converts and adds the intrinsic properties to the current node's properties
   */
  private Long getAclIDImpl(Pair<Long, NodeRef> nodePair)
    throws InvalidNodeRefException {
    Long nodeId = nodePair.getFirst();
  ...
  private boolean setPropertiesCommonWork(
    ...
    if (properties.containsKey(ContentModel.PROP_NAME)) {
      ...
      Pair<Long, ChildAssociationRef> primaryParentAssocPair = nodeDAO.getPrimaryParentAssoc(
        nodeId
      );
      if (primaryParentAssocPair != null) {
        String oldName = extractNameProperty(nodeDAO.getNodeProperties(nodeId));
        String newName = DefaultTypeConverter.INSTANCE.convert(
          String.class,
          name
        );
  ...
  public Collection<NodeRef> getParents(NodeRef nodeRef)
    throws InvalidNodeRefException {
  ...
  public List<ChildAssociationRef> getParentAssocs(
    ...
    final List<ChildAssociationRef> results = new ArrayList<ChildAssociationRef>(
      10
    );
    ...
    QName typeQName = (typeQNamePattern instanceof QName)
  ...
  public NodeRef getChildByName(
    ...
    Pair<Long, ChildAssociationRef> childAssocPair = nodeDAO.getChildAssoc(
      nodeId,
      assocTypeQName,
      childName
    );
  ...
  public AssociationRef createAssociation(
    ...
    Long assocId = nodeDAO.newNodeAssoc(
      sourceNodeId,
      targetNodeId,
      assocTypeQName,
      -1
  ...
  public void setAssociations(
    ...
    Collection<Pair<Long, AssociationRef>> assocsBefore = nodeDAO.getTargetNodeAssocs(
      sourceNodeId,
      assocTypeQName
    );
    Map<NodeRef, Long> targetRefsBefore = new HashMap<NodeRef, Long>(
      assocsBefore.size()
    );
    Map<NodeRef, Long> toRemoveMap = new HashMap<NodeRef, Long>(
      assocsBefore.size()
    );
  ...
  public Collection<ChildAssociationRef> getChildAssocsWithoutParentAssocsOfType(
    ...
    nodeDAO.getChildAssocsWithoutParentAssocsOfType(
      parentNodeId,
      assocTypeQName,
      callback
    );
  ...
  public List<ChildAssociationRef> getChildAssocsByPropertyValue(
    ...
    // Get the assocs pointing to it
    nodeDAO.getChildAssocsByPropertyValue(
      nodeId,
      propertyQName,
      value,
      callback
  ...
  public void removeAssociation(
    ...
    // delete it
    int assocsDeleted = nodeDAO.removeNodeAssoc(
      sourceNodeId,
      targetNodeId,
      assocTypeQName
    );
  ...
  public List<AssociationRef> getTargetAssocs(
    ...
    Collection<Pair<Long, AssociationRef>> assocPairs = nodeDAO.getTargetNodeAssocs(
      sourceNodeId,
      qnameFilter
    );
    List<AssociationRef> nodeAssocRefs = new ArrayList<AssociationRef>(
      assocPairs.size()
    );
    for (Pair<Long, AssociationRef> assocPair : assocPairs) {
      ...
      if (
        qnameFilter == null && !qnamePattern.isMatch(assocRef.getTypeQName())
      ) {
  ...
  public List<AssociationRef> getTargetAssocsByPropertyValue(
    ...
    Collection<Pair<Long, AssociationRef>> assocPairs = nodeDAO.getTargetAssocsByPropertyValue(
      sourceNodeId,
      qnameFilter,
      propertyQName,
      propertyValue
  ...
  public List<AssociationRef> getSourceAssocs(
    ...
    Collection<Pair<Long, AssociationRef>> assocPairs = nodeDAO.getSourceNodeAssocs(
      targetNodeId,
      qnameFilter
    );
    List<AssociationRef> nodeAssocRefs = new ArrayList<AssociationRef>(
   ...
   */
  private void archiveHierarchyImpl(
    NodeHierarchyWalker walker,
    StoreRef archiveStoreRef
  ) {
    // Start with the node we are archiving to
    Pair<Long, NodeRef> archiveStoreRootNodePair = nodeDAO.getRootNode(
      archiveStoreRef
    );
    ...
    for (VisitedNode node : walker.getNodes(false)) {
      // Get node metadata
      Map<QName, Serializable> archiveProperties = nodeDAO.getNodeProperties(
        node.id
      );
      ...
      if (firstNode) {
        ...
        Serializable originalOwner = archiveProperties.get(
      ...
      Set<AccessPermission> originalNodePermissions = permissionService.getAllSetPermissions(
        node.nodeRef
      );
      ...
      if (firstNode) {
        ...
        if (
          !nodeDAO.hasNodeAspect(
            archiveStoreRootNodeId,
            ContentModel.ASPECT_ARCHIVE_ROOT
          )
        ) {
          addAspect(
        ...
        String archiveNodeName = (String) archiveProperties.get(
          ContentModel.PROP_NAME
        );
  ...
  private NodeRef restoreNodeImpl(
    ...
    Map<QName, Serializable> existingProperties = nodeDAO.getNodeProperties(
      archivedNodeId
    );
    ...
    if (!existingAspects.contains(ContentModel.ASPECT_ARCHIVED)) {
      throw new AlfrescoRuntimeException(
        "The node to restore is not an archive node"
      );
    }
    ...
    Serializable originalOwner = existingProperties.get(
      ContentModel.PROP_ARCHIVED_ORIGINAL_OWNER
    );
    ...
    if (
      originalOwner == null || originalOwner.equals(OwnableService.NO_OWNER)
    ) {
  ...
  public ChildAssociationRef moveNode(
    ...
    Pair<Long, ChildAssociationRef> oldParentAssocPair = nodeDAO.getPrimaryParentAssoc(
      nodeToMoveId
    );
    ...
    if (movingStore) {
      ...
      for (ChildAssociationRef oldChildAssoc : childAssocs) {
        ...
        Pair<Long, NodeRef> newParentNodePair = movedNodePairs.get(
          oldParentNodeRef
        );
        ...
        try {
          moveNodeResult =
            nodeDAO.moveNode(oldChildNodeId, newParentNodeId, null, null);
        } catch (NodeExistsException e) {
      ...
      if (
        !oldParentAssocRef
          .getParentRef()
          .equals(newParentAssocRef.getParentRef())
      ) {
        propagateTimeStamps(oldParentAssocRef);
        propagateTimeStamps(newParentAssocRef);
      } else {
  ...
  private String extractNameProperty(Map<QName, Serializable> properties) {
    ...
    String name = (String) DefaultTypeConverter.INSTANCE.convert(
      String.class,
      nameValue
    );
    return name;
  ...
  private void propagateTimeStamps(ChildAssociationRef assocRef) {
    ...
    if (
      TransactionalResourceHelper
        .getMap(KEY_AUDITABLE_PROPAGATION_PRE)
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          "Update of cm:auditable already requested for " + parentNodePair
        );
      }
      return;
    }
    ...
    if (nodeDAO.isInCurrentTxn(parentNodeId)) {
      ...
      TransactionalResourceHelper
        .getMap(KEY_AUDITABLE_PROPAGATION_POST)
        .remove(parentNodeId);
  ...
  private static final String KEY_AUDITABLE_PROPAGATION_PRE =
    "node.auditable.propagation.pre";
  private static final String KEY_AUDITABLE_PROPAGATION_POST =
    "node.auditable.propagation.post";
  ...
  private class AuditableTransactionListener
    ...
    public void beforeCommit(boolean readOnly) {
      ...
      Map<Long, Long> parentNodeIds = TransactionalResourceHelper.getMap(
        KEY_AUDITABLE_PROPAGATION_PRE
      );
    ...
    public void afterCommit() {
      Map<Long, Long> parentNodeIds = TransactionalResourceHelper.getMap(
        KEY_AUDITABLE_PROPAGATION_POST
      );
      if (parentNodeIds.size() == 0) {
        return;
      }
    ...
    private void process(
      final Map<Long, Long> parentNodeIds,
      Date modifiedDate,
      ...
      for (Long parentNodeId : parentNodeIds.keySet()) {
        processSingle(
          parentNodeId,
          parentNodeIds.get(parentNodeId),
          modifiedDate,
          useCurrentTxn
        );
      }
    ...
    private void processSingle(
      ...
      RetryingTransactionCallback<Void> callback = new RetryingTransactionCallback<Void>() {
        ...
        public Void execute() throws Throwable {
          ...
          Pair<Long, NodeRef> parentNodePair = nodeDAO.getNodePair(
            parentNodeId
          );
          if (parentNodePair == null) {
            return null; // Parent has gone away
          } else if (
            !nodeDAO.hasNodeAspect(parentNodeId, ContentModel.ASPECT_AUDITABLE)
          ) {
          ...
          if (childNodePair == null) {
            ...
            !nodeDAO.hasNodeAspect(childNodeId, ContentModel.ASPECT_AUDITABLE)
          ) {
            ...
            modifiedByToPropagate =
              (String) nodeDAO.getNodeProperty(
                childNodeId,
                ContentModel.PROP_MODIFIER
              );
            modifiedDateToPropagate =
              (Date) nodeDAO.getNodeProperty(
                childNodeId,
          ...
          Date parentModifiedAt = (Date) nodeDAO.getNodeProperty(
            parentNodeId,
            ContentModel.PROP_MODIFIED
          );
          if (
            parentModifiedAt != null &&
          ...
          boolean changed = nodeDAO.setModifiedProperties(
            parentNodeId,
            modifiedDate,
            modifiedByToPropagate
          );
      ...
      try {
        ...
        logger.info(
          "Failed to update cm:modified date for node: " + parentNodeId
        );
      }
    }
  }

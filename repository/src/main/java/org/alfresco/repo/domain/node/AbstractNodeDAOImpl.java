 ...
 *
 * <p>This provides basic services such as caching, but defers to the underlying implementation for
 * CRUD operations.
 *
 * @author Derek Hulley
 * @since 3.4
...
public abstract class AbstractNodeDAOImpl implements NodeDAO, BatchingDAO {
    ...
    private static final String KEY_LOST_NODE_PAIRS =
            AbstractNodeDAOImpl.class.getName() + ".lostNodePairs";
    private static final String KEY_DELETED_ASSOCS =
            AbstractNodeDAOImpl.class.getName() + ".deletedAssocs";
     ...
     * VALUE KEY: None<br>
     */
    private EntityLookupCache<NodeVersionKey, Map<QName, Serializable>, Serializable>
            propertiesCache;
    /**
    ...
    public AbstractNodeDAOImpl() {
        ...
        // Caches
        rootNodesCache =
                new EntityLookupCache<StoreRef, Node, Serializable>(
                        new RootNodesCacheCallbackDAO());
        nodesCache = new EntityLookupCache<Long, Node, NodeRef>(new NodesCacheCallbackDAO());
        aspectsCache =
                new EntityLookupCache<NodeVersionKey, Set<QName>, Serializable>(
                        new AspectsCallbackDAO());
        propertiesCache =
                new EntityLookupCache<NodeVersionKey, Map<QName, Serializable>, Serializable>(
    ...
    /**
     * @param policyBehaviourFilter the service to determine the behaviour for <b>cm:auditable</b>
     *     and other inherent capabilities.
     */
    ...
    public void setRootNodesCache(SimpleCache<Serializable, Serializable> cache) {
        this.rootNodesCache =
                new EntityLookupCache<StoreRef, Node, Serializable>(
                        cache, CACHE_REGION_ROOT_NODES, new RootNodesCacheCallbackDAO());
    }
    ...
    public void setNodesCache(SimpleCache<Serializable, Serializable> cache) {
        this.nodesCache =
                new EntityLookupCache<Long, Node, NodeRef>(
                        cache, CACHE_REGION_NODES, new NodesCacheCallbackDAO());
    ...
    public void setAspectsCache(SimpleCache<NodeVersionKey, Set<QName>> aspectsCache) {
        this.aspectsCache =
                new EntityLookupCache<NodeVersionKey, Set<QName>, Serializable>(
                        aspectsCache, CACHE_REGION_ASPECTS, new AspectsCallbackDAO());
    }
     ...
     */
    public void setPropertiesCache(
            SimpleCache<NodeVersionKey, Map<QName, Serializable>> propertiesCache) {
    ...
    /**
     * Sets the average number of parents expected per cache entry. This parameter is multiplied by
     * the {@link #setParentAssocsCacheSize(int)} parameter to compute a limit on the total number
     * of cached parents, which will be proportional to the cache's memory usage. The cache will be
     * pruned when this limit is exceeded to avoid excessive memory usage.
     *
     * @param parentAssocsCacheLimitFactor the parentAssocsCacheLimitFactor to set
     */
     ...
     */
    public void setChildByNameCache(
            SimpleCache<ChildByNameKey, ChildAssocEntity> childByNameCache) {
    ...
    public void init() {
        ...
        this.nodePropertyHelper =
                new NodePropertyHelper(dictionaryService, qnameDAO, localeDAO, contentDataDAO);
        this.parentAssocsCache =
                new ParentAssocsCache(
    ...
    /**
     * Invalidate cache entries for all children of a give node. This usually applies where the
     * child associations or nodes are modified en-masse.
     *
     * @param parentNodeId the parent node of all child nodes to be invalidated (may be
     *     <tt>null</tt>)
    ...
    private int invalidateNodeChildrenCaches(
            Long parentNodeId, boolean primary, boolean touchNodes) {
        ...
        while (minChildNodeIdInclusive != null) {
            childNodeIds.clear();
            List<ChildAssocEntity> childAssocs =
                    selectChildNodeIds(
                            parentNodeId, Boolean.valueOf(primary), minChildNodeIdInclusive, 256);
            ...
            for (ChildAssocEntity childAssoc : childAssocs) {
                ...
                if (childNodeId.compareTo(minChildNodeIdInclusive) < 0) {
                    throw new RuntimeException(
                            "Query results did not increase for child node id ID");
                } else {
    ...
    public Pair<Long, StoreRef> getStore(StoreRef storeRef) {
        ...
        if (rootNodePair == null) {
            ...
            return new Pair<Long, StoreRef>(
                    rootNodePair.getSecond().getStore().getId(), rootNodePair.getFirst());
        }
    }
    ...
    public List<Pair<Long, StoreRef>> getStores() {
        ...
        List<Pair<Long, StoreRef>> storeRefs =
                new ArrayList<Pair<Long, StoreRef>>(storeEntities.size());
    ...
    public Set<NodeRef> getAllRootNodes(StoreRef storeRef) {
        ...
        if (rootNodes == null) {
            final Map<StoreRef, Set<NodeRef>> allRootNodes =
                    new HashMap<StoreRef, Set<NodeRef>>(97);
            getNodesWithAspects(
                    Collections.singleton(ContentModel.ASPECT_ROOT),
                    0L,
                    Long.MAX_VALUE,
    ...
    private class RootNodesCacheCallbackDAO
            extends EntityLookupCallbackDAOAdaptor<StoreRef, Node, Serializable> {
        ...
        public Pair<StoreRef, Node> createValue(Node value) {
            throw new UnsupportedOperationException(
                    "Root node creation is done externally: " + value);
        }
    ...
    /**
     * Callback to cache nodes by ID and {@link NodeRef}. When looking up objects based on the value
     * key, only the referencing properties need be populated. <b>ALL</b> nodes are cached, not just
     * live nodes.
     *
     * @see NodeEntity
     * @author Derek Hulley
    ...
    private class NodesCacheCallbackDAO
            extends EntityLookupCallbackDAOAdaptor<Long, Node, NodeRef> {
    ...
    public Pair<Long, NodeRef> getNodePair(NodeRef nodeRef) {
        ...
        if (pair == null || pair.getSecond().getDeleted(qnameDAO)) {
            ...
            if (dbNode == null) {
                ...
                // We may have reached this deleted node via an invalid association; trigger a post
                // transaction prune of
        ...
        selectChildAssocs(
                ...
                new ChildAssocRefQueryCallback() {
                    ...
                    public boolean handle(
                        ...
                        bindFixAssocAndCollectLostAndFound(
                                childNodePair,
                                "childNodeWithDeletedParent",
                                childAssocPair.getFirst(),
                                childAssocPair.getSecond().isPrimary()
                                        && exists(childAssocPair.getFirst()));
                        return true;
                    }
     ...
     * @param allowAuditableAspect Should we override the behaviour by potentially not adding the
     *     auditable aspect
    ...
    private NodeEntity newNodeImpl(
        ...
        if (addAuditableAspect) {
            Long auditableAspectQNameId =
                    qnameDAO.getOrCreateQName(ContentModel.ASPECT_AUDITABLE).getFirst();
    ...
    protected Long newNodeImplInsert(NodeEntity node) {
        ...
        try {
            ...
            // This is probably because there is an existing node.  We can handle existing deleted
            // nodes.
    ...
    public Pair<Pair<Long, ChildAssociationRef>, Pair<Long, NodeRef>> moveNode(
        ...
        if (!EqualsHelper.nullSafeEquals(newParentNodeId, oldParentNodeId)) {
            ...
            if (hasNodeAspect(oldParentNodeId, ContentModel.ASPECT_PENDING_FIX_ACL)) {
                Long oldParentSharedAclId =
                        (Long)
                                this.getNodeProperty(
                                        oldParentNodeId, ContentModel.PROP_SHARED_ACL_TO_REPLACE);
                accessControlListDAO.updateInheritance(
                        newChildNodeId, oldParentSharedAclId, newParentAclId);
            } else {
                accessControlListDAO.updateInheritance(
                        newChildNodeId, oldParentAclId, newParentAclId);
            }
        }
    ...
    protected void updatePrimaryParentAssocs(
            ...
            final QName assocTypeQName,
            final QName assocQName) {
        // Because we are retrying in-transaction i.e. absorbing exceptions, we need partial
        // rollback &/or via savepoint if needed (eg. PostgreSQL)
        RetryingCallback<Integer> callback =
                new RetryingCallback<Integer>() {
                    public Integer execute() throws Throwable {
                        return updatePrimaryParentAssocsImpl(
                                primaryParentAssoc,
                                newParentNode,
                                childNode,
                                newChildNodeId,
        ...
        try {
            int updated =
                    updatePrimaryParentAssocs(
                            ...
                            childNodeNameToUse);
            controlDAO.releaseSavepoint(savepoint);
            // Ensure we invalidate the name cache (the child version key might not have been
            // 'bumped' by the last
            // 'touch')
            if (updated > 0 && primaryParentAssoc != null) {
                Pair<Long, QName> oldTypeQnamePair =
                        qnameDAO.getQName(primaryParentAssoc.getTypeQNameId());
                if (oldTypeQnamePair != null) {
                    childByNameCache.remove(
                            new ChildByNameKey(
                                    oldParentNodeId,
                                    oldTypeQnamePair.getSecond(),
            ...
            // There are some cases - FK violations, specifically - where we DO actually want to
            // retry.
            // Detecting this is done by looking for the related FK names, 'fk_alf_cass_*' in the
            // error message
            ...
            throw new DuplicateChildNodeNameException(
                    newParentNode.getNodeRef(), assocTypeQName, childNodeName, e);
        }
    }
    ...
    /**
     * Updates the node's transaction and <b>cm:auditable</b> properties while providing a
     * convenient method to control cache entry invalidation.
     *
     * <p>Not all 'touch' signals actually produce a change: the node may already have been touched
     * in the current transaction. In this case, the required caches are explicitly invalidated as
     * requested.<br>
     * It is more complicated when the node is modified. If the node is modified against a previous
     * transaction then all cache entries are left untrusted and not pulled forward. But if the node
     * is modified but in the same transaction, then the cache entries are considered good and pull
     * forward against the current version of the node ... <b>unless</b> the cache was specicially
     * tagged for invalidation.
     *
     * <p>It is sometime necessary to provide the node's current aspects, particularly during
     ...
     * @param invalidateNodePropertiesCache <tt>true</tt> if the node's cached properties are
     *     unreliable
     * @param invalidateParentAssocsCache <tt>true</tt> if the node's cached parent assocs are
     *     unreliable
    ...
    private boolean touchNode(
        ...
        if (updatedNode) {
            ...
            if (invalidateParentAssocsCache) {
                // Because we cache parent assocs by transaction, we must manually invalidate on
                // this version change
                invalidateParentAssocsCached(node);
            } else {
    ...
    /**
     * Helper method that updates the node, bringing it into the current transaction with the
     * appropriate <b>cm:auditable</b> and transaction behaviour.
     *
     * <p>If the <tt>NodeRef</tt> of the node is changing (usually a store move) then deleted nodes
     * are cleaned out where they might exist.
     *
     ...
     */
    private boolean updateNodeImpl(
            Node oldNode, NodeUpdateEntity nodeUpdate, Set<QName> nodeAspects) {
        ...
        if (!EqualsHelper.nullSafeEquals(nodeId, nodeUpdate.getId())) {
            throw new IllegalArgumentException(
                    "NodeUpdateEntity node ID is not correct: " + nodeUpdate);
        }
        ...
        if (nodeAspects.contains(ContentModel.ASPECT_AUDITABLE)) {
            ...
            if (policyBehaviourFilter.isEnabled(oldNodeRef, ContentModel.ASPECT_AUDITABLE)) {
                ...
                boolean updateAuditableProperties =
                        auditableProps.setAuditValues(null, null, false, modifiedDateToleranceMs);
                nodeUpdate.setAuditableProperties(auditableProps);
                nodeUpdate.setUpdateAuditableProperties(updateAuditableProperties);
            } else if (nodeUpdate.getAuditableProperties() == null) {
                // cache the explicit setting of auditable properties when creating node (note:
                // auditable aspect is not yet present)
                AuditablePropertiesEntity auditableProps = oldNode.getAuditableProperties();
                if (auditableProps != null) {
                    nodeUpdate.setAuditableProperties(
                            auditableProps); // Can reuse the locked instance
        ...
        if (count != 1) {
            ...
            throw new ConcurrencyFailureException(
                    "Failed to update node " + nodeId, concurrencyException);
        } else {
    ...
    public void setPrimaryChildrenSharedAclId(
        ...
        updatePrimaryChildrenSharedAclId(
                txnId, primaryParentNodeId, optionalOldSharedAlcIdInAdditionToNull, newSharedAclId);
     ...
     * @param deleteAcl <tt>true</tt> to delete any associated ACLs otherwise <tt>false</tt> if the
     *     ACLs get reused elsewhere
     */
    private void deleteNodeImpl(Long nodeId, boolean deleteAcl) {
        ...
        Set<QName> contentQNames =
                new HashSet<QName>(dictionaryService.getAllProperties(DataTypeDefinition.CONTENT));
        ...
        Node deletedNode =
                newNodeImpl(store, uuid, deletedQNameId, defaultLocaleId, null, null, true);
        Long deletedNodeId = deletedNode.getId();
        // Store the original ID as a property
        Map<QName, Serializable> trackingProps =
                Collections.singletonMap(ContentModel.PROP_ORIGINAL_ID, (Serializable) nodeId);
    ...
    public Map<QName, Serializable> getNodeProperties(Long nodeId) {
        ...
        props =
                new ValueProtectingMap<QName, Serializable>(
                        props, NodePropertyValue.IMMUTABLE_CLASSES);

        // Done
        if (isDebugEnabled) {
            logger.debug(
                    "Fetched properties for Node: \n"
                            + "   Node:  "
                            + nodeId
                            + "\n"
                            + "   Props: "
    ...
    public Serializable getNodeProperty(Long nodeId, QName propertyQName) {
        ...
        } else if (ReferenceablePropertiesEntity.isReferenceableProperty(
                propertyQName)) // sys:referenceable
        {
            Node node = getNodeNotNull(nodeId, false);
     ...
     * updated or deleted.
     *
     * <p>Note: The cached properties are not updated
     *
     * @param nodeId the node ID
     * @param newProps the properties to add or update
     * @param isAddOnly <tt>true</tt> if the new properties are just an update or <tt>false</tt> if
     *     the properties are a complete set
     * @return Returns <tt>true</tt> if any properties were changed
     */
    private boolean setNodePropertiesImpl(
            Long nodeId, Map<QName, Serializable> newProps, boolean isAddOnly) {
        ...
        if (!policyBehaviourFilter.isEnabled(node.getNodeRef(), ContentModel.ASPECT_AUDITABLE)) {
            ...
            if (AuditablePropertiesEntity.hasAuditableProperty(newProps.keySet())) {
                ...
                if (auditableProps == null) {
                    ...
                    auditableProps =
                            new AuditablePropertiesEntity(auditableProps); // Unlocked instance
                }
        ...
        Locale newLocale =
                DefaultTypeConverter.INSTANCE.convert(
                        Locale.class, newProps.get(ContentModel.PROP_LOCALE));
        if (newLocale != null) {
        ...
        // completely.
        Map<QName, Serializable> oldPropsCached =
                getNodePropertiesCached(nodeId); // Keep pristine for caching
        ...
        Map<NodePropertyKey, NodePropertyValue> newPropsRaw =
                nodePropertyHelper.convertToPersistentProperties(newProps);
            ...
            switch (entry.getValue()) {
                ...
                case RIGHT_ONLY:
                    ...
                    if (isContent && value != null) {
                        ...
                        Long newContentDataId =
                                contentDataDAO.createContentData(newContentData).getFirst();
                ...
                default:
                    throw new IllegalStateException(
                            "Unknown MapValueComparison: " + entry.getValue());
            }
        }
        ...
        if (nodeUpdate.isUpdateAnything()) {
            ...
            if (updateNodeImpl(node, nodeUpdate, null)) {
                ...
                NodeVersionKey newNodeVersionKey =
                        getNodeNotNull(nodeId, false).getNodeVersionKey();
                copyNodeAspectsCached(nodeVersionKey, newNodeVersionKey);
                copyNodePropertiesCached(nodeVersionKey, newNodeVersionKey);
                copyParentAssocsCached(node);
            }
        ...
        if (modifyProps) {
            ...
            try {
                if (contentQNamesToDelete.size() > 0) {
                    Set<Long> contentQNameIdsToDelete =
                            qnameDAO.convertQNamesToIds(contentQNamesToDelete, false);
    ...
    private Map<QName, Serializable> getNodePropertiesCached(Long nodeId) {
        ...
        Pair<NodeVersionKey, Map<QName, Serializable>> cacheEntry =
                propertiesCache.getByKey(nodeVersionKey);
        if (cacheEntry == null) {
            invalidateNodeCaches(nodeId);
     ...
     *
     * <p><b>NOTE:</b> Incoming properties must exclude the <b>cm:auditable</b> properties
     */
    ...
    /**
     * Callback to cache node properties. The DAO callback only does the simple {@link
     * #findByKey(Serializable)}.
     *
     * @author Derek Hulley
     * @since 3.4
     */
    private class PropertiesCallbackDAO
            extends EntityLookupCallbackDAOAdaptor<
                    NodeVersionKey, Map<QName, Serializable>, Serializable> {
        public Pair<NodeVersionKey, Map<QName, Serializable>> createValue(
                Map<QName, Serializable> value) {
            throw new UnsupportedOperationException("A node always has a 'map' of properties.");
        }

        public Pair<NodeVersionKey, Map<QName, Serializable>> findByKey(
                NodeVersionKey nodeVersionKey) {
            Long nodeId = nodeVersionKey.getNodeId();
            Map<NodeVersionKey, Map<NodePropertyKey, NodePropertyValue>> propsRawByNodeVersionKey =
                    selectNodeProperties(nodeId);
            Map<NodePropertyKey, NodePropertyValue> propsRaw =
                    propsRawByNodeVersionKey.get(nodeVersionKey);
            if (propsRaw == null) {
                ...
                if (propsRawByNodeVersionKey.size() == 0) {
                    ...
                    throw new DataIntegrityViolationException(
                            "Detected stale node entry: "
                                    + nodeVersionKey
                                    + " (now "
                                    + propsRawByNodeVersionKey.keySet()
                                    + ")");
                }
            }
            ...
            // Done
            return new Pair<NodeVersionKey, Map<QName, Serializable>>(
                    nodeVersionKey, Collections.unmodifiableMap(props));
        }
    }
    ...
    /**
     * Callback to cache node aspects. The DAO callback only does the simple {@link
     * #findByKey(Serializable)}.
     *
     * @author Derek Hulley
     * @since 3.4
    ...
    private class AspectsCallbackDAO
        ...
        public Pair<NodeVersionKey, Set<QName>> findByKey(NodeVersionKey nodeVersionKey) {
            ...
            Map<NodeVersionKey, Set<QName>> nodeAspectQNameIdsByVersionKey =
                    selectNodeAspects(nodeIds);
                ...
                if (nodeAspectQNameIdsByVersionKey.size() == 0) {
                    ...
                    throw new DataIntegrityViolationException(
                            "Detected stale node entry: "
                                    + nodeVersionKey
                                    + " (now "
                                    + nodeAspectQNameIdsByVersionKey.keySet()
                                    + ")");
                }
            }
            // Done
            return new Pair<NodeVersionKey, Set<QName>>(
                    nodeVersionKey, Collections.unmodifiableSet(nodeAspectQNames));
        }
    }
    ...
    public Long newNodeAssoc(
            Long sourceNodeId, Long targetNodeId, QName assocTypeQName, int assocIndex) {
        if (assocIndex == 0) {
    ...
    public int removeNodeAssocs(List<Long> ids) {
        ...
        if (toDelete != deleted) {
            throw new ConcurrencyFailureException(
                    "Deleted " + deleted + " but expected " + toDelete);
        }
        return deleted;
    }
    ...
    public Collection<Pair<Long, AssociationRef>> getNodeAssocsToAndFrom(Long nodeId) {
        ...
        List<Pair<Long, AssociationRef>> results =
                new ArrayList<Pair<Long, AssociationRef>>(nodeAssocEntities.size());
    ...
    @Override
    public Collection<Pair<Long, AssociationRef>> getSourceNodeAssocs(
            Long targetNodeId, QName typeQName) {
        ...
        List<NodeAssocEntity> nodeAssocEntities =
                selectNodeAssocsByTarget(targetNodeId, typeQNameId);
        List<Pair<Long, AssociationRef>> results =
                new ArrayList<Pair<Long, AssociationRef>>(nodeAssocEntities.size());
        for (NodeAssocEntity nodeAssocEntity : nodeAssocEntities) {
            Long assocId = nodeAssocEntity.getId();
    ...
    @Override
    public Collection<Pair<Long, AssociationRef>> getTargetNodeAssocs(
            Long sourceNodeId, QName typeQName) {
        Long typeQNameId = null;
        if (typeQName != null) {
            Pair<Long, QName> typeQNamePair = qnameDAO.getQName(typeQName);
            if (typeQNamePair == null) {
        ...
        List<NodeAssocEntity> nodeAssocEntities =
                selectNodeAssocsBySource(sourceNodeId, typeQNameId);
        List<Pair<Long, AssociationRef>> results =
                new ArrayList<Pair<Long, AssociationRef>>(nodeAssocEntities.size());
        for (NodeAssocEntity nodeAssocEntity : nodeAssocEntities) {
            Long assocId = nodeAssocEntity.getId();
    ...
    public Collection<Pair<Long, AssociationRef>> getTargetAssocsByPropertyValue(
            Long sourceNodeId, QName typeQName, QName propertyQName, Serializable propertyValue) {
        Long typeQNameId = null;
        if (typeQName != null) {
            Pair<Long, QName> typeQNamePair = qnameDAO.getQName(typeQName);
            if (typeQNamePair == null) {
        ...
        if (propertyQName != null) {
            ...
            if (nodeValue != null) {
                switch (nodeValue.getPersistedType()) {
                    ...
                    default:
                        throw new IllegalArgumentException(
                                "method not supported for persisted value type "
                                        + nodeValue.getPersistedType());
                }
            }
        }
        ...
        List<NodeAssocEntity> nodeAssocEntities =
                selectNodeAssocsBySourceAndPropertyValue(
                        sourceNodeId, typeQNameId, propertyQNameId, nodeValue);
    ...
    public Pair<Long, AssociationRef> getNodeAssoc(Long assocId) {
        ...
        if (ret == null) {
            throw new ConcurrencyFailureException(
                    "Assoc ID does not point to a valid association: " + assocId);
        } else {
            return ret;
        }
    }
    ...
    protected Long newChildAssocInsert(
            final ChildAssocEntity assoc, final QName assocTypeQName, final String childNodeName) {
        // Because we are retrying in-transaction i.e. absorbing exceptions, we need partial
        // rollback &/or via savepoint if needed (eg. PostgreSQL)
        RetryingCallback<Long> callback =
                new RetryingCallback<Long>() {
    ...
    protected Long newChildAssocInsertImpl(
            final ChildAssocEntity assoc, final QName assocTypeQName, final String childNodeName) {
        Savepoint savepoint = controlDAO.createSavepoint("DuplicateChildNodeNameException");
        try {
            ...
            // Allow real DB concurrency issues (e.g. DeadlockLoserDataAccessException) straight
            // through for a retry
            ...
            if (lowerMsg.contains("fk_alf_cass_")) {
                throw new ConcurrencyFailureException(
                        "FK violation updating primary parent association:" + assoc, e);
            }

            // We assume that this is from the child cm:name constraint violation
            throw new DuplicateChildNodeNameException(
                    assoc.getParentNode().getNodeRef(), assocTypeQName, childNodeName, e);
        }
    }
    ...
    public void deleteChildAssoc(Long assocId) {
        ...
        if (assoc == null) {
            throw new ConcurrencyFailureException(
                    "Child association not found: "
                            + assocId
                            + ".  A concurrency violation is likely.\n"
                            + "This can also occur if code reacts to 'beforeDelete' callbacks and"
                            + " pre-emptively deletes associations \n"
                            + "that are about to be cascade-deleted.  The 'onDelete' phase then"
                            + " fails to delete the association.\n"
                            + "See links on issue"
                            + " ALF-12358."); // TODO: Get docs URL
        }
        // Update cache
    ...
    public int setChildAssocIndex(
            ...
            int index) {
        int count =
                updateChildAssocIndex(parentNodeId, childNodeId, assocTypeQName, assocQName, index);
        if (count > 0) {
    ...
    protected int updateChildAssocUniqueNameImpl(final Long childNodeId, final String childName) {
        ...
        Savepoint savepoint = controlDAO.createSavepoint("DuplicateChildNodeNameException");
        try {
            for (ChildAssocEntity parentAssoc :
                    getParentAssocsCached(childNodeId).getParentAssocs().values()) {
                // Subtlety: We only update those associations for which name uniqueness checking is
                // enforced.
                ...
                Pair<Long, QName> oldTypeQnamePair =
                        qnameDAO.getQName(parentAssoc.getTypeQNameId());
                // Ensure we invalidate the name cache (the child version key might not be 'bumped'
                // by the next
                // 'touch')
                if (oldTypeQnamePair != null) {
                    childByNameCache.remove(
                            new ChildByNameKey(
                                    parentAssoc.getParentNode().getId(),
                                    oldTypeQnamePair.getSecond(),
                                    parentAssoc.getChildNodeName()));
                }
                int count = updateChildAssocUniqueName(parentAssoc.getId(), childName);
                if (count <= 0) {
                    // Should not be attempting to delete a deleted node
                    throw new ConcurrencyFailureException(
                            "Failed to update an existing parent association "
                                    + parentAssoc.getId());
                }
                total += count;
            }
    ...
    public Pair<Long, ChildAssociationRef> getChildAssoc(
            Long parentNodeId, Long childNodeId, QName assocTypeQName, QName assocQName) {
        List<ChildAssocEntity> assocs =
                selectChildAssoc(parentNodeId, childNodeId, assocTypeQName, assocQName);
        if (assocs.size() == 0) {
            return null;
        ...
        Map<Long, ChildAssocEntity> assocsToDeleteById =
                new HashMap<Long, ChildAssocEntity>(assocs.size() * 2);
        Long minId = null;
        Long primaryId = null;
     ...
     *
     * <p>Instances must be used and discarded per query.
     *
     * @author Derek Hulley
     * @since 3.4
     */
     ...
     *
     * <p>Note: If we were to cach misses, then we would have to ensure that the cache is kept up to
     * date whenever any affection association is changed. This is actually not possible without
     * forcing the cache to be fully clustered. So to avoid clustering the cache, we instead watch
     * the node child version, which relies on a cache that is already clustered.
     */
    @Override
    public Pair<Long, ChildAssociationRef> getChildAssoc(
            Long parentNodeId, QName assocTypeQName, String childName) {
        ...
        if (assoc == null) {
            ...
            if (childNodeFromCache == null) {
                ...
                NodeVersionKey childNodeFromCacheVersionKey =
                        childNodeFromCache.getSecond().getNodeVersionKey();
    ...
    public void getChildAssocsWithoutParentAssocsOfType(
            Long parentNodeId, QName assocTypeQName, ChildAssocRefQueryCallback resultsCallback) {
    ...
    public void getParentAssocs(
        ...
        if (assocTypeQName == null && assocQName == null && isPrimary == null) {
            ...
            if (parentAssocs.getParentAssocs().size() > PARENT_ASSOCS_CACHE_FILTER_THRESHOLD) {
                // Query
                selectParentAssocs(
                        childNodeId, assocTypeQName, assocQName, isPrimary, resultsCallback);
            } else {
                ...
                for (ChildAssocEntity assoc : parentAssocs.getParentAssocs().values()) {
                    ...
                    if (((assocTypeQName == null)
                                    || (assocPair
                                            .getSecond()
                                            .getTypeQName()
                                            .equals(assocTypeQName)))
                            && ((assocQName == null)
                                    || (assocPair.getSecond().getQName().equals(assocQName)))) {
                        resultsCallback.handle(
                                assocPair,
                                assoc.getParentNode().getNodePair(),
                                assoc.getChildNode().getNodePair());
    ...
    /**
     * Potentially cheaper than evaluating all of a node's paths to check for child association
     * cycles
     *
     * <p>TODO: When is it cheaper to go up and when is it cheaper to go down? Look at using direct
     * queries to pass through layers both up and down.
     *
    ...
    private class CycleCallBack implements ChildAssocRefQueryCallback {
        ...
        public boolean handle(
            ...
            if (!nodeIds.add(nodeId)) {
                ...
                // Remember exception we want to throw and exit. If we throw within here, it will be
                // wrapped by IBatis
                toThrow =
                        new CyclicChildRelationshipException(
    ...
    @Override
    public List<Path> getPaths(Pair<Long, NodeRef> nodePair, boolean primaryOnly)
    ...
    private void bindFixAssocAndCollectLostAndFound(
            final Pair<Long, NodeRef> lostNodePair,
            final String lostName,
            final Long assocId,
            final boolean orphanChild) {
        // Remember the items already deleted in inner transactions
        final Set<Pair<Long, NodeRef>> lostNodePairs =
                TransactionalResourceHelper.getSet(KEY_LOST_NODE_PAIRS);
                            ...
                            RetryingTransactionCallback<Void> callback =
                                    new RetryingTransactionCallback<Void>() {
                                        public Void execute() throws Throwable {
                                            if (assocId == null) {
                                                // 'child' with missing parent assoc => collect
                                                // lost+found orphan child
                                                if (lostNodePairs.add(lostNodePair)) {
                                                    collectLostAndFoundNode(lostNodePair, lostName);
                                                    logger.error(
                                                            "ALF-13066: Orphan child node has been"
                                                                    + " re-homed under lost_found: "
                                                                    + lostNodePair);
                                                }
                                            } else {
                                                // 'child' with deleted parent assoc => delete
                                                // invalid parent assoc and if primary then
                                                // collect lost+found orphan child
                                                if (deletedAssocs.add(assocId)) {
                                                    deleteChildAssoc(
                                                            assocId); // Can't use caching version
                                                    // or may hit infinite loop
                                                    logger.error(
                                                            "ALF-12358: Deleted node - removed"
                                                                    + " child assoc: "
                                                                    + assocId);
                                                }

                                                if (orphanChild
                                                        && lostNodePairs.add(lostNodePair)) {
                                                    collectLostAndFoundNode(lostNodePair, lostName);
                                                    logger.error(
                                                            "ALF-12358: Orphan child node has been"
                                                                    + " re-homed under lost_found: "
                                                                    + lostNodePair);
                                                }
                                            }
    ...
    /**
     * TODO: Remove once ALF-12358 has been proven to be fixed i.e. no more orphans are created ...
     * ever.
     */
    ...
    private Node getOrCreateLostAndFoundContainer(StoreRef storeRef) {
        ...
        NodeDAO.ChildAssocRefQueryCallback callback =
                new NodeDAO.ChildAssocRefQueryCallback() {
                    public boolean handle(
                            Pair<Long, ChildAssociationRef> childAssocPair,
                            Pair<Long, NodeRef> parentNodePair,
                            Pair<Long, NodeRef> childNodePair) {
    ...
    private void prependPaths(
        ...
        if (currentRootNodePair == null
                || !currentStoreRef.equals(currentRootNodePair.getFirst())) {
            // We've changed stores
            Pair<Long, NodeRef> rootNodePair = getRootNode(currentStoreRef);
            currentRootNodePair =
                    new Pair<StoreRef, NodeRef>(currentStoreRef, rootNodePair.getSecond());
        }

        // get the parent associations of the given node
        ParentAssocsInfo parentAssocInfo =
                getParentAssocsCached(
                        currentNodeId); // note: currently may throw NotLiveNodeException
        ...
        for (Map.Entry<Long, ChildAssocEntity> entry :
                parentAssocInfo.getParentAssocs().entrySet()) {
        ...
        // look for a root. If we only want the primary root, then ignore all but the top-level
        // root.
        if (!(primaryOnly && hasParents)
                && parentAssocInfo.isRoot()) // exclude primary search with parents present
        {
            ...
            ChildAssociationRef assocRef =
                    new ChildAssociationRef(null, null, null, currentRootNodePair.getSecond());
            ...
            if (first != null) {
                // mimic an association that would appear if the current node was below the root
                // node
                // or if first beneath the root node it will make the real thing
                ChildAssociationRef updateAssocRef =
                        new ChildAssociationRef(
                                parentAssocInfo.isStoreRoot()
                                        ? ContentModel.ASSOC_CHILDREN
                                        : first.getRef().getTypeQName(),
        ...
        for (Map.Entry<Long, ChildAssocEntity> entry :
            ...
            Pair<Long, NodeRef> parentNodePair =
                    new Pair<Long, NodeRef>(assoc.getParentNode().getId(), assocRef.getParentRef());
            ...
            if (assocIdStack.contains(assocId)) {
                ...
                throw new CyclicChildRelationshipException(
                        "Node has been pasted into its own tree.", assocRef);
            }
    ...
    /**
     * A Map-like class for storing ParentAssocsInfos. It prunes its oldest ParentAssocsInfo entries
     * not only when a capacity is reached, but also when a total number of cached parents is
     * reached, as this is what dictates the overall memory usage.
     */
    private static class ParentAssocsCache {
        ...
        private void put(Pair<Long, String> cacheKey, ParentAssocsInfo parentAssocs) {
            ...
            try {
                ...
                // Now prune the oldest entries whilst we have more cache entries or cached parents
                // than desired
    ...
    private ParentAssocsInfo getParentAssocsCached(Long nodeId) {
        Node node = getNodeNotNull(nodeId, false);
        Pair<Long, String> cacheKey =
                new Pair<Long, String>(nodeId, node.getTransaction().getChangeTxnId());
        ...
        // We have already validated on loading that we have a list in sync with the child node, so
        // if the list is still
        // empty we have an integrity problem
        if (value.getPrimaryParentAssoc() == null
                && !node.getDeleted(qnameDAO)
                && !value.isStoreRoot()) {
            Pair<Long, NodeRef> currentNodePair = node.getNodePair();
            // We have a corrupt repository - non-root node has a missing parent ?!
            bindFixAssocAndCollectLostAndFound(
                    currentNodePair, "nonRootNodeWithoutParents", null, false);

            // throw - error will be logged and then bound txn listener (afterRollback) will be
            // called
    ...
    private void copyParentAssocsCached(Node from) {
        ...
        if (cacheEntry != null) {
            parentAssocsCache.put(
                    new Pair<Long, String>(from.getId(), toTransactionId), cacheEntry);
        }
    }
    ...
    private ParentAssocsInfo loadParentAssocs(NodeVersionKey nodeVersionKey) {
        ...
        if (assocs.isEmpty()) {
            ...
            if (nodeCheckFromDb == null
                    || !nodeCheckFromDb.getNodeVersionKey().equals(nodeVersionKey)) {
            ...
            NodeVersionKey childNodeVersionKeyFromDb =
                    childAssoc.getChildNode().getNodeVersionKey();
    ...
    /**
     * Uses the node and parent assocs cache content to recursively find the set of currently cached
     * ancestor node IDs
     */
    ...
    public void cacheNodesById(List<Long> nodeIds) {
        ...
        if (nodesTransactionalCache != null) {
            disableSharedCacheReadForTransaction =
     ...
     * {@inheritDoc}
     *
     * <p>Loads properties, aspects, parent associations and the ID-noderef cache.
     */
    @Override
    public void cacheNodes(List<NodeRef> nodeRefs) {
        ...
        // Group the nodes by store so that we don't *have* to eagerly join to store to get query
        // performance
    ...
    private void cacheNodesNoBatch(List<Node> nodes) {
        ...
        Map<Long, NodeVersionKey> nodeVersionKeysFromCache =
                new HashMap<Long, NodeVersionKey>(nodes.size() * 2); // Keep for quick lookup
        for (Node node : nodes) {
            Long nodeId = node.getId();
            NodeVersionKey nodeVersionKey = node.getNodeVersionKey();
        ...
        // First ensure all content data are pre-cached, so we don't have to load them individually
        // when converting properties
        contentDataDAO.cacheContentDataForNodes(propertiesNodeIds);

        // Now bulk load the properties
        Map<NodeVersionKey, Map<NodePropertyKey, NodePropertyValue>> propsByNodeId =
                selectNodeProperties(propertiesNodeIds);
        for (Map.Entry<NodeVersionKey, Map<NodePropertyKey, NodePropertyValue>> entry :
                propsByNodeId.entrySet()) {
            Long nodeId = entry.getKey().getNodeId();
            Map<NodePropertyKey, NodePropertyValue> propertyValues = entry.getValue();
            Map<QName, Serializable> props =
                    nodePropertyHelper.convertToPublicProperties(propertyValues);
            setNodePropertiesCached(nodeId, props);
        }
    }
     ...
     * {@inheritDoc}
     *
     * <p>Simply clears out all the node-related caches.
     */
    @Override
    public void clear() {
        clearCaches();
    ...
    protected abstract int deleteNodesByCommitTime(
            long fromTxnCommitTimeMs, long toTxnCommitTimeMs);
    ...
    protected abstract Map<NodeVersionKey, Map<NodePropertyKey, NodePropertyValue>>
            selectNodeProperties(Long nodeId, Set<Long> qnameIds);
    ...
    protected abstract void insertNodeProperties(
            Long nodeId, Map<NodePropertyKey, NodePropertyValue> persistableProps);
    ...
    protected abstract Long insertNodeAssoc(
            Long sourceNodeId, Long targetNodeId, Long assocTypeQNameId, int assocIndex);

    protected abstract int updateNodeAssoc(Long id, int assocIndex);

    protected abstract int deleteNodeAssoc(
            Long sourceNodeId, Long targetNodeId, Long assocTypeQNameId);
    ...
    protected abstract List<NodeAssocEntity> selectNodeAssocsBySource(
            Long sourceNodeId, Long typeQNameId);

    protected abstract List<NodeAssocEntity> selectNodeAssocsBySourceAndPropertyValue(
            Long sourceNodeId, Long typeQNameId, Long propertyQNameId, NodePropertyValue nodeValue);

    protected abstract List<NodeAssocEntity> selectNodeAssocsByTarget(
            Long targetNodeId, Long typeQNameId);
    ...
    protected abstract int updateChildAssocIndex(
            Long parentNodeId, Long childNodeId, QName assocTypeQName, QName assocQName, int index);
    ...
    protected abstract List<ChildAssocEntity> selectChildNodeIds(
            Long nodeId, Boolean isPrimary, Long minAssocIdInclusive, int maxResults);
    ...
    protected abstract int updatePrimaryParentAssocs(
            ...
            String childNodeName);
    /**
     * Moves all node-linked data from one node to another. The source node will be left in an
     * orphaned state and without any attached data other than the current transaction.
     *
    ...
    protected abstract List<Long> selectTxnsUnused(
            Long minTxnId, Long maxCommitTime, Integer count);

...
public class SolrStatsResult
  implements SearchEngineResultMetadata, StatsResultSet {
    ...
    if (json.has("stats")) {
      ...
      if (statsObj.has("stats_fields")) {
        ...
        if (fieldNames.length() == 1) {
          JSONObject contentsize = statsFields.getJSONObject(
            fieldNames.getString(0)
          );
          ...
          if (contentsize.has("facets")) {
            ...
            for (int i = 0; i < facetNames.length(); i++) {
              JSONObject facetType = facets.getJSONObject(
                String.valueOf(facetNames.get(i))
              );
   ...
   * @throws JSONException
   */
  private StatsResultStat processStat(String name, JSONObject facetVal)
    throws JSONException {
    return new StatsResultStat(
      nameIsADate ? formatAsDate(name) : name,
  ...
  public String toString() {
    ...
    builder
      .append("SolrStatsResult [status=")
      .append(this.status)
      .append(", queryTime=")
      .append(this.queryTime)
      .append(", numberFound=")
      .append(this.numberFound)
      .append(", sum=")
      .append(this.sum)
      .append(", max=")
      .append(this.max)

...
public class DefaultChildApplicationContextManager
  ...
  public void destroy(boolean permanent) {
    if (getState(false) != null) {
      ...
      try {
        ApplicationContextManagerState state = (ApplicationContextManagerState) getState(
          false
        );
        ...
        if (state != null) {
          ...
          for (String id : state.getInstanceIds()) {
            ChildApplicationContextFactory factory = state.getApplicationContextFactory(
              id
            );
  ...
  protected PropertyBackedBeanState createInitialState() throws IOException {
    return new ApplicationContextManagerState(
      this.defaultChain,
  ...
  public ApplicationContext getApplicationContext(String id) {
    ...
    try {
      return (
        (ApplicationContextManagerState) getState(true)
  ...
  public ChildApplicationContextFactory getChildApplicationContextFactory(
    String id
  ) {
    lock.readLock().lock();
    try {
      DefaultChildApplicationContextManager.ApplicationContextManagerState state = (DefaultChildApplicationContextManager.ApplicationContextManagerState) getState(
        true
      );
  ...
  protected class ApplicationContextManagerState
     ...
     *            the default type name
     */
    protected ApplicationContextManagerState(
      String defaultChain,
      ...
      if (defaultChain != null && defaultChain.length() > 0) {
        ...
        if (defaultTypeName == null) {
          updateOrder(
            defaultChain,
            AbstractPropertyBackedBean.DEFAULT_INSTANCE_NAME
          );
          this.defaultTypeName =
            this.childApplicationContexts.get(this.instanceIds.get(0))
              .getTypeName();
        } else {
          this.defaultTypeName = defaultTypeName;
    ...
    public Set<String> getPropertyNames() {
      return Collections.singleton(
        DefaultChildApplicationContextManager.ORDER_PROPERTY
      );
    }
     ...
     */
    public ApplicationContext getApplicationContext(String id) {
      ChildApplicationContextFactory child =
        this.childApplicationContexts.get(id);
     ...
     */
    protected ChildApplicationContextFactory getApplicationContextFactory(
      String id
    ) {
    ...
    private void updateOrder(String orderString, String defaultTypeName) {
      try {
        ...
        while (tkn.hasMoreTokens()) {
          ...
          String id = sepIndex == -1
            ? instance
            : instance.substring(0, sepIndex);
          String typeName = sepIndex == -1 || sepIndex + 1 >= instance.length()
            ? defaultTypeName
        ...
        Set<String> idsToRemove = new TreeSet<String>(
          this.childApplicationContexts.keySet()
        );

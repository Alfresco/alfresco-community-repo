...
public class ActivitiScriptNode extends ScriptNode {
    ...
    /**
     * Value converter for beanshell. Dates should be handled differently since default conversion
     * uses top-level scope which is sometimes missing.
     */
    private class ActivitiNodeConverter extends NodeValueConverter {
        ...
        public Serializable convertValueForScript(
                ...
                QName qname,
                Serializable value) {
            // ALF-14863: If script-node is used outside of Script-call (eg. Activiti evaluating an
            // expression that contains variables of type ScriptNode)
            // a scope should be created solely for this conversion. The scope will ALWAYS be set
            // when value-conversion is called from the
            // ScriptProcessor
            ensureScopePresent();
            if (theScope == null) {
                theScope = scope;
        ...
        private void ensureScopePresent() {
            if (scope == null) {
                ...
                // In case no context is active for the current thread, we can safely enter end exit
                // one to get hold of a scope

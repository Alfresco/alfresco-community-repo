...
public class RepoAdminServiceImpl implements RepoAdminService {
    ...
    public static final String CRITERIA_ALL = "/*"; // immediate children only
    ...
    private static final String MODELS_LOCATION_NOT_FOUND =
            "repoadmin_service.models_location_not_found";
    private static final String MODELS_LOCATION_MULTIPLE_FOUND =
            "repoadmin_service.models_location_multiple_found";
    private static final String MODEL_EXISTS = "repoadmin_service.model_exists";
    private static final String MODEL_DEPLOYMENT_FAILED =
            "repoadmin_service.model_deployment_failed";
    private static final String MODEL_UNDEPLOYMENT_FAILED =
            "repoadmin_service.model_undeployment_failed";
    private static final String MODEL_ACTIVATION_FAILED =
            "repoadmin_service.model_activation_failed";
    private static final String MODEL_DEACTIVATION_FAILED =
    ...
    private static final String MODEL_ALREADY_ACTIVATED =
            "repoadmin_service.model_already_activated";
    private static final String MODEL_ALREADY_DEACTIVATED =
            "repoadmin_service.model_already_deactivated";
    private static final String MODEL_NO_LONGER_EXISTS = "repoadmin_service.model_no_longer_exists";
    private static final String MSG_RESOURCES_NOT_FOUND =
            "repoadmin_service.msg_resource_not_found";
    private static final String MSG_RESOURCES_DEPLOYMENT_FAILED =
            "repoadmin_service.msg_resource_deployment_failed";
    private static final String MESSAGES_LOCATION_NOT_FOUND =
            "repoadmin_service.messages_location_not_found";
    private static final String MESSAGES_LOCATION_MULTIPLE_FOUND =
            "repoadmin_service.messages_location_multiple_found";
    private static final String MSG_RESOURCES_UNDEPLOYMENT_FAILED =
            "repoadmin_service.msg_resource_undeployment_failed";
    private static final String MSG_RESOURCES_RELOAD_FAILED =
            "repoadmin_service.msg_resource_reload_failed";
    private static final String MSG_MISSING_BUNDLE_BASE_NAME =
            "repoadmin_service.msg_missing_bundle_base_name";
    private static final String MSG_BASE_NAME_CONTAIN_UNDERSCORE =
            "repoadmin_service.msg_base_name_contain_underscore";
    private static final String MSG_BASE_NAME_CONTAIN_PERIOD =
            "repoadmin_service.msg_base_name_contain_period";
    ...
    public List<RepoModelDefinition> getModels() {
        ...
        if (nodeRefs.size() > 0) {
            for (NodeRef nodeRef : nodeRefs) {
                ...
                String repoVersion =
                        (String) nodeService.getProperty(nodeRef, ContentModel.PROP_VERSION_LABEL);
                ...
                try {
                    ...
                    if (dictionaryModels.contains(modelName)) {
                        // note: uses dictionary model cache, rather than getting content from repo
                        // and re-compiling
                        modelsInRepo.add(
                                new RepoModelDefinition(
                                        modelFileName,
                                        repoVersion,
                                        dictionaryDAO.getModel(
                                                QName.createQName(modelName, namespaceService)),
                                        true));
                    } else {
                        modelsInRepo.add(
                                new RepoModelDefinition(modelFileName, repoVersion, null, false));
                    }
                } catch (Throwable t) {
    ...
    public NodeRef deployModel(InputStream modelStream, String modelFileName, boolean activate) {
        try {
            ...
            List<NodeRef> nodeRefs =
                    searchService.selectNodes(
                            rootNode, repoModelsLocation.getPath(), null, namespaceService, false);

            if (nodeRefs.size() == 0) {
                throw new AlfrescoRuntimeException(
                        MODELS_LOCATION_NOT_FOUND, new Object[] {repoModelsLocation.getPath()});
                ...
                try {
                    ...
                    ChildAssociationRef association =
                            nodeService.createNode(
                                    ...
                                    QName.createQName(
                                            NamespaceService.CONTENT_MODEL_1_0_URI, modelFileName),
                                    ContentModel.TYPE_DICTIONARY_MODEL,
                                    contentProps); // also invokes policies for DictionaryModelType
                    // - e.g. onUpdateProperties
                    ...
                    throw new ConcurrencyFailureException(
                            getLocalisedMessage(MODEL_EXISTS, modelFileName));
                }
            ...
            ContentWriter writer =
                    contentService.getWriter(modelNodeRef, ContentModel.PROP_CONTENT, true);

            writer.setMimetype(MimetypeMap.MIMETYPE_XML);
            writer.setEncoding("UTF-8");

            writer.putContent(modelStream); // also invokes policies for DictionaryModelType - e.g.
            // onContentUpdate
            modelStream.close();

            // activate the model
            nodeService.setProperty(
                    modelNodeRef, ContentModel.PROP_MODEL_ACTIVE, Boolean.valueOf(activate));
    ...
    private QName activateOrDeactivate(String modelFileName, boolean activate) {
        ...
        Boolean value =
                (Boolean) nodeService.getProperty(modelNodeRef, ContentModel.PROP_MODEL_ACTIVE);
        if (value != null) {
        ...
        if (activate) {
            ...
            if (isActive) {
                if (modelDef != null) {
                    ...
                    throw new AlfrescoRuntimeException(
                            MODEL_ALREADY_ACTIVATED, new Object[] {modelQName});
                } else {
                    logger.warn(
                            "Model is set to active but not loaded in Dictionary - trying to"
                                    + " load...");
                }
            } else {
                if (modelDef != null) {
                    logger.warn(
                            "Model is loaded in Dictionary but is not set to active - trying to"
                                    + " activate...");
                }
            }
        } else {
            ...
            if (!isActive) {
                if (modelDef == null) {
                    ...
                    throw new AlfrescoRuntimeException(
                            MODEL_ALREADY_DEACTIVATED, new Object[] {modelQName});
                } else {
                    logger.warn(
                            "Model is set to inactive but loaded in Dictionary - trying to"
                                    + " unload...");
                }
            } else {
                if (modelDef == null) {
                    logger.warn(
                            "Model is not loaded in Dictionary but is set to active - trying to"
                                    + " deactivate...");
                }
            }
        }
        ...
        nodeService.setProperty(
                modelNodeRef, ContentModel.PROP_MODEL_ACTIVE, new Boolean(activate));
    ...
    public QName undeployModel(String modelFileName) {
        ...
        try {
            ...
            try {
                ...
                // for now, assume concurrency failure
                throw new ConcurrencyFailureException(
                        getLocalisedMessage(MODEL_NO_LONGER_EXISTS, modelFileName));
            }
            ...
            if (nodeRefs.size() == 0) {
                ...
                // unexpected: should not find multiple nodes with same name
                throw new AlfrescoRuntimeException(
                        MODELS_MULTIPLE_FOUND, new Object[] {modelFileName});
            }
    ...
    public String deployMessageBundle(String resourceClasspath) {
        ...
        String pattern =
                "classpath*:" + resourceClasspath + "*" + MessageServiceImpl.PROPERTIES_FILE_SUFFIX;
        ...
        try {
            ...
            if ((resources != null) && (resources.length > 0)) {
                ...
                throw new AlfrescoRuntimeException(
                        MSG_RESOURCES_NOT_FOUND, new Object[] {resourceClasspath});
            }
        } catch (Throwable e) {
            throw new AlfrescoRuntimeException(
                    MSG_RESOURCES_DEPLOYMENT_FAILED, new Object[] {resourceClasspath}, e);
        }
     ...
     */
    private void deployMessageResourceFile(
        ...
        try {
            ...
            if (nodeRefs.size() == 0) {
                throw new AlfrescoRuntimeException(
                        MESSAGES_LOCATION_NOT_FOUND, new Object[] {repoMessagesLocation.getPath()});
            } else if (nodeRefs.size() > 1) {
                // unexpected: should not find multiple nodes with same name
                throw new AlfrescoRuntimeException(
            ...
            ContentWriter writer =
                    contentService.getWriter(content, ContentModel.PROP_CONTENT, true);
    ...
    public void undeployMessageBundle(String bundleBaseName) {
        ...
        try {
            ...
            // unregister bundle
            String repoBundlePath =
                    storeRef.toString() + repoMessagesLocation.getPath() + "/cm:" + bundleBaseName;
            ...
            List<NodeRef> nodeRefs =
                    searchService.selectNodes(
                            rootNode,
                            repoMessagesLocation.getPath() + CRITERIA_ALL,
                            null,
                            namespaceService,
                            false);
            ...
            if (found) {
                ...
                throw new AlfrescoRuntimeException(
                        MSG_RESOURCES_NOT_FOUND, new Object[] {repoBundlePath});
            }
        } catch (Throwable t) {
    ...
    private void checkBundleBaseName(String bundleBaseName) {
        ...
        if (bundleBaseName.indexOf("_") != -1) {
            ...
            throw new AlfrescoRuntimeException(
                    MSG_BASE_NAME_CONTAIN_UNDERSCORE, new Object[] {bundleBaseName});
        }

        if (bundleBaseName.indexOf(".") != -1) {
            throw new AlfrescoRuntimeException(
                    MSG_BASE_NAME_CONTAIN_PERIOD, new Object[] {bundleBaseName});
        }
    }

...
public class StandardQuotaStrategy
  ...
  private void loadDiskUsage() {
    ...
    if (!usageFile.exists()) {
      ...
      if (log.isInfoEnabled()) {
        log.info(
          "Using last known disk usage estimate: " + getCurrentUsageBytes()
        );
      }
    }
  }
   ...
   */
  private void signalCleanerStart(
    final String reason,
    final boolean aggressive
  ) {
    if (aggressive) {
      long targetReductionBytes = (long) (
        ((double) targetUsagePct / 100) * maxUsageBytes
      );
      cleaner.executeAggressive(reason, targetReductionBytes);
    } else {
      long timePassedFromLastClean =
        System.currentTimeMillis() - lastCleanupStart.get();
  ...
  public long addUsageBytes(long sizeDelta) {
    ...
    if (log.isDebugEnabled()) {
      log.debug(
        String.format(
          "Disk usage changed by %d to %d bytes",
          sizeDelta,
          newUsage
        )
      );

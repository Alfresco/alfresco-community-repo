...
public class DictionaryDAOImpl
        implements DictionaryDAO, NamespaceDAO, ApplicationListener<ApplicationEvent> {
    // Tenant Service
     ...
     *
     * <p>This method is <b>deprecated</b>, use {@link #registerListener(DictionaryListener
     * dictionaryListener)} instead.
     *
    ...
    /**
     * @see
     *     org.alfresco.repo.dictionary.DictionaryDAO#removeModel(org.alfresco.service.namespace.QName)
     */
    ...
    public Collection<ConstraintDefinition> getConstraints(
            QName modelName, boolean referenceableDefsOnly) {
        CompiledModel model = getCompiledModel(modelName);
     ...
     *
     * <p>If the input model does not exist in the Dictionary then no diffs will be returned.
     *
     * @param model M2Model
     ...
     *
     * <p>note: - checks classes (types & aspects) for incremental updates - checks properties for
     * incremental updates, but does not include the diffs - checks assocs & child assocs for
     * incremental updates, but does not include the diffs - incremental updates include changes in
     * title/description, property default value, etc - ignores changes in model definition except
     * name (ie. title, description, author, published date, version are treated as an incremental
     * update)
     *
     * <p>TODO - imports - namespace - datatypes - constraints (including property constraints -
     * references and inline)
     *
    ...
    /* package */ List<M2ModelDiff> diffModel(CompiledModel previousVersion, CompiledModel model) {
        ...
        if (previousVersion != null) {
            ...
            Collection<ConstraintDefinition> previousConDefs =
                    getReferenceableConstraintDefs(previousVersion);
            Collection<NamespaceDefinition> previousImportedNamespaces =
                    previousVersion.getModelDefinition().getImportedNamespaces();
            ...
            if (model == null) {
                ...
                Collection<NamespaceDefinition> importedNamespaces =
                        model.getModelDefinition().getImportedNamespaces();

                if (previousTypes.size() != 0) {
                    M2ModelDiffs.addAll(
                            M2ClassDefinition.diffClassLists(
                ...
                if (previousConDefs.size() != 0) {
                    M2ModelDiffs.addAll(
                            M2ConstraintDefinition.diffConstraintLists(
                ...
                if (previousImportedNamespaces.size() != 0) {
                    M2ModelDiffs.addAll(
                            M2NamespaceDefinition.diffNamespaceDefinitionLists(
                    ...
                    for (NamespaceDefinition namespaceDefinition : importedNamespaces) {
                        M2ModelDiffs.add(
                                new M2ModelDiff(
                                        namespaceDefinition.getModel().getName(),
                                        namespaceDefinition,
    ...
    public Collection<String> getPrefixes() {
        return Collections.unmodifiableCollection(
                getTenantDictionaryRegistry().getPrefixesCache().keySet());
    }

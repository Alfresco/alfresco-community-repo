 ...
 *
 * <p>By default it will throw an exception if any method is called.
 *
 * <p>Any of the getter/setter methods can be enabled with a no action implementation.
 *
 * <p>This can support deleting users via the UI for LDAP and NTLM. The Alfresco person object is
 * deleted from the UI. The call to delete the user will return with no action.
 *
 * <p>The following methods will always fail.
 *
 * <p>getMD4HashedPassword(String userName) loadUserByUsername(String arg0) getSalt(UserDetails
 * user)
 *
 * @author Andy Hind
 */

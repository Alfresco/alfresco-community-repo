...
public class DateParameterProcessor
    ...
    if (!value.isEmpty()) {
      ...
      if (DAY.equalsIgnoreCase(field)) {
        ...
        throw new AlfrescoRuntimeException(
          "Cannot process the field '" + field + "'."
        );
      }
    }
  ...
  private String handleDay(String[] values) {
    ...
    if (SHORT.equalsIgnoreCase(style)) {
      ...
      throw new AlfrescoRuntimeException(
        "The pattern 'date.day." + style + "' is not supported!"
      );
    }
  ...
  private String handleMonth(String[] values) {
    ...
    if (SHORT.equalsIgnoreCase(style)) {
      ...
      throw new AlfrescoRuntimeException(
        "The pattern 'date.month." + style + "' is not supported!"
      );
    }
  ...
  private String handleYear(String[] values) {
    ...
    if (SHORT.equalsIgnoreCase(style)) {
      ...
      throw new AlfrescoRuntimeException(
        "The pattern 'date.year." + style + "' is not supported!"
      );
    }
  ...
  public List<String> getSubstitutionSuggestions(String substitutionFragment) {
    ...
    if (
      StringUtils.isBlank(substitutionFragment) ||
      this.getName().toLowerCase().contains(substitutionFragment.toLowerCase())
    ) {
      ...
      for (String field : ALL_FIELDS_FOR_SUBSTITUTION_QUERY) {
        ...
        if (
          prefixFieldName
            .toLowerCase()
            .contains(substitutionFragment.toLowerCase())
        ) {
          suggestions.add(namePrefix + field);
          if (suggestions.size() >= maximumNumberSuggestions) {
            break;

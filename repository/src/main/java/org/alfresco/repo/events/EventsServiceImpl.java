...
public class EventsServiceImpl
  ...
  private static final String RM_MODEL_PROP_NAME_RECORD_ORIGINATING_LOCATION =
    "PROP_RECORD_ORIGINATING_LOCATION";
  private static final String RECORDS_MANAGEMENT_MODEL =
    "org.alfresco.module.org_alfresco_module_rm.model.RecordsManagementModel";

  public static QName PROP_RMA_RECORD_ORIGINATING_LOCATION = getRmPropOriginatingLocation();

  private NodeRenamedEvent nodeRenamedEvent(
    NodeInfo nodeInfo,
    String oldName,
    String newName
  ) {
  ...
  public void nodeMoved(
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      String oldParentNodeName = (String) nodeService.getProperty(
        oldParentNodeRef,
        ContentModel.PROP_NAME
      );
      String newParentNodeName = (String) nodeService.getProperty(
        newParentNodeRef,
        ContentModel.PROP_NAME
      );
      String oldNodeName = (String) nodeService.getProperty(
        oldNodeRef,
        ContentModel.PROP_NAME
      );
      String newNodeName = (String) nodeService.getProperty(
        newNodeRef,
        ContentModel.PROP_NAME
      );
      List<Path> newParentPaths = nodeService.getPaths(newParentNodeRef, false);
      List<String> newPaths = getPaths(
        newParentPaths,
        Arrays.asList(newParentNodeName, newNodeName)
      );
      ...
      if (!oldParentNodeRef.equals(newParentNodeRef)) {
        ...
        List<Path> oldParentPaths = nodeService.getPaths(
          oldParentNodeRef,
          false
        );
        List<String> previousPaths = getPaths(
          oldParentPaths,
          Arrays.asList(oldParentNodeName, oldNodeName)
        );
        ...
        Event event = new NodeMovedEvent(
          nextSequenceNumber(),
          oldNodeName,
          newNodeName,
          txnId,
  ...
  public void nodeRenamed(NodeRef nodeRef, String oldName, String newName) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      NodeRenamedEvent nodeRenamedEvent = nodeRenamedEvent(
        nodeInfo,
        oldName,
        newName
      );
  ...
  public void nodeTagged(final NodeRef nodeRef, final String tag) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      Event event = new NodeTaggedEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  public void nodeTagRemoved(final NodeRef nodeRef, final String tag) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      Event event = new NodeUnTaggedEvent(
        nextSequenceNumber(),
        tag,
        name,
        txnId,
  ...
  public void nodeLiked(final NodeRef nodeRef) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      Event event = new NodeLikedEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  public void nodeUnLiked(final NodeRef nodeRef) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      Event event = new NodeUnLikedEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  public void nodeFavourited(NodeRef nodeRef) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      Event event = new NodeFavouritedEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  public void nodeUnFavourited(NodeRef nodeRef) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      NodeUnFavouritedEvent event = new NodeUnFavouritedEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  public void nodeCreated(final NodeRef nodeRef) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      Event event = new NodeAddedEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  @Override
  public void secondaryAssociationCreated(
    final ChildAssociationRef secAssociation
  ) {
    NodeInfo nodeInfo = getNodeInfo(
      secAssociation.getChildRef(),
      NodeAddedEvent.EVENT_TYPE
    );
    if (nodeInfo.checkNodeInfo()) {
      String username = AuthenticationUtil.getFullyAuthenticatedUser();
      String networkId = TenantUtil.getCurrentDomain();
      ...
      String secParentNodeName = (String) nodeService.getProperty(
        secAssociation.getParentRef(),
        ContentModel.PROP_NAME
      );
      List<Path> secParentPath = nodeService.getPaths(secParentNodeRef, true);
      List<String> nodePaths = getPaths(
        secParentPath,
        Arrays.asList(secParentNodeName, name)
      );
  ...
  @Override
  public void secondaryAssociationDeleted(
    final ChildAssociationRef secAssociation
  ) {
    NodeInfo nodeInfo = getNodeInfo(
      secAssociation.getChildRef(),
      NodeRemovedEvent.EVENT_TYPE
    );
    if (nodeInfo.checkNodeInfo()) {
      String username = AuthenticationUtil.getFullyAuthenticatedUser();
      String networkId = TenantUtil.getCurrentDomain();
      ...
      Event event = new NodeRemovedEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  public void nodeCommented(final NodeRef nodeRef, final String comment) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      Event event = new NodeCommentedEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  public void nodeUpdated(
    ...
    final Map<String, Property> propertiesAdded,
    final Set<String> propertiesRemoved,
    final Map<String, Property> propertiesChanged,
    final Set<String> aspectsAdded,
    final Set<String> aspectsRemoved
  ) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      Event event = new NodeUpdatedEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  public void contentGet(NodeRef nodeRef) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      Event event = new NodeContentGetEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  public void contentWrite(
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      Event event = new NodeContentPutEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  public void nodeCheckedOut(NodeRef workingCopyNodeRef) {
    NodeInfo workingCopyNodeInfo = getNodeInfo(
      workingCopyNodeRef,
      NodeCheckedOutEvent.EVENT_TYPE
    );
    if (workingCopyNodeInfo.checkNodeInfo()) {
      ...
      String nodeType = workingCopyNodeInfo
        .getType()
        .toPrefixString(namespaceService);
      Client alfrescoClient = getAlfrescoClient(
        workingCopyNodeInfo.getClient()
      );
      ...
      Event event = new NodeCheckedOutEvent(
        nextSequenceNumber(),
        workingCopyNodeId,
        name,
        txnId,
  ...
  public void nodeCheckOutCancelled(NodeRef nodeRef) {
    NodeInfo nodeInfo = getNodeInfo(
      nodeRef,
      NodeCheckOutCancelledEvent.EVENT_TYPE
    );
    if (nodeInfo.checkNodeInfo()) {
      String username = AuthenticationUtil.getFullyAuthenticatedUser();
      String networkId = TenantUtil.getCurrentDomain();
      ...
      Event event = new NodeCheckOutCancelledEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  public void nodeCheckedIn(NodeRef nodeRef) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      Event event = new NodeCheckedInEvent(
        nextSequenceNumber(),
        name,
        txnId,
        timestamp,
  ...
  @Override
  public void authorityRemovedFromGroup(
    String parentGroup,
    ...
    if (includeEventType(AuthorityRemovedFromGroupEvent.EVENT_TYPE)) {
      ...
      String txnId = AlfrescoTransactionSupport.getTransactionId();
      long timestamp = System.currentTimeMillis();
      Client client = getAlfrescoClient(
        ClientUtil.from(FileFilterMode.getClient())
      );

      Event event = AuthorityRemovedFromGroupEvent
        .builder()
        .parentGroup(parentGroup)
        .authorityName(childAuthority)
  ...
  public void authorityAddedToGroup(String parentGroup, String childAuthority) {
    if (includeEventType(AuthorityAddedToGroupEvent.EVENT_TYPE)) {
      ...
      Event event = AuthorityAddedToGroupEvent
        .builder()
        .parentGroup(parentGroup)
        .authorityName(childAuthority)
        .seqNumber(nextSequenceNumber())
  ...
  public void inheritPermissionsEnabled(NodeRef nodeRef) {
    NodeInfo nodeInfo = getNodeInfo(
      nodeRef,
      InheritPermissionsEnabledEvent.EVENT_TYPE
    );
    if (nodeInfo.checkNodeInfo()) {
      String username = AuthenticationUtil.getFullyAuthenticatedUser();
      String networkId = TenantUtil.getCurrentDomain();
      ...
      Event event = InheritPermissionsEnabledEvent
        .builder()
        .seqNumber(nextSequenceNumber())
        .name(name)
        .txnId(txnId)
  ...
  public void inheritPermissionsDisabled(NodeRef nodeRef, boolean async) {
    NodeInfo nodeInfo = getNodeInfo(
      nodeRef,
      InheritPermissionsDisabledEvent.EVENT_TYPE
    );
    if (nodeInfo.checkNodeInfo()) {
      String username = AuthenticationUtil.getFullyAuthenticatedUser();
      String networkId = TenantUtil.getCurrentDomain();
      ...
      Event event = InheritPermissionsDisabledEvent
        .builder()
        .async(async)
        .seqNumber(nextSequenceNumber())
        .name(name)
  ...
  @Override
  public void revokeLocalPermissions(
    NodeRef nodeRef,
    String authority,
    String permission
  ) {
    NodeInfo nodeInfo = getNodeInfo(
      nodeRef,
      LocalPermissionRevokedEvent.EVENT_TYPE
    );
    if (nodeInfo.checkNodeInfo()) {
      String username = AuthenticationUtil.getFullyAuthenticatedUser();
      String networkId = TenantUtil.getCurrentDomain();
      ...
      Event event = LocalPermissionRevokedEvent
        .builder()
        .authority(authority)
  ...
  @Override
  public void grantLocalPermission(
    NodeRef nodeRef,
    String authority,
    String permission
  ) {
    NodeInfo nodeInfo = getNodeInfo(
      nodeRef,
      LocalPermissionGrantedEvent.EVENT_TYPE
    );
    if (nodeInfo.checkNodeInfo()) {
      String username = AuthenticationUtil.getFullyAuthenticatedUser();
      String networkId = TenantUtil.getCurrentDomain();
      ...
      LocalPermissionGrantedEvent event = LocalPermissionGrantedEvent
        .builder()
        .authority(authority)
        .permission(permission)
        .seqNumber(nextSequenceNumber())
  ...
  public void fileUnclassified(NodeRef nodeRef) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      Event event = FileUnclassifiedEvent
        .builder()
        .seqNumber(nextSequenceNumber())
  ...
  public void fileClassified(NodeRef nodeRef) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      Event event = FileClassifiedEvent
        .builder()
        .seqNumber(nextSequenceNumber())
        .name(nodeInfo.getName())
        .txnId(AlfrescoTransactionSupport.getTransactionId())
  ...
  public void recordRejected(NodeRef nodeRef) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      if (PROP_RMA_RECORD_ORIGINATING_LOCATION == null) {
        logger.error(
          format(
            "Could not generate %s event for node %s because %s property is not found.",
            RecordRejectedEvent.EVENT_TYPE,
            nodeRef,
            RM_MODEL_PROP_NAME_RECORD_ORIGINATING_LOCATION
          )
        );
        return;
      }
      NodeRef recordOriginatingLocation = (NodeRef) nodeService.getProperty(
        nodeRef,
        PROP_RMA_RECORD_ORIGINATING_LOCATION
      );
      String recordOriginatingParentName = (String) nodeService.getProperty(
        recordOriginatingLocation,
        ContentModel.PROP_NAME
      );
      Path originatingParentPath = nodeService.getPath(
        recordOriginatingLocation
      );

      Event event = RecordRejectedEvent
        .builder()
        .seqNumber(nextSequenceNumber())
        .name(nodeInfo.getName())
        .txnId(AlfrescoTransactionSupport.getTransactionId())
        ...
        .paths(
          getPaths(
            singletonList(originatingParentPath),
            asList(recordOriginatingParentName, nodeInfo.getName())
          )
        )
  ...
  private static QName getRmPropOriginatingLocation() {
    ...
    try {
      Class<?> recordsManagementModel = ClassUtils.getClass(
        RECORDS_MANAGEMENT_MODEL
      );
      originatingLocation =
        (QName) FieldUtils.readStaticField(
      ...
      logger.info(
        format(
          "Could not retrieve property %s from class %s. Maybe RM isn't installed, the property %s will be null.",
          RM_MODEL_PROP_NAME_RECORD_ORIGINATING_LOCATION,
          RECORDS_MANAGEMENT_MODEL,
          "PROP_RMA_RECORD_ORIGINATING_LOCATION"
        )
  ...
  public void recordCreated(NodeRef nodeRef) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      ...
      Event event = RecordCreatedEvent
        .builder()
        .seqNumber(nextSequenceNumber())
        .name(nodeInfo.getName())
        .txnId(AlfrescoTransactionSupport.getTransactionId())
  ...
  public void nodeLocked(NodeRef nodeRef) {
    ...
    if (nodeInfo.checkNodeInfo()) {
      NodeLockedEvent event = new NodeLockedEvent(
        nextSequenceNumber(),
        nodeInfo.getName(),
        ...
        nodeInfo.getSiteId(),
        nodeInfo.getNodeId(),
        nodeInfo.getType().toPrefixString(namespaceService),
        nodeInfo.getPaths(),
        nodeInfo.getParentNodeIds(),
        AuthenticationUtil.getFullyAuthenticatedUser(),
        nodeInfo.getModificationTimestamp(),
        getAlfrescoClient(nodeInfo.getClient()),

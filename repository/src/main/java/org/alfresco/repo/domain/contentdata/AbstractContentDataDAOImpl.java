...
public abstract class AbstractContentDataDAOImpl implements ContentDataDAO {
   ...
   */
  private static final String KEY_PRE_COMMIT_CONTENT_URL_DELETIONS =
    "AbstractContentDataDAOImpl.PreCommitContentUrlDeletions";

  private static Log logger = LogFactory.getLog(
    AbstractContentDataDAOImpl.class
  );
  ...
  public AbstractContentDataDAOImpl() {
    ...
    this.contentDataCache =
      new EntityLookupCache<Long, ContentData, Serializable>(
        contentDataCallbackDAO
      );
    this.contentUrlCache =
      new EntityLookupCache<Long, ContentUrlEntity, String>(
   ...
   */
  public void setContentStoreCleaner(
    EagerContentStoreCleaner contentStoreCleaner
  ) {
   ...
   */
  public void setContentDataCache(
    SimpleCache<Long, ContentData> contentDataCache
  ) {
    this.contentDataCache =
      new EntityLookupCache<Long, ContentData, Serializable>(
        contentDataCache,
  ...
  public void setContentUrlCache(
    SimpleCache<Long, ContentUrlEntity> contentUrlCache
  ) {
    this.contentUrlCache =
      new EntityLookupCache<Long, ContentUrlEntity, String>(
        contentUrlCache,
  ...
  protected void registerDereferencedContentUrl(String contentUrl) {
    Set<String> contentUrls = TransactionalResourceHelper.getSet(
      KEY_PRE_COMMIT_CONTENT_URL_DELETIONS
    );
  ...
  public Pair<Long, ContentData> createContentData(ContentData contentData) {
    ...
    Pair<Long, ContentData> entityPair = contentDataCache.getOrCreateByValue(
      contentData
    );
    return entityPair;
  }
  ...
  public Pair<Long, ContentData> getContentData(Long id) {
    if (id == null) {
      throw new IllegalArgumentException(
        "Cannot look up ContentData by null ID."
      );
    }
    Pair<Long, ContentData> entityPair = contentDataCache.getByKey(id);
    if (entityPair == null) {
      throw new DataIntegrityViolationException(
        "No ContentData value exists for ID " + id
      );
    }
    return entityPair;
  }
  ...
  public void updateContentData(Long id, ContentData contentData) {
    ...
    if (contentData == null) {
      throw new IllegalArgumentException(
        "Cannot update ContentData with a null."
      );
    }
    ...
    if (updated < 1) {
      throw new ConcurrencyFailureException(
        "ContentData with ID " + id + " not updated"
      );
    }
  }
  ...
  public void deleteContentData(Long id) {
    if (id == null) {
      throw new IllegalArgumentException(
        "Cannot delete ContentData by null ID."
      );
    }
    int deleted = contentDataCache.deleteByKey(id);
    if (deleted < 1) {
      throw new ConcurrencyFailureException(
        "ContentData with ID " + id + " no longer exists"
      );
    }
    return;
  }
  ...
  private class ContentDataCallbackDAO
    extends EntityLookupCallbackDAOAdaptor<Long, ContentData, Serializable> {
  ...
  private class ContentUrlCallbackDAO
    extends EntityLookupCallbackDAOAdaptor<Long, ContentUrlEntity, String> {
    ...
    public Pair<Long, ContentUrlEntity> findByValue(ContentUrlEntity entity) {
      ...
      return (
        ret != null ? new Pair<Long, ContentUrlEntity>(ret.getId(), ret) : null
      );
    }

    public Pair<Long, ContentUrlEntity> createValue(ContentUrlEntity value) {
      ContentUrlEntity contentUrlEntity = createContentUrlEntity(
        value.getContentUrl(),
        value.getSize(),
        value.getContentUrlKey()
      );
      // Done
      return new Pair<Long, ContentUrlEntity>(
  ...
  private ContentData makeContentData(ContentDataEntity contentDataEntity) {
    ...
    if (contentUrlId != null) {
      Pair<Long, ContentUrlEntity> entityPair = contentUrlCache.getByKey(
        contentUrlId
      );
      if (entityPair == null) {
        throw new DataIntegrityViolationException(
          "No ContentUrl value exists for ID " + contentUrlId
        );
      }
    ...
    long size = contentDataEntity.getSize() == null
      ? 0L
      : contentDataEntity.getSize().longValue();
  ...
  protected ContentDataEntity createContentDataEntity(ContentData contentData) {
    ...
    if (contentUrl != null) {
      ...
      Pair<Long, ContentUrlEntity> pair = contentUrlCache.createOrGetByValue(
        contentUrlEntity,
        controlDAO
      );
    ...
    ContentDataEntity contentDataEntity = createContentDataEntity(
      contentUrlId,
      mimetypeId,
      encodingId,
      localeId
   ...
   * Translates the {@link ContentData} into persistable values using the helper DAOs
   */
  protected int updateContentDataEntity(
    ...
    if (oldContentUrlId != null) {
      ...
      if (entityPair == null) {
        throw new DataIntegrityViolationException(
          "No ContentUrl value exists for ID " + oldContentUrlId
        );
      }
    ...
    String oldContentUrl =
      (contentUrlEntity != null ? contentUrlEntity.getContentUrl() : null);
    ...
    if (!EqualsHelper.nullSafeEquals(oldContentUrl, newContentUrl)) {
      ...
      if (newContentUrl != null) {
        ...
        Pair<Long, ContentUrlEntity> pair = contentUrlCache.getOrCreateByValue(
          contentUrlEntity
        );
  ...
  @Override
  public boolean updateContentUrlKey(
    String contentUrl,
    ...
    if (existing == null) {
      existing =
        getOrCreateContentUrl(
          contentUrl,
          contentUrlKey.getUnencryptedFileSize()
        );
    }
    ContentUrlEntity entity = ContentUrlEntity.setContentUrlKey(
      existing,
      contentUrlKey
    );
   ...
   */
  protected abstract ContentUrlEntity createContentUrlEntity(
    String contentUrl,
    long size,
    ContentUrlKeyEntity contentUrlKey
  );
   ...
   */
  protected abstract ContentUrlEntity getContentUrlEntityUnreferenced(
    String contentUrl
  );
   ...
   */
  protected abstract int updateContentUrlOrphanTime(
    Long id,
    Long orphanTime,
    Long oldOrphanTime
  );
   ...
   */
  protected abstract List<ContentDataEntity> getContentDataEntitiesForNodes(
    Set<Long> nodeIds
  );
  ...
  protected abstract int updateContentUrlEntity(
  ...
  public class ContentUrlDeleteTransactionListener
    ...
    public void beforeCommit(boolean readOnly) {
      ...
      for (String contentUrl : contentUrls) {
        ContentUrlEntity contentUrlEntity = getContentUrlEntityUnreferenced(
          contentUrl
        );
        if (contentUrlEntity == null) {
        ...
        boolean isEagerCleanup = contentStoreCleaner.registerOrphanedContentUrl(
          contentUrl
        );
        if (!isEagerCleanup) {
          ...
          int updated = updateContentUrlOrphanTime(
            contentUrlId,
            orphanTime,
            oldOrphanTime
          );
          ...
          List<Long> contentUrlId = Collections.singletonList(
            contentUrlEntity.getId()
          );

...
public class GetChildrenCannedQuery
  ...
  private static final String QUERY_SELECT_GET_CHILDREN_WITH_PROPS =
    "select_GetChildrenCannedQueryWithProps";
  private static final String QUERY_SELECT_GET_CHILDREN_WITHOUT_PROPS =
    "select_GetChildrenCannedQueryWithoutProps";
  ...
  public static final QName SORT_QNAME_CONTENT_SIZE = QName.createQName(
    "http://www.alfresco.org/model/content/1.0",
    "content.size"
  );
  public static final QName SORT_QNAME_CONTENT_MIMETYPE = QName.createQName(
    "http://www.alfresco.org/model/content/1.0",
    "content.mimetype"
  );
  public static final QName SORT_QNAME_NODE_TYPE = QName.createQName(
    "",
    "TYPE"
  );
  public static final QName SORT_QNAME_NODE_IS_FOLDER = QName.createQName(
    "",
    "IS_FOLDER"
  ); // ALF-13968

  public static final QName FILTER_QNAME_NODE_IS_PRIMARY = QName.createQName(
    "",
    "IS_PRIMARY"
  );
  ...
  public GetChildrenCannedQuery(
    ...
    if (
      (params.getSortDetails() != null) &&
      (params.getSortDetails().getSortPairs().size() > 0)
    ) {
    ...
    if (
      (paramBean.getFilterProps() != null) &&
      (paramBean.getFilterProps().size() > 0)
    ) {
      applyPostQueryPermissions = true;
    }
  }

  protected FilterSortChildQueryCallback getFilterSortChildQuery(
    final List<FilterSortNode> children,
    final List<FilterProp> filterProps,
    ...
    return new DefaultFilterSortChildQueryCallback(
      children,
      filterProps,
      inclusiveAspects,
      exclusiveAspects
  ...
  protected UnsortedChildQueryCallback getUnsortedChildQueryCallback(
    final List<NodeRef> rawResult,
    final int requestedCount,
    GetChildrenCannedQueryParams paramBean
  ) {
    Set<QName> inclusiveAspects = paramBean.getInclusiveAspects();
    Set<QName> exclusiveAspects = paramBean.getExclusiveAspects();
    return new DefaultUnsortedChildQueryCallback(
      rawResult,
      requestedCount,
      inclusiveAspects,
      exclusiveAspects
  ...
  protected List<NodeRef> queryAndFilter(CannedQueryParameters parameters) {
    ...
    if (nodePair == null) {
      throw new InvalidNodeRefException(
        "Parent node does not exist: " + parentRef,
        parentRef
      );
    }
    ...
    final List<FilterProp> filterProps = new ArrayList<>(
      paramBean.getFilterProps().size()
    );
    ...
    if (filterProps.size() > 0) {
      ...
      for (FilterProp filter : filterProps) {
        if (
          (filter instanceof FilterPropBoolean) &&
          ((FilterPropBoolean) filter).getPropName()
            .equals(FILTER_QNAME_NODE_IS_PRIMARY)
        ) {
    ...
    List<QName> sortFilterProps = new ArrayList<QName>(
      filterProps.size() + sortPairs.size()
    );
    ...
    if (filterSortPropCnt > MAX_FILTER_SORT_PROPS) {
      throw new AlfrescoRuntimeException(
        "GetChildren: exceeded maximum number filter/sort properties: (max=" +
    ...
    try {
      if ((childNodeTypeQNames != null) && (childNodeTypeQNames.size() > 0)) {
        ...
        if (childNodeTypeQNameIds.size() > 0) {
          params.setChildNodeTypeQNameIds(
            new ArrayList<Long>(childNodeTypeQNameIds)
          );
        } else {
      ...
      if (pattern != null) {
        ...
        if (nameQName == null) {
          throw new AlfrescoRuntimeException(
            "Unable to determine qname id of name property"
          );
        }
      ...
      if (filterSortPropCnt > 0) {
        ...
        final List<FilterSortNode> children = new ArrayList<FilterSortNode>(
          100
        );
        final FilterSortChildQueryCallback c = getFilterSortChildQuery(
          children,
          filterProps,
          paramBean
        );
        ...
        if (sortPairs.size() > 0) {
          Long startSort =
            (logger.isDebugEnabled() ? System.currentTimeMillis() : null);
        ...
        final List<NodeRef> rawResult = new ArrayList<NodeRef>(
          Math.min(1000, requestedCount)
        );
        UnsortedChildQueryCallback callback = getUnsortedChildQueryCallback(
          rawResult,
          requestedCount,
          paramBean
        );
        UnsortedResultHandler resultHandler = new UnsortedResultHandler(
          callback
        );
        cannedQueryDAO.executeQuery(
          QUERY_NAMESPACE,
      ...
      if (start != null) {
        logger.debug(
          "Base query " +
          (filterSortPropCnt > 0 ? "(sort=y, perms=n)" : "(sort=n, perms=y)") +
          ": " +
          result.size() +
          " in " +
  ...
  private int setFilterSortParams(
    List<QName> filterSortProps,
    ...
    for (QName filterSortProp : filterSortProps) {
      if (
        AuditablePropertiesEntity
          ...
          .contains(filterSortProp)
      ) {
        params.setAuditableProps(true);
      } else if (
        filterSortProp.equals(SORT_QNAME_NODE_TYPE) ||
        filterSortProp.equals(SORT_QNAME_NODE_IS_FOLDER)
      ) {
        ...
        if (sortQNameId != null) {
          if (propCnt == 0) {
            ...
            // belts and braces
            throw new AlfrescoRuntimeException(
              "GetChildren: unexpected - cannot set sort parameter: " + cnt
            );
          }
          ...
          logger.warn(
            "Skipping filter/sort param - cannot find: " + filterSortProp
          );
          break;
        }
      }
  ...
  private Long getQNameId(QName sortPropQName) {
    if (
      sortPropQName.equals(SORT_QNAME_CONTENT_SIZE) ||
      sortPropQName.equals(SORT_QNAME_CONTENT_MIMETYPE)
    ) {
  ...
  private class PropComparatorAsc implements Comparator<FilterSortNode> {
    ...
    private int compareImpl(
      FilterSortNode node1In,
      FilterSortNode node2In,
      ...
      boolean sortAscending =
        (sortProps.get(0).getSecond() == SortOrder.ASCENDING);
  ...
  @Override
  protected List<NodeRef> applyPostQueryPermissions(
    ...
    int maxChecks =
      (
        ((requestTotalCountMax > 0) && (requestTotalCountMax > requestedCount))
    ...
    List<NodeRef> ret = super.applyPostQueryPermissions(
      results,
      requestedCount
    );
  ...
  protected class DefaultFilterSortChildQueryCallback
    ...
    public DefaultFilterSortChildQueryCallback(
      final List<FilterSortNode> children,
      final List<FilterProp> filterProps
    ) {
    ...
    protected boolean include(FilterSortNode node) {
      // filter, if needed
      return (
        (!applyFilter || includeFilter(node.getPropVals(), filterProps)) &&
        includeAspects(node.getNodeRef(), inclusiveAspects, exclusiveAspects)
      );
    }
  }
  ...
  protected class DefaultUnsortedChildQueryCallback
    ...
    public DefaultUnsortedChildQueryCallback(
      final List<NodeRef> rawResult,
      final int requestedCount,
      Set<QName> inclusiveAspects,
      Set<QName> exclusiveAspects
  ...
  protected class FilterSortResultHandler
    ...
    private void preloadFilterSort() {
      ...
      for (FilterSortNodeEntity result : results) {
        ...
        Map<NodePropertyKey, NodePropertyValue> propertyValues = new HashMap<NodePropertyKey, NodePropertyValue>(
          3
        );
        ...
        Map<QName, Serializable> propVals = nodePropertyHelper.convertToPublicProperties(
          propertyValues
        );

        // Add referenceable / spoofed properties (including spoofed name if null)
        ReferenceablePropertiesEntity.addReferenceableProperties(
          node,
          propVals
        );
        ...
        ContentData contentData = (ContentData) propVals.get(
        ...
        if (auditableProps != null) {
          for (Map.Entry<QName, Serializable> entry : auditableProps
        ...
        boolean more = resultsCallback.handle(
          new FilterSortNode(nodeRef, propVals)
        );
        if (!more) {
          this.more = false;
          break;
  ...
  protected class FilterSortNode {
    ...
    public String toString() {
      return (
        "FilterSortNode [nodeRef=" + nodeRef + ", propVals=" + propVals + "]"
      );
    }
  ...
  private class UnsortedResultHandler
    ...
    private void preloadAndApplyPermissions() {
      ...
      List<NodeRef> results = applyPostQueryPermissions(
        nodeRefs,
        nodeRefs.size()
      );

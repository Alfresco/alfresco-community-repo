 ...
 *
 * <p>The cached string-date mappings are stored in a <tt>WeakHashMap</tt>.
 *
...
public class CachingDateFormat extends SimpleDateFormat {
    ...
    static ThreadLocal<SimpleDateFormat> S_LOCAL_DATE_FORMAT =
            ThreadLocal.withInitial(() -> newDateFormat(FORMAT_FULL_GENERIC));

    static ThreadLocal<SimpleDateFormat> S_LOCAL_DATEONLY_FORMAT =
            ThreadLocal.withInitial(() -> newDateFormat(FORMAT_DATE_GENERIC));

    static ThreadLocal<SimpleDateFormat> S_LOCAL_TIMEONLY_FORMAT =
            ThreadLocal.withInitial(() -> newDateFormat(FORMAT_TIME_GENERIC));

    static ThreadLocal<SimpleDateFormat> S_LOCAL_CMIS_SQL_DATETIME =
            ThreadLocal.withInitial(() -> newDateFormat(FORMAT_CMIS_SQL));

    static ThreadLocal<SimpleDateFormat> S_LOCAL_SOLR_DATETIME =
            ThreadLocal.withInitial(
                    () -> {
                        CachingDateFormat formatter =
                                newDateFormatWithLocale(FORMAT_SOLR, Locale.ENGLISH);
                        /*
                            SEARCH-1263
    ...
    static ThreadLocal<SimpleDateFormat> S_UTC_DATETIME_WITHOUT_MSECS =
            ThreadLocal.withInitial(
                    () -> {
                        CachingDateFormat formatter =
                                newDateFormatWithLocale(UTC_WITHOUT_MSECS, Locale.ENGLISH);
                        formatter.setTimeZone(TimeZone.getTimeZone(UTC));
                        return formatter;
                    });
                    ...
                    () ->
                            stream(LENIENT_FORMATS)
                                    .map(
                                            format -> {
                                                CachingDateFormat formatter =
                                                        new CachingDateFormat(format.string);
                                                formatter.setLenient(false);
                                                return new SimpleDateFormatAndResolution(
    ...
    /**
     * @param length the type of date format, e.g. {@link CachingDateFormat#LONG }
     * @param locale the <code>Locale</code> that will be used to determine the date pattern
    ...
    public static SimpleDateFormat getDateFormat(int length, Locale locale, boolean lenient) {
        SimpleDateFormat dateFormat =
                (SimpleDateFormat) CachingDateFormat.getDateInstance(length, locale);
    ...
    /**
     * @param dateLength the type of date format, e.g. {@link CachingDateFormat#LONG }
     * @param timeLength the type of time format, e.g. {@link CachingDateFormat#LONG }
     * @param locale the <code>Locale</code> that will be used to determine the date pattern
     * @see #getDateFormat(String, boolean)
     * @see CachingDateFormat#SHORT
     * @see CachingDateFormat#MEDIUM
    ...
    public static SimpleDateFormat getDateTimeFormat(
            int dateLength, int timeLength, Locale locale, boolean lenient) {
        SimpleDateFormat dateFormat =
                (SimpleDateFormat)
                        CachingDateFormat.getDateTimeInstance(dateLength, timeLength, locale);
        // extract the format string
        String pattern = dateFormat.toPattern();
        // we have a pattern to use
        return getDateFormat(pattern, lenient);
    ...
    /**
     * @param pattern the conversion pattern to use
     * @param lenient true to allow the parser to extract the date in conceivable manner
     * @return Returns a conversion-cacheing formatter for the given pattern, but the instance
    ...
    /**
     * Returns a thread-safe formatter for the Solr ISO 8601 datetime format (without the msecs
     * part).
     *
     * @see #UTC_WITHOUT_MSECS
     * @return Returns a thread-safe formatter for the Solr ISO 8601 datetime format (without the
     *     msecs part).
     */
    ...
    public static Pair<Date, Integer> lenientParse(String text, int minimumResolution)

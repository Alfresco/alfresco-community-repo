...
public class AssociationGet extends AbstractAssociationGet {
    ...
    protected QName getClassQname(WebScriptRequest req) {
        ...
        String shortClassName =
                req.getServiceMatch().getTemplateVars().get(DICTIONARY_SHORT_CLASS_NAME);
    ...
    protected QName getAssociationQname(WebScriptRequest req) {
        String associationName =
                req.getServiceMatch().getTemplateVars().get(DICTIONARY_ASSOCIATION_SHORTNAME);
        String associationPrefix =
                req.getServiceMatch().getTemplateVars().get(DICTIONARY_ASSOCIATION_PREFIX);

        if (associationPrefix == null) {
            throw new WebScriptException(
                    Status.STATUS_NOT_FOUND, "Missing parameter association prefix in the URL");
        }
        if (associationName == null) {
            throw new WebScriptException(
                    Status.STATUS_NOT_FOUND, "Missing parameter association name in the URL");
        }

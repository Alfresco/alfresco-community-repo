...
public class CompositePasswordEncoderTest {
  ...
  public void testInvalidParamsEncode() throws Exception {
    ...
    try {
      ...
      assertTrue(
        are.getMessage().contains("encoderKey is a mandatory parameter")
      );
    }
  ...
  public void testInvalidParamsMatches() throws Exception {
    ...
    try {
      ...
      assertTrue(
        are.getMessage().contains("encodedPassword is a mandatory parameter")
      );
    }
    ...
    try {
      ...
      assertTrue(
        are.getMessage().contains("Invalid matches encoder specified")
      );
      assertTrue(are.getMessage().endsWith("nonsense"));
    }
  ...
  public void testEncodeMD4() throws Exception {
    ...
    assertNotEquals(
      "The salt must be ignored for MD4",
      sourceEncoded,
      encoder.encodePassword(SOURCE_PASSWORD, salt, Arrays.asList("md4"))
    );

    encoded = encoder.encode("md4", SOURCE_PASSWORD, null);
    assertEquals(sourceEncodedSaltFree, encoded);
    assertTrue(
      encoder.matches("md4", SOURCE_PASSWORD, sourceEncodedSaltFree, null)
    );
    assertTrue(
      encoder.matchesPassword(
        SOURCE_PASSWORD,
        sourceEncodedSaltFree,
        null,
        Arrays.asList("md4")
      )
    );
    assertEquals(
      sourceEncodedSaltFree,
      encoder.encodePassword(SOURCE_PASSWORD, null, Arrays.asList("md4"))
    );
  ...
  public void testEncodeSha256() throws Exception {
    ...
    assertTrue(
      encoder.matchesPassword(
        ...
        encoded,
        salt,
        Arrays.asList("sha256")
      )
    );
    assertEquals(
      sourceEncoded,
      encoder.encodePassword(SOURCE_PASSWORD, salt, Arrays.asList("sha256"))
    );

    encoded = encoder.encode("sha256", SOURCE_PASSWORD, null);
    assertEquals(sourceEncodedSaltFree, encoded);
    assertTrue(
      encoder.matches("sha256", SOURCE_PASSWORD, sourceEncodedSaltFree, null)
    );
    assertTrue(
      encoder.matchesPassword(
        SOURCE_PASSWORD,
    ...
    assertEquals(
      sourceEncodedSaltFree,
      encoder.encodePassword(SOURCE_PASSWORD, null, Arrays.asList("sha256"))
    );
  ...
  public void testEncodeChain() throws Exception {
    ...
    assertTrue(
      encoder.matchesPassword(SOURCE_PASSWORD, encoded, null, mdbChain)
    );

    mdbChain = Arrays.asList("md4", "bcrypt10");
    encoded = encoder.encodePassword(SOURCE_PASSWORD, salt, mdbChain);
    assertTrue(
      encoder.matchesPassword(SOURCE_PASSWORD, encoded, salt, mdbChain)
    );
    ...
    encoded = encoder.encodePassword(SOURCE_PASSWORD, salt, mdbChain);
    assertFalse(
      "bcrypt10 has its own internal salt so needs to be at the end of the chain.",
      encoder.matchesPassword(SOURCE_PASSWORD, encoded, salt, mdbChain)
    );
    ...
    encoded = encoder.encodePassword(SOURCE_PASSWORD, salt, mdbChain);
    assertFalse(
      "bcrypt10 has its own internal salt so you can only use it once.",
      encoder.matchesPassword(SOURCE_PASSWORD, encoded, salt, mdbChain)
    );
  ...
  public void testUserChain() throws Exception {
    ...
    assertTrue(
      encoder.matchesPassword(rawPassword, nowEncoded, salt, nowHashed)
    );
    assertTrue(
      encoder.matchesPassword(rawPassword, nowEncoded2, salt, nowHashed)
    );
    assertTrue(
      encoder.matchesPassword(rawPassword, nowEncoded3, salt, nowHashed)
    );
  }
  ...
  public void testIsPreferredEncoding() throws Exception {
    ...
    assertFalse(
      subject.lastEncodingIsPreferred(Collections.<String>emptyList())
    );
    assertTrue(subject.lastEncodingIsPreferred(Arrays.asList("fish")));
    ...
    assertFalse(
      subject.lastEncodingIsPreferred(
        Arrays.asList("bird", "fish", "dog", "cat")
      )
    );
    assertTrue(
      subject.lastEncodingIsPreferred(
        Arrays.asList("bird", "dog", "cat", "fish")
      )
    );
  }
}

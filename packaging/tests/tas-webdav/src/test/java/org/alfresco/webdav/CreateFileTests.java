...
public class CreateFileTests extends WebDavTest {
  ...
  public void siteManageShouldCreateFileWithSpacesInName() throws Exception {
    testFile =
      new FileModel("test file " + RandomData.getRandomFile(FileType.PDF));
    webDavProtocol
      .authenticateUser(managerUser)
      .usingSite(testSite)
      .createFile(testFile)
  ...
  @Test(
    groups = {
      TestGroup.PROTOCOLS, TestGroup.WEBDAV, TestGroup.CORE, TestGroup.OS_LINUX,
    },
  ...
  public void siteManagerShouldNotCreateFileTwice() throws Exception {
    ...
    Assert.assertTrue(
      webDavProtocol.usingSite(twiceSite).getFiles().size() == 1
    );
  }

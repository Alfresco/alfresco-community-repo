...
public class ExceptionResolverTests {
    @Autowired ApiAssistant assistant;
    ...
    public void testWebscriptException() {
        ...
        assertEquals(
                HttpServletResponse.SC_UNAUTHORIZED,
                response.getStatusCode()); // default to INTERNAL_SERVER_ERROR
    }
    ...
    public void testMatchException() {
        ...
        response =
                assistant.resolveException(new RelationshipResourceNotFoundException(null, null));
        assertEquals(404, response.getStatusCode()); // default to STATUS_NOT_FOUND
    ...
    public void testQueryParserException() {
        ErrorResponse response =
                assistant.resolveException(new QueryParserException("Endpoint not found", 404));
        assertNotNull(response);
        assertEquals(

...
public class FileImportPackageHandler implements ImportPackageHandler {
   ...
   */
  public FileImportPackageHandler(
    File sourceDir,
    File dataFile,
  ...
  public Reader getDataStream() {
    try {
      ...
      Reader inputReader = (dataFileEncoding == null)
        ? new InputStreamReader(inputStream, DEFAULT_ENCODING)
        : new InputStreamReader(inputStream, dataFileEncoding);
  ...
  public InputStream importStream(String content) {
    ...
    try {
      ...
      throw new ImporterException(
        "Failed to read content url " +
        content +
        " from file " +
        fileURL.getAbsolutePath()
      );
    }
  }

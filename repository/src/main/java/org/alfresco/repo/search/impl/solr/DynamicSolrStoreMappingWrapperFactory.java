...
public class DynamicSolrStoreMappingWrapperFactory {
    static ConcurrentHashMap<Pair<String, Integer>, HttpClient> clients =
            new ConcurrentHashMap<Pair<String, Integer>, HttpClient>();
    ...
    public static SolrStoreMappingWrapper wrap(List<ShardInstance> slice, BeanFactory beanFactory) {
        HttpClientFactory httpClientFactory =
                (HttpClientFactory) beanFactory.getBean("solrHttpClientFactory");
        for (ShardInstance instance : slice) {
            Pair<String, Integer> key =
                    new Pair<String, Integer>(instance.getHostName(), instance.getPort());
        ...
        public String getShards() {
            try {
                ...
                for (ShardInstance instance : slice) {
                    ...
                    builder.append(
                            encoder.encode(
                                    client.getHostConfiguration().getProtocol().getScheme() + "://",
                                    "UTF-8"));

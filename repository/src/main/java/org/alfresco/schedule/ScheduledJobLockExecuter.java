...
public class ScheduledJobLockExecuter {
  ...
  private static Log logger = LogFactory.getLog(
    ScheduledJobLockExecuter.class.getName()
  );
  ...
  public void execute(JobExecutionContext jobContext)
    ...
    try {
      if (logger.isDebugEnabled()) {
        logger.debug(
          String.format("   Job %s started.", lockQName.getLocalName())
        );
      }
      refreshLock();
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          String.format("   Job %s completed.", lockQName.getLocalName())
        );
      }
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
          String.format("   Job %s aborted.", lockQName.getLocalName())
        );
      }
    } finally {
      releaseLock();

...
public class InviteNominatedSender extends InviteSender {
  ...
  private static final String SITE_DASHBOARD_ENDPOINT_PATTERN =
    "/page/site/{0}/dashboard";

  private static final List<String> INVITE_NOMINATED_EXPECTED_PROPERTIES = Arrays.asList(
  ...
  public void sendMail(
    ...
    NodeRef inviter = personService.getPerson(
      properties.get(wfVarInviterUserName)
    );
    ...
    if (StringUtils.isEmpty(recipient)) {
      logger.warn(
        "Cannot send invitation: Invitee user account does not have email"
      );
      return;
    }
    ...
    mail.setParameterValue(
      ...
      getEmailTemplateNodeRef(emailTemplateXpath)
    );
    mail.setParameterValue(
  ...
  protected Map<String, Serializable> buildMailTextModel(
    ...
    NodeRef invitee = personService.getPerson(
      properties.get(wfVarInviteeUserName)
    );
  ...
  private Map<String, String> buildArgs(
    ...
    String siteDashboardLink = makeLink(
      properties.get(wfVarServerPath),
      siteDashboardEndpoint,
      null,
      null
    );
    String siteLeaveLink = makeLink(
      properties.get(wfVarServerPath),
      siteDashboardEndpoint,
      null,
      SITE_LEAVE_HASH
  ...
  private String getRoleName(Map<String, String> properties) {
    ...
    String role = messageService.getMessage(

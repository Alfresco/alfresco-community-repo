...
public abstract class AbstractSiteActivityTest {
  ...
  public void setUp() throws Exception {
    ...
    feedProcessor =
      (LocalFeedTaskProcessor) activitiesFeedCtx.getBean("feedTaskProcessor");
  ...
  protected void createSite(String siteId, boolean isPublic) throws Exception {
    siteService.createSite(
      ...
      "myTitle",
      "myDescription",
      (isPublic ? SiteVisibility.PUBLIC : SiteVisibility.PRIVATE)
    );
  ...
  public void testGetSiteFeedsBefore() throws Exception {
    ...
    try {
      ...
      fail(
        "Site feed for private site should not be accessible to non-admin / non-member"
      );
  ...
  protected void getSiteFeed(String siteId, int expectedCount)
    throws Exception {
    assertEquals(
      expectedCount,
      activityService.getSiteFeedEntries(siteId).size()
    );
  }
  ...
  protected void getUserFeed(String userId, boolean isAdmin, int expectedCount)
    throws Exception {
    getUserFeed(userId, null, isAdmin, false, false, expectedCount);
  }

  protected void getUserFeed(
    String userId,
    String siteId,
    boolean isAdmin,
  ...
  protected void addAndUpdateMemberships(String siteId, boolean includeUser4)
    ...
    if (includeUser4) {
      addMembership(siteId, user4, SiteModel.SITE_CONSUMER);
    }
    ...
    if (includeUser4) {
      updateMembership(siteId, user4, SiteModel.SITE_COLLABORATOR);
    }
  }

  protected void removeMemberships(String siteId, boolean includeUser4)
    throws Exception {
    ...
    if (includeUser4) {
      removeMembership(siteId, user4);
    }
  }

  private void addFeedControl(String siteId, String appToolId)
    throws Exception {
  ...
  private void addMembership(String siteId, String userName, String role)
    throws Exception {
    updateMembership(siteId, userName, role);
  }

  private void updateMembership(String siteId, String userName, String role)
    throws Exception {
    siteService.setMembership(siteId, userName, role);
  }

  private void removeMembership(String siteId, String userName)
    throws Exception {

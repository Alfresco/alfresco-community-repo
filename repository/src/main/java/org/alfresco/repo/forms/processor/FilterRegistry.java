...
public class FilterRegistry<ItemType, PersistType> {
  ...
  public void addFilter(Filter<ItemType, PersistType> filter) {
    if (filter.isActive()) {
      ...
      if (logger.isDebugEnabled()) logger.debug(
        "Registered filter: " + filter + " in registry: " + this
      );
    } else if (logger.isWarnEnabled()) {
      logger.warn(
        "Ignored registration of filter " +
        filter +
  ...
  public List<Filter<ItemType, PersistType>> getFilters() {
    List<Filter<ItemType, PersistType>> activeFilters = new ArrayList<Filter<ItemType, PersistType>>(
      4
    );
    ...
    if (logger.isDebugEnabled()) logger.debug(
      "Returning active filters: " + activeFilters
    );

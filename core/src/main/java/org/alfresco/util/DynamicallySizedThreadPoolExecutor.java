...
/**
 * This is an instance of {@link java.util.concurrent.ThreadPoolExecutor} which behaves how one
 * would expect it to, even when faced with an unlimited queue. Unlike the default {@link
 * java.util.concurrent.ThreadPoolExecutor}, it will add new Threads up to {@link
 * #setMaximumPoolSize(int) maximumPoolSize} when there is lots of pending work, rather than only
 * when the queue is full (which it often never will be, especially for unlimited queues)
 *
 * @author Nick Burch
 */
public class DynamicallySizedThreadPoolExecutor extends ThreadPoolExecutor {
    ...
    public DynamicallySizedThreadPoolExecutor(
            int corePoolSize,
            int maximumPoolSize,
            long keepAliveTime,
            TimeUnit unit,
            ...
            if (queueSize >= getMaximumPoolSize()) {
                ...
                if (currentCoreSize < getMaximumPoolSize()) {
                    ...
                    if (logger.isInfoEnabled()) {
                        logger.info(
                                "Increased pool size to "
                                        + getCorePoolSize()
                                        + " from "
                                        + currentCoreSize
                                        + " due to queue size of "
                                        + queueSize);
                    }
                }
                lock.unlock();

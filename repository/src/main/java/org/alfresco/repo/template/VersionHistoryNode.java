...
/**
 * Template Node wrapper representing a record in the version history of a node. Provides access to
 * basic properties and version information for the frozen state record.
 *
 * @author Kevin Roast
 */
public class VersionHistoryNode extends BaseContentNode implements NamespacePrefixResolverProvider {
    ...
    public Map<String, Serializable> getProperties() {
        if (propsRetrieved == false) {
            Map<QName, Serializable> props =
                    parent.services
                            .getNodeService()
                            .getProperties(this.version.getFrozenStateNodeRef());

            for (QName qname : props.keySet()) {
                Serializable propValue =
                        parent.new TemplatePropertyConverter()
                                .convertProperty(
                                        props.get(qname),
                                        qname,
                                        parent.services,
                                        parent.imageResolver);
    ...
    public Set<QName> getAspects() {
        if (this.aspects == null) {
            this.aspects =
                    parent.services
                            .getNodeService()
                            .getAspects(this.version.getFrozenStateNodeRef());
        }
    ...
    /**
     * @return Returns the URL to the content stream for the frozen state of the node from the

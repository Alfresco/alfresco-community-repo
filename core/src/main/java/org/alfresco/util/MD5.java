...
/**
 * The MD5 utility class computes the MD5 digest (aka: "hash") of a block of data; an MD5 digest is
 * a 32-char ASCII string.
 *
 * <p>The synchronized/static function "Digest" is useful for situations where lock contention in
 * the application is not expected to be an issue.
 *
 * <p>The unsynchronized/non-static method "digest" is useful in a multi-threaded program that
 * wanted to avoid locking by creating an MD5 object for exclusive use by a single thread.
 *
 * <pre>
...
public class MD5 {
    ...
    /**
     * Constructor for use with the unsynchronized/non-static method "digest" method. Note that the
     * "digest" function is not thread-safe, so if you want to use it, every thread must create its
     * own MD5 instance. If you don't want to bother & are willing to deal with the potential for
     * lock contention, use the synchronized static "Digest" function instead of creating an

...
public class CategoryTemplateNode extends TemplateNode {
  ...
  public List<TemplateNode> getCategoryMembers() {
    if (getIsCategory()) {
      return buildTemplateNodeList(
        services
          .getCategoryService()
          .getChildren(
            getNodeRef(),
            CategoryService.Mode.MEMBERS,
  ...
  public List<TemplateNode> getMembersAndSubCategories() {
    if (getIsCategory()) {
      return buildMixedNodeList(
        services
          .getCategoryService()
          .getChildren(
            getNodeRef(),
            CategoryService.Mode.ALL,
            CategoryService.Depth.ANY
          )
      );
    } else {
   ...
   */
  private List<TemplateNode> buildTemplateNodeList(
  ...
  private List<CategoryTemplateNode> buildCategoryNodeList(
    Collection<ChildAssociationRef> childRefs
  ) {
    List<CategoryTemplateNode> answer = new ArrayList<CategoryTemplateNode>(
      childRefs.size()
    );
    for (ChildAssociationRef ref : childRefs) {
      // create our Node representation from the NodeRef
      CategoryTemplateNode child = new CategoryTemplateNode(
        ref.getChildRef(),
        this.services,
        this.imageResolver
      );
  ...
  private List<TemplateNode> buildMixedNodeList(
    Collection<ChildAssociationRef> cars
  ) {
    ...
    for (ChildAssociationRef car : cars) {
      ...
      if (
        ...
        nodes.add(
          new TemplateNode(car.getChildRef(), this.services, this.imageResolver)
        );
      }
    }
    return nodes;

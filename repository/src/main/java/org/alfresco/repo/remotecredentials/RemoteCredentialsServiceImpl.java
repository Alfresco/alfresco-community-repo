...
public class RemoteCredentialsServiceImpl implements RemoteCredentialsService {
    ...
    /**
     * The name of the System Container used to hold Shared Credentials. This isn't final, as unit
     * tests change it to avoid trampling on the real credentials.
     */
    ...
    /**
     * Controls which Factory will be used to create {@link BaseCredentialsInfo} instances for a
     * given node, based on the type. eg rc:passwordCredentials -> PasswordCredentialsFactory
     */
    private Map<QName, RemoteCredentialsInfoFactory> credentialsFactories =
            new HashMap<QName, RemoteCredentialsInfoFactory>();
    ...
    /**
     * Registers a new Factory to produce {@link BaseCredentialsInfo} objects for a given data type.
     * This provides an alternative to {@link #setCredentialsFactories(Map)} to allow the
     * registering of a new type without overriding all of them.
     *
    ...
    public void registerCredentialsFactory(
            QName credentialsType, RemoteCredentialsInfoFactory factory) {
        // Check the hierarchy is valid
        if (!dictionaryService.isSubClass(
                credentialsType, RemoteCredentialsModel.TYPE_CREDENTIALS_BASE)) {
            logger.warn(
                    "Unable to register credentials factory for "
                            + credentialsType
                            + " as that type doesn't inherit from "
                            + RemoteCredentialsModel.TYPE_CREDENTIALS_BASE);
            return;
        }
        ...
        if (logger.isDebugEnabled())
            logger.debug(
                    "Registering credentials factory for " + credentialsType + " of " + factory);
    ...
    private static QName SHARED_CREDENTIALS_CONTAINER_QNAME =
            QName.createQName(
                    NamespaceService.SYSTEM_MODEL_1_0_URI, SHARED_CREDENTIALS_CONTAINER_NAME);
    /**
     * Gets the NodeRef of the holder of shared credentials remote systems.
     *
     * <p>This is stored under system
     *
     * <p>Protected, so that unit tests can make use of it
     */
    protected NodeRef getSharedContainerNodeRef(boolean required) {
        // Get the container, if available
        NodeRef container =
                SystemNodeUtils.getSystemChildContainer(
                        SHARED_CREDENTIALS_CONTAINER_QNAME, nodeService, repositoryHelper);
        ...
        if (container == null && required) {
            ...
            synchronized (this) {
                details =
                        SystemNodeUtils.getOrCreateSystemChildContainer(
                                SHARED_CREDENTIALS_CONTAINER_QNAME, nodeService, repositoryHelper);
            }
                ...
                AuthenticationUtil.runAsSystem(
                        new RunAsWork<Void>() {
                            ...
                            public Void doWork() throws Exception {
                                ...
                                // By default, anyone can add children, and read, but not edit
                                // other's credentials
        ...
        if (container == null) {
            if (logger.isInfoEnabled())
                logger.info(
                        "Required System Folder "
                                + SHARED_CREDENTIALS_CONTAINER_QNAME
                                + " not yet created, will be lazy created on write");
            return null;
        }
        return container;
    }
    ...
    private NodeRef getPersonContainer(String remoteSystem, boolean lazyCreate) {
        ...
        if (person == null) {
            ...
            throw new IllegalStateException(
                    "Person details required but none found! Running as "
    ...
    /**
     * Ensure the appropriate aspect is applied to the node which will hold the Remote Credentials
     * System
     */
                ...
                new RunAsWork<Void>() {
                    ...
                    public Void doWork() throws Exception {
                        if (!nodeService.hasAspect(
                            ...
                            if (logger.isDebugEnabled())
                                logger.debug(
                                        "Added the Credentials Container aspect to " + nodeRef);
                        }
                        return null;
                    }
                });
    ...
    private NodeRef findRemoteSystemContainer(
            NodeRef nodeRef, String remoteSystem, boolean lazyCreate) {
        QName remoteSystemQName = QName.createQName(remoteSystem);
        List<ChildAssociationRef> systems =
                nodeService.getChildAssocs(
                        nodeRef,
                        RemoteCredentialsModel.ASSOC_CREDENTIALS_SYSTEM,
                        remoteSystemQName);
        ...
        if (systems.size() > 0) {
            ...
            if (logger.isDebugEnabled())
                logger.debug(
                        "Resolved Remote Credentials Container for "
                                + remoteSystem
                                + " of "
                                + system
                                + " in parent "
            ...
            if (lazyCreate) {
                ...
                system =
                        nodeService
                                .createNode(
                                        ...
                                        RemoteCredentialsModel.TYPE_REMOTE_CREDENTIALS_SYSTEM)
                                .getChildRef();

                if (logger.isDebugEnabled())
                    logger.debug(
                            "Lazy created Remote Credentials Container for "
                                    + remoteSystem
                                    + " in parent "
                                    + nodeRef
                                    + ", new container is "
                ...
                if (logger.isDebugEnabled())
                    logger.debug(
                            "No Remote Credentials Container for "
                                    + remoteSystem
                                    + " found in "
                                    + nodeRef
                                    + ", will be lazy created on write");
            }
        }
    ...
    public void deleteCredentials(BaseCredentialsInfo credentialsInfo) {
        if (credentialsInfo.getNodeRef() == null) {
            throw new IllegalArgumentException(
                    "Cannot delete Credentials which haven't been persisted yet!");
        }
        ...
        if (logger.isDebugEnabled())
            logger.debug(
                    "Deleted credentials "
                            + credentialsInfo
                            + " from "
                            + credentialsInfo.getNodeRef()
                            + " from Remote System "
                            + credentialsInfo.getRemoteSystemName());
    ...
    @Override
    public BaseCredentialsInfo createPersonCredentials(
            String remoteSystem, BaseCredentialsInfo credentials) {
    ...
    @Override
    public BaseCredentialsInfo createSharedCredentials(
            String remoteSystem, BaseCredentialsInfo credentials) {
        NodeRef shared = getSharedContainer(remoteSystem, true);
        return createCredentials(remoteSystem, shared, credentials);
    }

    private BaseCredentialsInfo createCredentials(
            String remoteSystem, NodeRef remoteSystemNodeRef, BaseCredentialsInfo credentials) {
        if (credentials.getNodeRef() != null) {
            throw new IllegalArgumentException(
                    "Cannot create Credentials which have already been persisted!");
        }

        // Check we know about the type
        RemoteCredentialsInfoFactory factory =
                credentialsFactories.get(credentials.getCredentialsType());
        if (factory == null) {
            throw new TypeConversionException(
                    "No Factory registered for type " + credentials.getCredentialsType());
        }

        // Build the properties
        Map<QName, Serializable> properties =
                RemoteCredentialsInfoFactory.FactoryHelper.getCoreCredentials(credentials);
        ...
        NodeRef nodeRef =
                nodeService
                        .createNode(
                                remoteSystemNodeRef,
                                RemoteCredentialsModel.ASSOC_CREDENTIALS,
                                name,
    ...
    private BaseCredentialsInfo loadCredentials(
            String remoteSystem, NodeRef remoteSystemNodeRef, NodeRef credentialsNodeRef) {
    ...
    @Override
    public BaseCredentialsInfo updateCredentialsAuthenticationSucceeded(
        ...
        nodeService.setProperty(
                credentials.getNodeRef(),
                RemoteCredentialsModel.PROP_LAST_AUTHENTICATION_SUCCEEDED,
                succeeded);
        ...
        if (credentials instanceof AbstractCredentialsImpl) {
            ...
            return loadCredentials(
                    credentials.getRemoteSystemName(),
    ...
    private PagingResults<String> listRemoteSystems(
            boolean people, boolean shared, PagingRequest paging) {
        ...
        if (people) {
            ...
            NodeRef person = repositoryHelper.getPerson();
            if (nodeService.hasAspect(
                    person, RemoteCredentialsModel.ASPECT_REMOTE_CREDENTIALS_SYSTEM_CONTAINER)) {
                search.add(person);
            }
        }
        if (shared) {
    ...
    @Override
    public PagingResults<? extends BaseCredentialsInfo> listSharedCredentials(
            String remoteSystem, QName credentialsType, PagingRequest paging) {
    ...
    @Override
    public PagingResults<? extends BaseCredentialsInfo> listPersonCredentials(
            String remoteSystem, QName credentialsType, PagingRequest paging) {
        // Get the container for that system
    ...
    @Override
    public PagingResults<? extends BaseCredentialsInfo> listAllCredentials(
            String remoteSystem, QName credentialsType, PagingRequest paging) {
        NodeRef personContainer = getPersonContainer(remoteSystem, false);
        ...
        return listCredentials(
                new NodeRef[] {personContainer, systemContainer},
                remoteSystem,
                credentialsType,
                paging);
    }
    /**
     * TODO This would probably be better done as a dedicated Canned Query We want to filter by
     * Assoc Type and Child Node Type, and the node service currently only allows you to do one or
     * the other
     */
    private PagingResults<? extends BaseCredentialsInfo> listCredentials(
            NodeRef[] containers,
            String remoteSystem,
            QName credentialsType,
        ...
        if (credentialsType != null) {
            ...
            if (logger.isDebugEnabled())
                logger.debug(
                        "Searching for credentials of " + credentialsType + " as types " + types);
        }
        ...
        for (NodeRef nodeRef : containers) {
            if (nodeRef != null) {
                ...
                List<ChildAssociationRef> allCreds =
                        nodeService.getChildAssocs(
                                nodeRef,
                                RemoteCredentialsModel.ASSOC_CREDENTIALS,
                                RegexQNamePattern.MATCH_ALL);
    ...
    private class CredentialsPagingResults implements PagingResults<BaseCredentialsInfo> {
        ...
        private CredentialsPagingResults(
                List<ChildAssociationRef> refs, int size, boolean hasMore, String remoteSystem) {
            this.size = size;
            ...
            for (ChildAssociationRef ref : refs) {
                this.results.add(
                        loadCredentials(remoteSystem, ref.getParentRef(), ref.getChildRef()));
            }
        }
    ...
    protected static void setSharedCredentialsSystemContainerName(String container) {
        ...
        SHARED_CREDENTIALS_CONTAINER_QNAME =
                QName.createQName(
                        RemoteCredentialsModel.REMOTE_CREDENTIALS_MODEL_URL,

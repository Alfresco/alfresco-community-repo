...
/**
 * Returns a virtual parent reference upon execution by subtracting the last template path element
 * from of the given reference.<br>
    ...
    public Reference execute(VirtualProtocol virtualProtocol, Reference reference)
        ...
        if (path.trim().endsWith(PATH_SEPARATOR)) {
            ...
            if (trailingPathIndex == 0) {
                ...
                path = path.substring(0, trailingPathIndex);
            }
        }
        ...
        if (index < 0) {
            ...
            return virtualProtocol.replaceTemplatePath(reference, parentPath);
        }
    }

    @Override
    public Reference execute(NodeProtocol protocol, Reference reference)
            throws ProtocolMethodException {

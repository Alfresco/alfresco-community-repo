...
public class CronScheduledQueryBasedTemplateActionDefinitionTest
   ...
   */
  private static final String SCHEDULER_TRANSACTION_MODE =
    "UNTIL_FIRST_FAILURE";

  /**
   * {@link CronScheduledQueryBasedTemplateActionDefinition#setJobName(String)}
   */
  private static final String SCHEDULER_JOB_NAME =
    "jobTestName-" + System.currentTimeMillis();

  /**
   * {@link CronScheduledQueryBasedTemplateActionDefinition#setJobGroup(String)}
   */
  private static final String SCHEDULER_JOB_GROUP =
    "jobTestGroup-" + System.currentTimeMillis();
  ...
  private static final String POLICY_BEHAVIOUR_FILTER_BEAN_NAME =
    "policyBehaviourFilter";

  private static final String ROOT_TEST_FOLDER_NAME_TEMPLATE =
    "RootTestFolder-%d";

  private static final String TEST_DOCUMENT_NAME_TEMPLATE =
    "TestDocument-%d-%d.txt";
  ...
  private static final String TEST_FOLDER_NAME = String.format(
    ROOT_TEST_FOLDER_NAME_TEMPLATE,
  ...
  private void initializeScheduler() {
    Scheduler factory = (Scheduler) applicationContext.getBean(
      SCHEDULER_FACTORY_BEAN_NAME
    );
    ...
    templateActionDefinition.setDictionaryService(
      registry.getDictionaryService()
    );
    templateActionDefinition.setTemplateActionModelFactory(
  ...
  private void createTestContent() throws Exception {
    ...
    NodeRef companyHomeNodeRef = registry
    ...
    for (int i = 0; i < TEST_DOCUMENTS_AMOUNT; i++) {
      String freshDocName = String.format(
        FRESH_TEST_DOCUMENT_NAME_TEMPLATE,
        i,
        System.currentTimeMillis()
      );
    ...
    BehaviourFilter policyBehaviourFilter = (BehaviourFilter) applicationContext.getBean(
      POLICY_BEHAVIOUR_FILTER_BEAN_NAME
    );
    policyBehaviourFilter.disableBehaviour(ContentModel.ASPECT_AUDITABLE);
    try {
      for (int i = 0; i < TEST_DOCUMENTS_AMOUNT; i++) {
        String yesterdayDocName = String.format(

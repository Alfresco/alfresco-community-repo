...
public class AuthorityTypeBehaviour
        implements NodeServicePolicies.OnUpdatePropertiesPolicy, InitializingBean {
    ...
    public void onUpdateProperties(
        ...
        if (!(AuthenticationUtil.isRunAsUserTheSystemUser()
            ...
            throw new AccessDeniedException(
                    "Only users with ROLE_ADMINISTRATOR are allowed to manage users.");
        }
    }

    private boolean modifyingOwnAccount(
            Map<QName, Serializable> before, Map<QName, Serializable> after) {
        String beforeUsername =
                findUsernameInProperties(before, USERNAME_FIELD, INVALID_USERNAME_VALUE);
        String afterUsername =
                findUsernameInProperties(after, USERNAME_FIELD, INVALID_USERNAME_VALUE);
    ...
    private String findUsernameInProperties(
            Map<QName, Serializable> map, String usernameField, String invalidValue) {
        Optional<QName> first =
                map.keySet().stream()
                        .filter(q -> q.getLocalName().equals(usernameField))
                        .findFirst();
        if (first.isPresent()) {

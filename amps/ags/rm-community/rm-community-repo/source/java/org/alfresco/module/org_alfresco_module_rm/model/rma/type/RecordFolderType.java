...
public class RecordFolderType extends AbstractDisposableItem
    ...
    public CopyBehaviourCallback getCopyCallback(
        ...
        return new DefaultCopyBehaviourCallback() {
            ...
            public boolean getMustCopy(QName classQName, CopyDetails copyDetails) {
                boolean targetParentIsRecordFolder =
                        nodeService
    ...
    public void onCreateChildAssociation(ChildAssociationRef childAssocRef, boolean bNew) {
        ...
        if (nodeService.exists(nodeRef)) {
            ...
            if (isClosed != null && isClosed) {
                throw new IntegrityException(
                        I18NUtil.getMessage(MSG_CANNOT_CREATE_CHILDREN_IN_CLOSED_RECORD_FOLDER),
                        null);
            }
        }
    }
    ...
    public void onCreateChildAssociationOnCommit(ChildAssociationRef childAssocRef, boolean bNew) {
        ...
        // only records can be added in a record folder or hidden folders(is the case of e-mail
        // attachments)
        if (instanceOf(child, ContentModel.TYPE_FOLDER)
                && !nodeService.hasAspect(child, ContentModel.ASPECT_HIDDEN)) {
            throw new IntegrityException(
                    I18NUtil.getMessage(
                            MSG_CANNOT_CREATE_RECORD_FOLDER_CHILD, nodeService.getType(child)),
                    null);
        }

...
public class NodeAssociationsApiTest extends AbstractSingleNetworkSiteTest {
  ...
  public void testNodePeerAssocs() throws Exception {
    ...
    String o1Id = RestApiUtil
      .parseRestApiEntry(response.getJsonResponse(), Node.class)
      .getId();
    ...
    try {
      ...
      assertEquals(
        ASSOC_TYPE_CM_REFERENCES,
        nodes.get(0).getAssociation().getAssocType()
      );
...

      {
        ...
        getAll(
          getNodeTargetsUrl(UUID.randomUUID().toString()),
          paging,
          null,
          404
        );
        getAll(
          getNodeSourcesUrl(UUID.randomUUID().toString()),
          paging,
          null,
          404
        );
  ...
  public void testNodePeerAssocsPermissions() throws Exception {
    ...
    String sf1Id = createFolder(sharedFolderNodeId, "shared folder " + RUNID)
      .getId();
    ...
    n.setAspectNames(Arrays.asList(ASPECT_CM_REFERENCING, ASPECT_CM_PARTABLE));
    HttpResponse response = post(
      getNodeChildrenUrl(sf1Id),
      toJsonAsStringNonNull(n),
      201
    );
    String so1Id = RestApiUtil
      .parseRestApiEntry(response.getJsonResponse(), Node.class)
      .getId();
  ...
  public void testNodeSecondaryChildAssocs() throws Exception {
    ...
    HttpResponse response = post(
      getNodeChildrenUrl(myFolderNodeId),
      toJsonAsStringNonNull(n),
      201
    );
    String f1Id = RestApiUtil
      .parseRestApiEntry(response.getJsonResponse(), Node.class)
      .getId();
    ...
    try {
      ...
      post(
        getNodeSecondaryChildrenUrl(f1Id),
      ...
      for (Node node : nodes) {
        ...
        if (nodeAssoc.getAssocType().equals(ASSOC_TYPE_CM_CONTAINS)) {
          ...
          nodeAssoc.getAssocType().equals(ASSOC_TYPE_CM_PREFERENCE_IMAGE)
        ) {
          i++;
        }
      ...
      assertEquals(o2Id, nodes.get(0).getId());
      assertEquals(
        ASSOC_TYPE_CM_PREFERENCE_IMAGE,
        nodes.get(0).getAssociation().getAssocType()
      );
      ...
      List<String> nodeIds = Arrays.asList(
        new String[] { nodes.get(0).getId(), nodes.get(1).getId() }
      );
...

      {
        ...
        for (int j = 0; j < childCnt; j++) {
          ...
          n.setNodeType(TYPE_CM_CONTENT);
          response =
            post(getNodeChildrenUrl(f2Id), toJsonAsStringNonNull(n), 201);
          ...
          post(
            getNodeSecondaryChildrenUrl(f3Id),
            toJsonAsStringNonNull(secChild),
            201
          );
        }
        ...
        PublicApiClient.ExpectedPaging expectedPaging = RestApiUtil.parsePaging(
          response.getJsonResponse()
        );
...

      {
        ...
        n.setNodeType(TYPE_CM_CONTENT);
        response =
          post(getNodeChildrenUrl(f4Id), toJsonAsStringNonNull(n), 201);

        String childId = RestApiUtil
          .parseRestApiEntry(response.getJsonResponse(), Node.class)
          .getId();
...

      {
        ...
        secChild = new AssocChild(o2Id, ASSOC_TYPE_CM_CONTAINS);
        post(
          getNodeSecondaryChildrenUrl(o1Id),
          toJsonAsStringNonNull(secChild),
          422
        );
...

      {
        ...
        getAll(
          getNodeParentsUrl(UUID.randomUUID().toString()),
          paging,
          null,
          404
        );
   ...
   */
  @Test
  public void testListChildrenConsistentParentIdWithSecondaryAssociations()
    throws Exception {
    setRequestContext(user1);
    ...
    String primaryFolderId = createFolder(myNodeId, primaryFolderName, null)
      .getId();

    // Create content file
    String contentFileName = "content" + RUNID + " in folder";
    String contentId = createTextFile(
      primaryFolderId,
      contentFileName,
    ...
    String secondaryFolderId = RestApiUtil
      .parseRestApiEntry(response.getJsonResponse(), Node.class)
      .getId();
    AssocChild secChild = new AssocChild(contentId, ASSOC_TYPE_CM_CONTAINS);
    post(
      getNodeSecondaryChildrenUrl(secondaryFolderId),
      toJsonAsStringNonNull(secChild),
      201
    );
  ...
  public void testDeleteAndRestoreNodeWithAssocs() throws Exception {
    ...
    try {
      ...
      post(
        getNodeSecondaryChildrenUrl(f2Id),
        toJsonAsStringNonNull(secChild),
        201
      );
      ...
      secChild = new AssocChild(f1dId, ASSOC_TYPE_CM_CONTAINS);
      post(
        getNodeSecondaryChildrenUrl(f1bId),
        toJsonAsStringNonNull(secChild),
        201
      );

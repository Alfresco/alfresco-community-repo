 ...
 * specified in form dates with time zone offset value to. Web Script which saves a new event always
 * performs correction of the dates in accordance with time zone for 'All Day' events. Date becomes
 * on a day before, if time for date is set to '00:00' in this case. Share in 4.x does this
 * adjustment automatically before sending request to the Web Script.<br>
 * <br>
 * See "<a href="https://issues.alfresco.com/jira/browse/MNT-8977">CMIS (OpenCMIS version) is
 * sharing security context with other functionality in Alfresco</a>" for more details
 *
 * @since 4.1.5
...
public class CalendarAllDayEventDatesCorrectingPatch extends AbstractPatch {
    private static final String MSG_SUCCESS =
            "patch.calendarAllDayEventDatesCorrectingPatch.result";

    private static final Logger LOGGER =
            Logger.getLogger(CalendarAllDayEventDatesCorrectingPatch.class);
    ...
    protected String applyInternal() throws Exception {
        ...
        List<ChildAssociationRef> allSites =
                (null != siteRoot) ? (nodeService.getChildAssocs(siteRoot)) : (null);
                        ...
                        if (null != page) {
                            ...
                            for (CalendarEntry entry : page) {
                                if (isAllDay(entry)) {
                                    ...
                                    if (LOGGER.isDebugEnabled()) {
                                        LOGGER.debug(
                                                "'All Day' Calendar event has been detected:"
                                                        + " [title: "
                                                        + entry.getTitle()
                                                        + ", start: "
                                                        + entry.getStart()
    ...
    /**
     * Increases (or decreases) <code>propertyId</code> date-property by the extracted (from the
     * specified property value) time zone offset
     *
     * @param oldDate - {@link Date} instance, which represents not adjusted date for an 'All Day'
     *     event
     * @return {@link Date} instance, which represents adjusted date-property in accordance with
     *     time zone offset
     */

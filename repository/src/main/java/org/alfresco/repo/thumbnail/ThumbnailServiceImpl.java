...
public class ThumbnailServiceImpl
  ...
  private static final String THUMBNAIL_PARENT_NODES =
    "ThumbnailServiceImpl.ParentNodes";
  private static final String THUMBNAIL_TO_DELETE_NODES =
    "ThumbnailServiceImpl.ToDeleteNodes";
  ...
  public void init() {
    ...
    thumbnailsToDeleteTransactionListener =
  ...
  public void beforeCreateNode(
    ...
    if (ContentModel.TYPE_FAILED_THUMBNAIL.equals(nodeTypeQName)) {
      ...
      List<ChildAssociationRef> existingThumbnails = nodeService.getChildAssocs(
        parentRef,
        childNodeTypes
      );
      ...
      for (ChildAssociationRef chAssRef : existingThumbnails) {
        if (chAssRef.getQName().equals(assocQName)) {
          if (logger.isDebugEnabled()) {
            ...
            msg
              .append("Deleting thumbnail node ")
              .append(chAssRef.getChildRef());
  ...
  public void onCreateNode(ChildAssociationRef childAssoc) {
    ...
    String thumbnailName = (String) nodeService.getProperty(
      thumbnailNodeRef,
    ...
    if (nodeService.exists(thumbnailNodeRef)) {
      ...
      QName thumbnailQname = QName.createQName(
        NamespaceService.CONTENT_MODEL_1_0_URI,
        thumbnailName
      );
      if (!childAssoc.getQName().equals(thumbnailQname)) {
        List<ChildAssociationRef> allAssocList = nodeService.getChildAssocs(
    ...
    Map<String, FailedThumbnailInfo> failures = getFailedThumbnails(
      sourceNodeRef
    );
    ...
    if (existingFailedThumbnail != null) {
      if (logger.isDebugEnabled()) {
        StringBuilder msg = new StringBuilder();
        msg
          .append("Deleting failedThumbnail node ")
  ...
  public NodeRef createThumbnail(
    ...
    if (thumbnailName != null) {
      NodeRef existingThumbnail = getThumbnailByName(
        node,
        contentProperty,
        thumbnailName
      );
  ...
  public void updateThumbnail(
    ...
    if (logger.isDebugEnabled() == true) {
      logger.debug(
        "Updating thumbnail (thumbnail=" + thumbnail.toString() + ")"
      );
    }
    ...
    if (renditionService.isRendition(thumbnail)) {
      ...
      ChildAssociationRef parentAssoc = renditionService.getSourceNode(
        thumbnail
      );
      ...
      QName contentProperty = (QName) nodeService.getProperty(
        thumbnail,
      ...
      if (rendDefn == null) {
        String renderingEngineName = getRenderingEngineNameFor(
          transformationOptions
        );
      ...
      Map<String, Serializable> params = thumbnailRegistry
      ...
      if (logger.isDebugEnabled() == true) {
        logger.debug(
          "Updating thumbnail: cannot update a thumbnail node that isn't the correct thumbnail type (thumbnail=" +
          thumbnail.toString() +
          ")"
        );
      }
   ...
   */
  public NodeRef getThumbnailByName(
    NodeRef node,
    QName contentProperty,
    String thumbnailName
  ) {
    ...
    QName namespacedThumbnailName = QName.createQName(
      NamespaceService.CONTENT_MODEL_1_0_URI,
      thumbnailName
    );

    ChildAssociationRef existingRendition = renditionService.getRenditionByName(
      node,
      namespacedThumbnailName
    );
  ...
  public List<NodeRef> getThumbnails(
    ...
    List<ChildAssociationRef> renditions =
      this.renditionService.getRenditions(node);
  ...
  private boolean validateThumbnail(NodeRef thumbnailNode) {
    boolean valid = true;
    ContentData content = (ContentData) this.nodeService.getProperty(
        thumbnailNode,
    ...
    if (content.getSize() == 0) {
      TransactionalResourceHelper
        .getSet(THUMBNAIL_TO_DELETE_NODES)
        .add(thumbnailNode);
      TransactionSupportUtil.bindListener(
        this.thumbnailsToDeleteTransactionListener,
        0
      );
      valid = false;
    }
  ...
  @Override
  public Map<String, FailedThumbnailInfo> getFailedThumbnails(
    NodeRef sourceNode
  ) {
    ...
    if (
      ...
      List<ChildAssociationRef> failedThumbnailChildren = nodeService.getChildAssocs(
        sourceNode,
  ...
  private boolean matchMimetypeOptions(
    ...
    if (mimetype != null) {
      ...
      String thumbnailMimetype =
        (
          (ContentData) this.nodeService.getProperty(
              thumbnail,
              ContentModel.PROP_CONTENT
            )
        ).getMimetype();
      ...
      if (mimetype.endsWith(SUBTYPES_POSTFIX)) {
        String baseMimetype = mimetype.substring(
          0,
          mimetype.length() - SUBTYPES_POSTFIX.length()
        );
        if (
  ...
  private RenditionDefinition createRenditionDefinition(
    ...
    RenditionDefinition definition = createRawRenditionDefinition(
      thumbnailQName,
      transformationOptions
    );
  ...
  private NodeRef createThumbnailNode(
    ...
    RenditionDefinition definition = createRenditionDefinition(
      contentProperty,
      mimetype,
      transformationOptions,
      thumbnailQName,
    ...
    try {
      ChildAssociationRef thumbnailAssoc = renditionService.render(
        node,
        definition
      );
   ...
   */
  private void setThumbnailNameProperty(
    NodeRef thumbnail,
    String thumbnailName
  ) {
    if (thumbnailName != null && thumbnailName.length() > 0) {
      if (ContentModel.TYPE_THUMBNAIL.equals(nodeService.getType(thumbnail))) {
        nodeService.setProperty(
          thumbnail,
          ContentModel.PROP_THUMBNAIL_NAME,
          thumbnailName
        );
      }
    }
  ...
  private void addThumbnailModificationData(
    final NodeRef nodeRef,
    final String thumbnailName
  ) {
    if (nodeService.exists(nodeRef)) {
      if (
        thumbnailName != null && !nodeRef.toString().endsWith(thumbnailName)
      ) {
  ...
  private class ThumbnailTransactionListenerAdapter
    ...
    public void afterCommit() {
      ...
      final Set<ChildAssociationRef> childAssocs = TransactionalResourceHelper.getSet(
        THUMBNAIL_PARENT_NODES
      );
  ...
  private class ThumbnailCleanupTransactionListenerAdapter
    extends TransactionListenerAdapter {
    ...
    public void afterCommit() {
      ...
      final Set<NodeRef> thumbnailToDelete = TransactionalResourceHelper.getSet(
        THUMBNAIL_TO_DELETE_NODES
      );

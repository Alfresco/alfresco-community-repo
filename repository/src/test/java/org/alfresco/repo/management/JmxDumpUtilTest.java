...
public class JmxDumpUtilTest extends TestCase {
  ...
  public void testCleanPasswordsFromInputArgument() throws Exception {
    Pattern pattern = Pattern.compile(
      "(?i)(.*(password=|pwd=|token=))((?<=password=|pwd=|token=).*+)"
    );
    String passwordArg = "-Ddb.password=I should be stars \"£$%^&*()@";
    String expected = "-Ddb.password=" + JmxDumpUtil.PROTECTED_VALUE;
    String actual = JmxDumpUtil.cleanPasswordFromInputArgument(
      passwordArg,
      pattern
    );
    assertEquals(
      "Expectected output: " + expected + " Actual output: " + actual,
      expected,
      actual
    );
    ...
    passwordArg =
      "somePrefix.token=\"If i'm not replaced, something has gone very wrong\"";
  ...
  public void testCleanPasswordsFromInputArguments() throws Exception {
    ...
    String[] args = {
      ...
      "-Ddb.user=alfresco",
      "-DtestToken=asdoij3ifiej22244ojpgkmkfpsi3j55643pojpdjoismvi4563625mposvsd",
    };
    String[] expectedArray = {
      "-Ddb.password=" + JmxDumpUtil.PROTECTED_VALUE,
      "-Ddb.user=alfresco",
      "-DtestToken=" + JmxDumpUtil.PROTECTED_VALUE,
    };
    String[] actualArray = JmxDumpUtil.cleanPasswordsFromInputArguments(
      args,
      argEndingsTypical
    );
    assertArrayEquals(
      "Expectected output:" + expectedArray + " Actual output:" + actualArray,
      expectedArray,
      actualArray
    );
    ...
    actualArray =
      JmxDumpUtil.cleanPasswordsFromInputArguments(args, argEndingsTypical);
    assertArrayEquals(
      "Expectected output:" + expectedArray + " Actual output:" + actualArray,
      expectedArray,
      actualArray
  ...
  public void testCreatePasswordFindRegexString() throws Exception {
    ...
    String expected =
      "(?i)(.*(password=|any old ending :D=|token=))((?<=password=|any old ending :D=|token=).*+)";

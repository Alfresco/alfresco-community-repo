...
/**
 * A method interceptor to clean up node ref properties as they are passed in and out of the node
 * service. For getProperty and getProperies calls invalid node refs are removed from the returned
 * set (they appear to have be cleaned up). For setProperty and setProperties calls invalid node
 * refs are removed and thus not set. It only considers properties of type d:noderef.
 *
 * @author andyh
 */
public class NodeRefPropertyMethodInterceptor implements MethodInterceptor {
    ...
    public Object invoke(MethodInvocation invocation) throws Throwable {
        ...
        // We are going to change the method arguments as we proceed - so we keep them to set the
        // references back at the
        // end
        ...
        // org.springframework.aop.framework.ReflectiveMethodInvocation does not do any special
        // wrapping and this is
        // fine
            ...
            if (methodName.equals("addAspect")) {
                ...
                if (filterOnGet) {
                    ...
                    Map<QName, Serializable> properties =
                            (Map<QName, Serializable>) invocation.proceed();
                    Map<QName, Serializable> convertedProperties =
    ...
    /**
     * Remove unknown node ref values Remove unknowen categories - the node will be removed if it
     * does exist and it is not a category
     *
    ...
    private Serializable getValue(QName propertyQName, Serializable inboundValue) {
        ...
        if (propertyDef == null) {
            ...
            if ((propertyDef.getDataType().getName().equals(DataTypeDefinition.NODE_REF))
                    || (propertyDef.getDataType().getName().equals(DataTypeDefinition.CATEGORY))) {
                if (inboundValue instanceof Collection) {
                    ...
                    for (Object o : in) {
                        ...
                        if (value == null) {
                            ...
                            try {
                                NodeRef test =
                                        DefaultTypeConverter.INSTANCE.convert(NodeRef.class, value);
                                if (getNodeService().exists(test)) {
                                    if (propertyDef
                                        ...
                                        if (getDictionaryService()
                                                .isSubClass(type, ContentModel.TYPE_CATEGORY)) {

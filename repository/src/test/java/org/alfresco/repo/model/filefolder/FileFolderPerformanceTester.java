...
/**
 * Tests around some of the data structures that lead to performance degradation. We use the {@link
 * org.alfresco.service.cmr.model.FileFolderService FileFolderService} as it provides the most
 * convenient and most common test scenarios.
 *
 * <p>Note that this test is not designed to validate performance figures, but is rather a handy
 * tool for doing benchmarking. It is therefore not named <i>*Test</i> as is the pattern for getting
 * tests run by the continuous build.
 *
 * @author Derek Hulley
 */
...
public class FileFolderPerformanceTester extends TestCase {
    ...
    // private String USERNAME = AuthenticationUtil.getSystemUserName(); // as system (bypass
    // permissions)
    ...
    /**
     * Creates <code>folderCount</code> folders below the given parent and populates each folder
     * with <code>fileCount</code> files. The folders will be created as siblings in one go, but the
     * files are added one to each folder until each folder has the presribed number of files within
     * it. This can therefore be used to test the performance when the L2 cache sizes are exceeded.
     *
     * <p>Each creation (file or folder) uses the <b>PROPAGATION REQUIRED</b> transaction
     * declaration.
     *
     * @param parentNodeRef the level zero parent
     * @param threadCount
     * @param randomOrder true if each thread must put the children into the folders in a random
     *     order
    ...
    private void buildStructure(
        ...
        RetryingTransactionCallback<NodeRef[]> createFoldersCallback =
                ...
                new Runnable() {
                    ...
                    public void run() {
                        ...
                        for (int i = 0; i < batchCount; i++) {
                            ...
                            if ((nextDumpCount - 0.5) < completedCount
                            ...
                            for (int j = 0; j < folders.length; j++) {
                                ...
                                RetryingTransactionCallback<Void> createFileCallback =
                    ...
                    private void dumpResults(int currentBatchCount) {
                        ...
                        double average =
                                (double) time
                                        / (double)
                                                (folderCount * currentBatchCount * filesPerBatch);
                        double percentComplete =
                                (double) currentBatchCount / (double) batchCount * 100.0;
                    ...
                    public void run() {
                        ...
                        for (int i = 0; i < repetitions; i++) {
                            ...
                            for (final FileInfo fileInfo : children) {
                                ...
                                RetryingTransactionCallback<Object> readCallback =
    ...
    */
     ...
     * listings against it.
     *
     * @param args <x> <y> where 'x' is the number of files in a folder and 'y' is the number of
     *     threads to list
     */
    ...
    private static void run(final ApplicationContext ctx, String... args) throws Throwable {
        ...
        final NodeRef selectedFolderNodeRef =
                folderRefStr == null ? null : new NodeRef(folderRefStr);
        ...
        RunAsWork<String> createUserRunAs =
        ...
        final String user =
                AuthenticationUtil.runAs(createUserRunAs, AuthenticationUtil.getSystemUserName());

        // Create the files
        final RetryingTransactionCallback<NodeRef> createCallback =
                new RetryingTransactionCallback<NodeRef>() {
                            ...
                            for (int i = 0; i < fileCount; i++) {
                                ...
                                for (int m = 0; m < 30; m++) {
                                    nodeService.addAspect(
                                            nodeRef, QName.createQName("{test}aspect_" + m), null);
                                }
                                // write the content
        ...
        System.in.read();
        final RunAsWork<List<FileInfo>> readRunAs =
                new RunAsWork<List<FileInfo>>() {
    ...
    private static String getUsage() {
        ...
        sb.append(
                        "   FileFolderPerformanceTester --files=<filecount> --threads=<threadcount>"
                                + " --folder=<folderref>")
                .append("\n");

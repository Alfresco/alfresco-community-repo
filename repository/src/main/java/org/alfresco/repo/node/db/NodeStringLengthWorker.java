...
public class NodeStringLengthWorker implements ApplicationContextAware {
   ...
   */
  private synchronized void doWork(NodeStringLengthWorkResult progress)
    throws Exception {
    // Build batch processor
    BatchProcessWorkProvider<NodePropertyEntity> workProvider = new NodeStringLengthWorkProvider(
      progress
    );
    BatchProcessWorker<NodePropertyEntity> worker = new NodeStringLengthBatch(
      progress
    );
  ...
  private class NodeStringLengthBatch
    extends BatchProcessWorkerAdaptor<NodePropertyEntity> {

    private final int typeOrdinalText = NodePropertyValue.convertToTypeOrdinal(
      DataTypeDefinition.TEXT
    );
    private final int typeOrdinalAny = NodePropertyValue.convertToTypeOrdinal(
      ...
      try {
        ...
        if (repersist) {
          ...
          nodeDAO.removeNodeProperties(
            nodeId,
            Collections.singleton(propQName)
          );
  ...
  public static class NodeStringLengthWorkResult {
    ...
    public String toString() {
      ...
      String part2 = String.format(
        ...
        propertiesChanged.get(),
        propertiesProcessed.get()
      );
  ...
  public static class NodeStringLengthJob implements Job {
    ...
    public void execute(JobExecutionContext context)
      ...
      if (
        nodeStringLengthWorkerObj == null ||

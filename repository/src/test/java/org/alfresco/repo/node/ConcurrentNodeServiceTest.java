...
public class ConcurrentNodeServiceTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        RetryingTransactionCallback<Object> createRootNodeCallback =
    ...
    /**
     * Tests that when multiple threads try to edit different properties on a node, that
     * transactions + retries always mean that every change always ends up on the node.
     *
     * @since 3.4
     */
                                        ...
                                        RetryingTransactionCallback<Integer> callback =
                                                new RetryingTransactionCallback<Integer>() {
                                                    ...
                                                    public Integer execute() throws Throwable {
                                                        ...
                                                        // Check that the value is what we expected
                                                        // it to be
                                                        // We do this after the update so that we
                                                        // don't fall on retries
                                                        int expectedCurrent =
                                                                propCounts[propNumFinal];
                                                        if (expectedCurrent != (current - 1)) {
                                                            // We have a difference here already
                                                            // It will never catch up, but we'll
                                                            // detect that later
                                                            System.out.println(
                                                                    "Found difference: "
                    ...
                    new Runnable() {
                        ...
                        public void run() {
                            ...
                            for (int loop = 0; loop < loops; loop++) {
                                ...
                                RetryingTransactionCallback<Void> runCallback =
                                        new RetryingTransactionCallback<Void>() {
                                            @Override
                                            public Void execute() throws Throwable {
                                                // Add another aspect to the node
                                                QName qname =
                                                        QName.createQName(NAMESPACE, nameWithLoop);

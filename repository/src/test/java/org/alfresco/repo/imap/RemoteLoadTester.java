...
public class RemoteLoadTester extends TestCase {
  ...
  public static void main(String[] args) {
    if (args.length > 0) REMOTE_HOST = args[0];
  ...
  public void testListSequence() {
    ...
    try {
      ...
      System.out.println(
        String.format(
          "LIST '', folders.length = %d, execTime = %d sec",
          folders.length,
      ...
      System.out.println(
        String.format(
          "LSUB *, folders.length = %d, execTime = %d sec",
          folders.length,
          (endTime - startTime) / 1000
        )
      );
      ...
      System.out.println(
        String.format(
          "Folders Loop, folders.length = %d, execTime = %d sec",
          folders.length,
          (endTime - startTime) / 1000
        )
      );
    ...
    try {
      ...
      for (int i = 0, n = message.length; i < n; i++) {
        ...
        if (content instanceof MimeMultipart) {
          for (
            int j = 0, m = ((MimeMultipart) content).getCount();
            j < m;
            j++
          ) {

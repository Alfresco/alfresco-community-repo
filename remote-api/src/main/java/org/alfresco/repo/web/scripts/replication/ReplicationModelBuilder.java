...
public class ReplicationModelBuilder {
    ...
    public ReplicationModelBuilder(
    ...
    /**
     * Sorts simple definitions by their last run time. Things that have never been run go to the
     * bottom of the list, so we see most recently run, least recently run, never run.
     */
    ...
    /**
     * Build a model containing a list of simple definitions for the given list of Replication
     * Definitions.
     */
    protected Map<String, Object> buildSimpleList(
            List<ReplicationDefinition> replicationDefinitions,
        ...
        if (replicationDefinitions.size() > 0) {
            List<ExecutionSummary> executing =
                    actionTrackingService.getExecutingActions(
                            replicationDefinitions.get(0).getActionDefinitionName());
    ...
    protected Map<String, Object> buildDetails(ReplicationDefinition rd) {
        ...
        for (NodeRef node : rd.getPayload()) {
            if (nodeService.exists(node)) payload.add(node);
        }
    ...
    /**
     * Figures out the status that's one of: New|Running|CancelRequested|Completed|Failed|Cancelled
     * by merging data from the action tracking service. Will also set the start and end dates, from
     * either the replication definition or action tracking data, depending on the status.
     */
    protected void setStatus(
            ReplicationDefinition replicationDefinition, Map<String, Object> model) {
    ...
    /**
     * For the given Replication Definition, and list of executing actions (which may or may not be
     * only for this definition), return a single execution details.
     *
     * <p>Returns null if no copies of the definition are executing. Returns a predictable instance
     * if more than one copy is executing.
     */
    private ExecutionDetails getExecutionDetails(
            ReplicationDefinition replicationDefinition, List<ExecutionSummary> executing) {

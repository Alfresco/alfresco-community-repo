...
public class AuthenticatedHttp extends AbstractHttp {
    ...
    /**
     * Enable ticket-based authentication. If set to false, BASIC Authentication will be used
     ...
     * response-stream to release the connection. If response should be extracted, this should be
     * done in the {@link HttpRequestCallback}.
     *
     * @param method method to execute
     * @param userName name of user to authenticate
     * @param callback called after http-call is executed. When callback returns, the response
     *     stream is closed, so all respose-related operations should be done in the callback. Can
     *     be null.
     * @return result returned by the callback or status-code if no callback is given.
     */
    public <T extends Object> T executeHttpMethodAuthenticated(
            HttpMethod method, String userName, HttpRequestCallback<T> callback) {
        if (ticketBasedAuthentication) {
            return executeWithTicketAuthentication(
                    method, userName, authDetailProvider.getPasswordForUser(userName), callback);
        } else {
    ...
    public <T extends Object> T executeHttpMethodAuthenticated(
            HttpMethod method, String userName, String password, HttpRequestCallback<T> callback) {
        if (ticketBasedAuthentication) {
    ...
    public <T extends Object> T executeHttpMethodUnauthenticated(
            HttpMethod method, HttpRequestCallback<T> callback) {
        try {
            ...
            if (!handled) {
                throw new RuntimeException(
                        "Error while executing an un-authenticated HTTP-call ("
                                + method.getPath()
                                + ")",
                        t);
            }
     ...
     */
    public <T extends Object> T executeHttpMethodAsAdmin(
            HttpMethod method, HttpRequestCallback<T> callback) {
        if (ticketBasedAuthentication) {
            return executeWithTicketAuthentication(
                    method,
     ...
     *
     * @param method method to execute
     * @param userName name of user to authenticate (note: if null then attempts to run with no
     *     authentication - eq. Quick/Shared Link test)
     * @param callback called after http-call is executed. When callback returns, the response
     *     stream is closed, so all respose-related operations should be done in the callback. Can
     *     be null.
     * @return result returned by the callback or null if no callback is given.
    ...
    private <T extends Object> T executeWithBasicAuthentication(
        ...
        try {
            ...
            if (!handled) {
                throw new RuntimeException(
                        "Error while executing HTTP-call (" + method.getPath() + ")", t);
            }
    ...
    private <T extends Object> T executeWithTicketAuthentication(
        ...
        try {
            ...
            if (result == HttpStatus.SC_UNAUTHORIZED || result == HttpStatus.SC_FORBIDDEN) {
                ...
                if (!method.validate()) {
                    throw new RuntimeException(
                            "Ticket re-authentication failed for user "
                                    + userName
                                    + " (HTTPMethod not reusable)");
                }
    ...
    /**
     * Add the ticket to the method. In case of {@link EntityEnclosingMethod}s (which don't support
     * Query-parameters), the ticket is added as Username in BASIC Authentication, this is a
     * supported way of passing in ticket into Alfresco.
     *
    ...
    private HttpState applyTicketToMethod(HttpMethod method, String ticket) throws URIException {
        ...
        state.setCredentials(
                AuthScope.ANY,
                new UsernamePasswordCredentials(TICKET_CREDENTIAL_PLACEHOLDER, ticket));
        return state;
    }
    ...
    private void populateRequestBody(EntityEnclosingMethod method, JSONObject object) {
        try {
            ...
            throw new RuntimeException(
                    "All hell broke loose, a JVM that doesn't have UTF-8 encoding...");
        }
    }
    ...
    private String fetchLoginTicket(String userName, String password) {
        ...
        try {
            ...
            if (loginMethod.getStatusCode() == HttpStatus.SC_OK) {
                ...
                if (data == null) {
                    throw new RuntimeException(
                            "Failed to login to Alfresco with user "
                                    + userName
                ...
                throw new RuntimeException(
                        "Failed to login to Alfresco with user "
                                + userName
                                + " ("
                                + loginMethod.getStatusCode()
                                + loginMethod.getStatusLine().getReasonPhrase()
                                + ")");
            }
    ...
    /**
     * Callback used when executing HTTP-request. After this has been called, the response-stream is

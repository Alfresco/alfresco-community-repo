...
public class RepoStore extends AbstractStore implements TenantDeployer {
              ...
              return retryingTransactionHelper.doInTransaction(
                new RetryingTransactionCallback<NodeRef>() {
                  public NodeRef execute() throws Exception {
                    NodeRef repoStoreRootNodeRef = nodeService.getRootNode(
                      repoStore
                    );
                    ...
                    if (nodeRefs.size() == 1) {
                      ...
                      throw new WebScriptException(
                        ...
                        repoStore.toString() +
                        repoPath +
                        " must exist; multiple entries found."
                      );
                    } else {
                      throw new WebScriptException(
                        "Web Script Store " +
  ...
  protected String getPath(NodeRef nodeRef) {
    return (
      nodeService
        .getPath(nodeRef)
        .toDisplayPath(nodeService, permissionService) +
      "/" +
  ...
  protected NodeRef findNodeRef(String documentPath) {
    ...
    try {
      ...
      FileInfo file = fileService.resolveNamePath(
        getBaseNodeRef(),
        pathElementsList
      );
          ...
          return retryingTransactionHelper.doInTransaction(
            new RetryingTransactionCallback<String[]>() {
              public String[] execute() throws Exception {
                ...
                NodeRef scriptNodeRef = (scriptPath.length() == 0)
                  ? getBaseNodeRef()
                  : findNodeRef(scriptPath);
                if (scriptNodeRef != null) {
                  org.alfresco.service.cmr.repository.Path repoScriptPath = nodeService.getPath(
                    scriptNodeRef
                  );
                  String id = script
                    .getDescription()
                    .getId()
                    .substring(
                      scriptPath.length() + (scriptPath.length() > 0 ? 1 : 0)
                    );
                  NodeRef repoStoreRootNodeRef = nodeService.getRootNode(
                    repoStore
                  );
                ...
                return documentPaths != null
                  ? documentPaths.toArray(new String[documentPaths.size()])
                  : new String[0];
              }
            },
            true,
              ...
              public InputStream execute() throws Exception {
                ...
                if (nodeRef == null) {
                  throw new IOException(
                    "Document " + documentPath + " does not exist."
                  );
                }
                ContentReader reader = contentService.getReader(
                  nodeRef,
  ...
  public void createDocument(String documentPath, String content)
    ...
    FileInfo fileInfo = fileService.create(
      pathInfo.getNodeRef(),
      fileName,
  ...
  public void updateDocument(String documentPath, String content)
    ...
    if (pathElements.length == 1) {
      ...
      parentRef =
        findNodeRef(documentPath.substring(0, documentPath.lastIndexOf('/')));
    }

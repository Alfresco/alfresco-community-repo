 ...
 *
 * <p>The supplied NodeRef must be within a Site, and must be of a type supported by Share
 *
 * @author Nick Burch
 * @since 4.0.2
 */
public class SiteShareViewUrlGet extends DeclarativeWebScript {
    ...
    private static QName TYPE_LINK =
            QName.createQName(NamespaceService.LINKS_MODEL_1_0_URI, "link");

    protected String identifySharePage(
            NodeRef nodeRef, SiteInfo site, NodeRef siteContainer, QName nodeType) {
        ...
        if (logger.isDebugEnabled())
            logger.debug(
                    "COuldn't identify specific URL for Node " + nodeRef + " of type " + nodeType);
        return "dashboard";
    }

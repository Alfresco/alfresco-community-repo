...
public class ExtendedPermissionServiceImplUnitTest extends BaseUnitTest {
    ...
    private static final Set<String> READERS =
            Stream.of(generateText(), generateText()).collect(Collectors.toSet());
    ...
    private @InjectMocks @Spy ExtendedPermissionServiceImpl extendedPermissionServiceImpl =
            new ExtendedPermissionServiceImpl() {
    ...
    private @InjectMocks ExtendedPermissionServiceImpl
            extendedPermissionServiceImplWithReaderWritersSet =
                    new ExtendedPermissionServiceImpl() {
                        protected AccessStatus hasPermissionImpl(NodeRef nodeRef, String perm) {
                            return AccessStatus.UNDETERMINED;
                        }

                        public java.util.Set<String> getReaders(Long aclId) {
                            return READERS;
                        }

                        public java.util.Set<String> getWriters(Long aclId) {
                            return WRITERS;
                        }
                    };
    ...
    /**
     * Given a permission pre-processor has been registered And does not DENY When hasPermission is
     * called Then the pre-processor is executed And the ACL's are evaluated as normal
     */
    @Test
    ...
    /**
     * Given a permission pre-processor has been registered And DENY's When hasPermission is called
    ...
    /**
     * Given a permission post-processor has been registered When hasPermission is called Then the
     * permission post-processor is called
     */
    @Test
    public void postProcessorRegistered() {
        ...
        when(mockedPermissionPostProcessor.process(
        ...
        verify(mockedPermissionPostProcessor)
                .process(
                        AccessStatus.UNDETERMINED,
                        nodeRef,
                        perm,
    ...
    /**
     * Given a node with no owner aspect When we ask for the readers and writers Then the owner
     * isn't included in the result
     */
    @Test
    public void getReadersAndWritersForNodeWithNoOwnerAspect() {
        // setup node acl
        NodeRef nodeRef = generateNodeRef();
        when(mockedNodeService.getNodeAclId(nodeRef)).thenReturn(NODE_ACL_ID);

        // setup owner
        when(mockedNodeService.hasAspect(nodeRef, ContentModel.ASPECT_OWNABLE)).thenReturn(false);

        // when
        Pair<Set<String>, Set<String>> result =
                extendedPermissionServiceImplWithReaderWritersSet.getReadersAndWriters(nodeRef);
    ...
    /**
     * Given a node with no owner set When we ask for the readers and writers Then the owner isn't
    ...
    public void getReadersAndWritersForNodeWithNoOwnerSet() {
        ...
        // setup owner
        when(mockedNodeService.hasAspect(nodeRef, ContentModel.ASPECT_OWNABLE)).thenReturn(true);
        when(mockedOwnableService.getOwner(nodeRef)).thenReturn(StringUtils.EMPTY);
    ...
    /**
     * Given a node with NO_OWNER value set When we ask for the readers and writers Then the owner
     * isn't included in the result
     */
    @Test
    public void getReadersAndWritersForNodeWithNoOwnerValueSet() {
        ...
        // setup owner
        when(mockedNodeService.hasAspect(nodeRef, ContentModel.ASPECT_OWNABLE)).thenReturn(false);
        when(mockedOwnableService.getOwner(nodeRef)).thenReturn(OwnableService.NO_OWNER);
    ...
    /**
     * Given a node with an owner When we ask for the readers and writers Then the owner is included
     * in the writers set
     */
    @Test
    public void getReadersAndWritersForNodeWithOwner() {
        ...
        // setup owner
        when(mockedNodeService.hasAspect(nodeRef, ContentModel.ASPECT_OWNABLE)).thenReturn(true);
        when(mockedOwnableService.getOwner(nodeRef)).thenReturn(OWNER);
        when(mockedAuthorityService.authorityExists(OWNER)).thenReturn(true);

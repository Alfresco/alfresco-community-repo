...
public class ActivityServiceImpl implements ActivityService, InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        if (maxFeedItems > feedCleanerMaxFeedItems) {
            logger.warn(
                    "Cannot retrieve more items than feed cleaner max items (overriding "
                            + maxFeedItems
                            + " to "
                            + feedCleanerMaxFeedItems
                            + ")");
    ...
    public void postActivity(
            String activityType, String siteId, String appTool, String activityData) {
        // delegate
     ...
     */
    public void postActivity(
            String activityType, String siteId, String appTool, NodeRef nodeRef, String name) {
        // delegate
    ...
    public void postActivity(
        ...
        // delegate
        activityPostService.postActivity(
                activityType, siteId, appTool, nodeRef, name, typeQName, parentNodeRef);
    }
    ...
    public void postActivity(
        ...
        // delegate
        activityPostService.postActivity(
                activityType, siteId, appTool, jsonActivityData, client, contentNodeInfo);
    }
     ...
     */
    public List<String> getUserFeedEntries(
            String feedUserId, String siteId, boolean excludeThisUser, boolean excludeOtherUsers) {
        return getUserFeedEntries(
                feedUserId, siteId, excludeThisUser, excludeOtherUsers, null, null);
    }
    ...
    public List<String> getUserFeedEntries(
        ...
        try {
            List<ActivityFeedEntity> activityFeeds =
                    getUserFeedEntries(
                            feedUserId,
                            siteId,
                            excludeThisUser,
     ...
     */
    public PagingResults<ActivityFeedEntity> getPagedUserFeedEntries(
            String feedUserId,
            String siteId,
            boolean excludeThisUser,
            boolean excludeOtherUsers,
        ...
        try {
            ...
            if (!((currentUser == null)
                    ...
                    || (AuthenticationUtil.getSystemUserName()
                            .equals(this.tenantService.getBaseNameUser(currentUser))))) {
                throw new AccessDeniedException(
                        "Unable to get user feed entries for '"
                                + feedUserId
            ...
            PagingResults<ActivityFeedEntity> activityFeedEntries =
                    feedDAO.selectPagedUserFeedEntries(
                            feedUserId,
                            networkId,
                            siteId,
                            excludeThisUser,
            ...
            AlfrescoRuntimeException are =
                    new AlfrescoRuntimeException(
                            "Unable to get user feed entries: " + se.getMessage());
            logger.error(are);
            throw are;
        }
    }
    ...
    /**
     * Attempts to find the avatar {@link NodeRef} for the user in supplied {@link
     * ActivityFeedEntity}. As this is aimed at setting the {@link NodeRef} from a client-side point
     * of view there are a couple of activity types where the user is taken from the activity
     * summary rather than from the poster (e.g. when a user role is changed). A cache should be
     * passed in from which to retrieve previously fetched {@link NodeRef}s for efficiency.
     *
     * @param activityFeed ActivityFeedEntity
     * @return NodeRef
     */
    protected NodeRef getUserAvatarNodeRef(
            ActivityFeedEntity activityFeed, Map<String, NodeRef> userIdToAvatarNodeRefCache) {
        ...
        if (postUserId == null) {
            ...
            // user name and this will result in a 304 revalidation request. This should
            // theoretically
            // be a rare occurrence.
        } else if (userIdToAvatarNodeRefCache.containsKey(postUserId)) {
            // If we've previously cached the users avatar, or if we've determine that the user
            // doesn't
            ...
            try {
                ...
                List<AssociationRef> assocRefs =
                        this.nodeService.getTargetAssocs(postPerson, ContentModel.ASSOC_AVATAR);
                if (!assocRefs.isEmpty()) {
                    // Get the avatar for the user id, set it in the activity feed and update the
                    // cache
    ...
    public List<ActivityFeedEntity> getUserFeedEntries(
            String feedUserId,
            String siteId,
            boolean excludeThisUser,
            boolean excludeOtherUsers,
            long minFeedId) {
        return getUserFeedEntries(
                feedUserId, siteId, excludeThisUser, excludeOtherUsers, null, null, minFeedId);
    }
    ...
    public List<ActivityFeedEntity> getUserFeedEntries(
        ...
        try {
            ...
            // Create a local cache just for this method to map IDs of users to their avatar
            // NodeRef. This
            // is local to the method because we only want to cache per request - there is not point
            // in keeping
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Selected feed entries for user : '"
                                + feedUserId
                                + "',\n for site : '"
                                + siteId
                                + "',\n excluding this user : '"
            ...
            for (ActivityFeedEntity activityFeed : activityFeeds) {
                if (actvityFilter != null
                        && !actvityFilter.contains(activityFeed.getActivityType())) {
                ...
                if (userFilter != null && !userFilter.contains(activityFeed.getPostUserId())) {
                    if (logger.isTraceEnabled()) {
                        logger.trace(
                                "Filtering " + activityFeed.toString() + " \n by the user filter.");
                    }
                    continue;
                }
                String siteNetwork = activityFeed.getSiteNetwork();
                if (siteId == null) {
                    // note: pending requirements for THOR-224, for now assume all activities are
                    // within context of site and filter by current tenant
                    if (siteNetwork != null
                            && !currentTenantDomain.equals(tenantService.getDomain(siteNetwork))) {
                        continue;
                    }
                }

                // In order to prevent unnecessary 304 revalidations on user avatars in the activity
                // stream the
                // activity posting user avatars will be retrieved and added to the activity feed.
                // This will enable
                // avatars to be requested using the unique nodeRef which can be safely cached by
                // the browser and
                // improve performance...
                NodeRef avatarNodeRef = null;
                String postUserId = activityFeed.getPostUserId();
                if (userIdToAvatarNodeRefCache.containsKey(postUserId)) {
                    // If we've previously cached the users avatar, or if we've determine that the
                    // user doesn't
                    // have an avatar then use the cached data.
                    avatarNodeRef = userIdToAvatarNodeRefCache.get(postUserId);
                } else {
                    try {
                        ...
                        List<AssociationRef> assocRefs =
                                this.nodeService.getTargetAssocs(
                                        postPerson, ContentModel.ASSOC_AVATAR);
                        if (!assocRefs.isEmpty()) {
                            // Get the avatar for the user id, set it in the activity feed and
                            // update the cache
                            avatarNodeRef = assocRefs.get(0).getTargetRef();
                        }
                    } catch (NoSuchPersonException e) {
                        if (logger.isDebugEnabled()) {
                            logger.warn(
                                    "getUserFeedEntries: person no longer exists: " + postUserId);
                        }
                    }
    ...
    public List<String> getSiteFeedEntries(String siteId) {
        ...
        // Create a local cache just for this method to map IDs of users to their avatar NodeRef.
        // This
        // is local to the method because we only want to cache per request - there is not point in
        // keeping
        // an instance cache because the data will become stale if a user changes their avatar.
        Map<String, NodeRef> userIdToAvatarNodeRefCache = new HashMap<String, NodeRef>();
        ...
        try {
            ...
            List<ActivityFeedEntity> activityFeeds =
                    feedDAO.selectSiteFeedEntries(siteId, maxFeedItems);
            ...
            for (ActivityFeedEntity activityFeed : activityFeeds) {
                activityFeed.setSiteNetwork(
                        tenantService.getBaseName(activityFeed.getSiteNetwork()));
            ...
            AlfrescoRuntimeException are =
                    new AlfrescoRuntimeException(
                            "Unable to get site feed entries: " + se.getMessage());
            logger.error(are);
            throw are;
        } catch (JSONException je) {
            AlfrescoRuntimeException are =
                    new AlfrescoRuntimeException(
                            "Unable to get site feed entries: " + je.getMessage());
            logger.error(are);
            throw are;
        }
    ...
    public void setFeedControl(FeedControl feedControl) {
        ...
        if (userId == null) {
            throw new AlfrescoRuntimeException(
                    "Current user " + userId + " is not permitted to set feed control");
        }
        ...
        try {
            if (!existsFeedControl(feedControl)) {
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Inserting feed control for siteId: "
                                    + feedControl.getSiteId()
                                    + ",\n appToolId : "
                                    + feedControl.getAppToolId()
                                    + ",\n for user : "
                                    + userId);
                }
                // MNT-9104 If username contains uppercase letters the action of joining a site will
                // not be displayed in "My activities"
                if (!userNamesAreCaseSensitive) {
            ...
            AlfrescoRuntimeException are =
                    new AlfrescoRuntimeException("Failed to set feed control: " + e, e);
            logger.error(are);
            throw are;
        }
    }
    ...
    public List<FeedControl> getFeedControls(String userId) {
        ...
        if ((currentUser == null)
            ...
            throw new AlfrescoRuntimeException(
                    "Current user "
                            + currentUser
                            + " is not permitted to get feed controls for "
                            + userId);
        }
    ...
    private List<FeedControl> getFeedControlsImpl(String userId) {
        ...
        try {
            ...
            for (FeedControlEntity feedControlDao : feedControlDaos) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Found feed control for userId: "
                                    + userId
                                    + ",\n appToolId : "
                                    + feedCtrl.getAppToolId()
                                    + ",\n siteId: "
                                    + feedCtrl.getSiteId());
                }
            }
            ...
            AlfrescoRuntimeException are =
                    new AlfrescoRuntimeException("Failed to get feed controls: " + e, e);
            logger.error(are);
            throw are;
        }
    }
    ...
    public void unsetFeedControl(FeedControl feedControl) {
        ...
        if (userId == null) {
            throw new AlfrescoRuntimeException(
                    "Current user " + userId + " is not permitted to unset feed control");
        }

        try {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Deleting feed control for siteId: "
                                + feedControl.getSiteId()
                                + ",\n appToolId : "
                                + feedControl.getAppToolId()
                                + ",\n for user : "
            ...
            // MNT-9104 If username contains uppercase letters the action of joining a site will not
            // be displayed in "My activities"
            if (!userNamesAreCaseSensitive) {
                userId = userId.toLowerCase();
            }
            ...
            AlfrescoRuntimeException are =
                    new AlfrescoRuntimeException("Failed to unset feed control: " + e, e);
            logger.error(are);
            throw are;
        }
    }
    ...
    public boolean existsFeedControl(FeedControl feedControl) {
        ...
        try {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Selecting feed control for siteId: "
                                + feedControl.getSiteId()
                                + ",\n appToolId : "
                                + feedControl.getAppToolId()
                                + ",\n for user : "
            ...
            AlfrescoRuntimeException are =
                    new AlfrescoRuntimeException("Failed to query feed control: " + e, e);
            logger.error(are);
            throw are;
        }
    }
    ...
    private String getCurrentUser() {
        ...
        if ((userId != null)
                ...
                && (!AuthenticationUtil.getSystemUserName()
                        .equals(this.tenantService.getBaseNameUser(userId)))) {

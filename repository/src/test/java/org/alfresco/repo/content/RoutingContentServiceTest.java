...
public class RoutingContentServiceTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        contentService =
                (ContentService) ctx.getBean(ServiceRegistry.CONTENT_SERVICE.getLocalName());
    ...
    public void testSetUp() throws Exception {
        ...
        assertFalse(
                getUserTransaction() == getUserTransaction()); // ensure txn instances aren't shared
    }
    ...
    public void testAutoSettingOfProperties() throws Exception {
        ...
        long length =
                SOME_CONTENT.getBytes(reader.getEncoding()).length; // ensures correct decoding
    ...
    public void testWriteToNodeWithoutAnyContentProperties() throws Exception {
        ...
        assertEquals(
                "Metadata didn't get set on reader",
                writer.getContentData(),
    ...
    public void testGetRawReader() throws Exception {
        ...
        // Now write something
        ContentWriter writer =
                contentService.getWriter(contentNodeRef, ContentModel.PROP_CONTENT, false);
    ...
    public void testMissingContent() throws Exception {
        ...
        ContentPropertyRestrictionInterceptor contentPropertyRestrictionInterceptor =
                (ContentPropertyRestrictionInterceptor)
                        ctx.getBean("contentPropertyRestrictionInterceptor");
    ...
    public void testSimpleWrite() throws Exception {
        ...
        ContentWriter writer =
                contentService.getWriter(
                        contentNodeRef, ContentModel.PROP_CONTENT, false); // no updating of URL
        assertNotNull("Writer should not be null", writer);
    ...
    public void testOnContentUpdatePolicy() {
        ...
        ContentWriter contentWriter =
                this.contentService.getWriter(contentNodeRef, ContentModel.PROP_CONTENT, true);
        ...
        ContentWriter contentWriter3 =
                this.contentService.getWriter(contentNodeRef, ContentModel.PROP_CONTENT, false);
    ...
    public void testTempWrite() throws Exception {
        ...
        // check
        assertNotSame(
                "Temp URLs must be different", writer1.getContentUrl(), writer2.getContentUrl());
    }
    ...
    public void testUpdatingWrite() throws Exception {
        ...
        ContentData contentData =
                (ContentData) nodeService.getProperty(contentNodeRef, ContentModel.PROP_CONTENT);
        ...
        assertEquals(
                "Mismatched URL between writer and node",
                writer.getContentUrl(),
                contentData.getContentUrl());

        // check that the content size was set
        assertEquals(
                "Reader content length and node content length different",
                reader.getSize(),
                contentData.getSize());
     ...
     *
     * <p>It is only when the streams are closed that the node is updated.
     */
    ...
    /**
     * Create several threads that will attempt to write to the same node property. The
     * ContentWriter is handed to the thread, so this checks that the stream closure uses the
     * transaction that called <code>close</code> and not the transaction that fetched the <code>
     * ContentWriter</code>.
     */
    ...
    /**
     * Writes some content to the writer's output stream and then aquires a lock on the writer,
     * waits until notified and then closes the output stream before terminating.
     *
     * <p>When firing thread up, be sure to call <code>notify</code> on the Thread instance in order
     * to let the thread run to completion.
     */
    ...
    public void testPostCopyContentRetrieval() throws Exception {
        ...
        assertEquals(
                "ContentData not the same from NodeService and from ContentWriter",
                writerContentData,
                nodeContentData);
        ...
        ContentData copyNodeContentData =
                (ContentData)
                        nodeService.getProperty(contentCopyNodeRef, ContentModel.PROP_CONTENT);
        assertNotNull(copyNodeContentData);
        // The copy should share the same URL even
        assertEquals(
                "Copied node's cm:content ContentData was different",
                writerContentData,
                copyNodeContentData);

        // Delete the first node and ensure that the second valud remains good and the content is
        // editable
        nodeService.deleteNode(contentNodeRef);
        copyNodeContentData =
                (ContentData)
        ...
        assertNotNull(copyNodeContentData);
        assertEquals(
                "Post-delete value didn't remain the same", writerContentData, copyNodeContentData);
        ContentReader copyNodeContentReader =
                contentService.getReader(contentCopyNodeRef, ContentModel.PROP_CONTENT);
    ...
    public void testRollbackCleanup_ALF2890() throws Exception {
        ...
        try {
            ...
            nodeService.setProperty(
                    contentNodeRef, ContentModel.PROP_CONTENT, simpleWriter.getContentData());
            ...
            RetryingTransactionCallback<Void> failToSetPropCallback =
                    new RetryingTransactionCallback<Void>() {
                        public Void execute() throws Throwable {
                            nodeService.setProperty(
                                    contentNodeRef, ContentModel.PROP_CONTENT, simpleWriterData);
            ...
            try {
                transactionService
                        ...
                        .doInTransaction(failToSetPropCallback);
            ...
            assertTrue(
                    "Content was cleaned up when it originated outside of the transaction",
                    readerStep3.exists());
            assertEquals("Incorrect content", "STEP 3", readerStep3.getContentString());
            ...
            RetryingTransactionCallback<Void> rollbackCallback =
                    new RetryingTransactionCallback<Void>() {
                        public Void execute() throws Throwable {
                            ContentWriter writer =
                                    contentService.getWriter(
                                            contentNodeRef, ContentModel.PROP_CONTENT, true);
                            ...
                            assertEquals(
                                    "Incorrect content",
                                    "UNLUCKY CONTENT",
                                    writer.getReader().getContentString());
                            readers[0] = reader;
            ...
            assertFalse(
                    "Content was not cleaned up after having been created in-transaction",
                    readers[0].exists());
        } finally {
            contentPropertyRestrictionInterceptor.setGlobalContentPropertyRestrictionWhiteList("");
        }

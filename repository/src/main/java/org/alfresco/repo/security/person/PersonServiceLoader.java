...
public class PersonServiceLoader {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      int batchCount = argHelper.getIntegerValue(
        "batch-count",
        true,
        1,
        Integer.MAX_VALUE
      );
      int batchSize = argHelper.getIntegerValue(
        "batch-size",
        true,
        1,
        Integer.MAX_VALUE
      );
      int threads = argHelper.getIntegerValue(
        "threads",
        true,
        1,
        Integer.MAX_VALUE
      ...
      PersonServiceLoader loader = new PersonServiceLoader(
        ctx,
        batchSize,
        batchCount
      );
      ...
      NodeRef autoHomeFolder = DefaultTypeConverter.INSTANCE.convert(
        NodeRef.class,
        nodeService.getProperty(autoPerson, ContentModel.PROP_HOMEFOLDER)
      );
      if (autoHomeFolder == null) {
        throw new IllegalStateException(
          "Home folder not created lazily for auto created users"
        );
      }
      ...
      try {
        ...
        for (int i = 0; i < batchCount; i++) {
          ...
          transactionService
            ...
            .doInTransaction(makeUsersCallback, false, true);

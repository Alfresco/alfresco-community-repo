...
public class RepoService {
    ...
    public RepoService(ApplicationContext applicationContext) throws Exception {
        ...
        this.nodeRatingSchemeRegistry =
                (NamedObjectRegistry<RatingScheme>)
                        applicationContext.getBean("nodeRatingSchemeRegistry");
        this.cociService =
        ...
        CronTrigger contentStoreCleanerTrigger =
                (CronTrigger) applicationContext.getBean("contentStoreCleanerTrigger");
        scheduler.pauseJob(contentStoreCleanerTrigger.getJobKey());

        ChildApplicationContextFactory activitiesFeed =
                (ChildApplicationContextFactory) applicationContext.getBean("ActivitiesFeed");
    ...
    //
    // TODO replace with V1 REST API to Lock/Unlock - except calls to includeChildren (which may not
    ...
    public void lockNode(
            NodeRef nodeRef, LockType lockType, int timeToExpire, boolean includeChildren) {
    ...
    public NodeRef addUserDescription(
            final String personId, final TestNetwork network, final String personDescription) {
                    ...
                    public NodeRef doWork() throws Exception {
                        ...
                        ContentWriter writer =
                                contentService.getWriter(
                                        userRef, ContentModel.PROP_PERSONDESC, true);
    ...
    public TestPerson createUser(
            final PersonInfo personInfo, final String username, final TestNetwork network) {
                        ...
                        if (!isDefaultAdmin(username, network)) {
                            ...
                            if (personNodeRef == null) {
                                ...
                                authenticationService.createAuthentication(
                                        username, personInfo.getPassword().toCharArray());

                                if (EnterpriseTestFixture.WITH_AVATAR.equals(
                                        personInfo.getInstantmsg())) {
    ...
    public TestSite createSite(TestNetwork network, final SiteInformation siteInfoIn) {
        ...
        siteService.createContainer(
                siteInfoIn.getShortName(), "documentLibrary", ContentModel.TYPE_FOLDER, null);
    ...
    public List<Invitation> getModeratedSiteInvitations(
            String networkId, String runAsUserId, final String inviteeId, final String siteId) {
        List<Invitation> invitations =
                TenantUtil.runAsUserTenant(
                        new TenantRunAsWork<List<Invitation>>() {
                            @Override
                            public List<Invitation> doWork() throws Exception {
                                InvitationSearchCriteriaImpl searchCriteria =
                                        new InvitationSearchCriteriaImpl();
                                ...
                                List<Invitation> invitations =
                                        invitationService.searchInvitation(searchCriteria);
                                return invitations;
                            }
                        },
                        runAsUserId,
    ...
    public List<Activity> getActivities(
            String personId, String siteId, boolean excludeUser, boolean excludeOthers) {
        List<ActivityFeedEntity> feedEntities =
                activityService.getUserFeedEntries(personId, siteId, excludeUser, excludeOthers, 0);
    ...
    public void checkSiteMember(final TestPerson person, final TestSite site, SiteRole siteRole)
            throws Exception {
        assertTrue(siteService.isMember(site.getSiteId(), person.getId()));
        assertEquals(
                siteRole.toString(), siteService.getMembersRole(site.getSiteId(), person.getId()));
    }
    ...
    private PagingResults<SiteInfo> getFavouriteSites(
            String userName, PagingRequest pagingRequest) {
        final Collator collator = Collator.getInstance();

        final Set<SiteInfo> sortedFavouriteSites =
                new TreeSet<SiteInfo>(
        ...
        Map<String, Serializable> prefs =
                preferenceService.getPreferences(userName, FAVOURITE_SITES_PREFIX);
    ...
    public List<FavouriteSite> getFavouriteSites(TestPerson user) {
        ...
        PagingResults<SiteInfo> pagingResults =
                getFavouriteSites(user.getId(), new PagingRequest(0, Integer.MAX_VALUE));
        for (SiteInfo siteInfo : pagingResults.getPage()) {
            ...
            if (site.isMember(user.getId())) {
                ...
                favouriteSite =
                        new FavouriteSite(null, siteId, siteGuid, null, null, null, null, null);
            }
                        ...
                        if (dictionaryService.isSubClass(type, ContentModel.TYPE_CONTENT)) {
                            ...
                            document =
                                    FavouriteDocument.getDocument(
                                            nodeRef.getId(), nodeRef.getId(), properties);
                        } else {
    ...
    public List<NodeRating> getNodeRatings(
            String personId, String networkId, final NodeRef nodeRef) {
                ...
                TenantUtil.runAsUserTenant(
                        new TenantRunAsWork<List<NodeRating>>() {
                            ...
                            public List<NodeRating> doWork() throws Exception {
                                ...
                                {
                                    Rating likesRating =
                                            ratingService.getRatingByCurrentUser(
                                                    nodeRef, "likesRatingScheme");
                                    Object myRating = null;
                                    String ratedAt = null;
                                    if (likesRating != null) {
                                        myRating =
                                                likesRating.getScore() == 1.0f
                                                        ? Boolean.TRUE
                                                        : Boolean.FALSE;
                                        Date rateTime = likesRating.getAppliedAt();
                                        ratedAt =
                                                PublicApiDateFormat.getDateFormat()
                                                        .format(rateTime);
                                    }
                                    int ratingsCount =
                                            ratingService.getRatingsCount(
                                                    nodeRef, "likesRatingScheme");
    ...
    public NodeRef addToDocumentLibrary(TestSite site, String name, QName type) {
        NodeRef documentLibraryContainerNodeRef =
                siteService.getContainer(site.getSiteId(), "documentLibrary");
        NodeRef nodeRef =
                fileFolderService.create(documentLibraryContainerNodeRef, name, type).getNodeRef();
        return nodeRef;
    }
    ...
    public void postActivity(
            final String activityType, final String siteId, final JSONObject activityData)
            throws JobExecutionException {
        activityService.postActivity(
                activityType, siteId, "documentlibrary", activityData.toString());
    }

    public NodeRef createDocument(
            final NodeRef parentNodeRef, final String name, final String content) {
        NodeRef nodeRef =
                fileFolderService
    ...
    public NodeRef createObjectOfCustomType(
            final NodeRef parentNodeRef, final String name, final String typeName) {
        QName assocQName =
                QName.createQName(
                        NamespaceService.CONTENT_MODEL_1_0_URI, QName.createValidLocalName(name));
        ...
        NodeRef nodeRef =
                nodeService
                        .createNode(
                                parentNodeRef, ContentModel.ASSOC_CONTAINS, assocQName, typeQName)
                        .getChildRef();
    ...
    public NodeRef createComment(NodeRef nodeRef, final Comment comment) {
        NodeRef commentNodeRef =
                commentService.createComment(
                        nodeRef, comment.getTitle(), comment.getContent(), false);
    ...
    public class TestSite extends SiteImpl {
        ...
        public NodeRef createContainer(String containerName) {
            return siteService.createContainer(
                    getSiteId(), containerName, ContentModel.TYPE_FOLDER, null);
        }
                    ...
                    TenantUtil.runAsSystemTenant(
                            new TenantRunAsWork<Map<String, String>>() {
                                ...
                                public Map<String, String> doWork() throws Exception {
                                    ...
                                    Map<String, String> members =
                                            siteService.listMembers(getSiteId(), null, null, 0);
                                    return members;
                                }
                            },
                            getAccount().getId());
    ...
    public static <T extends Object> Iterator<T> getWrappingIterator(
            final int startIdx, final List<T> list) {
    ...
    public class TestNetwork extends NetworkImpl implements Comparable<TestNetwork> {
        ...
        public TestSite createSite(String siteRootName, SiteVisibility siteVisibility) {
            String shortName =
                    "TESTSITE" + (siteRootName != null ? siteRootName : "") + GUID.generate();
            SiteInformation siteInfo =
                    new SiteInformation(shortName, shortName, shortName, siteVisibility);
        ...
        public TestSite createSite(
            ...
            SiteInformation siteInfo =
                    new SiteInformation(id, title, description, sitePreset, visibility);
            // Used deprecated createSite method until will be allowed creating a site with
            // sitePreset
            return createSite(siteInfo);
        }
        ...
        /**
         * @deprecated replace with AbstractBaseApiTest.createSite (or
         *     PublicApiClient.sites.createSite)
         */
                            ...
                            new TenantRunAsWork<NodeRef>() {
                                public NodeRef doWork() throws Exception {
                                    return RepoService.this.addUserDescription(
                                            personId, TestNetwork.this, personDescription);
                                }
                            },
                            getId());
        ...
        public TestPerson createUser(final PersonInfo personInfo) {
            final String username =
                    publicApiContext.createUserName(personInfo.getUsername(), getId());
                                ...
                                public TestPerson doWork() throws Exception {
                                    TestPerson person =
                                            RepoService.this.createUser(
                                                    personInfo, username, TestNetwork.this);
                    ...
                    new TenantRunAsWork<TestSite>() {
                        ...
                        public TestSite doWork() throws Exception {
                            ...
                            for (SiteInfo info : siteService.listSites(null, null)) {
                                boolean isMember =
                                        siteService.isMember(info.getShortName(), personId);
                                if (!isMember) {
                                    match = info;
                                    break;
                                }
                            ...
                            new TenantRunAsWork<List<TestSite>>() {
                                ...
                                public List<TestSite> doWork() throws Exception {
                                    ...
                                    for (SiteInfo siteInfo : results) {
                                        TestSite site =
                                                new TestSite(TestNetwork.this, siteInfo /*, null*/);
                                    ...
                                    for (SiteInfo siteInfo : sites) {
                                        ...
                                        String role =
                                                siteService.getMembersRole(
                                                        siteInfo.getShortName(), userId);
                                        MemberOfSite ms =
                                                new MemberOfSite(site, SiteRole.valueOf(role));
                                        personSites.add(ms);
                                    }
            ...
            return TenantUtil.runAsUserTenant(
                    new TenantRunAsWork<List<PersonNetwork>>() {
                        ...
                        public List<PersonNetwork> doWork() throws Exception {
                            ...
                            PagingResults<Network> networks =
                            ...
                            for (Network network : networks.getPage()) {
                                ...
                                PersonNetwork personNetwork =
                                        new PersonNetwork(network.getIsHomeNetwork(), restNetwork);
    ...
    public static class SiteInformation implements Comparable<SiteInformation> {
        ...
        public SiteInformation(
                String shortName, String title, String description, SiteVisibility siteVisibility) {
            super();
            if (shortName == null) {
                throw new IllegalArgumentException();
            }

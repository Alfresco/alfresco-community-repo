...
public class RenditionsTest extends AbstractBaseApiTest {
    ...
    public void setup() throws Exception {
        ...
        synchronousTransformClient =
                applicationContext.getBean(
                        "synchronousTransformClient", SynchronousTransformClient.class);
     ...
     *
     * <p>GET: {@literal
     * <host>:<port>/alfresco/api/<networkId>/public/alfresco/versions/1/nodes/<nodeId>/renditions}
     */
    @Test
    public void testListNodeRenditions() throws Exception {
        ...
        String folder_Id =
                addToDocumentLibrary(userOneN1Site, folderName, TYPE_CM_FOLDER, userOneN1.getId());
        ...
        // List all available renditions (includes those that have been created and those that are
        // yet to be created)
        ...
        String emptyContentNodeId =
                addToDocumentLibrary(
                        userOneN1Site, "emptyDoc.txt", TYPE_CM_CONTENT, userOneN1.getId());
     ...
     *
     * <p>GET: {@literal
     * <host>:<port>/alfresco/api/<networkId>/public/alfresco/versions/1/nodes/<nodeId>/renditions/<renditionId>}
     */
    @Test
    ...
    public void testCreateRendition() throws Exception {
        ...
        if (areLocalTransformsAvailable()) {
            ...
            file =
                    TempFileProvider.createTempFile(
                            ...
                            ".bin");
            multiPartBuilder =
                    MultiPartBuilder.create().setFileData(new FileData("binaryFileName", file));
        ...
        RenditionService2Impl renditionService2 =
                applicationContext.getBean("renditionService2", RenditionService2Impl.class);
    ...
    protected void assertRenditionCreatedWithWait(String contentNodeId, String... renditionNames)
            throws Exception {
    ...
    public void testCreateRenditionForNewVersion() throws Exception {
        ...
        File file = getResourceFile(fileName);
        MultiPartRequest reqBody =
                MultiPartBuilder.create().setFileData(new FileData(fileName, file)).build();
        ...
        // TODO add test to request creation of rendition as another user (that has read-only access
        // on the content, not write)
        ...
        assertNotEquals(
                document1b.getProperties().get(PROP_LTM), document1.getProperties().get(PROP_LTM));
        ...
        assertNotEquals(
                document2b.getProperties().get(PROP_LTM), document1b.getProperties().get(PROP_LTM));
    }
     ...
     *
     * <p>GET: {@literal
     * <host>:<port>/alfresco/api/<networkId>/public/alfresco/versions/1/nodes/<nodeId>/renditions/<renditionId>/content}
     */
    @Test
    public void testDownloadRendition() throws Exception {
        ...
        // Here we want to overwrite/update the existing content in order to force a new rendition
        // creation,
        // so the ContentModel.PROP_MODIFIED date would be different. Hence, we use the multipart
        // upload by providing
        ...
        // The content of the rendition does not exist and the placeholder parameter has a value of
        // "false"
        params = Collections.singletonMap("placeholder", "false");
        getSingle(getNodeRenditionsUrl(contentNodeId), "doclib/content", params, 404);

        // The rendition does not exist, a placeholder is not available and the placeholder
    ...
    private String addToDocumentLibrary(Site testSite, String name, String nodeType, String userId)
            throws Exception {
    ...
    /**
     * Returns <code>true</code> if doc to pdf transformations available, indicating Local
     * transforms ar available.
     */

 ...
 *
 * <p>The keys must have good <code>equals</code> and <code>hashCode</code> implementations and must
 * respect the case-sensitivity of the use-case.
 *
 * <p>All keys will be unique to the given cache region, allowing the cache to be shared between
 * instances of this class.
 *
 * <p>Generics:
 *
 * <ul>
 *   <li>K: The database unique identifier.
 *   <li>V: The value stored against K.
 *   <li>VK: The a value-derived key that will be used as a cache key when caching K for lookups by
 *       V. This can be the value itself if it is itself a good key.
 * </ul>
 *
 * @author Derek Hulley
 * @since 3.2
...
public class EntityLookupCache<K extends Serializable, V extends Object, VK extends Serializable> {
    ...
    public static interface EntityLookupCallbackDAO<
            K1 extends Serializable, V1 extends Object, VK1 extends Serializable> {
        /**
         ...
         * possible.
         *
         * <p>Implementations will often return the value itself, provided that the value is both
         * serializable and has a good <code>equals</code> and <code>hashCode</code>.
         *
         * <p>Were no adequate key can be generated for the value, then <tt>null</tt> can be
         * returned. In this case, the {@link #findByValue(Object) findByValue} method might not
         * even do a search and just return <tt>null</tt> itself i.e. if it is difficult to look the
         * value up in storage then it is probably difficult to generate a cache key from it, too..
         * In this scenario, the cache will be purely for key-based lookups
         *
        ...
        /**
         * Find and entity using the given value key. The <code>equals</code> and <code>hashCode
         * </code> methods of the value object should respect case-sensitivity in the same way that
         * this lookup treats case-sensitivity i.e. if the <code>equals</code> method is
         * <b>case-sensitive</b> then this method should look the entity up using a
         * <b>case-sensitive</b> search.
         *
         * <p>Since this is a cache backed by some sort of database, <tt>null</tt> values are
         * allowed by the cache. The implementation of this method can throw an exception if
         * <tt>null</tt> is not appropriate for the use-case.
         *
         * <p>If the search is impossible or expensive, this method should just return
         * <tt>null</tt>. This would usually be the case if the {@link #getValueKey(Object)
         * getValueKey} method also returned <tt>null</tt> i.e. if it is difficult to look the value
         * up in storage then it is probably difficult to generate a cache key from it, too.
         *
         * @param value the value (business object) used to identify the entity (<tt>null</tt>
         *     allowed).
         ...
         *
         * <p>Since persistence mechanisms often allow <tt>null</tt> values, these can be expected
         ...
         *
         * <p>It is up to the client code to decide if a <tt>0</tt> return value indicates a
         ...
         * concurrency violation or not.
         *
         * @param value the value (business object) used to identify the enitity (<tt>null</tt>
         *     allowed)
         * @return Returns the row deletion count.
         * @throws UnsupportedOperationException if entity deletion is not supported
         */
     ...
     * @since 3.2
     */
    public abstract static class EntityLookupCallbackDAOAdaptor<
                    K2 extends Serializable, V2 extends Object, VK2 extends Serializable>
            implements EntityLookupCallbackDAO<K2, V2, VK2> {
        /**
         * This implementation never finds a value and is backed by {@link #getValueKey(Object)}
         * returning nothing.
         *
        ...
        /**
         * This implementation does not find by value and is backed by {@link #findByValue(Object)}
         * returning nothing.
         *
         * @return Returns <tt>null</tt> always
         */
    ...
    public EntityLookupCache(
            SimpleCache cache, String cacheRegion, EntityLookupCallbackDAO<K, V, VK> entityLookup) {
    ...
    /**
     * Find the entity associated with the given key. The {@link
     * EntityLookupCallbackDAO#findByKey(Serializable) entity callback} will be used if necessary.
     *
     * <p>It is up to the client code to decide if a <tt>null</tt> return value indicates a
     * concurrency violation or not; the former would normally result in a concurrency-related
     * exception such as {@link ConcurrencyFailureException}.
     *
    ...
    /**
     * Find the entity associated with the given value. The {@link
     * EntityLookupCallbackDAO#findByValue(Object) entity callback} will be used if no entry exists
     * in the cache.
     *
     * <p>It is up to the client code to decide if a <tt>null</tt> return value indicates a
     * concurrency violation or not; the former would normally result in a concurrency-related
     ...
     * This method takes the opposite approach to {@link #getOrCreateByValue(Object)}, which assumes
     * the entity's existence: in this case the entity is assumed to NOT exist. The {@link
     * EntityLookupCallbackDAO#createValue(Object)} and {@link
     * EntityLookupCallbackDAO#findByValue(Object)} will be used if necessary.<br>
     *
     * <p>Use this method when the data involved is seldom reused.
     *
     * @param value The entity value (<tt>null</tt> is allowed)
     * @param controlDAO an essential DAO required in order to ensure a transactionally-safe attempt
     *     at data creation
    ...
    public Pair<K, V> createOrGetByValue(V value, ControlDAO controlDAO) {
        if (controlDAO == null) {
            throw new IllegalArgumentException(
                    "The ControlDAO is required in order to perform a safe attempted insert.");
        }
        ...
        try {
            ...
            // Fall through to the usual way, which should find it if the failure cause was a
            // duplicate key
    ...
    /**
     * Find the entity associated with the given value and create it if it doesn't exist. The {@link
     * EntityLookupCallbackDAO#findByValue(Object)} and {@link
     * EntityLookupCallbackDAO#createValue(Object)} will be used if necessary.
     *
     * @param value The entity value (<tt>null</tt> is allowed)
     * @return Returns the key-value pair (new or existing and never <tt>null</tt>)
     */
    ...
    /**
     * Update the entity associated with the given key. The {@link
     * EntityLookupCallbackDAO#updateValue(Serializable, Object)} callback will be used if
     * necessary.
     *
     * <p>It is up to the client code to decide if a <tt>0</tt> return value indicates a concurrency
     * violation or not; usually the former will generate {@link ConcurrencyFailureException} or
     * something recognised by the {@link RetryingTransactionHelper#RETRY_EXCEPTIONS
    ...
    /**
     * Delete the entity associated with the given key. The {@link
     * EntityLookupCallbackDAO#deleteByKey(Serializable)} callback will be used if necessary.
     *
     * <p>It is up to the client code to decide if a <tt>0</tt> return value indicates a concurrency
     * violation or not; usually the former will generate {@link ConcurrencyFailureException} or
     * something recognised by the {@link RetryingTransactionHelper#RETRY_EXCEPTIONS
    ...
    /**
     * Delete the entity having the given value.. The {@link
     * EntityLookupCallbackDAO#deleteByValue(Object)} callback will be used if necessary.
     *
     * <p>It is up to the client code to decide if a <tt>0</tt> return value indicates a concurrency
     * violation or not; usually the former will generate {@link ConcurrencyFailureException} or
     * something recognised by the {@link RetryingTransactionHelper#RETRY_EXCEPTIONS
     ...
     *
     * <p><b>NOTE:</b> This operation removes ALL entries for ALL cache regions.
     */
    public void clear() {
        // Handle missing cache
        if (cache == null) {

...
public class ContentServiceImpl
  implements ContentService, ApplicationContextAware {
  ...
  public void setEagerContentStoreCleaner(
    EagerContentStoreCleaner eagerContentStoreCleaner
  ) {
  ...
  public void init() {
    ...
    this.onContentPropertyUpdateDelegate =
      this.policyComponent.registerClassPolicy(
          OnContentPropertyUpdatePolicy.class
        );
  ...
  public void onUpdateProperties(
    ...
    if (fire) {
      ...
      types = getTypes(nodeRef, types);
      OnContentUpdatePolicy policy = onContentUpdateDelegate.get(
        nodeRef,
        types
      );
  ...
  private ContentReader getReader(
    ...
    if (reader != null && fireContentReadPolicy == true) {
      ...
      types.add(this.nodeService.getType(nodeRef));
      OnContentReadPolicy policy =
        this.onContentReadDelegate.get(nodeRef, types);
  ...
  private ContentData getContentData(NodeRef nodeRef, QName propertyQName) {
    ...
    if (contentData == null) {
      PropertyDefinition contentPropDef = dictionaryService.getProperty(
        propertyQName
      );
      ...
      if (
        ...
        throw new InvalidTypeException(
          ...
          "\n" +
          "   property type: " +
          ((contentPropDef == null) ? "unknown" : contentPropDef.getDataType()),
          propertyQName
        );
      }
    }
  ...
  public ContentWriter getWriter(
    ...
    ContentReader existingContentReader = getReader(
      nodeRef,
      propertyQName,
      false
    );
    ...
    ContentContext ctx = new NodeContentContext(
    ...
    if (update) {
      ...
      WriteStreamListener listener = new WriteStreamListener(
        nodeService,
        nodeRef,
        propertyQName,
        writer
  ...
  public boolean isContentDirectUrlEnabled() {
    return (
      systemWideDirectUrlConfig.isEnabled() && store.isContentDirectUrlEnabled()
    );
  }
  ...
  public DirectAccessUrl requestContentDirectUrl(
    ...
    if (!systemWideDirectUrlConfig.isEnabled()) {
      throw new DirectAccessUrlDisabledException(
        "Direct access url isn't available."
      );
    }

...
public class PersonServiceTest extends BaseWebScriptTest {
  ...
  protected void setUp() throws Exception {
    ...
    this.userNameMatcherImpl =
      (UserNameMatcherImpl) ctx.getBean("userNameMatcher");
    this.nodeService = (NodeService) ctx.getBean("NodeService");
    this.contentService = (ContentService) ctx.getBean("contentService");
    this.userUsageTrackingComponent =
    ...
    this.transactionService =
      (TransactionService) ctx.getBean("TransactionService");

    serviceRegistry =
      (ServiceDescriptorRegistry) ctx.getBean("ServiceRegistry");
    serviceRegistry.setMockSearchService(mockSearchService);
    when(mockSearchService.query(any()))
      .thenReturn(mockSearchServiceQueryResultSet);
    when(mockSearchServiceQueryResultSet.getNodeRefs())
      .thenReturn(dummySearchServiceQueryNodeRefs);
  ...
  private JSONObject updatePerson(
    String userName,
    String title,
    String firstName,
    String lastName,
  ...
  private JSONObject createPerson(
    String userName,
    String title,
    String firstName,
    String lastName,
    ...
    Response response = sendRequest(
      new PostRequest(URL_PEOPLE, person.toString(), "application/json"),
      expectedStatus
    );
  ...
  private JSONObject deletePerson(String userName, int expectedStatus)
    ...
    Response response = sendRequest(
      new DeleteRequest(URL_PEOPLE + "/" + userName),
      expectedStatus
    );
  ...
  public void testJobWithSpace() throws Exception {
    ...
    Response response = sendRequest(
      new GetRequest(
        URL_PEOPLE +
        "?filter=" +
        URLEncoder.encode("jobtitle:" + jobSearchString)
      ),
      200
    );
    ...
    response =
      sendRequest(
        new GetRequest(
          URL_PEOPLE + "?filter=" + URLEncoder.encode("jobtitle:" + userJob)
        ),
        200
      );
    assertSearchQuery(
      "jobtitle:\"" + userJob.replace(" ", "\" \"") + "\" ",
      false
    );
  ...
  public void testGetPerson() throws Exception {
    ...
    Response response = sendRequest(
      new GetRequest(URL_PEOPLE + "/" + "nonExistantUser"),
      404
    );

    // Create a person and get him/her
    String userName = RandomStringUtils.randomNumeric(6);
    JSONObject result = createPerson(
      userName,
      "myTitle",
      "myFirstName",
      "myLastName",
  ...
  public void testGetPeopleSkipCount() throws Exception {
    ...
    Response response = sendRequest(
      new GetRequest(URL_PEOPLE + "?filter=" + filter),
      200
    );
    JSONObject res = new JSONObject(response.getContentAsString());
    ...
    response =
      sendRequest(
        new GetRequest(
          URL_PEOPLE + "?filter=" + filter + "&skipCount=" + skipCount
        ),
        200
      );
    ...
    assertTrue(
      "skipCount ignored",
      res.getJSONArray("people").length() < peopleFound
    );
  }
   ...
   */
  private TestWebScriptServer.Request addHeadersToRequest(
    TestWebScriptServer.Request req
  ) {
  ...
  public void testGetPeoplePaging() throws Exception {
    ...
    Response response = sendRequest(
      new GetRequest(
        URL_PEOPLE + "?filter=" + filter + "&startIndex=" + 0 + "&pageSize=" + 6
      ),
      Status.STATUS_OK
    );
    ...
    assertEquals(
      "The number of returned results is not correct.",
      6,
      peopleAsc.length()
    );
    ...
    for (int i = 0; i < peopleAsc.length(); i++) {
      ...
      assertEquals(
        "The name of a person does not match. Paging is not correct",
        filter + i,
  ...
  private void checkPaging(
    ...
    assertEquals(
      "totalItems was not correct in the response",
      totalItems,
      paging.getInt("totalItems")
    );
    assertEquals(
      "skipCount was not correct in the response",
      skipCount,
      paging.getInt("skipCount")
    );
    assertEquals(
      "maxItems was not correct in the response",
      maxItems,
      paging.getInt("maxItems")
    );
  }
  ...
  private void checkSorting(String filter, String sortBy, String... usernames)
    throws Exception {
    dummySearchServiceQueryNodeRefs.clear();
    ...
    for (int i = 0; i < peopleAsc.length(); i++) {
      assertEquals(
        peopleAsc.getJSONObject(i).getString("userName"),
        peopleDesc
          .getJSONObject(peopleAsc.length() - i - 1)
          .getString("userName")
      );
    }
    ...
    for (int i = 0; i < peopleAsc.length() - 1; i++) {
      if (SORT_BY_USERNAME.equals(sortBy)) {
        ...
        assertTrue(
          "Users are not ordered correctly ascending by username",
          userName1.compareToIgnoreCase(userName2) <= 0
        );
        ...
        String fullName1 =
          (firstName1 == null ? "" : firstName1) +
          (lastName1 == null ? "" : lastName1);
        ...
        assertTrue(
          "Users are not ordered correctly ascending by fullname",
          fullName1.compareToIgnoreCase(fullName2) <= 0
        );
        ...
        assertTrue(
          "Users are not ordered correctly ascending by jobtitle",
          (jobUser1 == null ? "" : jobUser1).compareToIgnoreCase(
              jobUser2 == null ? "" : jobUser2
            ) <=
          0
        );
        ...
        assertTrue(
          "Users are not ordered correctly ascending by email",
          (emailUser1 == null ? "" : emailUser1).compareToIgnoreCase(
              emailUser2 == null ? "" : emailUser2
            ) <=
          0
        );
        ...
        assertTrue(
          "Users are not ordered correctly ascending by quota",
          quotaUser1 <= quotaUser2
        );
        ...
        assertTrue(
          "Users are not ordered correctly ascending by usage",
          usageUser1 <= usageUser2
        );
      }
    }
  }
  ...
  private void assertSearchQuery(String term, boolean buildFilter) {
    ...
    String expectedQuery =
      ...
      term +
      ")";
    ArgumentCaptor<SearchParameters> searchParametersCaptor = ArgumentCaptor.forClass(
      SearchParameters.class
    );
    verify(mockSearchService, times(++callCount))
      .query(searchParametersCaptor.capture());
    SearchParameters parameters = searchParametersCaptor.getValue();
    assertEquals("Query", expectedQuery, parameters.getQuery());
  }

  private void addUserUsageContent(
    final String userName,
    final int stringDataLength
  ) {
  ...
  private NodeRef getHomeSpaceFolderNode(String userName) {
    return (NodeRef) nodeService.getProperty(
      personService.getPerson(userName),
      ContentModel.PROP_HOMEFOLDER
    );
  }

  private NodeRef addTextContent(
    NodeRef folderRef,
    String name,
    String textData
  ) {
  ...
  public void testUpdatePerson() throws Exception {
    ...
    JSONObject result = updatePerson(
      userName,
      "updatedTitle",
      "updatedFirstName",
      "updatedLastName",
      "updatedOrganisation",
      "updatedJobTitle",
      "updatedFN.updatedLN@email.com",
      "updatedBio",
      "images/updatedAvatar.jpg",
      Status.STATUS_OK
    );
  ...
  public void testUserNameCaseSensitivity() throws Exception {
    ...
    try {
      ...
      assertTrue(
        "case sensitive exists by matching case",
        personService.personExists(lowerCaseUserName)
      );
      assertFalse(
        "case sensitive exists by non matching case",
        personService.personExists(upperCaseUserName)
      );
      assertNotNull(
        "case sensitive lookup by matching case",
        personService.getPerson(lowerCaseUserName)
      );
      try {
      ...
      assertNotNull(
        "case insensitive lookup by matching case",
        personService.getPerson(lowerCaseUserName)
      );
      assertNotNull(
        "case insensitive lookup by non matching case",
        personService.getPerson(upperCaseUserName)
      );
      assertTrue(
        "case insensitive exists by matching case",
        personService.personExists(lowerCaseUserName)
      );
      assertTrue(
        "case insensitive exists by non matching case",
        personService.personExists(upperCaseUserName)
      );

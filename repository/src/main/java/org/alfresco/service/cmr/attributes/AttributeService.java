 ...
 *
 * <p>Attributes are uniquely identified by up to 3 keys; <tt>null</tt> keys are themselves treated
 * uniquely i.e. <code>['a','b']</code> is equivalent to <code>['a','b',null]</code> in all cases
 * except where multiple search results are possible. Keys can be any simple <code>Serializable
 * </code> type, typically being convertable using {@link DefaultTypeConverter}. The attribute
 * values persisted can be any <code>Serializable</code> (including collections) but the raw values
 * should be convertable by the {@link DefaultTypeConverter} for the most efficient persistence.
 *
 * @author Derek Hulley
 * @since 3.4
 */
...
public interface AttributeService {
     ...
     * @return <tt>true</tt> if the attribute exists (regardless of its value) or <tt>false</tt> if
     *     it doesn't exist
     */
    ...
    /**
     * Get all attributes that share the starter keys provided. If 3 key values are given, there can
    ...
    /**
     * Set an attribute, overwriting its prior value if it already existed. <tt>null</tt> values are
     * treated as unique i.e. if the value set is <tt>null</tt> then {@link
     * #exists(Serializable...)} will still return <tt>true</tt>. If the attribute doesn't exist, it
     * will be created otherwise it will be modified.
     *
    ...
    /**
     * Create an attribute with an optional value, assuming there is no existing attribute using the
     * same keys.
     *
     * @param value The value to store (can be a collection or <tt>null</tt>)
     * @param keys List of 1 to 3 keys to uniquely identify the attribute
    ...
    /**
     * Update an attribute key whilst preserving the associated value (if any). If there is no
     * existing key matching the original value, then nothing will happen.
     *

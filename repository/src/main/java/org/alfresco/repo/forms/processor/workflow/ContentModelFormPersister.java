...
public abstract class ContentModelFormPersister<T> implements FormPersister<T> {
  ...
  private static final Log LOGGER = LogFactory.getLog(
    ContentModelFormPersister.class
  );
  ...
  public ContentModelFormPersister(
  ...
  protected boolean changeTransientAssociation(
    String fieldName,
    List<NodeRef> values,
    boolean add
  ) {
  ...
  private boolean changeAssociation(
    QName qName,
    List<NodeRef> values,
    boolean add
  ) {
  ...
  protected void logIgnore(FieldData fieldData) {
    if (logger.isDebugEnabled()) logger.debug(
      "Ignoring unrecognized field: " + fieldData.getName()
    );
  }
  ...
  protected abstract boolean removeAssociation(
    QName qName,
    List<NodeRef> values
  );

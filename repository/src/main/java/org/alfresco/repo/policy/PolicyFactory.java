...
/*package*/ class PolicyFactory<B extends BehaviourBinding, P extends Policy> {
    ...
    public Collection<P> createList(B binding) {
        ...
        for (BehaviourDefinition behaviourDef : behaviourDefs) {
            ...
            if (!(behaviour.getNotificationFrequency().equals(NotificationFrequency.EVERY_EVENT))) {
                // wrap behaviour in transaction proxy which deals with delaying invocation until
                // necessary
                if (transactionHandlerFactory == null) {
                    throw new PolicyException(
                            "Transaction-level policies not supported as transaction support for"
                                    + " the Policy Component has not been initialised.");
                }
                InvocationHandler trxHandler =
                        transactionHandlerFactory.createHandler(
                                behaviour, behaviourDef.getPolicyDefinition(), policyIF);
                policyIF =
                        (P)
    ...
    /**
     * Construct a single aggregate policy implementation for the specified collection of policy
     * implementations.
     *
    ...
    public P toPolicy(Collection<P> policyList) {
        if (policyList.size() == 1) {
            ...
            return (P)
                    Proxy.newProxyInstance(
                            policyClass.getClassLoader(), new Class[] {policyClass}, NOOPHandler);
        } else {
            return (P)
                    Proxy.newProxyInstance(
                            policyClass.getClassLoader(),
     ...
     * Convert each of the arguments to the spoofed (no tenant prefix) reference.
     *
     * <p>Converts arguments of Type NodeRef to base name etc.
     *

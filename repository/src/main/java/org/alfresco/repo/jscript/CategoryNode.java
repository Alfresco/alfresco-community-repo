...
public class CategoryNode extends ScriptNode {
  ...
  public CategoryNode createSubCategory(String name) {
    return new CategoryNode(
      services.getCategoryService().createCategory(getNodeRef(), name),
      this.services,
      this.scope
    );
  }
  ...
  public void rename(String name) {
    ...
    services
      .getNodeService()
      .setProperty(getNodeRef(), ContentModel.PROP_NAME, name);

    // ALF-1788 Need to rename the association
    ChildAssociationRef assocRef = services
      .getNodeService()
   ...
   * @return CategoryNode[]
   */
  private CategoryNode[] buildCategoryNodes(
    ...
    for (ChildAssociationRef car : cars) {
      categoryNodes[i++] =
        new CategoryNode(car.getChildRef(), this.services, this.scope);
    }
  ...
  private ScriptNode[] buildMixedNodes(Collection<ChildAssociationRef> cars) {
    ...
    for (ChildAssociationRef car : cars) {
      ...
      if (
        ...
        nodes[i++] =
          new ScriptNode(car.getChildRef(), this.services, this.scope);
      }
    }
    return nodes;
  }

...
public class QuickShareRestApiTest extends BaseWebScriptTest {
  ...
  private static final String SHARE_URL =
    "/api/internal/shared/share/{node_ref_3}";

  private static final String UNSHARE_URL =
    "/api/internal/shared/unshare/{shared_id}";
  private static final String SHARE_METADATA_URL =
    "/api/internal/shared/node/{shared_id}/metadata";
  private static final String SHARE_CONTENT_URL =
    "/api/internal/shared/node/{shared_id}/content";
  private static final String SHARE_CONTENT_THUMBNAIL_URL =
    "/api/internal/shared/node/{shared_id}/content/thumbnails/{thumbnailname}?c=force";

  private static final String SITES_URL = "/api/sites";

  // note: node_ref_3 => three segments, eg. store_protocol/store_id/node_uuid
  private static final String AUTH_METADATA_URL =
    "/api/node/{node_ref_3}/metadata";
  private static final String AUTH_CONTENT_URL =
    "/api/node/{node_ref_3}/content";
  private static final String AUTH_CONTENT_THUMBNAIL_URL =
    "/api/node/{node_ref_3}/content/thumbnails/{thumbnailname}?c=force";
  ...
  private static final String TEST_MIMETYPE_JPEG =
    MimetypeMap.MIMETYPE_IMAGE_JPEG;
  private static final String TEST_MIMETYPE_PNG =
  ...
  protected void setUp() throws Exception {
    ...
    fileFolderService =
      (FileFolderService) getServer()
        .getApplicationContext()
        .getBean("FileFolderService");
    synchronousTransformClient =
      (SynchronousTransformClient) getServer()
        .getApplicationContext()
        .getBean("synchronousTransformClient");
    ...
    userOneHome =
      repositoryHelper.getUserHome(personService.getPerson(USER_ONE));
    // no pun intended
    ...
    assertEquals(
      AccessStatus.DENIED,
      permissionService.hasPermission(testNode, PermissionService.READ)
    );
  ...
  public void testSanityCheckUrls() throws Exception {
    ...
    Response rsp = sendRequest(
      new GetRequest(AUTH_METADATA_URL.replace("{node_ref_3}", testNodeRef_3)),
      expectedStatusOK,
      USER_ONE
    );
    ...
    rsp =
      sendRequest(
        new GetRequest(
          AUTH_CONTENT_THUMBNAIL_URL
            .replace("{node_ref_3}", testNodeRef_3)
            .replace("{thumbnailname}", "doclib")
        ),
        expectedStatusOK,
        USER_ONE
      );
    ...
    rsp =
      sendRequest(
        new GetRequest(
          AUTH_METADATA_URL.replace("{node_ref_3}", testNodeRef_3)
        ),
    ...
    rsp =
      sendRequest(
        new PostRequest(
          SHARE_URL.replace("{node_ref_3}", testNodeRef_3),
          "",
          APPLICATION_JSON
        ),
        expectedStatusOK,
    ...
    rsp =
      sendRequest(
        new GetRequest(SHARE_METADATA_URL.replace("{shared_id}", sharedId)),
        expectedStatusOK,
        USER_TWO
      );
    jsonRsp = new JSONObject(new JSONTokener(rsp.getContentAsString()));
    ...
    rsp =
      sendRequest(
        new GetRequest(SHARE_CONTENT_URL.replace("{shared_id}", sharedId)),
        expectedStatusOK,
        USER_TWO
      );
    byte[] content = rsp.getContentAsByteArray();
    ...
    rsp =
      sendRequest(
        new GetRequest(
          SHARE_CONTENT_THUMBNAIL_URL
            .replace("{shared_id}", sharedId)
            .replace("{thumbnailname}", "doclib")
        ),
        expectedStatusOK,
        USER_TWO
    ...
    rsp =
      sendRequest(
        new DeleteRequest(UNSHARE_URL.replace("{shared_id}", sharedId)),
        expectedStatusOK,
        USER_ONE
      );
  ...
  public void testUnshareContributer()
    ...
    NodeRef siteDocLib = siteService.getContainer(
      siteInfo.getShortName(),
      SiteService.DOCUMENT_LIBRARY
    );
    NodeRef testFile = createTestFile(
      siteDocLib,
      "unshare-test" + RUN_ID,
      quickFile
    );
    ...
    Response rsp = sendRequest(
      new PostRequest(
        SHARE_URL.replace("{node_ref_3}", strTestNodeRef),
        "",
        APPLICATION_JSON
      ),
      expectedStatusOK,
  ...
  public void testCopy()
    throws UnsupportedEncodingException, IOException, JSONException, FileExistsException, FileNotFoundException {
    final int expectedStatusOK = 200;
    ...
    Response rsp = sendRequest(
      new PostRequest(
        SHARE_URL.replace("{node_ref_3}", testNodeRef),
        "",
        APPLICATION_JSON
      ),
      expectedStatusOK,
    ...
    AuthenticationUtil.setFullyAuthenticatedUser(USER_ONE);
    FileInfo copyFileInfo = fileFolderService.copy(
      testNode,
      userOneHome,
      "Copied node"
    );
    ...
    assertFalse(
      nodeService.hasAspect(copyNodeRef, QuickShareModel.ASPECT_QSHARE)
    );
  }

  public void testContentDispositionInResponseHeader()
    ...
    assertNotNull(
      "The response should contain a Content-Disposition entry in the header",
      rsp.getHeader("Content-Disposition")
    );
    ...
    assertNull(
      "The response should not contain a Content-Disposition entry in the header",
      rsp.getHeader("Content-Disposition")
    );
  }
  ...
  private NodeRef createTestFile(
    final NodeRef parent,
    final String name,
    final File quickFile
  ) {
    return transactionHelper.doInTransaction(
      new RetryingTransactionHelper.RetryingTransactionCallback<NodeRef>() {
        public NodeRef execute() throws Throwable {
          ...
          ChildAssociationRef result = nodeService.createNode(
            parent,

...
public class DBOrdering
  ...
  public void prepare(
    ...
    if (getColumn().getFunction().getName().equals(PropertyAccessor.NAME)) {
      PropertyArgument property = (PropertyArgument) getColumn()
      ...
      if (property.getPropertyName().equals(PropertyIds.PARENT_ID)) {
        throw new QueryModelException(
          "Ordering is not supported for " + PropertyIds.PARENT_ID
        );
      } else if (property.getPropertyName().equals(PropertyIds.OBJECT_ID)) {
        throw new QueryModelException(
          "Ordering is not supported for " + PropertyIds.OBJECT_ID
        );
      } else if (
        property.getPropertyName().equals(PropertyIds.OBJECT_TYPE_ID)
      ) {
        throw new QueryModelException(
          "Ordering is not supported for " + PropertyIds.OBJECT_TYPE_ID
        );
      } else if (property.getPropertyName().equals(PropertyIds.BASE_TYPE_ID)) {
        throw new QueryModelException(
          "Ordering is not supported for " + PropertyIds.BASE_TYPE_ID
        );
      } else if (
        property.getPropertyName().equals(PropertyIds.CONTENT_STREAM_MIME_TYPE)
      ) {
        ...
        QName basePropertyQName = QName.createQName(
          DBQuery.expandQName(
            functionContext.getAlfrescoPropertyName(property.getPropertyName()),
            namespaceService
          )
        );
        ...
        propertySupport.setCommandType(
          DBQueryBuilderPredicatePartCommandType.ORDER
        );
        propertySupport.setLeftOuter(true);
        builderSupport = propertySupport;
      } else if (
        property.getPropertyName().equals(PropertyIds.CONTENT_STREAM_LENGTH)
      ) {
        PropertySupport propertySupport = new PropertySupport();
        if (getOrder() == Order.ASCENDING) {
          propertySupport.setValue("ASC");
      ...
      throw new QueryModelException(
        "Ordering not supported " + getColumn().getFunction().getName()
      );
    }
  }

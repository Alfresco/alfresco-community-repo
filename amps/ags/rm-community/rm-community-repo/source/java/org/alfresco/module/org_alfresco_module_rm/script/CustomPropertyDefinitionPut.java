...
public class CustomPropertyDefinitionPut extends BaseCustomPropertyWebScript {
  ...
  protected QName updatePropertyDefinition(Map<String, Serializable> params)
    ...
    if (params.containsKey(PARAM_CONSTRAINT_REF)) {
      ...
      List<ConstraintDefinition> constraints = rmAdminService
      ...
      if (constraintRef == null) {
        result =
          rmAdminService.removeCustomPropertyDefinitionConstraints(propQName);
        ...
        if (!exists) {
          QName constraintRefQName = QName.createQName(
            constraintRef,
            getNamespaceService()
          );
          result =
    ...
    if (params.containsKey(PARAM_LABEL)) {
      ...
      try {
        result =
          rmAdminService.updateCustomPropertyDefinitionName(propQName, label);
      } catch (PropertyAlreadyExistsMetadataException ex) {
        if (!updated) {
          String propIdAsString = rmAdminService

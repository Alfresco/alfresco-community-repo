...
public class CachedContentCleanupJobTest {
    ...
    public void filesNotInCacheAreDeleted() throws InterruptedException {
        ...
        for (int i = 0; i < numFiles; i++) {
            // Testing with a number of files. The cached file cleaner will be able to determine the
            // 'original'
            // content URL for each file by either retrieving from the companion properties file, or
            // performing
            // a 'reverse lookup' in the cache (i.e. cache.contains(Key.forCacheFile(...))), or
            // there will be no
        ...
        assertEquals(
                "Incorrect total size of files deleted", totalSize, cleaner.getSizeFilesDeleted());
    }
    ...
    public void filesNewerThanMinFileAgeMillisAreNotDeleted() throws InterruptedException {
        ...
        // The cleaner must finish before any of the newFiles are older than minFileAge. If the
        // files are too
        ...
        if (cleaner.getDurationMillis() > minFileAge) {
            fail(
                    "Test unable to complete, since cleaner took "
                            + cleaner.getDurationMillis()
                            + "ms"
                            + " which is longer than minFileAge ["
                            + minFileAge
                            + "ms]");
        }
        ...
        assertEquals(
                "Incorrect number of deleted files", newFiles.length, cleaner.getNumFilesDeleted());
        assertEquals(
                "Incorrect total size of files deleted",
                newFilesTotalSize,
    ...
    public void aggressiveCleanReclaimsTargetSpace() throws InterruptedException {
        ...
        // We'll get it to clean seven files worth aggressively and then it will continue
        // non-aggressively.
        // It will delete the older files aggressively (i.e. the ones prior to the two second sleep)
        // and
    ...
    public void standardCleanAfterAggressiveFinished() throws InterruptedException {
        // Don't use numFiles > 59! as we're using this for the minute element in the cache file
        // path.
        ...
        for (int i = 0; i < numFiles; i++) {
            ...
            if (i >= 21 && i <= 24) {
                // 21 to 24 will be deleted after the aggressive deletions (once the cleaner has
                // returned
        ...
        // We'll get it to clean seven files worth aggressively and then it will continue
        // non-aggressively.
        // It will delete the older files aggressively (i.e. even if they are actively in the cache)
        // and
        // then will examine the new files for potential deletion.
        ...
        for (int i = 0; i < numFiles; i++) {
            if (i < 7) {
                assertFalse(
                        "First 7 files should have been aggressively cleaned", files[i].exists());
            }
    ...
    public void emptyParentDirectoriesAreDeleted() throws FileNotFoundException {
        ...
        assertFalse(
                "Directory should have been deleted", new File(cacheRoot, "243235984").exists());
    }
    ...
    public void markedFilesHaveDeletionDeferredUntilCorrectPassOfCleaner() {
        ...
        // which should result in immediate deletion upon discovery of content no longer in the
        // cache.
        cleaner.setMaxDeleteWatchCount(0);
    ...
    private void checkFilesDeleted(File file) {
        ...
        assertFalse(
                "Properties file should have been deleted, cache file: " + file, props.exists());
    }
    ...
    private void checkWatchCountForCacheFile(File file, Integer expectedWatchCount) {
        ...
        props.load();
        assertEquals(
                "File should contain correct deleteWatchCount",
                expectedWatchCount,
                props.getDeleteWatchCount());
    }
    ...
    private File createCacheFile(
            Calendar calendar, /*int year, int month, int day, int hour, int minute,*/
            UrlSource urlSource,
            boolean putInCache) {
        ...
        switch (urlSource) {
            ...
            case REVERSE_CACHE_LOOKUP:
                // file with content URL in reverse lookup cache - but not 'in the cache' (forward
                // lookup).
    ...
    /**
     * Mimick functionality of ContentCacheImpl.createNewCacheFilePath() but allowing a specific
     * date (rather than 'now') to be used.
     *

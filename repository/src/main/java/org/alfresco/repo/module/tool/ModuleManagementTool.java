...
public class ModuleManagementTool implements LogOutput {
  ...
  private static final String FILE_MAPPING_PROPERTIES =
  ...
  public static final String BACKUP_DIR =
    WarHelper.MODULE_NAMESPACE_DIR + "/backup";
  ...
  public ModuleManagementTool() {
    ...
    try {
      ...
      throw new ModuleManagementToolException(
        "Unable to load default extension file mapping properties.",
        exception
      );
    }
  }
  ...
  public void installModule(
    ...
    try {
      ...
      if (preview == false) {
        ...
        TFile moduleDir = new TFile(
          warFileLocation + WarHelper.MODULE_NAMESPACE_DIR
        );
      ...
      ModuleDetails installingModuleDetails = ModuleDetailsHelper.createModuleDetailsFromPropertyLocation(
        propertiesLocation,
        this
      );
      ...
      ModuleDetails installedModuleDetails = warHelper.getModuleDetailsOrAlias(
        warFile,
        installingModuleDetails
      );

      //Check module directory exists
      TFile moduleInstallDirectory = new TFile(
      ...
      Properties directoryChanges = calculateChanges(
        ampFileLocation,
        warFileLocation,
        preview,
        forceInstall,
      ...
      if (preview == false) {
        ...
        ModuleDetailsHelper.saveModuleDetails(
          warFileLocation,
          installingModuleDetails
        );
    ...
    if (installedModuleDetails != null) {
      ...
      if (compareValue > 0) {
        ...
        outputVerboseMessage(
          ...
          "You could force the installation by passing the -force option.",
          false
        );
        return;
      }
  ...
  private Properties calculateChanges(
    ...
    Properties customFileMappingProperties = getCustomFileMappings(
      ampFileLocation
    );
    if (customFileMappingProperties == null) {
      ...
      String inheritDefaultStr = customFileMappingProperties.getProperty(
        PROP_INHERIT_DEFAULT,
        "true"
      );
    ...
    for (Map.Entry<Object, Object> entry : fileMappingProperties.entrySet()) {
      ...
      if (mappingTarget.length() == 0 || !mappingTarget.startsWith("/")) {
        throw new AlfrescoRuntimeException(
          "File mapping targets must start with '/' but was '" +
          mappingTarget +
          "'"
        );
      }
  ...
  public void uninstallModule(
    ...
    for (Map.Entry<String, String> update : installedFiles
      .getUpdates()
      .entrySet()) {
      if (preview == false) {
    ...
    String modulePropertiesFileLocationInWar = ModuleDetailsHelper.getModulePropertiesFilePathInWar(
      moduleId
    );
        ...
        if (sourceChild.isFile() == true) {
          ...
          if (destinationChild.exists() == false) {
            ...
            if (forceInstall) {
              ...
              if (preview == false) {
                ...
                TFile backupLocationDirectory = new TFile(
                  warFileLocation + BACKUP_DIR
                );
              ...
              throw new ModuleManagementToolException(
                ...
                "/" +
                sourceChild.getName() +
                "'. Execution halted.  By specifying -force , you can force installation of AMP regardless of the current war state."
              );
            }
          }
          ...
          if (createFile == true) {
            ...
            this.outputMessage(
                ...
                "/" +
                sourceChild.getName() +
                "' is being overwritten by this module. The original has been backed-up to '" +
                backupLocation +
                "'",
                true
              );
  ...
  public void disableModule(String moduleId, String warLocation) {
    throw new UnsupportedOperationException(
      "Disable module is not currently supported"
    );
  }
  ...
  public void enableModule(String moduleId, String warLocation) {
    throw new UnsupportedOperationException(
      "Enable module is not currently supported"
    );
  }
  ...
  public void listModules(String warLocation) {
    ...
    try {
      List<ModuleDetails> modulesFound = warHelper.listModules(
        new TFile(warLocation)
      );
      ...
      for (
        Iterator<ModuleDetails> iterator = modulesFound.iterator();
        iterator.hasNext();
      ) {
        ...
        outputVerboseMessage(
          "   Version:      " + moduleDetails.getModuleVersionNumber(),
          true
        );
        outputVerboseMessage(
          "   Install Date: " + moduleDetails.getInstallDate(),
          true
        );
        outputVerboseMessage(
      ...
      if (operation.equals(OPTION_HELP) == true) {
        ...
        if (args.length < 3) {
          throw new UsageException(
            OP_INSTALL + " requires at least 3 arguments."
          );
        }
        ...
        if (args.length < 3) {
          throw new UsageException(
            OP_UNINSTALL + " requires at least 3 arguments."
          );
        }

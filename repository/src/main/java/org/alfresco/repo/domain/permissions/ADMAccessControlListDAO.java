...
/**
 * DAO layer for the improved ACL implementation. This layer is responsible for setting ACLs and any
 * cascade behaviour required. It also implements the migration from the old implementation to the
 * new.
 *
 * @author andyh
 */
public class ADMAccessControlListDAO implements AccessControlListDAO {
    ...
    /**
     * maxim transaction time allowed for {@link #setFixedAcls(Long, Long, Long, Long, List,
     * boolean, AsyncCallParameters, boolean)}
     */
    ...
    public Long getInheritedAcl(NodeRef nodeRef) {
        ...
        Pair<Long, ChildAssociationRef> parentAssocRefPair =
                nodeDAO.getPrimaryParentAssoc(nodePair.getFirst());
    ...
    private CounterSet fixOldDmAcls(
            Long nodeId, Long existingNodeAclId, Long inheritedAclId, boolean isRoot) {
        CounterSet result = new CounterSet();

        // If existingNodeAclId is not null and equal to inheritedAclId then we know we have hit a
        // shared ACL we have bulk set
        ...
        if ((existingNodeAclId != null) && (existingNodeAclId.equals(inheritedAclId))) {
            ...
            if (existing != null) {
                if (existing.getProperties().getAclType() == ACLType.OLD) {
                    ...
                    Acl newAcl =
                            aclDaoComponent.createAccessControlList(
                                    properties, existing.getEntries(), actuallyInherited);
                    ...
                    // nothing to do just cascade into the children - we most likely did a bulk set
                    // above.
    ...
    public void setAccessControlList(NodeRef nodeRef, Long aclId) {
        boolean auditableBehaviorWasDisabled =
                preserveAuditableData && behaviourFilter.isEnabled(ContentModel.ASPECT_AUDITABLE);
    ...
    public List<AclChange> setInheritanceForChildren(
            NodeRef parent, Long inheritFrom, Long sharedAclToReplace) {
        // check transaction resource to determine if async call may be required
        boolean asyncCall =
                AlfrescoTransactionSupport.getResource(FixedAclUpdater.FIXED_ACL_ASYNC_CALL_KEY)
                                == null
                        ? false
                        : true;
    ...
    public List<AclChange> setInheritanceForChildren(
            NodeRef parent, Long inheritFrom, Long sharedAclToReplace, boolean asyncCall) {
     ...
     * @param inheritFrom the parent node's ACL
     * @param mergeFrom the shared ACL, if already known. If <code>null</code>, will be retrieved /
     *     created lazily
     * @param changes the list in which to record changes
     * @param set set the shared ACL on the parent ?
     */
     ...
     * @param changes the list in which to record changes
     * @param set set the shared ACL on the parent ?
     * @param asyncCall function may require asynchronous call depending the execution time; if time
     *     exceeds configured <code>fixedAclMaxTransactionTime</code> value, recursion is stopped
     *     using propagateOnChildren parameter(set on false) and those nodes for which the method
     *     execution was not finished in the classical way, will have ASPECT_PENDING_FIX_ACL, which
     *     will be used in {@link FixedAclUpdater} for later processing
     */
    public void setFixedAcls(
            Long nodeId,
            Long inheritFrom,
     ...
     *     execution was not finished in the classical way, will have ASPECT_PENDING_FIX_ACL, which
     *     will be used in {@link FixedAclUpdater} for later processing
     * @param forceSharedACL When a child node has an unexpected ACL, force it to assume the new
     *     shared ACL instead of throwing a concurrency exception.
     */
    public void setFixedAcls(
            Long nodeId,
            Long inheritFrom,
        ...
        if (nodeId == null) {
            ...
            // When node is copied when the aspect is applied, the sharedACLtoReplace will not match
            // the children's ACLS
            ...
            if (nodeDAO.hasNodeAspect(nodeId, ContentModel.ASPECT_PENDING_FIX_ACL)) {
                // If node has a pending acl, retrieve the sharedAclToReplace from node property.
                // When the job calls
                // this, it already does it but on move and copy operations, it uses the new parents
                // old ACL.
                sharedAclToReplace =
                        (Long)
                                nodeDAO.getNodeProperty(
                                        nodeId, ContentModel.PROP_SHARED_ACL_TO_REPLACE);
            }
            ...
            for (NodeIdAndAclId child : children) {
                // Use the current ACL instead of the stored value, it could've been changed
                // meanwhile
                    ...
                    if (acl.equals(sharedAclToReplace)
                        ...
                        if (dbAcl.getAclType() == ACLType.LAYERED) {
                            ...
                            if (dbAcl.getInherits()) {
                                ...
                                List<AclChange> newChanges =
                                        aclDaoComponent.mergeInheritedAccessControlList(
                                                mergeFrom, acl);
                            }
            ...
            // By doing an eager update of the direct children we canot see if another thread has
            // changed the ACL
            // between the time we get the child nodes and we update them. By updating the direct
            // children last it is
            ...
            // When this is not executed triggered by the job, but a move or copy operation occures
            // on a pending
     ...
     * MNT-18308: No longer checks if call is async in order to evaluate time passed to decide if
     * nodes should be processed by job. This is now the default behavior for both sync and async
     * calls: when fixedAclMaxTransactionTime is exceeded, call turns async and all remaining nodes
     * should be processed by job FixedACLUpdater
     *
     * @param nodeId the nodeId of the current node being processed
     * @param inheritFrom the parent node's ACL
     * @param mergeFrom the new shared ACL, if already known.
     * @param sharedAclToReplace the old shared ACL, to be replaced
     * @param changes the list in which to record changes
     * @param set if to set the shared ACL on the parent
     * @param asyncCall if the call was initially set as async
     * @param propagateOnChildren current setting of child propagation of ACL creation
    ...
    private boolean setFixAclPending(
        ...
        // If flag is still unset or false, the call until now has been sync and will turn async, we
        // should throw a
        // warning
    ...
    private void addFixedAclPendingAspect(
            Long nodeId, Long sharedAclToReplace, Long inheritFrom, Long mergeFrom) {
    ...
    public void updateInheritance(Long childNodeId, Long oldParentAclId, Long newParentAclId) {
        ...
        if (childAclId == null) {
            if (newParentAclId != null) {
                Long newParentSharedAclId =
                        aclDaoComponent.getInheritedAccessControlList(newParentAclId);
        ...
        if (acl != null && acl.getInherits()) {
            Long oldParentSharedAclId =
                    aclDaoComponent.getInheritedAccessControlList(oldParentAclId);
            ...
            if (childAclId.equals(oldParentSharedAclId)) {
                ...
                if (newParentAclId != null) {
                    Long newParentSharedAclId =
                            aclDaoComponent.getInheritedAccessControlList(newParentAclId);
                    setFixedAcls(
                            childNodeId, newParentSharedAclId, null, childAclId, changes, true);
                }
                ...
                if (acl.getAclType() == ACLType.LAYERED) {
                    ...
                    List<AclChange> newChanges =
                            aclDaoComponent.mergeInheritedAccessControlList(
                                    newParentSharedAclId, childAclId);

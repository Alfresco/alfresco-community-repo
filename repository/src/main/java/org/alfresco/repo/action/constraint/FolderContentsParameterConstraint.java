...
public class FolderContentsParameterConstraint extends BaseParameterConstraint {
    ...
    /**
     * This optional property defines a list of file extensions which should be included in the
     * result set from this class. By implication, all other file extensions will be excluded. (The
     * dot should not be specified i.e. "txt" not ".txt"). If present, the cm:name of each candidate
     * node will be checked against the values in this list and only those nodes whose cm:name ends
     * with one of these file extensions will be included.
     *
     * <p>If the property is not set then no inclusion filter is specified and all file extensions
     * will be included.
     *
    ...
    protected Map<String, String> getAllowableValuesImpl() {
        ...
        if (nodeRefs.size() == 0) {
            throw new AlfrescoRuntimeException(
                    "The path '" + searchPath + "' did not return any results.");
        } else {
    ...
    private void buildMap(Map<String, String> result, NodeRef folderNodeRef) {
        List<ChildAssociationRef> assocs =
                nodeService.getChildAssocs(
                        folderNodeRef, ContentModel.ASSOC_CONTAINS, RegexQNamePattern.MATCH_ALL);
     ...
     * contained content nodes. If no file extensions are included, then all content NodeRefs will
     * be included in the result set. If however, there are any file extensions specified, then the
     * cm:name must match one of those file extensions to be included in the result set.
     *

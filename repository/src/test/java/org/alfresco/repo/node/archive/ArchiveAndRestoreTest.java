...
public class ArchiveAndRestoreTest extends TestCase {
    ...
    private static final QName ASPECT_ATTACHABLE =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "attachable");
    private static final QName ASSOC_ATTACHMENTS =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "attachments");
    private static final QName QNAME_A =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "a");
    private static final QName QNAME_B =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "b");
    private static final QName QNAME_AA =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "aa");
    private static final QName QNAME_BB =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "bb");
    private static final QName TYPE_QNAME_TEST_CONTENT =
            QName.createQName("http://www.alfresco.org/test/nodearchive", "content");
    ...
    public void setUp() throws Exception {
        ...
        InputStream modelStream =
                cl.getResourceAsStream("org/alfresco/repo/node/archive/archiveTest_model.xml");
        ...
        try {
            ...
            workStoreRef =
                    nodeService.createStore(
                            StoreRef.PROTOCOL_WORKSPACE, getName() + System.currentTimeMillis());
            workStoreRootNodeRef = nodeService.getRootNode(workStoreRef);
            archiveStoreRef =
                    nodeService.createStore(
                            StoreRef.PROTOCOL_WORKSPACE,
                            "archive" + getName() + System.currentTimeMillis());
            archiveStoreRootNodeRef = nodeService.getRootNode(archiveStoreRef);

            // Map the work store to the archive store.  This will already be wired into the
            // NodeService.
            ...
            TestWithUserUtils.createUser(
                    USER_A, USER_A, workStoreRootNodeRef, nodeService, authenticationService);
            TestWithUserUtils.createUser(
                    USER_B, USER_B, workStoreRootNodeRef, nodeService, authenticationService);
            TestWithUserUtils.createUser(
                    USER_C, USER_C, workStoreRootNodeRef, nodeService, authenticationService);
            // grant A and B rights to the work store
            permissionService.setPermission(
                    workStoreRootNodeRef, USER_A, PermissionService.ALL_PERMISSIONS, true);
            permissionService.setPermission(
                    workStoreRootNodeRef, USER_B, PermissionService.ALL_PERMISSIONS, true);
            permissionService.setPermission(
                    workStoreRootNodeRef, USER_C, PermissionService.ALL_PERMISSIONS, false);
        } finally {
     ...
     *
     * <p><b>A</b>, <b>B</b>, <b>AA</b> and <b>BB</b> are set up to archive automatically on
     * deletion.
     */
    private void createNodeStructure() throws Exception {
        ...
        childAssocAtoAA_ =
                new ChildAssociationRef(
                        childAssocAtoAA.getTypeQName(), a_, childAssocAtoAA.getQName(), aa_);
        childAssocBtoBB_ =
                new ChildAssociationRef(
                        childAssocBtoBB.getTypeQName(), b_, childAssocBtoBB.getQName(), bb_);
    }

    private void verifyNodeExistence(NodeRef nodeRef, boolean exists) {
        assertEquals(
                "Node should " + (exists ? "" : "not ") + "exist",
                exists,
    ...
    private void verifyChildAssocExistence(ChildAssociationRef childAssocRef, boolean exists) {
        ...
        if (exists) {
            assertEquals(
                    "Expected exactly one match for child association: " + childAssocRef,
                    1,
                    childAssocs.size());
        } else {
            assertEquals(
                    "Expected zero matches for child association: " + childAssocRef,
                    0,
                    childAssocs.size());
        }
    }
    ...
    private void verifyTargetAssocExistence(AssociationRef assocRef, boolean exists) {
        List<AssociationRef> assocs =
                nodeService.getTargetAssocs(assocRef.getSourceRef(), assocRef.getTypeQName());
        if (exists) {
            assertEquals(
                    "Expected exactly one match for target association: " + assocRef,
                    1,
                    assocs.size());
        } else {
            assertEquals(
                    "Expected zero matches for target association: " + assocRef, 0, assocs.size());
        }
    }
    ...
    private void verifyPropertyExistence(NodeRef nodeRef, QName propertyQName, boolean exists) {
        assertEquals(
                ...
                exists,
                nodeService.getProperty(nodeRef, propertyQName) != null);
    }
    ...
    private void verifyAspectExistence(NodeRef nodeRef, QName aspectQName, boolean exists) {
        assertEquals(
                ...
                exists,
                nodeService.hasAspect(nodeRef, aspectQName));
    }
    ...
    public void testGetStoreArchiveNode() throws Exception {
        ...
        assertEquals(
                "Mapping of archived store is not correct",
    ...
    public void testUserTracking() {
        ...
        RunAsWork<List<ChildAssociationRef>> getAssocsWork =
    ...
    public void testArchivedAspect() throws Exception {
        ...
        assertTrue(
                "Archived aspect not present",
                nodeService.hasAspect(a_, ContentModel.ASPECT_ARCHIVED));
        Map<QName, Serializable> properties_a = nodeService.getProperties(a_);
        assertNotNull(
                "Original owner property should not be set",
                properties_a.get(ContentModel.PROP_ARCHIVED_ORIGINAL_OWNER));
        ...
        assertTrue(
                "Archived aspect not present",
                nodeService.hasAspect(b_, ContentModel.ASPECT_ARCHIVED));
        Map<QName, Serializable> properties_b = nodeService.getProperties(b_);
        assertNotNull(
                "Original owner property not present",
                properties_b.get(ContentModel.PROP_ARCHIVED_ORIGINAL_OWNER));
        assertEquals(
                "Original owner property is incorrect",
                USER_B,
                properties_b.get(ContentModel.PROP_ARCHIVED_ORIGINAL_OWNER));
    }
    ...
    public void testArchiveAndRestoreNodeBB() throws Exception {
        ...
        ChildAssociationRef bb_originalParent =
                (ChildAssociationRef)
                        bb_Properties.get(ContentModel.PROP_ARCHIVED_ORIGINAL_PARENT_ASSOC);
    ...
    public void testInTransactionRestore() throws Exception {
        ...
        assertEquals(
                "Expected failure", RestoreStatus.FAILURE_INVALID_ARCHIVE_NODE, report.getStatus());
    ...
    public void testRestoreToMissingParent() throws Exception {
        ...
        assertEquals(
                "Incorrect report status",
                RestoreStatus.FAILURE_INVALID_PARENT,
                report.getStatus());
    }
    ...
    public void testMNT8916RestoreWithoutPermissionsSet() throws Exception {
        ...
        AuthenticationUtil.setAdminUserAsFullyAuthenticatedUser();
        assertTrue(
                "Restored node shouldn't have any permissions set",
                permissionService.getAllSetPermissions(a).isEmpty());
        assertFalse(
                "Restored node shouldn't inherit parent permissions",
    ...
    public void testPermissionsForRestore() throws Exception {
        ...
        List<RestoreNodeReport> restoredByB =
                nodeArchiveService.restoreArchivedNodes(nodesToRestore);
        for (RestoreNodeReport restoreNodeReport : restoredByB) {
            if (restoreNodeReport.getArchivedNodeRef().equals(a_)) {
                assertEquals(
                        ...
                        RestoreStatus.FAILURE_PERMISSION,
                        restoreNodeReport.getStatus());
    ...
    /**
     * Deny the current user the rights to write to the destination location and ensure that the
     * use-case is handled properly.
     */
    public void testPermissionsLackingOnDestination() throws Exception {
        ...
        nodeService.deleteNode(b);
        permissionService.setPermission(
                workStoreRootNodeRef, USER_B, PermissionService.ADD_CHILDREN, false);
        commitAndBeginNewTransaction();
        ...
        assertEquals(
                "Expected permission denied status",
                RestoreStatus.FAILURE_PERMISSION,
                report.getStatus());
    }

    /**
     * Check that the existence of the node in the archive store doesn't prevent archival. It is
     * possible to restore a node to the SpacesStore from some other source. When that node is
     * archived, the currently archived node must be overwritten.
     */
    public void testAR1519ArchiveCleansDuplicateUuid() throws Exception {
        ...
        NodeRef aRecreated =
                nodeService
                        ...
                        .getChildRef();
        assertEquals(
                "NodeRef for recreated node should be the same as the original", a, aRecreated);
        ...
        NodeRef bRecreated =
                nodeService
                        ...
                        .getChildRef();
        assertEquals(
                "NodeRef for recreated node should be the same as the original", b, bRecreated);
    ...
    public void testALF17554ArchiveAndRestoreCheckPermission() throws Exception {
        ...
        assertTrue(
                ...
                permissionService.hasPermission(a, PermissionService.COORDINATOR)
                        == AccessStatus.DENIED);
        AuthenticationUtil.setAdminUserAsFullyAuthenticatedUser();
    ...
    public void testMNT2715ArchiveAndRestoreWithOwnableAndWithout() throws Exception {
        ...
        assertNull(
                "The node should'n have the ownable aspect if it didn't has it before"
                        + " deleting/restoring",
        ...
        assertTrue(
                "The node should have the same owner after restoring as before deleting",
    ...
    public void testListArchivedNodesPermissions() {
        ...
        // Create canned query
        ArchivedNodesCannedQueryBuilder queryBuilder =
                new ArchivedNodesCannedQueryBuilder.Builder(this.archiveStoreRootNodeRef, paging)
                        .build();
    ...
    private PagingResults<NodeRef> runListArchivedNodesAsAdmin(
            final ArchivedNodesCannedQueryBuilder queryBuilder) {
        return AuthenticationUtil.runAs(
                new RunAsWork<PagingResults<NodeRef>>() {
                    @Override
    ...
    public void testListArchivedNodesFilter() {
        ...
        PagingResults<NodeRef> result = nodeArchiveService.listArchivedNodes(queryBuilder);
        assertEquals(
                "There are 3 nodes that match 'picture*' pattern.", 3, result.getPage().size());
        // Node "pictureThreeTest.png" deleted last, so it must be first in the sorted list
        assertEquals(
                "pictureThreeTest.png",
                nodeService.getProperty(result.getPage().get(0), ContentModel.PROP_NAME));
        assertEquals(
                "pictureTwoTest.jpg",
                nodeService.getProperty(result.getPage().get(1), ContentModel.PROP_NAME));
        ...
        result = nodeArchiveService.listArchivedNodes(queryBuilder);
        assertEquals(
                "There is only 1 node that matches 'pictureT*.jpg' pattern.",
                1,
                result.getPage().size());
        assertEquals(
                "pictureTwoTest.jpg",
        ...
        result = nodeArchiveService.listArchivedNodes(queryBuilder);
        assertEquals(
                "There are 2 nodes that match '*Test.jpg' pattern.", 2, result.getPage().size());
        assertEquals(
                "pictureTwoTest.jpg",
                nodeService.getProperty(result.getPage().get(0), ContentModel.PROP_NAME));
        assertEquals(
        ...
        result = nodeArchiveService.listArchivedNodes(queryBuilder);
        assertEquals(
                "There are 2 nodes that matches '*test.jpg' pattern.", 2, result.getPage().size());
    }
}

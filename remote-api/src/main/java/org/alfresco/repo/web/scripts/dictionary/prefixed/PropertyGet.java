...
public class PropertyGet extends AbstractPropertyGet {
  ...
  protected QName getPropertyQname(WebScriptRequest req) {
    String propertyName = req
      .getServiceMatch()
      .getTemplateVars()
      .get(DICTIONARY_SHORTPROPERTY_NAME);
    String propertyPrefix = req
      .getServiceMatch()
      .getTemplateVars()
      .get(DICTIONARY_PROPERTY_FREFIX);

    //validate the presence of property name
    if (propertyName == null) {
      throw new WebScriptException(
        Status.STATUS_NOT_FOUND,
        "Missing parameter short propertyname in the URL"
      );
    }
    ...
    if (propertyPrefix == null) {
      throw new WebScriptException(
        Status.STATUS_NOT_FOUND,
        "Missing parameter propertyprefix in the URL"
      );
    }

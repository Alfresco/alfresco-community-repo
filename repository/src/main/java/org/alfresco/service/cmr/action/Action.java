...
public interface Action extends ParameterizedItem {
    ...
    /**
     * @return <tt>true</tt> if the action must be tracked by the {@link ActionTrackingService},
     *     <tt>false</tt> if it must NOT be tracked or <tt>null</tt> to use the action definition's
     *     default.
     */
     ...
     *
     * <p>The option of tracking can be <tt>null</tt> i.e. unset, indicating that the defaults of
     * the action definition should be used. This is to allow manual overriding of the property when
     * it becomes supported by the UI.
     *
     * @param trackStatus <tt>true</tt> if the action must be tracked by the {@link
     *     ActionTrackingService}, <tt>false</tt> if it must NOT be tracked or <tt>null</tt> to use
     *     the action definition's default.
     */
    void setTrackStatus(Boolean trackStatus);

    /**
     * A guide for the ActionService; Should the action be executed asychronously or not? It is not
     * a general purpose flag to indicate the action is running asychronously.
     *
     * <p>The default is to execute the action synchronously.
     *
     ...
     *
     * @param executeAsynchronously true if the action is to be executed asychronously, false
     *     otherwise.
     */
     ...
     *
     * <p>This action is executed if the failure behaviour is to compensate and the action being
     * executed fails.
     *
    ...
    /**
     * Gets the date that the action (last) began executing at. Null if the action has not yet been
     * run. For a saved action, this will be the last time at ran.
     *
    ...
    /**
     * Gets the date that the action (last) finished execution at. Null if the action has not yet
     * been run, or is currently running. For a saved action, this will normally be the last time it
     * finished running.
     *
    ...
    /**
     * Gets the message of the exception which caused the Action execution failure, or null if the
     * Action hasn't failed / has been retried.
     *

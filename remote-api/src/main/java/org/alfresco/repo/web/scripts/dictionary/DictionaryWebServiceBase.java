...
public abstract class DictionaryWebServiceBase extends DeclarativeWebScript {
  ...
  public boolean isValidAssociationFilter(String af) {
    return (
      af.equalsIgnoreCase(ASSOCIATION_FILTER_OPTION_TYPE1) ||
  ...
  public boolean isValidTypeorAspect(String prefix, String shorname) {
    try {
      QName qname = QName.createQName(
        this.getFullNamespaceURI(prefix, shorname)
      );
      return (
        (this.dictionaryservice.getClass(qname) != null) &&
        (this.dictionaryservice.getClass(qname).isAspect())
  ...
  public boolean isValidClassFilter(String classfilter) {
    return (
      classfilter.equals(CLASS_FILTER_OPTION_TYPE1) ||
  ...
  protected <T> Collection<T> reorderedValues(
    List<ClassDefinition> sortedClassDefs,
    Map<QName, T> dependent
  ) {

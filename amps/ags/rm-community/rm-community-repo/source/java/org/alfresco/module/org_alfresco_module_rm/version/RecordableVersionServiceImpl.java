...
public class RecordableVersionServiceImpl
   ...
   */
  public void setEnableAutoVersionOnRecordCreation(
    boolean isEnableAutoVersionOnRecordCreation
  ) {
    this.isEnableAutoVersionOnRecordCreation =
  ...
  protected Version createVersion(
    ...
    if (!origVersionProperties.containsKey(KEY_RECORDABLE_VERSION)) {
      ...
      if (origVersionProperties != null) {
        versionType =
          (VersionType) origVersionProperties.get(
  ...
  private NodeRef getFilePlan(NodeRef nodeRef) {
    NodeRef filePlan = (NodeRef) nodeService.getProperty(
      nodeRef,
      PROP_FILE_PLAN
    );
    if (filePlan == null) {
        ...
        public NodeRef doWork() throws Exception {
          ...
          if (filePlan == null) {
            throw new AlfrescoRuntimeException(
              "Can't create a recorded version, because there is no file plan."
            );
          }
          return filePlan;
        }
   ...
   * @return
   */
  private boolean isCreateRecordedVersion(
    NodeRef nodeRef,
    VersionType versionType
  ) {
    boolean result = false;
    if (
      nodeService.hasAspect(nodeRef, RecordableVersionModel.ASPECT_VERSIONABLE)
    ) {
      String policyString = (String) nodeService.getProperty(
        nodeRef,
        PROP_RECORDABLE_VERSION_POLICY
      );
      if (policyString != null) {
        RecordableVersionPolicy policy = RecordableVersionPolicy.valueOf(
          policyString.toUpperCase()
        );
        if (
          RecordableVersionPolicy.ALL.equals(policy) ||
          (
  ...
  protected NodeRef createNewVersion(
    ...
    if (
      versionProperties.containsKey(KEY_RECORDABLE_VERSION) &&
  ...
  protected NodeRef createNewRecordedVersion(
    ...
    try {
      ...
      if (filePlan == null) {
        throw new AlfrescoRuntimeException(
          "Can't create a new recorded version, because no file plan has been specified in the version properties."
        );
      }

      // create a copy of the source node and place in the file plan
      final NodeRef nodeRef = (NodeRef) standardVersionProperties.get(
      ...
      try {
        // create record
        final NodeRef record = recordService.createRecordFromCopy(
          filePlan,
          nodeRef
        );
    ...
    if (logger.isTraceEnabled()) {
      logger.trace(
        "createNewRecordedVersion created (" +
        versionNumber +
        ") " +
        versionNodeRef
      );
   ...
   */
  private void linkToPreviousVersionRecord(
  ...
  private NodeRef getLatestVersionRecord(NodeRef nodeRef) {
    ...
    if (versionHistory != null) {
      ...
      for (Version previousVersion : previousVersions) {
        ...
        final NodeRef previousRecord = (NodeRef) previousVersion
          .getVersionProperties()
          .get(PROP_VERSION_RECORD);
  ...
  public Version getRecordedVersion(NodeRef versionRecord) {
    ...
    NodeRef versionedNodeRef = (NodeRef) nodeService.getProperty(
      versionRecord,
      RecordableVersionModel.PROP_VERSIONED_NODEREF
    );
  ...
  private void freezeAuditableAspect(NodeRef nodeRef, NodeRef versionNodeRef) {
    if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_AUDITABLE)) {
      Map<QName, Serializable> properties = dbNodeService.getProperties(
        nodeRef
      );
      ...
      if (properties.get(ContentModel.PROP_OWNER) != null) {
        dbNodeService.setProperty(
          versionNodeRef,
          PROP_FROZEN_OWNER,
          properties.get(ContentModel.PROP_OWNER)
        );
      }
    }
  }
  ...
  protected Version getVersion(NodeRef versionRef) {
    ...
    NodeRef record = (NodeRef) dbNodeService.getProperty(
      versionRef,
      PROP_RECORD_NODE_REF
    );
    if (record != null) {
    ...
    Boolean destroyed = (Boolean) dbNodeService.getProperty(
      versionRef,
      PROP_DESTROYED
    );
    if (destroyed == null) {
    ...
    version
      .getVersionProperties()
      .put(PROP_RECORDED_VERSION_DESTROYED, destroyed);
  ...
  public void revert(NodeRef nodeRef, Version version, boolean deep) {
    String versionPolicy = (String) dbNodeService.getProperty(
      nodeRef,
      PROP_RECORDABLE_VERSION_POLICY
    );
  ...
  public NodeRef getVersionRecord(Version version) {
    ...
    if (
      ...
      result =
        (NodeRef) dbNodeService.getProperty(
          versionNodeRef,
  ...
  public NodeRef createRecordFromLatestVersion(
    ...
    if (nodeService.hasAspect(nodeRef, ContentModel.ASPECT_VERSIONABLE)) {
      ...
      if (currentVersion != null && !isRecordedVersion(currentVersion)) {
        ...
        Map<QName, Serializable> versionProperties = getVersionAspectProperties(
          currentVersion
        );
        ...
        String owner = (String) nodeService.getProperty(
  ...
  public boolean isRecordedVersionDestroyed(Version version) {
    ...
    Boolean isDestroyed = (Boolean) dbNodeService.getProperty(
      versionNodeRef,
      PROP_DESTROYED
    );
    if (isDestroyed != null) {
  ...
  private int getVersionNumber(Version version) {
    ...
    String versionNumber = fullVersionNumber.substring(
      fullVersionNumber.indexOf("-") + 1
    );
  ...
  private Map<QName, Serializable> getVersionAspectProperties(Version version) {
    NodeRef versionNodeRef = getVersionNodeRef(version);
    Map<QName, Serializable> versionProps = dbNodeService.getProperties(
      versionNodeRef
    );
  ...
  public boolean isCurrentVersionDirty(NodeRef nodeRef) {
    ...
    Date modificationDate = (Date) nodeService.getProperty(
      nodeRef,
    ...
    // grab the frozen state
    NodeRef currentFrozenState = currentVersion.getFrozenStateNodeRef();
    Date frozenModificationDate = (Date) nodeService.getProperty(
      currentFrozenState,
      ContentModel.PROP_MODIFIED
    );

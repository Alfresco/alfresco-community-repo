...
public abstract class AbstractRenditionIntegrationTest extends BaseSpringTest {
    @Autowired protected RenditionService2Impl renditionService2;

    @Autowired protected RenditionDefinitionRegistry2Impl renditionDefinitionRegistry2;

    @Autowired protected RenditionService renditionService;

    @Autowired protected ThumbnailRegistry thumbnailRegistry;

    @Autowired protected MimetypeMap mimetypeMap;

    @Autowired protected MimetypeService mimetypeService;
    ...
    @Autowired protected MutableAuthenticationService authenticationService;

    @Autowired protected PersonService personService;

    @Autowired protected PermissionService permissionService;

    @Autowired protected TransformServiceRegistry transformServiceRegistry;

    @Autowired protected LocalTransformServiceRegistry localTransformServiceRegistry;

    @Autowired protected TransformationOptionsConverter converter;

    @Autowired protected AsynchronousExtractor asynchronousExtractor;
    ...
    public void setUp() throws Exception {
        ...
        boolean localTransformServiceEnabled =
                Boolean.parseBoolean(System.getProperty("local.transform.service.enabled"));
        ...
        if (transformServiceRegistry instanceof LocalTransformServiceRegistry) {
            ((LocalTransformServiceRegistry) transformServiceRegistry)
                    .setEnabled(localTransformServiceEnabled);
        }
    ...
    protected void checkRendition(
            String testFileName, String renditionName, boolean expectedToPass) {
        try {
    ...
    private NodeRef updateContent(NodeRef sourceNodeRef, String testFileName)
        ...
        ContentWriter contentWriter =
                contentService.getWriter(sourceNodeRef, ContentModel.PROP_CONTENT, true);
    ...
    // As a given user waitForRendition for a rendition to appear. Creates new transactions to do
    // this.
    protected NodeRef waitForRendition(
            String user, NodeRef sourceNodeRef, String renditionName, boolean shouldExist)
            throws AssertionFailedError {
        try {
            return AuthenticationUtil.runAs(
                    () -> waitForRendition(sourceNodeRef, renditionName, shouldExist), user);
    ...
    protected void waitForExtract(String user, NodeRef sourceNodeRef, boolean nodePropsShouldChange)
            throws AssertionFailedError {
        try {
            AuthenticationUtil.runAs(
                    () -> waitForExtract(sourceNodeRef, nodePropsShouldChange), user);
        } catch (RuntimeException e) {
            Throwable cause = e.getCause();
            if (cause instanceof AssertionFailedError) {
                throw (AssertionFailedError) cause;
    ...
    // As the current user waitForRendition for a rendition to appear. Creates new transactions to
    // do this.
    private NodeRef waitForRendition(
            NodeRef sourceNodeRef, String renditionName, boolean shouldExist)
            throws InterruptedException {
        long maxMillis = 10000;
        ChildAssociationRef assoc = null;
        for (int i = (int) (maxMillis / 1000); i >= 0; i--) {
            // Must create a new transaction in order to see changes that take place after this
            // method started.
            assoc =
                    transactionService
                            .getRetryingTransactionHelper()
    ...
    // As the current user waitForRendition for a rendition to appear. Creates new transactions to
    // do this.
    private Object waitForExtract(NodeRef sourceNodeRef, boolean nodePropsShouldChange)
            throws InterruptedException {
        long maxMillis = 5000;
                                    ...
                                    () -> {
                                        Serializable created =
                                                nodeService.getProperty(
                                                        sourceNodeRef, ContentModel.PROP_CREATED);
                                        Serializable modified =
                                                nodeService.getProperty(
                                                        sourceNodeRef, ContentModel.PROP_MODIFIED);
        ...
        RetryingTransactionHelper.RetryingTransactionCallback<Void> createUserCallback =
                () -> {
                    ...
                    personProperties.put(
                            ContentModel.PROP_AUTHORITY_DISPLAY_NAME, "title" + username);

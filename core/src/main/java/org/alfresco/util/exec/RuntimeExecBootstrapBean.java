...
/**
 * Application bootstrap bean that is able to execute one or more native executable statements upon
...
public class RuntimeExecBootstrapBean extends AbstractLifecycleBean {
     ...
     * <ul>
     *   <li>failOnError = true
     *   <li>killProcessesOnShutdown = true
     *   <li>enabled = true
     * </ul>
     */
    ...
    /**
     * Set whether a process failure generates an error or not. Deviation from the default is useful
     * if use as part of a process where the command or the codes generated by the execution may be
     * ignored or avoided by the system.
     *
     * @param failOnError <tt>true<tt> (default) to issue an error message and throw an exception if
     *     the process fails to execute or generates an error return value.
     * @since 2.1
     */
    ...
    /**
     * Set whether or not to force a shutdown of successfully started processes. As most bootstrap
     * processes are kicked off in order to provide the server with some or other service, this is
     * <tt>true</tt> by default.
     *
     * @param killProcessesOnShutdown <tt>true</tt> to force any successfully executed commands'
     *     processes to be forcibly killed when the server shuts down.
     * @since 2.1.0
     */
    ...
    /**
     * Set whether or not the process should be disabled at ApplicationContext bootstrap. If a
     * RuntimeExecBootstrapBean is disabled, then the command will not be executed. This property is
     * not required and is <code>false</code> by default.
     *
     * <p>This method has been deprecated in favour of a clearer name introduced in 3.3. See {@link
     * #setEnabled}.
     *
     * @param disabledAtStartUp any String which <code>equalsIgnoreCase("true")</code> to prevent
     *     the command from being executed.
     * @since 3.2.1
     * @deprecated Use {@link #setEnabled} instead, remembering that the boolean property should be
     *     inverted.
     */
    ...
    /**
     * Set whether or not the process should be enabled at ApplicationContext bootstrap. If a
     * RuntimeExecBootstrapBean is not enabled, then the command will not be executed. This property
     * is not required and is <code>true</code> by default.
     *
     * @param enabled any String which does not <code>equalsIgnoreCase("true")</code> will prevent
     *     the command from being executed.
     * @since 3.3
     */
    ...
    protected synchronized void onBootstrap(ApplicationEvent event) {
        ...
        if (this.enabled == false) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Bootstrap execution of " + startupCommands.size() + " was not enabled");
            }
            return;
        }
        // execute
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Bootstrap execution of "
                            + startupCommands.size()
                            + " commands was successful");
        }
    }
    ...
    protected synchronized void onShutdown(ApplicationEvent event) {
        ...
        try {
            // We managed to stop the process ourselves (e.g. on subsystem shutdown). Remove the
            // shutdown hook

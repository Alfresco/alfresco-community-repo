...
public class FavouritesImpl implements Favourites {
  ...
  private Target getTarget(
    ...
    if (type.equals(Type.FILE)) {
      ...
      throw new AlfrescoRuntimeException(
        "Unexpected favourite target type: " + type
      );
    }
  ...
  private CollectionWithPagingInfo<Favourite> wrap(
    Paging paging,
  ...
  public Favourite addFavourite(
    ...
    if (target == null) {
      ...
      SiteInfo siteInfo = sites.validateSite(
        new NodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, guid)
      );
      NodeRef siteNodeRef = siteInfo.getNodeRef();
      String siteId = siteInfo.getShortName();

      try {
        PersonFavourite personFavourite = favouritesService.addFavourite(
          personId,
          siteNodeRef
        );
  ...
  public void removeFavourite(String personId, String id) {
    ...
    if (type.equals(Type.SITE)) {
      ...
      if (siteInfo == null) {
        ...
        throw new AlfrescoRuntimeException(
          "Unable to find site with nodeRef " + nodeRef
        );
      }
      exists =
        favouritesService.removeFavourite(personId, siteInfo.getNodeRef());
  ...
  public Favourite getFavourite(
    ...
    PersonFavourite personFavourite = favouritesService.getFavourite(
      personId,
      nodeRef
    );
  ...
  @Override
  public CollectionWithPagingInfo<Favourite> getFavourites(
    String personId,
    ...
    List<Pair<FavouritesService.SortFields, Boolean>> sortProps = getSortProps(
      parameters
    );
        ...
        public void exists(String filteredByClient, boolean negated) {
          if (filteredByClient != null) {
            ...
            if (idx == -1 || idx == filteredByClient.length()) {
              ...
              String filtertype = filteredByClient
                .substring(idx + 1)
                .toUpperCase();
    ...
    final PagingResults<PersonFavourite> favourites = favouritesService.getPagedFavourites(
      personId,
      filterTypes,
      sortProps,
  ...
  private Parameters getDefaultParameters(String personId, String favouriteId) {
    Params.RecognizedParams recognizedParams = new Params.RecognizedParams(
      null,
      null,
      null,
      null,
  ...
  private List<Pair<FavouritesService.SortFields, Boolean>> getSortProps(
    Parameters parameters
  ) {
    ...
    if ((sortCols != null) && (sortCols.size() > 0)) {
      for (SortColumn sortCol : sortCols) {
        ...
        sortProps.add(
          new Pair<>(sortField, (sortCol.asc ? Boolean.TRUE : Boolean.FALSE))
        );
      }
    } else {
      // default sort order

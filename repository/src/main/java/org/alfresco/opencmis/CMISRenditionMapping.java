...
public class CMISRenditionMapping {
              ...
              .doInTransaction(
                new RetryingTransactionCallback<Map<String, BigInteger[]>>() {
                  public Map<String, BigInteger[]> execute() throws Exception {
                    ...
                    List<RenditionDefinition> allRenditionDefs = renditionService.loadRenditionDefinitions();
                    for (RenditionDefinition rd : allRenditionDefs) {
                      QName renditionDefinitionName = rd.getRenditionName();

                      Number width = (Number) rd.getParameterValue(
                        ImageRenderingEngine.PARAM_RESIZE_WIDTH
                      );
                      Number height = (Number) rd.getParameterValue(
                      ...
                      if ((width != null) || (height != null)) {
                        ...
                        size[0] =
                          (
                            width == null
                              ? null
                              : BigInteger.valueOf(width.longValue())
                          );
                        size[1] =
                          (
                            height == null
                              ? null
                              : BigInteger.valueOf(height.longValue())
                          );
  ...
  public List<RenditionData> getRenditions(
    ...
    if (
      (filterSet != null) && (filterSet.contains(CMISConnector.RENDITION_NONE))
    ) {
    ...
    int skip =
      (
        skipCount == null || skipCount.intValue() < 0 ? 0 : skipCount.intValue()
      );

    if (max > 0) {
      // find all renditions and filter them
      List<ChildAssociationRef> renditionList = renditionService.getRenditions(
        nodeRef
      );

      int lastIndex =
        (
          max + skip > renditionList.size() ? renditionList.size() : max + skip
        ) -
        1;
  ...
  private Set<String> splitRenditionFilter(String filter) {
    ...
    for (String s : filter.split(",")) {
      ...
      if (s.equals("*")) {
        ...
        if (!s.endsWith("*")) {
          throw new CmisFilterNotValidException(
            "Rendition filter is invalid: " + s
          );
        }
        result.add(s);

...
public class Person
  ...
  public static Person parsePerson(JSONObject jsonObject) {
    ...
    Boolean emailNotificationsEnabled = (Boolean) jsonObject.get(
      "emailNotificationsEnabled"
    );
    List<String> aspectNames = (List<String>) jsonObject.get("aspectNames");
    Map<String, Object> properties = (Map<String, Object>) jsonObject.get(
      "properties"
    );
    Map<String, Boolean> capabilities = (Map<String, Boolean>) jsonObject.get(
      "capabilities"
    );
  ...
  public static String getNetworkId(String personId) {
    ...
    return (
      idx == -1 ? TenantService.DEFAULT_DOMAIN : personId.substring(idx + 1)
    );
  }

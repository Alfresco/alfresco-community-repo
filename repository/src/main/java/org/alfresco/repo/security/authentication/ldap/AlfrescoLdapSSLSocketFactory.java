...
public class AlfrescoLdapSSLSocketFactory extends SocketFactory {
    ...
    private void addHostNameVerification(SSLSocket sslSocket) {
        if (useJava6CodeBase == null || useJava6CodeBase) {
            // Try to use SSLSocketImpl.trySetHostnameVerification method that is supported by java6
            // and lower
            try {
                Method m =
                        sslSocket.getClass().getMethod("trySetHostnameVerification", String.class);
        ...
        if (useJava7CodeBase == null || useJava7CodeBase) {
            // Try to use sslParams.setEndpointIdentificationAlgorithm method that is supported by
            // java 7 and higher
            try {
                ...
                Method m =
                        sslParams
                                .getClass()
                                .getMethod("setEndpointIdentificationAlgorithm", String.class);
                ...
                if (useJava6CodeBase == false && logger.isWarnEnabled()) {
                    logger.warn(
                            "AlfrescoLdapSSLSocketFactory: Unable to turn on Hostname"
                                    + " Verification");
                }
            }
        }

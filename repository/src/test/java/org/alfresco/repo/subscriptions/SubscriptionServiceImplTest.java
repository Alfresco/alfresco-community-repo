...
public class SubscriptionServiceImplTest extends TestCase {
  ...
  public static final String[] CONTEXTS = new String[] {
    "classpath:alfresco/application-context.xml",
    "classpath:test/alfresco/test-subscriptions-context.xml",
  };
  ...
  public static final QName ASPECT_ARCHIVE = QName.createQName(
    "http://www.alfresco.org/model/testsubscriptionsmodel/1.0",
    "archive"
  );
  ...
  public void testFollow() throws Exception {
    ...
    PagingFollowingResults followers = subscriptionService.getFollowers(
      userId2,
      new PagingRequest(100000, null)
    );
  ...
  public void testSubscriptionsRemovalOnArchivingNodesForAlf12358()
    throws Exception {
    NodeRef companyHome = repositoryHelper.getCompanyHome();

    ChildAssociationRef followed = null;

    int initialCount = subscriptionService.getSubscriptionCount(
      USER_TOM,
    ...
    try {
      followed =
        nodeService.createNode(
          ...
          QName.createQName(
            ...
            QName.createValidLocalName(FOLLOWED_NODE_NAME)
          ),
          ContentModel.TYPE_USER
        );
      nodeService.addAspect(followed.getChildRef(), ASPECT_ARCHIVE, null);
      assertTrue(
        "Fake User node MUST BE archival!",
        nodeService.hasAspect(followed.getChildRef(), ASPECT_ARCHIVE)
      );

      subscriptionService.subscribe(USER_TOM, followed.getChildRef());
      assertEquals(
        "Initial subscriptions count MUST BE lesser by 1 after adding new subscription!",
        1,
        (
          subscriptionService.getSubscriptionCount(
            USER_TOM,
            SubscriptionItemTypeEnum.USER
          ) -
          initialCount
      ...
      assertEquals(
        "Archiving of node MUST cause removal all the subscriptions created against it!",
        initialCount,
        subscriptionService.getSubscriptionCount(
          USER_TOM,
          SubscriptionItemTypeEnum.USER

...
public class BasicHttpAuthenticatorFactory implements ServletAuthenticatorFactory {
    ...
    /**
     * Set the listener for authentication events, generated by {@link BasicHttpAuthenticator}
     * instances.
     */
    ...
    public class BasicHttpAuthenticator implements Authenticator {
        ...
        public boolean authenticate(RequiredAuthentication required, boolean isGuest) {
            ...
            if (isGuest && RequiredAuthentication.guest == required) {
                if (logger.isDebugEnabled()) logger.debug("Authenticating as Guest");
            ...
            // authenticate as specified by explicit ticket on url
                    ...
                    && (ticket.length() > 0)
                    && (doNotReportUrlTicketAuthenticationFailed = isTicketValid())) {
                if (logger.isDebugEnabled()) {
            ...
            // authenticate as specified by HTTP Basic Authentication
                ...
                if (!authorizationParts[0].equalsIgnoreCase("basic")) {
                    throw new WebScriptException(
                            401, "Authorization '" + authorizationParts[0] + "' not supported.");
                }
            ...
            if (!authorized) {
                ...
                if (logger.isDebugEnabled()) logger.debug("Requesting authorization credentials");
         ...
         *
         * @return {@link Boolean} value: <code>true</code> if the ticket is still valid, <code>
         *     false</code> if the ticket is not valid any more
         */
        private boolean isTicketValid() {
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "User ticket is not valid. Passing to the Basic authentication"
                                    + " handling. Reqeust information:\n"
                                    + "    ticket: "
                                    + ticket
                                    + "\n"
                                    + "    request: "
        ...
        protected boolean isBasicAuthHeaderPresentForAdmin() {
            ...
            return AuthenticationUtil.runAsSystem(
                    () -> authorityService.isAdminAuthority(auth.getUserName()));
        }
    }
}

...
/**
 * @author brian
 *     <p>The primary manifest processor is responsible for the first parsing the snapshot file and
 *     writing nodes into the receiving repository.
 *     <p>New nodes may be written into a "temporary" space if their primary parent node has not yet
 *     been transferred.
 */
public class RepoPrimaryManifestProcessorImpl extends AbstractManifestProcessorBase {
    ...
    private static final String MSG_NO_PRIMARY_PARENT_SUPPLIED =
            "transfer_service.receiver.no_primary_parent_supplied";
    private static final String MSG_ORPHANS_EXIST = "transfer_service.receiver.orphans_exist";
    private static final String MSG_REFERENCED_CONTENT_FILE_MISSING =
            "transfer_service.receiver.content_file_missing";
    ...
    /**
     * The list of orphans, during processing orphans are added and removed from this list. If at
     * the end of processing there are still orphans then an exception will be thrown.
     */
    private Map<NodeRef, List<ChildAssociationRef>> orphans =
            new HashMap<NodeRef, List<ChildAssociationRef>>(89);
    ...
    protected void processNode(TransferManifestDeletedNode node) {
        // This is a deleted node. First we need to check whether it has already been deleted in
        // this repo
        // too by looking in the local archive store. If we find it then we need not do anything.
        // If we can't find it in our archive store then we'll see if we can find a corresponding
        // node in the
        ...
        NodeRef origNodeRef =
                new NodeRef(
                        origPrimaryParent.getParentRef().getStoreRef(), node.getNodeRef().getId());

        CorrespondingNodeResolver.ResolvedParentChildPair resolvedNodes =
                nodeResolver.resolveCorrespondingNode(
        ...
        if (resolvedNodes.resolvedChild != null) {
            ...
            if (log.isDebugEnabled()) {
                log.debug(
                        "Incoming deleted noderef "
                                + node.getNodeRef()
                                + " has been resolved to existing local noderef "
                                + exNode
                                + "  - deleting");
            }
            ...
            logComment(
                    "Unable to find corresponding node for incoming deleted node: "
                            + node.getNodeRef());
            if (log.isDebugEnabled()) {
                log.debug(
                        "Incoming deleted noderef has no corresponding local noderef: "
                                + node.getNodeRef()
                                + "  - ignoring");
            }
        }
    ...
    protected void processNode(TransferManifestNormalNode node) {
        ...
        logComment(
                "Primary Processing incoming node: "
                        + node.getNodeRef()
                        + " --  Source path = "
                        + node.getParentPath()
                        + "/"
                        + node.getPrimaryParentAssoc().getQName());
        ...
        if (resolvedNodes.resolvedChild != null) {
            ...
            NodeRef archiveNodeRef =
                    new NodeRef(StoreRef.STORE_REF_ARCHIVE_SPACESSTORE, node.getNodeRef().getId());
            if (nodeService.exists(archiveNodeRef)) {
                // We have found a node in the archive store that has the same
                // UUID as the one that we've been sent.    If it remains it may cause problems
                // later on
                // We delete from the archive store and treat the new node as a create.
                if (log.isInfoEnabled()) {
                    log.info(
                            "Located an archived node with UUID matching transferred node: "
                                    + archiveNodeRef);
            ...
            if (log.isDebugEnabled()) {
                log.debug(
                        "Incoming noderef has no corresponding local noderef: "
                                + node.getNodeRef());
            }

            if (log.isTraceEnabled()) {
                log.trace(
                        "REPO_PRIMARY_MANIFEST_PROCESSOR - node DOES NOT esist yet! Name: '"
                                + node.getProperties().get(ContentModel.PROP_NAME)
                                + "', parentPath: '"
                                + node.getParentPath()
                                + "'");
            }
     ...
     */
    private void create(
            TransferManifestNormalNode node,
            ResolvedParentChildPair resolvedNodes,
        ...
        if (parentNodeRef == null) {
            if (log.isDebugEnabled()) {
                log.debug(
                        "Unable to resolve parent for inbound noderef "
                                + node.getNodeRef()
                                + ".\n  Supplied parent noderef is "
                                + primaryParentAssoc.getParentRef()
                                + ".\n  Supplied parent path is "
                                + node.getParentPath().toString());
            }
            ...
            logComment(
                    "Unable to resolve parent for new incoming node. Storing it in temp folder: "
                            + node.getNodeRef());
        ...
        ChildAssociationRef newNode =
                nodeService.createNode(
                        parentNodeRef, parentAssocType, parentAssocName, node.getType(), props);
        ...
        /**
         * are we adding an alien node here? The transfer service has policies disabled so have to
         * call the consequence of the policy directly.
         */
    ...
    protected void delete(TransferManifestDeletedNode node, NodeRef nodeToDelete) {
        if (alienProcessor.isAlien(nodeToDelete)) {
            ...
            /**
             * Check that if the destination is "from" the transferring repo if it is "from" another
             * repo then ignore
             */
            if (nodeService.hasAspect(nodeToDelete, TransferModel.ASPECT_TRANSFERRED)) {
                String fromRepository =
                        (String)
                                nodeService.getProperty(
                                        nodeToDelete, TransferModel.PROP_FROM_REPOSITORY_ID);
                ...
                if (fromRepository != null && transferringRepo != null) {
                    if (!fromRepository.equalsIgnoreCase(transferringRepo)) {
                        logComment(
                                "Not deleting local node (not from the transferring repository): "
                                        + nodeToDelete);
                        return;
                    }
                }
            ...
            logDeleted(
                    node.getNodeRef(), nodeToDelete, nodeService.getPath(nodeToDelete).toString());
            logSummaryDeleted(
                    node.getNodeRef(), nodeToDelete, nodeService.getPath(nodeToDelete).toString());
    ...
    private void checkOrphans(NodeRef parentNode) {
        ...
        if (orphansToClaim != null) {
            ...
            for (ChildAssociationRef orphan : orphansToClaim) {
                logComment(
                        "Re-parenting previously orphaned node ("
                                + orphan.getChildRef()
                                + ") with found parent "
                                + orphan.getParentRef());
                ChildAssociationRef newRef =
                ...
                if (nodeService.hasAspect(
                        newRef.getParentRef(), TransferModel.ASPECT_TRANSFERRED)) {
    ...
    private void update(
        ...
        if (nodeService.hasAspect(nodeToUpdate, TransferModel.ASPECT_TRANSFERRED)) {
            String fromRepository =
                    (String)
                            nodeService.getProperty(
                                    nodeToUpdate, TransferModel.PROP_FROM_REPOSITORY_ID);
            String transferringRepo = header.getRepositoryId();

            if (fromRepository != null && transferringRepo != null) {
                if (!fromRepository.equalsIgnoreCase(transferringRepo)) {
                    logComment(
                            "Not updating local node (not from the transferring repository): "
                                    + node.getNodeRef());
                    return;
                }
            }
            ...
            logComment(
                    "Not updating local node - node is local to this repository): "
                            + node.getNodeRef());
        ...
        if (!currentParent.getParentRef().equals(parentNodeRef)
            ...
            ChildAssociationRef newNode =
                    nodeService.moveNode(
                            nodeToUpdate, parentNodeRef, parentAssocType, parentAssocName);
            logMoved(
                    node.getNodeRef(),
                    nodeToUpdate,
                    node.getParentPath().toString(),
            ...
            /**
             * are we adding an alien node here? The transfer service has policies disabled so have
             * to call the consequence of the policy directly.
             */
            if (nodeService.hasAspect(newNode.getChildRef(), TransferModel.ASPECT_ALIEN)) {
                alienProcessor.afterMoveAlien(newNode);
            } else {
                /**
                 * are we adding an alien node here? The transfer service has policies disabled so
                 * have to call the consequence of the policy directly.
                 */
                if (nodeService.hasAspect(parentNodeRef, TransferModel.ASPECT_TRANSFERRED)
                        || nodeService.hasAspect(parentNodeRef, TransferModel.ASPECT_ALIEN)) {
                    alienProcessor.onCreateChild(newNode, header.getRepositoryId(), true);
        ...
        if (updateNeeded(node, nodeToUpdate)) {
            ...
            logUpdated(
                    node.getNodeRef(), nodeToUpdate, nodeService.getPath(nodeToUpdate).toString());
            ...
            Map<QName, Serializable> contentProps =
                    processProperties(nodeToUpdate, props, existingProps);

            // If there was already a value for invadedBy then leave it alone rather than replacing
            // it.
            if (existingProps.containsKey(TransferModel.PROP_INVADED_BY)) {
                props.put(
                        TransferModel.PROP_INVADED_BY,
                        existingProps.get(TransferModel.PROP_INVADED_BY));
            }
            ...
            if (contentUpdated) {
                logSummaryUpdated(
                        node.getNodeRef(),
                        nodeToUpdate,
                        nodeService.getPath(nodeToUpdate).toString());
            }
            ...
            // Add mandatory aspects to the supplied aspects (eg. should not explicitly remove
            // auditable aspect from a folder - see also DMDeploymentTarget for similar)
            List<AspectDefinition> aspectDefs =
                    dictionaryService
                            .getType(nodeService.getType(nodeToUpdate))
            ...
            if (acl != null) {
                ...
                Set<AccessPermission> existingPermissions =
                        permissionService.getAllSetPermissions(nodeToUpdate);
    ...
    /**
     * This method takes all the received properties and separates them into two parts. The content
     * properties are removed from the non-content properties such that the non-content properties
     * remain in the "props" map and the content properties are returned from this method
     * Subsequently, any properties that are to be retained from the local repository are copied
     * over into the "props" map. The result of all this is that, upon return, "props" contains all
     * the non-content properties that are to be written to the local repo, and "contentProps"
     * contains all the content properties that are to be written to the local repo.
     *
     * @param nodeToUpdate The noderef of the existing node in the local repo that is to be updated
     *     with these properties. May be null, indicating that these properties are destined for a
     *     brand new local node.
     * @param props the new properties
     * @param existingProps the existing properties, null if this is a create
     * @return A map containing the content properties which are going to be replaced from the
     *     supplied "props" map
     */
    private Map<QName, Serializable> processProperties(
                ...
                if (existingProps != null) {
                    ...
                    if (stagedFile.exists()) {
                        ...
                        if (props.containsKey(key)) {
                            if (log.isDebugEnabled()) {
                                log.debug(
                                        "keep existing content for node:"
                                                + nodeToUpdate
                                                + ", "
                                                + key);
                            }
        ...
        for (Map.Entry<QName, Serializable> contentEntry : contentProps.entrySet()) {
            ...
            if (contentUrl == null || contentUrl.isEmpty()) {
                ...
                ContentWriter writer =
                        contentService.getWriter(nodeToUpdate, contentEntry.getKey(), true);
    ...
    protected boolean updateNeeded(TransferManifestNormalNode node, NodeRef nodeToUpdate) {
        ...
        // Assumption: if the modified and modifier properties haven't changed, and the cm:content
        // property
        // (if it exists) hasn't changed size then we can assume that properties don't need to be
        // updated...
        ...
        //        ContentData suppliedContent = (ContentData)
        // suppliedProps.get(ContentModel.PROP_CONTENT);
        //
        //        Map<QName, Serializable> existingProps = nodeService.getProperties(nodeToUpdate);
        ...
        // existingProps.get(ContentModel.PROP_CONTENT);
        //
        //        updateNeeded = false;
        //        updateNeeded |= ((suppliedModifiedDate != null &&
        // !suppliedModifiedDate.equals(existingModifiedDate)) ||
        //                (existingModifiedDate != null &&
        // !existingModifiedDate.equals(suppliedModifiedDate)));
        //        updateNeeded |= ((suppliedContent != null && existingContent == null)
        //                || (suppliedContent == null && existingContent != null) ||
        // (suppliedContent != null
        //                && existingContent != null && suppliedContent.getSize() !=
        // existingContent.getSize()));
        //        updateNeeded |= ((suppliedModifier != null &&
        // !suppliedModifier.equals(existingModifier)) ||
     ...
     * Process categories.
     *
     * <p>CRUD of Categories and Tags - also maps noderefs of type d:content from source to target
     *
                        ...
                        if (thing != null) {
                            if (def.isMultiValued()) {
                                if (thing instanceof java.util.Collection) {
                                    ...
                                    java.util.Collection<NodeRef> c =
                                            (java.util.Collection<NodeRef>) thing;
                                    for (NodeRef sourceCategoryNodeRef : c) {
                                        if (log.isDebugEnabled()) {
                                            log.debug(
                                                    "sourceCategoryNodeRef"
                                                            + sourceCategoryNodeRef);
                                        }
                                        // substitute target node ref fot source node ref
                                        NodeRef targetNodeRef =
                                                processCategory(
                                                        sourceCategoryNodeRef, manifestCategories);
                                    ...
                                    throw new AlfrescoRuntimeException(
                                            "Multi valued object is not a collection"
                                                    + val.getKey());
                                }
                            } else {
                                ...
                                NodeRef targetNodeRef =
                                        processCategory(sourceCategoryNodeRef, manifestCategories);
     ...
     *
     * <p>It will lazily create any missing categories and tags as it executes.
     *
     * @param sourceCategoryNodeRef NodeRef
     * @param manifestCategories Map<NodeRef, ManifestCategory>
     * @return targetNodeRef
     */
    private NodeRef processCategory(
            final NodeRef sourceCategoryNodeRef,
        ...
        if (category != null) {
            ...
            if (aspectQName.equals(ContentModel.ASPECT_TAGGABLE)) {
                ...
                destinationNodeRef =
                        taggingService.getTagNodeRef(
                                sourceCategoryNodeRef.getStoreRef(), tagQName.getLocalName());
                if (destinationNodeRef != null) {
                ...
                RunAsWork<NodeRef> processCategory =
                        new RunAsWork<NodeRef>() {
                            @Override
                            public NodeRef doWork() throws Exception {
                                QName rootCatName =
                                        QName.createQName(catPath.get(3).getElementString());

                                Collection<ChildAssociationRef> roots =
                                        categoryService.getRootCategories(
                                                sourceCategoryNodeRef.getStoreRef(), aspectQName);
                                ...
                                if (rootCategoryNodeRef == null) {
                                    ...
                                    rootCategoryNodeRef =
                                            categoryService.createRootCategory(
                                                    sourceCategoryNodeRef.getStoreRef(),
                                                    aspectQName,
                                                    rootCatName.getLocalName());
                                }

                                NodeRef workingNodeRef = rootCategoryNodeRef;
                                // Root category does already exist - step through any
                                // sub-categories
                                    ...
                                    if (child != null) {
                                        ...
                                        workingNodeRef =
                                                categoryService.createCategory(
                                                        workingNodeRef, subCatName.getLocalName());
                                    }
                                }
                ...
                destinationNodeRef =
                        AuthenticationUtil.runAs(
                                processCategory, AuthenticationUtil.SYSTEM_USER_NAME);
                categoryMap.put(sourceCategoryNodeRef, destinationNodeRef);
                return destinationNodeRef;
            }

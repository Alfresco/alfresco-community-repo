...
public class ResourceDictionaryBuilder {
    ...
    private static void processResources(
            ...
            Map<ApiScopeKey, Map<Integer, List<ResourceWithMetadata>>> entityApiMap,
            Map<ApiScopeKey, Map<Integer, List<ResourceWithMetadata>>> relationshipApiMap) {
        logger.debug("Will process resources ");

        for (Entry<ApiScopeKey, Map<Integer, List<ResourceWithMetadata>>>
                apiGroupedByNameAndScopeEntry : entityApiMap.entrySet()) {
            logger.debug("Processing " + apiGroupedByNameAndScopeEntry.getKey());

            Map<Integer, List<ResourceWithMetadata>> apiVersions =
                    apiGroupedByNameAndScopeEntry.getValue();
            List<Integer> versions = new ArrayList<Integer>(apiVersions.keySet());
            logger.debug("Versions " + String.valueOf(versions));
            Collections.sort(versions);

            // Loop through the different versions of the API and add them to the Map of resources
            // By API with Version.
            for (int i = 0; i < versions.size(); i++) {
                Api apiVersion =
                        Api.valueOf(
                                apiGroupedByNameAndScopeEntry.getKey().name,
                                apiGroupedByNameAndScopeEntry.getKey().scope.toString(),
                                Integer.toString(versions.get(i)));
                Map<String, ResourceWithMetadata> resourcesByApiAndVersion =
                        findResources(rd.getAllResources(), apiVersion);
                ...
                if (i > 0) {
                    ...
                    Api previousVersion =
                            Api.valueOf(
                                    apiVersion.getName(),
                                    apiVersion.getScope().toString(),
                                    Integer.toString(apiVersion.getVersion() - 1));
                    logger.debug("Previous version is " + previousVersion);
                    Map<String, ResourceWithMetadata> resourcesForPreviousVersion =
                            findResources(rd.getAllResources(), previousVersion);
                ...
                for (ResourceWithMetadata resourceWithMetadata :
                        apiGroupedByNameAndScopeEntry.getValue().get(apiVersion.getVersion())) {
                    resourcesByApiAndVersion.put(
                            resourceWithMetadata.getMetaData().getUniqueId(), resourceWithMetadata);
                }
            }
        }
    }
     ...
     * @return Map
     */
    private static Map<String, ResourceWithMetadata> findResources(
            Map<Api, Map<String, ResourceWithMetadata>> allResources, Api api) {
     ...
     * @return Map
     */
    private static <T> Map<ApiScopeKey, Map<Integer, List<ResourceWithMetadata>>> parseResources(
            Collection<Object> resources) {
        Map<ApiScopeKey, Map<Integer, List<ResourceWithMetadata>>> apiMap =
                new HashMap<ApiScopeKey, Map<Integer, List<ResourceWithMetadata>>>();
        ...
        for (Object bean : resources) {
            ...
            if (api == null) {
                throw new PlatformRuntimeException(
                        "Invalid resource bean defintion.  No @WebApi defined for package: "
    ...
    public static ResourceDictionary build(
            Collection<Object> entityResources, Collection<Object> relationshipResources) {
        ...
        Map<ApiScopeKey, Map<Integer, List<ResourceWithMetadata>>> apiMap =
                parseResources(entitiesAndRelations);
    ...
    public static class ApiScopeKey {
        ...
        public String toString() {
            ...
            builder.append("ApiScopeKey [name=")
                    .append(this.name)
                    .append(", scope=")
                    .append(this.scope)
                    .append("]");

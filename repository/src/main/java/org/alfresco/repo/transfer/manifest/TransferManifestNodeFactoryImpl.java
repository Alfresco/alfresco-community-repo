...
/**
 * Factory to build TransferManifestNodes given their repository NodeRef. Extracts values from the
 * nodeService and instantiates TransferManifestNode.
 *
 * @author Mark Rogers
 */
public class TransferManifestNodeFactoryImpl implements TransferManifestNodeFactory {
    ...
    public TransferManifestNode createTransferManifestNode(
        ...
        if (status.isDeleted()) {
            // This node used to exist but doesn't any more. We can't discover anything about its
            // original parentage
            // so we will create a dummy record that contains the correct noderef but dummy parent
            // association and
            // parent path. This will keep the target side happy, and will result in the node being
            // deleted
            ...
            ChildAssociationRef dummyPrimaryParent =
                ...
                ChildAssociationRef car =
                        (ChildAssociationRef)
                                nodeService.getProperty(
                                        nodeRef, ContentModel.PROP_ARCHIVED_ORIGINAL_PARENT_ASSOC);
            ...
            node.setProperties(
                    getNodeProperties(
                            nodeRef, definition == null ? null : definition.getExcludedAspects()));
            node.setAspects(
                    getNodeAspects(
                            nodeRef, definition == null ? null : definition.getExcludedAspects()));
                                ...
                                for (NodeRef categoryNodeRef : c) {
                                    if (categoryNodeRef != null) {
                                        categories.put(
                                                categoryNodeRef,
                                                getManifestCategory(
                                                        transferContext, categoryNodeRef));
                                    }
                                }
                            } else {
    ...
    private Map<QName, Serializable> getNodeProperties(
            NodeRef nodeRef, Set<QName> excludedAspects) {
        Map<QName, Serializable> properties = mlAwareNodeService.getProperties(nodeRef);
        if (excludedAspects == null || excludedAspects.size() == 0) {
            return properties;
        } else {
            Map<QName, Serializable> filteredProperties =
    ...
    private ManifestCategory getManifestCategory(
            TransferContext transferContext, NodeRef categoryNodeRef) {

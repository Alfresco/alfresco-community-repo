 ...
 * <br>
 * This service listens to events from {@link LicenseChangeHandler} and enables or disables
 * Heartbeat accordingly.
 */
public class HBDataCollectorServiceImpl implements HBDataCollectorService, LicenseChangeHandler {
    ...
    /**
     * Register data collector with this service, a job will be scheduled for the collector if
     * Heartbeat is enabled. The registered collector will be called to provide heartbeat data at
     * the scheduled interval. Each collector registered via this method must have a unique
     * collector id.
     *
    ...
    public synchronized void registerCollector(final HBBaseDataCollector collector) {
        ...
        if (collectors.containsKey(collector.getCollectorId())) {
            throw new IllegalArgumentException(
                    "HeartBeat did not registered collector because a collector with ID: \n"
                            + collector.getCollectorId()
                            + " already exists. Collectors must have unique collector IDs");
        }
    ...
    /**
     * @param collector - Deregister data collector. Removed collector and unscheduled associated
     *     job.
     */
    @Override
    public synchronized void deregisterCollector(final HBBaseDataCollector collector) {
        if (collectors.remove(collector.getCollectorId(), collector)) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "HeartBeat unscheduled job and deregistered collector: "
                                + collector.getCollectorId());
            }
        }
    }
    ...
    /**
     * Listens for license changes. If a license is change or removed, the heartbeat job is
     * rescheduled.
     */
    @Override
    ...
    private void restartAllCollectorSchedules() {
        for (HBBaseDataCollector collector : collectors.values()) {
            try {
                ...
                // Log and ignore
                logger.error(
                        "HeartBeat failed to restart collector: " + collector.getCollectorId(), e);
            }
        }
    }

...
public class RepoService {
  ...
  private static final String FAVOURITE_SITES_PREFIX =
    "org.alfresco.share.sites.favourites.";
  ...
  public RepoService(ApplicationContext applicationContext) throws Exception {
    ...
    this.authenticationService =
      (MutableAuthenticationService) applicationContext.getBean(
        "AuthenticationService"
      );
    this.siteService = (SiteService) applicationContext.getBean("SiteService");
    this.activityService =
      (ActivityService) applicationContext.getBean("activityService");
    this.fileFolderService =
      (FileFolderService) applicationContext.getBean("FileFolderService");
    this.contentService =
    ...
    this.taggingService =
      (TaggingService) applicationContext.getBean("TaggingService");
    this.ratingService =
      (RatingService) applicationContext.getBean("RatingService");
    this.tenantService =
      (TenantService) applicationContext.getBean("tenantService");
    this.tenantAdminService =
      (TenantAdminService) applicationContext.getBean("tenantAdminService");
    this.personService =
      (PersonService) applicationContext.getBean("PersonService");
    this.contentStoreCleaner =
      (ContentStoreCleaner) applicationContext.getBean("contentStoreCleaner");
    this.postDAO = (ActivityPostDAO) applicationContext.getBean("postDAO");
    this.nodeRatingSchemeRegistry =
      (NamedObjectRegistry<RatingScheme>) applicationContext.getBean(
        "nodeRatingSchemeRegistry"
      );
    this.cociService =
      (CheckOutCheckInService) applicationContext.getBean(
        "CheckoutCheckinService"
      );
    this.favouritesService =
      (FavouritesService) applicationContext.getBean("FavouritesService");
    this.dictionaryService =
      (DictionaryService) applicationContext.getBean("dictionaryService");
    this.invitationService =
      (InvitationService) applicationContext.getBean("InvitationService");
    this.lockService = (LockService) applicationContext.getBean("LockService");
    this.cmisConnector =
      (CMISConnector) applicationContext.getBean("CMISConnector");
    this.activities = (Activities) applicationContext.getBean("activities");
    this.hiddenAspect =
      (HiddenAspect) applicationContext.getBean("hiddenAspect");
    this.networksService =
      (NetworksService) applicationContext.getBean("networksService");
    ...
    Scheduler scheduler = (Scheduler) applicationContext.getBean(
      "schedulerFactory"
    );

    CronTrigger contentStoreCleanerTrigger = (CronTrigger) applicationContext.getBean(
      "contentStoreCleanerTrigger"
    );
    scheduler.pauseJob(contentStoreCleanerTrigger.getJobKey());

    ChildApplicationContextFactory activitiesFeed = (ChildApplicationContextFactory) applicationContext.getBean(
      "ActivitiesFeed"
    );
    ApplicationContext activitiesFeedCtx = activitiesFeed.getApplicationContext();
    this.postLookup = (PostLookup) activitiesFeedCtx.getBean("postLookup");
    this.feedGenerator =
      (FeedGenerator) activitiesFeedCtx.getBean("feedGenerator");
    this.feedGeneratorJobDetail =
      (JobDetail) activitiesFeedCtx.getBean("feedGeneratorJobDetail");
    this.postLookupJobDetail =
      (JobDetail) activitiesFeedCtx.getBean("postLookupJobDetail");
    this.feedCleanerJobDetail =
      (JobDetail) activitiesFeedCtx.getBean("feedCleanerJobDetail");
    this.postCleanerJobDetail =
      (JobDetail) activitiesFeedCtx.getBean("postCleanerJobDetail");
    this.feedNotifierJobDetail =
      (JobDetail) activitiesFeedCtx.getBean("feedNotifierJobDetail");
  ...
  public Invitation approveSiteInvitation(String personId, String siteId) {
    ...
    List<Invitation> invitations = invitationService.listPendingInvitationsForInvitee(
      personId
    );
    for (Invitation invitation : invitations) {
      if (invitation.getResourceName().equals(siteId)) {
        ret =
          invitationService.approve(invitation.getInviteId(), "I accept you");
      }
    }
  ...
  public void moveNode(NodeRef nodeToMoveRef, NodeRef newParentNodeRef)
  ...
  public NodeRef addUserDescription(
        ...
        public NodeRef doWork() throws Exception {
          ...
          if (userRef == null) {
            throw new AuthenticationException(
              "User name does not exist: " + personId
            );
          }
          ...
          log(
            "Updated person description " +
            personId +
            (network != null ? " in network " + network : "")
          );
          return userRef;
        }
      }
  ...
  public TestPerson getOrCreateUser(
    final PersonInfo personInfo,
    final String username,
    final TestNetwork network
  ) {
  ...
  private boolean isDefaultAdmin(String username, TestNetwork network) {
    if (
      (network == null) ||
      (TenantService.DEFAULT_DOMAIN.equals(network.getId()))
    ) {
      return (DEFAULT_ADMIN.equalsIgnoreCase(username));
    } else {
      return (
        (DEFAULT_ADMIN + "@" + network.getId()).equalsIgnoreCase(username)
      );
    }
  }
  ...
  public TestPerson getOrCreateUser(
    ...
    final String username,
    final TestNetwork network,
    final boolean deletePerson
  ) {
    return AuthenticationUtil.runAsSystem(
      new RunAsWork<TestPerson>() {
        @Override
        public TestPerson doWork() throws Exception {
          final TestPerson testPerson = new TestPerson(
          ...
          if (!isDefaultAdmin(username, network)) {
            ...
            if (personNodeRef == null) {
              ...
              authenticationService.createAuthentication(
                username,
                personInfo.getPassword().toCharArray()
              );

              if (
                EnterpriseTestFixture.WITH_AVATAR.equals(
                  personInfo.getInstantmsg()
                )
              ) {
   ...
   * @deprecated
   */
  public TestSite createSite(
    TestNetwork network,
    final SiteInformation siteInfoIn
  ) {
    SiteInfo siteInfoOut = siteService.createSite(
      siteInfoIn.getSitePreset() != null
        ? siteInfoIn.getSitePreset()
        : TEST_SITE_PRESET,
      siteInfoIn.getShortName(),
      siteInfoIn.getTitle(),
      siteInfoIn.getDescription(),
      siteInfoIn.getSiteVisibility()
    );
  ...
  public Invitation rejectSiteInvitation(String personId, String siteId) {
    ...
    for (Invitation invitation : invitations) {
      if (invitation.getResourceName().equals(siteId)) {
        ret =
          invitationService.reject(invitation.getInviteId(), "I reject you");
      }
    }
  ...
  public List<Invitation> getModeratedSiteInvitations(
    String networkId,
    String runAsUserId,
    final String inviteeId,
    final String siteId
  ) {
    List<Invitation> invitations = TenantUtil.runAsUserTenant(
      new TenantRunAsWork<List<Invitation>>() {
        @Override
        public List<Invitation> doWork() throws Exception {
          ...
          List<Invitation> invitations = invitationService.searchInvitation(
            searchCriteria
          );
          return invitations;
        }
  ...
  public List<Activity> getActivities(
    ...
    List<ActivityFeedEntity> feedEntities = activityService.getUserFeedEntries(
      personId,
      siteId,
      excludeUser,
      excludeOthers,
    ...
    for (ActivityFeedEntity entity : feedEntities) {
      ...
      String postDateStr = PublicApiDateFormat
        .getDateFormat()
        .format(entity.getPostDate());
      Activity activity = new Activity(
        entity.getId(),
        networkId,
        siteId,
        entity.getFeedUserId(),
        entity.getPostUserId(),
        postDateStr,
        entity.getActivityType(),
        parseActivitySummary(entity)
      );
  ...
  @SuppressWarnings("unchecked")
  public Map<String, Object> getActivitySummary(ActivityFeedEntity entity)
  ...
  public void checkSiteMember(
    ...
    assertEquals(
      siteRole.toString(),
      siteService.getMembersRole(site.getSiteId(), person.getId())
    );
  }

  public void addFavouriteSite(
    final String username,
  ...
  private PagingResults<SiteInfo> getFavouriteSites(
    String userName,
    PagingRequest pagingRequest
  ) {
    final Collator collator = Collator.getInstance();

    final Set<SiteInfo> sortedFavouriteSites = new TreeSet<SiteInfo>(
      new Comparator<SiteInfo>() {
        @Override
    ...
    final List<SiteInfo> page = new ArrayList<SiteInfo>(
      pageDetails.getPageSize()
    );
    Iterator<SiteInfo> it = sortedFavouriteSites.iterator();
    for (
      int counter = 0;
      counter < pageDetails.getEnd() && it.hasNext();
      counter++
    ) {
  ...
  public List<FavouriteSite> getFavouriteSites(TestPerson user) {
    ...
    PagingResults<SiteInfo> pagingResults = getFavouriteSites(
      user.getId(),
      new PagingRequest(0, Integer.MAX_VALUE)
    );
  ...
  public List<MemberOfSite> getSiteMemberships(String personId) {
    ...
    for (SiteInfo siteInfo : sites) {
      String roleStr = siteService.getMembersRole(
  ...
  private Long getMaxActivitySeq() throws SQLException {
    Long maxSequence = transactionHelper.doInTransaction(
  ...
  public List<Tag> getTags() {
    ...
    for (String tag : taggingService.getTags(
      ...
      tags.add(
        new Tag(
          null,
          taggingService
            .getTagNodeRef(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE, tag)
            .getId(),
          tag
        )
      );
  ...
  private Properties getProperties(NodeRef nodeRef) {
    ...
    final Properties properties = cmisConnector.getNodeProperties(
      nodeInfo,
      null
    );
    ...
    final PropertyStringImpl titleProp = new PropertyStringImpl(
      ContentModel.PROP_TITLE.toString(),
      title
    );
    Properties wrapProperties = new Properties() {
      ...
      public Map<String, PropertyData<?>> getProperties() {
        Map<String, PropertyData<?>> updatedProperties = new HashMap<String, PropertyData<?>>(
  ...
  public FavouriteDocument getDocument(
    ...
    return TenantUtil.runAsSystemTenant(
      new TenantRunAsWork<FavouriteDocument>() {
        @Override
  ...
  public FavouriteFolder getFolder(String networkId, final NodeRef nodeRef) {
    return TenantUtil.runAsSystemTenant(
      new TenantRunAsWork<FavouriteFolder>() {
        @Override
  ...
  public List<NodeRating> getNodeRatings(
    String personId,
    String networkId,
    final NodeRef nodeRef
  ) {
    List<NodeRating> ratings = TenantUtil.runAsUserTenant(
      new TenantRunAsWork<List<NodeRating>>() {
        @Override
        public List<NodeRating> doWork() throws Exception {
          List<NodeRating> ratings = new ArrayList<NodeRating>();

          {
            Rating likesRating = ratingService.getRatingByCurrentUser(
              nodeRef,
              "likesRatingScheme"
            );
            ...
            if (likesRating != null) {
              myRating =
                likesRating.getScore() == 1.0f ? Boolean.TRUE : Boolean.FALSE;
              Date rateTime = likesRating.getAppliedAt();
              ratedAt = PublicApiDateFormat.getDateFormat().format(rateTime);
            }
            int ratingsCount = ratingService.getRatingsCount(
              nodeRef,
              "likesRatingScheme"
            );
          ...
          {
            Rating fiveStarRating = ratingService.getRatingByCurrentUser(
              nodeRef,
            ...
            float averageRating = ratingService.getAverageRating(
              nodeRef,
              "fiveStarRatingScheme"
            );
            Aggregate aggregate = new Aggregate(
              ratingsCount,
              averageRating == -1 ? null : averageRating
            );
            ratings.add(
              new NodeRating(
                nodeRef.getId(),
  ...
  public NodeRef addToDocumentLibrary(TestSite site, String name, QName type) {
    NodeRef documentLibraryContainerNodeRef = siteService.getContainer(
      site.getSiteId(),
      "documentLibrary"
    );
    NodeRef nodeRef = fileFolderService
      .create(documentLibraryContainerNodeRef, name, type)
      .getNodeRef();
    return nodeRef;
  }
  ...
  public NodeRef createDocument(
    final NodeRef parentNodeRef,
    final String name,
    final String content
  ) {
    NodeRef nodeRef = fileFolderService
      .create(parentNodeRef, name, ContentModel.TYPE_CONTENT)
      .getNodeRef();
    ContentWriter writer = contentService.getWriter(
      nodeRef,
  ...
  public NodeRef createFolder(final NodeRef parentNodeRef, final String name) {
    NodeRef nodeRef = fileFolderService
      .create(parentNodeRef, name, ContentModel.TYPE_FOLDER)
      .getNodeRef();
    return nodeRef;
  }

  public NodeRef createFolder(
    final NodeRef parentNodeRef,
    final String name,
    final String title,
    final String description
  ...
  public NodeRef createObjectOfCustomType(
    final NodeRef parentNodeRef,
    final String name,
  ...
  public NodeRef createComment(NodeRef nodeRef, final Comment comment) {
    NodeRef commentNodeRef = commentService.createComment(
      nodeRef,
      comment.getTitle(),
      comment.getContent(),
      false
  ...
  public class TestSite extends SiteImpl {
    ...
    public Map<String, String> getMembers(final String roleFilter) {
      Map<String, String> members = TenantUtil.runAsSystemTenant(
        new TenantRunAsWork<Map<String, String>>() {
          @Override
          public Map<String, String> doWork() throws Exception {
            Map<String, String> members = siteService.listMembers(
              getSiteId(),
              null,
              roleFilter,
              0
    ...
    public boolean isMember(final String personId) {
      Boolean isMember = TenantUtil.runAsSystemTenant(
    ...
    public SiteRole getMember(String personId) {
      return SiteRole.valueOf(
        siteService.getMembersRole(getSiteId(), personId)
      );
    }
  ...
  public static <T extends Object> Iterator<T> getWrappingIterator(
    final int startIdx,
    final List<T> list
  ) {
  ...
  public class TestNetwork
    ...
    public void create() {
      if (
        !getId().equals(TenantService.DEFAULT_DOMAIN) &&
        !tenantAdminService.existsTenant(getId())
      ) {
        tenantAdminService.createTenant(
          getId(),
          DEFAULT_ADMIN_PWD.toCharArray()
        );
        numNetworks++;
    ...
    public TestSite createSite(
      ...
      String shortName =
        "TESTSITE" +
        (siteRootName != null ? siteRootName : "") +
        GUID.generate();
    ...
    public NodeRef addUserDescription(
      ...
      NodeRef personRef = TenantUtil.runAsTenant(
        new TenantRunAsWork<NodeRef>() {
          public NodeRef doWork() throws Exception {
            return RepoService.this.addUserDescription(
                personId,
                TestNetwork.this,
                personDescription
              );
    ...
    public TestPerson createUser(final PersonInfo personInfo) {
      final String username = publicApiContext.createUserName(
        personInfo.getUsername(),
        getId()
      );
      TestPerson testPerson = TenantUtil.runAsTenant(
        new TenantRunAsWork<TestPerson>() {
          public TestPerson doWork() throws Exception {
            TestPerson person =
              RepoService.this.createUser(
                  personInfo,
                  username,
                  TestNetwork.this
                );
        ...
        new TenantRunAsWork<TestSite>() {
          ...
          public TestSite doWork() throws Exception {
            ...
            for (SiteInfo info : siteService.listSites(null, null)) {
              boolean isMember = siteService.isMember(
                info.getShortName(),
                personId
              );
              if (!isMember) {
    ...
    public TestSite getSite(final String siteShortName) {
      TestSite site = TenantUtil.runAsTenant(
        new TenantRunAsWork<TestSite>() {
          @Override
          public TestSite doWork() throws Exception {
            SiteInfo siteInfo = siteService.getSite(siteShortName);
    ...
    public List<TestSite> getSites(String personId) {
      List<TestSite> sites = TenantUtil.runAsUserTenant(
        new TenantRunAsWork<List<TestSite>>() {
          @Override
    ...
    public List<TestSite> getAllSites() {
      List<TestSite> sites = TenantUtil.runAsSystemTenant(
        new TenantRunAsWork<List<TestSite>>() {
          @Override
          public List<TestSite> doWork() throws Exception {
            List<SiteInfo> results = siteService.listSites(null, null);
    ...
    public List<SiteContainer> getSiteContainers(
      final String siteId,
      TestPerson runAs
    ) {
      List<SiteContainer> siteContainers = TenantUtil.runAsUserTenant(
        new TenantRunAsWork<List<SiteContainer>>() {
          @Override
          public List<SiteContainer> doWork() throws Exception {
            PagingResults<FileInfo> results = siteService.listContainers(
              siteId,
              new PagingRequest(0, Integer.MAX_VALUE)
            );
            List<SiteContainer> ret = new ArrayList<SiteContainer>(
    ...
    public Map<String, TestSite> getSitesForUser(String username) {
      ...
      List<SiteInfo> sites = TenantUtil.runAsUserTenant(
    ...
    public List<MemberOfSite> getSiteMemberships(String username) {
      ...
      List<MemberOfSite> sites = TenantUtil.runAsUserTenant(
        new TenantRunAsWork<List<MemberOfSite>>() {
          @Override
          public List<MemberOfSite> doWork() throws Exception {
            ...
            for (SiteInfo siteInfo : sites) {
              ...
              String role = siteService.getMembersRole(
                siteInfo.getShortName(),
                userId
              );
  ...
  public static class SiteMembership implements Comparable<SiteMembership> {
    ...
    public int compareTo(SiteMembership siteMembership) {
      int ret = site
        .getSiteId()
        .compareTo(siteMembership.getSite().getSiteId());
      if (ret == 0) {

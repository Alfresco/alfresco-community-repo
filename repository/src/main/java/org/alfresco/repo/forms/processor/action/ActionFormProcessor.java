...
public class ActionFormProcessor extends FilteredFormProcessor<ActionDefinition, ActionFormResult> {
    ...
    /**
     * This method creates the {@link Action} based on the submitted form data. It then sets the
     * action parameters based on the form data also.
     */
    private Action createActionAndParams(ActionDefinition item, FormData data) {
        ...
        for (ParameterDefinition actionParamDef : actionParamDefs) {
            ...
            FieldData fieldData =
                    data.getFieldData(FormFieldConstants.PROP_DATA_PREFIX + paramDefinitionName);
            if (fieldData == null) {
                fieldData =
                        data.getFieldData(
                ...
                if (actionParamDef.isMultiValued()) {
                    if (fieldValueObj instanceof String) {
                        if (((String) fieldValueObj).length() == 0) {
                            ...
                            // In order to allow short-form QNames, which are not handled by the
                            // default type converter,
                            // we'll do QName conversion ourselves.
                            StringTokenizer tokenizer =
                                    new StringTokenizer((String) fieldValueObj, ",");
                    ...
                    if (fieldValueObj instanceof Collection<?>) {
                        convertedObj =
                                DefaultTypeConverter.INSTANCE.convert(
                                        typeDef, (Collection<?>) fieldValueObj);
    ...
    protected List<Field> generateDefaultFields(
        ...
        for (ParameterDefinition paramDef :
                itemData.getActionDefinition().getParameterDefinitions()) {
            if (!fieldsToIgnore.contains(paramDef.getName())) {
                ActionParameterField actionParameterField =
                        new ActionParameterField(paramDef, actionService);
                fields.add(actionParameterField);
            }
        }

        // And also generate the special case of "execute asynchronously", which is not defined
        // as an ActionParameter within the ActionExecuter, but is instead available for all
        // actions.
    ...
    protected List<Field> generateSelectedFields(
            List<String> requestedFields, FormCreationData data) {

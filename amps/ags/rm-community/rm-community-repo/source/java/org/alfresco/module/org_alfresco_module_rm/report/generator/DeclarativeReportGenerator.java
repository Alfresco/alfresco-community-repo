...
public class DeclarativeReportGenerator extends BaseReportGenerator {
  ...
  protected static final NodeRef TEMPLATE_ROOT = new NodeRef(
   ...
   */
  @Override
  protected String generateReportName(
  ...
  protected ContentReader generateReportContent(
    ...
    // get the model
    Map<String, Serializable> model = createTemplateModel(
      reportTemplateNodeRef,
      reportedUponNodeRef,
      properties
    );

    // run the template
    String result = templateService.processTemplate(
      "freemarker",
      reportTemplateNodeRef.toString(),
      model
    );
  ...
  private NodeRef getReportTemplate(String mimetype) {
    ...
    if (!fileFolderService.exists(TEMPLATE_ROOT)) {
      throw new AlfrescoRuntimeException(
        "Unable to get report template, because the template root folder does not exist in the data dictionary."
      );
    }

    String reportTemplateName = getReportTemplateName(mimetype);

    NodeRef reportTemplateNodeRef = fileFolderService.searchSimple(
      TEMPLATE_ROOT,
      reportTemplateName
    );
  ...
  private String getReportTemplateName(String mimetype) {
    String typePrefixName = reportType
  ...
  protected void checkReportApplicability(NodeRef reportedUponNodeRef) {
    if (applicableTypes != null && applicableTypes.size() != 0) {
      ...
      if (!isTypeApplicable) {
        // throw an exception
        throw new AlfrescoRuntimeException(
          "Can't generate report, because the provided reported upon node reference is type " +
          type.toString() +
  ...
  @SuppressWarnings("unchecked")
  @Override
  protected Map<QName, Serializable> generateReportMetadata(
    NodeRef reportedUponNodeRef
  ) {
    return Collections.EMPTY_MAP;
  }

...
public class StatsGet extends DeclarativeWebScript {
  ...
  public void setPostProcessors(
    Map<String, ? extends StatsProcessor> postProcessors
  ) {
  ...
  protected Map<String, Object> executeImpl(
    ...
    if (templateVars != null && templateVars.containsKey("siteId")) {
      ...
      if (siteInfo == null) {
        throw new AccessDeniedException(
          "No such site: " + templateVars.get("siteId")
        );
      }
    }

    String facetKey = req.getParameter("facet");
    if (facetKey == null) facetKey =
      facets.entrySet().iterator().next().getKey(); //default
    String query;

    QName propFacet = findFacet(facetKey);
    Pair<LocalDate, LocalDate> startAndEnd = getStartAndEndDates(
    ...
    params.addStatsParameter(
      ...
      StatsParameters.FACET_PREFIX + propFacet.toString()
    );
  ...
  protected String buildQuery(
    SiteInfo siteInfo,
    String facetKey,
    Pair<LocalDate, LocalDate> startEndDate
  ) {
    ...
    if (siteInfo != null) {
      luceneQuery.append(
        " AND ANCESTOR:\"" + siteInfo.getNodeRef().toString() + "\""
      );
    }
   ...
   */
  public static Pair<LocalDate, LocalDate> getStartAndEndDates(
    String start,
    String end
  ) {

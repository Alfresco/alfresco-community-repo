...
/**
 * A default {@link RemoteUserMapper} implementation. Extracts a user ID using {@link
 * HttpServletRequest#getRemoteUser()} and optionally from a configured request header. If there is
 * no configured proxy user name, it returns the request header user name if there is one, or the
 * remote user name otherwise. If there is a configured proxy user, then it returns the request
 * header user name if the remote user matches the proxy user, or the remote user otherwise. An
 * optional regular expression defining how to convert the header to a user ID can be configured
 * using {@link #setUserIdPattern(String)}. This allows for the secure proxying of requests from a
 * Surf client such as Alfresco Share using SSL client certificates.
 *
 * @author dward
 */
public class DefaultRemoteUserMapper implements RemoteUserMapper, ActivateableBean {
    ...
    /**
     * Sets the name of the remote user used to 'proxy' requests securely in the name of another
     * user. Typically this remote identity will be protected by an SSL client certificate.
     *
     * @param proxyUserName the proxy user name. If <code>null</code> or empty, then the header will
     *     be checked regardless of remote user identity.
     */
    public void setProxyUserName(String proxyUserName) {
        this.proxyUserName =
                proxyUserName == null || proxyUserName.length() == 0 ? null : proxyUserName;
    }

    /**
     * Sets the name of the header containing the ID of a proxied user.
     *
     * @param proxyHeader the proxy header name
     */
    ...
    /**
     * Controls whether the mapper is enabled. When disabled {@link
     * #getRemoteUser(HttpServletRequest)} will always return <code>null</code>
     *
     * @param isEnabled Is this mapper enabled?
     */
    ...
    /**
     * Sets a regular expression for extracting a user ID from the header. If this is not set, then
     * the entire contents of the header will be used as the user ID.
     *
     * @param userIdPattern the regular expression
     */
    public void setUserIdPattern(String userIdPattern) {
        this.userIdPattern =
                userIdPattern == null || userIdPattern.length() == 0
                        ? null
                        : Pattern.compile(userIdPattern);
    }
    ...
    public String getRemoteUser(HttpServletRequest request) {
        ...
        if (this.proxyUserName == null) {
            ...
            String normalizedUserId =
                    normalizeUserId(headerUserId != null ? headerUserId : remoteUserId);
            ...
            if (request.getScheme().toLowerCase().equals("https")
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "Checking SSL certificate subject DN to match " + this.proxyUserName);
                }
    ...
    /**
     * Extracts a user ID from the proxy header. If a user ID pattern has been configured returns
     * the contents of the first matching regular expression group or <code>null</code>. Otherwise
     * returns the trimmed header contents or <code>null</code>.
     *
    ...
    private String extractUserFromProxyHeader(HttpServletRequest request) {
        ...
        try {
            if (userId != null && isEncode) {
                userId =
                        new String(
                                org.apache.commons.codec.binary.Base64.decodeBase64(userId),
                                "UTF-8");
            ...
            if (matcher.matches()) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "userId '"
                                    + AuthenticationUtil.maskUsername(userId)
                                    + "' did not match the userIdPattern '"
                                    + this.userIdPattern
                                    + "'. Returning null.");
                }
                return null;

...
/**
 * This class is responsible for placing a rendition node in the correct location given a temporary
 * rendition, a source node, a rendition location and optionally an old rendition. This manages the
 * complex logic of deciding whether to move an old rendition or orphan it and create a new one
...
public class RenditionNodeManager {
    ...
    private static final List<QName> unchangedProperties =
            Arrays.asList(PROP_NODE_REF, PROP_STORE_NAME);
    ...
    /**
     * This holds an existing rendition node if one exists and is linked to the source node by a
     * correctly named rendition association.
     */
    ...
    public RenditionNodeManager(
        ...
        if (logger.isDebugEnabled()) {
            ...
            msg.append("Creating/updating rendition based on:")
                    .append(LINE_BREAK)
                    .append("    sourceNode: ")
                    .append(sourceNode)
                    .append(LINE_BREAK)
                    .append("    tempRendition: ")
                    .append(tempRenditionNode)
                    .append(LINE_BREAK)
                    .append("    parentNode: ")
                    .append(location.getParentRef())
                    .append(LINE_BREAK)
                    .append("    childNode: ")
                    .append(location.getChildRef())
                    .append(LINE_BREAK)
                    .append("    childName: ")
                    .append(location.getChildName())
                    .append(LINE_BREAK)
                    .append("    renditionDefinition.name: ")
                    .append(renditionDefinition.getRenditionName());
            logger.debug(msg.toString());
        }
    }

    /**
     * This method returns the {@link ChildAssociationRef} for the rendition node. In doing this it
     * may reuse an existing rendition node, move an existing rendition node or create a new
     * rendition node as appropriate.
     *
     * @return the primary parent association of the rendition node, which may not be the rendition
     *     association.
     */
    ...
    /**
     * This method moves the old rendition to the required location giving it the correct
     * parent-assoc type and the specified association name.
     *
    ...
    private ChildAssociationRef moveOldRendition(QName renditionName) {
        ...
        QName assocType =
                sourceNode.equals(parent)
                        ? RenditionModel.ASSOC_RENDITION
                        : ContentModel.ASSOC_CONTAINS;
        ChildAssociationRef result =
                nodeService.moveNode(existingLinkedRendition, parent, assocType, assocName);
    ...
    /**
     * This method performs the 'orphaning' of the oldRendition. It removes the rendition aspect(s)
     * and removes the child-association linking the old rendition to its source node. The old
     * rendition node is not deleted.
     *
     * @param renditionName the name of the rendition.
     * @throws RenditionServiceException if there was not exactly one parent assoc from the
     *     oldRendition having the specified renditionName or if the matching parent assoc was not
     *     to the correct source node.
     */
    private void orphanOldRendition(QNamePattern renditionName) {
        ...
        List<ChildAssociationRef> parents =
                nodeService.getParentAssocs(
                        existingLinkedRendition, RenditionModel.ASSOC_RENDITION, renditionName);
        ...
        if (parents.size() == 1) {
            ...
            if (parentAssoc.getParentRef().equals(sourceNode)) {
                ...
                behaviourFilter.disableBehaviour(
                        existingLinkedRendition, ContentModel.ASPECT_AUDITABLE);
                behaviourFilter.disableBehaviour(sourceNode, ContentModel.ASPECT_AUDITABLE);
                try {
                    nodeService.removeAspect(
                            existingLinkedRendition, RenditionModel.ASPECT_HIDDEN_RENDITION);
                    nodeService.removeAspect(
                            existingLinkedRendition, RenditionModel.ASPECT_VISIBLE_RENDITION);
        ...
        String msg =
                "Node: "
                        + existingLinkedRendition
                        + " is not a rendition of type: "
                        + renditionName
                        + " for source node: "
                        + sourceNode;
        if (logger.isDebugEnabled()) {
    ...
    private boolean isOrphaningRequiredWithoutLog() {
        ...
        // existing rendition' param is set to true or the RenditionLocation specifies a destination
        // NodeRef.
        if (location.getChildRef() != null) return true;
        else
            return AbstractRenderingEngine.getParamWithDefault(
                    RenditionService.PARAM_ORPHAN_EXISTING_RENDITION,
                    Boolean.FALSE,
                    renditionDefinition);
    }
    ...
    private boolean isOldRenditionInCorrectLocationWithoutLog() {
        ...
        if (oldParent.equals(location.getParentRef())) {
            ...
            if (childName == null) return true;
            else {
                Serializable oldName =
                        nodeService.getProperty(existingLinkedRendition, ContentModel.PROP_NAME);
    ...
    private void checkDestinationNodeIsAcceptable(NodeRef destination) {
        ...
        if (!renditionService.isRendition(destination)) {
            throw new RenditionServiceException(
                    "Cannot perform a rendition to an existing node that is not a rendition.");
        }
        ChildAssociationRef sourceAssoc = renditionService.getSourceNode(destination);
        if (!sourceAssoc.getParentRef().equals(sourceNode)) {
            throw new RenditionServiceException(
                    "Cannot perform a rendition to an existing rendition node whose source is"
                            + " different.");
        }
        if (!sourceAssoc.getQName().equals(renditionDefinition.getRenditionName())) {
            throw new RenditionServiceException(
                    "Cannot perform a rendition to an existing rendition node whose rendition name"
                            + " is different.");
        }
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Using destination node "
                            + destination
                            + " with existing srcNode: "
                            + sourceAssoc);
        }
    }

    /**
     * This method creates a new rendition node. If the source node for this rendition is not the
     * primary parent of the newly created rendition node, the rendition node is added as a child of
     * the source node.
     *
        ...
        try {
            ...
            primaryAssoc =
                    nodeService.createNode(
                            parentRef, assocTypeQName, assocName, nodeTypeQName, indexProps);
        } finally {
    ...
    private QName getAssociationName(boolean parentIsSource, QName renditionName) {
        // If the parent is not the source node and the location has a child name then use that
        // name.
        QName assocName = renditionName;
        String childName = location.getChildName();
        if (parentIsSource == false && childName != null && childName.length() > 0) {
    ...
    /**
     * This method returns the rendition on the given sourceNode with the given renditionDefinition,
     * if such a rendition exists.
     *
    ...
    private NodeRef getExistingRendition() {
        ...
        ChildAssociationRef renditionAssoc =
                renditionService.getRenditionByName(sourceNode, renditionName);
    ...
    /**
     * This method copies properties from the temporary rendition node onto the targetNode. It also
     * sets the node type. {@link #unchangedProperties Some properties} are not copied.
     *
    ...
    private void transferNodeProperties(ChildAssociationRef finalRenditionAssoc) {
        ...
        if (logger.isDebugEnabled()) {
            StringBuilder msg = new StringBuilder();
            msg.append("Transferring some properties from ")
                    .append(tempRenditionNode)
                    .append(" to ")
                    .append(targetNode);
            logger.debug(msg.toString());
        ...
        for (Entry<QName, Serializable> entry :
                nodeService.getProperties(tempRenditionNode).entrySet()) {

...
/**
 * A simplistic policy that generates all applicable thumbnails for content as it is added or
 * updated. As this is done synchronously, this is not recommended for production use.
 *
 * @author dward
 ...
 */
@Deprecated
public class SimpleThumbnailer extends TransactionListenerAdapter
    ...
    private static final String KEY_POST_TXN_NODES_TO_THUMBNAIL =
            "SimpleThumbnailer.KEY_POST_TXN_NODES_TO_THUMBNAIL";
     ...
     * @param newContent is the content new?
     */
    public void onContentUpdate(NodeRef nodeRef, boolean newContent) {
        if (!this.nodeService.getType(nodeRef).equals(ContentModel.TYPE_THUMBNAIL)
            ...
            // Bind this service to the transaction and add the node to the set of nodes to
            // thumbnail post txn
                ...
                List<ThumbnailDefinition> thumbnailDefinitions =
                        this.thumbnailService
                                ...
                                .getThumbnailDefinitions(
                                        contentData.getMimetype(), contentData.getSize());
                for (final ThumbnailDefinition thumbnailDefinition : thumbnailDefinitions) {
                    final NodeRef existingThumbnail =
                            this.thumbnailService.getThumbnailByName(
                                    nodeRef,
                                    ContentModel.PROP_CONTENT,
                                    thumbnailDefinition.getName());
                    try {
                        // Generate each thumbnail in its own transaction, so that we can recover if
                                            ...
                                            public Object execute() throws Throwable {
                                                if (existingThumbnail == null) {
                                                    if (SimpleThumbnailer.logger.isDebugEnabled()) {
                                                        SimpleThumbnailer.logger.debug(
                                                                "Creating thumbnail \""
                                                                        + thumbnailDefinition
                                                                                .getName()
                                                                        + "\" for node "
                                                    ...
                                                    SimpleThumbnailer.this.thumbnailService
                                                            .createThumbnail(
                                                                    nodeRef,
                                                                    ContentModel.PROP_CONTENT,
                                                                    thumbnailDefinition
    ...
    private Set<NodeRef> getPostTxnNodesToThumbnail() {
        ...
        if (nodesToThumbnail == null) {
            ...
            AlfrescoTransactionSupport.bindResource(
                    SimpleThumbnailer.KEY_POST_TXN_NODES_TO_THUMBNAIL, nodesToThumbnail);
        }

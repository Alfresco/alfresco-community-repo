...
public interface ContentServicePolicies
{
    ...
    /**
     * Policy that is raised for each content property change. Any policy implementations must be aware that the transaction in which this is called could still roll back; no filesystem changes should occur against the source content until after the transaction has <u>successfully</u> completed.
     * 
     * @since 3.2
     */
    @AlfrescoPublicApi
    public interface OnContentPropertyUpdatePolicy extends ClassPolicy
    {
         ...
         * @param beforeValue
         *            the value of the content data prior to the change. Note that this value may be <tt>null</tt> or any of it's member values may be <tt>null</tt> according to the contract of the {@link ContentData} class.
         * @param afterValue
         *            the value of the content data after the change
         * 
    ...
    {
        static final QName QNAME = QName.createQName(NamespaceService.ALFRESCO_URI, "onContentDownload");

        /**
         * @param nodeRef
         *            the node reference
         */
        void onContentDownload(NodeRef nodeRef);
    }

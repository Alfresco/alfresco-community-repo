 ...
 *
 * <p>Responsible for providing remote HTTP based access to a store. Designed to be accessed from a
 * web-tier application to remotely mirror a WebScript Store instance.
 *
 * <p>Request format:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>optional request parameters:
 *
 * <pre>
 *      s                    -> the store id
 * </pre>
 *
 * <p>Note: path is relative to the root path as configured for this webscript bean
 *
 * <p>Further URL arguments may be provided if required by specific API methods.
 *
 * <p>For content create and update the request should be POSTed and the content sent as the payload
 * of the request content.
 *
 * <p>Supported API methods:
 *
 * <pre>
...
public abstract class BaseRemoteStore extends AbstractWebScript {
    ...
    public void execute(WebScriptRequest req, WebScriptResponse res) throws IOException {
        ...
        if (webScriptServletRequest == null) {
            throw new WebScriptException(
                    "Remote Store access must be executed in HTTP Servlet environment");
        }
        ...
        if (store == null) {
            ...
            if (store == null) {
                ...
                // this means that a store was not passed in and that we also didn't have a
                // configured store
                throw new WebScriptException(
                        "Unable to determine which store to operate against."
        ...
        try {
            ...
            throw new WebScriptException(
                    "Unknown method specified to remote store API: " + methodName);
     ...
     */
    protected abstract void listDocuments(
            WebScriptResponse res, String store, String path, boolean recurse) throws IOException;
     ...
     */
    protected abstract void listDocuments(
            WebScriptResponse res, String store, String path, String pattern) throws IOException;
    ...
    protected abstract void createDocument(
            WebScriptResponse res, String store, String path, InputStream content);
     ...
     * @param content content of the document to write
     */
    protected abstract void createDocuments(
            WebScriptResponse res, String store, InputStream content);

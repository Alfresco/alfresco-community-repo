...
public abstract class AbstractTransformationRenderingEngine extends AbstractRenderingEngine {
    ...
    /**
     * This optional {@link Long} parameter specifies the timeout for reading the source before an
     * exception is thrown.
     */
    public static final String PARAM_TIMEOUT_MS = TransformationOptionLimits.OPT_TIMEOUT_MS;

    /**
     * This optional {@link Long} parameter specifies how timeout for reading the source before EOF
     * is returned.
     */
    public static final String PARAM_READ_LIMIT_TIME_MS =
            TransformationOptionLimits.OPT_READ_LIMIT_TIME_MS;

    /**
     * This optional {@link Long} parameter specifies the maximum number of kbytes of the source may
     * be read. An exception is thrown before any are read if larger.
     */
    public static final String PARAM_MAX_SOURCE_SIZE_K_BYTES =
            TransformationOptionLimits.OPT_MAX_SOURCE_SIZE_K_BYTES;

    /**
     * This optional {@link Long} parameter specifies how many kbytes of the source to read in order
     * to create an image.
     */
    public static final String PARAM_READ_LIMIT_K_BYTES =
            TransformationOptionLimits.OPT_READ_LIMIT_K_BYTES;

    /**
     * This optional {@link Integer} parameter specifies the maximum number of pages of the source
     * that may be read. An exception is thrown before any are read if larger.
     */
    public static final String PARAM_MAX_PAGES = TransformationOptionLimits.OPT_MAX_PAGES;

    /**
     * This optional {@link Integer} parameter specifies how many source pages should be read in
     * order to create an image.
     */
    public static final String PARAM_PAGE_LIMIT = TransformationOptionLimits.OPT_PAGE_LIMIT;

    /**
     * The frequency in milliseconds with which the {@link ActionTrackingService} should be polled
     * for cancellation of the action.
     */
    ...
    private static final String NOT_TRANSFORMABLE_MESSAGE_PATTERN =
            TRANSFORMER_NOT_EXISTS_MESSAGE_PATTERN;
    private static final String TRANSFORMING_ERROR_MESSAGE =
            RenditionService2Impl.TRANSFORMING_ERROR_MESSAGE;
    ...
    public void setSourceOptionsSerializers(
            Collection<TransformationSourceOptionsSerializer> sourceOptionsSerializers) {
     ...
     * AbstractTransformationRenderingEngine#init()}.
     *
    ...
    public void setSynchronousTransformClient(
    ...
    protected void render(RenderingContext context) {
        ...
        // There will have been an exception if there is no content data so contentReader is not
        // null.
        ...
        try {
            ...
            if (logger.isInfoEnabled()) {
                logger.info(
                        "Cancelling of multiple concurrent action instances "
        ...
        FutureTask<ContentWriter> transformTask =
                new FutureTask<ContentWriter>(
                        new TransformationCallable(
                                contentReader,
                                targetMimeType,
        ...
        while (true) {
            try {
                ...
                if (transformationOptions.getTimeoutMs() > 0
                    ...
                    // We hit a timeout, let the transform thread continue but results will be
                    // ignored
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "Transformation did not obey timeout limit, "
        ...
        if (actionCompleted) {
            ...
            try {
                ...
                throw new RenditionServiceException(
                        TRANSFORMING_ERROR_MESSAGE + e.getCause().getMessage(), e.getCause());
            } catch (InterruptedException e) {
    ...
    protected TransformationOptions getTransformOptionsImpl(
        ...
        Long maxSourceSizeKBytes =
                context.getCheckedParam(PARAM_MAX_SOURCE_SIZE_K_BYTES, Long.class);
        ...
        if (getSourceOptionsSerializers() != null) {
            for (TransformationSourceOptionsSerializer sourceSerializer :
    ...
    /**
     * Implementation of <code>Callable</code> for doing the work of the transformation which
     * returns the temporary content writer if successful.
     */
    protected class TransformationCallable implements Callable<ContentWriter> {
        ...
        public ContentWriter call() throws Exception {
            ...
            Serializable runAsParam =
                    context.getDefinition().getParameterValue(AbstractRenderingEngine.PARAM_RUN_AS);
            String runAsName =
                    runAsParam == null
                            ? AbstractRenderingEngine.DEFAULT_RUN_AS_NAME
                            : (String) runAsParam;

            return AuthenticationUtil.runAs(
                    new AuthenticationUtil.RunAsWork<ContentWriter>() {
                        @Override
                        public ContentWriter doWork() throws Exception {
                            // ALF-15715: Use temporary write to avoid operating on the real node
                            // for fear of row locking while long transforms are in progress
                            ...
                            try {
                                ...
                                return tempContentWriter;
                                    ...
                                    | UnsupportedTransformationException ntx) {
                                {
                                    logger.debug(
                                            "No transformer found to execute rule: \n"
                                                    + "   reader: "
                                                    + contentReader
                                                    + "\n"
                                                    + "   writer: "
                                ...
                                throw new RenditionServiceException(
                                        TRANSFORMING_ERROR_MESSAGE + ntx.getMessage(), ntx);
                            }
                        }
                        ;
                    },
